...

{
  ...
  check(
    'name "Nav Link" → element "a", with a named inference note',
    link.semantics.element === "a" &&
      link.notes.some((n) => n.includes("inferred from the set name")),
  );
  const tooltip = semanticsOf("Tooltip");
  check(
    'name "Tooltip" → element "div" + role "tooltip"',
    tooltip.semantics.element === "div" && tooltip.semantics.role === "tooltip",
  );
  const chip = semanticsOf("Chip");
  check(
    'no table match ("Chip") → element stays "div" with the existing hedge note',
    chip.semantics.element === "div" &&
      chip.semantics.role === undefined &&
      chip.notes.some((n) =>
        n.includes('semantics.element defaulted to "div"'),
      ),
  );
...
const dump = read(
  path.join(
    ...
    "figma",
    "fixtures",
    "cbds-button-brand-primary.rest-dump.json",
  ),
);
...
check(
  ...
  c.semantics.element === "button" &&
    real.notes.some(
      (n) => n.includes('element "button"') && n.includes("inferred"),
    ),
);
...
check(
  ...
  JSON.stringify(c.states) ===
    JSON.stringify(["hover", "active", "focus-visible", "disabled"]),
);
check(
  "hover/active/disabled carry background overrides (hover #003e81, pressed→active #002854, disabled #dfe3eb through minted refs)",
  ["hover", "active", "disabled"].every(
    (s) => typeof rootStates[s]?.["background-color"] === "string",
  ),
);
check(
...
check(
  ...
  real.notes.some((n) =>
    n.includes('RENAME relative to the imported axis "state"'),
  ),
);
...
for (const size of ["large", "medium", "small"] as const) {
  ...
  const fontSize = resolve(
    ((c.anatomy as J).root.parts?.Button?.tokens ?? {})["font-size"],
    size,
  );
  check(
...
check(
  ...
  resolve(rootTokens["padding-inline"], "small") !==
    resolve(rootTokens["padding-inline"], "large") &&
    resolve(
      ((c.anatomy as J).root.parts?.Button?.tokens ?? {})["font-size"],
      "small",
    ) !==
...
check(
  ...
  withoutStub.includes("ds.icon") &&
    withoutStub.includes("no contract in scope"),
);
...
check(
  "emitReact: props extend ButtonHTMLAttributes<HTMLButtonElement>",
  tsx.includes("ButtonHTMLAttributes<HTMLButtonElement>"),
);
check(
  "emitReact: NATIVE disabled attribute (disabled={disabled})",
  tsx.includes("disabled={disabled}"),
);
check(
...
check(
  "emitReact CSS: real :hover rule",
  reactCss.includes(":hover:not(:disabled)"),
);
check(
  "emitReact CSS: real :active rule (pressed promoted)",
  reactCss.includes(":active:not(:disabled)"),
);
check(
  "emitReact CSS: real :focus-visible outline",
  reactCss.includes(":focus-visible"),
);
...
check(
  "emitHtml: renders <button — the preview is genuinely focusable",
  html.includes("<button"),
);
check(
  "emitHtml CSS: :hover and :focus-visible rules present (the preview hovers and shows a ring on Tab)",
  htmlCss.includes(":hover:not(:disabled)") &&
    htmlCss.includes(":focus-visible"),
);
