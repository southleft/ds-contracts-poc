...
const readDump = (name: string) =>
  JSON.parse(
    readFileSync(path.join(ROOT, "extract", "figma", "fixtures", name), "utf8"),
...
for (const [name, slug] of SLUG_CASES) {
  check(
    `componentIdSlug(${JSON.stringify(name)}) = "${slug}"`,
    componentIdSlug(name) === slug,
  );
}
...
const underscore = proposeFromDump(
  allDump["_variable-list-item"] as DumpSet,
  opts,
);
check(
...
check(
  ...
  underscore.notes.some(
    (n) =>
      n.includes('drawn set name "_variable-list-item"') &&
      n.includes('"ds.variable-list-item"') &&
      n.includes("illegal characters → hyphens"),
  ),
);
const template = proposeFromDump(
  allDump["Button / Primary / Medium"] as DumpSet,
  opts,
);
check(
...
check(
  ...
  template.notes.some(
    (n) =>
      n.includes('"Button / Primary / Medium"') &&
      n.includes('"ds.button-primary-medium"'),
  ),
);
...
check(
  "scoped dump: 1 proposed, 0 skipped",
  scopedBatch.proposals.length === 1 && scopedBatch.skipped.length === 0,
);
const scoped = scopedBatch.proposals[0];
const scopedContract = scoped.contract as {
  id?: string;
...
check(
  ...
  scopedContract.id === "ds.button-brand-primary",
);
check(
  'semantics.element "button"',
  scopedContract.semantics?.element === "button",
);
check(
  "states promoted (hover/active/focus-visible present)",
  ["hover", "active", "focus-visible"].every((s) =>
    scopedContract.states?.includes(s),
  ),
);
check(
  "ds.icon child stub proposed alongside",
  (scoped.childStubs ?? []).some(
    (s) => (s as { id?: string }).id === "ds.icon",
  ),
);
...
const total = Object.entries(allDump).filter(
  ([k, v]) =>
    k !== "_provenance" &&
    !!v &&
    typeof v === "object" &&
    "variants" in (v as object),
).length;
console.log(
  `  (replayed ${total} sets: ${batch.proposals.length} proposed, ${batch.skipped.length} skipped)`,
);
check(
...
const poisoned = JSON.parse(JSON.stringify(scopedDump)) as Record<
  string,
  unknown
>;
...
check(
  ...
  poisonedBatch.proposals.some((p) => p.setName === "Button-Brand Primary"),
);
check(
  "the poisoned set is a NAMED skip",
  poisonedBatch.skipped.length === 1 &&
    poisonedBatch.skipped[0].setName === "Poisoned",
);
const skip = poisonedBatch.skipped[0];
check(
  'the skip reason is plain words ("Set "Poisoned" could not be proposed: …"), not machine output',
  skip !== undefined &&
    skip.reason.startsWith('Set "Poisoned" could not be proposed:') &&
    !/[[{]/.test(skip.reason.charAt(0)),
);
...
check(
  ...
  rendered.headline.startsWith(
    "the proposed contract did not fit the contract schema",
  ),
);
check(
  "the raw zod text survives as expandable detail, not the headline",
  typeof rendered.detail === "string" && /"code"/.test(rendered.detail),
);
check(
  "the headline itself is never machine-looking",
  !/^\s*[[{]/.test(rendered.headline),
);
