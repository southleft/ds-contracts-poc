...
const opts = {
  corpus,
  contractIdByName: new Map([["Button", "ds.button"]]),
  fileKey: null,
...
function componentRefs(anatomy: J): string[] {
  ...
  const visit = (part: J) => {
    ...
    for (const child of Object.values((part.parts as J) ?? {}))
      visit(child as J);
  };
...
const realSet = read(
  path.join("extract", "figma", "fixtures", "eventz-button.rest-dump.json"),
...
check(
  "no component ref anywhere in the anatomy (no ds.button self-reference, no cycle)",
  componentRefs(rc.anatomy as J).length === 0,
);
check(
  'no "Button" wrapper part survives (the 4 focus+disabled base instances flattened)',
  rParts.Button === undefined,
);

// 2. Axes intact — variant × state × isDisabled.
check(
  "axis `variant` proposed as an enum [primary, knockout, secondary, bare]",
  JSON.stringify(rByName.get("variant")?.type) ===
    JSON.stringify({ enum: ["primary", "knockout", "secondary", "bare"] }),
);
check(
...
check(
  'no mangled spelling ("hasendicon"/"starticon") anywhere in the proposal',
  !JSON.stringify(rc).includes("hasendicon") &&
    !JSON.stringify(rc).includes('"starticon"'),
);
...
check(
  ...
  rText !== undefined &&
    rText.type === "text" &&
    rText.default === "Label" &&
    rText.bindings.figma.kind === "TEXT",
);
check(
  "no extra props invented (2 axes + text + 2 icon booleans; state promoted away)",
  rProps.length === 5,
);
...
check(
  ...
  rParts.startIcon?.slot?.name === "startIcon" &&
    rParts.startIcon?.visibleWhen?.prop === "hasStartIcon",
);
check(
  'endIcon slot part: slot name "endIcon", visibleWhen hasEndIcon',
  rParts.endIcon?.slot?.name === "endIcon" &&
    rParts.endIcon?.visibleWhen?.prop === "hasEndIcon",
);
check(
  "Label part binds content to the `text` prop",
  rParts.Label?.content?.prop === "text",
);
...
check(
  ...
  rStates["focus-visible"]?.["outline-color"] ===
    "{imported.button.state-focus-visible.outline-color}" &&
    rStates["focus-visible"]?.["outline-width"] ===
      "{imported.button.state-focus-visible.outline-width}" &&
...
check(
  ...
  rRootTokens["background-color"] ===
    "{imported.button.root.background-color.{variant}}",
);
check(
  "hover/active state overrides substitute {variant} (per-(variant×state) capture, promoted)",
  rStates.hover?.["background-color"] ===
    "{imported.button.state-hover.background-color.{variant}}" &&
    rStates.active?.["background-color"] ===
      "{imported.button.state-active.background-color.{variant}}",
);
...
check(
  `minted layer is not empty (${real.mintedTokens?.count ?? 0} leaves ≥ 25)`,
  (real.mintedTokens?.count ?? 0) >= 25,
);
...
const rBorderColorPlane = (
  (real.mintedTokens?.tree as J | undefined)?.imported as J
)?.button?.root?.["border-color"] as
...
check(
  ...
  rRootTokens["border-color"] ===
    "{imported.button.root.border-color.{variant}}" &&
...
check(
  "no unbound entry survives on the real dump (the stroke was the last one)",
  real.unbound.length === 0,
);
...
check(
  ...
  real.notes.some(
    (n) =>
      n.includes(
        "4 state-axis variant(s) wrapped an instance of the set's own base component",
      ) && n.includes("FILL defaults"),
  ),
);
check(
  "boolean defaults adopted from the base instance are named",
  ["hasStartIcon", "hasEndIcon"].every((n) =>
    real.notes.some((note) =>
      note.includes(
        `prop \`${n}\`: default true adopted from the base instance "Button"`,
      ),
    ),
  ),
);
...
check(
  ...
  rCss.tsxCss.includes(".variant-primary:hover:not(:disabled)") &&
    rCss.tsxCss.includes(
      "var(--imported-button-state-hover-background-color-primary)",
    ),
);
check(
...
check(
  ...
  realClassic.unbound.length > 0 &&
    realClassic.unbound.some((u) => u.property === "padding"),
);
...
const set = read(
  path.join("extract", "figma", "fixtures", "base-instance-dump.json"),
).Button as unknown as DumpSet;
...
check(
  "no component ref anywhere in the anatomy (no ds.button self-reference)",
  refs.length === 0,
);
...
check(
  'no mangled spelling ("hasendicon") anywhere in the proposal',
  !JSON.stringify(contract).includes("hasendicon"),
);
...
check(
  ...
  JSON.stringify(byName.get("variant")?.type) ===
    JSON.stringify({ enum: ["primary", "secondary"] }),
);
check(
  ...
  byName.get("state") === undefined &&
    flat.notes.some((n) =>
      n.includes(
        'state "hover": promoted from the axis but no root or part override was recoverable',
      ),
    ),
);
...
check(
  ...
  isDisabled !== undefined &&
    ...
    isDisabled.bindings.figma.property === "isDisabled",
);
...
check(
  ...
  ["hasEndIcon", "hasStartIcon", "label"].every((n) =>
    flat.notes.some(
      (note) =>
        note.includes(`prop \`${n}\``) &&
        note.includes('promoted from the base instance "Button"'),
    ),
  ),
);
check(
  ...
  flat.notes.some(
    (n) =>
      n.includes("wrap an instance of the set's own base component") &&
      n.includes("dump v1 stops at instances"),
  ),
);
...
check(
  ...
  ((contract.anatomy as J).root.tokens ?? {})["border-radius"] ===
    "{imported.button.root.border-radius}",
);
check(
  "no unbound value survives (the single literal is fully minted)",
  flat.unbound.length === 0,
);
const css = generates("flattened proposal", flat);
check(
  "emitted css references the minted custom property",
  css.tsxCss.includes("var(--imported-button-root-border-radius)"),
);
...
check(
  ...
  classic.unbound.length === 1 &&
    classic.unbound[0].property === "cornerRadius" &&
    classic.unbound[0].value === 8,
);
...
const v1 = clone(set);
for (const variant of v1.variants)
  for (const child of variant.children ?? [])
    delete (child as DumpNode).componentProperties;
const fallbackA = proposeFromDump(v1, { ...opts, mintUnbound: true });
const aContract = fallbackA.contract as J;
check(
  "still no component ref (self-reference guard holds without flattening)",
  componentRefs(aContract.anatomy as J).length === 0,
);
check(
  "nothing promoted — props are the axes alone (state axis promoted away, not a prop)",
  (aContract.props as J[]).length === 2 &&
    !(aContract.props as J[]).some((p) =>
      ["hasEndIcon", "hasStartIcon", "label", "state"].includes(p.name),
    ),
);
check(
...
check(
  "still no component ref (the instance dissolved, never referenced)",
  componentRefs(bContract.anatomy as J).length === 0,
);
check(
  "flattening fires with siblings present — booleans + text still promoted",
  ["hasEndIcon", "hasStartIcon", "label"].every((n) =>
    (bContract.props as J[]).some((p) => p.name === n),
  ),
);
check(
  "the sibling survives as its own part (caption), the instance leaves no part behind",
  (bContract.anatomy as J).root.parts?.caption !== undefined &&
    (bContract.anatomy as J).root.parts?.Button === undefined,
);
...
const wanted =
  "component ref creates a cycle (ds.button → ds.button) — a contract cannot compose itself";
const reactMsg = refusal(() => emitReact(selfRefContract, selfCtx));
check(
  "emitReact REFUSES the direct self-ref by name (no stack overflow)",
  reactMsg.includes(wanted),
);
const htmlMsg = refusal(() => emitHtml(selfRefContract, selfCtx));
check(
  "emitHtml REFUSES the direct self-ref by name (routes through validateContract)",
  htmlMsg.includes(wanted),
);
...
check(
  ...
  transMsg.includes(
    "creates a cycle (ds.alpha → ds.beta → ds.alpha) — a contract cannot compose itself",
  ),
);
