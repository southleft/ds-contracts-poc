...
console.log(
  "Part-level state overrides (P18 v13 — the B7 retirement), owner Button replay",
);
...
check(
  ...
  proposal.notes.some(
    (n) =>
      n.startsWith(
        'Button-Brand Primary:root/Button: state "disabled" part-level override proposed',
      ) &&
...
check(
  ...
  ["hover", "active", "focus-visible"].every(
    (s) => labelPart?.states?.[s] === undefined,
  ),
);
check(
  "root states still carry their own channels (disabled background {bg.disabled})",
  contract.anatomy.root.states?.["disabled"]?.["background-color"] ===
    "{bg.disabled}",
);
check(
...
const refuse = (mutate: (c: Contract) => void): string[] => {
  ...
  validateContract(
    clone,
    new Map([...contracts, [clone.id, clone]]),
    errors,
    new Map(),
  );
  return errors;
...
check(
  ...
  refuse((c) => {
    c.anatomy.root.parts!["Button"]!.states = {
      sparkle: { color: "{text.disabled}" },
    };
...
check(
  ...
  refuse((c) => {
    ...
    e.includes('declares "hover" but the contract\'s `states` does not'),
  ),
);
check(
  ...
  refuse((c) => {
    c.anatomy.root.parts!["Button"]!.states = {
      disabled: { "font-size": "{text.disabled}" },
    };
...
check(
  ...
  refuse((c) => {
    c.anatomy.root.parts!["Icon"]!.states = {
      disabled: { color: "{text.disabled}" },
    };
  }).some((e) =>
    e.includes("is a component instance — states cannot restyle it"),
  ),
);
...
check(
  `generateCss: zero violations over the layered inventory (got ${cssErrors.length})`,
  cssErrors.length === 0,
);
check(
...
check(
  ...
  inlineTsx.includes(
    "PART-level state overrides (Part.states, v13) are omitted",
  ),
);
...
const disabledCells = (data.stateVariants ?? []).filter((v) =>
  v.name.includes("State=Disabled"),
);
check(
  `the State axis compiles Disabled preview cells (got ${disabledCells.length})`,
...
check(
  ...
  disabledCells.length > 0 &&
    disabledCells.every(
      (v) =>
        findText(v.spec)?.textFill === "text/disabled" &&
        v.spec.fill === "bg/disabled",
    ),
);
