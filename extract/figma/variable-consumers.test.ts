...
test('hidden text captures its actual line-height binding and inherited consuming modes', async () => {
  ...
  assert.equal(dump._provenance.dumpVersion, '1.42');
...
test('aliases retain the selected raw edge and consumer-resolved value without claiming alias-graph proof', async () => {
  const f = fixture();
  ...
  const rows = f.labels(await f.capture()).map(n => n.variableConsumers![f.line.id]);
...
test('REST native line-height bindings agree with plugin precedence without inventing consuming modes', () => {
  ...
  for (const kind of ['native', 'conflict', 'mixed', 'missing', 'malformed', 'empty', 'unbound'] as const) {
    ...
    assert.equal(REST_DUMP_VERSION, '1.41');
