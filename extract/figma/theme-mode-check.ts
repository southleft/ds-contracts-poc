...
const themedVariant = (
  theme: string,
  variant: string,
  opts?: { accentHex?: string; extraChild?: boolean; textVar?: string },
): DumpNode => ({
  ...
  children: [
    {
      ...
      type: "FRAME",
      fill: {
        hex: opts?.accentHex ?? (theme === "light" ? "1e62d0" : "9ec2ff"),
      },
      layout: {
        mode: "HORIZONTAL",
        primary: "CENTER",
    ...
    },
    {
      ...
      type: "TEXT",
      text: {
        characters: "Something happened",
        fontSize: 14,
        fontStyle: "Regular",
        fillVar: opts?.textVar ?? "text/primary",
      },
    },
    ...(opts?.extraChild
...
const themedDump = {
  ...
  _variables: {
    "bg/info": {
      type: "COLOR",
      value: "#eef4ff",
      modes: { light: "#eef4ff", dark: "#0b1d3a" },
    },
    "bg/error": {
      type: "COLOR",
      value: "#fdeeee",
      modes: { light: "#fdeeee", dark: "#3a0b0b" },
    },
    "text/primary": {
      type: "COLOR",
      value: "#1b1f26",
      modes: { light: "#1b1f26", dark: "#fcfeff" },
    },
  },
  "Alert Banner": {
...
function replay(dump: Record<string, unknown>): Replay {
  ...
  for (const emitter of emitters) {
    try {
      ...
      refusals.push(
        `${emitter.name}: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
  }
...
console.log(
  "1. corroborated Theme=light|dark axis (CBDS-shaped synthetic, dump v1.6)",
);
{
  const r = replay(themedDump as unknown as Record<string, unknown>);
  check(
    "NO `theme` prop ships in the API",
    !r.contract.props.some((p) => p.name === "theme"),
  );
  check(
    `the API keeps the real axis (props: ${r.contract.props.map((p) => p.name).join(", ")})`,
  ...
  const note = r.proposal.notes.find(
    (n) =>
      n.includes(
        'variant axis "theme" (light|dark) IS a token-mode axis, not API (§3 — structurally corroborated',
      ) &&
      n.includes(
        'anatomy and facts build from the 2 "light" (default-mode) variant(s) only',
      ) &&
  ...
  check(
    `base facts bind the REAL variable names from the light variants (background-color = {bg.{variant}}; got ${String(rootTokens["background-color"])})`,
    rootTokens["background-color"] === "{bg.{variant}}",
  );
  const mintedJson = JSON.stringify(r.proposal.mintedTokens ?? {});
  check(
    "the LIGHT accent literal mints (#1e62d0 — the default mode feeds the mint pass)",
    mintedJson.includes("#1e62d0"),
  );
  check(
    "the DARK accent literal mints NOWHERE (#9ec2ff — mode-excluded variants never fabricate a second palette)",
    !mintedJson.includes("#9ec2ff"),
  );
  check(
...
);
{
  const layer = capturedTokensFromDump(
    themedDump as unknown as Record<string, unknown>,
  )!;
  check(
    `3 variables captured, 3 registrable (got ${layer.count})`,
    layer.count === 3,
  );
  check(
    "the layer carries per-mode trees (modes: light, dark)",
    layer.modes !== undefined &&
      JSON.stringify(Object.keys(layer.modes).sort()) === '["dark","light"]',
  );
  const darkFlat = layer.modes
    ? flattenTokens(layer.modes.dark.tree)
    : new Map();
  const lightFlat = layer.modes
    ? flattenTokens(layer.modes.light.tree)
    : new Map();
  check(
    `{bg.info} RESOLVES per mode — light #eef4ff, dark #0b1d3a (got ${String(lightFlat.get("bg.info")?.value)} / ${String(darkFlat.get("bg.info")?.value)})`,
    lightFlat.get("bg.info")?.value === "#eef4ff" &&
      darkFlat.get("bg.info")?.value === "#0b1d3a",
  );
  check(
    `{text.primary} RESOLVES per mode — dark #fcfeff (got ${String(darkFlat.get("text.primary")?.value)})`,
    darkFlat.get("text.primary")?.value === "#fcfeff",
  );
  check(
    "the per-mode trees mirror the repo token-vocabulary shape (tokens/modes/*.tokens.json: a tree per mode)",
...
);
{
  const dump = {
    ...
    "Alert Banner Divergent": {
      ...
      variants: [
        {
          ...themedVariant("light", "info"),
          name: "theme=light, variant=info",
        },
        {
          ...themedVariant("dark", "info", { extraChild: true }),
          name: "theme=dark, variant=info",
        },
      ],
    },
  };
  const r = replay(dump as unknown as Record<string, unknown>);
  const warning = r.proposal.notes.find(
    (n) =>
      n.includes(
        'variant axis "theme" (light|dark): named like a token mode but the variants differ beyond color across its values',
      ) &&
      n.includes("2 vs 3 children") &&
      n.includes(
        "NOT promoted; kept as an enum prop (if this is theming, unify the drawn structure), review",
      ),
  );
  check(
  ...
  check(
    ...
    themeProp !== undefined &&
      typeof themeProp.type === "object" &&
      "enum" in themeProp.type,
  );
...
);
{
  const dump = {
    ...
    "Alert Banner Branded": {
      ...
      variants: [
        ...
        {
          ...themedVariant("dark", "info"),
          name: "theme=contrast-plus, variant=info",
        },
      ],
    },
  };
  ...
  const note = r.proposal.notes.find((n) =>
    n.includes(
      'variant axis "theme": named like a token-mode axis but value(s) contrast-plus are outside the mode vocabulary (light|dark|high-contrast|dim|black|white) — kept as an enum prop, review',
    ),
  );
  check(
...
);
{
  ...
  check(
    ...
    variantProp !== undefined &&
      typeof variantProp.type === "object" &&
      "enum" in variantProp.type &&
      JSON.stringify((variantProp.type as { enum: string[] }).enum) ===
        '["default","inverse"]',
  );
  check(
    'no mode-axis note fires at all (the name table never matches "variant")',
    !r.proposal.notes.some(
      (n) => n.includes("token-mode axis") || n.includes("token mode"),
    ),
  );
  check("no `modes` metadata ships", r.contract.modes === undefined);
}
