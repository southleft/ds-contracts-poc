...
console.log(
  "CBDS Dialog send (cbds-plugin-dialog.dump.json — global part-name dedup)",
);
const dump = read(
  path.join("extract", "figma", "fixtures", "cbds-plugin-dialog.dump.json"),
);
...
check(
  `part names are UNIQUE contract-wide (${names.length} parts, ${new Set(names).size} distinct)`,
...
check(
  ...
  names.includes("Title2"),
);
check(
  'the first drawn "Icon" (title icon) keeps its name',
  names.includes("Icon"),
);
check(
  'the second "Icon" (close icon, under "Frame 2") takes the parent-derived prefix — "frame2Icon"',
  names.includes("frame2Icon"),
);
check(
  'the "Title" rename is a NAMED note carrying the node path',
  proposal.notes.some(
    (n) =>
      n.startsWith("Dialog:root/Container/Header/Title/Title:") &&
      n.includes('renamed to "Title2"'),
  ),
);
check(
  ...
  proposal.notes.some(
    (n) =>
      n.startsWith("Dialog:root/Container/Header/Frame 2/Icon:") &&
      n.includes('renamed to "frame2Icon"'),
  ),
);
check(
  ...
  proposal.notes.some(
    (n) =>
      n.includes("first drawn part keeps the name") &&
      n.includes("parent-derived prefix, else an ordinal suffix"),
  ),
);
check(
...
check(
  "Container / Header / Actions structure carried",
  names.includes("Container") &&
    names.includes("Header") &&
    names.includes("Actions"),
);
check(
  "BOTH _Slot-Dialog underscore-instances carry as slots (swap-bound INSTANCE_SWAP → slot parts, sanitized names)",
  partByName.get("swapSlotItem1")?.slot !== undefined &&
    partByName.get("swapSlotItem2")?.slot !== undefined,
);
check(
  'the slot names ride the canonicalPropName sanitize rule ("swap-slot-item-1"/"-2" → swapSlotItem1/2)',
  partByName.get("swapSlotItem1")?.slot?.name === "swapSlotItem1" &&
    partByName.get("swapSlotItem2")?.slot?.name === "swapSlotItem2",
);
const actionRefs = [
  "buttonNeutralTertiary",
...
check(
  ...
  [
    ...
    (id) =>
      stubIds.has(id) &&
      actionRefs.some((n) => partByName.get(n)?.component?.id === id),
  ),
);
check(
  "BOTH Icon instances (title icon + close icon) reference the ds.icon stub",
  partByName.get("Icon")?.component?.id === "ds.icon" &&
    partByName.get("frame2Icon")?.component?.id === "ds.icon",
);
check(
  'the scroll bar carries (hidden RECTANGLE → "scrollBar" part)',
  names.includes("scrollBar"),
);
...
check(
  `6 child stubs total (icon + 4 buttons + slot placeholder; got ${stubs.length})`,
  stubs.length === 6,
);
check(
...
const collectBound = (n: DumpNode) => {
  for (const v of Object.values(n.bound ?? {}))
    boundNames.add(String(v).split("/").join("."));
...
check(
  ...
  withoutNames.filter((e) => e.includes("does not exist in tokens/")).length >=
    10,
);
...
check(
  "the renamed Title text styles under its OWN class (__Title2, distinct from the __Title wrapper)",
  css.includes("__Title2"),
);
...
try {
  ...
  canvasVariants = engine.compileComponentData(contract, contracts).variants
    .length;
} catch (e) {
  canvasVariants = String(e).split("\n")[0];
}
check(
  `the canvas compiles — 4 variants (size axis; got ${canvasVariants})`,
  canvasVariants === 4,
);
...
const allSets = read(
  path.join("extract", "figma", "fixtures", "cbds-plugin-all-sets.dump.json"),
);
...
check(
  '"Modal / Confirmation Dialog" proposes (0 skips)',
  modalBatch.proposals.length === 1 && modalBatch.skipped.length === 0,
);
const modalId = (
  modalBatch.proposals[0]?.contract as { id?: string } | undefined
)?.id;
check(
  `its id rides the sanitize rule — "ds.modal-confirmation-dialog" (got ${modalId})`,
  modalId === "ds.modal-confirmation-dialog",
);
...
check(
  "its part names are unique contract-wide too (global registry holds everywhere)",
  new Set(modalNames).size === modalNames.length,
);
