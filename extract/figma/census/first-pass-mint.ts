...
export interface RecordMintResult {
  ...
  perSet: Array<{ set: string; from: StageStatus; to: StageStatus; canvas: number }>;
}
...
export function recordMint(exam: string): RecordMintResult {
  ...
  try {
    ...
    return {
      failures: [`${path.relative(REPO, ep)} does not parse: ${String((e as Error).message ?? e)}`],
      perSet,
    };
  }
  ...
  if (ev.exam !== exam)
    failures.push(`mint-evidence names exam ${JSON.stringify(ev.exam)}, recorded under ${exam}`);
  if (ev.driver !== "mcp")
    failures.push(`mint-evidence driver is ${JSON.stringify(ev.driver)}; the canvas write is always "mcp"`);
  ...
  for (const k of FORBIDDEN_FILE_KEYS)
    if (raw.includes(k)) failures.push(`mint-evidence names the forbidden file key ${k}`);
  if (ev.noRetry !== true)
    failures.push(`mint-evidence must set "noRetry": true — one attempt per set is the metric`);
  if (ev.attemptsPerSet !== 1)
    failures.push(`mint-evidence must set "attemptsPerSet": 1, not ${JSON.stringify(ev.attemptsPerSet)}`);

  const mp = manifestPath(exam);
  if (!existsSync(mp))
    return { failures: [...failures, `${FIRST_PASS_DIR}/${exam}/MANIFEST.json is missing — run the exam first`], perSet };
  const manifest = JSON.parse(readFileSync(mp, "utf8")) as ExamManifest;
  if (manifest.direction !== "code-to-canvas")
    failures.push(`${exam} is ${manifest.direction}; only code→canvas has a mint stage`);

  const named = new Set(Object.keys(ev.sets));
  for (const set of manifest.sets)
    if (!named.has(set))
      failures.push(`mint-evidence does not name MANIFEST set ${JSON.stringify(set)} — a set cannot be quietly DROPPED`);
  for (const set of named)
    if (!manifest.sets.includes(set))
      failures.push(`mint-evidence names ${JSON.stringify(set)}, which the MANIFEST does not — a set cannot be quietly ADDED`);
  if (failures.length > 0) return { failures, perSet };

  // ---- build every write before performing one ---------------------------
  const planned: Array<{ attempt: SetAttempt; from: StageStatus; to: StageStatus; canvas: number }> = [];
  for (const set of manifest.sets) {
    const e = ev.sets[set];
    ...
    if (to === undefined)
      failures.push(`${exam}/${set}: unknown mint status ${JSON.stringify(e.status)}`);
    ...
    if (!emitted) {
      failures.push(`${exam}/${set}: the mint stage names no emitted .figma.js artifact to check the evidence against`);
      continue;
    }
    ...
    const cells = a.images.ref.map((p) =>
      path.basename(p).replace(/^ref-/, "").replace(/\.png$/, ""),
    );
    ...
    for (const c of e.canvas) {
      ...
      if (!existsSync(abs)) {
        failures.push(`${exam}/${set}: the evidence names ${c.file}, which is not on disk`);
        continue;
      }
    ...
    for (const cell of excused.keys())
      if (covered.has(cell))
        failures.push(`${exam}/${set}: cell ${JSON.stringify(cell)} is named both present and absent`);
    ...
    stage.artifacts = [...stage.artifacts.filter((x) => !x.committed), ...canvasArts];
    ...
    const stop = a.stages.find((s) => s.status !== "ok" && s.status !== "SKIPPED") ?? null;
    a.chainComplete = a.stages.every((s) => s.status === "ok");
    a.firstStop = stop ? { stage: stop.stage, status: stop.status, message: stop.message } : null;
    a.totalMs = a.stages.reduce((n, s) => n + s.ms, 0);
    planned.push({ attempt: a, from, to: to as StageStatus, canvas: canvasPaths.length });
  }
  ...
  for (const p of planned) {
    ...
    perSet.push({ set: p.attempt.set, from: p.from, to: p.to, canvas: p.canvas });
  }

  const minted = Object.values(ev.sets).filter((s) => s.status === "minted").length;
  const unchanged = Object.values(ev.sets).filter((s) => s.status === "skipped-unchanged").length;
  const refused = Object.values(ev.sets).filter((s) => s.status === "refused").length;
  const failed = Object.values(ev.sets).filter((s) => s.status === "failed").length;
  manifest.mint = {
    requested: true,
    fileKey: ev.fileKey,
    status: minted + unchanged > 0 ? "ok" : failed > 0 ? "ERROR" : refused > 0 ? "REFUSED" : "PENDING",
    driver: "mcp",
