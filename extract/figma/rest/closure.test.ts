...
test("applied swap targets join the dependency walk without treating text or preferred values as references", async () => {
  ...
  parent.document.componentPropertyDefinitions = {
    "Unused#1": {
      type: "INSTANCE_SWAP",
      defaultValue: "2:1",
      preferredValues: [{ type: "COMPONENT", key: "unused-preference-key" }],
    },
  };
  const calls: string[][] = [];
    ...
    batches(
      {
        ...
        "3:0": entry(set("3:0", "Glyph", [[inst("leaf", "4:0")]]), {
          "4:0": { name: "Leaf" },
        }),
        "4:0": entry({
          id: "4:0",
  ...
  assert.deepEqual(
    mappedInstanceRefs(parent.document).map((ref) => ref.componentId),
    ["2:1", "3:1", "2:1", "3:1"],
  );
});
...
test("swap targets retain remote, missing and cap refusals, with property-specific locations", async () => {
  ...
  assert.deepEqual(
    closure.unresolved.map((row) => [row.targetId, row.reason]),
    [
      ["3:0", "cap-exceeded"],
      ["8:1", "remote-library-component"],
      ["9:1", "not-found"],
    ],
  );
  for (const row of closure.unresolved)
    assert.match(row.referencedFrom[0]!, /componentProperties\[/);
});

test("a swapped target pointing back to the parent records a bounded cycle", async () => {
  const node = inst("child", "2:1");
  node.componentProperties = {
    "Content#1": { type: "INSTANCE_SWAP", value: "1:1" },
  };
  const parent = entry(set("1:0", "Parent", [[node]]), {
    "2:1": { name: "Child", componentSetId: "2:0" },
  ...
  assert.match(
    closure.unresolved[0]!.referencedFrom[0]!,
    /componentProperties\[/,
  );
});
