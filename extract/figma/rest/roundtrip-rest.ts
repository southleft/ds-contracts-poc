 ...
 *             native fixture expectation. Bars: zero MISMATCH, zero map
 *             degradations. A separate comparison retains every difference
 *             from the shipping source contract, including lossy lowering.
 *
...
/** The committed REST fixtures predate per-row componentProperties. Their
 * set-level definitions are partial structured evidence and correctly refuse
 * exact mode, so this historical receipt explicitly removes that partial
 * channel before entering reviewable inversion. */
const legacyFixture = (set: DumpSet): DumpSet => {
  const copy = structuredClone(set);
  delete copy.propertyDefinitions;
  for (const variant of copy.variants) delete variant.variantProperties;
  return copy;
};
...
function main() {
  ...
  const variables = readJson<RestVariablesResponse>(
    path.join(FIXTURES, "variables.rest.json"),
  );

  const fixtureFiles = [
    { file: "badge.rest.json" },
    { file: "card.rest.json", nativeContract: "card.native.contract.json" },
  ];
  const full: FullResult[] = [];

  // ------------------------------------------------------------------ FULL
  for (const { file, nativeContract } of fixtureFiles) {
    const nodesResponse = readJson<RestNodesResponse>(
      path.join(FIXTURES, file),
    );
    const { dump, report } = mapRestToDump(nodesResponse, {
      variables,
      fileKey: "8nim1d0IPnehMxA7B7SYxC",
    });
    for (const [name, value] of Object.entries(dump)) {
      if (name === "_provenance" || !isDumpSet(value)) continue;
      const shipping = readJson<J>(
        path.join(contractsDir, `${kebab(name)}.contract.json`),
      );
      const proposal = proposeFromDump(legacyFixture(value), {
        projectionMode: "reviewable-inversion",
        corpus,
        contractIdByName,
        fileKey: "8nim1d0IPnehMxA7B7SYxC",
      });
      // The Card fixture records a FIXED 320px root with size.x bound, not
      // maxWidth. The historical emitter lowered the source max-width to
      // that fixed width. Never erase the measured bbox or teach the reader
      // to infer an absent ceiling to recover the original source spelling.
      const nativeExpected = nativeContract
        ? readJson<J>(path.join(FIXTURES, nativeContract))
        : shipping;
      const findings = compareContracts(nativeExpected, proposal.contract, corpus);
      const sourceFindings = compareContracts(shipping, proposal.contract, corpus);
      ...
      for (const d of report.degradations) {
        failures.push(
          `map degradation on the full path: [${d.code}] ${d.nodePath} — ${d.message}`,
        );
      }
      full.push({
        component: name,
  ...
  // -------------------------------------------------------------- DEGRADED
  const badgeNodes = readJson<RestNodesResponse>(
    path.join(FIXTURES, "badge.rest.json"),
  );
  const { dump: degradedDump, report: degradedReport } = mapRestToDump(
    badgeNodes,
    {
      fileKey: "8nim1d0IPnehMxA7B7SYxC",
    },
  ...
  if (!badgeSet || !isDumpSet(badgeSet))
    throw new Error("degraded map lost the Badge set");
  ...
  const degradedProposal = proposeFromDump(legacyFixture(badgeSet), {
    projectionMode: "reviewable-inversion",
    corpus,
    contractIdByName,
    fileKey: null,
  ...
  const unresolved = degradedReport.degradations.filter(
    (d) => d.code === "variable-unresolved",
  );
  check(
    unresolved.length > 0,
    `variables absent → variable-unresolved degradations named (${unresolved.length})`,
  );
  check(
    unresolved.every((d) =>
  ...
  check(
    fillUnbound !== undefined,
    `raw fill surfaced as a named UNBOUND entry (${fillUnbound?.value ?? "MISSING"})`,
  );
  check(
  ...
  check(
    !proposalJson.includes("{color."),
    "zero fabrication: no color token ref anywhere in the degraded proposal",
  );
  const rootTokens = ((degradedProposal.contract.anatomy as J).root as J)
  ...
  // may look semantic.
  const mintedProposal = proposeFromDump(legacyFixture(badgeSet), {
    projectionMode: "reviewable-inversion",
    corpus,
    contractIdByName,
    fileKey: null,
  ...
  mCheck(
    mintedTokens !== undefined && mintedTokens.count > 0,
    `provisional tree minted (${mintedTokens?.count ?? 0} leaves)`,
  );
  mCheck(
  ...
  const readRootJson = (rel: string) =>
    readJson<Record<string, unknown>>(path.resolve(root, rel));
  const repoTrees = [
  ...
  const enumValues = new Map<string, string[]>(
    (mintedProposal.contract.props as Array<{ name: string; type: unknown }>)
      .filter(
        (p): p is { name: string; type: { enum: string[] } } =>
          typeof p.type === "object" &&
          p.type !== null &&
          "enum" in (p.type as object),
      )
  ...
  const resolvable = collectRefs(mintedProposal.contract.anatomy, []).every(
    (r) => {
      const p = r.slice(1, -1);
      if (p.startsWith(`${MINT_NAMESPACE}.`)) return true;
      const ph = p.match(/\{([a-z][\w-]*)\}/);
      if (!ph) return repoInventory.has(p);
      return (enumValues.get(ph[1]) ?? []).every((v) =>
        repoInventory.has(p.replaceAll(`{${ph[1]}}`, v)),
      );
    },
  );
  mCheck(
  ...
  mCheck(
    generated !== null,
    `emitReact + emitHtml run green with repo + minted trees${generationError ? ` — ${generationError}` : ""}`,
  );
    ...
    generated !== null &&
      entries.some((e) =>
        generated!.css.includes(
          `var(--${e.ref.slice(1, -1).split(".").join("-")})`,
        ),
      ),
  ...
  // --------------------------------------------------------------- RECEIPT
  const count = (fs: Finding[], status: Finding["status"]) =>
    fs.filter((f) => f.status === status).length;
  const lines: string[] = [
    ...
    "",
    "Fixtures in `extract/figma/rest/fixtures/` are hand-crafted `GetFileNodesResponse` / `GetLocalVariablesResponse` renditions (shapes per figma/rest-api-spec) of the SAME Badge and Card canvases the plugin dump captured — semantic ground truth: `extract/figma/fixtures/main-file-dumps.json`. `map.ts` maps REST → dump v1; the EXISTING proposer and comparator do the rest. Bars: zero MISMATCH against the native expectation and zero map degradations on the full path; source-contract losses are reported separately; on the degraded path (variables response absent — the endpoint is Enterprise-only) the proposal must stay schema-valid, surface raw values as named UNBOUND entries with nearest-token candidates, and fabricate nothing. A third pass re-proposes the degraded dump with `mintUnbound: true` (core/mint-tokens.ts): every unbound value must bind to a provisional machine-named `imported.*` token, zero semantic-looking names, and the proposal must GENERATE (emitReact + emitHtml) against repo trees + the minted tree.",
    "",
    "Card's native fixture has a measured, FIXED 320px width bound through `size.x`; it has no `maxWidth`. Its independent native expectation (`fixtures/card.native.contract.json`) therefore carries `width`. The historical emitter lowered the shipping source's `max-width` into that fixed width. The source comparison below retains this loss as two MISMATCH findings. Native-fixture agreement does not qualify a lossless source round trip. The original REST fixture, plugin dump and shipping contract remain unchanged.",
    "",
    "## Native fixture agreement (nodes + variables)",
    "",
  ...
  for (const r of full) {
    ...
    if (bad) failed = true;
    lines.push(
      `| ${r.component} | ${m} | ${a} | ${x} | ${r.failures.length} | ${bad ? "❌ FAIL" : "✅ zero mismatch"} |`,
    );
  }
  lines.push("");
  for (const r of full) {
    ...
    lines.push(
      `### ${r.component} — CANVAS-ABSENT (${a.length}) — declared fidelity limits`,
      "",
    );
  ...
  lines.push("## Original source contract comparison", "",
    "These are the unmodified comparator findings against the shipping source contracts. They are not included in the native-agreement totals above.", "");
  for (const r of full) {
    const mismatches = r.sourceFindings.filter((f) => f.status === "mismatch");
    lines.push(`### ${r.component} — source MISMATCH (${mismatches.length})`, "");
  ...
  lines.push(
    "",
    `### Degradations named by the mapper (${degraded.mapReport.degradations.length})`,
    "",
  );
  ...
  for (const d of degraded.mapReport.degradations) {
    ...
    seen.add(key);
    lines.push(
      `- \`[${d.code}]\` ${d.nodePath}${d.field ? ` \`${d.field}\`` : ""} — ${d.message}`,
    );
  }
  lines.push(
    "",
  ...
  lines.push(
    "",
    `### Minted provisional tokens (${entries.length} leaves — every name machine-derived, every one a rename candidate)`,
    "",
  );
  for (const e of entries) {
    lines.push(
      `- \`${e.ref}\` = \`${e.value}\` — bound at ${e.usageSites.map((s) => `\`${s}\``).join(", ")}`,
    );
  }
  lines.push("");
  ...
  for (const r of full) {
    ...
    const sourceMismatches = r.sourceFindings.filter((f) => f.status === "mismatch");
    console.log(`${r.component} original source: MISMATCH ${sourceMismatches.length}`);
    for (const f of sourceMismatches) console.log(`  SOURCE LOSS: ${f.subject} — ${f.detail ?? ""}`);
  }
  console.log(
    `Badge (degraded): ${degraded.checks.filter((c) => c.ok).length}/${degraded.checks.length} checks`,
  );
  for (const c of degraded.checks) if (!c.ok) console.log(`  ✗ ${c.label}`);
  console.log(
    `Badge (degraded + minted): ${mChecks.filter((c) => c.ok).length}/${mChecks.length} checks · ${entries.length} minted leaves`,
  );
  for (const c of mChecks) if (!c.ok) console.log(`  ✗ ${c.label}`);
  console.log(`receipt → ${path.relative(root, outPath)}`);
  if (failed) {
    console.error(
      "REST round trip has failures — the bar is zero mismatch, zero silent degradation.",
    );
    process.exit(1);
  }
}
