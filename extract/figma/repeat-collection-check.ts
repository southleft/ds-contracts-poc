...
console.log(
  "1. real owner's-kit run (pattern-repeat-collection-navigation-header.dump.json)",
);
{
  const r = replay(
    read(
      path.join(
        FIXTURE_DIR,
        "pattern-repeat-collection-navigation-header.dump.json",
      ),
    ),
  );
  ...
  const script =
    ...
    [...(r.files.get("figma-script") ?? [])].map((f) => f.contents).join("");
  check(
    "the canvas constructs the OBSERVED instances (5 LinkNeutral sample instances in the sync script)",
    (
      script.match(/"dep":\s*"LinkNeutral"/g) ??
      script.match(/LinkNeutral/g) ??
      []
    ).length >= 5,
  );
}
...
);
{
  ...
  const repeatParts = walkAnatomy(r.contract).filter((w) => w.part.repeat);
  check(
    `ONE repeat part proposes (got ${repeatParts.length})`,
    repeatParts.length === 1,
  );
  const rp = repeatParts[0]?.part.repeat;
  const itemsProp = r.contract.props.find((p) => p.name === rp?.itemsProp);
...
);
{
  ...
  const fallback = r.proposal.notes.find((n) =>
    n.includes(
      '5 adjacent sibling instances of "_pagination-number" (repeated-children collection, P9) but no per-item field is carriable — kept as 5 fixed parts',
    ),
  );
  check(
  ...
  check(
    ...
    !r.contract.props.some(
      (p) => typeof p.type === "object" && "arrayOf" in p.type,
    ),
  );
  check(
...
);
{
  const r = replay(
    read(
      path.join(FIXTURE_DIR, "pattern-arrayof-candidate-text-area.dump.json"),
    ),
  );
  check(
