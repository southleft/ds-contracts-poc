...
console.log(
  "CBDS Tooltip (cbds-tooltip.rest-dump.json) — shadow, pointer geometry, text channels",
);
const dump = read(
  path.join("extract", "figma", "fixtures", "cbds-tooltip.rest-dump.json"),
);
...
for (const [p, entry] of flattenTokens(
...
const alphaByte = Math.round((dumpEffect.color!.alpha ?? 1) * 255)
  .toString(16)
  .padStart(2, "0");
...
const nodeOf = (variant: DumpNode, name: string) =>
  (variant.children ?? []).find((ch) => ch.name === name)!;
...
check(
  ...
  resolve(rootTokens["box-shadow"]) === dumpShadowCss &&
    dumpShadowCss === "0px 2px 4px #00000029",
);
check(
  `root background-color resolves to the dump fill ${dumpFill} (#fcfeff)`,
  resolve(rootTokens["background-color"]) === dumpFill &&
    dumpFill === "#fcfeff",
);
check(
  `root border-radius resolves to ${dumpRadius} (4px)`,
  resolve(rootTokens["border-radius"]) === dumpRadius && dumpRadius === "4px",
);
check(
  `root padding resolves to ${dumpPadding}/${dumpPadding} (8px, both axes)`,
  resolve(rootTokens["padding-inline"]) === dumpPadding &&
    resolve(rootTokens["padding-block"]) === dumpPadding &&
    dumpPadding === "8px",
);
...
check(
  ...
  css.includes("box-shadow: var(--imported-tooltip-root-box-shadow)"),
);
...
check(
  `Pointer fill resolves to the dump's ${dumpFill}`,
  resolve(pointerPart?.tokens?.["background-color"]) === dumpFill,
);
check(
  "emitReact CSS: triangle clip-path on .Pointer",
  css.includes("clip-path: polygon(50% 0%, 93.3013% 75%, 6.6987% 75%)"),
);

const placementOf = (value: string): Record<string, string> | undefined =>
  (pointerPart?.stylesWhen as J[] | undefined)?.find(
    (sw) => sw.equals === value,
  )?.styles;
...
check(
  ...
  topRight?.position === "absolute" &&
    topRight?.right === "12px" &&
    topRight?.top === "-8px" &&
    topRight?.transform === undefined,
);
check(
  "bottom-left placement EXACT (left: 12px, bottom: -8px, rotate(180deg))",
  bottomLeft?.left === "12px" &&
    bottomLeft?.bottom === "-8px" &&
    bottomLeft?.transform === "rotate(180deg)",
);
check(
  "left-center placement EXACT (left: -8px, vertically centered, rotate(-90deg))",
  leftCenter?.left === "-8px" &&
    leftCenter?.top === "50%" &&
    leftCenter?.transform === "translateY(-50%) rotate(-90deg)",
);
check(
  "the three placements genuinely DIFFER",
  JSON.stringify(topRight) !== JSON.stringify(bottomLeft) &&
    JSON.stringify(bottomLeft) !== JSON.stringify(leftCenter),
);
check(
  "emitReact CSS: per-placement rules under the enum class (.pointerPosition-topRight .Pointer …)",
  ["topRight", "bottomLeft", "leftCenter"].every((v) =>
    css.includes(`.pointerPosition-${v} .Pointer`),
  ),
);
check(
  "the placement carry is NOTED with exact-offset provenance",
  real.notes.some((n) => n.includes("offsets EXACT from the captured boxes")),
);
check(
  "the assumed triangle side count is NOTED (REST has no pointCount)",
  real.notes.some((n) => n.includes("sides: 3 (the Figma default")),
);
...
check(
  ...
  pointerPart?.visibleWhen?.prop === "pointer" &&
    pointerPart?.visibleWhen?.equals === undefined,
);
check(
  "emitReact TSX: the arrow renders conditionally ({pointer ? …})",
  tsx.includes("{pointer ? ("),
);
check(
  "pointer-position=none suppresses the arrow even against defaults (display: none stylesWhen)",
  placementOf("none")?.display === "none" &&
    css.includes(".pointerPosition-none .Pointer {\n  display: none;\n}"),
);
...
const supportPart = (c.anatomy as J).root.parts?.supportingText as
  J | undefined;
check(
  `Main text ("${mainText.text!.fontStyle}") font-weight resolves to 600 EXACTLY (weight-name table)`,
  resolve(mainPart?.tokens?.["font-weight"]) === "600" &&
    mainText.text!.fontStyle === "Semi Bold",
);
check(
  `Main text line-height resolves to ${mainText.text!.lineHeight}px EXACTLY (dump v1.3 PIXELS)`,
  resolve(mainPart?.tokens?.["line-height"]) ===
    `${mainText.text!.lineHeight}px` && mainText.text!.lineHeight === 16,
);
check(
  ...
  resolve(supportPart?.tokens?.["font-weight"]) === "400" &&
    resolve(supportPart?.tokens?.["line-height"]) ===
      `${supportText.text!.lineHeight}px`,
);
...
check(
  ...
  css.includes("var(--imported-text-label-small-font-weight)") &&
    ...
    resolve(mainPart?.tokens?.["font-weight"]) !==
      resolve(supportPart?.tokens?.["font-weight"]),
);
...
const shapeOf = (variantName: string): NodeSpec | undefined =>
  (
    data.variants.find((v) => v.name.includes(variantName))?.spec.children ?? []
  ).find((ch) => ch.type === "shape");
...
check(
  ...
  data.variants.every((v) => {
    ...
    return (
      sh !== undefined &&
      sh.x === dumpEffect.offset!.x &&
      sh.y === dumpEffect.offset!.y &&
      sh.radius === dumpEffect.radius &&
      sh.color === `#${dumpEffect.color!.hex}${alphaByte}`
    );
  }),
);
check(
  ...
  cTR?.absolute?.h === "MAX" &&
    cTR?.absolute?.right === 12 &&
    cTR?.absolute?.top === -8 &&
    (cTR?.shape?.rotation ?? 0) === 0 &&
    cBL?.absolute?.h === "MIN" &&
    cBL?.absolute?.left === 12 &&
    cBL?.absolute?.bottom === -8 &&
    cBL?.shape?.rotation === 180 &&
    cLC?.absolute?.v === "CENTER" &&
    cLC?.absolute?.left === -8 &&
    cLC?.shape?.rotation === -90,
);
check(
...
check(
  ...
  [
    "figma.createPolygon()",
    "node.rotation = -spec.shape.rotation",
    "applyShapeAbsolute",
    "type: 'DROP_SHADOW'",
    "node.lineHeight = { unit: 'PIXELS', value: spec.lineHeight }",
  ].every((m) => script.includes(m)),
);
...
check(
  ...
  htmlCss.includes("clip-path: polygon(50% 0%, 93.3013% 75%, 6.6987% 75%)") &&
    htmlCss.includes(
      ".tooltip--pointerPosition-bottomLeft .tooltip__Pointer",
    ) &&
...
check(
  ...
  real.notes.some((n) =>
    n.includes(
      "the canvas preview and the Figma sync script project it as a native DROP_SHADOW effect",
    ),
  ),
);
