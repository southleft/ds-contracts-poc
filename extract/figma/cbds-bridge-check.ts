...
console.log(
  "CBDS plugin-bridge send (cbds-plugin-button-brand-primary.dump.json, dump v1.4)",
);
const dump = read(
  path.join(
    "extract",
    "figma",
    "fixtures",
    "cbds-plugin-button-brand-primary.dump.json",
  ),
);
...
check(
  ...
  batch.proposals.length === 1 && batch.skipped.length === 0,
);
...
} = c.anatomy.root.tokensByProp ?? {};
const rootStates: Record<string, Record<string, string>> = c.anatomy.root
  .states ?? {};
...
check(
  `18 variables captured, 18 registrable, 0 skipped (got ${captured?.count}/${captured?.skipped.length ?? "?"} skips)`,
  captured?.count === 18 && captured?.skipped.length === 0,
);
const capturedValue = new Map(
  (captured?.entries ?? []).map((e) => [e.path, e.value]),
);
...
const shadowed = (captured?.entries ?? []).filter((e) =>
...
check(
  ...
  withoutCaptured.filter((e) => e.includes("does not exist in tokens/"))
    .length >= 9,
);
...
check(
  "root background-color = {bg.brand.default}",
  rootTokens["background-color"] === "{bg.brand.default}",
);
check("root gap = {spacing.100}", rootTokens["gap"] === "{spacing.100}");
check(
  "root padding-block = {spacing.100}",
  rootTokens["padding-block"] === "{spacing.100}",
);
check(
  "root border-radius = {corner-radius.100}",
  rootTokens["border-radius"] === "{corner-radius.100}",
);
check(
  "hover state = {bg.brand.hover}",
  rootStates.hover?.["background-color"] === "{bg.brand.hover}",
);
check(
  "active state (pressed promoted) = {bg.brand.pressed}",
  rootStates.active?.["background-color"] === "{bg.brand.pressed}",
);
check(
  "disabled state = {bg.disabled}",
  rootStates.disabled?.["background-color"] === "{bg.disabled}",
);
check(
  "focus-visible outline-color = {border.focus}",
  rootStates["focus-visible"]?.["outline-color"] === "{border.focus}",
);
check(
  "label color = {text.inverse-primary}",
  (c.anatomy.root.parts?.Button?.tokens ?? {})["color"] ===
...
check(
  ...
  rootTokens["padding-inline"] === "{spacing.200}",
);
...
check(
  ...
  proposal.notes.some(
    (n) =>
      n.includes("padding-inline") &&
      n.includes('function of variant axis "size" by VALUE'),
  ) &&
    proposal.notes.some(
      (n) =>
        n.startsWith("Button-Brand Primary:root height:") &&
        n.includes("by VALUE"),
    ),
);
...
check(
  ...
  typeof minHeightRef === "string" && minHeightRef.startsWith("{imported."),
);
check(
...
check(
  ...
  Array.isArray(dump._degradations) &&
    (dump._degradations as unknown[]).length === 0,
);
check(
  "zero UNBOUND leftovers (every raw literal minted or refused by name)",
  proposal.unbound.length === 0,
);
...
check(
  `computed background = #0e61ba from HIS {bg.brand.default} (got ${computed(k, "background-color")})`,
  computed(k, "background-color") === "#0e61ba",
);
check(
  `computed padding-inline = 16px (large; got ${computed(k, "padding-inline")})`,
  computed(k, "padding-inline") === "16px",
);
check(
  `computed padding-block = 8px (got ${computed(k, "padding-block")})`,
  computed(k, "padding-block") === "8px",
);
check(
  `computed height = 48px (large; got ${computed(k, "height")})`,
  computed(k, "height") === "48px",
);
check(
  `computed min-height = 44px (got ${computed(k, "min-height")})`,
  computed(k, "min-height") === "44px",
);
check(
  `computed border-radius = 8px (got ${computed(k, "border-radius")})`,
  computed(k, "border-radius") === "8px",
);
check(
...
check(
  "emitReact: <button + native disabled on the same layered inventory",
  tsx.includes("<button") && tsx.includes("disabled={disabled}"),
);
