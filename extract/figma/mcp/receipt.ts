...
async function main(): Promise<void> {
  ...
  // ------------------------------------------------------------------ BADGE
  const badgeFixture = readJson<McpImportFixture>(
    path.join(FIXTURES, "badge.mcp.json"),
  );
  const badge = await replayImport(badgeFixture);
  const badgeSet = Object.values(badge.dump).find(isDumpSet);
  if (!badgeSet)
    throw new Error("badge fixture replay produced no component set");
  const badgeProposal = propose(badgeSet, badgeFixture.fileKey);
  const shipping = readJson<J>(
    path.resolve(root, "contracts", `${kebab(badgeSet.setName)}.contract.json`),
  );
  ...
  bCheck(
    badgeFixture.restVariables === undefined,
  ...
  const nodeScoped = badge.augment.resolved.filter(
    (r) => r.via === "node-scope",
  );
  bCheck(
    nodeScoped.length > 0,
  ...
  bCheck(
    badge.report.degradations.length === 0,
    `zero map degradations (got ${badge.report.degradations.length})`,
  );
  bCheck(
    badgeProposal.unbound.length === 0 &&
      (badgeProposal.mintedTokens?.count ?? 0) === 0,
  ...
  // ----------------------------------------------------------------- EVENTZ
  const eventzFixture = readJson<McpImportFixture>(
    path.join(FIXTURES, "eventz-alert.mcp.json"),
  );
  const eventz = await replayImport(eventzFixture);
  const eventzSet = Object.values(eventz.dump).find(isDumpSet);
  if (!eventzSet)
    throw new Error("eventz fixture replay produced no component set");
  ...
  for (const name of [
    ...
    "spacing/3",
    "spacing/4",
    "component/border/radius/rounded-md",
    "color/content/inverse",
  ]) {
    eCheck(
      recoveredNames.includes(name),
      `foreign vocabulary recovered by name: "${name}"`,
    );
  }
  ...
  eCheck(
    eventzProposal.unbound.length === 0 &&
      (eventzProposal.mintedTokens?.count ?? 0) > 0,
  ...
  if (mismatches.length > 0) {
    ...
    for (const f of mismatches)
      lines.push(`- **${f.subject}** — ${f.detail ?? ""}`);
  }
  lines.push(
    "",
    `### Names recovered by the join (${badge.augment.resolved.length} ids, ${badge.augment.defsCalls} get_variable_defs calls)`,
    "",
  );
  ...
  lines.push(
    "",
    `### CANVAS-ABSENT (${absent.length}) — declared fidelity limits (same list as the plugin-dump receipt)`,
    "",
  );
  ...
  lines.push(
    "",
    `### Names recovered by the join (${eventz.augment.resolved.length} ids, ${eventz.augment.defsCalls} get_variable_defs calls)`,
    "",
  );
  ...
  lines.push(
    "",
    `### Proposal notes (review line items, ${eventzProposal.notes.length})`,
    "",
  );
  ...
  const report = (label: string, checks: Check[]) => {
    console.log(
      `${label}: ${checks.filter((c) => c.ok).length}/${checks.length} checks`,
    );
  ...
  if (failed) {
    console.error(
      "Desktop-MCP round trip has failures — the bar is zero mismatch, zero silent degradation.",
    );
    process.exit(1);
  }
}
...
function augmentLines(augment: AugmentReport): string[] {
  ...
  for (const r of augment.resolved) {
    lines.push(
      `- \`${r.variableId}\` → \`${r.name}\` (${r.via}, ${r.occurrences} occurrence(s))`,
    );
  }
  for (const u of augment.unresolved) {
    const at = u.occurrences
      .map((o) => `\`${o.nodePath}\` ${o.field}`)
      .join(", ");
    lines.push(
