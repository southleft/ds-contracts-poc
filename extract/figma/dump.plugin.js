// Design-side ANATOMY dump — the canonical node-tree capture (dump v1.7).
//
...
async function dumpNode(node, nodePath, parent) {
  ...
  // nodePath null suppresses per-node degradation spam).
  if (node.type === 'INSTANCE' && typeof node.findAll === 'function') {
    try {
      ...
      for (const n of kids) {
        ...
        if (!n.fills.some(function (p) { return p.visible !== false && p.type === 'SOLID'; })) continue;
...
const dumps = {
  _provenance: {
    ...
    note: 'Node-tree dump (extract/figma/dump.plugin.js, dump v1.7) for design→contract proposal.',
    dumpVersion: '1.7',
  },
};
