...
const contractIdByName = new Map(
...
const shoelaceDump = read(
  "extract/fidelity-matrix/fixtures/shoelace-button-group/dump.json",
) as DumpFile;
const shoelaceSet = shoelaceDump["Button group"] as DumpSet;
const eventzDump = read(
  "extract/fidelity-matrix/fixtures/eventz-button/dump.json",
) as DumpFile;
const eventzSet = eventzDump["Button"] as DumpSet;

const childInstance = shoelaceSet.variants[0].children!.find(
  (c) => c.type === "INSTANCE",
)!;
...
check(
  'fixture: the nested "Button" instance carries its owning set key (dump v1.5)',
...
const propose = (
  ...
  proposeFromDump(JSON.parse(JSON.stringify(set)) as DumpSet, {
    ...
    contractsById: new Map<string, MinimalChildContract>([
      ...contracts,
      ...(extra?.byId ?? new Map()),
    ]),
    fileKey: "fixture",
...
check(
  ...
  !refusedAnatomy.includes('"id":"ds.button"') &&
    refused.notes.some(
      (n) => n.includes("keys CONTRADICT") && n.includes("ds.button"),
    ),
);
check(
  "the stub id is suffixed PAST the contradicting in-scope contract (ds.button-2, never ds.button)",
  (refused.childStubs ?? []).some(
    (s) => (s as { id?: string }).id === "ds.button-2",
...
check(
  ...
  JSON.stringify(linked.contract).includes(
    '"id":"sl.totally-renamed-button"',
  ) &&
    linked.notes.some((n) =>
      n.includes("LINKED to sl.totally-renamed-button by componentSetKey"),
    ),
);
check(
  "no ds.button-2 stub when the key resolves",
  !(linked.childStubs ?? []).some(
    (s) => (s as { id?: string }).id === "ds.button-2",
  ),
);
...
const stub = ContractSchema.parse(
  (refused.childStubs ?? []).find(
    (s) => (s as { id?: string }).id === "ds.button-2",
  ),
);
...
check(
  ...
  stubTokens["background-color"] ===
    "{imported.stub-button-2.root.background-color.{variant}}",
);
check(
  "minted leaves carry the OBSERVED values (small width 44px, large 82px, default fill #ffffff)",
  (refused.mintedTokens?.entries ?? []).some(
    (e) =>
      e.ref === "{imported.stub-button-2.root.width.small}" &&
      e.value === "44px",
  ) &&
    (refused.mintedTokens?.entries ?? []).some(
      (e) =>
        e.ref === "{imported.stub-button-2.root.width.large}" &&
        e.value === "82px",
    ) &&
    (refused.mintedTokens?.entries ?? []).some(
      (e) =>
        e.ref === "{imported.stub-button-2.root.background-color.default}" &&
        e.value === "#ffffff",
    ),
);
check(
...
check(
  ...
  refused.notes.some((n) =>
    n.includes("border not carried on the stub geometry"),
  ),
);
...
// The stub renders: emit-html compiles the geometry to per-size rules.
{
  ...
  const { css, html } = emitHtml(parent, {
    tokens: tokenInventoryFromJson(trees),
    icons: new Map(),
    contracts: scope,
  });
  check(
    "emit-html: the stub box renders per size (.button--size-small { width: var(--imported-stub-button-2-root-width-small) })",
    css.includes(".button--size-small {") &&
      css.includes("width: var(--imported-stub-button-2-root-width-small)"),
  );
  check(
    "emit-html: the stub renders its OBSERVED label text, and never its contract name",
    html.includes(">Label<") &&
      !/>\s*Button\s*</.test(html.replaceAll("button-group", "")),
  );
}
...
check(
  ...
  eventzJson.includes('"defaultContent":[{"id":"ds.play"}]') &&
    eventzJson.includes('"defaultContent":[{"id":"ds.pause"}]'),
);
const playStub = (eventz.childStubs ?? []).find(
  (s) => (s as { id?: string }).id === "ds.play",
) as { anatomy?: { root?: { tokens?: Record<string, string> } } };
check(
  "eventz: the icon stub carries the observed 20×20 box (minted uniform width/height)",
  playStub?.anatomy?.root?.tokens?.width ===
    "{imported.stub-play.root.width}" &&
    (eventz.mintedTokens?.entries ?? []).some(
      (e) => e.ref === "{imported.stub-play.root.width}" && e.value === "20px",
    ),
);
check(
  "eventz: hasStartIcon/hasEndIcon default TRUE from the BOOLEAN property definitions (dump v1.5 boolDefaults)",
  eventz.notes.filter((n) =>
    n.includes("default true: the property definition's defaultValue"),
  ).length >= 2,
);
...
check(
  "fixture: startIcon/endIcon carry preferredValues component keys (dump v1.5)",
  typeof prefKey === "string" && prefKey.length > 0,
);
check(
  "unresolvable keys stay a NAMED note carrying the keys verbatim (no accepts invented)",
  !eventzJson.includes('"accepts"') &&
    eventz.notes.some(
      (n) => n.includes("preferredValues name") && n.includes(prefKey!),
    ),
);
...
check(
  ...
  linkedJson.includes('"accepts":["ev.icon"]') &&
    linkedJson.includes('"acceptsMode":"prefer"'),
);
check(
  "the resolution is NAMED (preferredValues → accepts note)",
  eventzLinked.notes.some(
    (n) =>
      n.includes("accepts proposed from INSTANCE_SWAP preferredValues") &&
      n.includes("ev.icon"),
  ),
);
...
console.log(
  "✔ all composite-children invariants hold (key linking rename-safe + contradiction-refused, stub geometry minted/threaded/rendered, preferredValues → accepts)",
);
