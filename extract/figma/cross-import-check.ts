...
check(
  "button proposes as ds.button-brand-primary",
  buttonContract.id === "ds.button-brand-primary",
);
check(
  ...
  (buttonProposal.mintedTokens?.entries ?? []).some(
    (e) => e.ref === "{imported.button-brand-primary.button.font-size.large}",
  ),
);
check(
  `button CAPTURES its variables (${buttonCaptured?.count ?? 0} — dump v1.4 _variables)`,
  (buttonCaptured?.count ?? 0) > 0,
);
...
const dialogStubs = (dialogProposal.childStubs ?? []).map((s) =>
...
check(
  ...
  JSON.stringify(dialogProposal.contract).includes(
...
const contracts = new Map<string, Contract>(
  loaded.byId as Map<string, Contract>,
);
...
check(
  ...
  controlRefusal ===
    'Cannot resolve token "imported.button-brand-primary.button.font-size.large"',
);
...
check(
  "the scope discovers the linked button through the composition graph",
  scope.receipts.length === 1,
);
check(
...
check(
  ...
  /^resolving through Button-Brand Primary's imported tokens — \d+ \(/.test(
    receipt,
  ),
);
check(
  ...
  receipt.includes(
    "already defined by the active source (base wins, never overridden)",
...
try {
  ...
  canvasVariants = engine.compileComponentData(dialogContract, contracts)
    .variants.length;
  childVariants = engine.compileComponentData(buttonContract, contracts)
    .variants.length;
} catch (e) {
  canvasVariants = String(e).split("\n")[0];
}
check(
  `the CANVAS compiles: dialog 4 variants (got ${canvasVariants})`,
  canvasVariants === 4,
);
...
check(
  `the LINKED button compiles too: 3 size variants (got ${childVariants})`,
  childVariants === 3,
);
...
const trySurface = (label: string, fn: () => void) => {
  try {
    ...
    check(
      `${label} emits with ZERO refusals — got: ${String(e).split("\n")[0]}`,
      false,
    );
  }
};
trySurface("react (css modules)", () =>
  emitReact(dialogContract, {
    tokens: scopedInventory,
    icons: new Map(),
    contracts,
  }),
);
trySurface("html (preview surface)", () =>
  emitHtml(dialogContract, {
    tokens: scopedInventory,
    icons: new Map(),
    contracts,
  }),
...
trySurface("react-inline (literal resolution through the scoped tree)", () =>
  emitReactInline(dialogContract, {
    tokens: scopedTree,
    icons: new Map(),
    contracts,
  }),
