...
console.log(
  "CANVAS METRICS PARITY (canvas cells vs the captured Figma variant boxes)",
);
const dump = read(
  path.join(
    "extract",
...
check(
  `15 captured variants in the dump (got ${set.variants.length})`,
  set.variants.length === 15,
);
...
check(
  ...
  new Set(cells.map((c) => dumpVariantFor(c.name)?.name).filter(Boolean))
    .size === 15,
);
...
for (const cell of cells) {
  ...
  check(
    `cell "${cell.name}" box == captured "${dv.name}" box (h=${want.height} via ${want.heightVar}, pad=[${want.padTop},${want.padRight},${want.padBottom},${want.padLeft}], gap=${want.gap}, hug width)` +
      (same
        ? ""
        : ` — GOT h=${got.height} via ${got.heightVar}, pad=[${got.padTop},${got.padRight},${got.padBottom},${got.padLeft}], gap=${got.gap}, hug=${got.hugWidth}`),
    same,
  );
...
check(
  ...
  heightsBySize.get("large") === 48 &&
    heightsBySize.get("medium") === 40 &&
    heightsBySize.get("small") === 32,
);
check(
  ...
  padInlineBySize.get("large") === 16 &&
    padInlineBySize.get("medium") === 16 &&
    padInlineBySize.get("small") === 12,
);
check(
...
const canvasPreviewPath = path.join(
  ROOT,
  "playground",
  "src",
  "engine",
...
check(
  "playground/src/engine/canvas-preview.ts present",
  existsSync(canvasPreviewPath),
);
const canvasSrc = existsSync(canvasPreviewPath)
  ? readFileSync(canvasPreviewPath, "utf8")
  : "";
check(
...
check(
  ...
  canvasSrc.includes("spec.fixedHeight") &&
    canvasSrc.includes("cssVarWithFallback"),
);
