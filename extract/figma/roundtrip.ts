...
const J_MAP: Record<string, string> = {
  start: "MIN",
  center: "CENTER",
  end: "MAX",
...
function normLayout(layout: J | undefined, isRoot: boolean): NormLayout {
  ...
  const direction =
    typeof layout.direction === "string" ? layout.direction : "row";
  return {
    mode: direction.startsWith("column") ? "VERTICAL" : "HORIZONTAL",
    primary:
      typeof layout.justify === "string"
        ? (J_MAP[layout.justify] ?? "MIN")
        : "MIN",
    counter:
      typeof layout.align === "string" ? (A_MAP[layout.align] ?? "MIN") : "MIN",
...
function walk(anatomy: J): WalkedPart[] {
  ...
  const visit = (name: string, part: J, prefix: string, chain: J[]) => {
    ...
    for (const [childName, child] of Object.entries(
      (part.parts as Record<string, J>) ?? {},
    )) {
      visit(childName, child, p, nextChain);
    }
  };
  for (const [name, part] of Object.entries(anatomy as Record<string, J>))
    visit(name, part, "", []);
  return out;
}
...
function isTextBearing(w: WalkedPart, contract: J): boolean {
  ...
  if (
    w.path === "root" &&
    Object.keys((w.part.parts as J) ?? {}).length === 0
  ) {
    const props = (contract.props as J[]) ?? [];
    return props.some(
      (p) =>
        p.type === "text" &&
        ((p.bindings as J)?.code as J)?.prop === "children",
    );
  }
  return false;
}
...
function effective(
  w: WalkedPart,
  prop: string,
): { ref: string; site: number } | undefined {
...
export function compareContracts(
  ...
  const F: Finding[] = [];
  const matched = (subject: string, detail?: string) =>
    F.push({ status: "matched", subject, ...(detail ? { detail } : {}) });
  const absent = (subject: string, detail: string) =>
    F.push({ status: "canvas-absent", subject, detail });
  const mismatch = (subject: string, detail: string) =>
    F.push({ status: "mismatch", subject, detail });
  ...
  if (!deepEqual(shipping.semantics, proposed.semantics)) {
    absent(
      "semantics",
      `element/role are not drawn on the canvas (${JSON.stringify(shipping.semantics)})`,
    );
  } else matched("semantics");
  if (shipping.a11y)
    absent("a11y", "accessibility requirements are not canvas-recoverable");
  if (Array.isArray(shipping.events) && shipping.events.length > 0) {
    absent(
      "events",
      `the canvas cannot run behavior — ${(shipping.events as J[]).map((e) => String(e.name)).join(", ")} not recoverable`,
    );
  }
  ...
  for (const sp of sProps) {
    ...
    const fields: Array<[string, unknown, unknown]> = [
      ...
      [
        "bindings.figma.kind",
        ((sp.bindings as J).figma as J).kind,
        ((pp.bindings as J).figma as J).kind,
      ],
      [
        "bindings.figma.property",
        ((sp.bindings as J).figma as J).property,
        ((pp.bindings as J).figma as J).property,
      ],
      [
        "bindings.figma.values",
        ((sp.bindings as J).figma as J).values,
        ((pp.bindings as J).figma as J).values,
      ],
      [
        "bindings.code.prop",
        ((sp.bindings as J).code as J).prop,
        ((pp.bindings as J).code as J).prop,
      ],
    ];
    let ok = true;
    ...
    if (sp.required === true && pp.required !== true) {
      absent(
        `prop ${sp.name} required`,
        "requiredness is not drawn on the canvas",
      );
  ...
  for (const w of pWalk) {
    if (!sByPath.has(w.path))
      mismatch(`part ${w.path}`, "proposal has a part the contract does not");
  }
  for (const w of sWalk) {
    ...
    if (!deepEqual(common, commonP)) {
      mismatch(
        `part ${w.path} child order`,
        `${common.join(",")} vs ${commonP.join(",")}`,
      );
  ...
  function comparePart(s: WalkedPart, p: WalkedPart) {
    ...
    for (const [prop, ref] of Object.entries(sTok)) {
      ...
      else if (pTok[prop] === undefined)
        mismatch(`${at} ${prop}`, `${ref} missing from proposal`);
    ...
    for (const prop of Object.keys(pTok)) {
      if (!CASCADE.has(prop) && sTok[prop] === undefined) {
        mismatch(
          `${at} ${prop}`,
          `proposal has ${pTok[prop]}, contract has nothing`,
        );
      }
    }
    ...
    if (s.part.visibleWhen || p.part.visibleWhen) {
      if (deepEqual(s.part.visibleWhen, p.part.visibleWhen))
        matched(`${at} visibleWhen`);
      else
        mismatch(
          `${at} visibleWhen`,
          `${JSON.stringify(s.part.visibleWhen)} vs ${JSON.stringify(p.part.visibleWhen)}`,
        );
    }
    if (s.part.optional === true || p.part.optional === true) {
      if (s.part.optional === p.part.optional) matched(`${at} optional`);
      else
        mismatch(`${at} optional`, `${s.part.optional} vs ${p.part.optional}`);
    }
    if (s.part.content || p.part.content) {
      if (deepEqual(s.part.content, p.part.content)) matched(`${at} content`);
        ...
        mismatch(
          `${at} content`,
          `${JSON.stringify(s.part.content)} vs ${JSON.stringify(p.part.content)}`,
        );
    }
    if (typeof s.part.text === "string" && s.part.text !== "") {
      if (p.part.text === s.part.text) matched(`${at} text`);
        ...
        mismatch(
          `${at} text`,
          `"${s.part.text}" vs ${JSON.stringify(p.part.text)}`,
        );
    } else if (s.part.text === "") {
      if (p.part.text === "" || p.part.text === undefined) {
        absent(
          `${at} text ""`,
          "empty static text renders no glyphs — the styled wrapper alone is on the canvas",
        );
      } else mismatch(`${at} text`, `"" vs ${JSON.stringify(p.part.text)}`);
    } else if (typeof p.part.text === "string") {
      mismatch(
        `${at} text`,
        `proposal has "${p.part.text}", contract has none`,
      );
    }
    if (s.part.element)
      absent(
        `${at} element`,
        `host element ("${s.part.element}") is not drawn on the canvas`,
      );
    if (s.part.attrs)
      absent(`${at} attrs`, "HTML/ARIA attributes are code-side surface");
    if (s.part.states)
      absent(
        `${at} states`,
        "interaction-state token overrides are code-side (CSS)",
      );
    if (s.part.stylesWhen)
      absent(
        `${at} stylesWhen`,
        "conditional literal styles are code-side (declared canvas limit)",
      );
    if (s.part.animation)
      absent(
        `${at} animation`,
        "motion is not canvas-representable (documented fidelity scope)",
      );
    ...
    if (s.part.slot || p.part.slot) {
      ...
      if (!ss || !ps) {
        mismatch(
          `${at} slot`,
          ss ? "missing from proposal" : "proposal invented a slot",
        );
      } else {
        ...
        if (ss.required === true && ps.required !== true) {
          absent(
            `${at} slot required`,
            "slot requiredness is not drawn on the canvas",
          );
        ...
        if (ss.accepts && !ps.accepts) {
          absent(
            `${at} slot accepts`,
            `INSTANCE_SWAP preferredValues not captured in dump v1 (${(ss.accepts as string[]).join(", ")})`,
          );
        } else if (!deepEqual(ss.accepts, ps.accepts) && ps.accepts) {
          mismatch(
            `${at} slot accepts`,
            `${JSON.stringify(ss.accepts)} vs ${JSON.stringify(ps.accepts)}`,
          );
        }
        for (const extra of [
          "acceptsMode",
          ...
          if (ss[extra] !== undefined && ps[extra] === undefined) {
            ...
            ps[extra] !== undefined &&
            !deepEqual(ss[extra], ps[extra])
          ) {
            mismatch(
              `${at} slot ${extra}`,
              `${JSON.stringify(ss[extra])} vs ${JSON.stringify(ps[extra])}`,
            );
          }
        }
      }
    ...
    if (s.part.component || p.part.component) {
      ...
      if (!sc || !pc) {
        mismatch(
          `${at} component`,
          sc ? "missing from proposal" : "proposal invented a component ref",
        );
      } else {
        ...
        if (sc.props !== undefined && pc.props === undefined) {
          absent(
            `${at} component props`,
            `fixed instance prop values not captured in dump v1 (${JSON.stringify(sc.props)})`,
          );
        } else if (!deepEqual(sc.props, pc.props)) {
          mismatch(
            `${at} component props`,
            `${JSON.stringify(sc.props)} vs ${JSON.stringify(pc.props)}`,
          );
        ...
        if (sc.text !== undefined && pc.text === undefined) {
          absent(
            `${at} component text`,
            "instance text override not captured in dump v1",
          );
        } else if (!deepEqual(sc.text, pc.text)) {
          mismatch(
            `${at} component text`,
            `${JSON.stringify(sc.text)} vs ${JSON.stringify(pc.text)}`,
          );
        }
      }
    }
    ...
    for (const prop of CASCADE) {
      ...
      if (se && !pe) {
        if (prop === "font-family") {
          absent(
            subject,
            `${se.ref} — the canvas renders Inter regardless (documented fidelity scope)`,
          );
        } else if (prop === "font-weight" && resolvedWeight(se.ref) === 500) {
          absent(
            subject,
            `${se.ref} resolves to the runtime text default (Medium) — canvas-indistinguishable from no weight token`,
          );
        } else {
          F.push({
            status: "mismatch",
            subject,
            detail: `${se.ref} missing from proposal`,
          });
        }
        continue;
      }
      if (!se && pe) {
        F.push({
          status: "mismatch",
          subject,
          detail: `proposal has ${pe.ref}, contract has nothing`,
        });
        continue;
      }
      F.push({
  ...
  for (const w of pWalk) {
    ...
    for (const prop of Object.keys(tokens)) {
      ...
      const feeds = pWalk.some(
        (t) =>
          isTextBearing(t, proposed) &&
          (t.path === w.path || t.path.startsWith(`${w.path}/`)) &&
          effective(t, prop)?.ref === tokens[prop],
      );
      if (!feeds) {
        F.push({
          status: "mismatch",
          subject: `part ${w.path} ${prop}`,
          detail: `proposal declares ${tokens[prop]} with no text to render it`,
        });
      }
    }
  }
...
export function runRoundtrip(
  ...
  for (const [name, value] of Object.entries(dump)) {
    ...
    const shippingPath = path.join(
      contractsDir,
      `${kebab(name)}.contract.json`,
    );
    ...
    results.push({
      component: name,
      findings: compareContracts(shipping, proposal.contract, corpus),
      proposal,
    });
  }
  return results;
...
const count = (fs: Finding[], status: Status) =>
...
export function receiptMarkdown(
  ...
  for (const r of results) {
    ...
    lines.push(
      `| ${r.component} | ${m} | ${a} | ${x} | ${x === 0 ? "✅ zero mismatch" : "❌ FAIL"} |`,
    );
  }
  lines.push("");
  for (const r of results) {
...
function main() {
  ...
  const fixtureRel = readFlag(
    "--fixtures",
    path.join("extract", "figma", "fixtures", "main-file-dumps.json"),
  );
  const contractsDir = readFlag("--contracts", "contracts");
  const outPath = readFlag(
    "--out",
    path.join("extract", "figma", "ROUNDTRIP.md"),
  );
  ...
  writeFileSync(
    ...
    receiptMarkdown(results, fixtureRel) + "\n",
  );
  ...
  for (const r of results) {
    ...
    const x = count(r.findings, "mismatch");
    console.log(
      `${r.component}: MATCHED ${m} · CANVAS-ABSENT ${a} · MISMATCH ${x}`,
    );
