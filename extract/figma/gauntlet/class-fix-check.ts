...
function replay(fixtureFile: string): Replay {
  ...
  if (batch.proposals.length !== 1)
    throw new Error(
      `${fixtureFile}: expected 1 proposal, got ${batch.proposals.length} (${batch.skipped.length} skipped)`,
    );
...
);
{
  ...
  check(
    ...
    !anatomy.includes("isVisible"),
  );
  check(
    `referee CLEAN (validateContract + generateCss report zero violations; got ${r.violations.length})`,
    r.violations.length === 0,
  );
  check(
    'no "sets unknown … prop" violation anywhere',
    !r.violations.some((v) => /sets unknown [a-z0-9.-]+ prop/.test(v)),
  );
  check(
            ...
            children: [
              {
                name: "Label",
                type: "TEXT",
                text: { characters: "Hi", fontSize: 14, fontStyle: "Regular" },
              },
            ],
          },
          {
...
);
{
  ...
  const digitNote = r.notes.find(
    (n) =>
      n.includes(
        'prop `p2ndParagraph`: Figma property "2nd paragraph" is digit-led',
      ) &&
      n.includes('deterministic "p" prefix') &&
      n.includes(
        "the original spelling stays the design binding (bindings.figma.property)",
      ),
  );
  check(
  ...
  check(
    ...
    prop !== undefined && prop.bindings.code.prop === "p2ndParagraph",
  );
  check(
    'the figma binding keeps the ORIGINAL spelling "2nd paragraph"',
    prop?.bindings.figma.property === "2nd paragraph",
  );
  check(
    `referee CLEAN (zero violations; got ${r.violations.length})`,
    r.violations.length === 0,
  ...
  check(
    ...
    !r.violations.some((v) =>
      v.includes("is not a legal camelCase identifier"),
    ),
  );
  check(
    `ALL FOUR surfaces emit (${surfaces})`,
...
);
{
  ...
  const invalid = ContractSchema.parse(
    JSON.parse(JSON.stringify(badge)),
  ) as Contract;
  (invalid.anatomy.root as { visibleWhen?: { prop: string } }).visibleWhen = {
    prop: "nonexistent",
  };
  let refusal: string | null = null;
  try {
    emitFigmaScript(invalid, {
      tokens: { ...repoTrees, brands },
      icons,
  ...
  check(
    ...
    refusal?.startsWith("Refused — 1 contract violation(s)") === true,
  );
  check(
    'the violation names the part and prop (visibleWhen references unknown prop "nonexistent")',
    refusal?.includes('visibleWhen references unknown prop "nonexistent"') ===
      true,
  );
  // And the valid original still emits byte-for-byte the same script.
  const script = emitFigmaScript(badge, {
    tokens: { ...repoTrees, brands },
    icons,
    contracts: repoContracts,
  });
  check(
    ...
    script.length > 0 && script.includes("ds.badge"),
  );
}
