...
| P18 | Part-level interaction states (menu-item hover, tab hover, row hover) | Universal in composites (APG/blueprints: items carry their own enabled/hover/focus/selected states — Tabs & Chip blueprints enumerate them per item) | Item component's own State axis; or per-part fills differing in root state variants | **SECOND HALF SHIPPED (2026-07-12, v13)** — `Part.states` unlocked BOUNDED: per-state token overrides on NON-ref parts (text/icon/box), color-kind channels only (color/background-color/border-color — emit-react `PART_STATE_CHANNELS`), refusal-ruled in validateContract (unknown/undeclared state names, ref/slot parts, non-color channels — all by name); react+html render `.root:disabled .part` descendant rules, canvas State-preview variants apply part overrides (`withPartStateOverrides`), react-inline documents the declared limit; `proposeStateDiffs` PROPOSES depth-1 child color diffs (same occurrence machinery — refs unify, literals mint `imported.<comp>.<part>-state-<state>.<channel>`). STYLE-FIDELITY B7 retired where the channel carries; narrow receipts remain (ref/slot children — the child contract owns its styling; CSS→contract inversion of the selector shape). Receipt extract/figma/part-state-check.ts; evals `part-level-state-overrides`, `part-state-disabled-label-browser-probe`. Field case: the CBDS disabled Button label #556275 on #dfe3eb. When the item is its OWN contract, P3 decomposition still preferred | shipped (b); (a) decomposition stays the first choice |
...
7. **Part-level states (P18)** — SHIPPED (v13, 2026-07-12): `Part.states`
   unlocked in the CSS emitters + canvas State previews, `proposeStateDiffs`
   depth-1 receipts became proposals (color-kind channels, refusal-ruled).
   STYLE-FIDELITY B7 retired on the design side; compound-contract
   decomposition stays the first choice when the item is its own contract.
