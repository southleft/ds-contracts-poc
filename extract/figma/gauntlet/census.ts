...
const DEFAULT_DUMP = path.join(
  ...
  "figma",
  "fixtures",
  "cbds-plugin-all-sets.v14.dump.json",
);
...
const outDir =
  outDirFlag !== undefined ? path.resolve(ROOT, outDirFlag) : OUT_DIR;
...
const capturedShadowed = (captured?.entries ?? []).filter((e) =>
  repoInventory.has(e.path),
);
...
for (const d of (dump._degradations as
  Array<{ code: string; nodePath: string; message: string }> | undefined) ??
  []) {
  const fingerprint = `${d.code} ${d.nodePath} ${d.message}`;
  if (seenDegradations.has(fingerprint)) {
    duplicateDegradations++;
    continue;
  }
...
function violationClass(message: string): string {
  ...
  if (message.includes("duplicate anatomy part name"))
    return "duplicate-part-name";
  if (/visibleWhen\.equals "[^"]*" is not a value of prop/.test(message))
    return "visiblewhen-value-outside-prop-enum";
  if (/sets unknown [a-z0-9.-]+ prop/.test(message))
    return "component-ref-unknown-child-prop";
  if (message.includes("is not a legal camelCase identifier"))
    return "prop-binding-not-camelcase";
...
function refusalClasses(error: unknown): {
  classes: string[];
  firstLine: string;
} {
  ...
  if (lines.length === 0)
    return {
      classes: [violationClass(message.split("\n")[0])],
      firstLine: message.split("\n")[0],
    };
  return {
    classes: [...new Set(lines.map(violationClass))],
    firstLine: lines[0],
  };
}
...
function featuresOf(set: {
  ...
  if (set.type === "COMPONENT_SET") {
    for (const v of variants) {
      for (const pair of (v.name ?? "").split(",")) {
        ...
        (
          axisValues.get(axis) ?? axisValues.set(axis, new Set()).get(axis)!
        ).add(value);
      }
    }
  }
  ...
  const stateLike = axes.filter((a) => {
    ...
    return (
      values.length >= 2 &&
      values.filter((v) => STATE_VALUES.has(v)).length >= 2
    );
  });
  ...
  const walk = (node: DumpNodeJson, d: number) => {
    ...
    if (node.type === "TEXT") {
      textStyles.add(
        node.text?.style ??
          `${node.text?.fontSize ?? "?"}/${node.text?.fontStyle ?? "?"}`,
      );
    }
...
const isSetKey = (
  name: string,
  value: unknown,
): value is { setName: string; type: string; variants: DumpNodeJson[] } =>
  name !== "_provenance" &&
  !!value &&
  typeof value === "object" &&
...
for (const setName of setKeys) {
  ...
  for (const d of degr)
    degradationsByCode[d.code] = (degradationsByCode[d.code] ?? 0) + 1;
  const rec: SetRecord = {
    setName,
    type: set.type,
    proposed: false,
  ...
  if (processed % 200 === 0)
    console.log(
      `  … ${processed}/${setKeys.length} (${records.filter((r) => r.clean).length} clean so far)`,
    );
}
...
const noteClass = (
  ...
  const agg: ClassAgg = classAgg.get(cls) ?? {
    cls,
    sets: [],
    surfaces: new Set(),
    exampleMessages: new Map(),
  };
  if (!agg.sets.includes(setName)) agg.sets.push(setName);
  agg.surfaces.add(surface);
  if (!agg.exampleMessages.has(setName))
    agg.exampleMessages.set(setName, message);
  classAgg.set(cls, agg);
};
for (const rec of records) {
  if (rec.skipClass)
    noteClass(rec.skipClass, rec.setName, "propose", rec.skipReason ?? "");
  rec.violationClasses.forEach((cls, i) =>
    noteClass(cls, rec.setName, "referee", rec.violations[i] ?? ""),
  );
  for (const r of rec.emitterRefusals)
    for (const cls of r.classes)
      noteClass(cls, rec.setName, r.emitter, r.message);
  if (rec.internalError)
    noteClass(
      "census-internal-error",
      rec.setName,
      "census",
      rec.internalError,
    );
}
const rankedClasses = [...classAgg.values()].sort(
  (a, b) => b.sets.length - a.sets.length || a.cls.localeCompare(b.cls),
);
...
for (const rec of records)
  for (const [code, n] of Object.entries(rec.degradationsByCode))
    degradationTotals[code] = (degradationTotals[code] ?? 0) + n;
for (const d of batchDegradations)
  degradationTotals[d.code] = (degradationTotals[d.code] ?? 0) + 1;
...
function representative(
  candidates: SetRecord[],
  score: (r: SetRecord) => number,
): string | null {
  ...
  for (const r of candidates) {
    if (
      !best ||
      score(r) > score(best) ||
      (score(r) === score(best) && r.setName < best.setName)
    )
      best = r;
  }
...
const PATTERN_CLASSES: Array<{ cls: string; pick: () => string | null }> = [
  {
    ...
    pick: () =>
      representative(
        records.filter((r) => r.features.arrayOfCandidateGroups > 0),
        (r) => r.features.arrayOfCandidateGroups,
      ),
  },
  {
    ...
    pick: () =>
      representative(
        records.filter((r) => r.repeatParts > 0),
        (r) => r.repeatParts,
      ),
  },
  {
    ...
    pick: () =>
      representative(
        records.filter((r) => r.features.slotPlaceholderNodes > 0),
        (r) => r.features.slotPlaceholderNodes,
      ),
  },
  {
    ...
    pick: () =>
      representative(
        records.filter((r) => r.features.instanceSwapRefs > 0),
        (r) => r.features.instanceSwapRefs,
      ),
  },
  {
    ...
    pick: () =>
      representative(
        records.filter((r) => r.features.themeLikeAxes.length > 0),
        (r) => r.features.variants,
      ),
  },
  {
    ...
    pick: () =>
      representative(
        records.filter((r) => r.features.stateLikeAxes.length > 0),
        (r) => r.features.variants,
      ),
  },
  {
    ...
    pick: () =>
      representative(
        records.filter((r) => r.features.depth >= 3),
        (r) => r.features.depth,
      ),
  },
];
...
const summary = {
  ...
  factsCarried: {
    ...
    tokenRefsMedian: median(
      records.filter((r) => r.proposed).map((r) => r.tokenRefsCarried),
    ),
    tokenRefsMedianComponentSets: median(
      componentSets.filter((r) => r.proposed).map((r) => r.tokenRefsCarried),
    ),
    ...
    JSON.stringify(
      {
        ...
        classes: rankedClasses.map((c) => ({
          ...
          examples: c.sets
            .slice(0, 3)
            .map((s) => ({ set: s, message: c.exampleMessages.get(s) ?? "" })),
        })),
        fixtures,
        sets: records,
      },
...
const pct = (n: number, of = records.length) =>
  `${((100 * n) / Math.max(1, of)).toFixed(1)}%`;
...
md.push("");
md.push(
  `- Dump: \`${dumpPath}\` (dump v1.4, recaptured ${(dump._provenance as { extractedAt?: string })?.extractedAt ?? "?"} — the \`_variables\` resolved-value channel rides along)`,
);
md.push(
...
md.push(
  `- Skips at proposal: **${skipped.length}** — the id-sanitize + batch-isolation chain (3982ac2) holds on live data.`,
);
if (batch.notes.length > 0)
  md.push(
    `- Batch notes: ${batch.notes.map((n) => `"${n.split(" — ")[0]}"`).join("; ")} (named, never merged).`,
  );
md.push("");
md.push("## Headline");
md.push("");
...
md.push("|---|---:|---:|---:|");
md.push(
  `| **whole kit** | ${clean.length} | ${notClean.length} | **${pct(clean.length)}** |`,
);
md.push(
  `| plain COMPONENTs (icon-class, single variant) | ${cleanPlain.length} | ${plainComponents.length - cleanPlain.length} | ${pct(cleanPlain.length, plainComponents.length)} |`,
);
md.push(
  `| **real COMPONENT_SETs (variant axes)** | ${cleanComponentSets.length} | ${componentSets.length - cleanComponentSets.length} | **${pct(cleanComponentSets.length, componentSets.length)}** |`,
);
md.push("");
md.push(
...
md.push("");
const failureClasses = rankedClasses.filter(
  (c) => !c.cls.startsWith("pattern-"),
);
if (failureClasses.length === 0) {
  ...
  for (const [i, agg] of failureClasses.entries()) {
    const flag = FIX_IN_FLIGHT.has(agg.cls)
      ...
      : "";
    md.push(
      `### ${i + 1}. \`${agg.cls}\` — ${agg.sets.length} sets (${pct(agg.sets.length)} of kit, ${pct(agg.sets.filter((s) => records.find((r) => r.setName === s)?.type === "COMPONENT_SET").length, componentSets.length)} of COMPONENT_SETs)${flag}`,
    );
    md.push("");
    ...
    if (setNames.length > 3)
      md.push(`(all ${setNames.length}: ${setNames.join(", ")})`);
    md.push("");
  }
}
...
const FIXED_CLASSES: Array<{
  ...
  },
  {
    ...
    fixtureSet: "Note",
    text: 'digit-led property spellings get the componentIdSlug digit-led discipline applied to prop code bindings — the deterministic "p" prefix ("2nd paragraph" → `p2ndParagraph`) with a named note; the figma binding keeps the original spelling.',
  },
];
const fixedNow = FIXED_CLASSES.filter(
  (f) => !rankedClasses.some((c) => c.cls === f.cls),
);
...
const featRow = (label: string, f: (r: SetRecord) => boolean) => {
  ...
  md.push(
    `| ${label} | ${n} | ${pct(n)} | ${pct(nSets, componentSets.length)} |`,
  );
};
featRow(
  "plain COMPONENT (single variant, mostly icons)",
  (r) => r.type !== "COMPONENT_SET",
);
...
featRow(
  "arrayOf candidates (≥3 same-named siblings)",
  (r) => r.features.arrayOfCandidateGroups > 0,
);
featRow(
  "repeat collections PROPOSED (P9, schema v12)",
  (r) => r.repeatParts > 0,
);
featRow(
  "slot-placeholder names (_Slot*/placeholder/swap)",
  (r) => r.features.slotPlaceholderNodes > 0,
);
featRow("INSTANCE_SWAP properties", (r) => r.features.instanceSwapRefs > 0);
featRow(
  "boolean-visibility pairs (Show X)",
  (r) => r.features.booleanVisibilityRefs > 0,
);
featRow("state-like variant axis", (r) => r.features.stateLikeAxes.length > 0);
featRow(
  "theme/mode/density-like axis",
  (r) => r.features.themeLikeAxes.length > 0,
);
...
md.push("");
md.push(
  `Composite share: ${compositeSets} sets (${pct(compositeSets)}) nest at least one instance — the kit is primitive-heavy overall (icons), but ${pct(componentSets.filter((r) => r.features.instances > 0).length, componentSets.length)} of the real COMPONENT_SETs are composites.`,
);
md.push("");
...
md.push("|---|---:|");
for (const [code, n] of Object.entries(degradationTotals).sort(
  (a, b) => b[1] - a[1],
))
  md.push(`| ${code} | ${n} |`);
md.push("");
...
const hitLines = failureClasses.slice(0, 4).map((agg) => {
  ...
  const fSets =
    agg.sets.filter(
      (s) => records.find((r) => r.setName === s)?.type === "COMPONENT_SET",
    ).length / Math.max(1, componentSets.length);
...
md.push("");
for (const f of fixtures)
  md.push(`- \`${f.file}\` — class \`${f.cls}\`, set **${f.setName}**`);
md.push("");
if (fixedNow.length > 0) {
  md.push(
  ...
  for (const f of fixedNow)
    md.push(
      `- \`${f.fixture}\` — class \`${f.cls}\` (fixed), set **${f.fixtureSet}**`,
    );
  md.push("");
}
if (writeOutputs)
  writeFileSync(path.join(outDir, "CENSUS.md"), md.join("\n") + "\n");
...
for (const agg of rankedClasses.slice(0, 12))
  console.log(
    `  ${String(agg.sets.length).padStart(5)}  ${agg.cls}  [${[...agg.surfaces].sort().join(", ")}]`,
  );
console.log(
  writeOutputs
