...
## 2026-08-22 → 08-23 — One truth, named or carried, a held-out exam, a published core, schema 17

Seven PRs (#18–#24) in two days, each with its receipt in the commit
message; docs/23 §D.12–D.28 register every closure with its gate.

- **One truth (Phase 0).** The eval record carries the commit it measured and
  `eval:record:check` fails the full lane row-by-row when CI disagrees;
  `schema:fresh`; `ci:lanes` expands `maintain` so every leaf provably runs
  in a lane; `maintain:visual` in the catalog-visual lane with the
  `FIGMA_TOKEN` secret; every receipt regenerated by its own command; the
  golden path re-run on a fresh clone at eight stems.
- **Named or carried (Phase 1).** Eight verified silent losses closed with
  their checks: root `attrs` on every code target, WC multi-axis refs,
  child-part state channels, the recovered ToggleSwitch thumb, per-fact
  `codeOnlyFacts` (2,321 named facts across 104 receipts), a visual gate that
  sees geometry, a token runtime that refuses and names, `tokens.css` on
  generate. The conformance fixture's canvas round trip measured at
  **0 SILENT** (`conformance:roundtrip`, 46 cases).
- **The held-out exam (Phase 2).** A hand-built kit through the REST Journey
  A path: 3,556 canvas facts, **295 silent → 2**, wrong-name 8 → 0,
  should-carry 25 → 0; Button recognisable, Card not, every Card loss named;
  the REST route names a missing `file_variables:read` scope once with the
  fix; `generate` refuses per contract; prop/DOM collisions `Omit<>`-ed.
  `parity/receipts/phase-2/FIGMA-DS-EXAM.md`.
- **`@ds-contracts/core` (Phase 3).** The emitter surface and the analysis
  half of `emit-react` as a package that depends on the schema alone;
  `verify:published` generates the Flowbite eight through a Vue emitter built
  from the tarballs.
- **Schema 17.** Every Figma-only field under `bindings.figma`; v16 refuses by
