...
Five of them compare one surface to the contract; `extract --reconcile` lays the two extractions side by side and only classifies. None of them writes to a surface, and none picks a side. The verb-by-verb walk of all five hops, with the refusal sentences quoted verbatim, is [docs/29 — How It Flows](docs/29-how-it-flows.md).
