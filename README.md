...
> - **The canvas→code exam now runs on designers' files.** Twenty-four
>   component sets from two Figma files this repo never wrote — the Altitude
>   Design System and the CBDS UI Kit — observed read-only, went through the
...
- **What it does — [docs/24 — What Works](docs/24-what-works.md).** Generated
  ...
  | determinism | byte-identical | 292 generated files hashed against a golden manifest, no model anywhere in the path |
...
The denominator belongs beside the mean: 113 of those measured components also carry a committed contract, but only 102 belong to the eight libraries with a measured size. For that known-size cohort, those 102 components are **10.0% of the 1015** recorded library units. The other 11 covered components remain in the inventory, excluded from both sides of that fraction. Held captures count as measured but never as covered — a scorecard is not a shipped stem. These are hand-picked slices, and the size units are heterogeneous ([docs/22 §8.3a](docs/22-generality.md#83a-the-published-denominator-is-incoherent--re-measured-both-columns-kept)); the fraction is a coverage proxy, not harmonized whole-library coverage.
...
**Fidelity per captured component is high — 86.4% mean, and every component is listed.** The contract is derived from the browser's measured facts; the emitted result still has the named losses and divergences below. Measured against the original npm package rendering, per prop combination × interaction state, as an exact string match with no tolerance and no whitelist, 133 components across 11 libraries (the measured capture corpus of [docs/24 §2](docs/24-what-works.md) — shadcn/ui, Fluent 2 and Ant Design are in it, this repo's own library is not) score **86.4% mean computed-style equality** (85.8% cell-weighted over 807,307 cells; 63 of 133 at ≥90%, 105 of 133 at ≥80%). [docs/24 §3.1](docs/24-what-works.md) lists all 133 worst-first, with nothing omitted — the worst is 47.6% (shadcn Avatar, pixel-AA-perfect 12/12; the divergence is channel-string spelling). The capture runs twice and refuses if the runs disagree, which catches uncontrolled state, random ids and animation sampling before any of it reaches a contract.
**Coverage per library is partial, and a first pass will not be your whole library.** Within the eight libraries with a measured size, those 102 components are **10.0% of the 1015** recorded library units. The other libraries have no measured size; their captured components cannot enlarge only the numerator. The per-library table and its heterogeneous-unit caveat are [docs/24 §2](docs/24-what-works.md), printed before any fidelity average; the source of those denominators is [docs/22 §8.3](docs/22-generality.md#83-the-coverage-fraction--how-much-of-each-library-is-actually-captured). Budget hours per library for the recon and the config, then machine time for the capture.
...
Requires Node ≥ 20. Several gates drive a real Chromium; see [CONTRIBUTING.md](CONTRIBUTING.md) for the pinned browser setup and lane definitions. If none is found, install it with `npx playwright-core install chromium` or set `PLAYWRIGHT_CHROMIUM_PATH`.
...
Exercise the loop (the full eval suite takes substantially longer than the individual checks):
