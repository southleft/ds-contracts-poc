...
All of it is gated by **163 executable checks** (`npm run eval`) that run the real pipeline in a scratch copy — not mocks.
...
| `evals/` | 163 deterministic checks on the machinery itself: byte-identical regeneration against golden manifests, refusal of illegal contracts, detection of every claimed drift class, convergence after promotion, extraction round-trips. | ✅ |
...
npm run eval     # ④ 163 checks that detection, refusal, and convergence still hold
