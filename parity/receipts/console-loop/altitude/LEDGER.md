...
## 2026-08-09 — CANVAS-DRIFT sweep: 7 of 8 in sync, and the one that drifted was HAND-BUILT

Snapshot: `canvas-drift/LIVE-SNAPSHOT.json` (minted off the Desktop Bridge with
`fileKey` pinned, generated by `scripts/console-loop-canvas-drift-capture.mjs altitude`).
Probe: `npm run console-loop:canvas-drift altitude`.

**7 in-sync / 1 DRIFT. No scored-pass is drifted** — `button`, `chip`, `divider`
and `heading` (this lane's four passes) are each exactly what
`examples/altitude/figma/<stem>.figma.js` would build today.

| stem | verdict | divergence |
|---|---|---|
| `avatar` (5.47, fail) | **DRIFT** | spec binds `minWidth` and `minHeight` → `imported/shared/size-0`; the live cell binds **neither**. And `avatar__badge.badge`'s `width`/`height` resolved `imported/shared/size-8` in the **Tailwind** collection, not Altitude — a cross-library name collision frozen into the node. |
| badge, button, chip, divider, heading, icon-close, link | in-sync | — |

### The regeneration attempt, and what the emitter itself said

`avatar` was carried through the **production regeneration path**, not
hand-patched: the committed 81,000-char script was chunk-uploaded into
`clientStorage['ds_loop_script']` in 5 `figma_execute` calls, **djb2-verified
byte-identical to `examples/altitude/figma/avatar.figma.js` (2191945420)** before
running, then `eval`'d.

**The script refused, by name, and wrote nothing:**

> `skipped: true, reason: "set/standalone shape mismatch (COMPONENT_SET vs isSet=false) — a human retires the old node"`

The live node is a **COMPONENT_SET** (`1:10498`, one child `Variant=Sm` = the
pinned cell `1:10494`); the committed script compiles Avatar as a **standalone
COMPONENT** (`isSet: false`). `resolveComponentIdentity` finds the right node by
`contractId`, sees the shape disagree, and refuses rather than delete-and-recreate
— which would re-mint the node id and key every instance binds to.

**And the live set says outright that no script built it:** its stored
`ds_contracts/specHash` is the literal string **`console-loop-manual-rebuild-v1`**.
This cell was hand-rebuilt. That is the drift class in its purest form, and it
explains the missing `minWidth`/`minHeight` bindings and the Tailwind-collection
badge sizes: a human built the node, so no emitter guard (`FC-THEME-ISO`
included) ever ran over it.

`createdNodeIds` was empty and the node's facts are byte-identical before and
after — the canvas was not touched. **Nothing on the board moves.** `avatar` was
already an honest fail-closed on both instruments and stays one; the lane's floor
of 4 is untouched, and `RATCHET.json` was not edited.

**What would unblock it:** a human retires `1:10498` (or the contract regains its
`isSet` shape), after which the committed script amends in place. Until then the
