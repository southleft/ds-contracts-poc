...
| FC-REF-DEGENERATE-CROP | The content-box trim collapses the pair to a text-only crop, so `pctAAMasked` measures GLYPH RASTER and not component parity — the component's own surface contributes zero pixels | mui `card` 31.87 and `paper` 25.94, both composition-clean. An elevation-0 Paper is white-on-white with no shadow and no border, so its 288×58 / 288×18 box draws NO ink; the 320×96 reference trims to the twelve glyphs of "Card content" / "Paper content" and nothing else | **NOT FIXABLE COMPONENT-SIDE — say so rather than chase it.** The whole delta is serif-vs-sans: the canvas renders Inter Regular 16 (read off the live cell: "Card content" 99px advance) and the reference renders browser-default **Times** (`captured-truth.json` `font-family: "Times"`, because the capture harness sets no document font-family), 83px. 99/83 = 1.19 ≈ the reported `scaleRatio` 1.181, and ink DENSITIES agree (inkΔ 4.7 / 3.8) so the extra width is glyphs, not padding or letterboxing. `font-family` appears in NEITHER contract nor either enriched sidecar — fuse drops it as inherited — so the contract cannot express the reference's typeface and the emitter's Inter is unconstrained. The real fix is lane-wide and harness-side (give the capture page a document font-family, or mint `font-family` as a token), which also moves Dialog and Accordion — its own round. Do NOT retarget to elevation-1/3/8: `mui/LEDGER.md:234-237` already refuted that as a scaleRatio-4.62 composition artifact |
