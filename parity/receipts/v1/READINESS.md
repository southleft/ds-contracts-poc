...
- **commit:** `a897620ec62875ca52738c9848c32dab454e194a`
...
- **prep:** ✔ `npm --prefix packages/schema run build` 1s · ✔ `npm --prefix packages/core run build` 1s · ✔ `npm --prefix packages/cli run build` 1s · ✔ `npm --prefix packages/emitter-web-components run build` 0s · ✔ `npm run build:lib` 2s · ✔ `npm run plugin:zip` 0s
...
| V1-JOURNEY-02 | **GREEN** | ✔ `npm run test:onboarding` && ✔ `npm run paste:check` && ✔ `npm run plugin:check` && ✔ `npm run first-party-bundle:check` && ✔ `npm run maintain` | 78 | — |
...
| V1-CLASS-03 | **GREEN** | ✔ `npm run recipe:button:check` && ✔ `npm run recipe:input-field:check` && ✔ `npm run recipe:combobox:check` && ✔ `npm run recipe:table:check` && ✔ `npm run recipe:calendar:check` && ✔ `npm run recipe:pivot-status:check` | 20 | ✔ 32-recipe-ir-pivot.md#e4-applied-2026-08-30 |
...
| V1-COMPAT-02 | **GREEN** | ✔ `npm run schema` && ✔ `npm run schema:fresh` && ✔ `npm run contracts:migrate:check` && ✔ `npm run slot-constraints:check` | 9 | ✔ ../CONTRIBUTING.md#contract-change-policy<br>human: release PR includes a contract-change classification using CONTRIBUTING § Contract change policy |
| V1-COMPAT-03 | **RED** | ✖ `npm run eval` && · `npm run eval:record:check` && · `npm run generation:atomic:check` && · `npm run provenance:check` && · `npm run figma:fresh` && · `npm run verify:catalog` | 3914 | — |
...
| V1-EVID-03 | **GREEN** | ✔ `npm run conformance` && ✔ `npm run conformance:roundtrip` && ✔ `npm run conformance:canvas` ⟨reused V1-JOURNEY-01⟩ && ✔ `npm run dagger:census` && ✔ `npm run closure:check` | 40 | — |
| V1-EVID-04 | **GREEN** | ✔ `npm run snapshot:schema:check` && ✔ `npm run canvas:binding:check` && ✔ `npm run variant-drift:check` | 14 | ✔ ../parity/receipts/live-figma-variant-drift.md |
...
| V1-CI-01 | **RED** | ✔ `npm run ci:lanes` && ✔ `npm run ci:lane fast` && ✖ `npm run ci:lane full` && · `npm run ci:lane catalog-visual` && · `npm run test:v1-definition` && · `npm run v1:definition:check` && · `npm run provenance:check` && · `npm run eval:record:check` | 7469 | — |
| V1-CI-02 | **GREEN** | ✔ `npm run prep:core` && ✔ `npm --prefix packages/schema run build` ⟨reused prep⟩ && ✔ `npm --prefix packages/cli run build` ⟨reused prep⟩ && ✔ `npm --prefix packages/emitter-web-components run build` ⟨reused prep⟩ && ✔ `npm run build:lib` ⟨reused prep⟩ && ✔ `npm run plugin:zip` ⟨reused prep⟩ && ✔ `npm run build:playground` && ✔ `npm run site:build` && ✔ `npm run publish:check` && ✔ `npm run verify:package` && ✔ `npm run verify:published` && ✔ `npm run schema:fresh` ⟨reused V1-COMPAT-02⟩ && ✔ `npm run figma:fresh` && ✔ `npm run generated:fresh` && ✔ `npm run verify:catalog` && ✔ `npm run catalog:visual:check` ⟨reused V1-EVID-05⟩ | 26 | — |
| V1-REL-01 | **RED** | — (evidence only) | 33 | human: the release PR contains a complete P0/P1 audit ledger with task ID, closing commit, acceptance command, and result<br>ledger: 60 rows — closed 56, refuted 2, open-human 2, red 0 ([AUDIT-LEDGER.md](AUDIT-LEDGER.md))<br>audit ledger: AUD-U17 OPEN-HUMAN, AUD-U22 OPEN-HUMAN |
| V1-REL-02 | **RED** | ✔ `npm --prefix packages/cli run build` ⟨reused prep⟩ && ✔ `npm run publish:check` ⟨reused V1-CI-02⟩ && ✔ `npm run verify:published` ⟨reused V1-CI-02⟩<br>after publish: ✔ `npm run plugin:zip` ⟨reused prep⟩ && ✔ `npm run build:playground` ⟨reused V1-CI-02⟩ && ✔ `npm run site:build` ⟨reused V1-CI-02⟩ && ✖ `npm run deploy:check` | 734 | — |
...
### V1-COMPAT-03 — `npm run eval` (exit 1, 3914s)
...
225/230 evals passed — evals/results.json (commit a897620e)
```

### V1-CI-01 — `npm run ci:lane full` (exit 1, 6984s)

```
  ✔   38.4s  npm run extract:computed:portal:check
  ✔    2.8s  npm run extract:computed:remount:check
  ✔    0.7s  npm run extract:computed:viewport:check
  ✔    4.9s  npm run extract:computed:geometry:census
  ✔    0.3s  npm run extract:computed:pseudo-decor-rotate:check
  ✔    0.3s  npm run extract:computed:pseudo-decor-size:check
  ✔    0.4s  npm run extract:computed:font-slant:check
  ✔   11.9s  npm run ua-baseline:check
  ✔    1.9s  npm run ua-baseline:present:check
  ✔    0.3s  npm run scrim-demotion:check
  ✔    0.4s  npm run extract:figma:base:check
  ✔    0.2s  npm run extract:figma:gauntlet:noteclass:check
  ✔    0.3s  npm run extract:figma:cbds:check
  ✔    0.7s  npm run extract:figma:cbds:batch:check
  ✔    0.3s  npm run extract:figma:cbds:bridge:check
  ✔    0.3s  npm run extract:figma:canvas:check
  ✔    0.3s  npm run extract:figma:dialog:check
  ✔    0.3s  npm run extract:figma:composite:check
  ✔    0.3s  npm run extract:figma:cross:check
  ✔    0.3s  npm run extract:figma:partstate:check
  ✔    0.3s  npm run extract:figma:tooltip:check
  ✔    0.3s  npm run extract:figma:overlap:check
  ✔    0.3s  npm run extract:figma:wrap:check
  ✔    0.3s  npm run extract:figma:gradient:check
  ✔    0.3s  npm run extract:figma:constraints:check
  ✔    0.3s  npm run extract:figma:repeat:check
  ✔    0.3s  npm run extract:figma:theme:check
  ✔    1.5s  npm run v1:readiness -- --trust-lanes --pre-release

✖ 1/49 gate(s) failed in lane "full".
```

### V1-REL-02 — `npm run deploy:check` (exit 1, 734s)
...
✘ 3 deployed surface(s) DIVERGE from the local build:
  ...
  - spec site STALE: /get-started/ live is 40276 bytes (sha fdf98969ffe6…), local build is 41936 bytes (sha 13d52a2fe523…)
