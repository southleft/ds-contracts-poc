...
- **commit:** `71c22cd5a18dd17f34a88c85beeea8616ccf35d3`
...
- **prep:** ✔ `npm --prefix packages/schema run build` 1s · ✔ `npm --prefix packages/core run build` 1s · ✔ `npm --prefix packages/cli run build` 1s · ✔ `npm --prefix packages/emitter-web-components run build` 0s · ✔ `npm run build:lib` 2s · ✔ `npm run plugin:zip` 1s

**Tally.** GREEN 19 · RED 5 — 24 rows.

| row | state | command | seconds | evidence |
|---|---|---|---|---|
| V1-SCOPE-01 | **GREEN** | ✔ `npm run docs:check` | 1 | ✔ 23-known-limitations.md#a4-out-of-scope-by-decision--not-gaps<br>human: Known Limitations §A.4 remains linked from the release notes |
| V1-JOURNEY-01 | **GREEN** | ✔ `npm run plugin:ui-check` && ✔ `npm run extract:figma:roundtrip:uui` && ✔ `npm run ledger:fresh` && ✔ `npm run conformance:canvas` | 16 | ✔ parity/receipts/phase-2/FIGMA-DS-EXAM.md |
| V1-JOURNEY-02 | **GREEN** | ✔ `npm run test:onboarding` && ✔ `npm run paste:check` && ✔ `npm run plugin:check` && ✔ `npm run first-party-bundle:check` && ✔ `npm run maintain` | 83 | — |
...
| V1-JOURNEY-04 | **GREEN** | ✔ `npm run recipe:canvas-to-code:held-out:check` | 3 | ✔ 32-recipe-ir-pivot.md#merge-execution-2026-08-30 |
| V1-CLASS-01 | **GREEN** | ✔ `npm run capability:fresh` && ✔ `npm run extract:computed:drift` | 1 | ✔ 23-known-limitations.md#c11-which-component-archetypes-are-proven--the-actionable-cut<br>✔ 23-known-limitations.md#d32-the-two-acceptance-rows-that-were-red-on-the-commit-itself--closed |
...
| V1-CLASS-03 | **GREEN** | ✔ `npm run recipe:button:check` && ✔ `npm run recipe:input-field:check` && ✔ `npm run recipe:combobox:check` && ✔ `npm run recipe:table:check` && ✔ `npm run recipe:calendar:check` && ✔ `npm run recipe:pivot-status:check` | 23 | ✔ 32-recipe-ir-pivot.md#e4-applied-2026-08-30 |
...
| V1-COMPAT-02 | **GREEN** | ✔ `npm run schema` && ✔ `npm run schema:fresh` && ✔ `npm run contracts:migrate:check` && ✔ `npm run slot-constraints:check` | 10 | ✔ ../CONTRIBUTING.md#contract-change-policy<br>human: release PR includes a contract-change classification using CONTRIBUTING § Contract change policy |
| V1-COMPAT-03 | **RED** | ✖ `npm run eval` && · `npm run eval:record:check` && · `npm run generation:atomic:check` && · `npm run provenance:check` && · `npm run figma:fresh` && · `npm run verify:catalog` | 3484 | — |
...
| V1-EVID-03 | **GREEN** | ✔ `npm run conformance` && ✔ `npm run conformance:roundtrip` && ✔ `npm run conformance:canvas` ⟨reused V1-JOURNEY-01⟩ && ✔ `npm run dagger:census` && ✔ `npm run closure:check` | 50 | — |
| V1-EVID-04 | **GREEN** | ✔ `npm run snapshot:schema:check` && ✔ `npm run canvas:binding:check` && ✔ `npm run variant-drift:check` | 15 | ✔ ../parity/receipts/live-figma-variant-drift.md |
| V1-EVID-05 | **GREEN** | ✔ `npm run catalog:visual:check` && ✔ `npm run maintain:visual` | 29 | — |
...
| V1-CI-01 | **RED** | ✔ `npm run ci:lanes` && ✖ `npm run ci:lane fast` && · `npm run ci:lane full` && · `npm run ci:lane catalog-visual` && · `npm run test:v1-definition` && · `npm run v1:definition:check` && · `npm run provenance:check` && · `npm run eval:record:check` | 585 | — |
| V1-CI-02 | **GREEN** | ✔ `npm run prep:core` && ✔ `npm --prefix packages/schema run build` ⟨reused prep⟩ && ✔ `npm --prefix packages/cli run build` ⟨reused prep⟩ && ✔ `npm --prefix packages/emitter-web-components run build` ⟨reused prep⟩ && ✔ `npm run build:lib` ⟨reused prep⟩ && ✔ `npm run plugin:zip` ⟨reused prep⟩ && ✔ `npm run build:playground` && ✔ `npm run site:build` && ✔ `npm run publish:check` && ✔ `npm run verify:package` && ✔ `npm run verify:published` && ✔ `npm run schema:fresh` ⟨reused V1-COMPAT-02⟩ && ✔ `npm run figma:fresh` && ✔ `npm run generated:fresh` && ✔ `npm run verify:catalog` && ✔ `npm run catalog:visual:check` ⟨reused V1-EVID-05⟩ | 38 | — |
| V1-REL-01 | **RED** | — (evidence only) | 30 | human: the release PR contains a complete P0/P1 audit ledger with task ID, closing commit, acceptance command, and result<br>ledger: 60 rows — closed 56, refuted 2, open-human 2, red 0 ([AUDIT-LEDGER.md](AUDIT-LEDGER.md))<br>audit ledger: AUD-U17 OPEN-HUMAN, AUD-U22 OPEN-HUMAN |
| V1-REL-02 | **RED** | ✔ `npm --prefix packages/cli run build` ⟨reused prep⟩ && ✔ `npm run publish:check` ⟨reused V1-CI-02⟩ && ✔ `npm run verify:published` ⟨reused V1-CI-02⟩<br>after publish: ✔ `npm run plugin:zip` ⟨reused prep⟩ && ✔ `npm run build:playground` ⟨reused V1-CI-02⟩ && ✔ `npm run site:build` ⟨reused V1-CI-02⟩ && ✖ `npm run deploy:check` | 731 | — |
...
### V1-COMPAT-03 — `npm run eval` (exit 1, 3484s)
...
225/230 evals passed — evals/results.json (commit 71c22cd5)
```

### V1-CI-01 — `npm run ci:lane fast` (exit 1, 584s)

```
  ✔    0.7s  npm run placeholder-ink:check
  ✔    4.2s  npm run first-party-bundle:check
  ✔    3.5s  npm run code-connect:check
  ✔    1.7s  npm run prop-collision:check
  ✔    0.7s  npm run playground:flow-check
  ...
  ✔   13.2s  npm run census:check -- --phase full --allow-red-verdicts
  ✔   14.8s  npm run census:check -- --self-test
  ✔    1.0s  npm run canvas:usable:check
  ✔    1.2s  npm run canvas:usable:self-test
  ✔   16.3s  npm run census:check -- --phase design-to-code
  ✔    0.7s  npm run first-pass:check
  ✔    1.3s  npm run first-pass:check -- --self-test
  ✔    3.3s  npm run render-browser:check
  ✔    1.9s  npm run mixed-browser:check
  ✔   11.3s  npm run door-register:check
  ✔   13.8s  npm run door-register:self-test
  ✔    0.4s  npm run lowering:check
  ✔    1.8s  npm run lowering:check -- --self-test
  ✔    0.5s  npm run grammar-coverage:check
  ✔    0.8s  npm run grammar-coverage:check -- --self-test
  ✔    3.6s  npm run corpus:reproducible:check
  ✔    0.5s  npm run corpus:reproducible:check -- --self-test
  ✔    1.7s  npm --prefix packages/cli run test
  ✔    2.6s  npm --prefix packages/cli run coverage

✖ 2/183 gate(s) failed in lane "fast".
```

### V1-REL-02 — `npm run deploy:check` (exit 1, 731s)
...
✘ 3 deployed surface(s) DIVERGE from the local build:
  - plugin zip STALE: live is 942148 bytes (sha af19cc985469…), local build is 1117430 bytes (sha 3f9947ab050e…) — a designer downloading today gets a different engine than this repo builds
  - playground STALE: live index references [/assets/index-C1ojNmZG.js, /assets/index-oTRYTN6T.css, /assets/rolldown-runtime-aKtaBQYM.js], local build references [/assets/index-CfaJLxBU.css, /assets/index-Cy7tnmYu.js, /assets/rolldown-runtime-aKtaBQYM.js] — vite renames every chunk on any content change, so these are different builds
  - spec site STALE: /get-started/ live is 40276 bytes (sha fdf98969ffe6…), local build is 41936 bytes (sha 07e42e8c72d1…)
