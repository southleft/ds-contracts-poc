...
## 6a. Three "checks" that are not checks

Laning the boilerplate archetypes surfaced this, and I nearly shipped it into CI.

`build-<archetype>-live-proof-vN.ts` writes its evidence **unconditionally at
module level**; `--check` only appends assertions afterwards. There is no
read-only mode. For ten archetypes that is invisible — the regenerated bytes are
identical. For **avatar, badge and chip** it is not: their receipts carry
`liveFigma: true` from an actual mint, the builder hardcodes
`liveFigma: false`, and running the "check" silently **downgrades a recorded
live-mint fact while exiting 0**.

Had I laned all thirteen, CI would have quietly reverted those three receipts on
every run. The ten safe ones are laned; the three are excluded with that reason
until the builders gain a real `--check` that compares instead of writing — the
way `build-reader-artifacts.ts --check` already does.
