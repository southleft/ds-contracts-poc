# First-pass exam — does it work on the first try, untouched?

GENERATED by `npm run first-pass:check -- --write-receipt` (scripts/first-pass-check.ts) — do not edit. Byte-stable: exams in registry order, sets in MANIFEST order; the date and the engine SHA in every row are RECORDED INTO THE EXAM'S MANIFEST at run time and rendered from it — this file never reads the clock, the environment, or git.

## The bar

Owner, 2026-08-24: *"I don't think I've seen one successful pass on the first try."* Every green number this repo has recorded was won by a heal loop. FIRST-PASS quality — the documented chain run end to end with **zero human and zero agent intervention and no retry** — is a different claim from end-state quality, and until this receipt existed it had never been measured. A REFUSAL is an honest outcome and is recorded as such; it is not counted as a completed chain, and it is not counted as a failure of the run.

See [docs/31 — First-pass](../../../docs/31-first-pass.md) for the metric and how to run an exam.

## The exams

| date | exam | direction | subject | held out | engine SHA | sets | chain complete #1 | stopped: REFUSED | stopped: ERROR | stopped: PENDING | minted | recognisable #1 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 2026-08-25 | selftest-tailwind | code-to-canvas | tailwind | no (self-test) | `4c43433f` | 8 | 0 | — | — | mint (8) | 0 | ungraded |
| 2026-08-25 | selftest-altitude | code-to-canvas | altitude | no (self-test) | `dde1d4f4` | 8 | 7 | mint (1) | — | — | 5 | ungraded |
| 2026-08-25 | selftest-flowbite-live | canvas-to-code | flowbite | no (self-test) | `4c43433f` | 8 | 8 | — | — | — | 0 | ungraded |

`chain complete #1` = **UNMEASURED** when every set of that exam stopped at ERROR — a stage that died without a named refusal. Such a run learned nothing about the engine, so it has no rate at all: `0/N` would read as "the engine failed N times" when the truth is "the harness never got to ask". The ratchet records nothing for an UNMEASURED exam and the gate refuses it by name. Otherwise `chain complete #1` = EVERY stage of the direction returned ok on the single attempt. `stopped: REFUSED` is the engine declining BY NAME — the honest outcome. `stopped: ERROR` is a stage that died without one. `stopped: PENDING` is a stage this harness does not execute at all: `mint` is MCP-DRIVEN (docs/31 §6 — the figma-console bridge speaks MCP over stdio to its own client and WebSocket to plugin clients, and a Node process is neither, so an agent holding the MCP tools performs the write and records its own evidence). It keeps its own column because a chain that ran clean to its last stage and stopped there is a different fact from one the engine refused. `minted` = sets whose bytes reached the canvas ON THIS ATTEMPT, evidenced. It is deliberately NARROWER than `chain complete #1`: a `mint` that the engine SKIPPED as unchanged — its own specHash already equal to the one stamped on the canvas node, so it wrote nothing because an earlier mint had already put those exact bytes there — is a stage that did its job and completes the chain, but it is not a set this attempt minted, and it is not counted here. The per-exam `Mint:` line gives the split. `recognisable #1` = graded blind against the owner's bar; `ungraded` means no `verdict.json` has been written yet, and is never rendered as a number.

## selftest-tailwind

SELF-TEST FIXTURE — flowbite-react@0.12.17 through the documented code→canvas chain (docs/21 §2). NOT held out: the engine has captured this library before, so its numbers measure the HARNESS, not the engine's reach. It exists so the machinery is exercised before an exam is pointed at a library nobody has captured.

Input: `captureConfig` = extract/computed/configs/tailwind.json; `harness` = examples/tailwind/.tw-sandbox; `library` = tailwind; `manifest` = examples/tailwind/ds-library.json; `tokens` = examples/tailwind/tokens/tailwind.dtcg.json, examples/tailwind/tokens/tailwind-minted.dtcg.json. Cell cap 8 per set.

Mint: **PENDING** — no MCP-driven mint evidence is recorded — the canvas write is an MCP-DRIVEN stage: the figma-console bridge speaks MCP over stdio to its own client and WebSocket to plugin clients, and this harness is neither — no port, flag or configuration lets a Node process issue the write, so it is performed by an agent holding the figma-console MCP tools and records its own evidence at parity/receipts/v1/first-pass/selftest-tailwind/mint-evidence.json. Bundle produced, mint pending: the emitted script carries the WRONG-FILE guard on byMp6lt0Ij9b2QbkDGFwBh and is the runnable artifact.

| set | contract id | chain | first stop | stage | message | wall-clock | ref | code | canvas | verdict |
|---|---|---|---|---|---|---|---|---|---|---|
| Button | flowbite.button | stopped | PENDING | mint | runnable script emitted with the WRONG-FILE guard on byMp6lt0Ij9b2QbkDGFwBh; no MCP-driven mint evidence is recorded — the canvas write is an MCP-DRIVEN stage: the figma-console bridge speaks MCP over | 212.0s | 8 | 8 | 0 | ungraded |
| Badge | flowbite.badge | stopped | PENDING | mint | runnable script emitted with the WRONG-FILE guard on byMp6lt0Ij9b2QbkDGFwBh; no MCP-driven mint evidence is recorded — the canvas write is an MCP-DRIVEN stage: the figma-console bridge speaks MCP over | 82.9s | 8 | 8 | 0 | ungraded |
| Card | flowbite.card | stopped | PENDING | mint | runnable script emitted with the WRONG-FILE guard on byMp6lt0Ij9b2QbkDGFwBh; no MCP-driven mint evidence is recorded — the canvas write is an MCP-DRIVEN stage: the figma-console bridge speaks MCP over | 32.5s | 1 | 1 | 0 | ungraded |
| Alert | flowbite.alert | stopped | PENDING | mint | runnable script emitted with the WRONG-FILE guard on byMp6lt0Ij9b2QbkDGFwBh; no MCP-driven mint evidence is recorded — the canvas write is an MCP-DRIVEN stage: the figma-console bridge speaks MCP over | 39.0s | 4 | 4 | 0 | ungraded |
| ToggleSwitch | flowbite.toggleswitch | stopped | PENDING | mint | runnable script emitted with the WRONG-FILE guard on byMp6lt0Ij9b2QbkDGFwBh; no MCP-driven mint evidence is recorded — the canvas write is an MCP-DRIVEN stage: the figma-console bridge speaks MCP over | 56.6s | 6 | 6 | 0 | ungraded |
| HelperText | flowbite.helpertext | stopped | PENDING | mint | runnable script emitted with the WRONG-FILE guard on byMp6lt0Ij9b2QbkDGFwBh; no MCP-driven mint evidence is recorded — the canvas write is an MCP-DRIVEN stage: the figma-console bridge speaks MCP over | 43.7s | 5 | 5 | 0 | ungraded |
| Label | flowbite.label | stopped | PENDING | mint | runnable script emitted with the WRONG-FILE guard on byMp6lt0Ij9b2QbkDGFwBh; no MCP-driven mint evidence is recorded — the canvas write is an MCP-DRIVEN stage: the figma-console bridge speaks MCP over | 42.6s | 5 | 5 | 0 | ungraded |
| Kbd | flowbite.kbd | stopped | PENDING | mint | runnable script emitted with the WRONG-FILE guard on byMp6lt0Ij9b2QbkDGFwBh; no MCP-driven mint evidence is recorded — the canvas write is an MCP-DRIVEN stage: the figma-console bridge speaks MCP over | 32.2s | 1 | 1 | 0 | ungraded |

Images absent, by named reason (a blank cell is never allowed):

- 8× canvas: mint PENDING — no MCP-driven mint evidence is recorded — the canvas write is an MCP-DRIVEN stage: the figma-console bridge speaks MCP over stdio to its own client and WebSocket to plugin clients, and this harness is neither — no port, flag or configuration lets a Node process issue the write, so it is performed by an agent holding the figma-console MCP tools and records its own evidence at parity/receipts/v1/first-pass/selftest-tailwind/mint-evidence.json. Bundle produced, mint pending: the emitted script carries the WRONG-FILE guard on byMp6lt0Ij9b2QbkDGFwBh and is the runnable artifact.

## selftest-altitude

SELF-TEST FIXTURE — altitude-web-components@1.0.2 through the documented code→canvas chain (docs/21 §2). NOT held out. It exists beside selftest-tailwind for one reason: tailwind's chain stops at `promote`, so tailwind alone can never exercise generate → bundle → mint. A library with no authored-facts ledger is the cheapest way to put those stages under the metric too.

Input: `captureConfig` = extract/computed/configs/altitude.json; `harness` = examples/altitude/.altitude-sandbox; `library` = altitude; `manifest` = examples/altitude/ds-library.json; `tokens` = examples/altitude/tokens/altitude.dtcg.json, examples/altitude/tokens/altitude-minted.dtcg.json. Cell cap 8 per set.

Mint: **ok** — MCP-DRIVEN, one attempt per set, 2026-08-25, by agent holding the figma-console MCP tools (figma_execute / exportAsync), Figma Desktop Bridge plugin 1.39.0 on ws port 9225 into Scratch Project (byMp6lt0Ij9b2QbkDGFwBh): 5 wrote to the canvas, 2 were SKIPPED as unchanged (the engine's specHash already matched the canvas node, so no byte moved), 1 REFUSED by name, 0 died without one. Evidence, with the exact message and the sha256 of the bytes actually executed, at parity/receipts/v1/first-pass/selftest-altitude/mint-evidence.json.

| set | contract id | chain | first stop | stage | message | wall-clock | ref | code | canvas | verdict |
|---|---|---|---|---|---|---|---|---|---|---|
| Button | altitude.button | complete | — | — | — | 36.2s | 5 | 5 | 4 | ungraded |
| Badge | altitude.badge | complete | — | — | — | 46.4s | 8 | 8 | 6 | ungraded |
| Chip | altitude.chip | complete | — | — | — | 51.3s | 8 | 8 | 3 | ungraded |
| Link | altitude.link | complete | — | — | — | 28.0s | 4 | 4 | 3 | ungraded |
| Avatar | altitude.avatar | complete | — | — | — | 25.6s | 4 | 4 | 1 | ungraded |
| Heading | altitude.heading | complete | — | — | — | 50.1s | 8 | 8 | 6 | ungraded |
| Divider | altitude.divider | stopped | REFUSED | mint | REFUSED BY NAME: "set/standalone shape mismatch (COMPONENT_SET vs isSet=false) — a human retires the old node". The contract emits altitude.divider as a STANDALONE COMPONENT (isSet=false); the node al | 14.2s | 2 | 2 | 0 | ungraded |
| IconClose | altitude.iconclose | complete | — | — | — | 33.6s | 8 | 8 | 7 | ungraded |

Images absent, by named reason (a blank cell is never allowed):

- 1× canvas: info-unset: the contract pins Type=Squared (statePreviewAxis.pinned.Type="Squared"), so the minted set carries no Type-unset cell; the Type=Default variants present on the canvas are the earlier mint's leftovers the amend reported as extraVariants, not this run's output
- 1× canvas: secondary-unset: the contract pins Type=Squared (statePreviewAxis.pinned.Type="Squared"), so the minted set carries no Type-unset cell; the Type=Default variants present on the canvas are the earlier mint's leftovers the amend reported as extraVariants, not this run's output
- 1× canvas: sm-on: hasBadge is minted as a Figma BOOLEAN COMPONENT PROPERTY ("Show HasBadge", default false), not as a variant, so there is no separate node to export for the on state; only the default (off) node exists on the canvas
- 1× canvas: success-unset: the contract pins Type=Squared (statePreviewAxis.pinned.Type="Squared"), so the minted set carries no Type-unset cell; the Type=Default variants present on the canvas are the earlier mint's leftovers the amend reported as extraVariants, not this run's output
- 1× canvas: unset-bold: the minted set enumerates the 6 declared Variant values (Display Lg/Md/Sm, Lg, Md, Sm) and has no cell for the capture's unset Variant plane (FC-UNSET-PLANE-UNDRAWN)
- 1× canvas: unset-default: the minted set enumerates the 4 declared Variant values (Info/Success/Warning/Danger) and has no cell for the capture's unset Variant plane (FC-UNSET-PLANE-UNDRAWN)
- 1× canvas: unset-dot: the minted set enumerates the 4 declared Variant values (Info/Success/Warning/Danger) and has no cell for the capture's unset Variant plane (FC-UNSET-PLANE-UNDRAWN)
- 1× canvas: unset-off: the minted component carries the single declared size (Variant=Sm) and has no cell for the capture's unset size plane (FC-UNSET-PLANE-UNDRAWN)
- 1× canvas: unset-on: the minted component carries the single declared size (Variant=Sm) and has no cell for the capture's unset size plane (FC-UNSET-PLANE-UNDRAWN)
- 1× canvas: unset-regular: the minted set enumerates the 6 declared Variant values (Display Lg/Md/Sm, Lg, Md, Sm) and has no cell for the capture's unset Variant plane (FC-UNSET-PLANE-UNDRAWN)
- 1× canvas: unset-squared: the minted set has no cell for the capture's unset Variant plane (FC-UNSET-PLANE-UNDRAWN)
- 1× canvas: unset-unset: the minted set has no cell for the capture's unset Variant plane (FC-UNSET-PLANE-UNDRAWN)
- 1× canvas: unset: mint REFUSED — set/standalone shape mismatch (COMPONENT_SET vs isSet=false); this run wrote no canvas node, and the COMPONENT_SET standing at 39:6751 is an EARLIER mint's output, so exporting it would be passing another run's pixels off as this attempt's
- 1× canvas: unset: the minted set enumerates the 3 declared Variant values (Xs/Sm/Lg) and has no cell for the capture's unset Variant plane (FC-UNSET-PLANE-UNDRAWN)
- 1× canvas: unset: the minted set enumerates the 7 declared Size values (Xs..Xxxl) and has no cell for the capture's unset Size plane (FC-UNSET-PLANE-UNDRAWN)
- 1× canvas: unset: the minted set has no cell for the capture's unset plane. The engine names this itself in the run's own code-only facts: root/variant [unset] — "defaultless axis — the library's own rendering when \"variant\" is absent (the capture's base plane, whose tokens ride the parts' base bindings) has no VARIANT cell: the set enumerates the 4 declared values only, and a proposal read back from the canvas will call \"secondary\" the default (FC-UNSET-PLANE-UNDRAWN)"
- 1× canvas: vertical: mint REFUSED — set/standalone shape mismatch (COMPONENT_SET vs isSet=false); this run wrote no canvas node, and the COMPONENT_SET standing at 39:6751 is an EARLIER mint's output, so exporting it would be passing another run's pixels off as this attempt's

## selftest-flowbite-live

SELF-TEST FIXTURE — the eight Flowbite pages of file 59mLQlOMiD5w5za6SUcoO5 through the documented canvas→code chain, LIVE over READ-ONLY REST rather than from the committed fixture the design→code census replays. NOT held out: these are pipeline-drawn, stamped sets the engine has proposed from before. It exists to exercise the live half — the network dump, the image export, the browser — that a fixture replay never touches.

Input: `corpus` = examples/tailwind/tokens/tailwind.dtcg.json, examples/tailwind/tokens/tailwind-minted.dtcg.json; `fileKey` = 59mLQlOMiD5w5za6SUcoO5; `kit` = flowbite; `mode` = exact; `pages` = 120:1942, 120:1981, 120:1985, 120:1998, 120:2003, 120:2016, 120:2049, 120:2112. Cell cap 8 per set.

Mint: **SKIPPED** — canvas→code is READ-ONLY; no exam in this direction writes to any Figma file

| set | contract id | chain | first stop | stage | message | wall-clock | ref | code | canvas | verdict |
|---|---|---|---|---|---|---|---|---|---|---|
| Alert (flowbite.alert) | flowbite.alert | complete | — | — | — | 4.4s | 4 | 4 | 0 | ungraded |
| Kbd | flowbite.kbd | complete | — | — | — | 4.3s | 1 | 1 | 0 | ungraded |
| Label | flowbite.label | complete | — | — | — | 4.1s | 5 | 5 | 0 | ungraded |
| Card (flowbite.card) | flowbite.card | complete | — | — | — | 3.6s | 1 | 1 | 0 | ungraded |
| HelperText | flowbite.helpertext | complete | — | — | — | 4.2s | 5 | 5 | 0 | ungraded |
| ToggleSwitch | flowbite.toggleswitch | complete | — | — | — | 4.5s | 6 | 6 | 0 | ungraded |
| Badge (flowbite.badge) | flowbite.badge | complete | — | — | — | 4.1s | 8 | 8 | 0 | ungraded |
| Button (flowbite.button) | flowbite.button | complete | — | — | — | 4.3s | 8 | 8 | 0 | ungraded |

Images absent, by named reason (a blank cell is never allowed):

- 8× canvas: canvas→code writes nothing to any canvas — the canvas IS the source here, and Figma's own render of it is ref-*.png

## The wave-3 queue — exams the metric has NOT been pointed at

| exam | direction | what it is |
|---|---|---|
| cbds-ui-kit | canvas-to-code | HELD OUT — CBDS UI Kit Demo (WofZT8xaxXuc2Q6Je9S4XE), a hand-built designer kit the engine has never proposed from. READ-ONLY. |
| collegetown | canvas-to-code | HELD OUT — CollegeTown Design System (ioFDU1TszfuEsopivSB6Ox), a shadcn-shaped designer kit the engine has never proposed from. READ-ONLY. |
| eventz | canvas-to-code | HELD OUT — DEMO Eventz Design System (E7oXr98i91HYQGZxA2USOQ) atoms/molecules, never proposed from in a first-pass exam. READ-ONLY. |

These are registered in `extract/figma/census/first-pass.ts` (`EXAM_QUEUE`) and have no packets. Listing them here keeps "never measured" an admission on the record rather than a silence.

## The ratchet

`parity/receipts/v1/first-pass-ratchet.json` records the BEST rate each exam has ever reached. `npm run first-pass:check` recomputes the current rate from the packets and refuses when it is lower without a named reason, and refuses when it is HIGHER and the recorded best was not raised — a stale ratchet is as dead as a falling one.

| exam | metric | best ever | this commit |
|---|---|---|---|
| selftest-tailwind | chain | 0/8 | 0/8 |
| selftest-tailwind | recognisable | 0/0 | 0/0 |
| selftest-altitude | chain | 7/8 | 7/8 |
| selftest-altitude | recognisable | 0/0 | 0/0 |
| selftest-flowbite-live | chain | 8/8 | 8/8 |
| selftest-flowbite-live | recognisable | 0/0 | 0/0 |

Named movements:

| date | exam | metric | from | to | reason |
|---|---|---|---|---|---|
| 2026-08-25 | selftest-flowbite-live | chain | 4/8 | 8/8 | ROSE. The four sets that stopped at generate-react did so because the fresh mint re-derived geometry from DUMP-ROUNDED boxes — imported.kbd.root.width came back "39.92px" where the corpus spells the same measurement "39.9219px" — and generate rightly refuses a slot holding two values. The census healed it with a token prune the documented CLI has no flag for; the exam, which performs no repair, took the refusal. Fixed at the cause instead: core/mint-tokens.ts now asks the corpus what it already spells at each claimed path and carries THAT when the observation is it re-rounded (MintOptions.corpusValueAt / sameUnderDumpRounding), naming every reconciled row in the proposal report. Six leaves reconciled across helper-text/kbd/label/toggle-switch; zero conflicts remain; the census prune is now a CHECKED de-dup that REFUSES a value disagreement instead of resolving it. No exam input, packet or rule changed. |
| 2026-08-25 | selftest-tailwind | chain | 0/8 — stopped: promote (8 REFUSED) | 0/8 — stopped: mint (8 PENDING) | FLAT, SHAPE MOVED, and this is the finding the exam existed to produce. Every set used to die at promote on “authored alert prop dismissable: prop “dismissable” is not exactly one entry of props[]” — the committed capture record carried an `icon` prop, a `dismissable` prop and six anatomy parts that NO committed input produces (hand-written into the record by ac5e6181, back-porting c924c9c2). Those four facts now ride the authored-facts door as explicit, refusable INPUTS (the door’s new `add` operation), and the record carries only what the capture produces. All eight sets now run capture → promote → validate → generate → bundle clean and stop at mint, which is MCP-DRIVEN and not a capability this harness has (docs/31 §6). The rate is still 0/8 because a PENDING mint is not a completed chain, and it will not move until an agent drives the write and records its evidence. |
| 2026-08-25 | selftest-altitude | chain | 0/8 — stopped: mint (8 PENDING) | 7/8 — stopped: mint (1 REFUSED) | ROSE, because the mint stage was DRIVEN for the first time. Nothing in the engine changed: the same chain, capture → promote → validate → generate → bundle, ran green on all 8 sets exactly as before. What changed is that `mint` stopped being a permanently-PENDING architectural note and became a stage with an operator, a procedure and evidence (docs/31 §6). An agent holding the figma-console MCP tools fetched each emitted mint/<component>.figma.js INSIDE the plugin sandbox, verified its sha256 against the hash the harness recorded for those same bytes, and evaluated it EXACTLY ONCE — no retry, no repair, no hand-edited script. `--record-mint` folds that evidence in through the exam's own writers and refuses whole on any of the ways it could lie (a forbidden file key, a dropped or added set, a script sha that is not the one the chain emitted, a minted set with no picture of it, a cell with neither an image nor a named reason, or a stage that is not PENDING — so recording can never be a retry). 5 sets WROTE to the canvas, 2 the engine SKIPPED as unchanged (its own specHash already equal to the one stamped on the canvas node, so no byte moved — they complete the chain and are deliberately NOT counted in `minted`), and 1, Divider, REFUSED BY NAME: "set/standalone shape mismatch (COMPONENT_SET vs isSet=false) — a human retires the old node". THE FINDING THIS RUN EXISTS TO REPORT, and it is not in the numerator: not one of the 8 took the CREATE path. The operator created an empty page `first-pass altitude 2026-08-25`, cleared it and made it current before every run; all 8 scripts ignored it and `newPages` was [] every time. The emitted script owns file layout — it resolves its target by ds_contracts/contractId across EVERY page and amends in place — and there is no --page flag, no file-scoping and no documented way to confine a mint to one page. In the only writable file, which already carried all 8 altitude stems, a first-pass mint can therefore only ever measure the AMEND path. 7/8 is a true statement about the documented chain and a WEAKER statement than it looks: read it beside that fact, not instead of it. |
