...
import { promoteStaticCandidate } from "../extract/static-promotion.js";
...
const sortedSet = (xs: string[]) =>
  [...new Set(xs.map(normalizeName))].sort().join("|");
...
export function runDiagnose(configArg?: string): number {
  ...
  const contractsDir =
    config.diagnose?.contracts ?? path.join(outDir(config), "contracts");
  if (!existsSync(contractsDir)) {
    throw new Error(
      `Contracts directory not found: ${contractsDir} — run \`npm run extract:code\` first, or point diagnose.contracts at your adopted contracts.`,
    );
  }
  const contracts: Contract[] = readdirSync(contractsDir)
    ...
    .map((f) => {
      const raw = JSON.parse(
        readFileSync(path.join(contractsDir, f), "utf8"),
      ) as ProvenancedContract;
      assertContractProvenance(raw, String(raw.id ?? f));
      return ContractSchema.parse(raw);
    });
  if (contracts.length === 0)
    throw new Error(`No *.contract.json in ${contractsDir}`);
  ...
  // Run the same stale-source referee as extraction before this diagnostic
  // can write a green report over an unchanged source that would revert a
  // design-led canonical.
  for (const contract of contracts) {
    const component = codeByName.get(normalizeName(contract.name));
    if (!component || !contract.provenance?.awaitingCodeAdoption) continue;
    const candidate = proposeContract(component, idPrefix(config))
      .contract as ProvenancedContract;
    promoteStaticCandidate(
      contract as unknown as ProvenancedContract,
      candidate,
      component,
    );
  }
  ...
  const design = config.design?.source
    ? loadDesign(config.design.source)
    : null;
  const designByName = design
    ? new Map(design.map((d) => [normalizeName(d.name), d]))
    : null;
  ...
  const age = config.design?.source
    ? designSnapshotAge(config.design.source)
    : null;
  if (design && !age) {
    console.warn(
  ...
  if (age && age.ageDays > MAX_SNAPSHOT_AGE_DAYS) {
    add({
      ...
      remedy:
        "Re-run extract/figma-dump.js in the design file and save it over this dump",
    });
  }
      ...
      for (const p of contract.props) {
        ...
        if (!found) {
          add({
            ...
            remedy:
              "Add the prop in code, or remove it from the contract (major version)",
          });
          continue;
        }
        if (isEnum(p.type)) {
          ...
          if (found.kind === "enum" && want !== got) {
            add({
              ...
              remedy:
                "Promote the code change into the contract, or fix the code",
            });
          }
        }
        ...
        if (
          p.default !== undefined &&
          found.default !== undefined &&
          String(p.default) !== String(found.default)
        ) {
          add({
            surface: "code",
            classification: "mismatch",
    ...
    if (!hit) {
      add({
        ...
        detail:
          ...
          (prefix
            ? ` (also tried without the "${idPrefix(config)}" prefix)`
            : ""),
      ...
      if (isEnum(p.type) && fig.kind === "VARIANT") {
        ...
        if (sortedSet(expected) !== sortedSet(axis[1])) {
          add({
            ...
            remedy:
              "Promote the design change into the contract, or fix the design set",
          });
        }
      } else if (fig.kind === "BOOLEAN" || fig.kind === "TEXT") {
        const pool = fig.kind === "BOOLEAN" ? d.boolProps : d.textProps;
        const hit = pool.find(
          (n) => normalizeName(n) === normalizeName(figProperty),
        );
  ...
  if (!designByName) {
    console.log(
      "ℹ design surface not provided — design checks SKIPPED (set design.source to include them)",
    );
  } else {
    console.log(
      `ℹ design surface: ${design!.length} set(s) from ${config.design!.source}` +
        (age
          ? ` (${age.ageDays.toFixed(1)}d old by ${age.basis})`
          : " (age unverifiable)"),
    );
  ...
  if (findings.length === 0) {
    ...
    for (const f of findings) {
      console.error(
        `  [${f.surface} ${f.classification.toUpperCase()}] ${f.subject} — ${f.detail}`,
      );
    }
...
if (
  process.argv[1] &&
  /diagnose\.(m?[tj]s)$/.test(path.resolve(process.argv[1]))
) {
