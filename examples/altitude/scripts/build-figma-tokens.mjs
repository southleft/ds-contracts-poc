/**
 * Altitude genesis token sync — `node examples/altitude/scripts/build-figma-tokens.mjs`
 *
 * Emits `examples/altitude/figma/00-tokens.figma.js`: a deterministic Figma
 * Plugin-API script that UPSERTS the Altitude theme token set as local
 * variables — one "Altitude" collection, Light/Dark modes — plus the minted
 * `imported.*` layer (slash spelling, the emitters' Figma naming).
 *
 * ALTITUDE MODES ARE REAL: Light = `dist/css/tokens-light.css`, Dark =
 * `dist/css/tokens-dark.css`. They are two separate shipped stylesheets, two
 * complete inventories of the same 339 names — a Figma mode pair with no
 * guessing anywhere.
 *
 * SOURCE-ALIASED minted leaves ({button-primary}, written by promote-floor.mjs
 * from the library's own CSS-variable references) become REAL Figma variable
 * aliases (VARIABLE_ALIAS), not baked literals — so they inherit the target's
 * Light/Dark values and the variables panel shows the semantic link Altitude's
 * own stylesheet declares. That is what makes an Altitude component set actually
 * re-theme when the mode flips.
 *
...
const base = read('tokens/altitude.dtcg.json');
let light = {}, dark = {};
try { light = read('tokens/modes/altitude.light.dtcg.json'); dark = read('tokens/modes/altitude.dark.dtcg.json'); } catch { /* single-mode theme */ }
...
try {
  ...
  walk(read('tokens/altitude-minted.dtcg.json'), []);
...
const script = `// Altitude genesis token sync — GENERATED by examples/altitude/scripts/build-figma-tokens.mjs
