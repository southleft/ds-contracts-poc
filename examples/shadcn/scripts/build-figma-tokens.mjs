/**
 * shadcn genesis token sync — `node examples/shadcn/scripts/build-figma-tokens.mjs`
 *
 * Emits `examples/shadcn/figma/00-tokens.figma.js`: a deterministic Figma
 * Plugin-API script that UPSERTS the shadcn semantic token set as local
 * variables — one "Shadcn" collection, Light/Dark modes (Light = the `:root`
 * block, Dark = the `.dark` class scope; RECON H5) — plus the minted
 * `imported.*` layer (slash spelling, the emitters' Figma naming).
 *
 * SOURCE-ALIASED minted leaves ({primary}, written by promote-floor.mjs from
 * the library's own CSS-variable references — shadcn's `@theme inline` makes
 * utilities reference the semantic var directly) become REAL Figma variable
 * aliases (VARIABLE_ALIAS), not baked literals — so they inherit the
 * target's Light/Dark values and the variables panel shows the semantic link
 * shadcn's own stylesheet declares.
 *
...
const base = read('tokens/shadcn.dtcg.json');
let light = {}, dark = {};
try { light = read('tokens/modes/shadcn.light.dtcg.json'); dark = read('tokens/modes/shadcn.dark.dtcg.json'); } catch { /* single-mode theme */ }
...
try {
  ...
  walk(read('tokens/shadcn-minted.dtcg.json'), []);
...
const script = `// shadcn genesis token sync — GENERATED by examples/shadcn/scripts/build-figma-tokens.mjs
// Deterministic variable UPSERT (re-run safe): one "Shadcn" collection,
...
let col = collections.find((c) => c.name === 'Shadcn');
if (!col) col = figma.variables.createVariableCollection('Shadcn');
...
figma.notify('Shadcn tokens: ' + created + ' created, ' + updated + ' updated (' + TOKENS.length + ' total, ' + aliased + ' aliases, Light/Dark)');
