/**
 * Fluent 2 genesis token sync — `node examples/fluent/scripts/build-figma-tokens.mjs`
 *
 * Emits `examples/fluent/figma/00-tokens.figma.js`: a deterministic Figma
 * Plugin-API script that UPSERTS the Fluent 2 theme token set as local
 * variables — one "Fluent" collection, Light/Dark modes — plus the minted
 * `imported.*` layer (slash spelling, the emitters' Figma naming).
 *
 * THE MODES ARE THEME OBJECTS, NOT A CSS SCOPE. shadcn's Dark mode is the
 * `.dark` class block of a stylesheet; Fluent ships no stylesheet at all
 * (Griffel injects every rule at runtime). Light/Dark here are Fluent's own
 * `webLightTheme` / `webDarkTheme` exports from @fluentui/tokens, built by
 * examples/fluent/scripts/build-tokens.mjs into
 * `tokens/modes/fluent.{light,dark}.dtcg.json`: 459 keys each, IDENTICAL key
 * set, 315 differing values. The base file `tokens/fluent.dtcg.json` is the
 * light theme flattened (459 leaves, no nesting — the reader's var→leaf
 * transform never emits a "."), and each leaf's key is Fluent's CSS custom-
 * property name verbatim, minus the `--`.
 *
 * SOURCE-ALIASED minted leaves ({colorNeutralForeground1}, written by
 * promote-floor.mjs from the library's own CSS-variable references — Griffel's
 * emitted rules reference the theme custom property at point of use) become
 * REAL Figma variable aliases (VARIABLE_ALIAS), not baked literals — so they
 * inherit the target's Light/Dark values and the variables panel shows the
 * semantic link Fluent's own theme declares.
 *
 ...
 *   number → FLOAT               (Fluent's fontWeight* leaves are numbers)
...
const base = read('tokens/fluent.dtcg.json');
let light = {}, dark = {};
try { light = read('tokens/modes/fluent.light.dtcg.json'); dark = read('tokens/modes/fluent.dark.dtcg.json'); } catch { /* single-mode theme */ }
...
try {
  ...
  walk(read('tokens/fluent-minted.dtcg.json'), []);
...
const script = `// Fluent 2 genesis token sync — GENERATED by examples/fluent/scripts/build-figma-tokens.mjs
// Deterministic variable UPSERT (re-run safe): one "Fluent" collection,
...
let col = collections.find((c) => c.name === 'Fluent');
if (!col) col = figma.variables.createVariableCollection('Fluent');
...
figma.notify('Fluent tokens: ' + created + ' created, ' + updated + ' updated (' + TOKENS.length + ' total, ' + aliased + ' aliases, Light/Dark)');
