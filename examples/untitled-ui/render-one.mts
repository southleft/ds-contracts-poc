/** Standalone component render: esbuild the generated component, mount with
 *  given props, screenshot the drawn content.
 *
 *  CLIP v2.0 (FIDELITY_CLIP=union, the default) — the screenshot is clipped to
 *  the UNION of the root's border box and every VISIBLE descendant's, clamped
 *  to the viewport, plus an 8px margin. v1.3 clipped to the ROOT's border box
 *  alone, so ABSOLUTELY-POSITIONED OVERFLOW — the slider's and progress bar's
 *  floating value tooltips, a badge hanging off a corner, any popover — was
 *  cropped out of the PNG and could never be scored, even when it was drawn
 *  correctly and the canvas reference shows it. The union is deliberately a
 *  SUPERSET (an `overflow: hidden` ancestor visually clips a child whose
 *  getBoundingClientRect does not): surplus is harmless because the scorer
 *  content-trims both images afterwards, whereas the v1.3 undercount destroyed
 *  ink before it could be measured. The page also gets a 96px gutter and a
 *  larger viewport so upward/leftward overflow has somewhere to land (measured
 *  worst case on this kit: 42px above the root, slider label=topFloating).
 *
 *  FIDELITY_CLIP=root reproduces the v1.3 harness byte-for-byte (root border
...
const GEN = `${ROOT}/examples/untitled-ui/storybook/src/generated`;
const entry = `
...
`;
const tmp = `${process.env.SCRATCH || '/tmp'}/entry-${comp}.tsx`;
...
const tokens = readFileSync(`${ROOT}/examples/untitled-ui/storybook/src/tokens.css`, 'utf8');
...
const page_path = `${process.env.SCRATCH || '/tmp'}/render-${comp}.html`;
