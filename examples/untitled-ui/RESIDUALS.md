...
**Where the 7.51 points between 92.49 and 100 actually go.** Every number below is READ from a committed artifact when this file is built; none is typed in. Generated by `npx tsx extract/figma/ledger/residuals.ts`. Sources and their content hashes: §7.
...
The scored table is **92.49%** over **537** variants, so **7.51 points** are outstanding. They split three ways.
...
Each set's contribution to the kit's 7.51-point gap is its own shortfall weighted by how many of the 537 scored variants it owns — which is why a set with a small per-variant loss can still dominate the table.
...
|---|---|---|---|---|---|
| `paint-unsupported` | 265 | 115 | 94.04 | +1.55 | avatar, avatar-group, avatar-label-group, badge-base |
| `stroke-weights-nonuniform` | 32 | 32 | 92.24 | -0.25 | button-group-base |
| `vector-geometry-unsupported` | 42 | 32 | 82.61 | -9.88 | progress-circle, tooltip |
| `rotation-unsupported` | 36 | 24 | 81.15 | -11.34 | tooltip |
| `stroke-align-unsupported` | 84 | 22 | 88.81 | -3.68 | avatar-add-button, progress-circle |
| `text-channel-unsupported` | 16 | 8 | 85.25 | -7.24 | progress-circle |
| `stroke-style-unsupported` | 12 | 6 | 96.69 | +4.20 | avatar-add-button |
| `radii-nonuniform` | 4 | 2 | 91.63 | -0.87 | input-field-base |
...
| `(untagged)` | 2,598 | not derivable, and not even classified |
