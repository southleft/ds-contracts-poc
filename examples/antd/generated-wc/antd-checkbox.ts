/**
 * Checkbox — vanilla Custom Element <antd-checkbox> emitted from contract
 * antd.checkbox v0.2.0 by @ds-contracts/emitter-web-components. Do not edit.
 *
 ...
 */
import sheet from './antd-checkbox.css.js';
...
export class CheckboxElement extends HTMLElement {
  ...
  /** Enum prop "checked". */
  get checked(): 'unchecked' | 'checked' | 'indeterminate' {
    return (this.getAttribute('checked') as 'unchecked' | 'checked' | 'indeterminate' | null) ?? 'unchecked';
  }
  set checked(v: 'unchecked' | 'checked' | 'indeterminate' | null) {
  ...
  #view(): string {
    ...
    return `<label part="root" data-checked="${__esc(String(p.checked))}"${p.disabled ? ' data-disabled=""' : ''}><span part="checkbox"><input part="checkbox-input"></input><span part="checkbox-inner"><div part="checkbox-inner-after"></div></span></span><span part="label"><slot>Checkbox</slot></span></label>`;
  }
...
/** Register <antd-checkbox> (idempotent). Runs on import; exported for explicit use. */
export function define(): void {
  if (!customElements.get('antd-checkbox')) customElements.define('antd-checkbox', CheckboxElement);
}
define();
