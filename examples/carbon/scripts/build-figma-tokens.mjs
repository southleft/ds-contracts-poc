/**
 * Carbon genesis token sync — `node examples/carbon/scripts/build-figma-tokens.mjs`
 *
 * Emits `examples/carbon/figma/00-tokens.figma.js`: a deterministic Figma
 * Plugin-API script that UPSERTS the Carbon theme token set as local
 * variables — one "Carbon" collection, Light/Dark modes — plus the minted
 * `imported.*` layer (slash spelling, the emitters' Figma naming).
 *
 * CARBON MODES ARE REAL: Light = the `.cds--white` theme block, Dark =
 * `.cds--g100`. Carbon themes are CLASS SCOPES, so the two blocks are two
 ...
 * Light/Dark values and the variables panel shows the semantic link Carbon's
 * own stylesheet declares. That is what makes a Carbon component set actually
...
const base = read('tokens/carbon.dtcg.json');
let light = {}, dark = {};
try { light = read('tokens/modes/carbon.light.dtcg.json'); dark = read('tokens/modes/carbon.dark.dtcg.json'); } catch { /* single-mode theme */ }
...
try {
  ...
  walk(read('tokens/carbon-minted.dtcg.json'), []);
...
const script = `// Carbon genesis token sync — GENERATED by examples/carbon/scripts/build-figma-tokens.mjs
// Deterministic variable UPSERT (re-run safe): one "Carbon" collection,
...
let col = collections.find((c) => c.name === 'Carbon');
if (!col) col = figma.variables.createVariableCollection('Carbon');
...
figma.notify('Carbon tokens: ' + created + ' created, ' + updated + ' updated (' + TOKENS.length + ' total, ' + aliased + ' aliases, Light/Dark)');
