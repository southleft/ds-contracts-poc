/**
 * CARBON FLOOR PROMOTION — `npx tsx examples/carbon/scripts/promote-floor.mjs`
 ...
 * MUI, unchanged here — the CONTROL-CASE point). Carbon ships a PRECOMPILED
 * stylesheet whose rules name the token each channel binds
 * (var(--cds-button-primary), one indirection hop followed).
 *
 * CARBON FAMILY SPLIT — read the alias count with this in hand: only the
 * colour/border/focus families are custom properties at all. Carbon's TYPE
 * tokens are referenced but never defined by the compiled CSS (80 such refs,
 * a Sass opt-in), and spacing/motion are not custom properties in the first
 * place (0 defined). So a Carbon alias is almost always a COLOUR, and a
 * literal-kept font-size or padding is the LIBRARY's shape, not a reader gap. The capture verified those references against
 ...
 * it changes SEMANTICS: the Figma variables and emitted code reference Carbon's
...
const OUT = path.join(REPO, 'extract', 'computed', 'out', 'carbon');

// CARBON round 1 — the ten components of the control case (capture dirs are
// extract/computed/out/carbon/<config component name lowercased>).
const COMPONENTS = ['button', 'tag', 'checkbox', 'toggle', 'textinput', 'inlinenotification', 'accordion', 'tabs', 'modal', 'iconbutton'];
...
// it. Capture-dir names come from the capture config (out/carbon/<comp.name
// lowercased>); this is the one place the two spellings differ.
// CARBON: the emitted script stem is kebab(contract.name) — TextInput →
// text-input, InlineNotification → inline-notification, IconButton →
// icon-button — while the capture dir is the flat lowercase name. The compile
...
const dtcg = JSON.parse(readFileSync(path.join(EX, 'tokens', 'carbon.dtcg.json'), 'utf8'));
...
for (const name of COMPONENTS) {
  ...
  // assets — the figma compile (`--icons examples/carbon/assets/icons`) emits
  ...
  contract.description =
    `${contract.description} FLOOR-PROMOTED (examples/carbon/scripts/promote-floor.mjs): ` +
    `${path.basename(src)} — computed-capture truth; minted leaves source-aliased to Carbon's ` +
...
writeFileSync(path.join(EX, 'tokens', 'carbon-minted.dtcg.json'), JSON.stringify(mintedMerged, null, 2) + '\n');
writeFileSync(
  ...
  `# Carbon minted tokens — promotion receipt\n\nGenerated by \`examples/carbon/scripts/promote-floor.mjs\`.\n\n` +
    `- **${aliased} leaves source-aliased** to Carbon's own CSS-variable-named tokens (value-verified twice: capture + promotion)\n` +
...
console.log(`✔ floor-promoted ${promoted.length} contract(s) → examples/carbon/contracts (v0.2.0): ${promoted.join(', ')}`);
if (promotedAssets.length > 0) console.log(`✔ ${promotedAssets.length} floor-reconstructed icon asset(s) → examples/carbon/assets/icons/: ${promotedAssets.join(', ')}`);
console.log(`✔ minted tree → examples/carbon/tokens/carbon-minted.dtcg.json (${aliased} source-aliased, ${literalKept} literal, ${aliasReceipts.length} named refusals)`);
console.log(
