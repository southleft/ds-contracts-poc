...
const OUT = path.join(REPO, 'extract', 'computed', 'out', 'tailwind');

const COMPONENTS = ['button', 'badge', 'card', 'alert', 'toggleswitch'];
const MINT_SOURCES = COMPONENTS;

// ---- DTCG base (for alias value verification) ----
const dtcg = JSON.parse(readFileSync(path.join(EX, 'tokens', 'tailwind.dtcg.json'), 'utf8'));
...
for (const name of COMPONENTS) {
  ...
  contract.description =
    `${contract.description} FLOOR-PROMOTED (examples/tailwind/scripts/promote-floor.mjs): ` +
    `${path.basename(src)} — computed-capture truth; minted leaves source-aliased to the library's ` +
...
writeFileSync(path.join(EX, 'tokens', 'tailwind-minted.dtcg.json'), JSON.stringify(mintedMerged, null, 2) + '\n');
writeFileSync(
  path.join(EX, 'tokens', 'MINTED.md'),
  `# MUI minted tokens — promotion receipt\n\nGenerated by \`examples/tailwind/scripts/promote-floor.mjs\`.\n\n` +
    `- **${aliased} leaves source-aliased** to the library's own CSS-variable-named tokens (value-verified twice: capture + promotion)\n` +
