 ...
 */
export const ARCHETYPE_REQUIRED_FACTS: Record<
  Exclude<Archetype, "unmapped">,
  ArchetypeFacts
> = {
  button: {
    required: [
      ...
      },
      {
        ...
        predicate: {
          ...
          of: [
            ...
            {
              kind: "channelAll",
              groups: [WIDTH_GROUP, HEIGHT_GROUP],
              scope: "root",
            },
          ],
        },
      },
      {
        ...
        noun: "surface ink",
        stated:
          "the root carries no background-color, border colour or box-shadow",
  ...
  "checkbox / radio": {
    required: [
      ...
      },
      {
        ...
        stated:
          "no part carries a border colour, border width or background-color",
      ...
      },
      {
        ...
        predicate: ink(
          ["gap", "column-gap", "margin-right", "margin-left", "padding-left"],
          "any",
        ),
      },
    ],
  ...
  "banner / alert / toast": {
    required: [
      ...
      },
      {
        ...
        noun: "surface ink",
        stated:
          "no part carries background-color, a border colour or a left accent",
  ...
  "input / field": {
    required: [
      ...
      },
      {
        ...
        noun: "a width rule",
        stated:
          "no part carries a width, a min/max-width, a grow flag or text content to hug",
  ...
  card: {
    required: [
      ...
      },
      {
        ...
        noun: "surface ink",
        stated:
          "no part carries background-color, a border colour or box-shadow",
  ...
  "progress / spinner": {
    required: [
      ...
      },
      {
        ...
        noun: "an ink fact",
        stated:
          "no part carries background-color, a border colour, fill, stroke or color",
  ...
  "modal / dialog": {
    required: [
      ...
      },
      {
        ...
        stated:
          "no single part carries a background-color together with a shadow or border",
        why: "an opaque panel raised over the page is the archetype.",
        predicate: {
          kind: "channelAll",
          groups: [
            ["background-color"],
            ["box-shadow", "border-color", "border-width", "border-top-width"],
          ],
          scope: "any",
        },
      },
  ...
  pagination: {
    required: [
      ...
      },
      {
        ...
        predicate: ink(
          [...WIDTH_GROUP, "padding-left", "padding-inline", "padding"],
          "any",
        ),
      },
    ],
    ...
    required: [
      {
        ...
        noun: "a day grid",
        stated: "no part carries a grid or a row axis that could hold a week",
        why: "a month is seven columns of days; with no grid axis the days stack into a list and stop being a calendar.",
        predicate: { kind: "layout", directions: ["row", "any"], scope: "any" },
      },
      {
        id: "calendar/day-cell-box",
        noun: "a day cell box",
        stated: "no part carries a width, a height, or padding",
        why: "day cells are a measured grid; without a box the columns cannot align and the month reads as ragged text.",
        predicate: {
          kind: "anyOf",
          of: [
            ink(PADDING_ANY, "any"),
      ...
      },
      {
        ...
        noun: "a selected-day treatment",
        stated:
          "no part carries a background colour or a border colour that could mark the selected day",
        why: "selection is the one thing a date picker must show; without it the grid is a printed month, not a control.",
        predicate: ink(["background-color", "border-color", "color"], "any"),
      },
    ],
    expected: [
      {
        id: "calendar/weekday-header",
        noun: "a weekday header type fact",
        stated: "no part carries font-weight or text-transform",
        why: "the weekday row is told apart from the day numbers by weight or case before anything else.",
        predicate: ink(["font-weight", "text-transform"], "any"),
      },
      {
        id: "calendar/outside-day",
        noun: "an outside-day treatment",
        stated:
          "no part carries opacity or a muted colour for days outside the visible month",
        why: "leading and trailing days belong to other months; undifferentiated, they read as part of this one.",
        predicate: ink(["opacity", "color"], "any"),
      },
    ],
  },
  ...
  breadcrumb: {
    required: [
      ...
      },
      {
        ...
        noun: "separation",
        stated:
          "no part carries gap, column-gap, a left margin or left padding",
    ...
    expected: [
      {
        ...
        stated:
          "no part carries an icon, a shape, static text or an icon component ref",
...
export function channelsOf(part: Part): Set<string> {
  ...
  if (Array.isArray(byProp))
    ...
    for (const v of Object.values(byProp.map ?? {}))
      add(v as Record<string, unknown>);
...
function hasGlyph(parts: FlatPart[]): boolean {
  return parts.some(({ part }) => {
    ...
    if (part.element && /^(svg|path|use|symbol)$/i.test(part.element))
      return true;
    return false;
  });
}
...
function hasStatesOrAxis(contract: Contract, parts: FlatPart[]): boolean {
  ...
  if (
    parts.some(
      ({ part }) => part.states || part.statesByProp || part.declaredStates,
    )
  )
    return true;
  return (contract.props ?? []).some((p) => {
    ...
    return (
      typeof p.type === "object" && "enum" in p.type && p.type.enum.length > 1
    );
  });
}

const WIDTH_CHANNELS = [
  "width",
  "min-width",
  "max-width",
  "flex-grow",
  ...
  switch (predicate.kind) {
    case "channel":
      return scopedParts(parts, predicate.scope).some(({ channels }) =>
        predicate.channels.some((c) => channels.has(c)),
      );
    case "channelAll":
      return scopedParts(parts, predicate.scope).some(({ channels }) =>
    ...
    case "layout":
      return scopedParts(parts, predicate.scope).some(({ part }) => {
        ...
        if (!direction)
          return (
            predicate.directions.includes("any") && Boolean(layout.display)
          );
        if (predicate.directions.includes(direction as "row" | "column"))
          return true;
...
export function generateWarnLine(
  contractId: string,
  archetype: string,
  fact: RequiredFact,
): string {
...
export function expectedWarnLine(
  contractId: string,
  archetype: string,
  fact: RequiredFact,
): string {
...
export function checkRequiredFacts(
  ...
  const resolved = options.archetype
    ? { archetype: options.archetype, source: "declared" as const }
    : resolveArchetype(contract);
  ...
  if (archetype === "unmapped")
    return { ...base, undeclared: UNDECLARED_ARCHETYPE_WARNING(contract.id) };
  ...
  for (const fact of table.required) {
    if (!holds(fact.predicate, contract, parts)) {
      base.missing.push({
        factId: fact.id,
        line: say(contract.id, archetype, fact),
      });
    }
  }
  for (const fact of table.expected) {
    if (!holds(fact.predicate, contract, parts)) {
      base.warns.push({
        factId: fact.id,
        line: expectedWarnLine(contract.id, archetype, fact),
      });
    }
  }
  return base;
