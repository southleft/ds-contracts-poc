...
export function fromProse(id: string, re: RegExp, what: string): RegExpMatchArray {
...
export const plain = (cell: string): string =>
  cell.replace(/\*\*/g, '').replace(/`/g, '').replace(/\[([^\]]+)\]\([^)]+\)/g, '$1').trim();
...
export function md(cell: string): string {
  let s = cell.replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;');
  s = s.replace(/`([^`]+)`/g, (_m, c: string) => `<code>${c}</code>`);
  s = s.replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>');
  s = s.replace(/\[([^\]]+)\]\(([^)\s]+)\)/g, (_m, t: string, h: string) => `<a href="${link(h)}">${t}</a>`);
  return s;
}

/** A parsed table as an HTML table, columns optionally dropped by header name. */
export function renderTable(t: MdTable, opts: { drop?: string[]; className?: string } = {}): string {
  const drop = new Set(opts.drop ?? []);
  const keep = t.head.map((h, i) => (drop.has(h) ? -1 : i)).filter((i) => i >= 0);
  const head = keep.map((i) => `<th>${md(t.head[i])}</th>`).join('');
  const rows = t.rows
    .map((r) => `<tr>${keep.map((i) => `<td>${md(r[i] ?? '')}</td>`).join('')}</tr>`)
    .join('');
...
export function cell(t: MdTable, label: string, col: number): string {
  const row = t.rows.find((r) => plain(r[0]).toLowerCase() === label.toLowerCase());
  if (!row) {
    throw new Error(
  ...
  if (!v) throw new Error(`${REPORT_REL}: row "${label}" has no value in column ${col}`);
  return v;
}
...
export function whatWorksData(): WhatWorksData {
  ...
  return {
    ...
    n: {
      ...
      libraries: denominator.rows.filter((r) => !/total/i.test(plain(r[0]))).length,
      ...
      closed: cell(buckets, 'components closed', 1),
