...
<p class="section-note"><strong>Historical scope (2026-08-31).</strong> Recipe-IR is the v1 proof surface; product v1 is incomplete (F1). Do not flip <code>overallSuccess</code>. Do not restamp hashed RECORDs. The published packages and this site's spec pages are the universal-contract envelope. Current product plan: <a href="/system/">the whole loop</a> (not a ship claim). See <a href="${REPO_URL}/blob/main/docs/32-recipe-ir-pivot.md">docs/32</a> and <a href="${REPO_URL}/blob/main/CONTRIBUTING.md">CONTRIBUTING.md</a>.</p>
