...
<p class="section-note"><strong>Current state (2026-08-31).</strong> This CLI is the <em>universal-contract</em> envelope (extract / generate / bundle / onboard). It is <strong>not</strong> the recipe-IR v1 proof. Recipe-IR never shipped as an npm RC; publish of a recipe surface is deferred. Product v1 is incomplete (F1). Published <code>latest</code> / <code>next</code> tags below are registry truth for that older envelope. Active v1 plan: <a href="${REPO_URL}/blob/main/docs/35-two-journey-v1-plan.md">docs/35</a> (not a ship claim). See <a href="${REPO_URL}/blob/main/docs/32-recipe-ir-pivot.md">docs/32</a>.</p>
