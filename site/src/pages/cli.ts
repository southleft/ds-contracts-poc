...
<p><code>ds-contracts promote --config &lt;ds-library.json&gt; [--root &lt;dir&gt;]</code> — computed-capture artifacts → promoted contracts and a minted token tree: the source-alias pass (a minted leaf whose covering combos all name one token, value-verified twice, becomes a DTCG alias to it), provenance-anchor sidecars, a <code>bindings.figma.statePreviews</code> probe against the real referee, and a resolution guard that refuses the whole promotion on a single dangling ref. This was six near-identical copies of a script under <code>examples/*/scripts/</code> until it became one module; four libraries reproduce their committed artifacts byte-for-byte through it, and two (polaris, astryx) keep their own scripts by name.</p>
...
<p><strong>Provenance wording reports claimed origin, not verification.</strong> The plugin and PR body share one module (<code>core/canvas-code-plan.ts</code>), including the no-recorded-provenance case. A <code>ds_contracts/contractId</code> marker alone does not establish a true round trip or byte-identical reproduction: those require a matching trusted canonical baseline, preserved semantics and runtime identity where applicable, and independent comparison. An unqualified canvas inversion produces a <strong>starting point, not a reproduction</strong>. Canvas metadata cannot authorize executable dependencies; the retained-runtime application-to-canvas path is still unfinished. See <a href="/system/">the current work order</a>.</p>
