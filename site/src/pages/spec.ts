    ...
    section(
      ...
      `<p>A contract is one JSON document per component — <code>contracts/&lt;component&gt;.contract.json</code>. It captures everything design and engineering must agree on; both libraries are renderers of it. Types below are rendered from the schema; follow the links for the fields that have their own pages.</p>` +
        fieldList(ContractSchema as AnySchema, {
          ...
          archetype: 'Optional (schema 19). The component class this contract belongs to — <code>button</code>, <code>card</code>, <code>dialog</code>, and so on, or <code>none</code> for a document that is not a component archetype at all (a divider, a heading). It is what the <strong>required-facts referee</strong> reads: per archetype, the load-bearing facts a set must carry before it will mint. A card with no part carrying <code>direction: "column"</code> minted as a pill and a dialog with no column axis minted as one row; both now refuse <em>by name</em> instead. Three-way by design — declared here always wins; absent but with a recognised name, the seeding name-map applies and <code>ds-contracts migrate</code> writes the field so the guess becomes a reviewed declaration exactly once; absent and unrecognised, nothing is enforced and the tool asks you to declare it, because a wrong archetype would demand facts the component does not owe. Carried, never derived at read time: no emitter reads it to draw.',
