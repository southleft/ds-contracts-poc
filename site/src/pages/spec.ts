...
function contractPage(): { route: string; html: string } {
  const semantics = (ContractSchema.shape as Record<string, AnySchema>)
    .semantics;
  void semantics;
  const body = [
    section(
      ...
      `<p>A contract is one JSON document per component — <code>contracts/&lt;component&gt;.contract.json</code>. It captures everything design and engineering must agree on; both libraries are renderers of it. Types below are rendered from the schema; follow the links for the fields that have their own pages.</p>` +
        fieldList(
          ...
          {
            $schema:
              "Editor affordance: points at the emitted JSON Schema (<code>contracts/contract.schema.json</code>, <code>npm run schema</code>) so contracts validate inline as you type.",
            ...
            version:
              'Semver, the unit of change management. Added optional prop = minor; removed or renamed prop or value = major. See <a href="/spec/versioning/">versioning</a>.',
            status:
              "Governance lifecycle: <code>draft</code> → <code>stable</code> → <code>deprecated</code>.",
            description:
            ...
            archetype:
              'Optional (schema 19). The component class this contract belongs to — one of <strong>20</strong> values, plus <code>none</code>: <code>button</code>, <code>badge / tag / chip</code>, <code>checkbox / radio</code>, <code>toggle / switch</code>, <code>banner / alert / toast</code>, <code>input / field</code>, <code>card</code>, <code>avatar</code>, <code>tabs</code>, <code>accordion</code>, <code>progress / spinner</code>, <code>slider</code>, <code>select / combobox</code>, <code>modal / dialog</code>, <code>tooltip / popover</code>, <code>menu / dropdown</code>, <code>pagination</code>, <code>table / data-grid</code>, <code>breadcrumb</code>, <code>nav (top / side)</code>. <code>none</code> is the honest opt-out for a document that is not a component archetype at all — a divider, a heading, a code block — and it is a <em>declaration</em>, not an absence.</p><p>It is what the <strong>required-facts referee</strong> reads: per archetype, the load-bearing facts a set must carry before <code>figma bundle</code> or the plugin engine will mint it. A card with no part carrying <code>direction: "column"</code> minted as a pill, and a dialog with no column axis minted as one row; both refuse <em>by name</em> now instead of minting something that lies.</p><p>Resolution is three-way, and deliberately refuses to guess. A value <strong>declared here always wins</strong>. If the field is absent but the component\'s name is recognised, the seeding name-map applies and <code>ds-contracts migrate &lt;dir&gt;</code> writes the field into the document — so a guess becomes a reviewed declaration exactly once, in a diff a human reads, rather than silently at every read. If the field is absent and the name is <em>not</em> recognised, <strong>nothing is enforced</strong> and the tool warns <code>declare archetype</code>: a wrong archetype would either demand facts the component does not owe or bless a mint that misrepresents it, and both are worse than no opinion. Carried, never derived at read time — no emitter reads it to draw.',
            ...
            modes:
              'Receipt-grade metadata naming the token modes a drawn theme/mode variant axis carried (e.g. <code>["light","dark"]</code>). The axis is never a prop — theming lives in the token collection’s modes — so this field changes no emitter output; it names the fact for reviewers and round-trip tooling.',
            ...
            bindings:
              "The contract-level per-surface bindings (schema 17): each tool’s identity anchors, plus the canvas-only facts <code>representation</code> and <code>statePreviews</code> under <code>bindings.figma</code> — see below.",
            provenance:
              "Optional deterministic source/canonical revision evidence and stale-source adoption state — see below.",
          },
    ...
    section(
      ...
      `<p>Schema 17 hoists the <code>bindings.&lt;surface&gt;</code> namespace props already use to the contract itself: everything one tool owns lives under its own key, and the vendor-neutral core carries no tool name at any level. The v16 spellings <code>figmaRepresentation</code>, <code>figmaStatePreviews</code>, <code>anchors.figma</code>, <code>anchors.code</code> and <code>slot.figmaProperty</code> are refused by name with the new spelling in the message; <code>ds-contracts migrate &lt;dir&gt;</code> rewrites a tree.</p><p><code>anchors</code> is the DTCG <code>$extensions</code> dual-ID pattern applied to components. After the first generation on each side, that side’s stable identifiers are written back here (<code>npm run anchors:writeback</code>). From then on, renames on either side never fork identity — parity matches by anchor, not by name.</p>` +
        fieldList(
          (ContractSchema.shape as Record<string, AnySchema>).bindings,
          {
            figma:
    ...
    section(
      ...
      `<p><code>provenance</code> is optional so pre-RC contracts remain valid. Once present, it makes an approved design change impossible to overwrite silently with an unchanged code extraction. <code>canonicalRevision</code> is a SHA-256 of the contract with the provenance block removed; <code>source.revision</code> hashes normalized, contract-relevant extraction facts rather than filesystem paths.</p>` +
        fieldList(
          unwrap((ContractSchema.shape as Record<string, AnySchema>).provenance)
            .schema,
          {
            version:
              "Provenance record version. v1 is the only accepted spelling.",
            canonicalRevision:
              "Deterministic <code>sha256:…</code> of the canonical contract content, excluding this provenance block.",
            source:
              "<code>{ kind, adapter, revision }</code> identifies whether code or design produced the current canonical revision and pins the normalized source facts.",
            awaitingCodeAdoption:
              "Present after a reviewed design-led change. It records the accepted design revision and the last code-source revision; an unchanged code extraction that would revert the design change is refused by name.",
          },
        ) +
        fidelity(
    ...
    section(
      ...
      ["generated", "curated"],
      fieldList(
        unwrap((ContractSchema.shape as Record<string, AnySchema>).a11y).schema,
        {
          focusVisible:
...
function semanticsPage(): { route: string; html: string } {
  const semanticsSchema = (ContractSchema.shape as Record<string, AnySchema>)
    .semantics;
  const body = [
    section(
      "semantics",
      ...
      `<p>The code renderer’s HTML element, and the ARIA role when it differs from the element’s native one. The element vocabulary is a closed enum — a contract cannot ask for an element the generator has no accessibility knowledge of.</p>` +
        fieldList(semanticsSchema, {
          ...
          roleException:
            'See <a href="#role-exception">declared role exceptions</a>.',
    ...
    section(
      ...
      `<p>The rendered HTML element follows an enum prop — Heading’s <code>level</code> maps <code>"2" → h2</code>. Code emits an <code>ELEMENT_MAP</code> lookup and renders a dynamic tag; <code>semantics.element</code> is the fallback. The canvas is unaffected — a declared fidelity boundary.</p>` +
        ...
        shippingExample("heading.contract.json", {
          paths: ["semantics", "props"],
          limit: { props: 1 },
        }),
    ),
    section(
...
function propsPage(replays: Awaited<ReturnType<typeof loadReplays>>): {
  route: string;
  html: string;
} {
  ...
  const bindingsShape = (
    resolveLazy(bindings) as unknown as { shape: Record<string, AnySchema> }
  ).shape;
  const body = [
    section(
      "props",
...
function anatomyPage(): { route: string; html: string } {
  const body = [
    section(
      ...
      "The part tree",
      ["generated", "curated"],
      `<p>Anatomy is a <strong>nested tree</strong> of named parts (<code>Record&lt;partName, Part&gt;</code>). At least one top-level part is required, typically named <code>root</code>; multiple top-level parts are supported. Part names are unique per contract. In native declarative generation, the part tree carries supported styling through <a href="/spec/tokens/">token bindings</a> and explicit literal channels. A retained original runtime keeps its authored implementation; this tree alone does not replace or qualify its styles. See <a href="/spec/contract/#anchors">runtime bindings</a> and <a href="/system/">the current integration boundary</a>.</p><p>A part is structural by default (a frame on the canvas, an element in code, containing <code>parts</code>), or it plays one of the roles on this and the following pages: text content, icon, meter, <a href="/spec/shape/">shape</a>, <a href="/spec/composition/">slot / component ref / repeat template</a>.</p>` +
        fieldList(PartSchema as unknown as AnySchema, {
          description: "Reviewer-facing note on the part’s purpose.",
          element:
            "HTML element for this part (code side). Defaults: <code>div</code> structural, <code>span</code> content. Root uses <code>semantics.element</code>.",
          roleException:
            'Named exception to the native-semantics lint — see <a href="/spec/semantics/#role-exception">Semantics</a>.',
          layout:
            'Flexbox / auto-layout — see <a href="/spec/layout/">Layout</a>.',
          layoutByProp:
            'Per-enum-value layout overrides — see <a href="/spec/layout/#layout-by-prop">Layout</a>.',
          stylesWhen:
            'Conditional literal styles — see <a href="/spec/conditionals/#styles-when">Conditionals</a>.',
          overlay:
          ...
          tokensByProp:
            'Per-enum-value token overrides — see <a href="/spec/tokens/#tokens-by-prop">Token bindings</a>.',
          states:
            'Interaction-state token overrides — see <a href="/spec/states/">States</a>.',
          statesByProp:
            'Per-state token overrides that also vary by an enum axis — see <a href="/spec/states/#states-by-prop">States</a>.',
          ...
          animation:
            'CSS-side motion — see <a href="/spec/conditionals/#animation">Conditionals</a>.',
          slot: 'Constrained insertion point — see <a href="/spec/composition/#slots">Composition</a>.',
          component:
            'Fixed instance of another contract — see <a href="/spec/composition/#component-refs">Composition</a>.',
          repeat:
            'Item template over an arrayOf prop — see <a href="/spec/composition/#repeat">Composition</a>.',
          icon: "Icon asset part — see below.",
          attrs: "HTML/ARIA attributes — see below.",
          codeOnly: "Capture-side code-only receipts (v18) — see below.",
          visibleWhen:
            'Conditional visibility — see <a href="/spec/conditionals/#visible-when">Conditionals</a>.',
          optional:
    ...
    section(
      ...
      `<p><code>content: { prop }</code> binds a part’s text to a declared text prop — <code>{title}</code> in the part’s element on the code side, a text node linked to the text property on the canvas. <code>text</code> is static literal text (a page number, an ellipsis) — the same on both surfaces, bound to nothing.</p>` +
        ...
        shippingExample("banner.contract.json", {
          paths: ["anatomy.root.parts.body.parts.title"],
        }),
    ),
    section(
      "text-by-prop",
    ...
    section(
      ...
      `<p><code>icon: { asset, size? }</code> renders <code>assets/icons/&lt;asset&gt;.svg</code> inline on the code side and as a vector on the canvas. <code>'{prop}'</code> substitutes an enum prop — icon-by-status. Icons are always decorative (<code>aria-hidden</code>).</p>` +
        ...
        shippingExample("banner.contract.json", {
          paths: ["anatomy.root.parts.statusIcon"],
        }),
    ),
    section(
      "attrs",
      ...
      `<p><code>attrs</code> sets HTML/ARIA attributes on the part’s element — literal strings or <code>'{prop}'</code> references (refused when the prop doesn’t exist). Code-side surface; the canvas ignores it. A real <code>&lt;input type="checkbox|radio"&gt;</code> declared this way is recognized as a native checkable control: code surfaces render it as the focusable control (checked state is DOM state, never ARIA), and the canvas draws nothing for it — semantics don’t draw.</p>` +
        shippingExample("checkbox.contract.json", {
          paths: ["anatomy.root.parts.box.parts.input"],
        }),
    ),
    section(
      "code-only",
    ...
    section(
      ...
      `<p><code>meter: { valueProp, maxProp }</code> — progress fill: width = value/max as a percentage of the parent track. Code computes live; the canvas renders the defaults’ fraction — its honest static state. The same discipline repeats across the spec: where a surface cannot run a live computation, it renders the declared sample and says so.</p>` +
        shippingExample("progress-bar.contract.json", {
          paths: ["anatomy.root.parts.track"],
        }),
    ),
  ].join("");
  return specPage(
    ...
    section(
      ...
      `<p>One vocabulary, two projections: flexbox on the code side, auto-layout on the canvas; <code>display: grid</code> is the third spelling (declared-track grids — see <a href="#grid">Grid</a>). The properties are the intersection both surfaces can honor — that is the point.</p>` +
        fieldList(LayoutSchema as AnySchema, {
          display:
            "<code>flex</code>, <code>inline-flex</code>, or <code>grid</code> (code). Flex spellings become auto-layout on the canvas; <code>grid</code> becomes a GRID frame with declared tracks.",
          direction:
            "Row or column. Reversed directions exist only as per-variant overrides (see below) — the canvas has no reverse, so they are compiled away. Flex-only — schema-invalid with <code>display: grid</code>.",
          align:
        ...
        shippingExample(
          "avatar-group.contract.json",
          {
            paths: [
              "anatomy.root.parts.stack.layout",
              "anatomy.root.parts.stack.tokens",
            ],
          },
          "overlap — the negative-spacing projection",
        ),
    ),
    section(
      "grid",
    ...
    section(
      ...
      `<p><code>textAutoResize: "WIDTH_AND_HEIGHT"</code> on a text part records one captured canvas fact about that part's box: <em>it sizes itself to its text</em>, and a Figma text box that does so is a <strong>whole number of pixels wide</strong> — the glyph advance rounded up. It is Figma's <code>textAutoResize</code> under its own name, and only the value the code surfaces lower is spelled; <code>NONE</code>, <code>HEIGHT</code> and the deprecated <code>TRUNCATE</code> are a fixed or filled box, which the width and fill vocabulary already carries. Measured on a designer's 72-variant Badge: 26 of the 48 × 16 px small variants missed the 5 % limit at 4.4–7.3 % with every content size equal, because <code>Label</code> in Inter Semi Bold 14 is a 32 px box in Figma and a 31.40625 px run in Chromium, so the hug root rendered 47.40625 px against 48 and its right edge antialiased across two columns.</p><p><strong>Absent</strong> is the meaning every contract already had: the text element is as wide as its glyph run at the browser's fractional advance. No existing contract or emitted byte changes.</p><p>The lowering gives the element the same box as Figma's: <code>inline-size: calc-size(max-content, round(up, size, 1px))</code> — its max-content inline size rounded up to the pixel — as a <strong>progressive enhancement</strong>: a browser without <code>calc-size()</code> drops the declaration at parse (or ignores the assignment on the inline surface) and keeps today's fractional box, under 1 px narrower and never a different layout. Logical <code>inline-size</code>, so a vertical or RTL writing mode rounds the axis the text runs along. <code>text-align</code>, <code>max-width</code> and <code>white-space</code> compose with it. The canvas writer sets <code>textAutoResize = 'WIDTH_AND_HEIGHT'</code> on the node it builds — the value <code>figma.createText()</code> is born with, so a set this pipeline wrote reads the fact back and proposes it: Figma has no fractional text box, and the re-read is the truth about the canvas. The fact is <strong>captured, never inferred</strong>: a dump that did not capture the field (dump ≤ v1.35) proposes the fractional box it always did; a node auto-width in some variants and not in others is named; a node that says auto-width and FILL at once is named as a contradiction; a sole root label hoisted into <code>anatomy.root.text</code> is named, not carried.</p>` +
        refusals("Refusals:", [
          ...
          "the flag together with a <code>width</code> / <code>inline-size</code> / <code>flex</code> / <code>flex-grow</code> / <code>flex-basis</code> channel, <code>layout.grow</code>, or a truncation channel (<code>text-overflow</code>, <code>-webkit-line-clamp</code>) — a box that is sized, filled or truncated by a channel is not sized by its text",
        ]),
    ),
    section(
      "layout-by-prop",
...
function tokensPage(): { route: string; html: string } {
  const body = [
    section(
      ...
      ["generated", "curated"],
      `<p>A token binding maps a CSS property to a DTCG token reference — <code>Record&lt;cssProperty, TokenRef&gt;</code> where a <code>TokenRef</code> is a brace-wrapped token path. Native declarative emitters generate CSS and supported canvas variable bindings from these references; explicit literal channels remain distinct. A retained original runtime uses its verified authored styles. A token or Contract change can affect it only through a separately qualified lowering, not merely because the new value exists in the document. See <a href="/system/">the current runtime and canvas boundary</a>.</p><p><strong>Substitution:</strong> a <code>{propName}</code> placeholder inside a token path expands over that enum prop’s values — <code>{color.action.{variant}.background}</code> produces one CSS rule per variant and one bound variable per canvas variant. Root parts support multi-axis substitution; nested parts one placeholder per reference.</p>` +
        codeBlock(
    ...
    section(
      ...
      `<p><code>tokensByProp: { prop, map }</code> — the value-level sibling of the substituted reference. A substituted ref can only carry bindings whose token <em>names</em> spell the axis value; real foreign vocabularies name tokens by scale step (<code>{spacing.200}</code> on large, <code>{spacing.150}</code> on small), so a binding that is a plain function of one enum axis needs a per-value map. Map values are overrides merged over the part’s base <code>tokens</code>; only the values that deviate appear, and refs are plain — no placeholders.</p><p><strong>Multiple entries (v14):</strong> a part may carry an <em>ordered array</em> of entries — one per driving axis (Button: variant colors <em>and</em> size paddings; Text: the variant scale <em>and</em> the fontWeight map). The single-object spelling stays valid. When two entries on <em>different</em> props override the same channel, the later entry wins — mirroring the CSS source-order cascade the values were extracted from. Two entries may not claim the same channel for the same prop: a conflicting channel+prop pair is refused by name.</p>` +
        ...
        shippingExample("token.contract.json", {
          paths: ["anatomy.root.tokensByProp"],
        }),
    ),
    section(
      "literals",
...
function statesPage(): { route: string; html: string } {
  const statesField = (ContractSchema.shape as Record<string, AnySchema>)
    .states;
  const body = [
    section(
      "declared-states",
      ...
      `<p>The interaction states the component must support. Declared once; rendered per surface at that surface’s fidelity — code gets real CSS pseudo-class rules, the canvas gets <a href="#state-previews">opt-in preview variants</a>.</p>` +
        codeBlock(
          `states: ${typeText(statesField)}  // default: []`,
          "ts",
          "contract.states — rendered from the schema",
        ) +
        shippingExample("button.contract.json", {
          paths: ["states", "anatomy.root.states"],
        }),
    ),
    section(
      "root-states",
    ...
    section(
      ...
      `<p><code>statesByProp</code> (v17, the hover-plane round) — an interaction state whose binding is <em>also</em> a function of an enum axis. <code>states</code> holds one ref per channel, and a root ref carrying a single <code>{prop}</code> placeholder already expands into per-value state rules — but that reaches only a token <em>family</em> (<code>{button.bg.hover.{variant}}</code>). The far commoner case is a design system whose per-variant state colours are <strong>unrelated names</strong>: Eventz’s hover backgrounds are <code>comp/button/primary/color/background/hover</code> on primary and <code>comp/button/color/background/knockout-hover</code> on knockout — no family, no shared stem, nothing a placeholder can reach. Measured, refusing that shape cost Button and Icon&nbsp;Button their entire hover and active planes, and a state crossed with a variant axis is the single most common pattern in a real design system.</p><p>The shape is <a href="/spec/tokens/#tokens-by-prop">tokensByProp</a>’s plus the state it applies to — one map grammar in the vocabulary, not two. Entries are ordered like tokensByProp entries; refs are <em>plain</em> (the axis is already pinned by the map key). Code renders enum-class state rules (<code>.variant-primary:hover</code>), emitted after the plain <code>states</code> rules so the per-value binding wins at equal specificity — the tokensByProp cascade discipline applied to a state selector.</p>` +
        ...
        refusals(
          "Refusals — the same discipline <code>states</code> is held to:",
          [
            ...
            statesByProp: [
              {
                ...
                map: {
                  primary: {
                    "background-color":
                      "{comp.button.primary.color.background.hover}",
                  },
                  knockout: {
                    "background-color":
                      "{comp.button.color.background.knockout-hover}",
                  },
                },
              },
            ],
    ...
    section(
      ...
      `<p>Code gets real <code>:hover</code>/<code>:focus-visible</code>/<code>:disabled</code>; the canvas cannot run pseudo-classes, so real systems hand-build “State=Hover” variant axes — and those rot. <code>bindings.figma.statePreviews: true</code> makes the generator own that axis instead: a <code>State</code> variant axis (<code>Default</code>, <code>Hover</code>, …) where each non-default state applies the state’s token overrides on top of the variant’s base bindings. The mirror image of code-only events: state previews are canvas-only, and the code surface is completely unaffected.</p><p>Bounded explosion: previews multiply only the <em>primary</em> enum axis — the one the overrides substitute (<code>{color.action.{variant}.background-hover}</code> names <code>variant</code>); every other axis sits at its default.</p><p><strong>The axis is live, not just drawn.</strong> The generator wires Figma prototype reactions from every <code>State=Default</code> cell on the default plane to its twin: <code>ON_HOVER</code> → <code>CHANGE_TO State=Hover</code>, <code>ON_PRESS</code> → <code>CHANGE_TO State=Active</code>, both auto-reverting, always with <code>transition: null</code> (durations are not contract facts). <code>focus-visible</code> and <code>disabled</code> are <em>excluded by name</em>: Figma’s trigger vocabulary has no focus and no disabled trigger, so those cells stay static previews — destinations of nothing. Cells whose non-primary axes sit off their defaults have no twin, so they carry no reaction: a named coverage limit that follows from the bounded explosion above.</p>` +
        ...
        shippingExample("button.contract.json", {
          paths: ["bindings.figma.statePreviews", "states"],
        }),
    ),
    section(
      "code-only",
...
function conditionalsPage(): { route: string; html: string } {
  const allowed = [...STYLES_WHEN_ALLOWED]
    .map((p) => `<code>${p}</code>`)
    .join(", ");
  const body = [
    section(
      ...
      `<p>The part renders only when the prop matches. Boolean props map to canvas visibility bindings; enum conditions resolve per variant. Omit <code>equals</code> for booleans (truthy).</p>` +
        ...
        shippingExample("banner.contract.json", {
          paths: ["anatomy.root.parts.endArea.parts.close"],
        }),
    ),
    section(
      "styles-when",
      ...
      `<p>Literal CSS — never tokens — applied when a prop matches. Boolean conditions ride the per-boolean data attribute the generator already emits (<code>.root[data-is-disabled] { … }</code>); enum conditions ride the root’s enum class. The whitelist is deliberately tight: behavioral and positional properties with no token vocabulary. A color or a dimension belongs in <code>tokens</code>, and a brace-wrapped value here is refused by name.</p><p>Whitelist (from <code>STYLES_WHEN_ALLOWED</code> in the schema): ${allowed}.</p>` +
        ...
        shippingExample("text-field.contract.json", {
          paths: ["anatomy.root.stylesWhen"],
        }),
    ),
    section(
      "overlay",
      ...
      `<p>The part renders out of flow, attached to one edge of the root — tooltip bubbles, combobox popups. Code: <code>position: absolute</code> with placement-derived insets, and the root becomes <code>position: relative</code>. Canvas: <code>layoutPositioning: ABSOLUTE</code> with placement-derived constraints, preserved through the amend path. Four placements in v1; offset and alignment tuning is a later axis.</p>` +
        ...
        illustrativeExample(
          PartSchema,
          {
            ...
            tokens: { "background-color": "{color.surface.raised}" },
          },
    ...
    section(
      ...
      `<p>CSS-side motion: <code>spin</code> for spinners, <code>pulse</code> for skeletons. Not representable on the canvas — a documented fidelity scope, like events. Nothing richer belongs in a contract: animation timing is behavior whose truth can’t be verified on both surfaces.</p>` +
        shippingExample("spinner.contract.json", {
          paths: ["anatomy.root.parts.arc"],
        }),
    ),
  ].join("");
  return specPage(
...
function shapePage(replays: Awaited<ReturnType<typeof loadReplays>>): {
  route: string;
  html: string;
} {
  const body = [
        ...
        illustrativeExample(
          PartSchema,
          {
            shape: {
              ...
              width: 16,
              height: 16,
              arc: { start: -1.5708, end: 3.1416, innerRadius: 1 },
            },
...
function compositionPage(replays: Awaited<ReturnType<typeof loadReplays>>): {
  route: string;
  html: string;
} {
  const body = [
      ...
      `<p>A slot is a constrained insertion point, aligned with the canvas’s two-tier constraint design. A slot named <code>children</code> is the default slot (React children); any other name becomes a <code>ReactNode</code> prop. <code>accepts</code> lists contract IDs, resolved per surface through each referenced contract’s anchors — a declared slot without a checkable constraint would leave generation and parity nothing to verify, so the constraint is first-class.</p>` +
        fieldList(
          SlotSchema as AnySchema,
          {
            ...
            acceptsMode:
              "<code>prefer</code> (default): accepts guides pickers and generators. <code>restrict</code>: only accepts is legal. <code>open</code>: explicitly anything — the escape hatch. Compatibility rule: widening is a minor version; narrowing is major.",
        ...
        shippingExample("banner.contract.json", {
          paths: ["anatomy.root.parts.endArea.parts.endContent"],
        }),
    ),
    section(
      "default-content",
      ...
      `<p>Design-time sample content: renders as instances inside the slot on the canvas and as the sample in code stories — never baked into the generated component itself. Items must be drawn from <code>accepts</code> when accepts is present. A slot whose default content has <em>multiple</em> items is a multi-child slot — inexpressible as a canvas instance-swap, so it renders its content directly until the native slot-property migration.</p>` +
        ...
        shippingExample("avatar-group.contract.json", {
          paths: ["anatomy.root.parts.stack"],
        }),
    ),
    section(
      "component-refs",
      ...
      `<p>A fixed instance of another contract, embedded by <em>reference</em> — composition never duplicates a child’s definition. Props are spelled canonically and mapped through the <em>child</em> contract’s own bindings on each surface. A string value of the form <code>"{parentProp}"</code> maps the parent’s enum prop into the child per variant.</p>` +
        ...
        shippingExample("card.contract.json", {
          paths: ["anatomy.root.parts.header.parts.avatar"],
        }),
    ),
    section(
      "ref-overrides",
  ...
  const body = [
    ...
    section(
      ...
      `<p>When <code>toggles</code> is present the generator emits the whole toggle mechanically: an uncontrolled <code>useState</code> fallback (interactive out of the box), the controlled/uncontrolled resolution, the flip between exactly two values of an enum prop, and the matching ARIA state attribute on the trigger. Values of the toggled enum <em>outside</em> the pair render <code>aria-*="mixed"</code> and resolve to the pair’s second value on activation — exactly Checkbox’s <code>indeterminate</code>.</p>` +
        fieldList(
          unwrap((EventSchema.shape as Record<string, AnySchema>).toggles)
            .schema,
          {
            prop: "The enum prop being flipped.",
            between:
              "<code>[offValue, onValue]</code> — activation flips within the pair; any non-member value flips to <code>onValue</code>.",
        ...
        shippingExample("switch.contract.json", {
          paths: ["events", "props"],
          limit: { props: 1 },
        }),
    ),
...
${section(
  ...
  `<p>At every build, the site enumerates every branch of the live schema and asserts each one has a documented home. <strong>${receipt.documented}/${receipt.schemaBranches} branches covered${
    receipt.missing.length === 0 && receipt.stale.length === 0
...
export async function buildSpecPages(
