...
<li><h3>Review the draft. This is the one irreducibly human step.</h3><p>Answer each <code>__review:*</code> field by reading your own library, delete the markers you have handled, then delete the top-level marker to approve it. Skip this and the next command refuses:</p>${codeBlock(`REFUSED: extract/computed/configs/acme.json is an UNREVIEWED DRAFT capture config
...
<li><h3>Get the contract into the repo — three doors, all reviewable</h3>
<ul>
<li><strong>GitHub PR.</strong> Fill in <code>owner/repo</code> and a fine-grained token scoped to that one repo (session-only, never stored; closing the plugin forgets it). Leave <em>Dry run</em> ticked first to see the exact plan and send nothing.</li>
<li><strong>Send to repo, no GitHub token.</strong> The developer runs <code>ds-contracts figma receive --out contracts</code>, which prints a 6-character code; the designer types it into the plugin. The proposal travels the pairing bridge and lands as a reviewed local diff — <strong>the CLI writes nothing without <code>--apply</code></strong>.</li>
<li><strong>Copy the JSON out</strong> and commit it yourself.</li>
</ul></li>
...
<h3 id="b-asymmetry">The honest asymmetry</h3>
<p>For a set <strong>this tool generated</strong>, journey B is a true round trip: the proposal is measured against the contract that built it, and the reference repo's own components re-extract to zero mismatches in both directions, red-tested.</p>
<p>For a <strong>hand-built</strong> set it is not a reproduction. The proposal is an <em>inversion</em> of what can be read off the canvas — real structure, real variants, real bound variables — but a canvas cannot tell you about a <code>useEffect</code>, a keyboard handler, or why a value is what it is. <strong>Treat the generated component as a strong, correct-by-construction starting point, not as your finished component.</strong> That boundary is deliberate and permanent: see <a href="${REPO_URL}/blob/main/docs/16-sync-boundary.md">the sync boundary</a>.</p>
