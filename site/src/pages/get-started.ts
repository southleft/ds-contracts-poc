...
<p><a href="${PLAYGROUND_URL}">${PLAYGROUND_URL.replace('https://', '')}</a> runs the repository's actual engine in your browser — no backend, no accounts, no analytics; credentials are session-only and never leave the browser. Import a component from a figma.com URL, from a pasted plugin dump (<code>extract/figma/dump.plugin.js</code> — token names <em>and</em> resolved values, on any plan; the one-time pairing-code relay from the plugin is <a href="${REPO_URL}/blob/main/playground/src/pages/Playground.tsx">disabled</a> until the plugin ships a sender again), or from a public GitHub file URL; edit the proposed contract under governance with refusals named on screen; share it as a ~1&nbsp;KB permalink. Ten minutes there teaches the model faster than any page here.</p>
...
<li><h3>The bundle comes to you</h3><p>Two doors, and the durable one is the default. <strong>Standing channel (recommended):</strong> an engineer runs <code>ds-contracts figma claim-channel</code> once; it mints a write-key/read-key pair (<code>readKey = sha256(writeKey)</code>, so the key you hold reads and can never inject). They keep the publish key in CI and send you the read key; you paste it once into the plugin's <strong>Changes</strong> tab. From then on CI publishes and the plugin checks on open — no codes to trade, nobody waiting. Deliveries carry a monotonic <code>seq</code>, so an out-of-order delivery is <em>named</em> and starts every Apply box unchecked, and the delivery renders its GitHub Actions provenance ("repo — CI run #N, commit abc1234, published 4 minutes ago") above the change report. <strong>Ad-hoc (unverified):</strong> the one-time 6-character pairing code, now folded into "Other ways to receive" in the plugin's <strong>Build</strong> tab — a colleague delivers the artifact to it:</p>${codeBlock(designerPush.command, 'bash', `${designerPush.doc}`)}<p>The pairing code is deliver-once with a 15-minute TTL and requires you both to be online at that moment; it carries no ordering, so it gets no freshness warning — a limit we state rather than paper over. Neither door is signed yet: anyone holding the write key can publish any provenance, so there is no "verified" badge on a delivery.</p></li>
