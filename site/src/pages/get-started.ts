...
<p class="section-note"><strong>Historical scope (2026-08-31).</strong> Paths A–C below are the universal-contract journeys. They still run. They are <strong>not</strong> the recipe-IR v1 proof (five live-minted, owner-signed archetypes; product v1 incomplete, F1 unmet). The playground linked from this site drives the same pre-pivot envelope. Current product plan: <a href="/system/">the whole loop</a> (not a ship claim). See <a href="${REPO_URL}/blob/main/docs/32-recipe-ir-pivot.md">docs/32</a>.</p>
...
<p><a href="${PLAYGROUND_URL}">${PLAYGROUND_URL.replace("https://", "")}</a> runs the repository's <strong>pre-pivot <code>core/</code> engine</strong> in your browser — no backend, no accounts, no analytics; credentials are session-only and never leave the browser. Ten minutes there teaches the <em>universal-contract</em> loop. <strong>It is not the recipe-IR v1 proof.</strong> Do not treat a playground mint or refusal as recipe-IR evidence.</p>
<p><strong>Try this first:</strong> open <em>Examples</em>, pick the Badge, and then break its contract on purpose — delete a required field, or point a token binding at a name that does not exist. The refusal appears on screen, named. That named refusal is the idea of this older loop; it is not the recipe-IR v1 proof. Everything else on this page is plumbing around that older path.</p>
...
<p>A <code>ds_contracts/contractId</code> marker records a set's <strong>claimed origin</strong>, not proof of a true round trip. Reproduction requires a matching trusted canonical baseline, preserved semantics and runtime identity where applicable, and independent comparison. The reference repo's own components have scoped, red-tested re-extraction receipts with zero mismatches in both directions; a marker alone does not extend those results to another component.</p>
...
<p>The plugin and PR body share provenance wording from one module, including when no canvas provenance was recorded. That wording is an origin report, not authentication or a fidelity check. A retained-runtime reference must resolve through the host's trusted artifact registry and canonical baseline; editable canvas integration remains unfinished. See <a href="/system/">the current boundary and work order</a>.</p>
...
<p>Everything above is published and MIT-licensed: <code>@ds-contracts/cli@${cli.latest}</code> — published stable; the pre-pivot release candidate on the <code>next</code> tag is <code>${cli.next}</code> — (the universal-contract engine, esbuild-bundled, zero required runtime dependencies) and <code>@ds-contracts/schema@${schema.latest}</code> — published stable, <code>${schema.next}</code> on <code>next</code> — (the contract schema, its generated JSON Schema, and the <code>validateContract</code> referee). <strong>These packages export the universal-contract envelope, not recipe-IR.</strong> Recipe-IR never shipped as an npm RC; publish of a recipe surface is deferred. These versions are read from the repo's registry-truth manifest — the record of what is actually on npm — never from the source tree's <code>package.json</code>, which may carry a newer, not-yet-published candidate. Point the extraction at what you already have — proposals only, nothing overwritten:</p>
${codeBlock(
  `npm i -g @ds-contracts/cli
