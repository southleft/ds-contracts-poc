...
<p class="section-note"><strong>Current state (2026-08-31).</strong> These numbers are the <em>capture / universal-contract</em> path. They are not the recipe-IR v1 proof. Five archetypes have stayed live mints and owner-signed grades; product v1 is incomplete (F1). The sentence below that date picker and data grid appear in zero committed contracts is still true of <code>contracts/</code> — it is not a claim that Calendar or Table have no recipe-IR proof. Active v1 plan: <a href="${REPO_URL}/blob/main/docs/35-two-journey-v1-plan.md">docs/35</a> (not a ship claim). See <a href="${REPO_URL}/blob/main/docs/32-recipe-ir-pivot.md">docs/32</a>.</p>
