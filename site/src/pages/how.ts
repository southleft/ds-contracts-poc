...
function indexPage(): { route: string; html: string } {
  const qMeta: Record<string, { q: string; a: string; meta: string }> = {
    ...
    'at-scale': {
      ...
      a: 'The dependency graph of a real 1,618-set enterprise kit, computed at build; deterministic build order; the whole-kit census — and a plain statement of what a contract does not cover.',
...
<p class="section-note"><strong>Historical scope (2026-08-31).</strong> These pages walk the <em>universal-contract</em> engine (<code>core/</code>, Journeys A–C, the playground). That path still ships. The <strong>v1 proof surface is recipe-IR</strong> (five live-minted, owner-signed archetypes); product v1 is incomplete (F1). A playground or how-it-flows replay is not recipe-IR evidence. Current product plan: <a href="/system/">the whole loop</a> (not a ship claim). See <a href="${REPO_URL}/blob/main/docs/32-recipe-ir-pivot.md">docs/32</a>.</p>
...
<p>The position this spec takes: <strong>the source of truth is neither surface.</strong> A versioned JSON contract records the supported agreement: the component's API (props, types, enum vocabularies, defaults), anatomy, token bindings, slots, semantics and declared events. Native declarative emitters generate those fields for each surface; independent observations must verify the result. A code-led contract may instead retain a host-verified original runtime for behavior the declarative schema cannot express. That implementation remains a pinned dependency, not behavior inferred from a picture.</p>
...
<p>In native declarative generation, the contract's <a href="/spec/anatomy/">anatomy</a> binds named parts to <a href="/spec/tokens/">design token references</a>, and the CSS Module is <em>generated</em> from those bindings. Missing token references fail validation rather than silently choosing a value. A retained original runtime has its own authored styles: its source and dependencies remain verified, and only explicitly qualified contract channels may alter it. Preserving that runtime does not make arbitrary design edits effective, and design-only input cannot introduce it through canvas metadata. The editable canvas path remains unfinished; <a href="/system/">the current work order</a> separates implemented preservation from pending qualification. For the declarative token path, see <a href="/how-it-works/styles/">How styles are applied →</a>.</p>
...
<p>Both directions, one door. An engineer's hand-added prop is flagged by the differ with a complete proposed contract patch (<a href="/how-it-works/adding-a-prop/">replayed step by step</a>). A designer's canvas edit becomes a proposed contract change through the plugin's Send tab or <code>ds-contracts propose-pr</code> — a pull request carrying one JSON diff. Nothing takes effect because someone had write access to a surface; it takes effect because a human reviewed a diff and merged it. The merge <em>is</em> the adoption decision, and the regeneration that follows is mechanical.</p>
...
<p>Not everything is expressible on both surfaces, and nothing here pretends otherwise. The canvas runs a <em>named slice</em> of behavior and no more: the generator wires Figma prototype reactions for <code>hover</code> and <code>press</code> on the State axis, so a generated set responds in presentation mode — but Figma's trigger vocabulary has no focus and no disabled trigger, so those previews stay drawn cells and are asserted as destinations of nothing. Everything past that is code-side: <a href="/spec/events/">events</a> are callbacks, and the canvas cannot fire a callback, so they surface there as description text. CSS animation and conditional literal styles are code-side by declaration. Structured <code>arrayOf</code> props have no canvas manifestation and say so in their binding. Each limit is written into the spec page for its feature — a limit you can read is a limit you can trust.</p>
...
<p>Every component set in a live enterprise Figma kit, replayed through the full deterministic import pipeline — the same <em>pre-pivot</em> engine the playground still runs: propose → captured-token layer → validate → all ${stats.emitters} emitters. Current standing: <strong>${census} sets clean</strong>, including all 76 real variant composites (menus, cards, avatar groups, dialogs) where failures used to concentrate. That census is not the recipe-IR v1 proof.</p>
...
<p>The differ verifies the contracted API surface: props with types <em>and kinds</em>, variant axes and option sets, defaults on both surfaces, slot properties and their accepts, nested instances, and every token variable. Per-variant anatomy (part tree, layout, bindings) is compared when the Figma snapshot carries v6 fingerprints — gated offline by <code>variant-drift:check</code>. Snapshots without <code>variants</code> are reported as not extracted, never as clean. Resolved anatomy token refs are also checked against generated CSS modules (<code>anatomy-diff:check</code>).</p>
`;
