...
<h2>Assembled, not invented</h2>
<p>The design lineage is deliberate borrowing: the member model from <a href="https://github.com/webcomponents/custom-elements-manifest">Custom Elements Manifest</a>, the prop/value binding grammar from Code Connect, dual-anchor identity from the <a href="https://www.designtokens.org/">DTCG</a> <code>$extensions</code> pattern, the spec taxonomy from Nathan Curtis's component specifications. The DTCG format proved a neutral, Git-versioned, tool-agnostic artifact can govern both sides at the token layer; this spec runs that playbook one level up, at the component-API layer.</p>
`;
