...
| 🟢 | `box-shadow-inset-hairline` | effects | `a stack that MIXES a drop layer with an `inset` hairline — astryx `shadow-med`'s third layer, the 1px inner ring every menu card and toast carries` | CARRIED | carried | PASS |
| 🟢 | `box-shadow-ok-color-ring` | effects | `a two-layer box-shadow whose RING layer is spelled in an OKLab colour space — `0 0 0 3px oklch(0.708 0 0 / 0.5), 0 1px 2px rgba(0,0,0,.05)` (Tailwind v4 / shadcn's focus ring, verbatim from examples/shadcn/tokens/shadcn-minted.dtcg.json)` | CARRIED | carried | PASS |
| 🟢 | `box-shadow-ok-stack` | effects | `a TWO-layer elevation whose every layer colour is an OKLab spelling — the drop half of astryx's `shadow-med` (examples/astryx/tokens/astryx.dtcg.json)` | CARRIED | carried | PASS |
