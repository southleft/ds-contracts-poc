...
| `aspect-ratio` | geometry | aspect-ratio: 2 / 1 | `aspect-ratio` | CARRIED | — |  |  | code-only channel root.aspect-ratio = 2 / 1 — the canvas has no aspect-ratio field — this part carries no bound or literal width to derive a height from, so nothing was drawn from the ratio (a parent that takes its height from this part's… |
...
| `grid-child-text-hug` | grid-sizing | a TEXT child hugging its content inside a grid cell | `__text` | CARRIED | — |  |  | code-only declared label.display = block — CSS display modes outside auto-layout flex (inline, block, list-item) have no direct Figma equivalent; the canvas approximates with frame nesting (a block-level box lowers to a vertical stack). |
...
| `text-decoration-by-axis` | typography | text-decoration-line that VARIES on one enum axis (none at the default value, underline on the other) — the mui.link `underline` shape | `text-decoration-line` | CARRIED | — |  |  | degradation text-channel-unsupported @ «self»:Underline=Always/label: text channel(s) with no dump v1 projection: textDecoration UNDERLINE (the canvas twin of CSS text-decoration-line: underline — drawn, not read back) — typography carries… |
...
| `transform-scale-zero-by-axis` | transform | transform: scale(0) on a child part at the default axis value, scale(1) on the other — the mui.radio `checked` shape (scale(0) IS the library's hide mechanism for the selection dot) | `transform` | CARRIED | — |  |  | code-only declared a.transform = matrix(0, 0, 0, 0, …) — a COLLAPSED transform (scale(0)) for checked=unchecked — Figma has no scale(0), so the canvas lowers it the only honest way it can: the part is NOT DRAWN in those cells. The fact is… |
