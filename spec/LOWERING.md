...
| `propose.display-root-layout-elided` | `implemented` | `propose-figma.ts:7009` | a canvas root drawn at exactly row / center / center → n/a — this is the return leg: the Figma fact is dropped rather than proposed | the elision is only sound if the absence re-inflates to the SAME shape it elided | **none** | `untested` |
