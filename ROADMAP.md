...
- ✅ **The coverage fraction is published with a matched population** — the 102 covered components are **10.0%** of the 1015 in the eight libraries with a measured size ([docs/24 §2](docs/24-what-works.md)). Unknown-size libraries are excluded from both sides, and heterogeneous size units remain an explicit limitation. Adoption still means a hand-picked slice, configured by an expert, one round per novel styling method.
