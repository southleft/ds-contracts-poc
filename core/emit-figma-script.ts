...
function buildNativeContractComparisonScript(contract: Contract, byId: Map<string, Contract>,
  ...
  return wrapNativeSourceWrite(prepared, buildSyncScript([data], context.operation.fileKey, {
    ...
    preamble: '', nativeSource: true, nativeContractComparison: true, nativeNestedComparison: !!compiled.instances?.length, nativeSourceOwnedComparison: !!compiled.instances?.some(ref => ref.contentMode === 'source-owned'), nativeInstanceWidthComparison: compiled.instanceWidth !== undefined, nativeFullWidthComparison: !!compiled.instances?.some(ref => ref.fillWidth), nativeGridComparison: !!compiled.contentSpecPath || !!compiled.instances?.some(ref => ref.contentSpecPath), nativeSampleSpecs: compiled.specs,
  }));
}
...
${opts.nativeContractComparison ? nativeContractComparisonRuntime(!!opts.nativeNestedComparison, !!opts.nativeGridComparison, !!opts.nativeFullWidthComparison, !!opts.nativeSourceOwnedComparison, !!opts.nativeInstanceWidthComparison) + '\nreturn await nativeBuildContractComparison();\n' : ''}const results = [];
