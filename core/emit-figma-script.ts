...
function buildNativeContractComparisonScript(contract: Contract, byId: Map<string, Contract>,
  ...
  return wrapNativeSourceWrite(prepared, buildSyncScript([data], context.operation.fileKey, {
    ...
    preamble: '', nativeSource: true, nativeContractComparison: true, nativeNestedComparison: !!compiled.instances?.length, nativeSourceOwnedComparison: !!compiled.instances?.some(ref => ref.contentMode === 'source-owned'), nativeFullWidthComparison: !!compiled.instances?.some(ref => ref.fillWidth), nativeGridComparison: !!compiled.contentSpecPath || !!compiled.instances?.some(ref => ref.contentSpecPath), nativeSampleSpecs: compiled.specs,
  }));
}
...
function buildSyncScript(
  datas: ComponentData[],
  fileKey: string | null,
  opts: { header: string; preamble: string; variableCollection?: string; nativeSource?: boolean; nativeComparisons?: boolean; nativeContractComparison?: boolean; nativeNestedComparison?: boolean; nativeSourceOwnedComparison?: boolean; nativeFullWidthComparison?: boolean; nativeGridComparison?: boolean; nativeSampleSpecs?: NodeSpec[] },
): string {
...
${opts.nativeContractComparison ? nativeContractComparisonRuntime(!!opts.nativeNestedComparison, !!opts.nativeGridComparison, !!opts.nativeFullWidthComparison, !!opts.nativeSourceOwnedComparison) + '\nreturn await nativeBuildContractComparison();\n' : ''}const results = [];
