...
export function validNativeGraphCreation(components: ComponentData[] | undefined, creation: unknown): boolean {
  if (!Array.isArray(components) || components.length < 2 || components.length > 30 ||
      !object(creation) || creation.graphVerification !== 1 || !Array.isArray(creation.nodes) ||
  ...
  for (const [index, component] of components.entries()) {
    ...
    if (!component || component.stateVariants?.length || !object(row) ||
        ...
        !object(row.propertyDefinitions) || !Array.isArray(row.variants) || row.variants.length !== component.variants.length ||
    ...
    for (const [i, variant] of row.variants.entries()) {
      if (!object(variant) || Object.keys(variant).sort().join(',') !== 'id,key,name' ||
          ...
          variant.name !== component.variants[i].name || nodes.get(variant.id)?.type !== 'COMPONENT' ||
