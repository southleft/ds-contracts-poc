...
type Change={nodeId:string;kind:'height';before:number;after:number;variableId:string}|{nodeId:string;kind:'mode';collectionId:string;before:string};
export interface NativeComparisonRepairPlan {
  version:1;input:NativeContractComparisonObservationInput;before:Row;after:Row;changes:Change[];revision:string;
}
...
export function emitNativeComparisonRepairScript(plan:NativeComparisonRepairPlan,readOnly=false):string {
  if(!same(prepareNativeComparisonRepair(plan.input,plan.before),plan))fail('plan-changed');
...
try{
 ...
 for(const change of plan.changes){
  ...
  else collections.set(change.collectionId,await figma.variables.getVariableCollectionByIdAsync(change.collectionId));
 }
