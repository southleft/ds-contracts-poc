...
export function generateTsx(
  ...
  const renderPart = (partName: string, part: Part): string => {
    ...
    if (part.component) {
      const dep = byId.get(part.component.id)!;
      const attrs = depAttrString(dep, part.component.props ?? {}, contract) + reactInitialAttributes(contract, dep, part.component) + (hasComponentGrow(part) ? ` className={${stylesRef(partName)}}` : '') + (selection?.attrs(part) ?? '');
