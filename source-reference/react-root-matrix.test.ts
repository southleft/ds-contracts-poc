...
export function Surface({${defaulted?"tone='quiet'":'tone'},density='roomy',children}:{density?:'roomy'|'compact';tone?:'quiet'|'loud'|'null'|null;children?:React.ReactNode}){
 return <section style={{display:'flex',gap:tone===null?'normal':tone==='loud'?20:8,padding:tone===null?2:tone==='loud'?(density==='compact'?28:16):4,backgroundColor:tone==='loud'?'var(--accent)':'var(--base)'}}>{children}</section>;
}`;
