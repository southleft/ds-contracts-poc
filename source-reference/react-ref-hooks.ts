...
export interface ReactRefHook {call:HelperSourcePoint;callee:HelperSourcePoint;consumer:HelperSourcePoint;receiver:'bare'|'namespace'|'default'}
...
export function planReactRefHooks(reference:ReactHelperReference,initializers:readonly ReactTargetInitializer[]):ReactRefHook[]{
 const result:ReactRefHook[]=[];
 for(const name of new Set(initializers.map(i=>i.render.file))){
  const file=path.resolve(reference.sourceRoot,name),hash=reference.files[file];if(!hash||realpathSync(file)!==file)throw Error('ref-hook-source-unavailable');
  const text=readFileSync(file,'utf8');if(createHash('sha256').update(text).digest('hex')!==hash)throw Error('ref-hook-source-changed');
  ...
  for(const initializer of initializers.filter(i=>i.render.file===name)){
   ...
   if(initializer.render.sha256!==hash||!fn||!(ts.isFunctionExpression(fn)||ts.isArrowFunction(fn)))throw Error('ref-hook-render-changed');
   const walk=(n:ts.Node)=>{
    if(ts.isFunctionLike(n)&&n!==fn)return;
    if(ts.isCallExpression(n)&&!n.questionDotToken&&n.arguments.length===1&&!n.arguments.some(ts.isSpreadElement)){
     let callee:ts.Expression=n.expression;while(ts.isParenthesizedExpression(callee))callee=callee.expression;
     let receiver:ReactRefHook['receiver']|undefined;
     if(ts.isIdentifier(callee)){
      const d=declaration(callee);
      if(d&&ts.isImportSpecifier(d)&&!d.isTypeOnly&&!d.parent.parent.isTypeOnly&&(d.propertyName??d.name).text==='useRef'){
       const imp=d.parent.parent.parent;if(ts.isStringLiteral(imp.moduleSpecifier)&&imp.moduleSpecifier.text==='react')receiver='bare';
      }
     }else if(ts.isPropertyAccessExpression(callee)&&!callee.questionDotToken&&callee.name.text==='useRef'&&ts.isIdentifier(callee.expression)){
      ...
      if(imp&&ts.isImportDeclaration(imp)&&ts.isStringLiteral(imp.moduleSpecifier)&&imp.moduleSpecifier.text==='react')receiver=d&&ts.isNamespaceImport(d)?'namespace':'default';
     }
     ...
     if(receiver&&!movesScope){
      const shadow=checker.getSymbolsInScope(n,ts.SymbolFlags.Value).find(s=>s.name==='globalThis');
      if(shadow?.declarations?.some(d=>!(ts.isIdentifier(d)&&(ts.isPropertyAccessExpression(d.parent)||ts.isElementAccessExpression(d.parent))&&d.parent.expression===d)))throw Error('ref-hook-global-shadowed');
      result.push({call:point(n),callee:point(callee),consumer:initializer.render,receiver});
     }
    }
