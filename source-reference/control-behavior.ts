...
export async function observeCheckboxBehavior(page: Page,
  ...
  const read = () => page.locator(input.selector).evaluate(element => {
    ...
    return {
      role: native ? 'checkbox' : element.getAttribute('role'),
  ...
  try {
    for (const action of ['associated-label', 'space'] as const) {
      ...
      if (before.role !== 'checkbox' || before.checked !== input.checked || before.disabled !== input.disabled)
      ...
      const passed = after.checked === expected && after.role === 'checkbox' && after.disabled === input.disabled;
