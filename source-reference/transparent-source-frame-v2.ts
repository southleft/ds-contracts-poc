/** Current source-only capture instrument. Version 1 remains byte-frozen for
 * historical receipt verification; new captures inspect Chromium shadow boundaries.
 * No native pixels influence framing. */
...
/** Chromium exposes open, closed and user-agent shadow boundaries even when
 * Element.shadowRoot is null. Inspect only the target light tree and ancestors;
 * independent siblings retain the capture's exact exclusion check. */
async function inspectLightTree(page: Page, selector: string) {
  const fail = (reason: string): never => { throw Error('transparent-source-frame-' + reason); };
  const cdp = await page.context().newCDPSession(page).catch(() => fail('scope-unavailable'));
  let objectId: string | undefined;
  try {
    const evaluated = await cdp.send('Runtime.evaluate', { expression:
      `document.querySelector(${JSON.stringify(selector)})` });
    objectId = evaluated.result.objectId;
    if (evaluated.exceptionDetails || !objectId) fail('scope-unavailable');
    let targetNodes = 0, ancestorNodes = 0;
    for (let target = true; objectId; target = false) {
      // pierce:false reports each shadow boundary without traversing its opaque
      // interior. Ancestor children are outside the target and are not walked.
      const { node } = await cdp.send('DOM.describeNode', { objectId, depth: target ? -1 : 0, pierce: false });
      const pending = [node];
      while (pending.length) {
        const n = pending.pop()!;
        if (target) targetNodes++; else ancestorNodes++;
        if (targetNodes + ancestorNodes + pending.length > 10_000) fail('scope-too-large');
        if (n.shadowRoots?.length || n.contentDocument) fail('opaque-content');
        if (target) {
          if ((n.childNodeCount ?? 0) !== (n.children?.length ?? 0)) fail('scope-unavailable');
          if (targetNodes + ancestorNodes + pending.length + (n.children?.length ?? 0) > 10_000) fail('scope-too-large');
          pending.push(...n.children ?? []);
        }
      }
      const parent = await cdp.send('Runtime.callFunctionOn', { objectId,
      ...
      if (parent.exceptionDetails || (!objectId && parent.result.subtype !== 'null')) fail('scope-unavailable');
    }
    return { kind: 'chromium-light-tree-v1' as const, targetNodes, ancestorNodes };
  } catch (error) {
    if (error instanceof Error && error.message.startsWith('transparent-source-frame-')) throw error;
    return fail('scope-unavailable');
  } finally {
    try { if (objectId) await cdp.send('Runtime.releaseObject', { objectId }); }
...
export async function captureTransparentSourceFrame(page: Page, selector: string, originalSha256: string) {
  ...
  const inspect = async () => {
    const scope = await inspectLightTree(page, selector);
    const observation = await page.evaluate<ReturnType<typeof observe>>(
      `(() => { const __name = value => value; return (${observe.toString()})(${JSON.stringify(selector)}); })()`);
    return { ...observation, opaqueScope: scope };
  };
  ...
  try {
    ...
    return { original, withContext, transparent, bytes: cropped.bytes, receipt: {
      version: 2 as const, kind: 'transparent-source-frame' as const,
