...
function fixture(key: BindingProbeKey = "label") {
  ...
  const bound = JSON.parse(original.measurement.json)
    .bound as BoundTopologyResult;
  ...
  const rawTree = JSON.parse(
    files["atoms-button--default/source-tree.json"].utf8,
  );
  ...
  const replay = {
    harPath: "/synthetic/reader-only/source.har",
    harSha256: "c".repeat(64),
    url: "http://127.0.0.1:6017/iframe.html?id=atoms-button--default&viewMode=story",
  };
      ...
      entry.story === "atoms-button--default"
        ? {
            ...
            topology: {
              ...
              channels: rawTree.channels,
              tree: rawTree.tree,
              treeSha256: rawTree.treeSha256,
              sourcePngSha256: rawTree.sourcePngSha256,
            },
          }
        : {
            story: entry.story,
  ...
  const planIndex = plans.findIndex((plan) => plan.key === key),
    intervention = plans[planIndex].intervention!;
  const imageBytes = new Map<string, Buffer>([
    ["atoms-button--default/replay.png", originalPng],
  ]);
  const profile = entries[0].profile;
  const cases: BindingDifferentialResult["cases"] = intervention.values.map(
    (value, caseIndex) => {
      const before = structuredClone(semantics.observation),
      ...
      if (intervention.kind === "property") {
        ...
        const native = after.nativeElements.find(
          (native) => native.path === intervention.target.path,
        )!;
        if (value.kind === "undefined")
          delete native.attributes[intervention.target.attribute];
        else
          native.attributes[intervention.target.attribute] = String(
            value.value,
          );
        interventionReceipt = {
      ...
      imageBytes.set(
        `atoms-button--default/probe-${planIndex}-case-${caseIndex}-before.png`,
        originalPng,
      );
      imageBytes.set(
        `atoms-button--default/probe-${planIndex}-case-${caseIndex}-after.png`,
        afterPng,
      );
      return {
        value,
        ...
        beforePngSha256: sha(originalPng),
        afterPngSha256: sha(afterPng),
        interventionReceipt,
        afterReadiness: {
          found: true,
          visible: true,
          ...
          tokens: {},
          fontsReady: true,
          platformFonts: [{ familyName: profile.fontFamily, glyphCount: 1 }],
          failedResources: [],
          runtimeErrors: [],
        },
      };
  ...
  const result: BindingDifferentialResult = {
    version: 1,
    status: "dependency-observed",
    acceptedContract: null,
    inputSha256: semanticHash({
      replay: { ...replay, profile },
      source,
      semantics,
      intervention,
    }),
    ...
    problems: [],
    limitations: [
      "Synthetic record-validation fixture; no runtime dependency claim.",
    ],
    digest: "",
  };
  ...
  const report: BindingTraceReport = {
    ...
    rows: entries.map((entry) =>
      entry.story === "atoms-button--default"
        ? {
            ...
            problems: ["reader-fixture-other-probes-unrun"],
            ...
            differentials: plans.map((plan, index) =>
              index === planIndex
                ? { key: plan.key, problems: [], result }
                : { key: plan.key, problems: ["binding-probe-not-run"] },
            ),
          }
        : {
            story: entry.story,
    ...
    scope:
      "Synthetic differential reader fixture over recorded original source; not actual runtime evidence.",
  };
  ...
  const image = (story: string, name: string) => {
    ...
    assert.ok(bytes, `Missing explicit test image ${story}/${name}`);
    return bytes;
  };
  const validate = () => validateBindingReport(saved, evidence, image);
  assert.doesNotThrow(
    validate,
    "positive synthetic dependency fixture must be structurally valid",
  );
...
test("resource/runtime failure evidence cannot retain an observed dependency after digest recomputation", () => {
  for (const channel of ["failedResources", "runtimeErrors"] as const) {
    const f = fixture();
    f.value.cases[0].afterReadiness![channel].push("explicit fixture failure");
    f.resign();
...
test("wrong, absent or retargeted setter receipts refuse despite matching semantic deltas", () => {
  for (const receipt of [
    undefined,
    {
      kind: "source-property-setter",
      property: "name",
      value: { kind: "value", value: "Contract label alpha" },
    },
    { kind: "native-attribute-write", property: "label" },
  ]) {
    const f = fixture();
    f.value.cases[0].interventionReceipt = receipt;
    f.resign();
...
test("slot receipt requires exact Node identity, unchanged sibling list and original before text", () => {
  for (const delta of [
    { domPath: "host/1" },
    { before: "Invented original label" },
    { preservedSiblings: [{ kind: 3, text: "extra" }] },
  ]) {
    const f = fixture("default-slot");
    f.value.cases[0].interventionReceipt = {
      ...(f.value.cases[0].interventionReceipt as Record<string, unknown>),
      ...delta,
    };
    f.resign();
    assert.throws(f.validate, /binding-report-case-intervention-mismatch/);
...
test("image bytes and exact post-intervention semantics are rechecked, not trusted from counters", () => {
  ...
  image.imageBytes.set(
    imageKey,
    syntheticPng(image.imageBytes.get(imageKey)!, 7),
  );
  assert.throws(image.validate, /binding-report-case-artifacts-mismatch/);
  const semantic = fixture();
  semantic.value.cases[0].after!.nativeElements[0].attributes["aria-label"] =
    "Not the requested input";
  semantic.value.cases[0].afterSha256 = semanticHash(
    semantic.value.cases[0].after,
  );
  semantic.resign();
...
test("malformed PNG bytes refuse even when the case image hash and result digest are updated", () => {
  const f = fixture("default-slot"),
    invalid = Buffer.from("not a PNG file");
  f.imageBytes.set(
    `atoms-button--default/probe-${f.planIndex}-case-0-after.png`,
    invalid,
  );
  f.value.cases[0].afterPngSha256 = sha(invalid);
  f.resign();
...
test("oversized PNG IHDR dimensions refuse before decoder allocation", () => {
  ...
  try {
    assert.throws(f.validate, /binding-report-image-invalid/);
    assert.equal(
      oversizedDecodeAttempted,
      false,
      "dimensions must be bounded before calling the PNG decoder",
    );
  } finally {
    PNG.sync.read = read;
  }
...
test("Checkbox reports rederive fresh lifecycle-correlated semantics and static render evidence", () => {
  const packed = JSON.parse(
    readFileSync(
      new URL(
  ...
  const decode = (record: any) => {
    const bytes = gunzipSync(Buffer.from(record.payload, "base64"));
    assert.equal(sha(bytes), record.sha256);
    return JSON.parse(bytes.toString());
  };
  const input = decode(packed),
    original = decode(packed.originalEvidence),
    png = Buffer.from(original.pngBase64, "base64");
  const story = "atoms-checkbox--default",
    profile = altitudeCohort.find((row) => row.story === story)!.profile;
  const match = matchLitRender(input);
  assert.equal(match.status, "structure-matched");
  assert.notDeepEqual(
    original.semantics.observation,
    input.semantics.observation,
    "the real lifecycle-generated IDs differ",
  );
  const evidence: BindingEvidence = {
    request: {
      version: 2,
    ...
    source: input.source,
    sourcePath: "components/checkbox/checkbox.ts",
    sourceProgramSha256: "b".repeat(64),
    sourceRevision: packed.sourceRevision,
    rows: [
      {
        story,
        runId: "00000000-0000-4000-8000-000000000001",
        ...
        topology: {
          ...
          treeSha256: original.semantics.sourceTreeSha256,
        } as any,
      },
    ],
  };
  const report: BindingTraceReport = {
    ...
    sourceStable: true,
    scope:
      "Synthetic job envelope around actual separate original/replay observations, not a completed product operation.",
    rows: [
      {
        story,
        status: "structure-matched",
        ...
        correspondence: match,
        replaySemantics: input.semantics,
        renderObservation: input.staticRender.observation,
      },
    ],
  };
  validateBindingReport(report, evidence, () => png);
  for (const mutate of [
    (r: BindingTraceReport) => {
      delete r.rows[0].replaySemantics;
    },
    (r: BindingTraceReport) => {
      delete r.rows[0].renderObservation;
    },
    (r: BindingTraceReport) => {
      r.rows[0].replaySemantics!.observation.properties.isChecked = {
        kind: "value",
        value: true,
      };
    },
    (r: BindingTraceReport) => {
      r.rows[0].renderObservation!.policy.sourceSha256 = "f".repeat(64);
    },
    (r: BindingTraceReport) => {
      r.rows[0].renderObservation!.last!.staticFields[0].value = "wrong-tag";
    },
    (r: BindingTraceReport) => {
      r.rows[0].correspondence!.texts![0].sourceProperty = "invented";
    },
    (r: BindingTraceReport) => {
      r.rows[0].boundTopology!.topology!.observation!.pseudoPlanes![0].ownerDomPath =
        "host";
    },
  ]) {
    ...
    mutate(changed);
    assert.throws(
      () => validateBindingReport(changed, evidence, () => png),
      /binding-report-/,
    );
  }
});
