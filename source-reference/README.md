...
Default-exported components may use a local `const` initialized with a supported
function or direct React `forwardRef` call. Declare both `mount.export` and
`subject` as `"default"`; the local name and `displayName` are not export aliases.
