...
Every other translated descendant refuses `react-initial-contract-descendant-translate-unqualified:<reason>`: `parent-not-observed-root`, `conditional-element`, `translation-unresolved` (a computed form outside px, % and one `calc(% ± px)`, a Z component, a non-2D matrix), `rotation-or-scale-present`, `parent-not-single-line-flex` (not flex, reversed, wrapping), `rtl-or-vertical-writing`, `parent-justification-not-start`, `not-sole-in-flow-child` (another in-flow element, direct text, an in-flow `::before`/`::after`), `child-positioned`, `cross-axis-translation`, `parent-main-size-not-fixed`, `child-main-size-not-own-fixed` (so an archive without descendant evidence that translates a part now REFUSES instead of silently dropping the move; it must be observed again), `partial-free-space`, `alignment-not-carried`. A rotation or scale on a part that never translates is outside this rule and unchanged.

One vocabulary change outside this directory: `layoutByProp` may be driven by an optional boolean with an omitted VARIANT plane, the exact condition `tokensByProp` already had (`packages/core/src/validate.ts`, and the matching axis filter in `extract/computed/fuse.ts` `enrichLayout`). A required boolean has no omitted plane and refuses as `alignment-not-carried`. To reverse the decision: make `lowerDescendantTranslations` refuse every translated descendant instead of rewriting `justify-content` (the refusals stay, so nothing becomes silent again), and restore the two enum-only conditions; no archive, token or plan written by other paths depends on either.

Not done: the fidelity lane (`scripts/react-native-fidelity-record.ts`) pairs initial states generically (`initialCohort`) but is pinned to the built-in reference and ownership archive and to operations that already exist, so a declared-cohort family is not recorded yet. A completed initial inspection is still final for its (anchor, case) key: after this reader change the app cannot observe an already-inspected case again without the operator superseding the saved evidence.
