...
**Review compiler update** prepares a host-derived proposal through `POST react/:reference/native-operation/:operation/update-plan`. It authenticates the saved native journal and rederives the desired initial-state contract from pinned source evidence. The immutable proposal lists existing targets and before/after values; the app does not dispatch a write. The current bounded update engine admits node-opacity changes only, requires unchanged topology and existing token values, independently rereads before and after a write, supports no-op repeats and partial-state inspection, and restores attempted opacity changes on failure without overwriting unrelated edits. These are controlled execution tests, not a completed application recovery journey. Companion update delivery and live repair remain unfinished.
