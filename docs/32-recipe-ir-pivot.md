...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29. Calendar live v4 RECORD `c315d0021`. **Calendar live v5 PREPARE opening** on the measured teaching: host-emit compile-carried empty `bindings: []` on `calendar/set`, `calendar/week-set`, and `calendar/day-set`. Astryx only. Run `19be1c96-calendar-v5`. AUTHORIZE and attempt 1 still ahead. `react-day-picker@10.0.1` remains a named refusal. Do not remint Table. Do not restart v4 attempt 2 as-is. |
...
| Calendar    | **live v4 attempt 1 failed closed; v5 PREPARE opening (host-emit empty set bindings); Astryx only; no mint; no human grade; overall false** | AUTHORIZE then attempt 1 on a new Scratch page. Do not restart v4 attempt 2 as-is. Do not invent a Calendar human grade. |
