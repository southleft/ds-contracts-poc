...
Do not restart V81 attempt 2 without a new teaching. Expected-plan opacity facts still encode compile default 1 for the 24 occupancy placeholders; live canvas is opacity 0. Do not demand plan 1 while extract is 0. Do not invent host `visible:false`. Do not invent an opacity variable. Do not invent overlap-zero. Do not reopen V82 cleanup-on-failure-only (probe was not otherwise green). Do not claim v1 complete.
