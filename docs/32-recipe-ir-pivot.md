...
| A5ch | PREPARE INPUT V38. Copy the v37 stack. Do not patch hashed v37 scene-readback, extract, restore, writer, or runtime bytes. Teach host to omit label `textDecoration` that compile never emits. Label role only. Do not invent variables. Do not invent a textDecoration value. Do not teach FIXED as a fill. | Open only after RECORD. If a hashed v38 file later fails closed, open v39. |
...
After RECORD INPUT V37 ATTEMPT 1 is published:

```
# PREPARE INPUT V38 as a separate commit. Do not patch hashed v37.
# Copy the v37 stack. Omit label textDecoration that compile never emits.
```
