...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29. Calendar live v1 RECORD `b6182cb8e`. Calendar live v2 PREPARE `33ec75ead` / AUTHORIZE `4e2e3e7e4`. **Attempt 1 failed closed 2026-08-30** at writer `setProperties` font unavailable: day-box teaching held (32×32 FIXED); first `calendar@1/day` instance Label update refused because SF Pro was not loaded on the instance. Cleanup removed page `173:49787` and collection `173:49789`. Input `115:295378`, Combobox `163:35981`, Button `85:6781`, Table `173:48924` stayed. Do not restart v2 attempt 2 as-is. Next teaching: load each day-instance TEXT font before `setProperties(Label)`. `react-day-picker@10.0.1` remains a named refusal. Do not remint Table. |
...
| Calendar    | **live v2 attempt 1 failed closed at writer setProperties font unavailable; day-box teaching held; cleanup executed; Input/Combobox/Button/Table stayed; no mint; no human grade; overall false** | Do not restart v2 attempt 2 as-is. Next teaching: load instance TEXT font before setProperties(Label). Astryx only. Do not invent a Calendar human grade. |
