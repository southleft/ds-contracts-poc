...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29. Calendar live v6 AUTHORIZE `82faa3dae`. **Attempt 1 failed closed 2026-08-30** at extract `unsupported structural edit at $.children[0].children[0].children[1].children[0].bindings[0].field`. Writer accepted (page `173:50217`, sets `173:50247` / `173:50279` / `173:50396`); restore 4 held; caption order held. Compile `calendar/weekday/spacer` is `type.fontSize`, `fills.0.color`, `width.value`; host emitted live extract order starting at `fills.0.color`. Cleanup removed the page and collection `173:50219`. Input/Combobox/Button/Table stayed. Do not restart v6 attempt 2 as-is. Next teaching: host-order weekday TEXT bindings to compile field order. `react-day-picker@10.0.1` remains a named refusal. Do not remint Table. |
...
| Calendar    | **live v6 attempt 1 failed closed at extract weekday/spacer bindings[0].field; writer accepted; restore 4 held; caption order held; mint cleaned; Input/Combobox/Button/Table stayed; no mint; no human grade; overall false** | Do not restart v6 attempt 2 as-is. Next teaching: host-order calendar/weekday TEXT bindings to compile field order. Do not invent a Calendar human grade. |
