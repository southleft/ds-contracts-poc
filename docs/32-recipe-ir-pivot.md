...
V85 PREPARE is the cleanup-on-failure-only teaching. Signed `requestOrder` no longer requires terminal cleanup on green; recovery cleanup stays persisted after writer (seq 2) and remains executable on failure/abort. Hashed v19 writer and v84 scene-readback stay frozen. Live remains forbidden until a separate authorization. Do not revert v19. Do not invent overlap-zero. Do not restart V84 attempt 2. Do not claim v1 complete.
