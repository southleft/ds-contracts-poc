...
| A5ce | PREPARE INPUT V37. Copy the v36 stack. Do not patch hashed v36 scene-readback, extract, restore, writer, or runtime bytes. Teach host to omit label `textCase` that compile never emits. Label role only. Do not invent variables. Do not invent a textCase value. Do not teach FIXED as a fill. | Open only after RECORD. If a hashed v37 file later fails closed, open v38. |
...
After RECORD INPUT V36 ATTEMPT 1 is published:

```
# PREPARE INPUT V37 as a separate commit. Do not patch hashed v36.
# Copy the v36 stack. Omit label textCase that compile never emits.
```
