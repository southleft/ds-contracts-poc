...
| A5ck | PREPARE INPUT V39. Copy the v38 stack. Do not patch hashed v38 scene-readback, extract, restore, writer, or runtime bytes. Teach host to omit label-row `clipsContent` that compile never emits. Label-row role only. Do not invent variables. Do not invent a clipsContent value. Do not teach FIXED as a fill. | Open only after RECORD. If a hashed v39 file later fails closed, open v40. |
...
After RECORD INPUT V38 ATTEMPT 1 is published:

```
# PREPARE INPUT V39 as a separate commit. Do not patch hashed v38.
# Copy the v38 stack. Omit label-row clipsContent that compile never emits.
```
