...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29. Calendar live v21 RECORD: writer accepted; restore 4 held; Label after-append held; extract accepted; probe Label-via-characters held. Probe refused `reflowPassed` only — every other source boolean true, 8/8 cells clean, overlap 0. Measured: defaultVariant instance is HUG/AUTO; `resizeWithoutConstraints(beforeWidth+64)` leaves width at content 256. Compile calendar/set is hug. Astryx names no fixed width. Cleanup accepted; page `173:53091` and collection `VariableCollectionId:173:53093` removed. Input/Combobox/Button/Table stayed. **Calendar live v22 PREPARE** on the measured teaching: probe hug-root reflow passes when width stays at content hug; do not require +64 to stick. Do not invent FIXED. Do not invent a column-width model. Astryx only. Run `19be1c96-calendar-v22`. Do not remint Table. Do not restart v21 attempt 2 as-is. |
...
| Calendar    | **live v21 attempt 1 failed closed at probe reflowPassed on hug root; extract accepted; v22 PREPARE (probe hug-root reflow is content width); Astryx only; no mint; no human grade; overall false** | AUTHORIZE then attempt 1 on a new Scratch page. Do not restart v21 attempt 2 as-is. Do not invent a Calendar human grade. |
