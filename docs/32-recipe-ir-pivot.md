...
> **Current correction status (2026-08-29 ~12:00 UTC-5; supersedes status
...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29. Calendar live v20 RECORD: writer accepted; restore 4 held; probe Label-via-characters held unreached; extract refused `$.children[1].children[0].children[7].properties.Label`. Host week/on `calendar/day-instance/6` kept set-default Label/`characters` `26`; compile wants `1`. First State=default instance whose Label is not the set default. Writer v3 writes characters before appendChild; that write did not persist. week/off and calendar-set `1` instances persisted. Cleanup accepted; page `173:52911` and collection `VariableCollectionId:173:52913` removed. Input/Combobox/Button/Table stayed. **Calendar live v21 PREPARE** on the measured teaching: writer re-apply instance Label via characters after appendChild; refuse if still mismatched. Do not invent a host Label rewrite. Do not retry setProperties. Astryx only. Run `19be1c96-calendar-v21`. Do not remint Table. Do not restart v20 attempt 2 as-is. |
...
- Attributable human signoff on Input V85 **passed** (2026-08-28, TJ Pitre, page `115:295378`). Combobox v41 live human grade **passed** (2026-08-29, TJ Pitre, page `163:35981`). Button remains pending. Later Data Table and Calendar.
...
| Calendar    | **live v20 attempt 1 failed closed at extract week/on day-instance/6 Label 26 vs 1; v21 PREPARE (writer Label after append); Astryx only; no mint; no human grade; overall false** | AUTHORIZE then attempt 1 on a new Scratch page. Do not restart v20 attempt 2 as-is. Do not invent a Calendar human grade. |
