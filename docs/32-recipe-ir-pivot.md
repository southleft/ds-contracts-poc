...
After AUTHORIZE INPUT V79 is published:

```
npm run recipe:input-field:live:v79:history:verify -- --expect-authorized
```

Then attestation (private 0600) + Scratch-only live attempt 1. New txn `private/input-live-v79-transaction`. File `byMp6lt0Ij9b2QbkDGFwBh` only. v19 writer. Keep V73–V78 teachings. Do **not** invent 9/30/0 or a Polar content-row. Do **not** invent overlap-zero. Do **not** probe-hide. Do **not** teach FIXED. Do **not** invent plugin `layoutSizingHorizontal`. Do **not** restart V78 attempt 2. Do **not** reuse page `111:267910` or txn dirs through `private/input-live-v78-transaction`.
