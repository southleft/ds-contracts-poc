...
V80 host omit of content TEXT `opacity` **held**. Occupancy **held on canvas** (trailing x=165, not x=0). Host then refused recipe collapse on `visible` (compile emits `visible:false` on 24 placeholders; scene omits it). Probe was **not issued**; do **not** invent overlap-zero. Do **not** restart V80 attempt 2 as-is. Do **not** revert v19 to `visible:false`. Do **not** probe-hide. Do **not** invent Polar 9/30/0 or a Polar content-row. Do **not** teach FIXED. Do **not** reuse page `111:275687` or `private/input-live-v80-transaction`. Do **not** claim v1 complete. Mint was cleaned — not review-worthy. Next teaching: compile omit `visible:false` on content/placeholder so compile matches occupancy. Host-omit `visible` is a no-op. V81 cleanup-on-failure-only stays unopened (probe was not otherwise green).
