...
After AUTHORIZE INPUT V75 is published:

```
npm run recipe:input-field:live:v75:history:verify -- --expect-authorized
```

Then attestation (private 0600) + Scratch-only live attempt 1. New txn `private/input-live-v75-transaction`. Keep v17 writer. Polar probe teaching only. Do **not** invent 9/30/0 or a Polar content-row. Do **not** restart V74. Do **not** reuse page `111:247718` or `private/input-live-v74-transaction`.
