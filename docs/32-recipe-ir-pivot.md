...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29. Calendar live v18 RECORD: writer accepted; restore 4 held; day-variant cornerRadius carry held; extract refused `$.children[2].children[0].strokes` on `calendar/day/default`. Compile omits strokes on default/selected/outside and carries them on today; host emitted strokes on all four. Cleanup accepted; page `173:52342` and collection `VariableCollectionId:173:52344` removed. Input/Combobox/Button/Table stayed. **Calendar live v19 PREPARE** on the measured teaching: host-omit strokes on `calendar/day/(default|selected|outside)` only; keep today. Astryx only. Run `19be1c96-calendar-v19`. Do not remint Table. Do not restart v18 attempt 2 as-is. |
...
| Calendar    | **live v18 attempt 1 failed closed at extract day/default strokes; v19 PREPARE (host-omit day default/selected/outside strokes, keep today); Astryx only; no mint; no human grade; overall false** | AUTHORIZE then attempt 1 on a new Scratch page. Do not restart v18 attempt 2 as-is. Do not invent a Calendar human grade. |
