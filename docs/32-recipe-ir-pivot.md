...
| A5fq | PREPARE INPUT V67. Copy the v66 stack. Do not patch hashed v16 writer or hashed v66 scene-readback `e92b6955…53e9`. Teach host-normalize to omit set `strokes` — compile-absent; extract/host `[]` both. Keep v17 writer bytes (HUG teaching already held). Restore/runtime/extract stay v16 frozen source hashes. Expected scene plans stay the v66 compile-carry set. Do not teach FIXED. Do not invent a stroke or `[]`. Do not omit `layout.width.mode`. Do not change set `layout.mode` back to `vertical`. | Draft prepared on top of `0ec36dec`. Do not self-hash this prepare commit. |
| A5fr | AUTHORIZE INPUT V67 as a **separate** later commit. New prepare-era Ed25519 signer. Antecedent = the PREPARE SHA. Auth lifecycle stays out of the hash set. Do not patch hashed v16, v66, or v67 bytes. | Pending after PREPARE. |
