...
| A5by | PREPARE INPUT V35. Copy the v34 stack. Do not patch hashed v34 scene-readback, extract, restore, writer, or runtime bytes. Teach host to drop label binding extras and order label bindings to compile `type.fontSize`, `type.lineHeight.value`, `fills.0.color`. Label role only. Do not invent variables. Do not teach FIXED as a fill. | **Draft prepared** on top of `b19f3d16`. Do not self-hash this prepare commit. |
| A5bz | AUTHORIZE INPUT V35 as a **separate** later commit. New prepare-era Ed25519 signer. Pin the PREPARE SHA. Auth lifecycle stays out of the antecedent hash set. | Open after PREPARE. Then attestation + Scratch-only live. Max 3. If a hashed v35 file fails closed, open v36. |
...
After PREPARE INPUT V35 is published:

```
# AUTHORIZE INPUT V35 as a separate commit. New prepare-era Ed25519 signer.
