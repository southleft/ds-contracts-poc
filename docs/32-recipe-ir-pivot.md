...
V82 is the occupancy-opacity expected-plan teaching (carry live 0; do not demand plan 1). Do not restart V81 attempt 2. Do not invent host `visible:false`. Do not invent an opacity variable. Do not invent compile TEXT opacity. Do not invent overlap-zero. Do not reopen cleanup-on-failure-only until a probe is otherwise green. Do not claim v1 complete.
