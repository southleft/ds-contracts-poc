...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29. Calendar live v4 PREPARE `2990ee182` / AUTHORIZE `06308bdc2`. **Attempt 1 failed closed 2026-08-30** at extract `unsupported structural edit at $.children[0].bindings`. Writer accepted (page `173:49857`, sets `173:49887` / `173:49919` / `173:50036`); restore 4 held. Compile set nodes carry `bindings: []`; host `sceneToNormalizedIr` omitted the key. Cleanup removed the page and collection `173:49859`. Input/Combobox/Button/Table stayed. Do not restart v4 attempt 2 as-is. Next teaching: host-emit those empty bindings arrays. `react-day-picker@10.0.1` remains a named refusal. Do not remint Table. |
...
| Calendar    | **live v4 attempt 1 failed closed at extract $.children[0].bindings; writer accepted; restore 4 held; mint cleaned; Input/Combobox/Button/Table stayed; no mint; no human grade; overall false** | Do not restart v4 attempt 2 as-is. Next teaching: host-emit empty bindings on calendar/week/day sets. Do not invent a Calendar human grade. |
