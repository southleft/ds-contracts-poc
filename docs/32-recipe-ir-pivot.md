...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29. Calendar live v13 RECORD `e6d7c8a30`. **Calendar live v14 PREPARE** on the measured teaching: host-omit `strokes` on `calendar/week/${n}` frames. Astryx only. Run `19be1c96-calendar-v14`. Do not remint Table. Do not restart v13 attempt 2 as-is. |
