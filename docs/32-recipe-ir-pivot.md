...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29. Calendar live v7 AUTHORIZE `aae970d3d`. **Attempt 1 failed closed 2026-08-30** at writer `CALENDAR-DAY-PROPERTY-ABSENT:Label`. Day set `173:50427` already carried `Label#173:26437`. Week `WeekNumbers=on` minted 6 day instances, then the 7th fresh `createInstance().componentProperties` lacked Label. Leftover page `173:50397` and collection `173:50399` cleaned. Input/Combobox/Button/Table stayed. Do not restart v7 attempt 2 as-is. Next teaching: take Label presence from the set-issued key. `react-day-picker@10.0.1` remains a named refusal. Do not remint Table. |
...
| Calendar    | **live v7 attempt 1 failed closed at writer CALENDAR-DAY-PROPERTY-ABSENT:Label; leftover cleaned; Input/Combobox/Button/Table stayed; no mint; no human grade; overall false** | Do not restart v7 attempt 2 as-is. Next teaching: writer Label presence from the set-issued key. Do not invent a Calendar human grade. |
