...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29. Calendar live v9 RECORD `0f3b2773b`. **Calendar live v10 PREPARE opening** on the measured teaching: host-order `calendar/week/${n}/number` TEXT bindings to compile `type.fontSize`, `fills.0.color`, `width.value`. Astryx only. Run `19be1c96-calendar-v10`. AUTHORIZE and attempt 1 still ahead. `react-day-picker@10.0.1` remains a named refusal. Do not remint Table. Do not restart v9 attempt 2 as-is. |
...
| Calendar    | **live v9 attempt 1 failed closed; v10 PREPARE opening (host-order week-number TEXT bindings); Astryx only; no mint; no human grade; overall false** | AUTHORIZE then attempt 1 on a new Scratch page. Do not restart v9 attempt 2 as-is. Do not invent a Calendar human grade. |
