...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29. Calendar live v14 RECORD: writer accepted; restore 4 held; week-frame strokes omit held; extract refused `$.children[0].children[0].cornerRadius` on `calendar/variant/on`. Cleanup accepted; page `173:51622` and collection `VariableCollectionId:173:51624` removed. Input/Combobox/Button/Table stayed. **Calendar live v15 PREPARE** on the measured teaching: host-omit `cornerRadius` on `calendar/variant/(on|off)` components. Do not also omit variant strokes in v15. Astryx only. Run `19be1c96-calendar-v15`. Do not remint Table. Do not restart v14 attempt 2 as-is. |
...
| Calendar    | **live v14 attempt 1 failed closed at extract variant/on cornerRadius; v15 PREPARE (host-omit variant cornerRadius); Astryx only; no mint; no human grade; overall false** | AUTHORIZE then attempt 1 on a new Scratch page. Do not restart v14 attempt 2 as-is. Do not invent a Calendar human grade. |
