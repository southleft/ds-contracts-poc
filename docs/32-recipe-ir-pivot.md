...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29. Calendar live v11 RECORD `d755f3b45`. **Calendar live v12 PREPARE opening** on the measured teaching: host-omit `cornerRadius` on `calendar/week/${n}` frames. Astryx only. Run `19be1c96-calendar-v12`. AUTHORIZE and attempt 1 still ahead. `react-day-picker@10.0.1` remains a named refusal. Do not remint Table. Do not restart v11 attempt 2 as-is. |
...
| Calendar    | **live v11 attempt 1 failed closed; v12 PREPARE opening (host-omit week-frame cornerRadius); Astryx only; no mint; no human grade; overall false** | AUTHORIZE then attempt 1 on a new Scratch page. Do not restart v11 attempt 2 as-is. Do not invent a Calendar human grade. |
