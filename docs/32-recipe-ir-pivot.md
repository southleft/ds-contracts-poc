...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29. Calendar live v15 RECORD: writer accepted; restore 4 held; variant cornerRadius omit held; extract refused `$.children[0].children[0].strokes` on `calendar/variant/on`. Cleanup accepted; page `173:51802` and collection `VariableCollectionId:173:51804` removed. Input/Combobox/Button/Table stayed. **Calendar live v16 PREPARE** on the measured teaching: host-omit `strokes` on `calendar/variant/(on|off)` components. Astryx only. Run `19be1c96-calendar-v16`. Do not remint Table. Do not restart v15 attempt 2 as-is. |
...
| Calendar    | **live v15 attempt 1 failed closed at extract variant/on strokes; v16 PREPARE (host-omit variant strokes); Astryx only; no mint; no human grade; overall false** | AUTHORIZE then attempt 1 on a new Scratch page. Do not restart v15 attempt 2 as-is. Do not invent a Calendar human grade. |
