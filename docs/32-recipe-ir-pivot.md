...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29. Calendar live v17 RECORD: writer accepted; restore 4 held; week-set number order held; extract refused `$.children[2].children[0].cornerRadius` on `calendar/day/default`. Compile carries day-cell `cornerRadius`; the host omitted it because `omitHeaderBodyCornerRadius` treated `CELL_COMPONENT_ROLE` like a Table cell. Cleanup accepted; page `173:52162` and collection `VariableCollectionId:173:52164` removed. Input/Combobox/Button/Table stayed. **Calendar live v18 PREPARE** on the measured teaching: host-carry `cornerRadius` on `calendar/day/*` components. Do not also teach day-variant strokes in v18. Astryx only. Run `19be1c96-calendar-v18`. Do not remint Table. Do not restart v17 attempt 2 as-is. |
...
| Calendar    | **live v17 attempt 1 failed closed at extract day/default cornerRadius over-omit; v18 PREPARE (host-carry day-variant cornerRadius); Astryx only; no mint; no human grade; overall false** | AUTHORIZE then attempt 1 on a new Scratch page. Do not restart v17 attempt 2 as-is. Do not invent a Calendar human grade. |
