...
V82 compile-carry occupancy opacity 0 held on the frozen expected-plan/observe pair (accounting 0/0 both). Recipe collapse refused MUI fixed-point; probe was not issued. Do not restart V82 attempt 2 without a new teaching. Do not invent host `visible:false`. Do not invent an opacity variable. Do not invent compile TEXT opacity. Do not invent overlap-zero. Do not reopen cleanup-on-failure-only (probe was not otherwise green). Do not claim v1 complete.
