...
| A5gi | Diagnose Polar silent 192 without inventing 9/30/0. Measure expected-number sources, named-or-not, V72 extract roles, MUI zero, library-specificity, and v17 writer assignment. Open V73 only if those keys are **not** named required facts (classification). | **Stop. V73 not opened.** Widths 8 / 25.78125 are Polar `textAdornment` intrinsic geometry on `polarisLeadingPayload` / `polarisTrailingPayload` in `recipe/fixtures/library-input-fields.ts`, cited to `TextField.js:112-128,241-247` (prefix/suffix as Text bodyMd). Polar source has no literals 8 or 25.78125; they are measured glyph metrics, then compile-carried in the v66 expected-scene-plan (64+64 `instancePayload` facts). Spreads 1 and 3 are `state()` focus-gap/focus-ring when `ring` is set; Polar `focusVisible` has `ring`; Polar `TextField.out.css` has no `0 0 0 1px`/`3px` box-shadow. Both keys are named: docs/32 intrinsic geometry + drawable focus effects; recipe `AdornmentPayloadSchema.intrinsicSize` / `ShadowSchema.spread`; `exhaustiveReviewedFacts` walks those landings; live `OBSERVABLE_CHANNELS` includes `instancePayload` and `effect`. Archetype `required-facts.ts` `input/width-rule` does **not** name these pixels. V72 extract `9d029458…` / 8436209: Polar leading `$` payload/text 9 (64), slot frame 20; Polar trailing `USD` payload/text 30 (64), slot frame still 25.78125; Polar `State=focus-visible` surfaces 32× two `DROP_SHADOW` with `spread:0` present, colors `#f7f7f7ff`/`#005bd3ff`. MUI silent 0 because MUI payloads match expected 9/31 and MUI has 0 effect facts (no ring), not because MUI lacks `instancePayload`. Drift is Polar-only; a Polar `if (library)` teaching is forbidden. Frozen writer hashes held. v17 payload `f5e1294a…7ce9` contains Polar `8×20` (65), `25.78125×20` (65), `"spread":1` (32), `"spread":3` (32); writer `makeHelper` `resizeWithoutConstraints(payload.intrinsicSize.width,height)` then FIXED; extract runtime reads `main.width` as `instancePayload.intrinsicSize`; writer assigns `spread:effect.spread`. Do **not** rewrite expected-plan to 9/30/0. Do **not** drop those keys from the required compare. Do **not** restart V72. Do **not** reuse page `110:239941` or `private/input-live-v72-transaction`. |
...
| Named required facts? | **Yes.** Recipe schema + reviewed-fact walk + docs/32 (intrinsic geometry, drawable focus effects) + live `OBSERVABLE_CHANNELS`. Not an incidental leftover. Not in archetype `required-facts.ts` pixel table. Classification drop from the required compare is forbidden. |
...
**V73 not opened.** Next is not compile-carry and not a required-compare drop.
...
**Stop.** V72 Polar silent 192 is named-fact value drift (`instancePayload.intrinsicSize.width` and focus `effect.spread`), not an incidental expected-plan leftover. Do **not** open V73 classification. Do **not** invent 9/30/0. Do **not** restart V72. Do **not** reuse page `110:239941` or `private/input-live-v72-transaction`. Human reviews whether source-cited fixture measurements or live Figma metrics win. A later writer (new bytes; do not patch hashed v16/v17) may re-assert 8 / 25.78125 / spread 1 and 3 only after that review.
