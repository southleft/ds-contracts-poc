...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29. Calendar live v3 PREPARE `6f4135a4a` / AUTHORIZE `712e26b5c`. **Attempt 1 failed closed 2026-08-30** at the same `setProperties` font class: loadFont-before-setProperties held and did not clear it. Measured on leftover instance `173:49855`: SF Pro Regular is listed and loadable; `setProperties(Label)` still refuses; `text.characters` writes. Cleanup removed page `173:49822` and collection `173:49824`. Input/Combobox/Button/Table stayed. Do not restart v3 attempt 2 as-is. Next teaching: apply the day instance Label via `text.characters`, not `setProperties`. Do not invent a different font. `react-day-picker@10.0.1` remains a named refusal. Do not remint Table. |
...
| Calendar    | **live v3 attempt 1 failed closed at writer setProperties font unavailable; loadFont teaching held; direct characters works; cleanup executed; Input/Combobox/Button/Table stayed; no mint; no human grade; overall false** | Do not restart v3 attempt 2 as-is. Next teaching: instance Label via text.characters, not setProperties. Do not invent a font. Do not invent a Calendar human grade. |
