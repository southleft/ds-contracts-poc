...
| A5ep | PREPARE INPUT V58. Copy the v57 stack. Do not patch hashed v57 scene-readback, extract, restore, writer, or runtime bytes. Teach host to omit `type.letterSpacing` on `input-field/required-indicator` that compile never emits (extract proved 64/64 both). Do not invent variables. Do not invent a letterSpacing value. Do not lift onto label. Do not omit textCase or textDecoration in the same prepare. Do not teach FIXED as a fill. | Draft prepared on top of `0794b671`. Antecedent index SHA-256 `1e368a9f02b8e61fdf0935d977a38021e8c70157876abd6ce5ba5e8ff69f980c`. AUTHORIZE as a later commit. |
| A5eq | AUTHORIZE INPUT V58 as a **separate** later commit. New prepare-era Ed25519 signer. Antecedent is the PREPARE V58 commit. Auth lifecycle stays out of the hash set. Do not patch hashed v57 or v58 bytes. | `--expect-authorized` after publish. Then attestation + Scratch-only live. Max 3. If a hashed v58 file fails closed, open v59. |
