...
| **C. MUI clip 104 + overlap 12** | **Unchanged.** Clip 104/128 max 0.5294117647058824 (overlay-label AABB; 52 small + 52 medium). Overlap 12/128 max 20 (trailing slot + label; all `mui/*/placeholder/*/trailing` except focus-visible). Polar 0/0. No new probe class. |

Stop. Do not invent clip or overlap. Clip-only later only as overlay-label probe classification if measured. Overlap is real and must not be zeroed by invention. Cleanup-on-failure-only only if a later probe is otherwise green.

### Immediate next command

V77 contentFill teaching **held** both sources. Stop here. Do **not** invent clip or overlap.

Clip 104/128 is overlay-label AABB (optional later probe classification only if measured as that class). Overlap 12/128 is real trailing+label and must **not** be zeroed by invention. Cleanup-on-failure-only is a separate later prepare only if a later probe is otherwise green. Do **not** invent Polar 9/30/0 or a Polar content-row. Do **not** teach FIXED. Do **not** invent plugin `layoutSizingHorizontal`. Do **not** restart V77 attempt 2 as-is. Do **not** claim v1 complete.
