...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29. Calendar live v5 AUTHORIZE `fbb2e5d39`. **Attempt 1 failed closed 2026-08-30** at extract `unsupported structural edit at $.children[0].children[0].children[0].bindings[0].field`. Writer accepted (page `173:50037`, sets `173:50067` / `173:50099` / `173:50216`); restore 4 held; empty set bindings held. Compile `calendar/caption` is `type.fontSize` then `fills.0.color`; host emitted live extract order starting at `fills.0.color`. Cleanup removed the page and collection `173:50039`. Input/Combobox/Button/Table stayed. Do not restart v5 attempt 2 as-is. Next teaching: host-order caption bindings to compile field order. `react-day-picker@10.0.1` remains a named refusal. Do not remint Table. |
...
| Calendar    | **live v5 attempt 1 failed closed at extract caption bindings[0].field; writer accepted; restore 4 held; mint cleaned; Input/Combobox/Button/Table stayed; no mint; no human grade; overall false** | Do not restart v5 attempt 2 as-is. Next teaching: host-order calendar/caption bindings to compile field order. Do not invent a Calendar human grade. |
