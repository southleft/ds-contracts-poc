...
| **C-overlap. Trailing + label** | **Unchanged.** 12/128 max 20. Same twelve `mui/*/placeholder/*/trailing` keys except focus-visible. Polar 0. Real minted geometry — do not invent it away. |

### Immediate next command

V78 overlay-label AABB teaching **held**. Clip is 0 both sources. Stop here. Remaining measured class is overlap 12/128 max 20 — real trailing-slot + label mint geometry on `mui/*/placeholder/*/trailing` except focus-visible. Polar overlap still 0.

The next command, if any, must name overlap as that real mint defect. Do **not** invent overlap-zero. Do **not** move labels in the writer to fake a box. Do **not** invent Polar 9/30/0 or a Polar content-row. Do **not** teach FIXED. Do **not** invent plugin `layoutSizingHorizontal`. Do **not** restart V77 or V78 attempt 2 as-is. Do **not** claim v1 complete. Mint was cleaned — not review-worthy. Cleanup-on-failure-only remains a named later teaching if a mint must stay.
