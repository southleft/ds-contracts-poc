...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter authored 2026-08-29: `recipe/adapters/calendar.ts` + `astryxCalendarAdapterConfig` (one library, `@astryxdesign/core` Calendar). `react-day-picker@10.0.1` remains a named refusal, not an adapter. **Human gate** on that Calendar adapter before any live PREPARE. Live still Scratch-only. Do not remint Table. Do not touch Input `115:295378` or Combobox `163:35981`. |
