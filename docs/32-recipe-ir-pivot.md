...
V79 hidden FILL occupancy **held on canvas** (trailing x=165, not x=0). Host then refused recipe collapse on `opacity`. Probe was **not issued**; do **not** invent overlap-zero. Do **not** restart V79 attempt 2 as-is. Do **not** probe-hide. Do **not** invent Polar 9/30/0 or a Polar content-row. Do **not** teach FIXED. Do **not** invent plugin `layoutSizingHorizontal`. Do **not** restart V78 attempt 2. Do **not** reuse page `111:272958` or `private/input-live-v79-transaction`. Do **not** claim v1 complete. Mint was cleaned — not review-worthy. Next command may be a **different cited mechanism** for the opacity collapse contradiction — not probe-hide. V80 cleanup-on-failure-only stays unopened (probe was not otherwise green).
