...
| A5ed | PREPARE INPUT V54. Copy the v53 stack. Do not patch hashed v53 scene-readback, extract, restore, writer, or runtime bytes. Teach host to omit `strokes` on `input-field/variant/*` that compile never emits (extract proved 128/128 both). Do not invent variables. Do not invent a strokes value. Do not lift onto surface. Do not omit variant clipsContent in the same prepare. Do not teach FIXED as a fill. | Antecedent index SHA-256 `fa120b104bcc191513c5f39f10f36138ed3e7b31e9ecae0cce4e7cb0573ac047`. AUTHORIZE as a later commit. |
| A5ee | AUTHORIZE INPUT V54 as a **separate** later commit. New prepare-era Ed25519 signer. Antecedent is the PREPARE V54 commit. Auth lifecycle stays out of the hash set. Do not patch hashed v53 or v54 bytes. | `--expect-authorized` after publish. Then attestation + Scratch-only live. Max 3. If a hashed v54 file fails closed, open v55. |
