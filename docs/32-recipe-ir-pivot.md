...
| A5df | PREPARE INPUT V46. Copy the v45 stack. Do not patch hashed v45 scene-readback, extract, restore, writer, or runtime bytes. Teach host to omit `type.textCase` on `input-field/message/helper` and `input-field/message/error` that compile never emits (extract proved both). Do not invent variables. Do not invent a textCase value. Do not teach FIXED as a fill. | Open after RECORD. |
...
After RECORD INPUT V45 is published:

```
# PREPARE INPUT V46 — copy v45; omit message helper/error textCase compile never emits
# do not patch hashed v45; do not restart v45 attempt 2 as-is
```
