...
| A5cq | PREPARE INPUT V41. Copy the v40 stack. Do not patch hashed v40 scene-readback, extract, restore, writer, or runtime bytes. Teach host to omit label-row `effects` that compile never emits. Label-row role only. Do not invent variables. Do not invent an effects value. Do not omit label-row strokes yet. Do not teach FIXED as a fill. | Open after RECORD. |
...
After RECORD INPUT V40 is published:

```
# PREPARE INPUT V41 — copy v40; omit label-row effects compile never emits
# do not patch hashed v40; do not restart v40 attempt 2 as-is
```
