...
Attempt 1 Scratch-only live after attestation and preflight. New txn `private/input-live-v81-transaction`. Scratch `byMp6lt0Ij9b2QbkDGFwBh` only. If collapse passes, probe must run. Do not invent host `visible:false`. Do not invent overlap-zero. Do not restart V80 attempt 2. Do not claim v1 complete.
