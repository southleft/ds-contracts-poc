...
| A5di | PREPARE INPUT V47. Copy the v46 stack. Do not patch hashed v46 scene-readback, extract, restore, writer, or runtime bytes. Teach host to omit `type.textDecoration` on `input-field/message/helper` and `input-field/message/error` that compile never emits (extract proved both). Do not invent variables. Do not invent a textDecoration value. Do not teach FIXED as a fill. | Open after RECORD. |
...
After RECORD INPUT V46 is published:

```
# PREPARE INPUT V47 — copy v46; omit message helper/error textDecoration compile never emits
# do not patch hashed v46; do not restart v46 attempt 2 as-is
```
