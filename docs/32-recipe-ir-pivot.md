...
After AUTHORIZE INPUT V78 is published:

```
npm run recipe:input-field:live:v78:history:verify -- --expect-authorized
```

Then attestation (private 0600) + Scratch-only live attempt 1. New txn `private/input-live-v78-transaction`. File `byMp6lt0Ij9b2QbkDGFwBh` only. Keep v18 writer. Keep V73–V77 teachings. Probe exclude overlay-label AABB only. Do **not** invent 9/30/0 or a Polar content-row. Do **not** invent overlap-zero. Do **not** teach FIXED. Do **not** invent plugin `layoutSizingHorizontal`. Do **not** restart V77. Do **not** reuse page `111:262862` or `private/input-live-v77-transaction`.
