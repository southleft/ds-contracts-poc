...
| A5ed | PREPARE INPUT V54. Copy the v53 stack. Do not patch hashed v53 scene-readback, extract, restore, writer, or runtime bytes. Teach host to omit `strokes` on `input-field/variant/*` that compile never emits (extract proved 128/128 both). Do not invent variables. Do not invent a strokes value. Do not lift onto surface. Do not omit variant clipsContent in the same prepare. Do not teach FIXED as a fill. | Next honest lineage. |
