...
| A5dr | PREPARE INPUT V50. Copy the v49 stack. Do not patch hashed v49 scene-readback, extract, restore, writer, or runtime bytes. Teach host to omit `effects` on `input-field/message-container` that compile never emits (extract proved 128/128 both). Do not invent variables. Do not invent an effects value. Do not lift onto surface. Do not omit strokes in the same prepare. Do not teach FIXED as a fill. | Open after RECORD. |
...
After RECORD INPUT V49 is published:

```
# PREPARE INPUT V50 — copy v49; omit message-container effects compile never emits
# do not patch hashed v49; do not restart v49 attempt 2 as-is
```
