...
**Decision named:** **Stop. No V79.** Real collision (trailing covers label text). Do **not** probe-hide. Do **not** invent a writer gap, opacity-0 placeholder, `primaryAxisAlign: max`, or v19 padding. Do **not** treat ABSOLUTE-row ∩ trailing as the V78 clip class — these labels are in-field, not sitting on the outline. Cleanup-on-failure-only / V80 stays unopened (probe is not otherwise green).

### Immediate next command

V78 overlay-label AABB teaching **held**. Overlap diagnosis **held** as real in-field trailing-on-label collision (12/128 max 20). **No V79.** Do **not** invent overlap-zero. Do **not** move labels or trailing in the writer to fake a box. Do **not** invent Polar 9/30/0 or a Polar content-row. Do **not** teach FIXED. Do **not** invent plugin `layoutSizingHorizontal`. Do **not** restart V77 or V78 attempt 2 as-is. Do **not** claim v1 complete. Mint was cleaned — not review-worthy. Cleanup-on-failure-only remains a named later teaching only if a later probe is otherwise green.
