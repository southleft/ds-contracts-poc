...
Diagnose the V73 probe refuse: all 256 cells `rolesExact`/`stateSemanticsExact` false; MUI `stateSemanticsPassed`/`textPropertiesRestored` false; Polar those plus `reflowPassed`/`contentFillPassed` false. Accounting silent 0/0 both is measured. Do **not** invent 9/30/0. Do **not** restart V73 attempt 2 as-is. Do **not** claim v1 complete. Mint was cleaned — not review-worthy.
