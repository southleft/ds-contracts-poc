...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29: `recipe/adapters/calendar.ts` + `astryxCalendarAdapterConfig` (one library, `@astryxdesign/core` Calendar). A first draft invented Inter, Monday-start, an 8px day-grid gap, `--radius-inner` 4px, and `--spacing-1` day padding; those were corrected against `styles.ts` / `tokens.stylex.ts` / `astryx.css` before any live PREPARE. `react-day-picker@10.0.1` remains a named refusal, not an adapter. TJ authorized continuing the hill (2026-08-29 evening). Calendar live v1 PREPARE opens on that honest adapter (Astryx only, 8 variants, 13 remote). Live still Scratch-only. Do not remint Table. Do not touch Input `115:295378` or Combobox `163:35981`. |
