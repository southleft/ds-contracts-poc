...
| **B. Polar `reflowPassed` / `contentFillPassed` false** | **Probe predicate** (MUI-shaped). | Quoted: `content=…role==="input-field/content-row"` then `reflowPassed=instance.width===beforeWidth+64&&surface&&surface.width>surfaceWidth&&content&&content.width>contentWidth`; `contentFillPassed:!!reflowPassed`. Hard-requires `content-row`. Polar architecture is recipe-legal **stacked**: first children `label-row`, `surface`, `message-container`; content-row **0** since v16 extract. Polar content texts FILL 128 / FIXED 0; surface FILL 128; label-row FILL 128. MUI floating has content-row 128 and reflow already true. | **V75.** Probe Polar reflow against surface grow + content text (`contentRow \|\| contentText`). Split `contentFillPassed` to content-text `layoutSizingHorizontal==="FILL"`. Do **not** invent a Polar content-row. |
...
Diagnose the remaining V75 probe classes before any V76: `textPropertiesRestored` false both; MUI `contentFillPassed` newly false after the Polar split (extract FILL 128; defaultVariant hidden placeholder); MUI clip 104/128 and trailing-placeholder overlap 12/128. Polar reflow/contentFill held. Accounting silent 0/0 both. One teaching cannot clear the conjunction. Do **not** invent 9/30/0 or a Polar content-row. Do **not** restart V75 attempt 2 as-is. Do **not** claim v1 complete. Mint was cleaned — not review-worthy. Cleanup-on-failure-only remains a named later teaching if a mint must stay.
