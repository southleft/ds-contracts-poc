...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29. Calendar live v16 RECORD: writer accepted; restore 4 held; variant strokes omit held; extract refused `$.children[1].children[0].children[0].bindings[0].field` on `calendar/week/number`. Compile order is `type.fontSize`, `fills.0.color`, `width.value`; host emitted fills-first with a duplicate fills binding. Cleanup accepted; page `173:51982` and collection `VariableCollectionId:173:51984` removed. Input/Combobox/Button/Table stayed. **Calendar live v17 PREPARE** on the measured teaching: host-order `calendar/week/number` TEXT bindings. Indexed `calendar/week/${n}/number` stays a different held role. Astryx only. Run `19be1c96-calendar-v17`. Do not remint Table. Do not restart v16 attempt 2 as-is. |
...
| Calendar    | **live v16 attempt 1 failed closed at extract week/number binding order; v17 PREPARE (host-order week-set number bindings); Astryx only; no mint; no human grade; overall false** | AUTHORIZE then attempt 1 on a new Scratch page. Do not restart v16 attempt 2 as-is. Do not invent a Calendar human grade. |
