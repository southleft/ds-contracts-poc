...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29. Calendar live v5 RECORD `5ca5385ee`. **Calendar live v6 PREPARE opening** on the measured teaching: host-order `calendar/caption` bindings to compile field order (`type.fontSize` then `fills.0.color`). Astryx only. Run `19be1c96-calendar-v6`. AUTHORIZE and attempt 1 still ahead. `react-day-picker@10.0.1` remains a named refusal. Do not remint Table. Do not restart v5 attempt 2 as-is. |
...
| Calendar    | **live v5 attempt 1 failed closed; v6 PREPARE opening (host-order caption bindings); Astryx only; no mint; no human grade; overall false** | AUTHORIZE then attempt 1 on a new Scratch page. Do not restart v5 attempt 2 as-is. Do not invent a Calendar human grade. |
