...
> sanitization taught; silent is now Altitude **751/8706**, Fluent **751/8778**.
> Remaining is font/name/set/componentRef/value drift. Roboto/SemiBold were not
...
| B2 | Fresh scene-derived inversion/accounting from the live Button page, not stamped/self-selected IR. Do not collide with or rewrite that page except as an explicit later proof if required. | **Measured 2026-08-28.** First run: Altitude 8706 / 5795 / **2911 silent**; Fluent 8778 / 5891 / **2887 silent**. Then two observe-side prepares (role, then unique same-key token sanitize). After both: Altitude 8706 / 7955 / **751 silent**; Fluent 8778 / 8027 / **751 silent**. Fixed point still not stable (`button/set` on live set name `Button / button@1 proof`). Page `85:6781` unread-write; Input page `115:295378` untouched. Not silent-zero. |
...
| zero-silent accounting                    | **measured / not zero** | silent derived as expectedFacts − matched: Altitude **751/8706**, Fluent **751/8778** after role+token teachings |
...
- First 0-write observe (pre-teaching): Altitude **2911/8706**, Fluent
  **2887/8778**.
- Two observe-side prepares, then 0-write live re-observe (current page stayed
...
- After both: Altitude **751/8706** (7955 matched), Fluent **751/8778**
  (8027 matched). Missing roles 0. Binding leftover 144/144 (compile
  `strokes.0.weight` still absent live). Extras 291/267 remain (duplicate
  `fills.0` / `strokes.0` plus set chrome).
- Stopped. Remaining silent is **value-invent / not same-class**: live
  resolved fonts (Fluent **Roboto**, `SemiBold` vs compile stacks /
  `Semi Bold`); live label name `button/label` vs compile
  `button/label :: Label`; live set name/role `Button / button@1 proof` vs
  `button/set :: …`; live `componentRef` `__button/helper/… / icon@1` vs
  `icon@1`; variant-axis order. Do **not** invent Roboto/SemiBold as compile
  fonts.
...
zero (Altitude **751/8706**, Fluent **751/8778** after role+token teachings),
