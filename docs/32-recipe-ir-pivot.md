...
| D4 | Human review of Table/Calendar adapters before live if a human-authored adapter is required. | **Table adapters already reviewed in the v32 climb.** Calendar adapter honesty pass 2026-08-29. Calendar live v6 RECORD `7ec1cd682`. **Calendar live v7 PREPARE opening** on the measured teaching: host-order `calendar/weekday/*` TEXT bindings to compile field order (`type.fontSize`, `fills.0.color`, `width.value`). Astryx only. Run `19be1c96-calendar-v7`. AUTHORIZE and attempt 1 still ahead. `react-day-picker@10.0.1` remains a named refusal. Do not remint Table. Do not restart v6 attempt 2 as-is. |
...
| Calendar    | **live v6 attempt 1 failed closed; v7 PREPARE opening (host-order weekday TEXT bindings); Astryx only; no mint; no human grade; overall false** | AUTHORIZE then attempt 1 on a new Scratch page. Do not restart v6 attempt 2 as-is. Do not invent a Calendar human grade. |
