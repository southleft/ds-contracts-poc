...
| A5ea | PREPARE INPUT V53. Copy the v52 stack. Do not patch hashed v52 scene-readback, extract, restore, writer, or runtime bytes. Teach host to omit `effects` on `input-field/variant/*` that compile never emits (extract proved 128/128 both). Do not invent variables. Do not invent an effects value. Do not lift onto surface. Do not omit variant clipsContent/strokes in the same prepare. Do not teach FIXED as a fill. | Next honest lineage. |
