...
| step | action | exit criterion |
| --- | --- | --- |
| D1 | Explicit Data Table recipe + reviewed adapters for two unrelated real libraries. Offline cross-library proof first: row/column templates, declared column axis, required-facts / door / lowering / grammar coverage. | **Offline recipe authored 2026-08-29.** `table@1` + adapters + `npm run recipe:table:check` green (2 densities × 2 row states; 10 components / 22 instances; two-cycle hash). Door/lowering/grammar corpus coverage, writer, and live are not claimed. |
...
| Data Table  | **offline recipe v1 authored; `recipe:table:check` green; no writer; no live; no human grade; overall false**                                                     | writer with new identity `ds.contracts.table.recipe.v1`, then Scratch-only new page. Do not invent a Data Table human grade. Do not write Input `115:295378` or Combobox `163:35981`. |
