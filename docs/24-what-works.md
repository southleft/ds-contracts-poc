...
Chromium, exact string comparison with no tolerance, over 749,437 compared style cells
...
generation manifest. **What that does not say:** those 127 components are
...
**14 measured component(s) are deliberately excluded here**
because they carry a scorecard but no committed contract — captured with full
receipts and then HELD: `antd/Textarea`, `astryx/TextArea`, `chakra/Alert`, `chakra/Avatar`, `chakra/Checkbox`, `chakra/Dialog`, `chakra/Radio`, `chakra/Switch`, `chakra/Tag`, `chakra/Tooltip`, `mui/Textarea`, `tailwind/Blockquote`, `tailwind/Spinner`, `tailwind/TextInput`.
...
| **all libraries** | **127** | **86.4** | 89.1 | **60/127** | **99/127** | **749,437** | **85.7** |  |
...
cut of the same data: **2,984 of 11,744** rendered rows
...
| real third-party components | 127 | 86.4 | yes | `extract/computed/out/**/scorecard.json` |
...
| capture double-sweep identity | 263/263 runs | a capture whose second sweep disagrees with its first | `extract/computed/out/**/numbers.json`, `determinism` |
| browser captures behind the corpus | 12,755 | a floor quoted from a sample smaller than it claims | same files, `captures` |
...
| every capture run carries the two-sweep determinism receipt | 263 — `extract/computed/out/**/numbers.json` | 263 — `count of numbers.json files` | ✔ |
...
| `extract/computed/out/**/numbers.json` | `c84121662526` | 1,812,737 | capture counts + determinism receipts — 263 files |
| `extract/computed/out/**/scorecard.json` | `a6686a86ca4a` | 21,997,752 | computed-equality per component — 262 files |
