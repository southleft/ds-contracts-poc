...
| A12 | per-side stroke weights (literal) | REST: `individualStrokeWeights` unread → **silent**; plugin: `figma.mixed` → **silent** (bound per-side weights ARE mapped; propose requires uniformity, notes otherwise) | — | — | **SILENT-LOSS** | **CARRIED** since dump v1.34 — both readers write `strokeWeights {top,right,bottom,left}` when the four sides are not all equal, INSTEAD of the uniform `strokeWeight` (REST reports 0 for sides [1,0,1,0], which is not a drawn fact; the plugin reports `figma.mixed`). Proposed as the four `border-<side>-width` LITERAL channels (`literals`, or `literalsByProp` when the weights follow one enum axis); a 0 side is carried as `0px`, and `borderStyleDecls` writes `border-<side>-style: solid` for the non-zero sides only. Still NAMED: a side that is not a finite non-negative number keeps the `stroke-weights-nonuniform` degradation; mixed/partial evidence across variants, an OUTSIDE (outline) stroke, partially bound weights, a per-side STATE override and per-side weights on an INSTANCE stub are named in the proposal notes, never guessed |
| A12b | stroke takes layout space? (`strokesIncludedInLayout`, auto-layout frames) | **no** — neither reader read it | every stroke lowered to a space-taking `border` | a designer's default (`false`) rendered 2× the weight wider and taller | **SILENT-LOSS** (uncounted) | **CARRIED** since dump v1.35 — both readers write the boolean on every stroked auto-layout frame (`false` included; REST omits its default); `false` proposes `Part.strokesIncludedInLayout: false`, the border channels keep the stroke, React / inline / WC draw it as an inset `box-shadow` ring that takes no space, the writer sets the field back. `true` and not-captured propose nothing (what a border already means). Mixed across variants is NAMED. Static HTML still draws a border (§D.39) |
| A12c | text box sizes itself to its text (`textAutoResize: WIDTH_AND_HEIGHT`) — a WHOLE number of pixels wide, the advance rounded up with no tracking after the last glyph | **no** — neither reader read it | the text element as wide as its fractional browser advance (plus trailing letter spacing) | 26 of the CBDS Badge's 48 × 16 px small variants over the 5 % limit with every content size equal (47.41 px root against 48) | **SILENT-LOSS** (uncounted) | **CARRIED** since dump v1.36 — both readers write `text.textAutoResize` verbatim on every text node; WIDTH_AND_HEIGHT proposes `Part.textAutoResize: WIDTH_AND_HEIGHT`, React / inline / WC give the element `inline-size: calc-size(fit-content, round(up, size[ - letter-spacing], 1px))` plus a container clamp, so a runtime string still wraps (a progressive enhancement; browsers without `calc-size()` keep the fractional box), the writer sets the field back. Fixed / filled boxes, mixed variants, a WIDTH_AND_HEIGHT-beside-FILL contradiction and a hoisted root label are NAMED. Static HTML unchanged (§D.42) |
...
| A19 | other bound variables (maxWidth, minHeight, maxHeight, …) | yes (`bound`, generic) | **dropped without a note** | — | **SILENT-LOSS** | **CARRIED** (dump v1.4) — bound min/max variables land as min-width/min-height/max-width/max-height tokens; remaining unconsumed `bound` fields stay named notes |
| A20 | literal min/max sizing | **yes** (dump v1.4: minWidth/minHeight/maxWidth/maxHeight on the node) | minted px facts (min-height/…) | CSS surfaces (canvas variants are drawn at their real size — binding elided there, golden-pinned) | OK | **CARRIED** — the `min-max-size-unsupported` degradation retired (dump v1.4); a drawn minHeight 44 is a tap-target fact and renders |
...
| A22 | text: lineHeight, letterSpacing, textCase, textDecoration, textAlign | **no** | — | — | **SILENT-LOSS** | NAMED-GAP — `text-channel-unsupported` degradation naming each non-default channel. `letterSpacing` is **CARRIED** since dump v1.33 (pixels; the plugin reader resolves PERCENT against the node's font size, and a mixed value keeps its receipt) |
