...
The rest of these docs walk the same path in detail: the contract spec, the token pipeline, generation into each surface, the parity loop, the validation evidence, and the road toward a contributable spec. Start with [Architecture & the Contract Model](./01-architecture.md); for the mechanics of each hop — what crosses, what is refused, what is written down — read [How It Flows](./29-how-it-flows.md).
