...
- **Governed generation measurably wins**: the same AI model building the same screens scored **100/100 adherence with zero violations** when constrained by the contract catalog, versus **69/100 with 91 violations** (invented APIs, hard-coded values, style overrides) without it. And when the constrained agent hit a real gap in the system, it *reported the gap instead of faking around it* — the gap became a contract proposal, the proposal became a version bump, and the score went back to 100. That's the loop working under pressure.
