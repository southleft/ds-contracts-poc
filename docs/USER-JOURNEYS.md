...
### Inspect a prepared library in Figma

This technical preview is available in the local development checkout containing
prepared-library native delivery. After **Prepare React library**, choose
**Prepare this library for editable Figma inspection** beside the download.

1. Choose the saved library's **Theme** and **Brand**, then **Prepare editable Figma components**. Unsupported anatomy or geometry refuses before creation; the React download stays available.
2. Open **DS Contracts Evaluations** in Figma Desktop. Open **DS Contracts Sync Runner → Build → Connect the local source workflow**.
3. In the app choose **Get connection code**. Paste it into the plugin's **Local app connection** field and choose **Connect / resume**. Keep the code private.
4. Once connected, choose **Create and inspect in Figma** in the app. Wait for **Native structure checked** or a named refusal. Close the plugin window before inspecting the native canvas.
5. Keep this library's page link. Use **Inspect native output again** to read the existing output. If an inspection was interrupted, reconnect the same companion and use **Retry interrupted inspection** when offered. An uncertain creation outcome requires its saved result; starting a replacement is not a recovery step.

The saved page retains its theme, brand and operation across reloads. Structural
verification checks the supported graph and its bindings. The displayed native
exports are diagnostic; fidelity, editability and React behavior need their own
checks. The measured Badge return and remaining Tabs, vector and recovery gaps
are recorded in [current acceptance evidence](CURRENT.md#v1-acceptance-evidence).
This route does not apply arbitrary canvas edits to hand-authored React.
...
**Prepare an observed authored composition:** when the selected case has a supported composition, the current local candidate shows **Observed composition** and **Prepare … composition for Figma** instead of the root-only preparation control. This preserves the component’s own nested components at the captured inputs. Prepare it, then use that saved operation’s connection and creation controls. Repeating preparation reopens the same operation; loading unchanged originals after a server restart restores the saved observation and operations. The two recorded Radix Switch appearances reach this preparation step. They have not yet been created or visually qualified in live Figma. Their state inspection and update actions remain unavailable; a compiled composition does not establish an interactive state API.
...
`npm run prep:core` builds both schema and core for the local server. If startup reports `Failed to resolve entry for package "@ds-contracts/core"`, run this command; `npm run prep:schema` alone does not prepare a fresh checkout.

The Chromium install is required for **Validate React sources**, **Trace React structure** and browser comparisons. It downloads the revision pinned by this checkout. If capture reports that the browser executable is missing after a dependency update, rerun `npx playwright-core install chromium`. On Linux, missing browser system libraries require the platform setup described by `npx playwright-core install --help` (`--with-deps` installs them and may require administrator access). Static code import and fixture exploration can run without Chromium.
...
Nothing in the install steps above does. `npm ci`, `npm run prep:core`, `npx playwright-core install chromium`, `npm run playground`, the static code import and **Demo import (Badge fixture)** run without credentials, and `npm run eval` and `npm run docs:check` do not read one.
