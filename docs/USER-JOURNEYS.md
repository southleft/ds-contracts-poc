...
On a saved operation, **Prepare caller-content comparison** reads its original text, painted fonts and SVG viewports. The source must still match its recorded rendering. The app shows whether that content compiles and names unsupported facts. This preparation survives restart, but placing it into a native comparison instance and checking visual fidelity remain unfinished. The reusable main stays empty.
