...
**Keep earlier work accessible:** use **Recorded source run** on `/sources` to reopen an earlier capture and its own candidates and native inspections. A new validation keeps previous results intact. New captures can distinguish supported generated local IDs from caller-provided IDs; this does not upgrade old captures or qualify behavior.

**Compare at the same scale:** open a state under **Inspect native output**, then choose **Frame original for comparison**. The app measures that component in a network-isolated replay and requires the complete rendered screenshot to match the archived original exactly. It crops the recorded pixels around the measured bounds, including fractional edge pixels and up to eight pixels of context. The full original remains available. Both images then use the same 1×, 2× or 4× zoom, with their actual dimensions shown; large images scroll rather than being independently shrunk. Framing is saved separately from the original evidence and reused after reload. It never creates or changes a Figma component. Native transparency, backgrounds, subpixel placement and raster rounding can still differ, so this remains a diagnostic review rather than a calibrated pixel score or fidelity pass.
...
**Prepare a stateful component:** on `/sources`, choose the recorded source run, select **Checkbox** under **Component to trace**, then run **Trace rendered bindings** and **Prepare source candidate**. The preparation packages the original implementation and shows which omitted/false/true values and slots were observed. Zero means unobserved, not unsupported or false. Source anatomy preserves the checkmark's pseudo-element styling, original text and nested FieldNote fallback separately from supplied slot content. The anatomy counts describe prepared source structures; Checkbox native generation is still unfinished. Switching back to Button retains its separate preparation history.
