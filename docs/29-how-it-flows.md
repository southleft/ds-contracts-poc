...
flowchart TB
  ...
  CAR --> X["neither carried nor named = HARD FAILURE<br/>conformance/MANIFEST.json (82) · extract/figma/conformance/MANIFEST.json (157)"]
  LOW --> X
  REF --> X
```
...
UNSUPPORTED 18); the canvas-construct fixture is 157 cases (CARRIED 110 ·
...
| 82 cases (42 / 4 / 18 / 18) and 157 cases (110 / 38 / 9) | `accuracy/grammar.json:33-55` over `conformance/MANIFEST.json` and `extract/figma/conformance/MANIFEST.json` | `npm run conformance`, `npm run conformance:canvas` |
