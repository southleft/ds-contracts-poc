...
| **G1** | **Async CI↔Figma channel — deliver + verify, provenance-first.** Replace the synchronous pairing-code courier with a standing channel: CI-signed bundles the plugin verifies (repo/run/commit shown before Apply enables; unsigned falls back to pairing-code with an "unverified" badge), plus the read half — headless fingerprint drift recompute off a REST file dump so CI referees drift without a human clicking a tab. | all three; the daily loop dies here | ENGINE |
...
the "zero manual sync chores" claim are all downstream of it. G6 gates
