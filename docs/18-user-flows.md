...
4. Staleness guard: a three-way merge fed a stale base can bless an
   ...
   proposing." **[PARTIAL → G3: the Send tab's stale-base guard shipped
   2026-08-08 — the proposal WARNS by name (panel, summary head, PR/export
   envelope) when the base's fingerprint is not what the set's sync
   markers record; the ancestor check + refusal ride the merge surface,
   still downstream of G1]**
...
| **G3** | **Three-way merge surface (genesis × incoming × canvas), per-channel resolution.** Auto-merge non-overlapping channels; true collisions get a named mine/theirs choice rendered as before/after swatches, never channel names; "keep mine" flows into Propose→PR. Must refuse a stale base ("canvas is 3 syncs behind — deliver before proposing"), so it ships after G1. **STALE-BASE GUARD SHIPPED** (2026-08-08): `proposeDiff` now takes the set's stored sync markers (`ds_contracts/contractId` + `specHash`, the same inventory row the Send tab already reads) and compares them against the provided base's spec fingerprint — a mismatch WARNS by name ("this canvas was last synced from a different contract version than the base you diffed against — the proposal may contain reverts") in the Send panel, at the head of the summary, and in the PR/export envelope (`baseFreshness`); a matching base stays silent, absent markers verdict `unverifiable` rather than a silent match. Warn-and-name, never block (the G2 posture). Pinned in `scripts/plugin-engine-check.mjs` §5b (stale + fresh fixtures) and the `plugin-stale-base-guard` eval. The merge SURFACE itself remains open. | **PARTIAL** — stale-base guard shipped; three-way merge surface OPEN | all three; the scale-breaker | ENGINE |
...
| **G7** | **Brownfield write-back suggested diffs.** Design-led merges leave hand-written components stale. Emit anchor-derived suggested patches on the PR ("`Badge.module.css:14`: `var(--radius-md)` → `var(--radius-lg)`") — human applies, ideally as GitHub suggested changes; never auto-write. | **PARTIAL** `core/suggested-diff.ts` + `npm run suggested-diff:check` — propose-only stubs from channel changes + provenance anchors; no PR comment emitter / file:line from static readers yet | engineer | ENGINE |
