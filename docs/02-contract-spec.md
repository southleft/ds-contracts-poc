...
| `codeOnly: [{ state?, channel, value, reason }]` | **v18 — a capture-side receipt, never hand-authored.** State-plane facts the computed capture observed on this part and the contract grammar refused (a nested part's `focus-visible` `outline-width` — `Part.states` carries plain colour-kind refs only below the root; a state delta outside every mintable kind). Written by `extract/computed` fusion, preserved by promote | ignored (the code surface already renders the real CSS) | repeated verbatim as `capture`-kind **code-only facts** — `bundle.codeOnlyFacts`, the plugin run report, the set's `ds_contracts/codeOnlyFacts` plugin data — so a dropped focus ring is named where the designer reads |
...
**Structured props.** `type: { arrayOf: Record<field, 'text' | 'number' | 'boolean' | { enum: string[] }> }` declares a list-of-records prop (Breadcrumbs items, Select options). Code-only by declared fidelity limit — the canvas has no list-of-records property type — so the design binding is `{ "kind": "NONE" }` with no `property`, and every design-side consumer (figma generator, differ, diagnose) skips the prop rather than reporting it behind. Code renders `items?: Array<{ … }>`: no default destructure (undefined means "not provided", never a silent `[]`) and excluded from `...rest`. Guardrails: `arrayOf` ⇔ `kind: "NONE"` in both directions, no defaults, at least one field. For a repeated child, scalar fields match the named child prop's type; an enum field declares a nonempty subset of that child's canonical enum. Samples must use the declared choices. React applies the child's code-value binding when forwarding each item. Figma and other static projections render the observed `repeat.sample`; changing a React array does not establish live design-side collection editing. The proposer admits observed per-item enums only when a known child supplies the domain and each item's choice remains constant across parent variants. Otherwise it retains separate child instances for existing property threading.
...
`absentVariants` (additive, optional; [docs/23 §D.40](23-known-limitations.md)) declares the variant combinations a designer's set does not draw — a component set is often not the full Cartesian product of its axes. Each entry is **one complete tuple** over the contract's variant axes (every enum prop and every `VARIANT`-bound boolean, keyed by prop name; an enum value, a JSON boolean, or `null` for an axis's `unsetValue` option), listed in canonical order — tuples in the product's enumeration order; the order of keys inside a tuple is not part of validity. It is a canvas fact only: the Figma writer emits no variant for a listed combination, the exact variant projection expects the product minus the list **exactly** (a set with no declaration is still held to the full product), and the code surfaces do not read it — they render any prop combination by composition (`undrawn-combination-rendered-by-composition`, named in the emitted component). `validateContract` refuses by name a tuple outside the product, an incomplete or duplicate tuple, a non-variant axis, a non-canonical order, the default combination, a list that erases an axis value or the whole product, a list that leaves more combinations undrawn than drawn (`absent-variants-mostly-undrawn`), a product above 4,096 combinations (`absent-variants-product-too-large`), and the field together with `statePreviews` or a `native` representation.
