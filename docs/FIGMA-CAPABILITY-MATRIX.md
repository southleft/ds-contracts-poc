...
*Addendum (2026-07-19, S4 round 1 — north-star push): schema v15 landed a slice of §a and the §b declaration mechanism. Now CARRIED (the "Contract today" column is stale for these rows): shadow stacks incl. multi-layer/inset (a.1 — native effect lists), linear-gradient fills (a.3 — native `GRADIENT_LINEAR`; radial/conic + per-stop **binding** still deferred by name, stops render as literals), per-corner radii (a.4 — bound), per-side widths (a.5 — bound), the A22 text channels (a.2 — `textCase`/`textDecoration`/`textAlignHorizontal`/pixel `letterSpacing` literal; the lineHeight/fontSize literal→binding upgrade still deferred), `font-family` first-stack-entry (a.6 — literal `fontName`, STRING-binding upgrade deferred), `flex-wrap`/`layoutWrap` (a.8), `textTruncation` (a.9). The §b inexpressible set is now a first-class DECLARED vocabulary (`Part.declared`/`Part.declaredStates`, `DECLARED_CHANNELS` in `scripts/contract-schema.ts`): code emitters render the facts verbatim; the canvas carries the §b annotation copy in the component description — declared-not-drawn. Still open from the ranked list: image fills (a.7), blend modes (a.10), dash patterns (a.11), blur (a.12), background-repeat/size approximations, per-stop gradient variable binding, and drawing declared facts inside canvas state previews.*

*Addendum (2026-07-19, Round 4 — the one-to-one round): (1) padding LONGHANDS (`padding-left/right/top/bottom`) now bind per side — they were silently falling through `applyTokens`/`applyLiterals` (canvas-gate finding; each field is independently bindable per S5). (2) `aspect-ratio` joined `DECLARED_CHANNELS` as a **draw** verdict: the canvas sizes height from the bound width (Avatar/Thumbnail squares whose real height rides a pseudo-element padding hack the anatomy cannot carry). (3) svg icon nodes carry their contract `icon.size` (`createNodeFromSvg` yields a viewBox-scaled node; an unsized svg drew 0×0 in shrink-to-fit contexts — canvas-gate finding). (4) `position: absolute` joined the declared position grammar for PROMOTED inset overlays, lowered to `layoutPositioning: 'ABSOLUTE'` + STRETCH constraints (the B-3 finding-5 class, now contract-spelled). (5) base-hidden presence (declared `display:none` + per-value `stylesWhen` restore) resolves per compiled variant — defaultless-axis glyphs (Badge pip, Checkbox check) draw on exactly the right cells; boolean-conditioned entries evaluate at the drawn cell's defaults, a named limit. (6) The six Phase B-3 engine findings are fixed at source (shadow-token rem parsing, placeholder-color hardcode, amend-seed alpha, shape-branch effects, amendSet resize, inset-overlay lowering) — the canvas-corrections precedent class is retired.*
