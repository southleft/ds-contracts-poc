...
| Astryx, Polaris | **not measured** — neither declares a `varPrefix`, so the reader never runs and no `source-bindings.json` is written at all. *Not measured is not the same as zero.* |
...
Not every example in this repository is equally alive. Two of the six cannot be
regenerated by any command in the repo today.
...
| **Astryx** | **STALE — root cause fixed, recapture not landed** | §3.1 |
| **Polaris** | **FROZEN — but the harness now exists** | §3.2 |

### 3.1 Astryx: the root cause is fixed and gated; the recapture is not landed

Astryx is the one library that could not complete a capture→promote round. The
**cause** is fixed. The **artifacts are still stale**, and the reason is worth
reading, because it is a cost this project chose to see rather than absorb.

**Fixed and gated.** `extract/computed/configs/astryx.json` pointed all five
...
switch, which had never promoted at all. Five components in three states. The
config now points at `examples/astryx/contracts-seed/`, and the eval
`capture-seeds-are-not-promote-output` refuses this for **every** library —
verified to fire on all five astryx entries at the pre-fix state. Six libraries
obeyed the rule by convention; nothing enforced it, which is why the seventh
drifted for rounds with every gate green.

**Proven, then deliberately reverted.** A full round was run to verify the fix
works end to end: capture exit 0 on all 5 components, promote exit 0 with the
resolution guard green, **promote twice byte-identical**, provenance back to one
sentence each, 13/13 Figma scripts passing their compile receipt. It was not
kept, because three independent gates caught what it costs:

| gate | before | after the recapture |
|---|---|---|
| `astryx-reanchor-minted` | 63 applied / 0 ambiguous | **6 applied / 40 ambiguous** |
| `minted-leaves-bind-to-something` | astryx 26 unreferenced | **42 (grew)** |
| `gate-inventory-shipped-minted` | a real pin | went decorative |

The cause is one fact: **12 of the 19 reviewed re-anchoring rows anchored
`row-rule-color` leaves**, the currentColor mirror the mint-cleanup round folds
away corpus-wide. Astryx could not be recaptured at the time, so it kept them,
and its ledger had been hand-anchoring a channel the engine now removes by
itself. Once the fold applies, those rows have nothing left to anchor, and
astryx's semantic coverage collapses with them.

**So the recapture is a two-part round, not a one-part round:** recapture, *then*
re-run the re-anchoring review against the fresh mint (40 ambiguous refs, each
needing role-grade evidence). Landing the first half alone would trade a stale
library for a semantically poorer one. That is named here rather than shipped.

### 3.2 Polaris: the harness now exists; the recapture has not been run

This section used to say Polaris could never be recaptured. That is no longer
true, and the sentence it rested on was itself wrong.

**What was fixed (2026-07-29).** `examples/polaris/.polaris-sandbox/` exists and
its **recipe is committed** — `package.json` + `package-lock.json` pinning
`@shopify/polaris@13.9.5`, `react@18.3.1`, `react-dom@18.3.1`, `esbuild`
(`cd examples/polaris/.polaris-sandbox && npm ci`). The install is git-ignored;
the lockfile is what makes it reproducible.

**A claim this repo made that was false.** Both this file and
`examples/polaris/README.md` asserted that every *other* library "commits a
git-ignored but reproducible sandbox recipe with a pinned `package.json` +
`package-lock.json`." No library did. `git ls-files examples/carbon/.carbon-sandbox`
returns nothing, and the same holds for mui, tailwind, altitude and astryx —
each `.gitignore` ignores the sandbox directory whole, and the recipes survive
only as prose in the `PROVENANCE.md` files. Polaris is now the **first** library
whose harness is reproducible from committed bytes. Bringing the other five to
that bar is follow-up work, named here rather than implied.

**What is still true.** The recapture itself **has not been run**, so Polaris's
12 committed contracts are still frozen, and engine fixes in the capture half
still reach it only through the offline re-fuse
(`npm run extract:computed:regate`). Polaris also remains the one library the
Figma-script freshness gate cannot check: its `figma/*.figma.js` come from
`generate.ts` (the provisional-minting path), not the CLI `figma` command, and
that invocation is still unrecorded.

**And the reader question is now answered — with a measurement.** Polaris's
`varPrefix` is absent, which is why all 980 of its minted leaves are anonymous
literals ([§2.1](#21-overlays-and-portals-lose-their-source-token-names-in-every-library)).
The open question was whether its CSS Modules compile token names away to
literals the way StyleX does — in which case the honest answer would be a
family-split receipt, not a re-run. Measured against the published 13.9.5
tarball (`build/esm/styles.css`, 499,065 bytes): `var(--p-*)` appears **2,727
times at point of use** across **328 distinct** custom properties, with 452
defined. **Polaris binds through custom properties.** The zero is not a property
of the library — the CSS-vars reader simply arrived at library #4 and library #2
was never re-run with it. Adding `varPrefix: "--p-"` and recapturing should
produce a large source-alias gain on the repo's most-cited exemplar.
...
27.04 still over the bar). The table has not been marked superseded.
