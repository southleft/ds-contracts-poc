...
| **Astryx** | **STALE — reverted, cannot be re-promoted** | §3.1 |
| **Polaris** | **FROZEN — a permanent hole** | §3.2 |

### 3.1 Astryx: two pre-existing blockers, both reproducible at HEAD

Astryx was re-captured successfully in the recapture wave **and then reverted**.
`examples/astryx/PROVENANCE.md` names both blockers:

1. **`promote` refuses on a stale ledger row.** `promote-floor.ts` fails with
   `REFUSED: decision RA-ffffff: leaf 'imported.button.label.color.primary' is
   not in the minted tree.` The committed `tokens/reanchor-decisions.json`
   acknowledges a leaf the current engine no longer mints. Verified pre-existing
   by stashing the fresh capture and re-running against the committed artifacts:
   it fails identically. **Astryx's contracts cannot be re-promoted by any
   command in this repository** until that entry is re-anchored or retired.

2. **The capture loop is not idempotent for Astryx.** It is the one library
   whose capture config reads its **own shipped contracts** —
   `extract/computed/configs/astryx.json` points every component at
   `examples/astryx/contracts/<c>.contract.json`, where Carbon/MUI/Tailwind point
   at a `contracts-seed/`. Each capture+promote cycle appends another provenance
   sentence to every `description` and bumps the `version` (observed: card
   `0.1.0` → `0.3.0`, with the same provenance line written twice). Committing
   one turn of that loop would ship a self-referential artifact that grows on
   every run.

Only its Figma emission half is fresh. Its Switch came back **77.344%** against
a committed 76.302%, which means a known engine improvement **would have closed**
a drift-baseline gap and did not.

Astryx is also the reason the drift instrument pins 54 rows against 62 committed
contracts: 8 of its 13 contracts came from the static Phase-A path and carry no
captured floor. **A contract existing is not the same as a contract being
measured.**

**Status** — closing it is a round of its own: re-anchor or retire `RA-ffffff`,
then repoint the capture config at a committed `contracts-seed/`. Named, not
started.

### 3.2 Polaris: no committed sandbox, therefore no recapture, ever

Every other library commits a git-ignored but **reproducible** sandbox recipe
with a pinned `package.json` + `package-lock.json`. Polaris has none — the
harness directory's path, contents and lockfile are recorded nowhere in this
repository (`examples/polaris/README.md`, "PERMANENT HOLE").

Consequences, all of them stated there:

- Polaris's committed `captured-truth.json`, `enriched.contract.json` and
  scorecards are **frozen at whatever engine produced them**. They are the only
  artifacts in the corpus that no command in this repo can regenerate.
- Engine fixes reach Polaris only through the **offline re-fuse**
  (`npm run extract:computed:regate`). **Anything that lives in the capture half
  — a new channel read, a new pseudo-element, a refusal that depends on
  re-measuring the DOM — cannot reach Polaris at all.**
- Polaris is the one library the Figma-script freshness gate cannot check: its
  `figma/*.figma.js` come from `generate.ts`, and the exact invocation is not
  recorded either. The gate **prints it as a named hole** rather than skipping
  the directory silently.
- Its committed bundle still lacks its icon SVGs.
- Its second capture config, `polaris-depth.json`, has no committed scorecard, so
  the drift instrument skips it — **and prints that it did**.

**Status** — closing it means committing a `.polaris-sandbox/` with a pinned
lockfile, adding an `examples/polaris/PROVENANCE.md` of the same shape as the
other five, and re-running the capture. A round of its own; not started.
