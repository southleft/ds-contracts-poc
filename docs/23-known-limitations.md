...
| canvas constructs | 152 | CARRIED 105 · LEDGERED 38 · REFUSED 9 (152 PASS · 0 RED-EXPECTED — the last two closed 2026-08-23, §D.32) | `extract/figma/conformance/MANIFEST.json` (`npm run conformance:canvas`) |
...
| D.32 | The two acceptance rows that were red on the commit itself: a drift "check" that wrote tracked files and a referee reading the wrong contracts | 2026-08-23 | `COMMIT-PENDING` |
...
recognisable; Card is not (§B.26); the two silences closed 2026-08-23 (§D.32).
**Gates:** `npm run conformance:canvas` (152 cases, 152 PASS — every exam case
pinned before any fix; the two pinned RED-EXPECTED here stayed red until §D.32), `npm run prop-collision:check`
...
## D.32 The held-out kit's last two silences, and the SLOT's primary-axis FILL — CLOSED
...
# the canvas-construct fixture, incl. the held-out kit's cases (§D.24, §D.32)
...
- *The committed numbers already disagreed with the committed artifacts.*
  ...
  wave was to re-record them; it never did), and 17 mui components plus 6
  tailwind ones had a harness scorecard and no baseline row at all — which
...
command above. Commit `COMMIT-PENDING`.
