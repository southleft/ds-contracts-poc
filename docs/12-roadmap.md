...
- [x] 176/176 deterministic machinery evals ([docs/07](07-validation.md))
...
- [ ] **Close the declared schema gaps** ([docs/08](08-composition-and-spec.md)): nested-part states, parent→child prop mapping, slot `min`/`max` + `restrict` enforcement, slot default content
  - *nested-part states, narrowed (state-plane projection round, 2026-07-25)*: `Part.states` exists on nested parts (v13) but carries **plain color-kind refs only** — no placeholder substitution. So a state delta that is a function of a state AND an enum axis (MUI Switch's hover/active track background, f(color, hover)) is a named `overflowBindings` refusal, not a carried fact. The BASE-plane equivalent is now solved — a nested two-axis base fact rides a per-value `tokensByProp` map keeping one placeholder (`core/mint-tokens.ts`, `extract/computed/fuse.ts`) — so the remaining gap is precisely "the same carriage, on the state plane". Prop-selected renderings are NOT part of this gap: `checked`/`expanded`/`selected` are variant axes and already ride the base plane.
