...
The position this PoC takes: **the source of truth is neither surface. It's a machine-readable contract that sits between them.** The canvas library and the React library are both *renderers* of the contract — generated from it on first pass, validated against it forever after.

This isn't a novel claim (see the spring 2026 essays from izaias, TJ Pitre, and Cristian Morales Achiardi converging on it), but nobody has shipped it: every existing artifact makes one side primary. Custom Elements Manifest, Storybook manifests, and story.to.design are code-first. Supernova and the EightShapes Specs plugin are design-tool-first. Subframe and Knapsack own both ends and are proprietary. The token layer is the exception — the DTCG format proved a neutral, Git-versioned, tool-agnostic artifact can govern both sides. **This PoC runs the DTCG playbook one level up, at the component-API layer.**

## The model: generative first, diagnostic forever after

Two prior architectures compete in the literature:

- **Compile-forward** (izaias): the contract is generative; everything downstream is regenerated; the design tool is demoted to a renderer. Clean, but has no answer for when code legitimately gets ahead of design.
...
1. **First pass is generative.** The contract compiles both sides. Code side: React components, CSS Modules, CSF3 stories (`npm run build`, working today). Design side: design-tool variables with modes/aliases and canvas component sets with variant properties (phase 2). (Our reference implementation binds to a specific commercial design tool; the tool-specific sync mechanics live in docs/internal/figma-sync.md.)
...
```
  code change ──extract──▶ contract diff ──PR + review──▶ contract ──generate──▶ canvas
...
| **2** | Design-tool variables + canvas component sets generated from the same contracts | The contract is generative for design; one definition, two renderers |
| **3** | Extraction + three-way diff + promotion flow | The contract is diagnostic; the arbitration model works in both directions |

The phase-3 demo that settles the argument: add a prop by hand in `Button.tsx` → parity flags code-ahead-of-contract → accept the generated contract patch → the canvas component set sprouts the property automatically. Then the reverse, starting from a variable change in the design tool.
...
| Prop/value binding grammar (`"primary"` ↔ `"Primary"`) | Code Connect (the reference design tool's design-to-code binding bridge) |
...
One cautionary datapoint shaped the scope: Specify built a standalone "design API" business between the design tool and Git and sunset. This PoC deliberately proves a **format + generators + linters** — plain files in a repo — not a hosted middleman.
