...
|---|---|---|
| **Eval suite** | 191/191 evals as of `evals/results.json` — by claim family: 30 refusal, 32 determinism, 48 detection, 58 extraction, 4 convergence, 5 CLI, 13 journey, 1 theming (derive it: `python3 -c "import json,collections; print(collections.Counter(x['claim'] for x in json.load(open('evals/results.json'))['results']))"`) | `npm run eval` |
...
npm run eval                       # 191/191 as of evals/results.json
