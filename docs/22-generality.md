...
|---|---|---|
| **Eval suite** | 225 evals as of `evals/results.json`, 223 green with two named reds — by claim family: 36 refusal, 35 determinism, 67 detection, 64 extraction, 4 convergence, 5 CLI, 13 journey, 1 theming (derive it: `python3 -c "import json,collections; print(collections.Counter(x['claim'] for x in json.load(open('evals/results.json'))['results']))"`) | `npm run eval` |
...
| **total** | **104** | **101** | **943 (7 libraries)** | **8.7% (7 libraries)** | the size and coverage totals cover the SEVEN libraries with a measured denominator; Fluent's 11 contracts and 11 pinned components are counted in the first two columns and excluded from the last two, because a fraction with an unmeasured denominator is not a smaller number, it is no number |
