...
| **total** | **101** | **98** | **943 (7 libraries)** | **8.7% (7 libraries)** | the size and coverage totals cover the SEVEN libraries with a measured denominator; Fluent's 11 contracts and 11 pinned components are counted in the first two columns and excluded from the last two, because a fraction with an unmeasured denominator is not a smaller number, it is no number |
