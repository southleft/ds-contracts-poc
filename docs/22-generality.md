...
|---|---|---|
| **Eval suite** | 227/227 evals as of `evals/results.json` — by claim family: 37 refusal, 35 determinism, 68 detection, 64 extraction, 4 convergence, 5 CLI, 13 journey, 1 theming (derive it: `python3 -c "import json,collections; print(collections.Counter(x['claim'] for x in json.load(open('evals/results.json'))['results']))"`) | `npm run eval` |
