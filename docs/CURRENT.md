...
The app now prepares source/runtime candidates for Button and Checkbox. Choose Checkbox in **Component to trace**, complete **Trace rendered bindings**, then choose **Prepare source candidate**. The four recorded Checkbox states retain six boolean properties, source text, nested hosts and all three declared slots. A per-property table distinguishes omission, explicit false and explicit true; missing values and the unobserved error slot remain visible. New Checkbox preparations also preserve source anatomy for all four states: pseudo-element styling remains attached to its actual source owner, and the authored FieldNote fallback remains distinct from supplied slot content. Reopening either an earlier preparation or a new one verifies its original evidence without rebuilding. The next stateful step is lowering this anatomy into native components, including qualified nested dependencies and editable state/content mappings.

The original Checkbox and its nested FieldNote can now be packaged from recorded source and dependencies. Local browser checks cover checked/disabled state, the original change and Enter-key handlers, field-note updates and slot content; strict consumer type-checking passes. A conflicting nested component registration is refused. This preparation action does not generate Checkbox components in Figma or qualify their visual fidelity. Full behavior and the source's mixed-state accessibility remain unqualified.
...
The source reader retains TypeScript return casts and authored static-HTML input/label syntax. Fresh app captures can now observe the actual Lit render result and corroborate registered nested-component tags against exact source strings and archived replay. The observer does not call render again or rewrite the source, and unexpected substitutions remain refusals. The app’s **Trace rendered bindings** action now lets users choose Button or Checkbox. The Checkbox trace replays all four recorded states with fresh lifecycle-correlated semantics and reports source text and nested hosts without claiming behavior or conversion. Component histories remain separate. The structural trace now retains pseudo-elements with their source-element owners and independently checked styles, and maps literal or scalar source text to its exact DOM position. The original Checkbox field note is traceable through its nested fallback component. Content changes still need causal verification; native nested-component preservation and editable state mappings still require integration. These observations do not enable a new Checkbox generation action.
