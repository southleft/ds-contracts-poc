...
**The next gap is complete, qualified component content.** Reusable mains contain empty editable slots. The app can now fill a separate instance of an observed main with supported caller content for comparison. Checkbox's runtime content and the complete nested Card remain unfinished and stay in the cohort denominator. Existing composition rules preserve supported nested scalar properties in React and expose child controls in Figma; live parent-to-child text and Boolean property aliases still refuse. Root inspection is not a completed code-to-design journey. Reloading unchanged originals after a restart now restores structure observations pinned by an existing operation, without rerunning the property matrix. The composed Card review lists all seven nested instances and the missing native mappings or unsupported inputs. Supported child roots can now be prepared from their saved source observations through the same companion workflow. One live CardContent main passed independent readback and appears as a verified match in the review (1 of 7 children). Its empty slot is editable; other inputs, behavior and visual fidelity remain unqualified. Composed generation remains blocked until every required child is resolved.

The shared conversion engine now preserves supported grid content inside editable Figma slots, including token-bound row and column gaps. A live comparison retained its main link, independently verified three editable cells, and refused duplicate creation on repeat. Reverse extraction preserves one React grid root; browser checks cover resized, reordered and removed content in both React styling formats. This is isolated integration evidence for declared tracks and forward row flow; differing grid layouts across variants still require qualified reverse mapping. The application’s source-derived Card still needs its authored layout and remaining nested mappings resolved before the composed journey is complete.

The shared engine also supports a declared extra-row sizing rule. Managed comparison writes grow native row declarations for supplied content while keeping reusable mains empty; reverse extraction preserves the rule after checking native tracks. Direct Figma child edits do not automatically maintain that rule and can require companion reconciliation. This shared capability has not yet been connected to the Card’s source constraints.

The caller-content review now shows current CSS grid constraints separately from rendered pixel tracks. This targeted read preserves the original source and saved observations. The Card header uses an implicit column and content-sized rows; converting that layout remains unfinished. The inspection does not qualify native conversion or responsive behavior.
