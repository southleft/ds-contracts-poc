...
**Trace React structure** pairs the untouched source with a separate component observation. It identifies mounted exports, repeated instances and nested parents, and requires identical screenshots and measured trees before reporting a match. The app shows the hierarchy and paired images for all ten selected cases. This uses a version-bounded React observer; unsupported renderers, ambiguous export aliases, unmanaged DOM and content outside the selected root remain named refusals. The result does not establish content-slot ownership, behavior, source styling rules or a qualified native contract. Those observations still need to feed native generation and verification.
