...
**Source property effects:** the structure trace now varies finite string/null properties one at a time in an isolated copy of the real source, including omission where the API allows it. The app shows the original and changed render, identifies affected nested components, and verifies restoration of the original screenshot, measured tree and component ownership. Unsupported properties remain listed. The shared compiler now assembles each completely observed property into a root style draft with typed values, omission/default handling, a replaceable children slot and retained source token bindings. The app reports compiled native root variants and unprojected styling. These drafts do not assemble descendant effects or combinations of properties, and have not qualified native visual output.
