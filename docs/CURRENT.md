...
**Inspect initial states** now reuses the saved source archive to vary finite caller-supplied inputs on fresh React mounts. The app saves the observed states and original-pixel previews, verifies restoration, and reopens the same result without rerunning the matrix. A live Checkbox action observed the supported initial input combinations, including omission. This is initial-render evidence only: native state mapping, keyboard behavior and interaction preservation remain unqualified. The action currently requires a previously prepared root operation to locate its source archive; general source onboarding is unfinished.
