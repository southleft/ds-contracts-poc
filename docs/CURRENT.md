...
The application has also generated the 24 observed Button initial states with native text, content-sized width, fixed height and bound variables. **This family remains visually unqualified:** seven exports exceed the existing image-difference limit. The application repaired unrequested invisible default root fills on the 24 retained mains, preserving all 74 node identities. A separate readback now verifies supported structure; the original failed observation remains in its journal. Fresh exports retain the same seven image failures. The selected disabled/default rendering passes the image limit at 2.58%, which does not override those failures. Static text support checks font availability before allocation and verifies text, font and token identity independently. Wrapping, constrained or mixed fixed/content-sized initial-state widths remain refused.
