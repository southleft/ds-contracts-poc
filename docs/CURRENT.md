...
The next stateful step is to carry Checkbox anatomy, checked/disabled state and content through candidate preparation and native generation. The current candidate adapter is still Button-specific. The original Checkbox and its nested FieldNote can now be packaged from recorded source and dependencies. Local browser checks cover checked/disabled state, the original change and Enter-key handlers, field-note updates and slot content; strict consumer type-checking passes. A conflicting nested component registration is refused. This is internal runtime preparation, not a new app generation action or visual qualification. Full behavior and the source's mixed-state accessibility remain unqualified.
