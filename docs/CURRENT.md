...
| Figma → reusable React | Readers propose contracts and deterministic emitters generate React. Both React emitters preserve declared root-toggle indeterminate state, disabled behavior and controlled values; an omitted state without a default remains unset. | Complete source-to-contract behavior mapping and install the design-only output in a clean consumer, including content, variants, accessibility and declared behavior. |
