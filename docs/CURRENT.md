...
| Figma → reusable React | Readers propose contracts and deterministic emitters generate React. Both React emitters preserve declared root-toggle indeterminate state, disabled behavior and controlled values; an omitted state without a default remains unset. Declared next-value callbacks let consumers update controlled root and nested toggles, including triggers with literal text or slots. | Complete source-to-contract behavior mapping and install the design-only output in a clean consumer, including content, variants, accessibility and declared behavior. |
...
**Inspect state and callback behavior** now runs targeted original-source trials from the saved structure observation. For the current Checkbox, the application observed `checked` controlling state and `defaultChecked` supplying only the initial state, with `onCheckedChange` returning the next boolean value. Twelve trials cover all three supplied values, label and Space activation twice, live input updates, and exact restoration of source rendering and ownership. This establishes a bounded source relationship; admission into the contract and verification of generated controlled/default behavior remain unfinished.
