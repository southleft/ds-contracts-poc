...
The app now prepares source/runtime candidates for Button and Checkbox. Choose Checkbox in **Component to trace**, complete **Trace rendered bindings**, then choose **Prepare source candidate**. The four recorded Checkbox states retain six boolean properties, source text, nested hosts and all three declared slots. A per-property table distinguishes omission, explicit false and explicit true; missing values and the unobserved error slot remain visible. Reopening the preparation verifies its evidence without rebuilding. The next stateful step is native visual projection and editable state/content mappings.
