...
**Inspect React APIs** reads the original component modules with their installed TypeScript declarations. It retains inherited property types (including Checkbox's boolean/indeterminate union), declared defaults, forwarded spreads, conditional root choices and nested component references. The app distinguishes unresolved reads and stores the original source/declaration hashes privately. These facts expose information the single-file importer currently misses, including inherited types and the `Comp` alias. The live family read retains nine definitions but remains incomplete because their inherited `inlist` field is typed `any`; that field is named as unresolved. The same action now feeds supported installed API facts into the existing contract proposer and displays nine incomplete contract drafts. Optional booleans retain their type and distinguish omission from false; required text receives no invented default. Inherited platform forwarding is listed separately from the native API. Finite mixed and nullable domains now retain exact React values in the drafts: Checkbox's checked states include booleans and `indeterminate`, and Button's variant and size include `null`. Omission remains distinct from an explicit value. Other unrepresentable types remain named omissions. These are incomplete proposals, not accepted contracts. Binding source roots, nested components, content and styling to rendered nodes, and native generation remain unfinished.

**Reusable React content:** installed-source proposals now retain a default `children` slot when the source proves unchanged caller content reaches a host root. Spread order, explicit JSX overrides, destructured aliases and excluded rest fields are checked; transformed, defaulted, escaped or mutated children remain unresolved. The app lists these slots separately from scalar APIs. The focused source-program and proposal checks cover these boundaries and mount both generated React formats with replacement nested content, empty content and zero. This preserves a content boundary, not the sample Card arrangement. Bounded native root-slot projection now preserves an editable content region for forward, non-wrapping flex and inline-flex layouts. Inline display is retained in checked readback to React; changing outer display across variants remains unsupported. A live compiler check retained caller content through repeat and size changes; captured empty-main structure returned to React children without an extra wrapper, with matching content geometry in both React output formats. This is an engineering check, not the completed source-to-Figma application journey. Grid, wrapping, reversed flow, space-between and readback of nonempty main defaults remain unqualified; imported and conditional roots still need proof of content consumption.

**Trace React structure** pairs the untouched source with a separate component observation. It identifies mounted exports, repeated instances and nested parents, and requires identical screenshots and measured trees before reporting a match. The app shows the hierarchy and paired images for all ten selected cases. This uses a version-bounded React observer; unsupported renderers, ambiguous export aliases, unmanaged DOM and content outside the selected root remain named refusals. The trace now joins exact source identities to rendered root boxes and source-proven caller-content boundaries. The composed Card retains nested instances as references rather than baking its sample arrangement into reusable anatomy. Conditional host branches and dependency-rendered roots remain visibly distinguished; unresolved APIs stay listed. This correspondence does not establish reusable styling rules, behavior or a qualified native contract. Those observations still need to feed native generation and verification.

**Source property effects:** the structure trace now observes admitted finite string/null properties together in an isolated copy of the real source. It covers their full Cartesian product, including optional omission, and refuses domains above the bounded observation limit. Each update changes the selected properties atomically, records the changed render and affected nested components, and verifies restoration of the original screenshot, measured tree and component ownership. Unsupported properties remain listed. The shared compiler assembles complete observations into a combined root style draft with typed values, verified omission/default handling, a replaceable children slot and retained source token bindings. Generated React checks exercise interacting properties in both output formats, including a defaultless property; the inline emitter now resolves conditional token references that depend on another property. The app reports compiled native root combinations and unprojected styling. The combined draft now retains fixed width/height constraints verified against winning CSS declarations and typed computed values, including conditional fixed sizing beside automatic sizing. Measured content widths and caller style overrides do not become component dimensions. This is scoped to the observed inputs and theme; responsive sizing, unresolved dimension ownership, Boolean/runtime state, descendant effects and native visual fidelity remain unqualified. These are diagnostic drafts, not application-created Figma components.

**Native conversion check:** the structure trace now passes each source-proven content root through the shared layout, token and native compilers. It retains font and border declarations on the component’s own box while leaving caller content empty. Each observed root reports compiled, unsupported-layout or unresolved-content status, with remaining style facts available for inspection. These are per-observation diagnostics: sample dimensions, the full source API, other property combinations, authored token identities and assembled native comparisons remain unqualified. The application does not create Figma components from these diagnostic drafts. The trace now retains direct CSS variable identities for supported root color and font-weight bindings using browser-reported layers and selector priority. Equal-valued variables stay distinct. Ambiguous precedence, indirect values and conflicting variable scopes remain unresolved; this does not qualify token modes or the full source API.
