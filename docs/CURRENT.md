...
| Changes and repair | The app applied reviewed opacity corrections to Checkbox variants, a fixed-height correction to a Button main, and shadow-stack corrections to two Card mains, with independent readbacks and preserved node identities. Channel diffs, three-way merge rules and proposal planning also exist. | Corrections cover opacity, literal dimensions on supported empty flex roots, and supported root shadow stacks. The general sync spine still plans without applying. General two-way apply, baseline advancement, interruption recovery and rollback still need end-to-end proof. |
...
The saved Checkbox control exports pass that image limit in ten of twelve initial-state combinations; enabled indeterminate states fail at 8.82%. Native inspection identified strokes retaining their original weight when an SVG was resized after import. The shared compiler now supplies the intended SVG viewport before import, and an isolated live experiment confirms correct stroke scaling. Existing Checkbox nodes still require a guarded application update and fresh comparison; the compiler fix alone does not qualify them. These control-only images do not establish label relationships or keyboard behavior.
