...
**Reusable React content:** installed-source proposals now retain a default `children` slot when the source proves unchanged caller content reaches a host root. Spread order, explicit JSX overrides, destructured aliases and excluded rest fields are checked; transformed, defaulted, escaped or mutated children remain unresolved. The app lists these slots separately from scalar APIs. The focused source-program and proposal checks cover these boundaries and mount both generated React formats with replacement nested content, empty content and zero. This preserves a content boundary, not the sample Card arrangement. Native root-slot projection is still unsupported and now refuses explicitly instead of dropping that content; imported and conditional roots still need proof of content consumption.
