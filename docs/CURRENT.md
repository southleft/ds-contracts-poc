...
The retained default Button comparison still fails the existing image limit: 10.18% antialias-tolerant difference against 5%. A guarded comparison-frame correction changed only its outer clipping flag; independent readback preserved the component, tokens and node identities. The source uses `background-clip: padding-box` with a transparent 1 px border, while native output paints the full outer box. That channel is currently annotated rather than implemented in Figma. Supporting this shared paint rule and rechecking the remaining Button cases are still required; structural verification does not resolve the image failure.
