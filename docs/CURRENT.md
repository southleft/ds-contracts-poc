...
The retained default Button comparison still fails the existing image limit: 10.18% antialias-tolerant difference against 5%. A guarded comparison-frame correction changed only its outer clipping flag; independent readback preserved the component, tokens and node identities. The source uses `background-clip: padding-box` with a transparent 1 px border, while native output paints the full outer box. The shared compiler now represents supported uniform padding-box backgrounds as separate editable paint layers, retaining outer geometry and color bindings. An isolated native experiment lowered the image difference to 3.92% and preserved the inset as text grew. Compiler/native execution checks also cover populated instances. The guarded main-component migration now passes companion execution checks, including repeat runs, lost acknowledgements and preservation of concurrent edits. It has not yet been applied to the retained application output. Linked-comparison migration now also passes application-journal checks for independent verification, restart, duplicate-write refusal and retained creation records. These checks use a simulated Figma host; live application of both migrations, remaining Button cases and reverse reconstruction of the paint layer are still required. Direct radius-token edits also require recompilation of the derived inner radius.
