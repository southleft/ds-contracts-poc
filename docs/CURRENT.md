...
The application has also generated the 24 observed Button initial states with native text, content-sized width, fixed height and bound variables. **This family remains unqualified:** readback rejects an unrequested invisible default fill on its roots, and seven exports exceed the existing image-difference limit. The retained run needs a guarded correction and fresh verification; it must not be recreated to hide the failure. The selected disabled/default rendering passes the image limit at 2.58%, which does not override those failures. Static text support checks font availability before allocation and verifies text, font and token identity independently. Wrapping, constrained or mixed fixed/content-sized initial-state widths remain refused.
