...
Two acceptance commands are known to fail or misbehave on the current commit
and are named in [Known Limitations §B.28](23-known-limitations.md#b28-two-release-evidence-commands-from-the-2026-08-22-audit-still-open):
`npm run diagnose` exits non-zero on the committed first-party snapshot, and
`npm run extract:computed:drift` dirties tracked files while it runs.
