...
| **V1-JOURNEY-04** | Journey A on the recipe path MUST include a canvas→code held-out exam: a component set the recipe path never minted as a first-class stay goes committed-observe → canvas facts → bridge → emit → Chromium computed-style diff with **zero silent losses**. Passing this exam does **not** alone flip product v1 or any archetype `overallSuccess`; v1 still requires F1 (code→canvas unseen-library) under the same release commit. | `npm run recipe:canvas-to-code:held-out:check && npm run recipe:canvas-to-code:held-out:v2:check`; evidence: [docs/32 F-C2C row](32-recipe-ir-pivot.md#merge-execution-2026-08-30) and, on a designer's file, [CANVAS-TO-CODE-DESIGNER-EXAM.md](../parity/receipts/v1/CANVAS-TO-CODE-DESIGNER-EXAM.md). Lanes today: fast (both). |
...
| **V1-COMPAT-03** | Given the same supported inputs, generation MUST remain byte-deterministic and a refused batch MUST leave its destination untouched. No model output may participate in conversion. | `npm run eval:carried:check && npm run eval:record:check && npm run generation:atomic:check && npm run provenance:check && npm run figma:fresh && npm run verify:catalog`; the committed eval record carries the commit it measured and eval:record:check refuses a dirty-tree or foreign record and, in the full lane, a record CI did not reproduce row by row. A refused contract in generate leaves no file and names itself; the rest of the batch is written. Lanes today: fast (eval:record:check, generation:atomic:check, provenance:check, verify:catalog), full (eval:carried:check, figma:fresh). `npm run eval:carried:check` runs the full suite and permits a red ONLY where parity/receipts/v1/eval-reds.json names it with a measured cause and a stated closing condition, refusing an unnamed red and a stale row whose eval has gone green — the owner's 2026-09-03 decision to carry the five remaining reds, made readable here instead of collapsing the row to a bare non-zero exit. |
...
| ID | Requirement | Exact acceptance command or evidence |
|---|---|---|
| **V1-EVID-01** | Every v1 capability claim MUST name an adversarial automated check or a committed measured receipt. A new claim without either blocks release, and a late refusal MUST leave generated output untouched. | `npm run eval:carried:check && npm run docs:check && npm run capability:fresh && npm run generation:atomic:check && npm run static:empty-content:check && npm run code-only-facts:check`; every canvas-dropped fact is named per part, channel, value and reason (codeOnlyFacts) where a person reads it — the bundle, the plugin report, figma bundle stdout. Lanes today: fast (all but the suite), full (eval:carried:check). `npm run eval:carried:check` runs the full suite and permits a red ONLY where parity/receipts/v1/eval-reds.json names it with a measured cause and a stated closing condition, refusing an unnamed red and a stale row whose eval has gone green — the owner's 2026-09-03 decision to carry the five remaining reds, made readable here instead of collapsing the row to a bare non-zero exit. |
