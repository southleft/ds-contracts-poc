...
| **V1-JOURNEY-04** | Journey A on the recipe path MUST include a canvas→code held-out exam: a component set the recipe path never minted as a first-class stay goes committed-observe → canvas facts → bridge → emit → Chromium computed-style diff with **zero silent losses**. Passing this exam does **not** alone flip product v1 or any archetype `overallSuccess`; v1 still requires F1 (code→canvas unseen-library) under the same release commit. | `npm run recipe:canvas-to-code:held-out:check`; evidence: [docs/32 F-C2C row](32-recipe-ir-pivot.md#merge-execution-2026-08-30). Lanes today: fast. |
