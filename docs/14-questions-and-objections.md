...
Both directions ship today. Greenfield: contracts generate both surfaces. Brownfield: `npm run extract:code` reads a real component library into schema-valid proposed contracts, `npm run reconcile` produces the disagreement report between your own two surfaces, and `npm run diagnose` referees them continuously — none of which assumes generation ([docs/13](13-try-it-with-your-system.md)). Receipts: extraction ran against **Shoelace** (60/60 components via its Custom Elements Manifest) and **Mantine** (245 components, 1,691 props, <1s, 100% of unreadable components reported rather than silently dropped) — two shipping libraries this project doesn't own (`extract/pilots/`).
