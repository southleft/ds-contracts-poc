...
This PoC makes eight falsifiable claims. Each is backed by an automated eval (`npm run eval`, 183 cases, runs the real pipeline in a scratch copy — not mocks) or an executed live design-tool check. Current status: **183/183 deterministic evals pass** (`evals/results.json`), all live checks pass. This section is written to be lifted into a PRD.
...
npm run eval      # 183 deterministic evals → evals/results.json
