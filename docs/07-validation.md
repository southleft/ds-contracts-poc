...
This PoC makes eight falsifiable claims. Each is backed by an automated eval (`npm run eval`, 228 cases, runs the real pipeline in a scratch copy — not mocks) or an executed live design-tool check. Current status: **228/228 deterministic evals pass** (`evals/results.json`) — the two long-standing named reds (`mui-figma-genesis`, `child-wider-ratchet-and-script-freshness`) were closed on 2026-08-16 — and all live checks pass. This section is written to be lifted into a PRD.
...
6. **INTERIOR-NODE overrides do not survive an amend, and nothing tests them.** The amend path deletes every child of an existing variant and rebuilds it from the spec (`core/emit-figma-script.ts:3958`; `:4107` for single components). Component *properties* live on the set and are re-attached by name, which is why the three overrides the 2026-07-08 forensics tested (label, variant, boolean) all survived. An override a designer applied to an interior node — a nudged child frame, a swapped nested instance, a recoloured layer — is **destroyed**, on greenfield update-in-place exactly as in a brownfield kit. No eval covers it (the mock would have to model instance-override inheritance), and the plugin's own success copy still says "Instances keep their overrides" without the qualifier.
7. **Amending a HAND-BUILT component set has never been done.** Every in-place amend on record — including the CBDS foreign-kit finale — targeted a set this tool created (`extract/pilots/cbds/README.md`: the identity gate found `Badge (ds.badge)` while the kit's native 72-variant `Badge` "stayed invisible to it"). That is the correct behaviour and it is genuine *coexistence* evidence; it is not evidence that the tool can adopt and amend a set drawn by hand, which is the brownfield case an adopter actually asks about.
...
npm run eval      # 228 deterministic evals → evals/results.json
