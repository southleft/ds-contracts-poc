...
This PoC makes eight falsifiable claims. Each is backed by an automated eval (`npm run eval`, 230 cases, runs the real pipeline in a scratch copy — not mocks) or an executed live design-tool check. Current status: **226 of 230 deterministic evals pass** (`evals/results.json`); the four reds are named with their measurements in [docs/23 §B.38](23-known-limitations.md) — `capability-report-is-fresh` plus the three the census/stack-to-main merge measured and refused to re-record green (`astryx-reanchor-minted`, `minted-leaves-bind-to-something`, `console-loop-canvas-drift-probe`). All live checks pass. This section is written to be lifted into a PRD.
...
npm run eval      # 230 deterministic evals → evals/results.json
