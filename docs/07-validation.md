...
This PoC makes five falsifiable claims. Each is backed by an automated eval (`npm run eval`, 37 cases, runs the real pipeline in a scratch copy — not mocks) or an executed live design-tool check. Current status: **49/49 deterministic evals pass** (`evals/results.json`), all live checks pass. This section is written to be lifted into a PRD.
