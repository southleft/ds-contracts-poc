...
function updateProblem(problem: string) {
  ...
  if (name === 'native-update-late-result-after-revocation') return 'A result arrived from a write you attested dead, so that companion was alive. The result is kept as evidence. It never counts as the write’s outcome: before the canvas read, the read decides; after it, a result that disagrees with the read stops the update for recovery. Close every companion window, then inspect the update again.';
