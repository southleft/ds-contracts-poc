...
const read = (p: string) =>
  ...
  >;
const dump = read(
  "extract/figma/gauntlet/live/fixtures/fill-matrix-depth-drop-badge-chip-alert.dump.json",
);
...
const batch = proposeBatchFromDump(dump, {
  ...
  fileKey:
    (dump._provenance as { fileKey?: string } | undefined)?.fileKey ?? null,
  mintUnbound: true,
...
const capturedValueByPath = new Map(
  capturedEntries.map((e) => [e.path, e.value]),
);
const baseInventory = new Set<string>([
  ...repoInventory,
  ...capturedEntries.map((e) => e.path),
]);
...
const EXPECT: Record<string, { placeholders: number; compound: RegExp }> = {
  Badge: {
    placeholders: 2,
    compound:
      /^\{imported\.badge\.root\.background-color\.\{type\}\.\{style\}\}$/,
  },
  Chip: {
    placeholders: 3,
    compound:
      /^\{imported\.chip\.root\.background-color\.\{type\}\.\{style\}\.\{state\}\}$/,
  },
  Alert: {
    placeholders: 2,
    compound:
      /^\{imported\.alert\.root\.background-color\.\{type\}\.\{style\}\}$/,
  },
};
...
for (const [setName, expect] of Object.entries(EXPECT)) {
  ...
  const rootTokens = (contract.anatomy.root.tokens ?? {}) as Record<
    string,
    string
  >;
  ...
  if (!bg)
    fail(
      `${setName}: root background-color DROPPED — the class-① silent paint loss is back`,
    );
  if (!expect.compound.test(bg)) {
    fail(
      `${setName}: root background-color is ${JSON.stringify(bg)} — expected the ${expect.placeholders}-placeholder minted ref (${expect.compound})`,
    );
  }
  // The drift stays NAMED and routed — never silent.
  const routed = p!.notes.find(
    (n) =>
      n.includes(`${setName}:root fill: token paths differ in depth`) &&
      n.includes("routed to the mint pass"),
  );
  if (!routed)
    fail(
      `${setName}: the depth-drift note (with mint routing) is missing — the refusal must stay named`,
    );
  // Minted leaves carry the CAPTURED resolved literals.
  for (const entry of p!.mintedTokens?.entries ?? []) {
    if (
      !entry.ref.startsWith(
        `{imported.${setName.toLowerCase()}.root.background-color.`,
      )
    )
      continue;
    ...
    const axisValues = Object.fromEntries(
      m[1].split(", ").map((kv) => kv.split("=") as [string, string]),
    );
    const variant = (
      dump[setName] as {
        variants: Array<{ name: string; fill?: { var?: string } }>;
      }
    ).variants.find((v) =>
      Object.entries(axisValues).every(([k, val]) =>
        v.name.includes(`${k}=${val}`),
      ),
    );
    const boundPath = variant?.fill?.var?.split("/").join(".");
    if (boundPath && capturedValueByPath.get(boundPath) !== entry.value) {
      fail(
        `${setName}: minted ${entry.ref} = ${entry.value} but the captured value of ${boundPath} is ${capturedValueByPath.get(boundPath)}`,
      );
    }
  }
  ...
  if (errors.length > 0)
    fail(
      `${setName}: referee violations after the fix: ${errors.slice(0, 3).join(" | ")}`,
    );
  const compoundRules = (css.match(/^(?:\.[a-z-]+-[a-z0-9-]+){2,3} \{/gm) ?? [])
    .length;
  if (compoundRules === 0)
    fail(
      `${setName}: generateCss emitted no compound enum-class rules for the minted matrix`,
    );
  const tokens: TokenTreeInput = {
    ...repoTrees,
    semantic: mergeTrees([
      repoTrees.semantic as Record<string, unknown>,
      (captured?.tree ?? {}) as Record<string, unknown>,
      mintedTree,
    ]),
    brands,
  } as TokenTreeInput;
  ...
  for (const emitter of emitters) {
    try {
      ...
      fail(
        `${setName}: emitter ${emitter.name} refused: ${(e as Error).message.split("\n")[0]}`,
      );
    }
  }
  const html = emitHtml(contract, ctx);
  const htmlMinted = (
    html.css.match(/background-color: var\(--imported-/g) ?? []
  ).length;
  if (htmlMinted === 0)
    fail(
      `${setName}: emit-html carries no minted background rules — the parity render would still drop the box`,
    );
  console.log(
    `  ${setName}: ${bg} — referee-clean, 4/4 emitters, ${htmlMinted} emit-html background rules`,
  );
}

console.log(
  "fill-matrix-mint ok: Badge/Alert pair-minted, Chip triple-minted, drift named, never a silent paint drop",
);
