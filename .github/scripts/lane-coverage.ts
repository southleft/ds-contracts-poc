...
const EXCLUDED: Record<string, string> = {
  ...
  // that is invisible, because the regenerated bytes are identical. For these
  // three it is not: their receipts carry `liveFigma: true` from an actual mint,
  // the builder hardcodes `liveFigma: false`, and running the "check" silently
  // DOWNGRADES a recorded live-mint fact while exiting 0.
  //
...
const RETIRED_LIVE_LINEAGES: Array<{
...
}> = [
  {
    ...
    retired: (v) => v <= 37,
    current: "recipe:table:live:v38:check",
  },
  {
    archetype: "combobox",
