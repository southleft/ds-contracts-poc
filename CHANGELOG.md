...
- **Mock fidelity: the two blind spots that hid the composite failures are modeled** — `plugin-engine-mock-figma.mjs` now computes auto-layout sizing (FILL children contribute zero intrinsic size, so hug↔fill collapse is measurable headlessly), follows the real component-property contract (set-level definitions, variant-child refusals, instance subtree deep-clones with TEXT/BOOLEAN reflection onto bound nodes, unknown-key throws), and simulates the exposure lag via a harness flag; `plugin-engine-check` builds the composite THROUGH the simulated lag and pins the built dialog width, the repeated badges' rendered item text, the footer Cancel/Save labels + gap, and backdrop-behind-dialog stacking.
- **Composite exhibit v1.1.0** — footer actions are real `ds.button` instances (secondary/primary) with a `{space.gap.control}` gap; the dialog declares a 480px width + surface/border/radius/padding/gap tokens; the backdrop is a parent-bound inset-0 scrim painted BEHIND the dialog (anatomy order = paint order).
...
- **Plugin distribution hardening** — `npm run plugin:zip` also refreshes an unpacked `figma-sync/plugin-dist/` dev-import folder in place (a one-time manifest import stays current; the repo `figma-sync/plugin/ui.html` is a stub and hand-unzipped copies go stale), and injects a content-derived engine build stamp (`engine <hash12> · <bytes>B`) shown in the plugin header — a stale engine is now diagnosable at a glance.
- **Determinism proof strengthened ~42×** — `deterministic-roundtrip.mjs` fingerprints the FULL tracked property surface (layout, sizing, paints, text, bindings, markers; run-scoped ids normalized to stable names) instead of names/types/nesting only, and runs under `npx tsx` (the documented `node` invocation never worked from a clean clone).
