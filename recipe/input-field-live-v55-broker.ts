...
import {
  assertInputLiveV55RootProofs,
  INPUT_LIVE_V55_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V55_RESPONSE_CONTRACTS,
  type InputLiveV55RootProof,
} from "./input-field-live-v55-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V55_BROKER_VERSION = "input-live-v55-broker-v2";
export const INPUT_LIVE_V55_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V55_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V55_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V55_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V55_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V55_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV55RemotePhase =
  (typeof INPUT_LIVE_V55_REMOTE_PHASES)[number];
export type InputLiveV55AuthorizationMode = "live" | "simulated";

export interface InputLiveV55CommitPins {
...
export interface InputLiveV55TransactionAuthorization extends InputLiveV55CommitPins {
  mode: InputLiveV55AuthorizationMode;
...
export interface InputLiveV55Manifest {
  artifactVersion: "input-live-v55-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V55_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V55_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V55_TARGET;
  authorization: InputLiveV55TransactionAuthorization;
...
export interface InputLiveV55ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV55RequestBody {
  artifactVersion: "input-live-v55-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V55_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV55RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV55CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V55_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV55ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV55Request extends InputLiveV55RequestBody {
...
export interface InputLiveV55Receipt {
  artifactVersion: "input-live-v55-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V55_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV55RemotePhase;
...
export interface InputLiveV55AcceptedResponse<T = unknown> {
  request: InputLiveV55Request;
  receipt: InputLiveV55Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV55IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV55ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V55_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V55_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V55_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V55_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V55_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V55_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V55_TARGET.editorType;
};
...
export const inputLiveV55RequestSequence = (
  phase: InputLiveV55RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV55RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV55RequestPath = (
  directory: string,
  phase: InputLiveV55RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV55ResponsePath = (
  directory: string,
  phase: InputLiveV55RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV55ReceiptPath = (
  directory: string,
  phase: InputLiveV55RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV55Manifest => {
  const manifest = json<InputLiveV55Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v55-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V55_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V55_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV55RemotePhase,
  captureIndex?: number,
): InputLiveV55Request | undefined => {
  const file = inputLiveV55RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV55Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV55RemotePhase,
  captureIndex?: number,
): InputLiveV55Receipt | undefined => {
  const file = inputLiveV55ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV55Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV55Request | undefined;
  receipt: InputLiveV55Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV55RemotePhase,
): InputLiveV55ResponseContract => ({
  ...INPUT_LIVE_V55_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V55_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v55-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V55_BROKER_VERSION,
  target: INPUT_LIVE_V55_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V55_PROGRAM_START*/${program}/*INPUT_LIVE_V55_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V55_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V55_PROGRAM_END*/";
...
export function createInputLiveV55Transaction(
  directory: string,
  ...
  const manifest: InputLiveV55Manifest = {
    artifactVersion: "input-live-v55-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V55_BROKER_VERSION,
...
export function validateInputLiveV55Request(
  directory: string,
  request: InputLiveV55Request,
): void {
  ...
  const expectedSequence = inputLiveV55RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v55-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V55_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V55_TARGET.fileKey ||
...
export function issueInputLiveV55Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV55RemotePhase,
  program: string,
  options: InputLiveV55IssueOptions = {},
): InputLiveV55Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV55RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV55Request>(targetPath);
    validateInputLiveV55Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV55ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV55TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV55RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V55_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V55_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V55_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV55RequestBody = {
    artifactVersion: "input-live-v55-broker-request-v2",
    brokerVersion: INPUT_LIVE_V55_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV55Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV55Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV55RawResponse(
  directory: string,
  phase: InputLiveV55RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV55RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV55Request>(requestFile);
  if (
  ...
  const target = inputLiveV55ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV55Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v55-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V55_BROKER_VERSION,
...
export function acceptInputLiveV55Response<T = unknown>(
  directory: string,
  phase: InputLiveV55RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV55AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV55ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV55Request>(
    inputLiveV55RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV55Request(directory, request);
  if (phase === "capture") validateInputLiveV55TechnicalGates(directory);
  const rawFile = inputLiveV55ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v55-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V55_BROKER_VERSION ||
...
export function persistInputLiveV55TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV55RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV55RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V55_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v55-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V55_BROKER_VERSION,
...
export function validateInputLiveV55TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v55-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V55_TECHNICAL_GATES.some(
...
export function validateInputLiveV55Receipt(
  directory: string,
  phase: InputLiveV55RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV55Request(directory, request);
  const receipt = json<InputLiveV55Receipt>(
    inputLiveV55ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV55ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v55-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V55_BROKER_VERSION ||
...
export function readInputLiveV55AcceptedRawPayload(
  directory: string,
  phase: InputLiveV55RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV55Receipt(directory, phase, captureIndex);
...
export function createInputLiveV55FakeDynamicResponse(
  request: InputLiveV55Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V55_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V55_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV55PersistedRequests(
  directory: string,
): InputLiveV55Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV55Request>(file)] : [];
    });
}

export const inputLiveV55Sha256 = sha256;
