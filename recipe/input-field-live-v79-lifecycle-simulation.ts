...
export interface InputLiveV79LifecycleSimulationReport {
  artifactVersion: "input-live-v79-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV79AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV79Lifecycle(): Promise<InputLiveV79LifecycleSimulationReport> {
  await buildInputLiveV79Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V79_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV79AntecedentIndex;
  const indexSha256 = inputLiveV79Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v79-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V79_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V79_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV79History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV79History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV79AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V79_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V79_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV79History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV79History(authorized, "pending");
    ...
    await buildInputLiveV79Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V79_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV79AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV79Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV79Lifecycle(), null, 2)}\n`,
  );
