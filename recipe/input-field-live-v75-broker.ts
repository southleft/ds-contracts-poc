...
import {
  assertInputLiveV75RootProofs,
  INPUT_LIVE_V75_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V75_RESPONSE_CONTRACTS,
  type InputLiveV75RootProof,
} from "./input-field-live-v75-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V75_BROKER_VERSION = "input-live-v75-broker-v2";
export const INPUT_LIVE_V75_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V75_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V75_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V75_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V75_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V75_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV75RemotePhase =
  (typeof INPUT_LIVE_V75_REMOTE_PHASES)[number];
export type InputLiveV75AuthorizationMode = "live" | "simulated";

export interface InputLiveV75CommitPins {
...
export interface InputLiveV75TransactionAuthorization extends InputLiveV75CommitPins {
  mode: InputLiveV75AuthorizationMode;
...
export interface InputLiveV75Manifest {
  artifactVersion: "input-live-v75-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V75_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V75_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V75_TARGET;
  authorization: InputLiveV75TransactionAuthorization;
...
export interface InputLiveV75ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV75RequestBody {
  artifactVersion: "input-live-v75-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V75_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV75RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV75CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V75_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV75ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV75Request extends InputLiveV75RequestBody {
...
export interface InputLiveV75Receipt {
  artifactVersion: "input-live-v75-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V75_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV75RemotePhase;
...
export interface InputLiveV75AcceptedResponse<T = unknown> {
  request: InputLiveV75Request;
  receipt: InputLiveV75Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV75IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV75ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V75_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V75_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V75_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V75_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V75_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V75_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V75_TARGET.editorType;
};
...
export const inputLiveV75RequestSequence = (
  phase: InputLiveV75RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV75RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV75RequestPath = (
  directory: string,
  phase: InputLiveV75RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV75ResponsePath = (
  directory: string,
  phase: InputLiveV75RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV75ReceiptPath = (
  directory: string,
  phase: InputLiveV75RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV75Manifest => {
  const manifest = json<InputLiveV75Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v75-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V75_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V75_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV75RemotePhase,
  captureIndex?: number,
): InputLiveV75Request | undefined => {
  const file = inputLiveV75RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV75Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV75RemotePhase,
  captureIndex?: number,
): InputLiveV75Receipt | undefined => {
  const file = inputLiveV75ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV75Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV75Request | undefined;
  receipt: InputLiveV75Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV75RemotePhase,
): InputLiveV75ResponseContract => ({
  ...INPUT_LIVE_V75_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V75_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V75_DYNAMIC_TOOL,
      target: INPUT_LIVE_V75_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v75-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V75_BROKER_VERSION,
  target: INPUT_LIVE_V75_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V75_PROGRAM_START*/${program}/*INPUT_LIVE_V75_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V75_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V75_PROGRAM_END*/";
...
export function createInputLiveV75Transaction(
  directory: string,
  ...
  const manifest: InputLiveV75Manifest = {
    artifactVersion: "input-live-v75-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V75_BROKER_VERSION,
...
export function validateInputLiveV75Request(
  directory: string,
  request: InputLiveV75Request,
): void {
  ...
  const expectedSequence = inputLiveV75RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v75-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V75_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V75_TARGET.fileKey ||
...
export function issueInputLiveV75Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV75RemotePhase,
  program: string,
  options: InputLiveV75IssueOptions = {},
): InputLiveV75Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV75RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV75Request>(targetPath);
    validateInputLiveV75Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV75ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV75TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV75RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V75_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V75_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V75_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV75RequestBody = {
    artifactVersion: "input-live-v75-broker-request-v2",
    brokerVersion: INPUT_LIVE_V75_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV75Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV75Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV75RawResponse(
  directory: string,
  phase: InputLiveV75RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV75RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV75Request>(requestFile);
  if (
  ...
  const target = inputLiveV75ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV75Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v75-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V75_BROKER_VERSION,
...
export function acceptInputLiveV75Response<T = unknown>(
  directory: string,
  phase: InputLiveV75RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV75AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV75ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV75Request>(
    inputLiveV75RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV75Request(directory, request);
  if (phase === "capture") validateInputLiveV75TechnicalGates(directory);
  const rawFile = inputLiveV75ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v75-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V75_BROKER_VERSION ||
...
export function persistInputLiveV75TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV75RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV75RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V75_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v75-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V75_BROKER_VERSION,
...
export function validateInputLiveV75TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v75-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V75_TECHNICAL_GATES.some(
...
export function validateInputLiveV75Receipt(
  directory: string,
  phase: InputLiveV75RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV75Request(directory, request);
  const receipt = json<InputLiveV75Receipt>(
    inputLiveV75ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV75ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v75-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V75_BROKER_VERSION ||
...
export function readInputLiveV75AcceptedRawPayload(
  directory: string,
  phase: InputLiveV75RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV75Receipt(directory, phase, captureIndex);
...
export function createInputLiveV75FakeDynamicResponse(
  request: InputLiveV75Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V75_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V75_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV75PersistedRequests(
  directory: string,
): InputLiveV75Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV75Request>(file)] : [];
    });
}

export const inputLiveV75Sha256 = sha256;
