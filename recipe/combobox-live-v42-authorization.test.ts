...
import { COMBOBOX_LIVE_V42_INDEX_PATH } from "./build-combobox-live-proof-v42.js";
import {
  COMBOBOX_LIVE_V42_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V42_TARGET,
} from "./combobox-live-v42-broker.js";
import {
  buildComboboxLiveV42AuthorizationArtifact,
  validateComboboxLiveV42AntecedentIndex,
  validateComboboxLiveV42AuthorizationArtifact,
  validateComboboxLiveV42History,
  validateComboboxLiveV42UnitTestSource,
  type ComboboxLiveV42AntecedentIndex,
  type ComboboxLiveV42AuthorizationArtifact,
  type ComboboxLiveV42AuthorizationProof,
  type ComboboxLiveV42HistoryState,
} from "./combobox-live-v42-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV42ControlFlowSource,
  validateComboboxLiveV42SecurityAttestation,
  type ComboboxLiveV42SecurityAttestation,
} from "./combobox-live-v42-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v42-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V42_INDEX_PATH);
const index = (): ComboboxLiveV42AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV42AntecedentIndex;

const artifact = (): ComboboxLiveV42AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV42AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV42HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV42HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV42AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v42/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV42AuthorizationArtifact,
): ComboboxLiveV42SecurityAttestation => {
  const body: Omit<ComboboxLiveV42SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V42_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV42History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV42History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV42History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV42History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV42History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV42UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV42UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV42AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV42AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV42AuthorizationArtifact(
        planted as ComboboxLiveV42AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV42SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV42SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV42SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV42ControlFlowSource(source), []);
});
