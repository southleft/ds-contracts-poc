...
import {
  verifyComboboxLiveV36FixedSceneFixedPoint,
  type ComboboxLiveV36FixedFixedPoint,
} from "./combobox-live-v36-fixed-point.js";

export const COMBOBOX_LIVE_V36_VERIFIER_VERSION = "combobox-live-v36-verifier-v1";

export function buildComboboxLiveV36RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV36RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV36RawNode[];
}

export interface ComboboxLiveV36NormalizedScene {
...
export interface ComboboxLiveV36SceneProof {
...
export function normalizeComboboxLiveV36Scene(
  root: ComboboxLiveV36RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV36NormalizedScene {
  const canonicalBindings: ComboboxLiveV36NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV36RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV36Scene<Instance>(
  normalized: ComboboxLiveV36NormalizedScene,
...
): ComboboxLiveV36SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV36FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV36ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV36PreCaptureGates(payload: unknown): void {
