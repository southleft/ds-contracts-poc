...
import {
  validateCalendarLiveV3AntecedentIndex,
  verifyCalendarLiveV3Authorization,
  type CalendarLiveV3AntecedentIndex,
  type CalendarLiveV3AuthorizationProof,
} from "./calendar-live-v3-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V3_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v3/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V3_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v3-security-attestation.json";

export interface CalendarLiveV3SecurityAttestation {
  artifactVersion: "calendar-live-v3-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV3PreflightReport {
  artifactVersion: "calendar-live-v3-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV3SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV3SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV3AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v3-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V3_TARGET) ||
...
export function validateCalendarLiveV3ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV3Authorization");
  const preflight = source.indexOf("runCalendarLiveV3Preflight");
  const initialize = source.indexOf("CalendarLiveV3Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV3Preflight(
  root: string,
  proof: CalendarLiveV3AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V3_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV3PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V3_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V3_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV3AntecedentIndex;
  failures.push(...validateCalendarLiveV3AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V3_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V3_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV3SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v3-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV3ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v3 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV3SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v3-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V3_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV3Preflight(
    process.cwd(),
    verifyCalendarLiveV3Authorization(),
    attestationPath,
