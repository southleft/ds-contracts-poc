...
export type CalendarLiveV13HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV13AuthorizationArtifact {
  artifactVersion: "calendar-live-v13-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V13_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV13AntecedentIndex {
...
export interface CalendarLiveV13HistoryState {
  index?: CalendarLiveV13AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV13AuthorizationArtifact;
...
export interface CalendarLiveV13AuthorizationProof
  extends CalendarLiveV13TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V13_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V13_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V13_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V13_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v13-authorization") ||
  artifactPath.includes("calendar-live-v13-preflight") ||
  artifactPath.includes("calendar-live-v13-authorized") ||
  artifactPath.includes("create-calendar-live-v13-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v13-status") ||
...
export function validateCalendarLiveV13AntecedentIndex(
  index: CalendarLiveV13AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v13-antecedent-index-v1" ||
...
export function validateCalendarLiveV13UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV13History\(\s*["']/.test(source) ||
    /readCalendarLiveV13HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV13AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV13AuthorizationArtifact {
  ...
  ) as CalendarLiveV13AntecedentIndex;
  const indexFailures = validateCalendarLiveV13AntecedentIndex(index);
...
export function validateCalendarLiveV13AuthorizationArtifact(
  artifact: CalendarLiveV13AuthorizationArtifact | undefined,
  index: CalendarLiveV13AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v13-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v13" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V13_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V13_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V13_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V13_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V13_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V13_SOURCE_ROOTS ||
...
export function validateCalendarLiveV13History(
  state: CalendarLiveV13HistoryState,
  expected: CalendarLiveV13HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV13AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV13AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV13HistoryState(
  root = process.cwd(),
): CalendarLiveV13HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V13_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV13AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V13_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V13_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V13_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV13History(
  expected: CalendarLiveV13HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV13HistoryState {
  const state = readCalendarLiveV13HistoryState(root);
  const failures = validateCalendarLiveV13History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v13 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV13Authorization(
  root = process.cwd(),
): CalendarLiveV13AuthorizationProof {
  const state = verifyCalendarLiveV13History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V13_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v13/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v13.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V13_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V13_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V13_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV13HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV13History(expected);
  process.stdout.write(
    `Calendar live v13 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
