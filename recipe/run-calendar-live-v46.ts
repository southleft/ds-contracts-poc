...
import {
  acceptCalendarLiveV46Response,
  createCalendarLiveV46Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV46Request,
  persistCalendarLiveV46RawResponse,
  persistCalendarLiveV46TechnicalGates,
  readCalendarLiveV46AcceptedRawPayload,
  type CalendarLiveV46Request,
  type CalendarLiveV46TransactionAuthorization,
} from "./calendar-live-v46-broker.js";
import {
  CALENDAR_LIVE_V46_CAPTURE_COUNT,
  assertCalendarLiveV46CaptureResponses,
  ...
  evaluateCalendarLiveV46Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV46Roots,
  validateCalendarLiveV46CaptureManifest,
  validateCalendarLiveV46CapturePayload,
  validateCalendarLiveV46CleanupPayload,
  validateCalendarLiveV46ExtractPayload,
  validateCalendarLiveV46ProbePayload,
  validateCalendarLiveV46WriterPayload,
  type CalendarLiveV46CaptureCell,
  type CalendarLiveV46CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V46_PLAN_PATH,
  type CalendarLiveV46ProofPlan,
} from "./build-calendar-live-proof-v46.js";
import {
...
export const CALENDAR_LIVE_V46_RUNNER_VERSION = "calendar-live-v46-runner-v1";

interface CalendarLiveV46JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV46OrchestratorState {
  artifactVersion: "calendar-live-v46-orchestrator-state-v1";
  ...
  journal: CalendarLiveV46JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV46ProofPlan;
  planSha256: string;
  captures: CalendarLiveV46CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV46SourceIdentity & {
...
export interface CalendarLiveV46NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV46OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v46 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V46_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V46_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV46ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV46CaptureCell[];
  validateCalendarLiveV46CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v46 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v46 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV46WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV46RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV46Request =>
  readJson<CalendarLiveV46Request>(file);

export class CalendarLiveV46Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV46Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV46OrchestratorState = {
      artifactVersion: "calendar-live-v46-orchestrator-state-v1",
    ...
    return new CalendarLiveV46Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV46OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV46Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV46NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV46NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v46 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV46NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v46 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV46RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV46Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV46CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV46AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV46WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV46Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV46ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV46Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV46Response(this.#directory, "writer", {
        validate: validateCalendarLiveV46WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV46Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV46RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV46Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV46ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV46Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV46RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v46-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV46Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV46ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV46Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV46ProbePayload(payload, ownership),
      });
      persistCalendarLiveV46TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v46 capture cell missing");
      const accepted = acceptCalendarLiveV46Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV46CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V46_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV46CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV46Objective(
    ...
    throw new Error(`Calendar live v46 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV46Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV46CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV46Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV46RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV46Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV46CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV46CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV46CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV46Authorization = (
  privateKey: KeyObject,
): CalendarLiveV46TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV46SimulationKey = (): KeyObject =>
