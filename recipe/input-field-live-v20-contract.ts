...
} from "./input-field-live-v3-verifier-v20.js";
import {
  buildInputLiveV20RawPropertyRuntime,
  normalizeInputLiveV20Scene,
  type InputLiveV20RawNode,
} from "./input-field-live-v20-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v20.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v20.js";
...
export const INPUT_LIVE_V20_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V20_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V20_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V20_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V20_VARIANT_COUNT = 256;
export const INPUT_LIVE_V20_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V20_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV20SourceId = (typeof INPUT_LIVE_V20_SOURCE_IDS)[number];

export interface InputLiveV20SourceIdentity {
  source: InputLiveV20SourceId;
...
export interface InputLiveV20WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV20ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV20SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV20RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV20RootProof {
  source: InputLiveV20SourceId;
  adapterIdentity: string;
...
export interface InputLiveV20ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV20SourceId;
    adapterIdentity: string;
...
export interface InputLiveV20CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV20SourceId;
  adapterIdentity: string;
...
export interface InputLiveV20CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV20SourceId;
  strata: InputLiveV20CaptureCell["strata"];
...
export interface InputLiveV20CleanupPayload {
...
export function validateInputLiveV20WriterPayload(
  value: unknown,
): InputLiveV20WriterOwnership {
...
const identityPlan = (source: InputLiveV20SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV20ExtractProgram(
  writer: InputLiveV20WriterOwnership,
  sources: readonly InputLiveV20SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V20_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V20_NAMESPACE)}
${buildInputLiveV20RawPropertyRuntime()}
...
export function validateInputLiveV20ExtractPayload(
  value: unknown,
  writer: InputLiveV20WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V20_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV20ExtractPayload;
}

export function proveInputLiveV20Roots<Instance>(
  extract: InputLiveV20ExtractPayload,
  sources: readonly (InputLiveV20SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV20Scene(
      root.scene,
...
export function assertInputLiveV20RootProofs(
  proofs: readonly InputLiveV20RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV20ProbeProgram(
  writer: InputLiveV20WriterOwnership,
  sources: readonly InputLiveV20SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V20_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV20ProbePayload(
  value: unknown,
  writer: InputLiveV20WriterOwnership,
): InputLiveV20ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV20ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V20_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V20_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV20CaptureManifest(
  cells: readonly InputLiveV20CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V20_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V20_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V20_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V20_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V20_SOURCE_IDS.some(
...
export const inputLiveV20CaptureManifestSha256 = (
  cells: readonly InputLiveV20CaptureCell[],
): string => {
  validateInputLiveV20CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV20CaptureProgram(
  writer: InputLiveV20WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V20_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V20_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V20_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV20CapturePayload(
  value: unknown,
  cell: InputLiveV20CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV20CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V20_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V20_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV20CaptureResponses(
  manifest: readonly InputLiveV20CaptureCell[],
  responses: readonly InputLiveV20CapturePayload[],
): void {
  validateInputLiveV20CaptureManifest(manifest);
  if (
...
export interface InputLiveV20ObjectiveReport {
  artifactVersion: "input-live-v20-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV20Objective(
  manifest: readonly InputLiveV20CaptureCell[],
  responses: readonly InputLiveV20CapturePayload[],
  readReference: (cell: InputLiveV20CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV20CaptureCell) => Buffer,
): InputLiveV20ObjectiveReport {
  assertInputLiveV20CaptureResponses(manifest, responses);
...
export function buildInputLiveV20CleanupProgram(
  writer: InputLiveV20WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V20_NAMESPACE,
...
export function validateInputLiveV20CleanupPayload(
  value: unknown,
  writer: InputLiveV20WriterOwnership,
): InputLiveV20CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV20CleanupPayload;
...
export const INPUT_LIVE_V20_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV20WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
