...
import { buildComboboxLiveV1CleanupRuntime } from "./combobox-live-v1-cleanup.js";
import {
  type ComboboxLiveV1FixedFixedPoint,
} from "./combobox-live-v1-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV1RestoreProgram,
  validateComboboxLiveV1RestorePayload,
  type ComboboxLiveV1RestorePayload,
} from "./combobox-live-v1-restore.js";
import {
  buildComboboxLiveV1RawPropertyRuntime,
  normalizeComboboxLiveV1Scene,
  type ComboboxLiveV1RawNode,
} from "./combobox-live-v1-verifier.js";
...
export const COMBOBOX_LIVE_V1_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V1_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V1_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V1_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V1_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V1_SET_COUNT = 4;
export const COMBOBOX_LIVE_V1_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V1_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V1_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V1_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V1_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV1SourceId = (typeof COMBOBOX_LIVE_V1_SOURCE_IDS)[number];

export interface ComboboxLiveV1SourceIdentity {
  source: ComboboxLiveV1SourceId;
...
export interface ComboboxLiveV1WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV1ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV1SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV1RawNode;
    optionScene: ComboboxLiveV1RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV1RootProof {
  source: ComboboxLiveV1SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV1FixedFixedPoint;
}

export interface ComboboxLiveV1ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV1SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV1CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV1SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV1CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV1SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV1CleanupPayload {
...
export function validateComboboxLiveV1WriterPayload(
  value: unknown,
): ComboboxLiveV1WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V1_NAMESPACE ||
...
const identityPlan = (
  source: ComboboxLiveV1SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV1ExtractProgram(
  writer: ComboboxLiveV1WriterOwnership,
  sources: readonly ComboboxLiveV1SourceIdentity[],
): string {
  if (sources.length !== 2)
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V1_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V1_NAMESPACE)}
${buildComboboxLiveV1RawPropertyRuntime()}
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("COMBOBOX-V1-EXTRACT-PAGE");
...
if(get(page,"pageOwner")!=="recipe/combobox/"+${JSON.stringify(writer.runIdentity)})throw new Error("COMBOBOX-V1-EXTRACT-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>SET_IDS.has(node.id));
if(sets.length!==4)throw new Error("COMBOBOX-V1-EXTRACT-ROOTS:"+sets.length);
...
for(const set of sets){
  ...
  if(!IDENTITIES[adapterIdentity]||(kind!=="combobox"&&kind!=="option"))throw new Error("COMBOBOX-V1-EXTRACT-ADAPTER:"+adapterIdentity+":"+kind);
...
if(roots.length!==2||roots.some(root=>!root.comboboxScene||!root.optionScene))throw new Error("COMBOBOX-V1-EXTRACT-PAIR");
...
export function validateComboboxLiveV1ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV1WriterOwnership,
): ComboboxLiveV1ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V1_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as ComboboxLiveV1ExtractPayload;
}

export function proveComboboxLiveV1Roots<Instance>(
  extract: ComboboxLiveV1ExtractPayload,
  sources: readonly (ComboboxLiveV1SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const comboboxNormalized = normalizeComboboxLiveV1Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV1Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV1FixedFixedPoint => {
      const cycle = () => {
...
export function assertComboboxLiveV1RootProofs(
  proofs: readonly ComboboxLiveV1RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildComboboxLiveV1ProbeProgram(
  writer: ComboboxLiveV1WriterOwnership,
  sources: readonly ComboboxLiveV1SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V1_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE"||page.id==="115:295378")throw new Error("COMBOBOX-V1-PROBE-PAGE");
...
if(sets.length!==4)throw new Error("COMBOBOX-V1-PROBE-ROOTS:"+sets.length);
...
for(const row of byAdapter.values()){
 ...
 if(!source||!combobox||!option||combobox.children.length!==64||option.children.length!==8)throw new Error("COMBOBOX-V1-PROBE-VARIANTS:"+adapterIdentity);
 ...
 for(const component of combobox.children){const target=axes(component),updates={};for(const name of axisNames){const key=propertyKey(instance,name);if(!key)throw new Error("COMBOBOX-V1-PROBE-AXIS:"+name);updates[key]=target[name];}instance.setProperties(updates);const main=await instance.getMainComponentAsync();if(main)visited.add(main.id);}
 ...
 for(const component of option.children){const target=axes(component),updates={};for(const name of ["Size","Option state"]){const key=propertyKey(optionInstance,name);if(!key)throw new Error("COMBOBOX-V1-PROBE-OPTION-AXIS:"+name);updates[key]=target[name];}optionInstance.setProperties(updates);const main=await optionInstance.getMainComponentAsync();if(main)optionVisited.add(main.id);}
...
export function validateComboboxLiveV1ProbePayload(
  value: unknown,
  writer: ComboboxLiveV1WriterOwnership,
): ComboboxLiveV1ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV1ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V1_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V1_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateComboboxLiveV1CaptureManifest(
  cells: readonly ComboboxLiveV1CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V1_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V1_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V1_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V1_SOURCE_IDS.includes(cell.source) ||
...
export const comboboxLiveV1CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV1CaptureCell[],
): string => {
  validateComboboxLiveV1CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV1CaptureProgram(
  writer: ComboboxLiveV1WriterOwnership,
  cell: ComboboxLiveV1CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V1_CAPTURE_COUNT
  )
...
if(!page||page.type!=="PAGE"||page.id==="115:295378"||!set||set.type!=="COMPONENT_SET")throw new Error("COMBOBOX-V1-CAPTURE-TARGET");
const axes=node=>Object.fromEntries(node.name.split(",").map(part=>part.trim()).filter(part=>part.includes("=")).map(part=>{const at=part.indexOf("=");return[part.slice(0,at),part.slice(at+1)];}));
...
if(!component)throw new Error("COMBOBOX-V1-CAPTURE-CELL:"+cell.cellKey);
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V1_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V1-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV1CapturePayload(
  value: unknown,
  cell: ComboboxLiveV1CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV1CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV1CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V1_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V1_CAPTURE_MAX_PNG_BYTES ||
...
export function assertComboboxLiveV1CaptureResponses(
  manifest: readonly ComboboxLiveV1CaptureCell[],
  responses: readonly ComboboxLiveV1CapturePayload[],
): void {
  validateComboboxLiveV1CaptureManifest(manifest);
  if (
...
export interface ComboboxLiveV1ObjectiveReport {
  artifactVersion: "combobox-live-v1-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV1Objective(
  manifest: readonly ComboboxLiveV1CaptureCell[],
  responses: readonly ComboboxLiveV1CapturePayload[],
): ComboboxLiveV1ObjectiveReport {
  assertComboboxLiveV1CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v1-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV1CleanupProgram(
  writer: ComboboxLiveV1WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV1CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V1_NAMESPACE,
  ...
  const guardedResult = `if(JSON.stringify(requestedNodeIds)!==JSON.stringify([expectedPageId])||JSON.stringify([...requestedCollectionIds].sort())!==JSON.stringify([...expectedCollectionIds].sort()))throw new Error("COMBOBOX-V1-CLEANUP-EXACT-OWNERSHIP");${resultStatement}`;
...
export function validateComboboxLiveV1CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV1WriterOwnership,
): ComboboxLiveV1CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV1CleanupPayload;
  if (
    value.complete !== true ||
...
export const COMBOBOX_LIVE_V1_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV1WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
