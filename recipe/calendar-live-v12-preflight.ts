...
import {
  validateCalendarLiveV12AntecedentIndex,
  verifyCalendarLiveV12Authorization,
  type CalendarLiveV12AntecedentIndex,
  type CalendarLiveV12AuthorizationProof,
} from "./calendar-live-v12-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V12_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v12/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V12_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v12-security-attestation.json";

export interface CalendarLiveV12SecurityAttestation {
  artifactVersion: "calendar-live-v12-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV12PreflightReport {
  artifactVersion: "calendar-live-v12-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV12SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV12SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV12AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v12-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V12_TARGET) ||
...
export function validateCalendarLiveV12ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV12Authorization");
  const preflight = source.indexOf("runCalendarLiveV12Preflight");
  const initialize = source.indexOf("CalendarLiveV12Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV12Preflight(
  root: string,
  proof: CalendarLiveV12AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V12_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV12PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V12_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V12_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV12AntecedentIndex;
  failures.push(...validateCalendarLiveV12AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V12_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V12_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV12SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v12-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV12ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v12 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV12SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v12-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V12_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV12Preflight(
    process.cwd(),
    verifyCalendarLiveV12Authorization(),
    attestationPath,
