...
import {
  assertInputLiveV66RootProofs,
  INPUT_LIVE_V66_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V66_RESPONSE_CONTRACTS,
  type InputLiveV66RootProof,
} from "./input-field-live-v66-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V66_BROKER_VERSION = "input-live-v66-broker-v2";
export const INPUT_LIVE_V66_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V66_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V66_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V66_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V66_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V66_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV66RemotePhase =
  (typeof INPUT_LIVE_V66_REMOTE_PHASES)[number];
export type InputLiveV66AuthorizationMode = "live" | "simulated";

export interface InputLiveV66CommitPins {
...
export interface InputLiveV66TransactionAuthorization extends InputLiveV66CommitPins {
  mode: InputLiveV66AuthorizationMode;
...
export interface InputLiveV66Manifest {
  artifactVersion: "input-live-v66-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V66_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V66_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V66_TARGET;
  authorization: InputLiveV66TransactionAuthorization;
...
export interface InputLiveV66ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV66RequestBody {
  artifactVersion: "input-live-v66-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V66_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV66RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV66CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V66_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV66ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV66Request extends InputLiveV66RequestBody {
...
export interface InputLiveV66Receipt {
  artifactVersion: "input-live-v66-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V66_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV66RemotePhase;
...
export interface InputLiveV66AcceptedResponse<T = unknown> {
  request: InputLiveV66Request;
  receipt: InputLiveV66Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV66IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV66ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V66_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V66_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V66_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V66_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V66_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V66_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V66_TARGET.editorType;
};
...
export const inputLiveV66RequestSequence = (
  phase: InputLiveV66RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV66RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV66RequestPath = (
  directory: string,
  phase: InputLiveV66RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV66ResponsePath = (
  directory: string,
  phase: InputLiveV66RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV66ReceiptPath = (
  directory: string,
  phase: InputLiveV66RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV66Manifest => {
  const manifest = json<InputLiveV66Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v66-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V66_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V66_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV66RemotePhase,
  captureIndex?: number,
): InputLiveV66Request | undefined => {
  const file = inputLiveV66RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV66Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV66RemotePhase,
  captureIndex?: number,
): InputLiveV66Receipt | undefined => {
  const file = inputLiveV66ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV66Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV66Request | undefined;
  receipt: InputLiveV66Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV66RemotePhase,
): InputLiveV66ResponseContract => ({
  ...INPUT_LIVE_V66_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V66_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V66_DYNAMIC_TOOL,
      target: INPUT_LIVE_V66_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v66-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V66_BROKER_VERSION,
  target: INPUT_LIVE_V66_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V66_PROGRAM_START*/${program}/*INPUT_LIVE_V66_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V66_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V66_PROGRAM_END*/";
...
export function createInputLiveV66Transaction(
  directory: string,
  ...
  const manifest: InputLiveV66Manifest = {
    artifactVersion: "input-live-v66-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V66_BROKER_VERSION,
...
export function validateInputLiveV66Request(
  directory: string,
  request: InputLiveV66Request,
): void {
  ...
  const expectedSequence = inputLiveV66RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v66-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V66_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V66_TARGET.fileKey ||
...
export function issueInputLiveV66Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV66RemotePhase,
  program: string,
  options: InputLiveV66IssueOptions = {},
): InputLiveV66Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV66RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV66Request>(targetPath);
    validateInputLiveV66Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV66ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV66TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV66RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V66_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V66_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V66_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV66RequestBody = {
    artifactVersion: "input-live-v66-broker-request-v2",
    brokerVersion: INPUT_LIVE_V66_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV66Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV66Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV66RawResponse(
  directory: string,
  phase: InputLiveV66RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV66RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV66Request>(requestFile);
  if (
  ...
  const target = inputLiveV66ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV66Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v66-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V66_BROKER_VERSION,
...
export function acceptInputLiveV66Response<T = unknown>(
  directory: string,
  phase: InputLiveV66RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV66AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV66ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV66Request>(
    inputLiveV66RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV66Request(directory, request);
  if (phase === "capture") validateInputLiveV66TechnicalGates(directory);
  const rawFile = inputLiveV66ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v66-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V66_BROKER_VERSION ||
...
export function persistInputLiveV66TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV66RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV66RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V66_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v66-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V66_BROKER_VERSION,
...
export function validateInputLiveV66TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v66-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V66_TECHNICAL_GATES.some(
...
export function validateInputLiveV66Receipt(
  directory: string,
  phase: InputLiveV66RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV66Request(directory, request);
  const receipt = json<InputLiveV66Receipt>(
    inputLiveV66ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV66ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v66-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V66_BROKER_VERSION ||
...
export function readInputLiveV66AcceptedRawPayload(
  directory: string,
  phase: InputLiveV66RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV66Receipt(directory, phase, captureIndex);
...
export function createInputLiveV66FakeDynamicResponse(
  request: InputLiveV66Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V66_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V66_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV66PersistedRequests(
  directory: string,
): InputLiveV66Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV66Request>(file)] : [];
    });
}

export const inputLiveV66Sha256 = sha256;
