...
import { verifyTableLiveV19Authorization } from "./table-live-v19-authorization.js";
import { TABLE_LIVE_V19_TARGET } from "./table-live-v19-broker.js";
import {
  TABLE_LIVE_V19_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV19Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-table-v19-session";
...
const attestation: TableLiveV19SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV19SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v19 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V19_SECURITY_ATTESTATION_DEFAULT_PATH,
);
