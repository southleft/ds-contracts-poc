...
import {
  validateCalendarLiveV47AntecedentIndex,
  verifyCalendarLiveV47Authorization,
  type CalendarLiveV47AntecedentIndex,
  type CalendarLiveV47AuthorizationProof,
} from "./calendar-live-v47-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V47_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v47/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V47_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v47-security-attestation.json";

export interface CalendarLiveV47SecurityAttestation {
  artifactVersion: "calendar-live-v47-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV47PreflightReport {
  artifactVersion: "calendar-live-v47-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV47SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV47SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV47AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v47-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V47_TARGET) ||
...
export function validateCalendarLiveV47ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV47Authorization");
  const preflight = source.indexOf("runCalendarLiveV47Preflight");
  const initialize = source.indexOf("CalendarLiveV47Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV47Preflight(
  root: string,
  proof: CalendarLiveV47AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V47_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV47PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V47_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V47_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV47AntecedentIndex;
  failures.push(...validateCalendarLiveV47AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V47_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V47_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV47SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v47-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV47ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v47 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV47SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v47-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V47_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV47Preflight(
    process.cwd(),
    verifyCalendarLiveV47Authorization(),
    attestationPath,
