...
import { INPUT_LIVE_V70_INDEX_PATH } from "./build-input-field-live-proof-v70.js";
import {
  INPUT_LIVE_V70_DYNAMIC_TOOL,
  INPUT_LIVE_V70_TARGET,
} from "./input-field-live-v70-broker.js";
import {
  buildInputLiveV70AuthorizationArtifact,
  validateInputLiveV70AntecedentIndex,
  validateInputLiveV70AuthorizationArtifact,
  validateInputLiveV70History,
  validateInputLiveV70UnitTestSource,
  type InputLiveV70AntecedentIndex,
  type InputLiveV70AuthorizationArtifact,
  type InputLiveV70AuthorizationProof,
  type InputLiveV70HistoryState,
} from "./input-field-live-v70-authorization.js";
import {
  inputLiveV70AttestationSha256,
  validateInputLiveV70ControlFlowSource,
  validateInputLiveV70SecurityAttestation,
  type InputLiveV70SecurityAttestation,
} from "./input-field-live-v70-preflight.js";
import { inputLiveV70Sha256 } from "./input-field-live-v70-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V70_INDEX_PATH);
const index = (): InputLiveV70AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV70AntecedentIndex;

const artifact = (): InputLiveV70AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV70AuthorizationArtifact({
...
const pendingState = (): InputLiveV70HistoryState => ({
  index: index(),
  indexSha256: inputLiveV70Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV70HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV70AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV70Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V70_TARGET,
  expectedDynamicTool: INPUT_LIVE_V70_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v70/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV70AuthorizationArtifact,
): InputLiveV70SecurityAttestation => {
  const body: Omit<InputLiveV70SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v70-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V70_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV70AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV70History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV70History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV70History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV70History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV70History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV70UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV70UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV70AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV70Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV70AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV70AuthorizationArtifact(
        planted as InputLiveV70AuthorizationArtifact,
        index(),
        inputLiveV70Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV70SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV70AttestationSha256(
      body as Omit<InputLiveV70SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV70SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV70ControlFlowSource(source), []);
});
