/**
 * Calendar live v36 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V36_FIXED_VERIFIER_VERSION = "calendar-live-v36-fixed-point-v1";
export const CALENDAR_LIVE_V36_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV36FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV36FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV36FixedSourceProbe {
...
export interface CalendarLiveV36FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV36FixedFixedPoint;
}

export interface CalendarLiveV36FixedHardGateInput {
  thresholds: CalendarLiveV36FixedThresholds;
  sourceProbes: CalendarLiveV36FixedSourceProbe[];
  cells: CalendarLiveV36FixedCellValidation[];
  sceneProofs: CalendarLiveV36FixedSceneProof[];
...
export interface CalendarLiveV36FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V36_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV36FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV36FixedHardGates(
  input: CalendarLiveV36FixedHardGateInput,
): CalendarLiveV36FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV36FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V36_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV36FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV36FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV36FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
