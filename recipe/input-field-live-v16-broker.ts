...
import {
  ...
  INPUT_LIVE_V16_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V16_RESPONSE_CONTRACTS,
  type InputLiveV16RootProof,
} from "./input-field-live-v16-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V16_BROKER_VERSION = "input-live-v16-broker-v2";
export const INPUT_LIVE_V16_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V16_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V16_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V16_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V16_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V16_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV16RemotePhase =
  (typeof INPUT_LIVE_V16_REMOTE_PHASES)[number];
export type InputLiveV16AuthorizationMode = "live" | "simulated";

export interface InputLiveV16CommitPins {
...
export interface InputLiveV16TransactionAuthorization extends InputLiveV16CommitPins {
  mode: InputLiveV16AuthorizationMode;
...
export interface InputLiveV16Manifest {
  artifactVersion: "input-live-v16-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V16_BROKER_VERSION;
...
export interface InputLiveV16ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV16RequestBody {
  artifactVersion: "input-live-v16-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V16_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV16RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV16CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V16_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV16ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV16Request extends InputLiveV16RequestBody {
...
export interface InputLiveV16Receipt {
  artifactVersion: "input-live-v16-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V16_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV16RemotePhase;
...
export interface InputLiveV16AcceptedResponse<T = unknown> {
  request: InputLiveV16Request;
  receipt: InputLiveV16Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV16IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV16ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V16_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V16_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V16_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V16_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V16_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V16_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V16_TARGET.editorType;
};
...
export const inputLiveV16RequestSequence = (
  phase: InputLiveV16RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV16RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV16RequestPath = (
  directory: string,
  phase: InputLiveV16RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV16ResponsePath = (
  directory: string,
  phase: InputLiveV16RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV16ReceiptPath = (
  directory: string,
  phase: InputLiveV16RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV16Manifest => {
  const manifest = json<InputLiveV16Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v16-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V16_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V16_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV16RemotePhase,
  captureIndex?: number,
): InputLiveV16Request | undefined => {
  const file = inputLiveV16RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV16Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV16RemotePhase,
  captureIndex?: number,
): InputLiveV16Receipt | undefined => {
  const file = inputLiveV16ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV16Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV16Request | undefined;
  receipt: InputLiveV16Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV16RemotePhase,
): InputLiveV16ResponseContract => ({
  ...INPUT_LIVE_V16_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V16_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v16-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V16_BROKER_VERSION,
  target: INPUT_LIVE_V16_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V16_PROGRAM_START*/${program}/*INPUT_LIVE_V16_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V16_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V16_PROGRAM_END*/";
...
export function createInputLiveV16Transaction(
  directory: string,
  ...
  const manifest: InputLiveV16Manifest = {
    artifactVersion: "input-live-v16-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V16_BROKER_VERSION,
...
export function validateInputLiveV16Request(
  directory: string,
  request: InputLiveV16Request,
): void {
  ...
  const expectedSequence = inputLiveV16RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v16-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V16_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V16_TARGET.fileKey ||
...
export function issueInputLiveV16Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV16RemotePhase,
  program: string,
  options: InputLiveV16IssueOptions = {},
): InputLiveV16Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV16RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV16Request>(targetPath);
    validateInputLiveV16Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV16ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV16TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV16RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V16_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V16_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V16_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV16RequestBody = {
    artifactVersion: "input-live-v16-broker-request-v2",
    brokerVersion: INPUT_LIVE_V16_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV16Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV16Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV16RawResponse(
  directory: string,
  phase: InputLiveV16RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV16RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV16Request>(requestFile);
  if (
  ...
  const target = inputLiveV16ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV16Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v16-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V16_BROKER_VERSION,
...
export function acceptInputLiveV16Response<T = unknown>(
  directory: string,
  phase: InputLiveV16RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV16AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV16ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV16Request>(
    inputLiveV16RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV16Request(directory, request);
  if (phase === "capture") validateInputLiveV16TechnicalGates(directory);
  const rawFile = inputLiveV16ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v16-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V16_BROKER_VERSION ||
...
export function persistInputLiveV16TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV16RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV16RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V16_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v16-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V16_BROKER_VERSION,
...
export function validateInputLiveV16TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v16-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V16_TECHNICAL_GATES.some(
...
export function validateInputLiveV16Receipt(
  directory: string,
  phase: InputLiveV16RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV16Request(directory, request);
  const receipt = json<InputLiveV16Receipt>(
    inputLiveV16ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV16ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v16-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V16_BROKER_VERSION ||
...
export function readInputLiveV16AcceptedRawPayload(
  directory: string,
  phase: InputLiveV16RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV16Receipt(directory, phase, captureIndex);
...
export function createInputLiveV16FakeDynamicResponse(
  request: InputLiveV16Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V16_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V16_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV16PersistedRequests(
  directory: string,
): InputLiveV16Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV16Request>(file)] : [];
    });
}

export const inputLiveV16Sha256 = sha256;
