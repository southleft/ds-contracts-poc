...
import { CALENDAR_LIVE_V33_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v33.js";
import { verifyCalendarLiveV33Authorization } from "./calendar-live-v33-authorization.js";
import { runCalendarLiveV33Preflight } from "./calendar-live-v33-preflight.js";
import { CalendarLiveV33Orchestrator } from "./run-calendar-live-v33.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v33 requires exact authorization artifact ${CALENDAR_LIVE_V33_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v33-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV33Authorization(root);
    runCalendarLiveV33Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV33Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV33Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
