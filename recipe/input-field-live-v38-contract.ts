...
} from "./input-field-live-v3-verifier-v38.js";
import {
  buildInputLiveV38RawPropertyRuntime,
  normalizeInputLiveV38Scene,
  type InputLiveV38RawNode,
} from "./input-field-live-v38-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v38.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v38.js";

export {
  buildInputLiveV38RestoreProgram,
  validateInputLiveV38RestorePayload,
  type InputLiveV38RestorePayload,
} from "./input-field-live-v38-restore.js";

export const INPUT_LIVE_V38_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V38_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V38_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V38_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V38_VARIANT_COUNT = 256;
export const INPUT_LIVE_V38_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V38_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV38SourceId = (typeof INPUT_LIVE_V38_SOURCE_IDS)[number];

export interface InputLiveV38SourceIdentity {
  source: InputLiveV38SourceId;
...
export interface InputLiveV38WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV38ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV38SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV38RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV38RootProof {
  source: InputLiveV38SourceId;
  adapterIdentity: string;
...
export interface InputLiveV38ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV38SourceId;
    adapterIdentity: string;
...
export interface InputLiveV38CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV38SourceId;
  adapterIdentity: string;
...
export interface InputLiveV38CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV38SourceId;
  strata: InputLiveV38CaptureCell["strata"];
...
export interface InputLiveV38CleanupPayload {
...
export function validateInputLiveV38WriterPayload(
  value: unknown,
): InputLiveV38WriterOwnership {
...
const identityPlan = (source: InputLiveV38SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV38ExtractProgram(
  writer: InputLiveV38WriterOwnership,
  sources: readonly InputLiveV38SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V38_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V38_NAMESPACE)}
${buildInputLiveV38RawPropertyRuntime()}
...
export function validateInputLiveV38ExtractPayload(
  value: unknown,
  writer: InputLiveV38WriterOwnership,
): InputLiveV38ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V38_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV38ExtractPayload;
}

export function proveInputLiveV38Roots<Instance>(
  extract: InputLiveV38ExtractPayload,
  sources: readonly (InputLiveV38SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV38Scene(
      root.scene,
...
export function assertInputLiveV38RootProofs(
  proofs: readonly InputLiveV38RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV38ProbeProgram(
  writer: InputLiveV38WriterOwnership,
  sources: readonly InputLiveV38SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V38_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV38ProbePayload(
  value: unknown,
  writer: InputLiveV38WriterOwnership,
): InputLiveV38ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV38ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V38_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V38_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV38CaptureManifest(
  cells: readonly InputLiveV38CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V38_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V38_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V38_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V38_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V38_SOURCE_IDS.some(
...
export const inputLiveV38CaptureManifestSha256 = (
  cells: readonly InputLiveV38CaptureCell[],
): string => {
  validateInputLiveV38CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV38CaptureProgram(
  writer: InputLiveV38WriterOwnership,
  cell: InputLiveV38CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V38_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V38_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V38_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV38CapturePayload(
  value: unknown,
  cell: InputLiveV38CaptureCell,
  rawResponseBytes: number,
): InputLiveV38CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV38CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V38_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V38_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV38CaptureResponses(
  manifest: readonly InputLiveV38CaptureCell[],
  responses: readonly InputLiveV38CapturePayload[],
): void {
  validateInputLiveV38CaptureManifest(manifest);
  if (
...
export interface InputLiveV38ObjectiveReport {
  artifactVersion: "input-live-v38-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV38Objective(
  manifest: readonly InputLiveV38CaptureCell[],
  responses: readonly InputLiveV38CapturePayload[],
  readReference: (cell: InputLiveV38CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV38CaptureCell) => Buffer,
): InputLiveV38ObjectiveReport {
  assertInputLiveV38CaptureResponses(manifest, responses);
...
export function buildInputLiveV38CleanupProgram(
  writer: InputLiveV38WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V38_NAMESPACE,
...
export function validateInputLiveV38CleanupPayload(
  value: unknown,
  writer: InputLiveV38WriterOwnership,
): InputLiveV38CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV38CleanupPayload;
...
export const INPUT_LIVE_V38_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV38WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
