...
import { INPUT_LIVE_V63_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v63.js";
import { verifyInputLiveV63Authorization } from "./input-field-live-v63-authorization.js";
import { runInputLiveV63Preflight } from "./input-field-live-v63-preflight.js";
import { InputLiveV63Orchestrator } from "./run-input-field-live-v63.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V63_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v63-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV63Authorization(root);
    runInputLiveV63Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV63Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
