...
import {
  ...
  createInputLiveV29FakeDynamicResponse,
  ...
  listInputLiveV29PersistedRequests,
  persistInputLiveV29RawResponse,
  persistInputLiveV29TechnicalGates,
  validateInputLiveV29Receipt,
  validateInputLiveV29Request,
  type InputLiveV29RemotePhase,
...
import {
  ...
  type InputLiveV29ExtractPayload,
  type InputLiveV29ProbePayload,
...
import {
  ...
  simulatedInputLiveV29Authorization,
} from "./run-input-field-live-v29.js";
...
const captures: InputLiveV29CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV29Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v29-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV29Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV29CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV29FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV29FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V29_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V29_TARGET);
  persistInputLiveV29RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV29Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV29Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV29WriterOwnership,
): InputLiveV29RestorePayload => ({
...
const rootProofs = (): InputLiveV29RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v29-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV29Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV29Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV29RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV29RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV29ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV29ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV29CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV29CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV29CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV29Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV29CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV29PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV29ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV29Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV29Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV29Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV29Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V29_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV29RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V29_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV29Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV29Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV29ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV29Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV29WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV29RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV29Request;
    validateInputLiveV29Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV29Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV29CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v29-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV29Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV29Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV29Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V29_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV29RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V29_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV29Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV29Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV29RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V29_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV29Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV29Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV29Request;
