...
import {
  ...
  INPUT_LIVE_V10_INDEX_PATH,
} from "./build-input-field-live-proof-v10.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV10HistoryExpectation = "pending" | "authorized";

export interface InputLiveV10AuthorizationArtifact {
  artifactVersion: "input-live-v10-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V10_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV10AntecedentIndex {
...
export interface InputLiveV10HistoryState {
  index?: InputLiveV10AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV10AuthorizationArtifact;
...
export interface InputLiveV10AuthorizationProof extends InputLiveV10TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V10_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V10_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V10_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V10_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v10-status") ||
...
export function validateInputLiveV10AntecedentIndex(
  index: InputLiveV10AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v10-antecedent-index-v1" ||
...
export function validateInputLiveV10UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV10History\(\s*["']/.test(source) ||
    /readInputLiveV10HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV10AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV10AuthorizationArtifact {
  ...
  ) as InputLiveV10AntecedentIndex;
  const indexFailures = validateInputLiveV10AntecedentIndex(index);
...
export function validateInputLiveV10AuthorizationArtifact(
  artifact: InputLiveV10AuthorizationArtifact | undefined,
  index: InputLiveV10AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v10-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v10" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V10_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V10_TARGET) ||
    !equalJson(
...
export function validateInputLiveV10History(
  state: InputLiveV10HistoryState,
  expected: InputLiveV10HistoryExpectation,
): string[] {
  const failures = validateInputLiveV10AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV10AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV10HistoryState(
  root = process.cwd(),
): InputLiveV10HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V10_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV10AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V10_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V10_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V10_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV10History(
  expected: InputLiveV10HistoryExpectation,
  root = process.cwd(),
): InputLiveV10HistoryState {
  const state = readInputLiveV10HistoryState(root);
  const failures = validateInputLiveV10History(state, expected);
  if (failures.length)
    throw new Error(
      `Input live v10 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyInputLiveV10Authorization(
  root = process.cwd(),
): InputLiveV10AuthorizationProof {
  const state = verifyInputLiveV10History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(path.join(repositoryRoot, INPUT_LIVE_V10_AUTHORIZATION_PATH)),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v10/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v10.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V10_TARGET,
    expectedDynamicTool: INPUT_LIVE_V10_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V10_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV10HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV10History(expected);
  process.stdout.write(
    `Input live v10 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
