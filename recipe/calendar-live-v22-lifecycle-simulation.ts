...
import { calendarLiveV1Sha256 } from "./calendar-live-v22-broker.js";

export interface CalendarLiveV22LifecycleSimulationReport {
  artifactVersion: "calendar-live-v22-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV22AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV22Lifecycle(): Promise<CalendarLiveV22LifecycleSimulationReport> {
  await buildCalendarLiveV22Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V22_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV22AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v22-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V22_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V22_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV22History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV22History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV22AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V22_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V22_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV22History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV22History(authorized, "pending");
    ...
    await buildCalendarLiveV22Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V22_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV22AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV22Lifecycle(), null, 2)}\n`,
  );
