...
import {
  assertInputLiveV67RootProofs,
  INPUT_LIVE_V67_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V67_RESPONSE_CONTRACTS,
  type InputLiveV67RootProof,
} from "./input-field-live-v67-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V67_BROKER_VERSION = "input-live-v67-broker-v2";
export const INPUT_LIVE_V67_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V67_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V67_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V67_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V67_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V67_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV67RemotePhase =
  (typeof INPUT_LIVE_V67_REMOTE_PHASES)[number];
export type InputLiveV67AuthorizationMode = "live" | "simulated";

export interface InputLiveV67CommitPins {
...
export interface InputLiveV67TransactionAuthorization extends InputLiveV67CommitPins {
  mode: InputLiveV67AuthorizationMode;
...
export interface InputLiveV67Manifest {
  artifactVersion: "input-live-v67-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V67_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V67_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V67_TARGET;
  authorization: InputLiveV67TransactionAuthorization;
...
export interface InputLiveV67ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV67RequestBody {
  artifactVersion: "input-live-v67-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V67_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV67RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV67CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V67_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV67ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV67Request extends InputLiveV67RequestBody {
...
export interface InputLiveV67Receipt {
  artifactVersion: "input-live-v67-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V67_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV67RemotePhase;
...
export interface InputLiveV67AcceptedResponse<T = unknown> {
  request: InputLiveV67Request;
  receipt: InputLiveV67Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV67IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV67ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V67_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V67_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V67_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V67_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V67_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V67_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V67_TARGET.editorType;
};
...
export const inputLiveV67RequestSequence = (
  phase: InputLiveV67RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV67RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV67RequestPath = (
  directory: string,
  phase: InputLiveV67RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV67ResponsePath = (
  directory: string,
  phase: InputLiveV67RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV67ReceiptPath = (
  directory: string,
  phase: InputLiveV67RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV67Manifest => {
  const manifest = json<InputLiveV67Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v67-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V67_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V67_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV67RemotePhase,
  captureIndex?: number,
): InputLiveV67Request | undefined => {
  const file = inputLiveV67RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV67Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV67RemotePhase,
  captureIndex?: number,
): InputLiveV67Receipt | undefined => {
  const file = inputLiveV67ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV67Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV67Request | undefined;
  receipt: InputLiveV67Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV67RemotePhase,
): InputLiveV67ResponseContract => ({
  ...INPUT_LIVE_V67_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V67_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v67-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V67_BROKER_VERSION,
  target: INPUT_LIVE_V67_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V67_PROGRAM_START*/${program}/*INPUT_LIVE_V67_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V67_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V67_PROGRAM_END*/";
...
export function createInputLiveV67Transaction(
  directory: string,
  ...
  const manifest: InputLiveV67Manifest = {
    artifactVersion: "input-live-v67-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V67_BROKER_VERSION,
...
export function validateInputLiveV67Request(
  directory: string,
  request: InputLiveV67Request,
): void {
  ...
  const expectedSequence = inputLiveV67RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v67-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V67_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V67_TARGET.fileKey ||
...
export function issueInputLiveV67Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV67RemotePhase,
  program: string,
  options: InputLiveV67IssueOptions = {},
): InputLiveV67Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV67RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV67Request>(targetPath);
    validateInputLiveV67Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV67ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV67TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV67RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V67_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V67_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V67_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV67RequestBody = {
    artifactVersion: "input-live-v67-broker-request-v2",
    brokerVersion: INPUT_LIVE_V67_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV67Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV67Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV67RawResponse(
  directory: string,
  phase: InputLiveV67RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV67RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV67Request>(requestFile);
  if (
  ...
  const target = inputLiveV67ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV67Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v67-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V67_BROKER_VERSION,
...
export function acceptInputLiveV67Response<T = unknown>(
  directory: string,
  phase: InputLiveV67RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV67AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV67ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV67Request>(
    inputLiveV67RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV67Request(directory, request);
  if (phase === "capture") validateInputLiveV67TechnicalGates(directory);
  const rawFile = inputLiveV67ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v67-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V67_BROKER_VERSION ||
...
export function persistInputLiveV67TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV67RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV67RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V67_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v67-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V67_BROKER_VERSION,
...
export function validateInputLiveV67TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v67-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V67_TECHNICAL_GATES.some(
...
export function validateInputLiveV67Receipt(
  directory: string,
  phase: InputLiveV67RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV67Request(directory, request);
  const receipt = json<InputLiveV67Receipt>(
    inputLiveV67ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV67ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v67-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V67_BROKER_VERSION ||
...
export function readInputLiveV67AcceptedRawPayload(
  directory: string,
  phase: InputLiveV67RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV67Receipt(directory, phase, captureIndex);
...
export function createInputLiveV67FakeDynamicResponse(
  request: InputLiveV67Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V67_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V67_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV67PersistedRequests(
  directory: string,
): InputLiveV67Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV67Request>(file)] : [];
    });
}

export const inputLiveV67Sha256 = sha256;
