...
export type TableLiveV17HistoryExpectation = "pending" | "authorized";

export interface TableLiveV17AuthorizationArtifact {
  artifactVersion: "table-live-v17-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V17_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV17AntecedentIndex {
...
export interface TableLiveV17HistoryState {
  index?: TableLiveV17AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV17AuthorizationArtifact;
...
export interface TableLiveV17AuthorizationProof
  extends TableLiveV17TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V17_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V17_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V17_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V17_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v17-authorization") ||
  artifactPath.includes("table-live-v17-preflight") ||
  artifactPath.includes("table-live-v17-authorized") ||
  artifactPath.includes("create-table-live-v17-security-attestation") ||
  artifactPath.includes("table-live-pivot-v17-status") ||
...
export function validateTableLiveV17AntecedentIndex(
  index: TableLiveV17AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v17-antecedent-index-v1" ||
...
export function validateTableLiveV17UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV17History\(\s*["']/.test(source) ||
    /readTableLiveV17HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV17AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV17AuthorizationArtifact {
  ...
  ) as TableLiveV17AntecedentIndex;
  const indexFailures = validateTableLiveV17AntecedentIndex(index);
...
export function validateTableLiveV17AuthorizationArtifact(
  artifact: TableLiveV17AuthorizationArtifact | undefined,
  index: TableLiveV17AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v17-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v17" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V17_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V17_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V17_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V17_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V17_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V17_SOURCE_ROOTS ||
...
export function validateTableLiveV17History(
  state: TableLiveV17HistoryState,
  expected: TableLiveV17HistoryExpectation,
): string[] {
  const failures = validateTableLiveV17AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV17AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV17HistoryState(
  root = process.cwd(),
): TableLiveV17HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V17_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV17AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V17_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V17_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V17_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV17History(
  expected: TableLiveV17HistoryExpectation,
  root = process.cwd(),
): TableLiveV17HistoryState {
  const state = readTableLiveV17HistoryState(root);
  const failures = validateTableLiveV17History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v17 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV17Authorization(
  root = process.cwd(),
): TableLiveV17AuthorizationProof {
  const state = verifyTableLiveV17History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V17_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v17/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v17.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V17_TARGET,
    expectedDynamicTool: TABLE_LIVE_V17_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V17_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV17HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV17History(expected);
  process.stdout.write(
    `Table live v17 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
