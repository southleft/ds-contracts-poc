...
import {
  acceptTableLiveV19Response,
  createTableLiveV19Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV19Request,
  persistTableLiveV19RawResponse,
  persistTableLiveV19TechnicalGates,
  readTableLiveV19AcceptedRawPayload,
  type TableLiveV19Request,
  type TableLiveV19TransactionAuthorization,
} from "./table-live-v19-broker.js";
import {
  TABLE_LIVE_V19_CAPTURE_COUNT,
  assertTableLiveV19CaptureResponses,
  ...
  evaluateTableLiveV19Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV19Roots,
  validateTableLiveV19CaptureManifest,
  validateTableLiveV19CapturePayload,
  validateTableLiveV19CleanupPayload,
  validateTableLiveV19ExtractPayload,
  validateTableLiveV19ProbePayload,
  validateTableLiveV19WriterPayload,
  type TableLiveV19CaptureCell,
  type TableLiveV19CapturePayload,
...
import {
  ...
  TABLE_LIVE_V19_PLAN_PATH,
  type TableLiveV19ProofPlan,
} from "./build-table-live-proof-v19.js";
import {
...
export const TABLE_LIVE_V19_RUNNER_VERSION = "table-live-v19-runner-v1";

interface TableLiveV19JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV19OrchestratorState {
  artifactVersion: "table-live-v19-orchestrator-state-v1";
  ...
  journal: TableLiveV19JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV19ProofPlan;
  planSha256: string;
  captures: TableLiveV19CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV19SourceIdentity & {
...
export interface TableLiveV19NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV19OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v19 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V19_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V19_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV19ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV19CaptureCell[];
  validateTableLiveV19CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v19 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v19 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV19WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV19RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV19Request =>
  readJson<TableLiveV19Request>(file);

export class TableLiveV19Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV19Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV19OrchestratorState = {
      artifactVersion: "table-live-v19-orchestrator-state-v1",
    ...
    return new TableLiveV19Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV19OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV19Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV19NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV19NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v19 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV19NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v19 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV19RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV19Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV19CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV19AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV19WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV19Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV19ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV19Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV19Response(this.#directory, "writer", {
        validate: validateTableLiveV19WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV19Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV19RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV19Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV19ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV19Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV19RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v19-host-root-proofs-v1",
      ...
      const next = issueTableLiveV19Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV19ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV19Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV19ProbePayload(payload, ownership),
      });
      persistTableLiveV19TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v19 capture cell missing");
      const accepted = acceptTableLiveV19Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV19CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V19_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV19CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV19Objective(
    ...
    throw new Error(`Table live v19 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV19Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV19CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV19Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV19RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV19Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV19CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV19CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV19CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV19Authorization = (
  privateKey: KeyObject,
): TableLiveV19TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV19SimulationKey = (): KeyObject =>
