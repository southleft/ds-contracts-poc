...
import { TABLE_LIVE_V2_AUTHORIZATION_PATH } from "./build-table-live-proof-v2.js";
import { verifyTableLiveV2Authorization } from "./table-live-v2-authorization.js";
import { runTableLiveV2Preflight } from "./table-live-v2-preflight.js";
import { TableLiveV2Orchestrator } from "./run-table-live-v2.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v2 requires exact authorization artifact ${TABLE_LIVE_V2_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v2-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV2Authorization(root);
    runTableLiveV2Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV2Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV2Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
