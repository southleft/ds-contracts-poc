...
import {
  ...
  INPUT_LIVE_V48_INDEX_PATH,
} from "./build-input-field-live-proof-v48.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV48HistoryExpectation = "pending" | "authorized";

export interface InputLiveV48AuthorizationArtifact {
  artifactVersion: "input-live-v48-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V48_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV48AntecedentIndex {
...
export interface InputLiveV48HistoryState {
  index?: InputLiveV48AntecedentIndex;
  indexSha256: string;
...
export interface InputLiveV48AuthorizationProof extends InputLiveV48TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V48_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V48_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V48_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V48_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v48-status") ||
...
export function validateInputLiveV48AntecedentIndex(
  index: InputLiveV48AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v48-antecedent-index-v1" ||
...
export function validateInputLiveV48UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV48History\(\s*["']/.test(source) ||
    /readInputLiveV48HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV48AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV48AuthorizationArtifact {
  ...
  ) as InputLiveV48AntecedentIndex;
  const indexFailures = validateInputLiveV48AntecedentIndex(index);
...
export function validateInputLiveV48AuthorizationArtifact(
  artifact: InputLiveV48AuthorizationArtifact | undefined,
  index: InputLiveV48AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v48-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v48" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V48_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V48_TARGET) ||
    !equalJson(
...
export function validateInputLiveV48History(
  state: InputLiveV48HistoryState,
  expected: InputLiveV48HistoryExpectation,
): string[] {
  const failures = validateInputLiveV48AntecedentIndex(state.index);
...
export function readInputLiveV48HistoryState(
  root = process.cwd(),
): InputLiveV48HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V48_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV48AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V48_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V48_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V48_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV48History(
  expected: InputLiveV48HistoryExpectation,
  root = process.cwd(),
): InputLiveV48HistoryState {
  const state = readInputLiveV48HistoryState(root);
  const failures = validateInputLiveV48History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV48Authorization(
  root = process.cwd(),
): InputLiveV48AuthorizationProof {
  const state = verifyInputLiveV48History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V48_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v48/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v48.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V48_TARGET,
    expectedDynamicTool: INPUT_LIVE_V48_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V48_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV48HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV48History(expected);
