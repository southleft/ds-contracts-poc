...
import {
  validateComboboxLiveV42AntecedentIndex,
  verifyComboboxLiveV42Authorization,
  type ComboboxLiveV42AntecedentIndex,
  type ComboboxLiveV42AuthorizationProof,
} from "./combobox-live-v42-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V42_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v42/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V42_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v42-security-attestation.json";

export interface ComboboxLiveV42SecurityAttestation {
  artifactVersion: "combobox-live-v42-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV42PreflightReport {
  artifactVersion: "combobox-live-v42-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV42SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV42SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV42AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v42-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V42_TARGET) ||
...
export function validateComboboxLiveV42ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV42Authorization");
  const preflight = source.indexOf("runComboboxLiveV42Preflight");
  const initialize = source.indexOf("ComboboxLiveV42Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV42Preflight(
  root: string,
  proof: ComboboxLiveV42AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V42_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV42PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V42_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V42_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V42_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV42AntecedentIndex;
  failures.push(...validateComboboxLiveV42AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V42_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V42_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV42SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v42-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV42ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v42 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV42SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v42-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V42_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV42Preflight(
    process.cwd(),
    verifyComboboxLiveV42Authorization(),
    attestationPath,
