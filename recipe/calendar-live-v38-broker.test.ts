...
test("Calendar live v38 broker pins Scratch and 8 captures", () => {
  assert.equal(CALENDAR_LIVE_V38_TARGET.fileKey, "byMp6lt0Ij9b2QbkDGFwBh");
  assert.equal(CALENDAR_LIVE_V38_CAPTURE_COUNT, 8);
  assert.equal(CALENDAR_LIVE_V38_SOURCE_ROOTS, 1);
...
test("gates validate one Astryx root — the Calendar live v22 class", () => {
  const broker = readFileSync("recipe/calendar-live-v38-broker.ts", "utf8");
  assert.match(
    broker,
