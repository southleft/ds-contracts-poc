...
} from "./input-field-live-v3-verifier-v77.js";
import {
  buildInputLiveV77RawPropertyRuntime,
  normalizeInputLiveV77Scene,
  type InputLiveV77RawNode,
} from "./input-field-live-v77-verifier.js";
...
export {
  ...
  type InputLiveV77RestorePayload,
} from "./input-field-live-v77-restore.js";

export const INPUT_LIVE_V77_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V77_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V77_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V77_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V77_VARIANT_COUNT = 256;
export const INPUT_LIVE_V77_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V77_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV77SourceId = (typeof INPUT_LIVE_V77_SOURCE_IDS)[number];

export interface InputLiveV77SourceIdentity {
  source: InputLiveV77SourceId;
...
export interface InputLiveV77WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV77ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV77SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV77RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV77RootProof {
  source: InputLiveV77SourceId;
  adapterIdentity: string;
...
export interface InputLiveV77ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV77SourceId;
    adapterIdentity: string;
...
export interface InputLiveV77CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV77SourceId;
  adapterIdentity: string;
...
export interface InputLiveV77CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV77SourceId;
  strata: InputLiveV77CaptureCell["strata"];
...
export interface InputLiveV77CleanupPayload {
...
export function validateInputLiveV77WriterPayload(
  value: unknown,
): InputLiveV77WriterOwnership {
...
const identityPlan = (source: InputLiveV77SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV77ExtractProgram(
  writer: InputLiveV77WriterOwnership,
  sources: readonly InputLiveV77SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V77_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V77_NAMESPACE)}
${buildInputLiveV77RawPropertyRuntime()}
...
export function validateInputLiveV77ExtractPayload(
  value: unknown,
  writer: InputLiveV77WriterOwnership,
): InputLiveV77ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V77_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV77ExtractPayload;
}

export function proveInputLiveV77Roots<Instance>(
  extract: InputLiveV77ExtractPayload,
  sources: readonly (InputLiveV77SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV77Scene(
      root.scene,
...
export function assertInputLiveV77RootProofs(
  proofs: readonly InputLiveV77RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV77ProbeProgram(
  writer: InputLiveV77WriterOwnership,
  sources: readonly InputLiveV77SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V77_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV77ProbePayload(
  value: unknown,
  writer: InputLiveV77WriterOwnership,
): InputLiveV77ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV77ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V77_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V77_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV77CaptureManifest(
  cells: readonly InputLiveV77CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V77_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V77_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V77_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V77_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V77_SOURCE_IDS.some(
...
export const inputLiveV77CaptureManifestSha256 = (
  cells: readonly InputLiveV77CaptureCell[],
): string => {
  validateInputLiveV77CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV77CaptureProgram(
  writer: InputLiveV77WriterOwnership,
  cell: InputLiveV77CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V77_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V77_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V77_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV77CapturePayload(
  value: unknown,
  cell: InputLiveV77CaptureCell,
  rawResponseBytes: number,
): InputLiveV77CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV77CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V77_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V77_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV77CaptureResponses(
  manifest: readonly InputLiveV77CaptureCell[],
  responses: readonly InputLiveV77CapturePayload[],
): void {
  validateInputLiveV77CaptureManifest(manifest);
  if (
...
export interface InputLiveV77ObjectiveReport {
  artifactVersion: "input-live-v77-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV77Objective(
  manifest: readonly InputLiveV77CaptureCell[],
  responses: readonly InputLiveV77CapturePayload[],
  readReference: (cell: InputLiveV77CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV77CaptureCell) => Buffer,
): InputLiveV77ObjectiveReport {
  assertInputLiveV77CaptureResponses(manifest, responses);
...
export function buildInputLiveV77CleanupProgram(
  writer: InputLiveV77WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V77_NAMESPACE,
...
export function validateInputLiveV77CleanupPayload(
  value: unknown,
  writer: InputLiveV77WriterOwnership,
): InputLiveV77CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV77CleanupPayload;
...
export const INPUT_LIVE_V77_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV77WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
