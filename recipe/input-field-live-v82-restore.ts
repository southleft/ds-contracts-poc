...
export const INPUT_LIVE_V82_MEASURE_VISIBLE_FILL_MARKER =
  "INPUT-TEXT-FILL-MEASURE-VISIBLE";
export const INPUT_LIVE_V82_RESTORE_CONTENT_ROLES = [
  "input-field/content/placeholder",
  "input-field/content/value",
] as const;
export const INPUT_LIVE_V82_RESTORE_PARENT_ROLES = [
  "input-field/surface",
  "input-field/content-row",
] as const;

export interface InputLiveV82RestoreWriter {
  pageId: string;
  runIdentity: string;
  setIds: readonly [string, string];
}

export interface InputLiveV82RestorePayload {
  pageId: string;
  setIds: string[];
  restoredCount: 256;
  fixedBefore: number;
  ...
  marker: typeof INPUT_LIVE_V82_MEASURE_VISIBLE_FILL_MARKER;
}

export function buildInputLiveV82RestoreProgram(
  writer: InputLiveV82RestoreWriter,
): string {
  if (writer.setIds.length !== 2)
    throw new TypeError("Input live v82 restore requires two owned sets");
  return String.raw`
await figma.loadAllPagesAsync();
const V82_NS=${JSON.stringify(INPUT_LIVE_V82_NAMESPACE)};
const V82_PAGE_ID=${JSON.stringify(writer.pageId)};
const V82_SET_IDS=${JSON.stringify([...writer.setIds])};
const V82_ROLES=new Set(${JSON.stringify([...INPUT_LIVE_V82_RESTORE_CONTENT_ROLES])});
const V82_PARENTS=new Set(${JSON.stringify([...INPUT_LIVE_V82_RESTORE_PARENT_ROLES])});
const V82_MARKER=${JSON.stringify(INPUT_LIVE_V82_MEASURE_VISIBLE_FILL_MARKER)};
const page=await figma.getNodeByIdAsync(V82_PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("INPUT-V81-RESTORE-PAGE");
if(page.getSharedPluginData(V82_NS,"pageOwner")!=="recipe/input-field/"+${JSON.stringify(writer.runIdentity)})throw new Error("INPUT-V81-RESTORE-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>V82_SET_IDS.includes(node.id));
...
for(const set of sets){
  for(const component of set.children){
    ...
    for(const descendant of component.findAllWithCriteria({types:["TEXT","FRAME"]})){
      ...
      if(descendant.type==="FRAME"&&V82_PARENTS.has(role))parents.push(descendant);
      if(descendant.type==="TEXT"&&V82_ROLES.has(role))texts.push({setId:set.id,componentName:component.name,role,node:descendant});
    }
  }
}
...
for(const entry of texts){
  ...
  if(node.width<=0||node.height<=0)throw new Error(V82_MARKER+":"+entry.role);
...
export function validateInputLiveV82RestorePayload(
  payload: unknown,
  writer: InputLiveV82RestoreWriter,
): InputLiveV82RestorePayload {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("Input live v82 restore payload must be an object");
  ...
  if (
    ...
    value.marker !== INPUT_LIVE_V82_MEASURE_VISIBLE_FILL_MARKER
  )
    throw new TypeError(
      "Input live v82 restore did not re-assert content FILL",
    );
  return value as InputLiveV82RestorePayload;
}
