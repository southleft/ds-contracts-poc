...
import {
  assertTableLiveV32RootProofs,
  TABLE_LIVE_V32_CAPTURE_COUNT,
  TABLE_LIVE_V32_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  TABLE_LIVE_V32_RESPONSE_CONTRACTS,
  type TableLiveV32RootProof,
} from "./table-live-v32-contract.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V32_BROKER_VERSION = "table-live-v32-broker-v1";
export const TABLE_LIVE_V32_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const TABLE_LIVE_V32_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const TABLE_LIVE_V32_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const TABLE_LIVE_V32_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const TABLE_LIVE_V32_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const TABLE_LIVE_V32_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type TableLiveV32RemotePhase =
  (typeof TABLE_LIVE_V32_REMOTE_PHASES)[number];
export type TableLiveV32AuthorizationMode = "live" | "simulated";

export interface TableLiveV32CommitPins {
...
export interface TableLiveV32TransactionAuthorization extends TableLiveV32CommitPins {
  mode: TableLiveV32AuthorizationMode;
...
export interface TableLiveV32Manifest {
  artifactVersion: "table-live-v32-broker-manifest-v2";
  brokerVersion: typeof TABLE_LIVE_V32_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof TABLE_LIVE_V32_DYNAMIC_TOOL;
  target: typeof TABLE_LIVE_V32_TARGET;
  authorization: TableLiveV32TransactionAuthorization;
...
export interface TableLiveV32ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface TableLiveV32RequestBody {
  artifactVersion: "table-live-v32-broker-request-v2";
  brokerVersion: typeof TABLE_LIVE_V32_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: TableLiveV32RemotePhase;
  sequence: number;
  ...
  commitPins: TableLiveV32CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof TABLE_LIVE_V32_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: TableLiveV32ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface TableLiveV32Request extends TableLiveV32RequestBody {
...
export interface TableLiveV32Receipt {
  artifactVersion: "table-live-v32-broker-receipt-v2";
  brokerVersion: typeof TABLE_LIVE_V32_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: TableLiveV32RemotePhase;
...
export interface TableLiveV32AcceptedResponse<T = unknown> {
  request: TableLiveV32Request;
  receipt: TableLiveV32Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface TableLiveV32IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: TableLiveV32ResponseContract;
...
const equalTarget = (value: unknown): value is typeof TABLE_LIVE_V32_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === TABLE_LIVE_V32_TARGET.fileKey &&
  (value as BrokerTarget).fileName === TABLE_LIVE_V32_TARGET.fileName &&
  (value as BrokerTarget).editorType === TABLE_LIVE_V32_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== TABLE_LIVE_V32_TARGET.fileKey ||
    context.fileName !== TABLE_LIVE_V32_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === TABLE_LIVE_V32_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Table live v32 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= TABLE_LIVE_V32_CAPTURE_COUNT
  )
    throw new TypeError("Table live v32 capture index must be 0..19");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Table live v32 authorization pins are malformed");
};

const readManifest = (directory: string): TableLiveV32Manifest => {
  const manifest = json<TableLiveV32Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Table live v32 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "table-live-v32-broker-manifest-v2" ||
    manifest.brokerVersion !== TABLE_LIVE_V32_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== TABLE_LIVE_V32_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V32_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Table live v32 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: TableLiveV32Request,
): TableLiveV32RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: TableLiveV32RemotePhase,
  captureIndex?: number,
): TableLiveV32Request | undefined => {
  const file = tableLiveV2RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV32Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: TableLiveV32RemotePhase,
  captureIndex?: number,
): TableLiveV32Receipt | undefined => {
  const file = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV32Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: TableLiveV32Request | undefined;
  receipt: TableLiveV32Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: TableLiveV32RemotePhase,
): TableLiveV32ResponseContract => ({
  ...TABLE_LIVE_V32_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: TABLE_LIVE_V32_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "table-live-v32-plugin-response-v2",
  brokerVersion: TABLE_LIVE_V32_BROKER_VERSION,
  target: TABLE_LIVE_V32_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*TABLE_LIVE_V32_PROGRAM_START*/${program}/*TABLE_LIVE_V32_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*TABLE_LIVE_V32_PROGRAM_START*/";
  const endMarker = "/*TABLE_LIVE_V32_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Table live v32 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createTableLiveV32Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Table live v32 private signing key does not match authorized public-key identity",
    );
  const manifest: TableLiveV32Manifest = {
    artifactVersion: "table-live-v32-broker-manifest-v2",
    brokerVersion: TABLE_LIVE_V32_BROKER_VERSION,
    ...
    captureRequests: TABLE_LIVE_V32_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateTableLiveV32Request(
  directory: string,
  request: TableLiveV32Request,
): void {
  ...
  if (
    request.artifactVersion !== "table-live-v32-broker-request-v2" ||
    request.brokerVersion !== TABLE_LIVE_V32_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== TABLE_LIVE_V32_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Table live v32 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueTableLiveV32Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: TableLiveV32RemotePhase,
  program: string,
  options: TableLiveV32IssueOptions = {},
): TableLiveV32Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<TableLiveV32Request>(targetPath);
    validateTableLiveV32Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Table live v32 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(tableLiveV2ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Table live v32 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Table live v32 empty code envelope refused");
  if (phase === "capture") validateTableLiveV32TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Table live v32 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: TABLE_LIVE_V32_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? TABLE_LIVE_V32_SIGNED_WRITER_TIMEOUT_MS
        : TABLE_LIVE_V32_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: TableLiveV32RequestBody = {
    artifactVersion: "table-live-v32-broker-request-v2",
    brokerVersion: TABLE_LIVE_V32_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: TableLiveV32Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateTableLiveV32Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistTableLiveV32RawResponse(
  directory: string,
  phase: TableLiveV32RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Table live v32 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<TableLiveV32Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Table live v32 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = tableLiveV2ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Table live v32 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<TableLiveV32Receipt, "receiptSha256"> = {
    artifactVersion: "table-live-v32-broker-receipt-v2",
    brokerVersion: TABLE_LIVE_V32_BROKER_VERSION,
...
export function acceptTableLiveV32Response<T = unknown>(
  directory: string,
  phase: TableLiveV32RemotePhase,
  options: {
    captureIndex?: number;
...
): TableLiveV32AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Table live v32 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<TableLiveV32Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Table live v32 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "table-live-v32-plugin-response-v2" ||
    plugin?.brokerVersion !== TABLE_LIVE_V32_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Table live v32 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistTableLiveV32TechnicalGates(
  directory: string,
  rootProofs: readonly TableLiveV32RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertTableLiveV32RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Table live v32 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Table live v32 probe payload hash malformed");
  const gates = Object.fromEntries(
    TABLE_LIVE_V32_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "table-live-v32-host-technical-gates-v2",
    brokerVersion: TABLE_LIVE_V32_BROKER_VERSION,
...
export function validateTableLiveV32TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "table-live-v32-host-technical-gates-v2" ||
    ...
    TABLE_LIVE_V32_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Table live v32 technical gates artifact");
}

export function validateTableLiveV32Receipt(
  directory: string,
  phase: TableLiveV32RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateTableLiveV32Request(directory, request);
  const receipt = json<TableLiveV32Receipt>(
  ...
  if (
    receipt.artifactVersion !== "table-live-v32-broker-receipt-v2" ||
    receipt.brokerVersion !== TABLE_LIVE_V32_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Table live v32 accepted receipt: ${request.requestId}`,
    );
}

export function readTableLiveV32AcceptedRawPayload(
  directory: string,
  phase: TableLiveV32RemotePhase,
  captureIndex?: number,
): unknown {
  validateTableLiveV32Receipt(directory, phase, captureIndex);
...
export function createTableLiveV32FakeDynamicResponse(
  request: TableLiveV32Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? TABLE_LIVE_V32_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? TABLE_LIVE_V32_TARGET,
    ...overrides.extra,
  };
}

export function listTableLiveV32PersistedRequests(
  directory: string,
): TableLiveV32Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<TableLiveV32Request>(file)] : [];
    });
}
