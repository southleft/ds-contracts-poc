...
import {
  ...
  verifyComboboxLiveV30Authorization,
  type ComboboxLiveV30AntecedentIndex,
  type ComboboxLiveV30AuthorizationProof,
} from "./combobox-live-v30-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V30_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v30/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V30_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v30-security-attestation.json";

export interface ComboboxLiveV30SecurityAttestation {
...
export interface ComboboxLiveV30PreflightReport {
  artifactVersion: "combobox-live-v30-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV30SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV30SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV30AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v30-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V30_TARGET) ||
...
export function validateComboboxLiveV30ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV30Authorization");
  const preflight = source.indexOf("runComboboxLiveV30Preflight");
  const initialize = source.indexOf("ComboboxLiveV30Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV30Preflight(
  root: string,
  proof: ComboboxLiveV30AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V30_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV30PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V30_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V30_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V30_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV30AntecedentIndex;
  failures.push(...validateComboboxLiveV30AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V30_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V30_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v30-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV30ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v30 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV30SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v30-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V30_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV30Preflight(
    process.cwd(),
    verifyComboboxLiveV30Authorization(),
    attestationPath,
