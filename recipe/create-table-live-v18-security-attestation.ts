...
import { verifyTableLiveV18Authorization } from "./table-live-v18-authorization.js";
import { TABLE_LIVE_V18_TARGET } from "./table-live-v18-broker.js";
import {
  TABLE_LIVE_V18_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV18Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-table-v18-session";
...
const attestation: TableLiveV18SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV18SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v18 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V18_SECURITY_ATTESTATION_DEFAULT_PATH,
);
