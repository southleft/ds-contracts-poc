...
import { COMBOBOX_LIVE_V38_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v38.js";
import { verifyComboboxLiveV38Authorization } from "./combobox-live-v38-authorization.js";
import { runComboboxLiveV38Preflight } from "./combobox-live-v38-preflight.js";
import { ComboboxLiveV38Orchestrator } from "./run-combobox-live-v38.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v38 requires exact authorization artifact ${COMBOBOX_LIVE_V38_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v38-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV38Authorization(root);
    runComboboxLiveV38Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV38Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV38Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
