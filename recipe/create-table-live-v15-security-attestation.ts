...
import { verifyTableLiveV15Authorization } from "./table-live-v15-authorization.js";
import { TABLE_LIVE_V15_TARGET } from "./table-live-v15-broker.js";
import {
  TABLE_LIVE_V15_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV15Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-table-v15-session";
...
const attestation: TableLiveV15SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV15SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v15 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V15_SECURITY_ATTESTATION_DEFAULT_PATH,
);
