...
import {
  acceptComboboxLiveV40Response,
  createComboboxLiveV40Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV40Request,
  persistComboboxLiveV40RawResponse,
  persistComboboxLiveV40TechnicalGates,
  readComboboxLiveV40AcceptedRawPayload,
  type ComboboxLiveV40Request,
  type ComboboxLiveV40TransactionAuthorization,
} from "./combobox-live-v40-broker.js";
import {
  COMBOBOX_LIVE_V40_CAPTURE_COUNT,
  assertComboboxLiveV40CaptureResponses,
  ...
  evaluateComboboxLiveV40Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV40Roots,
  validateComboboxLiveV40CaptureManifest,
  validateComboboxLiveV40CapturePayload,
  validateComboboxLiveV40CleanupPayload,
  validateComboboxLiveV40ExtractPayload,
  validateComboboxLiveV40ProbePayload,
  validateComboboxLiveV40WriterPayload,
  type ComboboxLiveV40CaptureCell,
  type ComboboxLiveV40CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V40_PLAN_PATH,
  type ComboboxLiveV40ProofPlan,
} from "./build-combobox-live-proof-v40.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v40.js";

export const COMBOBOX_LIVE_V40_RUNNER_VERSION = "combobox-live-v40-runner-v1";

interface ComboboxLiveV40JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV40OrchestratorState {
  artifactVersion: "combobox-live-v40-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV40JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV40ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV40CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV40SourceIdentity & {
...
export interface ComboboxLiveV40NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV40OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v40 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V40_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V40_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV40ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV40CaptureCell[];
  validateComboboxLiveV40CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v40 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v40 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV40WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV40RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV40Request =>
  readJson<ComboboxLiveV40Request>(file);

export class ComboboxLiveV40Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV40Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV40OrchestratorState = {
      artifactVersion: "combobox-live-v40-orchestrator-state-v1",
    ...
    return new ComboboxLiveV40Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV40OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV40Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV40NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV40NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v40 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV40NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v40 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV40RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV40Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV40CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV40AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV40WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV40Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV40ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV40Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV40Response(this.#directory, "writer", {
        validate: validateComboboxLiveV40WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV40Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV40RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV40Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV40ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV40Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV40RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v40-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV40Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV40ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV40Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV40ProbePayload(payload, ownership),
      });
      persistComboboxLiveV40TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v40 capture cell missing");
      const accepted = acceptComboboxLiveV40Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV40CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V40_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV40CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV40Objective(
    ...
    throw new Error(`Combobox live v40 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV40Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV40CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV40Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV40RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV40Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV40CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV40CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV40CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV40Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV40TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV40SimulationKey = (): KeyObject =>
