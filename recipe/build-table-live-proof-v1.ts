...
export const TABLE_LIVE_V1_EVIDENCE_ROOT = "recipe/evidence/table-live-pivot-v1";
export const TABLE_LIVE_V1_PROTOCOL_PATH = `${TABLE_LIVE_V1_EVIDENCE_ROOT}/protocol.json`;
export const TABLE_LIVE_V1_PLAN_PATH = `${TABLE_LIVE_V1_EVIDENCE_ROOT}/proof-plan.json`;
export const TABLE_LIVE_V1_CAPTURE_MANIFEST_PATH = `${TABLE_LIVE_V1_EVIDENCE_ROOT}/capture-manifest.json`;
export const TABLE_LIVE_V1_REQUEST_MANIFEST_PATH = `${TABLE_LIVE_V1_EVIDENCE_ROOT}/request-manifest.json`;
export const TABLE_LIVE_V1_INDEX_PATH = `${TABLE_LIVE_V1_EVIDENCE_ROOT}/antecedent-index.json`;
export const TABLE_LIVE_V1_AUTHORIZATION_TEMPLATE_PATH = `${TABLE_LIVE_V1_EVIDENCE_ROOT}/authorization-template.json`;
export const TABLE_LIVE_V1_AUTHORIZATION_PATH = `${TABLE_LIVE_V1_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface TableLiveV1ProofPlan {
  artifactVersion: "table-live-v1-proof-plan-v1";
...
const captureCells = (
...
): TableLiveV1CaptureCell[] => {
  const cells: TableLiveV1CaptureCell[] = [];
  ...
  if (cells.length !== TABLE_LIVE_V1_CAPTURE_COUNT)
    throw new Error(
      `Table live v1 capture cells ${cells.length}, expected ${TABLE_LIVE_V1_CAPTURE_COUNT}`,
    );
  tableLiveV1CaptureManifestSha256(cells);
  return cells;
};

const protocol = (expectedFacts: number) => ({
  artifactVersion: "table-live-v1-external-operator-protocol-v1",
  protocolId: "table-live-v1",
  status:
  ...
  denominator: {
    ...
    sets: 6,
    variants: TABLE_LIVE_V1_VARIANT_COUNT,
    captureCells: TABLE_LIVE_V1_CAPTURE_COUNT,
...
export async function buildTableLiveV1Proof(
  check = process.argv.includes("--check"),
): Promise<TableLiveV1ProofPlan> {
  ...
  if (
    ...
    throw new Error("Table live v1 writer reused Input or Combobox identity");
  const captures = captureCells(sources);
  const writerProgram = Buffer.from(writer.code);
  const writerOwnershipBlueprint: TableLiveV1WriterOwnership = {
  ...
  const identities: TableLiveV1SourceIdentity[] = sources.map((source) => ({
    source: source.source,
    adapterIdentity: source.adapterIdentity,
    recipeHash: source.recipeHash,
    envelopeHash: source.envelope.integrity.canonicalHash,
  ...
  const restoreBlueprint = Buffer.from(
    buildTableLiveV1RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildTableLiveV1ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildTableLiveV1ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildTableLiveV1CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );
  const cleanupBlueprint = Buffer.from(
    buildTableLiveV1CleanupProgram(writerOwnershipBlueprint),
  );
  ...
  const sourceMetadata = sources.map((source) => {
    ...
    const tablePath = `${TABLE_LIVE_V1_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-table.json.gz`;
    const rowPath = `${TABLE_LIVE_V1_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-row.json.gz`;
    const cellPath = `${TABLE_LIVE_V1_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-cell.json.gz`;
  ...
  antecedentOutputs.set(
    TABLE_LIVE_V1_PROTOCOL_PATH,
    jsonBytes(protocol(expectedFacts)),
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V1_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V1_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V1_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V1_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V1_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V1_EVIDENCE_ROOT}/programs/cleanup-blueprint.js`,
    cleanupBlueprint,
  );

  const captureManifest = {
    artifactVersion: "table-live-v1-capture-manifest-v1",
    ...
    cells: captures,
    cellsSha256: tableLiveV1CaptureManifestSha256(captures),
  };
  ...
  const proofPlan: TableLiveV1ProofPlan = {
    artifactVersion: "table-live-v1-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: TABLE_LIVE_V1_TARGET,
    namespace: TABLE_FIGMA_NAMESPACE,
    writer: {
      programPath: `${TABLE_LIVE_V1_EVIDENCE_ROOT}/programs/writer.txt`,
    ...
    captureManifest: {
      path: TABLE_LIVE_V1_CAPTURE_MANIFEST_PATH,
      cells: 20,
      requests: 20,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(TABLE_LIVE_V1_PLAN_PATH, jsonBytes(proofPlan));

  const requests = {
    artifactVersion: "table-live-v1-request-manifest-v1",
    expectedDynamicTool: TABLE_LIVE_V1_DYNAMIC_TOOL,
    target: TABLE_LIVE_V1_TARGET,
    ...
    requests: [
      {
        requestId: "writer",
        sequence: tableLiveV1RequestSequence("writer"),
        phase: "writer",
      },
      {
        requestId: "cleanup",
        sequence: tableLiveV1RequestSequence("cleanup"),
        phase: "cleanup",
        availability: "persisted immediately after writer acceptance",
      },
      {
        requestId: "restore",
        sequence: tableLiveV1RequestSequence("restore"),
        phase: "restore",
      },
      {
        requestId: "extract",
        sequence: tableLiveV1RequestSequence("extract"),
        phase: "extract",
      },
      {
        requestId: "probe",
        sequence: tableLiveV1RequestSequence("probe"),
        phase: "probe",
      },
      ...captures.map((cell) => ({
        requestId: `capture-${String(cell.index).padStart(3, "0")}`,
        sequence: tableLiveV1RequestSequence("capture", cell.index),
        phase: "capture",
  ...
  if (requests.requestCount !== TABLE_LIVE_V1_REMOTE_REQUESTS)
    throw new Error("Table live v1 remote request denominator drifted");
  antecedentOutputs.set(
    TABLE_LIVE_V1_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const lifecycleExcludedPaths = [
    TABLE_LIVE_V1_AUTHORIZATION_TEMPLATE_PATH,
    ...
    "recipe/table-live-v1-authorized.ts",
    "recipe/write-table-live-v1-authorization.ts",
    "recipe/create-table-live-v1-security-attestation.ts",
  ...
  const indexedPaths = [
    ...
    "recipe/table-live-v1-broker.ts",
    "recipe/table-live-v1-contract.ts",
    "recipe/table-live-v1-restore.ts",
    "recipe/table-live-v1-cleanup.ts",
    "recipe/table-live-v1-verifier.ts",
    "recipe/table-live-v1-fixed-point.ts",
    "recipe/run-table-live-v1.ts",
    "recipe/build-table-live-proof-v1.ts",
    "recipe/table-live-v1-lifecycle-simulation.ts",
    ...
    "recipe/normalize.ts",
  ].filter((artifactPath) => artifactPath !== TABLE_LIVE_V1_INDEX_PATH);
  ...
  antecedentOutputs.set(TABLE_LIVE_V1_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "table-live-v1-authorization-template-v1",
  ...
  const attestationTemplate = jsonBytes({
    artifactVersion: "table-live-v1-operator-security-attestation-template-v1",
    status: "template only; complete privately after PREPARE+AUTHORIZE",
    tokenValuesForbidden: true,
    privateDirectory: "private/table-live-v1-security-attestation.json",
  });
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Table live v1 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(TABLE_LIVE_V1_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(TABLE_LIVE_V1_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
    writeFileSync(
      `${TABLE_LIVE_V1_EVIDENCE_ROOT}/operator-security-attestation-template.json`,
      attestationTemplate,
    );
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(TABLE_LIVE_V1_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(TABLE_LIVE_V1_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Table live v1 authorization template drift");
  }
  if (
    writer.namespace !== TABLE_FIGMA_NAMESPACE ||
    writer.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    ...
    throw new Error("Table live v1 identity or cleanup invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildTableLiveV1Proof(), null, 2)}\n`,
  );
