/**
 * Calendar live v30 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V30_FIXED_VERIFIER_VERSION = "calendar-live-v30-fixed-point-v1";
export const CALENDAR_LIVE_V30_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV30FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV30FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV30FixedSourceProbe {
...
export interface CalendarLiveV30FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV30FixedFixedPoint;
}

export interface CalendarLiveV30FixedHardGateInput {
  thresholds: CalendarLiveV30FixedThresholds;
  sourceProbes: CalendarLiveV30FixedSourceProbe[];
  cells: CalendarLiveV30FixedCellValidation[];
  sceneProofs: CalendarLiveV30FixedSceneProof[];
...
export interface CalendarLiveV30FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V30_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV30FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV30FixedHardGates(
  input: CalendarLiveV30FixedHardGateInput,
): CalendarLiveV30FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV30FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V30_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV30FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV30FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV30FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
