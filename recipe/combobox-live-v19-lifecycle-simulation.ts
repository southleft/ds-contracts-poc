...
export interface ComboboxLiveV19LifecycleSimulationReport {
  artifactVersion: "combobox-live-v19-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV19AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV19Lifecycle(): Promise<ComboboxLiveV19LifecycleSimulationReport> {
  await buildComboboxLiveV19Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V19_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV19AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v19-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V19_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V19_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV19History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV19History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV19AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V19_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V19_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV19History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV19History(authorized, "pending");
    ...
    await buildComboboxLiveV19Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V19_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV19AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV19Lifecycle(), null, 2)}\n`,
  );
