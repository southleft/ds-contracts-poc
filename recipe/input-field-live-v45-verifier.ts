...
export const INPUT_LIVE_V45_VERIFIER_VERSION = "input-live-v45-verifier-v1";

export function buildInputLiveV45RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV45RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV45RawNode[];
}

export interface InputLiveV45NormalizedScene {
...
export interface InputLiveV45SceneProof {
...
export function normalizeInputLiveV45Scene(
  root: InputLiveV45RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV45NormalizedScene {
  const canonicalBindings: InputLiveV45NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV45RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV45Scene<Instance>(
  normalized: InputLiveV45NormalizedScene,
...
export function compileInputLiveV45ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV45PreCaptureGates(payload: unknown): void {
