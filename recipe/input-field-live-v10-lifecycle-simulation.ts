...
export interface InputLiveV10LifecycleSimulationReport {
  artifactVersion: "input-live-v10-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV10AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV10Lifecycle(): Promise<InputLiveV10LifecycleSimulationReport> {
  await buildInputLiveV10Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V10_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV10AntecedentIndex;
  const indexSha256 = inputLiveV10Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v10-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V10_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V10_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV10History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV10History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV10AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V10_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V10_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV10History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV10History(authorized, "pending");
    ...
    await buildInputLiveV10Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V10_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV10AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV10Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV10Lifecycle(), null, 2)}\n`,
  );
