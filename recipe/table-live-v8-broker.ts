...
import {
  assertTableLiveV8RootProofs,
  TABLE_LIVE_V8_CAPTURE_COUNT,
  TABLE_LIVE_V8_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  TABLE_LIVE_V8_RESPONSE_CONTRACTS,
  type TableLiveV8RootProof,
} from "./table-live-v8-contract.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V8_BROKER_VERSION = "table-live-v8-broker-v1";
export const TABLE_LIVE_V8_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const TABLE_LIVE_V8_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const TABLE_LIVE_V8_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const TABLE_LIVE_V8_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const TABLE_LIVE_V8_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const TABLE_LIVE_V8_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type TableLiveV8RemotePhase =
  (typeof TABLE_LIVE_V8_REMOTE_PHASES)[number];
export type TableLiveV8AuthorizationMode = "live" | "simulated";

export interface TableLiveV8CommitPins {
...
export interface TableLiveV8TransactionAuthorization extends TableLiveV8CommitPins {
  mode: TableLiveV8AuthorizationMode;
...
export interface TableLiveV8Manifest {
  artifactVersion: "table-live-v8-broker-manifest-v2";
  brokerVersion: typeof TABLE_LIVE_V8_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof TABLE_LIVE_V8_DYNAMIC_TOOL;
  target: typeof TABLE_LIVE_V8_TARGET;
  authorization: TableLiveV8TransactionAuthorization;
...
export interface TableLiveV8ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface TableLiveV8RequestBody {
  artifactVersion: "table-live-v8-broker-request-v2";
  brokerVersion: typeof TABLE_LIVE_V8_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: TableLiveV8RemotePhase;
  sequence: number;
  ...
  commitPins: TableLiveV8CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof TABLE_LIVE_V8_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: TableLiveV8ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface TableLiveV8Request extends TableLiveV8RequestBody {
...
export interface TableLiveV8Receipt {
  artifactVersion: "table-live-v8-broker-receipt-v2";
  brokerVersion: typeof TABLE_LIVE_V8_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: TableLiveV8RemotePhase;
...
export interface TableLiveV8AcceptedResponse<T = unknown> {
  request: TableLiveV8Request;
  receipt: TableLiveV8Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface TableLiveV8IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: TableLiveV8ResponseContract;
...
const equalTarget = (value: unknown): value is typeof TABLE_LIVE_V8_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === TABLE_LIVE_V8_TARGET.fileKey &&
  (value as BrokerTarget).fileName === TABLE_LIVE_V8_TARGET.fileName &&
  (value as BrokerTarget).editorType === TABLE_LIVE_V8_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== TABLE_LIVE_V8_TARGET.fileKey ||
    context.fileName !== TABLE_LIVE_V8_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === TABLE_LIVE_V8_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Table live v8 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= TABLE_LIVE_V8_CAPTURE_COUNT
  )
    throw new TypeError("Table live v8 capture index must be 0..19");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Table live v8 authorization pins are malformed");
};

const readManifest = (directory: string): TableLiveV8Manifest => {
  const manifest = json<TableLiveV8Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Table live v8 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "table-live-v8-broker-manifest-v2" ||
    manifest.brokerVersion !== TABLE_LIVE_V8_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== TABLE_LIVE_V8_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V8_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Table live v8 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: TableLiveV8Request,
): TableLiveV8RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: TableLiveV8RemotePhase,
  captureIndex?: number,
): TableLiveV8Request | undefined => {
  const file = tableLiveV2RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV8Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: TableLiveV8RemotePhase,
  captureIndex?: number,
): TableLiveV8Receipt | undefined => {
  const file = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV8Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: TableLiveV8Request | undefined;
  receipt: TableLiveV8Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: TableLiveV8RemotePhase,
): TableLiveV8ResponseContract => ({
  ...TABLE_LIVE_V8_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: TABLE_LIVE_V8_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: TABLE_LIVE_V8_DYNAMIC_TOOL,
      target: TABLE_LIVE_V8_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "table-live-v8-plugin-response-v2",
  brokerVersion: TABLE_LIVE_V8_BROKER_VERSION,
  target: TABLE_LIVE_V8_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*TABLE_LIVE_V8_PROGRAM_START*/${program}/*TABLE_LIVE_V8_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*TABLE_LIVE_V8_PROGRAM_START*/";
  const endMarker = "/*TABLE_LIVE_V8_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Table live v8 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createTableLiveV8Transaction(
  directory: string,
  ...
  if (existsSync(path.join(directory, MANIFEST_FILE)))
    throw new Error("Table live v8 transaction already exists");
  validateAuthorization(options.authorization);
  if (
    !SHA256.test(options.proofPlanSha256) ||
    !SHA256.test(options.captureManifestSha256)
  )
    throw new TypeError("Table live v8 transaction plan hashes malformed");
  const attempt = options.attempt ?? 1;
  if (!Number.isInteger(attempt) || attempt < 1 || attempt > 3)
    throw new TypeError("Table live v8 attempt must be 1..3");
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Table live v8 private signing key does not match authorized public-key identity",
    );
  const manifest: TableLiveV8Manifest = {
    artifactVersion: "table-live-v8-broker-manifest-v2",
    brokerVersion: TABLE_LIVE_V8_BROKER_VERSION,
    ...
    captureRequests: TABLE_LIVE_V8_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateTableLiveV8Request(
  directory: string,
  request: TableLiveV8Request,
): void {
  ...
  if (
    request.artifactVersion !== "table-live-v8-broker-request-v2" ||
    request.brokerVersion !== TABLE_LIVE_V8_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== TABLE_LIVE_V8_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Table live v8 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueTableLiveV8Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: TableLiveV8RemotePhase,
  program: string,
  options: TableLiveV8IssueOptions = {},
): TableLiveV8Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<TableLiveV8Request>(targetPath);
    validateTableLiveV8Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Table live v8 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(tableLiveV2ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Table live v8 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Table live v8 empty code envelope refused");
  if (phase === "capture") validateTableLiveV8TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Table live v8 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: TABLE_LIVE_V8_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? TABLE_LIVE_V8_SIGNED_WRITER_TIMEOUT_MS
        : TABLE_LIVE_V8_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: TableLiveV8RequestBody = {
    artifactVersion: "table-live-v8-broker-request-v2",
    brokerVersion: TABLE_LIVE_V8_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: TableLiveV8Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateTableLiveV8Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistTableLiveV8RawResponse(
  directory: string,
  phase: TableLiveV8RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Table live v8 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<TableLiveV8Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Table live v8 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = tableLiveV2ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Table live v8 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<TableLiveV8Receipt, "receiptSha256"> = {
    artifactVersion: "table-live-v8-broker-receipt-v2",
    brokerVersion: TABLE_LIVE_V8_BROKER_VERSION,
...
export function acceptTableLiveV8Response<T = unknown>(
  directory: string,
  phase: TableLiveV8RemotePhase,
  options: {
    captureIndex?: number;
...
): TableLiveV8AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Table live v8 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<TableLiveV8Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Table live v8 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "table-live-v8-plugin-response-v2" ||
    plugin?.brokerVersion !== TABLE_LIVE_V8_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Table live v8 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistTableLiveV8TechnicalGates(
  directory: string,
  rootProofs: readonly TableLiveV8RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertTableLiveV8RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Table live v8 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Table live v8 probe payload hash malformed");
  const gates = Object.fromEntries(
    TABLE_LIVE_V8_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "table-live-v8-host-technical-gates-v2",
    brokerVersion: TABLE_LIVE_V8_BROKER_VERSION,
...
export function validateTableLiveV8TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "table-live-v8-host-technical-gates-v2" ||
    ...
    TABLE_LIVE_V8_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Table live v8 technical gates artifact");
}

export function validateTableLiveV8Receipt(
  directory: string,
  phase: TableLiveV8RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateTableLiveV8Request(directory, request);
  const receipt = json<TableLiveV8Receipt>(
  ...
  if (
    receipt.artifactVersion !== "table-live-v8-broker-receipt-v2" ||
    receipt.brokerVersion !== TABLE_LIVE_V8_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Table live v8 accepted receipt: ${request.requestId}`,
    );
}

export function readTableLiveV8AcceptedRawPayload(
  directory: string,
  phase: TableLiveV8RemotePhase,
  captureIndex?: number,
): unknown {
  validateTableLiveV8Receipt(directory, phase, captureIndex);
...
export function createTableLiveV8FakeDynamicResponse(
  request: TableLiveV8Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? TABLE_LIVE_V8_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? TABLE_LIVE_V8_TARGET,
    ...overrides.extra,
  };
}

export function listTableLiveV8PersistedRequests(
  directory: string,
): TableLiveV8Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<TableLiveV8Request>(file)] : [];
    });
}
