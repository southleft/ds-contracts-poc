...
import {
  validateComboboxLiveV25AntecedentIndex,
  verifyComboboxLiveV25Authorization,
  type ComboboxLiveV25AntecedentIndex,
  type ComboboxLiveV25AuthorizationProof,
} from "./combobox-live-v25-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V25_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v25/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V25_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v25-security-attestation.json";

export interface ComboboxLiveV25SecurityAttestation {
  artifactVersion: "combobox-live-v25-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV25PreflightReport {
  artifactVersion: "combobox-live-v25-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV25SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV25SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV25AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v25-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V25_TARGET) ||
...
export function validateComboboxLiveV25ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV25Authorization");
  const preflight = source.indexOf("runComboboxLiveV25Preflight");
  const initialize = source.indexOf("ComboboxLiveV25Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV25Preflight(
  root: string,
  proof: ComboboxLiveV25AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V25_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV25PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V25_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V25_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V25_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV25AntecedentIndex;
  failures.push(...validateComboboxLiveV25AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V25_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V25_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV25SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v25-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV25ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v25 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV25SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v25-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V25_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV25Preflight(
    process.cwd(),
    verifyComboboxLiveV25Authorization(),
    attestationPath,
