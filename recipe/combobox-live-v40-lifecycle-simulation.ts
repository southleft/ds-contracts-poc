...
export interface ComboboxLiveV40LifecycleSimulationReport {
  artifactVersion: "combobox-live-v40-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV40AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV40Lifecycle(): Promise<ComboboxLiveV40LifecycleSimulationReport> {
  await buildComboboxLiveV40Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V40_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV40AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v40-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V40_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V40_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV40History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV40History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV40AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V40_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V40_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV40History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV40History(authorized, "pending");
    ...
    await buildComboboxLiveV40Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V40_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV40AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV40Lifecycle(), null, 2)}\n`,
  );
