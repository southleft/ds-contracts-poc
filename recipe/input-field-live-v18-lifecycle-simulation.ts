...
export interface InputLiveV18LifecycleSimulationReport {
  artifactVersion: "input-live-v18-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV18AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV18Lifecycle(): Promise<InputLiveV18LifecycleSimulationReport> {
  await buildInputLiveV18Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V18_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV18AntecedentIndex;
  const indexSha256 = inputLiveV18Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v18-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V18_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V18_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV18History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV18History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV18AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V18_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V18_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV18History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV18History(authorized, "pending");
    ...
    await buildInputLiveV18Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V18_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV18AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV18Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV18Lifecycle(), null, 2)}\n`,
  );
