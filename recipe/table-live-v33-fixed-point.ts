/**
 * Table live v33 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V33_FIXED_VERIFIER_VERSION = "table-live-v33-fixed-point-v1";
export const TABLE_LIVE_V33_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV33FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV33FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV33FixedSourceProbe {
...
export interface TableLiveV33FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV33FixedFixedPoint;
}

export interface TableLiveV33FixedHardGateInput {
  thresholds: TableLiveV33FixedThresholds;
  sourceProbes: TableLiveV33FixedSourceProbe[];
  cells: TableLiveV33FixedCellValidation[];
  sceneProofs: TableLiveV33FixedSceneProof[];
...
export interface TableLiveV33FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V33_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV33FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV33FixedHardGates(
  input: TableLiveV33FixedHardGateInput,
): TableLiveV33FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV33FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V33_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV33FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV33FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
