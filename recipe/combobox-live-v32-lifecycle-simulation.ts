...
export interface ComboboxLiveV32LifecycleSimulationReport {
  artifactVersion: "combobox-live-v32-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV32AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV32Lifecycle(): Promise<ComboboxLiveV32LifecycleSimulationReport> {
  await buildComboboxLiveV32Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V32_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV32AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v32-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V32_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V32_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV32History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV32History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV32AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V32_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V32_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV32History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV32History(authorized, "pending");
    ...
    await buildComboboxLiveV32Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V32_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV32AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV32Lifecycle(), null, 2)}\n`,
  );
