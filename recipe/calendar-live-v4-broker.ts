...
import {
  assertCalendarLiveV4RootProofs,
  CALENDAR_LIVE_V4_CAPTURE_COUNT,
  CALENDAR_LIVE_V4_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  CALENDAR_LIVE_V4_RESPONSE_CONTRACTS,
  type CalendarLiveV4RootProof,
} from "./calendar-live-v4-contract.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V4_BROKER_VERSION = "calendar-live-v4-broker-v1";
export const CALENDAR_LIVE_V4_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const CALENDAR_LIVE_V4_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const CALENDAR_LIVE_V4_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const CALENDAR_LIVE_V4_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const CALENDAR_LIVE_V4_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const CALENDAR_LIVE_V4_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type CalendarLiveV4RemotePhase =
  (typeof CALENDAR_LIVE_V4_REMOTE_PHASES)[number];
export type CalendarLiveV4AuthorizationMode = "live" | "simulated";

export interface CalendarLiveV4CommitPins {
...
export interface CalendarLiveV4TransactionAuthorization extends CalendarLiveV4CommitPins {
  mode: CalendarLiveV4AuthorizationMode;
...
export interface CalendarLiveV4Manifest {
  artifactVersion: "calendar-live-v4-broker-manifest-v2";
  brokerVersion: typeof CALENDAR_LIVE_V4_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof CALENDAR_LIVE_V4_DYNAMIC_TOOL;
  target: typeof CALENDAR_LIVE_V4_TARGET;
  authorization: CalendarLiveV4TransactionAuthorization;
...
export interface CalendarLiveV4ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface CalendarLiveV4RequestBody {
  artifactVersion: "calendar-live-v4-broker-request-v2";
  brokerVersion: typeof CALENDAR_LIVE_V4_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: CalendarLiveV4RemotePhase;
  sequence: number;
  ...
  commitPins: CalendarLiveV4CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof CALENDAR_LIVE_V4_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: CalendarLiveV4ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface CalendarLiveV4Request extends CalendarLiveV4RequestBody {
...
export interface CalendarLiveV4Receipt {
  artifactVersion: "calendar-live-v4-broker-receipt-v2";
  brokerVersion: typeof CALENDAR_LIVE_V4_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: CalendarLiveV4RemotePhase;
...
export interface CalendarLiveV4AcceptedResponse<T = unknown> {
  request: CalendarLiveV4Request;
  receipt: CalendarLiveV4Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface CalendarLiveV4IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: CalendarLiveV4ResponseContract;
...
const equalTarget = (value: unknown): value is typeof CALENDAR_LIVE_V4_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === CALENDAR_LIVE_V4_TARGET.fileKey &&
  (value as BrokerTarget).fileName === CALENDAR_LIVE_V4_TARGET.fileName &&
  (value as BrokerTarget).editorType === CALENDAR_LIVE_V4_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== CALENDAR_LIVE_V4_TARGET.fileKey ||
    context.fileName !== CALENDAR_LIVE_V4_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === CALENDAR_LIVE_V4_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Calendar live v4 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= CALENDAR_LIVE_V4_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v4 capture index must be 0..7");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Calendar live v4 authorization pins are malformed");
};

const readManifest = (directory: string): CalendarLiveV4Manifest => {
  const manifest = json<CalendarLiveV4Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Calendar live v4 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "calendar-live-v4-broker-manifest-v2" ||
    manifest.brokerVersion !== CALENDAR_LIVE_V4_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== CALENDAR_LIVE_V4_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(CALENDAR_LIVE_V4_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Calendar live v4 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: CalendarLiveV4Request,
): CalendarLiveV4RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: CalendarLiveV4RemotePhase,
  captureIndex?: number,
): CalendarLiveV4Request | undefined => {
  const file = calendarLiveV1RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV4Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: CalendarLiveV4RemotePhase,
  captureIndex?: number,
): CalendarLiveV4Receipt | undefined => {
  const file = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV4Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: CalendarLiveV4Request | undefined;
  receipt: CalendarLiveV4Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: CalendarLiveV4RemotePhase,
): CalendarLiveV4ResponseContract => ({
  ...CALENDAR_LIVE_V4_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: CalendarLiveV4Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: CALENDAR_LIVE_V4_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: CALENDAR_LIVE_V4_DYNAMIC_TOOL,
      target: CALENDAR_LIVE_V4_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "calendar-live-v4-plugin-response-v2",
  brokerVersion: CALENDAR_LIVE_V4_BROKER_VERSION,
  target: CALENDAR_LIVE_V4_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*CALENDAR_LIVE_V4_PROGRAM_START*/${program}/*CALENDAR_LIVE_V4_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*CALENDAR_LIVE_V4_PROGRAM_START*/";
  const endMarker = "/*CALENDAR_LIVE_V4_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Calendar live v4 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createCalendarLiveV4Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Calendar live v4 private signing key does not match authorized public-key identity",
    );
  const manifest: CalendarLiveV4Manifest = {
    artifactVersion: "calendar-live-v4-broker-manifest-v2",
    brokerVersion: CALENDAR_LIVE_V4_BROKER_VERSION,
    ...
    captureRequests: CALENDAR_LIVE_V4_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateCalendarLiveV4Request(
  directory: string,
  request: CalendarLiveV4Request,
): void {
  ...
  if (
    request.artifactVersion !== "calendar-live-v4-broker-request-v2" ||
    request.brokerVersion !== CALENDAR_LIVE_V4_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== CALENDAR_LIVE_V4_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Calendar live v4 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueCalendarLiveV4Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: CalendarLiveV4RemotePhase,
  program: string,
  options: CalendarLiveV4IssueOptions = {},
): CalendarLiveV4Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<CalendarLiveV4Request>(targetPath);
    validateCalendarLiveV4Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Calendar live v4 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(calendarLiveV1ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Calendar live v4 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Calendar live v4 empty code envelope refused");
  if (phase === "capture") validateCalendarLiveV4TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Calendar live v4 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: CALENDAR_LIVE_V4_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? CALENDAR_LIVE_V4_SIGNED_WRITER_TIMEOUT_MS
        : CALENDAR_LIVE_V4_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: CalendarLiveV4RequestBody = {
    artifactVersion: "calendar-live-v4-broker-request-v2",
    brokerVersion: CALENDAR_LIVE_V4_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: CalendarLiveV4Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateCalendarLiveV4Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistCalendarLiveV4RawResponse(
  directory: string,
  phase: CalendarLiveV4RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Calendar live v4 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<CalendarLiveV4Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Calendar live v4 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = calendarLiveV1ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Calendar live v4 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): CalendarLiveV4Receipt => {
  const body: Omit<CalendarLiveV4Receipt, "receiptSha256"> = {
    artifactVersion: "calendar-live-v4-broker-receipt-v2",
    brokerVersion: CALENDAR_LIVE_V4_BROKER_VERSION,
...
export function acceptCalendarLiveV4Response<T = unknown>(
  directory: string,
  phase: CalendarLiveV4RemotePhase,
  options: {
    captureIndex?: number;
...
): CalendarLiveV4AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Calendar live v4 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<CalendarLiveV4Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Calendar live v4 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "calendar-live-v4-plugin-response-v2" ||
    plugin?.brokerVersion !== CALENDAR_LIVE_V4_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Calendar live v4 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistCalendarLiveV4TechnicalGates(
  directory: string,
  rootProofs: readonly CalendarLiveV4RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertCalendarLiveV4RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Calendar live v4 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Calendar live v4 probe payload hash malformed");
  const gates = Object.fromEntries(
    CALENDAR_LIVE_V4_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "calendar-live-v4-host-technical-gates-v2",
    brokerVersion: CALENDAR_LIVE_V4_BROKER_VERSION,
...
export function validateCalendarLiveV4TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "calendar-live-v4-host-technical-gates-v2" ||
    ...
    CALENDAR_LIVE_V4_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Calendar live v4 technical gates artifact");
}

export function validateCalendarLiveV4Receipt(
  directory: string,
  phase: CalendarLiveV4RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateCalendarLiveV4Request(directory, request);
  const receipt = json<CalendarLiveV4Receipt>(
  ...
  if (
    receipt.artifactVersion !== "calendar-live-v4-broker-receipt-v2" ||
    receipt.brokerVersion !== CALENDAR_LIVE_V4_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Calendar live v4 accepted receipt: ${request.requestId}`,
    );
}

export function readCalendarLiveV4AcceptedRawPayload(
  directory: string,
  phase: CalendarLiveV4RemotePhase,
  captureIndex?: number,
): unknown {
  validateCalendarLiveV4Receipt(directory, phase, captureIndex);
...
export function createCalendarLiveV4FakeDynamicResponse(
  request: CalendarLiveV4Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? CALENDAR_LIVE_V4_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? CALENDAR_LIVE_V4_TARGET,
    ...overrides.extra,
  };
}

export function listCalendarLiveV4PersistedRequests(
  directory: string,
): CalendarLiveV4Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<CalendarLiveV4Request>(file)] : [];
    });
}
