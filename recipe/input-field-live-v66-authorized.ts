...
import { INPUT_LIVE_V66_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v66.js";
import { verifyInputLiveV66Authorization } from "./input-field-live-v66-authorization.js";
import { runInputLiveV66Preflight } from "./input-field-live-v66-preflight.js";
import { InputLiveV66Orchestrator } from "./run-input-field-live-v66.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V66_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v66-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV66Authorization(root);
    runInputLiveV66Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV66Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
