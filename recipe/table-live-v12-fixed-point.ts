/**
 * Table live v12 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V12_FIXED_VERIFIER_VERSION = "table-live-v12-fixed-point-v1";
export const TABLE_LIVE_V12_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV12FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV12FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV12FixedSourceProbe {
...
export interface TableLiveV12FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV12FixedFixedPoint;
}

export interface TableLiveV12FixedHardGateInput {
  thresholds: TableLiveV12FixedThresholds;
  sourceProbes: TableLiveV12FixedSourceProbe[];
  cells: TableLiveV12FixedCellValidation[];
  sceneProofs: TableLiveV12FixedSceneProof[];
...
export interface TableLiveV12FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V12_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV12FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV12FixedHardGates(
  input: TableLiveV12FixedHardGateInput,
): TableLiveV12FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV12FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  if (input.visualRows.length !== 0)
    failures.push("table live v12 must not invent a visual comparison");
  ...
  return {
    verifierVersion: TABLE_LIVE_V12_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV12FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV12FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
