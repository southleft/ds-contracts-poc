...
import { buildTableLiveV13CleanupRuntime } from "./table-live-v13-cleanup.js";
import { type TableLiveV13FixedFixedPoint } from "./table-live-v13-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV13RestoreProgram,
  validateTableLiveV13RestorePayload,
  type TableLiveV13RestorePayload,
} from "./table-live-v13-restore.js";
import {
  buildTableLiveV13RawPropertyRuntime,
  normalizeTableLiveV13Scene,
  type TableLiveV13RawNode,
} from "./table-live-v13-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v13.js";
import {
...
export const TABLE_LIVE_V13_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V13_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V13_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V13_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V13_VARIANT_COUNT = 20;
export const TABLE_LIVE_V13_SET_COUNT = 6;
export const TABLE_LIVE_V13_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V13_HOST_PHASES = 3;
export const TABLE_LIVE_V13_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V13_RESTORE_COUNT = 8;
export const TABLE_LIVE_V13_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V13_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV13SourceId = (typeof TABLE_LIVE_V13_SOURCE_IDS)[number];
export type TableLiveV13Kind = "table" | "row" | "cell";

export interface TableLiveV13SourceIdentity {
  source: TableLiveV13SourceId;
...
export interface TableLiveV13WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV13ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV13SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV13RawNode;
    rowScene: TableLiveV13RawNode;
    cellScene: TableLiveV13RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV13RootProof {
  source: TableLiveV13SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV13FixedFixedPoint;
}

export interface TableLiveV13ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV13SourceId;
    adapterIdentity: string;
...
export interface TableLiveV13CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV13SourceId;
  adapterIdentity: string;
...
export interface TableLiveV13CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV13SourceId;
  frameWidth: number;
...
export interface TableLiveV13CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v13 ${label} must be an object`);
...
export function validateTableLiveV13WriterPayload(
  value: unknown,
): TableLiveV13WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V13_NAMESPACE ||
    ...
    throw new TypeError("Table live v13 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v13 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v13 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV13SourceIdentity,
  kind: TableLiveV13Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV13ExtractProgram(
  writer: TableLiveV13WriterOwnership,
  sources: readonly TableLiveV13SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v13 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V13_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V13_NAMESPACE)}
${buildTableLiveV13RawPropertyRuntime()}
...
export function validateTableLiveV13ExtractPayload(
  value: unknown,
  writer: TableLiveV13WriterOwnership,
): TableLiveV13ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v13 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V13_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v13 extract schema/two-root mismatch");
  return payload as TableLiveV13ExtractPayload;
}

export function proveTableLiveV13Roots<Instance>(
  extract: TableLiveV13ExtractPayload,
  sources: readonly (TableLiveV13SourceIdentity & {
...
): TableLiveV13RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v13 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v13 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV13Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV13Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV13Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV13FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v13 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v13 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v13 compile lost owned sets");
...
export function assertTableLiveV13RootProofs(
  proofs: readonly TableLiveV13RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v13 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v13 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV13ProbeProgram(
  writer: TableLiveV13WriterOwnership,
  sources: readonly TableLiveV13SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V13_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV13ProbePayload(
  value: unknown,
  writer: TableLiveV13WriterOwnership,
): TableLiveV13ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV13ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V13_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V13_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Table live v13 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV13CaptureManifest(
  cells: readonly TableLiveV13CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V13_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V13_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V13_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V13_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v13 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV13CaptureCell[],
): string => {
  validateTableLiveV13CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV13CaptureProgram(
  writer: TableLiveV13WriterOwnership,
  cell: TableLiveV13CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V13_CAPTURE_COUNT
  )
    throw new TypeError("Table live v13 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v13 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V13_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV13CapturePayload(
  value: unknown,
  cell: TableLiveV13CaptureCell,
  rawResponseBytes: number,
): TableLiveV13CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV13CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V13_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V13_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v13 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV13CaptureResponses(
  manifest: readonly TableLiveV13CaptureCell[],
  responses: readonly TableLiveV13CapturePayload[],
): void {
  validateTableLiveV13CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v13 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV13ObjectiveReport {
  artifactVersion: "table-live-v13-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV13Objective(
  manifest: readonly TableLiveV13CaptureCell[],
  responses: readonly TableLiveV13CapturePayload[],
): TableLiveV13ObjectiveReport {
  assertTableLiveV13CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v13-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV13CleanupProgram(
  writer: TableLiveV13WriterOwnership,
): string {
  const baseProgram = buildTableLiveV13CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V13_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v13 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV13CleanupPayload(
  payload: unknown,
  writer: TableLiveV13WriterOwnership,
): TableLiveV13CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV13CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v13 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V13_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV13WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
