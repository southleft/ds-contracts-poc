...
import {
  verifyComboboxLiveV5FixedSceneFixedPoint,
  type ComboboxLiveV5FixedFixedPoint,
} from "./combobox-live-v5-fixed-point.js";

export const COMBOBOX_LIVE_V5_VERIFIER_VERSION = "combobox-live-v5-verifier-v1";

export function buildComboboxLiveV5RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV5RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV5RawNode[];
}

export interface ComboboxLiveV5NormalizedScene {
...
export interface ComboboxLiveV5SceneProof {
...
export function normalizeComboboxLiveV5Scene(
  root: ComboboxLiveV5RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV5NormalizedScene {
  const canonicalBindings: ComboboxLiveV5NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV5RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV5Scene<Instance>(
  normalized: ComboboxLiveV5NormalizedScene,
...
): ComboboxLiveV5SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV5FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV5ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV5PreCaptureGates(payload: unknown): void {
