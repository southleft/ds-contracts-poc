...
import {
  acceptCalendarLiveV33Response,
  createCalendarLiveV33Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV33Request,
  persistCalendarLiveV33RawResponse,
  persistCalendarLiveV33TechnicalGates,
  readCalendarLiveV33AcceptedRawPayload,
  type CalendarLiveV33Request,
  type CalendarLiveV33TransactionAuthorization,
} from "./calendar-live-v33-broker.js";
import {
  CALENDAR_LIVE_V33_CAPTURE_COUNT,
  assertCalendarLiveV33CaptureResponses,
  ...
  evaluateCalendarLiveV33Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV33Roots,
  validateCalendarLiveV33CaptureManifest,
  validateCalendarLiveV33CapturePayload,
  validateCalendarLiveV33CleanupPayload,
  validateCalendarLiveV33ExtractPayload,
  validateCalendarLiveV33ProbePayload,
  validateCalendarLiveV33WriterPayload,
  type CalendarLiveV33CaptureCell,
  type CalendarLiveV33CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V33_PLAN_PATH,
  type CalendarLiveV33ProofPlan,
} from "./build-calendar-live-proof-v33.js";
import {
...
export const CALENDAR_LIVE_V33_RUNNER_VERSION = "calendar-live-v33-runner-v1";

interface CalendarLiveV33JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV33OrchestratorState {
  artifactVersion: "calendar-live-v33-orchestrator-state-v1";
  ...
  journal: CalendarLiveV33JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV33ProofPlan;
  planSha256: string;
  captures: CalendarLiveV33CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV33SourceIdentity & {
...
export interface CalendarLiveV33NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV33OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v33 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V33_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V33_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV33ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV33CaptureCell[];
  validateCalendarLiveV33CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v33 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v33 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV33WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV33RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV33Request =>
  readJson<CalendarLiveV33Request>(file);

export class CalendarLiveV33Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV33Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV33OrchestratorState = {
      artifactVersion: "calendar-live-v33-orchestrator-state-v1",
    ...
    return new CalendarLiveV33Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV33OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV33Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV33NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV33NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v33 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV33NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v33 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV33RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV33Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV33CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV33AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV33WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV33Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV33ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV33Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV33Response(this.#directory, "writer", {
        validate: validateCalendarLiveV33WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV33Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV33RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV33Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV33ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV33Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV33RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v33-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV33Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV33ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV33Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV33ProbePayload(payload, ownership),
      });
      persistCalendarLiveV33TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v33 capture cell missing");
      const accepted = acceptCalendarLiveV33Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV33CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V33_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV33CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV33Objective(
    ...
    throw new Error(`Calendar live v33 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV33Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV33CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV33Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV33RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV33Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV33CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV33CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV33CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV33Authorization = (
  privateKey: KeyObject,
): CalendarLiveV33TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV33SimulationKey = (): KeyObject =>
