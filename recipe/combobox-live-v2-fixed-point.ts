...
export const COMBOBOX_LIVE_V2_FIXED_VERIFIER_VERSION = "combobox-live-v2-fixed-point-v1";
export const COMBOBOX_LIVE_V2_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV2FixedThresholds {
...
export interface ComboboxLiveV2FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV2FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV2FixedSourceProbe {
...
export interface ComboboxLiveV2FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV2FixedFixedPoint;
}

export interface ComboboxLiveV2FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV2FixedHardGateInput {
  thresholds: ComboboxLiveV2FixedThresholds;
  materialRegression: ComboboxLiveV2FixedMaterialRegression;
  sourceProbes: ComboboxLiveV2FixedSourceProbe[];
  cells: ComboboxLiveV2FixedCellValidation[];
  sceneProofs: ComboboxLiveV2FixedSceneProof[];
  visualRows: ComboboxLiveV2FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV2FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V2_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV2FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV2FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV2FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV2FixedHardGates(
  input: ComboboxLiveV2FixedHardGateInput,
): ComboboxLiveV2FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV2FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV2FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V2_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV2FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV2FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
