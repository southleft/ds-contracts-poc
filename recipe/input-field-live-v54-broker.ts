...
import {
  ...
  INPUT_LIVE_V54_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V54_RESPONSE_CONTRACTS,
  type InputLiveV54RootProof,
} from "./input-field-live-v54-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V54_BROKER_VERSION = "input-live-v54-broker-v2";
export const INPUT_LIVE_V54_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V54_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V54_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V54_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V54_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V54_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV54RemotePhase =
  (typeof INPUT_LIVE_V54_REMOTE_PHASES)[number];
export type InputLiveV54AuthorizationMode = "live" | "simulated";

export interface InputLiveV54CommitPins {
...
export interface InputLiveV54TransactionAuthorization extends InputLiveV54CommitPins {
  mode: InputLiveV54AuthorizationMode;
...
export interface InputLiveV54Manifest {
  artifactVersion: "input-live-v54-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V54_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V54_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V54_TARGET;
  authorization: InputLiveV54TransactionAuthorization;
...
export interface InputLiveV54ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV54RequestBody {
  ...
  commitPins: InputLiveV54CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V54_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV54ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV54Request extends InputLiveV54RequestBody {
...
export interface InputLiveV54Receipt {
  artifactVersion: "input-live-v54-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V54_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV54RemotePhase;
...
export interface InputLiveV54AcceptedResponse<T = unknown> {
  request: InputLiveV54Request;
  receipt: InputLiveV54Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV54IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV54ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V54_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V54_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V54_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V54_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V54_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V54_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V54_TARGET.editorType;
};
...
export const inputLiveV54RequestSequence = (
  phase: InputLiveV54RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV54RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV54RequestPath = (
  directory: string,
  phase: InputLiveV54RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV54ResponsePath = (
  directory: string,
  phase: InputLiveV54RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV54ReceiptPath = (
  directory: string,
  phase: InputLiveV54RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV54Manifest => {
  const manifest = json<InputLiveV54Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v54-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V54_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V54_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV54RemotePhase,
  captureIndex?: number,
): InputLiveV54Request | undefined => {
  const file = inputLiveV54RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV54Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV54RemotePhase,
  captureIndex?: number,
): InputLiveV54Receipt | undefined => {
  const file = inputLiveV54ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV54Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV54Request | undefined;
  receipt: InputLiveV54Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV54RemotePhase,
): InputLiveV54ResponseContract => ({
  ...INPUT_LIVE_V54_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V54_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V54_DYNAMIC_TOOL,
      target: INPUT_LIVE_V54_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v54-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V54_BROKER_VERSION,
  target: INPUT_LIVE_V54_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V54_PROGRAM_START*/${program}/*INPUT_LIVE_V54_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V54_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V54_PROGRAM_END*/";
...
export function createInputLiveV54Transaction(
  directory: string,
  ...
  const manifest: InputLiveV54Manifest = {
    artifactVersion: "input-live-v54-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V54_BROKER_VERSION,
...
export function validateInputLiveV54Request(
  directory: string,
  request: InputLiveV54Request,
): void {
  ...
  const expectedSequence = inputLiveV54RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v54-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V54_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V54_TARGET.fileKey ||
...
export function issueInputLiveV54Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV54RemotePhase,
  program: string,
  options: InputLiveV54IssueOptions = {},
): InputLiveV54Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV54RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV54Request>(targetPath);
    validateInputLiveV54Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV54ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV54TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV54RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    ...
    timeout:
      ...
      (phase === "writer"
        ...
        : INPUT_LIVE_V54_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV54RequestBody = {
    artifactVersion: "input-live-v54-broker-request-v2",
    brokerVersion: INPUT_LIVE_V54_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV54Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV54Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV54RawResponse(
  directory: string,
  phase: InputLiveV54RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV54RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV54Request>(requestFile);
  if (
  ...
  const target = inputLiveV54ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV54Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v54-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V54_BROKER_VERSION,
...
export function acceptInputLiveV54Response<T = unknown>(
  directory: string,
  phase: InputLiveV54RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV54AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV54ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV54Request>(
    inputLiveV54RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV54Request(directory, request);
  if (phase === "capture") validateInputLiveV54TechnicalGates(directory);
  const rawFile = inputLiveV54ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v54-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V54_BROKER_VERSION ||
...
export function persistInputLiveV54TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV54RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV54RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V54_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v54-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V54_BROKER_VERSION,
...
export function validateInputLiveV54TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v54-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V54_TECHNICAL_GATES.some(
...
export function validateInputLiveV54Receipt(
  directory: string,
  phase: InputLiveV54RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV54Request(directory, request);
  const receipt = json<InputLiveV54Receipt>(
    inputLiveV54ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV54ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v54-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V54_BROKER_VERSION ||
...
export function readInputLiveV54AcceptedRawPayload(
  directory: string,
  phase: InputLiveV54RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV54Receipt(directory, phase, captureIndex);
...
export function createInputLiveV54FakeDynamicResponse(
  request: InputLiveV54Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V54_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V54_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV54PersistedRequests(
  directory: string,
): InputLiveV54Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV54Request>(file)] : [];
    });
}

export const inputLiveV54Sha256 = sha256;
