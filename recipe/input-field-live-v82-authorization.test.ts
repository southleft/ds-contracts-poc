...
import { INPUT_LIVE_V82_INDEX_PATH } from "./build-input-field-live-proof-v82.js";
import {
  INPUT_LIVE_V82_DYNAMIC_TOOL,
  INPUT_LIVE_V82_TARGET,
} from "./input-field-live-v82-broker.js";
import {
  buildInputLiveV82AuthorizationArtifact,
  validateInputLiveV82AntecedentIndex,
  validateInputLiveV82AuthorizationArtifact,
  validateInputLiveV82History,
  validateInputLiveV82UnitTestSource,
  type InputLiveV82AntecedentIndex,
  type InputLiveV82AuthorizationArtifact,
  type InputLiveV82AuthorizationProof,
  type InputLiveV82HistoryState,
} from "./input-field-live-v82-authorization.js";
import {
  inputLiveV82AttestationSha256,
  validateInputLiveV82ControlFlowSource,
  validateInputLiveV82SecurityAttestation,
  type InputLiveV82SecurityAttestation,
} from "./input-field-live-v82-preflight.js";
import { inputLiveV82Sha256 } from "./input-field-live-v82-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V82_INDEX_PATH);
const index = (): InputLiveV82AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV82AntecedentIndex;

const artifact = (): InputLiveV82AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV82AuthorizationArtifact({
...
const pendingState = (): InputLiveV82HistoryState => ({
  index: index(),
  indexSha256: inputLiveV82Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV82HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV82AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV82Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V82_TARGET,
  expectedDynamicTool: INPUT_LIVE_V82_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v82/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV82AuthorizationArtifact,
): InputLiveV82SecurityAttestation => {
  const body: Omit<InputLiveV82SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v82-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V82_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV82AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV82History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV82History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV82History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV82History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV82History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV82UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV82UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV82AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV82Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV82AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV82AuthorizationArtifact(
        planted as InputLiveV82AuthorizationArtifact,
        index(),
        inputLiveV82Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV82SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV82AttestationSha256(
      body as Omit<InputLiveV82SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV82SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV82ControlFlowSource(source), []);
});
