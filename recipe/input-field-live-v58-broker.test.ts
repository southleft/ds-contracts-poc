...
import {
  ...
  createInputLiveV58FakeDynamicResponse,
  ...
  listInputLiveV58PersistedRequests,
  persistInputLiveV58RawResponse,
  persistInputLiveV58TechnicalGates,
  validateInputLiveV58Receipt,
  validateInputLiveV58Request,
  type InputLiveV58RemotePhase,
...
import {
  ...
  type InputLiveV58ExtractPayload,
  type InputLiveV58ProbePayload,
...
import {
  ...
  simulatedInputLiveV58Authorization,
} from "./run-input-field-live-v58.js";
...
const captures: InputLiveV58CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV58Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v58-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV58Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV58CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV58FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV58FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V58_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V58_TARGET);
  persistInputLiveV58RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV58Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV58Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV58WriterOwnership,
): InputLiveV58RestorePayload => ({
...
const rootProofs = (): InputLiveV58RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v58-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV58Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV58Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV58RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV58RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV58ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV58ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV58CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV58CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV58CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV58Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV58CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV58PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV58ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV58Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV58Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV58Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV58Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V58_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV58RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V58_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV58Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV58Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV58ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV58Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV58WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV58RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV58Request;
    validateInputLiveV58Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV58Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV58CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v58-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV58Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV58Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV58Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V58_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV58RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V58_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV58Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV58Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV58RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V58_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV58Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV58Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV58Request;
