...
import {
  acceptComboboxLiveV10Response,
  createComboboxLiveV10Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV10Request,
  persistComboboxLiveV10RawResponse,
  persistComboboxLiveV10TechnicalGates,
  readComboboxLiveV10AcceptedRawPayload,
  type ComboboxLiveV10Request,
  type ComboboxLiveV10TransactionAuthorization,
} from "./combobox-live-v10-broker.js";
import {
  COMBOBOX_LIVE_V10_CAPTURE_COUNT,
  assertComboboxLiveV10CaptureResponses,
  ...
  evaluateComboboxLiveV10Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV10Roots,
  validateComboboxLiveV10CaptureManifest,
  validateComboboxLiveV10CapturePayload,
  validateComboboxLiveV10CleanupPayload,
  validateComboboxLiveV10ExtractPayload,
  validateComboboxLiveV10ProbePayload,
  validateComboboxLiveV10WriterPayload,
  type ComboboxLiveV10CaptureCell,
  type ComboboxLiveV10CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V10_PLAN_PATH,
  type ComboboxLiveV10ProofPlan,
} from "./build-combobox-live-proof-v10.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v10.js";

export const COMBOBOX_LIVE_V10_RUNNER_VERSION = "combobox-live-v10-runner-v1";

interface ComboboxLiveV10JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV10OrchestratorState {
  artifactVersion: "combobox-live-v10-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV10JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV10ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV10CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV10SourceIdentity & {
...
export interface ComboboxLiveV10NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV10OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v10 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V10_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V10_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV10ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV10CaptureCell[];
  validateComboboxLiveV10CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v10 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v10 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV10WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV10RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV10Request =>
  readJson<ComboboxLiveV10Request>(file);

export class ComboboxLiveV10Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV10Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV10OrchestratorState = {
      artifactVersion: "combobox-live-v10-orchestrator-state-v1",
    ...
    return new ComboboxLiveV10Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV10OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV10Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV10NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV10NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v10 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV10NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v10 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV10RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV10Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV10CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV10AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV10WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV10Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV10ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV10Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV10Response(this.#directory, "writer", {
        validate: validateComboboxLiveV10WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV10Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV10RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV10Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV10ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV10Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV10RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v10-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV10Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV10ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV10Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV10ProbePayload(payload, ownership),
      });
      persistComboboxLiveV10TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v10 capture cell missing");
      const accepted = acceptComboboxLiveV10Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV10CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V10_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV10CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV10Objective(
    ...
    throw new Error(`Combobox live v10 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV10Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV10CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV10Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV10RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV10Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV10CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV10CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV10CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV10Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV10TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV10SimulationKey = (): KeyObject =>
