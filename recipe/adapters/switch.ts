...
export type SwitchFactCategory =
  | "anatomy"
  | "geometry"
  | "typography"
  | "fill"
...
export interface ReviewedSwitchSource {
  packageName: string;
  version: string;
  exportName: string;
  framework: "react";
...
export interface ReviewedSwitchSourceFact {
  occurrenceId: string;
  category: SwitchFactCategory;
  source:
...
export interface SwitchBenchmarkBoundary {
  packageName: string;
  version: string;
  exportName: string;
  importPath: string;
...
export interface ReviewedSwitchAdapterConfig {
  sourcePath: string;
  generatedAt: string;
  selection: RecipeSelection;
  identity: { id: string; name: string };
  content: SwitchRecipeInstance["content"];
  tokens: SwitchRecipeInstance["tokens"];
  axes?: SwitchRecipeInstance["axes"];
  sourceFacts: ReviewedSwitchSourceFact[];
  manualMappings: string[];
  receipts: SwitchRecipeInstance["receipts"];
  benchmark: SwitchBenchmarkBoundary;
}

export interface SwitchAcquisitionReport {
  occurrences: number;
  ir: number;
  refusals: number;
  byCategory: Record<SwitchFactCategory, number>;
  setupSeconds: number;
  mappingCount: number;
  failures: string[];
}
...
const expectedCategory = (target: string): SwitchFactCategory => {
  if (target.startsWith("tokens.typography")) return "typography";
  if (target.endsWith("rowAlign") || target.endsWith("hitClips") || target.endsWith("trackClips"))
    return "anatomy";
  if (target.includes("trackOpacity")) return "state";
  if (
    target.includes("trackFill") ||
    target.includes("thumbFill") ||
...
export function auditReviewedSwitchAcquisition(
  source: ReviewedSwitchSource,
  config: ReviewedSwitchAdapterConfig,
  instance: SwitchRecipeInstance,
): SwitchAcquisitionReport {
  const failures: string[] = [];
  const byCategory: Record<SwitchFactCategory, number> = {
    anatomy: 0,
    geometry: 0,
    typography: 0,
    fill: 0,
...
export function adaptReviewedSwitch(
  source: ReviewedSwitchSource,
  config: ReviewedSwitchAdapterConfig,
): SwitchRecipeInstance {
  requireExactRecipeSelection(config.selection, SWITCH_RECIPE_REF);
  if (
    ...
    throw new TypeError(
      "reviewed switch adapter: source identity does not match explicit config",
    );
  const instance = structuredClone(
    canonicalSwitchRecipeInstance,
  ) as SwitchRecipeInstance;
  ...
  const normalized = normalizeSwitchRecipeInstance(instance);
  const report = auditReviewedSwitchAcquisition(source, config, normalized);
  if (report.failures.length > 0)
    throw new TypeError(
      `reviewed switch adapter: ${report.failures.join("; ")}`,
    );
  return normalized;
}
