...
import {
  COMBOBOX_RECIPE_REF,
  normalizeComboboxRecipeInstance,
...
export type ComboboxFactCategory =
  | "anatomy"
  | "geometry"
  | "typography"
  | "fill"
...
export interface ReviewedComboboxSource {
  packageName: string;
  version: string;
  exportName: string;
  framework: "react";
...
export interface ReviewedComboboxSourceFact {
  occurrenceId: string;
  category: ComboboxFactCategory;
  source:
...
export interface ComboboxBenchmarkBoundary {
  packageName: string;
  version: string;
  exportName: string;
  importPath: string;
...
export interface ReviewedComboboxAdapterConfig {
  sourcePath: string;
  generatedAt: string;
  selection: RecipeSelection;
  identity: { id: string; name: string };
  content: ComboboxRecipeInstance["content"];
  slots: ComboboxRecipeInstance["slots"];
  tokens: ComboboxRecipeInstance["tokens"];
  sourceFacts: ReviewedComboboxSourceFact[];
  manualMappings: string[];
  benchmark: ComboboxBenchmarkBoundary;
}

export interface ComboboxAcquisitionReport {
  occurrences: number;
  ir: number;
  extensions: number;
  refusals: number;
  byCategory: Record<ComboboxFactCategory, number>;
  setupSeconds: number;
  mappingCount: number;
  failures: string[];
}
...
export function auditReviewedComboboxAcquisition(
  source: ReviewedComboboxSource,
  config: ReviewedComboboxAdapterConfig,
  instance: ComboboxRecipeInstance,
): ComboboxAcquisitionReport {
  const failures: string[] = [];
  const byCategory: Record<ComboboxFactCategory, number> = {
    anatomy: 0,
    geometry: 0,
    typography: 0,
    fill: 0,
...
export function adaptReviewedCombobox(
  source: ReviewedComboboxSource,
  config: ReviewedComboboxAdapterConfig,
): ComboboxRecipeInstance {
  requireExactRecipeSelection(config.selection, COMBOBOX_RECIPE_REF);
  if (
    ...
    throw new TypeError(
      "reviewed combobox adapter: source identity does not match explicit config",
    );
  ...
  instance.slots = structuredClone(config.slots);
  ...
  for (const selected of config.sourceFacts) {
    ...
    if (selected.disposition === "ir") instance.accounting.carried.push(fact);
      ...
      if (!extension)
        throw new TypeError(
          `reviewed combobox adapter: missing extension ${selected.target}`,
        );
  ...
  const normalized = normalizeComboboxRecipeInstance(instance);
  const report = auditReviewedComboboxAcquisition(source, config, normalized);
  if (report.failures.length > 0)
    throw new TypeError(
      `reviewed combobox adapter: acquisition failed: ${report.failures.join("; ")}`,
    );
  return normalized;
}
