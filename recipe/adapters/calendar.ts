...
import {
  CALENDAR_RECIPE_REF,
  normalizeCalendarRecipeInstance,
...
export type CalendarFactCategory =
  | "anatomy"
  | "geometry"
  | "typography"
  | "fill"
...
export interface ReviewedCalendarSource {
  packageName: string;
  version: string;
  exportName: string;
  framework: "react";
...
export interface ReviewedCalendarSourceFact {
  occurrenceId: string;
  category: CalendarFactCategory;
  source:
...
export interface CalendarBenchmarkBoundary {
  packageName: string;
  version: string;
  exportName: string;
  importPath: string;
...
export interface ReviewedCalendarAdapterConfig {
  sourcePath: string;
  generatedAt: string;
  ...
  benchmark: CalendarBenchmarkBoundary;
}

export interface CalendarAcquisitionReport {
  occurrences: number;
  ir: number;
  refusals: number;
  byCategory: Record<CalendarFactCategory, number>;
  setupSeconds: number;
  mappingCount: number;
  failures: string[];
}
...
const expectedCategory = (target: string): CalendarFactCategory => {
  if (target.startsWith("tokens.typography")) return "typography";
  if (target.startsWith("tokens.dayStates")) return "state";
  if (
...
export function auditReviewedCalendarAcquisition(
  source: ReviewedCalendarSource,
  config: ReviewedCalendarAdapterConfig,
  instance: CalendarRecipeInstance,
): CalendarAcquisitionReport {
  const failures: string[] = [];
  const byCategory: Record<CalendarFactCategory, number> = {
    anatomy: 0,
    geometry: 0,
    typography: 0,
    fill: 0,
...
export function adaptReviewedCalendar(
  source: ReviewedCalendarSource,
  config: ReviewedCalendarAdapterConfig,
): CalendarRecipeInstance {
  requireExactRecipeSelection(config.selection, CALENDAR_RECIPE_REF);
  if (
    ...
    throw new TypeError(
      "reviewed calendar adapter: source identity does not match explicit config",
    );
  const instance = structuredClone(
    canonicalCalendarRecipeInstance,
  ...
  const normalized = normalizeCalendarRecipeInstance(instance);
  const report = auditReviewedCalendarAcquisition(source, config, normalized);
  if (report.failures.length > 0)
    throw new TypeError(
      `reviewed calendar adapter: ${report.failures.join("; ")}`,
    );
  return normalized;
}
