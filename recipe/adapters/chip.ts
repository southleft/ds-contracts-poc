...
export interface ReviewedChipSource {
  packageName: string;
  version: string;
  exportName: string;
  framework: "react";
...
export interface ReviewedChipSourceFact {
  occurrenceId: string;
  category: ChipFactCategory;
  source:
...
export interface ChipBenchmarkBoundary {
  packageName: string;
  version: string;
  exportName: string;
  importPath: string;
...
export interface ReviewedChipAdapterConfig {
  sourcePath: string;
  generatedAt: string;
  selection: RecipeSelection;
  identity: { id: string; name: string };
  content: ChipRecipeInstance["content"];
  tokens: ChipRecipeInstance["tokens"];
  axes?: ChipRecipeInstance["axes"];
  sourceFacts: ReviewedChipSourceFact[];
  manualMappings: string[];
  receipts: ChipRecipeInstance["receipts"];
  benchmark: ChipBenchmarkBoundary;
}

export interface ChipAcquisitionReport {
  occurrences: number;
  ir: number;
  refusals: number;
  byCategory: Record<ChipFactCategory, number>;
  setupSeconds: number;
  mappingCount: number;
  failures: string[];
}
...
const expectedCategory = (target: string): ChipFactCategory => {
...
export function auditReviewedChipAcquisition(
  source: ReviewedChipSource,
  config: ReviewedChipAdapterConfig,
  instance: ChipRecipeInstance,
): ChipAcquisitionReport {
  const failures: string[] = [];
  const byCategory: Record<ChipFactCategory, number> = {
    anatomy: 0,
    geometry: 0,
    typography: 0,
    fill: 0,
...
export function adaptReviewedChip(
  source: ReviewedChipSource,
  config: ReviewedChipAdapterConfig,
): ChipRecipeInstance {
  requireExactRecipeSelection(config.selection, CHIP_RECIPE_REF);
  if (
  ...
  const instance = structuredClone(
    canonicalChipRecipeInstance,
  ) as ChipRecipeInstance;
  ...
  const normalized = normalizeChipRecipeInstance(instance);
  const report = auditReviewedChipAcquisition(source, config, normalized);
