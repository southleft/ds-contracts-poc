...
export type DialogFactCategory =
  | "anatomy"
  | "geometry"
  | "typography"
  | "fill"
...
export interface ReviewedDialogSource {
  packageName: string;
  version: string;
  exportName: string;
  framework: "react";
...
export interface ReviewedDialogSourceFact {
  occurrenceId: string;
  category: DialogFactCategory;
  source:
...
export interface DialogBenchmarkBoundary {
  packageName: string;
  version: string;
  exportName: string;
  importPath: string;
...
export interface ReviewedDialogAdapterConfig {
  sourcePath: string;
  generatedAt: string;
  selection: RecipeSelection;
  identity: { id: string; name: string };
  content: DialogRecipeInstance["content"];
  tokens: DialogRecipeInstance["tokens"];
  axes?: DialogRecipeInstance["axes"];
  sourceFacts: ReviewedDialogSourceFact[];
  manualMappings: string[];
  receipts: DialogRecipeInstance["receipts"];
  benchmark: DialogBenchmarkBoundary;
}

export interface DialogAcquisitionReport {
  occurrences: number;
  ir: number;
  refusals: number;
  byCategory: Record<DialogFactCategory, number>;
  setupSeconds: number;
  mappingCount: number;
  failures: string[];
}
...
const expectedCategory = (target: string): DialogFactCategory => {
  if (
    target.startsWith("tokens.typography") ||
    target.includes("lineHeight") ||
    target.includes("FontSize") ||
    target.includes("textCase")
  )
    return "typography";
  if (target.includes("minWidth")) return "state";
  if (
    target.includes("fill") ||
    target === "tokens.title" ||
    target === "tokens.body"
  )
    return "fill";
...
export function auditReviewedDialogAcquisition(
  source: ReviewedDialogSource,
  config: ReviewedDialogAdapterConfig,
  instance: DialogRecipeInstance,
): DialogAcquisitionReport {
  const failures: string[] = [];
  const byCategory: Record<DialogFactCategory, number> = {
    anatomy: 0,
    geometry: 0,
    typography: 0,
    fill: 0,
...
export function adaptReviewedDialog(
  source: ReviewedDialogSource,
  config: ReviewedDialogAdapterConfig,
): DialogRecipeInstance {
  requireExactRecipeSelection(config.selection, DIALOG_RECIPE_REF);
  if (
    ...
    throw new TypeError(
      "reviewed dialog adapter: source identity does not match explicit config",
    );
  const instance = structuredClone(
    canonicalDialogRecipeInstance,
  ...
  const normalized = normalizeDialogRecipeInstance(instance);
  const report = auditReviewedDialogAcquisition(source, config, normalized);
  if (report.failures.length > 0)
    throw new TypeError(
      `reviewed dialog adapter: ${report.failures.join("; ")}`,
    );
  return normalized;
}
