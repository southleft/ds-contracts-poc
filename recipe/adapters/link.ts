...
export interface ReviewedLinkSource {
  packageName: string;
  version: string;
  exportName: string;
  framework: "react";
...
export interface ReviewedLinkSourceFact {
  occurrenceId: string;
  category: LinkFactCategory;
  source:
...
export interface LinkBenchmarkBoundary {
  packageName: string;
  version: string;
  exportName: string;
  importPath: string;
...
export interface ReviewedLinkAdapterConfig {
  sourcePath: string;
  generatedAt: string;
  selection: RecipeSelection;
  identity: { id: string; name: string };
  content: LinkRecipeInstance["content"];
  tokens: LinkRecipeInstance["tokens"];
  axes?: LinkRecipeInstance["axes"];
  sourceFacts: ReviewedLinkSourceFact[];
  manualMappings: string[];
  receipts: LinkRecipeInstance["receipts"];
  benchmark: LinkBenchmarkBoundary;
}

export interface LinkAcquisitionReport {
  occurrences: number;
  ir: number;
  refusals: number;
  byCategory: Record<LinkFactCategory, number>;
  setupSeconds: number;
  mappingCount: number;
  failures: string[];
}
...
const expectedCategory = (target: string): LinkFactCategory => {
  if (
...
export function auditReviewedLinkAcquisition(
  source: ReviewedLinkSource,
  config: ReviewedLinkAdapterConfig,
  instance: LinkRecipeInstance,
): LinkAcquisitionReport {
  const failures: string[] = [];
  const byCategory: Record<LinkFactCategory, number> = {
    anatomy: 0,
    geometry: 0,
    typography: 0,
    fill: 0,
...
export function adaptReviewedLink(
  source: ReviewedLinkSource,
  config: ReviewedLinkAdapterConfig,
): LinkRecipeInstance {
  requireExactRecipeSelection(config.selection, LINK_RECIPE_REF);
  if (
    ...
    throw new TypeError(
      "reviewed link adapter: source identity does not match explicit config",
    );
  const instance = structuredClone(
    canonicalLinkRecipeInstance,
  ) as LinkRecipeInstance;
  ...
  const normalized = normalizeLinkRecipeInstance(instance);
  const report = auditReviewedLinkAcquisition(source, config, normalized);
  if (report.failures.length > 0)
    throw new TypeError(
      `reviewed link adapter: ${report.failures.join("; ")}`,
    );
  return normalized;
}
