...
export interface ReviewedMenuSource {
  packageName: string;
  version: string;
  exportName: string;
  framework: "react";
...
export interface ReviewedMenuSourceFact {
  occurrenceId: string;
  category: MenuFactCategory;
  source:
...
export interface MenuBenchmarkBoundary {
  packageName: string;
  version: string;
  exportName: string;
  importPath: string;
...
export interface ReviewedMenuAdapterConfig {
  sourcePath: string;
  generatedAt: string;
  selection: RecipeSelection;
  identity: { id: string; name: string };
  content: MenuRecipeInstance["content"];
  tokens: MenuRecipeInstance["tokens"];
  axes?: MenuRecipeInstance["axes"];
  sourceFacts: ReviewedMenuSourceFact[];
  manualMappings: string[];
  receipts: MenuRecipeInstance["receipts"];
  benchmark: MenuBenchmarkBoundary;
}

export interface MenuAcquisitionReport {
  occurrences: number;
  ir: number;
  refusals: number;
  byCategory: Record<MenuFactCategory, number>;
  setupSeconds: number;
  mappingCount: number;
  failures: string[];
}
...
const expectedCategory = (target: string): MenuFactCategory => {
  if (
...
export function auditReviewedMenuAcquisition(
  source: ReviewedMenuSource,
  config: ReviewedMenuAdapterConfig,
  instance: MenuRecipeInstance,
): MenuAcquisitionReport {
  const failures: string[] = [];
  const byCategory: Record<MenuFactCategory, number> = {
    anatomy: 0,
    geometry: 0,
    typography: 0,
    fill: 0,
...
export function adaptReviewedMenu(
  source: ReviewedMenuSource,
  config: ReviewedMenuAdapterConfig,
): MenuRecipeInstance {
  requireExactRecipeSelection(config.selection, MENU_RECIPE_REF);
  if (
    ...
    throw new TypeError(
      "reviewed menu adapter: source identity does not match explicit config",
    );
  const instance = structuredClone(
    canonicalMenuRecipeInstance,
  ) as MenuRecipeInstance;
  ...
  const normalized = normalizeMenuRecipeInstance(instance);
  const report = auditReviewedMenuAcquisition(source, config, normalized);
  if (report.failures.length > 0)
    throw new TypeError(`reviewed menu adapter: ${report.failures.join("; ")}`);
  return normalized;
}
