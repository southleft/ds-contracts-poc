...
export type CheckboxFactCategory =
  | "anatomy"
  | "geometry"
  | "typography"
  | "fill"
...
export interface ReviewedCheckboxSource {
  packageName: string;
  version: string;
  exportName: string;
  framework: "react";
...
export interface ReviewedCheckboxSourceFact {
  occurrenceId: string;
  category: CheckboxFactCategory;
  source:
...
export interface CheckboxBenchmarkBoundary {
  packageName: string;
  version: string;
  exportName: string;
  importPath: string;
...
export interface ReviewedCheckboxAdapterConfig {
  sourcePath: string;
  generatedAt: string;
  selection: RecipeSelection;
  identity: { id: string; name: string };
  content: CheckboxRecipeInstance["content"];
  tokens: CheckboxRecipeInstance["tokens"];
  axes?: CheckboxRecipeInstance["axes"];
  sourceFacts: ReviewedCheckboxSourceFact[];
  manualMappings: string[];
  receipts: CheckboxRecipeInstance["receipts"];
  benchmark: CheckboxBenchmarkBoundary;
}

export interface CheckboxAcquisitionReport {
  occurrences: number;
  ir: number;
  refusals: number;
  byCategory: Record<CheckboxFactCategory, number>;
  setupSeconds: number;
  mappingCount: number;
  failures: string[];
}
...
export function auditReviewedCheckboxAcquisition(
  source: ReviewedCheckboxSource,
  config: ReviewedCheckboxAdapterConfig,
  instance: CheckboxRecipeInstance,
): CheckboxAcquisitionReport {
  const failures: string[] = [];
  const byCategory: Record<CheckboxFactCategory, number> = {
    anatomy: 0,
    geometry: 0,
    typography: 0,
    fill: 0,
...
export function adaptReviewedCheckbox(
  source: ReviewedCheckboxSource,
  config: ReviewedCheckboxAdapterConfig,
): CheckboxRecipeInstance {
  requireExactRecipeSelection(config.selection, CHECKBOX_RECIPE_REF);
  if (
    ...
    throw new TypeError(
      "reviewed checkbox adapter: source identity does not match explicit config",
    );
  const instance = structuredClone(
    canonicalCheckboxRecipeInstance,
  ...
  const normalized = normalizeCheckboxRecipeInstance(instance);
  const report = auditReviewedCheckboxAcquisition(source, config, normalized);
  if (report.failures.length > 0)
    throw new TypeError(
      `reviewed checkbox adapter: ${report.failures.join("; ")}`,
    );
  return normalized;
}
