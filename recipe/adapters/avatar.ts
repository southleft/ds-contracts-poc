...
export type AvatarFactCategory =
  | "anatomy"
  | "geometry"
  | "typography"
  | "fill"
...
export interface ReviewedAvatarSource {
  packageName: string;
  version: string;
  exportName: string;
  framework: "react";
...
export interface ReviewedAvatarSourceFact {
  occurrenceId: string;
  category: AvatarFactCategory;
  source:
...
export interface AvatarBenchmarkBoundary {
  packageName: string;
  version: string;
  exportName: string;
  importPath: string;
...
export interface ReviewedAvatarAdapterConfig {
  sourcePath: string;
  generatedAt: string;
  selection: RecipeSelection;
  identity: { id: string; name: string };
  content: AvatarRecipeInstance["content"];
  tokens: AvatarRecipeInstance["tokens"];
  axes?: AvatarRecipeInstance["axes"];
  sourceFacts: ReviewedAvatarSourceFact[];
  manualMappings: string[];
  receipts: AvatarRecipeInstance["receipts"];
  benchmark: AvatarBenchmarkBoundary;
}

export interface AvatarAcquisitionReport {
  occurrences: number;
  ir: number;
  refusals: number;
  byCategory: Record<AvatarFactCategory, number>;
  setupSeconds: number;
  mappingCount: number;
  failures: string[];
}
...
const expectedCategory = (target: string): AvatarFactCategory => {
...
export function auditReviewedAvatarAcquisition(
  source: ReviewedAvatarSource,
  config: ReviewedAvatarAdapterConfig,
  instance: AvatarRecipeInstance,
): AvatarAcquisitionReport {
  const failures: string[] = [];
  const byCategory: Record<AvatarFactCategory, number> = {
    anatomy: 0,
    geometry: 0,
    typography: 0,
    fill: 0,
...
export function adaptReviewedAvatar(
  source: ReviewedAvatarSource,
  config: ReviewedAvatarAdapterConfig,
): AvatarRecipeInstance {
  requireExactRecipeSelection(config.selection, AVATAR_RECIPE_REF);
  if (
    ...
    throw new TypeError(
      "reviewed chip adapter: source identity does not match explicit config",
    );
  const instance = structuredClone(
    canonicalAvatarRecipeInstance,
  ) as AvatarRecipeInstance;
  ...
  const normalized = normalizeAvatarRecipeInstance(instance);
  const report = auditReviewedAvatarAcquisition(source, config, normalized);
  if (report.failures.length > 0)
    throw new TypeError(
      `reviewed chip adapter: ${report.failures.join("; ")}`,
    );
  return normalized;
}
