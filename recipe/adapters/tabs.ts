...
export interface ReviewedTabsSource {
  packageName: string;
  version: string;
  exportName: string;
  framework: "react";
...
export interface ReviewedTabsSourceFact {
  occurrenceId: string;
  category: TabsFactCategory;
  source:
...
export interface TabsBenchmarkBoundary {
  packageName: string;
  version: string;
  exportName: string;
  importPath: string;
...
export interface ReviewedTabsAdapterConfig {
  sourcePath: string;
  generatedAt: string;
  selection: RecipeSelection;
  identity: { id: string; name: string };
  content: TabsRecipeInstance["content"];
  tokens: TabsRecipeInstance["tokens"];
  axes?: TabsRecipeInstance["axes"];
  sourceFacts: ReviewedTabsSourceFact[];
  manualMappings: string[];
  receipts: TabsRecipeInstance["receipts"];
  benchmark: TabsBenchmarkBoundary;
}

export interface TabsAcquisitionReport {
  occurrences: number;
  ir: number;
  refusals: number;
  byCategory: Record<TabsFactCategory, number>;
  setupSeconds: number;
  mappingCount: number;
  failures: string[];
}
...
const expectedCategory = (target: string): TabsFactCategory => {
  if (
    ...
    return "typography";
  if (target.includes("indicator.opacity") || target.includes("indicator-opacity"))
    return "state";
  if (
    target.includes("fill") ||
    target.endsWith(".label") ||
    target.includes("rest") ||
    target.includes("selected.label")
  )
    return "fill";
...
export function auditReviewedTabsAcquisition(
  source: ReviewedTabsSource,
  config: ReviewedTabsAdapterConfig,
  instance: TabsRecipeInstance,
): TabsAcquisitionReport {
  const failures: string[] = [];
  const byCategory: Record<TabsFactCategory, number> = {
    anatomy: 0,
    geometry: 0,
    typography: 0,
    fill: 0,
...
export function adaptReviewedTabs(
  source: ReviewedTabsSource,
  config: ReviewedTabsAdapterConfig,
): TabsRecipeInstance {
  requireExactRecipeSelection(config.selection, TABS_RECIPE_REF);
  if (
    ...
    throw new TypeError(
      "reviewed tabs adapter: source identity does not match explicit config",
    );
  const instance = structuredClone(
    canonicalTabsRecipeInstance,
  ) as TabsRecipeInstance;
  ...
  const normalized = normalizeTabsRecipeInstance(instance);
  const report = auditReviewedTabsAcquisition(source, config, normalized);
  if (report.failures.length > 0)
    throw new TypeError(`reviewed tabs adapter: ${report.failures.join("; ")}`);
  return normalized;
}
