...
export type TextareaFactCategory =
  | "anatomy"
  | "geometry"
  | "typography"
  | "fill"
...
export interface ReviewedTextareaSource {
  packageName: string;
  version: string;
  exportName: string;
  framework: "react";
...
export interface ReviewedTextareaSourceFact {
  occurrenceId: string;
  category: TextareaFactCategory;
  source:
...
export interface TextareaBenchmarkBoundary {
  packageName: string;
  version: string;
  exportName: string;
  importPath: string;
...
export interface ReviewedTextareaAdapterConfig {
  sourcePath: string;
  generatedAt: string;
  selection: RecipeSelection;
  identity: { id: string; name: string };
  content: TextareaRecipeInstance["content"];
  tokens: TextareaRecipeInstance["tokens"];
  axes?: TextareaRecipeInstance["axes"];
  sourceFacts: ReviewedTextareaSourceFact[];
  manualMappings: string[];
  receipts: TextareaRecipeInstance["receipts"];
  benchmark: TextareaBenchmarkBoundary;
}

export interface TextareaAcquisitionReport {
  occurrences: number;
  ir: number;
  refusals: number;
  byCategory: Record<TextareaFactCategory, number>;
  setupSeconds: number;
  mappingCount: number;
  failures: string[];
}
...
export function auditReviewedTextareaAcquisition(
  source: ReviewedTextareaSource,
  config: ReviewedTextareaAdapterConfig,
  instance: TextareaRecipeInstance,
): TextareaAcquisitionReport {
  const failures: string[] = [];
  const byCategory: Record<TextareaFactCategory, number> = {
    anatomy: 0,
    geometry: 0,
    typography: 0,
    fill: 0,
...
export function adaptReviewedTextarea(
  source: ReviewedTextareaSource,
  config: ReviewedTextareaAdapterConfig,
): TextareaRecipeInstance {
  requireExactRecipeSelection(config.selection, TEXTAREA_RECIPE_REF);
  if (
    ...
    throw new TypeError(
      "reviewed textarea adapter: source identity does not match explicit config",
    );
  const instance = structuredClone(
    canonicalTextareaRecipeInstance,
  ...
  const normalized = normalizeTextareaRecipeInstance(instance);
  const report = auditReviewedTextareaAcquisition(source, config, normalized);
  if (report.failures.length > 0)
    throw new TypeError(
      `reviewed textarea adapter: ${report.failures.join("; ")}`,
    );
  return normalized;
}
