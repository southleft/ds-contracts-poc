...
export interface ReviewedBadgeSource {
  packageName: string;
  version: string;
  exportName: string;
  framework: "react";
...
export interface ReviewedBadgeSourceFact {
  occurrenceId: string;
  category: BadgeFactCategory;
  source:
...
export interface BadgeBenchmarkBoundary {
  packageName: string;
  version: string;
  exportName: string;
  importPath: string;
...
export interface ReviewedBadgeAdapterConfig {
  sourcePath: string;
  generatedAt: string;
  selection: RecipeSelection;
  identity: { id: string; name: string };
  content: BadgeRecipeInstance["content"];
  tokens: BadgeRecipeInstance["tokens"];
  axes?: BadgeRecipeInstance["axes"];
  sourceFacts: ReviewedBadgeSourceFact[];
  manualMappings: string[];
  receipts: BadgeRecipeInstance["receipts"];
  benchmark: BadgeBenchmarkBoundary;
}

export interface BadgeAcquisitionReport {
  occurrences: number;
  ir: number;
  refusals: number;
  byCategory: Record<BadgeFactCategory, number>;
  setupSeconds: number;
  mappingCount: number;
  failures: string[];
}
...
const expectedCategory = (target: string): BadgeFactCategory => {
  if (target.startsWith("tokens.typography")) return "typography";
  if (target.endsWith("strokeAlign")) return "anatomy";
  if (target.includes("opacity")) return "state";
  if (
...
export function auditReviewedBadgeAcquisition(
  source: ReviewedBadgeSource,
  config: ReviewedBadgeAdapterConfig,
  instance: BadgeRecipeInstance,
): BadgeAcquisitionReport {
  const failures: string[] = [];
  const byCategory: Record<BadgeFactCategory, number> = {
    anatomy: 0,
    geometry: 0,
    typography: 0,
    fill: 0,
...
export function adaptReviewedBadge(
  source: ReviewedBadgeSource,
  config: ReviewedBadgeAdapterConfig,
): BadgeRecipeInstance {
  requireExactRecipeSelection(config.selection, BADGE_RECIPE_REF);
  if (
    ...
    throw new TypeError(
      "reviewed badge adapter: source identity does not match explicit config",
    );
  const instance = structuredClone(
    canonicalBadgeRecipeInstance,
  ) as BadgeRecipeInstance;
  ...
  const normalized = normalizeBadgeRecipeInstance(instance);
  const report = auditReviewedBadgeAcquisition(source, config, normalized);
  if (report.failures.length > 0)
    throw new TypeError(
      `reviewed badge adapter: ${report.failures.join("; ")}`,
    );
  return normalized;
}
