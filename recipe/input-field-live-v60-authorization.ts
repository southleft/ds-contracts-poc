...
import {
  ...
  INPUT_LIVE_V60_INDEX_PATH,
} from "./build-input-field-live-proof-v60.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV60HistoryExpectation = "pending" | "authorized";

export interface InputLiveV60AuthorizationArtifact {
  artifactVersion: "input-live-v60-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V60_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV60AntecedentIndex {
...
export interface InputLiveV60HistoryState {
  index?: InputLiveV60AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV60AuthorizationArtifact;
...
export interface InputLiveV60AuthorizationProof extends InputLiveV60TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V60_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V60_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V60_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V60_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v60-authorization") ||
  artifactPath.includes("input-field-live-v60-preflight") ||
  artifactPath.includes("input-field-live-v60-authorized") ||
  artifactPath.includes("input-field-live-pivot-v60-status") ||
...
export function validateInputLiveV60AntecedentIndex(
  index: InputLiveV60AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v60-antecedent-index-v1" ||
...
export function validateInputLiveV60UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV60History\(\s*["']/.test(source) ||
    /readInputLiveV60HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV60AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV60AuthorizationArtifact {
  ...
  ) as InputLiveV60AntecedentIndex;
  const indexFailures = validateInputLiveV60AntecedentIndex(index);
...
export function validateInputLiveV60AuthorizationArtifact(
  artifact: InputLiveV60AuthorizationArtifact | undefined,
  index: InputLiveV60AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v60-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v60" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V60_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V60_TARGET) ||
    !equalJson(
...
export function validateInputLiveV60History(
  state: InputLiveV60HistoryState,
  expected: InputLiveV60HistoryExpectation,
): string[] {
  const failures = validateInputLiveV60AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV60AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV60HistoryState(
  root = process.cwd(),
): InputLiveV60HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V60_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV60AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V60_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V60_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V60_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV60History(
  expected: InputLiveV60HistoryExpectation,
  root = process.cwd(),
): InputLiveV60HistoryState {
  const state = readInputLiveV60HistoryState(root);
  const failures = validateInputLiveV60History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV60Authorization(
  root = process.cwd(),
): InputLiveV60AuthorizationProof {
  const state = verifyInputLiveV60History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V60_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v60/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v60.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V60_TARGET,
    expectedDynamicTool: INPUT_LIVE_V60_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V60_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV60HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV60History(expected);
