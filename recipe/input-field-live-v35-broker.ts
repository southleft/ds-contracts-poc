...
import {
  assertInputLiveV35RootProofs,
  INPUT_LIVE_V35_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V35_RESPONSE_CONTRACTS,
  type InputLiveV35RootProof,
} from "./input-field-live-v35-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V35_BROKER_VERSION = "input-live-v35-broker-v2";
export const INPUT_LIVE_V35_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V35_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V35_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V35_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V35_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V35_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV35RemotePhase =
  (typeof INPUT_LIVE_V35_REMOTE_PHASES)[number];
export type InputLiveV35AuthorizationMode = "live" | "simulated";

export interface InputLiveV35CommitPins {
...
export interface InputLiveV35TransactionAuthorization extends InputLiveV35CommitPins {
  mode: InputLiveV35AuthorizationMode;
...
export interface InputLiveV35Manifest {
  artifactVersion: "input-live-v35-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V35_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V35_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V35_TARGET;
  authorization: InputLiveV35TransactionAuthorization;
...
export interface InputLiveV35ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV35RequestBody {
  artifactVersion: "input-live-v35-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V35_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV35RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV35CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V35_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV35ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV35Request extends InputLiveV35RequestBody {
...
export interface InputLiveV35Receipt {
  artifactVersion: "input-live-v35-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V35_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV35RemotePhase;
...
export interface InputLiveV35AcceptedResponse<T = unknown> {
  request: InputLiveV35Request;
  receipt: InputLiveV35Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV35IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV35ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V35_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V35_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V35_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V35_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V35_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V35_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V35_TARGET.editorType;
};
...
export const inputLiveV35RequestSequence = (
  phase: InputLiveV35RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV35RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV35RequestPath = (
  directory: string,
  phase: InputLiveV35RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV35ResponsePath = (
  directory: string,
  phase: InputLiveV35RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV35ReceiptPath = (
  directory: string,
  phase: InputLiveV35RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV35Manifest => {
  const manifest = json<InputLiveV35Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v35-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V35_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V35_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV35RemotePhase,
  captureIndex?: number,
): InputLiveV35Request | undefined => {
  const file = inputLiveV35RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV35Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV35RemotePhase,
  captureIndex?: number,
): InputLiveV35Receipt | undefined => {
  const file = inputLiveV35ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV35Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV35Request | undefined;
  receipt: InputLiveV35Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV35RemotePhase,
): InputLiveV35ResponseContract => ({
  ...INPUT_LIVE_V35_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V35_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V35_DYNAMIC_TOOL,
      target: INPUT_LIVE_V35_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v35-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V35_BROKER_VERSION,
  target: INPUT_LIVE_V35_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V35_PROGRAM_START*/${program}/*INPUT_LIVE_V35_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V35_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V35_PROGRAM_END*/";
...
export function createInputLiveV35Transaction(
  directory: string,
  ...
  const manifest: InputLiveV35Manifest = {
    artifactVersion: "input-live-v35-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V35_BROKER_VERSION,
...
export function validateInputLiveV35Request(
  directory: string,
  request: InputLiveV35Request,
): void {
  ...
  const expectedSequence = inputLiveV35RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v35-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V35_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V35_TARGET.fileKey ||
...
export function issueInputLiveV35Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV35RemotePhase,
  program: string,
  options: InputLiveV35IssueOptions = {},
): InputLiveV35Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV35RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV35Request>(targetPath);
    validateInputLiveV35Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV35ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV35TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV35RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V35_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V35_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V35_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV35RequestBody = {
    artifactVersion: "input-live-v35-broker-request-v2",
    brokerVersion: INPUT_LIVE_V35_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV35Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV35Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV35RawResponse(
  directory: string,
  phase: InputLiveV35RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV35RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV35Request>(requestFile);
  if (
  ...
  const target = inputLiveV35ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV35Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v35-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V35_BROKER_VERSION,
...
export function acceptInputLiveV35Response<T = unknown>(
  directory: string,
  phase: InputLiveV35RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV35AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV35ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV35Request>(
    inputLiveV35RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV35Request(directory, request);
  if (phase === "capture") validateInputLiveV35TechnicalGates(directory);
  const rawFile = inputLiveV35ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v35-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V35_BROKER_VERSION ||
...
export function persistInputLiveV35TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV35RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV35RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V35_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v35-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V35_BROKER_VERSION,
...
export function validateInputLiveV35TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v35-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V35_TECHNICAL_GATES.some(
...
export function validateInputLiveV35Receipt(
  directory: string,
  phase: InputLiveV35RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV35Request(directory, request);
  const receipt = json<InputLiveV35Receipt>(
    inputLiveV35ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV35ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v35-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V35_BROKER_VERSION ||
...
export function readInputLiveV35AcceptedRawPayload(
  directory: string,
  phase: InputLiveV35RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV35Receipt(directory, phase, captureIndex);
...
export function createInputLiveV35FakeDynamicResponse(
  request: InputLiveV35Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V35_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V35_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV35PersistedRequests(
  directory: string,
): InputLiveV35Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV35Request>(file)] : [];
    });
}

export const inputLiveV35Sha256 = sha256;
