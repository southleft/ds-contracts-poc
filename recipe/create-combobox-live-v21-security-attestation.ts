...
import { verifyComboboxLiveV21Authorization } from "./combobox-live-v21-authorization.js";
import { COMBOBOX_LIVE_V21_TARGET } from "./combobox-live-v21-broker.js";
import {
  COMBOBOX_LIVE_V21_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV21Authorization();
...
const attestation: ComboboxLiveV21SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV21SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v21 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V21_SECURITY_ATTESTATION_DEFAULT_PATH,
);
