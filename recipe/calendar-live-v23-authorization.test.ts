...
import { CALENDAR_LIVE_V23_INDEX_PATH } from "./build-calendar-live-proof-v23.js";
import {
...
import {
  buildCalendarLiveV23AuthorizationArtifact,
  validateCalendarLiveV23AntecedentIndex,
  validateCalendarLiveV23AuthorizationArtifact,
  validateCalendarLiveV23History,
  validateCalendarLiveV23UnitTestSource,
  type CalendarLiveV23AntecedentIndex,
  type CalendarLiveV23AuthorizationArtifact,
  type CalendarLiveV23AuthorizationProof,
  type CalendarLiveV23HistoryState,
} from "./calendar-live-v23-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV23ControlFlowSource,
  validateCalendarLiveV23SecurityAttestation,
  type CalendarLiveV23SecurityAttestation,
} from "./calendar-live-v23-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V23_INDEX_PATH);
const index = (): CalendarLiveV23AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV23AntecedentIndex;

const artifact = (): CalendarLiveV23AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV23AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV23HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV23HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV23AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v23/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV23AuthorizationArtifact,
): CalendarLiveV23SecurityAttestation => {
  const body: Omit<CalendarLiveV23SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V23_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV23History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV23History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV23History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV23History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV23History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV23UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v23-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV23UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV23AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV23AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV23AuthorizationArtifact(
        planted as CalendarLiveV23AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV23SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV23SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV23SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v23-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV23ControlFlowSource(source), []);
});
