...
import {
  ...
  verifyInputLiveV57Authorization,
  type InputLiveV57AntecedentIndex,
  type InputLiveV57AuthorizationProof,
} from "./input-field-live-v57-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V57_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v57/operator-security-attestation-template.json";
export const INPUT_LIVE_V57_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v57-security-attestation.json";

export interface InputLiveV57SecurityAttestation {
...
export interface InputLiveV57PreflightReport {
  artifactVersion: "input-live-v57-preflight-v1";
...
export const inputLiveV57AttestationSha256 = (
  value: Omit<InputLiveV57SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV57SecurityAttestation(
  value: unknown,
  proof: InputLiveV57AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v57-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V57_TARGET) ||
...
export function validateInputLiveV57ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV57Authorization");
  const preflight = source.indexOf("runInputLiveV57Preflight");
  const initialize = source.indexOf("InputLiveV57Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV57Preflight(
  root: string,
  proof: InputLiveV57AuthorizationProof,
  attestationPath = INPUT_LIVE_V57_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV57PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V57_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V57_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V57_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV57AntecedentIndex;
  failures.push(...validateInputLiveV57AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V57_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V57_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v57-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV57ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV57SecurityAttestation;
  return {
    artifactVersion: "input-live-v57-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V57_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV57Preflight(
    process.cwd(),
    verifyInputLiveV57Authorization(),
    attestationPath,
