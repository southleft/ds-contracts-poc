...
import { CALENDAR_LIVE_V32_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v32.js";
import { verifyCalendarLiveV32Authorization } from "./calendar-live-v32-authorization.js";
import { runCalendarLiveV32Preflight } from "./calendar-live-v32-preflight.js";
import { CalendarLiveV32Orchestrator } from "./run-calendar-live-v32.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v32 requires exact authorization artifact ${CALENDAR_LIVE_V32_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v32-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV32Authorization(root);
    runCalendarLiveV32Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV32Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV32Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
