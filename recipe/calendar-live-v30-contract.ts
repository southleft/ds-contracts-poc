...
import { buildCalendarLiveV30CleanupRuntime } from "./calendar-live-v30-cleanup.js";
import { type CalendarLiveV30FixedFixedPoint } from "./calendar-live-v30-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV30RestoreProgram,
  validateCalendarLiveV30RestorePayload,
  type CalendarLiveV30RestorePayload,
} from "./calendar-live-v30-restore.js";
import {
  buildCalendarLiveV30RawPropertyRuntime,
  normalizeCalendarLiveV30Scene,
  type CalendarLiveV30RawNode,
} from "./calendar-live-v30-verifier.js";
...
export const CALENDAR_LIVE_V30_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V30_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V30_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V30_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V30_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V30_SET_COUNT = 3;
export const CALENDAR_LIVE_V30_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V30_HOST_PHASES = 3;
export const CALENDAR_LIVE_V30_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V30_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V30_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V30_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV30SourceId = (typeof CALENDAR_LIVE_V30_SOURCE_IDS)[number];
export type CalendarLiveV30Kind = "calendar" | "week" | "day";

export interface CalendarLiveV30SourceIdentity {
  source: CalendarLiveV30SourceId;
...
export interface CalendarLiveV30WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV30ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV30SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV30RawNode;
    weekScene: CalendarLiveV30RawNode;
    dayScene: CalendarLiveV30RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV30RootProof {
  source: CalendarLiveV30SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV30FixedFixedPoint;
}

export interface CalendarLiveV30ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV30SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV30CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV30SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV30CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV30SourceId;
  frameWidth: number;
...
export interface CalendarLiveV30CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v30 ${label} must be an object`);
...
export function validateCalendarLiveV30WriterPayload(
  value: unknown,
): CalendarLiveV30WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v30") ||
    ...
    result.namespace !== CALENDAR_LIVE_V30_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v30 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v30 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v30 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV30SourceIdentity,
  kind: CalendarLiveV30Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV30ExtractProgram(
  writer: CalendarLiveV30WriterOwnership,
  sources: readonly CalendarLiveV30SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v30 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V30_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V30_NAMESPACE)}
${buildCalendarLiveV30RawPropertyRuntime()}
...
export function validateCalendarLiveV30ExtractPayload(
  value: unknown,
  writer: CalendarLiveV30WriterOwnership,
): CalendarLiveV30ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v30 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V30_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v30 extract schema/one-root mismatch");
  return payload as CalendarLiveV30ExtractPayload;
}

export function proveCalendarLiveV30Roots<Instance>(
  extract: CalendarLiveV30ExtractPayload,
  sources: readonly (CalendarLiveV30SourceIdentity & {
...
): CalendarLiveV30RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v30 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v30 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV30Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV30Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV30Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV30FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v30 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v30 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v30 compile lost owned sets");
...
export function assertCalendarLiveV30RootProofs(
  proofs: readonly CalendarLiveV30RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v30 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v30 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV30ProbeProgram(
  writer: CalendarLiveV30WriterOwnership,
  sources: readonly CalendarLiveV30SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V30_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV30ProbePayload(
  value: unknown,
  writer: CalendarLiveV30WriterOwnership,
): CalendarLiveV30ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV30ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V30_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V30_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v30 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV30CaptureManifest(
  cells: readonly CalendarLiveV30CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V30_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V30_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V30_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V30_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v30 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV30CaptureCell[],
): string => {
  validateCalendarLiveV30CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV30CaptureProgram(
  writer: CalendarLiveV30WriterOwnership,
  cell: CalendarLiveV30CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V30_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v30 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v30 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V30_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV30CapturePayload(
  value: unknown,
  cell: CalendarLiveV30CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV30CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV30CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V30_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V30_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v30 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV30CaptureResponses(
  manifest: readonly CalendarLiveV30CaptureCell[],
  responses: readonly CalendarLiveV30CapturePayload[],
): void {
  validateCalendarLiveV30CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v30 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV30ObjectiveReport {
  artifactVersion: "calendar-live-v30-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV30Objective(
  manifest: readonly CalendarLiveV30CaptureCell[],
  responses: readonly CalendarLiveV30CapturePayload[],
): CalendarLiveV30ObjectiveReport {
  assertCalendarLiveV30CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v30-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV30CleanupProgram(
  writer: CalendarLiveV30WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV30CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V30_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v30 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV30CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV30WriterOwnership,
): CalendarLiveV30CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV30CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v30 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V30_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV30WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
