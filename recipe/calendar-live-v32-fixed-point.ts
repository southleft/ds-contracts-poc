/**
 * Calendar live v32 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V32_FIXED_VERIFIER_VERSION = "calendar-live-v32-fixed-point-v1";
export const CALENDAR_LIVE_V32_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV32FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV32FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV32FixedSourceProbe {
...
export interface CalendarLiveV32FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV32FixedFixedPoint;
}

export interface CalendarLiveV32FixedHardGateInput {
  thresholds: CalendarLiveV32FixedThresholds;
  sourceProbes: CalendarLiveV32FixedSourceProbe[];
  cells: CalendarLiveV32FixedCellValidation[];
  sceneProofs: CalendarLiveV32FixedSceneProof[];
...
export interface CalendarLiveV32FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V32_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV32FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV32FixedHardGates(
  input: CalendarLiveV32FixedHardGateInput,
): CalendarLiveV32FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV32FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V32_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV32FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV32FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV32FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
