...
import {
  ...
  createInputLiveV69FakeDynamicResponse,
  ...
  listInputLiveV69PersistedRequests,
  persistInputLiveV69RawResponse,
  persistInputLiveV69TechnicalGates,
  validateInputLiveV69Receipt,
  validateInputLiveV69Request,
  type InputLiveV69RemotePhase,
...
import {
  ...
  type InputLiveV69ExtractPayload,
  type InputLiveV69ProbePayload,
...
import {
  ...
  simulatedInputLiveV69Authorization,
} from "./run-input-field-live-v69.js";
...
const captures: InputLiveV69CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV69Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v69-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV69Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV69CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV69FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV69FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V69_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V69_TARGET);
  persistInputLiveV69RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV69Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV69Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV69WriterOwnership,
...
const rootProofs = (): InputLiveV69RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v69-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV69Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV69Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV69RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV69RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV69ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV69ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV69CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV69CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV69CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV69Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV69CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV69PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV69ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV69Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV69Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV69Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV69Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V69_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV69RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V69_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV69Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV69Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV69ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV69Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV69WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV69RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV69Request;
    validateInputLiveV69Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV69Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV69CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v69-state-"),
  );
  ...
  try {
    const orchestrator = InputLiveV69Orchestrator.initialize({
      ...
      privateKey,
      authorization: simulatedInputLiveV69Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV69Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V69_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV69RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V69_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV69Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV69Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV69RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V69_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV69Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV69Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV69Request;
