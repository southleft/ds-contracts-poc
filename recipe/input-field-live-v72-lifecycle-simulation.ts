...
export interface InputLiveV72LifecycleSimulationReport {
  artifactVersion: "input-live-v72-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV72AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV72Lifecycle(): Promise<InputLiveV72LifecycleSimulationReport> {
  await buildInputLiveV72Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V72_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV72AntecedentIndex;
  const indexSha256 = inputLiveV72Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v72-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V72_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V72_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV72History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV72History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV72AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V72_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V72_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV72History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV72History(authorized, "pending");
    ...
    await buildInputLiveV72Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V72_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV72AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV72Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV72Lifecycle(), null, 2)}\n`,
  );
