...
export const CALENDAR_LIVE_V8_MEASURE_HUG_MARKER = "CALENDAR-TEXT-HUG-MEASURE";
export const CALENDAR_LIVE_V8_RESTORE_CONTENT_ROLES = [
  "calendar/day/label",
] as const;
export const CALENDAR_LIVE_V8_RESTORE_DAY_SET_ROLE = "calendar/day-set";
...
export interface CalendarLiveV8RestoreWriter {
  pageId: string;
  runIdentity: string;
  setIds: readonly string[];
}

export interface CalendarLiveV8RestorePayload {
  pageId: string;
  setIds: string[];
  restoredCount: 4;
  fixedBefore: number;
  ...
  marker: typeof CALENDAR_LIVE_V8_MEASURE_HUG_MARKER;
}

export function buildCalendarLiveV8RestoreProgram(
  writer: CalendarLiveV8RestoreWriter,
): string {
  if (writer.setIds.length !== 3)
    throw new TypeError("Calendar live v8 restore requires three owned sets");
  if (
...
const ROLES=new Set(${JSON.stringify([...CALENDAR_LIVE_V8_RESTORE_CONTENT_ROLES])});
const DAY_SET=${JSON.stringify(CALENDAR_LIVE_V8_RESTORE_DAY_SET_ROLE)};
const MARKER=${JSON.stringify(CALENDAR_LIVE_V8_MEASURE_HUG_MARKER)};
...
export function validateCalendarLiveV8RestorePayload(
  payload: unknown,
  writer: CalendarLiveV8RestoreWriter,
): CalendarLiveV8RestorePayload {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("Calendar live v8 restore payload must be an object");
  ...
  if (
    ...
    value.marker !== CALENDAR_LIVE_V8_MEASURE_HUG_MARKER
  )
    throw new TypeError("Calendar live v8 restore did not re-assert content HUG");
  return value as CalendarLiveV8RestorePayload;
}
