...
import { calendarLiveV1Sha256 } from "./calendar-live-v30-broker.js";

export interface CalendarLiveV30LifecycleSimulationReport {
  artifactVersion: "calendar-live-v30-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV30AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV30Lifecycle(): Promise<CalendarLiveV30LifecycleSimulationReport> {
  await buildCalendarLiveV30Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V30_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV30AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v30-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V30_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V30_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV30History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV30History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV30AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V30_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V30_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV30History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV30History(authorized, "pending");
    ...
    await buildCalendarLiveV30Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V30_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV30AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV30Lifecycle(), null, 2)}\n`,
  );
