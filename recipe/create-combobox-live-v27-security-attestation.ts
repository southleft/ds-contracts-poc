...
import { verifyComboboxLiveV27Authorization } from "./combobox-live-v27-authorization.js";
import { COMBOBOX_LIVE_V27_TARGET } from "./combobox-live-v27-broker.js";
import {
  COMBOBOX_LIVE_V27_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV27Authorization();
...
const attestation: ComboboxLiveV27SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV27SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v27 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V27_SECURITY_ATTESTATION_DEFAULT_PATH,
);
