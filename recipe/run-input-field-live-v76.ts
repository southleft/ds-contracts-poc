...
import {
  acceptInputLiveV76Response,
  createInputLiveV76Transaction,
  inputLiveV76ReceiptPath,
  inputLiveV76RequestPath,
  inputLiveV76ResponsePath,
  inputLiveV76Sha256,
  issueInputLiveV76Request,
  persistInputLiveV76RawResponse,
  persistInputLiveV76TechnicalGates,
  readInputLiveV76AcceptedRawPayload,
  type InputLiveV76Request,
  type InputLiveV76TransactionAuthorization,
} from "./input-field-live-v76-broker.js";
import {
  INPUT_LIVE_V76_CAPTURE_COUNT,
  assertInputLiveV76CaptureResponses,
  assertInputLiveV76RootProofs,
  buildInputLiveV76CaptureProgram,
  buildInputLiveV76CleanupProgram,
  buildInputLiveV76ExtractProgram,
  buildInputLiveV76ProbeProgram,
  buildInputLiveV76RestoreProgram,
  validateInputLiveV76RestorePayload,
  evaluateInputLiveV76Objective,
  inputLiveV76CaptureManifestSha256,
  proveInputLiveV76Roots,
  validateInputLiveV76CaptureManifest,
  validateInputLiveV76CapturePayload,
  validateInputLiveV76CleanupPayload,
  validateInputLiveV76ExtractPayload,
  validateInputLiveV76ProbePayload,
  validateInputLiveV76WriterPayload,
  type InputLiveV76CaptureCell,
  type InputLiveV76CapturePayload,
  type InputLiveV76RootProof,
  type InputLiveV76SourceIdentity,
  type InputLiveV76WriterOwnership,
} from "./input-field-live-v76-contract.js";
import {
  INPUT_LIVE_V76_CAPTURE_MANIFEST_PATH,
  INPUT_LIVE_V76_PLAN_PATH,
  type InputLiveV76ProofPlan,
...
import type { ExpectedScenePlan } from "./scene-readback-v76.js";

export const INPUT_LIVE_V76_RUNNER_VERSION = "input-live-v76-runner-v1";

interface InputLiveV76JournalEntry {
  index: number;
  event: string;
...
interface InputLiveV76OrchestratorState {
  artifactVersion: "input-live-v76-orchestrator-state-v1";
  ...
  journal: InputLiveV76JournalEntry[];
}

interface LoadedProofInputs {
  plan: InputLiveV76ProofPlan;
  planSha256: string;
  captures: InputLiveV76CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    InputLiveV76SourceIdentity & {
...
export interface InputLiveV76NextAction {
...
const writeState = (
  directory: string,
  state: InputLiveV76OrchestratorState,
): void => {
...
const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, INPUT_LIVE_V76_PLAN_PATH);
  const capturePath = path.join(root, INPUT_LIVE_V76_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as InputLiveV76ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as InputLiveV76CaptureCell[];
  validateInputLiveV76CaptureManifest(captures);
  if (
    plan.captureManifest.sha256 !== inputLiveV76Sha256(captureBytes) ||
    captureArtifact.cellsSha256 !== inputLiveV76CaptureManifestSha256(captures)
  )
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (
      inputLiveV76Sha256(expectedBytes) !== source.expectedScenePlan.sha256 ||
      inputLiveV76Sha256(uncompressed) !==
  ...
  return {
    plan,
    planSha256: inputLiveV76Sha256(planBytes),
    captures,
    captureManifestSha256: inputLiveV76Sha256(captureBytes),
    sources,
...
const appendJournal = (
  ...
  const payloadSha256 = inputLiveV76Sha256(canonicalJson(payload));
  const body = {
  ...
  const entry = {
    ...body,
    entrySha256: inputLiveV76Sha256(canonicalJson(body)),
  };
  writeNew(
    path.join(
      directory,
...
const writerOwnership = (directory: string): InputLiveV76WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): InputLiveV76RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): InputLiveV76Request =>
  readJson<InputLiveV76Request>(file);

export class InputLiveV76Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueInputLiveV76Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: InputLiveV76OrchestratorState = {
      artifactVersion: "input-live-v76-orchestrator-state-v1",
      transactionDirectory: options.transactionDirectory,
      activeRequest: inputLiveV76RequestPath(
        options.transactionDirectory,
        "writer",
      ),
      cleanupRequest: null,
    ...
    return new InputLiveV76Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<InputLiveV76OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new InputLiveV76Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): InputLiveV76NextAction {
    const cleanup =
      this.#state.cleanupRequest &&
      !this.#state.cleanupAccepted &&
      !existsSync(inputLiveV76ReceiptPath(this.#directory, "cleanup"))
        ? this.#state.cleanupRequest
        : null;
  ...
  ingestAndAdvance(rawResponse: string): InputLiveV76NextAction {
  ...
  ingestCleanup(rawResponse: string): InputLiveV76NextAction {
    ...
    persistInputLiveV76RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptInputLiveV76Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV76CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      existsSync(inputLiveV76ReceiptPath(this.#directory, "writer")) &&
      !existsSync(path.join(this.#directory, OWNERSHIP_FILE))
    ) {
      const raw = readInputLiveV76AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateInputLiveV76WriterPayload(raw),
      );
    }
    if (existsSync(inputLiveV76RequestPath(this.#directory, "cleanup")))
      this.#state.cleanupRequest = inputLiveV76RequestPath(
        this.#directory,
        "cleanup",
      );
    if (
      existsSync(path.join(this.#directory, OWNERSHIP_FILE)) &&
      !existsSync(inputLiveV76RequestPath(this.#directory, "cleanup"))
    )
      this.#issueCleanupAndRestore(false);
    else if (
      existsSync(inputLiveV76RequestPath(this.#directory, "cleanup")) &&
      !existsSync(inputLiveV76RequestPath(this.#directory, "restore"))
    )
      this.#issueRestore(false);
    if (
      existsSync(inputLiveV76RequestPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV76ReceiptPath(this.#directory, "restore"))
    )
      this.#state.activeRequest = inputLiveV76RequestPath(
        this.#directory,
        "restore",
      );
    if (
      existsSync(inputLiveV76ReceiptPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV76RequestPath(this.#directory, "extract"))
    ) {
      const ownership = writerOwnership(this.#directory);
      const extract = issueInputLiveV76Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildInputLiveV76ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV76RequestPath(
        this.#directory,
        "extract",
    ...
    if (existsSync(inputLiveV76ReceiptPath(this.#directory, "cleanup")))
      this.#state.cleanupAccepted = true;
    this.#persist();
  }

  #acceptActive(request: InputLiveV76Request): void {
    if (request.phase === "writer") {
      const accepted = acceptInputLiveV76Response(this.#directory, "writer", {
        validate: validateInputLiveV76WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptInputLiveV76Response(this.#directory, "restore", {
        validate: (payload) =>
          validateInputLiveV76RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptInputLiveV76Response(this.#directory, "extract", {
        validate: (payload) =>
          validateInputLiveV76ExtractPayload(payload, ownership),
      });
      const proofs = proveInputLiveV76Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseInputFieldRecipe,
        compileInputFieldRecipe,
      );
      assertInputLiveV76RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "input-live-v76-host-root-proofs-v1",
      ...
      appendJournal(
        ...
        request.requestId,
        { proofsSha256: inputLiveV76Sha256(canonicalJson(proofs)) },
      );
      const next = issueInputLiveV76Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildInputLiveV76ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV76RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptInputLiveV76Response(this.#directory, "probe", {
        validate: (payload) =>
          validateInputLiveV76ProbePayload(payload, ownership),
      });
      persistInputLiveV76TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      const accepted = acceptInputLiveV76Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateInputLiveV76CapturePayload(payload, cell, rawBytes),
      });
      const png = Buffer.from(accepted.payload.pngBase64, "base64");
      const capturePath = path.join(
        this.#directory,
        "captures",
        `${String(cell.index).padStart(3, "0")}-${inputLiveV76Sha256(cell.cellKey).slice(0, 20)}.png`,
      );
      ...
      if (next < INPUT_LIVE_V76_CAPTURE_COUNT) this.#issueCapture(next);
      else {
          ...
          pngBase64: Buffer.from(
            readFileSync(
              path.join(
                this.#directory,
                "captures",
                `${String(response.index).padStart(3, "0")}-${inputLiveV76Sha256(response.cellKey).slice(0, 20)}.png`,
              ),
            ),
          ).toString("base64"),
        }));
        assertInputLiveV76CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateInputLiveV76Objective(
        ...
        appendJournal(
          ...
          request.requestId,
          {
            reportSha256: inputLiveV76Sha256(canonicalJson(objective)),
  ...
  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueInputLiveV76Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildInputLiveV76CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = inputLiveV76RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueInputLiveV76Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildInputLiveV76RestoreProgram(ownership),
    );
    this.#state.activeRequest = inputLiveV76RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueInputLiveV76Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildInputLiveV76CaptureProgram(
  ...
  #readCaptureResponseIndex(): InputLiveV76CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: InputLiveV76CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedInputLiveV76Authorization = (
  privateKey: KeyObject,
): InputLiveV76TransactionAuthorization => ({
  mode: "simulated",
  ...
  signingPublicKeySha256: inputLiveV76Sha256(
...
export const generateInputLiveV76SimulationKey = (): KeyObject =>
