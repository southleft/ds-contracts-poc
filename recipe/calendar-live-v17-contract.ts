...
import { buildCalendarLiveV17CleanupRuntime } from "./calendar-live-v17-cleanup.js";
import { type CalendarLiveV17FixedFixedPoint } from "./calendar-live-v17-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV17RestoreProgram,
  validateCalendarLiveV17RestorePayload,
  type CalendarLiveV17RestorePayload,
} from "./calendar-live-v17-restore.js";
import {
  buildCalendarLiveV17RawPropertyRuntime,
  normalizeCalendarLiveV17Scene,
  type CalendarLiveV17RawNode,
} from "./calendar-live-v17-verifier.js";
...
export const CALENDAR_LIVE_V17_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V17_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V17_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V17_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V17_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V17_SET_COUNT = 3;
export const CALENDAR_LIVE_V17_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V17_HOST_PHASES = 3;
export const CALENDAR_LIVE_V17_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V17_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V17_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V17_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV17SourceId = (typeof CALENDAR_LIVE_V17_SOURCE_IDS)[number];
export type CalendarLiveV17Kind = "calendar" | "week" | "day";

export interface CalendarLiveV17SourceIdentity {
  source: CalendarLiveV17SourceId;
...
export interface CalendarLiveV17WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV17ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV17SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV17RawNode;
    weekScene: CalendarLiveV17RawNode;
    dayScene: CalendarLiveV17RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV17RootProof {
  source: CalendarLiveV17SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV17FixedFixedPoint;
}

export interface CalendarLiveV17ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV17SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV17CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV17SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV17CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV17SourceId;
  frameWidth: number;
...
export interface CalendarLiveV17CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v17 ${label} must be an object`);
...
export function validateCalendarLiveV17WriterPayload(
  value: unknown,
): CalendarLiveV17WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v17") ||
    ...
    result.namespace !== CALENDAR_LIVE_V17_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v17 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v17 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v17 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV17SourceIdentity,
  kind: CalendarLiveV17Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV17ExtractProgram(
  writer: CalendarLiveV17WriterOwnership,
  sources: readonly CalendarLiveV17SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v17 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V17_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V17_NAMESPACE)}
${buildCalendarLiveV17RawPropertyRuntime()}
...
export function validateCalendarLiveV17ExtractPayload(
  value: unknown,
  writer: CalendarLiveV17WriterOwnership,
): CalendarLiveV17ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v17 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V17_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v17 extract schema/one-root mismatch");
  return payload as CalendarLiveV17ExtractPayload;
}

export function proveCalendarLiveV17Roots<Instance>(
  extract: CalendarLiveV17ExtractPayload,
  sources: readonly (CalendarLiveV17SourceIdentity & {
...
): CalendarLiveV17RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v17 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v17 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV17Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV17Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV17Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV17FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v17 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v17 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v17 compile lost owned sets");
...
export function assertCalendarLiveV17RootProofs(
  proofs: readonly CalendarLiveV17RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v17 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v17 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV17ProbeProgram(
  writer: CalendarLiveV17WriterOwnership,
  sources: readonly CalendarLiveV17SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V17_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV17ProbePayload(
  value: unknown,
  writer: CalendarLiveV17WriterOwnership,
): CalendarLiveV17ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV17ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V17_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V17_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v17 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV17CaptureManifest(
  cells: readonly CalendarLiveV17CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V17_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V17_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V17_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V17_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v17 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV17CaptureCell[],
): string => {
  validateCalendarLiveV17CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV17CaptureProgram(
  writer: CalendarLiveV17WriterOwnership,
  cell: CalendarLiveV17CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V17_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v17 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v17 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V17_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV17CapturePayload(
  value: unknown,
  cell: CalendarLiveV17CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV17CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV17CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V17_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V17_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v17 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV17CaptureResponses(
  manifest: readonly CalendarLiveV17CaptureCell[],
  responses: readonly CalendarLiveV17CapturePayload[],
): void {
  validateCalendarLiveV17CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v17 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV17ObjectiveReport {
  artifactVersion: "calendar-live-v17-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV17Objective(
  manifest: readonly CalendarLiveV17CaptureCell[],
  responses: readonly CalendarLiveV17CapturePayload[],
): CalendarLiveV17ObjectiveReport {
  assertCalendarLiveV17CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v17-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV17CleanupProgram(
  writer: CalendarLiveV17WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV17CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V17_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v17 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV17CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV17WriterOwnership,
): CalendarLiveV17CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV17CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v17 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V17_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV17WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
