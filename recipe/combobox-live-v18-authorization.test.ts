...
import { COMBOBOX_LIVE_V18_INDEX_PATH } from "./build-combobox-live-proof-v18.js";
import {
  COMBOBOX_LIVE_V18_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V18_TARGET,
} from "./combobox-live-v18-broker.js";
import {
  buildComboboxLiveV18AuthorizationArtifact,
  validateComboboxLiveV18AntecedentIndex,
  validateComboboxLiveV18AuthorizationArtifact,
  validateComboboxLiveV18History,
  validateComboboxLiveV18UnitTestSource,
  type ComboboxLiveV18AntecedentIndex,
  type ComboboxLiveV18AuthorizationArtifact,
  type ComboboxLiveV18AuthorizationProof,
  type ComboboxLiveV18HistoryState,
} from "./combobox-live-v18-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV18ControlFlowSource,
  validateComboboxLiveV18SecurityAttestation,
  type ComboboxLiveV18SecurityAttestation,
} from "./combobox-live-v18-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v18-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V18_INDEX_PATH);
const index = (): ComboboxLiveV18AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV18AntecedentIndex;

const artifact = (): ComboboxLiveV18AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV18AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV18HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV18HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV18AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v18/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV18AuthorizationArtifact,
): ComboboxLiveV18SecurityAttestation => {
  const body: Omit<ComboboxLiveV18SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V18_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV18History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV18History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV18History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV18History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV18History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV18UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV18UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV18AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV18AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV18AuthorizationArtifact(
        planted as ComboboxLiveV18AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV18SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV18SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV18SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV18ControlFlowSource(source), []);
});
