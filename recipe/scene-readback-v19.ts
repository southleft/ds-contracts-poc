...
/**
 * Carried Input live v19 scene-readback. Keeps the v17/v18 slot-fill
 * and slot-color teachings, and surfaces `strokes.0.weight` from the
 * existing uniform per-side stroke-weight FLOAT when a surface node's
 * own `strokeWeight` binding is absent. Does not invent variables,
...
export const INPUT_LIVE_V19_SLOT_FILL_MARKER = "INPUT-SLOT-FILL-FROM-PAYLOAD";
export const INPUT_LIVE_V19_SLOT_COLOR_BINDING_MARKER =
  "INPUT-SLOT-COLOR-BINDING-FROM-CHILD";
export const INPUT_LIVE_V19_SURFACE_STROKE_WEIGHT_MARKER =
  "INPUT-SURFACE-STROKE-WEIGHT-FROM-PER-SIDE";
export const INPUT_LIVE_V19_SLOT_ROLES = [
  "input-field/slot/leading",
  "input-field/slot/trailing",
] as const;
export const INPUT_LIVE_V19_SURFACE_ROLES = ["input-field/surface"] as const;
export const INPUT_LIVE_V19_PER_SIDE_STROKE_WEIGHT_FIELDS = [
  "strokeTopWeight",
  "strokeRightWeight",
  "strokeBottomWeight",
  "strokeLeftWeight",
...
const SURFACE_ROLES = new Set<string>(INPUT_LIVE_V19_SURFACE_ROLES);
...
const SLOT_ROLES = new Set<string>(INPUT_LIVE_V19_SLOT_ROLES);
...
export function sceneToNormalizedIr(scene: SceneNodeSnapshot): IRNode {
  const bindings = surfaceUniformStrokeWeightBinding(
    scene,
