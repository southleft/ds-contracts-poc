/**
 * Carried Combobox live v21 host verifier. Uses scene-readback-combobox-v21 so live
...
export const COMBOBOX_LIVE_V21_FIXED_VERIFIER_VERSION = "combobox-live-v21-fixed-point-v1";
export const COMBOBOX_LIVE_V21_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV21FixedThresholds {
...
export interface ComboboxLiveV21FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV21FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV21FixedSourceProbe {
...
export interface ComboboxLiveV21FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV21FixedFixedPoint;
}

export interface ComboboxLiveV21FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV21FixedHardGateInput {
  thresholds: ComboboxLiveV21FixedThresholds;
  materialRegression: ComboboxLiveV21FixedMaterialRegression;
  sourceProbes: ComboboxLiveV21FixedSourceProbe[];
  cells: ComboboxLiveV21FixedCellValidation[];
  sceneProofs: ComboboxLiveV21FixedSceneProof[];
  visualRows: ComboboxLiveV21FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV21FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V21_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV21FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV21FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV21FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV21FixedHardGates(
  input: ComboboxLiveV21FixedHardGateInput,
): ComboboxLiveV21FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV21FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV21FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V21_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV21FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV21FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV21FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
