...
import { verifyCalendarLiveV3Authorization } from "./calendar-live-v3-authorization.js";
import { CALENDAR_LIVE_V3_TARGET } from "./calendar-live-v3-broker.js";
import {
  CALENDAR_LIVE_V3_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV3Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v3-session";
...
const attestation: CalendarLiveV3SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV3SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v3 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V3_SECURITY_ATTESTATION_DEFAULT_PATH,
);
