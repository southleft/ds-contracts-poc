...
import {
  verifyCalendarLiveV5FixedSceneFixedPoint,
  type CalendarLiveV5FixedFixedPoint,
} from "./calendar-live-v5-fixed-point.js";

export const CALENDAR_LIVE_V5_VERIFIER_VERSION = "calendar-live-v5-verifier-v1";

export function buildCalendarLiveV5RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV5RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV5RawNode[];
}

export interface CalendarLiveV5NormalizedScene {
...
export interface CalendarLiveV5SceneProof {
...
export function normalizeCalendarLiveV5Scene(
  root: CalendarLiveV5RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV5NormalizedScene {
  const canonicalBindings: CalendarLiveV5NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV5RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV5Scene<Instance>(
  normalized: CalendarLiveV5NormalizedScene,
...
): CalendarLiveV5SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV5FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV5ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV5PreCaptureGates(payload: unknown): void {
