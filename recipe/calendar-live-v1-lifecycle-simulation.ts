...
import { calendarLiveV1Sha256 } from "./calendar-live-v1-broker.js";

export interface CalendarLiveV1LifecycleSimulationReport {
  artifactVersion: "calendar-live-v1-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV1AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV1Lifecycle(): Promise<CalendarLiveV1LifecycleSimulationReport> {
  await buildCalendarLiveV1Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V1_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV1AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v1-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V1_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V1_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV1History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV1History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV1AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V1_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V1_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV1History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV1History(authorized, "pending");
    ...
    await buildCalendarLiveV1Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V1_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV1AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV1Lifecycle(), null, 2)}\n`,
  );
