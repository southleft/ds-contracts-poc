...
import {
  ...
  createInputLiveV16FakeDynamicResponse,
  ...
  listInputLiveV16PersistedRequests,
  persistInputLiveV16RawResponse,
  persistInputLiveV16TechnicalGates,
  validateInputLiveV16Receipt,
  validateInputLiveV16Request,
  type InputLiveV16RemotePhase,
...
import {
  ...
  type InputLiveV16ExtractPayload,
  type InputLiveV16ProbePayload,
...
import {
  ...
  simulatedInputLiveV16Authorization,
} from "./run-input-field-live-v16.js";
...
const captures: InputLiveV16CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV16Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v16-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV16Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV16CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV16FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV16FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V16_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V16_TARGET);
  persistInputLiveV16RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV16Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV16Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV16WriterOwnership,
...
const rootProofs = (): InputLiveV16RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v16-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV16Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV16Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV16RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV16RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV16ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV16ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV16CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV16CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV16CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV16Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV16CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV16PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV16ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV16Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV16Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV16Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV16Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V16_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV16RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V16_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV16Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV16Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV16ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV16Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV16WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV16RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV16Request;
    validateInputLiveV16Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV16Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV16CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v16-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV16Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV16Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV16Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V16_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV16RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V16_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV16Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV16Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV16RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V16_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV16Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV16Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV16Request;
