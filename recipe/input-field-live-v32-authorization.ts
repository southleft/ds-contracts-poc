...
import {
  ...
  INPUT_LIVE_V32_INDEX_PATH,
} from "./build-input-field-live-proof-v32.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV32HistoryExpectation = "pending" | "authorized";

export interface InputLiveV32AuthorizationArtifact {
  artifactVersion: "input-live-v32-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V32_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV32AntecedentIndex {
...
export interface InputLiveV32HistoryState {
  index?: InputLiveV32AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV32AuthorizationArtifact;
...
export interface InputLiveV32AuthorizationProof extends InputLiveV32TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V32_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V32_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V32_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V32_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v32-status") ||
...
export function validateInputLiveV32AntecedentIndex(
  index: InputLiveV32AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v32-antecedent-index-v1" ||
...
export function validateInputLiveV32UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV32History\(\s*["']/.test(source) ||
    /readInputLiveV32HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV32AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV32AuthorizationArtifact {
  ...
  ) as InputLiveV32AntecedentIndex;
  const indexFailures = validateInputLiveV32AntecedentIndex(index);
...
export function validateInputLiveV32AuthorizationArtifact(
  artifact: InputLiveV32AuthorizationArtifact | undefined,
  index: InputLiveV32AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v32-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v32" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V32_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V32_TARGET) ||
    !equalJson(
...
export function validateInputLiveV32History(
  state: InputLiveV32HistoryState,
  expected: InputLiveV32HistoryExpectation,
): string[] {
  const failures = validateInputLiveV32AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV32AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV32HistoryState(
  root = process.cwd(),
): InputLiveV32HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V32_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV32AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V32_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V32_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V32_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV32History(
  expected: InputLiveV32HistoryExpectation,
  root = process.cwd(),
): InputLiveV32HistoryState {
  const state = readInputLiveV32HistoryState(root);
  const failures = validateInputLiveV32History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV32Authorization(
  root = process.cwd(),
): InputLiveV32AuthorizationProof {
  const state = verifyInputLiveV32History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V32_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v32/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v32.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V32_TARGET,
    expectedDynamicTool: INPUT_LIVE_V32_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V32_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV32HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV32History(expected);
