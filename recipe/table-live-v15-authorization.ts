...
export type TableLiveV15HistoryExpectation = "pending" | "authorized";

export interface TableLiveV15AuthorizationArtifact {
  artifactVersion: "table-live-v15-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V15_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV15AntecedentIndex {
...
export interface TableLiveV15HistoryState {
  index?: TableLiveV15AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV15AuthorizationArtifact;
...
export interface TableLiveV15AuthorizationProof
  extends TableLiveV15TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V15_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V15_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V15_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V15_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v15-authorization") ||
  artifactPath.includes("table-live-v15-preflight") ||
  artifactPath.includes("table-live-v15-authorized") ||
  artifactPath.includes("create-table-live-v15-security-attestation") ||
  artifactPath.includes("table-live-pivot-v15-status") ||
...
export function validateTableLiveV15AntecedentIndex(
  index: TableLiveV15AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v15-antecedent-index-v1" ||
...
export function validateTableLiveV15UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV15History\(\s*["']/.test(source) ||
    /readTableLiveV15HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV15AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV15AuthorizationArtifact {
  ...
  ) as TableLiveV15AntecedentIndex;
  const indexFailures = validateTableLiveV15AntecedentIndex(index);
...
export function validateTableLiveV15AuthorizationArtifact(
  artifact: TableLiveV15AuthorizationArtifact | undefined,
  index: TableLiveV15AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v15-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v15" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V15_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V15_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V15_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V15_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V15_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V15_SOURCE_ROOTS ||
  ...
  if (
    ...
    failures.push(
      "table v15 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
...
export function validateTableLiveV15History(
  state: TableLiveV15HistoryState,
  expected: TableLiveV15HistoryExpectation,
): string[] {
  const failures = validateTableLiveV15AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV15AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV15HistoryState(
  root = process.cwd(),
): TableLiveV15HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V15_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV15AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V15_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V15_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V15_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV15History(
  expected: TableLiveV15HistoryExpectation,
  root = process.cwd(),
): TableLiveV15HistoryState {
  const state = readTableLiveV15HistoryState(root);
  const failures = validateTableLiveV15History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v15 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV15Authorization(
  root = process.cwd(),
): TableLiveV15AuthorizationProof {
  const state = verifyTableLiveV15History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V15_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v15/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v15.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V15_TARGET,
    expectedDynamicTool: TABLE_LIVE_V15_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V15_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV15HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV15History(expected);
  process.stdout.write(
    `Table live v15 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
