...
export interface InputLiveV19LifecycleSimulationReport {
  artifactVersion: "input-live-v19-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV19AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV19Lifecycle(): Promise<InputLiveV19LifecycleSimulationReport> {
  await buildInputLiveV19Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V19_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV19AntecedentIndex;
  const indexSha256 = inputLiveV19Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v19-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V19_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V19_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV19History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV19History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV19AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V19_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V19_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV19History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV19History(authorized, "pending");
    ...
    await buildInputLiveV19Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V19_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV19AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV19Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV19Lifecycle(), null, 2)}\n`,
  );
