...
} from "./input-field-live-v3-verifier-v39.js";
import {
  buildInputLiveV39RawPropertyRuntime,
  normalizeInputLiveV39Scene,
  type InputLiveV39RawNode,
} from "./input-field-live-v39-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v39.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v39.js";
...
export const INPUT_LIVE_V39_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V39_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V39_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V39_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V39_VARIANT_COUNT = 256;
export const INPUT_LIVE_V39_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V39_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV39SourceId = (typeof INPUT_LIVE_V39_SOURCE_IDS)[number];

export interface InputLiveV39SourceIdentity {
  source: InputLiveV39SourceId;
...
export interface InputLiveV39WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV39ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV39SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV39RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV39RootProof {
  source: InputLiveV39SourceId;
  adapterIdentity: string;
...
export interface InputLiveV39ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV39SourceId;
    adapterIdentity: string;
...
export interface InputLiveV39CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV39SourceId;
  adapterIdentity: string;
...
export interface InputLiveV39CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV39SourceId;
  strata: InputLiveV39CaptureCell["strata"];
...
export interface InputLiveV39CleanupPayload {
...
export function validateInputLiveV39WriterPayload(
  value: unknown,
): InputLiveV39WriterOwnership {
...
const identityPlan = (source: InputLiveV39SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV39ExtractProgram(
  writer: InputLiveV39WriterOwnership,
  sources: readonly InputLiveV39SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V39_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V39_NAMESPACE)}
${buildInputLiveV39RawPropertyRuntime()}
...
export function validateInputLiveV39ExtractPayload(
  value: unknown,
  writer: InputLiveV39WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V39_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV39ExtractPayload;
}

export function proveInputLiveV39Roots<Instance>(
  extract: InputLiveV39ExtractPayload,
  sources: readonly (InputLiveV39SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV39Scene(
      root.scene,
...
export function assertInputLiveV39RootProofs(
  proofs: readonly InputLiveV39RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV39ProbeProgram(
  writer: InputLiveV39WriterOwnership,
  sources: readonly InputLiveV39SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V39_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV39ProbePayload(
  value: unknown,
  writer: InputLiveV39WriterOwnership,
): InputLiveV39ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV39ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V39_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V39_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV39CaptureManifest(
  cells: readonly InputLiveV39CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V39_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V39_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V39_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V39_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V39_SOURCE_IDS.some(
...
export const inputLiveV39CaptureManifestSha256 = (
  cells: readonly InputLiveV39CaptureCell[],
): string => {
  validateInputLiveV39CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV39CaptureProgram(
  writer: InputLiveV39WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V39_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V39_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V39_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV39CapturePayload(
  value: unknown,
  cell: InputLiveV39CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV39CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V39_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V39_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV39CaptureResponses(
  manifest: readonly InputLiveV39CaptureCell[],
  responses: readonly InputLiveV39CapturePayload[],
): void {
  validateInputLiveV39CaptureManifest(manifest);
  if (
...
export interface InputLiveV39ObjectiveReport {
  artifactVersion: "input-live-v39-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV39Objective(
  manifest: readonly InputLiveV39CaptureCell[],
  responses: readonly InputLiveV39CapturePayload[],
  readReference: (cell: InputLiveV39CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV39CaptureCell) => Buffer,
): InputLiveV39ObjectiveReport {
  assertInputLiveV39CaptureResponses(manifest, responses);
...
export function buildInputLiveV39CleanupProgram(
  writer: InputLiveV39WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V39_NAMESPACE,
...
export function validateInputLiveV39CleanupPayload(
  value: unknown,
  writer: InputLiveV39WriterOwnership,
): InputLiveV39CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV39CleanupPayload;
...
export const INPUT_LIVE_V39_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV39WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
