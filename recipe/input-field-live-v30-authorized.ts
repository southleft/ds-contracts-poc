...
import { INPUT_LIVE_V30_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v30.js";
import { verifyInputLiveV30Authorization } from "./input-field-live-v30-authorization.js";
import { runInputLiveV30Preflight } from "./input-field-live-v30-preflight.js";
import { InputLiveV30Orchestrator } from "./run-input-field-live-v30.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V30_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v30-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV30Authorization(root);
    runInputLiveV30Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV30Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
