...
import {
  ...
  INPUT_LIVE_V74_INDEX_PATH,
} from "./build-input-field-live-proof-v74.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV74HistoryExpectation = "pending" | "authorized";

export interface InputLiveV74AuthorizationArtifact {
  artifactVersion: "input-live-v74-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V74_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV74AntecedentIndex {
...
export interface InputLiveV74HistoryState {
  index?: InputLiveV74AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV74AuthorizationArtifact;
...
export interface InputLiveV74AuthorizationProof extends InputLiveV74TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V74_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V74_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V74_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V74_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v74-authorization") ||
  artifactPath.includes("input-field-live-v74-preflight") ||
  artifactPath.includes("input-field-live-v74-authorized") ||
  artifactPath.includes("input-field-live-pivot-v74-status") ||
...
export function validateInputLiveV74AntecedentIndex(
  index: InputLiveV74AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v74-antecedent-index-v1" ||
...
export function validateInputLiveV74UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV74History\(\s*["']/.test(source) ||
    /readInputLiveV74HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV74AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV74AuthorizationArtifact {
  ...
  ) as InputLiveV74AntecedentIndex;
  const indexFailures = validateInputLiveV74AntecedentIndex(index);
...
export function validateInputLiveV74AuthorizationArtifact(
  artifact: InputLiveV74AuthorizationArtifact | undefined,
  index: InputLiveV74AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v74-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v74" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V74_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V74_TARGET) ||
    !equalJson(
...
export function validateInputLiveV74History(
  state: InputLiveV74HistoryState,
  expected: InputLiveV74HistoryExpectation,
): string[] {
  const failures = validateInputLiveV74AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV74AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV74HistoryState(
  root = process.cwd(),
): InputLiveV74HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V74_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV74AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V74_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V74_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V74_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV74History(
  expected: InputLiveV74HistoryExpectation,
  root = process.cwd(),
): InputLiveV74HistoryState {
  const state = readInputLiveV74HistoryState(root);
  const failures = validateInputLiveV74History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV74Authorization(
  root = process.cwd(),
): InputLiveV74AuthorizationProof {
  const state = verifyInputLiveV74History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V74_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v74/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v74.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V74_TARGET,
    expectedDynamicTool: INPUT_LIVE_V74_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V74_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV74HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV74History(expected);
