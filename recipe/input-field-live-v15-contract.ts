...
} from "./input-field-live-v3-verifier-v15.js";
import {
  buildInputLiveV15RawPropertyRuntime,
  normalizeInputLiveV15Scene,
  type InputLiveV15RawNode,
} from "./input-field-live-v15-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v15.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v15.js";
...
export const INPUT_LIVE_V15_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V15_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V15_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V15_VARIANT_COUNT = 256;
export const INPUT_LIVE_V15_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V15_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV15SourceId = (typeof INPUT_LIVE_V15_SOURCE_IDS)[number];

export interface InputLiveV15SourceIdentity {
  source: InputLiveV15SourceId;
...
export interface InputLiveV15WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV15ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV15SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV15RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV15RootProof {
  source: InputLiveV15SourceId;
  adapterIdentity: string;
...
export interface InputLiveV15ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV15SourceId;
    adapterIdentity: string;
...
export interface InputLiveV15CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV15SourceId;
  adapterIdentity: string;
...
export interface InputLiveV15CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV15SourceId;
  strata: InputLiveV15CaptureCell["strata"];
...
export interface InputLiveV15CleanupPayload {
...
export function validateInputLiveV15WriterPayload(
  value: unknown,
): InputLiveV15WriterOwnership {
...
const identityPlan = (source: InputLiveV15SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV15ExtractProgram(
  writer: InputLiveV15WriterOwnership,
  sources: readonly InputLiveV15SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V15_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V15_NAMESPACE)}
${buildInputLiveV15RawPropertyRuntime()}
...
export function validateInputLiveV15ExtractPayload(
  value: unknown,
  writer: InputLiveV15WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V15_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV15ExtractPayload;
}

export function proveInputLiveV15Roots<Instance>(
  extract: InputLiveV15ExtractPayload,
  sources: readonly (InputLiveV15SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV15Scene(
      root.scene,
...
export function assertInputLiveV15RootProofs(
  proofs: readonly InputLiveV15RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV15ProbeProgram(
  writer: InputLiveV15WriterOwnership,
  sources: readonly InputLiveV15SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V15_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV15ProbePayload(
  value: unknown,
  writer: InputLiveV15WriterOwnership,
): InputLiveV15ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV15ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V15_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V15_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV15CaptureManifest(
  cells: readonly InputLiveV15CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V15_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V15_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V15_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V15_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V15_SOURCE_IDS.some(
...
export const inputLiveV15CaptureManifestSha256 = (
  cells: readonly InputLiveV15CaptureCell[],
): string => {
  validateInputLiveV15CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV15CaptureProgram(
  writer: InputLiveV15WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V15_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V15_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V15_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV15CapturePayload(
  value: unknown,
  cell: InputLiveV15CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV15CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V15_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V15_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV15CaptureResponses(
  manifest: readonly InputLiveV15CaptureCell[],
  responses: readonly InputLiveV15CapturePayload[],
): void {
  validateInputLiveV15CaptureManifest(manifest);
  if (
...
export interface InputLiveV15ObjectiveReport {
  artifactVersion: "input-live-v15-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV15Objective(
  manifest: readonly InputLiveV15CaptureCell[],
  responses: readonly InputLiveV15CapturePayload[],
  readReference: (cell: InputLiveV15CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV15CaptureCell) => Buffer,
): InputLiveV15ObjectiveReport {
  assertInputLiveV15CaptureResponses(manifest, responses);
...
export function buildInputLiveV15CleanupProgram(
  writer: InputLiveV15WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V15_NAMESPACE,
...
export function validateInputLiveV15CleanupPayload(
  value: unknown,
  writer: InputLiveV15WriterOwnership,
): InputLiveV15CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV15CleanupPayload;
...
export const INPUT_LIVE_V15_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV15WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
