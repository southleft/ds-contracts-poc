...
import {
  assertInputLiveV84RootProofs,
  INPUT_LIVE_V84_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V84_RESPONSE_CONTRACTS,
  type InputLiveV84RootProof,
} from "./input-field-live-v84-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V84_BROKER_VERSION = "input-live-v84-broker-v2";
export const INPUT_LIVE_V84_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V84_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V84_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V84_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V84_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V84_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV84RemotePhase =
  (typeof INPUT_LIVE_V84_REMOTE_PHASES)[number];
export type InputLiveV84AuthorizationMode = "live" | "simulated";

export interface InputLiveV84CommitPins {
...
export interface InputLiveV84TransactionAuthorization extends InputLiveV84CommitPins {
  mode: InputLiveV84AuthorizationMode;
...
export interface InputLiveV84Manifest {
  artifactVersion: "input-live-v84-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V84_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V84_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V84_TARGET;
  authorization: InputLiveV84TransactionAuthorization;
...
export interface InputLiveV84ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV84RequestBody {
  artifactVersion: "input-live-v84-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V84_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV84RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV84CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V84_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV84ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV84Request extends InputLiveV84RequestBody {
...
export interface InputLiveV84Receipt {
  artifactVersion: "input-live-v84-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V84_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV84RemotePhase;
...
export interface InputLiveV84AcceptedResponse<T = unknown> {
  request: InputLiveV84Request;
  receipt: InputLiveV84Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV84IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV84ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V84_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V84_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V84_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V84_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V84_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V84_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V84_TARGET.editorType;
};
...
export const inputLiveV84RequestSequence = (
  phase: InputLiveV84RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV84RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV84RequestPath = (
  directory: string,
  phase: InputLiveV84RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV84ResponsePath = (
  directory: string,
  phase: InputLiveV84RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV84ReceiptPath = (
  directory: string,
  phase: InputLiveV84RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV84Manifest => {
  const manifest = json<InputLiveV84Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v84-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V84_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V84_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV84RemotePhase,
  captureIndex?: number,
): InputLiveV84Request | undefined => {
  const file = inputLiveV84RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV84Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV84RemotePhase,
  captureIndex?: number,
): InputLiveV84Receipt | undefined => {
  const file = inputLiveV84ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV84Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV84Request | undefined;
  receipt: InputLiveV84Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV84RemotePhase,
): InputLiveV84ResponseContract => ({
  ...INPUT_LIVE_V84_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V84_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v84-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V84_BROKER_VERSION,
  target: INPUT_LIVE_V84_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V84_PROGRAM_START*/${program}/*INPUT_LIVE_V84_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V84_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V84_PROGRAM_END*/";
...
export function createInputLiveV84Transaction(
  directory: string,
  ...
  const manifest: InputLiveV84Manifest = {
    artifactVersion: "input-live-v84-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V84_BROKER_VERSION,
...
export function validateInputLiveV84Request(
  directory: string,
  request: InputLiveV84Request,
): void {
  ...
  const expectedSequence = inputLiveV84RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v84-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V84_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V84_TARGET.fileKey ||
...
export function issueInputLiveV84Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV84RemotePhase,
  program: string,
  options: InputLiveV84IssueOptions = {},
): InputLiveV84Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV84RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV84Request>(targetPath);
    validateInputLiveV84Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV84ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV84TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV84RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V84_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V84_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V84_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV84RequestBody = {
    artifactVersion: "input-live-v84-broker-request-v2",
    brokerVersion: INPUT_LIVE_V84_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV84Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV84Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV84RawResponse(
  directory: string,
  phase: InputLiveV84RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV84RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV84Request>(requestFile);
  if (
  ...
  const target = inputLiveV84ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV84Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v84-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V84_BROKER_VERSION,
...
export function acceptInputLiveV84Response<T = unknown>(
  directory: string,
  phase: InputLiveV84RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV84AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV84ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV84Request>(
    inputLiveV84RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV84Request(directory, request);
  if (phase === "capture") validateInputLiveV84TechnicalGates(directory);
  const rawFile = inputLiveV84ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v84-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V84_BROKER_VERSION ||
...
export function persistInputLiveV84TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV84RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV84RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V84_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v84-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V84_BROKER_VERSION,
...
export function validateInputLiveV84TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v84-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V84_TECHNICAL_GATES.some(
...
export function validateInputLiveV84Receipt(
  directory: string,
  phase: InputLiveV84RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV84Request(directory, request);
  const receipt = json<InputLiveV84Receipt>(
    inputLiveV84ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV84ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v84-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V84_BROKER_VERSION ||
...
export function readInputLiveV84AcceptedRawPayload(
  directory: string,
  phase: InputLiveV84RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV84Receipt(directory, phase, captureIndex);
...
export function createInputLiveV84FakeDynamicResponse(
  request: InputLiveV84Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V84_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V84_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV84PersistedRequests(
  directory: string,
): InputLiveV84Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV84Request>(file)] : [];
    });
}

export const inputLiveV84Sha256 = sha256;
