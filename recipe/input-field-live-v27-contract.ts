...
} from "./input-field-live-v3-verifier-v27.js";
import {
  buildInputLiveV27RawPropertyRuntime,
  normalizeInputLiveV27Scene,
  type InputLiveV27RawNode,
} from "./input-field-live-v27-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v27.js";
import {
...
export const INPUT_LIVE_V27_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V27_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V27_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V27_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V27_VARIANT_COUNT = 256;
export const INPUT_LIVE_V27_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V27_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV27SourceId = (typeof INPUT_LIVE_V27_SOURCE_IDS)[number];

export interface InputLiveV27SourceIdentity {
  source: InputLiveV27SourceId;
...
export interface InputLiveV27WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV27ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV27SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV27RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV27RootProof {
  source: InputLiveV27SourceId;
  adapterIdentity: string;
...
export interface InputLiveV27ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV27SourceId;
    adapterIdentity: string;
...
export interface InputLiveV27CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV27SourceId;
  adapterIdentity: string;
...
export interface InputLiveV27CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV27SourceId;
  strata: InputLiveV27CaptureCell["strata"];
...
export interface InputLiveV27CleanupPayload {
...
export function validateInputLiveV27WriterPayload(
  value: unknown,
): InputLiveV27WriterOwnership {
...
const identityPlan = (source: InputLiveV27SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV27ExtractProgram(
  writer: InputLiveV27WriterOwnership,
  sources: readonly InputLiveV27SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V27_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V27_NAMESPACE)}
${buildInputLiveV27RawPropertyRuntime()}
...
export function validateInputLiveV27ExtractPayload(
  value: unknown,
  writer: InputLiveV27WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V27_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV27ExtractPayload;
}

export function proveInputLiveV27Roots<Instance>(
  extract: InputLiveV27ExtractPayload,
  sources: readonly (InputLiveV27SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV27Scene(
      root.scene,
...
export function assertInputLiveV27RootProofs(
  proofs: readonly InputLiveV27RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV27ProbeProgram(
  writer: InputLiveV27WriterOwnership,
  sources: readonly InputLiveV27SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V27_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV27ProbePayload(
  value: unknown,
  writer: InputLiveV27WriterOwnership,
): InputLiveV27ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV27ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V27_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V27_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV27CaptureManifest(
  cells: readonly InputLiveV27CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V27_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V27_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V27_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V27_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V27_SOURCE_IDS.some(
...
export const inputLiveV27CaptureManifestSha256 = (
  cells: readonly InputLiveV27CaptureCell[],
): string => {
  validateInputLiveV27CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV27CaptureProgram(
  writer: InputLiveV27WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V27_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V27_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V27_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV27CapturePayload(
  value: unknown,
  cell: InputLiveV27CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV27CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V27_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V27_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV27CaptureResponses(
  manifest: readonly InputLiveV27CaptureCell[],
  responses: readonly InputLiveV27CapturePayload[],
): void {
  validateInputLiveV27CaptureManifest(manifest);
  if (
...
export interface InputLiveV27ObjectiveReport {
  artifactVersion: "input-live-v27-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV27Objective(
  manifest: readonly InputLiveV27CaptureCell[],
  responses: readonly InputLiveV27CapturePayload[],
  readReference: (cell: InputLiveV27CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV27CaptureCell) => Buffer,
): InputLiveV27ObjectiveReport {
  assertInputLiveV27CaptureResponses(manifest, responses);
...
export function buildInputLiveV27CleanupProgram(
  writer: InputLiveV27WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V27_NAMESPACE,
...
export function validateInputLiveV27CleanupPayload(
  value: unknown,
  writer: InputLiveV27WriterOwnership,
): InputLiveV27CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV27CleanupPayload;
...
export const INPUT_LIVE_V27_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV27WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
