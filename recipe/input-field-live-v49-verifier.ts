...
export const INPUT_LIVE_V49_VERIFIER_VERSION = "input-live-v49-verifier-v1";

export function buildInputLiveV49RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV49RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV49RawNode[];
}

export interface InputLiveV49NormalizedScene {
...
export interface InputLiveV49SceneProof {
...
export function normalizeInputLiveV49Scene(
  root: InputLiveV49RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV49NormalizedScene {
  const canonicalBindings: InputLiveV49NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV49RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV49Scene<Instance>(
  normalized: InputLiveV49NormalizedScene,
...
export function compileInputLiveV49ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV49PreCaptureGates(payload: unknown): void {
