...
import {
  assertInputLiveV53RootProofs,
  INPUT_LIVE_V53_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V53_RESPONSE_CONTRACTS,
  type InputLiveV53RootProof,
} from "./input-field-live-v53-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V53_BROKER_VERSION = "input-live-v53-broker-v2";
export const INPUT_LIVE_V53_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V53_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V53_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V53_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V53_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V53_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV53RemotePhase =
  (typeof INPUT_LIVE_V53_REMOTE_PHASES)[number];
export type InputLiveV53AuthorizationMode = "live" | "simulated";

export interface InputLiveV53CommitPins {
...
export interface InputLiveV53TransactionAuthorization extends InputLiveV53CommitPins {
  mode: InputLiveV53AuthorizationMode;
...
export interface InputLiveV53Manifest {
  artifactVersion: "input-live-v53-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V53_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V53_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V53_TARGET;
  authorization: InputLiveV53TransactionAuthorization;
...
export interface InputLiveV53ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV53RequestBody {
  artifactVersion: "input-live-v53-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V53_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV53RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV53CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V53_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV53ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV53Request extends InputLiveV53RequestBody {
...
export interface InputLiveV53Receipt {
  artifactVersion: "input-live-v53-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V53_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV53RemotePhase;
...
export interface InputLiveV53AcceptedResponse<T = unknown> {
  request: InputLiveV53Request;
  receipt: InputLiveV53Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV53IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV53ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V53_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V53_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V53_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V53_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V53_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V53_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V53_TARGET.editorType;
};
...
export const inputLiveV53RequestSequence = (
  phase: InputLiveV53RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV53RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV53RequestPath = (
  directory: string,
  phase: InputLiveV53RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV53ResponsePath = (
  directory: string,
  phase: InputLiveV53RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV53ReceiptPath = (
  directory: string,
  phase: InputLiveV53RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV53Manifest => {
  const manifest = json<InputLiveV53Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v53-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V53_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V53_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV53RemotePhase,
  captureIndex?: number,
): InputLiveV53Request | undefined => {
  const file = inputLiveV53RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV53Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV53RemotePhase,
  captureIndex?: number,
): InputLiveV53Receipt | undefined => {
  const file = inputLiveV53ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV53Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV53Request | undefined;
  receipt: InputLiveV53Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV53RemotePhase,
): InputLiveV53ResponseContract => ({
  ...INPUT_LIVE_V53_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V53_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V53_DYNAMIC_TOOL,
      target: INPUT_LIVE_V53_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v53-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V53_BROKER_VERSION,
  target: INPUT_LIVE_V53_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V53_PROGRAM_START*/${program}/*INPUT_LIVE_V53_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V53_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V53_PROGRAM_END*/";
...
export function createInputLiveV53Transaction(
  directory: string,
  ...
  const manifest: InputLiveV53Manifest = {
    artifactVersion: "input-live-v53-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V53_BROKER_VERSION,
...
export function validateInputLiveV53Request(
  directory: string,
  request: InputLiveV53Request,
): void {
  ...
  const expectedSequence = inputLiveV53RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v53-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V53_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V53_TARGET.fileKey ||
...
export function issueInputLiveV53Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV53RemotePhase,
  program: string,
  options: InputLiveV53IssueOptions = {},
): InputLiveV53Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV53RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV53Request>(targetPath);
    validateInputLiveV53Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV53ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV53TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV53RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V53_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V53_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V53_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV53RequestBody = {
    artifactVersion: "input-live-v53-broker-request-v2",
    brokerVersion: INPUT_LIVE_V53_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV53Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV53Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV53RawResponse(
  directory: string,
  phase: InputLiveV53RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV53RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV53Request>(requestFile);
  if (
  ...
  const target = inputLiveV53ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV53Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v53-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V53_BROKER_VERSION,
...
export function acceptInputLiveV53Response<T = unknown>(
  directory: string,
  phase: InputLiveV53RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV53AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV53ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV53Request>(
    inputLiveV53RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV53Request(directory, request);
  if (phase === "capture") validateInputLiveV53TechnicalGates(directory);
  const rawFile = inputLiveV53ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v53-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V53_BROKER_VERSION ||
...
export function persistInputLiveV53TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV53RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV53RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V53_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v53-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V53_BROKER_VERSION,
...
export function validateInputLiveV53TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v53-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V53_TECHNICAL_GATES.some(
...
export function validateInputLiveV53Receipt(
  directory: string,
  phase: InputLiveV53RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV53Request(directory, request);
  const receipt = json<InputLiveV53Receipt>(
    inputLiveV53ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV53ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v53-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V53_BROKER_VERSION ||
...
export function readInputLiveV53AcceptedRawPayload(
  directory: string,
  phase: InputLiveV53RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV53Receipt(directory, phase, captureIndex);
...
export function createInputLiveV53FakeDynamicResponse(
  request: InputLiveV53Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V53_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V53_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV53PersistedRequests(
  directory: string,
): InputLiveV53Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV53Request>(file)] : [];
    });
}

export const inputLiveV53Sha256 = sha256;
