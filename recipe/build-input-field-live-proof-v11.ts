...
import {
  INPUT_LIVE_V11_DYNAMIC_TOOL,
  INPUT_LIVE_V11_TARGET,
  inputLiveV11RequestSequence,
} from "./input-field-live-v11-broker.js";

export const INPUT_LIVE_V11_EVIDENCE_ROOT =
  "recipe/evidence/input-field-live-pivot-v11";
export const INPUT_LIVE_V11_PROTOCOL_PATH = `${INPUT_LIVE_V11_EVIDENCE_ROOT}/protocol.json`;
export const INPUT_LIVE_V11_PLAN_PATH = `${INPUT_LIVE_V11_EVIDENCE_ROOT}/proof-plan.json`;
export const INPUT_LIVE_V11_CAPTURE_MANIFEST_PATH = `${INPUT_LIVE_V11_EVIDENCE_ROOT}/capture-manifest.json`;
export const INPUT_LIVE_V11_REQUEST_MANIFEST_PATH = `${INPUT_LIVE_V11_EVIDENCE_ROOT}/request-manifest.json`;
export const INPUT_LIVE_V11_INDEX_PATH = `${INPUT_LIVE_V11_EVIDENCE_ROOT}/antecedent-index.json`;
export const INPUT_LIVE_V11_AUTHORIZATION_TEMPLATE_PATH = `${INPUT_LIVE_V11_EVIDENCE_ROOT}/authorization-template.json`;
export const INPUT_LIVE_V11_AUTHORIZATION_PATH = `${INPUT_LIVE_V11_EVIDENCE_ROOT}/capture-authorization.json`;

const V10_PROOF_ROOT = "recipe/evidence/input-field-live-pivot-v10";
const V8_PLAN = `${V10_PROOF_ROOT}/proof-plan.json`;
const V8_CAPTURE_MANIFEST = `${V10_PROOF_ROOT}/capture-manifest.json`;
...
export interface InputLiveV11ProofPlan {
  artifactVersion: "input-live-v11-proof-plan-v1";
...
interface GeneratedSource {
  ...
  recipeHash: string;
  envelopeHash: string;
  expectedScenePlan: InputLiveV11SourceIdentity["expectedScenePlan"];
  outputPath: string;
  compressed: Buffer;
  metadata: InputLiveV11ProofPlan["sources"][number]["expectedScenePlan"];
}
...
const sourcePlans = (): GeneratedSource[] => {
  ...
  return prior.sources.map((source: Record<string, any>) => {
    ...
    if (
      ...
      throw new Error(`Input live v11 source plan drift: ${inputPath}`);
    return {
      source: sourceId,
      ...
      outputPath: `${INPUT_LIVE_V11_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
      compressed,
      metadata: {
        path: `${INPUT_LIVE_V11_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
...
const captureCells = (): InputLiveV11CaptureCell[] => {
  const cells = json<Record<string, any>>(V8_CAPTURE_MANIFEST)
    .cells as InputLiveV11CaptureCell[];
  inputLiveV11CaptureManifestSha256(cells);
  return cells;
};

const protocol = () => ({
  artifactVersion: "input-live-v11-external-operator-protocol-v1",
  protocolId: "input-live-v11",
  status:
  ...
  hostNormalization: {
    ...
    carriedSceneReadback: "recipe/scene-readback-v11.ts",
    carriedSceneReadbackRuntime: "recipe/scene-readback-runtime-v11.ts",
    carriedV3Verifier: "recipe/input-field-live-v3-verifier-v11.ts",
...
const requestManifest = (
  captures: readonly InputLiveV11CaptureCell[],
  programs: {
    writerSha256: string;
  ...
  const requests = [
    {
      requestId: "writer",
      sequence: inputLiveV11RequestSequence("writer"),
      phase: "writer",
      predecessorRequestId: null,
      programSha256: programs.writerSha256,
      response: INPUT_LIVE_V11_RESPONSE_CONTRACTS.writer,
    },
    {
      requestId: "cleanup",
      sequence: inputLiveV11RequestSequence("cleanup"),
      phase: "cleanup",
      predecessorRequestId: "writer",
      programBuilderSha256: programs.cleanupBuilderSha256,
      response: INPUT_LIVE_V11_RESPONSE_CONTRACTS.cleanup,
      availability: "persisted immediately after writer acceptance",
    },
    {
      requestId: "extract",
      sequence: inputLiveV11RequestSequence("extract"),
      phase: "extract",
      predecessorRequestId: "writer",
      programSha256: programs.extractBlueprintSha256,
      response: INPUT_LIVE_V11_RESPONSE_CONTRACTS.extract,
    },
    {
      requestId: "probe",
      sequence: inputLiveV11RequestSequence("probe"),
      phase: "probe",
      predecessorRequestId: "extract",
      programSha256: programs.probeBlueprintSha256,
      response: INPUT_LIVE_V11_RESPONSE_CONTRACTS.probe,
    },
    ...captures.map((cell) => ({
      requestId: `capture-${String(cell.index).padStart(3, "0")}`,
      sequence: inputLiveV11RequestSequence("capture", cell.index),
      phase: "capture",
      ...
      response: INPUT_LIVE_V11_RESPONSE_CONTRACTS.capture,
    })),
  ];
  return {
    artifactVersion: "input-live-v11-request-manifest-v1",
    expectedDynamicTool: INPUT_LIVE_V11_DYNAMIC_TOOL,
    target: INPUT_LIVE_V11_TARGET,
...
const lifecycleExcludedPaths = [
  INPUT_LIVE_V11_AUTHORIZATION_TEMPLATE_PATH,
  INPUT_LIVE_V11_AUTHORIZATION_PATH,
  "recipe/input-field-live-v11-authorization.ts",
  "recipe/input-field-live-v11-authorization.test.ts",
  "recipe/input-field-live-v11-preflight.ts",
  "recipe/input-field-live-v11-authorized.ts",
  "recipe/evidence/input-field-live-pivot-v11/operator-security-attestation-template.json",
...
export async function buildInputLiveV11Proof(
  check = process.argv.includes("--check"),
): Promise<InputLiveV11ProofPlan> {
  ...
  if (
    ...
    throw new Error("Input live v11 exact committed writer bytes drifted");

  const writerOwnershipBlueprint: InputLiveV11WriterOwnership = {
    pageId: "__WRITER_PAGE_ID__",
    pageName: "Recipe Pivot / Input Field / v9",
    runIdentity: "input-live-v11",
  ...
  const identities = sources.map(
    (source) =>
      ...
      }) satisfies InputLiveV11SourceIdentity,
  );
  const extractBlueprint = Buffer.from(
    buildInputLiveV11ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildInputLiveV11ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildInputLiveV11CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );

  const antecedentOutputs = new Map<string, Buffer>();
  antecedentOutputs.set(INPUT_LIVE_V11_PROTOCOL_PATH, jsonBytes(protocol()));
  antecedentOutputs.set(
    `${INPUT_LIVE_V11_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V11_EVIDENCE_ROOT}/programs/writer-payload.js`,
    writerPayload,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V11_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V11_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V11_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  ...
  const captureManifest = {
    artifactVersion: "input-live-v11-capture-manifest-v1",
    ...
    cells: captures,
    cellsSha256: inputLiveV11CaptureManifestSha256(captures),
  };
  ...
  const proofPlan: InputLiveV11ProofPlan = {
    artifactVersion: "input-live-v11-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: INPUT_LIVE_V11_TARGET,
    namespace: "ds.contracts.input.recipe.v5",
    writer: {
      programPath: `${INPUT_LIVE_V11_EVIDENCE_ROOT}/programs/writer.txt`,
      programBytes: writerProgram.byteLength,
      programSha256: sha256(writerProgram),
      payloadPath: `${INPUT_LIVE_V11_EVIDENCE_ROOT}/programs/writer-payload.js`,
    ...
    captureManifest: {
      path: INPUT_LIVE_V11_CAPTURE_MANIFEST_PATH,
      cells: 128,
      requests: 128,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(INPUT_LIVE_V11_PLAN_PATH, proofPlanBytes);
  ...
  antecedentOutputs.set(
    INPUT_LIVE_V11_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/input-field-live-v11-broker.ts",
    "recipe/input-field-live-v11-contract.ts",
    "recipe/run-input-field-live-v11.ts",
    "recipe/build-input-field-live-proof-v11.ts",
    "recipe/input-field-live-v11-broker.test.ts",
    "recipe/figma-property-normalizer-v8.test.ts",
    "recipe/scene-readback-v11.test.ts",
    "recipe/input-field-live-v11-lifecycle-simulation.ts",
    ...sourceNeutralDependencies,
  ].filter((artifactPath) => artifactPath !== INPUT_LIVE_V11_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "input-live-v11-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    ...
    counts: {
      ...
      captureCells: INPUT_LIVE_V11_CAPTURE_COUNT,
  ...
  antecedentOutputs.set(INPUT_LIVE_V11_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "input-live-v11-authorization-template-v1",
    authorizationId: "input-live-v11",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: INPUT_LIVE_V11_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Input live v11 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(INPUT_LIVE_V11_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(INPUT_LIVE_V11_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(INPUT_LIVE_V11_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(INPUT_LIVE_V11_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Input live v11 authorization template drift");
  }
  if (
    ...
    throw new Error("Input live v11 deterministic hash/count invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildInputLiveV11Proof(), null, 2)}\n`,
  );
