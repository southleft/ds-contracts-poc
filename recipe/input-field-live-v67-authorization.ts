...
import {
  ...
  INPUT_LIVE_V67_INDEX_PATH,
} from "./build-input-field-live-proof-v67.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV67HistoryExpectation = "pending" | "authorized";

export interface InputLiveV67AuthorizationArtifact {
  artifactVersion: "input-live-v67-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V67_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV67AntecedentIndex {
...
export interface InputLiveV67HistoryState {
  index?: InputLiveV67AntecedentIndex;
  indexSha256: string;
...
export interface InputLiveV67AuthorizationProof extends InputLiveV67TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V67_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V67_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V67_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V67_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v67-authorization") ||
  artifactPath.includes("input-field-live-v67-preflight") ||
  artifactPath.includes("input-field-live-v67-authorized") ||
  artifactPath.includes("input-field-live-pivot-v67-status") ||
...
export function validateInputLiveV67AntecedentIndex(
  index: InputLiveV67AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v67-antecedent-index-v1" ||
...
export function validateInputLiveV67UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV67History\(\s*["']/.test(source) ||
    /readInputLiveV67HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV67AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV67AuthorizationArtifact {
  ...
  ) as InputLiveV67AntecedentIndex;
  const indexFailures = validateInputLiveV67AntecedentIndex(index);
...
export function validateInputLiveV67AuthorizationArtifact(
  artifact: InputLiveV67AuthorizationArtifact | undefined,
  index: InputLiveV67AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v67-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v67" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V67_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V67_TARGET) ||
    !equalJson(
...
export function validateInputLiveV67History(
  state: InputLiveV67HistoryState,
  expected: InputLiveV67HistoryExpectation,
): string[] {
  const failures = validateInputLiveV67AntecedentIndex(state.index);
...
export function readInputLiveV67HistoryState(
  root = process.cwd(),
): InputLiveV67HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V67_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV67AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V67_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V67_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V67_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV67History(
  expected: InputLiveV67HistoryExpectation,
  root = process.cwd(),
): InputLiveV67HistoryState {
  const state = readInputLiveV67HistoryState(root);
  const failures = validateInputLiveV67History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV67Authorization(
  root = process.cwd(),
): InputLiveV67AuthorizationProof {
  const state = verifyInputLiveV67History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V67_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v67/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v67.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V67_TARGET,
    expectedDynamicTool: INPUT_LIVE_V67_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V67_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV67HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV67History(expected);
