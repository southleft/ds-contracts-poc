...
import { buildCalendarLiveV25CleanupRuntime } from "./calendar-live-v25-cleanup.js";
import { type CalendarLiveV25FixedFixedPoint } from "./calendar-live-v25-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV25RestoreProgram,
  validateCalendarLiveV25RestorePayload,
  type CalendarLiveV25RestorePayload,
} from "./calendar-live-v25-restore.js";
import {
  buildCalendarLiveV25RawPropertyRuntime,
  normalizeCalendarLiveV25Scene,
  type CalendarLiveV25RawNode,
} from "./calendar-live-v25-verifier.js";
...
export const CALENDAR_LIVE_V25_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V25_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V25_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V25_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V25_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V25_SET_COUNT = 3;
export const CALENDAR_LIVE_V25_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V25_HOST_PHASES = 3;
export const CALENDAR_LIVE_V25_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V25_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V25_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V25_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV25SourceId = (typeof CALENDAR_LIVE_V25_SOURCE_IDS)[number];
export type CalendarLiveV25Kind = "calendar" | "week" | "day";

export interface CalendarLiveV25SourceIdentity {
  source: CalendarLiveV25SourceId;
...
export interface CalendarLiveV25WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV25ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV25SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV25RawNode;
    weekScene: CalendarLiveV25RawNode;
    dayScene: CalendarLiveV25RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV25RootProof {
  source: CalendarLiveV25SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV25FixedFixedPoint;
}

export interface CalendarLiveV25ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV25SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV25CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV25SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV25CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV25SourceId;
  frameWidth: number;
...
export interface CalendarLiveV25CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v25 ${label} must be an object`);
...
export function validateCalendarLiveV25WriterPayload(
  value: unknown,
): CalendarLiveV25WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v25") ||
    ...
    result.namespace !== CALENDAR_LIVE_V25_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v25 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v25 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v25 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV25SourceIdentity,
  kind: CalendarLiveV25Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV25ExtractProgram(
  writer: CalendarLiveV25WriterOwnership,
  sources: readonly CalendarLiveV25SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v25 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V25_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V25_NAMESPACE)}
${buildCalendarLiveV25RawPropertyRuntime()}
...
export function validateCalendarLiveV25ExtractPayload(
  value: unknown,
  writer: CalendarLiveV25WriterOwnership,
): CalendarLiveV25ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v25 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V25_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v25 extract schema/one-root mismatch");
  return payload as CalendarLiveV25ExtractPayload;
}

export function proveCalendarLiveV25Roots<Instance>(
  extract: CalendarLiveV25ExtractPayload,
  sources: readonly (CalendarLiveV25SourceIdentity & {
...
): CalendarLiveV25RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v25 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v25 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV25Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV25Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV25Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV25FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v25 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v25 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v25 compile lost owned sets");
...
export function assertCalendarLiveV25RootProofs(
  proofs: readonly CalendarLiveV25RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v25 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v25 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV25ProbeProgram(
  writer: CalendarLiveV25WriterOwnership,
  sources: readonly CalendarLiveV25SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V25_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV25ProbePayload(
  value: unknown,
  writer: CalendarLiveV25WriterOwnership,
): CalendarLiveV25ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV25ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V25_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V25_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v25 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV25CaptureManifest(
  cells: readonly CalendarLiveV25CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V25_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V25_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V25_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V25_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v25 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV25CaptureCell[],
): string => {
  validateCalendarLiveV25CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV25CaptureProgram(
  writer: CalendarLiveV25WriterOwnership,
  cell: CalendarLiveV25CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V25_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v25 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v25 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V25_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV25CapturePayload(
  value: unknown,
  cell: CalendarLiveV25CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV25CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV25CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V25_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V25_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v25 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV25CaptureResponses(
  manifest: readonly CalendarLiveV25CaptureCell[],
  responses: readonly CalendarLiveV25CapturePayload[],
): void {
  validateCalendarLiveV25CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v25 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV25ObjectiveReport {
  artifactVersion: "calendar-live-v25-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV25Objective(
  manifest: readonly CalendarLiveV25CaptureCell[],
  responses: readonly CalendarLiveV25CapturePayload[],
): CalendarLiveV25ObjectiveReport {
  assertCalendarLiveV25CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v25-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV25CleanupProgram(
  writer: CalendarLiveV25WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV25CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V25_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v25 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV25CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV25WriterOwnership,
): CalendarLiveV25CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV25CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v25 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V25_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV25WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
