...
import {
  ...
  readInputLiveV54AcceptedRawPayload,
...
import {
  ...
  buildInputLiveV54CleanupProgram,
...
import {
  ...
  INPUT_LIVE_V54_PLAN_PATH,
  type InputLiveV54ProofPlan,
} from "./build-input-field-live-proof-v54.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-v54.js";

export const INPUT_LIVE_V54_RUNNER_VERSION = "input-live-v54-runner-v1";

interface InputLiveV54JournalEntry {
  index: number;
  event: string;
...
interface InputLiveV54OrchestratorState {
  artifactVersion: "input-live-v54-orchestrator-state-v1";
  ...
  journal: InputLiveV54JournalEntry[];
}

interface LoadedProofInputs {
  plan: InputLiveV54ProofPlan;
  planSha256: string;
  captures: InputLiveV54CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    InputLiveV54SourceIdentity & {
...
export interface InputLiveV54NextAction {
...
const writeState = (
  directory: string,
  state: InputLiveV54OrchestratorState,
): void => {
...
const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, INPUT_LIVE_V54_PLAN_PATH);
  const capturePath = path.join(root, INPUT_LIVE_V54_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as InputLiveV54ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as InputLiveV54CaptureCell[];
  validateInputLiveV54CaptureManifest(captures);
  if (
    plan.captureManifest.sha256 !== inputLiveV54Sha256(captureBytes) ||
    captureArtifact.cellsSha256 !== inputLiveV54CaptureManifestSha256(captures)
  )
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (
      inputLiveV54Sha256(expectedBytes) !== source.expectedScenePlan.sha256 ||
      inputLiveV54Sha256(uncompressed) !==
  ...
  return {
    plan,
    planSha256: inputLiveV54Sha256(planBytes),
    captures,
    captureManifestSha256: inputLiveV54Sha256(captureBytes),
    sources,
...
const appendJournal = (
  ...
  const payloadSha256 = inputLiveV54Sha256(canonicalJson(payload));
  const body = {
  ...
  const entry = {
    ...body,
    entrySha256: inputLiveV54Sha256(canonicalJson(body)),
  };
  writeNew(
    path.join(
      directory,
...
const writerOwnership = (directory: string): InputLiveV54WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): InputLiveV54RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): InputLiveV54Request =>
  readJson<InputLiveV54Request>(file);

export class InputLiveV54Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueInputLiveV54Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: InputLiveV54OrchestratorState = {
      artifactVersion: "input-live-v54-orchestrator-state-v1",
      transactionDirectory: options.transactionDirectory,
      activeRequest: inputLiveV54RequestPath(
        options.transactionDirectory,
        "writer",
      ),
      cleanupRequest: null,
    ...
    return new InputLiveV54Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<InputLiveV54OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new InputLiveV54Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): InputLiveV54NextAction {
    const cleanup =
      this.#state.cleanupRequest &&
      !this.#state.cleanupAccepted &&
      !existsSync(inputLiveV54ReceiptPath(this.#directory, "cleanup"))
        ? this.#state.cleanupRequest
        : null;
  ...
  ingestAndAdvance(rawResponse: string): InputLiveV54NextAction {
  ...
  ingestCleanup(rawResponse: string): InputLiveV54NextAction {
    ...
    persistInputLiveV54RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptInputLiveV54Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV54CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      existsSync(inputLiveV54ReceiptPath(this.#directory, "writer")) &&
      !existsSync(path.join(this.#directory, OWNERSHIP_FILE))
    ) {
      const raw = readInputLiveV54AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateInputLiveV54WriterPayload(raw),
      );
    }
    if (existsSync(inputLiveV54RequestPath(this.#directory, "cleanup")))
      this.#state.cleanupRequest = inputLiveV54RequestPath(
        this.#directory,
        "cleanup",
      );
    if (
      existsSync(path.join(this.#directory, OWNERSHIP_FILE)) &&
      !existsSync(inputLiveV54RequestPath(this.#directory, "cleanup"))
    )
      this.#issueCleanupAndRestore(false);
    else if (
      existsSync(inputLiveV54RequestPath(this.#directory, "cleanup")) &&
      !existsSync(inputLiveV54RequestPath(this.#directory, "restore"))
    )
      this.#issueRestore(false);
    if (
      existsSync(inputLiveV54RequestPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV54ReceiptPath(this.#directory, "restore"))
    )
      this.#state.activeRequest = inputLiveV54RequestPath(
        this.#directory,
        "restore",
      );
    if (
      existsSync(inputLiveV54ReceiptPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV54RequestPath(this.#directory, "extract"))
    ) {
      const ownership = writerOwnership(this.#directory);
      const extract = issueInputLiveV54Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildInputLiveV54ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV54RequestPath(
        this.#directory,
        "extract",
    ...
    if (existsSync(inputLiveV54ReceiptPath(this.#directory, "cleanup")))
      this.#state.cleanupAccepted = true;
    this.#persist();
  }

  #acceptActive(request: InputLiveV54Request): void {
    if (request.phase === "writer") {
      const accepted = acceptInputLiveV54Response(this.#directory, "writer", {
    ...
    if (request.phase === "restore") {
      const accepted = acceptInputLiveV54Response(this.#directory, "restore", {
        validate: (payload) =>
          validateInputLiveV54RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptInputLiveV54Response(this.#directory, "extract", {
        validate: (payload) =>
          validateInputLiveV54ExtractPayload(payload, ownership),
      });
      const proofs = proveInputLiveV54Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseInputFieldRecipe,
        compileInputFieldRecipe,
      ...
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "input-live-v54-host-root-proofs-v1",
      ...
      appendJournal(
        ...
        request.requestId,
        { proofsSha256: inputLiveV54Sha256(canonicalJson(proofs)) },
      );
      const next = issueInputLiveV54Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildInputLiveV54ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV54RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptInputLiveV54Response(this.#directory, "probe", {
        validate: (payload) =>
          validateInputLiveV54ProbePayload(payload, ownership),
      });
      persistInputLiveV54TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      const accepted = acceptInputLiveV54Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateInputLiveV54CapturePayload(payload, cell, rawBytes),
      });
      const png = Buffer.from(accepted.payload.pngBase64, "base64");
      const capturePath = path.join(
        this.#directory,
        "captures",
        `${String(cell.index).padStart(3, "0")}-${inputLiveV54Sha256(cell.cellKey).slice(0, 20)}.png`,
      );
      ...
      if (next < INPUT_LIVE_V54_CAPTURE_COUNT) this.#issueCapture(next);
      else {
          ...
          pngBase64: Buffer.from(
            readFileSync(
              path.join(
                this.#directory,
                "captures",
                `${String(response.index).padStart(3, "0")}-${inputLiveV54Sha256(response.cellKey).slice(0, 20)}.png`,
              ),
            ),
          ).toString("base64"),
        }));
        assertInputLiveV54CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateInputLiveV54Objective(
        ...
        appendJournal(
          ...
          request.requestId,
          {
            reportSha256: inputLiveV54Sha256(canonicalJson(objective)),
  ...
  #issueCleanupAndRestore(journal: boolean): void {
    ...
    const cleanup = issueInputLiveV54Request(
      ...
      this.#privateKey,
      "cleanup",
      buildInputLiveV54CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = inputLiveV54RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueInputLiveV54Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildInputLiveV54RestoreProgram(ownership),
    );
    this.#state.activeRequest = inputLiveV54RequestPath(
      this.#directory,
      "restore",
  ...
  #readCaptureResponseIndex(): InputLiveV54CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: InputLiveV54CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedInputLiveV54Authorization = (
  privateKey: KeyObject,
...
export const generateInputLiveV54SimulationKey = (): KeyObject =>
