...
export interface InputLiveV36LifecycleSimulationReport {
  artifactVersion: "input-live-v36-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV36AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV36Lifecycle(): Promise<InputLiveV36LifecycleSimulationReport> {
  await buildInputLiveV36Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V36_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV36AntecedentIndex;
  const indexSha256 = inputLiveV36Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v36-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V36_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V36_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV36History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV36History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV36AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V36_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V36_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV36History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV36History(authorized, "pending");
    ...
    await buildInputLiveV36Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V36_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV36AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV36Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV36Lifecycle(), null, 2)}\n`,
  );
