...
import {
  ...
  verifyComboboxLiveV26Authorization,
  type ComboboxLiveV26AntecedentIndex,
  type ComboboxLiveV26AuthorizationProof,
} from "./combobox-live-v26-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V26_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v26/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V26_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v26-security-attestation.json";

export interface ComboboxLiveV26SecurityAttestation {
...
export interface ComboboxLiveV26PreflightReport {
  artifactVersion: "combobox-live-v26-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV26SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV26SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV26AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v26-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V26_TARGET) ||
...
export function validateComboboxLiveV26ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV26Authorization");
  const preflight = source.indexOf("runComboboxLiveV26Preflight");
  const initialize = source.indexOf("ComboboxLiveV26Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV26Preflight(
  root: string,
  proof: ComboboxLiveV26AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V26_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV26PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V26_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V26_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V26_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV26AntecedentIndex;
  failures.push(...validateComboboxLiveV26AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V26_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V26_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v26-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV26ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v26 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV26SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v26-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V26_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV26Preflight(
    process.cwd(),
    verifyComboboxLiveV26Authorization(),
    attestationPath,
