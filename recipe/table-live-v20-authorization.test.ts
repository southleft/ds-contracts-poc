...
import { TABLE_LIVE_V20_INDEX_PATH } from "./build-table-live-proof-v20.js";
import {
...
import {
  buildTableLiveV20AuthorizationArtifact,
  validateTableLiveV20AntecedentIndex,
  validateTableLiveV20AuthorizationArtifact,
  validateTableLiveV20History,
  validateTableLiveV20UnitTestSource,
  type TableLiveV20AntecedentIndex,
  type TableLiveV20AuthorizationArtifact,
  type TableLiveV20AuthorizationProof,
  type TableLiveV20HistoryState,
} from "./table-live-v20-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV20ControlFlowSource,
  validateTableLiveV20SecurityAttestation,
  type TableLiveV20SecurityAttestation,
} from "./table-live-v20-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V20_INDEX_PATH);
const index = (): TableLiveV20AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV20AntecedentIndex;

const artifact = (): TableLiveV20AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV20AuthorizationArtifact({
...
const pendingState = (): TableLiveV20HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV20HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV20AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v20/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV20AuthorizationArtifact,
): TableLiveV20SecurityAttestation => {
  const body: Omit<TableLiveV20SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V20_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV20History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV20History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV20History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV20History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV20History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV20UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v20-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV20UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV20AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV20AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV20AuthorizationArtifact(
        planted as TableLiveV20AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV20SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV20SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV20SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v20-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV20ControlFlowSource(source), []);
});
