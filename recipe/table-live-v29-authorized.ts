...
import { TABLE_LIVE_V29_AUTHORIZATION_PATH } from "./build-table-live-proof-v29.js";
import { verifyTableLiveV29Authorization } from "./table-live-v29-authorization.js";
import { runTableLiveV29Preflight } from "./table-live-v29-preflight.js";
import { TableLiveV29Orchestrator } from "./run-table-live-v29.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v29 requires exact authorization artifact ${TABLE_LIVE_V29_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v29-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV29Authorization(root);
    runTableLiveV29Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV29Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV29Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
