...
import { CALENDAR_LIVE_V42_INDEX_PATH } from "./build-calendar-live-proof-v42.js";
import {
...
import {
  buildCalendarLiveV42AuthorizationArtifact,
  validateCalendarLiveV42AntecedentIndex,
  validateCalendarLiveV42AuthorizationArtifact,
  validateCalendarLiveV42History,
  validateCalendarLiveV42UnitTestSource,
  type CalendarLiveV42AntecedentIndex,
  type CalendarLiveV42AuthorizationArtifact,
  type CalendarLiveV42AuthorizationProof,
  type CalendarLiveV42HistoryState,
} from "./calendar-live-v42-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV42ControlFlowSource,
  validateCalendarLiveV42SecurityAttestation,
  type CalendarLiveV42SecurityAttestation,
} from "./calendar-live-v42-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V42_INDEX_PATH);
const index = (): CalendarLiveV42AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV42AntecedentIndex;

const artifact = (): CalendarLiveV42AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV42AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV42HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV42HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV42AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v42/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV42AuthorizationArtifact,
): CalendarLiveV42SecurityAttestation => {
  const body: Omit<CalendarLiveV42SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "calendar-live-v42-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V42_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV42History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV42History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV42History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV42History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV42History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV42UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v42-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV42UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV42AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV42AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV42AuthorizationArtifact(
        planted as CalendarLiveV42AuthorizationArtifact,
        index(),
...
test("one-root denominator, capture gate, cleanup, and prior-lineage reuse are rejected", () => {
  ...
  for (const mutate of [
    ...
    (value: Record<string, any>) => {
      value.execution.comboboxLiveV42AuthorizationReusable = true;
    },
    (value: Record<string, any>) => {
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV42SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV42SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV42SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v42-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV42ControlFlowSource(source), []);
});
