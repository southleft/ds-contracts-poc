...
import { buildCalendarLiveV23CleanupRuntime } from "./calendar-live-v23-cleanup.js";
import { type CalendarLiveV23FixedFixedPoint } from "./calendar-live-v23-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV23RestoreProgram,
  validateCalendarLiveV23RestorePayload,
  type CalendarLiveV23RestorePayload,
} from "./calendar-live-v23-restore.js";
import {
  buildCalendarLiveV23RawPropertyRuntime,
  normalizeCalendarLiveV23Scene,
  type CalendarLiveV23RawNode,
} from "./calendar-live-v23-verifier.js";
...
export const CALENDAR_LIVE_V23_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V23_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V23_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V23_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V23_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V23_SET_COUNT = 3;
export const CALENDAR_LIVE_V23_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V23_HOST_PHASES = 3;
export const CALENDAR_LIVE_V23_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V23_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V23_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V23_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV23SourceId = (typeof CALENDAR_LIVE_V23_SOURCE_IDS)[number];
export type CalendarLiveV23Kind = "calendar" | "week" | "day";

export interface CalendarLiveV23SourceIdentity {
  source: CalendarLiveV23SourceId;
...
export interface CalendarLiveV23WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV23ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV23SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV23RawNode;
    weekScene: CalendarLiveV23RawNode;
    dayScene: CalendarLiveV23RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV23RootProof {
  source: CalendarLiveV23SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV23FixedFixedPoint;
}

export interface CalendarLiveV23ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV23SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV23CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV23SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV23CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV23SourceId;
  frameWidth: number;
...
export interface CalendarLiveV23CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v23 ${label} must be an object`);
...
export function validateCalendarLiveV23WriterPayload(
  value: unknown,
): CalendarLiveV23WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v23") ||
    ...
    result.namespace !== CALENDAR_LIVE_V23_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v23 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v23 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v23 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV23SourceIdentity,
  kind: CalendarLiveV23Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV23ExtractProgram(
  writer: CalendarLiveV23WriterOwnership,
  sources: readonly CalendarLiveV23SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v23 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V23_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V23_NAMESPACE)}
${buildCalendarLiveV23RawPropertyRuntime()}
...
export function validateCalendarLiveV23ExtractPayload(
  value: unknown,
  writer: CalendarLiveV23WriterOwnership,
): CalendarLiveV23ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v23 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V23_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v23 extract schema/one-root mismatch");
  return payload as CalendarLiveV23ExtractPayload;
}

export function proveCalendarLiveV23Roots<Instance>(
  extract: CalendarLiveV23ExtractPayload,
  sources: readonly (CalendarLiveV23SourceIdentity & {
...
): CalendarLiveV23RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v23 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v23 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV23Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV23Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV23Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV23FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v23 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v23 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v23 compile lost owned sets");
...
export function assertCalendarLiveV23RootProofs(
  proofs: readonly CalendarLiveV23RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v23 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v23 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV23ProbeProgram(
  writer: CalendarLiveV23WriterOwnership,
  sources: readonly CalendarLiveV23SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V23_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV23ProbePayload(
  value: unknown,
  writer: CalendarLiveV23WriterOwnership,
): CalendarLiveV23ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV23ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V23_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V23_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v23 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV23CaptureManifest(
  cells: readonly CalendarLiveV23CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V23_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V23_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V23_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V23_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v23 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV23CaptureCell[],
): string => {
  validateCalendarLiveV23CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV23CaptureProgram(
  writer: CalendarLiveV23WriterOwnership,
  cell: CalendarLiveV23CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V23_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v23 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v23 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V23_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV23CapturePayload(
  value: unknown,
  cell: CalendarLiveV23CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV23CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV23CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V23_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V23_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v23 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV23CaptureResponses(
  manifest: readonly CalendarLiveV23CaptureCell[],
  responses: readonly CalendarLiveV23CapturePayload[],
): void {
  validateCalendarLiveV23CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v23 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV23ObjectiveReport {
  artifactVersion: "calendar-live-v23-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV23Objective(
  manifest: readonly CalendarLiveV23CaptureCell[],
  responses: readonly CalendarLiveV23CapturePayload[],
): CalendarLiveV23ObjectiveReport {
  assertCalendarLiveV23CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v23-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV23CleanupProgram(
  writer: CalendarLiveV23WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV23CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V23_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v23 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV23CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV23WriterOwnership,
): CalendarLiveV23CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV23CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v23 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V23_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV23WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
