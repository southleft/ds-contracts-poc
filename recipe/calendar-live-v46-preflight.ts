...
import {
  validateCalendarLiveV46AntecedentIndex,
  verifyCalendarLiveV46Authorization,
  type CalendarLiveV46AntecedentIndex,
  type CalendarLiveV46AuthorizationProof,
} from "./calendar-live-v46-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V46_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v46/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V46_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v46-security-attestation.json";

export interface CalendarLiveV46SecurityAttestation {
  artifactVersion: "calendar-live-v46-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV46PreflightReport {
  artifactVersion: "calendar-live-v46-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV46SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV46SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV46AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v46-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V46_TARGET) ||
...
export function validateCalendarLiveV46ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV46Authorization");
  const preflight = source.indexOf("runCalendarLiveV46Preflight");
  const initialize = source.indexOf("CalendarLiveV46Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV46Preflight(
  root: string,
  proof: CalendarLiveV46AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V46_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV46PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V46_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V46_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV46AntecedentIndex;
  failures.push(...validateCalendarLiveV46AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V46_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V46_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV46SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v46-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV46ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v46 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV46SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v46-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V46_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV46Preflight(
    process.cwd(),
    verifyCalendarLiveV46Authorization(),
    attestationPath,
