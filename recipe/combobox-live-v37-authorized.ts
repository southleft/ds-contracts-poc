...
import { COMBOBOX_LIVE_V37_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v37.js";
import { verifyComboboxLiveV37Authorization } from "./combobox-live-v37-authorization.js";
import { runComboboxLiveV37Preflight } from "./combobox-live-v37-preflight.js";
import { ComboboxLiveV37Orchestrator } from "./run-combobox-live-v37.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v37 requires exact authorization artifact ${COMBOBOX_LIVE_V37_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v37-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV37Authorization(root);
    runComboboxLiveV37Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV37Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV37Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
