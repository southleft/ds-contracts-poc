...
import {
  ...
  verifyInputLiveV54Authorization,
  type InputLiveV54AntecedentIndex,
  type InputLiveV54AuthorizationProof,
} from "./input-field-live-v54-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V54_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v54/operator-security-attestation-template.json";
export const INPUT_LIVE_V54_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v54-security-attestation.json";

export interface InputLiveV54SecurityAttestation {
...
export interface InputLiveV54PreflightReport {
  artifactVersion: "input-live-v54-preflight-v1";
...
export const inputLiveV54AttestationSha256 = (
  value: Omit<InputLiveV54SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV54SecurityAttestation(
  value: unknown,
  proof: InputLiveV54AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v54-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V54_TARGET) ||
...
export function validateInputLiveV54ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV54Authorization");
  const preflight = source.indexOf("runInputLiveV54Preflight");
  const initialize = source.indexOf("InputLiveV54Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV54Preflight(
  root: string,
  proof: InputLiveV54AuthorizationProof,
  attestationPath = INPUT_LIVE_V54_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV54PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V54_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V54_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V54_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV54AntecedentIndex;
  failures.push(...validateInputLiveV54AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V54_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V54_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v54-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV54ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV54SecurityAttestation;
  return {
    artifactVersion: "input-live-v54-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V54_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV54Preflight(
    process.cwd(),
    verifyInputLiveV54Authorization(),
    attestationPath,
