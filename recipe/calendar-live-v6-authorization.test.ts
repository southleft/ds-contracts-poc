...
import { CALENDAR_LIVE_V6_INDEX_PATH } from "./build-calendar-live-proof-v6.js";
import {
...
import {
  buildCalendarLiveV6AuthorizationArtifact,
  validateCalendarLiveV6AntecedentIndex,
  validateCalendarLiveV6AuthorizationArtifact,
  validateCalendarLiveV6History,
  validateCalendarLiveV6UnitTestSource,
  type CalendarLiveV6AntecedentIndex,
  type CalendarLiveV6AuthorizationArtifact,
  type CalendarLiveV6AuthorizationProof,
  type CalendarLiveV6HistoryState,
} from "./calendar-live-v6-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV6ControlFlowSource,
  validateCalendarLiveV6SecurityAttestation,
  type CalendarLiveV6SecurityAttestation,
} from "./calendar-live-v6-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V6_INDEX_PATH);
const index = (): CalendarLiveV6AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV6AntecedentIndex;

const artifact = (): CalendarLiveV6AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV6AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV6HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV6HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV6AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v6/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV6AuthorizationArtifact,
): CalendarLiveV6SecurityAttestation => {
  const body: Omit<CalendarLiveV6SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "calendar-live-v6-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V6_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV6History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV6History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV6History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV6History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV6History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV6UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v6-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV6UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV6AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV6AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV6AuthorizationArtifact(
        planted as CalendarLiveV6AuthorizationArtifact,
        index(),
...
test("one-root denominator, capture gate, cleanup, and prior-lineage reuse are rejected", () => {
  ...
  for (const mutate of [
    ...
    (value: Record<string, any>) => {
      value.execution.calendarLiveV5AuthorizationReusable = true;
    },
    (value: Record<string, any>) => {
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV6SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV6SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV6SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v6-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV6ControlFlowSource(source), []);
});
