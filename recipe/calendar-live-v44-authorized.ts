...
import { CALENDAR_LIVE_V44_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v44.js";
import { verifyCalendarLiveV44Authorization } from "./calendar-live-v44-authorization.js";
import { runCalendarLiveV44Preflight } from "./calendar-live-v44-preflight.js";
import { CalendarLiveV44Orchestrator } from "./run-calendar-live-v44.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v44 requires exact authorization artifact ${CALENDAR_LIVE_V44_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v44-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV44Authorization(root);
    runCalendarLiveV44Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV44Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV44Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
