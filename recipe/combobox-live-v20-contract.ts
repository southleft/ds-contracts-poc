...
import { buildComboboxLiveV20CleanupRuntime } from "./combobox-live-v20-cleanup.js";
import {
  type ComboboxLiveV20FixedFixedPoint,
} from "./combobox-live-v20-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV20RestoreProgram,
  validateComboboxLiveV20RestorePayload,
  type ComboboxLiveV20RestorePayload,
} from "./combobox-live-v20-restore.js";
import {
  buildComboboxLiveV20RawPropertyRuntime,
  normalizeComboboxLiveV20Scene,
  type ComboboxLiveV20RawNode,
} from "./combobox-live-v20-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v20.js";
import {
...
} from "./scene-readback-combobox-v20.js";
...
export const COMBOBOX_LIVE_V20_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V20_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V20_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V20_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V20_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V20_SET_COUNT = 4;
export const COMBOBOX_LIVE_V20_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V20_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V20_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V20_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V20_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV20SourceId = (typeof COMBOBOX_LIVE_V20_SOURCE_IDS)[number];

export interface ComboboxLiveV20SourceIdentity {
  source: ComboboxLiveV20SourceId;
...
export interface ComboboxLiveV20WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV20ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV20SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV20RawNode;
    optionScene: ComboboxLiveV20RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV20RootProof {
  source: ComboboxLiveV20SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV20FixedFixedPoint;
}

export interface ComboboxLiveV20ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV20SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV20CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV20SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV20CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV20SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV20CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v20 ${label} must be an object`);
...
export function validateComboboxLiveV20WriterPayload(
  value: unknown,
): ComboboxLiveV20WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V20_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v20 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v20 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v20 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV20SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV20ExtractProgram(
  writer: ComboboxLiveV20WriterOwnership,
  sources: readonly ComboboxLiveV20SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v20 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V20_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V20_NAMESPACE)}
${buildComboboxLiveV20RawPropertyRuntime()}
...
export function validateComboboxLiveV20ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV20WriterOwnership,
): ComboboxLiveV20ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v20 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V20_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v20 extract schema/two-root mismatch");
  return payload as ComboboxLiveV20ExtractPayload;
}

export function proveComboboxLiveV20Roots<Instance>(
  extract: ComboboxLiveV20ExtractPayload,
  sources: readonly (ComboboxLiveV20SourceIdentity & {
...
): ComboboxLiveV20RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v20 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v20 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV20Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV20Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV20FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v20 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v20 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v20 compile lost owned sets");
...
export function assertComboboxLiveV20RootProofs(
  proofs: readonly ComboboxLiveV20RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v20 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v20 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV20ProbeProgram(
  writer: ComboboxLiveV20WriterOwnership,
  sources: readonly ComboboxLiveV20SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V20_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV20ProbePayload(
  value: unknown,
  writer: ComboboxLiveV20WriterOwnership,
): ComboboxLiveV20ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV20ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V20_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V20_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v20 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV20CaptureManifest(
  cells: readonly ComboboxLiveV20CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V20_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V20_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V20_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V20_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v20 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV20CaptureCell[],
): string => {
  validateComboboxLiveV20CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV20CaptureProgram(
  writer: ComboboxLiveV20WriterOwnership,
  cell: ComboboxLiveV20CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V20_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v20 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v20 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V20_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV20CapturePayload(
  value: unknown,
  cell: ComboboxLiveV20CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV20CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV20CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V20_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V20_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v20 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV20CaptureResponses(
  manifest: readonly ComboboxLiveV20CaptureCell[],
  responses: readonly ComboboxLiveV20CapturePayload[],
): void {
  validateComboboxLiveV20CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v20 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV20ObjectiveReport {
  artifactVersion: "combobox-live-v20-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV20Objective(
  manifest: readonly ComboboxLiveV20CaptureCell[],
  responses: readonly ComboboxLiveV20CapturePayload[],
): ComboboxLiveV20ObjectiveReport {
  assertComboboxLiveV20CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v20-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV20CleanupProgram(
  writer: ComboboxLiveV20WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV20CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V20_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v20 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV20CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV20WriterOwnership,
): ComboboxLiveV20CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV20CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v20 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V20_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV20WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
