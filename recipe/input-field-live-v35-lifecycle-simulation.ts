...
export interface InputLiveV35LifecycleSimulationReport {
  artifactVersion: "input-live-v35-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV35AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV35Lifecycle(): Promise<InputLiveV35LifecycleSimulationReport> {
  await buildInputLiveV35Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V35_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV35AntecedentIndex;
  const indexSha256 = inputLiveV35Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v35-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V35_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V35_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV35History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV35History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV35AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V35_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V35_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV35History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV35History(authorized, "pending");
    ...
    await buildInputLiveV35Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V35_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV35AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV35Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV35Lifecycle(), null, 2)}\n`,
  );
