...
import {
  acceptInputLiveV55Response,
  createInputLiveV55Transaction,
  inputLiveV55ReceiptPath,
  inputLiveV55RequestPath,
  inputLiveV55ResponsePath,
  inputLiveV55Sha256,
  issueInputLiveV55Request,
  persistInputLiveV55RawResponse,
  persistInputLiveV55TechnicalGates,
  readInputLiveV55AcceptedRawPayload,
  type InputLiveV55Request,
  type InputLiveV55TransactionAuthorization,
} from "./input-field-live-v55-broker.js";
import {
  INPUT_LIVE_V55_CAPTURE_COUNT,
  assertInputLiveV55CaptureResponses,
  assertInputLiveV55RootProofs,
  buildInputLiveV55CaptureProgram,
  buildInputLiveV55CleanupProgram,
  buildInputLiveV55ExtractProgram,
  buildInputLiveV55ProbeProgram,
  buildInputLiveV55RestoreProgram,
  validateInputLiveV55RestorePayload,
  evaluateInputLiveV55Objective,
  inputLiveV55CaptureManifestSha256,
  proveInputLiveV55Roots,
  validateInputLiveV55CaptureManifest,
  validateInputLiveV55CapturePayload,
  validateInputLiveV55CleanupPayload,
  validateInputLiveV55ExtractPayload,
  validateInputLiveV55ProbePayload,
  validateInputLiveV55WriterPayload,
  type InputLiveV55CaptureCell,
  type InputLiveV55CapturePayload,
  type InputLiveV55RootProof,
  type InputLiveV55SourceIdentity,
  type InputLiveV55WriterOwnership,
} from "./input-field-live-v55-contract.js";
import {
  INPUT_LIVE_V55_CAPTURE_MANIFEST_PATH,
  INPUT_LIVE_V55_PLAN_PATH,
  type InputLiveV55ProofPlan,
} from "./build-input-field-live-proof-v55.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-v55.js";

export const INPUT_LIVE_V55_RUNNER_VERSION = "input-live-v55-runner-v1";

interface InputLiveV55JournalEntry {
  index: number;
  event: string;
...
interface InputLiveV55OrchestratorState {
  artifactVersion: "input-live-v55-orchestrator-state-v1";
  ...
  journal: InputLiveV55JournalEntry[];
}

interface LoadedProofInputs {
  plan: InputLiveV55ProofPlan;
  planSha256: string;
  captures: InputLiveV55CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    InputLiveV55SourceIdentity & {
...
export interface InputLiveV55NextAction {
...
const writeState = (
  directory: string,
  state: InputLiveV55OrchestratorState,
): void => {
...
const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, INPUT_LIVE_V55_PLAN_PATH);
  const capturePath = path.join(root, INPUT_LIVE_V55_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as InputLiveV55ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as InputLiveV55CaptureCell[];
  validateInputLiveV55CaptureManifest(captures);
  if (
    plan.captureManifest.sha256 !== inputLiveV55Sha256(captureBytes) ||
    captureArtifact.cellsSha256 !== inputLiveV55CaptureManifestSha256(captures)
  )
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (
      inputLiveV55Sha256(expectedBytes) !== source.expectedScenePlan.sha256 ||
      inputLiveV55Sha256(uncompressed) !==
  ...
  return {
    plan,
    planSha256: inputLiveV55Sha256(planBytes),
    captures,
    captureManifestSha256: inputLiveV55Sha256(captureBytes),
    sources,
...
const appendJournal = (
  ...
  const payloadSha256 = inputLiveV55Sha256(canonicalJson(payload));
  const body = {
  ...
  const entry = {
    ...body,
    entrySha256: inputLiveV55Sha256(canonicalJson(body)),
  };
  writeNew(
    path.join(
      directory,
...
const writerOwnership = (directory: string): InputLiveV55WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): InputLiveV55RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): InputLiveV55Request =>
  readJson<InputLiveV55Request>(file);

export class InputLiveV55Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueInputLiveV55Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: InputLiveV55OrchestratorState = {
      artifactVersion: "input-live-v55-orchestrator-state-v1",
      transactionDirectory: options.transactionDirectory,
      activeRequest: inputLiveV55RequestPath(
        options.transactionDirectory,
        "writer",
      ),
      cleanupRequest: null,
    ...
    return new InputLiveV55Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<InputLiveV55OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new InputLiveV55Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): InputLiveV55NextAction {
    const cleanup =
      this.#state.cleanupRequest &&
      !this.#state.cleanupAccepted &&
      !existsSync(inputLiveV55ReceiptPath(this.#directory, "cleanup"))
        ? this.#state.cleanupRequest
        : null;
  ...
  ingestAndAdvance(rawResponse: string): InputLiveV55NextAction {
  ...
  ingestCleanup(rawResponse: string): InputLiveV55NextAction {
    ...
    persistInputLiveV55RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptInputLiveV55Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV55CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      existsSync(inputLiveV55ReceiptPath(this.#directory, "writer")) &&
      !existsSync(path.join(this.#directory, OWNERSHIP_FILE))
    ) {
      const raw = readInputLiveV55AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateInputLiveV55WriterPayload(raw),
      );
    }
    if (existsSync(inputLiveV55RequestPath(this.#directory, "cleanup")))
      this.#state.cleanupRequest = inputLiveV55RequestPath(
        this.#directory,
        "cleanup",
      );
    if (
      existsSync(path.join(this.#directory, OWNERSHIP_FILE)) &&
      !existsSync(inputLiveV55RequestPath(this.#directory, "cleanup"))
    )
      this.#issueCleanupAndRestore(false);
    else if (
      existsSync(inputLiveV55RequestPath(this.#directory, "cleanup")) &&
      !existsSync(inputLiveV55RequestPath(this.#directory, "restore"))
    )
      this.#issueRestore(false);
    if (
      existsSync(inputLiveV55RequestPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV55ReceiptPath(this.#directory, "restore"))
    )
      this.#state.activeRequest = inputLiveV55RequestPath(
        this.#directory,
        "restore",
      );
    if (
      existsSync(inputLiveV55ReceiptPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV55RequestPath(this.#directory, "extract"))
    ) {
      const ownership = writerOwnership(this.#directory);
      const extract = issueInputLiveV55Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildInputLiveV55ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV55RequestPath(
        this.#directory,
        "extract",
    ...
    if (existsSync(inputLiveV55ReceiptPath(this.#directory, "cleanup")))
      this.#state.cleanupAccepted = true;
    this.#persist();
  }

  #acceptActive(request: InputLiveV55Request): void {
    if (request.phase === "writer") {
      const accepted = acceptInputLiveV55Response(this.#directory, "writer", {
        validate: validateInputLiveV55WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptInputLiveV55Response(this.#directory, "restore", {
        validate: (payload) =>
          validateInputLiveV55RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptInputLiveV55Response(this.#directory, "extract", {
        validate: (payload) =>
          validateInputLiveV55ExtractPayload(payload, ownership),
      });
      const proofs = proveInputLiveV55Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseInputFieldRecipe,
        compileInputFieldRecipe,
      );
      assertInputLiveV55RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "input-live-v55-host-root-proofs-v1",
      ...
      appendJournal(
        ...
        request.requestId,
        { proofsSha256: inputLiveV55Sha256(canonicalJson(proofs)) },
      );
      const next = issueInputLiveV55Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildInputLiveV55ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV55RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptInputLiveV55Response(this.#directory, "probe", {
        validate: (payload) =>
          validateInputLiveV55ProbePayload(payload, ownership),
      });
      persistInputLiveV55TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      const accepted = acceptInputLiveV55Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateInputLiveV55CapturePayload(payload, cell, rawBytes),
      });
      const png = Buffer.from(accepted.payload.pngBase64, "base64");
      const capturePath = path.join(
        this.#directory,
        "captures",
        `${String(cell.index).padStart(3, "0")}-${inputLiveV55Sha256(cell.cellKey).slice(0, 20)}.png`,
      );
      ...
      if (next < INPUT_LIVE_V55_CAPTURE_COUNT) this.#issueCapture(next);
      else {
          ...
          pngBase64: Buffer.from(
            readFileSync(
              path.join(
                this.#directory,
                "captures",
                `${String(response.index).padStart(3, "0")}-${inputLiveV55Sha256(response.cellKey).slice(0, 20)}.png`,
              ),
            ),
          ).toString("base64"),
        }));
        assertInputLiveV55CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateInputLiveV55Objective(
        ...
        appendJournal(
          ...
          request.requestId,
          {
            reportSha256: inputLiveV55Sha256(canonicalJson(objective)),
  ...
  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueInputLiveV55Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildInputLiveV55CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = inputLiveV55RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueInputLiveV55Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildInputLiveV55RestoreProgram(ownership),
    );
    this.#state.activeRequest = inputLiveV55RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueInputLiveV55Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildInputLiveV55CaptureProgram(
  ...
  #readCaptureResponseIndex(): InputLiveV55CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: InputLiveV55CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedInputLiveV55Authorization = (
  privateKey: KeyObject,
): InputLiveV55TransactionAuthorization => ({
  mode: "simulated",
  ...
  signingPublicKeySha256: inputLiveV55Sha256(
...
export const generateInputLiveV55SimulationKey = (): KeyObject =>
