...
import {
  acceptCalendarLiveV44Response,
  createCalendarLiveV44Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV44Request,
  persistCalendarLiveV44RawResponse,
  persistCalendarLiveV44TechnicalGates,
  readCalendarLiveV44AcceptedRawPayload,
  type CalendarLiveV44Request,
  type CalendarLiveV44TransactionAuthorization,
} from "./calendar-live-v44-broker.js";
import {
  CALENDAR_LIVE_V44_CAPTURE_COUNT,
  assertCalendarLiveV44CaptureResponses,
  ...
  evaluateCalendarLiveV44Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV44Roots,
  validateCalendarLiveV44CaptureManifest,
  validateCalendarLiveV44CapturePayload,
  validateCalendarLiveV44CleanupPayload,
  validateCalendarLiveV44ExtractPayload,
  validateCalendarLiveV44ProbePayload,
  validateCalendarLiveV44WriterPayload,
  type CalendarLiveV44CaptureCell,
  type CalendarLiveV44CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V44_PLAN_PATH,
  type CalendarLiveV44ProofPlan,
...
export const CALENDAR_LIVE_V44_RUNNER_VERSION = "calendar-live-v44-runner-v1";

interface CalendarLiveV44JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV44OrchestratorState {
  artifactVersion: "calendar-live-v44-orchestrator-state-v1";
  ...
  journal: CalendarLiveV44JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV44ProofPlan;
  planSha256: string;
  captures: CalendarLiveV44CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV44SourceIdentity & {
...
export interface CalendarLiveV44NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV44OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v44 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V44_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V44_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV44ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV44CaptureCell[];
  validateCalendarLiveV44CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v44 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v44 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV44WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV44RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV44Request =>
  readJson<CalendarLiveV44Request>(file);

export class CalendarLiveV44Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV44Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV44OrchestratorState = {
      artifactVersion: "calendar-live-v44-orchestrator-state-v1",
    ...
    return new CalendarLiveV44Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV44OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV44Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV44NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV44NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v44 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV44NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v44 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV44RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV44Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV44CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV44AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV44WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV44Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV44ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV44Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV44Response(this.#directory, "writer", {
        validate: validateCalendarLiveV44WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV44Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV44RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV44Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV44ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV44Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV44RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v44-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV44Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV44ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV44Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV44ProbePayload(payload, ownership),
      });
      persistCalendarLiveV44TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v44 capture cell missing");
      const accepted = acceptCalendarLiveV44Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV44CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V44_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV44CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV44Objective(
    ...
    throw new Error(`Calendar live v44 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV44Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV44CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV44Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV44RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV44Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV44CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV44CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV44CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV44Authorization = (
  privateKey: KeyObject,
): CalendarLiveV44TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV44SimulationKey = (): KeyObject =>
