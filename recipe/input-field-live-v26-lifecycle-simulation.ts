...
export interface InputLiveV26LifecycleSimulationReport {
  artifactVersion: "input-live-v26-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV26AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV26Lifecycle(): Promise<InputLiveV26LifecycleSimulationReport> {
  await buildInputLiveV26Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V26_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV26AntecedentIndex;
  const indexSha256 = inputLiveV26Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v26-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V26_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V26_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV26History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV26History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV26AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V26_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V26_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV26History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV26History(authorized, "pending");
    ...
    await buildInputLiveV26Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V26_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV26AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV26Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV26Lifecycle(), null, 2)}\n`,
  );
