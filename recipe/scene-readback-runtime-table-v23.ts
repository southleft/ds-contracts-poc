...
/**
 * Table live v23 extract runtime. Keeps the v6 skip of untagged
 * `table/row/owned-cell-label` TEXT bind hosts. Extract bytes are
 * unchanged from v21. The v22 teaching lives in host-normalize
 * (`scene-readback-table-v1.ts`): omit `strokes` on `table/row`
 * variants because compile row variants omit `strokes`. Does not
 * omit cell-variant `clipsContent`, `cornerRadius`, or empty
 * `strokes`. Does not invent `strokes` onto compile row variants.
 * Does not omit `table/variant` `strokes` or `cornerRadius`.
 */
