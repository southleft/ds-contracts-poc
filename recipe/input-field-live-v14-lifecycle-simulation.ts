...
export interface InputLiveV14LifecycleSimulationReport {
  artifactVersion: "input-live-v14-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV14AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV14Lifecycle(): Promise<InputLiveV14LifecycleSimulationReport> {
  await buildInputLiveV14Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V14_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV14AntecedentIndex;
  const indexSha256 = inputLiveV14Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v14-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V14_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V14_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV14History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV14History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV14AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V14_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V14_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV14History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV14History(authorized, "pending");
    ...
    await buildInputLiveV14Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V14_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV14AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV14Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV14Lifecycle(), null, 2)}\n`,
  );
