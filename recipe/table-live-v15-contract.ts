...
import { buildTableLiveV15CleanupRuntime } from "./table-live-v15-cleanup.js";
import { type TableLiveV15FixedFixedPoint } from "./table-live-v15-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV15RestoreProgram,
  validateTableLiveV15RestorePayload,
  type TableLiveV15RestorePayload,
} from "./table-live-v15-restore.js";
import {
  buildTableLiveV15RawPropertyRuntime,
  normalizeTableLiveV15Scene,
  type TableLiveV15RawNode,
} from "./table-live-v15-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v15.js";
import {
...
export const TABLE_LIVE_V15_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V15_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V15_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V15_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V15_VARIANT_COUNT = 20;
export const TABLE_LIVE_V15_SET_COUNT = 6;
export const TABLE_LIVE_V15_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V15_HOST_PHASES = 3;
export const TABLE_LIVE_V15_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V15_RESTORE_COUNT = 8;
export const TABLE_LIVE_V15_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V15_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV15SourceId = (typeof TABLE_LIVE_V15_SOURCE_IDS)[number];
export type TableLiveV15Kind = "table" | "row" | "cell";

export interface TableLiveV15SourceIdentity {
  source: TableLiveV15SourceId;
...
export interface TableLiveV15WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV15ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV15SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV15RawNode;
    rowScene: TableLiveV15RawNode;
    cellScene: TableLiveV15RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV15RootProof {
  source: TableLiveV15SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV15FixedFixedPoint;
}

export interface TableLiveV15ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV15SourceId;
    adapterIdentity: string;
...
export interface TableLiveV15CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV15SourceId;
  adapterIdentity: string;
...
export interface TableLiveV15CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV15SourceId;
  frameWidth: number;
...
export interface TableLiveV15CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v15 ${label} must be an object`);
...
export function validateTableLiveV15WriterPayload(
  value: unknown,
): TableLiveV15WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V15_NAMESPACE ||
    ...
    throw new TypeError("Table live v15 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v15 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v15 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV15SourceIdentity,
  kind: TableLiveV15Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV15ExtractProgram(
  writer: TableLiveV15WriterOwnership,
  sources: readonly TableLiveV15SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v15 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V15_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V15_NAMESPACE)}
${buildTableLiveV15RawPropertyRuntime()}
...
export function validateTableLiveV15ExtractPayload(
  value: unknown,
  writer: TableLiveV15WriterOwnership,
): TableLiveV15ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v15 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V15_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v15 extract schema/two-root mismatch");
  return payload as TableLiveV15ExtractPayload;
}

export function proveTableLiveV15Roots<Instance>(
  extract: TableLiveV15ExtractPayload,
  sources: readonly (TableLiveV15SourceIdentity & {
...
): TableLiveV15RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v15 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v15 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV15Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV15Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV15Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV15FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v15 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v15 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v15 compile lost owned sets");
...
export function assertTableLiveV15RootProofs(
  proofs: readonly TableLiveV15RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v15 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v15 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV15ProbeProgram(
  writer: TableLiveV15WriterOwnership,
  sources: readonly TableLiveV15SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V15_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV15ProbePayload(
  value: unknown,
  writer: TableLiveV15WriterOwnership,
): TableLiveV15ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV15ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V15_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V15_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Table live v15 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV15CaptureManifest(
  cells: readonly TableLiveV15CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V15_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V15_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V15_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V15_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v15 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV15CaptureCell[],
): string => {
  validateTableLiveV15CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV15CaptureProgram(
  writer: TableLiveV15WriterOwnership,
  cell: TableLiveV15CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V15_CAPTURE_COUNT
  )
    throw new TypeError("Table live v15 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v15 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V15_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV15CapturePayload(
  value: unknown,
  cell: TableLiveV15CaptureCell,
  rawResponseBytes: number,
): TableLiveV15CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV15CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V15_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V15_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v15 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV15CaptureResponses(
  manifest: readonly TableLiveV15CaptureCell[],
  responses: readonly TableLiveV15CapturePayload[],
): void {
  validateTableLiveV15CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v15 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV15ObjectiveReport {
  artifactVersion: "table-live-v15-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV15Objective(
  manifest: readonly TableLiveV15CaptureCell[],
  responses: readonly TableLiveV15CapturePayload[],
): TableLiveV15ObjectiveReport {
  assertTableLiveV15CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v15-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV15CleanupProgram(
  writer: TableLiveV15WriterOwnership,
): string {
  const baseProgram = buildTableLiveV15CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V15_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v15 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV15CleanupPayload(
  payload: unknown,
  writer: TableLiveV15WriterOwnership,
): TableLiveV15CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV15CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v15 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V15_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV15WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
