...
export type ComboboxLiveV17HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV17AuthorizationArtifact {
  artifactVersion: "combobox-live-v17-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V17_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV17AntecedentIndex {
...
export interface ComboboxLiveV17HistoryState {
  index?: ComboboxLiveV17AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV17AuthorizationArtifact;
...
export interface ComboboxLiveV17AuthorizationProof extends ComboboxLiveV17TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V17_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V17_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V17_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V17_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v17-authorization") ||
  artifactPath.includes("combobox-live-v17-preflight") ||
  artifactPath.includes("combobox-live-v17-authorized") ||
  artifactPath.includes("create-combobox-live-v17-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v17-status") ||
...
export function validateComboboxLiveV17AntecedentIndex(
  index: ComboboxLiveV17AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v17-antecedent-index-v1" ||
...
export function validateComboboxLiveV17UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV17History\(\s*["']/.test(source) ||
    /readComboboxLiveV17HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV17AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV17AuthorizationArtifact {
  ...
  ) as ComboboxLiveV17AntecedentIndex;
  const indexFailures = validateComboboxLiveV17AntecedentIndex(index);
...
export function validateComboboxLiveV17AuthorizationArtifact(
  artifact: ComboboxLiveV17AuthorizationArtifact | undefined,
  index: ComboboxLiveV17AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v17-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v17" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V17_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V17_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V17_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V17_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V17_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V17_SOURCE_ROOTS ||
  ...
  if (
    ...
    failures.push(
      "v17 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
...
export function validateComboboxLiveV17History(
  state: ComboboxLiveV17HistoryState,
  expected: ComboboxLiveV17HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV17AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV17AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV17HistoryState(
  root = process.cwd(),
): ComboboxLiveV17HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V17_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV17AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V17_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V17_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V17_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV17History(
  expected: ComboboxLiveV17HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV17HistoryState {
  const state = readComboboxLiveV17HistoryState(root);
  const failures = validateComboboxLiveV17History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v17 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV17Authorization(
  root = process.cwd(),
): ComboboxLiveV17AuthorizationProof {
  const state = verifyComboboxLiveV17History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V17_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v17/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v17.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V17_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V17_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V17_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV17HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV17History(expected);
  process.stdout.write(
    `Combobox live v17 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
