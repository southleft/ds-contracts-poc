...
import {
  acceptTableLiveV20Response,
  createTableLiveV20Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV20Request,
  persistTableLiveV20RawResponse,
  persistTableLiveV20TechnicalGates,
  readTableLiveV20AcceptedRawPayload,
  type TableLiveV20Request,
  type TableLiveV20TransactionAuthorization,
} from "./table-live-v20-broker.js";
import {
  TABLE_LIVE_V20_CAPTURE_COUNT,
  assertTableLiveV20CaptureResponses,
  ...
  evaluateTableLiveV20Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV20Roots,
  validateTableLiveV20CaptureManifest,
  validateTableLiveV20CapturePayload,
  validateTableLiveV20CleanupPayload,
  validateTableLiveV20ExtractPayload,
  validateTableLiveV20ProbePayload,
  validateTableLiveV20WriterPayload,
  type TableLiveV20CaptureCell,
  type TableLiveV20CapturePayload,
...
import {
  ...
  TABLE_LIVE_V20_PLAN_PATH,
  type TableLiveV20ProofPlan,
} from "./build-table-live-proof-v20.js";
import {
...
export const TABLE_LIVE_V20_RUNNER_VERSION = "table-live-v20-runner-v1";

interface TableLiveV20JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV20OrchestratorState {
  artifactVersion: "table-live-v20-orchestrator-state-v1";
  ...
  journal: TableLiveV20JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV20ProofPlan;
  planSha256: string;
  captures: TableLiveV20CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV20SourceIdentity & {
...
export interface TableLiveV20NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV20OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v20 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V20_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V20_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV20ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV20CaptureCell[];
  validateTableLiveV20CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v20 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v20 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV20WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV20RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV20Request =>
  readJson<TableLiveV20Request>(file);

export class TableLiveV20Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV20Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV20OrchestratorState = {
      artifactVersion: "table-live-v20-orchestrator-state-v1",
    ...
    return new TableLiveV20Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV20OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV20Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV20NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV20NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v20 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV20NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v20 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV20RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV20Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV20CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV20AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV20WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV20Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV20ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV20Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV20Response(this.#directory, "writer", {
        validate: validateTableLiveV20WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV20Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV20RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV20Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV20ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV20Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV20RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v20-host-root-proofs-v1",
      ...
      const next = issueTableLiveV20Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV20ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV20Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV20ProbePayload(payload, ownership),
      });
      persistTableLiveV20TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v20 capture cell missing");
      const accepted = acceptTableLiveV20Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV20CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V20_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV20CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV20Objective(
    ...
    throw new Error(`Table live v20 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV20Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV20CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV20Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV20RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV20Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV20CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV20CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV20CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV20Authorization = (
  privateKey: KeyObject,
): TableLiveV20TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV20SimulationKey = (): KeyObject =>
