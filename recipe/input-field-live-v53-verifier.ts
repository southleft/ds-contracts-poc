...
export const INPUT_LIVE_V53_VERIFIER_VERSION = "input-live-v53-verifier-v1";

export function buildInputLiveV53RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV53RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV53RawNode[];
}

export interface InputLiveV53NormalizedScene {
...
export interface InputLiveV53SceneProof {
...
export function normalizeInputLiveV53Scene(
  root: InputLiveV53RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV53NormalizedScene {
  const canonicalBindings: InputLiveV53NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV53RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV53Scene<Instance>(
  normalized: InputLiveV53NormalizedScene,
...
export function compileInputLiveV53ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV53PreCaptureGates(payload: unknown): void {
