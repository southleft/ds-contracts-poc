...
import {
  ...
  INPUT_LIVE_V58_INDEX_PATH,
} from "./build-input-field-live-proof-v58.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV58HistoryExpectation = "pending" | "authorized";

export interface InputLiveV58AuthorizationArtifact {
  artifactVersion: "input-live-v58-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V58_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV58AntecedentIndex {
...
export interface InputLiveV58HistoryState {
  index?: InputLiveV58AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV58AuthorizationArtifact;
...
export interface InputLiveV58AuthorizationProof extends InputLiveV58TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V58_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V58_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V58_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V58_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v58-authorization") ||
  artifactPath.includes("input-field-live-v58-preflight") ||
  artifactPath.includes("input-field-live-v58-authorized") ||
  artifactPath.includes("input-field-live-pivot-v58-status") ||
...
export function validateInputLiveV58AntecedentIndex(
  index: InputLiveV58AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v58-antecedent-index-v1" ||
...
export function validateInputLiveV58UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV58History\(\s*["']/.test(source) ||
    /readInputLiveV58HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV58AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV58AuthorizationArtifact {
  ...
  ) as InputLiveV58AntecedentIndex;
  const indexFailures = validateInputLiveV58AntecedentIndex(index);
...
export function validateInputLiveV58AuthorizationArtifact(
  artifact: InputLiveV58AuthorizationArtifact | undefined,
  index: InputLiveV58AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v58-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v58" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V58_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V58_TARGET) ||
    !equalJson(
...
export function validateInputLiveV58History(
  state: InputLiveV58HistoryState,
  expected: InputLiveV58HistoryExpectation,
): string[] {
  const failures = validateInputLiveV58AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV58AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV58HistoryState(
  root = process.cwd(),
): InputLiveV58HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V58_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV58AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V58_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V58_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V58_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV58History(
  expected: InputLiveV58HistoryExpectation,
  root = process.cwd(),
): InputLiveV58HistoryState {
  const state = readInputLiveV58HistoryState(root);
  const failures = validateInputLiveV58History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV58Authorization(
  root = process.cwd(),
): InputLiveV58AuthorizationProof {
  const state = verifyInputLiveV58History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V58_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v58/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v58.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V58_TARGET,
    expectedDynamicTool: INPUT_LIVE_V58_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V58_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV58HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV58History(expected);
