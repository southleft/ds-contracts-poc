...
import {
  verifyCalendarLiveV6FixedSceneFixedPoint,
  type CalendarLiveV6FixedFixedPoint,
} from "./calendar-live-v6-fixed-point.js";

export const CALENDAR_LIVE_V6_VERIFIER_VERSION = "calendar-live-v6-verifier-v1";

export function buildCalendarLiveV6RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV6RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV6RawNode[];
}

export interface CalendarLiveV6NormalizedScene {
...
export interface CalendarLiveV6SceneProof {
...
export function normalizeCalendarLiveV6Scene(
  root: CalendarLiveV6RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV6NormalizedScene {
  const canonicalBindings: CalendarLiveV6NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV6RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV6Scene<Instance>(
  normalized: CalendarLiveV6NormalizedScene,
...
): CalendarLiveV6SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV6FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV6ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV6PreCaptureGates(payload: unknown): void {
