...
export const INPUT_LIVE_V77_MEASURE_VISIBLE_FILL_MARKER =
  "INPUT-TEXT-FILL-MEASURE-VISIBLE";
export const INPUT_LIVE_V77_RESTORE_CONTENT_ROLES = [
  "input-field/content/placeholder",
  "input-field/content/value",
] as const;
export const INPUT_LIVE_V77_RESTORE_PARENT_ROLES = [
  "input-field/surface",
  "input-field/content-row",
] as const;

export interface InputLiveV77RestoreWriter {
  pageId: string;
  runIdentity: string;
  setIds: readonly [string, string];
}

export interface InputLiveV77RestorePayload {
  pageId: string;
  setIds: string[];
  restoredCount: 256;
  fixedBefore: number;
  ...
  marker: typeof INPUT_LIVE_V77_MEASURE_VISIBLE_FILL_MARKER;
}

export function buildInputLiveV77RestoreProgram(
  writer: InputLiveV77RestoreWriter,
): string {
  if (writer.setIds.length !== 2)
    throw new TypeError("Input live v77 restore requires two owned sets");
  return String.raw`
await figma.loadAllPagesAsync();
const V77_NS=${JSON.stringify(INPUT_LIVE_V77_NAMESPACE)};
const V77_PAGE_ID=${JSON.stringify(writer.pageId)};
const V77_SET_IDS=${JSON.stringify([...writer.setIds])};
const V77_ROLES=new Set(${JSON.stringify([...INPUT_LIVE_V77_RESTORE_CONTENT_ROLES])});
const V77_PARENTS=new Set(${JSON.stringify([...INPUT_LIVE_V77_RESTORE_PARENT_ROLES])});
const V77_MARKER=${JSON.stringify(INPUT_LIVE_V77_MEASURE_VISIBLE_FILL_MARKER)};
const page=await figma.getNodeByIdAsync(V77_PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("INPUT-V77-RESTORE-PAGE");
if(page.getSharedPluginData(V77_NS,"pageOwner")!=="recipe/input-field/"+${JSON.stringify(writer.runIdentity)})throw new Error("INPUT-V77-RESTORE-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>V77_SET_IDS.includes(node.id));
if(sets.length!==2)throw new Error("INPUT-V77-RESTORE-ROOTS:"+sets.length);
const parents=[];
const texts=[];
for(const set of sets){
  for(const component of set.children){
    ...
    for(const descendant of component.findAllWithCriteria({types:["TEXT","FRAME"]})){
      ...
      if(descendant.type==="FRAME"&&V77_PARENTS.has(role))parents.push(descendant);
      if(descendant.type==="TEXT"&&V77_ROLES.has(role))texts.push({setId:set.id,componentName:component.name,role,node:descendant});
    }
  }
}
...
for(const entry of texts){
  ...
  if(node.width<=0||node.height<=0)throw new Error(V77_MARKER+":"+entry.role);
...
if(restored.length!==256)throw new Error("INPUT-V77-RESTORE-COUNT:"+restored.length);
if(restored.some(entry=>entry.after!=="FILL"))throw new Error("INPUT-V77-RESTORE-NOT-FILL");
...
export function validateInputLiveV77RestorePayload(
  payload: unknown,
  writer: InputLiveV77RestoreWriter,
): InputLiveV77RestorePayload {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("Input live v77 restore payload must be an object");
  ...
  if (
    ...
    value.marker !== INPUT_LIVE_V77_MEASURE_VISIBLE_FILL_MARKER
  )
    throw new TypeError(
      "Input live v77 restore did not re-assert content FILL",
    );
  return value as InputLiveV77RestorePayload;
}
