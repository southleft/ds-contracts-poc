...
export type ComboboxLiveV1HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV1AuthorizationArtifact {
  artifactVersion: "combobox-live-v1-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V1_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV1AntecedentIndex {
...
export interface ComboboxLiveV1HistoryState {
  index?: ComboboxLiveV1AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV1AuthorizationArtifact;
...
export interface ComboboxLiveV1AuthorizationProof extends ComboboxLiveV1TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V1_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V1_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V1_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V1_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v1-authorization") ||
  artifactPath.includes("combobox-live-v1-preflight") ||
  artifactPath.includes("combobox-live-v1-authorized") ||
  artifactPath.includes("create-combobox-live-v1-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v1-status") ||
...
export function validateComboboxLiveV1AntecedentIndex(
  index: ComboboxLiveV1AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v1-antecedent-index-v1" ||
...
export function validateComboboxLiveV1UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV1History\(\s*["']/.test(source) ||
    /readComboboxLiveV1HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV1AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV1AuthorizationArtifact {
  ...
  ) as ComboboxLiveV1AntecedentIndex;
  const indexFailures = validateComboboxLiveV1AntecedentIndex(index);
...
export function validateComboboxLiveV1AuthorizationArtifact(
  artifact: ComboboxLiveV1AuthorizationArtifact | undefined,
  index: ComboboxLiveV1AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v1-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v1" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V1_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V1_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V1_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V1_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V1_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V1_SOURCE_ROOTS ||
...
export function validateComboboxLiveV1History(
  state: ComboboxLiveV1HistoryState,
  expected: ComboboxLiveV1HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV1AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV1AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV1HistoryState(
  root = process.cwd(),
): ComboboxLiveV1HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V1_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV1AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V1_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V1_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V1_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV1History(
  expected: ComboboxLiveV1HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV1HistoryState {
  const state = readComboboxLiveV1HistoryState(root);
  const failures = validateComboboxLiveV1History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyComboboxLiveV1Authorization(
  root = process.cwd(),
): ComboboxLiveV1AuthorizationProof {
  const state = verifyComboboxLiveV1History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V1_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v1/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v1.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V1_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V1_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V1_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV1HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV1History(expected);
