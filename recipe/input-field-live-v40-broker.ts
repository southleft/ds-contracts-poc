...
import {
  assertInputLiveV40RootProofs,
  INPUT_LIVE_V40_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V40_RESPONSE_CONTRACTS,
  type InputLiveV40RootProof,
} from "./input-field-live-v40-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V40_BROKER_VERSION = "input-live-v40-broker-v2";
export const INPUT_LIVE_V40_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V40_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V40_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V40_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V40_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V40_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV40RemotePhase =
  (typeof INPUT_LIVE_V40_REMOTE_PHASES)[number];
export type InputLiveV40AuthorizationMode = "live" | "simulated";

export interface InputLiveV40CommitPins {
...
export interface InputLiveV40TransactionAuthorization extends InputLiveV40CommitPins {
  mode: InputLiveV40AuthorizationMode;
...
export interface InputLiveV40Manifest {
  artifactVersion: "input-live-v40-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V40_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V40_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V40_TARGET;
  authorization: InputLiveV40TransactionAuthorization;
...
export interface InputLiveV40ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV40RequestBody {
  artifactVersion: "input-live-v40-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V40_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV40RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV40CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V40_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV40ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV40Request extends InputLiveV40RequestBody {
...
export interface InputLiveV40Receipt {
  artifactVersion: "input-live-v40-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V40_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV40RemotePhase;
...
export interface InputLiveV40AcceptedResponse<T = unknown> {
  request: InputLiveV40Request;
  receipt: InputLiveV40Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV40IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV40ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V40_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V40_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V40_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V40_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V40_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V40_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V40_TARGET.editorType;
};
...
export const inputLiveV40RequestSequence = (
  phase: InputLiveV40RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV40RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV40RequestPath = (
  directory: string,
  phase: InputLiveV40RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV40ResponsePath = (
  directory: string,
  phase: InputLiveV40RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV40ReceiptPath = (
  directory: string,
  phase: InputLiveV40RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV40Manifest => {
  const manifest = json<InputLiveV40Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v40-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V40_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V40_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV40RemotePhase,
  captureIndex?: number,
): InputLiveV40Request | undefined => {
  const file = inputLiveV40RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV40Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV40RemotePhase,
  captureIndex?: number,
): InputLiveV40Receipt | undefined => {
  const file = inputLiveV40ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV40Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV40Request | undefined;
  receipt: InputLiveV40Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV40RemotePhase,
): InputLiveV40ResponseContract => ({
  ...INPUT_LIVE_V40_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V40_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V40_DYNAMIC_TOOL,
      target: INPUT_LIVE_V40_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v40-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V40_BROKER_VERSION,
  target: INPUT_LIVE_V40_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V40_PROGRAM_START*/${program}/*INPUT_LIVE_V40_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V40_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V40_PROGRAM_END*/";
...
export function createInputLiveV40Transaction(
  directory: string,
  ...
  const manifest: InputLiveV40Manifest = {
    artifactVersion: "input-live-v40-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V40_BROKER_VERSION,
...
export function validateInputLiveV40Request(
  directory: string,
  request: InputLiveV40Request,
): void {
  ...
  const expectedSequence = inputLiveV40RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v40-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V40_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V40_TARGET.fileKey ||
...
export function issueInputLiveV40Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV40RemotePhase,
  program: string,
  options: InputLiveV40IssueOptions = {},
): InputLiveV40Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV40RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV40Request>(targetPath);
    validateInputLiveV40Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV40ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV40TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV40RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V40_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V40_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V40_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV40RequestBody = {
    artifactVersion: "input-live-v40-broker-request-v2",
    brokerVersion: INPUT_LIVE_V40_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV40Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV40Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV40RawResponse(
  directory: string,
  phase: InputLiveV40RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV40RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV40Request>(requestFile);
  if (
  ...
  const target = inputLiveV40ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV40Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v40-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V40_BROKER_VERSION,
...
export function acceptInputLiveV40Response<T = unknown>(
  directory: string,
  phase: InputLiveV40RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV40AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV40ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV40Request>(
    inputLiveV40RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV40Request(directory, request);
  if (phase === "capture") validateInputLiveV40TechnicalGates(directory);
  const rawFile = inputLiveV40ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v40-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V40_BROKER_VERSION ||
...
export function persistInputLiveV40TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV40RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV40RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V40_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v40-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V40_BROKER_VERSION,
...
export function validateInputLiveV40TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v40-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V40_TECHNICAL_GATES.some(
...
export function validateInputLiveV40Receipt(
  directory: string,
  phase: InputLiveV40RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV40Request(directory, request);
  const receipt = json<InputLiveV40Receipt>(
    inputLiveV40ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV40ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v40-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V40_BROKER_VERSION ||
...
export function readInputLiveV40AcceptedRawPayload(
  directory: string,
  phase: InputLiveV40RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV40Receipt(directory, phase, captureIndex);
...
export function createInputLiveV40FakeDynamicResponse(
  request: InputLiveV40Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V40_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V40_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV40PersistedRequests(
  directory: string,
): InputLiveV40Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV40Request>(file)] : [];
    });
}

export const inputLiveV40Sha256 = sha256;
