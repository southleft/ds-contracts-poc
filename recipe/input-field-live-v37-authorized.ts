...
import { INPUT_LIVE_V37_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v37.js";
import { verifyInputLiveV37Authorization } from "./input-field-live-v37-authorization.js";
import { runInputLiveV37Preflight } from "./input-field-live-v37-preflight.js";
import { InputLiveV37Orchestrator } from "./run-input-field-live-v37.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V37_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v37-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV37Authorization(root);
    runInputLiveV37Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV37Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
