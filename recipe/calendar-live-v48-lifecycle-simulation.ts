...
import { calendarLiveV1Sha256 } from "./calendar-live-v48-broker.js";

export interface CalendarLiveV48LifecycleSimulationReport {
  artifactVersion: "calendar-live-v48-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV48AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV48Lifecycle(): Promise<CalendarLiveV48LifecycleSimulationReport> {
  await buildCalendarLiveV48Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V48_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV48AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v48-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V48_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V48_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV48History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV48History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV48AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V48_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V48_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV48History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV48History(authorized, "pending");
    ...
    await buildCalendarLiveV48Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V48_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV48AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV48Lifecycle(), null, 2)}\n`,
  );
