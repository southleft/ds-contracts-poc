...
import {
  acceptCalendarLiveV8Response,
  createCalendarLiveV8Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV8Request,
  persistCalendarLiveV8RawResponse,
  persistCalendarLiveV8TechnicalGates,
  readCalendarLiveV8AcceptedRawPayload,
  type CalendarLiveV8Request,
  type CalendarLiveV8TransactionAuthorization,
} from "./calendar-live-v8-broker.js";
import {
  CALENDAR_LIVE_V8_CAPTURE_COUNT,
  assertCalendarLiveV8CaptureResponses,
  ...
  evaluateCalendarLiveV8Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV8Roots,
  validateCalendarLiveV8CaptureManifest,
  validateCalendarLiveV8CapturePayload,
  validateCalendarLiveV8CleanupPayload,
  validateCalendarLiveV8ExtractPayload,
  validateCalendarLiveV8ProbePayload,
  validateCalendarLiveV8WriterPayload,
  type CalendarLiveV8CaptureCell,
  type CalendarLiveV8CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V8_PLAN_PATH,
  type CalendarLiveV8ProofPlan,
...
export const CALENDAR_LIVE_V8_RUNNER_VERSION = "calendar-live-v8-runner-v1";

interface CalendarLiveV8JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV8OrchestratorState {
  artifactVersion: "calendar-live-v8-orchestrator-state-v1";
  ...
  journal: CalendarLiveV8JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV8ProofPlan;
  planSha256: string;
  captures: CalendarLiveV8CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV8SourceIdentity & {
...
export interface CalendarLiveV8NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV8OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v8 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V8_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V8_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV8ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV8CaptureCell[];
  validateCalendarLiveV8CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v8 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v8 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV8WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV8RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV8Request =>
  readJson<CalendarLiveV8Request>(file);

export class CalendarLiveV8Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV8Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV8OrchestratorState = {
      artifactVersion: "calendar-live-v8-orchestrator-state-v1",
    ...
    return new CalendarLiveV8Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV8OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV8Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV8NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV8NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v8 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV8NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v8 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV8RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV8Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV8CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV8AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV8WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV8Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV8ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV8Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV8Response(this.#directory, "writer", {
        validate: validateCalendarLiveV8WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV8Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV8RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV8Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV8ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV8Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV8RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v8-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV8Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV8ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV8Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV8ProbePayload(payload, ownership),
      });
      persistCalendarLiveV8TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v8 capture cell missing");
      const accepted = acceptCalendarLiveV8Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV8CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V8_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV8CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV8Objective(
    ...
    throw new Error(`Calendar live v8 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV8Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV8CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV8Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV8RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV8Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV8CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV8CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV8CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV8Authorization = (
  privateKey: KeyObject,
): CalendarLiveV8TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV8SimulationKey = (): KeyObject =>
