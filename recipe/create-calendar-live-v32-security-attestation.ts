...
import { verifyCalendarLiveV32Authorization } from "./calendar-live-v32-authorization.js";
import { CALENDAR_LIVE_V32_TARGET } from "./calendar-live-v32-broker.js";
import {
  CALENDAR_LIVE_V32_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV32Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v32-session";
...
const attestation: CalendarLiveV32SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV32SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v32 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V32_SECURITY_ATTESTATION_DEFAULT_PATH,
);
