...
import {
  acceptCalendarLiveV4Response,
  createCalendarLiveV4Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV4Request,
  persistCalendarLiveV4RawResponse,
  persistCalendarLiveV4TechnicalGates,
  readCalendarLiveV4AcceptedRawPayload,
  type CalendarLiveV4Request,
  type CalendarLiveV4TransactionAuthorization,
} from "./calendar-live-v4-broker.js";
import {
  CALENDAR_LIVE_V4_CAPTURE_COUNT,
  assertCalendarLiveV4CaptureResponses,
  ...
  evaluateCalendarLiveV4Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV4Roots,
  validateCalendarLiveV4CaptureManifest,
  validateCalendarLiveV4CapturePayload,
  validateCalendarLiveV4CleanupPayload,
  validateCalendarLiveV4ExtractPayload,
  validateCalendarLiveV4ProbePayload,
  validateCalendarLiveV4WriterPayload,
  type CalendarLiveV4CaptureCell,
  type CalendarLiveV4CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V4_PLAN_PATH,
  type CalendarLiveV4ProofPlan,
} from "./build-calendar-live-proof-v4.js";
import {
...
export const CALENDAR_LIVE_V4_RUNNER_VERSION = "calendar-live-v4-runner-v1";

interface CalendarLiveV4JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV4OrchestratorState {
  artifactVersion: "calendar-live-v4-orchestrator-state-v1";
  ...
  journal: CalendarLiveV4JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV4ProofPlan;
  planSha256: string;
  captures: CalendarLiveV4CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV4SourceIdentity & {
...
export interface CalendarLiveV4NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV4OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v4 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V4_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V4_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV4ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV4CaptureCell[];
  validateCalendarLiveV4CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v4 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v4 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV4WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV4RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV4Request =>
  readJson<CalendarLiveV4Request>(file);

export class CalendarLiveV4Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV4Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV4OrchestratorState = {
      artifactVersion: "calendar-live-v4-orchestrator-state-v1",
    ...
    return new CalendarLiveV4Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV4OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV4Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV4NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV4NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v4 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV4NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v4 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV4RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV4Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV4CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV4AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV4WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV4Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV4ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV4Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV4Response(this.#directory, "writer", {
        validate: validateCalendarLiveV4WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV4Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV4RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV4Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV4ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV4Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV4RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v4-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV4Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV4ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV4Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV4ProbePayload(payload, ownership),
      });
      persistCalendarLiveV4TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v4 capture cell missing");
      const accepted = acceptCalendarLiveV4Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV4CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V4_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV4CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV4Objective(
    ...
    throw new Error(`Calendar live v4 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV4Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV4CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV4Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV4RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV4Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV4CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV4CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV4CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV4Authorization = (
  privateKey: KeyObject,
): CalendarLiveV4TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV4SimulationKey = (): KeyObject =>
