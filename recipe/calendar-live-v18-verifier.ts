...
import {
  verifyCalendarLiveV18FixedSceneFixedPoint,
  type CalendarLiveV18FixedFixedPoint,
} from "./calendar-live-v18-fixed-point.js";

export const CALENDAR_LIVE_V18_VERIFIER_VERSION = "calendar-live-v18-verifier-v1";

export function buildCalendarLiveV18RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV18RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV18RawNode[];
}

export interface CalendarLiveV18NormalizedScene {
...
export interface CalendarLiveV18SceneProof {
...
export function normalizeCalendarLiveV18Scene(
  root: CalendarLiveV18RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV18NormalizedScene {
  const canonicalBindings: CalendarLiveV18NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV18RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV18Scene<Instance>(
  normalized: CalendarLiveV18NormalizedScene,
...
): CalendarLiveV18SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV18FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV18ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV18PreCaptureGates(payload: unknown): void {
