...
import { INPUT_LIVE_V67_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v67.js";
import { verifyInputLiveV67Authorization } from "./input-field-live-v67-authorization.js";
import { runInputLiveV67Preflight } from "./input-field-live-v67-preflight.js";
import { InputLiveV67Orchestrator } from "./run-input-field-live-v67.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V67_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v67-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV67Authorization(root);
    runInputLiveV67Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV67Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
