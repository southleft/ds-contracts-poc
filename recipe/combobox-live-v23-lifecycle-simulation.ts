...
export interface ComboboxLiveV23LifecycleSimulationReport {
  artifactVersion: "combobox-live-v23-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV23AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV23Lifecycle(): Promise<ComboboxLiveV23LifecycleSimulationReport> {
  await buildComboboxLiveV23Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V23_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV23AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v23-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V23_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V23_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV23History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV23History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV23AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V23_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V23_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV23History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV23History(authorized, "pending");
    ...
    await buildComboboxLiveV23Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V23_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV23AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV23Lifecycle(), null, 2)}\n`,
  );
