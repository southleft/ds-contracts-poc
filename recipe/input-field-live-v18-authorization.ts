...
import {
  ...
  INPUT_LIVE_V18_INDEX_PATH,
} from "./build-input-field-live-proof-v18.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV18HistoryExpectation = "pending" | "authorized";

export interface InputLiveV18AuthorizationArtifact {
  artifactVersion: "input-live-v18-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V18_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV18AntecedentIndex {
...
export interface InputLiveV18HistoryState {
  index?: InputLiveV18AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV18AuthorizationArtifact;
...
export interface InputLiveV18AuthorizationProof extends InputLiveV18TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V18_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V18_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V18_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V18_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v18-status") ||
...
export function validateInputLiveV18AntecedentIndex(
  index: InputLiveV18AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v18-antecedent-index-v1" ||
...
export function validateInputLiveV18UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV18History\(\s*["']/.test(source) ||
    /readInputLiveV18HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV18AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV18AuthorizationArtifact {
  ...
  ) as InputLiveV18AntecedentIndex;
  const indexFailures = validateInputLiveV18AntecedentIndex(index);
...
export function validateInputLiveV18AuthorizationArtifact(
  artifact: InputLiveV18AuthorizationArtifact | undefined,
  index: InputLiveV18AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v18-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v18" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V18_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V18_TARGET) ||
    !equalJson(
...
export function validateInputLiveV18History(
  state: InputLiveV18HistoryState,
  expected: InputLiveV18HistoryExpectation,
): string[] {
  const failures = validateInputLiveV18AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV18AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV18HistoryState(
  root = process.cwd(),
): InputLiveV18HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V18_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV18AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V18_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V18_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V18_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV18History(
  expected: InputLiveV18HistoryExpectation,
  root = process.cwd(),
): InputLiveV18HistoryState {
  const state = readInputLiveV18HistoryState(root);
  const failures = validateInputLiveV18History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV18Authorization(
  root = process.cwd(),
): InputLiveV18AuthorizationProof {
  const state = verifyInputLiveV18History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V18_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v18/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v18.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V18_TARGET,
    expectedDynamicTool: INPUT_LIVE_V18_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V18_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV18HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV18History(expected);
