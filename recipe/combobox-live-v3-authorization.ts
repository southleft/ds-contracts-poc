...
export type ComboboxLiveV3HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV3AuthorizationArtifact {
  artifactVersion: "combobox-live-v3-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V3_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV3AntecedentIndex {
...
export interface ComboboxLiveV3HistoryState {
  index?: ComboboxLiveV3AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV3AuthorizationArtifact;
...
export interface ComboboxLiveV3AuthorizationProof extends ComboboxLiveV3TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V3_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V3_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V3_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V3_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v3-authorization") ||
  artifactPath.includes("combobox-live-v3-preflight") ||
  artifactPath.includes("combobox-live-v3-authorized") ||
  artifactPath.includes("create-combobox-live-v3-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v3-status") ||
...
export function validateComboboxLiveV3AntecedentIndex(
  index: ComboboxLiveV3AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v3-antecedent-index-v1" ||
...
export function validateComboboxLiveV3UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV3History\(\s*["']/.test(source) ||
    /readComboboxLiveV3HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV3AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV3AuthorizationArtifact {
  ...
  ) as ComboboxLiveV3AntecedentIndex;
  const indexFailures = validateComboboxLiveV3AntecedentIndex(index);
...
export function validateComboboxLiveV3AuthorizationArtifact(
  artifact: ComboboxLiveV3AuthorizationArtifact | undefined,
  index: ComboboxLiveV3AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v3-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v3" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V3_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V3_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V3_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V3_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V3_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V3_SOURCE_ROOTS ||
...
export function validateComboboxLiveV3History(
  state: ComboboxLiveV3HistoryState,
  expected: ComboboxLiveV3HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV3AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV3AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV3HistoryState(
  root = process.cwd(),
): ComboboxLiveV3HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V3_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV3AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V3_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V3_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V3_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV3History(
  expected: ComboboxLiveV3HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV3HistoryState {
  const state = readComboboxLiveV3HistoryState(root);
  const failures = validateComboboxLiveV3History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v3 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV3Authorization(
  root = process.cwd(),
): ComboboxLiveV3AuthorizationProof {
  const state = verifyComboboxLiveV3History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V3_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v3/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v3.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V3_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V3_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V3_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV3HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV3History(expected);
  process.stdout.write(
    `Combobox live v3 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
