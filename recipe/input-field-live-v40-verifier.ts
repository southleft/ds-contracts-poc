...
export const INPUT_LIVE_V40_VERIFIER_VERSION = "input-live-v40-verifier-v1";

export function buildInputLiveV40RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV40RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV40RawNode[];
}

export interface InputLiveV40NormalizedScene {
...
export interface InputLiveV40SceneProof {
...
export function normalizeInputLiveV40Scene(
  root: InputLiveV40RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV40NormalizedScene {
  const canonicalBindings: InputLiveV40NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV40RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV40Scene<Instance>(
  normalized: InputLiveV40NormalizedScene,
...
export function compileInputLiveV40ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV40PreCaptureGates(payload: unknown): void {
