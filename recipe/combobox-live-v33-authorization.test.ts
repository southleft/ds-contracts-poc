...
import { COMBOBOX_LIVE_V33_INDEX_PATH } from "./build-combobox-live-proof-v33.js";
import {
  COMBOBOX_LIVE_V33_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V33_TARGET,
} from "./combobox-live-v33-broker.js";
import {
  buildComboboxLiveV33AuthorizationArtifact,
  validateComboboxLiveV33AntecedentIndex,
  validateComboboxLiveV33AuthorizationArtifact,
  validateComboboxLiveV33History,
  validateComboboxLiveV33UnitTestSource,
  type ComboboxLiveV33AntecedentIndex,
  type ComboboxLiveV33AuthorizationArtifact,
  type ComboboxLiveV33AuthorizationProof,
  type ComboboxLiveV33HistoryState,
} from "./combobox-live-v33-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV33ControlFlowSource,
  validateComboboxLiveV33SecurityAttestation,
  type ComboboxLiveV33SecurityAttestation,
} from "./combobox-live-v33-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v33-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V33_INDEX_PATH);
const index = (): ComboboxLiveV33AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV33AntecedentIndex;

const artifact = (): ComboboxLiveV33AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV33AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV33HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV33HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV33AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v33/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV33AuthorizationArtifact,
): ComboboxLiveV33SecurityAttestation => {
  const body: Omit<ComboboxLiveV33SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V33_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV33History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV33History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV33History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV33History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV33History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV33UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV33UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV33AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV33AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV33AuthorizationArtifact(
        planted as ComboboxLiveV33AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV33SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV33SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV33SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV33ControlFlowSource(source), []);
});
