...
export type CalendarLiveV47HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV47AuthorizationArtifact {
  artifactVersion: "calendar-live-v47-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V47_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV47AntecedentIndex {
...
export interface CalendarLiveV47HistoryState {
  index?: CalendarLiveV47AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV47AuthorizationArtifact;
...
export interface CalendarLiveV47AuthorizationProof
  extends CalendarLiveV47TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V47_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V47_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V47_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V47_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v47-authorization") ||
  artifactPath.includes("calendar-live-v47-preflight") ||
  artifactPath.includes("calendar-live-v47-authorized") ||
  artifactPath.includes("create-calendar-live-v47-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v47-status") ||
...
export function validateCalendarLiveV47AntecedentIndex(
  index: CalendarLiveV47AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v47-antecedent-index-v1" ||
...
export function validateCalendarLiveV47UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV47History\(\s*["']/.test(source) ||
    /readCalendarLiveV47HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV47AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV47AuthorizationArtifact {
  ...
  ) as CalendarLiveV47AntecedentIndex;
  const indexFailures = validateCalendarLiveV47AntecedentIndex(index);
...
export function validateCalendarLiveV47AuthorizationArtifact(
  artifact: CalendarLiveV47AuthorizationArtifact | undefined,
  index: CalendarLiveV47AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v47-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v47" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V47_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V47_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V47_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V47_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V47_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V47_SOURCE_ROOTS ||
  ...
  if (
    ...
    artifact?.execution?.comboboxLiveV47AuthorizationReusable !== false ||
...
export function validateCalendarLiveV47History(
  state: CalendarLiveV47HistoryState,
  expected: CalendarLiveV47HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV47AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV47AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV47HistoryState(
  root = process.cwd(),
): CalendarLiveV47HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V47_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV47AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V47_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V47_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V47_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV47History(
  expected: CalendarLiveV47HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV47HistoryState {
  const state = readCalendarLiveV47HistoryState(root);
  const failures = validateCalendarLiveV47History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v47 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV47Authorization(
  root = process.cwd(),
): CalendarLiveV47AuthorizationProof {
  const state = verifyCalendarLiveV47History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V47_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v47/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v47.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V47_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V47_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V47_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV47HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV47History(expected);
  process.stdout.write(
    `Calendar live v47 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
