...
import {
  validateComboboxLiveV34AntecedentIndex,
  verifyComboboxLiveV34Authorization,
  type ComboboxLiveV34AntecedentIndex,
  type ComboboxLiveV34AuthorizationProof,
} from "./combobox-live-v34-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V34_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v34/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V34_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v34-security-attestation.json";

export interface ComboboxLiveV34SecurityAttestation {
  artifactVersion: "combobox-live-v34-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV34PreflightReport {
  artifactVersion: "combobox-live-v34-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV34SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV34SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV34AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v34-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V34_TARGET) ||
...
export function validateComboboxLiveV34ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV34Authorization");
  const preflight = source.indexOf("runComboboxLiveV34Preflight");
  const initialize = source.indexOf("ComboboxLiveV34Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV34Preflight(
  root: string,
  proof: ComboboxLiveV34AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V34_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV34PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V34_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V34_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V34_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV34AntecedentIndex;
  failures.push(...validateComboboxLiveV34AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V34_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V34_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV34SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v34-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV34ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v34 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV34SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v34-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V34_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV34Preflight(
    process.cwd(),
    verifyComboboxLiveV34Authorization(),
    attestationPath,
