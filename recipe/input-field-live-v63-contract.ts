...
} from "./input-field-live-v3-verifier-v63.js";
import {
  buildInputLiveV63RawPropertyRuntime,
  normalizeInputLiveV63Scene,
  type InputLiveV63RawNode,
} from "./input-field-live-v63-verifier.js";
...
export const INPUT_LIVE_V63_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V63_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V63_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V63_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V63_VARIANT_COUNT = 256;
export const INPUT_LIVE_V63_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V63_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV63SourceId = (typeof INPUT_LIVE_V63_SOURCE_IDS)[number];

export interface InputLiveV63SourceIdentity {
  source: InputLiveV63SourceId;
...
export interface InputLiveV63WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV63ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV63SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV63RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV63RootProof {
  source: InputLiveV63SourceId;
  adapterIdentity: string;
...
export interface InputLiveV63ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV63SourceId;
    adapterIdentity: string;
...
export interface InputLiveV63CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV63SourceId;
  adapterIdentity: string;
...
export interface InputLiveV63CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV63SourceId;
  strata: InputLiveV63CaptureCell["strata"];
...
export interface InputLiveV63CleanupPayload {
...
export function validateInputLiveV63WriterPayload(
  value: unknown,
): InputLiveV63WriterOwnership {
...
const identityPlan = (source: InputLiveV63SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV63ExtractProgram(
  writer: InputLiveV63WriterOwnership,
  sources: readonly InputLiveV63SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V63_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V63_NAMESPACE)}
${buildInputLiveV63RawPropertyRuntime()}
...
export function validateInputLiveV63ExtractPayload(
  value: unknown,
  writer: InputLiveV63WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V63_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV63ExtractPayload;
}

export function proveInputLiveV63Roots<Instance>(
  extract: InputLiveV63ExtractPayload,
  sources: readonly (InputLiveV63SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV63Scene(
      root.scene,
...
export function assertInputLiveV63RootProofs(
  proofs: readonly InputLiveV63RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV63ProbeProgram(
  writer: InputLiveV63WriterOwnership,
  sources: readonly InputLiveV63SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V63_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV63ProbePayload(
  value: unknown,
  writer: InputLiveV63WriterOwnership,
): InputLiveV63ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV63ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V63_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V63_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV63CaptureManifest(
  cells: readonly InputLiveV63CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V63_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V63_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V63_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V63_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V63_SOURCE_IDS.some(
...
export const inputLiveV63CaptureManifestSha256 = (
  cells: readonly InputLiveV63CaptureCell[],
): string => {
  validateInputLiveV63CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV63CaptureProgram(
  writer: InputLiveV63WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V63_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V63_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V63_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV63CapturePayload(
  value: unknown,
  cell: InputLiveV63CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV63CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V63_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V63_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV63CaptureResponses(
  manifest: readonly InputLiveV63CaptureCell[],
  responses: readonly InputLiveV63CapturePayload[],
): void {
  validateInputLiveV63CaptureManifest(manifest);
  if (
...
export interface InputLiveV63ObjectiveReport {
  artifactVersion: "input-live-v63-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV63Objective(
  manifest: readonly InputLiveV63CaptureCell[],
  responses: readonly InputLiveV63CapturePayload[],
  readReference: (cell: InputLiveV63CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV63CaptureCell) => Buffer,
): InputLiveV63ObjectiveReport {
  assertInputLiveV63CaptureResponses(manifest, responses);
...
export function buildInputLiveV63CleanupProgram(
  writer: InputLiveV63WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V63_NAMESPACE,
...
export function validateInputLiveV63CleanupPayload(
  value: unknown,
  writer: InputLiveV63WriterOwnership,
): InputLiveV63CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV63CleanupPayload;
...
export const INPUT_LIVE_V63_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV63WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
