...
import {
  acceptTableLiveV16Response,
  createTableLiveV16Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV16Request,
  persistTableLiveV16RawResponse,
  persistTableLiveV16TechnicalGates,
  readTableLiveV16AcceptedRawPayload,
  type TableLiveV16Request,
  type TableLiveV16TransactionAuthorization,
} from "./table-live-v16-broker.js";
import {
  TABLE_LIVE_V16_CAPTURE_COUNT,
  assertTableLiveV16CaptureResponses,
  ...
  evaluateTableLiveV16Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV16Roots,
  validateTableLiveV16CaptureManifest,
  validateTableLiveV16CapturePayload,
  validateTableLiveV16CleanupPayload,
  validateTableLiveV16ExtractPayload,
  validateTableLiveV16ProbePayload,
  validateTableLiveV16WriterPayload,
  type TableLiveV16CaptureCell,
  type TableLiveV16CapturePayload,
...
import {
  ...
  TABLE_LIVE_V16_PLAN_PATH,
  type TableLiveV16ProofPlan,
} from "./build-table-live-proof-v16.js";
import {
...
export const TABLE_LIVE_V16_RUNNER_VERSION = "table-live-v16-runner-v1";

interface TableLiveV16JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV16OrchestratorState {
  artifactVersion: "table-live-v16-orchestrator-state-v1";
  ...
  journal: TableLiveV16JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV16ProofPlan;
  planSha256: string;
  captures: TableLiveV16CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV16SourceIdentity & {
...
export interface TableLiveV16NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV16OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v16 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V16_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V16_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV16ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV16CaptureCell[];
  validateTableLiveV16CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v16 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v16 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV16WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV16RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV16Request =>
  readJson<TableLiveV16Request>(file);

export class TableLiveV16Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV16Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV16OrchestratorState = {
      artifactVersion: "table-live-v16-orchestrator-state-v1",
    ...
    return new TableLiveV16Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV16OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV16Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV16NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV16NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v16 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV16NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v16 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV16RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV16Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV16CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV16AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV16WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV16Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV16ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV16Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV16Response(this.#directory, "writer", {
        validate: validateTableLiveV16WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV16Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV16RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV16Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV16ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV16Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV16RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v16-host-root-proofs-v1",
      ...
      const next = issueTableLiveV16Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV16ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV16Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV16ProbePayload(payload, ownership),
      });
      persistTableLiveV16TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v16 capture cell missing");
      const accepted = acceptTableLiveV16Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV16CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V16_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV16CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV16Objective(
    ...
    throw new Error(`Table live v16 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV16Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV16CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV16Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV16RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV16Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV16CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV16CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV16CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV16Authorization = (
  privateKey: KeyObject,
): TableLiveV16TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV16SimulationKey = (): KeyObject =>
