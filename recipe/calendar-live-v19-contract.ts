...
import { buildCalendarLiveV19CleanupRuntime } from "./calendar-live-v19-cleanup.js";
import { type CalendarLiveV19FixedFixedPoint } from "./calendar-live-v19-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV19RestoreProgram,
  validateCalendarLiveV19RestorePayload,
  type CalendarLiveV19RestorePayload,
} from "./calendar-live-v19-restore.js";
import {
  buildCalendarLiveV19RawPropertyRuntime,
  normalizeCalendarLiveV19Scene,
  type CalendarLiveV19RawNode,
} from "./calendar-live-v19-verifier.js";
...
export const CALENDAR_LIVE_V19_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V19_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V19_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V19_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V19_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V19_SET_COUNT = 3;
export const CALENDAR_LIVE_V19_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V19_HOST_PHASES = 3;
export const CALENDAR_LIVE_V19_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V19_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V19_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V19_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV19SourceId = (typeof CALENDAR_LIVE_V19_SOURCE_IDS)[number];
export type CalendarLiveV19Kind = "calendar" | "week" | "day";

export interface CalendarLiveV19SourceIdentity {
  source: CalendarLiveV19SourceId;
...
export interface CalendarLiveV19WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV19ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV19SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV19RawNode;
    weekScene: CalendarLiveV19RawNode;
    dayScene: CalendarLiveV19RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV19RootProof {
  source: CalendarLiveV19SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV19FixedFixedPoint;
}

export interface CalendarLiveV19ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV19SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV19CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV19SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV19CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV19SourceId;
  frameWidth: number;
...
export interface CalendarLiveV19CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v19 ${label} must be an object`);
...
export function validateCalendarLiveV19WriterPayload(
  value: unknown,
): CalendarLiveV19WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v19") ||
    ...
    result.namespace !== CALENDAR_LIVE_V19_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v19 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v19 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v19 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV19SourceIdentity,
  kind: CalendarLiveV19Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV19ExtractProgram(
  writer: CalendarLiveV19WriterOwnership,
  sources: readonly CalendarLiveV19SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v19 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V19_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V19_NAMESPACE)}
${buildCalendarLiveV19RawPropertyRuntime()}
...
export function validateCalendarLiveV19ExtractPayload(
  value: unknown,
  writer: CalendarLiveV19WriterOwnership,
): CalendarLiveV19ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v19 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V19_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v19 extract schema/one-root mismatch");
  return payload as CalendarLiveV19ExtractPayload;
}

export function proveCalendarLiveV19Roots<Instance>(
  extract: CalendarLiveV19ExtractPayload,
  sources: readonly (CalendarLiveV19SourceIdentity & {
...
): CalendarLiveV19RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v19 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v19 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV19Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV19Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV19Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV19FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v19 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v19 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v19 compile lost owned sets");
...
export function assertCalendarLiveV19RootProofs(
  proofs: readonly CalendarLiveV19RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v19 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v19 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV19ProbeProgram(
  writer: CalendarLiveV19WriterOwnership,
  sources: readonly CalendarLiveV19SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V19_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV19ProbePayload(
  value: unknown,
  writer: CalendarLiveV19WriterOwnership,
): CalendarLiveV19ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV19ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V19_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V19_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v19 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV19CaptureManifest(
  cells: readonly CalendarLiveV19CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V19_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V19_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V19_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V19_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v19 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV19CaptureCell[],
): string => {
  validateCalendarLiveV19CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV19CaptureProgram(
  writer: CalendarLiveV19WriterOwnership,
  cell: CalendarLiveV19CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V19_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v19 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v19 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V19_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV19CapturePayload(
  value: unknown,
  cell: CalendarLiveV19CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV19CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV19CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V19_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V19_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v19 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV19CaptureResponses(
  manifest: readonly CalendarLiveV19CaptureCell[],
  responses: readonly CalendarLiveV19CapturePayload[],
): void {
  validateCalendarLiveV19CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v19 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV19ObjectiveReport {
  artifactVersion: "calendar-live-v19-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV19Objective(
  manifest: readonly CalendarLiveV19CaptureCell[],
  responses: readonly CalendarLiveV19CapturePayload[],
): CalendarLiveV19ObjectiveReport {
  assertCalendarLiveV19CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v19-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV19CleanupProgram(
  writer: CalendarLiveV19WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV19CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V19_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v19 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV19CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV19WriterOwnership,
): CalendarLiveV19CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV19CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v19 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V19_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV19WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
