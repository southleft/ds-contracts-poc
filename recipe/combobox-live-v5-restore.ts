...
export const COMBOBOX_LIVE_V5_MEASURE_VISIBLE_FILL_MARKER =
  "COMBOBOX-TEXT-FILL-MEASURE-VISIBLE";
export const COMBOBOX_LIVE_V5_RESTORE_CONTENT_ROLES = [
  "combobox/input",
  "combobox/option/label",
] as const;
export const COMBOBOX_LIVE_V5_RESTORE_PARENT_ROLES = [
  "combobox/trigger",
] as const;
export const COMBOBOX_LIVE_V5_RESTORE_INPUT_SET_ROLE = "combobox/set";
export const COMBOBOX_LIVE_V5_RESTORE_OPTION_LABEL_SET_ROLE =
  "combobox/option-set";
export const COMBOBOX_LIVE_V5_RESTORE_COUNT = 144;
export const FORBIDDEN_INPUT_PAGE_ID = "115:295378";

export interface ComboboxLiveV5RestoreWriter {
  pageId: string;
  runIdentity: string;
  setIds: readonly string[];
}

export interface ComboboxLiveV5RestorePayload {
  pageId: string;
  setIds: string[];
  restoredCount: 144;
  fixedBefore: number;
  ...
  marker: typeof COMBOBOX_LIVE_V5_MEASURE_VISIBLE_FILL_MARKER;
}

export function buildComboboxLiveV5RestoreProgram(
  writer: ComboboxLiveV5RestoreWriter,
): string {
...
const ROLES=new Set(${JSON.stringify([...COMBOBOX_LIVE_V5_RESTORE_CONTENT_ROLES])});
const PARENTS=new Set(${JSON.stringify([...COMBOBOX_LIVE_V5_RESTORE_PARENT_ROLES])});
const INPUT_SET=${JSON.stringify(COMBOBOX_LIVE_V5_RESTORE_INPUT_SET_ROLE)};
const OPTION_SET=${JSON.stringify(COMBOBOX_LIVE_V5_RESTORE_OPTION_LABEL_SET_ROLE)};
const MARKER=${JSON.stringify(COMBOBOX_LIVE_V5_MEASURE_VISIBLE_FILL_MARKER)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("COMBOBOX-V5-RESTORE-PAGE");
if(page.id==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
if(page.getSharedPluginData(NS,"pageOwner")!=="recipe/combobox/"+${JSON.stringify(writer.runIdentity)})throw new Error("COMBOBOX-V5-RESTORE-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>SET_IDS.includes(node.id));
if(sets.length!==4)throw new Error("COMBOBOX-V5-RESTORE-ROOTS:"+sets.length);
const parents=[];
const texts=[];
...
if(restored.length!==144)throw new Error("COMBOBOX-V5-RESTORE-COUNT:"+restored.length);
if(restored.some(entry=>entry.after!=="FILL"))throw new Error("COMBOBOX-V5-RESTORE-NOT-FILL");
...
export function validateComboboxLiveV5RestorePayload(
  payload: unknown,
  writer: ComboboxLiveV5RestoreWriter,
): ComboboxLiveV5RestorePayload {
  ...
  if (
    ...
    value.marker !== COMBOBOX_LIVE_V5_MEASURE_VISIBLE_FILL_MARKER
  )
    throw new TypeError(
      "Combobox live v4 restore did not re-assert content FILL",
    );
  return value as ComboboxLiveV5RestorePayload;
}
