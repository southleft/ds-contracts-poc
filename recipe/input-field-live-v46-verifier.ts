...
export const INPUT_LIVE_V46_VERIFIER_VERSION = "input-live-v46-verifier-v1";

export function buildInputLiveV46RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV46RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV46RawNode[];
}

export interface InputLiveV46NormalizedScene {
...
export interface InputLiveV46SceneProof {
...
export function normalizeInputLiveV46Scene(
  root: InputLiveV46RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV46NormalizedScene {
  const canonicalBindings: InputLiveV46NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV46RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV46Scene<Instance>(
  normalized: InputLiveV46NormalizedScene,
...
export function compileInputLiveV46ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV46PreCaptureGates(payload: unknown): void {
