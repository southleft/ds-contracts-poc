...
} from "./input-field-live-v3-verifier-v41.js";
import {
  buildInputLiveV41RawPropertyRuntime,
  normalizeInputLiveV41Scene,
  type InputLiveV41RawNode,
} from "./input-field-live-v41-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v41.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v41.js";

export {
  buildInputLiveV41RestoreProgram,
  validateInputLiveV41RestorePayload,
  type InputLiveV41RestorePayload,
} from "./input-field-live-v41-restore.js";

export const INPUT_LIVE_V41_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V41_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V41_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V41_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V41_VARIANT_COUNT = 256;
export const INPUT_LIVE_V41_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V41_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV41SourceId = (typeof INPUT_LIVE_V41_SOURCE_IDS)[number];

export interface InputLiveV41SourceIdentity {
  source: InputLiveV41SourceId;
...
export interface InputLiveV41WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV41ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV41SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV41RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV41RootProof {
  source: InputLiveV41SourceId;
  adapterIdentity: string;
...
export interface InputLiveV41ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV41SourceId;
    adapterIdentity: string;
...
export interface InputLiveV41CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV41SourceId;
  adapterIdentity: string;
...
export interface InputLiveV41CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV41SourceId;
  strata: InputLiveV41CaptureCell["strata"];
...
export interface InputLiveV41CleanupPayload {
...
export function validateInputLiveV41WriterPayload(
  value: unknown,
): InputLiveV41WriterOwnership {
...
const identityPlan = (source: InputLiveV41SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV41ExtractProgram(
  writer: InputLiveV41WriterOwnership,
  sources: readonly InputLiveV41SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V41_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V41_NAMESPACE)}
${buildInputLiveV41RawPropertyRuntime()}
...
export function validateInputLiveV41ExtractPayload(
  value: unknown,
  writer: InputLiveV41WriterOwnership,
): InputLiveV41ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V41_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV41ExtractPayload;
}

export function proveInputLiveV41Roots<Instance>(
  extract: InputLiveV41ExtractPayload,
  sources: readonly (InputLiveV41SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV41Scene(
      root.scene,
...
export function assertInputLiveV41RootProofs(
  proofs: readonly InputLiveV41RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV41ProbeProgram(
  writer: InputLiveV41WriterOwnership,
  sources: readonly InputLiveV41SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V41_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV41ProbePayload(
  value: unknown,
  writer: InputLiveV41WriterOwnership,
): InputLiveV41ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV41ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V41_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V41_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV41CaptureManifest(
  cells: readonly InputLiveV41CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V41_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V41_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V41_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V41_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V41_SOURCE_IDS.some(
...
export const inputLiveV41CaptureManifestSha256 = (
  cells: readonly InputLiveV41CaptureCell[],
): string => {
  validateInputLiveV41CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV41CaptureProgram(
  writer: InputLiveV41WriterOwnership,
  cell: InputLiveV41CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V41_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V41_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V41_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV41CapturePayload(
  value: unknown,
  cell: InputLiveV41CaptureCell,
  rawResponseBytes: number,
): InputLiveV41CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV41CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V41_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V41_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV41CaptureResponses(
  manifest: readonly InputLiveV41CaptureCell[],
  responses: readonly InputLiveV41CapturePayload[],
): void {
  validateInputLiveV41CaptureManifest(manifest);
  if (
...
export interface InputLiveV41ObjectiveReport {
  artifactVersion: "input-live-v41-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV41Objective(
  manifest: readonly InputLiveV41CaptureCell[],
  responses: readonly InputLiveV41CapturePayload[],
  readReference: (cell: InputLiveV41CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV41CaptureCell) => Buffer,
): InputLiveV41ObjectiveReport {
  assertInputLiveV41CaptureResponses(manifest, responses);
...
export function buildInputLiveV41CleanupProgram(
  writer: InputLiveV41WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V41_NAMESPACE,
...
export function validateInputLiveV41CleanupPayload(
  value: unknown,
  writer: InputLiveV41WriterOwnership,
): InputLiveV41CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV41CleanupPayload;
...
export const INPUT_LIVE_V41_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV41WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
