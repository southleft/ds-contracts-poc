...
import {
  validateInputLiveV62AntecedentIndex,
  verifyInputLiveV62Authorization,
  type InputLiveV62AntecedentIndex,
  type InputLiveV62AuthorizationProof,
} from "./input-field-live-v62-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V62_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v62/operator-security-attestation-template.json";
export const INPUT_LIVE_V62_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v62-security-attestation.json";

export interface InputLiveV62SecurityAttestation {
  artifactVersion: "input-live-v62-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface InputLiveV62PreflightReport {
  artifactVersion: "input-live-v62-preflight-v1";
...
export const inputLiveV62AttestationSha256 = (
  value: Omit<InputLiveV62SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV62SecurityAttestation(
  value: unknown,
  proof: InputLiveV62AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v62-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V62_TARGET) ||
...
export function validateInputLiveV62ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV62Authorization");
  const preflight = source.indexOf("runInputLiveV62Preflight");
  const initialize = source.indexOf("InputLiveV62Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV62Preflight(
  root: string,
  proof: InputLiveV62AuthorizationProof,
  attestationPath = INPUT_LIVE_V62_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV62PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V62_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V62_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V62_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV62AntecedentIndex;
  failures.push(...validateInputLiveV62AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V62_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V62_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateInputLiveV62SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v62-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV62ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV62SecurityAttestation;
  return {
    artifactVersion: "input-live-v62-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V62_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV62Preflight(
    process.cwd(),
    verifyInputLiveV62Authorization(),
    attestationPath,
