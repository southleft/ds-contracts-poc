...
import { COMBOBOX_LIVE_V38_INDEX_PATH } from "./build-combobox-live-proof-v38.js";
import {
...
import {
  ...
  validateComboboxLiveV38History,
  validateComboboxLiveV38UnitTestSource,
  type ComboboxLiveV38AntecedentIndex,
  type ComboboxLiveV38AuthorizationArtifact,
  type ComboboxLiveV38AuthorizationProof,
  type ComboboxLiveV38HistoryState,
} from "./combobox-live-v38-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV38ControlFlowSource,
  validateComboboxLiveV38SecurityAttestation,
  type ComboboxLiveV38SecurityAttestation,
} from "./combobox-live-v38-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v38-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V38_INDEX_PATH);
const index = (): ComboboxLiveV38AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV38AntecedentIndex;

const artifact = (): ComboboxLiveV38AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV38AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV38HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV38HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV38AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v38/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV38AuthorizationArtifact,
): ComboboxLiveV38SecurityAttestation => {
  const body: Omit<ComboboxLiveV38SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V38_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV38History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV38History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV38History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV38History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV38History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV38UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV38UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV38AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV38AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV38AuthorizationArtifact(
        planted as ComboboxLiveV38AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV38SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV38SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV38SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV38ControlFlowSource(source), []);
});
