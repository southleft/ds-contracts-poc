...
import {
  INPUT_LIVE_V28_CONTENT_BINDING_COMPILE_ORDER_MARKER,
  INPUT_LIVE_V28_CONTENT_BINDING_EXTRAS_MARKER,
  INPUT_LIVE_V28_CONTENT_HIDDEN_HEIGHT_HUG_MARKER,
  INPUT_LIVE_V28_CONTENT_LETTER_SPACING_OMITTED_MARKER,
  INPUT_LIVE_V28_CONTENT_COMPILE_BINDING_FIELDS,
  INPUT_LIVE_V28_CONTENT_ROLES,
  INPUT_LIVE_V28_SLOT_COLOR_BINDING_MARKER,
  INPUT_LIVE_V28_SLOT_FILL_MARKER,
  INPUT_LIVE_V28_SLOT_ROLES,
  INPUT_LIVE_V28_SURFACE_BINDING_COMPILE_ORDER_MARKER,
  INPUT_LIVE_V28_SURFACE_BINDING_EXTRAS_MARKER,
  INPUT_LIVE_V28_SURFACE_COMPILE_BINDING_FIELDS,
  INPUT_LIVE_V28_SURFACE_LAYOUT_HEIGHT_ALIAS_MARKER,
  INPUT_LIVE_V28_SURFACE_LAYOUT_HEIGHT_MARKER,
  INPUT_LIVE_V28_SURFACE_STROKE_WEIGHT_MARKER,
  INPUT_LIVE_V28_VARIANT_LAYOUT_WIDTH_ALIAS_MARKER,
  INPUT_LIVE_V28_VARIANT_LAYOUT_WIDTH_COMPILE_INDEX_MARKER,
  INPUT_LIVE_V28_VARIANT_LAYOUT_WIDTH_MARKER,
...
test("v17 surfaces empty leading-slot fills from instancePayload", () => {
  ...
  assert.equal(INPUT_LIVE_V28_SLOT_FILL_MARKER, "INPUT-SLOT-FILL-FROM-PAYLOAD");
  assert.deepEqual([...INPUT_LIVE_V28_SLOT_ROLES], [
...
test("v26 drops duplicate mapped content color bindings", () => {
  ...
  assert.deepEqual([...INPUT_LIVE_V28_CONTENT_ROLES], [
...
test("v28 recovers hug height for hidden content text with FIXED vertical sizing", () => {
  const ir = sceneToNormalizedIr({
    ...contentScene("input-field/content/placeholder", [
      {
        field: "fills.0.color",
...
test("v28 does not rewrite visible content FIXED height or invent a height variable", () => {
  const ir = sceneToNormalizedIr({
    ...contentScene("input-field/content/value", [
      {
        field: "fontSize.0",
...
test("v28 does not lift hidden FIXED height onto non-content text", () => {
  const ir = sceneToNormalizedIr({
    ownershipKey: "label",
    type: "TEXT",
...
test("v28 omits content letterSpacing that compile never emits", () => {
  const ir = sceneToNormalizedIr({
    ...contentScene("input-field/content/placeholder", [
      {
        field: "fontSize.0",
...
test("v28 does not omit letterSpacing on non-content text", () => {
  const ir = sceneToNormalizedIr({
    ownershipKey: "label",
    type: "TEXT",
    name: "input-field/label :: Label",
