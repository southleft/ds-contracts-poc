/**
 * Calendar live v3 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V3_FIXED_VERIFIER_VERSION = "calendar-live-v3-fixed-point-v1";
export const CALENDAR_LIVE_V3_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV3FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV3FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV3FixedSourceProbe {
...
export interface CalendarLiveV3FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV3FixedFixedPoint;
}

export interface CalendarLiveV3FixedHardGateInput {
  thresholds: CalendarLiveV3FixedThresholds;
  sourceProbes: CalendarLiveV3FixedSourceProbe[];
  cells: CalendarLiveV3FixedCellValidation[];
  sceneProofs: CalendarLiveV3FixedSceneProof[];
...
export interface CalendarLiveV3FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V3_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV3FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV3FixedHardGates(
  input: CalendarLiveV3FixedHardGateInput,
): CalendarLiveV3FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV3FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V3_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV3FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV3FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
