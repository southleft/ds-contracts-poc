...
export const INPUT_LIVE_V9_VERIFIER_VERSION = "input-live-v9-verifier-v1";

export function buildInputLiveV9RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV9RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV9RawNode[];
}

export interface InputLiveV9NormalizedScene {
...
export interface InputLiveV9SceneProof {
...
export function normalizeInputLiveV9Scene(
  root: InputLiveV9RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV9NormalizedScene {
  const canonicalBindings: InputLiveV9NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: InputLiveV9RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV9Scene<Instance>(
  normalized: InputLiveV9NormalizedScene,
...
export function compileInputLiveV9ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV9PreCaptureGates(payload: unknown): void {
