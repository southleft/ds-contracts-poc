...
import {
  ...
  type CalendarLiveV43TransactionAuthorization,
...
export type CalendarLiveV43HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV43AuthorizationArtifact {
  artifactVersion: "calendar-live-v43-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V43_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV43AntecedentIndex {
...
export interface CalendarLiveV43HistoryState {
  index?: CalendarLiveV43AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV43AuthorizationArtifact;
...
export interface CalendarLiveV43AuthorizationProof
  extends CalendarLiveV43TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V43_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V43_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V43_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V43_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v43-authorization") ||
  artifactPath.includes("calendar-live-v43-preflight") ||
  artifactPath.includes("calendar-live-v43-authorized") ||
  artifactPath.includes("create-calendar-live-v43-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v43-status") ||
...
export function validateCalendarLiveV43AntecedentIndex(
  index: CalendarLiveV43AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v43-antecedent-index-v1" ||
...
export function validateCalendarLiveV43UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV43History\(\s*["']/.test(source) ||
    /readCalendarLiveV43HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV43AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV43AuthorizationArtifact {
  ...
  ) as CalendarLiveV43AntecedentIndex;
  const indexFailures = validateCalendarLiveV43AntecedentIndex(index);
...
export function validateCalendarLiveV43AuthorizationArtifact(
  artifact: CalendarLiveV43AuthorizationArtifact | undefined,
  index: CalendarLiveV43AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v43-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v43" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V43_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V43_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V43_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V43_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V43_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V43_SOURCE_ROOTS ||
  ...
  if (
    ...
    artifact?.execution?.comboboxLiveV43AuthorizationReusable !== false ||
...
export function validateCalendarLiveV43History(
  state: CalendarLiveV43HistoryState,
  expected: CalendarLiveV43HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV43AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV43AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV43HistoryState(
  root = process.cwd(),
): CalendarLiveV43HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V43_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV43AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V43_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V43_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V43_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV43History(
  expected: CalendarLiveV43HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV43HistoryState {
  const state = readCalendarLiveV43HistoryState(root);
  const failures = validateCalendarLiveV43History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v43 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV43Authorization(
  root = process.cwd(),
): CalendarLiveV43AuthorizationProof {
  const state = verifyCalendarLiveV43History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V43_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v43/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v43.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V43_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V43_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V43_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV43HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV43History(expected);
  process.stdout.write(
    `Calendar live v43 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
