...
export interface InputLiveV80LifecycleSimulationReport {
  artifactVersion: "input-live-v80-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV80AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV80Lifecycle(): Promise<InputLiveV80LifecycleSimulationReport> {
  await buildInputLiveV80Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V80_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV80AntecedentIndex;
  const indexSha256 = inputLiveV80Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v80-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V80_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V80_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV80History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV80History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV80AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V80_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V80_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV80History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV80History(authorized, "pending");
    ...
    await buildInputLiveV80Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V80_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV80AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV80Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV80Lifecycle(), null, 2)}\n`,
  );
