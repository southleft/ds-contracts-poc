...
import {
  ...
  createInputLiveV71FakeDynamicResponse,
  ...
  listInputLiveV71PersistedRequests,
  persistInputLiveV71RawResponse,
  persistInputLiveV71TechnicalGates,
  validateInputLiveV71Receipt,
  validateInputLiveV71Request,
  type InputLiveV71RemotePhase,
...
import {
  ...
  type InputLiveV71ExtractPayload,
  type InputLiveV71ProbePayload,
...
import {
  ...
  simulatedInputLiveV71Authorization,
} from "./run-input-field-live-v71.js";
...
const captures: InputLiveV71CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV71Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v71-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV71Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV71CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV71FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV71FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V71_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V71_TARGET);
  persistInputLiveV71RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV71Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV71Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV71WriterOwnership,
...
const rootProofs = (): InputLiveV71RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v71-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV71Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV71Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV71RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV71RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV71ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV71ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV71CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV71CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV71CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV71Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV71CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV71PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV71ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV71Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV71Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV71Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV71Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V71_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV71RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V71_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV71Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV71Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV71ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV71Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV71WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV71RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV71Request;
    validateInputLiveV71Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV71Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV71CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v71-state-"),
  );
  ...
  try {
    const orchestrator = InputLiveV71Orchestrator.initialize({
      ...
      privateKey,
      authorization: simulatedInputLiveV71Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV71Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V71_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV71RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V71_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV71Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV71Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV71RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V71_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV71Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV71Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV71Request;
