...
import { CALENDAR_LIVE_V25_INDEX_PATH } from "./build-calendar-live-proof-v25.js";
import {
...
import {
  buildCalendarLiveV25AuthorizationArtifact,
  validateCalendarLiveV25AntecedentIndex,
  validateCalendarLiveV25AuthorizationArtifact,
  validateCalendarLiveV25History,
  validateCalendarLiveV25UnitTestSource,
  type CalendarLiveV25AntecedentIndex,
  type CalendarLiveV25AuthorizationArtifact,
  type CalendarLiveV25AuthorizationProof,
  type CalendarLiveV25HistoryState,
} from "./calendar-live-v25-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV25ControlFlowSource,
  validateCalendarLiveV25SecurityAttestation,
  type CalendarLiveV25SecurityAttestation,
} from "./calendar-live-v25-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V25_INDEX_PATH);
const index = (): CalendarLiveV25AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV25AntecedentIndex;

const artifact = (): CalendarLiveV25AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV25AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV25HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV25HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV25AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v25/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV25AuthorizationArtifact,
): CalendarLiveV25SecurityAttestation => {
  const body: Omit<CalendarLiveV25SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V25_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV25History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV25History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV25History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV25History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV25History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV25UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v25-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV25UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV25AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV25AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV25AuthorizationArtifact(
        planted as CalendarLiveV25AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV25SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV25SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV25SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v25-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV25ControlFlowSource(source), []);
});
