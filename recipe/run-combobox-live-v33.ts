...
import {
  acceptComboboxLiveV33Response,
  createComboboxLiveV33Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV33Request,
  persistComboboxLiveV33RawResponse,
  persistComboboxLiveV33TechnicalGates,
  readComboboxLiveV33AcceptedRawPayload,
  type ComboboxLiveV33Request,
  type ComboboxLiveV33TransactionAuthorization,
} from "./combobox-live-v33-broker.js";
import {
  COMBOBOX_LIVE_V33_CAPTURE_COUNT,
  assertComboboxLiveV33CaptureResponses,
  ...
  evaluateComboboxLiveV33Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV33Roots,
  validateComboboxLiveV33CaptureManifest,
  validateComboboxLiveV33CapturePayload,
  validateComboboxLiveV33CleanupPayload,
  validateComboboxLiveV33ExtractPayload,
  validateComboboxLiveV33ProbePayload,
  validateComboboxLiveV33WriterPayload,
  type ComboboxLiveV33CaptureCell,
  type ComboboxLiveV33CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V33_PLAN_PATH,
  type ComboboxLiveV33ProofPlan,
} from "./build-combobox-live-proof-v33.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v33.js";

export const COMBOBOX_LIVE_V33_RUNNER_VERSION = "combobox-live-v33-runner-v1";

interface ComboboxLiveV33JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV33OrchestratorState {
  artifactVersion: "combobox-live-v33-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV33JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV33ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV33CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV33SourceIdentity & {
...
export interface ComboboxLiveV33NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV33OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v33 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V33_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V33_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV33ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV33CaptureCell[];
  validateComboboxLiveV33CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v33 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v33 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV33WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV33RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV33Request =>
  readJson<ComboboxLiveV33Request>(file);

export class ComboboxLiveV33Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV33Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV33OrchestratorState = {
      artifactVersion: "combobox-live-v33-orchestrator-state-v1",
    ...
    return new ComboboxLiveV33Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV33OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV33Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV33NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV33NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v33 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV33NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v33 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV33RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV33Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV33CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV33AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV33WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV33Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV33ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV33Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV33Response(this.#directory, "writer", {
        validate: validateComboboxLiveV33WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV33Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV33RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV33Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV33ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV33Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV33RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v33-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV33Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV33ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV33Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV33ProbePayload(payload, ownership),
      });
      persistComboboxLiveV33TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v33 capture cell missing");
      const accepted = acceptComboboxLiveV33Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV33CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V33_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV33CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV33Objective(
    ...
    throw new Error(`Combobox live v33 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV33Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV33CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV33Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV33RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV33Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV33CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV33CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV33CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV33Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV33TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV33SimulationKey = (): KeyObject =>
