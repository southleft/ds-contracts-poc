/**
 * Carried Combobox live v17 host verifier. Uses scene-readback-combobox-v17 so live
...
export const COMBOBOX_LIVE_V17_FIXED_VERIFIER_VERSION = "combobox-live-v17-fixed-point-v1";
export const COMBOBOX_LIVE_V17_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV17FixedThresholds {
...
export interface ComboboxLiveV17FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV17FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV17FixedSourceProbe {
...
export interface ComboboxLiveV17FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV17FixedFixedPoint;
}

export interface ComboboxLiveV17FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV17FixedHardGateInput {
  thresholds: ComboboxLiveV17FixedThresholds;
  materialRegression: ComboboxLiveV17FixedMaterialRegression;
  sourceProbes: ComboboxLiveV17FixedSourceProbe[];
  cells: ComboboxLiveV17FixedCellValidation[];
  sceneProofs: ComboboxLiveV17FixedSceneProof[];
  visualRows: ComboboxLiveV17FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV17FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V17_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV17FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV17FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV17FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV17FixedHardGates(
  input: ComboboxLiveV17FixedHardGateInput,
): ComboboxLiveV17FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV17FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV17FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V17_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV17FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV17FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV17FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
