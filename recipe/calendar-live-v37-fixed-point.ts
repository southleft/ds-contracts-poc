/**
 * Calendar live v37 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V37_FIXED_VERIFIER_VERSION = "calendar-live-v37-fixed-point-v1";
export const CALENDAR_LIVE_V37_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV37FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV37FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV37FixedSourceProbe {
...
export interface CalendarLiveV37FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV37FixedFixedPoint;
}

export interface CalendarLiveV37FixedHardGateInput {
  thresholds: CalendarLiveV37FixedThresholds;
  sourceProbes: CalendarLiveV37FixedSourceProbe[];
  cells: CalendarLiveV37FixedCellValidation[];
  sceneProofs: CalendarLiveV37FixedSceneProof[];
...
export interface CalendarLiveV37FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V37_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV37FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV37FixedHardGates(
  input: CalendarLiveV37FixedHardGateInput,
): CalendarLiveV37FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV37FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V37_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV37FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV37FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV37FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
