...
import {
  verifyComboboxLiveV13FixedSceneFixedPoint,
  type ComboboxLiveV13FixedFixedPoint,
} from "./combobox-live-v13-fixed-point.js";

export const COMBOBOX_LIVE_V13_VERIFIER_VERSION = "combobox-live-v13-verifier-v1";

export function buildComboboxLiveV13RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV13RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV13RawNode[];
}

export interface ComboboxLiveV13NormalizedScene {
...
export interface ComboboxLiveV13SceneProof {
...
export function normalizeComboboxLiveV13Scene(
  root: ComboboxLiveV13RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV13NormalizedScene {
  const canonicalBindings: ComboboxLiveV13NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV13RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV13Scene<Instance>(
  normalized: ComboboxLiveV13NormalizedScene,
...
): ComboboxLiveV13SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV13FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV13ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV13PreCaptureGates(payload: unknown): void {
