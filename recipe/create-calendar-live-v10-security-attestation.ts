...
import { verifyCalendarLiveV10Authorization } from "./calendar-live-v10-authorization.js";
import { CALENDAR_LIVE_V10_TARGET } from "./calendar-live-v10-broker.js";
import {
  CALENDAR_LIVE_V10_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV10Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v10-session";
...
const attestation: CalendarLiveV10SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV10SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v10 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V10_SECURITY_ATTESTATION_DEFAULT_PATH,
);
