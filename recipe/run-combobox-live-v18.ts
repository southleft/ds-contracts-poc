...
import {
  acceptComboboxLiveV18Response,
  createComboboxLiveV18Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV18Request,
  persistComboboxLiveV18RawResponse,
  persistComboboxLiveV18TechnicalGates,
  readComboboxLiveV18AcceptedRawPayload,
  type ComboboxLiveV18Request,
  type ComboboxLiveV18TransactionAuthorization,
} from "./combobox-live-v18-broker.js";
import {
  COMBOBOX_LIVE_V18_CAPTURE_COUNT,
  assertComboboxLiveV18CaptureResponses,
  ...
  evaluateComboboxLiveV18Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV18Roots,
  validateComboboxLiveV18CaptureManifest,
  validateComboboxLiveV18CapturePayload,
  validateComboboxLiveV18CleanupPayload,
  validateComboboxLiveV18ExtractPayload,
  validateComboboxLiveV18ProbePayload,
  validateComboboxLiveV18WriterPayload,
  type ComboboxLiveV18CaptureCell,
  type ComboboxLiveV18CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V18_PLAN_PATH,
  type ComboboxLiveV18ProofPlan,
} from "./build-combobox-live-proof-v18.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v18.js";

export const COMBOBOX_LIVE_V18_RUNNER_VERSION = "combobox-live-v18-runner-v1";

interface ComboboxLiveV18JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV18OrchestratorState {
  artifactVersion: "combobox-live-v18-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV18JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV18ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV18CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV18SourceIdentity & {
...
export interface ComboboxLiveV18NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV18OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v18 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V18_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V18_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV18ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV18CaptureCell[];
  validateComboboxLiveV18CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v18 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v18 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV18WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV18RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV18Request =>
  readJson<ComboboxLiveV18Request>(file);

export class ComboboxLiveV18Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV18Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV18OrchestratorState = {
      artifactVersion: "combobox-live-v18-orchestrator-state-v1",
    ...
    return new ComboboxLiveV18Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV18OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV18Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV18NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV18NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v18 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV18NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v18 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV18RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV18Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV18CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV18AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV18WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV18Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV18ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV18Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV18Response(this.#directory, "writer", {
        validate: validateComboboxLiveV18WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV18Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV18RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV18Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV18ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV18Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV18RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v18-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV18Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV18ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV18Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV18ProbePayload(payload, ownership),
      });
      persistComboboxLiveV18TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v18 capture cell missing");
      const accepted = acceptComboboxLiveV18Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV18CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V18_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV18CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV18Objective(
    ...
    throw new Error(`Combobox live v18 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV18Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV18CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV18Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV18RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV18Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV18CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV18CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV18CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV18Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV18TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV18SimulationKey = (): KeyObject =>
