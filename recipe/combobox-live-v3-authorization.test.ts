...
import { COMBOBOX_LIVE_V3_INDEX_PATH } from "./build-combobox-live-proof-v3.js";
import {
  COMBOBOX_LIVE_V3_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V3_TARGET,
} from "./combobox-live-v3-broker.js";
import {
  buildComboboxLiveV3AuthorizationArtifact,
  validateComboboxLiveV3AntecedentIndex,
  validateComboboxLiveV3AuthorizationArtifact,
  validateComboboxLiveV3History,
  validateComboboxLiveV3UnitTestSource,
  type ComboboxLiveV3AntecedentIndex,
  type ComboboxLiveV3AuthorizationArtifact,
  type ComboboxLiveV3AuthorizationProof,
  type ComboboxLiveV3HistoryState,
} from "./combobox-live-v3-authorization.js";
import {
  comboboxLiveV3AttestationSha256,
  validateComboboxLiveV3ControlFlowSource,
  validateComboboxLiveV3SecurityAttestation,
  type ComboboxLiveV3SecurityAttestation,
} from "./combobox-live-v3-preflight.js";
import { comboboxLiveV3Sha256 } from "./combobox-live-v3-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V3_INDEX_PATH);
const index = (): ComboboxLiveV3AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV3AntecedentIndex;

const artifact = (): ComboboxLiveV3AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV3AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV3HistoryState => ({
  index: index(),
  indexSha256: comboboxLiveV3Sha256(indexBytes()),
...
const authorizedState = (): ComboboxLiveV3HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV3AuthorizationArtifact,
  ...
  antecedentIndexSha256: comboboxLiveV3Sha256(indexBytes()),
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v3/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV3AuthorizationArtifact,
): ComboboxLiveV3SecurityAttestation => {
  const body: Omit<ComboboxLiveV3SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V3_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: comboboxLiveV3AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV3History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV3History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV3History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV3History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV3History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV3UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV3UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV3AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = comboboxLiveV3Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateComboboxLiveV3AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV3AuthorizationArtifact(
        planted as ComboboxLiveV3AuthorizationArtifact,
        index(),
        comboboxLiveV3Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV3SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV3AttestationSha256(
      body as Omit<ComboboxLiveV3SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV3SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV3ControlFlowSource(source), []);
});
