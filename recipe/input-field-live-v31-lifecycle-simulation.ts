...
export interface InputLiveV31LifecycleSimulationReport {
  artifactVersion: "input-live-v31-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV31AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV31Lifecycle(): Promise<InputLiveV31LifecycleSimulationReport> {
  await buildInputLiveV31Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V31_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV31AntecedentIndex;
  const indexSha256 = inputLiveV31Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v31-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V31_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V31_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV31History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV31History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV31AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V31_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V31_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV31History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV31History(authorized, "pending");
    ...
    await buildInputLiveV31Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V31_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV31AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV31Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV31Lifecycle(), null, 2)}\n`,
  );
