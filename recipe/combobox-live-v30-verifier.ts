...
import {
  verifyComboboxLiveV30FixedSceneFixedPoint,
  type ComboboxLiveV30FixedFixedPoint,
} from "./combobox-live-v30-fixed-point.js";

export const COMBOBOX_LIVE_V30_VERIFIER_VERSION = "combobox-live-v30-verifier-v1";

export function buildComboboxLiveV30RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV30RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV30RawNode[];
}

export interface ComboboxLiveV30NormalizedScene {
...
export interface ComboboxLiveV30SceneProof {
...
export function normalizeComboboxLiveV30Scene(
  root: ComboboxLiveV30RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV30NormalizedScene {
  const canonicalBindings: ComboboxLiveV30NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV30RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV30Scene<Instance>(
  normalized: ComboboxLiveV30NormalizedScene,
...
): ComboboxLiveV30SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV30FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV30ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV30PreCaptureGates(payload: unknown): void {
