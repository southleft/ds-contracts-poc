...
import { buildTableLiveV17CleanupRuntime } from "./table-live-v17-cleanup.js";
import { type TableLiveV17FixedFixedPoint } from "./table-live-v17-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV17RestoreProgram,
  validateTableLiveV17RestorePayload,
  type TableLiveV17RestorePayload,
} from "./table-live-v17-restore.js";
import {
  buildTableLiveV17RawPropertyRuntime,
  normalizeTableLiveV17Scene,
  type TableLiveV17RawNode,
} from "./table-live-v17-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v17.js";
import {
...
export const TABLE_LIVE_V17_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V17_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V17_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V17_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V17_VARIANT_COUNT = 20;
export const TABLE_LIVE_V17_SET_COUNT = 6;
export const TABLE_LIVE_V17_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V17_HOST_PHASES = 3;
export const TABLE_LIVE_V17_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V17_RESTORE_COUNT = 8;
export const TABLE_LIVE_V17_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V17_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV17SourceId = (typeof TABLE_LIVE_V17_SOURCE_IDS)[number];
export type TableLiveV17Kind = "table" | "row" | "cell";

export interface TableLiveV17SourceIdentity {
  source: TableLiveV17SourceId;
...
export interface TableLiveV17WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV17ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV17SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV17RawNode;
    rowScene: TableLiveV17RawNode;
    cellScene: TableLiveV17RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV17RootProof {
  source: TableLiveV17SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV17FixedFixedPoint;
}

export interface TableLiveV17ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV17SourceId;
    adapterIdentity: string;
...
export interface TableLiveV17CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV17SourceId;
  adapterIdentity: string;
...
export interface TableLiveV17CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV17SourceId;
  frameWidth: number;
...
export interface TableLiveV17CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v17 ${label} must be an object`);
...
export function validateTableLiveV17WriterPayload(
  value: unknown,
): TableLiveV17WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V17_NAMESPACE ||
    ...
    throw new TypeError("Table live v17 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v17 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v17 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV17SourceIdentity,
  kind: TableLiveV17Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV17ExtractProgram(
  writer: TableLiveV17WriterOwnership,
  sources: readonly TableLiveV17SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v17 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V17_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V17_NAMESPACE)}
${buildTableLiveV17RawPropertyRuntime()}
...
export function validateTableLiveV17ExtractPayload(
  value: unknown,
  writer: TableLiveV17WriterOwnership,
): TableLiveV17ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v17 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V17_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v17 extract schema/two-root mismatch");
  return payload as TableLiveV17ExtractPayload;
}

export function proveTableLiveV17Roots<Instance>(
  extract: TableLiveV17ExtractPayload,
  sources: readonly (TableLiveV17SourceIdentity & {
...
): TableLiveV17RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v17 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v17 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV17Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV17Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV17Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV17FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v17 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v17 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v17 compile lost owned sets");
...
export function assertTableLiveV17RootProofs(
  proofs: readonly TableLiveV17RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v17 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v17 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV17ProbeProgram(
  writer: TableLiveV17WriterOwnership,
  sources: readonly TableLiveV17SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V17_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV17ProbePayload(
  value: unknown,
  writer: TableLiveV17WriterOwnership,
): TableLiveV17ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV17ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V17_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v17 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV17CaptureManifest(
  cells: readonly TableLiveV17CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V17_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V17_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V17_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V17_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v17 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV17CaptureCell[],
): string => {
  validateTableLiveV17CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV17CaptureProgram(
  writer: TableLiveV17WriterOwnership,
  cell: TableLiveV17CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V17_CAPTURE_COUNT
  )
    throw new TypeError("Table live v17 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v17 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V17_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV17CapturePayload(
  value: unknown,
  cell: TableLiveV17CaptureCell,
  rawResponseBytes: number,
): TableLiveV17CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV17CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V17_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V17_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v17 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV17CaptureResponses(
  manifest: readonly TableLiveV17CaptureCell[],
  responses: readonly TableLiveV17CapturePayload[],
): void {
  validateTableLiveV17CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v17 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV17ObjectiveReport {
  artifactVersion: "table-live-v17-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV17Objective(
  manifest: readonly TableLiveV17CaptureCell[],
  responses: readonly TableLiveV17CapturePayload[],
): TableLiveV17ObjectiveReport {
  assertTableLiveV17CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v17-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV17CleanupProgram(
  writer: TableLiveV17WriterOwnership,
): string {
  const baseProgram = buildTableLiveV17CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V17_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v17 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV17CleanupPayload(
  payload: unknown,
  writer: TableLiveV17WriterOwnership,
): TableLiveV17CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV17CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v17 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V17_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV17WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
