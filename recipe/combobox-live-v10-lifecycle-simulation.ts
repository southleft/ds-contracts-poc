...
export interface ComboboxLiveV10LifecycleSimulationReport {
  artifactVersion: "combobox-live-v10-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV10AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV10Lifecycle(): Promise<ComboboxLiveV10LifecycleSimulationReport> {
  await buildComboboxLiveV10Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V10_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV10AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v10-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V10_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V10_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV10History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV10History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV10AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V10_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V10_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV10History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV10History(authorized, "pending");
    ...
    await buildComboboxLiveV10Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V10_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV10AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV10Lifecycle(), null, 2)}\n`,
  );
