...
import {
  ...
  INPUT_LIVE_V50_INDEX_PATH,
} from "./build-input-field-live-proof-v50.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV50HistoryExpectation = "pending" | "authorized";

export interface InputLiveV50AuthorizationArtifact {
  artifactVersion: "input-live-v50-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V50_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV50AntecedentIndex {
...
export interface InputLiveV50HistoryState {
  index?: InputLiveV50AntecedentIndex;
  indexSha256: string;
...
export interface InputLiveV50AuthorizationProof extends InputLiveV50TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V50_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V50_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V50_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V50_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v50-status") ||
...
export function validateInputLiveV50AntecedentIndex(
  index: InputLiveV50AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v50-antecedent-index-v1" ||
...
export function validateInputLiveV50UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV50History\(\s*["']/.test(source) ||
    /readInputLiveV50HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV50AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV50AuthorizationArtifact {
  ...
  ) as InputLiveV50AntecedentIndex;
  const indexFailures = validateInputLiveV50AntecedentIndex(index);
...
export function validateInputLiveV50AuthorizationArtifact(
  artifact: InputLiveV50AuthorizationArtifact | undefined,
  index: InputLiveV50AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v50-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v50" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V50_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V50_TARGET) ||
    !equalJson(
...
export function validateInputLiveV50History(
  state: InputLiveV50HistoryState,
  expected: InputLiveV50HistoryExpectation,
): string[] {
  const failures = validateInputLiveV50AntecedentIndex(state.index);
...
export function readInputLiveV50HistoryState(
  root = process.cwd(),
): InputLiveV50HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V50_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV50AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V50_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V50_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V50_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV50History(
  expected: InputLiveV50HistoryExpectation,
  root = process.cwd(),
): InputLiveV50HistoryState {
  const state = readInputLiveV50HistoryState(root);
  const failures = validateInputLiveV50History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV50Authorization(
  root = process.cwd(),
): InputLiveV50AuthorizationProof {
  const state = verifyInputLiveV50History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V50_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v50/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v50.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V50_TARGET,
    expectedDynamicTool: INPUT_LIVE_V50_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V50_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV50HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV50History(expected);
