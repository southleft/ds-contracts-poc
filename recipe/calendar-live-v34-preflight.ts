...
import {
  validateCalendarLiveV34AntecedentIndex,
  verifyCalendarLiveV34Authorization,
  type CalendarLiveV34AntecedentIndex,
  type CalendarLiveV34AuthorizationProof,
} from "./calendar-live-v34-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V34_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v34/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V34_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v34-security-attestation.json";

export interface CalendarLiveV34SecurityAttestation {
  artifactVersion: "calendar-live-v34-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV34PreflightReport {
  artifactVersion: "calendar-live-v34-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV34SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV34SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV34AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v34-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V34_TARGET) ||
...
export function validateCalendarLiveV34ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV34Authorization");
  const preflight = source.indexOf("runCalendarLiveV34Preflight");
  const initialize = source.indexOf("CalendarLiveV34Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV34Preflight(
  root: string,
  proof: CalendarLiveV34AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V34_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV34PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V34_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V34_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV34AntecedentIndex;
  failures.push(...validateCalendarLiveV34AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V34_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V34_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV34SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v34-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV34ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v34 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV34SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v34-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V34_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV34Preflight(
    process.cwd(),
    verifyCalendarLiveV34Authorization(),
    attestationPath,
