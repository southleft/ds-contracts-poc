...
import { TABLE_LIVE_V33_INDEX_PATH } from "./build-table-live-proof-v33.js";
import {
...
import {
  buildTableLiveV33AuthorizationArtifact,
  validateTableLiveV33AntecedentIndex,
  validateTableLiveV33AuthorizationArtifact,
  validateTableLiveV33History,
  validateTableLiveV33UnitTestSource,
  type TableLiveV33AntecedentIndex,
  type TableLiveV33AuthorizationArtifact,
  type TableLiveV33AuthorizationProof,
  type TableLiveV33HistoryState,
} from "./table-live-v33-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV33ControlFlowSource,
  validateTableLiveV33SecurityAttestation,
  type TableLiveV33SecurityAttestation,
} from "./table-live-v33-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V33_INDEX_PATH);
const index = (): TableLiveV33AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV33AntecedentIndex;

const artifact = (): TableLiveV33AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV33AuthorizationArtifact({
...
const pendingState = (): TableLiveV33HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV33HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV33AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v33/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV33AuthorizationArtifact,
): TableLiveV33SecurityAttestation => {
  const body: Omit<TableLiveV33SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V33_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV33History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV33History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV33History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV33History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV33History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV33UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v33-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV33UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV33AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV33AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV33AuthorizationArtifact(
        planted as TableLiveV33AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV33SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV33SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV33SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v33-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV33ControlFlowSource(source), []);
});
