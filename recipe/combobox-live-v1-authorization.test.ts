...
import { COMBOBOX_LIVE_V1_INDEX_PATH } from "./build-combobox-live-proof-v1.js";
import {
...
import {
  ...
  validateComboboxLiveV1AntecedentIndex,
  validateComboboxLiveV1AuthorizationArtifact,
  validateComboboxLiveV1History,
  validateComboboxLiveV1UnitTestSource,
  type ComboboxLiveV1AntecedentIndex,
  type ComboboxLiveV1AuthorizationArtifact,
  type ComboboxLiveV1AuthorizationProof,
  type ComboboxLiveV1HistoryState,
} from "./combobox-live-v1-authorization.js";
import {
  comboboxLiveV1AttestationSha256,
  validateComboboxLiveV1ControlFlowSource,
  validateComboboxLiveV1SecurityAttestation,
  type ComboboxLiveV1SecurityAttestation,
} from "./combobox-live-v1-preflight.js";
import { comboboxLiveV1Sha256 } from "./combobox-live-v1-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V1_INDEX_PATH);
const index = (): ComboboxLiveV1AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV1AntecedentIndex;

const artifact = (): ComboboxLiveV1AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV1AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV1HistoryState => ({
  index: index(),
  indexSha256: comboboxLiveV1Sha256(indexBytes()),
...
const authorizedState = (): ComboboxLiveV1HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV1AuthorizationArtifact,
  ...
  antecedentIndexSha256: comboboxLiveV1Sha256(indexBytes()),
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v1/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV1AuthorizationArtifact,
): ComboboxLiveV1SecurityAttestation => {
  const body: Omit<ComboboxLiveV1SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "combobox-live-v1-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V1_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: comboboxLiveV1AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV1History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV1History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV1History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV1History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV1History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV1UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV1UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV1AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = comboboxLiveV1Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateComboboxLiveV1AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV1AuthorizationArtifact(
        planted as ComboboxLiveV1AuthorizationArtifact,
        index(),
        comboboxLiveV1Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV1SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV1AttestationSha256(
      body as Omit<ComboboxLiveV1SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV1SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV1ControlFlowSource(source), []);
});
