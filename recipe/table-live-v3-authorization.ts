...
import {
  TABLE_LIVE_V3_DYNAMIC_TOOL,
  TABLE_LIVE_V3_TARGET,
  type TableLiveV3TransactionAuthorization,
} from "./table-live-v3-broker.js";
import {
...
import {
  ...
  TABLE_LIVE_V3_HOST_PHASES,
  TABLE_LIVE_V3_REMOTE_REQUESTS,
  TABLE_LIVE_V3_SOURCE_ROOTS,
} from "./table-live-v3-contract.js";
import { canonicalJson } from "./normalize.js";

export type TableLiveV3HistoryExpectation = "pending" | "authorized";

export interface TableLiveV3AuthorizationArtifact {
  artifactVersion: "table-live-v3-capture-authorization-v1";
  authorizationId: "table-live-v3";
  status: "authorization declared; runtime security prerequisites still mandatory";
  authorizationIntent: true;
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V3_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV3AntecedentIndex {
  artifactVersion: "table-live-v3-antecedent-index-v1";
  status: string;
...
export interface TableLiveV3HistoryState {
  ...
  authorization?: TableLiveV3AuthorizationArtifact;
...
export interface TableLiveV3AuthorizationProof
  extends TableLiveV3TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V3_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V3_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V3_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V3_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v3-authorization") ||
  artifactPath.includes("table-live-v3-preflight") ||
  artifactPath.includes("table-live-v3-authorized") ||
  artifactPath.includes("create-table-live-v3-security-attestation") ||
  artifactPath.includes("table-live-pivot-v3-status") ||
...
export function validateTableLiveV3AntecedentIndex(
  index: TableLiveV3AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v3-antecedent-index-v1" ||
    index.authorizationCanBeAddedWithoutAntecedentRebuild !== true
  )
    failures.push("table v3 antecedent index missing or malformed");
  const artifactEntries = Object.entries(index?.artifacts ?? {});
  if (artifactEntries.length === 0)
    failures.push("table v3 antecedent hash denominator is empty");
  for (const [artifactPath, metadata] of artifactEntries) {
    ...
    if (
      ...
      failures.push(`malformed table v3 antecedent hash: ${artifactPath}`);
  }
  ...
  if (index?.hashSetSha256 !== expectedHashSet)
    failures.push("table v3 antecedent hash-set digest mismatch");
  if (
    ...
    failures.push("table v3 antecedent counts missing or drifted");
  for (const required of [
    TABLE_LIVE_V3_AUTHORIZATION_PATH,
    "recipe/table-live-v3-authorization.ts",
    "recipe/table-live-v3-authorization.test.ts",
    "recipe/table-live-v3-preflight.ts",
    "recipe/evidence/table-live-pivot-v3/operator-security-attestation-template.json",
    "recipe/evidence/status-index.json",
  ])
    if (!index?.authorizationLifecycleExcluded?.includes(required))
      failures.push(`table v3 lifecycle exclusion missing: ${required}`);
  const leaks = forbiddenAntecedentFieldPaths(index);
  if (leaks.length)
    failures.push(`table v3 antecedent result leakage: ${leaks.join(",")}`);
  return failures;
}

export function validateTableLiveV3UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV3History\(\s*["']/.test(source) ||
    /readTableLiveV3HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV3AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV3AuthorizationArtifact {
  if (!SHA40.test(options.antecedentCommit))
    throw new TypeError("table v3 antecedent commit must be a full SHA");
  const index = JSON.parse(
    Buffer.from(options.antecedentIndexBytes).toString("utf8"),
  ) as TableLiveV3AntecedentIndex;
  const indexFailures = validateTableLiveV3AntecedentIndex(index);
  if (indexFailures.length)
    throw new TypeError(
      `table v3 antecedent index refused:\n${indexFailures.join("\n")}`,
    );
  const publicKey =
  ...
  return {
    ...
    antecedent: {
      ...
      indexPath: TABLE_LIVE_V3_INDEX_PATH,
    ...
    denominator: {
      remoteRequests: TABLE_LIVE_V3_REMOTE_REQUESTS,
      hostPhases: TABLE_LIVE_V3_HOST_PHASES,
      captures: TABLE_LIVE_V3_CAPTURE_COUNT,
      sourceRoots: TABLE_LIVE_V3_SOURCE_ROOTS,
...
export function validateTableLiveV3AuthorizationArtifact(
  artifact: TableLiveV3AuthorizationArtifact | undefined,
  index: TableLiveV3AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v3-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v3" ||
    artifact.authorizationIntent !== true
  )
    failures.push("table v3 authorization declaration missing or malformed");
  const resultLeaks = forbiddenAntecedentFieldPaths(artifact);
  if (resultLeaks.length)
    failures.push(`table v3 authorization result leakage: ${resultLeaks.join(",")}`);
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V3_INDEX_PATH ||
    artifact?.antecedent?.indexSha256 !== indexSha256 ||
    artifact?.antecedent?.hashSetSha256 !== index?.hashSetSha256
  )
    failures.push("table v3 authorization pins wrong antecedent index");
  if (privateMaterialPaths(artifact).length)
    failures.push("table v3 authorization contains private key or token material");
  try {
    ...
    if (
      ...
      failures.push("table v3 Ed25519 public-key identity mismatch");
  } catch {
    failures.push("table v3 Ed25519 public key is invalid");
  }
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V3_TARGET) ||
    !equalJson(
    ...
    failures.push("wrong table v3 target or dynamic tool");
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V3_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V3_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V3_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V3_SOURCE_ROOTS ||
    artifact?.denominator?.expectedFacts !== index?.counts.expectedSceneFacts
  )
    failures.push("table v3 two-root or capture denominator changed");
  if (
    ...
    failures.push(
      "table v3 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
    ...
    failures.push("table v3 security prerequisites weakened");
  if (
    artifact?.humanSignoff?.mandatory !== true ||
    artifact?.humanSignoff?.status !== "pending"
  )
    failures.push("table v3 human signoff overclaim");
  return failures;
}

export function validateTableLiveV3History(
  state: TableLiveV3HistoryState,
  expected: TableLiveV3HistoryExpectation,
): string[] {
  const failures = validateTableLiveV3AntecedentIndex(state.index);
  if (!SHA40.test(state.codeCommit))
    failures.push("table v3 code commit is not a full SHA");
  ...
  if (
    ...
    failures.push("table v3 antecedent first-add commit is not unique");
  if (!state.antecedentIsAncestorOfCode)
    failures.push("code commit does not descend from table v3 antecedent");
  if (!state.antecedentIndexBytesMatchFirstAddition)
    failures.push("table v3 antecedent index changed after first addition");
  if (!state.antecedentArtifactsMatchIndexAtCommit)
    failures.push("table v3 antecedent commit artifacts do not match index");
  if (!state.workingAntecedentArtifactsMatchIndex)
    failures.push("table v3 antecedent working bytes drifted");
  ...
  failures.push(
    ...validateTableLiveV3AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
  ...
  if (
    ...
    failures.push("table v3 authorization first-add lineage mismatch");
  if (!state.authorizationPresentAtCodeCommit)
    failures.push("table v3 authorization missing from code commit");
  if (!state.authorizationBytesMatchFirstAddition)
    failures.push("table v3 authorization bytes changed after first addition");
  if (!state.authorizationStrictlyDescendsFromAntecedent)
    failures.push("table v3 authorization does not strictly descend from antecedent");
  if (!state.authorizationIsAncestorOfCode)
    failures.push("code commit predates table v3 authorization");
  return failures;
}
...
export function readTableLiveV3HistoryState(
  root = process.cwd(),
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V3_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV3AuthorizationArtifact)
    : undefined;
  ...
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V3_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V3_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV3History(
  expected: TableLiveV3HistoryExpectation,
  root = process.cwd(),
): TableLiveV3HistoryState {
  const state = readTableLiveV3HistoryState(root);
  const failures = validateTableLiveV3History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v3 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV3Authorization(
  root = process.cwd(),
): TableLiveV3AuthorizationProof {
  const state = verifyTableLiveV3History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V3_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v3/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v3.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V3_TARGET,
    expectedDynamicTool: TABLE_LIVE_V3_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V3_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV3HistoryExpectation => {
  ...
  if (pending === authorized)
    throw new Error(
      "Choose exactly one table v3 history mode: --expect-pending or --expect-authorized",
    );
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV3History(expected);
  process.stdout.write(
    `Table live v3 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
