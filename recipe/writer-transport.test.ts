...
test("transport corruption fails closed before eval", () => {
  ...
  const cases: Array<[string, (copy: WriterTransportEnvelope) => void, RegExp]> =
    [
      [
        "truncated payload",
        (copy) => {
...
test("historical manually reconstructed chunk mismatch is hash-visible", () => {
  const bytes = readFileSync(
    "recipe/evidence/button-live-pivot-v2/writer.js",
  );
  const envelope = createWriterTransportEnvelope(bytes);
...
test("wrapper executes only after byte length and SHA-256 verification", async () => {
  ...
  const execute = new AsyncFunction(
    ...
    "crypto",
    wrapper,
  ) as (...args: unknown[]) => Promise<Record<string, any>>;
  ...
  assert.equal(globalThis.__transportEvalCount, 1);
...
test("staged wrapper verifies the reassembled generated payload before eval", async () => {
  ...
  const AsyncFunction = Object.getPrototypeOf(async function () {}).constructor;
  globalThis.__recipeTransportV3Payload = envelope.payload;
  globalThis.__transportEvalCount = 0;
  const execute = new AsyncFunction(
    "figma",
    "atob",
  ...
  assert.equal(
    ...
    "verified-js-fallback",
  );
  assert.equal(globalThis.__transportEvalCount, 1);
  delete globalThis.__recipeTransportV3Payload;
});
