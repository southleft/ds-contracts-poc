...
import {
  INPUT_LIVE_V17_DYNAMIC_TOOL,
  INPUT_LIVE_V17_TARGET,
  inputLiveV17RequestSequence,
...
} from "./input-field-live-v17-restore.js";

export const INPUT_LIVE_V17_EVIDENCE_ROOT =
  "recipe/evidence/input-field-live-pivot-v17";
export const INPUT_LIVE_V17_PROTOCOL_PATH = `${INPUT_LIVE_V17_EVIDENCE_ROOT}/protocol.json`;
export const INPUT_LIVE_V17_PLAN_PATH = `${INPUT_LIVE_V17_EVIDENCE_ROOT}/proof-plan.json`;
export const INPUT_LIVE_V17_CAPTURE_MANIFEST_PATH = `${INPUT_LIVE_V17_EVIDENCE_ROOT}/capture-manifest.json`;
export const INPUT_LIVE_V17_REQUEST_MANIFEST_PATH = `${INPUT_LIVE_V17_EVIDENCE_ROOT}/request-manifest.json`;
export const INPUT_LIVE_V17_INDEX_PATH = `${INPUT_LIVE_V17_EVIDENCE_ROOT}/antecedent-index.json`;
export const INPUT_LIVE_V17_AUTHORIZATION_TEMPLATE_PATH = `${INPUT_LIVE_V17_EVIDENCE_ROOT}/authorization-template.json`;
export const INPUT_LIVE_V17_AUTHORIZATION_PATH = `${INPUT_LIVE_V17_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface InputLiveV17ProofPlan {
  artifactVersion: "input-live-v17-proof-plan-v1";
...
interface GeneratedSource {
  ...
  recipeHash: string;
  envelopeHash: string;
  expectedScenePlan: InputLiveV17SourceIdentity["expectedScenePlan"];
  outputPath: string;
  compressed: Buffer;
  metadata: InputLiveV17ProofPlan["sources"][number]["expectedScenePlan"];
}
...
const sourcePlans = (): GeneratedSource[] => {
  ...
  return prior.sources.map((source: Record<string, any>) => {
    ...
    return {
      ...
      outputPath: `${INPUT_LIVE_V17_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
      compressed,
      metadata: {
        path: `${INPUT_LIVE_V17_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
...
const captureCells = (): InputLiveV17CaptureCell[] => {
  const cells = json<Record<string, any>>(V16_CAPTURE_MANIFEST)
    .cells as InputLiveV17CaptureCell[];
  inputLiveV17CaptureManifestSha256(cells);
  return cells;
};

const protocol = () => ({
  artifactVersion: "input-live-v17-external-operator-protocol-v1",
  protocolId: "input-live-v17",
  status:
  ...
  hostNormalization: {
    ...
    carriedSceneReadback: "recipe/scene-readback-v17.ts",
    carriedSceneReadbackRuntime: "recipe/scene-readback-runtime-v17.ts",
    carriedV3Verifier: "recipe/input-field-live-v3-verifier-v17.ts",
...
const requestManifest = (
  captures: readonly InputLiveV17CaptureCell[],
  programs: {
    writerSha256: string;
  ...
  const requests = [
    {
      requestId: "writer",
      sequence: inputLiveV17RequestSequence("writer"),
      phase: "writer",
      predecessorRequestId: null,
      programSha256: programs.writerSha256,
      response: INPUT_LIVE_V17_RESPONSE_CONTRACTS.writer,
    },
    {
      requestId: "cleanup",
      sequence: inputLiveV17RequestSequence("cleanup"),
      phase: "cleanup",
      predecessorRequestId: "writer",
      programBuilderSha256: programs.cleanupBuilderSha256,
      response: INPUT_LIVE_V17_RESPONSE_CONTRACTS.cleanup,
      availability: "persisted immediately after writer acceptance",
    },
    {
      requestId: "restore",
      sequence: inputLiveV17RequestSequence("restore"),
      phase: "restore",
      predecessorRequestId: "writer",
      programSha256: programs.restoreBlueprintSha256,
      response: INPUT_LIVE_V17_RESPONSE_CONTRACTS.restore,
      availability:
        "issued immediately after writer acceptance; before extract",
    },
    {
      requestId: "extract",
      sequence: inputLiveV17RequestSequence("extract"),
      phase: "extract",
      predecessorRequestId: "restore",
      programSha256: programs.extractBlueprintSha256,
      response: INPUT_LIVE_V17_RESPONSE_CONTRACTS.extract,
    },
    {
      requestId: "probe",
      sequence: inputLiveV17RequestSequence("probe"),
      phase: "probe",
      predecessorRequestId: "extract",
      programSha256: programs.probeBlueprintSha256,
      response: INPUT_LIVE_V17_RESPONSE_CONTRACTS.probe,
    },
    ...captures.map((cell) => ({
      requestId: `capture-${String(cell.index).padStart(3, "0")}`,
      sequence: inputLiveV17RequestSequence("capture", cell.index),
      phase: "capture",
      ...
      response: INPUT_LIVE_V17_RESPONSE_CONTRACTS.capture,
    })),
  ];
  return {
    artifactVersion: "input-live-v17-request-manifest-v1",
    expectedDynamicTool: INPUT_LIVE_V17_DYNAMIC_TOOL,
    target: INPUT_LIVE_V17_TARGET,
...
const lifecycleExcludedPaths = [
  INPUT_LIVE_V17_AUTHORIZATION_TEMPLATE_PATH,
  INPUT_LIVE_V17_AUTHORIZATION_PATH,
  "recipe/input-field-live-v17-authorization.ts",
  "recipe/input-field-live-v17-authorization.test.ts",
  "recipe/input-field-live-v17-preflight.ts",
  "recipe/input-field-live-v17-authorized.ts",
  "recipe/evidence/input-field-live-pivot-v17/operator-security-attestation-template.json",
...
const sourceNeutralDependencies = [
  ...
  "recipe/normalize.ts",
  "recipe/scene-readback-runtime-v17.ts",
  "recipe/scene-readback-v17.ts",
  "recipe/input-field-live-v17-restore.ts",
] as const;

export async function buildInputLiveV17Proof(
  check = process.argv.includes("--check"),
): Promise<InputLiveV17ProofPlan> {
  ...
  if (
    ...
    throw new Error(
      "Input live v17 exact committed v16 writer, restore, runtime, or extract bytes drifted",
    );

  const writerOwnershipBlueprint: InputLiveV17WriterOwnership = {
    pageId: "__WRITER_PAGE_ID__",
    pageName: "Recipe Pivot / Input Field / v9",
    runIdentity: "input-live-v17",
  ...
  const identities = sources.map(
    (source) =>
      ...
      }) satisfies InputLiveV17SourceIdentity,
  );
  const restoreBlueprint = Buffer.from(
    buildInputLiveV17RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildInputLiveV17ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildInputLiveV17ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildInputLiveV17CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );

  const antecedentOutputs = new Map<string, Buffer>();
  antecedentOutputs.set(INPUT_LIVE_V17_PROTOCOL_PATH, jsonBytes(protocol()));
  antecedentOutputs.set(
    `${INPUT_LIVE_V17_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V17_EVIDENCE_ROOT}/programs/writer-payload.js`,
    writerPayload,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V17_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V17_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V17_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V17_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  ...
  const captureManifest = {
    artifactVersion: "input-live-v17-capture-manifest-v1",
    ...
    cells: captures,
    cellsSha256: inputLiveV17CaptureManifestSha256(captures),
  };
  ...
  const proofPlan: InputLiveV17ProofPlan = {
    artifactVersion: "input-live-v17-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: INPUT_LIVE_V17_TARGET,
    namespace: "ds.contracts.input.recipe.v5",
    writer: {
      programPath: `${INPUT_LIVE_V17_EVIDENCE_ROOT}/programs/writer.txt`,
      programBytes: writerProgram.byteLength,
      programSha256: sha256(writerProgram),
      payloadPath: `${INPUT_LIVE_V17_EVIDENCE_ROOT}/programs/writer-payload.js`,
    ...
    captureManifest: {
      path: INPUT_LIVE_V17_CAPTURE_MANIFEST_PATH,
      cells: 128,
      requests: 128,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(INPUT_LIVE_V17_PLAN_PATH, proofPlanBytes);
  ...
  antecedentOutputs.set(
    INPUT_LIVE_V17_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/input-field-live-v17-broker.ts",
    "recipe/input-field-live-v17-contract.ts",
    "recipe/run-input-field-live-v17.ts",
    "recipe/build-input-field-live-proof-v17.ts",
    "recipe/input-field-live-v17-broker.test.ts",
    "recipe/figma-property-normalizer-v8.test.ts",
    "recipe/scene-readback-v17.test.ts",
    "recipe/input-field-live-v17-restore.ts",
    "recipe/input-field-live-v17-restore.test.ts",
    "recipe/input-field-live-v17-lifecycle-simulation.ts",
    ...sourceNeutralDependencies,
  ].filter((artifactPath) => artifactPath !== INPUT_LIVE_V17_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "input-live-v17-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    ...
    counts: {
      ...
      captureCells: INPUT_LIVE_V17_CAPTURE_COUNT,
  ...
  antecedentOutputs.set(INPUT_LIVE_V17_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "input-live-v17-authorization-template-v1",
    authorizationId: "input-live-v17",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: INPUT_LIVE_V17_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
  ...
  if (check) {
    ...
    mkdirSync(path.dirname(INPUT_LIVE_V17_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(INPUT_LIVE_V17_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(INPUT_LIVE_V17_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(INPUT_LIVE_V17_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildInputLiveV17Proof(), null, 2)}\n`,
  );
