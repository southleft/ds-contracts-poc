...
import { INPUT_LIVE_V28_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v28.js";
import { verifyInputLiveV28Authorization } from "./input-field-live-v28-authorization.js";
import { runInputLiveV28Preflight } from "./input-field-live-v28-preflight.js";
import { InputLiveV28Orchestrator } from "./run-input-field-live-v28.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V28_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v28-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV28Authorization(root);
    runInputLiveV28Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV28Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
