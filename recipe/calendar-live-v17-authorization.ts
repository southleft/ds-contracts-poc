...
export type CalendarLiveV17HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV17AuthorizationArtifact {
  artifactVersion: "calendar-live-v17-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V17_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV17AntecedentIndex {
...
export interface CalendarLiveV17HistoryState {
  index?: CalendarLiveV17AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV17AuthorizationArtifact;
...
export interface CalendarLiveV17AuthorizationProof
  extends CalendarLiveV17TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V17_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V17_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V17_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V17_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v17-authorization") ||
  artifactPath.includes("calendar-live-v17-preflight") ||
  artifactPath.includes("calendar-live-v17-authorized") ||
  artifactPath.includes("create-calendar-live-v17-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v17-status") ||
...
export function validateCalendarLiveV17AntecedentIndex(
  index: CalendarLiveV17AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v17-antecedent-index-v1" ||
...
export function validateCalendarLiveV17UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV17History\(\s*["']/.test(source) ||
    /readCalendarLiveV17HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV17AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV17AuthorizationArtifact {
  ...
  ) as CalendarLiveV17AntecedentIndex;
  const indexFailures = validateCalendarLiveV17AntecedentIndex(index);
...
export function validateCalendarLiveV17AuthorizationArtifact(
  artifact: CalendarLiveV17AuthorizationArtifact | undefined,
  index: CalendarLiveV17AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v17-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v17" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V17_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V17_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V17_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V17_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V17_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V17_SOURCE_ROOTS ||
...
export function validateCalendarLiveV17History(
  state: CalendarLiveV17HistoryState,
  expected: CalendarLiveV17HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV17AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV17AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV17HistoryState(
  root = process.cwd(),
): CalendarLiveV17HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V17_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV17AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V17_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V17_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V17_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV17History(
  expected: CalendarLiveV17HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV17HistoryState {
  const state = readCalendarLiveV17HistoryState(root);
  const failures = validateCalendarLiveV17History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v17 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV17Authorization(
  root = process.cwd(),
): CalendarLiveV17AuthorizationProof {
  const state = verifyCalendarLiveV17History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V17_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v17/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v17.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V17_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V17_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V17_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV17HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV17History(expected);
  process.stdout.write(
    `Calendar live v17 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
