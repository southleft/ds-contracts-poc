...
import { TABLE_LIVE_V1_AUTHORIZATION_PATH } from "./build-table-live-proof-v1.js";
import { verifyTableLiveV1Authorization } from "./table-live-v1-authorization.js";
import { runTableLiveV1Preflight } from "./table-live-v1-preflight.js";
import { TableLiveV1Orchestrator } from "./run-table-live-v1.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v1 requires exact authorization artifact ${TABLE_LIVE_V1_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v1-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV1Authorization(root);
    runTableLiveV1Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV1Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV1Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
