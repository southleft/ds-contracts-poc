...
import {
  ...
  createInputLiveV10FakeDynamicResponse,
  ...
  listInputLiveV10PersistedRequests,
  persistInputLiveV10RawResponse,
  persistInputLiveV10TechnicalGates,
  validateInputLiveV10Receipt,
  validateInputLiveV10Request,
  type InputLiveV10RemotePhase,
...
import {
  ...
  type InputLiveV10ExtractPayload,
  type InputLiveV10ProbePayload,
  type InputLiveV10RootProof,
  type InputLiveV10WriterOwnership,
} from "./input-field-live-v10-contract.js";
import {
  InputLiveV10Orchestrator,
  simulatedInputLiveV10Authorization,
} from "./run-input-field-live-v10.js";
...
const captures: InputLiveV10CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV10Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v10-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV10Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV10CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV10FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV10FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V10_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V10_TARGET);
  persistInputLiveV10RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV10Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV10Receipt(directory, phase, options.captureIndex);
...
const extractPayload = (
  ownership: InputLiveV10WriterOwnership,
...
const rootProofs = (): InputLiveV10RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v10-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV10Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV10Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV10RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) => validateInputLiveV10ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      probePayload(writer),
      {
        validate: (payload) => validateInputLiveV10ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV10CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV10CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV10CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV10Response(directory, "cleanup", {
      validate: (payload) => validateInputLiveV10CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV10PersistedRequests(directory).length, 132);
    assert.ok(inputLiveV10ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV10Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV10Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV10Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV10Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V10_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV10RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V10_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV10Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV10Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV10ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV10Receipt(third.directory, "writer"),
      /invalid Input live v10 accepted receipt/,
    );
  } finally {
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV10WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV10RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV10Request;
    validateInputLiveV10Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV10Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV10CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active extract and surfaced cleanup", () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v10-state-"));
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV10Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV10Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV10Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V10_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV10RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V10_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV10Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV10Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV10RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V10_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV10Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV10Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV10Request;
