...
import { COMBOBOX_LIVE_V16_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v16.js";
import { verifyComboboxLiveV16Authorization } from "./combobox-live-v16-authorization.js";
import { runComboboxLiveV16Preflight } from "./combobox-live-v16-preflight.js";
import { ComboboxLiveV16Orchestrator } from "./run-combobox-live-v16.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v16 requires exact authorization artifact ${COMBOBOX_LIVE_V16_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v16-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV16Authorization(root);
    runComboboxLiveV16Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV16Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV16Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
