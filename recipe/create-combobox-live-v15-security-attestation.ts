...
import { COMBOBOX_LIVE_V15_TARGET } from "./combobox-live-v15-broker.js";
import {
  COMBOBOX_LIVE_V15_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV15Authorization();
...
const attestation: ComboboxLiveV15SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV15SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v15 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V15_SECURITY_ATTESTATION_DEFAULT_PATH,
);
