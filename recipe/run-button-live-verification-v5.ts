...
const PLAN = JSON.parse(readFileSync("recipe/evidence/button-live-pivot-v5/writer-plan.json", "utf8")) as Record<string, any>;
const PAGE_NAME: string = PLAN.pageName;
...
if(!page)throw new Error("V5-PROOF-PAGE-ABSENT");
...
for(const stale of [...page.children])if(stale.type==="SECTION"&&(stale.name==="Recipe Pivot / Live proof / v5"||stale.name==="Recipe Pivot / Paired cells / v5"))stale.remove();
const proof=figma.createSection();proof.name="Recipe Pivot / Live proof / v5";proof.x=0;proof.y=Math.max(...page.children.map(node=>node.y+node.height),0)+240;page.appendChild(proof);
const paired=figma.createSection();paired.name="Recipe Pivot / Paired cells / v5";paired.x=0;paired.y=proof.y+560;page.appendChild(paired);
...
// The scene-derived accounting and fixed point for v5 live in the
// button-scene-inversion-v2 lineage (B-series observe program, which does
// NOT descend into instance internals); this verification carries the LIVE
// usability probes and screenshots only. readSceneDerivedTree's
// instance-descent path requires an expectedPlan.generatedDescendants map
// this writer never plans, so it is not invoked here.
...
try {
  ...
  // Attempts 2-3 of the v5 mint measured that LARGE single-message evals
  // through this channel crawl while small ones stay fast; stage the program
  // in small exact chunks and trigger it with a tiny eval (same staged-chunk
  // discipline as run-button-live-proof-v5.ts).
  const reset = await connector.executeCodeViaUI(
    `globalThis.__v5VerifyProgram="";return {ok:true};`,
    15_000,
    FILE_KEY,
  );
  if (!reset?.success)
    throw new Error(reset?.error ?? "verification stage reset failed");
  ...
  for (let offset = 0; offset < code.length; offset += CHUNK_SIZE) {
    ...
    const staged = await connector.executeCodeViaUI(
      `globalThis.__v5VerifyProgram+=${JSON.stringify(chunk)};return {staged:globalThis.__v5VerifyProgram.length};`,
      15_000,
      FILE_KEY,
    );
    if (!staged?.success)
      throw new Error(staged?.error ?? `verification stage @${offset} failed`);
  }
  const response = await connector.executeCodeViaUI(
    `if(globalThis.__v5VerifyProgram.length!==${code.length})throw new Error("VERIFY-PROGRAM-LENGTH");return await eval("(async()=>{"+globalThis.__v5VerifyProgram+"\\n})()");`,
    300_000,
    FILE_KEY,
  );
  if (!response?.success)
...
const verification = {
  ...result,
  sceneAccounting: {
    carriedBy: "recipe/evidence/button-scene-inversion-v2 (B-series observe program; accounting and collapse-compile fixed point live there)",
    inlineReadback: false,
  },
};
writeFileSync(
...
if (failures.length > 0) {
  throw new Error(`v5 live verification failed: ${failures.join("; ")}`);
}
console.log(
  JSON.stringify({
