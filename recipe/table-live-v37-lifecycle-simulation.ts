...
import { tableLiveV2Sha256 } from "./table-live-v37-broker.js";

export interface TableLiveV37LifecycleSimulationReport {
  artifactVersion: "table-live-v37-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV37AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV37Lifecycle(): Promise<TableLiveV37LifecycleSimulationReport> {
  await buildTableLiveV37Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V37_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV37AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v37-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V37_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V37_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV37History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV37History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV37AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V37_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V37_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV37History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV37History(authorized, "pending");
    ...
    await buildTableLiveV37Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V37_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV37AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV37Lifecycle(), null, 2)}\n`,
  );
