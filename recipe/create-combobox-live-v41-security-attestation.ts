...
import { COMBOBOX_LIVE_V41_TARGET } from "./combobox-live-v41-broker.js";
import {
  COMBOBOX_LIVE_V41_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV41Authorization();
...
const attestation: ComboboxLiveV41SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV41SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v41 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V41_SECURITY_ATTESTATION_DEFAULT_PATH,
);
