...
import {
  ...
  INPUT_LIVE_V17_INDEX_PATH,
} from "./build-input-field-live-proof-v17.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV17HistoryExpectation = "pending" | "authorized";

export interface InputLiveV17AuthorizationArtifact {
  artifactVersion: "input-live-v17-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V17_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV17AntecedentIndex {
...
export interface InputLiveV17HistoryState {
  index?: InputLiveV17AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV17AuthorizationArtifact;
...
export interface InputLiveV17AuthorizationProof extends InputLiveV17TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V17_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V17_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V17_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V17_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v17-status") ||
...
export function validateInputLiveV17AntecedentIndex(
  index: InputLiveV17AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v17-antecedent-index-v1" ||
...
export function validateInputLiveV17UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV17History\(\s*["']/.test(source) ||
    /readInputLiveV17HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV17AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV17AuthorizationArtifact {
  ...
  ) as InputLiveV17AntecedentIndex;
  const indexFailures = validateInputLiveV17AntecedentIndex(index);
...
export function validateInputLiveV17AuthorizationArtifact(
  artifact: InputLiveV17AuthorizationArtifact | undefined,
  index: InputLiveV17AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v17-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v17" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V17_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V17_TARGET) ||
    !equalJson(
...
export function validateInputLiveV17History(
  state: InputLiveV17HistoryState,
  expected: InputLiveV17HistoryExpectation,
): string[] {
  const failures = validateInputLiveV17AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV17AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV17HistoryState(
  root = process.cwd(),
): InputLiveV17HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V17_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV17AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V17_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V17_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V17_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV17History(
  expected: InputLiveV17HistoryExpectation,
  root = process.cwd(),
): InputLiveV17HistoryState {
  const state = readInputLiveV17HistoryState(root);
  const failures = validateInputLiveV17History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV17Authorization(
  root = process.cwd(),
): InputLiveV17AuthorizationProof {
  const state = verifyInputLiveV17History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V17_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v17/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v17.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V17_TARGET,
    expectedDynamicTool: INPUT_LIVE_V17_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V17_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV17HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV17History(expected);
