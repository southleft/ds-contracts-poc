...
import { verifyCalendarLiveV4Authorization } from "./calendar-live-v4-authorization.js";
import { CALENDAR_LIVE_V4_TARGET } from "./calendar-live-v4-broker.js";
import {
  CALENDAR_LIVE_V4_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV4Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v4-session";
...
const attestation: CalendarLiveV4SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV4SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v4 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V4_SECURITY_ATTESTATION_DEFAULT_PATH,
);
