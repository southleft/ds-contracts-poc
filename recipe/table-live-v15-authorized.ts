...
import { TABLE_LIVE_V15_AUTHORIZATION_PATH } from "./build-table-live-proof-v15.js";
import { verifyTableLiveV15Authorization } from "./table-live-v15-authorization.js";
import { runTableLiveV15Preflight } from "./table-live-v15-preflight.js";
import { TableLiveV15Orchestrator } from "./run-table-live-v15.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v15 requires exact authorization artifact ${TABLE_LIVE_V15_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v15-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV15Authorization(root);
    runTableLiveV15Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV15Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV15Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
