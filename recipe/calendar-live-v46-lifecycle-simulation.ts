...
import { calendarLiveV1Sha256 } from "./calendar-live-v46-broker.js";

export interface CalendarLiveV46LifecycleSimulationReport {
  artifactVersion: "calendar-live-v46-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV46AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV46Lifecycle(): Promise<CalendarLiveV46LifecycleSimulationReport> {
  await buildCalendarLiveV46Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V46_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV46AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v46-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V46_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V46_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV46History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV46History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV46AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V46_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V46_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV46History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV46History(authorized, "pending");
    ...
    await buildCalendarLiveV46Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V46_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV46AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV46Lifecycle(), null, 2)}\n`,
  );
