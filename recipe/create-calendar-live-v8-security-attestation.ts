...
import { CALENDAR_LIVE_V8_TARGET } from "./calendar-live-v8-broker.js";
import {
  CALENDAR_LIVE_V8_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV8Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v8-session";
...
const attestation: CalendarLiveV8SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV8SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v8 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V8_SECURITY_ATTESTATION_DEFAULT_PATH,
);
