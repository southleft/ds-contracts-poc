...
import {
  ...
  INPUT_LIVE_V52_INDEX_PATH,
} from "./build-input-field-live-proof-v52.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV52HistoryExpectation = "pending" | "authorized";

export interface InputLiveV52AuthorizationArtifact {
  artifactVersion: "input-live-v52-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V52_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV52AntecedentIndex {
...
export interface InputLiveV52HistoryState {
  index?: InputLiveV52AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV52AuthorizationArtifact;
...
export interface InputLiveV52AuthorizationProof extends InputLiveV52TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V52_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V52_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V52_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V52_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v52-authorization") ||
  artifactPath.includes("input-field-live-v52-preflight") ||
  artifactPath.includes("input-field-live-v52-authorized") ||
  artifactPath.includes("input-field-live-pivot-v52-status") ||
...
export function validateInputLiveV52AntecedentIndex(
  index: InputLiveV52AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v52-antecedent-index-v1" ||
...
export function validateInputLiveV52UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV52History\(\s*["']/.test(source) ||
    /readInputLiveV52HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV52AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV52AuthorizationArtifact {
  ...
  ) as InputLiveV52AntecedentIndex;
  const indexFailures = validateInputLiveV52AntecedentIndex(index);
...
export function validateInputLiveV52AuthorizationArtifact(
  artifact: InputLiveV52AuthorizationArtifact | undefined,
  index: InputLiveV52AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v52-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v52" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V52_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V52_TARGET) ||
    !equalJson(
...
export function validateInputLiveV52History(
  state: InputLiveV52HistoryState,
  expected: InputLiveV52HistoryExpectation,
): string[] {
  const failures = validateInputLiveV52AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV52AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV52HistoryState(
  root = process.cwd(),
): InputLiveV52HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V52_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV52AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V52_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V52_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V52_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV52History(
  expected: InputLiveV52HistoryExpectation,
  root = process.cwd(),
): InputLiveV52HistoryState {
  const state = readInputLiveV52HistoryState(root);
  const failures = validateInputLiveV52History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV52Authorization(
  root = process.cwd(),
): InputLiveV52AuthorizationProof {
  const state = verifyInputLiveV52History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V52_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v52/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v52.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V52_TARGET,
    expectedDynamicTool: INPUT_LIVE_V52_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V52_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV52HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV52History(expected);
