...
import { INPUT_LIVE_V46_INDEX_PATH } from "./build-input-field-live-proof-v46.js";
import {
  INPUT_LIVE_V46_DYNAMIC_TOOL,
  INPUT_LIVE_V46_TARGET,
} from "./input-field-live-v46-broker.js";
import {
  buildInputLiveV46AuthorizationArtifact,
  validateInputLiveV46AntecedentIndex,
  validateInputLiveV46AuthorizationArtifact,
  validateInputLiveV46History,
  validateInputLiveV46UnitTestSource,
  type InputLiveV46AntecedentIndex,
  type InputLiveV46AuthorizationArtifact,
  type InputLiveV46AuthorizationProof,
  type InputLiveV46HistoryState,
} from "./input-field-live-v46-authorization.js";
import {
  inputLiveV46AttestationSha256,
  validateInputLiveV46ControlFlowSource,
  validateInputLiveV46SecurityAttestation,
  type InputLiveV46SecurityAttestation,
} from "./input-field-live-v46-preflight.js";
import { inputLiveV46Sha256 } from "./input-field-live-v46-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V46_INDEX_PATH);
const index = (): InputLiveV46AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV46AntecedentIndex;

const artifact = (): InputLiveV46AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV46AuthorizationArtifact({
...
const pendingState = (): InputLiveV46HistoryState => ({
  index: index(),
  indexSha256: inputLiveV46Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV46HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV46AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV46Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V46_TARGET,
  expectedDynamicTool: INPUT_LIVE_V46_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v46/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV46AuthorizationArtifact,
): InputLiveV46SecurityAttestation => {
  const body: Omit<InputLiveV46SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v46-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V46_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV46AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV46History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV46History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV46History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV46History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV46History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV46UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV46UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV46AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV46Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV46AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV46AuthorizationArtifact(
        planted as InputLiveV46AuthorizationArtifact,
        index(),
        inputLiveV46Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV46SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV46AttestationSha256(
      body as Omit<InputLiveV46SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV46SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV46ControlFlowSource(source), []);
});
