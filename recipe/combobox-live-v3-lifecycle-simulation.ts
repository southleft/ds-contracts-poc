...
export interface ComboboxLiveV3LifecycleSimulationReport {
  artifactVersion: "combobox-live-v3-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV3AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV3Lifecycle(): Promise<ComboboxLiveV3LifecycleSimulationReport> {
  await buildComboboxLiveV3Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V3_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV3AntecedentIndex;
  const indexSha256 = comboboxLiveV3Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v3-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V3_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V3_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV3History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV3History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV3AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V3_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V3_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV3History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV3History(authorized, "pending");
    ...
    await buildComboboxLiveV3Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V3_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV3AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: comboboxLiveV3Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV3Lifecycle(), null, 2)}\n`,
  );
