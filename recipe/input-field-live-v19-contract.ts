...
} from "./input-field-live-v3-verifier-v19.js";
import {
  buildInputLiveV19RawPropertyRuntime,
  normalizeInputLiveV19Scene,
  type InputLiveV19RawNode,
} from "./input-field-live-v19-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v19.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v19.js";
...
export const INPUT_LIVE_V19_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V19_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V19_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V19_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V19_VARIANT_COUNT = 256;
export const INPUT_LIVE_V19_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V19_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV19SourceId = (typeof INPUT_LIVE_V19_SOURCE_IDS)[number];

export interface InputLiveV19SourceIdentity {
  source: InputLiveV19SourceId;
...
export interface InputLiveV19WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV19ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV19SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV19RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV19RootProof {
  source: InputLiveV19SourceId;
  adapterIdentity: string;
...
export interface InputLiveV19ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV19SourceId;
    adapterIdentity: string;
...
export interface InputLiveV19CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV19SourceId;
  adapterIdentity: string;
...
export interface InputLiveV19CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV19SourceId;
  strata: InputLiveV19CaptureCell["strata"];
...
export interface InputLiveV19CleanupPayload {
...
export function validateInputLiveV19WriterPayload(
  value: unknown,
): InputLiveV19WriterOwnership {
...
const identityPlan = (source: InputLiveV19SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV19ExtractProgram(
  writer: InputLiveV19WriterOwnership,
  sources: readonly InputLiveV19SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V19_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V19_NAMESPACE)}
${buildInputLiveV19RawPropertyRuntime()}
...
export function validateInputLiveV19ExtractPayload(
  value: unknown,
  writer: InputLiveV19WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V19_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV19ExtractPayload;
}

export function proveInputLiveV19Roots<Instance>(
  extract: InputLiveV19ExtractPayload,
  sources: readonly (InputLiveV19SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV19Scene(
      root.scene,
...
export function assertInputLiveV19RootProofs(
  proofs: readonly InputLiveV19RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV19ProbeProgram(
  writer: InputLiveV19WriterOwnership,
  sources: readonly InputLiveV19SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V19_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV19ProbePayload(
  value: unknown,
  writer: InputLiveV19WriterOwnership,
): InputLiveV19ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV19ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V19_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V19_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV19CaptureManifest(
  cells: readonly InputLiveV19CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V19_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V19_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V19_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V19_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V19_SOURCE_IDS.some(
...
export const inputLiveV19CaptureManifestSha256 = (
  cells: readonly InputLiveV19CaptureCell[],
): string => {
  validateInputLiveV19CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV19CaptureProgram(
  writer: InputLiveV19WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V19_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V19_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V19_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV19CapturePayload(
  value: unknown,
  cell: InputLiveV19CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV19CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V19_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V19_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV19CaptureResponses(
  manifest: readonly InputLiveV19CaptureCell[],
  responses: readonly InputLiveV19CapturePayload[],
): void {
  validateInputLiveV19CaptureManifest(manifest);
  if (
...
export interface InputLiveV19ObjectiveReport {
  artifactVersion: "input-live-v19-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV19Objective(
  manifest: readonly InputLiveV19CaptureCell[],
  responses: readonly InputLiveV19CapturePayload[],
  readReference: (cell: InputLiveV19CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV19CaptureCell) => Buffer,
): InputLiveV19ObjectiveReport {
  assertInputLiveV19CaptureResponses(manifest, responses);
...
export function buildInputLiveV19CleanupProgram(
  writer: InputLiveV19WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V19_NAMESPACE,
...
export function validateInputLiveV19CleanupPayload(
  value: unknown,
  writer: InputLiveV19WriterOwnership,
): InputLiveV19CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV19CleanupPayload;
...
export const INPUT_LIVE_V19_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV19WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
