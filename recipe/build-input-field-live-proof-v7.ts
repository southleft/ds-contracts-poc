...
import {
  ...
  inputLiveV7RequestSequence,
} from "./input-field-live-v7-broker.js";

export const INPUT_LIVE_V7_EVIDENCE_ROOT =
  "recipe/evidence/input-field-live-pivot-v7";
export const INPUT_LIVE_V7_PROTOCOL_PATH = `${INPUT_LIVE_V7_EVIDENCE_ROOT}/protocol.json`;
export const INPUT_LIVE_V7_PLAN_PATH = `${INPUT_LIVE_V7_EVIDENCE_ROOT}/proof-plan.json`;
export const INPUT_LIVE_V7_CAPTURE_MANIFEST_PATH = `${INPUT_LIVE_V7_EVIDENCE_ROOT}/capture-manifest.json`;
export const INPUT_LIVE_V7_REQUEST_MANIFEST_PATH = `${INPUT_LIVE_V7_EVIDENCE_ROOT}/request-manifest.json`;
export const INPUT_LIVE_V7_INDEX_PATH = `${INPUT_LIVE_V7_EVIDENCE_ROOT}/antecedent-index.json`;
export const INPUT_LIVE_V7_AUTHORIZATION_TEMPLATE_PATH = `${INPUT_LIVE_V7_EVIDENCE_ROOT}/authorization-template.json`;
export const INPUT_LIVE_V7_AUTHORIZATION_PATH = `${INPUT_LIVE_V7_EVIDENCE_ROOT}/capture-authorization.json`;

const V6_ROOT = "recipe/evidence/input-field-live-pivot-v6";
const V6_PLAN = `${V6_ROOT}/proof-plan.json`;
const V6_CAPTURE_MANIFEST = `${V6_ROOT}/capture-manifest.json`;
...
export interface InputLiveV7ProofPlan {
  artifactVersion: "input-live-v7-proof-plan-v1";
...
interface GeneratedSource {
  ...
  recipeHash: string;
  envelopeHash: string;
  expectedScenePlan: InputLiveV7SourceIdentity["expectedScenePlan"];
  outputPath: string;
  compressed: Buffer;
  metadata: InputLiveV7ProofPlan["sources"][number]["expectedScenePlan"];
}

const sourcePlans = (): GeneratedSource[] => {
  const v6 = json<Record<string, any>>(V6_PLAN);
  return v6.sources.map((source: Record<string, any>) => {
    ...
    if (
      ...
      throw new Error(`Input live v7 source plan drift: ${inputPath}`);
    return {
      source: sourceId,
      ...
      outputPath: `${INPUT_LIVE_V7_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
      compressed,
      metadata: {
        path: `${INPUT_LIVE_V7_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
...
const captureCells = (): InputLiveV7CaptureCell[] => {
  const cells = json<Record<string, any>>(V6_CAPTURE_MANIFEST)
    .cells as InputLiveV7CaptureCell[];
  inputLiveV7CaptureManifestSha256(cells);
  return cells;
};

const protocol = () => ({
  artifactVersion: "input-live-v7-external-operator-protocol-v1",
  protocolId: "input-live-v7",
  status:
...
const requestManifest = (
  captures: readonly InputLiveV7CaptureCell[],
  programs: {
    writerSha256: string;
  ...
  const requests = [
    {
      requestId: "writer",
      sequence: inputLiveV7RequestSequence("writer"),
      phase: "writer",
      predecessorRequestId: null,
      programSha256: programs.writerSha256,
      response: INPUT_LIVE_V7_RESPONSE_CONTRACTS.writer,
    },
    {
      requestId: "cleanup",
      sequence: inputLiveV7RequestSequence("cleanup"),
      phase: "cleanup",
      predecessorRequestId: "writer",
      programBuilderSha256: programs.cleanupBuilderSha256,
      response: INPUT_LIVE_V7_RESPONSE_CONTRACTS.cleanup,
      availability: "persisted immediately after writer acceptance",
    },
    {
      requestId: "extract",
      sequence: inputLiveV7RequestSequence("extract"),
      phase: "extract",
      predecessorRequestId: "writer",
      programSha256: programs.extractBlueprintSha256,
      response: INPUT_LIVE_V7_RESPONSE_CONTRACTS.extract,
    },
    {
      requestId: "probe",
      sequence: inputLiveV7RequestSequence("probe"),
      phase: "probe",
      predecessorRequestId: "extract",
      programSha256: programs.probeBlueprintSha256,
      response: INPUT_LIVE_V7_RESPONSE_CONTRACTS.probe,
    },
    ...captures.map((cell) => ({
      requestId: `capture-${String(cell.index).padStart(3, "0")}`,
      sequence: inputLiveV7RequestSequence("capture", cell.index),
      phase: "capture",
      ...
      response: INPUT_LIVE_V7_RESPONSE_CONTRACTS.capture,
    })),
  ];
  return {
    artifactVersion: "input-live-v7-request-manifest-v1",
    expectedDynamicTool: INPUT_LIVE_V7_DYNAMIC_TOOL,
    target: INPUT_LIVE_V7_TARGET,
...
const lifecycleExcludedPaths = [
  INPUT_LIVE_V7_AUTHORIZATION_TEMPLATE_PATH,
  ...
  "recipe/evidence/input-field-live-pivot-v7-status.json",
...
export async function buildInputLiveV7Proof(
  check = process.argv.includes("--check"),
): Promise<InputLiveV7ProofPlan> {
  const captures = captureCells();
  const sources = sourcePlans();
  const v6Plan = json<Record<string, any>>(V6_PLAN);
  const writerProgram = readFileSync(v6Plan.writer.programPath);
  const writerPayload = readFileSync(v6Plan.writer.payloadPath);
  if (
    sha256(writerProgram) !== v6Plan.writer.programSha256 ||
    sha256(writerPayload) !== v6Plan.writer.payloadSha256
  )
    throw new Error("Input live v7 exact committed writer bytes drifted");

  const writerOwnershipBlueprint: InputLiveV7WriterOwnership = {
    pageId: "__WRITER_PAGE_ID__",
    pageName: "Recipe Pivot / Input Field / v7",
    runIdentity: "input-live-v7",
  ...
  const identities = sources.map(
    (source) =>
      ...
      }) satisfies InputLiveV7SourceIdentity,
  );
  const extractBlueprint = Buffer.from(
    buildInputLiveV7ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildInputLiveV7ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildInputLiveV7CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );

  const antecedentOutputs = new Map<string, Buffer>();
  antecedentOutputs.set(INPUT_LIVE_V7_PROTOCOL_PATH, jsonBytes(protocol()));
  antecedentOutputs.set(
    `${INPUT_LIVE_V7_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V7_EVIDENCE_ROOT}/programs/writer-payload.js`,
    writerPayload,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V7_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V7_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V7_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  ...
  const captureManifest = {
    artifactVersion: "input-live-v7-capture-manifest-v1",
    ...
    cells: captures,
    cellsSha256: inputLiveV7CaptureManifestSha256(captures),
  };
  ...
  const proofPlan: InputLiveV7ProofPlan = {
    artifactVersion: "input-live-v7-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: INPUT_LIVE_V7_TARGET,
    namespace: "ds.contracts.input.recipe.v5",
    writer: {
      programPath: `${INPUT_LIVE_V7_EVIDENCE_ROOT}/programs/writer.txt`,
      programBytes: writerProgram.byteLength,
      programSha256: sha256(writerProgram),
      payloadPath: `${INPUT_LIVE_V7_EVIDENCE_ROOT}/programs/writer-payload.js`,
    ...
    captureManifest: {
      path: INPUT_LIVE_V7_CAPTURE_MANIFEST_PATH,
      cells: 128,
      requests: 128,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(INPUT_LIVE_V7_PLAN_PATH, proofPlanBytes);
  ...
  antecedentOutputs.set(
    INPUT_LIVE_V7_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/input-field-live-v7-broker.ts",
    "recipe/input-field-live-v7-contract.ts",
    "recipe/run-input-field-live-v7.ts",
    "recipe/build-input-field-live-proof-v7.ts",
    "recipe/input-field-live-v7-broker.test.ts",
    "recipe/input-field-live-v7-lifecycle-simulation.ts",
    ...sourceNeutralDependencies,
  ].filter((artifactPath) => artifactPath !== INPUT_LIVE_V7_INDEX_PATH);
  ...
  const index = {
    ...
    counts: {
      ...
      captureCells: INPUT_LIVE_V7_CAPTURE_COUNT,
  ...
  antecedentOutputs.set(INPUT_LIVE_V7_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "input-live-v7-authorization-template-v1",
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Input live v7 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(INPUT_LIVE_V7_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(INPUT_LIVE_V7_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(INPUT_LIVE_V7_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(INPUT_LIVE_V7_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Input live v7 authorization template drift");
  }
  if (
    ...
    throw new Error("Input live v7 deterministic hash/count invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildInputLiveV7Proof(), null, 2)}\n`,
  );
