...
import { CALENDAR_LIVE_V34_INDEX_PATH } from "./build-calendar-live-proof-v34.js";
import {
...
import {
  buildCalendarLiveV34AuthorizationArtifact,
  validateCalendarLiveV34AntecedentIndex,
  validateCalendarLiveV34AuthorizationArtifact,
  validateCalendarLiveV34History,
  validateCalendarLiveV34UnitTestSource,
  type CalendarLiveV34AntecedentIndex,
  type CalendarLiveV34AuthorizationArtifact,
  type CalendarLiveV34AuthorizationProof,
  type CalendarLiveV34HistoryState,
} from "./calendar-live-v34-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV34ControlFlowSource,
  validateCalendarLiveV34SecurityAttestation,
  type CalendarLiveV34SecurityAttestation,
} from "./calendar-live-v34-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V34_INDEX_PATH);
const index = (): CalendarLiveV34AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV34AntecedentIndex;

const artifact = (): CalendarLiveV34AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV34AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV34HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV34HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV34AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v34/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV34AuthorizationArtifact,
): CalendarLiveV34SecurityAttestation => {
  const body: Omit<CalendarLiveV34SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V34_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV34History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV34History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV34History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV34History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV34History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV34UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v34-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV34UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV34AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV34AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV34AuthorizationArtifact(
        planted as CalendarLiveV34AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV34SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV34SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV34SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v34-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV34ControlFlowSource(source), []);
});
