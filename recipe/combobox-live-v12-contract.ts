...
import { buildComboboxLiveV12CleanupRuntime } from "./combobox-live-v12-cleanup.js";
import {
  type ComboboxLiveV12FixedFixedPoint,
} from "./combobox-live-v12-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV12RestoreProgram,
  validateComboboxLiveV12RestorePayload,
  type ComboboxLiveV12RestorePayload,
} from "./combobox-live-v12-restore.js";
import {
  buildComboboxLiveV12RawPropertyRuntime,
  normalizeComboboxLiveV12Scene,
  type ComboboxLiveV12RawNode,
} from "./combobox-live-v12-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v12.js";
import {
...
export const COMBOBOX_LIVE_V12_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V12_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V12_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V12_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V12_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V12_SET_COUNT = 4;
export const COMBOBOX_LIVE_V12_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V12_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V12_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V12_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V12_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV12SourceId = (typeof COMBOBOX_LIVE_V12_SOURCE_IDS)[number];

export interface ComboboxLiveV12SourceIdentity {
  source: ComboboxLiveV12SourceId;
...
export interface ComboboxLiveV12WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV12ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV12SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV12RawNode;
    optionScene: ComboboxLiveV12RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV12RootProof {
  source: ComboboxLiveV12SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV12FixedFixedPoint;
}

export interface ComboboxLiveV12ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV12SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV12CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV12SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV12CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV12SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV12CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v12 ${label} must be an object`);
...
export function validateComboboxLiveV12WriterPayload(
  value: unknown,
): ComboboxLiveV12WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V12_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v12 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v12 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v12 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV12SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV12ExtractProgram(
  writer: ComboboxLiveV12WriterOwnership,
  sources: readonly ComboboxLiveV12SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v12 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V12_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V12_NAMESPACE)}
${buildComboboxLiveV12RawPropertyRuntime()}
...
export function validateComboboxLiveV12ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV12WriterOwnership,
): ComboboxLiveV12ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v12 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V12_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v12 extract schema/two-root mismatch");
  return payload as ComboboxLiveV12ExtractPayload;
}

export function proveComboboxLiveV12Roots<Instance>(
  extract: ComboboxLiveV12ExtractPayload,
  sources: readonly (ComboboxLiveV12SourceIdentity & {
...
): ComboboxLiveV12RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v12 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v12 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV12Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV12Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV12FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v12 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v12 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v12 compile lost owned sets");
...
export function assertComboboxLiveV12RootProofs(
  proofs: readonly ComboboxLiveV12RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v12 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v12 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV12ProbeProgram(
  writer: ComboboxLiveV12WriterOwnership,
  sources: readonly ComboboxLiveV12SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V12_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV12ProbePayload(
  value: unknown,
  writer: ComboboxLiveV12WriterOwnership,
): ComboboxLiveV12ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV12ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V12_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V12_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v12 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV12CaptureManifest(
  cells: readonly ComboboxLiveV12CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V12_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V12_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V12_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V12_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v12 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV12CaptureCell[],
): string => {
  validateComboboxLiveV12CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV12CaptureProgram(
  writer: ComboboxLiveV12WriterOwnership,
  cell: ComboboxLiveV12CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V12_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v12 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v12 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V12_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV12CapturePayload(
  value: unknown,
  cell: ComboboxLiveV12CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV12CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV12CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V12_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V12_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v12 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV12CaptureResponses(
  manifest: readonly ComboboxLiveV12CaptureCell[],
  responses: readonly ComboboxLiveV12CapturePayload[],
): void {
  validateComboboxLiveV12CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v12 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV12ObjectiveReport {
  artifactVersion: "combobox-live-v12-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV12Objective(
  manifest: readonly ComboboxLiveV12CaptureCell[],
  responses: readonly ComboboxLiveV12CapturePayload[],
): ComboboxLiveV12ObjectiveReport {
  assertComboboxLiveV12CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v12-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV12CleanupProgram(
  writer: ComboboxLiveV12WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV12CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V12_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v12 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV12CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV12WriterOwnership,
): ComboboxLiveV12CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV12CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v12 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V12_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV12WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
