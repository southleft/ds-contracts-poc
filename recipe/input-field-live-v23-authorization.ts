...
import {
  ...
  INPUT_LIVE_V23_INDEX_PATH,
} from "./build-input-field-live-proof-v23.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV23HistoryExpectation = "pending" | "authorized";

export interface InputLiveV23AuthorizationArtifact {
  artifactVersion: "input-live-v23-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V23_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV23AntecedentIndex {
...
export interface InputLiveV23HistoryState {
  index?: InputLiveV23AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV23AuthorizationArtifact;
...
export interface InputLiveV23AuthorizationProof extends InputLiveV23TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V23_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V23_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V23_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V23_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v23-status") ||
...
export function validateInputLiveV23AntecedentIndex(
  index: InputLiveV23AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v23-antecedent-index-v1" ||
...
export function validateInputLiveV23UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV23History\(\s*["']/.test(source) ||
    /readInputLiveV23HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV23AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV23AuthorizationArtifact {
  ...
  ) as InputLiveV23AntecedentIndex;
  const indexFailures = validateInputLiveV23AntecedentIndex(index);
...
export function validateInputLiveV23AuthorizationArtifact(
  artifact: InputLiveV23AuthorizationArtifact | undefined,
  index: InputLiveV23AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v23-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v23" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V23_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V23_TARGET) ||
    !equalJson(
...
export function validateInputLiveV23History(
  state: InputLiveV23HistoryState,
  expected: InputLiveV23HistoryExpectation,
): string[] {
  const failures = validateInputLiveV23AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV23AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV23HistoryState(
  root = process.cwd(),
): InputLiveV23HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V23_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV23AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V23_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V23_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V23_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV23History(
  expected: InputLiveV23HistoryExpectation,
  root = process.cwd(),
): InputLiveV23HistoryState {
  const state = readInputLiveV23HistoryState(root);
  const failures = validateInputLiveV23History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV23Authorization(
  root = process.cwd(),
): InputLiveV23AuthorizationProof {
  const state = verifyInputLiveV23History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V23_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v23/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v23.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V23_TARGET,
    expectedDynamicTool: INPUT_LIVE_V23_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V23_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV23HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV23History(expected);
