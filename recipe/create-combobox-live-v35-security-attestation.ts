...
import { verifyComboboxLiveV35Authorization } from "./combobox-live-v35-authorization.js";
import { COMBOBOX_LIVE_V35_TARGET } from "./combobox-live-v35-broker.js";
import {
  COMBOBOX_LIVE_V35_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV35Authorization();
...
const attestation: ComboboxLiveV35SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV35SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v35 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V35_SECURITY_ATTESTATION_DEFAULT_PATH,
);
