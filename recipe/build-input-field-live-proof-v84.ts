...
import {
  buildInputLiveV84CaptureProgram,
  buildInputLiveV84ExtractProgram,
  buildInputLiveV84ProbeProgram,
  buildInputLiveV84RestoreProgram,
  INPUT_LIVE_V84_CAPTURE_COUNT,
  INPUT_LIVE_V84_RESPONSE_CONTRACTS,
  inputLiveV84CaptureManifestSha256,
  type InputLiveV84CaptureCell,
  type InputLiveV84SourceIdentity,
  type InputLiveV84WriterOwnership,
} from "./input-field-live-v84-contract.js";
import {
  INPUT_LIVE_V84_DYNAMIC_TOOL,
  INPUT_LIVE_V84_TARGET,
  inputLiveV84RequestSequence,
...
} from "./input-field-live-v84-restore.js";

export const INPUT_LIVE_V84_EVIDENCE_ROOT =
  "recipe/evidence/input-field-live-pivot-v84";
export const INPUT_LIVE_V84_PROTOCOL_PATH = `${INPUT_LIVE_V84_EVIDENCE_ROOT}/protocol.json`;
export const INPUT_LIVE_V84_PLAN_PATH = `${INPUT_LIVE_V84_EVIDENCE_ROOT}/proof-plan.json`;
export const INPUT_LIVE_V84_CAPTURE_MANIFEST_PATH = `${INPUT_LIVE_V84_EVIDENCE_ROOT}/capture-manifest.json`;
export const INPUT_LIVE_V84_REQUEST_MANIFEST_PATH = `${INPUT_LIVE_V84_EVIDENCE_ROOT}/request-manifest.json`;
export const INPUT_LIVE_V84_INDEX_PATH = `${INPUT_LIVE_V84_EVIDENCE_ROOT}/antecedent-index.json`;
export const INPUT_LIVE_V84_AUTHORIZATION_TEMPLATE_PATH = `${INPUT_LIVE_V84_EVIDENCE_ROOT}/authorization-template.json`;
export const INPUT_LIVE_V84_AUTHORIZATION_PATH = `${INPUT_LIVE_V84_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface InputLiveV84ProofPlan {
  artifactVersion: "input-live-v84-proof-plan-v1";
  status: "draft antecedent; pending separate authorization; live execution forbidden";
  target: typeof INPUT_LIVE_V84_TARGET;
...
interface GeneratedSource {
  ...
  recipeHash: string;
  envelopeHash: string;
  expectedScenePlan: InputLiveV84SourceIdentity["expectedScenePlan"];
  outputPath: string;
  compressed: Buffer;
  metadata: InputLiveV84ProofPlan["sources"][number]["expectedScenePlan"];
}
...
const sourcePlans = (): GeneratedSource[] => {
  ...
  return prior.sources.map((source: Record<string, any>) => {
    ...
    if (!rewrittenText.includes(needle))
      throw new Error(
        `Input live v84 missing set layout.mode vertical fact: ${inputPath}`,
      );
    ...
    if (!modeRewritten.includes(paddingNeedle))
      throw new Error(
        `Input live v84 missing set layout.padding 0 fact: ${inputPath}`,
      );
    ...
    if (sourceId === "mui" && visibleFalseCount !== 24)
      throw new Error(
        `Input live v84 MUI expected-plan visible:false count ${visibleFalseCount}, expected 24`,
      );
    if (sourceId === "polaris" && visibleFalseCount !== 0)
      throw new Error(
        `Input live v84 Polar expected-plan has ${visibleFalseCount} visible:false facts; do not restamp Polar`,
      );
    ...
    if (sourceId === "mui" && occupancyKeys.length !== 24)
      throw new Error(
        `Input live v84 MUI occupancy keys ${occupancyKeys.length}, expected 24`,
      );
    if (sourceId === "polaris" && occupancyKeys.length !== 0)
      throw new Error(
        `Input live v84 Polar expected-plan has ${occupancyKeys.length} occupancy keys; do not restamp Polar`,
      );
    ...
    if (sourceId === "mui") {
      for (const key of occupancyKeys) {
        ...
        if (occurrences !== 1)
          throw new Error(
            `Input live v84 occupancy opacity:1 fact count ${occurrences} for ${key}`,
          );
    ...
    if (
      ...
      throw new Error(
        `Input live v84 MUI occupancy opacity rewrite failed: zero=${occupancyOpacityZero} one=${occupancyOpacityOne} otherOne=${otherOpacityOne}`,
      );
    if (
      sourceId === "polaris" &&
      ...
      throw new Error(
        `Input live v84 Polar opacity facts drifted; do not restamp Polar`,
      );
    ...
    if (sourceId === "mui" && (visibleFalseAfter !== 0 || visibleTrueAfter !== 1217))
      throw new Error(
        `Input live v84 MUI visible rewrite failed: false=${visibleFalseAfter} true=${visibleTrueAfter}`,
      );
    if (sourceId === "polaris" && (visibleFalseAfter !== 0 || visibleTrueAfter !== 1089))
      throw new Error(
        `Input live v84 Polar visible facts drifted: false=${visibleFalseAfter} true=${visibleTrueAfter}`,
      );
    const setMode = plan.facts.find((fact) => fact.id === "root#layout.mode@0000");
    if (setMode?.value !== "horizontal")
      throw new Error(
        `Input live v84 set layout.mode rewrite failed: ${inputPath}`,
      );
    ...
    if (
      ...
      throw new Error(
        `Input live v84 set layout.padding rewrite failed: ${inputPath}`,
      );
    ...
    if (sourceId === "polaris") {
      ...
      if (!priorUncompressed.equals(rewritten))
        throw new Error(
          "Input live v84 Polar uncompressed drifted from V83; do not restamp Polar",
        );
    }
    return {
      source: sourceId,
      ...
      outputPath: `${INPUT_LIVE_V84_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
      compressed: rewrittenCompressed,
      metadata: {
        path: `${INPUT_LIVE_V84_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
...
const captureCells = (): InputLiveV84CaptureCell[] => {
  const cells = json<Record<string, any>>(V16_CAPTURE_MANIFEST)
    .cells as InputLiveV84CaptureCell[];
  inputLiveV84CaptureManifestSha256(cells);
  return cells;
};

const protocol = () => ({
  artifactVersion: "input-live-v84-external-operator-protocol-v1",
  protocolId: "input-live-v84",
  status:
  ...
  hostNormalization: {
    ...
    carriedSceneReadback: "recipe/scene-readback-v84.ts",
    carriedSceneReadbackRuntime: "recipe/scene-readback-runtime-v84.ts",
    carriedV3Verifier: "recipe/input-field-live-v3-verifier-v84.ts",
...
const requestManifest = (
  captures: readonly InputLiveV84CaptureCell[],
  programs: {
    writerSha256: string;
  ...
  const requests = [
    {
      requestId: "writer",
      sequence: inputLiveV84RequestSequence("writer"),
      phase: "writer",
      predecessorRequestId: null,
      programSha256: programs.writerSha256,
      response: INPUT_LIVE_V84_RESPONSE_CONTRACTS.writer,
    },
    {
      requestId: "cleanup",
      sequence: inputLiveV84RequestSequence("cleanup"),
      phase: "cleanup",
      predecessorRequestId: "writer",
      programBuilderSha256: programs.cleanupBuilderSha256,
      response: INPUT_LIVE_V84_RESPONSE_CONTRACTS.cleanup,
      availability: "persisted immediately after writer acceptance",
    },
    {
      requestId: "restore",
      sequence: inputLiveV84RequestSequence("restore"),
      phase: "restore",
      predecessorRequestId: "writer",
      programSha256: programs.restoreBlueprintSha256,
      response: INPUT_LIVE_V84_RESPONSE_CONTRACTS.restore,
      availability:
        "issued immediately after writer acceptance; before extract",
    },
    {
      requestId: "extract",
      sequence: inputLiveV84RequestSequence("extract"),
      phase: "extract",
      predecessorRequestId: "restore",
      programSha256: programs.extractBlueprintSha256,
      response: INPUT_LIVE_V84_RESPONSE_CONTRACTS.extract,
    },
    {
      requestId: "probe",
      sequence: inputLiveV84RequestSequence("probe"),
      phase: "probe",
      predecessorRequestId: "extract",
      programSha256: programs.probeBlueprintSha256,
      response: INPUT_LIVE_V84_RESPONSE_CONTRACTS.probe,
    },
    ...captures.map((cell) => ({
      requestId: `capture-${String(cell.index).padStart(3, "0")}`,
      sequence: inputLiveV84RequestSequence("capture", cell.index),
      phase: "capture",
      ...
      response: INPUT_LIVE_V84_RESPONSE_CONTRACTS.capture,
    })),
  ];
  return {
    artifactVersion: "input-live-v84-request-manifest-v1",
    expectedDynamicTool: INPUT_LIVE_V84_DYNAMIC_TOOL,
    target: INPUT_LIVE_V84_TARGET,
...
const lifecycleExcludedPaths = [
  INPUT_LIVE_V84_AUTHORIZATION_TEMPLATE_PATH,
...
const sourceNeutralDependencies = [
  ...
  "recipe/normalize.ts",
  "recipe/scene-readback-runtime-v84.ts",
  "recipe/scene-readback-v84.ts",
  "recipe/input-field-live-v84-restore.ts",
  "recipe/input-field-figma-writer-v18.ts",
  "recipe/input-field-figma-writer-v19.ts",
] as const;

export async function buildInputLiveV84Proof(
  check = process.argv.includes("--check"),
): Promise<InputLiveV84ProofPlan> {
  ...
  if (
    ...
    throw new Error("Input live v84 frozen v16 writer bytes drifted");
  ...
  if (
    ...
    throw new Error(
      "Input live v84 exact committed v16 writer/restore/runtime/extract or hashed v16/v17/v18/v19 writer or v17-v66/v70/v83 scene-readback bytes drifted",
    );

  const writerOwnershipBlueprint: InputLiveV84WriterOwnership = {
    pageId: "__WRITER_PAGE_ID__",
    pageName: "Recipe Pivot / Input Field / v9",
    runIdentity: "input-live-v84",
  ...
  const identities = sources.map(
    (source) =>
      ...
      }) satisfies InputLiveV84SourceIdentity,
  );
  const restoreBlueprint = Buffer.from(
    buildInputLiveV84RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildInputLiveV84ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildInputLiveV84ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildInputLiveV84CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );

  const antecedentOutputs = new Map<string, Buffer>();
  antecedentOutputs.set(INPUT_LIVE_V84_PROTOCOL_PATH, jsonBytes(protocol()));
  antecedentOutputs.set(
    `${INPUT_LIVE_V84_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V84_EVIDENCE_ROOT}/programs/writer-payload.js`,
    writerPayload,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V84_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V84_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V84_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V84_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  ...
  const captureManifest = {
    artifactVersion: "input-live-v84-capture-manifest-v1",
    ...
    cells: captures,
    cellsSha256: inputLiveV84CaptureManifestSha256(captures),
  };
  const captureManifestBytes = jsonBytes(captureManifest);
  antecedentOutputs.set(
    INPUT_LIVE_V84_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: InputLiveV84ProofPlan = {
    artifactVersion: "input-live-v84-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: INPUT_LIVE_V84_TARGET,
    namespace: "ds.contracts.input.recipe.v5",
    writer: {
      programPath: `${INPUT_LIVE_V84_EVIDENCE_ROOT}/programs/writer.txt`,
      programBytes: writerProgram.byteLength,
      programSha256: sha256(writerProgram),
      payloadPath: `${INPUT_LIVE_V84_EVIDENCE_ROOT}/programs/writer-payload.js`,
    ...
    captureManifest: {
      path: INPUT_LIVE_V84_CAPTURE_MANIFEST_PATH,
      cells: 128,
      requests: 128,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(INPUT_LIVE_V84_PLAN_PATH, proofPlanBytes);
  ...
  antecedentOutputs.set(
    INPUT_LIVE_V84_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/input-field-live-v84-broker.ts",
    "recipe/input-field-live-v84-contract.ts",
    "recipe/run-input-field-live-v84.ts",
    "recipe/build-input-field-live-proof-v84.ts",
    "recipe/input-field-live-v84-broker.test.ts",
    "recipe/figma-property-normalizer-v8.test.ts",
    "recipe/scene-readback-v84.test.ts",
    "recipe/input-field-live-v84-restore.ts",
    "recipe/input-field-live-v84-restore.test.ts",
    "recipe/input-field-live-v84-lifecycle-simulation.ts",
    ...sourceNeutralDependencies,
  ].filter((artifactPath) => artifactPath !== INPUT_LIVE_V84_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "input-live-v84-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    ...
    counts: {
      ...
      captureCells: INPUT_LIVE_V84_CAPTURE_COUNT,
  ...
  antecedentOutputs.set(INPUT_LIVE_V84_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "input-live-v84-authorization-template-v1",
    authorizationId: "input-live-v84",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: INPUT_LIVE_V84_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
  ...
  if (check) {
    ...
    mkdirSync(path.dirname(INPUT_LIVE_V84_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(INPUT_LIVE_V84_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(INPUT_LIVE_V84_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(INPUT_LIVE_V84_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildInputLiveV84Proof(), null, 2)}\n`,
  );
