...
import { INPUT_LIVE_V63_INDEX_PATH } from "./build-input-field-live-proof-v63.js";
import {
  INPUT_LIVE_V63_DYNAMIC_TOOL,
  INPUT_LIVE_V63_TARGET,
} from "./input-field-live-v63-broker.js";
import {
  buildInputLiveV63AuthorizationArtifact,
  validateInputLiveV63AntecedentIndex,
  validateInputLiveV63AuthorizationArtifact,
  validateInputLiveV63History,
  validateInputLiveV63UnitTestSource,
  type InputLiveV63AntecedentIndex,
  type InputLiveV63AuthorizationArtifact,
  type InputLiveV63AuthorizationProof,
  type InputLiveV63HistoryState,
} from "./input-field-live-v63-authorization.js";
import {
  inputLiveV63AttestationSha256,
  validateInputLiveV63ControlFlowSource,
  validateInputLiveV63SecurityAttestation,
  type InputLiveV63SecurityAttestation,
} from "./input-field-live-v63-preflight.js";
import { inputLiveV63Sha256 } from "./input-field-live-v63-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V63_INDEX_PATH);
const index = (): InputLiveV63AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV63AntecedentIndex;

const artifact = (): InputLiveV63AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV63AuthorizationArtifact({
...
const pendingState = (): InputLiveV63HistoryState => ({
  index: index(),
  indexSha256: inputLiveV63Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV63HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV63AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV63Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V63_TARGET,
  expectedDynamicTool: INPUT_LIVE_V63_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v63/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV63AuthorizationArtifact,
): InputLiveV63SecurityAttestation => {
  const body: Omit<InputLiveV63SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v63-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V63_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV63AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV63History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV63History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV63History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV63History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV63History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV63UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV63UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV63AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV63Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV63AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV63AuthorizationArtifact(
        planted as InputLiveV63AuthorizationArtifact,
        index(),
        inputLiveV63Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV63SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV63AttestationSha256(
      body as Omit<InputLiveV63SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV63SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV63ControlFlowSource(source), []);
});
