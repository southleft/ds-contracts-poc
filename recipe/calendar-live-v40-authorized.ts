...
import { CALENDAR_LIVE_V40_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v40.js";
import { verifyCalendarLiveV40Authorization } from "./calendar-live-v40-authorization.js";
import { runCalendarLiveV40Preflight } from "./calendar-live-v40-preflight.js";
import { CalendarLiveV40Orchestrator } from "./run-calendar-live-v40.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v40 requires exact authorization artifact ${CALENDAR_LIVE_V40_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v40-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV40Authorization(root);
    runCalendarLiveV40Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV40Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV40Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
