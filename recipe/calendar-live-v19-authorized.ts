...
import { CALENDAR_LIVE_V19_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v19.js";
import { verifyCalendarLiveV19Authorization } from "./calendar-live-v19-authorization.js";
import { runCalendarLiveV19Preflight } from "./calendar-live-v19-preflight.js";
import { CalendarLiveV19Orchestrator } from "./run-calendar-live-v19.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v19 requires exact authorization artifact ${CALENDAR_LIVE_V19_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v19-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV19Authorization(root);
    runCalendarLiveV19Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV19Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV19Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
