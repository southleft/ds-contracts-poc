...
export type TableLiveV16HistoryExpectation = "pending" | "authorized";

export interface TableLiveV16AuthorizationArtifact {
  artifactVersion: "table-live-v16-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V16_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV16AntecedentIndex {
...
export interface TableLiveV16HistoryState {
  index?: TableLiveV16AntecedentIndex;
  indexSha256: string;
...
export interface TableLiveV16AuthorizationProof
  extends TableLiveV16TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V16_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V16_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V16_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V16_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v16-authorization") ||
  artifactPath.includes("table-live-v16-preflight") ||
  artifactPath.includes("table-live-v16-authorized") ||
  artifactPath.includes("create-table-live-v16-security-attestation") ||
  artifactPath.includes("table-live-pivot-v16-status") ||
...
export function validateTableLiveV16AntecedentIndex(
  index: TableLiveV16AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v16-antecedent-index-v1" ||
...
export function validateTableLiveV16UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV16History\(\s*["']/.test(source) ||
    /readTableLiveV16HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV16AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV16AuthorizationArtifact {
  ...
  ) as TableLiveV16AntecedentIndex;
  const indexFailures = validateTableLiveV16AntecedentIndex(index);
...
export function validateTableLiveV16AuthorizationArtifact(
  artifact: TableLiveV16AuthorizationArtifact | undefined,
  index: TableLiveV16AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v16-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v16" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V16_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V16_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V16_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V16_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V16_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V16_SOURCE_ROOTS ||
...
export function validateTableLiveV16History(
  state: TableLiveV16HistoryState,
  expected: TableLiveV16HistoryExpectation,
): string[] {
  const failures = validateTableLiveV16AntecedentIndex(state.index);
...
export function readTableLiveV16HistoryState(
  root = process.cwd(),
): TableLiveV16HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V16_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV16AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V16_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V16_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V16_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV16History(
  expected: TableLiveV16HistoryExpectation,
  root = process.cwd(),
): TableLiveV16HistoryState {
  const state = readTableLiveV16HistoryState(root);
  const failures = validateTableLiveV16History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v16 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV16Authorization(
  root = process.cwd(),
): TableLiveV16AuthorizationProof {
  const state = verifyTableLiveV16History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V16_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v16/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v16.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V16_TARGET,
    expectedDynamicTool: TABLE_LIVE_V16_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V16_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV16HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV16History(expected);
  process.stdout.write(
    `Table live v16 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
