...
import {
  SCENE_READBACK_V11_TAUGHT_FILL_KINDS,
  scenePaintToIr,
  sceneToNormalizedIr,
...
test("v10 live host path does not import hashed scene-readback.ts", () => {
  const verifier = readFileSync("recipe/input-field-live-v11-verifier.ts", "utf8");
  const contract = readFileSync("recipe/input-field-live-v11-contract.ts", "utf8");
  const carried = readFileSync(
    "recipe/input-field-live-v3-verifier-v11.ts",
    "utf8",
  );
  assert.match(verifier, /input-field-live-v3-verifier-v11/);
  assert.doesNotMatch(
    verifier,
    /from "\.\/input-field-live-v3-verifier\.js"/,
  );
  assert.doesNotMatch(verifier, /from "\.\/scene-readback\.js"/);
  assert.match(contract, /input-field-live-v3-verifier-v11/);
  assert.doesNotMatch(
    contract,
    /from "\.\/input-field-live-v3-verifier\.js"/,
  );
  assert.doesNotMatch(contract, /from "\.\/scene-readback\.js"/);
  assert.match(carried, /from "\.\/scene-readback-v11\.js"/);
