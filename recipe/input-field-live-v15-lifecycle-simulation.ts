...
export interface InputLiveV15LifecycleSimulationReport {
  artifactVersion: "input-live-v15-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV15AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV15Lifecycle(): Promise<InputLiveV15LifecycleSimulationReport> {
  await buildInputLiveV15Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V15_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV15AntecedentIndex;
  const indexSha256 = inputLiveV15Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v15-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V15_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V15_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV15History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV15History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV15AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V15_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V15_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV15History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV15History(authorized, "pending");
    ...
    await buildInputLiveV15Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V15_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV15AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV15Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV15Lifecycle(), null, 2)}\n`,
  );
