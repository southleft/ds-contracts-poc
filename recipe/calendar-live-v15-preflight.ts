...
import {
  validateCalendarLiveV15AntecedentIndex,
  verifyCalendarLiveV15Authorization,
  type CalendarLiveV15AntecedentIndex,
  type CalendarLiveV15AuthorizationProof,
} from "./calendar-live-v15-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V15_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v15/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V15_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v15-security-attestation.json";

export interface CalendarLiveV15SecurityAttestation {
  artifactVersion: "calendar-live-v15-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV15PreflightReport {
  artifactVersion: "calendar-live-v15-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV15SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV15SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV15AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v15-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V15_TARGET) ||
...
export function validateCalendarLiveV15ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV15Authorization");
  const preflight = source.indexOf("runCalendarLiveV15Preflight");
  const initialize = source.indexOf("CalendarLiveV15Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV15Preflight(
  root: string,
  proof: CalendarLiveV15AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V15_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV15PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V15_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V15_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV15AntecedentIndex;
  failures.push(...validateCalendarLiveV15AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V15_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V15_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV15SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v15-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV15ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v15 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV15SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v15-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V15_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV15Preflight(
    process.cwd(),
    verifyCalendarLiveV15Authorization(),
    attestationPath,
