...
import {
  verifyComboboxLiveV37FixedSceneFixedPoint,
  type ComboboxLiveV37FixedFixedPoint,
} from "./combobox-live-v37-fixed-point.js";

export const COMBOBOX_LIVE_V37_VERIFIER_VERSION = "combobox-live-v37-verifier-v1";

export function buildComboboxLiveV37RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV37RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV37RawNode[];
}

export interface ComboboxLiveV37NormalizedScene {
...
export interface ComboboxLiveV37SceneProof {
...
export function normalizeComboboxLiveV37Scene(
  root: ComboboxLiveV37RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV37NormalizedScene {
  const canonicalBindings: ComboboxLiveV37NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV37RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV37Scene<Instance>(
  normalized: ComboboxLiveV37NormalizedScene,
...
): ComboboxLiveV37SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV37FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV37ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV37PreCaptureGates(payload: unknown): void {
