...
import { INPUT_LIVE_V32_INDEX_PATH } from "./build-input-field-live-proof-v32.js";
import {
  INPUT_LIVE_V32_DYNAMIC_TOOL,
  INPUT_LIVE_V32_TARGET,
} from "./input-field-live-v32-broker.js";
import {
  buildInputLiveV32AuthorizationArtifact,
  validateInputLiveV32AntecedentIndex,
  validateInputLiveV32AuthorizationArtifact,
  validateInputLiveV32History,
  validateInputLiveV32UnitTestSource,
  type InputLiveV32AntecedentIndex,
  type InputLiveV32AuthorizationArtifact,
  type InputLiveV32AuthorizationProof,
  type InputLiveV32HistoryState,
} from "./input-field-live-v32-authorization.js";
import {
  inputLiveV32AttestationSha256,
  validateInputLiveV32ControlFlowSource,
  validateInputLiveV32SecurityAttestation,
  type InputLiveV32SecurityAttestation,
} from "./input-field-live-v32-preflight.js";
import { inputLiveV32Sha256 } from "./input-field-live-v32-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V32_INDEX_PATH);
const index = (): InputLiveV32AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV32AntecedentIndex;

const artifact = (): InputLiveV32AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV32AuthorizationArtifact({
...
const pendingState = (): InputLiveV32HistoryState => ({
  index: index(),
  indexSha256: inputLiveV32Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV32HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV32AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV32Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V32_TARGET,
  expectedDynamicTool: INPUT_LIVE_V32_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v32/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV32AuthorizationArtifact,
): InputLiveV32SecurityAttestation => {
  const body: Omit<InputLiveV32SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v32-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V32_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV32AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV32History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV32History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV32History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV32History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV32History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV32UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV32UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV32AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV32Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV32AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV32AuthorizationArtifact(
        planted as InputLiveV32AuthorizationArtifact,
        index(),
        inputLiveV32Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV32SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV32AttestationSha256(
      body as Omit<InputLiveV32SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV32SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV32ControlFlowSource(source), []);
});
