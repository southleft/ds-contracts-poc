/**
 * Calendar live v35 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V35_FIXED_VERIFIER_VERSION = "calendar-live-v35-fixed-point-v1";
export const CALENDAR_LIVE_V35_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV35FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV35FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV35FixedSourceProbe {
...
export interface CalendarLiveV35FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV35FixedFixedPoint;
}

export interface CalendarLiveV35FixedHardGateInput {
  thresholds: CalendarLiveV35FixedThresholds;
  sourceProbes: CalendarLiveV35FixedSourceProbe[];
  cells: CalendarLiveV35FixedCellValidation[];
  sceneProofs: CalendarLiveV35FixedSceneProof[];
...
export interface CalendarLiveV35FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V35_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV35FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV35FixedHardGates(
  input: CalendarLiveV35FixedHardGateInput,
): CalendarLiveV35FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV35FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V35_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV35FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV35FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV35FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
