...
import { TABLE_LIVE_V23_INDEX_PATH } from "./build-table-live-proof-v23.js";
import {
...
import {
  buildTableLiveV23AuthorizationArtifact,
  validateTableLiveV23AntecedentIndex,
  validateTableLiveV23AuthorizationArtifact,
  validateTableLiveV23History,
  validateTableLiveV23UnitTestSource,
  type TableLiveV23AntecedentIndex,
  type TableLiveV23AuthorizationArtifact,
  type TableLiveV23AuthorizationProof,
  type TableLiveV23HistoryState,
} from "./table-live-v23-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV23ControlFlowSource,
  validateTableLiveV23SecurityAttestation,
  type TableLiveV23SecurityAttestation,
} from "./table-live-v23-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V23_INDEX_PATH);
const index = (): TableLiveV23AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV23AntecedentIndex;

const artifact = (): TableLiveV23AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV23AuthorizationArtifact({
...
const pendingState = (): TableLiveV23HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV23HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV23AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v23/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV23AuthorizationArtifact,
): TableLiveV23SecurityAttestation => {
  const body: Omit<TableLiveV23SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "table-live-v23-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V23_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV23History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV23History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV23History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV23History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV23History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV23UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v23-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV23UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV23AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV23AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV23AuthorizationArtifact(
        planted as TableLiveV23AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV23SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV23SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV23SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v23-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV23ControlFlowSource(source), []);
});
