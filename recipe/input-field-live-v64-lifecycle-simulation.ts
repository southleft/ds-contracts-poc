...
export interface InputLiveV64LifecycleSimulationReport {
  artifactVersion: "input-live-v64-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV64AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV64Lifecycle(): Promise<InputLiveV64LifecycleSimulationReport> {
  await buildInputLiveV64Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V64_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV64AntecedentIndex;
  const indexSha256 = inputLiveV64Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v64-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V64_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V64_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV64History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV64History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV64AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V64_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V64_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV64History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV64History(authorized, "pending");
    ...
    await buildInputLiveV64Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V64_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV64AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV64Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV64Lifecycle(), null, 2)}\n`,
  );
