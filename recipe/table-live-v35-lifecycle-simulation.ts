...
import { tableLiveV2Sha256 } from "./table-live-v35-broker.js";

export interface TableLiveV35LifecycleSimulationReport {
  artifactVersion: "table-live-v35-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV35AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV35Lifecycle(): Promise<TableLiveV35LifecycleSimulationReport> {
  await buildTableLiveV35Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V35_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV35AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v35-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V35_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V35_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV35History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV35History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV35AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V35_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V35_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV35History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV35History(authorized, "pending");
    ...
    await buildTableLiveV35Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V35_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV35AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV35Lifecycle(), null, 2)}\n`,
  );
