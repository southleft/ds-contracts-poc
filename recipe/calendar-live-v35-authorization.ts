...
export type CalendarLiveV35HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV35AuthorizationArtifact {
  artifactVersion: "calendar-live-v35-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V35_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV35AntecedentIndex {
...
export interface CalendarLiveV35HistoryState {
  index?: CalendarLiveV35AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV35AuthorizationArtifact;
...
export interface CalendarLiveV35AuthorizationProof
  extends CalendarLiveV35TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V35_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V35_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V35_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V35_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v35-authorization") ||
  artifactPath.includes("calendar-live-v35-preflight") ||
  artifactPath.includes("calendar-live-v35-authorized") ||
  artifactPath.includes("create-calendar-live-v35-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v35-status") ||
...
export function validateCalendarLiveV35AntecedentIndex(
  index: CalendarLiveV35AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v35-antecedent-index-v1" ||
...
export function validateCalendarLiveV35UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV35History\(\s*["']/.test(source) ||
    /readCalendarLiveV35HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV35AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV35AuthorizationArtifact {
  ...
  ) as CalendarLiveV35AntecedentIndex;
  const indexFailures = validateCalendarLiveV35AntecedentIndex(index);
...
export function validateCalendarLiveV35AuthorizationArtifact(
  artifact: CalendarLiveV35AuthorizationArtifact | undefined,
  index: CalendarLiveV35AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v35-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v35" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V35_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V35_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V35_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V35_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V35_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V35_SOURCE_ROOTS ||
...
export function validateCalendarLiveV35History(
  state: CalendarLiveV35HistoryState,
  expected: CalendarLiveV35HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV35AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV35AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV35HistoryState(
  root = process.cwd(),
): CalendarLiveV35HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V35_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV35AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V35_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V35_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V35_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV35History(
  expected: CalendarLiveV35HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV35HistoryState {
  const state = readCalendarLiveV35HistoryState(root);
  const failures = validateCalendarLiveV35History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v35 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV35Authorization(
  root = process.cwd(),
): CalendarLiveV35AuthorizationProof {
  const state = verifyCalendarLiveV35History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V35_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v35/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v35.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V35_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V35_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V35_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV35HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV35History(expected);
  process.stdout.write(
    `Calendar live v35 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
