...
} from "./input-field-live-v3-verifier-v24.js";
import {
  buildInputLiveV24RawPropertyRuntime,
  normalizeInputLiveV24Scene,
  type InputLiveV24RawNode,
} from "./input-field-live-v24-verifier.js";
...
export const INPUT_LIVE_V24_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V24_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V24_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V24_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V24_VARIANT_COUNT = 256;
export const INPUT_LIVE_V24_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V24_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV24SourceId = (typeof INPUT_LIVE_V24_SOURCE_IDS)[number];

export interface InputLiveV24SourceIdentity {
  source: InputLiveV24SourceId;
...
export interface InputLiveV24WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV24ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV24SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV24RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV24RootProof {
  source: InputLiveV24SourceId;
  adapterIdentity: string;
...
export interface InputLiveV24ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV24SourceId;
    adapterIdentity: string;
...
export interface InputLiveV24CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV24SourceId;
  adapterIdentity: string;
...
export interface InputLiveV24CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV24SourceId;
  strata: InputLiveV24CaptureCell["strata"];
...
export interface InputLiveV24CleanupPayload {
...
export function validateInputLiveV24WriterPayload(
  value: unknown,
): InputLiveV24WriterOwnership {
...
const identityPlan = (source: InputLiveV24SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV24ExtractProgram(
  writer: InputLiveV24WriterOwnership,
  sources: readonly InputLiveV24SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V24_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V24_NAMESPACE)}
${buildInputLiveV24RawPropertyRuntime()}
...
export function validateInputLiveV24ExtractPayload(
  value: unknown,
  writer: InputLiveV24WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V24_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV24ExtractPayload;
}

export function proveInputLiveV24Roots<Instance>(
  extract: InputLiveV24ExtractPayload,
  sources: readonly (InputLiveV24SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV24Scene(
      root.scene,
...
export function assertInputLiveV24RootProofs(
  proofs: readonly InputLiveV24RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV24ProbeProgram(
  writer: InputLiveV24WriterOwnership,
  sources: readonly InputLiveV24SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V24_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV24ProbePayload(
  value: unknown,
  writer: InputLiveV24WriterOwnership,
): InputLiveV24ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV24ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V24_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V24_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV24CaptureManifest(
  cells: readonly InputLiveV24CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V24_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V24_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V24_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V24_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V24_SOURCE_IDS.some(
...
export const inputLiveV24CaptureManifestSha256 = (
  cells: readonly InputLiveV24CaptureCell[],
): string => {
  validateInputLiveV24CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV24CaptureProgram(
  writer: InputLiveV24WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V24_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V24_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V24_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV24CapturePayload(
  value: unknown,
  cell: InputLiveV24CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV24CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V24_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V24_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV24CaptureResponses(
  manifest: readonly InputLiveV24CaptureCell[],
  responses: readonly InputLiveV24CapturePayload[],
): void {
  validateInputLiveV24CaptureManifest(manifest);
  if (
...
export interface InputLiveV24ObjectiveReport {
  artifactVersion: "input-live-v24-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV24Objective(
  manifest: readonly InputLiveV24CaptureCell[],
  responses: readonly InputLiveV24CapturePayload[],
  readReference: (cell: InputLiveV24CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV24CaptureCell) => Buffer,
): InputLiveV24ObjectiveReport {
  assertInputLiveV24CaptureResponses(manifest, responses);
...
export function buildInputLiveV24CleanupProgram(
  writer: InputLiveV24WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V24_NAMESPACE,
...
export function validateInputLiveV24CleanupPayload(
  value: unknown,
  writer: InputLiveV24WriterOwnership,
): InputLiveV24CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV24CleanupPayload;
...
export const INPUT_LIVE_V24_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV24WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
