...
import {
  ...
  verifyComboboxLiveV2Authorization,
  type ComboboxLiveV2AntecedentIndex,
  type ComboboxLiveV2AuthorizationProof,
} from "./combobox-live-v2-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V2_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v2/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V2_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v2-security-attestation.json";

export interface ComboboxLiveV2SecurityAttestation {
...
export interface ComboboxLiveV2PreflightReport {
  artifactVersion: "combobox-live-v2-preflight-v1";
...
export const comboboxLiveV2AttestationSha256 = (
  value: Omit<ComboboxLiveV2SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV2SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV2AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v2-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V2_TARGET) ||
...
export function validateComboboxLiveV2ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV2Authorization");
  const preflight = source.indexOf("runComboboxLiveV2Preflight");
  const initialize = source.indexOf("ComboboxLiveV2Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV2Preflight(
  root: string,
  proof: ComboboxLiveV2AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V2_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV2PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V2_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V2_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V2_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV2AntecedentIndex;
  failures.push(...validateComboboxLiveV2AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V2_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V2_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v2-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV2ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v1 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV2SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v2-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V2_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV2Preflight(
    process.cwd(),
    verifyComboboxLiveV2Authorization(),
    attestationPath,
