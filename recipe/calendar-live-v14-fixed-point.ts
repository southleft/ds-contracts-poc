/**
 * Calendar live v14 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V14_FIXED_VERIFIER_VERSION = "calendar-live-v14-fixed-point-v1";
export const CALENDAR_LIVE_V14_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV14FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV14FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV14FixedSourceProbe {
...
export interface CalendarLiveV14FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV14FixedFixedPoint;
}

export interface CalendarLiveV14FixedHardGateInput {
  thresholds: CalendarLiveV14FixedThresholds;
  sourceProbes: CalendarLiveV14FixedSourceProbe[];
  cells: CalendarLiveV14FixedCellValidation[];
  sceneProofs: CalendarLiveV14FixedSceneProof[];
...
export interface CalendarLiveV14FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V14_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV14FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV14FixedHardGates(
  input: CalendarLiveV14FixedHardGateInput,
): CalendarLiveV14FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV14FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V14_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV14FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV14FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV14FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
