...
import {
  ...
  CALENDAR_LIVE_V26_HOST_PHASES,
  CALENDAR_LIVE_V26_REMOTE_REQUESTS,
  CALENDAR_LIVE_V26_SOURCE_ROOTS,
  CALENDAR_LIVE_V26_VARIANT_COUNT,
  buildCalendarLiveV26CaptureProgram,
  buildCalendarLiveV26CleanupProgram,
  buildCalendarLiveV26ExtractProgram,
  buildCalendarLiveV26ProbeProgram,
  buildCalendarLiveV26RestoreProgram,
  calendarLiveV1CaptureManifestSha256,
  type CalendarLiveV26CaptureCell,
  type CalendarLiveV26SourceIdentity,
  type CalendarLiveV26WriterOwnership,
} from "./calendar-live-v26-contract.js";
import {
  CALENDAR_LIVE_V26_DYNAMIC_TOOL,
  CALENDAR_LIVE_V26_TARGET,
  calendarLiveV1RequestSequence,
} from "./calendar-live-v26-broker.js";
import {
...
export const CALENDAR_LIVE_V26_EVIDENCE_ROOT =
  "recipe/evidence/calendar-live-pivot-v26";
export const CALENDAR_LIVE_V26_PROTOCOL_PATH = `${CALENDAR_LIVE_V26_EVIDENCE_ROOT}/protocol.json`;
export const CALENDAR_LIVE_V26_PLAN_PATH = `${CALENDAR_LIVE_V26_EVIDENCE_ROOT}/proof-plan.json`;
export const CALENDAR_LIVE_V26_CAPTURE_MANIFEST_PATH = `${CALENDAR_LIVE_V26_EVIDENCE_ROOT}/capture-manifest.json`;
export const CALENDAR_LIVE_V26_REQUEST_MANIFEST_PATH = `${CALENDAR_LIVE_V26_EVIDENCE_ROOT}/request-manifest.json`;
export const CALENDAR_LIVE_V26_INDEX_PATH = `${CALENDAR_LIVE_V26_EVIDENCE_ROOT}/antecedent-index.json`;
export const CALENDAR_LIVE_V26_AUTHORIZATION_TEMPLATE_PATH = `${CALENDAR_LIVE_V26_EVIDENCE_ROOT}/authorization-template.json`;
export const CALENDAR_LIVE_V26_AUTHORIZATION_PATH = `${CALENDAR_LIVE_V26_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface CalendarLiveV26ProofPlan {
  artifactVersion: "calendar-live-v26-proof-plan-v1";
...
const captureCells = (
...
): CalendarLiveV26CaptureCell[] => {
  const cells: CalendarLiveV26CaptureCell[] = [];
  ...
  if (cells.length !== CALENDAR_LIVE_V26_CAPTURE_COUNT)
    throw new Error(
      `Calendar live v26 capture cells ${cells.length}, expected ${CALENDAR_LIVE_V26_CAPTURE_COUNT}`,
    );
...
const protocol = (expectedFacts: number) => ({
  artifactVersion: "calendar-live-v26-external-operator-protocol-v1",
  protocolId: "calendar-live-v26",
  status:
  ...
  denominator: {
    sourceRoots: CALENDAR_LIVE_V26_SOURCE_ROOTS,
    sets: 3,
    variants: CALENDAR_LIVE_V26_VARIANT_COUNT,
    captureCells: CALENDAR_LIVE_V26_CAPTURE_COUNT,
    remoteRequests: CALENDAR_LIVE_V26_REMOTE_REQUESTS,
    hostPhases: CALENDAR_LIVE_V26_HOST_PHASES,
...
export async function buildCalendarLiveV26Proof(
  check = process.argv.includes("--check"),
): Promise<CalendarLiveV26ProofPlan> {
  ...
  if (
    ...
    !writer.runIdentity.endsWith("-calendar-v26") ||
    ...
    throw new Error(
      "Calendar live v26 writer reused Input, Combobox, or Table v1–v24 identity",
    );
  const captures = captureCells(sources);
  const writerProgram = Buffer.from(writer.code);
  const writerOwnershipBlueprint: CalendarLiveV26WriterOwnership = {
  ...
  const identities: CalendarLiveV26SourceIdentity[] = sources.map((source) => ({
    source: source.source,
    adapterIdentity: source.adapterIdentity,
    recipeHash: source.recipeHash,
    envelopeHash: source.envelope.integrity.canonicalHash,
  ...
  const restoreBlueprint = Buffer.from(
    buildCalendarLiveV26RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildCalendarLiveV26ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildCalendarLiveV26ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildCalendarLiveV26CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );
  const cleanupBlueprint = Buffer.from(
    buildCalendarLiveV26CleanupProgram(writerOwnershipBlueprint),
  );
  ...
  const sourceMetadata = sources.map((source) => {
    ...
    const tablePath = `${CALENDAR_LIVE_V26_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-calendar.json.gz`;
    const rowPath = `${CALENDAR_LIVE_V26_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-week.json.gz`;
    const cellPath = `${CALENDAR_LIVE_V26_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-day.json.gz`;
  ...
  antecedentOutputs.set(
    CALENDAR_LIVE_V26_PROTOCOL_PATH,
    jsonBytes(protocol(expectedFacts)),
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V26_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V26_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V26_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V26_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V26_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V26_EVIDENCE_ROOT}/programs/cleanup-blueprint.js`,
    cleanupBlueprint,
  );

  const captureManifest = {
    artifactVersion: "calendar-live-v26-capture-manifest-v1",
  ...
  antecedentOutputs.set(
    CALENDAR_LIVE_V26_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: CalendarLiveV26ProofPlan = {
    artifactVersion: "calendar-live-v26-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: CALENDAR_LIVE_V26_TARGET,
    namespace: CALENDAR_FIGMA_NAMESPACE,
    writer: {
      programPath: `${CALENDAR_LIVE_V26_EVIDENCE_ROOT}/programs/writer.txt`,
    ...
    captureManifest: {
      path: CALENDAR_LIVE_V26_CAPTURE_MANIFEST_PATH,
      cells: 8,
      requests: 8,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(CALENDAR_LIVE_V26_PLAN_PATH, jsonBytes(proofPlan));

  const requests = {
    artifactVersion: "calendar-live-v26-request-manifest-v1",
    expectedDynamicTool: CALENDAR_LIVE_V26_DYNAMIC_TOOL,
    target: CALENDAR_LIVE_V26_TARGET,
    signedAtRuntime: true,
    requestCount: CALENDAR_LIVE_V26_REMOTE_REQUESTS,
    requests: [
      {
        requestId: "writer",
  ...
  if (requests.requestCount !== CALENDAR_LIVE_V26_REMOTE_REQUESTS)
    throw new Error("Calendar live v26 remote request denominator drifted");
  antecedentOutputs.set(
    CALENDAR_LIVE_V26_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const lifecycleExcludedPaths = [
    CALENDAR_LIVE_V26_AUTHORIZATION_TEMPLATE_PATH,
    CALENDAR_LIVE_V26_AUTHORIZATION_PATH,
    "recipe/calendar-live-v26-authorization.ts",
    "recipe/calendar-live-v26-authorization.test.ts",
    "recipe/calendar-live-v26-preflight.ts",
    "recipe/calendar-live-v26-authorized.ts",
    "recipe/write-calendar-live-v26-authorization.ts",
    "recipe/create-calendar-live-v26-security-attestation.ts",
    "recipe/evidence/calendar-live-pivot-v26/operator-security-attestation-template.json",
    "recipe/evidence/calendar-live-pivot-v26-status.json",
    "recipe/evidence/status-index.json",
  ];
  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/calendar-live-v26-broker.ts",
    "recipe/calendar-live-v26-contract.ts",
    "recipe/calendar-live-v26-restore.ts",
    "recipe/calendar-live-v26-cleanup.ts",
    "recipe/calendar-live-v26-verifier.ts",
    "recipe/calendar-live-v26-fixed-point.ts",
    "recipe/run-calendar-live-v26.ts",
    "recipe/build-calendar-live-proof-v26.ts",
    "recipe/calendar-live-v26-lifecycle-simulation.ts",
    ...
    "recipe/normalize.ts",
  ].filter((artifactPath) => artifactPath !== CALENDAR_LIVE_V26_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "calendar-live-v26-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
  ...
  antecedentOutputs.set(CALENDAR_LIVE_V26_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "calendar-live-v26-authorization-template-v1",
    authorizationId: "calendar-live-v26",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: CALENDAR_LIVE_V26_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
    ...
    denominator: {
      remoteRequests: CALENDAR_LIVE_V26_REMOTE_REQUESTS,
      hostPhases: CALENDAR_LIVE_V26_HOST_PHASES,
      captures: CALENDAR_LIVE_V26_CAPTURE_COUNT,
      sourceRoots: CALENDAR_LIVE_V26_SOURCE_ROOTS,
      expectedFacts,
    },
    execution: {
  ...
  const attestationTemplate = jsonBytes({
    artifactVersion: "calendar-live-v26-operator-security-attestation-template-v1",
    status: "template only; complete privately after PREPARE+AUTHORIZE",
    tokenValuesForbidden: true,
    privateDirectory: "private/calendar-live-v26-security-attestation.json",
  });
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Calendar live v26 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(CALENDAR_LIVE_V26_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(CALENDAR_LIVE_V26_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
    writeFileSync(
      `${CALENDAR_LIVE_V26_EVIDENCE_ROOT}/operator-security-attestation-template.json`,
      attestationTemplate,
    );
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(CALENDAR_LIVE_V26_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(CALENDAR_LIVE_V26_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Calendar live v26 authorization template drift");
  }
  if (
    writer.namespace !== CALENDAR_FIGMA_NAMESPACE ||
    writer.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    ...
    !readFileSync("recipe/calendar-live-v26-contract.ts", "utf8").includes(
    ...
    throw new Error("Calendar live v26 identity or cleanup invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildCalendarLiveV26Proof(), null, 2)}\n`,
  );
