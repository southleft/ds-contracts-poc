/**
 * Carried Combobox live v5 host verifier. Uses scene-readback-combobox-v5 so live
...
export const COMBOBOX_LIVE_V5_FIXED_VERIFIER_VERSION = "combobox-live-v5-fixed-point-v1";
export const COMBOBOX_LIVE_V5_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV5FixedThresholds {
...
export interface ComboboxLiveV5FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV5FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV5FixedSourceProbe {
...
export interface ComboboxLiveV5FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV5FixedFixedPoint;
}

export interface ComboboxLiveV5FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV5FixedHardGateInput {
  thresholds: ComboboxLiveV5FixedThresholds;
  materialRegression: ComboboxLiveV5FixedMaterialRegression;
  sourceProbes: ComboboxLiveV5FixedSourceProbe[];
  cells: ComboboxLiveV5FixedCellValidation[];
  sceneProofs: ComboboxLiveV5FixedSceneProof[];
  visualRows: ComboboxLiveV5FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV5FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V5_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV5FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV5FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV5FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV5FixedHardGates(
  input: ComboboxLiveV5FixedHardGateInput,
): ComboboxLiveV5FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV5FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV5FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V5_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV5FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV5FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
