...
import {
  acceptTableLiveV14Response,
  createTableLiveV14Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV14Request,
  persistTableLiveV14RawResponse,
  persistTableLiveV14TechnicalGates,
  readTableLiveV14AcceptedRawPayload,
  type TableLiveV14Request,
  type TableLiveV14TransactionAuthorization,
} from "./table-live-v14-broker.js";
import {
  TABLE_LIVE_V14_CAPTURE_COUNT,
  assertTableLiveV14CaptureResponses,
  ...
  evaluateTableLiveV14Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV14Roots,
  validateTableLiveV14CaptureManifest,
  validateTableLiveV14CapturePayload,
  validateTableLiveV14CleanupPayload,
  validateTableLiveV14ExtractPayload,
  validateTableLiveV14ProbePayload,
  validateTableLiveV14WriterPayload,
  type TableLiveV14CaptureCell,
  type TableLiveV14CapturePayload,
...
import {
  ...
  TABLE_LIVE_V14_PLAN_PATH,
  type TableLiveV14ProofPlan,
...
export const TABLE_LIVE_V14_RUNNER_VERSION = "table-live-v14-runner-v1";

interface TableLiveV14JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV14OrchestratorState {
  artifactVersion: "table-live-v14-orchestrator-state-v1";
  ...
  journal: TableLiveV14JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV14ProofPlan;
  planSha256: string;
  captures: TableLiveV14CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV14SourceIdentity & {
...
export interface TableLiveV14NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV14OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v14 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V14_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V14_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV14ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV14CaptureCell[];
  validateTableLiveV14CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v14 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v14 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV14WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV14RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV14Request =>
  readJson<TableLiveV14Request>(file);

export class TableLiveV14Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV14Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV14OrchestratorState = {
      artifactVersion: "table-live-v14-orchestrator-state-v1",
    ...
    return new TableLiveV14Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV14OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV14Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV14NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV14NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v14 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV14NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v14 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV14RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV14Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV14CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV14AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV14WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV14Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV14ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV14Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV14Response(this.#directory, "writer", {
        validate: validateTableLiveV14WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV14Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV14RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV14Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV14ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV14Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV14RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v14-host-root-proofs-v1",
      ...
      const next = issueTableLiveV14Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV14ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV14Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV14ProbePayload(payload, ownership),
      });
      persistTableLiveV14TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v14 capture cell missing");
      const accepted = acceptTableLiveV14Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV14CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V14_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV14CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV14Objective(
    ...
    throw new Error(`Table live v14 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV14Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV14CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV14Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV14RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV14Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV14CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV14CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV14CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV14Authorization = (
  privateKey: KeyObject,
): TableLiveV14TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV14SimulationKey = (): KeyObject =>
