...
export const INPUT_LIVE_V73_VERIFIER_VERSION = "input-live-v73-verifier-v1";

export function buildInputLiveV73RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV73RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV73RawNode[];
}

export interface InputLiveV73NormalizedScene {
...
export interface InputLiveV73SceneProof {
...
export function normalizeInputLiveV73Scene(
  root: InputLiveV73RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV73NormalizedScene {
  const canonicalBindings: InputLiveV73NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV73RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV73Scene<Instance>(
  normalized: InputLiveV73NormalizedScene,
...
export function compileInputLiveV73ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV73PreCaptureGates(payload: unknown): void {
