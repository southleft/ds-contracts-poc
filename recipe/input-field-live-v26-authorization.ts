...
import {
  ...
  INPUT_LIVE_V26_INDEX_PATH,
} from "./build-input-field-live-proof-v26.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV26HistoryExpectation = "pending" | "authorized";

export interface InputLiveV26AuthorizationArtifact {
  artifactVersion: "input-live-v26-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V26_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV26AntecedentIndex {
...
export interface InputLiveV26HistoryState {
  index?: InputLiveV26AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV26AuthorizationArtifact;
...
export interface InputLiveV26AuthorizationProof extends InputLiveV26TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V26_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V26_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V26_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V26_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v26-status") ||
...
export function validateInputLiveV26AntecedentIndex(
  index: InputLiveV26AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v26-antecedent-index-v1" ||
...
export function validateInputLiveV26UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV26History\(\s*["']/.test(source) ||
    /readInputLiveV26HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV26AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV26AuthorizationArtifact {
  ...
  ) as InputLiveV26AntecedentIndex;
  const indexFailures = validateInputLiveV26AntecedentIndex(index);
...
export function validateInputLiveV26AuthorizationArtifact(
  artifact: InputLiveV26AuthorizationArtifact | undefined,
  index: InputLiveV26AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v26-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v26" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V26_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V26_TARGET) ||
    !equalJson(
...
export function validateInputLiveV26History(
  state: InputLiveV26HistoryState,
  expected: InputLiveV26HistoryExpectation,
): string[] {
  const failures = validateInputLiveV26AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV26AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV26HistoryState(
  root = process.cwd(),
): InputLiveV26HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V26_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV26AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V26_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V26_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V26_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV26History(
  expected: InputLiveV26HistoryExpectation,
  root = process.cwd(),
): InputLiveV26HistoryState {
  const state = readInputLiveV26HistoryState(root);
  const failures = validateInputLiveV26History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV26Authorization(
  root = process.cwd(),
): InputLiveV26AuthorizationProof {
  const state = verifyInputLiveV26History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V26_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v26/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v26.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V26_TARGET,
    expectedDynamicTool: INPUT_LIVE_V26_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V26_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV26HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV26History(expected);
