...
import { COMBOBOX_LIVE_V1_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v1.js";
import { verifyComboboxLiveV1Authorization } from "./combobox-live-v1-authorization.js";
import { runComboboxLiveV1Preflight } from "./combobox-live-v1-preflight.js";
import { ComboboxLiveV1Orchestrator } from "./run-combobox-live-v1.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v1 requires exact authorization artifact ${COMBOBOX_LIVE_V1_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v1-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV1Authorization(root);
    runComboboxLiveV1Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV1Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV1Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
