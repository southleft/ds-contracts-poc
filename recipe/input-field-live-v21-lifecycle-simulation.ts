...
export interface InputLiveV21LifecycleSimulationReport {
  artifactVersion: "input-live-v21-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV21AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV21Lifecycle(): Promise<InputLiveV21LifecycleSimulationReport> {
  await buildInputLiveV21Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V21_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV21AntecedentIndex;
  const indexSha256 = inputLiveV21Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v21-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V21_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V21_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV21History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV21History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV21AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V21_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V21_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV21History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV21History(authorized, "pending");
    ...
    await buildInputLiveV21Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V21_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV21AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV21Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV21Lifecycle(), null, 2)}\n`,
  );
