...
export type InputLiveV84HistoryExpectation = "pending" | "authorized";

export interface InputLiveV84AuthorizationArtifact {
  artifactVersion: "input-live-v84-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V84_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV84AntecedentIndex {
...
export interface InputLiveV84HistoryState {
...
export interface InputLiveV84AuthorizationProof extends InputLiveV84TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V84_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V84_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V84_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V84_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v84-authorization") ||
  artifactPath.includes("input-field-live-v84-preflight") ||
  artifactPath.includes("input-field-live-v84-authorized") ||
  artifactPath.includes("input-field-live-pivot-v84-status") ||
...
export function validateInputLiveV84AntecedentIndex(
  index: InputLiveV84AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v84-antecedent-index-v1" ||
...
export function validateInputLiveV84UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV84History\(\s*["']/.test(source) ||
    /readInputLiveV84HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV84AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV84AuthorizationArtifact {
  ...
  const indexFailures = validateInputLiveV84AntecedentIndex(index);
...
export function validateInputLiveV84AuthorizationArtifact(
  artifact: InputLiveV84AuthorizationArtifact | undefined,
  index: InputLiveV84AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v84-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v84" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V84_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V84_TARGET) ||
    !equalJson(
...
export function validateInputLiveV84History(
  state: InputLiveV84HistoryState,
  expected: InputLiveV84HistoryExpectation,
): string[] {
  const failures = validateInputLiveV84AntecedentIndex(state.index);
...
export function readInputLiveV84HistoryState(
  root = process.cwd(),
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V84_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV84AuthorizationArtifact)
    : undefined;
  ...
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V84_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V84_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV84History(
  expected: InputLiveV84HistoryExpectation,
  root = process.cwd(),
): InputLiveV84HistoryState {
  const state = readInputLiveV84HistoryState(root);
  const failures = validateInputLiveV84History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV84Authorization(
  root = process.cwd(),
): InputLiveV84AuthorizationProof {
  const state = verifyInputLiveV84History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V84_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v84/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v84.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V84_TARGET,
    expectedDynamicTool: INPUT_LIVE_V84_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V84_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV84HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV84History(expected);
