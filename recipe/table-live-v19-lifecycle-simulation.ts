...
import { tableLiveV2Sha256 } from "./table-live-v19-broker.js";

export interface TableLiveV19LifecycleSimulationReport {
  artifactVersion: "table-live-v19-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV19AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV19Lifecycle(): Promise<TableLiveV19LifecycleSimulationReport> {
  await buildTableLiveV19Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V19_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV19AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v19-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V19_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V19_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV19History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV19History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV19AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V19_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V19_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV19History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV19History(authorized, "pending");
    ...
    await buildTableLiveV19Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V19_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV19AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV19Lifecycle(), null, 2)}\n`,
  );
