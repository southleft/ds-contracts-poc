...
import {
  verifyTableLiveV29FixedSceneFixedPoint,
  type TableLiveV29FixedFixedPoint,
} from "./table-live-v29-fixed-point.js";

export const TABLE_LIVE_V29_VERIFIER_VERSION = "table-live-v29-verifier-v1";

export function buildTableLiveV29RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV29RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV29RawNode[];
}

export interface TableLiveV29NormalizedScene {
...
export interface TableLiveV29SceneProof {
...
export function normalizeTableLiveV29Scene(
  root: TableLiveV29RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV29NormalizedScene {
  const canonicalBindings: TableLiveV29NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV29RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV29Scene<Instance>(
  normalized: TableLiveV29NormalizedScene,
...
): TableLiveV29SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV29FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV29ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV29PreCaptureGates(payload: unknown): void {
