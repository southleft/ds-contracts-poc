...
export interface InputLiveV41LifecycleSimulationReport {
  artifactVersion: "input-live-v41-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV41AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV41Lifecycle(): Promise<InputLiveV41LifecycleSimulationReport> {
  await buildInputLiveV41Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V41_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV41AntecedentIndex;
  const indexSha256 = inputLiveV41Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v41-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V41_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V41_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV41History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV41History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV41AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V41_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V41_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV41History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV41History(authorized, "pending");
    ...
    await buildInputLiveV41Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V41_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV41AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV41Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV41Lifecycle(), null, 2)}\n`,
  );
