...
import { INPUT_LIVE_V22_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v22.js";
import { verifyInputLiveV22Authorization } from "./input-field-live-v22-authorization.js";
import { runInputLiveV22Preflight } from "./input-field-live-v22-preflight.js";
import { InputLiveV22Orchestrator } from "./run-input-field-live-v22.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V22_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v22-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV22Authorization(root);
    runInputLiveV22Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV22Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
