...
import { TABLE_LIVE_V17_AUTHORIZATION_PATH } from "./build-table-live-proof-v17.js";
import { verifyTableLiveV17Authorization } from "./table-live-v17-authorization.js";
import { runTableLiveV17Preflight } from "./table-live-v17-preflight.js";
import { TableLiveV17Orchestrator } from "./run-table-live-v17.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v17 requires exact authorization artifact ${TABLE_LIVE_V17_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v17-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV17Authorization(root);
    runTableLiveV17Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV17Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV17Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
