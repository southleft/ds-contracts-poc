...
import {
  verifyTableLiveV19FixedSceneFixedPoint,
  type TableLiveV19FixedFixedPoint,
} from "./table-live-v19-fixed-point.js";

export const TABLE_LIVE_V19_VERIFIER_VERSION = "table-live-v19-verifier-v1";

export function buildTableLiveV19RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV19RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV19RawNode[];
}

export interface TableLiveV19NormalizedScene {
...
export interface TableLiveV19SceneProof {
...
export function normalizeTableLiveV19Scene(
  root: TableLiveV19RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV19NormalizedScene {
  const canonicalBindings: TableLiveV19NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV19RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV19Scene<Instance>(
  normalized: TableLiveV19NormalizedScene,
...
): TableLiveV19SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV19FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV19ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV19PreCaptureGates(payload: unknown): void {
