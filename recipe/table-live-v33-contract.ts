...
import { buildTableLiveV33CleanupRuntime } from "./table-live-v33-cleanup.js";
import { type TableLiveV33FixedFixedPoint } from "./table-live-v33-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV33RestoreProgram,
  validateTableLiveV33RestorePayload,
  type TableLiveV33RestorePayload,
} from "./table-live-v33-restore.js";
import {
  buildTableLiveV33RawPropertyRuntime,
  normalizeTableLiveV33Scene,
  type TableLiveV33RawNode,
} from "./table-live-v33-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v33.js";
import {
...
export const TABLE_LIVE_V33_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V33_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V33_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V33_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V33_VARIANT_COUNT = 20;
export const TABLE_LIVE_V33_SET_COUNT = 6;
export const TABLE_LIVE_V33_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V33_HOST_PHASES = 3;
export const TABLE_LIVE_V33_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V33_RESTORE_COUNT = 8;
export const TABLE_LIVE_V33_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V33_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV33SourceId = (typeof TABLE_LIVE_V33_SOURCE_IDS)[number];
export type TableLiveV33Kind = "table" | "row" | "cell";

export interface TableLiveV33SourceIdentity {
  source: TableLiveV33SourceId;
...
export interface TableLiveV33WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV33ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV33SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV33RawNode;
    rowScene: TableLiveV33RawNode;
    cellScene: TableLiveV33RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV33RootProof {
  source: TableLiveV33SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV33FixedFixedPoint;
}

export interface TableLiveV33ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV33SourceId;
    adapterIdentity: string;
...
export interface TableLiveV33CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV33SourceId;
  adapterIdentity: string;
...
export interface TableLiveV33CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV33SourceId;
  frameWidth: number;
...
export interface TableLiveV33CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v33 ${label} must be an object`);
...
export function validateTableLiveV33WriterPayload(
  value: unknown,
): TableLiveV33WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V33_NAMESPACE ||
    ...
    throw new TypeError("Table live v33 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Table live v33 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v33 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV33SourceIdentity,
  kind: TableLiveV33Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV33ExtractProgram(
  writer: TableLiveV33WriterOwnership,
  sources: readonly TableLiveV33SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
      "Table live v33 extract requires two source identities",
    );
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V33_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V33_NAMESPACE)}
${buildTableLiveV33RawPropertyRuntime()}
...
export function validateTableLiveV33ExtractPayload(
  value: unknown,
  writer: TableLiveV33WriterOwnership,
): TableLiveV33ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v33 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V33_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v33 extract schema/two-root mismatch");
  return payload as TableLiveV33ExtractPayload;
}

export function proveTableLiveV33Roots<Instance>(
  extract: TableLiveV33ExtractPayload,
  sources: readonly (TableLiveV33SourceIdentity & {
...
): TableLiveV33RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v33 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v33 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV33Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV33Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV33Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV33FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Table live v33 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v33 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v33 compile lost owned sets");
...
export function assertTableLiveV33RootProofs(
  proofs: readonly TableLiveV33RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v33 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v33 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV33ProbeProgram(
  writer: TableLiveV33WriterOwnership,
  sources: readonly TableLiveV33SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V33_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV33ProbePayload(
  value: unknown,
  writer: TableLiveV33WriterOwnership,
): TableLiveV33ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV33ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V33_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v33 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV33CaptureManifest(
  cells: readonly TableLiveV33CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V33_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V33_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V33_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V33_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v33 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV33CaptureCell[],
): string => {
  validateTableLiveV33CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV33CaptureProgram(
  writer: TableLiveV33WriterOwnership,
  cell: TableLiveV33CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V33_CAPTURE_COUNT
  )
    throw new TypeError("Table live v33 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Table live v33 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V33_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV33CapturePayload(
  value: unknown,
  cell: TableLiveV33CaptureCell,
  rawResponseBytes: number,
): TableLiveV33CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV33CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V33_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V33_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v33 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV33CaptureResponses(
  manifest: readonly TableLiveV33CaptureCell[],
  responses: readonly TableLiveV33CapturePayload[],
): void {
  validateTableLiveV33CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v33 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV33ObjectiveReport {
  artifactVersion: "table-live-v33-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV33Objective(
  manifest: readonly TableLiveV33CaptureCell[],
  responses: readonly TableLiveV33CapturePayload[],
): TableLiveV33ObjectiveReport {
  assertTableLiveV33CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v33-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV33CleanupProgram(
  writer: TableLiveV33WriterOwnership,
): string {
  const baseProgram = buildTableLiveV33CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V33_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v33 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV33CleanupPayload(
  payload: unknown,
  writer: TableLiveV33WriterOwnership,
): TableLiveV33CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as TableLiveV33CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v33 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V33_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV33WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
