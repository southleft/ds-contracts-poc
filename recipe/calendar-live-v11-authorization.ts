...
export type CalendarLiveV11HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV11AuthorizationArtifact {
  artifactVersion: "calendar-live-v11-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V11_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV11AntecedentIndex {
...
export interface CalendarLiveV11HistoryState {
  index?: CalendarLiveV11AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV11AuthorizationArtifact;
...
export interface CalendarLiveV11AuthorizationProof
  extends CalendarLiveV11TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V11_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V11_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V11_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V11_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v11-authorization") ||
  artifactPath.includes("calendar-live-v11-preflight") ||
  artifactPath.includes("calendar-live-v11-authorized") ||
  artifactPath.includes("create-calendar-live-v11-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v11-status") ||
...
export function validateCalendarLiveV11AntecedentIndex(
  index: CalendarLiveV11AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v11-antecedent-index-v1" ||
...
export function validateCalendarLiveV11UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV11History\(\s*["']/.test(source) ||
    /readCalendarLiveV11HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV11AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV11AuthorizationArtifact {
  ...
  ) as CalendarLiveV11AntecedentIndex;
  const indexFailures = validateCalendarLiveV11AntecedentIndex(index);
...
export function validateCalendarLiveV11AuthorizationArtifact(
  artifact: CalendarLiveV11AuthorizationArtifact | undefined,
  index: CalendarLiveV11AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v11-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v11" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V11_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V11_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V11_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V11_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V11_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V11_SOURCE_ROOTS ||
...
export function validateCalendarLiveV11History(
  state: CalendarLiveV11HistoryState,
  expected: CalendarLiveV11HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV11AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV11AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV11HistoryState(
  root = process.cwd(),
): CalendarLiveV11HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V11_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV11AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V11_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V11_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V11_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV11History(
  expected: CalendarLiveV11HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV11HistoryState {
  const state = readCalendarLiveV11HistoryState(root);
  const failures = validateCalendarLiveV11History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v11 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV11Authorization(
  root = process.cwd(),
): CalendarLiveV11AuthorizationProof {
  const state = verifyCalendarLiveV11History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V11_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v11/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v11.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V11_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V11_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V11_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV11HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV11History(expected);
  process.stdout.write(
    `Calendar live v11 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
