...
import {
  validateTableLiveV31AntecedentIndex,
  verifyTableLiveV31Authorization,
  type TableLiveV31AntecedentIndex,
  type TableLiveV31AuthorizationProof,
} from "./table-live-v31-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V31_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v31/operator-security-attestation-template.json";
export const TABLE_LIVE_V31_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v31-security-attestation.json";

export interface TableLiveV31SecurityAttestation {
  artifactVersion: "table-live-v31-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV31PreflightReport {
  artifactVersion: "table-live-v31-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV31SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV31SecurityAttestation(
  value: unknown,
  proof: TableLiveV31AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v31-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V31_TARGET) ||
...
export function validateTableLiveV31ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV31Authorization");
  const preflight = source.indexOf("runTableLiveV31Preflight");
  const initialize = source.indexOf("TableLiveV31Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV31Preflight(
  root: string,
  proof: TableLiveV31AuthorizationProof,
  attestationPath = TABLE_LIVE_V31_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV31PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V31_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V31_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV31AntecedentIndex;
  failures.push(...validateTableLiveV31AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V31_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V31_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV31SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v31-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV31ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v31 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV31SecurityAttestation;
  return {
    artifactVersion: "table-live-v31-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V31_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV31Preflight(
    process.cwd(),
    verifyTableLiveV31Authorization(),
    attestationPath,
