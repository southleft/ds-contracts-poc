...
import { TABLE_LIVE_V2_INDEX_PATH } from "./build-table-live-proof-v2.js";
import {
...
import {
  buildTableLiveV2AuthorizationArtifact,
  validateTableLiveV2AntecedentIndex,
  validateTableLiveV2AuthorizationArtifact,
  validateTableLiveV2History,
  validateTableLiveV2UnitTestSource,
  type TableLiveV2AntecedentIndex,
  type TableLiveV2AuthorizationArtifact,
  type TableLiveV2AuthorizationProof,
  type TableLiveV2HistoryState,
} from "./table-live-v2-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV2ControlFlowSource,
  validateTableLiveV2SecurityAttestation,
  type TableLiveV2SecurityAttestation,
} from "./table-live-v2-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V2_INDEX_PATH);
const index = (): TableLiveV2AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV2AntecedentIndex;

const artifact = (): TableLiveV2AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV2AuthorizationArtifact({
...
const pendingState = (): TableLiveV2HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV2HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV2AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v2/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV2AuthorizationArtifact,
): TableLiveV2SecurityAttestation => {
  const body: Omit<TableLiveV2SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V2_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV2History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV2History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV2History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV2History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV2History(authorizedState(), "pending").join("\n"),
    /stale history mode.*authorization exists/,
  );
});

test("the exact v6 phase-sensitive self-test defect is planted and table v2 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV2UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v2-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV2UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV2AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV2AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV2AuthorizationArtifact(
        planted as TableLiveV2AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV2SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV2SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV2SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v2-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV2ControlFlowSource(source), []);
});
