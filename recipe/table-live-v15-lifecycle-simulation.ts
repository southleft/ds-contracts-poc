...
import { tableLiveV2Sha256 } from "./table-live-v15-broker.js";

export interface TableLiveV15LifecycleSimulationReport {
  artifactVersion: "table-live-v15-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV15AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV15Lifecycle(): Promise<TableLiveV15LifecycleSimulationReport> {
  await buildTableLiveV15Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V15_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV15AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v15-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V15_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V15_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV15History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV15History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV15AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V15_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V15_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV15History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV15History(authorized, "pending");
    ...
    await buildTableLiveV15Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V15_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV15AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV15Lifecycle(), null, 2)}\n`,
  );
