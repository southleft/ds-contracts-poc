...
export interface InputLiveV38LifecycleSimulationReport {
  artifactVersion: "input-live-v38-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV38AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV38Lifecycle(): Promise<InputLiveV38LifecycleSimulationReport> {
  await buildInputLiveV38Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V38_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV38AntecedentIndex;
  const indexSha256 = inputLiveV38Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v38-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V38_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V38_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV38History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV38History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV38AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V38_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V38_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV38History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV38History(authorized, "pending");
    ...
    await buildInputLiveV38Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V38_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV38AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV38Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV38Lifecycle(), null, 2)}\n`,
  );
