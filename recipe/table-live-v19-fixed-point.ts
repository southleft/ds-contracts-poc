/**
 * Table live v19 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V19_FIXED_VERIFIER_VERSION = "table-live-v19-fixed-point-v1";
export const TABLE_LIVE_V19_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV19FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV19FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV19FixedSourceProbe {
...
export interface TableLiveV19FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV19FixedFixedPoint;
}

export interface TableLiveV19FixedHardGateInput {
  thresholds: TableLiveV19FixedThresholds;
  sourceProbes: TableLiveV19FixedSourceProbe[];
  cells: TableLiveV19FixedCellValidation[];
  sceneProofs: TableLiveV19FixedSceneProof[];
...
export interface TableLiveV19FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V19_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV19FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV19FixedHardGates(
  input: TableLiveV19FixedHardGateInput,
): TableLiveV19FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV19FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V19_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV19FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV19FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
