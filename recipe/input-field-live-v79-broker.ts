...
import {
  ...
  INPUT_LIVE_V79_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V79_RESPONSE_CONTRACTS,
  type InputLiveV79RootProof,
} from "./input-field-live-v79-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V79_BROKER_VERSION = "input-live-v79-broker-v2";
export const INPUT_LIVE_V79_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V79_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V79_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V79_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V79_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V79_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV79RemotePhase =
  (typeof INPUT_LIVE_V79_REMOTE_PHASES)[number];
export type InputLiveV79AuthorizationMode = "live" | "simulated";

export interface InputLiveV79CommitPins {
...
export interface InputLiveV79TransactionAuthorization extends InputLiveV79CommitPins {
  mode: InputLiveV79AuthorizationMode;
...
export interface InputLiveV79Manifest {
  artifactVersion: "input-live-v79-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V79_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V79_DYNAMIC_TOOL;
...
export interface InputLiveV79ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV79RequestBody {
  artifactVersion: "input-live-v79-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V79_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV79RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV79CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V79_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV79ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV79Request extends InputLiveV79RequestBody {
...
export interface InputLiveV79Receipt {
  artifactVersion: "input-live-v79-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V79_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV79RemotePhase;
...
export interface InputLiveV79AcceptedResponse<T = unknown> {
  request: InputLiveV79Request;
  receipt: InputLiveV79Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV79IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV79ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V79_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V79_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V79_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V79_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V79_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V79_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V79_TARGET.editorType;
};
...
export const inputLiveV79RequestSequence = (
  phase: InputLiveV79RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV79RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV79RequestPath = (
  directory: string,
  phase: InputLiveV79RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV79ResponsePath = (
  directory: string,
  phase: InputLiveV79RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV79ReceiptPath = (
  directory: string,
  phase: InputLiveV79RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV79Manifest => {
  const manifest = json<InputLiveV79Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v79-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V79_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V79_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV79RemotePhase,
  captureIndex?: number,
): InputLiveV79Request | undefined => {
  const file = inputLiveV79RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV79Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV79RemotePhase,
  captureIndex?: number,
): InputLiveV79Receipt | undefined => {
  const file = inputLiveV79ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV79Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV79Request | undefined;
  receipt: InputLiveV79Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV79RemotePhase,
): InputLiveV79ResponseContract => ({
  ...INPUT_LIVE_V79_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V79_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v79-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V79_BROKER_VERSION,
  target: INPUT_LIVE_V79_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V79_PROGRAM_START*/${program}/*INPUT_LIVE_V79_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V79_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V79_PROGRAM_END*/";
...
export function createInputLiveV79Transaction(
  directory: string,
  ...
  const manifest: InputLiveV79Manifest = {
    artifactVersion: "input-live-v79-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V79_BROKER_VERSION,
...
export function validateInputLiveV79Request(
  directory: string,
  request: InputLiveV79Request,
): void {
  ...
  const expectedSequence = inputLiveV79RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v79-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V79_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V79_TARGET.fileKey ||
...
export function issueInputLiveV79Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV79RemotePhase,
  program: string,
  options: InputLiveV79IssueOptions = {},
): InputLiveV79Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV79RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV79Request>(targetPath);
    validateInputLiveV79Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV79ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV79TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV79RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V79_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V79_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V79_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV79RequestBody = {
    artifactVersion: "input-live-v79-broker-request-v2",
    brokerVersion: INPUT_LIVE_V79_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV79Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV79Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV79RawResponse(
  directory: string,
  phase: InputLiveV79RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV79RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV79Request>(requestFile);
  if (
  ...
  const target = inputLiveV79ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV79Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v79-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V79_BROKER_VERSION,
...
export function acceptInputLiveV79Response<T = unknown>(
  directory: string,
  phase: InputLiveV79RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV79AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV79ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV79Request>(
    inputLiveV79RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV79Request(directory, request);
  if (phase === "capture") validateInputLiveV79TechnicalGates(directory);
  const rawFile = inputLiveV79ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v79-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V79_BROKER_VERSION ||
...
export function persistInputLiveV79TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV79RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV79RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V79_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v79-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V79_BROKER_VERSION,
...
export function validateInputLiveV79TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v79-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V79_TECHNICAL_GATES.some(
...
export function validateInputLiveV79Receipt(
  directory: string,
  phase: InputLiveV79RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV79Request(directory, request);
  const receipt = json<InputLiveV79Receipt>(
    inputLiveV79ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV79ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v79-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V79_BROKER_VERSION ||
...
export function readInputLiveV79AcceptedRawPayload(
  directory: string,
  phase: InputLiveV79RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV79Receipt(directory, phase, captureIndex);
...
export function createInputLiveV79FakeDynamicResponse(
  request: InputLiveV79Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V79_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V79_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV79PersistedRequests(
  directory: string,
): InputLiveV79Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV79Request>(file)] : [];
    });
}

export const inputLiveV79Sha256 = sha256;
