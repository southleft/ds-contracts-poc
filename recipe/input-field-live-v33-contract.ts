...
} from "./input-field-live-v3-verifier-v33.js";
import {
  buildInputLiveV33RawPropertyRuntime,
  normalizeInputLiveV33Scene,
  type InputLiveV33RawNode,
} from "./input-field-live-v33-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v33.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v33.js";

export {
  buildInputLiveV33RestoreProgram,
  validateInputLiveV33RestorePayload,
  type InputLiveV33RestorePayload,
} from "./input-field-live-v33-restore.js";

export const INPUT_LIVE_V33_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V33_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V33_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V33_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V33_VARIANT_COUNT = 256;
export const INPUT_LIVE_V33_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V33_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV33SourceId = (typeof INPUT_LIVE_V33_SOURCE_IDS)[number];

export interface InputLiveV33SourceIdentity {
  source: InputLiveV33SourceId;
...
export interface InputLiveV33WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV33ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV33SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV33RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV33RootProof {
  source: InputLiveV33SourceId;
  adapterIdentity: string;
...
export interface InputLiveV33ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV33SourceId;
    adapterIdentity: string;
...
export interface InputLiveV33CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV33SourceId;
  adapterIdentity: string;
...
export interface InputLiveV33CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV33SourceId;
  strata: InputLiveV33CaptureCell["strata"];
...
export interface InputLiveV33CleanupPayload {
...
export function validateInputLiveV33WriterPayload(
  value: unknown,
): InputLiveV33WriterOwnership {
...
const identityPlan = (source: InputLiveV33SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV33ExtractProgram(
  writer: InputLiveV33WriterOwnership,
  sources: readonly InputLiveV33SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V33_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V33_NAMESPACE)}
${buildInputLiveV33RawPropertyRuntime()}
...
export function validateInputLiveV33ExtractPayload(
  value: unknown,
  writer: InputLiveV33WriterOwnership,
): InputLiveV33ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V33_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV33ExtractPayload;
}

export function proveInputLiveV33Roots<Instance>(
  extract: InputLiveV33ExtractPayload,
  sources: readonly (InputLiveV33SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV33Scene(
      root.scene,
...
export function assertInputLiveV33RootProofs(
  proofs: readonly InputLiveV33RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV33ProbeProgram(
  writer: InputLiveV33WriterOwnership,
  sources: readonly InputLiveV33SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V33_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV33ProbePayload(
  value: unknown,
  writer: InputLiveV33WriterOwnership,
): InputLiveV33ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV33ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V33_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V33_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV33CaptureManifest(
  cells: readonly InputLiveV33CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V33_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V33_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V33_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V33_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V33_SOURCE_IDS.some(
...
export const inputLiveV33CaptureManifestSha256 = (
  cells: readonly InputLiveV33CaptureCell[],
): string => {
  validateInputLiveV33CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV33CaptureProgram(
  writer: InputLiveV33WriterOwnership,
  cell: InputLiveV33CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V33_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V33_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V33_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV33CapturePayload(
  value: unknown,
  cell: InputLiveV33CaptureCell,
  rawResponseBytes: number,
): InputLiveV33CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV33CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V33_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V33_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV33CaptureResponses(
  manifest: readonly InputLiveV33CaptureCell[],
  responses: readonly InputLiveV33CapturePayload[],
): void {
  validateInputLiveV33CaptureManifest(manifest);
  if (
...
export interface InputLiveV33ObjectiveReport {
  artifactVersion: "input-live-v33-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV33Objective(
  manifest: readonly InputLiveV33CaptureCell[],
  responses: readonly InputLiveV33CapturePayload[],
  readReference: (cell: InputLiveV33CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV33CaptureCell) => Buffer,
): InputLiveV33ObjectiveReport {
  assertInputLiveV33CaptureResponses(manifest, responses);
...
export function buildInputLiveV33CleanupProgram(
  writer: InputLiveV33WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V33_NAMESPACE,
...
export function validateInputLiveV33CleanupPayload(
  value: unknown,
  writer: InputLiveV33WriterOwnership,
): InputLiveV33CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV33CleanupPayload;
...
export const INPUT_LIVE_V33_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV33WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
