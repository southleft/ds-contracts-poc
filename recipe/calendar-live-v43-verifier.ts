...
import {
  verifyCalendarLiveV43FixedSceneFixedPoint,
  type CalendarLiveV43FixedFixedPoint,
} from "./calendar-live-v43-fixed-point.js";

export const CALENDAR_LIVE_V43_VERIFIER_VERSION = "calendar-live-v43-verifier-v1";

export function buildCalendarLiveV43RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV43RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV43RawNode[];
}

export interface CalendarLiveV43NormalizedScene {
...
export interface CalendarLiveV43SceneProof {
...
export function normalizeCalendarLiveV43Scene(
  root: CalendarLiveV43RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV43NormalizedScene {
  const canonicalBindings: CalendarLiveV43NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV43RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV43Scene<Instance>(
  normalized: CalendarLiveV43NormalizedScene,
...
): CalendarLiveV43SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV43FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV43ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV43PreCaptureGates(payload: unknown): void {
