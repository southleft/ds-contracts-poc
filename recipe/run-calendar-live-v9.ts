...
import {
  acceptCalendarLiveV9Response,
  createCalendarLiveV9Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV9Request,
  persistCalendarLiveV9RawResponse,
  persistCalendarLiveV9TechnicalGates,
  readCalendarLiveV9AcceptedRawPayload,
  type CalendarLiveV9Request,
  type CalendarLiveV9TransactionAuthorization,
} from "./calendar-live-v9-broker.js";
import {
  CALENDAR_LIVE_V9_CAPTURE_COUNT,
  assertCalendarLiveV9CaptureResponses,
  ...
  evaluateCalendarLiveV9Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV9Roots,
  validateCalendarLiveV9CaptureManifest,
  validateCalendarLiveV9CapturePayload,
  validateCalendarLiveV9CleanupPayload,
  validateCalendarLiveV9ExtractPayload,
  validateCalendarLiveV9ProbePayload,
  validateCalendarLiveV9WriterPayload,
  type CalendarLiveV9CaptureCell,
  type CalendarLiveV9CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V9_PLAN_PATH,
  type CalendarLiveV9ProofPlan,
...
export const CALENDAR_LIVE_V9_RUNNER_VERSION = "calendar-live-v9-runner-v1";

interface CalendarLiveV9JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV9OrchestratorState {
  artifactVersion: "calendar-live-v9-orchestrator-state-v1";
  ...
  journal: CalendarLiveV9JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV9ProofPlan;
  planSha256: string;
  captures: CalendarLiveV9CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV9SourceIdentity & {
...
export interface CalendarLiveV9NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV9OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v9 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V9_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V9_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV9ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV9CaptureCell[];
  validateCalendarLiveV9CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v9 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v9 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV9WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV9RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV9Request =>
  readJson<CalendarLiveV9Request>(file);

export class CalendarLiveV9Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV9Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV9OrchestratorState = {
      artifactVersion: "calendar-live-v9-orchestrator-state-v1",
    ...
    return new CalendarLiveV9Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV9OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV9Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV9NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV9NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v9 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV9NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v9 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV9RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV9Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV9CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV9AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV9WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV9Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV9ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV9Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV9Response(this.#directory, "writer", {
        validate: validateCalendarLiveV9WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV9Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV9RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV9Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV9ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV9Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV9RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v9-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV9Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV9ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV9Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV9ProbePayload(payload, ownership),
      });
      persistCalendarLiveV9TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v9 capture cell missing");
      const accepted = acceptCalendarLiveV9Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV9CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V9_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV9CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV9Objective(
    ...
    throw new Error(`Calendar live v9 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV9Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV9CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV9Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV9RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV9Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV9CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV9CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV9CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV9Authorization = (
  privateKey: KeyObject,
): CalendarLiveV9TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV9SimulationKey = (): KeyObject =>
