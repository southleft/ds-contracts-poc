...
import {
  validateComboboxLiveV13AntecedentIndex,
  verifyComboboxLiveV13Authorization,
  type ComboboxLiveV13AntecedentIndex,
  type ComboboxLiveV13AuthorizationProof,
} from "./combobox-live-v13-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V13_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v13/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V13_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v13-security-attestation.json";

export interface ComboboxLiveV13SecurityAttestation {
  artifactVersion: "combobox-live-v13-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV13PreflightReport {
  artifactVersion: "combobox-live-v13-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV13SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV13SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV13AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v13-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V13_TARGET) ||
...
export function validateComboboxLiveV13ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV13Authorization");
  const preflight = source.indexOf("runComboboxLiveV13Preflight");
  const initialize = source.indexOf("ComboboxLiveV13Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV13Preflight(
  root: string,
  proof: ComboboxLiveV13AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V13_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV13PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V13_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V13_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V13_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV13AntecedentIndex;
  failures.push(...validateComboboxLiveV13AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V13_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V13_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV13SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v13-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV13ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v13 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV13SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v13-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V13_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV13Preflight(
    process.cwd(),
    verifyComboboxLiveV13Authorization(),
    attestationPath,
