...
import {
  validateComboboxLiveV39AntecedentIndex,
  verifyComboboxLiveV39Authorization,
  type ComboboxLiveV39AntecedentIndex,
  type ComboboxLiveV39AuthorizationProof,
} from "./combobox-live-v39-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V39_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v39/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V39_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v39-security-attestation.json";

export interface ComboboxLiveV39SecurityAttestation {
...
export interface ComboboxLiveV39PreflightReport {
  artifactVersion: "combobox-live-v39-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV39SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV39SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV39AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v39-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V39_TARGET) ||
...
export function validateComboboxLiveV39ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV39Authorization");
  const preflight = source.indexOf("runComboboxLiveV39Preflight");
  const initialize = source.indexOf("ComboboxLiveV39Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV39Preflight(
  root: string,
  proof: ComboboxLiveV39AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V39_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV39PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V39_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V39_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V39_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV39AntecedentIndex;
  failures.push(...validateComboboxLiveV39AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V39_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V39_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v39-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV39ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v39 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV39SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v39-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V39_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV39Preflight(
    process.cwd(),
    verifyComboboxLiveV39Authorization(),
    attestationPath,
