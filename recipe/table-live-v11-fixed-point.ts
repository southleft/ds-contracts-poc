/**
 * Table live v11 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V11_FIXED_VERIFIER_VERSION = "table-live-v11-fixed-point-v1";
export const TABLE_LIVE_V11_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV11FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV11FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV11FixedSourceProbe {
...
export interface TableLiveV11FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV11FixedFixedPoint;
}

export interface TableLiveV11FixedHardGateInput {
  thresholds: TableLiveV11FixedThresholds;
  sourceProbes: TableLiveV11FixedSourceProbe[];
  cells: TableLiveV11FixedCellValidation[];
  sceneProofs: TableLiveV11FixedSceneProof[];
...
export interface TableLiveV11FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V11_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV11FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV11FixedHardGates(
  input: TableLiveV11FixedHardGateInput,
): TableLiveV11FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV11FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  if (input.visualRows.length !== 0)
    failures.push("table live v11 must not invent a visual comparison");
  ...
  return {
    verifierVersion: TABLE_LIVE_V11_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV11FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV11FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
