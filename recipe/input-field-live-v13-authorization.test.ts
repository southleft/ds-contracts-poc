...
import { INPUT_LIVE_V13_INDEX_PATH } from "./build-input-field-live-proof-v13.js";
import {
  INPUT_LIVE_V13_DYNAMIC_TOOL,
  INPUT_LIVE_V13_TARGET,
} from "./input-field-live-v13-broker.js";
import {
  buildInputLiveV13AuthorizationArtifact,
  validateInputLiveV13AntecedentIndex,
  validateInputLiveV13AuthorizationArtifact,
  validateInputLiveV13History,
  validateInputLiveV13UnitTestSource,
  type InputLiveV13AntecedentIndex,
  type InputLiveV13AuthorizationArtifact,
  type InputLiveV13AuthorizationProof,
  type InputLiveV13HistoryState,
} from "./input-field-live-v13-authorization.js";
import {
  inputLiveV13AttestationSha256,
  validateInputLiveV13ControlFlowSource,
  validateInputLiveV13SecurityAttestation,
  type InputLiveV13SecurityAttestation,
} from "./input-field-live-v13-preflight.js";
import { inputLiveV13Sha256 } from "./input-field-live-v13-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V13_INDEX_PATH);
const index = (): InputLiveV13AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV13AntecedentIndex;

const artifact = (): InputLiveV13AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV13AuthorizationArtifact({
...
const pendingState = (): InputLiveV13HistoryState => ({
  index: index(),
  indexSha256: inputLiveV13Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV13HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV13AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV13Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V13_TARGET,
  expectedDynamicTool: INPUT_LIVE_V13_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v13/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV13AuthorizationArtifact,
): InputLiveV13SecurityAttestation => {
  const body: Omit<InputLiveV13SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v13-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V13_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV13AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV13History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV13History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV13History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV13History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV13History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV13UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV13UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV13AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV13Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV13AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV13AuthorizationArtifact(
        planted as InputLiveV13AuthorizationArtifact,
        index(),
        inputLiveV13Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV13SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV13AttestationSha256(
      body as Omit<InputLiveV13SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV13SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV13ControlFlowSource(source), []);
});
