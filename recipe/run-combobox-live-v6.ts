...
import {
  acceptComboboxLiveV6Response,
  createComboboxLiveV6Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV6Request,
  persistComboboxLiveV6RawResponse,
  persistComboboxLiveV6TechnicalGates,
  readComboboxLiveV6AcceptedRawPayload,
  type ComboboxLiveV6Request,
  type ComboboxLiveV6TransactionAuthorization,
} from "./combobox-live-v6-broker.js";
import {
  COMBOBOX_LIVE_V6_CAPTURE_COUNT,
  assertComboboxLiveV6CaptureResponses,
  ...
  evaluateComboboxLiveV6Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV6Roots,
  validateComboboxLiveV6CaptureManifest,
  validateComboboxLiveV6CapturePayload,
  validateComboboxLiveV6CleanupPayload,
  validateComboboxLiveV6ExtractPayload,
  validateComboboxLiveV6ProbePayload,
  validateComboboxLiveV6WriterPayload,
  type ComboboxLiveV6CaptureCell,
  type ComboboxLiveV6CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V6_PLAN_PATH,
  type ComboboxLiveV6ProofPlan,
} from "./build-combobox-live-proof-v6.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v6.js";

export const COMBOBOX_LIVE_V6_RUNNER_VERSION = "combobox-live-v6-runner-v1";

interface ComboboxLiveV6JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV6OrchestratorState {
  artifactVersion: "combobox-live-v6-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV6JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV6ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV6CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV6SourceIdentity & {
...
export interface ComboboxLiveV6NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV6OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v6 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V6_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V6_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV6ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV6CaptureCell[];
  validateComboboxLiveV6CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v6 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v6 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV6WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV6RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV6Request =>
  readJson<ComboboxLiveV6Request>(file);

export class ComboboxLiveV6Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV6Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV6OrchestratorState = {
      artifactVersion: "combobox-live-v6-orchestrator-state-v1",
    ...
    return new ComboboxLiveV6Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV6OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV6Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV6NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV6NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v6 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV6NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v6 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV6RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV6Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV6CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV6AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV6WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV6Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV6ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV6Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV6Response(this.#directory, "writer", {
        validate: validateComboboxLiveV6WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV6Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV6RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV6Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV6ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV6Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV6RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v6-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV6Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV6ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV6Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV6ProbePayload(payload, ownership),
      });
      persistComboboxLiveV6TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v6 capture cell missing");
      const accepted = acceptComboboxLiveV6Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV6CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V6_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV6CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV6Objective(
    ...
    throw new Error(`Combobox live v6 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV6Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV6CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV6Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV6RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV6Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV6CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV6CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV6CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV6Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV6TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV6SimulationKey = (): KeyObject =>
