...
import {
  validateCalendarLiveV19AntecedentIndex,
  verifyCalendarLiveV19Authorization,
  type CalendarLiveV19AntecedentIndex,
  type CalendarLiveV19AuthorizationProof,
} from "./calendar-live-v19-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V19_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v19/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V19_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v19-security-attestation.json";

export interface CalendarLiveV19SecurityAttestation {
  artifactVersion: "calendar-live-v19-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV19PreflightReport {
  artifactVersion: "calendar-live-v19-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV19SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV19SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV19AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v19-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V19_TARGET) ||
...
export function validateCalendarLiveV19ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV19Authorization");
  const preflight = source.indexOf("runCalendarLiveV19Preflight");
  const initialize = source.indexOf("CalendarLiveV19Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV19Preflight(
  root: string,
  proof: CalendarLiveV19AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V19_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV19PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V19_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V19_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV19AntecedentIndex;
  failures.push(...validateCalendarLiveV19AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V19_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V19_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV19SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v19-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV19ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v19 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV19SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v19-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V19_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV19Preflight(
    process.cwd(),
    verifyCalendarLiveV19Authorization(),
    attestationPath,
