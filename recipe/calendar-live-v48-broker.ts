...
import {
  assertCalendarLiveV48RootProofs,
  CALENDAR_LIVE_V48_CAPTURE_COUNT,
  CALENDAR_LIVE_V48_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  CALENDAR_LIVE_V48_RESPONSE_CONTRACTS,
  CALENDAR_LIVE_V48_SOURCE_ROOTS,
  type CalendarLiveV48RootProof,
} from "./calendar-live-v48-contract.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V48_BROKER_VERSION = "calendar-live-v48-broker-v1";
export const CALENDAR_LIVE_V48_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const CALENDAR_LIVE_V48_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const CALENDAR_LIVE_V48_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const CALENDAR_LIVE_V48_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const CALENDAR_LIVE_V48_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const CALENDAR_LIVE_V48_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type CalendarLiveV48RemotePhase =
  (typeof CALENDAR_LIVE_V48_REMOTE_PHASES)[number];
export type CalendarLiveV48AuthorizationMode = "live" | "simulated";

export interface CalendarLiveV48CommitPins {
...
export interface CalendarLiveV48TransactionAuthorization extends CalendarLiveV48CommitPins {
  mode: CalendarLiveV48AuthorizationMode;
...
export interface CalendarLiveV48Manifest {
  artifactVersion: "calendar-live-v48-broker-manifest-v2";
  brokerVersion: typeof CALENDAR_LIVE_V48_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof CALENDAR_LIVE_V48_DYNAMIC_TOOL;
  target: typeof CALENDAR_LIVE_V48_TARGET;
  authorization: CalendarLiveV48TransactionAuthorization;
...
export interface CalendarLiveV48ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface CalendarLiveV48RequestBody {
  artifactVersion: "calendar-live-v48-broker-request-v2";
  brokerVersion: typeof CALENDAR_LIVE_V48_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: CalendarLiveV48RemotePhase;
  sequence: number;
  ...
  commitPins: CalendarLiveV48CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof CALENDAR_LIVE_V48_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: CalendarLiveV48ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface CalendarLiveV48Request extends CalendarLiveV48RequestBody {
...
export interface CalendarLiveV48Receipt {
  artifactVersion: "calendar-live-v48-broker-receipt-v2";
  brokerVersion: typeof CALENDAR_LIVE_V48_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: CalendarLiveV48RemotePhase;
...
export interface CalendarLiveV48AcceptedResponse<T = unknown> {
  request: CalendarLiveV48Request;
  receipt: CalendarLiveV48Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface CalendarLiveV48IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: CalendarLiveV48ResponseContract;
...
const equalTarget = (value: unknown): value is typeof CALENDAR_LIVE_V48_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === CALENDAR_LIVE_V48_TARGET.fileKey &&
  (value as BrokerTarget).fileName === CALENDAR_LIVE_V48_TARGET.fileName &&
  (value as BrokerTarget).editorType === CALENDAR_LIVE_V48_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== CALENDAR_LIVE_V48_TARGET.fileKey ||
    context.fileName !== CALENDAR_LIVE_V48_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === CALENDAR_LIVE_V48_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Calendar live v48 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= CALENDAR_LIVE_V48_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v48 capture index must be 0..7");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Calendar live v48 authorization pins are malformed");
};

const readManifest = (directory: string): CalendarLiveV48Manifest => {
  const manifest = json<CalendarLiveV48Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Calendar live v48 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "calendar-live-v48-broker-manifest-v2" ||
    manifest.brokerVersion !== CALENDAR_LIVE_V48_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== CALENDAR_LIVE_V48_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(CALENDAR_LIVE_V48_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Calendar live v48 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: CalendarLiveV48Request,
): CalendarLiveV48RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: CalendarLiveV48RemotePhase,
  captureIndex?: number,
): CalendarLiveV48Request | undefined => {
  const file = calendarLiveV1RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV48Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: CalendarLiveV48RemotePhase,
  captureIndex?: number,
): CalendarLiveV48Receipt | undefined => {
  const file = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV48Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: CalendarLiveV48Request | undefined;
  receipt: CalendarLiveV48Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: CalendarLiveV48RemotePhase,
): CalendarLiveV48ResponseContract => ({
  ...CALENDAR_LIVE_V48_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: CalendarLiveV48Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: CALENDAR_LIVE_V48_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "calendar-live-v48-plugin-response-v2",
  brokerVersion: CALENDAR_LIVE_V48_BROKER_VERSION,
  target: CALENDAR_LIVE_V48_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*CALENDAR_LIVE_V48_PROGRAM_START*/${program}/*CALENDAR_LIVE_V48_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*CALENDAR_LIVE_V48_PROGRAM_START*/";
  const endMarker = "/*CALENDAR_LIVE_V48_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Calendar live v48 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createCalendarLiveV48Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Calendar live v48 private signing key does not match authorized public-key identity",
    );
  const manifest: CalendarLiveV48Manifest = {
    artifactVersion: "calendar-live-v48-broker-manifest-v2",
    brokerVersion: CALENDAR_LIVE_V48_BROKER_VERSION,
    ...
    captureRequests: CALENDAR_LIVE_V48_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateCalendarLiveV48Request(
  directory: string,
  request: CalendarLiveV48Request,
): void {
  ...
  if (
    request.artifactVersion !== "calendar-live-v48-broker-request-v2" ||
    request.brokerVersion !== CALENDAR_LIVE_V48_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== CALENDAR_LIVE_V48_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Calendar live v48 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueCalendarLiveV48Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: CalendarLiveV48RemotePhase,
  program: string,
  options: CalendarLiveV48IssueOptions = {},
): CalendarLiveV48Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<CalendarLiveV48Request>(targetPath);
    validateCalendarLiveV48Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Calendar live v48 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(calendarLiveV1ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Calendar live v48 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Calendar live v48 empty code envelope refused");
  if (phase === "capture") validateCalendarLiveV48TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Calendar live v48 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: CALENDAR_LIVE_V48_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? CALENDAR_LIVE_V48_SIGNED_WRITER_TIMEOUT_MS
        : CALENDAR_LIVE_V48_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: CalendarLiveV48RequestBody = {
    artifactVersion: "calendar-live-v48-broker-request-v2",
    brokerVersion: CALENDAR_LIVE_V48_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: CalendarLiveV48Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateCalendarLiveV48Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistCalendarLiveV48RawResponse(
  directory: string,
  phase: CalendarLiveV48RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Calendar live v48 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<CalendarLiveV48Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Calendar live v48 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = calendarLiveV1ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Calendar live v48 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): CalendarLiveV48Receipt => {
  const body: Omit<CalendarLiveV48Receipt, "receiptSha256"> = {
    artifactVersion: "calendar-live-v48-broker-receipt-v2",
    brokerVersion: CALENDAR_LIVE_V48_BROKER_VERSION,
...
export function acceptCalendarLiveV48Response<T = unknown>(
  directory: string,
  phase: CalendarLiveV48RemotePhase,
  options: {
    captureIndex?: number;
...
): CalendarLiveV48AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Calendar live v48 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<CalendarLiveV48Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Calendar live v48 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "calendar-live-v48-plugin-response-v2" ||
    plugin?.brokerVersion !== CALENDAR_LIVE_V48_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Calendar live v48 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistCalendarLiveV48TechnicalGates(
  directory: string,
  rootProofs: readonly CalendarLiveV48RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertCalendarLiveV48RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Calendar live v48 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Calendar live v48 probe payload hash malformed");
  const gates = Object.fromEntries(
    CALENDAR_LIVE_V48_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "calendar-live-v48-host-technical-gates-v2",
    brokerVersion: CALENDAR_LIVE_V48_BROKER_VERSION,
...
export function validateCalendarLiveV48TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "calendar-live-v48-host-technical-gates-v2" ||
    ...
    artifact.roots.length !== CALENDAR_LIVE_V48_SOURCE_ROOTS ||
    ...
    CALENDAR_LIVE_V48_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Calendar live v48 technical gates artifact");
}

export function validateCalendarLiveV48Receipt(
  directory: string,
  phase: CalendarLiveV48RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateCalendarLiveV48Request(directory, request);
  const receipt = json<CalendarLiveV48Receipt>(
  ...
  if (
    receipt.artifactVersion !== "calendar-live-v48-broker-receipt-v2" ||
    receipt.brokerVersion !== CALENDAR_LIVE_V48_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Calendar live v48 accepted receipt: ${request.requestId}`,
    );
}

export function readCalendarLiveV48AcceptedRawPayload(
  directory: string,
  phase: CalendarLiveV48RemotePhase,
  captureIndex?: number,
): unknown {
  validateCalendarLiveV48Receipt(directory, phase, captureIndex);
...
export function createCalendarLiveV48FakeDynamicResponse(
  request: CalendarLiveV48Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? CALENDAR_LIVE_V48_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? CALENDAR_LIVE_V48_TARGET,
    ...overrides.extra,
  };
}

export function listCalendarLiveV48PersistedRequests(
  directory: string,
): CalendarLiveV48Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<CalendarLiveV48Request>(file)] : [];
    });
}
