...
import { buildComboboxLiveV42CleanupRuntime } from "./combobox-live-v42-cleanup.js";
import {
  type ComboboxLiveV42FixedFixedPoint,
} from "./combobox-live-v42-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV42RestoreProgram,
  validateComboboxLiveV42RestorePayload,
  type ComboboxLiveV42RestorePayload,
} from "./combobox-live-v42-restore.js";
import {
  buildComboboxLiveV42RawPropertyRuntime,
  normalizeComboboxLiveV42Scene,
  type ComboboxLiveV42RawNode,
} from "./combobox-live-v42-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v42.js";
import {
...
} from "./scene-readback-combobox-v42.js";
...
export const COMBOBOX_LIVE_V42_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V42_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V42_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V42_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V42_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V42_SET_COUNT = 4;
export const COMBOBOX_LIVE_V42_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V42_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V42_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V42_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V42_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV42SourceId = (typeof COMBOBOX_LIVE_V42_SOURCE_IDS)[number];

export interface ComboboxLiveV42SourceIdentity {
  source: ComboboxLiveV42SourceId;
...
export interface ComboboxLiveV42WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV42ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV42SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV42RawNode;
    optionScene: ComboboxLiveV42RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV42RootProof {
  source: ComboboxLiveV42SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV42FixedFixedPoint;
}

export interface ComboboxLiveV42ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV42SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV42CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV42SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV42CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV42SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV42CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v42 ${label} must be an object`);
...
export function validateComboboxLiveV42WriterPayload(
  value: unknown,
): ComboboxLiveV42WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V42_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v42 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v42 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v42 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV42SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV42ExtractProgram(
  writer: ComboboxLiveV42WriterOwnership,
  sources: readonly ComboboxLiveV42SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v42 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V42_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V42_NAMESPACE)}
${buildComboboxLiveV42RawPropertyRuntime()}
...
export function validateComboboxLiveV42ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV42WriterOwnership,
): ComboboxLiveV42ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v42 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V42_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v42 extract schema/two-root mismatch");
  return payload as ComboboxLiveV42ExtractPayload;
}

export function proveComboboxLiveV42Roots<Instance>(
  extract: ComboboxLiveV42ExtractPayload,
  sources: readonly (ComboboxLiveV42SourceIdentity & {
...
): ComboboxLiveV42RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v42 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v42 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV42Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV42Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV42FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v42 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v42 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v42 compile lost owned sets");
...
export function assertComboboxLiveV42RootProofs(
  proofs: readonly ComboboxLiveV42RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v42 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v42 independent root accounting failed: ${failures.join(";")}`,
    );
}
...
export function buildComboboxLiveV42ProbeProgram(
  writer: ComboboxLiveV42WriterOwnership,
  sources: readonly ComboboxLiveV42SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V42_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV42ProbePayload(
  value: unknown,
  writer: ComboboxLiveV42WriterOwnership,
): ComboboxLiveV42ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV42ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V42_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V42_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v42 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV42CaptureManifest(
  cells: readonly ComboboxLiveV42CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V42_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V42_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V42_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V42_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v42 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV42CaptureCell[],
): string => {
  validateComboboxLiveV42CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV42CaptureProgram(
  writer: ComboboxLiveV42WriterOwnership,
  cell: ComboboxLiveV42CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V42_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v42 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v42 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V42_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV42CapturePayload(
  value: unknown,
  cell: ComboboxLiveV42CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV42CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV42CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V42_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V42_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v42 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV42CaptureResponses(
  manifest: readonly ComboboxLiveV42CaptureCell[],
  responses: readonly ComboboxLiveV42CapturePayload[],
): void {
  validateComboboxLiveV42CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v42 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV42ObjectiveReport {
  artifactVersion: "combobox-live-v42-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV42Objective(
  manifest: readonly ComboboxLiveV42CaptureCell[],
  responses: readonly ComboboxLiveV42CapturePayload[],
): ComboboxLiveV42ObjectiveReport {
  assertComboboxLiveV42CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v42-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV42CleanupProgram(
  writer: ComboboxLiveV42WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV42CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V42_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v42 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV42CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV42WriterOwnership,
): ComboboxLiveV42CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV42CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v42 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V42_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV42WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
