...
import { buildCalendarLiveV16CleanupRuntime } from "./calendar-live-v16-cleanup.js";
import { type CalendarLiveV16FixedFixedPoint } from "./calendar-live-v16-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV16RestoreProgram,
  validateCalendarLiveV16RestorePayload,
  type CalendarLiveV16RestorePayload,
} from "./calendar-live-v16-restore.js";
import {
  buildCalendarLiveV16RawPropertyRuntime,
  normalizeCalendarLiveV16Scene,
  type CalendarLiveV16RawNode,
} from "./calendar-live-v16-verifier.js";
...
export const CALENDAR_LIVE_V16_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V16_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V16_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V16_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V16_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V16_SET_COUNT = 3;
export const CALENDAR_LIVE_V16_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V16_HOST_PHASES = 3;
export const CALENDAR_LIVE_V16_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V16_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V16_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V16_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV16SourceId = (typeof CALENDAR_LIVE_V16_SOURCE_IDS)[number];
export type CalendarLiveV16Kind = "calendar" | "week" | "day";

export interface CalendarLiveV16SourceIdentity {
  source: CalendarLiveV16SourceId;
...
export interface CalendarLiveV16WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV16ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV16SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV16RawNode;
    weekScene: CalendarLiveV16RawNode;
    dayScene: CalendarLiveV16RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV16RootProof {
  source: CalendarLiveV16SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV16FixedFixedPoint;
}

export interface CalendarLiveV16ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV16SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV16CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV16SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV16CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV16SourceId;
  frameWidth: number;
...
export interface CalendarLiveV16CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v16 ${label} must be an object`);
...
export function validateCalendarLiveV16WriterPayload(
  value: unknown,
): CalendarLiveV16WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v16") ||
    ...
    result.namespace !== CALENDAR_LIVE_V16_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v16 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v16 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v16 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV16SourceIdentity,
  kind: CalendarLiveV16Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV16ExtractProgram(
  writer: CalendarLiveV16WriterOwnership,
  sources: readonly CalendarLiveV16SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v16 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V16_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V16_NAMESPACE)}
${buildCalendarLiveV16RawPropertyRuntime()}
...
export function validateCalendarLiveV16ExtractPayload(
  value: unknown,
  writer: CalendarLiveV16WriterOwnership,
): CalendarLiveV16ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v16 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V16_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v16 extract schema/one-root mismatch");
  return payload as CalendarLiveV16ExtractPayload;
}

export function proveCalendarLiveV16Roots<Instance>(
  extract: CalendarLiveV16ExtractPayload,
  sources: readonly (CalendarLiveV16SourceIdentity & {
...
): CalendarLiveV16RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v16 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v16 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV16Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV16Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV16Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV16FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v16 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v16 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v16 compile lost owned sets");
...
export function assertCalendarLiveV16RootProofs(
  proofs: readonly CalendarLiveV16RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v16 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v16 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV16ProbeProgram(
  writer: CalendarLiveV16WriterOwnership,
  sources: readonly CalendarLiveV16SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V16_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV16ProbePayload(
  value: unknown,
  writer: CalendarLiveV16WriterOwnership,
): CalendarLiveV16ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV16ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V16_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V16_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v16 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV16CaptureManifest(
  cells: readonly CalendarLiveV16CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V16_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V16_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V16_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V16_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v16 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV16CaptureCell[],
): string => {
  validateCalendarLiveV16CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV16CaptureProgram(
  writer: CalendarLiveV16WriterOwnership,
  cell: CalendarLiveV16CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V16_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v16 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v16 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V16_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV16CapturePayload(
  value: unknown,
  cell: CalendarLiveV16CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV16CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV16CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V16_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V16_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v16 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV16CaptureResponses(
  manifest: readonly CalendarLiveV16CaptureCell[],
  responses: readonly CalendarLiveV16CapturePayload[],
): void {
  validateCalendarLiveV16CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v16 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV16ObjectiveReport {
  artifactVersion: "calendar-live-v16-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV16Objective(
  manifest: readonly CalendarLiveV16CaptureCell[],
  responses: readonly CalendarLiveV16CapturePayload[],
): CalendarLiveV16ObjectiveReport {
  assertCalendarLiveV16CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v16-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV16CleanupProgram(
  writer: CalendarLiveV16WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV16CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V16_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v16 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV16CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV16WriterOwnership,
): CalendarLiveV16CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV16CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v16 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V16_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV16WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
