/**
 * Table live v1 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V1_FIXED_VERIFIER_VERSION = "table-live-v1-fixed-point-v1";
export const TABLE_LIVE_V1_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV1FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV1FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV1FixedSourceProbe {
...
export interface TableLiveV1FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV1FixedFixedPoint;
}

export interface TableLiveV1FixedHardGateInput {
  thresholds: TableLiveV1FixedThresholds;
  sourceProbes: TableLiveV1FixedSourceProbe[];
  cells: TableLiveV1FixedCellValidation[];
  sceneProofs: TableLiveV1FixedSceneProof[];
...
export interface TableLiveV1FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V1_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV1FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV1FixedHardGates(
  input: TableLiveV1FixedHardGateInput,
): TableLiveV1FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV1FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  if (input.visualRows.length !== 0)
    failures.push("table live v1 must not invent a visual comparison");
  ...
  return {
    verifierVersion: TABLE_LIVE_V1_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV1FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV1FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
