...
import {
  ...
  FORBIDDEN_CALENDAR_V29_RUN_IDENTITY,
  ...
  FORBIDDEN_CALENDAR_V36_RUN_IDENTITY,
  FORBIDDEN_CALENDAR_V37_RUN_IDENTITY,
  FORBIDDEN_CALENDAR_V38_RUN_IDENTITY,
  FORBIDDEN_CALENDAR_V39_RUN_IDENTITY,
  FORBIDDEN_CALENDAR_V40_RUN_IDENTITY,
  FORBIDDEN_CALENDAR_V41_RUN_IDENTITY,
  FORBIDDEN_CALENDAR_V42_RUN_IDENTITY,
  FORBIDDEN_CALENDAR_V43_RUN_IDENTITY,
  FORBIDDEN_CALENDAR_V44_RUN_IDENTITY,
  FORBIDDEN_CALENDAR_V45_RUN_IDENTITY,
  FORBIDDEN_CALENDAR_V46_RUN_IDENTITY,
  FORBIDDEN_CALENDAR_V47_RUN_IDENTITY,
  FORBIDDEN_CALENDAR_V48_RUN_IDENTITY,
  FORBIDDEN_CALENDAR_V49_RUN_IDENTITY,
  CALENDAR_FIGMA_NAMESPACE,
} from "./calendar-figma-writer.js";
import { buildCalendarLiveV50CleanupRuntime } from "./calendar-live-v50-cleanup.js";
import { type CalendarLiveV50FixedFixedPoint } from "./calendar-live-v50-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV50RestoreProgram,
  validateCalendarLiveV50RestorePayload,
  type CalendarLiveV50RestorePayload,
} from "./calendar-live-v50-restore.js";
import {
  buildCalendarLiveV50RawPropertyRuntime,
  normalizeCalendarLiveV50Scene,
  type CalendarLiveV50RawNode,
} from "./calendar-live-v50-verifier.js";
...
export const CALENDAR_LIVE_V50_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V50_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V50_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V50_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V50_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V50_SET_COUNT = 3;
export const CALENDAR_LIVE_V50_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V50_HOST_PHASES = 3;
export const CALENDAR_LIVE_V50_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V50_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V50_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V50_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV50SourceId = (typeof CALENDAR_LIVE_V50_SOURCE_IDS)[number];
export type CalendarLiveV50Kind = "calendar" | "week" | "day";

export interface CalendarLiveV50SourceIdentity {
  source: CalendarLiveV50SourceId;
...
export interface CalendarLiveV50WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV50ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV50SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV50RawNode;
    weekScene: CalendarLiveV50RawNode;
    dayScene: CalendarLiveV50RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV50RootProof {
  source: CalendarLiveV50SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV50FixedFixedPoint;
}

export interface CalendarLiveV50ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV50SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV50CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV50SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV50CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV50SourceId;
  frameWidth: number;
...
export interface CalendarLiveV50CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v50 ${label} must be an object`);
...
export function validateCalendarLiveV50WriterPayload(
  value: unknown,
): CalendarLiveV50WriterOwnership {
  ...
  if (
    ...
    result.runIdentity === FORBIDDEN_CALENDAR_V8_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V9_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V10_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V11_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V12_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V29_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V30_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V31_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V32_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V33_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V34_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V35_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V36_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V37_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V38_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V39_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V40_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V41_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V42_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V43_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V44_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V45_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V46_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V47_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V48_RUN_IDENTITY ||
    result.runIdentity === FORBIDDEN_CALENDAR_V49_RUN_IDENTITY ||
    !String(result.runIdentity).endsWith("-calendar-v50") ||
    result.namespace === FORBIDDEN_TABLE_NAMESPACE ||
    result.pageId === FORBIDDEN_TABLE_PAGE_ID ||
    result.pageId === FORBIDDEN_BUTTON_PAGE_ID ||
    result.pageId === FORBIDDEN_CALENDAR_V30_PAGE_ID ||
    result.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    result.namespace === FORBIDDEN_COMBOBOX_NAMESPACE ||
    result.namespace !== CALENDAR_LIVE_V50_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v50 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v50 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v50 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV50SourceIdentity,
  kind: CalendarLiveV50Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV50ExtractProgram(
  writer: CalendarLiveV50WriterOwnership,
  sources: readonly CalendarLiveV50SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v50 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V50_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V50_NAMESPACE)}
${buildCalendarLiveV50RawPropertyRuntime()}
...
export function validateCalendarLiveV50ExtractPayload(
  value: unknown,
  writer: CalendarLiveV50WriterOwnership,
): CalendarLiveV50ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v50 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V50_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v50 extract schema/one-root mismatch");
  return payload as CalendarLiveV50ExtractPayload;
}

export function proveCalendarLiveV50Roots<Instance>(
  extract: CalendarLiveV50ExtractPayload,
  sources: readonly (CalendarLiveV50SourceIdentity & {
...
): CalendarLiveV50RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v50 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v50 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV50Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV50Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV50Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV50FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v50 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v50 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v50 compile lost owned sets");
...
export function assertCalendarLiveV50RootProofs(
  proofs: readonly CalendarLiveV50RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v50 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v50 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV50ProbeProgram(
  writer: CalendarLiveV50WriterOwnership,
  sources: readonly CalendarLiveV50SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V50_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
for(const row of byAdapter.values()){
 ...
 void "CALENDAR-PROBE-HUG-ROOT-REFLOW-IS-CONTENT-WIDTH";
 instance.resizeWithoutConstraints(beforeWidth+64,instance.height);const reflowPassed=instance.layoutSizingHorizontal==="HUG"?instance.width===beforeWidth:instance.width===beforeWidth+64;void "CALENDAR-PROBE-CAPTION-FILL-IS-SOURCE-NAMED";const measureContentHug=node=>{if(!node||node.type!=="TEXT")return false;const hidden=node.visible===false;if(hidden)node.visible=true;const sized=node.layoutSizingHorizontal==="HUG"||node.layoutSizingHorizontal==="FILL";if(hidden)node.visible=false;return sized;};const contentHugPassed=!!measureContentHug(label);instance.resizeWithoutConstraints(beforeWidth,instance.height);
 ...
 void "CALENDAR-PROBE-DAY-LABEL-VIA-CHARACTERS";
 const labelKey=propertyKey(dayInstance,"Label");let textPropertiesRestored=false;if(labelKey){const dayLabels=nodes(dayInstance).filter(node=>node.type==="TEXT"&&role(node)==="calendar/day/label");const beforeChars=dayLabels.map(node=>node.characters);for(const text of dayLabels){if(text.fontName!==figma.mixed)await figma.loadFontAsync(text.fontName);text.characters="Calendar v1 deterministic probe";}const changed=nodes(dayInstance).some(node=>node.type==="TEXT"&&node.characters==="Calendar v1 deterministic probe");for(const [i,text] of dayLabels.entries()){if(text.fontName!==figma.mixed)await figma.loadFontAsync(text.fontName);text.characters=beforeChars[i];}textPropertiesRestored=changed&&JSON.stringify(original)===JSON.stringify(plain(instance));}
 dayInstance.remove();
...
export function validateCalendarLiveV50ProbePayload(
  value: unknown,
  writer: CalendarLiveV50WriterOwnership,
): CalendarLiveV50ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV50ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V50_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V50_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v50 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV50CaptureManifest(
  cells: readonly CalendarLiveV50CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V50_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V50_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V50_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V50_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v50 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV50CaptureCell[],
): string => {
  validateCalendarLiveV50CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV50CaptureProgram(
  writer: CalendarLiveV50WriterOwnership,
  cell: CalendarLiveV50CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V50_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v50 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v50 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V50_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV50CapturePayload(
  value: unknown,
  cell: CalendarLiveV50CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV50CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV50CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V50_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V50_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v50 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV50CaptureResponses(
  manifest: readonly CalendarLiveV50CaptureCell[],
  responses: readonly CalendarLiveV50CapturePayload[],
): void {
  validateCalendarLiveV50CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v50 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV50ObjectiveReport {
  artifactVersion: "calendar-live-v50-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV50Objective(
  manifest: readonly CalendarLiveV50CaptureCell[],
  responses: readonly CalendarLiveV50CapturePayload[],
): CalendarLiveV50ObjectiveReport {
  assertCalendarLiveV50CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v50-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV50CleanupProgram(
  writer: CalendarLiveV50WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV50CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V50_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v50 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV50CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV50WriterOwnership,
): CalendarLiveV50CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV50CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v50 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V50_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV50WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
