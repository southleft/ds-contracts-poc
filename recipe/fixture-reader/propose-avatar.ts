/**
 * PROPOSE AN avatar@1 FIXTURE FROM A CAPTURE LEDGER — same contract as the
 ...
 * name. The initials are the label part's text.
 */
...
import { AVATAR_SPELLINGS, avatarSchemaMappings, type AvatarRoles } from "./schema-avatar.js";
...
export interface ProposeAvatarInput {
  library: string;
  ledger: string;
  roles: AvatarRoles;
  combo: string;
...
export function proposeAvatarFixture(input: ProposeAvatarInput): { proposal: AvatarProposal; modulePath: string; proposalPath: string; refused: string[] } {
  ...
  const mappings = avatarSchemaMappings(roles, { combo });
  const evaluated = evaluate(ledger, mappings, sets, AVATAR_SPELLINGS);
  ...
  const proposalPath = path.join(REPO, "recipe/fixture-reader/out/proposals", `avatar.${library}.json`);
  if (evaluated.refused.length > 0) {
    return { proposal: null as unknown as AvatarProposal, modulePath: path.join(REPO, input.out), proposalPath, refused: evaluated.refused };
  }
  ...
  const initials = (labelPart?.text ?? []).map((t) => t.trim()).find((t) => t.length > 0);
  if (!initials) {
    return { proposal: null as unknown as AvatarProposal, modulePath: path.join(REPO, input.out), proposalPath, refused: [`content.label: the label part ${labelSel} carries no text in ${key} — avatar@1 needs initials`] };
  }
  ...
  const proposal: AvatarProposal = {
    ...
    combo,
    ...evaluated,
    content: { label: initials },
    typography: { family, style, stack },
  };
  const module = renderAvatarModule(proposal, {
    slug: library,
    displayName: input.displayName ?? library,
    exportName: input.exportName ?? "Avatar",
 ...
 */
function typographyBlock(p: AvatarProposal, style: string): string {
...
export function renderAvatarModule(p: AvatarProposal, opts: { slug: string; displayName: string; exportName: string; sourceRoot: string; unsupported: string[] }): string {
  ...
  return `// GENERATED by recipe/fixture-reader/propose-avatar.ts — do not edit by hand.
// Source of every value: the capture ledger ${p.ledger} (${p.provenance.package}@${p.provenance.version},
// ${p.provenance.browser}) read through the avatar@1 role schema; every leaf
...
export const ${slug.toUpperCase()}_AVATAR_LEDGER = ${q(p.ledger)};
export const ${slug}AvatarCombo = ${q(p.combo)};

export const ${slug}AvatarRoles: AvatarRoles = ${JSON.stringify(p.roles, null, 2)};

const VALUES: Record<string, number | string> = {
${tokenLines}
};

const ${slug}Tokens = cloneTokens(${q(`${opts.slug}.avatar`)}, (path) => {
...
export const ${slug}AvatarSource: ReviewedAvatarSource = {
  ...
  anatomy: {
    ...
    label: ${q(`ledger part ${p.roles.label ?? p.roles.box}: ${p.typography.family} ${style} ${p.leaves["labelFontSize"]?.value}px / ${p.leaves["labelLineHeight"]?.value}px; initials ${JSON.stringify(p.content.label)}`)},
  },
...
export const ${slug}AvatarAdapterConfig = buildConfig(
  ${q(opts.slug)},
  ${slug}AvatarSource,
  ${slug}Tokens,
  { id: ${q(`${opts.slug}.avatar`)}, name: ${q(`${opts.displayName} Avatar`)} },
  ${slug}Refusals,
  anatomyFacts(${q(opts.slug)}, ${slug}AvatarSource),
  ${q(opts.unsupported)},
  ${q(p.content)},
);

/** The same schema the proposer evaluated, for the drift gate. */
export const ${slug}AvatarMappings = avatarSchemaMappings(${slug}AvatarRoles, { combo: ${slug}AvatarCombo, receipts: ${q(Object.fromEntries(p.receipts.map((r) => [r.path, { why: r.why, evidence: r.evidence }])))} });
`;
}
