...
test("the thirteen archetypes cover astryx/mui/antd (39 reviewed subjects) plus the proposed chakra and shadcn checkboxes and the proposed MUI, shadcn and Chakra switches and the five proposed avatars three proposed tooltips, four proposed chips, two proposed links, two proposed tabs and the five Chakra held-outs and the AntD radio proposal (66) with a non-zero denominator", () => {
  assert.equal(results.length, 66);
  const proposed = results.filter((r) => r.library === "chakra");
  assert.equal(proposed.length, 7, "seven Chakra subjects, all proposed (checkbox, switch, avatar, chip, link, tooltip, radio)");
