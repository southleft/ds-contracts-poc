...
test("the thirteen archetypes cover astryx/mui/antd (39 reviewed subjects) plus the proposed chakra and shadcn checkboxes and the proposed MUI, shadcn and Chakra switches and the five proposed avatars three proposed tooltips, four proposed chips, two proposed links, two proposed tabs and the six Chakra held-outs and the radio/textarea proposals (69) with a non-zero denominator", () => {
  assert.equal(results.length, 69);
  const proposed = results.filter((r) => r.library === "chakra");
  assert.equal(proposed.length, 8, "eight Chakra subjects, all proposed (checkbox, switch, avatar, chip, link, tooltip, radio, textarea)");
