...
test("the thirteen archetypes cover astryx/mui/antd (39 reviewed subjects) plus the proposed chakra and shadcn checkboxes and the proposed MUI, shadcn and Chakra switches and the five proposed avatars three proposed tooltips, four proposed chips, two proposed links, two proposed tabs and the Chakra held-outs and the radio/textarea/alert/badge/menu/dialog proposals (79) with a non-zero denominator", () => {
  assert.equal(results.length, 79);
  const proposed = results.filter((r) => r.library === "chakra" || r.library === "chakra-field");
  assert.equal(proposed.length, 12, "twelve Chakra subjects, all proposed (checkbox, switch, avatar, chip, link, tooltip, radio, textarea, textarea-field, alert, menu, dialog)");
