...
test("the thirteen archetypes cover astryx/mui/antd (39 reviewed subjects) plus the proposed chakra and shadcn checkboxes and the proposed MUI and shadcn switches (43) with a non-zero denominator", () => {
  assert.equal(results.length, 43);
  const proposed = results.filter((r) => r.library === "chakra");
  assert.equal(proposed.length, 1, "exactly one proposed subject so far (checkbox/chakra)");
