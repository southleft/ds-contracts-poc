...
export const ${opts.slug.toUpperCase()}_SWITCH_LEDGER = ${q(p.ledger)};

export const ${opts.slug}SwitchRoles: SwitchRoles = ${JSON.stringify(p.roles, null, 2)};
export const ${opts.slug}SwitchCombos: SwitchComboMap = ${JSON.stringify(p.combos, null, 2)};

const VALUES: Record<string, number | string> = {
${tokenLines}
};

const ${opts.slug}Tokens = cloneTokens(${q(`${opts.slug}.switch`)}, (path) => {
...
export const ${opts.slug}SwitchSource: ReviewedSwitchSource = {
...
export const ${opts.slug}SwitchAdapterConfig = buildConfig(
  ${q(opts.slug)},
  ${opts.slug}SwitchSource,
  ${opts.slug}Tokens,
  { id: ${q(`${opts.slug}.switch`)}, name: ${q(`${opts.displayName} Switch`)} },
  ${opts.slug}Refusals,
  anatomyFacts(${q(opts.slug)}, ${opts.slug}SwitchSource),
  ${q(opts.unsupported)},
  ${q(p.content)},
);

/** The same schema the proposer evaluated, for the drift gate. */
export const ${opts.slug}SwitchMappings = switchSchemaMappings(${opts.slug}SwitchRoles, { combos: ${opts.slug}SwitchCombos, receipts: ${q(Object.fromEntries(p.receipts.map((r) => [r.path, { why: r.why, evidence: r.evidence }])))} });
`;
}
