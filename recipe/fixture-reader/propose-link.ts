/**
 * PROPOSE AN link@1 FIXTURE FROM A CAPTURE LEDGER — same contract as the
...
import { LINK_SPELLINGS, linkSchemaMappings, type LinkRoles } from "./schema-link.js";
...
export function proposeLinkFixture(input: ProposeLinkInput): { proposal: LinkProposal; modulePath: string; proposalPath: string; refused: string[] } {
  ...
  const mappings = linkSchemaMappings(roles, { combo });
  const evaluated = evaluate(ledger, mappings, sets, LINK_SPELLINGS);
  ...
  const proposalPath = path.join(REPO, "recipe/fixture-reader/out/proposals", `link.${library}.json`);
  if (evaluated.refused.length > 0) {
    return { proposal: null as unknown as LinkProposal, modulePath: path.join(REPO, input.out), proposalPath, refused: evaluated.refused };
  }
  ...
  if (!text) {
    return { proposal: null as unknown as LinkProposal, modulePath: path.join(REPO, input.out), proposalPath, refused: [`content.label: the label part ${labelSel} carries no text in ${key} — link@1 needs text`] };
  }
  ...
  const module = renderLinkModule(proposal, {
    slug: library,
    displayName: input.displayName ?? library,
    exportName: input.exportName ?? "Link",
 ...
 */
function typographyBlock(p: LinkProposal, style: string): string {
...
export function renderLinkModule(p: LinkProposal, opts: { slug: string; displayName: string; exportName: string; sourceRoot: string; unsupported: string[] }): string {
  ...
  return `// GENERATED by recipe/fixture-reader/propose-link.ts — do not edit by hand.
// Source of every value: the capture ledger ${p.ledger} (${p.provenance.package}@${p.provenance.version},
// ${p.provenance.browser}) read through the link@1 role schema; every leaf
...
export const ${slug.toUpperCase()}_LINK_LEDGER = ${q(p.ledger)};
export const ${slug}LinkCombo = ${q(p.combo)};

export const ${slug}LinkRoles: LinkRoles = ${JSON.stringify(p.roles, null, 2)};

const VALUES: Record<string, number | string> = {
${tokenLines}
};

const ${slug}Tokens = cloneTokens(${q(`${opts.slug}.link`)}, (path) => {
...
export const ${slug}LinkSource: ReviewedLinkSource = {
...
export const ${slug}LinkAdapterConfig = buildConfig(
  ${q(opts.slug)},
  ${slug}LinkSource,
  ${slug}Tokens,
  { id: ${q(`${opts.slug}.link`)}, name: ${q(`${opts.displayName} Link`)} },
  ${slug}Refusals,
  anatomyFacts(${q(opts.slug)}, ${slug}LinkSource),
  ${q(opts.unsupported)},
  ${q(p.content)},
);

/** The same schema the proposer evaluated, for the drift gate. */
export const ${slug}LinkMappings = linkSchemaMappings(${slug}LinkRoles, { combo: ${slug}LinkCombo, receipts: ${q(Object.fromEntries(p.receipts.map((r) => [r.path, { why: r.why, evidence: r.evidence }])))} });
`;
}
