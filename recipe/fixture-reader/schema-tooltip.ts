/**
 * tooltip@1 READER SCHEMA — the archetype's fixture leaves as ledger reads over
 * ONE role, the tip box (the painted part carrying the text). The height is
...
export interface TooltipSchemaOptions {
  combo: string;
...
export const TOOLTIP_SPELLINGS: Record<string, Spelling> = {
  "box.height": 0,
  strokeAlign: "inside",
};
...
export function tooltipSchemaMappings(roles: TooltipRoles, opts: TooltipSchemaOptions): FactMapping[] {
  ...
  return [
    receipt("box.height", "the tip hugs its text; its captured height is the sample's, not a recipe fact — recipe spelling 0 (hug)", "reviewed 0"),
