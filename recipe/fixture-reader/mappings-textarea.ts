...
export const muiTextareaMappings: FactMapping[] = [
  ...
  one("labelLineHeight", "px", { combo: mBase, part: mLabel, channel: "line-height" }),
  one("labelLineHeightUnit", "string", { combo: mBase, part: mLabel, channel: "line-height" }, { formula: "a px length → px; normal → auto", combine: (raw) => (/^-?\d/.test(raw.v) ? "px" : "auto") }),
...
export const astryxTextareaMappings: FactMapping[] = [
  ...
  one("labelLineHeight", "px", { combo: xBase, part: xLabel, channel: "line-height" }, { formula: "sub-pixel leading rounded (20.0004 → 20)", combine: (raw) => Math.round(px(raw.v)) }),
  one("labelLineHeightUnit", "string", { combo: xBase, part: xLabel, channel: "line-height" }, { formula: "a px length → px; normal → auto", combine: (raw) => (/^-?\d/.test(raw.v) ? "px" : "auto") }),
