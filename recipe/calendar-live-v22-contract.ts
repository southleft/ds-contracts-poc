...
import { buildCalendarLiveV22CleanupRuntime } from "./calendar-live-v22-cleanup.js";
import { type CalendarLiveV22FixedFixedPoint } from "./calendar-live-v22-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV22RestoreProgram,
  validateCalendarLiveV22RestorePayload,
  type CalendarLiveV22RestorePayload,
} from "./calendar-live-v22-restore.js";
import {
  buildCalendarLiveV22RawPropertyRuntime,
  normalizeCalendarLiveV22Scene,
  type CalendarLiveV22RawNode,
} from "./calendar-live-v22-verifier.js";
...
export const CALENDAR_LIVE_V22_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V22_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V22_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V22_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V22_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V22_SET_COUNT = 3;
export const CALENDAR_LIVE_V22_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V22_HOST_PHASES = 3;
export const CALENDAR_LIVE_V22_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V22_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V22_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V22_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV22SourceId = (typeof CALENDAR_LIVE_V22_SOURCE_IDS)[number];
export type CalendarLiveV22Kind = "calendar" | "week" | "day";

export interface CalendarLiveV22SourceIdentity {
  source: CalendarLiveV22SourceId;
...
export interface CalendarLiveV22WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV22ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV22SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV22RawNode;
    weekScene: CalendarLiveV22RawNode;
    dayScene: CalendarLiveV22RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV22RootProof {
  source: CalendarLiveV22SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV22FixedFixedPoint;
}

export interface CalendarLiveV22ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV22SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV22CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV22SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV22CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV22SourceId;
  frameWidth: number;
...
export interface CalendarLiveV22CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v22 ${label} must be an object`);
...
export function validateCalendarLiveV22WriterPayload(
  value: unknown,
): CalendarLiveV22WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v22") ||
    ...
    result.namespace !== CALENDAR_LIVE_V22_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v22 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v22 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v22 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV22SourceIdentity,
  kind: CalendarLiveV22Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV22ExtractProgram(
  writer: CalendarLiveV22WriterOwnership,
  sources: readonly CalendarLiveV22SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v22 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V22_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V22_NAMESPACE)}
${buildCalendarLiveV22RawPropertyRuntime()}
...
export function validateCalendarLiveV22ExtractPayload(
  value: unknown,
  writer: CalendarLiveV22WriterOwnership,
): CalendarLiveV22ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v22 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V22_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v22 extract schema/one-root mismatch");
  return payload as CalendarLiveV22ExtractPayload;
}

export function proveCalendarLiveV22Roots<Instance>(
  extract: CalendarLiveV22ExtractPayload,
  sources: readonly (CalendarLiveV22SourceIdentity & {
...
): CalendarLiveV22RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v22 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v22 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV22Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV22Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV22Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV22FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v22 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v22 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v22 compile lost owned sets");
...
export function assertCalendarLiveV22RootProofs(
  proofs: readonly CalendarLiveV22RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v22 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v22 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV22ProbeProgram(
  writer: CalendarLiveV22WriterOwnership,
  sources: readonly CalendarLiveV22SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V22_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV22ProbePayload(
  value: unknown,
  writer: CalendarLiveV22WriterOwnership,
): CalendarLiveV22ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV22ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V22_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V22_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v22 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV22CaptureManifest(
  cells: readonly CalendarLiveV22CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V22_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V22_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V22_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V22_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v22 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV22CaptureCell[],
): string => {
  validateCalendarLiveV22CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV22CaptureProgram(
  writer: CalendarLiveV22WriterOwnership,
  cell: CalendarLiveV22CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V22_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v22 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v22 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V22_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV22CapturePayload(
  value: unknown,
  cell: CalendarLiveV22CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV22CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV22CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V22_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V22_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v22 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV22CaptureResponses(
  manifest: readonly CalendarLiveV22CaptureCell[],
  responses: readonly CalendarLiveV22CapturePayload[],
): void {
  validateCalendarLiveV22CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v22 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV22ObjectiveReport {
  artifactVersion: "calendar-live-v22-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV22Objective(
  manifest: readonly CalendarLiveV22CaptureCell[],
  responses: readonly CalendarLiveV22CapturePayload[],
): CalendarLiveV22ObjectiveReport {
  assertCalendarLiveV22CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v22-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV22CleanupProgram(
  writer: CalendarLiveV22WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV22CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V22_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v22 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV22CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV22WriterOwnership,
): CalendarLiveV22CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV22CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v22 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V22_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV22WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
