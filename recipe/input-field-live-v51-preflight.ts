...
import {
  ...
  verifyInputLiveV51Authorization,
  type InputLiveV51AntecedentIndex,
  type InputLiveV51AuthorizationProof,
} from "./input-field-live-v51-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V51_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v51/operator-security-attestation-template.json";
export const INPUT_LIVE_V51_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v51-security-attestation.json";

export interface InputLiveV51SecurityAttestation {
...
export interface InputLiveV51PreflightReport {
  artifactVersion: "input-live-v51-preflight-v1";
...
export const inputLiveV51AttestationSha256 = (
  value: Omit<InputLiveV51SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV51SecurityAttestation(
  value: unknown,
  proof: InputLiveV51AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v51-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V51_TARGET) ||
...
export function validateInputLiveV51ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV51Authorization");
  const preflight = source.indexOf("runInputLiveV51Preflight");
  const initialize = source.indexOf("InputLiveV51Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV51Preflight(
  root: string,
  proof: InputLiveV51AuthorizationProof,
  attestationPath = INPUT_LIVE_V51_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV51PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V51_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V51_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V51_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV51AntecedentIndex;
  failures.push(...validateInputLiveV51AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V51_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V51_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v51-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV51ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV51SecurityAttestation;
  return {
    artifactVersion: "input-live-v51-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V51_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV51Preflight(
    process.cwd(),
    verifyInputLiveV51Authorization(),
    attestationPath,
