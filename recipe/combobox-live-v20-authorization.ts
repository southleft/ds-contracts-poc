...
export type ComboboxLiveV20HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV20AuthorizationArtifact {
  artifactVersion: "combobox-live-v20-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V20_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV20AntecedentIndex {
...
export interface ComboboxLiveV20HistoryState {
  index?: ComboboxLiveV20AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV20AuthorizationArtifact;
...
export interface ComboboxLiveV20AuthorizationProof extends ComboboxLiveV20TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V20_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V20_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V20_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V20_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v20-authorization") ||
  artifactPath.includes("combobox-live-v20-preflight") ||
  artifactPath.includes("combobox-live-v20-authorized") ||
  artifactPath.includes("create-combobox-live-v20-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v20-status") ||
...
export function validateComboboxLiveV20AntecedentIndex(
  index: ComboboxLiveV20AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v20-antecedent-index-v1" ||
...
export function validateComboboxLiveV20UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV20History\(\s*["']/.test(source) ||
    /readComboboxLiveV20HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV20AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV20AuthorizationArtifact {
  ...
  ) as ComboboxLiveV20AntecedentIndex;
  const indexFailures = validateComboboxLiveV20AntecedentIndex(index);
...
export function validateComboboxLiveV20AuthorizationArtifact(
  artifact: ComboboxLiveV20AuthorizationArtifact | undefined,
  index: ComboboxLiveV20AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v20-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v20" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V20_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V20_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V20_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V20_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V20_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V20_SOURCE_ROOTS ||
  ...
  if (
    ...
    failures.push(
      "v20 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
...
export function validateComboboxLiveV20History(
  state: ComboboxLiveV20HistoryState,
  expected: ComboboxLiveV20HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV20AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV20AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV20HistoryState(
  root = process.cwd(),
): ComboboxLiveV20HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V20_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV20AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V20_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V20_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V20_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV20History(
  expected: ComboboxLiveV20HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV20HistoryState {
  const state = readComboboxLiveV20HistoryState(root);
  const failures = validateComboboxLiveV20History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v20 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV20Authorization(
  root = process.cwd(),
): ComboboxLiveV20AuthorizationProof {
  const state = verifyComboboxLiveV20History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V20_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v20/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v20.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V20_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V20_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V20_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV20HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV20History(expected);
  process.stdout.write(
    `Combobox live v20 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
