...
import {
  verifyCalendarLiveV49FixedSceneFixedPoint,
  type CalendarLiveV49FixedFixedPoint,
} from "./calendar-live-v49-fixed-point.js";

export const CALENDAR_LIVE_V49_VERIFIER_VERSION = "calendar-live-v49-verifier-v1";

export function buildCalendarLiveV49RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV49RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV49RawNode[];
}

export interface CalendarLiveV49NormalizedScene {
...
export interface CalendarLiveV49SceneProof {
...
export function normalizeCalendarLiveV49Scene(
  root: CalendarLiveV49RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV49NormalizedScene {
  const canonicalBindings: CalendarLiveV49NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV49RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV49Scene<Instance>(
  normalized: CalendarLiveV49NormalizedScene,
...
): CalendarLiveV49SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV49FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV49ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV49PreCaptureGates(payload: unknown): void {
