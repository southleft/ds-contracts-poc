...
import {
  ...
  INPUT_LIVE_V54_INDEX_PATH,
} from "./build-input-field-live-proof-v54.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV54HistoryExpectation = "pending" | "authorized";

export interface InputLiveV54AuthorizationArtifact {
  artifactVersion: "input-live-v54-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V54_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV54AntecedentIndex {
...
export interface InputLiveV54HistoryState {
  index?: InputLiveV54AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV54AuthorizationArtifact;
...
export interface InputLiveV54AuthorizationProof extends InputLiveV54TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V54_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V54_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V54_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V54_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v54-authorization") ||
  artifactPath.includes("input-field-live-v54-preflight") ||
  artifactPath.includes("input-field-live-v54-authorized") ||
  artifactPath.includes("input-field-live-pivot-v54-status") ||
...
export function validateInputLiveV54AntecedentIndex(
  index: InputLiveV54AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v54-antecedent-index-v1" ||
...
export function validateInputLiveV54UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV54History\(\s*["']/.test(source) ||
    /readInputLiveV54HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV54AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV54AuthorizationArtifact {
  ...
  ) as InputLiveV54AntecedentIndex;
  const indexFailures = validateInputLiveV54AntecedentIndex(index);
...
export function validateInputLiveV54AuthorizationArtifact(
  artifact: InputLiveV54AuthorizationArtifact | undefined,
  index: InputLiveV54AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v54-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v54" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V54_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V54_TARGET) ||
    !equalJson(
...
export function validateInputLiveV54History(
  state: InputLiveV54HistoryState,
  expected: InputLiveV54HistoryExpectation,
): string[] {
  const failures = validateInputLiveV54AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV54AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV54HistoryState(
  root = process.cwd(),
): InputLiveV54HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V54_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV54AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V54_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V54_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V54_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV54History(
  expected: InputLiveV54HistoryExpectation,
  root = process.cwd(),
): InputLiveV54HistoryState {
  const state = readInputLiveV54HistoryState(root);
  const failures = validateInputLiveV54History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV54Authorization(
  root = process.cwd(),
): InputLiveV54AuthorizationProof {
  const state = verifyInputLiveV54History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V54_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v54/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v54.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V54_TARGET,
    expectedDynamicTool: INPUT_LIVE_V54_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V54_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV54HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV54History(expected);
