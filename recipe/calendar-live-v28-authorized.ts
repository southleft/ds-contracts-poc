...
import { CALENDAR_LIVE_V28_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v28.js";
import { verifyCalendarLiveV28Authorization } from "./calendar-live-v28-authorization.js";
import { runCalendarLiveV28Preflight } from "./calendar-live-v28-preflight.js";
import { CalendarLiveV28Orchestrator } from "./run-calendar-live-v28.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v28 requires exact authorization artifact ${CALENDAR_LIVE_V28_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v28-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV28Authorization(root);
    runCalendarLiveV28Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV28Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV28Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
