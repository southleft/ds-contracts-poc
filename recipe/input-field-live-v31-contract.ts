...
} from "./input-field-live-v3-verifier-v31.js";
import {
  buildInputLiveV31RawPropertyRuntime,
  normalizeInputLiveV31Scene,
  type InputLiveV31RawNode,
} from "./input-field-live-v31-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v31.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v31.js";
...
export const INPUT_LIVE_V31_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V31_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V31_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V31_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V31_VARIANT_COUNT = 256;
export const INPUT_LIVE_V31_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V31_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV31SourceId = (typeof INPUT_LIVE_V31_SOURCE_IDS)[number];

export interface InputLiveV31SourceIdentity {
  source: InputLiveV31SourceId;
...
export interface InputLiveV31WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV31ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV31SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV31RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV31RootProof {
  source: InputLiveV31SourceId;
  adapterIdentity: string;
...
export interface InputLiveV31ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV31SourceId;
    adapterIdentity: string;
...
export interface InputLiveV31CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV31SourceId;
  adapterIdentity: string;
...
export interface InputLiveV31CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV31SourceId;
  strata: InputLiveV31CaptureCell["strata"];
...
export interface InputLiveV31CleanupPayload {
...
export function validateInputLiveV31WriterPayload(
  value: unknown,
): InputLiveV31WriterOwnership {
...
const identityPlan = (source: InputLiveV31SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV31ExtractProgram(
  writer: InputLiveV31WriterOwnership,
  sources: readonly InputLiveV31SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V31_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V31_NAMESPACE)}
${buildInputLiveV31RawPropertyRuntime()}
...
export function validateInputLiveV31ExtractPayload(
  value: unknown,
  writer: InputLiveV31WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V31_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV31ExtractPayload;
}

export function proveInputLiveV31Roots<Instance>(
  extract: InputLiveV31ExtractPayload,
  sources: readonly (InputLiveV31SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV31Scene(
      root.scene,
...
export function assertInputLiveV31RootProofs(
  proofs: readonly InputLiveV31RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV31ProbeProgram(
  writer: InputLiveV31WriterOwnership,
  sources: readonly InputLiveV31SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V31_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV31ProbePayload(
  value: unknown,
  writer: InputLiveV31WriterOwnership,
): InputLiveV31ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV31ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V31_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V31_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV31CaptureManifest(
  cells: readonly InputLiveV31CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V31_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V31_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V31_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V31_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V31_SOURCE_IDS.some(
...
export const inputLiveV31CaptureManifestSha256 = (
  cells: readonly InputLiveV31CaptureCell[],
): string => {
  validateInputLiveV31CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV31CaptureProgram(
  writer: InputLiveV31WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V31_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V31_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V31_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV31CapturePayload(
  value: unknown,
  cell: InputLiveV31CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV31CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V31_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V31_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV31CaptureResponses(
  manifest: readonly InputLiveV31CaptureCell[],
  responses: readonly InputLiveV31CapturePayload[],
): void {
  validateInputLiveV31CaptureManifest(manifest);
  if (
...
export interface InputLiveV31ObjectiveReport {
  artifactVersion: "input-live-v31-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV31Objective(
  manifest: readonly InputLiveV31CaptureCell[],
  responses: readonly InputLiveV31CapturePayload[],
  readReference: (cell: InputLiveV31CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV31CaptureCell) => Buffer,
): InputLiveV31ObjectiveReport {
  assertInputLiveV31CaptureResponses(manifest, responses);
...
export function buildInputLiveV31CleanupProgram(
  writer: InputLiveV31WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V31_NAMESPACE,
...
export function validateInputLiveV31CleanupPayload(
  value: unknown,
  writer: InputLiveV31WriterOwnership,
): InputLiveV31CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV31CleanupPayload;
...
export const INPUT_LIVE_V31_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV31WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
