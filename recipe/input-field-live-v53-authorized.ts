...
import { INPUT_LIVE_V53_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v53.js";
import { verifyInputLiveV53Authorization } from "./input-field-live-v53-authorization.js";
import { runInputLiveV53Preflight } from "./input-field-live-v53-preflight.js";
import { InputLiveV53Orchestrator } from "./run-input-field-live-v53.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V53_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v53-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV53Authorization(root);
    runInputLiveV53Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV53Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
