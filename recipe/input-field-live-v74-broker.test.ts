...
import {
  ...
  createInputLiveV74FakeDynamicResponse,
  ...
  listInputLiveV74PersistedRequests,
  persistInputLiveV74RawResponse,
  persistInputLiveV74TechnicalGates,
  validateInputLiveV74Receipt,
  validateInputLiveV74Request,
  type InputLiveV74RemotePhase,
...
import {
  ...
  type InputLiveV74ExtractPayload,
  type InputLiveV74ProbePayload,
  type InputLiveV74RestorePayload,
  type InputLiveV74RootProof,
  type InputLiveV74WriterOwnership,
} from "./input-field-live-v74-contract.js";
import {
  InputLiveV74Orchestrator,
  simulatedInputLiveV74Authorization,
} from "./run-input-field-live-v74.js";
...
const captures: InputLiveV74CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV74Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v74-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV74Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV74CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV74FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV74FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V74_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V74_TARGET);
  persistInputLiveV74RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV74Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV74Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV74WriterOwnership,
): InputLiveV74RestorePayload => ({
...
const rootProofs = (): InputLiveV74RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV74CaptureCell,
): InputLiveV74CapturePayload => {
  const bytes = Buffer.from(`input-live-v74-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV74Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV74Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV74RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV74RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV74ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV74ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV74CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV74CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV74CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV74Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV74CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV74PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV74ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV74Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV74Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV74Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV74Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V74_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV74RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V74_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV74Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV74Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV74ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV74Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV74WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV74RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV74Request;
    validateInputLiveV74Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV74Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV74CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v74-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV74Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV74Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV74Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V74_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV74RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V74_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV74Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV74Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV74RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V74_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV74Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV74Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV74Request;
