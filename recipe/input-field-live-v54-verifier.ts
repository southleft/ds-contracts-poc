...
export const INPUT_LIVE_V54_VERIFIER_VERSION = "input-live-v54-verifier-v1";

export function buildInputLiveV54RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV54RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV54RawNode[];
}

export interface InputLiveV54NormalizedScene {
...
export interface InputLiveV54SceneProof {
...
export function normalizeInputLiveV54Scene(
  root: InputLiveV54RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV54NormalizedScene {
  const canonicalBindings: InputLiveV54NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV54RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV54Scene<Instance>(
  normalized: InputLiveV54NormalizedScene,
...
export function compileInputLiveV54ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV54PreCaptureGates(payload: unknown): void {
