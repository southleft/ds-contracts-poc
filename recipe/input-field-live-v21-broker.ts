...
import {
  assertInputLiveV21RootProofs,
  INPUT_LIVE_V21_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V21_RESPONSE_CONTRACTS,
  type InputLiveV21RootProof,
} from "./input-field-live-v21-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V21_BROKER_VERSION = "input-live-v21-broker-v2";
export const INPUT_LIVE_V21_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V21_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V21_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V21_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V21_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V21_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV21RemotePhase =
  (typeof INPUT_LIVE_V21_REMOTE_PHASES)[number];
export type InputLiveV21AuthorizationMode = "live" | "simulated";

export interface InputLiveV21CommitPins {
...
export interface InputLiveV21TransactionAuthorization extends InputLiveV21CommitPins {
  mode: InputLiveV21AuthorizationMode;
...
export interface InputLiveV21Manifest {
  artifactVersion: "input-live-v21-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V21_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V21_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V21_TARGET;
  authorization: InputLiveV21TransactionAuthorization;
...
export interface InputLiveV21ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV21RequestBody {
  artifactVersion: "input-live-v21-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V21_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV21RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV21CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V21_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV21ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV21Request extends InputLiveV21RequestBody {
...
export interface InputLiveV21Receipt {
  artifactVersion: "input-live-v21-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V21_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV21RemotePhase;
...
export interface InputLiveV21AcceptedResponse<T = unknown> {
  request: InputLiveV21Request;
  receipt: InputLiveV21Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV21IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV21ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V21_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V21_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V21_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V21_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V21_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V21_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V21_TARGET.editorType;
};
...
export const inputLiveV21RequestSequence = (
  phase: InputLiveV21RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV21RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV21RequestPath = (
  directory: string,
  phase: InputLiveV21RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV21ResponsePath = (
  directory: string,
  phase: InputLiveV21RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV21ReceiptPath = (
  directory: string,
  phase: InputLiveV21RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV21Manifest => {
  const manifest = json<InputLiveV21Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v21-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V21_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V21_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV21RemotePhase,
  captureIndex?: number,
): InputLiveV21Request | undefined => {
  const file = inputLiveV21RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV21Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV21RemotePhase,
  captureIndex?: number,
): InputLiveV21Receipt | undefined => {
  const file = inputLiveV21ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV21Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV21Request | undefined;
  receipt: InputLiveV21Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV21RemotePhase,
): InputLiveV21ResponseContract => ({
  ...INPUT_LIVE_V21_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V21_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V21_DYNAMIC_TOOL,
      target: INPUT_LIVE_V21_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v21-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V21_BROKER_VERSION,
  target: INPUT_LIVE_V21_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V21_PROGRAM_START*/${program}/*INPUT_LIVE_V21_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V21_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V21_PROGRAM_END*/";
...
export function createInputLiveV21Transaction(
  directory: string,
  ...
  const manifest: InputLiveV21Manifest = {
    artifactVersion: "input-live-v21-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V21_BROKER_VERSION,
...
export function validateInputLiveV21Request(
  directory: string,
  request: InputLiveV21Request,
): void {
  ...
  const expectedSequence = inputLiveV21RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v21-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V21_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V21_TARGET.fileKey ||
...
export function issueInputLiveV21Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV21RemotePhase,
  program: string,
  options: InputLiveV21IssueOptions = {},
): InputLiveV21Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV21RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV21Request>(targetPath);
    validateInputLiveV21Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV21ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV21TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV21RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V21_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V21_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V21_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV21RequestBody = {
    artifactVersion: "input-live-v21-broker-request-v2",
    brokerVersion: INPUT_LIVE_V21_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV21Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV21Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV21RawResponse(
  directory: string,
  phase: InputLiveV21RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV21RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV21Request>(requestFile);
  if (
  ...
  const target = inputLiveV21ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV21Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v21-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V21_BROKER_VERSION,
...
export function acceptInputLiveV21Response<T = unknown>(
  directory: string,
  phase: InputLiveV21RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV21AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV21ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV21Request>(
    inputLiveV21RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV21Request(directory, request);
  if (phase === "capture") validateInputLiveV21TechnicalGates(directory);
  const rawFile = inputLiveV21ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v21-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V21_BROKER_VERSION ||
...
export function persistInputLiveV21TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV21RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV21RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V21_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v21-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V21_BROKER_VERSION,
...
export function validateInputLiveV21TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v21-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V21_TECHNICAL_GATES.some(
...
export function validateInputLiveV21Receipt(
  directory: string,
  phase: InputLiveV21RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV21Request(directory, request);
  const receipt = json<InputLiveV21Receipt>(
    inputLiveV21ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV21ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v21-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V21_BROKER_VERSION ||
...
export function readInputLiveV21AcceptedRawPayload(
  directory: string,
  phase: InputLiveV21RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV21Receipt(directory, phase, captureIndex);
...
export function createInputLiveV21FakeDynamicResponse(
  request: InputLiveV21Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V21_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V21_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV21PersistedRequests(
  directory: string,
): InputLiveV21Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV21Request>(file)] : [];
    });
}

export const inputLiveV21Sha256 = sha256;
