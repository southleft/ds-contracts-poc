...
export type InputLiveV8HistoryExpectation = "pending" | "authorized";

export interface InputLiveV8AuthorizationArtifact {
  artifactVersion: "input-live-v8-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V8_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV8AntecedentIndex {
...
export interface InputLiveV8HistoryState {
...
export interface InputLiveV8AuthorizationProof extends InputLiveV8TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V8_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V8_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V8_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V8_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v8-status") ||
...
export function validateInputLiveV8AntecedentIndex(
  index: InputLiveV8AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v8-antecedent-index-v1" ||
...
export function validateInputLiveV8UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV8History\(\s*["']/.test(source) ||
    /readInputLiveV8HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV8AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV8AuthorizationArtifact {
  ...
  const indexFailures = validateInputLiveV8AntecedentIndex(index);
...
export function validateInputLiveV8AuthorizationArtifact(
  artifact: InputLiveV8AuthorizationArtifact | undefined,
  index: InputLiveV8AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v8-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v8" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V8_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V8_TARGET) ||
    !equalJson(
  ...
  if (
    ...
    failures.push(
      "v8 execution, capture, cleanup, or v7-reuse policy weakened",
    );
  if (
...
export function validateInputLiveV8History(
  state: InputLiveV8HistoryState,
  expected: InputLiveV8HistoryExpectation,
): string[] {
  const failures = validateInputLiveV8AntecedentIndex(state.index);
...
export function readInputLiveV8HistoryState(
  root = process.cwd(),
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V8_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV8AuthorizationArtifact)
    : undefined;
  ...
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V8_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V8_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV8History(
  expected: InputLiveV8HistoryExpectation,
  root = process.cwd(),
): InputLiveV8HistoryState {
  const state = readInputLiveV8HistoryState(root);
  const failures = validateInputLiveV8History(state, expected);
  if (failures.length)
    throw new Error(
      `Input live v8 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyInputLiveV8Authorization(
  root = process.cwd(),
): InputLiveV8AuthorizationProof {
  const state = verifyInputLiveV8History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(path.join(repositoryRoot, INPUT_LIVE_V8_AUTHORIZATION_PATH)),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v8/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v8.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V8_TARGET,
    expectedDynamicTool: INPUT_LIVE_V8_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V8_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV8HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV8History(expected);
  process.stdout.write(
    `Input live v8 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
