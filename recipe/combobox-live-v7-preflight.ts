...
import {
  validateComboboxLiveV7AntecedentIndex,
  verifyComboboxLiveV7Authorization,
  type ComboboxLiveV7AntecedentIndex,
  type ComboboxLiveV7AuthorizationProof,
} from "./combobox-live-v7-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V7_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v7/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V7_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v7-security-attestation.json";

export interface ComboboxLiveV7SecurityAttestation {
  artifactVersion: "combobox-live-v7-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV7PreflightReport {
  artifactVersion: "combobox-live-v7-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV7SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV7SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV7AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v7-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V7_TARGET) ||
...
export function validateComboboxLiveV7ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV7Authorization");
  const preflight = source.indexOf("runComboboxLiveV7Preflight");
  const initialize = source.indexOf("ComboboxLiveV7Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV7Preflight(
  root: string,
  proof: ComboboxLiveV7AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V7_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV7PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V7_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V7_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V7_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV7AntecedentIndex;
  failures.push(...validateComboboxLiveV7AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V7_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V7_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV7SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v7-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV7ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v7 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV7SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v7-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V7_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV7Preflight(
    process.cwd(),
    verifyComboboxLiveV7Authorization(),
    attestationPath,
