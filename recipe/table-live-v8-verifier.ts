...
import {
  verifyTableLiveV8FixedSceneFixedPoint,
  type TableLiveV8FixedFixedPoint,
} from "./table-live-v8-fixed-point.js";

export const TABLE_LIVE_V8_VERIFIER_VERSION = "table-live-v8-verifier-v1";

export function buildTableLiveV8RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV8RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV8RawNode[];
}

export interface TableLiveV8NormalizedScene {
...
export interface TableLiveV8SceneProof {
...
export function normalizeTableLiveV8Scene(
  root: TableLiveV8RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV8NormalizedScene {
  const canonicalBindings: TableLiveV8NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV8RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV8Scene<Instance>(
  normalized: TableLiveV8NormalizedScene,
...
): TableLiveV8SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV8FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV8ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV8PreCaptureGates(payload: unknown): void {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("table live v8 pre-capture gate payload absent");
