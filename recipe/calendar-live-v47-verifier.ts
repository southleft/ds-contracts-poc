...
import {
  verifyCalendarLiveV47FixedSceneFixedPoint,
  type CalendarLiveV47FixedFixedPoint,
} from "./calendar-live-v47-fixed-point.js";

export const CALENDAR_LIVE_V47_VERIFIER_VERSION = "calendar-live-v47-verifier-v1";

export function buildCalendarLiveV47RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV47RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV47RawNode[];
}

export interface CalendarLiveV47NormalizedScene {
...
export interface CalendarLiveV47SceneProof {
...
export function normalizeCalendarLiveV47Scene(
  root: CalendarLiveV47RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV47NormalizedScene {
  const canonicalBindings: CalendarLiveV47NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV47RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV47Scene<Instance>(
  normalized: CalendarLiveV47NormalizedScene,
...
): CalendarLiveV47SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV47FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV47ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV47PreCaptureGates(payload: unknown): void {
