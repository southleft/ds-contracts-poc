...
export interface ComboboxLiveV29LifecycleSimulationReport {
  artifactVersion: "combobox-live-v29-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV29AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV29Lifecycle(): Promise<ComboboxLiveV29LifecycleSimulationReport> {
  await buildComboboxLiveV29Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V29_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV29AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v29-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V29_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V29_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV29History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV29History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV29AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V29_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V29_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV29History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV29History(authorized, "pending");
    ...
    await buildComboboxLiveV29Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V29_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV29AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV29Lifecycle(), null, 2)}\n`,
  );
