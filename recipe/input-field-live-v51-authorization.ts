...
import {
  ...
  INPUT_LIVE_V51_INDEX_PATH,
} from "./build-input-field-live-proof-v51.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV51HistoryExpectation = "pending" | "authorized";

export interface InputLiveV51AuthorizationArtifact {
  artifactVersion: "input-live-v51-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V51_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV51AntecedentIndex {
...
export interface InputLiveV51HistoryState {
  index?: InputLiveV51AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV51AuthorizationArtifact;
...
export interface InputLiveV51AuthorizationProof extends InputLiveV51TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V51_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V51_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V51_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V51_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v51-authorization") ||
  artifactPath.includes("input-field-live-v51-preflight") ||
  artifactPath.includes("input-field-live-v51-authorized") ||
  artifactPath.includes("input-field-live-pivot-v51-status") ||
...
export function validateInputLiveV51AntecedentIndex(
  index: InputLiveV51AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v51-antecedent-index-v1" ||
...
export function validateInputLiveV51UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV51History\(\s*["']/.test(source) ||
    /readInputLiveV51HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV51AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV51AuthorizationArtifact {
  ...
  ) as InputLiveV51AntecedentIndex;
  const indexFailures = validateInputLiveV51AntecedentIndex(index);
...
export function validateInputLiveV51AuthorizationArtifact(
  artifact: InputLiveV51AuthorizationArtifact | undefined,
  index: InputLiveV51AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v51-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v51" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V51_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V51_TARGET) ||
    !equalJson(
...
export function validateInputLiveV51History(
  state: InputLiveV51HistoryState,
  expected: InputLiveV51HistoryExpectation,
): string[] {
  const failures = validateInputLiveV51AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV51AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV51HistoryState(
  root = process.cwd(),
): InputLiveV51HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V51_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV51AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V51_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V51_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V51_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV51History(
  expected: InputLiveV51HistoryExpectation,
  root = process.cwd(),
): InputLiveV51HistoryState {
  const state = readInputLiveV51HistoryState(root);
  const failures = validateInputLiveV51History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV51Authorization(
  root = process.cwd(),
): InputLiveV51AuthorizationProof {
  const state = verifyInputLiveV51History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V51_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v51/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v51.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V51_TARGET,
    expectedDynamicTool: INPUT_LIVE_V51_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V51_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV51HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV51History(expected);
