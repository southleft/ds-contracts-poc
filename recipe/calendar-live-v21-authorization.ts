...
export type CalendarLiveV21HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV21AuthorizationArtifact {
  artifactVersion: "calendar-live-v21-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V21_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV21AntecedentIndex {
...
export interface CalendarLiveV21HistoryState {
  index?: CalendarLiveV21AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV21AuthorizationArtifact;
...
export interface CalendarLiveV21AuthorizationProof
  extends CalendarLiveV21TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V21_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V21_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V21_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V21_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v21-authorization") ||
  artifactPath.includes("calendar-live-v21-preflight") ||
  artifactPath.includes("calendar-live-v21-authorized") ||
  artifactPath.includes("create-calendar-live-v21-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v21-status") ||
...
export function validateCalendarLiveV21AntecedentIndex(
  index: CalendarLiveV21AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v21-antecedent-index-v1" ||
...
export function validateCalendarLiveV21UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV21History\(\s*["']/.test(source) ||
    /readCalendarLiveV21HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV21AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV21AuthorizationArtifact {
  ...
  const indexFailures = validateCalendarLiveV21AntecedentIndex(index);
...
export function validateCalendarLiveV21AuthorizationArtifact(
  artifact: CalendarLiveV21AuthorizationArtifact | undefined,
  index: CalendarLiveV21AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v21-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v21" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V21_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V21_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V21_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V21_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V21_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V21_SOURCE_ROOTS ||
...
export function validateCalendarLiveV21History(
  state: CalendarLiveV21HistoryState,
  expected: CalendarLiveV21HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV21AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV21AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV21HistoryState(
  root = process.cwd(),
): CalendarLiveV21HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V21_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV21AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V21_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V21_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V21_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV21History(
  expected: CalendarLiveV21HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV21HistoryState {
  const state = readCalendarLiveV21HistoryState(root);
  const failures = validateCalendarLiveV21History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v21 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV21Authorization(
  root = process.cwd(),
): CalendarLiveV21AuthorizationProof {
  const state = verifyCalendarLiveV21History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V21_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v21/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v21.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V21_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V21_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V21_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV21HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV21History(expected);
  process.stdout.write(
    `Calendar live v21 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
