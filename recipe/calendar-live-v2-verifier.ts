...
import {
  verifyCalendarLiveV2FixedSceneFixedPoint,
  type CalendarLiveV2FixedFixedPoint,
} from "./calendar-live-v2-fixed-point.js";

export const CALENDAR_LIVE_V2_VERIFIER_VERSION = "calendar-live-v2-verifier-v1";

export function buildCalendarLiveV2RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV2RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV2RawNode[];
}

export interface CalendarLiveV2NormalizedScene {
...
export interface CalendarLiveV2SceneProof {
...
export function normalizeCalendarLiveV2Scene(
  root: CalendarLiveV2RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV2NormalizedScene {
  const canonicalBindings: CalendarLiveV2NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV2RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV2Scene<Instance>(
  normalized: CalendarLiveV2NormalizedScene,
...
): CalendarLiveV2SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV2FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV2ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV2PreCaptureGates(payload: unknown): void {
