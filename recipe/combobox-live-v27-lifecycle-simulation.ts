...
export interface ComboboxLiveV27LifecycleSimulationReport {
  artifactVersion: "combobox-live-v27-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV27AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV27Lifecycle(): Promise<ComboboxLiveV27LifecycleSimulationReport> {
  await buildComboboxLiveV27Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V27_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV27AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v27-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V27_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V27_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV27History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV27History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV27AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V27_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V27_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV27History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV27History(authorized, "pending");
    ...
    await buildComboboxLiveV27Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V27_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV27AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV27Lifecycle(), null, 2)}\n`,
  );
