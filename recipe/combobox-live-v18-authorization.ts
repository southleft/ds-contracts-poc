...
export type ComboboxLiveV18HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV18AuthorizationArtifact {
  artifactVersion: "combobox-live-v18-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V18_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV18AntecedentIndex {
...
export interface ComboboxLiveV18HistoryState {
  index?: ComboboxLiveV18AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV18AuthorizationArtifact;
...
export interface ComboboxLiveV18AuthorizationProof extends ComboboxLiveV18TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V18_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V18_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V18_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V18_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v18-authorization") ||
  artifactPath.includes("combobox-live-v18-preflight") ||
  artifactPath.includes("combobox-live-v18-authorized") ||
  artifactPath.includes("create-combobox-live-v18-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v18-status") ||
...
export function validateComboboxLiveV18AntecedentIndex(
  index: ComboboxLiveV18AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v18-antecedent-index-v1" ||
...
export function validateComboboxLiveV18UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV18History\(\s*["']/.test(source) ||
    /readComboboxLiveV18HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV18AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV18AuthorizationArtifact {
  ...
  ) as ComboboxLiveV18AntecedentIndex;
  const indexFailures = validateComboboxLiveV18AntecedentIndex(index);
...
export function validateComboboxLiveV18AuthorizationArtifact(
  artifact: ComboboxLiveV18AuthorizationArtifact | undefined,
  index: ComboboxLiveV18AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v18-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v18" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V18_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V18_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V18_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V18_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V18_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V18_SOURCE_ROOTS ||
  ...
  if (
    ...
    failures.push(
      "v18 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
...
export function validateComboboxLiveV18History(
  state: ComboboxLiveV18HistoryState,
  expected: ComboboxLiveV18HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV18AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV18AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV18HistoryState(
  root = process.cwd(),
): ComboboxLiveV18HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V18_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV18AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V18_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V18_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V18_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV18History(
  expected: ComboboxLiveV18HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV18HistoryState {
  const state = readComboboxLiveV18HistoryState(root);
  const failures = validateComboboxLiveV18History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v18 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV18Authorization(
  root = process.cwd(),
): ComboboxLiveV18AuthorizationProof {
  const state = verifyComboboxLiveV18History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V18_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v18/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v18.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V18_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V18_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V18_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV18HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV18History(expected);
  process.stdout.write(
    `Combobox live v18 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
