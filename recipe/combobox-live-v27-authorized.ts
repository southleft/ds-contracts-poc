...
import { COMBOBOX_LIVE_V27_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v27.js";
import { verifyComboboxLiveV27Authorization } from "./combobox-live-v27-authorization.js";
import { runComboboxLiveV27Preflight } from "./combobox-live-v27-preflight.js";
import { ComboboxLiveV27Orchestrator } from "./run-combobox-live-v27.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v27 requires exact authorization artifact ${COMBOBOX_LIVE_V27_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v27-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV27Authorization(root);
    runComboboxLiveV27Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV27Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV27Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
