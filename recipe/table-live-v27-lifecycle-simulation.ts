...
import { tableLiveV2Sha256 } from "./table-live-v27-broker.js";

export interface TableLiveV27LifecycleSimulationReport {
  artifactVersion: "table-live-v27-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV27AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV27Lifecycle(): Promise<TableLiveV27LifecycleSimulationReport> {
  await buildTableLiveV27Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V27_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV27AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v27-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V27_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V27_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV27History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV27History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV27AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V27_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V27_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV27History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV27History(authorized, "pending");
    ...
    await buildTableLiveV27Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V27_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV27AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV27Lifecycle(), null, 2)}\n`,
  );
