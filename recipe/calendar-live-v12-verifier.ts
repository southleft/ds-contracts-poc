...
import {
  verifyCalendarLiveV12FixedSceneFixedPoint,
  type CalendarLiveV12FixedFixedPoint,
} from "./calendar-live-v12-fixed-point.js";

export const CALENDAR_LIVE_V12_VERIFIER_VERSION = "calendar-live-v12-verifier-v1";

export function buildCalendarLiveV12RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV12RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV12RawNode[];
}

export interface CalendarLiveV12NormalizedScene {
...
export interface CalendarLiveV12SceneProof {
...
export function normalizeCalendarLiveV12Scene(
  root: CalendarLiveV12RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV12NormalizedScene {
  const canonicalBindings: CalendarLiveV12NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV12RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV12Scene<Instance>(
  normalized: CalendarLiveV12NormalizedScene,
...
): CalendarLiveV12SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV12FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV12ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV12PreCaptureGates(payload: unknown): void {
