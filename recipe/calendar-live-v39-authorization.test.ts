...
import { CALENDAR_LIVE_V39_INDEX_PATH } from "./build-calendar-live-proof-v39.js";
import {
...
import {
  buildCalendarLiveV39AuthorizationArtifact,
  validateCalendarLiveV39AntecedentIndex,
  validateCalendarLiveV39AuthorizationArtifact,
  validateCalendarLiveV39History,
  validateCalendarLiveV39UnitTestSource,
  type CalendarLiveV39AntecedentIndex,
  type CalendarLiveV39AuthorizationArtifact,
  type CalendarLiveV39AuthorizationProof,
  type CalendarLiveV39HistoryState,
} from "./calendar-live-v39-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV39ControlFlowSource,
  validateCalendarLiveV39SecurityAttestation,
  type CalendarLiveV39SecurityAttestation,
} from "./calendar-live-v39-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V39_INDEX_PATH);
const index = (): CalendarLiveV39AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV39AntecedentIndex;

const artifact = (): CalendarLiveV39AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV39AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV39HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV39HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV39AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v39/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV39AuthorizationArtifact,
): CalendarLiveV39SecurityAttestation => {
  const body: Omit<CalendarLiveV39SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V39_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV39History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV39History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV39History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV39History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV39History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV39UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v39-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV39UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV39AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV39AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV39AuthorizationArtifact(
        planted as CalendarLiveV39AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV39SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV39SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV39SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v39-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV39ControlFlowSource(source), []);
});
