...
export const INPUT_LIVE_V7_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V7_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V7_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V7_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V7_VARIANT_COUNT = 256;
export const INPUT_LIVE_V7_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V7_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV7SourceId = (typeof INPUT_LIVE_V7_SOURCE_IDS)[number];

export interface InputLiveV7SourceIdentity {
  source: InputLiveV7SourceId;
...
export interface InputLiveV7WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV7ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV7SourceId;
    adapterIdentity: string;
...
export interface InputLiveV7RootProof {
  source: InputLiveV7SourceId;
  adapterIdentity: string;
...
export interface InputLiveV7ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV7SourceId;
    adapterIdentity: string;
...
export interface InputLiveV7CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV7SourceId;
  adapterIdentity: string;
...
export interface InputLiveV7CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV7SourceId;
  strata: InputLiveV7CaptureCell["strata"];
...
export interface InputLiveV7CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Input live v7 ${label} must be an object`);
...
const exactUniqueStrings = (
  ...
  if (
    ...
    throw new TypeError(`Input live v7 ${label} cardinality/identity mismatch`);
  return value;
};
const finiteNonnegative = (value: unknown): value is number =>
  typeof value === "number" && Number.isFinite(value) && value >= 0;

export function validateInputLiveV7WriterPayload(
  value: unknown,
): InputLiveV7WriterOwnership {
  ...
  if (
    ...
    throw new TypeError("Input live v7 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Input live v7 writer created-node denominator invalid",
    );
  return {
...
const identityPlan = (source: InputLiveV7SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV7ExtractProgram(
  writer: InputLiveV7WriterOwnership,
  sources: readonly InputLiveV7SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Input live v7 extract requires two source identities");
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V7_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V7_NAMESPACE)}
${buildInputLiveV4RawPropertyRuntime()}
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("INPUT-V7-EXTRACT-PAGE");
const get=(node,key)=>node.getSharedPluginData(V7_NS,key);
if(get(page,"pageOwner")!=="recipe/input-field/"+${JSON.stringify(writer.runIdentity)})throw new Error("INPUT-V7-EXTRACT-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>V7_SET_IDS.has(node.id));
if(sets.length!==2)throw new Error("INPUT-V7-EXTRACT-ROOTS:"+sets.length);
...
for(const set of sets){
  ...
  if(!expected)throw new Error("INPUT-V7-EXTRACT-ADAPTER:"+adapterIdentity);
...
export function validateInputLiveV7ExtractPayload(
  value: unknown,
  writer: InputLiveV7WriterOwnership,
): InputLiveV7ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Input live v7 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V7_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Input live v7 extract schema/two-root mismatch");
  return payload as InputLiveV7ExtractPayload;
}

export function proveInputLiveV7Roots<Instance>(
  extract: InputLiveV7ExtractPayload,
  sources: readonly (InputLiveV7SourceIdentity & {
...
): InputLiveV7RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Input live v7 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Input live v7 omitted ${source.source} root`);
...
export function assertInputLiveV7RootProofs(
  proofs: readonly InputLiveV7RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Input live v7 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Input live v7 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildInputLiveV7ProbeProgram(
  writer: InputLiveV7WriterOwnership,
  sources: readonly InputLiveV7SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V7_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV7ProbePayload(
  value: unknown,
  writer: InputLiveV7WriterOwnership,
): InputLiveV7ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV7ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V7_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V7_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Input live v7 probe/usability/restoration failed");
  return payload;
}

export function validateInputLiveV7CaptureManifest(
  cells: readonly InputLiveV7CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V7_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V7_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V7_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V7_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V7_SOURCE_IDS.some(
      (source) => cells.filter((cell) => cell.source === source).length !== 64,
    )
  )
    throw new TypeError(
      "Input live v7 capture manifest must be exact 128 cells",
    );
}

export const inputLiveV7CaptureManifestSha256 = (
  cells: readonly InputLiveV7CaptureCell[],
): string => {
  validateInputLiveV7CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV7CaptureProgram(
  writer: InputLiveV7WriterOwnership,
  cell: InputLiveV7CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V7_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V7_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
    ...
    throw new TypeError("Input live v7 capture cell is malformed");
  const expectedSet = writer.sources.find(
    (source) => source.adapterIdentity === cell.adapterIdentity,
  )?.setId;
  if (!expectedSet)
    throw new TypeError(
      `Input live v7 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
if(!page||page.type!=="PAGE"||!set||set.type!=="COMPONENT_SET")throw new Error("INPUT-V7-CAPTURE-TARGET");
const axes=node=>Object.fromEntries(node.name.split(",").map(part=>part.trim()).filter(part=>part.includes("=")).map(part=>{const at=part.indexOf("=");return[part.slice(0,at),part.slice(at+1)];}));
...
if(!component)throw new Error("INPUT-V7-CAPTURE-CELL:"+cell.cellKey);
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V7_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V7-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV7CapturePayload(
  value: unknown,
  cell: InputLiveV7CaptureCell,
  rawResponseBytes: number,
): InputLiveV7CapturePayload {
  const payload = record(value, "capture payload") as InputLiveV7CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V7_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V7_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Input live v7 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertInputLiveV7CaptureResponses(
  manifest: readonly InputLiveV7CaptureCell[],
  responses: readonly InputLiveV7CapturePayload[],
): void {
  validateInputLiveV7CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Input live v7 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface InputLiveV7ObjectiveReport {
  artifactVersion: "input-live-v7-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV7Objective(
  manifest: readonly InputLiveV7CaptureCell[],
  responses: readonly InputLiveV7CapturePayload[],
  readReference: (cell: InputLiveV7CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV7CaptureCell) => Buffer,
): InputLiveV7ObjectiveReport {
  assertInputLiveV7CaptureResponses(manifest, responses);
  const rows = manifest.map((cell, index) => {
    ...
    if (
      ...
      throw new TypeError(
        `Input live v7 objective byte drift: ${cell.cellKey}`,
      );
    ...
    if (
      ...
      throw new TypeError(
        `Input live v7 objective measurement refused: ${cell.cellKey}`,
      );
    return {
      index: cell.index,
...
export function buildInputLiveV7CleanupProgram(
  writer: InputLiveV7WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V7_NAMESPACE,
  ...
  const guardedResult = `if(JSON.stringify(requestedNodeIds)!==JSON.stringify([expectedPageId])||JSON.stringify([...requestedCollectionIds].sort())!==JSON.stringify([...expectedCollectionIds].sort()))throw new Error("INPUT-V7-CLEANUP-EXACT-OWNERSHIP");${resultStatement}`;
  const program = baseProgram.replace(resultStatement, guardedResult);
  if (program === baseProgram)
    throw new Error("Input live v7 cleanup runtime result hook absent");
  return String.raw`
...
export function validateInputLiveV7CleanupPayload(
  value: unknown,
  writer: InputLiveV7WriterOwnership,
): InputLiveV7CleanupPayload {
  const payload = record(value, "cleanup payload") as InputLiveV7CleanupPayload;
  ...
  if (
    ...
    throw new TypeError("Input live v7 cleanup is incomplete or overbroad");
  return payload;
}

export const INPUT_LIVE_V7_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV7WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
