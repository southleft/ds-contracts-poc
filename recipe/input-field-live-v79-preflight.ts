...
import {
  ...
  verifyInputLiveV79Authorization,
  type InputLiveV79AntecedentIndex,
  type InputLiveV79AuthorizationProof,
} from "./input-field-live-v79-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V79_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v79/operator-security-attestation-template.json";
export const INPUT_LIVE_V79_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v79-security-attestation.json";

export interface InputLiveV79SecurityAttestation {
...
export interface InputLiveV79PreflightReport {
  artifactVersion: "input-live-v79-preflight-v1";
...
export const inputLiveV79AttestationSha256 = (
  value: Omit<InputLiveV79SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV79SecurityAttestation(
  value: unknown,
  proof: InputLiveV79AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v79-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V79_TARGET) ||
...
export function validateInputLiveV79ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV79Authorization");
  const preflight = source.indexOf("runInputLiveV79Preflight");
  const initialize = source.indexOf("InputLiveV79Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV79Preflight(
  root: string,
  proof: InputLiveV79AuthorizationProof,
  attestationPath = INPUT_LIVE_V79_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV79PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V79_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V79_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V79_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV79AntecedentIndex;
  failures.push(...validateInputLiveV79AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V79_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V79_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v79-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV79ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV79SecurityAttestation;
  return {
    artifactVersion: "input-live-v79-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V79_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV79Preflight(
    process.cwd(),
    verifyInputLiveV79Authorization(),
    attestationPath,
