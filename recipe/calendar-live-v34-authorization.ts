...
export type CalendarLiveV34HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV34AuthorizationArtifact {
  artifactVersion: "calendar-live-v34-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V34_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV34AntecedentIndex {
...
export interface CalendarLiveV34HistoryState {
  index?: CalendarLiveV34AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV34AuthorizationArtifact;
...
export interface CalendarLiveV34AuthorizationProof
  extends CalendarLiveV34TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V34_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V34_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V34_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V34_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v34-authorization") ||
  artifactPath.includes("calendar-live-v34-preflight") ||
  artifactPath.includes("calendar-live-v34-authorized") ||
  artifactPath.includes("create-calendar-live-v34-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v34-status") ||
...
export function validateCalendarLiveV34AntecedentIndex(
  index: CalendarLiveV34AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v34-antecedent-index-v1" ||
...
export function validateCalendarLiveV34UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV34History\(\s*["']/.test(source) ||
    /readCalendarLiveV34HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV34AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV34AuthorizationArtifact {
  ...
  ) as CalendarLiveV34AntecedentIndex;
  const indexFailures = validateCalendarLiveV34AntecedentIndex(index);
...
export function validateCalendarLiveV34AuthorizationArtifact(
  artifact: CalendarLiveV34AuthorizationArtifact | undefined,
  index: CalendarLiveV34AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v34-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v34" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V34_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V34_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V34_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V34_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V34_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V34_SOURCE_ROOTS ||
...
export function validateCalendarLiveV34History(
  state: CalendarLiveV34HistoryState,
  expected: CalendarLiveV34HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV34AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV34AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV34HistoryState(
  root = process.cwd(),
): CalendarLiveV34HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V34_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV34AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V34_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V34_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V34_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV34History(
  expected: CalendarLiveV34HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV34HistoryState {
  const state = readCalendarLiveV34HistoryState(root);
  const failures = validateCalendarLiveV34History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v34 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV34Authorization(
  root = process.cwd(),
): CalendarLiveV34AuthorizationProof {
  const state = verifyCalendarLiveV34History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V34_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v34/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v34.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V34_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V34_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V34_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV34HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV34History(expected);
  process.stdout.write(
    `Calendar live v34 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
