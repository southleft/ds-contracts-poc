...
import { TABLE_LIVE_V26_INDEX_PATH } from "./build-table-live-proof-v26.js";
import {
...
import {
  buildTableLiveV26AuthorizationArtifact,
  validateTableLiveV26AntecedentIndex,
  validateTableLiveV26AuthorizationArtifact,
  validateTableLiveV26History,
  validateTableLiveV26UnitTestSource,
  type TableLiveV26AntecedentIndex,
  type TableLiveV26AuthorizationArtifact,
  type TableLiveV26AuthorizationProof,
  type TableLiveV26HistoryState,
} from "./table-live-v26-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV26ControlFlowSource,
  validateTableLiveV26SecurityAttestation,
  type TableLiveV26SecurityAttestation,
} from "./table-live-v26-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V26_INDEX_PATH);
const index = (): TableLiveV26AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV26AntecedentIndex;

const artifact = (): TableLiveV26AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV26AuthorizationArtifact({
...
const pendingState = (): TableLiveV26HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV26HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV26AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v26/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV26AuthorizationArtifact,
): TableLiveV26SecurityAttestation => {
  const body: Omit<TableLiveV26SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V26_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV26History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV26History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV26History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV26History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV26History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV26UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v26-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV26UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV26AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV26AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV26AuthorizationArtifact(
        planted as TableLiveV26AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV26SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV26SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV26SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v26-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV26ControlFlowSource(source), []);
});
