...
export interface ComboboxLiveV6CleanupPlan {
  fileKey: string;
  fileName: string;
  editorType: string;
  namespace: string;
...
export function buildComboboxLiveV6CleanupRuntime(
  plan: ComboboxLiveV6CleanupPlan,
): string {
  ...
  return String.raw`
if(figma.fileKey!==${JSON.stringify(plan.fileKey)}||figma.root.name!==${JSON.stringify(plan.fileName)}||figma.editorType!==${JSON.stringify(plan.editorType)})throw new Error("COMBOBOX-V6-CLEANUP-WRONG-TARGET");
