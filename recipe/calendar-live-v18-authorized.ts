...
import { CALENDAR_LIVE_V18_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v18.js";
import { verifyCalendarLiveV18Authorization } from "./calendar-live-v18-authorization.js";
import { runCalendarLiveV18Preflight } from "./calendar-live-v18-preflight.js";
import { CalendarLiveV18Orchestrator } from "./run-calendar-live-v18.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v18 requires exact authorization artifact ${CALENDAR_LIVE_V18_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v18-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV18Authorization(root);
    runCalendarLiveV18Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV18Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV18Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
