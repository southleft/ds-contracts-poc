...
import { calendarLiveV1Sha256 } from "./calendar-live-v14-broker.js";

export interface CalendarLiveV14LifecycleSimulationReport {
  artifactVersion: "calendar-live-v14-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV14AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV14Lifecycle(): Promise<CalendarLiveV14LifecycleSimulationReport> {
  await buildCalendarLiveV14Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V14_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV14AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v14-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V14_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V14_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV14History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV14History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV14AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V14_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V14_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV14History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV14History(authorized, "pending");
    ...
    await buildCalendarLiveV14Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V14_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV14AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV14Lifecycle(), null, 2)}\n`,
  );
