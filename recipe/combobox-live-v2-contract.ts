...
import { buildComboboxLiveV2CleanupRuntime } from "./combobox-live-v2-cleanup.js";
import {
  type ComboboxLiveV2FixedFixedPoint,
} from "./combobox-live-v2-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV2RestoreProgram,
  validateComboboxLiveV2RestorePayload,
  type ComboboxLiveV2RestorePayload,
} from "./combobox-live-v2-restore.js";
import {
  buildComboboxLiveV2RawPropertyRuntime,
  normalizeComboboxLiveV2Scene,
  type ComboboxLiveV2RawNode,
} from "./combobox-live-v2-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v1.js";
import {
...
export const COMBOBOX_LIVE_V2_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V2_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V2_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V2_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V2_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V2_SET_COUNT = 4;
export const COMBOBOX_LIVE_V2_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V2_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V2_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V2_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V2_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV2SourceId = (typeof COMBOBOX_LIVE_V2_SOURCE_IDS)[number];

export interface ComboboxLiveV2SourceIdentity {
  source: ComboboxLiveV2SourceId;
...
export interface ComboboxLiveV2WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV2ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV2SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV2RawNode;
    optionScene: ComboboxLiveV2RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV2RootProof {
  source: ComboboxLiveV2SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV2FixedFixedPoint;
}

export interface ComboboxLiveV2ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV2SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV2CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV2SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV2CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV2SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV2CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v1 ${label} must be an object`);
...
export function validateComboboxLiveV2WriterPayload(
  value: unknown,
): ComboboxLiveV2WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V2_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v1 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v1 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v1 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV2SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV2ExtractProgram(
  writer: ComboboxLiveV2WriterOwnership,
  sources: readonly ComboboxLiveV2SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v1 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V2_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V2_NAMESPACE)}
${buildComboboxLiveV2RawPropertyRuntime()}
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("COMBOBOX-V2-EXTRACT-PAGE");
...
if(get(page,"pageOwner")!=="recipe/combobox/"+${JSON.stringify(writer.runIdentity)})throw new Error("COMBOBOX-V2-EXTRACT-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>SET_IDS.has(node.id));
if(sets.length!==4)throw new Error("COMBOBOX-V2-EXTRACT-ROOTS:"+sets.length);
...
for(const set of sets){
  ...
  if(!IDENTITIES[adapterIdentity]||(kind!=="combobox"&&kind!=="option"))throw new Error("COMBOBOX-V2-EXTRACT-ADAPTER:"+adapterIdentity+":"+kind);
...
if(roots.length!==2||roots.some(root=>!root.comboboxScene||!root.optionScene))throw new Error("COMBOBOX-V2-EXTRACT-PAIR");
...
export function validateComboboxLiveV2ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV2WriterOwnership,
): ComboboxLiveV2ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v1 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V2_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v1 extract schema/two-root mismatch");
  return payload as ComboboxLiveV2ExtractPayload;
}

export function proveComboboxLiveV2Roots<Instance>(
  extract: ComboboxLiveV2ExtractPayload,
  sources: readonly (ComboboxLiveV2SourceIdentity & {
...
): ComboboxLiveV2RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v1 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v1 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV2Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV2Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV2FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v1 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v1 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v1 compile lost owned sets");
...
export function assertComboboxLiveV2RootProofs(
  proofs: readonly ComboboxLiveV2RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v1 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v1 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV2ProbeProgram(
  writer: ComboboxLiveV2WriterOwnership,
  sources: readonly ComboboxLiveV2SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V2_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE"||page.id==="115:295378")throw new Error("COMBOBOX-V2-PROBE-PAGE");
...
if(sets.length!==4)throw new Error("COMBOBOX-V2-PROBE-ROOTS:"+sets.length);
...
for(const row of byAdapter.values()){
 ...
 if(!source||!combobox||!option||combobox.children.length!==64||option.children.length!==8)throw new Error("COMBOBOX-V2-PROBE-VARIANTS:"+adapterIdentity);
 ...
 for(const component of combobox.children){const target=axes(component),updates={};for(const name of axisNames){const key=propertyKey(instance,name);if(!key)throw new Error("COMBOBOX-V2-PROBE-AXIS:"+name);updates[key]=target[name];}instance.setProperties(updates);const main=await instance.getMainComponentAsync();if(main)visited.add(main.id);}
 ...
 for(const component of option.children){const target=axes(component),updates={};for(const name of ["Size","Option state"]){const key=propertyKey(optionInstance,name);if(!key)throw new Error("COMBOBOX-V2-PROBE-OPTION-AXIS:"+name);updates[key]=target[name];}optionInstance.setProperties(updates);const main=await optionInstance.getMainComponentAsync();if(main)optionVisited.add(main.id);}
...
export function validateComboboxLiveV2ProbePayload(
  value: unknown,
  writer: ComboboxLiveV2WriterOwnership,
): ComboboxLiveV2ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV2ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V2_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V2_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v1 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV2CaptureManifest(
  cells: readonly ComboboxLiveV2CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V2_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V2_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V2_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V2_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v1 capture manifest is malformed");
}

export const comboboxLiveV2CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV2CaptureCell[],
): string => {
  validateComboboxLiveV2CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV2CaptureProgram(
  writer: ComboboxLiveV2WriterOwnership,
  cell: ComboboxLiveV2CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V2_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v1 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v1 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
if(!page||page.type!=="PAGE"||page.id==="115:295378"||!set||set.type!=="COMPONENT_SET")throw new Error("COMBOBOX-V2-CAPTURE-TARGET");
const axes=node=>Object.fromEntries(node.name.split(",").map(part=>part.trim()).filter(part=>part.includes("=")).map(part=>{const at=part.indexOf("=");return[part.slice(0,at),part.slice(at+1)];}));
...
if(!component)throw new Error("COMBOBOX-V2-CAPTURE-CELL:"+cell.cellKey);
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V2_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV2CapturePayload(
  value: unknown,
  cell: ComboboxLiveV2CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV2CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV2CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V2_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V2_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v1 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV2CaptureResponses(
  manifest: readonly ComboboxLiveV2CaptureCell[],
  responses: readonly ComboboxLiveV2CapturePayload[],
): void {
  validateComboboxLiveV2CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v1 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV2ObjectiveReport {
  artifactVersion: "combobox-live-v2-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV2Objective(
  manifest: readonly ComboboxLiveV2CaptureCell[],
  responses: readonly ComboboxLiveV2CapturePayload[],
): ComboboxLiveV2ObjectiveReport {
  assertComboboxLiveV2CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v2-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV2CleanupProgram(
  writer: ComboboxLiveV2WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV2CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V2_NAMESPACE,
  ...
  const guardedResult = `if(JSON.stringify(requestedNodeIds)!==JSON.stringify([expectedPageId])||JSON.stringify([...requestedCollectionIds].sort())!==JSON.stringify([...expectedCollectionIds].sort()))throw new Error("COMBOBOX-V2-CLEANUP-EXACT-OWNERSHIP");${resultStatement}`;
  const program = baseProgram.replace(resultStatement, guardedResult);
  if (program === baseProgram)
    throw new Error("Combobox live v1 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV2CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV2WriterOwnership,
): ComboboxLiveV2CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV2CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v1 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V2_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV2WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
