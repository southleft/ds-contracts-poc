...
import {
  verifyCalendarLiveV35FixedSceneFixedPoint,
  type CalendarLiveV35FixedFixedPoint,
} from "./calendar-live-v35-fixed-point.js";

export const CALENDAR_LIVE_V35_VERIFIER_VERSION = "calendar-live-v35-verifier-v1";

export function buildCalendarLiveV35RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV35RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV35RawNode[];
}

export interface CalendarLiveV35NormalizedScene {
...
export interface CalendarLiveV35SceneProof {
...
export function normalizeCalendarLiveV35Scene(
  root: CalendarLiveV35RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV35NormalizedScene {
  const canonicalBindings: CalendarLiveV35NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV35RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV35Scene<Instance>(
  normalized: CalendarLiveV35NormalizedScene,
...
): CalendarLiveV35SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV35FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV35ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV35PreCaptureGates(payload: unknown): void {
