...
import {
  ...
  verifyInputLiveV85Authorization,
  type InputLiveV85AntecedentIndex,
  type InputLiveV85AuthorizationProof,
} from "./input-field-live-v85-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V85_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v85/operator-security-attestation-template.json";
export const INPUT_LIVE_V85_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v85-security-attestation.json";

export interface InputLiveV85SecurityAttestation {
...
export interface InputLiveV85PreflightReport {
  artifactVersion: "input-live-v85-preflight-v1";
...
export const inputLiveV85AttestationSha256 = (
  value: Omit<InputLiveV85SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV85SecurityAttestation(
  value: unknown,
  proof: InputLiveV85AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v85-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V85_TARGET) ||
...
export function validateInputLiveV85ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV85Authorization");
  const preflight = source.indexOf("runInputLiveV85Preflight");
  const initialize = source.indexOf("InputLiveV85Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV85Preflight(
  root: string,
  proof: InputLiveV85AuthorizationProof,
  attestationPath = INPUT_LIVE_V85_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV85PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V85_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V85_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V85_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV85AntecedentIndex;
  failures.push(...validateInputLiveV85AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V85_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V85_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v85-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV85ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV85SecurityAttestation;
  return {
    artifactVersion: "input-live-v85-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V85_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV85Preflight(
    process.cwd(),
    verifyInputLiveV85Authorization(),
    attestationPath,
