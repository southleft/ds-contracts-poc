...
} from "./input-field-live-v3-verifier-v49.js";
import {
  buildInputLiveV49RawPropertyRuntime,
  normalizeInputLiveV49Scene,
  type InputLiveV49RawNode,
} from "./input-field-live-v49-verifier.js";
...
export const INPUT_LIVE_V49_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V49_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V49_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V49_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V49_VARIANT_COUNT = 256;
export const INPUT_LIVE_V49_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V49_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV49SourceId = (typeof INPUT_LIVE_V49_SOURCE_IDS)[number];

export interface InputLiveV49SourceIdentity {
  source: InputLiveV49SourceId;
...
export interface InputLiveV49WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV49ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV49SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV49RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV49RootProof {
  source: InputLiveV49SourceId;
  adapterIdentity: string;
...
export interface InputLiveV49ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV49SourceId;
    adapterIdentity: string;
...
export interface InputLiveV49CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV49SourceId;
  adapterIdentity: string;
...
export interface InputLiveV49CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV49SourceId;
  strata: InputLiveV49CaptureCell["strata"];
...
export interface InputLiveV49CleanupPayload {
...
export function validateInputLiveV49WriterPayload(
  value: unknown,
): InputLiveV49WriterOwnership {
...
const identityPlan = (source: InputLiveV49SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV49ExtractProgram(
  writer: InputLiveV49WriterOwnership,
  sources: readonly InputLiveV49SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V49_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V49_NAMESPACE)}
${buildInputLiveV49RawPropertyRuntime()}
...
export function validateInputLiveV49ExtractPayload(
  value: unknown,
  writer: InputLiveV49WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V49_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV49ExtractPayload;
}

export function proveInputLiveV49Roots<Instance>(
  extract: InputLiveV49ExtractPayload,
  sources: readonly (InputLiveV49SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV49Scene(
      root.scene,
...
export function assertInputLiveV49RootProofs(
  proofs: readonly InputLiveV49RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV49ProbeProgram(
  writer: InputLiveV49WriterOwnership,
  sources: readonly InputLiveV49SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V49_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV49ProbePayload(
  value: unknown,
  writer: InputLiveV49WriterOwnership,
): InputLiveV49ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV49ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V49_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V49_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV49CaptureManifest(
  cells: readonly InputLiveV49CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V49_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V49_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V49_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V49_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V49_SOURCE_IDS.some(
...
export const inputLiveV49CaptureManifestSha256 = (
  cells: readonly InputLiveV49CaptureCell[],
): string => {
  validateInputLiveV49CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV49CaptureProgram(
  writer: InputLiveV49WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V49_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V49_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V49_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV49CapturePayload(
  value: unknown,
  cell: InputLiveV49CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV49CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V49_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V49_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV49CaptureResponses(
  manifest: readonly InputLiveV49CaptureCell[],
  responses: readonly InputLiveV49CapturePayload[],
): void {
  validateInputLiveV49CaptureManifest(manifest);
  if (
...
export interface InputLiveV49ObjectiveReport {
  artifactVersion: "input-live-v49-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV49Objective(
  manifest: readonly InputLiveV49CaptureCell[],
  responses: readonly InputLiveV49CapturePayload[],
  readReference: (cell: InputLiveV49CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV49CaptureCell) => Buffer,
): InputLiveV49ObjectiveReport {
  assertInputLiveV49CaptureResponses(manifest, responses);
...
export function buildInputLiveV49CleanupProgram(
  writer: InputLiveV49WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V49_NAMESPACE,
...
export function validateInputLiveV49CleanupPayload(
  value: unknown,
  writer: InputLiveV49WriterOwnership,
): InputLiveV49CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV49CleanupPayload;
...
export const INPUT_LIVE_V49_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV49WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
