...
export interface ComboboxLiveV26LifecycleSimulationReport {
  artifactVersion: "combobox-live-v26-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV26AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV26Lifecycle(): Promise<ComboboxLiveV26LifecycleSimulationReport> {
  await buildComboboxLiveV26Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V26_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV26AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v26-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V26_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V26_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV26History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV26History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV26AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V26_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V26_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV26History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV26History(authorized, "pending");
    ...
    await buildComboboxLiveV26Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V26_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV26AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV26Lifecycle(), null, 2)}\n`,
  );
