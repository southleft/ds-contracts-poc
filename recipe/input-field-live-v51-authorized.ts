...
import { INPUT_LIVE_V51_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v51.js";
import { verifyInputLiveV51Authorization } from "./input-field-live-v51-authorization.js";
import { runInputLiveV51Preflight } from "./input-field-live-v51-preflight.js";
import { InputLiveV51Orchestrator } from "./run-input-field-live-v51.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V51_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v51-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV51Authorization(root);
    runInputLiveV51Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV51Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
