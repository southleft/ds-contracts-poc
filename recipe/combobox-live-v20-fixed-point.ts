/**
 * Carried Combobox live v20 host verifier. Uses scene-readback-combobox-v20 so live
...
export const COMBOBOX_LIVE_V20_FIXED_VERIFIER_VERSION = "combobox-live-v20-fixed-point-v1";
export const COMBOBOX_LIVE_V20_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV20FixedThresholds {
...
export interface ComboboxLiveV20FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV20FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV20FixedSourceProbe {
...
export interface ComboboxLiveV20FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV20FixedFixedPoint;
}

export interface ComboboxLiveV20FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV20FixedHardGateInput {
  thresholds: ComboboxLiveV20FixedThresholds;
  materialRegression: ComboboxLiveV20FixedMaterialRegression;
  sourceProbes: ComboboxLiveV20FixedSourceProbe[];
  cells: ComboboxLiveV20FixedCellValidation[];
  sceneProofs: ComboboxLiveV20FixedSceneProof[];
  visualRows: ComboboxLiveV20FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV20FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V20_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV20FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV20FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV20FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV20FixedHardGates(
  input: ComboboxLiveV20FixedHardGateInput,
): ComboboxLiveV20FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV20FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV20FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V20_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV20FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV20FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV20FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
