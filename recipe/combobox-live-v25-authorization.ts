...
export type ComboboxLiveV25HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV25AuthorizationArtifact {
  artifactVersion: "combobox-live-v25-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V25_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV25AntecedentIndex {
...
export interface ComboboxLiveV25HistoryState {
  index?: ComboboxLiveV25AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV25AuthorizationArtifact;
...
export interface ComboboxLiveV25AuthorizationProof extends ComboboxLiveV25TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V25_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V25_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V25_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V25_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v25-authorization") ||
  artifactPath.includes("combobox-live-v25-preflight") ||
  artifactPath.includes("combobox-live-v25-authorized") ||
  artifactPath.includes("create-combobox-live-v25-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v25-status") ||
...
export function validateComboboxLiveV25AntecedentIndex(
  index: ComboboxLiveV25AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v25-antecedent-index-v1" ||
...
export function validateComboboxLiveV25UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV25History\(\s*["']/.test(source) ||
    /readComboboxLiveV25HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV25AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV25AuthorizationArtifact {
  ...
  ) as ComboboxLiveV25AntecedentIndex;
  const indexFailures = validateComboboxLiveV25AntecedentIndex(index);
...
export function validateComboboxLiveV25AuthorizationArtifact(
  artifact: ComboboxLiveV25AuthorizationArtifact | undefined,
  index: ComboboxLiveV25AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v25-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v25" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V25_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V25_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V25_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V25_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V25_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V25_SOURCE_ROOTS ||
...
export function validateComboboxLiveV25History(
  state: ComboboxLiveV25HistoryState,
  expected: ComboboxLiveV25HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV25AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV25AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV25HistoryState(
  root = process.cwd(),
): ComboboxLiveV25HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V25_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV25AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V25_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V25_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V25_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV25History(
  expected: ComboboxLiveV25HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV25HistoryState {
  const state = readComboboxLiveV25HistoryState(root);
  const failures = validateComboboxLiveV25History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v25 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV25Authorization(
  root = process.cwd(),
): ComboboxLiveV25AuthorizationProof {
  const state = verifyComboboxLiveV25History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V25_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v25/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v25.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V25_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V25_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V25_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV25HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV25History(expected);
  process.stdout.write(
    `Combobox live v25 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
