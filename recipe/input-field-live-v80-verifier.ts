...
export const INPUT_LIVE_V80_VERIFIER_VERSION = "input-live-v80-verifier-v1";

export function buildInputLiveV80RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV80RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV80RawNode[];
}

export interface InputLiveV80NormalizedScene {
...
export interface InputLiveV80SceneProof {
...
export function normalizeInputLiveV80Scene(
  root: InputLiveV80RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV80NormalizedScene {
  const canonicalBindings: InputLiveV80NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV80RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV80Scene<Instance>(
  normalized: InputLiveV80NormalizedScene,
...
export function compileInputLiveV80ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV80PreCaptureGates(payload: unknown): void {
