...
import { calendarLiveV1Sha256 } from "./calendar-live-v34-broker.js";

export interface CalendarLiveV34LifecycleSimulationReport {
  artifactVersion: "calendar-live-v34-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV34AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV34Lifecycle(): Promise<CalendarLiveV34LifecycleSimulationReport> {
  await buildCalendarLiveV34Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V34_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV34AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v34-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V34_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V34_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV34History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV34History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV34AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V34_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V34_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV34History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV34History(authorized, "pending");
    ...
    await buildCalendarLiveV34Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V34_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV34AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV34Lifecycle(), null, 2)}\n`,
  );
