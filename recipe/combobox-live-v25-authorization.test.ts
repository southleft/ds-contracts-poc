...
import { COMBOBOX_LIVE_V25_INDEX_PATH } from "./build-combobox-live-proof-v25.js";
import {
  COMBOBOX_LIVE_V25_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V25_TARGET,
} from "./combobox-live-v25-broker.js";
import {
  buildComboboxLiveV25AuthorizationArtifact,
  validateComboboxLiveV25AntecedentIndex,
  validateComboboxLiveV25AuthorizationArtifact,
  validateComboboxLiveV25History,
  validateComboboxLiveV25UnitTestSource,
  type ComboboxLiveV25AntecedentIndex,
  type ComboboxLiveV25AuthorizationArtifact,
  type ComboboxLiveV25AuthorizationProof,
  type ComboboxLiveV25HistoryState,
} from "./combobox-live-v25-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV25ControlFlowSource,
  validateComboboxLiveV25SecurityAttestation,
  type ComboboxLiveV25SecurityAttestation,
} from "./combobox-live-v25-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v25-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V25_INDEX_PATH);
const index = (): ComboboxLiveV25AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV25AntecedentIndex;

const artifact = (): ComboboxLiveV25AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV25AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV25HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV25HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV25AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v25/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV25AuthorizationArtifact,
): ComboboxLiveV25SecurityAttestation => {
  const body: Omit<ComboboxLiveV25SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V25_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV25History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV25History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV25History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV25History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV25History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV25UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV25UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV25AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV25AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV25AuthorizationArtifact(
        planted as ComboboxLiveV25AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV25SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV25SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV25SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV25ControlFlowSource(source), []);
});
