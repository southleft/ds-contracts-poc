...
} from "./input-field-live-v3-verifier-v72.js";
import {
  buildInputLiveV72RawPropertyRuntime,
  normalizeInputLiveV72Scene,
  type InputLiveV72RawNode,
} from "./input-field-live-v72-verifier.js";
...
export const INPUT_LIVE_V72_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V72_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V72_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V72_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V72_VARIANT_COUNT = 256;
export const INPUT_LIVE_V72_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V72_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV72SourceId = (typeof INPUT_LIVE_V72_SOURCE_IDS)[number];

export interface InputLiveV72SourceIdentity {
  source: InputLiveV72SourceId;
...
export interface InputLiveV72WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV72ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV72SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV72RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV72RootProof {
  source: InputLiveV72SourceId;
  adapterIdentity: string;
...
export interface InputLiveV72ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV72SourceId;
    adapterIdentity: string;
...
export interface InputLiveV72CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV72SourceId;
  adapterIdentity: string;
...
export interface InputLiveV72CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV72SourceId;
  strata: InputLiveV72CaptureCell["strata"];
...
export interface InputLiveV72CleanupPayload {
...
export function validateInputLiveV72WriterPayload(
  value: unknown,
): InputLiveV72WriterOwnership {
...
const identityPlan = (source: InputLiveV72SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV72ExtractProgram(
  writer: InputLiveV72WriterOwnership,
  sources: readonly InputLiveV72SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V72_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V72_NAMESPACE)}
${buildInputLiveV72RawPropertyRuntime()}
...
export function validateInputLiveV72ExtractPayload(
  value: unknown,
  writer: InputLiveV72WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V72_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV72ExtractPayload;
}

export function proveInputLiveV72Roots<Instance>(
  extract: InputLiveV72ExtractPayload,
  sources: readonly (InputLiveV72SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV72Scene(
      root.scene,
...
export function assertInputLiveV72RootProofs(
  proofs: readonly InputLiveV72RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV72ProbeProgram(
  writer: InputLiveV72WriterOwnership,
  sources: readonly InputLiveV72SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V72_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV72ProbePayload(
  value: unknown,
  writer: InputLiveV72WriterOwnership,
): InputLiveV72ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV72ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V72_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V72_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV72CaptureManifest(
  cells: readonly InputLiveV72CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V72_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V72_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V72_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V72_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V72_SOURCE_IDS.some(
...
export const inputLiveV72CaptureManifestSha256 = (
  cells: readonly InputLiveV72CaptureCell[],
): string => {
  validateInputLiveV72CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV72CaptureProgram(
  writer: InputLiveV72WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V72_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V72_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V72_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV72CapturePayload(
  value: unknown,
  cell: InputLiveV72CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV72CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V72_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V72_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV72CaptureResponses(
  manifest: readonly InputLiveV72CaptureCell[],
  responses: readonly InputLiveV72CapturePayload[],
): void {
  validateInputLiveV72CaptureManifest(manifest);
  if (
...
export interface InputLiveV72ObjectiveReport {
  artifactVersion: "input-live-v72-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV72Objective(
  manifest: readonly InputLiveV72CaptureCell[],
  responses: readonly InputLiveV72CapturePayload[],
  readReference: (cell: InputLiveV72CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV72CaptureCell) => Buffer,
): InputLiveV72ObjectiveReport {
  assertInputLiveV72CaptureResponses(manifest, responses);
...
export function buildInputLiveV72CleanupProgram(
  writer: InputLiveV72WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V72_NAMESPACE,
...
export function validateInputLiveV72CleanupPayload(
  value: unknown,
  writer: InputLiveV72WriterOwnership,
): InputLiveV72CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV72CleanupPayload;
...
export const INPUT_LIVE_V72_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV72WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
