...
import { buildTableLiveV38CleanupRuntime } from "./table-live-v38-cleanup.js";
import { type TableLiveV38FixedFixedPoint } from "./table-live-v38-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV38RestoreProgram,
  validateTableLiveV38RestorePayload,
  type TableLiveV38RestorePayload,
} from "./table-live-v38-restore.js";
import {
  buildTableLiveV38RawPropertyRuntime,
  normalizeTableLiveV38Scene,
  type TableLiveV38RawNode,
} from "./table-live-v38-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v38.js";
import {
...
export const TABLE_LIVE_V38_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V38_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V38_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V38_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V38_VARIANT_COUNT = 20;
export const TABLE_LIVE_V38_SET_COUNT = 6;
export const TABLE_LIVE_V38_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V38_HOST_PHASES = 3;
export const TABLE_LIVE_V38_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V38_RESTORE_COUNT = 8;
export const TABLE_LIVE_V38_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V38_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV38SourceId = (typeof TABLE_LIVE_V38_SOURCE_IDS)[number];
export type TableLiveV38Kind = "table" | "row" | "cell";

export interface TableLiveV38SourceIdentity {
  source: TableLiveV38SourceId;
...
export interface TableLiveV38WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV38ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV38SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV38RawNode;
    rowScene: TableLiveV38RawNode;
    cellScene: TableLiveV38RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV38RootProof {
  source: TableLiveV38SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV38FixedFixedPoint;
}

export interface TableLiveV38ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV38SourceId;
    adapterIdentity: string;
...
export interface TableLiveV38CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV38SourceId;
  adapterIdentity: string;
...
export interface TableLiveV38CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV38SourceId;
  frameWidth: number;
...
export interface TableLiveV38CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v38 ${label} must be an object`);
...
export function validateTableLiveV38WriterPayload(
  value: unknown,
): TableLiveV38WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V38_NAMESPACE ||
    ...
    throw new TypeError("Table live v38 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Table live v38 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v38 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV38SourceIdentity,
  kind: TableLiveV38Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV38ExtractProgram(
  writer: TableLiveV38WriterOwnership,
  sources: readonly TableLiveV38SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
      "Table live v38 extract requires two source identities",
    );
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V38_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V38_NAMESPACE)}
${buildTableLiveV38RawPropertyRuntime()}
...
export function validateTableLiveV38ExtractPayload(
  value: unknown,
  writer: TableLiveV38WriterOwnership,
): TableLiveV38ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v38 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V38_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v38 extract schema/two-root mismatch");
  return payload as TableLiveV38ExtractPayload;
}

export function proveTableLiveV38Roots<Instance>(
  extract: TableLiveV38ExtractPayload,
  sources: readonly (TableLiveV38SourceIdentity & {
...
): TableLiveV38RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v38 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v38 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV38Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV38Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV38Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV38FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Table live v38 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v38 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v38 compile lost owned sets");
...
export function assertTableLiveV38RootProofs(
  proofs: readonly TableLiveV38RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v38 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v38 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV38ProbeProgram(
  writer: TableLiveV38WriterOwnership,
  sources: readonly TableLiveV38SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V38_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV38ProbePayload(
  value: unknown,
  writer: TableLiveV38WriterOwnership,
): TableLiveV38ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV38ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V38_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v38 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV38CaptureManifest(
  cells: readonly TableLiveV38CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V38_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V38_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V38_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V38_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v38 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV38CaptureCell[],
): string => {
  validateTableLiveV38CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV38CaptureProgram(
  writer: TableLiveV38WriterOwnership,
  cell: TableLiveV38CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V38_CAPTURE_COUNT
  )
    throw new TypeError("Table live v38 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Table live v38 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V38_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV38CapturePayload(
  value: unknown,
  cell: TableLiveV38CaptureCell,
  rawResponseBytes: number,
): TableLiveV38CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV38CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V38_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V38_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v38 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV38CaptureResponses(
  manifest: readonly TableLiveV38CaptureCell[],
  responses: readonly TableLiveV38CapturePayload[],
): void {
  validateTableLiveV38CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v38 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV38ObjectiveReport {
  artifactVersion: "table-live-v38-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV38Objective(
  manifest: readonly TableLiveV38CaptureCell[],
  responses: readonly TableLiveV38CapturePayload[],
): TableLiveV38ObjectiveReport {
  assertTableLiveV38CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v38-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV38CleanupProgram(
  writer: TableLiveV38WriterOwnership,
): string {
  const baseProgram = buildTableLiveV38CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V38_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v38 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV38CleanupPayload(
  payload: unknown,
  writer: TableLiveV38WriterOwnership,
): TableLiveV38CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as TableLiveV38CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v38 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V38_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV38WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
