...
import { verifyCalendarLiveV23Authorization } from "./calendar-live-v23-authorization.js";
import { CALENDAR_LIVE_V23_TARGET } from "./calendar-live-v23-broker.js";
import {
  CALENDAR_LIVE_V23_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV23Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v23-session";
...
const attestation: CalendarLiveV23SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV23SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v23 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V23_SECURITY_ATTESTATION_DEFAULT_PATH,
);
