/**
 * Carried Combobox live v35 host verifier. Uses scene-readback-combobox-v35 so live
...
export const COMBOBOX_LIVE_V35_FIXED_VERIFIER_VERSION = "combobox-live-v35-fixed-point-v1";
export const COMBOBOX_LIVE_V35_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV35FixedThresholds {
...
export interface ComboboxLiveV35FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV35FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV35FixedSourceProbe {
...
export interface ComboboxLiveV35FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV35FixedFixedPoint;
}

export interface ComboboxLiveV35FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV35FixedHardGateInput {
  thresholds: ComboboxLiveV35FixedThresholds;
  materialRegression: ComboboxLiveV35FixedMaterialRegression;
  sourceProbes: ComboboxLiveV35FixedSourceProbe[];
  cells: ComboboxLiveV35FixedCellValidation[];
  sceneProofs: ComboboxLiveV35FixedSceneProof[];
  visualRows: ComboboxLiveV35FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV35FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V35_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV35FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV35FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV35FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV35FixedHardGates(
  input: ComboboxLiveV35FixedHardGateInput,
): ComboboxLiveV35FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV35FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV35FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V35_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV35FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV35FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV35FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
