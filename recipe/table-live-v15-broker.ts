...
import {
  assertTableLiveV15RootProofs,
  TABLE_LIVE_V15_CAPTURE_COUNT,
  TABLE_LIVE_V15_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  TABLE_LIVE_V15_RESPONSE_CONTRACTS,
  type TableLiveV15RootProof,
} from "./table-live-v15-contract.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V15_BROKER_VERSION = "table-live-v15-broker-v1";
export const TABLE_LIVE_V15_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const TABLE_LIVE_V15_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const TABLE_LIVE_V15_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const TABLE_LIVE_V15_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const TABLE_LIVE_V15_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const TABLE_LIVE_V15_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type TableLiveV15RemotePhase =
  (typeof TABLE_LIVE_V15_REMOTE_PHASES)[number];
export type TableLiveV15AuthorizationMode = "live" | "simulated";

export interface TableLiveV15CommitPins {
...
export interface TableLiveV15TransactionAuthorization extends TableLiveV15CommitPins {
  mode: TableLiveV15AuthorizationMode;
...
export interface TableLiveV15Manifest {
  artifactVersion: "table-live-v15-broker-manifest-v2";
  brokerVersion: typeof TABLE_LIVE_V15_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof TABLE_LIVE_V15_DYNAMIC_TOOL;
  target: typeof TABLE_LIVE_V15_TARGET;
  authorization: TableLiveV15TransactionAuthorization;
...
export interface TableLiveV15ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface TableLiveV15RequestBody {
  artifactVersion: "table-live-v15-broker-request-v2";
  brokerVersion: typeof TABLE_LIVE_V15_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: TableLiveV15RemotePhase;
  sequence: number;
  ...
  commitPins: TableLiveV15CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof TABLE_LIVE_V15_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: TableLiveV15ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface TableLiveV15Request extends TableLiveV15RequestBody {
...
export interface TableLiveV15Receipt {
  artifactVersion: "table-live-v15-broker-receipt-v2";
  brokerVersion: typeof TABLE_LIVE_V15_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: TableLiveV15RemotePhase;
...
export interface TableLiveV15AcceptedResponse<T = unknown> {
  request: TableLiveV15Request;
  receipt: TableLiveV15Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface TableLiveV15IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: TableLiveV15ResponseContract;
...
const equalTarget = (value: unknown): value is typeof TABLE_LIVE_V15_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === TABLE_LIVE_V15_TARGET.fileKey &&
  (value as BrokerTarget).fileName === TABLE_LIVE_V15_TARGET.fileName &&
  (value as BrokerTarget).editorType === TABLE_LIVE_V15_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== TABLE_LIVE_V15_TARGET.fileKey ||
    context.fileName !== TABLE_LIVE_V15_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === TABLE_LIVE_V15_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Table live v15 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= TABLE_LIVE_V15_CAPTURE_COUNT
  )
    throw new TypeError("Table live v15 capture index must be 0..19");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Table live v15 authorization pins are malformed");
};

const readManifest = (directory: string): TableLiveV15Manifest => {
  const manifest = json<TableLiveV15Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Table live v15 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "table-live-v15-broker-manifest-v2" ||
    manifest.brokerVersion !== TABLE_LIVE_V15_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== TABLE_LIVE_V15_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V15_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Table live v15 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: TableLiveV15Request,
): TableLiveV15RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: TableLiveV15RemotePhase,
  captureIndex?: number,
): TableLiveV15Request | undefined => {
  const file = tableLiveV2RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV15Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: TableLiveV15RemotePhase,
  captureIndex?: number,
): TableLiveV15Receipt | undefined => {
  const file = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV15Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: TableLiveV15Request | undefined;
  receipt: TableLiveV15Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: TableLiveV15RemotePhase,
): TableLiveV15ResponseContract => ({
  ...TABLE_LIVE_V15_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: TABLE_LIVE_V15_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: TABLE_LIVE_V15_DYNAMIC_TOOL,
      target: TABLE_LIVE_V15_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "table-live-v15-plugin-response-v2",
  brokerVersion: TABLE_LIVE_V15_BROKER_VERSION,
  target: TABLE_LIVE_V15_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*TABLE_LIVE_V15_PROGRAM_START*/${program}/*TABLE_LIVE_V15_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*TABLE_LIVE_V15_PROGRAM_START*/";
  const endMarker = "/*TABLE_LIVE_V15_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Table live v15 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createTableLiveV15Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Table live v15 private signing key does not match authorized public-key identity",
    );
  const manifest: TableLiveV15Manifest = {
    artifactVersion: "table-live-v15-broker-manifest-v2",
    brokerVersion: TABLE_LIVE_V15_BROKER_VERSION,
    ...
    captureRequests: TABLE_LIVE_V15_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateTableLiveV15Request(
  directory: string,
  request: TableLiveV15Request,
): void {
  ...
  if (
    request.artifactVersion !== "table-live-v15-broker-request-v2" ||
    request.brokerVersion !== TABLE_LIVE_V15_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== TABLE_LIVE_V15_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Table live v15 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueTableLiveV15Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: TableLiveV15RemotePhase,
  program: string,
  options: TableLiveV15IssueOptions = {},
): TableLiveV15Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<TableLiveV15Request>(targetPath);
    validateTableLiveV15Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Table live v15 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(tableLiveV2ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Table live v15 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Table live v15 empty code envelope refused");
  if (phase === "capture") validateTableLiveV15TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Table live v15 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: TABLE_LIVE_V15_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? TABLE_LIVE_V15_SIGNED_WRITER_TIMEOUT_MS
        : TABLE_LIVE_V15_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: TableLiveV15RequestBody = {
    artifactVersion: "table-live-v15-broker-request-v2",
    brokerVersion: TABLE_LIVE_V15_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: TableLiveV15Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateTableLiveV15Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistTableLiveV15RawResponse(
  directory: string,
  phase: TableLiveV15RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Table live v15 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<TableLiveV15Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Table live v15 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = tableLiveV2ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Table live v15 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<TableLiveV15Receipt, "receiptSha256"> = {
    artifactVersion: "table-live-v15-broker-receipt-v2",
    brokerVersion: TABLE_LIVE_V15_BROKER_VERSION,
...
export function acceptTableLiveV15Response<T = unknown>(
  directory: string,
  phase: TableLiveV15RemotePhase,
  options: {
    captureIndex?: number;
...
): TableLiveV15AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Table live v15 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<TableLiveV15Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Table live v15 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "table-live-v15-plugin-response-v2" ||
    plugin?.brokerVersion !== TABLE_LIVE_V15_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Table live v15 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistTableLiveV15TechnicalGates(
  directory: string,
  rootProofs: readonly TableLiveV15RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertTableLiveV15RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Table live v15 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Table live v15 probe payload hash malformed");
  const gates = Object.fromEntries(
    TABLE_LIVE_V15_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "table-live-v15-host-technical-gates-v2",
    brokerVersion: TABLE_LIVE_V15_BROKER_VERSION,
...
export function validateTableLiveV15TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "table-live-v15-host-technical-gates-v2" ||
    ...
    TABLE_LIVE_V15_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Table live v15 technical gates artifact");
}

export function validateTableLiveV15Receipt(
  directory: string,
  phase: TableLiveV15RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateTableLiveV15Request(directory, request);
  const receipt = json<TableLiveV15Receipt>(
  ...
  if (
    receipt.artifactVersion !== "table-live-v15-broker-receipt-v2" ||
    receipt.brokerVersion !== TABLE_LIVE_V15_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Table live v15 accepted receipt: ${request.requestId}`,
    );
}

export function readTableLiveV15AcceptedRawPayload(
  directory: string,
  phase: TableLiveV15RemotePhase,
  captureIndex?: number,
): unknown {
  validateTableLiveV15Receipt(directory, phase, captureIndex);
...
export function createTableLiveV15FakeDynamicResponse(
  request: TableLiveV15Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? TABLE_LIVE_V15_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? TABLE_LIVE_V15_TARGET,
    ...overrides.extra,
  };
}

export function listTableLiveV15PersistedRequests(
  directory: string,
): TableLiveV15Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<TableLiveV15Request>(file)] : [];
    });
}
