...
import { CALENDAR_LIVE_V26_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v26.js";
import { verifyCalendarLiveV26Authorization } from "./calendar-live-v26-authorization.js";
import { runCalendarLiveV26Preflight } from "./calendar-live-v26-preflight.js";
import { CalendarLiveV26Orchestrator } from "./run-calendar-live-v26.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v26 requires exact authorization artifact ${CALENDAR_LIVE_V26_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v26-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV26Authorization(root);
    runCalendarLiveV26Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV26Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV26Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
