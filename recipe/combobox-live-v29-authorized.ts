...
import { COMBOBOX_LIVE_V29_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v29.js";
import { verifyComboboxLiveV29Authorization } from "./combobox-live-v29-authorization.js";
import { runComboboxLiveV29Preflight } from "./combobox-live-v29-preflight.js";
import { ComboboxLiveV29Orchestrator } from "./run-combobox-live-v29.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v29 requires exact authorization artifact ${COMBOBOX_LIVE_V29_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v29-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV29Authorization(root);
    runComboboxLiveV29Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV29Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV29Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
