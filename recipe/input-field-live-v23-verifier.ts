...
export const INPUT_LIVE_V23_VERIFIER_VERSION = "input-live-v23-verifier-v1";

export function buildInputLiveV23RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV23RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV23RawNode[];
}

export interface InputLiveV23NormalizedScene {
...
export interface InputLiveV23SceneProof {
...
export function normalizeInputLiveV23Scene(
  root: InputLiveV23RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV23NormalizedScene {
  const canonicalBindings: InputLiveV23NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV23RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV23Scene<Instance>(
  normalized: InputLiveV23NormalizedScene,
...
export function compileInputLiveV23ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV23PreCaptureGates(payload: unknown): void {
