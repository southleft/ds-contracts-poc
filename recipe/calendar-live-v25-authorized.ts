...
import { CALENDAR_LIVE_V25_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v25.js";
import { verifyCalendarLiveV25Authorization } from "./calendar-live-v25-authorization.js";
import { runCalendarLiveV25Preflight } from "./calendar-live-v25-preflight.js";
import { CalendarLiveV25Orchestrator } from "./run-calendar-live-v25.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v25 requires exact authorization artifact ${CALENDAR_LIVE_V25_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v25-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV25Authorization(root);
    runCalendarLiveV25Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV25Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV25Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
