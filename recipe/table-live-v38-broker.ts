...
import {
  assertTableLiveV38RootProofs,
  TABLE_LIVE_V38_CAPTURE_COUNT,
  TABLE_LIVE_V38_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  TABLE_LIVE_V38_RESPONSE_CONTRACTS,
  type TableLiveV38RootProof,
} from "./table-live-v38-contract.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V38_BROKER_VERSION = "table-live-v38-broker-v1";
export const TABLE_LIVE_V38_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const TABLE_LIVE_V38_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const TABLE_LIVE_V38_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const TABLE_LIVE_V38_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const TABLE_LIVE_V38_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const TABLE_LIVE_V38_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type TableLiveV38RemotePhase =
  (typeof TABLE_LIVE_V38_REMOTE_PHASES)[number];
export type TableLiveV38AuthorizationMode = "live" | "simulated";

export interface TableLiveV38CommitPins {
...
export interface TableLiveV38TransactionAuthorization extends TableLiveV38CommitPins {
  mode: TableLiveV38AuthorizationMode;
...
export interface TableLiveV38Manifest {
  artifactVersion: "table-live-v38-broker-manifest-v2";
  brokerVersion: typeof TABLE_LIVE_V38_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof TABLE_LIVE_V38_DYNAMIC_TOOL;
  target: typeof TABLE_LIVE_V38_TARGET;
  authorization: TableLiveV38TransactionAuthorization;
...
export interface TableLiveV38ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface TableLiveV38RequestBody {
  artifactVersion: "table-live-v38-broker-request-v2";
  brokerVersion: typeof TABLE_LIVE_V38_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: TableLiveV38RemotePhase;
  sequence: number;
  ...
  commitPins: TableLiveV38CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof TABLE_LIVE_V38_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: TableLiveV38ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface TableLiveV38Request extends TableLiveV38RequestBody {
...
export interface TableLiveV38Receipt {
  artifactVersion: "table-live-v38-broker-receipt-v2";
  brokerVersion: typeof TABLE_LIVE_V38_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: TableLiveV38RemotePhase;
...
export interface TableLiveV38AcceptedResponse<T = unknown> {
  request: TableLiveV38Request;
  receipt: TableLiveV38Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface TableLiveV38IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: TableLiveV38ResponseContract;
...
const equalTarget = (value: unknown): value is typeof TABLE_LIVE_V38_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === TABLE_LIVE_V38_TARGET.fileKey &&
  (value as BrokerTarget).fileName === TABLE_LIVE_V38_TARGET.fileName &&
  (value as BrokerTarget).editorType === TABLE_LIVE_V38_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== TABLE_LIVE_V38_TARGET.fileKey ||
    context.fileName !== TABLE_LIVE_V38_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === TABLE_LIVE_V38_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Table live v38 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= TABLE_LIVE_V38_CAPTURE_COUNT
  )
    throw new TypeError("Table live v38 capture index must be 0..19");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Table live v38 authorization pins are malformed");
};

const readManifest = (directory: string): TableLiveV38Manifest => {
  const manifest = json<TableLiveV38Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Table live v38 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "table-live-v38-broker-manifest-v2" ||
    manifest.brokerVersion !== TABLE_LIVE_V38_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== TABLE_LIVE_V38_CAPTURE_COUNT ||
    ...
    throw new TypeError("invalid Table live v38 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: TableLiveV38Request,
): TableLiveV38RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: TableLiveV38RemotePhase,
  captureIndex?: number,
): TableLiveV38Request | undefined => {
  const file = tableLiveV2RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV38Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: TableLiveV38RemotePhase,
  captureIndex?: number,
): TableLiveV38Receipt | undefined => {
  const file = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV38Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: TableLiveV38Request | undefined;
  receipt: TableLiveV38Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: TableLiveV38RemotePhase,
): TableLiveV38ResponseContract => ({
  ...TABLE_LIVE_V38_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: TABLE_LIVE_V38_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "table-live-v38-plugin-response-v2",
  brokerVersion: TABLE_LIVE_V38_BROKER_VERSION,
  target: TABLE_LIVE_V38_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*TABLE_LIVE_V38_PROGRAM_START*/${program}/*TABLE_LIVE_V38_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*TABLE_LIVE_V38_PROGRAM_START*/";
  const endMarker = "/*TABLE_LIVE_V38_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Table live v38 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createTableLiveV38Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Table live v38 private signing key does not match authorized public-key identity",
    );
  const manifest: TableLiveV38Manifest = {
    artifactVersion: "table-live-v38-broker-manifest-v2",
    brokerVersion: TABLE_LIVE_V38_BROKER_VERSION,
    ...
    captureRequests: TABLE_LIVE_V38_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateTableLiveV38Request(
  directory: string,
  request: TableLiveV38Request,
): void {
  ...
  if (
    request.artifactVersion !== "table-live-v38-broker-request-v2" ||
    request.brokerVersion !== TABLE_LIVE_V38_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== TABLE_LIVE_V38_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Table live v38 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueTableLiveV38Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: TableLiveV38RemotePhase,
  program: string,
  options: TableLiveV38IssueOptions = {},
): TableLiveV38Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<TableLiveV38Request>(targetPath);
    validateTableLiveV38Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Table live v38 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(tableLiveV2ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Table live v38 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Table live v38 empty code envelope refused");
  if (phase === "capture") validateTableLiveV38TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Table live v38 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: TABLE_LIVE_V38_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? TABLE_LIVE_V38_SIGNED_WRITER_TIMEOUT_MS
        : TABLE_LIVE_V38_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: TableLiveV38RequestBody = {
    artifactVersion: "table-live-v38-broker-request-v2",
    brokerVersion: TABLE_LIVE_V38_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: TableLiveV38Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateTableLiveV38Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistTableLiveV38RawResponse(
  directory: string,
  phase: TableLiveV38RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Table live v38 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<TableLiveV38Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Table live v38 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = tableLiveV2ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Table live v38 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<TableLiveV38Receipt, "receiptSha256"> = {
    artifactVersion: "table-live-v38-broker-receipt-v2",
    brokerVersion: TABLE_LIVE_V38_BROKER_VERSION,
...
export function acceptTableLiveV38Response<T = unknown>(
  directory: string,
  phase: TableLiveV38RemotePhase,
  options: {
    captureIndex?: number;
...
): TableLiveV38AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Table live v38 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<TableLiveV38Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Table live v38 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "table-live-v38-plugin-response-v2" ||
    plugin?.brokerVersion !== TABLE_LIVE_V38_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Table live v38 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistTableLiveV38TechnicalGates(
  directory: string,
  rootProofs: readonly TableLiveV38RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertTableLiveV38RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Table live v38 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Table live v38 probe payload hash malformed");
  const gates = Object.fromEntries(
    TABLE_LIVE_V38_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "table-live-v38-host-technical-gates-v2",
    brokerVersion: TABLE_LIVE_V38_BROKER_VERSION,
...
export function validateTableLiveV38TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "table-live-v38-host-technical-gates-v2" ||
    ...
    TABLE_LIVE_V38_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Table live v38 technical gates artifact");
}

export function validateTableLiveV38Receipt(
  directory: string,
  phase: TableLiveV38RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateTableLiveV38Request(directory, request);
  const receipt = json<TableLiveV38Receipt>(
  ...
  if (
    receipt.artifactVersion !== "table-live-v38-broker-receipt-v2" ||
    receipt.brokerVersion !== TABLE_LIVE_V38_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Table live v38 accepted receipt: ${request.requestId}`,
    );
}

export function readTableLiveV38AcceptedRawPayload(
  directory: string,
  phase: TableLiveV38RemotePhase,
  captureIndex?: number,
): unknown {
  validateTableLiveV38Receipt(directory, phase, captureIndex);
...
export function createTableLiveV38FakeDynamicResponse(
  request: TableLiveV38Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? TABLE_LIVE_V38_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? TABLE_LIVE_V38_TARGET,
    ...overrides.extra,
  };
}

export function listTableLiveV38PersistedRequests(
  directory: string,
): TableLiveV38Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<TableLiveV38Request>(file)] : [];
    });
}
