...
export type CalendarLiveV50HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV50AuthorizationArtifact {
  artifactVersion: "calendar-live-v50-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V50_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV50AntecedentIndex {
...
export interface CalendarLiveV50HistoryState {
  index?: CalendarLiveV50AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV50AuthorizationArtifact;
...
export interface CalendarLiveV50AuthorizationProof
  extends CalendarLiveV50TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V50_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V50_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V50_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V50_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v50-authorization") ||
  artifactPath.includes("calendar-live-v50-preflight") ||
  artifactPath.includes("calendar-live-v50-authorized") ||
  artifactPath.includes("create-calendar-live-v50-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v50-status") ||
...
export function validateCalendarLiveV50AntecedentIndex(
  index: CalendarLiveV50AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v50-antecedent-index-v1" ||
...
export function validateCalendarLiveV50UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV50History\(\s*["']/.test(source) ||
    /readCalendarLiveV50HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV50AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV50AuthorizationArtifact {
  ...
  ) as CalendarLiveV50AntecedentIndex;
  const indexFailures = validateCalendarLiveV50AntecedentIndex(index);
...
export function validateCalendarLiveV50AuthorizationArtifact(
  artifact: CalendarLiveV50AuthorizationArtifact | undefined,
  index: CalendarLiveV50AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v50-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v50" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V50_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V50_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V50_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V50_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V50_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V50_SOURCE_ROOTS ||
  ...
  if (
    ...
    artifact?.execution?.comboboxLiveV50AuthorizationReusable !== false ||
...
export function validateCalendarLiveV50History(
  state: CalendarLiveV50HistoryState,
  expected: CalendarLiveV50HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV50AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV50AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV50HistoryState(
  root = process.cwd(),
): CalendarLiveV50HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V50_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV50AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V50_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V50_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V50_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV50History(
  expected: CalendarLiveV50HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV50HistoryState {
  const state = readCalendarLiveV50HistoryState(root);
  const failures = validateCalendarLiveV50History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v50 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV50Authorization(
  root = process.cwd(),
): CalendarLiveV50AuthorizationProof {
  const state = verifyCalendarLiveV50History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V50_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v50/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v50.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V50_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V50_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V50_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV50HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV50History(expected);
  process.stdout.write(
    `Calendar live v50 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
