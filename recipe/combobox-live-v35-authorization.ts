...
export type ComboboxLiveV35HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV35AuthorizationArtifact {
  artifactVersion: "combobox-live-v35-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V35_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV35AntecedentIndex {
...
export interface ComboboxLiveV35HistoryState {
  index?: ComboboxLiveV35AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV35AuthorizationArtifact;
...
export interface ComboboxLiveV35AuthorizationProof extends ComboboxLiveV35TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V35_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V35_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V35_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V35_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v35-authorization") ||
  artifactPath.includes("combobox-live-v35-preflight") ||
  artifactPath.includes("combobox-live-v35-authorized") ||
  artifactPath.includes("create-combobox-live-v35-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v35-status") ||
...
export function validateComboboxLiveV35AntecedentIndex(
  index: ComboboxLiveV35AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v35-antecedent-index-v1" ||
...
export function validateComboboxLiveV35UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV35History\(\s*["']/.test(source) ||
    /readComboboxLiveV35HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV35AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV35AuthorizationArtifact {
  ...
  ) as ComboboxLiveV35AntecedentIndex;
  const indexFailures = validateComboboxLiveV35AntecedentIndex(index);
...
export function validateComboboxLiveV35AuthorizationArtifact(
  artifact: ComboboxLiveV35AuthorizationArtifact | undefined,
  index: ComboboxLiveV35AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v35-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v35" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V35_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V35_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V35_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V35_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V35_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V35_SOURCE_ROOTS ||
...
export function validateComboboxLiveV35History(
  state: ComboboxLiveV35HistoryState,
  expected: ComboboxLiveV35HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV35AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV35AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV35HistoryState(
  root = process.cwd(),
): ComboboxLiveV35HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V35_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV35AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V35_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V35_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V35_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV35History(
  expected: ComboboxLiveV35HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV35HistoryState {
  const state = readComboboxLiveV35HistoryState(root);
  const failures = validateComboboxLiveV35History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v35 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV35Authorization(
  root = process.cwd(),
): ComboboxLiveV35AuthorizationProof {
  const state = verifyComboboxLiveV35History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V35_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v35/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v35.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V35_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V35_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V35_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV35HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV35History(expected);
  process.stdout.write(
    `Combobox live v35 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
