/**
 * Carried Combobox live v14 host verifier. Uses scene-readback-combobox-v14 so live
...
export const COMBOBOX_LIVE_V14_FIXED_VERIFIER_VERSION = "combobox-live-v14-fixed-point-v1";
export const COMBOBOX_LIVE_V14_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV14FixedThresholds {
...
export interface ComboboxLiveV14FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV14FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV14FixedSourceProbe {
...
export interface ComboboxLiveV14FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV14FixedFixedPoint;
}

export interface ComboboxLiveV14FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV14FixedHardGateInput {
  thresholds: ComboboxLiveV14FixedThresholds;
  materialRegression: ComboboxLiveV14FixedMaterialRegression;
  sourceProbes: ComboboxLiveV14FixedSourceProbe[];
  cells: ComboboxLiveV14FixedCellValidation[];
  sceneProofs: ComboboxLiveV14FixedSceneProof[];
  visualRows: ComboboxLiveV14FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV14FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V14_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV14FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV14FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV14FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV14FixedHardGates(
  input: ComboboxLiveV14FixedHardGateInput,
): ComboboxLiveV14FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV14FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV14FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V14_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV14FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV14FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV14FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
