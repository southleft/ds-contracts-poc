...
import { verifyComboboxLiveV10Authorization } from "./combobox-live-v10-authorization.js";
import { COMBOBOX_LIVE_V10_TARGET } from "./combobox-live-v10-broker.js";
import {
  COMBOBOX_LIVE_V10_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV10Authorization();
...
const attestation: ComboboxLiveV10SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV10SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v10 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V10_SECURITY_ATTESTATION_DEFAULT_PATH,
);
