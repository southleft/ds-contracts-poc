...
export const INPUT_LIVE_V8_VERIFIER_VERSION = "input-live-v8-verifier-v1";

export function buildInputLiveV8RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV8RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV8RawNode[];
}

export interface InputLiveV8NormalizedScene {
...
export interface InputLiveV8SceneProof {
...
export function normalizeInputLiveV8Scene(
  root: InputLiveV8RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV8NormalizedScene {
  const canonicalBindings: InputLiveV8NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: InputLiveV8RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV8Scene<Instance>(
  normalized: InputLiveV8NormalizedScene,
...
export function compileInputLiveV8ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV8PreCaptureGates(payload: unknown): void {
