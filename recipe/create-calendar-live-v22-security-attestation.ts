...
import { verifyCalendarLiveV22Authorization } from "./calendar-live-v22-authorization.js";
import { CALENDAR_LIVE_V22_TARGET } from "./calendar-live-v22-broker.js";
import {
  CALENDAR_LIVE_V22_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV22Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v22-session";
...
const attestation: CalendarLiveV22SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV22SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v22 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V22_SECURITY_ATTESTATION_DEFAULT_PATH,
);
