...
import {
  ...
  INPUT_LIVE_V25_INDEX_PATH,
} from "./build-input-field-live-proof-v25.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV25HistoryExpectation = "pending" | "authorized";

export interface InputLiveV25AuthorizationArtifact {
  artifactVersion: "input-live-v25-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V25_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV25AntecedentIndex {
...
export interface InputLiveV25HistoryState {
  index?: InputLiveV25AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV25AuthorizationArtifact;
...
export interface InputLiveV25AuthorizationProof extends InputLiveV25TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V25_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V25_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V25_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V25_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v25-status") ||
...
export function validateInputLiveV25AntecedentIndex(
  index: InputLiveV25AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v25-antecedent-index-v1" ||
...
export function validateInputLiveV25UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV25History\(\s*["']/.test(source) ||
    /readInputLiveV25HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV25AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV25AuthorizationArtifact {
  ...
  ) as InputLiveV25AntecedentIndex;
  const indexFailures = validateInputLiveV25AntecedentIndex(index);
...
export function validateInputLiveV25AuthorizationArtifact(
  artifact: InputLiveV25AuthorizationArtifact | undefined,
  index: InputLiveV25AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v25-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v25" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V25_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V25_TARGET) ||
    !equalJson(
...
export function validateInputLiveV25History(
  state: InputLiveV25HistoryState,
  expected: InputLiveV25HistoryExpectation,
): string[] {
  const failures = validateInputLiveV25AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV25AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV25HistoryState(
  root = process.cwd(),
): InputLiveV25HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V25_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV25AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V25_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V25_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V25_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV25History(
  expected: InputLiveV25HistoryExpectation,
  root = process.cwd(),
): InputLiveV25HistoryState {
  const state = readInputLiveV25HistoryState(root);
  const failures = validateInputLiveV25History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV25Authorization(
  root = process.cwd(),
): InputLiveV25AuthorizationProof {
  const state = verifyInputLiveV25History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V25_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v25/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v25.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V25_TARGET,
    expectedDynamicTool: INPUT_LIVE_V25_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V25_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV25HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV25History(expected);
