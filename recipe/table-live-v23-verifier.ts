...
import {
  verifyTableLiveV23FixedSceneFixedPoint,
  type TableLiveV23FixedFixedPoint,
} from "./table-live-v23-fixed-point.js";

export const TABLE_LIVE_V23_VERIFIER_VERSION = "table-live-v23-verifier-v1";

export function buildTableLiveV23RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV23RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV23RawNode[];
}

export interface TableLiveV23NormalizedScene {
...
export interface TableLiveV23SceneProof {
...
export function normalizeTableLiveV23Scene(
  root: TableLiveV23RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV23NormalizedScene {
  const canonicalBindings: TableLiveV23NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV23RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV23Scene<Instance>(
  normalized: TableLiveV23NormalizedScene,
...
): TableLiveV23SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV23FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV23ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV23PreCaptureGates(payload: unknown): void {
