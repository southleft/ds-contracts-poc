...
import { INPUT_LIVE_V45_INDEX_PATH } from "./build-input-field-live-proof-v45.js";
import {
  INPUT_LIVE_V45_DYNAMIC_TOOL,
  INPUT_LIVE_V45_TARGET,
} from "./input-field-live-v45-broker.js";
import {
  buildInputLiveV45AuthorizationArtifact,
  validateInputLiveV45AntecedentIndex,
  validateInputLiveV45AuthorizationArtifact,
  validateInputLiveV45History,
  validateInputLiveV45UnitTestSource,
  type InputLiveV45AntecedentIndex,
  type InputLiveV45AuthorizationArtifact,
  type InputLiveV45AuthorizationProof,
  type InputLiveV45HistoryState,
} from "./input-field-live-v45-authorization.js";
import {
  inputLiveV45AttestationSha256,
  validateInputLiveV45ControlFlowSource,
  validateInputLiveV45SecurityAttestation,
  type InputLiveV45SecurityAttestation,
} from "./input-field-live-v45-preflight.js";
import { inputLiveV45Sha256 } from "./input-field-live-v45-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V45_INDEX_PATH);
const index = (): InputLiveV45AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV45AntecedentIndex;

const artifact = (): InputLiveV45AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV45AuthorizationArtifact({
...
const pendingState = (): InputLiveV45HistoryState => ({
  index: index(),
  indexSha256: inputLiveV45Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV45HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV45AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV45Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V45_TARGET,
  expectedDynamicTool: INPUT_LIVE_V45_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v45/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV45AuthorizationArtifact,
): InputLiveV45SecurityAttestation => {
  const body: Omit<InputLiveV45SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v45-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V45_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV45AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV45History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV45History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV45History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV45History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV45History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV45UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV45UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV45AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV45Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV45AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV45AuthorizationArtifact(
        planted as InputLiveV45AuthorizationArtifact,
        index(),
        inputLiveV45Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV45SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV45AttestationSha256(
      body as Omit<InputLiveV45SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV45SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV45ControlFlowSource(source), []);
});
