...
import {
  acceptComboboxLiveV8Response,
  createComboboxLiveV8Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV8Request,
  persistComboboxLiveV8RawResponse,
  persistComboboxLiveV8TechnicalGates,
  readComboboxLiveV8AcceptedRawPayload,
  type ComboboxLiveV8Request,
  type ComboboxLiveV8TransactionAuthorization,
} from "./combobox-live-v8-broker.js";
import {
  COMBOBOX_LIVE_V8_CAPTURE_COUNT,
  assertComboboxLiveV8CaptureResponses,
  ...
  evaluateComboboxLiveV8Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV8Roots,
  validateComboboxLiveV8CaptureManifest,
  validateComboboxLiveV8CapturePayload,
  validateComboboxLiveV8CleanupPayload,
  validateComboboxLiveV8ExtractPayload,
  validateComboboxLiveV8ProbePayload,
  validateComboboxLiveV8WriterPayload,
  type ComboboxLiveV8CaptureCell,
  type ComboboxLiveV8CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V8_PLAN_PATH,
  type ComboboxLiveV8ProofPlan,
} from "./build-combobox-live-proof-v8.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v8.js";

export const COMBOBOX_LIVE_V8_RUNNER_VERSION = "combobox-live-v8-runner-v1";

interface ComboboxLiveV8JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV8OrchestratorState {
  artifactVersion: "combobox-live-v8-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV8JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV8ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV8CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV8SourceIdentity & {
...
export interface ComboboxLiveV8NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV8OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v8 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V8_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V8_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV8ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV8CaptureCell[];
  validateComboboxLiveV8CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v8 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v8 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV8WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV8RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV8Request =>
  readJson<ComboboxLiveV8Request>(file);

export class ComboboxLiveV8Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV8Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV8OrchestratorState = {
      artifactVersion: "combobox-live-v8-orchestrator-state-v1",
    ...
    return new ComboboxLiveV8Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV8OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV8Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV8NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV8NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v8 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV8NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v8 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV8RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV8Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV8CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV8AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV8WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV8Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV8ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV8Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV8Response(this.#directory, "writer", {
        validate: validateComboboxLiveV8WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV8Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV8RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV8Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV8ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV8Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV8RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v8-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV8Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV8ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV8Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV8ProbePayload(payload, ownership),
      });
      persistComboboxLiveV8TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v8 capture cell missing");
      const accepted = acceptComboboxLiveV8Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV8CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V8_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV8CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV8Objective(
    ...
    throw new Error(`Combobox live v8 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV8Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV8CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV8Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV8RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV8Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV8CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV8CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV8CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV8Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV8TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV8SimulationKey = (): KeyObject =>
