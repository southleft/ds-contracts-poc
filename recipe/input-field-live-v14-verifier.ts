...
export const INPUT_LIVE_V14_VERIFIER_VERSION = "input-live-v14-verifier-v1";

export function buildInputLiveV14RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV14RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV14RawNode[];
}

export interface InputLiveV14NormalizedScene {
...
export interface InputLiveV14SceneProof {
...
export function normalizeInputLiveV14Scene(
  root: InputLiveV14RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV14NormalizedScene {
  const canonicalBindings: InputLiveV14NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV14RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV14Scene<Instance>(
  normalized: InputLiveV14NormalizedScene,
...
export function compileInputLiveV14ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV14PreCaptureGates(payload: unknown): void {
