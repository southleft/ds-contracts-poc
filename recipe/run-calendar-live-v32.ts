...
import {
  acceptCalendarLiveV32Response,
  createCalendarLiveV32Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV32Request,
  persistCalendarLiveV32RawResponse,
  persistCalendarLiveV32TechnicalGates,
  readCalendarLiveV32AcceptedRawPayload,
  type CalendarLiveV32Request,
  type CalendarLiveV32TransactionAuthorization,
} from "./calendar-live-v32-broker.js";
import {
  CALENDAR_LIVE_V32_CAPTURE_COUNT,
  assertCalendarLiveV32CaptureResponses,
  ...
  evaluateCalendarLiveV32Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV32Roots,
  validateCalendarLiveV32CaptureManifest,
  validateCalendarLiveV32CapturePayload,
  validateCalendarLiveV32CleanupPayload,
  validateCalendarLiveV32ExtractPayload,
  validateCalendarLiveV32ProbePayload,
  validateCalendarLiveV32WriterPayload,
  type CalendarLiveV32CaptureCell,
  type CalendarLiveV32CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V32_PLAN_PATH,
  type CalendarLiveV32ProofPlan,
} from "./build-calendar-live-proof-v32.js";
import {
...
export const CALENDAR_LIVE_V32_RUNNER_VERSION = "calendar-live-v32-runner-v1";

interface CalendarLiveV32JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV32OrchestratorState {
  artifactVersion: "calendar-live-v32-orchestrator-state-v1";
  ...
  journal: CalendarLiveV32JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV32ProofPlan;
  planSha256: string;
  captures: CalendarLiveV32CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV32SourceIdentity & {
...
export interface CalendarLiveV32NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV32OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v32 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V32_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V32_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV32ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV32CaptureCell[];
  validateCalendarLiveV32CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v32 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v32 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV32WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV32RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV32Request =>
  readJson<CalendarLiveV32Request>(file);

export class CalendarLiveV32Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV32Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV32OrchestratorState = {
      artifactVersion: "calendar-live-v32-orchestrator-state-v1",
    ...
    return new CalendarLiveV32Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV32OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV32Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV32NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV32NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v32 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV32NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v32 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV32RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV32Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV32CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV32AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV32WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV32Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV32ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV32Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV32Response(this.#directory, "writer", {
        validate: validateCalendarLiveV32WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV32Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV32RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV32Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV32ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV32Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV32RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v32-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV32Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV32ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV32Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV32ProbePayload(payload, ownership),
      });
      persistCalendarLiveV32TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v32 capture cell missing");
      const accepted = acceptCalendarLiveV32Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV32CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V32_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV32CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV32Objective(
    ...
    throw new Error(`Calendar live v32 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV32Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV32CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV32Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV32RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV32Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV32CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV32CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV32CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV32Authorization = (
  privateKey: KeyObject,
): CalendarLiveV32TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV32SimulationKey = (): KeyObject =>
