...
} from "./input-field-live-v3-verifier-v61.js";
import {
  buildInputLiveV61RawPropertyRuntime,
  normalizeInputLiveV61Scene,
  type InputLiveV61RawNode,
} from "./input-field-live-v61-verifier.js";
...
export const INPUT_LIVE_V61_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V61_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V61_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V61_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V61_VARIANT_COUNT = 256;
export const INPUT_LIVE_V61_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V61_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV61SourceId = (typeof INPUT_LIVE_V61_SOURCE_IDS)[number];

export interface InputLiveV61SourceIdentity {
  source: InputLiveV61SourceId;
...
export interface InputLiveV61WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV61ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV61SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV61RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV61RootProof {
  source: InputLiveV61SourceId;
  adapterIdentity: string;
...
export interface InputLiveV61ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV61SourceId;
    adapterIdentity: string;
...
export interface InputLiveV61CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV61SourceId;
  adapterIdentity: string;
...
export interface InputLiveV61CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV61SourceId;
  strata: InputLiveV61CaptureCell["strata"];
...
export interface InputLiveV61CleanupPayload {
...
export function validateInputLiveV61WriterPayload(
  value: unknown,
): InputLiveV61WriterOwnership {
...
const identityPlan = (source: InputLiveV61SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV61ExtractProgram(
  writer: InputLiveV61WriterOwnership,
  sources: readonly InputLiveV61SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V61_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V61_NAMESPACE)}
${buildInputLiveV61RawPropertyRuntime()}
...
export function validateInputLiveV61ExtractPayload(
  value: unknown,
  writer: InputLiveV61WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V61_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV61ExtractPayload;
}

export function proveInputLiveV61Roots<Instance>(
  extract: InputLiveV61ExtractPayload,
  sources: readonly (InputLiveV61SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV61Scene(
      root.scene,
...
export function assertInputLiveV61RootProofs(
  proofs: readonly InputLiveV61RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV61ProbeProgram(
  writer: InputLiveV61WriterOwnership,
  sources: readonly InputLiveV61SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V61_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV61ProbePayload(
  value: unknown,
  writer: InputLiveV61WriterOwnership,
): InputLiveV61ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV61ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V61_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V61_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV61CaptureManifest(
  cells: readonly InputLiveV61CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V61_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V61_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V61_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V61_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V61_SOURCE_IDS.some(
...
export const inputLiveV61CaptureManifestSha256 = (
  cells: readonly InputLiveV61CaptureCell[],
): string => {
  validateInputLiveV61CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV61CaptureProgram(
  writer: InputLiveV61WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V61_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V61_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V61_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV61CapturePayload(
  value: unknown,
  cell: InputLiveV61CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV61CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V61_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V61_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV61CaptureResponses(
  manifest: readonly InputLiveV61CaptureCell[],
  responses: readonly InputLiveV61CapturePayload[],
): void {
  validateInputLiveV61CaptureManifest(manifest);
  if (
...
export interface InputLiveV61ObjectiveReport {
  artifactVersion: "input-live-v61-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV61Objective(
  manifest: readonly InputLiveV61CaptureCell[],
  responses: readonly InputLiveV61CapturePayload[],
  readReference: (cell: InputLiveV61CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV61CaptureCell) => Buffer,
): InputLiveV61ObjectiveReport {
  assertInputLiveV61CaptureResponses(manifest, responses);
...
export function buildInputLiveV61CleanupProgram(
  writer: InputLiveV61WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V61_NAMESPACE,
...
export function validateInputLiveV61CleanupPayload(
  value: unknown,
  writer: InputLiveV61WriterOwnership,
): InputLiveV61CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV61CleanupPayload;
...
export const INPUT_LIVE_V61_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV61WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
