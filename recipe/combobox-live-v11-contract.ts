...
import { buildComboboxLiveV11CleanupRuntime } from "./combobox-live-v11-cleanup.js";
import {
  type ComboboxLiveV11FixedFixedPoint,
} from "./combobox-live-v11-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV11RestoreProgram,
  validateComboboxLiveV11RestorePayload,
  type ComboboxLiveV11RestorePayload,
} from "./combobox-live-v11-restore.js";
import {
  buildComboboxLiveV11RawPropertyRuntime,
  normalizeComboboxLiveV11Scene,
  type ComboboxLiveV11RawNode,
} from "./combobox-live-v11-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v11.js";
import {
...
} from "./scene-readback-combobox-v11.js";
...
export const COMBOBOX_LIVE_V11_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V11_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V11_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V11_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V11_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V11_SET_COUNT = 4;
export const COMBOBOX_LIVE_V11_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V11_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V11_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V11_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V11_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV11SourceId = (typeof COMBOBOX_LIVE_V11_SOURCE_IDS)[number];

export interface ComboboxLiveV11SourceIdentity {
  source: ComboboxLiveV11SourceId;
...
export interface ComboboxLiveV11WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV11ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV11SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV11RawNode;
    optionScene: ComboboxLiveV11RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV11RootProof {
  source: ComboboxLiveV11SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV11FixedFixedPoint;
}

export interface ComboboxLiveV11ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV11SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV11CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV11SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV11CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV11SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV11CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v11 ${label} must be an object`);
...
export function validateComboboxLiveV11WriterPayload(
  value: unknown,
): ComboboxLiveV11WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V11_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v11 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v11 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v11 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV11SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV11ExtractProgram(
  writer: ComboboxLiveV11WriterOwnership,
  sources: readonly ComboboxLiveV11SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v11 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V11_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V11_NAMESPACE)}
${buildComboboxLiveV11RawPropertyRuntime()}
...
export function validateComboboxLiveV11ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV11WriterOwnership,
): ComboboxLiveV11ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v11 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V11_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v11 extract schema/two-root mismatch");
  return payload as ComboboxLiveV11ExtractPayload;
}

export function proveComboboxLiveV11Roots<Instance>(
  extract: ComboboxLiveV11ExtractPayload,
  sources: readonly (ComboboxLiveV11SourceIdentity & {
...
): ComboboxLiveV11RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v11 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v11 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV11Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV11Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV11FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v11 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v11 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v11 compile lost owned sets");
...
export function assertComboboxLiveV11RootProofs(
  proofs: readonly ComboboxLiveV11RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v11 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v11 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV11ProbeProgram(
  writer: ComboboxLiveV11WriterOwnership,
  sources: readonly ComboboxLiveV11SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V11_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV11ProbePayload(
  value: unknown,
  writer: ComboboxLiveV11WriterOwnership,
): ComboboxLiveV11ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV11ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V11_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V11_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v11 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV11CaptureManifest(
  cells: readonly ComboboxLiveV11CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V11_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V11_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V11_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V11_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v11 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV11CaptureCell[],
): string => {
  validateComboboxLiveV11CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV11CaptureProgram(
  writer: ComboboxLiveV11WriterOwnership,
  cell: ComboboxLiveV11CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V11_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v11 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v11 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V11_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV11CapturePayload(
  value: unknown,
  cell: ComboboxLiveV11CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV11CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV11CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V11_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V11_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v11 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV11CaptureResponses(
  manifest: readonly ComboboxLiveV11CaptureCell[],
  responses: readonly ComboboxLiveV11CapturePayload[],
): void {
  validateComboboxLiveV11CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v11 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV11ObjectiveReport {
  artifactVersion: "combobox-live-v11-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV11Objective(
  manifest: readonly ComboboxLiveV11CaptureCell[],
  responses: readonly ComboboxLiveV11CapturePayload[],
): ComboboxLiveV11ObjectiveReport {
  assertComboboxLiveV11CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v11-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV11CleanupProgram(
  writer: ComboboxLiveV11WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV11CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V11_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v11 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV11CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV11WriterOwnership,
): ComboboxLiveV11CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV11CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v11 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V11_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV11WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
