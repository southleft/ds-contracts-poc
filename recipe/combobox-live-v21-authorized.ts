...
import { COMBOBOX_LIVE_V21_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v21.js";
import { verifyComboboxLiveV21Authorization } from "./combobox-live-v21-authorization.js";
import { runComboboxLiveV21Preflight } from "./combobox-live-v21-preflight.js";
import { ComboboxLiveV21Orchestrator } from "./run-combobox-live-v21.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v21 requires exact authorization artifact ${COMBOBOX_LIVE_V21_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v21-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV21Authorization(root);
    runComboboxLiveV21Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV21Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV21Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
