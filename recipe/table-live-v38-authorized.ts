...
import { TABLE_LIVE_V38_AUTHORIZATION_PATH } from "./build-table-live-proof-v38.js";
import { verifyTableLiveV38Authorization } from "./table-live-v38-authorization.js";
import { runTableLiveV38Preflight } from "./table-live-v38-preflight.js";
import { TableLiveV38Orchestrator } from "./run-table-live-v38.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v38 requires exact authorization artifact ${TABLE_LIVE_V38_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v38-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV38Authorization(root);
    runTableLiveV38Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV38Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV38Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
