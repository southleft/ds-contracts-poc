...
import { INPUT_LIVE_V15_INDEX_PATH } from "./build-input-field-live-proof-v15.js";
import {
  INPUT_LIVE_V15_DYNAMIC_TOOL,
  INPUT_LIVE_V15_TARGET,
} from "./input-field-live-v15-broker.js";
import {
  buildInputLiveV15AuthorizationArtifact,
  validateInputLiveV15AntecedentIndex,
  validateInputLiveV15AuthorizationArtifact,
  validateInputLiveV15History,
  validateInputLiveV15UnitTestSource,
  type InputLiveV15AntecedentIndex,
  type InputLiveV15AuthorizationArtifact,
  type InputLiveV15AuthorizationProof,
  type InputLiveV15HistoryState,
} from "./input-field-live-v15-authorization.js";
import {
  inputLiveV15AttestationSha256,
  validateInputLiveV15ControlFlowSource,
  validateInputLiveV15SecurityAttestation,
  type InputLiveV15SecurityAttestation,
} from "./input-field-live-v15-preflight.js";
import { inputLiveV15Sha256 } from "./input-field-live-v15-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V15_INDEX_PATH);
const index = (): InputLiveV15AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV15AntecedentIndex;

const artifact = (): InputLiveV15AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV15AuthorizationArtifact({
...
const pendingState = (): InputLiveV15HistoryState => ({
  index: index(),
  indexSha256: inputLiveV15Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV15HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV15AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV15Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V15_TARGET,
  expectedDynamicTool: INPUT_LIVE_V15_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v15/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV15AuthorizationArtifact,
): InputLiveV15SecurityAttestation => {
  const body: Omit<InputLiveV15SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v15-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V15_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV15AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV15History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV15History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV15History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV15History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV15History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV15UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV15UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV15AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV15Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV15AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV15AuthorizationArtifact(
        planted as InputLiveV15AuthorizationArtifact,
        index(),
        inputLiveV15Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV15SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV15AttestationSha256(
      body as Omit<InputLiveV15SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV15SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV15ControlFlowSource(source), []);
});
