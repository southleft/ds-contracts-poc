...
import {
  ...
  createInputLiveV30FakeDynamicResponse,
  ...
  listInputLiveV30PersistedRequests,
  persistInputLiveV30RawResponse,
  persistInputLiveV30TechnicalGates,
  validateInputLiveV30Receipt,
  validateInputLiveV30Request,
  type InputLiveV30RemotePhase,
...
import {
  ...
  type InputLiveV30ExtractPayload,
  type InputLiveV30ProbePayload,
...
import {
  ...
  simulatedInputLiveV30Authorization,
} from "./run-input-field-live-v30.js";
...
const captures: InputLiveV30CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV30Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v30-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV30Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV30CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV30FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV30FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V30_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V30_TARGET);
  persistInputLiveV30RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV30Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV30Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV30WriterOwnership,
): InputLiveV30RestorePayload => ({
...
const rootProofs = (): InputLiveV30RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v30-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV30Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV30Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV30RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV30RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV30ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV30ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV30CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV30CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV30CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV30Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV30CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV30PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV30ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV30Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV30Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV30Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV30Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V30_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV30RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V30_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV30Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV30Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV30ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV30Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV30WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV30RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV30Request;
    validateInputLiveV30Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV30Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV30CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v30-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV30Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV30Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV30Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V30_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV30RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V30_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV30Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV30Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV30RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V30_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV30Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV30Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV30Request;
