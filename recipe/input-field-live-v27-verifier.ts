...
export const INPUT_LIVE_V27_VERIFIER_VERSION = "input-live-v27-verifier-v1";

export function buildInputLiveV27RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV27RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV27RawNode[];
}

export interface InputLiveV27NormalizedScene {
...
export interface InputLiveV27SceneProof {
...
export function normalizeInputLiveV27Scene(
  root: InputLiveV27RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV27NormalizedScene {
  const canonicalBindings: InputLiveV27NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV27RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV27Scene<Instance>(
  normalized: InputLiveV27NormalizedScene,
...
export function compileInputLiveV27ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV27PreCaptureGates(payload: unknown): void {
