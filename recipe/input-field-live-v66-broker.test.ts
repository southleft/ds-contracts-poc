...
import {
  ...
  createInputLiveV66FakeDynamicResponse,
  ...
  listInputLiveV66PersistedRequests,
  persistInputLiveV66RawResponse,
  persistInputLiveV66TechnicalGates,
  validateInputLiveV66Receipt,
  validateInputLiveV66Request,
  type InputLiveV66RemotePhase,
...
import {
  ...
  type InputLiveV66ExtractPayload,
  type InputLiveV66ProbePayload,
  type InputLiveV66RestorePayload,
  type InputLiveV66RootProof,
  type InputLiveV66WriterOwnership,
} from "./input-field-live-v66-contract.js";
import {
  InputLiveV66Orchestrator,
  simulatedInputLiveV66Authorization,
} from "./run-input-field-live-v66.js";
...
const captures: InputLiveV66CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV66Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v66-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV66Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV66CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV66FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV66FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V66_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V66_TARGET);
  persistInputLiveV66RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV66Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV66Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV66WriterOwnership,
): InputLiveV66RestorePayload => ({
...
const rootProofs = (): InputLiveV66RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV66CaptureCell,
): InputLiveV66CapturePayload => {
  const bytes = Buffer.from(`input-live-v66-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV66Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV66Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV66RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV66RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV66ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV66ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV66CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV66CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV66CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV66Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV66CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV66PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV66ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV66Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV66Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV66Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV66Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V66_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV66RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V66_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV66Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV66Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV66ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV66Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV66WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV66RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV66Request;
    validateInputLiveV66Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV66Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV66CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v66-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV66Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV66Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV66Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V66_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV66RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V66_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV66Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV66Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV66RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V66_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV66Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV66Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV66Request;
