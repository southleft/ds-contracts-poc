...
import {
  assertInputLiveV56RootProofs,
  INPUT_LIVE_V56_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V56_RESPONSE_CONTRACTS,
  type InputLiveV56RootProof,
} from "./input-field-live-v56-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V56_BROKER_VERSION = "input-live-v56-broker-v2";
export const INPUT_LIVE_V56_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V56_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V56_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V56_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V56_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V56_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV56RemotePhase =
  (typeof INPUT_LIVE_V56_REMOTE_PHASES)[number];
export type InputLiveV56AuthorizationMode = "live" | "simulated";

export interface InputLiveV56CommitPins {
...
export interface InputLiveV56TransactionAuthorization extends InputLiveV56CommitPins {
  mode: InputLiveV56AuthorizationMode;
...
export interface InputLiveV56Manifest {
  artifactVersion: "input-live-v56-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V56_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V56_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V56_TARGET;
  authorization: InputLiveV56TransactionAuthorization;
...
export interface InputLiveV56ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV56RequestBody {
  artifactVersion: "input-live-v56-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V56_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV56RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV56CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V56_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV56ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV56Request extends InputLiveV56RequestBody {
...
export interface InputLiveV56Receipt {
  artifactVersion: "input-live-v56-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V56_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV56RemotePhase;
...
export interface InputLiveV56AcceptedResponse<T = unknown> {
  request: InputLiveV56Request;
  receipt: InputLiveV56Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV56IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV56ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V56_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V56_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V56_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V56_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V56_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V56_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V56_TARGET.editorType;
};
...
export const inputLiveV56RequestSequence = (
  phase: InputLiveV56RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV56RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV56RequestPath = (
  directory: string,
  phase: InputLiveV56RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV56ResponsePath = (
  directory: string,
  phase: InputLiveV56RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV56ReceiptPath = (
  directory: string,
  phase: InputLiveV56RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV56Manifest => {
  const manifest = json<InputLiveV56Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v56-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V56_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V56_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV56RemotePhase,
  captureIndex?: number,
): InputLiveV56Request | undefined => {
  const file = inputLiveV56RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV56Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV56RemotePhase,
  captureIndex?: number,
): InputLiveV56Receipt | undefined => {
  const file = inputLiveV56ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV56Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV56Request | undefined;
  receipt: InputLiveV56Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV56RemotePhase,
): InputLiveV56ResponseContract => ({
  ...INPUT_LIVE_V56_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V56_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V56_DYNAMIC_TOOL,
      target: INPUT_LIVE_V56_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v56-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V56_BROKER_VERSION,
  target: INPUT_LIVE_V56_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V56_PROGRAM_START*/${program}/*INPUT_LIVE_V56_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V56_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V56_PROGRAM_END*/";
...
export function createInputLiveV56Transaction(
  directory: string,
  ...
  const manifest: InputLiveV56Manifest = {
    artifactVersion: "input-live-v56-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V56_BROKER_VERSION,
...
export function validateInputLiveV56Request(
  directory: string,
  request: InputLiveV56Request,
): void {
  ...
  const expectedSequence = inputLiveV56RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v56-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V56_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V56_TARGET.fileKey ||
...
export function issueInputLiveV56Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV56RemotePhase,
  program: string,
  options: InputLiveV56IssueOptions = {},
): InputLiveV56Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV56RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV56Request>(targetPath);
    validateInputLiveV56Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV56ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV56TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV56RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V56_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V56_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V56_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV56RequestBody = {
    artifactVersion: "input-live-v56-broker-request-v2",
    brokerVersion: INPUT_LIVE_V56_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV56Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV56Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV56RawResponse(
  directory: string,
  phase: InputLiveV56RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV56RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV56Request>(requestFile);
  if (
  ...
  const target = inputLiveV56ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV56Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v56-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V56_BROKER_VERSION,
...
export function acceptInputLiveV56Response<T = unknown>(
  directory: string,
  phase: InputLiveV56RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV56AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV56ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV56Request>(
    inputLiveV56RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV56Request(directory, request);
  if (phase === "capture") validateInputLiveV56TechnicalGates(directory);
  const rawFile = inputLiveV56ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v56-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V56_BROKER_VERSION ||
...
export function persistInputLiveV56TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV56RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV56RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V56_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v56-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V56_BROKER_VERSION,
...
export function validateInputLiveV56TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v56-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V56_TECHNICAL_GATES.some(
...
export function validateInputLiveV56Receipt(
  directory: string,
  phase: InputLiveV56RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV56Request(directory, request);
  const receipt = json<InputLiveV56Receipt>(
    inputLiveV56ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV56ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v56-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V56_BROKER_VERSION ||
...
export function readInputLiveV56AcceptedRawPayload(
  directory: string,
  phase: InputLiveV56RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV56Receipt(directory, phase, captureIndex);
...
export function createInputLiveV56FakeDynamicResponse(
  request: InputLiveV56Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V56_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V56_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV56PersistedRequests(
  directory: string,
): InputLiveV56Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV56Request>(file)] : [];
    });
}

export const inputLiveV56Sha256 = sha256;
