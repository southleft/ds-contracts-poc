...
import {
  ...
  verifyInputLiveV38Authorization,
  type InputLiveV38AntecedentIndex,
  type InputLiveV38AuthorizationProof,
} from "./input-field-live-v38-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V38_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v38/operator-security-attestation-template.json";
export const INPUT_LIVE_V38_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v38-security-attestation.json";

export interface InputLiveV38SecurityAttestation {
...
export interface InputLiveV38PreflightReport {
  artifactVersion: "input-live-v38-preflight-v1";
...
export const inputLiveV38AttestationSha256 = (
  value: Omit<InputLiveV38SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV38SecurityAttestation(
  value: unknown,
  proof: InputLiveV38AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v38-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V38_TARGET) ||
...
export function validateInputLiveV38ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV38Authorization");
  const preflight = source.indexOf("runInputLiveV38Preflight");
  const initialize = source.indexOf("InputLiveV38Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV38Preflight(
  root: string,
  proof: InputLiveV38AuthorizationProof,
  attestationPath = INPUT_LIVE_V38_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV38PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V38_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V38_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V38_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV38AntecedentIndex;
  failures.push(...validateInputLiveV38AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V38_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V38_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v38-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV38ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV38SecurityAttestation;
  return {
    artifactVersion: "input-live-v38-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V38_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV38Preflight(
    process.cwd(),
    verifyInputLiveV38Authorization(),
    attestationPath,
