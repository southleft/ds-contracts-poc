...
import { INPUT_LIVE_V23_INDEX_PATH } from "./build-input-field-live-proof-v23.js";
import {
  INPUT_LIVE_V23_DYNAMIC_TOOL,
  INPUT_LIVE_V23_TARGET,
} from "./input-field-live-v23-broker.js";
import {
  buildInputLiveV23AuthorizationArtifact,
  validateInputLiveV23AntecedentIndex,
  validateInputLiveV23AuthorizationArtifact,
  validateInputLiveV23History,
  validateInputLiveV23UnitTestSource,
  type InputLiveV23AntecedentIndex,
  type InputLiveV23AuthorizationArtifact,
  type InputLiveV23AuthorizationProof,
  type InputLiveV23HistoryState,
} from "./input-field-live-v23-authorization.js";
import {
  inputLiveV23AttestationSha256,
  validateInputLiveV23ControlFlowSource,
  validateInputLiveV23SecurityAttestation,
  type InputLiveV23SecurityAttestation,
} from "./input-field-live-v23-preflight.js";
import { inputLiveV23Sha256 } from "./input-field-live-v23-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V23_INDEX_PATH);
const index = (): InputLiveV23AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV23AntecedentIndex;

const artifact = (): InputLiveV23AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV23AuthorizationArtifact({
...
const pendingState = (): InputLiveV23HistoryState => ({
  index: index(),
  indexSha256: inputLiveV23Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV23HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV23AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV23Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V23_TARGET,
  expectedDynamicTool: INPUT_LIVE_V23_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v23/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV23AuthorizationArtifact,
): InputLiveV23SecurityAttestation => {
  const body: Omit<InputLiveV23SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v23-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V23_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV23AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV23History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV23History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV23History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV23History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV23History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV23UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV23UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV23AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV23Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV23AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV23AuthorizationArtifact(
        planted as InputLiveV23AuthorizationArtifact,
        index(),
        inputLiveV23Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV23SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV23AttestationSha256(
      body as Omit<InputLiveV23SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV23SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV23ControlFlowSource(source), []);
});
