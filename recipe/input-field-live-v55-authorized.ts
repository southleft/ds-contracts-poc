...
import { INPUT_LIVE_V55_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v55.js";
import { verifyInputLiveV55Authorization } from "./input-field-live-v55-authorization.js";
import { runInputLiveV55Preflight } from "./input-field-live-v55-preflight.js";
import { InputLiveV55Orchestrator } from "./run-input-field-live-v55.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V55_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v55-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV55Authorization(root);
    runInputLiveV55Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV55Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
