...
import {
  ...
  createInputLiveV52FakeDynamicResponse,
  ...
  listInputLiveV52PersistedRequests,
  persistInputLiveV52RawResponse,
  persistInputLiveV52TechnicalGates,
  validateInputLiveV52Receipt,
  validateInputLiveV52Request,
  type InputLiveV52RemotePhase,
...
import {
  ...
  type InputLiveV52ExtractPayload,
  type InputLiveV52ProbePayload,
  type InputLiveV52RestorePayload,
  type InputLiveV52RootProof,
  type InputLiveV52WriterOwnership,
} from "./input-field-live-v52-contract.js";
import {
  InputLiveV52Orchestrator,
  simulatedInputLiveV52Authorization,
} from "./run-input-field-live-v52.js";
...
const captures: InputLiveV52CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV52Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v52-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV52Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV52CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV52FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV52FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V52_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V52_TARGET);
  persistInputLiveV52RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV52Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV52Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV52WriterOwnership,
): InputLiveV52RestorePayload => ({
...
const rootProofs = (): InputLiveV52RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV52CaptureCell,
): InputLiveV52CapturePayload => {
  const bytes = Buffer.from(`input-live-v52-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV52Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV52Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV52RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV52RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV52ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV52ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV52CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV52CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV52CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV52Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV52CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV52PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV52ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV52Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV52Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV52Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV52Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V52_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV52RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V52_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV52Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV52Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV52ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV52Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV52WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV52RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV52Request;
    validateInputLiveV52Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV52Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV52CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v52-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV52Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV52Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV52Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V52_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV52RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V52_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV52Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV52Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV52RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V52_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV52Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV52Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV52Request;
