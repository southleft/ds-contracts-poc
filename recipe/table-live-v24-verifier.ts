...
import {
  verifyTableLiveV24FixedSceneFixedPoint,
  type TableLiveV24FixedFixedPoint,
} from "./table-live-v24-fixed-point.js";

export const TABLE_LIVE_V24_VERIFIER_VERSION = "table-live-v24-verifier-v1";

export function buildTableLiveV24RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV24RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV24RawNode[];
}

export interface TableLiveV24NormalizedScene {
...
export interface TableLiveV24SceneProof {
...
export function normalizeTableLiveV24Scene(
  root: TableLiveV24RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV24NormalizedScene {
  const canonicalBindings: TableLiveV24NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV24RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV24Scene<Instance>(
  normalized: TableLiveV24NormalizedScene,
...
): TableLiveV24SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV24FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV24ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV24PreCaptureGates(payload: unknown): void {
