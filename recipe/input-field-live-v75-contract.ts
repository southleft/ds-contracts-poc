...
} from "./input-field-live-v3-verifier-v75.js";
import {
  buildInputLiveV75RawPropertyRuntime,
  normalizeInputLiveV75Scene,
  type InputLiveV75RawNode,
} from "./input-field-live-v75-verifier.js";
...
export {
  ...
  type InputLiveV75RestorePayload,
} from "./input-field-live-v75-restore.js";

export const INPUT_LIVE_V75_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V75_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V75_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V75_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V75_VARIANT_COUNT = 256;
export const INPUT_LIVE_V75_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V75_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV75SourceId = (typeof INPUT_LIVE_V75_SOURCE_IDS)[number];

export interface InputLiveV75SourceIdentity {
  source: InputLiveV75SourceId;
...
export interface InputLiveV75WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV75ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV75SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV75RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV75RootProof {
  source: InputLiveV75SourceId;
  adapterIdentity: string;
...
export interface InputLiveV75ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV75SourceId;
    adapterIdentity: string;
...
export interface InputLiveV75CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV75SourceId;
  adapterIdentity: string;
...
export interface InputLiveV75CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV75SourceId;
  strata: InputLiveV75CaptureCell["strata"];
...
export interface InputLiveV75CleanupPayload {
...
export function validateInputLiveV75WriterPayload(
  value: unknown,
): InputLiveV75WriterOwnership {
...
const identityPlan = (source: InputLiveV75SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV75ExtractProgram(
  writer: InputLiveV75WriterOwnership,
  sources: readonly InputLiveV75SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V75_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V75_NAMESPACE)}
${buildInputLiveV75RawPropertyRuntime()}
...
export function validateInputLiveV75ExtractPayload(
  value: unknown,
  writer: InputLiveV75WriterOwnership,
): InputLiveV75ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V75_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV75ExtractPayload;
}

export function proveInputLiveV75Roots<Instance>(
  extract: InputLiveV75ExtractPayload,
  sources: readonly (InputLiveV75SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV75Scene(
      root.scene,
...
export function assertInputLiveV75RootProofs(
  proofs: readonly InputLiveV75RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV75ProbeProgram(
  writer: InputLiveV75WriterOwnership,
  sources: readonly InputLiveV75SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V75_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV75ProbePayload(
  value: unknown,
  writer: InputLiveV75WriterOwnership,
): InputLiveV75ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV75ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V75_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V75_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV75CaptureManifest(
  cells: readonly InputLiveV75CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V75_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V75_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V75_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V75_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V75_SOURCE_IDS.some(
...
export const inputLiveV75CaptureManifestSha256 = (
  cells: readonly InputLiveV75CaptureCell[],
): string => {
  validateInputLiveV75CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV75CaptureProgram(
  writer: InputLiveV75WriterOwnership,
  cell: InputLiveV75CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V75_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V75_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V75_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV75CapturePayload(
  value: unknown,
  cell: InputLiveV75CaptureCell,
  rawResponseBytes: number,
): InputLiveV75CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV75CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V75_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V75_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV75CaptureResponses(
  manifest: readonly InputLiveV75CaptureCell[],
  responses: readonly InputLiveV75CapturePayload[],
): void {
  validateInputLiveV75CaptureManifest(manifest);
  if (
...
export interface InputLiveV75ObjectiveReport {
  artifactVersion: "input-live-v75-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV75Objective(
  manifest: readonly InputLiveV75CaptureCell[],
  responses: readonly InputLiveV75CapturePayload[],
  readReference: (cell: InputLiveV75CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV75CaptureCell) => Buffer,
): InputLiveV75ObjectiveReport {
  assertInputLiveV75CaptureResponses(manifest, responses);
...
export function buildInputLiveV75CleanupProgram(
  writer: InputLiveV75WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V75_NAMESPACE,
...
export function validateInputLiveV75CleanupPayload(
  value: unknown,
  writer: InputLiveV75WriterOwnership,
): InputLiveV75CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV75CleanupPayload;
...
export const INPUT_LIVE_V75_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV75WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
