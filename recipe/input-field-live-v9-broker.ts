...
import {
  assertInputLiveV9RootProofs,
  INPUT_LIVE_V9_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V9_RESPONSE_CONTRACTS,
  type InputLiveV9RootProof,
} from "./input-field-live-v9-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V9_BROKER_VERSION = "input-live-v9-broker-v2";
export const INPUT_LIVE_V9_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V9_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V9_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V9_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V9_REMOTE_PHASES = [
  "writer",
  "extract",
  "probe",
  "capture",
...
export const INPUT_LIVE_V9_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV9RemotePhase =
  (typeof INPUT_LIVE_V9_REMOTE_PHASES)[number];
export type InputLiveV9AuthorizationMode = "live" | "simulated";

export interface InputLiveV9CommitPins {
...
export interface InputLiveV9TransactionAuthorization extends InputLiveV9CommitPins {
  mode: InputLiveV9AuthorizationMode;
...
export interface InputLiveV9Manifest {
  artifactVersion: "input-live-v9-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V9_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V9_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V9_TARGET;
  authorization: InputLiveV9TransactionAuthorization;
...
export interface InputLiveV9ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV9RequestBody {
  artifactVersion: "input-live-v9-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V9_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV9RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV9CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V9_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV9ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV9Request extends InputLiveV9RequestBody {
...
export interface InputLiveV9Receipt {
  artifactVersion: "input-live-v9-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V9_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV9RemotePhase;
...
export interface InputLiveV9AcceptedResponse<T = unknown> {
  request: InputLiveV9Request;
  receipt: InputLiveV9Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV9IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV9ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V9_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V9_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V9_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V9_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V9_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V9_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V9_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Input live v9 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    throw new TypeError("Input live v9 capture index must be 0..127");
  return `capture-${String(captureIndex).padStart(3, "0")}`;
};

export const inputLiveV9RequestSequence = (
  phase: InputLiveV9RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV9RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV9RequestPath = (
  directory: string,
  phase: InputLiveV9RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV9ResponsePath = (
  directory: string,
  phase: InputLiveV9RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV9ReceiptPath = (
  directory: string,
  phase: InputLiveV9RemotePhase,
  captureIndex?: number,
): string =>
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Input live v9 authorization pins are malformed");
};

const readManifest = (directory: string): InputLiveV9Manifest => {
  const manifest = json<InputLiveV9Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Input live v9 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "input-live-v9-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V9_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V9_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Input live v9 broker manifest");
  return manifest;
};

const unsignedBody = (request: InputLiveV9Request): InputLiveV9RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV9RemotePhase,
  captureIndex?: number,
): InputLiveV9Request | undefined => {
  const file = inputLiveV9RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV9Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV9RemotePhase,
  captureIndex?: number,
): InputLiveV9Receipt | undefined => {
  const file = inputLiveV9ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV9Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV9Request | undefined;
  receipt: InputLiveV9Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV9RemotePhase,
): InputLiveV9ResponseContract => ({
  ...INPUT_LIVE_V9_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V9_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V9_DYNAMIC_TOOL,
      target: INPUT_LIVE_V9_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v9-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V9_BROKER_VERSION,
  target: INPUT_LIVE_V9_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V9_PROGRAM_START*/${program}/*INPUT_LIVE_V9_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V9_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V9_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Input live v9 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createInputLiveV9Transaction(
  directory: string,
  ...
  if (existsSync(path.join(directory, MANIFEST_FILE)))
    throw new Error("Input live v9 transaction already exists");
  validateAuthorization(options.authorization);
  if (
    !SHA256.test(options.proofPlanSha256) ||
    !SHA256.test(options.captureManifestSha256)
  )
    throw new TypeError("Input live v9 transaction plan hashes malformed");
  const attempt = options.attempt ?? 1;
  if (!Number.isInteger(attempt) || attempt < 1 || attempt > 3)
    throw new TypeError("Input live v9 attempt must be 1..3");
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Input live v9 private signing key does not match authorized public-key identity",
    );
  const manifest: InputLiveV9Manifest = {
    artifactVersion: "input-live-v9-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V9_BROKER_VERSION,
...
export function validateInputLiveV9Request(
  directory: string,
  request: InputLiveV9Request,
): void {
  ...
  const expectedSequence = inputLiveV9RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v9-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V9_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V9_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Input live v9 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueInputLiveV9Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV9RemotePhase,
  program: string,
  options: InputLiveV9IssueOptions = {},
): InputLiveV9Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV9RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV9Request>(targetPath);
    validateInputLiveV9Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v9 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV9ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Input live v9 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Input live v9 empty code envelope refused");
  if (phase === "capture") validateInputLiveV9TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Input live v9 out-of-order phase: ${id}`);
  const expectedResponse =
    options.expectedResponse ?? responseContractFor(phase);
  const programSha256 = sha256(program);
  const sequence = inputLiveV9RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V9_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V9_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V9_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV9RequestBody = {
    artifactVersion: "input-live-v9-broker-request-v2",
    brokerVersion: INPUT_LIVE_V9_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV9Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV9Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV9RawResponse(
  directory: string,
  phase: InputLiveV9RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV9RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v9 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV9Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Input live v9 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = inputLiveV9ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Input live v9 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV9Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v9-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V9_BROKER_VERSION,
...
export function acceptInputLiveV9Response<T = unknown>(
  directory: string,
  phase: InputLiveV9RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV9AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV9ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v9 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV9Request>(
    inputLiveV9RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV9Request(directory, request);
  if (phase === "capture") validateInputLiveV9TechnicalGates(directory);
  const rawFile = inputLiveV9ResponsePath(directory, phase, captureIndex);
  if (!existsSync(rawFile))
    throw new Error(`Input live v9 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v9-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V9_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Input live v9 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistInputLiveV9TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV9RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV9RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Input live v9 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Input live v9 probe payload hash malformed");
  const gates = Object.fromEntries(
    INPUT_LIVE_V9_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v9-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V9_BROKER_VERSION,
...
export function validateInputLiveV9TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v9-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V9_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Input live v9 technical gates artifact");
}

export function validateInputLiveV9Receipt(
  directory: string,
  phase: InputLiveV9RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV9Request(directory, request);
  const receipt = json<InputLiveV9Receipt>(
    inputLiveV9ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV9ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v9-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V9_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Input live v9 accepted receipt: ${request.requestId}`,
    );
}

export function readInputLiveV9AcceptedRawPayload(
  directory: string,
  phase: InputLiveV9RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV9Receipt(directory, phase, captureIndex);
...
export function createInputLiveV9FakeDynamicResponse(
  request: InputLiveV9Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V9_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V9_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV9PersistedRequests(
  directory: string,
): InputLiveV9Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV9Request>(file)] : [];
    });
}

export const inputLiveV9Sha256 = sha256;
