...
import {
  ...
  COMBOBOX_LIVE_V1_HOST_PHASES,
  COMBOBOX_LIVE_V1_REMOTE_REQUESTS,
  COMBOBOX_LIVE_V1_SOURCE_ROOTS,
...
export const COMBOBOX_LIVE_V1_EVIDENCE_ROOT =
  "recipe/evidence/combobox-live-pivot-v1";
export const COMBOBOX_LIVE_V1_PROTOCOL_PATH = `${COMBOBOX_LIVE_V1_EVIDENCE_ROOT}/protocol.json`;
export const COMBOBOX_LIVE_V1_PLAN_PATH = `${COMBOBOX_LIVE_V1_EVIDENCE_ROOT}/proof-plan.json`;
export const COMBOBOX_LIVE_V1_CAPTURE_MANIFEST_PATH = `${COMBOBOX_LIVE_V1_EVIDENCE_ROOT}/capture-manifest.json`;
export const COMBOBOX_LIVE_V1_REQUEST_MANIFEST_PATH = `${COMBOBOX_LIVE_V1_EVIDENCE_ROOT}/request-manifest.json`;
export const COMBOBOX_LIVE_V1_INDEX_PATH = `${COMBOBOX_LIVE_V1_EVIDENCE_ROOT}/antecedent-index.json`;
export const COMBOBOX_LIVE_V1_AUTHORIZATION_TEMPLATE_PATH = `${COMBOBOX_LIVE_V1_EVIDENCE_ROOT}/authorization-template.json`;
export const COMBOBOX_LIVE_V1_AUTHORIZATION_PATH = `${COMBOBOX_LIVE_V1_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface ComboboxLiveV1ProofPlan {
  artifactVersion: "combobox-live-v1-proof-plan-v1";
...
const captureCells = (
...
): ComboboxLiveV1CaptureCell[] => {
  const cells: ComboboxLiveV1CaptureCell[] = [];
  ...
  if (cells.length !== COMBOBOX_LIVE_V1_CAPTURE_COUNT)
    throw new Error(
      `Combobox live v1 capture cells ${cells.length}, expected ${COMBOBOX_LIVE_V1_CAPTURE_COUNT}`,
    );
  comboboxLiveV1CaptureManifestSha256(cells);
  return cells;
};

const protocol = (expectedFacts: number) => ({
  artifactVersion: "combobox-live-v1-external-operator-protocol-v1",
  protocolId: "combobox-live-v1",
  status:
  ...
  denominator: {
    sourceRoots: COMBOBOX_LIVE_V1_SOURCE_ROOTS,
    sets: 4,
    variants: COMBOBOX_LIVE_V1_VARIANT_COUNT,
    captureCells: COMBOBOX_LIVE_V1_CAPTURE_COUNT,
    remoteRequests: COMBOBOX_LIVE_V1_REMOTE_REQUESTS,
    hostPhases: COMBOBOX_LIVE_V1_HOST_PHASES,
...
export async function buildComboboxLiveV1Proof(
  check = process.argv.includes("--check"),
): Promise<ComboboxLiveV1ProofPlan> {
  ...
  const writerOwnershipBlueprint: ComboboxLiveV1WriterOwnership = {
  ...
  const identities: ComboboxLiveV1SourceIdentity[] = sources.map((source) => ({
    source: source.source,
    adapterIdentity: source.adapterIdentity,
    recipeHash: source.recipeHash,
    envelopeHash: source.envelope.integrity.canonicalHash,
  ...
  const restoreBlueprint = Buffer.from(
    buildComboboxLiveV1RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildComboboxLiveV1ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildComboboxLiveV1ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildComboboxLiveV1CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );
  const cleanupBlueprint = Buffer.from(
    buildComboboxLiveV1CleanupProgram(writerOwnershipBlueprint),
  );
  ...
  const sourceMetadata = sources.map((source) => {
    ...
    const comboboxPath = `${COMBOBOX_LIVE_V1_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-combobox.json.gz`;
    const optionPath = `${COMBOBOX_LIVE_V1_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-option.json.gz`;
  ...
  antecedentOutputs.set(
    COMBOBOX_LIVE_V1_PROTOCOL_PATH,
    jsonBytes(protocol(expectedFacts)),
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V1_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V1_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V1_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V1_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V1_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V1_EVIDENCE_ROOT}/programs/cleanup-blueprint.js`,
    cleanupBlueprint,
  );

  const captureManifest = {
    artifactVersion: "combobox-live-v1-capture-manifest-v1",
    ...
    cells: captures,
    cellsSha256: comboboxLiveV1CaptureManifestSha256(captures),
  };
  ...
  const proofPlan: ComboboxLiveV1ProofPlan = {
    artifactVersion: "combobox-live-v1-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: COMBOBOX_LIVE_V1_TARGET,
    namespace: COMBOBOX_FIGMA_NAMESPACE,
    writer: {
      programPath: `${COMBOBOX_LIVE_V1_EVIDENCE_ROOT}/programs/writer.txt`,
    ...
    captureManifest: {
      path: COMBOBOX_LIVE_V1_CAPTURE_MANIFEST_PATH,
      cells: 72,
      requests: 72,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(COMBOBOX_LIVE_V1_PLAN_PATH, jsonBytes(proofPlan));

  const requests = {
    artifactVersion: "combobox-live-v1-request-manifest-v1",
    expectedDynamicTool: COMBOBOX_LIVE_V1_DYNAMIC_TOOL,
    target: COMBOBOX_LIVE_V1_TARGET,
    ...
    requests: [
      {
        requestId: "writer",
        sequence: comboboxLiveV1RequestSequence("writer"),
        phase: "writer",
      },
      {
        requestId: "cleanup",
        sequence: comboboxLiveV1RequestSequence("cleanup"),
        phase: "cleanup",
        availability: "persisted immediately after writer acceptance",
      },
      {
        requestId: "restore",
        sequence: comboboxLiveV1RequestSequence("restore"),
        phase: "restore",
      },
      {
        requestId: "extract",
        sequence: comboboxLiveV1RequestSequence("extract"),
        phase: "extract",
      },
      {
        requestId: "probe",
        sequence: comboboxLiveV1RequestSequence("probe"),
        phase: "probe",
      },
      ...captures.map((cell) => ({
        requestId: `capture-${String(cell.index).padStart(3, "0")}`,
        sequence: comboboxLiveV1RequestSequence("capture", cell.index),
        phase: "capture",
  ...
  if (requests.requestCount !== COMBOBOX_LIVE_V1_REMOTE_REQUESTS)
    throw new Error("Combobox live v1 remote request denominator drifted");
  antecedentOutputs.set(
    COMBOBOX_LIVE_V1_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const lifecycleExcludedPaths = [
    COMBOBOX_LIVE_V1_AUTHORIZATION_TEMPLATE_PATH,
    COMBOBOX_LIVE_V1_AUTHORIZATION_PATH,
    "recipe/combobox-live-v1-authorization.ts",
    "recipe/combobox-live-v1-authorization.test.ts",
    "recipe/combobox-live-v1-preflight.ts",
    "recipe/combobox-live-v1-authorized.ts",
    "recipe/write-combobox-live-v1-authorization.ts",
    "recipe/create-combobox-live-v1-security-attestation.ts",
    "recipe/evidence/combobox-live-pivot-v1/operator-security-attestation-template.json",
    "recipe/evidence/combobox-live-pivot-v1-status.json",
    "recipe/evidence/status-index.json",
  ];
  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/combobox-live-v1-broker.ts",
    "recipe/combobox-live-v1-contract.ts",
    "recipe/combobox-live-v1-restore.ts",
    "recipe/combobox-live-v1-cleanup.ts",
    "recipe/combobox-live-v1-verifier.ts",
    "recipe/combobox-live-v1-fixed-point.ts",
    "recipe/run-combobox-live-v1.ts",
    "recipe/build-combobox-live-proof-v1.ts",
    "recipe/combobox-live-v1-lifecycle-simulation.ts",
    ...
    "recipe/normalize.ts",
  ].filter((artifactPath) => artifactPath !== COMBOBOX_LIVE_V1_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "combobox-live-v1-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
  ...
  antecedentOutputs.set(COMBOBOX_LIVE_V1_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "combobox-live-v1-authorization-template-v1",
    authorizationId: "combobox-live-v1",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: COMBOBOX_LIVE_V1_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
    ...
    denominator: {
      remoteRequests: COMBOBOX_LIVE_V1_REMOTE_REQUESTS,
      hostPhases: COMBOBOX_LIVE_V1_HOST_PHASES,
      captures: COMBOBOX_LIVE_V1_CAPTURE_COUNT,
      sourceRoots: COMBOBOX_LIVE_V1_SOURCE_ROOTS,
      expectedFacts,
    },
    execution: {
  ...
  const attestationTemplate = jsonBytes({
    artifactVersion: "combobox-live-v1-operator-security-attestation-template-v1",
    status: "template only; complete privately after PREPARE+AUTHORIZE",
    tokenValuesForbidden: true,
    privateDirectory: "private/combobox-live-v1-security-attestation.json",
  });
  ...
  if (check) {
    ...
    mkdirSync(path.dirname(COMBOBOX_LIVE_V1_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(COMBOBOX_LIVE_V1_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
    writeFileSync(
      `${COMBOBOX_LIVE_V1_EVIDENCE_ROOT}/operator-security-attestation-template.json`,
      attestationTemplate,
    );
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(COMBOBOX_LIVE_V1_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(COMBOBOX_LIVE_V1_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildComboboxLiveV1Proof(), null, 2)}\n`,
  );
