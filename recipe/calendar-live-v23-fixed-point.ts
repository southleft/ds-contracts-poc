/**
 * Calendar live v23 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V23_FIXED_VERIFIER_VERSION = "calendar-live-v23-fixed-point-v1";
export const CALENDAR_LIVE_V23_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV23FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV23FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV23FixedSourceProbe {
...
export interface CalendarLiveV23FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV23FixedFixedPoint;
}

export interface CalendarLiveV23FixedHardGateInput {
  thresholds: CalendarLiveV23FixedThresholds;
  sourceProbes: CalendarLiveV23FixedSourceProbe[];
  cells: CalendarLiveV23FixedCellValidation[];
  sceneProofs: CalendarLiveV23FixedSceneProof[];
...
export interface CalendarLiveV23FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V23_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV23FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV23FixedHardGates(
  input: CalendarLiveV23FixedHardGateInput,
): CalendarLiveV23FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV23FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V23_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV23FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV23FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV23FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
