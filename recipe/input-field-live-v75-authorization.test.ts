...
import { INPUT_LIVE_V75_INDEX_PATH } from "./build-input-field-live-proof-v75.js";
import {
  INPUT_LIVE_V75_DYNAMIC_TOOL,
  INPUT_LIVE_V75_TARGET,
} from "./input-field-live-v75-broker.js";
import {
  buildInputLiveV75AuthorizationArtifact,
  validateInputLiveV75AntecedentIndex,
  validateInputLiveV75AuthorizationArtifact,
  validateInputLiveV75History,
  validateInputLiveV75UnitTestSource,
  type InputLiveV75AntecedentIndex,
  type InputLiveV75AuthorizationArtifact,
  type InputLiveV75AuthorizationProof,
  type InputLiveV75HistoryState,
} from "./input-field-live-v75-authorization.js";
import {
  inputLiveV75AttestationSha256,
  validateInputLiveV75ControlFlowSource,
  validateInputLiveV75SecurityAttestation,
  type InputLiveV75SecurityAttestation,
} from "./input-field-live-v75-preflight.js";
import { inputLiveV75Sha256 } from "./input-field-live-v75-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V75_INDEX_PATH);
const index = (): InputLiveV75AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV75AntecedentIndex;

const artifact = (): InputLiveV75AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV75AuthorizationArtifact({
...
const pendingState = (): InputLiveV75HistoryState => ({
  index: index(),
  indexSha256: inputLiveV75Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV75HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV75AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV75Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V75_TARGET,
  expectedDynamicTool: INPUT_LIVE_V75_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v75/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV75AuthorizationArtifact,
): InputLiveV75SecurityAttestation => {
  const body: Omit<InputLiveV75SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v75-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V75_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV75AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV75History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV75History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV75History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV75History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV75History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV75UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV75UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV75AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV75Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV75AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV75AuthorizationArtifact(
        planted as InputLiveV75AuthorizationArtifact,
        index(),
        inputLiveV75Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV75SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV75AttestationSha256(
      body as Omit<InputLiveV75SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV75SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV75ControlFlowSource(source), []);
});
