...
export interface ComboboxLiveV34LifecycleSimulationReport {
  artifactVersion: "combobox-live-v34-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV34AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV34Lifecycle(): Promise<ComboboxLiveV34LifecycleSimulationReport> {
  await buildComboboxLiveV34Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V34_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV34AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v34-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V34_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V34_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV34History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV34History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV34AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V34_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V34_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV34History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV34History(authorized, "pending");
    ...
    await buildComboboxLiveV34Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V34_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV34AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV34Lifecycle(), null, 2)}\n`,
  );
