...
import { INPUT_LIVE_V29_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v29.js";
import { verifyInputLiveV29Authorization } from "./input-field-live-v29-authorization.js";
import { runInputLiveV29Preflight } from "./input-field-live-v29-preflight.js";
import { InputLiveV29Orchestrator } from "./run-input-field-live-v29.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V29_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v29-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV29Authorization(root);
    runInputLiveV29Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV29Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
