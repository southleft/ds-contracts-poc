/**
 * Calendar live v44 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V44_FIXED_VERIFIER_VERSION = "calendar-live-v44-fixed-point-v1";
export const CALENDAR_LIVE_V44_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV44FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV44FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV44FixedSourceProbe {
...
export interface CalendarLiveV44FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV44FixedFixedPoint;
}

export interface CalendarLiveV44FixedHardGateInput {
  thresholds: CalendarLiveV44FixedThresholds;
  sourceProbes: CalendarLiveV44FixedSourceProbe[];
  cells: CalendarLiveV44FixedCellValidation[];
  sceneProofs: CalendarLiveV44FixedSceneProof[];
...
export interface CalendarLiveV44FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V44_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV44FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV44FixedHardGates(
  input: CalendarLiveV44FixedHardGateInput,
): CalendarLiveV44FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV44FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  if (input.visualRows.length !== 0)
    failures.push("table live v17 must not invent a visual comparison");
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V44_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV44FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV44FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV44FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
