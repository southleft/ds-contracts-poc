...
import { COMBOBOX_LIVE_V9_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v9.js";
import { verifyComboboxLiveV9Authorization } from "./combobox-live-v9-authorization.js";
import { runComboboxLiveV9Preflight } from "./combobox-live-v9-preflight.js";
import { ComboboxLiveV9Orchestrator } from "./run-combobox-live-v9.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v9 requires exact authorization artifact ${COMBOBOX_LIVE_V9_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v9-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV9Authorization(root);
    runComboboxLiveV9Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV9Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV9Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
