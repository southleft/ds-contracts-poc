...
import { CALENDAR_LIVE_V3_INDEX_PATH } from "./build-calendar-live-proof-v3.js";
import {
...
import {
  buildCalendarLiveV3AuthorizationArtifact,
  validateCalendarLiveV3AntecedentIndex,
  validateCalendarLiveV3AuthorizationArtifact,
  validateCalendarLiveV3History,
  validateCalendarLiveV3UnitTestSource,
  type CalendarLiveV3AntecedentIndex,
  type CalendarLiveV3AuthorizationArtifact,
  type CalendarLiveV3AuthorizationProof,
  type CalendarLiveV3HistoryState,
} from "./calendar-live-v3-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV3ControlFlowSource,
  validateCalendarLiveV3SecurityAttestation,
  type CalendarLiveV3SecurityAttestation,
} from "./calendar-live-v3-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V3_INDEX_PATH);
const index = (): CalendarLiveV3AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV3AntecedentIndex;

const artifact = (): CalendarLiveV3AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV3AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV3HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV3HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV3AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v3/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV3AuthorizationArtifact,
): CalendarLiveV3SecurityAttestation => {
  const body: Omit<CalendarLiveV3SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V3_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV3History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV3History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV3History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV3History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV3History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV3UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v3-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV3UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV3AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV3AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV3AuthorizationArtifact(
        planted as CalendarLiveV3AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV3SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV3SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV3SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v3-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV3ControlFlowSource(source), []);
});
