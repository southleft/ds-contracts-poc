...
import { COMBOBOX_LIVE_V34_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v34.js";
import { verifyComboboxLiveV34Authorization } from "./combobox-live-v34-authorization.js";
import { runComboboxLiveV34Preflight } from "./combobox-live-v34-preflight.js";
import { ComboboxLiveV34Orchestrator } from "./run-combobox-live-v34.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v34 requires exact authorization artifact ${COMBOBOX_LIVE_V34_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v34-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV34Authorization(root);
    runComboboxLiveV34Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV34Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV34Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
