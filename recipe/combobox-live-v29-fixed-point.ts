/**
 * Carried Combobox live v29 host verifier. Uses scene-readback-combobox-v29 so live
...
export const COMBOBOX_LIVE_V29_FIXED_VERIFIER_VERSION = "combobox-live-v29-fixed-point-v1";
export const COMBOBOX_LIVE_V29_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV29FixedThresholds {
...
export interface ComboboxLiveV29FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV29FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV29FixedSourceProbe {
...
export interface ComboboxLiveV29FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV29FixedFixedPoint;
}

export interface ComboboxLiveV29FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV29FixedHardGateInput {
  thresholds: ComboboxLiveV29FixedThresholds;
  materialRegression: ComboboxLiveV29FixedMaterialRegression;
  sourceProbes: ComboboxLiveV29FixedSourceProbe[];
  cells: ComboboxLiveV29FixedCellValidation[];
  sceneProofs: ComboboxLiveV29FixedSceneProof[];
  visualRows: ComboboxLiveV29FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV29FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V29_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV29FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV29FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV29FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV29FixedHardGates(
  input: ComboboxLiveV29FixedHardGateInput,
): ComboboxLiveV29FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV29FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV29FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V29_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV29FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV29FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV29FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
