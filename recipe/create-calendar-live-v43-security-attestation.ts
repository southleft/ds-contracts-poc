...
import { verifyCalendarLiveV43Authorization } from "./calendar-live-v43-authorization.js";
import { CALENDAR_LIVE_V43_TARGET } from "./calendar-live-v43-broker.js";
import {
  CALENDAR_LIVE_V43_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV43Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v43-session";
...
const attestation: CalendarLiveV43SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV43SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v43 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V43_SECURITY_ATTESTATION_DEFAULT_PATH,
);
