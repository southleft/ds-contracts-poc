...
import { INPUT_LIVE_V14_INDEX_PATH } from "./build-input-field-live-proof-v14.js";
import {
  INPUT_LIVE_V14_DYNAMIC_TOOL,
  INPUT_LIVE_V14_TARGET,
} from "./input-field-live-v14-broker.js";
import {
  buildInputLiveV14AuthorizationArtifact,
  validateInputLiveV14AntecedentIndex,
  validateInputLiveV14AuthorizationArtifact,
  validateInputLiveV14History,
  validateInputLiveV14UnitTestSource,
  type InputLiveV14AntecedentIndex,
  type InputLiveV14AuthorizationArtifact,
  type InputLiveV14AuthorizationProof,
  type InputLiveV14HistoryState,
} from "./input-field-live-v14-authorization.js";
import {
  inputLiveV14AttestationSha256,
  validateInputLiveV14ControlFlowSource,
  validateInputLiveV14SecurityAttestation,
  type InputLiveV14SecurityAttestation,
} from "./input-field-live-v14-preflight.js";
import { inputLiveV14Sha256 } from "./input-field-live-v14-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V14_INDEX_PATH);
const index = (): InputLiveV14AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV14AntecedentIndex;

const artifact = (): InputLiveV14AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV14AuthorizationArtifact({
...
const pendingState = (): InputLiveV14HistoryState => ({
  index: index(),
  indexSha256: inputLiveV14Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV14HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV14AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV14Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V14_TARGET,
  expectedDynamicTool: INPUT_LIVE_V14_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v14/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV14AuthorizationArtifact,
): InputLiveV14SecurityAttestation => {
  const body: Omit<InputLiveV14SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v14-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V14_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV14AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV14History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV14History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV14History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV14History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV14History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV14UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV14UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV14AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV14Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV14AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV14AuthorizationArtifact(
        planted as InputLiveV14AuthorizationArtifact,
        index(),
        inputLiveV14Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV14SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV14AttestationSha256(
      body as Omit<InputLiveV14SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV14SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV14ControlFlowSource(source), []);
});
