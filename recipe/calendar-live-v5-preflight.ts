...
import {
  validateCalendarLiveV5AntecedentIndex,
  verifyCalendarLiveV5Authorization,
  type CalendarLiveV5AntecedentIndex,
  type CalendarLiveV5AuthorizationProof,
} from "./calendar-live-v5-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V5_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v5/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V5_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v5-security-attestation.json";

export interface CalendarLiveV5SecurityAttestation {
  artifactVersion: "calendar-live-v5-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV5PreflightReport {
  artifactVersion: "calendar-live-v5-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV5SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV5SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV5AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v5-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V5_TARGET) ||
...
export function validateCalendarLiveV5ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV5Authorization");
  const preflight = source.indexOf("runCalendarLiveV5Preflight");
  const initialize = source.indexOf("CalendarLiveV5Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV5Preflight(
  root: string,
  proof: CalendarLiveV5AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V5_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV5PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V5_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V5_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV5AntecedentIndex;
  failures.push(...validateCalendarLiveV5AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V5_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V5_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV5SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v5-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV5ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v5 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV5SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v5-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V5_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV5Preflight(
    process.cwd(),
    verifyCalendarLiveV5Authorization(),
    attestationPath,
