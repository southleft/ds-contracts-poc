/**
 * Calendar live v2 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V2_FIXED_VERIFIER_VERSION = "calendar-live-v2-fixed-point-v1";
export const CALENDAR_LIVE_V2_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV2FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV2FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV2FixedSourceProbe {
...
export interface CalendarLiveV2FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV2FixedFixedPoint;
}

export interface CalendarLiveV2FixedHardGateInput {
  thresholds: CalendarLiveV2FixedThresholds;
  sourceProbes: CalendarLiveV2FixedSourceProbe[];
  cells: CalendarLiveV2FixedCellValidation[];
  sceneProofs: CalendarLiveV2FixedSceneProof[];
...
export interface CalendarLiveV2FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V2_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV2FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV2FixedHardGates(
  input: CalendarLiveV2FixedHardGateInput,
): CalendarLiveV2FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV2FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V2_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV2FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV2FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
