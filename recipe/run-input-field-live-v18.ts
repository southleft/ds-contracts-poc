...
import {
  ...
  readInputLiveV18AcceptedRawPayload,
...
import {
  ...
  buildInputLiveV18CleanupProgram,
...
import {
  ...
  INPUT_LIVE_V18_PLAN_PATH,
  type InputLiveV18ProofPlan,
} from "./build-input-field-live-proof-v18.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-v18.js";

export const INPUT_LIVE_V18_RUNNER_VERSION = "input-live-v18-runner-v1";

interface InputLiveV18JournalEntry {
  index: number;
  event: string;
...
interface InputLiveV18OrchestratorState {
  artifactVersion: "input-live-v18-orchestrator-state-v1";
  ...
  journal: InputLiveV18JournalEntry[];
}

interface LoadedProofInputs {
  plan: InputLiveV18ProofPlan;
  planSha256: string;
  captures: InputLiveV18CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    InputLiveV18SourceIdentity & {
...
export interface InputLiveV18NextAction {
...
const writeState = (
  directory: string,
  state: InputLiveV18OrchestratorState,
): void => {
...
const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, INPUT_LIVE_V18_PLAN_PATH);
  const capturePath = path.join(root, INPUT_LIVE_V18_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as InputLiveV18ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as InputLiveV18CaptureCell[];
  validateInputLiveV18CaptureManifest(captures);
  if (
    plan.captureManifest.sha256 !== inputLiveV18Sha256(captureBytes) ||
    captureArtifact.cellsSha256 !== inputLiveV18CaptureManifestSha256(captures)
  )
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (
      inputLiveV18Sha256(expectedBytes) !== source.expectedScenePlan.sha256 ||
      inputLiveV18Sha256(uncompressed) !==
  ...
  return {
    plan,
    planSha256: inputLiveV18Sha256(planBytes),
    captures,
    captureManifestSha256: inputLiveV18Sha256(captureBytes),
    sources,
...
const appendJournal = (
  ...
  const payloadSha256 = inputLiveV18Sha256(canonicalJson(payload));
  const body = {
  ...
  const entry = {
    ...body,
    entrySha256: inputLiveV18Sha256(canonicalJson(body)),
  };
  writeNew(
    path.join(
      directory,
...
const writerOwnership = (directory: string): InputLiveV18WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): InputLiveV18RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): InputLiveV18Request =>
  readJson<InputLiveV18Request>(file);

export class InputLiveV18Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueInputLiveV18Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: InputLiveV18OrchestratorState = {
      artifactVersion: "input-live-v18-orchestrator-state-v1",
      transactionDirectory: options.transactionDirectory,
      activeRequest: inputLiveV18RequestPath(
        options.transactionDirectory,
        "writer",
      ),
      cleanupRequest: null,
    ...
    return new InputLiveV18Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<InputLiveV18OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new InputLiveV18Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): InputLiveV18NextAction {
    const cleanup =
      this.#state.cleanupRequest &&
      !this.#state.cleanupAccepted &&
      !existsSync(inputLiveV18ReceiptPath(this.#directory, "cleanup"))
        ? this.#state.cleanupRequest
        : null;
  ...
  ingestAndAdvance(rawResponse: string): InputLiveV18NextAction {
  ...
  ingestCleanup(rawResponse: string): InputLiveV18NextAction {
    ...
    persistInputLiveV18RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptInputLiveV18Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV18CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      existsSync(inputLiveV18ReceiptPath(this.#directory, "writer")) &&
      !existsSync(path.join(this.#directory, OWNERSHIP_FILE))
    ) {
      const raw = readInputLiveV18AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateInputLiveV18WriterPayload(raw),
      );
    }
    if (existsSync(inputLiveV18RequestPath(this.#directory, "cleanup")))
      this.#state.cleanupRequest = inputLiveV18RequestPath(
        this.#directory,
        "cleanup",
      );
    if (
      existsSync(path.join(this.#directory, OWNERSHIP_FILE)) &&
      !existsSync(inputLiveV18RequestPath(this.#directory, "cleanup"))
    )
      this.#issueCleanupAndRestore(false);
    else if (
      existsSync(inputLiveV18RequestPath(this.#directory, "cleanup")) &&
      !existsSync(inputLiveV18RequestPath(this.#directory, "restore"))
    )
      this.#issueRestore(false);
    if (
      existsSync(inputLiveV18RequestPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV18ReceiptPath(this.#directory, "restore"))
    )
      this.#state.activeRequest = inputLiveV18RequestPath(
        this.#directory,
        "restore",
      );
    if (
      existsSync(inputLiveV18ReceiptPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV18RequestPath(this.#directory, "extract"))
    ) {
      const ownership = writerOwnership(this.#directory);
      const extract = issueInputLiveV18Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildInputLiveV18ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV18RequestPath(
        this.#directory,
        "extract",
    ...
    if (existsSync(inputLiveV18ReceiptPath(this.#directory, "cleanup")))
      this.#state.cleanupAccepted = true;
    this.#persist();
  }

  #acceptActive(request: InputLiveV18Request): void {
    if (request.phase === "writer") {
      const accepted = acceptInputLiveV18Response(this.#directory, "writer", {
    ...
    if (request.phase === "restore") {
      const accepted = acceptInputLiveV18Response(this.#directory, "restore", {
        validate: (payload) =>
          validateInputLiveV18RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptInputLiveV18Response(this.#directory, "extract", {
        validate: (payload) =>
          validateInputLiveV18ExtractPayload(payload, ownership),
      });
      const proofs = proveInputLiveV18Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseInputFieldRecipe,
        compileInputFieldRecipe,
      ...
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "input-live-v18-host-root-proofs-v1",
      ...
      appendJournal(
        ...
        request.requestId,
        { proofsSha256: inputLiveV18Sha256(canonicalJson(proofs)) },
      );
      const next = issueInputLiveV18Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildInputLiveV18ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV18RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptInputLiveV18Response(this.#directory, "probe", {
        validate: (payload) =>
          validateInputLiveV18ProbePayload(payload, ownership),
      });
      persistInputLiveV18TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      const accepted = acceptInputLiveV18Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateInputLiveV18CapturePayload(payload, cell, rawBytes),
      });
      const png = Buffer.from(accepted.payload.pngBase64, "base64");
      const capturePath = path.join(
        this.#directory,
        "captures",
        `${String(cell.index).padStart(3, "0")}-${inputLiveV18Sha256(cell.cellKey).slice(0, 20)}.png`,
      );
      ...
      if (next < INPUT_LIVE_V18_CAPTURE_COUNT) this.#issueCapture(next);
      else {
          ...
          pngBase64: Buffer.from(
            readFileSync(
              path.join(
                this.#directory,
                "captures",
                `${String(response.index).padStart(3, "0")}-${inputLiveV18Sha256(response.cellKey).slice(0, 20)}.png`,
              ),
            ),
          ).toString("base64"),
        }));
        assertInputLiveV18CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateInputLiveV18Objective(
        ...
        appendJournal(
          ...
          request.requestId,
          {
            reportSha256: inputLiveV18Sha256(canonicalJson(objective)),
  ...
  #issueCleanupAndRestore(journal: boolean): void {
    ...
    const cleanup = issueInputLiveV18Request(
      ...
      this.#privateKey,
      "cleanup",
      buildInputLiveV18CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = inputLiveV18RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueInputLiveV18Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildInputLiveV18RestoreProgram(ownership),
    );
    this.#state.activeRequest = inputLiveV18RequestPath(
      this.#directory,
      "restore",
  ...
  #readCaptureResponseIndex(): InputLiveV18CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: InputLiveV18CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedInputLiveV18Authorization = (
  privateKey: KeyObject,
...
export const generateInputLiveV18SimulationKey = (): KeyObject =>
