...
import {
  validateCalendarLiveV27AntecedentIndex,
  verifyCalendarLiveV27Authorization,
  type CalendarLiveV27AntecedentIndex,
  type CalendarLiveV27AuthorizationProof,
} from "./calendar-live-v27-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V27_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v27/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V27_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v27-security-attestation.json";

export interface CalendarLiveV27SecurityAttestation {
  artifactVersion: "calendar-live-v27-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV27PreflightReport {
  artifactVersion: "calendar-live-v27-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV27SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV27SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV27AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v27-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V27_TARGET) ||
...
export function validateCalendarLiveV27ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV27Authorization");
  const preflight = source.indexOf("runCalendarLiveV27Preflight");
  const initialize = source.indexOf("CalendarLiveV27Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV27Preflight(
  root: string,
  proof: CalendarLiveV27AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V27_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV27PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V27_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V27_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV27AntecedentIndex;
  failures.push(...validateCalendarLiveV27AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V27_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V27_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV27SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v27-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV27ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v27 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV27SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v27-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V27_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV27Preflight(
    process.cwd(),
    verifyCalendarLiveV27Authorization(),
    attestationPath,
