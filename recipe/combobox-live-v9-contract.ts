...
import { buildComboboxLiveV9CleanupRuntime } from "./combobox-live-v9-cleanup.js";
import {
  type ComboboxLiveV9FixedFixedPoint,
} from "./combobox-live-v9-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV9RestoreProgram,
  validateComboboxLiveV9RestorePayload,
  type ComboboxLiveV9RestorePayload,
} from "./combobox-live-v9-restore.js";
import {
  buildComboboxLiveV9RawPropertyRuntime,
  normalizeComboboxLiveV9Scene,
  type ComboboxLiveV9RawNode,
} from "./combobox-live-v9-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v9.js";
import {
...
export const COMBOBOX_LIVE_V9_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V9_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V9_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V9_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V9_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V9_SET_COUNT = 4;
export const COMBOBOX_LIVE_V9_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V9_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V9_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V9_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V9_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV9SourceId = (typeof COMBOBOX_LIVE_V9_SOURCE_IDS)[number];

export interface ComboboxLiveV9SourceIdentity {
  source: ComboboxLiveV9SourceId;
...
export interface ComboboxLiveV9WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV9ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV9SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV9RawNode;
    optionScene: ComboboxLiveV9RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV9RootProof {
  source: ComboboxLiveV9SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV9FixedFixedPoint;
}

export interface ComboboxLiveV9ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV9SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV9CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV9SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV9CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV9SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV9CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v9 ${label} must be an object`);
...
export function validateComboboxLiveV9WriterPayload(
  value: unknown,
): ComboboxLiveV9WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V9_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v9 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v9 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v9 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV9SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV9ExtractProgram(
  writer: ComboboxLiveV9WriterOwnership,
  sources: readonly ComboboxLiveV9SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v9 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V9_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V9_NAMESPACE)}
${buildComboboxLiveV9RawPropertyRuntime()}
...
export function validateComboboxLiveV9ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV9WriterOwnership,
): ComboboxLiveV9ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v9 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V9_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v9 extract schema/two-root mismatch");
  return payload as ComboboxLiveV9ExtractPayload;
}

export function proveComboboxLiveV9Roots<Instance>(
  extract: ComboboxLiveV9ExtractPayload,
  sources: readonly (ComboboxLiveV9SourceIdentity & {
...
): ComboboxLiveV9RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v9 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v9 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV9Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV9Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV9FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v9 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v9 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v9 compile lost owned sets");
...
export function assertComboboxLiveV9RootProofs(
  proofs: readonly ComboboxLiveV9RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v9 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v9 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV9ProbeProgram(
  writer: ComboboxLiveV9WriterOwnership,
  sources: readonly ComboboxLiveV9SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V9_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV9ProbePayload(
  value: unknown,
  writer: ComboboxLiveV9WriterOwnership,
): ComboboxLiveV9ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV9ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V9_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V9_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v9 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV9CaptureManifest(
  cells: readonly ComboboxLiveV9CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V9_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V9_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V9_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V9_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v9 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV9CaptureCell[],
): string => {
  validateComboboxLiveV9CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV9CaptureProgram(
  writer: ComboboxLiveV9WriterOwnership,
  cell: ComboboxLiveV9CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V9_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v9 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v9 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V9_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV9CapturePayload(
  value: unknown,
  cell: ComboboxLiveV9CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV9CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV9CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V9_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V9_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v9 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV9CaptureResponses(
  manifest: readonly ComboboxLiveV9CaptureCell[],
  responses: readonly ComboboxLiveV9CapturePayload[],
): void {
  validateComboboxLiveV9CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v9 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV9ObjectiveReport {
  artifactVersion: "combobox-live-v9-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV9Objective(
  manifest: readonly ComboboxLiveV9CaptureCell[],
  responses: readonly ComboboxLiveV9CapturePayload[],
): ComboboxLiveV9ObjectiveReport {
  assertComboboxLiveV9CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v9-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV9CleanupProgram(
  writer: ComboboxLiveV9WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV9CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V9_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v9 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV9CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV9WriterOwnership,
): ComboboxLiveV9CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV9CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v9 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V9_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV9WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
