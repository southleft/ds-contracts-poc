...
import { INPUT_LIVE_V49_INDEX_PATH } from "./build-input-field-live-proof-v49.js";
import {
  INPUT_LIVE_V49_DYNAMIC_TOOL,
  INPUT_LIVE_V49_TARGET,
} from "./input-field-live-v49-broker.js";
import {
  buildInputLiveV49AuthorizationArtifact,
  validateInputLiveV49AntecedentIndex,
  validateInputLiveV49AuthorizationArtifact,
  validateInputLiveV49History,
  validateInputLiveV49UnitTestSource,
  type InputLiveV49AntecedentIndex,
  type InputLiveV49AuthorizationArtifact,
  type InputLiveV49AuthorizationProof,
  type InputLiveV49HistoryState,
} from "./input-field-live-v49-authorization.js";
import {
  inputLiveV49AttestationSha256,
  validateInputLiveV49ControlFlowSource,
  validateInputLiveV49SecurityAttestation,
  type InputLiveV49SecurityAttestation,
} from "./input-field-live-v49-preflight.js";
import { inputLiveV49Sha256 } from "./input-field-live-v49-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V49_INDEX_PATH);
const index = (): InputLiveV49AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV49AntecedentIndex;

const artifact = (): InputLiveV49AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV49AuthorizationArtifact({
...
const pendingState = (): InputLiveV49HistoryState => ({
  index: index(),
  indexSha256: inputLiveV49Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV49HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV49AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV49Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V49_TARGET,
  expectedDynamicTool: INPUT_LIVE_V49_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v49/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV49AuthorizationArtifact,
): InputLiveV49SecurityAttestation => {
  const body: Omit<InputLiveV49SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v49-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V49_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV49AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV49History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV49History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV49History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV49History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV49History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV49UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV49UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV49AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV49Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV49AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV49AuthorizationArtifact(
        planted as InputLiveV49AuthorizationArtifact,
        index(),
        inputLiveV49Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV49SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV49AttestationSha256(
      body as Omit<InputLiveV49SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV49SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV49ControlFlowSource(source), []);
});
