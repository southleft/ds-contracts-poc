...
import {
  validateCalendarLiveV26AntecedentIndex,
  verifyCalendarLiveV26Authorization,
  type CalendarLiveV26AntecedentIndex,
  type CalendarLiveV26AuthorizationProof,
} from "./calendar-live-v26-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V26_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v26/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V26_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v26-security-attestation.json";

export interface CalendarLiveV26SecurityAttestation {
  artifactVersion: "calendar-live-v26-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV26PreflightReport {
  artifactVersion: "calendar-live-v26-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV26SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV26SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV26AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v26-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V26_TARGET) ||
...
export function validateCalendarLiveV26ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV26Authorization");
  const preflight = source.indexOf("runCalendarLiveV26Preflight");
  const initialize = source.indexOf("CalendarLiveV26Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV26Preflight(
  root: string,
  proof: CalendarLiveV26AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V26_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV26PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V26_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V26_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV26AntecedentIndex;
  failures.push(...validateCalendarLiveV26AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V26_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V26_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV26SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v26-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV26ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v26 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV26SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v26-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V26_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV26Preflight(
    process.cwd(),
    verifyCalendarLiveV26Authorization(),
    attestationPath,
