...
test("the writer walks the named zero-glyph fallback for FILL text — the Calendar live v49 class", () => {
  const writerSource = readFileSync("recipe/calendar-figma-writer.ts", "utf8");
  assert.match(
    writerSource,
...
test("probe content sizing accepts the source-named FILL caption — the Calendar live v48 class", () => {
  const contract = readFileSync("recipe/calendar-live-v50-contract.ts", "utf8");
  assert.match(contract, /CALENDAR-PROBE-CAPTION-FILL-IS-SOURCE-NAMED/);
  assert.match(
    contract,
    /node.layoutSizingHorizontal==="HUG"\|\|node.layoutSizingHorizontal==="FILL"/,
  );
  const host = readFileSync("recipe/scene-readback-calendar-v1.ts", "utf8");
  assert.doesNotMatch(host, /CALENDAR-PROBE-CAPTION-FILL-IS-SOURCE-NAMED/);
});
