/**
 * Carried Combobox live v27 host verifier. Uses scene-readback-combobox-v27 so live
...
export const COMBOBOX_LIVE_V27_FIXED_VERIFIER_VERSION = "combobox-live-v27-fixed-point-v1";
export const COMBOBOX_LIVE_V27_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV27FixedThresholds {
...
export interface ComboboxLiveV27FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV27FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV27FixedSourceProbe {
...
export interface ComboboxLiveV27FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV27FixedFixedPoint;
}

export interface ComboboxLiveV27FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV27FixedHardGateInput {
  thresholds: ComboboxLiveV27FixedThresholds;
  materialRegression: ComboboxLiveV27FixedMaterialRegression;
  sourceProbes: ComboboxLiveV27FixedSourceProbe[];
  cells: ComboboxLiveV27FixedCellValidation[];
  sceneProofs: ComboboxLiveV27FixedSceneProof[];
  visualRows: ComboboxLiveV27FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV27FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V27_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV27FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV27FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV27FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV27FixedHardGates(
  input: ComboboxLiveV27FixedHardGateInput,
): ComboboxLiveV27FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV27FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV27FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V27_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV27FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV27FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV27FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
