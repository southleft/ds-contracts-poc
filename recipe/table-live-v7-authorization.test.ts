...
import { TABLE_LIVE_V7_INDEX_PATH } from "./build-table-live-proof-v7.js";
import {
...
import {
  ...
  validateTableLiveV7UnitTestSource,
  type TableLiveV7AntecedentIndex,
  type TableLiveV7AuthorizationArtifact,
...
import {
  ...
  validateTableLiveV7ControlFlowSource,
  validateTableLiveV7SecurityAttestation,
  type TableLiveV7SecurityAttestation,
} from "./table-live-v7-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V7_INDEX_PATH);
const index = (): TableLiveV7AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV7AntecedentIndex;

const artifact = (): TableLiveV7AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV7AuthorizationArtifact({
...
const pendingState = (): TableLiveV7HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV7HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV7AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v7/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV7AuthorizationArtifact,
): TableLiveV7SecurityAttestation => {
  const body: Omit<TableLiveV7SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V7_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV7History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV7History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV7History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV7History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV7History(authorizedState(), "pending").join("\n"),
    /stale history mode.*authorization exists/,
  );
});

test("the exact v6 phase-sensitive self-test defect is planted and table v7 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV7UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v7-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV7UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV7AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV7AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV7AuthorizationArtifact(
        planted as TableLiveV7AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV7SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV7SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV7SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v7-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV7ControlFlowSource(source), []);
});
