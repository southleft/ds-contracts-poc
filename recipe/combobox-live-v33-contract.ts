...
import { buildComboboxLiveV33CleanupRuntime } from "./combobox-live-v33-cleanup.js";
import {
  type ComboboxLiveV33FixedFixedPoint,
} from "./combobox-live-v33-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV33RestoreProgram,
  validateComboboxLiveV33RestorePayload,
  type ComboboxLiveV33RestorePayload,
} from "./combobox-live-v33-restore.js";
import {
  buildComboboxLiveV33RawPropertyRuntime,
  normalizeComboboxLiveV33Scene,
  type ComboboxLiveV33RawNode,
} from "./combobox-live-v33-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v33.js";
import {
...
export const COMBOBOX_LIVE_V33_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V33_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V33_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V33_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V33_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V33_SET_COUNT = 4;
export const COMBOBOX_LIVE_V33_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V33_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V33_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V33_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V33_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV33SourceId = (typeof COMBOBOX_LIVE_V33_SOURCE_IDS)[number];

export interface ComboboxLiveV33SourceIdentity {
  source: ComboboxLiveV33SourceId;
...
export interface ComboboxLiveV33WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV33ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV33SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV33RawNode;
    optionScene: ComboboxLiveV33RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV33RootProof {
  source: ComboboxLiveV33SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV33FixedFixedPoint;
}

export interface ComboboxLiveV33ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV33SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV33CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV33SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV33CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV33SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV33CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v33 ${label} must be an object`);
...
export function validateComboboxLiveV33WriterPayload(
  value: unknown,
): ComboboxLiveV33WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V33_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v33 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v33 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v33 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV33SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV33ExtractProgram(
  writer: ComboboxLiveV33WriterOwnership,
  sources: readonly ComboboxLiveV33SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v33 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V33_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V33_NAMESPACE)}
${buildComboboxLiveV33RawPropertyRuntime()}
...
export function validateComboboxLiveV33ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV33WriterOwnership,
): ComboboxLiveV33ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v33 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V33_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v33 extract schema/two-root mismatch");
  return payload as ComboboxLiveV33ExtractPayload;
}

export function proveComboboxLiveV33Roots<Instance>(
  extract: ComboboxLiveV33ExtractPayload,
  sources: readonly (ComboboxLiveV33SourceIdentity & {
...
): ComboboxLiveV33RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v33 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v33 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV33Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV33Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV33FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v33 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v33 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v33 compile lost owned sets");
...
export function assertComboboxLiveV33RootProofs(
  proofs: readonly ComboboxLiveV33RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v33 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v33 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV33ProbeProgram(
  writer: ComboboxLiveV33WriterOwnership,
  sources: readonly ComboboxLiveV33SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V33_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV33ProbePayload(
  value: unknown,
  writer: ComboboxLiveV33WriterOwnership,
): ComboboxLiveV33ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV33ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V33_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V33_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v33 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV33CaptureManifest(
  cells: readonly ComboboxLiveV33CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V33_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V33_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V33_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V33_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v33 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV33CaptureCell[],
): string => {
  validateComboboxLiveV33CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV33CaptureProgram(
  writer: ComboboxLiveV33WriterOwnership,
  cell: ComboboxLiveV33CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V33_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v33 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v33 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V33_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV33CapturePayload(
  value: unknown,
  cell: ComboboxLiveV33CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV33CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV33CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V33_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V33_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v33 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV33CaptureResponses(
  manifest: readonly ComboboxLiveV33CaptureCell[],
  responses: readonly ComboboxLiveV33CapturePayload[],
): void {
  validateComboboxLiveV33CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v33 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV33ObjectiveReport {
  artifactVersion: "combobox-live-v33-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV33Objective(
  manifest: readonly ComboboxLiveV33CaptureCell[],
  responses: readonly ComboboxLiveV33CapturePayload[],
): ComboboxLiveV33ObjectiveReport {
  assertComboboxLiveV33CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v33-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV33CleanupProgram(
  writer: ComboboxLiveV33WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV33CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V33_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v33 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV33CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV33WriterOwnership,
): ComboboxLiveV33CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV33CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v33 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V33_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV33WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
