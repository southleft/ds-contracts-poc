...
import { buildTableLiveV7CleanupRuntime } from "./table-live-v7-cleanup.js";
import { type TableLiveV7FixedFixedPoint } from "./table-live-v7-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV7RestoreProgram,
  validateTableLiveV7RestorePayload,
  type TableLiveV7RestorePayload,
} from "./table-live-v7-restore.js";
import {
  buildTableLiveV7RawPropertyRuntime,
  normalizeTableLiveV7Scene,
  type TableLiveV7RawNode,
} from "./table-live-v7-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v7.js";
import {
...
export const TABLE_LIVE_V7_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V7_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V7_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V7_VARIANT_COUNT = 20;
export const TABLE_LIVE_V7_SET_COUNT = 6;
export const TABLE_LIVE_V7_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V7_HOST_PHASES = 3;
export const TABLE_LIVE_V7_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V7_RESTORE_COUNT = 8;
export const TABLE_LIVE_V7_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V7_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV7SourceId = (typeof TABLE_LIVE_V7_SOURCE_IDS)[number];
export type TableLiveV7Kind = "table" | "row" | "cell";

export interface TableLiveV7SourceIdentity {
  source: TableLiveV7SourceId;
...
export interface TableLiveV7WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV7ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV7SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV7RawNode;
    rowScene: TableLiveV7RawNode;
    cellScene: TableLiveV7RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV7RootProof {
  source: TableLiveV7SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV7FixedFixedPoint;
}

export interface TableLiveV7ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV7SourceId;
    adapterIdentity: string;
...
export interface TableLiveV7CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV7SourceId;
  adapterIdentity: string;
...
export interface TableLiveV7CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV7SourceId;
  frameWidth: number;
...
export interface TableLiveV7CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v7 ${label} must be an object`);
...
export function validateTableLiveV7WriterPayload(
  value: unknown,
): TableLiveV7WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V7_NAMESPACE ||
    ...
    throw new TypeError("Table live v7 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v7 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v7 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV7SourceIdentity,
  kind: TableLiveV7Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV7ExtractProgram(
  writer: TableLiveV7WriterOwnership,
  sources: readonly TableLiveV7SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v7 extract requires two source identities");
  if (
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V7_NAMESPACE)}
${buildTableLiveV7RawPropertyRuntime()}
...
export function validateTableLiveV7ExtractPayload(
  value: unknown,
  writer: TableLiveV7WriterOwnership,
): TableLiveV7ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v7 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V7_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v7 extract schema/two-root mismatch");
  return payload as TableLiveV7ExtractPayload;
}

export function proveTableLiveV7Roots<Instance>(
  extract: TableLiveV7ExtractPayload,
  sources: readonly (TableLiveV7SourceIdentity & {
...
): TableLiveV7RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v7 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v7 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV7Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV7Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV7Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV7FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v7 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v7 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v7 compile lost owned sets");
...
export function assertTableLiveV7RootProofs(
  proofs: readonly TableLiveV7RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v7 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v7 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV7ProbeProgram(
  writer: TableLiveV7WriterOwnership,
  sources: readonly TableLiveV7SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V7_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
for(const row of byAdapter.values()){
 ...
 const cell0=propertyKey(rowInstance,"Cell 0"),cell0Before=cell0&&rowInstance.componentProperties[cell0].value;let textPropertiesRestored=false;if(cell0){rowInstance.setProperties({[cell0]:"Table v7 deterministic probe"});const changed=nodes(rowInstance).some(node=>node.type==="TEXT"&&node.characters==="Table v7 deterministic probe");rowInstance.setProperties({[cell0]:cell0Before});textPropertiesRestored=changed&&JSON.stringify(original)===JSON.stringify(plain(instance));}
 rowInstance.remove();
...
export function validateTableLiveV7ProbePayload(
  value: unknown,
  writer: TableLiveV7WriterOwnership,
): TableLiveV7ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV7ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V7_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V7_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Table live v7 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV7CaptureManifest(
  cells: readonly TableLiveV7CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V7_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V7_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V7_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V7_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v7 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV7CaptureCell[],
): string => {
  validateTableLiveV7CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV7CaptureProgram(
  writer: TableLiveV7WriterOwnership,
  cell: TableLiveV7CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V7_CAPTURE_COUNT
  )
    throw new TypeError("Table live v7 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v7 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
const frame=figma.createFrame();frame.name="Table v7 ephemeral / "+cell.cellKey;page.appendChild(frame);
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V7_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV7CapturePayload(
  value: unknown,
  cell: TableLiveV7CaptureCell,
  rawResponseBytes: number,
): TableLiveV7CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV7CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V7_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V7_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v7 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV7CaptureResponses(
  manifest: readonly TableLiveV7CaptureCell[],
  responses: readonly TableLiveV7CapturePayload[],
): void {
  validateTableLiveV7CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v7 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV7ObjectiveReport {
  artifactVersion: "table-live-v7-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV7Objective(
  manifest: readonly TableLiveV7CaptureCell[],
  responses: readonly TableLiveV7CapturePayload[],
): TableLiveV7ObjectiveReport {
  assertTableLiveV7CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v7-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV7CleanupProgram(
  writer: TableLiveV7WriterOwnership,
): string {
  const baseProgram = buildTableLiveV7CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V7_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v7 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV7CleanupPayload(
  payload: unknown,
  writer: TableLiveV7WriterOwnership,
): TableLiveV7CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV7CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v7 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V7_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV7WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
