/**
 * Calendar live v48 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V48_FIXED_VERIFIER_VERSION = "calendar-live-v48-fixed-point-v1";
export const CALENDAR_LIVE_V48_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV48FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV48FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV48FixedSourceProbe {
...
export interface CalendarLiveV48FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV48FixedFixedPoint;
}

export interface CalendarLiveV48FixedHardGateInput {
  thresholds: CalendarLiveV48FixedThresholds;
  sourceProbes: CalendarLiveV48FixedSourceProbe[];
  cells: CalendarLiveV48FixedCellValidation[];
  sceneProofs: CalendarLiveV48FixedSceneProof[];
...
export interface CalendarLiveV48FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V48_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV48FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV48FixedHardGates(
  input: CalendarLiveV48FixedHardGateInput,
): CalendarLiveV48FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV48FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V48_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV48FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV48FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV48FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
