...
import {
  TABLE_LIVE_V8_DYNAMIC_TOOL,
  TABLE_LIVE_V8_TARGET,
  type TableLiveV8TransactionAuthorization,
} from "./table-live-v8-broker.js";
import {
  TABLE_LIVE_V8_AUTHORIZATION_PATH,
  TABLE_LIVE_V8_INDEX_PATH,
} from "./build-table-live-proof-v8.js";
import {
  TABLE_LIVE_V8_CAPTURE_COUNT,
  TABLE_LIVE_V8_HOST_PHASES,
  TABLE_LIVE_V8_REMOTE_REQUESTS,
  TABLE_LIVE_V8_SOURCE_ROOTS,
} from "./table-live-v8-contract.js";
import { canonicalJson } from "./normalize.js";

export type TableLiveV8HistoryExpectation = "pending" | "authorized";

export interface TableLiveV8AuthorizationArtifact {
  artifactVersion: "table-live-v8-capture-authorization-v1";
  authorizationId: "table-live-v8";
  status: "authorization declared; runtime security prerequisites still mandatory";
  authorizationIntent: true;
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V8_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV8AntecedentIndex {
  artifactVersion: "table-live-v8-antecedent-index-v1";
  status: string;
...
export interface TableLiveV8HistoryState {
  index?: TableLiveV8AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV8AuthorizationArtifact;
...
export interface TableLiveV8AuthorizationProof
  extends TableLiveV8TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V8_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V8_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V8_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V8_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v8-authorization") ||
  artifactPath.includes("table-live-v8-preflight") ||
  artifactPath.includes("table-live-v8-authorized") ||
  artifactPath.includes("create-table-live-v8-security-attestation") ||
  artifactPath.includes("table-live-pivot-v8-status") ||
...
export function validateTableLiveV8AntecedentIndex(
  index: TableLiveV8AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v8-antecedent-index-v1" ||
    index.authorizationCanBeAddedWithoutAntecedentRebuild !== true
  )
    failures.push("table v8 antecedent index missing or malformed");
  const artifactEntries = Object.entries(index?.artifacts ?? {});
  if (artifactEntries.length === 0)
    failures.push("table v8 antecedent hash denominator is empty");
  for (const [artifactPath, metadata] of artifactEntries) {
    ...
    if (
      ...
      failures.push(`malformed table v8 antecedent hash: ${artifactPath}`);
  }
  ...
  if (index?.hashSetSha256 !== expectedHashSet)
    failures.push("table v8 antecedent hash-set digest mismatch");
  if (
    ...
    failures.push("table v8 antecedent counts missing or drifted");
  for (const required of [
    TABLE_LIVE_V8_AUTHORIZATION_PATH,
    "recipe/table-live-v8-authorization.ts",
    "recipe/table-live-v8-authorization.test.ts",
    "recipe/table-live-v8-preflight.ts",
    "recipe/evidence/table-live-pivot-v8/operator-security-attestation-template.json",
    "recipe/evidence/status-index.json",
  ])
    if (!index?.authorizationLifecycleExcluded?.includes(required))
      failures.push(`table v8 lifecycle exclusion missing: ${required}`);
  const leaks = forbiddenAntecedentFieldPaths(index);
  if (leaks.length)
    failures.push(`table v8 antecedent result leakage: ${leaks.join(",")}`);
  return failures;
}

export function validateTableLiveV8UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV8History\(\s*["']/.test(source) ||
    /readTableLiveV8HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV8AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV8AuthorizationArtifact {
  if (!SHA40.test(options.antecedentCommit))
    throw new TypeError("table v8 antecedent commit must be a full SHA");
  const index = JSON.parse(
    Buffer.from(options.antecedentIndexBytes).toString("utf8"),
  ) as TableLiveV8AntecedentIndex;
  const indexFailures = validateTableLiveV8AntecedentIndex(index);
  if (indexFailures.length)
    throw new TypeError(
      `table v8 antecedent index refused:\n${indexFailures.join("\n")}`,
    );
  const publicKey =
  ...
  return {
    ...
    antecedent: {
      ...
      indexPath: TABLE_LIVE_V8_INDEX_PATH,
    ...
    denominator: {
      remoteRequests: TABLE_LIVE_V8_REMOTE_REQUESTS,
      hostPhases: TABLE_LIVE_V8_HOST_PHASES,
      captures: TABLE_LIVE_V8_CAPTURE_COUNT,
      sourceRoots: TABLE_LIVE_V8_SOURCE_ROOTS,
...
export function validateTableLiveV8AuthorizationArtifact(
  artifact: TableLiveV8AuthorizationArtifact | undefined,
  index: TableLiveV8AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v8-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v8" ||
    artifact.authorizationIntent !== true
  )
    failures.push("table v8 authorization declaration missing or malformed");
  const resultLeaks = forbiddenAntecedentFieldPaths(artifact);
  if (resultLeaks.length)
    failures.push(`table v8 authorization result leakage: ${resultLeaks.join(",")}`);
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V8_INDEX_PATH ||
    artifact?.antecedent?.indexSha256 !== indexSha256 ||
    artifact?.antecedent?.hashSetSha256 !== index?.hashSetSha256
  )
    failures.push("table v8 authorization pins wrong antecedent index");
  if (privateMaterialPaths(artifact).length)
    failures.push("table v8 authorization contains private key or token material");
  try {
    ...
    if (
      ...
      failures.push("table v8 Ed25519 public-key identity mismatch");
  } catch {
    failures.push("table v8 Ed25519 public key is invalid");
  }
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V8_TARGET) ||
    !equalJson(
    ...
    failures.push("wrong table v8 target or dynamic tool");
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V8_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V8_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V8_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V8_SOURCE_ROOTS ||
    artifact?.denominator?.expectedFacts !== index?.counts.expectedSceneFacts
  )
    failures.push("table v8 two-root or capture denominator changed");
  if (
    ...
    failures.push(
      "table v8 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
    ...
    failures.push("table v8 security prerequisites weakened");
  if (
    artifact?.humanSignoff?.mandatory !== true ||
    artifact?.humanSignoff?.status !== "pending"
  )
    failures.push("table v8 human signoff overclaim");
  return failures;
}

export function validateTableLiveV8History(
  state: TableLiveV8HistoryState,
  expected: TableLiveV8HistoryExpectation,
): string[] {
  const failures = validateTableLiveV8AntecedentIndex(state.index);
  if (!SHA40.test(state.codeCommit))
    failures.push("table v8 code commit is not a full SHA");
  ...
  if (
    ...
    failures.push("table v8 antecedent first-add commit is not unique");
  if (!state.antecedentIsAncestorOfCode)
    failures.push("code commit does not descend from table v8 antecedent");
  if (!state.antecedentIndexBytesMatchFirstAddition)
    failures.push("table v8 antecedent index changed after first addition");
  if (!state.antecedentArtifactsMatchIndexAtCommit)
    failures.push("table v8 antecedent commit artifacts do not match index");
  if (!state.workingAntecedentArtifactsMatchIndex)
    failures.push("table v8 antecedent working bytes drifted");
  ...
  failures.push(
    ...validateTableLiveV8AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
  ...
  if (
    ...
    failures.push("table v8 authorization first-add lineage mismatch");
  if (!state.authorizationPresentAtCodeCommit)
    failures.push("table v8 authorization missing from code commit");
  if (!state.authorizationBytesMatchFirstAddition)
    failures.push("table v8 authorization bytes changed after first addition");
  if (!state.authorizationStrictlyDescendsFromAntecedent)
    failures.push("table v8 authorization does not strictly descend from antecedent");
  if (!state.authorizationIsAncestorOfCode)
    failures.push("code commit predates table v8 authorization");
  return failures;
}
...
export function readTableLiveV8HistoryState(
  root = process.cwd(),
): TableLiveV8HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V8_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV8AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V8_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V8_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V8_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV8History(
  expected: TableLiveV8HistoryExpectation,
  root = process.cwd(),
): TableLiveV8HistoryState {
  const state = readTableLiveV8HistoryState(root);
  const failures = validateTableLiveV8History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v8 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV8Authorization(
  root = process.cwd(),
): TableLiveV8AuthorizationProof {
  const state = verifyTableLiveV8History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V8_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v8/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v8.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V8_TARGET,
    expectedDynamicTool: TABLE_LIVE_V8_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V8_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV8HistoryExpectation => {
  ...
  if (pending === authorized)
    throw new Error(
      "Choose exactly one table v8 history mode: --expect-pending or --expect-authorized",
    );
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV8History(expected);
  process.stdout.write(
    `Table live v8 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
