...
import {
  ...
  INPUT_LIVE_V73_INDEX_PATH,
} from "./build-input-field-live-proof-v73.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV73HistoryExpectation = "pending" | "authorized";

export interface InputLiveV73AuthorizationArtifact {
  artifactVersion: "input-live-v73-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V73_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV73AntecedentIndex {
...
export interface InputLiveV73HistoryState {
  index?: InputLiveV73AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV73AuthorizationArtifact;
...
export interface InputLiveV73AuthorizationProof extends InputLiveV73TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V73_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V73_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V73_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V73_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v73-authorization") ||
  artifactPath.includes("input-field-live-v73-preflight") ||
  artifactPath.includes("input-field-live-v73-authorized") ||
  artifactPath.includes("input-field-live-pivot-v73-status") ||
...
export function validateInputLiveV73AntecedentIndex(
  index: InputLiveV73AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v73-antecedent-index-v1" ||
...
export function validateInputLiveV73UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV73History\(\s*["']/.test(source) ||
    /readInputLiveV73HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV73AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV73AuthorizationArtifact {
  ...
  ) as InputLiveV73AntecedentIndex;
  const indexFailures = validateInputLiveV73AntecedentIndex(index);
...
export function validateInputLiveV73AuthorizationArtifact(
  artifact: InputLiveV73AuthorizationArtifact | undefined,
  index: InputLiveV73AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v73-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v73" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V73_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V73_TARGET) ||
    !equalJson(
...
export function validateInputLiveV73History(
  state: InputLiveV73HistoryState,
  expected: InputLiveV73HistoryExpectation,
): string[] {
  const failures = validateInputLiveV73AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV73AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV73HistoryState(
  root = process.cwd(),
): InputLiveV73HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V73_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV73AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V73_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V73_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V73_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV73History(
  expected: InputLiveV73HistoryExpectation,
  root = process.cwd(),
): InputLiveV73HistoryState {
  const state = readInputLiveV73HistoryState(root);
  const failures = validateInputLiveV73History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV73Authorization(
  root = process.cwd(),
): InputLiveV73AuthorizationProof {
  const state = verifyInputLiveV73History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V73_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v73/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v73.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V73_TARGET,
    expectedDynamicTool: INPUT_LIVE_V73_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V73_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV73HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV73History(expected);
