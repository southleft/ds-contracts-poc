...
import { CALENDAR_LIVE_V34_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v34.js";
import { verifyCalendarLiveV34Authorization } from "./calendar-live-v34-authorization.js";
import { runCalendarLiveV34Preflight } from "./calendar-live-v34-preflight.js";
import { CalendarLiveV34Orchestrator } from "./run-calendar-live-v34.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v34 requires exact authorization artifact ${CALENDAR_LIVE_V34_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v34-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV34Authorization(root);
    runCalendarLiveV34Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV34Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV34Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
