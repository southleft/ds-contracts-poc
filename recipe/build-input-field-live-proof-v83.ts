...
import {
  buildInputLiveV83CaptureProgram,
  buildInputLiveV83ExtractProgram,
  buildInputLiveV83ProbeProgram,
  buildInputLiveV83RestoreProgram,
  INPUT_LIVE_V83_CAPTURE_COUNT,
  INPUT_LIVE_V83_RESPONSE_CONTRACTS,
  inputLiveV83CaptureManifestSha256,
  type InputLiveV83CaptureCell,
  type InputLiveV83SourceIdentity,
  type InputLiveV83WriterOwnership,
} from "./input-field-live-v83-contract.js";
import {
  INPUT_LIVE_V83_DYNAMIC_TOOL,
  INPUT_LIVE_V83_TARGET,
  inputLiveV83RequestSequence,
...
} from "./input-field-live-v83-restore.js";

export const INPUT_LIVE_V83_EVIDENCE_ROOT =
  "recipe/evidence/input-field-live-pivot-v83";
export const INPUT_LIVE_V83_PROTOCOL_PATH = `${INPUT_LIVE_V83_EVIDENCE_ROOT}/protocol.json`;
export const INPUT_LIVE_V83_PLAN_PATH = `${INPUT_LIVE_V83_EVIDENCE_ROOT}/proof-plan.json`;
export const INPUT_LIVE_V83_CAPTURE_MANIFEST_PATH = `${INPUT_LIVE_V83_EVIDENCE_ROOT}/capture-manifest.json`;
export const INPUT_LIVE_V83_REQUEST_MANIFEST_PATH = `${INPUT_LIVE_V83_EVIDENCE_ROOT}/request-manifest.json`;
export const INPUT_LIVE_V83_INDEX_PATH = `${INPUT_LIVE_V83_EVIDENCE_ROOT}/antecedent-index.json`;
export const INPUT_LIVE_V83_AUTHORIZATION_TEMPLATE_PATH = `${INPUT_LIVE_V83_EVIDENCE_ROOT}/authorization-template.json`;
export const INPUT_LIVE_V83_AUTHORIZATION_PATH = `${INPUT_LIVE_V83_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface InputLiveV83ProofPlan {
  artifactVersion: "input-live-v83-proof-plan-v1";
  status: "draft antecedent; pending separate authorization; live execution forbidden";
  target: typeof INPUT_LIVE_V83_TARGET;
...
interface GeneratedSource {
  ...
  recipeHash: string;
  envelopeHash: string;
  expectedScenePlan: InputLiveV83SourceIdentity["expectedScenePlan"];
  outputPath: string;
  compressed: Buffer;
  metadata: InputLiveV83ProofPlan["sources"][number]["expectedScenePlan"];
}
...
const sourcePlans = (): GeneratedSource[] => {
  ...
  return prior.sources.map((source: Record<string, any>) => {
    ...
    if (!rewrittenText.includes(needle))
      throw new Error(
        `Input live v83 missing set layout.mode vertical fact: ${inputPath}`,
      );
    ...
    if (!modeRewritten.includes(paddingNeedle))
      throw new Error(
        `Input live v83 missing set layout.padding 0 fact: ${inputPath}`,
      );
    ...
    if (sourceId === "mui" && visibleFalseCount !== 24)
      throw new Error(
        `Input live v83 MUI expected-plan visible:false count ${visibleFalseCount}, expected 24`,
      );
    if (sourceId === "polaris" && visibleFalseCount !== 0)
      throw new Error(
        `Input live v83 Polar expected-plan has ${visibleFalseCount} visible:false facts; do not restamp Polar`,
      );
    ...
    if (sourceId === "mui" && occupancyKeys.length !== 24)
      throw new Error(
        `Input live v83 MUI occupancy keys ${occupancyKeys.length}, expected 24`,
      );
    if (sourceId === "polaris" && occupancyKeys.length !== 0)
      throw new Error(
        `Input live v83 Polar expected-plan has ${occupancyKeys.length} occupancy keys; do not restamp Polar`,
      );
    ...
    if (sourceId === "mui") {
      for (const key of occupancyKeys) {
        ...
        if (occurrences !== 1)
          throw new Error(
            `Input live v83 occupancy opacity:1 fact count ${occurrences} for ${key}`,
          );
    ...
    if (
      ...
      throw new Error(
        `Input live v83 MUI occupancy opacity rewrite failed: zero=${occupancyOpacityZero} one=${occupancyOpacityOne} otherOne=${otherOpacityOne}`,
      );
    if (
      sourceId === "polaris" &&
      ...
      throw new Error(
        `Input live v83 Polar opacity facts drifted; do not restamp Polar`,
      );
    ...
    if (sourceId === "mui" && (visibleFalseAfter !== 0 || visibleTrueAfter !== 1217))
      throw new Error(
        `Input live v83 MUI visible rewrite failed: false=${visibleFalseAfter} true=${visibleTrueAfter}`,
      );
    if (sourceId === "polaris" && (visibleFalseAfter !== 0 || visibleTrueAfter !== 1089))
      throw new Error(
        `Input live v83 Polar visible facts drifted: false=${visibleFalseAfter} true=${visibleTrueAfter}`,
      );
    const setMode = plan.facts.find((fact) => fact.id === "root#layout.mode@0000");
    if (setMode?.value !== "horizontal")
      throw new Error(
        `Input live v83 set layout.mode rewrite failed: ${inputPath}`,
      );
    ...
    if (
      ...
      throw new Error(
        `Input live v83 set layout.padding rewrite failed: ${inputPath}`,
      );
    ...
    if (sourceId === "polaris") {
      ...
      if (!priorUncompressed.equals(rewritten))
        throw new Error(
          "Input live v83 Polar uncompressed drifted from V82; do not restamp Polar",
        );
    }
    return {
      source: sourceId,
      ...
      outputPath: `${INPUT_LIVE_V83_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
      compressed: rewrittenCompressed,
      metadata: {
        path: `${INPUT_LIVE_V83_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
...
const captureCells = (): InputLiveV83CaptureCell[] => {
  const cells = json<Record<string, any>>(V16_CAPTURE_MANIFEST)
    .cells as InputLiveV83CaptureCell[];
  inputLiveV83CaptureManifestSha256(cells);
  return cells;
};

const protocol = () => ({
  artifactVersion: "input-live-v83-external-operator-protocol-v1",
  protocolId: "input-live-v83",
  status:
  ...
  operatorBoundary: {
    expectedDynamicTool: INPUT_LIVE_V83_DYNAMIC_TOOL,
    target: INPUT_LIVE_V83_TARGET,
  ...
  hostNormalization: {
    ...
    carriedSceneReadback: "recipe/scene-readback-v83.ts",
    carriedSceneReadbackRuntime: "recipe/scene-readback-runtime-v83.ts",
    carriedV3Verifier: "recipe/input-field-live-v3-verifier-v83.ts",
...
const requestManifest = (
  captures: readonly InputLiveV83CaptureCell[],
  programs: {
    writerSha256: string;
  ...
  const requests = [
    {
      requestId: "writer",
      sequence: inputLiveV83RequestSequence("writer"),
      phase: "writer",
      predecessorRequestId: null,
      programSha256: programs.writerSha256,
      response: INPUT_LIVE_V83_RESPONSE_CONTRACTS.writer,
    },
    {
      requestId: "cleanup",
      sequence: inputLiveV83RequestSequence("cleanup"),
      phase: "cleanup",
      predecessorRequestId: "writer",
      programBuilderSha256: programs.cleanupBuilderSha256,
      response: INPUT_LIVE_V83_RESPONSE_CONTRACTS.cleanup,
      availability: "persisted immediately after writer acceptance",
    },
    {
      requestId: "restore",
      sequence: inputLiveV83RequestSequence("restore"),
      phase: "restore",
      predecessorRequestId: "writer",
      programSha256: programs.restoreBlueprintSha256,
      response: INPUT_LIVE_V83_RESPONSE_CONTRACTS.restore,
      availability:
        "issued immediately after writer acceptance; before extract",
    },
    {
      requestId: "extract",
      sequence: inputLiveV83RequestSequence("extract"),
      phase: "extract",
      predecessorRequestId: "restore",
      programSha256: programs.extractBlueprintSha256,
      response: INPUT_LIVE_V83_RESPONSE_CONTRACTS.extract,
    },
    {
      requestId: "probe",
      sequence: inputLiveV83RequestSequence("probe"),
      phase: "probe",
      predecessorRequestId: "extract",
      programSha256: programs.probeBlueprintSha256,
      response: INPUT_LIVE_V83_RESPONSE_CONTRACTS.probe,
    },
    ...captures.map((cell) => ({
      requestId: `capture-${String(cell.index).padStart(3, "0")}`,
      sequence: inputLiveV83RequestSequence("capture", cell.index),
      phase: "capture",
      ...
      response: INPUT_LIVE_V83_RESPONSE_CONTRACTS.capture,
    })),
  ];
  return {
    artifactVersion: "input-live-v83-request-manifest-v1",
    expectedDynamicTool: INPUT_LIVE_V83_DYNAMIC_TOOL,
    target: INPUT_LIVE_V83_TARGET,
...
const lifecycleExcludedPaths = [
  INPUT_LIVE_V83_AUTHORIZATION_TEMPLATE_PATH,
  INPUT_LIVE_V83_AUTHORIZATION_PATH,
  "recipe/input-field-live-v83-authorization.ts",
  "recipe/input-field-live-v83-authorization.test.ts",
  "recipe/input-field-live-v83-preflight.ts",
  "recipe/input-field-live-v83-authorized.ts",
  "recipe/evidence/input-field-live-pivot-v83/operator-security-attestation-template.json",
...
const sourceNeutralDependencies = [
  ...
  "recipe/normalize.ts",
  "recipe/scene-readback-runtime-v83.ts",
  "recipe/scene-readback-v83.ts",
  "recipe/input-field-live-v83-restore.ts",
  "recipe/input-field-figma-writer-v18.ts",
  "recipe/input-field-figma-writer-v19.ts",
] as const;

export async function buildInputLiveV83Proof(
  check = process.argv.includes("--check"),
): Promise<InputLiveV83ProofPlan> {
  ...
  if (
    ...
    throw new Error("Input live v83 frozen v16 writer bytes drifted");
  ...
  if (
    ...
    throw new Error(
      "Input live v83 exact committed v16 writer/restore/runtime/extract or hashed v16/v17/v18/v19 writer or v17-v66/v70/v82 scene-readback bytes drifted",
    );

  const writerOwnershipBlueprint: InputLiveV83WriterOwnership = {
  ...
  const identities = sources.map(
    (source) =>
      ...
      }) satisfies InputLiveV83SourceIdentity,
  );
  const restoreBlueprint = Buffer.from(
    buildInputLiveV83RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildInputLiveV83ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildInputLiveV83ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildInputLiveV83CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );

  const antecedentOutputs = new Map<string, Buffer>();
  antecedentOutputs.set(INPUT_LIVE_V83_PROTOCOL_PATH, jsonBytes(protocol()));
  antecedentOutputs.set(
    `${INPUT_LIVE_V83_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V83_EVIDENCE_ROOT}/programs/writer-payload.js`,
    writerPayload,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V83_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V83_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V83_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V83_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  ...
  const captureManifest = {
    artifactVersion: "input-live-v83-capture-manifest-v1",
    ...
    cells: captures,
    cellsSha256: inputLiveV83CaptureManifestSha256(captures),
  };
  const captureManifestBytes = jsonBytes(captureManifest);
  antecedentOutputs.set(
    INPUT_LIVE_V83_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: InputLiveV83ProofPlan = {
    artifactVersion: "input-live-v83-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: INPUT_LIVE_V83_TARGET,
    namespace: "ds.contracts.input.recipe.v5",
    writer: {
      programPath: `${INPUT_LIVE_V83_EVIDENCE_ROOT}/programs/writer.txt`,
      programBytes: writerProgram.byteLength,
      programSha256: sha256(writerProgram),
      payloadPath: `${INPUT_LIVE_V83_EVIDENCE_ROOT}/programs/writer-payload.js`,
    ...
    captureManifest: {
      path: INPUT_LIVE_V83_CAPTURE_MANIFEST_PATH,
      cells: 128,
      requests: 128,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(INPUT_LIVE_V83_PLAN_PATH, proofPlanBytes);
  ...
  antecedentOutputs.set(
    INPUT_LIVE_V83_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/input-field-live-v83-broker.ts",
    "recipe/input-field-live-v83-contract.ts",
    "recipe/run-input-field-live-v83.ts",
    "recipe/build-input-field-live-proof-v83.ts",
    "recipe/input-field-live-v83-broker.test.ts",
    "recipe/figma-property-normalizer-v8.test.ts",
    "recipe/scene-readback-v83.test.ts",
    "recipe/input-field-live-v83-restore.ts",
    "recipe/input-field-live-v83-restore.test.ts",
    "recipe/input-field-live-v83-lifecycle-simulation.ts",
    ...sourceNeutralDependencies,
  ].filter((artifactPath) => artifactPath !== INPUT_LIVE_V83_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "input-live-v83-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    ...
    counts: {
      ...
      captureCells: INPUT_LIVE_V83_CAPTURE_COUNT,
  ...
  antecedentOutputs.set(INPUT_LIVE_V83_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "input-live-v83-authorization-template-v1",
    authorizationId: "input-live-v83",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: INPUT_LIVE_V83_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
  ...
  if (check) {
    ...
    mkdirSync(path.dirname(INPUT_LIVE_V83_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(INPUT_LIVE_V83_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(INPUT_LIVE_V83_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(INPUT_LIVE_V83_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildInputLiveV83Proof(), null, 2)}\n`,
  );
