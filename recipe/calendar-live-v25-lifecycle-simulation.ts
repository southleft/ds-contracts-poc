...
import { calendarLiveV1Sha256 } from "./calendar-live-v25-broker.js";

export interface CalendarLiveV25LifecycleSimulationReport {
  artifactVersion: "calendar-live-v25-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV25AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV25Lifecycle(): Promise<CalendarLiveV25LifecycleSimulationReport> {
  await buildCalendarLiveV25Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V25_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV25AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v25-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V25_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V25_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV25History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV25History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV25AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V25_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V25_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV25History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV25History(authorized, "pending");
    ...
    await buildCalendarLiveV25Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V25_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV25AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV25Lifecycle(), null, 2)}\n`,
  );
