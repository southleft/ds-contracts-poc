...
export type ComboboxLiveV29HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV29AuthorizationArtifact {
  artifactVersion: "combobox-live-v29-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V29_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV29AntecedentIndex {
...
export interface ComboboxLiveV29HistoryState {
  index?: ComboboxLiveV29AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV29AuthorizationArtifact;
...
export interface ComboboxLiveV29AuthorizationProof extends ComboboxLiveV29TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V29_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V29_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V29_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V29_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v29-authorization") ||
  artifactPath.includes("combobox-live-v29-preflight") ||
  artifactPath.includes("combobox-live-v29-authorized") ||
  artifactPath.includes("create-combobox-live-v29-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v29-status") ||
...
export function validateComboboxLiveV29AntecedentIndex(
  index: ComboboxLiveV29AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v29-antecedent-index-v1" ||
...
export function validateComboboxLiveV29UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV29History\(\s*["']/.test(source) ||
    /readComboboxLiveV29HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV29AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV29AuthorizationArtifact {
  ...
  ) as ComboboxLiveV29AntecedentIndex;
  const indexFailures = validateComboboxLiveV29AntecedentIndex(index);
...
export function validateComboboxLiveV29AuthorizationArtifact(
  artifact: ComboboxLiveV29AuthorizationArtifact | undefined,
  index: ComboboxLiveV29AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v29-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v29" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V29_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V29_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V29_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V29_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V29_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V29_SOURCE_ROOTS ||
...
export function validateComboboxLiveV29History(
  state: ComboboxLiveV29HistoryState,
  expected: ComboboxLiveV29HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV29AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV29AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV29HistoryState(
  root = process.cwd(),
): ComboboxLiveV29HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V29_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV29AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V29_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V29_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V29_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV29History(
  expected: ComboboxLiveV29HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV29HistoryState {
  const state = readComboboxLiveV29HistoryState(root);
  const failures = validateComboboxLiveV29History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v29 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV29Authorization(
  root = process.cwd(),
): ComboboxLiveV29AuthorizationProof {
  const state = verifyComboboxLiveV29History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V29_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v29/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v29.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V29_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V29_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V29_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV29HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV29History(expected);
  process.stdout.write(
    `Combobox live v29 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
