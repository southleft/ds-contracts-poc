...
import { TABLE_LIVE_V28_INDEX_PATH } from "./build-table-live-proof-v28.js";
import {
...
import {
  ...
  validateTableLiveV28History,
  validateTableLiveV28UnitTestSource,
  type TableLiveV28AntecedentIndex,
  type TableLiveV28AuthorizationArtifact,
  type TableLiveV28AuthorizationProof,
  type TableLiveV28HistoryState,
} from "./table-live-v28-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV28ControlFlowSource,
  validateTableLiveV28SecurityAttestation,
  type TableLiveV28SecurityAttestation,
} from "./table-live-v28-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V28_INDEX_PATH);
const index = (): TableLiveV28AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV28AntecedentIndex;

const artifact = (): TableLiveV28AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV28AuthorizationArtifact({
...
const pendingState = (): TableLiveV28HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV28HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV28AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v28/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV28AuthorizationArtifact,
): TableLiveV28SecurityAttestation => {
  const body: Omit<TableLiveV28SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V28_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV28History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV28History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV28History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV28History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV28History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV28UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v28-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV28UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV28AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV28AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV28AuthorizationArtifact(
        planted as TableLiveV28AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV28SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV28SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV28SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v28-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV28ControlFlowSource(source), []);
});
