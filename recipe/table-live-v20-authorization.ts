...
export type TableLiveV20HistoryExpectation = "pending" | "authorized";

export interface TableLiveV20AuthorizationArtifact {
  artifactVersion: "table-live-v20-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V20_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV20AntecedentIndex {
...
export interface TableLiveV20HistoryState {
  index?: TableLiveV20AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV20AuthorizationArtifact;
...
export interface TableLiveV20AuthorizationProof
  extends TableLiveV20TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V20_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V20_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V20_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V20_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v20-authorization") ||
  artifactPath.includes("table-live-v20-preflight") ||
  artifactPath.includes("table-live-v20-authorized") ||
  artifactPath.includes("create-table-live-v20-security-attestation") ||
  artifactPath.includes("table-live-pivot-v20-status") ||
...
export function validateTableLiveV20AntecedentIndex(
  index: TableLiveV20AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v20-antecedent-index-v1" ||
...
export function validateTableLiveV20UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV20History\(\s*["']/.test(source) ||
    /readTableLiveV20HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV20AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV20AuthorizationArtifact {
  ...
  ) as TableLiveV20AntecedentIndex;
  const indexFailures = validateTableLiveV20AntecedentIndex(index);
...
export function validateTableLiveV20AuthorizationArtifact(
  artifact: TableLiveV20AuthorizationArtifact | undefined,
  index: TableLiveV20AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v20-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v20" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V20_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V20_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V20_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V20_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V20_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V20_SOURCE_ROOTS ||
...
export function validateTableLiveV20History(
  state: TableLiveV20HistoryState,
  expected: TableLiveV20HistoryExpectation,
): string[] {
  const failures = validateTableLiveV20AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV20AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV20HistoryState(
  root = process.cwd(),
): TableLiveV20HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V20_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV20AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V20_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V20_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V20_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV20History(
  expected: TableLiveV20HistoryExpectation,
  root = process.cwd(),
): TableLiveV20HistoryState {
  const state = readTableLiveV20HistoryState(root);
  const failures = validateTableLiveV20History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v20 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV20Authorization(
  root = process.cwd(),
): TableLiveV20AuthorizationProof {
  const state = verifyTableLiveV20History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V20_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v20/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v20.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V20_TARGET,
    expectedDynamicTool: TABLE_LIVE_V20_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V20_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV20HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV20History(expected);
  process.stdout.write(
    `Table live v20 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
