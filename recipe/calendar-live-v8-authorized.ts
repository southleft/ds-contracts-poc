...
import { CALENDAR_LIVE_V8_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v8.js";
import { verifyCalendarLiveV8Authorization } from "./calendar-live-v8-authorization.js";
import { runCalendarLiveV8Preflight } from "./calendar-live-v8-preflight.js";
import { CalendarLiveV8Orchestrator } from "./run-calendar-live-v8.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v8 requires exact authorization artifact ${CALENDAR_LIVE_V8_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v8-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV8Authorization(root);
    runCalendarLiveV8Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV8Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV8Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
