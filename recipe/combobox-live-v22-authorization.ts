...
export type ComboboxLiveV22HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV22AuthorizationArtifact {
  artifactVersion: "combobox-live-v22-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V22_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV22AntecedentIndex {
...
export interface ComboboxLiveV22HistoryState {
  index?: ComboboxLiveV22AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV22AuthorizationArtifact;
...
export interface ComboboxLiveV22AuthorizationProof extends ComboboxLiveV22TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V22_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V22_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V22_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V22_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v22-authorization") ||
  artifactPath.includes("combobox-live-v22-preflight") ||
  artifactPath.includes("combobox-live-v22-authorized") ||
  artifactPath.includes("create-combobox-live-v22-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v22-status") ||
...
export function validateComboboxLiveV22AntecedentIndex(
  index: ComboboxLiveV22AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v22-antecedent-index-v1" ||
...
export function validateComboboxLiveV22UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV22History\(\s*["']/.test(source) ||
    /readComboboxLiveV22HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV22AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV22AuthorizationArtifact {
  ...
  ) as ComboboxLiveV22AntecedentIndex;
  const indexFailures = validateComboboxLiveV22AntecedentIndex(index);
...
export function validateComboboxLiveV22AuthorizationArtifact(
  artifact: ComboboxLiveV22AuthorizationArtifact | undefined,
  index: ComboboxLiveV22AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v22-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v22" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V22_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V22_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V22_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V22_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V22_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V22_SOURCE_ROOTS ||
  ...
  if (
    ...
    failures.push(
      "v22 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
...
export function validateComboboxLiveV22History(
  state: ComboboxLiveV22HistoryState,
  expected: ComboboxLiveV22HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV22AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV22AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV22HistoryState(
  root = process.cwd(),
): ComboboxLiveV22HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V22_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV22AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V22_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V22_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V22_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV22History(
  expected: ComboboxLiveV22HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV22HistoryState {
  const state = readComboboxLiveV22HistoryState(root);
  const failures = validateComboboxLiveV22History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v22 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV22Authorization(
  root = process.cwd(),
): ComboboxLiveV22AuthorizationProof {
  const state = verifyComboboxLiveV22History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V22_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v22/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v22.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V22_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V22_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V22_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV22HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV22History(expected);
  process.stdout.write(
    `Combobox live v22 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
