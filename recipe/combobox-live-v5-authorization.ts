...
export type ComboboxLiveV5HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV5AuthorizationArtifact {
  artifactVersion: "combobox-live-v5-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V5_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV5AntecedentIndex {
...
export interface ComboboxLiveV5HistoryState {
  index?: ComboboxLiveV5AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV5AuthorizationArtifact;
...
export interface ComboboxLiveV5AuthorizationProof extends ComboboxLiveV5TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V5_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V5_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V5_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V5_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v5-authorization") ||
  artifactPath.includes("combobox-live-v5-preflight") ||
  artifactPath.includes("combobox-live-v5-authorized") ||
  artifactPath.includes("create-combobox-live-v5-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v5-status") ||
...
export function validateComboboxLiveV5AntecedentIndex(
  index: ComboboxLiveV5AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v5-antecedent-index-v1" ||
...
export function validateComboboxLiveV5UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV5History\(\s*["']/.test(source) ||
    /readComboboxLiveV5HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV5AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV5AuthorizationArtifact {
  ...
  const indexFailures = validateComboboxLiveV5AntecedentIndex(index);
...
export function validateComboboxLiveV5AuthorizationArtifact(
  artifact: ComboboxLiveV5AuthorizationArtifact | undefined,
  index: ComboboxLiveV5AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v5-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v5" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V5_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V5_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V5_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V5_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V5_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V5_SOURCE_ROOTS ||
...
export function validateComboboxLiveV5History(
  state: ComboboxLiveV5HistoryState,
  expected: ComboboxLiveV5HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV5AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV5AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV5HistoryState(
  root = process.cwd(),
): ComboboxLiveV5HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V5_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV5AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V5_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V5_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V5_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV5History(
  expected: ComboboxLiveV5HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV5HistoryState {
  const state = readComboboxLiveV5HistoryState(root);
  const failures = validateComboboxLiveV5History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyComboboxLiveV5Authorization(
  root = process.cwd(),
): ComboboxLiveV5AuthorizationProof {
  const state = verifyComboboxLiveV5History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V5_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v5/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v5.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V5_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V5_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V5_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV5HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV5History(expected);
