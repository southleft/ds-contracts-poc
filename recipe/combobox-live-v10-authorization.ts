...
export type ComboboxLiveV10HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV10AuthorizationArtifact {
  artifactVersion: "combobox-live-v10-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V10_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV10AntecedentIndex {
...
export interface ComboboxLiveV10HistoryState {
  index?: ComboboxLiveV10AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV10AuthorizationArtifact;
...
export interface ComboboxLiveV10AuthorizationProof extends ComboboxLiveV10TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V10_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V10_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V10_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V10_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v10-authorization") ||
  artifactPath.includes("combobox-live-v10-preflight") ||
  artifactPath.includes("combobox-live-v10-authorized") ||
  artifactPath.includes("create-combobox-live-v10-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v10-status") ||
...
export function validateComboboxLiveV10AntecedentIndex(
  index: ComboboxLiveV10AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v10-antecedent-index-v1" ||
...
export function validateComboboxLiveV10UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV10History\(\s*["']/.test(source) ||
    /readComboboxLiveV10HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV10AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV10AuthorizationArtifact {
  ...
  ) as ComboboxLiveV10AntecedentIndex;
  const indexFailures = validateComboboxLiveV10AntecedentIndex(index);
...
export function validateComboboxLiveV10AuthorizationArtifact(
  artifact: ComboboxLiveV10AuthorizationArtifact | undefined,
  index: ComboboxLiveV10AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v10-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v10" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V10_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V10_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V10_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V10_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V10_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V10_SOURCE_ROOTS ||
  ...
  if (
    ...
    failures.push(
      "v10 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
...
export function validateComboboxLiveV10History(
  state: ComboboxLiveV10HistoryState,
  expected: ComboboxLiveV10HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV10AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV10AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV10HistoryState(
  root = process.cwd(),
): ComboboxLiveV10HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V10_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV10AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V10_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V10_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V10_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV10History(
  expected: ComboboxLiveV10HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV10HistoryState {
  const state = readComboboxLiveV10HistoryState(root);
  const failures = validateComboboxLiveV10History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v10 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV10Authorization(
  root = process.cwd(),
): ComboboxLiveV10AuthorizationProof {
  const state = verifyComboboxLiveV10History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V10_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v10/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v10.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V10_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V10_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V10_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV10HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV10History(expected);
  process.stdout.write(
    `Combobox live v10 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
