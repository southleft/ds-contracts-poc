...
import {
  assertInputLiveV69RootProofs,
  INPUT_LIVE_V69_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V69_RESPONSE_CONTRACTS,
  type InputLiveV69RootProof,
} from "./input-field-live-v69-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V69_BROKER_VERSION = "input-live-v69-broker-v2";
export const INPUT_LIVE_V69_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V69_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V69_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V69_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V69_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V69_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV69RemotePhase =
  (typeof INPUT_LIVE_V69_REMOTE_PHASES)[number];
export type InputLiveV69AuthorizationMode = "live" | "simulated";

export interface InputLiveV69CommitPins {
...
export interface InputLiveV69TransactionAuthorization extends InputLiveV69CommitPins {
  mode: InputLiveV69AuthorizationMode;
...
export interface InputLiveV69Manifest {
  artifactVersion: "input-live-v69-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V69_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V69_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V69_TARGET;
  authorization: InputLiveV69TransactionAuthorization;
...
export interface InputLiveV69ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV69RequestBody {
  artifactVersion: "input-live-v69-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V69_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV69RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV69CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V69_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV69ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV69Request extends InputLiveV69RequestBody {
...
export interface InputLiveV69Receipt {
  artifactVersion: "input-live-v69-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V69_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV69RemotePhase;
...
export interface InputLiveV69AcceptedResponse<T = unknown> {
  request: InputLiveV69Request;
  receipt: InputLiveV69Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV69IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV69ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V69_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V69_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V69_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V69_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V69_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V69_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V69_TARGET.editorType;
};
...
export const inputLiveV69RequestSequence = (
  phase: InputLiveV69RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV69RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV69RequestPath = (
  directory: string,
  phase: InputLiveV69RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV69ResponsePath = (
  directory: string,
  phase: InputLiveV69RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV69ReceiptPath = (
  directory: string,
  phase: InputLiveV69RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV69Manifest => {
  const manifest = json<InputLiveV69Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v69-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V69_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V69_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV69RemotePhase,
  captureIndex?: number,
): InputLiveV69Request | undefined => {
  const file = inputLiveV69RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV69Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV69RemotePhase,
  captureIndex?: number,
): InputLiveV69Receipt | undefined => {
  const file = inputLiveV69ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV69Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV69Request | undefined;
  receipt: InputLiveV69Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV69RemotePhase,
): InputLiveV69ResponseContract => ({
  ...INPUT_LIVE_V69_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V69_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V69_DYNAMIC_TOOL,
      target: INPUT_LIVE_V69_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v69-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V69_BROKER_VERSION,
  target: INPUT_LIVE_V69_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V69_PROGRAM_START*/${program}/*INPUT_LIVE_V69_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V69_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V69_PROGRAM_END*/";
...
export function createInputLiveV69Transaction(
  directory: string,
  ...
  const manifest: InputLiveV69Manifest = {
    artifactVersion: "input-live-v69-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V69_BROKER_VERSION,
...
export function validateInputLiveV69Request(
  directory: string,
  request: InputLiveV69Request,
): void {
  ...
  const expectedSequence = inputLiveV69RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v69-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V69_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V69_TARGET.fileKey ||
...
export function issueInputLiveV69Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV69RemotePhase,
  program: string,
  options: InputLiveV69IssueOptions = {},
): InputLiveV69Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV69RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV69Request>(targetPath);
    validateInputLiveV69Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV69ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV69TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV69RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V69_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V69_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V69_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV69RequestBody = {
    artifactVersion: "input-live-v69-broker-request-v2",
    brokerVersion: INPUT_LIVE_V69_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV69Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV69Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV69RawResponse(
  directory: string,
  phase: InputLiveV69RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV69RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV69Request>(requestFile);
  if (
  ...
  const target = inputLiveV69ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV69Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v69-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V69_BROKER_VERSION,
...
export function acceptInputLiveV69Response<T = unknown>(
  directory: string,
  phase: InputLiveV69RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV69AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV69ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV69Request>(
    inputLiveV69RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV69Request(directory, request);
  if (phase === "capture") validateInputLiveV69TechnicalGates(directory);
  const rawFile = inputLiveV69ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v69-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V69_BROKER_VERSION ||
...
export function persistInputLiveV69TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV69RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV69RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V69_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v69-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V69_BROKER_VERSION,
...
export function validateInputLiveV69TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v69-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V69_TECHNICAL_GATES.some(
...
export function validateInputLiveV69Receipt(
  directory: string,
  phase: InputLiveV69RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV69Request(directory, request);
  const receipt = json<InputLiveV69Receipt>(
    inputLiveV69ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV69ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v69-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V69_BROKER_VERSION ||
...
export function readInputLiveV69AcceptedRawPayload(
  directory: string,
  phase: InputLiveV69RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV69Receipt(directory, phase, captureIndex);
...
export function createInputLiveV69FakeDynamicResponse(
  request: InputLiveV69Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V69_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V69_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV69PersistedRequests(
  directory: string,
): InputLiveV69Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV69Request>(file)] : [];
    });
}

export const inputLiveV69Sha256 = sha256;
