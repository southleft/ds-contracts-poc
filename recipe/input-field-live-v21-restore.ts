...
export const INPUT_LIVE_V21_MEASURE_VISIBLE_FILL_MARKER =
  "INPUT-TEXT-FILL-MEASURE-VISIBLE";
export const INPUT_LIVE_V21_RESTORE_CONTENT_ROLES = [
  "input-field/content/placeholder",
  "input-field/content/value",
] as const;
export const INPUT_LIVE_V21_RESTORE_PARENT_ROLES = [
  "input-field/surface",
  "input-field/content-row",
] as const;

export interface InputLiveV21RestoreWriter {
  pageId: string;
  runIdentity: string;
  setIds: readonly [string, string];
}

export interface InputLiveV21RestorePayload {
  pageId: string;
  setIds: string[];
  restoredCount: 256;
  fixedBefore: number;
  ...
  marker: typeof INPUT_LIVE_V21_MEASURE_VISIBLE_FILL_MARKER;
}

export function buildInputLiveV21RestoreProgram(
  writer: InputLiveV21RestoreWriter,
): string {
  if (writer.setIds.length !== 2)
    throw new TypeError("Input live v21 restore requires two owned sets");
  return String.raw`
await figma.loadAllPagesAsync();
const V21_NS=${JSON.stringify(INPUT_LIVE_V21_NAMESPACE)};
const V21_PAGE_ID=${JSON.stringify(writer.pageId)};
const V21_SET_IDS=${JSON.stringify([...writer.setIds])};
const V21_ROLES=new Set(${JSON.stringify([...INPUT_LIVE_V21_RESTORE_CONTENT_ROLES])});
const V21_PARENTS=new Set(${JSON.stringify([...INPUT_LIVE_V21_RESTORE_PARENT_ROLES])});
const V21_MARKER=${JSON.stringify(INPUT_LIVE_V21_MEASURE_VISIBLE_FILL_MARKER)};
const page=await figma.getNodeByIdAsync(V21_PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("INPUT-V21-RESTORE-PAGE");
if(page.getSharedPluginData(V21_NS,"pageOwner")!=="recipe/input-field/"+${JSON.stringify(writer.runIdentity)})throw new Error("INPUT-V21-RESTORE-OWNER");
...
export function validateInputLiveV21RestorePayload(
  payload: unknown,
  writer: InputLiveV21RestoreWriter,
): InputLiveV21RestorePayload {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("Input live v21 restore payload must be an object");
  ...
  if (
    ...
    value.marker !== INPUT_LIVE_V21_MEASURE_VISIBLE_FILL_MARKER
  )
    throw new TypeError(
      "Input live v21 restore did not re-assert content FILL",
    );
  return value as InputLiveV21RestorePayload;
}
