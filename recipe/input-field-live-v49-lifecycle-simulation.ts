...
export interface InputLiveV49LifecycleSimulationReport {
  artifactVersion: "input-live-v49-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV49AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV49Lifecycle(): Promise<InputLiveV49LifecycleSimulationReport> {
  await buildInputLiveV49Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V49_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV49AntecedentIndex;
  const indexSha256 = inputLiveV49Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v49-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V49_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V49_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV49History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV49History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV49AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V49_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V49_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV49History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV49History(authorized, "pending");
    ...
    await buildInputLiveV49Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V49_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV49AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV49Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV49Lifecycle(), null, 2)}\n`,
  );
