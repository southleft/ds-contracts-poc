...
export interface InputLiveV20LifecycleSimulationReport {
  artifactVersion: "input-live-v20-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV20AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV20Lifecycle(): Promise<InputLiveV20LifecycleSimulationReport> {
  await buildInputLiveV20Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V20_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV20AntecedentIndex;
  const indexSha256 = inputLiveV20Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v20-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V20_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V20_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV20History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV20History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV20AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V20_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V20_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV20History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV20History(authorized, "pending");
    ...
    await buildInputLiveV20Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V20_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV20AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV20Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV20Lifecycle(), null, 2)}\n`,
  );
