...
import { TABLE_LIVE_V9_AUTHORIZATION_PATH } from "./build-table-live-proof-v9.js";
import { verifyTableLiveV9Authorization } from "./table-live-v9-authorization.js";
import { runTableLiveV9Preflight } from "./table-live-v9-preflight.js";
import { TableLiveV9Orchestrator } from "./run-table-live-v9.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v9 requires exact authorization artifact ${TABLE_LIVE_V9_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v9-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV9Authorization(root);
    runTableLiveV9Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV9Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV9Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
