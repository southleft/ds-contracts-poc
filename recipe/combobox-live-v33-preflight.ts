...
import {
  validateComboboxLiveV33AntecedentIndex,
  verifyComboboxLiveV33Authorization,
  type ComboboxLiveV33AntecedentIndex,
  type ComboboxLiveV33AuthorizationProof,
} from "./combobox-live-v33-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V33_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v33/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V33_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v33-security-attestation.json";

export interface ComboboxLiveV33SecurityAttestation {
...
export interface ComboboxLiveV33PreflightReport {
  artifactVersion: "combobox-live-v33-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV33SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV33SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV33AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v33-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V33_TARGET) ||
...
export function validateComboboxLiveV33ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV33Authorization");
  const preflight = source.indexOf("runComboboxLiveV33Preflight");
  const initialize = source.indexOf("ComboboxLiveV33Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV33Preflight(
  root: string,
  proof: ComboboxLiveV33AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V33_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV33PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V33_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V33_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V33_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV33AntecedentIndex;
  failures.push(...validateComboboxLiveV33AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V33_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V33_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v33-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV33ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v33 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV33SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v33-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V33_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV33Preflight(
    process.cwd(),
    verifyComboboxLiveV33Authorization(),
    attestationPath,
