...
import {
  acceptCalendarLiveV45Response,
  createCalendarLiveV45Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV45Request,
  persistCalendarLiveV45RawResponse,
  persistCalendarLiveV45TechnicalGates,
  readCalendarLiveV45AcceptedRawPayload,
  type CalendarLiveV45Request,
  type CalendarLiveV45TransactionAuthorization,
} from "./calendar-live-v45-broker.js";
import {
  CALENDAR_LIVE_V45_CAPTURE_COUNT,
  assertCalendarLiveV45CaptureResponses,
  ...
  evaluateCalendarLiveV45Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV45Roots,
  validateCalendarLiveV45CaptureManifest,
  validateCalendarLiveV45CapturePayload,
  validateCalendarLiveV45CleanupPayload,
  validateCalendarLiveV45ExtractPayload,
  validateCalendarLiveV45ProbePayload,
  validateCalendarLiveV45WriterPayload,
  type CalendarLiveV45CaptureCell,
  type CalendarLiveV45CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V45_PLAN_PATH,
  type CalendarLiveV45ProofPlan,
} from "./build-calendar-live-proof-v45.js";
import {
...
export const CALENDAR_LIVE_V45_RUNNER_VERSION = "calendar-live-v45-runner-v1";

interface CalendarLiveV45JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV45OrchestratorState {
  artifactVersion: "calendar-live-v45-orchestrator-state-v1";
  ...
  journal: CalendarLiveV45JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV45ProofPlan;
  planSha256: string;
  captures: CalendarLiveV45CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV45SourceIdentity & {
...
export interface CalendarLiveV45NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV45OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v45 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V45_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V45_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV45ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV45CaptureCell[];
  validateCalendarLiveV45CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v45 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v45 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV45WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV45RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV45Request =>
  readJson<CalendarLiveV45Request>(file);

export class CalendarLiveV45Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV45Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV45OrchestratorState = {
      artifactVersion: "calendar-live-v45-orchestrator-state-v1",
    ...
    return new CalendarLiveV45Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV45OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV45Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV45NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV45NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v45 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV45NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v45 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV45RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV45Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV45CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV45AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV45WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV45Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV45ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV45Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV45Response(this.#directory, "writer", {
        validate: validateCalendarLiveV45WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV45Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV45RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV45Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV45ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV45Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV45RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v45-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV45Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV45ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV45Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV45ProbePayload(payload, ownership),
      });
      persistCalendarLiveV45TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v45 capture cell missing");
      const accepted = acceptCalendarLiveV45Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV45CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V45_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV45CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV45Objective(
    ...
    throw new Error(`Calendar live v45 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV45Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV45CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV45Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV45RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV45Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV45CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV45CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV45CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV45Authorization = (
  privateKey: KeyObject,
): CalendarLiveV45TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV45SimulationKey = (): KeyObject =>
