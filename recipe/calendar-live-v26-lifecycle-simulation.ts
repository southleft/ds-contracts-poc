...
import { calendarLiveV1Sha256 } from "./calendar-live-v26-broker.js";

export interface CalendarLiveV26LifecycleSimulationReport {
  artifactVersion: "calendar-live-v26-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV26AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV26Lifecycle(): Promise<CalendarLiveV26LifecycleSimulationReport> {
  await buildCalendarLiveV26Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V26_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV26AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v26-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V26_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V26_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV26History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV26History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV26AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V26_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V26_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV26History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV26History(authorized, "pending");
    ...
    await buildCalendarLiveV26Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V26_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV26AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV26Lifecycle(), null, 2)}\n`,
  );
