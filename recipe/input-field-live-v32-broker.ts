...
import {
  assertInputLiveV32RootProofs,
  INPUT_LIVE_V32_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V32_RESPONSE_CONTRACTS,
  type InputLiveV32RootProof,
} from "./input-field-live-v32-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V32_BROKER_VERSION = "input-live-v32-broker-v2";
export const INPUT_LIVE_V32_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V32_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V32_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V32_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V32_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V32_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV32RemotePhase =
  (typeof INPUT_LIVE_V32_REMOTE_PHASES)[number];
export type InputLiveV32AuthorizationMode = "live" | "simulated";

export interface InputLiveV32CommitPins {
...
export interface InputLiveV32TransactionAuthorization extends InputLiveV32CommitPins {
  mode: InputLiveV32AuthorizationMode;
...
export interface InputLiveV32Manifest {
  artifactVersion: "input-live-v32-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V32_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V32_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V32_TARGET;
  authorization: InputLiveV32TransactionAuthorization;
...
export interface InputLiveV32ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV32RequestBody {
  artifactVersion: "input-live-v32-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V32_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV32RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV32CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V32_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV32ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV32Request extends InputLiveV32RequestBody {
...
export interface InputLiveV32Receipt {
  artifactVersion: "input-live-v32-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V32_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV32RemotePhase;
...
export interface InputLiveV32AcceptedResponse<T = unknown> {
  request: InputLiveV32Request;
  receipt: InputLiveV32Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV32IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV32ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V32_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V32_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V32_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V32_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V32_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V32_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V32_TARGET.editorType;
};
...
export const inputLiveV32RequestSequence = (
  phase: InputLiveV32RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV32RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV32RequestPath = (
  directory: string,
  phase: InputLiveV32RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV32ResponsePath = (
  directory: string,
  phase: InputLiveV32RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV32ReceiptPath = (
  directory: string,
  phase: InputLiveV32RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV32Manifest => {
  const manifest = json<InputLiveV32Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v32-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V32_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V32_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV32RemotePhase,
  captureIndex?: number,
): InputLiveV32Request | undefined => {
  const file = inputLiveV32RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV32Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV32RemotePhase,
  captureIndex?: number,
): InputLiveV32Receipt | undefined => {
  const file = inputLiveV32ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV32Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV32Request | undefined;
  receipt: InputLiveV32Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV32RemotePhase,
): InputLiveV32ResponseContract => ({
  ...INPUT_LIVE_V32_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V32_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V32_DYNAMIC_TOOL,
      target: INPUT_LIVE_V32_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v32-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V32_BROKER_VERSION,
  target: INPUT_LIVE_V32_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V32_PROGRAM_START*/${program}/*INPUT_LIVE_V32_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V32_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V32_PROGRAM_END*/";
...
export function createInputLiveV32Transaction(
  directory: string,
  ...
  const manifest: InputLiveV32Manifest = {
    artifactVersion: "input-live-v32-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V32_BROKER_VERSION,
...
export function validateInputLiveV32Request(
  directory: string,
  request: InputLiveV32Request,
): void {
  ...
  const expectedSequence = inputLiveV32RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v32-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V32_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V32_TARGET.fileKey ||
...
export function issueInputLiveV32Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV32RemotePhase,
  program: string,
  options: InputLiveV32IssueOptions = {},
): InputLiveV32Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV32RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV32Request>(targetPath);
    validateInputLiveV32Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV32ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV32TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV32RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V32_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V32_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V32_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV32RequestBody = {
    artifactVersion: "input-live-v32-broker-request-v2",
    brokerVersion: INPUT_LIVE_V32_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV32Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV32Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV32RawResponse(
  directory: string,
  phase: InputLiveV32RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV32RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV32Request>(requestFile);
  if (
  ...
  const target = inputLiveV32ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV32Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v32-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V32_BROKER_VERSION,
...
export function acceptInputLiveV32Response<T = unknown>(
  directory: string,
  phase: InputLiveV32RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV32AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV32ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV32Request>(
    inputLiveV32RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV32Request(directory, request);
  if (phase === "capture") validateInputLiveV32TechnicalGates(directory);
  const rawFile = inputLiveV32ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v32-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V32_BROKER_VERSION ||
...
export function persistInputLiveV32TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV32RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV32RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V32_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v32-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V32_BROKER_VERSION,
...
export function validateInputLiveV32TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v32-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V32_TECHNICAL_GATES.some(
...
export function validateInputLiveV32Receipt(
  directory: string,
  phase: InputLiveV32RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV32Request(directory, request);
  const receipt = json<InputLiveV32Receipt>(
    inputLiveV32ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV32ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v32-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V32_BROKER_VERSION ||
...
export function readInputLiveV32AcceptedRawPayload(
  directory: string,
  phase: InputLiveV32RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV32Receipt(directory, phase, captureIndex);
...
export function createInputLiveV32FakeDynamicResponse(
  request: InputLiveV32Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V32_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V32_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV32PersistedRequests(
  directory: string,
): InputLiveV32Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV32Request>(file)] : [];
    });
}

export const inputLiveV32Sha256 = sha256;
