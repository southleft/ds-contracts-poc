...
import { INPUT_LIVE_V43_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v43.js";
import { verifyInputLiveV43Authorization } from "./input-field-live-v43-authorization.js";
import { runInputLiveV43Preflight } from "./input-field-live-v43-preflight.js";
import { InputLiveV43Orchestrator } from "./run-input-field-live-v43.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V43_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v43-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV43Authorization(root);
    runInputLiveV43Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV43Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
