...
export type CalendarLiveV3HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV3AuthorizationArtifact {
  artifactVersion: "calendar-live-v3-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V3_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV3AntecedentIndex {
...
export interface CalendarLiveV3HistoryState {
  index?: CalendarLiveV3AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV3AuthorizationArtifact;
...
export interface CalendarLiveV3AuthorizationProof
  extends CalendarLiveV3TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V3_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V3_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V3_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V3_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v3-authorization") ||
  artifactPath.includes("calendar-live-v3-preflight") ||
  artifactPath.includes("calendar-live-v3-authorized") ||
  artifactPath.includes("create-calendar-live-v3-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v3-status") ||
...
export function validateCalendarLiveV3AntecedentIndex(
  index: CalendarLiveV3AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v3-antecedent-index-v1" ||
...
export function validateCalendarLiveV3UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV3History\(\s*["']/.test(source) ||
    /readCalendarLiveV3HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV3AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV3AuthorizationArtifact {
  ...
  ) as CalendarLiveV3AntecedentIndex;
  const indexFailures = validateCalendarLiveV3AntecedentIndex(index);
...
export function validateCalendarLiveV3AuthorizationArtifact(
  artifact: CalendarLiveV3AuthorizationArtifact | undefined,
  index: CalendarLiveV3AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v3-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v3" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V3_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V3_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V3_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V3_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V3_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V3_SOURCE_ROOTS ||
...
export function validateCalendarLiveV3History(
  state: CalendarLiveV3HistoryState,
  expected: CalendarLiveV3HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV3AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV3AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV3HistoryState(
  root = process.cwd(),
): CalendarLiveV3HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V3_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV3AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V3_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V3_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V3_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV3History(
  expected: CalendarLiveV3HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV3HistoryState {
  const state = readCalendarLiveV3HistoryState(root);
  const failures = validateCalendarLiveV3History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v3 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV3Authorization(
  root = process.cwd(),
): CalendarLiveV3AuthorizationProof {
  const state = verifyCalendarLiveV3History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V3_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v3/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v3.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V3_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V3_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V3_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV3HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV3History(expected);
  process.stdout.write(
    `Calendar live v3 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
