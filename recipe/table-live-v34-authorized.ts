...
import { TABLE_LIVE_V34_AUTHORIZATION_PATH } from "./build-table-live-proof-v34.js";
import { verifyTableLiveV34Authorization } from "./table-live-v34-authorization.js";
import { runTableLiveV34Preflight } from "./table-live-v34-preflight.js";
import { TableLiveV34Orchestrator } from "./run-table-live-v34.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v34 requires exact authorization artifact ${TABLE_LIVE_V34_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v34-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV34Authorization(root);
    runTableLiveV34Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV34Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV34Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
