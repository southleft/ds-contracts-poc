...
import { calendarLiveV1Sha256 } from "./calendar-live-v11-broker.js";

export interface CalendarLiveV11LifecycleSimulationReport {
  artifactVersion: "calendar-live-v11-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV11AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV11Lifecycle(): Promise<CalendarLiveV11LifecycleSimulationReport> {
  await buildCalendarLiveV11Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V11_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV11AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v11-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V11_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V11_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV11History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV11History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV11AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V11_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V11_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV11History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV11History(authorized, "pending");
    ...
    await buildCalendarLiveV11Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V11_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV11AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV11Lifecycle(), null, 2)}\n`,
  );
