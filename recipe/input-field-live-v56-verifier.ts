...
export const INPUT_LIVE_V56_VERIFIER_VERSION = "input-live-v56-verifier-v1";

export function buildInputLiveV56RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV56RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV56RawNode[];
}

export interface InputLiveV56NormalizedScene {
...
export interface InputLiveV56SceneProof {
...
export function normalizeInputLiveV56Scene(
  root: InputLiveV56RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV56NormalizedScene {
  const canonicalBindings: InputLiveV56NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV56RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV56Scene<Instance>(
  normalized: InputLiveV56NormalizedScene,
...
export function compileInputLiveV56ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV56PreCaptureGates(payload: unknown): void {
