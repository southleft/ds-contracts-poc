/**
 * Table live v14 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V14_FIXED_VERIFIER_VERSION = "table-live-v14-fixed-point-v1";
export const TABLE_LIVE_V14_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV14FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV14FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV14FixedSourceProbe {
...
export interface TableLiveV14FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV14FixedFixedPoint;
}

export interface TableLiveV14FixedHardGateInput {
  thresholds: TableLiveV14FixedThresholds;
  sourceProbes: TableLiveV14FixedSourceProbe[];
  cells: TableLiveV14FixedCellValidation[];
  sceneProofs: TableLiveV14FixedSceneProof[];
...
export interface TableLiveV14FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V14_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV14FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV14FixedHardGates(
  input: TableLiveV14FixedHardGateInput,
): TableLiveV14FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV14FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  if (input.visualRows.length !== 0)
    failures.push("table live v14 must not invent a visual comparison");
  ...
  return {
    verifierVersion: TABLE_LIVE_V14_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV14FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV14FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
