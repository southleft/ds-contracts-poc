...
import {
  buildInputLiveV8RawPropertyRuntime,
  normalizeInputLiveV8Scene,
  type InputLiveV8RawNode,
} from "./input-field-live-v8-verifier.js";
...
export const INPUT_LIVE_V8_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V8_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V8_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V8_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V8_VARIANT_COUNT = 256;
export const INPUT_LIVE_V8_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V8_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV8SourceId = (typeof INPUT_LIVE_V8_SOURCE_IDS)[number];

export interface InputLiveV8SourceIdentity {
  source: InputLiveV8SourceId;
...
export interface InputLiveV8WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV8ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV8SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV8RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV8RootProof {
  source: InputLiveV8SourceId;
  adapterIdentity: string;
...
export interface InputLiveV8ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV8SourceId;
    adapterIdentity: string;
...
export interface InputLiveV8CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV8SourceId;
  adapterIdentity: string;
...
export interface InputLiveV8CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV8SourceId;
  strata: InputLiveV8CaptureCell["strata"];
...
export interface InputLiveV8CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Input live v8 ${label} must be an object`);
...
const exactUniqueStrings = (
  ...
  if (
    ...
    throw new TypeError(`Input live v8 ${label} cardinality/identity mismatch`);
  return value;
};
const finiteNonnegative = (value: unknown): value is number =>
  typeof value === "number" && Number.isFinite(value) && value >= 0;

export function validateInputLiveV8WriterPayload(
  value: unknown,
): InputLiveV8WriterOwnership {
  ...
  if (
    ...
    throw new TypeError("Input live v8 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Input live v8 writer created-node denominator invalid",
    );
  return {
...
const identityPlan = (source: InputLiveV8SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV8ExtractProgram(
  writer: InputLiveV8WriterOwnership,
  sources: readonly InputLiveV8SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Input live v8 extract requires two source identities");
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V8_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V8_NAMESPACE)}
${buildInputLiveV8RawPropertyRuntime()}
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("INPUT-V8-EXTRACT-PAGE");
const get=(node,key)=>node.getSharedPluginData(V7_NS,key);
if(get(page,"pageOwner")!=="recipe/input-field/"+${JSON.stringify(writer.runIdentity)})throw new Error("INPUT-V8-EXTRACT-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>V7_SET_IDS.has(node.id));
if(sets.length!==2)throw new Error("INPUT-V8-EXTRACT-ROOTS:"+sets.length);
...
for(const set of sets){
  ...
  if(!expected)throw new Error("INPUT-V8-EXTRACT-ADAPTER:"+adapterIdentity);
...
export function validateInputLiveV8ExtractPayload(
  value: unknown,
  writer: InputLiveV8WriterOwnership,
): InputLiveV8ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Input live v8 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V8_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Input live v8 extract schema/two-root mismatch");
  return payload as InputLiveV8ExtractPayload;
}

export function proveInputLiveV8Roots<Instance>(
  extract: InputLiveV8ExtractPayload,
  sources: readonly (InputLiveV8SourceIdentity & {
...
): InputLiveV8RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Input live v8 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Input live v8 omitted ${source.source} root`);
    const normalized = normalizeInputLiveV8Scene(
      root.scene,
...
export function assertInputLiveV8RootProofs(
  proofs: readonly InputLiveV8RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Input live v8 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Input live v8 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildInputLiveV8ProbeProgram(
  writer: InputLiveV8WriterOwnership,
  sources: readonly InputLiveV8SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V8_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("INPUT-V8-PROBE-PAGE");
...
if(sets.length!==2)throw new Error("INPUT-V8-PROBE-ROOTS:"+sets.length);
...
for(const set of sets){
 ...
 if(!source||components.length!==128)throw new Error("INPUT-V8-PROBE-VARIANTS:"+adapterIdentity+":"+components.length);
 ...
 for(const component of components){const target=axes(component),updates={};for(const name of axisNames){const key=propertyKey(instance,name);if(!key)throw new Error("INPUT-V8-PROBE-AXIS:"+name);updates[key]=target[name];}instance.setProperties(updates);const main=await instance.getMainComponentAsync();if(main)visited.add(main.id);}
...
export function validateInputLiveV8ProbePayload(
  value: unknown,
  writer: InputLiveV8WriterOwnership,
): InputLiveV8ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV8ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V8_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V8_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Input live v8 probe/usability/restoration failed");
  return payload;
}

export function validateInputLiveV8CaptureManifest(
  cells: readonly InputLiveV8CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V8_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V8_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V8_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V8_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V8_SOURCE_IDS.some(
      (source) => cells.filter((cell) => cell.source === source).length !== 64,
    )
  )
    throw new TypeError(
      "Input live v8 capture manifest must be exact 128 cells",
    );
}

export const inputLiveV8CaptureManifestSha256 = (
  cells: readonly InputLiveV8CaptureCell[],
): string => {
  validateInputLiveV8CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV8CaptureProgram(
  writer: InputLiveV8WriterOwnership,
  cell: InputLiveV8CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V8_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V8_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
    ...
    throw new TypeError("Input live v8 capture cell is malformed");
  const expectedSet = writer.sources.find(
    (source) => source.adapterIdentity === cell.adapterIdentity,
  )?.setId;
  if (!expectedSet)
    throw new TypeError(
      `Input live v8 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
if(!page||page.type!=="PAGE"||!set||set.type!=="COMPONENT_SET")throw new Error("INPUT-V8-CAPTURE-TARGET");
const axes=node=>Object.fromEntries(node.name.split(",").map(part=>part.trim()).filter(part=>part.includes("=")).map(part=>{const at=part.indexOf("=");return[part.slice(0,at),part.slice(at+1)];}));
...
if(!component)throw new Error("INPUT-V8-CAPTURE-CELL:"+cell.cellKey);
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V8_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV8CapturePayload(
  value: unknown,
  cell: InputLiveV8CaptureCell,
  rawResponseBytes: number,
): InputLiveV8CapturePayload {
  const payload = record(value, "capture payload") as InputLiveV8CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V8_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V8_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Input live v8 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertInputLiveV8CaptureResponses(
  manifest: readonly InputLiveV8CaptureCell[],
  responses: readonly InputLiveV8CapturePayload[],
): void {
  validateInputLiveV8CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Input live v8 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface InputLiveV8ObjectiveReport {
  artifactVersion: "input-live-v8-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV8Objective(
  manifest: readonly InputLiveV8CaptureCell[],
  responses: readonly InputLiveV8CapturePayload[],
  readReference: (cell: InputLiveV8CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV8CaptureCell) => Buffer,
): InputLiveV8ObjectiveReport {
  assertInputLiveV8CaptureResponses(manifest, responses);
  const rows = manifest.map((cell, index) => {
    ...
    if (
      ...
      throw new TypeError(
        `Input live v8 objective byte drift: ${cell.cellKey}`,
      );
    ...
    if (
      ...
      throw new TypeError(
        `Input live v8 objective measurement refused: ${cell.cellKey}`,
      );
    return {
      index: cell.index,
...
export function buildInputLiveV8CleanupProgram(
  writer: InputLiveV8WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V8_NAMESPACE,
  ...
  const guardedResult = `if(JSON.stringify(requestedNodeIds)!==JSON.stringify([expectedPageId])||JSON.stringify([...requestedCollectionIds].sort())!==JSON.stringify([...expectedCollectionIds].sort()))throw new Error("INPUT-V8-CLEANUP-EXACT-OWNERSHIP");${resultStatement}`;
  const program = baseProgram.replace(resultStatement, guardedResult);
  if (program === baseProgram)
    throw new Error("Input live v8 cleanup runtime result hook absent");
  return String.raw`
...
export function validateInputLiveV8CleanupPayload(
  value: unknown,
  writer: InputLiveV8WriterOwnership,
): InputLiveV8CleanupPayload {
  const payload = record(value, "cleanup payload") as InputLiveV8CleanupPayload;
  ...
  if (
    ...
    throw new TypeError("Input live v8 cleanup is incomplete or overbroad");
  return payload;
}

export const INPUT_LIVE_V8_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV8WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
