...
import { TABLE_LIVE_V5_AUTHORIZATION_PATH } from "./build-table-live-proof-v5.js";
import { verifyTableLiveV5Authorization } from "./table-live-v5-authorization.js";
import { runTableLiveV5Preflight } from "./table-live-v5-preflight.js";
import { TableLiveV5Orchestrator } from "./run-table-live-v5.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v5 requires exact authorization artifact ${TABLE_LIVE_V5_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v5-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV5Authorization(root);
    runTableLiveV5Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV5Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV5Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
