...
import { TABLE_LIVE_V19_INDEX_PATH } from "./build-table-live-proof-v19.js";
import {
...
import {
  buildTableLiveV19AuthorizationArtifact,
  validateTableLiveV19AntecedentIndex,
  validateTableLiveV19AuthorizationArtifact,
  validateTableLiveV19History,
  validateTableLiveV19UnitTestSource,
  type TableLiveV19AntecedentIndex,
  type TableLiveV19AuthorizationArtifact,
  type TableLiveV19AuthorizationProof,
  type TableLiveV19HistoryState,
} from "./table-live-v19-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV19ControlFlowSource,
  validateTableLiveV19SecurityAttestation,
  type TableLiveV19SecurityAttestation,
} from "./table-live-v19-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V19_INDEX_PATH);
const index = (): TableLiveV19AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV19AntecedentIndex;

const artifact = (): TableLiveV19AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV19AuthorizationArtifact({
...
const pendingState = (): TableLiveV19HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV19HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV19AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v19/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV19AuthorizationArtifact,
): TableLiveV19SecurityAttestation => {
  const body: Omit<TableLiveV19SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "table-live-v19-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V19_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV19History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV19History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV19History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV19History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV19History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV19UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v19-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV19UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV19AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV19AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV19AuthorizationArtifact(
        planted as TableLiveV19AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV19SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV19SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV19SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v19-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV19ControlFlowSource(source), []);
});
