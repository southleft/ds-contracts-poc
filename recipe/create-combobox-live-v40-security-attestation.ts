...
import { COMBOBOX_LIVE_V40_TARGET } from "./combobox-live-v40-broker.js";
import {
  COMBOBOX_LIVE_V40_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV40Authorization();
...
const attestation: ComboboxLiveV40SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV40SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v40 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V40_SECURITY_ATTESTATION_DEFAULT_PATH,
);
