...
import { calendarLiveV1Sha256 } from "./calendar-live-v4-broker.js";

export interface CalendarLiveV4LifecycleSimulationReport {
  artifactVersion: "calendar-live-v4-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV4AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV4Lifecycle(): Promise<CalendarLiveV4LifecycleSimulationReport> {
  await buildCalendarLiveV4Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V4_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV4AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v4-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V4_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V4_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV4History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV4History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV4AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V4_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V4_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV4History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV4History(authorized, "pending");
    ...
    await buildCalendarLiveV4Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V4_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV4AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV4Lifecycle(), null, 2)}\n`,
  );
