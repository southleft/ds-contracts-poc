...
import {
  validateCalendarLiveV49AntecedentIndex,
  verifyCalendarLiveV49Authorization,
  type CalendarLiveV49AntecedentIndex,
  type CalendarLiveV49AuthorizationProof,
} from "./calendar-live-v49-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V49_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v49/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V49_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v49-security-attestation.json";

export interface CalendarLiveV49SecurityAttestation {
  artifactVersion: "calendar-live-v49-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV49PreflightReport {
  artifactVersion: "calendar-live-v49-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV49SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV49SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV49AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v49-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V49_TARGET) ||
...
export function validateCalendarLiveV49ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV49Authorization");
  const preflight = source.indexOf("runCalendarLiveV49Preflight");
  const initialize = source.indexOf("CalendarLiveV49Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV49Preflight(
  root: string,
  proof: CalendarLiveV49AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V49_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV49PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V49_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V49_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV49AntecedentIndex;
  failures.push(...validateCalendarLiveV49AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V49_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V49_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV49SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v49-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV49ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v49 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV49SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v49-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V49_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV49Preflight(
    process.cwd(),
    verifyCalendarLiveV49Authorization(),
    attestationPath,
