...
import {
  ...
  INPUT_LIVE_V64_INDEX_PATH,
} from "./build-input-field-live-proof-v64.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV64HistoryExpectation = "pending" | "authorized";

export interface InputLiveV64AuthorizationArtifact {
  artifactVersion: "input-live-v64-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V64_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV64AntecedentIndex {
...
export interface InputLiveV64HistoryState {
  index?: InputLiveV64AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV64AuthorizationArtifact;
...
export interface InputLiveV64AuthorizationProof extends InputLiveV64TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V64_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V64_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V64_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V64_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v64-authorization") ||
  artifactPath.includes("input-field-live-v64-preflight") ||
  artifactPath.includes("input-field-live-v64-authorized") ||
  artifactPath.includes("input-field-live-pivot-v64-status") ||
...
export function validateInputLiveV64AntecedentIndex(
  index: InputLiveV64AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v64-antecedent-index-v1" ||
...
export function validateInputLiveV64UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV64History\(\s*["']/.test(source) ||
    /readInputLiveV64HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV64AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV64AuthorizationArtifact {
  ...
  ) as InputLiveV64AntecedentIndex;
  const indexFailures = validateInputLiveV64AntecedentIndex(index);
...
export function validateInputLiveV64AuthorizationArtifact(
  artifact: InputLiveV64AuthorizationArtifact | undefined,
  index: InputLiveV64AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v64-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v64" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V64_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V64_TARGET) ||
    !equalJson(
...
export function validateInputLiveV64History(
  state: InputLiveV64HistoryState,
  expected: InputLiveV64HistoryExpectation,
): string[] {
  const failures = validateInputLiveV64AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV64AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV64HistoryState(
  root = process.cwd(),
): InputLiveV64HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V64_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV64AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V64_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V64_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V64_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV64History(
  expected: InputLiveV64HistoryExpectation,
  root = process.cwd(),
): InputLiveV64HistoryState {
  const state = readInputLiveV64HistoryState(root);
  const failures = validateInputLiveV64History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV64Authorization(
  root = process.cwd(),
): InputLiveV64AuthorizationProof {
  const state = verifyInputLiveV64History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V64_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v64/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v64.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V64_TARGET,
    expectedDynamicTool: INPUT_LIVE_V64_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V64_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV64HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV64History(expected);
