...
import { CALENDAR_LIVE_V44_INDEX_PATH } from "./build-calendar-live-proof-v44.js";
import {
...
import {
  ...
  validateCalendarLiveV44History,
  validateCalendarLiveV44UnitTestSource,
  type CalendarLiveV44AntecedentIndex,
  type CalendarLiveV44AuthorizationArtifact,
  type CalendarLiveV44AuthorizationProof,
  type CalendarLiveV44HistoryState,
} from "./calendar-live-v44-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV44ControlFlowSource,
  validateCalendarLiveV44SecurityAttestation,
  type CalendarLiveV44SecurityAttestation,
} from "./calendar-live-v44-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V44_INDEX_PATH);
const index = (): CalendarLiveV44AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV44AntecedentIndex;

const artifact = (): CalendarLiveV44AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV44AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV44HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV44HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV44AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v44/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV44AuthorizationArtifact,
): CalendarLiveV44SecurityAttestation => {
  const body: Omit<CalendarLiveV44SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V44_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV44History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV44History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV44History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV44History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV44History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV44UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v44-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV44UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV44AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV44AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV44AuthorizationArtifact(
        planted as CalendarLiveV44AuthorizationArtifact,
        index(),
...
test("one-root denominator, capture gate, cleanup, and prior-lineage reuse are rejected", () => {
  ...
  for (const mutate of [
    ...
    (value: Record<string, any>) => {
      value.execution.comboboxLiveV44AuthorizationReusable = true;
    },
    (value: Record<string, any>) => {
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV44SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV44SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV44SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v44-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV44ControlFlowSource(source), []);
});
