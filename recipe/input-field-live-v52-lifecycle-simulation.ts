...
export interface InputLiveV52LifecycleSimulationReport {
  artifactVersion: "input-live-v52-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV52AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV52Lifecycle(): Promise<InputLiveV52LifecycleSimulationReport> {
  await buildInputLiveV52Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V52_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV52AntecedentIndex;
  const indexSha256 = inputLiveV52Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v52-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V52_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V52_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV52History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV52History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV52AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V52_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V52_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV52History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV52History(authorized, "pending");
    ...
    await buildInputLiveV52Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V52_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV52AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV52Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV52Lifecycle(), null, 2)}\n`,
  );
