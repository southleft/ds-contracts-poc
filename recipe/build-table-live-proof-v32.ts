...
import {
  TABLE_LIVE_V32_CAPTURE_COUNT,
  TABLE_LIVE_V32_HOST_PHASES,
  TABLE_LIVE_V32_REMOTE_REQUESTS,
  TABLE_LIVE_V32_SOURCE_ROOTS,
  TABLE_LIVE_V32_VARIANT_COUNT,
  buildTableLiveV32CaptureProgram,
  buildTableLiveV32CleanupProgram,
  buildTableLiveV32ExtractProgram,
  buildTableLiveV32ProbeProgram,
  buildTableLiveV32RestoreProgram,
  tableLiveV2CaptureManifestSha256,
  type TableLiveV32CaptureCell,
  type TableLiveV32SourceIdentity,
  type TableLiveV32WriterOwnership,
} from "./table-live-v32-contract.js";
import {
  TABLE_LIVE_V32_DYNAMIC_TOOL,
  TABLE_LIVE_V32_TARGET,
  tableLiveV2RequestSequence,
} from "./table-live-v32-broker.js";
import {
...
export const TABLE_LIVE_V32_EVIDENCE_ROOT =
  "recipe/evidence/table-live-pivot-v32";
export const TABLE_LIVE_V32_PROTOCOL_PATH = `${TABLE_LIVE_V32_EVIDENCE_ROOT}/protocol.json`;
export const TABLE_LIVE_V32_PLAN_PATH = `${TABLE_LIVE_V32_EVIDENCE_ROOT}/proof-plan.json`;
export const TABLE_LIVE_V32_CAPTURE_MANIFEST_PATH = `${TABLE_LIVE_V32_EVIDENCE_ROOT}/capture-manifest.json`;
export const TABLE_LIVE_V32_REQUEST_MANIFEST_PATH = `${TABLE_LIVE_V32_EVIDENCE_ROOT}/request-manifest.json`;
export const TABLE_LIVE_V32_INDEX_PATH = `${TABLE_LIVE_V32_EVIDENCE_ROOT}/antecedent-index.json`;
export const TABLE_LIVE_V32_AUTHORIZATION_TEMPLATE_PATH = `${TABLE_LIVE_V32_EVIDENCE_ROOT}/authorization-template.json`;
export const TABLE_LIVE_V32_AUTHORIZATION_PATH = `${TABLE_LIVE_V32_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface TableLiveV32ProofPlan {
  artifactVersion: "table-live-v32-proof-plan-v1";
  status: "draft antecedent; pending separate authorization; live execution forbidden";
  target: typeof TABLE_LIVE_V32_TARGET;
...
const captureCells = (
...
): TableLiveV32CaptureCell[] => {
  const cells: TableLiveV32CaptureCell[] = [];
  ...
  if (cells.length !== TABLE_LIVE_V32_CAPTURE_COUNT)
    throw new Error(
      `Table live v32 capture cells ${cells.length}, expected ${TABLE_LIVE_V32_CAPTURE_COUNT}`,
    );
...
const protocol = (expectedFacts: number) => ({
  artifactVersion: "table-live-v32-external-operator-protocol-v1",
  protocolId: "table-live-v32",
  status:
  ...
  target: TABLE_LIVE_V32_TARGET,
  expectedDynamicTool: TABLE_LIVE_V32_DYNAMIC_TOOL,
  denominator: {
    sourceRoots: TABLE_LIVE_V32_SOURCE_ROOTS,
    sets: 6,
    variants: TABLE_LIVE_V32_VARIANT_COUNT,
    captureCells: TABLE_LIVE_V32_CAPTURE_COUNT,
    remoteRequests: TABLE_LIVE_V32_REMOTE_REQUESTS,
    hostPhases: TABLE_LIVE_V32_HOST_PHASES,
...
export async function buildTableLiveV32Proof(
  check = process.argv.includes("--check"),
): Promise<TableLiveV32ProofPlan> {
  ...
  if (
    ...
    throw new Error(
      "Table live v32 writer reused Input, Combobox, or Table v1–v24 identity",
    );
  const captures = captureCells(sources);
  const writerProgram = Buffer.from(writer.code);
  const writerOwnershipBlueprint: TableLiveV32WriterOwnership = {
  ...
  const identities: TableLiveV32SourceIdentity[] = sources.map((source) => ({
    source: source.source,
    adapterIdentity: source.adapterIdentity,
    recipeHash: source.recipeHash,
    envelopeHash: source.envelope.integrity.canonicalHash,
  ...
  const restoreBlueprint = Buffer.from(
    buildTableLiveV32RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildTableLiveV32ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildTableLiveV32ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildTableLiveV32CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );
  const cleanupBlueprint = Buffer.from(
    buildTableLiveV32CleanupProgram(writerOwnershipBlueprint),
  );
  ...
  const sourceMetadata = sources.map((source) => {
    ...
    const tablePath = `${TABLE_LIVE_V32_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-table.json.gz`;
    const rowPath = `${TABLE_LIVE_V32_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-row.json.gz`;
    const cellPath = `${TABLE_LIVE_V32_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-cell.json.gz`;
  ...
  antecedentOutputs.set(
    TABLE_LIVE_V32_PROTOCOL_PATH,
    jsonBytes(protocol(expectedFacts)),
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V32_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V32_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V32_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V32_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V32_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V32_EVIDENCE_ROOT}/programs/cleanup-blueprint.js`,
    cleanupBlueprint,
  );

  const captureManifest = {
    artifactVersion: "table-live-v32-capture-manifest-v1",
  ...
  antecedentOutputs.set(
    TABLE_LIVE_V32_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: TableLiveV32ProofPlan = {
    artifactVersion: "table-live-v32-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: TABLE_LIVE_V32_TARGET,
    namespace: TABLE_FIGMA_NAMESPACE,
    writer: {
      programPath: `${TABLE_LIVE_V32_EVIDENCE_ROOT}/programs/writer.txt`,
    ...
    captureManifest: {
      path: TABLE_LIVE_V32_CAPTURE_MANIFEST_PATH,
      cells: 20,
      requests: 20,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(TABLE_LIVE_V32_PLAN_PATH, jsonBytes(proofPlan));

  const requests = {
    artifactVersion: "table-live-v32-request-manifest-v1",
    expectedDynamicTool: TABLE_LIVE_V32_DYNAMIC_TOOL,
    target: TABLE_LIVE_V32_TARGET,
  ...
  if (requests.requestCount !== TABLE_LIVE_V32_REMOTE_REQUESTS)
    throw new Error("Table live v32 remote request denominator drifted");
  antecedentOutputs.set(
    TABLE_LIVE_V32_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const lifecycleExcludedPaths = [
    TABLE_LIVE_V32_AUTHORIZATION_TEMPLATE_PATH,
    TABLE_LIVE_V32_AUTHORIZATION_PATH,
    "recipe/table-live-v32-authorization.ts",
    "recipe/table-live-v32-authorization.test.ts",
    "recipe/table-live-v32-preflight.ts",
    "recipe/table-live-v32-authorized.ts",
    "recipe/write-table-live-v32-authorization.ts",
    "recipe/create-table-live-v32-security-attestation.ts",
    "recipe/evidence/table-live-pivot-v32/operator-security-attestation-template.json",
    "recipe/evidence/table-live-pivot-v32-status.json",
    "recipe/evidence/status-index.json",
  ];
  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/table-live-v32-broker.ts",
    "recipe/table-live-v32-contract.ts",
    "recipe/table-live-v32-restore.ts",
    "recipe/table-live-v32-cleanup.ts",
    "recipe/table-live-v32-verifier.ts",
    "recipe/table-live-v32-fixed-point.ts",
    "recipe/run-table-live-v32.ts",
    "recipe/build-table-live-proof-v32.ts",
    "recipe/table-live-v32-lifecycle-simulation.ts",
    "recipe/scene-readback-table-v1.ts",
    "recipe/scene-readback-runtime-table-v32.ts",
    "recipe/table-figma-writer.ts",
    "recipe/figma-property-normalizer-v8.ts",
    "recipe/figma-runtime-portability.ts",
    "recipe/normalize.ts",
  ].filter((artifactPath) => artifactPath !== TABLE_LIVE_V32_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "table-live-v32-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
  ...
  antecedentOutputs.set(TABLE_LIVE_V32_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "table-live-v32-authorization-template-v1",
    authorizationId: "table-live-v32",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: TABLE_LIVE_V32_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
    ...
    denominator: {
      remoteRequests: TABLE_LIVE_V32_REMOTE_REQUESTS,
      hostPhases: TABLE_LIVE_V32_HOST_PHASES,
      captures: TABLE_LIVE_V32_CAPTURE_COUNT,
      sourceRoots: TABLE_LIVE_V32_SOURCE_ROOTS,
      expectedFacts,
    },
    execution: {
  ...
  const attestationTemplate = jsonBytes({
    artifactVersion: "table-live-v32-operator-security-attestation-template-v1",
    status: "template only; complete privately after PREPARE+AUTHORIZE",
    tokenValuesForbidden: true,
    privateDirectory: "private/table-live-v32-security-attestation.json",
  });
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Table live v32 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(TABLE_LIVE_V32_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(TABLE_LIVE_V32_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
    writeFileSync(
      `${TABLE_LIVE_V32_EVIDENCE_ROOT}/operator-security-attestation-template.json`,
      attestationTemplate,
    );
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(TABLE_LIVE_V32_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(TABLE_LIVE_V32_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Table live v32 authorization template drift");
  }
  if (
    writer.namespace !== TABLE_FIGMA_NAMESPACE ||
    writer.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    ...
    throw new Error("Table live v32 identity or cleanup invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildTableLiveV32Proof(), null, 2)}\n`,
  );
