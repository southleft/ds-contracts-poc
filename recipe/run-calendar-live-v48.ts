...
import {
  acceptCalendarLiveV48Response,
  createCalendarLiveV48Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV48Request,
  persistCalendarLiveV48RawResponse,
  persistCalendarLiveV48TechnicalGates,
  readCalendarLiveV48AcceptedRawPayload,
  type CalendarLiveV48Request,
  type CalendarLiveV48TransactionAuthorization,
} from "./calendar-live-v48-broker.js";
import {
  CALENDAR_LIVE_V48_CAPTURE_COUNT,
  assertCalendarLiveV48CaptureResponses,
  ...
  evaluateCalendarLiveV48Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV48Roots,
  validateCalendarLiveV48CaptureManifest,
  validateCalendarLiveV48CapturePayload,
  validateCalendarLiveV48CleanupPayload,
  validateCalendarLiveV48ExtractPayload,
  validateCalendarLiveV48ProbePayload,
  validateCalendarLiveV48WriterPayload,
  type CalendarLiveV48CaptureCell,
  type CalendarLiveV48CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V48_PLAN_PATH,
  type CalendarLiveV48ProofPlan,
} from "./build-calendar-live-proof-v48.js";
import {
...
export const CALENDAR_LIVE_V48_RUNNER_VERSION = "calendar-live-v48-runner-v1";

interface CalendarLiveV48JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV48OrchestratorState {
  artifactVersion: "calendar-live-v48-orchestrator-state-v1";
  ...
  journal: CalendarLiveV48JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV48ProofPlan;
  planSha256: string;
  captures: CalendarLiveV48CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV48SourceIdentity & {
...
export interface CalendarLiveV48NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV48OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v48 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V48_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V48_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV48ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV48CaptureCell[];
  validateCalendarLiveV48CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v48 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v48 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV48WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV48RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV48Request =>
  readJson<CalendarLiveV48Request>(file);

export class CalendarLiveV48Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV48Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV48OrchestratorState = {
      artifactVersion: "calendar-live-v48-orchestrator-state-v1",
    ...
    return new CalendarLiveV48Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV48OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV48Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV48NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV48NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v48 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV48NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v48 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV48RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV48Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV48CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV48AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV48WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV48Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV48ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV48Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV48Response(this.#directory, "writer", {
        validate: validateCalendarLiveV48WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV48Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV48RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV48Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV48ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV48Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV48RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v48-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV48Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV48ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV48Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV48ProbePayload(payload, ownership),
      });
      persistCalendarLiveV48TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v48 capture cell missing");
      const accepted = acceptCalendarLiveV48Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV48CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V48_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV48CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV48Objective(
    ...
    throw new Error(`Calendar live v48 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV48Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV48CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV48Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV48RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV48Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV48CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV48CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV48CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV48Authorization = (
  privateKey: KeyObject,
): CalendarLiveV48TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV48SimulationKey = (): KeyObject =>
