...
export interface InputLiveV54LifecycleSimulationReport {
  artifactVersion: "input-live-v54-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV54AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV54Lifecycle(): Promise<InputLiveV54LifecycleSimulationReport> {
  await buildInputLiveV54Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V54_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV54AntecedentIndex;
  const indexSha256 = inputLiveV54Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v54-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V54_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V54_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV54History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV54History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV54AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V54_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V54_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV54History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV54History(authorized, "pending");
    ...
    await buildInputLiveV54Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V54_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV54AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV54Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV54Lifecycle(), null, 2)}\n`,
  );
