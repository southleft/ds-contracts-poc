...
export interface InputLiveV34LifecycleSimulationReport {
  artifactVersion: "input-live-v34-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV34AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV34Lifecycle(): Promise<InputLiveV34LifecycleSimulationReport> {
  await buildInputLiveV34Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V34_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV34AntecedentIndex;
  const indexSha256 = inputLiveV34Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v34-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V34_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V34_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV34History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV34History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV34AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V34_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V34_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV34History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV34History(authorized, "pending");
    ...
    await buildInputLiveV34Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V34_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV34AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV34Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV34Lifecycle(), null, 2)}\n`,
  );
