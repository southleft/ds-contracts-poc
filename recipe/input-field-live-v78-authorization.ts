...
import {
  ...
  INPUT_LIVE_V78_INDEX_PATH,
} from "./build-input-field-live-proof-v78.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV78HistoryExpectation = "pending" | "authorized";

export interface InputLiveV78AuthorizationArtifact {
  artifactVersion: "input-live-v78-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V78_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV78AntecedentIndex {
...
export interface InputLiveV78HistoryState {
  index?: InputLiveV78AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV78AuthorizationArtifact;
...
export interface InputLiveV78AuthorizationProof extends InputLiveV78TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V78_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V78_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V78_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V78_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v78-authorization") ||
  artifactPath.includes("input-field-live-v78-preflight") ||
  artifactPath.includes("input-field-live-v78-authorized") ||
  artifactPath.includes("input-field-live-pivot-v78-status") ||
...
export function validateInputLiveV78AntecedentIndex(
  index: InputLiveV78AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v78-antecedent-index-v1" ||
...
export function validateInputLiveV78UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV78History\(\s*["']/.test(source) ||
    /readInputLiveV78HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV78AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV78AuthorizationArtifact {
  ...
  ) as InputLiveV78AntecedentIndex;
  const indexFailures = validateInputLiveV78AntecedentIndex(index);
...
export function validateInputLiveV78AuthorizationArtifact(
  artifact: InputLiveV78AuthorizationArtifact | undefined,
  index: InputLiveV78AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v78-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v78" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V78_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V78_TARGET) ||
    !equalJson(
...
export function validateInputLiveV78History(
  state: InputLiveV78HistoryState,
  expected: InputLiveV78HistoryExpectation,
): string[] {
  const failures = validateInputLiveV78AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV78AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV78HistoryState(
  root = process.cwd(),
): InputLiveV78HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V78_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV78AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V78_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V78_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V78_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV78History(
  expected: InputLiveV78HistoryExpectation,
  root = process.cwd(),
): InputLiveV78HistoryState {
  const state = readInputLiveV78HistoryState(root);
  const failures = validateInputLiveV78History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV78Authorization(
  root = process.cwd(),
): InputLiveV78AuthorizationProof {
  const state = verifyInputLiveV78History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V78_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v78/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v78.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V78_TARGET,
    expectedDynamicTool: INPUT_LIVE_V78_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V78_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV78HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV78History(expected);
