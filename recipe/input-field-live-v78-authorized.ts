...
import { INPUT_LIVE_V78_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v78.js";
import { verifyInputLiveV78Authorization } from "./input-field-live-v78-authorization.js";
import { runInputLiveV78Preflight } from "./input-field-live-v78-preflight.js";
import { InputLiveV78Orchestrator } from "./run-input-field-live-v78.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V78_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v78-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV78Authorization(root);
    runInputLiveV78Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV78Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
