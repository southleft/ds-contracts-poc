...
import {
  verifyComboboxLiveV2FixedSceneFixedPoint,
  type ComboboxLiveV2FixedFixedPoint,
} from "./combobox-live-v2-fixed-point.js";

export const COMBOBOX_LIVE_V2_VERIFIER_VERSION = "combobox-live-v2-verifier-v1";

export function buildComboboxLiveV2RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV2RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV2RawNode[];
}

export interface ComboboxLiveV2NormalizedScene {
...
export interface ComboboxLiveV2SceneProof {
...
export function normalizeComboboxLiveV2Scene(
  root: ComboboxLiveV2RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV2NormalizedScene {
  const canonicalBindings: ComboboxLiveV2NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV2RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV2Scene<Instance>(
  normalized: ComboboxLiveV2NormalizedScene,
...
): ComboboxLiveV2SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV2FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV2ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV2PreCaptureGates(payload: unknown): void {
