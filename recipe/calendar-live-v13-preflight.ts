...
import {
  validateCalendarLiveV13AntecedentIndex,
  verifyCalendarLiveV13Authorization,
  type CalendarLiveV13AntecedentIndex,
  type CalendarLiveV13AuthorizationProof,
} from "./calendar-live-v13-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V13_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v13/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V13_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v13-security-attestation.json";

export interface CalendarLiveV13SecurityAttestation {
...
export interface CalendarLiveV13PreflightReport {
  artifactVersion: "calendar-live-v13-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV13SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV13SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV13AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v13-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V13_TARGET) ||
...
export function validateCalendarLiveV13ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV13Authorization");
  const preflight = source.indexOf("runCalendarLiveV13Preflight");
  const initialize = source.indexOf("CalendarLiveV13Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV13Preflight(
  root: string,
  proof: CalendarLiveV13AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V13_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV13PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V13_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V13_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV13AntecedentIndex;
  failures.push(...validateCalendarLiveV13AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V13_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V13_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v13-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV13ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v13 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV13SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v13-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V13_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV13Preflight(
    process.cwd(),
    verifyCalendarLiveV13Authorization(),
    attestationPath,
