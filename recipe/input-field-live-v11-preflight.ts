...
import {
  ...
  verifyInputLiveV11Authorization,
  type InputLiveV11AntecedentIndex,
  type InputLiveV11AuthorizationProof,
} from "./input-field-live-v11-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V11_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v11/operator-security-attestation-template.json";
export const INPUT_LIVE_V11_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v11-security-attestation.json";

export interface InputLiveV11SecurityAttestation {
...
export interface InputLiveV11PreflightReport {
  artifactVersion: "input-live-v11-preflight-v1";
...
export const inputLiveV11AttestationSha256 = (
  value: Omit<InputLiveV11SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV11SecurityAttestation(
  value: unknown,
  proof: InputLiveV11AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v11-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V11_TARGET) ||
...
export function validateInputLiveV11ControlFlowSource(source: string): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV11Authorization");
  const preflight = source.indexOf("runInputLiveV11Preflight");
  const initialize = source.indexOf("InputLiveV11Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV11Preflight(
  root: string,
  proof: InputLiveV11AuthorizationProof,
  attestationPath = INPUT_LIVE_V11_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV11PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V11_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V11_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V11_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV11AntecedentIndex;
  failures.push(...validateInputLiveV11AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V11_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V11_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v11-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV11ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(`Input live v11 preflight refused:\n${failures.join("\n")}`);
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as InputLiveV11SecurityAttestation;
  return {
    artifactVersion: "input-live-v11-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V11_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV11Preflight(
    process.cwd(),
    verifyInputLiveV11Authorization(),
    attestationPath,
