...
export type CalendarLiveV40HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV40AuthorizationArtifact {
  artifactVersion: "calendar-live-v40-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V40_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV40AntecedentIndex {
...
export interface CalendarLiveV40HistoryState {
  index?: CalendarLiveV40AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV40AuthorizationArtifact;
...
export interface CalendarLiveV40AuthorizationProof
  extends CalendarLiveV40TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V40_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V40_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V40_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V40_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v40-authorization") ||
  artifactPath.includes("calendar-live-v40-preflight") ||
  artifactPath.includes("calendar-live-v40-authorized") ||
  artifactPath.includes("create-calendar-live-v40-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v40-status") ||
...
export function validateCalendarLiveV40AntecedentIndex(
  index: CalendarLiveV40AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v40-antecedent-index-v1" ||
...
export function validateCalendarLiveV40UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV40History\(\s*["']/.test(source) ||
    /readCalendarLiveV40HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV40AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV40AuthorizationArtifact {
  ...
  ) as CalendarLiveV40AntecedentIndex;
  const indexFailures = validateCalendarLiveV40AntecedentIndex(index);
...
export function validateCalendarLiveV40AuthorizationArtifact(
  artifact: CalendarLiveV40AuthorizationArtifact | undefined,
  index: CalendarLiveV40AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v40-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v40" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V40_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V40_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V40_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V40_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V40_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V40_SOURCE_ROOTS ||
...
export function validateCalendarLiveV40History(
  state: CalendarLiveV40HistoryState,
  expected: CalendarLiveV40HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV40AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV40AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV40HistoryState(
  root = process.cwd(),
): CalendarLiveV40HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V40_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV40AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V40_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V40_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V40_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV40History(
  expected: CalendarLiveV40HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV40HistoryState {
  const state = readCalendarLiveV40HistoryState(root);
  const failures = validateCalendarLiveV40History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v40 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV40Authorization(
  root = process.cwd(),
): CalendarLiveV40AuthorizationProof {
  const state = verifyCalendarLiveV40History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V40_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v40/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v40.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V40_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V40_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V40_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV40HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV40History(expected);
  process.stdout.write(
    `Calendar live v40 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
