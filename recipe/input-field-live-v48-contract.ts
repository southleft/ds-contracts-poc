...
} from "./input-field-live-v3-verifier-v48.js";
import {
  buildInputLiveV48RawPropertyRuntime,
  normalizeInputLiveV48Scene,
  type InputLiveV48RawNode,
} from "./input-field-live-v48-verifier.js";
...
export {
  ...
  type InputLiveV48RestorePayload,
} from "./input-field-live-v48-restore.js";

export const INPUT_LIVE_V48_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V48_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V48_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V48_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V48_VARIANT_COUNT = 256;
export const INPUT_LIVE_V48_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V48_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV48SourceId = (typeof INPUT_LIVE_V48_SOURCE_IDS)[number];

export interface InputLiveV48SourceIdentity {
  source: InputLiveV48SourceId;
...
export interface InputLiveV48WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV48ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV48SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV48RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV48RootProof {
  source: InputLiveV48SourceId;
  adapterIdentity: string;
...
export interface InputLiveV48ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV48SourceId;
    adapterIdentity: string;
...
export interface InputLiveV48CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV48SourceId;
  adapterIdentity: string;
...
export interface InputLiveV48CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV48SourceId;
  strata: InputLiveV48CaptureCell["strata"];
...
export interface InputLiveV48CleanupPayload {
...
export function validateInputLiveV48WriterPayload(
  value: unknown,
): InputLiveV48WriterOwnership {
...
const identityPlan = (source: InputLiveV48SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV48ExtractProgram(
  writer: InputLiveV48WriterOwnership,
  sources: readonly InputLiveV48SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V48_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V48_NAMESPACE)}
${buildInputLiveV48RawPropertyRuntime()}
...
export function validateInputLiveV48ExtractPayload(
  value: unknown,
  writer: InputLiveV48WriterOwnership,
): InputLiveV48ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V48_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV48ExtractPayload;
}

export function proveInputLiveV48Roots<Instance>(
  extract: InputLiveV48ExtractPayload,
  sources: readonly (InputLiveV48SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV48Scene(
      root.scene,
...
export function assertInputLiveV48RootProofs(
  proofs: readonly InputLiveV48RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV48ProbeProgram(
  writer: InputLiveV48WriterOwnership,
  sources: readonly InputLiveV48SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V48_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV48ProbePayload(
  value: unknown,
  writer: InputLiveV48WriterOwnership,
): InputLiveV48ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV48ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V48_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V48_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV48CaptureManifest(
  cells: readonly InputLiveV48CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V48_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V48_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V48_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V48_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V48_SOURCE_IDS.some(
...
export const inputLiveV48CaptureManifestSha256 = (
  cells: readonly InputLiveV48CaptureCell[],
): string => {
  validateInputLiveV48CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV48CaptureProgram(
  writer: InputLiveV48WriterOwnership,
  cell: InputLiveV48CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V48_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V48_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V48_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV48CapturePayload(
  value: unknown,
  cell: InputLiveV48CaptureCell,
  rawResponseBytes: number,
): InputLiveV48CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV48CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V48_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V48_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV48CaptureResponses(
  manifest: readonly InputLiveV48CaptureCell[],
  responses: readonly InputLiveV48CapturePayload[],
): void {
  validateInputLiveV48CaptureManifest(manifest);
  if (
...
export interface InputLiveV48ObjectiveReport {
  artifactVersion: "input-live-v48-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV48Objective(
  manifest: readonly InputLiveV48CaptureCell[],
  responses: readonly InputLiveV48CapturePayload[],
  readReference: (cell: InputLiveV48CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV48CaptureCell) => Buffer,
): InputLiveV48ObjectiveReport {
  assertInputLiveV48CaptureResponses(manifest, responses);
...
export function buildInputLiveV48CleanupProgram(
  writer: InputLiveV48WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V48_NAMESPACE,
...
export function validateInputLiveV48CleanupPayload(
  value: unknown,
  writer: InputLiveV48WriterOwnership,
): InputLiveV48CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV48CleanupPayload;
...
export const INPUT_LIVE_V48_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV48WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
