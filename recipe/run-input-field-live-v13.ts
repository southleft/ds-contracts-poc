...
import {
  acceptInputLiveV13Response,
  createInputLiveV13Transaction,
  inputLiveV13ReceiptPath,
  inputLiveV13RequestPath,
  inputLiveV13ResponsePath,
  inputLiveV13Sha256,
  issueInputLiveV13Request,
  persistInputLiveV13RawResponse,
  persistInputLiveV13TechnicalGates,
  readInputLiveV13AcceptedRawPayload,
  type InputLiveV13Request,
  type InputLiveV13TransactionAuthorization,
} from "./input-field-live-v13-broker.js";
import {
  INPUT_LIVE_V13_CAPTURE_COUNT,
  assertInputLiveV13CaptureResponses,
  assertInputLiveV13RootProofs,
  buildInputLiveV13CaptureProgram,
  buildInputLiveV13CleanupProgram,
  ...
  evaluateInputLiveV13Objective,
  inputLiveV13CaptureManifestSha256,
  proveInputLiveV13Roots,
  validateInputLiveV13CaptureManifest,
  validateInputLiveV13CapturePayload,
  validateInputLiveV13CleanupPayload,
  validateInputLiveV13ExtractPayload,
  validateInputLiveV13ProbePayload,
  validateInputLiveV13WriterPayload,
  type InputLiveV13CaptureCell,
  type InputLiveV13CapturePayload,
...
import {
  ...
  INPUT_LIVE_V13_PLAN_PATH,
  type InputLiveV13ProofPlan,
} from "./build-input-field-live-proof-v13.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-v13.js";

export const INPUT_LIVE_V13_RUNNER_VERSION = "input-live-v13-runner-v1";

interface InputLiveV13JournalEntry {
  index: number;
  event: string;
...
interface InputLiveV13OrchestratorState {
  artifactVersion: "input-live-v13-orchestrator-state-v1";
  ...
  journal: InputLiveV13JournalEntry[];
}

interface LoadedProofInputs {
  plan: InputLiveV13ProofPlan;
  planSha256: string;
  captures: InputLiveV13CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    InputLiveV13SourceIdentity & {
...
export interface InputLiveV13NextAction {
...
const writeState = (
  directory: string,
  state: InputLiveV13OrchestratorState,
): void => {
...
const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, INPUT_LIVE_V13_PLAN_PATH);
  const capturePath = path.join(root, INPUT_LIVE_V13_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as InputLiveV13ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as InputLiveV13CaptureCell[];
  validateInputLiveV13CaptureManifest(captures);
  if (
    plan.captureManifest.sha256 !== inputLiveV13Sha256(captureBytes) ||
    captureArtifact.cellsSha256 !== inputLiveV13CaptureManifestSha256(captures)
  )
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (
      inputLiveV13Sha256(expectedBytes) !== source.expectedScenePlan.sha256 ||
      inputLiveV13Sha256(uncompressed) !==
  ...
  return {
    plan,
    planSha256: inputLiveV13Sha256(planBytes),
    captures,
    captureManifestSha256: inputLiveV13Sha256(captureBytes),
    sources,
...
const appendJournal = (
  ...
  const payloadSha256 = inputLiveV13Sha256(canonicalJson(payload));
  const body = {
  ...
  const entry = {
    ...body,
    entrySha256: inputLiveV13Sha256(canonicalJson(body)),
  };
  writeNew(
    path.join(
      directory,
...
const writerOwnership = (directory: string): InputLiveV13WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): InputLiveV13RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): InputLiveV13Request =>
  readJson<InputLiveV13Request>(file);

export class InputLiveV13Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueInputLiveV13Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: InputLiveV13OrchestratorState = {
      artifactVersion: "input-live-v13-orchestrator-state-v1",
      transactionDirectory: options.transactionDirectory,
      activeRequest: inputLiveV13RequestPath(
        options.transactionDirectory,
        "writer",
      ),
      cleanupRequest: null,
    ...
    return new InputLiveV13Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<InputLiveV13OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new InputLiveV13Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): InputLiveV13NextAction {
    const cleanup =
      this.#state.cleanupRequest &&
      !this.#state.cleanupAccepted &&
      !existsSync(inputLiveV13ReceiptPath(this.#directory, "cleanup"))
        ? this.#state.cleanupRequest
        : null;
  ...
  ingestAndAdvance(rawResponse: string): InputLiveV13NextAction {
  ...
  ingestCleanup(rawResponse: string): InputLiveV13NextAction {
    ...
    persistInputLiveV13RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptInputLiveV13Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV13CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      existsSync(inputLiveV13ReceiptPath(this.#directory, "writer")) &&
      !existsSync(path.join(this.#directory, OWNERSHIP_FILE))
    ) {
      const raw = readInputLiveV13AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateInputLiveV13WriterPayload(raw),
      );
    }
    if (existsSync(inputLiveV13RequestPath(this.#directory, "cleanup")))
      this.#state.cleanupRequest = inputLiveV13RequestPath(
        this.#directory,
        "cleanup",
      );
    if (
      existsSync(path.join(this.#directory, OWNERSHIP_FILE)) &&
      !existsSync(inputLiveV13RequestPath(this.#directory, "cleanup"))
    )
      this.#issueCleanupAndRestore(false);
    else if (
      existsSync(inputLiveV13RequestPath(this.#directory, "cleanup")) &&
      !existsSync(inputLiveV13RequestPath(this.#directory, "restore"))
    )
      this.#issueRestore(false);
    if (
      existsSync(inputLiveV13RequestPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV13ReceiptPath(this.#directory, "restore"))
    )
      this.#state.activeRequest = inputLiveV13RequestPath(
        this.#directory,
        "restore",
      );
    if (
      existsSync(inputLiveV13ReceiptPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV13RequestPath(this.#directory, "extract"))
    ) {
      const ownership = writerOwnership(this.#directory);
      const extract = issueInputLiveV13Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildInputLiveV13ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV13RequestPath(
        this.#directory,
        "extract",
    ...
    if (existsSync(inputLiveV13ReceiptPath(this.#directory, "cleanup")))
      this.#state.cleanupAccepted = true;
    this.#persist();
  }

  #acceptActive(request: InputLiveV13Request): void {
    if (request.phase === "writer") {
      const accepted = acceptInputLiveV13Response(this.#directory, "writer", {
        validate: validateInputLiveV13WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptInputLiveV13Response(this.#directory, "restore", {
        validate: (payload) =>
          validateInputLiveV13RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptInputLiveV13Response(this.#directory, "extract", {
        validate: (payload) =>
          validateInputLiveV13ExtractPayload(payload, ownership),
      });
      const proofs = proveInputLiveV13Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseInputFieldRecipe,
        compileInputFieldRecipe,
      );
      assertInputLiveV13RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "input-live-v13-host-root-proofs-v1",
      ...
      appendJournal(
        ...
        request.requestId,
        { proofsSha256: inputLiveV13Sha256(canonicalJson(proofs)) },
      );
      const next = issueInputLiveV13Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildInputLiveV13ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV13RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptInputLiveV13Response(this.#directory, "probe", {
        validate: (payload) =>
          validateInputLiveV13ProbePayload(payload, ownership),
      });
      persistInputLiveV13TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      const accepted = acceptInputLiveV13Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateInputLiveV13CapturePayload(payload, cell, rawBytes),
      });
      const png = Buffer.from(accepted.payload.pngBase64, "base64");
      const capturePath = path.join(
        this.#directory,
        "captures",
        `${String(cell.index).padStart(3, "0")}-${inputLiveV13Sha256(cell.cellKey).slice(0, 20)}.png`,
      );
      ...
      if (next < INPUT_LIVE_V13_CAPTURE_COUNT) this.#issueCapture(next);
      else {
          ...
          pngBase64: Buffer.from(
            readFileSync(
              path.join(
                this.#directory,
                "captures",
                `${String(response.index).padStart(3, "0")}-${inputLiveV13Sha256(response.cellKey).slice(0, 20)}.png`,
              ),
            ),
          ).toString("base64"),
        }));
        assertInputLiveV13CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateInputLiveV13Objective(
        ...
        appendJournal(
          ...
          request.requestId,
          {
            reportSha256: inputLiveV13Sha256(canonicalJson(objective)),
  ...
  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueInputLiveV13Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildInputLiveV13CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = inputLiveV13RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueInputLiveV13Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildInputLiveV13RestoreProgram(ownership),
    );
    this.#state.activeRequest = inputLiveV13RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueInputLiveV13Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildInputLiveV13CaptureProgram(
  ...
  #readCaptureResponseIndex(): InputLiveV13CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: InputLiveV13CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedInputLiveV13Authorization = (
  privateKey: KeyObject,
): InputLiveV13TransactionAuthorization => ({
  mode: "simulated",
  ...
  signingPublicKeySha256: inputLiveV13Sha256(
...
export const generateInputLiveV13SimulationKey = (): KeyObject =>
