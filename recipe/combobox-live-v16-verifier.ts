...
import {
  verifyComboboxLiveV16FixedSceneFixedPoint,
  type ComboboxLiveV16FixedFixedPoint,
} from "./combobox-live-v16-fixed-point.js";

export const COMBOBOX_LIVE_V16_VERIFIER_VERSION = "combobox-live-v16-verifier-v1";

export function buildComboboxLiveV16RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV16RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV16RawNode[];
}

export interface ComboboxLiveV16NormalizedScene {
...
export interface ComboboxLiveV16SceneProof {
...
export function normalizeComboboxLiveV16Scene(
  root: ComboboxLiveV16RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV16NormalizedScene {
  const canonicalBindings: ComboboxLiveV16NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV16RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV16Scene<Instance>(
  normalized: ComboboxLiveV16NormalizedScene,
...
): ComboboxLiveV16SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV16FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV16ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV16PreCaptureGates(payload: unknown): void {
