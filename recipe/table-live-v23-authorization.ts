...
export type TableLiveV23HistoryExpectation = "pending" | "authorized";

export interface TableLiveV23AuthorizationArtifact {
  artifactVersion: "table-live-v23-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V23_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV23AntecedentIndex {
...
export interface TableLiveV23HistoryState {
  index?: TableLiveV23AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV23AuthorizationArtifact;
...
export interface TableLiveV23AuthorizationProof
  extends TableLiveV23TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V23_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V23_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V23_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V23_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v23-authorization") ||
  artifactPath.includes("table-live-v23-preflight") ||
  artifactPath.includes("table-live-v23-authorized") ||
  artifactPath.includes("create-table-live-v23-security-attestation") ||
  artifactPath.includes("table-live-pivot-v23-status") ||
...
export function validateTableLiveV23AntecedentIndex(
  index: TableLiveV23AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v23-antecedent-index-v1" ||
...
export function validateTableLiveV23UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV23History\(\s*["']/.test(source) ||
    /readTableLiveV23HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV23AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV23AuthorizationArtifact {
  ...
  ) as TableLiveV23AntecedentIndex;
  const indexFailures = validateTableLiveV23AntecedentIndex(index);
...
export function validateTableLiveV23AuthorizationArtifact(
  artifact: TableLiveV23AuthorizationArtifact | undefined,
  index: TableLiveV23AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v23-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v23" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V23_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V23_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V23_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V23_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V23_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V23_SOURCE_ROOTS ||
...
export function validateTableLiveV23History(
  state: TableLiveV23HistoryState,
  expected: TableLiveV23HistoryExpectation,
): string[] {
  const failures = validateTableLiveV23AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV23AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV23HistoryState(
  root = process.cwd(),
): TableLiveV23HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V23_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV23AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V23_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V23_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V23_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV23History(
  expected: TableLiveV23HistoryExpectation,
  root = process.cwd(),
): TableLiveV23HistoryState {
  const state = readTableLiveV23HistoryState(root);
  const failures = validateTableLiveV23History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v23 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV23Authorization(
  root = process.cwd(),
): TableLiveV23AuthorizationProof {
  const state = verifyTableLiveV23History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V23_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v23/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v23.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V23_TARGET,
    expectedDynamicTool: TABLE_LIVE_V23_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V23_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV23HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV23History(expected);
  process.stdout.write(
    `Table live v23 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
