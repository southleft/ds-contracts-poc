...
import {
  ...
  createInputLiveV34FakeDynamicResponse,
  ...
  listInputLiveV34PersistedRequests,
  persistInputLiveV34RawResponse,
  persistInputLiveV34TechnicalGates,
  validateInputLiveV34Receipt,
  validateInputLiveV34Request,
  type InputLiveV34RemotePhase,
...
import {
  ...
  type InputLiveV34ExtractPayload,
  type InputLiveV34ProbePayload,
...
import {
  ...
  simulatedInputLiveV34Authorization,
} from "./run-input-field-live-v34.js";
...
const captures: InputLiveV34CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV34Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v34-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV34Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV34CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV34FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV34FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V34_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V34_TARGET);
  persistInputLiveV34RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV34Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV34Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV34WriterOwnership,
): InputLiveV34RestorePayload => ({
...
const rootProofs = (): InputLiveV34RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v34-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV34Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV34Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV34RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV34RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV34ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV34ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV34CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV34CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV34CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV34Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV34CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV34PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV34ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV34Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV34Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV34Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV34Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V34_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV34RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V34_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV34Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV34Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV34ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV34Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV34WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV34RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV34Request;
    validateInputLiveV34Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV34Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV34CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v34-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV34Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV34Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV34Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V34_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV34RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V34_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV34Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV34Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV34RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V34_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV34Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV34Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV34Request;
