...
import {
  ...
  INPUT_LIVE_V19_INDEX_PATH,
} from "./build-input-field-live-proof-v19.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV19HistoryExpectation = "pending" | "authorized";

export interface InputLiveV19AuthorizationArtifact {
  artifactVersion: "input-live-v19-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V19_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV19AntecedentIndex {
...
export interface InputLiveV19HistoryState {
  index?: InputLiveV19AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV19AuthorizationArtifact;
...
export interface InputLiveV19AuthorizationProof extends InputLiveV19TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V19_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V19_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V19_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V19_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v19-status") ||
...
export function validateInputLiveV19AntecedentIndex(
  index: InputLiveV19AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v19-antecedent-index-v1" ||
...
export function validateInputLiveV19UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV19History\(\s*["']/.test(source) ||
    /readInputLiveV19HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV19AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV19AuthorizationArtifact {
  ...
  ) as InputLiveV19AntecedentIndex;
  const indexFailures = validateInputLiveV19AntecedentIndex(index);
...
export function validateInputLiveV19AuthorizationArtifact(
  artifact: InputLiveV19AuthorizationArtifact | undefined,
  index: InputLiveV19AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v19-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v19" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V19_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V19_TARGET) ||
    !equalJson(
...
export function validateInputLiveV19History(
  state: InputLiveV19HistoryState,
  expected: InputLiveV19HistoryExpectation,
): string[] {
  const failures = validateInputLiveV19AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV19AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV19HistoryState(
  root = process.cwd(),
): InputLiveV19HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V19_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV19AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V19_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V19_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V19_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV19History(
  expected: InputLiveV19HistoryExpectation,
  root = process.cwd(),
): InputLiveV19HistoryState {
  const state = readInputLiveV19HistoryState(root);
  const failures = validateInputLiveV19History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV19Authorization(
  root = process.cwd(),
): InputLiveV19AuthorizationProof {
  const state = verifyInputLiveV19History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V19_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v19/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v19.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V19_TARGET,
    expectedDynamicTool: INPUT_LIVE_V19_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V19_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV19HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV19History(expected);
