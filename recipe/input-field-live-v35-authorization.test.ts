...
import { INPUT_LIVE_V35_INDEX_PATH } from "./build-input-field-live-proof-v35.js";
import {
  INPUT_LIVE_V35_DYNAMIC_TOOL,
  INPUT_LIVE_V35_TARGET,
} from "./input-field-live-v35-broker.js";
import {
  buildInputLiveV35AuthorizationArtifact,
  validateInputLiveV35AntecedentIndex,
  validateInputLiveV35AuthorizationArtifact,
  validateInputLiveV35History,
  validateInputLiveV35UnitTestSource,
  type InputLiveV35AntecedentIndex,
  type InputLiveV35AuthorizationArtifact,
  type InputLiveV35AuthorizationProof,
  type InputLiveV35HistoryState,
} from "./input-field-live-v35-authorization.js";
import {
  inputLiveV35AttestationSha256,
  validateInputLiveV35ControlFlowSource,
  validateInputLiveV35SecurityAttestation,
  type InputLiveV35SecurityAttestation,
} from "./input-field-live-v35-preflight.js";
import { inputLiveV35Sha256 } from "./input-field-live-v35-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V35_INDEX_PATH);
const index = (): InputLiveV35AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV35AntecedentIndex;

const artifact = (): InputLiveV35AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV35AuthorizationArtifact({
...
const pendingState = (): InputLiveV35HistoryState => ({
  index: index(),
  indexSha256: inputLiveV35Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV35HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV35AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV35Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V35_TARGET,
  expectedDynamicTool: INPUT_LIVE_V35_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v35/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV35AuthorizationArtifact,
): InputLiveV35SecurityAttestation => {
  const body: Omit<InputLiveV35SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v35-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V35_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV35AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV35History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV35History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV35History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV35History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV35History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV35UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV35UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV35AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV35Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV35AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV35AuthorizationArtifact(
        planted as InputLiveV35AuthorizationArtifact,
        index(),
        inputLiveV35Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV35SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV35AttestationSha256(
      body as Omit<InputLiveV35SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV35SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV35ControlFlowSource(source), []);
});
