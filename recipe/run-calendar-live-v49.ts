...
import {
  acceptCalendarLiveV49Response,
  createCalendarLiveV49Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV49Request,
  persistCalendarLiveV49RawResponse,
  persistCalendarLiveV49TechnicalGates,
  readCalendarLiveV49AcceptedRawPayload,
  type CalendarLiveV49Request,
  type CalendarLiveV49TransactionAuthorization,
} from "./calendar-live-v49-broker.js";
import {
  CALENDAR_LIVE_V49_CAPTURE_COUNT,
  assertCalendarLiveV49CaptureResponses,
  ...
  evaluateCalendarLiveV49Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV49Roots,
  validateCalendarLiveV49CaptureManifest,
  validateCalendarLiveV49CapturePayload,
  validateCalendarLiveV49CleanupPayload,
  validateCalendarLiveV49ExtractPayload,
  validateCalendarLiveV49ProbePayload,
  validateCalendarLiveV49WriterPayload,
  type CalendarLiveV49CaptureCell,
  type CalendarLiveV49CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V49_PLAN_PATH,
  type CalendarLiveV49ProofPlan,
} from "./build-calendar-live-proof-v49.js";
import {
...
export const CALENDAR_LIVE_V49_RUNNER_VERSION = "calendar-live-v49-runner-v1";

interface CalendarLiveV49JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV49OrchestratorState {
  artifactVersion: "calendar-live-v49-orchestrator-state-v1";
  ...
  journal: CalendarLiveV49JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV49ProofPlan;
  planSha256: string;
  captures: CalendarLiveV49CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV49SourceIdentity & {
...
export interface CalendarLiveV49NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV49OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v49 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V49_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V49_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV49ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV49CaptureCell[];
  validateCalendarLiveV49CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v49 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v49 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV49WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV49RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV49Request =>
  readJson<CalendarLiveV49Request>(file);

export class CalendarLiveV49Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV49Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV49OrchestratorState = {
      artifactVersion: "calendar-live-v49-orchestrator-state-v1",
    ...
    return new CalendarLiveV49Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV49OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV49Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV49NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV49NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v49 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV49NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v49 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV49RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV49Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV49CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV49AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV49WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV49Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV49ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV49Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV49Response(this.#directory, "writer", {
        validate: validateCalendarLiveV49WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV49Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV49RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV49Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV49ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV49Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV49RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v49-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV49Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV49ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV49Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV49ProbePayload(payload, ownership),
      });
      persistCalendarLiveV49TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v49 capture cell missing");
      const accepted = acceptCalendarLiveV49Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV49CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V49_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV49CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV49Objective(
    ...
    throw new Error(`Calendar live v49 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV49Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV49CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV49Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV49RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV49Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV49CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV49CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV49CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV49Authorization = (
  privateKey: KeyObject,
): CalendarLiveV49TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV49SimulationKey = (): KeyObject =>
