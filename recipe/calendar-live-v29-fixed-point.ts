/**
 * Calendar live v29 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V29_FIXED_VERIFIER_VERSION = "calendar-live-v29-fixed-point-v1";
export const CALENDAR_LIVE_V29_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV29FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV29FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV29FixedSourceProbe {
...
export interface CalendarLiveV29FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV29FixedFixedPoint;
}

export interface CalendarLiveV29FixedHardGateInput {
  thresholds: CalendarLiveV29FixedThresholds;
  sourceProbes: CalendarLiveV29FixedSourceProbe[];
  cells: CalendarLiveV29FixedCellValidation[];
  sceneProofs: CalendarLiveV29FixedSceneProof[];
...
export interface CalendarLiveV29FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V29_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV29FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV29FixedHardGates(
  input: CalendarLiveV29FixedHardGateInput,
): CalendarLiveV29FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV29FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V29_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV29FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV29FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV29FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
