...
import { calendarLiveV1Sha256 } from "./calendar-live-v19-broker.js";

export interface CalendarLiveV19LifecycleSimulationReport {
  artifactVersion: "calendar-live-v19-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV19AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV19Lifecycle(): Promise<CalendarLiveV19LifecycleSimulationReport> {
  await buildCalendarLiveV19Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V19_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV19AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v19-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V19_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V19_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV19History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV19History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV19AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V19_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V19_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV19History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV19History(authorized, "pending");
    ...
    await buildCalendarLiveV19Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V19_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV19AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV19Lifecycle(), null, 2)}\n`,
  );
