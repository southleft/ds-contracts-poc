...
import {
  ...
  createInputLiveV19FakeDynamicResponse,
  ...
  listInputLiveV19PersistedRequests,
  persistInputLiveV19RawResponse,
  persistInputLiveV19TechnicalGates,
  validateInputLiveV19Receipt,
  validateInputLiveV19Request,
  type InputLiveV19RemotePhase,
...
import {
  ...
  type InputLiveV19ExtractPayload,
  type InputLiveV19ProbePayload,
  type InputLiveV19RestorePayload,
  type InputLiveV19RootProof,
  type InputLiveV19WriterOwnership,
} from "./input-field-live-v19-contract.js";
import {
  InputLiveV19Orchestrator,
  simulatedInputLiveV19Authorization,
} from "./run-input-field-live-v19.js";
...
const captures: InputLiveV19CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV19Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v19-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV19Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV19CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV19FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV19FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V19_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V19_TARGET);
  persistInputLiveV19RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV19Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV19Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV19WriterOwnership,
): InputLiveV19RestorePayload => ({
...
const rootProofs = (): InputLiveV19RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV19CaptureCell,
): InputLiveV19CapturePayload => {
  const bytes = Buffer.from(`input-live-v19-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV19Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV19Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV19RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV19RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV19ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV19ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV19CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV19CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV19CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV19Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV19CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV19PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV19ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV19Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV19Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV19Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV19Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V19_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV19RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V19_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV19Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV19Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV19ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV19Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV19WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV19RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV19Request;
    validateInputLiveV19Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV19Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV19CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v19-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV19Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV19Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV19Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V19_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV19RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V19_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV19Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV19Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV19RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V19_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV19Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV19Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV19Request;
