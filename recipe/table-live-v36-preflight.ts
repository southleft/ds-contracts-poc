...
import {
  validateTableLiveV36AntecedentIndex,
  verifyTableLiveV36Authorization,
  type TableLiveV36AntecedentIndex,
  type TableLiveV36AuthorizationProof,
} from "./table-live-v36-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V36_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v36/operator-security-attestation-template.json";
export const TABLE_LIVE_V36_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v36-security-attestation.json";

export interface TableLiveV36SecurityAttestation {
  artifactVersion: "table-live-v36-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV36PreflightReport {
  artifactVersion: "table-live-v36-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV36SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV36SecurityAttestation(
  value: unknown,
  proof: TableLiveV36AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v36-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V36_TARGET) ||
...
export function validateTableLiveV36ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV36Authorization");
  const preflight = source.indexOf("runTableLiveV36Preflight");
  const initialize = source.indexOf("TableLiveV36Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV36Preflight(
  root: string,
  proof: TableLiveV36AuthorizationProof,
  attestationPath = TABLE_LIVE_V36_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV36PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V36_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V36_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV36AntecedentIndex;
  failures.push(...validateTableLiveV36AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V36_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V36_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV36SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v36-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV36ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v36 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV36SecurityAttestation;
  return {
    artifactVersion: "table-live-v36-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V36_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV36Preflight(
    process.cwd(),
    verifyTableLiveV36Authorization(),
    attestationPath,
