...
import {
  ...
  createInputLiveV47FakeDynamicResponse,
  ...
  listInputLiveV47PersistedRequests,
  persistInputLiveV47RawResponse,
  persistInputLiveV47TechnicalGates,
  validateInputLiveV47Receipt,
  validateInputLiveV47Request,
  type InputLiveV47RemotePhase,
...
import {
  ...
  type InputLiveV47ExtractPayload,
  type InputLiveV47ProbePayload,
  type InputLiveV47RestorePayload,
  type InputLiveV47RootProof,
  type InputLiveV47WriterOwnership,
} from "./input-field-live-v47-contract.js";
import {
  InputLiveV47Orchestrator,
  simulatedInputLiveV47Authorization,
} from "./run-input-field-live-v47.js";
...
const captures: InputLiveV47CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV47Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v47-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV47Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV47CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV47FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV47FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V47_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V47_TARGET);
  persistInputLiveV47RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV47Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV47Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV47WriterOwnership,
): InputLiveV47RestorePayload => ({
...
const rootProofs = (): InputLiveV47RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV47CaptureCell,
): InputLiveV47CapturePayload => {
  const bytes = Buffer.from(`input-live-v47-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV47Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV47Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV47RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV47RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV47ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV47ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV47CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV47CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV47CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV47Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV47CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV47PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV47ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV47Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV47Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV47Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV47Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V47_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV47RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V47_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV47Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV47Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV47ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV47Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV47WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV47RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV47Request;
    validateInputLiveV47Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV47Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV47CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v47-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV47Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV47Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV47Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V47_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV47RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V47_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV47Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV47Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV47RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V47_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV47Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV47Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV47Request;
