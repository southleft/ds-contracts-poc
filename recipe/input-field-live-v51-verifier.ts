...
export const INPUT_LIVE_V51_VERIFIER_VERSION = "input-live-v51-verifier-v1";

export function buildInputLiveV51RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV51RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV51RawNode[];
}

export interface InputLiveV51NormalizedScene {
...
export interface InputLiveV51SceneProof {
...
export function normalizeInputLiveV51Scene(
  root: InputLiveV51RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV51NormalizedScene {
  const canonicalBindings: InputLiveV51NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV51RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV51Scene<Instance>(
  normalized: InputLiveV51NormalizedScene,
...
export function compileInputLiveV51ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV51PreCaptureGates(payload: unknown): void {
