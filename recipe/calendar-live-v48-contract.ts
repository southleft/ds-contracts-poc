...
import { buildCalendarLiveV48CleanupRuntime } from "./calendar-live-v48-cleanup.js";
import { type CalendarLiveV48FixedFixedPoint } from "./calendar-live-v48-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV48RestoreProgram,
  validateCalendarLiveV48RestorePayload,
  type CalendarLiveV48RestorePayload,
} from "./calendar-live-v48-restore.js";
import {
  buildCalendarLiveV48RawPropertyRuntime,
  normalizeCalendarLiveV48Scene,
  type CalendarLiveV48RawNode,
} from "./calendar-live-v48-verifier.js";
...
export const CALENDAR_LIVE_V48_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V48_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V48_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V48_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V48_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V48_SET_COUNT = 3;
export const CALENDAR_LIVE_V48_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V48_HOST_PHASES = 3;
export const CALENDAR_LIVE_V48_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V48_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V48_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V48_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV48SourceId = (typeof CALENDAR_LIVE_V48_SOURCE_IDS)[number];
export type CalendarLiveV48Kind = "calendar" | "week" | "day";

export interface CalendarLiveV48SourceIdentity {
  source: CalendarLiveV48SourceId;
...
export interface CalendarLiveV48WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV48ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV48SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV48RawNode;
    weekScene: CalendarLiveV48RawNode;
    dayScene: CalendarLiveV48RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV48RootProof {
  source: CalendarLiveV48SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV48FixedFixedPoint;
}

export interface CalendarLiveV48ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV48SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV48CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV48SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV48CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV48SourceId;
  frameWidth: number;
...
export interface CalendarLiveV48CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v48 ${label} must be an object`);
...
export function validateCalendarLiveV48WriterPayload(
  value: unknown,
): CalendarLiveV48WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v48") ||
    ...
    result.namespace !== CALENDAR_LIVE_V48_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v48 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v48 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v48 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV48SourceIdentity,
  kind: CalendarLiveV48Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV48ExtractProgram(
  writer: CalendarLiveV48WriterOwnership,
  sources: readonly CalendarLiveV48SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v48 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V48_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V48_NAMESPACE)}
${buildCalendarLiveV48RawPropertyRuntime()}
...
export function validateCalendarLiveV48ExtractPayload(
  value: unknown,
  writer: CalendarLiveV48WriterOwnership,
): CalendarLiveV48ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v48 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V48_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v48 extract schema/one-root mismatch");
  return payload as CalendarLiveV48ExtractPayload;
}

export function proveCalendarLiveV48Roots<Instance>(
  extract: CalendarLiveV48ExtractPayload,
  sources: readonly (CalendarLiveV48SourceIdentity & {
...
): CalendarLiveV48RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v48 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v48 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV48Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV48Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV48Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV48FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v48 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v48 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v48 compile lost owned sets");
...
export function assertCalendarLiveV48RootProofs(
  proofs: readonly CalendarLiveV48RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v48 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v48 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV48ProbeProgram(
  writer: CalendarLiveV48WriterOwnership,
  sources: readonly CalendarLiveV48SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V48_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
for(const row of byAdapter.values()){
 ...
 instance.resizeWithoutConstraints(beforeWidth+64,instance.height);const reflowPassed=instance.layoutSizingHorizontal==="HUG"?instance.width===beforeWidth:instance.width===beforeWidth+64;const measureContentHug=node=>{if(!node||node.type!=="TEXT")return false;const hidden=node.visible===false;if(hidden)node.visible=true;const hug=node.layoutSizingHorizontal==="HUG";if(hidden)node.visible=false;return hug;};const contentHugPassed=!!measureContentHug(label);instance.resizeWithoutConstraints(beforeWidth,instance.height);
...
export function validateCalendarLiveV48ProbePayload(
  value: unknown,
  writer: CalendarLiveV48WriterOwnership,
): CalendarLiveV48ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV48ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V48_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V48_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v48 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV48CaptureManifest(
  cells: readonly CalendarLiveV48CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V48_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V48_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V48_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V48_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v48 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV48CaptureCell[],
): string => {
  validateCalendarLiveV48CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV48CaptureProgram(
  writer: CalendarLiveV48WriterOwnership,
  cell: CalendarLiveV48CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V48_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v48 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v48 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V48_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV48CapturePayload(
  value: unknown,
  cell: CalendarLiveV48CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV48CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV48CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V48_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V48_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v48 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV48CaptureResponses(
  manifest: readonly CalendarLiveV48CaptureCell[],
  responses: readonly CalendarLiveV48CapturePayload[],
): void {
  validateCalendarLiveV48CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v48 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV48ObjectiveReport {
  artifactVersion: "calendar-live-v48-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV48Objective(
  manifest: readonly CalendarLiveV48CaptureCell[],
  responses: readonly CalendarLiveV48CapturePayload[],
): CalendarLiveV48ObjectiveReport {
  assertCalendarLiveV48CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v48-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV48CleanupProgram(
  writer: CalendarLiveV48WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV48CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V48_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v48 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV48CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV48WriterOwnership,
): CalendarLiveV48CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV48CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v48 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V48_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV48WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
