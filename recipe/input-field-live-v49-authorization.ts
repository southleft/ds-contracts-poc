...
import {
  ...
  INPUT_LIVE_V49_INDEX_PATH,
} from "./build-input-field-live-proof-v49.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV49HistoryExpectation = "pending" | "authorized";

export interface InputLiveV49AuthorizationArtifact {
  artifactVersion: "input-live-v49-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V49_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV49AntecedentIndex {
...
export interface InputLiveV49HistoryState {
  index?: InputLiveV49AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV49AuthorizationArtifact;
...
export interface InputLiveV49AuthorizationProof extends InputLiveV49TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V49_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V49_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V49_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V49_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v49-status") ||
...
export function validateInputLiveV49AntecedentIndex(
  index: InputLiveV49AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v49-antecedent-index-v1" ||
...
export function validateInputLiveV49UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV49History\(\s*["']/.test(source) ||
    /readInputLiveV49HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV49AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV49AuthorizationArtifact {
  ...
  ) as InputLiveV49AntecedentIndex;
  const indexFailures = validateInputLiveV49AntecedentIndex(index);
...
export function validateInputLiveV49AuthorizationArtifact(
  artifact: InputLiveV49AuthorizationArtifact | undefined,
  index: InputLiveV49AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v49-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v49" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V49_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V49_TARGET) ||
    !equalJson(
...
export function validateInputLiveV49History(
  state: InputLiveV49HistoryState,
  expected: InputLiveV49HistoryExpectation,
): string[] {
  const failures = validateInputLiveV49AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV49AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV49HistoryState(
  root = process.cwd(),
): InputLiveV49HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V49_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV49AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V49_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V49_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V49_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV49History(
  expected: InputLiveV49HistoryExpectation,
  root = process.cwd(),
): InputLiveV49HistoryState {
  const state = readInputLiveV49HistoryState(root);
  const failures = validateInputLiveV49History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV49Authorization(
  root = process.cwd(),
): InputLiveV49AuthorizationProof {
  const state = verifyInputLiveV49History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V49_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v49/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v49.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V49_TARGET,
    expectedDynamicTool: INPUT_LIVE_V49_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V49_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV49HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV49History(expected);
