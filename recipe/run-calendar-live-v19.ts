...
import {
  acceptCalendarLiveV19Response,
  createCalendarLiveV19Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV19Request,
  persistCalendarLiveV19RawResponse,
  persistCalendarLiveV19TechnicalGates,
  readCalendarLiveV19AcceptedRawPayload,
  type CalendarLiveV19Request,
  type CalendarLiveV19TransactionAuthorization,
} from "./calendar-live-v19-broker.js";
import {
  CALENDAR_LIVE_V19_CAPTURE_COUNT,
  assertCalendarLiveV19CaptureResponses,
  ...
  evaluateCalendarLiveV19Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV19Roots,
  validateCalendarLiveV19CaptureManifest,
  validateCalendarLiveV19CapturePayload,
  validateCalendarLiveV19CleanupPayload,
  validateCalendarLiveV19ExtractPayload,
  validateCalendarLiveV19ProbePayload,
  validateCalendarLiveV19WriterPayload,
  type CalendarLiveV19CaptureCell,
  type CalendarLiveV19CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V19_PLAN_PATH,
  type CalendarLiveV19ProofPlan,
...
export const CALENDAR_LIVE_V19_RUNNER_VERSION = "calendar-live-v19-runner-v1";

interface CalendarLiveV19JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV19OrchestratorState {
  artifactVersion: "calendar-live-v19-orchestrator-state-v1";
  ...
  journal: CalendarLiveV19JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV19ProofPlan;
  planSha256: string;
  captures: CalendarLiveV19CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV19SourceIdentity & {
...
export interface CalendarLiveV19NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV19OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v19 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V19_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V19_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV19ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV19CaptureCell[];
  validateCalendarLiveV19CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v19 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v19 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV19WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV19RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV19Request =>
  readJson<CalendarLiveV19Request>(file);

export class CalendarLiveV19Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV19Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV19OrchestratorState = {
      artifactVersion: "calendar-live-v19-orchestrator-state-v1",
    ...
    return new CalendarLiveV19Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV19OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV19Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV19NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV19NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v19 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV19NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v19 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV19RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV19Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV19CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV19AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV19WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV19Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV19ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV19Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV19Response(this.#directory, "writer", {
        validate: validateCalendarLiveV19WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV19Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV19RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV19Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV19ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV19Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV19RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v19-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV19Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV19ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV19Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV19ProbePayload(payload, ownership),
      });
      persistCalendarLiveV19TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v19 capture cell missing");
      const accepted = acceptCalendarLiveV19Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV19CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V19_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV19CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV19Objective(
    ...
    throw new Error(`Calendar live v19 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV19Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV19CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV19Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV19RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV19Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV19CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV19CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV19CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV19Authorization = (
  privateKey: KeyObject,
): CalendarLiveV19TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV19SimulationKey = (): KeyObject =>
