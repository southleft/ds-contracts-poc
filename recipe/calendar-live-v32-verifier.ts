...
import {
  verifyCalendarLiveV32FixedSceneFixedPoint,
  type CalendarLiveV32FixedFixedPoint,
} from "./calendar-live-v32-fixed-point.js";

export const CALENDAR_LIVE_V32_VERIFIER_VERSION = "calendar-live-v32-verifier-v1";

export function buildCalendarLiveV32RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV32RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV32RawNode[];
}

export interface CalendarLiveV32NormalizedScene {
...
export interface CalendarLiveV32SceneProof {
...
export function normalizeCalendarLiveV32Scene(
  root: CalendarLiveV32RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV32NormalizedScene {
  const canonicalBindings: CalendarLiveV32NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV32RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV32Scene<Instance>(
  normalized: CalendarLiveV32NormalizedScene,
...
): CalendarLiveV32SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV32FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV32ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV32PreCaptureGates(payload: unknown): void {
