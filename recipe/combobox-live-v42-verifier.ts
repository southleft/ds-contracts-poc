...
import {
  verifyComboboxLiveV42FixedSceneFixedPoint,
  type ComboboxLiveV42FixedFixedPoint,
} from "./combobox-live-v42-fixed-point.js";

export const COMBOBOX_LIVE_V42_VERIFIER_VERSION = "combobox-live-v42-verifier-v1";

export function buildComboboxLiveV42RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV42RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV42RawNode[];
}

export interface ComboboxLiveV42NormalizedScene {
...
export interface ComboboxLiveV42SceneProof {
...
export function normalizeComboboxLiveV42Scene(
  root: ComboboxLiveV42RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV42NormalizedScene {
  const canonicalBindings: ComboboxLiveV42NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV42RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV42Scene<Instance>(
  normalized: ComboboxLiveV42NormalizedScene,
...
): ComboboxLiveV42SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV42FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV42ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV42PreCaptureGates(payload: unknown): void {
