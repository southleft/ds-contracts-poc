...
import {
  assertComboboxLiveV3RootProofs,
  COMBOBOX_LIVE_V3_CAPTURE_COUNT,
  COMBOBOX_LIVE_V3_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  COMBOBOX_LIVE_V3_RESPONSE_CONTRACTS,
  type ComboboxLiveV3RootProof,
} from "./combobox-live-v3-contract.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V3_BROKER_VERSION = "combobox-live-v3-broker-v2";
export const COMBOBOX_LIVE_V3_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const COMBOBOX_LIVE_V3_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const COMBOBOX_LIVE_V3_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const COMBOBOX_LIVE_V3_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const COMBOBOX_LIVE_V3_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const COMBOBOX_LIVE_V3_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type ComboboxLiveV3RemotePhase =
  (typeof COMBOBOX_LIVE_V3_REMOTE_PHASES)[number];
export type ComboboxLiveV3AuthorizationMode = "live" | "simulated";

export interface ComboboxLiveV3CommitPins {
...
export interface ComboboxLiveV3TransactionAuthorization extends ComboboxLiveV3CommitPins {
  mode: ComboboxLiveV3AuthorizationMode;
...
export interface ComboboxLiveV3Manifest {
  artifactVersion: "combobox-live-v3-broker-manifest-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V3_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V3_DYNAMIC_TOOL;
  target: typeof COMBOBOX_LIVE_V3_TARGET;
  authorization: ComboboxLiveV3TransactionAuthorization;
...
export interface ComboboxLiveV3ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface ComboboxLiveV3RequestBody {
  artifactVersion: "combobox-live-v3-broker-request-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V3_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: ComboboxLiveV3RemotePhase;
  sequence: number;
  ...
  commitPins: ComboboxLiveV3CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof COMBOBOX_LIVE_V3_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: ComboboxLiveV3ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface ComboboxLiveV3Request extends ComboboxLiveV3RequestBody {
...
export interface ComboboxLiveV3Receipt {
  artifactVersion: "combobox-live-v3-broker-receipt-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V3_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: ComboboxLiveV3RemotePhase;
...
export interface ComboboxLiveV3AcceptedResponse<T = unknown> {
  request: ComboboxLiveV3Request;
  receipt: ComboboxLiveV3Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface ComboboxLiveV3IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: ComboboxLiveV3ResponseContract;
...
const equalTarget = (value: unknown): value is typeof COMBOBOX_LIVE_V3_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === COMBOBOX_LIVE_V3_TARGET.fileKey &&
  (value as BrokerTarget).fileName === COMBOBOX_LIVE_V3_TARGET.fileName &&
  (value as BrokerTarget).editorType === COMBOBOX_LIVE_V3_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== COMBOBOX_LIVE_V3_TARGET.fileKey ||
    context.fileName !== COMBOBOX_LIVE_V3_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === COMBOBOX_LIVE_V3_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Combobox live v3 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= COMBOBOX_LIVE_V3_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v3 capture index must be 0..71");
  return `capture-${String(captureIndex).padStart(3, "0")}`;
};

export const comboboxLiveV3RequestSequence = (
  phase: ComboboxLiveV3RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(comboboxLiveV3RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const comboboxLiveV3RequestPath = (
  directory: string,
  phase: ComboboxLiveV3RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const comboboxLiveV3ResponsePath = (
  directory: string,
  phase: ComboboxLiveV3RemotePhase,
  captureIndex?: number,
): string =>
...
export const comboboxLiveV3ReceiptPath = (
  directory: string,
  phase: ComboboxLiveV3RemotePhase,
  captureIndex?: number,
): string =>
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Combobox live v3 authorization pins are malformed");
};

const readManifest = (directory: string): ComboboxLiveV3Manifest => {
  const manifest = json<ComboboxLiveV3Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Combobox live v3 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "combobox-live-v3-broker-manifest-v2" ||
    manifest.brokerVersion !== COMBOBOX_LIVE_V3_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== COMBOBOX_LIVE_V3_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(COMBOBOX_LIVE_V3_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Combobox live v3 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: ComboboxLiveV3Request,
): ComboboxLiveV3RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: ComboboxLiveV3RemotePhase,
  captureIndex?: number,
): ComboboxLiveV3Request | undefined => {
  const file = comboboxLiveV3RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<ComboboxLiveV3Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: ComboboxLiveV3RemotePhase,
  captureIndex?: number,
): ComboboxLiveV3Receipt | undefined => {
  const file = comboboxLiveV3ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<ComboboxLiveV3Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: ComboboxLiveV3Request | undefined;
  receipt: ComboboxLiveV3Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: ComboboxLiveV3RemotePhase,
): ComboboxLiveV3ResponseContract => ({
  ...COMBOBOX_LIVE_V3_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: ComboboxLiveV3Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: COMBOBOX_LIVE_V3_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: COMBOBOX_LIVE_V3_DYNAMIC_TOOL,
...
const __broker=${JSON.stringify({
  artifactVersion: "combobox-live-v3-plugin-response-v2",
  brokerVersion: COMBOBOX_LIVE_V3_BROKER_VERSION,
  target: COMBOBOX_LIVE_V3_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*COMBOBOX_LIVE_V3_PROGRAM_START*/${program}/*COMBOBOX_LIVE_V3_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*COMBOBOX_LIVE_V3_PROGRAM_START*/";
  const endMarker = "/*COMBOBOX_LIVE_V3_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Combobox live v3 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createComboboxLiveV3Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Combobox live v3 private signing key does not match authorized public-key identity",
    );
  const manifest: ComboboxLiveV3Manifest = {
    artifactVersion: "combobox-live-v3-broker-manifest-v2",
    brokerVersion: COMBOBOX_LIVE_V3_BROKER_VERSION,
    ...
    captureRequests: COMBOBOX_LIVE_V3_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateComboboxLiveV3Request(
  directory: string,
  request: ComboboxLiveV3Request,
): void {
  ...
  const expectedSequence = comboboxLiveV3RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "combobox-live-v3-broker-request-v2" ||
    request.brokerVersion !== COMBOBOX_LIVE_V3_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== COMBOBOX_LIVE_V3_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Combobox live v3 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueComboboxLiveV3Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: ComboboxLiveV3RemotePhase,
  program: string,
  options: ComboboxLiveV3IssueOptions = {},
): ComboboxLiveV3Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = comboboxLiveV3RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<ComboboxLiveV3Request>(targetPath);
    validateComboboxLiveV3Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Combobox live v3 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(comboboxLiveV3ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Combobox live v3 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Combobox live v3 empty code envelope refused");
  if (phase === "capture") validateComboboxLiveV3TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Combobox live v3 out-of-order phase: ${id}`);
  const expectedResponse =
    options.expectedResponse ?? responseContractFor(phase);
  const programSha256 = sha256(program);
  const sequence = comboboxLiveV3RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: COMBOBOX_LIVE_V3_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? COMBOBOX_LIVE_V3_SIGNED_WRITER_TIMEOUT_MS
        : COMBOBOX_LIVE_V3_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: ComboboxLiveV3RequestBody = {
    artifactVersion: "combobox-live-v3-broker-request-v2",
    brokerVersion: COMBOBOX_LIVE_V3_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: ComboboxLiveV3Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateComboboxLiveV3Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistComboboxLiveV3RawResponse(
  directory: string,
  phase: ComboboxLiveV3RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = comboboxLiveV3RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Combobox live v3 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<ComboboxLiveV3Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Combobox live v3 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = comboboxLiveV3ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Combobox live v3 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): ComboboxLiveV3Receipt => {
  const body: Omit<ComboboxLiveV3Receipt, "receiptSha256"> = {
    artifactVersion: "combobox-live-v3-broker-receipt-v2",
    brokerVersion: COMBOBOX_LIVE_V3_BROKER_VERSION,
...
export function acceptComboboxLiveV3Response<T = unknown>(
  directory: string,
  phase: ComboboxLiveV3RemotePhase,
  options: {
    captureIndex?: number;
...
): ComboboxLiveV3AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = comboboxLiveV3ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Combobox live v3 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<ComboboxLiveV3Request>(
    comboboxLiveV3RequestPath(directory, phase, captureIndex),
  );
  validateComboboxLiveV3Request(directory, request);
  if (phase === "capture") validateComboboxLiveV3TechnicalGates(directory);
  const rawFile = comboboxLiveV3ResponsePath(directory, phase, captureIndex);
  if (!existsSync(rawFile))
    throw new Error(`Combobox live v3 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "combobox-live-v3-plugin-response-v2" ||
    plugin?.brokerVersion !== COMBOBOX_LIVE_V3_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Combobox live v3 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistComboboxLiveV3TechnicalGates(
  directory: string,
  rootProofs: readonly ComboboxLiveV3RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertComboboxLiveV3RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Combobox live v3 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Combobox live v3 probe payload hash malformed");
  const gates = Object.fromEntries(
    COMBOBOX_LIVE_V3_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "combobox-live-v3-host-technical-gates-v2",
    brokerVersion: COMBOBOX_LIVE_V3_BROKER_VERSION,
...
export function validateComboboxLiveV3TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "combobox-live-v3-host-technical-gates-v2" ||
    ...
    COMBOBOX_LIVE_V3_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Combobox live v3 technical gates artifact");
}

export function validateComboboxLiveV3Receipt(
  directory: string,
  phase: ComboboxLiveV3RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateComboboxLiveV3Request(directory, request);
  const receipt = json<ComboboxLiveV3Receipt>(
    comboboxLiveV3ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    comboboxLiveV3ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "combobox-live-v3-broker-receipt-v2" ||
    receipt.brokerVersion !== COMBOBOX_LIVE_V3_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Combobox live v3 accepted receipt: ${request.requestId}`,
    );
}

export function readComboboxLiveV3AcceptedRawPayload(
  directory: string,
  phase: ComboboxLiveV3RemotePhase,
  captureIndex?: number,
): unknown {
  validateComboboxLiveV3Receipt(directory, phase, captureIndex);
...
export function createComboboxLiveV3FakeDynamicResponse(
  request: ComboboxLiveV3Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? COMBOBOX_LIVE_V3_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? COMBOBOX_LIVE_V3_TARGET,
    ...overrides.extra,
  };
}

export function listComboboxLiveV3PersistedRequests(
  directory: string,
): ComboboxLiveV3Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<ComboboxLiveV3Request>(file)] : [];
    });
}

export const comboboxLiveV3Sha256 = sha256;
