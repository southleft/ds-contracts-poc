...
} from "./input-field-live-v3-verifier-v82.js";
import {
  buildInputLiveV82RawPropertyRuntime,
  normalizeInputLiveV82Scene,
  type InputLiveV82RawNode,
} from "./input-field-live-v82-verifier.js";
...
export {
  ...
  type InputLiveV82RestorePayload,
} from "./input-field-live-v82-restore.js";

export const INPUT_LIVE_V82_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V82_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V82_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V82_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V82_VARIANT_COUNT = 256;
export const INPUT_LIVE_V82_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V82_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV82SourceId = (typeof INPUT_LIVE_V82_SOURCE_IDS)[number];

export interface InputLiveV82SourceIdentity {
  source: InputLiveV82SourceId;
...
export interface InputLiveV82WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV82ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV82SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV82RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV82RootProof {
  source: InputLiveV82SourceId;
  adapterIdentity: string;
...
export interface InputLiveV82ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV82SourceId;
    adapterIdentity: string;
...
export interface InputLiveV82CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV82SourceId;
  adapterIdentity: string;
...
export interface InputLiveV82CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV82SourceId;
  strata: InputLiveV82CaptureCell["strata"];
...
export interface InputLiveV82CleanupPayload {
...
export function validateInputLiveV82WriterPayload(
  value: unknown,
): InputLiveV82WriterOwnership {
...
const identityPlan = (source: InputLiveV82SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV82ExtractProgram(
  writer: InputLiveV82WriterOwnership,
  sources: readonly InputLiveV82SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V82_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V82_NAMESPACE)}
${buildInputLiveV82RawPropertyRuntime()}
...
export function validateInputLiveV82ExtractPayload(
  value: unknown,
  writer: InputLiveV82WriterOwnership,
): InputLiveV82ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V82_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV82ExtractPayload;
}

export function proveInputLiveV82Roots<Instance>(
  extract: InputLiveV82ExtractPayload,
  sources: readonly (InputLiveV82SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV82Scene(
      root.scene,
...
export function assertInputLiveV82RootProofs(
  proofs: readonly InputLiveV82RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV82ProbeProgram(
  writer: InputLiveV82WriterOwnership,
  sources: readonly InputLiveV82SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V82_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV82ProbePayload(
  value: unknown,
  writer: InputLiveV82WriterOwnership,
): InputLiveV82ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV82ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V82_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V82_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV82CaptureManifest(
  cells: readonly InputLiveV82CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V82_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V82_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V82_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V82_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V82_SOURCE_IDS.some(
...
export const inputLiveV82CaptureManifestSha256 = (
  cells: readonly InputLiveV82CaptureCell[],
): string => {
  validateInputLiveV82CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV82CaptureProgram(
  writer: InputLiveV82WriterOwnership,
  cell: InputLiveV82CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V82_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V82_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V82_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV82CapturePayload(
  value: unknown,
  cell: InputLiveV82CaptureCell,
  rawResponseBytes: number,
): InputLiveV82CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV82CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V82_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V82_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV82CaptureResponses(
  manifest: readonly InputLiveV82CaptureCell[],
  responses: readonly InputLiveV82CapturePayload[],
): void {
  validateInputLiveV82CaptureManifest(manifest);
  if (
...
export interface InputLiveV82ObjectiveReport {
  artifactVersion: "input-live-v82-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV82Objective(
  manifest: readonly InputLiveV82CaptureCell[],
  responses: readonly InputLiveV82CapturePayload[],
  readReference: (cell: InputLiveV82CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV82CaptureCell) => Buffer,
): InputLiveV82ObjectiveReport {
  assertInputLiveV82CaptureResponses(manifest, responses);
...
export function buildInputLiveV82CleanupProgram(
  writer: InputLiveV82WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V82_NAMESPACE,
...
export function validateInputLiveV82CleanupPayload(
  value: unknown,
  writer: InputLiveV82WriterOwnership,
): InputLiveV82CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV82CleanupPayload;
...
export const INPUT_LIVE_V82_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV82WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
