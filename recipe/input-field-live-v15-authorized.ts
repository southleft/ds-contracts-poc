...
import { INPUT_LIVE_V15_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v15.js";
import { verifyInputLiveV15Authorization } from "./input-field-live-v15-authorization.js";
import { runInputLiveV15Preflight } from "./input-field-live-v15-preflight.js";
import { InputLiveV15Orchestrator } from "./run-input-field-live-v15.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V15_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v15-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV15Authorization(root);
    runInputLiveV15Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV15Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
