...
import {
  ...
  verifyInputLiveV69Authorization,
  type InputLiveV69AntecedentIndex,
  type InputLiveV69AuthorizationProof,
} from "./input-field-live-v69-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V69_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v69/operator-security-attestation-template.json";
export const INPUT_LIVE_V69_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v69-security-attestation.json";

export interface InputLiveV69SecurityAttestation {
...
export interface InputLiveV69PreflightReport {
  artifactVersion: "input-live-v69-preflight-v1";
...
export const inputLiveV69AttestationSha256 = (
  value: Omit<InputLiveV69SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV69SecurityAttestation(
  value: unknown,
  proof: InputLiveV69AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v69-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V69_TARGET) ||
...
export function validateInputLiveV69ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV69Authorization");
  const preflight = source.indexOf("runInputLiveV69Preflight");
  const initialize = source.indexOf("InputLiveV69Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV69Preflight(
  root: string,
  proof: InputLiveV69AuthorizationProof,
  attestationPath = INPUT_LIVE_V69_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV69PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V69_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V69_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V69_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV69AntecedentIndex;
  failures.push(...validateInputLiveV69AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V69_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V69_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v69-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV69ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV69SecurityAttestation;
  return {
    artifactVersion: "input-live-v69-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V69_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV69Preflight(
    process.cwd(),
    verifyInputLiveV69Authorization(),
    attestationPath,
