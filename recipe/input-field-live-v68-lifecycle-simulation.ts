...
export interface InputLiveV68LifecycleSimulationReport {
  artifactVersion: "input-live-v68-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV68AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV68Lifecycle(): Promise<InputLiveV68LifecycleSimulationReport> {
  await buildInputLiveV68Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V68_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV68AntecedentIndex;
  const indexSha256 = inputLiveV68Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v68-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V68_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V68_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV68History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV68History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV68AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V68_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V68_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV68History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV68History(authorized, "pending");
    ...
    await buildInputLiveV68Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V68_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV68AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV68Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV68Lifecycle(), null, 2)}\n`,
  );
