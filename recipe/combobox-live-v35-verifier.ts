...
import {
  verifyComboboxLiveV35FixedSceneFixedPoint,
  type ComboboxLiveV35FixedFixedPoint,
} from "./combobox-live-v35-fixed-point.js";

export const COMBOBOX_LIVE_V35_VERIFIER_VERSION = "combobox-live-v35-verifier-v1";

export function buildComboboxLiveV35RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV35RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV35RawNode[];
}

export interface ComboboxLiveV35NormalizedScene {
...
export interface ComboboxLiveV35SceneProof {
...
export function normalizeComboboxLiveV35Scene(
  root: ComboboxLiveV35RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV35NormalizedScene {
  const canonicalBindings: ComboboxLiveV35NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV35RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV35Scene<Instance>(
  normalized: ComboboxLiveV35NormalizedScene,
...
): ComboboxLiveV35SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV35FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV35ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV35PreCaptureGates(payload: unknown): void {
