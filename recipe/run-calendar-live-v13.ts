...
import {
  acceptCalendarLiveV13Response,
  createCalendarLiveV13Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV13Request,
  persistCalendarLiveV13RawResponse,
  persistCalendarLiveV13TechnicalGates,
  readCalendarLiveV13AcceptedRawPayload,
  type CalendarLiveV13Request,
  type CalendarLiveV13TransactionAuthorization,
} from "./calendar-live-v13-broker.js";
import {
  CALENDAR_LIVE_V13_CAPTURE_COUNT,
  assertCalendarLiveV13CaptureResponses,
  ...
  evaluateCalendarLiveV13Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV13Roots,
  validateCalendarLiveV13CaptureManifest,
  validateCalendarLiveV13CapturePayload,
  validateCalendarLiveV13CleanupPayload,
  validateCalendarLiveV13ExtractPayload,
  validateCalendarLiveV13ProbePayload,
  validateCalendarLiveV13WriterPayload,
  type CalendarLiveV13CaptureCell,
  type CalendarLiveV13CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V13_PLAN_PATH,
  type CalendarLiveV13ProofPlan,
} from "./build-calendar-live-proof-v13.js";
import {
...
export const CALENDAR_LIVE_V13_RUNNER_VERSION = "calendar-live-v13-runner-v1";

interface CalendarLiveV13JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV13OrchestratorState {
  artifactVersion: "calendar-live-v13-orchestrator-state-v1";
  ...
  journal: CalendarLiveV13JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV13ProofPlan;
  planSha256: string;
  captures: CalendarLiveV13CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV13SourceIdentity & {
...
export interface CalendarLiveV13NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV13OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v13 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V13_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V13_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV13ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV13CaptureCell[];
  validateCalendarLiveV13CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v13 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v13 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV13WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV13RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV13Request =>
  readJson<CalendarLiveV13Request>(file);

export class CalendarLiveV13Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV13Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV13OrchestratorState = {
      artifactVersion: "calendar-live-v13-orchestrator-state-v1",
    ...
    return new CalendarLiveV13Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV13OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV13Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV13NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV13NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v13 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV13NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v13 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV13RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV13Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV13CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV13AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV13WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV13Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV13ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV13Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV13Response(this.#directory, "writer", {
        validate: validateCalendarLiveV13WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV13Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV13RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV13Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV13ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV13Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV13RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v13-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV13Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV13ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV13Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV13ProbePayload(payload, ownership),
      });
      persistCalendarLiveV13TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v13 capture cell missing");
      const accepted = acceptCalendarLiveV13Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV13CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V13_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV13CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV13Objective(
    ...
    throw new Error(`Calendar live v13 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV13Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV13CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV13Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV13RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV13Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV13CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV13CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV13CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV13Authorization = (
  privateKey: KeyObject,
): CalendarLiveV13TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV13SimulationKey = (): KeyObject =>
