...
import { TABLE_LIVE_V15_INDEX_PATH } from "./build-table-live-proof-v15.js";
import {
...
import {
  buildTableLiveV15AuthorizationArtifact,
  validateTableLiveV15AntecedentIndex,
  validateTableLiveV15AuthorizationArtifact,
  validateTableLiveV15History,
  validateTableLiveV15UnitTestSource,
  type TableLiveV15AntecedentIndex,
  type TableLiveV15AuthorizationArtifact,
  type TableLiveV15AuthorizationProof,
  type TableLiveV15HistoryState,
} from "./table-live-v15-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV15ControlFlowSource,
  validateTableLiveV15SecurityAttestation,
  type TableLiveV15SecurityAttestation,
} from "./table-live-v15-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V15_INDEX_PATH);
const index = (): TableLiveV15AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV15AntecedentIndex;

const artifact = (): TableLiveV15AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV15AuthorizationArtifact({
...
const pendingState = (): TableLiveV15HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV15HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV15AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v15/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV15AuthorizationArtifact,
): TableLiveV15SecurityAttestation => {
  const body: Omit<TableLiveV15SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V15_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV15History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV15History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV15History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV15History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV15History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV15UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v15-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV15UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV15AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV15AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV15AuthorizationArtifact(
        planted as TableLiveV15AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV15SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV15SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV15SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v15-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV15ControlFlowSource(source), []);
});
