...
export const INPUT_LIVE_V55_VERIFIER_VERSION = "input-live-v55-verifier-v1";

export function buildInputLiveV55RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV55RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV55RawNode[];
}

export interface InputLiveV55NormalizedScene {
...
export interface InputLiveV55SceneProof {
...
export function normalizeInputLiveV55Scene(
  root: InputLiveV55RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV55NormalizedScene {
  const canonicalBindings: InputLiveV55NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV55RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV55Scene<Instance>(
  normalized: InputLiveV55NormalizedScene,
...
export function compileInputLiveV55ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV55PreCaptureGates(payload: unknown): void {
