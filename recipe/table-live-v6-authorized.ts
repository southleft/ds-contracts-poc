...
import { TABLE_LIVE_V6_AUTHORIZATION_PATH } from "./build-table-live-proof-v6.js";
import { verifyTableLiveV6Authorization } from "./table-live-v6-authorization.js";
import { runTableLiveV6Preflight } from "./table-live-v6-preflight.js";
import { TableLiveV6Orchestrator } from "./run-table-live-v6.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v6 requires exact authorization artifact ${TABLE_LIVE_V6_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v6-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV6Authorization(root);
    runTableLiveV6Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV6Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV6Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
