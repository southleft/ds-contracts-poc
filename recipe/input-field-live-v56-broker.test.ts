...
import {
  ...
  createInputLiveV56FakeDynamicResponse,
  ...
  listInputLiveV56PersistedRequests,
  persistInputLiveV56RawResponse,
  persistInputLiveV56TechnicalGates,
  validateInputLiveV56Receipt,
  validateInputLiveV56Request,
  type InputLiveV56RemotePhase,
...
import {
  ...
  type InputLiveV56ExtractPayload,
  type InputLiveV56ProbePayload,
  type InputLiveV56RestorePayload,
  type InputLiveV56RootProof,
  type InputLiveV56WriterOwnership,
} from "./input-field-live-v56-contract.js";
import {
  InputLiveV56Orchestrator,
  simulatedInputLiveV56Authorization,
} from "./run-input-field-live-v56.js";
...
const captures: InputLiveV56CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV56Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v56-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV56Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV56CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV56FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV56FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V56_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V56_TARGET);
  persistInputLiveV56RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV56Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV56Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV56WriterOwnership,
): InputLiveV56RestorePayload => ({
...
const rootProofs = (): InputLiveV56RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV56CaptureCell,
): InputLiveV56CapturePayload => {
  const bytes = Buffer.from(`input-live-v56-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV56Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV56Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV56RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV56RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV56ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV56ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV56CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV56CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV56CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV56Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV56CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV56PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV56ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV56Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV56Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV56Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV56Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V56_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV56RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V56_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV56Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV56Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV56ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV56Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV56WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV56RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV56Request;
    validateInputLiveV56Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV56Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV56CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v56-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV56Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV56Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV56Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V56_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV56RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V56_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV56Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV56Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV56RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V56_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV56Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV56Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV56Request;
