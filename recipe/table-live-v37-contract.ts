...
import { buildTableLiveV37CleanupRuntime } from "./table-live-v37-cleanup.js";
import { type TableLiveV37FixedFixedPoint } from "./table-live-v37-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV37RestoreProgram,
  validateTableLiveV37RestorePayload,
  type TableLiveV37RestorePayload,
} from "./table-live-v37-restore.js";
import {
  buildTableLiveV37RawPropertyRuntime,
  normalizeTableLiveV37Scene,
  type TableLiveV37RawNode,
} from "./table-live-v37-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v37.js";
import {
...
export const TABLE_LIVE_V37_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V37_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V37_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V37_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V37_VARIANT_COUNT = 20;
export const TABLE_LIVE_V37_SET_COUNT = 6;
export const TABLE_LIVE_V37_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V37_HOST_PHASES = 3;
export const TABLE_LIVE_V37_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V37_RESTORE_COUNT = 8;
export const TABLE_LIVE_V37_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V37_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV37SourceId = (typeof TABLE_LIVE_V37_SOURCE_IDS)[number];
export type TableLiveV37Kind = "table" | "row" | "cell";

export interface TableLiveV37SourceIdentity {
  source: TableLiveV37SourceId;
...
export interface TableLiveV37WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV37ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV37SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV37RawNode;
    rowScene: TableLiveV37RawNode;
    cellScene: TableLiveV37RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV37RootProof {
  source: TableLiveV37SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV37FixedFixedPoint;
}

export interface TableLiveV37ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV37SourceId;
    adapterIdentity: string;
...
export interface TableLiveV37CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV37SourceId;
  adapterIdentity: string;
...
export interface TableLiveV37CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV37SourceId;
  frameWidth: number;
...
export interface TableLiveV37CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v37 ${label} must be an object`);
...
export function validateTableLiveV37WriterPayload(
  value: unknown,
): TableLiveV37WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V37_NAMESPACE ||
    ...
    throw new TypeError("Table live v37 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Table live v37 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v37 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV37SourceIdentity,
  kind: TableLiveV37Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV37ExtractProgram(
  writer: TableLiveV37WriterOwnership,
  sources: readonly TableLiveV37SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
      "Table live v37 extract requires two source identities",
    );
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V37_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V37_NAMESPACE)}
${buildTableLiveV37RawPropertyRuntime()}
...
export function validateTableLiveV37ExtractPayload(
  value: unknown,
  writer: TableLiveV37WriterOwnership,
): TableLiveV37ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v37 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V37_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v37 extract schema/two-root mismatch");
  return payload as TableLiveV37ExtractPayload;
}

export function proveTableLiveV37Roots<Instance>(
  extract: TableLiveV37ExtractPayload,
  sources: readonly (TableLiveV37SourceIdentity & {
...
): TableLiveV37RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v37 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v37 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV37Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV37Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV37Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV37FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Table live v37 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v37 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v37 compile lost owned sets");
...
export function assertTableLiveV37RootProofs(
  proofs: readonly TableLiveV37RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v37 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v37 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV37ProbeProgram(
  writer: TableLiveV37WriterOwnership,
  sources: readonly TableLiveV37SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V37_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV37ProbePayload(
  value: unknown,
  writer: TableLiveV37WriterOwnership,
): TableLiveV37ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV37ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V37_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v37 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV37CaptureManifest(
  cells: readonly TableLiveV37CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V37_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V37_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V37_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V37_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v37 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV37CaptureCell[],
): string => {
  validateTableLiveV37CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV37CaptureProgram(
  writer: TableLiveV37WriterOwnership,
  cell: TableLiveV37CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V37_CAPTURE_COUNT
  )
    throw new TypeError("Table live v37 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Table live v37 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V37_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV37CapturePayload(
  value: unknown,
  cell: TableLiveV37CaptureCell,
  rawResponseBytes: number,
): TableLiveV37CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV37CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V37_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V37_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v37 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV37CaptureResponses(
  manifest: readonly TableLiveV37CaptureCell[],
  responses: readonly TableLiveV37CapturePayload[],
): void {
  validateTableLiveV37CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v37 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV37ObjectiveReport {
  artifactVersion: "table-live-v37-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV37Objective(
  manifest: readonly TableLiveV37CaptureCell[],
  responses: readonly TableLiveV37CapturePayload[],
): TableLiveV37ObjectiveReport {
  assertTableLiveV37CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v37-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV37CleanupProgram(
  writer: TableLiveV37WriterOwnership,
): string {
  const baseProgram = buildTableLiveV37CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V37_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v37 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV37CleanupPayload(
  payload: unknown,
  writer: TableLiveV37WriterOwnership,
): TableLiveV37CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as TableLiveV37CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v37 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V37_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV37WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
