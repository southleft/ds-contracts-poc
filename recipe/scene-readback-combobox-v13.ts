...
/**
 * Carried Combobox live v13 scene-readback. Keeps v1 host-normalize,
 ...
 * binding compile-order, v11 leading-slot binding compile-order, and
 * v12 leading-slot compile-carry visible. Teaches host to order
 * `combobox/control/clear` bindings to compile field order so
 * `width.value` precedes `height.value` precedes `fills.0.color`,
 * not live extract order or the inherited fills-first trailing-slot
 * list. Sibling `combobox/control/popup` shares that same list; one
 * sort covers both. Same class as v11 leading-slot compile-order
 * sort and Input trailing-slot binding order. Reuses the existing
 * trailing-slot compile-order sort. Does not invent a Combobox-only
 * `if (combobox)`. Does not patch hashed v1–v12 lineage bytes.
 * Writer bytes stay frozen.
 */
export const SCENE_READBACK_VERSION = 1;
export const SCENE_READBACK_V13_TAUGHT_FILL_KINDS = [
  "VARIABLE_ALIAS",
  "boundVariablesOnly",
] as const;
export const COMBOBOX_LIVE_V13_OMIT_EMPTY_INSTANCE_PAYLOAD_MARKER =
  "COMBOBOX-HOST-OMIT-EMPTY-INSTANCE-PAYLOAD";
export const COMBOBOX_LIVE_V13_PROJECT_LIVE_ROOT_OWNERSHIP_KEY_MARKER =
  "COMBOBOX-HOST-PROJECT-LIVE-ROOT-OWNERSHIP-KEY";
export const COMBOBOX_LIVE_V13_RECOVER_RECIPE_COMPONENT_REF_MARKER =
  "COMBOBOX-HOST-RECOVER-RECIPE-COMPONENT-REF";
export const COMBOBOX_LIVE_V13_RECOVER_COMPONENT_PROPERTY_NAME_MARKER =
  "COMBOBOX-HOST-RECOVER-COMPONENT-PROPERTY-NAME-BEFORE-HASH";
export const COMBOBOX_LIVE_V13_TRIGGER_BINDING_COMPILE_ORDER_MARKER =
  "COMBOBOX-TRIGGER-BINDING-COMPILE-ORDER";
export const COMBOBOX_LIVE_V13_LEADING_SLOT_BINDING_COMPILE_ORDER_MARKER =
  "COMBOBOX-LEADING-SLOT-BINDING-COMPILE-ORDER-WIDTH-HEIGHT-FILL";
export const COMBOBOX_LIVE_V13_LEADING_SLOT_COMPILE_CARRY_VISIBLE_MARKER =
  "COMBOBOX-LEADING-SLOT-COMPILE-CARRY-VISIBLE-TRUE";
export const COMBOBOX_LIVE_V13_TRAILING_SLOT_BINDING_COMPILE_ORDER_MARKER =
...
const compileCarriedLeadingSlotVisible = (
  ...
  void COMBOBOX_LIVE_V13_LEADING_SLOT_COMPILE_CARRY_VISIBLE_MARKER;
  void COMBOBOX_LIVE_V1_COMPILE_CARRY_LIVE_VISIBLE_MARKER;
  const role = sceneRole(scene);
  if (
    role !== undefined &&
    LEADING_SLOT_ROLES.has(role) &&
    scene.visible !== false
  ) {
    return { visible: true };
  }
...
const orderLeadingSlotBindingsToCompileFields = (
  ...
  void COMBOBOX_LIVE_V13_LEADING_SLOT_BINDING_COMPILE_ORDER_MARKER;
  const role = sceneRole(scene);
...
const orderTrailingSlotBindingsToCompileFields = (
  ...
  void COMBOBOX_LIVE_V13_TRAILING_SLOT_BINDING_COMPILE_ORDER_MARKER;
  const role = sceneRole(scene);
...
export function canonicalizeObservedComponentRef(
  ...
  void COMBOBOX_LIVE_V13_RECOVER_RECIPE_COMPONENT_REF_MARKER;
...
export function canonicalizeObservedComponentPropertyName(key: string): string {
  void COMBOBOX_LIVE_V13_RECOVER_COMPONENT_PROPERTY_NAME_MARKER;
...
const shouldOmitEmptyInstancePayload = (
  ...
  void COMBOBOX_LIVE_V13_OMIT_EMPTY_INSTANCE_PAYLOAD_MARKER;
...
const observeSceneFacts = (root: SceneNodeSnapshot): SceneFact[] => {
  void COMBOBOX_LIVE_V13_PROJECT_LIVE_ROOT_OWNERSHIP_KEY_MARKER;
