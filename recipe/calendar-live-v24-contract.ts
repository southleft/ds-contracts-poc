...
import { buildCalendarLiveV24CleanupRuntime } from "./calendar-live-v24-cleanup.js";
import { type CalendarLiveV24FixedFixedPoint } from "./calendar-live-v24-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV24RestoreProgram,
  validateCalendarLiveV24RestorePayload,
  type CalendarLiveV24RestorePayload,
} from "./calendar-live-v24-restore.js";
import {
  buildCalendarLiveV24RawPropertyRuntime,
  normalizeCalendarLiveV24Scene,
  type CalendarLiveV24RawNode,
} from "./calendar-live-v24-verifier.js";
...
export const CALENDAR_LIVE_V24_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V24_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V24_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V24_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V24_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V24_SET_COUNT = 3;
export const CALENDAR_LIVE_V24_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V24_HOST_PHASES = 3;
export const CALENDAR_LIVE_V24_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V24_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V24_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V24_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV24SourceId = (typeof CALENDAR_LIVE_V24_SOURCE_IDS)[number];
export type CalendarLiveV24Kind = "calendar" | "week" | "day";

export interface CalendarLiveV24SourceIdentity {
  source: CalendarLiveV24SourceId;
...
export interface CalendarLiveV24WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV24ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV24SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV24RawNode;
    weekScene: CalendarLiveV24RawNode;
    dayScene: CalendarLiveV24RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV24RootProof {
  source: CalendarLiveV24SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV24FixedFixedPoint;
}

export interface CalendarLiveV24ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV24SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV24CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV24SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV24CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV24SourceId;
  frameWidth: number;
...
export interface CalendarLiveV24CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v24 ${label} must be an object`);
...
export function validateCalendarLiveV24WriterPayload(
  value: unknown,
): CalendarLiveV24WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v24") ||
    ...
    result.namespace !== CALENDAR_LIVE_V24_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v24 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v24 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v24 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV24SourceIdentity,
  kind: CalendarLiveV24Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV24ExtractProgram(
  writer: CalendarLiveV24WriterOwnership,
  sources: readonly CalendarLiveV24SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v24 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V24_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V24_NAMESPACE)}
${buildCalendarLiveV24RawPropertyRuntime()}
...
export function validateCalendarLiveV24ExtractPayload(
  value: unknown,
  writer: CalendarLiveV24WriterOwnership,
): CalendarLiveV24ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v24 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V24_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v24 extract schema/one-root mismatch");
  return payload as CalendarLiveV24ExtractPayload;
}

export function proveCalendarLiveV24Roots<Instance>(
  extract: CalendarLiveV24ExtractPayload,
  sources: readonly (CalendarLiveV24SourceIdentity & {
...
): CalendarLiveV24RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v24 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v24 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV24Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV24Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV24Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV24FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v24 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v24 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v24 compile lost owned sets");
...
export function assertCalendarLiveV24RootProofs(
  proofs: readonly CalendarLiveV24RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v24 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v24 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV24ProbeProgram(
  writer: CalendarLiveV24WriterOwnership,
  sources: readonly CalendarLiveV24SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V24_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV24ProbePayload(
  value: unknown,
  writer: CalendarLiveV24WriterOwnership,
): CalendarLiveV24ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV24ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V24_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V24_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v24 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV24CaptureManifest(
  cells: readonly CalendarLiveV24CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V24_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V24_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V24_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V24_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v24 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV24CaptureCell[],
): string => {
  validateCalendarLiveV24CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV24CaptureProgram(
  writer: CalendarLiveV24WriterOwnership,
  cell: CalendarLiveV24CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V24_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v24 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v24 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V24_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV24CapturePayload(
  value: unknown,
  cell: CalendarLiveV24CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV24CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV24CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V24_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V24_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v24 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV24CaptureResponses(
  manifest: readonly CalendarLiveV24CaptureCell[],
  responses: readonly CalendarLiveV24CapturePayload[],
): void {
  validateCalendarLiveV24CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v24 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV24ObjectiveReport {
  artifactVersion: "calendar-live-v24-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV24Objective(
  manifest: readonly CalendarLiveV24CaptureCell[],
  responses: readonly CalendarLiveV24CapturePayload[],
): CalendarLiveV24ObjectiveReport {
  assertCalendarLiveV24CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v24-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV24CleanupProgram(
  writer: CalendarLiveV24WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV24CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V24_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v24 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV24CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV24WriterOwnership,
): CalendarLiveV24CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV24CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v24 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V24_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV24WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
