...
import {
  ...
  verifyInputLiveV14Authorization,
  type InputLiveV14AntecedentIndex,
  type InputLiveV14AuthorizationProof,
} from "./input-field-live-v14-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V14_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v14/operator-security-attestation-template.json";
export const INPUT_LIVE_V14_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v14-security-attestation.json";

export interface InputLiveV14SecurityAttestation {
...
export interface InputLiveV14PreflightReport {
  artifactVersion: "input-live-v14-preflight-v1";
...
export const inputLiveV14AttestationSha256 = (
  value: Omit<InputLiveV14SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV14SecurityAttestation(
  value: unknown,
  proof: InputLiveV14AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v14-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V14_TARGET) ||
...
export function validateInputLiveV14ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV14Authorization");
  const preflight = source.indexOf("runInputLiveV14Preflight");
  const initialize = source.indexOf("InputLiveV14Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV14Preflight(
  root: string,
  proof: InputLiveV14AuthorizationProof,
  attestationPath = INPUT_LIVE_V14_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV14PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V14_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V14_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V14_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV14AntecedentIndex;
  failures.push(...validateInputLiveV14AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V14_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V14_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v14-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV14ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV14SecurityAttestation;
  return {
    artifactVersion: "input-live-v14-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V14_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV14Preflight(
    process.cwd(),
    verifyInputLiveV14Authorization(),
    attestationPath,
