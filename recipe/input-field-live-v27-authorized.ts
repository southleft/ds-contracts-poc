...
import { INPUT_LIVE_V27_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v27.js";
import { verifyInputLiveV27Authorization } from "./input-field-live-v27-authorization.js";
import { runInputLiveV27Preflight } from "./input-field-live-v27-preflight.js";
import { InputLiveV27Orchestrator } from "./run-input-field-live-v27.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V27_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v27-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV27Authorization(root);
    runInputLiveV27Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV27Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
