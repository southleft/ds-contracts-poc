...
import {
  TABLE_LIVE_V14_DYNAMIC_TOOL,
  TABLE_LIVE_V14_TARGET,
  type TableLiveV14TransactionAuthorization,
} from "./table-live-v14-broker.js";
import {
...
import {
  ...
  TABLE_LIVE_V14_HOST_PHASES,
  TABLE_LIVE_V14_REMOTE_REQUESTS,
  TABLE_LIVE_V14_SOURCE_ROOTS,
} from "./table-live-v14-contract.js";
import { canonicalJson } from "./normalize.js";

export type TableLiveV14HistoryExpectation = "pending" | "authorized";

export interface TableLiveV14AuthorizationArtifact {
  artifactVersion: "table-live-v14-capture-authorization-v1";
  authorizationId: "table-live-v14";
  status: "authorization declared; runtime security prerequisites still mandatory";
  authorizationIntent: true;
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V14_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV14AntecedentIndex {
  artifactVersion: "table-live-v14-antecedent-index-v1";
  status: string;
...
export interface TableLiveV14HistoryState {
  index?: TableLiveV14AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV14AuthorizationArtifact;
...
export interface TableLiveV14AuthorizationProof
  extends TableLiveV14TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V14_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V14_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V14_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V14_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v14-authorization") ||
  artifactPath.includes("table-live-v14-preflight") ||
  artifactPath.includes("table-live-v14-authorized") ||
  artifactPath.includes("create-table-live-v14-security-attestation") ||
  artifactPath.includes("table-live-pivot-v14-status") ||
...
export function validateTableLiveV14AntecedentIndex(
  index: TableLiveV14AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v14-antecedent-index-v1" ||
    index.authorizationCanBeAddedWithoutAntecedentRebuild !== true
  )
    failures.push("table v14 antecedent index missing or malformed");
  const artifactEntries = Object.entries(index?.artifacts ?? {});
  if (artifactEntries.length === 0)
    failures.push("table v14 antecedent hash denominator is empty");
  for (const [artifactPath, metadata] of artifactEntries) {
    ...
    if (
      ...
      failures.push(`malformed table v14 antecedent hash: ${artifactPath}`);
  }
  ...
  if (index?.hashSetSha256 !== expectedHashSet)
    failures.push("table v14 antecedent hash-set digest mismatch");
  if (
    ...
    failures.push("table v14 antecedent counts missing or drifted");
  for (const required of [
    TABLE_LIVE_V14_AUTHORIZATION_PATH,
    "recipe/table-live-v14-authorization.ts",
    "recipe/table-live-v14-authorization.test.ts",
    "recipe/table-live-v14-preflight.ts",
    "recipe/evidence/table-live-pivot-v14/operator-security-attestation-template.json",
    "recipe/evidence/status-index.json",
  ])
    if (!index?.authorizationLifecycleExcluded?.includes(required))
      failures.push(`table v14 lifecycle exclusion missing: ${required}`);
  const leaks = forbiddenAntecedentFieldPaths(index);
  if (leaks.length)
    failures.push(`table v14 antecedent result leakage: ${leaks.join(",")}`);
  return failures;
}

export function validateTableLiveV14UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV14History\(\s*["']/.test(source) ||
    /readTableLiveV14HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV14AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV14AuthorizationArtifact {
  if (!SHA40.test(options.antecedentCommit))
    throw new TypeError("table v14 antecedent commit must be a full SHA");
  const index = JSON.parse(
    Buffer.from(options.antecedentIndexBytes).toString("utf8"),
  ) as TableLiveV14AntecedentIndex;
  const indexFailures = validateTableLiveV14AntecedentIndex(index);
  if (indexFailures.length)
    throw new TypeError(
      `table v14 antecedent index refused:\n${indexFailures.join("\n")}`,
    );
  const publicKey =
  ...
  return {
    ...
    antecedent: {
      ...
      indexPath: TABLE_LIVE_V14_INDEX_PATH,
    ...
    denominator: {
      remoteRequests: TABLE_LIVE_V14_REMOTE_REQUESTS,
      hostPhases: TABLE_LIVE_V14_HOST_PHASES,
      captures: TABLE_LIVE_V14_CAPTURE_COUNT,
      sourceRoots: TABLE_LIVE_V14_SOURCE_ROOTS,
...
export function validateTableLiveV14AuthorizationArtifact(
  artifact: TableLiveV14AuthorizationArtifact | undefined,
  index: TableLiveV14AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v14-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v14" ||
    artifact.authorizationIntent !== true
  )
    failures.push("table v14 authorization declaration missing or malformed");
  const resultLeaks = forbiddenAntecedentFieldPaths(artifact);
  if (resultLeaks.length)
    failures.push(`table v14 authorization result leakage: ${resultLeaks.join(",")}`);
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V14_INDEX_PATH ||
    artifact?.antecedent?.indexSha256 !== indexSha256 ||
    artifact?.antecedent?.hashSetSha256 !== index?.hashSetSha256
  )
    failures.push("table v14 authorization pins wrong antecedent index");
  if (privateMaterialPaths(artifact).length)
    failures.push("table v14 authorization contains private key or token material");
  try {
    ...
    if (
      ...
      failures.push("table v14 Ed25519 public-key identity mismatch");
  } catch {
    failures.push("table v14 Ed25519 public key is invalid");
  }
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V14_TARGET) ||
    !equalJson(
    ...
    failures.push("wrong table v14 target or dynamic tool");
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V14_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V14_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V14_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V14_SOURCE_ROOTS ||
    artifact?.denominator?.expectedFacts !== index?.counts.expectedSceneFacts
  )
    failures.push("table v14 two-root or capture denominator changed");
  if (
    ...
    failures.push(
      "table v14 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
    ...
    failures.push("table v14 security prerequisites weakened");
  if (
    artifact?.humanSignoff?.mandatory !== true ||
    artifact?.humanSignoff?.status !== "pending"
  )
    failures.push("table v14 human signoff overclaim");
  return failures;
}

export function validateTableLiveV14History(
  state: TableLiveV14HistoryState,
  expected: TableLiveV14HistoryExpectation,
): string[] {
  const failures = validateTableLiveV14AntecedentIndex(state.index);
  if (!SHA40.test(state.codeCommit))
    failures.push("table v14 code commit is not a full SHA");
  ...
  if (
    ...
    failures.push("table v14 antecedent first-add commit is not unique");
  if (!state.antecedentIsAncestorOfCode)
    failures.push("code commit does not descend from table v14 antecedent");
  if (!state.antecedentIndexBytesMatchFirstAddition)
    failures.push("table v14 antecedent index changed after first addition");
  if (!state.antecedentArtifactsMatchIndexAtCommit)
    failures.push("table v14 antecedent commit artifacts do not match index");
  if (!state.workingAntecedentArtifactsMatchIndex)
    failures.push("table v14 antecedent working bytes drifted");
  ...
  failures.push(
    ...validateTableLiveV14AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
  ...
  if (
    ...
    failures.push("table v14 authorization first-add lineage mismatch");
  if (!state.authorizationPresentAtCodeCommit)
    failures.push("table v14 authorization missing from code commit");
  if (!state.authorizationBytesMatchFirstAddition)
    failures.push("table v14 authorization bytes changed after first addition");
  if (!state.authorizationStrictlyDescendsFromAntecedent)
    failures.push("table v14 authorization does not strictly descend from antecedent");
  if (!state.authorizationIsAncestorOfCode)
    failures.push("code commit predates table v14 authorization");
  return failures;
}
...
export function readTableLiveV14HistoryState(
  root = process.cwd(),
): TableLiveV14HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V14_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV14AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V14_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V14_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V14_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV14History(
  expected: TableLiveV14HistoryExpectation,
  root = process.cwd(),
): TableLiveV14HistoryState {
  const state = readTableLiveV14HistoryState(root);
  const failures = validateTableLiveV14History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v14 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV14Authorization(
  root = process.cwd(),
): TableLiveV14AuthorizationProof {
  const state = verifyTableLiveV14History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V14_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v14/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v14.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V14_TARGET,
    expectedDynamicTool: TABLE_LIVE_V14_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V14_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV14HistoryExpectation => {
  ...
  if (pending === authorized)
    throw new Error(
      "Choose exactly one table v14 history mode: --expect-pending or --expect-authorized",
    );
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV14History(expected);
  process.stdout.write(
    `Table live v14 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
