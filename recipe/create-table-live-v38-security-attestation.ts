...
import { TABLE_LIVE_V38_TARGET } from "./table-live-v38-broker.js";
import {
  TABLE_LIVE_V38_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV38Authorization();
...
const attestation: TableLiveV38SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV38SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v38 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V38_SECURITY_ATTESTATION_DEFAULT_PATH,
);
