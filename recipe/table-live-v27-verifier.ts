...
import {
  verifyTableLiveV27FixedSceneFixedPoint,
  type TableLiveV27FixedFixedPoint,
} from "./table-live-v27-fixed-point.js";

export const TABLE_LIVE_V27_VERIFIER_VERSION = "table-live-v27-verifier-v1";

export function buildTableLiveV27RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV27RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV27RawNode[];
}

export interface TableLiveV27NormalizedScene {
...
export interface TableLiveV27SceneProof {
...
export function normalizeTableLiveV27Scene(
  root: TableLiveV27RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV27NormalizedScene {
  const canonicalBindings: TableLiveV27NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV27RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV27Scene<Instance>(
  normalized: TableLiveV27NormalizedScene,
...
): TableLiveV27SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV27FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV27ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV27PreCaptureGates(payload: unknown): void {
