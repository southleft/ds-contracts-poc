...
} from "./input-field-live-v3-verifier-v55.js";
import {
  buildInputLiveV55RawPropertyRuntime,
  normalizeInputLiveV55Scene,
  type InputLiveV55RawNode,
} from "./input-field-live-v55-verifier.js";
...
export const INPUT_LIVE_V55_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V55_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V55_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V55_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V55_VARIANT_COUNT = 256;
export const INPUT_LIVE_V55_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V55_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV55SourceId = (typeof INPUT_LIVE_V55_SOURCE_IDS)[number];

export interface InputLiveV55SourceIdentity {
  source: InputLiveV55SourceId;
...
export interface InputLiveV55WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV55ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV55SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV55RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV55RootProof {
  source: InputLiveV55SourceId;
  adapterIdentity: string;
...
export interface InputLiveV55ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV55SourceId;
    adapterIdentity: string;
...
export interface InputLiveV55CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV55SourceId;
  adapterIdentity: string;
...
export interface InputLiveV55CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV55SourceId;
  strata: InputLiveV55CaptureCell["strata"];
...
export interface InputLiveV55CleanupPayload {
...
export function validateInputLiveV55WriterPayload(
  value: unknown,
): InputLiveV55WriterOwnership {
...
const identityPlan = (source: InputLiveV55SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV55ExtractProgram(
  writer: InputLiveV55WriterOwnership,
  sources: readonly InputLiveV55SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V55_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V55_NAMESPACE)}
${buildInputLiveV55RawPropertyRuntime()}
...
export function validateInputLiveV55ExtractPayload(
  value: unknown,
  writer: InputLiveV55WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V55_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV55ExtractPayload;
}

export function proveInputLiveV55Roots<Instance>(
  extract: InputLiveV55ExtractPayload,
  sources: readonly (InputLiveV55SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV55Scene(
      root.scene,
...
export function assertInputLiveV55RootProofs(
  proofs: readonly InputLiveV55RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV55ProbeProgram(
  writer: InputLiveV55WriterOwnership,
  sources: readonly InputLiveV55SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V55_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV55ProbePayload(
  value: unknown,
  writer: InputLiveV55WriterOwnership,
): InputLiveV55ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV55ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V55_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V55_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV55CaptureManifest(
  cells: readonly InputLiveV55CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V55_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V55_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V55_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V55_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V55_SOURCE_IDS.some(
...
export const inputLiveV55CaptureManifestSha256 = (
  cells: readonly InputLiveV55CaptureCell[],
): string => {
  validateInputLiveV55CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV55CaptureProgram(
  writer: InputLiveV55WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V55_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V55_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V55_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV55CapturePayload(
  value: unknown,
  cell: InputLiveV55CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV55CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V55_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V55_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV55CaptureResponses(
  manifest: readonly InputLiveV55CaptureCell[],
  responses: readonly InputLiveV55CapturePayload[],
): void {
  validateInputLiveV55CaptureManifest(manifest);
  if (
...
export interface InputLiveV55ObjectiveReport {
  artifactVersion: "input-live-v55-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV55Objective(
  manifest: readonly InputLiveV55CaptureCell[],
  responses: readonly InputLiveV55CapturePayload[],
  readReference: (cell: InputLiveV55CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV55CaptureCell) => Buffer,
): InputLiveV55ObjectiveReport {
  assertInputLiveV55CaptureResponses(manifest, responses);
...
export function buildInputLiveV55CleanupProgram(
  writer: InputLiveV55WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V55_NAMESPACE,
...
export function validateInputLiveV55CleanupPayload(
  value: unknown,
  writer: InputLiveV55WriterOwnership,
): InputLiveV55CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV55CleanupPayload;
...
export const INPUT_LIVE_V55_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV55WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
