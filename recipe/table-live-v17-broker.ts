...
import {
  assertTableLiveV17RootProofs,
  TABLE_LIVE_V17_CAPTURE_COUNT,
  TABLE_LIVE_V17_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  TABLE_LIVE_V17_RESPONSE_CONTRACTS,
  type TableLiveV17RootProof,
} from "./table-live-v17-contract.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V17_BROKER_VERSION = "table-live-v17-broker-v1";
export const TABLE_LIVE_V17_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const TABLE_LIVE_V17_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const TABLE_LIVE_V17_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const TABLE_LIVE_V17_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const TABLE_LIVE_V17_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const TABLE_LIVE_V17_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type TableLiveV17RemotePhase =
  (typeof TABLE_LIVE_V17_REMOTE_PHASES)[number];
export type TableLiveV17AuthorizationMode = "live" | "simulated";

export interface TableLiveV17CommitPins {
...
export interface TableLiveV17TransactionAuthorization extends TableLiveV17CommitPins {
  mode: TableLiveV17AuthorizationMode;
...
export interface TableLiveV17Manifest {
  artifactVersion: "table-live-v17-broker-manifest-v2";
  brokerVersion: typeof TABLE_LIVE_V17_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof TABLE_LIVE_V17_DYNAMIC_TOOL;
  target: typeof TABLE_LIVE_V17_TARGET;
  authorization: TableLiveV17TransactionAuthorization;
...
export interface TableLiveV17ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface TableLiveV17RequestBody {
  artifactVersion: "table-live-v17-broker-request-v2";
  brokerVersion: typeof TABLE_LIVE_V17_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: TableLiveV17RemotePhase;
  sequence: number;
  ...
  commitPins: TableLiveV17CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof TABLE_LIVE_V17_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: TableLiveV17ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface TableLiveV17Request extends TableLiveV17RequestBody {
...
export interface TableLiveV17Receipt {
  artifactVersion: "table-live-v17-broker-receipt-v2";
  brokerVersion: typeof TABLE_LIVE_V17_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: TableLiveV17RemotePhase;
...
export interface TableLiveV17AcceptedResponse<T = unknown> {
  request: TableLiveV17Request;
  receipt: TableLiveV17Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface TableLiveV17IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: TableLiveV17ResponseContract;
...
const equalTarget = (value: unknown): value is typeof TABLE_LIVE_V17_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === TABLE_LIVE_V17_TARGET.fileKey &&
  (value as BrokerTarget).fileName === TABLE_LIVE_V17_TARGET.fileName &&
  (value as BrokerTarget).editorType === TABLE_LIVE_V17_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== TABLE_LIVE_V17_TARGET.fileKey ||
    context.fileName !== TABLE_LIVE_V17_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === TABLE_LIVE_V17_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Table live v17 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= TABLE_LIVE_V17_CAPTURE_COUNT
  )
    throw new TypeError("Table live v17 capture index must be 0..19");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Table live v17 authorization pins are malformed");
};

const readManifest = (directory: string): TableLiveV17Manifest => {
  const manifest = json<TableLiveV17Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Table live v17 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "table-live-v17-broker-manifest-v2" ||
    manifest.brokerVersion !== TABLE_LIVE_V17_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== TABLE_LIVE_V17_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V17_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Table live v17 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: TableLiveV17Request,
): TableLiveV17RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: TableLiveV17RemotePhase,
  captureIndex?: number,
): TableLiveV17Request | undefined => {
  const file = tableLiveV2RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV17Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: TableLiveV17RemotePhase,
  captureIndex?: number,
): TableLiveV17Receipt | undefined => {
  const file = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV17Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: TableLiveV17Request | undefined;
  receipt: TableLiveV17Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: TableLiveV17RemotePhase,
): TableLiveV17ResponseContract => ({
  ...TABLE_LIVE_V17_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: TABLE_LIVE_V17_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: TABLE_LIVE_V17_DYNAMIC_TOOL,
      target: TABLE_LIVE_V17_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "table-live-v17-plugin-response-v2",
  brokerVersion: TABLE_LIVE_V17_BROKER_VERSION,
  target: TABLE_LIVE_V17_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*TABLE_LIVE_V17_PROGRAM_START*/${program}/*TABLE_LIVE_V17_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*TABLE_LIVE_V17_PROGRAM_START*/";
  const endMarker = "/*TABLE_LIVE_V17_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Table live v17 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createTableLiveV17Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Table live v17 private signing key does not match authorized public-key identity",
    );
  const manifest: TableLiveV17Manifest = {
    artifactVersion: "table-live-v17-broker-manifest-v2",
    brokerVersion: TABLE_LIVE_V17_BROKER_VERSION,
    ...
    captureRequests: TABLE_LIVE_V17_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateTableLiveV17Request(
  directory: string,
  request: TableLiveV17Request,
): void {
  ...
  if (
    request.artifactVersion !== "table-live-v17-broker-request-v2" ||
    request.brokerVersion !== TABLE_LIVE_V17_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== TABLE_LIVE_V17_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Table live v17 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueTableLiveV17Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: TableLiveV17RemotePhase,
  program: string,
  options: TableLiveV17IssueOptions = {},
): TableLiveV17Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<TableLiveV17Request>(targetPath);
    validateTableLiveV17Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Table live v17 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(tableLiveV2ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Table live v17 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Table live v17 empty code envelope refused");
  if (phase === "capture") validateTableLiveV17TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Table live v17 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: TABLE_LIVE_V17_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? TABLE_LIVE_V17_SIGNED_WRITER_TIMEOUT_MS
        : TABLE_LIVE_V17_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: TableLiveV17RequestBody = {
    artifactVersion: "table-live-v17-broker-request-v2",
    brokerVersion: TABLE_LIVE_V17_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: TableLiveV17Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateTableLiveV17Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistTableLiveV17RawResponse(
  directory: string,
  phase: TableLiveV17RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Table live v17 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<TableLiveV17Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Table live v17 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = tableLiveV2ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Table live v17 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<TableLiveV17Receipt, "receiptSha256"> = {
    artifactVersion: "table-live-v17-broker-receipt-v2",
    brokerVersion: TABLE_LIVE_V17_BROKER_VERSION,
...
export function acceptTableLiveV17Response<T = unknown>(
  directory: string,
  phase: TableLiveV17RemotePhase,
  options: {
    captureIndex?: number;
...
): TableLiveV17AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Table live v17 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<TableLiveV17Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Table live v17 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "table-live-v17-plugin-response-v2" ||
    plugin?.brokerVersion !== TABLE_LIVE_V17_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Table live v17 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistTableLiveV17TechnicalGates(
  directory: string,
  rootProofs: readonly TableLiveV17RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertTableLiveV17RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Table live v17 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Table live v17 probe payload hash malformed");
  const gates = Object.fromEntries(
    TABLE_LIVE_V17_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "table-live-v17-host-technical-gates-v2",
    brokerVersion: TABLE_LIVE_V17_BROKER_VERSION,
...
export function validateTableLiveV17TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "table-live-v17-host-technical-gates-v2" ||
    ...
    TABLE_LIVE_V17_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Table live v17 technical gates artifact");
}

export function validateTableLiveV17Receipt(
  directory: string,
  phase: TableLiveV17RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateTableLiveV17Request(directory, request);
  const receipt = json<TableLiveV17Receipt>(
  ...
  if (
    receipt.artifactVersion !== "table-live-v17-broker-receipt-v2" ||
    receipt.brokerVersion !== TABLE_LIVE_V17_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Table live v17 accepted receipt: ${request.requestId}`,
    );
}

export function readTableLiveV17AcceptedRawPayload(
  directory: string,
  phase: TableLiveV17RemotePhase,
  captureIndex?: number,
): unknown {
  validateTableLiveV17Receipt(directory, phase, captureIndex);
...
export function createTableLiveV17FakeDynamicResponse(
  request: TableLiveV17Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? TABLE_LIVE_V17_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? TABLE_LIVE_V17_TARGET,
    ...overrides.extra,
  };
}

export function listTableLiveV17PersistedRequests(
  directory: string,
): TableLiveV17Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<TableLiveV17Request>(file)] : [];
    });
}
