...
import {
  verifyTableLiveV36FixedSceneFixedPoint,
  type TableLiveV36FixedFixedPoint,
} from "./table-live-v36-fixed-point.js";

export const TABLE_LIVE_V36_VERIFIER_VERSION = "table-live-v36-verifier-v1";

export function buildTableLiveV36RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV36RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV36RawNode[];
}

export interface TableLiveV36NormalizedScene {
...
export interface TableLiveV36SceneProof {
...
export function normalizeTableLiveV36Scene(
  root: TableLiveV36RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV36NormalizedScene {
  const canonicalBindings: TableLiveV36NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV36RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV36Scene<Instance>(
  normalized: TableLiveV36NormalizedScene,
...
): TableLiveV36SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV36FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV36ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV36PreCaptureGates(payload: unknown): void {
