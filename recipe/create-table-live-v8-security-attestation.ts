...
import { verifyTableLiveV8Authorization } from "./table-live-v8-authorization.js";
import { TABLE_LIVE_V8_TARGET } from "./table-live-v8-broker.js";
import {
  TABLE_LIVE_V8_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV8Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-table-v8-session";
...
const attestation: TableLiveV8SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV8SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v8 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V8_SECURITY_ATTESTATION_DEFAULT_PATH,
);
