...
import { verifyCalendarLiveV14Authorization } from "./calendar-live-v14-authorization.js";
import { CALENDAR_LIVE_V14_TARGET } from "./calendar-live-v14-broker.js";
import {
  CALENDAR_LIVE_V14_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV14Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v14-session";
...
const attestation: CalendarLiveV14SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV14SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v14 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V14_SECURITY_ATTESTATION_DEFAULT_PATH,
);
