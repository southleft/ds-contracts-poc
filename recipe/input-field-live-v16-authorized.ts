...
import { INPUT_LIVE_V16_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v16.js";
import { verifyInputLiveV16Authorization } from "./input-field-live-v16-authorization.js";
import { runInputLiveV16Preflight } from "./input-field-live-v16-preflight.js";
import { InputLiveV16Orchestrator } from "./run-input-field-live-v16.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V16_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v16-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV16Authorization(root);
    runInputLiveV16Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV16Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
