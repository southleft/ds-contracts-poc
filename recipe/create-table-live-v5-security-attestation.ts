...
import { verifyTableLiveV5Authorization } from "./table-live-v5-authorization.js";
import { TABLE_LIVE_V5_TARGET } from "./table-live-v5-broker.js";
import {
  TABLE_LIVE_V5_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV5Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-table-v5-session";
...
const attestation: TableLiveV5SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV5SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v5 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V5_SECURITY_ATTESTATION_DEFAULT_PATH,
);
