...
import { TABLE_LIVE_V8_INDEX_PATH } from "./build-table-live-proof-v8.js";
import {
...
import {
  buildTableLiveV8AuthorizationArtifact,
  validateTableLiveV8AntecedentIndex,
  validateTableLiveV8AuthorizationArtifact,
  validateTableLiveV8History,
  validateTableLiveV8UnitTestSource,
  type TableLiveV8AntecedentIndex,
  type TableLiveV8AuthorizationArtifact,
  type TableLiveV8AuthorizationProof,
  type TableLiveV8HistoryState,
} from "./table-live-v8-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV8ControlFlowSource,
  validateTableLiveV8SecurityAttestation,
  type TableLiveV8SecurityAttestation,
} from "./table-live-v8-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V8_INDEX_PATH);
const index = (): TableLiveV8AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV8AntecedentIndex;

const artifact = (): TableLiveV8AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV8AuthorizationArtifact({
...
const pendingState = (): TableLiveV8HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV8HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV8AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v8/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV8AuthorizationArtifact,
): TableLiveV8SecurityAttestation => {
  const body: Omit<TableLiveV8SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V8_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV8History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV8History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV8History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV8History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV8History(authorizedState(), "pending").join("\n"),
    /stale history mode.*authorization exists/,
  );
});

test("the exact v6 phase-sensitive self-test defect is planted and table v8 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV8UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v8-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV8UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV8AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV8AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV8AuthorizationArtifact(
        planted as TableLiveV8AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV8SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV8SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV8SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v8-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV8ControlFlowSource(source), []);
});
