...
export type ComboboxLiveV42HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV42AuthorizationArtifact {
  artifactVersion: "combobox-live-v42-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V42_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV42AntecedentIndex {
...
export interface ComboboxLiveV42HistoryState {
  index?: ComboboxLiveV42AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV42AuthorizationArtifact;
...
export interface ComboboxLiveV42AuthorizationProof extends ComboboxLiveV42TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V42_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V42_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V42_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V42_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v42-authorization") ||
  artifactPath.includes("combobox-live-v42-preflight") ||
  artifactPath.includes("combobox-live-v42-authorized") ||
  artifactPath.includes("create-combobox-live-v42-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v42-status") ||
...
export function validateComboboxLiveV42AntecedentIndex(
  index: ComboboxLiveV42AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v42-antecedent-index-v1" ||
...
export function validateComboboxLiveV42UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV42History\(\s*["']/.test(source) ||
    /readComboboxLiveV42HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV42AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV42AuthorizationArtifact {
  ...
  ) as ComboboxLiveV42AntecedentIndex;
  const indexFailures = validateComboboxLiveV42AntecedentIndex(index);
...
export function validateComboboxLiveV42AuthorizationArtifact(
  artifact: ComboboxLiveV42AuthorizationArtifact | undefined,
  index: ComboboxLiveV42AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v42-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v42" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V42_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V42_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V42_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V42_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V42_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V42_SOURCE_ROOTS ||
  ...
  if (
    ...
    artifact?.execution?.v40AuthorizationReusable !== false ||
...
export function validateComboboxLiveV42History(
  state: ComboboxLiveV42HistoryState,
  expected: ComboboxLiveV42HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV42AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV42AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV42HistoryState(
  root = process.cwd(),
): ComboboxLiveV42HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V42_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV42AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V42_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V42_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V42_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV42History(
  expected: ComboboxLiveV42HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV42HistoryState {
  const state = readComboboxLiveV42HistoryState(root);
  const failures = validateComboboxLiveV42History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v42 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV42Authorization(
  root = process.cwd(),
): ComboboxLiveV42AuthorizationProof {
  const state = verifyComboboxLiveV42History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V42_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v42/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v42.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V42_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V42_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V42_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV42HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV42History(expected);
  process.stdout.write(
    `Combobox live v42 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
