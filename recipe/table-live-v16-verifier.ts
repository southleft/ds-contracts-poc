...
import {
  verifyTableLiveV16FixedSceneFixedPoint,
  type TableLiveV16FixedFixedPoint,
} from "./table-live-v16-fixed-point.js";

export const TABLE_LIVE_V16_VERIFIER_VERSION = "table-live-v16-verifier-v1";

export function buildTableLiveV16RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV16RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV16RawNode[];
}

export interface TableLiveV16NormalizedScene {
...
export interface TableLiveV16SceneProof {
...
export function normalizeTableLiveV16Scene(
  root: TableLiveV16RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV16NormalizedScene {
  const canonicalBindings: TableLiveV16NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV16RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV16Scene<Instance>(
  normalized: TableLiveV16NormalizedScene,
...
): TableLiveV16SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV16FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV16ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV16PreCaptureGates(payload: unknown): void {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("table live v16 pre-capture gate payload absent");
