...
import { verifyCalendarLiveV15Authorization } from "./calendar-live-v15-authorization.js";
import { CALENDAR_LIVE_V15_TARGET } from "./calendar-live-v15-broker.js";
import {
  CALENDAR_LIVE_V15_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV15Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v15-session";
...
const attestation: CalendarLiveV15SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV15SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v15 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V15_SECURITY_ATTESTATION_DEFAULT_PATH,
);
