...
import { CALENDAR_LIVE_V38_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v38.js";
import { verifyCalendarLiveV38Authorization } from "./calendar-live-v38-authorization.js";
import { runCalendarLiveV38Preflight } from "./calendar-live-v38-preflight.js";
import { CalendarLiveV38Orchestrator } from "./run-calendar-live-v38.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v38 requires exact authorization artifact ${CALENDAR_LIVE_V38_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v38-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV38Authorization(root);
    runCalendarLiveV38Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV38Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV38Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
