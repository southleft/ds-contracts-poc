/**
 * Table live v4 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V4_FIXED_VERIFIER_VERSION = "table-live-v4-fixed-point-v1";
export const TABLE_LIVE_V4_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV4FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV4FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV4FixedSourceProbe {
...
export interface TableLiveV4FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV4FixedFixedPoint;
}

export interface TableLiveV4FixedHardGateInput {
  thresholds: TableLiveV4FixedThresholds;
  sourceProbes: TableLiveV4FixedSourceProbe[];
  cells: TableLiveV4FixedCellValidation[];
  sceneProofs: TableLiveV4FixedSceneProof[];
...
export interface TableLiveV4FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V4_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV4FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV4FixedHardGates(
  input: TableLiveV4FixedHardGateInput,
): TableLiveV4FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV4FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  if (input.visualRows.length !== 0)
    failures.push("table live v4 must not invent a visual comparison");
  ...
  return {
    verifierVersion: TABLE_LIVE_V4_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV4FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV4FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
