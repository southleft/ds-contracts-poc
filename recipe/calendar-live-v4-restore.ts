...
export const CALENDAR_LIVE_V4_MEASURE_HUG_MARKER = "CALENDAR-TEXT-HUG-MEASURE";
export const CALENDAR_LIVE_V4_RESTORE_CONTENT_ROLES = [
  "calendar/day/label",
] as const;
export const CALENDAR_LIVE_V4_RESTORE_DAY_SET_ROLE = "calendar/day-set";
...
export interface CalendarLiveV4RestoreWriter {
  pageId: string;
  runIdentity: string;
  setIds: readonly string[];
}

export interface CalendarLiveV4RestorePayload {
  pageId: string;
  setIds: string[];
  restoredCount: 4;
  fixedBefore: number;
  ...
  marker: typeof CALENDAR_LIVE_V4_MEASURE_HUG_MARKER;
}

export function buildCalendarLiveV4RestoreProgram(
  writer: CalendarLiveV4RestoreWriter,
): string {
  if (writer.setIds.length !== 3)
    throw new TypeError("Calendar live v4 restore requires three owned sets");
  if (
...
const ROLES=new Set(${JSON.stringify([...CALENDAR_LIVE_V4_RESTORE_CONTENT_ROLES])});
const DAY_SET=${JSON.stringify(CALENDAR_LIVE_V4_RESTORE_DAY_SET_ROLE)};
const MARKER=${JSON.stringify(CALENDAR_LIVE_V4_MEASURE_HUG_MARKER)};
...
if(!page||page.type!=="PAGE")throw new Error("CALENDAR-V4-RESTORE-PAGE");
...
if(page.getSharedPluginData(NS,"pageOwner")!=="recipe/calendar/"+${JSON.stringify(writer.runIdentity)})throw new Error("CALENDAR-V4-RESTORE-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>SET_IDS.includes(node.id));
if(sets.length!==3)throw new Error("CALENDAR-V4-RESTORE-ROOTS:"+sets.length);
const texts=[];
...
for(const entry of texts){
  ...
  if(after==="FILL")throw new Error("CALENDAR-V4-RESTORE-INVENTED-FILL:"+entry.role);
...
if(restored.length!==4)throw new Error("CALENDAR-V4-RESTORE-COUNT:"+restored.length);
if(restored.some(entry=>entry.after!=="HUG"))throw new Error("CALENDAR-V4-RESTORE-NOT-HUG");
...
export function validateCalendarLiveV4RestorePayload(
  payload: unknown,
  writer: CalendarLiveV4RestoreWriter,
): CalendarLiveV4RestorePayload {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("Calendar live v4 restore payload must be an object");
  ...
  if (
    ...
    value.marker !== CALENDAR_LIVE_V4_MEASURE_HUG_MARKER
  )
    throw new TypeError("Calendar live v4 restore did not re-assert content HUG");
  return value as CalendarLiveV4RestorePayload;
}
