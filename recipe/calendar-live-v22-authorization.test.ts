...
import { CALENDAR_LIVE_V22_INDEX_PATH } from "./build-calendar-live-proof-v22.js";
import {
...
import {
  buildCalendarLiveV22AuthorizationArtifact,
  validateCalendarLiveV22AntecedentIndex,
  validateCalendarLiveV22AuthorizationArtifact,
  validateCalendarLiveV22History,
  validateCalendarLiveV22UnitTestSource,
  type CalendarLiveV22AntecedentIndex,
  type CalendarLiveV22AuthorizationArtifact,
  type CalendarLiveV22AuthorizationProof,
  type CalendarLiveV22HistoryState,
} from "./calendar-live-v22-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV22ControlFlowSource,
  validateCalendarLiveV22SecurityAttestation,
  type CalendarLiveV22SecurityAttestation,
} from "./calendar-live-v22-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V22_INDEX_PATH);
const index = (): CalendarLiveV22AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV22AntecedentIndex;

const artifact = (): CalendarLiveV22AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV22AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV22HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV22HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV22AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v22/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV22AuthorizationArtifact,
): CalendarLiveV22SecurityAttestation => {
  const body: Omit<CalendarLiveV22SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V22_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV22History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV22History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV22History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV22History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV22History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV22UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v22-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV22UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV22AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV22AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV22AuthorizationArtifact(
        planted as CalendarLiveV22AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV22SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV22SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV22SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v22-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV22ControlFlowSource(source), []);
});
