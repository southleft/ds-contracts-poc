...
export const INPUT_LIVE_V76_VERIFIER_VERSION = "input-live-v76-verifier-v1";

export function buildInputLiveV76RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV76RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV76RawNode[];
}

export interface InputLiveV76NormalizedScene {
...
export interface InputLiveV76SceneProof {
...
export function normalizeInputLiveV76Scene(
  root: InputLiveV76RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV76NormalizedScene {
  const canonicalBindings: InputLiveV76NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV76RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV76Scene<Instance>(
  normalized: InputLiveV76NormalizedScene,
...
export function compileInputLiveV76ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV76PreCaptureGates(payload: unknown): void {
