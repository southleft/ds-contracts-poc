...
import {
  ...
  COMBOBOX_LIVE_V30_LEADING_SLOT_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V30_LEADING_SLOT_COMPILE_CARRY_VISIBLE_MARKER,
  COMBOBOX_LIVE_V30_LISTBOX_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V30_LISTBOX_CLIPS_CONTENT_OMITTED_MARKER,
  COMBOBOX_LIVE_V30_LISTBOX_CORNER_RADIUS_OMITTED_MARKER,
  COMBOBOX_LIVE_V30_LISTBOX_EMPTY_EFFECTS_OMITTED_MARKER,
  COMBOBOX_LIVE_V30_LISTBOX_EMPTY_STROKES_OMITTED_MARKER,
  COMBOBOX_LIVE_V30_OMIT_EMPTY_INSTANCE_PAYLOAD_MARKER,
  COMBOBOX_LIVE_V30_OPTION_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V30_OPTION_CLIPS_CONTENT_OMITTED_MARKER,
  COMBOBOX_LIVE_V30_OPTION_LAYOUT_HEIGHT_ALIAS_MARKER,
  COMBOBOX_LIVE_V30_OPTION_INSTANCE_BINDING_EXTRAS_MARKER,
  COMBOBOX_LIVE_V30_OPTION_INSTANCE_FILLS_OMITTED_MARKER,
  COMBOBOX_LIVE_V30_OPTION_INSTANCE_PAYLOAD_OMITTED_MARKER,
  COMBOBOX_LIVE_V30_OVERLAY_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V30_OVERLAY_EMPTY_DASH_PATTERN_OMITTED_MARKER,
  COMBOBOX_LIVE_V30_SET_CLIPS_CONTENT_OMITTED_MARKER,
  COMBOBOX_LIVE_V30_OVERLAY_LAYOUT_WIDTH_ALIAS_MARKER,
  COMBOBOX_LIVE_V30_PROJECT_LIVE_ROOT_OWNERSHIP_KEY_MARKER,
  COMBOBOX_LIVE_V30_RECOVER_COMPONENT_PROPERTY_NAME_MARKER,
  COMBOBOX_LIVE_V30_RECOVER_RECIPE_COMPONENT_REF_MARKER,
  COMBOBOX_LIVE_V30_TRAILING_SLOT_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V30_TRAILING_SLOT_COMPILE_CARRY_VISIBLE_MARKER,
  COMBOBOX_LIVE_V30_TRIGGER_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V30_TRIGGER_EMPTY_EFFECTS_OMITTED_MARKER,
...
test("host omits combobox/option clipsContent that compile never emits", () => {
  ...
  assert.equal(
    compiledOverlay.kind === "frame" && compiledOverlay.clipsContent,
    true,
    "compile overlay emits clipsContent true; v30 must not omit it",
  );
  assert.equal(
    compiledVariant.kind === "component" && compiledVariant.clipsContent,
    false,
    "compile variant emits clipsContent false; v30 must not omit it",
  );
  assert.equal(
    "clipsContent" in compiledListbox,
    false,
  ...
  assert.equal(
    ...
    false,
    "hashed v29 still emits live option clipsContent false; v30 must not patch those bytes",
  );
  const observed = sceneToNormalizedIr(live);
  assert.equal(observed.kind, "component");
  assert.equal(observed.role, "combobox/option/small/default");
  assert.equal(
    "clipsContent" in observed,
    false,
    "v30 host must omit combobox/option clipsContent that compile never emits",
  );
  const overlay = sceneToNormalizedIr(liveOverlayWithDropShadow());
  assert.equal(overlay.kind, "frame");
  assert.equal(
    overlay.kind === "frame" && overlay.clipsContent,
    true,
    "v30 must keep overlay clipsContent that compile emits",
  );
  const variant = sceneToNormalizedIr(liveVariantWithClipsContentFalse());
  assert.equal(variant.kind, "component");
  assert.equal(
    variant.kind === "component" && variant.clipsContent,
    false,
    "v30 must keep variant clipsContent false that compile emits",
  );
  for (const role of ["combobox/set", "combobox/option-set"] as const) {
    ...
    assert.equal(
      ...
      false,
      `v30 must keep the v27 ${role} clipsContent omit`,
    );
  }
});
