...
import {
  ...
  INPUT_LIVE_V33_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V33_RESPONSE_CONTRACTS,
  type InputLiveV33RootProof,
} from "./input-field-live-v33-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V33_BROKER_VERSION = "input-live-v33-broker-v2";
export const INPUT_LIVE_V33_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V33_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V33_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V33_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V33_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V33_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV33RemotePhase =
  (typeof INPUT_LIVE_V33_REMOTE_PHASES)[number];
export type InputLiveV33AuthorizationMode = "live" | "simulated";

export interface InputLiveV33CommitPins {
...
export interface InputLiveV33TransactionAuthorization extends InputLiveV33CommitPins {
  mode: InputLiveV33AuthorizationMode;
...
export interface InputLiveV33Manifest {
  artifactVersion: "input-live-v33-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V33_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V33_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V33_TARGET;
...
export interface InputLiveV33ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV33RequestBody {
  artifactVersion: "input-live-v33-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V33_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV33RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV33CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V33_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV33ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV33Request extends InputLiveV33RequestBody {
...
export interface InputLiveV33Receipt {
  artifactVersion: "input-live-v33-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V33_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV33RemotePhase;
...
export interface InputLiveV33AcceptedResponse<T = unknown> {
  request: InputLiveV33Request;
  receipt: InputLiveV33Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV33IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV33ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V33_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V33_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V33_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V33_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V33_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V33_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V33_TARGET.editorType;
};
...
export const inputLiveV33RequestSequence = (
  phase: InputLiveV33RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV33RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV33RequestPath = (
  directory: string,
  phase: InputLiveV33RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV33ResponsePath = (
  directory: string,
  phase: InputLiveV33RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV33ReceiptPath = (
  directory: string,
  phase: InputLiveV33RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV33Manifest => {
  const manifest = json<InputLiveV33Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v33-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V33_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V33_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV33RemotePhase,
  captureIndex?: number,
): InputLiveV33Request | undefined => {
  const file = inputLiveV33RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV33Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV33RemotePhase,
  captureIndex?: number,
): InputLiveV33Receipt | undefined => {
  const file = inputLiveV33ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV33Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV33Request | undefined;
  receipt: InputLiveV33Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV33RemotePhase,
): InputLiveV33ResponseContract => ({
  ...INPUT_LIVE_V33_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V33_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V33_DYNAMIC_TOOL,
      target: INPUT_LIVE_V33_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v33-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V33_BROKER_VERSION,
  target: INPUT_LIVE_V33_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V33_PROGRAM_START*/${program}/*INPUT_LIVE_V33_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V33_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V33_PROGRAM_END*/";
...
export function createInputLiveV33Transaction(
  directory: string,
  ...
  const manifest: InputLiveV33Manifest = {
    artifactVersion: "input-live-v33-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V33_BROKER_VERSION,
...
export function validateInputLiveV33Request(
  directory: string,
  request: InputLiveV33Request,
): void {
  ...
  const expectedSequence = inputLiveV33RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v33-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V33_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V33_TARGET.fileKey ||
...
export function issueInputLiveV33Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV33RemotePhase,
  program: string,
  options: InputLiveV33IssueOptions = {},
): InputLiveV33Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV33RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV33Request>(targetPath);
    validateInputLiveV33Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV33ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV33TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV33RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V33_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V33_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V33_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV33RequestBody = {
    artifactVersion: "input-live-v33-broker-request-v2",
    brokerVersion: INPUT_LIVE_V33_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV33Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV33Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV33RawResponse(
  directory: string,
  phase: InputLiveV33RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV33RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV33Request>(requestFile);
  if (
  ...
  const target = inputLiveV33ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV33Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v33-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V33_BROKER_VERSION,
...
export function acceptInputLiveV33Response<T = unknown>(
  directory: string,
  phase: InputLiveV33RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV33AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV33ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV33Request>(
    inputLiveV33RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV33Request(directory, request);
  if (phase === "capture") validateInputLiveV33TechnicalGates(directory);
  const rawFile = inputLiveV33ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v33-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V33_BROKER_VERSION ||
...
export function persistInputLiveV33TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV33RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV33RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V33_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v33-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V33_BROKER_VERSION,
...
export function validateInputLiveV33TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v33-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V33_TECHNICAL_GATES.some(
...
export function validateInputLiveV33Receipt(
  directory: string,
  phase: InputLiveV33RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV33Request(directory, request);
  const receipt = json<InputLiveV33Receipt>(
    inputLiveV33ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV33ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v33-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V33_BROKER_VERSION ||
...
export function readInputLiveV33AcceptedRawPayload(
  directory: string,
  phase: InputLiveV33RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV33Receipt(directory, phase, captureIndex);
...
export function createInputLiveV33FakeDynamicResponse(
  request: InputLiveV33Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V33_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V33_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV33PersistedRequests(
  directory: string,
): InputLiveV33Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV33Request>(file)] : [];
    });
}

export const inputLiveV33Sha256 = sha256;
