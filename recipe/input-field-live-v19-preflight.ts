...
import {
  ...
  verifyInputLiveV19Authorization,
  type InputLiveV19AntecedentIndex,
  type InputLiveV19AuthorizationProof,
} from "./input-field-live-v19-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V19_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v19/operator-security-attestation-template.json";
export const INPUT_LIVE_V19_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v19-security-attestation.json";

export interface InputLiveV19SecurityAttestation {
...
export interface InputLiveV19PreflightReport {
  artifactVersion: "input-live-v19-preflight-v1";
...
export const inputLiveV19AttestationSha256 = (
  value: Omit<InputLiveV19SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV19SecurityAttestation(
  value: unknown,
  proof: InputLiveV19AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v19-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V19_TARGET) ||
...
export function validateInputLiveV19ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV19Authorization");
  const preflight = source.indexOf("runInputLiveV19Preflight");
  const initialize = source.indexOf("InputLiveV19Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV19Preflight(
  root: string,
  proof: InputLiveV19AuthorizationProof,
  attestationPath = INPUT_LIVE_V19_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV19PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V19_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V19_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V19_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV19AntecedentIndex;
  failures.push(...validateInputLiveV19AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V19_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V19_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v19-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV19ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV19SecurityAttestation;
  return {
    artifactVersion: "input-live-v19-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V19_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV19Preflight(
    process.cwd(),
    verifyInputLiveV19Authorization(),
    attestationPath,
