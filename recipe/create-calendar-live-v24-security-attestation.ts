...
import { verifyCalendarLiveV24Authorization } from "./calendar-live-v24-authorization.js";
import { CALENDAR_LIVE_V24_TARGET } from "./calendar-live-v24-broker.js";
import {
  CALENDAR_LIVE_V24_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV24Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v24-session";
...
const attestation: CalendarLiveV24SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV24SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v24 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V24_SECURITY_ATTESTATION_DEFAULT_PATH,
);
