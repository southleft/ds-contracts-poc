...
export interface CalendarLiveV3CleanupPlan {
  fileKey: string;
  fileName: string;
  editorType: string;
  namespace: string;
...
export function buildCalendarLiveV3CleanupRuntime(
  plan: CalendarLiveV3CleanupPlan,
): string {
  ...
  return String.raw`
if(figma.fileKey!==${JSON.stringify(plan.fileKey)}||figma.root.name!==${JSON.stringify(plan.fileName)}||figma.editorType!==${JSON.stringify(plan.editorType)})throw new Error("CALENDAR-V3-CLEANUP-WRONG-TARGET");
