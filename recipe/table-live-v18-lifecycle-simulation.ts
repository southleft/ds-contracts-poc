...
import { tableLiveV2Sha256 } from "./table-live-v18-broker.js";

export interface TableLiveV18LifecycleSimulationReport {
  artifactVersion: "table-live-v18-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV18AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV18Lifecycle(): Promise<TableLiveV18LifecycleSimulationReport> {
  await buildTableLiveV18Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V18_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV18AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v18-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V18_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V18_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV18History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV18History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV18AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V18_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V18_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV18History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV18History(authorized, "pending");
    ...
    await buildTableLiveV18Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V18_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV18AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV18Lifecycle(), null, 2)}\n`,
  );
