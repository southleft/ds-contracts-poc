...
import {
  ...
  verifyTableLiveV1Authorization,
  type TableLiveV1AntecedentIndex,
  type TableLiveV1AuthorizationProof,
} from "./table-live-v1-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V1_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v1/operator-security-attestation-template.json";
export const TABLE_LIVE_V1_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v1-security-attestation.json";

export interface TableLiveV1SecurityAttestation {
...
export interface TableLiveV1PreflightReport {
  artifactVersion: "table-live-v1-preflight-v1";
...
export const tableLiveV1AttestationSha256 = (
  value: Omit<TableLiveV1SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV1SecurityAttestation(
  value: unknown,
  proof: TableLiveV1AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v1-operator-security-attestation-v1" ||
    artifact.status !== "complete"
  )
    failures.push("missing completed table v1 security attestation");
  if (
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V1_TARGET) ||
...
export function validateTableLiveV1ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV1Authorization");
  const preflight = source.indexOf("runTableLiveV1Preflight");
  const initialize = source.indexOf("TableLiveV1Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV1Preflight(
  root: string,
  proof: TableLiveV1AuthorizationProof,
  attestationPath = TABLE_LIVE_V1_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV1PreflightReport {
  const failures: string[] = [];
  if (
    ...
    failures.push("table v1 attempt chronology invalid or exceeds maximum 3");
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V1_TARGET) ||
    canonicalJson(proof.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V1_DYNAMIC_TOOL)
  )
    failures.push("table v1 authorization proof target or tool mismatch");
  const indexPath = path.join(root, TABLE_LIVE_V1_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV1AntecedentIndex;
  failures.push(...validateTableLiveV1AntecedentIndex(index));
  if (
    sha256(indexBytes) !== proof.antecedentIndexSha256 ||
    index.hashSetSha256 !== proof.antecedentHashSetSha256
  )
    failures.push("table v1 authorization proof antecedent index mismatch");
  for (const [relativePath, metadata] of Object.entries(index.artifacts)) {
    ...
    if (
      ...
      failures.push(
        `table v1 pinned antecedent working bytes drift: ${relativePath}`,
      );
  }
  if (!existsSync(path.join(root, TABLE_LIVE_V1_AUTHORIZATION_PATH)))
    failures.push("table v1 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V1_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v1-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV1ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v1 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV1SecurityAttestation;
  return {
    artifactVersion: "table-live-v1-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V1_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV1Preflight(
    process.cwd(),
    verifyTableLiveV1Authorization(),
    attestationPath,
