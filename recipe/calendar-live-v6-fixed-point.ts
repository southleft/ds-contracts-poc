/**
 * Calendar live v6 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V6_FIXED_VERIFIER_VERSION = "calendar-live-v6-fixed-point-v1";
export const CALENDAR_LIVE_V6_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV6FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV6FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV6FixedSourceProbe {
...
export interface CalendarLiveV6FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV6FixedFixedPoint;
}

export interface CalendarLiveV6FixedHardGateInput {
  thresholds: CalendarLiveV6FixedThresholds;
  sourceProbes: CalendarLiveV6FixedSourceProbe[];
  cells: CalendarLiveV6FixedCellValidation[];
  sceneProofs: CalendarLiveV6FixedSceneProof[];
...
export interface CalendarLiveV6FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V6_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV6FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV6FixedHardGates(
  input: CalendarLiveV6FixedHardGateInput,
): CalendarLiveV6FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV6FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V6_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV6FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV6FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
