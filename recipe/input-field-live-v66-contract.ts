...
} from "./input-field-live-v3-verifier-v66.js";
import {
  buildInputLiveV66RawPropertyRuntime,
  normalizeInputLiveV66Scene,
  type InputLiveV66RawNode,
} from "./input-field-live-v66-verifier.js";
...
export const INPUT_LIVE_V66_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V66_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V66_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V66_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V66_VARIANT_COUNT = 256;
export const INPUT_LIVE_V66_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V66_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV66SourceId = (typeof INPUT_LIVE_V66_SOURCE_IDS)[number];

export interface InputLiveV66SourceIdentity {
  source: InputLiveV66SourceId;
...
export interface InputLiveV66WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV66ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV66SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV66RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV66RootProof {
  source: InputLiveV66SourceId;
  adapterIdentity: string;
...
export interface InputLiveV66ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV66SourceId;
    adapterIdentity: string;
...
export interface InputLiveV66CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV66SourceId;
  adapterIdentity: string;
...
export interface InputLiveV66CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV66SourceId;
  strata: InputLiveV66CaptureCell["strata"];
...
export interface InputLiveV66CleanupPayload {
...
export function validateInputLiveV66WriterPayload(
  value: unknown,
): InputLiveV66WriterOwnership {
...
const identityPlan = (source: InputLiveV66SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV66ExtractProgram(
  writer: InputLiveV66WriterOwnership,
  sources: readonly InputLiveV66SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V66_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V66_NAMESPACE)}
${buildInputLiveV66RawPropertyRuntime()}
...
export function validateInputLiveV66ExtractPayload(
  value: unknown,
  writer: InputLiveV66WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V66_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV66ExtractPayload;
}

export function proveInputLiveV66Roots<Instance>(
  extract: InputLiveV66ExtractPayload,
  sources: readonly (InputLiveV66SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV66Scene(
      root.scene,
...
export function assertInputLiveV66RootProofs(
  proofs: readonly InputLiveV66RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV66ProbeProgram(
  writer: InputLiveV66WriterOwnership,
  sources: readonly InputLiveV66SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V66_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV66ProbePayload(
  value: unknown,
  writer: InputLiveV66WriterOwnership,
): InputLiveV66ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV66ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V66_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V66_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV66CaptureManifest(
  cells: readonly InputLiveV66CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V66_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V66_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V66_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V66_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V66_SOURCE_IDS.some(
...
export const inputLiveV66CaptureManifestSha256 = (
  cells: readonly InputLiveV66CaptureCell[],
): string => {
  validateInputLiveV66CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV66CaptureProgram(
  writer: InputLiveV66WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V66_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V66_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V66_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV66CapturePayload(
  value: unknown,
  cell: InputLiveV66CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV66CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V66_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V66_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV66CaptureResponses(
  manifest: readonly InputLiveV66CaptureCell[],
  responses: readonly InputLiveV66CapturePayload[],
): void {
  validateInputLiveV66CaptureManifest(manifest);
  if (
...
export interface InputLiveV66ObjectiveReport {
  artifactVersion: "input-live-v66-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV66Objective(
  manifest: readonly InputLiveV66CaptureCell[],
  responses: readonly InputLiveV66CapturePayload[],
  readReference: (cell: InputLiveV66CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV66CaptureCell) => Buffer,
): InputLiveV66ObjectiveReport {
  assertInputLiveV66CaptureResponses(manifest, responses);
...
export function buildInputLiveV66CleanupProgram(
  writer: InputLiveV66WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V66_NAMESPACE,
...
export function validateInputLiveV66CleanupPayload(
  value: unknown,
  writer: InputLiveV66WriterOwnership,
): InputLiveV66CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV66CleanupPayload;
...
export const INPUT_LIVE_V66_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV66WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
