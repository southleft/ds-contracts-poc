...
import { buildComboboxLiveV8CleanupRuntime } from "./combobox-live-v8-cleanup.js";
import {
  type ComboboxLiveV8FixedFixedPoint,
} from "./combobox-live-v8-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV8RestoreProgram,
  validateComboboxLiveV8RestorePayload,
  type ComboboxLiveV8RestorePayload,
} from "./combobox-live-v8-restore.js";
import {
  buildComboboxLiveV8RawPropertyRuntime,
  normalizeComboboxLiveV8Scene,
  type ComboboxLiveV8RawNode,
} from "./combobox-live-v8-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v8.js";
import {
...
export const COMBOBOX_LIVE_V8_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V8_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V8_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V8_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V8_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V8_SET_COUNT = 4;
export const COMBOBOX_LIVE_V8_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V8_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V8_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V8_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V8_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV8SourceId = (typeof COMBOBOX_LIVE_V8_SOURCE_IDS)[number];

export interface ComboboxLiveV8SourceIdentity {
  source: ComboboxLiveV8SourceId;
...
export interface ComboboxLiveV8WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV8ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV8SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV8RawNode;
    optionScene: ComboboxLiveV8RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV8RootProof {
  source: ComboboxLiveV8SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV8FixedFixedPoint;
}

export interface ComboboxLiveV8ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV8SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV8CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV8SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV8CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV8SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV8CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v8 ${label} must be an object`);
...
export function validateComboboxLiveV8WriterPayload(
  value: unknown,
): ComboboxLiveV8WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V8_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v8 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v8 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v8 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV8SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV8ExtractProgram(
  writer: ComboboxLiveV8WriterOwnership,
  sources: readonly ComboboxLiveV8SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v8 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V8_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V8_NAMESPACE)}
${buildComboboxLiveV8RawPropertyRuntime()}
...
export function validateComboboxLiveV8ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV8WriterOwnership,
): ComboboxLiveV8ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v8 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V8_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v8 extract schema/two-root mismatch");
  return payload as ComboboxLiveV8ExtractPayload;
}

export function proveComboboxLiveV8Roots<Instance>(
  extract: ComboboxLiveV8ExtractPayload,
  sources: readonly (ComboboxLiveV8SourceIdentity & {
...
): ComboboxLiveV8RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v8 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v8 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV8Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV8Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV8FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v8 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v8 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v8 compile lost owned sets");
...
export function assertComboboxLiveV8RootProofs(
  proofs: readonly ComboboxLiveV8RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v8 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v8 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV8ProbeProgram(
  writer: ComboboxLiveV8WriterOwnership,
  sources: readonly ComboboxLiveV8SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V8_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV8ProbePayload(
  value: unknown,
  writer: ComboboxLiveV8WriterOwnership,
): ComboboxLiveV8ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV8ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V8_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V8_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v8 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV8CaptureManifest(
  cells: readonly ComboboxLiveV8CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V8_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V8_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V8_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V8_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v8 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV8CaptureCell[],
): string => {
  validateComboboxLiveV8CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV8CaptureProgram(
  writer: ComboboxLiveV8WriterOwnership,
  cell: ComboboxLiveV8CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V8_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v8 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v8 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V8_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV8CapturePayload(
  value: unknown,
  cell: ComboboxLiveV8CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV8CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV8CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V8_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V8_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v8 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV8CaptureResponses(
  manifest: readonly ComboboxLiveV8CaptureCell[],
  responses: readonly ComboboxLiveV8CapturePayload[],
): void {
  validateComboboxLiveV8CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v8 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV8ObjectiveReport {
  artifactVersion: "combobox-live-v8-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV8Objective(
  manifest: readonly ComboboxLiveV8CaptureCell[],
  responses: readonly ComboboxLiveV8CapturePayload[],
): ComboboxLiveV8ObjectiveReport {
  assertComboboxLiveV8CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v8-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV8CleanupProgram(
  writer: ComboboxLiveV8WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV8CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V8_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v8 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV8CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV8WriterOwnership,
): ComboboxLiveV8CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV8CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v8 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V8_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV8WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
