...
} from "./input-field-live-v3-verifier-v25.js";
import {
  buildInputLiveV25RawPropertyRuntime,
  normalizeInputLiveV25Scene,
  type InputLiveV25RawNode,
} from "./input-field-live-v25-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v25.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v25.js";
...
export const INPUT_LIVE_V25_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V25_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V25_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V25_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V25_VARIANT_COUNT = 256;
export const INPUT_LIVE_V25_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V25_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV25SourceId = (typeof INPUT_LIVE_V25_SOURCE_IDS)[number];

export interface InputLiveV25SourceIdentity {
  source: InputLiveV25SourceId;
...
export interface InputLiveV25WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV25ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV25SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV25RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV25RootProof {
  source: InputLiveV25SourceId;
  adapterIdentity: string;
...
export interface InputLiveV25ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV25SourceId;
    adapterIdentity: string;
...
export interface InputLiveV25CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV25SourceId;
  adapterIdentity: string;
...
export interface InputLiveV25CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV25SourceId;
  strata: InputLiveV25CaptureCell["strata"];
...
export interface InputLiveV25CleanupPayload {
...
export function validateInputLiveV25WriterPayload(
  value: unknown,
): InputLiveV25WriterOwnership {
...
const identityPlan = (source: InputLiveV25SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV25ExtractProgram(
  writer: InputLiveV25WriterOwnership,
  sources: readonly InputLiveV25SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V25_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V25_NAMESPACE)}
${buildInputLiveV25RawPropertyRuntime()}
...
export function validateInputLiveV25ExtractPayload(
  value: unknown,
  writer: InputLiveV25WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V25_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV25ExtractPayload;
}

export function proveInputLiveV25Roots<Instance>(
  extract: InputLiveV25ExtractPayload,
  sources: readonly (InputLiveV25SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV25Scene(
      root.scene,
...
export function assertInputLiveV25RootProofs(
  proofs: readonly InputLiveV25RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV25ProbeProgram(
  writer: InputLiveV25WriterOwnership,
  sources: readonly InputLiveV25SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V25_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV25ProbePayload(
  value: unknown,
  writer: InputLiveV25WriterOwnership,
): InputLiveV25ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV25ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V25_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V25_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV25CaptureManifest(
  cells: readonly InputLiveV25CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V25_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V25_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V25_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V25_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V25_SOURCE_IDS.some(
...
export const inputLiveV25CaptureManifestSha256 = (
  cells: readonly InputLiveV25CaptureCell[],
): string => {
  validateInputLiveV25CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV25CaptureProgram(
  writer: InputLiveV25WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V25_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V25_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V25_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV25CapturePayload(
  value: unknown,
  cell: InputLiveV25CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV25CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V25_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V25_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV25CaptureResponses(
  manifest: readonly InputLiveV25CaptureCell[],
  responses: readonly InputLiveV25CapturePayload[],
): void {
  validateInputLiveV25CaptureManifest(manifest);
  if (
...
export interface InputLiveV25ObjectiveReport {
  artifactVersion: "input-live-v25-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV25Objective(
  manifest: readonly InputLiveV25CaptureCell[],
  responses: readonly InputLiveV25CapturePayload[],
  readReference: (cell: InputLiveV25CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV25CaptureCell) => Buffer,
): InputLiveV25ObjectiveReport {
  assertInputLiveV25CaptureResponses(manifest, responses);
...
export function buildInputLiveV25CleanupProgram(
  writer: InputLiveV25WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V25_NAMESPACE,
...
export function validateInputLiveV25CleanupPayload(
  value: unknown,
  writer: InputLiveV25WriterOwnership,
): InputLiveV25CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV25CleanupPayload;
...
export const INPUT_LIVE_V25_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV25WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
