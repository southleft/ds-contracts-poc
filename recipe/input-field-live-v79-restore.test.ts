...
import {
  INPUT_LIVE_V79_MEASURE_VISIBLE_FILL_MARKER,
  INPUT_LIVE_V79_RESTORE_CONTENT_ROLES,
  INPUT_LIVE_V79_RESTORE_PARENT_ROLES,
...
test("v79 probe reveals hidden content FILL before measuring, then restores visibility", () => {
  const program = readFileSync(
    "recipe/input-field-live-v79-contract.ts",
    "utf8",
  );
  const probe = program.slice(
    program.indexOf("export function buildInputLiveV79ProbeProgram"),
  );
...
test("v16 restore still measures FILL while visible, then restores visibility", () => {
  const program = buildInputLiveV79RestoreProgram(writer);
  assert.match(program, new RegExp(INPUT_LIVE_V79_MEASURE_VISIBLE_FILL_MARKER));
  ...
  assert.match(program, /INPUT-V79-RESTORE-COUNT/);
  assert.match(program, /INPUT-V79-RESTORE-NOT-FILL/);
  ...
  for (const role of INPUT_LIVE_V79_RESTORE_CONTENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
  for (const role of INPUT_LIVE_V79_RESTORE_PARENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
});

test("v16 restore payload requires 256 filled content texts", () => {
  const accepted = validateInputLiveV79RestorePayload(
    {
      pageId: "1:1",
      setIds: ["2:2", "2:1"],
      restoredCount: 256,
      ...
      marker: INPUT_LIVE_V79_MEASURE_VISIBLE_FILL_MARKER,
    },
    writer,
  );
...
test("v16 writer bytes stay frozen and v17 writer assigns set HUG", () => {
  ...
  const v76Program = readFileSync(
    "recipe/evidence/input-field-live-pivot-v79/programs/writer.txt",
  );
  const v76Payload = readFileSync(
    "recipe/evidence/input-field-live-pivot-v79/programs/writer-payload.js",
  );
...
test("v79 probe excludes overlay-label AABB from visibleAreaLoss; overlap still uses full semantic", () => {
  const program = readFileSync(
    "recipe/input-field-live-v79-contract.ts",
    "utf8",
  );
...
test("v19 writer preserves hidden FILL occupancy; hashed v16/v17/v18 stay frozen", () => {
  ...
  assert.match(v79Payload, /INPUT-WRITER-HIDDEN-FILL-OCCUPANCY/);
  assert.equal(
    v79Payload.includes(
  ...
  assert.equal(sha256(v79Payload), V19_WRITER_PAYLOAD_SHA256);
  assert.equal(
    sha256(
      readFileSync(
        "recipe/evidence/input-field-live-pivot-v79/programs/writer.txt",
