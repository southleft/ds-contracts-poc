...
import {
  assertInputLiveV12RootProofs,
  INPUT_LIVE_V12_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V12_RESPONSE_CONTRACTS,
  type InputLiveV12RootProof,
} from "./input-field-live-v12-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V12_BROKER_VERSION = "input-live-v12-broker-v2";
export const INPUT_LIVE_V12_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V12_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V12_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V12_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V12_REMOTE_PHASES = [
  "writer",
  "extract",
  "probe",
  "capture",
...
export const INPUT_LIVE_V12_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV12RemotePhase =
  (typeof INPUT_LIVE_V12_REMOTE_PHASES)[number];
export type InputLiveV12AuthorizationMode = "live" | "simulated";

export interface InputLiveV12CommitPins {
...
export interface InputLiveV12TransactionAuthorization extends InputLiveV12CommitPins {
  mode: InputLiveV12AuthorizationMode;
...
export interface InputLiveV12Manifest {
  artifactVersion: "input-live-v12-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V12_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V12_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V12_TARGET;
  authorization: InputLiveV12TransactionAuthorization;
...
export interface InputLiveV12ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV12RequestBody {
  artifactVersion: "input-live-v12-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V12_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV12RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV12CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V12_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV12ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV12Request extends InputLiveV12RequestBody {
...
export interface InputLiveV12Receipt {
  artifactVersion: "input-live-v12-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V12_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV12RemotePhase;
...
export interface InputLiveV12AcceptedResponse<T = unknown> {
  request: InputLiveV12Request;
  receipt: InputLiveV12Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV12IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV12ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V12_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V12_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V12_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V12_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V12_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V12_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V12_TARGET.editorType;
};
...
export const inputLiveV12RequestSequence = (
  phase: InputLiveV12RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV12RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV12RequestPath = (
  directory: string,
  phase: InputLiveV12RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV12ResponsePath = (
  directory: string,
  phase: InputLiveV12RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV12ReceiptPath = (
  directory: string,
  phase: InputLiveV12RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV12Manifest => {
  const manifest = json<InputLiveV12Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v12-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V12_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V12_DYNAMIC_TOOL) ||
...
const unsignedBody = (request: InputLiveV12Request): InputLiveV12RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV12RemotePhase,
  captureIndex?: number,
): InputLiveV12Request | undefined => {
  const file = inputLiveV12RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV12Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV12RemotePhase,
  captureIndex?: number,
): InputLiveV12Receipt | undefined => {
  const file = inputLiveV12ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV12Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV12Request | undefined;
  receipt: InputLiveV12Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV12RemotePhase,
): InputLiveV12ResponseContract => ({
  ...INPUT_LIVE_V12_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V12_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v12-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V12_BROKER_VERSION,
  target: INPUT_LIVE_V12_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V12_PROGRAM_START*/${program}/*INPUT_LIVE_V12_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V12_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V12_PROGRAM_END*/";
...
export function createInputLiveV12Transaction(
  directory: string,
  ...
  const manifest: InputLiveV12Manifest = {
    artifactVersion: "input-live-v12-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V12_BROKER_VERSION,
...
export function validateInputLiveV12Request(
  directory: string,
  request: InputLiveV12Request,
): void {
  ...
  const expectedSequence = inputLiveV12RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v12-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V12_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V12_TARGET.fileKey ||
...
export function issueInputLiveV12Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV12RemotePhase,
  program: string,
  options: InputLiveV12IssueOptions = {},
): InputLiveV12Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV12RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV12Request>(targetPath);
    validateInputLiveV12Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV12ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV12TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV12RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V12_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V12_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V12_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV12RequestBody = {
    artifactVersion: "input-live-v12-broker-request-v2",
    brokerVersion: INPUT_LIVE_V12_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV12Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV12Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV12RawResponse(
  directory: string,
  phase: InputLiveV12RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV12RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV12Request>(requestFile);
  if (
  ...
  const target = inputLiveV12ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV12Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v12-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V12_BROKER_VERSION,
...
export function acceptInputLiveV12Response<T = unknown>(
  directory: string,
  phase: InputLiveV12RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV12AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV12ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV12Request>(
    inputLiveV12RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV12Request(directory, request);
  if (phase === "capture") validateInputLiveV12TechnicalGates(directory);
  const rawFile = inputLiveV12ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v12-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V12_BROKER_VERSION ||
...
export function persistInputLiveV12TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV12RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV12RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V12_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v12-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V12_BROKER_VERSION,
...
export function validateInputLiveV12TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v12-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V12_TECHNICAL_GATES.some(
...
export function validateInputLiveV12Receipt(
  directory: string,
  phase: InputLiveV12RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV12Request(directory, request);
  const receipt = json<InputLiveV12Receipt>(
    inputLiveV12ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV12ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v12-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V12_BROKER_VERSION ||
...
export function readInputLiveV12AcceptedRawPayload(
  directory: string,
  phase: InputLiveV12RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV12Receipt(directory, phase, captureIndex);
...
export function createInputLiveV12FakeDynamicResponse(
  request: InputLiveV12Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V12_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V12_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV12PersistedRequests(
  directory: string,
): InputLiveV12Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV12Request>(file)] : [];
    });
}

export const inputLiveV12Sha256 = sha256;
