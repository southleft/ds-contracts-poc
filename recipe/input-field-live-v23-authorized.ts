...
import { INPUT_LIVE_V23_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v23.js";
import { verifyInputLiveV23Authorization } from "./input-field-live-v23-authorization.js";
import { runInputLiveV23Preflight } from "./input-field-live-v23-preflight.js";
import { InputLiveV23Orchestrator } from "./run-input-field-live-v23.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V23_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v23-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV23Authorization(root);
    runInputLiveV23Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV23Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
