...
import { verifyTableLiveV35Authorization } from "./table-live-v35-authorization.js";
import { TABLE_LIVE_V35_TARGET } from "./table-live-v35-broker.js";
import {
  TABLE_LIVE_V35_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV35Authorization();
...
const attestation: TableLiveV35SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV35SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v35 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V35_SECURITY_ATTESTATION_DEFAULT_PATH,
);
