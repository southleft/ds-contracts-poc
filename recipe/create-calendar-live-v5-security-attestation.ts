...
import { verifyCalendarLiveV5Authorization } from "./calendar-live-v5-authorization.js";
import { CALENDAR_LIVE_V5_TARGET } from "./calendar-live-v5-broker.js";
import {
  CALENDAR_LIVE_V5_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV5Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v5-session";
...
const attestation: CalendarLiveV5SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV5SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v5 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V5_SECURITY_ATTESTATION_DEFAULT_PATH,
);
