...
import { TABLE_LIVE_V14_AUTHORIZATION_PATH } from "./build-table-live-proof-v14.js";
import { verifyTableLiveV14Authorization } from "./table-live-v14-authorization.js";
import { runTableLiveV14Preflight } from "./table-live-v14-preflight.js";
import { TableLiveV14Orchestrator } from "./run-table-live-v14.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v14 requires exact authorization artifact ${TABLE_LIVE_V14_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v14-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV14Authorization(root);
    runTableLiveV14Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV14Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV14Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
