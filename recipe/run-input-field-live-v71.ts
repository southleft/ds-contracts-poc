...
import {
  acceptInputLiveV71Response,
  createInputLiveV71Transaction,
  inputLiveV71ReceiptPath,
  inputLiveV71RequestPath,
  inputLiveV71ResponsePath,
  inputLiveV71Sha256,
  issueInputLiveV71Request,
  persistInputLiveV71RawResponse,
  persistInputLiveV71TechnicalGates,
  readInputLiveV71AcceptedRawPayload,
  type InputLiveV71Request,
  type InputLiveV71TransactionAuthorization,
} from "./input-field-live-v71-broker.js";
import {
  INPUT_LIVE_V71_CAPTURE_COUNT,
  assertInputLiveV71CaptureResponses,
  assertInputLiveV71RootProofs,
  buildInputLiveV71CaptureProgram,
  buildInputLiveV71CleanupProgram,
  buildInputLiveV71ExtractProgram,
  buildInputLiveV71ProbeProgram,
  buildInputLiveV71RestoreProgram,
  validateInputLiveV71RestorePayload,
  evaluateInputLiveV71Objective,
  inputLiveV71CaptureManifestSha256,
  proveInputLiveV71Roots,
  validateInputLiveV71CaptureManifest,
  validateInputLiveV71CapturePayload,
  validateInputLiveV71CleanupPayload,
  validateInputLiveV71ExtractPayload,
  validateInputLiveV71ProbePayload,
  validateInputLiveV71WriterPayload,
  type InputLiveV71CaptureCell,
  type InputLiveV71CapturePayload,
...
import {
  ...
  INPUT_LIVE_V71_PLAN_PATH,
  type InputLiveV71ProofPlan,
...
import type { ExpectedScenePlan } from "./scene-readback-v71.js";

export const INPUT_LIVE_V71_RUNNER_VERSION = "input-live-v71-runner-v1";

interface InputLiveV71JournalEntry {
  index: number;
  event: string;
...
interface InputLiveV71OrchestratorState {
  artifactVersion: "input-live-v71-orchestrator-state-v1";
  ...
  journal: InputLiveV71JournalEntry[];
}

interface LoadedProofInputs {
  plan: InputLiveV71ProofPlan;
  planSha256: string;
  captures: InputLiveV71CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    InputLiveV71SourceIdentity & {
...
export interface InputLiveV71NextAction {
...
const writeState = (
  directory: string,
  state: InputLiveV71OrchestratorState,
): void => {
...
const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, INPUT_LIVE_V71_PLAN_PATH);
  const capturePath = path.join(root, INPUT_LIVE_V71_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as InputLiveV71ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as InputLiveV71CaptureCell[];
  validateInputLiveV71CaptureManifest(captures);
  if (
    plan.captureManifest.sha256 !== inputLiveV71Sha256(captureBytes) ||
    captureArtifact.cellsSha256 !== inputLiveV71CaptureManifestSha256(captures)
  )
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (
      inputLiveV71Sha256(expectedBytes) !== source.expectedScenePlan.sha256 ||
      inputLiveV71Sha256(uncompressed) !==
  ...
  return {
    plan,
    planSha256: inputLiveV71Sha256(planBytes),
    captures,
    captureManifestSha256: inputLiveV71Sha256(captureBytes),
    sources,
...
const appendJournal = (
  ...
  const payloadSha256 = inputLiveV71Sha256(canonicalJson(payload));
  const body = {
  ...
  const entry = {
    ...body,
    entrySha256: inputLiveV71Sha256(canonicalJson(body)),
  };
  writeNew(
    path.join(
      directory,
...
const writerOwnership = (directory: string): InputLiveV71WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): InputLiveV71RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): InputLiveV71Request =>
  readJson<InputLiveV71Request>(file);

export class InputLiveV71Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueInputLiveV71Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: InputLiveV71OrchestratorState = {
      artifactVersion: "input-live-v71-orchestrator-state-v1",
      transactionDirectory: options.transactionDirectory,
      activeRequest: inputLiveV71RequestPath(
        options.transactionDirectory,
        "writer",
      ),
      cleanupRequest: null,
    ...
    return new InputLiveV71Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<InputLiveV71OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new InputLiveV71Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): InputLiveV71NextAction {
    const cleanup =
      this.#state.cleanupRequest &&
      !this.#state.cleanupAccepted &&
      !existsSync(inputLiveV71ReceiptPath(this.#directory, "cleanup"))
        ? this.#state.cleanupRequest
        : null;
  ...
  ingestAndAdvance(rawResponse: string): InputLiveV71NextAction {
  ...
  ingestCleanup(rawResponse: string): InputLiveV71NextAction {
    ...
    persistInputLiveV71RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptInputLiveV71Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV71CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      existsSync(inputLiveV71ReceiptPath(this.#directory, "writer")) &&
      !existsSync(path.join(this.#directory, OWNERSHIP_FILE))
    ) {
      const raw = readInputLiveV71AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateInputLiveV71WriterPayload(raw),
      );
    }
    if (existsSync(inputLiveV71RequestPath(this.#directory, "cleanup")))
      this.#state.cleanupRequest = inputLiveV71RequestPath(
        this.#directory,
        "cleanup",
      );
    if (
      existsSync(path.join(this.#directory, OWNERSHIP_FILE)) &&
      !existsSync(inputLiveV71RequestPath(this.#directory, "cleanup"))
    )
      this.#issueCleanupAndRestore(false);
    else if (
      existsSync(inputLiveV71RequestPath(this.#directory, "cleanup")) &&
      !existsSync(inputLiveV71RequestPath(this.#directory, "restore"))
    )
      this.#issueRestore(false);
    if (
      existsSync(inputLiveV71RequestPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV71ReceiptPath(this.#directory, "restore"))
    )
      this.#state.activeRequest = inputLiveV71RequestPath(
        this.#directory,
        "restore",
      );
    if (
      existsSync(inputLiveV71ReceiptPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV71RequestPath(this.#directory, "extract"))
    ) {
      const ownership = writerOwnership(this.#directory);
      const extract = issueInputLiveV71Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildInputLiveV71ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV71RequestPath(
        this.#directory,
        "extract",
    ...
    if (existsSync(inputLiveV71ReceiptPath(this.#directory, "cleanup")))
      this.#state.cleanupAccepted = true;
    this.#persist();
  }

  #acceptActive(request: InputLiveV71Request): void {
    if (request.phase === "writer") {
      const accepted = acceptInputLiveV71Response(this.#directory, "writer", {
        validate: validateInputLiveV71WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptInputLiveV71Response(this.#directory, "restore", {
        validate: (payload) =>
          validateInputLiveV71RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptInputLiveV71Response(this.#directory, "extract", {
        validate: (payload) =>
          validateInputLiveV71ExtractPayload(payload, ownership),
      });
      const proofs = proveInputLiveV71Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseInputFieldRecipe,
        compileInputFieldRecipe,
      );
      assertInputLiveV71RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "input-live-v71-host-root-proofs-v1",
      ...
      appendJournal(
        ...
        request.requestId,
        { proofsSha256: inputLiveV71Sha256(canonicalJson(proofs)) },
      );
      const next = issueInputLiveV71Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildInputLiveV71ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV71RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptInputLiveV71Response(this.#directory, "probe", {
        validate: (payload) =>
          validateInputLiveV71ProbePayload(payload, ownership),
      });
      persistInputLiveV71TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      const accepted = acceptInputLiveV71Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateInputLiveV71CapturePayload(payload, cell, rawBytes),
      });
      const png = Buffer.from(accepted.payload.pngBase64, "base64");
      const capturePath = path.join(
        this.#directory,
        "captures",
        `${String(cell.index).padStart(3, "0")}-${inputLiveV71Sha256(cell.cellKey).slice(0, 20)}.png`,
      );
      ...
      if (next < INPUT_LIVE_V71_CAPTURE_COUNT) this.#issueCapture(next);
      else {
          ...
          pngBase64: Buffer.from(
            readFileSync(
              path.join(
                this.#directory,
                "captures",
                `${String(response.index).padStart(3, "0")}-${inputLiveV71Sha256(response.cellKey).slice(0, 20)}.png`,
              ),
            ),
          ).toString("base64"),
        }));
        assertInputLiveV71CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateInputLiveV71Objective(
        ...
        appendJournal(
          ...
          request.requestId,
          {
            reportSha256: inputLiveV71Sha256(canonicalJson(objective)),
  ...
  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueInputLiveV71Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildInputLiveV71CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = inputLiveV71RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueInputLiveV71Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildInputLiveV71RestoreProgram(ownership),
    );
    this.#state.activeRequest = inputLiveV71RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueInputLiveV71Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildInputLiveV71CaptureProgram(
  ...
  #readCaptureResponseIndex(): InputLiveV71CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: InputLiveV71CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedInputLiveV71Authorization = (
  privateKey: KeyObject,
): InputLiveV71TransactionAuthorization => ({
  mode: "simulated",
  ...
  signingPublicKeySha256: inputLiveV71Sha256(
...
export const generateInputLiveV71SimulationKey = (): KeyObject =>
