...
import { TABLE_LIVE_V31_INDEX_PATH } from "./build-table-live-proof-v31.js";
import {
...
import {
  buildTableLiveV31AuthorizationArtifact,
  validateTableLiveV31AntecedentIndex,
  validateTableLiveV31AuthorizationArtifact,
  validateTableLiveV31History,
  validateTableLiveV31UnitTestSource,
  type TableLiveV31AntecedentIndex,
  type TableLiveV31AuthorizationArtifact,
  type TableLiveV31AuthorizationProof,
  type TableLiveV31HistoryState,
} from "./table-live-v31-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV31ControlFlowSource,
  validateTableLiveV31SecurityAttestation,
  type TableLiveV31SecurityAttestation,
} from "./table-live-v31-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V31_INDEX_PATH);
const index = (): TableLiveV31AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV31AntecedentIndex;

const artifact = (): TableLiveV31AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV31AuthorizationArtifact({
...
const pendingState = (): TableLiveV31HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV31HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV31AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v31/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV31AuthorizationArtifact,
): TableLiveV31SecurityAttestation => {
  const body: Omit<TableLiveV31SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V31_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV31History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV31History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV31History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV31History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV31History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV31UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v31-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV31UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV31AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV31AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV31AuthorizationArtifact(
        planted as TableLiveV31AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV31SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV31SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV31SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v31-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV31ControlFlowSource(source), []);
});
