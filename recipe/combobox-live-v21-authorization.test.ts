...
import { COMBOBOX_LIVE_V21_INDEX_PATH } from "./build-combobox-live-proof-v21.js";
import {
...
import {
  ...
  validateComboboxLiveV21UnitTestSource,
  type ComboboxLiveV21AntecedentIndex,
  type ComboboxLiveV21AuthorizationArtifact,
...
import {
  ...
  validateComboboxLiveV21ControlFlowSource,
  validateComboboxLiveV21SecurityAttestation,
  type ComboboxLiveV21SecurityAttestation,
} from "./combobox-live-v21-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v21-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V21_INDEX_PATH);
const index = (): ComboboxLiveV21AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV21AntecedentIndex;

const artifact = (): ComboboxLiveV21AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV21AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV21HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV21HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV21AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v21/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV21AuthorizationArtifact,
): ComboboxLiveV21SecurityAttestation => {
  const body: Omit<ComboboxLiveV21SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V21_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV21History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV21History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV21History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV21History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV21History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV21UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV21UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV21AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV21AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV21AuthorizationArtifact(
        planted as ComboboxLiveV21AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV21SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV21SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV21SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV21ControlFlowSource(source), []);
});
