...
import { calendarLiveV1Sha256 } from "./calendar-live-v28-broker.js";

export interface CalendarLiveV28LifecycleSimulationReport {
  artifactVersion: "calendar-live-v28-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV28AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV28Lifecycle(): Promise<CalendarLiveV28LifecycleSimulationReport> {
  await buildCalendarLiveV28Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V28_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV28AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v28-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V28_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V28_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV28History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV28History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV28AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V28_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V28_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV28History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV28History(authorized, "pending");
    ...
    await buildCalendarLiveV28Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V28_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV28AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV28Lifecycle(), null, 2)}\n`,
  );
