...
import { COMBOBOX_LIVE_V12_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v12.js";
import { verifyComboboxLiveV12Authorization } from "./combobox-live-v12-authorization.js";
import { runComboboxLiveV12Preflight } from "./combobox-live-v12-preflight.js";
import { ComboboxLiveV12Orchestrator } from "./run-combobox-live-v12.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v12 requires exact authorization artifact ${COMBOBOX_LIVE_V12_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v12-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV12Authorization(root);
    runComboboxLiveV12Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV12Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV12Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
