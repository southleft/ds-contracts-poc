...
import {
  ...
  INPUT_LIVE_V79_INDEX_PATH,
} from "./build-input-field-live-proof-v79.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV79HistoryExpectation = "pending" | "authorized";

export interface InputLiveV79AuthorizationArtifact {
  artifactVersion: "input-live-v79-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V79_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV79AntecedentIndex {
...
export interface InputLiveV79HistoryState {
  index?: InputLiveV79AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV79AuthorizationArtifact;
...
export interface InputLiveV79AuthorizationProof extends InputLiveV79TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V79_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V79_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V79_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V79_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v79-authorization") ||
  artifactPath.includes("input-field-live-v79-preflight") ||
  artifactPath.includes("input-field-live-v79-authorized") ||
  artifactPath.includes("input-field-live-pivot-v79-status") ||
...
export function validateInputLiveV79AntecedentIndex(
  index: InputLiveV79AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v79-antecedent-index-v1" ||
...
export function validateInputLiveV79UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV79History\(\s*["']/.test(source) ||
    /readInputLiveV79HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV79AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV79AuthorizationArtifact {
  ...
  ) as InputLiveV79AntecedentIndex;
  const indexFailures = validateInputLiveV79AntecedentIndex(index);
...
export function validateInputLiveV79AuthorizationArtifact(
  artifact: InputLiveV79AuthorizationArtifact | undefined,
  index: InputLiveV79AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v79-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v79" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V79_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V79_TARGET) ||
    !equalJson(
...
export function validateInputLiveV79History(
  state: InputLiveV79HistoryState,
  expected: InputLiveV79HistoryExpectation,
): string[] {
  const failures = validateInputLiveV79AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV79AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV79HistoryState(
  root = process.cwd(),
): InputLiveV79HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V79_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV79AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V79_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V79_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V79_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV79History(
  expected: InputLiveV79HistoryExpectation,
  root = process.cwd(),
): InputLiveV79HistoryState {
  const state = readInputLiveV79HistoryState(root);
  const failures = validateInputLiveV79History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV79Authorization(
  root = process.cwd(),
): InputLiveV79AuthorizationProof {
  const state = verifyInputLiveV79History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V79_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v79/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v79.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V79_TARGET,
    expectedDynamicTool: INPUT_LIVE_V79_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V79_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV79HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV79History(expected);
