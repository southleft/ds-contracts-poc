...
import { calendarLiveV1Sha256 } from "./calendar-live-v6-broker.js";

export interface CalendarLiveV6LifecycleSimulationReport {
  artifactVersion: "calendar-live-v6-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV6AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV6Lifecycle(): Promise<CalendarLiveV6LifecycleSimulationReport> {
  await buildCalendarLiveV6Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V6_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV6AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v6-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V6_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V6_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV6History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV6History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV6AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V6_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V6_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV6History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV6History(authorized, "pending");
    ...
    await buildCalendarLiveV6Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V6_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV6AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV6Lifecycle(), null, 2)}\n`,
  );
