...
import { buildCalendarLiveV20CleanupRuntime } from "./calendar-live-v20-cleanup.js";
import { type CalendarLiveV20FixedFixedPoint } from "./calendar-live-v20-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV20RestoreProgram,
  validateCalendarLiveV20RestorePayload,
  type CalendarLiveV20RestorePayload,
} from "./calendar-live-v20-restore.js";
import {
  buildCalendarLiveV20RawPropertyRuntime,
  normalizeCalendarLiveV20Scene,
  type CalendarLiveV20RawNode,
} from "./calendar-live-v20-verifier.js";
...
export const CALENDAR_LIVE_V20_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V20_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V20_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V20_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V20_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V20_SET_COUNT = 3;
export const CALENDAR_LIVE_V20_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V20_HOST_PHASES = 3;
export const CALENDAR_LIVE_V20_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V20_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V20_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V20_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV20SourceId = (typeof CALENDAR_LIVE_V20_SOURCE_IDS)[number];
export type CalendarLiveV20Kind = "calendar" | "week" | "day";

export interface CalendarLiveV20SourceIdentity {
  source: CalendarLiveV20SourceId;
...
export interface CalendarLiveV20WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV20ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV20SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV20RawNode;
    weekScene: CalendarLiveV20RawNode;
    dayScene: CalendarLiveV20RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV20RootProof {
  source: CalendarLiveV20SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV20FixedFixedPoint;
}

export interface CalendarLiveV20ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV20SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV20CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV20SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV20CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV20SourceId;
  frameWidth: number;
...
export interface CalendarLiveV20CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v20 ${label} must be an object`);
...
export function validateCalendarLiveV20WriterPayload(
  value: unknown,
): CalendarLiveV20WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v20") ||
    ...
    result.namespace !== CALENDAR_LIVE_V20_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v20 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v20 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v20 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV20SourceIdentity,
  kind: CalendarLiveV20Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV20ExtractProgram(
  writer: CalendarLiveV20WriterOwnership,
  sources: readonly CalendarLiveV20SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v20 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V20_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V20_NAMESPACE)}
${buildCalendarLiveV20RawPropertyRuntime()}
...
export function validateCalendarLiveV20ExtractPayload(
  value: unknown,
  writer: CalendarLiveV20WriterOwnership,
): CalendarLiveV20ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v20 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V20_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v20 extract schema/one-root mismatch");
  return payload as CalendarLiveV20ExtractPayload;
}

export function proveCalendarLiveV20Roots<Instance>(
  extract: CalendarLiveV20ExtractPayload,
  sources: readonly (CalendarLiveV20SourceIdentity & {
...
): CalendarLiveV20RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v20 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v20 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV20Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV20Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV20Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV20FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v20 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v20 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v20 compile lost owned sets");
...
export function assertCalendarLiveV20RootProofs(
  proofs: readonly CalendarLiveV20RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v20 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v20 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV20ProbeProgram(
  writer: CalendarLiveV20WriterOwnership,
  sources: readonly CalendarLiveV20SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V20_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV20ProbePayload(
  value: unknown,
  writer: CalendarLiveV20WriterOwnership,
): CalendarLiveV20ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV20ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V20_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V20_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v20 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV20CaptureManifest(
  cells: readonly CalendarLiveV20CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V20_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V20_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V20_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V20_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v20 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV20CaptureCell[],
): string => {
  validateCalendarLiveV20CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV20CaptureProgram(
  writer: CalendarLiveV20WriterOwnership,
  cell: CalendarLiveV20CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V20_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v20 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v20 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V20_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV20CapturePayload(
  value: unknown,
  cell: CalendarLiveV20CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV20CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV20CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V20_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V20_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v20 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV20CaptureResponses(
  manifest: readonly CalendarLiveV20CaptureCell[],
  responses: readonly CalendarLiveV20CapturePayload[],
): void {
  validateCalendarLiveV20CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v20 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV20ObjectiveReport {
  artifactVersion: "calendar-live-v20-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV20Objective(
  manifest: readonly CalendarLiveV20CaptureCell[],
  responses: readonly CalendarLiveV20CapturePayload[],
): CalendarLiveV20ObjectiveReport {
  assertCalendarLiveV20CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v20-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV20CleanupProgram(
  writer: CalendarLiveV20WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV20CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V20_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v20 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV20CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV20WriterOwnership,
): CalendarLiveV20CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV20CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v20 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V20_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV20WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
