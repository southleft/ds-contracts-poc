...
import { CALENDAR_LIVE_V14_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v14.js";
import { verifyCalendarLiveV14Authorization } from "./calendar-live-v14-authorization.js";
import { runCalendarLiveV14Preflight } from "./calendar-live-v14-preflight.js";
import { CalendarLiveV14Orchestrator } from "./run-calendar-live-v14.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v14 requires exact authorization artifact ${CALENDAR_LIVE_V14_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v14-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV14Authorization(root);
    runCalendarLiveV14Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV14Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV14Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
