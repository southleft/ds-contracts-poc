...
import { INPUT_LIVE_V85_INDEX_PATH } from "./build-input-field-live-proof-v85.js";
import {
  INPUT_LIVE_V85_DYNAMIC_TOOL,
  INPUT_LIVE_V85_TARGET,
} from "./input-field-live-v85-broker.js";
import {
  buildInputLiveV85AuthorizationArtifact,
  validateInputLiveV85AntecedentIndex,
  validateInputLiveV85AuthorizationArtifact,
  validateInputLiveV85History,
  validateInputLiveV85UnitTestSource,
  type InputLiveV85AntecedentIndex,
  type InputLiveV85AuthorizationArtifact,
  type InputLiveV85AuthorizationProof,
  type InputLiveV85HistoryState,
} from "./input-field-live-v85-authorization.js";
import {
  inputLiveV85AttestationSha256,
  validateInputLiveV85ControlFlowSource,
  validateInputLiveV85SecurityAttestation,
  type InputLiveV85SecurityAttestation,
} from "./input-field-live-v85-preflight.js";
import { inputLiveV85Sha256 } from "./input-field-live-v85-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V85_INDEX_PATH);
const index = (): InputLiveV85AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV85AntecedentIndex;

const artifact = (): InputLiveV85AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV85AuthorizationArtifact({
...
const pendingState = (): InputLiveV85HistoryState => ({
  index: index(),
  indexSha256: inputLiveV85Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV85HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV85AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV85Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V85_TARGET,
  expectedDynamicTool: INPUT_LIVE_V85_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v85/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV85AuthorizationArtifact,
): InputLiveV85SecurityAttestation => {
  const body: Omit<InputLiveV85SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v85-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V85_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV85AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV85History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV85History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV85History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV85History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV85History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV85UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV85UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV85AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV85Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV85AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV85AuthorizationArtifact(
        planted as InputLiveV85AuthorizationArtifact,
        index(),
        inputLiveV85Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV85SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV85AttestationSha256(
      body as Omit<InputLiveV85SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV85SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV85ControlFlowSource(source), []);
});
