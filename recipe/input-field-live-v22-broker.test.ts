...
import {
  ...
  createInputLiveV22FakeDynamicResponse,
  ...
  listInputLiveV22PersistedRequests,
  persistInputLiveV22RawResponse,
  persistInputLiveV22TechnicalGates,
  validateInputLiveV22Receipt,
  validateInputLiveV22Request,
  type InputLiveV22RemotePhase,
...
import {
  ...
  type InputLiveV22ExtractPayload,
  type InputLiveV22ProbePayload,
  type InputLiveV22RestorePayload,
  type InputLiveV22RootProof,
  type InputLiveV22WriterOwnership,
} from "./input-field-live-v22-contract.js";
import {
  InputLiveV22Orchestrator,
  simulatedInputLiveV22Authorization,
} from "./run-input-field-live-v22.js";
...
const captures: InputLiveV22CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV22Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v22-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV22Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV22CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV22FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV22FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V22_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V22_TARGET);
  persistInputLiveV22RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV22Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV22Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV22WriterOwnership,
...
const rootProofs = (): InputLiveV22RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV22CaptureCell,
): InputLiveV22CapturePayload => {
  const bytes = Buffer.from(`input-live-v22-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV22Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV22Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV22RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV22RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV22ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV22ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV22CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV22CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV22CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV22Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV22CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV22PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV22ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV22Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV22Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV22Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV22Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V22_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV22RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V22_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV22Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV22Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV22ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV22Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV22WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV22RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV22Request;
    validateInputLiveV22Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV22Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV22CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v22-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV22Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV22Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV22Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V22_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV22RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V22_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV22Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV22Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV22RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V22_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV22Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV22Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV22Request;
