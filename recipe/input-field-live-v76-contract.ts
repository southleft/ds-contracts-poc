...
} from "./input-field-live-v3-verifier-v76.js";
import {
  buildInputLiveV76RawPropertyRuntime,
  normalizeInputLiveV76Scene,
  type InputLiveV76RawNode,
} from "./input-field-live-v76-verifier.js";
...
export {
  buildInputLiveV76RestoreProgram,
  validateInputLiveV76RestorePayload,
  type InputLiveV76RestorePayload,
} from "./input-field-live-v76-restore.js";

export const INPUT_LIVE_V76_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V76_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V76_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V76_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V76_VARIANT_COUNT = 256;
export const INPUT_LIVE_V76_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V76_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV76SourceId = (typeof INPUT_LIVE_V76_SOURCE_IDS)[number];

export interface InputLiveV76SourceIdentity {
  source: InputLiveV76SourceId;
...
export interface InputLiveV76WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV76ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV76SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV76RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV76RootProof {
  source: InputLiveV76SourceId;
  adapterIdentity: string;
...
export interface InputLiveV76ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV76SourceId;
    adapterIdentity: string;
...
export interface InputLiveV76CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV76SourceId;
  adapterIdentity: string;
...
export interface InputLiveV76CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV76SourceId;
  strata: InputLiveV76CaptureCell["strata"];
...
export interface InputLiveV76CleanupPayload {
...
export function validateInputLiveV76WriterPayload(
  value: unknown,
): InputLiveV76WriterOwnership {
...
const identityPlan = (source: InputLiveV76SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV76ExtractProgram(
  writer: InputLiveV76WriterOwnership,
  sources: readonly InputLiveV76SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V76_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V76_NAMESPACE)}
${buildInputLiveV76RawPropertyRuntime()}
...
export function validateInputLiveV76ExtractPayload(
  value: unknown,
  writer: InputLiveV76WriterOwnership,
): InputLiveV76ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V76_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV76ExtractPayload;
}

export function proveInputLiveV76Roots<Instance>(
  extract: InputLiveV76ExtractPayload,
  sources: readonly (InputLiveV76SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV76Scene(
      root.scene,
...
export function assertInputLiveV76RootProofs(
  proofs: readonly InputLiveV76RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV76ProbeProgram(
  writer: InputLiveV76WriterOwnership,
  sources: readonly InputLiveV76SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V76_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
for(const set of sets){
 ...
 instance.resizeWithoutConstraints(beforeWidth+64,instance.height);const reflowPassed=instance.width===beforeWidth+64&&surface&&surface.width>surfaceWidth&&reflowTarget&&reflowTarget.width>reflowTargetWidth;const contentFillPassed=!!(contentText&&contentText.layoutSizingHorizontal==="FILL");instance.resizeWithoutConstraints(beforeWidth,instance.height);
...
export function validateInputLiveV76ProbePayload(
  value: unknown,
  writer: InputLiveV76WriterOwnership,
): InputLiveV76ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV76ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V76_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V76_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV76CaptureManifest(
  cells: readonly InputLiveV76CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V76_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V76_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V76_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V76_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V76_SOURCE_IDS.some(
...
export const inputLiveV76CaptureManifestSha256 = (
  cells: readonly InputLiveV76CaptureCell[],
): string => {
  validateInputLiveV76CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV76CaptureProgram(
  writer: InputLiveV76WriterOwnership,
  cell: InputLiveV76CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V76_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V76_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V76_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV76CapturePayload(
  value: unknown,
  cell: InputLiveV76CaptureCell,
  rawResponseBytes: number,
): InputLiveV76CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV76CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V76_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V76_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV76CaptureResponses(
  manifest: readonly InputLiveV76CaptureCell[],
  responses: readonly InputLiveV76CapturePayload[],
): void {
  validateInputLiveV76CaptureManifest(manifest);
  if (
...
export interface InputLiveV76ObjectiveReport {
  artifactVersion: "input-live-v76-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV76Objective(
  manifest: readonly InputLiveV76CaptureCell[],
  responses: readonly InputLiveV76CapturePayload[],
  readReference: (cell: InputLiveV76CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV76CaptureCell) => Buffer,
): InputLiveV76ObjectiveReport {
  assertInputLiveV76CaptureResponses(manifest, responses);
...
export function buildInputLiveV76CleanupProgram(
  writer: InputLiveV76WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V76_NAMESPACE,
...
export function validateInputLiveV76CleanupPayload(
  value: unknown,
  writer: InputLiveV76WriterOwnership,
): InputLiveV76CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV76CleanupPayload;
...
export const INPUT_LIVE_V76_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV76WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
