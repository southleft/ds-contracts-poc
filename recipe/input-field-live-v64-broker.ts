...
import {
  assertInputLiveV64RootProofs,
  INPUT_LIVE_V64_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V64_RESPONSE_CONTRACTS,
  type InputLiveV64RootProof,
} from "./input-field-live-v64-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V64_BROKER_VERSION = "input-live-v64-broker-v2";
export const INPUT_LIVE_V64_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V64_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V64_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V64_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V64_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V64_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV64RemotePhase =
  (typeof INPUT_LIVE_V64_REMOTE_PHASES)[number];
export type InputLiveV64AuthorizationMode = "live" | "simulated";

export interface InputLiveV64CommitPins {
...
export interface InputLiveV64TransactionAuthorization extends InputLiveV64CommitPins {
  mode: InputLiveV64AuthorizationMode;
...
export interface InputLiveV64Manifest {
  artifactVersion: "input-live-v64-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V64_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V64_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V64_TARGET;
  authorization: InputLiveV64TransactionAuthorization;
...
export interface InputLiveV64ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV64RequestBody {
  artifactVersion: "input-live-v64-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V64_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV64RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV64CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V64_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV64ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV64Request extends InputLiveV64RequestBody {
...
export interface InputLiveV64Receipt {
  artifactVersion: "input-live-v64-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V64_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV64RemotePhase;
...
export interface InputLiveV64AcceptedResponse<T = unknown> {
  request: InputLiveV64Request;
  receipt: InputLiveV64Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV64IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV64ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V64_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V64_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V64_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V64_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V64_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V64_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V64_TARGET.editorType;
};
...
export const inputLiveV64RequestSequence = (
  phase: InputLiveV64RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV64RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV64RequestPath = (
  directory: string,
  phase: InputLiveV64RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV64ResponsePath = (
  directory: string,
  phase: InputLiveV64RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV64ReceiptPath = (
  directory: string,
  phase: InputLiveV64RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV64Manifest => {
  const manifest = json<InputLiveV64Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v64-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V64_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V64_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV64RemotePhase,
  captureIndex?: number,
): InputLiveV64Request | undefined => {
  const file = inputLiveV64RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV64Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV64RemotePhase,
  captureIndex?: number,
): InputLiveV64Receipt | undefined => {
  const file = inputLiveV64ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV64Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV64Request | undefined;
  receipt: InputLiveV64Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV64RemotePhase,
): InputLiveV64ResponseContract => ({
  ...INPUT_LIVE_V64_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V64_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V64_DYNAMIC_TOOL,
      target: INPUT_LIVE_V64_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v64-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V64_BROKER_VERSION,
  target: INPUT_LIVE_V64_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V64_PROGRAM_START*/${program}/*INPUT_LIVE_V64_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V64_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V64_PROGRAM_END*/";
...
export function createInputLiveV64Transaction(
  directory: string,
  ...
  const manifest: InputLiveV64Manifest = {
    artifactVersion: "input-live-v64-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V64_BROKER_VERSION,
...
export function validateInputLiveV64Request(
  directory: string,
  request: InputLiveV64Request,
): void {
  ...
  const expectedSequence = inputLiveV64RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v64-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V64_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V64_TARGET.fileKey ||
...
export function issueInputLiveV64Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV64RemotePhase,
  program: string,
  options: InputLiveV64IssueOptions = {},
): InputLiveV64Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV64RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV64Request>(targetPath);
    validateInputLiveV64Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV64ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV64TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV64RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V64_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V64_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V64_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV64RequestBody = {
    artifactVersion: "input-live-v64-broker-request-v2",
    brokerVersion: INPUT_LIVE_V64_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV64Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV64Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV64RawResponse(
  directory: string,
  phase: InputLiveV64RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV64RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV64Request>(requestFile);
  if (
  ...
  const target = inputLiveV64ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV64Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v64-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V64_BROKER_VERSION,
...
export function acceptInputLiveV64Response<T = unknown>(
  directory: string,
  phase: InputLiveV64RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV64AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV64ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV64Request>(
    inputLiveV64RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV64Request(directory, request);
  if (phase === "capture") validateInputLiveV64TechnicalGates(directory);
  const rawFile = inputLiveV64ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v64-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V64_BROKER_VERSION ||
...
export function persistInputLiveV64TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV64RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV64RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V64_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v64-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V64_BROKER_VERSION,
...
export function validateInputLiveV64TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v64-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V64_TECHNICAL_GATES.some(
...
export function validateInputLiveV64Receipt(
  directory: string,
  phase: InputLiveV64RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV64Request(directory, request);
  const receipt = json<InputLiveV64Receipt>(
    inputLiveV64ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV64ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v64-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V64_BROKER_VERSION ||
...
export function readInputLiveV64AcceptedRawPayload(
  directory: string,
  phase: InputLiveV64RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV64Receipt(directory, phase, captureIndex);
...
export function createInputLiveV64FakeDynamicResponse(
  request: InputLiveV64Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V64_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V64_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV64PersistedRequests(
  directory: string,
): InputLiveV64Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV64Request>(file)] : [];
    });
}

export const inputLiveV64Sha256 = sha256;
