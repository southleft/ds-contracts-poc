...
import {
  verifyComboboxLiveV40FixedSceneFixedPoint,
  type ComboboxLiveV40FixedFixedPoint,
} from "./combobox-live-v40-fixed-point.js";

export const COMBOBOX_LIVE_V40_VERIFIER_VERSION = "combobox-live-v40-verifier-v1";

export function buildComboboxLiveV40RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV40RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV40RawNode[];
}

export interface ComboboxLiveV40NormalizedScene {
...
export interface ComboboxLiveV40SceneProof {
...
export function normalizeComboboxLiveV40Scene(
  root: ComboboxLiveV40RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV40NormalizedScene {
  const canonicalBindings: ComboboxLiveV40NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV40RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV40Scene<Instance>(
  normalized: ComboboxLiveV40NormalizedScene,
...
): ComboboxLiveV40SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV40FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV40ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV40PreCaptureGates(payload: unknown): void {
