...
import {
  ...
  verifyInputLiveV32Authorization,
  type InputLiveV32AntecedentIndex,
  type InputLiveV32AuthorizationProof,
} from "./input-field-live-v32-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V32_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v32/operator-security-attestation-template.json";
export const INPUT_LIVE_V32_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v32-security-attestation.json";

export interface InputLiveV32SecurityAttestation {
...
export interface InputLiveV32PreflightReport {
  artifactVersion: "input-live-v32-preflight-v1";
...
export const inputLiveV32AttestationSha256 = (
  value: Omit<InputLiveV32SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV32SecurityAttestation(
  value: unknown,
  proof: InputLiveV32AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v32-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V32_TARGET) ||
...
export function validateInputLiveV32ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV32Authorization");
  const preflight = source.indexOf("runInputLiveV32Preflight");
  const initialize = source.indexOf("InputLiveV32Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV32Preflight(
  root: string,
  proof: InputLiveV32AuthorizationProof,
  attestationPath = INPUT_LIVE_V32_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV32PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V32_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V32_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V32_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV32AntecedentIndex;
  failures.push(...validateInputLiveV32AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V32_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V32_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v32-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV32ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV32SecurityAttestation;
  return {
    artifactVersion: "input-live-v32-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V32_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV32Preflight(
    process.cwd(),
    verifyInputLiveV32Authorization(),
    attestationPath,
