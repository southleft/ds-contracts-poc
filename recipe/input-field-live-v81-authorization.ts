...
import {
  ...
  INPUT_LIVE_V81_INDEX_PATH,
} from "./build-input-field-live-proof-v81.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV81HistoryExpectation = "pending" | "authorized";

export interface InputLiveV81AuthorizationArtifact {
  artifactVersion: "input-live-v81-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V81_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV81AntecedentIndex {
...
export interface InputLiveV81HistoryState {
  index?: InputLiveV81AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV81AuthorizationArtifact;
...
export interface InputLiveV81AuthorizationProof extends InputLiveV81TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V81_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V81_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V81_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V81_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v81-authorization") ||
  artifactPath.includes("input-field-live-v81-preflight") ||
  artifactPath.includes("input-field-live-v81-authorized") ||
  artifactPath.includes("input-field-live-pivot-v81-status") ||
...
export function validateInputLiveV81AntecedentIndex(
  index: InputLiveV81AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v81-antecedent-index-v1" ||
...
export function validateInputLiveV81UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV81History\(\s*["']/.test(source) ||
    /readInputLiveV81HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV81AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV81AuthorizationArtifact {
  ...
  ) as InputLiveV81AntecedentIndex;
  const indexFailures = validateInputLiveV81AntecedentIndex(index);
...
export function validateInputLiveV81AuthorizationArtifact(
  artifact: InputLiveV81AuthorizationArtifact | undefined,
  index: InputLiveV81AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v81-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v81" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V81_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V81_TARGET) ||
    !equalJson(
...
export function validateInputLiveV81History(
  state: InputLiveV81HistoryState,
  expected: InputLiveV81HistoryExpectation,
): string[] {
  const failures = validateInputLiveV81AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV81AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV81HistoryState(
  root = process.cwd(),
): InputLiveV81HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V81_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV81AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V81_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V81_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V81_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV81History(
  expected: InputLiveV81HistoryExpectation,
  root = process.cwd(),
): InputLiveV81HistoryState {
  const state = readInputLiveV81HistoryState(root);
  const failures = validateInputLiveV81History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV81Authorization(
  root = process.cwd(),
): InputLiveV81AuthorizationProof {
  const state = verifyInputLiveV81History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V81_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v81/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v81.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V81_TARGET,
    expectedDynamicTool: INPUT_LIVE_V81_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V81_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV81HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV81History(expected);
