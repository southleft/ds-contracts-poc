...
import { INPUT_LIVE_V27_INDEX_PATH } from "./build-input-field-live-proof-v27.js";
import {
  INPUT_LIVE_V27_DYNAMIC_TOOL,
  INPUT_LIVE_V27_TARGET,
} from "./input-field-live-v27-broker.js";
import {
  buildInputLiveV27AuthorizationArtifact,
  validateInputLiveV27AntecedentIndex,
  validateInputLiveV27AuthorizationArtifact,
  validateInputLiveV27History,
  validateInputLiveV27UnitTestSource,
  type InputLiveV27AntecedentIndex,
  type InputLiveV27AuthorizationArtifact,
  type InputLiveV27AuthorizationProof,
  type InputLiveV27HistoryState,
} from "./input-field-live-v27-authorization.js";
import {
  inputLiveV27AttestationSha256,
  validateInputLiveV27ControlFlowSource,
  validateInputLiveV27SecurityAttestation,
  type InputLiveV27SecurityAttestation,
} from "./input-field-live-v27-preflight.js";
import { inputLiveV27Sha256 } from "./input-field-live-v27-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V27_INDEX_PATH);
const index = (): InputLiveV27AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV27AntecedentIndex;

const artifact = (): InputLiveV27AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV27AuthorizationArtifact({
...
const pendingState = (): InputLiveV27HistoryState => ({
  index: index(),
  indexSha256: inputLiveV27Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV27HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV27AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV27Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V27_TARGET,
  expectedDynamicTool: INPUT_LIVE_V27_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v27/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV27AuthorizationArtifact,
): InputLiveV27SecurityAttestation => {
  const body: Omit<InputLiveV27SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v27-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V27_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV27AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV27History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV27History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV27History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV27History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV27History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV27UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV27UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV27AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV27Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV27AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV27AuthorizationArtifact(
        planted as InputLiveV27AuthorizationArtifact,
        index(),
        inputLiveV27Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV27SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV27AttestationSha256(
      body as Omit<InputLiveV27SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV27SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV27ControlFlowSource(source), []);
});
