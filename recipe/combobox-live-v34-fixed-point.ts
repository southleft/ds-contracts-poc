/**
 * Carried Combobox live v34 host verifier. Uses scene-readback-combobox-v34 so live
...
export const COMBOBOX_LIVE_V34_FIXED_VERIFIER_VERSION = "combobox-live-v34-fixed-point-v1";
export const COMBOBOX_LIVE_V34_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV34FixedThresholds {
...
export interface ComboboxLiveV34FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV34FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV34FixedSourceProbe {
...
export interface ComboboxLiveV34FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV34FixedFixedPoint;
}

export interface ComboboxLiveV34FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV34FixedHardGateInput {
  thresholds: ComboboxLiveV34FixedThresholds;
  materialRegression: ComboboxLiveV34FixedMaterialRegression;
  sourceProbes: ComboboxLiveV34FixedSourceProbe[];
  cells: ComboboxLiveV34FixedCellValidation[];
  sceneProofs: ComboboxLiveV34FixedSceneProof[];
  visualRows: ComboboxLiveV34FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV34FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V34_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV34FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV34FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV34FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV34FixedHardGates(
  input: ComboboxLiveV34FixedHardGateInput,
): ComboboxLiveV34FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV34FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV34FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V34_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV34FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV34FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV34FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
