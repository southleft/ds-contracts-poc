...
export const INPUT_LIVE_V72_VERIFIER_VERSION = "input-live-v72-verifier-v1";

export function buildInputLiveV72RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV72RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV72RawNode[];
}

export interface InputLiveV72NormalizedScene {
...
export interface InputLiveV72SceneProof {
...
export function normalizeInputLiveV72Scene(
  root: InputLiveV72RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV72NormalizedScene {
  const canonicalBindings: InputLiveV72NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV72RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV72Scene<Instance>(
  normalized: InputLiveV72NormalizedScene,
...
export function compileInputLiveV72ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV72PreCaptureGates(payload: unknown): void {
