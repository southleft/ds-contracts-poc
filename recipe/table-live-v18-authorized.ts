...
import { TABLE_LIVE_V18_AUTHORIZATION_PATH } from "./build-table-live-proof-v18.js";
import { verifyTableLiveV18Authorization } from "./table-live-v18-authorization.js";
import { runTableLiveV18Preflight } from "./table-live-v18-preflight.js";
import { TableLiveV18Orchestrator } from "./run-table-live-v18.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v18 requires exact authorization artifact ${TABLE_LIVE_V18_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v18-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV18Authorization(root);
    runTableLiveV18Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV18Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV18Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
