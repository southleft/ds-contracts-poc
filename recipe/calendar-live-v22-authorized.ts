...
import { CALENDAR_LIVE_V22_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v22.js";
import { verifyCalendarLiveV22Authorization } from "./calendar-live-v22-authorization.js";
import { runCalendarLiveV22Preflight } from "./calendar-live-v22-preflight.js";
import { CalendarLiveV22Orchestrator } from "./run-calendar-live-v22.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v22 requires exact authorization artifact ${CALENDAR_LIVE_V22_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v22-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV22Authorization(root);
    runCalendarLiveV22Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV22Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV22Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
