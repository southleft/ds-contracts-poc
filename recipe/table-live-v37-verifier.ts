...
import {
  verifyTableLiveV37FixedSceneFixedPoint,
  type TableLiveV37FixedFixedPoint,
} from "./table-live-v37-fixed-point.js";

export const TABLE_LIVE_V37_VERIFIER_VERSION = "table-live-v37-verifier-v1";

export function buildTableLiveV37RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV37RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV37RawNode[];
}

export interface TableLiveV37NormalizedScene {
...
export interface TableLiveV37SceneProof {
...
export function normalizeTableLiveV37Scene(
  root: TableLiveV37RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV37NormalizedScene {
  const canonicalBindings: TableLiveV37NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV37RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV37Scene<Instance>(
  normalized: TableLiveV37NormalizedScene,
...
): TableLiveV37SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV37FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV37ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV37PreCaptureGates(payload: unknown): void {
