/**
 * Table live v38 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V38_FIXED_VERIFIER_VERSION = "table-live-v38-fixed-point-v1";
export const TABLE_LIVE_V38_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV38FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV38FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV38FixedSourceProbe {
...
export interface TableLiveV38FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV38FixedFixedPoint;
}

export interface TableLiveV38FixedHardGateInput {
  thresholds: TableLiveV38FixedThresholds;
  sourceProbes: TableLiveV38FixedSourceProbe[];
  cells: TableLiveV38FixedCellValidation[];
  sceneProofs: TableLiveV38FixedSceneProof[];
...
export interface TableLiveV38FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V38_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV38FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV38FixedHardGates(
  input: TableLiveV38FixedHardGateInput,
): TableLiveV38FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV38FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V38_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV38FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV38FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
