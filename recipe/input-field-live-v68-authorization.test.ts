...
import { INPUT_LIVE_V68_INDEX_PATH } from "./build-input-field-live-proof-v68.js";
import {
  INPUT_LIVE_V68_DYNAMIC_TOOL,
  INPUT_LIVE_V68_TARGET,
} from "./input-field-live-v68-broker.js";
import {
  buildInputLiveV68AuthorizationArtifact,
  validateInputLiveV68AntecedentIndex,
  validateInputLiveV68AuthorizationArtifact,
  validateInputLiveV68History,
  validateInputLiveV68UnitTestSource,
  type InputLiveV68AntecedentIndex,
  type InputLiveV68AuthorizationArtifact,
  type InputLiveV68AuthorizationProof,
  type InputLiveV68HistoryState,
} from "./input-field-live-v68-authorization.js";
import {
  inputLiveV68AttestationSha256,
  validateInputLiveV68ControlFlowSource,
  validateInputLiveV68SecurityAttestation,
  type InputLiveV68SecurityAttestation,
} from "./input-field-live-v68-preflight.js";
import { inputLiveV68Sha256 } from "./input-field-live-v68-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V68_INDEX_PATH);
const index = (): InputLiveV68AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV68AntecedentIndex;

const artifact = (): InputLiveV68AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV68AuthorizationArtifact({
...
const pendingState = (): InputLiveV68HistoryState => ({
  index: index(),
  indexSha256: inputLiveV68Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV68HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV68AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV68Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V68_TARGET,
  expectedDynamicTool: INPUT_LIVE_V68_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v68/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV68AuthorizationArtifact,
): InputLiveV68SecurityAttestation => {
  const body: Omit<InputLiveV68SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v68-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V68_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV68AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV68History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV68History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV68History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV68History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV68History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV68UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV68UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV68AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV68Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV68AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV68AuthorizationArtifact(
        planted as InputLiveV68AuthorizationArtifact,
        index(),
        inputLiveV68Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV68SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV68AttestationSha256(
      body as Omit<InputLiveV68SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV68SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV68ControlFlowSource(source), []);
});
