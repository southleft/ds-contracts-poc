...
import { calendarLiveV1Sha256 } from "./calendar-live-v17-broker.js";

export interface CalendarLiveV17LifecycleSimulationReport {
  artifactVersion: "calendar-live-v17-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV17AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV17Lifecycle(): Promise<CalendarLiveV17LifecycleSimulationReport> {
  await buildCalendarLiveV17Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V17_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV17AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v17-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V17_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V17_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV17History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV17History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV17AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V17_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V17_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV17History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV17History(authorized, "pending");
    ...
    await buildCalendarLiveV17Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V17_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV17AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV17Lifecycle(), null, 2)}\n`,
  );
