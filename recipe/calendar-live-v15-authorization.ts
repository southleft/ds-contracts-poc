...
export type CalendarLiveV15HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV15AuthorizationArtifact {
  artifactVersion: "calendar-live-v15-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V15_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV15AntecedentIndex {
...
export interface CalendarLiveV15HistoryState {
  index?: CalendarLiveV15AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV15AuthorizationArtifact;
...
export interface CalendarLiveV15AuthorizationProof
  extends CalendarLiveV15TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V15_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V15_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V15_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V15_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v15-authorization") ||
  artifactPath.includes("calendar-live-v15-preflight") ||
  artifactPath.includes("calendar-live-v15-authorized") ||
  artifactPath.includes("create-calendar-live-v15-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v15-status") ||
...
export function validateCalendarLiveV15AntecedentIndex(
  index: CalendarLiveV15AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v15-antecedent-index-v1" ||
...
export function validateCalendarLiveV15UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV15History\(\s*["']/.test(source) ||
    /readCalendarLiveV15HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV15AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV15AuthorizationArtifact {
  ...
  ) as CalendarLiveV15AntecedentIndex;
  const indexFailures = validateCalendarLiveV15AntecedentIndex(index);
...
export function validateCalendarLiveV15AuthorizationArtifact(
  artifact: CalendarLiveV15AuthorizationArtifact | undefined,
  index: CalendarLiveV15AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v15-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v15" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V15_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V15_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V15_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V15_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V15_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V15_SOURCE_ROOTS ||
...
export function validateCalendarLiveV15History(
  state: CalendarLiveV15HistoryState,
  expected: CalendarLiveV15HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV15AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV15AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV15HistoryState(
  root = process.cwd(),
): CalendarLiveV15HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V15_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV15AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V15_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V15_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V15_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV15History(
  expected: CalendarLiveV15HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV15HistoryState {
  const state = readCalendarLiveV15HistoryState(root);
  const failures = validateCalendarLiveV15History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v15 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV15Authorization(
  root = process.cwd(),
): CalendarLiveV15AuthorizationProof {
  const state = verifyCalendarLiveV15History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V15_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v15/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v15.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V15_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V15_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V15_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV15HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV15History(expected);
  process.stdout.write(
    `Calendar live v15 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
