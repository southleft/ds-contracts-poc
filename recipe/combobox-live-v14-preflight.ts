...
import {
  validateComboboxLiveV14AntecedentIndex,
  verifyComboboxLiveV14Authorization,
  type ComboboxLiveV14AntecedentIndex,
  type ComboboxLiveV14AuthorizationProof,
} from "./combobox-live-v14-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V14_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v14/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V14_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v14-security-attestation.json";

export interface ComboboxLiveV14SecurityAttestation {
  artifactVersion: "combobox-live-v14-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV14PreflightReport {
  artifactVersion: "combobox-live-v14-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV14SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV14SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV14AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v14-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V14_TARGET) ||
...
export function validateComboboxLiveV14ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV14Authorization");
  const preflight = source.indexOf("runComboboxLiveV14Preflight");
  const initialize = source.indexOf("ComboboxLiveV14Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV14Preflight(
  root: string,
  proof: ComboboxLiveV14AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V14_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV14PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V14_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V14_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V14_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV14AntecedentIndex;
  failures.push(...validateComboboxLiveV14AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V14_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V14_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV14SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v14-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV14ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v14 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV14SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v14-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V14_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV14Preflight(
    process.cwd(),
    verifyComboboxLiveV14Authorization(),
    attestationPath,
