...
export interface InputLiveV77LifecycleSimulationReport {
  artifactVersion: "input-live-v77-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV77AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV77Lifecycle(): Promise<InputLiveV77LifecycleSimulationReport> {
  await buildInputLiveV77Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V77_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV77AntecedentIndex;
  const indexSha256 = inputLiveV77Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v77-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V77_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V77_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV77History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV77History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV77AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V77_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V77_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV77History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV77History(authorized, "pending");
    ...
    await buildInputLiveV77Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V77_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV77AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV77Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV77Lifecycle(), null, 2)}\n`,
  );
