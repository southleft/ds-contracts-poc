...
export interface InputLiveV65LifecycleSimulationReport {
  artifactVersion: "input-live-v65-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV65AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV65Lifecycle(): Promise<InputLiveV65LifecycleSimulationReport> {
  await buildInputLiveV65Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V65_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV65AntecedentIndex;
  const indexSha256 = inputLiveV65Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v65-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V65_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V65_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV65History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV65History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV65AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V65_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V65_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV65History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV65History(authorized, "pending");
    ...
    await buildInputLiveV65Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V65_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV65AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV65Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV65Lifecycle(), null, 2)}\n`,
  );
