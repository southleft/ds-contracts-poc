...
import { CALENDAR_LIVE_V50_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v50.js";
import { verifyCalendarLiveV50Authorization } from "./calendar-live-v50-authorization.js";
import { runCalendarLiveV50Preflight } from "./calendar-live-v50-preflight.js";
import { CalendarLiveV50Orchestrator } from "./run-calendar-live-v50.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v50 requires exact authorization artifact ${CALENDAR_LIVE_V50_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v50-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV50Authorization(root);
    runCalendarLiveV50Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV50Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV50Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
