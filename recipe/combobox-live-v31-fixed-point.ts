/**
 * Carried Combobox live v31 host verifier. Uses scene-readback-combobox-v31 so live
...
export const COMBOBOX_LIVE_V31_FIXED_VERIFIER_VERSION = "combobox-live-v31-fixed-point-v1";
export const COMBOBOX_LIVE_V31_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV31FixedThresholds {
...
export interface ComboboxLiveV31FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV31FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV31FixedSourceProbe {
...
export interface ComboboxLiveV31FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV31FixedFixedPoint;
}

export interface ComboboxLiveV31FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV31FixedHardGateInput {
  thresholds: ComboboxLiveV31FixedThresholds;
  materialRegression: ComboboxLiveV31FixedMaterialRegression;
  sourceProbes: ComboboxLiveV31FixedSourceProbe[];
  cells: ComboboxLiveV31FixedCellValidation[];
  sceneProofs: ComboboxLiveV31FixedSceneProof[];
  visualRows: ComboboxLiveV31FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV31FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V31_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV31FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV31FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV31FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV31FixedHardGates(
  input: ComboboxLiveV31FixedHardGateInput,
): ComboboxLiveV31FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV31FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV31FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V31_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV31FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV31FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV31FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
