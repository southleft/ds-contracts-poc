...
import {
  verifyComboboxLiveV17FixedSceneFixedPoint,
  type ComboboxLiveV17FixedFixedPoint,
} from "./combobox-live-v17-fixed-point.js";

export const COMBOBOX_LIVE_V17_VERIFIER_VERSION = "combobox-live-v17-verifier-v1";

export function buildComboboxLiveV17RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV17RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV17RawNode[];
}

export interface ComboboxLiveV17NormalizedScene {
...
export interface ComboboxLiveV17SceneProof {
...
export function normalizeComboboxLiveV17Scene(
  root: ComboboxLiveV17RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV17NormalizedScene {
  const canonicalBindings: ComboboxLiveV17NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV17RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV17Scene<Instance>(
  normalized: ComboboxLiveV17NormalizedScene,
...
): ComboboxLiveV17SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV17FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV17ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV17PreCaptureGates(payload: unknown): void {
