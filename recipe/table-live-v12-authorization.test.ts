...
import { TABLE_LIVE_V12_INDEX_PATH } from "./build-table-live-proof-v12.js";
import {
...
import {
  ...
  validateTableLiveV12UnitTestSource,
  type TableLiveV12AntecedentIndex,
  type TableLiveV12AuthorizationArtifact,
...
import {
  ...
  validateTableLiveV12ControlFlowSource,
  validateTableLiveV12SecurityAttestation,
  type TableLiveV12SecurityAttestation,
} from "./table-live-v12-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V12_INDEX_PATH);
const index = (): TableLiveV12AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV12AntecedentIndex;

const artifact = (): TableLiveV12AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV12AuthorizationArtifact({
...
const pendingState = (): TableLiveV12HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV12HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV12AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v12/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV12AuthorizationArtifact,
): TableLiveV12SecurityAttestation => {
  const body: Omit<TableLiveV12SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V12_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV12History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV12History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV12History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV12History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV12History(authorizedState(), "pending").join("\n"),
    /stale history mode.*authorization exists/,
  );
});

test("the exact v6 phase-sensitive self-test defect is planted and table v12 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV12UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v12-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV12UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV12AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV12AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV12AuthorizationArtifact(
        planted as TableLiveV12AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV12SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV12SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV12SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v12-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV12ControlFlowSource(source), []);
});
