...
import {
  ...
  INPUT_LIVE_V45_INDEX_PATH,
} from "./build-input-field-live-proof-v45.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV45HistoryExpectation = "pending" | "authorized";

export interface InputLiveV45AuthorizationArtifact {
  artifactVersion: "input-live-v45-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V45_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV45AntecedentIndex {
...
export interface InputLiveV45HistoryState {
  index?: InputLiveV45AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV45AuthorizationArtifact;
...
export interface InputLiveV45AuthorizationProof extends InputLiveV45TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V45_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V45_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V45_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V45_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v45-status") ||
...
export function validateInputLiveV45AntecedentIndex(
  index: InputLiveV45AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v45-antecedent-index-v1" ||
...
export function validateInputLiveV45UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV45History\(\s*["']/.test(source) ||
    /readInputLiveV45HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV45AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV45AuthorizationArtifact {
  ...
  ) as InputLiveV45AntecedentIndex;
  const indexFailures = validateInputLiveV45AntecedentIndex(index);
...
export function validateInputLiveV45AuthorizationArtifact(
  artifact: InputLiveV45AuthorizationArtifact | undefined,
  index: InputLiveV45AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v45-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v45" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V45_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V45_TARGET) ||
    !equalJson(
...
export function validateInputLiveV45History(
  state: InputLiveV45HistoryState,
  expected: InputLiveV45HistoryExpectation,
): string[] {
  const failures = validateInputLiveV45AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV45AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV45HistoryState(
  root = process.cwd(),
): InputLiveV45HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V45_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV45AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V45_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V45_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V45_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV45History(
  expected: InputLiveV45HistoryExpectation,
  root = process.cwd(),
): InputLiveV45HistoryState {
  const state = readInputLiveV45HistoryState(root);
  const failures = validateInputLiveV45History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV45Authorization(
  root = process.cwd(),
): InputLiveV45AuthorizationProof {
  const state = verifyInputLiveV45History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V45_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v45/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v45.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V45_TARGET,
    expectedDynamicTool: INPUT_LIVE_V45_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V45_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV45HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV45History(expected);
