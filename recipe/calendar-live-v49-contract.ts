...
import { buildCalendarLiveV49CleanupRuntime } from "./calendar-live-v49-cleanup.js";
import { type CalendarLiveV49FixedFixedPoint } from "./calendar-live-v49-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV49RestoreProgram,
  validateCalendarLiveV49RestorePayload,
  type CalendarLiveV49RestorePayload,
} from "./calendar-live-v49-restore.js";
import {
  buildCalendarLiveV49RawPropertyRuntime,
  normalizeCalendarLiveV49Scene,
  type CalendarLiveV49RawNode,
} from "./calendar-live-v49-verifier.js";
...
export const CALENDAR_LIVE_V49_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V49_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V49_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V49_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V49_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V49_SET_COUNT = 3;
export const CALENDAR_LIVE_V49_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V49_HOST_PHASES = 3;
export const CALENDAR_LIVE_V49_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V49_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V49_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V49_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV49SourceId = (typeof CALENDAR_LIVE_V49_SOURCE_IDS)[number];
export type CalendarLiveV49Kind = "calendar" | "week" | "day";

export interface CalendarLiveV49SourceIdentity {
  source: CalendarLiveV49SourceId;
...
export interface CalendarLiveV49WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV49ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV49SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV49RawNode;
    weekScene: CalendarLiveV49RawNode;
    dayScene: CalendarLiveV49RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV49RootProof {
  source: CalendarLiveV49SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV49FixedFixedPoint;
}

export interface CalendarLiveV49ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV49SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV49CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV49SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV49CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV49SourceId;
  frameWidth: number;
...
export interface CalendarLiveV49CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v49 ${label} must be an object`);
...
export function validateCalendarLiveV49WriterPayload(
  value: unknown,
): CalendarLiveV49WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v49") ||
    ...
    result.namespace !== CALENDAR_LIVE_V49_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v49 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v49 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v49 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV49SourceIdentity,
  kind: CalendarLiveV49Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV49ExtractProgram(
  writer: CalendarLiveV49WriterOwnership,
  sources: readonly CalendarLiveV49SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v49 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V49_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V49_NAMESPACE)}
${buildCalendarLiveV49RawPropertyRuntime()}
...
export function validateCalendarLiveV49ExtractPayload(
  value: unknown,
  writer: CalendarLiveV49WriterOwnership,
): CalendarLiveV49ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v49 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V49_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v49 extract schema/one-root mismatch");
  return payload as CalendarLiveV49ExtractPayload;
}

export function proveCalendarLiveV49Roots<Instance>(
  extract: CalendarLiveV49ExtractPayload,
  sources: readonly (CalendarLiveV49SourceIdentity & {
...
): CalendarLiveV49RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v49 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v49 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV49Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV49Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV49Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV49FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v49 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v49 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v49 compile lost owned sets");
...
export function assertCalendarLiveV49RootProofs(
  proofs: readonly CalendarLiveV49RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v49 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v49 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV49ProbeProgram(
  writer: CalendarLiveV49WriterOwnership,
  sources: readonly CalendarLiveV49SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V49_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV49ProbePayload(
  value: unknown,
  writer: CalendarLiveV49WriterOwnership,
): CalendarLiveV49ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV49ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V49_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V49_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v49 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV49CaptureManifest(
  cells: readonly CalendarLiveV49CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V49_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V49_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V49_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V49_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v49 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV49CaptureCell[],
): string => {
  validateCalendarLiveV49CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV49CaptureProgram(
  writer: CalendarLiveV49WriterOwnership,
  cell: CalendarLiveV49CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V49_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v49 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v49 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V49_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV49CapturePayload(
  value: unknown,
  cell: CalendarLiveV49CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV49CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV49CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V49_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V49_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v49 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV49CaptureResponses(
  manifest: readonly CalendarLiveV49CaptureCell[],
  responses: readonly CalendarLiveV49CapturePayload[],
): void {
  validateCalendarLiveV49CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v49 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV49ObjectiveReport {
  artifactVersion: "calendar-live-v49-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV49Objective(
  manifest: readonly CalendarLiveV49CaptureCell[],
  responses: readonly CalendarLiveV49CapturePayload[],
): CalendarLiveV49ObjectiveReport {
  assertCalendarLiveV49CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v49-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV49CleanupProgram(
  writer: CalendarLiveV49WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV49CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V49_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v49 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV49CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV49WriterOwnership,
): CalendarLiveV49CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV49CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v49 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V49_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV49WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
