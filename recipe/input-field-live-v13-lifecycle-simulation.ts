...
export interface InputLiveV13LifecycleSimulationReport {
  artifactVersion: "input-live-v13-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV13AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV13Lifecycle(): Promise<InputLiveV13LifecycleSimulationReport> {
  await buildInputLiveV13Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V13_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV13AntecedentIndex;
  const indexSha256 = inputLiveV13Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v13-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V13_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V13_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV13History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV13History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV13AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V13_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V13_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV13History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV13History(authorized, "pending");
    ...
    await buildInputLiveV13Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V13_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV13AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV13Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV13Lifecycle(), null, 2)}\n`,
  );
