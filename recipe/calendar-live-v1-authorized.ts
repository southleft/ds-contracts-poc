...
import { CALENDAR_LIVE_V1_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v1.js";
import { verifyCalendarLiveV1Authorization } from "./calendar-live-v1-authorization.js";
import { runCalendarLiveV1Preflight } from "./calendar-live-v1-preflight.js";
import { CalendarLiveV1Orchestrator } from "./run-calendar-live-v1.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v1 requires exact authorization artifact ${CALENDAR_LIVE_V1_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v1-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV1Authorization(root);
    runCalendarLiveV1Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV1Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV1Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
