...
import {
  ...
  INPUT_LIVE_V48_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V48_RESPONSE_CONTRACTS,
  type InputLiveV48RootProof,
} from "./input-field-live-v48-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V48_BROKER_VERSION = "input-live-v48-broker-v2";
export const INPUT_LIVE_V48_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V48_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V48_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V48_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V48_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V48_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV48RemotePhase =
  (typeof INPUT_LIVE_V48_REMOTE_PHASES)[number];
export type InputLiveV48AuthorizationMode = "live" | "simulated";

export interface InputLiveV48CommitPins {
...
export interface InputLiveV48TransactionAuthorization extends InputLiveV48CommitPins {
  mode: InputLiveV48AuthorizationMode;
...
export interface InputLiveV48Manifest {
  artifactVersion: "input-live-v48-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V48_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V48_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V48_TARGET;
...
export interface InputLiveV48ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV48RequestBody {
  artifactVersion: "input-live-v48-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V48_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV48RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV48CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V48_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV48ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV48Request extends InputLiveV48RequestBody {
...
export interface InputLiveV48Receipt {
  artifactVersion: "input-live-v48-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V48_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV48RemotePhase;
...
export interface InputLiveV48AcceptedResponse<T = unknown> {
  request: InputLiveV48Request;
  receipt: InputLiveV48Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV48IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV48ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V48_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V48_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V48_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V48_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V48_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V48_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V48_TARGET.editorType;
};
...
export const inputLiveV48RequestSequence = (
  phase: InputLiveV48RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV48RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV48RequestPath = (
  directory: string,
  phase: InputLiveV48RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV48ResponsePath = (
  directory: string,
  phase: InputLiveV48RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV48ReceiptPath = (
  directory: string,
  phase: InputLiveV48RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV48Manifest => {
  const manifest = json<InputLiveV48Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v48-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V48_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V48_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV48RemotePhase,
  captureIndex?: number,
): InputLiveV48Request | undefined => {
  const file = inputLiveV48RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV48Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV48RemotePhase,
  captureIndex?: number,
): InputLiveV48Receipt | undefined => {
  const file = inputLiveV48ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV48Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV48Request | undefined;
  receipt: InputLiveV48Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV48RemotePhase,
): InputLiveV48ResponseContract => ({
  ...INPUT_LIVE_V48_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V48_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v48-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V48_BROKER_VERSION,
  target: INPUT_LIVE_V48_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V48_PROGRAM_START*/${program}/*INPUT_LIVE_V48_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V48_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V48_PROGRAM_END*/";
...
export function createInputLiveV48Transaction(
  directory: string,
  ...
  const manifest: InputLiveV48Manifest = {
    artifactVersion: "input-live-v48-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V48_BROKER_VERSION,
...
export function validateInputLiveV48Request(
  directory: string,
  request: InputLiveV48Request,
): void {
  ...
  const expectedSequence = inputLiveV48RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v48-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V48_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V48_TARGET.fileKey ||
...
export function issueInputLiveV48Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV48RemotePhase,
  program: string,
  options: InputLiveV48IssueOptions = {},
): InputLiveV48Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV48RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV48Request>(targetPath);
    validateInputLiveV48Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV48ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV48TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV48RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V48_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V48_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V48_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV48RequestBody = {
    artifactVersion: "input-live-v48-broker-request-v2",
    brokerVersion: INPUT_LIVE_V48_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV48Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV48Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV48RawResponse(
  directory: string,
  phase: InputLiveV48RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV48RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV48Request>(requestFile);
  if (
  ...
  const target = inputLiveV48ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV48Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v48-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V48_BROKER_VERSION,
...
export function acceptInputLiveV48Response<T = unknown>(
  directory: string,
  phase: InputLiveV48RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV48AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV48ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV48Request>(
    inputLiveV48RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV48Request(directory, request);
  if (phase === "capture") validateInputLiveV48TechnicalGates(directory);
  const rawFile = inputLiveV48ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v48-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V48_BROKER_VERSION ||
...
export function persistInputLiveV48TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV48RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV48RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V48_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v48-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V48_BROKER_VERSION,
...
export function validateInputLiveV48TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v48-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V48_TECHNICAL_GATES.some(
...
export function validateInputLiveV48Receipt(
  directory: string,
  phase: InputLiveV48RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV48Request(directory, request);
  const receipt = json<InputLiveV48Receipt>(
    inputLiveV48ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV48ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v48-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V48_BROKER_VERSION ||
...
export function readInputLiveV48AcceptedRawPayload(
  directory: string,
  phase: InputLiveV48RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV48Receipt(directory, phase, captureIndex);
...
export function createInputLiveV48FakeDynamicResponse(
  request: InputLiveV48Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V48_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V48_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV48PersistedRequests(
  directory: string,
): InputLiveV48Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV48Request>(file)] : [];
    });
}

export const inputLiveV48Sha256 = sha256;
