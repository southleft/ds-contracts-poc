...
import {
  ...
  verifyInputLiveV33Authorization,
  type InputLiveV33AntecedentIndex,
  type InputLiveV33AuthorizationProof,
} from "./input-field-live-v33-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V33_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v33/operator-security-attestation-template.json";
export const INPUT_LIVE_V33_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v33-security-attestation.json";

export interface InputLiveV33SecurityAttestation {
...
export interface InputLiveV33PreflightReport {
  artifactVersion: "input-live-v33-preflight-v1";
...
export const inputLiveV33AttestationSha256 = (
  value: Omit<InputLiveV33SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV33SecurityAttestation(
  value: unknown,
  proof: InputLiveV33AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v33-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V33_TARGET) ||
...
export function validateInputLiveV33ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV33Authorization");
  const preflight = source.indexOf("runInputLiveV33Preflight");
  const initialize = source.indexOf("InputLiveV33Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV33Preflight(
  root: string,
  proof: InputLiveV33AuthorizationProof,
  attestationPath = INPUT_LIVE_V33_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV33PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V33_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V33_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V33_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV33AntecedentIndex;
  failures.push(...validateInputLiveV33AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V33_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V33_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v33-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV33ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV33SecurityAttestation;
  return {
    artifactVersion: "input-live-v33-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V33_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV33Preflight(
    process.cwd(),
    verifyInputLiveV33Authorization(),
    attestationPath,
