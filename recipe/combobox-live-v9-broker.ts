...
import {
  ...
  COMBOBOX_LIVE_V9_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  COMBOBOX_LIVE_V9_RESPONSE_CONTRACTS,
  type ComboboxLiveV9RootProof,
} from "./combobox-live-v9-contract.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V9_BROKER_VERSION = "combobox-live-v9-broker-v2";
export const COMBOBOX_LIVE_V9_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const COMBOBOX_LIVE_V9_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const COMBOBOX_LIVE_V9_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const COMBOBOX_LIVE_V9_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const COMBOBOX_LIVE_V9_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const COMBOBOX_LIVE_V9_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type ComboboxLiveV9RemotePhase =
  (typeof COMBOBOX_LIVE_V9_REMOTE_PHASES)[number];
export type ComboboxLiveV9AuthorizationMode = "live" | "simulated";

export interface ComboboxLiveV9CommitPins {
...
export interface ComboboxLiveV9TransactionAuthorization extends ComboboxLiveV9CommitPins {
  mode: ComboboxLiveV9AuthorizationMode;
...
export interface ComboboxLiveV9Manifest {
  artifactVersion: "combobox-live-v9-broker-manifest-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V9_BROKER_VERSION;
...
export interface ComboboxLiveV9ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface ComboboxLiveV9RequestBody {
  artifactVersion: "combobox-live-v9-broker-request-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V9_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: ComboboxLiveV9RemotePhase;
  sequence: number;
  ...
  commitPins: ComboboxLiveV9CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof COMBOBOX_LIVE_V9_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: ComboboxLiveV9ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface ComboboxLiveV9Request extends ComboboxLiveV9RequestBody {
...
export interface ComboboxLiveV9Receipt {
  artifactVersion: "combobox-live-v9-broker-receipt-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V9_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: ComboboxLiveV9RemotePhase;
...
export interface ComboboxLiveV9AcceptedResponse<T = unknown> {
  request: ComboboxLiveV9Request;
  receipt: ComboboxLiveV9Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface ComboboxLiveV9IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: ComboboxLiveV9ResponseContract;
...
const equalTarget = (value: unknown): value is typeof COMBOBOX_LIVE_V9_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === COMBOBOX_LIVE_V9_TARGET.fileKey &&
  (value as BrokerTarget).fileName === COMBOBOX_LIVE_V9_TARGET.fileName &&
  (value as BrokerTarget).editorType === COMBOBOX_LIVE_V9_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== COMBOBOX_LIVE_V9_TARGET.fileKey ||
    context.fileName !== COMBOBOX_LIVE_V9_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === COMBOBOX_LIVE_V9_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Combobox live v9 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= COMBOBOX_LIVE_V9_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v9 capture index must be 0..71");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Combobox live v9 authorization pins are malformed");
};

const readManifest = (directory: string): ComboboxLiveV9Manifest => {
  const manifest = json<ComboboxLiveV9Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Combobox live v9 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "combobox-live-v9-broker-manifest-v2" ||
    manifest.brokerVersion !== COMBOBOX_LIVE_V9_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== COMBOBOX_LIVE_V9_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(COMBOBOX_LIVE_V9_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Combobox live v9 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: ComboboxLiveV9Request,
): ComboboxLiveV9RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: ComboboxLiveV9RemotePhase,
  captureIndex?: number,
): ComboboxLiveV9Request | undefined => {
  const file = comboboxLiveV5RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<ComboboxLiveV9Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: ComboboxLiveV9RemotePhase,
  captureIndex?: number,
): ComboboxLiveV9Receipt | undefined => {
  const file = comboboxLiveV5ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<ComboboxLiveV9Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: ComboboxLiveV9Request | undefined;
  receipt: ComboboxLiveV9Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: ComboboxLiveV9RemotePhase,
): ComboboxLiveV9ResponseContract => ({
  ...COMBOBOX_LIVE_V9_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: ComboboxLiveV9Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: COMBOBOX_LIVE_V9_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "combobox-live-v9-plugin-response-v2",
  brokerVersion: COMBOBOX_LIVE_V9_BROKER_VERSION,
  target: COMBOBOX_LIVE_V9_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*COMBOBOX_LIVE_V9_PROGRAM_START*/${program}/*COMBOBOX_LIVE_V9_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*COMBOBOX_LIVE_V9_PROGRAM_START*/";
  const endMarker = "/*COMBOBOX_LIVE_V9_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Combobox live v9 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createComboboxLiveV9Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Combobox live v9 private signing key does not match authorized public-key identity",
    );
  const manifest: ComboboxLiveV9Manifest = {
    artifactVersion: "combobox-live-v9-broker-manifest-v2",
    brokerVersion: COMBOBOX_LIVE_V9_BROKER_VERSION,
    ...
    captureRequests: COMBOBOX_LIVE_V9_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateComboboxLiveV9Request(
  directory: string,
  request: ComboboxLiveV9Request,
): void {
  ...
  if (
    request.artifactVersion !== "combobox-live-v9-broker-request-v2" ||
    request.brokerVersion !== COMBOBOX_LIVE_V9_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== COMBOBOX_LIVE_V9_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Combobox live v9 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueComboboxLiveV9Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: ComboboxLiveV9RemotePhase,
  program: string,
  options: ComboboxLiveV9IssueOptions = {},
): ComboboxLiveV9Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<ComboboxLiveV9Request>(targetPath);
    validateComboboxLiveV9Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Combobox live v9 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(comboboxLiveV5ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Combobox live v9 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Combobox live v9 empty code envelope refused");
  if (phase === "capture") validateComboboxLiveV9TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Combobox live v9 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: COMBOBOX_LIVE_V9_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? COMBOBOX_LIVE_V9_SIGNED_WRITER_TIMEOUT_MS
        : COMBOBOX_LIVE_V9_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: ComboboxLiveV9RequestBody = {
    artifactVersion: "combobox-live-v9-broker-request-v2",
    brokerVersion: COMBOBOX_LIVE_V9_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: ComboboxLiveV9Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateComboboxLiveV9Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistComboboxLiveV9RawResponse(
  directory: string,
  phase: ComboboxLiveV9RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Combobox live v9 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<ComboboxLiveV9Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Combobox live v9 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = comboboxLiveV5ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Combobox live v9 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): ComboboxLiveV9Receipt => {
  const body: Omit<ComboboxLiveV9Receipt, "receiptSha256"> = {
    artifactVersion: "combobox-live-v9-broker-receipt-v2",
    brokerVersion: COMBOBOX_LIVE_V9_BROKER_VERSION,
...
export function acceptComboboxLiveV9Response<T = unknown>(
  directory: string,
  phase: ComboboxLiveV9RemotePhase,
  options: {
    captureIndex?: number;
...
): ComboboxLiveV9AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = comboboxLiveV5ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Combobox live v9 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<ComboboxLiveV9Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Combobox live v9 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "combobox-live-v9-plugin-response-v2" ||
    plugin?.brokerVersion !== COMBOBOX_LIVE_V9_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Combobox live v9 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistComboboxLiveV9TechnicalGates(
  directory: string,
  rootProofs: readonly ComboboxLiveV9RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertComboboxLiveV9RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Combobox live v9 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Combobox live v9 probe payload hash malformed");
  const gates = Object.fromEntries(
    COMBOBOX_LIVE_V9_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "combobox-live-v9-host-technical-gates-v2",
    brokerVersion: COMBOBOX_LIVE_V9_BROKER_VERSION,
...
export function validateComboboxLiveV9TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "combobox-live-v9-host-technical-gates-v2" ||
    ...
    COMBOBOX_LIVE_V9_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Combobox live v9 technical gates artifact");
}

export function validateComboboxLiveV9Receipt(
  directory: string,
  phase: ComboboxLiveV9RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateComboboxLiveV9Request(directory, request);
  const receipt = json<ComboboxLiveV9Receipt>(
  ...
  if (
    receipt.artifactVersion !== "combobox-live-v9-broker-receipt-v2" ||
    receipt.brokerVersion !== COMBOBOX_LIVE_V9_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Combobox live v9 accepted receipt: ${request.requestId}`,
    );
}

export function readComboboxLiveV9AcceptedRawPayload(
  directory: string,
  phase: ComboboxLiveV9RemotePhase,
  captureIndex?: number,
): unknown {
  validateComboboxLiveV9Receipt(directory, phase, captureIndex);
...
export function createComboboxLiveV9FakeDynamicResponse(
  request: ComboboxLiveV9Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? COMBOBOX_LIVE_V9_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? COMBOBOX_LIVE_V9_TARGET,
    ...overrides.extra,
  };
}

export function listComboboxLiveV9PersistedRequests(
  directory: string,
): ComboboxLiveV9Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<ComboboxLiveV9Request>(file)] : [];
    });
}
