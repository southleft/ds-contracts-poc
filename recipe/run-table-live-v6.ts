...
import {
  acceptTableLiveV6Response,
  createTableLiveV6Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV6Request,
  persistTableLiveV6RawResponse,
  persistTableLiveV6TechnicalGates,
  readTableLiveV6AcceptedRawPayload,
  type TableLiveV6Request,
  type TableLiveV6TransactionAuthorization,
} from "./table-live-v6-broker.js";
import {
  TABLE_LIVE_V6_CAPTURE_COUNT,
  assertTableLiveV6CaptureResponses,
  ...
  evaluateTableLiveV6Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV6Roots,
  validateTableLiveV6CaptureManifest,
  validateTableLiveV6CapturePayload,
  validateTableLiveV6CleanupPayload,
  validateTableLiveV6ExtractPayload,
  validateTableLiveV6ProbePayload,
  validateTableLiveV6WriterPayload,
  type TableLiveV6CaptureCell,
  type TableLiveV6CapturePayload,
...
import {
  ...
  TABLE_LIVE_V6_PLAN_PATH,
  type TableLiveV6ProofPlan,
...
export const TABLE_LIVE_V6_RUNNER_VERSION = "table-live-v6-runner-v1";

interface TableLiveV6JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV6OrchestratorState {
  artifactVersion: "table-live-v6-orchestrator-state-v1";
  ...
  journal: TableLiveV6JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV6ProofPlan;
  planSha256: string;
  captures: TableLiveV6CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV6SourceIdentity & {
...
export interface TableLiveV6NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV6OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v6 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V6_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V6_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV6ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV6CaptureCell[];
  validateTableLiveV6CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v6 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v6 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV6WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV6RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV6Request =>
  readJson<TableLiveV6Request>(file);

export class TableLiveV6Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV6Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV6OrchestratorState = {
      artifactVersion: "table-live-v6-orchestrator-state-v1",
    ...
    return new TableLiveV6Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV6OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV6Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV6NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV6NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v6 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV6NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v6 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV6RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV6Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV6CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV6AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV6WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV6Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV6ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV6Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV6Response(this.#directory, "writer", {
        validate: validateTableLiveV6WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV6Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV6RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV6Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV6ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV6Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV6RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v6-host-root-proofs-v1",
      ...
      const next = issueTableLiveV6Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV6ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV6Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV6ProbePayload(payload, ownership),
      });
      persistTableLiveV6TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v6 capture cell missing");
      const accepted = acceptTableLiveV6Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV6CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V6_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV6CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV6Objective(
    ...
    throw new Error(`Table live v6 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV6Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV6CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV6Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV6RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV6Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV6CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV6CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV6CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV6Authorization = (
  privateKey: KeyObject,
): TableLiveV6TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV6SimulationKey = (): KeyObject =>
