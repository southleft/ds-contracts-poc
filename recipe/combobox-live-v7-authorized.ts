...
import { COMBOBOX_LIVE_V7_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v7.js";
import { verifyComboboxLiveV7Authorization } from "./combobox-live-v7-authorization.js";
import { runComboboxLiveV7Preflight } from "./combobox-live-v7-preflight.js";
import { ComboboxLiveV7Orchestrator } from "./run-combobox-live-v7.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v7 requires exact authorization artifact ${COMBOBOX_LIVE_V7_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v7-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV7Authorization(root);
    runComboboxLiveV7Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV7Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV7Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
