...
import { INPUT_LIVE_V52_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v52.js";
import { verifyInputLiveV52Authorization } from "./input-field-live-v52-authorization.js";
import { runInputLiveV52Preflight } from "./input-field-live-v52-preflight.js";
import { InputLiveV52Orchestrator } from "./run-input-field-live-v52.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V52_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v52-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV52Authorization(root);
    runInputLiveV52Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV52Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
