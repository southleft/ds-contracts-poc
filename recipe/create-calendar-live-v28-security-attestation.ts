...
import { verifyCalendarLiveV28Authorization } from "./calendar-live-v28-authorization.js";
import { CALENDAR_LIVE_V28_TARGET } from "./calendar-live-v28-broker.js";
import {
  CALENDAR_LIVE_V28_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV28Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v28-session";
...
const attestation: CalendarLiveV28SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV28SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v28 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V28_SECURITY_ATTESTATION_DEFAULT_PATH,
);
