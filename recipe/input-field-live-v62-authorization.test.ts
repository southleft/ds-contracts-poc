...
import { INPUT_LIVE_V62_INDEX_PATH } from "./build-input-field-live-proof-v62.js";
import {
...
import {
  ...
  validateInputLiveV62UnitTestSource,
  type InputLiveV62AntecedentIndex,
  type InputLiveV62AuthorizationArtifact,
  type InputLiveV62AuthorizationProof,
  type InputLiveV62HistoryState,
} from "./input-field-live-v62-authorization.js";
import {
  inputLiveV62AttestationSha256,
  validateInputLiveV62ControlFlowSource,
  validateInputLiveV62SecurityAttestation,
  type InputLiveV62SecurityAttestation,
} from "./input-field-live-v62-preflight.js";
import { inputLiveV62Sha256 } from "./input-field-live-v62-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V62_INDEX_PATH);
const index = (): InputLiveV62AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV62AntecedentIndex;

const artifact = (): InputLiveV62AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV62AuthorizationArtifact({
...
const pendingState = (): InputLiveV62HistoryState => ({
  index: index(),
  indexSha256: inputLiveV62Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV62HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV62AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV62Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V62_TARGET,
  expectedDynamicTool: INPUT_LIVE_V62_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v62/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV62AuthorizationArtifact,
): InputLiveV62SecurityAttestation => {
  const body: Omit<InputLiveV62SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V62_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV62AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV62History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV62History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV62History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV62History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV62History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV62UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV62UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV62AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV62Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV62AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV62AuthorizationArtifact(
        planted as InputLiveV62AuthorizationArtifact,
        index(),
        inputLiveV62Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV62SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV62AttestationSha256(
      body as Omit<InputLiveV62SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV62SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV62ControlFlowSource(source), []);
});
