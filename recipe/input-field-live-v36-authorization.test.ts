...
import { INPUT_LIVE_V36_INDEX_PATH } from "./build-input-field-live-proof-v36.js";
import {
  INPUT_LIVE_V36_DYNAMIC_TOOL,
  INPUT_LIVE_V36_TARGET,
} from "./input-field-live-v36-broker.js";
import {
  buildInputLiveV36AuthorizationArtifact,
  validateInputLiveV36AntecedentIndex,
  validateInputLiveV36AuthorizationArtifact,
  validateInputLiveV36History,
  validateInputLiveV36UnitTestSource,
  type InputLiveV36AntecedentIndex,
  type InputLiveV36AuthorizationArtifact,
  type InputLiveV36AuthorizationProof,
  type InputLiveV36HistoryState,
} from "./input-field-live-v36-authorization.js";
import {
  inputLiveV36AttestationSha256,
  validateInputLiveV36ControlFlowSource,
  validateInputLiveV36SecurityAttestation,
  type InputLiveV36SecurityAttestation,
} from "./input-field-live-v36-preflight.js";
import { inputLiveV36Sha256 } from "./input-field-live-v36-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V36_INDEX_PATH);
const index = (): InputLiveV36AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV36AntecedentIndex;

const artifact = (): InputLiveV36AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV36AuthorizationArtifact({
...
const pendingState = (): InputLiveV36HistoryState => ({
  index: index(),
  indexSha256: inputLiveV36Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV36HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV36AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV36Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V36_TARGET,
  expectedDynamicTool: INPUT_LIVE_V36_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v36/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV36AuthorizationArtifact,
): InputLiveV36SecurityAttestation => {
  const body: Omit<InputLiveV36SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v36-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V36_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV36AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV36History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV36History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV36History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV36History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV36History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV36UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV36UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV36AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV36Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV36AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV36AuthorizationArtifact(
        planted as InputLiveV36AuthorizationArtifact,
        index(),
        inputLiveV36Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV36SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV36AttestationSha256(
      body as Omit<InputLiveV36SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV36SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV36ControlFlowSource(source), []);
});
