...
export interface ComboboxLiveV25LifecycleSimulationReport {
  artifactVersion: "combobox-live-v25-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV25AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV25Lifecycle(): Promise<ComboboxLiveV25LifecycleSimulationReport> {
  await buildComboboxLiveV25Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V25_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV25AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v25-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V25_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V25_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV25History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV25History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV25AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V25_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V25_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV25History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV25History(authorized, "pending");
    ...
    await buildComboboxLiveV25Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V25_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV25AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV25Lifecycle(), null, 2)}\n`,
  );
