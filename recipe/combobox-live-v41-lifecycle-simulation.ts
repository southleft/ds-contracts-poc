...
export interface ComboboxLiveV41LifecycleSimulationReport {
  artifactVersion: "combobox-live-v41-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV41AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV41Lifecycle(): Promise<ComboboxLiveV41LifecycleSimulationReport> {
  await buildComboboxLiveV41Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V41_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV41AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v41-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V41_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V41_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV41History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV41History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV41AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V41_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V41_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV41History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV41History(authorized, "pending");
    ...
    await buildComboboxLiveV41Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V41_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV41AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV41Lifecycle(), null, 2)}\n`,
  );
