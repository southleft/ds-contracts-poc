...
import { CALENDAR_LIVE_V49_INDEX_PATH } from "./build-calendar-live-proof-v49.js";
import {
...
import {
  buildCalendarLiveV49AuthorizationArtifact,
  validateCalendarLiveV49AntecedentIndex,
  validateCalendarLiveV49AuthorizationArtifact,
  validateCalendarLiveV49History,
  validateCalendarLiveV49UnitTestSource,
  type CalendarLiveV49AntecedentIndex,
  type CalendarLiveV49AuthorizationArtifact,
  type CalendarLiveV49AuthorizationProof,
  type CalendarLiveV49HistoryState,
} from "./calendar-live-v49-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV49ControlFlowSource,
  validateCalendarLiveV49SecurityAttestation,
  type CalendarLiveV49SecurityAttestation,
} from "./calendar-live-v49-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V49_INDEX_PATH);
const index = (): CalendarLiveV49AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV49AntecedentIndex;

const artifact = (): CalendarLiveV49AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV49AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV49HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV49HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV49AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v49/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV49AuthorizationArtifact,
): CalendarLiveV49SecurityAttestation => {
  const body: Omit<CalendarLiveV49SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V49_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV49History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV49History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV49History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV49History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV49History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV49UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v49-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV49UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV49AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV49AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV49AuthorizationArtifact(
        planted as CalendarLiveV49AuthorizationArtifact,
        index(),
...
test("one-root denominator, capture gate, cleanup, and prior-lineage reuse are rejected", () => {
  ...
  for (const mutate of [
    ...
    (value: Record<string, any>) => {
      value.execution.comboboxLiveV49AuthorizationReusable = true;
    },
    (value: Record<string, any>) => {
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV49SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV49SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV49SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v49-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV49ControlFlowSource(source), []);
});
