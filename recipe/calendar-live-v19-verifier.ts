...
import {
  verifyCalendarLiveV19FixedSceneFixedPoint,
  type CalendarLiveV19FixedFixedPoint,
} from "./calendar-live-v19-fixed-point.js";

export const CALENDAR_LIVE_V19_VERIFIER_VERSION = "calendar-live-v19-verifier-v1";

export function buildCalendarLiveV19RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV19RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV19RawNode[];
}

export interface CalendarLiveV19NormalizedScene {
...
export interface CalendarLiveV19SceneProof {
...
export function normalizeCalendarLiveV19Scene(
  root: CalendarLiveV19RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV19NormalizedScene {
  const canonicalBindings: CalendarLiveV19NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV19RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV19Scene<Instance>(
  normalized: CalendarLiveV19NormalizedScene,
...
): CalendarLiveV19SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV19FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV19ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV19PreCaptureGates(payload: unknown): void {
