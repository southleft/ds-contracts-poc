...
import { TABLE_LIVE_V10_AUTHORIZATION_PATH } from "./build-table-live-proof-v10.js";
import { verifyTableLiveV10Authorization } from "./table-live-v10-authorization.js";
import { runTableLiveV10Preflight } from "./table-live-v10-preflight.js";
import { TableLiveV10Orchestrator } from "./run-table-live-v10.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v10 requires exact authorization artifact ${TABLE_LIVE_V10_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v10-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV10Authorization(root);
    runTableLiveV10Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV10Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV10Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
