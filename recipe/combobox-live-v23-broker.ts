...
import {
  ...
  COMBOBOX_LIVE_V23_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  COMBOBOX_LIVE_V23_RESPONSE_CONTRACTS,
  type ComboboxLiveV23RootProof,
} from "./combobox-live-v23-contract.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V23_BROKER_VERSION = "combobox-live-v23-broker-v2";
export const COMBOBOX_LIVE_V23_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const COMBOBOX_LIVE_V23_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const COMBOBOX_LIVE_V23_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const COMBOBOX_LIVE_V23_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const COMBOBOX_LIVE_V23_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const COMBOBOX_LIVE_V23_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type ComboboxLiveV23RemotePhase =
  (typeof COMBOBOX_LIVE_V23_REMOTE_PHASES)[number];
export type ComboboxLiveV23AuthorizationMode = "live" | "simulated";

export interface ComboboxLiveV23CommitPins {
...
export interface ComboboxLiveV23TransactionAuthorization extends ComboboxLiveV23CommitPins {
  mode: ComboboxLiveV23AuthorizationMode;
...
export interface ComboboxLiveV23Manifest {
  artifactVersion: "combobox-live-v23-broker-manifest-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V23_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V23_DYNAMIC_TOOL;
  target: typeof COMBOBOX_LIVE_V23_TARGET;
...
export interface ComboboxLiveV23ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface ComboboxLiveV23RequestBody {
  artifactVersion: "combobox-live-v23-broker-request-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V23_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: ComboboxLiveV23RemotePhase;
  sequence: number;
  ...
  commitPins: ComboboxLiveV23CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof COMBOBOX_LIVE_V23_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: ComboboxLiveV23ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface ComboboxLiveV23Request extends ComboboxLiveV23RequestBody {
...
export interface ComboboxLiveV23Receipt {
  artifactVersion: "combobox-live-v23-broker-receipt-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V23_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: ComboboxLiveV23RemotePhase;
...
export interface ComboboxLiveV23AcceptedResponse<T = unknown> {
  request: ComboboxLiveV23Request;
  receipt: ComboboxLiveV23Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface ComboboxLiveV23IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: ComboboxLiveV23ResponseContract;
...
const equalTarget = (value: unknown): value is typeof COMBOBOX_LIVE_V23_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === COMBOBOX_LIVE_V23_TARGET.fileKey &&
  (value as BrokerTarget).fileName === COMBOBOX_LIVE_V23_TARGET.fileName &&
  (value as BrokerTarget).editorType === COMBOBOX_LIVE_V23_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== COMBOBOX_LIVE_V23_TARGET.fileKey ||
    context.fileName !== COMBOBOX_LIVE_V23_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === COMBOBOX_LIVE_V23_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Combobox live v23 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= COMBOBOX_LIVE_V23_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v23 capture index must be 0..71");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Combobox live v23 authorization pins are malformed");
};

const readManifest = (directory: string): ComboboxLiveV23Manifest => {
  const manifest = json<ComboboxLiveV23Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Combobox live v23 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "combobox-live-v23-broker-manifest-v2" ||
    manifest.brokerVersion !== COMBOBOX_LIVE_V23_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== COMBOBOX_LIVE_V23_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(COMBOBOX_LIVE_V23_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Combobox live v23 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: ComboboxLiveV23Request,
): ComboboxLiveV23RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: ComboboxLiveV23RemotePhase,
  captureIndex?: number,
): ComboboxLiveV23Request | undefined => {
  const file = comboboxLiveV5RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<ComboboxLiveV23Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: ComboboxLiveV23RemotePhase,
  captureIndex?: number,
): ComboboxLiveV23Receipt | undefined => {
  const file = comboboxLiveV5ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<ComboboxLiveV23Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: ComboboxLiveV23Request | undefined;
  receipt: ComboboxLiveV23Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: ComboboxLiveV23RemotePhase,
): ComboboxLiveV23ResponseContract => ({
  ...COMBOBOX_LIVE_V23_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: ComboboxLiveV23Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: COMBOBOX_LIVE_V23_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: COMBOBOX_LIVE_V23_DYNAMIC_TOOL,
      target: COMBOBOX_LIVE_V23_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "combobox-live-v23-plugin-response-v2",
  brokerVersion: COMBOBOX_LIVE_V23_BROKER_VERSION,
  target: COMBOBOX_LIVE_V23_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*COMBOBOX_LIVE_V23_PROGRAM_START*/${program}/*COMBOBOX_LIVE_V23_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*COMBOBOX_LIVE_V23_PROGRAM_START*/";
  const endMarker = "/*COMBOBOX_LIVE_V23_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Combobox live v23 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createComboboxLiveV23Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Combobox live v23 private signing key does not match authorized public-key identity",
    );
  const manifest: ComboboxLiveV23Manifest = {
    artifactVersion: "combobox-live-v23-broker-manifest-v2",
    brokerVersion: COMBOBOX_LIVE_V23_BROKER_VERSION,
    ...
    captureRequests: COMBOBOX_LIVE_V23_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateComboboxLiveV23Request(
  directory: string,
  request: ComboboxLiveV23Request,
): void {
  ...
  if (
    request.artifactVersion !== "combobox-live-v23-broker-request-v2" ||
    request.brokerVersion !== COMBOBOX_LIVE_V23_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== COMBOBOX_LIVE_V23_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Combobox live v23 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueComboboxLiveV23Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: ComboboxLiveV23RemotePhase,
  program: string,
  options: ComboboxLiveV23IssueOptions = {},
): ComboboxLiveV23Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<ComboboxLiveV23Request>(targetPath);
    validateComboboxLiveV23Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Combobox live v23 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(comboboxLiveV5ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Combobox live v23 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Combobox live v23 empty code envelope refused");
  if (phase === "capture") validateComboboxLiveV23TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Combobox live v23 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: COMBOBOX_LIVE_V23_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? COMBOBOX_LIVE_V23_SIGNED_WRITER_TIMEOUT_MS
        : COMBOBOX_LIVE_V23_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: ComboboxLiveV23RequestBody = {
    artifactVersion: "combobox-live-v23-broker-request-v2",
    brokerVersion: COMBOBOX_LIVE_V23_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: ComboboxLiveV23Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateComboboxLiveV23Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistComboboxLiveV23RawResponse(
  directory: string,
  phase: ComboboxLiveV23RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Combobox live v23 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<ComboboxLiveV23Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Combobox live v23 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = comboboxLiveV5ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Combobox live v23 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): ComboboxLiveV23Receipt => {
  const body: Omit<ComboboxLiveV23Receipt, "receiptSha256"> = {
    artifactVersion: "combobox-live-v23-broker-receipt-v2",
    brokerVersion: COMBOBOX_LIVE_V23_BROKER_VERSION,
...
export function acceptComboboxLiveV23Response<T = unknown>(
  directory: string,
  phase: ComboboxLiveV23RemotePhase,
  options: {
    captureIndex?: number;
...
): ComboboxLiveV23AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = comboboxLiveV5ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Combobox live v23 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<ComboboxLiveV23Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Combobox live v23 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "combobox-live-v23-plugin-response-v2" ||
    plugin?.brokerVersion !== COMBOBOX_LIVE_V23_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Combobox live v23 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistComboboxLiveV23TechnicalGates(
  directory: string,
  rootProofs: readonly ComboboxLiveV23RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertComboboxLiveV23RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Combobox live v23 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Combobox live v23 probe payload hash malformed");
  const gates = Object.fromEntries(
    COMBOBOX_LIVE_V23_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "combobox-live-v23-host-technical-gates-v2",
    brokerVersion: COMBOBOX_LIVE_V23_BROKER_VERSION,
...
export function validateComboboxLiveV23TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "combobox-live-v23-host-technical-gates-v2" ||
    ...
    COMBOBOX_LIVE_V23_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Combobox live v23 technical gates artifact");
}

export function validateComboboxLiveV23Receipt(
  directory: string,
  phase: ComboboxLiveV23RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateComboboxLiveV23Request(directory, request);
  const receipt = json<ComboboxLiveV23Receipt>(
  ...
  if (
    receipt.artifactVersion !== "combobox-live-v23-broker-receipt-v2" ||
    receipt.brokerVersion !== COMBOBOX_LIVE_V23_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Combobox live v23 accepted receipt: ${request.requestId}`,
    );
}

export function readComboboxLiveV23AcceptedRawPayload(
  directory: string,
  phase: ComboboxLiveV23RemotePhase,
  captureIndex?: number,
): unknown {
  validateComboboxLiveV23Receipt(directory, phase, captureIndex);
...
export function createComboboxLiveV23FakeDynamicResponse(
  request: ComboboxLiveV23Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? COMBOBOX_LIVE_V23_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? COMBOBOX_LIVE_V23_TARGET,
    ...overrides.extra,
  };
}

export function listComboboxLiveV23PersistedRequests(
  directory: string,
): ComboboxLiveV23Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<ComboboxLiveV23Request>(file)] : [];
    });
}
