...
import {
  ...
  verifyInputLiveV81Authorization,
  type InputLiveV81AntecedentIndex,
  type InputLiveV81AuthorizationProof,
} from "./input-field-live-v81-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V81_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v81/operator-security-attestation-template.json";
export const INPUT_LIVE_V81_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v81-security-attestation.json";

export interface InputLiveV81SecurityAttestation {
...
export interface InputLiveV81PreflightReport {
  artifactVersion: "input-live-v81-preflight-v1";
...
export const inputLiveV81AttestationSha256 = (
  value: Omit<InputLiveV81SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV81SecurityAttestation(
  value: unknown,
  proof: InputLiveV81AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v81-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V81_TARGET) ||
...
export function validateInputLiveV81ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV81Authorization");
  const preflight = source.indexOf("runInputLiveV81Preflight");
  const initialize = source.indexOf("InputLiveV81Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV81Preflight(
  root: string,
  proof: InputLiveV81AuthorizationProof,
  attestationPath = INPUT_LIVE_V81_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV81PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V81_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V81_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V81_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV81AntecedentIndex;
  failures.push(...validateInputLiveV81AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V81_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V81_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v81-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV81ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV81SecurityAttestation;
  return {
    artifactVersion: "input-live-v81-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V81_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV81Preflight(
    process.cwd(),
    verifyInputLiveV81Authorization(),
    attestationPath,
