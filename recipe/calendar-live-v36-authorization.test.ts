...
import { CALENDAR_LIVE_V36_INDEX_PATH } from "./build-calendar-live-proof-v36.js";
import {
...
import {
  buildCalendarLiveV36AuthorizationArtifact,
  validateCalendarLiveV36AntecedentIndex,
  validateCalendarLiveV36AuthorizationArtifact,
  validateCalendarLiveV36History,
  validateCalendarLiveV36UnitTestSource,
  type CalendarLiveV36AntecedentIndex,
  type CalendarLiveV36AuthorizationArtifact,
  type CalendarLiveV36AuthorizationProof,
  type CalendarLiveV36HistoryState,
} from "./calendar-live-v36-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV36ControlFlowSource,
  validateCalendarLiveV36SecurityAttestation,
  type CalendarLiveV36SecurityAttestation,
} from "./calendar-live-v36-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V36_INDEX_PATH);
const index = (): CalendarLiveV36AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV36AntecedentIndex;

const artifact = (): CalendarLiveV36AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV36AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV36HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV36HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV36AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v36/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV36AuthorizationArtifact,
): CalendarLiveV36SecurityAttestation => {
  const body: Omit<CalendarLiveV36SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "calendar-live-v36-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V36_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV36History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV36History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV36History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV36History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV36History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV36UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v36-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV36UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV36AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV36AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV36AuthorizationArtifact(
        planted as CalendarLiveV36AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV36SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV36SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV36SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v36-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV36ControlFlowSource(source), []);
});
