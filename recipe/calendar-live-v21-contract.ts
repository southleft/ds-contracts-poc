...
import { buildCalendarLiveV21CleanupRuntime } from "./calendar-live-v21-cleanup.js";
import { type CalendarLiveV21FixedFixedPoint } from "./calendar-live-v21-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV21RestoreProgram,
  validateCalendarLiveV21RestorePayload,
  type CalendarLiveV21RestorePayload,
} from "./calendar-live-v21-restore.js";
import {
  buildCalendarLiveV21RawPropertyRuntime,
  normalizeCalendarLiveV21Scene,
  type CalendarLiveV21RawNode,
} from "./calendar-live-v21-verifier.js";
...
export const CALENDAR_LIVE_V21_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V21_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V21_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V21_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V21_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V21_SET_COUNT = 3;
export const CALENDAR_LIVE_V21_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V21_HOST_PHASES = 3;
export const CALENDAR_LIVE_V21_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V21_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V21_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V21_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV21SourceId = (typeof CALENDAR_LIVE_V21_SOURCE_IDS)[number];
export type CalendarLiveV21Kind = "calendar" | "week" | "day";

export interface CalendarLiveV21SourceIdentity {
  source: CalendarLiveV21SourceId;
...
export interface CalendarLiveV21WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV21ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV21SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV21RawNode;
    weekScene: CalendarLiveV21RawNode;
    dayScene: CalendarLiveV21RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV21RootProof {
  source: CalendarLiveV21SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV21FixedFixedPoint;
}

export interface CalendarLiveV21ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV21SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV21CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV21SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV21CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV21SourceId;
  frameWidth: number;
...
export interface CalendarLiveV21CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v21 ${label} must be an object`);
...
export function validateCalendarLiveV21WriterPayload(
  value: unknown,
): CalendarLiveV21WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v21") ||
    ...
    result.namespace !== CALENDAR_LIVE_V21_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v21 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v21 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v21 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV21SourceIdentity,
  kind: CalendarLiveV21Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV21ExtractProgram(
  writer: CalendarLiveV21WriterOwnership,
  sources: readonly CalendarLiveV21SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v21 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V21_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V21_NAMESPACE)}
${buildCalendarLiveV21RawPropertyRuntime()}
...
export function validateCalendarLiveV21ExtractPayload(
  value: unknown,
  writer: CalendarLiveV21WriterOwnership,
): CalendarLiveV21ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v21 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V21_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v21 extract schema/one-root mismatch");
  return payload as CalendarLiveV21ExtractPayload;
}

export function proveCalendarLiveV21Roots<Instance>(
  extract: CalendarLiveV21ExtractPayload,
  sources: readonly (CalendarLiveV21SourceIdentity & {
...
): CalendarLiveV21RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v21 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v21 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV21Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV21Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV21Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV21FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v21 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v21 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v21 compile lost owned sets");
...
export function assertCalendarLiveV21RootProofs(
  proofs: readonly CalendarLiveV21RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v21 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v21 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV21ProbeProgram(
  writer: CalendarLiveV21WriterOwnership,
  sources: readonly CalendarLiveV21SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V21_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV21ProbePayload(
  value: unknown,
  writer: CalendarLiveV21WriterOwnership,
): CalendarLiveV21ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV21ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V21_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V21_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v21 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV21CaptureManifest(
  cells: readonly CalendarLiveV21CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V21_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V21_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V21_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V21_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v21 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV21CaptureCell[],
): string => {
  validateCalendarLiveV21CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV21CaptureProgram(
  writer: CalendarLiveV21WriterOwnership,
  cell: CalendarLiveV21CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V21_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v21 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v21 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V21_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV21CapturePayload(
  value: unknown,
  cell: CalendarLiveV21CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV21CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV21CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V21_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V21_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v21 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV21CaptureResponses(
  manifest: readonly CalendarLiveV21CaptureCell[],
  responses: readonly CalendarLiveV21CapturePayload[],
): void {
  validateCalendarLiveV21CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v21 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV21ObjectiveReport {
  artifactVersion: "calendar-live-v21-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV21Objective(
  manifest: readonly CalendarLiveV21CaptureCell[],
  responses: readonly CalendarLiveV21CapturePayload[],
): CalendarLiveV21ObjectiveReport {
  assertCalendarLiveV21CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v21-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV21CleanupProgram(
  writer: CalendarLiveV21WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV21CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V21_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v21 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV21CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV21WriterOwnership,
): CalendarLiveV21CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV21CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v21 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V21_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV21WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
