...
export const INPUT_LIVE_V29_VERIFIER_VERSION = "input-live-v29-verifier-v1";

export function buildInputLiveV29RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV29RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV29RawNode[];
}

export interface InputLiveV29NormalizedScene {
...
export interface InputLiveV29SceneProof {
...
export function normalizeInputLiveV29Scene(
  root: InputLiveV29RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV29NormalizedScene {
  const canonicalBindings: InputLiveV29NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV29RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV29Scene<Instance>(
  normalized: InputLiveV29NormalizedScene,
...
export function compileInputLiveV29ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV29PreCaptureGates(payload: unknown): void {
