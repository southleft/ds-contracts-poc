...
import { TABLE_LIVE_V26_AUTHORIZATION_PATH } from "./build-table-live-proof-v26.js";
import { verifyTableLiveV26Authorization } from "./table-live-v26-authorization.js";
import { runTableLiveV26Preflight } from "./table-live-v26-preflight.js";
import { TableLiveV26Orchestrator } from "./run-table-live-v26.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v26 requires exact authorization artifact ${TABLE_LIVE_V26_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v26-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV26Authorization(root);
    runTableLiveV26Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV26Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV26Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
