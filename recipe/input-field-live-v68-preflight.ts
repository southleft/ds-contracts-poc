...
import {
  ...
  verifyInputLiveV68Authorization,
  type InputLiveV68AntecedentIndex,
  type InputLiveV68AuthorizationProof,
} from "./input-field-live-v68-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V68_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v68/operator-security-attestation-template.json";
export const INPUT_LIVE_V68_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v68-security-attestation.json";

export interface InputLiveV68SecurityAttestation {
...
export interface InputLiveV68PreflightReport {
  artifactVersion: "input-live-v68-preflight-v1";
...
export const inputLiveV68AttestationSha256 = (
  value: Omit<InputLiveV68SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV68SecurityAttestation(
  value: unknown,
  proof: InputLiveV68AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v68-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V68_TARGET) ||
...
export function validateInputLiveV68ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV68Authorization");
  const preflight = source.indexOf("runInputLiveV68Preflight");
  const initialize = source.indexOf("InputLiveV68Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV68Preflight(
  root: string,
  proof: InputLiveV68AuthorizationProof,
  attestationPath = INPUT_LIVE_V68_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV68PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V68_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V68_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V68_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV68AntecedentIndex;
  failures.push(...validateInputLiveV68AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V68_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V68_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v68-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV68ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV68SecurityAttestation;
  return {
    artifactVersion: "input-live-v68-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V68_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV68Preflight(
    process.cwd(),
    verifyInputLiveV68Authorization(),
    attestationPath,
