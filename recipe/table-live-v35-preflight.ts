...
import {
  validateTableLiveV35AntecedentIndex,
  verifyTableLiveV35Authorization,
  type TableLiveV35AntecedentIndex,
  type TableLiveV35AuthorizationProof,
} from "./table-live-v35-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V35_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v35/operator-security-attestation-template.json";
export const TABLE_LIVE_V35_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v35-security-attestation.json";

export interface TableLiveV35SecurityAttestation {
  artifactVersion: "table-live-v35-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV35PreflightReport {
  artifactVersion: "table-live-v35-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV35SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV35SecurityAttestation(
  value: unknown,
  proof: TableLiveV35AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v35-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V35_TARGET) ||
...
export function validateTableLiveV35ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV35Authorization");
  const preflight = source.indexOf("runTableLiveV35Preflight");
  const initialize = source.indexOf("TableLiveV35Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV35Preflight(
  root: string,
  proof: TableLiveV35AuthorizationProof,
  attestationPath = TABLE_LIVE_V35_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV35PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V35_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V35_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV35AntecedentIndex;
  failures.push(...validateTableLiveV35AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V35_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V35_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV35SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v35-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV35ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v35 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV35SecurityAttestation;
  return {
    artifactVersion: "table-live-v35-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V35_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV35Preflight(
    process.cwd(),
    verifyTableLiveV35Authorization(),
    attestationPath,
