...
import { TABLE_LIVE_V3_INDEX_PATH } from "./build-table-live-proof-v3.js";
import {
...
import {
  ...
  validateTableLiveV3UnitTestSource,
  type TableLiveV3AntecedentIndex,
  type TableLiveV3AuthorizationArtifact,
...
import {
  ...
  validateTableLiveV3ControlFlowSource,
  validateTableLiveV3SecurityAttestation,
  type TableLiveV3SecurityAttestation,
} from "./table-live-v3-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V3_INDEX_PATH);
const index = (): TableLiveV3AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV3AntecedentIndex;

const artifact = (): TableLiveV3AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV3AuthorizationArtifact({
...
const pendingState = (): TableLiveV3HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV3HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV3AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v3/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV3AuthorizationArtifact,
): TableLiveV3SecurityAttestation => {
  const body: Omit<TableLiveV3SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V3_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV3History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV3History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV3History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV3History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV3History(authorizedState(), "pending").join("\n"),
    /stale history mode.*authorization exists/,
  );
});

test("the exact v6 phase-sensitive self-test defect is planted and table v3 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV3UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v3-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV3UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV3AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV3AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV3AuthorizationArtifact(
        planted as TableLiveV3AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV3SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV3SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV3SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v3-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV3ControlFlowSource(source), []);
});
