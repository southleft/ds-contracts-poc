...
import { INPUT_LIVE_V47_INDEX_PATH } from "./build-input-field-live-proof-v47.js";
import {
  INPUT_LIVE_V47_DYNAMIC_TOOL,
  INPUT_LIVE_V47_TARGET,
} from "./input-field-live-v47-broker.js";
import {
  buildInputLiveV47AuthorizationArtifact,
  validateInputLiveV47AntecedentIndex,
  validateInputLiveV47AuthorizationArtifact,
  validateInputLiveV47History,
  validateInputLiveV47UnitTestSource,
  type InputLiveV47AntecedentIndex,
  type InputLiveV47AuthorizationArtifact,
  type InputLiveV47AuthorizationProof,
  type InputLiveV47HistoryState,
} from "./input-field-live-v47-authorization.js";
import {
  inputLiveV47AttestationSha256,
  validateInputLiveV47ControlFlowSource,
  validateInputLiveV47SecurityAttestation,
  type InputLiveV47SecurityAttestation,
} from "./input-field-live-v47-preflight.js";
import { inputLiveV47Sha256 } from "./input-field-live-v47-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V47_INDEX_PATH);
const index = (): InputLiveV47AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV47AntecedentIndex;

const artifact = (): InputLiveV47AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV47AuthorizationArtifact({
...
const pendingState = (): InputLiveV47HistoryState => ({
  index: index(),
  indexSha256: inputLiveV47Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV47HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV47AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV47Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V47_TARGET,
  expectedDynamicTool: INPUT_LIVE_V47_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v47/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV47AuthorizationArtifact,
): InputLiveV47SecurityAttestation => {
  const body: Omit<InputLiveV47SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v47-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V47_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV47AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV47History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV47History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV47History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV47History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV47History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV47UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV47UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV47AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV47Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV47AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV47AuthorizationArtifact(
        planted as InputLiveV47AuthorizationArtifact,
        index(),
        inputLiveV47Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV47SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV47AttestationSha256(
      body as Omit<InputLiveV47SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV47SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV47ControlFlowSource(source), []);
});
