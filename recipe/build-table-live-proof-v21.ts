...
import {
  TABLE_LIVE_V21_CAPTURE_COUNT,
  TABLE_LIVE_V21_HOST_PHASES,
  TABLE_LIVE_V21_REMOTE_REQUESTS,
  TABLE_LIVE_V21_SOURCE_ROOTS,
  TABLE_LIVE_V21_VARIANT_COUNT,
  buildTableLiveV21CaptureProgram,
  buildTableLiveV21CleanupProgram,
  buildTableLiveV21ExtractProgram,
  buildTableLiveV21ProbeProgram,
  buildTableLiveV21RestoreProgram,
  tableLiveV2CaptureManifestSha256,
  type TableLiveV21CaptureCell,
  type TableLiveV21SourceIdentity,
  type TableLiveV21WriterOwnership,
} from "./table-live-v21-contract.js";
import {
  TABLE_LIVE_V21_DYNAMIC_TOOL,
  TABLE_LIVE_V21_TARGET,
  tableLiveV2RequestSequence,
} from "./table-live-v21-broker.js";
import {
...
export const TABLE_LIVE_V21_EVIDENCE_ROOT = "recipe/evidence/table-live-pivot-v21";
export const TABLE_LIVE_V21_PROTOCOL_PATH = `${TABLE_LIVE_V21_EVIDENCE_ROOT}/protocol.json`;
export const TABLE_LIVE_V21_PLAN_PATH = `${TABLE_LIVE_V21_EVIDENCE_ROOT}/proof-plan.json`;
export const TABLE_LIVE_V21_CAPTURE_MANIFEST_PATH = `${TABLE_LIVE_V21_EVIDENCE_ROOT}/capture-manifest.json`;
export const TABLE_LIVE_V21_REQUEST_MANIFEST_PATH = `${TABLE_LIVE_V21_EVIDENCE_ROOT}/request-manifest.json`;
export const TABLE_LIVE_V21_INDEX_PATH = `${TABLE_LIVE_V21_EVIDENCE_ROOT}/antecedent-index.json`;
export const TABLE_LIVE_V21_AUTHORIZATION_TEMPLATE_PATH = `${TABLE_LIVE_V21_EVIDENCE_ROOT}/authorization-template.json`;
export const TABLE_LIVE_V21_AUTHORIZATION_PATH = `${TABLE_LIVE_V21_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface TableLiveV21ProofPlan {
  artifactVersion: "table-live-v21-proof-plan-v1";
  status: "draft antecedent; pending separate authorization; live execution forbidden";
  target: typeof TABLE_LIVE_V21_TARGET;
...
const captureCells = (
...
): TableLiveV21CaptureCell[] => {
  const cells: TableLiveV21CaptureCell[] = [];
  ...
  if (cells.length !== TABLE_LIVE_V21_CAPTURE_COUNT)
    throw new Error(
      `Table live v21 capture cells ${cells.length}, expected ${TABLE_LIVE_V21_CAPTURE_COUNT}`,
    );
...
const protocol = (expectedFacts: number) => ({
  artifactVersion: "table-live-v21-external-operator-protocol-v1",
  protocolId: "table-live-v21",
  status:
  ...
  target: TABLE_LIVE_V21_TARGET,
  expectedDynamicTool: TABLE_LIVE_V21_DYNAMIC_TOOL,
  denominator: {
    sourceRoots: TABLE_LIVE_V21_SOURCE_ROOTS,
    sets: 6,
    variants: TABLE_LIVE_V21_VARIANT_COUNT,
    captureCells: TABLE_LIVE_V21_CAPTURE_COUNT,
    remoteRequests: TABLE_LIVE_V21_REMOTE_REQUESTS,
    hostPhases: TABLE_LIVE_V21_HOST_PHASES,
...
export async function buildTableLiveV21Proof(
  check = process.argv.includes("--check"),
): Promise<TableLiveV21ProofPlan> {
  ...
  if (
    ...
    throw new Error("Table live v21 writer reused Input, Combobox, or Table v1–v20 identity");
  const captures = captureCells(sources);
  const writerProgram = Buffer.from(writer.code);
  const writerOwnershipBlueprint: TableLiveV21WriterOwnership = {
  ...
  const identities: TableLiveV21SourceIdentity[] = sources.map((source) => ({
    source: source.source,
    adapterIdentity: source.adapterIdentity,
    recipeHash: source.recipeHash,
    envelopeHash: source.envelope.integrity.canonicalHash,
  ...
  const restoreBlueprint = Buffer.from(
    buildTableLiveV21RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildTableLiveV21ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildTableLiveV21ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildTableLiveV21CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );
  const cleanupBlueprint = Buffer.from(
    buildTableLiveV21CleanupProgram(writerOwnershipBlueprint),
  );
  ...
  const sourceMetadata = sources.map((source) => {
    ...
    const tablePath = `${TABLE_LIVE_V21_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-table.json.gz`;
    const rowPath = `${TABLE_LIVE_V21_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-row.json.gz`;
    const cellPath = `${TABLE_LIVE_V21_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-cell.json.gz`;
  ...
  antecedentOutputs.set(
    TABLE_LIVE_V21_PROTOCOL_PATH,
    jsonBytes(protocol(expectedFacts)),
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V21_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V21_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V21_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V21_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V21_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V21_EVIDENCE_ROOT}/programs/cleanup-blueprint.js`,
    cleanupBlueprint,
  );

  const captureManifest = {
    artifactVersion: "table-live-v21-capture-manifest-v1",
  ...
  antecedentOutputs.set(
    TABLE_LIVE_V21_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: TableLiveV21ProofPlan = {
    artifactVersion: "table-live-v21-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: TABLE_LIVE_V21_TARGET,
    namespace: TABLE_FIGMA_NAMESPACE,
    writer: {
      programPath: `${TABLE_LIVE_V21_EVIDENCE_ROOT}/programs/writer.txt`,
    ...
    captureManifest: {
      path: TABLE_LIVE_V21_CAPTURE_MANIFEST_PATH,
      cells: 20,
      requests: 20,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(TABLE_LIVE_V21_PLAN_PATH, jsonBytes(proofPlan));

  const requests = {
    artifactVersion: "table-live-v21-request-manifest-v1",
    expectedDynamicTool: TABLE_LIVE_V21_DYNAMIC_TOOL,
    target: TABLE_LIVE_V21_TARGET,
  ...
  if (requests.requestCount !== TABLE_LIVE_V21_REMOTE_REQUESTS)
    throw new Error("Table live v21 remote request denominator drifted");
  antecedentOutputs.set(
    TABLE_LIVE_V21_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const lifecycleExcludedPaths = [
    TABLE_LIVE_V21_AUTHORIZATION_TEMPLATE_PATH,
    TABLE_LIVE_V21_AUTHORIZATION_PATH,
    "recipe/table-live-v21-authorization.ts",
    "recipe/table-live-v21-authorization.test.ts",
    "recipe/table-live-v21-preflight.ts",
    "recipe/table-live-v21-authorized.ts",
    "recipe/write-table-live-v21-authorization.ts",
    "recipe/create-table-live-v21-security-attestation.ts",
    "recipe/evidence/table-live-pivot-v21/operator-security-attestation-template.json",
    "recipe/evidence/table-live-pivot-v21-status.json",
    "recipe/evidence/status-index.json",
  ];
  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/table-live-v21-broker.ts",
    "recipe/table-live-v21-contract.ts",
    "recipe/table-live-v21-restore.ts",
    "recipe/table-live-v21-cleanup.ts",
    "recipe/table-live-v21-verifier.ts",
    "recipe/table-live-v21-fixed-point.ts",
    "recipe/run-table-live-v21.ts",
    "recipe/build-table-live-proof-v21.ts",
    "recipe/table-live-v21-lifecycle-simulation.ts",
    "recipe/scene-readback-table-v1.ts",
    "recipe/scene-readback-runtime-table-v21.ts",
    "recipe/table-figma-writer.ts",
    "recipe/figma-property-normalizer-v8.ts",
    "recipe/figma-runtime-portability.ts",
    "recipe/normalize.ts",
  ].filter((artifactPath) => artifactPath !== TABLE_LIVE_V21_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "table-live-v21-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
  ...
  antecedentOutputs.set(TABLE_LIVE_V21_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "table-live-v21-authorization-template-v1",
    authorizationId: "table-live-v21",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: TABLE_LIVE_V21_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
    ...
    denominator: {
      remoteRequests: TABLE_LIVE_V21_REMOTE_REQUESTS,
      hostPhases: TABLE_LIVE_V21_HOST_PHASES,
      captures: TABLE_LIVE_V21_CAPTURE_COUNT,
      sourceRoots: TABLE_LIVE_V21_SOURCE_ROOTS,
      expectedFacts,
    },
    execution: {
  ...
  const attestationTemplate = jsonBytes({
    artifactVersion: "table-live-v21-operator-security-attestation-template-v1",
    status: "template only; complete privately after PREPARE+AUTHORIZE",
    tokenValuesForbidden: true,
    privateDirectory: "private/table-live-v21-security-attestation.json",
  });
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Table live v21 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(TABLE_LIVE_V21_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(TABLE_LIVE_V21_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
    writeFileSync(
      `${TABLE_LIVE_V21_EVIDENCE_ROOT}/operator-security-attestation-template.json`,
      attestationTemplate,
    );
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(TABLE_LIVE_V21_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(TABLE_LIVE_V21_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Table live v21 authorization template drift");
  }
  if (
    writer.namespace !== TABLE_FIGMA_NAMESPACE ||
    writer.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    ...
    throw new Error("Table live v21 identity or cleanup invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildTableLiveV21Proof(), null, 2)}\n`,
  );
