...
import {
  ...
  createInputLiveV81FakeDynamicResponse,
  ...
  listInputLiveV81PersistedRequests,
  persistInputLiveV81RawResponse,
  persistInputLiveV81TechnicalGates,
  validateInputLiveV81Receipt,
  validateInputLiveV81Request,
  type InputLiveV81RemotePhase,
...
import {
  ...
  type InputLiveV81ExtractPayload,
  type InputLiveV81ProbePayload,
...
import {
  ...
  simulatedInputLiveV81Authorization,
} from "./run-input-field-live-v81.js";
...
const captures: InputLiveV81CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV81Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v81-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV81Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV81CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV81FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV81FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V81_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V81_TARGET);
  persistInputLiveV81RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV81Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV81Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV81WriterOwnership,
...
const rootProofs = (): InputLiveV81RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v81-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV81Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV81Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV81RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV81RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV81ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV81ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV81CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV81CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV81CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV81Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV81CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV81PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV81ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV81Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV81Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV81Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV81Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V81_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV81RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V81_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV81Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV81Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV81ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV81Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV81WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV81RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV81Request;
    validateInputLiveV81Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV81Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV81CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v81-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV81Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV81Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV81Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V81_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV81RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V81_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV81Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV81Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV81RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V81_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV81Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV81Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV81Request;
