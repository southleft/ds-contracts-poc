...
import {
  validateCalendarLiveV29AntecedentIndex,
  verifyCalendarLiveV29Authorization,
  type CalendarLiveV29AntecedentIndex,
  type CalendarLiveV29AuthorizationProof,
} from "./calendar-live-v29-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V29_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v29/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V29_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v29-security-attestation.json";

export interface CalendarLiveV29SecurityAttestation {
...
export interface CalendarLiveV29PreflightReport {
  artifactVersion: "calendar-live-v29-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV29SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV29SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV29AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v29-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V29_TARGET) ||
...
export function validateCalendarLiveV29ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV29Authorization");
  const preflight = source.indexOf("runCalendarLiveV29Preflight");
  const initialize = source.indexOf("CalendarLiveV29Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV29Preflight(
  root: string,
  proof: CalendarLiveV29AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V29_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV29PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V29_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V29_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV29AntecedentIndex;
  failures.push(...validateCalendarLiveV29AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V29_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V29_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v29-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV29ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v29 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV29SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v29-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V29_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV29Preflight(
    process.cwd(),
    verifyCalendarLiveV29Authorization(),
    attestationPath,
