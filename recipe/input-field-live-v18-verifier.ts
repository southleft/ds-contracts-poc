...
export const INPUT_LIVE_V18_VERIFIER_VERSION = "input-live-v18-verifier-v1";

export function buildInputLiveV18RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV18RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV18RawNode[];
}

export interface InputLiveV18NormalizedScene {
...
export interface InputLiveV18SceneProof {
...
export function normalizeInputLiveV18Scene(
  root: InputLiveV18RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV18NormalizedScene {
  const canonicalBindings: InputLiveV18NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV18RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV18Scene<Instance>(
  normalized: InputLiveV18NormalizedScene,
...
export function compileInputLiveV18ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV18PreCaptureGates(payload: unknown): void {
