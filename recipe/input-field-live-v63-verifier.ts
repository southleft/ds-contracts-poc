...
export const INPUT_LIVE_V63_VERIFIER_VERSION = "input-live-v63-verifier-v1";

export function buildInputLiveV63RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV63RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV63RawNode[];
}

export interface InputLiveV63NormalizedScene {
...
export interface InputLiveV63SceneProof {
...
export function normalizeInputLiveV63Scene(
  root: InputLiveV63RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV63NormalizedScene {
  const canonicalBindings: InputLiveV63NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV63RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV63Scene<Instance>(
  normalized: InputLiveV63NormalizedScene,
...
export function compileInputLiveV63ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV63PreCaptureGates(payload: unknown): void {
