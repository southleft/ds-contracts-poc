...
import { buildCalendarLiveV42CleanupRuntime } from "./calendar-live-v42-cleanup.js";
import { type CalendarLiveV42FixedFixedPoint } from "./calendar-live-v42-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV42RestoreProgram,
  validateCalendarLiveV42RestorePayload,
  type CalendarLiveV42RestorePayload,
} from "./calendar-live-v42-restore.js";
import {
  buildCalendarLiveV42RawPropertyRuntime,
  normalizeCalendarLiveV42Scene,
  type CalendarLiveV42RawNode,
} from "./calendar-live-v42-verifier.js";
...
export const CALENDAR_LIVE_V42_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V42_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V42_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V42_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V42_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V42_SET_COUNT = 3;
export const CALENDAR_LIVE_V42_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V42_HOST_PHASES = 3;
export const CALENDAR_LIVE_V42_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V42_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V42_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V42_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV42SourceId = (typeof CALENDAR_LIVE_V42_SOURCE_IDS)[number];
export type CalendarLiveV42Kind = "calendar" | "week" | "day";

export interface CalendarLiveV42SourceIdentity {
  source: CalendarLiveV42SourceId;
...
export interface CalendarLiveV42WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV42ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV42SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV42RawNode;
    weekScene: CalendarLiveV42RawNode;
    dayScene: CalendarLiveV42RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV42RootProof {
  source: CalendarLiveV42SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV42FixedFixedPoint;
}

export interface CalendarLiveV42ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV42SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV42CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV42SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV42CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV42SourceId;
  frameWidth: number;
...
export interface CalendarLiveV42CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v42 ${label} must be an object`);
...
export function validateCalendarLiveV42WriterPayload(
  value: unknown,
): CalendarLiveV42WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v42") ||
    ...
    result.namespace !== CALENDAR_LIVE_V42_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v42 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v42 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v42 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV42SourceIdentity,
  kind: CalendarLiveV42Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV42ExtractProgram(
  writer: CalendarLiveV42WriterOwnership,
  sources: readonly CalendarLiveV42SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v42 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V42_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V42_NAMESPACE)}
${buildCalendarLiveV42RawPropertyRuntime()}
...
export function validateCalendarLiveV42ExtractPayload(
  value: unknown,
  writer: CalendarLiveV42WriterOwnership,
): CalendarLiveV42ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v42 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V42_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v42 extract schema/one-root mismatch");
  return payload as CalendarLiveV42ExtractPayload;
}

export function proveCalendarLiveV42Roots<Instance>(
  extract: CalendarLiveV42ExtractPayload,
  sources: readonly (CalendarLiveV42SourceIdentity & {
...
): CalendarLiveV42RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v42 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v42 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV42Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV42Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV42Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV42FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v42 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v42 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v42 compile lost owned sets");
...
export function assertCalendarLiveV42RootProofs(
  proofs: readonly CalendarLiveV42RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v42 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v42 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV42ProbeProgram(
  writer: CalendarLiveV42WriterOwnership,
  sources: readonly CalendarLiveV42SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V42_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV42ProbePayload(
  value: unknown,
  writer: CalendarLiveV42WriterOwnership,
): CalendarLiveV42ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV42ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V42_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V42_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v42 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV42CaptureManifest(
  cells: readonly CalendarLiveV42CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V42_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V42_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V42_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V42_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v42 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV42CaptureCell[],
): string => {
  validateCalendarLiveV42CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV42CaptureProgram(
  writer: CalendarLiveV42WriterOwnership,
  cell: CalendarLiveV42CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V42_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v42 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v42 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V42_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV42CapturePayload(
  value: unknown,
  cell: CalendarLiveV42CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV42CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV42CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V42_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V42_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v42 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV42CaptureResponses(
  manifest: readonly CalendarLiveV42CaptureCell[],
  responses: readonly CalendarLiveV42CapturePayload[],
): void {
  validateCalendarLiveV42CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v42 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV42ObjectiveReport {
  artifactVersion: "calendar-live-v42-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV42Objective(
  manifest: readonly CalendarLiveV42CaptureCell[],
  responses: readonly CalendarLiveV42CapturePayload[],
): CalendarLiveV42ObjectiveReport {
  assertCalendarLiveV42CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v42-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV42CleanupProgram(
  writer: CalendarLiveV42WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV42CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V42_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v42 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV42CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV42WriterOwnership,
): CalendarLiveV42CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV42CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v42 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V42_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV42WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
