...
export const INPUT_LIVE_V15_VERIFIER_VERSION = "input-live-v15-verifier-v1";

export function buildInputLiveV15RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV15RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV15RawNode[];
}

export interface InputLiveV15NormalizedScene {
...
export interface InputLiveV15SceneProof {
...
export function normalizeInputLiveV15Scene(
  root: InputLiveV15RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV15NormalizedScene {
  const canonicalBindings: InputLiveV15NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV15RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV15Scene<Instance>(
  normalized: InputLiveV15NormalizedScene,
...
export function compileInputLiveV15ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV15PreCaptureGates(payload: unknown): void {
