...
import { verifyComboboxLiveV8Authorization } from "./combobox-live-v8-authorization.js";
import { COMBOBOX_LIVE_V8_TARGET } from "./combobox-live-v8-broker.js";
import {
  COMBOBOX_LIVE_V8_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV8Authorization();
...
const attestation: ComboboxLiveV8SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV8SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v8 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V8_SECURITY_ATTESTATION_DEFAULT_PATH,
);
