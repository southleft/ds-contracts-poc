...
export type TableLiveV27HistoryExpectation = "pending" | "authorized";

export interface TableLiveV27AuthorizationArtifact {
  artifactVersion: "table-live-v27-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V27_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV27AntecedentIndex {
...
export interface TableLiveV27HistoryState {
  index?: TableLiveV27AntecedentIndex;
  indexSha256: string;
...
export interface TableLiveV27AuthorizationProof
  extends TableLiveV27TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V27_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V27_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V27_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V27_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v27-authorization") ||
  artifactPath.includes("table-live-v27-preflight") ||
  artifactPath.includes("table-live-v27-authorized") ||
  artifactPath.includes("create-table-live-v27-security-attestation") ||
  artifactPath.includes("table-live-pivot-v27-status") ||
...
export function validateTableLiveV27AntecedentIndex(
  index: TableLiveV27AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v27-antecedent-index-v1" ||
...
export function validateTableLiveV27UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV27History\(\s*["']/.test(source) ||
    /readTableLiveV27HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV27AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV27AuthorizationArtifact {
  ...
  ) as TableLiveV27AntecedentIndex;
  const indexFailures = validateTableLiveV27AntecedentIndex(index);
...
export function validateTableLiveV27AuthorizationArtifact(
  artifact: TableLiveV27AuthorizationArtifact | undefined,
  index: TableLiveV27AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v27-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v27" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V27_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V27_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V27_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V27_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V27_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V27_SOURCE_ROOTS ||
...
export function validateTableLiveV27History(
  state: TableLiveV27HistoryState,
  expected: TableLiveV27HistoryExpectation,
): string[] {
  const failures = validateTableLiveV27AntecedentIndex(state.index);
...
export function readTableLiveV27HistoryState(
  root = process.cwd(),
): TableLiveV27HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V27_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV27AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V27_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V27_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V27_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV27History(
  expected: TableLiveV27HistoryExpectation,
  root = process.cwd(),
): TableLiveV27HistoryState {
  const state = readTableLiveV27HistoryState(root);
  const failures = validateTableLiveV27History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v27 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV27Authorization(
  root = process.cwd(),
): TableLiveV27AuthorizationProof {
  const state = verifyTableLiveV27History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V27_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v27/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v27.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V27_TARGET,
    expectedDynamicTool: TABLE_LIVE_V27_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V27_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV27HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV27History(expected);
  process.stdout.write(
    `Table live v27 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
