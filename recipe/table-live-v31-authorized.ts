...
import { TABLE_LIVE_V31_AUTHORIZATION_PATH } from "./build-table-live-proof-v31.js";
import { verifyTableLiveV31Authorization } from "./table-live-v31-authorization.js";
import { runTableLiveV31Preflight } from "./table-live-v31-preflight.js";
import { TableLiveV31Orchestrator } from "./run-table-live-v31.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v31 requires exact authorization artifact ${TABLE_LIVE_V31_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v31-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV31Authorization(root);
    runTableLiveV31Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV31Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV31Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
