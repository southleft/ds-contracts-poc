...
import { buildTableLiveV31CleanupRuntime } from "./table-live-v31-cleanup.js";
import { type TableLiveV31FixedFixedPoint } from "./table-live-v31-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV31RestoreProgram,
  validateTableLiveV31RestorePayload,
  type TableLiveV31RestorePayload,
} from "./table-live-v31-restore.js";
import {
  buildTableLiveV31RawPropertyRuntime,
  normalizeTableLiveV31Scene,
  type TableLiveV31RawNode,
} from "./table-live-v31-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v31.js";
import {
...
export const TABLE_LIVE_V31_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V31_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V31_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V31_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V31_VARIANT_COUNT = 20;
export const TABLE_LIVE_V31_SET_COUNT = 6;
export const TABLE_LIVE_V31_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V31_HOST_PHASES = 3;
export const TABLE_LIVE_V31_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V31_RESTORE_COUNT = 8;
export const TABLE_LIVE_V31_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V31_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV31SourceId = (typeof TABLE_LIVE_V31_SOURCE_IDS)[number];
export type TableLiveV31Kind = "table" | "row" | "cell";

export interface TableLiveV31SourceIdentity {
  source: TableLiveV31SourceId;
...
export interface TableLiveV31WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV31ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV31SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV31RawNode;
    rowScene: TableLiveV31RawNode;
    cellScene: TableLiveV31RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV31RootProof {
  source: TableLiveV31SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV31FixedFixedPoint;
}

export interface TableLiveV31ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV31SourceId;
    adapterIdentity: string;
...
export interface TableLiveV31CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV31SourceId;
  adapterIdentity: string;
...
export interface TableLiveV31CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV31SourceId;
  frameWidth: number;
...
export interface TableLiveV31CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v31 ${label} must be an object`);
...
export function validateTableLiveV31WriterPayload(
  value: unknown,
): TableLiveV31WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V31_NAMESPACE ||
    ...
    throw new TypeError("Table live v31 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Table live v31 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v31 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV31SourceIdentity,
  kind: TableLiveV31Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV31ExtractProgram(
  writer: TableLiveV31WriterOwnership,
  sources: readonly TableLiveV31SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
      "Table live v31 extract requires two source identities",
    );
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V31_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V31_NAMESPACE)}
${buildTableLiveV31RawPropertyRuntime()}
...
export function validateTableLiveV31ExtractPayload(
  value: unknown,
  writer: TableLiveV31WriterOwnership,
): TableLiveV31ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v31 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V31_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v31 extract schema/two-root mismatch");
  return payload as TableLiveV31ExtractPayload;
}

export function proveTableLiveV31Roots<Instance>(
  extract: TableLiveV31ExtractPayload,
  sources: readonly (TableLiveV31SourceIdentity & {
...
): TableLiveV31RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v31 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v31 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV31Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV31Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV31Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV31FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Table live v31 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v31 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v31 compile lost owned sets");
...
export function assertTableLiveV31RootProofs(
  proofs: readonly TableLiveV31RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v31 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v31 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV31ProbeProgram(
  writer: TableLiveV31WriterOwnership,
  sources: readonly TableLiveV31SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V31_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV31ProbePayload(
  value: unknown,
  writer: TableLiveV31WriterOwnership,
): TableLiveV31ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV31ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V31_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v31 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV31CaptureManifest(
  cells: readonly TableLiveV31CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V31_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V31_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V31_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V31_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v31 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV31CaptureCell[],
): string => {
  validateTableLiveV31CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV31CaptureProgram(
  writer: TableLiveV31WriterOwnership,
  cell: TableLiveV31CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V31_CAPTURE_COUNT
  )
    throw new TypeError("Table live v31 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Table live v31 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V31_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV31CapturePayload(
  value: unknown,
  cell: TableLiveV31CaptureCell,
  rawResponseBytes: number,
): TableLiveV31CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV31CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V31_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V31_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v31 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV31CaptureResponses(
  manifest: readonly TableLiveV31CaptureCell[],
  responses: readonly TableLiveV31CapturePayload[],
): void {
  validateTableLiveV31CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v31 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV31ObjectiveReport {
  artifactVersion: "table-live-v31-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV31Objective(
  manifest: readonly TableLiveV31CaptureCell[],
  responses: readonly TableLiveV31CapturePayload[],
): TableLiveV31ObjectiveReport {
  assertTableLiveV31CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v31-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV31CleanupProgram(
  writer: TableLiveV31WriterOwnership,
): string {
  const baseProgram = buildTableLiveV31CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V31_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v31 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV31CleanupPayload(
  payload: unknown,
  writer: TableLiveV31WriterOwnership,
): TableLiveV31CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as TableLiveV31CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v31 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V31_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV31WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
