...
import { TABLE_LIVE_V14_INDEX_PATH } from "./build-table-live-proof-v14.js";
import {
...
import {
  ...
  validateTableLiveV14History,
  validateTableLiveV14UnitTestSource,
  type TableLiveV14AntecedentIndex,
  type TableLiveV14AuthorizationArtifact,
  type TableLiveV14AuthorizationProof,
  type TableLiveV14HistoryState,
} from "./table-live-v14-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV14ControlFlowSource,
  validateTableLiveV14SecurityAttestation,
  type TableLiveV14SecurityAttestation,
} from "./table-live-v14-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V14_INDEX_PATH);
const index = (): TableLiveV14AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV14AntecedentIndex;

const artifact = (): TableLiveV14AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV14AuthorizationArtifact({
...
const pendingState = (): TableLiveV14HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV14HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV14AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v14/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV14AuthorizationArtifact,
): TableLiveV14SecurityAttestation => {
  const body: Omit<TableLiveV14SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V14_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV14History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV14History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV14History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV14History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV14History(authorizedState(), "pending").join("\n"),
    /stale history mode.*authorization exists/,
  );
});

test("the exact v6 phase-sensitive self-test defect is planted and table v14 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV14UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v14-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV14UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV14AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV14AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV14AuthorizationArtifact(
        planted as TableLiveV14AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV14SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV14SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV14SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v14-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV14ControlFlowSource(source), []);
});
