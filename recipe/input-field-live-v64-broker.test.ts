...
import {
  ...
  createInputLiveV64FakeDynamicResponse,
  ...
  listInputLiveV64PersistedRequests,
  persistInputLiveV64RawResponse,
  persistInputLiveV64TechnicalGates,
  validateInputLiveV64Receipt,
  validateInputLiveV64Request,
  type InputLiveV64RemotePhase,
...
import {
  ...
  type InputLiveV64ExtractPayload,
  type InputLiveV64ProbePayload,
  type InputLiveV64RestorePayload,
  type InputLiveV64RootProof,
  type InputLiveV64WriterOwnership,
} from "./input-field-live-v64-contract.js";
import {
  InputLiveV64Orchestrator,
  simulatedInputLiveV64Authorization,
} from "./run-input-field-live-v64.js";
...
const captures: InputLiveV64CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV64Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v64-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV64Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV64CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV64FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV64FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V64_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V64_TARGET);
  persistInputLiveV64RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV64Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV64Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV64WriterOwnership,
): InputLiveV64RestorePayload => ({
...
const rootProofs = (): InputLiveV64RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV64CaptureCell,
): InputLiveV64CapturePayload => {
  const bytes = Buffer.from(`input-live-v64-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV64Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV64Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV64RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV64RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV64ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV64ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV64CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV64CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV64CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV64Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV64CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV64PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV64ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV64Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV64Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV64Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV64Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V64_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV64RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V64_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV64Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV64Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV64ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV64Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV64WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV64RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV64Request;
    validateInputLiveV64Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV64Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV64CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v64-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV64Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV64Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV64Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V64_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV64RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V64_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV64Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV64Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV64RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V64_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV64Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV64Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV64Request;
