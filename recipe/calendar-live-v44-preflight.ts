...
import {
  validateCalendarLiveV44AntecedentIndex,
  verifyCalendarLiveV44Authorization,
  type CalendarLiveV44AntecedentIndex,
  type CalendarLiveV44AuthorizationProof,
} from "./calendar-live-v44-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V44_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v44/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V44_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v44-security-attestation.json";

export interface CalendarLiveV44SecurityAttestation {
...
export interface CalendarLiveV44PreflightReport {
  artifactVersion: "calendar-live-v44-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV44SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV44SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV44AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v44-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V44_TARGET) ||
...
export function validateCalendarLiveV44ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV44Authorization");
  const preflight = source.indexOf("runCalendarLiveV44Preflight");
  const initialize = source.indexOf("CalendarLiveV44Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV44Preflight(
  root: string,
  proof: CalendarLiveV44AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V44_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV44PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V44_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V44_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV44AntecedentIndex;
  failures.push(...validateCalendarLiveV44AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V44_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V44_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v44-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV44ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v44 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV44SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v44-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V44_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV44Preflight(
    process.cwd(),
    verifyCalendarLiveV44Authorization(),
    attestationPath,
