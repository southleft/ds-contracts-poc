...
import {
  ...
  verifyInputLiveV80Authorization,
  type InputLiveV80AntecedentIndex,
  type InputLiveV80AuthorizationProof,
} from "./input-field-live-v80-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V80_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v80/operator-security-attestation-template.json";
export const INPUT_LIVE_V80_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v80-security-attestation.json";

export interface InputLiveV80SecurityAttestation {
...
export interface InputLiveV80PreflightReport {
  artifactVersion: "input-live-v80-preflight-v1";
...
export const inputLiveV80AttestationSha256 = (
  value: Omit<InputLiveV80SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV80SecurityAttestation(
  value: unknown,
  proof: InputLiveV80AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v80-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V80_TARGET) ||
...
export function validateInputLiveV80ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV80Authorization");
  const preflight = source.indexOf("runInputLiveV80Preflight");
  const initialize = source.indexOf("InputLiveV80Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV80Preflight(
  root: string,
  proof: InputLiveV80AuthorizationProof,
  attestationPath = INPUT_LIVE_V80_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV80PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V80_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V80_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V80_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV80AntecedentIndex;
  failures.push(...validateInputLiveV80AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V80_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V80_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v80-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV80ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV80SecurityAttestation;
  return {
    artifactVersion: "input-live-v80-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V80_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV80Preflight(
    process.cwd(),
    verifyInputLiveV80Authorization(),
    attestationPath,
