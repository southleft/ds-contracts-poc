...
import { TABLE_LIVE_V21_AUTHORIZATION_PATH } from "./build-table-live-proof-v21.js";
import { verifyTableLiveV21Authorization } from "./table-live-v21-authorization.js";
import { runTableLiveV21Preflight } from "./table-live-v21-preflight.js";
import { TableLiveV21Orchestrator } from "./run-table-live-v21.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v21 requires exact authorization artifact ${TABLE_LIVE_V21_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v21-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV21Authorization(root);
    runTableLiveV21Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV21Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV21Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
