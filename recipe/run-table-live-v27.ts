...
import {
  acceptTableLiveV27Response,
  createTableLiveV27Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV27Request,
  persistTableLiveV27RawResponse,
  persistTableLiveV27TechnicalGates,
  readTableLiveV27AcceptedRawPayload,
  type TableLiveV27Request,
  type TableLiveV27TransactionAuthorization,
} from "./table-live-v27-broker.js";
import {
  TABLE_LIVE_V27_CAPTURE_COUNT,
  assertTableLiveV27CaptureResponses,
  ...
  evaluateTableLiveV27Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV27Roots,
  validateTableLiveV27CaptureManifest,
  validateTableLiveV27CapturePayload,
  validateTableLiveV27CleanupPayload,
  validateTableLiveV27ExtractPayload,
  validateTableLiveV27ProbePayload,
  validateTableLiveV27WriterPayload,
  type TableLiveV27CaptureCell,
  type TableLiveV27CapturePayload,
...
import {
  ...
  TABLE_LIVE_V27_PLAN_PATH,
  type TableLiveV27ProofPlan,
} from "./build-table-live-proof-v27.js";
import {
...
export const TABLE_LIVE_V27_RUNNER_VERSION = "table-live-v27-runner-v1";

interface TableLiveV27JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV27OrchestratorState {
  artifactVersion: "table-live-v27-orchestrator-state-v1";
  ...
  journal: TableLiveV27JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV27ProofPlan;
  planSha256: string;
  captures: TableLiveV27CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV27SourceIdentity & {
...
export interface TableLiveV27NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV27OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v27 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V27_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V27_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV27ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV27CaptureCell[];
  validateTableLiveV27CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v27 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v27 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV27WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV27RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV27Request =>
  readJson<TableLiveV27Request>(file);

export class TableLiveV27Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV27Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV27OrchestratorState = {
      artifactVersion: "table-live-v27-orchestrator-state-v1",
    ...
    return new TableLiveV27Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV27OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV27Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV27NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV27NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v27 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV27NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v27 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV27RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV27Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV27CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV27AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV27WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV27Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV27ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV27Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV27Response(this.#directory, "writer", {
        validate: validateTableLiveV27WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV27Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV27RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV27Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV27ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV27Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV27RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v27-host-root-proofs-v1",
      ...
      const next = issueTableLiveV27Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV27ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV27Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV27ProbePayload(payload, ownership),
      });
      persistTableLiveV27TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v27 capture cell missing");
      const accepted = acceptTableLiveV27Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV27CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V27_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV27CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV27Objective(
    ...
    throw new Error(`Table live v27 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV27Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV27CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV27Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV27RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV27Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV27CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV27CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV27CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV27Authorization = (
  privateKey: KeyObject,
): TableLiveV27TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV27SimulationKey = (): KeyObject =>
