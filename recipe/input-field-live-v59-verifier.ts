...
export const INPUT_LIVE_V59_VERIFIER_VERSION = "input-live-v59-verifier-v1";

export function buildInputLiveV59RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV59RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV59RawNode[];
}

export interface InputLiveV59NormalizedScene {
...
export interface InputLiveV59SceneProof {
...
export function normalizeInputLiveV59Scene(
  root: InputLiveV59RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV59NormalizedScene {
  const canonicalBindings: InputLiveV59NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV59RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV59Scene<Instance>(
  normalized: InputLiveV59NormalizedScene,
...
export function compileInputLiveV59ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV59PreCaptureGates(payload: unknown): void {
