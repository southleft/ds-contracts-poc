...
import {
  ...
  createInputLiveV68FakeDynamicResponse,
  ...
  listInputLiveV68PersistedRequests,
  persistInputLiveV68RawResponse,
  persistInputLiveV68TechnicalGates,
  validateInputLiveV68Receipt,
  validateInputLiveV68Request,
  type InputLiveV68RemotePhase,
...
import {
  ...
  type InputLiveV68ExtractPayload,
  type InputLiveV68ProbePayload,
...
import {
  ...
  simulatedInputLiveV68Authorization,
} from "./run-input-field-live-v68.js";
...
const captures: InputLiveV68CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV68Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v68-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV68Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV68CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV68FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV68FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V68_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V68_TARGET);
  persistInputLiveV68RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV68Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV68Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV68WriterOwnership,
): InputLiveV68RestorePayload => ({
...
const rootProofs = (): InputLiveV68RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v68-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV68Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV68Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV68RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV68RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV68ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV68ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV68CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV68CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV68CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV68Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV68CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV68PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV68ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV68Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV68Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV68Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV68Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V68_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV68RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V68_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV68Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV68Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV68ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV68Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV68WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV68RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV68Request;
    validateInputLiveV68Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV68Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV68CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v68-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV68Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV68Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV68Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V68_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV68RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V68_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV68Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV68Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV68RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V68_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV68Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV68Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV68Request;
