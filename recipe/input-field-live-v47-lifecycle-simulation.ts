...
export interface InputLiveV47LifecycleSimulationReport {
  artifactVersion: "input-live-v47-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV47AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV47Lifecycle(): Promise<InputLiveV47LifecycleSimulationReport> {
  await buildInputLiveV47Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V47_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV47AntecedentIndex;
  const indexSha256 = inputLiveV47Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v47-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V47_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V47_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV47History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV47History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV47AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V47_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V47_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV47History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV47History(authorized, "pending");
    ...
    await buildInputLiveV47Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V47_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV47AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV47Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV47Lifecycle(), null, 2)}\n`,
  );
