...
export interface InputLiveV17LifecycleSimulationReport {
  artifactVersion: "input-live-v17-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV17AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV17Lifecycle(): Promise<InputLiveV17LifecycleSimulationReport> {
  await buildInputLiveV17Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V17_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV17AntecedentIndex;
  const indexSha256 = inputLiveV17Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v17-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V17_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V17_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV17History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV17History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV17AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V17_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V17_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV17History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV17History(authorized, "pending");
    ...
    await buildInputLiveV17Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V17_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV17AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV17Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV17Lifecycle(), null, 2)}\n`,
  );
