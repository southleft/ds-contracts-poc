...
import { INPUT_LIVE_V76_INDEX_PATH } from "./build-input-field-live-proof-v76.js";
import {
  INPUT_LIVE_V76_DYNAMIC_TOOL,
  INPUT_LIVE_V76_TARGET,
} from "./input-field-live-v76-broker.js";
import {
  buildInputLiveV76AuthorizationArtifact,
  validateInputLiveV76AntecedentIndex,
  validateInputLiveV76AuthorizationArtifact,
  validateInputLiveV76History,
  validateInputLiveV76UnitTestSource,
  type InputLiveV76AntecedentIndex,
  type InputLiveV76AuthorizationArtifact,
  type InputLiveV76AuthorizationProof,
  type InputLiveV76HistoryState,
} from "./input-field-live-v76-authorization.js";
import {
  inputLiveV76AttestationSha256,
  validateInputLiveV76ControlFlowSource,
  validateInputLiveV76SecurityAttestation,
  type InputLiveV76SecurityAttestation,
} from "./input-field-live-v76-preflight.js";
import { inputLiveV76Sha256 } from "./input-field-live-v76-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V76_INDEX_PATH);
const index = (): InputLiveV76AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV76AntecedentIndex;

const artifact = (): InputLiveV76AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV76AuthorizationArtifact({
...
const pendingState = (): InputLiveV76HistoryState => ({
  index: index(),
  indexSha256: inputLiveV76Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV76HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV76AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV76Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V76_TARGET,
  expectedDynamicTool: INPUT_LIVE_V76_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v76/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV76AuthorizationArtifact,
): InputLiveV76SecurityAttestation => {
  const body: Omit<InputLiveV76SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v76-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V76_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV76AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV76History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV76History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV76History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV76History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV76History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV76UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV76UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV76AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV76Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV76AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV76AuthorizationArtifact(
        planted as InputLiveV76AuthorizationArtifact,
        index(),
        inputLiveV76Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV76SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV76AttestationSha256(
      body as Omit<InputLiveV76SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV76SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV76ControlFlowSource(source), []);
});
