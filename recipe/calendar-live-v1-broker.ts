...
import {
  assertCalendarLiveV1RootProofs,
  CALENDAR_LIVE_V1_CAPTURE_COUNT,
  CALENDAR_LIVE_V1_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  CALENDAR_LIVE_V1_RESPONSE_CONTRACTS,
  type CalendarLiveV1RootProof,
} from "./calendar-live-v1-contract.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V1_BROKER_VERSION = "calendar-live-v1-broker-v1";
export const CALENDAR_LIVE_V1_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const CALENDAR_LIVE_V1_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const CALENDAR_LIVE_V1_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const CALENDAR_LIVE_V1_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const CALENDAR_LIVE_V1_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const CALENDAR_LIVE_V1_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type CalendarLiveV1RemotePhase =
  (typeof CALENDAR_LIVE_V1_REMOTE_PHASES)[number];
export type CalendarLiveV1AuthorizationMode = "live" | "simulated";

export interface CalendarLiveV1CommitPins {
...
export interface CalendarLiveV1TransactionAuthorization extends CalendarLiveV1CommitPins {
  mode: CalendarLiveV1AuthorizationMode;
...
export interface CalendarLiveV1Manifest {
  artifactVersion: "calendar-live-v1-broker-manifest-v2";
  brokerVersion: typeof CALENDAR_LIVE_V1_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof CALENDAR_LIVE_V1_DYNAMIC_TOOL;
  target: typeof CALENDAR_LIVE_V1_TARGET;
  authorization: CalendarLiveV1TransactionAuthorization;
...
export interface CalendarLiveV1ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface CalendarLiveV1RequestBody {
  artifactVersion: "calendar-live-v1-broker-request-v2";
  brokerVersion: typeof CALENDAR_LIVE_V1_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: CalendarLiveV1RemotePhase;
  sequence: number;
  ...
  commitPins: CalendarLiveV1CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof CALENDAR_LIVE_V1_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: CalendarLiveV1ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface CalendarLiveV1Request extends CalendarLiveV1RequestBody {
...
export interface CalendarLiveV1Receipt {
  artifactVersion: "calendar-live-v1-broker-receipt-v2";
  brokerVersion: typeof CALENDAR_LIVE_V1_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: CalendarLiveV1RemotePhase;
...
export interface CalendarLiveV1AcceptedResponse<T = unknown> {
  request: CalendarLiveV1Request;
  receipt: CalendarLiveV1Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface CalendarLiveV1IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: CalendarLiveV1ResponseContract;
...
const equalTarget = (value: unknown): value is typeof CALENDAR_LIVE_V1_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === CALENDAR_LIVE_V1_TARGET.fileKey &&
  (value as BrokerTarget).fileName === CALENDAR_LIVE_V1_TARGET.fileName &&
  (value as BrokerTarget).editorType === CALENDAR_LIVE_V1_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== CALENDAR_LIVE_V1_TARGET.fileKey ||
    context.fileName !== CALENDAR_LIVE_V1_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === CALENDAR_LIVE_V1_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Calendar live v1 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= CALENDAR_LIVE_V1_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v1 capture index must be 0..7");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Calendar live v1 authorization pins are malformed");
};

const readManifest = (directory: string): CalendarLiveV1Manifest => {
  const manifest = json<CalendarLiveV1Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Calendar live v1 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "calendar-live-v1-broker-manifest-v2" ||
    manifest.brokerVersion !== CALENDAR_LIVE_V1_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== CALENDAR_LIVE_V1_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(CALENDAR_LIVE_V1_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Calendar live v1 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: CalendarLiveV1Request,
): CalendarLiveV1RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: CalendarLiveV1RemotePhase,
  captureIndex?: number,
): CalendarLiveV1Request | undefined => {
  const file = calendarLiveV1RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV1Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: CalendarLiveV1RemotePhase,
  captureIndex?: number,
): CalendarLiveV1Receipt | undefined => {
  const file = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV1Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: CalendarLiveV1Request | undefined;
  receipt: CalendarLiveV1Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: CalendarLiveV1RemotePhase,
): CalendarLiveV1ResponseContract => ({
  ...CALENDAR_LIVE_V1_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: CalendarLiveV1Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: CALENDAR_LIVE_V1_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: CALENDAR_LIVE_V1_DYNAMIC_TOOL,
      target: CALENDAR_LIVE_V1_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "calendar-live-v1-plugin-response-v2",
  brokerVersion: CALENDAR_LIVE_V1_BROKER_VERSION,
  target: CALENDAR_LIVE_V1_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*CALENDAR_LIVE_V1_PROGRAM_START*/${program}/*CALENDAR_LIVE_V1_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*CALENDAR_LIVE_V1_PROGRAM_START*/";
  const endMarker = "/*CALENDAR_LIVE_V1_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Calendar live v1 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createCalendarLiveV1Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Calendar live v1 private signing key does not match authorized public-key identity",
    );
  const manifest: CalendarLiveV1Manifest = {
    artifactVersion: "calendar-live-v1-broker-manifest-v2",
    brokerVersion: CALENDAR_LIVE_V1_BROKER_VERSION,
    ...
    captureRequests: CALENDAR_LIVE_V1_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateCalendarLiveV1Request(
  directory: string,
  request: CalendarLiveV1Request,
): void {
  ...
  if (
    request.artifactVersion !== "calendar-live-v1-broker-request-v2" ||
    request.brokerVersion !== CALENDAR_LIVE_V1_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== CALENDAR_LIVE_V1_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Calendar live v1 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueCalendarLiveV1Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: CalendarLiveV1RemotePhase,
  program: string,
  options: CalendarLiveV1IssueOptions = {},
): CalendarLiveV1Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<CalendarLiveV1Request>(targetPath);
    validateCalendarLiveV1Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Calendar live v1 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(calendarLiveV1ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Calendar live v1 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Calendar live v1 empty code envelope refused");
  if (phase === "capture") validateCalendarLiveV1TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Calendar live v1 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: CALENDAR_LIVE_V1_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? CALENDAR_LIVE_V1_SIGNED_WRITER_TIMEOUT_MS
        : CALENDAR_LIVE_V1_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: CalendarLiveV1RequestBody = {
    artifactVersion: "calendar-live-v1-broker-request-v2",
    brokerVersion: CALENDAR_LIVE_V1_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: CalendarLiveV1Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateCalendarLiveV1Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistCalendarLiveV1RawResponse(
  directory: string,
  phase: CalendarLiveV1RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Calendar live v1 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<CalendarLiveV1Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Calendar live v1 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = calendarLiveV1ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Calendar live v1 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): CalendarLiveV1Receipt => {
  const body: Omit<CalendarLiveV1Receipt, "receiptSha256"> = {
    artifactVersion: "calendar-live-v1-broker-receipt-v2",
    brokerVersion: CALENDAR_LIVE_V1_BROKER_VERSION,
...
export function acceptCalendarLiveV1Response<T = unknown>(
  directory: string,
  phase: CalendarLiveV1RemotePhase,
  options: {
    captureIndex?: number;
...
): CalendarLiveV1AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Calendar live v1 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<CalendarLiveV1Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Calendar live v1 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "calendar-live-v1-plugin-response-v2" ||
    plugin?.brokerVersion !== CALENDAR_LIVE_V1_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Calendar live v1 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistCalendarLiveV1TechnicalGates(
  directory: string,
  rootProofs: readonly CalendarLiveV1RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertCalendarLiveV1RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Calendar live v1 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Calendar live v1 probe payload hash malformed");
  const gates = Object.fromEntries(
    CALENDAR_LIVE_V1_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "calendar-live-v1-host-technical-gates-v2",
    brokerVersion: CALENDAR_LIVE_V1_BROKER_VERSION,
...
export function validateCalendarLiveV1TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "calendar-live-v1-host-technical-gates-v2" ||
    ...
    CALENDAR_LIVE_V1_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Calendar live v1 technical gates artifact");
}

export function validateCalendarLiveV1Receipt(
  directory: string,
  phase: CalendarLiveV1RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateCalendarLiveV1Request(directory, request);
  const receipt = json<CalendarLiveV1Receipt>(
  ...
  if (
    receipt.artifactVersion !== "calendar-live-v1-broker-receipt-v2" ||
    receipt.brokerVersion !== CALENDAR_LIVE_V1_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Calendar live v1 accepted receipt: ${request.requestId}`,
    );
}

export function readCalendarLiveV1AcceptedRawPayload(
  directory: string,
  phase: CalendarLiveV1RemotePhase,
  captureIndex?: number,
): unknown {
  validateCalendarLiveV1Receipt(directory, phase, captureIndex);
...
export function createCalendarLiveV1FakeDynamicResponse(
  request: CalendarLiveV1Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? CALENDAR_LIVE_V1_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? CALENDAR_LIVE_V1_TARGET,
    ...overrides.extra,
  };
}

export function listCalendarLiveV1PersistedRequests(
  directory: string,
): CalendarLiveV1Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<CalendarLiveV1Request>(file)] : [];
    });
}
