...
import { CALENDAR_LIVE_V15_INDEX_PATH } from "./build-calendar-live-proof-v15.js";
import {
...
import {
  buildCalendarLiveV15AuthorizationArtifact,
  validateCalendarLiveV15AntecedentIndex,
  validateCalendarLiveV15AuthorizationArtifact,
  validateCalendarLiveV15History,
  validateCalendarLiveV15UnitTestSource,
  type CalendarLiveV15AntecedentIndex,
  type CalendarLiveV15AuthorizationArtifact,
  type CalendarLiveV15AuthorizationProof,
  type CalendarLiveV15HistoryState,
} from "./calendar-live-v15-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV15ControlFlowSource,
  validateCalendarLiveV15SecurityAttestation,
  type CalendarLiveV15SecurityAttestation,
} from "./calendar-live-v15-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V15_INDEX_PATH);
const index = (): CalendarLiveV15AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV15AntecedentIndex;

const artifact = (): CalendarLiveV15AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV15AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV15HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV15HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV15AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v15/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV15AuthorizationArtifact,
): CalendarLiveV15SecurityAttestation => {
  const body: Omit<CalendarLiveV15SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V15_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV15History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV15History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV15History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV15History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV15History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV15UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v15-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV15UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV15AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV15AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV15AuthorizationArtifact(
        planted as CalendarLiveV15AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV15SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV15SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV15SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v15-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV15ControlFlowSource(source), []);
});
