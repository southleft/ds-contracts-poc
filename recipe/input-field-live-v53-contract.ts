...
} from "./input-field-live-v3-verifier-v53.js";
import {
  buildInputLiveV53RawPropertyRuntime,
  normalizeInputLiveV53Scene,
  type InputLiveV53RawNode,
} from "./input-field-live-v53-verifier.js";
...
export const INPUT_LIVE_V53_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V53_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V53_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V53_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V53_VARIANT_COUNT = 256;
export const INPUT_LIVE_V53_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V53_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV53SourceId = (typeof INPUT_LIVE_V53_SOURCE_IDS)[number];

export interface InputLiveV53SourceIdentity {
  source: InputLiveV53SourceId;
...
export interface InputLiveV53WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV53ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV53SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV53RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV53RootProof {
  source: InputLiveV53SourceId;
  adapterIdentity: string;
...
export interface InputLiveV53ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV53SourceId;
    adapterIdentity: string;
...
export interface InputLiveV53CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV53SourceId;
  adapterIdentity: string;
...
export interface InputLiveV53CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV53SourceId;
  strata: InputLiveV53CaptureCell["strata"];
...
export interface InputLiveV53CleanupPayload {
...
export function validateInputLiveV53WriterPayload(
  value: unknown,
): InputLiveV53WriterOwnership {
...
const identityPlan = (source: InputLiveV53SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV53ExtractProgram(
  writer: InputLiveV53WriterOwnership,
  sources: readonly InputLiveV53SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V53_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V53_NAMESPACE)}
${buildInputLiveV53RawPropertyRuntime()}
...
export function validateInputLiveV53ExtractPayload(
  value: unknown,
  writer: InputLiveV53WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V53_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV53ExtractPayload;
}

export function proveInputLiveV53Roots<Instance>(
  extract: InputLiveV53ExtractPayload,
  sources: readonly (InputLiveV53SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV53Scene(
      root.scene,
...
export function assertInputLiveV53RootProofs(
  proofs: readonly InputLiveV53RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV53ProbeProgram(
  writer: InputLiveV53WriterOwnership,
  sources: readonly InputLiveV53SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V53_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV53ProbePayload(
  value: unknown,
  writer: InputLiveV53WriterOwnership,
): InputLiveV53ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV53ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V53_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V53_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV53CaptureManifest(
  cells: readonly InputLiveV53CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V53_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V53_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V53_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V53_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V53_SOURCE_IDS.some(
...
export const inputLiveV53CaptureManifestSha256 = (
  cells: readonly InputLiveV53CaptureCell[],
): string => {
  validateInputLiveV53CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV53CaptureProgram(
  writer: InputLiveV53WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V53_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V53_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V53_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV53CapturePayload(
  value: unknown,
  cell: InputLiveV53CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV53CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V53_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V53_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV53CaptureResponses(
  manifest: readonly InputLiveV53CaptureCell[],
  responses: readonly InputLiveV53CapturePayload[],
): void {
  validateInputLiveV53CaptureManifest(manifest);
  if (
...
export interface InputLiveV53ObjectiveReport {
  artifactVersion: "input-live-v53-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV53Objective(
  manifest: readonly InputLiveV53CaptureCell[],
  responses: readonly InputLiveV53CapturePayload[],
  readReference: (cell: InputLiveV53CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV53CaptureCell) => Buffer,
): InputLiveV53ObjectiveReport {
  assertInputLiveV53CaptureResponses(manifest, responses);
...
export function buildInputLiveV53CleanupProgram(
  writer: InputLiveV53WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V53_NAMESPACE,
...
export function validateInputLiveV53CleanupPayload(
  value: unknown,
  writer: InputLiveV53WriterOwnership,
): InputLiveV53CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV53CleanupPayload;
...
export const INPUT_LIVE_V53_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV53WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
