...
import {
  assertTableLiveV35RootProofs,
  TABLE_LIVE_V35_CAPTURE_COUNT,
  TABLE_LIVE_V35_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  TABLE_LIVE_V35_RESPONSE_CONTRACTS,
  type TableLiveV35RootProof,
} from "./table-live-v35-contract.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V35_BROKER_VERSION = "table-live-v35-broker-v1";
export const TABLE_LIVE_V35_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const TABLE_LIVE_V35_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const TABLE_LIVE_V35_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const TABLE_LIVE_V35_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const TABLE_LIVE_V35_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const TABLE_LIVE_V35_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type TableLiveV35RemotePhase =
  (typeof TABLE_LIVE_V35_REMOTE_PHASES)[number];
export type TableLiveV35AuthorizationMode = "live" | "simulated";

export interface TableLiveV35CommitPins {
...
export interface TableLiveV35TransactionAuthorization extends TableLiveV35CommitPins {
  mode: TableLiveV35AuthorizationMode;
...
export interface TableLiveV35Manifest {
  artifactVersion: "table-live-v35-broker-manifest-v2";
  brokerVersion: typeof TABLE_LIVE_V35_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof TABLE_LIVE_V35_DYNAMIC_TOOL;
  target: typeof TABLE_LIVE_V35_TARGET;
  authorization: TableLiveV35TransactionAuthorization;
...
export interface TableLiveV35ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface TableLiveV35RequestBody {
  artifactVersion: "table-live-v35-broker-request-v2";
  brokerVersion: typeof TABLE_LIVE_V35_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: TableLiveV35RemotePhase;
  sequence: number;
  ...
  commitPins: TableLiveV35CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof TABLE_LIVE_V35_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: TableLiveV35ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface TableLiveV35Request extends TableLiveV35RequestBody {
...
export interface TableLiveV35Receipt {
  artifactVersion: "table-live-v35-broker-receipt-v2";
  brokerVersion: typeof TABLE_LIVE_V35_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: TableLiveV35RemotePhase;
...
export interface TableLiveV35AcceptedResponse<T = unknown> {
  request: TableLiveV35Request;
  receipt: TableLiveV35Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface TableLiveV35IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: TableLiveV35ResponseContract;
...
const equalTarget = (value: unknown): value is typeof TABLE_LIVE_V35_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === TABLE_LIVE_V35_TARGET.fileKey &&
  (value as BrokerTarget).fileName === TABLE_LIVE_V35_TARGET.fileName &&
  (value as BrokerTarget).editorType === TABLE_LIVE_V35_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== TABLE_LIVE_V35_TARGET.fileKey ||
    context.fileName !== TABLE_LIVE_V35_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === TABLE_LIVE_V35_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Table live v35 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= TABLE_LIVE_V35_CAPTURE_COUNT
  )
    throw new TypeError("Table live v35 capture index must be 0..19");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Table live v35 authorization pins are malformed");
};

const readManifest = (directory: string): TableLiveV35Manifest => {
  const manifest = json<TableLiveV35Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Table live v35 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "table-live-v35-broker-manifest-v2" ||
    manifest.brokerVersion !== TABLE_LIVE_V35_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== TABLE_LIVE_V35_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V35_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Table live v35 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: TableLiveV35Request,
): TableLiveV35RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: TableLiveV35RemotePhase,
  captureIndex?: number,
): TableLiveV35Request | undefined => {
  const file = tableLiveV2RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV35Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: TableLiveV35RemotePhase,
  captureIndex?: number,
): TableLiveV35Receipt | undefined => {
  const file = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV35Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: TableLiveV35Request | undefined;
  receipt: TableLiveV35Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: TableLiveV35RemotePhase,
): TableLiveV35ResponseContract => ({
  ...TABLE_LIVE_V35_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: TABLE_LIVE_V35_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "table-live-v35-plugin-response-v2",
  brokerVersion: TABLE_LIVE_V35_BROKER_VERSION,
  target: TABLE_LIVE_V35_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*TABLE_LIVE_V35_PROGRAM_START*/${program}/*TABLE_LIVE_V35_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*TABLE_LIVE_V35_PROGRAM_START*/";
  const endMarker = "/*TABLE_LIVE_V35_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Table live v35 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createTableLiveV35Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Table live v35 private signing key does not match authorized public-key identity",
    );
  const manifest: TableLiveV35Manifest = {
    artifactVersion: "table-live-v35-broker-manifest-v2",
    brokerVersion: TABLE_LIVE_V35_BROKER_VERSION,
    ...
    captureRequests: TABLE_LIVE_V35_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateTableLiveV35Request(
  directory: string,
  request: TableLiveV35Request,
): void {
  ...
  if (
    request.artifactVersion !== "table-live-v35-broker-request-v2" ||
    request.brokerVersion !== TABLE_LIVE_V35_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== TABLE_LIVE_V35_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Table live v35 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueTableLiveV35Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: TableLiveV35RemotePhase,
  program: string,
  options: TableLiveV35IssueOptions = {},
): TableLiveV35Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<TableLiveV35Request>(targetPath);
    validateTableLiveV35Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Table live v35 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(tableLiveV2ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Table live v35 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Table live v35 empty code envelope refused");
  if (phase === "capture") validateTableLiveV35TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Table live v35 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: TABLE_LIVE_V35_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? TABLE_LIVE_V35_SIGNED_WRITER_TIMEOUT_MS
        : TABLE_LIVE_V35_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: TableLiveV35RequestBody = {
    artifactVersion: "table-live-v35-broker-request-v2",
    brokerVersion: TABLE_LIVE_V35_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: TableLiveV35Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateTableLiveV35Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistTableLiveV35RawResponse(
  directory: string,
  phase: TableLiveV35RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Table live v35 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<TableLiveV35Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Table live v35 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = tableLiveV2ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Table live v35 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<TableLiveV35Receipt, "receiptSha256"> = {
    artifactVersion: "table-live-v35-broker-receipt-v2",
    brokerVersion: TABLE_LIVE_V35_BROKER_VERSION,
...
export function acceptTableLiveV35Response<T = unknown>(
  directory: string,
  phase: TableLiveV35RemotePhase,
  options: {
    captureIndex?: number;
...
): TableLiveV35AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Table live v35 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<TableLiveV35Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Table live v35 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "table-live-v35-plugin-response-v2" ||
    plugin?.brokerVersion !== TABLE_LIVE_V35_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Table live v35 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistTableLiveV35TechnicalGates(
  directory: string,
  rootProofs: readonly TableLiveV35RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertTableLiveV35RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Table live v35 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Table live v35 probe payload hash malformed");
  const gates = Object.fromEntries(
    TABLE_LIVE_V35_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "table-live-v35-host-technical-gates-v2",
    brokerVersion: TABLE_LIVE_V35_BROKER_VERSION,
...
export function validateTableLiveV35TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "table-live-v35-host-technical-gates-v2" ||
    ...
    TABLE_LIVE_V35_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Table live v35 technical gates artifact");
}

export function validateTableLiveV35Receipt(
  directory: string,
  phase: TableLiveV35RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateTableLiveV35Request(directory, request);
  const receipt = json<TableLiveV35Receipt>(
  ...
  if (
    receipt.artifactVersion !== "table-live-v35-broker-receipt-v2" ||
    receipt.brokerVersion !== TABLE_LIVE_V35_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Table live v35 accepted receipt: ${request.requestId}`,
    );
}

export function readTableLiveV35AcceptedRawPayload(
  directory: string,
  phase: TableLiveV35RemotePhase,
  captureIndex?: number,
): unknown {
  validateTableLiveV35Receipt(directory, phase, captureIndex);
...
export function createTableLiveV35FakeDynamicResponse(
  request: TableLiveV35Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? TABLE_LIVE_V35_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? TABLE_LIVE_V35_TARGET,
    ...overrides.extra,
  };
}

export function listTableLiveV35PersistedRequests(
  directory: string,
): TableLiveV35Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<TableLiveV35Request>(file)] : [];
    });
}
