...
import { calendarLiveV1Sha256 } from "./calendar-live-v21-broker.js";

export interface CalendarLiveV21LifecycleSimulationReport {
  artifactVersion: "calendar-live-v21-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV21AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV21Lifecycle(): Promise<CalendarLiveV21LifecycleSimulationReport> {
  await buildCalendarLiveV21Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V21_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV21AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v21-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V21_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V21_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV21History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV21History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV21AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V21_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V21_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV21History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV21History(authorized, "pending");
    ...
    await buildCalendarLiveV21Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V21_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV21AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV21Lifecycle(), null, 2)}\n`,
  );
