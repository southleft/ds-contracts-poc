...
import { buildComboboxLiveV40CleanupRuntime } from "./combobox-live-v40-cleanup.js";
import {
  type ComboboxLiveV40FixedFixedPoint,
} from "./combobox-live-v40-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV40RestoreProgram,
  validateComboboxLiveV40RestorePayload,
  type ComboboxLiveV40RestorePayload,
} from "./combobox-live-v40-restore.js";
import {
  buildComboboxLiveV40RawPropertyRuntime,
  normalizeComboboxLiveV40Scene,
  type ComboboxLiveV40RawNode,
} from "./combobox-live-v40-verifier.js";
...
} from "./scene-readback-combobox-v40.js";
...
export const COMBOBOX_LIVE_V40_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V40_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V40_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V40_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V40_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V40_SET_COUNT = 4;
export const COMBOBOX_LIVE_V40_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V40_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V40_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V40_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V40_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV40SourceId = (typeof COMBOBOX_LIVE_V40_SOURCE_IDS)[number];

export interface ComboboxLiveV40SourceIdentity {
  source: ComboboxLiveV40SourceId;
...
export interface ComboboxLiveV40WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV40ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV40SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV40RawNode;
    optionScene: ComboboxLiveV40RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV40RootProof {
  source: ComboboxLiveV40SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV40FixedFixedPoint;
}

export interface ComboboxLiveV40ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV40SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV40CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV40SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV40CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV40SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV40CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v40 ${label} must be an object`);
...
export function validateComboboxLiveV40WriterPayload(
  value: unknown,
): ComboboxLiveV40WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V40_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v40 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v40 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v40 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV40SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV40ExtractProgram(
  writer: ComboboxLiveV40WriterOwnership,
  sources: readonly ComboboxLiveV40SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v40 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V40_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V40_NAMESPACE)}
${buildComboboxLiveV40RawPropertyRuntime()}
...
export function validateComboboxLiveV40ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV40WriterOwnership,
): ComboboxLiveV40ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v40 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V40_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v40 extract schema/two-root mismatch");
  return payload as ComboboxLiveV40ExtractPayload;
}

export function proveComboboxLiveV40Roots<Instance>(
  extract: ComboboxLiveV40ExtractPayload,
  sources: readonly (ComboboxLiveV40SourceIdentity & {
...
): ComboboxLiveV40RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v40 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v40 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV40Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV40Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV40FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v40 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v40 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v40 compile lost owned sets");
...
export function assertComboboxLiveV40RootProofs(
  proofs: readonly ComboboxLiveV40RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v40 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v40 independent root accounting failed: ${failures.join(";")}`,
    );
}

/** v40: keep exactSceneRestoration after the open-variant walk when resize and property restore already pass. */
export function buildComboboxLiveV40ProbeProgram(
  writer: ComboboxLiveV40WriterOwnership,
  sources: readonly ComboboxLiveV40SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V40_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV40ProbePayload(
  value: unknown,
  writer: ComboboxLiveV40WriterOwnership,
): ComboboxLiveV40ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV40ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V40_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V40_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v40 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV40CaptureManifest(
  cells: readonly ComboboxLiveV40CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V40_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V40_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V40_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V40_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v40 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV40CaptureCell[],
): string => {
  validateComboboxLiveV40CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV40CaptureProgram(
  writer: ComboboxLiveV40WriterOwnership,
  cell: ComboboxLiveV40CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V40_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v40 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v40 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V40_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV40CapturePayload(
  value: unknown,
  cell: ComboboxLiveV40CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV40CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV40CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V40_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V40_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v40 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV40CaptureResponses(
  manifest: readonly ComboboxLiveV40CaptureCell[],
  responses: readonly ComboboxLiveV40CapturePayload[],
): void {
  validateComboboxLiveV40CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v40 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV40ObjectiveReport {
  artifactVersion: "combobox-live-v40-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV40Objective(
  manifest: readonly ComboboxLiveV40CaptureCell[],
  responses: readonly ComboboxLiveV40CapturePayload[],
): ComboboxLiveV40ObjectiveReport {
  assertComboboxLiveV40CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v40-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV40CleanupProgram(
  writer: ComboboxLiveV40WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV40CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V40_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v40 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV40CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV40WriterOwnership,
): ComboboxLiveV40CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV40CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v40 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V40_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV40WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
