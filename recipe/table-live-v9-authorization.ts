...
import {
  TABLE_LIVE_V9_DYNAMIC_TOOL,
  TABLE_LIVE_V9_TARGET,
  type TableLiveV9TransactionAuthorization,
} from "./table-live-v9-broker.js";
import {
  TABLE_LIVE_V9_AUTHORIZATION_PATH,
  TABLE_LIVE_V9_INDEX_PATH,
} from "./build-table-live-proof-v9.js";
import {
  TABLE_LIVE_V9_CAPTURE_COUNT,
  TABLE_LIVE_V9_HOST_PHASES,
  TABLE_LIVE_V9_REMOTE_REQUESTS,
  TABLE_LIVE_V9_SOURCE_ROOTS,
} from "./table-live-v9-contract.js";
import { canonicalJson } from "./normalize.js";

export type TableLiveV9HistoryExpectation = "pending" | "authorized";

export interface TableLiveV9AuthorizationArtifact {
  artifactVersion: "table-live-v9-capture-authorization-v1";
  authorizationId: "table-live-v9";
  status: "authorization declared; runtime security prerequisites still mandatory";
  authorizationIntent: true;
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V9_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV9AntecedentIndex {
  artifactVersion: "table-live-v9-antecedent-index-v1";
  status: string;
...
export interface TableLiveV9HistoryState {
  index?: TableLiveV9AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV9AuthorizationArtifact;
...
export interface TableLiveV9AuthorizationProof
  extends TableLiveV9TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V9_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V9_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V9_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V9_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v9-authorization") ||
  artifactPath.includes("table-live-v9-preflight") ||
  artifactPath.includes("table-live-v9-authorized") ||
  artifactPath.includes("create-table-live-v9-security-attestation") ||
  artifactPath.includes("table-live-pivot-v9-status") ||
...
export function validateTableLiveV9AntecedentIndex(
  index: TableLiveV9AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v9-antecedent-index-v1" ||
    index.authorizationCanBeAddedWithoutAntecedentRebuild !== true
  )
    failures.push("table v9 antecedent index missing or malformed");
  const artifactEntries = Object.entries(index?.artifacts ?? {});
  if (artifactEntries.length === 0)
    failures.push("table v9 antecedent hash denominator is empty");
  for (const [artifactPath, metadata] of artifactEntries) {
    ...
    if (
      ...
      failures.push(`malformed table v9 antecedent hash: ${artifactPath}`);
  }
  ...
  if (index?.hashSetSha256 !== expectedHashSet)
    failures.push("table v9 antecedent hash-set digest mismatch");
  if (
    ...
    failures.push("table v9 antecedent counts missing or drifted");
  for (const required of [
    TABLE_LIVE_V9_AUTHORIZATION_PATH,
    "recipe/table-live-v9-authorization.ts",
    "recipe/table-live-v9-authorization.test.ts",
    "recipe/table-live-v9-preflight.ts",
    "recipe/evidence/table-live-pivot-v9/operator-security-attestation-template.json",
    "recipe/evidence/status-index.json",
  ])
    if (!index?.authorizationLifecycleExcluded?.includes(required))
      failures.push(`table v9 lifecycle exclusion missing: ${required}`);
  const leaks = forbiddenAntecedentFieldPaths(index);
  if (leaks.length)
    failures.push(`table v9 antecedent result leakage: ${leaks.join(",")}`);
  return failures;
}

export function validateTableLiveV9UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV9History\(\s*["']/.test(source) ||
    /readTableLiveV9HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV9AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV9AuthorizationArtifact {
  if (!SHA40.test(options.antecedentCommit))
    throw new TypeError("table v9 antecedent commit must be a full SHA");
  const index = JSON.parse(
    Buffer.from(options.antecedentIndexBytes).toString("utf8"),
  ) as TableLiveV9AntecedentIndex;
  const indexFailures = validateTableLiveV9AntecedentIndex(index);
  if (indexFailures.length)
    throw new TypeError(
      `table v9 antecedent index refused:\n${indexFailures.join("\n")}`,
    );
  const publicKey =
  ...
  return {
    ...
    antecedent: {
      ...
      indexPath: TABLE_LIVE_V9_INDEX_PATH,
    ...
    denominator: {
      remoteRequests: TABLE_LIVE_V9_REMOTE_REQUESTS,
      hostPhases: TABLE_LIVE_V9_HOST_PHASES,
      captures: TABLE_LIVE_V9_CAPTURE_COUNT,
      sourceRoots: TABLE_LIVE_V9_SOURCE_ROOTS,
...
export function validateTableLiveV9AuthorizationArtifact(
  artifact: TableLiveV9AuthorizationArtifact | undefined,
  index: TableLiveV9AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v9-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v9" ||
    artifact.authorizationIntent !== true
  )
    failures.push("table v9 authorization declaration missing or malformed");
  const resultLeaks = forbiddenAntecedentFieldPaths(artifact);
  if (resultLeaks.length)
    failures.push(`table v9 authorization result leakage: ${resultLeaks.join(",")}`);
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V9_INDEX_PATH ||
    artifact?.antecedent?.indexSha256 !== indexSha256 ||
    artifact?.antecedent?.hashSetSha256 !== index?.hashSetSha256
  )
    failures.push("table v9 authorization pins wrong antecedent index");
  if (privateMaterialPaths(artifact).length)
    failures.push("table v9 authorization contains private key or token material");
  try {
    ...
    if (
      ...
      failures.push("table v9 Ed25519 public-key identity mismatch");
  } catch {
    failures.push("table v9 Ed25519 public key is invalid");
  }
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V9_TARGET) ||
    !equalJson(
    ...
    failures.push("wrong table v9 target or dynamic tool");
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V9_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V9_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V9_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V9_SOURCE_ROOTS ||
    artifact?.denominator?.expectedFacts !== index?.counts.expectedSceneFacts
  )
    failures.push("table v9 two-root or capture denominator changed");
  if (
    ...
    failures.push(
      "table v9 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
    ...
    failures.push("table v9 security prerequisites weakened");
  if (
    artifact?.humanSignoff?.mandatory !== true ||
    artifact?.humanSignoff?.status !== "pending"
  )
    failures.push("table v9 human signoff overclaim");
  return failures;
}

export function validateTableLiveV9History(
  state: TableLiveV9HistoryState,
  expected: TableLiveV9HistoryExpectation,
): string[] {
  const failures = validateTableLiveV9AntecedentIndex(state.index);
  if (!SHA40.test(state.codeCommit))
    failures.push("table v9 code commit is not a full SHA");
  ...
  if (
    ...
    failures.push("table v9 antecedent first-add commit is not unique");
  if (!state.antecedentIsAncestorOfCode)
    failures.push("code commit does not descend from table v9 antecedent");
  if (!state.antecedentIndexBytesMatchFirstAddition)
    failures.push("table v9 antecedent index changed after first addition");
  if (!state.antecedentArtifactsMatchIndexAtCommit)
    failures.push("table v9 antecedent commit artifacts do not match index");
  if (!state.workingAntecedentArtifactsMatchIndex)
    failures.push("table v9 antecedent working bytes drifted");
  ...
  failures.push(
    ...validateTableLiveV9AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
  ...
  if (
    ...
    failures.push("table v9 authorization first-add lineage mismatch");
  if (!state.authorizationPresentAtCodeCommit)
    failures.push("table v9 authorization missing from code commit");
  if (!state.authorizationBytesMatchFirstAddition)
    failures.push("table v9 authorization bytes changed after first addition");
  if (!state.authorizationStrictlyDescendsFromAntecedent)
    failures.push("table v9 authorization does not strictly descend from antecedent");
  if (!state.authorizationIsAncestorOfCode)
    failures.push("code commit predates table v9 authorization");
  return failures;
}
...
export function readTableLiveV9HistoryState(
  root = process.cwd(),
): TableLiveV9HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V9_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV9AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V9_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V9_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V9_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV9History(
  expected: TableLiveV9HistoryExpectation,
  root = process.cwd(),
): TableLiveV9HistoryState {
  const state = readTableLiveV9HistoryState(root);
  const failures = validateTableLiveV9History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v9 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV9Authorization(
  root = process.cwd(),
): TableLiveV9AuthorizationProof {
  const state = verifyTableLiveV9History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V9_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v9/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v9.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V9_TARGET,
    expectedDynamicTool: TABLE_LIVE_V9_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V9_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV9HistoryExpectation => {
  ...
  if (pending === authorized)
    throw new Error(
      "Choose exactly one table v9 history mode: --expect-pending or --expect-authorized",
    );
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV9History(expected);
  process.stdout.write(
    `Table live v9 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
