...
import { tableLiveV2Sha256 } from "./table-live-v30-broker.js";

export interface TableLiveV30LifecycleSimulationReport {
  artifactVersion: "table-live-v30-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV30AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV30Lifecycle(): Promise<TableLiveV30LifecycleSimulationReport> {
  await buildTableLiveV30Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V30_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV30AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v30-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V30_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V30_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV30History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV30History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV30AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V30_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V30_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV30History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV30History(authorized, "pending");
    ...
    await buildTableLiveV30Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V30_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV30AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV30Lifecycle(), null, 2)}\n`,
  );
