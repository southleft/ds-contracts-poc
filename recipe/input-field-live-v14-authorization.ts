...
import {
  ...
  INPUT_LIVE_V14_INDEX_PATH,
} from "./build-input-field-live-proof-v14.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV14HistoryExpectation = "pending" | "authorized";

export interface InputLiveV14AuthorizationArtifact {
  artifactVersion: "input-live-v14-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V14_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV14AntecedentIndex {
...
export interface InputLiveV14HistoryState {
  index?: InputLiveV14AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV14AuthorizationArtifact;
...
export interface InputLiveV14AuthorizationProof extends InputLiveV14TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V14_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V14_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V14_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V14_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v14-status") ||
...
export function validateInputLiveV14AntecedentIndex(
  index: InputLiveV14AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v14-antecedent-index-v1" ||
...
export function validateInputLiveV14UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV14History\(\s*["']/.test(source) ||
    /readInputLiveV14HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV14AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV14AuthorizationArtifact {
  ...
  ) as InputLiveV14AntecedentIndex;
  const indexFailures = validateInputLiveV14AntecedentIndex(index);
...
export function validateInputLiveV14AuthorizationArtifact(
  artifact: InputLiveV14AuthorizationArtifact | undefined,
  index: InputLiveV14AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v14-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v14" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V14_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V14_TARGET) ||
    !equalJson(
...
export function validateInputLiveV14History(
  state: InputLiveV14HistoryState,
  expected: InputLiveV14HistoryExpectation,
): string[] {
  const failures = validateInputLiveV14AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV14AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV14HistoryState(
  root = process.cwd(),
): InputLiveV14HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V14_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV14AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V14_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V14_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V14_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV14History(
  expected: InputLiveV14HistoryExpectation,
  root = process.cwd(),
): InputLiveV14HistoryState {
  const state = readInputLiveV14HistoryState(root);
  const failures = validateInputLiveV14History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV14Authorization(
  root = process.cwd(),
): InputLiveV14AuthorizationProof {
  const state = verifyInputLiveV14History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V14_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v14/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v14.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V14_TARGET,
    expectedDynamicTool: INPUT_LIVE_V14_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V14_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV14HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV14History(expected);
