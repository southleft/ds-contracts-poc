...
import {
  verifyCalendarLiveV48FixedSceneFixedPoint,
  type CalendarLiveV48FixedFixedPoint,
} from "./calendar-live-v48-fixed-point.js";

export const CALENDAR_LIVE_V48_VERIFIER_VERSION = "calendar-live-v48-verifier-v1";

export function buildCalendarLiveV48RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV48RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV48RawNode[];
}

export interface CalendarLiveV48NormalizedScene {
...
export interface CalendarLiveV48SceneProof {
...
export function normalizeCalendarLiveV48Scene(
  root: CalendarLiveV48RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV48NormalizedScene {
  const canonicalBindings: CalendarLiveV48NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV48RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV48Scene<Instance>(
  normalized: CalendarLiveV48NormalizedScene,
...
): CalendarLiveV48SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV48FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV48ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV48PreCaptureGates(payload: unknown): void {
