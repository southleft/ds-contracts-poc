...
export type ComboboxLiveV19HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV19AuthorizationArtifact {
  artifactVersion: "combobox-live-v19-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V19_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV19AntecedentIndex {
...
export interface ComboboxLiveV19HistoryState {
  index?: ComboboxLiveV19AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV19AuthorizationArtifact;
...
export interface ComboboxLiveV19AuthorizationProof extends ComboboxLiveV19TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V19_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V19_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V19_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V19_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v19-authorization") ||
  artifactPath.includes("combobox-live-v19-preflight") ||
  artifactPath.includes("combobox-live-v19-authorized") ||
  artifactPath.includes("create-combobox-live-v19-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v19-status") ||
...
export function validateComboboxLiveV19AntecedentIndex(
  index: ComboboxLiveV19AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v19-antecedent-index-v1" ||
...
export function validateComboboxLiveV19UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV19History\(\s*["']/.test(source) ||
    /readComboboxLiveV19HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV19AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV19AuthorizationArtifact {
  ...
  ) as ComboboxLiveV19AntecedentIndex;
  const indexFailures = validateComboboxLiveV19AntecedentIndex(index);
...
export function validateComboboxLiveV19AuthorizationArtifact(
  artifact: ComboboxLiveV19AuthorizationArtifact | undefined,
  index: ComboboxLiveV19AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v19-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v19" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V19_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V19_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V19_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V19_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V19_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V19_SOURCE_ROOTS ||
  ...
  if (
    ...
    failures.push(
      "v19 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
...
export function validateComboboxLiveV19History(
  state: ComboboxLiveV19HistoryState,
  expected: ComboboxLiveV19HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV19AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV19AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV19HistoryState(
  root = process.cwd(),
): ComboboxLiveV19HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V19_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV19AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V19_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V19_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V19_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV19History(
  expected: ComboboxLiveV19HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV19HistoryState {
  const state = readComboboxLiveV19HistoryState(root);
  const failures = validateComboboxLiveV19History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v19 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV19Authorization(
  root = process.cwd(),
): ComboboxLiveV19AuthorizationProof {
  const state = verifyComboboxLiveV19History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V19_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v19/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v19.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V19_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V19_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V19_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV19HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV19History(expected);
  process.stdout.write(
    `Combobox live v19 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
