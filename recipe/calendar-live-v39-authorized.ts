...
import { CALENDAR_LIVE_V39_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v39.js";
import { verifyCalendarLiveV39Authorization } from "./calendar-live-v39-authorization.js";
import { runCalendarLiveV39Preflight } from "./calendar-live-v39-preflight.js";
import { CalendarLiveV39Orchestrator } from "./run-calendar-live-v39.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v39 requires exact authorization artifact ${CALENDAR_LIVE_V39_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v39-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV39Authorization(root);
    runCalendarLiveV39Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV39Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV39Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
