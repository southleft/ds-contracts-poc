...
import { buildCalendarLiveV43CleanupRuntime } from "./calendar-live-v43-cleanup.js";
import { type CalendarLiveV43FixedFixedPoint } from "./calendar-live-v43-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV43RestoreProgram,
  validateCalendarLiveV43RestorePayload,
  type CalendarLiveV43RestorePayload,
} from "./calendar-live-v43-restore.js";
import {
  buildCalendarLiveV43RawPropertyRuntime,
  normalizeCalendarLiveV43Scene,
  type CalendarLiveV43RawNode,
} from "./calendar-live-v43-verifier.js";
...
export const CALENDAR_LIVE_V43_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V43_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V43_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V43_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V43_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V43_SET_COUNT = 3;
export const CALENDAR_LIVE_V43_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V43_HOST_PHASES = 3;
export const CALENDAR_LIVE_V43_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V43_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V43_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V43_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV43SourceId = (typeof CALENDAR_LIVE_V43_SOURCE_IDS)[number];
export type CalendarLiveV43Kind = "calendar" | "week" | "day";

export interface CalendarLiveV43SourceIdentity {
  source: CalendarLiveV43SourceId;
...
export interface CalendarLiveV43WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV43ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV43SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV43RawNode;
    weekScene: CalendarLiveV43RawNode;
    dayScene: CalendarLiveV43RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV43RootProof {
  source: CalendarLiveV43SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV43FixedFixedPoint;
}

export interface CalendarLiveV43ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV43SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV43CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV43SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV43CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV43SourceId;
  frameWidth: number;
...
export interface CalendarLiveV43CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v43 ${label} must be an object`);
...
export function validateCalendarLiveV43WriterPayload(
  value: unknown,
): CalendarLiveV43WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v43") ||
    ...
    result.namespace !== CALENDAR_LIVE_V43_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v43 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v43 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v43 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV43SourceIdentity,
  kind: CalendarLiveV43Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV43ExtractProgram(
  writer: CalendarLiveV43WriterOwnership,
  sources: readonly CalendarLiveV43SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v43 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V43_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V43_NAMESPACE)}
${buildCalendarLiveV43RawPropertyRuntime()}
...
export function validateCalendarLiveV43ExtractPayload(
  value: unknown,
  writer: CalendarLiveV43WriterOwnership,
): CalendarLiveV43ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v43 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V43_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v43 extract schema/one-root mismatch");
  return payload as CalendarLiveV43ExtractPayload;
}

export function proveCalendarLiveV43Roots<Instance>(
  extract: CalendarLiveV43ExtractPayload,
  sources: readonly (CalendarLiveV43SourceIdentity & {
...
): CalendarLiveV43RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v43 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v43 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV43Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV43Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV43Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV43FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v43 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v43 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v43 compile lost owned sets");
...
export function assertCalendarLiveV43RootProofs(
  proofs: readonly CalendarLiveV43RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v43 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v43 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV43ProbeProgram(
  writer: CalendarLiveV43WriterOwnership,
  sources: readonly CalendarLiveV43SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V43_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV43ProbePayload(
  value: unknown,
  writer: CalendarLiveV43WriterOwnership,
): CalendarLiveV43ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV43ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V43_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V43_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v43 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV43CaptureManifest(
  cells: readonly CalendarLiveV43CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V43_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V43_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V43_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V43_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v43 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV43CaptureCell[],
): string => {
  validateCalendarLiveV43CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV43CaptureProgram(
  writer: CalendarLiveV43WriterOwnership,
  cell: CalendarLiveV43CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V43_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v43 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v43 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V43_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV43CapturePayload(
  value: unknown,
  cell: CalendarLiveV43CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV43CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV43CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V43_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V43_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v43 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV43CaptureResponses(
  manifest: readonly CalendarLiveV43CaptureCell[],
  responses: readonly CalendarLiveV43CapturePayload[],
): void {
  validateCalendarLiveV43CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v43 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV43ObjectiveReport {
  artifactVersion: "calendar-live-v43-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV43Objective(
  manifest: readonly CalendarLiveV43CaptureCell[],
  responses: readonly CalendarLiveV43CapturePayload[],
): CalendarLiveV43ObjectiveReport {
  assertCalendarLiveV43CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v43-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV43CleanupProgram(
  writer: CalendarLiveV43WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV43CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V43_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v43 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV43CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV43WriterOwnership,
): CalendarLiveV43CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV43CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v43 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V43_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV43WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
