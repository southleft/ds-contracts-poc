/**
 * Calendar live v39 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V39_FIXED_VERIFIER_VERSION = "calendar-live-v39-fixed-point-v1";
export const CALENDAR_LIVE_V39_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV39FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV39FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV39FixedSourceProbe {
...
export interface CalendarLiveV39FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV39FixedFixedPoint;
}

export interface CalendarLiveV39FixedHardGateInput {
  thresholds: CalendarLiveV39FixedThresholds;
  sourceProbes: CalendarLiveV39FixedSourceProbe[];
  cells: CalendarLiveV39FixedCellValidation[];
  sceneProofs: CalendarLiveV39FixedSceneProof[];
...
export interface CalendarLiveV39FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V39_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV39FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV39FixedHardGates(
  input: CalendarLiveV39FixedHardGateInput,
): CalendarLiveV39FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV39FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V39_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV39FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV39FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV39FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
