...
import { INPUT_LIVE_V61_INDEX_PATH } from "./build-input-field-live-proof-v61.js";
import {
  INPUT_LIVE_V61_DYNAMIC_TOOL,
  INPUT_LIVE_V61_TARGET,
} from "./input-field-live-v61-broker.js";
import {
  buildInputLiveV61AuthorizationArtifact,
  validateInputLiveV61AntecedentIndex,
  validateInputLiveV61AuthorizationArtifact,
  validateInputLiveV61History,
  validateInputLiveV61UnitTestSource,
  type InputLiveV61AntecedentIndex,
  type InputLiveV61AuthorizationArtifact,
  type InputLiveV61AuthorizationProof,
  type InputLiveV61HistoryState,
} from "./input-field-live-v61-authorization.js";
import {
  inputLiveV61AttestationSha256,
  validateInputLiveV61ControlFlowSource,
  validateInputLiveV61SecurityAttestation,
  type InputLiveV61SecurityAttestation,
} from "./input-field-live-v61-preflight.js";
import { inputLiveV61Sha256 } from "./input-field-live-v61-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V61_INDEX_PATH);
const index = (): InputLiveV61AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV61AntecedentIndex;

const artifact = (): InputLiveV61AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV61AuthorizationArtifact({
...
const pendingState = (): InputLiveV61HistoryState => ({
  index: index(),
  indexSha256: inputLiveV61Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV61HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV61AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV61Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V61_TARGET,
  expectedDynamicTool: INPUT_LIVE_V61_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v61/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV61AuthorizationArtifact,
): InputLiveV61SecurityAttestation => {
  const body: Omit<InputLiveV61SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v61-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V61_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV61AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV61History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV61History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV61History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV61History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV61History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV61UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV61UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV61AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV61Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV61AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV61AuthorizationArtifact(
        planted as InputLiveV61AuthorizationArtifact,
        index(),
        inputLiveV61Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV61SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV61AttestationSha256(
      body as Omit<InputLiveV61SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV61SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV61ControlFlowSource(source), []);
});
