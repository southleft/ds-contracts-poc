...
import {
  ...
  INPUT_LIVE_V47_INDEX_PATH,
} from "./build-input-field-live-proof-v47.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV47HistoryExpectation = "pending" | "authorized";

export interface InputLiveV47AuthorizationArtifact {
  artifactVersion: "input-live-v47-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V47_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV47AntecedentIndex {
...
export interface InputLiveV47HistoryState {
  index?: InputLiveV47AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV47AuthorizationArtifact;
...
export interface InputLiveV47AuthorizationProof extends InputLiveV47TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V47_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V47_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V47_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V47_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v47-status") ||
...
export function validateInputLiveV47AntecedentIndex(
  index: InputLiveV47AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v47-antecedent-index-v1" ||
...
export function validateInputLiveV47UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV47History\(\s*["']/.test(source) ||
    /readInputLiveV47HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV47AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV47AuthorizationArtifact {
  ...
  ) as InputLiveV47AntecedentIndex;
  const indexFailures = validateInputLiveV47AntecedentIndex(index);
...
export function validateInputLiveV47AuthorizationArtifact(
  artifact: InputLiveV47AuthorizationArtifact | undefined,
  index: InputLiveV47AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v47-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v47" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V47_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V47_TARGET) ||
    !equalJson(
...
export function validateInputLiveV47History(
  state: InputLiveV47HistoryState,
  expected: InputLiveV47HistoryExpectation,
): string[] {
  const failures = validateInputLiveV47AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV47AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV47HistoryState(
  root = process.cwd(),
): InputLiveV47HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V47_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV47AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V47_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V47_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V47_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV47History(
  expected: InputLiveV47HistoryExpectation,
  root = process.cwd(),
): InputLiveV47HistoryState {
  const state = readInputLiveV47HistoryState(root);
  const failures = validateInputLiveV47History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV47Authorization(
  root = process.cwd(),
): InputLiveV47AuthorizationProof {
  const state = verifyInputLiveV47History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V47_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v47/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v47.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V47_TARGET,
    expectedDynamicTool: INPUT_LIVE_V47_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V47_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV47HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV47History(expected);
