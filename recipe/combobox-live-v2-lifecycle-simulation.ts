...
export interface ComboboxLiveV2LifecycleSimulationReport {
  artifactVersion: "combobox-live-v2-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV2AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV2Lifecycle(): Promise<ComboboxLiveV2LifecycleSimulationReport> {
  await buildComboboxLiveV2Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V2_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV2AntecedentIndex;
  const indexSha256 = comboboxLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v2-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V2_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V2_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV2History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV2History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV2AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V2_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V2_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV2History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV2History(authorized, "pending");
    ...
    await buildComboboxLiveV2Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V2_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV2AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: comboboxLiveV2Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV2Lifecycle(), null, 2)}\n`,
  );
