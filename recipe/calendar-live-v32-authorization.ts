...
export type CalendarLiveV32HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV32AuthorizationArtifact {
  artifactVersion: "calendar-live-v32-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V32_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV32AntecedentIndex {
...
export interface CalendarLiveV32HistoryState {
  index?: CalendarLiveV32AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV32AuthorizationArtifact;
...
export interface CalendarLiveV32AuthorizationProof
  extends CalendarLiveV32TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V32_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V32_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V32_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V32_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v32-authorization") ||
  artifactPath.includes("calendar-live-v32-preflight") ||
  artifactPath.includes("calendar-live-v32-authorized") ||
  artifactPath.includes("create-calendar-live-v32-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v32-status") ||
...
export function validateCalendarLiveV32AntecedentIndex(
  index: CalendarLiveV32AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v32-antecedent-index-v1" ||
...
export function validateCalendarLiveV32UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV32History\(\s*["']/.test(source) ||
    /readCalendarLiveV32HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV32AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV32AuthorizationArtifact {
  ...
  ) as CalendarLiveV32AntecedentIndex;
  const indexFailures = validateCalendarLiveV32AntecedentIndex(index);
...
export function validateCalendarLiveV32AuthorizationArtifact(
  artifact: CalendarLiveV32AuthorizationArtifact | undefined,
  index: CalendarLiveV32AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v32-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v32" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V32_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V32_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V32_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V32_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V32_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V32_SOURCE_ROOTS ||
...
export function validateCalendarLiveV32History(
  state: CalendarLiveV32HistoryState,
  expected: CalendarLiveV32HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV32AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV32AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV32HistoryState(
  root = process.cwd(),
): CalendarLiveV32HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V32_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV32AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V32_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V32_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V32_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV32History(
  expected: CalendarLiveV32HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV32HistoryState {
  const state = readCalendarLiveV32HistoryState(root);
  const failures = validateCalendarLiveV32History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v32 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV32Authorization(
  root = process.cwd(),
): CalendarLiveV32AuthorizationProof {
  const state = verifyCalendarLiveV32History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V32_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v32/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v32.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V32_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V32_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V32_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV32HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV32History(expected);
  process.stdout.write(
    `Calendar live v32 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
