...
import { TABLE_LIVE_V32_INDEX_PATH } from "./build-table-live-proof-v32.js";
import {
...
import {
  buildTableLiveV32AuthorizationArtifact,
  validateTableLiveV32AntecedentIndex,
  validateTableLiveV32AuthorizationArtifact,
  validateTableLiveV32History,
  validateTableLiveV32UnitTestSource,
  type TableLiveV32AntecedentIndex,
  type TableLiveV32AuthorizationArtifact,
  type TableLiveV32AuthorizationProof,
  type TableLiveV32HistoryState,
} from "./table-live-v32-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV32ControlFlowSource,
  validateTableLiveV32SecurityAttestation,
  type TableLiveV32SecurityAttestation,
} from "./table-live-v32-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V32_INDEX_PATH);
const index = (): TableLiveV32AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV32AntecedentIndex;

const artifact = (): TableLiveV32AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV32AuthorizationArtifact({
...
const pendingState = (): TableLiveV32HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV32HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV32AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v32/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV32AuthorizationArtifact,
): TableLiveV32SecurityAttestation => {
  const body: Omit<TableLiveV32SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V32_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV32History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV32History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV32History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV32History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV32History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV32UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v32-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV32UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV32AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV32AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV32AuthorizationArtifact(
        planted as TableLiveV32AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV32SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV32SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV32SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v32-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV32ControlFlowSource(source), []);
});
