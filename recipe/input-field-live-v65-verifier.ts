...
export const INPUT_LIVE_V65_VERIFIER_VERSION = "input-live-v65-verifier-v1";

export function buildInputLiveV65RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV65RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV65RawNode[];
}

export interface InputLiveV65NormalizedScene {
...
export interface InputLiveV65SceneProof {
...
export function normalizeInputLiveV65Scene(
  root: InputLiveV65RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV65NormalizedScene {
  const canonicalBindings: InputLiveV65NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV65RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV65Scene<Instance>(
  normalized: InputLiveV65NormalizedScene,
...
export function compileInputLiveV65ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV65PreCaptureGates(payload: unknown): void {
