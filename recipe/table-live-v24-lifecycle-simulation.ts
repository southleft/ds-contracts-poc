...
import { tableLiveV2Sha256 } from "./table-live-v24-broker.js";

export interface TableLiveV24LifecycleSimulationReport {
  artifactVersion: "table-live-v24-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV24AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV24Lifecycle(): Promise<TableLiveV24LifecycleSimulationReport> {
  await buildTableLiveV24Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V24_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV24AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v24-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V24_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V24_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV24History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV24History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV24AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V24_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V24_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV24History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV24History(authorized, "pending");
    ...
    await buildTableLiveV24Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V24_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV24AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV24Lifecycle(), null, 2)}\n`,
  );
