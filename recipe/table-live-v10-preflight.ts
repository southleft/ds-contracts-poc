...
import {
  validateTableLiveV10AntecedentIndex,
  verifyTableLiveV10Authorization,
  type TableLiveV10AntecedentIndex,
  type TableLiveV10AuthorizationProof,
} from "./table-live-v10-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V10_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v10/operator-security-attestation-template.json";
export const TABLE_LIVE_V10_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v10-security-attestation.json";

export interface TableLiveV10SecurityAttestation {
  artifactVersion: "table-live-v10-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV10PreflightReport {
  artifactVersion: "table-live-v10-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV10SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV10SecurityAttestation(
  value: unknown,
  proof: TableLiveV10AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v10-operator-security-attestation-v1" ||
    artifact.status !== "complete"
  )
    failures.push("missing completed table v10 security attestation");
  if (
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V10_TARGET) ||
...
export function validateTableLiveV10ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV10Authorization");
  const preflight = source.indexOf("runTableLiveV10Preflight");
  const initialize = source.indexOf("TableLiveV10Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV10Preflight(
  root: string,
  proof: TableLiveV10AuthorizationProof,
  attestationPath = TABLE_LIVE_V10_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV10PreflightReport {
  const failures: string[] = [];
  if (
    ...
    failures.push("table v10 attempt chronology invalid or exceeds maximum 3");
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V10_TARGET) ||
    canonicalJson(proof.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V10_DYNAMIC_TOOL)
  )
    failures.push("table v10 authorization proof target or tool mismatch");
  const indexPath = path.join(root, TABLE_LIVE_V10_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV10AntecedentIndex;
  failures.push(...validateTableLiveV10AntecedentIndex(index));
  if (
    sha256(indexBytes) !== proof.antecedentIndexSha256 ||
    index.hashSetSha256 !== proof.antecedentHashSetSha256
  )
    failures.push("table v10 authorization proof antecedent index mismatch");
  for (const [relativePath, metadata] of Object.entries(index.artifacts)) {
    ...
    if (
      ...
      failures.push(
        `table v10 pinned antecedent working bytes drift: ${relativePath}`,
      );
  }
  if (!existsSync(path.join(root, TABLE_LIVE_V10_AUTHORIZATION_PATH)))
    failures.push("table v10 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V10_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV10SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v10-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV10ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v10 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV10SecurityAttestation;
  return {
    artifactVersion: "table-live-v10-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V10_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV10Preflight(
    process.cwd(),
    verifyTableLiveV10Authorization(),
    attestationPath,
