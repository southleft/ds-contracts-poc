...
import { COMBOBOX_LIVE_V19_INDEX_PATH } from "./build-combobox-live-proof-v19.js";
import {
  COMBOBOX_LIVE_V19_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V19_TARGET,
} from "./combobox-live-v19-broker.js";
import {
  buildComboboxLiveV19AuthorizationArtifact,
  validateComboboxLiveV19AntecedentIndex,
  validateComboboxLiveV19AuthorizationArtifact,
  validateComboboxLiveV19History,
  validateComboboxLiveV19UnitTestSource,
  type ComboboxLiveV19AntecedentIndex,
  type ComboboxLiveV19AuthorizationArtifact,
  type ComboboxLiveV19AuthorizationProof,
  type ComboboxLiveV19HistoryState,
} from "./combobox-live-v19-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV19ControlFlowSource,
  validateComboboxLiveV19SecurityAttestation,
  type ComboboxLiveV19SecurityAttestation,
} from "./combobox-live-v19-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v19-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V19_INDEX_PATH);
const index = (): ComboboxLiveV19AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV19AntecedentIndex;

const artifact = (): ComboboxLiveV19AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV19AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV19HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV19HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV19AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v19/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV19AuthorizationArtifact,
): ComboboxLiveV19SecurityAttestation => {
  const body: Omit<ComboboxLiveV19SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V19_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV19History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV19History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV19History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV19History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV19History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV19UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV19UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV19AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV19AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV19AuthorizationArtifact(
        planted as ComboboxLiveV19AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV19SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV19SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV19SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV19ControlFlowSource(source), []);
});
