...
export interface InputLiveV73LifecycleSimulationReport {
  artifactVersion: "input-live-v73-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV73AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV73Lifecycle(): Promise<InputLiveV73LifecycleSimulationReport> {
  await buildInputLiveV73Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V73_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV73AntecedentIndex;
  const indexSha256 = inputLiveV73Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v73-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V73_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V73_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV73History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV73History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV73AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V73_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V73_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV73History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV73History(authorized, "pending");
    ...
    await buildInputLiveV73Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V73_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV73AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV73Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV73Lifecycle(), null, 2)}\n`,
  );
