...
import {
  validateComboboxLiveV21AntecedentIndex,
  verifyComboboxLiveV21Authorization,
  type ComboboxLiveV21AntecedentIndex,
  type ComboboxLiveV21AuthorizationProof,
} from "./combobox-live-v21-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V21_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v21/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V21_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v21-security-attestation.json";

export interface ComboboxLiveV21SecurityAttestation {
  artifactVersion: "combobox-live-v21-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV21PreflightReport {
  artifactVersion: "combobox-live-v21-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV21SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV21SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV21AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v21-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V21_TARGET) ||
...
export function validateComboboxLiveV21ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV21Authorization");
  const preflight = source.indexOf("runComboboxLiveV21Preflight");
  const initialize = source.indexOf("ComboboxLiveV21Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV21Preflight(
  root: string,
  proof: ComboboxLiveV21AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V21_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV21PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V21_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V21_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V21_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV21AntecedentIndex;
  failures.push(...validateComboboxLiveV21AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V21_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V21_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV21SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v21-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV21ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v21 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV21SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v21-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V21_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV21Preflight(
    process.cwd(),
    verifyComboboxLiveV21Authorization(),
    attestationPath,
