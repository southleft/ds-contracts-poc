...
export type CalendarLiveV23HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV23AuthorizationArtifact {
  artifactVersion: "calendar-live-v23-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V23_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV23AntecedentIndex {
...
export interface CalendarLiveV23HistoryState {
  index?: CalendarLiveV23AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV23AuthorizationArtifact;
...
export interface CalendarLiveV23AuthorizationProof
  extends CalendarLiveV23TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V23_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V23_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V23_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V23_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v23-authorization") ||
  artifactPath.includes("calendar-live-v23-preflight") ||
  artifactPath.includes("calendar-live-v23-authorized") ||
  artifactPath.includes("create-calendar-live-v23-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v23-status") ||
...
export function validateCalendarLiveV23AntecedentIndex(
  index: CalendarLiveV23AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v23-antecedent-index-v1" ||
...
export function validateCalendarLiveV23UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV23History\(\s*["']/.test(source) ||
    /readCalendarLiveV23HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV23AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV23AuthorizationArtifact {
  ...
  ) as CalendarLiveV23AntecedentIndex;
  const indexFailures = validateCalendarLiveV23AntecedentIndex(index);
...
export function validateCalendarLiveV23AuthorizationArtifact(
  artifact: CalendarLiveV23AuthorizationArtifact | undefined,
  index: CalendarLiveV23AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v23-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v23" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V23_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V23_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V23_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V23_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V23_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V23_SOURCE_ROOTS ||
...
export function validateCalendarLiveV23History(
  state: CalendarLiveV23HistoryState,
  expected: CalendarLiveV23HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV23AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV23AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV23HistoryState(
  root = process.cwd(),
): CalendarLiveV23HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V23_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV23AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V23_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V23_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V23_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV23History(
  expected: CalendarLiveV23HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV23HistoryState {
  const state = readCalendarLiveV23HistoryState(root);
  const failures = validateCalendarLiveV23History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v23 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV23Authorization(
  root = process.cwd(),
): CalendarLiveV23AuthorizationProof {
  const state = verifyCalendarLiveV23History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V23_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v23/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v23.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V23_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V23_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V23_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV23HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV23History(expected);
  process.stdout.write(
    `Calendar live v23 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
