...
import { buildCalendarLiveV46CleanupRuntime } from "./calendar-live-v46-cleanup.js";
import { type CalendarLiveV46FixedFixedPoint } from "./calendar-live-v46-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV46RestoreProgram,
  validateCalendarLiveV46RestorePayload,
  type CalendarLiveV46RestorePayload,
} from "./calendar-live-v46-restore.js";
import {
  buildCalendarLiveV46RawPropertyRuntime,
  normalizeCalendarLiveV46Scene,
  type CalendarLiveV46RawNode,
} from "./calendar-live-v46-verifier.js";
...
export const CALENDAR_LIVE_V46_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V46_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V46_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V46_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V46_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V46_SET_COUNT = 3;
export const CALENDAR_LIVE_V46_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V46_HOST_PHASES = 3;
export const CALENDAR_LIVE_V46_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V46_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V46_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V46_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV46SourceId = (typeof CALENDAR_LIVE_V46_SOURCE_IDS)[number];
export type CalendarLiveV46Kind = "calendar" | "week" | "day";

export interface CalendarLiveV46SourceIdentity {
  source: CalendarLiveV46SourceId;
...
export interface CalendarLiveV46WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV46ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV46SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV46RawNode;
    weekScene: CalendarLiveV46RawNode;
    dayScene: CalendarLiveV46RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV46RootProof {
  source: CalendarLiveV46SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV46FixedFixedPoint;
}

export interface CalendarLiveV46ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV46SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV46CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV46SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV46CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV46SourceId;
  frameWidth: number;
...
export interface CalendarLiveV46CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v46 ${label} must be an object`);
...
export function validateCalendarLiveV46WriterPayload(
  value: unknown,
): CalendarLiveV46WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v46") ||
    ...
    result.namespace !== CALENDAR_LIVE_V46_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v46 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v46 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v46 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV46SourceIdentity,
  kind: CalendarLiveV46Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV46ExtractProgram(
  writer: CalendarLiveV46WriterOwnership,
  sources: readonly CalendarLiveV46SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v46 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V46_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V46_NAMESPACE)}
${buildCalendarLiveV46RawPropertyRuntime()}
...
export function validateCalendarLiveV46ExtractPayload(
  value: unknown,
  writer: CalendarLiveV46WriterOwnership,
): CalendarLiveV46ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v46 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V46_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v46 extract schema/one-root mismatch");
  return payload as CalendarLiveV46ExtractPayload;
}

export function proveCalendarLiveV46Roots<Instance>(
  extract: CalendarLiveV46ExtractPayload,
  sources: readonly (CalendarLiveV46SourceIdentity & {
...
): CalendarLiveV46RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v46 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v46 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV46Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV46Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV46Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV46FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v46 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v46 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v46 compile lost owned sets");
...
export function assertCalendarLiveV46RootProofs(
  proofs: readonly CalendarLiveV46RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v46 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v46 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV46ProbeProgram(
  writer: CalendarLiveV46WriterOwnership,
  sources: readonly CalendarLiveV46SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V46_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV46ProbePayload(
  value: unknown,
  writer: CalendarLiveV46WriterOwnership,
): CalendarLiveV46ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV46ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V46_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V46_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v46 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV46CaptureManifest(
  cells: readonly CalendarLiveV46CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V46_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V46_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V46_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V46_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v46 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV46CaptureCell[],
): string => {
  validateCalendarLiveV46CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV46CaptureProgram(
  writer: CalendarLiveV46WriterOwnership,
  cell: CalendarLiveV46CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V46_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v46 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v46 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V46_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV46CapturePayload(
  value: unknown,
  cell: CalendarLiveV46CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV46CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV46CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V46_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V46_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v46 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV46CaptureResponses(
  manifest: readonly CalendarLiveV46CaptureCell[],
  responses: readonly CalendarLiveV46CapturePayload[],
): void {
  validateCalendarLiveV46CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v46 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV46ObjectiveReport {
  artifactVersion: "calendar-live-v46-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV46Objective(
  manifest: readonly CalendarLiveV46CaptureCell[],
  responses: readonly CalendarLiveV46CapturePayload[],
): CalendarLiveV46ObjectiveReport {
  assertCalendarLiveV46CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v46-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV46CleanupProgram(
  writer: CalendarLiveV46WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV46CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V46_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v46 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV46CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV46WriterOwnership,
): CalendarLiveV46CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV46CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v46 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V46_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV46WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
