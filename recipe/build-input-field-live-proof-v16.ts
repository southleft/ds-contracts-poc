...
import {
  ...
  inputLiveV16RequestSequence,
} from "./input-field-live-v16-broker.js";
import {
  V12_WRITER_PAYLOAD_SHA256,
  V12_WRITER_PROGRAM_SHA256,
  V15_RESTORE_BLUEPRINT_SHA256,
} from "./input-field-live-v16-restore.js";

export const INPUT_LIVE_V16_EVIDENCE_ROOT =
  "recipe/evidence/input-field-live-pivot-v16";
export const INPUT_LIVE_V16_PROTOCOL_PATH = `${INPUT_LIVE_V16_EVIDENCE_ROOT}/protocol.json`;
export const INPUT_LIVE_V16_PLAN_PATH = `${INPUT_LIVE_V16_EVIDENCE_ROOT}/proof-plan.json`;
export const INPUT_LIVE_V16_CAPTURE_MANIFEST_PATH = `${INPUT_LIVE_V16_EVIDENCE_ROOT}/capture-manifest.json`;
export const INPUT_LIVE_V16_REQUEST_MANIFEST_PATH = `${INPUT_LIVE_V16_EVIDENCE_ROOT}/request-manifest.json`;
export const INPUT_LIVE_V16_INDEX_PATH = `${INPUT_LIVE_V16_EVIDENCE_ROOT}/antecedent-index.json`;
export const INPUT_LIVE_V16_AUTHORIZATION_TEMPLATE_PATH = `${INPUT_LIVE_V16_EVIDENCE_ROOT}/authorization-template.json`;
export const INPUT_LIVE_V16_AUTHORIZATION_PATH = `${INPUT_LIVE_V16_EVIDENCE_ROOT}/capture-authorization.json`;

const V15_PROOF_ROOT = "recipe/evidence/input-field-live-pivot-v15";
const V15_PLAN = `${V15_PROOF_ROOT}/proof-plan.json`;
const V15_CAPTURE_MANIFEST = `${V15_PROOF_ROOT}/capture-manifest.json`;
...
export interface InputLiveV16ProofPlan {
  artifactVersion: "input-live-v16-proof-plan-v1";
...
interface GeneratedSource {
  ...
  recipeHash: string;
  envelopeHash: string;
  expectedScenePlan: InputLiveV16SourceIdentity["expectedScenePlan"];
  outputPath: string;
  compressed: Buffer;
  metadata: InputLiveV16ProofPlan["sources"][number]["expectedScenePlan"];
}

const sourcePlans = (): GeneratedSource[] => {
  const prior = json<Record<string, any>>(V15_PLAN);
  return prior.sources.map((source: Record<string, any>) => {
    ...
    return {
      ...
      outputPath: `${INPUT_LIVE_V16_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
      compressed,
      metadata: {
        path: `${INPUT_LIVE_V16_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
...
const captureCells = (): InputLiveV16CaptureCell[] => {
  const cells = json<Record<string, any>>(V15_CAPTURE_MANIFEST)
    .cells as InputLiveV16CaptureCell[];
  inputLiveV16CaptureManifestSha256(cells);
  return cells;
};

const protocol = () => ({
  artifactVersion: "input-live-v16-external-operator-protocol-v1",
  protocolId: "input-live-v16",
  status:
  ...
  hostNormalization: {
    ...
    carriedSceneReadback: "recipe/scene-readback-v16.ts",
    carriedSceneReadbackRuntime: "recipe/scene-readback-runtime-v16.ts",
    carriedV3Verifier: "recipe/input-field-live-v3-verifier-v16.ts",
...
const requestManifest = (
  captures: readonly InputLiveV16CaptureCell[],
  programs: {
    writerSha256: string;
  ...
  const requests = [
    {
      requestId: "writer",
      sequence: inputLiveV16RequestSequence("writer"),
      phase: "writer",
      predecessorRequestId: null,
      programSha256: programs.writerSha256,
      response: INPUT_LIVE_V16_RESPONSE_CONTRACTS.writer,
    },
    {
      requestId: "cleanup",
      sequence: inputLiveV16RequestSequence("cleanup"),
      phase: "cleanup",
      predecessorRequestId: "writer",
      programBuilderSha256: programs.cleanupBuilderSha256,
      response: INPUT_LIVE_V16_RESPONSE_CONTRACTS.cleanup,
      availability: "persisted immediately after writer acceptance",
    },
    {
      requestId: "restore",
      sequence: inputLiveV16RequestSequence("restore"),
      phase: "restore",
      predecessorRequestId: "writer",
      programSha256: programs.restoreBlueprintSha256,
      response: INPUT_LIVE_V16_RESPONSE_CONTRACTS.restore,
      availability:
        "issued immediately after writer acceptance; before extract",
    },
    {
      requestId: "extract",
      sequence: inputLiveV16RequestSequence("extract"),
      phase: "extract",
      predecessorRequestId: "restore",
      programSha256: programs.extractBlueprintSha256,
      response: INPUT_LIVE_V16_RESPONSE_CONTRACTS.extract,
    },
    {
      requestId: "probe",
      sequence: inputLiveV16RequestSequence("probe"),
      phase: "probe",
      predecessorRequestId: "extract",
      programSha256: programs.probeBlueprintSha256,
      response: INPUT_LIVE_V16_RESPONSE_CONTRACTS.probe,
    },
    ...captures.map((cell) => ({
      requestId: `capture-${String(cell.index).padStart(3, "0")}`,
      sequence: inputLiveV16RequestSequence("capture", cell.index),
      phase: "capture",
      ...
      response: INPUT_LIVE_V16_RESPONSE_CONTRACTS.capture,
    })),
  ];
  return {
    artifactVersion: "input-live-v16-request-manifest-v1",
    expectedDynamicTool: INPUT_LIVE_V16_DYNAMIC_TOOL,
    target: INPUT_LIVE_V16_TARGET,
...
const lifecycleExcludedPaths = [
  INPUT_LIVE_V16_AUTHORIZATION_TEMPLATE_PATH,
  ...
  "recipe/input-field-live-v16-authorized.ts",
...
const sourceNeutralDependencies = [
  ...
  "recipe/input-field-live-v16-verifier.ts",
  "recipe/figma-property-normalizer-v8.ts",
  "recipe/figma-runtime-portability.ts",
  "recipe/input-field-objective-comparison-v1.ts",
  "recipe/normalize.ts",
  "recipe/scene-readback-runtime-v16.ts",
  "recipe/scene-readback-v16.ts",
  "recipe/input-field-live-v16-restore.ts",
] as const;

export async function buildInputLiveV16Proof(
  check = process.argv.includes("--check"),
): Promise<InputLiveV16ProofPlan> {
  const captures = captureCells();
  const sources = sourcePlans();
  const priorPlan = json<Record<string, any>>(V15_PLAN);
  ...
  const priorRestore = readFileSync(
    `${V15_PROOF_ROOT}/programs/restore-blueprint.js`,
  );
  if (
    ...
    sha256(priorRestore) !== V15_RESTORE_BLUEPRINT_SHA256
  )
    throw new Error(
      "Input live v16 exact committed v15 writer or restore bytes drifted",
    );

  const writerOwnershipBlueprint: InputLiveV16WriterOwnership = {
    pageId: "__WRITER_PAGE_ID__",
    pageName: "Recipe Pivot / Input Field / v9",
    runIdentity: "input-live-v16",
  ...
  const identities = sources.map(
    (source) =>
      ...
      }) satisfies InputLiveV16SourceIdentity,
  );
  const restoreBlueprint = Buffer.from(
    buildInputLiveV16RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildInputLiveV16ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildInputLiveV16ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildInputLiveV16CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );

  const antecedentOutputs = new Map<string, Buffer>();
  antecedentOutputs.set(INPUT_LIVE_V16_PROTOCOL_PATH, jsonBytes(protocol()));
  antecedentOutputs.set(
    `${INPUT_LIVE_V16_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V16_EVIDENCE_ROOT}/programs/writer-payload.js`,
    writerPayload,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V16_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V16_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V16_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V16_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  ...
  const captureManifest = {
    artifactVersion: "input-live-v16-capture-manifest-v1",
    ...
    cells: captures,
    cellsSha256: inputLiveV16CaptureManifestSha256(captures),
  };
  ...
  const proofPlan: InputLiveV16ProofPlan = {
    artifactVersion: "input-live-v16-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: INPUT_LIVE_V16_TARGET,
    namespace: "ds.contracts.input.recipe.v5",
    writer: {
      programPath: `${INPUT_LIVE_V16_EVIDENCE_ROOT}/programs/writer.txt`,
      programBytes: writerProgram.byteLength,
      programSha256: sha256(writerProgram),
      payloadPath: `${INPUT_LIVE_V16_EVIDENCE_ROOT}/programs/writer-payload.js`,
    ...
    captureManifest: {
      path: INPUT_LIVE_V16_CAPTURE_MANIFEST_PATH,
      cells: 128,
      requests: 128,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(INPUT_LIVE_V16_PLAN_PATH, proofPlanBytes);
  ...
  antecedentOutputs.set(
    INPUT_LIVE_V16_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/input-field-live-v16-broker.ts",
    "recipe/input-field-live-v16-contract.ts",
    "recipe/run-input-field-live-v16.ts",
    "recipe/build-input-field-live-proof-v16.ts",
    "recipe/input-field-live-v16-broker.test.ts",
    "recipe/figma-property-normalizer-v8.test.ts",
    "recipe/scene-readback-v16.test.ts",
    "recipe/input-field-live-v16-restore.ts",
    "recipe/input-field-live-v16-restore.test.ts",
    "recipe/input-field-live-v16-lifecycle-simulation.ts",
    ...sourceNeutralDependencies,
  ].filter((artifactPath) => artifactPath !== INPUT_LIVE_V16_INDEX_PATH);
  ...
  const index = {
    ...
    counts: {
      ...
      captureCells: INPUT_LIVE_V16_CAPTURE_COUNT,
  ...
  antecedentOutputs.set(INPUT_LIVE_V16_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "input-live-v16-authorization-template-v1",
  ...
  if (check) {
    ...
    mkdirSync(path.dirname(INPUT_LIVE_V16_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(INPUT_LIVE_V16_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(INPUT_LIVE_V16_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(INPUT_LIVE_V16_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildInputLiveV16Proof(), null, 2)}\n`,
  );
