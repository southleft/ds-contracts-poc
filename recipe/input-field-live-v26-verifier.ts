...
export const INPUT_LIVE_V26_VERIFIER_VERSION = "input-live-v26-verifier-v1";

export function buildInputLiveV26RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV26RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV26RawNode[];
}

export interface InputLiveV26NormalizedScene {
...
export interface InputLiveV26SceneProof {
...
export function normalizeInputLiveV26Scene(
  root: InputLiveV26RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV26NormalizedScene {
  const canonicalBindings: InputLiveV26NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV26RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV26Scene<Instance>(
  normalized: InputLiveV26NormalizedScene,
...
export function compileInputLiveV26ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV26PreCaptureGates(payload: unknown): void {
