...
import {
  acceptComboboxLiveV31Response,
  createComboboxLiveV31Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV31Request,
  persistComboboxLiveV31RawResponse,
  persistComboboxLiveV31TechnicalGates,
  readComboboxLiveV31AcceptedRawPayload,
  type ComboboxLiveV31Request,
  type ComboboxLiveV31TransactionAuthorization,
} from "./combobox-live-v31-broker.js";
import {
  COMBOBOX_LIVE_V31_CAPTURE_COUNT,
  assertComboboxLiveV31CaptureResponses,
  ...
  evaluateComboboxLiveV31Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV31Roots,
  validateComboboxLiveV31CaptureManifest,
  validateComboboxLiveV31CapturePayload,
  validateComboboxLiveV31CleanupPayload,
  validateComboboxLiveV31ExtractPayload,
  validateComboboxLiveV31ProbePayload,
  validateComboboxLiveV31WriterPayload,
  type ComboboxLiveV31CaptureCell,
  type ComboboxLiveV31CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V31_PLAN_PATH,
  type ComboboxLiveV31ProofPlan,
} from "./build-combobox-live-proof-v31.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v31.js";

export const COMBOBOX_LIVE_V31_RUNNER_VERSION = "combobox-live-v31-runner-v1";

interface ComboboxLiveV31JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV31OrchestratorState {
  artifactVersion: "combobox-live-v31-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV31JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV31ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV31CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV31SourceIdentity & {
...
export interface ComboboxLiveV31NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV31OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v31 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V31_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V31_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV31ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV31CaptureCell[];
  validateComboboxLiveV31CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v31 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v31 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV31WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV31RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV31Request =>
  readJson<ComboboxLiveV31Request>(file);

export class ComboboxLiveV31Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV31Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV31OrchestratorState = {
      artifactVersion: "combobox-live-v31-orchestrator-state-v1",
    ...
    return new ComboboxLiveV31Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV31OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV31Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV31NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV31NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v31 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV31NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v31 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV31RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV31Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV31CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV31AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV31WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV31Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV31ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV31Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV31Response(this.#directory, "writer", {
        validate: validateComboboxLiveV31WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV31Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV31RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV31Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV31ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV31Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV31RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v31-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV31Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV31ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV31Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV31ProbePayload(payload, ownership),
      });
      persistComboboxLiveV31TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v31 capture cell missing");
      const accepted = acceptComboboxLiveV31Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV31CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V31_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV31CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV31Objective(
    ...
    throw new Error(`Combobox live v31 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV31Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV31CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV31Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV31RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV31Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV31CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV31CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV31CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV31Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV31TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV31SimulationKey = (): KeyObject =>
