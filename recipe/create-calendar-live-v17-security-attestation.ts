...
import { verifyCalendarLiveV17Authorization } from "./calendar-live-v17-authorization.js";
import { CALENDAR_LIVE_V17_TARGET } from "./calendar-live-v17-broker.js";
import {
  CALENDAR_LIVE_V17_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV17Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v17-session";
...
const attestation: CalendarLiveV17SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV17SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v17 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V17_SECURITY_ATTESTATION_DEFAULT_PATH,
);
