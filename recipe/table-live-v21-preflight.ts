...
import {
  validateTableLiveV21AntecedentIndex,
  verifyTableLiveV21Authorization,
  type TableLiveV21AntecedentIndex,
  type TableLiveV21AuthorizationProof,
} from "./table-live-v21-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V21_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v21/operator-security-attestation-template.json";
export const TABLE_LIVE_V21_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v21-security-attestation.json";

export interface TableLiveV21SecurityAttestation {
  artifactVersion: "table-live-v21-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV21PreflightReport {
  artifactVersion: "table-live-v21-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV21SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV21SecurityAttestation(
  value: unknown,
  proof: TableLiveV21AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v21-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V21_TARGET) ||
...
export function validateTableLiveV21ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV21Authorization");
  const preflight = source.indexOf("runTableLiveV21Preflight");
  const initialize = source.indexOf("TableLiveV21Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV21Preflight(
  root: string,
  proof: TableLiveV21AuthorizationProof,
  attestationPath = TABLE_LIVE_V21_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV21PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V21_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V21_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV21AntecedentIndex;
  failures.push(...validateTableLiveV21AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V21_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V21_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV21SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v21-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV21ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v21 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV21SecurityAttestation;
  return {
    artifactVersion: "table-live-v21-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V21_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV21Preflight(
    process.cwd(),
    verifyTableLiveV21Authorization(),
    attestationPath,
