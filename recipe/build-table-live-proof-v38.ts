...
import {
  ...
  TABLE_LIVE_V38_VARIANT_COUNT,
  buildTableLiveV38CaptureProgram,
  buildTableLiveV38CleanupProgram,
  buildTableLiveV38ExtractProgram,
  buildTableLiveV38ProbeProgram,
  buildTableLiveV38RestoreProgram,
  tableLiveV2CaptureManifestSha256,
  type TableLiveV38CaptureCell,
  type TableLiveV38SourceIdentity,
  type TableLiveV38WriterOwnership,
...
export const TABLE_LIVE_V38_EVIDENCE_ROOT =
  "recipe/evidence/table-live-pivot-v38";
export const TABLE_LIVE_V38_PROTOCOL_PATH = `${TABLE_LIVE_V38_EVIDENCE_ROOT}/protocol.json`;
export const TABLE_LIVE_V38_PLAN_PATH = `${TABLE_LIVE_V38_EVIDENCE_ROOT}/proof-plan.json`;
export const TABLE_LIVE_V38_CAPTURE_MANIFEST_PATH = `${TABLE_LIVE_V38_EVIDENCE_ROOT}/capture-manifest.json`;
export const TABLE_LIVE_V38_REQUEST_MANIFEST_PATH = `${TABLE_LIVE_V38_EVIDENCE_ROOT}/request-manifest.json`;
export const TABLE_LIVE_V38_INDEX_PATH = `${TABLE_LIVE_V38_EVIDENCE_ROOT}/antecedent-index.json`;
export const TABLE_LIVE_V38_AUTHORIZATION_TEMPLATE_PATH = `${TABLE_LIVE_V38_EVIDENCE_ROOT}/authorization-template.json`;
export const TABLE_LIVE_V38_AUTHORIZATION_PATH = `${TABLE_LIVE_V38_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface TableLiveV38ProofPlan {
  artifactVersion: "table-live-v38-proof-plan-v1";
...
const captureCells = (
...
): TableLiveV38CaptureCell[] => {
  const cells: TableLiveV38CaptureCell[] = [];
  ...
  if (cells.length !== TABLE_LIVE_V38_CAPTURE_COUNT)
    throw new Error(
      `Table live v38 capture cells ${cells.length}, expected ${TABLE_LIVE_V38_CAPTURE_COUNT}`,
    );
...
const protocol = (expectedFacts: number) => ({
  artifactVersion: "table-live-v38-external-operator-protocol-v1",
  protocolId: "table-live-v38",
  status:
  ...
  denominator: {
    ...
    sets: 6,
    variants: TABLE_LIVE_V38_VARIANT_COUNT,
    captureCells: TABLE_LIVE_V38_CAPTURE_COUNT,
...
export async function buildTableLiveV38Proof(
  check = process.argv.includes("--check"),
): Promise<TableLiveV38ProofPlan> {
  ...
  if (
    ...
    throw new Error(
      "Table live v38 writer reused Input, Combobox, or Table v1–v24 identity",
    );
  const captures = captureCells(sources);
  const writerProgram = Buffer.from(writer.code);
  const writerOwnershipBlueprint: TableLiveV38WriterOwnership = {
  ...
  const identities: TableLiveV38SourceIdentity[] = sources.map((source) => ({
    source: source.source,
    adapterIdentity: source.adapterIdentity,
    recipeHash: source.recipeHash,
    envelopeHash: source.envelope.integrity.canonicalHash,
  ...
  const restoreBlueprint = Buffer.from(
    buildTableLiveV38RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildTableLiveV38ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildTableLiveV38ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildTableLiveV38CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );
  const cleanupBlueprint = Buffer.from(
    buildTableLiveV38CleanupProgram(writerOwnershipBlueprint),
  );
  ...
  antecedentOutputs.set(
    TABLE_LIVE_V38_PROTOCOL_PATH,
    jsonBytes(protocol(expectedFacts)),
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V38_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V38_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V38_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V38_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V38_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V38_EVIDENCE_ROOT}/programs/cleanup-blueprint.js`,
    cleanupBlueprint,
  );

  const captureManifest = {
    artifactVersion: "table-live-v38-capture-manifest-v1",
  ...
  antecedentOutputs.set(
    TABLE_LIVE_V38_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: TableLiveV38ProofPlan = {
    artifactVersion: "table-live-v38-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: TABLE_LIVE_V38_TARGET,
    namespace: TABLE_FIGMA_NAMESPACE,
    writer: {
      programPath: `${TABLE_LIVE_V38_EVIDENCE_ROOT}/programs/writer.txt`,
    ...
    captureManifest: {
      path: TABLE_LIVE_V38_CAPTURE_MANIFEST_PATH,
      cells: 20,
      requests: 20,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(TABLE_LIVE_V38_PLAN_PATH, jsonBytes(proofPlan));

  const requests = {
    artifactVersion: "table-live-v38-request-manifest-v1",
    expectedDynamicTool: TABLE_LIVE_V38_DYNAMIC_TOOL,
    target: TABLE_LIVE_V38_TARGET,
  ...
  if (requests.requestCount !== TABLE_LIVE_V38_REMOTE_REQUESTS)
    throw new Error("Table live v38 remote request denominator drifted");
  antecedentOutputs.set(
    TABLE_LIVE_V38_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const lifecycleExcludedPaths = [
    TABLE_LIVE_V38_AUTHORIZATION_TEMPLATE_PATH,
    ...
    "recipe/table-live-v38-authorized.ts",
    "recipe/write-table-live-v38-authorization.ts",
    "recipe/create-table-live-v38-security-attestation.ts",
    "recipe/evidence/table-live-pivot-v38/operator-security-attestation-template.json",
    "recipe/evidence/table-live-pivot-v38-status.json",
    "recipe/evidence/status-index.json",
  ];
  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/table-live-v38-broker.ts",
    "recipe/table-live-v38-contract.ts",
    "recipe/table-live-v38-restore.ts",
    "recipe/table-live-v38-cleanup.ts",
    "recipe/table-live-v38-verifier.ts",
    "recipe/table-live-v38-fixed-point.ts",
    "recipe/run-table-live-v38.ts",
    "recipe/build-table-live-proof-v38.ts",
    "recipe/table-live-v38-lifecycle-simulation.ts",
    "recipe/scene-readback-table-v1.ts",
    "recipe/scene-readback-runtime-table-v38.ts",
    "recipe/table-figma-writer.ts",
    "recipe/figma-property-normalizer-v8.ts",
    "recipe/figma-runtime-portability.ts",
    "recipe/normalize.ts",
  ].filter((artifactPath) => artifactPath !== TABLE_LIVE_V38_INDEX_PATH);
  ...
  antecedentOutputs.set(TABLE_LIVE_V38_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "table-live-v38-authorization-template-v1",
  ...
  const attestationTemplate = jsonBytes({
    artifactVersion: "table-live-v38-operator-security-attestation-template-v1",
    status: "template only; complete privately after PREPARE+AUTHORIZE",
    tokenValuesForbidden: true,
    privateDirectory: "private/table-live-v38-security-attestation.json",
  });
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Table live v38 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(TABLE_LIVE_V38_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(TABLE_LIVE_V38_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
    writeFileSync(
      `${TABLE_LIVE_V38_EVIDENCE_ROOT}/operator-security-attestation-template.json`,
      attestationTemplate,
    );
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(TABLE_LIVE_V38_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(TABLE_LIVE_V38_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Table live v38 authorization template drift");
  }
  if (
    writer.namespace !== TABLE_FIGMA_NAMESPACE ||
    writer.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    ...
    throw new Error("Table live v38 identity or cleanup invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildTableLiveV38Proof(), null, 2)}\n`,
  );
