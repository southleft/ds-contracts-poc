...
export type TableLiveV30HistoryExpectation = "pending" | "authorized";

export interface TableLiveV30AuthorizationArtifact {
  artifactVersion: "table-live-v30-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V30_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV30AntecedentIndex {
...
export interface TableLiveV30HistoryState {
  index?: TableLiveV30AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV30AuthorizationArtifact;
...
export interface TableLiveV30AuthorizationProof
  extends TableLiveV30TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V30_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V30_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V30_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V30_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v30-authorization") ||
  artifactPath.includes("table-live-v30-preflight") ||
  artifactPath.includes("table-live-v30-authorized") ||
  artifactPath.includes("create-table-live-v30-security-attestation") ||
  artifactPath.includes("table-live-pivot-v30-status") ||
...
export function validateTableLiveV30AntecedentIndex(
  index: TableLiveV30AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v30-antecedent-index-v1" ||
...
export function validateTableLiveV30UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV30History\(\s*["']/.test(source) ||
    /readTableLiveV30HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV30AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV30AuthorizationArtifact {
  ...
  ) as TableLiveV30AntecedentIndex;
  const indexFailures = validateTableLiveV30AntecedentIndex(index);
...
export function validateTableLiveV30AuthorizationArtifact(
  artifact: TableLiveV30AuthorizationArtifact | undefined,
  index: TableLiveV30AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v30-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v30" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V30_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V30_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V30_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V30_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V30_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V30_SOURCE_ROOTS ||
...
export function validateTableLiveV30History(
  state: TableLiveV30HistoryState,
  expected: TableLiveV30HistoryExpectation,
): string[] {
  const failures = validateTableLiveV30AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV30AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV30HistoryState(
  root = process.cwd(),
): TableLiveV30HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V30_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV30AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V30_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V30_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V30_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV30History(
  expected: TableLiveV30HistoryExpectation,
  root = process.cwd(),
): TableLiveV30HistoryState {
  const state = readTableLiveV30HistoryState(root);
  const failures = validateTableLiveV30History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v30 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV30Authorization(
  root = process.cwd(),
): TableLiveV30AuthorizationProof {
  const state = verifyTableLiveV30History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V30_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v30/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v30.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V30_TARGET,
    expectedDynamicTool: TABLE_LIVE_V30_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V30_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV30HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV30History(expected);
  process.stdout.write(
    `Table live v30 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
