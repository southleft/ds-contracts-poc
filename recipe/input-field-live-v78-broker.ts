...
import {
  assertInputLiveV78RootProofs,
  INPUT_LIVE_V78_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V78_RESPONSE_CONTRACTS,
  type InputLiveV78RootProof,
} from "./input-field-live-v78-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V78_BROKER_VERSION = "input-live-v78-broker-v2";
export const INPUT_LIVE_V78_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V78_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V78_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V78_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V78_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V78_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV78RemotePhase =
  (typeof INPUT_LIVE_V78_REMOTE_PHASES)[number];
export type InputLiveV78AuthorizationMode = "live" | "simulated";

export interface InputLiveV78CommitPins {
...
export interface InputLiveV78TransactionAuthorization extends InputLiveV78CommitPins {
  mode: InputLiveV78AuthorizationMode;
...
export interface InputLiveV78Manifest {
  artifactVersion: "input-live-v78-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V78_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V78_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V78_TARGET;
  authorization: InputLiveV78TransactionAuthorization;
...
export interface InputLiveV78ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV78RequestBody {
  artifactVersion: "input-live-v78-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V78_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV78RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV78CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V78_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV78ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV78Request extends InputLiveV78RequestBody {
...
export interface InputLiveV78Receipt {
  artifactVersion: "input-live-v78-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V78_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV78RemotePhase;
...
export interface InputLiveV78AcceptedResponse<T = unknown> {
  request: InputLiveV78Request;
  receipt: InputLiveV78Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV78IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV78ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V78_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V78_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V78_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V78_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V78_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V78_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V78_TARGET.editorType;
};
...
export const inputLiveV78RequestSequence = (
  phase: InputLiveV78RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV78RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV78RequestPath = (
  directory: string,
  phase: InputLiveV78RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV78ResponsePath = (
  directory: string,
  phase: InputLiveV78RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV78ReceiptPath = (
  directory: string,
  phase: InputLiveV78RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV78Manifest => {
  const manifest = json<InputLiveV78Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v78-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V78_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V78_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV78RemotePhase,
  captureIndex?: number,
): InputLiveV78Request | undefined => {
  const file = inputLiveV78RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV78Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV78RemotePhase,
  captureIndex?: number,
): InputLiveV78Receipt | undefined => {
  const file = inputLiveV78ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV78Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV78Request | undefined;
  receipt: InputLiveV78Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV78RemotePhase,
): InputLiveV78ResponseContract => ({
  ...INPUT_LIVE_V78_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V78_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V78_DYNAMIC_TOOL,
      target: INPUT_LIVE_V78_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v78-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V78_BROKER_VERSION,
  target: INPUT_LIVE_V78_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V78_PROGRAM_START*/${program}/*INPUT_LIVE_V78_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V78_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V78_PROGRAM_END*/";
...
export function createInputLiveV78Transaction(
  directory: string,
  ...
  const manifest: InputLiveV78Manifest = {
    artifactVersion: "input-live-v78-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V78_BROKER_VERSION,
...
export function validateInputLiveV78Request(
  directory: string,
  request: InputLiveV78Request,
): void {
  ...
  const expectedSequence = inputLiveV78RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v78-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V78_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V78_TARGET.fileKey ||
...
export function issueInputLiveV78Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV78RemotePhase,
  program: string,
  options: InputLiveV78IssueOptions = {},
): InputLiveV78Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV78RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV78Request>(targetPath);
    validateInputLiveV78Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV78ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV78TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV78RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V78_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V78_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V78_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV78RequestBody = {
    artifactVersion: "input-live-v78-broker-request-v2",
    brokerVersion: INPUT_LIVE_V78_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV78Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV78Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV78RawResponse(
  directory: string,
  phase: InputLiveV78RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV78RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV78Request>(requestFile);
  if (
  ...
  const target = inputLiveV78ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV78Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v78-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V78_BROKER_VERSION,
...
export function acceptInputLiveV78Response<T = unknown>(
  directory: string,
  phase: InputLiveV78RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV78AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV78ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV78Request>(
    inputLiveV78RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV78Request(directory, request);
  if (phase === "capture") validateInputLiveV78TechnicalGates(directory);
  const rawFile = inputLiveV78ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v78-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V78_BROKER_VERSION ||
...
export function persistInputLiveV78TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV78RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV78RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V78_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v78-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V78_BROKER_VERSION,
...
export function validateInputLiveV78TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v78-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V78_TECHNICAL_GATES.some(
...
export function validateInputLiveV78Receipt(
  directory: string,
  phase: InputLiveV78RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV78Request(directory, request);
  const receipt = json<InputLiveV78Receipt>(
    inputLiveV78ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV78ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v78-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V78_BROKER_VERSION ||
...
export function readInputLiveV78AcceptedRawPayload(
  directory: string,
  phase: InputLiveV78RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV78Receipt(directory, phase, captureIndex);
...
export function createInputLiveV78FakeDynamicResponse(
  request: InputLiveV78Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V78_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V78_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV78PersistedRequests(
  directory: string,
): InputLiveV78Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV78Request>(file)] : [];
    });
}

export const inputLiveV78Sha256 = sha256;
