...
import { buildCalendarLiveV38CleanupRuntime } from "./calendar-live-v38-cleanup.js";
import { type CalendarLiveV38FixedFixedPoint } from "./calendar-live-v38-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV38RestoreProgram,
  validateCalendarLiveV38RestorePayload,
  type CalendarLiveV38RestorePayload,
} from "./calendar-live-v38-restore.js";
import {
  buildCalendarLiveV38RawPropertyRuntime,
  normalizeCalendarLiveV38Scene,
  type CalendarLiveV38RawNode,
} from "./calendar-live-v38-verifier.js";
...
export const CALENDAR_LIVE_V38_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V38_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V38_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V38_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V38_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V38_SET_COUNT = 3;
export const CALENDAR_LIVE_V38_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V38_HOST_PHASES = 3;
export const CALENDAR_LIVE_V38_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V38_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V38_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V38_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV38SourceId = (typeof CALENDAR_LIVE_V38_SOURCE_IDS)[number];
export type CalendarLiveV38Kind = "calendar" | "week" | "day";

export interface CalendarLiveV38SourceIdentity {
  source: CalendarLiveV38SourceId;
...
export interface CalendarLiveV38WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV38ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV38SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV38RawNode;
    weekScene: CalendarLiveV38RawNode;
    dayScene: CalendarLiveV38RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV38RootProof {
  source: CalendarLiveV38SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV38FixedFixedPoint;
}

export interface CalendarLiveV38ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV38SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV38CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV38SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV38CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV38SourceId;
  frameWidth: number;
...
export interface CalendarLiveV38CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v38 ${label} must be an object`);
...
export function validateCalendarLiveV38WriterPayload(
  value: unknown,
): CalendarLiveV38WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v38") ||
    ...
    result.namespace !== CALENDAR_LIVE_V38_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v38 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v38 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v38 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV38SourceIdentity,
  kind: CalendarLiveV38Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV38ExtractProgram(
  writer: CalendarLiveV38WriterOwnership,
  sources: readonly CalendarLiveV38SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v38 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V38_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V38_NAMESPACE)}
${buildCalendarLiveV38RawPropertyRuntime()}
...
export function validateCalendarLiveV38ExtractPayload(
  value: unknown,
  writer: CalendarLiveV38WriterOwnership,
): CalendarLiveV38ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v38 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V38_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v38 extract schema/one-root mismatch");
  return payload as CalendarLiveV38ExtractPayload;
}

export function proveCalendarLiveV38Roots<Instance>(
  extract: CalendarLiveV38ExtractPayload,
  sources: readonly (CalendarLiveV38SourceIdentity & {
...
): CalendarLiveV38RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v38 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v38 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV38Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV38Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV38Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV38FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v38 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v38 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v38 compile lost owned sets");
...
export function assertCalendarLiveV38RootProofs(
  proofs: readonly CalendarLiveV38RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v38 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v38 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV38ProbeProgram(
  writer: CalendarLiveV38WriterOwnership,
  sources: readonly CalendarLiveV38SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V38_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV38ProbePayload(
  value: unknown,
  writer: CalendarLiveV38WriterOwnership,
): CalendarLiveV38ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV38ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V38_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V38_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v38 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV38CaptureManifest(
  cells: readonly CalendarLiveV38CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V38_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V38_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V38_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V38_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v38 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV38CaptureCell[],
): string => {
  validateCalendarLiveV38CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV38CaptureProgram(
  writer: CalendarLiveV38WriterOwnership,
  cell: CalendarLiveV38CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V38_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v38 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v38 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V38_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV38CapturePayload(
  value: unknown,
  cell: CalendarLiveV38CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV38CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV38CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V38_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V38_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v38 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV38CaptureResponses(
  manifest: readonly CalendarLiveV38CaptureCell[],
  responses: readonly CalendarLiveV38CapturePayload[],
): void {
  validateCalendarLiveV38CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v38 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV38ObjectiveReport {
  artifactVersion: "calendar-live-v38-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV38Objective(
  manifest: readonly CalendarLiveV38CaptureCell[],
  responses: readonly CalendarLiveV38CapturePayload[],
): CalendarLiveV38ObjectiveReport {
  assertCalendarLiveV38CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v38-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV38CleanupProgram(
  writer: CalendarLiveV38WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV38CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V38_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v38 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV38CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV38WriterOwnership,
): CalendarLiveV38CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV38CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v38 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V38_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV38WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
