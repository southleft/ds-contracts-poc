...
import {
  verifyCalendarLiveV1FixedSceneFixedPoint,
  type CalendarLiveV1FixedFixedPoint,
} from "./calendar-live-v1-fixed-point.js";

export const CALENDAR_LIVE_V1_VERIFIER_VERSION = "calendar-live-v1-verifier-v1";

export function buildCalendarLiveV1RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV1RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV1RawNode[];
}

export interface CalendarLiveV1NormalizedScene {
...
export interface CalendarLiveV1SceneProof {
...
export function normalizeCalendarLiveV1Scene(
  root: CalendarLiveV1RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV1NormalizedScene {
  const canonicalBindings: CalendarLiveV1NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV1RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV1Scene<Instance>(
  normalized: CalendarLiveV1NormalizedScene,
...
): CalendarLiveV1SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV1FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV1ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV1PreCaptureGates(payload: unknown): void {
