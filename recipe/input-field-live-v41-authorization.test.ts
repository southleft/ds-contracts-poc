...
import { INPUT_LIVE_V41_INDEX_PATH } from "./build-input-field-live-proof-v41.js";
import {
  INPUT_LIVE_V41_DYNAMIC_TOOL,
  INPUT_LIVE_V41_TARGET,
} from "./input-field-live-v41-broker.js";
import {
  buildInputLiveV41AuthorizationArtifact,
  validateInputLiveV41AntecedentIndex,
  validateInputLiveV41AuthorizationArtifact,
  validateInputLiveV41History,
  validateInputLiveV41UnitTestSource,
  type InputLiveV41AntecedentIndex,
  type InputLiveV41AuthorizationArtifact,
  type InputLiveV41AuthorizationProof,
  type InputLiveV41HistoryState,
} from "./input-field-live-v41-authorization.js";
import {
  inputLiveV41AttestationSha256,
  validateInputLiveV41ControlFlowSource,
  validateInputLiveV41SecurityAttestation,
  type InputLiveV41SecurityAttestation,
} from "./input-field-live-v41-preflight.js";
import { inputLiveV41Sha256 } from "./input-field-live-v41-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V41_INDEX_PATH);
const index = (): InputLiveV41AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV41AntecedentIndex;

const artifact = (): InputLiveV41AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV41AuthorizationArtifact({
...
const pendingState = (): InputLiveV41HistoryState => ({
  index: index(),
  indexSha256: inputLiveV41Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV41HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV41AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV41Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V41_TARGET,
  expectedDynamicTool: INPUT_LIVE_V41_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v41/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV41AuthorizationArtifact,
): InputLiveV41SecurityAttestation => {
  const body: Omit<InputLiveV41SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v41-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V41_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV41AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV41History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV41History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV41History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV41History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV41History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV41UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV41UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV41AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV41Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV41AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV41AuthorizationArtifact(
        planted as InputLiveV41AuthorizationArtifact,
        index(),
        inputLiveV41Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV41SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV41AttestationSha256(
      body as Omit<InputLiveV41SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV41SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV41ControlFlowSource(source), []);
});
