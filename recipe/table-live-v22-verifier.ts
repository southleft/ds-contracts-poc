...
import {
  verifyTableLiveV22FixedSceneFixedPoint,
  type TableLiveV22FixedFixedPoint,
} from "./table-live-v22-fixed-point.js";

export const TABLE_LIVE_V22_VERIFIER_VERSION = "table-live-v22-verifier-v1";

export function buildTableLiveV22RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV22RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV22RawNode[];
}

export interface TableLiveV22NormalizedScene {
...
export interface TableLiveV22SceneProof {
...
export function normalizeTableLiveV22Scene(
  root: TableLiveV22RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV22NormalizedScene {
  const canonicalBindings: TableLiveV22NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV22RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV22Scene<Instance>(
  normalized: TableLiveV22NormalizedScene,
...
): TableLiveV22SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV22FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV22ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV22PreCaptureGates(payload: unknown): void {
