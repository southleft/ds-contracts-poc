...
export const INPUT_LIVE_V31_VERIFIER_VERSION = "input-live-v31-verifier-v1";

export function buildInputLiveV31RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV31RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV31RawNode[];
}

export interface InputLiveV31NormalizedScene {
...
export interface InputLiveV31SceneProof {
...
export function normalizeInputLiveV31Scene(
  root: InputLiveV31RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV31NormalizedScene {
  const canonicalBindings: InputLiveV31NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV31RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV31Scene<Instance>(
  normalized: InputLiveV31NormalizedScene,
...
export function compileInputLiveV31ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV31PreCaptureGates(payload: unknown): void {
