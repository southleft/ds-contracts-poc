/**
 * Calendar live v42 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V42_FIXED_VERIFIER_VERSION = "calendar-live-v42-fixed-point-v1";
export const CALENDAR_LIVE_V42_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV42FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV42FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV42FixedSourceProbe {
...
export interface CalendarLiveV42FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV42FixedFixedPoint;
}

export interface CalendarLiveV42FixedHardGateInput {
  thresholds: CalendarLiveV42FixedThresholds;
  sourceProbes: CalendarLiveV42FixedSourceProbe[];
  cells: CalendarLiveV42FixedCellValidation[];
  sceneProofs: CalendarLiveV42FixedSceneProof[];
...
export interface CalendarLiveV42FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V42_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV42FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV42FixedHardGates(
  input: CalendarLiveV42FixedHardGateInput,
): CalendarLiveV42FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV42FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V42_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV42FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV42FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV42FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
