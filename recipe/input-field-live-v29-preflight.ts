...
import {
  ...
  verifyInputLiveV29Authorization,
  type InputLiveV29AntecedentIndex,
  type InputLiveV29AuthorizationProof,
} from "./input-field-live-v29-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V29_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v29/operator-security-attestation-template.json";
export const INPUT_LIVE_V29_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v29-security-attestation.json";

export interface InputLiveV29SecurityAttestation {
...
export interface InputLiveV29PreflightReport {
  artifactVersion: "input-live-v29-preflight-v1";
...
export const inputLiveV29AttestationSha256 = (
  value: Omit<InputLiveV29SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV29SecurityAttestation(
  value: unknown,
  proof: InputLiveV29AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v29-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V29_TARGET) ||
...
export function validateInputLiveV29ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV29Authorization");
  const preflight = source.indexOf("runInputLiveV29Preflight");
  const initialize = source.indexOf("InputLiveV29Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV29Preflight(
  root: string,
  proof: InputLiveV29AuthorizationProof,
  attestationPath = INPUT_LIVE_V29_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV29PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V29_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V29_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V29_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV29AntecedentIndex;
  failures.push(...validateInputLiveV29AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V29_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V29_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v29-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV29ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV29SecurityAttestation;
  return {
    artifactVersion: "input-live-v29-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V29_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV29Preflight(
    process.cwd(),
    verifyInputLiveV29Authorization(),
    attestationPath,
