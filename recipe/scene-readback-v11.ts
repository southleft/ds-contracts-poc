...
/**
 * Carried Input live v11 scene-readback. Keeps the v9/v10 fill-kind
 * teaching and recovers text roles from the first ` :: ` name segment
 * even when a later `font-provenance=` segment would make hashed
 * sceneRole treat the whole name as variant-like.
 */
export const SCENE_READBACK_VERSION = 1;
export const SCENE_READBACK_V11_TAUGHT_FILL_KINDS = [
  "VARIABLE_ALIAS",
  "boundVariablesOnly",
] as const;
...
const sceneLabel = (scene: SceneNodeSnapshot): string => {
  ...
  if (rest.startsWith("font-provenance=")) return scene.name.slice(0, separator);
