...
import {
  ...
  createInputLiveV20FakeDynamicResponse,
  ...
  listInputLiveV20PersistedRequests,
  persistInputLiveV20RawResponse,
  persistInputLiveV20TechnicalGates,
  validateInputLiveV20Receipt,
  validateInputLiveV20Request,
  type InputLiveV20RemotePhase,
...
import {
  ...
  type InputLiveV20ExtractPayload,
  type InputLiveV20ProbePayload,
  type InputLiveV20RestorePayload,
  type InputLiveV20RootProof,
  type InputLiveV20WriterOwnership,
} from "./input-field-live-v20-contract.js";
import {
  InputLiveV20Orchestrator,
  simulatedInputLiveV20Authorization,
} from "./run-input-field-live-v20.js";
...
const captures: InputLiveV20CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV20Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v20-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV20Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV20CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV20FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV20FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V20_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V20_TARGET);
  persistInputLiveV20RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV20Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV20Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV20WriterOwnership,
): InputLiveV20RestorePayload => ({
...
const rootProofs = (): InputLiveV20RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV20CaptureCell,
): InputLiveV20CapturePayload => {
  const bytes = Buffer.from(`input-live-v20-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV20Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV20Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV20RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV20RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV20ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV20ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV20CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV20CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV20CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV20Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV20CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV20PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV20ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV20Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV20Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV20Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV20Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V20_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV20RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V20_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV20Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV20Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV20ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV20Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV20WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV20RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV20Request;
    validateInputLiveV20Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV20Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV20CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v20-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV20Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV20Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV20Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V20_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV20RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V20_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV20Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV20Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV20RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V20_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV20Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV20Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV20Request;
