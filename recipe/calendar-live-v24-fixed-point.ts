/**
 * Calendar live v24 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V24_FIXED_VERIFIER_VERSION = "calendar-live-v24-fixed-point-v1";
export const CALENDAR_LIVE_V24_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV24FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV24FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV24FixedSourceProbe {
...
export interface CalendarLiveV24FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV24FixedFixedPoint;
}

export interface CalendarLiveV24FixedHardGateInput {
  thresholds: CalendarLiveV24FixedThresholds;
  sourceProbes: CalendarLiveV24FixedSourceProbe[];
  cells: CalendarLiveV24FixedCellValidation[];
  sceneProofs: CalendarLiveV24FixedSceneProof[];
...
export interface CalendarLiveV24FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V24_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV24FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV24FixedHardGates(
  input: CalendarLiveV24FixedHardGateInput,
): CalendarLiveV24FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV24FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V24_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV24FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV24FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV24FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
