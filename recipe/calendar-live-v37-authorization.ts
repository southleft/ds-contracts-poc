...
export type CalendarLiveV37HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV37AuthorizationArtifact {
  artifactVersion: "calendar-live-v37-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V37_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV37AntecedentIndex {
...
export interface CalendarLiveV37HistoryState {
  index?: CalendarLiveV37AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV37AuthorizationArtifact;
...
export interface CalendarLiveV37AuthorizationProof
  extends CalendarLiveV37TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V37_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V37_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V37_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V37_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v37-authorization") ||
  artifactPath.includes("calendar-live-v37-preflight") ||
  artifactPath.includes("calendar-live-v37-authorized") ||
  artifactPath.includes("create-calendar-live-v37-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v37-status") ||
...
export function validateCalendarLiveV37AntecedentIndex(
  index: CalendarLiveV37AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v37-antecedent-index-v1" ||
...
export function validateCalendarLiveV37UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV37History\(\s*["']/.test(source) ||
    /readCalendarLiveV37HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV37AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV37AuthorizationArtifact {
  ...
  ) as CalendarLiveV37AntecedentIndex;
  const indexFailures = validateCalendarLiveV37AntecedentIndex(index);
...
export function validateCalendarLiveV37AuthorizationArtifact(
  artifact: CalendarLiveV37AuthorizationArtifact | undefined,
  index: CalendarLiveV37AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v37-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v37" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V37_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V37_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V37_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V37_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V37_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V37_SOURCE_ROOTS ||
...
export function validateCalendarLiveV37History(
  state: CalendarLiveV37HistoryState,
  expected: CalendarLiveV37HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV37AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV37AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV37HistoryState(
  root = process.cwd(),
): CalendarLiveV37HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V37_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV37AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V37_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V37_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V37_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV37History(
  expected: CalendarLiveV37HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV37HistoryState {
  const state = readCalendarLiveV37HistoryState(root);
  const failures = validateCalendarLiveV37History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v37 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV37Authorization(
  root = process.cwd(),
): CalendarLiveV37AuthorizationProof {
  const state = verifyCalendarLiveV37History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V37_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v37/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v37.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V37_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V37_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V37_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV37HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV37History(expected);
  process.stdout.write(
    `Calendar live v37 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
