/**
 * Calendar live v17 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V17_FIXED_VERIFIER_VERSION = "calendar-live-v17-fixed-point-v1";
export const CALENDAR_LIVE_V17_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV17FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV17FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV17FixedSourceProbe {
...
export interface CalendarLiveV17FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV17FixedFixedPoint;
}

export interface CalendarLiveV17FixedHardGateInput {
  thresholds: CalendarLiveV17FixedThresholds;
  sourceProbes: CalendarLiveV17FixedSourceProbe[];
  cells: CalendarLiveV17FixedCellValidation[];
  sceneProofs: CalendarLiveV17FixedSceneProof[];
...
export interface CalendarLiveV17FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V17_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV17FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV17FixedHardGates(
  input: CalendarLiveV17FixedHardGateInput,
): CalendarLiveV17FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV17FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V17_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV17FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV17FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV17FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
