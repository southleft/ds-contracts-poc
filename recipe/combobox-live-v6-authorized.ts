...
import { COMBOBOX_LIVE_V6_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v6.js";
import { verifyComboboxLiveV6Authorization } from "./combobox-live-v6-authorization.js";
import { runComboboxLiveV6Preflight } from "./combobox-live-v6-preflight.js";
import { ComboboxLiveV6Orchestrator } from "./run-combobox-live-v6.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v6 requires exact authorization artifact ${COMBOBOX_LIVE_V6_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v6-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV6Authorization(root);
    runComboboxLiveV6Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV6Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV6Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
