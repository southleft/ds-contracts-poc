...
export const INPUT_LIVE_V84_VERIFIER_VERSION = "input-live-v84-verifier-v1";

export function buildInputLiveV84RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV84RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV84RawNode[];
}

export interface InputLiveV84NormalizedScene {
...
export interface InputLiveV84SceneProof {
...
export function normalizeInputLiveV84Scene(
  root: InputLiveV84RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV84NormalizedScene {
  const canonicalBindings: InputLiveV84NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV84RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV84Scene<Instance>(
  normalized: InputLiveV84NormalizedScene,
...
export function compileInputLiveV84ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV84PreCaptureGates(payload: unknown): void {
