...
import { CALENDAR_LIVE_V4_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v4.js";
import { verifyCalendarLiveV4Authorization } from "./calendar-live-v4-authorization.js";
import { runCalendarLiveV4Preflight } from "./calendar-live-v4-preflight.js";
import { CalendarLiveV4Orchestrator } from "./run-calendar-live-v4.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v4 requires exact authorization artifact ${CALENDAR_LIVE_V4_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v4-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV4Authorization(root);
    runCalendarLiveV4Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV4Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV4Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
