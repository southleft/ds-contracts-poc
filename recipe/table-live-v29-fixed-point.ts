/**
 * Table live v29 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V29_FIXED_VERIFIER_VERSION = "table-live-v29-fixed-point-v1";
export const TABLE_LIVE_V29_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV29FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV29FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV29FixedSourceProbe {
...
export interface TableLiveV29FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV29FixedFixedPoint;
}

export interface TableLiveV29FixedHardGateInput {
  thresholds: TableLiveV29FixedThresholds;
  sourceProbes: TableLiveV29FixedSourceProbe[];
  cells: TableLiveV29FixedCellValidation[];
  sceneProofs: TableLiveV29FixedSceneProof[];
...
export interface TableLiveV29FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V29_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV29FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV29FixedHardGates(
  input: TableLiveV29FixedHardGateInput,
): TableLiveV29FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV29FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V29_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV29FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV29FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
