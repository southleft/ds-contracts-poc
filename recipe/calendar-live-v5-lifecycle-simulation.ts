...
import { calendarLiveV1Sha256 } from "./calendar-live-v5-broker.js";

export interface CalendarLiveV5LifecycleSimulationReport {
  artifactVersion: "calendar-live-v5-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV5AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV5Lifecycle(): Promise<CalendarLiveV5LifecycleSimulationReport> {
  await buildCalendarLiveV5Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V5_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV5AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v5-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V5_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V5_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV5History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV5History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV5AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V5_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V5_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV5History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV5History(authorized, "pending");
    ...
    await buildCalendarLiveV5Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V5_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV5AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV5Lifecycle(), null, 2)}\n`,
  );
