...
import {
  acceptInputLiveV84Response,
  createInputLiveV84Transaction,
  inputLiveV84ReceiptPath,
  inputLiveV84RequestPath,
  inputLiveV84ResponsePath,
  inputLiveV84Sha256,
  issueInputLiveV84Request,
  persistInputLiveV84RawResponse,
  persistInputLiveV84TechnicalGates,
  readInputLiveV84AcceptedRawPayload,
  type InputLiveV84Request,
  type InputLiveV84TransactionAuthorization,
} from "./input-field-live-v84-broker.js";
import {
  INPUT_LIVE_V84_CAPTURE_COUNT,
  assertInputLiveV84CaptureResponses,
  assertInputLiveV84RootProofs,
  buildInputLiveV84CaptureProgram,
  buildInputLiveV84CleanupProgram,
  ...
  evaluateInputLiveV84Objective,
  inputLiveV84CaptureManifestSha256,
  proveInputLiveV84Roots,
  validateInputLiveV84CaptureManifest,
  validateInputLiveV84CapturePayload,
  validateInputLiveV84CleanupPayload,
  validateInputLiveV84ExtractPayload,
  validateInputLiveV84ProbePayload,
  validateInputLiveV84WriterPayload,
  type InputLiveV84CaptureCell,
  type InputLiveV84CapturePayload,
...
import {
  ...
  INPUT_LIVE_V84_PLAN_PATH,
  type InputLiveV84ProofPlan,
...
import type { ExpectedScenePlan } from "./scene-readback-v84.js";

export const INPUT_LIVE_V84_RUNNER_VERSION = "input-live-v84-runner-v1";

interface InputLiveV84JournalEntry {
  index: number;
  event: string;
...
interface InputLiveV84OrchestratorState {
  artifactVersion: "input-live-v84-orchestrator-state-v1";
  ...
  journal: InputLiveV84JournalEntry[];
}

interface LoadedProofInputs {
  plan: InputLiveV84ProofPlan;
  planSha256: string;
  captures: InputLiveV84CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    InputLiveV84SourceIdentity & {
...
export interface InputLiveV84NextAction {
...
const writeState = (
  directory: string,
  state: InputLiveV84OrchestratorState,
): void => {
...
const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, INPUT_LIVE_V84_PLAN_PATH);
  const capturePath = path.join(root, INPUT_LIVE_V84_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as InputLiveV84ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as InputLiveV84CaptureCell[];
  validateInputLiveV84CaptureManifest(captures);
  if (
    plan.captureManifest.sha256 !== inputLiveV84Sha256(captureBytes) ||
    captureArtifact.cellsSha256 !== inputLiveV84CaptureManifestSha256(captures)
  )
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (
      inputLiveV84Sha256(expectedBytes) !== source.expectedScenePlan.sha256 ||
      inputLiveV84Sha256(uncompressed) !==
  ...
  return {
    plan,
    planSha256: inputLiveV84Sha256(planBytes),
    captures,
    captureManifestSha256: inputLiveV84Sha256(captureBytes),
    sources,
...
const appendJournal = (
  ...
  const payloadSha256 = inputLiveV84Sha256(canonicalJson(payload));
  const body = {
  ...
  const entry = {
    ...body,
    entrySha256: inputLiveV84Sha256(canonicalJson(body)),
  };
  writeNew(
    path.join(
      directory,
...
const writerOwnership = (directory: string): InputLiveV84WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): InputLiveV84RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): InputLiveV84Request =>
  readJson<InputLiveV84Request>(file);

export class InputLiveV84Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueInputLiveV84Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: InputLiveV84OrchestratorState = {
      artifactVersion: "input-live-v84-orchestrator-state-v1",
      transactionDirectory: options.transactionDirectory,
      activeRequest: inputLiveV84RequestPath(
        options.transactionDirectory,
        "writer",
      ),
      cleanupRequest: null,
    ...
    return new InputLiveV84Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<InputLiveV84OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new InputLiveV84Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): InputLiveV84NextAction {
    const cleanup =
      this.#state.cleanupRequest &&
      !this.#state.cleanupAccepted &&
      !existsSync(inputLiveV84ReceiptPath(this.#directory, "cleanup"))
        ? this.#state.cleanupRequest
        : null;
  ...
  ingestAndAdvance(rawResponse: string): InputLiveV84NextAction {
  ...
  ingestCleanup(rawResponse: string): InputLiveV84NextAction {
    ...
    persistInputLiveV84RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptInputLiveV84Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV84CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      existsSync(inputLiveV84ReceiptPath(this.#directory, "writer")) &&
      !existsSync(path.join(this.#directory, OWNERSHIP_FILE))
    ) {
      const raw = readInputLiveV84AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateInputLiveV84WriterPayload(raw),
      );
    }
    if (existsSync(inputLiveV84RequestPath(this.#directory, "cleanup")))
      this.#state.cleanupRequest = inputLiveV84RequestPath(
        this.#directory,
        "cleanup",
      );
    if (
      existsSync(path.join(this.#directory, OWNERSHIP_FILE)) &&
      !existsSync(inputLiveV84RequestPath(this.#directory, "cleanup"))
    )
      this.#issueCleanupAndRestore(false);
    else if (
      existsSync(inputLiveV84RequestPath(this.#directory, "cleanup")) &&
      !existsSync(inputLiveV84RequestPath(this.#directory, "restore"))
    )
      this.#issueRestore(false);
    if (
      existsSync(inputLiveV84RequestPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV84ReceiptPath(this.#directory, "restore"))
    )
      this.#state.activeRequest = inputLiveV84RequestPath(
        this.#directory,
        "restore",
      );
    if (
      existsSync(inputLiveV84ReceiptPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV84RequestPath(this.#directory, "extract"))
    ) {
      const ownership = writerOwnership(this.#directory);
      const extract = issueInputLiveV84Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildInputLiveV84ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV84RequestPath(
        this.#directory,
        "extract",
    ...
    if (existsSync(inputLiveV84ReceiptPath(this.#directory, "cleanup")))
      this.#state.cleanupAccepted = true;
    this.#persist();
  }

  #acceptActive(request: InputLiveV84Request): void {
    if (request.phase === "writer") {
      const accepted = acceptInputLiveV84Response(this.#directory, "writer", {
        validate: validateInputLiveV84WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptInputLiveV84Response(this.#directory, "restore", {
        validate: (payload) =>
          validateInputLiveV84RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptInputLiveV84Response(this.#directory, "extract", {
        validate: (payload) =>
          validateInputLiveV84ExtractPayload(payload, ownership),
      });
      const proofs = proveInputLiveV84Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseInputFieldRecipe,
        compileInputFieldRecipe,
      );
      assertInputLiveV84RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "input-live-v84-host-root-proofs-v1",
      ...
      appendJournal(
        ...
        request.requestId,
        { proofsSha256: inputLiveV84Sha256(canonicalJson(proofs)) },
      );
      const next = issueInputLiveV84Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildInputLiveV84ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV84RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptInputLiveV84Response(this.#directory, "probe", {
        validate: (payload) =>
          validateInputLiveV84ProbePayload(payload, ownership),
      });
      persistInputLiveV84TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      const accepted = acceptInputLiveV84Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateInputLiveV84CapturePayload(payload, cell, rawBytes),
      });
      const png = Buffer.from(accepted.payload.pngBase64, "base64");
      const capturePath = path.join(
        this.#directory,
        "captures",
        `${String(cell.index).padStart(3, "0")}-${inputLiveV84Sha256(cell.cellKey).slice(0, 20)}.png`,
      );
      ...
      if (next < INPUT_LIVE_V84_CAPTURE_COUNT) this.#issueCapture(next);
      else {
          ...
          pngBase64: Buffer.from(
            readFileSync(
              path.join(
                this.#directory,
                "captures",
                `${String(response.index).padStart(3, "0")}-${inputLiveV84Sha256(response.cellKey).slice(0, 20)}.png`,
              ),
            ),
          ).toString("base64"),
        }));
        assertInputLiveV84CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateInputLiveV84Objective(
        ...
        appendJournal(
          ...
          request.requestId,
          {
            reportSha256: inputLiveV84Sha256(canonicalJson(objective)),
  ...
  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueInputLiveV84Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildInputLiveV84CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = inputLiveV84RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueInputLiveV84Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildInputLiveV84RestoreProgram(ownership),
    );
    this.#state.activeRequest = inputLiveV84RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueInputLiveV84Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildInputLiveV84CaptureProgram(
  ...
  #readCaptureResponseIndex(): InputLiveV84CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: InputLiveV84CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedInputLiveV84Authorization = (
  privateKey: KeyObject,
): InputLiveV84TransactionAuthorization => ({
  mode: "simulated",
  ...
  signingPublicKeySha256: inputLiveV84Sha256(
...
export const generateInputLiveV84SimulationKey = (): KeyObject =>
