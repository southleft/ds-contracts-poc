...
import { tableLiveV2Sha256 } from "./table-live-v21-broker.js";

export interface TableLiveV21LifecycleSimulationReport {
  artifactVersion: "table-live-v21-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV21AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV21Lifecycle(): Promise<TableLiveV21LifecycleSimulationReport> {
  await buildTableLiveV21Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V21_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV21AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v21-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V21_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V21_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV21History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV21History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV21AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V21_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V21_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV21History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV21History(authorized, "pending");
    ...
    await buildTableLiveV21Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V21_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV21AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV21Lifecycle(), null, 2)}\n`,
  );
