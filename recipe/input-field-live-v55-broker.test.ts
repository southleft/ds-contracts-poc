...
import {
  ...
  createInputLiveV55FakeDynamicResponse,
  ...
  listInputLiveV55PersistedRequests,
  persistInputLiveV55RawResponse,
  persistInputLiveV55TechnicalGates,
  validateInputLiveV55Receipt,
  validateInputLiveV55Request,
  type InputLiveV55RemotePhase,
...
import {
  ...
  type InputLiveV55ExtractPayload,
  type InputLiveV55ProbePayload,
  type InputLiveV55RestorePayload,
  type InputLiveV55RootProof,
  type InputLiveV55WriterOwnership,
} from "./input-field-live-v55-contract.js";
import {
  InputLiveV55Orchestrator,
  simulatedInputLiveV55Authorization,
} from "./run-input-field-live-v55.js";
...
const captures: InputLiveV55CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV55Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v55-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV55Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV55CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV55FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV55FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V55_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V55_TARGET);
  persistInputLiveV55RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV55Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV55Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV55WriterOwnership,
): InputLiveV55RestorePayload => ({
...
const rootProofs = (): InputLiveV55RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV55CaptureCell,
): InputLiveV55CapturePayload => {
  const bytes = Buffer.from(`input-live-v55-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV55Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV55Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV55RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV55RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV55ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV55ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV55CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV55CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV55CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV55Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV55CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV55PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV55ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV55Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV55Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV55Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV55Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V55_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV55RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V55_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV55Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV55Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV55ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV55Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV55WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV55RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV55Request;
    validateInputLiveV55Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV55Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV55CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v55-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV55Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV55Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV55Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V55_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV55RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V55_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV55Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV55Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV55RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V55_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV55Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV55Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV55Request;
