...
export interface ComboboxLiveV13LifecycleSimulationReport {
  artifactVersion: "combobox-live-v13-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV13AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV13Lifecycle(): Promise<ComboboxLiveV13LifecycleSimulationReport> {
  await buildComboboxLiveV13Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V13_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV13AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v13-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V13_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V13_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV13History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV13History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV13AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V13_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V13_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV13History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV13History(authorized, "pending");
    ...
    await buildComboboxLiveV13Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V13_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV13AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV13Lifecycle(), null, 2)}\n`,
  );
