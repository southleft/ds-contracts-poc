...
} from "./input-field-live-v3-verifier-v51.js";
import {
  buildInputLiveV51RawPropertyRuntime,
  normalizeInputLiveV51Scene,
  type InputLiveV51RawNode,
} from "./input-field-live-v51-verifier.js";
...
export const INPUT_LIVE_V51_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V51_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V51_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V51_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V51_VARIANT_COUNT = 256;
export const INPUT_LIVE_V51_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V51_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV51SourceId = (typeof INPUT_LIVE_V51_SOURCE_IDS)[number];

export interface InputLiveV51SourceIdentity {
  source: InputLiveV51SourceId;
...
export interface InputLiveV51WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV51ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV51SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV51RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV51RootProof {
  source: InputLiveV51SourceId;
  adapterIdentity: string;
...
export interface InputLiveV51ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV51SourceId;
    adapterIdentity: string;
...
export interface InputLiveV51CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV51SourceId;
  adapterIdentity: string;
...
export interface InputLiveV51CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV51SourceId;
  strata: InputLiveV51CaptureCell["strata"];
...
export interface InputLiveV51CleanupPayload {
...
export function validateInputLiveV51WriterPayload(
  value: unknown,
): InputLiveV51WriterOwnership {
...
const identityPlan = (source: InputLiveV51SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV51ExtractProgram(
  writer: InputLiveV51WriterOwnership,
  sources: readonly InputLiveV51SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V51_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V51_NAMESPACE)}
${buildInputLiveV51RawPropertyRuntime()}
...
export function validateInputLiveV51ExtractPayload(
  value: unknown,
  writer: InputLiveV51WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V51_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV51ExtractPayload;
}

export function proveInputLiveV51Roots<Instance>(
  extract: InputLiveV51ExtractPayload,
  sources: readonly (InputLiveV51SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV51Scene(
      root.scene,
...
export function assertInputLiveV51RootProofs(
  proofs: readonly InputLiveV51RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV51ProbeProgram(
  writer: InputLiveV51WriterOwnership,
  sources: readonly InputLiveV51SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V51_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV51ProbePayload(
  value: unknown,
  writer: InputLiveV51WriterOwnership,
): InputLiveV51ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV51ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V51_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V51_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV51CaptureManifest(
  cells: readonly InputLiveV51CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V51_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V51_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V51_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V51_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V51_SOURCE_IDS.some(
...
export const inputLiveV51CaptureManifestSha256 = (
  cells: readonly InputLiveV51CaptureCell[],
): string => {
  validateInputLiveV51CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV51CaptureProgram(
  writer: InputLiveV51WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V51_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V51_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V51_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV51CapturePayload(
  value: unknown,
  cell: InputLiveV51CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV51CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V51_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V51_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV51CaptureResponses(
  manifest: readonly InputLiveV51CaptureCell[],
  responses: readonly InputLiveV51CapturePayload[],
): void {
  validateInputLiveV51CaptureManifest(manifest);
  if (
...
export interface InputLiveV51ObjectiveReport {
  artifactVersion: "input-live-v51-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV51Objective(
  manifest: readonly InputLiveV51CaptureCell[],
  responses: readonly InputLiveV51CapturePayload[],
  readReference: (cell: InputLiveV51CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV51CaptureCell) => Buffer,
): InputLiveV51ObjectiveReport {
  assertInputLiveV51CaptureResponses(manifest, responses);
...
export function buildInputLiveV51CleanupProgram(
  writer: InputLiveV51WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V51_NAMESPACE,
...
export function validateInputLiveV51CleanupPayload(
  value: unknown,
  writer: InputLiveV51WriterOwnership,
): InputLiveV51CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV51CleanupPayload;
...
export const INPUT_LIVE_V51_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV51WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
