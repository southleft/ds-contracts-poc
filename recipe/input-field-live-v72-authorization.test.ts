...
import { INPUT_LIVE_V72_INDEX_PATH } from "./build-input-field-live-proof-v72.js";
import {
  INPUT_LIVE_V72_DYNAMIC_TOOL,
  INPUT_LIVE_V72_TARGET,
} from "./input-field-live-v72-broker.js";
import {
  buildInputLiveV72AuthorizationArtifact,
  validateInputLiveV72AntecedentIndex,
  validateInputLiveV72AuthorizationArtifact,
  validateInputLiveV72History,
  validateInputLiveV72UnitTestSource,
  type InputLiveV72AntecedentIndex,
  type InputLiveV72AuthorizationArtifact,
  type InputLiveV72AuthorizationProof,
  type InputLiveV72HistoryState,
} from "./input-field-live-v72-authorization.js";
import {
  inputLiveV72AttestationSha256,
  validateInputLiveV72ControlFlowSource,
  validateInputLiveV72SecurityAttestation,
  type InputLiveV72SecurityAttestation,
} from "./input-field-live-v72-preflight.js";
import { inputLiveV72Sha256 } from "./input-field-live-v72-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V72_INDEX_PATH);
const index = (): InputLiveV72AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV72AntecedentIndex;

const artifact = (): InputLiveV72AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV72AuthorizationArtifact({
...
const pendingState = (): InputLiveV72HistoryState => ({
  index: index(),
  indexSha256: inputLiveV72Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV72HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV72AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV72Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V72_TARGET,
  expectedDynamicTool: INPUT_LIVE_V72_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v72/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV72AuthorizationArtifact,
): InputLiveV72SecurityAttestation => {
  const body: Omit<InputLiveV72SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V72_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV72AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV72History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV72History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV72History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV72History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV72History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV72UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV72UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV72AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV72Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV72AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV72AuthorizationArtifact(
        planted as InputLiveV72AuthorizationArtifact,
        index(),
        inputLiveV72Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV72SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV72AttestationSha256(
      body as Omit<InputLiveV72SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV72SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV72ControlFlowSource(source), []);
});
