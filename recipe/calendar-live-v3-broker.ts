...
import {
  assertCalendarLiveV3RootProofs,
  CALENDAR_LIVE_V3_CAPTURE_COUNT,
  CALENDAR_LIVE_V3_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  CALENDAR_LIVE_V3_RESPONSE_CONTRACTS,
  type CalendarLiveV3RootProof,
} from "./calendar-live-v3-contract.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V3_BROKER_VERSION = "calendar-live-v3-broker-v1";
export const CALENDAR_LIVE_V3_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const CALENDAR_LIVE_V3_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const CALENDAR_LIVE_V3_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const CALENDAR_LIVE_V3_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const CALENDAR_LIVE_V3_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const CALENDAR_LIVE_V3_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type CalendarLiveV3RemotePhase =
  (typeof CALENDAR_LIVE_V3_REMOTE_PHASES)[number];
export type CalendarLiveV3AuthorizationMode = "live" | "simulated";

export interface CalendarLiveV3CommitPins {
...
export interface CalendarLiveV3TransactionAuthorization extends CalendarLiveV3CommitPins {
  mode: CalendarLiveV3AuthorizationMode;
...
export interface CalendarLiveV3Manifest {
  artifactVersion: "calendar-live-v3-broker-manifest-v2";
  brokerVersion: typeof CALENDAR_LIVE_V3_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof CALENDAR_LIVE_V3_DYNAMIC_TOOL;
  target: typeof CALENDAR_LIVE_V3_TARGET;
  authorization: CalendarLiveV3TransactionAuthorization;
...
export interface CalendarLiveV3ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface CalendarLiveV3RequestBody {
  artifactVersion: "calendar-live-v3-broker-request-v2";
  brokerVersion: typeof CALENDAR_LIVE_V3_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: CalendarLiveV3RemotePhase;
  sequence: number;
  ...
  commitPins: CalendarLiveV3CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof CALENDAR_LIVE_V3_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: CalendarLiveV3ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface CalendarLiveV3Request extends CalendarLiveV3RequestBody {
...
export interface CalendarLiveV3Receipt {
  artifactVersion: "calendar-live-v3-broker-receipt-v2";
  brokerVersion: typeof CALENDAR_LIVE_V3_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: CalendarLiveV3RemotePhase;
...
export interface CalendarLiveV3AcceptedResponse<T = unknown> {
  request: CalendarLiveV3Request;
  receipt: CalendarLiveV3Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface CalendarLiveV3IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: CalendarLiveV3ResponseContract;
...
const equalTarget = (value: unknown): value is typeof CALENDAR_LIVE_V3_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === CALENDAR_LIVE_V3_TARGET.fileKey &&
  (value as BrokerTarget).fileName === CALENDAR_LIVE_V3_TARGET.fileName &&
  (value as BrokerTarget).editorType === CALENDAR_LIVE_V3_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== CALENDAR_LIVE_V3_TARGET.fileKey ||
    context.fileName !== CALENDAR_LIVE_V3_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === CALENDAR_LIVE_V3_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Calendar live v3 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= CALENDAR_LIVE_V3_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v3 capture index must be 0..7");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Calendar live v3 authorization pins are malformed");
};

const readManifest = (directory: string): CalendarLiveV3Manifest => {
  const manifest = json<CalendarLiveV3Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Calendar live v3 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "calendar-live-v3-broker-manifest-v2" ||
    manifest.brokerVersion !== CALENDAR_LIVE_V3_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== CALENDAR_LIVE_V3_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(CALENDAR_LIVE_V3_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Calendar live v3 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: CalendarLiveV3Request,
): CalendarLiveV3RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: CalendarLiveV3RemotePhase,
  captureIndex?: number,
): CalendarLiveV3Request | undefined => {
  const file = calendarLiveV1RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV3Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: CalendarLiveV3RemotePhase,
  captureIndex?: number,
): CalendarLiveV3Receipt | undefined => {
  const file = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV3Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: CalendarLiveV3Request | undefined;
  receipt: CalendarLiveV3Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: CalendarLiveV3RemotePhase,
): CalendarLiveV3ResponseContract => ({
  ...CALENDAR_LIVE_V3_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: CalendarLiveV3Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: CALENDAR_LIVE_V3_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: CALENDAR_LIVE_V3_DYNAMIC_TOOL,
      target: CALENDAR_LIVE_V3_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "calendar-live-v3-plugin-response-v2",
  brokerVersion: CALENDAR_LIVE_V3_BROKER_VERSION,
  target: CALENDAR_LIVE_V3_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*CALENDAR_LIVE_V3_PROGRAM_START*/${program}/*CALENDAR_LIVE_V3_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*CALENDAR_LIVE_V3_PROGRAM_START*/";
  const endMarker = "/*CALENDAR_LIVE_V3_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Calendar live v3 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createCalendarLiveV3Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Calendar live v3 private signing key does not match authorized public-key identity",
    );
  const manifest: CalendarLiveV3Manifest = {
    artifactVersion: "calendar-live-v3-broker-manifest-v2",
    brokerVersion: CALENDAR_LIVE_V3_BROKER_VERSION,
    ...
    captureRequests: CALENDAR_LIVE_V3_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateCalendarLiveV3Request(
  directory: string,
  request: CalendarLiveV3Request,
): void {
  ...
  if (
    request.artifactVersion !== "calendar-live-v3-broker-request-v2" ||
    request.brokerVersion !== CALENDAR_LIVE_V3_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== CALENDAR_LIVE_V3_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Calendar live v3 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueCalendarLiveV3Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: CalendarLiveV3RemotePhase,
  program: string,
  options: CalendarLiveV3IssueOptions = {},
): CalendarLiveV3Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<CalendarLiveV3Request>(targetPath);
    validateCalendarLiveV3Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Calendar live v3 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(calendarLiveV1ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Calendar live v3 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Calendar live v3 empty code envelope refused");
  if (phase === "capture") validateCalendarLiveV3TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Calendar live v3 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: CALENDAR_LIVE_V3_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? CALENDAR_LIVE_V3_SIGNED_WRITER_TIMEOUT_MS
        : CALENDAR_LIVE_V3_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: CalendarLiveV3RequestBody = {
    artifactVersion: "calendar-live-v3-broker-request-v2",
    brokerVersion: CALENDAR_LIVE_V3_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: CalendarLiveV3Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateCalendarLiveV3Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistCalendarLiveV3RawResponse(
  directory: string,
  phase: CalendarLiveV3RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Calendar live v3 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<CalendarLiveV3Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Calendar live v3 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = calendarLiveV1ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Calendar live v3 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): CalendarLiveV3Receipt => {
  const body: Omit<CalendarLiveV3Receipt, "receiptSha256"> = {
    artifactVersion: "calendar-live-v3-broker-receipt-v2",
    brokerVersion: CALENDAR_LIVE_V3_BROKER_VERSION,
...
export function acceptCalendarLiveV3Response<T = unknown>(
  directory: string,
  phase: CalendarLiveV3RemotePhase,
  options: {
    captureIndex?: number;
...
): CalendarLiveV3AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Calendar live v3 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<CalendarLiveV3Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Calendar live v3 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "calendar-live-v3-plugin-response-v2" ||
    plugin?.brokerVersion !== CALENDAR_LIVE_V3_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Calendar live v3 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistCalendarLiveV3TechnicalGates(
  directory: string,
  rootProofs: readonly CalendarLiveV3RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertCalendarLiveV3RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Calendar live v3 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Calendar live v3 probe payload hash malformed");
  const gates = Object.fromEntries(
    CALENDAR_LIVE_V3_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "calendar-live-v3-host-technical-gates-v2",
    brokerVersion: CALENDAR_LIVE_V3_BROKER_VERSION,
...
export function validateCalendarLiveV3TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "calendar-live-v3-host-technical-gates-v2" ||
    ...
    CALENDAR_LIVE_V3_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Calendar live v3 technical gates artifact");
}

export function validateCalendarLiveV3Receipt(
  directory: string,
  phase: CalendarLiveV3RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateCalendarLiveV3Request(directory, request);
  const receipt = json<CalendarLiveV3Receipt>(
  ...
  if (
    receipt.artifactVersion !== "calendar-live-v3-broker-receipt-v2" ||
    receipt.brokerVersion !== CALENDAR_LIVE_V3_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Calendar live v3 accepted receipt: ${request.requestId}`,
    );
}

export function readCalendarLiveV3AcceptedRawPayload(
  directory: string,
  phase: CalendarLiveV3RemotePhase,
  captureIndex?: number,
): unknown {
  validateCalendarLiveV3Receipt(directory, phase, captureIndex);
...
export function createCalendarLiveV3FakeDynamicResponse(
  request: CalendarLiveV3Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? CALENDAR_LIVE_V3_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? CALENDAR_LIVE_V3_TARGET,
    ...overrides.extra,
  };
}

export function listCalendarLiveV3PersistedRequests(
  directory: string,
): CalendarLiveV3Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<CalendarLiveV3Request>(file)] : [];
    });
}
