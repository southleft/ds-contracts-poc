...
import {
  ...
  verifyCalendarLiveV48Authorization,
  type CalendarLiveV48AntecedentIndex,
  type CalendarLiveV48AuthorizationProof,
} from "./calendar-live-v48-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V48_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v48/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V48_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v48-security-attestation.json";

export interface CalendarLiveV48SecurityAttestation {
...
export interface CalendarLiveV48PreflightReport {
  artifactVersion: "calendar-live-v48-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV48SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV48SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV48AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v48-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V48_TARGET) ||
...
export function validateCalendarLiveV48ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV48Authorization");
  const preflight = source.indexOf("runCalendarLiveV48Preflight");
  const initialize = source.indexOf("CalendarLiveV48Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV48Preflight(
  root: string,
  proof: CalendarLiveV48AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V48_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV48PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V48_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V48_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV48AntecedentIndex;
  failures.push(...validateCalendarLiveV48AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V48_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V48_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v48-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV48ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v48 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV48SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v48-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V48_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV48Preflight(
    process.cwd(),
    verifyCalendarLiveV48Authorization(),
    attestationPath,
