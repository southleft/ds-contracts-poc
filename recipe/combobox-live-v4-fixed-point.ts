/**
 * Carried Combobox live v4 host verifier. Uses scene-readback-combobox-v1 so live
...
export const COMBOBOX_LIVE_V4_FIXED_VERIFIER_VERSION = "combobox-live-v4-fixed-point-v1";
export const COMBOBOX_LIVE_V4_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV4FixedThresholds {
...
export interface ComboboxLiveV4FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV4FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV4FixedSourceProbe {
...
export interface ComboboxLiveV4FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV4FixedFixedPoint;
}

export interface ComboboxLiveV4FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV4FixedHardGateInput {
  thresholds: ComboboxLiveV4FixedThresholds;
  materialRegression: ComboboxLiveV4FixedMaterialRegression;
  sourceProbes: ComboboxLiveV4FixedSourceProbe[];
  cells: ComboboxLiveV4FixedCellValidation[];
  sceneProofs: ComboboxLiveV4FixedSceneProof[];
  visualRows: ComboboxLiveV4FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV4FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V4_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV4FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV4FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV4FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV4FixedHardGates(
  input: ComboboxLiveV4FixedHardGateInput,
): ComboboxLiveV4FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV4FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV4FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V4_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV4FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV4FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
