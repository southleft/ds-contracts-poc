/**
 * Calendar live v45 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V45_FIXED_VERIFIER_VERSION = "calendar-live-v45-fixed-point-v1";
export const CALENDAR_LIVE_V45_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV45FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV45FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV45FixedSourceProbe {
...
export interface CalendarLiveV45FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV45FixedFixedPoint;
}

export interface CalendarLiveV45FixedHardGateInput {
  thresholds: CalendarLiveV45FixedThresholds;
  sourceProbes: CalendarLiveV45FixedSourceProbe[];
  cells: CalendarLiveV45FixedCellValidation[];
  sceneProofs: CalendarLiveV45FixedSceneProof[];
...
export interface CalendarLiveV45FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V45_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV45FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV45FixedHardGates(
  input: CalendarLiveV45FixedHardGateInput,
): CalendarLiveV45FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV45FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V45_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV45FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV45FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV45FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
