...
import {
  validateCalendarLiveV33AntecedentIndex,
  verifyCalendarLiveV33Authorization,
  type CalendarLiveV33AntecedentIndex,
  type CalendarLiveV33AuthorizationProof,
} from "./calendar-live-v33-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V33_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v33/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V33_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v33-security-attestation.json";

export interface CalendarLiveV33SecurityAttestation {
  artifactVersion: "calendar-live-v33-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV33PreflightReport {
  artifactVersion: "calendar-live-v33-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV33SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV33SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV33AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v33-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V33_TARGET) ||
...
export function validateCalendarLiveV33ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV33Authorization");
  const preflight = source.indexOf("runCalendarLiveV33Preflight");
  const initialize = source.indexOf("CalendarLiveV33Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV33Preflight(
  root: string,
  proof: CalendarLiveV33AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V33_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV33PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V33_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V33_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV33AntecedentIndex;
  failures.push(...validateCalendarLiveV33AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V33_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V33_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV33SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v33-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV33ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v33 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV33SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v33-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V33_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV33Preflight(
    process.cwd(),
    verifyCalendarLiveV33Authorization(),
    attestationPath,
