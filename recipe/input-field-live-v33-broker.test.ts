...
import {
  ...
  createInputLiveV33FakeDynamicResponse,
  ...
  listInputLiveV33PersistedRequests,
  persistInputLiveV33RawResponse,
  persistInputLiveV33TechnicalGates,
  validateInputLiveV33Receipt,
  validateInputLiveV33Request,
  type InputLiveV33RemotePhase,
...
import {
  ...
  type InputLiveV33ExtractPayload,
  type InputLiveV33ProbePayload,
...
import {
  ...
  simulatedInputLiveV33Authorization,
} from "./run-input-field-live-v33.js";
...
const captures: InputLiveV33CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV33Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v33-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV33Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV33CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV33FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV33FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V33_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V33_TARGET);
  persistInputLiveV33RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV33Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV33Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV33WriterOwnership,
...
const rootProofs = (): InputLiveV33RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v33-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV33Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV33Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV33RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV33RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV33ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV33ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV33CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV33CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV33CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV33Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV33CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV33PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV33ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV33Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV33Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV33Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV33Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V33_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV33RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V33_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV33Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV33Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV33ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV33Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV33WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV33RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV33Request;
    validateInputLiveV33Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV33Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV33CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v33-state-"),
  );
  ...
  try {
    const orchestrator = InputLiveV33Orchestrator.initialize({
      ...
      privateKey,
      authorization: simulatedInputLiveV33Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV33Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V33_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV33RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V33_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV33Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV33Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV33RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V33_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV33Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV33Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV33Request;
