...
import {
  validateCalendarLiveV37AntecedentIndex,
  verifyCalendarLiveV37Authorization,
  type CalendarLiveV37AntecedentIndex,
  type CalendarLiveV37AuthorizationProof,
} from "./calendar-live-v37-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V37_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v37/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V37_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v37-security-attestation.json";

export interface CalendarLiveV37SecurityAttestation {
...
export interface CalendarLiveV37PreflightReport {
  artifactVersion: "calendar-live-v37-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV37SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV37SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV37AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v37-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V37_TARGET) ||
...
export function validateCalendarLiveV37ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV37Authorization");
  const preflight = source.indexOf("runCalendarLiveV37Preflight");
  const initialize = source.indexOf("CalendarLiveV37Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV37Preflight(
  root: string,
  proof: CalendarLiveV37AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V37_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV37PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V37_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V37_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV37AntecedentIndex;
  failures.push(...validateCalendarLiveV37AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V37_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V37_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v37-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV37ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v37 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV37SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v37-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V37_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV37Preflight(
    process.cwd(),
    verifyCalendarLiveV37Authorization(),
    attestationPath,
