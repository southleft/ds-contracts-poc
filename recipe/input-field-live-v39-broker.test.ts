...
import {
  ...
  createInputLiveV39FakeDynamicResponse,
  ...
  listInputLiveV39PersistedRequests,
  persistInputLiveV39RawResponse,
  persistInputLiveV39TechnicalGates,
  validateInputLiveV39Receipt,
  validateInputLiveV39Request,
  type InputLiveV39RemotePhase,
...
import {
  ...
  type InputLiveV39ExtractPayload,
  type InputLiveV39ProbePayload,
  type InputLiveV39RestorePayload,
  type InputLiveV39RootProof,
  type InputLiveV39WriterOwnership,
} from "./input-field-live-v39-contract.js";
import {
  InputLiveV39Orchestrator,
  simulatedInputLiveV39Authorization,
} from "./run-input-field-live-v39.js";
...
const captures: InputLiveV39CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV39Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v39-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV39Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV39CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV39FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV39FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V39_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V39_TARGET);
  persistInputLiveV39RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV39Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV39Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV39WriterOwnership,
): InputLiveV39RestorePayload => ({
...
const rootProofs = (): InputLiveV39RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV39CaptureCell,
): InputLiveV39CapturePayload => {
  const bytes = Buffer.from(`input-live-v39-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV39Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV39Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV39RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV39RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV39ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV39ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV39CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV39CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV39CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV39Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV39CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV39PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV39ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV39Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV39Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV39Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV39Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V39_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV39RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V39_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV39Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV39Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV39ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV39Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV39WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV39RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV39Request;
    validateInputLiveV39Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV39Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV39CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v39-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV39Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV39Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV39Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V39_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV39RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V39_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV39Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV39Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV39RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V39_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV39Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV39Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV39Request;
