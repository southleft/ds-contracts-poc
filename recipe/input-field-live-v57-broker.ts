...
import {
  ...
  INPUT_LIVE_V57_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V57_RESPONSE_CONTRACTS,
  type InputLiveV57RootProof,
} from "./input-field-live-v57-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V57_BROKER_VERSION = "input-live-v57-broker-v2";
export const INPUT_LIVE_V57_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V57_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V57_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V57_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V57_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V57_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV57RemotePhase =
  (typeof INPUT_LIVE_V57_REMOTE_PHASES)[number];
export type InputLiveV57AuthorizationMode = "live" | "simulated";

export interface InputLiveV57CommitPins {
...
export interface InputLiveV57TransactionAuthorization extends InputLiveV57CommitPins {
  mode: InputLiveV57AuthorizationMode;
...
export interface InputLiveV57Manifest {
  artifactVersion: "input-live-v57-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V57_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V57_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V57_TARGET;
...
export interface InputLiveV57ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV57RequestBody {
  artifactVersion: "input-live-v57-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V57_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV57RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV57CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V57_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV57ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV57Request extends InputLiveV57RequestBody {
...
export interface InputLiveV57Receipt {
  artifactVersion: "input-live-v57-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V57_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV57RemotePhase;
...
export interface InputLiveV57AcceptedResponse<T = unknown> {
  request: InputLiveV57Request;
  receipt: InputLiveV57Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV57IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV57ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V57_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V57_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V57_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V57_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V57_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V57_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V57_TARGET.editorType;
};
...
export const inputLiveV57RequestSequence = (
  phase: InputLiveV57RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV57RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV57RequestPath = (
  directory: string,
  phase: InputLiveV57RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV57ResponsePath = (
  directory: string,
  phase: InputLiveV57RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV57ReceiptPath = (
  directory: string,
  phase: InputLiveV57RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV57Manifest => {
  const manifest = json<InputLiveV57Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v57-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V57_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V57_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV57RemotePhase,
  captureIndex?: number,
): InputLiveV57Request | undefined => {
  const file = inputLiveV57RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV57Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV57RemotePhase,
  captureIndex?: number,
): InputLiveV57Receipt | undefined => {
  const file = inputLiveV57ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV57Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV57Request | undefined;
  receipt: InputLiveV57Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV57RemotePhase,
): InputLiveV57ResponseContract => ({
  ...INPUT_LIVE_V57_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V57_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V57_DYNAMIC_TOOL,
      target: INPUT_LIVE_V57_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v57-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V57_BROKER_VERSION,
  target: INPUT_LIVE_V57_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V57_PROGRAM_START*/${program}/*INPUT_LIVE_V57_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V57_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V57_PROGRAM_END*/";
...
export function createInputLiveV57Transaction(
  directory: string,
  ...
  const manifest: InputLiveV57Manifest = {
    artifactVersion: "input-live-v57-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V57_BROKER_VERSION,
...
export function validateInputLiveV57Request(
  directory: string,
  request: InputLiveV57Request,
): void {
  ...
  const expectedSequence = inputLiveV57RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v57-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V57_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V57_TARGET.fileKey ||
...
export function issueInputLiveV57Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV57RemotePhase,
  program: string,
  options: InputLiveV57IssueOptions = {},
): InputLiveV57Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV57RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV57Request>(targetPath);
    validateInputLiveV57Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV57ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV57TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV57RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V57_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V57_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V57_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV57RequestBody = {
    artifactVersion: "input-live-v57-broker-request-v2",
    brokerVersion: INPUT_LIVE_V57_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV57Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV57Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV57RawResponse(
  directory: string,
  phase: InputLiveV57RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV57RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV57Request>(requestFile);
  if (
  ...
  const target = inputLiveV57ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV57Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v57-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V57_BROKER_VERSION,
...
export function acceptInputLiveV57Response<T = unknown>(
  directory: string,
  phase: InputLiveV57RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV57AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV57ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV57Request>(
    inputLiveV57RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV57Request(directory, request);
  if (phase === "capture") validateInputLiveV57TechnicalGates(directory);
  const rawFile = inputLiveV57ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v57-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V57_BROKER_VERSION ||
...
export function persistInputLiveV57TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV57RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV57RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V57_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v57-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V57_BROKER_VERSION,
...
export function validateInputLiveV57TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v57-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V57_TECHNICAL_GATES.some(
...
export function validateInputLiveV57Receipt(
  directory: string,
  phase: InputLiveV57RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV57Request(directory, request);
  const receipt = json<InputLiveV57Receipt>(
    inputLiveV57ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV57ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v57-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V57_BROKER_VERSION ||
...
export function readInputLiveV57AcceptedRawPayload(
  directory: string,
  phase: InputLiveV57RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV57Receipt(directory, phase, captureIndex);
...
export function createInputLiveV57FakeDynamicResponse(
  request: InputLiveV57Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V57_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V57_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV57PersistedRequests(
  directory: string,
): InputLiveV57Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV57Request>(file)] : [];
    });
}

export const inputLiveV57Sha256 = sha256;
