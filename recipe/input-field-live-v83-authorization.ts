...
import {
  ...
  INPUT_LIVE_V83_INDEX_PATH,
} from "./build-input-field-live-proof-v83.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV83HistoryExpectation = "pending" | "authorized";

export interface InputLiveV83AuthorizationArtifact {
  artifactVersion: "input-live-v83-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V83_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV83AntecedentIndex {
...
export interface InputLiveV83HistoryState {
  index?: InputLiveV83AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV83AuthorizationArtifact;
...
export interface InputLiveV83AuthorizationProof extends InputLiveV83TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V83_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V83_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V83_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V83_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v83-authorization") ||
  artifactPath.includes("input-field-live-v83-preflight") ||
  artifactPath.includes("input-field-live-v83-authorized") ||
  artifactPath.includes("input-field-live-pivot-v83-status") ||
...
export function validateInputLiveV83AntecedentIndex(
  index: InputLiveV83AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v83-antecedent-index-v1" ||
...
export function validateInputLiveV83UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV83History\(\s*["']/.test(source) ||
    /readInputLiveV83HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV83AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV83AuthorizationArtifact {
  ...
  ) as InputLiveV83AntecedentIndex;
  const indexFailures = validateInputLiveV83AntecedentIndex(index);
...
export function validateInputLiveV83AuthorizationArtifact(
  artifact: InputLiveV83AuthorizationArtifact | undefined,
  index: InputLiveV83AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v83-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v83" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V83_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V83_TARGET) ||
    !equalJson(
...
export function validateInputLiveV83History(
  state: InputLiveV83HistoryState,
  expected: InputLiveV83HistoryExpectation,
): string[] {
  const failures = validateInputLiveV83AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV83AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV83HistoryState(
  root = process.cwd(),
): InputLiveV83HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V83_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV83AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V83_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V83_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V83_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV83History(
  expected: InputLiveV83HistoryExpectation,
  root = process.cwd(),
): InputLiveV83HistoryState {
  const state = readInputLiveV83HistoryState(root);
  const failures = validateInputLiveV83History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV83Authorization(
  root = process.cwd(),
): InputLiveV83AuthorizationProof {
  const state = verifyInputLiveV83History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V83_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v83/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v83.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V83_TARGET,
    expectedDynamicTool: INPUT_LIVE_V83_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V83_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV83HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV83History(expected);
