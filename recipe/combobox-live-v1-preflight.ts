...
import {
  ...
  verifyComboboxLiveV1Authorization,
  type ComboboxLiveV1AntecedentIndex,
  type ComboboxLiveV1AuthorizationProof,
} from "./combobox-live-v1-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V1_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v1/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V1_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v1-security-attestation.json";

export interface ComboboxLiveV1SecurityAttestation {
...
export interface ComboboxLiveV1PreflightReport {
  artifactVersion: "combobox-live-v1-preflight-v1";
...
export const comboboxLiveV1AttestationSha256 = (
  value: Omit<ComboboxLiveV1SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV1SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV1AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v1-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V1_TARGET) ||
...
export function validateComboboxLiveV1ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV1Authorization");
  const preflight = source.indexOf("runComboboxLiveV1Preflight");
  const initialize = source.indexOf("ComboboxLiveV1Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV1Preflight(
  root: string,
  proof: ComboboxLiveV1AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V1_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV1PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V1_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V1_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V1_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV1AntecedentIndex;
  failures.push(...validateComboboxLiveV1AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V1_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V1_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v1-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV1ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as ComboboxLiveV1SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v1-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V1_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV1Preflight(
    process.cwd(),
    verifyComboboxLiveV1Authorization(),
    attestationPath,
