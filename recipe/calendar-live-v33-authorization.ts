...
export type CalendarLiveV33HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV33AuthorizationArtifact {
  artifactVersion: "calendar-live-v33-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V33_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV33AntecedentIndex {
...
export interface CalendarLiveV33HistoryState {
  index?: CalendarLiveV33AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV33AuthorizationArtifact;
...
export interface CalendarLiveV33AuthorizationProof
  extends CalendarLiveV33TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V33_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V33_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V33_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V33_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v33-authorization") ||
  artifactPath.includes("calendar-live-v33-preflight") ||
  artifactPath.includes("calendar-live-v33-authorized") ||
  artifactPath.includes("create-calendar-live-v33-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v33-status") ||
...
export function validateCalendarLiveV33AntecedentIndex(
  index: CalendarLiveV33AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v33-antecedent-index-v1" ||
...
export function validateCalendarLiveV33UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV33History\(\s*["']/.test(source) ||
    /readCalendarLiveV33HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV33AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV33AuthorizationArtifact {
  ...
  ) as CalendarLiveV33AntecedentIndex;
  const indexFailures = validateCalendarLiveV33AntecedentIndex(index);
...
export function validateCalendarLiveV33AuthorizationArtifact(
  artifact: CalendarLiveV33AuthorizationArtifact | undefined,
  index: CalendarLiveV33AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v33-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v33" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V33_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V33_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V33_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V33_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V33_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V33_SOURCE_ROOTS ||
...
export function validateCalendarLiveV33History(
  state: CalendarLiveV33HistoryState,
  expected: CalendarLiveV33HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV33AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV33AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV33HistoryState(
  root = process.cwd(),
): CalendarLiveV33HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V33_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV33AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V33_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V33_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V33_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV33History(
  expected: CalendarLiveV33HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV33HistoryState {
  const state = readCalendarLiveV33HistoryState(root);
  const failures = validateCalendarLiveV33History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v33 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV33Authorization(
  root = process.cwd(),
): CalendarLiveV33AuthorizationProof {
  const state = verifyCalendarLiveV33History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V33_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v33/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v33.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V33_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V33_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V33_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV33HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV33History(expected);
  process.stdout.write(
    `Calendar live v33 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
