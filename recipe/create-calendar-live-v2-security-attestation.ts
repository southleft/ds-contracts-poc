...
import { verifyCalendarLiveV2Authorization } from "./calendar-live-v2-authorization.js";
import { CALENDAR_LIVE_V2_TARGET } from "./calendar-live-v2-broker.js";
import {
  CALENDAR_LIVE_V2_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV2Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v2-session";
...
const attestation: CalendarLiveV2SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV2SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v2 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V2_SECURITY_ATTESTATION_DEFAULT_PATH,
);
