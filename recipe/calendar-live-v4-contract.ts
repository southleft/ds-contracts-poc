...
import { buildCalendarLiveV4CleanupRuntime } from "./calendar-live-v4-cleanup.js";
import { type CalendarLiveV4FixedFixedPoint } from "./calendar-live-v4-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV4RestoreProgram,
  validateCalendarLiveV4RestorePayload,
  type CalendarLiveV4RestorePayload,
} from "./calendar-live-v4-restore.js";
import {
  buildCalendarLiveV4RawPropertyRuntime,
  normalizeCalendarLiveV4Scene,
  type CalendarLiveV4RawNode,
} from "./calendar-live-v4-verifier.js";
...
export const CALENDAR_LIVE_V4_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V4_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V4_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V4_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V4_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V4_SET_COUNT = 3;
export const CALENDAR_LIVE_V4_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V4_HOST_PHASES = 3;
export const CALENDAR_LIVE_V4_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V4_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V4_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V4_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV4SourceId = (typeof CALENDAR_LIVE_V4_SOURCE_IDS)[number];
export type CalendarLiveV4Kind = "calendar" | "week" | "day";

export interface CalendarLiveV4SourceIdentity {
  source: CalendarLiveV4SourceId;
...
export interface CalendarLiveV4WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV4ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV4SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV4RawNode;
    weekScene: CalendarLiveV4RawNode;
    dayScene: CalendarLiveV4RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV4RootProof {
  source: CalendarLiveV4SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV4FixedFixedPoint;
}

export interface CalendarLiveV4ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV4SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV4CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV4SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV4CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV4SourceId;
  frameWidth: number;
...
export interface CalendarLiveV4CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v4 ${label} must be an object`);
...
export function validateCalendarLiveV4WriterPayload(
  value: unknown,
): CalendarLiveV4WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v4") ||
    ...
    result.namespace !== CALENDAR_LIVE_V4_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v4 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v4 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v4 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV4SourceIdentity,
  kind: CalendarLiveV4Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV4ExtractProgram(
  writer: CalendarLiveV4WriterOwnership,
  sources: readonly CalendarLiveV4SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v4 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V4_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V4_NAMESPACE)}
${buildCalendarLiveV4RawPropertyRuntime()}
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("CALENDAR-V4-EXTRACT-PAGE");
...
if(get(page,"pageOwner")!=="recipe/calendar/"+${JSON.stringify(writer.runIdentity)})throw new Error("CALENDAR-V4-EXTRACT-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>SET_IDS.has(node.id));
if(sets.length!==3)throw new Error("CALENDAR-V4-EXTRACT-ROOTS:"+sets.length);
...
for(const set of sets){
  ...
  if(!IDENTITIES[adapterIdentity]||(kind!=="calendar"&&kind!=="week"&&kind!=="day"))throw new Error("CALENDAR-V4-EXTRACT-ADAPTER:"+adapterIdentity+":"+kind);
...
if(roots.length!==1||roots.some(root=>!root.calendarScene||!root.weekScene||!root.dayScene))throw new Error("CALENDAR-V4-EXTRACT-TRIPLE");
...
export function validateCalendarLiveV4ExtractPayload(
  value: unknown,
  writer: CalendarLiveV4WriterOwnership,
): CalendarLiveV4ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v4 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V4_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v4 extract schema/one-root mismatch");
  return payload as CalendarLiveV4ExtractPayload;
}

export function proveCalendarLiveV4Roots<Instance>(
  extract: CalendarLiveV4ExtractPayload,
  sources: readonly (CalendarLiveV4SourceIdentity & {
...
): CalendarLiveV4RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v4 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v4 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV4Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV4Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV4Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV4FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v4 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v4 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v4 compile lost owned sets");
...
export function assertCalendarLiveV4RootProofs(
  proofs: readonly CalendarLiveV4RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v4 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v4 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV4ProbeProgram(
  writer: CalendarLiveV4WriterOwnership,
  sources: readonly CalendarLiveV4SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V4_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE"||page.id==="115:295378"||page.id==="163:35981")throw new Error("CALENDAR-V4-PROBE-PAGE");
...
if(sets.length!==3)throw new Error("CALENDAR-V4-PROBE-ROOTS:"+sets.length);
...
for(const row of byAdapter.values()){
 ...
 if(!source||!calendar||!week||!day||calendar.children.length!==2||week.children.length!==2||day.children.length!==4)throw new Error("CALENDAR-V4-PROBE-VARIANTS:"+adapterIdentity);
...
export function validateCalendarLiveV4ProbePayload(
  value: unknown,
  writer: CalendarLiveV4WriterOwnership,
): CalendarLiveV4ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV4ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V4_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V4_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v4 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV4CaptureManifest(
  cells: readonly CalendarLiveV4CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V4_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V4_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V4_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V4_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v4 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV4CaptureCell[],
): string => {
  validateCalendarLiveV4CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV4CaptureProgram(
  writer: CalendarLiveV4WriterOwnership,
  cell: CalendarLiveV4CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V4_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v4 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v4 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
if(!page||page.type!=="PAGE"||page.id==="115:295378"||page.id==="163:35981"||!set||set.type!=="COMPONENT_SET")throw new Error("CALENDAR-V4-CAPTURE-TARGET");
const axes=node=>Object.fromEntries(node.name.split(",").map(part=>part.trim()).filter(part=>part.includes("=")).map(part=>{const at=part.indexOf("=");return[part.slice(0,at),part.slice(at+1)];}));
...
if(!component)throw new Error("CALENDAR-V4-CAPTURE-CELL:"+cell.cellKey);
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V4_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V4-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV4CapturePayload(
  value: unknown,
  cell: CalendarLiveV4CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV4CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV4CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V4_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V4_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v4 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV4CaptureResponses(
  manifest: readonly CalendarLiveV4CaptureCell[],
  responses: readonly CalendarLiveV4CapturePayload[],
): void {
  validateCalendarLiveV4CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v4 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV4ObjectiveReport {
  artifactVersion: "calendar-live-v4-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV4Objective(
  manifest: readonly CalendarLiveV4CaptureCell[],
  responses: readonly CalendarLiveV4CapturePayload[],
): CalendarLiveV4ObjectiveReport {
  assertCalendarLiveV4CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v4-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV4CleanupProgram(
  writer: CalendarLiveV4WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV4CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V4_NAMESPACE,
  ...
  const guardedResult = `if(JSON.stringify(requestedNodeIds)!==JSON.stringify([expectedPageId])||JSON.stringify([...requestedCollectionIds].sort())!==JSON.stringify([...expectedCollectionIds].sort()))throw new Error("CALENDAR-V4-CLEANUP-EXACT-OWNERSHIP");${resultStatement}`;
  const program = baseProgram.replace(resultStatement, guardedResult);
  if (program === baseProgram)
    throw new Error("Calendar live v4 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV4CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV4WriterOwnership,
): CalendarLiveV4CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV4CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v4 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V4_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV4WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
