...
import { buildCalendarLiveV11CleanupRuntime } from "./calendar-live-v11-cleanup.js";
import { type CalendarLiveV11FixedFixedPoint } from "./calendar-live-v11-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV11RestoreProgram,
  validateCalendarLiveV11RestorePayload,
  type CalendarLiveV11RestorePayload,
} from "./calendar-live-v11-restore.js";
import {
  buildCalendarLiveV11RawPropertyRuntime,
  normalizeCalendarLiveV11Scene,
  type CalendarLiveV11RawNode,
} from "./calendar-live-v11-verifier.js";
...
export const CALENDAR_LIVE_V11_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V11_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V11_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V11_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V11_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V11_SET_COUNT = 3;
export const CALENDAR_LIVE_V11_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V11_HOST_PHASES = 3;
export const CALENDAR_LIVE_V11_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V11_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V11_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V11_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV11SourceId = (typeof CALENDAR_LIVE_V11_SOURCE_IDS)[number];
export type CalendarLiveV11Kind = "calendar" | "week" | "day";

export interface CalendarLiveV11SourceIdentity {
  source: CalendarLiveV11SourceId;
...
export interface CalendarLiveV11WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV11ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV11SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV11RawNode;
    weekScene: CalendarLiveV11RawNode;
    dayScene: CalendarLiveV11RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV11RootProof {
  source: CalendarLiveV11SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV11FixedFixedPoint;
}

export interface CalendarLiveV11ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV11SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV11CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV11SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV11CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV11SourceId;
  frameWidth: number;
...
export interface CalendarLiveV11CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v11 ${label} must be an object`);
...
export function validateCalendarLiveV11WriterPayload(
  value: unknown,
): CalendarLiveV11WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v11") ||
    ...
    result.namespace !== CALENDAR_LIVE_V11_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v11 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v11 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v11 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV11SourceIdentity,
  kind: CalendarLiveV11Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV11ExtractProgram(
  writer: CalendarLiveV11WriterOwnership,
  sources: readonly CalendarLiveV11SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v11 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V11_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V11_NAMESPACE)}
${buildCalendarLiveV11RawPropertyRuntime()}
...
export function validateCalendarLiveV11ExtractPayload(
  value: unknown,
  writer: CalendarLiveV11WriterOwnership,
): CalendarLiveV11ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v11 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V11_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v11 extract schema/one-root mismatch");
  return payload as CalendarLiveV11ExtractPayload;
}

export function proveCalendarLiveV11Roots<Instance>(
  extract: CalendarLiveV11ExtractPayload,
  sources: readonly (CalendarLiveV11SourceIdentity & {
...
): CalendarLiveV11RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v11 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v11 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV11Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV11Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV11Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV11FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v11 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v11 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v11 compile lost owned sets");
...
export function assertCalendarLiveV11RootProofs(
  proofs: readonly CalendarLiveV11RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v11 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v11 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV11ProbeProgram(
  writer: CalendarLiveV11WriterOwnership,
  sources: readonly CalendarLiveV11SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V11_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV11ProbePayload(
  value: unknown,
  writer: CalendarLiveV11WriterOwnership,
): CalendarLiveV11ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV11ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V11_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V11_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v11 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV11CaptureManifest(
  cells: readonly CalendarLiveV11CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V11_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V11_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V11_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V11_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v11 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV11CaptureCell[],
): string => {
  validateCalendarLiveV11CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV11CaptureProgram(
  writer: CalendarLiveV11WriterOwnership,
  cell: CalendarLiveV11CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V11_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v11 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v11 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V11_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV11CapturePayload(
  value: unknown,
  cell: CalendarLiveV11CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV11CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV11CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V11_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V11_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v11 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV11CaptureResponses(
  manifest: readonly CalendarLiveV11CaptureCell[],
  responses: readonly CalendarLiveV11CapturePayload[],
): void {
  validateCalendarLiveV11CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v11 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV11ObjectiveReport {
  artifactVersion: "calendar-live-v11-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV11Objective(
  manifest: readonly CalendarLiveV11CaptureCell[],
  responses: readonly CalendarLiveV11CapturePayload[],
): CalendarLiveV11ObjectiveReport {
  assertCalendarLiveV11CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v11-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV11CleanupProgram(
  writer: CalendarLiveV11WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV11CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V11_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v11 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV11CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV11WriterOwnership,
): CalendarLiveV11CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV11CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v11 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V11_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV11WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
