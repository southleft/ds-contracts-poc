...
import {
  assertInputLiveV42RootProofs,
  INPUT_LIVE_V42_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V42_RESPONSE_CONTRACTS,
  type InputLiveV42RootProof,
} from "./input-field-live-v42-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V42_BROKER_VERSION = "input-live-v42-broker-v2";
export const INPUT_LIVE_V42_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V42_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V42_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V42_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V42_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V42_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV42RemotePhase =
  (typeof INPUT_LIVE_V42_REMOTE_PHASES)[number];
export type InputLiveV42AuthorizationMode = "live" | "simulated";

export interface InputLiveV42CommitPins {
...
export interface InputLiveV42TransactionAuthorization extends InputLiveV42CommitPins {
  mode: InputLiveV42AuthorizationMode;
...
export interface InputLiveV42Manifest {
  artifactVersion: "input-live-v42-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V42_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V42_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V42_TARGET;
  authorization: InputLiveV42TransactionAuthorization;
...
export interface InputLiveV42ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV42RequestBody {
  artifactVersion: "input-live-v42-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V42_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV42RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV42CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V42_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV42ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV42Request extends InputLiveV42RequestBody {
...
export interface InputLiveV42Receipt {
  artifactVersion: "input-live-v42-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V42_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV42RemotePhase;
...
export interface InputLiveV42AcceptedResponse<T = unknown> {
  request: InputLiveV42Request;
  receipt: InputLiveV42Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV42IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV42ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V42_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V42_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V42_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V42_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V42_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V42_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V42_TARGET.editorType;
};
...
export const inputLiveV42RequestSequence = (
  phase: InputLiveV42RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV42RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV42RequestPath = (
  directory: string,
  phase: InputLiveV42RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV42ResponsePath = (
  directory: string,
  phase: InputLiveV42RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV42ReceiptPath = (
  directory: string,
  phase: InputLiveV42RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV42Manifest => {
  const manifest = json<InputLiveV42Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v42-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V42_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V42_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV42RemotePhase,
  captureIndex?: number,
): InputLiveV42Request | undefined => {
  const file = inputLiveV42RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV42Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV42RemotePhase,
  captureIndex?: number,
): InputLiveV42Receipt | undefined => {
  const file = inputLiveV42ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV42Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV42Request | undefined;
  receipt: InputLiveV42Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV42RemotePhase,
): InputLiveV42ResponseContract => ({
  ...INPUT_LIVE_V42_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V42_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V42_DYNAMIC_TOOL,
      target: INPUT_LIVE_V42_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v42-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V42_BROKER_VERSION,
  target: INPUT_LIVE_V42_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V42_PROGRAM_START*/${program}/*INPUT_LIVE_V42_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V42_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V42_PROGRAM_END*/";
...
export function createInputLiveV42Transaction(
  directory: string,
  ...
  const manifest: InputLiveV42Manifest = {
    artifactVersion: "input-live-v42-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V42_BROKER_VERSION,
...
export function validateInputLiveV42Request(
  directory: string,
  request: InputLiveV42Request,
): void {
  ...
  const expectedSequence = inputLiveV42RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v42-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V42_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V42_TARGET.fileKey ||
...
export function issueInputLiveV42Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV42RemotePhase,
  program: string,
  options: InputLiveV42IssueOptions = {},
): InputLiveV42Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV42RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV42Request>(targetPath);
    validateInputLiveV42Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV42ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV42TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV42RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V42_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V42_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V42_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV42RequestBody = {
    artifactVersion: "input-live-v42-broker-request-v2",
    brokerVersion: INPUT_LIVE_V42_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV42Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV42Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV42RawResponse(
  directory: string,
  phase: InputLiveV42RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV42RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV42Request>(requestFile);
  if (
  ...
  const target = inputLiveV42ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV42Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v42-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V42_BROKER_VERSION,
...
export function acceptInputLiveV42Response<T = unknown>(
  directory: string,
  phase: InputLiveV42RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV42AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV42ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV42Request>(
    inputLiveV42RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV42Request(directory, request);
  if (phase === "capture") validateInputLiveV42TechnicalGates(directory);
  const rawFile = inputLiveV42ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v42-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V42_BROKER_VERSION ||
...
export function persistInputLiveV42TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV42RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV42RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V42_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v42-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V42_BROKER_VERSION,
...
export function validateInputLiveV42TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v42-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V42_TECHNICAL_GATES.some(
...
export function validateInputLiveV42Receipt(
  directory: string,
  phase: InputLiveV42RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV42Request(directory, request);
  const receipt = json<InputLiveV42Receipt>(
    inputLiveV42ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV42ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v42-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V42_BROKER_VERSION ||
...
export function readInputLiveV42AcceptedRawPayload(
  directory: string,
  phase: InputLiveV42RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV42Receipt(directory, phase, captureIndex);
...
export function createInputLiveV42FakeDynamicResponse(
  request: InputLiveV42Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V42_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V42_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV42PersistedRequests(
  directory: string,
): InputLiveV42Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV42Request>(file)] : [];
    });
}

export const inputLiveV42Sha256 = sha256;
