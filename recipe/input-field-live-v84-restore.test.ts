...
import {
  INPUT_LIVE_V84_MEASURE_VISIBLE_FILL_MARKER,
  INPUT_LIVE_V84_RESTORE_CONTENT_ROLES,
  INPUT_LIVE_V84_RESTORE_PARENT_ROLES,
...
test("v84 probe reveals hidden content FILL before measuring, then restores visibility", () => {
  const program = readFileSync(
    "recipe/input-field-live-v84-contract.ts",
    "utf8",
  );
  const probe = program.slice(
    program.indexOf("export function buildInputLiveV84ProbeProgram"),
  );
...
test("v16 restore still measures FILL while visible, then restores visibility", () => {
  const program = buildInputLiveV84RestoreProgram(writer);
  assert.match(program, new RegExp(INPUT_LIVE_V84_MEASURE_VISIBLE_FILL_MARKER));
  ...
  for (const role of INPUT_LIVE_V84_RESTORE_CONTENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
  for (const role of INPUT_LIVE_V84_RESTORE_PARENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
});

test("v16 restore payload requires 256 filled content texts", () => {
  const accepted = validateInputLiveV84RestorePayload(
    {
      pageId: "1:1",
      setIds: ["2:2", "2:1"],
      restoredCount: 256,
      ...
      marker: INPUT_LIVE_V84_MEASURE_VISIBLE_FILL_MARKER,
    },
    writer,
  );
...
test("v16 writer bytes stay frozen and v17 writer assigns set HUG", () => {
  ...
  const v76Program = readFileSync(
    "recipe/evidence/input-field-live-pivot-v84/programs/writer.txt",
  );
  const v76Payload = readFileSync(
    "recipe/evidence/input-field-live-pivot-v84/programs/writer-payload.js",
  );
...
test("v84 probe excludes overlay-label AABB from visibleAreaLoss; overlap excludes opacity-0 occupancy", () => {
  const program = readFileSync(
    "recipe/input-field-live-v84-contract.ts",
    "utf8",
  );
...
test("v19 writer preserves hidden FILL occupancy; hashed v16/v17/v18 stay frozen", () => {
  ...
  assert.match(v84Payload, /INPUT-WRITER-HIDDEN-FILL-OCCUPANCY/);
  assert.equal(
    v84Payload.includes(
  ...
  assert.equal(sha256(v84Payload), V19_WRITER_PAYLOAD_SHA256);
  assert.equal(
    sha256(
      readFileSync(
        "recipe/evidence/input-field-live-pivot-v84/programs/writer.txt",
...
test("v84 expected-plan carries live occupancy opacity 0; Polar stays V82", () => {
  const mui = JSON.parse(
    gunzipSync(
      readFileSync(
        "recipe/evidence/input-field-live-pivot-v84/expected-scene-plan-mui.json.gz",
      ),
    ).toString("utf8"),
  ) as {
    facts: Array<{
  ...
  assert.equal(
    sha256(
      readFileSync(
        "recipe/evidence/input-field-live-pivot-v84/expected-scene-plan-polaris.json.gz",
      ),
    ),
    sha256(
      readFileSync(
