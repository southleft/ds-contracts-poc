...
import {
  acceptTableLiveV3Response,
  createTableLiveV3Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV3Request,
  persistTableLiveV3RawResponse,
  persistTableLiveV3TechnicalGates,
  readTableLiveV3AcceptedRawPayload,
  type TableLiveV3Request,
  type TableLiveV3TransactionAuthorization,
} from "./table-live-v3-broker.js";
import {
  TABLE_LIVE_V3_CAPTURE_COUNT,
  assertTableLiveV3CaptureResponses,
  ...
  evaluateTableLiveV3Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV3Roots,
  validateTableLiveV3CaptureManifest,
  validateTableLiveV3CapturePayload,
  validateTableLiveV3CleanupPayload,
  validateTableLiveV3ExtractPayload,
  validateTableLiveV3ProbePayload,
  validateTableLiveV3WriterPayload,
  type TableLiveV3CaptureCell,
  type TableLiveV3CapturePayload,
...
import {
  ...
  TABLE_LIVE_V3_PLAN_PATH,
  type TableLiveV3ProofPlan,
...
export const TABLE_LIVE_V3_RUNNER_VERSION = "table-live-v3-runner-v1";

interface TableLiveV3JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV3OrchestratorState {
  artifactVersion: "table-live-v3-orchestrator-state-v1";
  ...
  journal: TableLiveV3JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV3ProofPlan;
  planSha256: string;
  captures: TableLiveV3CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV3SourceIdentity & {
...
export interface TableLiveV3NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV3OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v3 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V3_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V3_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV3ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV3CaptureCell[];
  validateTableLiveV3CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v3 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v3 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV3WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV3RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV3Request =>
  readJson<TableLiveV3Request>(file);

export class TableLiveV3Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV3Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV3OrchestratorState = {
      artifactVersion: "table-live-v3-orchestrator-state-v1",
    ...
    return new TableLiveV3Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV3OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV3Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV3NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV3NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v3 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV3NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v3 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV3RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV3Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV3CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV3AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV3WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV3Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV3ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV3Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV3Response(this.#directory, "writer", {
        validate: validateTableLiveV3WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV3Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV3RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV3Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV3ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV3Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV3RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v3-host-root-proofs-v1",
      ...
      const next = issueTableLiveV3Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV3ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV3Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV3ProbePayload(payload, ownership),
      });
      persistTableLiveV3TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v3 capture cell missing");
      const accepted = acceptTableLiveV3Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV3CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V3_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV3CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV3Objective(
    ...
    throw new Error(`Table live v3 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV3Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV3CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV3Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV3RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV3Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV3CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV3CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV3CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV3Authorization = (
  privateKey: KeyObject,
): TableLiveV3TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV3SimulationKey = (): KeyObject =>
