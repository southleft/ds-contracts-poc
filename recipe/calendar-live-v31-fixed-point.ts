/**
 * Calendar live v31 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V31_FIXED_VERIFIER_VERSION = "calendar-live-v31-fixed-point-v1";
export const CALENDAR_LIVE_V31_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV31FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV31FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV31FixedSourceProbe {
...
export interface CalendarLiveV31FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV31FixedFixedPoint;
}

export interface CalendarLiveV31FixedHardGateInput {
  thresholds: CalendarLiveV31FixedThresholds;
  sourceProbes: CalendarLiveV31FixedSourceProbe[];
  cells: CalendarLiveV31FixedCellValidation[];
  sceneProofs: CalendarLiveV31FixedSceneProof[];
...
export interface CalendarLiveV31FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V31_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV31FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV31FixedHardGates(
  input: CalendarLiveV31FixedHardGateInput,
): CalendarLiveV31FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV31FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V31_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV31FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV31FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV31FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
