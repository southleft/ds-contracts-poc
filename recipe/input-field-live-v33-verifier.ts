...
export const INPUT_LIVE_V33_VERIFIER_VERSION = "input-live-v33-verifier-v1";

export function buildInputLiveV33RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV33RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV33RawNode[];
}

export interface InputLiveV33NormalizedScene {
...
export interface InputLiveV33SceneProof {
...
export function normalizeInputLiveV33Scene(
  root: InputLiveV33RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV33NormalizedScene {
  const canonicalBindings: InputLiveV33NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV33RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV33Scene<Instance>(
  normalized: InputLiveV33NormalizedScene,
...
export function compileInputLiveV33ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV33PreCaptureGates(payload: unknown): void {
