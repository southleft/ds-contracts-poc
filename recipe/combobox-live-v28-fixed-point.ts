/**
 * Carried Combobox live v28 host verifier. Uses scene-readback-combobox-v28 so live
...
export const COMBOBOX_LIVE_V28_FIXED_VERIFIER_VERSION = "combobox-live-v28-fixed-point-v1";
export const COMBOBOX_LIVE_V28_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV28FixedThresholds {
...
export interface ComboboxLiveV28FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV28FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV28FixedSourceProbe {
...
export interface ComboboxLiveV28FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV28FixedFixedPoint;
}

export interface ComboboxLiveV28FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV28FixedHardGateInput {
  thresholds: ComboboxLiveV28FixedThresholds;
  materialRegression: ComboboxLiveV28FixedMaterialRegression;
  sourceProbes: ComboboxLiveV28FixedSourceProbe[];
  cells: ComboboxLiveV28FixedCellValidation[];
  sceneProofs: ComboboxLiveV28FixedSceneProof[];
  visualRows: ComboboxLiveV28FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV28FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V28_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV28FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV28FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV28FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV28FixedHardGates(
  input: ComboboxLiveV28FixedHardGateInput,
): ComboboxLiveV28FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV28FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV28FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V28_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV28FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV28FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV28FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
