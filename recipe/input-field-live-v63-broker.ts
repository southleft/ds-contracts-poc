...
import {
  assertInputLiveV63RootProofs,
  INPUT_LIVE_V63_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V63_RESPONSE_CONTRACTS,
  type InputLiveV63RootProof,
} from "./input-field-live-v63-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V63_BROKER_VERSION = "input-live-v63-broker-v2";
export const INPUT_LIVE_V63_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V63_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V63_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V63_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V63_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V63_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV63RemotePhase =
  (typeof INPUT_LIVE_V63_REMOTE_PHASES)[number];
export type InputLiveV63AuthorizationMode = "live" | "simulated";

export interface InputLiveV63CommitPins {
...
export interface InputLiveV63TransactionAuthorization extends InputLiveV63CommitPins {
  mode: InputLiveV63AuthorizationMode;
...
export interface InputLiveV63Manifest {
  artifactVersion: "input-live-v63-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V63_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V63_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V63_TARGET;
  authorization: InputLiveV63TransactionAuthorization;
...
export interface InputLiveV63ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV63RequestBody {
  artifactVersion: "input-live-v63-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V63_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV63RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV63CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V63_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV63ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV63Request extends InputLiveV63RequestBody {
...
export interface InputLiveV63Receipt {
  artifactVersion: "input-live-v63-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V63_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV63RemotePhase;
...
export interface InputLiveV63AcceptedResponse<T = unknown> {
  request: InputLiveV63Request;
  receipt: InputLiveV63Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV63IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV63ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V63_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V63_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V63_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V63_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V63_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V63_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V63_TARGET.editorType;
};
...
export const inputLiveV63RequestSequence = (
  phase: InputLiveV63RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV63RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV63RequestPath = (
  directory: string,
  phase: InputLiveV63RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV63ResponsePath = (
  directory: string,
  phase: InputLiveV63RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV63ReceiptPath = (
  directory: string,
  phase: InputLiveV63RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV63Manifest => {
  const manifest = json<InputLiveV63Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v63-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V63_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V63_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV63RemotePhase,
  captureIndex?: number,
): InputLiveV63Request | undefined => {
  const file = inputLiveV63RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV63Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV63RemotePhase,
  captureIndex?: number,
): InputLiveV63Receipt | undefined => {
  const file = inputLiveV63ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV63Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV63Request | undefined;
  receipt: InputLiveV63Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV63RemotePhase,
): InputLiveV63ResponseContract => ({
  ...INPUT_LIVE_V63_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V63_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V63_DYNAMIC_TOOL,
      target: INPUT_LIVE_V63_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v63-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V63_BROKER_VERSION,
  target: INPUT_LIVE_V63_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V63_PROGRAM_START*/${program}/*INPUT_LIVE_V63_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V63_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V63_PROGRAM_END*/";
...
export function createInputLiveV63Transaction(
  directory: string,
  ...
  const manifest: InputLiveV63Manifest = {
    artifactVersion: "input-live-v63-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V63_BROKER_VERSION,
...
export function validateInputLiveV63Request(
  directory: string,
  request: InputLiveV63Request,
): void {
  ...
  const expectedSequence = inputLiveV63RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v63-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V63_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V63_TARGET.fileKey ||
...
export function issueInputLiveV63Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV63RemotePhase,
  program: string,
  options: InputLiveV63IssueOptions = {},
): InputLiveV63Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV63RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV63Request>(targetPath);
    validateInputLiveV63Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV63ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV63TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV63RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V63_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V63_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V63_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV63RequestBody = {
    artifactVersion: "input-live-v63-broker-request-v2",
    brokerVersion: INPUT_LIVE_V63_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV63Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV63Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV63RawResponse(
  directory: string,
  phase: InputLiveV63RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV63RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV63Request>(requestFile);
  if (
  ...
  const target = inputLiveV63ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV63Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v63-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V63_BROKER_VERSION,
...
export function acceptInputLiveV63Response<T = unknown>(
  directory: string,
  phase: InputLiveV63RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV63AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV63ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV63Request>(
    inputLiveV63RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV63Request(directory, request);
  if (phase === "capture") validateInputLiveV63TechnicalGates(directory);
  const rawFile = inputLiveV63ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v63-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V63_BROKER_VERSION ||
...
export function persistInputLiveV63TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV63RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV63RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V63_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v63-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V63_BROKER_VERSION,
...
export function validateInputLiveV63TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v63-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V63_TECHNICAL_GATES.some(
...
export function validateInputLiveV63Receipt(
  directory: string,
  phase: InputLiveV63RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV63Request(directory, request);
  const receipt = json<InputLiveV63Receipt>(
    inputLiveV63ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV63ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v63-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V63_BROKER_VERSION ||
...
export function readInputLiveV63AcceptedRawPayload(
  directory: string,
  phase: InputLiveV63RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV63Receipt(directory, phase, captureIndex);
...
export function createInputLiveV63FakeDynamicResponse(
  request: InputLiveV63Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V63_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V63_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV63PersistedRequests(
  directory: string,
): InputLiveV63Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV63Request>(file)] : [];
    });
}

export const inputLiveV63Sha256 = sha256;
