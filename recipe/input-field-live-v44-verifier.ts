...
export const INPUT_LIVE_V44_VERIFIER_VERSION = "input-live-v44-verifier-v1";

export function buildInputLiveV44RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV44RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV44RawNode[];
}

export interface InputLiveV44NormalizedScene {
...
export interface InputLiveV44SceneProof {
...
export function normalizeInputLiveV44Scene(
  root: InputLiveV44RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV44NormalizedScene {
  const canonicalBindings: InputLiveV44NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV44RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV44Scene<Instance>(
  normalized: InputLiveV44NormalizedScene,
...
export function compileInputLiveV44ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV44PreCaptureGates(payload: unknown): void {
