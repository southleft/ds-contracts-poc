/**
 * Calendar live v12 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V12_FIXED_VERIFIER_VERSION = "calendar-live-v12-fixed-point-v1";
export const CALENDAR_LIVE_V12_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV12FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV12FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV12FixedSourceProbe {
...
export interface CalendarLiveV12FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV12FixedFixedPoint;
}

export interface CalendarLiveV12FixedHardGateInput {
  thresholds: CalendarLiveV12FixedThresholds;
  sourceProbes: CalendarLiveV12FixedSourceProbe[];
  cells: CalendarLiveV12FixedCellValidation[];
  sceneProofs: CalendarLiveV12FixedSceneProof[];
...
export interface CalendarLiveV12FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V12_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV12FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV12FixedHardGates(
  input: CalendarLiveV12FixedHardGateInput,
): CalendarLiveV12FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV12FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V12_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV12FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV12FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV12FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
