...
import { INPUT_LIVE_V26_INDEX_PATH } from "./build-input-field-live-proof-v26.js";
import {
  INPUT_LIVE_V26_DYNAMIC_TOOL,
  INPUT_LIVE_V26_TARGET,
} from "./input-field-live-v26-broker.js";
import {
  buildInputLiveV26AuthorizationArtifact,
  validateInputLiveV26AntecedentIndex,
  validateInputLiveV26AuthorizationArtifact,
  validateInputLiveV26History,
  validateInputLiveV26UnitTestSource,
  type InputLiveV26AntecedentIndex,
  type InputLiveV26AuthorizationArtifact,
  type InputLiveV26AuthorizationProof,
  type InputLiveV26HistoryState,
} from "./input-field-live-v26-authorization.js";
import {
  inputLiveV26AttestationSha256,
  validateInputLiveV26ControlFlowSource,
  validateInputLiveV26SecurityAttestation,
  type InputLiveV26SecurityAttestation,
} from "./input-field-live-v26-preflight.js";
import { inputLiveV26Sha256 } from "./input-field-live-v26-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V26_INDEX_PATH);
const index = (): InputLiveV26AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV26AntecedentIndex;

const artifact = (): InputLiveV26AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV26AuthorizationArtifact({
...
const pendingState = (): InputLiveV26HistoryState => ({
  index: index(),
  indexSha256: inputLiveV26Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV26HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV26AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV26Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V26_TARGET,
  expectedDynamicTool: INPUT_LIVE_V26_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v26/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV26AuthorizationArtifact,
): InputLiveV26SecurityAttestation => {
  const body: Omit<InputLiveV26SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v26-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V26_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV26AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV26History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV26History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV26History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV26History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV26History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV26UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV26UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV26AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV26Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV26AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV26AuthorizationArtifact(
        planted as InputLiveV26AuthorizationArtifact,
        index(),
        inputLiveV26Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV26SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV26AttestationSha256(
      body as Omit<InputLiveV26SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV26SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV26ControlFlowSource(source), []);
});
