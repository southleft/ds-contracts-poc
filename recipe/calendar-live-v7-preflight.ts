...
import {
  validateCalendarLiveV7AntecedentIndex,
  verifyCalendarLiveV7Authorization,
  type CalendarLiveV7AntecedentIndex,
  type CalendarLiveV7AuthorizationProof,
} from "./calendar-live-v7-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V7_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v7/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V7_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v7-security-attestation.json";

export interface CalendarLiveV7SecurityAttestation {
  artifactVersion: "calendar-live-v7-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV7PreflightReport {
  artifactVersion: "calendar-live-v7-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV7SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV7SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV7AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v7-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V7_TARGET) ||
...
export function validateCalendarLiveV7ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV7Authorization");
  const preflight = source.indexOf("runCalendarLiveV7Preflight");
  const initialize = source.indexOf("CalendarLiveV7Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV7Preflight(
  root: string,
  proof: CalendarLiveV7AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V7_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV7PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V7_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V7_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV7AntecedentIndex;
  failures.push(...validateCalendarLiveV7AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V7_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V7_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV7SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v7-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV7ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v7 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV7SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v7-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V7_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV7Preflight(
    process.cwd(),
    verifyCalendarLiveV7Authorization(),
    attestationPath,
