...
} from "./input-field-live-v3-verifier-v52.js";
import {
  buildInputLiveV52RawPropertyRuntime,
  normalizeInputLiveV52Scene,
  type InputLiveV52RawNode,
} from "./input-field-live-v52-verifier.js";
...
export const INPUT_LIVE_V52_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V52_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V52_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V52_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V52_VARIANT_COUNT = 256;
export const INPUT_LIVE_V52_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V52_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV52SourceId = (typeof INPUT_LIVE_V52_SOURCE_IDS)[number];

export interface InputLiveV52SourceIdentity {
  source: InputLiveV52SourceId;
...
export interface InputLiveV52WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV52ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV52SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV52RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV52RootProof {
  source: InputLiveV52SourceId;
  adapterIdentity: string;
...
export interface InputLiveV52ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV52SourceId;
    adapterIdentity: string;
...
export interface InputLiveV52CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV52SourceId;
  adapterIdentity: string;
...
export interface InputLiveV52CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV52SourceId;
  strata: InputLiveV52CaptureCell["strata"];
...
export interface InputLiveV52CleanupPayload {
...
export function validateInputLiveV52WriterPayload(
  value: unknown,
): InputLiveV52WriterOwnership {
...
const identityPlan = (source: InputLiveV52SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV52ExtractProgram(
  writer: InputLiveV52WriterOwnership,
  sources: readonly InputLiveV52SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V52_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V52_NAMESPACE)}
${buildInputLiveV52RawPropertyRuntime()}
...
export function validateInputLiveV52ExtractPayload(
  value: unknown,
  writer: InputLiveV52WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V52_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV52ExtractPayload;
}

export function proveInputLiveV52Roots<Instance>(
  extract: InputLiveV52ExtractPayload,
  sources: readonly (InputLiveV52SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV52Scene(
      root.scene,
...
export function assertInputLiveV52RootProofs(
  proofs: readonly InputLiveV52RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV52ProbeProgram(
  writer: InputLiveV52WriterOwnership,
  sources: readonly InputLiveV52SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V52_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV52ProbePayload(
  value: unknown,
  writer: InputLiveV52WriterOwnership,
): InputLiveV52ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV52ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V52_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V52_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV52CaptureManifest(
  cells: readonly InputLiveV52CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V52_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V52_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V52_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V52_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V52_SOURCE_IDS.some(
...
export const inputLiveV52CaptureManifestSha256 = (
  cells: readonly InputLiveV52CaptureCell[],
): string => {
  validateInputLiveV52CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV52CaptureProgram(
  writer: InputLiveV52WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V52_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V52_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V52_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV52CapturePayload(
  value: unknown,
  cell: InputLiveV52CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV52CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V52_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V52_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV52CaptureResponses(
  manifest: readonly InputLiveV52CaptureCell[],
  responses: readonly InputLiveV52CapturePayload[],
): void {
  validateInputLiveV52CaptureManifest(manifest);
  if (
...
export interface InputLiveV52ObjectiveReport {
  artifactVersion: "input-live-v52-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV52Objective(
  manifest: readonly InputLiveV52CaptureCell[],
  responses: readonly InputLiveV52CapturePayload[],
  readReference: (cell: InputLiveV52CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV52CaptureCell) => Buffer,
): InputLiveV52ObjectiveReport {
  assertInputLiveV52CaptureResponses(manifest, responses);
...
export function buildInputLiveV52CleanupProgram(
  writer: InputLiveV52WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V52_NAMESPACE,
...
export function validateInputLiveV52CleanupPayload(
  value: unknown,
  writer: InputLiveV52WriterOwnership,
): InputLiveV52CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV52CleanupPayload;
...
export const INPUT_LIVE_V52_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV52WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
