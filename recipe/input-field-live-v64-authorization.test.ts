...
import { INPUT_LIVE_V64_INDEX_PATH } from "./build-input-field-live-proof-v64.js";
import {
  INPUT_LIVE_V64_DYNAMIC_TOOL,
  INPUT_LIVE_V64_TARGET,
} from "./input-field-live-v64-broker.js";
import {
  buildInputLiveV64AuthorizationArtifact,
  validateInputLiveV64AntecedentIndex,
  validateInputLiveV64AuthorizationArtifact,
  validateInputLiveV64History,
  validateInputLiveV64UnitTestSource,
  type InputLiveV64AntecedentIndex,
  type InputLiveV64AuthorizationArtifact,
  type InputLiveV64AuthorizationProof,
  type InputLiveV64HistoryState,
} from "./input-field-live-v64-authorization.js";
import {
  inputLiveV64AttestationSha256,
  validateInputLiveV64ControlFlowSource,
  validateInputLiveV64SecurityAttestation,
  type InputLiveV64SecurityAttestation,
} from "./input-field-live-v64-preflight.js";
import { inputLiveV64Sha256 } from "./input-field-live-v64-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V64_INDEX_PATH);
const index = (): InputLiveV64AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV64AntecedentIndex;

const artifact = (): InputLiveV64AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV64AuthorizationArtifact({
...
const pendingState = (): InputLiveV64HistoryState => ({
  index: index(),
  indexSha256: inputLiveV64Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV64HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV64AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV64Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V64_TARGET,
  expectedDynamicTool: INPUT_LIVE_V64_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v64/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV64AuthorizationArtifact,
): InputLiveV64SecurityAttestation => {
  const body: Omit<InputLiveV64SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v64-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V64_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV64AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV64History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV64History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV64History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV64History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV64History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV64UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV64UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV64AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV64Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV64AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV64AuthorizationArtifact(
        planted as InputLiveV64AuthorizationArtifact,
        index(),
        inputLiveV64Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV64SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV64AttestationSha256(
      body as Omit<InputLiveV64SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV64SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV64ControlFlowSource(source), []);
});
