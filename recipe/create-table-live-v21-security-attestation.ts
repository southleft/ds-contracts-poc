...
import { verifyTableLiveV21Authorization } from "./table-live-v21-authorization.js";
import { TABLE_LIVE_V21_TARGET } from "./table-live-v21-broker.js";
import {
  TABLE_LIVE_V21_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV21Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-table-v21-session";
...
const attestation: TableLiveV21SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV21SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v21 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V21_SECURITY_ATTESTATION_DEFAULT_PATH,
);
