...
} from "./input-field-live-v3-verifier-v54.js";
import {
  buildInputLiveV54RawPropertyRuntime,
  normalizeInputLiveV54Scene,
  type InputLiveV54RawNode,
} from "./input-field-live-v54-verifier.js";
...
export const INPUT_LIVE_V54_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V54_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V54_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V54_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V54_VARIANT_COUNT = 256;
export const INPUT_LIVE_V54_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V54_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV54SourceId = (typeof INPUT_LIVE_V54_SOURCE_IDS)[number];

export interface InputLiveV54SourceIdentity {
  source: InputLiveV54SourceId;
...
export interface InputLiveV54WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV54ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV54SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV54RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV54RootProof {
  source: InputLiveV54SourceId;
  adapterIdentity: string;
...
export interface InputLiveV54ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV54SourceId;
    adapterIdentity: string;
...
export interface InputLiveV54CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV54SourceId;
  adapterIdentity: string;
...
export interface InputLiveV54CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV54SourceId;
  strata: InputLiveV54CaptureCell["strata"];
...
export interface InputLiveV54CleanupPayload {
...
export function validateInputLiveV54WriterPayload(
  value: unknown,
): InputLiveV54WriterOwnership {
...
const identityPlan = (source: InputLiveV54SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV54ExtractProgram(
  writer: InputLiveV54WriterOwnership,
  sources: readonly InputLiveV54SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V54_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V54_NAMESPACE)}
${buildInputLiveV54RawPropertyRuntime()}
...
export function validateInputLiveV54ExtractPayload(
  value: unknown,
  writer: InputLiveV54WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V54_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV54ExtractPayload;
}

export function proveInputLiveV54Roots<Instance>(
  extract: InputLiveV54ExtractPayload,
  sources: readonly (InputLiveV54SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV54Scene(
      root.scene,
...
export function assertInputLiveV54RootProofs(
  proofs: readonly InputLiveV54RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV54ProbeProgram(
  writer: InputLiveV54WriterOwnership,
  sources: readonly InputLiveV54SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V54_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV54ProbePayload(
  value: unknown,
  writer: InputLiveV54WriterOwnership,
): InputLiveV54ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV54ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V54_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V54_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV54CaptureManifest(
  cells: readonly InputLiveV54CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V54_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V54_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V54_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V54_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V54_SOURCE_IDS.some(
...
export const inputLiveV54CaptureManifestSha256 = (
  cells: readonly InputLiveV54CaptureCell[],
): string => {
  validateInputLiveV54CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV54CaptureProgram(
  writer: InputLiveV54WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V54_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V54_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V54_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV54CapturePayload(
  value: unknown,
  cell: InputLiveV54CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV54CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V54_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V54_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV54CaptureResponses(
  manifest: readonly InputLiveV54CaptureCell[],
  responses: readonly InputLiveV54CapturePayload[],
): void {
  validateInputLiveV54CaptureManifest(manifest);
  if (
...
export interface InputLiveV54ObjectiveReport {
  artifactVersion: "input-live-v54-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV54Objective(
  manifest: readonly InputLiveV54CaptureCell[],
  responses: readonly InputLiveV54CapturePayload[],
  readReference: (cell: InputLiveV54CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV54CaptureCell) => Buffer,
): InputLiveV54ObjectiveReport {
  assertInputLiveV54CaptureResponses(manifest, responses);
...
export function buildInputLiveV54CleanupProgram(
  writer: InputLiveV54WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V54_NAMESPACE,
...
export function validateInputLiveV54CleanupPayload(
  value: unknown,
  writer: InputLiveV54WriterOwnership,
): InputLiveV54CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV54CleanupPayload;
...
export const INPUT_LIVE_V54_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV54WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
