...
import {
  verifyCalendarLiveV9FixedSceneFixedPoint,
  type CalendarLiveV9FixedFixedPoint,
} from "./calendar-live-v9-fixed-point.js";

export const CALENDAR_LIVE_V9_VERIFIER_VERSION = "calendar-live-v9-verifier-v1";

export function buildCalendarLiveV9RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV9RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV9RawNode[];
}

export interface CalendarLiveV9NormalizedScene {
...
export interface CalendarLiveV9SceneProof {
...
export function normalizeCalendarLiveV9Scene(
  root: CalendarLiveV9RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV9NormalizedScene {
  const canonicalBindings: CalendarLiveV9NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV9RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV9Scene<Instance>(
  normalized: CalendarLiveV9NormalizedScene,
...
): CalendarLiveV9SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV9FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV9ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV9PreCaptureGates(payload: unknown): void {
