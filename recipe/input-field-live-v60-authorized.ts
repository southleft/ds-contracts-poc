...
import { INPUT_LIVE_V60_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v60.js";
import { verifyInputLiveV60Authorization } from "./input-field-live-v60-authorization.js";
import { runInputLiveV60Preflight } from "./input-field-live-v60-preflight.js";
import { InputLiveV60Orchestrator } from "./run-input-field-live-v60.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V60_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v60-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV60Authorization(root);
    runInputLiveV60Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV60Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
