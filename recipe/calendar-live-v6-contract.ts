...
import { buildCalendarLiveV6CleanupRuntime } from "./calendar-live-v6-cleanup.js";
import { type CalendarLiveV6FixedFixedPoint } from "./calendar-live-v6-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV6RestoreProgram,
  validateCalendarLiveV6RestorePayload,
  type CalendarLiveV6RestorePayload,
} from "./calendar-live-v6-restore.js";
import {
  buildCalendarLiveV6RawPropertyRuntime,
  normalizeCalendarLiveV6Scene,
  type CalendarLiveV6RawNode,
} from "./calendar-live-v6-verifier.js";
...
export const CALENDAR_LIVE_V6_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V6_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V6_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V6_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V6_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V6_SET_COUNT = 3;
export const CALENDAR_LIVE_V6_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V6_HOST_PHASES = 3;
export const CALENDAR_LIVE_V6_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V6_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V6_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V6_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV6SourceId = (typeof CALENDAR_LIVE_V6_SOURCE_IDS)[number];
export type CalendarLiveV6Kind = "calendar" | "week" | "day";

export interface CalendarLiveV6SourceIdentity {
  source: CalendarLiveV6SourceId;
...
export interface CalendarLiveV6WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV6ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV6SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV6RawNode;
    weekScene: CalendarLiveV6RawNode;
    dayScene: CalendarLiveV6RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV6RootProof {
  source: CalendarLiveV6SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV6FixedFixedPoint;
}

export interface CalendarLiveV6ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV6SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV6CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV6SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV6CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV6SourceId;
  frameWidth: number;
...
export interface CalendarLiveV6CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v6 ${label} must be an object`);
...
export function validateCalendarLiveV6WriterPayload(
  value: unknown,
): CalendarLiveV6WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v6") ||
    ...
    result.namespace !== CALENDAR_LIVE_V6_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v6 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v6 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v6 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV6SourceIdentity,
  kind: CalendarLiveV6Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV6ExtractProgram(
  writer: CalendarLiveV6WriterOwnership,
  sources: readonly CalendarLiveV6SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v6 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V6_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V6_NAMESPACE)}
${buildCalendarLiveV6RawPropertyRuntime()}
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("CALENDAR-V6-EXTRACT-PAGE");
...
if(get(page,"pageOwner")!=="recipe/calendar/"+${JSON.stringify(writer.runIdentity)})throw new Error("CALENDAR-V6-EXTRACT-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>SET_IDS.has(node.id));
if(sets.length!==3)throw new Error("CALENDAR-V6-EXTRACT-ROOTS:"+sets.length);
...
for(const set of sets){
  ...
  if(!IDENTITIES[adapterIdentity]||(kind!=="calendar"&&kind!=="week"&&kind!=="day"))throw new Error("CALENDAR-V6-EXTRACT-ADAPTER:"+adapterIdentity+":"+kind);
...
if(roots.length!==1||roots.some(root=>!root.calendarScene||!root.weekScene||!root.dayScene))throw new Error("CALENDAR-V6-EXTRACT-TRIPLE");
...
export function validateCalendarLiveV6ExtractPayload(
  value: unknown,
  writer: CalendarLiveV6WriterOwnership,
): CalendarLiveV6ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v6 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V6_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v6 extract schema/one-root mismatch");
  return payload as CalendarLiveV6ExtractPayload;
}

export function proveCalendarLiveV6Roots<Instance>(
  extract: CalendarLiveV6ExtractPayload,
  sources: readonly (CalendarLiveV6SourceIdentity & {
...
): CalendarLiveV6RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v6 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v6 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV6Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV6Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV6Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV6FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v6 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v6 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v6 compile lost owned sets");
...
export function assertCalendarLiveV6RootProofs(
  proofs: readonly CalendarLiveV6RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v6 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v6 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV6ProbeProgram(
  writer: CalendarLiveV6WriterOwnership,
  sources: readonly CalendarLiveV6SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V6_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV6ProbePayload(
  value: unknown,
  writer: CalendarLiveV6WriterOwnership,
): CalendarLiveV6ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV6ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V6_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V6_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v6 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV6CaptureManifest(
  cells: readonly CalendarLiveV6CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V6_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V6_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V6_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V6_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v6 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV6CaptureCell[],
): string => {
  validateCalendarLiveV6CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV6CaptureProgram(
  writer: CalendarLiveV6WriterOwnership,
  cell: CalendarLiveV6CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V6_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v6 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v6 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V6_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV6CapturePayload(
  value: unknown,
  cell: CalendarLiveV6CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV6CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV6CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V6_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V6_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v6 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV6CaptureResponses(
  manifest: readonly CalendarLiveV6CaptureCell[],
  responses: readonly CalendarLiveV6CapturePayload[],
): void {
  validateCalendarLiveV6CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v6 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV6ObjectiveReport {
  artifactVersion: "calendar-live-v6-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV6Objective(
  manifest: readonly CalendarLiveV6CaptureCell[],
  responses: readonly CalendarLiveV6CapturePayload[],
): CalendarLiveV6ObjectiveReport {
  assertCalendarLiveV6CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v6-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV6CleanupProgram(
  writer: CalendarLiveV6WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV6CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V6_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v6 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV6CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV6WriterOwnership,
): CalendarLiveV6CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV6CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v6 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V6_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV6WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
