...
import { TABLE_LIVE_V12_TARGET } from "./table-live-v12-broker.js";
import {
  TABLE_LIVE_V12_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV12Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-table-v12-session";
...
const attestation: TableLiveV12SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV12SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v12 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V12_SECURITY_ATTESTATION_DEFAULT_PATH,
);
