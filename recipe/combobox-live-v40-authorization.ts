...
export type ComboboxLiveV40HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV40AuthorizationArtifact {
  artifactVersion: "combobox-live-v40-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V40_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV40AntecedentIndex {
...
export interface ComboboxLiveV40HistoryState {
  index?: ComboboxLiveV40AntecedentIndex;
  indexSha256: string;
...
export interface ComboboxLiveV40AuthorizationProof extends ComboboxLiveV40TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V40_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V40_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V40_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V40_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v40-authorization") ||
  artifactPath.includes("combobox-live-v40-preflight") ||
  artifactPath.includes("combobox-live-v40-authorized") ||
  artifactPath.includes("create-combobox-live-v40-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v40-status") ||
...
export function validateComboboxLiveV40AntecedentIndex(
  index: ComboboxLiveV40AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v40-antecedent-index-v1" ||
...
export function validateComboboxLiveV40UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV40History\(\s*["']/.test(source) ||
    /readComboboxLiveV40HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV40AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV40AuthorizationArtifact {
  ...
  ) as ComboboxLiveV40AntecedentIndex;
  const indexFailures = validateComboboxLiveV40AntecedentIndex(index);
...
export function validateComboboxLiveV40AuthorizationArtifact(
  artifact: ComboboxLiveV40AuthorizationArtifact | undefined,
  index: ComboboxLiveV40AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v40-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v40" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V40_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V40_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V40_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V40_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V40_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V40_SOURCE_ROOTS ||
...
export function validateComboboxLiveV40History(
  state: ComboboxLiveV40HistoryState,
  expected: ComboboxLiveV40HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV40AntecedentIndex(state.index);
...
export function readComboboxLiveV40HistoryState(
  root = process.cwd(),
): ComboboxLiveV40HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V40_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV40AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V40_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V40_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V40_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV40History(
  expected: ComboboxLiveV40HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV40HistoryState {
  const state = readComboboxLiveV40HistoryState(root);
  const failures = validateComboboxLiveV40History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v40 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV40Authorization(
  root = process.cwd(),
): ComboboxLiveV40AuthorizationProof {
  const state = verifyComboboxLiveV40History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V40_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v40/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v40.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V40_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V40_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V40_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV40HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV40History(expected);
  process.stdout.write(
    `Combobox live v40 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
