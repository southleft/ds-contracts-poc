...
import {
  assertTableLiveV7RootProofs,
  TABLE_LIVE_V7_CAPTURE_COUNT,
  TABLE_LIVE_V7_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  TABLE_LIVE_V7_RESPONSE_CONTRACTS,
  type TableLiveV7RootProof,
} from "./table-live-v7-contract.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V7_BROKER_VERSION = "table-live-v7-broker-v1";
export const TABLE_LIVE_V7_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const TABLE_LIVE_V7_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const TABLE_LIVE_V7_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const TABLE_LIVE_V7_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const TABLE_LIVE_V7_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const TABLE_LIVE_V7_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type TableLiveV7RemotePhase =
  (typeof TABLE_LIVE_V7_REMOTE_PHASES)[number];
export type TableLiveV7AuthorizationMode = "live" | "simulated";

export interface TableLiveV7CommitPins {
...
export interface TableLiveV7TransactionAuthorization extends TableLiveV7CommitPins {
  mode: TableLiveV7AuthorizationMode;
...
export interface TableLiveV7Manifest {
  artifactVersion: "table-live-v7-broker-manifest-v2";
  brokerVersion: typeof TABLE_LIVE_V7_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof TABLE_LIVE_V7_DYNAMIC_TOOL;
  target: typeof TABLE_LIVE_V7_TARGET;
  authorization: TableLiveV7TransactionAuthorization;
...
export interface TableLiveV7ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface TableLiveV7RequestBody {
  artifactVersion: "table-live-v7-broker-request-v2";
  brokerVersion: typeof TABLE_LIVE_V7_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: TableLiveV7RemotePhase;
  sequence: number;
  ...
  commitPins: TableLiveV7CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof TABLE_LIVE_V7_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: TableLiveV7ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface TableLiveV7Request extends TableLiveV7RequestBody {
...
export interface TableLiveV7Receipt {
  artifactVersion: "table-live-v7-broker-receipt-v2";
  brokerVersion: typeof TABLE_LIVE_V7_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: TableLiveV7RemotePhase;
...
export interface TableLiveV7AcceptedResponse<T = unknown> {
  request: TableLiveV7Request;
  receipt: TableLiveV7Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface TableLiveV7IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: TableLiveV7ResponseContract;
...
const equalTarget = (value: unknown): value is typeof TABLE_LIVE_V7_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === TABLE_LIVE_V7_TARGET.fileKey &&
  (value as BrokerTarget).fileName === TABLE_LIVE_V7_TARGET.fileName &&
  (value as BrokerTarget).editorType === TABLE_LIVE_V7_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== TABLE_LIVE_V7_TARGET.fileKey ||
    context.fileName !== TABLE_LIVE_V7_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === TABLE_LIVE_V7_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Table live v7 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= TABLE_LIVE_V7_CAPTURE_COUNT
  )
    throw new TypeError("Table live v7 capture index must be 0..19");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Table live v7 authorization pins are malformed");
};

const readManifest = (directory: string): TableLiveV7Manifest => {
  const manifest = json<TableLiveV7Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Table live v7 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "table-live-v7-broker-manifest-v2" ||
    manifest.brokerVersion !== TABLE_LIVE_V7_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== TABLE_LIVE_V7_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V7_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Table live v7 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: TableLiveV7Request,
): TableLiveV7RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: TableLiveV7RemotePhase,
  captureIndex?: number,
): TableLiveV7Request | undefined => {
  const file = tableLiveV2RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV7Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: TableLiveV7RemotePhase,
  captureIndex?: number,
): TableLiveV7Receipt | undefined => {
  const file = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV7Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: TableLiveV7Request | undefined;
  receipt: TableLiveV7Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: TableLiveV7RemotePhase,
): TableLiveV7ResponseContract => ({
  ...TABLE_LIVE_V7_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: TABLE_LIVE_V7_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: TABLE_LIVE_V7_DYNAMIC_TOOL,
      target: TABLE_LIVE_V7_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "table-live-v7-plugin-response-v2",
  brokerVersion: TABLE_LIVE_V7_BROKER_VERSION,
  target: TABLE_LIVE_V7_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*TABLE_LIVE_V7_PROGRAM_START*/${program}/*TABLE_LIVE_V7_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*TABLE_LIVE_V7_PROGRAM_START*/";
  const endMarker = "/*TABLE_LIVE_V7_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Table live v7 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createTableLiveV7Transaction(
  directory: string,
  ...
  if (existsSync(path.join(directory, MANIFEST_FILE)))
    throw new Error("Table live v7 transaction already exists");
  validateAuthorization(options.authorization);
  if (
    !SHA256.test(options.proofPlanSha256) ||
    !SHA256.test(options.captureManifestSha256)
  )
    throw new TypeError("Table live v7 transaction plan hashes malformed");
  const attempt = options.attempt ?? 1;
  if (!Number.isInteger(attempt) || attempt < 1 || attempt > 3)
    throw new TypeError("Table live v7 attempt must be 1..3");
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Table live v7 private signing key does not match authorized public-key identity",
    );
  const manifest: TableLiveV7Manifest = {
    artifactVersion: "table-live-v7-broker-manifest-v2",
    brokerVersion: TABLE_LIVE_V7_BROKER_VERSION,
    ...
    captureRequests: TABLE_LIVE_V7_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateTableLiveV7Request(
  directory: string,
  request: TableLiveV7Request,
): void {
  ...
  if (
    request.artifactVersion !== "table-live-v7-broker-request-v2" ||
    request.brokerVersion !== TABLE_LIVE_V7_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== TABLE_LIVE_V7_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Table live v7 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueTableLiveV7Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: TableLiveV7RemotePhase,
  program: string,
  options: TableLiveV7IssueOptions = {},
): TableLiveV7Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<TableLiveV7Request>(targetPath);
    validateTableLiveV7Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Table live v7 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(tableLiveV2ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Table live v7 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Table live v7 empty code envelope refused");
  if (phase === "capture") validateTableLiveV7TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Table live v7 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: TABLE_LIVE_V7_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? TABLE_LIVE_V7_SIGNED_WRITER_TIMEOUT_MS
        : TABLE_LIVE_V7_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: TableLiveV7RequestBody = {
    artifactVersion: "table-live-v7-broker-request-v2",
    brokerVersion: TABLE_LIVE_V7_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: TableLiveV7Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateTableLiveV7Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistTableLiveV7RawResponse(
  directory: string,
  phase: TableLiveV7RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Table live v7 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<TableLiveV7Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Table live v7 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = tableLiveV2ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Table live v7 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<TableLiveV7Receipt, "receiptSha256"> = {
    artifactVersion: "table-live-v7-broker-receipt-v2",
    brokerVersion: TABLE_LIVE_V7_BROKER_VERSION,
...
export function acceptTableLiveV7Response<T = unknown>(
  directory: string,
  phase: TableLiveV7RemotePhase,
  options: {
    captureIndex?: number;
...
): TableLiveV7AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Table live v7 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<TableLiveV7Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Table live v7 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "table-live-v7-plugin-response-v2" ||
    plugin?.brokerVersion !== TABLE_LIVE_V7_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Table live v7 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistTableLiveV7TechnicalGates(
  directory: string,
  rootProofs: readonly TableLiveV7RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertTableLiveV7RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Table live v7 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Table live v7 probe payload hash malformed");
  const gates = Object.fromEntries(
    TABLE_LIVE_V7_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "table-live-v7-host-technical-gates-v2",
    brokerVersion: TABLE_LIVE_V7_BROKER_VERSION,
...
export function validateTableLiveV7TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "table-live-v7-host-technical-gates-v2" ||
    ...
    TABLE_LIVE_V7_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Table live v7 technical gates artifact");
}

export function validateTableLiveV7Receipt(
  directory: string,
  phase: TableLiveV7RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateTableLiveV7Request(directory, request);
  const receipt = json<TableLiveV7Receipt>(
  ...
  if (
    receipt.artifactVersion !== "table-live-v7-broker-receipt-v2" ||
    receipt.brokerVersion !== TABLE_LIVE_V7_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Table live v7 accepted receipt: ${request.requestId}`,
    );
}

export function readTableLiveV7AcceptedRawPayload(
  directory: string,
  phase: TableLiveV7RemotePhase,
  captureIndex?: number,
): unknown {
  validateTableLiveV7Receipt(directory, phase, captureIndex);
...
export function createTableLiveV7FakeDynamicResponse(
  request: TableLiveV7Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? TABLE_LIVE_V7_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? TABLE_LIVE_V7_TARGET,
    ...overrides.extra,
  };
}

export function listTableLiveV7PersistedRequests(
  directory: string,
): TableLiveV7Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<TableLiveV7Request>(file)] : [];
    });
}
