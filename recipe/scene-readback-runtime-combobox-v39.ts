...
/**
 * Carried Combobox live v39 extract runtime. Keeps the v3 teaching that
 * owned COMPONENT_SET roots may omit `envelopeHash` and the v4 teaching
 * that Figma-copied `ownershipKey` on descendants inside an owned
 * INSTANCE must be ignored. Host omit of empty instance payload,
 * name-before-`#` property recovery, trigger compile-order binding
 * sort, leading-slot compile-order binding sort, leading-slot
 * compile-carry visible, trailing-slot compile-order binding sort,
 * trailing-slot compile-carry visible, trigger empty-effects
 * omit, overlay compile-order binding sort, overlay
 * `width.value` → `layout.width.value` alias, listbox
 * compile-order binding sort, option-instance binding
 * extras-drop, option-instance inherited-fills omit,
 * option-instance compile-omit payload, listbox
 * clipsContent omit, listbox cornerRadius omit,
 * listbox empty-effects omit, listbox empty-strokes
 * omit, set-root clipsContent omit, option
 * compile-order binding sort, option
 * `height.value` → `layout.height.value` alias,
 * option clipsContent omit, selected-indicator
 * compile-order binding sort, selected-indicator
 * compile-carry visible, option-set
 * compile-carry label, observe empty
 * slot-instance `instancePayload` omit, and
 * v35 nonempty option-instance
 * `instancePayload` omit live in
 * scene-readback-combobox-v39. Trigger-slot
 * `componentRef` compile sibling-order
 * recovery, empty-content trigger
 * compile-carry characters, and observe
 * compile-carry option-set name also
 * live there. Does not restamp hashed
 * v1–v37 lineage. Writer bytes stay
 * frozen.
 */
...
const readSceneDerivedTree=async(node,expectedPlan,expectedOwner)=>{
  ...
  const read=async(current,generatedContext)=>{
    ...
    void "COMBOBOX-EXTRACT-IGNORE-COPIED-INSTANCE-OWNERSHIP-KEY";
    if(explicit&&!generatedContext){
      ...
      if(expectedOwner){
        ...
        void "COMBOBOX-EXTRACT-SET-ROOT-ENVELOPE-HASH";
    ...
    const COMBOBOX_EXTRACT_MEASURE_HIDDEN_CONTENT_FILL="COMBOBOX-EXTRACT-MEASURE-HIDDEN-CONTENT-FILL";
    const contentFillRoles=new Set(["combobox/input","combobox/option/label","combobox/listbox/empty","combobox/listbox/loading"]);
    for(const field of ["layoutMode","layoutSizingHorizontal","layoutSizingVertical","primaryAxisAlignItems","counterAxisAlignItems","itemSpacing","paddingTop","paddingRight","paddingBottom","paddingLeft","minWidth","minHeight","layoutPositioning","x","y","constraints","clipsContent","strokeWeight","strokeAlign","dashPattern","characters","fontName","fontSize","lineHeight","letterSpacing","textCase","textDecoration","textAlignHorizontal","textAlignVertical"]){
      if(field==="layoutSizingHorizontal"&&current.type==="TEXT"&&contentFillRoles.has(snapshot.semanticRole)&&current.visible===false){
        void COMBOBOX_EXTRACT_MEASURE_HIDDEN_CONTENT_FILL;
