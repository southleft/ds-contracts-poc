...
import { CALENDAR_LIVE_V9_TARGET } from "./calendar-live-v9-broker.js";
import {
  CALENDAR_LIVE_V9_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV9Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v9-session";
...
const attestation: CalendarLiveV9SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV9SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v9 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V9_SECURITY_ATTESTATION_DEFAULT_PATH,
);
