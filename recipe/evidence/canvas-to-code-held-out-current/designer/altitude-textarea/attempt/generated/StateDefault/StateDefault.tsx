 ...
 * Source of truth: contracts/state-default.contract.json (ds.state-default v0.1.0)
...
export interface StateDefaultProps extends Omit<HTMLAttributes<HTMLSpanElement>, 'children'> {
  state?: 'default' | 'error' | 'disabled';
}

/** STUB contract auto-proposed for the nested "State=Default" instances of Textarea — the child set was not imported. Props are the observed applied values ONLY; anatomy and styling are NOT captured (dump v1 stops at instance boundaries); the root renders the OBSERVED bounding box and primary paint (dump v1.5) as honest provisional geometry. Import the child set to replace this stub. */
export const StateDefault = forwardRef<HTMLSpanElement, StateDefaultProps>(function StateDefault(
  { state = 'default', className, ...rest },
  ref,
) {
  // axis-inert (ledgered, not a throw): state — no `.<axis>-*` rule
  // exists in StateDefault.module.css, so no class is composed for it. A reference
