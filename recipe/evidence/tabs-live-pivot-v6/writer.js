...
for(const source of PLAN.sources){
  ...
  const applyLayout=(node,ir)=>{
    ...
    void "TABS-WRITER-ABSOLUTE-POSITIONING";
    if(layout.positioning==="absolute"){
      ...
      node.constraints={horizontal:horiz[c.horizontal||"left"],vertical:vert[c.vertical||"top"]};
    }
  };
  void "TABS-WRITER-PLACE-ABSOLUTE-AFTER-PARENT-SIZES";
  // Absolute children are placed once their parent has its final (hug) size:
  // stretch → x = offset.x, width = parent.width − 2·offset.x; bottom →
  // y = parent.height − height − offset.y (a negative offset hangs below).
  const placeAbsolute=(parentNode,parentIr)=>{
    for(const childIr of parentIr.children){
      if(!childIr.layout||childIr.layout.positioning!=="absolute")continue;
      ...
      const off=childIr.layout.offset||{x:0,y:0},c=childIr.layout.constraints||{};
      ...
      if(c.vertical==="bottom")child.y=parentNode.height-child.height-off.y;
      else child.y=off.y;
    }
  };
  const applySizing=(node,ir)=>{
    ...
    const absolute=!!(ir.layout&&ir.layout.positioning==="absolute");
    void "TABS-WRITER-ABSOLUTE-CHILD-IS-FIXED-SIZED";
    // An absolute child is outside auto-layout flow: Figma refuses FILL on it
    // at assignment ("FILL cannot be set on absolute positioned auto-layout
    // children"), so the guard must come BEFORE the sizing assignment.
    if(absolute){node.layoutSizingHorizontal="FIXED";node.layoutSizingVertical="FIXED";}
    else{
  ...
  const render=async(ir,parent,ownershipKey)=>{
    ...
    if(ir.kind==="frame"){applyLayout(node,ir);const hugKids=[],fillKids=[];for(const [childIndex,child] of ir.children.entries()){const width=child.layout?child.layout.width:child.width;((width&&width.mode==="fill")?fillKids:hugKids).push([childIndex,child]);}for(const [childIndex,child] of hugKids)await render(child,node,ownershipKey+"/children/"+childIndex);applySizing(node,ir);for(const [childIndex,child] of fillKids)await render(child,node,ownershipKey+"/children/"+childIndex);placeAbsolute(node,ir);}
    else applySizing(node,ir);
    if(ir.kind==="text"){
      bindFloat(node,"fontSize",bindingFor(ir,"type.fontSize"));bindFloat(node,"letterSpacing",bindingFor(ir,"type.letterSpacing.value"));
