...
const runIdentity = "3a63b6dc-7f1ebbdc-d4c3859c-tooltip-v1";
const pageName = "Recipe Pivot / Tooltip / 3a63b6dc-7f1ebbdc-d4c3859c-tooltip-v1";
const adapterIdentity = "astryx-tooltip-reviewed-v1";
const recipeHash = "3a63b6dc7436c4c074cf66ad055216512d393e53eb0629553d19158c0cefdb1b";
const envelopeHash = "7d141adb91e11238a59ca9119566f63b108bbf6ed67f7a3f01c4ceb47afd24a4";
const signed = ["115:295378","163:35981","183:70641","183:69150","85:6781","173:48924","181:64873","183:74742","183:75031","183:75302","183:75495","183:75801","183:75976","183:76022","183:76063","183:76109"];
await figma.loadAllPagesAsync();
const page = figma.root.children.find((candidate) => candidate.name === pageName);
if (!page) throw new Error("TOOLTIP-PAGE-MISSING");
if (signed.includes(page.id)) throw new Error("TOOLTIP-MUST-NOT-WRITE-SIGNED-PAGE:" + page.id);
if (page.getSharedPluginData(NS, "runIdentity") !== runIdentity)
  throw new Error("TOOLTIP-PAGE-IDENTITY-MISMATCH:" + page.id);
await figma.setCurrentPageAsync(page);
if (signed.includes(figma.currentPage.id)) throw new Error("TOOLTIP-MUST-NOT-WRITE-SIGNED-PAGE:" + figma.currentPage.id);
if (page.children.some((node) => node.type === "SECTION" && node.getSharedPluginData(NS, "adapterIdentity") === adapterIdentity))
  throw new Error("TOOLTIP-SECTION-EXISTS:" + adapterIdentity);
...
const resolvePainted = async (spec) => {
  ...
  if (found.fontName.family !== spec.resolvedFamily || found.fontName.style !== spec.resolvedStyle)
    throw new Error("TOOLTIP-FONT-PROVENANCE-TAMPER:" + found.fontName.family + ":" + found.fontName.style);
...
const collectionName = "Recipe Tooltip / " + runIdentity + " / " + adapterIdentity;
const locals = figma.variables.getLocalVariableCollectionsAsync
  ? await figma.variables.getLocalVariableCollectionsAsync()
  : [];
if (locals.some((collection) => collection.name === collectionName)) throw new Error("TOOLTIP-VARIABLE-COLLECTION-COLLISION:" + collectionName);
const collection = figma.variables.createVariableCollection(collectionName);
collection.renameMode(collection.modes[0].modeId, "Default");
collection.hiddenFromPublishing = true;
const modeId = collection.modes[0].modeId;
const prefix = "astryx.tooltip";
const planned = [["FLOAT","astryx.tooltip.box-paddingX",8],["FLOAT","astryx.tooltip.box-paddingY",4],["FLOAT","astryx.tooltip.box-radius",12],["FLOAT","astryx.tooltip.box-borderWidth",0],["FLOAT","astryx.tooltip.labelFontSize",14],["FLOAT","astryx.tooltip.labelLineHeight",20],["COLOR","astryx.tooltip.rest-boxFill","#0a1317ff"],["COLOR","astryx.tooltip.rest-boxBorder","#00000000"],["COLOR","astryx.tooltip.rest-label","#ffffffff"]];
const vars = new Map();
...
const geom = {"labelFontSize":14,"labelLineHeight":20,"lineHeightUnit":"PIXELS","decoration":"NONE","label":"Tooltip","rest":{"boxFill":"#0a1317ff","boxBorder":"#00000000","boxOpacity":1,"label":"#ffffffff"},"box":{"paddingX":8,"paddingY":4,"radius":12,"borderWidth":0},"componentLabel":"Astryx Tooltip"};
...
const paintText = async (resolved, characters, fontSize, lineHeight, lineHeightUnit, decoration) => {
  ...
  if (node.characters.trim().length > 0 && (node.width <= 0 || node.absoluteRenderBounds === null)) {
    ...
    if (!ok && (node.width <= 0 || node.absoluteRenderBounds === null))
      throw new Error("TOOLTIP-FONT-ZERO-INTRINSIC");
  }
  if (node.width <= 0 || node.height <= 0) throw new Error("TOOLTIP-TEXT-GEOMETRY");
  return node;
};
...
component.name = "tooltip/variant/default :: " + geom.componentLabel;
component.description = "Experimental tooltip@1 bubble mint. Recipe " + recipeHash + "; source adapter " + adapterIdentity + ".";
...
label.name = "tooltip/label :: font-provenance=" + encodeURIComponent(JSON.stringify(labelResolved.spec));
...
if (component.layoutMode !== "HORIZONTAL") throw new Error("TOOLTIP-FAKE-LAYOUT:" + component.name);
...
container.setSharedPluginData(NS, "ownershipKey", "tooltip/container");
