...
if(figma.fileKey!=="byMp6lt0Ij9b2QbkDGFwBh"||figma.root.name!=="Scratch Project"||figma.editorType!=="figma")throw new Error("CALENDAR-V3-CLEANUP-WRONG-TARGET");
await figma.loadAllPagesAsync();
const CLEANUP_NS="ds.contracts.calendar.recipe.v1";
const CLEANUP_PAGE_NAME="Recipe Pivot / Calendar / 19be1c96-calendar-v3";
const CLEANUP_RUN_IDENTITY="19be1c96-calendar-v3";
const CLEANUP_PAGE_OWNER="recipe/calendar/19be1c96-calendar-v3";
const CLEANUP_COLLECTION_OWNER="recipe/calendar/19be1c96-calendar-v3/variable-collection";
const CLEANUP_ADAPTERS=new Set(["astryx-calendar-reviewed-v1"]);
const CLEANUP_COLLECTION_NAMES=new Set(["Recipe Calendar / 19be1c96-calendar-v3 / astryx-calendar-reviewed-v1"]);
...
if(cleanupUnexpectedCollections.length>0)throw new Error("CALENDAR-V3-CLEANUP-OWNED-COLLECTION-UNEXPECTED:"+cleanupUnexpectedCollections.map(collection=>collection.id).join(","));
const cleanupCollections=cleanupOwnedCollections;
if(cleanupPages.length>1)throw new Error("CALENDAR-V3-CLEANUP-PAGE-DENOMINATOR:"+cleanupPages.map(page=>page.id).join(","));
if(cleanupPages.some(page=>page.name!==CLEANUP_PAGE_NAME))throw new Error("CALENDAR-V3-CLEANUP-OWNED-PAGE-UNEXPECTED:"+cleanupPages.map(page=>page.id).join(","));
if(new Set(cleanupCollections.map(collection=>cleanupGet(collection,"adapterIdentity"))).size!==cleanupCollections.length)throw new Error("CALENDAR-V3-CLEANUP-COLLECTION-DENOMINATOR");
...
if(cleanupPages.includes(figma.currentPage)){
  ...
  if(!safePage)throw new Error("CALENDAR-V3-CLEANUP-SAFE-PAGE-ABSENT");
  if(typeof figma.setCurrentPageAsync!=="function")throw new Error("CALENDAR-V3-CLEANUP-PAGE-SWITCH-API-ABSENT");
  await figma.setCurrentPageAsync(safePage);
  if(figma.currentPage!==safePage)throw new Error("CALENDAR-V3-CLEANUP-PAGE-SWITCH-FAILED");
}
...
if(remainingOwnedNodes!==0||remainingOwnedCollections!==0)throw new Error("CALENDAR-V3-CLEANUP-OWNED-LEFTOVERS:"+remainingOwnedNodes+":"+remainingOwnedCollections);
if(!figma.root.children.some(page=>page.id===FORBIDDEN_INPUT_PAGE))throw new Error("CALENDAR-V3-CLEANUP-INPUT-PAGE-MISSING");
if(!figma.root.children.some(page=>page.id===FORBIDDEN_COMBOBOX_PAGE))throw new Error("CALENDAR-V3-CLEANUP-COMBOBOX-PAGE-MISSING");
if(!figma.root.children.some(page=>page.id===FORBIDDEN_BUTTON_PAGE))throw new Error("CALENDAR-V3-CLEANUP-BUTTON-PAGE-MISSING");
if(!figma.root.children.some(page=>page.id===FORBIDDEN_TABLE_PAGE))throw new Error("CALENDAR-V3-CLEANUP-TABLE-PAGE-MISSING");
if(JSON.stringify(requestedNodeIds)!==JSON.stringify([expectedPageId])||JSON.stringify([...requestedCollectionIds].sort())!==JSON.stringify([...expectedCollectionIds].sort()))throw new Error("CALENDAR-V3-CLEANUP-EXACT-OWNERSHIP");return{requestedNodeIds,removedNodeIds,requestedCollectionIds,removedCollectionIds,remainingOwnedNodes,remainingOwnedCollections,complete:true};
