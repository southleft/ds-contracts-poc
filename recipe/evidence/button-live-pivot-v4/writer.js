...
const sanitizeVariableName = (tokenIdentity, type) => {
  const segment = tokenIdentity
    ...
    .replace(/-+/g, "-")
    .replace(/^[-_]+|[-_]+$/g, "");
  const name = "token/" + type.toLowerCase() + "/" + segment;
...
for (const source of PLAN.sources) {
  ...
  const ensureVariable = (bound, type, scope) => {
    ...
    if (!variable) {
      ...
      setSharedData(variable, "irVariable", bound.variable);
      setSharedData(variable, "irVariableType", type);
      setSharedData(variable, "figmaVariableName", variableName);
  ...
  for (const cell of sourceCells) {
    ...
    setSharedData(component, "irCell", JSON.stringify(cell));
    setSharedData(component, "irFontFamily", size.fontFamily);
    setSharedData(component, "irFontStyle", size.fontStyle);
    ...
    const addIcon = (role, helper, ref) => {
      ...
      setSharedData(instance, "componentRef", ref);
    ...
    setSharedData(label, "irFontFamily", size.fontFamily);
    setSharedData(label, "irFontStyle", size.fontStyle);
    setSharedData(label, "resolvedFontFamily", font.family);
    setSharedData(label, "resolvedFontStyle", font.style);
  ...
  setSharedData(set, "sourceName", source.sourceName);
  setSharedData(set, "axes", JSON.stringify(source.axes));
  setSharedData(set, "cells", JSON.stringify(sourceCells));
  setSharedData(set, "comparedIrFacts", String(source.comparedIrFacts));
