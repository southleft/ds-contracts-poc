...
Recorded 2026-09-19 under dump v1.36, after the whole-pixel text box was lowered (`Part.textAutoResize: WIDTH_AND_HEIGHT`, docs/23 §D.42) and revised by the adversarial review (`fit-content` basis, container clamp — every row unchanged to the digit from the first cut, because every label here fits) and the harness stopped placing roots on fractional x positions (see `docs/CURRENT.md` row 2). Tabs is unchanged to the digit: its text lives inside child instance stubs the dump does not capture.
