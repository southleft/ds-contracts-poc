...
**Re-recorded 2026-09-19 after docs/23 §D.44, as revised by its adversarial review** (same file version: every `inputs/rest-dump.json` is byte-identical to the previous run). Rule A: a STRUCTURAL `button` guess is withheld when the set holds a child that is interactive by fact, so `Tab Panel` (around Altitude's named, text-drawing `Button`) is a `<div>`, and both `interactive-content-nested` rows and the 453 px width are gone. Rule B: the proposer carries a padding side the canvas draws as 0 on a button-like root — Altitude `Button`'s contract now declares `padding-top: 0px; padding-bottom: 0px`. What remains on Tabs is one `content-size-mismatch`, 441 vs 438 px on `variant-default` (`variant-stretch` is 441 vs 439, inside the 2 px slack): the two `Text Passage` STUBS carry the 441 px box observed on `Tab Panel`'s main component, while the panel instance inside Tabs is 439 px — the stub gap, not either rule. Each folder's `inputs/figma-proposals.md` is the proposer's own report (machine paths rewritten to `.`): the reason for every inferred or withheld element is there. Altitude Badge, Checkbox Group and CBDS Badge: every score identical to the digit, every problem unchanged, PNGs byte-identical.
