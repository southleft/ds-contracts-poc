...
import styles from './ArrowArcLeft.module.css';

export interface ArrowArcLeftProps extends Omit<HTMLAttributes<HTMLSpanElement>, 'children'> {
  format?: 'outline';
  weight?: 'regular';
}
...
export const ArrowArcLeft = forwardRef<HTMLSpanElement, ArrowArcLeftProps>(function ArrowArcLeft(
  { format = 'outline', weight = 'regular', className, ...rest },
  ref,
) {
  // axis-inert (ledgered, not a throw): format, weight — no `.<axis>-*` rule
  // exists in ArrowArcLeft.module.css, so no class is composed for them. A reference
