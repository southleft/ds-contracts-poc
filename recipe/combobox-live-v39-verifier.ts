...
import {
  verifyComboboxLiveV39FixedSceneFixedPoint,
  type ComboboxLiveV39FixedFixedPoint,
} from "./combobox-live-v39-fixed-point.js";

export const COMBOBOX_LIVE_V39_VERIFIER_VERSION = "combobox-live-v39-verifier-v1";

export function buildComboboxLiveV39RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV39RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV39RawNode[];
}

export interface ComboboxLiveV39NormalizedScene {
...
export interface ComboboxLiveV39SceneProof {
...
export function normalizeComboboxLiveV39Scene(
  root: ComboboxLiveV39RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV39NormalizedScene {
  const canonicalBindings: ComboboxLiveV39NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV39RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV39Scene<Instance>(
  normalized: ComboboxLiveV39NormalizedScene,
...
): ComboboxLiveV39SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV39FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV39ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV39PreCaptureGates(payload: unknown): void {
