...
const EVIDENCE = "recipe/evidence/alert-live-pivot-v9";
...
const receiptOwned = {
      artifactVersion: "alert-live-pivot-v9-prepare",
      teaching:
...
if (process.argv.includes("--check")) {
  ...
  if (writer.sourcePlans.length !== 6)
    throw new Error("alert prepare requires 6 library sources (reviewed + proposed)");
  console.log(
    JSON.stringify({
      check: "ok",
      runIdentity: writer.runIdentity,
