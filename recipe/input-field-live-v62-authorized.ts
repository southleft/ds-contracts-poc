...
import { INPUT_LIVE_V62_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v62.js";
import { verifyInputLiveV62Authorization } from "./input-field-live-v62-authorization.js";
import { runInputLiveV62Preflight } from "./input-field-live-v62-preflight.js";
import { InputLiveV62Orchestrator } from "./run-input-field-live-v62.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V62_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v62-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV62Authorization(root);
    runInputLiveV62Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV62Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
