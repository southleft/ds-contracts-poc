...
import {
  acceptCalendarLiveV16Response,
  createCalendarLiveV16Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV16Request,
  persistCalendarLiveV16RawResponse,
  persistCalendarLiveV16TechnicalGates,
  readCalendarLiveV16AcceptedRawPayload,
  type CalendarLiveV16Request,
  type CalendarLiveV16TransactionAuthorization,
} from "./calendar-live-v16-broker.js";
import {
  CALENDAR_LIVE_V16_CAPTURE_COUNT,
  assertCalendarLiveV16CaptureResponses,
  ...
  evaluateCalendarLiveV16Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV16Roots,
  validateCalendarLiveV16CaptureManifest,
  validateCalendarLiveV16CapturePayload,
  validateCalendarLiveV16CleanupPayload,
  validateCalendarLiveV16ExtractPayload,
  validateCalendarLiveV16ProbePayload,
  validateCalendarLiveV16WriterPayload,
  type CalendarLiveV16CaptureCell,
  type CalendarLiveV16CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V16_PLAN_PATH,
  type CalendarLiveV16ProofPlan,
} from "./build-calendar-live-proof-v16.js";
import {
...
export const CALENDAR_LIVE_V16_RUNNER_VERSION = "calendar-live-v16-runner-v1";

interface CalendarLiveV16JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV16OrchestratorState {
  artifactVersion: "calendar-live-v16-orchestrator-state-v1";
  ...
  journal: CalendarLiveV16JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV16ProofPlan;
  planSha256: string;
  captures: CalendarLiveV16CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV16SourceIdentity & {
...
export interface CalendarLiveV16NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV16OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v16 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V16_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V16_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV16ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV16CaptureCell[];
  validateCalendarLiveV16CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v16 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v16 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV16WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV16RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV16Request =>
  readJson<CalendarLiveV16Request>(file);

export class CalendarLiveV16Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV16Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV16OrchestratorState = {
      artifactVersion: "calendar-live-v16-orchestrator-state-v1",
    ...
    return new CalendarLiveV16Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV16OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV16Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV16NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV16NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v16 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV16NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v16 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV16RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV16Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV16CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV16AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV16WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV16Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV16ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV16Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV16Response(this.#directory, "writer", {
        validate: validateCalendarLiveV16WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV16Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV16RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV16Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV16ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV16Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV16RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v16-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV16Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV16ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV16Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV16ProbePayload(payload, ownership),
      });
      persistCalendarLiveV16TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v16 capture cell missing");
      const accepted = acceptCalendarLiveV16Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV16CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V16_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV16CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV16Objective(
    ...
    throw new Error(`Calendar live v16 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV16Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV16CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV16Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV16RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV16Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV16CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV16CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV16CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV16Authorization = (
  privateKey: KeyObject,
): CalendarLiveV16TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV16SimulationKey = (): KeyObject =>
