...
import { CALENDAR_LIVE_V23_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v23.js";
import { verifyCalendarLiveV23Authorization } from "./calendar-live-v23-authorization.js";
import { runCalendarLiveV23Preflight } from "./calendar-live-v23-preflight.js";
import { CalendarLiveV23Orchestrator } from "./run-calendar-live-v23.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v23 requires exact authorization artifact ${CALENDAR_LIVE_V23_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v23-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV23Authorization(root);
    runCalendarLiveV23Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV23Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV23Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
