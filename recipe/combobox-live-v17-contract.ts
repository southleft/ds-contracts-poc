...
import { buildComboboxLiveV17CleanupRuntime } from "./combobox-live-v17-cleanup.js";
import {
  type ComboboxLiveV17FixedFixedPoint,
} from "./combobox-live-v17-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV17RestoreProgram,
  validateComboboxLiveV17RestorePayload,
  type ComboboxLiveV17RestorePayload,
} from "./combobox-live-v17-restore.js";
import {
  buildComboboxLiveV17RawPropertyRuntime,
  normalizeComboboxLiveV17Scene,
  type ComboboxLiveV17RawNode,
} from "./combobox-live-v17-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v17.js";
import {
...
} from "./scene-readback-combobox-v17.js";
...
export const COMBOBOX_LIVE_V17_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V17_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V17_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V17_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V17_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V17_SET_COUNT = 4;
export const COMBOBOX_LIVE_V17_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V17_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V17_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V17_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V17_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV17SourceId = (typeof COMBOBOX_LIVE_V17_SOURCE_IDS)[number];

export interface ComboboxLiveV17SourceIdentity {
  source: ComboboxLiveV17SourceId;
...
export interface ComboboxLiveV17WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV17ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV17SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV17RawNode;
    optionScene: ComboboxLiveV17RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV17RootProof {
  source: ComboboxLiveV17SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV17FixedFixedPoint;
}

export interface ComboboxLiveV17ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV17SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV17CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV17SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV17CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV17SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV17CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v17 ${label} must be an object`);
...
export function validateComboboxLiveV17WriterPayload(
  value: unknown,
): ComboboxLiveV17WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V17_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v17 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v17 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v17 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV17SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV17ExtractProgram(
  writer: ComboboxLiveV17WriterOwnership,
  sources: readonly ComboboxLiveV17SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v17 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V17_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V17_NAMESPACE)}
${buildComboboxLiveV17RawPropertyRuntime()}
...
export function validateComboboxLiveV17ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV17WriterOwnership,
): ComboboxLiveV17ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v17 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V17_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v17 extract schema/two-root mismatch");
  return payload as ComboboxLiveV17ExtractPayload;
}

export function proveComboboxLiveV17Roots<Instance>(
  extract: ComboboxLiveV17ExtractPayload,
  sources: readonly (ComboboxLiveV17SourceIdentity & {
...
): ComboboxLiveV17RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v17 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v17 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV17Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV17Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV17FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v17 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v17 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v17 compile lost owned sets");
...
export function assertComboboxLiveV17RootProofs(
  proofs: readonly ComboboxLiveV17RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v17 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v17 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV17ProbeProgram(
  writer: ComboboxLiveV17WriterOwnership,
  sources: readonly ComboboxLiveV17SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V17_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV17ProbePayload(
  value: unknown,
  writer: ComboboxLiveV17WriterOwnership,
): ComboboxLiveV17ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV17ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V17_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V17_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v17 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV17CaptureManifest(
  cells: readonly ComboboxLiveV17CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V17_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V17_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V17_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V17_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v17 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV17CaptureCell[],
): string => {
  validateComboboxLiveV17CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV17CaptureProgram(
  writer: ComboboxLiveV17WriterOwnership,
  cell: ComboboxLiveV17CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V17_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v17 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v17 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V17_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV17CapturePayload(
  value: unknown,
  cell: ComboboxLiveV17CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV17CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV17CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V17_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V17_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v17 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV17CaptureResponses(
  manifest: readonly ComboboxLiveV17CaptureCell[],
  responses: readonly ComboboxLiveV17CapturePayload[],
): void {
  validateComboboxLiveV17CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v17 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV17ObjectiveReport {
  artifactVersion: "combobox-live-v17-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV17Objective(
  manifest: readonly ComboboxLiveV17CaptureCell[],
  responses: readonly ComboboxLiveV17CapturePayload[],
): ComboboxLiveV17ObjectiveReport {
  assertComboboxLiveV17CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v17-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV17CleanupProgram(
  writer: ComboboxLiveV17WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV17CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V17_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v17 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV17CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV17WriterOwnership,
): ComboboxLiveV17CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV17CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v17 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V17_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV17WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
