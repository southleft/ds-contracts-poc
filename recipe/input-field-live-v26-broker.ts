...
import {
  ...
  INPUT_LIVE_V26_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V26_RESPONSE_CONTRACTS,
  type InputLiveV26RootProof,
} from "./input-field-live-v26-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V26_BROKER_VERSION = "input-live-v26-broker-v2";
export const INPUT_LIVE_V26_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V26_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V26_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V26_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V26_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V26_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV26RemotePhase =
  (typeof INPUT_LIVE_V26_REMOTE_PHASES)[number];
export type InputLiveV26AuthorizationMode = "live" | "simulated";

export interface InputLiveV26CommitPins {
...
export interface InputLiveV26TransactionAuthorization extends InputLiveV26CommitPins {
  mode: InputLiveV26AuthorizationMode;
...
export interface InputLiveV26Manifest {
  artifactVersion: "input-live-v26-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V26_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V26_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V26_TARGET;
...
export interface InputLiveV26ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV26RequestBody {
  artifactVersion: "input-live-v26-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V26_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV26RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV26CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V26_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV26ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV26Request extends InputLiveV26RequestBody {
...
export interface InputLiveV26Receipt {
  artifactVersion: "input-live-v26-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V26_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV26RemotePhase;
...
export interface InputLiveV26AcceptedResponse<T = unknown> {
  request: InputLiveV26Request;
  receipt: InputLiveV26Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV26IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV26ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V26_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V26_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V26_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V26_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V26_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V26_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V26_TARGET.editorType;
};
...
export const inputLiveV26RequestSequence = (
  phase: InputLiveV26RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV26RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV26RequestPath = (
  directory: string,
  phase: InputLiveV26RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV26ResponsePath = (
  directory: string,
  phase: InputLiveV26RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV26ReceiptPath = (
  directory: string,
  phase: InputLiveV26RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV26Manifest => {
  const manifest = json<InputLiveV26Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v26-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V26_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V26_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV26RemotePhase,
  captureIndex?: number,
): InputLiveV26Request | undefined => {
  const file = inputLiveV26RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV26Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV26RemotePhase,
  captureIndex?: number,
): InputLiveV26Receipt | undefined => {
  const file = inputLiveV26ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV26Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV26Request | undefined;
  receipt: InputLiveV26Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV26RemotePhase,
): InputLiveV26ResponseContract => ({
  ...INPUT_LIVE_V26_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V26_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V26_DYNAMIC_TOOL,
      target: INPUT_LIVE_V26_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v26-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V26_BROKER_VERSION,
  target: INPUT_LIVE_V26_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V26_PROGRAM_START*/${program}/*INPUT_LIVE_V26_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V26_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V26_PROGRAM_END*/";
...
export function createInputLiveV26Transaction(
  directory: string,
  ...
  const manifest: InputLiveV26Manifest = {
    artifactVersion: "input-live-v26-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V26_BROKER_VERSION,
...
export function validateInputLiveV26Request(
  directory: string,
  request: InputLiveV26Request,
): void {
  ...
  const expectedSequence = inputLiveV26RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v26-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V26_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V26_TARGET.fileKey ||
...
export function issueInputLiveV26Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV26RemotePhase,
  program: string,
  options: InputLiveV26IssueOptions = {},
): InputLiveV26Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV26RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV26Request>(targetPath);
    validateInputLiveV26Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV26ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV26TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV26RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V26_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V26_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V26_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV26RequestBody = {
    artifactVersion: "input-live-v26-broker-request-v2",
    brokerVersion: INPUT_LIVE_V26_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV26Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV26Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV26RawResponse(
  directory: string,
  phase: InputLiveV26RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV26RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV26Request>(requestFile);
  if (
  ...
  const target = inputLiveV26ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV26Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v26-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V26_BROKER_VERSION,
...
export function acceptInputLiveV26Response<T = unknown>(
  directory: string,
  phase: InputLiveV26RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV26AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV26ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV26Request>(
    inputLiveV26RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV26Request(directory, request);
  if (phase === "capture") validateInputLiveV26TechnicalGates(directory);
  const rawFile = inputLiveV26ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v26-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V26_BROKER_VERSION ||
...
export function persistInputLiveV26TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV26RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV26RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V26_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v26-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V26_BROKER_VERSION,
...
export function validateInputLiveV26TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v26-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V26_TECHNICAL_GATES.some(
...
export function validateInputLiveV26Receipt(
  directory: string,
  phase: InputLiveV26RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV26Request(directory, request);
  const receipt = json<InputLiveV26Receipt>(
    inputLiveV26ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV26ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v26-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V26_BROKER_VERSION ||
...
export function readInputLiveV26AcceptedRawPayload(
  directory: string,
  phase: InputLiveV26RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV26Receipt(directory, phase, captureIndex);
...
export function createInputLiveV26FakeDynamicResponse(
  request: InputLiveV26Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V26_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V26_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV26PersistedRequests(
  directory: string,
): InputLiveV26Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV26Request>(file)] : [];
    });
}

export const inputLiveV26Sha256 = sha256;
