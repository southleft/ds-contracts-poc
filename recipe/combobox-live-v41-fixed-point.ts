/**
 * Carried Combobox live v41 host verifier. Uses scene-readback-combobox-v41 so live
...
export const COMBOBOX_LIVE_V41_FIXED_VERIFIER_VERSION = "combobox-live-v41-fixed-point-v1";
export const COMBOBOX_LIVE_V41_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV41FixedThresholds {
...
export interface ComboboxLiveV41FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV41FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV41FixedSourceProbe {
...
export interface ComboboxLiveV41FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV41FixedFixedPoint;
}

export interface ComboboxLiveV41FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV41FixedHardGateInput {
  thresholds: ComboboxLiveV41FixedThresholds;
  materialRegression: ComboboxLiveV41FixedMaterialRegression;
  sourceProbes: ComboboxLiveV41FixedSourceProbe[];
  cells: ComboboxLiveV41FixedCellValidation[];
  sceneProofs: ComboboxLiveV41FixedSceneProof[];
  visualRows: ComboboxLiveV41FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV41FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V41_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV41FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV41FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV41FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV41FixedHardGates(
  input: ComboboxLiveV41FixedHardGateInput,
): ComboboxLiveV41FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV41FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV41FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V41_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV41FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV41FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV41FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
