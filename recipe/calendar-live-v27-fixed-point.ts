/**
 * Calendar live v27 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V27_FIXED_VERIFIER_VERSION = "calendar-live-v27-fixed-point-v1";
export const CALENDAR_LIVE_V27_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV27FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV27FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV27FixedSourceProbe {
...
export interface CalendarLiveV27FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV27FixedFixedPoint;
}

export interface CalendarLiveV27FixedHardGateInput {
  thresholds: CalendarLiveV27FixedThresholds;
  sourceProbes: CalendarLiveV27FixedSourceProbe[];
  cells: CalendarLiveV27FixedCellValidation[];
  sceneProofs: CalendarLiveV27FixedSceneProof[];
...
export interface CalendarLiveV27FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V27_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV27FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV27FixedHardGates(
  input: CalendarLiveV27FixedHardGateInput,
): CalendarLiveV27FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV27FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V27_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV27FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV27FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV27FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
