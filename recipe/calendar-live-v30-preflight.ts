...
import {
  validateCalendarLiveV30AntecedentIndex,
  verifyCalendarLiveV30Authorization,
  type CalendarLiveV30AntecedentIndex,
  type CalendarLiveV30AuthorizationProof,
} from "./calendar-live-v30-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V30_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v30/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V30_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v30-security-attestation.json";

export interface CalendarLiveV30SecurityAttestation {
  artifactVersion: "calendar-live-v30-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV30PreflightReport {
  artifactVersion: "calendar-live-v30-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV30SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV30SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV30AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v30-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V30_TARGET) ||
...
export function validateCalendarLiveV30ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV30Authorization");
  const preflight = source.indexOf("runCalendarLiveV30Preflight");
  const initialize = source.indexOf("CalendarLiveV30Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV30Preflight(
  root: string,
  proof: CalendarLiveV30AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V30_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV30PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V30_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V30_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV30AntecedentIndex;
  failures.push(...validateCalendarLiveV30AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V30_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V30_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV30SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v30-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV30ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v30 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV30SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v30-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V30_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV30Preflight(
    process.cwd(),
    verifyCalendarLiveV30Authorization(),
    attestationPath,
