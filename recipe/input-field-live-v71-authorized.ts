...
import { INPUT_LIVE_V71_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v71.js";
import { verifyInputLiveV71Authorization } from "./input-field-live-v71-authorization.js";
import { runInputLiveV71Preflight } from "./input-field-live-v71-preflight.js";
import { InputLiveV71Orchestrator } from "./run-input-field-live-v71.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V71_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v71-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV71Authorization(root);
    runInputLiveV71Preflight(
      root,
      authorization,
    ...
    const orchestrator = InputLiveV71Orchestrator.initialize({
      root,
  ...
  const orchestrator = InputLiveV71Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
