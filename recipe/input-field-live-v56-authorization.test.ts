...
import { INPUT_LIVE_V56_INDEX_PATH } from "./build-input-field-live-proof-v56.js";
import {
  INPUT_LIVE_V56_DYNAMIC_TOOL,
  INPUT_LIVE_V56_TARGET,
} from "./input-field-live-v56-broker.js";
import {
  buildInputLiveV56AuthorizationArtifact,
  validateInputLiveV56AntecedentIndex,
  validateInputLiveV56AuthorizationArtifact,
  validateInputLiveV56History,
  validateInputLiveV56UnitTestSource,
  type InputLiveV56AntecedentIndex,
  type InputLiveV56AuthorizationArtifact,
  type InputLiveV56AuthorizationProof,
  type InputLiveV56HistoryState,
} from "./input-field-live-v56-authorization.js";
import {
  inputLiveV56AttestationSha256,
  validateInputLiveV56ControlFlowSource,
  validateInputLiveV56SecurityAttestation,
  type InputLiveV56SecurityAttestation,
} from "./input-field-live-v56-preflight.js";
import { inputLiveV56Sha256 } from "./input-field-live-v56-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V56_INDEX_PATH);
const index = (): InputLiveV56AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV56AntecedentIndex;

const artifact = (): InputLiveV56AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV56AuthorizationArtifact({
...
const pendingState = (): InputLiveV56HistoryState => ({
  index: index(),
  indexSha256: inputLiveV56Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV56HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV56AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV56Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V56_TARGET,
  expectedDynamicTool: INPUT_LIVE_V56_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v56/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV56AuthorizationArtifact,
): InputLiveV56SecurityAttestation => {
  const body: Omit<InputLiveV56SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v56-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V56_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV56AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV56History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV56History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV56History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV56History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV56History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV56UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV56UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV56AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV56Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV56AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV56AuthorizationArtifact(
        planted as InputLiveV56AuthorizationArtifact,
        index(),
        inputLiveV56Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV56SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV56AttestationSha256(
      body as Omit<InputLiveV56SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV56SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV56ControlFlowSource(source), []);
});
