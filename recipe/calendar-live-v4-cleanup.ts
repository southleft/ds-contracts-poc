...
export interface CalendarLiveV4CleanupPlan {
  fileKey: string;
  fileName: string;
  editorType: string;
  namespace: string;
...
export function buildCalendarLiveV4CleanupRuntime(
  plan: CalendarLiveV4CleanupPlan,
): string {
  ...
  return String.raw`
if(figma.fileKey!==${JSON.stringify(plan.fileKey)}||figma.root.name!==${JSON.stringify(plan.fileName)}||figma.editorType!==${JSON.stringify(plan.editorType)})throw new Error("CALENDAR-V4-CLEANUP-WRONG-TARGET");
