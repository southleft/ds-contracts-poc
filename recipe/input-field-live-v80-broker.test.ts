...
import {
  ...
  createInputLiveV80FakeDynamicResponse,
  ...
  listInputLiveV80PersistedRequests,
  persistInputLiveV80RawResponse,
  persistInputLiveV80TechnicalGates,
  validateInputLiveV80Receipt,
  validateInputLiveV80Request,
  type InputLiveV80RemotePhase,
...
import {
  ...
  type InputLiveV80ExtractPayload,
  type InputLiveV80ProbePayload,
...
import {
  ...
  simulatedInputLiveV80Authorization,
} from "./run-input-field-live-v80.js";
...
const captures: InputLiveV80CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV80Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v80-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV80Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV80CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV80FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV80FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V80_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V80_TARGET);
  persistInputLiveV80RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV80Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV80Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV80WriterOwnership,
...
const rootProofs = (): InputLiveV80RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v80-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV80Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV80Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV80RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV80RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV80ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV80ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV80CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV80CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV80CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV80Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV80CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV80PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV80ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV80Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV80Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV80Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV80Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V80_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV80RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V80_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV80Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV80Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV80ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV80Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV80WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV80RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV80Request;
    validateInputLiveV80Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV80Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV80CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v80-state-"),
  );
  ...
  try {
    const orchestrator = InputLiveV80Orchestrator.initialize({
      ...
      privateKey,
      authorization: simulatedInputLiveV80Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV80Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V80_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV80RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V80_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV80Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV80Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV80RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V80_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV80Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV80Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV80Request;
