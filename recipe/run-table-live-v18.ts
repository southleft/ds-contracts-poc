...
import {
  acceptTableLiveV18Response,
  createTableLiveV18Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV18Request,
  persistTableLiveV18RawResponse,
  persistTableLiveV18TechnicalGates,
  readTableLiveV18AcceptedRawPayload,
  type TableLiveV18Request,
  type TableLiveV18TransactionAuthorization,
} from "./table-live-v18-broker.js";
import {
  TABLE_LIVE_V18_CAPTURE_COUNT,
  assertTableLiveV18CaptureResponses,
  ...
  evaluateTableLiveV18Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV18Roots,
  validateTableLiveV18CaptureManifest,
  validateTableLiveV18CapturePayload,
  validateTableLiveV18CleanupPayload,
  validateTableLiveV18ExtractPayload,
  validateTableLiveV18ProbePayload,
  validateTableLiveV18WriterPayload,
  type TableLiveV18CaptureCell,
  type TableLiveV18CapturePayload,
...
import {
  ...
  TABLE_LIVE_V18_PLAN_PATH,
  type TableLiveV18ProofPlan,
} from "./build-table-live-proof-v18.js";
import {
...
export const TABLE_LIVE_V18_RUNNER_VERSION = "table-live-v18-runner-v1";

interface TableLiveV18JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV18OrchestratorState {
  artifactVersion: "table-live-v18-orchestrator-state-v1";
  ...
  journal: TableLiveV18JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV18ProofPlan;
  planSha256: string;
  captures: TableLiveV18CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV18SourceIdentity & {
...
export interface TableLiveV18NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV18OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v18 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V18_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V18_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV18ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV18CaptureCell[];
  validateTableLiveV18CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v18 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v18 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV18WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV18RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV18Request =>
  readJson<TableLiveV18Request>(file);

export class TableLiveV18Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV18Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV18OrchestratorState = {
      artifactVersion: "table-live-v18-orchestrator-state-v1",
    ...
    return new TableLiveV18Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV18OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV18Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV18NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV18NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v18 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV18NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v18 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV18RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV18Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV18CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV18AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV18WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV18Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV18ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV18Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV18Response(this.#directory, "writer", {
        validate: validateTableLiveV18WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV18Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV18RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV18Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV18ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV18Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV18RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v18-host-root-proofs-v1",
      ...
      const next = issueTableLiveV18Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV18ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV18Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV18ProbePayload(payload, ownership),
      });
      persistTableLiveV18TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v18 capture cell missing");
      const accepted = acceptTableLiveV18Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV18CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V18_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV18CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV18Objective(
    ...
    throw new Error(`Table live v18 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV18Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV18CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV18Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV18RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV18Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV18CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV18CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV18CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV18Authorization = (
  privateKey: KeyObject,
): TableLiveV18TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV18SimulationKey = (): KeyObject =>
