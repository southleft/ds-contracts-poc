...
import { CALENDAR_LIVE_V7_INDEX_PATH } from "./build-calendar-live-proof-v7.js";
import {
...
import {
  buildCalendarLiveV7AuthorizationArtifact,
  validateCalendarLiveV7AntecedentIndex,
  validateCalendarLiveV7AuthorizationArtifact,
  validateCalendarLiveV7History,
  validateCalendarLiveV7UnitTestSource,
  type CalendarLiveV7AntecedentIndex,
  type CalendarLiveV7AuthorizationArtifact,
  type CalendarLiveV7AuthorizationProof,
  type CalendarLiveV7HistoryState,
} from "./calendar-live-v7-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV7ControlFlowSource,
  validateCalendarLiveV7SecurityAttestation,
  type CalendarLiveV7SecurityAttestation,
} from "./calendar-live-v7-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V7_INDEX_PATH);
const index = (): CalendarLiveV7AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV7AntecedentIndex;

const artifact = (): CalendarLiveV7AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV7AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV7HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV7HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV7AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v7/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV7AuthorizationArtifact,
): CalendarLiveV7SecurityAttestation => {
  const body: Omit<CalendarLiveV7SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V7_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV7History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV7History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV7History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV7History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV7History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV7UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v7-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV7UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV7AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV7AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV7AuthorizationArtifact(
        planted as CalendarLiveV7AuthorizationArtifact,
        index(),
...
test("one-root denominator, capture gate, cleanup, and prior-lineage reuse are rejected", () => {
  ...
  for (const mutate of [
    ...
    (value: Record<string, any>) => {
      value.execution.calendarLiveV6AuthorizationReusable = true;
    },
    (value: Record<string, any>) => {
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV7SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV7SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV7SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v7-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV7ControlFlowSource(source), []);
});
