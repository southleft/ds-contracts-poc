...
import { INPUT_LIVE_V22_INDEX_PATH } from "./build-input-field-live-proof-v22.js";
import {
  INPUT_LIVE_V22_DYNAMIC_TOOL,
  INPUT_LIVE_V22_TARGET,
} from "./input-field-live-v22-broker.js";
import {
  buildInputLiveV22AuthorizationArtifact,
  validateInputLiveV22AntecedentIndex,
  validateInputLiveV22AuthorizationArtifact,
  validateInputLiveV22History,
  validateInputLiveV22UnitTestSource,
  type InputLiveV22AntecedentIndex,
  type InputLiveV22AuthorizationArtifact,
  type InputLiveV22AuthorizationProof,
  type InputLiveV22HistoryState,
} from "./input-field-live-v22-authorization.js";
import {
  inputLiveV22AttestationSha256,
  validateInputLiveV22ControlFlowSource,
  validateInputLiveV22SecurityAttestation,
  type InputLiveV22SecurityAttestation,
} from "./input-field-live-v22-preflight.js";
import { inputLiveV22Sha256 } from "./input-field-live-v22-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V22_INDEX_PATH);
const index = (): InputLiveV22AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV22AntecedentIndex;

const artifact = (): InputLiveV22AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV22AuthorizationArtifact({
...
const pendingState = (): InputLiveV22HistoryState => ({
  index: index(),
  indexSha256: inputLiveV22Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV22HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV22AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV22Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V22_TARGET,
  expectedDynamicTool: INPUT_LIVE_V22_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v22/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV22AuthorizationArtifact,
): InputLiveV22SecurityAttestation => {
  const body: Omit<InputLiveV22SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v22-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V22_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV22AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV22History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV22History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV22History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV22History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV22History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV22UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV22UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV22AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV22Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV22AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV22AuthorizationArtifact(
        planted as InputLiveV22AuthorizationArtifact,
        index(),
        inputLiveV22Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV22SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV22AttestationSha256(
      body as Omit<InputLiveV22SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV22SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV22ControlFlowSource(source), []);
});
