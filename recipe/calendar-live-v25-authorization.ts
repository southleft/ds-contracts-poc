...
export type CalendarLiveV25HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV25AuthorizationArtifact {
  artifactVersion: "calendar-live-v25-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V25_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV25AntecedentIndex {
...
export interface CalendarLiveV25HistoryState {
  index?: CalendarLiveV25AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV25AuthorizationArtifact;
...
export interface CalendarLiveV25AuthorizationProof
  extends CalendarLiveV25TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V25_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V25_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V25_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V25_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v25-authorization") ||
  artifactPath.includes("calendar-live-v25-preflight") ||
  artifactPath.includes("calendar-live-v25-authorized") ||
  artifactPath.includes("create-calendar-live-v25-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v25-status") ||
...
export function validateCalendarLiveV25AntecedentIndex(
  index: CalendarLiveV25AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v25-antecedent-index-v1" ||
...
export function validateCalendarLiveV25UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV25History\(\s*["']/.test(source) ||
    /readCalendarLiveV25HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV25AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV25AuthorizationArtifact {
  ...
  ) as CalendarLiveV25AntecedentIndex;
  const indexFailures = validateCalendarLiveV25AntecedentIndex(index);
...
export function validateCalendarLiveV25AuthorizationArtifact(
  artifact: CalendarLiveV25AuthorizationArtifact | undefined,
  index: CalendarLiveV25AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v25-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v25" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V25_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V25_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V25_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V25_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V25_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V25_SOURCE_ROOTS ||
...
export function validateCalendarLiveV25History(
  state: CalendarLiveV25HistoryState,
  expected: CalendarLiveV25HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV25AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV25AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV25HistoryState(
  root = process.cwd(),
): CalendarLiveV25HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V25_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV25AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V25_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V25_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V25_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV25History(
  expected: CalendarLiveV25HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV25HistoryState {
  const state = readCalendarLiveV25HistoryState(root);
  const failures = validateCalendarLiveV25History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v25 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV25Authorization(
  root = process.cwd(),
): CalendarLiveV25AuthorizationProof {
  const state = verifyCalendarLiveV25History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V25_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v25/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v25.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V25_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V25_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V25_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV25HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV25History(expected);
  process.stdout.write(
    `Calendar live v25 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
