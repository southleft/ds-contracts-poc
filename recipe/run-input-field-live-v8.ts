...
import {
  acceptInputLiveV8Response,
  createInputLiveV8Transaction,
  inputLiveV8ReceiptPath,
  inputLiveV8RequestPath,
  inputLiveV8ResponsePath,
  inputLiveV8Sha256,
  issueInputLiveV8Request,
  persistInputLiveV8RawResponse,
  persistInputLiveV8TechnicalGates,
  readInputLiveV8AcceptedRawPayload,
  type InputLiveV8Request,
  type InputLiveV8TransactionAuthorization,
} from "./input-field-live-v8-broker.js";
import {
  INPUT_LIVE_V8_CAPTURE_COUNT,
  assertInputLiveV8CaptureResponses,
  assertInputLiveV8RootProofs,
  buildInputLiveV8CaptureProgram,
  buildInputLiveV8CleanupProgram,
  buildInputLiveV8ExtractProgram,
  buildInputLiveV8ProbeProgram,
  evaluateInputLiveV8Objective,
  inputLiveV8CaptureManifestSha256,
  proveInputLiveV8Roots,
  validateInputLiveV8CaptureManifest,
  validateInputLiveV8CapturePayload,
  validateInputLiveV8CleanupPayload,
  validateInputLiveV8ExtractPayload,
  validateInputLiveV8ProbePayload,
  validateInputLiveV8WriterPayload,
  type InputLiveV8CaptureCell,
  type InputLiveV8CapturePayload,
...
import {
  ...
  INPUT_LIVE_V8_PLAN_PATH,
  type InputLiveV8ProofPlan,
...
export const INPUT_LIVE_V8_RUNNER_VERSION = "input-live-v8-runner-v1";

interface InputLiveV8JournalEntry {
  index: number;
  event: string;
...
interface InputLiveV8OrchestratorState {
  artifactVersion: "input-live-v8-orchestrator-state-v1";
  ...
  journal: InputLiveV8JournalEntry[];
}

interface LoadedProofInputs {
  plan: InputLiveV8ProofPlan;
  planSha256: string;
  captures: InputLiveV8CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    InputLiveV8SourceIdentity & {
...
export interface InputLiveV8NextAction {
...
const writeState = (
  directory: string,
  state: InputLiveV8OrchestratorState,
): void => {
...
const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, INPUT_LIVE_V8_PLAN_PATH);
  const capturePath = path.join(root, INPUT_LIVE_V8_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as InputLiveV8ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as InputLiveV8CaptureCell[];
  validateInputLiveV8CaptureManifest(captures);
  if (
    plan.captureManifest.sha256 !== inputLiveV8Sha256(captureBytes) ||
    captureArtifact.cellsSha256 !== inputLiveV8CaptureManifestSha256(captures)
  )
    throw new TypeError("Input live v8 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Input live v8 unknown source ${source.source}`);
    const expectedBytes = readFileSync(
      path.join(root, source.expectedScenePlan.path),
    );
    const uncompressed = gunzipSync(expectedBytes);
    if (
      inputLiveV8Sha256(expectedBytes) !== source.expectedScenePlan.sha256 ||
      inputLiveV8Sha256(uncompressed) !==
        source.expectedScenePlan.uncompressedSha256
    )
      throw new TypeError(
        `Input live v8 expected plan drift: ${source.source}`,
      );
  ...
  return {
    plan,
    planSha256: inputLiveV8Sha256(planBytes),
    captures,
    captureManifestSha256: inputLiveV8Sha256(captureBytes),
    sources,
...
const appendJournal = (
  ...
  const payloadSha256 = inputLiveV8Sha256(canonicalJson(payload));
  const body = {
  ...
  const entry = {
    ...body,
    entrySha256: inputLiveV8Sha256(canonicalJson(body)),
  };
  writeNew(
    path.join(
      directory,
...
const writerOwnership = (directory: string): InputLiveV8WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): InputLiveV8RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): InputLiveV8Request =>
  readJson<InputLiveV8Request>(file);

export class InputLiveV8Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueInputLiveV8Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: InputLiveV8OrchestratorState = {
      artifactVersion: "input-live-v8-orchestrator-state-v1",
      transactionDirectory: options.transactionDirectory,
      activeRequest: inputLiveV8RequestPath(
        options.transactionDirectory,
        "writer",
      ),
      cleanupRequest: null,
    ...
    return new InputLiveV8Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<InputLiveV8OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new InputLiveV8Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): InputLiveV8NextAction {
    const cleanup =
      this.#state.cleanupRequest &&
      !this.#state.cleanupAccepted &&
      !existsSync(inputLiveV8ReceiptPath(this.#directory, "cleanup"))
        ? this.#state.cleanupRequest
        : null;
  ...
  ingestAndAdvance(rawResponse: string): InputLiveV8NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Input live v8 has no active main request");
  ...
  ingestCleanup(rawResponse: string): InputLiveV8NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Input live v8 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistInputLiveV8RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptInputLiveV8Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV8CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      existsSync(inputLiveV8ReceiptPath(this.#directory, "writer")) &&
      !existsSync(path.join(this.#directory, OWNERSHIP_FILE))
    ) {
      const raw = readInputLiveV8AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateInputLiveV8WriterPayload(raw),
      );
    }
    if (
      existsSync(path.join(this.#directory, OWNERSHIP_FILE)) &&
      !existsSync(inputLiveV8RequestPath(this.#directory, "cleanup"))
    )
      this.#issueCleanupAndExtract(false);
    if (existsSync(inputLiveV8ReceiptPath(this.#directory, "cleanup")))
      this.#state.cleanupAccepted = true;
    this.#persist();
  }

  #acceptActive(request: InputLiveV8Request): void {
    if (request.phase === "writer") {
      const accepted = acceptInputLiveV8Response(this.#directory, "writer", {
        validate: validateInputLiveV8WriterPayload,
      });
    ...
    if (request.phase === "extract") {
      const accepted = acceptInputLiveV8Response(this.#directory, "extract", {
        validate: (payload) =>
          validateInputLiveV8ExtractPayload(payload, ownership),
      });
      const proofs = proveInputLiveV8Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseInputFieldRecipe,
        compileInputFieldRecipe,
      );
      assertInputLiveV8RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "input-live-v8-host-root-proofs-v1",
      ...
      appendJournal(
        ...
        request.requestId,
        { proofsSha256: inputLiveV8Sha256(canonicalJson(proofs)) },
      );
      const next = issueInputLiveV8Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildInputLiveV8ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV8RequestPath(
        this.#directory,
        "probe",
      );
      appendJournal(
    ...
    if (request.phase === "probe") {
      const accepted = acceptInputLiveV8Response(this.#directory, "probe", {
        validate: (payload) =>
          validateInputLiveV8ProbePayload(payload, ownership),
      });
      persistInputLiveV8TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Input live v8 capture cell missing");
      const accepted = acceptInputLiveV8Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateInputLiveV8CapturePayload(payload, cell, rawBytes),
      });
      const png = Buffer.from(accepted.payload.pngBase64, "base64");
      const capturePath = path.join(
        this.#directory,
        "captures",
        `${String(cell.index).padStart(3, "0")}-${inputLiveV8Sha256(cell.cellKey).slice(0, 20)}.png`,
      );
      ...
      if (next < INPUT_LIVE_V8_CAPTURE_COUNT) this.#issueCapture(next);
      else {
          ...
          pngBase64: Buffer.from(
            readFileSync(
              path.join(
                this.#directory,
                "captures",
                `${String(response.index).padStart(3, "0")}-${inputLiveV8Sha256(response.cellKey).slice(0, 20)}.png`,
              ),
            ),
          ).toString("base64"),
        }));
        assertInputLiveV8CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateInputLiveV8Objective(
        ...
        appendJournal(
          ...
          request.requestId,
          {
            reportSha256: inputLiveV8Sha256(canonicalJson(objective)),
    ...
    throw new Error(`Input live v8 cleanup is not a main-lane request`);
  }

  #issueCleanupAndExtract(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueInputLiveV8Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildInputLiveV8CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = inputLiveV8RequestPath(
      this.#directory,
      "cleanup",
    );
    const extract = issueInputLiveV8Request(
      this.#directory,
      this.#privateKey,
      "extract",
      buildInputLiveV8ExtractProgram(ownership, this.#inputs.sources),
    );
    this.#state.activeRequest = inputLiveV8RequestPath(
      this.#directory,
      "extract",
  ...
  #issueCapture(index: number): void {
    const request = issueInputLiveV8Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildInputLiveV8CaptureProgram(
  ...
  #readCaptureResponseIndex(): InputLiveV8CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: InputLiveV8CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedInputLiveV8Authorization = (
  privateKey: KeyObject,
): InputLiveV8TransactionAuthorization => ({
  mode: "simulated",
  ...
  signingPublicKeySha256: inputLiveV8Sha256(
...
export const generateInputLiveV8SimulationKey = (): KeyObject =>
