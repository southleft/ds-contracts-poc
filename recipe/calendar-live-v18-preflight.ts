...
import {
  ...
  verifyCalendarLiveV18Authorization,
  type CalendarLiveV18AntecedentIndex,
  type CalendarLiveV18AuthorizationProof,
} from "./calendar-live-v18-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V18_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v18/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V18_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v18-security-attestation.json";

export interface CalendarLiveV18SecurityAttestation {
...
export interface CalendarLiveV18PreflightReport {
  artifactVersion: "calendar-live-v18-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV18SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV18SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV18AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v18-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V18_TARGET) ||
...
export function validateCalendarLiveV18ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV18Authorization");
  const preflight = source.indexOf("runCalendarLiveV18Preflight");
  const initialize = source.indexOf("CalendarLiveV18Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV18Preflight(
  root: string,
  proof: CalendarLiveV18AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V18_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV18PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V18_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V18_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV18AntecedentIndex;
  failures.push(...validateCalendarLiveV18AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V18_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V18_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v18-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV18ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v18 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV18SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v18-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V18_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV18Preflight(
    process.cwd(),
    verifyCalendarLiveV18Authorization(),
    attestationPath,
