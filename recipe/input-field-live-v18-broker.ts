...
import {
  ...
  INPUT_LIVE_V18_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V18_RESPONSE_CONTRACTS,
  type InputLiveV18RootProof,
} from "./input-field-live-v18-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V18_BROKER_VERSION = "input-live-v18-broker-v2";
export const INPUT_LIVE_V18_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V18_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V18_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V18_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V18_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V18_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV18RemotePhase =
  (typeof INPUT_LIVE_V18_REMOTE_PHASES)[number];
export type InputLiveV18AuthorizationMode = "live" | "simulated";

export interface InputLiveV18CommitPins {
...
export interface InputLiveV18TransactionAuthorization extends InputLiveV18CommitPins {
  mode: InputLiveV18AuthorizationMode;
...
export interface InputLiveV18Manifest {
  artifactVersion: "input-live-v18-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V18_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V18_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V18_TARGET;
  authorization: InputLiveV18TransactionAuthorization;
...
export interface InputLiveV18ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV18RequestBody {
  ...
  commitPins: InputLiveV18CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V18_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV18ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV18Request extends InputLiveV18RequestBody {
...
export interface InputLiveV18Receipt {
  artifactVersion: "input-live-v18-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V18_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV18RemotePhase;
...
export interface InputLiveV18AcceptedResponse<T = unknown> {
  request: InputLiveV18Request;
  receipt: InputLiveV18Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV18IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV18ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V18_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V18_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V18_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V18_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V18_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V18_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V18_TARGET.editorType;
};
...
export const inputLiveV18RequestSequence = (
  phase: InputLiveV18RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV18RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV18RequestPath = (
  directory: string,
  phase: InputLiveV18RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV18ResponsePath = (
  directory: string,
  phase: InputLiveV18RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV18ReceiptPath = (
  directory: string,
  phase: InputLiveV18RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV18Manifest => {
  const manifest = json<InputLiveV18Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v18-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V18_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V18_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV18RemotePhase,
  captureIndex?: number,
): InputLiveV18Request | undefined => {
  const file = inputLiveV18RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV18Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV18RemotePhase,
  captureIndex?: number,
): InputLiveV18Receipt | undefined => {
  const file = inputLiveV18ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV18Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV18Request | undefined;
  receipt: InputLiveV18Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV18RemotePhase,
): InputLiveV18ResponseContract => ({
  ...INPUT_LIVE_V18_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V18_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v18-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V18_BROKER_VERSION,
  target: INPUT_LIVE_V18_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V18_PROGRAM_START*/${program}/*INPUT_LIVE_V18_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V18_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V18_PROGRAM_END*/";
...
export function createInputLiveV18Transaction(
  directory: string,
  ...
  const manifest: InputLiveV18Manifest = {
    artifactVersion: "input-live-v18-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V18_BROKER_VERSION,
...
export function validateInputLiveV18Request(
  directory: string,
  request: InputLiveV18Request,
): void {
  ...
  const expectedSequence = inputLiveV18RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v18-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V18_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V18_TARGET.fileKey ||
...
export function issueInputLiveV18Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV18RemotePhase,
  program: string,
  options: InputLiveV18IssueOptions = {},
): InputLiveV18Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV18RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV18Request>(targetPath);
    validateInputLiveV18Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV18ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV18TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV18RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    ...
    timeout:
      ...
      (phase === "writer"
        ...
        : INPUT_LIVE_V18_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV18RequestBody = {
    artifactVersion: "input-live-v18-broker-request-v2",
    brokerVersion: INPUT_LIVE_V18_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV18Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV18Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV18RawResponse(
  directory: string,
  phase: InputLiveV18RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV18RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV18Request>(requestFile);
  if (
  ...
  const target = inputLiveV18ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV18Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v18-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V18_BROKER_VERSION,
...
export function acceptInputLiveV18Response<T = unknown>(
  directory: string,
  phase: InputLiveV18RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV18AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV18ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV18Request>(
    inputLiveV18RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV18Request(directory, request);
  if (phase === "capture") validateInputLiveV18TechnicalGates(directory);
  const rawFile = inputLiveV18ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v18-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V18_BROKER_VERSION ||
...
export function persistInputLiveV18TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV18RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV18RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V18_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v18-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V18_BROKER_VERSION,
...
export function validateInputLiveV18TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v18-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V18_TECHNICAL_GATES.some(
...
export function validateInputLiveV18Receipt(
  directory: string,
  phase: InputLiveV18RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV18Request(directory, request);
  const receipt = json<InputLiveV18Receipt>(
    inputLiveV18ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV18ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v18-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V18_BROKER_VERSION ||
...
export function readInputLiveV18AcceptedRawPayload(
  directory: string,
  phase: InputLiveV18RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV18Receipt(directory, phase, captureIndex);
...
export function createInputLiveV18FakeDynamicResponse(
  request: InputLiveV18Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V18_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V18_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV18PersistedRequests(
  directory: string,
): InputLiveV18Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV18Request>(file)] : [];
    });
}

export const inputLiveV18Sha256 = sha256;
