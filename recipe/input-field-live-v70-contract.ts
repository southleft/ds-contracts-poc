...
} from "./input-field-live-v3-verifier-v70.js";
import {
  buildInputLiveV70RawPropertyRuntime,
  normalizeInputLiveV70Scene,
  type InputLiveV70RawNode,
} from "./input-field-live-v70-verifier.js";
...
export {
  ...
  type InputLiveV70RestorePayload,
} from "./input-field-live-v70-restore.js";

export const INPUT_LIVE_V70_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V70_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V70_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V70_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V70_VARIANT_COUNT = 256;
export const INPUT_LIVE_V70_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V70_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV70SourceId = (typeof INPUT_LIVE_V70_SOURCE_IDS)[number];

export interface InputLiveV70SourceIdentity {
  source: InputLiveV70SourceId;
...
export interface InputLiveV70WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV70ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV70SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV70RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV70RootProof {
  source: InputLiveV70SourceId;
  adapterIdentity: string;
...
export interface InputLiveV70ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV70SourceId;
    adapterIdentity: string;
...
export interface InputLiveV70CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV70SourceId;
  adapterIdentity: string;
...
export interface InputLiveV70CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV70SourceId;
  strata: InputLiveV70CaptureCell["strata"];
...
export interface InputLiveV70CleanupPayload {
...
export function validateInputLiveV70WriterPayload(
  value: unknown,
): InputLiveV70WriterOwnership {
...
const identityPlan = (source: InputLiveV70SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV70ExtractProgram(
  writer: InputLiveV70WriterOwnership,
  sources: readonly InputLiveV70SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V70_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V70_NAMESPACE)}
${buildInputLiveV70RawPropertyRuntime()}
...
export function validateInputLiveV70ExtractPayload(
  value: unknown,
  writer: InputLiveV70WriterOwnership,
): InputLiveV70ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V70_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV70ExtractPayload;
}

export function proveInputLiveV70Roots<Instance>(
  extract: InputLiveV70ExtractPayload,
  sources: readonly (InputLiveV70SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV70Scene(
      root.scene,
...
export function assertInputLiveV70RootProofs(
  proofs: readonly InputLiveV70RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV70ProbeProgram(
  writer: InputLiveV70WriterOwnership,
  sources: readonly InputLiveV70SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V70_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV70ProbePayload(
  value: unknown,
  writer: InputLiveV70WriterOwnership,
): InputLiveV70ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV70ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V70_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V70_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV70CaptureManifest(
  cells: readonly InputLiveV70CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V70_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V70_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V70_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V70_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V70_SOURCE_IDS.some(
...
export const inputLiveV70CaptureManifestSha256 = (
  cells: readonly InputLiveV70CaptureCell[],
): string => {
  validateInputLiveV70CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV70CaptureProgram(
  writer: InputLiveV70WriterOwnership,
  cell: InputLiveV70CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V70_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V70_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V70_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV70CapturePayload(
  value: unknown,
  cell: InputLiveV70CaptureCell,
  rawResponseBytes: number,
): InputLiveV70CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV70CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V70_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V70_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV70CaptureResponses(
  manifest: readonly InputLiveV70CaptureCell[],
  responses: readonly InputLiveV70CapturePayload[],
): void {
  validateInputLiveV70CaptureManifest(manifest);
  if (
...
export interface InputLiveV70ObjectiveReport {
  artifactVersion: "input-live-v70-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV70Objective(
  manifest: readonly InputLiveV70CaptureCell[],
  responses: readonly InputLiveV70CapturePayload[],
  readReference: (cell: InputLiveV70CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV70CaptureCell) => Buffer,
): InputLiveV70ObjectiveReport {
  assertInputLiveV70CaptureResponses(manifest, responses);
...
export function buildInputLiveV70CleanupProgram(
  writer: InputLiveV70WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V70_NAMESPACE,
...
export function validateInputLiveV70CleanupPayload(
  value: unknown,
  writer: InputLiveV70WriterOwnership,
): InputLiveV70CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV70CleanupPayload;
...
export const INPUT_LIVE_V70_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV70WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
