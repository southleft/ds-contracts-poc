...
export const CALENDAR_LIVE_V1_NAV_BINDING_COMPILE_ORDER_MARKER =
  "CALENDAR-HOST-NAV-BINDING-COMPILE-ORDER";
export const CALENDAR_LIVE_V1_NAV_ICON_BINDING_COMPILE_ORDER_MARKER =
...
export const CALENDAR_LIVE_V1_DAY_BUTTON_CLIPS_CONTENT_OMITTED_MARKER =
...
export const CALENDAR_LIVE_V1_DAY_VARIANT_STROKES_OMITTED_MARKER =
...
export const CALENDAR_LIVE_V1_DAY_VARIANT_CORNER_RADIUS_CARRIED_MARKER =
...
const NAV_BUTTON_ROLE = /^calendar\/nav\/(?:previous|next)$/;
const NAV_COMPILE_BINDING_FIELDS = [
  "layout.width.value",
  "layout.height.value",
] as const;
const NAV_ICON_ROLE = /^calendar\/nav\/(?:previous|next)\/icon$/;
...
const omitNavClipsContent = <T extends { clipsContent?: unknown }>(
  scene: SceneNodeSnapshot,
  frame: T,
): T => {
  void CALENDAR_LIVE_V1_NAV_CLIPS_CONTENT_OMITTED_MARKER;
  const role = sceneRole(scene);
...
const omitDayButtonClipsContent = <T extends { clipsContent?: unknown }>(
  scene: SceneNodeSnapshot,
  frame: T,
): T => {
  void CALENDAR_LIVE_V1_DAY_BUTTON_CLIPS_CONTENT_OMITTED_MARKER;
  const role = sceneRole(scene);
...
const omitDayButtonEffects = <T extends { effects?: unknown }>(
  scene: SceneNodeSnapshot,
  frame: T,
): T => {
  void CALENDAR_LIVE_V1_DAY_BUTTON_EFFECTS_OMITTED_MARKER;
  const role = sceneRole(scene);
  if (role !== "calendar/day/button" || scene.type !== "FRAME") return frame;
  if (frame.effects === undefined) return frame;
  const { effects: _omitted, ...rest } = frame;
...
const omitHeaderBodyCornerRadius = <T extends { cornerRadius?: unknown }>(
  ...
  void CALENDAR_LIVE_V1_DAY_VARIANT_CORNER_RADIUS_CARRIED_MARKER;
  const role = sceneRole(scene);
  const headerBody =
    !!role && HEADER_BODY_ROLES.has(role) && scene.type === "FRAME";
  const tableVariant =
...
const omitDayVariantStrokes = <T extends { strokes?: unknown }>(
  scene: SceneNodeSnapshot,
  frame: T,
): T => {
...
export function sceneToNormalizedIr(
  ...
  if (
    ...
    frame = omitDayButtonClipsContent(scene, frame);
    ...
    frame = omitDayButtonEffects(scene, frame);
