...
import { COMBOBOX_LIVE_V9_INDEX_PATH } from "./build-combobox-live-proof-v9.js";
import {
...
import {
  ...
  validateComboboxLiveV9History,
  validateComboboxLiveV9UnitTestSource,
  type ComboboxLiveV9AntecedentIndex,
  type ComboboxLiveV9AuthorizationArtifact,
  type ComboboxLiveV9AuthorizationProof,
  type ComboboxLiveV9HistoryState,
} from "./combobox-live-v9-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV9ControlFlowSource,
  validateComboboxLiveV9SecurityAttestation,
  type ComboboxLiveV9SecurityAttestation,
} from "./combobox-live-v9-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v9-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V9_INDEX_PATH);
const index = (): ComboboxLiveV9AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV9AntecedentIndex;

const artifact = (): ComboboxLiveV9AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV9AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV9HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV9HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV9AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v9/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV9AuthorizationArtifact,
): ComboboxLiveV9SecurityAttestation => {
  const body: Omit<ComboboxLiveV9SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V9_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV9History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV9History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV9History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV9History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV9History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV9UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV9UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV9AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV9AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV9AuthorizationArtifact(
        planted as ComboboxLiveV9AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV9SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV9SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV9SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV9ControlFlowSource(source), []);
});
