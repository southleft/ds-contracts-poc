...
import { CALENDAR_LIVE_V15_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v15.js";
import { verifyCalendarLiveV15Authorization } from "./calendar-live-v15-authorization.js";
import { runCalendarLiveV15Preflight } from "./calendar-live-v15-preflight.js";
import { CalendarLiveV15Orchestrator } from "./run-calendar-live-v15.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v15 requires exact authorization artifact ${CALENDAR_LIVE_V15_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v15-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV15Authorization(root);
    runCalendarLiveV15Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV15Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV15Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
