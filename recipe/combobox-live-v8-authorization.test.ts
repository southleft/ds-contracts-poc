...
import { COMBOBOX_LIVE_V8_INDEX_PATH } from "./build-combobox-live-proof-v8.js";
import {
  COMBOBOX_LIVE_V8_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V8_TARGET,
} from "./combobox-live-v8-broker.js";
import {
  buildComboboxLiveV8AuthorizationArtifact,
  validateComboboxLiveV8AntecedentIndex,
  validateComboboxLiveV8AuthorizationArtifact,
  validateComboboxLiveV8History,
  validateComboboxLiveV8UnitTestSource,
  type ComboboxLiveV8AntecedentIndex,
  type ComboboxLiveV8AuthorizationArtifact,
  type ComboboxLiveV8AuthorizationProof,
  type ComboboxLiveV8HistoryState,
} from "./combobox-live-v8-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV8ControlFlowSource,
  validateComboboxLiveV8SecurityAttestation,
  type ComboboxLiveV8SecurityAttestation,
} from "./combobox-live-v8-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v8-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V8_INDEX_PATH);
const index = (): ComboboxLiveV8AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV8AntecedentIndex;

const artifact = (): ComboboxLiveV8AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV8AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV8HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV8HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV8AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v8/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV8AuthorizationArtifact,
): ComboboxLiveV8SecurityAttestation => {
  const body: Omit<ComboboxLiveV8SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V8_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV8History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV8History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV8History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV8History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV8History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV8UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV8UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV8AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV8AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV8AuthorizationArtifact(
        planted as ComboboxLiveV8AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV8SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV8SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV8SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV8ControlFlowSource(source), []);
});
