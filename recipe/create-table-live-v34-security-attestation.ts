...
import { verifyTableLiveV34Authorization } from "./table-live-v34-authorization.js";
import { TABLE_LIVE_V34_TARGET } from "./table-live-v34-broker.js";
import {
  TABLE_LIVE_V34_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV34Authorization();
...
const attestation: TableLiveV34SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV34SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v34 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V34_SECURITY_ATTESTATION_DEFAULT_PATH,
);
