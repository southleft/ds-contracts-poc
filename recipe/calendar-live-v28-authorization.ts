...
export type CalendarLiveV28HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV28AuthorizationArtifact {
  artifactVersion: "calendar-live-v28-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V28_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV28AntecedentIndex {
...
export interface CalendarLiveV28HistoryState {
  index?: CalendarLiveV28AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV28AuthorizationArtifact;
...
export interface CalendarLiveV28AuthorizationProof
  extends CalendarLiveV28TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V28_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V28_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V28_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V28_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v28-authorization") ||
  artifactPath.includes("calendar-live-v28-preflight") ||
  artifactPath.includes("calendar-live-v28-authorized") ||
  artifactPath.includes("create-calendar-live-v28-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v28-status") ||
...
export function validateCalendarLiveV28AntecedentIndex(
  index: CalendarLiveV28AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v28-antecedent-index-v1" ||
...
export function validateCalendarLiveV28UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV28History\(\s*["']/.test(source) ||
    /readCalendarLiveV28HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV28AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV28AuthorizationArtifact {
  ...
  ) as CalendarLiveV28AntecedentIndex;
  const indexFailures = validateCalendarLiveV28AntecedentIndex(index);
...
export function validateCalendarLiveV28AuthorizationArtifact(
  artifact: CalendarLiveV28AuthorizationArtifact | undefined,
  index: CalendarLiveV28AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v28-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v28" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V28_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V28_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V28_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V28_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V28_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V28_SOURCE_ROOTS ||
...
export function validateCalendarLiveV28History(
  state: CalendarLiveV28HistoryState,
  expected: CalendarLiveV28HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV28AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV28AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV28HistoryState(
  root = process.cwd(),
): CalendarLiveV28HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V28_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV28AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V28_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V28_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V28_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV28History(
  expected: CalendarLiveV28HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV28HistoryState {
  const state = readCalendarLiveV28HistoryState(root);
  const failures = validateCalendarLiveV28History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v28 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV28Authorization(
  root = process.cwd(),
): CalendarLiveV28AuthorizationProof {
  const state = verifyCalendarLiveV28History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V28_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v28/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v28.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V28_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V28_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V28_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV28HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV28History(expected);
  process.stdout.write(
    `Calendar live v28 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
