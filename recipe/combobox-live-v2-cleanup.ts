...
export interface ComboboxLiveV2CleanupPlan {
  fileKey: string;
  fileName: string;
  editorType: string;
  namespace: string;
...
export function buildComboboxLiveV2CleanupRuntime(
  plan: ComboboxLiveV2CleanupPlan,
): string {
  ...
  return String.raw`
if(figma.fileKey!==${JSON.stringify(plan.fileKey)}||figma.root.name!==${JSON.stringify(plan.fileName)}||figma.editorType!==${JSON.stringify(plan.editorType)})throw new Error("COMBOBOX-V2-CLEANUP-WRONG-TARGET");
