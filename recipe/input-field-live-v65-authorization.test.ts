...
import { INPUT_LIVE_V65_INDEX_PATH } from "./build-input-field-live-proof-v65.js";
import {
  INPUT_LIVE_V65_DYNAMIC_TOOL,
  INPUT_LIVE_V65_TARGET,
} from "./input-field-live-v65-broker.js";
import {
  buildInputLiveV65AuthorizationArtifact,
  validateInputLiveV65AntecedentIndex,
  validateInputLiveV65AuthorizationArtifact,
  validateInputLiveV65History,
  validateInputLiveV65UnitTestSource,
  type InputLiveV65AntecedentIndex,
  type InputLiveV65AuthorizationArtifact,
  type InputLiveV65AuthorizationProof,
  type InputLiveV65HistoryState,
} from "./input-field-live-v65-authorization.js";
import {
  inputLiveV65AttestationSha256,
  validateInputLiveV65ControlFlowSource,
  validateInputLiveV65SecurityAttestation,
  type InputLiveV65SecurityAttestation,
} from "./input-field-live-v65-preflight.js";
import { inputLiveV65Sha256 } from "./input-field-live-v65-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V65_INDEX_PATH);
const index = (): InputLiveV65AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV65AntecedentIndex;

const artifact = (): InputLiveV65AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV65AuthorizationArtifact({
...
const pendingState = (): InputLiveV65HistoryState => ({
  index: index(),
  indexSha256: inputLiveV65Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV65HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV65AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV65Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V65_TARGET,
  expectedDynamicTool: INPUT_LIVE_V65_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v65/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV65AuthorizationArtifact,
): InputLiveV65SecurityAttestation => {
  const body: Omit<InputLiveV65SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v65-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V65_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV65AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV65History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV65History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV65History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV65History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV65History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV65UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV65UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV65AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV65Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV65AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV65AuthorizationArtifact(
        planted as InputLiveV65AuthorizationArtifact,
        index(),
        inputLiveV65Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV65SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV65AttestationSha256(
      body as Omit<InputLiveV65SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV65SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV65ControlFlowSource(source), []);
});
