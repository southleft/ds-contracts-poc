...
import {
  acceptComboboxLiveV14Response,
  createComboboxLiveV14Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV14Request,
  persistComboboxLiveV14RawResponse,
  persistComboboxLiveV14TechnicalGates,
  readComboboxLiveV14AcceptedRawPayload,
  type ComboboxLiveV14Request,
  type ComboboxLiveV14TransactionAuthorization,
} from "./combobox-live-v14-broker.js";
import {
  COMBOBOX_LIVE_V14_CAPTURE_COUNT,
  assertComboboxLiveV14CaptureResponses,
  ...
  evaluateComboboxLiveV14Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV14Roots,
  validateComboboxLiveV14CaptureManifest,
  validateComboboxLiveV14CapturePayload,
  validateComboboxLiveV14CleanupPayload,
  validateComboboxLiveV14ExtractPayload,
  validateComboboxLiveV14ProbePayload,
  validateComboboxLiveV14WriterPayload,
  type ComboboxLiveV14CaptureCell,
  type ComboboxLiveV14CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V14_PLAN_PATH,
  type ComboboxLiveV14ProofPlan,
} from "./build-combobox-live-proof-v14.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v14.js";

export const COMBOBOX_LIVE_V14_RUNNER_VERSION = "combobox-live-v14-runner-v1";

interface ComboboxLiveV14JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV14OrchestratorState {
  artifactVersion: "combobox-live-v14-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV14JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV14ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV14CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV14SourceIdentity & {
...
export interface ComboboxLiveV14NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV14OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v14 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V14_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V14_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV14ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV14CaptureCell[];
  validateComboboxLiveV14CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v14 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v14 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV14WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV14RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV14Request =>
  readJson<ComboboxLiveV14Request>(file);

export class ComboboxLiveV14Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV14Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV14OrchestratorState = {
      artifactVersion: "combobox-live-v14-orchestrator-state-v1",
    ...
    return new ComboboxLiveV14Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV14OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV14Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV14NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV14NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v14 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV14NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v14 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV14RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV14Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV14CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV14AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV14WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV14Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV14ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV14Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV14Response(this.#directory, "writer", {
        validate: validateComboboxLiveV14WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV14Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV14RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV14Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV14ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV14Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV14RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v14-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV14Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV14ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV14Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV14ProbePayload(payload, ownership),
      });
      persistComboboxLiveV14TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v14 capture cell missing");
      const accepted = acceptComboboxLiveV14Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV14CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V14_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV14CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV14Objective(
    ...
    throw new Error(`Combobox live v14 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV14Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV14CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV14Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV14RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV14Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV14CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV14CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV14CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV14Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV14TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV14SimulationKey = (): KeyObject =>
