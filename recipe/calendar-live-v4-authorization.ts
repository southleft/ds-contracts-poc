...
export type CalendarLiveV4HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV4AuthorizationArtifact {
  artifactVersion: "calendar-live-v4-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V4_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV4AntecedentIndex {
...
export interface CalendarLiveV4HistoryState {
  index?: CalendarLiveV4AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV4AuthorizationArtifact;
...
export interface CalendarLiveV4AuthorizationProof
  extends CalendarLiveV4TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V4_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V4_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V4_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V4_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v4-authorization") ||
  artifactPath.includes("calendar-live-v4-preflight") ||
  artifactPath.includes("calendar-live-v4-authorized") ||
  artifactPath.includes("create-calendar-live-v4-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v4-status") ||
...
export function validateCalendarLiveV4AntecedentIndex(
  index: CalendarLiveV4AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v4-antecedent-index-v1" ||
...
export function validateCalendarLiveV4UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV4History\(\s*["']/.test(source) ||
    /readCalendarLiveV4HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV4AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV4AuthorizationArtifact {
  ...
  ) as CalendarLiveV4AntecedentIndex;
  const indexFailures = validateCalendarLiveV4AntecedentIndex(index);
...
export function validateCalendarLiveV4AuthorizationArtifact(
  artifact: CalendarLiveV4AuthorizationArtifact | undefined,
  index: CalendarLiveV4AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v4-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v4" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V4_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V4_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V4_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V4_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V4_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V4_SOURCE_ROOTS ||
...
export function validateCalendarLiveV4History(
  state: CalendarLiveV4HistoryState,
  expected: CalendarLiveV4HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV4AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV4AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV4HistoryState(
  root = process.cwd(),
): CalendarLiveV4HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V4_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV4AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V4_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V4_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V4_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV4History(
  expected: CalendarLiveV4HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV4HistoryState {
  const state = readCalendarLiveV4HistoryState(root);
  const failures = validateCalendarLiveV4History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v4 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV4Authorization(
  root = process.cwd(),
): CalendarLiveV4AuthorizationProof {
  const state = verifyCalendarLiveV4History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V4_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v4/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v4.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V4_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V4_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V4_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV4HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV4History(expected);
  process.stdout.write(
    `Calendar live v4 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
