...
import { TABLE_LIVE_V25_AUTHORIZATION_PATH } from "./build-table-live-proof-v25.js";
import { verifyTableLiveV25Authorization } from "./table-live-v25-authorization.js";
import { runTableLiveV25Preflight } from "./table-live-v25-preflight.js";
import { TableLiveV25Orchestrator } from "./run-table-live-v25.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v25 requires exact authorization artifact ${TABLE_LIVE_V25_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v25-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV25Authorization(root);
    runTableLiveV25Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV25Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV25Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
