...
export const INPUT_LIVE_V42_VERIFIER_VERSION = "input-live-v42-verifier-v1";

export function buildInputLiveV42RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV42RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV42RawNode[];
}

export interface InputLiveV42NormalizedScene {
...
export interface InputLiveV42SceneProof {
...
export function normalizeInputLiveV42Scene(
  root: InputLiveV42RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV42NormalizedScene {
  const canonicalBindings: InputLiveV42NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV42RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV42Scene<Instance>(
  normalized: InputLiveV42NormalizedScene,
...
export function compileInputLiveV42ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV42PreCaptureGates(payload: unknown): void {
