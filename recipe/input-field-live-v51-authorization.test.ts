...
import { INPUT_LIVE_V51_INDEX_PATH } from "./build-input-field-live-proof-v51.js";
import {
  INPUT_LIVE_V51_DYNAMIC_TOOL,
  INPUT_LIVE_V51_TARGET,
} from "./input-field-live-v51-broker.js";
import {
  buildInputLiveV51AuthorizationArtifact,
  validateInputLiveV51AntecedentIndex,
  validateInputLiveV51AuthorizationArtifact,
  validateInputLiveV51History,
  validateInputLiveV51UnitTestSource,
  type InputLiveV51AntecedentIndex,
  type InputLiveV51AuthorizationArtifact,
  type InputLiveV51AuthorizationProof,
  type InputLiveV51HistoryState,
} from "./input-field-live-v51-authorization.js";
import {
  inputLiveV51AttestationSha256,
  validateInputLiveV51ControlFlowSource,
  validateInputLiveV51SecurityAttestation,
  type InputLiveV51SecurityAttestation,
} from "./input-field-live-v51-preflight.js";
import { inputLiveV51Sha256 } from "./input-field-live-v51-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V51_INDEX_PATH);
const index = (): InputLiveV51AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV51AntecedentIndex;

const artifact = (): InputLiveV51AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV51AuthorizationArtifact({
...
const pendingState = (): InputLiveV51HistoryState => ({
  index: index(),
  indexSha256: inputLiveV51Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV51HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV51AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV51Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V51_TARGET,
  expectedDynamicTool: INPUT_LIVE_V51_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v51/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV51AuthorizationArtifact,
): InputLiveV51SecurityAttestation => {
  const body: Omit<InputLiveV51SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v51-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V51_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV51AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV51History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV51History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV51History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV51History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV51History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV51UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV51UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV51AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV51Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV51AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV51AuthorizationArtifact(
        planted as InputLiveV51AuthorizationArtifact,
        index(),
        inputLiveV51Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV51SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV51AttestationSha256(
      body as Omit<InputLiveV51SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV51SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV51ControlFlowSource(source), []);
});
