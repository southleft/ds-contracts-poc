...
import { INPUT_LIVE_V74_INDEX_PATH } from "./build-input-field-live-proof-v74.js";
import {
  INPUT_LIVE_V74_DYNAMIC_TOOL,
  INPUT_LIVE_V74_TARGET,
} from "./input-field-live-v74-broker.js";
import {
  buildInputLiveV74AuthorizationArtifact,
  validateInputLiveV74AntecedentIndex,
  validateInputLiveV74AuthorizationArtifact,
  validateInputLiveV74History,
  validateInputLiveV74UnitTestSource,
  type InputLiveV74AntecedentIndex,
  type InputLiveV74AuthorizationArtifact,
  type InputLiveV74AuthorizationProof,
  type InputLiveV74HistoryState,
} from "./input-field-live-v74-authorization.js";
import {
  inputLiveV74AttestationSha256,
  validateInputLiveV74ControlFlowSource,
  validateInputLiveV74SecurityAttestation,
  type InputLiveV74SecurityAttestation,
} from "./input-field-live-v74-preflight.js";
import { inputLiveV74Sha256 } from "./input-field-live-v74-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V74_INDEX_PATH);
const index = (): InputLiveV74AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV74AntecedentIndex;

const artifact = (): InputLiveV74AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV74AuthorizationArtifact({
...
const pendingState = (): InputLiveV74HistoryState => ({
  index: index(),
  indexSha256: inputLiveV74Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV74HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV74AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV74Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V74_TARGET,
  expectedDynamicTool: INPUT_LIVE_V74_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v74/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV74AuthorizationArtifact,
): InputLiveV74SecurityAttestation => {
  const body: Omit<InputLiveV74SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v74-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V74_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV74AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV74History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV74History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV74History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV74History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV74History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV74UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV74UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV74AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV74Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV74AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV74AuthorizationArtifact(
        planted as InputLiveV74AuthorizationArtifact,
        index(),
        inputLiveV74Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV74SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV74AttestationSha256(
      body as Omit<InputLiveV74SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV74SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV74ControlFlowSource(source), []);
});
