...
import { INPUT_LIVE_V58_INDEX_PATH } from "./build-input-field-live-proof-v58.js";
import {
  INPUT_LIVE_V58_DYNAMIC_TOOL,
  INPUT_LIVE_V58_TARGET,
} from "./input-field-live-v58-broker.js";
import {
  buildInputLiveV58AuthorizationArtifact,
  validateInputLiveV58AntecedentIndex,
  validateInputLiveV58AuthorizationArtifact,
  validateInputLiveV58History,
  validateInputLiveV58UnitTestSource,
  type InputLiveV58AntecedentIndex,
  type InputLiveV58AuthorizationArtifact,
  type InputLiveV58AuthorizationProof,
  type InputLiveV58HistoryState,
} from "./input-field-live-v58-authorization.js";
import {
  inputLiveV58AttestationSha256,
  validateInputLiveV58ControlFlowSource,
  validateInputLiveV58SecurityAttestation,
  type InputLiveV58SecurityAttestation,
} from "./input-field-live-v58-preflight.js";
import { inputLiveV58Sha256 } from "./input-field-live-v58-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V58_INDEX_PATH);
const index = (): InputLiveV58AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV58AntecedentIndex;

const artifact = (): InputLiveV58AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV58AuthorizationArtifact({
...
const pendingState = (): InputLiveV58HistoryState => ({
  index: index(),
  indexSha256: inputLiveV58Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV58HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV58AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV58Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V58_TARGET,
  expectedDynamicTool: INPUT_LIVE_V58_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v58/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV58AuthorizationArtifact,
): InputLiveV58SecurityAttestation => {
  const body: Omit<InputLiveV58SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v58-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V58_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV58AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV58History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV58History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV58History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV58History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV58History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV58UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV58UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV58AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV58Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV58AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV58AuthorizationArtifact(
        planted as InputLiveV58AuthorizationArtifact,
        index(),
        inputLiveV58Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV58SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV58AttestationSha256(
      body as Omit<InputLiveV58SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV58SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV58ControlFlowSource(source), []);
});
