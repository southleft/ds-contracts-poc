...
import {
  assertInputLiveV36RootProofs,
  INPUT_LIVE_V36_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V36_RESPONSE_CONTRACTS,
  type InputLiveV36RootProof,
} from "./input-field-live-v36-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V36_BROKER_VERSION = "input-live-v36-broker-v2";
export const INPUT_LIVE_V36_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V36_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V36_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V36_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V36_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V36_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV36RemotePhase =
  (typeof INPUT_LIVE_V36_REMOTE_PHASES)[number];
export type InputLiveV36AuthorizationMode = "live" | "simulated";

export interface InputLiveV36CommitPins {
...
export interface InputLiveV36TransactionAuthorization extends InputLiveV36CommitPins {
  mode: InputLiveV36AuthorizationMode;
...
export interface InputLiveV36Manifest {
  artifactVersion: "input-live-v36-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V36_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V36_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V36_TARGET;
  authorization: InputLiveV36TransactionAuthorization;
...
export interface InputLiveV36ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV36RequestBody {
  artifactVersion: "input-live-v36-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V36_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV36RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV36CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V36_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV36ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV36Request extends InputLiveV36RequestBody {
...
export interface InputLiveV36Receipt {
  artifactVersion: "input-live-v36-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V36_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV36RemotePhase;
...
export interface InputLiveV36AcceptedResponse<T = unknown> {
  request: InputLiveV36Request;
  receipt: InputLiveV36Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV36IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV36ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V36_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V36_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V36_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V36_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V36_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V36_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V36_TARGET.editorType;
};
...
export const inputLiveV36RequestSequence = (
  phase: InputLiveV36RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV36RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV36RequestPath = (
  directory: string,
  phase: InputLiveV36RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV36ResponsePath = (
  directory: string,
  phase: InputLiveV36RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV36ReceiptPath = (
  directory: string,
  phase: InputLiveV36RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV36Manifest => {
  const manifest = json<InputLiveV36Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v36-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V36_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V36_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV36RemotePhase,
  captureIndex?: number,
): InputLiveV36Request | undefined => {
  const file = inputLiveV36RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV36Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV36RemotePhase,
  captureIndex?: number,
): InputLiveV36Receipt | undefined => {
  const file = inputLiveV36ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV36Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV36Request | undefined;
  receipt: InputLiveV36Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV36RemotePhase,
): InputLiveV36ResponseContract => ({
  ...INPUT_LIVE_V36_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V36_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V36_DYNAMIC_TOOL,
      target: INPUT_LIVE_V36_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v36-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V36_BROKER_VERSION,
  target: INPUT_LIVE_V36_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V36_PROGRAM_START*/${program}/*INPUT_LIVE_V36_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V36_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V36_PROGRAM_END*/";
...
export function createInputLiveV36Transaction(
  directory: string,
  ...
  const manifest: InputLiveV36Manifest = {
    artifactVersion: "input-live-v36-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V36_BROKER_VERSION,
...
export function validateInputLiveV36Request(
  directory: string,
  request: InputLiveV36Request,
): void {
  ...
  const expectedSequence = inputLiveV36RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v36-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V36_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V36_TARGET.fileKey ||
...
export function issueInputLiveV36Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV36RemotePhase,
  program: string,
  options: InputLiveV36IssueOptions = {},
): InputLiveV36Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV36RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV36Request>(targetPath);
    validateInputLiveV36Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV36ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV36TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV36RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V36_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V36_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V36_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV36RequestBody = {
    artifactVersion: "input-live-v36-broker-request-v2",
    brokerVersion: INPUT_LIVE_V36_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV36Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV36Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV36RawResponse(
  directory: string,
  phase: InputLiveV36RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV36RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV36Request>(requestFile);
  if (
  ...
  const target = inputLiveV36ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV36Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v36-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V36_BROKER_VERSION,
...
export function acceptInputLiveV36Response<T = unknown>(
  directory: string,
  phase: InputLiveV36RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV36AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV36ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV36Request>(
    inputLiveV36RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV36Request(directory, request);
  if (phase === "capture") validateInputLiveV36TechnicalGates(directory);
  const rawFile = inputLiveV36ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v36-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V36_BROKER_VERSION ||
...
export function persistInputLiveV36TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV36RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV36RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V36_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v36-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V36_BROKER_VERSION,
...
export function validateInputLiveV36TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v36-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V36_TECHNICAL_GATES.some(
...
export function validateInputLiveV36Receipt(
  directory: string,
  phase: InputLiveV36RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV36Request(directory, request);
  const receipt = json<InputLiveV36Receipt>(
    inputLiveV36ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV36ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v36-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V36_BROKER_VERSION ||
...
export function readInputLiveV36AcceptedRawPayload(
  directory: string,
  phase: InputLiveV36RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV36Receipt(directory, phase, captureIndex);
...
export function createInputLiveV36FakeDynamicResponse(
  request: InputLiveV36Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V36_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V36_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV36PersistedRequests(
  directory: string,
): InputLiveV36Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV36Request>(file)] : [];
    });
}

export const inputLiveV36Sha256 = sha256;
