...
import { INPUT_LIVE_V41_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v41.js";
import { verifyInputLiveV41Authorization } from "./input-field-live-v41-authorization.js";
import { runInputLiveV41Preflight } from "./input-field-live-v41-preflight.js";
import { InputLiveV41Orchestrator } from "./run-input-field-live-v41.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V41_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v41-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV41Authorization(root);
    runInputLiveV41Preflight(
      root,
      authorization,
    ...
    const orchestrator = InputLiveV41Orchestrator.initialize({
      root,
  ...
  const orchestrator = InputLiveV41Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
