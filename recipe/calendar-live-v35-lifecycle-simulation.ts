...
import { calendarLiveV1Sha256 } from "./calendar-live-v35-broker.js";

export interface CalendarLiveV35LifecycleSimulationReport {
  artifactVersion: "calendar-live-v35-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV35AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV35Lifecycle(): Promise<CalendarLiveV35LifecycleSimulationReport> {
  await buildCalendarLiveV35Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V35_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV35AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v35-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V35_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V35_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV35History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV35History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV35AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V35_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V35_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV35History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV35History(authorized, "pending");
    ...
    await buildCalendarLiveV35Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V35_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV35AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV35Lifecycle(), null, 2)}\n`,
  );
