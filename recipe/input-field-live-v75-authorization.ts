...
import {
  ...
  INPUT_LIVE_V75_INDEX_PATH,
} from "./build-input-field-live-proof-v75.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV75HistoryExpectation = "pending" | "authorized";

export interface InputLiveV75AuthorizationArtifact {
  artifactVersion: "input-live-v75-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V75_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV75AntecedentIndex {
...
export interface InputLiveV75HistoryState {
  index?: InputLiveV75AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV75AuthorizationArtifact;
...
export interface InputLiveV75AuthorizationProof extends InputLiveV75TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V75_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V75_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V75_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V75_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v75-authorization") ||
  artifactPath.includes("input-field-live-v75-preflight") ||
  artifactPath.includes("input-field-live-v75-authorized") ||
  artifactPath.includes("input-field-live-pivot-v75-status") ||
...
export function validateInputLiveV75AntecedentIndex(
  index: InputLiveV75AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v75-antecedent-index-v1" ||
...
export function validateInputLiveV75UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV75History\(\s*["']/.test(source) ||
    /readInputLiveV75HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV75AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV75AuthorizationArtifact {
  ...
  ) as InputLiveV75AntecedentIndex;
  const indexFailures = validateInputLiveV75AntecedentIndex(index);
...
export function validateInputLiveV75AuthorizationArtifact(
  artifact: InputLiveV75AuthorizationArtifact | undefined,
  index: InputLiveV75AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v75-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v75" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V75_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V75_TARGET) ||
    !equalJson(
...
export function validateInputLiveV75History(
  state: InputLiveV75HistoryState,
  expected: InputLiveV75HistoryExpectation,
): string[] {
  const failures = validateInputLiveV75AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV75AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV75HistoryState(
  root = process.cwd(),
): InputLiveV75HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V75_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV75AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V75_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V75_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V75_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV75History(
  expected: InputLiveV75HistoryExpectation,
  root = process.cwd(),
): InputLiveV75HistoryState {
  const state = readInputLiveV75HistoryState(root);
  const failures = validateInputLiveV75History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV75Authorization(
  root = process.cwd(),
): InputLiveV75AuthorizationProof {
  const state = verifyInputLiveV75History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V75_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v75/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v75.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V75_TARGET,
    expectedDynamicTool: INPUT_LIVE_V75_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V75_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV75HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV75History(expected);
