/**
 * Table live v18 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V18_FIXED_VERIFIER_VERSION = "table-live-v18-fixed-point-v1";
export const TABLE_LIVE_V18_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV18FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV18FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV18FixedSourceProbe {
...
export interface TableLiveV18FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV18FixedFixedPoint;
}

export interface TableLiveV18FixedHardGateInput {
  thresholds: TableLiveV18FixedThresholds;
  sourceProbes: TableLiveV18FixedSourceProbe[];
  cells: TableLiveV18FixedCellValidation[];
  sceneProofs: TableLiveV18FixedSceneProof[];
...
export interface TableLiveV18FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V18_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV18FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV18FixedHardGates(
  input: TableLiveV18FixedHardGateInput,
): TableLiveV18FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV18FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V18_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV18FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV18FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
