...
import {
  ...
  INPUT_LIVE_V30_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V30_RESPONSE_CONTRACTS,
  type InputLiveV30RootProof,
} from "./input-field-live-v30-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V30_BROKER_VERSION = "input-live-v30-broker-v2";
export const INPUT_LIVE_V30_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V30_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V30_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V30_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V30_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V30_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV30RemotePhase =
  (typeof INPUT_LIVE_V30_REMOTE_PHASES)[number];
export type InputLiveV30AuthorizationMode = "live" | "simulated";

export interface InputLiveV30CommitPins {
...
export interface InputLiveV30TransactionAuthorization extends InputLiveV30CommitPins {
  mode: InputLiveV30AuthorizationMode;
...
export interface InputLiveV30Manifest {
  artifactVersion: "input-live-v30-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V30_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V30_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V30_TARGET;
...
export interface InputLiveV30ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV30RequestBody {
  artifactVersion: "input-live-v30-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V30_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV30RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV30CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V30_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV30ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV30Request extends InputLiveV30RequestBody {
...
export interface InputLiveV30Receipt {
  artifactVersion: "input-live-v30-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V30_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV30RemotePhase;
...
export interface InputLiveV30AcceptedResponse<T = unknown> {
  request: InputLiveV30Request;
  receipt: InputLiveV30Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV30IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV30ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V30_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V30_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V30_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V30_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V30_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V30_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V30_TARGET.editorType;
};
...
export const inputLiveV30RequestSequence = (
  phase: InputLiveV30RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV30RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV30RequestPath = (
  directory: string,
  phase: InputLiveV30RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV30ResponsePath = (
  directory: string,
  phase: InputLiveV30RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV30ReceiptPath = (
  directory: string,
  phase: InputLiveV30RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV30Manifest => {
  const manifest = json<InputLiveV30Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v30-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V30_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V30_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV30RemotePhase,
  captureIndex?: number,
): InputLiveV30Request | undefined => {
  const file = inputLiveV30RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV30Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV30RemotePhase,
  captureIndex?: number,
): InputLiveV30Receipt | undefined => {
  const file = inputLiveV30ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV30Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV30Request | undefined;
  receipt: InputLiveV30Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV30RemotePhase,
): InputLiveV30ResponseContract => ({
  ...INPUT_LIVE_V30_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V30_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V30_DYNAMIC_TOOL,
      target: INPUT_LIVE_V30_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v30-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V30_BROKER_VERSION,
  target: INPUT_LIVE_V30_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V30_PROGRAM_START*/${program}/*INPUT_LIVE_V30_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V30_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V30_PROGRAM_END*/";
...
export function createInputLiveV30Transaction(
  directory: string,
  ...
  const manifest: InputLiveV30Manifest = {
    artifactVersion: "input-live-v30-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V30_BROKER_VERSION,
...
export function validateInputLiveV30Request(
  directory: string,
  request: InputLiveV30Request,
): void {
  ...
  const expectedSequence = inputLiveV30RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v30-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V30_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V30_TARGET.fileKey ||
...
export function issueInputLiveV30Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV30RemotePhase,
  program: string,
  options: InputLiveV30IssueOptions = {},
): InputLiveV30Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV30RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV30Request>(targetPath);
    validateInputLiveV30Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV30ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV30TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV30RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V30_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V30_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V30_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV30RequestBody = {
    artifactVersion: "input-live-v30-broker-request-v2",
    brokerVersion: INPUT_LIVE_V30_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV30Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV30Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV30RawResponse(
  directory: string,
  phase: InputLiveV30RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV30RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV30Request>(requestFile);
  if (
  ...
  const target = inputLiveV30ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV30Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v30-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V30_BROKER_VERSION,
...
export function acceptInputLiveV30Response<T = unknown>(
  directory: string,
  phase: InputLiveV30RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV30AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV30ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV30Request>(
    inputLiveV30RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV30Request(directory, request);
  if (phase === "capture") validateInputLiveV30TechnicalGates(directory);
  const rawFile = inputLiveV30ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v30-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V30_BROKER_VERSION ||
...
export function persistInputLiveV30TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV30RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV30RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V30_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v30-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V30_BROKER_VERSION,
...
export function validateInputLiveV30TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v30-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V30_TECHNICAL_GATES.some(
...
export function validateInputLiveV30Receipt(
  directory: string,
  phase: InputLiveV30RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV30Request(directory, request);
  const receipt = json<InputLiveV30Receipt>(
    inputLiveV30ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV30ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v30-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V30_BROKER_VERSION ||
...
export function readInputLiveV30AcceptedRawPayload(
  directory: string,
  phase: InputLiveV30RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV30Receipt(directory, phase, captureIndex);
...
export function createInputLiveV30FakeDynamicResponse(
  request: InputLiveV30Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V30_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V30_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV30PersistedRequests(
  directory: string,
): InputLiveV30Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV30Request>(file)] : [];
    });
}

export const inputLiveV30Sha256 = sha256;
