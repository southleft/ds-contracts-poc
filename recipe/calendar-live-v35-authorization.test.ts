...
import { CALENDAR_LIVE_V35_INDEX_PATH } from "./build-calendar-live-proof-v35.js";
import {
...
import {
  buildCalendarLiveV35AuthorizationArtifact,
  validateCalendarLiveV35AntecedentIndex,
  validateCalendarLiveV35AuthorizationArtifact,
  validateCalendarLiveV35History,
  validateCalendarLiveV35UnitTestSource,
  type CalendarLiveV35AntecedentIndex,
  type CalendarLiveV35AuthorizationArtifact,
  type CalendarLiveV35AuthorizationProof,
  type CalendarLiveV35HistoryState,
} from "./calendar-live-v35-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV35ControlFlowSource,
  validateCalendarLiveV35SecurityAttestation,
  type CalendarLiveV35SecurityAttestation,
} from "./calendar-live-v35-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V35_INDEX_PATH);
const index = (): CalendarLiveV35AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV35AntecedentIndex;

const artifact = (): CalendarLiveV35AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV35AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV35HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV35HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV35AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v35/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV35AuthorizationArtifact,
): CalendarLiveV35SecurityAttestation => {
  const body: Omit<CalendarLiveV35SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V35_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV35History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV35History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV35History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV35History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV35History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV35UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v35-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV35UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV35AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV35AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV35AuthorizationArtifact(
        planted as CalendarLiveV35AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV35SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV35SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV35SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v35-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV35ControlFlowSource(source), []);
});
