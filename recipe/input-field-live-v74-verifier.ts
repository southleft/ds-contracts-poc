...
export const INPUT_LIVE_V74_VERIFIER_VERSION = "input-live-v74-verifier-v1";

export function buildInputLiveV74RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV74RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV74RawNode[];
}

export interface InputLiveV74NormalizedScene {
...
export interface InputLiveV74SceneProof {
...
export function normalizeInputLiveV74Scene(
  root: InputLiveV74RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV74NormalizedScene {
  const canonicalBindings: InputLiveV74NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV74RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV74Scene<Instance>(
  normalized: InputLiveV74NormalizedScene,
...
export function compileInputLiveV74ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV74PreCaptureGates(payload: unknown): void {
