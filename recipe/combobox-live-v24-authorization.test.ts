...
import { COMBOBOX_LIVE_V24_INDEX_PATH } from "./build-combobox-live-proof-v24.js";
import {
...
import {
  ...
  validateComboboxLiveV24UnitTestSource,
  type ComboboxLiveV24AntecedentIndex,
  type ComboboxLiveV24AuthorizationArtifact,
...
import {
  ...
  validateComboboxLiveV24ControlFlowSource,
  validateComboboxLiveV24SecurityAttestation,
  type ComboboxLiveV24SecurityAttestation,
} from "./combobox-live-v24-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v24-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V24_INDEX_PATH);
const index = (): ComboboxLiveV24AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV24AntecedentIndex;

const artifact = (): ComboboxLiveV24AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV24AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV24HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV24HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV24AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v24/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV24AuthorizationArtifact,
): ComboboxLiveV24SecurityAttestation => {
  const body: Omit<ComboboxLiveV24SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V24_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV24History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV24History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV24History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV24History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV24History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV24UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV24UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV24AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV24AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV24AuthorizationArtifact(
        planted as ComboboxLiveV24AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV24SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV24SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV24SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV24ControlFlowSource(source), []);
});
