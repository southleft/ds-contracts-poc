...
import { calendarLiveV1Sha256 } from "./calendar-live-v27-broker.js";

export interface CalendarLiveV27LifecycleSimulationReport {
  artifactVersion: "calendar-live-v27-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV27AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV27Lifecycle(): Promise<CalendarLiveV27LifecycleSimulationReport> {
  await buildCalendarLiveV27Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V27_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV27AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v27-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V27_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V27_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV27History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV27History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV27AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V27_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V27_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV27History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV27History(authorized, "pending");
    ...
    await buildCalendarLiveV27Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V27_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV27AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV27Lifecycle(), null, 2)}\n`,
  );
