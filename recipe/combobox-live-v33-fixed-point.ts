/**
 * Carried Combobox live v33 host verifier. Uses scene-readback-combobox-v33 so live
...
export const COMBOBOX_LIVE_V33_FIXED_VERIFIER_VERSION = "combobox-live-v33-fixed-point-v1";
export const COMBOBOX_LIVE_V33_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV33FixedThresholds {
...
export interface ComboboxLiveV33FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV33FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV33FixedSourceProbe {
...
export interface ComboboxLiveV33FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV33FixedFixedPoint;
}

export interface ComboboxLiveV33FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV33FixedHardGateInput {
  thresholds: ComboboxLiveV33FixedThresholds;
  materialRegression: ComboboxLiveV33FixedMaterialRegression;
  sourceProbes: ComboboxLiveV33FixedSourceProbe[];
  cells: ComboboxLiveV33FixedCellValidation[];
  sceneProofs: ComboboxLiveV33FixedSceneProof[];
  visualRows: ComboboxLiveV33FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV33FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V33_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV33FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV33FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV33FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV33FixedHardGates(
  input: ComboboxLiveV33FixedHardGateInput,
): ComboboxLiveV33FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV33FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV33FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V33_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV33FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV33FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV33FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
