...
import { tableLiveV2Sha256 } from "./table-live-v16-broker.js";

export interface TableLiveV16LifecycleSimulationReport {
  artifactVersion: "table-live-v16-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV16AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV16Lifecycle(): Promise<TableLiveV16LifecycleSimulationReport> {
  await buildTableLiveV16Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V16_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV16AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v16-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V16_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V16_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV16History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV16History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV16AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V16_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V16_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV16History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV16History(authorized, "pending");
    ...
    await buildTableLiveV16Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V16_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV16AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV16Lifecycle(), null, 2)}\n`,
  );
