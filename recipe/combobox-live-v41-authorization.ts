...
export type ComboboxLiveV41HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV41AuthorizationArtifact {
  artifactVersion: "combobox-live-v41-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V41_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV41AntecedentIndex {
...
export interface ComboboxLiveV41HistoryState {
  index?: ComboboxLiveV41AntecedentIndex;
  indexSha256: string;
...
export interface ComboboxLiveV41AuthorizationProof extends ComboboxLiveV41TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V41_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V41_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V41_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V41_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v41-authorization") ||
  artifactPath.includes("combobox-live-v41-preflight") ||
  artifactPath.includes("combobox-live-v41-authorized") ||
  artifactPath.includes("create-combobox-live-v41-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v41-status") ||
...
export function validateComboboxLiveV41AntecedentIndex(
  index: ComboboxLiveV41AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v41-antecedent-index-v1" ||
...
export function validateComboboxLiveV41UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV41History\(\s*["']/.test(source) ||
    /readComboboxLiveV41HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV41AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV41AuthorizationArtifact {
  ...
  ) as ComboboxLiveV41AntecedentIndex;
  const indexFailures = validateComboboxLiveV41AntecedentIndex(index);
...
export function validateComboboxLiveV41AuthorizationArtifact(
  artifact: ComboboxLiveV41AuthorizationArtifact | undefined,
  index: ComboboxLiveV41AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v41-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v41" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V41_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V41_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V41_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V41_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V41_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V41_SOURCE_ROOTS ||
...
export function validateComboboxLiveV41History(
  state: ComboboxLiveV41HistoryState,
  expected: ComboboxLiveV41HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV41AntecedentIndex(state.index);
...
export function readComboboxLiveV41HistoryState(
  root = process.cwd(),
): ComboboxLiveV41HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V41_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV41AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V41_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V41_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V41_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV41History(
  expected: ComboboxLiveV41HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV41HistoryState {
  const state = readComboboxLiveV41HistoryState(root);
  const failures = validateComboboxLiveV41History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v41 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV41Authorization(
  root = process.cwd(),
): ComboboxLiveV41AuthorizationProof {
  const state = verifyComboboxLiveV41History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V41_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v41/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v41.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V41_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V41_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V41_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV41HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV41History(expected);
  process.stdout.write(
    `Combobox live v41 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
