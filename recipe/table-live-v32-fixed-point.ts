/**
 * Table live v32 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V32_FIXED_VERIFIER_VERSION = "table-live-v32-fixed-point-v1";
export const TABLE_LIVE_V32_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV32FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV32FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV32FixedSourceProbe {
...
export interface TableLiveV32FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV32FixedFixedPoint;
}

export interface TableLiveV32FixedHardGateInput {
  thresholds: TableLiveV32FixedThresholds;
  sourceProbes: TableLiveV32FixedSourceProbe[];
  cells: TableLiveV32FixedCellValidation[];
  sceneProofs: TableLiveV32FixedSceneProof[];
...
export interface TableLiveV32FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V32_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV32FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV32FixedHardGates(
  input: TableLiveV32FixedHardGateInput,
): TableLiveV32FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV32FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V32_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV32FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV32FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
