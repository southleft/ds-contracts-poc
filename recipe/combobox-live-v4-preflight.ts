...
import {
  ...
  verifyComboboxLiveV4Authorization,
  type ComboboxLiveV4AntecedentIndex,
  type ComboboxLiveV4AuthorizationProof,
} from "./combobox-live-v4-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V4_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v4/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V4_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v4-security-attestation.json";

export interface ComboboxLiveV4SecurityAttestation {
...
export interface ComboboxLiveV4PreflightReport {
  artifactVersion: "combobox-live-v4-preflight-v1";
...
export const comboboxLiveV4AttestationSha256 = (
  value: Omit<ComboboxLiveV4SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV4SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV4AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v4-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V4_TARGET) ||
...
export function validateComboboxLiveV4ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV4Authorization");
  const preflight = source.indexOf("runComboboxLiveV4Preflight");
  const initialize = source.indexOf("ComboboxLiveV4Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV4Preflight(
  root: string,
  proof: ComboboxLiveV4AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V4_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV4PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V4_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V4_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V4_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV4AntecedentIndex;
  failures.push(...validateComboboxLiveV4AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V4_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V4_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v4-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV4ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as ComboboxLiveV4SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v4-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V4_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV4Preflight(
    process.cwd(),
    verifyComboboxLiveV4Authorization(),
    attestationPath,
