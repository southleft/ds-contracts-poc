...
export interface ComboboxLiveV5LifecycleSimulationReport {
  artifactVersion: "combobox-live-v5-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV5AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV5Lifecycle(): Promise<ComboboxLiveV5LifecycleSimulationReport> {
  await buildComboboxLiveV5Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V5_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV5AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v5-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V5_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V5_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV5History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV5History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV5AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V5_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V5_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV5History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV5History(authorized, "pending");
    ...
    await buildComboboxLiveV5Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V5_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV5AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV5Lifecycle(), null, 2)}\n`,
  );
