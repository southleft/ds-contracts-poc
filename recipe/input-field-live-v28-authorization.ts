...
import {
  ...
  INPUT_LIVE_V28_INDEX_PATH,
} from "./build-input-field-live-proof-v28.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV28HistoryExpectation = "pending" | "authorized";

export interface InputLiveV28AuthorizationArtifact {
  artifactVersion: "input-live-v28-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V28_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV28AntecedentIndex {
...
export interface InputLiveV28HistoryState {
  index?: InputLiveV28AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV28AuthorizationArtifact;
...
export interface InputLiveV28AuthorizationProof extends InputLiveV28TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V28_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V28_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V28_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V28_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v28-status") ||
...
export function validateInputLiveV28AntecedentIndex(
  index: InputLiveV28AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v28-antecedent-index-v1" ||
...
export function validateInputLiveV28UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV28History\(\s*["']/.test(source) ||
    /readInputLiveV28HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV28AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV28AuthorizationArtifact {
  ...
  ) as InputLiveV28AntecedentIndex;
  const indexFailures = validateInputLiveV28AntecedentIndex(index);
...
export function validateInputLiveV28AuthorizationArtifact(
  artifact: InputLiveV28AuthorizationArtifact | undefined,
  index: InputLiveV28AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v28-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v28" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V28_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V28_TARGET) ||
    !equalJson(
...
export function validateInputLiveV28History(
  state: InputLiveV28HistoryState,
  expected: InputLiveV28HistoryExpectation,
): string[] {
  const failures = validateInputLiveV28AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV28AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV28HistoryState(
  root = process.cwd(),
): InputLiveV28HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V28_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV28AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V28_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V28_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V28_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV28History(
  expected: InputLiveV28HistoryExpectation,
  root = process.cwd(),
): InputLiveV28HistoryState {
  const state = readInputLiveV28HistoryState(root);
  const failures = validateInputLiveV28History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV28Authorization(
  root = process.cwd(),
): InputLiveV28AuthorizationProof {
  const state = verifyInputLiveV28History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V28_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v28/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v28.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V28_TARGET,
    expectedDynamicTool: INPUT_LIVE_V28_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V28_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV28HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV28History(expected);
