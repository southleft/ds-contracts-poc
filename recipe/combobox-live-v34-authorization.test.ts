...
import { COMBOBOX_LIVE_V34_INDEX_PATH } from "./build-combobox-live-proof-v34.js";
import {
  COMBOBOX_LIVE_V34_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V34_TARGET,
} from "./combobox-live-v34-broker.js";
import {
  buildComboboxLiveV34AuthorizationArtifact,
  validateComboboxLiveV34AntecedentIndex,
  validateComboboxLiveV34AuthorizationArtifact,
  validateComboboxLiveV34History,
  validateComboboxLiveV34UnitTestSource,
  type ComboboxLiveV34AntecedentIndex,
  type ComboboxLiveV34AuthorizationArtifact,
  type ComboboxLiveV34AuthorizationProof,
  type ComboboxLiveV34HistoryState,
} from "./combobox-live-v34-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV34ControlFlowSource,
  validateComboboxLiveV34SecurityAttestation,
  type ComboboxLiveV34SecurityAttestation,
} from "./combobox-live-v34-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v34-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V34_INDEX_PATH);
const index = (): ComboboxLiveV34AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV34AntecedentIndex;

const artifact = (): ComboboxLiveV34AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV34AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV34HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV34HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV34AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v34/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV34AuthorizationArtifact,
): ComboboxLiveV34SecurityAttestation => {
  const body: Omit<ComboboxLiveV34SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V34_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV34History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV34History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV34History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV34History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV34History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV34UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV34UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV34AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV34AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV34AuthorizationArtifact(
        planted as ComboboxLiveV34AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV34SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV34SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV34SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV34ControlFlowSource(source), []);
});
