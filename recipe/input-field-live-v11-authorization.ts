...
import {
  ...
  INPUT_LIVE_V11_INDEX_PATH,
} from "./build-input-field-live-proof-v11.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV11HistoryExpectation = "pending" | "authorized";

export interface InputLiveV11AuthorizationArtifact {
  artifactVersion: "input-live-v11-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V11_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV11AntecedentIndex {
...
export interface InputLiveV11HistoryState {
  index?: InputLiveV11AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV11AuthorizationArtifact;
...
export interface InputLiveV11AuthorizationProof extends InputLiveV11TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V11_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V11_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V11_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V11_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v11-status") ||
...
export function validateInputLiveV11AntecedentIndex(
  index: InputLiveV11AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v11-antecedent-index-v1" ||
...
export function validateInputLiveV11UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV11History\(\s*["']/.test(source) ||
    /readInputLiveV11HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV11AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV11AuthorizationArtifact {
  ...
  ) as InputLiveV11AntecedentIndex;
  const indexFailures = validateInputLiveV11AntecedentIndex(index);
...
export function validateInputLiveV11AuthorizationArtifact(
  artifact: InputLiveV11AuthorizationArtifact | undefined,
  index: InputLiveV11AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v11-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v11" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V11_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V11_TARGET) ||
    !equalJson(
...
export function validateInputLiveV11History(
  state: InputLiveV11HistoryState,
  expected: InputLiveV11HistoryExpectation,
): string[] {
  const failures = validateInputLiveV11AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV11AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV11HistoryState(
  root = process.cwd(),
): InputLiveV11HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V11_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV11AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V11_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V11_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V11_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV11History(
  expected: InputLiveV11HistoryExpectation,
  root = process.cwd(),
): InputLiveV11HistoryState {
  const state = readInputLiveV11HistoryState(root);
  const failures = validateInputLiveV11History(state, expected);
  if (failures.length)
    throw new Error(
      `Input live v11 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyInputLiveV11Authorization(
  root = process.cwd(),
): InputLiveV11AuthorizationProof {
  const state = verifyInputLiveV11History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(path.join(repositoryRoot, INPUT_LIVE_V11_AUTHORIZATION_PATH)),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v11/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v11.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V11_TARGET,
    expectedDynamicTool: INPUT_LIVE_V11_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V11_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV11HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV11History(expected);
  process.stdout.write(
    `Input live v11 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
