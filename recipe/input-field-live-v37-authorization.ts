...
import {
  ...
  INPUT_LIVE_V37_INDEX_PATH,
} from "./build-input-field-live-proof-v37.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV37HistoryExpectation = "pending" | "authorized";

export interface InputLiveV37AuthorizationArtifact {
  artifactVersion: "input-live-v37-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V37_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV37AntecedentIndex {
...
export interface InputLiveV37HistoryState {
  index?: InputLiveV37AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV37AuthorizationArtifact;
...
export interface InputLiveV37AuthorizationProof extends InputLiveV37TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V37_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V37_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V37_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V37_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v37-status") ||
...
export function validateInputLiveV37AntecedentIndex(
  index: InputLiveV37AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v37-antecedent-index-v1" ||
...
export function validateInputLiveV37UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV37History\(\s*["']/.test(source) ||
    /readInputLiveV37HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV37AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV37AuthorizationArtifact {
  ...
  ) as InputLiveV37AntecedentIndex;
  const indexFailures = validateInputLiveV37AntecedentIndex(index);
...
export function validateInputLiveV37AuthorizationArtifact(
  artifact: InputLiveV37AuthorizationArtifact | undefined,
  index: InputLiveV37AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v37-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v37" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V37_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V37_TARGET) ||
    !equalJson(
...
export function validateInputLiveV37History(
  state: InputLiveV37HistoryState,
  expected: InputLiveV37HistoryExpectation,
): string[] {
  const failures = validateInputLiveV37AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV37AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV37HistoryState(
  root = process.cwd(),
): InputLiveV37HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V37_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV37AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V37_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V37_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V37_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV37History(
  expected: InputLiveV37HistoryExpectation,
  root = process.cwd(),
): InputLiveV37HistoryState {
  const state = readInputLiveV37HistoryState(root);
  const failures = validateInputLiveV37History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV37Authorization(
  root = process.cwd(),
): InputLiveV37AuthorizationProof {
  const state = verifyInputLiveV37History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V37_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v37/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v37.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V37_TARGET,
    expectedDynamicTool: INPUT_LIVE_V37_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V37_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV37HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV37History(expected);
