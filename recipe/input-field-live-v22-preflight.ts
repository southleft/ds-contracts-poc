...
import {
  ...
  verifyInputLiveV22Authorization,
  type InputLiveV22AntecedentIndex,
  type InputLiveV22AuthorizationProof,
} from "./input-field-live-v22-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V22_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v22/operator-security-attestation-template.json";
export const INPUT_LIVE_V22_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v22-security-attestation.json";

export interface InputLiveV22SecurityAttestation {
...
export interface InputLiveV22PreflightReport {
  artifactVersion: "input-live-v22-preflight-v1";
...
export const inputLiveV22AttestationSha256 = (
  value: Omit<InputLiveV22SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV22SecurityAttestation(
  value: unknown,
  proof: InputLiveV22AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v22-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V22_TARGET) ||
...
export function validateInputLiveV22ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV22Authorization");
  const preflight = source.indexOf("runInputLiveV22Preflight");
  const initialize = source.indexOf("InputLiveV22Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV22Preflight(
  root: string,
  proof: InputLiveV22AuthorizationProof,
  attestationPath = INPUT_LIVE_V22_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV22PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V22_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V22_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V22_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV22AntecedentIndex;
  failures.push(...validateInputLiveV22AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V22_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V22_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v22-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV22ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV22SecurityAttestation;
  return {
    artifactVersion: "input-live-v22-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V22_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV22Preflight(
    process.cwd(),
    verifyInputLiveV22Authorization(),
    attestationPath,
