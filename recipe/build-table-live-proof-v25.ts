...
import {
  TABLE_LIVE_V25_CAPTURE_COUNT,
  TABLE_LIVE_V25_HOST_PHASES,
  TABLE_LIVE_V25_REMOTE_REQUESTS,
  TABLE_LIVE_V25_SOURCE_ROOTS,
  TABLE_LIVE_V25_VARIANT_COUNT,
  buildTableLiveV25CaptureProgram,
  buildTableLiveV25CleanupProgram,
  buildTableLiveV25ExtractProgram,
  buildTableLiveV25ProbeProgram,
  buildTableLiveV25RestoreProgram,
  tableLiveV2CaptureManifestSha256,
  type TableLiveV25CaptureCell,
  type TableLiveV25SourceIdentity,
  type TableLiveV25WriterOwnership,
} from "./table-live-v25-contract.js";
import {
  TABLE_LIVE_V25_DYNAMIC_TOOL,
  TABLE_LIVE_V25_TARGET,
  tableLiveV2RequestSequence,
} from "./table-live-v25-broker.js";
import {
...
export const TABLE_LIVE_V25_EVIDENCE_ROOT =
  "recipe/evidence/table-live-pivot-v25";
export const TABLE_LIVE_V25_PROTOCOL_PATH = `${TABLE_LIVE_V25_EVIDENCE_ROOT}/protocol.json`;
export const TABLE_LIVE_V25_PLAN_PATH = `${TABLE_LIVE_V25_EVIDENCE_ROOT}/proof-plan.json`;
export const TABLE_LIVE_V25_CAPTURE_MANIFEST_PATH = `${TABLE_LIVE_V25_EVIDENCE_ROOT}/capture-manifest.json`;
export const TABLE_LIVE_V25_REQUEST_MANIFEST_PATH = `${TABLE_LIVE_V25_EVIDENCE_ROOT}/request-manifest.json`;
export const TABLE_LIVE_V25_INDEX_PATH = `${TABLE_LIVE_V25_EVIDENCE_ROOT}/antecedent-index.json`;
export const TABLE_LIVE_V25_AUTHORIZATION_TEMPLATE_PATH = `${TABLE_LIVE_V25_EVIDENCE_ROOT}/authorization-template.json`;
export const TABLE_LIVE_V25_AUTHORIZATION_PATH = `${TABLE_LIVE_V25_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface TableLiveV25ProofPlan {
  artifactVersion: "table-live-v25-proof-plan-v1";
  status: "draft antecedent; pending separate authorization; live execution forbidden";
  target: typeof TABLE_LIVE_V25_TARGET;
...
const captureCells = (
...
): TableLiveV25CaptureCell[] => {
  const cells: TableLiveV25CaptureCell[] = [];
  ...
  if (cells.length !== TABLE_LIVE_V25_CAPTURE_COUNT)
    throw new Error(
      `Table live v25 capture cells ${cells.length}, expected ${TABLE_LIVE_V25_CAPTURE_COUNT}`,
    );
...
const protocol = (expectedFacts: number) => ({
  artifactVersion: "table-live-v25-external-operator-protocol-v1",
  protocolId: "table-live-v25",
  status:
  ...
  target: TABLE_LIVE_V25_TARGET,
  expectedDynamicTool: TABLE_LIVE_V25_DYNAMIC_TOOL,
  denominator: {
    sourceRoots: TABLE_LIVE_V25_SOURCE_ROOTS,
    sets: 6,
    variants: TABLE_LIVE_V25_VARIANT_COUNT,
    captureCells: TABLE_LIVE_V25_CAPTURE_COUNT,
    remoteRequests: TABLE_LIVE_V25_REMOTE_REQUESTS,
    hostPhases: TABLE_LIVE_V25_HOST_PHASES,
...
export async function buildTableLiveV25Proof(
  check = process.argv.includes("--check"),
): Promise<TableLiveV25ProofPlan> {
  ...
  if (
    ...
    throw new Error(
      "Table live v25 writer reused Input, Combobox, or Table v1–v24 identity",
    );
  const captures = captureCells(sources);
  const writerProgram = Buffer.from(writer.code);
  const writerOwnershipBlueprint: TableLiveV25WriterOwnership = {
  ...
  const identities: TableLiveV25SourceIdentity[] = sources.map((source) => ({
    source: source.source,
    adapterIdentity: source.adapterIdentity,
    recipeHash: source.recipeHash,
    envelopeHash: source.envelope.integrity.canonicalHash,
  ...
  const restoreBlueprint = Buffer.from(
    buildTableLiveV25RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildTableLiveV25ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildTableLiveV25ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildTableLiveV25CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );
  const cleanupBlueprint = Buffer.from(
    buildTableLiveV25CleanupProgram(writerOwnershipBlueprint),
  );
  ...
  const sourceMetadata = sources.map((source) => {
    ...
    const tablePath = `${TABLE_LIVE_V25_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-table.json.gz`;
    const rowPath = `${TABLE_LIVE_V25_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-row.json.gz`;
    const cellPath = `${TABLE_LIVE_V25_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-cell.json.gz`;
  ...
  antecedentOutputs.set(
    TABLE_LIVE_V25_PROTOCOL_PATH,
    jsonBytes(protocol(expectedFacts)),
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V25_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V25_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V25_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V25_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V25_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V25_EVIDENCE_ROOT}/programs/cleanup-blueprint.js`,
    cleanupBlueprint,
  );

  const captureManifest = {
    artifactVersion: "table-live-v25-capture-manifest-v1",
  ...
  antecedentOutputs.set(
    TABLE_LIVE_V25_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: TableLiveV25ProofPlan = {
    artifactVersion: "table-live-v25-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: TABLE_LIVE_V25_TARGET,
    namespace: TABLE_FIGMA_NAMESPACE,
    writer: {
      programPath: `${TABLE_LIVE_V25_EVIDENCE_ROOT}/programs/writer.txt`,
    ...
    captureManifest: {
      path: TABLE_LIVE_V25_CAPTURE_MANIFEST_PATH,
      cells: 20,
      requests: 20,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(TABLE_LIVE_V25_PLAN_PATH, jsonBytes(proofPlan));

  const requests = {
    artifactVersion: "table-live-v25-request-manifest-v1",
    expectedDynamicTool: TABLE_LIVE_V25_DYNAMIC_TOOL,
    target: TABLE_LIVE_V25_TARGET,
  ...
  if (requests.requestCount !== TABLE_LIVE_V25_REMOTE_REQUESTS)
    throw new Error("Table live v25 remote request denominator drifted");
  antecedentOutputs.set(
    TABLE_LIVE_V25_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const lifecycleExcludedPaths = [
    TABLE_LIVE_V25_AUTHORIZATION_TEMPLATE_PATH,
    TABLE_LIVE_V25_AUTHORIZATION_PATH,
    "recipe/table-live-v25-authorization.ts",
    "recipe/table-live-v25-authorization.test.ts",
    "recipe/table-live-v25-preflight.ts",
    "recipe/table-live-v25-authorized.ts",
    "recipe/write-table-live-v25-authorization.ts",
    "recipe/create-table-live-v25-security-attestation.ts",
    "recipe/evidence/table-live-pivot-v25/operator-security-attestation-template.json",
    "recipe/evidence/table-live-pivot-v25-status.json",
    "recipe/evidence/status-index.json",
  ];
  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/table-live-v25-broker.ts",
    "recipe/table-live-v25-contract.ts",
    "recipe/table-live-v25-restore.ts",
    "recipe/table-live-v25-cleanup.ts",
    "recipe/table-live-v25-verifier.ts",
    "recipe/table-live-v25-fixed-point.ts",
    "recipe/run-table-live-v25.ts",
    "recipe/build-table-live-proof-v25.ts",
    "recipe/table-live-v25-lifecycle-simulation.ts",
    "recipe/scene-readback-table-v1.ts",
    "recipe/scene-readback-runtime-table-v25.ts",
    "recipe/table-figma-writer.ts",
    "recipe/figma-property-normalizer-v8.ts",
    "recipe/figma-runtime-portability.ts",
    "recipe/normalize.ts",
  ].filter((artifactPath) => artifactPath !== TABLE_LIVE_V25_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "table-live-v25-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
  ...
  antecedentOutputs.set(TABLE_LIVE_V25_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "table-live-v25-authorization-template-v1",
    authorizationId: "table-live-v25",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: TABLE_LIVE_V25_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
    ...
    denominator: {
      remoteRequests: TABLE_LIVE_V25_REMOTE_REQUESTS,
      hostPhases: TABLE_LIVE_V25_HOST_PHASES,
      captures: TABLE_LIVE_V25_CAPTURE_COUNT,
      sourceRoots: TABLE_LIVE_V25_SOURCE_ROOTS,
      expectedFacts,
    },
    execution: {
  ...
  const attestationTemplate = jsonBytes({
    artifactVersion: "table-live-v25-operator-security-attestation-template-v1",
    status: "template only; complete privately after PREPARE+AUTHORIZE",
    tokenValuesForbidden: true,
    privateDirectory: "private/table-live-v25-security-attestation.json",
  });
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Table live v25 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(TABLE_LIVE_V25_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(TABLE_LIVE_V25_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
    writeFileSync(
      `${TABLE_LIVE_V25_EVIDENCE_ROOT}/operator-security-attestation-template.json`,
      attestationTemplate,
    );
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(TABLE_LIVE_V25_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(TABLE_LIVE_V25_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Table live v25 authorization template drift");
  }
  if (
    writer.namespace !== TABLE_FIGMA_NAMESPACE ||
    writer.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    ...
    throw new Error("Table live v25 identity or cleanup invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildTableLiveV25Proof(), null, 2)}\n`,
  );
