...
import {
  validateCalendarLiveV28AntecedentIndex,
  verifyCalendarLiveV28Authorization,
  type CalendarLiveV28AntecedentIndex,
  type CalendarLiveV28AuthorizationProof,
} from "./calendar-live-v28-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V28_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v28/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V28_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v28-security-attestation.json";

export interface CalendarLiveV28SecurityAttestation {
  artifactVersion: "calendar-live-v28-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV28PreflightReport {
  artifactVersion: "calendar-live-v28-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV28SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV28SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV28AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v28-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V28_TARGET) ||
...
export function validateCalendarLiveV28ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV28Authorization");
  const preflight = source.indexOf("runCalendarLiveV28Preflight");
  const initialize = source.indexOf("CalendarLiveV28Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV28Preflight(
  root: string,
  proof: CalendarLiveV28AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V28_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV28PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V28_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V28_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV28AntecedentIndex;
  failures.push(...validateCalendarLiveV28AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V28_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V28_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV28SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v28-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV28ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v28 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV28SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v28-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V28_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV28Preflight(
    process.cwd(),
    verifyCalendarLiveV28Authorization(),
    attestationPath,
