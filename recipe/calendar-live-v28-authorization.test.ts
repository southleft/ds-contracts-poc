...
import { CALENDAR_LIVE_V28_INDEX_PATH } from "./build-calendar-live-proof-v28.js";
import {
...
import {
  buildCalendarLiveV28AuthorizationArtifact,
  validateCalendarLiveV28AntecedentIndex,
  validateCalendarLiveV28AuthorizationArtifact,
  validateCalendarLiveV28History,
  validateCalendarLiveV28UnitTestSource,
  type CalendarLiveV28AntecedentIndex,
  type CalendarLiveV28AuthorizationArtifact,
  type CalendarLiveV28AuthorizationProof,
  type CalendarLiveV28HistoryState,
} from "./calendar-live-v28-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV28ControlFlowSource,
  validateCalendarLiveV28SecurityAttestation,
  type CalendarLiveV28SecurityAttestation,
} from "./calendar-live-v28-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V28_INDEX_PATH);
const index = (): CalendarLiveV28AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV28AntecedentIndex;

const artifact = (): CalendarLiveV28AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV28AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV28HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV28HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV28AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v28/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV28AuthorizationArtifact,
): CalendarLiveV28SecurityAttestation => {
  const body: Omit<CalendarLiveV28SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V28_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV28History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV28History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV28History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV28History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV28History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV28UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v28-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV28UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV28AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV28AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV28AuthorizationArtifact(
        planted as CalendarLiveV28AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV28SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV28SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV28SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v28-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV28ControlFlowSource(source), []);
});
