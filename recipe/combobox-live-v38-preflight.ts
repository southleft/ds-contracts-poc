...
import {
  validateComboboxLiveV38AntecedentIndex,
  verifyComboboxLiveV38Authorization,
  type ComboboxLiveV38AntecedentIndex,
  type ComboboxLiveV38AuthorizationProof,
} from "./combobox-live-v38-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V38_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v38/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V38_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v38-security-attestation.json";

export interface ComboboxLiveV38SecurityAttestation {
  artifactVersion: "combobox-live-v38-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV38PreflightReport {
  artifactVersion: "combobox-live-v38-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV38SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV38SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV38AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v38-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V38_TARGET) ||
...
export function validateComboboxLiveV38ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV38Authorization");
  const preflight = source.indexOf("runComboboxLiveV38Preflight");
  const initialize = source.indexOf("ComboboxLiveV38Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV38Preflight(
  root: string,
  proof: ComboboxLiveV38AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V38_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV38PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V38_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V38_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V38_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV38AntecedentIndex;
  failures.push(...validateComboboxLiveV38AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V38_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V38_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV38SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v38-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV38ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v38 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV38SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v38-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V38_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV38Preflight(
    process.cwd(),
    verifyComboboxLiveV38Authorization(),
    attestationPath,
