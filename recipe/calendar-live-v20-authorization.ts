...
export type CalendarLiveV20HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV20AuthorizationArtifact {
  artifactVersion: "calendar-live-v20-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V20_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV20AntecedentIndex {
...
export interface CalendarLiveV20HistoryState {
  index?: CalendarLiveV20AntecedentIndex;
  indexSha256: string;
...
export interface CalendarLiveV20AuthorizationProof
  extends CalendarLiveV20TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V20_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V20_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V20_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V20_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v20-authorization") ||
  artifactPath.includes("calendar-live-v20-preflight") ||
  artifactPath.includes("calendar-live-v20-authorized") ||
  artifactPath.includes("create-calendar-live-v20-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v20-status") ||
...
export function validateCalendarLiveV20AntecedentIndex(
  index: CalendarLiveV20AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v20-antecedent-index-v1" ||
...
export function validateCalendarLiveV20UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV20History\(\s*["']/.test(source) ||
    /readCalendarLiveV20HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV20AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV20AuthorizationArtifact {
  ...
  ) as CalendarLiveV20AntecedentIndex;
  const indexFailures = validateCalendarLiveV20AntecedentIndex(index);
...
export function validateCalendarLiveV20AuthorizationArtifact(
  artifact: CalendarLiveV20AuthorizationArtifact | undefined,
  index: CalendarLiveV20AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v20-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v20" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V20_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V20_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V20_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V20_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V20_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V20_SOURCE_ROOTS ||
...
export function validateCalendarLiveV20History(
  state: CalendarLiveV20HistoryState,
  expected: CalendarLiveV20HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV20AntecedentIndex(state.index);
...
export function readCalendarLiveV20HistoryState(
  root = process.cwd(),
): CalendarLiveV20HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V20_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV20AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V20_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V20_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V20_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV20History(
  expected: CalendarLiveV20HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV20HistoryState {
  const state = readCalendarLiveV20HistoryState(root);
  const failures = validateCalendarLiveV20History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v20 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV20Authorization(
  root = process.cwd(),
): CalendarLiveV20AuthorizationProof {
  const state = verifyCalendarLiveV20History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V20_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v20/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v20.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V20_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V20_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V20_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV20HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV20History(expected);
  process.stdout.write(
    `Calendar live v20 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
