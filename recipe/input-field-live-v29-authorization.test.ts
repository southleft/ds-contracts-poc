...
import { INPUT_LIVE_V29_INDEX_PATH } from "./build-input-field-live-proof-v29.js";
import {
  INPUT_LIVE_V29_DYNAMIC_TOOL,
  INPUT_LIVE_V29_TARGET,
} from "./input-field-live-v29-broker.js";
import {
  buildInputLiveV29AuthorizationArtifact,
  validateInputLiveV29AntecedentIndex,
  validateInputLiveV29AuthorizationArtifact,
  validateInputLiveV29History,
  validateInputLiveV29UnitTestSource,
  type InputLiveV29AntecedentIndex,
  type InputLiveV29AuthorizationArtifact,
  type InputLiveV29AuthorizationProof,
  type InputLiveV29HistoryState,
} from "./input-field-live-v29-authorization.js";
import {
  inputLiveV29AttestationSha256,
  validateInputLiveV29ControlFlowSource,
  validateInputLiveV29SecurityAttestation,
  type InputLiveV29SecurityAttestation,
} from "./input-field-live-v29-preflight.js";
import { inputLiveV29Sha256 } from "./input-field-live-v29-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V29_INDEX_PATH);
const index = (): InputLiveV29AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV29AntecedentIndex;

const artifact = (): InputLiveV29AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV29AuthorizationArtifact({
...
const pendingState = (): InputLiveV29HistoryState => ({
  index: index(),
  indexSha256: inputLiveV29Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV29HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV29AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV29Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V29_TARGET,
  expectedDynamicTool: INPUT_LIVE_V29_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v29/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV29AuthorizationArtifact,
): InputLiveV29SecurityAttestation => {
  const body: Omit<InputLiveV29SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v29-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V29_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV29AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV29History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV29History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV29History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV29History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV29History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV29UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV29UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV29AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV29Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV29AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV29AuthorizationArtifact(
        planted as InputLiveV29AuthorizationArtifact,
        index(),
        inputLiveV29Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV29SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV29AttestationSha256(
      body as Omit<InputLiveV29SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV29SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV29ControlFlowSource(source), []);
});
