...
import { buildComboboxLiveV25CleanupRuntime } from "./combobox-live-v25-cleanup.js";
import {
  type ComboboxLiveV25FixedFixedPoint,
} from "./combobox-live-v25-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV25RestoreProgram,
  validateComboboxLiveV25RestorePayload,
  type ComboboxLiveV25RestorePayload,
} from "./combobox-live-v25-restore.js";
import {
  buildComboboxLiveV25RawPropertyRuntime,
  normalizeComboboxLiveV25Scene,
  type ComboboxLiveV25RawNode,
} from "./combobox-live-v25-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v25.js";
import {
...
} from "./scene-readback-combobox-v25.js";
...
export const COMBOBOX_LIVE_V25_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V25_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V25_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V25_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V25_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V25_SET_COUNT = 4;
export const COMBOBOX_LIVE_V25_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V25_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V25_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V25_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V25_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV25SourceId = (typeof COMBOBOX_LIVE_V25_SOURCE_IDS)[number];

export interface ComboboxLiveV25SourceIdentity {
  source: ComboboxLiveV25SourceId;
...
export interface ComboboxLiveV25WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV25ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV25SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV25RawNode;
    optionScene: ComboboxLiveV25RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV25RootProof {
  source: ComboboxLiveV25SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV25FixedFixedPoint;
}

export interface ComboboxLiveV25ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV25SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV25CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV25SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV25CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV25SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV25CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v25 ${label} must be an object`);
...
export function validateComboboxLiveV25WriterPayload(
  value: unknown,
): ComboboxLiveV25WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V25_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v25 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v25 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v25 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV25SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV25ExtractProgram(
  writer: ComboboxLiveV25WriterOwnership,
  sources: readonly ComboboxLiveV25SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v25 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V25_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V25_NAMESPACE)}
${buildComboboxLiveV25RawPropertyRuntime()}
...
export function validateComboboxLiveV25ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV25WriterOwnership,
): ComboboxLiveV25ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v25 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V25_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v25 extract schema/two-root mismatch");
  return payload as ComboboxLiveV25ExtractPayload;
}

export function proveComboboxLiveV25Roots<Instance>(
  extract: ComboboxLiveV25ExtractPayload,
  sources: readonly (ComboboxLiveV25SourceIdentity & {
...
): ComboboxLiveV25RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v25 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v25 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV25Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV25Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV25FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v25 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v25 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v25 compile lost owned sets");
...
export function assertComboboxLiveV25RootProofs(
  proofs: readonly ComboboxLiveV25RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v25 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v25 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV25ProbeProgram(
  writer: ComboboxLiveV25WriterOwnership,
  sources: readonly ComboboxLiveV25SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V25_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV25ProbePayload(
  value: unknown,
  writer: ComboboxLiveV25WriterOwnership,
): ComboboxLiveV25ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV25ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V25_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V25_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v25 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV25CaptureManifest(
  cells: readonly ComboboxLiveV25CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V25_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V25_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V25_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V25_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v25 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV25CaptureCell[],
): string => {
  validateComboboxLiveV25CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV25CaptureProgram(
  writer: ComboboxLiveV25WriterOwnership,
  cell: ComboboxLiveV25CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V25_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v25 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v25 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V25_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV25CapturePayload(
  value: unknown,
  cell: ComboboxLiveV25CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV25CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV25CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V25_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V25_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v25 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV25CaptureResponses(
  manifest: readonly ComboboxLiveV25CaptureCell[],
  responses: readonly ComboboxLiveV25CapturePayload[],
): void {
  validateComboboxLiveV25CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v25 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV25ObjectiveReport {
  artifactVersion: "combobox-live-v25-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV25Objective(
  manifest: readonly ComboboxLiveV25CaptureCell[],
  responses: readonly ComboboxLiveV25CapturePayload[],
): ComboboxLiveV25ObjectiveReport {
  assertComboboxLiveV25CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v25-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV25CleanupProgram(
  writer: ComboboxLiveV25WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV25CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V25_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v25 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV25CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV25WriterOwnership,
): ComboboxLiveV25CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV25CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v25 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V25_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV25WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
