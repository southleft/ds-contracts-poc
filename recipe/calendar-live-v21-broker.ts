...
import {
  assertCalendarLiveV21RootProofs,
  CALENDAR_LIVE_V21_CAPTURE_COUNT,
  CALENDAR_LIVE_V21_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  CALENDAR_LIVE_V21_RESPONSE_CONTRACTS,
  type CalendarLiveV21RootProof,
} from "./calendar-live-v21-contract.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V21_BROKER_VERSION = "calendar-live-v21-broker-v1";
export const CALENDAR_LIVE_V21_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const CALENDAR_LIVE_V21_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const CALENDAR_LIVE_V21_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const CALENDAR_LIVE_V21_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const CALENDAR_LIVE_V21_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const CALENDAR_LIVE_V21_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type CalendarLiveV21RemotePhase =
  (typeof CALENDAR_LIVE_V21_REMOTE_PHASES)[number];
export type CalendarLiveV21AuthorizationMode = "live" | "simulated";

export interface CalendarLiveV21CommitPins {
...
export interface CalendarLiveV21TransactionAuthorization extends CalendarLiveV21CommitPins {
  mode: CalendarLiveV21AuthorizationMode;
...
export interface CalendarLiveV21Manifest {
  artifactVersion: "calendar-live-v21-broker-manifest-v2";
  brokerVersion: typeof CALENDAR_LIVE_V21_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof CALENDAR_LIVE_V21_DYNAMIC_TOOL;
  target: typeof CALENDAR_LIVE_V21_TARGET;
  authorization: CalendarLiveV21TransactionAuthorization;
...
export interface CalendarLiveV21ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface CalendarLiveV21RequestBody {
  artifactVersion: "calendar-live-v21-broker-request-v2";
  brokerVersion: typeof CALENDAR_LIVE_V21_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: CalendarLiveV21RemotePhase;
  sequence: number;
  ...
  commitPins: CalendarLiveV21CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof CALENDAR_LIVE_V21_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: CalendarLiveV21ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface CalendarLiveV21Request extends CalendarLiveV21RequestBody {
...
export interface CalendarLiveV21Receipt {
  artifactVersion: "calendar-live-v21-broker-receipt-v2";
  brokerVersion: typeof CALENDAR_LIVE_V21_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: CalendarLiveV21RemotePhase;
...
export interface CalendarLiveV21AcceptedResponse<T = unknown> {
  request: CalendarLiveV21Request;
  receipt: CalendarLiveV21Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface CalendarLiveV21IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: CalendarLiveV21ResponseContract;
...
const equalTarget = (value: unknown): value is typeof CALENDAR_LIVE_V21_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === CALENDAR_LIVE_V21_TARGET.fileKey &&
  (value as BrokerTarget).fileName === CALENDAR_LIVE_V21_TARGET.fileName &&
  (value as BrokerTarget).editorType === CALENDAR_LIVE_V21_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== CALENDAR_LIVE_V21_TARGET.fileKey ||
    context.fileName !== CALENDAR_LIVE_V21_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === CALENDAR_LIVE_V21_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Calendar live v21 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= CALENDAR_LIVE_V21_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v21 capture index must be 0..7");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Calendar live v21 authorization pins are malformed");
};

const readManifest = (directory: string): CalendarLiveV21Manifest => {
  const manifest = json<CalendarLiveV21Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Calendar live v21 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "calendar-live-v21-broker-manifest-v2" ||
    manifest.brokerVersion !== CALENDAR_LIVE_V21_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== CALENDAR_LIVE_V21_CAPTURE_COUNT ||
    ...
    throw new TypeError("invalid Calendar live v21 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: CalendarLiveV21Request,
): CalendarLiveV21RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: CalendarLiveV21RemotePhase,
  captureIndex?: number,
): CalendarLiveV21Request | undefined => {
  const file = calendarLiveV1RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV21Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: CalendarLiveV21RemotePhase,
  captureIndex?: number,
): CalendarLiveV21Receipt | undefined => {
  const file = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV21Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: CalendarLiveV21Request | undefined;
  receipt: CalendarLiveV21Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: CalendarLiveV21RemotePhase,
): CalendarLiveV21ResponseContract => ({
  ...CALENDAR_LIVE_V21_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: CalendarLiveV21Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: CALENDAR_LIVE_V21_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "calendar-live-v21-plugin-response-v2",
  brokerVersion: CALENDAR_LIVE_V21_BROKER_VERSION,
  target: CALENDAR_LIVE_V21_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*CALENDAR_LIVE_V21_PROGRAM_START*/${program}/*CALENDAR_LIVE_V21_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*CALENDAR_LIVE_V21_PROGRAM_START*/";
  const endMarker = "/*CALENDAR_LIVE_V21_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Calendar live v21 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createCalendarLiveV21Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Calendar live v21 private signing key does not match authorized public-key identity",
    );
  const manifest: CalendarLiveV21Manifest = {
    artifactVersion: "calendar-live-v21-broker-manifest-v2",
    brokerVersion: CALENDAR_LIVE_V21_BROKER_VERSION,
    ...
    captureRequests: CALENDAR_LIVE_V21_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateCalendarLiveV21Request(
  directory: string,
  request: CalendarLiveV21Request,
): void {
  ...
  if (
    request.artifactVersion !== "calendar-live-v21-broker-request-v2" ||
    request.brokerVersion !== CALENDAR_LIVE_V21_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== CALENDAR_LIVE_V21_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Calendar live v21 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueCalendarLiveV21Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: CalendarLiveV21RemotePhase,
  program: string,
  options: CalendarLiveV21IssueOptions = {},
): CalendarLiveV21Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<CalendarLiveV21Request>(targetPath);
    validateCalendarLiveV21Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Calendar live v21 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(calendarLiveV1ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Calendar live v21 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Calendar live v21 empty code envelope refused");
  if (phase === "capture") validateCalendarLiveV21TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Calendar live v21 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: CALENDAR_LIVE_V21_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? CALENDAR_LIVE_V21_SIGNED_WRITER_TIMEOUT_MS
        : CALENDAR_LIVE_V21_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: CalendarLiveV21RequestBody = {
    artifactVersion: "calendar-live-v21-broker-request-v2",
    brokerVersion: CALENDAR_LIVE_V21_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: CalendarLiveV21Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateCalendarLiveV21Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistCalendarLiveV21RawResponse(
  directory: string,
  phase: CalendarLiveV21RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Calendar live v21 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<CalendarLiveV21Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Calendar live v21 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = calendarLiveV1ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Calendar live v21 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): CalendarLiveV21Receipt => {
  const body: Omit<CalendarLiveV21Receipt, "receiptSha256"> = {
    artifactVersion: "calendar-live-v21-broker-receipt-v2",
    brokerVersion: CALENDAR_LIVE_V21_BROKER_VERSION,
...
export function acceptCalendarLiveV21Response<T = unknown>(
  directory: string,
  phase: CalendarLiveV21RemotePhase,
  options: {
    captureIndex?: number;
...
): CalendarLiveV21AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Calendar live v21 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<CalendarLiveV21Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Calendar live v21 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "calendar-live-v21-plugin-response-v2" ||
    plugin?.brokerVersion !== CALENDAR_LIVE_V21_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Calendar live v21 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistCalendarLiveV21TechnicalGates(
  directory: string,
  rootProofs: readonly CalendarLiveV21RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertCalendarLiveV21RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Calendar live v21 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Calendar live v21 probe payload hash malformed");
  const gates = Object.fromEntries(
    CALENDAR_LIVE_V21_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "calendar-live-v21-host-technical-gates-v2",
    brokerVersion: CALENDAR_LIVE_V21_BROKER_VERSION,
...
export function validateCalendarLiveV21TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "calendar-live-v21-host-technical-gates-v2" ||
    ...
    CALENDAR_LIVE_V21_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Calendar live v21 technical gates artifact");
}

export function validateCalendarLiveV21Receipt(
  directory: string,
  phase: CalendarLiveV21RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateCalendarLiveV21Request(directory, request);
  const receipt = json<CalendarLiveV21Receipt>(
  ...
  if (
    receipt.artifactVersion !== "calendar-live-v21-broker-receipt-v2" ||
    receipt.brokerVersion !== CALENDAR_LIVE_V21_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Calendar live v21 accepted receipt: ${request.requestId}`,
    );
}

export function readCalendarLiveV21AcceptedRawPayload(
  directory: string,
  phase: CalendarLiveV21RemotePhase,
  captureIndex?: number,
): unknown {
  validateCalendarLiveV21Receipt(directory, phase, captureIndex);
...
export function createCalendarLiveV21FakeDynamicResponse(
  request: CalendarLiveV21Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? CALENDAR_LIVE_V21_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? CALENDAR_LIVE_V21_TARGET,
    ...overrides.extra,
  };
}

export function listCalendarLiveV21PersistedRequests(
  directory: string,
): CalendarLiveV21Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<CalendarLiveV21Request>(file)] : [];
    });
}
