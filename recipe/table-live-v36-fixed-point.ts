/**
 * Table live v36 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V36_FIXED_VERIFIER_VERSION = "table-live-v36-fixed-point-v1";
export const TABLE_LIVE_V36_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV36FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV36FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV36FixedSourceProbe {
...
export interface TableLiveV36FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV36FixedFixedPoint;
}

export interface TableLiveV36FixedHardGateInput {
  thresholds: TableLiveV36FixedThresholds;
  sourceProbes: TableLiveV36FixedSourceProbe[];
  cells: TableLiveV36FixedCellValidation[];
  sceneProofs: TableLiveV36FixedSceneProof[];
...
export interface TableLiveV36FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V36_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV36FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV36FixedHardGates(
  input: TableLiveV36FixedHardGateInput,
): TableLiveV36FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV36FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V36_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV36FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV36FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
