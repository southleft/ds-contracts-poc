...
import {
  verifyCalendarLiveV50FixedSceneFixedPoint,
  type CalendarLiveV50FixedFixedPoint,
} from "./calendar-live-v50-fixed-point.js";

export const CALENDAR_LIVE_V50_VERIFIER_VERSION = "calendar-live-v50-verifier-v1";

export function buildCalendarLiveV50RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV50RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV50RawNode[];
}

export interface CalendarLiveV50NormalizedScene {
...
export interface CalendarLiveV50SceneProof {
...
export function normalizeCalendarLiveV50Scene(
  root: CalendarLiveV50RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV50NormalizedScene {
  const canonicalBindings: CalendarLiveV50NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV50RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV50Scene<Instance>(
  normalized: CalendarLiveV50NormalizedScene,
...
): CalendarLiveV50SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV50FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV50ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV50PreCaptureGates(payload: unknown): void {
