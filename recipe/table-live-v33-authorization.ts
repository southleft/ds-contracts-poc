...
export type TableLiveV33HistoryExpectation = "pending" | "authorized";

export interface TableLiveV33AuthorizationArtifact {
  artifactVersion: "table-live-v33-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V33_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV33AntecedentIndex {
...
export interface TableLiveV33HistoryState {
  index?: TableLiveV33AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV33AuthorizationArtifact;
...
export interface TableLiveV33AuthorizationProof
  extends TableLiveV33TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V33_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V33_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V33_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V33_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v33-authorization") ||
  artifactPath.includes("table-live-v33-preflight") ||
  artifactPath.includes("table-live-v33-authorized") ||
  artifactPath.includes("create-table-live-v33-security-attestation") ||
  artifactPath.includes("table-live-pivot-v33-status") ||
...
export function validateTableLiveV33AntecedentIndex(
  index: TableLiveV33AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v33-antecedent-index-v1" ||
...
export function validateTableLiveV33UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV33History\(\s*["']/.test(source) ||
    /readTableLiveV33HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV33AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV33AuthorizationArtifact {
  ...
  const indexFailures = validateTableLiveV33AntecedentIndex(index);
...
export function validateTableLiveV33AuthorizationArtifact(
  artifact: TableLiveV33AuthorizationArtifact | undefined,
  index: TableLiveV33AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v33-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v33" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V33_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V33_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V33_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V33_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V33_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V33_SOURCE_ROOTS ||
...
export function validateTableLiveV33History(
  state: TableLiveV33HistoryState,
  expected: TableLiveV33HistoryExpectation,
): string[] {
  const failures = validateTableLiveV33AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV33AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV33HistoryState(
  root = process.cwd(),
): TableLiveV33HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V33_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV33AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V33_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V33_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V33_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV33History(
  expected: TableLiveV33HistoryExpectation,
  root = process.cwd(),
): TableLiveV33HistoryState {
  const state = readTableLiveV33HistoryState(root);
  const failures = validateTableLiveV33History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v33 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV33Authorization(
  root = process.cwd(),
): TableLiveV33AuthorizationProof {
  const state = verifyTableLiveV33History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V33_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v33/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v33.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V33_TARGET,
    expectedDynamicTool: TABLE_LIVE_V33_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V33_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV33HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV33History(expected);
  process.stdout.write(
    `Table live v33 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
