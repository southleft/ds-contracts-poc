/**
 * Carried Combobox live v6 host verifier. Uses scene-readback-combobox-v6 so live
...
export const COMBOBOX_LIVE_V6_FIXED_VERIFIER_VERSION = "combobox-live-v6-fixed-point-v1";
export const COMBOBOX_LIVE_V6_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV6FixedThresholds {
...
export interface ComboboxLiveV6FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV6FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV6FixedSourceProbe {
...
export interface ComboboxLiveV6FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV6FixedFixedPoint;
}

export interface ComboboxLiveV6FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV6FixedHardGateInput {
  thresholds: ComboboxLiveV6FixedThresholds;
  materialRegression: ComboboxLiveV6FixedMaterialRegression;
  sourceProbes: ComboboxLiveV6FixedSourceProbe[];
  cells: ComboboxLiveV6FixedCellValidation[];
  sceneProofs: ComboboxLiveV6FixedSceneProof[];
  visualRows: ComboboxLiveV6FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV6FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V6_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV6FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV6FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV6FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV6FixedHardGates(
  input: ComboboxLiveV6FixedHardGateInput,
): ComboboxLiveV6FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV6FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV6FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V6_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV6FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV6FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
