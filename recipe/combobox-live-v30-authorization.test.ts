...
import { COMBOBOX_LIVE_V30_INDEX_PATH } from "./build-combobox-live-proof-v30.js";
import {
...
import {
  ...
  validateComboboxLiveV30AntecedentIndex,
  validateComboboxLiveV30AuthorizationArtifact,
  validateComboboxLiveV30History,
  validateComboboxLiveV30UnitTestSource,
  type ComboboxLiveV30AntecedentIndex,
  type ComboboxLiveV30AuthorizationArtifact,
  type ComboboxLiveV30AuthorizationProof,
  type ComboboxLiveV30HistoryState,
} from "./combobox-live-v30-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV30ControlFlowSource,
  validateComboboxLiveV30SecurityAttestation,
  type ComboboxLiveV30SecurityAttestation,
} from "./combobox-live-v30-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v30-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V30_INDEX_PATH);
const index = (): ComboboxLiveV30AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV30AntecedentIndex;

const artifact = (): ComboboxLiveV30AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV30AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV30HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV30HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV30AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v30/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV30AuthorizationArtifact,
): ComboboxLiveV30SecurityAttestation => {
  const body: Omit<ComboboxLiveV30SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "combobox-live-v30-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V30_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV30History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV30History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV30History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV30History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV30History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV30UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV30UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV30AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV30AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV30AuthorizationArtifact(
        planted as ComboboxLiveV30AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV30SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV30SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV30SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV30ControlFlowSource(source), []);
});
