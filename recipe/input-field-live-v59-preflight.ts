...
import {
  ...
  verifyInputLiveV59Authorization,
  type InputLiveV59AntecedentIndex,
  type InputLiveV59AuthorizationProof,
} from "./input-field-live-v59-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V59_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v59/operator-security-attestation-template.json";
export const INPUT_LIVE_V59_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v59-security-attestation.json";

export interface InputLiveV59SecurityAttestation {
...
export interface InputLiveV59PreflightReport {
  artifactVersion: "input-live-v59-preflight-v1";
...
export const inputLiveV59AttestationSha256 = (
  value: Omit<InputLiveV59SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV59SecurityAttestation(
  value: unknown,
  proof: InputLiveV59AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v59-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V59_TARGET) ||
...
export function validateInputLiveV59ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV59Authorization");
  const preflight = source.indexOf("runInputLiveV59Preflight");
  const initialize = source.indexOf("InputLiveV59Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV59Preflight(
  root: string,
  proof: InputLiveV59AuthorizationProof,
  attestationPath = INPUT_LIVE_V59_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV59PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V59_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V59_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V59_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV59AntecedentIndex;
  failures.push(...validateInputLiveV59AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V59_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V59_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v59-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV59ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV59SecurityAttestation;
  return {
    artifactVersion: "input-live-v59-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V59_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV59Preflight(
    process.cwd(),
    verifyInputLiveV59Authorization(),
    attestationPath,
