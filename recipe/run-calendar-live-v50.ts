...
import {
  acceptCalendarLiveV50Response,
  createCalendarLiveV50Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV50Request,
  persistCalendarLiveV50RawResponse,
  persistCalendarLiveV50TechnicalGates,
  readCalendarLiveV50AcceptedRawPayload,
  type CalendarLiveV50Request,
  type CalendarLiveV50TransactionAuthorization,
} from "./calendar-live-v50-broker.js";
import {
  CALENDAR_LIVE_V50_CAPTURE_COUNT,
  assertCalendarLiveV50CaptureResponses,
  ...
  evaluateCalendarLiveV50Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV50Roots,
  validateCalendarLiveV50CaptureManifest,
  validateCalendarLiveV50CapturePayload,
  validateCalendarLiveV50CleanupPayload,
  validateCalendarLiveV50ExtractPayload,
  validateCalendarLiveV50ProbePayload,
  validateCalendarLiveV50WriterPayload,
  type CalendarLiveV50CaptureCell,
  type CalendarLiveV50CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V50_PLAN_PATH,
  type CalendarLiveV50ProofPlan,
...
export const CALENDAR_LIVE_V50_RUNNER_VERSION = "calendar-live-v50-runner-v1";

interface CalendarLiveV50JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV50OrchestratorState {
  artifactVersion: "calendar-live-v50-orchestrator-state-v1";
  ...
  journal: CalendarLiveV50JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV50ProofPlan;
  planSha256: string;
  captures: CalendarLiveV50CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV50SourceIdentity & {
...
export interface CalendarLiveV50NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV50OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v50 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V50_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V50_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV50ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV50CaptureCell[];
  validateCalendarLiveV50CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v50 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v50 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV50WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV50RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV50Request =>
  readJson<CalendarLiveV50Request>(file);

export class CalendarLiveV50Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV50Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV50OrchestratorState = {
      artifactVersion: "calendar-live-v50-orchestrator-state-v1",
    ...
    return new CalendarLiveV50Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV50OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV50Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV50NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV50NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v50 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV50NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v50 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV50RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV50Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV50CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV50AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV50WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV50Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV50ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV50Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV50Response(this.#directory, "writer", {
        validate: validateCalendarLiveV50WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV50Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV50RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV50Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV50ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV50Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV50RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v50-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV50Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV50ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV50Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV50ProbePayload(payload, ownership),
      });
      persistCalendarLiveV50TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v50 capture cell missing");
      const accepted = acceptCalendarLiveV50Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV50CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V50_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV50CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV50Objective(
    ...
    throw new Error(`Calendar live v50 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV50Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV50CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV50Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV50RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV50Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV50CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV50CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV50CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV50Authorization = (
  privateKey: KeyObject,
): CalendarLiveV50TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV50SimulationKey = (): KeyObject =>
