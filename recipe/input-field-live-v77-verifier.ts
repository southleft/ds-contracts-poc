...
export const INPUT_LIVE_V77_VERIFIER_VERSION = "input-live-v77-verifier-v1";

export function buildInputLiveV77RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV77RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV77RawNode[];
}

export interface InputLiveV77NormalizedScene {
...
export interface InputLiveV77SceneProof {
...
export function normalizeInputLiveV77Scene(
  root: InputLiveV77RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV77NormalizedScene {
  const canonicalBindings: InputLiveV77NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV77RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV77Scene<Instance>(
  normalized: InputLiveV77NormalizedScene,
...
export function compileInputLiveV77ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV77PreCaptureGates(payload: unknown): void {
