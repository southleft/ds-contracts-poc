...
import {
  ...
  verifyInputLiveV25Authorization,
  type InputLiveV25AntecedentIndex,
  type InputLiveV25AuthorizationProof,
} from "./input-field-live-v25-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V25_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v25/operator-security-attestation-template.json";
export const INPUT_LIVE_V25_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v25-security-attestation.json";

export interface InputLiveV25SecurityAttestation {
...
export interface InputLiveV25PreflightReport {
  artifactVersion: "input-live-v25-preflight-v1";
...
export const inputLiveV25AttestationSha256 = (
  value: Omit<InputLiveV25SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV25SecurityAttestation(
  value: unknown,
  proof: InputLiveV25AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v25-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V25_TARGET) ||
...
export function validateInputLiveV25ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV25Authorization");
  const preflight = source.indexOf("runInputLiveV25Preflight");
  const initialize = source.indexOf("InputLiveV25Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV25Preflight(
  root: string,
  proof: InputLiveV25AuthorizationProof,
  attestationPath = INPUT_LIVE_V25_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV25PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V25_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V25_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V25_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV25AntecedentIndex;
  failures.push(...validateInputLiveV25AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V25_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V25_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v25-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV25ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV25SecurityAttestation;
  return {
    artifactVersion: "input-live-v25-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V25_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV25Preflight(
    process.cwd(),
    verifyInputLiveV25Authorization(),
    attestationPath,
