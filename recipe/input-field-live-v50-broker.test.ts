...
import {
  ...
  createInputLiveV50FakeDynamicResponse,
  ...
  listInputLiveV50PersistedRequests,
  persistInputLiveV50RawResponse,
  persistInputLiveV50TechnicalGates,
  validateInputLiveV50Receipt,
  validateInputLiveV50Request,
  type InputLiveV50RemotePhase,
...
import {
  ...
  type InputLiveV50ExtractPayload,
  type InputLiveV50ProbePayload,
...
import {
  ...
  simulatedInputLiveV50Authorization,
} from "./run-input-field-live-v50.js";
...
const captures: InputLiveV50CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV50Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v50-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV50Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV50CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV50FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV50FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V50_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V50_TARGET);
  persistInputLiveV50RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV50Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV50Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV50WriterOwnership,
): InputLiveV50RestorePayload => ({
...
const rootProofs = (): InputLiveV50RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v50-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV50Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV50Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV50RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV50RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV50ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV50ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV50CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV50CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV50CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV50Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV50CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV50PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV50ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV50Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV50Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV50Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV50Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V50_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV50RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V50_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV50Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV50Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV50ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV50Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV50WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV50RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV50Request;
    validateInputLiveV50Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV50Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV50CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v50-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV50Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV50Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV50Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V50_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV50RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V50_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV50Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV50Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV50RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V50_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV50Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV50Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV50Request;
