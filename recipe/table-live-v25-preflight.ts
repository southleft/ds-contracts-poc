...
import {
  validateTableLiveV25AntecedentIndex,
  verifyTableLiveV25Authorization,
  type TableLiveV25AntecedentIndex,
  type TableLiveV25AuthorizationProof,
} from "./table-live-v25-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V25_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v25/operator-security-attestation-template.json";
export const TABLE_LIVE_V25_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v25-security-attestation.json";

export interface TableLiveV25SecurityAttestation {
  artifactVersion: "table-live-v25-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV25PreflightReport {
  artifactVersion: "table-live-v25-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV25SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV25SecurityAttestation(
  value: unknown,
  proof: TableLiveV25AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v25-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V25_TARGET) ||
...
export function validateTableLiveV25ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV25Authorization");
  const preflight = source.indexOf("runTableLiveV25Preflight");
  const initialize = source.indexOf("TableLiveV25Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV25Preflight(
  root: string,
  proof: TableLiveV25AuthorizationProof,
  attestationPath = TABLE_LIVE_V25_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV25PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V25_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V25_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV25AntecedentIndex;
  failures.push(...validateTableLiveV25AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V25_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V25_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV25SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v25-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV25ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v25 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV25SecurityAttestation;
  return {
    artifactVersion: "table-live-v25-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V25_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV25Preflight(
    process.cwd(),
    verifyTableLiveV25Authorization(),
    attestationPath,
