...
import {
  acceptTableLiveV15Response,
  createTableLiveV15Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV15Request,
  persistTableLiveV15RawResponse,
  persistTableLiveV15TechnicalGates,
  readTableLiveV15AcceptedRawPayload,
  type TableLiveV15Request,
  type TableLiveV15TransactionAuthorization,
} from "./table-live-v15-broker.js";
import {
  TABLE_LIVE_V15_CAPTURE_COUNT,
  assertTableLiveV15CaptureResponses,
  ...
  evaluateTableLiveV15Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV15Roots,
  validateTableLiveV15CaptureManifest,
  validateTableLiveV15CapturePayload,
  validateTableLiveV15CleanupPayload,
  validateTableLiveV15ExtractPayload,
  validateTableLiveV15ProbePayload,
  validateTableLiveV15WriterPayload,
  type TableLiveV15CaptureCell,
  type TableLiveV15CapturePayload,
...
import {
  ...
  TABLE_LIVE_V15_PLAN_PATH,
  type TableLiveV15ProofPlan,
} from "./build-table-live-proof-v15.js";
import {
...
export const TABLE_LIVE_V15_RUNNER_VERSION = "table-live-v15-runner-v1";

interface TableLiveV15JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV15OrchestratorState {
  artifactVersion: "table-live-v15-orchestrator-state-v1";
  ...
  journal: TableLiveV15JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV15ProofPlan;
  planSha256: string;
  captures: TableLiveV15CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV15SourceIdentity & {
...
export interface TableLiveV15NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV15OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v15 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V15_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V15_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV15ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV15CaptureCell[];
  validateTableLiveV15CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v15 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v15 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV15WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV15RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV15Request =>
  readJson<TableLiveV15Request>(file);

export class TableLiveV15Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV15Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV15OrchestratorState = {
      artifactVersion: "table-live-v15-orchestrator-state-v1",
    ...
    return new TableLiveV15Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV15OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV15Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV15NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV15NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v15 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV15NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v15 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV15RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV15Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV15CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV15AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV15WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV15Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV15ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV15Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV15Response(this.#directory, "writer", {
        validate: validateTableLiveV15WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV15Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV15RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV15Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV15ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV15Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV15RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v15-host-root-proofs-v1",
      ...
      const next = issueTableLiveV15Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV15ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV15Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV15ProbePayload(payload, ownership),
      });
      persistTableLiveV15TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v15 capture cell missing");
      const accepted = acceptTableLiveV15Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV15CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V15_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV15CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV15Objective(
    ...
    throw new Error(`Table live v15 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV15Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV15CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV15Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV15RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV15Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV15CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV15CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV15CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV15Authorization = (
  privateKey: KeyObject,
): TableLiveV15TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV15SimulationKey = (): KeyObject =>
