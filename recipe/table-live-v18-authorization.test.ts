...
import { TABLE_LIVE_V18_INDEX_PATH } from "./build-table-live-proof-v18.js";
import {
...
import {
  buildTableLiveV18AuthorizationArtifact,
  validateTableLiveV18AntecedentIndex,
  validateTableLiveV18AuthorizationArtifact,
  validateTableLiveV18History,
  validateTableLiveV18UnitTestSource,
  type TableLiveV18AntecedentIndex,
  type TableLiveV18AuthorizationArtifact,
  type TableLiveV18AuthorizationProof,
  type TableLiveV18HistoryState,
} from "./table-live-v18-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV18ControlFlowSource,
  validateTableLiveV18SecurityAttestation,
  type TableLiveV18SecurityAttestation,
} from "./table-live-v18-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V18_INDEX_PATH);
const index = (): TableLiveV18AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV18AntecedentIndex;

const artifact = (): TableLiveV18AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV18AuthorizationArtifact({
...
const pendingState = (): TableLiveV18HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV18HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV18AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v18/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV18AuthorizationArtifact,
): TableLiveV18SecurityAttestation => {
  const body: Omit<TableLiveV18SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V18_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV18History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV18History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV18History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV18History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV18History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV18UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v18-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV18UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV18AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV18AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV18AuthorizationArtifact(
        planted as TableLiveV18AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV18SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV18SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV18SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v18-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV18ControlFlowSource(source), []);
});
