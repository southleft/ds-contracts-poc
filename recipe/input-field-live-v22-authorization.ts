...
import {
  ...
  INPUT_LIVE_V22_INDEX_PATH,
} from "./build-input-field-live-proof-v22.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV22HistoryExpectation = "pending" | "authorized";

export interface InputLiveV22AuthorizationArtifact {
  artifactVersion: "input-live-v22-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V22_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV22AntecedentIndex {
...
export interface InputLiveV22HistoryState {
  index?: InputLiveV22AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV22AuthorizationArtifact;
...
export interface InputLiveV22AuthorizationProof extends InputLiveV22TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V22_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V22_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V22_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V22_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v22-status") ||
...
export function validateInputLiveV22AntecedentIndex(
  index: InputLiveV22AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v22-antecedent-index-v1" ||
...
export function validateInputLiveV22UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV22History\(\s*["']/.test(source) ||
    /readInputLiveV22HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV22AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV22AuthorizationArtifact {
  ...
  ) as InputLiveV22AntecedentIndex;
  const indexFailures = validateInputLiveV22AntecedentIndex(index);
...
export function validateInputLiveV22AuthorizationArtifact(
  artifact: InputLiveV22AuthorizationArtifact | undefined,
  index: InputLiveV22AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v22-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v22" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V22_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V22_TARGET) ||
    !equalJson(
...
export function validateInputLiveV22History(
  state: InputLiveV22HistoryState,
  expected: InputLiveV22HistoryExpectation,
): string[] {
  const failures = validateInputLiveV22AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV22AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV22HistoryState(
  root = process.cwd(),
): InputLiveV22HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V22_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV22AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V22_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V22_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V22_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV22History(
  expected: InputLiveV22HistoryExpectation,
  root = process.cwd(),
): InputLiveV22HistoryState {
  const state = readInputLiveV22HistoryState(root);
  const failures = validateInputLiveV22History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV22Authorization(
  root = process.cwd(),
): InputLiveV22AuthorizationProof {
  const state = verifyInputLiveV22History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V22_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v22/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v22.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V22_TARGET,
    expectedDynamicTool: INPUT_LIVE_V22_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V22_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV22HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV22History(expected);
