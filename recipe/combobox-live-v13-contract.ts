...
import { buildComboboxLiveV13CleanupRuntime } from "./combobox-live-v13-cleanup.js";
import {
  type ComboboxLiveV13FixedFixedPoint,
} from "./combobox-live-v13-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV13RestoreProgram,
  validateComboboxLiveV13RestorePayload,
  type ComboboxLiveV13RestorePayload,
} from "./combobox-live-v13-restore.js";
import {
  buildComboboxLiveV13RawPropertyRuntime,
  normalizeComboboxLiveV13Scene,
  type ComboboxLiveV13RawNode,
} from "./combobox-live-v13-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v13.js";
import {
...
export const COMBOBOX_LIVE_V13_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V13_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V13_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V13_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V13_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V13_SET_COUNT = 4;
export const COMBOBOX_LIVE_V13_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V13_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V13_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V13_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V13_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV13SourceId = (typeof COMBOBOX_LIVE_V13_SOURCE_IDS)[number];

export interface ComboboxLiveV13SourceIdentity {
  source: ComboboxLiveV13SourceId;
...
export interface ComboboxLiveV13WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV13ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV13SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV13RawNode;
    optionScene: ComboboxLiveV13RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV13RootProof {
  source: ComboboxLiveV13SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV13FixedFixedPoint;
}

export interface ComboboxLiveV13ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV13SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV13CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV13SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV13CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV13SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV13CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v13 ${label} must be an object`);
...
export function validateComboboxLiveV13WriterPayload(
  value: unknown,
): ComboboxLiveV13WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V13_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v13 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v13 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v13 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV13SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV13ExtractProgram(
  writer: ComboboxLiveV13WriterOwnership,
  sources: readonly ComboboxLiveV13SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v13 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V13_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V13_NAMESPACE)}
${buildComboboxLiveV13RawPropertyRuntime()}
...
export function validateComboboxLiveV13ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV13WriterOwnership,
): ComboboxLiveV13ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v13 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V13_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v13 extract schema/two-root mismatch");
  return payload as ComboboxLiveV13ExtractPayload;
}

export function proveComboboxLiveV13Roots<Instance>(
  extract: ComboboxLiveV13ExtractPayload,
  sources: readonly (ComboboxLiveV13SourceIdentity & {
...
): ComboboxLiveV13RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v13 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v13 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV13Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV13Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV13FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v13 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v13 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v13 compile lost owned sets");
...
export function assertComboboxLiveV13RootProofs(
  proofs: readonly ComboboxLiveV13RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v13 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v13 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV13ProbeProgram(
  writer: ComboboxLiveV13WriterOwnership,
  sources: readonly ComboboxLiveV13SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V13_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV13ProbePayload(
  value: unknown,
  writer: ComboboxLiveV13WriterOwnership,
): ComboboxLiveV13ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV13ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V13_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V13_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v13 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV13CaptureManifest(
  cells: readonly ComboboxLiveV13CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V13_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V13_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V13_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V13_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v13 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV13CaptureCell[],
): string => {
  validateComboboxLiveV13CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV13CaptureProgram(
  writer: ComboboxLiveV13WriterOwnership,
  cell: ComboboxLiveV13CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V13_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v13 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v13 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V13_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV13CapturePayload(
  value: unknown,
  cell: ComboboxLiveV13CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV13CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV13CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V13_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V13_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v13 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV13CaptureResponses(
  manifest: readonly ComboboxLiveV13CaptureCell[],
  responses: readonly ComboboxLiveV13CapturePayload[],
): void {
  validateComboboxLiveV13CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v13 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV13ObjectiveReport {
  artifactVersion: "combobox-live-v13-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV13Objective(
  manifest: readonly ComboboxLiveV13CaptureCell[],
  responses: readonly ComboboxLiveV13CapturePayload[],
): ComboboxLiveV13ObjectiveReport {
  assertComboboxLiveV13CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v13-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV13CleanupProgram(
  writer: ComboboxLiveV13WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV13CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V13_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v13 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV13CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV13WriterOwnership,
): ComboboxLiveV13CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV13CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v13 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V13_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV13WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
