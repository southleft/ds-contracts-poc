...
} from "./input-field-live-v3-verifier-v73.js";
import {
  buildInputLiveV73RawPropertyRuntime,
  normalizeInputLiveV73Scene,
  type InputLiveV73RawNode,
} from "./input-field-live-v73-verifier.js";
...
export const INPUT_LIVE_V73_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V73_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V73_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V73_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V73_VARIANT_COUNT = 256;
export const INPUT_LIVE_V73_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V73_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV73SourceId = (typeof INPUT_LIVE_V73_SOURCE_IDS)[number];

export interface InputLiveV73SourceIdentity {
  source: InputLiveV73SourceId;
...
export interface InputLiveV73WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV73ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV73SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV73RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV73RootProof {
  source: InputLiveV73SourceId;
  adapterIdentity: string;
...
export interface InputLiveV73ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV73SourceId;
    adapterIdentity: string;
...
export interface InputLiveV73CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV73SourceId;
  adapterIdentity: string;
...
export interface InputLiveV73CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV73SourceId;
  strata: InputLiveV73CaptureCell["strata"];
...
export interface InputLiveV73CleanupPayload {
...
export function validateInputLiveV73WriterPayload(
  value: unknown,
): InputLiveV73WriterOwnership {
...
const identityPlan = (source: InputLiveV73SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV73ExtractProgram(
  writer: InputLiveV73WriterOwnership,
  sources: readonly InputLiveV73SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V73_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V73_NAMESPACE)}
${buildInputLiveV73RawPropertyRuntime()}
...
export function validateInputLiveV73ExtractPayload(
  value: unknown,
  writer: InputLiveV73WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V73_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV73ExtractPayload;
}

export function proveInputLiveV73Roots<Instance>(
  extract: InputLiveV73ExtractPayload,
  sources: readonly (InputLiveV73SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV73Scene(
      root.scene,
...
export function assertInputLiveV73RootProofs(
  proofs: readonly InputLiveV73RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV73ProbeProgram(
  writer: InputLiveV73WriterOwnership,
  sources: readonly InputLiveV73SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V73_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV73ProbePayload(
  value: unknown,
  writer: InputLiveV73WriterOwnership,
): InputLiveV73ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV73ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V73_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V73_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV73CaptureManifest(
  cells: readonly InputLiveV73CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V73_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V73_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V73_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V73_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V73_SOURCE_IDS.some(
...
export const inputLiveV73CaptureManifestSha256 = (
  cells: readonly InputLiveV73CaptureCell[],
): string => {
  validateInputLiveV73CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV73CaptureProgram(
  writer: InputLiveV73WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V73_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V73_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V73_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV73CapturePayload(
  value: unknown,
  cell: InputLiveV73CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV73CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V73_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V73_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV73CaptureResponses(
  manifest: readonly InputLiveV73CaptureCell[],
  responses: readonly InputLiveV73CapturePayload[],
): void {
  validateInputLiveV73CaptureManifest(manifest);
  if (
...
export interface InputLiveV73ObjectiveReport {
  artifactVersion: "input-live-v73-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV73Objective(
  manifest: readonly InputLiveV73CaptureCell[],
  responses: readonly InputLiveV73CapturePayload[],
  readReference: (cell: InputLiveV73CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV73CaptureCell) => Buffer,
): InputLiveV73ObjectiveReport {
  assertInputLiveV73CaptureResponses(manifest, responses);
...
export function buildInputLiveV73CleanupProgram(
  writer: InputLiveV73WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V73_NAMESPACE,
...
export function validateInputLiveV73CleanupPayload(
  value: unknown,
  writer: InputLiveV73WriterOwnership,
): InputLiveV73CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV73CleanupPayload;
...
export const INPUT_LIVE_V73_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV73WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
