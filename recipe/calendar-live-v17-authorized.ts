...
import { CALENDAR_LIVE_V17_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v17.js";
import { verifyCalendarLiveV17Authorization } from "./calendar-live-v17-authorization.js";
import { runCalendarLiveV17Preflight } from "./calendar-live-v17-preflight.js";
import { CalendarLiveV17Orchestrator } from "./run-calendar-live-v17.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v17 requires exact authorization artifact ${CALENDAR_LIVE_V17_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v17-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV17Authorization(root);
    runCalendarLiveV17Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV17Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV17Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
