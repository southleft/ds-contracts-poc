...
import { verifyTableLiveV14Authorization } from "./table-live-v14-authorization.js";
import { TABLE_LIVE_V14_TARGET } from "./table-live-v14-broker.js";
import {
  TABLE_LIVE_V14_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV14Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-table-v14-session";
...
const attestation: TableLiveV14SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV14SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v14 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V14_SECURITY_ATTESTATION_DEFAULT_PATH,
);
