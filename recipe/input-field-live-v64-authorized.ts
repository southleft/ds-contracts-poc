...
import { INPUT_LIVE_V64_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v64.js";
import { verifyInputLiveV64Authorization } from "./input-field-live-v64-authorization.js";
import { runInputLiveV64Preflight } from "./input-field-live-v64-preflight.js";
import { InputLiveV64Orchestrator } from "./run-input-field-live-v64.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V64_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v64-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV64Authorization(root);
    runInputLiveV64Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV64Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
