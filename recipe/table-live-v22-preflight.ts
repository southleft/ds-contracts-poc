...
import {
  validateTableLiveV22AntecedentIndex,
  verifyTableLiveV22Authorization,
  type TableLiveV22AntecedentIndex,
  type TableLiveV22AuthorizationProof,
} from "./table-live-v22-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V22_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v22/operator-security-attestation-template.json";
export const TABLE_LIVE_V22_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v22-security-attestation.json";

export interface TableLiveV22SecurityAttestation {
  artifactVersion: "table-live-v22-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV22PreflightReport {
  artifactVersion: "table-live-v22-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV22SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV22SecurityAttestation(
  value: unknown,
  proof: TableLiveV22AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v22-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V22_TARGET) ||
...
export function validateTableLiveV22ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV22Authorization");
  const preflight = source.indexOf("runTableLiveV22Preflight");
  const initialize = source.indexOf("TableLiveV22Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV22Preflight(
  root: string,
  proof: TableLiveV22AuthorizationProof,
  attestationPath = TABLE_LIVE_V22_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV22PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V22_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V22_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV22AntecedentIndex;
  failures.push(...validateTableLiveV22AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V22_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V22_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV22SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v22-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV22ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v22 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV22SecurityAttestation;
  return {
    artifactVersion: "table-live-v22-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V22_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV22Preflight(
    process.cwd(),
    verifyTableLiveV22Authorization(),
    attestationPath,
