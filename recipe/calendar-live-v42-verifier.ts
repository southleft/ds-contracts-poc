...
import {
  verifyCalendarLiveV42FixedSceneFixedPoint,
  type CalendarLiveV42FixedFixedPoint,
} from "./calendar-live-v42-fixed-point.js";

export const CALENDAR_LIVE_V42_VERIFIER_VERSION = "calendar-live-v42-verifier-v1";

export function buildCalendarLiveV42RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV42RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV42RawNode[];
}

export interface CalendarLiveV42NormalizedScene {
...
export interface CalendarLiveV42SceneProof {
...
export function normalizeCalendarLiveV42Scene(
  root: CalendarLiveV42RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV42NormalizedScene {
  const canonicalBindings: CalendarLiveV42NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV42RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV42Scene<Instance>(
  normalized: CalendarLiveV42NormalizedScene,
...
): CalendarLiveV42SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV42FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV42ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV42PreCaptureGates(payload: unknown): void {
