...
import { buildComboboxLiveV27CleanupRuntime } from "./combobox-live-v27-cleanup.js";
import {
  type ComboboxLiveV27FixedFixedPoint,
} from "./combobox-live-v27-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV27RestoreProgram,
  validateComboboxLiveV27RestorePayload,
  type ComboboxLiveV27RestorePayload,
} from "./combobox-live-v27-restore.js";
import {
  buildComboboxLiveV27RawPropertyRuntime,
  normalizeComboboxLiveV27Scene,
  type ComboboxLiveV27RawNode,
} from "./combobox-live-v27-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v27.js";
import {
...
} from "./scene-readback-combobox-v27.js";
...
export const COMBOBOX_LIVE_V27_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V27_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V27_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V27_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V27_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V27_SET_COUNT = 4;
export const COMBOBOX_LIVE_V27_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V27_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V27_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V27_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V27_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV27SourceId = (typeof COMBOBOX_LIVE_V27_SOURCE_IDS)[number];

export interface ComboboxLiveV27SourceIdentity {
  source: ComboboxLiveV27SourceId;
...
export interface ComboboxLiveV27WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV27ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV27SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV27RawNode;
    optionScene: ComboboxLiveV27RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV27RootProof {
  source: ComboboxLiveV27SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV27FixedFixedPoint;
}

export interface ComboboxLiveV27ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV27SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV27CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV27SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV27CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV27SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV27CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v27 ${label} must be an object`);
...
export function validateComboboxLiveV27WriterPayload(
  value: unknown,
): ComboboxLiveV27WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V27_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v27 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v27 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v27 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV27SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV27ExtractProgram(
  writer: ComboboxLiveV27WriterOwnership,
  sources: readonly ComboboxLiveV27SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v27 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V27_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V27_NAMESPACE)}
${buildComboboxLiveV27RawPropertyRuntime()}
...
export function validateComboboxLiveV27ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV27WriterOwnership,
): ComboboxLiveV27ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v27 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V27_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v27 extract schema/two-root mismatch");
  return payload as ComboboxLiveV27ExtractPayload;
}

export function proveComboboxLiveV27Roots<Instance>(
  extract: ComboboxLiveV27ExtractPayload,
  sources: readonly (ComboboxLiveV27SourceIdentity & {
...
): ComboboxLiveV27RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v27 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v27 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV27Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV27Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV27FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v27 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v27 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v27 compile lost owned sets");
...
export function assertComboboxLiveV27RootProofs(
  proofs: readonly ComboboxLiveV27RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v27 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v27 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV27ProbeProgram(
  writer: ComboboxLiveV27WriterOwnership,
  sources: readonly ComboboxLiveV27SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V27_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV27ProbePayload(
  value: unknown,
  writer: ComboboxLiveV27WriterOwnership,
): ComboboxLiveV27ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV27ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V27_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V27_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v27 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV27CaptureManifest(
  cells: readonly ComboboxLiveV27CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V27_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V27_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V27_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V27_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v27 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV27CaptureCell[],
): string => {
  validateComboboxLiveV27CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV27CaptureProgram(
  writer: ComboboxLiveV27WriterOwnership,
  cell: ComboboxLiveV27CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V27_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v27 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v27 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V27_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV27CapturePayload(
  value: unknown,
  cell: ComboboxLiveV27CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV27CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV27CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V27_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V27_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v27 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV27CaptureResponses(
  manifest: readonly ComboboxLiveV27CaptureCell[],
  responses: readonly ComboboxLiveV27CapturePayload[],
): void {
  validateComboboxLiveV27CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v27 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV27ObjectiveReport {
  artifactVersion: "combobox-live-v27-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV27Objective(
  manifest: readonly ComboboxLiveV27CaptureCell[],
  responses: readonly ComboboxLiveV27CapturePayload[],
): ComboboxLiveV27ObjectiveReport {
  assertComboboxLiveV27CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v27-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV27CleanupProgram(
  writer: ComboboxLiveV27WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV27CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V27_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v27 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV27CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV27WriterOwnership,
): ComboboxLiveV27CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV27CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v27 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V27_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV27WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
