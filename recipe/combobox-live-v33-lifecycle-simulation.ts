...
export interface ComboboxLiveV33LifecycleSimulationReport {
  artifactVersion: "combobox-live-v33-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV33AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV33Lifecycle(): Promise<ComboboxLiveV33LifecycleSimulationReport> {
  await buildComboboxLiveV33Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V33_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV33AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v33-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V33_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V33_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV33History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV33History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV33AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V33_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V33_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV33History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV33History(authorized, "pending");
    ...
    await buildComboboxLiveV33Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V33_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV33AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV33Lifecycle(), null, 2)}\n`,
  );
