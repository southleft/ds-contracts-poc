...
import {
  ...
  verifyInputLiveV67Authorization,
  type InputLiveV67AntecedentIndex,
  type InputLiveV67AuthorizationProof,
} from "./input-field-live-v67-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V67_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v67/operator-security-attestation-template.json";
export const INPUT_LIVE_V67_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v67-security-attestation.json";

export interface InputLiveV67SecurityAttestation {
...
export interface InputLiveV67PreflightReport {
  artifactVersion: "input-live-v67-preflight-v1";
...
export const inputLiveV67AttestationSha256 = (
  value: Omit<InputLiveV67SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV67SecurityAttestation(
  value: unknown,
  proof: InputLiveV67AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v67-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V67_TARGET) ||
...
export function validateInputLiveV67ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV67Authorization");
  const preflight = source.indexOf("runInputLiveV67Preflight");
  const initialize = source.indexOf("InputLiveV67Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV67Preflight(
  root: string,
  proof: InputLiveV67AuthorizationProof,
  attestationPath = INPUT_LIVE_V67_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV67PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V67_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V67_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V67_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV67AntecedentIndex;
  failures.push(...validateInputLiveV67AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V67_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V67_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v67-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV67ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV67SecurityAttestation;
  return {
    artifactVersion: "input-live-v67-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V67_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV67Preflight(
    process.cwd(),
    verifyInputLiveV67Authorization(),
    attestationPath,
