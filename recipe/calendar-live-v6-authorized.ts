...
import { CALENDAR_LIVE_V6_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v6.js";
import { verifyCalendarLiveV6Authorization } from "./calendar-live-v6-authorization.js";
import { runCalendarLiveV6Preflight } from "./calendar-live-v6-preflight.js";
import { CalendarLiveV6Orchestrator } from "./run-calendar-live-v6.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v6 requires exact authorization artifact ${CALENDAR_LIVE_V6_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v6-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV6Authorization(root);
    runCalendarLiveV6Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV6Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV6Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
