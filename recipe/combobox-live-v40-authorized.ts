...
import { COMBOBOX_LIVE_V40_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v40.js";
import { verifyComboboxLiveV40Authorization } from "./combobox-live-v40-authorization.js";
import { runComboboxLiveV40Preflight } from "./combobox-live-v40-preflight.js";
import { ComboboxLiveV40Orchestrator } from "./run-combobox-live-v40.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v40 requires exact authorization artifact ${COMBOBOX_LIVE_V40_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v40-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV40Authorization(root);
    runComboboxLiveV40Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV40Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV40Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
