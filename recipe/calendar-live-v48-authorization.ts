...
export type CalendarLiveV48HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV48AuthorizationArtifact {
  artifactVersion: "calendar-live-v48-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V48_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV48AntecedentIndex {
...
export interface CalendarLiveV48HistoryState {
  index?: CalendarLiveV48AntecedentIndex;
  indexSha256: string;
...
export interface CalendarLiveV48AuthorizationProof
  extends CalendarLiveV48TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V48_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V48_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V48_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V48_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v48-authorization") ||
  artifactPath.includes("calendar-live-v48-preflight") ||
  artifactPath.includes("calendar-live-v48-authorized") ||
  artifactPath.includes("create-calendar-live-v48-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v48-status") ||
...
export function validateCalendarLiveV48AntecedentIndex(
  index: CalendarLiveV48AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v48-antecedent-index-v1" ||
...
export function validateCalendarLiveV48UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV48History\(\s*["']/.test(source) ||
    /readCalendarLiveV48HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV48AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV48AuthorizationArtifact {
  ...
  ) as CalendarLiveV48AntecedentIndex;
  const indexFailures = validateCalendarLiveV48AntecedentIndex(index);
...
export function validateCalendarLiveV48AuthorizationArtifact(
  artifact: CalendarLiveV48AuthorizationArtifact | undefined,
  index: CalendarLiveV48AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v48-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v48" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V48_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V48_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V48_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V48_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V48_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V48_SOURCE_ROOTS ||
  ...
  if (
    ...
    artifact?.execution?.comboboxLiveV48AuthorizationReusable !== false ||
...
export function validateCalendarLiveV48History(
  state: CalendarLiveV48HistoryState,
  expected: CalendarLiveV48HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV48AntecedentIndex(state.index);
...
export function readCalendarLiveV48HistoryState(
  root = process.cwd(),
): CalendarLiveV48HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V48_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV48AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V48_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V48_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V48_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV48History(
  expected: CalendarLiveV48HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV48HistoryState {
  const state = readCalendarLiveV48HistoryState(root);
  const failures = validateCalendarLiveV48History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v48 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV48Authorization(
  root = process.cwd(),
): CalendarLiveV48AuthorizationProof {
  const state = verifyCalendarLiveV48History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V48_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v48/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v48.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V48_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V48_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V48_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV48HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV48History(expected);
  process.stdout.write(
    `Calendar live v48 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
