...
import {
  assertInputLiveV52RootProofs,
  INPUT_LIVE_V52_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V52_RESPONSE_CONTRACTS,
  type InputLiveV52RootProof,
} from "./input-field-live-v52-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V52_BROKER_VERSION = "input-live-v52-broker-v2";
export const INPUT_LIVE_V52_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V52_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V52_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V52_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V52_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V52_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV52RemotePhase =
  (typeof INPUT_LIVE_V52_REMOTE_PHASES)[number];
export type InputLiveV52AuthorizationMode = "live" | "simulated";

export interface InputLiveV52CommitPins {
...
export interface InputLiveV52TransactionAuthorization extends InputLiveV52CommitPins {
  mode: InputLiveV52AuthorizationMode;
...
export interface InputLiveV52Manifest {
  artifactVersion: "input-live-v52-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V52_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V52_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V52_TARGET;
  authorization: InputLiveV52TransactionAuthorization;
...
export interface InputLiveV52ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV52RequestBody {
  artifactVersion: "input-live-v52-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V52_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV52RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV52CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V52_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV52ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV52Request extends InputLiveV52RequestBody {
...
export interface InputLiveV52Receipt {
  artifactVersion: "input-live-v52-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V52_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV52RemotePhase;
...
export interface InputLiveV52AcceptedResponse<T = unknown> {
  request: InputLiveV52Request;
  receipt: InputLiveV52Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV52IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV52ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V52_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V52_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V52_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V52_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V52_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V52_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V52_TARGET.editorType;
};
...
export const inputLiveV52RequestSequence = (
  phase: InputLiveV52RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV52RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV52RequestPath = (
  directory: string,
  phase: InputLiveV52RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV52ResponsePath = (
  directory: string,
  phase: InputLiveV52RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV52ReceiptPath = (
  directory: string,
  phase: InputLiveV52RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV52Manifest => {
  const manifest = json<InputLiveV52Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v52-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V52_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V52_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV52RemotePhase,
  captureIndex?: number,
): InputLiveV52Request | undefined => {
  const file = inputLiveV52RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV52Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV52RemotePhase,
  captureIndex?: number,
): InputLiveV52Receipt | undefined => {
  const file = inputLiveV52ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV52Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV52Request | undefined;
  receipt: InputLiveV52Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV52RemotePhase,
): InputLiveV52ResponseContract => ({
  ...INPUT_LIVE_V52_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V52_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V52_DYNAMIC_TOOL,
      target: INPUT_LIVE_V52_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v52-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V52_BROKER_VERSION,
  target: INPUT_LIVE_V52_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V52_PROGRAM_START*/${program}/*INPUT_LIVE_V52_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V52_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V52_PROGRAM_END*/";
...
export function createInputLiveV52Transaction(
  directory: string,
  ...
  const manifest: InputLiveV52Manifest = {
    artifactVersion: "input-live-v52-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V52_BROKER_VERSION,
...
export function validateInputLiveV52Request(
  directory: string,
  request: InputLiveV52Request,
): void {
  ...
  const expectedSequence = inputLiveV52RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v52-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V52_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V52_TARGET.fileKey ||
...
export function issueInputLiveV52Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV52RemotePhase,
  program: string,
  options: InputLiveV52IssueOptions = {},
): InputLiveV52Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV52RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV52Request>(targetPath);
    validateInputLiveV52Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV52ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV52TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV52RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V52_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V52_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V52_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV52RequestBody = {
    artifactVersion: "input-live-v52-broker-request-v2",
    brokerVersion: INPUT_LIVE_V52_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV52Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV52Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV52RawResponse(
  directory: string,
  phase: InputLiveV52RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV52RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV52Request>(requestFile);
  if (
  ...
  const target = inputLiveV52ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV52Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v52-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V52_BROKER_VERSION,
...
export function acceptInputLiveV52Response<T = unknown>(
  directory: string,
  phase: InputLiveV52RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV52AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV52ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV52Request>(
    inputLiveV52RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV52Request(directory, request);
  if (phase === "capture") validateInputLiveV52TechnicalGates(directory);
  const rawFile = inputLiveV52ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v52-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V52_BROKER_VERSION ||
...
export function persistInputLiveV52TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV52RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV52RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V52_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v52-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V52_BROKER_VERSION,
...
export function validateInputLiveV52TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v52-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V52_TECHNICAL_GATES.some(
...
export function validateInputLiveV52Receipt(
  directory: string,
  phase: InputLiveV52RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV52Request(directory, request);
  const receipt = json<InputLiveV52Receipt>(
    inputLiveV52ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV52ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v52-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V52_BROKER_VERSION ||
...
export function readInputLiveV52AcceptedRawPayload(
  directory: string,
  phase: InputLiveV52RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV52Receipt(directory, phase, captureIndex);
...
export function createInputLiveV52FakeDynamicResponse(
  request: InputLiveV52Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V52_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V52_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV52PersistedRequests(
  directory: string,
): InputLiveV52Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV52Request>(file)] : [];
    });
}

export const inputLiveV52Sha256 = sha256;
