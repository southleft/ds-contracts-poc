...
export const INPUT_LIVE_V25_VERIFIER_VERSION = "input-live-v25-verifier-v1";

export function buildInputLiveV25RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV25RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV25RawNode[];
}

export interface InputLiveV25NormalizedScene {
...
export interface InputLiveV25SceneProof {
...
export function normalizeInputLiveV25Scene(
  root: InputLiveV25RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV25NormalizedScene {
  const canonicalBindings: InputLiveV25NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV25RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV25Scene<Instance>(
  normalized: InputLiveV25NormalizedScene,
...
export function compileInputLiveV25ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV25PreCaptureGates(payload: unknown): void {
