...
export const INPUT_LIVE_V67_VERIFIER_VERSION = "input-live-v67-verifier-v1";

export function buildInputLiveV67RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV67RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV67RawNode[];
}

export interface InputLiveV67NormalizedScene {
...
export interface InputLiveV67SceneProof {
...
export function normalizeInputLiveV67Scene(
  root: InputLiveV67RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV67NormalizedScene {
  const canonicalBindings: InputLiveV67NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV67RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV67Scene<Instance>(
  normalized: InputLiveV67NormalizedScene,
...
export function compileInputLiveV67ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV67PreCaptureGates(payload: unknown): void {
