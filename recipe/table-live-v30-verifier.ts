...
import {
  verifyTableLiveV30FixedSceneFixedPoint,
  type TableLiveV30FixedFixedPoint,
} from "./table-live-v30-fixed-point.js";

export const TABLE_LIVE_V30_VERIFIER_VERSION = "table-live-v30-verifier-v1";

export function buildTableLiveV30RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV30RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV30RawNode[];
}

export interface TableLiveV30NormalizedScene {
...
export interface TableLiveV30SceneProof {
...
export function normalizeTableLiveV30Scene(
  root: TableLiveV30RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV30NormalizedScene {
  const canonicalBindings: TableLiveV30NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV30RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV30Scene<Instance>(
  normalized: TableLiveV30NormalizedScene,
...
): TableLiveV30SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV30FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV30ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV30PreCaptureGates(payload: unknown): void {
