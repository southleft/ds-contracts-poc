...
} from "./input-field-live-v3-verifier-v13.js";
import {
  buildInputLiveV13RawPropertyRuntime,
  normalizeInputLiveV13Scene,
  type InputLiveV13RawNode,
} from "./input-field-live-v13-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v13.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v13.js";

export {
  buildInputLiveV13RestoreProgram,
  validateInputLiveV13RestorePayload,
  type InputLiveV13RestorePayload,
} from "./input-field-live-v13-restore.js";

export const INPUT_LIVE_V13_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V13_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V13_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V13_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V13_VARIANT_COUNT = 256;
export const INPUT_LIVE_V13_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V13_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV13SourceId = (typeof INPUT_LIVE_V13_SOURCE_IDS)[number];

export interface InputLiveV13SourceIdentity {
  source: InputLiveV13SourceId;
...
export interface InputLiveV13WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV13ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV13SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV13RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV13RootProof {
  source: InputLiveV13SourceId;
  adapterIdentity: string;
...
export interface InputLiveV13ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV13SourceId;
    adapterIdentity: string;
...
export interface InputLiveV13CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV13SourceId;
  adapterIdentity: string;
...
export interface InputLiveV13CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV13SourceId;
  strata: InputLiveV13CaptureCell["strata"];
...
export interface InputLiveV13CleanupPayload {
...
export function validateInputLiveV13WriterPayload(
  value: unknown,
): InputLiveV13WriterOwnership {
...
const identityPlan = (source: InputLiveV13SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV13ExtractProgram(
  writer: InputLiveV13WriterOwnership,
  sources: readonly InputLiveV13SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V13_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V13_NAMESPACE)}
${buildInputLiveV13RawPropertyRuntime()}
...
export function validateInputLiveV13ExtractPayload(
  value: unknown,
  writer: InputLiveV13WriterOwnership,
): InputLiveV13ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V13_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV13ExtractPayload;
}

export function proveInputLiveV13Roots<Instance>(
  extract: InputLiveV13ExtractPayload,
  sources: readonly (InputLiveV13SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV13Scene(
      root.scene,
...
export function assertInputLiveV13RootProofs(
  proofs: readonly InputLiveV13RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV13ProbeProgram(
  writer: InputLiveV13WriterOwnership,
  sources: readonly InputLiveV13SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V13_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV13ProbePayload(
  value: unknown,
  writer: InputLiveV13WriterOwnership,
): InputLiveV13ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV13ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V13_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V13_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV13CaptureManifest(
  cells: readonly InputLiveV13CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V13_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V13_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V13_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V13_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V13_SOURCE_IDS.some(
...
export const inputLiveV13CaptureManifestSha256 = (
  cells: readonly InputLiveV13CaptureCell[],
): string => {
  validateInputLiveV13CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV13CaptureProgram(
  writer: InputLiveV13WriterOwnership,
  cell: InputLiveV13CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V13_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V13_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V13_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV13CapturePayload(
  value: unknown,
  cell: InputLiveV13CaptureCell,
  rawResponseBytes: number,
): InputLiveV13CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV13CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V13_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V13_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV13CaptureResponses(
  manifest: readonly InputLiveV13CaptureCell[],
  responses: readonly InputLiveV13CapturePayload[],
): void {
  validateInputLiveV13CaptureManifest(manifest);
  if (
...
export interface InputLiveV13ObjectiveReport {
  artifactVersion: "input-live-v13-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV13Objective(
  manifest: readonly InputLiveV13CaptureCell[],
  responses: readonly InputLiveV13CapturePayload[],
  readReference: (cell: InputLiveV13CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV13CaptureCell) => Buffer,
): InputLiveV13ObjectiveReport {
  assertInputLiveV13CaptureResponses(manifest, responses);
...
export function buildInputLiveV13CleanupProgram(
  writer: InputLiveV13WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V13_NAMESPACE,
...
export function validateInputLiveV13CleanupPayload(
  value: unknown,
  writer: InputLiveV13WriterOwnership,
): InputLiveV13CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV13CleanupPayload;
...
export const INPUT_LIVE_V13_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV13WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
