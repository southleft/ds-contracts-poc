...
import {
  validateCalendarLiveV14AntecedentIndex,
  verifyCalendarLiveV14Authorization,
  type CalendarLiveV14AntecedentIndex,
  type CalendarLiveV14AuthorizationProof,
} from "./calendar-live-v14-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V14_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v14/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V14_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v14-security-attestation.json";

export interface CalendarLiveV14SecurityAttestation {
  artifactVersion: "calendar-live-v14-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV14PreflightReport {
  artifactVersion: "calendar-live-v14-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV14SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV14SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV14AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v14-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V14_TARGET) ||
...
export function validateCalendarLiveV14ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV14Authorization");
  const preflight = source.indexOf("runCalendarLiveV14Preflight");
  const initialize = source.indexOf("CalendarLiveV14Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV14Preflight(
  root: string,
  proof: CalendarLiveV14AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V14_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV14PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V14_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V14_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV14AntecedentIndex;
  failures.push(...validateCalendarLiveV14AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V14_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V14_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV14SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v14-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV14ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v14 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV14SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v14-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V14_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV14Preflight(
    process.cwd(),
    verifyCalendarLiveV14Authorization(),
    attestationPath,
