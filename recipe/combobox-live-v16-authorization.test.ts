...
import { COMBOBOX_LIVE_V16_INDEX_PATH } from "./build-combobox-live-proof-v16.js";
import {
  COMBOBOX_LIVE_V16_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V16_TARGET,
} from "./combobox-live-v16-broker.js";
import {
  buildComboboxLiveV16AuthorizationArtifact,
  validateComboboxLiveV16AntecedentIndex,
  validateComboboxLiveV16AuthorizationArtifact,
  validateComboboxLiveV16History,
  validateComboboxLiveV16UnitTestSource,
  type ComboboxLiveV16AntecedentIndex,
  type ComboboxLiveV16AuthorizationArtifact,
  type ComboboxLiveV16AuthorizationProof,
  type ComboboxLiveV16HistoryState,
} from "./combobox-live-v16-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV16ControlFlowSource,
  validateComboboxLiveV16SecurityAttestation,
  type ComboboxLiveV16SecurityAttestation,
} from "./combobox-live-v16-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v16-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V16_INDEX_PATH);
const index = (): ComboboxLiveV16AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV16AntecedentIndex;

const artifact = (): ComboboxLiveV16AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV16AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV16HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV16HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV16AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v16/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV16AuthorizationArtifact,
): ComboboxLiveV16SecurityAttestation => {
  const body: Omit<ComboboxLiveV16SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V16_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV16History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV16History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV16History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV16History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV16History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV16UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV16UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV16AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV16AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV16AuthorizationArtifact(
        planted as ComboboxLiveV16AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV16SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV16SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV16SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV16ControlFlowSource(source), []);
});
