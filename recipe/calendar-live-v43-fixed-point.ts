/**
 * Calendar live v43 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V43_FIXED_VERIFIER_VERSION = "calendar-live-v43-fixed-point-v1";
export const CALENDAR_LIVE_V43_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV43FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV43FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV43FixedSourceProbe {
...
export interface CalendarLiveV43FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV43FixedFixedPoint;
}

export interface CalendarLiveV43FixedHardGateInput {
  thresholds: CalendarLiveV43FixedThresholds;
  sourceProbes: CalendarLiveV43FixedSourceProbe[];
  cells: CalendarLiveV43FixedCellValidation[];
  sceneProofs: CalendarLiveV43FixedSceneProof[];
...
export interface CalendarLiveV43FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V43_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV43FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV43FixedHardGates(
  input: CalendarLiveV43FixedHardGateInput,
): CalendarLiveV43FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV43FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V43_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV43FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV43FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV43FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
