...
export const INPUT_LIVE_V50_VERIFIER_VERSION = "input-live-v50-verifier-v1";

export function buildInputLiveV50RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV50RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV50RawNode[];
}

export interface InputLiveV50NormalizedScene {
...
export interface InputLiveV50SceneProof {
...
export function normalizeInputLiveV50Scene(
  root: InputLiveV50RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV50NormalizedScene {
  const canonicalBindings: InputLiveV50NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV50RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV50Scene<Instance>(
  normalized: InputLiveV50NormalizedScene,
...
export function compileInputLiveV50ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV50PreCaptureGates(payload: unknown): void {
