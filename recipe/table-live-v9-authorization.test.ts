...
import { TABLE_LIVE_V9_INDEX_PATH } from "./build-table-live-proof-v9.js";
import {
...
import {
  buildTableLiveV9AuthorizationArtifact,
  validateTableLiveV9AntecedentIndex,
  validateTableLiveV9AuthorizationArtifact,
  validateTableLiveV9History,
  validateTableLiveV9UnitTestSource,
  type TableLiveV9AntecedentIndex,
  type TableLiveV9AuthorizationArtifact,
  type TableLiveV9AuthorizationProof,
  type TableLiveV9HistoryState,
} from "./table-live-v9-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV9ControlFlowSource,
  validateTableLiveV9SecurityAttestation,
  type TableLiveV9SecurityAttestation,
} from "./table-live-v9-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V9_INDEX_PATH);
const index = (): TableLiveV9AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV9AntecedentIndex;

const artifact = (): TableLiveV9AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV9AuthorizationArtifact({
...
const pendingState = (): TableLiveV9HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV9HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV9AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v9/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV9AuthorizationArtifact,
): TableLiveV9SecurityAttestation => {
  const body: Omit<TableLiveV9SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V9_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV9History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV9History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV9History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV9History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV9History(authorizedState(), "pending").join("\n"),
    /stale history mode.*authorization exists/,
  );
});

test("the exact v6 phase-sensitive self-test defect is planted and table v9 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV9UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v9-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV9UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV9AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV9AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV9AuthorizationArtifact(
        planted as TableLiveV9AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV9SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV9SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV9SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v9-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV9ControlFlowSource(source), []);
});
