...
} from "./input-field-live-v3-verifier-v67.js";
import {
  buildInputLiveV67RawPropertyRuntime,
  normalizeInputLiveV67Scene,
  type InputLiveV67RawNode,
} from "./input-field-live-v67-verifier.js";
...
export const INPUT_LIVE_V67_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V67_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V67_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V67_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V67_VARIANT_COUNT = 256;
export const INPUT_LIVE_V67_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V67_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV67SourceId = (typeof INPUT_LIVE_V67_SOURCE_IDS)[number];

export interface InputLiveV67SourceIdentity {
  source: InputLiveV67SourceId;
...
export interface InputLiveV67WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV67ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV67SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV67RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV67RootProof {
  source: InputLiveV67SourceId;
  adapterIdentity: string;
...
export interface InputLiveV67ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV67SourceId;
    adapterIdentity: string;
...
export interface InputLiveV67CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV67SourceId;
  adapterIdentity: string;
...
export interface InputLiveV67CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV67SourceId;
  strata: InputLiveV67CaptureCell["strata"];
...
export interface InputLiveV67CleanupPayload {
...
export function validateInputLiveV67WriterPayload(
  value: unknown,
): InputLiveV67WriterOwnership {
...
const identityPlan = (source: InputLiveV67SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV67ExtractProgram(
  writer: InputLiveV67WriterOwnership,
  sources: readonly InputLiveV67SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V67_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V67_NAMESPACE)}
${buildInputLiveV67RawPropertyRuntime()}
...
export function validateInputLiveV67ExtractPayload(
  value: unknown,
  writer: InputLiveV67WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V67_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV67ExtractPayload;
}

export function proveInputLiveV67Roots<Instance>(
  extract: InputLiveV67ExtractPayload,
  sources: readonly (InputLiveV67SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV67Scene(
      root.scene,
...
export function assertInputLiveV67RootProofs(
  proofs: readonly InputLiveV67RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV67ProbeProgram(
  writer: InputLiveV67WriterOwnership,
  sources: readonly InputLiveV67SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V67_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV67ProbePayload(
  value: unknown,
  writer: InputLiveV67WriterOwnership,
): InputLiveV67ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV67ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V67_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V67_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV67CaptureManifest(
  cells: readonly InputLiveV67CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V67_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V67_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V67_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V67_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V67_SOURCE_IDS.some(
...
export const inputLiveV67CaptureManifestSha256 = (
  cells: readonly InputLiveV67CaptureCell[],
): string => {
  validateInputLiveV67CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV67CaptureProgram(
  writer: InputLiveV67WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V67_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V67_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V67_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV67CapturePayload(
  value: unknown,
  cell: InputLiveV67CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV67CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V67_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V67_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV67CaptureResponses(
  manifest: readonly InputLiveV67CaptureCell[],
  responses: readonly InputLiveV67CapturePayload[],
): void {
  validateInputLiveV67CaptureManifest(manifest);
  if (
...
export interface InputLiveV67ObjectiveReport {
  artifactVersion: "input-live-v67-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV67Objective(
  manifest: readonly InputLiveV67CaptureCell[],
  responses: readonly InputLiveV67CapturePayload[],
  readReference: (cell: InputLiveV67CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV67CaptureCell) => Buffer,
): InputLiveV67ObjectiveReport {
  assertInputLiveV67CaptureResponses(manifest, responses);
...
export function buildInputLiveV67CleanupProgram(
  writer: InputLiveV67WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V67_NAMESPACE,
...
export function validateInputLiveV67CleanupPayload(
  value: unknown,
  writer: InputLiveV67WriterOwnership,
): InputLiveV67CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV67CleanupPayload;
...
export const INPUT_LIVE_V67_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV67WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
