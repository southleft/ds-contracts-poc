...
import { calendarLiveV1Sha256 } from "./calendar-live-v39-broker.js";

export interface CalendarLiveV39LifecycleSimulationReport {
  artifactVersion: "calendar-live-v39-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV39AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV39Lifecycle(): Promise<CalendarLiveV39LifecycleSimulationReport> {
  await buildCalendarLiveV39Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V39_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV39AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v39-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V39_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V39_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV39History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV39History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV39AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V39_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V39_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV39History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV39History(authorized, "pending");
    ...
    await buildCalendarLiveV39Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V39_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV39AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV39Lifecycle(), null, 2)}\n`,
  );
