...
import { buildComboboxLiveV35CleanupRuntime } from "./combobox-live-v35-cleanup.js";
import {
  type ComboboxLiveV35FixedFixedPoint,
} from "./combobox-live-v35-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV35RestoreProgram,
  validateComboboxLiveV35RestorePayload,
  type ComboboxLiveV35RestorePayload,
} from "./combobox-live-v35-restore.js";
import {
  buildComboboxLiveV35RawPropertyRuntime,
  normalizeComboboxLiveV35Scene,
  type ComboboxLiveV35RawNode,
} from "./combobox-live-v35-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v35.js";
import {
...
export const COMBOBOX_LIVE_V35_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V35_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V35_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V35_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V35_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V35_SET_COUNT = 4;
export const COMBOBOX_LIVE_V35_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V35_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V35_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V35_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V35_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV35SourceId = (typeof COMBOBOX_LIVE_V35_SOURCE_IDS)[number];

export interface ComboboxLiveV35SourceIdentity {
  source: ComboboxLiveV35SourceId;
...
export interface ComboboxLiveV35WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV35ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV35SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV35RawNode;
    optionScene: ComboboxLiveV35RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV35RootProof {
  source: ComboboxLiveV35SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV35FixedFixedPoint;
}

export interface ComboboxLiveV35ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV35SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV35CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV35SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV35CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV35SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV35CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v35 ${label} must be an object`);
...
export function validateComboboxLiveV35WriterPayload(
  value: unknown,
): ComboboxLiveV35WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V35_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v35 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v35 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v35 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV35SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV35ExtractProgram(
  writer: ComboboxLiveV35WriterOwnership,
  sources: readonly ComboboxLiveV35SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v35 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V35_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V35_NAMESPACE)}
${buildComboboxLiveV35RawPropertyRuntime()}
...
export function validateComboboxLiveV35ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV35WriterOwnership,
): ComboboxLiveV35ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v35 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V35_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v35 extract schema/two-root mismatch");
  return payload as ComboboxLiveV35ExtractPayload;
}

export function proveComboboxLiveV35Roots<Instance>(
  extract: ComboboxLiveV35ExtractPayload,
  sources: readonly (ComboboxLiveV35SourceIdentity & {
...
): ComboboxLiveV35RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v35 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v35 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV35Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV35Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV35FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v35 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v35 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v35 compile lost owned sets");
...
export function assertComboboxLiveV35RootProofs(
  proofs: readonly ComboboxLiveV35RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v35 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v35 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV35ProbeProgram(
  writer: ComboboxLiveV35WriterOwnership,
  sources: readonly ComboboxLiveV35SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V35_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV35ProbePayload(
  value: unknown,
  writer: ComboboxLiveV35WriterOwnership,
): ComboboxLiveV35ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV35ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V35_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V35_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v35 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV35CaptureManifest(
  cells: readonly ComboboxLiveV35CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V35_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V35_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V35_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V35_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v35 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV35CaptureCell[],
): string => {
  validateComboboxLiveV35CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV35CaptureProgram(
  writer: ComboboxLiveV35WriterOwnership,
  cell: ComboboxLiveV35CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V35_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v35 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v35 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V35_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV35CapturePayload(
  value: unknown,
  cell: ComboboxLiveV35CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV35CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV35CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V35_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V35_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v35 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV35CaptureResponses(
  manifest: readonly ComboboxLiveV35CaptureCell[],
  responses: readonly ComboboxLiveV35CapturePayload[],
): void {
  validateComboboxLiveV35CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v35 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV35ObjectiveReport {
  artifactVersion: "combobox-live-v35-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV35Objective(
  manifest: readonly ComboboxLiveV35CaptureCell[],
  responses: readonly ComboboxLiveV35CapturePayload[],
): ComboboxLiveV35ObjectiveReport {
  assertComboboxLiveV35CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v35-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV35CleanupProgram(
  writer: ComboboxLiveV35WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV35CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V35_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v35 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV35CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV35WriterOwnership,
): ComboboxLiveV35CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV35CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v35 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V35_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV35WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
