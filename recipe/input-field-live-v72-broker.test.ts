...
import {
  ...
  createInputLiveV72FakeDynamicResponse,
  ...
  listInputLiveV72PersistedRequests,
  persistInputLiveV72RawResponse,
  persistInputLiveV72TechnicalGates,
  validateInputLiveV72Receipt,
  validateInputLiveV72Request,
  type InputLiveV72RemotePhase,
...
import {
  ...
  type InputLiveV72ExtractPayload,
  type InputLiveV72ProbePayload,
  type InputLiveV72RestorePayload,
  type InputLiveV72RootProof,
  type InputLiveV72WriterOwnership,
} from "./input-field-live-v72-contract.js";
import {
  InputLiveV72Orchestrator,
  simulatedInputLiveV72Authorization,
} from "./run-input-field-live-v72.js";
...
const captures: InputLiveV72CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV72Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v72-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV72Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV72CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV72FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV72FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V72_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V72_TARGET);
  persistInputLiveV72RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV72Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV72Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV72WriterOwnership,
): InputLiveV72RestorePayload => ({
...
const rootProofs = (): InputLiveV72RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV72CaptureCell,
): InputLiveV72CapturePayload => {
  const bytes = Buffer.from(`input-live-v72-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV72Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV72Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV72RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV72RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV72ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV72ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV72CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV72CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV72CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV72Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV72CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV72PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV72ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV72Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV72Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV72Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV72Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V72_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV72RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V72_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV72Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV72Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV72ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV72Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV72WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV72RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV72Request;
    validateInputLiveV72Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV72Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV72CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v72-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV72Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV72Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV72Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V72_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV72RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V72_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV72Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV72Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV72RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V72_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV72Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV72Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV72Request;
