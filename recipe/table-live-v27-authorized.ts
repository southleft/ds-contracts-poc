...
import { TABLE_LIVE_V27_AUTHORIZATION_PATH } from "./build-table-live-proof-v27.js";
import { verifyTableLiveV27Authorization } from "./table-live-v27-authorization.js";
import { runTableLiveV27Preflight } from "./table-live-v27-preflight.js";
import { TableLiveV27Orchestrator } from "./run-table-live-v27.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v27 requires exact authorization artifact ${TABLE_LIVE_V27_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v27-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV27Authorization(root);
    runTableLiveV27Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV27Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV27Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
