...
import { COMBOBOX_LIVE_V23_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v23.js";
import { verifyComboboxLiveV23Authorization } from "./combobox-live-v23-authorization.js";
import { runComboboxLiveV23Preflight } from "./combobox-live-v23-preflight.js";
import { ComboboxLiveV23Orchestrator } from "./run-combobox-live-v23.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v23 requires exact authorization artifact ${COMBOBOX_LIVE_V23_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v23-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV23Authorization(root);
    runComboboxLiveV23Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV23Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV23Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
