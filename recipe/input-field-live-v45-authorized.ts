...
import { INPUT_LIVE_V45_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v45.js";
import { verifyInputLiveV45Authorization } from "./input-field-live-v45-authorization.js";
import { runInputLiveV45Preflight } from "./input-field-live-v45-preflight.js";
import { InputLiveV45Orchestrator } from "./run-input-field-live-v45.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V45_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v45-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV45Authorization(root);
    runInputLiveV45Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV45Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
