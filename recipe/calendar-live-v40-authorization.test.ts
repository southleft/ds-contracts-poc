...
import { CALENDAR_LIVE_V40_INDEX_PATH } from "./build-calendar-live-proof-v40.js";
import {
...
import {
  buildCalendarLiveV40AuthorizationArtifact,
  validateCalendarLiveV40AntecedentIndex,
  validateCalendarLiveV40AuthorizationArtifact,
  validateCalendarLiveV40History,
  validateCalendarLiveV40UnitTestSource,
  type CalendarLiveV40AntecedentIndex,
  type CalendarLiveV40AuthorizationArtifact,
  type CalendarLiveV40AuthorizationProof,
  type CalendarLiveV40HistoryState,
} from "./calendar-live-v40-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV40ControlFlowSource,
  validateCalendarLiveV40SecurityAttestation,
  type CalendarLiveV40SecurityAttestation,
} from "./calendar-live-v40-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V40_INDEX_PATH);
const index = (): CalendarLiveV40AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV40AntecedentIndex;

const artifact = (): CalendarLiveV40AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV40AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV40HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV40HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV40AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v40/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV40AuthorizationArtifact,
): CalendarLiveV40SecurityAttestation => {
  const body: Omit<CalendarLiveV40SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V40_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV40History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV40History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV40History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV40History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV40History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV40UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v40-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV40UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV40AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV40AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV40AuthorizationArtifact(
        planted as CalendarLiveV40AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV40SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV40SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV40SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v40-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV40ControlFlowSource(source), []);
});
