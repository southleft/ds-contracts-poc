...
import { CALENDAR_LIVE_V9_INDEX_PATH } from "./build-calendar-live-proof-v9.js";
import {
...
import {
  ...
  validateCalendarLiveV9History,
  validateCalendarLiveV9UnitTestSource,
  type CalendarLiveV9AntecedentIndex,
  type CalendarLiveV9AuthorizationArtifact,
  type CalendarLiveV9AuthorizationProof,
  type CalendarLiveV9HistoryState,
} from "./calendar-live-v9-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV9ControlFlowSource,
  validateCalendarLiveV9SecurityAttestation,
  type CalendarLiveV9SecurityAttestation,
} from "./calendar-live-v9-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V9_INDEX_PATH);
const index = (): CalendarLiveV9AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV9AntecedentIndex;

const artifact = (): CalendarLiveV9AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV9AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV9HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV9HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV9AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v9/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV9AuthorizationArtifact,
): CalendarLiveV9SecurityAttestation => {
  const body: Omit<CalendarLiveV9SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V9_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV9History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV9History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV9History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV9History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV9History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV9UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v9-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV9UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV9AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV9AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV9AuthorizationArtifact(
        planted as CalendarLiveV9AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV9SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV9SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV9SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v9-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV9ControlFlowSource(source), []);
});
