...
import {
  acceptCalendarLiveV42Response,
  createCalendarLiveV42Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV42Request,
  persistCalendarLiveV42RawResponse,
  persistCalendarLiveV42TechnicalGates,
  readCalendarLiveV42AcceptedRawPayload,
  type CalendarLiveV42Request,
  type CalendarLiveV42TransactionAuthorization,
} from "./calendar-live-v42-broker.js";
import {
  CALENDAR_LIVE_V42_CAPTURE_COUNT,
  assertCalendarLiveV42CaptureResponses,
  ...
  evaluateCalendarLiveV42Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV42Roots,
  validateCalendarLiveV42CaptureManifest,
  validateCalendarLiveV42CapturePayload,
  validateCalendarLiveV42CleanupPayload,
  validateCalendarLiveV42ExtractPayload,
  validateCalendarLiveV42ProbePayload,
  validateCalendarLiveV42WriterPayload,
  type CalendarLiveV42CaptureCell,
  type CalendarLiveV42CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V42_PLAN_PATH,
  type CalendarLiveV42ProofPlan,
} from "./build-calendar-live-proof-v42.js";
import {
...
export const CALENDAR_LIVE_V42_RUNNER_VERSION = "calendar-live-v42-runner-v1";

interface CalendarLiveV42JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV42OrchestratorState {
  artifactVersion: "calendar-live-v42-orchestrator-state-v1";
  ...
  journal: CalendarLiveV42JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV42ProofPlan;
  planSha256: string;
  captures: CalendarLiveV42CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV42SourceIdentity & {
...
export interface CalendarLiveV42NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV42OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v42 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V42_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V42_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV42ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV42CaptureCell[];
  validateCalendarLiveV42CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v42 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v42 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV42WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV42RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV42Request =>
  readJson<CalendarLiveV42Request>(file);

export class CalendarLiveV42Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV42Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV42OrchestratorState = {
      artifactVersion: "calendar-live-v42-orchestrator-state-v1",
    ...
    return new CalendarLiveV42Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV42OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV42Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV42NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV42NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v42 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV42NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v42 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV42RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV42Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV42CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV42AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV42WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV42Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV42ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV42Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV42Response(this.#directory, "writer", {
        validate: validateCalendarLiveV42WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV42Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV42RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV42Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV42ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV42Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV42RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v42-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV42Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV42ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV42Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV42ProbePayload(payload, ownership),
      });
      persistCalendarLiveV42TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v42 capture cell missing");
      const accepted = acceptCalendarLiveV42Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV42CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V42_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV42CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV42Objective(
    ...
    throw new Error(`Calendar live v42 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV42Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV42CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV42Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV42RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV42Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV42CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV42CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV42CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV42Authorization = (
  privateKey: KeyObject,
): CalendarLiveV42TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV42SimulationKey = (): KeyObject =>
