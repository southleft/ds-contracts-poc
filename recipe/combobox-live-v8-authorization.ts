...
export type ComboboxLiveV8HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV8AuthorizationArtifact {
  artifactVersion: "combobox-live-v8-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V8_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV8AntecedentIndex {
...
export interface ComboboxLiveV8HistoryState {
  index?: ComboboxLiveV8AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV8AuthorizationArtifact;
...
export interface ComboboxLiveV8AuthorizationProof extends ComboboxLiveV8TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V8_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V8_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V8_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V8_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v8-authorization") ||
  artifactPath.includes("combobox-live-v8-preflight") ||
  artifactPath.includes("combobox-live-v8-authorized") ||
  artifactPath.includes("create-combobox-live-v8-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v8-status") ||
...
export function validateComboboxLiveV8AntecedentIndex(
  index: ComboboxLiveV8AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v8-antecedent-index-v1" ||
...
export function validateComboboxLiveV8UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV8History\(\s*["']/.test(source) ||
    /readComboboxLiveV8HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV8AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV8AuthorizationArtifact {
  ...
  ) as ComboboxLiveV8AntecedentIndex;
  const indexFailures = validateComboboxLiveV8AntecedentIndex(index);
...
export function validateComboboxLiveV8AuthorizationArtifact(
  artifact: ComboboxLiveV8AuthorizationArtifact | undefined,
  index: ComboboxLiveV8AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v8-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v8" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V8_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V8_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V8_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V8_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V8_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V8_SOURCE_ROOTS ||
...
export function validateComboboxLiveV8History(
  state: ComboboxLiveV8HistoryState,
  expected: ComboboxLiveV8HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV8AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV8AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV8HistoryState(
  root = process.cwd(),
): ComboboxLiveV8HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V8_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV8AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V8_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V8_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V8_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV8History(
  expected: ComboboxLiveV8HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV8HistoryState {
  const state = readComboboxLiveV8HistoryState(root);
  const failures = validateComboboxLiveV8History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v8 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV8Authorization(
  root = process.cwd(),
): ComboboxLiveV8AuthorizationProof {
  const state = verifyComboboxLiveV8History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V8_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v8/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v8.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V8_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V8_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V8_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV8HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV8History(expected);
  process.stdout.write(
    `Combobox live v8 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
