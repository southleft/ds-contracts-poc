/**
 * Carried Combobox live v23 host verifier. Uses scene-readback-combobox-v23 so live
...
export const COMBOBOX_LIVE_V23_FIXED_VERIFIER_VERSION = "combobox-live-v23-fixed-point-v1";
export const COMBOBOX_LIVE_V23_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV23FixedThresholds {
...
export interface ComboboxLiveV23FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV23FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV23FixedSourceProbe {
...
export interface ComboboxLiveV23FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV23FixedFixedPoint;
}

export interface ComboboxLiveV23FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV23FixedHardGateInput {
  thresholds: ComboboxLiveV23FixedThresholds;
  materialRegression: ComboboxLiveV23FixedMaterialRegression;
  sourceProbes: ComboboxLiveV23FixedSourceProbe[];
  cells: ComboboxLiveV23FixedCellValidation[];
  sceneProofs: ComboboxLiveV23FixedSceneProof[];
  visualRows: ComboboxLiveV23FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV23FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V23_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV23FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV23FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV23FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV23FixedHardGates(
  input: ComboboxLiveV23FixedHardGateInput,
): ComboboxLiveV23FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV23FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV23FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V23_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV23FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV23FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV23FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
