...
import { INPUT_LIVE_V79_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v79.js";
import { verifyInputLiveV79Authorization } from "./input-field-live-v79-authorization.js";
import { runInputLiveV79Preflight } from "./input-field-live-v79-preflight.js";
import { InputLiveV79Orchestrator } from "./run-input-field-live-v79.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V79_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v79-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV79Authorization(root);
    runInputLiveV79Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV79Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
