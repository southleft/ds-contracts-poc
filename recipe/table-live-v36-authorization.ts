...
export type TableLiveV36HistoryExpectation = "pending" | "authorized";

export interface TableLiveV36AuthorizationArtifact {
  artifactVersion: "table-live-v36-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V36_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV36AntecedentIndex {
...
export interface TableLiveV36HistoryState {
  index?: TableLiveV36AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV36AuthorizationArtifact;
...
export interface TableLiveV36AuthorizationProof
  extends TableLiveV36TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V36_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V36_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V36_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V36_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v36-authorization") ||
  artifactPath.includes("table-live-v36-preflight") ||
  artifactPath.includes("table-live-v36-authorized") ||
  artifactPath.includes("create-table-live-v36-security-attestation") ||
  artifactPath.includes("table-live-pivot-v36-status") ||
...
export function validateTableLiveV36AntecedentIndex(
  index: TableLiveV36AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v36-antecedent-index-v1" ||
...
export function validateTableLiveV36UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV36History\(\s*["']/.test(source) ||
    /readTableLiveV36HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV36AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV36AuthorizationArtifact {
  ...
  ) as TableLiveV36AntecedentIndex;
  const indexFailures = validateTableLiveV36AntecedentIndex(index);
...
export function validateTableLiveV36AuthorizationArtifact(
  artifact: TableLiveV36AuthorizationArtifact | undefined,
  index: TableLiveV36AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v36-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v36" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V36_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V36_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V36_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V36_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V36_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V36_SOURCE_ROOTS ||
...
export function validateTableLiveV36History(
  state: TableLiveV36HistoryState,
  expected: TableLiveV36HistoryExpectation,
): string[] {
  const failures = validateTableLiveV36AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV36AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV36HistoryState(
  root = process.cwd(),
): TableLiveV36HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V36_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV36AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V36_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V36_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V36_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV36History(
  expected: TableLiveV36HistoryExpectation,
  root = process.cwd(),
): TableLiveV36HistoryState {
  const state = readTableLiveV36HistoryState(root);
  const failures = validateTableLiveV36History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v36 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV36Authorization(
  root = process.cwd(),
): TableLiveV36AuthorizationProof {
  const state = verifyTableLiveV36History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V36_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v36/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v36.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V36_TARGET,
    expectedDynamicTool: TABLE_LIVE_V36_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V36_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV36HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV36History(expected);
  process.stdout.write(
    `Table live v36 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
