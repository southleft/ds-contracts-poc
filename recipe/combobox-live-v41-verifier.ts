...
import {
  verifyComboboxLiveV41FixedSceneFixedPoint,
  type ComboboxLiveV41FixedFixedPoint,
} from "./combobox-live-v41-fixed-point.js";

export const COMBOBOX_LIVE_V41_VERIFIER_VERSION = "combobox-live-v41-verifier-v1";

export function buildComboboxLiveV41RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV41RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV41RawNode[];
}

export interface ComboboxLiveV41NormalizedScene {
...
export interface ComboboxLiveV41SceneProof {
...
export function normalizeComboboxLiveV41Scene(
  root: ComboboxLiveV41RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV41NormalizedScene {
  const canonicalBindings: ComboboxLiveV41NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV41RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV41Scene<Instance>(
  normalized: ComboboxLiveV41NormalizedScene,
...
): ComboboxLiveV41SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV41FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV41ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV41PreCaptureGates(payload: unknown): void {
