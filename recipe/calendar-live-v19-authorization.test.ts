...
import { CALENDAR_LIVE_V19_INDEX_PATH } from "./build-calendar-live-proof-v19.js";
import {
...
import {
  ...
  validateCalendarLiveV19History,
  validateCalendarLiveV19UnitTestSource,
  type CalendarLiveV19AntecedentIndex,
  type CalendarLiveV19AuthorizationArtifact,
  type CalendarLiveV19AuthorizationProof,
  type CalendarLiveV19HistoryState,
} from "./calendar-live-v19-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV19ControlFlowSource,
  validateCalendarLiveV19SecurityAttestation,
  type CalendarLiveV19SecurityAttestation,
} from "./calendar-live-v19-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V19_INDEX_PATH);
const index = (): CalendarLiveV19AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV19AntecedentIndex;

const artifact = (): CalendarLiveV19AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV19AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV19HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV19HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV19AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v19/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV19AuthorizationArtifact,
): CalendarLiveV19SecurityAttestation => {
  const body: Omit<CalendarLiveV19SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V19_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV19History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV19History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV19History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV19History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV19History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV19UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v19-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV19UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV19AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV19AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV19AuthorizationArtifact(
        planted as CalendarLiveV19AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV19SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV19SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV19SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v19-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV19ControlFlowSource(source), []);
});
