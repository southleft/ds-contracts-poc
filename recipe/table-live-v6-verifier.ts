...
import {
  verifyTableLiveV6FixedSceneFixedPoint,
  type TableLiveV6FixedFixedPoint,
} from "./table-live-v6-fixed-point.js";

export const TABLE_LIVE_V6_VERIFIER_VERSION = "table-live-v6-verifier-v1";

export function buildTableLiveV6RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV6RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV6RawNode[];
}

export interface TableLiveV6NormalizedScene {
...
export interface TableLiveV6SceneProof {
...
export function normalizeTableLiveV6Scene(
  root: TableLiveV6RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV6NormalizedScene {
  const canonicalBindings: TableLiveV6NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV6RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV6Scene<Instance>(
  normalized: TableLiveV6NormalizedScene,
...
): TableLiveV6SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV6FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV6ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV6PreCaptureGates(payload: unknown): void {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("table live v6 pre-capture gate payload absent");
