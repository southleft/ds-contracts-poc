...
export interface ComboboxLiveV35LifecycleSimulationReport {
  artifactVersion: "combobox-live-v35-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV35AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV35Lifecycle(): Promise<ComboboxLiveV35LifecycleSimulationReport> {
  await buildComboboxLiveV35Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V35_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV35AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v35-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V35_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V35_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV35History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV35History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV35AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V35_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V35_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV35History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV35History(authorized, "pending");
    ...
    await buildComboboxLiveV35Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V35_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV35AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV35Lifecycle(), null, 2)}\n`,
  );
