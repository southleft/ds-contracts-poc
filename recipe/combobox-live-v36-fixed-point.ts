/**
 * Carried Combobox live v36 host verifier. Uses scene-readback-combobox-v36 so live
...
export const COMBOBOX_LIVE_V36_FIXED_VERIFIER_VERSION = "combobox-live-v36-fixed-point-v1";
export const COMBOBOX_LIVE_V36_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV36FixedThresholds {
...
export interface ComboboxLiveV36FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV36FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV36FixedSourceProbe {
...
export interface ComboboxLiveV36FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV36FixedFixedPoint;
}

export interface ComboboxLiveV36FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV36FixedHardGateInput {
  thresholds: ComboboxLiveV36FixedThresholds;
  materialRegression: ComboboxLiveV36FixedMaterialRegression;
  sourceProbes: ComboboxLiveV36FixedSourceProbe[];
  cells: ComboboxLiveV36FixedCellValidation[];
  sceneProofs: ComboboxLiveV36FixedSceneProof[];
  visualRows: ComboboxLiveV36FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV36FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V36_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV36FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV36FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV36FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV36FixedHardGates(
  input: ComboboxLiveV36FixedHardGateInput,
): ComboboxLiveV36FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV36FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV36FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V36_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV36FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV36FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV36FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
