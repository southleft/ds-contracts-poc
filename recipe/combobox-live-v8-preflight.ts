...
import {
  validateComboboxLiveV8AntecedentIndex,
  verifyComboboxLiveV8Authorization,
  type ComboboxLiveV8AntecedentIndex,
  type ComboboxLiveV8AuthorizationProof,
} from "./combobox-live-v8-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V8_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v8/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V8_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v8-security-attestation.json";

export interface ComboboxLiveV8SecurityAttestation {
  artifactVersion: "combobox-live-v8-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV8PreflightReport {
  artifactVersion: "combobox-live-v8-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV8SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV8SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV8AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v8-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V8_TARGET) ||
...
export function validateComboboxLiveV8ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV8Authorization");
  const preflight = source.indexOf("runComboboxLiveV8Preflight");
  const initialize = source.indexOf("ComboboxLiveV8Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV8Preflight(
  root: string,
  proof: ComboboxLiveV8AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V8_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV8PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V8_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V8_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V8_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV8AntecedentIndex;
  failures.push(...validateComboboxLiveV8AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V8_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V8_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV8SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v8-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV8ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v8 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV8SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v8-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V8_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV8Preflight(
    process.cwd(),
    verifyComboboxLiveV8Authorization(),
    attestationPath,
