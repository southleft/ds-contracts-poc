...
import { INPUT_LIVE_V18_INDEX_PATH } from "./build-input-field-live-proof-v18.js";
import {
  INPUT_LIVE_V18_DYNAMIC_TOOL,
  INPUT_LIVE_V18_TARGET,
} from "./input-field-live-v18-broker.js";
import {
  buildInputLiveV18AuthorizationArtifact,
  validateInputLiveV18AntecedentIndex,
  validateInputLiveV18AuthorizationArtifact,
  validateInputLiveV18History,
  validateInputLiveV18UnitTestSource,
  type InputLiveV18AntecedentIndex,
  type InputLiveV18AuthorizationArtifact,
  type InputLiveV18AuthorizationProof,
  type InputLiveV18HistoryState,
} from "./input-field-live-v18-authorization.js";
import {
  inputLiveV18AttestationSha256,
  validateInputLiveV18ControlFlowSource,
  validateInputLiveV18SecurityAttestation,
  type InputLiveV18SecurityAttestation,
} from "./input-field-live-v18-preflight.js";
import { inputLiveV18Sha256 } from "./input-field-live-v18-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V18_INDEX_PATH);
const index = (): InputLiveV18AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV18AntecedentIndex;

const artifact = (): InputLiveV18AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV18AuthorizationArtifact({
...
const pendingState = (): InputLiveV18HistoryState => ({
  index: index(),
  indexSha256: inputLiveV18Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV18HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV18AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV18Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V18_TARGET,
  expectedDynamicTool: INPUT_LIVE_V18_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v18/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV18AuthorizationArtifact,
): InputLiveV18SecurityAttestation => {
  const body: Omit<InputLiveV18SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v18-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V18_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV18AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV18History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV18History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV18History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV18History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV18History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV18UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV18UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV18AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV18Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV18AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV18AuthorizationArtifact(
        planted as InputLiveV18AuthorizationArtifact,
        index(),
        inputLiveV18Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV18SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV18AttestationSha256(
      body as Omit<InputLiveV18SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV18SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV18ControlFlowSource(source), []);
});
