...
import {
  ...
  CALENDAR_LIVE_V50_HOST_PHASES,
  CALENDAR_LIVE_V50_REMOTE_REQUESTS,
  CALENDAR_LIVE_V50_SOURCE_ROOTS,
  CALENDAR_LIVE_V50_VARIANT_COUNT,
  buildCalendarLiveV50CaptureProgram,
  buildCalendarLiveV50CleanupProgram,
  buildCalendarLiveV50ExtractProgram,
  buildCalendarLiveV50ProbeProgram,
  buildCalendarLiveV50RestoreProgram,
  calendarLiveV1CaptureManifestSha256,
  type CalendarLiveV50CaptureCell,
  type CalendarLiveV50SourceIdentity,
  type CalendarLiveV50WriterOwnership,
} from "./calendar-live-v50-contract.js";
import {
  CALENDAR_LIVE_V50_DYNAMIC_TOOL,
  CALENDAR_LIVE_V50_TARGET,
  calendarLiveV1RequestSequence,
} from "./calendar-live-v50-broker.js";
import {
...
export const CALENDAR_LIVE_V50_EVIDENCE_ROOT =
  "recipe/evidence/calendar-live-pivot-v50";
export const CALENDAR_LIVE_V50_PROTOCOL_PATH = `${CALENDAR_LIVE_V50_EVIDENCE_ROOT}/protocol.json`;
export const CALENDAR_LIVE_V50_PLAN_PATH = `${CALENDAR_LIVE_V50_EVIDENCE_ROOT}/proof-plan.json`;
export const CALENDAR_LIVE_V50_CAPTURE_MANIFEST_PATH = `${CALENDAR_LIVE_V50_EVIDENCE_ROOT}/capture-manifest.json`;
export const CALENDAR_LIVE_V50_REQUEST_MANIFEST_PATH = `${CALENDAR_LIVE_V50_EVIDENCE_ROOT}/request-manifest.json`;
export const CALENDAR_LIVE_V50_INDEX_PATH = `${CALENDAR_LIVE_V50_EVIDENCE_ROOT}/antecedent-index.json`;
export const CALENDAR_LIVE_V50_AUTHORIZATION_TEMPLATE_PATH = `${CALENDAR_LIVE_V50_EVIDENCE_ROOT}/authorization-template.json`;
export const CALENDAR_LIVE_V50_AUTHORIZATION_PATH = `${CALENDAR_LIVE_V50_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface CalendarLiveV50ProofPlan {
  artifactVersion: "calendar-live-v50-proof-plan-v1";
...
const captureCells = (
...
): CalendarLiveV50CaptureCell[] => {
  const cells: CalendarLiveV50CaptureCell[] = [];
  ...
  if (cells.length !== CALENDAR_LIVE_V50_CAPTURE_COUNT)
    throw new Error(
      `Calendar live v50 capture cells ${cells.length}, expected ${CALENDAR_LIVE_V50_CAPTURE_COUNT}`,
    );
...
const protocol = (expectedFacts: number) => ({
  artifactVersion: "calendar-live-v50-external-operator-protocol-v1",
  protocolId: "calendar-live-v50",
  status:
  ...
  lifecycle: {
    ...
    comboboxLiveV50AuthorizationReusable: false,
  ...
  denominator: {
    sourceRoots: CALENDAR_LIVE_V50_SOURCE_ROOTS,
    sets: 3,
    variants: CALENDAR_LIVE_V50_VARIANT_COUNT,
    captureCells: CALENDAR_LIVE_V50_CAPTURE_COUNT,
    remoteRequests: CALENDAR_LIVE_V50_REMOTE_REQUESTS,
    hostPhases: CALENDAR_LIVE_V50_HOST_PHASES,
...
export async function buildCalendarLiveV50Proof(
  check = process.argv.includes("--check"),
): Promise<CalendarLiveV50ProofPlan> {
  ...
  if (
    ...
    writer.runIdentity === FORBIDDEN_CALENDAR_V49_RUN_IDENTITY ||
    !writer.runIdentity.endsWith("-calendar-v50") ||
    ...
    throw new Error(
      "Calendar live v50 writer reused Input, Combobox, or Table v1–v24 identity",
    );
  const captures = captureCells(sources);
  const writerProgram = Buffer.from(writer.code);
  const writerOwnershipBlueprint: CalendarLiveV50WriterOwnership = {
  ...
  const identities: CalendarLiveV50SourceIdentity[] = sources.map((source) => ({
    source: source.source,
    adapterIdentity: source.adapterIdentity,
    recipeHash: source.recipeHash,
    envelopeHash: source.envelope.integrity.canonicalHash,
  ...
  const restoreBlueprint = Buffer.from(
    buildCalendarLiveV50RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildCalendarLiveV50ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildCalendarLiveV50ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildCalendarLiveV50CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );
  const cleanupBlueprint = Buffer.from(
    buildCalendarLiveV50CleanupProgram(writerOwnershipBlueprint),
  );
  ...
  antecedentOutputs.set(
    CALENDAR_LIVE_V50_PROTOCOL_PATH,
    jsonBytes(protocol(expectedFacts)),
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V50_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V50_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V50_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V50_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V50_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V50_EVIDENCE_ROOT}/programs/cleanup-blueprint.js`,
    cleanupBlueprint,
  );

  const captureManifest = {
    artifactVersion: "calendar-live-v50-capture-manifest-v1",
  ...
  antecedentOutputs.set(
    CALENDAR_LIVE_V50_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: CalendarLiveV50ProofPlan = {
    artifactVersion: "calendar-live-v50-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: CALENDAR_LIVE_V50_TARGET,
    namespace: CALENDAR_FIGMA_NAMESPACE,
    writer: {
      programPath: `${CALENDAR_LIVE_V50_EVIDENCE_ROOT}/programs/writer.txt`,
    ...
    captureManifest: {
      path: CALENDAR_LIVE_V50_CAPTURE_MANIFEST_PATH,
      cells: 8,
      requests: 8,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(CALENDAR_LIVE_V50_PLAN_PATH, jsonBytes(proofPlan));

  const requests = {
    artifactVersion: "calendar-live-v50-request-manifest-v1",
    expectedDynamicTool: CALENDAR_LIVE_V50_DYNAMIC_TOOL,
    target: CALENDAR_LIVE_V50_TARGET,
    signedAtRuntime: true,
    requestCount: CALENDAR_LIVE_V50_REMOTE_REQUESTS,
    requests: [
      {
        requestId: "writer",
  ...
  if (requests.requestCount !== CALENDAR_LIVE_V50_REMOTE_REQUESTS)
    throw new Error("Calendar live v50 remote request denominator drifted");
  antecedentOutputs.set(
    CALENDAR_LIVE_V50_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const lifecycleExcludedPaths = [
    CALENDAR_LIVE_V50_AUTHORIZATION_TEMPLATE_PATH,
    CALENDAR_LIVE_V50_AUTHORIZATION_PATH,
    "recipe/calendar-live-v50-authorization.ts",
    "recipe/calendar-live-v50-authorization.test.ts",
    "recipe/calendar-live-v50-preflight.ts",
    "recipe/calendar-live-v50-authorized.ts",
    "recipe/write-calendar-live-v50-authorization.ts",
    "recipe/create-calendar-live-v50-security-attestation.ts",
    "recipe/evidence/calendar-live-pivot-v50/operator-security-attestation-template.json",
    "recipe/evidence/calendar-live-pivot-v50-status.json",
    "recipe/evidence/status-index.json",
  ];
  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/calendar-live-v50-broker.ts",
    "recipe/calendar-live-v50-contract.ts",
    "recipe/calendar-live-v50-restore.ts",
    "recipe/calendar-live-v50-cleanup.ts",
    "recipe/calendar-live-v50-verifier.ts",
    "recipe/calendar-live-v50-fixed-point.ts",
    "recipe/run-calendar-live-v50.ts",
    "recipe/build-calendar-live-proof-v50.ts",
    "recipe/calendar-live-v50-lifecycle-simulation.ts",
    ...
    "recipe/normalize.ts",
  ].filter((artifactPath) => artifactPath !== CALENDAR_LIVE_V50_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "calendar-live-v50-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
  ...
  antecedentOutputs.set(CALENDAR_LIVE_V50_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "calendar-live-v50-authorization-template-v1",
    authorizationId: "calendar-live-v50",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: CALENDAR_LIVE_V50_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
    ...
    denominator: {
      remoteRequests: CALENDAR_LIVE_V50_REMOTE_REQUESTS,
      hostPhases: CALENDAR_LIVE_V50_HOST_PHASES,
      captures: CALENDAR_LIVE_V50_CAPTURE_COUNT,
      sourceRoots: CALENDAR_LIVE_V50_SOURCE_ROOTS,
      expectedFacts,
    },
    execution: {
  ...
  const attestationTemplate = jsonBytes({
    artifactVersion: "calendar-live-v50-operator-security-attestation-template-v1",
    status: "template only; complete privately after PREPARE+AUTHORIZE",
    tokenValuesForbidden: true,
    privateDirectory: "private/calendar-live-v50-security-attestation.json",
  });
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Calendar live v50 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(CALENDAR_LIVE_V50_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(CALENDAR_LIVE_V50_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
    writeFileSync(
      `${CALENDAR_LIVE_V50_EVIDENCE_ROOT}/operator-security-attestation-template.json`,
      attestationTemplate,
    );
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(CALENDAR_LIVE_V50_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(CALENDAR_LIVE_V50_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Calendar live v50 authorization template drift");
  }
  if (
    writer.namespace !== CALENDAR_FIGMA_NAMESPACE ||
    writer.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    ...
    !readFileSync("recipe/calendar-figma-writer.ts", "utf8")
      .includes("CALENDAR-WRITER-FILL-TEXT-NAMED-FALLBACK-AFTER-ZERO-GLYPH") ||
    !writerProgram
      .toString("utf8")
    ...
    !readFileSync("recipe/calendar-live-v50-contract.ts", "utf8").includes(
    ...
    throw new Error("Calendar live v50 identity or cleanup invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildCalendarLiveV50Proof(), null, 2)}\n`,
  );
