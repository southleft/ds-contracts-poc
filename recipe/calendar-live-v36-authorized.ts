...
import { CALENDAR_LIVE_V36_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v36.js";
import { verifyCalendarLiveV36Authorization } from "./calendar-live-v36-authorization.js";
import { runCalendarLiveV36Preflight } from "./calendar-live-v36-preflight.js";
import { CalendarLiveV36Orchestrator } from "./run-calendar-live-v36.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v36 requires exact authorization artifact ${CALENDAR_LIVE_V36_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v36-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV36Authorization(root);
    runCalendarLiveV36Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV36Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV36Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
