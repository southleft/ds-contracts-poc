...
import { buildComboboxLiveV18CleanupRuntime } from "./combobox-live-v18-cleanup.js";
import {
  type ComboboxLiveV18FixedFixedPoint,
} from "./combobox-live-v18-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV18RestoreProgram,
  validateComboboxLiveV18RestorePayload,
  type ComboboxLiveV18RestorePayload,
} from "./combobox-live-v18-restore.js";
import {
  buildComboboxLiveV18RawPropertyRuntime,
  normalizeComboboxLiveV18Scene,
  type ComboboxLiveV18RawNode,
} from "./combobox-live-v18-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v18.js";
import {
...
} from "./scene-readback-combobox-v18.js";
...
export const COMBOBOX_LIVE_V18_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V18_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V18_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V18_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V18_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V18_SET_COUNT = 4;
export const COMBOBOX_LIVE_V18_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V18_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V18_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V18_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V18_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV18SourceId = (typeof COMBOBOX_LIVE_V18_SOURCE_IDS)[number];

export interface ComboboxLiveV18SourceIdentity {
  source: ComboboxLiveV18SourceId;
...
export interface ComboboxLiveV18WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV18ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV18SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV18RawNode;
    optionScene: ComboboxLiveV18RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV18RootProof {
  source: ComboboxLiveV18SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV18FixedFixedPoint;
}

export interface ComboboxLiveV18ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV18SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV18CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV18SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV18CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV18SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV18CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v18 ${label} must be an object`);
...
export function validateComboboxLiveV18WriterPayload(
  value: unknown,
): ComboboxLiveV18WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V18_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v18 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v18 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v18 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV18SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV18ExtractProgram(
  writer: ComboboxLiveV18WriterOwnership,
  sources: readonly ComboboxLiveV18SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v18 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V18_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V18_NAMESPACE)}
${buildComboboxLiveV18RawPropertyRuntime()}
...
export function validateComboboxLiveV18ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV18WriterOwnership,
): ComboboxLiveV18ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v18 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V18_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v18 extract schema/two-root mismatch");
  return payload as ComboboxLiveV18ExtractPayload;
}

export function proveComboboxLiveV18Roots<Instance>(
  extract: ComboboxLiveV18ExtractPayload,
  sources: readonly (ComboboxLiveV18SourceIdentity & {
...
): ComboboxLiveV18RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v18 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v18 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV18Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV18Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV18FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v18 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v18 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v18 compile lost owned sets");
...
export function assertComboboxLiveV18RootProofs(
  proofs: readonly ComboboxLiveV18RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v18 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v18 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV18ProbeProgram(
  writer: ComboboxLiveV18WriterOwnership,
  sources: readonly ComboboxLiveV18SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V18_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV18ProbePayload(
  value: unknown,
  writer: ComboboxLiveV18WriterOwnership,
): ComboboxLiveV18ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV18ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V18_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V18_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v18 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV18CaptureManifest(
  cells: readonly ComboboxLiveV18CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V18_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V18_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V18_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V18_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v18 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV18CaptureCell[],
): string => {
  validateComboboxLiveV18CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV18CaptureProgram(
  writer: ComboboxLiveV18WriterOwnership,
  cell: ComboboxLiveV18CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V18_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v18 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v18 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V18_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV18CapturePayload(
  value: unknown,
  cell: ComboboxLiveV18CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV18CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV18CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V18_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V18_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v18 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV18CaptureResponses(
  manifest: readonly ComboboxLiveV18CaptureCell[],
  responses: readonly ComboboxLiveV18CapturePayload[],
): void {
  validateComboboxLiveV18CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v18 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV18ObjectiveReport {
  artifactVersion: "combobox-live-v18-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV18Objective(
  manifest: readonly ComboboxLiveV18CaptureCell[],
  responses: readonly ComboboxLiveV18CapturePayload[],
): ComboboxLiveV18ObjectiveReport {
  assertComboboxLiveV18CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v18-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV18CleanupProgram(
  writer: ComboboxLiveV18WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV18CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V18_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v18 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV18CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV18WriterOwnership,
): ComboboxLiveV18CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV18CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v18 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V18_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV18WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
