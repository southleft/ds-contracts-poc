...
import { INPUT_LIVE_V70_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v70.js";
import { verifyInputLiveV70Authorization } from "./input-field-live-v70-authorization.js";
import { runInputLiveV70Preflight } from "./input-field-live-v70-preflight.js";
import { InputLiveV70Orchestrator } from "./run-input-field-live-v70.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V70_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v70-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV70Authorization(root);
    runInputLiveV70Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV70Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
