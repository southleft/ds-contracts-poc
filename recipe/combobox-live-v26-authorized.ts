...
import { COMBOBOX_LIVE_V26_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v26.js";
import { verifyComboboxLiveV26Authorization } from "./combobox-live-v26-authorization.js";
import { runComboboxLiveV26Preflight } from "./combobox-live-v26-preflight.js";
import { ComboboxLiveV26Orchestrator } from "./run-combobox-live-v26.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v26 requires exact authorization artifact ${COMBOBOX_LIVE_V26_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v26-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV26Authorization(root);
    runComboboxLiveV26Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV26Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV26Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
