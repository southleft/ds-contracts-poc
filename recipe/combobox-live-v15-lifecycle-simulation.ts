...
export interface ComboboxLiveV15LifecycleSimulationReport {
  artifactVersion: "combobox-live-v15-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV15AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV15Lifecycle(): Promise<ComboboxLiveV15LifecycleSimulationReport> {
  await buildComboboxLiveV15Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V15_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV15AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v15-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V15_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V15_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV15History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV15History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV15AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V15_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V15_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV15History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV15History(authorized, "pending");
    ...
    await buildComboboxLiveV15Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V15_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV15AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV15Lifecycle(), null, 2)}\n`,
  );
