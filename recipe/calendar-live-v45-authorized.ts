...
import { CALENDAR_LIVE_V45_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v45.js";
import { verifyCalendarLiveV45Authorization } from "./calendar-live-v45-authorization.js";
import { runCalendarLiveV45Preflight } from "./calendar-live-v45-preflight.js";
import { CalendarLiveV45Orchestrator } from "./run-calendar-live-v45.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v45 requires exact authorization artifact ${CALENDAR_LIVE_V45_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v45-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV45Authorization(root);
    runCalendarLiveV45Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV45Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV45Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
