...
import { buildCalendarLiveV39CleanupRuntime } from "./calendar-live-v39-cleanup.js";
import { type CalendarLiveV39FixedFixedPoint } from "./calendar-live-v39-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV39RestoreProgram,
  validateCalendarLiveV39RestorePayload,
  type CalendarLiveV39RestorePayload,
} from "./calendar-live-v39-restore.js";
import {
  buildCalendarLiveV39RawPropertyRuntime,
  normalizeCalendarLiveV39Scene,
  type CalendarLiveV39RawNode,
} from "./calendar-live-v39-verifier.js";
...
export const CALENDAR_LIVE_V39_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V39_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V39_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V39_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V39_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V39_SET_COUNT = 3;
export const CALENDAR_LIVE_V39_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V39_HOST_PHASES = 3;
export const CALENDAR_LIVE_V39_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V39_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V39_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V39_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV39SourceId = (typeof CALENDAR_LIVE_V39_SOURCE_IDS)[number];
export type CalendarLiveV39Kind = "calendar" | "week" | "day";

export interface CalendarLiveV39SourceIdentity {
  source: CalendarLiveV39SourceId;
...
export interface CalendarLiveV39WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV39ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV39SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV39RawNode;
    weekScene: CalendarLiveV39RawNode;
    dayScene: CalendarLiveV39RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV39RootProof {
  source: CalendarLiveV39SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV39FixedFixedPoint;
}

export interface CalendarLiveV39ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV39SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV39CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV39SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV39CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV39SourceId;
  frameWidth: number;
...
export interface CalendarLiveV39CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v39 ${label} must be an object`);
...
export function validateCalendarLiveV39WriterPayload(
  value: unknown,
): CalendarLiveV39WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v39") ||
    ...
    result.namespace !== CALENDAR_LIVE_V39_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v39 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v39 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v39 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV39SourceIdentity,
  kind: CalendarLiveV39Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV39ExtractProgram(
  writer: CalendarLiveV39WriterOwnership,
  sources: readonly CalendarLiveV39SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v39 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V39_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V39_NAMESPACE)}
${buildCalendarLiveV39RawPropertyRuntime()}
...
export function validateCalendarLiveV39ExtractPayload(
  value: unknown,
  writer: CalendarLiveV39WriterOwnership,
): CalendarLiveV39ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v39 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V39_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v39 extract schema/one-root mismatch");
  return payload as CalendarLiveV39ExtractPayload;
}

export function proveCalendarLiveV39Roots<Instance>(
  extract: CalendarLiveV39ExtractPayload,
  sources: readonly (CalendarLiveV39SourceIdentity & {
...
): CalendarLiveV39RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v39 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v39 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV39Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV39Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV39Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV39FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v39 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v39 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v39 compile lost owned sets");
...
export function assertCalendarLiveV39RootProofs(
  proofs: readonly CalendarLiveV39RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v39 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v39 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV39ProbeProgram(
  writer: CalendarLiveV39WriterOwnership,
  sources: readonly CalendarLiveV39SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V39_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV39ProbePayload(
  value: unknown,
  writer: CalendarLiveV39WriterOwnership,
): CalendarLiveV39ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV39ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V39_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V39_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v39 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV39CaptureManifest(
  cells: readonly CalendarLiveV39CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V39_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V39_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V39_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V39_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v39 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV39CaptureCell[],
): string => {
  validateCalendarLiveV39CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV39CaptureProgram(
  writer: CalendarLiveV39WriterOwnership,
  cell: CalendarLiveV39CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V39_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v39 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v39 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V39_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV39CapturePayload(
  value: unknown,
  cell: CalendarLiveV39CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV39CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV39CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V39_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V39_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v39 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV39CaptureResponses(
  manifest: readonly CalendarLiveV39CaptureCell[],
  responses: readonly CalendarLiveV39CapturePayload[],
): void {
  validateCalendarLiveV39CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v39 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV39ObjectiveReport {
  artifactVersion: "calendar-live-v39-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV39Objective(
  manifest: readonly CalendarLiveV39CaptureCell[],
  responses: readonly CalendarLiveV39CapturePayload[],
): CalendarLiveV39ObjectiveReport {
  assertCalendarLiveV39CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v39-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV39CleanupProgram(
  writer: CalendarLiveV39WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV39CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V39_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v39 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV39CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV39WriterOwnership,
): CalendarLiveV39CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV39CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v39 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V39_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV39WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
