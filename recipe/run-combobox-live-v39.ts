...
import {
  acceptComboboxLiveV39Response,
  createComboboxLiveV39Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV39Request,
  persistComboboxLiveV39RawResponse,
  persistComboboxLiveV39TechnicalGates,
  readComboboxLiveV39AcceptedRawPayload,
  type ComboboxLiveV39Request,
  type ComboboxLiveV39TransactionAuthorization,
} from "./combobox-live-v39-broker.js";
import {
  COMBOBOX_LIVE_V39_CAPTURE_COUNT,
  assertComboboxLiveV39CaptureResponses,
  ...
  evaluateComboboxLiveV39Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV39Roots,
  validateComboboxLiveV39CaptureManifest,
  validateComboboxLiveV39CapturePayload,
  validateComboboxLiveV39CleanupPayload,
  validateComboboxLiveV39ExtractPayload,
  validateComboboxLiveV39ProbePayload,
  validateComboboxLiveV39WriterPayload,
  type ComboboxLiveV39CaptureCell,
  type ComboboxLiveV39CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V39_PLAN_PATH,
  type ComboboxLiveV39ProofPlan,
} from "./build-combobox-live-proof-v39.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v39.js";

export const COMBOBOX_LIVE_V39_RUNNER_VERSION = "combobox-live-v39-runner-v1";

interface ComboboxLiveV39JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV39OrchestratorState {
  artifactVersion: "combobox-live-v39-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV39JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV39ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV39CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV39SourceIdentity & {
...
export interface ComboboxLiveV39NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV39OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v39 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V39_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V39_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV39ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV39CaptureCell[];
  validateComboboxLiveV39CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v39 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v39 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV39WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV39RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV39Request =>
  readJson<ComboboxLiveV39Request>(file);

export class ComboboxLiveV39Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV39Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV39OrchestratorState = {
      artifactVersion: "combobox-live-v39-orchestrator-state-v1",
    ...
    return new ComboboxLiveV39Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV39OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV39Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV39NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV39NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v39 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV39NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v39 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV39RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV39Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV39CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV39AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV39WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV39Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV39ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV39Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV39Response(this.#directory, "writer", {
        validate: validateComboboxLiveV39WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV39Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV39RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV39Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV39ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV39Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV39RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v39-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV39Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV39ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV39Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV39ProbePayload(payload, ownership),
      });
      persistComboboxLiveV39TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v39 capture cell missing");
      const accepted = acceptComboboxLiveV39Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV39CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V39_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV39CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV39Objective(
    ...
    throw new Error(`Combobox live v39 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV39Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV39CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV39Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV39RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV39Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV39CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV39CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV39CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV39Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV39TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV39SimulationKey = (): KeyObject =>
