...
import {
  ...
  verifyInputLiveV27Authorization,
  type InputLiveV27AntecedentIndex,
  type InputLiveV27AuthorizationProof,
} from "./input-field-live-v27-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V27_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v27/operator-security-attestation-template.json";
export const INPUT_LIVE_V27_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v27-security-attestation.json";

export interface InputLiveV27SecurityAttestation {
...
export interface InputLiveV27PreflightReport {
  artifactVersion: "input-live-v27-preflight-v1";
...
export const inputLiveV27AttestationSha256 = (
  value: Omit<InputLiveV27SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV27SecurityAttestation(
  value: unknown,
  proof: InputLiveV27AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v27-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V27_TARGET) ||
...
export function validateInputLiveV27ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV27Authorization");
  const preflight = source.indexOf("runInputLiveV27Preflight");
  const initialize = source.indexOf("InputLiveV27Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV27Preflight(
  root: string,
  proof: InputLiveV27AuthorizationProof,
  attestationPath = INPUT_LIVE_V27_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV27PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V27_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V27_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V27_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV27AntecedentIndex;
  failures.push(...validateInputLiveV27AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V27_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V27_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v27-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV27ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV27SecurityAttestation;
  return {
    artifactVersion: "input-live-v27-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V27_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV27Preflight(
    process.cwd(),
    verifyInputLiveV27Authorization(),
    attestationPath,
