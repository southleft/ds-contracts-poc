...
import {
  ...
  INPUT_LIVE_V65_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V65_RESPONSE_CONTRACTS,
  type InputLiveV65RootProof,
} from "./input-field-live-v65-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V65_BROKER_VERSION = "input-live-v65-broker-v2";
export const INPUT_LIVE_V65_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V65_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V65_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V65_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V65_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V65_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV65RemotePhase =
  (typeof INPUT_LIVE_V65_REMOTE_PHASES)[number];
export type InputLiveV65AuthorizationMode = "live" | "simulated";

export interface InputLiveV65CommitPins {
...
export interface InputLiveV65TransactionAuthorization extends InputLiveV65CommitPins {
  mode: InputLiveV65AuthorizationMode;
...
export interface InputLiveV65Manifest {
  artifactVersion: "input-live-v65-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V65_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V65_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V65_TARGET;
...
export interface InputLiveV65ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV65RequestBody {
  artifactVersion: "input-live-v65-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V65_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV65RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV65CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V65_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV65ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV65Request extends InputLiveV65RequestBody {
...
export interface InputLiveV65Receipt {
  artifactVersion: "input-live-v65-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V65_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV65RemotePhase;
...
export interface InputLiveV65AcceptedResponse<T = unknown> {
  request: InputLiveV65Request;
  receipt: InputLiveV65Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV65IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV65ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V65_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V65_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V65_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V65_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V65_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V65_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V65_TARGET.editorType;
};
...
export const inputLiveV65RequestSequence = (
  phase: InputLiveV65RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV65RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV65RequestPath = (
  directory: string,
  phase: InputLiveV65RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV65ResponsePath = (
  directory: string,
  phase: InputLiveV65RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV65ReceiptPath = (
  directory: string,
  phase: InputLiveV65RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV65Manifest => {
  const manifest = json<InputLiveV65Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v65-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V65_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V65_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV65RemotePhase,
  captureIndex?: number,
): InputLiveV65Request | undefined => {
  const file = inputLiveV65RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV65Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV65RemotePhase,
  captureIndex?: number,
): InputLiveV65Receipt | undefined => {
  const file = inputLiveV65ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV65Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV65Request | undefined;
  receipt: InputLiveV65Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV65RemotePhase,
): InputLiveV65ResponseContract => ({
  ...INPUT_LIVE_V65_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V65_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V65_DYNAMIC_TOOL,
      target: INPUT_LIVE_V65_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v65-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V65_BROKER_VERSION,
  target: INPUT_LIVE_V65_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V65_PROGRAM_START*/${program}/*INPUT_LIVE_V65_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V65_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V65_PROGRAM_END*/";
...
export function createInputLiveV65Transaction(
  directory: string,
  ...
  const manifest: InputLiveV65Manifest = {
    artifactVersion: "input-live-v65-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V65_BROKER_VERSION,
...
export function validateInputLiveV65Request(
  directory: string,
  request: InputLiveV65Request,
): void {
  ...
  const expectedSequence = inputLiveV65RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v65-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V65_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V65_TARGET.fileKey ||
...
export function issueInputLiveV65Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV65RemotePhase,
  program: string,
  options: InputLiveV65IssueOptions = {},
): InputLiveV65Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV65RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV65Request>(targetPath);
    validateInputLiveV65Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV65ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV65TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV65RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V65_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V65_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V65_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV65RequestBody = {
    artifactVersion: "input-live-v65-broker-request-v2",
    brokerVersion: INPUT_LIVE_V65_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV65Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV65Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV65RawResponse(
  directory: string,
  phase: InputLiveV65RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV65RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV65Request>(requestFile);
  if (
  ...
  const target = inputLiveV65ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV65Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v65-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V65_BROKER_VERSION,
...
export function acceptInputLiveV65Response<T = unknown>(
  directory: string,
  phase: InputLiveV65RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV65AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV65ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV65Request>(
    inputLiveV65RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV65Request(directory, request);
  if (phase === "capture") validateInputLiveV65TechnicalGates(directory);
  const rawFile = inputLiveV65ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v65-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V65_BROKER_VERSION ||
...
export function persistInputLiveV65TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV65RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV65RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V65_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v65-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V65_BROKER_VERSION,
...
export function validateInputLiveV65TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v65-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V65_TECHNICAL_GATES.some(
...
export function validateInputLiveV65Receipt(
  directory: string,
  phase: InputLiveV65RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV65Request(directory, request);
  const receipt = json<InputLiveV65Receipt>(
    inputLiveV65ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV65ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v65-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V65_BROKER_VERSION ||
...
export function readInputLiveV65AcceptedRawPayload(
  directory: string,
  phase: InputLiveV65RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV65Receipt(directory, phase, captureIndex);
...
export function createInputLiveV65FakeDynamicResponse(
  request: InputLiveV65Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V65_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V65_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV65PersistedRequests(
  directory: string,
): InputLiveV65Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV65Request>(file)] : [];
    });
}

export const inputLiveV65Sha256 = sha256;
