...
import {
  assertInputLiveV25RootProofs,
  INPUT_LIVE_V25_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V25_RESPONSE_CONTRACTS,
  type InputLiveV25RootProof,
} from "./input-field-live-v25-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V25_BROKER_VERSION = "input-live-v25-broker-v2";
export const INPUT_LIVE_V25_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V25_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V25_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V25_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V25_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V25_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV25RemotePhase =
  (typeof INPUT_LIVE_V25_REMOTE_PHASES)[number];
export type InputLiveV25AuthorizationMode = "live" | "simulated";

export interface InputLiveV25CommitPins {
...
export interface InputLiveV25TransactionAuthorization extends InputLiveV25CommitPins {
  mode: InputLiveV25AuthorizationMode;
...
export interface InputLiveV25Manifest {
  artifactVersion: "input-live-v25-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V25_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V25_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V25_TARGET;
  authorization: InputLiveV25TransactionAuthorization;
...
export interface InputLiveV25ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV25RequestBody {
  artifactVersion: "input-live-v25-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V25_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV25RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV25CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V25_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV25ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV25Request extends InputLiveV25RequestBody {
...
export interface InputLiveV25Receipt {
  artifactVersion: "input-live-v25-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V25_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV25RemotePhase;
...
export interface InputLiveV25AcceptedResponse<T = unknown> {
  request: InputLiveV25Request;
  receipt: InputLiveV25Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV25IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV25ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V25_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V25_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V25_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V25_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V25_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V25_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V25_TARGET.editorType;
};
...
export const inputLiveV25RequestSequence = (
  phase: InputLiveV25RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV25RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV25RequestPath = (
  directory: string,
  phase: InputLiveV25RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV25ResponsePath = (
  directory: string,
  phase: InputLiveV25RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV25ReceiptPath = (
  directory: string,
  phase: InputLiveV25RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV25Manifest => {
  const manifest = json<InputLiveV25Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v25-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V25_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V25_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV25RemotePhase,
  captureIndex?: number,
): InputLiveV25Request | undefined => {
  const file = inputLiveV25RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV25Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV25RemotePhase,
  captureIndex?: number,
): InputLiveV25Receipt | undefined => {
  const file = inputLiveV25ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV25Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV25Request | undefined;
  receipt: InputLiveV25Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV25RemotePhase,
): InputLiveV25ResponseContract => ({
  ...INPUT_LIVE_V25_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V25_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V25_DYNAMIC_TOOL,
      target: INPUT_LIVE_V25_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v25-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V25_BROKER_VERSION,
  target: INPUT_LIVE_V25_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V25_PROGRAM_START*/${program}/*INPUT_LIVE_V25_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V25_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V25_PROGRAM_END*/";
...
export function createInputLiveV25Transaction(
  directory: string,
  ...
  const manifest: InputLiveV25Manifest = {
    artifactVersion: "input-live-v25-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V25_BROKER_VERSION,
...
export function validateInputLiveV25Request(
  directory: string,
  request: InputLiveV25Request,
): void {
  ...
  const expectedSequence = inputLiveV25RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v25-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V25_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V25_TARGET.fileKey ||
...
export function issueInputLiveV25Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV25RemotePhase,
  program: string,
  options: InputLiveV25IssueOptions = {},
): InputLiveV25Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV25RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV25Request>(targetPath);
    validateInputLiveV25Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV25ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV25TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV25RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V25_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V25_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V25_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV25RequestBody = {
    artifactVersion: "input-live-v25-broker-request-v2",
    brokerVersion: INPUT_LIVE_V25_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV25Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV25Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV25RawResponse(
  directory: string,
  phase: InputLiveV25RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV25RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV25Request>(requestFile);
  if (
  ...
  const target = inputLiveV25ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV25Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v25-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V25_BROKER_VERSION,
...
export function acceptInputLiveV25Response<T = unknown>(
  directory: string,
  phase: InputLiveV25RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV25AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV25ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV25Request>(
    inputLiveV25RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV25Request(directory, request);
  if (phase === "capture") validateInputLiveV25TechnicalGates(directory);
  const rawFile = inputLiveV25ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v25-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V25_BROKER_VERSION ||
...
export function persistInputLiveV25TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV25RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV25RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V25_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v25-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V25_BROKER_VERSION,
...
export function validateInputLiveV25TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v25-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V25_TECHNICAL_GATES.some(
...
export function validateInputLiveV25Receipt(
  directory: string,
  phase: InputLiveV25RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV25Request(directory, request);
  const receipt = json<InputLiveV25Receipt>(
    inputLiveV25ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV25ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v25-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V25_BROKER_VERSION ||
...
export function readInputLiveV25AcceptedRawPayload(
  directory: string,
  phase: InputLiveV25RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV25Receipt(directory, phase, captureIndex);
...
export function createInputLiveV25FakeDynamicResponse(
  request: InputLiveV25Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V25_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V25_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV25PersistedRequests(
  directory: string,
): InputLiveV25Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV25Request>(file)] : [];
    });
}

export const inputLiveV25Sha256 = sha256;
