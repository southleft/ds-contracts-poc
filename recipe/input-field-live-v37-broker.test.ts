...
import {
  ...
  createInputLiveV37FakeDynamicResponse,
  ...
  listInputLiveV37PersistedRequests,
  persistInputLiveV37RawResponse,
  persistInputLiveV37TechnicalGates,
  validateInputLiveV37Receipt,
  validateInputLiveV37Request,
  type InputLiveV37RemotePhase,
...
import {
  ...
  type InputLiveV37ExtractPayload,
  type InputLiveV37ProbePayload,
  type InputLiveV37RestorePayload,
  type InputLiveV37RootProof,
  type InputLiveV37WriterOwnership,
} from "./input-field-live-v37-contract.js";
import {
  InputLiveV37Orchestrator,
  simulatedInputLiveV37Authorization,
} from "./run-input-field-live-v37.js";
...
const captures: InputLiveV37CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV37Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v37-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV37Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV37CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV37FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV37FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V37_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V37_TARGET);
  persistInputLiveV37RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV37Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV37Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV37WriterOwnership,
): InputLiveV37RestorePayload => ({
...
const rootProofs = (): InputLiveV37RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV37CaptureCell,
): InputLiveV37CapturePayload => {
  const bytes = Buffer.from(`input-live-v37-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV37Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV37Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV37RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV37RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV37ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV37ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV37CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV37CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV37CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV37Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV37CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV37PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV37ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV37Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV37Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV37Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV37Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V37_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV37RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V37_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV37Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV37Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV37ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV37Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV37WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV37RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV37Request;
    validateInputLiveV37Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV37Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV37CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v37-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV37Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV37Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV37Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V37_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV37RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V37_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV37Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV37Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV37RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V37_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV37Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV37Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV37Request;
