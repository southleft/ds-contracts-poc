...
import {
  validateComboboxLiveV16AntecedentIndex,
  verifyComboboxLiveV16Authorization,
  type ComboboxLiveV16AntecedentIndex,
  type ComboboxLiveV16AuthorizationProof,
} from "./combobox-live-v16-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V16_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v16/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V16_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v16-security-attestation.json";

export interface ComboboxLiveV16SecurityAttestation {
  artifactVersion: "combobox-live-v16-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV16PreflightReport {
  artifactVersion: "combobox-live-v16-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV16SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV16SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV16AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v16-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V16_TARGET) ||
...
export function validateComboboxLiveV16ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV16Authorization");
  const preflight = source.indexOf("runComboboxLiveV16Preflight");
  const initialize = source.indexOf("ComboboxLiveV16Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV16Preflight(
  root: string,
  proof: ComboboxLiveV16AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V16_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV16PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V16_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V16_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V16_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV16AntecedentIndex;
  failures.push(...validateComboboxLiveV16AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V16_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V16_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV16SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v16-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV16ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v16 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV16SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v16-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V16_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV16Preflight(
    process.cwd(),
    verifyComboboxLiveV16Authorization(),
    attestationPath,
