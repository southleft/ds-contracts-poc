...
import {
  ...
  verifyInputLiveV39Authorization,
  type InputLiveV39AntecedentIndex,
  type InputLiveV39AuthorizationProof,
} from "./input-field-live-v39-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V39_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v39/operator-security-attestation-template.json";
export const INPUT_LIVE_V39_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v39-security-attestation.json";

export interface InputLiveV39SecurityAttestation {
...
export interface InputLiveV39PreflightReport {
  artifactVersion: "input-live-v39-preflight-v1";
...
export const inputLiveV39AttestationSha256 = (
  value: Omit<InputLiveV39SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV39SecurityAttestation(
  value: unknown,
  proof: InputLiveV39AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v39-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V39_TARGET) ||
...
export function validateInputLiveV39ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV39Authorization");
  const preflight = source.indexOf("runInputLiveV39Preflight");
  const initialize = source.indexOf("InputLiveV39Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV39Preflight(
  root: string,
  proof: InputLiveV39AuthorizationProof,
  attestationPath = INPUT_LIVE_V39_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV39PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V39_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V39_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V39_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV39AntecedentIndex;
  failures.push(...validateInputLiveV39AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V39_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V39_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v39-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV39ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV39SecurityAttestation;
  return {
    artifactVersion: "input-live-v39-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V39_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV39Preflight(
    process.cwd(),
    verifyInputLiveV39Authorization(),
    attestationPath,
