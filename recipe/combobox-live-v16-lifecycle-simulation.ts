...
export interface ComboboxLiveV16LifecycleSimulationReport {
  artifactVersion: "combobox-live-v16-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV16AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV16Lifecycle(): Promise<ComboboxLiveV16LifecycleSimulationReport> {
  await buildComboboxLiveV16Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V16_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV16AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v16-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V16_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V16_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV16History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV16History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV16AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V16_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V16_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV16History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV16History(authorized, "pending");
    ...
    await buildComboboxLiveV16Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V16_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV16AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV16Lifecycle(), null, 2)}\n`,
  );
