...
export type ComboboxLiveV6HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV6AuthorizationArtifact {
  artifactVersion: "combobox-live-v6-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V6_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV6AntecedentIndex {
...
export interface ComboboxLiveV6HistoryState {
  index?: ComboboxLiveV6AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV6AuthorizationArtifact;
...
export interface ComboboxLiveV6AuthorizationProof extends ComboboxLiveV6TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V6_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V6_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V6_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V6_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v6-authorization") ||
  artifactPath.includes("combobox-live-v6-preflight") ||
  artifactPath.includes("combobox-live-v6-authorized") ||
  artifactPath.includes("create-combobox-live-v6-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v6-status") ||
...
export function validateComboboxLiveV6AntecedentIndex(
  index: ComboboxLiveV6AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v6-antecedent-index-v1" ||
...
export function validateComboboxLiveV6UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV6History\(\s*["']/.test(source) ||
    /readComboboxLiveV6HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV6AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV6AuthorizationArtifact {
  ...
  ) as ComboboxLiveV6AntecedentIndex;
  const indexFailures = validateComboboxLiveV6AntecedentIndex(index);
...
export function validateComboboxLiveV6AuthorizationArtifact(
  artifact: ComboboxLiveV6AuthorizationArtifact | undefined,
  index: ComboboxLiveV6AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v6-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v6" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V6_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V6_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V6_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V6_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V6_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V6_SOURCE_ROOTS ||
...
export function validateComboboxLiveV6History(
  state: ComboboxLiveV6HistoryState,
  expected: ComboboxLiveV6HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV6AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV6AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV6HistoryState(
  root = process.cwd(),
): ComboboxLiveV6HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V6_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV6AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V6_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V6_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V6_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV6History(
  expected: ComboboxLiveV6HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV6HistoryState {
  const state = readComboboxLiveV6HistoryState(root);
  const failures = validateComboboxLiveV6History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v6 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV6Authorization(
  root = process.cwd(),
): ComboboxLiveV6AuthorizationProof {
  const state = verifyComboboxLiveV6History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V6_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v6/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v6.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V6_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V6_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V6_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV6HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV6History(expected);
  process.stdout.write(
    `Combobox live v6 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
