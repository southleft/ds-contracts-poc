...
export type ComboboxLiveV27HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV27AuthorizationArtifact {
  artifactVersion: "combobox-live-v27-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V27_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV27AntecedentIndex {
...
export interface ComboboxLiveV27HistoryState {
  index?: ComboboxLiveV27AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV27AuthorizationArtifact;
...
export interface ComboboxLiveV27AuthorizationProof extends ComboboxLiveV27TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V27_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V27_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V27_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V27_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v27-authorization") ||
  artifactPath.includes("combobox-live-v27-preflight") ||
  artifactPath.includes("combobox-live-v27-authorized") ||
  artifactPath.includes("create-combobox-live-v27-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v27-status") ||
...
export function validateComboboxLiveV27AntecedentIndex(
  index: ComboboxLiveV27AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v27-antecedent-index-v1" ||
...
export function validateComboboxLiveV27UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV27History\(\s*["']/.test(source) ||
    /readComboboxLiveV27HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV27AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV27AuthorizationArtifact {
  ...
  ) as ComboboxLiveV27AntecedentIndex;
  const indexFailures = validateComboboxLiveV27AntecedentIndex(index);
...
export function validateComboboxLiveV27AuthorizationArtifact(
  artifact: ComboboxLiveV27AuthorizationArtifact | undefined,
  index: ComboboxLiveV27AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v27-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v27" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V27_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V27_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V27_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V27_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V27_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V27_SOURCE_ROOTS ||
...
export function validateComboboxLiveV27History(
  state: ComboboxLiveV27HistoryState,
  expected: ComboboxLiveV27HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV27AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV27AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV27HistoryState(
  root = process.cwd(),
): ComboboxLiveV27HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V27_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV27AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V27_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V27_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V27_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV27History(
  expected: ComboboxLiveV27HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV27HistoryState {
  const state = readComboboxLiveV27HistoryState(root);
  const failures = validateComboboxLiveV27History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v27 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV27Authorization(
  root = process.cwd(),
): ComboboxLiveV27AuthorizationProof {
  const state = verifyComboboxLiveV27History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V27_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v27/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v27.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V27_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V27_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V27_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV27HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV27History(expected);
  process.stdout.write(
    `Combobox live v27 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
