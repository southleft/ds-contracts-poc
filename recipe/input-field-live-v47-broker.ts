...
import {
  assertInputLiveV47RootProofs,
  INPUT_LIVE_V47_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V47_RESPONSE_CONTRACTS,
  type InputLiveV47RootProof,
} from "./input-field-live-v47-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V47_BROKER_VERSION = "input-live-v47-broker-v2";
export const INPUT_LIVE_V47_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V47_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V47_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V47_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V47_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V47_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV47RemotePhase =
  (typeof INPUT_LIVE_V47_REMOTE_PHASES)[number];
export type InputLiveV47AuthorizationMode = "live" | "simulated";

export interface InputLiveV47CommitPins {
...
export interface InputLiveV47TransactionAuthorization extends InputLiveV47CommitPins {
  mode: InputLiveV47AuthorizationMode;
...
export interface InputLiveV47Manifest {
  artifactVersion: "input-live-v47-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V47_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V47_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V47_TARGET;
  authorization: InputLiveV47TransactionAuthorization;
...
export interface InputLiveV47ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV47RequestBody {
  artifactVersion: "input-live-v47-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V47_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV47RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV47CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V47_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV47ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV47Request extends InputLiveV47RequestBody {
...
export interface InputLiveV47Receipt {
  artifactVersion: "input-live-v47-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V47_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV47RemotePhase;
...
export interface InputLiveV47AcceptedResponse<T = unknown> {
  request: InputLiveV47Request;
  receipt: InputLiveV47Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV47IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV47ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V47_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V47_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V47_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V47_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V47_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V47_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V47_TARGET.editorType;
};
...
export const inputLiveV47RequestSequence = (
  phase: InputLiveV47RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV47RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV47RequestPath = (
  directory: string,
  phase: InputLiveV47RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV47ResponsePath = (
  directory: string,
  phase: InputLiveV47RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV47ReceiptPath = (
  directory: string,
  phase: InputLiveV47RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV47Manifest => {
  const manifest = json<InputLiveV47Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v47-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V47_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V47_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV47RemotePhase,
  captureIndex?: number,
): InputLiveV47Request | undefined => {
  const file = inputLiveV47RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV47Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV47RemotePhase,
  captureIndex?: number,
): InputLiveV47Receipt | undefined => {
  const file = inputLiveV47ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV47Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV47Request | undefined;
  receipt: InputLiveV47Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV47RemotePhase,
): InputLiveV47ResponseContract => ({
  ...INPUT_LIVE_V47_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V47_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v47-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V47_BROKER_VERSION,
  target: INPUT_LIVE_V47_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V47_PROGRAM_START*/${program}/*INPUT_LIVE_V47_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V47_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V47_PROGRAM_END*/";
...
export function createInputLiveV47Transaction(
  directory: string,
  ...
  const manifest: InputLiveV47Manifest = {
    artifactVersion: "input-live-v47-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V47_BROKER_VERSION,
...
export function validateInputLiveV47Request(
  directory: string,
  request: InputLiveV47Request,
): void {
  ...
  const expectedSequence = inputLiveV47RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v47-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V47_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V47_TARGET.fileKey ||
...
export function issueInputLiveV47Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV47RemotePhase,
  program: string,
  options: InputLiveV47IssueOptions = {},
): InputLiveV47Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV47RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV47Request>(targetPath);
    validateInputLiveV47Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV47ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV47TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV47RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V47_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V47_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V47_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV47RequestBody = {
    artifactVersion: "input-live-v47-broker-request-v2",
    brokerVersion: INPUT_LIVE_V47_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV47Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV47Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV47RawResponse(
  directory: string,
  phase: InputLiveV47RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV47RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV47Request>(requestFile);
  if (
  ...
  const target = inputLiveV47ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV47Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v47-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V47_BROKER_VERSION,
...
export function acceptInputLiveV47Response<T = unknown>(
  directory: string,
  phase: InputLiveV47RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV47AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV47ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV47Request>(
    inputLiveV47RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV47Request(directory, request);
  if (phase === "capture") validateInputLiveV47TechnicalGates(directory);
  const rawFile = inputLiveV47ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v47-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V47_BROKER_VERSION ||
...
export function persistInputLiveV47TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV47RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV47RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V47_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v47-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V47_BROKER_VERSION,
...
export function validateInputLiveV47TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v47-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V47_TECHNICAL_GATES.some(
...
export function validateInputLiveV47Receipt(
  directory: string,
  phase: InputLiveV47RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV47Request(directory, request);
  const receipt = json<InputLiveV47Receipt>(
    inputLiveV47ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV47ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v47-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V47_BROKER_VERSION ||
...
export function readInputLiveV47AcceptedRawPayload(
  directory: string,
  phase: InputLiveV47RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV47Receipt(directory, phase, captureIndex);
...
export function createInputLiveV47FakeDynamicResponse(
  request: InputLiveV47Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V47_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V47_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV47PersistedRequests(
  directory: string,
): InputLiveV47Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV47Request>(file)] : [];
    });
}

export const inputLiveV47Sha256 = sha256;
