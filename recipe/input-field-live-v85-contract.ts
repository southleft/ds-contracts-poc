...
} from "./input-field-live-v3-verifier-v85.js";
import {
  buildInputLiveV85RawPropertyRuntime,
  normalizeInputLiveV85Scene,
  type InputLiveV85RawNode,
} from "./input-field-live-v85-verifier.js";
...
export {
  ...
  type InputLiveV85RestorePayload,
} from "./input-field-live-v85-restore.js";

export const INPUT_LIVE_V85_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V85_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V85_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V85_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V85_VARIANT_COUNT = 256;
export const INPUT_LIVE_V85_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V85_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV85SourceId = (typeof INPUT_LIVE_V85_SOURCE_IDS)[number];

export interface InputLiveV85SourceIdentity {
  source: InputLiveV85SourceId;
...
export interface InputLiveV85WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV85ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV85SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV85RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV85RootProof {
  source: InputLiveV85SourceId;
  adapterIdentity: string;
...
export interface InputLiveV85ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV85SourceId;
    adapterIdentity: string;
...
export interface InputLiveV85CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV85SourceId;
  adapterIdentity: string;
...
export interface InputLiveV85CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV85SourceId;
  strata: InputLiveV85CaptureCell["strata"];
...
export interface InputLiveV85CleanupPayload {
...
export function validateInputLiveV85WriterPayload(
  value: unknown,
): InputLiveV85WriterOwnership {
...
const identityPlan = (source: InputLiveV85SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV85ExtractProgram(
  writer: InputLiveV85WriterOwnership,
  sources: readonly InputLiveV85SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V85_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V85_NAMESPACE)}
${buildInputLiveV85RawPropertyRuntime()}
...
export function validateInputLiveV85ExtractPayload(
  value: unknown,
  writer: InputLiveV85WriterOwnership,
): InputLiveV85ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V85_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV85ExtractPayload;
}

export function proveInputLiveV85Roots<Instance>(
  extract: InputLiveV85ExtractPayload,
  sources: readonly (InputLiveV85SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV85Scene(
      root.scene,
...
export function assertInputLiveV85RootProofs(
  proofs: readonly InputLiveV85RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV85ProbeProgram(
  writer: InputLiveV85WriterOwnership,
  sources: readonly InputLiveV85SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V85_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV85ProbePayload(
  value: unknown,
  writer: InputLiveV85WriterOwnership,
): InputLiveV85ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV85ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V85_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V85_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV85CaptureManifest(
  cells: readonly InputLiveV85CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V85_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V85_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V85_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V85_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V85_SOURCE_IDS.some(
...
export const inputLiveV85CaptureManifestSha256 = (
  cells: readonly InputLiveV85CaptureCell[],
): string => {
  validateInputLiveV85CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV85CaptureProgram(
  writer: InputLiveV85WriterOwnership,
  cell: InputLiveV85CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V85_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V85_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V85_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV85CapturePayload(
  value: unknown,
  cell: InputLiveV85CaptureCell,
  rawResponseBytes: number,
): InputLiveV85CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV85CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V85_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V85_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV85CaptureResponses(
  manifest: readonly InputLiveV85CaptureCell[],
  responses: readonly InputLiveV85CapturePayload[],
): void {
  validateInputLiveV85CaptureManifest(manifest);
  if (
...
export interface InputLiveV85ObjectiveReport {
  artifactVersion: "input-live-v85-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV85Objective(
  manifest: readonly InputLiveV85CaptureCell[],
  responses: readonly InputLiveV85CapturePayload[],
  readReference: (cell: InputLiveV85CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV85CaptureCell) => Buffer,
): InputLiveV85ObjectiveReport {
  assertInputLiveV85CaptureResponses(manifest, responses);
...
export function buildInputLiveV85CleanupProgram(
  writer: InputLiveV85WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V85_NAMESPACE,
...
export function validateInputLiveV85CleanupPayload(
  value: unknown,
  writer: InputLiveV85WriterOwnership,
): InputLiveV85CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV85CleanupPayload;
...
export const INPUT_LIVE_V85_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV85WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
