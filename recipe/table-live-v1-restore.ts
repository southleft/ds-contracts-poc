...
export const TABLE_LIVE_V1_MEASURE_HUG_MARKER = "TABLE-TEXT-HUG-MEASURE";
export const TABLE_LIVE_V1_RESTORE_CONTENT_ROLES = ["table/cell/label"] as const;
export const TABLE_LIVE_V1_RESTORE_CELL_SET_ROLE = "table/cell-set";
...
export interface TableLiveV1RestoreWriter {
  pageId: string;
  runIdentity: string;
  setIds: readonly string[];
}

export interface TableLiveV1RestorePayload {
  pageId: string;
  setIds: string[];
  restoredCount: 8;
  fixedBefore: number;
  ...
  marker: typeof TABLE_LIVE_V1_MEASURE_HUG_MARKER;
}

export function buildTableLiveV1RestoreProgram(
  writer: TableLiveV1RestoreWriter,
): string {
  if (writer.setIds.length !== 6)
    throw new TypeError("Table live v1 restore requires six owned sets");
  if (
...
const ROLES=new Set(${JSON.stringify([...TABLE_LIVE_V1_RESTORE_CONTENT_ROLES])});
const CELL_SET=${JSON.stringify(TABLE_LIVE_V1_RESTORE_CELL_SET_ROLE)};
const MARKER=${JSON.stringify(TABLE_LIVE_V1_MEASURE_HUG_MARKER)};
...
if(!page||page.type!=="PAGE")throw new Error("TABLE-V1-RESTORE-PAGE");
...
if(page.getSharedPluginData(NS,"pageOwner")!=="recipe/table/"+${JSON.stringify(writer.runIdentity)})throw new Error("TABLE-V1-RESTORE-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>SET_IDS.includes(node.id));
if(sets.length!==6)throw new Error("TABLE-V1-RESTORE-ROOTS:"+sets.length);
const texts=[];
...
for(const entry of texts){
  ...
  if(after==="FILL")throw new Error("TABLE-V1-RESTORE-INVENTED-FILL:"+entry.role);
...
if(restored.length!==8)throw new Error("TABLE-V1-RESTORE-COUNT:"+restored.length);
if(restored.some(entry=>entry.after!=="HUG"))throw new Error("TABLE-V1-RESTORE-NOT-HUG");
...
export function validateTableLiveV1RestorePayload(
  payload: unknown,
  writer: TableLiveV1RestoreWriter,
): TableLiveV1RestorePayload {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("Table live v1 restore payload must be an object");
  ...
  if (
    ...
    value.marker !== TABLE_LIVE_V1_MEASURE_HUG_MARKER
  )
    throw new TypeError("Table live v1 restore did not re-assert content HUG");
  return value as TableLiveV1RestorePayload;
}
