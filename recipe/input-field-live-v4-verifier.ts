...
export const INPUT_LIVE_V4_VERIFIER_VERSION = "input-live-v4-verifier-v1";

export function buildInputLiveV4RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV4RawNode
  extends Omit<
    SceneNodeSnapshot,
  ...
  children: InputLiveV4RawNode[];
}

export interface InputLiveV4NormalizedScene {
...
export function normalizeInputLiveV4Scene(
  root: InputLiveV4RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV4NormalizedScene {
  const canonicalBindings: InputLiveV4NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: InputLiveV4RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV4Scene<Instance>(
  normalized: InputLiveV4NormalizedScene,
...
export function compileInputLiveV4ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV4PreCaptureGates(payload: unknown): void {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("v4 pre-capture gate payload absent");
  ...
  if (failed.length > 0)
    throw new TypeError(`captures forbidden before gates pass: ${failed.join(",")}`);
}
