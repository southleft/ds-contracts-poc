...
import {
  ...
  CALENDAR_LIVE_V4_HOST_PHASES,
  CALENDAR_LIVE_V4_REMOTE_REQUESTS,
  CALENDAR_LIVE_V4_SOURCE_ROOTS,
  CALENDAR_LIVE_V4_VARIANT_COUNT,
  buildCalendarLiveV4CaptureProgram,
  buildCalendarLiveV4CleanupProgram,
  buildCalendarLiveV4ExtractProgram,
  buildCalendarLiveV4ProbeProgram,
  buildCalendarLiveV4RestoreProgram,
  calendarLiveV1CaptureManifestSha256,
  type CalendarLiveV4CaptureCell,
  type CalendarLiveV4SourceIdentity,
  type CalendarLiveV4WriterOwnership,
} from "./calendar-live-v4-contract.js";
import {
  CALENDAR_LIVE_V4_DYNAMIC_TOOL,
  CALENDAR_LIVE_V4_TARGET,
  calendarLiveV1RequestSequence,
} from "./calendar-live-v4-broker.js";
import {
...
export const CALENDAR_LIVE_V4_EVIDENCE_ROOT =
  "recipe/evidence/calendar-live-pivot-v4";
export const CALENDAR_LIVE_V4_PROTOCOL_PATH = `${CALENDAR_LIVE_V4_EVIDENCE_ROOT}/protocol.json`;
export const CALENDAR_LIVE_V4_PLAN_PATH = `${CALENDAR_LIVE_V4_EVIDENCE_ROOT}/proof-plan.json`;
export const CALENDAR_LIVE_V4_CAPTURE_MANIFEST_PATH = `${CALENDAR_LIVE_V4_EVIDENCE_ROOT}/capture-manifest.json`;
export const CALENDAR_LIVE_V4_REQUEST_MANIFEST_PATH = `${CALENDAR_LIVE_V4_EVIDENCE_ROOT}/request-manifest.json`;
export const CALENDAR_LIVE_V4_INDEX_PATH = `${CALENDAR_LIVE_V4_EVIDENCE_ROOT}/antecedent-index.json`;
export const CALENDAR_LIVE_V4_AUTHORIZATION_TEMPLATE_PATH = `${CALENDAR_LIVE_V4_EVIDENCE_ROOT}/authorization-template.json`;
export const CALENDAR_LIVE_V4_AUTHORIZATION_PATH = `${CALENDAR_LIVE_V4_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface CalendarLiveV4ProofPlan {
  artifactVersion: "calendar-live-v4-proof-plan-v1";
...
const captureCells = (
...
): CalendarLiveV4CaptureCell[] => {
  const cells: CalendarLiveV4CaptureCell[] = [];
  ...
  if (cells.length !== CALENDAR_LIVE_V4_CAPTURE_COUNT)
    throw new Error(
      `Calendar live v4 capture cells ${cells.length}, expected ${CALENDAR_LIVE_V4_CAPTURE_COUNT}`,
    );
...
const protocol = (expectedFacts: number) => ({
  artifactVersion: "calendar-live-v4-external-operator-protocol-v1",
  protocolId: "calendar-live-v4",
  status:
  ...
  denominator: {
    sourceRoots: CALENDAR_LIVE_V4_SOURCE_ROOTS,
    sets: 3,
    variants: CALENDAR_LIVE_V4_VARIANT_COUNT,
    captureCells: CALENDAR_LIVE_V4_CAPTURE_COUNT,
    remoteRequests: CALENDAR_LIVE_V4_REMOTE_REQUESTS,
    hostPhases: CALENDAR_LIVE_V4_HOST_PHASES,
...
export async function buildCalendarLiveV4Proof(
  check = process.argv.includes("--check"),
): Promise<CalendarLiveV4ProofPlan> {
  ...
  if (
    ...
    !writer.runIdentity.endsWith("-calendar-v4") ||
    ...
    throw new Error(
      "Calendar live v4 writer reused Input, Combobox, or Table v1–v24 identity",
    );
  const captures = captureCells(sources);
  const writerProgram = Buffer.from(writer.code);
  const writerOwnershipBlueprint: CalendarLiveV4WriterOwnership = {
  ...
  const identities: CalendarLiveV4SourceIdentity[] = sources.map((source) => ({
    source: source.source,
    adapterIdentity: source.adapterIdentity,
    recipeHash: source.recipeHash,
    envelopeHash: source.envelope.integrity.canonicalHash,
  ...
  const restoreBlueprint = Buffer.from(
    buildCalendarLiveV4RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildCalendarLiveV4ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildCalendarLiveV4ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildCalendarLiveV4CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );
  const cleanupBlueprint = Buffer.from(
    buildCalendarLiveV4CleanupProgram(writerOwnershipBlueprint),
  );
  ...
  const sourceMetadata = sources.map((source) => {
    ...
    const tablePath = `${CALENDAR_LIVE_V4_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-calendar.json.gz`;
    const rowPath = `${CALENDAR_LIVE_V4_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-week.json.gz`;
    const cellPath = `${CALENDAR_LIVE_V4_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-day.json.gz`;
  ...
  antecedentOutputs.set(
    CALENDAR_LIVE_V4_PROTOCOL_PATH,
    jsonBytes(protocol(expectedFacts)),
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V4_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V4_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V4_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V4_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V4_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V4_EVIDENCE_ROOT}/programs/cleanup-blueprint.js`,
    cleanupBlueprint,
  );

  const captureManifest = {
    artifactVersion: "calendar-live-v4-capture-manifest-v1",
  ...
  antecedentOutputs.set(
    CALENDAR_LIVE_V4_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: CalendarLiveV4ProofPlan = {
    artifactVersion: "calendar-live-v4-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: CALENDAR_LIVE_V4_TARGET,
    namespace: CALENDAR_FIGMA_NAMESPACE,
    writer: {
      programPath: `${CALENDAR_LIVE_V4_EVIDENCE_ROOT}/programs/writer.txt`,
    ...
    captureManifest: {
      path: CALENDAR_LIVE_V4_CAPTURE_MANIFEST_PATH,
      cells: 8,
      requests: 8,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(CALENDAR_LIVE_V4_PLAN_PATH, jsonBytes(proofPlan));

  const requests = {
    artifactVersion: "calendar-live-v4-request-manifest-v1",
    expectedDynamicTool: CALENDAR_LIVE_V4_DYNAMIC_TOOL,
    target: CALENDAR_LIVE_V4_TARGET,
    signedAtRuntime: true,
    requestCount: CALENDAR_LIVE_V4_REMOTE_REQUESTS,
    requests: [
      {
        requestId: "writer",
  ...
  if (requests.requestCount !== CALENDAR_LIVE_V4_REMOTE_REQUESTS)
    throw new Error("Calendar live v4 remote request denominator drifted");
  antecedentOutputs.set(
    CALENDAR_LIVE_V4_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const lifecycleExcludedPaths = [
    CALENDAR_LIVE_V4_AUTHORIZATION_TEMPLATE_PATH,
    CALENDAR_LIVE_V4_AUTHORIZATION_PATH,
    "recipe/calendar-live-v4-authorization.ts",
    "recipe/calendar-live-v4-authorization.test.ts",
    "recipe/calendar-live-v4-preflight.ts",
    "recipe/calendar-live-v4-authorized.ts",
    "recipe/write-calendar-live-v4-authorization.ts",
    "recipe/create-calendar-live-v4-security-attestation.ts",
    "recipe/evidence/calendar-live-pivot-v4/operator-security-attestation-template.json",
    "recipe/evidence/calendar-live-pivot-v4-status.json",
    "recipe/evidence/status-index.json",
  ];
  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/calendar-live-v4-broker.ts",
    "recipe/calendar-live-v4-contract.ts",
    "recipe/calendar-live-v4-restore.ts",
    "recipe/calendar-live-v4-cleanup.ts",
    "recipe/calendar-live-v4-verifier.ts",
    "recipe/calendar-live-v4-fixed-point.ts",
    "recipe/run-calendar-live-v4.ts",
    "recipe/build-calendar-live-proof-v4.ts",
    "recipe/calendar-live-v4-lifecycle-simulation.ts",
    ...
    "recipe/normalize.ts",
  ].filter((artifactPath) => artifactPath !== CALENDAR_LIVE_V4_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "calendar-live-v4-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
  ...
  antecedentOutputs.set(CALENDAR_LIVE_V4_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "calendar-live-v4-authorization-template-v1",
    authorizationId: "calendar-live-v4",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: CALENDAR_LIVE_V4_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
    ...
    denominator: {
      remoteRequests: CALENDAR_LIVE_V4_REMOTE_REQUESTS,
      hostPhases: CALENDAR_LIVE_V4_HOST_PHASES,
      captures: CALENDAR_LIVE_V4_CAPTURE_COUNT,
      sourceRoots: CALENDAR_LIVE_V4_SOURCE_ROOTS,
      expectedFacts,
    },
    execution: {
  ...
  const attestationTemplate = jsonBytes({
    artifactVersion: "calendar-live-v4-operator-security-attestation-template-v1",
    status: "template only; complete privately after PREPARE+AUTHORIZE",
    tokenValuesForbidden: true,
    privateDirectory: "private/calendar-live-v4-security-attestation.json",
  });
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Calendar live v4 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(CALENDAR_LIVE_V4_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(CALENDAR_LIVE_V4_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
    writeFileSync(
      `${CALENDAR_LIVE_V4_EVIDENCE_ROOT}/operator-security-attestation-template.json`,
      attestationTemplate,
    );
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(CALENDAR_LIVE_V4_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(CALENDAR_LIVE_V4_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Calendar live v4 authorization template drift");
  }
  if (
    writer.namespace !== CALENDAR_FIGMA_NAMESPACE ||
    writer.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    ...
    throw new Error("Calendar live v4 identity or cleanup invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildCalendarLiveV4Proof(), null, 2)}\n`,
  );
