...
export const CALENDAR_LIVE_V31_MEASURE_HUG_MARKER = "CALENDAR-TEXT-HUG-MEASURE";
export const CALENDAR_LIVE_V31_RESTORE_CONTENT_ROLES = [
  "calendar/day/label",
] as const;
export const CALENDAR_LIVE_V31_RESTORE_DAY_SET_ROLE = "calendar/day-set";
...
export interface CalendarLiveV31RestoreWriter {
  pageId: string;
  runIdentity: string;
  setIds: readonly string[];
}

export interface CalendarLiveV31RestorePayload {
  pageId: string;
  setIds: string[];
  restoredCount: 4;
  fixedBefore: number;
  ...
  marker: typeof CALENDAR_LIVE_V31_MEASURE_HUG_MARKER;
}

export function buildCalendarLiveV31RestoreProgram(
  writer: CalendarLiveV31RestoreWriter,
): string {
  if (writer.setIds.length !== 3)
    throw new TypeError("Calendar live v31 restore requires three owned sets");
  if (
...
const ROLES=new Set(${JSON.stringify([...CALENDAR_LIVE_V31_RESTORE_CONTENT_ROLES])});
const DAY_SET=${JSON.stringify(CALENDAR_LIVE_V31_RESTORE_DAY_SET_ROLE)};
const MARKER=${JSON.stringify(CALENDAR_LIVE_V31_MEASURE_HUG_MARKER)};
...
export function validateCalendarLiveV31RestorePayload(
  payload: unknown,
  writer: CalendarLiveV31RestoreWriter,
): CalendarLiveV31RestorePayload {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("Calendar live v31 restore payload must be an object");
  ...
  if (
    ...
    value.marker !== CALENDAR_LIVE_V31_MEASURE_HUG_MARKER
  )
    throw new TypeError("Calendar live v31 restore did not re-assert content HUG");
  return value as CalendarLiveV31RestorePayload;
}
