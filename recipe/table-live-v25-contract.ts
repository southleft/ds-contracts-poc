...
import { buildTableLiveV25CleanupRuntime } from "./table-live-v25-cleanup.js";
import { type TableLiveV25FixedFixedPoint } from "./table-live-v25-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV25RestoreProgram,
  validateTableLiveV25RestorePayload,
  type TableLiveV25RestorePayload,
} from "./table-live-v25-restore.js";
import {
  buildTableLiveV25RawPropertyRuntime,
  normalizeTableLiveV25Scene,
  type TableLiveV25RawNode,
} from "./table-live-v25-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v25.js";
import {
...
export const TABLE_LIVE_V25_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V25_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V25_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V25_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V25_VARIANT_COUNT = 20;
export const TABLE_LIVE_V25_SET_COUNT = 6;
export const TABLE_LIVE_V25_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V25_HOST_PHASES = 3;
export const TABLE_LIVE_V25_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V25_RESTORE_COUNT = 8;
export const TABLE_LIVE_V25_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V25_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV25SourceId = (typeof TABLE_LIVE_V25_SOURCE_IDS)[number];
export type TableLiveV25Kind = "table" | "row" | "cell";

export interface TableLiveV25SourceIdentity {
  source: TableLiveV25SourceId;
...
export interface TableLiveV25WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV25ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV25SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV25RawNode;
    rowScene: TableLiveV25RawNode;
    cellScene: TableLiveV25RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV25RootProof {
  source: TableLiveV25SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV25FixedFixedPoint;
}

export interface TableLiveV25ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV25SourceId;
    adapterIdentity: string;
...
export interface TableLiveV25CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV25SourceId;
  adapterIdentity: string;
...
export interface TableLiveV25CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV25SourceId;
  frameWidth: number;
...
export interface TableLiveV25CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v25 ${label} must be an object`);
...
export function validateTableLiveV25WriterPayload(
  value: unknown,
): TableLiveV25WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V25_NAMESPACE ||
    ...
    throw new TypeError("Table live v25 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v25 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v25 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV25SourceIdentity,
  kind: TableLiveV25Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV25ExtractProgram(
  writer: TableLiveV25WriterOwnership,
  sources: readonly TableLiveV25SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v25 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V25_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V25_NAMESPACE)}
${buildTableLiveV25RawPropertyRuntime()}
...
export function validateTableLiveV25ExtractPayload(
  value: unknown,
  writer: TableLiveV25WriterOwnership,
): TableLiveV25ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v25 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V25_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v25 extract schema/two-root mismatch");
  return payload as TableLiveV25ExtractPayload;
}

export function proveTableLiveV25Roots<Instance>(
  extract: TableLiveV25ExtractPayload,
  sources: readonly (TableLiveV25SourceIdentity & {
...
): TableLiveV25RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v25 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v25 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV25Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV25Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV25Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV25FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v25 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v25 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v25 compile lost owned sets");
...
export function assertTableLiveV25RootProofs(
  proofs: readonly TableLiveV25RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v25 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v25 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV25ProbeProgram(
  writer: TableLiveV25WriterOwnership,
  sources: readonly TableLiveV25SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V25_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV25ProbePayload(
  value: unknown,
  writer: TableLiveV25WriterOwnership,
): TableLiveV25ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV25ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V25_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v25 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV25CaptureManifest(
  cells: readonly TableLiveV25CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V25_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V25_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V25_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V25_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v25 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV25CaptureCell[],
): string => {
  validateTableLiveV25CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV25CaptureProgram(
  writer: TableLiveV25WriterOwnership,
  cell: TableLiveV25CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V25_CAPTURE_COUNT
  )
    throw new TypeError("Table live v25 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v25 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V25_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV25CapturePayload(
  value: unknown,
  cell: TableLiveV25CaptureCell,
  rawResponseBytes: number,
): TableLiveV25CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV25CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V25_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V25_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v25 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV25CaptureResponses(
  manifest: readonly TableLiveV25CaptureCell[],
  responses: readonly TableLiveV25CapturePayload[],
): void {
  validateTableLiveV25CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v25 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV25ObjectiveReport {
  artifactVersion: "table-live-v25-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV25Objective(
  manifest: readonly TableLiveV25CaptureCell[],
  responses: readonly TableLiveV25CapturePayload[],
): TableLiveV25ObjectiveReport {
  assertTableLiveV25CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v25-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV25CleanupProgram(
  writer: TableLiveV25WriterOwnership,
): string {
  const baseProgram = buildTableLiveV25CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V25_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v25 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV25CleanupPayload(
  payload: unknown,
  writer: TableLiveV25WriterOwnership,
): TableLiveV25CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV25CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v25 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V25_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV25WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
