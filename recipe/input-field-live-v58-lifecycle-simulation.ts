...
export interface InputLiveV58LifecycleSimulationReport {
  artifactVersion: "input-live-v58-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV58AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV58Lifecycle(): Promise<InputLiveV58LifecycleSimulationReport> {
  await buildInputLiveV58Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V58_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV58AntecedentIndex;
  const indexSha256 = inputLiveV58Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v58-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V58_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V58_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV58History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV58History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV58AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V58_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V58_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV58History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV58History(authorized, "pending");
    ...
    await buildInputLiveV58Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V58_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV58AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV58Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV58Lifecycle(), null, 2)}\n`,
  );
