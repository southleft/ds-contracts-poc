/**
 * Carried Combobox live v16 host verifier. Uses scene-readback-combobox-v16 so live
...
export const COMBOBOX_LIVE_V16_FIXED_VERIFIER_VERSION = "combobox-live-v16-fixed-point-v1";
export const COMBOBOX_LIVE_V16_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV16FixedThresholds {
...
export interface ComboboxLiveV16FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV16FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV16FixedSourceProbe {
...
export interface ComboboxLiveV16FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV16FixedFixedPoint;
}

export interface ComboboxLiveV16FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV16FixedHardGateInput {
  thresholds: ComboboxLiveV16FixedThresholds;
  materialRegression: ComboboxLiveV16FixedMaterialRegression;
  sourceProbes: ComboboxLiveV16FixedSourceProbe[];
  cells: ComboboxLiveV16FixedCellValidation[];
  sceneProofs: ComboboxLiveV16FixedSceneProof[];
  visualRows: ComboboxLiveV16FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV16FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V16_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV16FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV16FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV16FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV16FixedHardGates(
  input: ComboboxLiveV16FixedHardGateInput,
): ComboboxLiveV16FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV16FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV16FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V16_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV16FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV16FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV16FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
