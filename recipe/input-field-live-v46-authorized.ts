...
import { INPUT_LIVE_V46_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v46.js";
import { verifyInputLiveV46Authorization } from "./input-field-live-v46-authorization.js";
import { runInputLiveV46Preflight } from "./input-field-live-v46-preflight.js";
import { InputLiveV46Orchestrator } from "./run-input-field-live-v46.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V46_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v46-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV46Authorization(root);
    runInputLiveV46Preflight(
      root,
      authorization,
    ...
    const orchestrator = InputLiveV46Orchestrator.initialize({
      root,
  ...
  const orchestrator = InputLiveV46Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
