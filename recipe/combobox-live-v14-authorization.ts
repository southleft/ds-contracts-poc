...
export type ComboboxLiveV14HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV14AuthorizationArtifact {
  artifactVersion: "combobox-live-v14-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V14_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV14AntecedentIndex {
...
export interface ComboboxLiveV14HistoryState {
  index?: ComboboxLiveV14AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV14AuthorizationArtifact;
...
export interface ComboboxLiveV14AuthorizationProof extends ComboboxLiveV14TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V14_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V14_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V14_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V14_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v14-authorization") ||
  artifactPath.includes("combobox-live-v14-preflight") ||
  artifactPath.includes("combobox-live-v14-authorized") ||
  artifactPath.includes("create-combobox-live-v14-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v14-status") ||
...
export function validateComboboxLiveV14AntecedentIndex(
  index: ComboboxLiveV14AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v14-antecedent-index-v1" ||
...
export function validateComboboxLiveV14UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV14History\(\s*["']/.test(source) ||
    /readComboboxLiveV14HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV14AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV14AuthorizationArtifact {
  ...
  ) as ComboboxLiveV14AntecedentIndex;
  const indexFailures = validateComboboxLiveV14AntecedentIndex(index);
...
export function validateComboboxLiveV14AuthorizationArtifact(
  artifact: ComboboxLiveV14AuthorizationArtifact | undefined,
  index: ComboboxLiveV14AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v14-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v14" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V14_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V14_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V14_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V14_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V14_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V14_SOURCE_ROOTS ||
  ...
  if (
    ...
    failures.push(
      "v14 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
...
export function validateComboboxLiveV14History(
  state: ComboboxLiveV14HistoryState,
  expected: ComboboxLiveV14HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV14AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV14AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV14HistoryState(
  root = process.cwd(),
): ComboboxLiveV14HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V14_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV14AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V14_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V14_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V14_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV14History(
  expected: ComboboxLiveV14HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV14HistoryState {
  const state = readComboboxLiveV14HistoryState(root);
  const failures = validateComboboxLiveV14History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v14 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV14Authorization(
  root = process.cwd(),
): ComboboxLiveV14AuthorizationProof {
  const state = verifyComboboxLiveV14History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V14_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v14/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v14.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V14_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V14_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V14_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV14HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV14History(expected);
  process.stdout.write(
    `Combobox live v14 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
