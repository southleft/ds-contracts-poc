...
import { buildTableLiveV14CleanupRuntime } from "./table-live-v14-cleanup.js";
import { type TableLiveV14FixedFixedPoint } from "./table-live-v14-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV14RestoreProgram,
  validateTableLiveV14RestorePayload,
  type TableLiveV14RestorePayload,
} from "./table-live-v14-restore.js";
import {
  buildTableLiveV14RawPropertyRuntime,
  normalizeTableLiveV14Scene,
  type TableLiveV14RawNode,
} from "./table-live-v14-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v14.js";
import {
...
export const TABLE_LIVE_V14_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V14_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V14_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V14_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V14_VARIANT_COUNT = 20;
export const TABLE_LIVE_V14_SET_COUNT = 6;
export const TABLE_LIVE_V14_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V14_HOST_PHASES = 3;
export const TABLE_LIVE_V14_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V14_RESTORE_COUNT = 8;
export const TABLE_LIVE_V14_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V14_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV14SourceId = (typeof TABLE_LIVE_V14_SOURCE_IDS)[number];
export type TableLiveV14Kind = "table" | "row" | "cell";

export interface TableLiveV14SourceIdentity {
  source: TableLiveV14SourceId;
...
export interface TableLiveV14WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV14ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV14SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV14RawNode;
    rowScene: TableLiveV14RawNode;
    cellScene: TableLiveV14RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV14RootProof {
  source: TableLiveV14SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV14FixedFixedPoint;
}

export interface TableLiveV14ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV14SourceId;
    adapterIdentity: string;
...
export interface TableLiveV14CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV14SourceId;
  adapterIdentity: string;
...
export interface TableLiveV14CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV14SourceId;
  frameWidth: number;
...
export interface TableLiveV14CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v14 ${label} must be an object`);
...
export function validateTableLiveV14WriterPayload(
  value: unknown,
): TableLiveV14WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V14_NAMESPACE ||
    ...
    throw new TypeError("Table live v14 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v14 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v14 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV14SourceIdentity,
  kind: TableLiveV14Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV14ExtractProgram(
  writer: TableLiveV14WriterOwnership,
  sources: readonly TableLiveV14SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v14 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V14_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V14_NAMESPACE)}
${buildTableLiveV14RawPropertyRuntime()}
...
export function validateTableLiveV14ExtractPayload(
  value: unknown,
  writer: TableLiveV14WriterOwnership,
): TableLiveV14ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v14 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V14_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v14 extract schema/two-root mismatch");
  return payload as TableLiveV14ExtractPayload;
}

export function proveTableLiveV14Roots<Instance>(
  extract: TableLiveV14ExtractPayload,
  sources: readonly (TableLiveV14SourceIdentity & {
...
): TableLiveV14RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v14 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v14 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV14Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV14Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV14Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV14FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v14 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v14 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v14 compile lost owned sets");
...
export function assertTableLiveV14RootProofs(
  proofs: readonly TableLiveV14RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v14 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v14 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV14ProbeProgram(
  writer: TableLiveV14WriterOwnership,
  sources: readonly TableLiveV14SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V14_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV14ProbePayload(
  value: unknown,
  writer: TableLiveV14WriterOwnership,
): TableLiveV14ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV14ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V14_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V14_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Table live v14 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV14CaptureManifest(
  cells: readonly TableLiveV14CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V14_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V14_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V14_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V14_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v14 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV14CaptureCell[],
): string => {
  validateTableLiveV14CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV14CaptureProgram(
  writer: TableLiveV14WriterOwnership,
  cell: TableLiveV14CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V14_CAPTURE_COUNT
  )
    throw new TypeError("Table live v14 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v14 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V14_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV14CapturePayload(
  value: unknown,
  cell: TableLiveV14CaptureCell,
  rawResponseBytes: number,
): TableLiveV14CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV14CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V14_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V14_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v14 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV14CaptureResponses(
  manifest: readonly TableLiveV14CaptureCell[],
  responses: readonly TableLiveV14CapturePayload[],
): void {
  validateTableLiveV14CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v14 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV14ObjectiveReport {
  artifactVersion: "table-live-v14-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV14Objective(
  manifest: readonly TableLiveV14CaptureCell[],
  responses: readonly TableLiveV14CapturePayload[],
): TableLiveV14ObjectiveReport {
  assertTableLiveV14CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v14-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV14CleanupProgram(
  writer: TableLiveV14WriterOwnership,
): string {
  const baseProgram = buildTableLiveV14CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V14_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v14 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV14CleanupPayload(
  payload: unknown,
  writer: TableLiveV14WriterOwnership,
): TableLiveV14CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV14CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v14 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V14_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV14WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
