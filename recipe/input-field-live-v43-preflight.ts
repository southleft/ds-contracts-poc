...
import {
  ...
  verifyInputLiveV43Authorization,
  type InputLiveV43AntecedentIndex,
  type InputLiveV43AuthorizationProof,
} from "./input-field-live-v43-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V43_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v43/operator-security-attestation-template.json";
export const INPUT_LIVE_V43_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v43-security-attestation.json";

export interface InputLiveV43SecurityAttestation {
...
export interface InputLiveV43PreflightReport {
  artifactVersion: "input-live-v43-preflight-v1";
...
export const inputLiveV43AttestationSha256 = (
  value: Omit<InputLiveV43SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV43SecurityAttestation(
  value: unknown,
  proof: InputLiveV43AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v43-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V43_TARGET) ||
...
export function validateInputLiveV43ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV43Authorization");
  const preflight = source.indexOf("runInputLiveV43Preflight");
  const initialize = source.indexOf("InputLiveV43Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV43Preflight(
  root: string,
  proof: InputLiveV43AuthorizationProof,
  attestationPath = INPUT_LIVE_V43_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV43PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V43_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V43_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V43_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV43AntecedentIndex;
  failures.push(...validateInputLiveV43AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V43_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V43_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v43-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV43ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV43SecurityAttestation;
  return {
    artifactVersion: "input-live-v43-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V43_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV43Preflight(
    process.cwd(),
    verifyInputLiveV43Authorization(),
    attestationPath,
