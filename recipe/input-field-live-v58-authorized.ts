...
import { INPUT_LIVE_V58_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v58.js";
import { verifyInputLiveV58Authorization } from "./input-field-live-v58-authorization.js";
import { runInputLiveV58Preflight } from "./input-field-live-v58-preflight.js";
import { InputLiveV58Orchestrator } from "./run-input-field-live-v58.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V58_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v58-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV58Authorization(root);
    runInputLiveV58Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV58Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
