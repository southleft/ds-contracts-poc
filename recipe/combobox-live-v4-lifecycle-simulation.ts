...
export interface ComboboxLiveV4LifecycleSimulationReport {
  artifactVersion: "combobox-live-v4-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV4AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV4Lifecycle(): Promise<ComboboxLiveV4LifecycleSimulationReport> {
  await buildComboboxLiveV4Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V4_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV4AntecedentIndex;
  const indexSha256 = comboboxLiveV4Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v4-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V4_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V4_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV4History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV4History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV4AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V4_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V4_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV4History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV4History(authorized, "pending");
    ...
    await buildComboboxLiveV4Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V4_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV4AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: comboboxLiveV4Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV4Lifecycle(), null, 2)}\n`,
  );
