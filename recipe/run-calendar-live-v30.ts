...
import {
  acceptCalendarLiveV30Response,
  createCalendarLiveV30Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV30Request,
  persistCalendarLiveV30RawResponse,
  persistCalendarLiveV30TechnicalGates,
  readCalendarLiveV30AcceptedRawPayload,
  type CalendarLiveV30Request,
  type CalendarLiveV30TransactionAuthorization,
} from "./calendar-live-v30-broker.js";
import {
  CALENDAR_LIVE_V30_CAPTURE_COUNT,
  assertCalendarLiveV30CaptureResponses,
  ...
  evaluateCalendarLiveV30Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV30Roots,
  validateCalendarLiveV30CaptureManifest,
  validateCalendarLiveV30CapturePayload,
  validateCalendarLiveV30CleanupPayload,
  validateCalendarLiveV30ExtractPayload,
  validateCalendarLiveV30ProbePayload,
  validateCalendarLiveV30WriterPayload,
  type CalendarLiveV30CaptureCell,
  type CalendarLiveV30CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V30_PLAN_PATH,
  type CalendarLiveV30ProofPlan,
} from "./build-calendar-live-proof-v30.js";
import {
...
export const CALENDAR_LIVE_V30_RUNNER_VERSION = "calendar-live-v30-runner-v1";

interface CalendarLiveV30JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV30OrchestratorState {
  artifactVersion: "calendar-live-v30-orchestrator-state-v1";
  ...
  journal: CalendarLiveV30JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV30ProofPlan;
  planSha256: string;
  captures: CalendarLiveV30CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV30SourceIdentity & {
...
export interface CalendarLiveV30NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV30OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v30 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V30_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V30_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV30ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV30CaptureCell[];
  validateCalendarLiveV30CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v30 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v30 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV30WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV30RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV30Request =>
  readJson<CalendarLiveV30Request>(file);

export class CalendarLiveV30Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV30Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV30OrchestratorState = {
      artifactVersion: "calendar-live-v30-orchestrator-state-v1",
    ...
    return new CalendarLiveV30Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV30OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV30Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV30NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV30NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v30 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV30NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v30 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV30RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV30Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV30CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV30AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV30WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV30Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV30ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV30Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV30Response(this.#directory, "writer", {
        validate: validateCalendarLiveV30WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV30Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV30RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV30Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV30ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV30Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV30RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v30-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV30Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV30ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV30Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV30ProbePayload(payload, ownership),
      });
      persistCalendarLiveV30TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v30 capture cell missing");
      const accepted = acceptCalendarLiveV30Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV30CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V30_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV30CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV30Objective(
    ...
    throw new Error(`Calendar live v30 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV30Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV30CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV30Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV30RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV30Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV30CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV30CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV30CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV30Authorization = (
  privateKey: KeyObject,
): CalendarLiveV30TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV30SimulationKey = (): KeyObject =>
