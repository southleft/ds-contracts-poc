...
import {
  ...
  verifyInputLiveV42Authorization,
  type InputLiveV42AntecedentIndex,
  type InputLiveV42AuthorizationProof,
} from "./input-field-live-v42-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V42_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v42/operator-security-attestation-template.json";
export const INPUT_LIVE_V42_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v42-security-attestation.json";

export interface InputLiveV42SecurityAttestation {
...
export interface InputLiveV42PreflightReport {
  artifactVersion: "input-live-v42-preflight-v1";
...
export const inputLiveV42AttestationSha256 = (
  value: Omit<InputLiveV42SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV42SecurityAttestation(
  value: unknown,
  proof: InputLiveV42AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v42-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V42_TARGET) ||
...
export function validateInputLiveV42ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV42Authorization");
  const preflight = source.indexOf("runInputLiveV42Preflight");
  const initialize = source.indexOf("InputLiveV42Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV42Preflight(
  root: string,
  proof: InputLiveV42AuthorizationProof,
  attestationPath = INPUT_LIVE_V42_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV42PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V42_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V42_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V42_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV42AntecedentIndex;
  failures.push(...validateInputLiveV42AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V42_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V42_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v42-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV42ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV42SecurityAttestation;
  return {
    artifactVersion: "input-live-v42-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V42_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV42Preflight(
    process.cwd(),
    verifyInputLiveV42Authorization(),
    attestationPath,
