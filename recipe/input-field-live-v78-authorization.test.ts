...
import { INPUT_LIVE_V78_INDEX_PATH } from "./build-input-field-live-proof-v78.js";
import {
  INPUT_LIVE_V78_DYNAMIC_TOOL,
  INPUT_LIVE_V78_TARGET,
} from "./input-field-live-v78-broker.js";
import {
  buildInputLiveV78AuthorizationArtifact,
  validateInputLiveV78AntecedentIndex,
  validateInputLiveV78AuthorizationArtifact,
  validateInputLiveV78History,
  validateInputLiveV78UnitTestSource,
  type InputLiveV78AntecedentIndex,
  type InputLiveV78AuthorizationArtifact,
  type InputLiveV78AuthorizationProof,
  type InputLiveV78HistoryState,
} from "./input-field-live-v78-authorization.js";
import {
  inputLiveV78AttestationSha256,
  validateInputLiveV78ControlFlowSource,
  validateInputLiveV78SecurityAttestation,
  type InputLiveV78SecurityAttestation,
} from "./input-field-live-v78-preflight.js";
import { inputLiveV78Sha256 } from "./input-field-live-v78-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V78_INDEX_PATH);
const index = (): InputLiveV78AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV78AntecedentIndex;

const artifact = (): InputLiveV78AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV78AuthorizationArtifact({
...
const pendingState = (): InputLiveV78HistoryState => ({
  index: index(),
  indexSha256: inputLiveV78Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV78HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV78AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV78Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V78_TARGET,
  expectedDynamicTool: INPUT_LIVE_V78_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v78/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV78AuthorizationArtifact,
): InputLiveV78SecurityAttestation => {
  const body: Omit<InputLiveV78SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v78-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V78_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV78AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV78History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV78History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV78History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV78History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV78History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV78UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV78UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV78AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV78Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV78AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV78AuthorizationArtifact(
        planted as InputLiveV78AuthorizationArtifact,
        index(),
        inputLiveV78Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV78SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV78AttestationSha256(
      body as Omit<InputLiveV78SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV78SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV78ControlFlowSource(source), []);
});
