...
export interface InputLiveV22LifecycleSimulationReport {
  artifactVersion: "input-live-v22-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV22AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV22Lifecycle(): Promise<InputLiveV22LifecycleSimulationReport> {
  await buildInputLiveV22Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V22_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV22AntecedentIndex;
  const indexSha256 = inputLiveV22Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v22-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V22_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V22_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV22History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV22History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV22AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V22_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V22_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV22History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV22History(authorized, "pending");
    ...
    await buildInputLiveV22Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V22_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV22AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV22Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV22Lifecycle(), null, 2)}\n`,
  );
