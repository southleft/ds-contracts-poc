...
import {
  assertTableLiveV22RootProofs,
  TABLE_LIVE_V22_CAPTURE_COUNT,
  TABLE_LIVE_V22_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  TABLE_LIVE_V22_RESPONSE_CONTRACTS,
  type TableLiveV22RootProof,
} from "./table-live-v22-contract.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V22_BROKER_VERSION = "table-live-v22-broker-v1";
export const TABLE_LIVE_V22_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const TABLE_LIVE_V22_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const TABLE_LIVE_V22_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const TABLE_LIVE_V22_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const TABLE_LIVE_V22_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const TABLE_LIVE_V22_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type TableLiveV22RemotePhase =
  (typeof TABLE_LIVE_V22_REMOTE_PHASES)[number];
export type TableLiveV22AuthorizationMode = "live" | "simulated";

export interface TableLiveV22CommitPins {
...
export interface TableLiveV22TransactionAuthorization extends TableLiveV22CommitPins {
  mode: TableLiveV22AuthorizationMode;
...
export interface TableLiveV22Manifest {
  artifactVersion: "table-live-v22-broker-manifest-v2";
  brokerVersion: typeof TABLE_LIVE_V22_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof TABLE_LIVE_V22_DYNAMIC_TOOL;
  target: typeof TABLE_LIVE_V22_TARGET;
  authorization: TableLiveV22TransactionAuthorization;
...
export interface TableLiveV22ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface TableLiveV22RequestBody {
  artifactVersion: "table-live-v22-broker-request-v2";
  brokerVersion: typeof TABLE_LIVE_V22_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: TableLiveV22RemotePhase;
  sequence: number;
  ...
  commitPins: TableLiveV22CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof TABLE_LIVE_V22_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: TableLiveV22ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface TableLiveV22Request extends TableLiveV22RequestBody {
...
export interface TableLiveV22Receipt {
  artifactVersion: "table-live-v22-broker-receipt-v2";
  brokerVersion: typeof TABLE_LIVE_V22_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: TableLiveV22RemotePhase;
...
export interface TableLiveV22AcceptedResponse<T = unknown> {
  request: TableLiveV22Request;
  receipt: TableLiveV22Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface TableLiveV22IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: TableLiveV22ResponseContract;
...
const equalTarget = (value: unknown): value is typeof TABLE_LIVE_V22_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === TABLE_LIVE_V22_TARGET.fileKey &&
  (value as BrokerTarget).fileName === TABLE_LIVE_V22_TARGET.fileName &&
  (value as BrokerTarget).editorType === TABLE_LIVE_V22_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== TABLE_LIVE_V22_TARGET.fileKey ||
    context.fileName !== TABLE_LIVE_V22_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === TABLE_LIVE_V22_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Table live v22 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= TABLE_LIVE_V22_CAPTURE_COUNT
  )
    throw new TypeError("Table live v22 capture index must be 0..19");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Table live v22 authorization pins are malformed");
};

const readManifest = (directory: string): TableLiveV22Manifest => {
  const manifest = json<TableLiveV22Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Table live v22 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "table-live-v22-broker-manifest-v2" ||
    manifest.brokerVersion !== TABLE_LIVE_V22_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== TABLE_LIVE_V22_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V22_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Table live v22 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: TableLiveV22Request,
): TableLiveV22RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: TableLiveV22RemotePhase,
  captureIndex?: number,
): TableLiveV22Request | undefined => {
  const file = tableLiveV2RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV22Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: TableLiveV22RemotePhase,
  captureIndex?: number,
): TableLiveV22Receipt | undefined => {
  const file = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV22Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: TableLiveV22Request | undefined;
  receipt: TableLiveV22Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: TableLiveV22RemotePhase,
): TableLiveV22ResponseContract => ({
  ...TABLE_LIVE_V22_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: TABLE_LIVE_V22_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "table-live-v22-plugin-response-v2",
  brokerVersion: TABLE_LIVE_V22_BROKER_VERSION,
  target: TABLE_LIVE_V22_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*TABLE_LIVE_V22_PROGRAM_START*/${program}/*TABLE_LIVE_V22_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*TABLE_LIVE_V22_PROGRAM_START*/";
  const endMarker = "/*TABLE_LIVE_V22_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Table live v22 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createTableLiveV22Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Table live v22 private signing key does not match authorized public-key identity",
    );
  const manifest: TableLiveV22Manifest = {
    artifactVersion: "table-live-v22-broker-manifest-v2",
    brokerVersion: TABLE_LIVE_V22_BROKER_VERSION,
    ...
    captureRequests: TABLE_LIVE_V22_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateTableLiveV22Request(
  directory: string,
  request: TableLiveV22Request,
): void {
  ...
  if (
    request.artifactVersion !== "table-live-v22-broker-request-v2" ||
    request.brokerVersion !== TABLE_LIVE_V22_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== TABLE_LIVE_V22_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Table live v22 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueTableLiveV22Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: TableLiveV22RemotePhase,
  program: string,
  options: TableLiveV22IssueOptions = {},
): TableLiveV22Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<TableLiveV22Request>(targetPath);
    validateTableLiveV22Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Table live v22 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(tableLiveV2ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Table live v22 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Table live v22 empty code envelope refused");
  if (phase === "capture") validateTableLiveV22TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Table live v22 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: TABLE_LIVE_V22_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? TABLE_LIVE_V22_SIGNED_WRITER_TIMEOUT_MS
        : TABLE_LIVE_V22_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: TableLiveV22RequestBody = {
    artifactVersion: "table-live-v22-broker-request-v2",
    brokerVersion: TABLE_LIVE_V22_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: TableLiveV22Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateTableLiveV22Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistTableLiveV22RawResponse(
  directory: string,
  phase: TableLiveV22RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Table live v22 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<TableLiveV22Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Table live v22 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = tableLiveV2ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Table live v22 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<TableLiveV22Receipt, "receiptSha256"> = {
    artifactVersion: "table-live-v22-broker-receipt-v2",
    brokerVersion: TABLE_LIVE_V22_BROKER_VERSION,
...
export function acceptTableLiveV22Response<T = unknown>(
  directory: string,
  phase: TableLiveV22RemotePhase,
  options: {
    captureIndex?: number;
...
): TableLiveV22AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Table live v22 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<TableLiveV22Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Table live v22 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "table-live-v22-plugin-response-v2" ||
    plugin?.brokerVersion !== TABLE_LIVE_V22_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Table live v22 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistTableLiveV22TechnicalGates(
  directory: string,
  rootProofs: readonly TableLiveV22RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertTableLiveV22RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Table live v22 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Table live v22 probe payload hash malformed");
  const gates = Object.fromEntries(
    TABLE_LIVE_V22_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "table-live-v22-host-technical-gates-v2",
    brokerVersion: TABLE_LIVE_V22_BROKER_VERSION,
...
export function validateTableLiveV22TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "table-live-v22-host-technical-gates-v2" ||
    ...
    TABLE_LIVE_V22_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Table live v22 technical gates artifact");
}

export function validateTableLiveV22Receipt(
  directory: string,
  phase: TableLiveV22RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateTableLiveV22Request(directory, request);
  const receipt = json<TableLiveV22Receipt>(
  ...
  if (
    receipt.artifactVersion !== "table-live-v22-broker-receipt-v2" ||
    receipt.brokerVersion !== TABLE_LIVE_V22_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Table live v22 accepted receipt: ${request.requestId}`,
    );
}

export function readTableLiveV22AcceptedRawPayload(
  directory: string,
  phase: TableLiveV22RemotePhase,
  captureIndex?: number,
): unknown {
  validateTableLiveV22Receipt(directory, phase, captureIndex);
...
export function createTableLiveV22FakeDynamicResponse(
  request: TableLiveV22Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? TABLE_LIVE_V22_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? TABLE_LIVE_V22_TARGET,
    ...overrides.extra,
  };
}

export function listTableLiveV22PersistedRequests(
  directory: string,
): TableLiveV22Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<TableLiveV22Request>(file)] : [];
    });
}
