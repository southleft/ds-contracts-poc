...
import { TABLE_LIVE_V36_AUTHORIZATION_PATH } from "./build-table-live-proof-v36.js";
import { verifyTableLiveV36Authorization } from "./table-live-v36-authorization.js";
import { runTableLiveV36Preflight } from "./table-live-v36-preflight.js";
import { TableLiveV36Orchestrator } from "./run-table-live-v36.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v36 requires exact authorization artifact ${TABLE_LIVE_V36_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v36-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV36Authorization(root);
    runTableLiveV36Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV36Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV36Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
