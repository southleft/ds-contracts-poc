...
import { CALENDAR_LIVE_V20_TARGET } from "./calendar-live-v20-broker.js";
import {
  CALENDAR_LIVE_V20_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV20Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v20-session";
...
const attestation: CalendarLiveV20SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV20SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v20 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V20_SECURITY_ATTESTATION_DEFAULT_PATH,
);
