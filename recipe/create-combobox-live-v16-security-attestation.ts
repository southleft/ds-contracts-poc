...
import { verifyComboboxLiveV16Authorization } from "./combobox-live-v16-authorization.js";
import { COMBOBOX_LIVE_V16_TARGET } from "./combobox-live-v16-broker.js";
import {
  COMBOBOX_LIVE_V16_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV16Authorization();
...
const attestation: ComboboxLiveV16SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV16SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v16 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V16_SECURITY_ATTESTATION_DEFAULT_PATH,
);
