...
export const TABLE_LIVE_V34_MEASURE_HUG_MARKER = "TABLE-TEXT-HUG-MEASURE";
export const TABLE_LIVE_V34_RESTORE_CONTENT_ROLES = ["table/cell/label"] as const;
export const TABLE_LIVE_V34_RESTORE_CELL_SET_ROLE = "table/cell-set";
...
export interface TableLiveV34RestoreWriter {
  pageId: string;
  runIdentity: string;
  setIds: readonly string[];
}

export interface TableLiveV34RestorePayload {
  pageId: string;
  setIds: string[];
  restoredCount: 8;
  fixedBefore: number;
  ...
  marker: typeof TABLE_LIVE_V34_MEASURE_HUG_MARKER;
}

export function buildTableLiveV34RestoreProgram(
  writer: TableLiveV34RestoreWriter,
): string {
  if (writer.setIds.length !== 6)
    throw new TypeError("Table live v34 restore requires six owned sets");
  if (
...
const ROLES=new Set(${JSON.stringify([...TABLE_LIVE_V34_RESTORE_CONTENT_ROLES])});
const CELL_SET=${JSON.stringify(TABLE_LIVE_V34_RESTORE_CELL_SET_ROLE)};
const MARKER=${JSON.stringify(TABLE_LIVE_V34_MEASURE_HUG_MARKER)};
...
export function validateTableLiveV34RestorePayload(
  payload: unknown,
  writer: TableLiveV34RestoreWriter,
): TableLiveV34RestorePayload {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("Table live v34 restore payload must be an object");
  ...
  if (
    ...
    value.marker !== TABLE_LIVE_V34_MEASURE_HUG_MARKER
  )
    throw new TypeError("Table live v34 restore did not re-assert content HUG");
  return value as TableLiveV34RestorePayload;
}
