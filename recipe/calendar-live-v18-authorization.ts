...
export type CalendarLiveV18HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV18AuthorizationArtifact {
  artifactVersion: "calendar-live-v18-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V18_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV18AntecedentIndex {
...
export interface CalendarLiveV18HistoryState {
  index?: CalendarLiveV18AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV18AuthorizationArtifact;
...
export interface CalendarLiveV18AuthorizationProof
  extends CalendarLiveV18TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V18_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V18_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V18_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V18_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v18-authorization") ||
  artifactPath.includes("calendar-live-v18-preflight") ||
  artifactPath.includes("calendar-live-v18-authorized") ||
  artifactPath.includes("create-calendar-live-v18-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v18-status") ||
...
export function validateCalendarLiveV18AntecedentIndex(
  index: CalendarLiveV18AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v18-antecedent-index-v1" ||
...
export function validateCalendarLiveV18UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV18History\(\s*["']/.test(source) ||
    /readCalendarLiveV18HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV18AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV18AuthorizationArtifact {
  ...
  ) as CalendarLiveV18AntecedentIndex;
  const indexFailures = validateCalendarLiveV18AntecedentIndex(index);
...
export function validateCalendarLiveV18AuthorizationArtifact(
  artifact: CalendarLiveV18AuthorizationArtifact | undefined,
  index: CalendarLiveV18AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v18-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v18" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V18_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V18_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V18_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V18_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V18_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V18_SOURCE_ROOTS ||
...
export function validateCalendarLiveV18History(
  state: CalendarLiveV18HistoryState,
  expected: CalendarLiveV18HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV18AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV18AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV18HistoryState(
  root = process.cwd(),
): CalendarLiveV18HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V18_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV18AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V18_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V18_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V18_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV18History(
  expected: CalendarLiveV18HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV18HistoryState {
  const state = readCalendarLiveV18HistoryState(root);
  const failures = validateCalendarLiveV18History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v18 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV18Authorization(
  root = process.cwd(),
): CalendarLiveV18AuthorizationProof {
  const state = verifyCalendarLiveV18History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V18_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v18/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v18.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V18_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V18_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V18_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV18HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV18History(expected);
  process.stdout.write(
    `Calendar live v18 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
