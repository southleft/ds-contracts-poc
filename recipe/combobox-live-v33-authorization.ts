...
export type ComboboxLiveV33HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV33AuthorizationArtifact {
  artifactVersion: "combobox-live-v33-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V33_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV33AntecedentIndex {
...
export interface ComboboxLiveV33HistoryState {
  index?: ComboboxLiveV33AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV33AuthorizationArtifact;
...
export interface ComboboxLiveV33AuthorizationProof extends ComboboxLiveV33TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V33_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V33_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V33_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V33_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v33-authorization") ||
  artifactPath.includes("combobox-live-v33-preflight") ||
  artifactPath.includes("combobox-live-v33-authorized") ||
  artifactPath.includes("create-combobox-live-v33-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v33-status") ||
...
export function validateComboboxLiveV33AntecedentIndex(
  index: ComboboxLiveV33AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v33-antecedent-index-v1" ||
...
export function validateComboboxLiveV33UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV33History\(\s*["']/.test(source) ||
    /readComboboxLiveV33HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV33AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV33AuthorizationArtifact {
  ...
  ) as ComboboxLiveV33AntecedentIndex;
  const indexFailures = validateComboboxLiveV33AntecedentIndex(index);
...
export function validateComboboxLiveV33AuthorizationArtifact(
  artifact: ComboboxLiveV33AuthorizationArtifact | undefined,
  index: ComboboxLiveV33AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v33-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v33" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V33_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V33_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V33_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V33_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V33_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V33_SOURCE_ROOTS ||
...
export function validateComboboxLiveV33History(
  state: ComboboxLiveV33HistoryState,
  expected: ComboboxLiveV33HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV33AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV33AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV33HistoryState(
  root = process.cwd(),
): ComboboxLiveV33HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V33_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV33AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V33_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V33_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V33_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV33History(
  expected: ComboboxLiveV33HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV33HistoryState {
  const state = readComboboxLiveV33HistoryState(root);
  const failures = validateComboboxLiveV33History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v33 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV33Authorization(
  root = process.cwd(),
): ComboboxLiveV33AuthorizationProof {
  const state = verifyComboboxLiveV33History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V33_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v33/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v33.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V33_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V33_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V33_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV33HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV33History(expected);
  process.stdout.write(
    `Combobox live v33 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
