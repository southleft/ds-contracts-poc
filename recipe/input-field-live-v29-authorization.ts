...
import {
  ...
  INPUT_LIVE_V29_INDEX_PATH,
} from "./build-input-field-live-proof-v29.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV29HistoryExpectation = "pending" | "authorized";

export interface InputLiveV29AuthorizationArtifact {
  artifactVersion: "input-live-v29-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V29_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV29AntecedentIndex {
...
export interface InputLiveV29HistoryState {
  index?: InputLiveV29AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV29AuthorizationArtifact;
...
export interface InputLiveV29AuthorizationProof extends InputLiveV29TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V29_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V29_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V29_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V29_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v29-status") ||
...
export function validateInputLiveV29AntecedentIndex(
  index: InputLiveV29AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v29-antecedent-index-v1" ||
...
export function validateInputLiveV29UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV29History\(\s*["']/.test(source) ||
    /readInputLiveV29HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV29AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV29AuthorizationArtifact {
  ...
  ) as InputLiveV29AntecedentIndex;
  const indexFailures = validateInputLiveV29AntecedentIndex(index);
...
export function validateInputLiveV29AuthorizationArtifact(
  artifact: InputLiveV29AuthorizationArtifact | undefined,
  index: InputLiveV29AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v29-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v29" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V29_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V29_TARGET) ||
    !equalJson(
...
export function validateInputLiveV29History(
  state: InputLiveV29HistoryState,
  expected: InputLiveV29HistoryExpectation,
): string[] {
  const failures = validateInputLiveV29AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV29AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV29HistoryState(
  root = process.cwd(),
): InputLiveV29HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V29_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV29AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V29_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V29_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V29_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV29History(
  expected: InputLiveV29HistoryExpectation,
  root = process.cwd(),
): InputLiveV29HistoryState {
  const state = readInputLiveV29HistoryState(root);
  const failures = validateInputLiveV29History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV29Authorization(
  root = process.cwd(),
): InputLiveV29AuthorizationProof {
  const state = verifyInputLiveV29History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V29_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v29/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v29.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V29_TARGET,
    expectedDynamicTool: INPUT_LIVE_V29_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V29_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV29HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV29History(expected);
