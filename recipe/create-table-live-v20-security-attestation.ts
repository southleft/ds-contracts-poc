...
import { verifyTableLiveV20Authorization } from "./table-live-v20-authorization.js";
import { TABLE_LIVE_V20_TARGET } from "./table-live-v20-broker.js";
import {
  TABLE_LIVE_V20_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV20Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-table-v20-session";
...
const attestation: TableLiveV20SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV20SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v20 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V20_SECURITY_ATTESTATION_DEFAULT_PATH,
);
