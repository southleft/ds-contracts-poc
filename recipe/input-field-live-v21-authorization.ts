...
import {
  ...
  INPUT_LIVE_V21_INDEX_PATH,
} from "./build-input-field-live-proof-v21.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV21HistoryExpectation = "pending" | "authorized";

export interface InputLiveV21AuthorizationArtifact {
  artifactVersion: "input-live-v21-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V21_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV21AntecedentIndex {
...
export interface InputLiveV21HistoryState {
  index?: InputLiveV21AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV21AuthorizationArtifact;
...
export interface InputLiveV21AuthorizationProof extends InputLiveV21TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V21_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V21_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V21_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V21_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v21-status") ||
...
export function validateInputLiveV21AntecedentIndex(
  index: InputLiveV21AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v21-antecedent-index-v1" ||
...
export function validateInputLiveV21UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV21History\(\s*["']/.test(source) ||
    /readInputLiveV21HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV21AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV21AuthorizationArtifact {
  ...
  ) as InputLiveV21AntecedentIndex;
  const indexFailures = validateInputLiveV21AntecedentIndex(index);
...
export function validateInputLiveV21AuthorizationArtifact(
  artifact: InputLiveV21AuthorizationArtifact | undefined,
  index: InputLiveV21AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v21-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v21" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V21_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V21_TARGET) ||
    !equalJson(
...
export function validateInputLiveV21History(
  state: InputLiveV21HistoryState,
  expected: InputLiveV21HistoryExpectation,
): string[] {
  const failures = validateInputLiveV21AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV21AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV21HistoryState(
  root = process.cwd(),
): InputLiveV21HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V21_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV21AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V21_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V21_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V21_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV21History(
  expected: InputLiveV21HistoryExpectation,
  root = process.cwd(),
): InputLiveV21HistoryState {
  const state = readInputLiveV21HistoryState(root);
  const failures = validateInputLiveV21History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV21Authorization(
  root = process.cwd(),
): InputLiveV21AuthorizationProof {
  const state = verifyInputLiveV21History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V21_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v21/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v21.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V21_TARGET,
    expectedDynamicTool: INPUT_LIVE_V21_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V21_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV21HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV21History(expected);
