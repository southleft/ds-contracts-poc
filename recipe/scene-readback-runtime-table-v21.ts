...
/**
 * Table live v21 extract runtime. Keeps the v6 skip of untagged
 * `table/row/owned-cell-label` TEXT bind hosts. Extract bytes are
 * unchanged from v20. The v21 teaching lives in host-normalize
 * (`scene-readback-table-v1.ts`): omit `cornerRadius` on `table/row`
 * variants because compile row variants omit `cornerRadius`. Does not
 * omit cell-variant `cornerRadius` or `clipsContent`. Does not invent
 * `cornerRadius` onto compile row variants. Does not omit
 * `table/variant` `strokes` or `cornerRadius`.
 */
