...
import {
  validateTableLiveV37AntecedentIndex,
  verifyTableLiveV37Authorization,
  type TableLiveV37AntecedentIndex,
  type TableLiveV37AuthorizationProof,
} from "./table-live-v37-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V37_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v37/operator-security-attestation-template.json";
export const TABLE_LIVE_V37_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v37-security-attestation.json";

export interface TableLiveV37SecurityAttestation {
  artifactVersion: "table-live-v37-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV37PreflightReport {
  artifactVersion: "table-live-v37-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV37SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV37SecurityAttestation(
  value: unknown,
  proof: TableLiveV37AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v37-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V37_TARGET) ||
...
export function validateTableLiveV37ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV37Authorization");
  const preflight = source.indexOf("runTableLiveV37Preflight");
  const initialize = source.indexOf("TableLiveV37Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV37Preflight(
  root: string,
  proof: TableLiveV37AuthorizationProof,
  attestationPath = TABLE_LIVE_V37_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV37PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V37_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V37_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV37AntecedentIndex;
  failures.push(...validateTableLiveV37AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V37_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V37_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV37SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v37-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV37ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v37 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV37SecurityAttestation;
  return {
    artifactVersion: "table-live-v37-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V37_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV37Preflight(
    process.cwd(),
    verifyTableLiveV37Authorization(),
    attestationPath,
