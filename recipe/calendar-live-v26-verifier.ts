...
import {
  verifyCalendarLiveV26FixedSceneFixedPoint,
  type CalendarLiveV26FixedFixedPoint,
} from "./calendar-live-v26-fixed-point.js";

export const CALENDAR_LIVE_V26_VERIFIER_VERSION = "calendar-live-v26-verifier-v1";

export function buildCalendarLiveV26RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV26RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV26RawNode[];
}

export interface CalendarLiveV26NormalizedScene {
...
export interface CalendarLiveV26SceneProof {
...
export function normalizeCalendarLiveV26Scene(
  root: CalendarLiveV26RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV26NormalizedScene {
  const canonicalBindings: CalendarLiveV26NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV26RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV26Scene<Instance>(
  normalized: CalendarLiveV26NormalizedScene,
...
): CalendarLiveV26SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV26FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV26ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV26PreCaptureGates(payload: unknown): void {
