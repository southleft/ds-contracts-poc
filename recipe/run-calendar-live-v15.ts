...
import {
  acceptCalendarLiveV15Response,
  createCalendarLiveV15Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV15Request,
  persistCalendarLiveV15RawResponse,
  persistCalendarLiveV15TechnicalGates,
  readCalendarLiveV15AcceptedRawPayload,
  type CalendarLiveV15Request,
  type CalendarLiveV15TransactionAuthorization,
} from "./calendar-live-v15-broker.js";
import {
  CALENDAR_LIVE_V15_CAPTURE_COUNT,
  assertCalendarLiveV15CaptureResponses,
  ...
  evaluateCalendarLiveV15Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV15Roots,
  validateCalendarLiveV15CaptureManifest,
  validateCalendarLiveV15CapturePayload,
  validateCalendarLiveV15CleanupPayload,
  validateCalendarLiveV15ExtractPayload,
  validateCalendarLiveV15ProbePayload,
  validateCalendarLiveV15WriterPayload,
  type CalendarLiveV15CaptureCell,
  type CalendarLiveV15CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V15_PLAN_PATH,
  type CalendarLiveV15ProofPlan,
} from "./build-calendar-live-proof-v15.js";
import {
...
export const CALENDAR_LIVE_V15_RUNNER_VERSION = "calendar-live-v15-runner-v1";

interface CalendarLiveV15JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV15OrchestratorState {
  artifactVersion: "calendar-live-v15-orchestrator-state-v1";
  ...
  journal: CalendarLiveV15JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV15ProofPlan;
  planSha256: string;
  captures: CalendarLiveV15CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV15SourceIdentity & {
...
export interface CalendarLiveV15NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV15OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v15 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V15_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V15_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV15ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV15CaptureCell[];
  validateCalendarLiveV15CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v15 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v15 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV15WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV15RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV15Request =>
  readJson<CalendarLiveV15Request>(file);

export class CalendarLiveV15Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV15Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV15OrchestratorState = {
      artifactVersion: "calendar-live-v15-orchestrator-state-v1",
    ...
    return new CalendarLiveV15Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV15OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV15Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV15NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV15NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v15 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV15NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v15 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV15RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV15Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV15CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV15AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV15WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV15Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV15ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV15Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV15Response(this.#directory, "writer", {
        validate: validateCalendarLiveV15WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV15Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV15RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV15Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV15ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV15Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV15RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v15-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV15Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV15ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV15Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV15ProbePayload(payload, ownership),
      });
      persistCalendarLiveV15TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v15 capture cell missing");
      const accepted = acceptCalendarLiveV15Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV15CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V15_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV15CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV15Objective(
    ...
    throw new Error(`Calendar live v15 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV15Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV15CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV15Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV15RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV15Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV15CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV15CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV15CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV15Authorization = (
  privateKey: KeyObject,
): CalendarLiveV15TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV15SimulationKey = (): KeyObject =>
