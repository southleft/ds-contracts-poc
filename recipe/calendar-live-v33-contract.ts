...
import { buildCalendarLiveV33CleanupRuntime } from "./calendar-live-v33-cleanup.js";
import { type CalendarLiveV33FixedFixedPoint } from "./calendar-live-v33-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV33RestoreProgram,
  validateCalendarLiveV33RestorePayload,
  type CalendarLiveV33RestorePayload,
} from "./calendar-live-v33-restore.js";
import {
  buildCalendarLiveV33RawPropertyRuntime,
  normalizeCalendarLiveV33Scene,
  type CalendarLiveV33RawNode,
} from "./calendar-live-v33-verifier.js";
...
export const CALENDAR_LIVE_V33_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V33_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V33_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V33_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V33_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V33_SET_COUNT = 3;
export const CALENDAR_LIVE_V33_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V33_HOST_PHASES = 3;
export const CALENDAR_LIVE_V33_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V33_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V33_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V33_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV33SourceId = (typeof CALENDAR_LIVE_V33_SOURCE_IDS)[number];
export type CalendarLiveV33Kind = "calendar" | "week" | "day";

export interface CalendarLiveV33SourceIdentity {
  source: CalendarLiveV33SourceId;
...
export interface CalendarLiveV33WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV33ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV33SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV33RawNode;
    weekScene: CalendarLiveV33RawNode;
    dayScene: CalendarLiveV33RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV33RootProof {
  source: CalendarLiveV33SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV33FixedFixedPoint;
}

export interface CalendarLiveV33ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV33SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV33CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV33SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV33CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV33SourceId;
  frameWidth: number;
...
export interface CalendarLiveV33CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v33 ${label} must be an object`);
...
export function validateCalendarLiveV33WriterPayload(
  value: unknown,
): CalendarLiveV33WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v33") ||
    ...
    result.namespace !== CALENDAR_LIVE_V33_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v33 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v33 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v33 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV33SourceIdentity,
  kind: CalendarLiveV33Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV33ExtractProgram(
  writer: CalendarLiveV33WriterOwnership,
  sources: readonly CalendarLiveV33SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v33 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V33_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V33_NAMESPACE)}
${buildCalendarLiveV33RawPropertyRuntime()}
...
export function validateCalendarLiveV33ExtractPayload(
  value: unknown,
  writer: CalendarLiveV33WriterOwnership,
): CalendarLiveV33ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v33 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V33_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v33 extract schema/one-root mismatch");
  return payload as CalendarLiveV33ExtractPayload;
}

export function proveCalendarLiveV33Roots<Instance>(
  extract: CalendarLiveV33ExtractPayload,
  sources: readonly (CalendarLiveV33SourceIdentity & {
...
): CalendarLiveV33RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v33 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v33 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV33Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV33Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV33Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV33FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v33 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v33 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v33 compile lost owned sets");
...
export function assertCalendarLiveV33RootProofs(
  proofs: readonly CalendarLiveV33RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v33 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v33 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV33ProbeProgram(
  writer: CalendarLiveV33WriterOwnership,
  sources: readonly CalendarLiveV33SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V33_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV33ProbePayload(
  value: unknown,
  writer: CalendarLiveV33WriterOwnership,
): CalendarLiveV33ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV33ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V33_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V33_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v33 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV33CaptureManifest(
  cells: readonly CalendarLiveV33CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V33_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V33_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V33_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V33_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v33 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV33CaptureCell[],
): string => {
  validateCalendarLiveV33CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV33CaptureProgram(
  writer: CalendarLiveV33WriterOwnership,
  cell: CalendarLiveV33CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V33_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v33 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v33 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V33_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV33CapturePayload(
  value: unknown,
  cell: CalendarLiveV33CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV33CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV33CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V33_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V33_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v33 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV33CaptureResponses(
  manifest: readonly CalendarLiveV33CaptureCell[],
  responses: readonly CalendarLiveV33CapturePayload[],
): void {
  validateCalendarLiveV33CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v33 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV33ObjectiveReport {
  artifactVersion: "calendar-live-v33-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV33Objective(
  manifest: readonly CalendarLiveV33CaptureCell[],
  responses: readonly CalendarLiveV33CapturePayload[],
): CalendarLiveV33ObjectiveReport {
  assertCalendarLiveV33CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v33-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV33CleanupProgram(
  writer: CalendarLiveV33WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV33CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V33_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v33 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV33CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV33WriterOwnership,
): CalendarLiveV33CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV33CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v33 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V33_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV33WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
