...
export const INPUT_LIVE_V34_VERIFIER_VERSION = "input-live-v34-verifier-v1";

export function buildInputLiveV34RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV34RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV34RawNode[];
}

export interface InputLiveV34NormalizedScene {
...
export interface InputLiveV34SceneProof {
...
export function normalizeInputLiveV34Scene(
  root: InputLiveV34RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV34NormalizedScene {
  const canonicalBindings: InputLiveV34NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV34RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV34Scene<Instance>(
  normalized: InputLiveV34NormalizedScene,
...
export function compileInputLiveV34ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV34PreCaptureGates(payload: unknown): void {
