...
import {
  buildInputLiveV75CaptureProgram,
  buildInputLiveV75ExtractProgram,
  buildInputLiveV75ProbeProgram,
  buildInputLiveV75RestoreProgram,
  INPUT_LIVE_V75_CAPTURE_COUNT,
  INPUT_LIVE_V75_RESPONSE_CONTRACTS,
  inputLiveV75CaptureManifestSha256,
  type InputLiveV75CaptureCell,
  type InputLiveV75SourceIdentity,
  type InputLiveV75WriterOwnership,
} from "./input-field-live-v75-contract.js";
import {
  INPUT_LIVE_V75_DYNAMIC_TOOL,
  INPUT_LIVE_V75_TARGET,
  inputLiveV75RequestSequence,
} from "./input-field-live-v75-broker.js";
import {
...
} from "./input-field-live-v75-restore.js";

export const INPUT_LIVE_V75_EVIDENCE_ROOT =
  "recipe/evidence/input-field-live-pivot-v75";
export const INPUT_LIVE_V75_PROTOCOL_PATH = `${INPUT_LIVE_V75_EVIDENCE_ROOT}/protocol.json`;
export const INPUT_LIVE_V75_PLAN_PATH = `${INPUT_LIVE_V75_EVIDENCE_ROOT}/proof-plan.json`;
export const INPUT_LIVE_V75_CAPTURE_MANIFEST_PATH = `${INPUT_LIVE_V75_EVIDENCE_ROOT}/capture-manifest.json`;
export const INPUT_LIVE_V75_REQUEST_MANIFEST_PATH = `${INPUT_LIVE_V75_EVIDENCE_ROOT}/request-manifest.json`;
export const INPUT_LIVE_V75_INDEX_PATH = `${INPUT_LIVE_V75_EVIDENCE_ROOT}/antecedent-index.json`;
export const INPUT_LIVE_V75_AUTHORIZATION_TEMPLATE_PATH = `${INPUT_LIVE_V75_EVIDENCE_ROOT}/authorization-template.json`;
export const INPUT_LIVE_V75_AUTHORIZATION_PATH = `${INPUT_LIVE_V75_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface InputLiveV75ProofPlan {
  artifactVersion: "input-live-v75-proof-plan-v1";
...
interface GeneratedSource {
  ...
  recipeHash: string;
  envelopeHash: string;
  expectedScenePlan: InputLiveV75SourceIdentity["expectedScenePlan"];
  outputPath: string;
  compressed: Buffer;
  metadata: InputLiveV75ProofPlan["sources"][number]["expectedScenePlan"];
}
...
const sourcePlans = (): GeneratedSource[] => {
  ...
  return prior.sources.map((source: Record<string, any>) => {
    ...
    if (!rewrittenText.includes(needle))
      throw new Error(
        `Input live v75 missing set layout.mode vertical fact: ${inputPath}`,
      );
    ...
    if (!modeRewritten.includes(paddingNeedle))
      throw new Error(
        `Input live v75 missing set layout.padding 0 fact: ${inputPath}`,
      );
    ...
    if (setMode?.value !== "horizontal")
      throw new Error(
        `Input live v75 set layout.mode rewrite failed: ${inputPath}`,
      );
    ...
    if (
      ...
      throw new Error(
        `Input live v75 set layout.padding rewrite failed: ${inputPath}`,
      );
    ...
    return {
      ...
      outputPath: `${INPUT_LIVE_V75_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
      compressed: rewrittenCompressed,
      metadata: {
        path: `${INPUT_LIVE_V75_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
...
const captureCells = (): InputLiveV75CaptureCell[] => {
  const cells = json<Record<string, any>>(V16_CAPTURE_MANIFEST)
    .cells as InputLiveV75CaptureCell[];
  inputLiveV75CaptureManifestSha256(cells);
  return cells;
};

const protocol = () => ({
  artifactVersion: "input-live-v75-external-operator-protocol-v1",
  protocolId: "input-live-v75",
  status:
  ...
  hostNormalization: {
    ...
    carriedSceneReadback: "recipe/scene-readback-v75.ts",
    carriedSceneReadbackRuntime: "recipe/scene-readback-runtime-v75.ts",
    carriedV3Verifier: "recipe/input-field-live-v3-verifier-v75.ts",
...
const requestManifest = (
  captures: readonly InputLiveV75CaptureCell[],
  programs: {
    writerSha256: string;
  ...
  const requests = [
    {
      requestId: "writer",
      sequence: inputLiveV75RequestSequence("writer"),
      phase: "writer",
      predecessorRequestId: null,
      programSha256: programs.writerSha256,
      response: INPUT_LIVE_V75_RESPONSE_CONTRACTS.writer,
    },
    {
      requestId: "cleanup",
      sequence: inputLiveV75RequestSequence("cleanup"),
      phase: "cleanup",
      predecessorRequestId: "writer",
      programBuilderSha256: programs.cleanupBuilderSha256,
      response: INPUT_LIVE_V75_RESPONSE_CONTRACTS.cleanup,
      availability: "persisted immediately after writer acceptance",
    },
    {
      requestId: "restore",
      sequence: inputLiveV75RequestSequence("restore"),
      phase: "restore",
      predecessorRequestId: "writer",
      programSha256: programs.restoreBlueprintSha256,
      response: INPUT_LIVE_V75_RESPONSE_CONTRACTS.restore,
      availability:
        "issued immediately after writer acceptance; before extract",
    },
    {
      requestId: "extract",
      sequence: inputLiveV75RequestSequence("extract"),
      phase: "extract",
      predecessorRequestId: "restore",
      programSha256: programs.extractBlueprintSha256,
      response: INPUT_LIVE_V75_RESPONSE_CONTRACTS.extract,
    },
    {
      requestId: "probe",
      sequence: inputLiveV75RequestSequence("probe"),
      phase: "probe",
      predecessorRequestId: "extract",
      programSha256: programs.probeBlueprintSha256,
      response: INPUT_LIVE_V75_RESPONSE_CONTRACTS.probe,
    },
    ...captures.map((cell) => ({
      requestId: `capture-${String(cell.index).padStart(3, "0")}`,
      sequence: inputLiveV75RequestSequence("capture", cell.index),
      phase: "capture",
      ...
      response: INPUT_LIVE_V75_RESPONSE_CONTRACTS.capture,
    })),
  ];
  return {
    artifactVersion: "input-live-v75-request-manifest-v1",
    expectedDynamicTool: INPUT_LIVE_V75_DYNAMIC_TOOL,
    target: INPUT_LIVE_V75_TARGET,
...
const lifecycleExcludedPaths = [
  INPUT_LIVE_V75_AUTHORIZATION_TEMPLATE_PATH,
  INPUT_LIVE_V75_AUTHORIZATION_PATH,
  "recipe/input-field-live-v75-authorization.ts",
  "recipe/input-field-live-v75-authorization.test.ts",
  "recipe/input-field-live-v75-preflight.ts",
  "recipe/input-field-live-v75-authorized.ts",
  "recipe/evidence/input-field-live-pivot-v75/operator-security-attestation-template.json",
...
const sourceNeutralDependencies = [
  ...
  "recipe/normalize.ts",
  "recipe/scene-readback-runtime-v75.ts",
  "recipe/scene-readback-v75.ts",
  "recipe/input-field-live-v75-restore.ts",
  "recipe/input-field-figma-writer-v17.ts",
] as const;

export async function buildInputLiveV75Proof(
  check = process.argv.includes("--check"),
): Promise<InputLiveV75ProofPlan> {
  ...
  if (
    ...
    throw new Error("Input live v75 frozen v16 writer bytes drifted");
  const builtWriter = buildInputLiveV17WriterProgram(
  ...
  if (
    ...
    throw new Error(
      "Input live v75 exact committed v16 writer/restore/runtime/extract or hashed v16 writer or v17-v66/v70/v74 scene-readback bytes drifted",
    );

  const writerOwnershipBlueprint: InputLiveV75WriterOwnership = {
    pageId: "__WRITER_PAGE_ID__",
    pageName: "Recipe Pivot / Input Field / v9",
    runIdentity: "input-live-v75",
  ...
  const identities = sources.map(
    (source) =>
      ...
      }) satisfies InputLiveV75SourceIdentity,
  );
  const restoreBlueprint = Buffer.from(
    buildInputLiveV75RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildInputLiveV75ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildInputLiveV75ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildInputLiveV75CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );

  const antecedentOutputs = new Map<string, Buffer>();
  antecedentOutputs.set(INPUT_LIVE_V75_PROTOCOL_PATH, jsonBytes(protocol()));
  antecedentOutputs.set(
    `${INPUT_LIVE_V75_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V75_EVIDENCE_ROOT}/programs/writer-payload.js`,
    writerPayload,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V75_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V75_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V75_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V75_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  ...
  const captureManifest = {
    artifactVersion: "input-live-v75-capture-manifest-v1",
    ...
    cells: captures,
    cellsSha256: inputLiveV75CaptureManifestSha256(captures),
  };
  const captureManifestBytes = jsonBytes(captureManifest);
  antecedentOutputs.set(
    INPUT_LIVE_V75_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: InputLiveV75ProofPlan = {
    artifactVersion: "input-live-v75-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: INPUT_LIVE_V75_TARGET,
    namespace: "ds.contracts.input.recipe.v5",
    writer: {
      programPath: `${INPUT_LIVE_V75_EVIDENCE_ROOT}/programs/writer.txt`,
      programBytes: writerProgram.byteLength,
      programSha256: sha256(writerProgram),
      payloadPath: `${INPUT_LIVE_V75_EVIDENCE_ROOT}/programs/writer-payload.js`,
    ...
    captureManifest: {
      path: INPUT_LIVE_V75_CAPTURE_MANIFEST_PATH,
      cells: 128,
      requests: 128,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(INPUT_LIVE_V75_PLAN_PATH, proofPlanBytes);
  ...
  antecedentOutputs.set(
    INPUT_LIVE_V75_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/input-field-live-v75-broker.ts",
    "recipe/input-field-live-v75-contract.ts",
    "recipe/run-input-field-live-v75.ts",
    "recipe/build-input-field-live-proof-v75.ts",
    "recipe/input-field-live-v75-broker.test.ts",
    "recipe/figma-property-normalizer-v8.test.ts",
    "recipe/scene-readback-v75.test.ts",
    "recipe/input-field-live-v75-restore.ts",
    "recipe/input-field-live-v75-restore.test.ts",
    "recipe/input-field-live-v75-lifecycle-simulation.ts",
    ...sourceNeutralDependencies,
  ].filter((artifactPath) => artifactPath !== INPUT_LIVE_V75_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "input-live-v75-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    ...
    counts: {
      ...
      captureCells: INPUT_LIVE_V75_CAPTURE_COUNT,
  ...
  antecedentOutputs.set(INPUT_LIVE_V75_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "input-live-v75-authorization-template-v1",
    authorizationId: "input-live-v75",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: INPUT_LIVE_V75_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
  ...
  if (check) {
    ...
    mkdirSync(path.dirname(INPUT_LIVE_V75_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(INPUT_LIVE_V75_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(INPUT_LIVE_V75_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(INPUT_LIVE_V75_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildInputLiveV75Proof(), null, 2)}\n`,
  );
