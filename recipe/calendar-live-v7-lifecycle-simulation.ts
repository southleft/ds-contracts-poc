...
import { calendarLiveV1Sha256 } from "./calendar-live-v7-broker.js";

export interface CalendarLiveV7LifecycleSimulationReport {
  artifactVersion: "calendar-live-v7-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV7AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV7Lifecycle(): Promise<CalendarLiveV7LifecycleSimulationReport> {
  await buildCalendarLiveV7Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V7_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV7AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v7-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V7_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V7_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV7History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV7History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV7AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V7_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V7_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV7History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV7History(authorized, "pending");
    ...
    await buildCalendarLiveV7Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V7_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV7AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV7Lifecycle(), null, 2)}\n`,
  );
