...
import { verifyTableLiveV6Authorization } from "./table-live-v6-authorization.js";
import { TABLE_LIVE_V6_TARGET } from "./table-live-v6-broker.js";
import {
  TABLE_LIVE_V6_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV6Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-table-v6-session";
...
const attestation: TableLiveV6SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV6SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v6 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V6_SECURITY_ATTESTATION_DEFAULT_PATH,
);
