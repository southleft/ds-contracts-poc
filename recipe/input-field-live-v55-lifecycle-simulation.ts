...
export interface InputLiveV55LifecycleSimulationReport {
  artifactVersion: "input-live-v55-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV55AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV55Lifecycle(): Promise<InputLiveV55LifecycleSimulationReport> {
  await buildInputLiveV55Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V55_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV55AntecedentIndex;
  const indexSha256 = inputLiveV55Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v55-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V55_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V55_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV55History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV55History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV55AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V55_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V55_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV55History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV55History(authorized, "pending");
    ...
    await buildInputLiveV55Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V55_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV55AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV55Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV55Lifecycle(), null, 2)}\n`,
  );
