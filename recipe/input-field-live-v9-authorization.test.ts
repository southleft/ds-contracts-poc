...
import { INPUT_LIVE_V9_INDEX_PATH } from "./build-input-field-live-proof-v9.js";
import {
...
import {
  ...
  validateInputLiveV9AntecedentIndex,
  validateInputLiveV9AuthorizationArtifact,
  validateInputLiveV9History,
  validateInputLiveV9UnitTestSource,
  type InputLiveV9AntecedentIndex,
  type InputLiveV9AuthorizationArtifact,
  type InputLiveV9AuthorizationProof,
  type InputLiveV9HistoryState,
} from "./input-field-live-v9-authorization.js";
import {
  inputLiveV9AttestationSha256,
  validateInputLiveV9ControlFlowSource,
  validateInputLiveV9SecurityAttestation,
  type InputLiveV9SecurityAttestation,
} from "./input-field-live-v9-preflight.js";
import { inputLiveV9Sha256 } from "./input-field-live-v9-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V9_INDEX_PATH);
const index = (): InputLiveV9AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV9AntecedentIndex;

const artifact = (): InputLiveV9AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV9AuthorizationArtifact({
...
const pendingState = (): InputLiveV9HistoryState => ({
  index: index(),
  indexSha256: inputLiveV9Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV9HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV9AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV9Sha256(indexBytes()),
...
const attestation = (
...
): InputLiveV9SecurityAttestation => {
  const body: Omit<InputLiveV9SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v9-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V9_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV9AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV9History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV9History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV9History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV9History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV9History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV9UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV9UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV9AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV9Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV9AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV9AuthorizationArtifact(
        planted as InputLiveV9AuthorizationArtifact,
        index(),
        inputLiveV9Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV9SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV9AttestationSha256(
      body as Omit<InputLiveV9SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV9SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV9ControlFlowSource(source), []);
});
