...
import {
  acceptTableLiveV21Response,
  createTableLiveV21Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV21Request,
  persistTableLiveV21RawResponse,
  persistTableLiveV21TechnicalGates,
  readTableLiveV21AcceptedRawPayload,
  type TableLiveV21Request,
  type TableLiveV21TransactionAuthorization,
} from "./table-live-v21-broker.js";
import {
  TABLE_LIVE_V21_CAPTURE_COUNT,
  assertTableLiveV21CaptureResponses,
  ...
  evaluateTableLiveV21Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV21Roots,
  validateTableLiveV21CaptureManifest,
  validateTableLiveV21CapturePayload,
  validateTableLiveV21CleanupPayload,
  validateTableLiveV21ExtractPayload,
  validateTableLiveV21ProbePayload,
  validateTableLiveV21WriterPayload,
  type TableLiveV21CaptureCell,
  type TableLiveV21CapturePayload,
...
import {
  ...
  TABLE_LIVE_V21_PLAN_PATH,
  type TableLiveV21ProofPlan,
} from "./build-table-live-proof-v21.js";
import {
...
export const TABLE_LIVE_V21_RUNNER_VERSION = "table-live-v21-runner-v1";

interface TableLiveV21JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV21OrchestratorState {
  artifactVersion: "table-live-v21-orchestrator-state-v1";
  ...
  journal: TableLiveV21JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV21ProofPlan;
  planSha256: string;
  captures: TableLiveV21CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV21SourceIdentity & {
...
export interface TableLiveV21NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV21OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v21 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V21_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V21_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV21ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV21CaptureCell[];
  validateTableLiveV21CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v21 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v21 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV21WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV21RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV21Request =>
  readJson<TableLiveV21Request>(file);

export class TableLiveV21Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV21Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV21OrchestratorState = {
      artifactVersion: "table-live-v21-orchestrator-state-v1",
    ...
    return new TableLiveV21Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV21OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV21Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV21NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV21NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v21 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV21NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v21 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV21RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV21Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV21CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV21AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV21WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV21Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV21ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV21Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV21Response(this.#directory, "writer", {
        validate: validateTableLiveV21WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV21Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV21RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV21Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV21ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV21Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV21RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v21-host-root-proofs-v1",
      ...
      const next = issueTableLiveV21Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV21ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV21Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV21ProbePayload(payload, ownership),
      });
      persistTableLiveV21TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v21 capture cell missing");
      const accepted = acceptTableLiveV21Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV21CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V21_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV21CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV21Objective(
    ...
    throw new Error(`Table live v21 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV21Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV21CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV21Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV21RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV21Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV21CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV21CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV21CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV21Authorization = (
  privateKey: KeyObject,
): TableLiveV21TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV21SimulationKey = (): KeyObject =>
