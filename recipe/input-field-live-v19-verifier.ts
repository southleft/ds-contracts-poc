...
export const INPUT_LIVE_V19_VERIFIER_VERSION = "input-live-v19-verifier-v1";

export function buildInputLiveV19RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV19RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV19RawNode[];
}

export interface InputLiveV19NormalizedScene {
...
export interface InputLiveV19SceneProof {
...
export function normalizeInputLiveV19Scene(
  root: InputLiveV19RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV19NormalizedScene {
  const canonicalBindings: InputLiveV19NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV19RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV19Scene<Instance>(
  normalized: InputLiveV19NormalizedScene,
...
export function compileInputLiveV19ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV19PreCaptureGates(payload: unknown): void {
