...
export type CalendarLiveV14HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV14AuthorizationArtifact {
  artifactVersion: "calendar-live-v14-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V14_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV14AntecedentIndex {
...
export interface CalendarLiveV14HistoryState {
  index?: CalendarLiveV14AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV14AuthorizationArtifact;
...
export interface CalendarLiveV14AuthorizationProof
  extends CalendarLiveV14TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V14_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V14_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V14_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V14_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v14-authorization") ||
  artifactPath.includes("calendar-live-v14-preflight") ||
  artifactPath.includes("calendar-live-v14-authorized") ||
  artifactPath.includes("create-calendar-live-v14-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v14-status") ||
...
export function validateCalendarLiveV14AntecedentIndex(
  index: CalendarLiveV14AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v14-antecedent-index-v1" ||
...
export function validateCalendarLiveV14UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV14History\(\s*["']/.test(source) ||
    /readCalendarLiveV14HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV14AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV14AuthorizationArtifact {
  ...
  ) as CalendarLiveV14AntecedentIndex;
  const indexFailures = validateCalendarLiveV14AntecedentIndex(index);
...
export function validateCalendarLiveV14AuthorizationArtifact(
  artifact: CalendarLiveV14AuthorizationArtifact | undefined,
  index: CalendarLiveV14AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v14-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v14" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V14_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V14_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V14_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V14_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V14_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V14_SOURCE_ROOTS ||
...
export function validateCalendarLiveV14History(
  state: CalendarLiveV14HistoryState,
  expected: CalendarLiveV14HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV14AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV14AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV14HistoryState(
  root = process.cwd(),
): CalendarLiveV14HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V14_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV14AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V14_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V14_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V14_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV14History(
  expected: CalendarLiveV14HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV14HistoryState {
  const state = readCalendarLiveV14HistoryState(root);
  const failures = validateCalendarLiveV14History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v14 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV14Authorization(
  root = process.cwd(),
): CalendarLiveV14AuthorizationProof {
  const state = verifyCalendarLiveV14History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V14_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v14/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v14.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V14_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V14_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V14_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV14HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV14History(expected);
  process.stdout.write(
    `Calendar live v14 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
