...
import { COMBOBOX_LIVE_V29_INDEX_PATH } from "./build-combobox-live-proof-v29.js";
import {
  COMBOBOX_LIVE_V29_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V29_TARGET,
} from "./combobox-live-v29-broker.js";
import {
  buildComboboxLiveV29AuthorizationArtifact,
  validateComboboxLiveV29AntecedentIndex,
  validateComboboxLiveV29AuthorizationArtifact,
  validateComboboxLiveV29History,
  validateComboboxLiveV29UnitTestSource,
  type ComboboxLiveV29AntecedentIndex,
  type ComboboxLiveV29AuthorizationArtifact,
  type ComboboxLiveV29AuthorizationProof,
  type ComboboxLiveV29HistoryState,
} from "./combobox-live-v29-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV29ControlFlowSource,
  validateComboboxLiveV29SecurityAttestation,
  type ComboboxLiveV29SecurityAttestation,
} from "./combobox-live-v29-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v29-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V29_INDEX_PATH);
const index = (): ComboboxLiveV29AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV29AntecedentIndex;

const artifact = (): ComboboxLiveV29AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV29AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV29HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV29HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV29AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v29/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV29AuthorizationArtifact,
): ComboboxLiveV29SecurityAttestation => {
  const body: Omit<ComboboxLiveV29SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V29_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV29History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV29History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV29History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV29History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV29History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV29UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV29UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV29AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV29AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV29AuthorizationArtifact(
        planted as ComboboxLiveV29AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV29SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV29SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV29SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV29ControlFlowSource(source), []);
});
