...
import {
  acceptCalendarLiveV11Response,
  createCalendarLiveV11Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV11Request,
  persistCalendarLiveV11RawResponse,
  persistCalendarLiveV11TechnicalGates,
  readCalendarLiveV11AcceptedRawPayload,
  type CalendarLiveV11Request,
  type CalendarLiveV11TransactionAuthorization,
} from "./calendar-live-v11-broker.js";
import {
  CALENDAR_LIVE_V11_CAPTURE_COUNT,
  assertCalendarLiveV11CaptureResponses,
  ...
  evaluateCalendarLiveV11Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV11Roots,
  validateCalendarLiveV11CaptureManifest,
  validateCalendarLiveV11CapturePayload,
  validateCalendarLiveV11CleanupPayload,
  validateCalendarLiveV11ExtractPayload,
  validateCalendarLiveV11ProbePayload,
  validateCalendarLiveV11WriterPayload,
  type CalendarLiveV11CaptureCell,
  type CalendarLiveV11CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V11_PLAN_PATH,
  type CalendarLiveV11ProofPlan,
} from "./build-calendar-live-proof-v11.js";
import {
...
export const CALENDAR_LIVE_V11_RUNNER_VERSION = "calendar-live-v11-runner-v1";

interface CalendarLiveV11JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV11OrchestratorState {
  artifactVersion: "calendar-live-v11-orchestrator-state-v1";
  ...
  journal: CalendarLiveV11JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV11ProofPlan;
  planSha256: string;
  captures: CalendarLiveV11CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV11SourceIdentity & {
...
export interface CalendarLiveV11NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV11OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v11 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V11_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V11_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV11ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV11CaptureCell[];
  validateCalendarLiveV11CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v11 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v11 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV11WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV11RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV11Request =>
  readJson<CalendarLiveV11Request>(file);

export class CalendarLiveV11Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV11Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV11OrchestratorState = {
      artifactVersion: "calendar-live-v11-orchestrator-state-v1",
    ...
    return new CalendarLiveV11Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV11OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV11Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV11NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV11NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v11 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV11NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v11 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV11RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV11Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV11CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV11AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV11WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV11Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV11ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV11Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV11Response(this.#directory, "writer", {
        validate: validateCalendarLiveV11WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV11Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV11RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV11Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV11ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV11Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV11RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v11-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV11Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV11ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV11Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV11ProbePayload(payload, ownership),
      });
      persistCalendarLiveV11TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v11 capture cell missing");
      const accepted = acceptCalendarLiveV11Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV11CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V11_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV11CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV11Objective(
    ...
    throw new Error(`Calendar live v11 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV11Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV11CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV11Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV11RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV11Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV11CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV11CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV11CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV11Authorization = (
  privateKey: KeyObject,
): CalendarLiveV11TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV11SimulationKey = (): KeyObject =>
