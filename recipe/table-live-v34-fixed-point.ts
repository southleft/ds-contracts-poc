/**
 * Table live v34 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V34_FIXED_VERIFIER_VERSION = "table-live-v34-fixed-point-v1";
export const TABLE_LIVE_V34_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV34FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV34FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV34FixedSourceProbe {
...
export interface TableLiveV34FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV34FixedFixedPoint;
}

export interface TableLiveV34FixedHardGateInput {
  thresholds: TableLiveV34FixedThresholds;
  sourceProbes: TableLiveV34FixedSourceProbe[];
  cells: TableLiveV34FixedCellValidation[];
  sceneProofs: TableLiveV34FixedSceneProof[];
...
export interface TableLiveV34FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V34_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV34FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV34FixedHardGates(
  input: TableLiveV34FixedHardGateInput,
): TableLiveV34FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV34FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V34_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV34FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV34FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
