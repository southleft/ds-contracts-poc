/**
 * Calendar live v26 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V26_FIXED_VERIFIER_VERSION = "calendar-live-v26-fixed-point-v1";
export const CALENDAR_LIVE_V26_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV26FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV26FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV26FixedSourceProbe {
...
export interface CalendarLiveV26FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV26FixedFixedPoint;
}

export interface CalendarLiveV26FixedHardGateInput {
  thresholds: CalendarLiveV26FixedThresholds;
  sourceProbes: CalendarLiveV26FixedSourceProbe[];
  cells: CalendarLiveV26FixedCellValidation[];
  sceneProofs: CalendarLiveV26FixedSceneProof[];
...
export interface CalendarLiveV26FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V26_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV26FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV26FixedHardGates(
  input: CalendarLiveV26FixedHardGateInput,
): CalendarLiveV26FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV26FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V26_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV26FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV26FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV26FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
