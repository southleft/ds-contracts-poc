...
import { verifyComboboxLiveV19Authorization } from "./combobox-live-v19-authorization.js";
import { COMBOBOX_LIVE_V19_TARGET } from "./combobox-live-v19-broker.js";
import {
  COMBOBOX_LIVE_V19_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV19Authorization();
...
const attestation: ComboboxLiveV19SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV19SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v19 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V19_SECURITY_ATTESTATION_DEFAULT_PATH,
);
