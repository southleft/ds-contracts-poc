/**
 * Carried Combobox live v18 host verifier. Uses scene-readback-combobox-v18 so live
...
export const COMBOBOX_LIVE_V18_FIXED_VERIFIER_VERSION = "combobox-live-v18-fixed-point-v1";
export const COMBOBOX_LIVE_V18_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV18FixedThresholds {
...
export interface ComboboxLiveV18FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV18FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV18FixedSourceProbe {
...
export interface ComboboxLiveV18FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV18FixedFixedPoint;
}

export interface ComboboxLiveV18FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV18FixedHardGateInput {
  thresholds: ComboboxLiveV18FixedThresholds;
  materialRegression: ComboboxLiveV18FixedMaterialRegression;
  sourceProbes: ComboboxLiveV18FixedSourceProbe[];
  cells: ComboboxLiveV18FixedCellValidation[];
  sceneProofs: ComboboxLiveV18FixedSceneProof[];
  visualRows: ComboboxLiveV18FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV18FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V18_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV18FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV18FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV18FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV18FixedHardGates(
  input: ComboboxLiveV18FixedHardGateInput,
): ComboboxLiveV18FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV18FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV18FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V18_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV18FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV18FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV18FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
