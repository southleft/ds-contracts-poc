...
import { CALENDAR_LIVE_V26_INDEX_PATH } from "./build-calendar-live-proof-v26.js";
import {
...
import {
  buildCalendarLiveV26AuthorizationArtifact,
  validateCalendarLiveV26AntecedentIndex,
  validateCalendarLiveV26AuthorizationArtifact,
  validateCalendarLiveV26History,
  validateCalendarLiveV26UnitTestSource,
  type CalendarLiveV26AntecedentIndex,
  type CalendarLiveV26AuthorizationArtifact,
  type CalendarLiveV26AuthorizationProof,
  type CalendarLiveV26HistoryState,
} from "./calendar-live-v26-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV26ControlFlowSource,
  validateCalendarLiveV26SecurityAttestation,
  type CalendarLiveV26SecurityAttestation,
} from "./calendar-live-v26-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V26_INDEX_PATH);
const index = (): CalendarLiveV26AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV26AntecedentIndex;

const artifact = (): CalendarLiveV26AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV26AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV26HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV26HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV26AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v26/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV26AuthorizationArtifact,
): CalendarLiveV26SecurityAttestation => {
  const body: Omit<CalendarLiveV26SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V26_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV26History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV26History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV26History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV26History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV26History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV26UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v26-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV26UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV26AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV26AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV26AuthorizationArtifact(
        planted as CalendarLiveV26AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV26SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV26SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV26SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v26-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV26ControlFlowSource(source), []);
});
