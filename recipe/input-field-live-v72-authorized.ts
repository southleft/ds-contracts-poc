...
import { INPUT_LIVE_V72_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v72.js";
import { verifyInputLiveV72Authorization } from "./input-field-live-v72-authorization.js";
import { runInputLiveV72Preflight } from "./input-field-live-v72-preflight.js";
import { InputLiveV72Orchestrator } from "./run-input-field-live-v72.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V72_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v72-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV72Authorization(root);
    runInputLiveV72Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV72Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
