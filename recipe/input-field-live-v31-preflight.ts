...
import {
  ...
  verifyInputLiveV31Authorization,
  type InputLiveV31AntecedentIndex,
  type InputLiveV31AuthorizationProof,
} from "./input-field-live-v31-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V31_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v31/operator-security-attestation-template.json";
export const INPUT_LIVE_V31_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v31-security-attestation.json";

export interface InputLiveV31SecurityAttestation {
...
export interface InputLiveV31PreflightReport {
  artifactVersion: "input-live-v31-preflight-v1";
...
export const inputLiveV31AttestationSha256 = (
  value: Omit<InputLiveV31SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV31SecurityAttestation(
  value: unknown,
  proof: InputLiveV31AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v31-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V31_TARGET) ||
...
export function validateInputLiveV31ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV31Authorization");
  const preflight = source.indexOf("runInputLiveV31Preflight");
  const initialize = source.indexOf("InputLiveV31Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV31Preflight(
  root: string,
  proof: InputLiveV31AuthorizationProof,
  attestationPath = INPUT_LIVE_V31_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV31PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V31_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V31_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V31_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV31AntecedentIndex;
  failures.push(...validateInputLiveV31AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V31_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V31_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v31-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV31ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV31SecurityAttestation;
  return {
    artifactVersion: "input-live-v31-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V31_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV31Preflight(
    process.cwd(),
    verifyInputLiveV31Authorization(),
    attestationPath,
