...
import { INPUT_LIVE_V20_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v20.js";
import { verifyInputLiveV20Authorization } from "./input-field-live-v20-authorization.js";
import { runInputLiveV20Preflight } from "./input-field-live-v20-preflight.js";
import { InputLiveV20Orchestrator } from "./run-input-field-live-v20.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V20_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v20-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV20Authorization(root);
    runInputLiveV20Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV20Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
