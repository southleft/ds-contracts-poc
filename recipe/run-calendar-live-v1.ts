...
import {
  acceptCalendarLiveV1Response,
  createCalendarLiveV1Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV1Request,
  persistCalendarLiveV1RawResponse,
  persistCalendarLiveV1TechnicalGates,
  readCalendarLiveV1AcceptedRawPayload,
  type CalendarLiveV1Request,
  type CalendarLiveV1TransactionAuthorization,
} from "./calendar-live-v1-broker.js";
import {
  CALENDAR_LIVE_V1_CAPTURE_COUNT,
  assertCalendarLiveV1CaptureResponses,
  ...
  evaluateCalendarLiveV1Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV1Roots,
  validateCalendarLiveV1CaptureManifest,
  validateCalendarLiveV1CapturePayload,
  validateCalendarLiveV1CleanupPayload,
  validateCalendarLiveV1ExtractPayload,
  validateCalendarLiveV1ProbePayload,
  validateCalendarLiveV1WriterPayload,
  type CalendarLiveV1CaptureCell,
  type CalendarLiveV1CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V1_PLAN_PATH,
  type CalendarLiveV1ProofPlan,
} from "./build-calendar-live-proof-v1.js";
import {
...
export const CALENDAR_LIVE_V1_RUNNER_VERSION = "calendar-live-v1-runner-v1";

interface CalendarLiveV1JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV1OrchestratorState {
  artifactVersion: "calendar-live-v1-orchestrator-state-v1";
  ...
  journal: CalendarLiveV1JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV1ProofPlan;
  planSha256: string;
  captures: CalendarLiveV1CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV1SourceIdentity & {
...
export interface CalendarLiveV1NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV1OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v1 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V1_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V1_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV1ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV1CaptureCell[];
  validateCalendarLiveV1CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v1 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v1 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV1WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV1RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV1Request =>
  readJson<CalendarLiveV1Request>(file);

export class CalendarLiveV1Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV1Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV1OrchestratorState = {
      artifactVersion: "calendar-live-v1-orchestrator-state-v1",
    ...
    return new CalendarLiveV1Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV1OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV1Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV1NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV1NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v1 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV1NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v1 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV1RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV1Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV1CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV1AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV1WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV1Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV1ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV1Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV1Response(this.#directory, "writer", {
        validate: validateCalendarLiveV1WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV1Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV1RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV1Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV1ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV1Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV1RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v1-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV1Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV1ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV1Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV1ProbePayload(payload, ownership),
      });
      persistCalendarLiveV1TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v1 capture cell missing");
      const accepted = acceptCalendarLiveV1Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV1CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V1_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV1CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV1Objective(
    ...
    throw new Error(`Calendar live v1 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV1Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV1CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV1Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV1RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV1Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV1CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV1CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV1CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV1Authorization = (
  privateKey: KeyObject,
): CalendarLiveV1TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV1SimulationKey = (): KeyObject =>
