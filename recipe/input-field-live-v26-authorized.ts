...
import { INPUT_LIVE_V26_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v26.js";
import { verifyInputLiveV26Authorization } from "./input-field-live-v26-authorization.js";
import { runInputLiveV26Preflight } from "./input-field-live-v26-preflight.js";
import { InputLiveV26Orchestrator } from "./run-input-field-live-v26.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V26_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v26-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV26Authorization(root);
    runInputLiveV26Preflight(
      root,
      authorization,
    ...
    const orchestrator = InputLiveV26Orchestrator.initialize({
      root,
  ...
  const orchestrator = InputLiveV26Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
