...
import { verifyTableLiveV28Authorization } from "./table-live-v28-authorization.js";
import { TABLE_LIVE_V28_TARGET } from "./table-live-v28-broker.js";
import {
  TABLE_LIVE_V28_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV28Authorization();
...
const attestation: TableLiveV28SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV28SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v28 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V28_SECURITY_ATTESTATION_DEFAULT_PATH,
);
