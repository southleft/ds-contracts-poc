...
import { verifyCalendarLiveV21Authorization } from "./calendar-live-v21-authorization.js";
import { CALENDAR_LIVE_V21_TARGET } from "./calendar-live-v21-broker.js";
import {
  CALENDAR_LIVE_V21_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV21Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v21-session";
...
const attestation: CalendarLiveV21SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV21SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v21 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V21_SECURITY_ATTESTATION_DEFAULT_PATH,
);
