...
import { INPUT_LIVE_V13_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v13.js";
import { verifyInputLiveV13Authorization } from "./input-field-live-v13-authorization.js";
import { runInputLiveV13Preflight } from "./input-field-live-v13-preflight.js";
import { InputLiveV13Orchestrator } from "./run-input-field-live-v13.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V13_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v13-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV13Authorization(root);
    runInputLiveV13Preflight(
      root,
      authorization,
    ...
    const orchestrator = InputLiveV13Orchestrator.initialize({
      root,
  ...
  const orchestrator = InputLiveV13Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
