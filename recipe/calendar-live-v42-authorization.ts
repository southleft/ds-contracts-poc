...
export type CalendarLiveV42HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV42AuthorizationArtifact {
  artifactVersion: "calendar-live-v42-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V42_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV42AntecedentIndex {
...
export interface CalendarLiveV42HistoryState {
  index?: CalendarLiveV42AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV42AuthorizationArtifact;
...
export interface CalendarLiveV42AuthorizationProof
  extends CalendarLiveV42TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V42_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V42_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V42_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V42_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v42-authorization") ||
  artifactPath.includes("calendar-live-v42-preflight") ||
  artifactPath.includes("calendar-live-v42-authorized") ||
  artifactPath.includes("create-calendar-live-v42-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v42-status") ||
...
export function validateCalendarLiveV42AntecedentIndex(
  index: CalendarLiveV42AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v42-antecedent-index-v1" ||
...
export function validateCalendarLiveV42UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV42History\(\s*["']/.test(source) ||
    /readCalendarLiveV42HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV42AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV42AuthorizationArtifact {
  ...
  ) as CalendarLiveV42AntecedentIndex;
  const indexFailures = validateCalendarLiveV42AntecedentIndex(index);
...
export function validateCalendarLiveV42AuthorizationArtifact(
  artifact: CalendarLiveV42AuthorizationArtifact | undefined,
  index: CalendarLiveV42AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v42-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v42" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V42_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V42_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V42_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V42_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V42_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V42_SOURCE_ROOTS ||
  ...
  if (
    ...
    artifact?.execution?.comboboxLiveV42AuthorizationReusable !== false ||
...
export function validateCalendarLiveV42History(
  state: CalendarLiveV42HistoryState,
  expected: CalendarLiveV42HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV42AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV42AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV42HistoryState(
  root = process.cwd(),
): CalendarLiveV42HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V42_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV42AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V42_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V42_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V42_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV42History(
  expected: CalendarLiveV42HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV42HistoryState {
  const state = readCalendarLiveV42HistoryState(root);
  const failures = validateCalendarLiveV42History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v42 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV42Authorization(
  root = process.cwd(),
): CalendarLiveV42AuthorizationProof {
  const state = verifyCalendarLiveV42History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V42_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v42/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v42.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V42_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V42_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V42_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV42HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV42History(expected);
  process.stdout.write(
    `Calendar live v42 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
