...
import { buildTableLiveV3CleanupRuntime } from "./table-live-v3-cleanup.js";
import { type TableLiveV3FixedFixedPoint } from "./table-live-v3-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV3RestoreProgram,
  validateTableLiveV3RestorePayload,
  type TableLiveV3RestorePayload,
} from "./table-live-v3-restore.js";
import {
  buildTableLiveV3RawPropertyRuntime,
  normalizeTableLiveV3Scene,
  type TableLiveV3RawNode,
} from "./table-live-v3-verifier.js";
...
export const TABLE_LIVE_V3_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V3_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V3_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V3_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V3_VARIANT_COUNT = 20;
export const TABLE_LIVE_V3_SET_COUNT = 6;
export const TABLE_LIVE_V3_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V3_HOST_PHASES = 3;
export const TABLE_LIVE_V3_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V3_RESTORE_COUNT = 8;
export const TABLE_LIVE_V3_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V3_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV3SourceId = (typeof TABLE_LIVE_V3_SOURCE_IDS)[number];
export type TableLiveV3Kind = "table" | "row" | "cell";

export interface TableLiveV3SourceIdentity {
  source: TableLiveV3SourceId;
...
export interface TableLiveV3WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV3ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV3SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV3RawNode;
    rowScene: TableLiveV3RawNode;
    cellScene: TableLiveV3RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV3RootProof {
  source: TableLiveV3SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV3FixedFixedPoint;
}

export interface TableLiveV3ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV3SourceId;
    adapterIdentity: string;
...
export interface TableLiveV3CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV3SourceId;
  adapterIdentity: string;
...
export interface TableLiveV3CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV3SourceId;
  frameWidth: number;
...
export interface TableLiveV3CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v3 ${label} must be an object`);
...
export function validateTableLiveV3WriterPayload(
  value: unknown,
): TableLiveV3WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V3_NAMESPACE ||
    ...
    throw new TypeError("Table live v3 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v3 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v3 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV3SourceIdentity,
  kind: TableLiveV3Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV3ExtractProgram(
  writer: TableLiveV3WriterOwnership,
  sources: readonly TableLiveV3SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v3 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V3_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V3_NAMESPACE)}
${buildTableLiveV3RawPropertyRuntime()}
...
export function validateTableLiveV3ExtractPayload(
  value: unknown,
  writer: TableLiveV3WriterOwnership,
): TableLiveV3ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v3 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V3_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v3 extract schema/two-root mismatch");
  return payload as TableLiveV3ExtractPayload;
}

export function proveTableLiveV3Roots<Instance>(
  extract: TableLiveV3ExtractPayload,
  sources: readonly (TableLiveV3SourceIdentity & {
...
): TableLiveV3RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v3 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v3 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV3Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV3Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV3Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV3FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v3 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v3 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v3 compile lost owned sets");
...
export function assertTableLiveV3RootProofs(
  proofs: readonly TableLiveV3RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v3 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v3 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV3ProbeProgram(
  writer: TableLiveV3WriterOwnership,
  sources: readonly TableLiveV3SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V3_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
for(const row of byAdapter.values()){
 ...
 const cell0=propertyKey(rowInstance,"Cell 0"),cell0Before=cell0&&rowInstance.componentProperties[cell0].value;let textPropertiesRestored=false;if(cell0){rowInstance.setProperties({[cell0]:"Table v3 deterministic probe"});const changed=nodes(rowInstance).some(node=>node.type==="TEXT"&&node.characters==="Table v3 deterministic probe");rowInstance.setProperties({[cell0]:cell0Before});textPropertiesRestored=changed&&JSON.stringify(original)===JSON.stringify(plain(instance));}
 rowInstance.remove();
...
export function validateTableLiveV3ProbePayload(
  value: unknown,
  writer: TableLiveV3WriterOwnership,
): TableLiveV3ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV3ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V3_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V3_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Table live v3 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV3CaptureManifest(
  cells: readonly TableLiveV3CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V3_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V3_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V3_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V3_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v3 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV3CaptureCell[],
): string => {
  validateTableLiveV3CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV3CaptureProgram(
  writer: TableLiveV3WriterOwnership,
  cell: TableLiveV3CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V3_CAPTURE_COUNT
  )
    throw new TypeError("Table live v3 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v3 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
const frame=figma.createFrame();frame.name="Table v3 ephemeral / "+cell.cellKey;page.appendChild(frame);
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V3_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV3CapturePayload(
  value: unknown,
  cell: TableLiveV3CaptureCell,
  rawResponseBytes: number,
): TableLiveV3CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV3CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V3_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V3_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v3 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV3CaptureResponses(
  manifest: readonly TableLiveV3CaptureCell[],
  responses: readonly TableLiveV3CapturePayload[],
): void {
  validateTableLiveV3CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v3 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV3ObjectiveReport {
  artifactVersion: "table-live-v3-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV3Objective(
  manifest: readonly TableLiveV3CaptureCell[],
  responses: readonly TableLiveV3CapturePayload[],
): TableLiveV3ObjectiveReport {
  assertTableLiveV3CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v3-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV3CleanupProgram(
  writer: TableLiveV3WriterOwnership,
): string {
  const baseProgram = buildTableLiveV3CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V3_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v3 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV3CleanupPayload(
  payload: unknown,
  writer: TableLiveV3WriterOwnership,
): TableLiveV3CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV3CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v3 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V3_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV3WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
