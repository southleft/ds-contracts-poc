...
export type CalendarLiveV49HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV49AuthorizationArtifact {
  artifactVersion: "calendar-live-v49-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V49_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV49AntecedentIndex {
...
export interface CalendarLiveV49HistoryState {
  index?: CalendarLiveV49AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV49AuthorizationArtifact;
...
export interface CalendarLiveV49AuthorizationProof
  extends CalendarLiveV49TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V49_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V49_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V49_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V49_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v49-authorization") ||
  artifactPath.includes("calendar-live-v49-preflight") ||
  artifactPath.includes("calendar-live-v49-authorized") ||
  artifactPath.includes("create-calendar-live-v49-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v49-status") ||
...
export function validateCalendarLiveV49AntecedentIndex(
  index: CalendarLiveV49AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v49-antecedent-index-v1" ||
...
export function validateCalendarLiveV49UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV49History\(\s*["']/.test(source) ||
    /readCalendarLiveV49HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV49AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV49AuthorizationArtifact {
  ...
  ) as CalendarLiveV49AntecedentIndex;
  const indexFailures = validateCalendarLiveV49AntecedentIndex(index);
...
export function validateCalendarLiveV49AuthorizationArtifact(
  artifact: CalendarLiveV49AuthorizationArtifact | undefined,
  index: CalendarLiveV49AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v49-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v49" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V49_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V49_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V49_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V49_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V49_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V49_SOURCE_ROOTS ||
  ...
  if (
    ...
    artifact?.execution?.comboboxLiveV49AuthorizationReusable !== false ||
...
export function validateCalendarLiveV49History(
  state: CalendarLiveV49HistoryState,
  expected: CalendarLiveV49HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV49AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV49AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV49HistoryState(
  root = process.cwd(),
): CalendarLiveV49HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V49_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV49AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V49_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V49_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V49_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV49History(
  expected: CalendarLiveV49HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV49HistoryState {
  const state = readCalendarLiveV49HistoryState(root);
  const failures = validateCalendarLiveV49History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v49 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV49Authorization(
  root = process.cwd(),
): CalendarLiveV49AuthorizationProof {
  const state = verifyCalendarLiveV49History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V49_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v49/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v49.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V49_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V49_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V49_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV49HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV49History(expected);
  process.stdout.write(
    `Calendar live v49 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
