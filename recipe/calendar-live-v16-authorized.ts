...
import { CALENDAR_LIVE_V16_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v16.js";
import { verifyCalendarLiveV16Authorization } from "./calendar-live-v16-authorization.js";
import { runCalendarLiveV16Preflight } from "./calendar-live-v16-preflight.js";
import { CalendarLiveV16Orchestrator } from "./run-calendar-live-v16.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v16 requires exact authorization artifact ${CALENDAR_LIVE_V16_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v16-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV16Authorization(root);
    runCalendarLiveV16Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV16Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV16Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
