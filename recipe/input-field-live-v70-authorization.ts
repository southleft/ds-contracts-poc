...
import {
  ...
  INPUT_LIVE_V70_INDEX_PATH,
} from "./build-input-field-live-proof-v70.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV70HistoryExpectation = "pending" | "authorized";

export interface InputLiveV70AuthorizationArtifact {
  artifactVersion: "input-live-v70-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V70_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV70AntecedentIndex {
...
export interface InputLiveV70HistoryState {
  index?: InputLiveV70AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV70AuthorizationArtifact;
...
export interface InputLiveV70AuthorizationProof extends InputLiveV70TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V70_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V70_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V70_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V70_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v70-authorization") ||
  artifactPath.includes("input-field-live-v70-preflight") ||
  artifactPath.includes("input-field-live-v70-authorized") ||
  artifactPath.includes("input-field-live-pivot-v70-status") ||
...
export function validateInputLiveV70AntecedentIndex(
  index: InputLiveV70AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v70-antecedent-index-v1" ||
...
export function validateInputLiveV70UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV70History\(\s*["']/.test(source) ||
    /readInputLiveV70HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV70AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV70AuthorizationArtifact {
  ...
  ) as InputLiveV70AntecedentIndex;
  const indexFailures = validateInputLiveV70AntecedentIndex(index);
...
export function validateInputLiveV70AuthorizationArtifact(
  artifact: InputLiveV70AuthorizationArtifact | undefined,
  index: InputLiveV70AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v70-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v70" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V70_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V70_TARGET) ||
    !equalJson(
...
export function validateInputLiveV70History(
  state: InputLiveV70HistoryState,
  expected: InputLiveV70HistoryExpectation,
): string[] {
  const failures = validateInputLiveV70AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV70AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV70HistoryState(
  root = process.cwd(),
): InputLiveV70HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V70_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV70AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V70_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V70_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V70_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV70History(
  expected: InputLiveV70HistoryExpectation,
  root = process.cwd(),
): InputLiveV70HistoryState {
  const state = readInputLiveV70HistoryState(root);
  const failures = validateInputLiveV70History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV70Authorization(
  root = process.cwd(),
): InputLiveV70AuthorizationProof {
  const state = verifyInputLiveV70History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V70_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v70/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v70.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V70_TARGET,
    expectedDynamicTool: INPUT_LIVE_V70_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V70_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV70HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV70History(expected);
