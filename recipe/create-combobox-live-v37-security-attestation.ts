...
import { verifyComboboxLiveV37Authorization } from "./combobox-live-v37-authorization.js";
import { COMBOBOX_LIVE_V37_TARGET } from "./combobox-live-v37-broker.js";
import {
  COMBOBOX_LIVE_V37_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV37Authorization();
...
const attestation: ComboboxLiveV37SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV37SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v37 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V37_SECURITY_ATTESTATION_DEFAULT_PATH,
);
