...
import {
  ...
  verifyInputLiveV23Authorization,
  type InputLiveV23AntecedentIndex,
  type InputLiveV23AuthorizationProof,
} from "./input-field-live-v23-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V23_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v23/operator-security-attestation-template.json";
export const INPUT_LIVE_V23_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v23-security-attestation.json";

export interface InputLiveV23SecurityAttestation {
...
export interface InputLiveV23PreflightReport {
  artifactVersion: "input-live-v23-preflight-v1";
...
export const inputLiveV23AttestationSha256 = (
  value: Omit<InputLiveV23SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV23SecurityAttestation(
  value: unknown,
  proof: InputLiveV23AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v23-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V23_TARGET) ||
...
export function validateInputLiveV23ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV23Authorization");
  const preflight = source.indexOf("runInputLiveV23Preflight");
  const initialize = source.indexOf("InputLiveV23Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV23Preflight(
  root: string,
  proof: InputLiveV23AuthorizationProof,
  attestationPath = INPUT_LIVE_V23_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV23PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V23_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V23_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V23_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV23AntecedentIndex;
  failures.push(...validateInputLiveV23AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V23_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V23_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v23-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV23ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV23SecurityAttestation;
  return {
    artifactVersion: "input-live-v23-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V23_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV23Preflight(
    process.cwd(),
    verifyInputLiveV23Authorization(),
    attestationPath,
