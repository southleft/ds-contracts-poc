...
import {
  ...
  verifyInputLiveV21Authorization,
  type InputLiveV21AntecedentIndex,
  type InputLiveV21AuthorizationProof,
} from "./input-field-live-v21-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V21_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v21/operator-security-attestation-template.json";
export const INPUT_LIVE_V21_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v21-security-attestation.json";

export interface InputLiveV21SecurityAttestation {
...
export interface InputLiveV21PreflightReport {
  artifactVersion: "input-live-v21-preflight-v1";
...
export const inputLiveV21AttestationSha256 = (
  value: Omit<InputLiveV21SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV21SecurityAttestation(
  value: unknown,
  proof: InputLiveV21AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v21-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V21_TARGET) ||
...
export function validateInputLiveV21ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV21Authorization");
  const preflight = source.indexOf("runInputLiveV21Preflight");
  const initialize = source.indexOf("InputLiveV21Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV21Preflight(
  root: string,
  proof: InputLiveV21AuthorizationProof,
  attestationPath = INPUT_LIVE_V21_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV21PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V21_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V21_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V21_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV21AntecedentIndex;
  failures.push(...validateInputLiveV21AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V21_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V21_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v21-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV21ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV21SecurityAttestation;
  return {
    artifactVersion: "input-live-v21-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V21_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV21Preflight(
    process.cwd(),
    verifyInputLiveV21Authorization(),
    attestationPath,
