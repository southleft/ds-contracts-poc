...
export const FIGMA_PROPERTY_NORMALIZER_VERSION =
  "figma-property-normalizer-v8" as const;

export const FIGMA_PER_SIDE_STROKE_WEIGHT_FIELDS = [
  "strokeTopWeight",
  "strokeRightWeight",
  "strokeBottomWeight",
  "strokeLeftWeight",
...
export const FIGMA_UNIFORM_STROKE_WEIGHT_FIELD = "strokeWeight" as const;

export const FIGMA_STROKE_WEIGHT_CANONICAL = {
...
const FLOAT_FIELDS =
  /^(?:width\.value|height\.value|layout\.(?:itemSpacing|minWidth|minHeight|width\.value|height\.value|padding\.(?:top|right|bottom|left))|cornerRadius\.(?:topLeft|topRight|bottomRight|bottomLeft)|strokes\.\d+\.weight(?:\.(?:top|right|bottom|left))?|effects\.\d+\.(?:radius|spread)|type\.(?:fontSize|lineHeight\.value|letterSpacing\.value))$/;
...
export function normalizeFigmaBindings(
  ...
  const add = (
    ...
    if (!variable)
      throw new TypeError(`${path}: stale variable id ${parsed.id}`);
