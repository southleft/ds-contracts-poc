...
import { COMBOBOX_LIVE_V13_INDEX_PATH } from "./build-combobox-live-proof-v13.js";
import {
  COMBOBOX_LIVE_V13_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V13_TARGET,
} from "./combobox-live-v13-broker.js";
import {
  buildComboboxLiveV13AuthorizationArtifact,
  validateComboboxLiveV13AntecedentIndex,
  validateComboboxLiveV13AuthorizationArtifact,
  validateComboboxLiveV13History,
  validateComboboxLiveV13UnitTestSource,
  type ComboboxLiveV13AntecedentIndex,
  type ComboboxLiveV13AuthorizationArtifact,
  type ComboboxLiveV13AuthorizationProof,
  type ComboboxLiveV13HistoryState,
} from "./combobox-live-v13-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV13ControlFlowSource,
  validateComboboxLiveV13SecurityAttestation,
  type ComboboxLiveV13SecurityAttestation,
} from "./combobox-live-v13-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v13-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V13_INDEX_PATH);
const index = (): ComboboxLiveV13AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV13AntecedentIndex;

const artifact = (): ComboboxLiveV13AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV13AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV13HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV13HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV13AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v13/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV13AuthorizationArtifact,
): ComboboxLiveV13SecurityAttestation => {
  const body: Omit<ComboboxLiveV13SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V13_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV13History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV13History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV13History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV13History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV13History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV13UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV13UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV13AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV13AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV13AuthorizationArtifact(
        planted as ComboboxLiveV13AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV13SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV13SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV13SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV13ControlFlowSource(source), []);
});
