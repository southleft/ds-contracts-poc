...
import {
  assertComboboxLiveV1RootProofs,
  COMBOBOX_LIVE_V1_CAPTURE_COUNT,
  COMBOBOX_LIVE_V1_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  COMBOBOX_LIVE_V1_RESPONSE_CONTRACTS,
  type ComboboxLiveV1RootProof,
} from "./combobox-live-v1-contract.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V1_BROKER_VERSION = "combobox-live-v1-broker-v2";
export const COMBOBOX_LIVE_V1_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const COMBOBOX_LIVE_V1_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const COMBOBOX_LIVE_V1_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const COMBOBOX_LIVE_V1_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const COMBOBOX_LIVE_V1_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const COMBOBOX_LIVE_V1_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type ComboboxLiveV1RemotePhase =
  (typeof COMBOBOX_LIVE_V1_REMOTE_PHASES)[number];
export type ComboboxLiveV1AuthorizationMode = "live" | "simulated";

export interface ComboboxLiveV1CommitPins {
...
export interface ComboboxLiveV1TransactionAuthorization extends ComboboxLiveV1CommitPins {
  mode: ComboboxLiveV1AuthorizationMode;
...
export interface ComboboxLiveV1Manifest {
  artifactVersion: "combobox-live-v1-broker-manifest-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V1_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V1_DYNAMIC_TOOL;
  target: typeof COMBOBOX_LIVE_V1_TARGET;
  authorization: ComboboxLiveV1TransactionAuthorization;
...
export interface ComboboxLiveV1ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface ComboboxLiveV1RequestBody {
  artifactVersion: "combobox-live-v1-broker-request-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V1_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: ComboboxLiveV1RemotePhase;
  sequence: number;
  ...
  commitPins: ComboboxLiveV1CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof COMBOBOX_LIVE_V1_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: ComboboxLiveV1ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface ComboboxLiveV1Request extends ComboboxLiveV1RequestBody {
...
export interface ComboboxLiveV1Receipt {
  artifactVersion: "combobox-live-v1-broker-receipt-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V1_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: ComboboxLiveV1RemotePhase;
...
export interface ComboboxLiveV1AcceptedResponse<T = unknown> {
  request: ComboboxLiveV1Request;
  receipt: ComboboxLiveV1Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface ComboboxLiveV1IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: ComboboxLiveV1ResponseContract;
...
const equalTarget = (value: unknown): value is typeof COMBOBOX_LIVE_V1_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === COMBOBOX_LIVE_V1_TARGET.fileKey &&
  (value as BrokerTarget).fileName === COMBOBOX_LIVE_V1_TARGET.fileName &&
  (value as BrokerTarget).editorType === COMBOBOX_LIVE_V1_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== COMBOBOX_LIVE_V1_TARGET.fileKey ||
    context.fileName !== COMBOBOX_LIVE_V1_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === COMBOBOX_LIVE_V1_TARGET.editorType;
};
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(comboboxLiveV1RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const comboboxLiveV1RequestPath = (
  directory: string,
  phase: ComboboxLiveV1RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const comboboxLiveV1ResponsePath = (
  directory: string,
  phase: ComboboxLiveV1RemotePhase,
  captureIndex?: number,
): string =>
...
export const comboboxLiveV1ReceiptPath = (
  directory: string,
  phase: ComboboxLiveV1RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): ComboboxLiveV1Manifest => {
  const manifest = json<ComboboxLiveV1Manifest>(
  ...
  if (
    manifest.artifactVersion !== "combobox-live-v1-broker-manifest-v2" ||
    manifest.brokerVersion !== COMBOBOX_LIVE_V1_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== COMBOBOX_LIVE_V1_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(COMBOBOX_LIVE_V1_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: ComboboxLiveV1RemotePhase,
  captureIndex?: number,
): ComboboxLiveV1Request | undefined => {
  const file = comboboxLiveV1RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<ComboboxLiveV1Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: ComboboxLiveV1RemotePhase,
  captureIndex?: number,
): ComboboxLiveV1Receipt | undefined => {
  const file = comboboxLiveV1ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<ComboboxLiveV1Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: ComboboxLiveV1Request | undefined;
  receipt: ComboboxLiveV1Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: ComboboxLiveV1RemotePhase,
): ComboboxLiveV1ResponseContract => ({
  ...COMBOBOX_LIVE_V1_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: ComboboxLiveV1Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: COMBOBOX_LIVE_V1_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: COMBOBOX_LIVE_V1_DYNAMIC_TOOL,
      target: COMBOBOX_LIVE_V1_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "combobox-live-v1-plugin-response-v2",
  brokerVersion: COMBOBOX_LIVE_V1_BROKER_VERSION,
  target: COMBOBOX_LIVE_V1_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*COMBOBOX_LIVE_V1_PROGRAM_START*/${program}/*COMBOBOX_LIVE_V1_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*COMBOBOX_LIVE_V1_PROGRAM_START*/";
  const endMarker = "/*COMBOBOX_LIVE_V1_PROGRAM_END*/";
...
export function createComboboxLiveV1Transaction(
  directory: string,
  ...
  const manifest: ComboboxLiveV1Manifest = {
    artifactVersion: "combobox-live-v1-broker-manifest-v2",
    brokerVersion: COMBOBOX_LIVE_V1_BROKER_VERSION,
    ...
    captureRequests: COMBOBOX_LIVE_V1_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateComboboxLiveV1Request(
  directory: string,
  request: ComboboxLiveV1Request,
): void {
  ...
  const expectedSequence = comboboxLiveV1RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "combobox-live-v1-broker-request-v2" ||
    request.brokerVersion !== COMBOBOX_LIVE_V1_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== COMBOBOX_LIVE_V1_TARGET.fileKey ||
...
export function issueComboboxLiveV1Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: ComboboxLiveV1RemotePhase,
  program: string,
  options: ComboboxLiveV1IssueOptions = {},
): ComboboxLiveV1Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = comboboxLiveV1RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<ComboboxLiveV1Request>(targetPath);
    validateComboboxLiveV1Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Combobox live v1 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(comboboxLiveV1ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateComboboxLiveV1TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = comboboxLiveV1RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: COMBOBOX_LIVE_V1_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? COMBOBOX_LIVE_V1_SIGNED_WRITER_TIMEOUT_MS
        : COMBOBOX_LIVE_V1_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: ComboboxLiveV1RequestBody = {
    artifactVersion: "combobox-live-v1-broker-request-v2",
    brokerVersion: COMBOBOX_LIVE_V1_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: ComboboxLiveV1Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateComboboxLiveV1Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistComboboxLiveV1RawResponse(
  directory: string,
  phase: ComboboxLiveV1RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = comboboxLiveV1RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Combobox live v1 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<ComboboxLiveV1Request>(requestFile);
  if (
  ...
  const target = comboboxLiveV1ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): ComboboxLiveV1Receipt => {
  const body: Omit<ComboboxLiveV1Receipt, "receiptSha256"> = {
    artifactVersion: "combobox-live-v1-broker-receipt-v2",
    brokerVersion: COMBOBOX_LIVE_V1_BROKER_VERSION,
...
export function acceptComboboxLiveV1Response<T = unknown>(
  directory: string,
  phase: ComboboxLiveV1RemotePhase,
  options: {
    captureIndex?: number;
...
): ComboboxLiveV1AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = comboboxLiveV1ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Combobox live v1 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<ComboboxLiveV1Request>(
    comboboxLiveV1RequestPath(directory, phase, captureIndex),
  );
  validateComboboxLiveV1Request(directory, request);
  if (phase === "capture") validateComboboxLiveV1TechnicalGates(directory);
  const rawFile = comboboxLiveV1ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "combobox-live-v1-plugin-response-v2" ||
    plugin?.brokerVersion !== COMBOBOX_LIVE_V1_BROKER_VERSION ||
...
export function persistComboboxLiveV1TechnicalGates(
  directory: string,
  rootProofs: readonly ComboboxLiveV1RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertComboboxLiveV1RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    COMBOBOX_LIVE_V1_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "combobox-live-v1-host-technical-gates-v2",
    brokerVersion: COMBOBOX_LIVE_V1_BROKER_VERSION,
...
export function validateComboboxLiveV1TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "combobox-live-v1-host-technical-gates-v2" ||
    ...
    COMBOBOX_LIVE_V1_TECHNICAL_GATES.some(
...
export function validateComboboxLiveV1Receipt(
  directory: string,
  phase: ComboboxLiveV1RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateComboboxLiveV1Request(directory, request);
  const receipt = json<ComboboxLiveV1Receipt>(
    comboboxLiveV1ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    comboboxLiveV1ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "combobox-live-v1-broker-receipt-v2" ||
    receipt.brokerVersion !== COMBOBOX_LIVE_V1_BROKER_VERSION ||
...
export function readComboboxLiveV1AcceptedRawPayload(
  directory: string,
  phase: ComboboxLiveV1RemotePhase,
  captureIndex?: number,
): unknown {
  validateComboboxLiveV1Receipt(directory, phase, captureIndex);
...
export function createComboboxLiveV1FakeDynamicResponse(
  request: ComboboxLiveV1Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? COMBOBOX_LIVE_V1_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? COMBOBOX_LIVE_V1_TARGET,
    ...overrides.extra,
  };
}

export function listComboboxLiveV1PersistedRequests(
  directory: string,
): ComboboxLiveV1Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<ComboboxLiveV1Request>(file)] : [];
    });
}

export const comboboxLiveV1Sha256 = sha256;
