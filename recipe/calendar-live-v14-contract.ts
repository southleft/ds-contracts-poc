...
import { buildCalendarLiveV14CleanupRuntime } from "./calendar-live-v14-cleanup.js";
import { type CalendarLiveV14FixedFixedPoint } from "./calendar-live-v14-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV14RestoreProgram,
  validateCalendarLiveV14RestorePayload,
  type CalendarLiveV14RestorePayload,
} from "./calendar-live-v14-restore.js";
import {
  buildCalendarLiveV14RawPropertyRuntime,
  normalizeCalendarLiveV14Scene,
  type CalendarLiveV14RawNode,
} from "./calendar-live-v14-verifier.js";
...
export const CALENDAR_LIVE_V14_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V14_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V14_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V14_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V14_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V14_SET_COUNT = 3;
export const CALENDAR_LIVE_V14_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V14_HOST_PHASES = 3;
export const CALENDAR_LIVE_V14_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V14_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V14_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V14_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV14SourceId = (typeof CALENDAR_LIVE_V14_SOURCE_IDS)[number];
export type CalendarLiveV14Kind = "calendar" | "week" | "day";

export interface CalendarLiveV14SourceIdentity {
  source: CalendarLiveV14SourceId;
...
export interface CalendarLiveV14WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV14ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV14SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV14RawNode;
    weekScene: CalendarLiveV14RawNode;
    dayScene: CalendarLiveV14RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV14RootProof {
  source: CalendarLiveV14SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV14FixedFixedPoint;
}

export interface CalendarLiveV14ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV14SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV14CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV14SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV14CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV14SourceId;
  frameWidth: number;
...
export interface CalendarLiveV14CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v14 ${label} must be an object`);
...
export function validateCalendarLiveV14WriterPayload(
  value: unknown,
): CalendarLiveV14WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v14") ||
    ...
    result.namespace !== CALENDAR_LIVE_V14_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v14 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v14 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v14 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV14SourceIdentity,
  kind: CalendarLiveV14Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV14ExtractProgram(
  writer: CalendarLiveV14WriterOwnership,
  sources: readonly CalendarLiveV14SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v14 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V14_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V14_NAMESPACE)}
${buildCalendarLiveV14RawPropertyRuntime()}
...
export function validateCalendarLiveV14ExtractPayload(
  value: unknown,
  writer: CalendarLiveV14WriterOwnership,
): CalendarLiveV14ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v14 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V14_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v14 extract schema/one-root mismatch");
  return payload as CalendarLiveV14ExtractPayload;
}

export function proveCalendarLiveV14Roots<Instance>(
  extract: CalendarLiveV14ExtractPayload,
  sources: readonly (CalendarLiveV14SourceIdentity & {
...
): CalendarLiveV14RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v14 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v14 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV14Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV14Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV14Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV14FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v14 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v14 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v14 compile lost owned sets");
...
export function assertCalendarLiveV14RootProofs(
  proofs: readonly CalendarLiveV14RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v14 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v14 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV14ProbeProgram(
  writer: CalendarLiveV14WriterOwnership,
  sources: readonly CalendarLiveV14SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V14_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV14ProbePayload(
  value: unknown,
  writer: CalendarLiveV14WriterOwnership,
): CalendarLiveV14ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV14ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V14_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V14_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v14 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV14CaptureManifest(
  cells: readonly CalendarLiveV14CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V14_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V14_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V14_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V14_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v14 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV14CaptureCell[],
): string => {
  validateCalendarLiveV14CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV14CaptureProgram(
  writer: CalendarLiveV14WriterOwnership,
  cell: CalendarLiveV14CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V14_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v14 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v14 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V14_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV14CapturePayload(
  value: unknown,
  cell: CalendarLiveV14CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV14CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV14CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V14_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V14_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v14 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV14CaptureResponses(
  manifest: readonly CalendarLiveV14CaptureCell[],
  responses: readonly CalendarLiveV14CapturePayload[],
): void {
  validateCalendarLiveV14CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v14 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV14ObjectiveReport {
  artifactVersion: "calendar-live-v14-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV14Objective(
  manifest: readonly CalendarLiveV14CaptureCell[],
  responses: readonly CalendarLiveV14CapturePayload[],
): CalendarLiveV14ObjectiveReport {
  assertCalendarLiveV14CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v14-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV14CleanupProgram(
  writer: CalendarLiveV14WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV14CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V14_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v14 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV14CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV14WriterOwnership,
): CalendarLiveV14CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV14CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v14 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V14_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV14WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
