...
import { buildTableLiveV34CleanupRuntime } from "./table-live-v34-cleanup.js";
import { type TableLiveV34FixedFixedPoint } from "./table-live-v34-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV34RestoreProgram,
  validateTableLiveV34RestorePayload,
  type TableLiveV34RestorePayload,
} from "./table-live-v34-restore.js";
import {
  buildTableLiveV34RawPropertyRuntime,
  normalizeTableLiveV34Scene,
  type TableLiveV34RawNode,
} from "./table-live-v34-verifier.js";
...
export const TABLE_LIVE_V34_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V34_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V34_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V34_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V34_VARIANT_COUNT = 20;
export const TABLE_LIVE_V34_SET_COUNT = 6;
export const TABLE_LIVE_V34_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V34_HOST_PHASES = 3;
export const TABLE_LIVE_V34_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V34_RESTORE_COUNT = 8;
export const TABLE_LIVE_V34_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V34_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV34SourceId = (typeof TABLE_LIVE_V34_SOURCE_IDS)[number];
export type TableLiveV34Kind = "table" | "row" | "cell";

export interface TableLiveV34SourceIdentity {
  source: TableLiveV34SourceId;
...
export interface TableLiveV34WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV34ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV34SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV34RawNode;
    rowScene: TableLiveV34RawNode;
    cellScene: TableLiveV34RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV34RootProof {
  source: TableLiveV34SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV34FixedFixedPoint;
}

export interface TableLiveV34ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV34SourceId;
    adapterIdentity: string;
...
export interface TableLiveV34CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV34SourceId;
  adapterIdentity: string;
...
export interface TableLiveV34CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV34SourceId;
  frameWidth: number;
...
export interface TableLiveV34CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v34 ${label} must be an object`);
...
export function validateTableLiveV34WriterPayload(
  value: unknown,
): TableLiveV34WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V34_NAMESPACE ||
    ...
    throw new TypeError("Table live v34 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Table live v34 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v34 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV34SourceIdentity,
  kind: TableLiveV34Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV34ExtractProgram(
  writer: TableLiveV34WriterOwnership,
  sources: readonly TableLiveV34SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
      "Table live v34 extract requires two source identities",
    );
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V34_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V34_NAMESPACE)}
${buildTableLiveV34RawPropertyRuntime()}
...
export function validateTableLiveV34ExtractPayload(
  value: unknown,
  writer: TableLiveV34WriterOwnership,
): TableLiveV34ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v34 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V34_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v34 extract schema/two-root mismatch");
  return payload as TableLiveV34ExtractPayload;
}

export function proveTableLiveV34Roots<Instance>(
  extract: TableLiveV34ExtractPayload,
  sources: readonly (TableLiveV34SourceIdentity & {
...
): TableLiveV34RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v34 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v34 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV34Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV34Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV34Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV34FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Table live v34 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v34 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v34 compile lost owned sets");
...
export function assertTableLiveV34RootProofs(
  proofs: readonly TableLiveV34RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v34 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v34 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV34ProbeProgram(
  writer: TableLiveV34WriterOwnership,
  sources: readonly TableLiveV34SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V34_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV34ProbePayload(
  value: unknown,
  writer: TableLiveV34WriterOwnership,
): TableLiveV34ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV34ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V34_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v34 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV34CaptureManifest(
  cells: readonly TableLiveV34CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V34_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V34_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V34_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V34_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v34 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV34CaptureCell[],
): string => {
  validateTableLiveV34CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV34CaptureProgram(
  writer: TableLiveV34WriterOwnership,
  cell: TableLiveV34CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V34_CAPTURE_COUNT
  )
    throw new TypeError("Table live v34 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Table live v34 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V34_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV34CapturePayload(
  value: unknown,
  cell: TableLiveV34CaptureCell,
  rawResponseBytes: number,
): TableLiveV34CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV34CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V34_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V34_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v34 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV34CaptureResponses(
  manifest: readonly TableLiveV34CaptureCell[],
  responses: readonly TableLiveV34CapturePayload[],
): void {
  validateTableLiveV34CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v34 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV34ObjectiveReport {
  artifactVersion: "table-live-v34-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV34Objective(
  manifest: readonly TableLiveV34CaptureCell[],
  responses: readonly TableLiveV34CapturePayload[],
): TableLiveV34ObjectiveReport {
  assertTableLiveV34CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v34-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV34CleanupProgram(
  writer: TableLiveV34WriterOwnership,
): string {
  const baseProgram = buildTableLiveV34CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V34_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v34 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV34CleanupPayload(
  payload: unknown,
  writer: TableLiveV34WriterOwnership,
): TableLiveV34CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as TableLiveV34CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v34 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V34_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV34WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
