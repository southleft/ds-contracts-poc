...
import {
  ...
  INPUT_LIVE_V41_INDEX_PATH,
} from "./build-input-field-live-proof-v41.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV41HistoryExpectation = "pending" | "authorized";

export interface InputLiveV41AuthorizationArtifact {
  artifactVersion: "input-live-v41-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V41_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV41AntecedentIndex {
...
export interface InputLiveV41HistoryState {
  index?: InputLiveV41AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV41AuthorizationArtifact;
...
export interface InputLiveV41AuthorizationProof extends InputLiveV41TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V41_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V41_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V41_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V41_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v41-status") ||
...
export function validateInputLiveV41AntecedentIndex(
  index: InputLiveV41AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v41-antecedent-index-v1" ||
...
export function validateInputLiveV41UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV41History\(\s*["']/.test(source) ||
    /readInputLiveV41HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV41AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV41AuthorizationArtifact {
  ...
  ) as InputLiveV41AntecedentIndex;
  const indexFailures = validateInputLiveV41AntecedentIndex(index);
...
export function validateInputLiveV41AuthorizationArtifact(
  artifact: InputLiveV41AuthorizationArtifact | undefined,
  index: InputLiveV41AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v41-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v41" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V41_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V41_TARGET) ||
    !equalJson(
...
export function validateInputLiveV41History(
  state: InputLiveV41HistoryState,
  expected: InputLiveV41HistoryExpectation,
): string[] {
  const failures = validateInputLiveV41AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV41AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV41HistoryState(
  root = process.cwd(),
): InputLiveV41HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V41_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV41AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V41_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V41_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V41_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV41History(
  expected: InputLiveV41HistoryExpectation,
  root = process.cwd(),
): InputLiveV41HistoryState {
  const state = readInputLiveV41HistoryState(root);
  const failures = validateInputLiveV41History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV41Authorization(
  root = process.cwd(),
): InputLiveV41AuthorizationProof {
  const state = verifyInputLiveV41History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V41_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v41/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v41.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V41_TARGET,
    expectedDynamicTool: INPUT_LIVE_V41_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V41_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV41HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV41History(expected);
