...
export interface ComboboxLiveV1LifecycleSimulationReport {
  artifactVersion: "combobox-live-v1-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV1AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV1Lifecycle(): Promise<ComboboxLiveV1LifecycleSimulationReport> {
  await buildComboboxLiveV1Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V1_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV1AntecedentIndex;
  const indexSha256 = comboboxLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v1-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V1_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V1_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV1History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV1History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV1AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V1_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V1_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV1History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV1History(authorized, "pending");
    ...
    await buildComboboxLiveV1Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V1_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV1AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: comboboxLiveV1Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV1Lifecycle(), null, 2)}\n`,
  );
