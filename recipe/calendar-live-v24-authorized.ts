...
import { CALENDAR_LIVE_V24_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v24.js";
import { verifyCalendarLiveV24Authorization } from "./calendar-live-v24-authorization.js";
import { runCalendarLiveV24Preflight } from "./calendar-live-v24-preflight.js";
import { CalendarLiveV24Orchestrator } from "./run-calendar-live-v24.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v24 requires exact authorization artifact ${CALENDAR_LIVE_V24_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v24-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV24Authorization(root);
    runCalendarLiveV24Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV24Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV24Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
