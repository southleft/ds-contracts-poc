...
import { TABLE_LIVE_V7_AUTHORIZATION_PATH } from "./build-table-live-proof-v7.js";
import { verifyTableLiveV7Authorization } from "./table-live-v7-authorization.js";
import { runTableLiveV7Preflight } from "./table-live-v7-preflight.js";
import { TableLiveV7Orchestrator } from "./run-table-live-v7.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v7 requires exact authorization artifact ${TABLE_LIVE_V7_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v7-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV7Authorization(root);
    runTableLiveV7Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV7Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV7Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
