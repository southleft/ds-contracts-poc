...
import {
  ...
  createInputLiveV12FakeDynamicResponse,
  ...
  listInputLiveV12PersistedRequests,
  persistInputLiveV12RawResponse,
  persistInputLiveV12TechnicalGates,
  validateInputLiveV12Receipt,
  validateInputLiveV12Request,
  type InputLiveV12RemotePhase,
...
import {
  ...
  type InputLiveV12ExtractPayload,
  type InputLiveV12ProbePayload,
  type InputLiveV12RootProof,
  type InputLiveV12WriterOwnership,
} from "./input-field-live-v12-contract.js";
import {
  InputLiveV12Orchestrator,
  simulatedInputLiveV12Authorization,
} from "./run-input-field-live-v12.js";
...
const captures: InputLiveV12CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV12Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v12-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV12Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV12CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV12FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV12FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V12_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V12_TARGET);
  persistInputLiveV12RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV12Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV12Receipt(directory, phase, options.captureIndex);
...
const extractPayload = (
  ownership: InputLiveV12WriterOwnership,
): InputLiveV12ExtractPayload => ({
...
const rootProofs = (): InputLiveV12RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV12CaptureCell,
): InputLiveV12CapturePayload => {
  const bytes = Buffer.from(`input-live-v12-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV12Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV12Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV12RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) => validateInputLiveV12ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      probePayload(writer),
      {
        validate: (payload) => validateInputLiveV12ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV12CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV12CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV12CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV12Response(directory, "cleanup", {
      validate: (payload) => validateInputLiveV12CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV12PersistedRequests(directory).length, 132);
    assert.ok(inputLiveV12ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV12Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV12Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV12Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV12Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V12_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV12RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V12_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV12Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV12Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV12ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV12Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV12WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV12RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV12Request;
    validateInputLiveV12Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV12Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV12CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active extract and surfaced cleanup", () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v12-state-"));
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV12Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV12Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV12Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V12_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV12RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V12_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV12Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV12Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV12RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V12_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV12Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV12Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV12Request;
