...
export const INPUT_LIVE_V13_VERIFIER_VERSION = "input-live-v13-verifier-v1";

export function buildInputLiveV13RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV13RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV13RawNode[];
}

export interface InputLiveV13NormalizedScene {
...
export interface InputLiveV13SceneProof {
...
export function normalizeInputLiveV13Scene(
  root: InputLiveV13RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV13NormalizedScene {
  const canonicalBindings: InputLiveV13NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV13RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV13Scene<Instance>(
  normalized: InputLiveV13NormalizedScene,
...
export function compileInputLiveV13ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV13PreCaptureGates(payload: unknown): void {
