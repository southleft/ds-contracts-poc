...
import { calendarLiveV1Sha256 } from "./calendar-live-v13-broker.js";

export interface CalendarLiveV13LifecycleSimulationReport {
  artifactVersion: "calendar-live-v13-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV13AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV13Lifecycle(): Promise<CalendarLiveV13LifecycleSimulationReport> {
  await buildCalendarLiveV13Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V13_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV13AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v13-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V13_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V13_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV13History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV13History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV13AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V13_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V13_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV13History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV13History(authorized, "pending");
    ...
    await buildCalendarLiveV13Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V13_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV13AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV13Lifecycle(), null, 2)}\n`,
  );
