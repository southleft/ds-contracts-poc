...
import {
  ...
  createInputLiveV8FakeDynamicResponse,
  ...
  listInputLiveV8PersistedRequests,
  persistInputLiveV8RawResponse,
  persistInputLiveV8TechnicalGates,
  validateInputLiveV8Receipt,
  validateInputLiveV8Request,
  type InputLiveV8RemotePhase,
...
import {
  ...
  type InputLiveV8ExtractPayload,
  type InputLiveV8ProbePayload,
  type InputLiveV8RootProof,
  type InputLiveV8WriterOwnership,
} from "./input-field-live-v8-contract.js";
import {
  InputLiveV8Orchestrator,
  simulatedInputLiveV8Authorization,
} from "./run-input-field-live-v8.js";
...
const captures: InputLiveV8CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV8Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v8-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV8Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV8CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV8FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV8FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V8_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V8_TARGET);
  persistInputLiveV8RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV8Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV8Receipt(directory, phase, options.captureIndex);
...
const extractPayload = (
  ownership: InputLiveV8WriterOwnership,
...
const rootProofs = (): InputLiveV8RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v8-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV8Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV8Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV8RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) => validateInputLiveV8ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      probePayload(writer),
      {
        validate: (payload) => validateInputLiveV8ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV8CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV8CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV8CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV8Response(directory, "cleanup", {
      validate: (payload) => validateInputLiveV8CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV8PersistedRequests(directory).length, 132);
    assert.ok(inputLiveV8ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV8Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV8Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV8Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV8Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V8_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV8RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V8_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV8Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV8Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV8ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV8Receipt(third.directory, "writer"),
      /invalid Input live v8 accepted receipt/,
    );
  } finally {
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV8WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV8RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV8Request;
    validateInputLiveV8Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV8Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV8CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active extract and surfaced cleanup", () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v8-state-"));
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV8Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV8Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV8Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V8_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV8RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V8_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV8Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV8Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV8RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V8_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV8Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV8Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV8Request;
