...
export type ComboboxLiveV12HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV12AuthorizationArtifact {
  artifactVersion: "combobox-live-v12-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V12_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV12AntecedentIndex {
...
export interface ComboboxLiveV12HistoryState {
  index?: ComboboxLiveV12AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV12AuthorizationArtifact;
...
export interface ComboboxLiveV12AuthorizationProof extends ComboboxLiveV12TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V12_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V12_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V12_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V12_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v12-authorization") ||
  artifactPath.includes("combobox-live-v12-preflight") ||
  artifactPath.includes("combobox-live-v12-authorized") ||
  artifactPath.includes("create-combobox-live-v12-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v12-status") ||
...
export function validateComboboxLiveV12AntecedentIndex(
  index: ComboboxLiveV12AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v12-antecedent-index-v1" ||
...
export function validateComboboxLiveV12UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV12History\(\s*["']/.test(source) ||
    /readComboboxLiveV12HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV12AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV12AuthorizationArtifact {
  ...
  ) as ComboboxLiveV12AntecedentIndex;
  const indexFailures = validateComboboxLiveV12AntecedentIndex(index);
...
export function validateComboboxLiveV12AuthorizationArtifact(
  artifact: ComboboxLiveV12AuthorizationArtifact | undefined,
  index: ComboboxLiveV12AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v12-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v12" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V12_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V12_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V12_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V12_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V12_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V12_SOURCE_ROOTS ||
  ...
  if (
    ...
    failures.push(
      "v12 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
...
export function validateComboboxLiveV12History(
  state: ComboboxLiveV12HistoryState,
  expected: ComboboxLiveV12HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV12AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV12AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV12HistoryState(
  root = process.cwd(),
): ComboboxLiveV12HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V12_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV12AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V12_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V12_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V12_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV12History(
  expected: ComboboxLiveV12HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV12HistoryState {
  const state = readComboboxLiveV12HistoryState(root);
  const failures = validateComboboxLiveV12History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v12 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV12Authorization(
  root = process.cwd(),
): ComboboxLiveV12AuthorizationProof {
  const state = verifyComboboxLiveV12History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V12_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v12/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v12.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V12_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V12_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V12_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV12HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV12History(expected);
  process.stdout.write(
    `Combobox live v12 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
