...
export interface InputLiveV69LifecycleSimulationReport {
  artifactVersion: "input-live-v69-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV69AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV69Lifecycle(): Promise<InputLiveV69LifecycleSimulationReport> {
  await buildInputLiveV69Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V69_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV69AntecedentIndex;
  const indexSha256 = inputLiveV69Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v69-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V69_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V69_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV69History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV69History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV69AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V69_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V69_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV69History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV69History(authorized, "pending");
    ...
    await buildInputLiveV69Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V69_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV69AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV69Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV69Lifecycle(), null, 2)}\n`,
  );
