...
import { INPUT_LIVE_V17_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v17.js";
import { verifyInputLiveV17Authorization } from "./input-field-live-v17-authorization.js";
import { runInputLiveV17Preflight } from "./input-field-live-v17-preflight.js";
import { InputLiveV17Orchestrator } from "./run-input-field-live-v17.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V17_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v17-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV17Authorization(root);
    runInputLiveV17Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV17Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
