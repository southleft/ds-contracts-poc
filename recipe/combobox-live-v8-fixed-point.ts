/**
 * Carried Combobox live v8 host verifier. Uses scene-readback-combobox-v8 so live
...
export const COMBOBOX_LIVE_V8_FIXED_VERIFIER_VERSION = "combobox-live-v8-fixed-point-v1";
export const COMBOBOX_LIVE_V8_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV8FixedThresholds {
...
export interface ComboboxLiveV8FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV8FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV8FixedSourceProbe {
...
export interface ComboboxLiveV8FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV8FixedFixedPoint;
}

export interface ComboboxLiveV8FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV8FixedHardGateInput {
  thresholds: ComboboxLiveV8FixedThresholds;
  materialRegression: ComboboxLiveV8FixedMaterialRegression;
  sourceProbes: ComboboxLiveV8FixedSourceProbe[];
  cells: ComboboxLiveV8FixedCellValidation[];
  sceneProofs: ComboboxLiveV8FixedSceneProof[];
  visualRows: ComboboxLiveV8FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV8FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V8_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV8FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV8FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV8FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV8FixedHardGates(
  input: ComboboxLiveV8FixedHardGateInput,
): ComboboxLiveV8FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV8FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV8FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V8_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV8FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV8FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
