...
/**
 * Carried Input live v20 scene-readback. Keeps the v17/v18 slot-fill
 * and slot-color teachings and the v19 uniform per-side stroke-weight
 * teaching, and surfaces `layout.width.value` from the existing Figma
 * `width` / `width.value` FLOAT when a variant node's own
 * `layout.width.value` binding is absent. Does not invent variables,
 * colors, or treat FIXED as a fill.
 */
...
export const INPUT_LIVE_V20_SLOT_FILL_MARKER = "INPUT-SLOT-FILL-FROM-PAYLOAD";
export const INPUT_LIVE_V20_SLOT_COLOR_BINDING_MARKER =
  "INPUT-SLOT-COLOR-BINDING-FROM-CHILD";
export const INPUT_LIVE_V20_SURFACE_STROKE_WEIGHT_MARKER =
  "INPUT-SURFACE-STROKE-WEIGHT-FROM-PER-SIDE";
export const INPUT_LIVE_V20_VARIANT_LAYOUT_WIDTH_MARKER =
  "INPUT-VARIANT-LAYOUT-WIDTH-FROM-WIDTH";
export const INPUT_LIVE_V20_SLOT_ROLES = [
  "input-field/slot/leading",
  "input-field/slot/trailing",
] as const;
export const INPUT_LIVE_V20_SURFACE_ROLES = ["input-field/surface"] as const;
export const INPUT_LIVE_V20_VARIANT_ROLE_PREFIX = "input-field/variant/";
export const INPUT_LIVE_V20_PER_SIDE_STROKE_WEIGHT_FIELDS = [
  "strokeTopWeight",
  "strokeRightWeight",
  "strokeBottomWeight",
  "strokeLeftWeight",
...
const SURFACE_ROLES = new Set<string>(INPUT_LIVE_V20_SURFACE_ROLES);
...
const surfaceVariantLayoutWidthBinding = (
  ...
  if (
    role === undefined ||
    !role.startsWith(INPUT_LIVE_V20_VARIANT_ROLE_PREFIX)
  )
    return own;
...
const SLOT_ROLES = new Set<string>(INPUT_LIVE_V20_SLOT_ROLES);
...
export function sceneToNormalizedIr(scene: SceneNodeSnapshot): IRNode {
  const bindings = surfaceVariantLayoutWidthBinding(
    scene,
