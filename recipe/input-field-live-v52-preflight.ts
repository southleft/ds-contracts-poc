...
import {
  ...
  verifyInputLiveV52Authorization,
  type InputLiveV52AntecedentIndex,
  type InputLiveV52AuthorizationProof,
} from "./input-field-live-v52-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V52_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v52/operator-security-attestation-template.json";
export const INPUT_LIVE_V52_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v52-security-attestation.json";

export interface InputLiveV52SecurityAttestation {
...
export interface InputLiveV52PreflightReport {
  artifactVersion: "input-live-v52-preflight-v1";
...
export const inputLiveV52AttestationSha256 = (
  value: Omit<InputLiveV52SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV52SecurityAttestation(
  value: unknown,
  proof: InputLiveV52AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v52-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V52_TARGET) ||
...
export function validateInputLiveV52ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV52Authorization");
  const preflight = source.indexOf("runInputLiveV52Preflight");
  const initialize = source.indexOf("InputLiveV52Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV52Preflight(
  root: string,
  proof: InputLiveV52AuthorizationProof,
  attestationPath = INPUT_LIVE_V52_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV52PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V52_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V52_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V52_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV52AntecedentIndex;
  failures.push(...validateInputLiveV52AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V52_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V52_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v52-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV52ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV52SecurityAttestation;
  return {
    artifactVersion: "input-live-v52-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V52_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV52Preflight(
    process.cwd(),
    verifyInputLiveV52Authorization(),
    attestationPath,
