...
import {
  acceptCalendarLiveV2Response,
  createCalendarLiveV2Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV2Request,
  persistCalendarLiveV2RawResponse,
  persistCalendarLiveV2TechnicalGates,
  readCalendarLiveV2AcceptedRawPayload,
  type CalendarLiveV2Request,
  type CalendarLiveV2TransactionAuthorization,
} from "./calendar-live-v2-broker.js";
import {
  CALENDAR_LIVE_V2_CAPTURE_COUNT,
  assertCalendarLiveV2CaptureResponses,
  ...
  evaluateCalendarLiveV2Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV2Roots,
  validateCalendarLiveV2CaptureManifest,
  validateCalendarLiveV2CapturePayload,
  validateCalendarLiveV2CleanupPayload,
  validateCalendarLiveV2ExtractPayload,
  validateCalendarLiveV2ProbePayload,
  validateCalendarLiveV2WriterPayload,
  type CalendarLiveV2CaptureCell,
  type CalendarLiveV2CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V2_PLAN_PATH,
  type CalendarLiveV2ProofPlan,
} from "./build-calendar-live-proof-v2.js";
import {
...
export const CALENDAR_LIVE_V2_RUNNER_VERSION = "calendar-live-v2-runner-v1";

interface CalendarLiveV2JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV2OrchestratorState {
  artifactVersion: "calendar-live-v2-orchestrator-state-v1";
  ...
  journal: CalendarLiveV2JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV2ProofPlan;
  planSha256: string;
  captures: CalendarLiveV2CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV2SourceIdentity & {
...
export interface CalendarLiveV2NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV2OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v2 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V2_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V2_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV2ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV2CaptureCell[];
  validateCalendarLiveV2CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v2 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v2 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV2WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV2RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV2Request =>
  readJson<CalendarLiveV2Request>(file);

export class CalendarLiveV2Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV2Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV2OrchestratorState = {
      artifactVersion: "calendar-live-v2-orchestrator-state-v1",
    ...
    return new CalendarLiveV2Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV2OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV2Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV2NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV2NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v2 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV2NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v2 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV2RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV2Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV2CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV2AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV2WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV2Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV2ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV2Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV2Response(this.#directory, "writer", {
        validate: validateCalendarLiveV2WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV2Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV2RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV2Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV2ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV2Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV2RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v2-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV2Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV2ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV2Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV2ProbePayload(payload, ownership),
      });
      persistCalendarLiveV2TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v2 capture cell missing");
      const accepted = acceptCalendarLiveV2Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV2CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V2_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV2CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV2Objective(
    ...
    throw new Error(`Calendar live v2 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV2Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV2CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV2Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV2RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV2Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV2CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV2CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV2CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV2Authorization = (
  privateKey: KeyObject,
): CalendarLiveV2TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV2SimulationKey = (): KeyObject =>
