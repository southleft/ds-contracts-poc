...
export type ComboboxLiveV24HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV24AuthorizationArtifact {
  artifactVersion: "combobox-live-v24-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V24_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV24AntecedentIndex {
...
export interface ComboboxLiveV24HistoryState {
  ...
  authorization?: ComboboxLiveV24AuthorizationArtifact;
...
export interface ComboboxLiveV24AuthorizationProof extends ComboboxLiveV24TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V24_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V24_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V24_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V24_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v24-authorization") ||
  artifactPath.includes("combobox-live-v24-preflight") ||
  artifactPath.includes("combobox-live-v24-authorized") ||
  artifactPath.includes("create-combobox-live-v24-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v24-status") ||
...
export function validateComboboxLiveV24AntecedentIndex(
  index: ComboboxLiveV24AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v24-antecedent-index-v1" ||
...
export function validateComboboxLiveV24UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV24History\(\s*["']/.test(source) ||
    /readComboboxLiveV24HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV24AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV24AuthorizationArtifact {
  ...
  const indexFailures = validateComboboxLiveV24AntecedentIndex(index);
...
export function validateComboboxLiveV24AuthorizationArtifact(
  artifact: ComboboxLiveV24AuthorizationArtifact | undefined,
  index: ComboboxLiveV24AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v24-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v24" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V24_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V24_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V24_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V24_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V24_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V24_SOURCE_ROOTS ||
...
export function validateComboboxLiveV24History(
  state: ComboboxLiveV24HistoryState,
  expected: ComboboxLiveV24HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV24AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV24AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV24HistoryState(
  root = process.cwd(),
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V24_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV24AuthorizationArtifact)
    : undefined;
  ...
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V24_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V24_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV24History(
  expected: ComboboxLiveV24HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV24HistoryState {
  const state = readComboboxLiveV24HistoryState(root);
  const failures = validateComboboxLiveV24History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v24 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV24Authorization(
  root = process.cwd(),
): ComboboxLiveV24AuthorizationProof {
  const state = verifyComboboxLiveV24History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V24_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v24/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v24.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V24_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V24_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V24_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV24HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV24History(expected);
  process.stdout.write(
    `Combobox live v24 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
