/**
 * Calendar live v1 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V1_FIXED_VERIFIER_VERSION = "calendar-live-v1-fixed-point-v1";
export const CALENDAR_LIVE_V1_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV1FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV1FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV1FixedSourceProbe {
...
export interface CalendarLiveV1FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV1FixedFixedPoint;
}

export interface CalendarLiveV1FixedHardGateInput {
  thresholds: CalendarLiveV1FixedThresholds;
  sourceProbes: CalendarLiveV1FixedSourceProbe[];
  cells: CalendarLiveV1FixedCellValidation[];
  sceneProofs: CalendarLiveV1FixedSceneProof[];
...
export interface CalendarLiveV1FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V1_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV1FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV1FixedHardGates(
  input: CalendarLiveV1FixedHardGateInput,
): CalendarLiveV1FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV1FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V1_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV1FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV1FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
