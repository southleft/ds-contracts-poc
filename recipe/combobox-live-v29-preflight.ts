...
import {
  validateComboboxLiveV29AntecedentIndex,
  verifyComboboxLiveV29Authorization,
  type ComboboxLiveV29AntecedentIndex,
  type ComboboxLiveV29AuthorizationProof,
} from "./combobox-live-v29-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V29_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v29/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V29_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v29-security-attestation.json";

export interface ComboboxLiveV29SecurityAttestation {
  artifactVersion: "combobox-live-v29-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV29PreflightReport {
  artifactVersion: "combobox-live-v29-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV29SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV29SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV29AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v29-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V29_TARGET) ||
...
export function validateComboboxLiveV29ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV29Authorization");
  const preflight = source.indexOf("runComboboxLiveV29Preflight");
  const initialize = source.indexOf("ComboboxLiveV29Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV29Preflight(
  root: string,
  proof: ComboboxLiveV29AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V29_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV29PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V29_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V29_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V29_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV29AntecedentIndex;
  failures.push(...validateComboboxLiveV29AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V29_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V29_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV29SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v29-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV29ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v29 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV29SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v29-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V29_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV29Preflight(
    process.cwd(),
    verifyComboboxLiveV29Authorization(),
    attestationPath,
