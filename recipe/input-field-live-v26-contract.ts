...
} from "./input-field-live-v3-verifier-v26.js";
import {
  buildInputLiveV26RawPropertyRuntime,
  normalizeInputLiveV26Scene,
  type InputLiveV26RawNode,
} from "./input-field-live-v26-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v26.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v26.js";

export {
  buildInputLiveV26RestoreProgram,
  validateInputLiveV26RestorePayload,
  type InputLiveV26RestorePayload,
} from "./input-field-live-v26-restore.js";

export const INPUT_LIVE_V26_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V26_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V26_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V26_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V26_VARIANT_COUNT = 256;
export const INPUT_LIVE_V26_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V26_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV26SourceId = (typeof INPUT_LIVE_V26_SOURCE_IDS)[number];

export interface InputLiveV26SourceIdentity {
  source: InputLiveV26SourceId;
...
export interface InputLiveV26WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV26ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV26SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV26RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV26RootProof {
  source: InputLiveV26SourceId;
  adapterIdentity: string;
...
export interface InputLiveV26ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV26SourceId;
    adapterIdentity: string;
...
export interface InputLiveV26CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV26SourceId;
  adapterIdentity: string;
...
export interface InputLiveV26CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV26SourceId;
  strata: InputLiveV26CaptureCell["strata"];
...
export interface InputLiveV26CleanupPayload {
...
export function validateInputLiveV26WriterPayload(
  value: unknown,
): InputLiveV26WriterOwnership {
...
const identityPlan = (source: InputLiveV26SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV26ExtractProgram(
  writer: InputLiveV26WriterOwnership,
  sources: readonly InputLiveV26SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V26_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V26_NAMESPACE)}
${buildInputLiveV26RawPropertyRuntime()}
...
export function validateInputLiveV26ExtractPayload(
  value: unknown,
  writer: InputLiveV26WriterOwnership,
): InputLiveV26ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V26_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV26ExtractPayload;
}

export function proveInputLiveV26Roots<Instance>(
  extract: InputLiveV26ExtractPayload,
  sources: readonly (InputLiveV26SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV26Scene(
      root.scene,
...
export function assertInputLiveV26RootProofs(
  proofs: readonly InputLiveV26RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV26ProbeProgram(
  writer: InputLiveV26WriterOwnership,
  sources: readonly InputLiveV26SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V26_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV26ProbePayload(
  value: unknown,
  writer: InputLiveV26WriterOwnership,
): InputLiveV26ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV26ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V26_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V26_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV26CaptureManifest(
  cells: readonly InputLiveV26CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V26_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V26_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V26_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V26_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V26_SOURCE_IDS.some(
...
export const inputLiveV26CaptureManifestSha256 = (
  cells: readonly InputLiveV26CaptureCell[],
): string => {
  validateInputLiveV26CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV26CaptureProgram(
  writer: InputLiveV26WriterOwnership,
  cell: InputLiveV26CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V26_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V26_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V26_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV26CapturePayload(
  value: unknown,
  cell: InputLiveV26CaptureCell,
  rawResponseBytes: number,
): InputLiveV26CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV26CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V26_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V26_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV26CaptureResponses(
  manifest: readonly InputLiveV26CaptureCell[],
  responses: readonly InputLiveV26CapturePayload[],
): void {
  validateInputLiveV26CaptureManifest(manifest);
  if (
...
export interface InputLiveV26ObjectiveReport {
  artifactVersion: "input-live-v26-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV26Objective(
  manifest: readonly InputLiveV26CaptureCell[],
  responses: readonly InputLiveV26CapturePayload[],
  readReference: (cell: InputLiveV26CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV26CaptureCell) => Buffer,
): InputLiveV26ObjectiveReport {
  assertInputLiveV26CaptureResponses(manifest, responses);
...
export function buildInputLiveV26CleanupProgram(
  writer: InputLiveV26WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V26_NAMESPACE,
...
export function validateInputLiveV26CleanupPayload(
  value: unknown,
  writer: InputLiveV26WriterOwnership,
): InputLiveV26CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV26CleanupPayload;
...
export const INPUT_LIVE_V26_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV26WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
