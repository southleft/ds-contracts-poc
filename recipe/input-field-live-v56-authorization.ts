...
import {
  ...
  INPUT_LIVE_V56_INDEX_PATH,
} from "./build-input-field-live-proof-v56.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV56HistoryExpectation = "pending" | "authorized";

export interface InputLiveV56AuthorizationArtifact {
  artifactVersion: "input-live-v56-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V56_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV56AntecedentIndex {
...
export interface InputLiveV56HistoryState {
  index?: InputLiveV56AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV56AuthorizationArtifact;
...
export interface InputLiveV56AuthorizationProof extends InputLiveV56TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V56_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V56_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V56_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V56_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v56-authorization") ||
  artifactPath.includes("input-field-live-v56-preflight") ||
  artifactPath.includes("input-field-live-v56-authorized") ||
  artifactPath.includes("input-field-live-pivot-v56-status") ||
...
export function validateInputLiveV56AntecedentIndex(
  index: InputLiveV56AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v56-antecedent-index-v1" ||
...
export function validateInputLiveV56UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV56History\(\s*["']/.test(source) ||
    /readInputLiveV56HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV56AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV56AuthorizationArtifact {
  ...
  ) as InputLiveV56AntecedentIndex;
  const indexFailures = validateInputLiveV56AntecedentIndex(index);
...
export function validateInputLiveV56AuthorizationArtifact(
  artifact: InputLiveV56AuthorizationArtifact | undefined,
  index: InputLiveV56AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v56-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v56" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V56_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V56_TARGET) ||
    !equalJson(
...
export function validateInputLiveV56History(
  state: InputLiveV56HistoryState,
  expected: InputLiveV56HistoryExpectation,
): string[] {
  const failures = validateInputLiveV56AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV56AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV56HistoryState(
  root = process.cwd(),
): InputLiveV56HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V56_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV56AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V56_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V56_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V56_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV56History(
  expected: InputLiveV56HistoryExpectation,
  root = process.cwd(),
): InputLiveV56HistoryState {
  const state = readInputLiveV56HistoryState(root);
  const failures = validateInputLiveV56History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV56Authorization(
  root = process.cwd(),
): InputLiveV56AuthorizationProof {
  const state = verifyInputLiveV56History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V56_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v56/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v56.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V56_TARGET,
    expectedDynamicTool: INPUT_LIVE_V56_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V56_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV56HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV56History(expected);
