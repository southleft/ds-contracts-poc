...
export const INPUT_LIVE_V83_VERIFIER_VERSION = "input-live-v83-verifier-v1";

export function buildInputLiveV83RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV83RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV83RawNode[];
}

export interface InputLiveV83NormalizedScene {
...
export interface InputLiveV83SceneProof {
...
export function normalizeInputLiveV83Scene(
  root: InputLiveV83RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV83NormalizedScene {
  const canonicalBindings: InputLiveV83NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV83RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV83Scene<Instance>(
  normalized: InputLiveV83NormalizedScene,
...
export function compileInputLiveV83ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV83PreCaptureGates(payload: unknown): void {
