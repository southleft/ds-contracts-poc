...
import { TABLE_LIVE_V32_AUTHORIZATION_PATH } from "./build-table-live-proof-v32.js";
import { verifyTableLiveV32Authorization } from "./table-live-v32-authorization.js";
import { runTableLiveV32Preflight } from "./table-live-v32-preflight.js";
import { TableLiveV32Orchestrator } from "./run-table-live-v32.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v32 requires exact authorization artifact ${TABLE_LIVE_V32_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v32-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV32Authorization(root);
    runTableLiveV32Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV32Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV32Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
