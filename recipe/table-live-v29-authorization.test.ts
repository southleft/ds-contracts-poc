...
import { TABLE_LIVE_V29_INDEX_PATH } from "./build-table-live-proof-v29.js";
import {
...
import {
  ...
  validateTableLiveV29UnitTestSource,
  type TableLiveV29AntecedentIndex,
  type TableLiveV29AuthorizationArtifact,
...
import {
  ...
  validateTableLiveV29ControlFlowSource,
  validateTableLiveV29SecurityAttestation,
  type TableLiveV29SecurityAttestation,
} from "./table-live-v29-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V29_INDEX_PATH);
const index = (): TableLiveV29AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV29AntecedentIndex;

const artifact = (): TableLiveV29AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV29AuthorizationArtifact({
...
const pendingState = (): TableLiveV29HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV29HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV29AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v29/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV29AuthorizationArtifact,
): TableLiveV29SecurityAttestation => {
  const body: Omit<TableLiveV29SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V29_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV29History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV29History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV29History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV29History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV29History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV29UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v29-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV29UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV29AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV29AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV29AuthorizationArtifact(
        planted as TableLiveV29AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV29SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV29SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV29SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v29-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV29ControlFlowSource(source), []);
});
