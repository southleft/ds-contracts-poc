...
import {
  validateTableLiveV28AntecedentIndex,
  verifyTableLiveV28Authorization,
  type TableLiveV28AntecedentIndex,
  type TableLiveV28AuthorizationProof,
} from "./table-live-v28-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V28_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v28/operator-security-attestation-template.json";
export const TABLE_LIVE_V28_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v28-security-attestation.json";

export interface TableLiveV28SecurityAttestation {
  artifactVersion: "table-live-v28-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV28PreflightReport {
  artifactVersion: "table-live-v28-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV28SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV28SecurityAttestation(
  value: unknown,
  proof: TableLiveV28AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v28-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V28_TARGET) ||
...
export function validateTableLiveV28ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV28Authorization");
  const preflight = source.indexOf("runTableLiveV28Preflight");
  const initialize = source.indexOf("TableLiveV28Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV28Preflight(
  root: string,
  proof: TableLiveV28AuthorizationProof,
  attestationPath = TABLE_LIVE_V28_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV28PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V28_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V28_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV28AntecedentIndex;
  failures.push(...validateTableLiveV28AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V28_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V28_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV28SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v28-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV28ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v28 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV28SecurityAttestation;
  return {
    artifactVersion: "table-live-v28-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V28_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV28Preflight(
    process.cwd(),
    verifyTableLiveV28Authorization(),
    attestationPath,
