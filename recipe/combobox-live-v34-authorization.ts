...
export type ComboboxLiveV34HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV34AuthorizationArtifact {
  artifactVersion: "combobox-live-v34-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V34_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV34AntecedentIndex {
...
export interface ComboboxLiveV34HistoryState {
  index?: ComboboxLiveV34AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV34AuthorizationArtifact;
...
export interface ComboboxLiveV34AuthorizationProof extends ComboboxLiveV34TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V34_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V34_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V34_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V34_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v34-authorization") ||
  artifactPath.includes("combobox-live-v34-preflight") ||
  artifactPath.includes("combobox-live-v34-authorized") ||
  artifactPath.includes("create-combobox-live-v34-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v34-status") ||
...
export function validateComboboxLiveV34AntecedentIndex(
  index: ComboboxLiveV34AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v34-antecedent-index-v1" ||
...
export function validateComboboxLiveV34UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV34History\(\s*["']/.test(source) ||
    /readComboboxLiveV34HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV34AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV34AuthorizationArtifact {
  ...
  ) as ComboboxLiveV34AntecedentIndex;
  const indexFailures = validateComboboxLiveV34AntecedentIndex(index);
...
export function validateComboboxLiveV34AuthorizationArtifact(
  artifact: ComboboxLiveV34AuthorizationArtifact | undefined,
  index: ComboboxLiveV34AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v34-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v34" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V34_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V34_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V34_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V34_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V34_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V34_SOURCE_ROOTS ||
...
export function validateComboboxLiveV34History(
  state: ComboboxLiveV34HistoryState,
  expected: ComboboxLiveV34HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV34AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV34AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV34HistoryState(
  root = process.cwd(),
): ComboboxLiveV34HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V34_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV34AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V34_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V34_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V34_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV34History(
  expected: ComboboxLiveV34HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV34HistoryState {
  const state = readComboboxLiveV34HistoryState(root);
  const failures = validateComboboxLiveV34History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v34 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV34Authorization(
  root = process.cwd(),
): ComboboxLiveV34AuthorizationProof {
  const state = verifyComboboxLiveV34History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V34_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v34/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v34.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V34_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V34_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V34_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV34HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV34History(expected);
  process.stdout.write(
    `Combobox live v34 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
