...
} from "./input-field-live-v3-verifier-v29.js";
import {
  buildInputLiveV29RawPropertyRuntime,
  normalizeInputLiveV29Scene,
  type InputLiveV29RawNode,
} from "./input-field-live-v29-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v29.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v29.js";

export {
  buildInputLiveV29RestoreProgram,
  validateInputLiveV29RestorePayload,
  type InputLiveV29RestorePayload,
} from "./input-field-live-v29-restore.js";

export const INPUT_LIVE_V29_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V29_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V29_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V29_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V29_VARIANT_COUNT = 256;
export const INPUT_LIVE_V29_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V29_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV29SourceId = (typeof INPUT_LIVE_V29_SOURCE_IDS)[number];

export interface InputLiveV29SourceIdentity {
  source: InputLiveV29SourceId;
...
export interface InputLiveV29WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV29ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV29SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV29RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV29RootProof {
  source: InputLiveV29SourceId;
  adapterIdentity: string;
...
export interface InputLiveV29ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV29SourceId;
    adapterIdentity: string;
...
export interface InputLiveV29CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV29SourceId;
  adapterIdentity: string;
...
export interface InputLiveV29CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV29SourceId;
  strata: InputLiveV29CaptureCell["strata"];
...
export interface InputLiveV29CleanupPayload {
...
export function validateInputLiveV29WriterPayload(
  value: unknown,
): InputLiveV29WriterOwnership {
...
const identityPlan = (source: InputLiveV29SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV29ExtractProgram(
  writer: InputLiveV29WriterOwnership,
  sources: readonly InputLiveV29SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V29_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V29_NAMESPACE)}
${buildInputLiveV29RawPropertyRuntime()}
...
export function validateInputLiveV29ExtractPayload(
  value: unknown,
  writer: InputLiveV29WriterOwnership,
): InputLiveV29ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V29_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV29ExtractPayload;
}

export function proveInputLiveV29Roots<Instance>(
  extract: InputLiveV29ExtractPayload,
  sources: readonly (InputLiveV29SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV29Scene(
      root.scene,
...
export function assertInputLiveV29RootProofs(
  proofs: readonly InputLiveV29RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV29ProbeProgram(
  writer: InputLiveV29WriterOwnership,
  sources: readonly InputLiveV29SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V29_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV29ProbePayload(
  value: unknown,
  writer: InputLiveV29WriterOwnership,
): InputLiveV29ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV29ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V29_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V29_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV29CaptureManifest(
  cells: readonly InputLiveV29CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V29_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V29_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V29_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V29_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V29_SOURCE_IDS.some(
...
export const inputLiveV29CaptureManifestSha256 = (
  cells: readonly InputLiveV29CaptureCell[],
): string => {
  validateInputLiveV29CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV29CaptureProgram(
  writer: InputLiveV29WriterOwnership,
  cell: InputLiveV29CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V29_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V29_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V29_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV29CapturePayload(
  value: unknown,
  cell: InputLiveV29CaptureCell,
  rawResponseBytes: number,
): InputLiveV29CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV29CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V29_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V29_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV29CaptureResponses(
  manifest: readonly InputLiveV29CaptureCell[],
  responses: readonly InputLiveV29CapturePayload[],
): void {
  validateInputLiveV29CaptureManifest(manifest);
  if (
...
export interface InputLiveV29ObjectiveReport {
  artifactVersion: "input-live-v29-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV29Objective(
  manifest: readonly InputLiveV29CaptureCell[],
  responses: readonly InputLiveV29CapturePayload[],
  readReference: (cell: InputLiveV29CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV29CaptureCell) => Buffer,
): InputLiveV29ObjectiveReport {
  assertInputLiveV29CaptureResponses(manifest, responses);
...
export function buildInputLiveV29CleanupProgram(
  writer: InputLiveV29WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V29_NAMESPACE,
...
export function validateInputLiveV29CleanupPayload(
  value: unknown,
  writer: InputLiveV29WriterOwnership,
): InputLiveV29CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV29CleanupPayload;
...
export const INPUT_LIVE_V29_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV29WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
