...
import { CALENDAR_LIVE_V27_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v27.js";
import { verifyCalendarLiveV27Authorization } from "./calendar-live-v27-authorization.js";
import { runCalendarLiveV27Preflight } from "./calendar-live-v27-preflight.js";
import { CalendarLiveV27Orchestrator } from "./run-calendar-live-v27.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v27 requires exact authorization artifact ${CALENDAR_LIVE_V27_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v27-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV27Authorization(root);
    runCalendarLiveV27Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV27Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV27Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
