...
import {
  acceptComboboxLiveV16Response,
  createComboboxLiveV16Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV16Request,
  persistComboboxLiveV16RawResponse,
  persistComboboxLiveV16TechnicalGates,
  readComboboxLiveV16AcceptedRawPayload,
  type ComboboxLiveV16Request,
  type ComboboxLiveV16TransactionAuthorization,
} from "./combobox-live-v16-broker.js";
import {
  COMBOBOX_LIVE_V16_CAPTURE_COUNT,
  assertComboboxLiveV16CaptureResponses,
  ...
  evaluateComboboxLiveV16Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV16Roots,
  validateComboboxLiveV16CaptureManifest,
  validateComboboxLiveV16CapturePayload,
  validateComboboxLiveV16CleanupPayload,
  validateComboboxLiveV16ExtractPayload,
  validateComboboxLiveV16ProbePayload,
  validateComboboxLiveV16WriterPayload,
  type ComboboxLiveV16CaptureCell,
  type ComboboxLiveV16CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V16_PLAN_PATH,
  type ComboboxLiveV16ProofPlan,
} from "./build-combobox-live-proof-v16.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v16.js";

export const COMBOBOX_LIVE_V16_RUNNER_VERSION = "combobox-live-v16-runner-v1";

interface ComboboxLiveV16JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV16OrchestratorState {
  artifactVersion: "combobox-live-v16-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV16JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV16ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV16CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV16SourceIdentity & {
...
export interface ComboboxLiveV16NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV16OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v16 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V16_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V16_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV16ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV16CaptureCell[];
  validateComboboxLiveV16CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v16 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v16 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV16WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV16RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV16Request =>
  readJson<ComboboxLiveV16Request>(file);

export class ComboboxLiveV16Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV16Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV16OrchestratorState = {
      artifactVersion: "combobox-live-v16-orchestrator-state-v1",
    ...
    return new ComboboxLiveV16Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV16OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV16Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV16NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV16NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v16 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV16NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v16 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV16RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV16Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV16CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV16AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV16WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV16Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV16ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV16Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV16Response(this.#directory, "writer", {
        validate: validateComboboxLiveV16WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV16Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV16RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV16Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV16ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV16Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV16RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v16-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV16Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV16ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV16Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV16ProbePayload(payload, ownership),
      });
      persistComboboxLiveV16TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v16 capture cell missing");
      const accepted = acceptComboboxLiveV16Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV16CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V16_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV16CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV16Objective(
    ...
    throw new Error(`Combobox live v16 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV16Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV16CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV16Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV16RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV16Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV16CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV16CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV16CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV16Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV16TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV16SimulationKey = (): KeyObject =>
