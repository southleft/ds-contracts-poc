...
import { COMBOBOX_LIVE_V6_INDEX_PATH } from "./build-combobox-live-proof-v6.js";
import {
  COMBOBOX_LIVE_V6_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V6_TARGET,
} from "./combobox-live-v6-broker.js";
import {
  buildComboboxLiveV6AuthorizationArtifact,
  validateComboboxLiveV6AntecedentIndex,
  validateComboboxLiveV6AuthorizationArtifact,
  validateComboboxLiveV6History,
  validateComboboxLiveV6UnitTestSource,
  type ComboboxLiveV6AntecedentIndex,
  type ComboboxLiveV6AuthorizationArtifact,
  type ComboboxLiveV6AuthorizationProof,
  type ComboboxLiveV6HistoryState,
} from "./combobox-live-v6-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV6ControlFlowSource,
  validateComboboxLiveV6SecurityAttestation,
  type ComboboxLiveV6SecurityAttestation,
} from "./combobox-live-v6-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v6-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V6_INDEX_PATH);
const index = (): ComboboxLiveV6AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV6AntecedentIndex;

const artifact = (): ComboboxLiveV6AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV6AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV6HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV6HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV6AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v6/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV6AuthorizationArtifact,
): ComboboxLiveV6SecurityAttestation => {
  const body: Omit<ComboboxLiveV6SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V6_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV6History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV6History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV6History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV6History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV6History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV6UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV6UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV6AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV6AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV6AuthorizationArtifact(
        planted as ComboboxLiveV6AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV6SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV6SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV6SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV6ControlFlowSource(source), []);
});
