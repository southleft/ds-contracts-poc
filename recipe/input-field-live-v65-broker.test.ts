...
import {
  ...
  createInputLiveV65FakeDynamicResponse,
  ...
  listInputLiveV65PersistedRequests,
  persistInputLiveV65RawResponse,
  persistInputLiveV65TechnicalGates,
  validateInputLiveV65Receipt,
  validateInputLiveV65Request,
  type InputLiveV65RemotePhase,
...
import {
  ...
  type InputLiveV65ExtractPayload,
  type InputLiveV65ProbePayload,
...
import {
  ...
  simulatedInputLiveV65Authorization,
} from "./run-input-field-live-v65.js";
...
const captures: InputLiveV65CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV65Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v65-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV65Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV65CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV65FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV65FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V65_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V65_TARGET);
  persistInputLiveV65RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV65Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV65Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV65WriterOwnership,
...
const rootProofs = (): InputLiveV65RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v65-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV65Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV65Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV65RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV65RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV65ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV65ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV65CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV65CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV65CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV65Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV65CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV65PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV65ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV65Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV65Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV65Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV65Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V65_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV65RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V65_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV65Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV65Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV65ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV65Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV65WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV65RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV65Request;
    validateInputLiveV65Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV65Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV65CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v65-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV65Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV65Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV65Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V65_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV65RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V65_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV65Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV65Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV65RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V65_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV65Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV65Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV65Request;
