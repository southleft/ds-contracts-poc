...
export const CALENDAR_LIVE_V43_MEASURE_HUG_MARKER = "CALENDAR-TEXT-HUG-MEASURE";
export const CALENDAR_LIVE_V43_RESTORE_CONTENT_ROLES = [
  "calendar/day/label",
] as const;
export const CALENDAR_LIVE_V43_RESTORE_DAY_SET_ROLE = "calendar/day-set";
...
export interface CalendarLiveV43RestoreWriter {
  pageId: string;
  runIdentity: string;
  setIds: readonly string[];
}

export interface CalendarLiveV43RestorePayload {
  pageId: string;
  setIds: string[];
  restoredCount: 4;
  fixedBefore: number;
  ...
  marker: typeof CALENDAR_LIVE_V43_MEASURE_HUG_MARKER;
}

export function buildCalendarLiveV43RestoreProgram(
  writer: CalendarLiveV43RestoreWriter,
): string {
  if (writer.setIds.length !== 3)
    throw new TypeError("Calendar live v43 restore requires three owned sets");
  if (
...
const ROLES=new Set(${JSON.stringify([...CALENDAR_LIVE_V43_RESTORE_CONTENT_ROLES])});
const DAY_SET=${JSON.stringify(CALENDAR_LIVE_V43_RESTORE_DAY_SET_ROLE)};
const MARKER=${JSON.stringify(CALENDAR_LIVE_V43_MEASURE_HUG_MARKER)};
...
export function validateCalendarLiveV43RestorePayload(
  payload: unknown,
  writer: CalendarLiveV43RestoreWriter,
): CalendarLiveV43RestorePayload {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("Calendar live v43 restore payload must be an object");
  ...
  if (
    ...
    value.marker !== CALENDAR_LIVE_V43_MEASURE_HUG_MARKER
  )
    throw new TypeError("Calendar live v43 restore did not re-assert content HUG");
  return value as CalendarLiveV43RestorePayload;
}
