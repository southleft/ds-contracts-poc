...
import {
  validateCalendarLiveV11AntecedentIndex,
  verifyCalendarLiveV11Authorization,
  type CalendarLiveV11AntecedentIndex,
  type CalendarLiveV11AuthorizationProof,
} from "./calendar-live-v11-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V11_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v11/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V11_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v11-security-attestation.json";

export interface CalendarLiveV11SecurityAttestation {
  artifactVersion: "calendar-live-v11-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV11PreflightReport {
  artifactVersion: "calendar-live-v11-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV11SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV11SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV11AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v11-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V11_TARGET) ||
...
export function validateCalendarLiveV11ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV11Authorization");
  const preflight = source.indexOf("runCalendarLiveV11Preflight");
  const initialize = source.indexOf("CalendarLiveV11Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV11Preflight(
  root: string,
  proof: CalendarLiveV11AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V11_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV11PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V11_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V11_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV11AntecedentIndex;
  failures.push(...validateCalendarLiveV11AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V11_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V11_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV11SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v11-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV11ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v11 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV11SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v11-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V11_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV11Preflight(
    process.cwd(),
    verifyCalendarLiveV11Authorization(),
    attestationPath,
