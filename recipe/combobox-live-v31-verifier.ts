...
import {
  verifyComboboxLiveV31FixedSceneFixedPoint,
  type ComboboxLiveV31FixedFixedPoint,
} from "./combobox-live-v31-fixed-point.js";

export const COMBOBOX_LIVE_V31_VERIFIER_VERSION = "combobox-live-v31-verifier-v1";

export function buildComboboxLiveV31RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV31RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV31RawNode[];
}

export interface ComboboxLiveV31NormalizedScene {
...
export interface ComboboxLiveV31SceneProof {
...
export function normalizeComboboxLiveV31Scene(
  root: ComboboxLiveV31RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV31NormalizedScene {
  const canonicalBindings: ComboboxLiveV31NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV31RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV31Scene<Instance>(
  normalized: ComboboxLiveV31NormalizedScene,
...
): ComboboxLiveV31SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV31FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV31ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV31PreCaptureGates(payload: unknown): void {
