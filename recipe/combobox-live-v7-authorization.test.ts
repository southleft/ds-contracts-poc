...
import { COMBOBOX_LIVE_V7_INDEX_PATH } from "./build-combobox-live-proof-v7.js";
import {
  COMBOBOX_LIVE_V7_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V7_TARGET,
} from "./combobox-live-v7-broker.js";
import {
  buildComboboxLiveV7AuthorizationArtifact,
  validateComboboxLiveV7AntecedentIndex,
  validateComboboxLiveV7AuthorizationArtifact,
  validateComboboxLiveV7History,
  validateComboboxLiveV7UnitTestSource,
  type ComboboxLiveV7AntecedentIndex,
  type ComboboxLiveV7AuthorizationArtifact,
  type ComboboxLiveV7AuthorizationProof,
  type ComboboxLiveV7HistoryState,
} from "./combobox-live-v7-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV7ControlFlowSource,
  validateComboboxLiveV7SecurityAttestation,
  type ComboboxLiveV7SecurityAttestation,
} from "./combobox-live-v7-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v7-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V7_INDEX_PATH);
const index = (): ComboboxLiveV7AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV7AntecedentIndex;

const artifact = (): ComboboxLiveV7AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV7AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV7HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV7HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV7AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v7/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV7AuthorizationArtifact,
): ComboboxLiveV7SecurityAttestation => {
  const body: Omit<ComboboxLiveV7SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V7_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV7History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV7History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV7History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV7History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV7History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV7UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV7UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV7AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV7AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV7AuthorizationArtifact(
        planted as ComboboxLiveV7AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV7SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV7SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV7SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV7ControlFlowSource(source), []);
});
