...
import {
  verifyTableLiveV4FixedSceneFixedPoint,
  type TableLiveV4FixedFixedPoint,
} from "./table-live-v4-fixed-point.js";

export const TABLE_LIVE_V4_VERIFIER_VERSION = "table-live-v4-verifier-v1";

export function buildTableLiveV4RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV4RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV4RawNode[];
}

export interface TableLiveV4NormalizedScene {
...
export interface TableLiveV4SceneProof {
...
export function normalizeTableLiveV4Scene(
  root: TableLiveV4RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV4NormalizedScene {
  const canonicalBindings: TableLiveV4NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV4RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV4Scene<Instance>(
  normalized: TableLiveV4NormalizedScene,
...
): TableLiveV4SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV4FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV4ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV4PreCaptureGates(payload: unknown): void {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("table live v4 pre-capture gate payload absent");
