...
import {
  assertTableLiveV6RootProofs,
  TABLE_LIVE_V6_CAPTURE_COUNT,
  TABLE_LIVE_V6_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  TABLE_LIVE_V6_RESPONSE_CONTRACTS,
  type TableLiveV6RootProof,
} from "./table-live-v6-contract.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V6_BROKER_VERSION = "table-live-v6-broker-v1";
export const TABLE_LIVE_V6_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const TABLE_LIVE_V6_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const TABLE_LIVE_V6_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const TABLE_LIVE_V6_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const TABLE_LIVE_V6_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const TABLE_LIVE_V6_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type TableLiveV6RemotePhase =
  (typeof TABLE_LIVE_V6_REMOTE_PHASES)[number];
export type TableLiveV6AuthorizationMode = "live" | "simulated";

export interface TableLiveV6CommitPins {
...
export interface TableLiveV6TransactionAuthorization extends TableLiveV6CommitPins {
  mode: TableLiveV6AuthorizationMode;
...
export interface TableLiveV6Manifest {
  artifactVersion: "table-live-v6-broker-manifest-v2";
  brokerVersion: typeof TABLE_LIVE_V6_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof TABLE_LIVE_V6_DYNAMIC_TOOL;
  target: typeof TABLE_LIVE_V6_TARGET;
  authorization: TableLiveV6TransactionAuthorization;
...
export interface TableLiveV6ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface TableLiveV6RequestBody {
  artifactVersion: "table-live-v6-broker-request-v2";
  brokerVersion: typeof TABLE_LIVE_V6_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: TableLiveV6RemotePhase;
  sequence: number;
  ...
  commitPins: TableLiveV6CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof TABLE_LIVE_V6_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: TableLiveV6ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface TableLiveV6Request extends TableLiveV6RequestBody {
...
export interface TableLiveV6Receipt {
  artifactVersion: "table-live-v6-broker-receipt-v2";
  brokerVersion: typeof TABLE_LIVE_V6_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: TableLiveV6RemotePhase;
...
export interface TableLiveV6AcceptedResponse<T = unknown> {
  request: TableLiveV6Request;
  receipt: TableLiveV6Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface TableLiveV6IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: TableLiveV6ResponseContract;
...
const equalTarget = (value: unknown): value is typeof TABLE_LIVE_V6_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === TABLE_LIVE_V6_TARGET.fileKey &&
  (value as BrokerTarget).fileName === TABLE_LIVE_V6_TARGET.fileName &&
  (value as BrokerTarget).editorType === TABLE_LIVE_V6_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== TABLE_LIVE_V6_TARGET.fileKey ||
    context.fileName !== TABLE_LIVE_V6_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === TABLE_LIVE_V6_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Table live v6 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= TABLE_LIVE_V6_CAPTURE_COUNT
  )
    throw new TypeError("Table live v6 capture index must be 0..19");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Table live v6 authorization pins are malformed");
};

const readManifest = (directory: string): TableLiveV6Manifest => {
  const manifest = json<TableLiveV6Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Table live v6 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "table-live-v6-broker-manifest-v2" ||
    manifest.brokerVersion !== TABLE_LIVE_V6_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== TABLE_LIVE_V6_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V6_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Table live v6 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: TableLiveV6Request,
): TableLiveV6RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: TableLiveV6RemotePhase,
  captureIndex?: number,
): TableLiveV6Request | undefined => {
  const file = tableLiveV2RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV6Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: TableLiveV6RemotePhase,
  captureIndex?: number,
): TableLiveV6Receipt | undefined => {
  const file = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV6Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: TableLiveV6Request | undefined;
  receipt: TableLiveV6Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: TableLiveV6RemotePhase,
): TableLiveV6ResponseContract => ({
  ...TABLE_LIVE_V6_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: TABLE_LIVE_V6_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: TABLE_LIVE_V6_DYNAMIC_TOOL,
      target: TABLE_LIVE_V6_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "table-live-v6-plugin-response-v2",
  brokerVersion: TABLE_LIVE_V6_BROKER_VERSION,
  target: TABLE_LIVE_V6_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*TABLE_LIVE_V6_PROGRAM_START*/${program}/*TABLE_LIVE_V6_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*TABLE_LIVE_V6_PROGRAM_START*/";
  const endMarker = "/*TABLE_LIVE_V6_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Table live v6 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createTableLiveV6Transaction(
  directory: string,
  ...
  if (existsSync(path.join(directory, MANIFEST_FILE)))
    throw new Error("Table live v6 transaction already exists");
  validateAuthorization(options.authorization);
  if (
    !SHA256.test(options.proofPlanSha256) ||
    !SHA256.test(options.captureManifestSha256)
  )
    throw new TypeError("Table live v6 transaction plan hashes malformed");
  const attempt = options.attempt ?? 1;
  if (!Number.isInteger(attempt) || attempt < 1 || attempt > 3)
    throw new TypeError("Table live v6 attempt must be 1..3");
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Table live v6 private signing key does not match authorized public-key identity",
    );
  const manifest: TableLiveV6Manifest = {
    artifactVersion: "table-live-v6-broker-manifest-v2",
    brokerVersion: TABLE_LIVE_V6_BROKER_VERSION,
    ...
    captureRequests: TABLE_LIVE_V6_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateTableLiveV6Request(
  directory: string,
  request: TableLiveV6Request,
): void {
  ...
  if (
    request.artifactVersion !== "table-live-v6-broker-request-v2" ||
    request.brokerVersion !== TABLE_LIVE_V6_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== TABLE_LIVE_V6_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Table live v6 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueTableLiveV6Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: TableLiveV6RemotePhase,
  program: string,
  options: TableLiveV6IssueOptions = {},
): TableLiveV6Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<TableLiveV6Request>(targetPath);
    validateTableLiveV6Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Table live v6 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(tableLiveV2ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Table live v6 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Table live v6 empty code envelope refused");
  if (phase === "capture") validateTableLiveV6TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Table live v6 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: TABLE_LIVE_V6_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? TABLE_LIVE_V6_SIGNED_WRITER_TIMEOUT_MS
        : TABLE_LIVE_V6_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: TableLiveV6RequestBody = {
    artifactVersion: "table-live-v6-broker-request-v2",
    brokerVersion: TABLE_LIVE_V6_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: TableLiveV6Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateTableLiveV6Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistTableLiveV6RawResponse(
  directory: string,
  phase: TableLiveV6RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Table live v6 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<TableLiveV6Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Table live v6 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = tableLiveV2ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Table live v6 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<TableLiveV6Receipt, "receiptSha256"> = {
    artifactVersion: "table-live-v6-broker-receipt-v2",
    brokerVersion: TABLE_LIVE_V6_BROKER_VERSION,
...
export function acceptTableLiveV6Response<T = unknown>(
  directory: string,
  phase: TableLiveV6RemotePhase,
  options: {
    captureIndex?: number;
...
): TableLiveV6AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Table live v6 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<TableLiveV6Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Table live v6 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "table-live-v6-plugin-response-v2" ||
    plugin?.brokerVersion !== TABLE_LIVE_V6_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Table live v6 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistTableLiveV6TechnicalGates(
  directory: string,
  rootProofs: readonly TableLiveV6RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertTableLiveV6RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Table live v6 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Table live v6 probe payload hash malformed");
  const gates = Object.fromEntries(
    TABLE_LIVE_V6_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "table-live-v6-host-technical-gates-v2",
    brokerVersion: TABLE_LIVE_V6_BROKER_VERSION,
...
export function validateTableLiveV6TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "table-live-v6-host-technical-gates-v2" ||
    ...
    TABLE_LIVE_V6_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Table live v6 technical gates artifact");
}

export function validateTableLiveV6Receipt(
  directory: string,
  phase: TableLiveV6RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateTableLiveV6Request(directory, request);
  const receipt = json<TableLiveV6Receipt>(
  ...
  if (
    receipt.artifactVersion !== "table-live-v6-broker-receipt-v2" ||
    receipt.brokerVersion !== TABLE_LIVE_V6_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Table live v6 accepted receipt: ${request.requestId}`,
    );
}

export function readTableLiveV6AcceptedRawPayload(
  directory: string,
  phase: TableLiveV6RemotePhase,
  captureIndex?: number,
): unknown {
  validateTableLiveV6Receipt(directory, phase, captureIndex);
...
export function createTableLiveV6FakeDynamicResponse(
  request: TableLiveV6Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? TABLE_LIVE_V6_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? TABLE_LIVE_V6_TARGET,
    ...overrides.extra,
  };
}

export function listTableLiveV6PersistedRequests(
  directory: string,
): TableLiveV6Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<TableLiveV6Request>(file)] : [];
    });
}
