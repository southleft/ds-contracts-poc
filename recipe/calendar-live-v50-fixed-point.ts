/**
 * Calendar live v50 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V50_FIXED_VERIFIER_VERSION = "calendar-live-v50-fixed-point-v1";
export const CALENDAR_LIVE_V50_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV50FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV50FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV50FixedSourceProbe {
...
export interface CalendarLiveV50FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV50FixedFixedPoint;
}

export interface CalendarLiveV50FixedHardGateInput {
  thresholds: CalendarLiveV50FixedThresholds;
  sourceProbes: CalendarLiveV50FixedSourceProbe[];
  cells: CalendarLiveV50FixedCellValidation[];
  sceneProofs: CalendarLiveV50FixedSceneProof[];
...
export interface CalendarLiveV50FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V50_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV50FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV50FixedHardGates(
  input: CalendarLiveV50FixedHardGateInput,
): CalendarLiveV50FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV50FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V50_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV50FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV50FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV50FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
