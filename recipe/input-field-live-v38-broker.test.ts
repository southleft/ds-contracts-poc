...
import {
  ...
  createInputLiveV38FakeDynamicResponse,
  ...
  listInputLiveV38PersistedRequests,
  persistInputLiveV38RawResponse,
  persistInputLiveV38TechnicalGates,
  validateInputLiveV38Receipt,
  validateInputLiveV38Request,
  type InputLiveV38RemotePhase,
...
import {
  ...
  type InputLiveV38ExtractPayload,
  type InputLiveV38ProbePayload,
...
import {
  ...
  simulatedInputLiveV38Authorization,
} from "./run-input-field-live-v38.js";
...
const captures: InputLiveV38CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV38Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v38-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV38Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV38CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV38FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV38FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V38_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V38_TARGET);
  persistInputLiveV38RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV38Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV38Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV38WriterOwnership,
): InputLiveV38RestorePayload => ({
...
const rootProofs = (): InputLiveV38RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v38-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV38Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV38Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV38RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV38RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV38ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV38ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV38CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV38CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV38CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV38Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV38CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV38PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV38ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV38Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV38Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV38Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV38Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V38_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV38RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V38_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV38Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV38Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV38ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV38Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV38WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV38RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV38Request;
    validateInputLiveV38Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV38Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV38CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v38-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV38Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV38Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV38Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V38_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV38RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V38_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV38Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV38Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV38RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V38_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV38Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV38Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV38Request;
