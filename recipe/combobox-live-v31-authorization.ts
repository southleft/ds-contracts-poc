...
export type ComboboxLiveV31HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV31AuthorizationArtifact {
  artifactVersion: "combobox-live-v31-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V31_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV31AntecedentIndex {
...
export interface ComboboxLiveV31HistoryState {
  index?: ComboboxLiveV31AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV31AuthorizationArtifact;
...
export interface ComboboxLiveV31AuthorizationProof extends ComboboxLiveV31TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V31_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V31_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V31_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V31_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v31-authorization") ||
  artifactPath.includes("combobox-live-v31-preflight") ||
  artifactPath.includes("combobox-live-v31-authorized") ||
  artifactPath.includes("create-combobox-live-v31-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v31-status") ||
...
export function validateComboboxLiveV31AntecedentIndex(
  index: ComboboxLiveV31AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v31-antecedent-index-v1" ||
...
export function validateComboboxLiveV31UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV31History\(\s*["']/.test(source) ||
    /readComboboxLiveV31HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV31AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV31AuthorizationArtifact {
  ...
  ) as ComboboxLiveV31AntecedentIndex;
  const indexFailures = validateComboboxLiveV31AntecedentIndex(index);
...
export function validateComboboxLiveV31AuthorizationArtifact(
  artifact: ComboboxLiveV31AuthorizationArtifact | undefined,
  index: ComboboxLiveV31AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v31-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v31" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V31_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V31_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V31_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V31_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V31_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V31_SOURCE_ROOTS ||
...
export function validateComboboxLiveV31History(
  state: ComboboxLiveV31HistoryState,
  expected: ComboboxLiveV31HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV31AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV31AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV31HistoryState(
  root = process.cwd(),
): ComboboxLiveV31HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V31_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV31AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V31_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V31_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V31_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV31History(
  expected: ComboboxLiveV31HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV31HistoryState {
  const state = readComboboxLiveV31HistoryState(root);
  const failures = validateComboboxLiveV31History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v31 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV31Authorization(
  root = process.cwd(),
): ComboboxLiveV31AuthorizationProof {
  const state = verifyComboboxLiveV31History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V31_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v31/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v31.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V31_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V31_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V31_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV31HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV31History(expected);
  process.stdout.write(
    `Combobox live v31 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
