...
import {
  verifyComboboxLiveV19FixedSceneFixedPoint,
  type ComboboxLiveV19FixedFixedPoint,
} from "./combobox-live-v19-fixed-point.js";

export const COMBOBOX_LIVE_V19_VERIFIER_VERSION = "combobox-live-v19-verifier-v1";

export function buildComboboxLiveV19RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV19RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV19RawNode[];
}

export interface ComboboxLiveV19NormalizedScene {
...
export interface ComboboxLiveV19SceneProof {
...
export function normalizeComboboxLiveV19Scene(
  root: ComboboxLiveV19RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV19NormalizedScene {
  const canonicalBindings: ComboboxLiveV19NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV19RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV19Scene<Instance>(
  normalized: ComboboxLiveV19NormalizedScene,
...
): ComboboxLiveV19SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV19FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV19ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV19PreCaptureGates(payload: unknown): void {
