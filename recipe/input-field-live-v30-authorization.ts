...
import {
  ...
  INPUT_LIVE_V30_INDEX_PATH,
} from "./build-input-field-live-proof-v30.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV30HistoryExpectation = "pending" | "authorized";

export interface InputLiveV30AuthorizationArtifact {
  artifactVersion: "input-live-v30-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V30_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV30AntecedentIndex {
...
export interface InputLiveV30HistoryState {
  index?: InputLiveV30AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV30AuthorizationArtifact;
...
export interface InputLiveV30AuthorizationProof extends InputLiveV30TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V30_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V30_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V30_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V30_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v30-status") ||
...
export function validateInputLiveV30AntecedentIndex(
  index: InputLiveV30AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v30-antecedent-index-v1" ||
...
export function validateInputLiveV30UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV30History\(\s*["']/.test(source) ||
    /readInputLiveV30HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV30AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV30AuthorizationArtifact {
  ...
  ) as InputLiveV30AntecedentIndex;
  const indexFailures = validateInputLiveV30AntecedentIndex(index);
...
export function validateInputLiveV30AuthorizationArtifact(
  artifact: InputLiveV30AuthorizationArtifact | undefined,
  index: InputLiveV30AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v30-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v30" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V30_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V30_TARGET) ||
    !equalJson(
...
export function validateInputLiveV30History(
  state: InputLiveV30HistoryState,
  expected: InputLiveV30HistoryExpectation,
): string[] {
  const failures = validateInputLiveV30AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV30AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV30HistoryState(
  root = process.cwd(),
): InputLiveV30HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V30_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV30AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V30_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V30_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V30_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV30History(
  expected: InputLiveV30HistoryExpectation,
  root = process.cwd(),
): InputLiveV30HistoryState {
  const state = readInputLiveV30HistoryState(root);
  const failures = validateInputLiveV30History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV30Authorization(
  root = process.cwd(),
): InputLiveV30AuthorizationProof {
  const state = verifyInputLiveV30History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V30_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v30/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v30.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V30_TARGET,
    expectedDynamicTool: INPUT_LIVE_V30_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V30_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV30HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV30History(expected);
