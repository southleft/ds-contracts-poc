...
import { TABLE_LIVE_V33_AUTHORIZATION_PATH } from "./build-table-live-proof-v33.js";
import { verifyTableLiveV33Authorization } from "./table-live-v33-authorization.js";
import { runTableLiveV33Preflight } from "./table-live-v33-preflight.js";
import { TableLiveV33Orchestrator } from "./run-table-live-v33.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v33 requires exact authorization artifact ${TABLE_LIVE_V33_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v33-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV33Authorization(root);
    runTableLiveV33Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV33Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV33Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
