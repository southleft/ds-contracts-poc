...
import { verifyComboboxLiveV30Authorization } from "./combobox-live-v30-authorization.js";
import { COMBOBOX_LIVE_V30_TARGET } from "./combobox-live-v30-broker.js";
import {
  COMBOBOX_LIVE_V30_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV30Authorization();
...
const attestation: ComboboxLiveV30SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV30SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v30 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V30_SECURITY_ATTESTATION_DEFAULT_PATH,
);
