...
import {
  validateCalendarLiveV39AntecedentIndex,
  verifyCalendarLiveV39Authorization,
  type CalendarLiveV39AntecedentIndex,
  type CalendarLiveV39AuthorizationProof,
} from "./calendar-live-v39-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V39_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v39/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V39_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v39-security-attestation.json";

export interface CalendarLiveV39SecurityAttestation {
  artifactVersion: "calendar-live-v39-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV39PreflightReport {
  artifactVersion: "calendar-live-v39-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV39SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV39SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV39AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v39-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V39_TARGET) ||
...
export function validateCalendarLiveV39ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV39Authorization");
  const preflight = source.indexOf("runCalendarLiveV39Preflight");
  const initialize = source.indexOf("CalendarLiveV39Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV39Preflight(
  root: string,
  proof: CalendarLiveV39AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V39_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV39PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V39_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V39_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV39AntecedentIndex;
  failures.push(...validateCalendarLiveV39AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V39_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V39_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV39SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v39-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV39ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v39 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV39SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v39-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V39_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV39Preflight(
    process.cwd(),
    verifyCalendarLiveV39Authorization(),
    attestationPath,
