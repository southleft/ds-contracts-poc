...
import {
  verifyCalendarLiveV31FixedSceneFixedPoint,
  type CalendarLiveV31FixedFixedPoint,
} from "./calendar-live-v31-fixed-point.js";

export const CALENDAR_LIVE_V31_VERIFIER_VERSION = "calendar-live-v31-verifier-v1";

export function buildCalendarLiveV31RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV31RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV31RawNode[];
}

export interface CalendarLiveV31NormalizedScene {
...
export interface CalendarLiveV31SceneProof {
...
export function normalizeCalendarLiveV31Scene(
  root: CalendarLiveV31RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV31NormalizedScene {
  const canonicalBindings: CalendarLiveV31NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV31RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV31Scene<Instance>(
  normalized: CalendarLiveV31NormalizedScene,
...
): CalendarLiveV31SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV31FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV31ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV31PreCaptureGates(payload: unknown): void {
