...
import { INPUT_LIVE_V17_INDEX_PATH } from "./build-input-field-live-proof-v17.js";
import {
  INPUT_LIVE_V17_DYNAMIC_TOOL,
  INPUT_LIVE_V17_TARGET,
} from "./input-field-live-v17-broker.js";
import {
  buildInputLiveV17AuthorizationArtifact,
  validateInputLiveV17AntecedentIndex,
  validateInputLiveV17AuthorizationArtifact,
  validateInputLiveV17History,
  validateInputLiveV17UnitTestSource,
  type InputLiveV17AntecedentIndex,
  type InputLiveV17AuthorizationArtifact,
  type InputLiveV17AuthorizationProof,
  type InputLiveV17HistoryState,
} from "./input-field-live-v17-authorization.js";
import {
  inputLiveV17AttestationSha256,
  validateInputLiveV17ControlFlowSource,
  validateInputLiveV17SecurityAttestation,
  type InputLiveV17SecurityAttestation,
} from "./input-field-live-v17-preflight.js";
import { inputLiveV17Sha256 } from "./input-field-live-v17-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V17_INDEX_PATH);
const index = (): InputLiveV17AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV17AntecedentIndex;

const artifact = (): InputLiveV17AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV17AuthorizationArtifact({
...
const pendingState = (): InputLiveV17HistoryState => ({
  index: index(),
  indexSha256: inputLiveV17Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV17HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV17AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV17Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V17_TARGET,
  expectedDynamicTool: INPUT_LIVE_V17_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v17/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV17AuthorizationArtifact,
): InputLiveV17SecurityAttestation => {
  const body: Omit<InputLiveV17SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v17-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V17_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV17AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV17History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV17History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV17History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV17History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV17History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV17UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV17UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV17AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV17Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV17AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV17AuthorizationArtifact(
        planted as InputLiveV17AuthorizationArtifact,
        index(),
        inputLiveV17Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV17SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV17AttestationSha256(
      body as Omit<InputLiveV17SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV17SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV17ControlFlowSource(source), []);
});
