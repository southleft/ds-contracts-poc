...
import {
  validateCalendarLiveV1AntecedentIndex,
  verifyCalendarLiveV1Authorization,
  type CalendarLiveV1AntecedentIndex,
  type CalendarLiveV1AuthorizationProof,
} from "./calendar-live-v1-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V1_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v1/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V1_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v1-security-attestation.json";

export interface CalendarLiveV1SecurityAttestation {
  artifactVersion: "calendar-live-v1-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV1PreflightReport {
  artifactVersion: "calendar-live-v1-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV1SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV1SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV1AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v1-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V1_TARGET) ||
...
export function validateCalendarLiveV1ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV1Authorization");
  const preflight = source.indexOf("runCalendarLiveV1Preflight");
  const initialize = source.indexOf("CalendarLiveV1Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV1Preflight(
  root: string,
  proof: CalendarLiveV1AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V1_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV1PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V1_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V1_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV1AntecedentIndex;
  failures.push(...validateCalendarLiveV1AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V1_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V1_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV1SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v1-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV1ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v1 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV1SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v1-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V1_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV1Preflight(
    process.cwd(),
    verifyCalendarLiveV1Authorization(),
    attestationPath,
