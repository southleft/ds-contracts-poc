...
const EVIDENCE = "recipe/evidence/chip-live-pivot-v8";
...
const receiptOwned = {
      artifactVersion: "chip-live-pivot-v8-prepare",
      teaching:
...
if (process.argv.includes("--check")) {
  ...
  if (writer.sourcePlans.length !== 7)
    throw new Error("chip prepare requires 7 library sources (reviewed + proposed)");
  console.log(
    JSON.stringify({
      check: "ok",
      runIdentity: writer.runIdentity,
