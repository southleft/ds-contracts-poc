...
import {
  ...
  INPUT_LIVE_V38_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V38_RESPONSE_CONTRACTS,
  type InputLiveV38RootProof,
} from "./input-field-live-v38-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V38_BROKER_VERSION = "input-live-v38-broker-v2";
export const INPUT_LIVE_V38_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V38_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V38_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V38_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V38_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V38_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV38RemotePhase =
  (typeof INPUT_LIVE_V38_REMOTE_PHASES)[number];
export type InputLiveV38AuthorizationMode = "live" | "simulated";

export interface InputLiveV38CommitPins {
...
export interface InputLiveV38TransactionAuthorization extends InputLiveV38CommitPins {
  mode: InputLiveV38AuthorizationMode;
...
export interface InputLiveV38Manifest {
  artifactVersion: "input-live-v38-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V38_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V38_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V38_TARGET;
...
export interface InputLiveV38ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV38RequestBody {
  artifactVersion: "input-live-v38-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V38_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV38RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV38CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V38_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV38ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV38Request extends InputLiveV38RequestBody {
...
export interface InputLiveV38Receipt {
  artifactVersion: "input-live-v38-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V38_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV38RemotePhase;
...
export interface InputLiveV38AcceptedResponse<T = unknown> {
  request: InputLiveV38Request;
  receipt: InputLiveV38Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV38IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV38ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V38_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V38_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V38_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V38_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V38_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V38_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V38_TARGET.editorType;
};
...
export const inputLiveV38RequestSequence = (
  phase: InputLiveV38RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV38RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV38RequestPath = (
  directory: string,
  phase: InputLiveV38RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV38ResponsePath = (
  directory: string,
  phase: InputLiveV38RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV38ReceiptPath = (
  directory: string,
  phase: InputLiveV38RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV38Manifest => {
  const manifest = json<InputLiveV38Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v38-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V38_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V38_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV38RemotePhase,
  captureIndex?: number,
): InputLiveV38Request | undefined => {
  const file = inputLiveV38RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV38Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV38RemotePhase,
  captureIndex?: number,
): InputLiveV38Receipt | undefined => {
  const file = inputLiveV38ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV38Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV38Request | undefined;
  receipt: InputLiveV38Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV38RemotePhase,
): InputLiveV38ResponseContract => ({
  ...INPUT_LIVE_V38_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V38_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V38_DYNAMIC_TOOL,
      target: INPUT_LIVE_V38_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v38-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V38_BROKER_VERSION,
  target: INPUT_LIVE_V38_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V38_PROGRAM_START*/${program}/*INPUT_LIVE_V38_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V38_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V38_PROGRAM_END*/";
...
export function createInputLiveV38Transaction(
  directory: string,
  ...
  const manifest: InputLiveV38Manifest = {
    artifactVersion: "input-live-v38-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V38_BROKER_VERSION,
...
export function validateInputLiveV38Request(
  directory: string,
  request: InputLiveV38Request,
): void {
  ...
  const expectedSequence = inputLiveV38RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v38-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V38_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V38_TARGET.fileKey ||
...
export function issueInputLiveV38Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV38RemotePhase,
  program: string,
  options: InputLiveV38IssueOptions = {},
): InputLiveV38Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV38RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV38Request>(targetPath);
    validateInputLiveV38Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV38ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV38TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV38RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V38_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V38_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V38_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV38RequestBody = {
    artifactVersion: "input-live-v38-broker-request-v2",
    brokerVersion: INPUT_LIVE_V38_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV38Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV38Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV38RawResponse(
  directory: string,
  phase: InputLiveV38RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV38RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV38Request>(requestFile);
  if (
  ...
  const target = inputLiveV38ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV38Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v38-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V38_BROKER_VERSION,
...
export function acceptInputLiveV38Response<T = unknown>(
  directory: string,
  phase: InputLiveV38RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV38AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV38ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV38Request>(
    inputLiveV38RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV38Request(directory, request);
  if (phase === "capture") validateInputLiveV38TechnicalGates(directory);
  const rawFile = inputLiveV38ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v38-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V38_BROKER_VERSION ||
...
export function persistInputLiveV38TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV38RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV38RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V38_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v38-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V38_BROKER_VERSION,
...
export function validateInputLiveV38TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v38-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V38_TECHNICAL_GATES.some(
...
export function validateInputLiveV38Receipt(
  directory: string,
  phase: InputLiveV38RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV38Request(directory, request);
  const receipt = json<InputLiveV38Receipt>(
    inputLiveV38ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV38ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v38-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V38_BROKER_VERSION ||
...
export function readInputLiveV38AcceptedRawPayload(
  directory: string,
  phase: InputLiveV38RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV38Receipt(directory, phase, captureIndex);
...
export function createInputLiveV38FakeDynamicResponse(
  request: InputLiveV38Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V38_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V38_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV38PersistedRequests(
  directory: string,
): InputLiveV38Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV38Request>(file)] : [];
    });
}

export const inputLiveV38Sha256 = sha256;
