...
export const INPUT_LIVE_V12_EVIDENCE_ROOT =
  "recipe/evidence/input-field-live-pivot-v12";
export const INPUT_LIVE_V12_PROTOCOL_PATH = `${INPUT_LIVE_V12_EVIDENCE_ROOT}/protocol.json`;
export const INPUT_LIVE_V12_PLAN_PATH = `${INPUT_LIVE_V12_EVIDENCE_ROOT}/proof-plan.json`;
export const INPUT_LIVE_V12_CAPTURE_MANIFEST_PATH = `${INPUT_LIVE_V12_EVIDENCE_ROOT}/capture-manifest.json`;
export const INPUT_LIVE_V12_REQUEST_MANIFEST_PATH = `${INPUT_LIVE_V12_EVIDENCE_ROOT}/request-manifest.json`;
export const INPUT_LIVE_V12_INDEX_PATH = `${INPUT_LIVE_V12_EVIDENCE_ROOT}/antecedent-index.json`;
export const INPUT_LIVE_V12_AUTHORIZATION_TEMPLATE_PATH = `${INPUT_LIVE_V12_EVIDENCE_ROOT}/authorization-template.json`;
export const INPUT_LIVE_V12_AUTHORIZATION_PATH = `${INPUT_LIVE_V12_EVIDENCE_ROOT}/capture-authorization.json`;

const V11_PROOF_ROOT = "recipe/evidence/input-field-live-pivot-v11";
const V8_PLAN = `${V11_PROOF_ROOT}/proof-plan.json`;
const V8_CAPTURE_MANIFEST = `${V11_PROOF_ROOT}/capture-manifest.json`;
...
export interface InputLiveV12ProofPlan {
  artifactVersion: "input-live-v12-proof-plan-v1";
...
interface GeneratedSource {
  ...
  recipeHash: string;
  envelopeHash: string;
  expectedScenePlan: InputLiveV12SourceIdentity["expectedScenePlan"];
  outputPath: string;
  compressed: Buffer;
  metadata: InputLiveV12ProofPlan["sources"][number]["expectedScenePlan"];
}
...
const sourcePlans = (): GeneratedSource[] => {
  ...
  return prior.sources.map((source: Record<string, any>) => {
    ...
    return {
      ...
      outputPath: `${INPUT_LIVE_V12_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
      compressed,
      metadata: {
        path: `${INPUT_LIVE_V12_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
...
const captureCells = (): InputLiveV12CaptureCell[] => {
  const cells = json<Record<string, any>>(V8_CAPTURE_MANIFEST)
    .cells as InputLiveV12CaptureCell[];
  inputLiveV12CaptureManifestSha256(cells);
  return cells;
};

const protocol = () => ({
  artifactVersion: "input-live-v12-external-operator-protocol-v1",
  protocolId: "input-live-v12",
  status:
  ...
  hostNormalization: {
    ...
    carriedSceneReadback: "recipe/scene-readback-v12.ts",
    carriedSceneReadbackRuntime: "recipe/scene-readback-runtime-v12.ts",
    carriedV3Verifier: "recipe/input-field-live-v3-verifier-v12.ts",
...
const requestManifest = (
  captures: readonly InputLiveV12CaptureCell[],
  programs: {
    writerSha256: string;
  ...
  const requests = [
    {
      requestId: "writer",
      sequence: inputLiveV12RequestSequence("writer"),
      phase: "writer",
      predecessorRequestId: null,
      programSha256: programs.writerSha256,
      response: INPUT_LIVE_V12_RESPONSE_CONTRACTS.writer,
    },
    {
      requestId: "cleanup",
      sequence: inputLiveV12RequestSequence("cleanup"),
      phase: "cleanup",
      predecessorRequestId: "writer",
      programBuilderSha256: programs.cleanupBuilderSha256,
      response: INPUT_LIVE_V12_RESPONSE_CONTRACTS.cleanup,
      availability: "persisted immediately after writer acceptance",
    },
    {
      requestId: "extract",
      sequence: inputLiveV12RequestSequence("extract"),
      phase: "extract",
      predecessorRequestId: "writer",
      programSha256: programs.extractBlueprintSha256,
      response: INPUT_LIVE_V12_RESPONSE_CONTRACTS.extract,
    },
    {
      requestId: "probe",
      sequence: inputLiveV12RequestSequence("probe"),
      phase: "probe",
      predecessorRequestId: "extract",
      programSha256: programs.probeBlueprintSha256,
      response: INPUT_LIVE_V12_RESPONSE_CONTRACTS.probe,
    },
    ...captures.map((cell) => ({
      requestId: `capture-${String(cell.index).padStart(3, "0")}`,
      sequence: inputLiveV12RequestSequence("capture", cell.index),
      phase: "capture",
      ...
      response: INPUT_LIVE_V12_RESPONSE_CONTRACTS.capture,
    })),
  ];
  return {
    artifactVersion: "input-live-v12-request-manifest-v1",
    expectedDynamicTool: INPUT_LIVE_V12_DYNAMIC_TOOL,
    target: INPUT_LIVE_V12_TARGET,
...
const lifecycleExcludedPaths = [
  INPUT_LIVE_V12_AUTHORIZATION_TEMPLATE_PATH,
  INPUT_LIVE_V12_AUTHORIZATION_PATH,
...
const sourceNeutralDependencies = [
  ...
  "recipe/input-field-live-v12-verifier.ts",
  "recipe/figma-property-normalizer-v8.ts",
  "recipe/figma-runtime-portability.ts",
  "recipe/input-field-objective-comparison-v1.ts",
  "recipe/normalize.ts",
  "recipe/scene-readback-runtime-v12.ts",
  "recipe/scene-readback-v12.ts",
  "recipe/input-field-live-v12-writer-patch.ts",
] as const;

export async function buildInputLiveV12Proof(
  check = process.argv.includes("--check"),
): Promise<InputLiveV12ProofPlan> {
  ...
  if (
    ...
    sha256(priorWriterProgram) !== V11_WRITER_PROGRAM_SHA256 ||
    sha256(priorWriterPayload) !== V11_WRITER_PAYLOAD_SHA256
  )
    throw new Error("Input live v12 prior v11 writer bytes drifted");
  const writerPayload = Buffer.from(
    patchInputLiveV12WriterPayload(priorWriterPayload.toString("utf8")),
  );
  const writerProgram = Buffer.from(
    createWriterTransportArtifact(writerPayload).wrapper,
  );

  const writerOwnershipBlueprint: InputLiveV12WriterOwnership = {
    pageId: "__WRITER_PAGE_ID__",
    pageName: "Recipe Pivot / Input Field / v9",
    runIdentity: "input-live-v12",
  ...
  const identities = sources.map(
    (source) =>
      ...
      }) satisfies InputLiveV12SourceIdentity,
  );
  const extractBlueprint = Buffer.from(
    buildInputLiveV12ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildInputLiveV12ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildInputLiveV12CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );

  const antecedentOutputs = new Map<string, Buffer>();
  antecedentOutputs.set(INPUT_LIVE_V12_PROTOCOL_PATH, jsonBytes(protocol()));
  antecedentOutputs.set(
    `${INPUT_LIVE_V12_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V12_EVIDENCE_ROOT}/programs/writer-payload.js`,
    writerPayload,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V12_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V12_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V12_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  ...
  const captureManifest = {
    artifactVersion: "input-live-v12-capture-manifest-v1",
    ...
    cells: captures,
    cellsSha256: inputLiveV12CaptureManifestSha256(captures),
  };
  ...
  const proofPlan: InputLiveV12ProofPlan = {
    artifactVersion: "input-live-v12-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: INPUT_LIVE_V12_TARGET,
    namespace: "ds.contracts.input.recipe.v5",
    writer: {
      programPath: `${INPUT_LIVE_V12_EVIDENCE_ROOT}/programs/writer.txt`,
      programBytes: writerProgram.byteLength,
      programSha256: sha256(writerProgram),
      payloadPath: `${INPUT_LIVE_V12_EVIDENCE_ROOT}/programs/writer-payload.js`,
    ...
    captureManifest: {
      path: INPUT_LIVE_V12_CAPTURE_MANIFEST_PATH,
      cells: 128,
      requests: 128,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(INPUT_LIVE_V12_PLAN_PATH, proofPlanBytes);
  ...
  antecedentOutputs.set(
    INPUT_LIVE_V12_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/input-field-live-v12-broker.ts",
    "recipe/input-field-live-v12-contract.ts",
    "recipe/run-input-field-live-v12.ts",
    "recipe/build-input-field-live-proof-v12.ts",
    "recipe/input-field-live-v12-broker.test.ts",
    "recipe/figma-property-normalizer-v8.test.ts",
    "recipe/scene-readback-v12.test.ts",
    "recipe/input-field-live-v12-writer-patch.ts",
    "recipe/input-field-live-v12-writer-patch.test.ts",
    "recipe/input-field-live-v12-lifecycle-simulation.ts",
    ...sourceNeutralDependencies,
  ].filter((artifactPath) => artifactPath !== INPUT_LIVE_V12_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "input-live-v12-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    ...
    counts: {
      ...
      captureCells: INPUT_LIVE_V12_CAPTURE_COUNT,
  ...
  antecedentOutputs.set(INPUT_LIVE_V12_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "input-live-v12-authorization-template-v1",
    authorizationId: "input-live-v12",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: INPUT_LIVE_V12_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
  ...
  if (check) {
    ...
    mkdirSync(path.dirname(INPUT_LIVE_V12_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(INPUT_LIVE_V12_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(INPUT_LIVE_V12_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(INPUT_LIVE_V12_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildInputLiveV12Proof(), null, 2)}\n`,
  );
