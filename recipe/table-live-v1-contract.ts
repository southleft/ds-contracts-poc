...
import { buildTableLiveV1CleanupRuntime } from "./table-live-v1-cleanup.js";
import { type TableLiveV1FixedFixedPoint } from "./table-live-v1-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV1RestoreProgram,
  validateTableLiveV1RestorePayload,
  type TableLiveV1RestorePayload,
} from "./table-live-v1-restore.js";
import {
  buildTableLiveV1RawPropertyRuntime,
  normalizeTableLiveV1Scene,
  type TableLiveV1RawNode,
} from "./table-live-v1-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v1.js";
import {
...
export const TABLE_LIVE_V1_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V1_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V1_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V1_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V1_VARIANT_COUNT = 20;
export const TABLE_LIVE_V1_SET_COUNT = 6;
export const TABLE_LIVE_V1_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V1_HOST_PHASES = 3;
export const TABLE_LIVE_V1_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V1_RESTORE_COUNT = 8;
export const TABLE_LIVE_V1_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V1_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV1SourceId = (typeof TABLE_LIVE_V1_SOURCE_IDS)[number];
export type TableLiveV1Kind = "table" | "row" | "cell";

export interface TableLiveV1SourceIdentity {
  source: TableLiveV1SourceId;
...
export interface TableLiveV1WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV1ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV1SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV1RawNode;
    rowScene: TableLiveV1RawNode;
    cellScene: TableLiveV1RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV1RootProof {
  source: TableLiveV1SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV1FixedFixedPoint;
}

export interface TableLiveV1ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV1SourceId;
    adapterIdentity: string;
...
export interface TableLiveV1CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV1SourceId;
  adapterIdentity: string;
...
export interface TableLiveV1CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV1SourceId;
  frameWidth: number;
...
export interface TableLiveV1CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v1 ${label} must be an object`);
...
export function validateTableLiveV1WriterPayload(
  value: unknown,
): TableLiveV1WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V1_NAMESPACE ||
    ...
    throw new TypeError("Table live v1 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v1 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v1 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV1SourceIdentity,
  kind: TableLiveV1Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV1ExtractProgram(
  writer: TableLiveV1WriterOwnership,
  sources: readonly TableLiveV1SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v1 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V1_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V1_NAMESPACE)}
${buildTableLiveV1RawPropertyRuntime()}
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("TABLE-V1-EXTRACT-PAGE");
...
if(get(page,"pageOwner")!=="recipe/table/"+${JSON.stringify(writer.runIdentity)})throw new Error("TABLE-V1-EXTRACT-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>SET_IDS.has(node.id));
if(sets.length!==6)throw new Error("TABLE-V1-EXTRACT-ROOTS:"+sets.length);
...
for(const set of sets){
  ...
  if(!IDENTITIES[adapterIdentity]||(kind!=="table"&&kind!=="row"&&kind!=="cell"))throw new Error("TABLE-V1-EXTRACT-ADAPTER:"+adapterIdentity+":"+kind);
...
if(roots.length!==2||roots.some(root=>!root.tableScene||!root.rowScene||!root.cellScene))throw new Error("TABLE-V1-EXTRACT-TRIPLE");
...
export function validateTableLiveV1ExtractPayload(
  value: unknown,
  writer: TableLiveV1WriterOwnership,
): TableLiveV1ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v1 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V1_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v1 extract schema/two-root mismatch");
  return payload as TableLiveV1ExtractPayload;
}

export function proveTableLiveV1Roots<Instance>(
  extract: TableLiveV1ExtractPayload,
  sources: readonly (TableLiveV1SourceIdentity & {
...
): TableLiveV1RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v1 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v1 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV1Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV1Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV1Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV1FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v1 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v1 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v1 compile lost owned sets");
...
export function assertTableLiveV1RootProofs(
  proofs: readonly TableLiveV1RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v1 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v1 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV1ProbeProgram(
  writer: TableLiveV1WriterOwnership,
  sources: readonly TableLiveV1SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V1_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE"||page.id==="115:295378"||page.id==="163:35981")throw new Error("TABLE-V1-PROBE-PAGE");
...
if(sets.length!==6)throw new Error("TABLE-V1-PROBE-ROOTS:"+sets.length);
...
for(const row of byAdapter.values()){
 ...
 if(!source||!table||!rowSet||!cell||table.children.length!==2||rowSet.children.length!==4||cell.children.length!==4)throw new Error("TABLE-V1-PROBE-VARIANTS:"+adapterIdentity);
 ...
 for(const component of table.children){const target=axes(component),updates={};for(const name of axisNames){const key=propertyKey(instance,name);if(!key)throw new Error("TABLE-V1-PROBE-AXIS:"+name);updates[key]=target[name];}instance.setProperties(updates);const main=await instance.getMainComponentAsync();if(main)visited.add(main.id);}
 ...
 for(const component of rowSet.children){const target=axes(component),updates={};for(const name of ["Density","State"]){const key=propertyKey(rowInstance,name);if(!key)throw new Error("TABLE-V1-PROBE-ROW-AXIS:"+name);updates[key]=target[name];}rowInstance.setProperties(updates);const main=await rowInstance.getMainComponentAsync();if(main)rowVisited.add(main.id);}
 const cell0=propertyKey(rowInstance,"Cell 0"),cell0Before=cell0&&rowInstance.componentProperties[cell0].value;let textPropertiesRestored=false;if(cell0){rowInstance.setProperties({[cell0]:"Table v1 deterministic probe"});const changed=nodes(rowInstance).some(node=>node.type==="TEXT"&&node.characters==="Table v1 deterministic probe");rowInstance.setProperties({[cell0]:cell0Before});textPropertiesRestored=changed&&JSON.stringify(original)===JSON.stringify(plain(instance));}
 rowInstance.remove();
 const cellInstance=cell.defaultVariant.createInstance();page.appendChild(cellInstance);const cellVisited=new Set();
 for(const component of cell.children){const target=axes(component),updates={};for(const name of ["Density","Kind"]){const key=propertyKey(cellInstance,name);if(!key)throw new Error("TABLE-V1-PROBE-CELL-AXIS:"+name);updates[key]=target[name];}cellInstance.setProperties(updates);const main=await cellInstance.getMainComponentAsync();if(main)cellVisited.add(main.id);}
 cellInstance.remove();
...
export function validateTableLiveV1ProbePayload(
  value: unknown,
  writer: TableLiveV1WriterOwnership,
): TableLiveV1ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV1ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V1_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V1_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Table live v1 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV1CaptureManifest(
  cells: readonly TableLiveV1CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V1_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V1_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V1_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V1_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v1 capture manifest is malformed");
}

export const tableLiveV1CaptureManifestSha256 = (
  cells: readonly TableLiveV1CaptureCell[],
): string => {
  validateTableLiveV1CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV1CaptureProgram(
  writer: TableLiveV1WriterOwnership,
  cell: TableLiveV1CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V1_CAPTURE_COUNT
  )
    throw new TypeError("Table live v1 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v1 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
if(!page||page.type!=="PAGE"||page.id==="115:295378"||page.id==="163:35981"||!set||set.type!=="COMPONENT_SET")throw new Error("TABLE-V1-CAPTURE-TARGET");
const axes=node=>Object.fromEntries(node.name.split(",").map(part=>part.trim()).filter(part=>part.includes("=")).map(part=>{const at=part.indexOf("=");return[part.slice(0,at),part.slice(at+1)];}));
...
if(!component)throw new Error("TABLE-V1-CAPTURE-CELL:"+cell.cellKey);
const frame=figma.createFrame();frame.name="Table v1 ephemeral / "+cell.cellKey;page.appendChild(frame);
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V1_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V1-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV1CapturePayload(
  value: unknown,
  cell: TableLiveV1CaptureCell,
  rawResponseBytes: number,
): TableLiveV1CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV1CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V1_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V1_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v1 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV1CaptureResponses(
  manifest: readonly TableLiveV1CaptureCell[],
  responses: readonly TableLiveV1CapturePayload[],
): void {
  validateTableLiveV1CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v1 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV1ObjectiveReport {
  artifactVersion: "table-live-v1-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV1Objective(
  manifest: readonly TableLiveV1CaptureCell[],
  responses: readonly TableLiveV1CapturePayload[],
): TableLiveV1ObjectiveReport {
  assertTableLiveV1CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v1-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV1CleanupProgram(
  writer: TableLiveV1WriterOwnership,
): string {
  const baseProgram = buildTableLiveV1CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V1_NAMESPACE,
  ...
  const guardedResult = `if(JSON.stringify(requestedNodeIds)!==JSON.stringify([expectedPageId])||JSON.stringify([...requestedCollectionIds].sort())!==JSON.stringify([...expectedCollectionIds].sort()))throw new Error("TABLE-V1-CLEANUP-EXACT-OWNERSHIP");${resultStatement}`;
  const program = baseProgram.replace(resultStatement, guardedResult);
  if (program === baseProgram)
    throw new Error("Table live v1 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV1CleanupPayload(
  payload: unknown,
  writer: TableLiveV1WriterOwnership,
): TableLiveV1CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV1CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v1 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V1_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV1WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
