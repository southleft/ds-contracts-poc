...
import {
  ...
  INPUT_LIVE_V40_INDEX_PATH,
} from "./build-input-field-live-proof-v40.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV40HistoryExpectation = "pending" | "authorized";

export interface InputLiveV40AuthorizationArtifact {
  artifactVersion: "input-live-v40-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V40_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV40AntecedentIndex {
...
export interface InputLiveV40HistoryState {
  index?: InputLiveV40AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV40AuthorizationArtifact;
...
export interface InputLiveV40AuthorizationProof extends InputLiveV40TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V40_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V40_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V40_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V40_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v40-status") ||
...
export function validateInputLiveV40AntecedentIndex(
  index: InputLiveV40AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v40-antecedent-index-v1" ||
...
export function validateInputLiveV40UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV40History\(\s*["']/.test(source) ||
    /readInputLiveV40HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV40AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV40AuthorizationArtifact {
  ...
  ) as InputLiveV40AntecedentIndex;
  const indexFailures = validateInputLiveV40AntecedentIndex(index);
...
export function validateInputLiveV40AuthorizationArtifact(
  artifact: InputLiveV40AuthorizationArtifact | undefined,
  index: InputLiveV40AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v40-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v40" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V40_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V40_TARGET) ||
    !equalJson(
...
export function validateInputLiveV40History(
  state: InputLiveV40HistoryState,
  expected: InputLiveV40HistoryExpectation,
): string[] {
  const failures = validateInputLiveV40AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV40AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV40HistoryState(
  root = process.cwd(),
): InputLiveV40HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V40_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV40AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V40_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V40_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V40_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV40History(
  expected: InputLiveV40HistoryExpectation,
  root = process.cwd(),
): InputLiveV40HistoryState {
  const state = readInputLiveV40HistoryState(root);
  const failures = validateInputLiveV40History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV40Authorization(
  root = process.cwd(),
): InputLiveV40AuthorizationProof {
  const state = verifyInputLiveV40History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V40_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v40/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v40.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V40_TARGET,
    expectedDynamicTool: INPUT_LIVE_V40_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V40_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV40HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV40History(expected);
