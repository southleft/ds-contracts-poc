...
/**
 * Table live v6 extract runtime. Same as table v1 plus one teaching: when
 * extract walks a row COMPONENT, skip untagged writer-created
 * `table/row/owned-cell-label` TEXT nodes that exist only as host-listed
 * `characters` bind targets. They are not owned scene children. The same
 * skip applies to copies inside an owned INSTANCE so extract does not
 * invent derived ownership keys for those bind hosts.
 */
