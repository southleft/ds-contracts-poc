...
export type ComboboxLiveV26HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV26AuthorizationArtifact {
  artifactVersion: "combobox-live-v26-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V26_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV26AntecedentIndex {
...
export interface ComboboxLiveV26HistoryState {
  index?: ComboboxLiveV26AntecedentIndex;
  indexSha256: string;
...
export interface ComboboxLiveV26AuthorizationProof extends ComboboxLiveV26TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V26_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V26_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V26_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V26_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v26-authorization") ||
  artifactPath.includes("combobox-live-v26-preflight") ||
  artifactPath.includes("combobox-live-v26-authorized") ||
  artifactPath.includes("create-combobox-live-v26-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v26-status") ||
...
export function validateComboboxLiveV26AntecedentIndex(
  index: ComboboxLiveV26AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v26-antecedent-index-v1" ||
...
export function validateComboboxLiveV26UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV26History\(\s*["']/.test(source) ||
    /readComboboxLiveV26HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV26AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV26AuthorizationArtifact {
  ...
  ) as ComboboxLiveV26AntecedentIndex;
  const indexFailures = validateComboboxLiveV26AntecedentIndex(index);
...
export function validateComboboxLiveV26AuthorizationArtifact(
  artifact: ComboboxLiveV26AuthorizationArtifact | undefined,
  index: ComboboxLiveV26AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v26-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v26" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V26_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V26_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V26_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V26_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V26_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V26_SOURCE_ROOTS ||
...
export function validateComboboxLiveV26History(
  state: ComboboxLiveV26HistoryState,
  expected: ComboboxLiveV26HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV26AntecedentIndex(state.index);
...
export function readComboboxLiveV26HistoryState(
  root = process.cwd(),
): ComboboxLiveV26HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V26_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV26AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V26_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V26_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V26_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV26History(
  expected: ComboboxLiveV26HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV26HistoryState {
  const state = readComboboxLiveV26HistoryState(root);
  const failures = validateComboboxLiveV26History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v26 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV26Authorization(
  root = process.cwd(),
): ComboboxLiveV26AuthorizationProof {
  const state = verifyComboboxLiveV26History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V26_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v26/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v26.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V26_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V26_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V26_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV26HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV26History(expected);
  process.stdout.write(
    `Combobox live v26 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
