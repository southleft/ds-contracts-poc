...
import {
  validateCalendarLiveV38AntecedentIndex,
  verifyCalendarLiveV38Authorization,
  type CalendarLiveV38AntecedentIndex,
  type CalendarLiveV38AuthorizationProof,
} from "./calendar-live-v38-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V38_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v38/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V38_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v38-security-attestation.json";

export interface CalendarLiveV38SecurityAttestation {
  artifactVersion: "calendar-live-v38-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV38PreflightReport {
  artifactVersion: "calendar-live-v38-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV38SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV38SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV38AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v38-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V38_TARGET) ||
...
export function validateCalendarLiveV38ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV38Authorization");
  const preflight = source.indexOf("runCalendarLiveV38Preflight");
  const initialize = source.indexOf("CalendarLiveV38Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV38Preflight(
  root: string,
  proof: CalendarLiveV38AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V38_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV38PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V38_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V38_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV38AntecedentIndex;
  failures.push(...validateCalendarLiveV38AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V38_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V38_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV38SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v38-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV38ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v38 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV38SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v38-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V38_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV38Preflight(
    process.cwd(),
    verifyCalendarLiveV38Authorization(),
    attestationPath,
