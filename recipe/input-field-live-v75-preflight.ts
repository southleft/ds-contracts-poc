...
import {
  ...
  verifyInputLiveV75Authorization,
  type InputLiveV75AntecedentIndex,
  type InputLiveV75AuthorizationProof,
} from "./input-field-live-v75-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V75_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v75/operator-security-attestation-template.json";
export const INPUT_LIVE_V75_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v75-security-attestation.json";

export interface InputLiveV75SecurityAttestation {
...
export interface InputLiveV75PreflightReport {
  artifactVersion: "input-live-v75-preflight-v1";
...
export const inputLiveV75AttestationSha256 = (
  value: Omit<InputLiveV75SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV75SecurityAttestation(
  value: unknown,
  proof: InputLiveV75AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v75-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V75_TARGET) ||
...
export function validateInputLiveV75ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV75Authorization");
  const preflight = source.indexOf("runInputLiveV75Preflight");
  const initialize = source.indexOf("InputLiveV75Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV75Preflight(
  root: string,
  proof: InputLiveV75AuthorizationProof,
  attestationPath = INPUT_LIVE_V75_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV75PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V75_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V75_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V75_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV75AntecedentIndex;
  failures.push(...validateInputLiveV75AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V75_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V75_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v75-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV75ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV75SecurityAttestation;
  return {
    artifactVersion: "input-live-v75-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V75_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV75Preflight(
    process.cwd(),
    verifyInputLiveV75Authorization(),
    attestationPath,
