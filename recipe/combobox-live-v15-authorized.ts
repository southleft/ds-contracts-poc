...
import { COMBOBOX_LIVE_V15_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v15.js";
import { verifyComboboxLiveV15Authorization } from "./combobox-live-v15-authorization.js";
import { runComboboxLiveV15Preflight } from "./combobox-live-v15-preflight.js";
import { ComboboxLiveV15Orchestrator } from "./run-combobox-live-v15.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v15 requires exact authorization artifact ${COMBOBOX_LIVE_V15_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v15-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV15Authorization(root);
    runComboboxLiveV15Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV15Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV15Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
