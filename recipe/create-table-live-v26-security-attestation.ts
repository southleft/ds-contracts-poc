...
import { verifyTableLiveV26Authorization } from "./table-live-v26-authorization.js";
import { TABLE_LIVE_V26_TARGET } from "./table-live-v26-broker.js";
import {
  TABLE_LIVE_V26_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV26Authorization();
...
const attestation: TableLiveV26SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV26SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v26 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V26_SECURITY_ATTESTATION_DEFAULT_PATH,
);
