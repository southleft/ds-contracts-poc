...
import { verifyTableLiveV4Authorization } from "./table-live-v4-authorization.js";
import { TABLE_LIVE_V4_TARGET } from "./table-live-v4-broker.js";
import {
  TABLE_LIVE_V4_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV4Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-table-v4-session";
...
const attestation: TableLiveV4SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV4SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v4 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V4_SECURITY_ATTESTATION_DEFAULT_PATH,
);
