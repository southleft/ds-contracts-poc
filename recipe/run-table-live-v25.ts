...
import {
  acceptTableLiveV25Response,
  createTableLiveV25Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV25Request,
  persistTableLiveV25RawResponse,
  persistTableLiveV25TechnicalGates,
  readTableLiveV25AcceptedRawPayload,
  type TableLiveV25Request,
  type TableLiveV25TransactionAuthorization,
} from "./table-live-v25-broker.js";
import {
  TABLE_LIVE_V25_CAPTURE_COUNT,
  assertTableLiveV25CaptureResponses,
  ...
  evaluateTableLiveV25Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV25Roots,
  validateTableLiveV25CaptureManifest,
  validateTableLiveV25CapturePayload,
  validateTableLiveV25CleanupPayload,
  validateTableLiveV25ExtractPayload,
  validateTableLiveV25ProbePayload,
  validateTableLiveV25WriterPayload,
  type TableLiveV25CaptureCell,
  type TableLiveV25CapturePayload,
...
import {
  ...
  TABLE_LIVE_V25_PLAN_PATH,
  type TableLiveV25ProofPlan,
} from "./build-table-live-proof-v25.js";
import {
...
export const TABLE_LIVE_V25_RUNNER_VERSION = "table-live-v25-runner-v1";

interface TableLiveV25JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV25OrchestratorState {
  artifactVersion: "table-live-v25-orchestrator-state-v1";
  ...
  journal: TableLiveV25JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV25ProofPlan;
  planSha256: string;
  captures: TableLiveV25CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV25SourceIdentity & {
...
export interface TableLiveV25NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV25OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v25 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V25_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V25_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV25ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV25CaptureCell[];
  validateTableLiveV25CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v25 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v25 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV25WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV25RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV25Request =>
  readJson<TableLiveV25Request>(file);

export class TableLiveV25Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV25Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV25OrchestratorState = {
      artifactVersion: "table-live-v25-orchestrator-state-v1",
    ...
    return new TableLiveV25Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV25OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV25Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV25NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV25NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v25 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV25NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v25 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV25RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV25Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV25CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV25AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV25WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV25Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV25ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV25Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV25Response(this.#directory, "writer", {
        validate: validateTableLiveV25WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV25Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV25RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV25Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV25ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV25Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV25RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v25-host-root-proofs-v1",
      ...
      const next = issueTableLiveV25Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV25ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV25Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV25ProbePayload(payload, ownership),
      });
      persistTableLiveV25TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v25 capture cell missing");
      const accepted = acceptTableLiveV25Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV25CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V25_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV25CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV25Objective(
    ...
    throw new Error(`Table live v25 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV25Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV25CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV25Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV25RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV25Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV25CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV25CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV25CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV25Authorization = (
  privateKey: KeyObject,
): TableLiveV25TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV25SimulationKey = (): KeyObject =>
