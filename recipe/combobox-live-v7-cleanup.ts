...
export interface ComboboxLiveV7CleanupPlan {
  fileKey: string;
  fileName: string;
  editorType: string;
  namespace: string;
...
export function buildComboboxLiveV7CleanupRuntime(
  plan: ComboboxLiveV7CleanupPlan,
): string {
  ...
  return String.raw`
if(figma.fileKey!==${JSON.stringify(plan.fileKey)}||figma.root.name!==${JSON.stringify(plan.fileName)}||figma.editorType!==${JSON.stringify(plan.editorType)})throw new Error("COMBOBOX-V7-CLEANUP-WRONG-TARGET");
