...
import {
  acceptCalendarLiveV25Response,
  createCalendarLiveV25Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV25Request,
  persistCalendarLiveV25RawResponse,
  persistCalendarLiveV25TechnicalGates,
  readCalendarLiveV25AcceptedRawPayload,
  type CalendarLiveV25Request,
  type CalendarLiveV25TransactionAuthorization,
} from "./calendar-live-v25-broker.js";
import {
  CALENDAR_LIVE_V25_CAPTURE_COUNT,
  assertCalendarLiveV25CaptureResponses,
  ...
  evaluateCalendarLiveV25Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV25Roots,
  validateCalendarLiveV25CaptureManifest,
  validateCalendarLiveV25CapturePayload,
  validateCalendarLiveV25CleanupPayload,
  validateCalendarLiveV25ExtractPayload,
  validateCalendarLiveV25ProbePayload,
  validateCalendarLiveV25WriterPayload,
  type CalendarLiveV25CaptureCell,
  type CalendarLiveV25CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V25_PLAN_PATH,
  type CalendarLiveV25ProofPlan,
} from "./build-calendar-live-proof-v25.js";
import {
...
export const CALENDAR_LIVE_V25_RUNNER_VERSION = "calendar-live-v25-runner-v1";

interface CalendarLiveV25JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV25OrchestratorState {
  artifactVersion: "calendar-live-v25-orchestrator-state-v1";
  ...
  journal: CalendarLiveV25JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV25ProofPlan;
  planSha256: string;
  captures: CalendarLiveV25CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV25SourceIdentity & {
...
export interface CalendarLiveV25NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV25OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v25 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V25_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V25_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV25ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV25CaptureCell[];
  validateCalendarLiveV25CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v25 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v25 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV25WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV25RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV25Request =>
  readJson<CalendarLiveV25Request>(file);

export class CalendarLiveV25Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV25Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV25OrchestratorState = {
      artifactVersion: "calendar-live-v25-orchestrator-state-v1",
    ...
    return new CalendarLiveV25Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV25OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV25Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV25NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV25NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v25 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV25NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v25 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV25RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV25Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV25CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV25AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV25WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV25Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV25ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV25Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV25Response(this.#directory, "writer", {
        validate: validateCalendarLiveV25WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV25Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV25RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV25Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV25ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV25Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV25RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v25-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV25Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV25ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV25Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV25ProbePayload(payload, ownership),
      });
      persistCalendarLiveV25TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v25 capture cell missing");
      const accepted = acceptCalendarLiveV25Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV25CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V25_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV25CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV25Objective(
    ...
    throw new Error(`Calendar live v25 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV25Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV25CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV25Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV25RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV25Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV25CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV25CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV25CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV25Authorization = (
  privateKey: KeyObject,
): CalendarLiveV25TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV25SimulationKey = (): KeyObject =>
