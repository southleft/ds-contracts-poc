/**
 * Carried Combobox live v30 host verifier. Uses scene-readback-combobox-v30 so live
...
export const COMBOBOX_LIVE_V30_FIXED_VERIFIER_VERSION = "combobox-live-v30-fixed-point-v1";
export const COMBOBOX_LIVE_V30_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV30FixedThresholds {
...
export interface ComboboxLiveV30FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV30FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV30FixedSourceProbe {
...
export interface ComboboxLiveV30FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV30FixedFixedPoint;
}

export interface ComboboxLiveV30FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV30FixedHardGateInput {
  thresholds: ComboboxLiveV30FixedThresholds;
  materialRegression: ComboboxLiveV30FixedMaterialRegression;
  sourceProbes: ComboboxLiveV30FixedSourceProbe[];
  cells: ComboboxLiveV30FixedCellValidation[];
  sceneProofs: ComboboxLiveV30FixedSceneProof[];
  visualRows: ComboboxLiveV30FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV30FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V30_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV30FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV30FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV30FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV30FixedHardGates(
  input: ComboboxLiveV30FixedHardGateInput,
): ComboboxLiveV30FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV30FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV30FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V30_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV30FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV30FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV30FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
