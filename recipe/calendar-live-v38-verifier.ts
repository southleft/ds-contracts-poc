...
import {
  verifyCalendarLiveV38FixedSceneFixedPoint,
  type CalendarLiveV38FixedFixedPoint,
} from "./calendar-live-v38-fixed-point.js";

export const CALENDAR_LIVE_V38_VERIFIER_VERSION = "calendar-live-v38-verifier-v1";

export function buildCalendarLiveV38RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV38RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV38RawNode[];
}

export interface CalendarLiveV38NormalizedScene {
...
export interface CalendarLiveV38SceneProof {
...
export function normalizeCalendarLiveV38Scene(
  root: CalendarLiveV38RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV38NormalizedScene {
  const canonicalBindings: CalendarLiveV38NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV38RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV38Scene<Instance>(
  normalized: CalendarLiveV38NormalizedScene,
...
): CalendarLiveV38SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV38FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV38ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV38PreCaptureGates(payload: unknown): void {
