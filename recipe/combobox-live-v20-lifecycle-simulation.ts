...
export interface ComboboxLiveV20LifecycleSimulationReport {
  artifactVersion: "combobox-live-v20-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV20AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV20Lifecycle(): Promise<ComboboxLiveV20LifecycleSimulationReport> {
  await buildComboboxLiveV20Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V20_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV20AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v20-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V20_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V20_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV20History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV20History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV20AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V20_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V20_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV20History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV20History(authorized, "pending");
    ...
    await buildComboboxLiveV20Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V20_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV20AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV20Lifecycle(), null, 2)}\n`,
  );
