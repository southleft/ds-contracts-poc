...
import { buildTableLiveV35CleanupRuntime } from "./table-live-v35-cleanup.js";
import { type TableLiveV35FixedFixedPoint } from "./table-live-v35-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV35RestoreProgram,
  validateTableLiveV35RestorePayload,
  type TableLiveV35RestorePayload,
} from "./table-live-v35-restore.js";
import {
  buildTableLiveV35RawPropertyRuntime,
  normalizeTableLiveV35Scene,
  type TableLiveV35RawNode,
} from "./table-live-v35-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v35.js";
import {
...
export const TABLE_LIVE_V35_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V35_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V35_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V35_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V35_VARIANT_COUNT = 20;
export const TABLE_LIVE_V35_SET_COUNT = 6;
export const TABLE_LIVE_V35_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V35_HOST_PHASES = 3;
export const TABLE_LIVE_V35_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V35_RESTORE_COUNT = 8;
export const TABLE_LIVE_V35_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V35_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV35SourceId = (typeof TABLE_LIVE_V35_SOURCE_IDS)[number];
export type TableLiveV35Kind = "table" | "row" | "cell";

export interface TableLiveV35SourceIdentity {
  source: TableLiveV35SourceId;
...
export interface TableLiveV35WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV35ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV35SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV35RawNode;
    rowScene: TableLiveV35RawNode;
    cellScene: TableLiveV35RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV35RootProof {
  source: TableLiveV35SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV35FixedFixedPoint;
}

export interface TableLiveV35ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV35SourceId;
    adapterIdentity: string;
...
export interface TableLiveV35CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV35SourceId;
  adapterIdentity: string;
...
export interface TableLiveV35CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV35SourceId;
  frameWidth: number;
...
export interface TableLiveV35CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v35 ${label} must be an object`);
...
export function validateTableLiveV35WriterPayload(
  value: unknown,
): TableLiveV35WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V35_NAMESPACE ||
    ...
    throw new TypeError("Table live v35 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Table live v35 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v35 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV35SourceIdentity,
  kind: TableLiveV35Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV35ExtractProgram(
  writer: TableLiveV35WriterOwnership,
  sources: readonly TableLiveV35SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
      "Table live v35 extract requires two source identities",
    );
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V35_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V35_NAMESPACE)}
${buildTableLiveV35RawPropertyRuntime()}
...
export function validateTableLiveV35ExtractPayload(
  value: unknown,
  writer: TableLiveV35WriterOwnership,
): TableLiveV35ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v35 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V35_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v35 extract schema/two-root mismatch");
  return payload as TableLiveV35ExtractPayload;
}

export function proveTableLiveV35Roots<Instance>(
  extract: TableLiveV35ExtractPayload,
  sources: readonly (TableLiveV35SourceIdentity & {
...
): TableLiveV35RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v35 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v35 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV35Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV35Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV35Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV35FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Table live v35 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v35 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v35 compile lost owned sets");
...
export function assertTableLiveV35RootProofs(
  proofs: readonly TableLiveV35RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v35 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v35 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV35ProbeProgram(
  writer: TableLiveV35WriterOwnership,
  sources: readonly TableLiveV35SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V35_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV35ProbePayload(
  value: unknown,
  writer: TableLiveV35WriterOwnership,
): TableLiveV35ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV35ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V35_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v35 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV35CaptureManifest(
  cells: readonly TableLiveV35CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V35_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V35_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V35_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V35_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v35 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV35CaptureCell[],
): string => {
  validateTableLiveV35CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV35CaptureProgram(
  writer: TableLiveV35WriterOwnership,
  cell: TableLiveV35CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V35_CAPTURE_COUNT
  )
    throw new TypeError("Table live v35 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Table live v35 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V35_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV35CapturePayload(
  value: unknown,
  cell: TableLiveV35CaptureCell,
  rawResponseBytes: number,
): TableLiveV35CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV35CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V35_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V35_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v35 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV35CaptureResponses(
  manifest: readonly TableLiveV35CaptureCell[],
  responses: readonly TableLiveV35CapturePayload[],
): void {
  validateTableLiveV35CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v35 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV35ObjectiveReport {
  artifactVersion: "table-live-v35-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV35Objective(
  manifest: readonly TableLiveV35CaptureCell[],
  responses: readonly TableLiveV35CapturePayload[],
): TableLiveV35ObjectiveReport {
  assertTableLiveV35CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v35-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV35CleanupProgram(
  writer: TableLiveV35WriterOwnership,
): string {
  const baseProgram = buildTableLiveV35CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V35_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v35 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV35CleanupPayload(
  payload: unknown,
  writer: TableLiveV35WriterOwnership,
): TableLiveV35CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as TableLiveV35CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v35 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V35_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV35WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
