...
/**
 * Carried Input live v10 scene-readback. Keeps the v9 VARIABLE_ALIAS /
 * bound-variable-only teaching. Live host normalize uses this file via
 * input-field-live-v3-verifier-v10.ts and must not call hashed
 * scene-readback.ts.
 */
export const SCENE_READBACK_VERSION = 1;
export const SCENE_READBACK_V10_TAUGHT_FILL_KINDS = [
  "VARIABLE_ALIAS",
  "boundVariablesOnly",
] as const;
...
const sceneRole = (scene: SceneNodeSnapshot): string | undefined =>
  scene.semanticRole ??
  (scene.name.includes("/") && !scene.name.includes("=")
