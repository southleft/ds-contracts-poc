...
import { tableLiveV1Sha256 } from "./table-live-v1-broker.js";

export interface TableLiveV1LifecycleSimulationReport {
  artifactVersion: "table-live-v1-lifecycle-simulation-v1";
...
const gitEnvironment = {
  ...process.env,
  GIT_AUTHOR_NAME: "Table v1 Lifecycle Simulation",
  GIT_AUTHOR_EMAIL: "table-v1-simulation@invalid",
  GIT_COMMITTER_NAME: "Table v1 Lifecycle Simulation",
  GIT_COMMITTER_EMAIL: "table-v1-simulation@invalid",
};
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV1AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV1Lifecycle(): Promise<TableLiveV1LifecycleSimulationReport> {
  await buildTableLiveV1Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V1_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV1AntecedentIndex;
  const indexSha256 = tableLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v1-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V1_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V1_INDEX_PATH]);
    git(temporary, ["commit", "-m", "synthetic table v1 antecedent"]);
    const antecedentCommit = git(temporary, ["rev-parse", "HEAD"]);
    const pending = historyState(index, indexSha256, antecedentCommit);
    const pendingFailures = validateTableLiveV1History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV1History(pending, "authorized");
    if (
      !staleAuthorized.some((failure) => failure.includes("stale history mode"))
    )
      throw new Error("table v1 lifecycle did not refuse stale authorized mode");

    const { publicKey } = generateKeyPairSync("ed25519");
    const authorization = buildTableLiveV1AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V1_AUTHORIZATION_PATH]);
    git(temporary, ["commit", "-m", "synthetic table v1 authorization"]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V1_INDEX_PATH}`,
    ]);
    if (!committedAntecedentBefore.equals(committedAntecedentAfter))
      throw new Error(
        "table v1 antecedent index changed across authorization commit",
      );
    ...
    const authorizedFailures = validateTableLiveV1History(
      authorized,
      "authorized",
    );
    if (authorizedFailures.length)
      throw new Error(
        `table v1 lifecycle authorized phase failed:\n${authorizedFailures.join("\n")}`,
      );
    const stalePending = validateTableLiveV1History(authorized, "pending");
    if (!stalePending.some((failure) => failure.includes("stale history mode")))
      throw new Error("table v1 lifecycle did not refuse stale pending mode");

    await buildTableLiveV1Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V1_INDEX_PATH);
    if (!indexBefore.equals(indexAfter))
      throw new Error(
        "table v1 generated check changed after synthetic authorization",
      );
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV1AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
      throw new Error("table v1 antecedent hash set changed after authorization");
    return {
      artifactVersion: "table-live-v1-lifecycle-simulation-v1",
      syntheticGitCommits: 2,
      antecedentCheckBeforeAuthorization: true,
      ...
      authorizationSha256: tableLiveV1Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV1Lifecycle(), null, 2)}\n`,
  );
