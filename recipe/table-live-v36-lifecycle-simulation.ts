...
import { tableLiveV2Sha256 } from "./table-live-v36-broker.js";

export interface TableLiveV36LifecycleSimulationReport {
  artifactVersion: "table-live-v36-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV36AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV36Lifecycle(): Promise<TableLiveV36LifecycleSimulationReport> {
  await buildTableLiveV36Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V36_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV36AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v36-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V36_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V36_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV36History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV36History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV36AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V36_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V36_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV36History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV36History(authorized, "pending");
    ...
    await buildTableLiveV36Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V36_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV36AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV36Lifecycle(), null, 2)}\n`,
  );
