...
export interface InputLiveV11LifecycleSimulationReport {
  artifactVersion: "input-live-v11-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV11AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV11Lifecycle(): Promise<InputLiveV11LifecycleSimulationReport> {
  await buildInputLiveV11Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V11_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV11AntecedentIndex;
  const indexSha256 = inputLiveV11Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v11-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V11_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V11_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV11History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV11History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV11AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V11_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V11_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV11History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV11History(authorized, "pending");
    ...
    await buildInputLiveV11Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V11_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV11AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV11Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV11Lifecycle(), null, 2)}\n`,
  );
