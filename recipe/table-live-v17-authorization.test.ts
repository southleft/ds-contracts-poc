...
import { TABLE_LIVE_V17_INDEX_PATH } from "./build-table-live-proof-v17.js";
import {
...
import {
  buildTableLiveV17AuthorizationArtifact,
  validateTableLiveV17AntecedentIndex,
  validateTableLiveV17AuthorizationArtifact,
  validateTableLiveV17History,
  validateTableLiveV17UnitTestSource,
  type TableLiveV17AntecedentIndex,
  type TableLiveV17AuthorizationArtifact,
  type TableLiveV17AuthorizationProof,
  type TableLiveV17HistoryState,
} from "./table-live-v17-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV17ControlFlowSource,
  validateTableLiveV17SecurityAttestation,
  type TableLiveV17SecurityAttestation,
} from "./table-live-v17-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V17_INDEX_PATH);
const index = (): TableLiveV17AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV17AntecedentIndex;

const artifact = (): TableLiveV17AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV17AuthorizationArtifact({
...
const pendingState = (): TableLiveV17HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV17HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV17AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v17/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV17AuthorizationArtifact,
): TableLiveV17SecurityAttestation => {
  const body: Omit<TableLiveV17SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V17_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV17History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV17History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV17History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV17History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV17History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV17UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v17-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV17UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV17AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV17AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV17AuthorizationArtifact(
        planted as TableLiveV17AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV17SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV17SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV17SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v17-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV17ControlFlowSource(source), []);
});
