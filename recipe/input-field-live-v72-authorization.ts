...
export type InputLiveV72HistoryExpectation = "pending" | "authorized";

export interface InputLiveV72AuthorizationArtifact {
  artifactVersion: "input-live-v72-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V72_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV72AntecedentIndex {
...
export interface InputLiveV72HistoryState {
  index?: InputLiveV72AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV72AuthorizationArtifact;
...
export interface InputLiveV72AuthorizationProof extends InputLiveV72TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V72_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V72_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V72_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V72_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v72-authorization") ||
  artifactPath.includes("input-field-live-v72-preflight") ||
  artifactPath.includes("input-field-live-v72-authorized") ||
  artifactPath.includes("input-field-live-pivot-v72-status") ||
...
export function validateInputLiveV72AntecedentIndex(
  index: InputLiveV72AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v72-antecedent-index-v1" ||
...
export function validateInputLiveV72UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV72History\(\s*["']/.test(source) ||
    /readInputLiveV72HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV72AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV72AuthorizationArtifact {
  ...
  const indexFailures = validateInputLiveV72AntecedentIndex(index);
...
export function validateInputLiveV72AuthorizationArtifact(
  artifact: InputLiveV72AuthorizationArtifact | undefined,
  index: InputLiveV72AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v72-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v72" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V72_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V72_TARGET) ||
    !equalJson(
...
export function validateInputLiveV72History(
  state: InputLiveV72HistoryState,
  expected: InputLiveV72HistoryExpectation,
): string[] {
  const failures = validateInputLiveV72AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV72AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV72HistoryState(
  root = process.cwd(),
): InputLiveV72HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V72_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV72AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V72_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V72_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V72_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV72History(
  expected: InputLiveV72HistoryExpectation,
  root = process.cwd(),
): InputLiveV72HistoryState {
  const state = readInputLiveV72HistoryState(root);
  const failures = validateInputLiveV72History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV72Authorization(
  root = process.cwd(),
): InputLiveV72AuthorizationProof {
  const state = verifyInputLiveV72History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V72_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v72/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v72.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V72_TARGET,
    expectedDynamicTool: INPUT_LIVE_V72_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V72_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV72HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV72History(expected);
