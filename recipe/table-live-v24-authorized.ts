...
import { TABLE_LIVE_V24_AUTHORIZATION_PATH } from "./build-table-live-proof-v24.js";
import { verifyTableLiveV24Authorization } from "./table-live-v24-authorization.js";
import { runTableLiveV24Preflight } from "./table-live-v24-preflight.js";
import { TableLiveV24Orchestrator } from "./run-table-live-v24.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v24 requires exact authorization artifact ${TABLE_LIVE_V24_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v24-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV24Authorization(root);
    runTableLiveV24Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV24Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV24Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
