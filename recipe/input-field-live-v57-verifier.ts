...
export const INPUT_LIVE_V57_VERIFIER_VERSION = "input-live-v57-verifier-v1";

export function buildInputLiveV57RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV57RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV57RawNode[];
}

export interface InputLiveV57NormalizedScene {
...
export interface InputLiveV57SceneProof {
...
export function normalizeInputLiveV57Scene(
  root: InputLiveV57RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV57NormalizedScene {
  const canonicalBindings: InputLiveV57NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV57RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV57Scene<Instance>(
  normalized: InputLiveV57NormalizedScene,
...
export function compileInputLiveV57ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV57PreCaptureGates(payload: unknown): void {
