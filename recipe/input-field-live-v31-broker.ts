...
import {
  assertInputLiveV31RootProofs,
  INPUT_LIVE_V31_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V31_RESPONSE_CONTRACTS,
  type InputLiveV31RootProof,
} from "./input-field-live-v31-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V31_BROKER_VERSION = "input-live-v31-broker-v2";
export const INPUT_LIVE_V31_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V31_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V31_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V31_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V31_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V31_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV31RemotePhase =
  (typeof INPUT_LIVE_V31_REMOTE_PHASES)[number];
export type InputLiveV31AuthorizationMode = "live" | "simulated";

export interface InputLiveV31CommitPins {
...
export interface InputLiveV31TransactionAuthorization extends InputLiveV31CommitPins {
  mode: InputLiveV31AuthorizationMode;
...
export interface InputLiveV31Manifest {
  artifactVersion: "input-live-v31-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V31_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V31_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V31_TARGET;
  authorization: InputLiveV31TransactionAuthorization;
...
export interface InputLiveV31ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV31RequestBody {
  artifactVersion: "input-live-v31-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V31_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV31RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV31CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V31_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV31ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV31Request extends InputLiveV31RequestBody {
...
export interface InputLiveV31Receipt {
  artifactVersion: "input-live-v31-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V31_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV31RemotePhase;
...
export interface InputLiveV31AcceptedResponse<T = unknown> {
  request: InputLiveV31Request;
  receipt: InputLiveV31Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV31IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV31ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V31_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V31_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V31_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V31_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V31_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V31_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V31_TARGET.editorType;
};
...
export const inputLiveV31RequestSequence = (
  phase: InputLiveV31RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV31RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV31RequestPath = (
  directory: string,
  phase: InputLiveV31RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV31ResponsePath = (
  directory: string,
  phase: InputLiveV31RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV31ReceiptPath = (
  directory: string,
  phase: InputLiveV31RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV31Manifest => {
  const manifest = json<InputLiveV31Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v31-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V31_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V31_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV31RemotePhase,
  captureIndex?: number,
): InputLiveV31Request | undefined => {
  const file = inputLiveV31RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV31Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV31RemotePhase,
  captureIndex?: number,
): InputLiveV31Receipt | undefined => {
  const file = inputLiveV31ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV31Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV31Request | undefined;
  receipt: InputLiveV31Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV31RemotePhase,
): InputLiveV31ResponseContract => ({
  ...INPUT_LIVE_V31_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V31_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V31_DYNAMIC_TOOL,
      target: INPUT_LIVE_V31_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v31-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V31_BROKER_VERSION,
  target: INPUT_LIVE_V31_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V31_PROGRAM_START*/${program}/*INPUT_LIVE_V31_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V31_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V31_PROGRAM_END*/";
...
export function createInputLiveV31Transaction(
  directory: string,
  ...
  const manifest: InputLiveV31Manifest = {
    artifactVersion: "input-live-v31-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V31_BROKER_VERSION,
...
export function validateInputLiveV31Request(
  directory: string,
  request: InputLiveV31Request,
): void {
  ...
  const expectedSequence = inputLiveV31RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v31-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V31_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V31_TARGET.fileKey ||
...
export function issueInputLiveV31Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV31RemotePhase,
  program: string,
  options: InputLiveV31IssueOptions = {},
): InputLiveV31Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV31RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV31Request>(targetPath);
    validateInputLiveV31Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV31ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV31TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV31RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V31_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V31_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V31_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV31RequestBody = {
    artifactVersion: "input-live-v31-broker-request-v2",
    brokerVersion: INPUT_LIVE_V31_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV31Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV31Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV31RawResponse(
  directory: string,
  phase: InputLiveV31RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV31RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV31Request>(requestFile);
  if (
  ...
  const target = inputLiveV31ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV31Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v31-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V31_BROKER_VERSION,
...
export function acceptInputLiveV31Response<T = unknown>(
  directory: string,
  phase: InputLiveV31RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV31AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV31ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV31Request>(
    inputLiveV31RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV31Request(directory, request);
  if (phase === "capture") validateInputLiveV31TechnicalGates(directory);
  const rawFile = inputLiveV31ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v31-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V31_BROKER_VERSION ||
...
export function persistInputLiveV31TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV31RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV31RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V31_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v31-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V31_BROKER_VERSION,
...
export function validateInputLiveV31TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v31-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V31_TECHNICAL_GATES.some(
...
export function validateInputLiveV31Receipt(
  directory: string,
  phase: InputLiveV31RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV31Request(directory, request);
  const receipt = json<InputLiveV31Receipt>(
    inputLiveV31ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV31ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v31-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V31_BROKER_VERSION ||
...
export function readInputLiveV31AcceptedRawPayload(
  directory: string,
  phase: InputLiveV31RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV31Receipt(directory, phase, captureIndex);
...
export function createInputLiveV31FakeDynamicResponse(
  request: InputLiveV31Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V31_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V31_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV31PersistedRequests(
  directory: string,
): InputLiveV31Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV31Request>(file)] : [];
    });
}

export const inputLiveV31Sha256 = sha256;
