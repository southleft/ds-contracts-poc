...
import { verifyComboboxLiveV6Authorization } from "./combobox-live-v6-authorization.js";
import { COMBOBOX_LIVE_V6_TARGET } from "./combobox-live-v6-broker.js";
import {
  COMBOBOX_LIVE_V6_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV6Authorization();
...
const attestation: ComboboxLiveV6SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV6SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v6 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V6_SECURITY_ATTESTATION_DEFAULT_PATH,
);
