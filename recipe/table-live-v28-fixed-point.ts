/**
 * Table live v28 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V28_FIXED_VERIFIER_VERSION = "table-live-v28-fixed-point-v1";
export const TABLE_LIVE_V28_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV28FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV28FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV28FixedSourceProbe {
...
export interface TableLiveV28FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV28FixedFixedPoint;
}

export interface TableLiveV28FixedHardGateInput {
  thresholds: TableLiveV28FixedThresholds;
  sourceProbes: TableLiveV28FixedSourceProbe[];
  cells: TableLiveV28FixedCellValidation[];
  sceneProofs: TableLiveV28FixedSceneProof[];
...
export interface TableLiveV28FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V28_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV28FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV28FixedHardGates(
  input: TableLiveV28FixedHardGateInput,
): TableLiveV28FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV28FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V28_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV28FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV28FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
