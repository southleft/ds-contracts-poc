...
} from "./input-field-live-v3-verifier-v59.js";
import {
  buildInputLiveV59RawPropertyRuntime,
  normalizeInputLiveV59Scene,
  type InputLiveV59RawNode,
} from "./input-field-live-v59-verifier.js";
...
export const INPUT_LIVE_V59_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V59_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V59_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V59_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V59_VARIANT_COUNT = 256;
export const INPUT_LIVE_V59_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V59_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV59SourceId = (typeof INPUT_LIVE_V59_SOURCE_IDS)[number];

export interface InputLiveV59SourceIdentity {
  source: InputLiveV59SourceId;
...
export interface InputLiveV59WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV59ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV59SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV59RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV59RootProof {
  source: InputLiveV59SourceId;
  adapterIdentity: string;
...
export interface InputLiveV59ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV59SourceId;
    adapterIdentity: string;
...
export interface InputLiveV59CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV59SourceId;
  adapterIdentity: string;
...
export interface InputLiveV59CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV59SourceId;
  strata: InputLiveV59CaptureCell["strata"];
...
export interface InputLiveV59CleanupPayload {
...
export function validateInputLiveV59WriterPayload(
  value: unknown,
): InputLiveV59WriterOwnership {
...
const identityPlan = (source: InputLiveV59SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV59ExtractProgram(
  writer: InputLiveV59WriterOwnership,
  sources: readonly InputLiveV59SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V59_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V59_NAMESPACE)}
${buildInputLiveV59RawPropertyRuntime()}
...
export function validateInputLiveV59ExtractPayload(
  value: unknown,
  writer: InputLiveV59WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V59_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV59ExtractPayload;
}

export function proveInputLiveV59Roots<Instance>(
  extract: InputLiveV59ExtractPayload,
  sources: readonly (InputLiveV59SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV59Scene(
      root.scene,
...
export function assertInputLiveV59RootProofs(
  proofs: readonly InputLiveV59RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV59ProbeProgram(
  writer: InputLiveV59WriterOwnership,
  sources: readonly InputLiveV59SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V59_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV59ProbePayload(
  value: unknown,
  writer: InputLiveV59WriterOwnership,
): InputLiveV59ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV59ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V59_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V59_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV59CaptureManifest(
  cells: readonly InputLiveV59CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V59_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V59_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V59_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V59_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V59_SOURCE_IDS.some(
...
export const inputLiveV59CaptureManifestSha256 = (
  cells: readonly InputLiveV59CaptureCell[],
): string => {
  validateInputLiveV59CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV59CaptureProgram(
  writer: InputLiveV59WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V59_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V59_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V59_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV59CapturePayload(
  value: unknown,
  cell: InputLiveV59CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV59CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V59_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V59_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV59CaptureResponses(
  manifest: readonly InputLiveV59CaptureCell[],
  responses: readonly InputLiveV59CapturePayload[],
): void {
  validateInputLiveV59CaptureManifest(manifest);
  if (
...
export interface InputLiveV59ObjectiveReport {
  artifactVersion: "input-live-v59-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV59Objective(
  manifest: readonly InputLiveV59CaptureCell[],
  responses: readonly InputLiveV59CapturePayload[],
  readReference: (cell: InputLiveV59CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV59CaptureCell) => Buffer,
): InputLiveV59ObjectiveReport {
  assertInputLiveV59CaptureResponses(manifest, responses);
...
export function buildInputLiveV59CleanupProgram(
  writer: InputLiveV59WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V59_NAMESPACE,
...
export function validateInputLiveV59CleanupPayload(
  value: unknown,
  writer: InputLiveV59WriterOwnership,
): InputLiveV59CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV59CleanupPayload;
...
export const INPUT_LIVE_V59_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV59WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
