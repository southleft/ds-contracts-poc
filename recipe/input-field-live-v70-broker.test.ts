...
import {
  ...
  createInputLiveV70FakeDynamicResponse,
  ...
  listInputLiveV70PersistedRequests,
  persistInputLiveV70RawResponse,
  persistInputLiveV70TechnicalGates,
  validateInputLiveV70Receipt,
  validateInputLiveV70Request,
  type InputLiveV70RemotePhase,
...
import {
  ...
  type InputLiveV70ExtractPayload,
  type InputLiveV70ProbePayload,
...
import {
  ...
  simulatedInputLiveV70Authorization,
} from "./run-input-field-live-v70.js";
...
const captures: InputLiveV70CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV70Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v70-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV70Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV70CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV70FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV70FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V70_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V70_TARGET);
  persistInputLiveV70RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV70Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV70Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV70WriterOwnership,
...
const rootProofs = (): InputLiveV70RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v70-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV70Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV70Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV70RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV70RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV70ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV70ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV70CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV70CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV70CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV70Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV70CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV70PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV70ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV70Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV70Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV70Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV70Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V70_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV70RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V70_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV70Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV70Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV70ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV70Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV70WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV70RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV70Request;
    validateInputLiveV70Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV70Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV70CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v70-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV70Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV70Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV70Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V70_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV70RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V70_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV70Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV70Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV70RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V70_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV70Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV70Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV70Request;
