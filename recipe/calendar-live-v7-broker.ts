...
import {
  assertCalendarLiveV7RootProofs,
  CALENDAR_LIVE_V7_CAPTURE_COUNT,
  CALENDAR_LIVE_V7_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  CALENDAR_LIVE_V7_RESPONSE_CONTRACTS,
  type CalendarLiveV7RootProof,
} from "./calendar-live-v7-contract.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V7_BROKER_VERSION = "calendar-live-v7-broker-v1";
export const CALENDAR_LIVE_V7_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const CALENDAR_LIVE_V7_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const CALENDAR_LIVE_V7_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const CALENDAR_LIVE_V7_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const CALENDAR_LIVE_V7_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const CALENDAR_LIVE_V7_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type CalendarLiveV7RemotePhase =
  (typeof CALENDAR_LIVE_V7_REMOTE_PHASES)[number];
export type CalendarLiveV7AuthorizationMode = "live" | "simulated";

export interface CalendarLiveV7CommitPins {
...
export interface CalendarLiveV7TransactionAuthorization extends CalendarLiveV7CommitPins {
  mode: CalendarLiveV7AuthorizationMode;
...
export interface CalendarLiveV7Manifest {
  artifactVersion: "calendar-live-v7-broker-manifest-v2";
  brokerVersion: typeof CALENDAR_LIVE_V7_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof CALENDAR_LIVE_V7_DYNAMIC_TOOL;
  target: typeof CALENDAR_LIVE_V7_TARGET;
  authorization: CalendarLiveV7TransactionAuthorization;
...
export interface CalendarLiveV7ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface CalendarLiveV7RequestBody {
  artifactVersion: "calendar-live-v7-broker-request-v2";
  brokerVersion: typeof CALENDAR_LIVE_V7_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: CalendarLiveV7RemotePhase;
  sequence: number;
  ...
  commitPins: CalendarLiveV7CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof CALENDAR_LIVE_V7_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: CalendarLiveV7ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface CalendarLiveV7Request extends CalendarLiveV7RequestBody {
...
export interface CalendarLiveV7Receipt {
  artifactVersion: "calendar-live-v7-broker-receipt-v2";
  brokerVersion: typeof CALENDAR_LIVE_V7_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: CalendarLiveV7RemotePhase;
...
export interface CalendarLiveV7AcceptedResponse<T = unknown> {
  request: CalendarLiveV7Request;
  receipt: CalendarLiveV7Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface CalendarLiveV7IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: CalendarLiveV7ResponseContract;
...
const equalTarget = (value: unknown): value is typeof CALENDAR_LIVE_V7_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === CALENDAR_LIVE_V7_TARGET.fileKey &&
  (value as BrokerTarget).fileName === CALENDAR_LIVE_V7_TARGET.fileName &&
  (value as BrokerTarget).editorType === CALENDAR_LIVE_V7_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== CALENDAR_LIVE_V7_TARGET.fileKey ||
    context.fileName !== CALENDAR_LIVE_V7_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === CALENDAR_LIVE_V7_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Calendar live v7 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= CALENDAR_LIVE_V7_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v7 capture index must be 0..7");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Calendar live v7 authorization pins are malformed");
};

const readManifest = (directory: string): CalendarLiveV7Manifest => {
  const manifest = json<CalendarLiveV7Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Calendar live v7 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "calendar-live-v7-broker-manifest-v2" ||
    manifest.brokerVersion !== CALENDAR_LIVE_V7_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== CALENDAR_LIVE_V7_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(CALENDAR_LIVE_V7_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Calendar live v7 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: CalendarLiveV7Request,
): CalendarLiveV7RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: CalendarLiveV7RemotePhase,
  captureIndex?: number,
): CalendarLiveV7Request | undefined => {
  const file = calendarLiveV1RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV7Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: CalendarLiveV7RemotePhase,
  captureIndex?: number,
): CalendarLiveV7Receipt | undefined => {
  const file = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV7Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: CalendarLiveV7Request | undefined;
  receipt: CalendarLiveV7Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: CalendarLiveV7RemotePhase,
): CalendarLiveV7ResponseContract => ({
  ...CALENDAR_LIVE_V7_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: CalendarLiveV7Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: CALENDAR_LIVE_V7_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: CALENDAR_LIVE_V7_DYNAMIC_TOOL,
      target: CALENDAR_LIVE_V7_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "calendar-live-v7-plugin-response-v2",
  brokerVersion: CALENDAR_LIVE_V7_BROKER_VERSION,
  target: CALENDAR_LIVE_V7_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*CALENDAR_LIVE_V7_PROGRAM_START*/${program}/*CALENDAR_LIVE_V7_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*CALENDAR_LIVE_V7_PROGRAM_START*/";
  const endMarker = "/*CALENDAR_LIVE_V7_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Calendar live v7 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createCalendarLiveV7Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Calendar live v7 private signing key does not match authorized public-key identity",
    );
  const manifest: CalendarLiveV7Manifest = {
    artifactVersion: "calendar-live-v7-broker-manifest-v2",
    brokerVersion: CALENDAR_LIVE_V7_BROKER_VERSION,
    ...
    captureRequests: CALENDAR_LIVE_V7_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateCalendarLiveV7Request(
  directory: string,
  request: CalendarLiveV7Request,
): void {
  ...
  if (
    request.artifactVersion !== "calendar-live-v7-broker-request-v2" ||
    request.brokerVersion !== CALENDAR_LIVE_V7_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== CALENDAR_LIVE_V7_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Calendar live v7 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueCalendarLiveV7Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: CalendarLiveV7RemotePhase,
  program: string,
  options: CalendarLiveV7IssueOptions = {},
): CalendarLiveV7Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<CalendarLiveV7Request>(targetPath);
    validateCalendarLiveV7Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Calendar live v7 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(calendarLiveV1ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Calendar live v7 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Calendar live v7 empty code envelope refused");
  if (phase === "capture") validateCalendarLiveV7TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Calendar live v7 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: CALENDAR_LIVE_V7_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? CALENDAR_LIVE_V7_SIGNED_WRITER_TIMEOUT_MS
        : CALENDAR_LIVE_V7_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: CalendarLiveV7RequestBody = {
    artifactVersion: "calendar-live-v7-broker-request-v2",
    brokerVersion: CALENDAR_LIVE_V7_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: CalendarLiveV7Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateCalendarLiveV7Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistCalendarLiveV7RawResponse(
  directory: string,
  phase: CalendarLiveV7RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Calendar live v7 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<CalendarLiveV7Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Calendar live v7 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = calendarLiveV1ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Calendar live v7 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): CalendarLiveV7Receipt => {
  const body: Omit<CalendarLiveV7Receipt, "receiptSha256"> = {
    artifactVersion: "calendar-live-v7-broker-receipt-v2",
    brokerVersion: CALENDAR_LIVE_V7_BROKER_VERSION,
...
export function acceptCalendarLiveV7Response<T = unknown>(
  directory: string,
  phase: CalendarLiveV7RemotePhase,
  options: {
    captureIndex?: number;
...
): CalendarLiveV7AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Calendar live v7 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<CalendarLiveV7Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Calendar live v7 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "calendar-live-v7-plugin-response-v2" ||
    plugin?.brokerVersion !== CALENDAR_LIVE_V7_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Calendar live v7 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistCalendarLiveV7TechnicalGates(
  directory: string,
  rootProofs: readonly CalendarLiveV7RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertCalendarLiveV7RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Calendar live v7 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Calendar live v7 probe payload hash malformed");
  const gates = Object.fromEntries(
    CALENDAR_LIVE_V7_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "calendar-live-v7-host-technical-gates-v2",
    brokerVersion: CALENDAR_LIVE_V7_BROKER_VERSION,
...
export function validateCalendarLiveV7TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "calendar-live-v7-host-technical-gates-v2" ||
    ...
    CALENDAR_LIVE_V7_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Calendar live v7 technical gates artifact");
}

export function validateCalendarLiveV7Receipt(
  directory: string,
  phase: CalendarLiveV7RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateCalendarLiveV7Request(directory, request);
  const receipt = json<CalendarLiveV7Receipt>(
  ...
  if (
    receipt.artifactVersion !== "calendar-live-v7-broker-receipt-v2" ||
    receipt.brokerVersion !== CALENDAR_LIVE_V7_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Calendar live v7 accepted receipt: ${request.requestId}`,
    );
}

export function readCalendarLiveV7AcceptedRawPayload(
  directory: string,
  phase: CalendarLiveV7RemotePhase,
  captureIndex?: number,
): unknown {
  validateCalendarLiveV7Receipt(directory, phase, captureIndex);
...
export function createCalendarLiveV7FakeDynamicResponse(
  request: CalendarLiveV7Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? CALENDAR_LIVE_V7_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? CALENDAR_LIVE_V7_TARGET,
    ...overrides.extra,
  };
}

export function listCalendarLiveV7PersistedRequests(
  directory: string,
): CalendarLiveV7Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<CalendarLiveV7Request>(file)] : [];
    });
}
