...
import { INPUT_LIVE_V44_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v44.js";
import { verifyInputLiveV44Authorization } from "./input-field-live-v44-authorization.js";
import { runInputLiveV44Preflight } from "./input-field-live-v44-preflight.js";
import { InputLiveV44Orchestrator } from "./run-input-field-live-v44.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V44_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v44-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV44Authorization(root);
    runInputLiveV44Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV44Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
