...
import { buildCalendarLiveV10CleanupRuntime } from "./calendar-live-v10-cleanup.js";
import { type CalendarLiveV10FixedFixedPoint } from "./calendar-live-v10-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV10RestoreProgram,
  validateCalendarLiveV10RestorePayload,
  type CalendarLiveV10RestorePayload,
} from "./calendar-live-v10-restore.js";
import {
  buildCalendarLiveV10RawPropertyRuntime,
  normalizeCalendarLiveV10Scene,
  type CalendarLiveV10RawNode,
} from "./calendar-live-v10-verifier.js";
...
export const CALENDAR_LIVE_V10_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V10_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V10_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V10_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V10_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V10_SET_COUNT = 3;
export const CALENDAR_LIVE_V10_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V10_HOST_PHASES = 3;
export const CALENDAR_LIVE_V10_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V10_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V10_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V10_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV10SourceId = (typeof CALENDAR_LIVE_V10_SOURCE_IDS)[number];
export type CalendarLiveV10Kind = "calendar" | "week" | "day";

export interface CalendarLiveV10SourceIdentity {
  source: CalendarLiveV10SourceId;
...
export interface CalendarLiveV10WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV10ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV10SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV10RawNode;
    weekScene: CalendarLiveV10RawNode;
    dayScene: CalendarLiveV10RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV10RootProof {
  source: CalendarLiveV10SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV10FixedFixedPoint;
}

export interface CalendarLiveV10ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV10SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV10CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV10SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV10CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV10SourceId;
  frameWidth: number;
...
export interface CalendarLiveV10CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v10 ${label} must be an object`);
...
export function validateCalendarLiveV10WriterPayload(
  value: unknown,
): CalendarLiveV10WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v10") ||
    ...
    result.namespace !== CALENDAR_LIVE_V10_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v10 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v10 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v10 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV10SourceIdentity,
  kind: CalendarLiveV10Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV10ExtractProgram(
  writer: CalendarLiveV10WriterOwnership,
  sources: readonly CalendarLiveV10SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v10 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V10_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V10_NAMESPACE)}
${buildCalendarLiveV10RawPropertyRuntime()}
...
export function validateCalendarLiveV10ExtractPayload(
  value: unknown,
  writer: CalendarLiveV10WriterOwnership,
): CalendarLiveV10ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v10 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V10_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v10 extract schema/one-root mismatch");
  return payload as CalendarLiveV10ExtractPayload;
}

export function proveCalendarLiveV10Roots<Instance>(
  extract: CalendarLiveV10ExtractPayload,
  sources: readonly (CalendarLiveV10SourceIdentity & {
...
): CalendarLiveV10RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v10 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v10 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV10Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV10Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV10Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV10FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v10 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v10 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v10 compile lost owned sets");
...
export function assertCalendarLiveV10RootProofs(
  proofs: readonly CalendarLiveV10RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v10 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v10 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV10ProbeProgram(
  writer: CalendarLiveV10WriterOwnership,
  sources: readonly CalendarLiveV10SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V10_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV10ProbePayload(
  value: unknown,
  writer: CalendarLiveV10WriterOwnership,
): CalendarLiveV10ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV10ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V10_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V10_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v10 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV10CaptureManifest(
  cells: readonly CalendarLiveV10CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V10_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V10_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V10_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V10_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v10 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV10CaptureCell[],
): string => {
  validateCalendarLiveV10CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV10CaptureProgram(
  writer: CalendarLiveV10WriterOwnership,
  cell: CalendarLiveV10CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V10_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v10 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v10 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V10_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV10CapturePayload(
  value: unknown,
  cell: CalendarLiveV10CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV10CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV10CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V10_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V10_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v10 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV10CaptureResponses(
  manifest: readonly CalendarLiveV10CaptureCell[],
  responses: readonly CalendarLiveV10CapturePayload[],
): void {
  validateCalendarLiveV10CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v10 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV10ObjectiveReport {
  artifactVersion: "calendar-live-v10-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV10Objective(
  manifest: readonly CalendarLiveV10CaptureCell[],
  responses: readonly CalendarLiveV10CapturePayload[],
): CalendarLiveV10ObjectiveReport {
  assertCalendarLiveV10CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v10-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV10CleanupProgram(
  writer: CalendarLiveV10WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV10CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V10_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v10 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV10CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV10WriterOwnership,
): CalendarLiveV10CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV10CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v10 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V10_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV10WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
