...
import { COMBOBOX_LIVE_V22_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v22.js";
import { verifyComboboxLiveV22Authorization } from "./combobox-live-v22-authorization.js";
import { runComboboxLiveV22Preflight } from "./combobox-live-v22-preflight.js";
import { ComboboxLiveV22Orchestrator } from "./run-combobox-live-v22.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v22 requires exact authorization artifact ${COMBOBOX_LIVE_V22_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v22-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV22Authorization(root);
    runComboboxLiveV22Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV22Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV22Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
