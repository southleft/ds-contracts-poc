...
import { INPUT_LIVE_V57_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v57.js";
import { verifyInputLiveV57Authorization } from "./input-field-live-v57-authorization.js";
import { runInputLiveV57Preflight } from "./input-field-live-v57-preflight.js";
import { InputLiveV57Orchestrator } from "./run-input-field-live-v57.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V57_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v57-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV57Authorization(root);
    runInputLiveV57Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV57Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
