...
import {
  INPUT_LIVE_V40_CONTENT_BINDING_COMPILE_ORDER_MARKER,
  INPUT_LIVE_V40_CONTENT_BINDING_EXTRAS_MARKER,
  INPUT_LIVE_V40_CONTENT_HIDDEN_HEIGHT_HUG_MARKER,
  INPUT_LIVE_V40_CONTENT_LETTER_SPACING_OMITTED_MARKER,
  INPUT_LIVE_V40_CONTENT_ROW_CLIPS_CONTENT_OMITTED_MARKER,
  INPUT_LIVE_V40_CONTENT_ROW_CORNER_RADIUS_OMITTED_MARKER,
  INPUT_LIVE_V40_CONTENT_ROW_EFFECTS_OMITTED_MARKER,
  INPUT_LIVE_V40_CONTENT_ROW_STROKES_OMITTED_MARKER,
  INPUT_LIVE_V40_LABEL_BINDING_COMPILE_ORDER_MARKER,
  INPUT_LIVE_V40_LABEL_BINDING_EXTRAS_MARKER,
  INPUT_LIVE_V40_LABEL_LETTER_SPACING_OMITTED_MARKER,
  INPUT_LIVE_V40_LABEL_TEXT_CASE_OMITTED_MARKER,
  INPUT_LIVE_V40_LABEL_TEXT_DECORATION_OMITTED_MARKER,
  INPUT_LIVE_V40_LABEL_ROW_CLIPS_CONTENT_OMITTED_MARKER,
  INPUT_LIVE_V40_LABEL_ROW_CORNER_RADIUS_OMITTED_MARKER,
  INPUT_LIVE_V40_LABEL_ROLES,
  INPUT_LIVE_V40_CONTENT_TEXT_CASE_OMITTED_MARKER,
  INPUT_LIVE_V40_CONTENT_TEXT_DECORATION_OMITTED_MARKER,
  INPUT_LIVE_V40_CONTENT_COMPILE_BINDING_FIELDS,
  INPUT_LIVE_V40_CONTENT_ROLES,
  INPUT_LIVE_V40_SLOT_COLOR_BINDING_MARKER,
  INPUT_LIVE_V40_SLOT_FILL_MARKER,
  INPUT_LIVE_V40_SLOT_ROLES,
  INPUT_LIVE_V40_SURFACE_BINDING_COMPILE_ORDER_MARKER,
  INPUT_LIVE_V40_SURFACE_BINDING_EXTRAS_MARKER,
  INPUT_LIVE_V40_SURFACE_COMPILE_BINDING_FIELDS,
  INPUT_LIVE_V40_SURFACE_LAYOUT_HEIGHT_ALIAS_MARKER,
  INPUT_LIVE_V40_SURFACE_LAYOUT_HEIGHT_MARKER,
  INPUT_LIVE_V40_SURFACE_STROKE_WEIGHT_MARKER,
  INPUT_LIVE_V40_VARIANT_LAYOUT_WIDTH_ALIAS_MARKER,
  INPUT_LIVE_V40_VARIANT_LAYOUT_WIDTH_COMPILE_INDEX_MARKER,
  INPUT_LIVE_V40_VARIANT_LAYOUT_WIDTH_MARKER,
...
test("v17 surfaces empty leading-slot fills from instancePayload", () => {
  ...
  assert.equal(INPUT_LIVE_V40_SLOT_FILL_MARKER, "INPUT-SLOT-FILL-FROM-PAYLOAD");
  assert.deepEqual([...INPUT_LIVE_V40_SLOT_ROLES], [
...
test("v26 drops duplicate mapped content color bindings", () => {
  ...
  assert.deepEqual([...INPUT_LIVE_V40_CONTENT_ROLES], [
...
test("v40 recovers hug height for hidden content text with FIXED vertical sizing", () => {
  const ir = sceneToNormalizedIr({
    ...contentScene("input-field/content/placeholder", [
      {
        field: "fills.0.color",
...
test("v40 does not rewrite visible content FIXED height or invent a height variable", () => {
  const ir = sceneToNormalizedIr({
    ...contentScene("input-field/content/value", [
      {
        field: "fontSize.0",
...
test("v40 does not lift hidden FIXED height onto non-content text", () => {
  const ir = sceneToNormalizedIr({
    ownershipKey: "label",
    type: "TEXT",
    name: "input-field/label :: Label",
...
test("v40 omits content letterSpacing that compile never emits", () => {
  const ir = sceneToNormalizedIr({
    ...contentScene("input-field/content/placeholder", [
      {
        field: "fontSize.0",
...
test("v40 does not omit letterSpacing on non-content non-label text", () => {
  const ir = sceneToNormalizedIr({
    ownershipKey: "helper",
    type: "TEXT",
...
test("v40 omits content textCase that compile never emits", () => {
  const ir = sceneToNormalizedIr({
    ...contentScene("input-field/content/placeholder", [
      {
        field: "fontSize.0",
...
test("v40 does not omit textCase on non-content non-label text", () => {
  const ir = sceneToNormalizedIr({
    ownershipKey: "helper",
    type: "TEXT",
    name: "input-field/message/helper :: Helper",
...
test("v40 omits content textDecoration that compile never emits", () => {
  const ir = sceneToNormalizedIr({
    ...contentScene("input-field/content/placeholder", [
      {
        field: "fontSize.0",
...
test("v40 does not omit textDecoration on non-content non-label text", () => {
  const ir = sceneToNormalizedIr({
    ownershipKey: "helper",
    type: "TEXT",
    name: "input-field/message/helper :: Helper",
...
test("v40 omits content-row clipsContent that compile never emits", () => {
  const ir = sceneToNormalizedIr({
    ...surfaceScene([]),
    name: "input-field/content-row :: Row",
    clipsContent: true,
...
test("v40 omits label-row clipsContent that compile never emits", () => {
  const ir = sceneToNormalizedIr({
    ...surfaceScene([]),
...
test("v40 does not omit clipsContent on surfaces", () => {
  const ir = sceneToNormalizedIr({
    ...surfaceScene([]),
...
test("v40 omits content-row cornerRadius that compile never emits", () => {
  const ir = sceneToNormalizedIr({
    ...surfaceScene([]),
    name: "input-field/content-row :: Row",
    cornerRadius: {
...
test("v40 omits label-row cornerRadius that compile never emits", () => {
  const ir = sceneToNormalizedIr({
    ...surfaceScene([]),
    name: "input-field/label-row :: Row",
    cornerRadius: {
...
test("v40 omits content-row effects that compile never emits", () => {
  const ir = sceneToNormalizedIr({
    ...surfaceScene([]),
    name: "input-field/content-row :: Row",
    effects: [],
...
test("v40 does not omit effects on labels", () => {
  const ir = sceneToNormalizedIr({
    ...surfaceScene([]),
    name: "input-field/label-row :: Row",
    effects: [],
...
test("v40 omits content-row strokes that compile never emits", () => {
  const ir = sceneToNormalizedIr({
    ...surfaceScene([]),
    name: "input-field/content-row :: Row",
    strokes: [],
...
test("v40 does not omit strokes on labels", () => {
  const ir = sceneToNormalizedIr({
    ...surfaceScene([]),
    name: "input-field/label-row :: Row",
    strokes: [],
...
test("v40 drops duplicate label bindings and orders remaining fields to compile", () => {
  const ir = sceneToNormalizedIr(
    labelScene([
      {
        field: "fills.0.color",
  ...
  assert.deepEqual([...INPUT_LIVE_V40_LABEL_ROLES], ["input-field/label"]);
});

test("v40 does not apply label binding teaching to surfaces", () => {
  const ir = sceneToNormalizedIr(
    surfaceScene([
      {
        field: "fills.0.color",
...
test("v40 omits label letterSpacing that compile never emits", () => {
  const ir = sceneToNormalizedIr({
    ownershipKey: "label",
    type: "TEXT",
    name: "input-field/label :: Label",
...
test("v40 omits label textCase that compile never emits", () => {
  const ir = sceneToNormalizedIr({
    ownershipKey: "label",
    type: "TEXT",
    name: "input-field/label :: Label",
...
test("v40 omits label textDecoration that compile never emits", () => {
  const ir = sceneToNormalizedIr({
    ownershipKey: "label",
    type: "TEXT",
    name: "input-field/label :: Label",
