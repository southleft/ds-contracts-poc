...
import { tableLiveV2Sha256 } from "./table-live-v34-broker.js";

export interface TableLiveV34LifecycleSimulationReport {
  artifactVersion: "table-live-v34-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV34AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV34Lifecycle(): Promise<TableLiveV34LifecycleSimulationReport> {
  await buildTableLiveV34Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V34_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV34AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v34-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V34_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V34_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV34History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV34History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV34AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V34_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V34_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV34History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV34History(authorized, "pending");
    ...
    await buildTableLiveV34Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V34_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV34AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV34Lifecycle(), null, 2)}\n`,
  );
