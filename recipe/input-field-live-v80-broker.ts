...
import {
  assertInputLiveV80RootProofs,
  INPUT_LIVE_V80_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V80_RESPONSE_CONTRACTS,
  type InputLiveV80RootProof,
} from "./input-field-live-v80-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V80_BROKER_VERSION = "input-live-v80-broker-v2";
export const INPUT_LIVE_V80_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V80_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V80_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V80_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V80_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V80_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV80RemotePhase =
  (typeof INPUT_LIVE_V80_REMOTE_PHASES)[number];
export type InputLiveV80AuthorizationMode = "live" | "simulated";

export interface InputLiveV80CommitPins {
...
export interface InputLiveV80TransactionAuthorization extends InputLiveV80CommitPins {
  mode: InputLiveV80AuthorizationMode;
...
export interface InputLiveV80Manifest {
  artifactVersion: "input-live-v80-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V80_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V80_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V80_TARGET;
  authorization: InputLiveV80TransactionAuthorization;
...
export interface InputLiveV80ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV80RequestBody {
  artifactVersion: "input-live-v80-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V80_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV80RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV80CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V80_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV80ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV80Request extends InputLiveV80RequestBody {
...
export interface InputLiveV80Receipt {
  artifactVersion: "input-live-v80-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V80_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV80RemotePhase;
...
export interface InputLiveV80AcceptedResponse<T = unknown> {
  request: InputLiveV80Request;
  receipt: InputLiveV80Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV80IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV80ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V80_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V80_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V80_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V80_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V80_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V80_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V80_TARGET.editorType;
};
...
export const inputLiveV80RequestSequence = (
  phase: InputLiveV80RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV80RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV80RequestPath = (
  directory: string,
  phase: InputLiveV80RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV80ResponsePath = (
  directory: string,
  phase: InputLiveV80RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV80ReceiptPath = (
  directory: string,
  phase: InputLiveV80RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV80Manifest => {
  const manifest = json<InputLiveV80Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v80-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V80_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V80_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV80RemotePhase,
  captureIndex?: number,
): InputLiveV80Request | undefined => {
  const file = inputLiveV80RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV80Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV80RemotePhase,
  captureIndex?: number,
): InputLiveV80Receipt | undefined => {
  const file = inputLiveV80ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV80Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV80Request | undefined;
  receipt: InputLiveV80Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV80RemotePhase,
): InputLiveV80ResponseContract => ({
  ...INPUT_LIVE_V80_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V80_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V80_DYNAMIC_TOOL,
      target: INPUT_LIVE_V80_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v80-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V80_BROKER_VERSION,
  target: INPUT_LIVE_V80_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V80_PROGRAM_START*/${program}/*INPUT_LIVE_V80_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V80_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V80_PROGRAM_END*/";
...
export function createInputLiveV80Transaction(
  directory: string,
  ...
  const manifest: InputLiveV80Manifest = {
    artifactVersion: "input-live-v80-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V80_BROKER_VERSION,
...
export function validateInputLiveV80Request(
  directory: string,
  request: InputLiveV80Request,
): void {
  ...
  const expectedSequence = inputLiveV80RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v80-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V80_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V80_TARGET.fileKey ||
...
export function issueInputLiveV80Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV80RemotePhase,
  program: string,
  options: InputLiveV80IssueOptions = {},
): InputLiveV80Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV80RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV80Request>(targetPath);
    validateInputLiveV80Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV80ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV80TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV80RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V80_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V80_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V80_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV80RequestBody = {
    artifactVersion: "input-live-v80-broker-request-v2",
    brokerVersion: INPUT_LIVE_V80_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV80Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV80Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV80RawResponse(
  directory: string,
  phase: InputLiveV80RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV80RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV80Request>(requestFile);
  if (
  ...
  const target = inputLiveV80ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV80Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v80-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V80_BROKER_VERSION,
...
export function acceptInputLiveV80Response<T = unknown>(
  directory: string,
  phase: InputLiveV80RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV80AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV80ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV80Request>(
    inputLiveV80RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV80Request(directory, request);
  if (phase === "capture") validateInputLiveV80TechnicalGates(directory);
  const rawFile = inputLiveV80ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v80-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V80_BROKER_VERSION ||
...
export function persistInputLiveV80TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV80RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV80RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V80_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v80-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V80_BROKER_VERSION,
...
export function validateInputLiveV80TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v80-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V80_TECHNICAL_GATES.some(
...
export function validateInputLiveV80Receipt(
  directory: string,
  phase: InputLiveV80RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV80Request(directory, request);
  const receipt = json<InputLiveV80Receipt>(
    inputLiveV80ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV80ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v80-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V80_BROKER_VERSION ||
...
export function readInputLiveV80AcceptedRawPayload(
  directory: string,
  phase: InputLiveV80RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV80Receipt(directory, phase, captureIndex);
...
export function createInputLiveV80FakeDynamicResponse(
  request: InputLiveV80Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V80_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V80_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV80PersistedRequests(
  directory: string,
): InputLiveV80Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV80Request>(file)] : [];
    });
}

export const inputLiveV80Sha256 = sha256;
