...
import { verifyComboboxLiveV42Authorization } from "./combobox-live-v42-authorization.js";
import { COMBOBOX_LIVE_V42_TARGET } from "./combobox-live-v42-broker.js";
import {
  COMBOBOX_LIVE_V42_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV42Authorization();
...
const attestation: ComboboxLiveV42SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV42SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v42 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V42_SECURITY_ATTESTATION_DEFAULT_PATH,
);
