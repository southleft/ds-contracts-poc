...
import {
  ...
  verifyInputLiveV49Authorization,
  type InputLiveV49AntecedentIndex,
  type InputLiveV49AuthorizationProof,
} from "./input-field-live-v49-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V49_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v49/operator-security-attestation-template.json";
export const INPUT_LIVE_V49_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v49-security-attestation.json";

export interface InputLiveV49SecurityAttestation {
...
export interface InputLiveV49PreflightReport {
  artifactVersion: "input-live-v49-preflight-v1";
...
export const inputLiveV49AttestationSha256 = (
  value: Omit<InputLiveV49SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV49SecurityAttestation(
  value: unknown,
  proof: InputLiveV49AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v49-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V49_TARGET) ||
...
export function validateInputLiveV49ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV49Authorization");
  const preflight = source.indexOf("runInputLiveV49Preflight");
  const initialize = source.indexOf("InputLiveV49Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV49Preflight(
  root: string,
  proof: InputLiveV49AuthorizationProof,
  attestationPath = INPUT_LIVE_V49_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV49PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V49_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V49_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V49_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV49AntecedentIndex;
  failures.push(...validateInputLiveV49AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V49_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V49_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v49-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV49ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV49SecurityAttestation;
  return {
    artifactVersion: "input-live-v49-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V49_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV49Preflight(
    process.cwd(),
    verifyInputLiveV49Authorization(),
    attestationPath,
