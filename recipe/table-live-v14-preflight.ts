...
import {
  validateTableLiveV14AntecedentIndex,
  verifyTableLiveV14Authorization,
  type TableLiveV14AntecedentIndex,
  type TableLiveV14AuthorizationProof,
} from "./table-live-v14-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V14_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v14/operator-security-attestation-template.json";
export const TABLE_LIVE_V14_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v14-security-attestation.json";

export interface TableLiveV14SecurityAttestation {
  artifactVersion: "table-live-v14-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV14PreflightReport {
  artifactVersion: "table-live-v14-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV14SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV14SecurityAttestation(
  value: unknown,
  proof: TableLiveV14AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v14-operator-security-attestation-v1" ||
    artifact.status !== "complete"
  )
    failures.push("missing completed table v14 security attestation");
  if (
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V14_TARGET) ||
...
export function validateTableLiveV14ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV14Authorization");
  const preflight = source.indexOf("runTableLiveV14Preflight");
  const initialize = source.indexOf("TableLiveV14Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV14Preflight(
  root: string,
  proof: TableLiveV14AuthorizationProof,
  attestationPath = TABLE_LIVE_V14_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV14PreflightReport {
  const failures: string[] = [];
  if (
    ...
    failures.push("table v14 attempt chronology invalid or exceeds maximum 3");
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V14_TARGET) ||
    canonicalJson(proof.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V14_DYNAMIC_TOOL)
  )
    failures.push("table v14 authorization proof target or tool mismatch");
  const indexPath = path.join(root, TABLE_LIVE_V14_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV14AntecedentIndex;
  failures.push(...validateTableLiveV14AntecedentIndex(index));
  if (
    sha256(indexBytes) !== proof.antecedentIndexSha256 ||
    index.hashSetSha256 !== proof.antecedentHashSetSha256
  )
    failures.push("table v14 authorization proof antecedent index mismatch");
  for (const [relativePath, metadata] of Object.entries(index.artifacts)) {
    ...
    if (
      ...
      failures.push(
        `table v14 pinned antecedent working bytes drift: ${relativePath}`,
      );
  }
  if (!existsSync(path.join(root, TABLE_LIVE_V14_AUTHORIZATION_PATH)))
    failures.push("table v14 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V14_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV14SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v14-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV14ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v14 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV14SecurityAttestation;
  return {
    artifactVersion: "table-live-v14-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V14_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV14Preflight(
    process.cwd(),
    verifyTableLiveV14Authorization(),
    attestationPath,
