...
} from "./input-field-live-v3-verifier-v32.js";
import {
  buildInputLiveV32RawPropertyRuntime,
  normalizeInputLiveV32Scene,
  type InputLiveV32RawNode,
} from "./input-field-live-v32-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v32.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v32.js";
...
export const INPUT_LIVE_V32_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V32_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V32_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V32_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V32_VARIANT_COUNT = 256;
export const INPUT_LIVE_V32_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V32_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV32SourceId = (typeof INPUT_LIVE_V32_SOURCE_IDS)[number];

export interface InputLiveV32SourceIdentity {
  source: InputLiveV32SourceId;
...
export interface InputLiveV32WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV32ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV32SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV32RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV32RootProof {
  source: InputLiveV32SourceId;
  adapterIdentity: string;
...
export interface InputLiveV32ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV32SourceId;
    adapterIdentity: string;
...
export interface InputLiveV32CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV32SourceId;
  adapterIdentity: string;
...
export interface InputLiveV32CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV32SourceId;
  strata: InputLiveV32CaptureCell["strata"];
...
export interface InputLiveV32CleanupPayload {
...
export function validateInputLiveV32WriterPayload(
  value: unknown,
): InputLiveV32WriterOwnership {
...
const identityPlan = (source: InputLiveV32SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV32ExtractProgram(
  writer: InputLiveV32WriterOwnership,
  sources: readonly InputLiveV32SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V32_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V32_NAMESPACE)}
${buildInputLiveV32RawPropertyRuntime()}
...
export function validateInputLiveV32ExtractPayload(
  value: unknown,
  writer: InputLiveV32WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V32_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV32ExtractPayload;
}

export function proveInputLiveV32Roots<Instance>(
  extract: InputLiveV32ExtractPayload,
  sources: readonly (InputLiveV32SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV32Scene(
      root.scene,
...
export function assertInputLiveV32RootProofs(
  proofs: readonly InputLiveV32RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV32ProbeProgram(
  writer: InputLiveV32WriterOwnership,
  sources: readonly InputLiveV32SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V32_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV32ProbePayload(
  value: unknown,
  writer: InputLiveV32WriterOwnership,
): InputLiveV32ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV32ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V32_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V32_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV32CaptureManifest(
  cells: readonly InputLiveV32CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V32_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V32_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V32_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V32_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V32_SOURCE_IDS.some(
...
export const inputLiveV32CaptureManifestSha256 = (
  cells: readonly InputLiveV32CaptureCell[],
): string => {
  validateInputLiveV32CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV32CaptureProgram(
  writer: InputLiveV32WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V32_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V32_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V32_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV32CapturePayload(
  value: unknown,
  cell: InputLiveV32CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV32CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V32_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V32_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV32CaptureResponses(
  manifest: readonly InputLiveV32CaptureCell[],
  responses: readonly InputLiveV32CapturePayload[],
): void {
  validateInputLiveV32CaptureManifest(manifest);
  if (
...
export interface InputLiveV32ObjectiveReport {
  artifactVersion: "input-live-v32-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV32Objective(
  manifest: readonly InputLiveV32CaptureCell[],
  responses: readonly InputLiveV32CapturePayload[],
  readReference: (cell: InputLiveV32CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV32CaptureCell) => Buffer,
): InputLiveV32ObjectiveReport {
  assertInputLiveV32CaptureResponses(manifest, responses);
...
export function buildInputLiveV32CleanupProgram(
  writer: InputLiveV32WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V32_NAMESPACE,
...
export function validateInputLiveV32CleanupPayload(
  value: unknown,
  writer: InputLiveV32WriterOwnership,
): InputLiveV32CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV32CleanupPayload;
...
export const INPUT_LIVE_V32_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV32WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
