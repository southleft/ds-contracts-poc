...
export const INPUT_LIVE_V66_VERIFIER_VERSION = "input-live-v66-verifier-v1";

export function buildInputLiveV66RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV66RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV66RawNode[];
}

export interface InputLiveV66NormalizedScene {
...
export interface InputLiveV66SceneProof {
...
export function normalizeInputLiveV66Scene(
  root: InputLiveV66RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV66NormalizedScene {
  const canonicalBindings: InputLiveV66NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV66RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV66Scene<Instance>(
  normalized: InputLiveV66NormalizedScene,
...
export function compileInputLiveV66ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV66PreCaptureGates(payload: unknown): void {
