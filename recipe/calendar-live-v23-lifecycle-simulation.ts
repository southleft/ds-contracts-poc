...
import { calendarLiveV1Sha256 } from "./calendar-live-v23-broker.js";

export interface CalendarLiveV23LifecycleSimulationReport {
  artifactVersion: "calendar-live-v23-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV23AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV23Lifecycle(): Promise<CalendarLiveV23LifecycleSimulationReport> {
  await buildCalendarLiveV23Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V23_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV23AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v23-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V23_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V23_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV23History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV23History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV23AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V23_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V23_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV23History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV23History(authorized, "pending");
    ...
    await buildCalendarLiveV23Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V23_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV23AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV23Lifecycle(), null, 2)}\n`,
  );
