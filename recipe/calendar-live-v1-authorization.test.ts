...
import { CALENDAR_LIVE_V1_INDEX_PATH } from "./build-calendar-live-proof-v1.js";
import {
...
import {
  buildCalendarLiveV1AuthorizationArtifact,
  validateCalendarLiveV1AntecedentIndex,
  validateCalendarLiveV1AuthorizationArtifact,
  validateCalendarLiveV1History,
  validateCalendarLiveV1UnitTestSource,
  type CalendarLiveV1AntecedentIndex,
  type CalendarLiveV1AuthorizationArtifact,
  type CalendarLiveV1AuthorizationProof,
  type CalendarLiveV1HistoryState,
} from "./calendar-live-v1-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV1ControlFlowSource,
  validateCalendarLiveV1SecurityAttestation,
  type CalendarLiveV1SecurityAttestation,
} from "./calendar-live-v1-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V1_INDEX_PATH);
const index = (): CalendarLiveV1AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV1AntecedentIndex;

const artifact = (): CalendarLiveV1AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV1AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV1HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV1HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV1AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v1/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV1AuthorizationArtifact,
): CalendarLiveV1SecurityAttestation => {
  const body: Omit<CalendarLiveV1SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V1_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV1History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV1History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV1History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV1History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV1History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV1UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v1-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV1UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV1AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV1AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV1AuthorizationArtifact(
        planted as CalendarLiveV1AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV1SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV1SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV1SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v1-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV1ControlFlowSource(source), []);
});
