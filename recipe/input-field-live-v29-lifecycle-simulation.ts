...
export interface InputLiveV29LifecycleSimulationReport {
  artifactVersion: "input-live-v29-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV29AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV29Lifecycle(): Promise<InputLiveV29LifecycleSimulationReport> {
  await buildInputLiveV29Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V29_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV29AntecedentIndex;
  const indexSha256 = inputLiveV29Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v29-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V29_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V29_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV29History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV29History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV29AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V29_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V29_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV29History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV29History(authorized, "pending");
    ...
    await buildInputLiveV29Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V29_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV29AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV29Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV29Lifecycle(), null, 2)}\n`,
  );
