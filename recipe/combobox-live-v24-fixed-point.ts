/**
 * Carried Combobox live v24 host verifier. Uses scene-readback-combobox-v24 so live
...
export const COMBOBOX_LIVE_V24_FIXED_VERIFIER_VERSION = "combobox-live-v24-fixed-point-v1";
export const COMBOBOX_LIVE_V24_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV24FixedThresholds {
...
export interface ComboboxLiveV24FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV24FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV24FixedSourceProbe {
...
export interface ComboboxLiveV24FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV24FixedFixedPoint;
}

export interface ComboboxLiveV24FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV24FixedHardGateInput {
  thresholds: ComboboxLiveV24FixedThresholds;
  materialRegression: ComboboxLiveV24FixedMaterialRegression;
  sourceProbes: ComboboxLiveV24FixedSourceProbe[];
  cells: ComboboxLiveV24FixedCellValidation[];
  sceneProofs: ComboboxLiveV24FixedSceneProof[];
  visualRows: ComboboxLiveV24FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV24FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V24_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV24FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV24FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV24FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV24FixedHardGates(
  input: ComboboxLiveV24FixedHardGateInput,
): ComboboxLiveV24FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV24FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV24FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V24_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV24FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV24FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV24FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
