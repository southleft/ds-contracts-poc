...
export type CalendarLiveV2HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV2AuthorizationArtifact {
  artifactVersion: "calendar-live-v2-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V2_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV2AntecedentIndex {
...
export interface CalendarLiveV2HistoryState {
  index?: CalendarLiveV2AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV2AuthorizationArtifact;
...
export interface CalendarLiveV2AuthorizationProof
  extends CalendarLiveV2TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V2_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V2_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V2_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V2_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v2-authorization") ||
  artifactPath.includes("calendar-live-v2-preflight") ||
  artifactPath.includes("calendar-live-v2-authorized") ||
  artifactPath.includes("create-calendar-live-v2-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v2-status") ||
...
export function validateCalendarLiveV2AntecedentIndex(
  index: CalendarLiveV2AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v2-antecedent-index-v1" ||
...
export function validateCalendarLiveV2UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV2History\(\s*["']/.test(source) ||
    /readCalendarLiveV2HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV2AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV2AuthorizationArtifact {
  ...
  ) as CalendarLiveV2AntecedentIndex;
  const indexFailures = validateCalendarLiveV2AntecedentIndex(index);
...
export function validateCalendarLiveV2AuthorizationArtifact(
  artifact: CalendarLiveV2AuthorizationArtifact | undefined,
  index: CalendarLiveV2AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v2-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v2" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V2_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V2_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V2_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V2_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V2_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V2_SOURCE_ROOTS ||
...
export function validateCalendarLiveV2History(
  state: CalendarLiveV2HistoryState,
  expected: CalendarLiveV2HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV2AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV2AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV2HistoryState(
  root = process.cwd(),
): CalendarLiveV2HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V2_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV2AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V2_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V2_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V2_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV2History(
  expected: CalendarLiveV2HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV2HistoryState {
  const state = readCalendarLiveV2HistoryState(root);
  const failures = validateCalendarLiveV2History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v2 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV2Authorization(
  root = process.cwd(),
): CalendarLiveV2AuthorizationProof {
  const state = verifyCalendarLiveV2History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V2_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v2/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v2.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V2_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V2_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V2_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV2HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV2History(expected);
  process.stdout.write(
    `Calendar live v2 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
