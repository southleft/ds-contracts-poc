...
import { buildComboboxLiveV37CleanupRuntime } from "./combobox-live-v37-cleanup.js";
import {
  type ComboboxLiveV37FixedFixedPoint,
} from "./combobox-live-v37-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV37RestoreProgram,
  validateComboboxLiveV37RestorePayload,
  type ComboboxLiveV37RestorePayload,
} from "./combobox-live-v37-restore.js";
import {
  buildComboboxLiveV37RawPropertyRuntime,
  normalizeComboboxLiveV37Scene,
  type ComboboxLiveV37RawNode,
} from "./combobox-live-v37-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v37.js";
import {
...
export const COMBOBOX_LIVE_V37_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V37_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V37_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V37_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V37_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V37_SET_COUNT = 4;
export const COMBOBOX_LIVE_V37_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V37_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V37_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V37_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V37_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV37SourceId = (typeof COMBOBOX_LIVE_V37_SOURCE_IDS)[number];

export interface ComboboxLiveV37SourceIdentity {
  source: ComboboxLiveV37SourceId;
...
export interface ComboboxLiveV37WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV37ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV37SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV37RawNode;
    optionScene: ComboboxLiveV37RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV37RootProof {
  source: ComboboxLiveV37SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV37FixedFixedPoint;
}

export interface ComboboxLiveV37ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV37SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV37CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV37SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV37CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV37SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV37CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v37 ${label} must be an object`);
...
export function validateComboboxLiveV37WriterPayload(
  value: unknown,
): ComboboxLiveV37WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V37_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v37 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v37 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v37 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV37SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV37ExtractProgram(
  writer: ComboboxLiveV37WriterOwnership,
  sources: readonly ComboboxLiveV37SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v37 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V37_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V37_NAMESPACE)}
${buildComboboxLiveV37RawPropertyRuntime()}
...
export function validateComboboxLiveV37ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV37WriterOwnership,
): ComboboxLiveV37ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v37 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V37_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v37 extract schema/two-root mismatch");
  return payload as ComboboxLiveV37ExtractPayload;
}

export function proveComboboxLiveV37Roots<Instance>(
  extract: ComboboxLiveV37ExtractPayload,
  sources: readonly (ComboboxLiveV37SourceIdentity & {
...
): ComboboxLiveV37RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v37 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v37 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV37Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV37Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV37FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v37 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v37 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v37 compile lost owned sets");
...
export function assertComboboxLiveV37RootProofs(
  proofs: readonly ComboboxLiveV37RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v37 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v37 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV37ProbeProgram(
  writer: ComboboxLiveV37WriterOwnership,
  sources: readonly ComboboxLiveV37SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V37_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV37ProbePayload(
  value: unknown,
  writer: ComboboxLiveV37WriterOwnership,
): ComboboxLiveV37ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV37ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V37_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V37_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v37 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV37CaptureManifest(
  cells: readonly ComboboxLiveV37CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V37_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V37_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V37_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V37_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v37 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV37CaptureCell[],
): string => {
  validateComboboxLiveV37CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV37CaptureProgram(
  writer: ComboboxLiveV37WriterOwnership,
  cell: ComboboxLiveV37CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V37_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v37 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v37 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V37_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV37CapturePayload(
  value: unknown,
  cell: ComboboxLiveV37CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV37CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV37CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V37_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V37_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v37 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV37CaptureResponses(
  manifest: readonly ComboboxLiveV37CaptureCell[],
  responses: readonly ComboboxLiveV37CapturePayload[],
): void {
  validateComboboxLiveV37CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v37 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV37ObjectiveReport {
  artifactVersion: "combobox-live-v37-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV37Objective(
  manifest: readonly ComboboxLiveV37CaptureCell[],
  responses: readonly ComboboxLiveV37CapturePayload[],
): ComboboxLiveV37ObjectiveReport {
  assertComboboxLiveV37CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v37-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV37CleanupProgram(
  writer: ComboboxLiveV37WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV37CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V37_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v37 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV37CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV37WriterOwnership,
): ComboboxLiveV37CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV37CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v37 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V37_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV37WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
