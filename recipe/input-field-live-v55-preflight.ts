...
import {
  ...
  verifyInputLiveV55Authorization,
  type InputLiveV55AntecedentIndex,
  type InputLiveV55AuthorizationProof,
} from "./input-field-live-v55-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V55_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v55/operator-security-attestation-template.json";
export const INPUT_LIVE_V55_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v55-security-attestation.json";

export interface InputLiveV55SecurityAttestation {
...
export interface InputLiveV55PreflightReport {
  artifactVersion: "input-live-v55-preflight-v1";
...
export const inputLiveV55AttestationSha256 = (
  value: Omit<InputLiveV55SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV55SecurityAttestation(
  value: unknown,
  proof: InputLiveV55AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v55-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V55_TARGET) ||
...
export function validateInputLiveV55ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV55Authorization");
  const preflight = source.indexOf("runInputLiveV55Preflight");
  const initialize = source.indexOf("InputLiveV55Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV55Preflight(
  root: string,
  proof: InputLiveV55AuthorizationProof,
  attestationPath = INPUT_LIVE_V55_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV55PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V55_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V55_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V55_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV55AntecedentIndex;
  failures.push(...validateInputLiveV55AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V55_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V55_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v55-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV55ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV55SecurityAttestation;
  return {
    artifactVersion: "input-live-v55-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V55_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV55Preflight(
    process.cwd(),
    verifyInputLiveV55Authorization(),
    attestationPath,
