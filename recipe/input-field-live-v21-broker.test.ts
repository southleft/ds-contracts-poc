...
import {
  ...
  createInputLiveV21FakeDynamicResponse,
  ...
  listInputLiveV21PersistedRequests,
  persistInputLiveV21RawResponse,
  persistInputLiveV21TechnicalGates,
  validateInputLiveV21Receipt,
  validateInputLiveV21Request,
  type InputLiveV21RemotePhase,
...
import {
  ...
  type InputLiveV21ExtractPayload,
  type InputLiveV21ProbePayload,
  type InputLiveV21RestorePayload,
  type InputLiveV21RootProof,
  type InputLiveV21WriterOwnership,
} from "./input-field-live-v21-contract.js";
import {
  InputLiveV21Orchestrator,
  simulatedInputLiveV21Authorization,
} from "./run-input-field-live-v21.js";
...
const captures: InputLiveV21CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV21Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v21-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV21Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV21CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV21FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV21FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V21_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V21_TARGET);
  persistInputLiveV21RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV21Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV21Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV21WriterOwnership,
): InputLiveV21RestorePayload => ({
...
const rootProofs = (): InputLiveV21RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV21CaptureCell,
): InputLiveV21CapturePayload => {
  const bytes = Buffer.from(`input-live-v21-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV21Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV21Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV21RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV21RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV21ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV21ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV21CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV21CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV21CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV21Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV21CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV21PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV21ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV21Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV21Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV21Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV21Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V21_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV21RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V21_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV21Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV21Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV21ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV21Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV21WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV21RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV21Request;
    validateInputLiveV21Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV21Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV21CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v21-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV21Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV21Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV21Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V21_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV21RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V21_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV21Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV21Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV21RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V21_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV21Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV21Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV21Request;
