...
import {
  ...
  verifyInputLiveV63Authorization,
  type InputLiveV63AntecedentIndex,
  type InputLiveV63AuthorizationProof,
} from "./input-field-live-v63-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V63_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v63/operator-security-attestation-template.json";
export const INPUT_LIVE_V63_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v63-security-attestation.json";

export interface InputLiveV63SecurityAttestation {
...
export interface InputLiveV63PreflightReport {
  artifactVersion: "input-live-v63-preflight-v1";
...
export const inputLiveV63AttestationSha256 = (
  value: Omit<InputLiveV63SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV63SecurityAttestation(
  value: unknown,
  proof: InputLiveV63AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v63-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V63_TARGET) ||
...
export function validateInputLiveV63ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV63Authorization");
  const preflight = source.indexOf("runInputLiveV63Preflight");
  const initialize = source.indexOf("InputLiveV63Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV63Preflight(
  root: string,
  proof: InputLiveV63AuthorizationProof,
  attestationPath = INPUT_LIVE_V63_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV63PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V63_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V63_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V63_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV63AntecedentIndex;
  failures.push(...validateInputLiveV63AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V63_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V63_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v63-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV63ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV63SecurityAttestation;
  return {
    artifactVersion: "input-live-v63-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V63_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV63Preflight(
    process.cwd(),
    verifyInputLiveV63Authorization(),
    attestationPath,
