...
import { TABLE_LIVE_V16_TARGET } from "./table-live-v16-broker.js";
import {
  TABLE_LIVE_V16_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV16Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-table-v16-session";
...
const attestation: TableLiveV16SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV16SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v16 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V16_SECURITY_ATTESTATION_DEFAULT_PATH,
);
