...
import {
  verifyTableLiveV20FixedSceneFixedPoint,
  type TableLiveV20FixedFixedPoint,
} from "./table-live-v20-fixed-point.js";

export const TABLE_LIVE_V20_VERIFIER_VERSION = "table-live-v20-verifier-v1";

export function buildTableLiveV20RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV20RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV20RawNode[];
}

export interface TableLiveV20NormalizedScene {
...
export interface TableLiveV20SceneProof {
...
export function normalizeTableLiveV20Scene(
  root: TableLiveV20RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV20NormalizedScene {
  const canonicalBindings: TableLiveV20NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV20RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV20Scene<Instance>(
  normalized: TableLiveV20NormalizedScene,
...
): TableLiveV20SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV20FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV20ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV20PreCaptureGates(payload: unknown): void {
