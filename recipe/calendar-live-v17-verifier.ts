...
import {
  verifyCalendarLiveV17FixedSceneFixedPoint,
  type CalendarLiveV17FixedFixedPoint,
} from "./calendar-live-v17-fixed-point.js";

export const CALENDAR_LIVE_V17_VERIFIER_VERSION = "calendar-live-v17-verifier-v1";

export function buildCalendarLiveV17RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV17RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV17RawNode[];
}

export interface CalendarLiveV17NormalizedScene {
...
export interface CalendarLiveV17SceneProof {
...
export function normalizeCalendarLiveV17Scene(
  root: CalendarLiveV17RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV17NormalizedScene {
  const canonicalBindings: CalendarLiveV17NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV17RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV17Scene<Instance>(
  normalized: CalendarLiveV17NormalizedScene,
...
): CalendarLiveV17SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV17FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV17ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV17PreCaptureGates(payload: unknown): void {
