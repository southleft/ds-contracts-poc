...
import { buildTableLiveV10CleanupRuntime } from "./table-live-v10-cleanup.js";
import { type TableLiveV10FixedFixedPoint } from "./table-live-v10-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV10RestoreProgram,
  validateTableLiveV10RestorePayload,
  type TableLiveV10RestorePayload,
} from "./table-live-v10-restore.js";
import {
  buildTableLiveV10RawPropertyRuntime,
  normalizeTableLiveV10Scene,
  type TableLiveV10RawNode,
} from "./table-live-v10-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v10.js";
import {
...
export const TABLE_LIVE_V10_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V10_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V10_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V10_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V10_VARIANT_COUNT = 20;
export const TABLE_LIVE_V10_SET_COUNT = 6;
export const TABLE_LIVE_V10_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V10_HOST_PHASES = 3;
export const TABLE_LIVE_V10_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V10_RESTORE_COUNT = 8;
export const TABLE_LIVE_V10_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V10_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV10SourceId = (typeof TABLE_LIVE_V10_SOURCE_IDS)[number];
export type TableLiveV10Kind = "table" | "row" | "cell";

export interface TableLiveV10SourceIdentity {
  source: TableLiveV10SourceId;
...
export interface TableLiveV10WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV10ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV10SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV10RawNode;
    rowScene: TableLiveV10RawNode;
    cellScene: TableLiveV10RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV10RootProof {
  source: TableLiveV10SourceId;
  adapterIdentity: string;
...
export interface TableLiveV10ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV10SourceId;
    adapterIdentity: string;
...
export interface TableLiveV10CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV10SourceId;
  adapterIdentity: string;
...
export interface TableLiveV10CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV10SourceId;
  frameWidth: number;
...
export interface TableLiveV10CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v10 ${label} must be an object`);
...
export function validateTableLiveV10WriterPayload(
  value: unknown,
): TableLiveV10WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V10_NAMESPACE ||
    ...
    throw new TypeError("Table live v10 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v10 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v10 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV10SourceIdentity,
  kind: TableLiveV10Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV10ExtractProgram(
  writer: TableLiveV10WriterOwnership,
  sources: readonly TableLiveV10SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v10 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V10_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V10_NAMESPACE)}
${buildTableLiveV10RawPropertyRuntime()}
...
export function validateTableLiveV10ExtractPayload(
  value: unknown,
  writer: TableLiveV10WriterOwnership,
): TableLiveV10ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v10 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V10_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v10 extract schema/two-root mismatch");
  return payload as TableLiveV10ExtractPayload;
}

export function proveTableLiveV10Roots<Instance>(
  extract: TableLiveV10ExtractPayload,
  sources: readonly (TableLiveV10SourceIdentity & {
...
): TableLiveV10RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v10 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v10 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV10Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV10Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV10Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV10FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v10 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v10 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v10 compile lost owned sets");
...
export function assertTableLiveV10RootProofs(
  proofs: readonly TableLiveV10RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v10 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v10 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV10ProbeProgram(
  writer: TableLiveV10WriterOwnership,
  sources: readonly TableLiveV10SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V10_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
for(const row of byAdapter.values()){
 ...
 const cell0=propertyKey(rowInstance,"Cell 0"),cell0Before=cell0&&rowInstance.componentProperties[cell0].value;let textPropertiesRestored=false;if(cell0){rowInstance.setProperties({[cell0]:"Table v10 deterministic probe"});const changed=nodes(rowInstance).some(node=>node.type==="TEXT"&&node.characters==="Table v10 deterministic probe");rowInstance.setProperties({[cell0]:cell0Before});textPropertiesRestored=changed&&JSON.stringify(original)===JSON.stringify(plain(instance));}
 rowInstance.remove();
...
export function validateTableLiveV10ProbePayload(
  value: unknown,
  writer: TableLiveV10WriterOwnership,
): TableLiveV10ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV10ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V10_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V10_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Table live v10 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV10CaptureManifest(
  cells: readonly TableLiveV10CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V10_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V10_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V10_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V10_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v10 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV10CaptureCell[],
): string => {
  validateTableLiveV10CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV10CaptureProgram(
  writer: TableLiveV10WriterOwnership,
  cell: TableLiveV10CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V10_CAPTURE_COUNT
  )
    throw new TypeError("Table live v10 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v10 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
const frame=figma.createFrame();frame.name="Table v10 ephemeral / "+cell.cellKey;page.appendChild(frame);
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V10_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV10CapturePayload(
  value: unknown,
  cell: TableLiveV10CaptureCell,
  rawResponseBytes: number,
): TableLiveV10CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV10CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V10_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V10_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v10 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV10CaptureResponses(
  manifest: readonly TableLiveV10CaptureCell[],
  responses: readonly TableLiveV10CapturePayload[],
): void {
  validateTableLiveV10CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v10 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV10ObjectiveReport {
  artifactVersion: "table-live-v10-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV10Objective(
  manifest: readonly TableLiveV10CaptureCell[],
  responses: readonly TableLiveV10CapturePayload[],
): TableLiveV10ObjectiveReport {
  assertTableLiveV10CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v10-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV10CleanupProgram(
  writer: TableLiveV10WriterOwnership,
): string {
  const baseProgram = buildTableLiveV10CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V10_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v10 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV10CleanupPayload(
  payload: unknown,
  writer: TableLiveV10WriterOwnership,
): TableLiveV10CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV10CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v10 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V10_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV10WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
