/**
 * Table live v26 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V26_FIXED_VERIFIER_VERSION = "table-live-v26-fixed-point-v1";
export const TABLE_LIVE_V26_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV26FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV26FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV26FixedSourceProbe {
...
export interface TableLiveV26FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV26FixedFixedPoint;
}

export interface TableLiveV26FixedHardGateInput {
  thresholds: TableLiveV26FixedThresholds;
  sourceProbes: TableLiveV26FixedSourceProbe[];
  cells: TableLiveV26FixedCellValidation[];
  sceneProofs: TableLiveV26FixedSceneProof[];
...
export interface TableLiveV26FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V26_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV26FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV26FixedHardGates(
  input: TableLiveV26FixedHardGateInput,
): TableLiveV26FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV26FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V26_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV26FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV26FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
