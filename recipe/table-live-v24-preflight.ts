...
import {
  validateTableLiveV24AntecedentIndex,
  verifyTableLiveV24Authorization,
  type TableLiveV24AntecedentIndex,
  type TableLiveV24AuthorizationProof,
} from "./table-live-v24-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V24_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v24/operator-security-attestation-template.json";
export const TABLE_LIVE_V24_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v24-security-attestation.json";

export interface TableLiveV24SecurityAttestation {
  artifactVersion: "table-live-v24-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV24PreflightReport {
  artifactVersion: "table-live-v24-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV24SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV24SecurityAttestation(
  value: unknown,
  proof: TableLiveV24AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v24-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V24_TARGET) ||
...
export function validateTableLiveV24ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV24Authorization");
  const preflight = source.indexOf("runTableLiveV24Preflight");
  const initialize = source.indexOf("TableLiveV24Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV24Preflight(
  root: string,
  proof: TableLiveV24AuthorizationProof,
  attestationPath = TABLE_LIVE_V24_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV24PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V24_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V24_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV24AntecedentIndex;
  failures.push(...validateTableLiveV24AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V24_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V24_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV24SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v24-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV24ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v24 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV24SecurityAttestation;
  return {
    artifactVersion: "table-live-v24-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V24_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV24Preflight(
    process.cwd(),
    verifyTableLiveV24Authorization(),
    attestationPath,
