...
import { verifyCalendarLiveV35Authorization } from "./calendar-live-v35-authorization.js";
import { CALENDAR_LIVE_V35_TARGET } from "./calendar-live-v35-broker.js";
import {
  CALENDAR_LIVE_V35_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV35Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v35-session";
...
const attestation: CalendarLiveV35SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV35SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v35 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V35_SECURITY_ATTESTATION_DEFAULT_PATH,
);
