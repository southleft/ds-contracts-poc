...
import { CALENDAR_LIVE_V43_INDEX_PATH } from "./build-calendar-live-proof-v43.js";
import {
...
import {
  ...
  validateCalendarLiveV43History,
  validateCalendarLiveV43UnitTestSource,
  type CalendarLiveV43AntecedentIndex,
  type CalendarLiveV43AuthorizationArtifact,
  type CalendarLiveV43AuthorizationProof,
  type CalendarLiveV43HistoryState,
} from "./calendar-live-v43-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV43ControlFlowSource,
  validateCalendarLiveV43SecurityAttestation,
  type CalendarLiveV43SecurityAttestation,
} from "./calendar-live-v43-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V43_INDEX_PATH);
const index = (): CalendarLiveV43AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV43AntecedentIndex;

const artifact = (): CalendarLiveV43AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV43AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV43HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV43HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV43AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v43/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV43AuthorizationArtifact,
): CalendarLiveV43SecurityAttestation => {
  const body: Omit<CalendarLiveV43SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V43_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV43History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV43History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV43History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV43History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV43History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV43UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v43-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV43UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV43AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV43AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV43AuthorizationArtifact(
        planted as CalendarLiveV43AuthorizationArtifact,
        index(),
...
test("one-root denominator, capture gate, cleanup, and prior-lineage reuse are rejected", () => {
  ...
  for (const mutate of [
    ...
    (value: Record<string, any>) => {
      value.execution.comboboxLiveV43AuthorizationReusable = true;
    },
    (value: Record<string, any>) => {
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV43SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV43SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV43SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v43-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV43ControlFlowSource(source), []);
});
