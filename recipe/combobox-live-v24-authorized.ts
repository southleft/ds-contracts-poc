...
import { COMBOBOX_LIVE_V24_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v24.js";
import { verifyComboboxLiveV24Authorization } from "./combobox-live-v24-authorization.js";
import { runComboboxLiveV24Preflight } from "./combobox-live-v24-preflight.js";
import { ComboboxLiveV24Orchestrator } from "./run-combobox-live-v24.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v24 requires exact authorization artifact ${COMBOBOX_LIVE_V24_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v24-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV24Authorization(root);
    runComboboxLiveV24Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV24Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV24Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
