...
import {
  ...
  verifyInputLiveV64Authorization,
  type InputLiveV64AntecedentIndex,
  type InputLiveV64AuthorizationProof,
} from "./input-field-live-v64-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V64_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v64/operator-security-attestation-template.json";
export const INPUT_LIVE_V64_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v64-security-attestation.json";

export interface InputLiveV64SecurityAttestation {
...
export interface InputLiveV64PreflightReport {
  artifactVersion: "input-live-v64-preflight-v1";
...
export const inputLiveV64AttestationSha256 = (
  value: Omit<InputLiveV64SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV64SecurityAttestation(
  value: unknown,
  proof: InputLiveV64AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v64-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V64_TARGET) ||
...
export function validateInputLiveV64ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV64Authorization");
  const preflight = source.indexOf("runInputLiveV64Preflight");
  const initialize = source.indexOf("InputLiveV64Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV64Preflight(
  root: string,
  proof: InputLiveV64AuthorizationProof,
  attestationPath = INPUT_LIVE_V64_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV64PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V64_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V64_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V64_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV64AntecedentIndex;
  failures.push(...validateInputLiveV64AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V64_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V64_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v64-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV64ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV64SecurityAttestation;
  return {
    artifactVersion: "input-live-v64-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V64_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV64Preflight(
    process.cwd(),
    verifyInputLiveV64Authorization(),
    attestationPath,
