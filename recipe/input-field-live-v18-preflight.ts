...
import {
  ...
  verifyInputLiveV18Authorization,
  type InputLiveV18AntecedentIndex,
  type InputLiveV18AuthorizationProof,
} from "./input-field-live-v18-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V18_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v18/operator-security-attestation-template.json";
export const INPUT_LIVE_V18_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v18-security-attestation.json";

export interface InputLiveV18SecurityAttestation {
...
export interface InputLiveV18PreflightReport {
  artifactVersion: "input-live-v18-preflight-v1";
...
export const inputLiveV18AttestationSha256 = (
  value: Omit<InputLiveV18SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV18SecurityAttestation(
  value: unknown,
  proof: InputLiveV18AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v18-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V18_TARGET) ||
...
export function validateInputLiveV18ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV18Authorization");
  const preflight = source.indexOf("runInputLiveV18Preflight");
  const initialize = source.indexOf("InputLiveV18Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV18Preflight(
  root: string,
  proof: InputLiveV18AuthorizationProof,
  attestationPath = INPUT_LIVE_V18_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV18PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V18_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V18_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V18_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV18AntecedentIndex;
  failures.push(...validateInputLiveV18AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V18_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V18_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v18-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV18ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV18SecurityAttestation;
  return {
    artifactVersion: "input-live-v18-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V18_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV18Preflight(
    process.cwd(),
    verifyInputLiveV18Authorization(),
    attestationPath,
