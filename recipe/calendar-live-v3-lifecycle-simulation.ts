...
import { calendarLiveV1Sha256 } from "./calendar-live-v3-broker.js";

export interface CalendarLiveV3LifecycleSimulationReport {
  artifactVersion: "calendar-live-v3-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV3AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV3Lifecycle(): Promise<CalendarLiveV3LifecycleSimulationReport> {
  await buildCalendarLiveV3Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V3_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV3AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v3-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V3_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V3_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV3History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV3History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV3AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V3_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V3_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV3History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV3History(authorized, "pending");
    ...
    await buildCalendarLiveV3Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V3_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV3AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV3Lifecycle(), null, 2)}\n`,
  );
