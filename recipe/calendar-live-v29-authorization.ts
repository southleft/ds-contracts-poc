...
export type CalendarLiveV29HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV29AuthorizationArtifact {
  artifactVersion: "calendar-live-v29-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V29_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV29AntecedentIndex {
...
export interface CalendarLiveV29HistoryState {
  index?: CalendarLiveV29AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV29AuthorizationArtifact;
...
export interface CalendarLiveV29AuthorizationProof
  extends CalendarLiveV29TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V29_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V29_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V29_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V29_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v29-authorization") ||
  artifactPath.includes("calendar-live-v29-preflight") ||
  artifactPath.includes("calendar-live-v29-authorized") ||
  artifactPath.includes("create-calendar-live-v29-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v29-status") ||
...
export function validateCalendarLiveV29AntecedentIndex(
  index: CalendarLiveV29AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v29-antecedent-index-v1" ||
...
export function validateCalendarLiveV29UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV29History\(\s*["']/.test(source) ||
    /readCalendarLiveV29HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV29AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV29AuthorizationArtifact {
  ...
  ) as CalendarLiveV29AntecedentIndex;
  const indexFailures = validateCalendarLiveV29AntecedentIndex(index);
...
export function validateCalendarLiveV29AuthorizationArtifact(
  artifact: CalendarLiveV29AuthorizationArtifact | undefined,
  index: CalendarLiveV29AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v29-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v29" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V29_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V29_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V29_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V29_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V29_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V29_SOURCE_ROOTS ||
...
export function validateCalendarLiveV29History(
  state: CalendarLiveV29HistoryState,
  expected: CalendarLiveV29HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV29AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV29AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV29HistoryState(
  root = process.cwd(),
): CalendarLiveV29HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V29_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV29AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V29_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V29_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V29_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV29History(
  expected: CalendarLiveV29HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV29HistoryState {
  const state = readCalendarLiveV29HistoryState(root);
  const failures = validateCalendarLiveV29History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v29 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV29Authorization(
  root = process.cwd(),
): CalendarLiveV29AuthorizationProof {
  const state = verifyCalendarLiveV29History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V29_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v29/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v29.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V29_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V29_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V29_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV29HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV29History(expected);
  process.stdout.write(
    `Calendar live v29 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
