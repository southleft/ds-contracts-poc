...
import {
  ...
  INPUT_LIVE_V65_INDEX_PATH,
} from "./build-input-field-live-proof-v65.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV65HistoryExpectation = "pending" | "authorized";

export interface InputLiveV65AuthorizationArtifact {
  artifactVersion: "input-live-v65-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V65_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV65AntecedentIndex {
...
export interface InputLiveV65HistoryState {
  index?: InputLiveV65AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV65AuthorizationArtifact;
...
export interface InputLiveV65AuthorizationProof extends InputLiveV65TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V65_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V65_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V65_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V65_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v65-authorization") ||
  artifactPath.includes("input-field-live-v65-preflight") ||
  artifactPath.includes("input-field-live-v65-authorized") ||
  artifactPath.includes("input-field-live-pivot-v65-status") ||
...
export function validateInputLiveV65AntecedentIndex(
  index: InputLiveV65AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v65-antecedent-index-v1" ||
...
export function validateInputLiveV65UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV65History\(\s*["']/.test(source) ||
    /readInputLiveV65HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV65AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV65AuthorizationArtifact {
  ...
  ) as InputLiveV65AntecedentIndex;
  const indexFailures = validateInputLiveV65AntecedentIndex(index);
...
export function validateInputLiveV65AuthorizationArtifact(
  artifact: InputLiveV65AuthorizationArtifact | undefined,
  index: InputLiveV65AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v65-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v65" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V65_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V65_TARGET) ||
    !equalJson(
...
export function validateInputLiveV65History(
  state: InputLiveV65HistoryState,
  expected: InputLiveV65HistoryExpectation,
): string[] {
  const failures = validateInputLiveV65AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV65AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV65HistoryState(
  root = process.cwd(),
): InputLiveV65HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V65_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV65AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V65_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V65_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V65_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV65History(
  expected: InputLiveV65HistoryExpectation,
  root = process.cwd(),
): InputLiveV65HistoryState {
  const state = readInputLiveV65HistoryState(root);
  const failures = validateInputLiveV65History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV65Authorization(
  root = process.cwd(),
): InputLiveV65AuthorizationProof {
  const state = verifyInputLiveV65History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V65_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v65/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v65.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V65_TARGET,
    expectedDynamicTool: INPUT_LIVE_V65_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V65_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV65HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV65History(expected);
