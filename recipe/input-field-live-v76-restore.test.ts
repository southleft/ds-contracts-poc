...
import {
  INPUT_LIVE_V76_MEASURE_VISIBLE_FILL_MARKER,
  INPUT_LIVE_V76_RESTORE_CONTENT_ROLES,
  INPUT_LIVE_V76_RESTORE_PARENT_ROLES,
...
test("v76 probe teaches Polar reflow against content text, not a minted content-row", () => {
...
test("v16 restore still measures FILL while visible, then restores visibility", () => {
  const program = buildInputLiveV76RestoreProgram(writer);
  assert.match(program, new RegExp(INPUT_LIVE_V76_MEASURE_VISIBLE_FILL_MARKER));
  ...
  assert.match(program, /INPUT-V76-RESTORE-COUNT/);
  assert.match(program, /INPUT-V76-RESTORE-NOT-FILL/);
  ...
  for (const role of INPUT_LIVE_V76_RESTORE_CONTENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
  for (const role of INPUT_LIVE_V76_RESTORE_PARENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
});

test("v16 restore payload requires 256 filled content texts", () => {
  const accepted = validateInputLiveV76RestorePayload(
    {
      pageId: "1:1",
      setIds: ["2:2", "2:1"],
      restoredCount: 256,
      ...
      marker: INPUT_LIVE_V76_MEASURE_VISIBLE_FILL_MARKER,
    },
    writer,
  );
...
test("v16 writer bytes stay frozen and v17 writer assigns set HUG", () => {
  ...
  const v76Program = readFileSync(
    "recipe/evidence/input-field-live-pivot-v76/programs/writer.txt",
  );
  const v76Payload = readFileSync(
    "recipe/evidence/input-field-live-pivot-v76/programs/writer-payload.js",
  );
