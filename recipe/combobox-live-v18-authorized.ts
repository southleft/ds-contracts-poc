...
import { COMBOBOX_LIVE_V18_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v18.js";
import { verifyComboboxLiveV18Authorization } from "./combobox-live-v18-authorization.js";
import { runComboboxLiveV18Preflight } from "./combobox-live-v18-preflight.js";
import { ComboboxLiveV18Orchestrator } from "./run-combobox-live-v18.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v18 requires exact authorization artifact ${COMBOBOX_LIVE_V18_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v18-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV18Authorization(root);
    runComboboxLiveV18Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV18Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV18Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
