...
export const INPUT_LIVE_V30_VERIFIER_VERSION = "input-live-v30-verifier-v1";

export function buildInputLiveV30RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV30RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV30RawNode[];
}

export interface InputLiveV30NormalizedScene {
...
export interface InputLiveV30SceneProof {
...
export function normalizeInputLiveV30Scene(
  root: InputLiveV30RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV30NormalizedScene {
  const canonicalBindings: InputLiveV30NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV30RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV30Scene<Instance>(
  normalized: InputLiveV30NormalizedScene,
...
export function compileInputLiveV30ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV30PreCaptureGates(payload: unknown): void {
