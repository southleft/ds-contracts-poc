...
import { TABLE_LIVE_V6_INDEX_PATH } from "./build-table-live-proof-v6.js";
import {
...
import {
  ...
  validateTableLiveV6UnitTestSource,
  type TableLiveV6AntecedentIndex,
  type TableLiveV6AuthorizationArtifact,
...
import {
  ...
  validateTableLiveV6ControlFlowSource,
  validateTableLiveV6SecurityAttestation,
  type TableLiveV6SecurityAttestation,
} from "./table-live-v6-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V6_INDEX_PATH);
const index = (): TableLiveV6AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV6AntecedentIndex;

const artifact = (): TableLiveV6AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV6AuthorizationArtifact({
...
const pendingState = (): TableLiveV6HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV6HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV6AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v6/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV6AuthorizationArtifact,
): TableLiveV6SecurityAttestation => {
  const body: Omit<TableLiveV6SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V6_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV6History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV6History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV6History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV6History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV6History(authorizedState(), "pending").join("\n"),
    /stale history mode.*authorization exists/,
  );
});

test("the exact v6 phase-sensitive self-test defect is planted and table v6 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV6UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v6-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV6UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV6AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV6AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV6AuthorizationArtifact(
        planted as TableLiveV6AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV6SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV6SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV6SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v6-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV6ControlFlowSource(source), []);
});
