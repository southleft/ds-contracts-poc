...
import {
  ...
  createInputLiveV24FakeDynamicResponse,
  ...
  listInputLiveV24PersistedRequests,
  persistInputLiveV24RawResponse,
  persistInputLiveV24TechnicalGates,
  validateInputLiveV24Receipt,
  validateInputLiveV24Request,
  type InputLiveV24RemotePhase,
...
import {
  ...
  type InputLiveV24ExtractPayload,
  type InputLiveV24ProbePayload,
  type InputLiveV24RestorePayload,
  type InputLiveV24RootProof,
  type InputLiveV24WriterOwnership,
} from "./input-field-live-v24-contract.js";
import {
  InputLiveV24Orchestrator,
  simulatedInputLiveV24Authorization,
} from "./run-input-field-live-v24.js";
...
const captures: InputLiveV24CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV24Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v24-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV24Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV24CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV24FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV24FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V24_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V24_TARGET);
  persistInputLiveV24RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV24Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV24Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV24WriterOwnership,
): InputLiveV24RestorePayload => ({
...
const rootProofs = (): InputLiveV24RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV24CaptureCell,
): InputLiveV24CapturePayload => {
  const bytes = Buffer.from(`input-live-v24-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV24Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV24Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV24RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV24RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV24ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV24ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV24CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV24CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV24CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV24Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV24CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV24PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV24ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV24Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV24Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV24Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV24Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V24_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV24RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V24_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV24Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV24Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV24ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV24Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV24WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV24RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV24Request;
    validateInputLiveV24Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV24Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV24CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v24-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV24Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV24Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV24Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V24_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV24RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V24_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV24Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV24Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV24RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V24_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV24Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV24Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV24Request;
