...
export type InputLiveV24HistoryExpectation = "pending" | "authorized";

export interface InputLiveV24AuthorizationArtifact {
  artifactVersion: "input-live-v24-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V24_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV24AntecedentIndex {
...
export interface InputLiveV24HistoryState {
  ...
  authorization?: InputLiveV24AuthorizationArtifact;
...
export interface InputLiveV24AuthorizationProof extends InputLiveV24TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V24_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V24_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V24_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V24_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v24-status") ||
...
export function validateInputLiveV24AntecedentIndex(
  index: InputLiveV24AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v24-antecedent-index-v1" ||
...
export function validateInputLiveV24UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV24History\(\s*["']/.test(source) ||
    /readInputLiveV24HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV24AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV24AuthorizationArtifact {
  ...
  const indexFailures = validateInputLiveV24AntecedentIndex(index);
...
export function validateInputLiveV24AuthorizationArtifact(
  artifact: InputLiveV24AuthorizationArtifact | undefined,
  index: InputLiveV24AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v24-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v24" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V24_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V24_TARGET) ||
    !equalJson(
...
export function validateInputLiveV24History(
  state: InputLiveV24HistoryState,
  expected: InputLiveV24HistoryExpectation,
): string[] {
  const failures = validateInputLiveV24AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV24AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV24HistoryState(
  root = process.cwd(),
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V24_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV24AuthorizationArtifact)
    : undefined;
  ...
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V24_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V24_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV24History(
  expected: InputLiveV24HistoryExpectation,
  root = process.cwd(),
): InputLiveV24HistoryState {
  const state = readInputLiveV24HistoryState(root);
  const failures = validateInputLiveV24History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV24Authorization(
  root = process.cwd(),
): InputLiveV24AuthorizationProof {
  const state = verifyInputLiveV24History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V24_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v24/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v24.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V24_TARGET,
    expectedDynamicTool: INPUT_LIVE_V24_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V24_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV24HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV24History(expected);
