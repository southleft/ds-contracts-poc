...
import { buildCalendarLiveV2CleanupRuntime } from "./calendar-live-v2-cleanup.js";
import { type CalendarLiveV2FixedFixedPoint } from "./calendar-live-v2-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV2RestoreProgram,
  validateCalendarLiveV2RestorePayload,
  type CalendarLiveV2RestorePayload,
} from "./calendar-live-v2-restore.js";
import {
  buildCalendarLiveV2RawPropertyRuntime,
  normalizeCalendarLiveV2Scene,
  type CalendarLiveV2RawNode,
} from "./calendar-live-v2-verifier.js";
...
export const CALENDAR_LIVE_V2_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V2_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V2_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V2_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V2_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V2_SET_COUNT = 3;
export const CALENDAR_LIVE_V2_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V2_HOST_PHASES = 3;
export const CALENDAR_LIVE_V2_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V2_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V2_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V2_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV2SourceId = (typeof CALENDAR_LIVE_V2_SOURCE_IDS)[number];
export type CalendarLiveV2Kind = "calendar" | "week" | "day";

export interface CalendarLiveV2SourceIdentity {
  source: CalendarLiveV2SourceId;
...
export interface CalendarLiveV2WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV2ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV2SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV2RawNode;
    weekScene: CalendarLiveV2RawNode;
    dayScene: CalendarLiveV2RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV2RootProof {
  source: CalendarLiveV2SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV2FixedFixedPoint;
}

export interface CalendarLiveV2ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV2SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV2CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV2SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV2CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV2SourceId;
  frameWidth: number;
...
export interface CalendarLiveV2CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v2 ${label} must be an object`);
...
export function validateCalendarLiveV2WriterPayload(
  value: unknown,
): CalendarLiveV2WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v2") ||
    ...
    result.namespace !== CALENDAR_LIVE_V2_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v2 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v2 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v2 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV2SourceIdentity,
  kind: CalendarLiveV2Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV2ExtractProgram(
  writer: CalendarLiveV2WriterOwnership,
  sources: readonly CalendarLiveV2SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v2 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V2_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V2_NAMESPACE)}
${buildCalendarLiveV2RawPropertyRuntime()}
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("CALENDAR-V2-EXTRACT-PAGE");
...
if(get(page,"pageOwner")!=="recipe/calendar/"+${JSON.stringify(writer.runIdentity)})throw new Error("CALENDAR-V2-EXTRACT-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>SET_IDS.has(node.id));
if(sets.length!==3)throw new Error("CALENDAR-V2-EXTRACT-ROOTS:"+sets.length);
...
for(const set of sets){
  ...
  if(!IDENTITIES[adapterIdentity]||(kind!=="calendar"&&kind!=="week"&&kind!=="day"))throw new Error("CALENDAR-V2-EXTRACT-ADAPTER:"+adapterIdentity+":"+kind);
...
if(roots.length!==1||roots.some(root=>!root.calendarScene||!root.weekScene||!root.dayScene))throw new Error("CALENDAR-V2-EXTRACT-TRIPLE");
...
export function validateCalendarLiveV2ExtractPayload(
  value: unknown,
  writer: CalendarLiveV2WriterOwnership,
): CalendarLiveV2ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v2 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V2_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v2 extract schema/one-root mismatch");
  return payload as CalendarLiveV2ExtractPayload;
}

export function proveCalendarLiveV2Roots<Instance>(
  extract: CalendarLiveV2ExtractPayload,
  sources: readonly (CalendarLiveV2SourceIdentity & {
...
): CalendarLiveV2RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v2 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v2 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV2Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV2Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV2Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV2FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v2 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v2 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v2 compile lost owned sets");
...
export function assertCalendarLiveV2RootProofs(
  proofs: readonly CalendarLiveV2RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v2 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v2 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV2ProbeProgram(
  writer: CalendarLiveV2WriterOwnership,
  sources: readonly CalendarLiveV2SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V2_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE"||page.id==="115:295378"||page.id==="163:35981")throw new Error("CALENDAR-V2-PROBE-PAGE");
...
if(sets.length!==3)throw new Error("CALENDAR-V2-PROBE-ROOTS:"+sets.length);
...
for(const row of byAdapter.values()){
 ...
 if(!source||!calendar||!week||!day||calendar.children.length!==2||week.children.length!==2||day.children.length!==4)throw new Error("CALENDAR-V2-PROBE-VARIANTS:"+adapterIdentity);
...
export function validateCalendarLiveV2ProbePayload(
  value: unknown,
  writer: CalendarLiveV2WriterOwnership,
): CalendarLiveV2ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV2ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V2_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V2_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v2 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV2CaptureManifest(
  cells: readonly CalendarLiveV2CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V2_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V2_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V2_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V2_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v2 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV2CaptureCell[],
): string => {
  validateCalendarLiveV2CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV2CaptureProgram(
  writer: CalendarLiveV2WriterOwnership,
  cell: CalendarLiveV2CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V2_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v2 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v2 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
if(!page||page.type!=="PAGE"||page.id==="115:295378"||page.id==="163:35981"||!set||set.type!=="COMPONENT_SET")throw new Error("CALENDAR-V2-CAPTURE-TARGET");
const axes=node=>Object.fromEntries(node.name.split(",").map(part=>part.trim()).filter(part=>part.includes("=")).map(part=>{const at=part.indexOf("=");return[part.slice(0,at),part.slice(at+1)];}));
...
if(!component)throw new Error("CALENDAR-V2-CAPTURE-CELL:"+cell.cellKey);
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V2_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV2CapturePayload(
  value: unknown,
  cell: CalendarLiveV2CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV2CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV2CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V2_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V2_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v2 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV2CaptureResponses(
  manifest: readonly CalendarLiveV2CaptureCell[],
  responses: readonly CalendarLiveV2CapturePayload[],
): void {
  validateCalendarLiveV2CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v2 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV2ObjectiveReport {
  artifactVersion: "calendar-live-v2-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV2Objective(
  manifest: readonly CalendarLiveV2CaptureCell[],
  responses: readonly CalendarLiveV2CapturePayload[],
): CalendarLiveV2ObjectiveReport {
  assertCalendarLiveV2CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v2-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV2CleanupProgram(
  writer: CalendarLiveV2WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV2CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V2_NAMESPACE,
  ...
  const guardedResult = `if(JSON.stringify(requestedNodeIds)!==JSON.stringify([expectedPageId])||JSON.stringify([...requestedCollectionIds].sort())!==JSON.stringify([...expectedCollectionIds].sort()))throw new Error("CALENDAR-V2-CLEANUP-EXACT-OWNERSHIP");${resultStatement}`;
  const program = baseProgram.replace(resultStatement, guardedResult);
  if (program === baseProgram)
    throw new Error("Calendar live v2 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV2CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV2WriterOwnership,
): CalendarLiveV2CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV2CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v2 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V2_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV2WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
