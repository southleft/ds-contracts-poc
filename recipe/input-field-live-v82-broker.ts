...
import {
  ...
  INPUT_LIVE_V82_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V82_RESPONSE_CONTRACTS,
  type InputLiveV82RootProof,
} from "./input-field-live-v82-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V82_BROKER_VERSION = "input-live-v82-broker-v2";
export const INPUT_LIVE_V82_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V82_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V82_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V82_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V82_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V82_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV82RemotePhase =
  (typeof INPUT_LIVE_V82_REMOTE_PHASES)[number];
export type InputLiveV82AuthorizationMode = "live" | "simulated";

export interface InputLiveV82CommitPins {
...
export interface InputLiveV82TransactionAuthorization extends InputLiveV82CommitPins {
  mode: InputLiveV82AuthorizationMode;
...
export interface InputLiveV82Manifest {
  artifactVersion: "input-live-v82-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V82_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V82_DYNAMIC_TOOL;
...
export interface InputLiveV82ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV82RequestBody {
  artifactVersion: "input-live-v82-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V82_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV82RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV82CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V82_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV82ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV82Request extends InputLiveV82RequestBody {
...
export interface InputLiveV82Receipt {
  artifactVersion: "input-live-v82-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V82_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV82RemotePhase;
...
export interface InputLiveV82AcceptedResponse<T = unknown> {
  request: InputLiveV82Request;
  receipt: InputLiveV82Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV82IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV82ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V82_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V82_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V82_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V82_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V82_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V82_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V82_TARGET.editorType;
};
...
export const inputLiveV82RequestSequence = (
  phase: InputLiveV82RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV82RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV82RequestPath = (
  directory: string,
  phase: InputLiveV82RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV82ResponsePath = (
  directory: string,
  phase: InputLiveV82RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV82ReceiptPath = (
  directory: string,
  phase: InputLiveV82RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV82Manifest => {
  const manifest = json<InputLiveV82Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v82-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V82_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V82_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV82RemotePhase,
  captureIndex?: number,
): InputLiveV82Request | undefined => {
  const file = inputLiveV82RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV82Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV82RemotePhase,
  captureIndex?: number,
): InputLiveV82Receipt | undefined => {
  const file = inputLiveV82ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV82Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV82Request | undefined;
  receipt: InputLiveV82Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV82RemotePhase,
): InputLiveV82ResponseContract => ({
  ...INPUT_LIVE_V82_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V82_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v82-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V82_BROKER_VERSION,
  target: INPUT_LIVE_V82_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V82_PROGRAM_START*/${program}/*INPUT_LIVE_V82_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V82_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V82_PROGRAM_END*/";
...
export function createInputLiveV82Transaction(
  directory: string,
  ...
  const manifest: InputLiveV82Manifest = {
    artifactVersion: "input-live-v82-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V82_BROKER_VERSION,
...
export function validateInputLiveV82Request(
  directory: string,
  request: InputLiveV82Request,
): void {
  ...
  const expectedSequence = inputLiveV82RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v82-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V82_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V82_TARGET.fileKey ||
...
export function issueInputLiveV82Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV82RemotePhase,
  program: string,
  options: InputLiveV82IssueOptions = {},
): InputLiveV82Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV82RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV82Request>(targetPath);
    validateInputLiveV82Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV82ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV82TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV82RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V82_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V82_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V82_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV82RequestBody = {
    artifactVersion: "input-live-v82-broker-request-v2",
    brokerVersion: INPUT_LIVE_V82_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV82Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV82Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV82RawResponse(
  directory: string,
  phase: InputLiveV82RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV82RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV82Request>(requestFile);
  if (
  ...
  const target = inputLiveV82ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV82Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v82-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V82_BROKER_VERSION,
...
export function acceptInputLiveV82Response<T = unknown>(
  directory: string,
  phase: InputLiveV82RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV82AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV82ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV82Request>(
    inputLiveV82RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV82Request(directory, request);
  if (phase === "capture") validateInputLiveV82TechnicalGates(directory);
  const rawFile = inputLiveV82ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v82-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V82_BROKER_VERSION ||
...
export function persistInputLiveV82TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV82RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV82RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V82_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v82-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V82_BROKER_VERSION,
...
export function validateInputLiveV82TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v82-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V82_TECHNICAL_GATES.some(
...
export function validateInputLiveV82Receipt(
  directory: string,
  phase: InputLiveV82RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV82Request(directory, request);
  const receipt = json<InputLiveV82Receipt>(
    inputLiveV82ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV82ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v82-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V82_BROKER_VERSION ||
...
export function readInputLiveV82AcceptedRawPayload(
  directory: string,
  phase: InputLiveV82RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV82Receipt(directory, phase, captureIndex);
...
export function createInputLiveV82FakeDynamicResponse(
  request: InputLiveV82Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V82_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V82_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV82PersistedRequests(
  directory: string,
): InputLiveV82Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV82Request>(file)] : [];
    });
}

export const inputLiveV82Sha256 = sha256;
