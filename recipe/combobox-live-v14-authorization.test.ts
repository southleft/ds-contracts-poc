...
import { COMBOBOX_LIVE_V14_INDEX_PATH } from "./build-combobox-live-proof-v14.js";
import {
  COMBOBOX_LIVE_V14_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V14_TARGET,
} from "./combobox-live-v14-broker.js";
import {
  buildComboboxLiveV14AuthorizationArtifact,
  validateComboboxLiveV14AntecedentIndex,
  validateComboboxLiveV14AuthorizationArtifact,
  validateComboboxLiveV14History,
  validateComboboxLiveV14UnitTestSource,
  type ComboboxLiveV14AntecedentIndex,
  type ComboboxLiveV14AuthorizationArtifact,
  type ComboboxLiveV14AuthorizationProof,
  type ComboboxLiveV14HistoryState,
} from "./combobox-live-v14-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV14ControlFlowSource,
  validateComboboxLiveV14SecurityAttestation,
  type ComboboxLiveV14SecurityAttestation,
} from "./combobox-live-v14-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v14-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V14_INDEX_PATH);
const index = (): ComboboxLiveV14AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV14AntecedentIndex;

const artifact = (): ComboboxLiveV14AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV14AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV14HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV14HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV14AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v14/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV14AuthorizationArtifact,
): ComboboxLiveV14SecurityAttestation => {
  const body: Omit<ComboboxLiveV14SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V14_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV14History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV14History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV14History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV14History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV14History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV14UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV14UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV14AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV14AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV14AuthorizationArtifact(
        planted as ComboboxLiveV14AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV14SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV14SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV14SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV14ControlFlowSource(source), []);
});
