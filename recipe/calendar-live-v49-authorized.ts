...
import { CALENDAR_LIVE_V49_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v49.js";
import { verifyCalendarLiveV49Authorization } from "./calendar-live-v49-authorization.js";
import { runCalendarLiveV49Preflight } from "./calendar-live-v49-preflight.js";
import { CalendarLiveV49Orchestrator } from "./run-calendar-live-v49.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v49 requires exact authorization artifact ${CALENDAR_LIVE_V49_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v49-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV49Authorization(root);
    runCalendarLiveV49Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV49Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV49Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
