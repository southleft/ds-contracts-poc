...
import {
  buildInputLiveV4RawPropertyRuntime,
...
export const INPUT_LIVE_V6_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V6_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V6_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V6_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V6_VARIANT_COUNT = 256;
export const INPUT_LIVE_V6_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V6_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV6SourceId = (typeof INPUT_LIVE_V6_SOURCE_IDS)[number];

export interface InputLiveV6SourceIdentity {
  source: InputLiveV6SourceId;
...
export interface InputLiveV6WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV6ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV6SourceId;
    adapterIdentity: string;
...
export interface InputLiveV6RootProof {
  source: InputLiveV6SourceId;
  adapterIdentity: string;
...
export interface InputLiveV6ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV6SourceId;
    adapterIdentity: string;
...
export interface InputLiveV6CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV6SourceId;
  adapterIdentity: string;
...
export interface InputLiveV6CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV6SourceId;
  strata: InputLiveV6CaptureCell["strata"];
...
export interface InputLiveV6CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Input live v6 ${label} must be an object`);
...
const exactUniqueStrings = (
  ...
  if (
    ...
    throw new TypeError(`Input live v6 ${label} cardinality/identity mismatch`);
  return value;
};
const finiteNonnegative = (value: unknown): value is number =>
  typeof value === "number" && Number.isFinite(value) && value >= 0;

export function validateInputLiveV6WriterPayload(
  value: unknown,
): InputLiveV6WriterOwnership {
  ...
  if (
    ...
    throw new TypeError("Input live v6 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Input live v6 writer created-node denominator invalid",
    );
  return {
...
const identityPlan = (source: InputLiveV6SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV6ExtractProgram(
  writer: InputLiveV6WriterOwnership,
  sources: readonly InputLiveV6SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Input live v6 extract requires two source identities");
...
const V6_NS=${JSON.stringify(INPUT_LIVE_V6_NAMESPACE)};
const V6_PAGE_ID=${JSON.stringify(writer.pageId)};
const V6_SET_IDS=new Set(${JSON.stringify(writer.setIds)});
const V6_SOURCE_BY_ADAPTER=${JSON.stringify(
    Object.fromEntries(
      sources.map((source) => [source.adapterIdentity, source.source]),
    ),
  )};
const V6_IDENTITIES=${JSON.stringify(identities)};
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V6_NAMESPACE)}
${buildInputLiveV4RawPropertyRuntime()}
const page=await figma.getNodeByIdAsync(V6_PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("INPUT-V6-EXTRACT-PAGE");
const get=(node,key)=>node.getSharedPluginData(V6_NS,key);
if(get(page,"pageOwner")!=="recipe/input-field/"+${JSON.stringify(writer.runIdentity)})throw new Error("INPUT-V6-EXTRACT-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>V6_SET_IDS.has(node.id));
if(sets.length!==2)throw new Error("INPUT-V6-EXTRACT-ROOTS:"+sets.length);
...
for(const set of sets){
  const adapterIdentity=get(set,"adapterIdentity"),expected=V6_IDENTITIES[adapterIdentity];
  if(!expected)throw new Error("INPUT-V6-EXTRACT-ADAPTER:"+adapterIdentity);
  ...
  roots.push({source:V6_SOURCE_BY_ADAPTER[adapterIdentity],adapterIdentity,setId:set.id,scene,lineage:await lineageFor(set)});
}
...
export function validateInputLiveV6ExtractPayload(
  value: unknown,
  writer: InputLiveV6WriterOwnership,
  ...
  if (leaks.length)
    throw new TypeError(
      `Input live v6 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V6_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Input live v6 extract schema/two-root mismatch");
  return payload as InputLiveV6ExtractPayload;
}

export function proveInputLiveV6Roots<Instance>(
  extract: InputLiveV6ExtractPayload,
  sources: readonly (InputLiveV6SourceIdentity & {
...
): InputLiveV6RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Input live v6 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Input live v6 omitted ${source.source} root`);
...
export function assertInputLiveV6RootProofs(
  proofs: readonly InputLiveV6RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Input live v6 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Input live v6 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildInputLiveV6ProbeProgram(
  writer: InputLiveV6WriterOwnership,
  sources: readonly InputLiveV6SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V6_NS=${JSON.stringify(INPUT_LIVE_V6_NAMESPACE)},V6_PAGE_ID=${JSON.stringify(writer.pageId)},V6_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V6_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V6_PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("INPUT-V6-PROBE-PAGE");
const get=(node,key)=>node.getSharedPluginData(V6_NS,key),sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>V6_SET_IDS.has(node.id));
if(sets.length!==2)throw new Error("INPUT-V6-PROBE-ROOTS:"+sets.length);
...
const sources=[],cells=[];
for(const set of sets){
 const adapterIdentity=get(set,"adapterIdentity"),source=V6_SOURCE_BY_ADAPTER[adapterIdentity],components=[...set.children];
 if(!source||components.length!==128)throw new Error("INPUT-V6-PROBE-VARIANTS:"+adapterIdentity+":"+components.length);
 ...
 for(const component of components){const target=axes(component),updates={};for(const name of axisNames){const key=propertyKey(instance,name);if(!key)throw new Error("INPUT-V6-PROBE-AXIS:"+name);updates[key]=target[name];}instance.setProperties(updates);const main=await instance.getMainComponentAsync();if(main)visited.add(main.id);}
 instance.setProperties(original);const labelKey=propertyKey(instance,"Label"),labelBefore=labelKey&&instance.componentProperties[labelKey].value;let textPropertiesRestored=false;if(labelKey){instance.setProperties({[labelKey]:"Input v6 deterministic probe"});const changed=nodes(instance).some(node=>node.type==="TEXT"&&role(node)==="input-field/label"&&node.characters==="Input v6 deterministic probe");instance.setProperties({[labelKey]:labelBefore});textPropertiesRestored=changed&&JSON.stringify(original)===JSON.stringify(plain(instance));}
...
export function validateInputLiveV6ProbePayload(
  value: unknown,
  writer: InputLiveV6WriterOwnership,
): InputLiveV6ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV6ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V6_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V6_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Input live v6 probe/usability/restoration failed");
  return payload;
}

export function validateInputLiveV6CaptureManifest(
  cells: readonly InputLiveV6CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V6_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V6_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V6_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V6_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V6_SOURCE_IDS.some(
      (source) => cells.filter((cell) => cell.source === source).length !== 64,
    )
  )
    throw new TypeError(
      "Input live v6 capture manifest must be exact 128 cells",
    );
}

export const inputLiveV6CaptureManifestSha256 = (
  cells: readonly InputLiveV6CaptureCell[],
): string => {
  validateInputLiveV6CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV6CaptureProgram(
  writer: InputLiveV6WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V6_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V6_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
    ...
    throw new TypeError("Input live v6 capture cell is malformed");
  const expectedSet = writer.sources.find(
    (source) => source.adapterIdentity === cell.adapterIdentity,
  )?.setId;
  if (!expectedSet)
    throw new TypeError(
      `Input live v6 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
if(!page||page.type!=="PAGE"||!set||set.type!=="COMPONENT_SET")throw new Error("INPUT-V6-CAPTURE-TARGET");
const axes=node=>Object.fromEntries(node.name.split(",").map(part=>part.trim()).filter(part=>part.includes("=")).map(part=>{const at=part.indexOf("=");return[part.slice(0,at),part.slice(at+1)];}));
...
if(!component)throw new Error("INPUT-V6-CAPTURE-CELL:"+cell.cellKey);
const frame=figma.createFrame();frame.name="Input v6 ephemeral / "+cell.cellKey;page.appendChild(frame);
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V6_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V6-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV6CapturePayload(
  value: unknown,
  cell: InputLiveV6CaptureCell,
  rawResponseBytes: number,
): InputLiveV6CapturePayload {
  const payload = record(value, "capture payload") as InputLiveV6CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V6_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V6_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Input live v6 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertInputLiveV6CaptureResponses(
  manifest: readonly InputLiveV6CaptureCell[],
  responses: readonly InputLiveV6CapturePayload[],
): void {
  validateInputLiveV6CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Input live v6 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface InputLiveV6ObjectiveReport {
  artifactVersion: "input-live-v6-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV6Objective(
  manifest: readonly InputLiveV6CaptureCell[],
  responses: readonly InputLiveV6CapturePayload[],
  readReference: (cell: InputLiveV6CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV6CaptureCell) => Buffer,
): InputLiveV6ObjectiveReport {
  assertInputLiveV6CaptureResponses(manifest, responses);
  const rows = manifest.map((cell, index) => {
    ...
    if (
      ...
      throw new TypeError(
        `Input live v6 objective byte drift: ${cell.cellKey}`,
      );
    ...
    if (
      ...
      throw new TypeError(
        `Input live v6 objective measurement refused: ${cell.cellKey}`,
      );
    return {
      index: cell.index,
...
export function buildInputLiveV6CleanupProgram(
  writer: InputLiveV6WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V6_NAMESPACE,
  ...
  const guardedResult = `if(JSON.stringify(requestedNodeIds)!==JSON.stringify([expectedPageId])||JSON.stringify([...requestedCollectionIds].sort())!==JSON.stringify([...expectedCollectionIds].sort()))throw new Error("INPUT-V6-CLEANUP-EXACT-OWNERSHIP");${resultStatement}`;
  const program = baseProgram.replace(resultStatement, guardedResult);
  if (program === baseProgram)
    throw new Error("Input live v6 cleanup runtime result hook absent");
  return String.raw`
...
export function validateInputLiveV6CleanupPayload(
  value: unknown,
  writer: InputLiveV6WriterOwnership,
): InputLiveV6CleanupPayload {
  const payload = record(value, "cleanup payload") as InputLiveV6CleanupPayload;
  ...
  if (
    ...
    throw new TypeError("Input live v6 cleanup is incomplete or overbroad");
  return payload;
}

export const INPUT_LIVE_V6_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV6WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
