...
import {
  ...
  createInputLiveV59FakeDynamicResponse,
  ...
  listInputLiveV59PersistedRequests,
  persistInputLiveV59RawResponse,
  persistInputLiveV59TechnicalGates,
  validateInputLiveV59Receipt,
  validateInputLiveV59Request,
  type InputLiveV59RemotePhase,
...
import {
  ...
  type InputLiveV59ExtractPayload,
  type InputLiveV59ProbePayload,
  type InputLiveV59RestorePayload,
  type InputLiveV59RootProof,
  type InputLiveV59WriterOwnership,
} from "./input-field-live-v59-contract.js";
import {
  InputLiveV59Orchestrator,
  simulatedInputLiveV59Authorization,
} from "./run-input-field-live-v59.js";
...
const captures: InputLiveV59CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV59Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v59-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV59Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV59CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV59FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV59FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V59_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V59_TARGET);
  persistInputLiveV59RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV59Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV59Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV59WriterOwnership,
): InputLiveV59RestorePayload => ({
...
const rootProofs = (): InputLiveV59RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV59CaptureCell,
): InputLiveV59CapturePayload => {
  const bytes = Buffer.from(`input-live-v59-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV59Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV59Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV59RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV59RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV59ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV59ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV59CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV59CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV59CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV59Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV59CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV59PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV59ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV59Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV59Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV59Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV59Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V59_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV59RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V59_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV59Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV59Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV59ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV59Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV59WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV59RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV59Request;
    validateInputLiveV59Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV59Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV59CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v59-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV59Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV59Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV59Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V59_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV59RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V59_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV59Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV59Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV59RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V59_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV59Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV59Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV59Request;
