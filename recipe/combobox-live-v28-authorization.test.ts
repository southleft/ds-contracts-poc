...
import { COMBOBOX_LIVE_V28_INDEX_PATH } from "./build-combobox-live-proof-v28.js";
import {
  COMBOBOX_LIVE_V28_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V28_TARGET,
} from "./combobox-live-v28-broker.js";
import {
  buildComboboxLiveV28AuthorizationArtifact,
  validateComboboxLiveV28AntecedentIndex,
  validateComboboxLiveV28AuthorizationArtifact,
  validateComboboxLiveV28History,
  validateComboboxLiveV28UnitTestSource,
  type ComboboxLiveV28AntecedentIndex,
  type ComboboxLiveV28AuthorizationArtifact,
  type ComboboxLiveV28AuthorizationProof,
  type ComboboxLiveV28HistoryState,
} from "./combobox-live-v28-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV28ControlFlowSource,
  validateComboboxLiveV28SecurityAttestation,
  type ComboboxLiveV28SecurityAttestation,
} from "./combobox-live-v28-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v28-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V28_INDEX_PATH);
const index = (): ComboboxLiveV28AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV28AntecedentIndex;

const artifact = (): ComboboxLiveV28AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV28AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV28HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV28HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV28AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v28/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV28AuthorizationArtifact,
): ComboboxLiveV28SecurityAttestation => {
  const body: Omit<ComboboxLiveV28SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V28_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV28History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV28History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV28History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV28History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV28History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV28UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV28UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV28AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV28AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV28AuthorizationArtifact(
        planted as ComboboxLiveV28AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV28SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV28SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV28SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV28ControlFlowSource(source), []);
});
