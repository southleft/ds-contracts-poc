...
import {
  verifyComboboxLiveV4FixedSceneFixedPoint,
  type ComboboxLiveV4FixedFixedPoint,
} from "./combobox-live-v4-fixed-point.js";

export const COMBOBOX_LIVE_V4_VERIFIER_VERSION = "combobox-live-v4-verifier-v1";

export function buildComboboxLiveV4RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV4RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV4RawNode[];
}

export interface ComboboxLiveV4NormalizedScene {
...
export interface ComboboxLiveV4SceneProof {
...
export function normalizeComboboxLiveV4Scene(
  root: ComboboxLiveV4RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV4NormalizedScene {
  const canonicalBindings: ComboboxLiveV4NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV4RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV4Scene<Instance>(
  normalized: ComboboxLiveV4NormalizedScene,
...
): ComboboxLiveV4SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV4FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV4ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV4PreCaptureGates(payload: unknown): void {
