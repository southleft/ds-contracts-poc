...
import { buildComboboxLiveV39CleanupRuntime } from "./combobox-live-v39-cleanup.js";
import {
  type ComboboxLiveV39FixedFixedPoint,
} from "./combobox-live-v39-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV39RestoreProgram,
  validateComboboxLiveV39RestorePayload,
  type ComboboxLiveV39RestorePayload,
} from "./combobox-live-v39-restore.js";
import {
  buildComboboxLiveV39RawPropertyRuntime,
  normalizeComboboxLiveV39Scene,
  type ComboboxLiveV39RawNode,
} from "./combobox-live-v39-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v39.js";
import {
...
} from "./scene-readback-combobox-v39.js";
...
export const COMBOBOX_LIVE_V39_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V39_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V39_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V39_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V39_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V39_SET_COUNT = 4;
export const COMBOBOX_LIVE_V39_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V39_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V39_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V39_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V39_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV39SourceId = (typeof COMBOBOX_LIVE_V39_SOURCE_IDS)[number];

export interface ComboboxLiveV39SourceIdentity {
  source: ComboboxLiveV39SourceId;
...
export interface ComboboxLiveV39WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV39ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV39SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV39RawNode;
    optionScene: ComboboxLiveV39RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV39RootProof {
  source: ComboboxLiveV39SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV39FixedFixedPoint;
}

export interface ComboboxLiveV39ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV39SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV39CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV39SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV39CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV39SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV39CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v39 ${label} must be an object`);
...
export function validateComboboxLiveV39WriterPayload(
  value: unknown,
): ComboboxLiveV39WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V39_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v39 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v39 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v39 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV39SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV39ExtractProgram(
  writer: ComboboxLiveV39WriterOwnership,
  sources: readonly ComboboxLiveV39SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v39 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V39_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V39_NAMESPACE)}
${buildComboboxLiveV39RawPropertyRuntime()}
...
export function validateComboboxLiveV39ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV39WriterOwnership,
): ComboboxLiveV39ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v39 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V39_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v39 extract schema/two-root mismatch");
  return payload as ComboboxLiveV39ExtractPayload;
}

export function proveComboboxLiveV39Roots<Instance>(
  extract: ComboboxLiveV39ExtractPayload,
  sources: readonly (ComboboxLiveV39SourceIdentity & {
...
): ComboboxLiveV39RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v39 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v39 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV39Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV39Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV39FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v39 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v39 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v39 compile lost owned sets");
...
export function assertComboboxLiveV39RootProofs(
  proofs: readonly ComboboxLiveV39RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v39 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v39 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV39ProbeProgram(
  writer: ComboboxLiveV39WriterOwnership,
  sources: readonly ComboboxLiveV39SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V39_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV39ProbePayload(
  value: unknown,
  writer: ComboboxLiveV39WriterOwnership,
): ComboboxLiveV39ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV39ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V39_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V39_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v39 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV39CaptureManifest(
  cells: readonly ComboboxLiveV39CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V39_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V39_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V39_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V39_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v39 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV39CaptureCell[],
): string => {
  validateComboboxLiveV39CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV39CaptureProgram(
  writer: ComboboxLiveV39WriterOwnership,
  cell: ComboboxLiveV39CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V39_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v39 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v39 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V39_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV39CapturePayload(
  value: unknown,
  cell: ComboboxLiveV39CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV39CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV39CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V39_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V39_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v39 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV39CaptureResponses(
  manifest: readonly ComboboxLiveV39CaptureCell[],
  responses: readonly ComboboxLiveV39CapturePayload[],
): void {
  validateComboboxLiveV39CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v39 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV39ObjectiveReport {
  artifactVersion: "combobox-live-v39-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV39Objective(
  manifest: readonly ComboboxLiveV39CaptureCell[],
  responses: readonly ComboboxLiveV39CapturePayload[],
): ComboboxLiveV39ObjectiveReport {
  assertComboboxLiveV39CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v39-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV39CleanupProgram(
  writer: ComboboxLiveV39WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV39CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V39_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v39 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV39CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV39WriterOwnership,
): ComboboxLiveV39CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV39CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v39 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V39_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV39WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
