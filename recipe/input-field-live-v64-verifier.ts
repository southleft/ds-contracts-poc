...
export const INPUT_LIVE_V64_VERIFIER_VERSION = "input-live-v64-verifier-v1";

export function buildInputLiveV64RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV64RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV64RawNode[];
}

export interface InputLiveV64NormalizedScene {
...
export interface InputLiveV64SceneProof {
...
export function normalizeInputLiveV64Scene(
  root: InputLiveV64RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV64NormalizedScene {
  const canonicalBindings: InputLiveV64NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV64RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV64Scene<Instance>(
  normalized: InputLiveV64NormalizedScene,
...
export function compileInputLiveV64ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV64PreCaptureGates(payload: unknown): void {
