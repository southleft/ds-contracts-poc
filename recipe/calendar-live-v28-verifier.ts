...
import {
  verifyCalendarLiveV28FixedSceneFixedPoint,
  type CalendarLiveV28FixedFixedPoint,
} from "./calendar-live-v28-fixed-point.js";

export const CALENDAR_LIVE_V28_VERIFIER_VERSION = "calendar-live-v28-verifier-v1";

export function buildCalendarLiveV28RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV28RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV28RawNode[];
}

export interface CalendarLiveV28NormalizedScene {
...
export interface CalendarLiveV28SceneProof {
...
export function normalizeCalendarLiveV28Scene(
  root: CalendarLiveV28RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV28NormalizedScene {
  const canonicalBindings: CalendarLiveV28NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV28RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV28Scene<Instance>(
  normalized: CalendarLiveV28NormalizedScene,
...
): CalendarLiveV28SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV28FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV28ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV28PreCaptureGates(payload: unknown): void {
