...
export type TableLiveV19HistoryExpectation = "pending" | "authorized";

export interface TableLiveV19AuthorizationArtifact {
  artifactVersion: "table-live-v19-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V19_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV19AntecedentIndex {
...
export interface TableLiveV19HistoryState {
  index?: TableLiveV19AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV19AuthorizationArtifact;
...
export interface TableLiveV19AuthorizationProof
  extends TableLiveV19TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V19_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V19_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V19_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V19_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v19-authorization") ||
  artifactPath.includes("table-live-v19-preflight") ||
  artifactPath.includes("table-live-v19-authorized") ||
  artifactPath.includes("create-table-live-v19-security-attestation") ||
  artifactPath.includes("table-live-pivot-v19-status") ||
...
export function validateTableLiveV19AntecedentIndex(
  index: TableLiveV19AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v19-antecedent-index-v1" ||
...
export function validateTableLiveV19UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV19History\(\s*["']/.test(source) ||
    /readTableLiveV19HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV19AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV19AuthorizationArtifact {
  ...
  ) as TableLiveV19AntecedentIndex;
  const indexFailures = validateTableLiveV19AntecedentIndex(index);
...
export function validateTableLiveV19AuthorizationArtifact(
  artifact: TableLiveV19AuthorizationArtifact | undefined,
  index: TableLiveV19AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v19-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v19" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V19_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V19_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V19_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V19_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V19_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V19_SOURCE_ROOTS ||
...
export function validateTableLiveV19History(
  state: TableLiveV19HistoryState,
  expected: TableLiveV19HistoryExpectation,
): string[] {
  const failures = validateTableLiveV19AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV19AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV19HistoryState(
  root = process.cwd(),
): TableLiveV19HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V19_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV19AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V19_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V19_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V19_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV19History(
  expected: TableLiveV19HistoryExpectation,
  root = process.cwd(),
): TableLiveV19HistoryState {
  const state = readTableLiveV19HistoryState(root);
  const failures = validateTableLiveV19History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v19 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV19Authorization(
  root = process.cwd(),
): TableLiveV19AuthorizationProof {
  const state = verifyTableLiveV19History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V19_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v19/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v19.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V19_TARGET,
    expectedDynamicTool: TABLE_LIVE_V19_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V19_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV19HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV19History(expected);
  process.stdout.write(
    `Table live v19 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
