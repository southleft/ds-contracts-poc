...
import { INPUT_LIVE_V81_INDEX_PATH } from "./build-input-field-live-proof-v81.js";
import {
  INPUT_LIVE_V81_DYNAMIC_TOOL,
  INPUT_LIVE_V81_TARGET,
} from "./input-field-live-v81-broker.js";
import {
  buildInputLiveV81AuthorizationArtifact,
  validateInputLiveV81AntecedentIndex,
  validateInputLiveV81AuthorizationArtifact,
  validateInputLiveV81History,
  validateInputLiveV81UnitTestSource,
  type InputLiveV81AntecedentIndex,
  type InputLiveV81AuthorizationArtifact,
  type InputLiveV81AuthorizationProof,
  type InputLiveV81HistoryState,
} from "./input-field-live-v81-authorization.js";
import {
  inputLiveV81AttestationSha256,
  validateInputLiveV81ControlFlowSource,
  validateInputLiveV81SecurityAttestation,
  type InputLiveV81SecurityAttestation,
} from "./input-field-live-v81-preflight.js";
import { inputLiveV81Sha256 } from "./input-field-live-v81-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V81_INDEX_PATH);
const index = (): InputLiveV81AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV81AntecedentIndex;

const artifact = (): InputLiveV81AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV81AuthorizationArtifact({
...
const pendingState = (): InputLiveV81HistoryState => ({
  index: index(),
  indexSha256: inputLiveV81Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV81HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV81AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV81Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V81_TARGET,
  expectedDynamicTool: INPUT_LIVE_V81_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v81/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV81AuthorizationArtifact,
): InputLiveV81SecurityAttestation => {
  const body: Omit<InputLiveV81SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v81-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V81_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV81AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV81History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV81History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV81History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV81History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV81History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV81UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV81UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV81AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV81Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV81AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV81AuthorizationArtifact(
        planted as InputLiveV81AuthorizationArtifact,
        index(),
        inputLiveV81Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV81SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV81AttestationSha256(
      body as Omit<InputLiveV81SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV81SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV81ControlFlowSource(source), []);
});
