...
import {
  validateCalendarLiveV17AntecedentIndex,
  verifyCalendarLiveV17Authorization,
  type CalendarLiveV17AntecedentIndex,
  type CalendarLiveV17AuthorizationProof,
} from "./calendar-live-v17-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V17_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v17/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V17_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v17-security-attestation.json";

export interface CalendarLiveV17SecurityAttestation {
  artifactVersion: "calendar-live-v17-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV17PreflightReport {
  artifactVersion: "calendar-live-v17-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV17SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV17SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV17AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v17-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V17_TARGET) ||
...
export function validateCalendarLiveV17ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV17Authorization");
  const preflight = source.indexOf("runCalendarLiveV17Preflight");
  const initialize = source.indexOf("CalendarLiveV17Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV17Preflight(
  root: string,
  proof: CalendarLiveV17AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V17_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV17PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V17_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V17_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV17AntecedentIndex;
  failures.push(...validateCalendarLiveV17AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V17_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V17_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV17SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v17-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV17ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v17 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV17SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v17-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V17_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV17Preflight(
    process.cwd(),
    verifyCalendarLiveV17Authorization(),
    attestationPath,
