...
import {
  assertInputLiveV22RootProofs,
  INPUT_LIVE_V22_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V22_RESPONSE_CONTRACTS,
  type InputLiveV22RootProof,
} from "./input-field-live-v22-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V22_BROKER_VERSION = "input-live-v22-broker-v2";
export const INPUT_LIVE_V22_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V22_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V22_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V22_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V22_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V22_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV22RemotePhase =
  (typeof INPUT_LIVE_V22_REMOTE_PHASES)[number];
export type InputLiveV22AuthorizationMode = "live" | "simulated";

export interface InputLiveV22CommitPins {
...
export interface InputLiveV22TransactionAuthorization extends InputLiveV22CommitPins {
  mode: InputLiveV22AuthorizationMode;
...
export interface InputLiveV22Manifest {
  artifactVersion: "input-live-v22-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V22_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V22_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V22_TARGET;
  authorization: InputLiveV22TransactionAuthorization;
...
export interface InputLiveV22ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV22RequestBody {
  artifactVersion: "input-live-v22-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V22_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV22RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV22CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V22_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV22ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV22Request extends InputLiveV22RequestBody {
...
export interface InputLiveV22Receipt {
  artifactVersion: "input-live-v22-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V22_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV22RemotePhase;
...
export interface InputLiveV22AcceptedResponse<T = unknown> {
  request: InputLiveV22Request;
  receipt: InputLiveV22Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV22IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV22ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V22_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V22_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V22_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V22_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V22_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V22_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V22_TARGET.editorType;
};
...
export const inputLiveV22RequestSequence = (
  phase: InputLiveV22RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV22RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV22RequestPath = (
  directory: string,
  phase: InputLiveV22RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV22ResponsePath = (
  directory: string,
  phase: InputLiveV22RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV22ReceiptPath = (
  directory: string,
  phase: InputLiveV22RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV22Manifest => {
  const manifest = json<InputLiveV22Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v22-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V22_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V22_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV22RemotePhase,
  captureIndex?: number,
): InputLiveV22Request | undefined => {
  const file = inputLiveV22RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV22Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV22RemotePhase,
  captureIndex?: number,
): InputLiveV22Receipt | undefined => {
  const file = inputLiveV22ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV22Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV22Request | undefined;
  receipt: InputLiveV22Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV22RemotePhase,
): InputLiveV22ResponseContract => ({
  ...INPUT_LIVE_V22_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V22_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V22_DYNAMIC_TOOL,
      target: INPUT_LIVE_V22_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v22-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V22_BROKER_VERSION,
  target: INPUT_LIVE_V22_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V22_PROGRAM_START*/${program}/*INPUT_LIVE_V22_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V22_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V22_PROGRAM_END*/";
...
export function createInputLiveV22Transaction(
  directory: string,
  ...
  const manifest: InputLiveV22Manifest = {
    artifactVersion: "input-live-v22-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V22_BROKER_VERSION,
...
export function validateInputLiveV22Request(
  directory: string,
  request: InputLiveV22Request,
): void {
  ...
  const expectedSequence = inputLiveV22RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v22-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V22_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V22_TARGET.fileKey ||
...
export function issueInputLiveV22Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV22RemotePhase,
  program: string,
  options: InputLiveV22IssueOptions = {},
): InputLiveV22Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV22RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV22Request>(targetPath);
    validateInputLiveV22Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV22ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV22TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV22RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V22_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V22_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V22_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV22RequestBody = {
    artifactVersion: "input-live-v22-broker-request-v2",
    brokerVersion: INPUT_LIVE_V22_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV22Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV22Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV22RawResponse(
  directory: string,
  phase: InputLiveV22RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV22RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV22Request>(requestFile);
  if (
  ...
  const target = inputLiveV22ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV22Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v22-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V22_BROKER_VERSION,
...
export function acceptInputLiveV22Response<T = unknown>(
  directory: string,
  phase: InputLiveV22RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV22AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV22ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV22Request>(
    inputLiveV22RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV22Request(directory, request);
  if (phase === "capture") validateInputLiveV22TechnicalGates(directory);
  const rawFile = inputLiveV22ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v22-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V22_BROKER_VERSION ||
...
export function persistInputLiveV22TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV22RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV22RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V22_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v22-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V22_BROKER_VERSION,
...
export function validateInputLiveV22TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v22-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V22_TECHNICAL_GATES.some(
...
export function validateInputLiveV22Receipt(
  directory: string,
  phase: InputLiveV22RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV22Request(directory, request);
  const receipt = json<InputLiveV22Receipt>(
    inputLiveV22ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV22ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v22-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V22_BROKER_VERSION ||
...
export function readInputLiveV22AcceptedRawPayload(
  directory: string,
  phase: InputLiveV22RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV22Receipt(directory, phase, captureIndex);
...
export function createInputLiveV22FakeDynamicResponse(
  request: InputLiveV22Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V22_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V22_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV22PersistedRequests(
  directory: string,
): InputLiveV22Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV22Request>(file)] : [];
    });
}

export const inputLiveV22Sha256 = sha256;
