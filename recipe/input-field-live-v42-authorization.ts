...
export type InputLiveV42HistoryExpectation = "pending" | "authorized";

export interface InputLiveV42AuthorizationArtifact {
  artifactVersion: "input-live-v42-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V42_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV42AntecedentIndex {
...
export interface InputLiveV42HistoryState {
  ...
  authorization?: InputLiveV42AuthorizationArtifact;
...
export interface InputLiveV42AuthorizationProof extends InputLiveV42TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V42_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V42_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V42_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V42_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v42-status") ||
...
export function validateInputLiveV42AntecedentIndex(
  index: InputLiveV42AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v42-antecedent-index-v1" ||
...
export function validateInputLiveV42UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV42History\(\s*["']/.test(source) ||
    /readInputLiveV42HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV42AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV42AuthorizationArtifact {
  ...
  const indexFailures = validateInputLiveV42AntecedentIndex(index);
...
export function validateInputLiveV42AuthorizationArtifact(
  artifact: InputLiveV42AuthorizationArtifact | undefined,
  index: InputLiveV42AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v42-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v42" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V42_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V42_TARGET) ||
    !equalJson(
...
export function validateInputLiveV42History(
  state: InputLiveV42HistoryState,
  expected: InputLiveV42HistoryExpectation,
): string[] {
  const failures = validateInputLiveV42AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV42AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV42HistoryState(
  root = process.cwd(),
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V42_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV42AuthorizationArtifact)
    : undefined;
  ...
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V42_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V42_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV42History(
  expected: InputLiveV42HistoryExpectation,
  root = process.cwd(),
): InputLiveV42HistoryState {
  const state = readInputLiveV42HistoryState(root);
  const failures = validateInputLiveV42History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV42Authorization(
  root = process.cwd(),
): InputLiveV42AuthorizationProof {
  const state = verifyInputLiveV42History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V42_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v42/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v42.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V42_TARGET,
    expectedDynamicTool: INPUT_LIVE_V42_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V42_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV42HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV42History(expected);
