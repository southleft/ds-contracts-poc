...
import { CALENDAR_LIVE_V4_INDEX_PATH } from "./build-calendar-live-proof-v4.js";
import {
...
import {
  buildCalendarLiveV4AuthorizationArtifact,
  validateCalendarLiveV4AntecedentIndex,
  validateCalendarLiveV4AuthorizationArtifact,
  validateCalendarLiveV4History,
  validateCalendarLiveV4UnitTestSource,
  type CalendarLiveV4AntecedentIndex,
  type CalendarLiveV4AuthorizationArtifact,
  type CalendarLiveV4AuthorizationProof,
  type CalendarLiveV4HistoryState,
} from "./calendar-live-v4-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV4ControlFlowSource,
  validateCalendarLiveV4SecurityAttestation,
  type CalendarLiveV4SecurityAttestation,
} from "./calendar-live-v4-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V4_INDEX_PATH);
const index = (): CalendarLiveV4AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV4AntecedentIndex;

const artifact = (): CalendarLiveV4AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV4AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV4HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV4HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV4AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v4/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV4AuthorizationArtifact,
): CalendarLiveV4SecurityAttestation => {
  const body: Omit<CalendarLiveV4SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V4_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV4History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV4History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV4History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV4History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV4History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV4UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v4-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV4UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV4AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV4AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV4AuthorizationArtifact(
        planted as CalendarLiveV4AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV4SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV4SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV4SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v4-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV4ControlFlowSource(source), []);
});
