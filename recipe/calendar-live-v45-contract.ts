...
import { buildCalendarLiveV45CleanupRuntime } from "./calendar-live-v45-cleanup.js";
import { type CalendarLiveV45FixedFixedPoint } from "./calendar-live-v45-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV45RestoreProgram,
  validateCalendarLiveV45RestorePayload,
  type CalendarLiveV45RestorePayload,
} from "./calendar-live-v45-restore.js";
import {
  buildCalendarLiveV45RawPropertyRuntime,
  normalizeCalendarLiveV45Scene,
  type CalendarLiveV45RawNode,
} from "./calendar-live-v45-verifier.js";
...
export const CALENDAR_LIVE_V45_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V45_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V45_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V45_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V45_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V45_SET_COUNT = 3;
export const CALENDAR_LIVE_V45_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V45_HOST_PHASES = 3;
export const CALENDAR_LIVE_V45_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V45_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V45_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V45_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV45SourceId = (typeof CALENDAR_LIVE_V45_SOURCE_IDS)[number];
export type CalendarLiveV45Kind = "calendar" | "week" | "day";

export interface CalendarLiveV45SourceIdentity {
  source: CalendarLiveV45SourceId;
...
export interface CalendarLiveV45WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV45ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV45SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV45RawNode;
    weekScene: CalendarLiveV45RawNode;
    dayScene: CalendarLiveV45RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV45RootProof {
  source: CalendarLiveV45SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV45FixedFixedPoint;
}

export interface CalendarLiveV45ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV45SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV45CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV45SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV45CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV45SourceId;
  frameWidth: number;
...
export interface CalendarLiveV45CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v45 ${label} must be an object`);
...
export function validateCalendarLiveV45WriterPayload(
  value: unknown,
): CalendarLiveV45WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v45") ||
    ...
    result.namespace !== CALENDAR_LIVE_V45_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v45 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v45 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v45 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV45SourceIdentity,
  kind: CalendarLiveV45Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV45ExtractProgram(
  writer: CalendarLiveV45WriterOwnership,
  sources: readonly CalendarLiveV45SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v45 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V45_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V45_NAMESPACE)}
${buildCalendarLiveV45RawPropertyRuntime()}
...
export function validateCalendarLiveV45ExtractPayload(
  value: unknown,
  writer: CalendarLiveV45WriterOwnership,
): CalendarLiveV45ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v45 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V45_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v45 extract schema/one-root mismatch");
  return payload as CalendarLiveV45ExtractPayload;
}

export function proveCalendarLiveV45Roots<Instance>(
  extract: CalendarLiveV45ExtractPayload,
  sources: readonly (CalendarLiveV45SourceIdentity & {
...
): CalendarLiveV45RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v45 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v45 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV45Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV45Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV45Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV45FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v45 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v45 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v45 compile lost owned sets");
...
export function assertCalendarLiveV45RootProofs(
  proofs: readonly CalendarLiveV45RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v45 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v45 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV45ProbeProgram(
  writer: CalendarLiveV45WriterOwnership,
  sources: readonly CalendarLiveV45SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V45_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV45ProbePayload(
  value: unknown,
  writer: CalendarLiveV45WriterOwnership,
): CalendarLiveV45ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV45ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V45_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V45_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v45 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV45CaptureManifest(
  cells: readonly CalendarLiveV45CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V45_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V45_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V45_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V45_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v45 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV45CaptureCell[],
): string => {
  validateCalendarLiveV45CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV45CaptureProgram(
  writer: CalendarLiveV45WriterOwnership,
  cell: CalendarLiveV45CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V45_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v45 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v45 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V45_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV45CapturePayload(
  value: unknown,
  cell: CalendarLiveV45CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV45CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV45CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V45_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V45_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v45 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV45CaptureResponses(
  manifest: readonly CalendarLiveV45CaptureCell[],
  responses: readonly CalendarLiveV45CapturePayload[],
): void {
  validateCalendarLiveV45CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v45 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV45ObjectiveReport {
  artifactVersion: "calendar-live-v45-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV45Objective(
  manifest: readonly CalendarLiveV45CaptureCell[],
  responses: readonly CalendarLiveV45CapturePayload[],
): CalendarLiveV45ObjectiveReport {
  assertCalendarLiveV45CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v45-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV45CleanupProgram(
  writer: CalendarLiveV45WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV45CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V45_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v45 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV45CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV45WriterOwnership,
): CalendarLiveV45CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV45CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v45 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V45_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV45WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
