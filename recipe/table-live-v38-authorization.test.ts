...
import { TABLE_LIVE_V38_INDEX_PATH } from "./build-table-live-proof-v38.js";
import {
...
import {
  buildTableLiveV38AuthorizationArtifact,
  validateTableLiveV38AntecedentIndex,
  validateTableLiveV38AuthorizationArtifact,
  validateTableLiveV38History,
  validateTableLiveV38UnitTestSource,
  type TableLiveV38AntecedentIndex,
  type TableLiveV38AuthorizationArtifact,
  type TableLiveV38AuthorizationProof,
  type TableLiveV38HistoryState,
} from "./table-live-v38-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV38ControlFlowSource,
  validateTableLiveV38SecurityAttestation,
  type TableLiveV38SecurityAttestation,
} from "./table-live-v38-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V38_INDEX_PATH);
const index = (): TableLiveV38AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV38AntecedentIndex;

const artifact = (): TableLiveV38AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV38AuthorizationArtifact({
...
const pendingState = (): TableLiveV38HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV38HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV38AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v38/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV38AuthorizationArtifact,
): TableLiveV38SecurityAttestation => {
  const body: Omit<TableLiveV38SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V38_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV38History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV38History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV38History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV38History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV38History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV38UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v38-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV38UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV38AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV38AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV38AuthorizationArtifact(
        planted as TableLiveV38AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV38SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV38SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV38SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v38-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV38ControlFlowSource(source), []);
});
