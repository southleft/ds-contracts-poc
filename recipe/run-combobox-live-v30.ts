...
import {
  acceptComboboxLiveV30Response,
  createComboboxLiveV30Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV30Request,
  persistComboboxLiveV30RawResponse,
  persistComboboxLiveV30TechnicalGates,
  readComboboxLiveV30AcceptedRawPayload,
  type ComboboxLiveV30Request,
  type ComboboxLiveV30TransactionAuthorization,
} from "./combobox-live-v30-broker.js";
import {
  COMBOBOX_LIVE_V30_CAPTURE_COUNT,
  assertComboboxLiveV30CaptureResponses,
  assertComboboxLiveV30RootProofs,
  buildComboboxLiveV30CaptureProgram,
  buildComboboxLiveV30CleanupProgram,
  buildComboboxLiveV30ExtractProgram,
  buildComboboxLiveV30ProbeProgram,
  buildComboboxLiveV30RestoreProgram,
  validateComboboxLiveV30RestorePayload,
  evaluateComboboxLiveV30Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV30Roots,
  validateComboboxLiveV30CaptureManifest,
  validateComboboxLiveV30CapturePayload,
  validateComboboxLiveV30CleanupPayload,
  validateComboboxLiveV30ExtractPayload,
  validateComboboxLiveV30ProbePayload,
  validateComboboxLiveV30WriterPayload,
  type ComboboxLiveV30CaptureCell,
  type ComboboxLiveV30CapturePayload,
  type ComboboxLiveV30RootProof,
  type ComboboxLiveV30SourceIdentity,
  type ComboboxLiveV30WriterOwnership,
} from "./combobox-live-v30-contract.js";
import {
  COMBOBOX_LIVE_V30_CAPTURE_MANIFEST_PATH,
  COMBOBOX_LIVE_V30_PLAN_PATH,
  type ComboboxLiveV30ProofPlan,
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v30.js";

export const COMBOBOX_LIVE_V30_RUNNER_VERSION = "combobox-live-v30-runner-v1";

interface ComboboxLiveV30JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV30OrchestratorState {
  artifactVersion: "combobox-live-v30-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV30JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV30ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV30CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV30SourceIdentity & {
...
export interface ComboboxLiveV30NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV30OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v30 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V30_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V30_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV30ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV30CaptureCell[];
  validateComboboxLiveV30CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v30 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v30 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV30WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV30RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV30Request =>
  readJson<ComboboxLiveV30Request>(file);

export class ComboboxLiveV30Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV30Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV30OrchestratorState = {
      artifactVersion: "combobox-live-v30-orchestrator-state-v1",
    ...
    return new ComboboxLiveV30Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV30OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV30Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV30NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV30NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v30 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV30NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v30 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV30RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV30Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV30CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV30AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV30WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV30Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV30ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV30Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV30Response(this.#directory, "writer", {
        validate: validateComboboxLiveV30WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV30Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV30RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV30Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV30ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV30Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV30RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v30-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV30Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV30ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV30Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV30ProbePayload(payload, ownership),
      });
      persistComboboxLiveV30TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v30 capture cell missing");
      const accepted = acceptComboboxLiveV30Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV30CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V30_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV30CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV30Objective(
    ...
    throw new Error(`Combobox live v30 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV30Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV30CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV30Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV30RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV30Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV30CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV30CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV30CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV30Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV30TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV30SimulationKey = (): KeyObject =>
