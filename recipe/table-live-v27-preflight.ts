...
import {
  ...
  verifyTableLiveV27Authorization,
  type TableLiveV27AntecedentIndex,
  type TableLiveV27AuthorizationProof,
} from "./table-live-v27-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V27_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v27/operator-security-attestation-template.json";
export const TABLE_LIVE_V27_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v27-security-attestation.json";

export interface TableLiveV27SecurityAttestation {
...
export interface TableLiveV27PreflightReport {
  artifactVersion: "table-live-v27-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV27SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV27SecurityAttestation(
  value: unknown,
  proof: TableLiveV27AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v27-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V27_TARGET) ||
...
export function validateTableLiveV27ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV27Authorization");
  const preflight = source.indexOf("runTableLiveV27Preflight");
  const initialize = source.indexOf("TableLiveV27Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV27Preflight(
  root: string,
  proof: TableLiveV27AuthorizationProof,
  attestationPath = TABLE_LIVE_V27_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV27PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V27_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V27_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV27AntecedentIndex;
  failures.push(...validateTableLiveV27AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V27_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V27_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v27-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV27ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v27 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV27SecurityAttestation;
  return {
    artifactVersion: "table-live-v27-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V27_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV27Preflight(
    process.cwd(),
    verifyTableLiveV27Authorization(),
    attestationPath,
