...
import {
  validateTableLiveV3AntecedentIndex,
  verifyTableLiveV3Authorization,
  type TableLiveV3AntecedentIndex,
  type TableLiveV3AuthorizationProof,
} from "./table-live-v3-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V3_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v3/operator-security-attestation-template.json";
export const TABLE_LIVE_V3_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v3-security-attestation.json";

export interface TableLiveV3SecurityAttestation {
...
export interface TableLiveV3PreflightReport {
  artifactVersion: "table-live-v3-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV3SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV3SecurityAttestation(
  value: unknown,
  proof: TableLiveV3AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v3-operator-security-attestation-v1" ||
    artifact.status !== "complete"
  )
    failures.push("missing completed table v3 security attestation");
  if (
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V3_TARGET) ||
...
export function validateTableLiveV3ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV3Authorization");
  const preflight = source.indexOf("runTableLiveV3Preflight");
  const initialize = source.indexOf("TableLiveV3Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV3Preflight(
  root: string,
  proof: TableLiveV3AuthorizationProof,
  attestationPath = TABLE_LIVE_V3_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV3PreflightReport {
  const failures: string[] = [];
  if (
    ...
    failures.push("table v3 attempt chronology invalid or exceeds maximum 3");
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V3_TARGET) ||
    canonicalJson(proof.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V3_DYNAMIC_TOOL)
  )
    failures.push("table v3 authorization proof target or tool mismatch");
  const indexPath = path.join(root, TABLE_LIVE_V3_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV3AntecedentIndex;
  failures.push(...validateTableLiveV3AntecedentIndex(index));
  if (
    sha256(indexBytes) !== proof.antecedentIndexSha256 ||
    index.hashSetSha256 !== proof.antecedentHashSetSha256
  )
    failures.push("table v3 authorization proof antecedent index mismatch");
  for (const [relativePath, metadata] of Object.entries(index.artifacts)) {
    ...
    if (
      ...
      failures.push(
        `table v3 pinned antecedent working bytes drift: ${relativePath}`,
      );
  }
  if (!existsSync(path.join(root, TABLE_LIVE_V3_AUTHORIZATION_PATH)))
    failures.push("table v3 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V3_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v3-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV3ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v3 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV3SecurityAttestation;
  return {
    artifactVersion: "table-live-v3-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V3_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV3Preflight(
    process.cwd(),
    verifyTableLiveV3Authorization(),
    attestationPath,
