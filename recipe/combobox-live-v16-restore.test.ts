...
import {
  ...
  COMBOBOX_LIVE_V16_RESTORE_CONTENT_ROLES,
  COMBOBOX_LIVE_V16_RESTORE_COUNT,
  COMBOBOX_LIVE_V16_RESTORE_INPUT_SET_ROLE,
  COMBOBOX_LIVE_V16_RESTORE_OPTION_LABEL_SET_ROLE,
...
test("Combobox restore owns input from combobox/set and option/label from option-set", () => {
  ...
  assert.equal(COMBOBOX_LIVE_V16_RESTORE_COUNT, 144);
  assert.equal(COMBOBOX_LIVE_V16_RESTORE_INPUT_SET_ROLE, "combobox/set");
  assert.equal(
    COMBOBOX_LIVE_V16_RESTORE_OPTION_LABEL_SET_ROLE,
    "combobox/option-set",
  );
  ...
  const program = buildComboboxLiveV16RestoreProgram({
    pageId: "1:2",
    runIdentity: "x-combobox-v1",
    setIds: ["a", "b", "c", "d"],
  });
