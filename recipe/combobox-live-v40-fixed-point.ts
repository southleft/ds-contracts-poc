/**
 * Carried Combobox live v40 host verifier. Uses scene-readback-combobox-v40 so live
...
export const COMBOBOX_LIVE_V40_FIXED_VERIFIER_VERSION = "combobox-live-v40-fixed-point-v1";
export const COMBOBOX_LIVE_V40_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV40FixedThresholds {
...
export interface ComboboxLiveV40FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV40FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV40FixedSourceProbe {
...
export interface ComboboxLiveV40FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV40FixedFixedPoint;
}

export interface ComboboxLiveV40FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV40FixedHardGateInput {
  thresholds: ComboboxLiveV40FixedThresholds;
  materialRegression: ComboboxLiveV40FixedMaterialRegression;
  sourceProbes: ComboboxLiveV40FixedSourceProbe[];
  cells: ComboboxLiveV40FixedCellValidation[];
  sceneProofs: ComboboxLiveV40FixedSceneProof[];
  visualRows: ComboboxLiveV40FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV40FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V40_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV40FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV40FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV40FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV40FixedHardGates(
  input: ComboboxLiveV40FixedHardGateInput,
): ComboboxLiveV40FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV40FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV40FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V40_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV40FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV40FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV40FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
