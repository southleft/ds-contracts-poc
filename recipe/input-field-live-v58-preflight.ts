...
import {
  ...
  verifyInputLiveV58Authorization,
  type InputLiveV58AntecedentIndex,
  type InputLiveV58AuthorizationProof,
} from "./input-field-live-v58-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V58_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v58/operator-security-attestation-template.json";
export const INPUT_LIVE_V58_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v58-security-attestation.json";

export interface InputLiveV58SecurityAttestation {
...
export interface InputLiveV58PreflightReport {
  artifactVersion: "input-live-v58-preflight-v1";
...
export const inputLiveV58AttestationSha256 = (
  value: Omit<InputLiveV58SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV58SecurityAttestation(
  value: unknown,
  proof: InputLiveV58AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v58-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V58_TARGET) ||
...
export function validateInputLiveV58ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV58Authorization");
  const preflight = source.indexOf("runInputLiveV58Preflight");
  const initialize = source.indexOf("InputLiveV58Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV58Preflight(
  root: string,
  proof: InputLiveV58AuthorizationProof,
  attestationPath = INPUT_LIVE_V58_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV58PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V58_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V58_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V58_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV58AntecedentIndex;
  failures.push(...validateInputLiveV58AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V58_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V58_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v58-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV58ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV58SecurityAttestation;
  return {
    artifactVersion: "input-live-v58-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V58_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV58Preflight(
    process.cwd(),
    verifyInputLiveV58Authorization(),
    attestationPath,
