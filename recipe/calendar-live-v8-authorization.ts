...
export type CalendarLiveV8HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV8AuthorizationArtifact {
  artifactVersion: "calendar-live-v8-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V8_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV8AntecedentIndex {
...
export interface CalendarLiveV8HistoryState {
...
export interface CalendarLiveV8AuthorizationProof
  extends CalendarLiveV8TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V8_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V8_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V8_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V8_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v8-authorization") ||
  artifactPath.includes("calendar-live-v8-preflight") ||
  artifactPath.includes("calendar-live-v8-authorized") ||
  artifactPath.includes("create-calendar-live-v8-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v8-status") ||
...
export function validateCalendarLiveV8AntecedentIndex(
  index: CalendarLiveV8AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v8-antecedent-index-v1" ||
...
export function validateCalendarLiveV8UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV8History\(\s*["']/.test(source) ||
    /readCalendarLiveV8HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV8AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV8AuthorizationArtifact {
  ...
  const indexFailures = validateCalendarLiveV8AntecedentIndex(index);
...
export function validateCalendarLiveV8AuthorizationArtifact(
  artifact: CalendarLiveV8AuthorizationArtifact | undefined,
  index: CalendarLiveV8AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v8-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v8" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V8_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V8_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V8_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V8_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V8_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V8_SOURCE_ROOTS ||
...
export function validateCalendarLiveV8History(
  state: CalendarLiveV8HistoryState,
  expected: CalendarLiveV8HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV8AntecedentIndex(state.index);
...
export function readCalendarLiveV8HistoryState(
  root = process.cwd(),
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V8_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV8AuthorizationArtifact)
    : undefined;
  ...
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V8_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V8_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV8History(
  expected: CalendarLiveV8HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV8HistoryState {
  const state = readCalendarLiveV8HistoryState(root);
  const failures = validateCalendarLiveV8History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v8 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV8Authorization(
  root = process.cwd(),
): CalendarLiveV8AuthorizationProof {
  const state = verifyCalendarLiveV8History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V8_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v8/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v8.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V8_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V8_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V8_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV8HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV8History(expected);
  process.stdout.write(
    `Calendar live v8 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
