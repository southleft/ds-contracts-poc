...
import {
  ...
  buildTableLiveV6CaptureProgram,
  buildTableLiveV6CleanupProgram,
  buildTableLiveV6ExtractProgram,
  buildTableLiveV6ProbeProgram,
  buildTableLiveV6RestoreProgram,
  tableLiveV2CaptureManifestSha256,
  type TableLiveV6CaptureCell,
  type TableLiveV6SourceIdentity,
  type TableLiveV6WriterOwnership,
...
export const TABLE_LIVE_V6_EVIDENCE_ROOT = "recipe/evidence/table-live-pivot-v6";
export const TABLE_LIVE_V6_PROTOCOL_PATH = `${TABLE_LIVE_V6_EVIDENCE_ROOT}/protocol.json`;
export const TABLE_LIVE_V6_PLAN_PATH = `${TABLE_LIVE_V6_EVIDENCE_ROOT}/proof-plan.json`;
export const TABLE_LIVE_V6_CAPTURE_MANIFEST_PATH = `${TABLE_LIVE_V6_EVIDENCE_ROOT}/capture-manifest.json`;
export const TABLE_LIVE_V6_REQUEST_MANIFEST_PATH = `${TABLE_LIVE_V6_EVIDENCE_ROOT}/request-manifest.json`;
export const TABLE_LIVE_V6_INDEX_PATH = `${TABLE_LIVE_V6_EVIDENCE_ROOT}/antecedent-index.json`;
export const TABLE_LIVE_V6_AUTHORIZATION_TEMPLATE_PATH = `${TABLE_LIVE_V6_EVIDENCE_ROOT}/authorization-template.json`;
export const TABLE_LIVE_V6_AUTHORIZATION_PATH = `${TABLE_LIVE_V6_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface TableLiveV6ProofPlan {
  artifactVersion: "table-live-v6-proof-plan-v1";
...
const captureCells = (
...
): TableLiveV6CaptureCell[] => {
  const cells: TableLiveV6CaptureCell[] = [];
  ...
  if (cells.length !== TABLE_LIVE_V6_CAPTURE_COUNT)
    throw new Error(
      `Table live v6 capture cells ${cells.length}, expected ${TABLE_LIVE_V6_CAPTURE_COUNT}`,
    );
...
const protocol = (expectedFacts: number) => ({
  artifactVersion: "table-live-v6-external-operator-protocol-v1",
  protocolId: "table-live-v6",
  status:
  ...
  denominator: {
    ...
    sets: 6,
    variants: TABLE_LIVE_V6_VARIANT_COUNT,
    captureCells: TABLE_LIVE_V6_CAPTURE_COUNT,
...
export async function buildTableLiveV6Proof(
  check = process.argv.includes("--check"),
): Promise<TableLiveV6ProofPlan> {
  ...
  if (
    ...
    throw new Error("Table live v6 writer reused Input, Combobox, or Table v1–v5 identity");
  const captures = captureCells(sources);
  const writerProgram = Buffer.from(writer.code);
  const writerOwnershipBlueprint: TableLiveV6WriterOwnership = {
  ...
  const identities: TableLiveV6SourceIdentity[] = sources.map((source) => ({
    source: source.source,
    adapterIdentity: source.adapterIdentity,
    recipeHash: source.recipeHash,
    envelopeHash: source.envelope.integrity.canonicalHash,
  ...
  const restoreBlueprint = Buffer.from(
    buildTableLiveV6RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildTableLiveV6ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildTableLiveV6ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildTableLiveV6CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );
  const cleanupBlueprint = Buffer.from(
    buildTableLiveV6CleanupProgram(writerOwnershipBlueprint),
  );
  ...
  const sourceMetadata = sources.map((source) => {
    ...
    const tablePath = `${TABLE_LIVE_V6_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-table.json.gz`;
    const rowPath = `${TABLE_LIVE_V6_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-row.json.gz`;
    const cellPath = `${TABLE_LIVE_V6_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-cell.json.gz`;
  ...
  antecedentOutputs.set(
    TABLE_LIVE_V6_PROTOCOL_PATH,
    jsonBytes(protocol(expectedFacts)),
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V6_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V6_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V6_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V6_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V6_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V6_EVIDENCE_ROOT}/programs/cleanup-blueprint.js`,
    cleanupBlueprint,
  );

  const captureManifest = {
    artifactVersion: "table-live-v6-capture-manifest-v1",
  ...
  antecedentOutputs.set(
    TABLE_LIVE_V6_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: TableLiveV6ProofPlan = {
    artifactVersion: "table-live-v6-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: TABLE_LIVE_V6_TARGET,
    namespace: TABLE_FIGMA_NAMESPACE,
    writer: {
      programPath: `${TABLE_LIVE_V6_EVIDENCE_ROOT}/programs/writer.txt`,
    ...
    captureManifest: {
      path: TABLE_LIVE_V6_CAPTURE_MANIFEST_PATH,
      cells: 20,
      requests: 20,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(TABLE_LIVE_V6_PLAN_PATH, jsonBytes(proofPlan));

  const requests = {
    artifactVersion: "table-live-v6-request-manifest-v1",
    expectedDynamicTool: TABLE_LIVE_V6_DYNAMIC_TOOL,
    target: TABLE_LIVE_V6_TARGET,
  ...
  if (requests.requestCount !== TABLE_LIVE_V6_REMOTE_REQUESTS)
    throw new Error("Table live v6 remote request denominator drifted");
  antecedentOutputs.set(
    TABLE_LIVE_V6_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const lifecycleExcludedPaths = [
    TABLE_LIVE_V6_AUTHORIZATION_TEMPLATE_PATH,
    ...
    "recipe/table-live-v6-authorized.ts",
    "recipe/write-table-live-v6-authorization.ts",
    "recipe/create-table-live-v6-security-attestation.ts",
  ...
  const indexedPaths = [
    ...
    "recipe/table-live-v6-broker.ts",
    "recipe/table-live-v6-contract.ts",
    "recipe/table-live-v6-restore.ts",
    "recipe/table-live-v6-cleanup.ts",
    "recipe/table-live-v6-verifier.ts",
    "recipe/table-live-v6-fixed-point.ts",
    "recipe/run-table-live-v6.ts",
    "recipe/build-table-live-proof-v6.ts",
    "recipe/table-live-v6-lifecycle-simulation.ts",
    "recipe/scene-readback-table-v1.ts",
    "recipe/scene-readback-runtime-table-v6.ts",
    "recipe/table-figma-writer.ts",
    "recipe/figma-property-normalizer-v8.ts",
    "recipe/figma-runtime-portability.ts",
    "recipe/normalize.ts",
  ].filter((artifactPath) => artifactPath !== TABLE_LIVE_V6_INDEX_PATH);
  ...
  antecedentOutputs.set(TABLE_LIVE_V6_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "table-live-v6-authorization-template-v1",
  ...
  const attestationTemplate = jsonBytes({
    artifactVersion: "table-live-v6-operator-security-attestation-template-v1",
    status: "template only; complete privately after PREPARE+AUTHORIZE",
    tokenValuesForbidden: true,
    privateDirectory: "private/table-live-v6-security-attestation.json",
  });
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Table live v6 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(TABLE_LIVE_V6_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(TABLE_LIVE_V6_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
    writeFileSync(
      `${TABLE_LIVE_V6_EVIDENCE_ROOT}/operator-security-attestation-template.json`,
      attestationTemplate,
    );
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(TABLE_LIVE_V6_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(TABLE_LIVE_V6_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Table live v6 authorization template drift");
  }
  if (
    writer.namespace !== TABLE_FIGMA_NAMESPACE ||
    writer.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    ...
    throw new Error("Table live v6 identity or cleanup invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildTableLiveV6Proof(), null, 2)}\n`,
  );
