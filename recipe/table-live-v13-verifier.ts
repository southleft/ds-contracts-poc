...
import {
  verifyTableLiveV13FixedSceneFixedPoint,
  type TableLiveV13FixedFixedPoint,
} from "./table-live-v13-fixed-point.js";

export const TABLE_LIVE_V13_VERIFIER_VERSION = "table-live-v13-verifier-v1";

export function buildTableLiveV13RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV13RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV13RawNode[];
}

export interface TableLiveV13NormalizedScene {
...
export interface TableLiveV13SceneProof {
...
export function normalizeTableLiveV13Scene(
  root: TableLiveV13RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV13NormalizedScene {
  const canonicalBindings: TableLiveV13NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV13RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV13Scene<Instance>(
  normalized: TableLiveV13NormalizedScene,
...
): TableLiveV13SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV13FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV13ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV13PreCaptureGates(payload: unknown): void {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("table live v13 pre-capture gate payload absent");
