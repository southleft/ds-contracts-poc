...
import { buildComboboxLiveV5CleanupRuntime } from "./combobox-live-v5-cleanup.js";
import {
  type ComboboxLiveV5FixedFixedPoint,
} from "./combobox-live-v5-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV5RestoreProgram,
  validateComboboxLiveV5RestorePayload,
  type ComboboxLiveV5RestorePayload,
} from "./combobox-live-v5-restore.js";
import {
  buildComboboxLiveV5RawPropertyRuntime,
  normalizeComboboxLiveV5Scene,
  type ComboboxLiveV5RawNode,
} from "./combobox-live-v5-verifier.js";
...
export const COMBOBOX_LIVE_V5_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V5_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V5_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V5_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V5_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V5_SET_COUNT = 4;
export const COMBOBOX_LIVE_V5_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V5_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V5_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V5_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V5_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV5SourceId = (typeof COMBOBOX_LIVE_V5_SOURCE_IDS)[number];

export interface ComboboxLiveV5SourceIdentity {
  source: ComboboxLiveV5SourceId;
...
export interface ComboboxLiveV5WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV5ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV5SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV5RawNode;
    optionScene: ComboboxLiveV5RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV5RootProof {
  source: ComboboxLiveV5SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV5FixedFixedPoint;
}

export interface ComboboxLiveV5ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV5SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV5CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV5SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV5CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV5SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV5CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v4 ${label} must be an object`);
...
export function validateComboboxLiveV5WriterPayload(
  value: unknown,
): ComboboxLiveV5WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V5_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v4 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v4 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v4 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV5SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV5ExtractProgram(
  writer: ComboboxLiveV5WriterOwnership,
  sources: readonly ComboboxLiveV5SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v4 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V5_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V5_NAMESPACE)}
${buildComboboxLiveV5RawPropertyRuntime()}
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("COMBOBOX-V5-EXTRACT-PAGE");
...
if(get(page,"pageOwner")!=="recipe/combobox/"+${JSON.stringify(writer.runIdentity)})throw new Error("COMBOBOX-V5-EXTRACT-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>SET_IDS.has(node.id));
if(sets.length!==4)throw new Error("COMBOBOX-V5-EXTRACT-ROOTS:"+sets.length);
...
for(const set of sets){
  ...
  if(!IDENTITIES[adapterIdentity]||(kind!=="combobox"&&kind!=="option"))throw new Error("COMBOBOX-V5-EXTRACT-ADAPTER:"+adapterIdentity+":"+kind);
...
if(roots.length!==2||roots.some(root=>!root.comboboxScene||!root.optionScene))throw new Error("COMBOBOX-V5-EXTRACT-PAIR");
...
export function validateComboboxLiveV5ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV5WriterOwnership,
): ComboboxLiveV5ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v4 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V5_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v4 extract schema/two-root mismatch");
  return payload as ComboboxLiveV5ExtractPayload;
}

export function proveComboboxLiveV5Roots<Instance>(
  extract: ComboboxLiveV5ExtractPayload,
  sources: readonly (ComboboxLiveV5SourceIdentity & {
...
): ComboboxLiveV5RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v4 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v4 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV5Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV5Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV5FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v4 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v4 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v4 compile lost owned sets");
...
export function assertComboboxLiveV5RootProofs(
  proofs: readonly ComboboxLiveV5RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v4 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v4 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV5ProbeProgram(
  writer: ComboboxLiveV5WriterOwnership,
  sources: readonly ComboboxLiveV5SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V5_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE"||page.id==="115:295378")throw new Error("COMBOBOX-V5-PROBE-PAGE");
...
if(sets.length!==4)throw new Error("COMBOBOX-V5-PROBE-ROOTS:"+sets.length);
...
for(const row of byAdapter.values()){
 ...
 if(!source||!combobox||!option||combobox.children.length!==64||option.children.length!==8)throw new Error("COMBOBOX-V5-PROBE-VARIANTS:"+adapterIdentity);
 ...
 for(const component of combobox.children){const target=axes(component),updates={};for(const name of axisNames){const key=propertyKey(instance,name);if(!key)throw new Error("COMBOBOX-V5-PROBE-AXIS:"+name);updates[key]=target[name];}instance.setProperties(updates);const main=await instance.getMainComponentAsync();if(main)visited.add(main.id);}
 ...
 for(const component of option.children){const target=axes(component),updates={};for(const name of ["Size","Option state"]){const key=propertyKey(optionInstance,name);if(!key)throw new Error("COMBOBOX-V5-PROBE-OPTION-AXIS:"+name);updates[key]=target[name];}optionInstance.setProperties(updates);const main=await optionInstance.getMainComponentAsync();if(main)optionVisited.add(main.id);}
...
export function validateComboboxLiveV5ProbePayload(
  value: unknown,
  writer: ComboboxLiveV5WriterOwnership,
): ComboboxLiveV5ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV5ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V5_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V5_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v4 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV5CaptureManifest(
  cells: readonly ComboboxLiveV5CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V5_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V5_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V5_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V5_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v4 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV5CaptureCell[],
): string => {
  validateComboboxLiveV5CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV5CaptureProgram(
  writer: ComboboxLiveV5WriterOwnership,
  cell: ComboboxLiveV5CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V5_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v4 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v4 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
if(!page||page.type!=="PAGE"||page.id==="115:295378"||!set||set.type!=="COMPONENT_SET")throw new Error("COMBOBOX-V5-CAPTURE-TARGET");
const axes=node=>Object.fromEntries(node.name.split(",").map(part=>part.trim()).filter(part=>part.includes("=")).map(part=>{const at=part.indexOf("=");return[part.slice(0,at),part.slice(at+1)];}));
...
if(!component)throw new Error("COMBOBOX-V5-CAPTURE-CELL:"+cell.cellKey);
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V5_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV5CapturePayload(
  value: unknown,
  cell: ComboboxLiveV5CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV5CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV5CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V5_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V5_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v4 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV5CaptureResponses(
  manifest: readonly ComboboxLiveV5CaptureCell[],
  responses: readonly ComboboxLiveV5CapturePayload[],
): void {
  validateComboboxLiveV5CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v4 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV5ObjectiveReport {
  artifactVersion: "combobox-live-v5-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV5Objective(
  manifest: readonly ComboboxLiveV5CaptureCell[],
  responses: readonly ComboboxLiveV5CapturePayload[],
): ComboboxLiveV5ObjectiveReport {
  assertComboboxLiveV5CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v5-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV5CleanupProgram(
  writer: ComboboxLiveV5WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV5CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V5_NAMESPACE,
  ...
  const guardedResult = `if(JSON.stringify(requestedNodeIds)!==JSON.stringify([expectedPageId])||JSON.stringify([...requestedCollectionIds].sort())!==JSON.stringify([...expectedCollectionIds].sort()))throw new Error("COMBOBOX-V5-CLEANUP-EXACT-OWNERSHIP");${resultStatement}`;
  const program = baseProgram.replace(resultStatement, guardedResult);
  if (program === baseProgram)
    throw new Error("Combobox live v4 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV5CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV5WriterOwnership,
): ComboboxLiveV5CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV5CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v4 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V5_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV5WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
