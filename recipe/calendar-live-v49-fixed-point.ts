/**
 * Calendar live v49 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V49_FIXED_VERIFIER_VERSION = "calendar-live-v49-fixed-point-v1";
export const CALENDAR_LIVE_V49_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV49FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV49FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV49FixedSourceProbe {
...
export interface CalendarLiveV49FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV49FixedFixedPoint;
}

export interface CalendarLiveV49FixedHardGateInput {
  thresholds: CalendarLiveV49FixedThresholds;
  sourceProbes: CalendarLiveV49FixedSourceProbe[];
  cells: CalendarLiveV49FixedCellValidation[];
  sceneProofs: CalendarLiveV49FixedSceneProof[];
...
export interface CalendarLiveV49FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V49_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV49FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV49FixedHardGates(
  input: CalendarLiveV49FixedHardGateInput,
): CalendarLiveV49FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV49FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V49_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV49FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV49FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV49FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
