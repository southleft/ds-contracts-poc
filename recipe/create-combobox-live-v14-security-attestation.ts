...
import { verifyComboboxLiveV14Authorization } from "./combobox-live-v14-authorization.js";
import { COMBOBOX_LIVE_V14_TARGET } from "./combobox-live-v14-broker.js";
import {
  COMBOBOX_LIVE_V14_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV14Authorization();
...
const attestation: ComboboxLiveV14SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV14SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v14 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V14_SECURITY_ATTESTATION_DEFAULT_PATH,
);
