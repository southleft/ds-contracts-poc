...
export interface ComboboxLiveV39LifecycleSimulationReport {
  artifactVersion: "combobox-live-v39-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV39AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV39Lifecycle(): Promise<ComboboxLiveV39LifecycleSimulationReport> {
  await buildComboboxLiveV39Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V39_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV39AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v39-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V39_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V39_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV39History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV39History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV39AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V39_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V39_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV39History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV39History(authorized, "pending");
    ...
    await buildComboboxLiveV39Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V39_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV39AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV39Lifecycle(), null, 2)}\n`,
  );
