...
import {
  assertInputLiveV72RootProofs,
  INPUT_LIVE_V72_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V72_RESPONSE_CONTRACTS,
  type InputLiveV72RootProof,
} from "./input-field-live-v72-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V72_BROKER_VERSION = "input-live-v72-broker-v2";
export const INPUT_LIVE_V72_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V72_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V72_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V72_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V72_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V72_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV72RemotePhase =
  (typeof INPUT_LIVE_V72_REMOTE_PHASES)[number];
export type InputLiveV72AuthorizationMode = "live" | "simulated";

export interface InputLiveV72CommitPins {
...
export interface InputLiveV72TransactionAuthorization extends InputLiveV72CommitPins {
  mode: InputLiveV72AuthorizationMode;
...
export interface InputLiveV72Manifest {
  artifactVersion: "input-live-v72-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V72_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V72_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V72_TARGET;
  authorization: InputLiveV72TransactionAuthorization;
...
export interface InputLiveV72ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV72RequestBody {
  artifactVersion: "input-live-v72-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V72_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV72RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV72CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V72_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV72ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV72Request extends InputLiveV72RequestBody {
...
export interface InputLiveV72Receipt {
  artifactVersion: "input-live-v72-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V72_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV72RemotePhase;
...
export interface InputLiveV72AcceptedResponse<T = unknown> {
  request: InputLiveV72Request;
  receipt: InputLiveV72Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV72IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV72ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V72_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V72_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V72_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V72_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V72_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V72_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V72_TARGET.editorType;
};
...
export const inputLiveV72RequestSequence = (
  phase: InputLiveV72RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV72RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV72RequestPath = (
  directory: string,
  phase: InputLiveV72RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV72ResponsePath = (
  directory: string,
  phase: InputLiveV72RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV72ReceiptPath = (
  directory: string,
  phase: InputLiveV72RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV72Manifest => {
  const manifest = json<InputLiveV72Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v72-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V72_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V72_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV72RemotePhase,
  captureIndex?: number,
): InputLiveV72Request | undefined => {
  const file = inputLiveV72RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV72Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV72RemotePhase,
  captureIndex?: number,
): InputLiveV72Receipt | undefined => {
  const file = inputLiveV72ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV72Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV72Request | undefined;
  receipt: InputLiveV72Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV72RemotePhase,
): InputLiveV72ResponseContract => ({
  ...INPUT_LIVE_V72_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V72_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v72-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V72_BROKER_VERSION,
  target: INPUT_LIVE_V72_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V72_PROGRAM_START*/${program}/*INPUT_LIVE_V72_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V72_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V72_PROGRAM_END*/";
...
export function createInputLiveV72Transaction(
  directory: string,
  ...
  const manifest: InputLiveV72Manifest = {
    artifactVersion: "input-live-v72-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V72_BROKER_VERSION,
...
export function validateInputLiveV72Request(
  directory: string,
  request: InputLiveV72Request,
): void {
  ...
  const expectedSequence = inputLiveV72RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v72-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V72_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V72_TARGET.fileKey ||
...
export function issueInputLiveV72Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV72RemotePhase,
  program: string,
  options: InputLiveV72IssueOptions = {},
): InputLiveV72Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV72RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV72Request>(targetPath);
    validateInputLiveV72Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV72ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV72TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV72RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V72_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V72_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V72_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV72RequestBody = {
    artifactVersion: "input-live-v72-broker-request-v2",
    brokerVersion: INPUT_LIVE_V72_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV72Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV72Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV72RawResponse(
  directory: string,
  phase: InputLiveV72RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV72RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV72Request>(requestFile);
  if (
  ...
  const target = inputLiveV72ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV72Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v72-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V72_BROKER_VERSION,
...
export function acceptInputLiveV72Response<T = unknown>(
  directory: string,
  phase: InputLiveV72RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV72AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV72ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV72Request>(
    inputLiveV72RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV72Request(directory, request);
  if (phase === "capture") validateInputLiveV72TechnicalGates(directory);
  const rawFile = inputLiveV72ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v72-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V72_BROKER_VERSION ||
...
export function persistInputLiveV72TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV72RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV72RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V72_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v72-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V72_BROKER_VERSION,
...
export function validateInputLiveV72TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v72-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V72_TECHNICAL_GATES.some(
...
export function validateInputLiveV72Receipt(
  directory: string,
  phase: InputLiveV72RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV72Request(directory, request);
  const receipt = json<InputLiveV72Receipt>(
    inputLiveV72ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV72ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v72-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V72_BROKER_VERSION ||
...
export function readInputLiveV72AcceptedRawPayload(
  directory: string,
  phase: InputLiveV72RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV72Receipt(directory, phase, captureIndex);
...
export function createInputLiveV72FakeDynamicResponse(
  request: InputLiveV72Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V72_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V72_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV72PersistedRequests(
  directory: string,
): InputLiveV72Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV72Request>(file)] : [];
    });
}

export const inputLiveV72Sha256 = sha256;
