...
import { buildCalendarLiveV18CleanupRuntime } from "./calendar-live-v18-cleanup.js";
import { type CalendarLiveV18FixedFixedPoint } from "./calendar-live-v18-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV18RestoreProgram,
  validateCalendarLiveV18RestorePayload,
  type CalendarLiveV18RestorePayload,
} from "./calendar-live-v18-restore.js";
import {
  buildCalendarLiveV18RawPropertyRuntime,
  normalizeCalendarLiveV18Scene,
  type CalendarLiveV18RawNode,
} from "./calendar-live-v18-verifier.js";
...
export const CALENDAR_LIVE_V18_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V18_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V18_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V18_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V18_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V18_SET_COUNT = 3;
export const CALENDAR_LIVE_V18_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V18_HOST_PHASES = 3;
export const CALENDAR_LIVE_V18_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V18_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V18_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V18_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV18SourceId = (typeof CALENDAR_LIVE_V18_SOURCE_IDS)[number];
export type CalendarLiveV18Kind = "calendar" | "week" | "day";

export interface CalendarLiveV18SourceIdentity {
  source: CalendarLiveV18SourceId;
...
export interface CalendarLiveV18WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV18ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV18SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV18RawNode;
    weekScene: CalendarLiveV18RawNode;
    dayScene: CalendarLiveV18RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV18RootProof {
  source: CalendarLiveV18SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV18FixedFixedPoint;
}

export interface CalendarLiveV18ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV18SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV18CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV18SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV18CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV18SourceId;
  frameWidth: number;
...
export interface CalendarLiveV18CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v18 ${label} must be an object`);
...
export function validateCalendarLiveV18WriterPayload(
  value: unknown,
): CalendarLiveV18WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v18") ||
    ...
    result.namespace !== CALENDAR_LIVE_V18_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v18 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v18 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v18 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV18SourceIdentity,
  kind: CalendarLiveV18Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV18ExtractProgram(
  writer: CalendarLiveV18WriterOwnership,
  sources: readonly CalendarLiveV18SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v18 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V18_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V18_NAMESPACE)}
${buildCalendarLiveV18RawPropertyRuntime()}
...
export function validateCalendarLiveV18ExtractPayload(
  value: unknown,
  writer: CalendarLiveV18WriterOwnership,
): CalendarLiveV18ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v18 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V18_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v18 extract schema/one-root mismatch");
  return payload as CalendarLiveV18ExtractPayload;
}

export function proveCalendarLiveV18Roots<Instance>(
  extract: CalendarLiveV18ExtractPayload,
  sources: readonly (CalendarLiveV18SourceIdentity & {
...
): CalendarLiveV18RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v18 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v18 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV18Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV18Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV18Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV18FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v18 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v18 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v18 compile lost owned sets");
...
export function assertCalendarLiveV18RootProofs(
  proofs: readonly CalendarLiveV18RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v18 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v18 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV18ProbeProgram(
  writer: CalendarLiveV18WriterOwnership,
  sources: readonly CalendarLiveV18SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V18_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV18ProbePayload(
  value: unknown,
  writer: CalendarLiveV18WriterOwnership,
): CalendarLiveV18ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV18ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V18_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V18_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v18 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV18CaptureManifest(
  cells: readonly CalendarLiveV18CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V18_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V18_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V18_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V18_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v18 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV18CaptureCell[],
): string => {
  validateCalendarLiveV18CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV18CaptureProgram(
  writer: CalendarLiveV18WriterOwnership,
  cell: CalendarLiveV18CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V18_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v18 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v18 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V18_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV18CapturePayload(
  value: unknown,
  cell: CalendarLiveV18CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV18CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV18CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V18_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V18_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v18 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV18CaptureResponses(
  manifest: readonly CalendarLiveV18CaptureCell[],
  responses: readonly CalendarLiveV18CapturePayload[],
): void {
  validateCalendarLiveV18CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v18 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV18ObjectiveReport {
  artifactVersion: "calendar-live-v18-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV18Objective(
  manifest: readonly CalendarLiveV18CaptureCell[],
  responses: readonly CalendarLiveV18CapturePayload[],
): CalendarLiveV18ObjectiveReport {
  assertCalendarLiveV18CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v18-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV18CleanupProgram(
  writer: CalendarLiveV18WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV18CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V18_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v18 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV18CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV18WriterOwnership,
): CalendarLiveV18CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV18CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v18 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V18_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV18WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
