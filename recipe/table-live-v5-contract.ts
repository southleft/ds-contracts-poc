...
import { buildTableLiveV5CleanupRuntime } from "./table-live-v5-cleanup.js";
import { type TableLiveV5FixedFixedPoint } from "./table-live-v5-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV5RestoreProgram,
  validateTableLiveV5RestorePayload,
  type TableLiveV5RestorePayload,
} from "./table-live-v5-restore.js";
import {
  buildTableLiveV5RawPropertyRuntime,
  normalizeTableLiveV5Scene,
  type TableLiveV5RawNode,
} from "./table-live-v5-verifier.js";
...
export const TABLE_LIVE_V5_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V5_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V5_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V5_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V5_VARIANT_COUNT = 20;
export const TABLE_LIVE_V5_SET_COUNT = 6;
export const TABLE_LIVE_V5_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V5_HOST_PHASES = 3;
export const TABLE_LIVE_V5_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V5_RESTORE_COUNT = 8;
export const TABLE_LIVE_V5_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V5_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV5SourceId = (typeof TABLE_LIVE_V5_SOURCE_IDS)[number];
export type TableLiveV5Kind = "table" | "row" | "cell";

export interface TableLiveV5SourceIdentity {
  source: TableLiveV5SourceId;
...
export interface TableLiveV5WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV5ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV5SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV5RawNode;
    rowScene: TableLiveV5RawNode;
    cellScene: TableLiveV5RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV5RootProof {
  source: TableLiveV5SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV5FixedFixedPoint;
}

export interface TableLiveV5ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV5SourceId;
    adapterIdentity: string;
...
export interface TableLiveV5CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV5SourceId;
  adapterIdentity: string;
...
export interface TableLiveV5CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV5SourceId;
  frameWidth: number;
...
export interface TableLiveV5CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v5 ${label} must be an object`);
...
export function validateTableLiveV5WriterPayload(
  value: unknown,
): TableLiveV5WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V5_NAMESPACE ||
    ...
    throw new TypeError("Table live v5 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v5 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v5 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV5SourceIdentity,
  kind: TableLiveV5Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV5ExtractProgram(
  writer: TableLiveV5WriterOwnership,
  sources: readonly TableLiveV5SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v5 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V5_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V5_NAMESPACE)}
${buildTableLiveV5RawPropertyRuntime()}
...
export function validateTableLiveV5ExtractPayload(
  value: unknown,
  writer: TableLiveV5WriterOwnership,
): TableLiveV5ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v5 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V5_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v5 extract schema/two-root mismatch");
  return payload as TableLiveV5ExtractPayload;
}

export function proveTableLiveV5Roots<Instance>(
  extract: TableLiveV5ExtractPayload,
  sources: readonly (TableLiveV5SourceIdentity & {
...
): TableLiveV5RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v5 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v5 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV5Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV5Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV5Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV5FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v5 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v5 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v5 compile lost owned sets");
...
export function assertTableLiveV5RootProofs(
  proofs: readonly TableLiveV5RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v5 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v5 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV5ProbeProgram(
  writer: TableLiveV5WriterOwnership,
  sources: readonly TableLiveV5SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V5_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV5ProbePayload(
  value: unknown,
  writer: TableLiveV5WriterOwnership,
): TableLiveV5ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV5ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V5_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V5_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Table live v5 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV5CaptureManifest(
  cells: readonly TableLiveV5CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V5_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V5_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V5_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V5_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v5 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV5CaptureCell[],
): string => {
  validateTableLiveV5CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV5CaptureProgram(
  writer: TableLiveV5WriterOwnership,
  cell: TableLiveV5CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V5_CAPTURE_COUNT
  )
    throw new TypeError("Table live v5 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v5 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
const frame=figma.createFrame();frame.name="Table v5 ephemeral / "+cell.cellKey;page.appendChild(frame);
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V5_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV5CapturePayload(
  value: unknown,
  cell: TableLiveV5CaptureCell,
  rawResponseBytes: number,
): TableLiveV5CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV5CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V5_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V5_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v5 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV5CaptureResponses(
  manifest: readonly TableLiveV5CaptureCell[],
  responses: readonly TableLiveV5CapturePayload[],
): void {
  validateTableLiveV5CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v5 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV5ObjectiveReport {
  artifactVersion: "table-live-v5-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV5Objective(
  manifest: readonly TableLiveV5CaptureCell[],
  responses: readonly TableLiveV5CapturePayload[],
): TableLiveV5ObjectiveReport {
  assertTableLiveV5CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v5-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV5CleanupProgram(
  writer: TableLiveV5WriterOwnership,
): string {
  const baseProgram = buildTableLiveV5CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V5_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v5 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV5CleanupPayload(
  payload: unknown,
  writer: TableLiveV5WriterOwnership,
): TableLiveV5CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV5CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v5 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V5_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV5WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
