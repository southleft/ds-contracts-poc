...
import {
  ...
  INPUT_LIVE_V59_INDEX_PATH,
} from "./build-input-field-live-proof-v59.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV59HistoryExpectation = "pending" | "authorized";

export interface InputLiveV59AuthorizationArtifact {
  artifactVersion: "input-live-v59-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V59_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV59AntecedentIndex {
...
export interface InputLiveV59HistoryState {
  index?: InputLiveV59AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV59AuthorizationArtifact;
...
export interface InputLiveV59AuthorizationProof extends InputLiveV59TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V59_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V59_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V59_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V59_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v59-authorization") ||
  artifactPath.includes("input-field-live-v59-preflight") ||
  artifactPath.includes("input-field-live-v59-authorized") ||
  artifactPath.includes("input-field-live-pivot-v59-status") ||
...
export function validateInputLiveV59AntecedentIndex(
  index: InputLiveV59AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v59-antecedent-index-v1" ||
...
export function validateInputLiveV59UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV59History\(\s*["']/.test(source) ||
    /readInputLiveV59HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV59AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV59AuthorizationArtifact {
  ...
  ) as InputLiveV59AntecedentIndex;
  const indexFailures = validateInputLiveV59AntecedentIndex(index);
...
export function validateInputLiveV59AuthorizationArtifact(
  artifact: InputLiveV59AuthorizationArtifact | undefined,
  index: InputLiveV59AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v59-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v59" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V59_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V59_TARGET) ||
    !equalJson(
...
export function validateInputLiveV59History(
  state: InputLiveV59HistoryState,
  expected: InputLiveV59HistoryExpectation,
): string[] {
  const failures = validateInputLiveV59AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV59AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV59HistoryState(
  root = process.cwd(),
): InputLiveV59HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V59_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV59AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V59_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V59_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V59_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV59History(
  expected: InputLiveV59HistoryExpectation,
  root = process.cwd(),
): InputLiveV59HistoryState {
  const state = readInputLiveV59HistoryState(root);
  const failures = validateInputLiveV59History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV59Authorization(
  root = process.cwd(),
): InputLiveV59AuthorizationProof {
  const state = verifyInputLiveV59History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V59_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v59/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v59.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V59_TARGET,
    expectedDynamicTool: INPUT_LIVE_V59_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V59_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV59HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV59History(expected);
