...
import {
  verifyCalendarLiveV16FixedSceneFixedPoint,
  type CalendarLiveV16FixedFixedPoint,
} from "./calendar-live-v16-fixed-point.js";

export const CALENDAR_LIVE_V16_VERIFIER_VERSION = "calendar-live-v16-verifier-v1";

export function buildCalendarLiveV16RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV16RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV16RawNode[];
}

export interface CalendarLiveV16NormalizedScene {
...
export interface CalendarLiveV16SceneProof {
...
export function normalizeCalendarLiveV16Scene(
  root: CalendarLiveV16RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV16NormalizedScene {
  const canonicalBindings: CalendarLiveV16NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV16RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV16Scene<Instance>(
  normalized: CalendarLiveV16NormalizedScene,
...
): CalendarLiveV16SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV16FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV16ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV16PreCaptureGates(payload: unknown): void {
