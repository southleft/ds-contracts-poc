...
export const INPUT_LIVE_V21_VERIFIER_VERSION = "input-live-v21-verifier-v1";

export function buildInputLiveV21RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV21RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV21RawNode[];
}

export interface InputLiveV21NormalizedScene {
...
export interface InputLiveV21SceneProof {
...
export function normalizeInputLiveV21Scene(
  root: InputLiveV21RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV21NormalizedScene {
  const canonicalBindings: InputLiveV21NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV21RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV21Scene<Instance>(
  normalized: InputLiveV21NormalizedScene,
...
export function compileInputLiveV21ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV21PreCaptureGates(payload: unknown): void {
