...
export const INPUT_LIVE_V28_VERIFIER_VERSION = "input-live-v28-verifier-v1";

export function buildInputLiveV28RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV28RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV28RawNode[];
}

export interface InputLiveV28NormalizedScene {
...
export interface InputLiveV28SceneProof {
...
export function normalizeInputLiveV28Scene(
  root: InputLiveV28RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV28NormalizedScene {
  const canonicalBindings: InputLiveV28NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV28RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV28Scene<Instance>(
  normalized: InputLiveV28NormalizedScene,
...
export function compileInputLiveV28ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV28PreCaptureGates(payload: unknown): void {
