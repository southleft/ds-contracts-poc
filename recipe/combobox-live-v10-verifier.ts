...
import {
  verifyComboboxLiveV10FixedSceneFixedPoint,
  type ComboboxLiveV10FixedFixedPoint,
} from "./combobox-live-v10-fixed-point.js";

export const COMBOBOX_LIVE_V10_VERIFIER_VERSION = "combobox-live-v10-verifier-v1";

export function buildComboboxLiveV10RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV10RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV10RawNode[];
}

export interface ComboboxLiveV10NormalizedScene {
...
export interface ComboboxLiveV10SceneProof {
...
export function normalizeComboboxLiveV10Scene(
  root: ComboboxLiveV10RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV10NormalizedScene {
  const canonicalBindings: ComboboxLiveV10NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV10RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV10Scene<Instance>(
  normalized: ComboboxLiveV10NormalizedScene,
...
): ComboboxLiveV10SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV10FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV10ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV10PreCaptureGates(payload: unknown): void {
