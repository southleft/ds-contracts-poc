...
import { verifyTableLiveV9Authorization } from "./table-live-v9-authorization.js";
import { TABLE_LIVE_V9_TARGET } from "./table-live-v9-broker.js";
import {
  TABLE_LIVE_V9_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV9Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-table-v9-session";
...
const attestation: TableLiveV9SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV9SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v9 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V9_SECURITY_ATTESTATION_DEFAULT_PATH,
);
