...
import { buildComboboxLiveV19CleanupRuntime } from "./combobox-live-v19-cleanup.js";
import {
  type ComboboxLiveV19FixedFixedPoint,
} from "./combobox-live-v19-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV19RestoreProgram,
  validateComboboxLiveV19RestorePayload,
  type ComboboxLiveV19RestorePayload,
} from "./combobox-live-v19-restore.js";
import {
  buildComboboxLiveV19RawPropertyRuntime,
  normalizeComboboxLiveV19Scene,
  type ComboboxLiveV19RawNode,
} from "./combobox-live-v19-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v19.js";
import {
...
} from "./scene-readback-combobox-v19.js";
...
export const COMBOBOX_LIVE_V19_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V19_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V19_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V19_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V19_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V19_SET_COUNT = 4;
export const COMBOBOX_LIVE_V19_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V19_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V19_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V19_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V19_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV19SourceId = (typeof COMBOBOX_LIVE_V19_SOURCE_IDS)[number];

export interface ComboboxLiveV19SourceIdentity {
  source: ComboboxLiveV19SourceId;
...
export interface ComboboxLiveV19WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV19ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV19SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV19RawNode;
    optionScene: ComboboxLiveV19RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV19RootProof {
  source: ComboboxLiveV19SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV19FixedFixedPoint;
}

export interface ComboboxLiveV19ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV19SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV19CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV19SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV19CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV19SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV19CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v19 ${label} must be an object`);
...
export function validateComboboxLiveV19WriterPayload(
  value: unknown,
): ComboboxLiveV19WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V19_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v19 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v19 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v19 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV19SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV19ExtractProgram(
  writer: ComboboxLiveV19WriterOwnership,
  sources: readonly ComboboxLiveV19SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v19 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V19_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V19_NAMESPACE)}
${buildComboboxLiveV19RawPropertyRuntime()}
...
export function validateComboboxLiveV19ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV19WriterOwnership,
): ComboboxLiveV19ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v19 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V19_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v19 extract schema/two-root mismatch");
  return payload as ComboboxLiveV19ExtractPayload;
}

export function proveComboboxLiveV19Roots<Instance>(
  extract: ComboboxLiveV19ExtractPayload,
  sources: readonly (ComboboxLiveV19SourceIdentity & {
...
): ComboboxLiveV19RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v19 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v19 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV19Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV19Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV19FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v19 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v19 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v19 compile lost owned sets");
...
export function assertComboboxLiveV19RootProofs(
  proofs: readonly ComboboxLiveV19RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v19 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v19 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV19ProbeProgram(
  writer: ComboboxLiveV19WriterOwnership,
  sources: readonly ComboboxLiveV19SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V19_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV19ProbePayload(
  value: unknown,
  writer: ComboboxLiveV19WriterOwnership,
): ComboboxLiveV19ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV19ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V19_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V19_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v19 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV19CaptureManifest(
  cells: readonly ComboboxLiveV19CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V19_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V19_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V19_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V19_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v19 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV19CaptureCell[],
): string => {
  validateComboboxLiveV19CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV19CaptureProgram(
  writer: ComboboxLiveV19WriterOwnership,
  cell: ComboboxLiveV19CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V19_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v19 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v19 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V19_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV19CapturePayload(
  value: unknown,
  cell: ComboboxLiveV19CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV19CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV19CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V19_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V19_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v19 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV19CaptureResponses(
  manifest: readonly ComboboxLiveV19CaptureCell[],
  responses: readonly ComboboxLiveV19CapturePayload[],
): void {
  validateComboboxLiveV19CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v19 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV19ObjectiveReport {
  artifactVersion: "combobox-live-v19-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV19Objective(
  manifest: readonly ComboboxLiveV19CaptureCell[],
  responses: readonly ComboboxLiveV19CapturePayload[],
): ComboboxLiveV19ObjectiveReport {
  assertComboboxLiveV19CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v19-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV19CleanupProgram(
  writer: ComboboxLiveV19WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV19CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V19_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v19 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV19CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV19WriterOwnership,
): ComboboxLiveV19CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV19CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v19 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V19_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV19WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
