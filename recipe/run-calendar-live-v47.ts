...
import {
  acceptCalendarLiveV47Response,
  createCalendarLiveV47Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV47Request,
  persistCalendarLiveV47RawResponse,
  persistCalendarLiveV47TechnicalGates,
  readCalendarLiveV47AcceptedRawPayload,
  type CalendarLiveV47Request,
  type CalendarLiveV47TransactionAuthorization,
} from "./calendar-live-v47-broker.js";
import {
  CALENDAR_LIVE_V47_CAPTURE_COUNT,
  assertCalendarLiveV47CaptureResponses,
  ...
  evaluateCalendarLiveV47Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV47Roots,
  validateCalendarLiveV47CaptureManifest,
  validateCalendarLiveV47CapturePayload,
  validateCalendarLiveV47CleanupPayload,
  validateCalendarLiveV47ExtractPayload,
  validateCalendarLiveV47ProbePayload,
  validateCalendarLiveV47WriterPayload,
  type CalendarLiveV47CaptureCell,
  type CalendarLiveV47CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V47_PLAN_PATH,
  type CalendarLiveV47ProofPlan,
} from "./build-calendar-live-proof-v47.js";
import {
...
export const CALENDAR_LIVE_V47_RUNNER_VERSION = "calendar-live-v47-runner-v1";

interface CalendarLiveV47JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV47OrchestratorState {
  artifactVersion: "calendar-live-v47-orchestrator-state-v1";
  ...
  journal: CalendarLiveV47JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV47ProofPlan;
  planSha256: string;
  captures: CalendarLiveV47CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV47SourceIdentity & {
...
export interface CalendarLiveV47NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV47OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v47 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V47_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V47_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV47ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV47CaptureCell[];
  validateCalendarLiveV47CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v47 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v47 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV47WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV47RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV47Request =>
  readJson<CalendarLiveV47Request>(file);

export class CalendarLiveV47Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV47Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV47OrchestratorState = {
      artifactVersion: "calendar-live-v47-orchestrator-state-v1",
    ...
    return new CalendarLiveV47Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV47OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV47Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV47NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV47NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v47 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV47NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v47 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV47RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV47Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV47CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV47AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV47WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV47Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV47ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV47Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV47Response(this.#directory, "writer", {
        validate: validateCalendarLiveV47WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV47Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV47RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV47Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV47ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV47Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV47RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v47-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV47Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV47ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV47Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV47ProbePayload(payload, ownership),
      });
      persistCalendarLiveV47TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v47 capture cell missing");
      const accepted = acceptCalendarLiveV47Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV47CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V47_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV47CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV47Objective(
    ...
    throw new Error(`Calendar live v47 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV47Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV47CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV47Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV47RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV47Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV47CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV47CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV47CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV47Authorization = (
  privateKey: KeyObject,
): CalendarLiveV47TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV47SimulationKey = (): KeyObject =>
