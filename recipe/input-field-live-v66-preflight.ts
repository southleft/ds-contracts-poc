...
import {
  ...
  verifyInputLiveV66Authorization,
  type InputLiveV66AntecedentIndex,
  type InputLiveV66AuthorizationProof,
} from "./input-field-live-v66-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V66_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v66/operator-security-attestation-template.json";
export const INPUT_LIVE_V66_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v66-security-attestation.json";

export interface InputLiveV66SecurityAttestation {
...
export interface InputLiveV66PreflightReport {
  artifactVersion: "input-live-v66-preflight-v1";
...
export const inputLiveV66AttestationSha256 = (
  value: Omit<InputLiveV66SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV66SecurityAttestation(
  value: unknown,
  proof: InputLiveV66AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v66-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V66_TARGET) ||
...
export function validateInputLiveV66ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV66Authorization");
  const preflight = source.indexOf("runInputLiveV66Preflight");
  const initialize = source.indexOf("InputLiveV66Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV66Preflight(
  root: string,
  proof: InputLiveV66AuthorizationProof,
  attestationPath = INPUT_LIVE_V66_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV66PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V66_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V66_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V66_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV66AntecedentIndex;
  failures.push(...validateInputLiveV66AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V66_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V66_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v66-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV66ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV66SecurityAttestation;
  return {
    artifactVersion: "input-live-v66-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V66_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV66Preflight(
    process.cwd(),
    verifyInputLiveV66Authorization(),
    attestationPath,
