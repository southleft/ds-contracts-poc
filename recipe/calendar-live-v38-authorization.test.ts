...
import { CALENDAR_LIVE_V38_INDEX_PATH } from "./build-calendar-live-proof-v38.js";
import {
...
import {
  buildCalendarLiveV38AuthorizationArtifact,
  validateCalendarLiveV38AntecedentIndex,
  validateCalendarLiveV38AuthorizationArtifact,
  validateCalendarLiveV38History,
  validateCalendarLiveV38UnitTestSource,
  type CalendarLiveV38AntecedentIndex,
  type CalendarLiveV38AuthorizationArtifact,
  type CalendarLiveV38AuthorizationProof,
  type CalendarLiveV38HistoryState,
} from "./calendar-live-v38-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV38ControlFlowSource,
  validateCalendarLiveV38SecurityAttestation,
  type CalendarLiveV38SecurityAttestation,
} from "./calendar-live-v38-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V38_INDEX_PATH);
const index = (): CalendarLiveV38AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV38AntecedentIndex;

const artifact = (): CalendarLiveV38AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV38AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV38HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV38HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV38AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v38/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV38AuthorizationArtifact,
): CalendarLiveV38SecurityAttestation => {
  const body: Omit<CalendarLiveV38SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V38_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV38History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV38History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV38History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV38History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV38History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV38UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v38-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV38UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV38AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV38AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV38AuthorizationArtifact(
        planted as CalendarLiveV38AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV38SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV38SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV38SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v38-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV38ControlFlowSource(source), []);
});
