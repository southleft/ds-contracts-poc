...
import { COMBOBOX_LIVE_V20_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v20.js";
import { verifyComboboxLiveV20Authorization } from "./combobox-live-v20-authorization.js";
import { runComboboxLiveV20Preflight } from "./combobox-live-v20-preflight.js";
import { ComboboxLiveV20Orchestrator } from "./run-combobox-live-v20.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v20 requires exact authorization artifact ${COMBOBOX_LIVE_V20_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v20-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV20Authorization(root);
    runComboboxLiveV20Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV20Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV20Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
