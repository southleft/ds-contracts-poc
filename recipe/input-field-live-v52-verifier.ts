...
export const INPUT_LIVE_V52_VERIFIER_VERSION = "input-live-v52-verifier-v1";

export function buildInputLiveV52RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV52RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV52RawNode[];
}

export interface InputLiveV52NormalizedScene {
...
export interface InputLiveV52SceneProof {
...
export function normalizeInputLiveV52Scene(
  root: InputLiveV52RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV52NormalizedScene {
  const canonicalBindings: InputLiveV52NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV52RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV52Scene<Instance>(
  normalized: InputLiveV52NormalizedScene,
...
export function compileInputLiveV52ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV52PreCaptureGates(payload: unknown): void {
