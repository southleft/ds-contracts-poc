...
export const INPUT_LIVE_V10_VERIFIER_VERSION = "input-live-v10-verifier-v1";

export function buildInputLiveV10RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV10RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV10RawNode[];
}

export interface InputLiveV10NormalizedScene {
...
export interface InputLiveV10SceneProof {
...
export function normalizeInputLiveV10Scene(
  root: InputLiveV10RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV10NormalizedScene {
  const canonicalBindings: InputLiveV10NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: InputLiveV10RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV10Scene<Instance>(
  normalized: InputLiveV10NormalizedScene,
...
export function compileInputLiveV10ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV10PreCaptureGates(payload: unknown): void {
