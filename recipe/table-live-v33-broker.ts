...
import {
  ...
  TABLE_LIVE_V33_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  TABLE_LIVE_V33_RESPONSE_CONTRACTS,
  type TableLiveV33RootProof,
} from "./table-live-v33-contract.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V33_BROKER_VERSION = "table-live-v33-broker-v1";
export const TABLE_LIVE_V33_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const TABLE_LIVE_V33_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const TABLE_LIVE_V33_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const TABLE_LIVE_V33_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const TABLE_LIVE_V33_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const TABLE_LIVE_V33_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type TableLiveV33RemotePhase =
  (typeof TABLE_LIVE_V33_REMOTE_PHASES)[number];
export type TableLiveV33AuthorizationMode = "live" | "simulated";

export interface TableLiveV33CommitPins {
...
export interface TableLiveV33TransactionAuthorization extends TableLiveV33CommitPins {
  mode: TableLiveV33AuthorizationMode;
...
export interface TableLiveV33Manifest {
  artifactVersion: "table-live-v33-broker-manifest-v2";
  brokerVersion: typeof TABLE_LIVE_V33_BROKER_VERSION;
...
export interface TableLiveV33ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface TableLiveV33RequestBody {
  artifactVersion: "table-live-v33-broker-request-v2";
  brokerVersion: typeof TABLE_LIVE_V33_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: TableLiveV33RemotePhase;
  sequence: number;
  ...
  commitPins: TableLiveV33CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof TABLE_LIVE_V33_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: TableLiveV33ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface TableLiveV33Request extends TableLiveV33RequestBody {
...
export interface TableLiveV33Receipt {
  artifactVersion: "table-live-v33-broker-receipt-v2";
  brokerVersion: typeof TABLE_LIVE_V33_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: TableLiveV33RemotePhase;
...
export interface TableLiveV33AcceptedResponse<T = unknown> {
  request: TableLiveV33Request;
  receipt: TableLiveV33Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface TableLiveV33IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: TableLiveV33ResponseContract;
...
const equalTarget = (value: unknown): value is typeof TABLE_LIVE_V33_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === TABLE_LIVE_V33_TARGET.fileKey &&
  (value as BrokerTarget).fileName === TABLE_LIVE_V33_TARGET.fileName &&
  (value as BrokerTarget).editorType === TABLE_LIVE_V33_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== TABLE_LIVE_V33_TARGET.fileKey ||
    context.fileName !== TABLE_LIVE_V33_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === TABLE_LIVE_V33_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Table live v33 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= TABLE_LIVE_V33_CAPTURE_COUNT
  )
    throw new TypeError("Table live v33 capture index must be 0..19");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Table live v33 authorization pins are malformed");
};

const readManifest = (directory: string): TableLiveV33Manifest => {
  const manifest = json<TableLiveV33Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Table live v33 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "table-live-v33-broker-manifest-v2" ||
    manifest.brokerVersion !== TABLE_LIVE_V33_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== TABLE_LIVE_V33_CAPTURE_COUNT ||
    ...
    throw new TypeError("invalid Table live v33 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: TableLiveV33Request,
): TableLiveV33RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: TableLiveV33RemotePhase,
  captureIndex?: number,
): TableLiveV33Request | undefined => {
  const file = tableLiveV2RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV33Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: TableLiveV33RemotePhase,
  captureIndex?: number,
): TableLiveV33Receipt | undefined => {
  const file = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV33Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: TableLiveV33Request | undefined;
  receipt: TableLiveV33Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: TableLiveV33RemotePhase,
): TableLiveV33ResponseContract => ({
  ...TABLE_LIVE_V33_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: TABLE_LIVE_V33_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "table-live-v33-plugin-response-v2",
  brokerVersion: TABLE_LIVE_V33_BROKER_VERSION,
  target: TABLE_LIVE_V33_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*TABLE_LIVE_V33_PROGRAM_START*/${program}/*TABLE_LIVE_V33_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*TABLE_LIVE_V33_PROGRAM_START*/";
  const endMarker = "/*TABLE_LIVE_V33_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Table live v33 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createTableLiveV33Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Table live v33 private signing key does not match authorized public-key identity",
    );
  const manifest: TableLiveV33Manifest = {
    artifactVersion: "table-live-v33-broker-manifest-v2",
    brokerVersion: TABLE_LIVE_V33_BROKER_VERSION,
    ...
    captureRequests: TABLE_LIVE_V33_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateTableLiveV33Request(
  directory: string,
  request: TableLiveV33Request,
): void {
  ...
  if (
    request.artifactVersion !== "table-live-v33-broker-request-v2" ||
    request.brokerVersion !== TABLE_LIVE_V33_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== TABLE_LIVE_V33_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Table live v33 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueTableLiveV33Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: TableLiveV33RemotePhase,
  program: string,
  options: TableLiveV33IssueOptions = {},
): TableLiveV33Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<TableLiveV33Request>(targetPath);
    validateTableLiveV33Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Table live v33 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(tableLiveV2ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Table live v33 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Table live v33 empty code envelope refused");
  if (phase === "capture") validateTableLiveV33TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Table live v33 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: TABLE_LIVE_V33_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? TABLE_LIVE_V33_SIGNED_WRITER_TIMEOUT_MS
        : TABLE_LIVE_V33_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: TableLiveV33RequestBody = {
    artifactVersion: "table-live-v33-broker-request-v2",
    brokerVersion: TABLE_LIVE_V33_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: TableLiveV33Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateTableLiveV33Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistTableLiveV33RawResponse(
  directory: string,
  phase: TableLiveV33RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Table live v33 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<TableLiveV33Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Table live v33 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = tableLiveV2ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Table live v33 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<TableLiveV33Receipt, "receiptSha256"> = {
    artifactVersion: "table-live-v33-broker-receipt-v2",
    brokerVersion: TABLE_LIVE_V33_BROKER_VERSION,
...
export function acceptTableLiveV33Response<T = unknown>(
  directory: string,
  phase: TableLiveV33RemotePhase,
  options: {
    captureIndex?: number;
...
): TableLiveV33AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Table live v33 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<TableLiveV33Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Table live v33 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "table-live-v33-plugin-response-v2" ||
    plugin?.brokerVersion !== TABLE_LIVE_V33_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Table live v33 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistTableLiveV33TechnicalGates(
  directory: string,
  rootProofs: readonly TableLiveV33RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertTableLiveV33RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Table live v33 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Table live v33 probe payload hash malformed");
  const gates = Object.fromEntries(
    TABLE_LIVE_V33_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "table-live-v33-host-technical-gates-v2",
    brokerVersion: TABLE_LIVE_V33_BROKER_VERSION,
...
export function validateTableLiveV33TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "table-live-v33-host-technical-gates-v2" ||
    ...
    TABLE_LIVE_V33_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Table live v33 technical gates artifact");
}

export function validateTableLiveV33Receipt(
  directory: string,
  phase: TableLiveV33RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateTableLiveV33Request(directory, request);
  const receipt = json<TableLiveV33Receipt>(
  ...
  if (
    receipt.artifactVersion !== "table-live-v33-broker-receipt-v2" ||
    receipt.brokerVersion !== TABLE_LIVE_V33_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Table live v33 accepted receipt: ${request.requestId}`,
    );
}

export function readTableLiveV33AcceptedRawPayload(
  directory: string,
  phase: TableLiveV33RemotePhase,
  captureIndex?: number,
): unknown {
  validateTableLiveV33Receipt(directory, phase, captureIndex);
...
export function createTableLiveV33FakeDynamicResponse(
  request: TableLiveV33Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? TABLE_LIVE_V33_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? TABLE_LIVE_V33_TARGET,
    ...overrides.extra,
  };
}

export function listTableLiveV33PersistedRequests(
  directory: string,
): TableLiveV33Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<TableLiveV33Request>(file)] : [];
    });
}
