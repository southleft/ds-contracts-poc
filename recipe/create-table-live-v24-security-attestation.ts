...
import { verifyTableLiveV24Authorization } from "./table-live-v24-authorization.js";
import { TABLE_LIVE_V24_TARGET } from "./table-live-v24-broker.js";
import {
  TABLE_LIVE_V24_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV24Authorization();
...
const attestation: TableLiveV24SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV24SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v24 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V24_SECURITY_ATTESTATION_DEFAULT_PATH,
);
