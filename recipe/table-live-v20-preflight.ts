...
import {
  validateTableLiveV20AntecedentIndex,
  verifyTableLiveV20Authorization,
  type TableLiveV20AntecedentIndex,
  type TableLiveV20AuthorizationProof,
} from "./table-live-v20-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V20_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v20/operator-security-attestation-template.json";
export const TABLE_LIVE_V20_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v20-security-attestation.json";

export interface TableLiveV20SecurityAttestation {
  artifactVersion: "table-live-v20-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV20PreflightReport {
  artifactVersion: "table-live-v20-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV20SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV20SecurityAttestation(
  value: unknown,
  proof: TableLiveV20AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v20-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V20_TARGET) ||
...
export function validateTableLiveV20ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV20Authorization");
  const preflight = source.indexOf("runTableLiveV20Preflight");
  const initialize = source.indexOf("TableLiveV20Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV20Preflight(
  root: string,
  proof: TableLiveV20AuthorizationProof,
  attestationPath = TABLE_LIVE_V20_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV20PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V20_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V20_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV20AntecedentIndex;
  failures.push(...validateTableLiveV20AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V20_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V20_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV20SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v20-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV20ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v20 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV20SecurityAttestation;
  return {
    artifactVersion: "table-live-v20-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V20_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV20Preflight(
    process.cwd(),
    verifyTableLiveV20Authorization(),
    attestationPath,
