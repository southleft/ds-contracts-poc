...
import {
  ...
  TABLE_LIVE_V15_HOST_PHASES,
  TABLE_LIVE_V15_REMOTE_REQUESTS,
  TABLE_LIVE_V15_SOURCE_ROOTS,
  TABLE_LIVE_V15_VARIANT_COUNT,
  buildTableLiveV15CaptureProgram,
  buildTableLiveV15CleanupProgram,
  buildTableLiveV15ExtractProgram,
  buildTableLiveV15ProbeProgram,
  buildTableLiveV15RestoreProgram,
  tableLiveV2CaptureManifestSha256,
  type TableLiveV15CaptureCell,
  type TableLiveV15SourceIdentity,
  type TableLiveV15WriterOwnership,
} from "./table-live-v15-contract.js";
import {
  TABLE_LIVE_V15_DYNAMIC_TOOL,
  TABLE_LIVE_V15_TARGET,
  tableLiveV2RequestSequence,
} from "./table-live-v15-broker.js";
import {
...
export const TABLE_LIVE_V15_EVIDENCE_ROOT = "recipe/evidence/table-live-pivot-v15";
export const TABLE_LIVE_V15_PROTOCOL_PATH = `${TABLE_LIVE_V15_EVIDENCE_ROOT}/protocol.json`;
export const TABLE_LIVE_V15_PLAN_PATH = `${TABLE_LIVE_V15_EVIDENCE_ROOT}/proof-plan.json`;
export const TABLE_LIVE_V15_CAPTURE_MANIFEST_PATH = `${TABLE_LIVE_V15_EVIDENCE_ROOT}/capture-manifest.json`;
export const TABLE_LIVE_V15_REQUEST_MANIFEST_PATH = `${TABLE_LIVE_V15_EVIDENCE_ROOT}/request-manifest.json`;
export const TABLE_LIVE_V15_INDEX_PATH = `${TABLE_LIVE_V15_EVIDENCE_ROOT}/antecedent-index.json`;
export const TABLE_LIVE_V15_AUTHORIZATION_TEMPLATE_PATH = `${TABLE_LIVE_V15_EVIDENCE_ROOT}/authorization-template.json`;
export const TABLE_LIVE_V15_AUTHORIZATION_PATH = `${TABLE_LIVE_V15_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface TableLiveV15ProofPlan {
  artifactVersion: "table-live-v15-proof-plan-v1";
...
const captureCells = (
...
): TableLiveV15CaptureCell[] => {
  const cells: TableLiveV15CaptureCell[] = [];
  ...
  if (cells.length !== TABLE_LIVE_V15_CAPTURE_COUNT)
    throw new Error(
      `Table live v15 capture cells ${cells.length}, expected ${TABLE_LIVE_V15_CAPTURE_COUNT}`,
    );
...
const protocol = (expectedFacts: number) => ({
  artifactVersion: "table-live-v15-external-operator-protocol-v1",
  protocolId: "table-live-v15",
  status:
  ...
  denominator: {
    sourceRoots: TABLE_LIVE_V15_SOURCE_ROOTS,
    sets: 6,
    variants: TABLE_LIVE_V15_VARIANT_COUNT,
    captureCells: TABLE_LIVE_V15_CAPTURE_COUNT,
    remoteRequests: TABLE_LIVE_V15_REMOTE_REQUESTS,
    hostPhases: TABLE_LIVE_V15_HOST_PHASES,
...
export async function buildTableLiveV15Proof(
  check = process.argv.includes("--check"),
): Promise<TableLiveV15ProofPlan> {
  ...
  if (
    ...
    throw new Error("Table live v15 writer reused Input, Combobox, or Table v1–v14 identity");
  const captures = captureCells(sources);
  const writerProgram = Buffer.from(writer.code);
  const writerOwnershipBlueprint: TableLiveV15WriterOwnership = {
  ...
  const identities: TableLiveV15SourceIdentity[] = sources.map((source) => ({
    source: source.source,
    adapterIdentity: source.adapterIdentity,
    recipeHash: source.recipeHash,
    envelopeHash: source.envelope.integrity.canonicalHash,
  ...
  const restoreBlueprint = Buffer.from(
    buildTableLiveV15RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildTableLiveV15ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildTableLiveV15ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildTableLiveV15CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );
  const cleanupBlueprint = Buffer.from(
    buildTableLiveV15CleanupProgram(writerOwnershipBlueprint),
  );
  ...
  const sourceMetadata = sources.map((source) => {
    ...
    const tablePath = `${TABLE_LIVE_V15_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-table.json.gz`;
    const rowPath = `${TABLE_LIVE_V15_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-row.json.gz`;
    const cellPath = `${TABLE_LIVE_V15_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-cell.json.gz`;
  ...
  antecedentOutputs.set(
    TABLE_LIVE_V15_PROTOCOL_PATH,
    jsonBytes(protocol(expectedFacts)),
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V15_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V15_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V15_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V15_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V15_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V15_EVIDENCE_ROOT}/programs/cleanup-blueprint.js`,
    cleanupBlueprint,
  );

  const captureManifest = {
    artifactVersion: "table-live-v15-capture-manifest-v1",
  ...
  antecedentOutputs.set(
    TABLE_LIVE_V15_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: TableLiveV15ProofPlan = {
    artifactVersion: "table-live-v15-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: TABLE_LIVE_V15_TARGET,
    namespace: TABLE_FIGMA_NAMESPACE,
    writer: {
      programPath: `${TABLE_LIVE_V15_EVIDENCE_ROOT}/programs/writer.txt`,
    ...
    captureManifest: {
      path: TABLE_LIVE_V15_CAPTURE_MANIFEST_PATH,
      cells: 20,
      requests: 20,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(TABLE_LIVE_V15_PLAN_PATH, jsonBytes(proofPlan));

  const requests = {
    artifactVersion: "table-live-v15-request-manifest-v1",
    expectedDynamicTool: TABLE_LIVE_V15_DYNAMIC_TOOL,
    target: TABLE_LIVE_V15_TARGET,
  ...
  if (requests.requestCount !== TABLE_LIVE_V15_REMOTE_REQUESTS)
    throw new Error("Table live v15 remote request denominator drifted");
  antecedentOutputs.set(
    TABLE_LIVE_V15_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const lifecycleExcludedPaths = [
    TABLE_LIVE_V15_AUTHORIZATION_TEMPLATE_PATH,
    TABLE_LIVE_V15_AUTHORIZATION_PATH,
    "recipe/table-live-v15-authorization.ts",
    "recipe/table-live-v15-authorization.test.ts",
    "recipe/table-live-v15-preflight.ts",
    "recipe/table-live-v15-authorized.ts",
    "recipe/write-table-live-v15-authorization.ts",
    "recipe/create-table-live-v15-security-attestation.ts",
    "recipe/evidence/table-live-pivot-v15/operator-security-attestation-template.json",
    "recipe/evidence/table-live-pivot-v15-status.json",
    "recipe/evidence/status-index.json",
  ];
  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/table-live-v15-broker.ts",
    "recipe/table-live-v15-contract.ts",
    "recipe/table-live-v15-restore.ts",
    "recipe/table-live-v15-cleanup.ts",
    "recipe/table-live-v15-verifier.ts",
    "recipe/table-live-v15-fixed-point.ts",
    "recipe/run-table-live-v15.ts",
    "recipe/build-table-live-proof-v15.ts",
    "recipe/table-live-v15-lifecycle-simulation.ts",
    "recipe/scene-readback-table-v1.ts",
    "recipe/scene-readback-runtime-table-v15.ts",
    "recipe/table-figma-writer.ts",
    "recipe/figma-property-normalizer-v8.ts",
    "recipe/figma-runtime-portability.ts",
    "recipe/normalize.ts",
  ].filter((artifactPath) => artifactPath !== TABLE_LIVE_V15_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "table-live-v15-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
  ...
  antecedentOutputs.set(TABLE_LIVE_V15_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "table-live-v15-authorization-template-v1",
    authorizationId: "table-live-v15",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: TABLE_LIVE_V15_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
    ...
    denominator: {
      remoteRequests: TABLE_LIVE_V15_REMOTE_REQUESTS,
      hostPhases: TABLE_LIVE_V15_HOST_PHASES,
      captures: TABLE_LIVE_V15_CAPTURE_COUNT,
      sourceRoots: TABLE_LIVE_V15_SOURCE_ROOTS,
      expectedFacts,
    },
    execution: {
  ...
  const attestationTemplate = jsonBytes({
    artifactVersion: "table-live-v15-operator-security-attestation-template-v1",
    status: "template only; complete privately after PREPARE+AUTHORIZE",
    tokenValuesForbidden: true,
    privateDirectory: "private/table-live-v15-security-attestation.json",
  });
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Table live v15 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(TABLE_LIVE_V15_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(TABLE_LIVE_V15_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
    writeFileSync(
      `${TABLE_LIVE_V15_EVIDENCE_ROOT}/operator-security-attestation-template.json`,
      attestationTemplate,
    );
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(TABLE_LIVE_V15_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(TABLE_LIVE_V15_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Table live v15 authorization template drift");
  }
  if (
    writer.namespace !== TABLE_FIGMA_NAMESPACE ||
    writer.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    ...
    throw new Error("Table live v15 identity or cleanup invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildTableLiveV15Proof(), null, 2)}\n`,
  );
