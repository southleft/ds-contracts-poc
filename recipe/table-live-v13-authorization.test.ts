...
import { TABLE_LIVE_V13_INDEX_PATH } from "./build-table-live-proof-v13.js";
import {
...
import {
  buildTableLiveV13AuthorizationArtifact,
  validateTableLiveV13AntecedentIndex,
  validateTableLiveV13AuthorizationArtifact,
  validateTableLiveV13History,
  validateTableLiveV13UnitTestSource,
  type TableLiveV13AntecedentIndex,
  type TableLiveV13AuthorizationArtifact,
  type TableLiveV13AuthorizationProof,
  type TableLiveV13HistoryState,
} from "./table-live-v13-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV13ControlFlowSource,
  validateTableLiveV13SecurityAttestation,
  type TableLiveV13SecurityAttestation,
} from "./table-live-v13-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V13_INDEX_PATH);
const index = (): TableLiveV13AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV13AntecedentIndex;

const artifact = (): TableLiveV13AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV13AuthorizationArtifact({
...
const pendingState = (): TableLiveV13HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV13HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV13AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v13/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV13AuthorizationArtifact,
): TableLiveV13SecurityAttestation => {
  const body: Omit<TableLiveV13SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V13_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV13History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV13History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV13History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV13History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV13History(authorizedState(), "pending").join("\n"),
    /stale history mode.*authorization exists/,
  );
});

test("the exact v6 phase-sensitive self-test defect is planted and table v13 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV13UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v13-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV13UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV13AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV13AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV13AuthorizationArtifact(
        planted as TableLiveV13AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV13SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV13SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV13SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v13-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV13ControlFlowSource(source), []);
});
