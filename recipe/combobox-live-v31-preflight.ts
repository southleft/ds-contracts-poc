...
import {
  validateComboboxLiveV31AntecedentIndex,
  verifyComboboxLiveV31Authorization,
  type ComboboxLiveV31AntecedentIndex,
  type ComboboxLiveV31AuthorizationProof,
} from "./combobox-live-v31-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V31_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v31/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V31_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v31-security-attestation.json";

export interface ComboboxLiveV31SecurityAttestation {
  artifactVersion: "combobox-live-v31-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV31PreflightReport {
  artifactVersion: "combobox-live-v31-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV31SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV31SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV31AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v31-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V31_TARGET) ||
...
export function validateComboboxLiveV31ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV31Authorization");
  const preflight = source.indexOf("runComboboxLiveV31Preflight");
  const initialize = source.indexOf("ComboboxLiveV31Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV31Preflight(
  root: string,
  proof: ComboboxLiveV31AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V31_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV31PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V31_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V31_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V31_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV31AntecedentIndex;
  failures.push(...validateComboboxLiveV31AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V31_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V31_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV31SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v31-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV31ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v31 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV31SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v31-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V31_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV31Preflight(
    process.cwd(),
    verifyComboboxLiveV31Authorization(),
    attestationPath,
