...
import { buildComboboxLiveV31CleanupRuntime } from "./combobox-live-v31-cleanup.js";
import {
  type ComboboxLiveV31FixedFixedPoint,
} from "./combobox-live-v31-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV31RestoreProgram,
  validateComboboxLiveV31RestorePayload,
  type ComboboxLiveV31RestorePayload,
} from "./combobox-live-v31-restore.js";
import {
  buildComboboxLiveV31RawPropertyRuntime,
  normalizeComboboxLiveV31Scene,
  type ComboboxLiveV31RawNode,
} from "./combobox-live-v31-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v31.js";
import {
...
} from "./scene-readback-combobox-v31.js";
...
export const COMBOBOX_LIVE_V31_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V31_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V31_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V31_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V31_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V31_SET_COUNT = 4;
export const COMBOBOX_LIVE_V31_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V31_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V31_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V31_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V31_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV31SourceId = (typeof COMBOBOX_LIVE_V31_SOURCE_IDS)[number];

export interface ComboboxLiveV31SourceIdentity {
  source: ComboboxLiveV31SourceId;
...
export interface ComboboxLiveV31WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV31ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV31SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV31RawNode;
    optionScene: ComboboxLiveV31RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV31RootProof {
  source: ComboboxLiveV31SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV31FixedFixedPoint;
}

export interface ComboboxLiveV31ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV31SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV31CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV31SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV31CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV31SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV31CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v31 ${label} must be an object`);
...
export function validateComboboxLiveV31WriterPayload(
  value: unknown,
): ComboboxLiveV31WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V31_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v31 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v31 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v31 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV31SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV31ExtractProgram(
  writer: ComboboxLiveV31WriterOwnership,
  sources: readonly ComboboxLiveV31SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v31 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V31_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V31_NAMESPACE)}
${buildComboboxLiveV31RawPropertyRuntime()}
...
export function validateComboboxLiveV31ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV31WriterOwnership,
): ComboboxLiveV31ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v31 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V31_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v31 extract schema/two-root mismatch");
  return payload as ComboboxLiveV31ExtractPayload;
}

export function proveComboboxLiveV31Roots<Instance>(
  extract: ComboboxLiveV31ExtractPayload,
  sources: readonly (ComboboxLiveV31SourceIdentity & {
...
): ComboboxLiveV31RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v31 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v31 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV31Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV31Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV31FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v31 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v31 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v31 compile lost owned sets");
...
export function assertComboboxLiveV31RootProofs(
  proofs: readonly ComboboxLiveV31RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v31 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v31 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV31ProbeProgram(
  writer: ComboboxLiveV31WriterOwnership,
  sources: readonly ComboboxLiveV31SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V31_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV31ProbePayload(
  value: unknown,
  writer: ComboboxLiveV31WriterOwnership,
): ComboboxLiveV31ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV31ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V31_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V31_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v31 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV31CaptureManifest(
  cells: readonly ComboboxLiveV31CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V31_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V31_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V31_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V31_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v31 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV31CaptureCell[],
): string => {
  validateComboboxLiveV31CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV31CaptureProgram(
  writer: ComboboxLiveV31WriterOwnership,
  cell: ComboboxLiveV31CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V31_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v31 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v31 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V31_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV31CapturePayload(
  value: unknown,
  cell: ComboboxLiveV31CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV31CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV31CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V31_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V31_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v31 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV31CaptureResponses(
  manifest: readonly ComboboxLiveV31CaptureCell[],
  responses: readonly ComboboxLiveV31CapturePayload[],
): void {
  validateComboboxLiveV31CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v31 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV31ObjectiveReport {
  artifactVersion: "combobox-live-v31-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV31Objective(
  manifest: readonly ComboboxLiveV31CaptureCell[],
  responses: readonly ComboboxLiveV31CapturePayload[],
): ComboboxLiveV31ObjectiveReport {
  assertComboboxLiveV31CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v31-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV31CleanupProgram(
  writer: ComboboxLiveV31WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV31CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V31_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v31 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV31CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV31WriterOwnership,
): ComboboxLiveV31CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV31CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v31 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V31_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV31WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
