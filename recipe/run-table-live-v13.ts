...
import {
  acceptTableLiveV13Response,
  createTableLiveV13Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV13Request,
  persistTableLiveV13RawResponse,
  persistTableLiveV13TechnicalGates,
  readTableLiveV13AcceptedRawPayload,
  type TableLiveV13Request,
  type TableLiveV13TransactionAuthorization,
} from "./table-live-v13-broker.js";
import {
  TABLE_LIVE_V13_CAPTURE_COUNT,
  assertTableLiveV13CaptureResponses,
  ...
  evaluateTableLiveV13Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV13Roots,
  validateTableLiveV13CaptureManifest,
  validateTableLiveV13CapturePayload,
  validateTableLiveV13CleanupPayload,
  validateTableLiveV13ExtractPayload,
  validateTableLiveV13ProbePayload,
  validateTableLiveV13WriterPayload,
  type TableLiveV13CaptureCell,
  type TableLiveV13CapturePayload,
...
import {
  ...
  TABLE_LIVE_V13_PLAN_PATH,
  type TableLiveV13ProofPlan,
...
export const TABLE_LIVE_V13_RUNNER_VERSION = "table-live-v13-runner-v1";

interface TableLiveV13JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV13OrchestratorState {
  artifactVersion: "table-live-v13-orchestrator-state-v1";
  ...
  journal: TableLiveV13JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV13ProofPlan;
  planSha256: string;
  captures: TableLiveV13CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV13SourceIdentity & {
...
export interface TableLiveV13NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV13OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v13 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V13_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V13_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV13ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV13CaptureCell[];
  validateTableLiveV13CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v13 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v13 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV13WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV13RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV13Request =>
  readJson<TableLiveV13Request>(file);

export class TableLiveV13Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV13Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV13OrchestratorState = {
      artifactVersion: "table-live-v13-orchestrator-state-v1",
    ...
    return new TableLiveV13Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV13OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV13Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV13NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV13NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v13 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV13NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v13 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV13RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV13Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV13CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV13AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV13WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV13Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV13ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV13Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV13Response(this.#directory, "writer", {
        validate: validateTableLiveV13WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV13Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV13RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV13Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV13ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV13Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV13RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v13-host-root-proofs-v1",
      ...
      const next = issueTableLiveV13Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV13ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV13Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV13ProbePayload(payload, ownership),
      });
      persistTableLiveV13TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v13 capture cell missing");
      const accepted = acceptTableLiveV13Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV13CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V13_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV13CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV13Objective(
    ...
    throw new Error(`Table live v13 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV13Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV13CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV13Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV13RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV13Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV13CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV13CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV13CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV13Authorization = (
  privateKey: KeyObject,
): TableLiveV13TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV13SimulationKey = (): KeyObject =>
