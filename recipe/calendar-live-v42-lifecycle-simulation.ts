...
import { calendarLiveV1Sha256 } from "./calendar-live-v42-broker.js";

export interface CalendarLiveV42LifecycleSimulationReport {
  artifactVersion: "calendar-live-v42-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV42AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV42Lifecycle(): Promise<CalendarLiveV42LifecycleSimulationReport> {
  await buildCalendarLiveV42Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V42_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV42AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v42-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V42_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V42_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV42History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV42History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV42AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V42_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V42_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV42History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV42History(authorized, "pending");
    ...
    await buildCalendarLiveV42Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V42_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV42AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV42Lifecycle(), null, 2)}\n`,
  );
