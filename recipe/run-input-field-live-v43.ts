...
import {
  ...
  readInputLiveV43AcceptedRawPayload,
...
import {
  ...
  buildInputLiveV43CleanupProgram,
...
import {
  ...
  INPUT_LIVE_V43_PLAN_PATH,
  type InputLiveV43ProofPlan,
} from "./build-input-field-live-proof-v43.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-v43.js";

export const INPUT_LIVE_V43_RUNNER_VERSION = "input-live-v43-runner-v1";

interface InputLiveV43JournalEntry {
  index: number;
  event: string;
...
interface InputLiveV43OrchestratorState {
  artifactVersion: "input-live-v43-orchestrator-state-v1";
  ...
  journal: InputLiveV43JournalEntry[];
}

interface LoadedProofInputs {
  plan: InputLiveV43ProofPlan;
  planSha256: string;
  captures: InputLiveV43CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    InputLiveV43SourceIdentity & {
...
export interface InputLiveV43NextAction {
...
const writeState = (
  directory: string,
  state: InputLiveV43OrchestratorState,
): void => {
...
const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, INPUT_LIVE_V43_PLAN_PATH);
  const capturePath = path.join(root, INPUT_LIVE_V43_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as InputLiveV43ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as InputLiveV43CaptureCell[];
  validateInputLiveV43CaptureManifest(captures);
  if (
    plan.captureManifest.sha256 !== inputLiveV43Sha256(captureBytes) ||
    captureArtifact.cellsSha256 !== inputLiveV43CaptureManifestSha256(captures)
  )
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (
      inputLiveV43Sha256(expectedBytes) !== source.expectedScenePlan.sha256 ||
      inputLiveV43Sha256(uncompressed) !==
  ...
  return {
    plan,
    planSha256: inputLiveV43Sha256(planBytes),
    captures,
    captureManifestSha256: inputLiveV43Sha256(captureBytes),
    sources,
...
const appendJournal = (
  ...
  const payloadSha256 = inputLiveV43Sha256(canonicalJson(payload));
  const body = {
  ...
  const entry = {
    ...body,
    entrySha256: inputLiveV43Sha256(canonicalJson(body)),
  };
  writeNew(
    path.join(
      directory,
...
const writerOwnership = (directory: string): InputLiveV43WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): InputLiveV43RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): InputLiveV43Request =>
  readJson<InputLiveV43Request>(file);

export class InputLiveV43Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueInputLiveV43Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: InputLiveV43OrchestratorState = {
      artifactVersion: "input-live-v43-orchestrator-state-v1",
      transactionDirectory: options.transactionDirectory,
      activeRequest: inputLiveV43RequestPath(
        options.transactionDirectory,
        "writer",
      ),
      cleanupRequest: null,
    ...
    return new InputLiveV43Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<InputLiveV43OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new InputLiveV43Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): InputLiveV43NextAction {
    const cleanup =
      this.#state.cleanupRequest &&
      !this.#state.cleanupAccepted &&
      !existsSync(inputLiveV43ReceiptPath(this.#directory, "cleanup"))
        ? this.#state.cleanupRequest
        : null;
  ...
  ingestAndAdvance(rawResponse: string): InputLiveV43NextAction {
  ...
  ingestCleanup(rawResponse: string): InputLiveV43NextAction {
    ...
    persistInputLiveV43RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptInputLiveV43Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV43CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      existsSync(inputLiveV43ReceiptPath(this.#directory, "writer")) &&
      !existsSync(path.join(this.#directory, OWNERSHIP_FILE))
    ) {
      const raw = readInputLiveV43AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateInputLiveV43WriterPayload(raw),
      );
    }
    if (existsSync(inputLiveV43RequestPath(this.#directory, "cleanup")))
      this.#state.cleanupRequest = inputLiveV43RequestPath(
        this.#directory,
        "cleanup",
      );
    if (
      existsSync(path.join(this.#directory, OWNERSHIP_FILE)) &&
      !existsSync(inputLiveV43RequestPath(this.#directory, "cleanup"))
    )
      this.#issueCleanupAndRestore(false);
    else if (
      existsSync(inputLiveV43RequestPath(this.#directory, "cleanup")) &&
      !existsSync(inputLiveV43RequestPath(this.#directory, "restore"))
    )
      this.#issueRestore(false);
    if (
      existsSync(inputLiveV43RequestPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV43ReceiptPath(this.#directory, "restore"))
    )
      this.#state.activeRequest = inputLiveV43RequestPath(
        this.#directory,
        "restore",
      );
    if (
      existsSync(inputLiveV43ReceiptPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV43RequestPath(this.#directory, "extract"))
    ) {
      const ownership = writerOwnership(this.#directory);
      const extract = issueInputLiveV43Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildInputLiveV43ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV43RequestPath(
        this.#directory,
        "extract",
    ...
    if (existsSync(inputLiveV43ReceiptPath(this.#directory, "cleanup")))
      this.#state.cleanupAccepted = true;
    this.#persist();
  }

  #acceptActive(request: InputLiveV43Request): void {
    if (request.phase === "writer") {
      const accepted = acceptInputLiveV43Response(this.#directory, "writer", {
    ...
    if (request.phase === "restore") {
      const accepted = acceptInputLiveV43Response(this.#directory, "restore", {
        validate: (payload) =>
          validateInputLiveV43RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptInputLiveV43Response(this.#directory, "extract", {
        validate: (payload) =>
          validateInputLiveV43ExtractPayload(payload, ownership),
      });
      const proofs = proveInputLiveV43Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseInputFieldRecipe,
        compileInputFieldRecipe,
      ...
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "input-live-v43-host-root-proofs-v1",
      ...
      appendJournal(
        ...
        request.requestId,
        { proofsSha256: inputLiveV43Sha256(canonicalJson(proofs)) },
      );
      const next = issueInputLiveV43Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildInputLiveV43ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV43RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptInputLiveV43Response(this.#directory, "probe", {
        validate: (payload) =>
          validateInputLiveV43ProbePayload(payload, ownership),
      });
      persistInputLiveV43TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      const accepted = acceptInputLiveV43Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateInputLiveV43CapturePayload(payload, cell, rawBytes),
      });
      const png = Buffer.from(accepted.payload.pngBase64, "base64");
      const capturePath = path.join(
        this.#directory,
        "captures",
        `${String(cell.index).padStart(3, "0")}-${inputLiveV43Sha256(cell.cellKey).slice(0, 20)}.png`,
      );
      ...
      if (next < INPUT_LIVE_V43_CAPTURE_COUNT) this.#issueCapture(next);
      else {
          ...
          pngBase64: Buffer.from(
            readFileSync(
              path.join(
                this.#directory,
                "captures",
                `${String(response.index).padStart(3, "0")}-${inputLiveV43Sha256(response.cellKey).slice(0, 20)}.png`,
              ),
            ),
          ).toString("base64"),
        }));
        assertInputLiveV43CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateInputLiveV43Objective(
        ...
        appendJournal(
          ...
          request.requestId,
          {
            reportSha256: inputLiveV43Sha256(canonicalJson(objective)),
  ...
  #issueCleanupAndRestore(journal: boolean): void {
    ...
    const cleanup = issueInputLiveV43Request(
      ...
      this.#privateKey,
      "cleanup",
      buildInputLiveV43CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = inputLiveV43RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueInputLiveV43Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildInputLiveV43RestoreProgram(ownership),
    );
    this.#state.activeRequest = inputLiveV43RequestPath(
      this.#directory,
      "restore",
  ...
  #readCaptureResponseIndex(): InputLiveV43CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: InputLiveV43CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedInputLiveV43Authorization = (
  privateKey: KeyObject,
...
export const generateInputLiveV43SimulationKey = (): KeyObject =>
