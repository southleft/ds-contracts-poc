...
export type ComboboxLiveV4HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV4AuthorizationArtifact {
  artifactVersion: "combobox-live-v4-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V4_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV4AntecedentIndex {
...
export interface ComboboxLiveV4HistoryState {
  index?: ComboboxLiveV4AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV4AuthorizationArtifact;
...
export interface ComboboxLiveV4AuthorizationProof extends ComboboxLiveV4TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V4_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V4_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V4_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V4_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v4-authorization") ||
  artifactPath.includes("combobox-live-v4-preflight") ||
  artifactPath.includes("combobox-live-v4-authorized") ||
  artifactPath.includes("create-combobox-live-v4-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v4-status") ||
...
export function validateComboboxLiveV4AntecedentIndex(
  index: ComboboxLiveV4AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v4-antecedent-index-v1" ||
...
export function validateComboboxLiveV4UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV4History\(\s*["']/.test(source) ||
    /readComboboxLiveV4HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV4AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV4AuthorizationArtifact {
  ...
  ) as ComboboxLiveV4AntecedentIndex;
  const indexFailures = validateComboboxLiveV4AntecedentIndex(index);
...
export function validateComboboxLiveV4AuthorizationArtifact(
  artifact: ComboboxLiveV4AuthorizationArtifact | undefined,
  index: ComboboxLiveV4AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v4-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v4" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V4_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V4_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V4_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V4_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V4_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V4_SOURCE_ROOTS ||
...
export function validateComboboxLiveV4History(
  state: ComboboxLiveV4HistoryState,
  expected: ComboboxLiveV4HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV4AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV4AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV4HistoryState(
  root = process.cwd(),
): ComboboxLiveV4HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V4_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV4AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V4_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V4_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V4_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV4History(
  expected: ComboboxLiveV4HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV4HistoryState {
  const state = readComboboxLiveV4HistoryState(root);
  const failures = validateComboboxLiveV4History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v4 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV4Authorization(
  root = process.cwd(),
): ComboboxLiveV4AuthorizationProof {
  const state = verifyComboboxLiveV4History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V4_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v4/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v4.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V4_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V4_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V4_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV4HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV4History(expected);
  process.stdout.write(
    `Combobox live v4 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
