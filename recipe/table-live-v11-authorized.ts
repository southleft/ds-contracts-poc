...
import { TABLE_LIVE_V11_AUTHORIZATION_PATH } from "./build-table-live-proof-v11.js";
import { verifyTableLiveV11Authorization } from "./table-live-v11-authorization.js";
import { runTableLiveV11Preflight } from "./table-live-v11-preflight.js";
import { TableLiveV11Orchestrator } from "./run-table-live-v11.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v11 requires exact authorization artifact ${TABLE_LIVE_V11_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v11-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV11Authorization(root);
    runTableLiveV11Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV11Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV11Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
