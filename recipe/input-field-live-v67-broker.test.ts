...
import {
  ...
  createInputLiveV67FakeDynamicResponse,
  ...
  listInputLiveV67PersistedRequests,
  persistInputLiveV67RawResponse,
  persistInputLiveV67TechnicalGates,
  validateInputLiveV67Receipt,
  validateInputLiveV67Request,
  type InputLiveV67RemotePhase,
...
import {
  ...
  type InputLiveV67ExtractPayload,
  type InputLiveV67ProbePayload,
  type InputLiveV67RestorePayload,
  type InputLiveV67RootProof,
  type InputLiveV67WriterOwnership,
} from "./input-field-live-v67-contract.js";
import {
  InputLiveV67Orchestrator,
  simulatedInputLiveV67Authorization,
} from "./run-input-field-live-v67.js";
...
const captures: InputLiveV67CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV67Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v67-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV67Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV67CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV67FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV67FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V67_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V67_TARGET);
  persistInputLiveV67RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV67Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV67Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV67WriterOwnership,
): InputLiveV67RestorePayload => ({
...
const rootProofs = (): InputLiveV67RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV67CaptureCell,
): InputLiveV67CapturePayload => {
  const bytes = Buffer.from(`input-live-v67-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV67Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV67Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV67RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV67RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV67ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV67ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV67CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV67CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV67CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV67Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV67CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV67PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV67ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV67Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV67Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV67Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV67Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V67_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV67RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V67_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV67Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV67Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV67ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV67Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV67WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV67RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV67Request;
    validateInputLiveV67Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV67Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV67CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v67-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV67Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV67Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV67Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V67_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV67RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V67_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV67Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV67Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV67RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V67_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV67Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV67Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV67Request;
