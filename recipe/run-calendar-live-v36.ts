...
import {
  acceptCalendarLiveV36Response,
  createCalendarLiveV36Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV36Request,
  persistCalendarLiveV36RawResponse,
  persistCalendarLiveV36TechnicalGates,
  readCalendarLiveV36AcceptedRawPayload,
  type CalendarLiveV36Request,
  type CalendarLiveV36TransactionAuthorization,
} from "./calendar-live-v36-broker.js";
import {
  CALENDAR_LIVE_V36_CAPTURE_COUNT,
  assertCalendarLiveV36CaptureResponses,
  ...
  evaluateCalendarLiveV36Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV36Roots,
  validateCalendarLiveV36CaptureManifest,
  validateCalendarLiveV36CapturePayload,
  validateCalendarLiveV36CleanupPayload,
  validateCalendarLiveV36ExtractPayload,
  validateCalendarLiveV36ProbePayload,
  validateCalendarLiveV36WriterPayload,
  type CalendarLiveV36CaptureCell,
  type CalendarLiveV36CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V36_PLAN_PATH,
  type CalendarLiveV36ProofPlan,
} from "./build-calendar-live-proof-v36.js";
import {
...
export const CALENDAR_LIVE_V36_RUNNER_VERSION = "calendar-live-v36-runner-v1";

interface CalendarLiveV36JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV36OrchestratorState {
  artifactVersion: "calendar-live-v36-orchestrator-state-v1";
  ...
  journal: CalendarLiveV36JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV36ProofPlan;
  planSha256: string;
  captures: CalendarLiveV36CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV36SourceIdentity & {
...
export interface CalendarLiveV36NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV36OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v36 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V36_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V36_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV36ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV36CaptureCell[];
  validateCalendarLiveV36CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v36 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v36 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV36WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV36RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV36Request =>
  readJson<CalendarLiveV36Request>(file);

export class CalendarLiveV36Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV36Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV36OrchestratorState = {
      artifactVersion: "calendar-live-v36-orchestrator-state-v1",
    ...
    return new CalendarLiveV36Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV36OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV36Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV36NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV36NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v36 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV36NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v36 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV36RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV36Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV36CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV36AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV36WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV36Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV36ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV36Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV36Response(this.#directory, "writer", {
        validate: validateCalendarLiveV36WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV36Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV36RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV36Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV36ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV36Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV36RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v36-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV36Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV36ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV36Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV36ProbePayload(payload, ownership),
      });
      persistCalendarLiveV36TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v36 capture cell missing");
      const accepted = acceptCalendarLiveV36Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV36CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V36_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV36CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV36Objective(
    ...
    throw new Error(`Calendar live v36 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV36Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV36CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV36Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV36RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV36Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV36CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV36CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV36CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV36Authorization = (
  privateKey: KeyObject,
): CalendarLiveV36TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV36SimulationKey = (): KeyObject =>
