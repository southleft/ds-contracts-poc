...
import { INPUT_LIVE_V42_INDEX_PATH } from "./build-input-field-live-proof-v42.js";
import {
...
import {
  ...
  validateInputLiveV42AntecedentIndex,
  validateInputLiveV42AuthorizationArtifact,
  validateInputLiveV42History,
  validateInputLiveV42UnitTestSource,
  type InputLiveV42AntecedentIndex,
  type InputLiveV42AuthorizationArtifact,
  type InputLiveV42AuthorizationProof,
  type InputLiveV42HistoryState,
} from "./input-field-live-v42-authorization.js";
import {
  inputLiveV42AttestationSha256,
  validateInputLiveV42ControlFlowSource,
  validateInputLiveV42SecurityAttestation,
  type InputLiveV42SecurityAttestation,
} from "./input-field-live-v42-preflight.js";
import { inputLiveV42Sha256 } from "./input-field-live-v42-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V42_INDEX_PATH);
const index = (): InputLiveV42AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV42AntecedentIndex;

const artifact = (): InputLiveV42AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV42AuthorizationArtifact({
...
const pendingState = (): InputLiveV42HistoryState => ({
  index: index(),
  indexSha256: inputLiveV42Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV42HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV42AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV42Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V42_TARGET,
  expectedDynamicTool: INPUT_LIVE_V42_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v42/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV42AuthorizationArtifact,
): InputLiveV42SecurityAttestation => {
  const body: Omit<InputLiveV42SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v42-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V42_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV42AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV42History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV42History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV42History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV42History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV42History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV42UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV42UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV42AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV42Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV42AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV42AuthorizationArtifact(
        planted as InputLiveV42AuthorizationArtifact,
        index(),
        inputLiveV42Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV42SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV42AttestationSha256(
      body as Omit<InputLiveV42SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV42SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV42ControlFlowSource(source), []);
});
