...
export type TableLiveV37HistoryExpectation = "pending" | "authorized";

export interface TableLiveV37AuthorizationArtifact {
  artifactVersion: "table-live-v37-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V37_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV37AntecedentIndex {
...
export interface TableLiveV37HistoryState {
  index?: TableLiveV37AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV37AuthorizationArtifact;
...
export interface TableLiveV37AuthorizationProof
  extends TableLiveV37TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V37_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V37_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V37_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V37_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v37-authorization") ||
  artifactPath.includes("table-live-v37-preflight") ||
  artifactPath.includes("table-live-v37-authorized") ||
  artifactPath.includes("create-table-live-v37-security-attestation") ||
  artifactPath.includes("table-live-pivot-v37-status") ||
...
export function validateTableLiveV37AntecedentIndex(
  index: TableLiveV37AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v37-antecedent-index-v1" ||
...
export function validateTableLiveV37UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV37History\(\s*["']/.test(source) ||
    /readTableLiveV37HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV37AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV37AuthorizationArtifact {
  ...
  ) as TableLiveV37AntecedentIndex;
  const indexFailures = validateTableLiveV37AntecedentIndex(index);
...
export function validateTableLiveV37AuthorizationArtifact(
  artifact: TableLiveV37AuthorizationArtifact | undefined,
  index: TableLiveV37AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v37-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v37" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V37_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V37_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V37_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V37_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V37_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V37_SOURCE_ROOTS ||
...
export function validateTableLiveV37History(
  state: TableLiveV37HistoryState,
  expected: TableLiveV37HistoryExpectation,
): string[] {
  const failures = validateTableLiveV37AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV37AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV37HistoryState(
  root = process.cwd(),
): TableLiveV37HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V37_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV37AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V37_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V37_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V37_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV37History(
  expected: TableLiveV37HistoryExpectation,
  root = process.cwd(),
): TableLiveV37HistoryState {
  const state = readTableLiveV37HistoryState(root);
  const failures = validateTableLiveV37History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v37 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV37Authorization(
  root = process.cwd(),
): TableLiveV37AuthorizationProof {
  const state = verifyTableLiveV37History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V37_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v37/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v37.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V37_TARGET,
    expectedDynamicTool: TABLE_LIVE_V37_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V37_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV37HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV37History(expected);
  process.stdout.write(
    `Table live v37 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
