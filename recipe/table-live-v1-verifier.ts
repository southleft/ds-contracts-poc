...
import {
  verifyTableLiveV1FixedSceneFixedPoint,
  type TableLiveV1FixedFixedPoint,
} from "./table-live-v1-fixed-point.js";

export const TABLE_LIVE_V1_VERIFIER_VERSION = "table-live-v1-verifier-v1";

export function buildTableLiveV1RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV1RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV1RawNode[];
}

export interface TableLiveV1NormalizedScene {
...
export interface TableLiveV1SceneProof {
...
export function normalizeTableLiveV1Scene(
  root: TableLiveV1RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV1NormalizedScene {
  const canonicalBindings: TableLiveV1NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV1RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV1Scene<Instance>(
  normalized: TableLiveV1NormalizedScene,
...
): TableLiveV1SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV1FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV1ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV1PreCaptureGates(payload: unknown): void {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("table live v1 pre-capture gate payload absent");
