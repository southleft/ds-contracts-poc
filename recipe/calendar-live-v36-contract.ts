...
import { buildCalendarLiveV36CleanupRuntime } from "./calendar-live-v36-cleanup.js";
import { type CalendarLiveV36FixedFixedPoint } from "./calendar-live-v36-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV36RestoreProgram,
  validateCalendarLiveV36RestorePayload,
  type CalendarLiveV36RestorePayload,
} from "./calendar-live-v36-restore.js";
import {
  buildCalendarLiveV36RawPropertyRuntime,
  normalizeCalendarLiveV36Scene,
  type CalendarLiveV36RawNode,
} from "./calendar-live-v36-verifier.js";
...
export const CALENDAR_LIVE_V36_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V36_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V36_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V36_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V36_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V36_SET_COUNT = 3;
export const CALENDAR_LIVE_V36_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V36_HOST_PHASES = 3;
export const CALENDAR_LIVE_V36_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V36_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V36_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V36_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV36SourceId = (typeof CALENDAR_LIVE_V36_SOURCE_IDS)[number];
export type CalendarLiveV36Kind = "calendar" | "week" | "day";

export interface CalendarLiveV36SourceIdentity {
  source: CalendarLiveV36SourceId;
...
export interface CalendarLiveV36WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV36ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV36SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV36RawNode;
    weekScene: CalendarLiveV36RawNode;
    dayScene: CalendarLiveV36RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV36RootProof {
  source: CalendarLiveV36SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV36FixedFixedPoint;
}

export interface CalendarLiveV36ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV36SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV36CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV36SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV36CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV36SourceId;
  frameWidth: number;
...
export interface CalendarLiveV36CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v36 ${label} must be an object`);
...
export function validateCalendarLiveV36WriterPayload(
  value: unknown,
): CalendarLiveV36WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v36") ||
    ...
    result.namespace !== CALENDAR_LIVE_V36_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v36 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v36 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v36 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV36SourceIdentity,
  kind: CalendarLiveV36Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV36ExtractProgram(
  writer: CalendarLiveV36WriterOwnership,
  sources: readonly CalendarLiveV36SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v36 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V36_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V36_NAMESPACE)}
${buildCalendarLiveV36RawPropertyRuntime()}
...
export function validateCalendarLiveV36ExtractPayload(
  value: unknown,
  writer: CalendarLiveV36WriterOwnership,
): CalendarLiveV36ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v36 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V36_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v36 extract schema/one-root mismatch");
  return payload as CalendarLiveV36ExtractPayload;
}

export function proveCalendarLiveV36Roots<Instance>(
  extract: CalendarLiveV36ExtractPayload,
  sources: readonly (CalendarLiveV36SourceIdentity & {
...
): CalendarLiveV36RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v36 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v36 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV36Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV36Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV36Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV36FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v36 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v36 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v36 compile lost owned sets");
...
export function assertCalendarLiveV36RootProofs(
  proofs: readonly CalendarLiveV36RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v36 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v36 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV36ProbeProgram(
  writer: CalendarLiveV36WriterOwnership,
  sources: readonly CalendarLiveV36SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V36_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV36ProbePayload(
  value: unknown,
  writer: CalendarLiveV36WriterOwnership,
): CalendarLiveV36ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV36ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V36_VARIANT_COUNT ||
    ...
    throw new TypeError("Calendar live v36 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV36CaptureManifest(
  cells: readonly CalendarLiveV36CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V36_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V36_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V36_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V36_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v36 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV36CaptureCell[],
): string => {
  validateCalendarLiveV36CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV36CaptureProgram(
  writer: CalendarLiveV36WriterOwnership,
  cell: CalendarLiveV36CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V36_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v36 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v36 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V36_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV36CapturePayload(
  value: unknown,
  cell: CalendarLiveV36CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV36CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV36CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V36_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V36_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v36 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV36CaptureResponses(
  manifest: readonly CalendarLiveV36CaptureCell[],
  responses: readonly CalendarLiveV36CapturePayload[],
): void {
  validateCalendarLiveV36CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v36 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV36ObjectiveReport {
  artifactVersion: "calendar-live-v36-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV36Objective(
  manifest: readonly CalendarLiveV36CaptureCell[],
  responses: readonly CalendarLiveV36CapturePayload[],
): CalendarLiveV36ObjectiveReport {
  assertCalendarLiveV36CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v36-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV36CleanupProgram(
  writer: CalendarLiveV36WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV36CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V36_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v36 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV36CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV36WriterOwnership,
): CalendarLiveV36CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV36CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v36 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V36_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV36WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
