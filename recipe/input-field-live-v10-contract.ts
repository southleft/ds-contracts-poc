...
} from "./input-field-live-v3-verifier-v10.js";
import {
  buildInputLiveV10RawPropertyRuntime,
  normalizeInputLiveV10Scene,
  type InputLiveV10RawNode,
} from "./input-field-live-v10-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v10.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v10.js";

export const INPUT_LIVE_V10_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V10_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V10_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V10_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V10_VARIANT_COUNT = 256;
export const INPUT_LIVE_V10_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V10_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV10SourceId = (typeof INPUT_LIVE_V10_SOURCE_IDS)[number];

export interface InputLiveV10SourceIdentity {
  source: InputLiveV10SourceId;
...
export interface InputLiveV10WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV10ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV10SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV10RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV10RootProof {
  source: InputLiveV10SourceId;
  adapterIdentity: string;
...
export interface InputLiveV10ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV10SourceId;
    adapterIdentity: string;
...
export interface InputLiveV10CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV10SourceId;
  adapterIdentity: string;
...
export interface InputLiveV10CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV10SourceId;
  strata: InputLiveV10CaptureCell["strata"];
...
export interface InputLiveV10CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Input live v10 ${label} must be an object`);
...
const exactUniqueStrings = (
  ...
  if (
    ...
    throw new TypeError(`Input live v10 ${label} cardinality/identity mismatch`);
  return value;
};
const finiteNonnegative = (value: unknown): value is number =>
  typeof value === "number" && Number.isFinite(value) && value >= 0;

export function validateInputLiveV10WriterPayload(
  value: unknown,
): InputLiveV10WriterOwnership {
  ...
  if (
    ...
    throw new TypeError("Input live v10 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Input live v10 writer created-node denominator invalid",
    );
  return {
...
const identityPlan = (source: InputLiveV10SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV10ExtractProgram(
  writer: InputLiveV10WriterOwnership,
  sources: readonly InputLiveV10SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Input live v10 extract requires two source identities");
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V10_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V10_NAMESPACE)}
${buildInputLiveV10RawPropertyRuntime()}
...
export function validateInputLiveV10ExtractPayload(
  value: unknown,
  writer: InputLiveV10WriterOwnership,
): InputLiveV10ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Input live v10 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V10_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Input live v10 extract schema/two-root mismatch");
  return payload as InputLiveV10ExtractPayload;
}

export function proveInputLiveV10Roots<Instance>(
  extract: InputLiveV10ExtractPayload,
  sources: readonly (InputLiveV10SourceIdentity & {
...
): InputLiveV10RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Input live v10 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Input live v10 omitted ${source.source} root`);
    const normalized = normalizeInputLiveV10Scene(
      root.scene,
...
export function assertInputLiveV10RootProofs(
  proofs: readonly InputLiveV10RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Input live v10 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Input live v10 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildInputLiveV10ProbeProgram(
  writer: InputLiveV10WriterOwnership,
  sources: readonly InputLiveV10SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V10_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV10ProbePayload(
  value: unknown,
  writer: InputLiveV10WriterOwnership,
): InputLiveV10ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV10ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V10_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V10_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Input live v10 probe/usability/restoration failed");
  return payload;
}

export function validateInputLiveV10CaptureManifest(
  cells: readonly InputLiveV10CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V10_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V10_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V10_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V10_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V10_SOURCE_IDS.some(
      (source) => cells.filter((cell) => cell.source === source).length !== 64,
    )
  )
    throw new TypeError(
      "Input live v10 capture manifest must be exact 128 cells",
    );
}

export const inputLiveV10CaptureManifestSha256 = (
  cells: readonly InputLiveV10CaptureCell[],
): string => {
  validateInputLiveV10CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV10CaptureProgram(
  writer: InputLiveV10WriterOwnership,
  cell: InputLiveV10CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V10_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V10_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
    ...
    throw new TypeError("Input live v10 capture cell is malformed");
  const expectedSet = writer.sources.find(
    (source) => source.adapterIdentity === cell.adapterIdentity,
  )?.setId;
  if (!expectedSet)
    throw new TypeError(
      `Input live v10 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V10_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV10CapturePayload(
  value: unknown,
  cell: InputLiveV10CaptureCell,
  rawResponseBytes: number,
): InputLiveV10CapturePayload {
  const payload = record(value, "capture payload") as InputLiveV10CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V10_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V10_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Input live v10 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertInputLiveV10CaptureResponses(
  manifest: readonly InputLiveV10CaptureCell[],
  responses: readonly InputLiveV10CapturePayload[],
): void {
  validateInputLiveV10CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Input live v10 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface InputLiveV10ObjectiveReport {
  artifactVersion: "input-live-v10-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV10Objective(
  manifest: readonly InputLiveV10CaptureCell[],
  responses: readonly InputLiveV10CapturePayload[],
  readReference: (cell: InputLiveV10CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV10CaptureCell) => Buffer,
): InputLiveV10ObjectiveReport {
  assertInputLiveV10CaptureResponses(manifest, responses);
  const rows = manifest.map((cell, index) => {
    ...
    if (
      ...
      throw new TypeError(
        `Input live v10 objective byte drift: ${cell.cellKey}`,
      );
    ...
    if (
      ...
      throw new TypeError(
        `Input live v10 objective measurement refused: ${cell.cellKey}`,
      );
    return {
      index: cell.index,
...
export function buildInputLiveV10CleanupProgram(
  writer: InputLiveV10WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V10_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Input live v10 cleanup runtime result hook absent");
  return String.raw`
...
export function validateInputLiveV10CleanupPayload(
  value: unknown,
  writer: InputLiveV10WriterOwnership,
): InputLiveV10CleanupPayload {
  const payload = record(value, "cleanup payload") as InputLiveV10CleanupPayload;
  ...
  if (
    ...
    throw new TypeError("Input live v10 cleanup is incomplete or overbroad");
  return payload;
}

export const INPUT_LIVE_V10_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV10WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
