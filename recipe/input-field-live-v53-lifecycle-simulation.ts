...
export interface InputLiveV53LifecycleSimulationReport {
  artifactVersion: "input-live-v53-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV53AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV53Lifecycle(): Promise<InputLiveV53LifecycleSimulationReport> {
  await buildInputLiveV53Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V53_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV53AntecedentIndex;
  const indexSha256 = inputLiveV53Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v53-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V53_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V53_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV53History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV53History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV53AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V53_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V53_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV53History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV53History(authorized, "pending");
    ...
    await buildInputLiveV53Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V53_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV53AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV53Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV53Lifecycle(), null, 2)}\n`,
  );
