...
import { buildCalendarLiveV28CleanupRuntime } from "./calendar-live-v28-cleanup.js";
import { type CalendarLiveV28FixedFixedPoint } from "./calendar-live-v28-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV28RestoreProgram,
  validateCalendarLiveV28RestorePayload,
  type CalendarLiveV28RestorePayload,
} from "./calendar-live-v28-restore.js";
import {
  buildCalendarLiveV28RawPropertyRuntime,
  normalizeCalendarLiveV28Scene,
  type CalendarLiveV28RawNode,
} from "./calendar-live-v28-verifier.js";
...
export const CALENDAR_LIVE_V28_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V28_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V28_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V28_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V28_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V28_SET_COUNT = 3;
export const CALENDAR_LIVE_V28_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V28_HOST_PHASES = 3;
export const CALENDAR_LIVE_V28_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V28_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V28_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V28_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV28SourceId = (typeof CALENDAR_LIVE_V28_SOURCE_IDS)[number];
export type CalendarLiveV28Kind = "calendar" | "week" | "day";

export interface CalendarLiveV28SourceIdentity {
  source: CalendarLiveV28SourceId;
...
export interface CalendarLiveV28WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV28ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV28SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV28RawNode;
    weekScene: CalendarLiveV28RawNode;
    dayScene: CalendarLiveV28RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV28RootProof {
  source: CalendarLiveV28SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV28FixedFixedPoint;
}

export interface CalendarLiveV28ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV28SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV28CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV28SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV28CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV28SourceId;
  frameWidth: number;
...
export interface CalendarLiveV28CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v28 ${label} must be an object`);
...
export function validateCalendarLiveV28WriterPayload(
  value: unknown,
): CalendarLiveV28WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v28") ||
    ...
    result.namespace !== CALENDAR_LIVE_V28_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v28 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v28 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v28 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV28SourceIdentity,
  kind: CalendarLiveV28Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV28ExtractProgram(
  writer: CalendarLiveV28WriterOwnership,
  sources: readonly CalendarLiveV28SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v28 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V28_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V28_NAMESPACE)}
${buildCalendarLiveV28RawPropertyRuntime()}
...
export function validateCalendarLiveV28ExtractPayload(
  value: unknown,
  writer: CalendarLiveV28WriterOwnership,
): CalendarLiveV28ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v28 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V28_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v28 extract schema/one-root mismatch");
  return payload as CalendarLiveV28ExtractPayload;
}

export function proveCalendarLiveV28Roots<Instance>(
  extract: CalendarLiveV28ExtractPayload,
  sources: readonly (CalendarLiveV28SourceIdentity & {
...
): CalendarLiveV28RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v28 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v28 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV28Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV28Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV28Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV28FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v28 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v28 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v28 compile lost owned sets");
...
export function assertCalendarLiveV28RootProofs(
  proofs: readonly CalendarLiveV28RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v28 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v28 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV28ProbeProgram(
  writer: CalendarLiveV28WriterOwnership,
  sources: readonly CalendarLiveV28SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V28_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV28ProbePayload(
  value: unknown,
  writer: CalendarLiveV28WriterOwnership,
): CalendarLiveV28ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV28ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V28_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V28_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v28 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV28CaptureManifest(
  cells: readonly CalendarLiveV28CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V28_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V28_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V28_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V28_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v28 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV28CaptureCell[],
): string => {
  validateCalendarLiveV28CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV28CaptureProgram(
  writer: CalendarLiveV28WriterOwnership,
  cell: CalendarLiveV28CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V28_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v28 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v28 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V28_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV28CapturePayload(
  value: unknown,
  cell: CalendarLiveV28CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV28CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV28CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V28_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V28_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v28 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV28CaptureResponses(
  manifest: readonly CalendarLiveV28CaptureCell[],
  responses: readonly CalendarLiveV28CapturePayload[],
): void {
  validateCalendarLiveV28CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v28 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV28ObjectiveReport {
  artifactVersion: "calendar-live-v28-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV28Objective(
  manifest: readonly CalendarLiveV28CaptureCell[],
  responses: readonly CalendarLiveV28CapturePayload[],
): CalendarLiveV28ObjectiveReport {
  assertCalendarLiveV28CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v28-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV28CleanupProgram(
  writer: CalendarLiveV28WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV28CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V28_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v28 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV28CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV28WriterOwnership,
): CalendarLiveV28CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV28CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v28 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V28_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV28WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
