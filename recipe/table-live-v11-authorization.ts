...
import {
  TABLE_LIVE_V11_DYNAMIC_TOOL,
  TABLE_LIVE_V11_TARGET,
  type TableLiveV11TransactionAuthorization,
} from "./table-live-v11-broker.js";
import {
  TABLE_LIVE_V11_AUTHORIZATION_PATH,
  TABLE_LIVE_V11_INDEX_PATH,
} from "./build-table-live-proof-v11.js";
import {
  TABLE_LIVE_V11_CAPTURE_COUNT,
  TABLE_LIVE_V11_HOST_PHASES,
  TABLE_LIVE_V11_REMOTE_REQUESTS,
  TABLE_LIVE_V11_SOURCE_ROOTS,
} from "./table-live-v11-contract.js";
import { canonicalJson } from "./normalize.js";

export type TableLiveV11HistoryExpectation = "pending" | "authorized";

export interface TableLiveV11AuthorizationArtifact {
  artifactVersion: "table-live-v11-capture-authorization-v1";
  authorizationId: "table-live-v11";
  status: "authorization declared; runtime security prerequisites still mandatory";
  authorizationIntent: true;
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V11_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV11AntecedentIndex {
  artifactVersion: "table-live-v11-antecedent-index-v1";
  status: string;
...
export interface TableLiveV11HistoryState {
  index?: TableLiveV11AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV11AuthorizationArtifact;
...
export interface TableLiveV11AuthorizationProof
  extends TableLiveV11TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V11_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V11_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V11_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V11_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v11-authorization") ||
  artifactPath.includes("table-live-v11-preflight") ||
  artifactPath.includes("table-live-v11-authorized") ||
  artifactPath.includes("create-table-live-v11-security-attestation") ||
  artifactPath.includes("table-live-pivot-v11-status") ||
...
export function validateTableLiveV11AntecedentIndex(
  index: TableLiveV11AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v11-antecedent-index-v1" ||
    index.authorizationCanBeAddedWithoutAntecedentRebuild !== true
  )
    failures.push("table v11 antecedent index missing or malformed");
  const artifactEntries = Object.entries(index?.artifacts ?? {});
  if (artifactEntries.length === 0)
    failures.push("table v11 antecedent hash denominator is empty");
  for (const [artifactPath, metadata] of artifactEntries) {
    ...
    if (
      ...
      failures.push(`malformed table v11 antecedent hash: ${artifactPath}`);
  }
  ...
  if (index?.hashSetSha256 !== expectedHashSet)
    failures.push("table v11 antecedent hash-set digest mismatch");
  if (
    ...
    failures.push("table v11 antecedent counts missing or drifted");
  for (const required of [
    TABLE_LIVE_V11_AUTHORIZATION_PATH,
    "recipe/table-live-v11-authorization.ts",
    "recipe/table-live-v11-authorization.test.ts",
    "recipe/table-live-v11-preflight.ts",
    "recipe/evidence/table-live-pivot-v11/operator-security-attestation-template.json",
    "recipe/evidence/status-index.json",
  ])
    if (!index?.authorizationLifecycleExcluded?.includes(required))
      failures.push(`table v11 lifecycle exclusion missing: ${required}`);
  const leaks = forbiddenAntecedentFieldPaths(index);
  if (leaks.length)
    failures.push(`table v11 antecedent result leakage: ${leaks.join(",")}`);
  return failures;
}

export function validateTableLiveV11UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV11History\(\s*["']/.test(source) ||
    /readTableLiveV11HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV11AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV11AuthorizationArtifact {
  if (!SHA40.test(options.antecedentCommit))
    throw new TypeError("table v11 antecedent commit must be a full SHA");
  const index = JSON.parse(
    Buffer.from(options.antecedentIndexBytes).toString("utf8"),
  ) as TableLiveV11AntecedentIndex;
  const indexFailures = validateTableLiveV11AntecedentIndex(index);
  if (indexFailures.length)
    throw new TypeError(
      `table v11 antecedent index refused:\n${indexFailures.join("\n")}`,
    );
  const publicKey =
  ...
  return {
    ...
    antecedent: {
      ...
      indexPath: TABLE_LIVE_V11_INDEX_PATH,
    ...
    denominator: {
      remoteRequests: TABLE_LIVE_V11_REMOTE_REQUESTS,
      hostPhases: TABLE_LIVE_V11_HOST_PHASES,
      captures: TABLE_LIVE_V11_CAPTURE_COUNT,
      sourceRoots: TABLE_LIVE_V11_SOURCE_ROOTS,
...
export function validateTableLiveV11AuthorizationArtifact(
  artifact: TableLiveV11AuthorizationArtifact | undefined,
  index: TableLiveV11AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v11-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v11" ||
    artifact.authorizationIntent !== true
  )
    failures.push("table v11 authorization declaration missing or malformed");
  const resultLeaks = forbiddenAntecedentFieldPaths(artifact);
  if (resultLeaks.length)
    failures.push(`table v11 authorization result leakage: ${resultLeaks.join(",")}`);
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V11_INDEX_PATH ||
    artifact?.antecedent?.indexSha256 !== indexSha256 ||
    artifact?.antecedent?.hashSetSha256 !== index?.hashSetSha256
  )
    failures.push("table v11 authorization pins wrong antecedent index");
  if (privateMaterialPaths(artifact).length)
    failures.push("table v11 authorization contains private key or token material");
  try {
    ...
    if (
      ...
      failures.push("table v11 Ed25519 public-key identity mismatch");
  } catch {
    failures.push("table v11 Ed25519 public key is invalid");
  }
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V11_TARGET) ||
    !equalJson(
    ...
    failures.push("wrong table v11 target or dynamic tool");
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V11_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V11_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V11_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V11_SOURCE_ROOTS ||
    artifact?.denominator?.expectedFacts !== index?.counts.expectedSceneFacts
  )
    failures.push("table v11 two-root or capture denominator changed");
  if (
    ...
    failures.push(
      "table v11 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
    ...
    failures.push("table v11 security prerequisites weakened");
  if (
    artifact?.humanSignoff?.mandatory !== true ||
    artifact?.humanSignoff?.status !== "pending"
  )
    failures.push("table v11 human signoff overclaim");
  return failures;
}

export function validateTableLiveV11History(
  state: TableLiveV11HistoryState,
  expected: TableLiveV11HistoryExpectation,
): string[] {
  const failures = validateTableLiveV11AntecedentIndex(state.index);
  if (!SHA40.test(state.codeCommit))
    failures.push("table v11 code commit is not a full SHA");
  ...
  if (
    ...
    failures.push("table v11 antecedent first-add commit is not unique");
  if (!state.antecedentIsAncestorOfCode)
    failures.push("code commit does not descend from table v11 antecedent");
  if (!state.antecedentIndexBytesMatchFirstAddition)
    failures.push("table v11 antecedent index changed after first addition");
  if (!state.antecedentArtifactsMatchIndexAtCommit)
    failures.push("table v11 antecedent commit artifacts do not match index");
  if (!state.workingAntecedentArtifactsMatchIndex)
    failures.push("table v11 antecedent working bytes drifted");
  ...
  failures.push(
    ...validateTableLiveV11AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
  ...
  if (
    ...
    failures.push("table v11 authorization first-add lineage mismatch");
  if (!state.authorizationPresentAtCodeCommit)
    failures.push("table v11 authorization missing from code commit");
  if (!state.authorizationBytesMatchFirstAddition)
    failures.push("table v11 authorization bytes changed after first addition");
  if (!state.authorizationStrictlyDescendsFromAntecedent)
    failures.push("table v11 authorization does not strictly descend from antecedent");
  if (!state.authorizationIsAncestorOfCode)
    failures.push("code commit predates table v11 authorization");
  return failures;
}
...
export function readTableLiveV11HistoryState(
  root = process.cwd(),
): TableLiveV11HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V11_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV11AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V11_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V11_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V11_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV11History(
  expected: TableLiveV11HistoryExpectation,
  root = process.cwd(),
): TableLiveV11HistoryState {
  const state = readTableLiveV11HistoryState(root);
  const failures = validateTableLiveV11History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v11 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV11Authorization(
  root = process.cwd(),
): TableLiveV11AuthorizationProof {
  const state = verifyTableLiveV11History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V11_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v11/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v11.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V11_TARGET,
    expectedDynamicTool: TABLE_LIVE_V11_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V11_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV11HistoryExpectation => {
  ...
  if (pending === authorized)
    throw new Error(
      "Choose exactly one table v11 history mode: --expect-pending or --expect-authorized",
    );
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV11History(expected);
  process.stdout.write(
    `Table live v11 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
