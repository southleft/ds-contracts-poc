...
test("host omits empty extract instance payload; writer is unchanged", () => {
  const host = readFileSync("recipe/scene-readback-combobox-v37.ts", "utf8");
...
test("host emits compile-carried trigger characters Choose a person on empty-content combobox/input; writer is unchanged", () => {
  ...
  assert.match(
    host,
    /if \(\s*sceneRole\(scene\) === "combobox\/input" &&\s*variantProperties\?\.Content === "empty"\s*\)\s*return COMBOBOX_LIVE_V37_TRIGGER_COMPILE_CARRY_CHARACTERS/,
  );
  assert.match(
    host,
    /COMBOBOX_LIVE_V37_TRIGGER_COMPILE_CARRY_CHARACTERS =\s+"Choose a person"/,
  );
  assert.doesNotMatch(host, /if \(combobox\) \{/);
  assert.doesNotMatch(
    host,
    /COMBOBOX_LIVE_V37_TRIGGER_COMPILE_CARRY_CHARACTERS =\s+"Ada Lovelace"/,
  );
...
test("host emits compile-carried label Combobox option on combobox/option-set; writer is unchanged", () => {
  ...
  assert.match(
    host,
    /if \(sceneRole\(scene\) === "combobox\/option-set"\)\s+return COMBOBOX_LIVE_V37_OPTION_SET_COMPILE_CARRY_LABEL/,
  );
  assert.match(
    host,
    /COMBOBOX_LIVE_V37_OPTION_SET_COMPILE_CARRY_LABEL =\s+"Combobox option"/,
  );
