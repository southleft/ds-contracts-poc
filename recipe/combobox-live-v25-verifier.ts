...
import {
  verifyComboboxLiveV25FixedSceneFixedPoint,
  type ComboboxLiveV25FixedFixedPoint,
} from "./combobox-live-v25-fixed-point.js";

export const COMBOBOX_LIVE_V25_VERIFIER_VERSION = "combobox-live-v25-verifier-v1";

export function buildComboboxLiveV25RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV25RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV25RawNode[];
}

export interface ComboboxLiveV25NormalizedScene {
...
export interface ComboboxLiveV25SceneProof {
...
export function normalizeComboboxLiveV25Scene(
  root: ComboboxLiveV25RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV25NormalizedScene {
  const canonicalBindings: ComboboxLiveV25NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV25RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV25Scene<Instance>(
  normalized: ComboboxLiveV25NormalizedScene,
...
): ComboboxLiveV25SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV25FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV25ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV25PreCaptureGates(payload: unknown): void {
