...
import {
  verifyComboboxLiveV9FixedSceneFixedPoint,
  type ComboboxLiveV9FixedFixedPoint,
} from "./combobox-live-v9-fixed-point.js";

export const COMBOBOX_LIVE_V9_VERIFIER_VERSION = "combobox-live-v9-verifier-v1";

export function buildComboboxLiveV9RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV9RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV9RawNode[];
}

export interface ComboboxLiveV9NormalizedScene {
...
export interface ComboboxLiveV9SceneProof {
...
export function normalizeComboboxLiveV9Scene(
  root: ComboboxLiveV9RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV9NormalizedScene {
  const canonicalBindings: ComboboxLiveV9NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV9RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV9Scene<Instance>(
  normalized: ComboboxLiveV9NormalizedScene,
...
): ComboboxLiveV9SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV9FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV9ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV9PreCaptureGates(payload: unknown): void {
