...
import { CALENDAR_LIVE_V11_INDEX_PATH } from "./build-calendar-live-proof-v11.js";
import {
...
import {
  buildCalendarLiveV11AuthorizationArtifact,
  validateCalendarLiveV11AntecedentIndex,
  validateCalendarLiveV11AuthorizationArtifact,
  validateCalendarLiveV11History,
  validateCalendarLiveV11UnitTestSource,
  type CalendarLiveV11AntecedentIndex,
  type CalendarLiveV11AuthorizationArtifact,
  type CalendarLiveV11AuthorizationProof,
  type CalendarLiveV11HistoryState,
} from "./calendar-live-v11-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV11ControlFlowSource,
  validateCalendarLiveV11SecurityAttestation,
  type CalendarLiveV11SecurityAttestation,
} from "./calendar-live-v11-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V11_INDEX_PATH);
const index = (): CalendarLiveV11AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV11AntecedentIndex;

const artifact = (): CalendarLiveV11AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV11AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV11HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV11HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV11AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v11/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV11AuthorizationArtifact,
): CalendarLiveV11SecurityAttestation => {
  const body: Omit<CalendarLiveV11SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V11_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV11History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV11History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV11History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV11History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV11History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV11UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v11-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV11UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV11AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV11AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV11AuthorizationArtifact(
        planted as CalendarLiveV11AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV11SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV11SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV11SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v11-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV11ControlFlowSource(source), []);
});
