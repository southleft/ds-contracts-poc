...
import {
  ...
  INPUT_LIVE_V81_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V81_RESPONSE_CONTRACTS,
  type InputLiveV81RootProof,
} from "./input-field-live-v81-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V81_BROKER_VERSION = "input-live-v81-broker-v2";
export const INPUT_LIVE_V81_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V81_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V81_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V81_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V81_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V81_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV81RemotePhase =
  (typeof INPUT_LIVE_V81_REMOTE_PHASES)[number];
export type InputLiveV81AuthorizationMode = "live" | "simulated";

export interface InputLiveV81CommitPins {
...
export interface InputLiveV81TransactionAuthorization extends InputLiveV81CommitPins {
  mode: InputLiveV81AuthorizationMode;
...
export interface InputLiveV81Manifest {
  artifactVersion: "input-live-v81-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V81_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V81_DYNAMIC_TOOL;
...
export interface InputLiveV81ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV81RequestBody {
  artifactVersion: "input-live-v81-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V81_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV81RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV81CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V81_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV81ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV81Request extends InputLiveV81RequestBody {
...
export interface InputLiveV81Receipt {
  artifactVersion: "input-live-v81-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V81_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV81RemotePhase;
...
export interface InputLiveV81AcceptedResponse<T = unknown> {
  request: InputLiveV81Request;
  receipt: InputLiveV81Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV81IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV81ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V81_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V81_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V81_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V81_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V81_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V81_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V81_TARGET.editorType;
};
...
export const inputLiveV81RequestSequence = (
  phase: InputLiveV81RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV81RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV81RequestPath = (
  directory: string,
  phase: InputLiveV81RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV81ResponsePath = (
  directory: string,
  phase: InputLiveV81RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV81ReceiptPath = (
  directory: string,
  phase: InputLiveV81RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV81Manifest => {
  const manifest = json<InputLiveV81Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v81-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V81_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V81_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV81RemotePhase,
  captureIndex?: number,
): InputLiveV81Request | undefined => {
  const file = inputLiveV81RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV81Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV81RemotePhase,
  captureIndex?: number,
): InputLiveV81Receipt | undefined => {
  const file = inputLiveV81ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV81Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV81Request | undefined;
  receipt: InputLiveV81Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV81RemotePhase,
): InputLiveV81ResponseContract => ({
  ...INPUT_LIVE_V81_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V81_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v81-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V81_BROKER_VERSION,
  target: INPUT_LIVE_V81_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V81_PROGRAM_START*/${program}/*INPUT_LIVE_V81_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V81_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V81_PROGRAM_END*/";
...
export function createInputLiveV81Transaction(
  directory: string,
  ...
  const manifest: InputLiveV81Manifest = {
    artifactVersion: "input-live-v81-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V81_BROKER_VERSION,
...
export function validateInputLiveV81Request(
  directory: string,
  request: InputLiveV81Request,
): void {
  ...
  const expectedSequence = inputLiveV81RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v81-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V81_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V81_TARGET.fileKey ||
...
export function issueInputLiveV81Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV81RemotePhase,
  program: string,
  options: InputLiveV81IssueOptions = {},
): InputLiveV81Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV81RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV81Request>(targetPath);
    validateInputLiveV81Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV81ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV81TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV81RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V81_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V81_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V81_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV81RequestBody = {
    artifactVersion: "input-live-v81-broker-request-v2",
    brokerVersion: INPUT_LIVE_V81_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV81Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV81Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV81RawResponse(
  directory: string,
  phase: InputLiveV81RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV81RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV81Request>(requestFile);
  if (
  ...
  const target = inputLiveV81ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV81Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v81-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V81_BROKER_VERSION,
...
export function acceptInputLiveV81Response<T = unknown>(
  directory: string,
  phase: InputLiveV81RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV81AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV81ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV81Request>(
    inputLiveV81RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV81Request(directory, request);
  if (phase === "capture") validateInputLiveV81TechnicalGates(directory);
  const rawFile = inputLiveV81ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v81-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V81_BROKER_VERSION ||
...
export function persistInputLiveV81TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV81RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV81RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V81_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v81-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V81_BROKER_VERSION,
...
export function validateInputLiveV81TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v81-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V81_TECHNICAL_GATES.some(
...
export function validateInputLiveV81Receipt(
  directory: string,
  phase: InputLiveV81RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV81Request(directory, request);
  const receipt = json<InputLiveV81Receipt>(
    inputLiveV81ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV81ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v81-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V81_BROKER_VERSION ||
...
export function readInputLiveV81AcceptedRawPayload(
  directory: string,
  phase: InputLiveV81RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV81Receipt(directory, phase, captureIndex);
...
export function createInputLiveV81FakeDynamicResponse(
  request: InputLiveV81Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V81_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V81_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV81PersistedRequests(
  directory: string,
): InputLiveV81Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV81Request>(file)] : [];
    });
}

export const inputLiveV81Sha256 = sha256;
