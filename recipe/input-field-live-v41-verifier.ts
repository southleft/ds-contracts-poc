...
export const INPUT_LIVE_V41_VERIFIER_VERSION = "input-live-v41-verifier-v1";

export function buildInputLiveV41RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV41RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV41RawNode[];
}

export interface InputLiveV41NormalizedScene {
...
export interface InputLiveV41SceneProof {
...
export function normalizeInputLiveV41Scene(
  root: InputLiveV41RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV41NormalizedScene {
  const canonicalBindings: InputLiveV41NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV41RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV41Scene<Instance>(
  normalized: InputLiveV41NormalizedScene,
...
export function compileInputLiveV41ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV41PreCaptureGates(payload: unknown): void {
