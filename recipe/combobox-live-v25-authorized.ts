...
import { COMBOBOX_LIVE_V25_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v25.js";
import { verifyComboboxLiveV25Authorization } from "./combobox-live-v25-authorization.js";
import { runComboboxLiveV25Preflight } from "./combobox-live-v25-preflight.js";
import { ComboboxLiveV25Orchestrator } from "./run-combobox-live-v25.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v25 requires exact authorization artifact ${COMBOBOX_LIVE_V25_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v25-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV25Authorization(root);
    runComboboxLiveV25Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV25Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV25Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
