...
import { buildCalendarLiveV3CleanupRuntime } from "./calendar-live-v3-cleanup.js";
import { type CalendarLiveV3FixedFixedPoint } from "./calendar-live-v3-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV3RestoreProgram,
  validateCalendarLiveV3RestorePayload,
  type CalendarLiveV3RestorePayload,
} from "./calendar-live-v3-restore.js";
import {
  buildCalendarLiveV3RawPropertyRuntime,
  normalizeCalendarLiveV3Scene,
  type CalendarLiveV3RawNode,
} from "./calendar-live-v3-verifier.js";
...
export const CALENDAR_LIVE_V3_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V3_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V3_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V3_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V3_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V3_SET_COUNT = 3;
export const CALENDAR_LIVE_V3_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V3_HOST_PHASES = 3;
export const CALENDAR_LIVE_V3_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V3_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V3_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V3_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV3SourceId = (typeof CALENDAR_LIVE_V3_SOURCE_IDS)[number];
export type CalendarLiveV3Kind = "calendar" | "week" | "day";

export interface CalendarLiveV3SourceIdentity {
  source: CalendarLiveV3SourceId;
...
export interface CalendarLiveV3WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV3ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV3SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV3RawNode;
    weekScene: CalendarLiveV3RawNode;
    dayScene: CalendarLiveV3RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV3RootProof {
  source: CalendarLiveV3SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV3ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV3SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV3CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV3SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV3CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV3SourceId;
  frameWidth: number;
...
export interface CalendarLiveV3CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v3 ${label} must be an object`);
...
export function validateCalendarLiveV3WriterPayload(
  value: unknown,
): CalendarLiveV3WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v3") ||
    ...
    result.namespace !== CALENDAR_LIVE_V3_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v3 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v3 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v3 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV3SourceIdentity,
  kind: CalendarLiveV3Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV3ExtractProgram(
  writer: CalendarLiveV3WriterOwnership,
  sources: readonly CalendarLiveV3SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v3 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V3_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V3_NAMESPACE)}
${buildCalendarLiveV3RawPropertyRuntime()}
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("CALENDAR-V3-EXTRACT-PAGE");
...
if(get(page,"pageOwner")!=="recipe/calendar/"+${JSON.stringify(writer.runIdentity)})throw new Error("CALENDAR-V3-EXTRACT-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>SET_IDS.has(node.id));
if(sets.length!==3)throw new Error("CALENDAR-V3-EXTRACT-ROOTS:"+sets.length);
...
for(const set of sets){
  ...
  if(!IDENTITIES[adapterIdentity]||(kind!=="calendar"&&kind!=="week"&&kind!=="day"))throw new Error("CALENDAR-V3-EXTRACT-ADAPTER:"+adapterIdentity+":"+kind);
...
if(roots.length!==1||roots.some(root=>!root.calendarScene||!root.weekScene||!root.dayScene))throw new Error("CALENDAR-V3-EXTRACT-TRIPLE");
...
export function validateCalendarLiveV3ExtractPayload(
  value: unknown,
  writer: CalendarLiveV3WriterOwnership,
): CalendarLiveV3ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v3 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V3_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v3 extract schema/one-root mismatch");
  return payload as CalendarLiveV3ExtractPayload;
}

export function proveCalendarLiveV3Roots<Instance>(
  extract: CalendarLiveV3ExtractPayload,
  sources: readonly (CalendarLiveV3SourceIdentity & {
...
): CalendarLiveV3RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v3 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v3 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV3Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV3Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV3Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV3FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v3 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v3 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v3 compile lost owned sets");
...
export function assertCalendarLiveV3RootProofs(
  proofs: readonly CalendarLiveV3RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v3 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v3 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV3ProbeProgram(
  writer: CalendarLiveV3WriterOwnership,
  sources: readonly CalendarLiveV3SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V3_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV3ProbePayload(
  value: unknown,
  writer: CalendarLiveV3WriterOwnership,
): CalendarLiveV3ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV3ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V3_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V3_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v3 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV3CaptureManifest(
  cells: readonly CalendarLiveV3CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V3_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V3_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V3_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V3_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v3 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV3CaptureCell[],
): string => {
  validateCalendarLiveV3CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV3CaptureProgram(
  writer: CalendarLiveV3WriterOwnership,
  cell: CalendarLiveV3CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V3_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v3 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v3 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
if(!page||page.type!=="PAGE"||page.id==="115:295378"||page.id==="163:35981"||!set||set.type!=="COMPONENT_SET")throw new Error("CALENDAR-V3-CAPTURE-TARGET");
const axes=node=>Object.fromEntries(node.name.split(",").map(part=>part.trim()).filter(part=>part.includes("=")).map(part=>{const at=part.indexOf("=");return[part.slice(0,at),part.slice(at+1)];}));
...
if(!component)throw new Error("CALENDAR-V3-CAPTURE-CELL:"+cell.cellKey);
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V3_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V3-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV3CapturePayload(
  value: unknown,
  cell: CalendarLiveV3CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV3CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV3CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V3_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V3_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v3 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV3CaptureResponses(
  manifest: readonly CalendarLiveV3CaptureCell[],
  responses: readonly CalendarLiveV3CapturePayload[],
): void {
  validateCalendarLiveV3CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v3 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV3ObjectiveReport {
  artifactVersion: "calendar-live-v3-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV3Objective(
  manifest: readonly CalendarLiveV3CaptureCell[],
  responses: readonly CalendarLiveV3CapturePayload[],
): CalendarLiveV3ObjectiveReport {
  assertCalendarLiveV3CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v3-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV3CleanupProgram(
  writer: CalendarLiveV3WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV3CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V3_NAMESPACE,
  ...
  const guardedResult = `if(JSON.stringify(requestedNodeIds)!==JSON.stringify([expectedPageId])||JSON.stringify([...requestedCollectionIds].sort())!==JSON.stringify([...expectedCollectionIds].sort()))throw new Error("CALENDAR-V3-CLEANUP-EXACT-OWNERSHIP");${resultStatement}`;
  const program = baseProgram.replace(resultStatement, guardedResult);
  if (program === baseProgram)
    throw new Error("Calendar live v3 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV3CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV3WriterOwnership,
): CalendarLiveV3CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV3CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v3 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V3_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV3WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
