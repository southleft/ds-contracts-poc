...
export interface InputLiveV37LifecycleSimulationReport {
  artifactVersion: "input-live-v37-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV37AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV37Lifecycle(): Promise<InputLiveV37LifecycleSimulationReport> {
  await buildInputLiveV37Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V37_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV37AntecedentIndex;
  const indexSha256 = inputLiveV37Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v37-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V37_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V37_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV37History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV37History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV37AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V37_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V37_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV37History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV37History(authorized, "pending");
    ...
    await buildInputLiveV37Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V37_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV37AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV37Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV37Lifecycle(), null, 2)}\n`,
  );
