...
import {
  acceptTableLiveV17Response,
  createTableLiveV17Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV17Request,
  persistTableLiveV17RawResponse,
  persistTableLiveV17TechnicalGates,
  readTableLiveV17AcceptedRawPayload,
  type TableLiveV17Request,
  type TableLiveV17TransactionAuthorization,
} from "./table-live-v17-broker.js";
import {
  TABLE_LIVE_V17_CAPTURE_COUNT,
  assertTableLiveV17CaptureResponses,
  ...
  evaluateTableLiveV17Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV17Roots,
  validateTableLiveV17CaptureManifest,
  validateTableLiveV17CapturePayload,
  validateTableLiveV17CleanupPayload,
  validateTableLiveV17ExtractPayload,
  validateTableLiveV17ProbePayload,
  validateTableLiveV17WriterPayload,
  type TableLiveV17CaptureCell,
  type TableLiveV17CapturePayload,
...
import {
  ...
  TABLE_LIVE_V17_PLAN_PATH,
  type TableLiveV17ProofPlan,
} from "./build-table-live-proof-v17.js";
import {
...
export const TABLE_LIVE_V17_RUNNER_VERSION = "table-live-v17-runner-v1";

interface TableLiveV17JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV17OrchestratorState {
  artifactVersion: "table-live-v17-orchestrator-state-v1";
  ...
  journal: TableLiveV17JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV17ProofPlan;
  planSha256: string;
  captures: TableLiveV17CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV17SourceIdentity & {
...
export interface TableLiveV17NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV17OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v17 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V17_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V17_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV17ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV17CaptureCell[];
  validateTableLiveV17CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v17 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v17 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV17WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV17RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV17Request =>
  readJson<TableLiveV17Request>(file);

export class TableLiveV17Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV17Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV17OrchestratorState = {
      artifactVersion: "table-live-v17-orchestrator-state-v1",
    ...
    return new TableLiveV17Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV17OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV17Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV17NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV17NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v17 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV17NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v17 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV17RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV17Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV17CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV17AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV17WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV17Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV17ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV17Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV17Response(this.#directory, "writer", {
        validate: validateTableLiveV17WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV17Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV17RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV17Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV17ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV17Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV17RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v17-host-root-proofs-v1",
      ...
      const next = issueTableLiveV17Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV17ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV17Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV17ProbePayload(payload, ownership),
      });
      persistTableLiveV17TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v17 capture cell missing");
      const accepted = acceptTableLiveV17Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV17CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V17_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV17CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV17Objective(
    ...
    throw new Error(`Table live v17 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV17Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV17CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV17Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV17RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV17Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV17CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV17CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV17CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV17Authorization = (
  privateKey: KeyObject,
): TableLiveV17TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV17SimulationKey = (): KeyObject =>
