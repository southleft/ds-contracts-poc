...
import { COMBOBOX_LIVE_V3_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v3.js";
import { verifyComboboxLiveV3Authorization } from "./combobox-live-v3-authorization.js";
import { runComboboxLiveV3Preflight } from "./combobox-live-v3-preflight.js";
import { ComboboxLiveV3Orchestrator } from "./run-combobox-live-v3.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v3 requires exact authorization artifact ${COMBOBOX_LIVE_V3_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v3-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV3Authorization(root);
    runComboboxLiveV3Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV3Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV3Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
