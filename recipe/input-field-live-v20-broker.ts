...
import {
  assertInputLiveV20RootProofs,
  INPUT_LIVE_V20_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V20_RESPONSE_CONTRACTS,
  type InputLiveV20RootProof,
} from "./input-field-live-v20-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V20_BROKER_VERSION = "input-live-v20-broker-v2";
export const INPUT_LIVE_V20_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V20_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V20_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V20_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V20_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V20_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV20RemotePhase =
  (typeof INPUT_LIVE_V20_REMOTE_PHASES)[number];
export type InputLiveV20AuthorizationMode = "live" | "simulated";

export interface InputLiveV20CommitPins {
...
export interface InputLiveV20TransactionAuthorization extends InputLiveV20CommitPins {
  mode: InputLiveV20AuthorizationMode;
...
export interface InputLiveV20Manifest {
  artifactVersion: "input-live-v20-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V20_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V20_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V20_TARGET;
  authorization: InputLiveV20TransactionAuthorization;
...
export interface InputLiveV20ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV20RequestBody {
  artifactVersion: "input-live-v20-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V20_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV20RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV20CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V20_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV20ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV20Request extends InputLiveV20RequestBody {
...
export interface InputLiveV20Receipt {
  artifactVersion: "input-live-v20-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V20_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV20RemotePhase;
...
export interface InputLiveV20AcceptedResponse<T = unknown> {
  request: InputLiveV20Request;
  receipt: InputLiveV20Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV20IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV20ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V20_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V20_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V20_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V20_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V20_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V20_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V20_TARGET.editorType;
};
...
export const inputLiveV20RequestSequence = (
  phase: InputLiveV20RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV20RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV20RequestPath = (
  directory: string,
  phase: InputLiveV20RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV20ResponsePath = (
  directory: string,
  phase: InputLiveV20RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV20ReceiptPath = (
  directory: string,
  phase: InputLiveV20RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV20Manifest => {
  const manifest = json<InputLiveV20Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v20-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V20_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V20_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV20RemotePhase,
  captureIndex?: number,
): InputLiveV20Request | undefined => {
  const file = inputLiveV20RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV20Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV20RemotePhase,
  captureIndex?: number,
): InputLiveV20Receipt | undefined => {
  const file = inputLiveV20ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV20Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV20Request | undefined;
  receipt: InputLiveV20Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV20RemotePhase,
): InputLiveV20ResponseContract => ({
  ...INPUT_LIVE_V20_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V20_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v20-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V20_BROKER_VERSION,
  target: INPUT_LIVE_V20_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V20_PROGRAM_START*/${program}/*INPUT_LIVE_V20_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V20_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V20_PROGRAM_END*/";
...
export function createInputLiveV20Transaction(
  directory: string,
  ...
  const manifest: InputLiveV20Manifest = {
    artifactVersion: "input-live-v20-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V20_BROKER_VERSION,
...
export function validateInputLiveV20Request(
  directory: string,
  request: InputLiveV20Request,
): void {
  ...
  const expectedSequence = inputLiveV20RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v20-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V20_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V20_TARGET.fileKey ||
...
export function issueInputLiveV20Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV20RemotePhase,
  program: string,
  options: InputLiveV20IssueOptions = {},
): InputLiveV20Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV20RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV20Request>(targetPath);
    validateInputLiveV20Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV20ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV20TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV20RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V20_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V20_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V20_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV20RequestBody = {
    artifactVersion: "input-live-v20-broker-request-v2",
    brokerVersion: INPUT_LIVE_V20_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV20Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV20Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV20RawResponse(
  directory: string,
  phase: InputLiveV20RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV20RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV20Request>(requestFile);
  if (
  ...
  const target = inputLiveV20ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV20Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v20-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V20_BROKER_VERSION,
...
export function acceptInputLiveV20Response<T = unknown>(
  directory: string,
  phase: InputLiveV20RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV20AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV20ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV20Request>(
    inputLiveV20RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV20Request(directory, request);
  if (phase === "capture") validateInputLiveV20TechnicalGates(directory);
  const rawFile = inputLiveV20ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v20-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V20_BROKER_VERSION ||
...
export function persistInputLiveV20TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV20RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV20RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V20_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v20-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V20_BROKER_VERSION,
...
export function validateInputLiveV20TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v20-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V20_TECHNICAL_GATES.some(
...
export function validateInputLiveV20Receipt(
  directory: string,
  phase: InputLiveV20RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV20Request(directory, request);
  const receipt = json<InputLiveV20Receipt>(
    inputLiveV20ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV20ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v20-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V20_BROKER_VERSION ||
...
export function readInputLiveV20AcceptedRawPayload(
  directory: string,
  phase: InputLiveV20RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV20Receipt(directory, phase, captureIndex);
...
export function createInputLiveV20FakeDynamicResponse(
  request: InputLiveV20Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V20_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V20_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV20PersistedRequests(
  directory: string,
): InputLiveV20Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV20Request>(file)] : [];
    });
}

export const inputLiveV20Sha256 = sha256;
