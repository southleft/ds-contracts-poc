...
export interface InputLiveV43LifecycleSimulationReport {
  artifactVersion: "input-live-v43-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV43AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV43Lifecycle(): Promise<InputLiveV43LifecycleSimulationReport> {
  await buildInputLiveV43Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V43_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV43AntecedentIndex;
  const indexSha256 = inputLiveV43Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v43-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V43_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V43_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV43History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV43History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV43AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V43_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V43_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV43History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV43History(authorized, "pending");
    ...
    await buildInputLiveV43Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V43_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV43AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV43Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV43Lifecycle(), null, 2)}\n`,
  );
