/**
 * Table live v35 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V35_FIXED_VERIFIER_VERSION = "table-live-v35-fixed-point-v1";
export const TABLE_LIVE_V35_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV35FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV35FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV35FixedSourceProbe {
...
export interface TableLiveV35FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV35FixedFixedPoint;
}

export interface TableLiveV35FixedHardGateInput {
  thresholds: TableLiveV35FixedThresholds;
  sourceProbes: TableLiveV35FixedSourceProbe[];
  cells: TableLiveV35FixedCellValidation[];
  sceneProofs: TableLiveV35FixedSceneProof[];
...
export interface TableLiveV35FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V35_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV35FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV35FixedHardGates(
  input: TableLiveV35FixedHardGateInput,
): TableLiveV35FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV35FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V35_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV35FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV35FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
