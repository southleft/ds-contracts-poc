...
import {
  assertComboboxLiveV5RootProofs,
  COMBOBOX_LIVE_V5_CAPTURE_COUNT,
  COMBOBOX_LIVE_V5_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  COMBOBOX_LIVE_V5_RESPONSE_CONTRACTS,
  type ComboboxLiveV5RootProof,
} from "./combobox-live-v5-contract.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V5_BROKER_VERSION = "combobox-live-v5-broker-v2";
export const COMBOBOX_LIVE_V5_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const COMBOBOX_LIVE_V5_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const COMBOBOX_LIVE_V5_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const COMBOBOX_LIVE_V5_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const COMBOBOX_LIVE_V5_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const COMBOBOX_LIVE_V5_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type ComboboxLiveV5RemotePhase =
  (typeof COMBOBOX_LIVE_V5_REMOTE_PHASES)[number];
export type ComboboxLiveV5AuthorizationMode = "live" | "simulated";

export interface ComboboxLiveV5CommitPins {
...
export interface ComboboxLiveV5TransactionAuthorization extends ComboboxLiveV5CommitPins {
  mode: ComboboxLiveV5AuthorizationMode;
...
export interface ComboboxLiveV5Manifest {
  artifactVersion: "combobox-live-v5-broker-manifest-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V5_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V5_DYNAMIC_TOOL;
  target: typeof COMBOBOX_LIVE_V5_TARGET;
  authorization: ComboboxLiveV5TransactionAuthorization;
...
export interface ComboboxLiveV5ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface ComboboxLiveV5RequestBody {
  artifactVersion: "combobox-live-v5-broker-request-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V5_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: ComboboxLiveV5RemotePhase;
  sequence: number;
  ...
  commitPins: ComboboxLiveV5CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof COMBOBOX_LIVE_V5_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: ComboboxLiveV5ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface ComboboxLiveV5Request extends ComboboxLiveV5RequestBody {
...
export interface ComboboxLiveV5Receipt {
  artifactVersion: "combobox-live-v5-broker-receipt-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V5_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: ComboboxLiveV5RemotePhase;
...
export interface ComboboxLiveV5AcceptedResponse<T = unknown> {
  request: ComboboxLiveV5Request;
  receipt: ComboboxLiveV5Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface ComboboxLiveV5IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: ComboboxLiveV5ResponseContract;
...
const equalTarget = (value: unknown): value is typeof COMBOBOX_LIVE_V5_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === COMBOBOX_LIVE_V5_TARGET.fileKey &&
  (value as BrokerTarget).fileName === COMBOBOX_LIVE_V5_TARGET.fileName &&
  (value as BrokerTarget).editorType === COMBOBOX_LIVE_V5_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== COMBOBOX_LIVE_V5_TARGET.fileKey ||
    context.fileName !== COMBOBOX_LIVE_V5_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === COMBOBOX_LIVE_V5_TARGET.editorType;
};
...
const requestId = (
  ...
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= COMBOBOX_LIVE_V5_CAPTURE_COUNT
  )
...
const readManifest = (directory: string): ComboboxLiveV5Manifest => {
  const manifest = json<ComboboxLiveV5Manifest>(
  ...
  if (
    manifest.artifactVersion !== "combobox-live-v5-broker-manifest-v2" ||
    manifest.brokerVersion !== COMBOBOX_LIVE_V5_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== COMBOBOX_LIVE_V5_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(COMBOBOX_LIVE_V5_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: ComboboxLiveV5RemotePhase,
  captureIndex?: number,
): ComboboxLiveV5Request | undefined => {
  const file = comboboxLiveV5RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<ComboboxLiveV5Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: ComboboxLiveV5RemotePhase,
  captureIndex?: number,
): ComboboxLiveV5Receipt | undefined => {
  const file = comboboxLiveV5ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<ComboboxLiveV5Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: ComboboxLiveV5Request | undefined;
  receipt: ComboboxLiveV5Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: ComboboxLiveV5RemotePhase,
): ComboboxLiveV5ResponseContract => ({
  ...COMBOBOX_LIVE_V5_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: ComboboxLiveV5Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: COMBOBOX_LIVE_V5_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "combobox-live-v5-plugin-response-v2",
  brokerVersion: COMBOBOX_LIVE_V5_BROKER_VERSION,
  target: COMBOBOX_LIVE_V5_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*COMBOBOX_LIVE_V5_PROGRAM_START*/${program}/*COMBOBOX_LIVE_V5_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*COMBOBOX_LIVE_V5_PROGRAM_START*/";
  const endMarker = "/*COMBOBOX_LIVE_V5_PROGRAM_END*/";
...
export function createComboboxLiveV5Transaction(
  directory: string,
  ...
  const manifest: ComboboxLiveV5Manifest = {
    artifactVersion: "combobox-live-v5-broker-manifest-v2",
    brokerVersion: COMBOBOX_LIVE_V5_BROKER_VERSION,
    ...
    captureRequests: COMBOBOX_LIVE_V5_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateComboboxLiveV5Request(
  directory: string,
  request: ComboboxLiveV5Request,
): void {
  ...
  if (
    request.artifactVersion !== "combobox-live-v5-broker-request-v2" ||
    request.brokerVersion !== COMBOBOX_LIVE_V5_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== COMBOBOX_LIVE_V5_TARGET.fileKey ||
...
export function issueComboboxLiveV5Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: ComboboxLiveV5RemotePhase,
  program: string,
  options: ComboboxLiveV5IssueOptions = {},
): ComboboxLiveV5Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<ComboboxLiveV5Request>(targetPath);
    validateComboboxLiveV5Request(directory, existing);
  ...
  if (phase === "capture") validateComboboxLiveV5TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const arguments_ = {
    fileKey: COMBOBOX_LIVE_V5_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? COMBOBOX_LIVE_V5_SIGNED_WRITER_TIMEOUT_MS
        : COMBOBOX_LIVE_V5_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: ComboboxLiveV5RequestBody = {
    artifactVersion: "combobox-live-v5-broker-request-v2",
    brokerVersion: COMBOBOX_LIVE_V5_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: ComboboxLiveV5Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateComboboxLiveV5Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistComboboxLiveV5RawResponse(
  directory: string,
  phase: ComboboxLiveV5RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  const request = json<ComboboxLiveV5Request>(requestFile);
  if (
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): ComboboxLiveV5Receipt => {
  const body: Omit<ComboboxLiveV5Receipt, "receiptSha256"> = {
    artifactVersion: "combobox-live-v5-broker-receipt-v2",
    brokerVersion: COMBOBOX_LIVE_V5_BROKER_VERSION,
...
export function acceptComboboxLiveV5Response<T = unknown>(
  directory: string,
  phase: ComboboxLiveV5RemotePhase,
  options: {
    captureIndex?: number;
...
): ComboboxLiveV5AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  ...
  const request = json<ComboboxLiveV5Request>(
  ...
  if (
    ...
    plugin?.artifactVersion !== "combobox-live-v5-plugin-response-v2" ||
    plugin?.brokerVersion !== COMBOBOX_LIVE_V5_BROKER_VERSION ||
...
export function persistComboboxLiveV5TechnicalGates(
  directory: string,
  rootProofs: readonly ComboboxLiveV5RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertComboboxLiveV5RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    COMBOBOX_LIVE_V5_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "combobox-live-v5-host-technical-gates-v2",
    brokerVersion: COMBOBOX_LIVE_V5_BROKER_VERSION,
...
export function validateComboboxLiveV5TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "combobox-live-v5-host-technical-gates-v2" ||
    ...
    COMBOBOX_LIVE_V5_TECHNICAL_GATES.some(
...
export function validateComboboxLiveV5Receipt(
  directory: string,
  phase: ComboboxLiveV5RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateComboboxLiveV5Request(directory, request);
  const receipt = json<ComboboxLiveV5Receipt>(
  ...
  if (
    receipt.artifactVersion !== "combobox-live-v5-broker-receipt-v2" ||
    receipt.brokerVersion !== COMBOBOX_LIVE_V5_BROKER_VERSION ||
...
export function readComboboxLiveV5AcceptedRawPayload(
  directory: string,
  phase: ComboboxLiveV5RemotePhase,
  captureIndex?: number,
): unknown {
  validateComboboxLiveV5Receipt(directory, phase, captureIndex);
...
export function createComboboxLiveV5FakeDynamicResponse(
  request: ComboboxLiveV5Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? COMBOBOX_LIVE_V5_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? COMBOBOX_LIVE_V5_TARGET,
    ...overrides.extra,
  };
}

export function listComboboxLiveV5PersistedRequests(
  directory: string,
): ComboboxLiveV5Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<ComboboxLiveV5Request>(file)] : [];
    });
}
