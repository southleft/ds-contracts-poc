...
import {
  acceptTableLiveV29Response,
  createTableLiveV29Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV29Request,
  persistTableLiveV29RawResponse,
  persistTableLiveV29TechnicalGates,
  readTableLiveV29AcceptedRawPayload,
  type TableLiveV29Request,
  type TableLiveV29TransactionAuthorization,
} from "./table-live-v29-broker.js";
import {
  TABLE_LIVE_V29_CAPTURE_COUNT,
  assertTableLiveV29CaptureResponses,
  ...
  evaluateTableLiveV29Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV29Roots,
  validateTableLiveV29CaptureManifest,
  validateTableLiveV29CapturePayload,
  validateTableLiveV29CleanupPayload,
  validateTableLiveV29ExtractPayload,
  validateTableLiveV29ProbePayload,
  validateTableLiveV29WriterPayload,
  type TableLiveV29CaptureCell,
  type TableLiveV29CapturePayload,
...
import {
  ...
  TABLE_LIVE_V29_PLAN_PATH,
  type TableLiveV29ProofPlan,
...
export const TABLE_LIVE_V29_RUNNER_VERSION = "table-live-v29-runner-v1";

interface TableLiveV29JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV29OrchestratorState {
  artifactVersion: "table-live-v29-orchestrator-state-v1";
  ...
  journal: TableLiveV29JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV29ProofPlan;
  planSha256: string;
  captures: TableLiveV29CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV29SourceIdentity & {
...
export interface TableLiveV29NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV29OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v29 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V29_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V29_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV29ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV29CaptureCell[];
  validateTableLiveV29CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v29 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v29 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV29WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV29RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV29Request =>
  readJson<TableLiveV29Request>(file);

export class TableLiveV29Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV29Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV29OrchestratorState = {
      artifactVersion: "table-live-v29-orchestrator-state-v1",
    ...
    return new TableLiveV29Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV29OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV29Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV29NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV29NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v29 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV29NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v29 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV29RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV29Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV29CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV29AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV29WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV29Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV29ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV29Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV29Response(this.#directory, "writer", {
        validate: validateTableLiveV29WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV29Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV29RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV29Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV29ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV29Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV29RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v29-host-root-proofs-v1",
      ...
      const next = issueTableLiveV29Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV29ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV29Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV29ProbePayload(payload, ownership),
      });
      persistTableLiveV29TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v29 capture cell missing");
      const accepted = acceptTableLiveV29Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV29CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V29_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV29CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV29Objective(
    ...
    throw new Error(`Table live v29 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV29Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV29CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV29Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV29RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV29Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV29CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV29CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV29CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV29Authorization = (
  privateKey: KeyObject,
): TableLiveV29TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV29SimulationKey = (): KeyObject =>
