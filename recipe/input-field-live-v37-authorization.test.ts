...
import { INPUT_LIVE_V37_INDEX_PATH } from "./build-input-field-live-proof-v37.js";
import {
  INPUT_LIVE_V37_DYNAMIC_TOOL,
  INPUT_LIVE_V37_TARGET,
} from "./input-field-live-v37-broker.js";
import {
  buildInputLiveV37AuthorizationArtifact,
  validateInputLiveV37AntecedentIndex,
  validateInputLiveV37AuthorizationArtifact,
  validateInputLiveV37History,
  validateInputLiveV37UnitTestSource,
  type InputLiveV37AntecedentIndex,
  type InputLiveV37AuthorizationArtifact,
  type InputLiveV37AuthorizationProof,
  type InputLiveV37HistoryState,
} from "./input-field-live-v37-authorization.js";
import {
  inputLiveV37AttestationSha256,
  validateInputLiveV37ControlFlowSource,
  validateInputLiveV37SecurityAttestation,
  type InputLiveV37SecurityAttestation,
} from "./input-field-live-v37-preflight.js";
import { inputLiveV37Sha256 } from "./input-field-live-v37-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V37_INDEX_PATH);
const index = (): InputLiveV37AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV37AntecedentIndex;

const artifact = (): InputLiveV37AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV37AuthorizationArtifact({
...
const pendingState = (): InputLiveV37HistoryState => ({
  index: index(),
  indexSha256: inputLiveV37Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV37HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV37AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV37Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V37_TARGET,
  expectedDynamicTool: INPUT_LIVE_V37_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v37/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV37AuthorizationArtifact,
): InputLiveV37SecurityAttestation => {
  const body: Omit<InputLiveV37SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v37-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V37_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV37AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV37History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV37History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV37History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV37History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV37History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV37UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV37UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV37AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV37Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV37AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV37AuthorizationArtifact(
        planted as InputLiveV37AuthorizationArtifact,
        index(),
        inputLiveV37Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV37SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV37AttestationSha256(
      body as Omit<InputLiveV37SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV37SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV37ControlFlowSource(source), []);
});
