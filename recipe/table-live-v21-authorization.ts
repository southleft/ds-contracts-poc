...
export type TableLiveV21HistoryExpectation = "pending" | "authorized";

export interface TableLiveV21AuthorizationArtifact {
  artifactVersion: "table-live-v21-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V21_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV21AntecedentIndex {
...
export interface TableLiveV21HistoryState {
  index?: TableLiveV21AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV21AuthorizationArtifact;
...
export interface TableLiveV21AuthorizationProof
  extends TableLiveV21TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V21_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V21_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V21_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V21_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v21-authorization") ||
  artifactPath.includes("table-live-v21-preflight") ||
  artifactPath.includes("table-live-v21-authorized") ||
  artifactPath.includes("create-table-live-v21-security-attestation") ||
  artifactPath.includes("table-live-pivot-v21-status") ||
...
export function validateTableLiveV21AntecedentIndex(
  index: TableLiveV21AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v21-antecedent-index-v1" ||
...
export function validateTableLiveV21UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV21History\(\s*["']/.test(source) ||
    /readTableLiveV21HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV21AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV21AuthorizationArtifact {
  ...
  ) as TableLiveV21AntecedentIndex;
  const indexFailures = validateTableLiveV21AntecedentIndex(index);
...
export function validateTableLiveV21AuthorizationArtifact(
  artifact: TableLiveV21AuthorizationArtifact | undefined,
  index: TableLiveV21AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v21-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v21" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V21_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V21_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V21_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V21_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V21_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V21_SOURCE_ROOTS ||
...
export function validateTableLiveV21History(
  state: TableLiveV21HistoryState,
  expected: TableLiveV21HistoryExpectation,
): string[] {
  const failures = validateTableLiveV21AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV21AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV21HistoryState(
  root = process.cwd(),
): TableLiveV21HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V21_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV21AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V21_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V21_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V21_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV21History(
  expected: TableLiveV21HistoryExpectation,
  root = process.cwd(),
): TableLiveV21HistoryState {
  const state = readTableLiveV21HistoryState(root);
  const failures = validateTableLiveV21History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v21 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV21Authorization(
  root = process.cwd(),
): TableLiveV21AuthorizationProof {
  const state = verifyTableLiveV21History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V21_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v21/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v21.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V21_TARGET,
    expectedDynamicTool: TABLE_LIVE_V21_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V21_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV21HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV21History(expected);
  process.stdout.write(
    `Table live v21 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
