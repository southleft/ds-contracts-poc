...
import { CALENDAR_LIVE_V50_INDEX_PATH } from "./build-calendar-live-proof-v50.js";
import {
...
import {
  ...
  validateCalendarLiveV50History,
  validateCalendarLiveV50UnitTestSource,
  type CalendarLiveV50AntecedentIndex,
  type CalendarLiveV50AuthorizationArtifact,
  type CalendarLiveV50AuthorizationProof,
  type CalendarLiveV50HistoryState,
} from "./calendar-live-v50-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV50ControlFlowSource,
  validateCalendarLiveV50SecurityAttestation,
  type CalendarLiveV50SecurityAttestation,
} from "./calendar-live-v50-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V50_INDEX_PATH);
const index = (): CalendarLiveV50AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV50AntecedentIndex;

const artifact = (): CalendarLiveV50AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV50AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV50HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV50HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV50AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v50/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV50AuthorizationArtifact,
): CalendarLiveV50SecurityAttestation => {
  const body: Omit<CalendarLiveV50SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V50_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV50History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV50History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV50History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV50History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV50History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV50UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v50-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV50UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV50AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV50AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV50AuthorizationArtifact(
        planted as CalendarLiveV50AuthorizationArtifact,
        index(),
...
test("one-root denominator, capture gate, cleanup, and prior-lineage reuse are rejected", () => {
  ...
  for (const mutate of [
    ...
    (value: Record<string, any>) => {
      value.execution.comboboxLiveV50AuthorizationReusable = true;
    },
    (value: Record<string, any>) => {
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV50SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV50SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV50SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v50-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV50ControlFlowSource(source), []);
});
