...
export interface InputLiveV40LifecycleSimulationReport {
  artifactVersion: "input-live-v40-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV40AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV40Lifecycle(): Promise<InputLiveV40LifecycleSimulationReport> {
  await buildInputLiveV40Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V40_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV40AntecedentIndex;
  const indexSha256 = inputLiveV40Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v40-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V40_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V40_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV40History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV40History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV40AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V40_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V40_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV40History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV40History(authorized, "pending");
    ...
    await buildInputLiveV40Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V40_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV40AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV40Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV40Lifecycle(), null, 2)}\n`,
  );
