/**
 * Table live v24 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V24_FIXED_VERIFIER_VERSION = "table-live-v24-fixed-point-v1";
export const TABLE_LIVE_V24_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV24FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV24FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV24FixedSourceProbe {
...
export interface TableLiveV24FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV24FixedFixedPoint;
}

export interface TableLiveV24FixedHardGateInput {
  thresholds: TableLiveV24FixedThresholds;
  sourceProbes: TableLiveV24FixedSourceProbe[];
  cells: TableLiveV24FixedCellValidation[];
  sceneProofs: TableLiveV24FixedSceneProof[];
...
export interface TableLiveV24FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V24_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV24FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV24FixedHardGates(
  input: TableLiveV24FixedHardGateInput,
): TableLiveV24FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV24FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V24_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV24FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV24FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
