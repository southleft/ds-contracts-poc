...
import {
  verifyCalendarLiveV13FixedSceneFixedPoint,
  type CalendarLiveV13FixedFixedPoint,
} from "./calendar-live-v13-fixed-point.js";

export const CALENDAR_LIVE_V13_VERIFIER_VERSION = "calendar-live-v13-verifier-v1";

export function buildCalendarLiveV13RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV13RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV13RawNode[];
}

export interface CalendarLiveV13NormalizedScene {
...
export interface CalendarLiveV13SceneProof {
...
export function normalizeCalendarLiveV13Scene(
  root: CalendarLiveV13RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV13NormalizedScene {
  const canonicalBindings: CalendarLiveV13NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV13RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV13Scene<Instance>(
  normalized: CalendarLiveV13NormalizedScene,
...
): CalendarLiveV13SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV13FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV13ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV13PreCaptureGates(payload: unknown): void {
