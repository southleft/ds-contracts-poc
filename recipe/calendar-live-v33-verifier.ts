...
import {
  verifyCalendarLiveV33FixedSceneFixedPoint,
  type CalendarLiveV33FixedFixedPoint,
} from "./calendar-live-v33-fixed-point.js";

export const CALENDAR_LIVE_V33_VERIFIER_VERSION = "calendar-live-v33-verifier-v1";

export function buildCalendarLiveV33RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV33RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV33RawNode[];
}

export interface CalendarLiveV33NormalizedScene {
...
export interface CalendarLiveV33SceneProof {
...
export function normalizeCalendarLiveV33Scene(
  root: CalendarLiveV33RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV33NormalizedScene {
  const canonicalBindings: CalendarLiveV33NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV33RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV33Scene<Instance>(
  normalized: CalendarLiveV33NormalizedScene,
...
): CalendarLiveV33SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV33FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV33ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV33PreCaptureGates(payload: unknown): void {
