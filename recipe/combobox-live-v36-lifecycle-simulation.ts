...
export interface ComboboxLiveV36LifecycleSimulationReport {
  artifactVersion: "combobox-live-v36-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV36AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV36Lifecycle(): Promise<ComboboxLiveV36LifecycleSimulationReport> {
  await buildComboboxLiveV36Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V36_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV36AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v36-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V36_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V36_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV36History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV36History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV36AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V36_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V36_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV36History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV36History(authorized, "pending");
    ...
    await buildComboboxLiveV36Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V36_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV36AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV36Lifecycle(), null, 2)}\n`,
  );
