...
import { COMBOBOX_LIVE_V5_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v5.js";
import { verifyComboboxLiveV5Authorization } from "./combobox-live-v5-authorization.js";
import { runComboboxLiveV5Preflight } from "./combobox-live-v5-preflight.js";
import { ComboboxLiveV5Orchestrator } from "./run-combobox-live-v5.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v4 requires exact authorization artifact ${COMBOBOX_LIVE_V5_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v5-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV5Authorization(root);
    runComboboxLiveV5Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV5Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV5Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
