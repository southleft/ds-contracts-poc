...
import {
  ...
  verifyInputLiveV36Authorization,
  type InputLiveV36AntecedentIndex,
  type InputLiveV36AuthorizationProof,
} from "./input-field-live-v36-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V36_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v36/operator-security-attestation-template.json";
export const INPUT_LIVE_V36_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v36-security-attestation.json";

export interface InputLiveV36SecurityAttestation {
...
export interface InputLiveV36PreflightReport {
  artifactVersion: "input-live-v36-preflight-v1";
...
export const inputLiveV36AttestationSha256 = (
  value: Omit<InputLiveV36SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV36SecurityAttestation(
  value: unknown,
  proof: InputLiveV36AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v36-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V36_TARGET) ||
...
export function validateInputLiveV36ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV36Authorization");
  const preflight = source.indexOf("runInputLiveV36Preflight");
  const initialize = source.indexOf("InputLiveV36Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV36Preflight(
  root: string,
  proof: InputLiveV36AuthorizationProof,
  attestationPath = INPUT_LIVE_V36_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV36PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V36_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V36_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V36_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV36AntecedentIndex;
  failures.push(...validateInputLiveV36AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V36_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V36_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v36-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV36ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV36SecurityAttestation;
  return {
    artifactVersion: "input-live-v36-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V36_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV36Preflight(
    process.cwd(),
    verifyInputLiveV36Authorization(),
    attestationPath,
