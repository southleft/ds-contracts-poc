/**
 * Carried Combobox live v9 host verifier. Uses scene-readback-combobox-v9 so live
...
export const COMBOBOX_LIVE_V9_FIXED_VERIFIER_VERSION = "combobox-live-v9-fixed-point-v1";
export const COMBOBOX_LIVE_V9_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV9FixedThresholds {
...
export interface ComboboxLiveV9FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV9FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV9FixedSourceProbe {
...
export interface ComboboxLiveV9FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV9FixedFixedPoint;
}

export interface ComboboxLiveV9FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV9FixedHardGateInput {
  thresholds: ComboboxLiveV9FixedThresholds;
  materialRegression: ComboboxLiveV9FixedMaterialRegression;
  sourceProbes: ComboboxLiveV9FixedSourceProbe[];
  cells: ComboboxLiveV9FixedCellValidation[];
  sceneProofs: ComboboxLiveV9FixedSceneProof[];
  visualRows: ComboboxLiveV9FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV9FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V9_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV9FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV9FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV9FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV9FixedHardGates(
  input: ComboboxLiveV9FixedHardGateInput,
): ComboboxLiveV9FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV9FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV9FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V9_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV9FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV9FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
