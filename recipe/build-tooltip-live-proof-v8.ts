...
const EVIDENCE = "recipe/evidence/tooltip-live-pivot-v8";
...
const receiptOwned = {
      artifactVersion: "tooltip-live-pivot-v8-prepare",
      teaching:
...
if (process.argv.includes("--check")) {
  ...
  if (writer.sourcePlans.length !== 6)
    throw new Error("tooltip prepare requires six library sources (three reviewed + three proposed)");
  console.log(
    JSON.stringify({
      check: "ok",
      runIdentity: writer.runIdentity,
