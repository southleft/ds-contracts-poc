...
export interface InputLiveV83LifecycleSimulationReport {
  artifactVersion: "input-live-v83-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV83AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV83Lifecycle(): Promise<InputLiveV83LifecycleSimulationReport> {
  await buildInputLiveV83Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V83_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV83AntecedentIndex;
  const indexSha256 = inputLiveV83Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v83-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V83_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V83_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV83History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV83History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV83AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V83_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V83_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV83History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV83History(authorized, "pending");
    ...
    await buildInputLiveV83Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V83_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV83AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV83Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV83Lifecycle(), null, 2)}\n`,
  );
