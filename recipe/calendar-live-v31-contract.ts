...
import { buildCalendarLiveV31CleanupRuntime } from "./calendar-live-v31-cleanup.js";
import { type CalendarLiveV31FixedFixedPoint } from "./calendar-live-v31-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV31RestoreProgram,
  validateCalendarLiveV31RestorePayload,
  type CalendarLiveV31RestorePayload,
} from "./calendar-live-v31-restore.js";
import {
  buildCalendarLiveV31RawPropertyRuntime,
  normalizeCalendarLiveV31Scene,
  type CalendarLiveV31RawNode,
} from "./calendar-live-v31-verifier.js";
...
export const CALENDAR_LIVE_V31_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V31_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V31_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V31_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V31_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V31_SET_COUNT = 3;
export const CALENDAR_LIVE_V31_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V31_HOST_PHASES = 3;
export const CALENDAR_LIVE_V31_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V31_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V31_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V31_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV31SourceId = (typeof CALENDAR_LIVE_V31_SOURCE_IDS)[number];
export type CalendarLiveV31Kind = "calendar" | "week" | "day";

export interface CalendarLiveV31SourceIdentity {
  source: CalendarLiveV31SourceId;
...
export interface CalendarLiveV31WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV31ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV31SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV31RawNode;
    weekScene: CalendarLiveV31RawNode;
    dayScene: CalendarLiveV31RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV31RootProof {
  source: CalendarLiveV31SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV31FixedFixedPoint;
}

export interface CalendarLiveV31ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV31SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV31CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV31SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV31CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV31SourceId;
  frameWidth: number;
...
export interface CalendarLiveV31CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v31 ${label} must be an object`);
...
export function validateCalendarLiveV31WriterPayload(
  value: unknown,
): CalendarLiveV31WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v31") ||
    ...
    result.namespace !== CALENDAR_LIVE_V31_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v31 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v31 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v31 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV31SourceIdentity,
  kind: CalendarLiveV31Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV31ExtractProgram(
  writer: CalendarLiveV31WriterOwnership,
  sources: readonly CalendarLiveV31SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v31 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V31_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V31_NAMESPACE)}
${buildCalendarLiveV31RawPropertyRuntime()}
...
export function validateCalendarLiveV31ExtractPayload(
  value: unknown,
  writer: CalendarLiveV31WriterOwnership,
): CalendarLiveV31ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v31 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V31_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v31 extract schema/one-root mismatch");
  return payload as CalendarLiveV31ExtractPayload;
}

export function proveCalendarLiveV31Roots<Instance>(
  extract: CalendarLiveV31ExtractPayload,
  sources: readonly (CalendarLiveV31SourceIdentity & {
...
): CalendarLiveV31RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v31 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v31 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV31Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV31Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV31Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV31FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v31 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v31 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v31 compile lost owned sets");
...
export function assertCalendarLiveV31RootProofs(
  proofs: readonly CalendarLiveV31RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v31 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v31 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV31ProbeProgram(
  writer: CalendarLiveV31WriterOwnership,
  sources: readonly CalendarLiveV31SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V31_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV31ProbePayload(
  value: unknown,
  writer: CalendarLiveV31WriterOwnership,
): CalendarLiveV31ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV31ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V31_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V31_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v31 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV31CaptureManifest(
  cells: readonly CalendarLiveV31CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V31_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V31_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V31_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V31_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v31 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV31CaptureCell[],
): string => {
  validateCalendarLiveV31CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV31CaptureProgram(
  writer: CalendarLiveV31WriterOwnership,
  cell: CalendarLiveV31CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V31_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v31 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v31 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V31_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV31CapturePayload(
  value: unknown,
  cell: CalendarLiveV31CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV31CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV31CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V31_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V31_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v31 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV31CaptureResponses(
  manifest: readonly CalendarLiveV31CaptureCell[],
  responses: readonly CalendarLiveV31CapturePayload[],
): void {
  validateCalendarLiveV31CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v31 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV31ObjectiveReport {
  artifactVersion: "calendar-live-v31-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV31Objective(
  manifest: readonly CalendarLiveV31CaptureCell[],
  responses: readonly CalendarLiveV31CapturePayload[],
): CalendarLiveV31ObjectiveReport {
  assertCalendarLiveV31CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v31-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV31CleanupProgram(
  writer: CalendarLiveV31WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV31CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V31_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v31 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV31CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV31WriterOwnership,
): CalendarLiveV31CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV31CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v31 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V31_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV31WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
