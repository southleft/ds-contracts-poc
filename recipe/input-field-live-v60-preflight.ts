...
import {
  ...
  verifyInputLiveV60Authorization,
  type InputLiveV60AntecedentIndex,
  type InputLiveV60AuthorizationProof,
} from "./input-field-live-v60-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V60_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v60/operator-security-attestation-template.json";
export const INPUT_LIVE_V60_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v60-security-attestation.json";

export interface InputLiveV60SecurityAttestation {
...
export interface InputLiveV60PreflightReport {
  artifactVersion: "input-live-v60-preflight-v1";
...
export const inputLiveV60AttestationSha256 = (
  value: Omit<InputLiveV60SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV60SecurityAttestation(
  value: unknown,
  proof: InputLiveV60AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v60-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V60_TARGET) ||
...
export function validateInputLiveV60ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV60Authorization");
  const preflight = source.indexOf("runInputLiveV60Preflight");
  const initialize = source.indexOf("InputLiveV60Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV60Preflight(
  root: string,
  proof: InputLiveV60AuthorizationProof,
  attestationPath = INPUT_LIVE_V60_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV60PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V60_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V60_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V60_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV60AntecedentIndex;
  failures.push(...validateInputLiveV60AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V60_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V60_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v60-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV60ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV60SecurityAttestation;
  return {
    artifactVersion: "input-live-v60-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V60_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV60Preflight(
    process.cwd(),
    verifyInputLiveV60Authorization(),
    attestationPath,
