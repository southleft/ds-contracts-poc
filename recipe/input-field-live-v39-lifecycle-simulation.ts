...
export interface InputLiveV39LifecycleSimulationReport {
  artifactVersion: "input-live-v39-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV39AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV39Lifecycle(): Promise<InputLiveV39LifecycleSimulationReport> {
  await buildInputLiveV39Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V39_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV39AntecedentIndex;
  const indexSha256 = inputLiveV39Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v39-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V39_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V39_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV39History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV39History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV39AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V39_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V39_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV39History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV39History(authorized, "pending");
    ...
    await buildInputLiveV39Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V39_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV39AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV39Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV39Lifecycle(), null, 2)}\n`,
  );
