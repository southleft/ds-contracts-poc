...
import { INPUT_LIVE_V59_INDEX_PATH } from "./build-input-field-live-proof-v59.js";
import {
  INPUT_LIVE_V59_DYNAMIC_TOOL,
  INPUT_LIVE_V59_TARGET,
} from "./input-field-live-v59-broker.js";
import {
  buildInputLiveV59AuthorizationArtifact,
  validateInputLiveV59AntecedentIndex,
  validateInputLiveV59AuthorizationArtifact,
  validateInputLiveV59History,
  validateInputLiveV59UnitTestSource,
  type InputLiveV59AntecedentIndex,
  type InputLiveV59AuthorizationArtifact,
  type InputLiveV59AuthorizationProof,
  type InputLiveV59HistoryState,
} from "./input-field-live-v59-authorization.js";
import {
  inputLiveV59AttestationSha256,
  validateInputLiveV59ControlFlowSource,
  validateInputLiveV59SecurityAttestation,
  type InputLiveV59SecurityAttestation,
} from "./input-field-live-v59-preflight.js";
import { inputLiveV59Sha256 } from "./input-field-live-v59-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V59_INDEX_PATH);
const index = (): InputLiveV59AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV59AntecedentIndex;

const artifact = (): InputLiveV59AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV59AuthorizationArtifact({
...
const pendingState = (): InputLiveV59HistoryState => ({
  index: index(),
  indexSha256: inputLiveV59Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV59HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV59AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV59Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V59_TARGET,
  expectedDynamicTool: INPUT_LIVE_V59_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v59/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV59AuthorizationArtifact,
): InputLiveV59SecurityAttestation => {
  const body: Omit<InputLiveV59SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v59-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V59_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV59AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV59History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV59History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV59History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV59History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV59History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV59UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV59UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV59AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV59Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV59AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV59AuthorizationArtifact(
        planted as InputLiveV59AuthorizationArtifact,
        index(),
        inputLiveV59Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV59SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV59AttestationSha256(
      body as Omit<InputLiveV59SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV59SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV59ControlFlowSource(source), []);
});
