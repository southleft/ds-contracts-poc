...
export interface InputLiveV44LifecycleSimulationReport {
  artifactVersion: "input-live-v44-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV44AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV44Lifecycle(): Promise<InputLiveV44LifecycleSimulationReport> {
  await buildInputLiveV44Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V44_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV44AntecedentIndex;
  const indexSha256 = inputLiveV44Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v44-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V44_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V44_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV44History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV44History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV44AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V44_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V44_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV44History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV44History(authorized, "pending");
    ...
    await buildInputLiveV44Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V44_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV44AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV44Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV44Lifecycle(), null, 2)}\n`,
  );
