...
import { buildCalendarLiveV29CleanupRuntime } from "./calendar-live-v29-cleanup.js";
import { type CalendarLiveV29FixedFixedPoint } from "./calendar-live-v29-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV29RestoreProgram,
  validateCalendarLiveV29RestorePayload,
  type CalendarLiveV29RestorePayload,
} from "./calendar-live-v29-restore.js";
import {
  buildCalendarLiveV29RawPropertyRuntime,
  normalizeCalendarLiveV29Scene,
  type CalendarLiveV29RawNode,
} from "./calendar-live-v29-verifier.js";
...
export const CALENDAR_LIVE_V29_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V29_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V29_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V29_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V29_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V29_SET_COUNT = 3;
export const CALENDAR_LIVE_V29_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V29_HOST_PHASES = 3;
export const CALENDAR_LIVE_V29_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V29_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V29_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V29_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV29SourceId = (typeof CALENDAR_LIVE_V29_SOURCE_IDS)[number];
export type CalendarLiveV29Kind = "calendar" | "week" | "day";

export interface CalendarLiveV29SourceIdentity {
  source: CalendarLiveV29SourceId;
...
export interface CalendarLiveV29WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV29ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV29SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV29RawNode;
    weekScene: CalendarLiveV29RawNode;
    dayScene: CalendarLiveV29RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV29RootProof {
  source: CalendarLiveV29SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV29FixedFixedPoint;
}

export interface CalendarLiveV29ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV29SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV29CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV29SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV29CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV29SourceId;
  frameWidth: number;
...
export interface CalendarLiveV29CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v29 ${label} must be an object`);
...
export function validateCalendarLiveV29WriterPayload(
  value: unknown,
): CalendarLiveV29WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v29") ||
    ...
    result.namespace !== CALENDAR_LIVE_V29_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v29 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v29 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v29 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV29SourceIdentity,
  kind: CalendarLiveV29Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV29ExtractProgram(
  writer: CalendarLiveV29WriterOwnership,
  sources: readonly CalendarLiveV29SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v29 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V29_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V29_NAMESPACE)}
${buildCalendarLiveV29RawPropertyRuntime()}
...
export function validateCalendarLiveV29ExtractPayload(
  value: unknown,
  writer: CalendarLiveV29WriterOwnership,
): CalendarLiveV29ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v29 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V29_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v29 extract schema/one-root mismatch");
  return payload as CalendarLiveV29ExtractPayload;
}

export function proveCalendarLiveV29Roots<Instance>(
  extract: CalendarLiveV29ExtractPayload,
  sources: readonly (CalendarLiveV29SourceIdentity & {
...
): CalendarLiveV29RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v29 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v29 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV29Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV29Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV29Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV29FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v29 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v29 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v29 compile lost owned sets");
...
export function assertCalendarLiveV29RootProofs(
  proofs: readonly CalendarLiveV29RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v29 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v29 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV29ProbeProgram(
  writer: CalendarLiveV29WriterOwnership,
  sources: readonly CalendarLiveV29SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V29_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV29ProbePayload(
  value: unknown,
  writer: CalendarLiveV29WriterOwnership,
): CalendarLiveV29ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV29ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V29_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V29_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v29 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV29CaptureManifest(
  cells: readonly CalendarLiveV29CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V29_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V29_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V29_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V29_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v29 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV29CaptureCell[],
): string => {
  validateCalendarLiveV29CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV29CaptureProgram(
  writer: CalendarLiveV29WriterOwnership,
  cell: CalendarLiveV29CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V29_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v29 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v29 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V29_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV29CapturePayload(
  value: unknown,
  cell: CalendarLiveV29CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV29CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV29CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V29_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V29_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v29 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV29CaptureResponses(
  manifest: readonly CalendarLiveV29CaptureCell[],
  responses: readonly CalendarLiveV29CapturePayload[],
): void {
  validateCalendarLiveV29CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v29 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV29ObjectiveReport {
  artifactVersion: "calendar-live-v29-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV29Objective(
  manifest: readonly CalendarLiveV29CaptureCell[],
  responses: readonly CalendarLiveV29CapturePayload[],
): CalendarLiveV29ObjectiveReport {
  assertCalendarLiveV29CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v29-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV29CleanupProgram(
  writer: CalendarLiveV29WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV29CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V29_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v29 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV29CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV29WriterOwnership,
): CalendarLiveV29CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV29CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v29 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V29_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV29WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
