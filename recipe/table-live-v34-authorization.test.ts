...
import { TABLE_LIVE_V34_INDEX_PATH } from "./build-table-live-proof-v34.js";
import {
...
import {
  buildTableLiveV34AuthorizationArtifact,
  validateTableLiveV34AntecedentIndex,
  validateTableLiveV34AuthorizationArtifact,
  validateTableLiveV34History,
  validateTableLiveV34UnitTestSource,
  type TableLiveV34AntecedentIndex,
  type TableLiveV34AuthorizationArtifact,
  type TableLiveV34AuthorizationProof,
  type TableLiveV34HistoryState,
} from "./table-live-v34-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV34ControlFlowSource,
  validateTableLiveV34SecurityAttestation,
  type TableLiveV34SecurityAttestation,
} from "./table-live-v34-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V34_INDEX_PATH);
const index = (): TableLiveV34AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV34AntecedentIndex;

const artifact = (): TableLiveV34AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV34AuthorizationArtifact({
...
const pendingState = (): TableLiveV34HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV34HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV34AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v34/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV34AuthorizationArtifact,
): TableLiveV34SecurityAttestation => {
  const body: Omit<TableLiveV34SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V34_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV34History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV34History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV34History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV34History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV34History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV34UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v34-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV34UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV34AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV34AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV34AuthorizationArtifact(
        planted as TableLiveV34AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV34SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV34SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV34SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v34-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV34ControlFlowSource(source), []);
});
