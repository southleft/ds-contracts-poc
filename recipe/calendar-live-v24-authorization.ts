...
export type CalendarLiveV24HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV24AuthorizationArtifact {
  artifactVersion: "calendar-live-v24-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V24_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV24AntecedentIndex {
...
export interface CalendarLiveV24HistoryState {
  index?: CalendarLiveV24AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV24AuthorizationArtifact;
...
export interface CalendarLiveV24AuthorizationProof
  extends CalendarLiveV24TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V24_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V24_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V24_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V24_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v24-authorization") ||
  artifactPath.includes("calendar-live-v24-preflight") ||
  artifactPath.includes("calendar-live-v24-authorized") ||
  artifactPath.includes("create-calendar-live-v24-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v24-status") ||
...
export function validateCalendarLiveV24AntecedentIndex(
  index: CalendarLiveV24AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v24-antecedent-index-v1" ||
...
export function validateCalendarLiveV24UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV24History\(\s*["']/.test(source) ||
    /readCalendarLiveV24HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV24AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV24AuthorizationArtifact {
  ...
  ) as CalendarLiveV24AntecedentIndex;
  const indexFailures = validateCalendarLiveV24AntecedentIndex(index);
...
export function validateCalendarLiveV24AuthorizationArtifact(
  artifact: CalendarLiveV24AuthorizationArtifact | undefined,
  index: CalendarLiveV24AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v24-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v24" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V24_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V24_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V24_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V24_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V24_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V24_SOURCE_ROOTS ||
...
export function validateCalendarLiveV24History(
  state: CalendarLiveV24HistoryState,
  expected: CalendarLiveV24HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV24AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV24AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV24HistoryState(
  root = process.cwd(),
): CalendarLiveV24HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V24_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV24AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V24_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V24_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V24_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV24History(
  expected: CalendarLiveV24HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV24HistoryState {
  const state = readCalendarLiveV24HistoryState(root);
  const failures = validateCalendarLiveV24History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v24 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV24Authorization(
  root = process.cwd(),
): CalendarLiveV24AuthorizationProof {
  const state = verifyCalendarLiveV24History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V24_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v24/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v24.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V24_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V24_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V24_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV24HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV24History(expected);
  process.stdout.write(
    `Calendar live v24 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
