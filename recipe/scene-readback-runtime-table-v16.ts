...
/**
 * Table live v16 extract runtime. Keeps the v6 skip of untagged
 * `table/row/owned-cell-label` TEXT bind hosts. Extract bytes are
 * unchanged from v15. The v16 teaching lives in host-normalize
 * (`scene-readback-table-v1.ts`): omit empty `dashPattern` on
 * `table/variant` strokes because compile variant strokes omit
 * `dashPattern`. Does not omit variant `strokes` or `cornerRadius`.
 * Does not invent `dashPattern` onto compile.
 */
