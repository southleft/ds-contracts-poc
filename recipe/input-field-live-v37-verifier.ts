...
export const INPUT_LIVE_V37_VERIFIER_VERSION = "input-live-v37-verifier-v1";

export function buildInputLiveV37RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV37RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV37RawNode[];
}

export interface InputLiveV37NormalizedScene {
...
export interface InputLiveV37SceneProof {
...
export function normalizeInputLiveV37Scene(
  root: InputLiveV37RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV37NormalizedScene {
  const canonicalBindings: InputLiveV37NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV37RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV37Scene<Instance>(
  normalized: InputLiveV37NormalizedScene,
...
export function compileInputLiveV37ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV37PreCaptureGates(payload: unknown): void {
