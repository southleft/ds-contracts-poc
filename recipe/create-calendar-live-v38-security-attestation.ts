...
import { verifyCalendarLiveV38Authorization } from "./calendar-live-v38-authorization.js";
import { CALENDAR_LIVE_V38_TARGET } from "./calendar-live-v38-broker.js";
import {
  CALENDAR_LIVE_V38_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV38Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v38-session";
...
const attestation: CalendarLiveV38SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV38SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v38 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V38_SECURITY_ATTESTATION_DEFAULT_PATH,
);
