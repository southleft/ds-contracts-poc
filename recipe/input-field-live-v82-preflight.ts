...
import {
  ...
  verifyInputLiveV82Authorization,
  type InputLiveV82AntecedentIndex,
  type InputLiveV82AuthorizationProof,
} from "./input-field-live-v82-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V82_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v82/operator-security-attestation-template.json";
export const INPUT_LIVE_V82_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v82-security-attestation.json";

export interface InputLiveV82SecurityAttestation {
...
export interface InputLiveV82PreflightReport {
  artifactVersion: "input-live-v82-preflight-v1";
...
export const inputLiveV82AttestationSha256 = (
  value: Omit<InputLiveV82SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV82SecurityAttestation(
  value: unknown,
  proof: InputLiveV82AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v82-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V82_TARGET) ||
...
export function validateInputLiveV82ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV82Authorization");
  const preflight = source.indexOf("runInputLiveV82Preflight");
  const initialize = source.indexOf("InputLiveV82Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV82Preflight(
  root: string,
  proof: InputLiveV82AuthorizationProof,
  attestationPath = INPUT_LIVE_V82_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV82PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V82_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V82_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V82_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV82AntecedentIndex;
  failures.push(...validateInputLiveV82AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V82_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V82_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v82-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV82ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV82SecurityAttestation;
  return {
    artifactVersion: "input-live-v82-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V82_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV82Preflight(
    process.cwd(),
    verifyInputLiveV82Authorization(),
    attestationPath,
