...
import { buildTableLiveV8CleanupRuntime } from "./table-live-v8-cleanup.js";
import { type TableLiveV8FixedFixedPoint } from "./table-live-v8-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV8RestoreProgram,
  validateTableLiveV8RestorePayload,
  type TableLiveV8RestorePayload,
} from "./table-live-v8-restore.js";
import {
  buildTableLiveV8RawPropertyRuntime,
  normalizeTableLiveV8Scene,
  type TableLiveV8RawNode,
} from "./table-live-v8-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v8.js";
import {
...
export const TABLE_LIVE_V8_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V8_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V8_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V8_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V8_VARIANT_COUNT = 20;
export const TABLE_LIVE_V8_SET_COUNT = 6;
export const TABLE_LIVE_V8_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V8_HOST_PHASES = 3;
export const TABLE_LIVE_V8_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V8_RESTORE_COUNT = 8;
export const TABLE_LIVE_V8_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V8_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV8SourceId = (typeof TABLE_LIVE_V8_SOURCE_IDS)[number];
export type TableLiveV8Kind = "table" | "row" | "cell";

export interface TableLiveV8SourceIdentity {
  source: TableLiveV8SourceId;
...
export interface TableLiveV8WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV8ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV8SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV8RawNode;
    rowScene: TableLiveV8RawNode;
    cellScene: TableLiveV8RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV8RootProof {
  source: TableLiveV8SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV8FixedFixedPoint;
}

export interface TableLiveV8ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV8SourceId;
    adapterIdentity: string;
...
export interface TableLiveV8CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV8SourceId;
  adapterIdentity: string;
...
export interface TableLiveV8CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV8SourceId;
  frameWidth: number;
...
export interface TableLiveV8CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v8 ${label} must be an object`);
...
export function validateTableLiveV8WriterPayload(
  value: unknown,
): TableLiveV8WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V8_NAMESPACE ||
    ...
    throw new TypeError("Table live v8 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v8 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v8 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV8SourceIdentity,
  kind: TableLiveV8Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV8ExtractProgram(
  writer: TableLiveV8WriterOwnership,
  sources: readonly TableLiveV8SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v8 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V8_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V8_NAMESPACE)}
${buildTableLiveV8RawPropertyRuntime()}
...
export function validateTableLiveV8ExtractPayload(
  value: unknown,
  writer: TableLiveV8WriterOwnership,
): TableLiveV8ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v8 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V8_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v8 extract schema/two-root mismatch");
  return payload as TableLiveV8ExtractPayload;
}

export function proveTableLiveV8Roots<Instance>(
  extract: TableLiveV8ExtractPayload,
  sources: readonly (TableLiveV8SourceIdentity & {
...
): TableLiveV8RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v8 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v8 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV8Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV8Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV8Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV8FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v8 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v8 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v8 compile lost owned sets");
...
export function assertTableLiveV8RootProofs(
  proofs: readonly TableLiveV8RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v8 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v8 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV8ProbeProgram(
  writer: TableLiveV8WriterOwnership,
  sources: readonly TableLiveV8SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V8_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
for(const row of byAdapter.values()){
 ...
 const cell0=propertyKey(rowInstance,"Cell 0"),cell0Before=cell0&&rowInstance.componentProperties[cell0].value;let textPropertiesRestored=false;if(cell0){rowInstance.setProperties({[cell0]:"Table v8 deterministic probe"});const changed=nodes(rowInstance).some(node=>node.type==="TEXT"&&node.characters==="Table v8 deterministic probe");rowInstance.setProperties({[cell0]:cell0Before});textPropertiesRestored=changed&&JSON.stringify(original)===JSON.stringify(plain(instance));}
 rowInstance.remove();
...
export function validateTableLiveV8ProbePayload(
  value: unknown,
  writer: TableLiveV8WriterOwnership,
): TableLiveV8ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV8ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V8_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V8_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Table live v8 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV8CaptureManifest(
  cells: readonly TableLiveV8CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V8_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V8_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V8_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V8_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v8 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV8CaptureCell[],
): string => {
  validateTableLiveV8CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV8CaptureProgram(
  writer: TableLiveV8WriterOwnership,
  cell: TableLiveV8CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V8_CAPTURE_COUNT
  )
    throw new TypeError("Table live v8 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v8 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
const frame=figma.createFrame();frame.name="Table v8 ephemeral / "+cell.cellKey;page.appendChild(frame);
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V8_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV8CapturePayload(
  value: unknown,
  cell: TableLiveV8CaptureCell,
  rawResponseBytes: number,
): TableLiveV8CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV8CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V8_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V8_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v8 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV8CaptureResponses(
  manifest: readonly TableLiveV8CaptureCell[],
  responses: readonly TableLiveV8CapturePayload[],
): void {
  validateTableLiveV8CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v8 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV8ObjectiveReport {
  artifactVersion: "table-live-v8-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV8Objective(
  manifest: readonly TableLiveV8CaptureCell[],
  responses: readonly TableLiveV8CapturePayload[],
): TableLiveV8ObjectiveReport {
  assertTableLiveV8CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v8-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV8CleanupProgram(
  writer: TableLiveV8WriterOwnership,
): string {
  const baseProgram = buildTableLiveV8CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V8_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v8 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV8CleanupPayload(
  payload: unknown,
  writer: TableLiveV8WriterOwnership,
): TableLiveV8CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV8CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v8 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V8_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV8WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
