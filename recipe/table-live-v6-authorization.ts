...
import {
  TABLE_LIVE_V6_DYNAMIC_TOOL,
  TABLE_LIVE_V6_TARGET,
  type TableLiveV6TransactionAuthorization,
} from "./table-live-v6-broker.js";
import {
...
import {
  ...
  TABLE_LIVE_V6_HOST_PHASES,
  TABLE_LIVE_V6_REMOTE_REQUESTS,
  TABLE_LIVE_V6_SOURCE_ROOTS,
} from "./table-live-v6-contract.js";
import { canonicalJson } from "./normalize.js";

export type TableLiveV6HistoryExpectation = "pending" | "authorized";

export interface TableLiveV6AuthorizationArtifact {
  artifactVersion: "table-live-v6-capture-authorization-v1";
  authorizationId: "table-live-v6";
  status: "authorization declared; runtime security prerequisites still mandatory";
  authorizationIntent: true;
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V6_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV6AntecedentIndex {
  artifactVersion: "table-live-v6-antecedent-index-v1";
  status: string;
...
export interface TableLiveV6HistoryState {
  ...
  authorization?: TableLiveV6AuthorizationArtifact;
...
export interface TableLiveV6AuthorizationProof
  extends TableLiveV6TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V6_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V6_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V6_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V6_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v6-authorization") ||
  artifactPath.includes("table-live-v6-preflight") ||
  artifactPath.includes("table-live-v6-authorized") ||
  artifactPath.includes("create-table-live-v6-security-attestation") ||
  artifactPath.includes("table-live-pivot-v6-status") ||
...
export function validateTableLiveV6AntecedentIndex(
  index: TableLiveV6AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v6-antecedent-index-v1" ||
    index.authorizationCanBeAddedWithoutAntecedentRebuild !== true
  )
    failures.push("table v6 antecedent index missing or malformed");
  const artifactEntries = Object.entries(index?.artifacts ?? {});
  if (artifactEntries.length === 0)
    failures.push("table v6 antecedent hash denominator is empty");
  for (const [artifactPath, metadata] of artifactEntries) {
    ...
    if (
      ...
      failures.push(`malformed table v6 antecedent hash: ${artifactPath}`);
  }
  ...
  if (index?.hashSetSha256 !== expectedHashSet)
    failures.push("table v6 antecedent hash-set digest mismatch");
  if (
    ...
    failures.push("table v6 antecedent counts missing or drifted");
  for (const required of [
    TABLE_LIVE_V6_AUTHORIZATION_PATH,
    "recipe/table-live-v6-authorization.ts",
    "recipe/table-live-v6-authorization.test.ts",
    "recipe/table-live-v6-preflight.ts",
    "recipe/evidence/table-live-pivot-v6/operator-security-attestation-template.json",
    "recipe/evidence/status-index.json",
  ])
    if (!index?.authorizationLifecycleExcluded?.includes(required))
      failures.push(`table v6 lifecycle exclusion missing: ${required}`);
  const leaks = forbiddenAntecedentFieldPaths(index);
  if (leaks.length)
    failures.push(`table v6 antecedent result leakage: ${leaks.join(",")}`);
  return failures;
}

export function validateTableLiveV6UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV6History\(\s*["']/.test(source) ||
    /readTableLiveV6HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV6AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV6AuthorizationArtifact {
  if (!SHA40.test(options.antecedentCommit))
    throw new TypeError("table v6 antecedent commit must be a full SHA");
  const index = JSON.parse(
    Buffer.from(options.antecedentIndexBytes).toString("utf8"),
  ) as TableLiveV6AntecedentIndex;
  const indexFailures = validateTableLiveV6AntecedentIndex(index);
  if (indexFailures.length)
    throw new TypeError(
      `table v6 antecedent index refused:\n${indexFailures.join("\n")}`,
    );
  const publicKey =
  ...
  return {
    ...
    antecedent: {
      ...
      indexPath: TABLE_LIVE_V6_INDEX_PATH,
    ...
    denominator: {
      remoteRequests: TABLE_LIVE_V6_REMOTE_REQUESTS,
      hostPhases: TABLE_LIVE_V6_HOST_PHASES,
      captures: TABLE_LIVE_V6_CAPTURE_COUNT,
      sourceRoots: TABLE_LIVE_V6_SOURCE_ROOTS,
...
export function validateTableLiveV6AuthorizationArtifact(
  artifact: TableLiveV6AuthorizationArtifact | undefined,
  index: TableLiveV6AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v6-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v6" ||
    artifact.authorizationIntent !== true
  )
    failures.push("table v6 authorization declaration missing or malformed");
  const resultLeaks = forbiddenAntecedentFieldPaths(artifact);
  if (resultLeaks.length)
    failures.push(`table v6 authorization result leakage: ${resultLeaks.join(",")}`);
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V6_INDEX_PATH ||
    artifact?.antecedent?.indexSha256 !== indexSha256 ||
    artifact?.antecedent?.hashSetSha256 !== index?.hashSetSha256
  )
    failures.push("table v6 authorization pins wrong antecedent index");
  if (privateMaterialPaths(artifact).length)
    failures.push("table v6 authorization contains private key or token material");
  try {
    ...
    if (
      ...
      failures.push("table v6 Ed25519 public-key identity mismatch");
  } catch {
    failures.push("table v6 Ed25519 public key is invalid");
  }
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V6_TARGET) ||
    !equalJson(
    ...
    failures.push("wrong table v6 target or dynamic tool");
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V6_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V6_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V6_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V6_SOURCE_ROOTS ||
    artifact?.denominator?.expectedFacts !== index?.counts.expectedSceneFacts
  )
    failures.push("table v6 two-root or capture denominator changed");
  if (
    ...
    failures.push(
      "table v6 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
    ...
    failures.push("table v6 security prerequisites weakened");
  if (
    artifact?.humanSignoff?.mandatory !== true ||
    artifact?.humanSignoff?.status !== "pending"
  )
    failures.push("table v6 human signoff overclaim");
  return failures;
}

export function validateTableLiveV6History(
  state: TableLiveV6HistoryState,
  expected: TableLiveV6HistoryExpectation,
): string[] {
  const failures = validateTableLiveV6AntecedentIndex(state.index);
  if (!SHA40.test(state.codeCommit))
    failures.push("table v6 code commit is not a full SHA");
  ...
  if (
    ...
    failures.push("table v6 antecedent first-add commit is not unique");
  if (!state.antecedentIsAncestorOfCode)
    failures.push("code commit does not descend from table v6 antecedent");
  if (!state.antecedentIndexBytesMatchFirstAddition)
    failures.push("table v6 antecedent index changed after first addition");
  if (!state.antecedentArtifactsMatchIndexAtCommit)
    failures.push("table v6 antecedent commit artifacts do not match index");
  if (!state.workingAntecedentArtifactsMatchIndex)
    failures.push("table v6 antecedent working bytes drifted");
  ...
  failures.push(
    ...validateTableLiveV6AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
  ...
  if (
    ...
    failures.push("table v6 authorization first-add lineage mismatch");
  if (!state.authorizationPresentAtCodeCommit)
    failures.push("table v6 authorization missing from code commit");
  if (!state.authorizationBytesMatchFirstAddition)
    failures.push("table v6 authorization bytes changed after first addition");
  if (!state.authorizationStrictlyDescendsFromAntecedent)
    failures.push("table v6 authorization does not strictly descend from antecedent");
  if (!state.authorizationIsAncestorOfCode)
    failures.push("code commit predates table v6 authorization");
  return failures;
}
...
export function readTableLiveV6HistoryState(
  root = process.cwd(),
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V6_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV6AuthorizationArtifact)
    : undefined;
  ...
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V6_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V6_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV6History(
  expected: TableLiveV6HistoryExpectation,
  root = process.cwd(),
): TableLiveV6HistoryState {
  const state = readTableLiveV6HistoryState(root);
  const failures = validateTableLiveV6History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v6 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV6Authorization(
  root = process.cwd(),
): TableLiveV6AuthorizationProof {
  const state = verifyTableLiveV6History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V6_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v6/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v6.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V6_TARGET,
    expectedDynamicTool: TABLE_LIVE_V6_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V6_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV6HistoryExpectation => {
  ...
  if (pending === authorized)
    throw new Error(
      "Choose exactly one table v6 history mode: --expect-pending or --expect-authorized",
    );
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV6History(expected);
  process.stdout.write(
    `Table live v6 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
