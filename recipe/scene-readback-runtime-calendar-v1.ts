...
/**
 * Calendar live v1 extract runtime. Keeps the v6 skip of untagged
 * `calendar/day/label` TEXT bind hosts. Extract bytes are
 * unchanged from v21. The v32 teaching lives in host-normalize
 * (`scene-readback-calendar-v1.ts`): emit compile-carried label
 * `Calendar week` on `calendar/week-set` instead of the live display name
...
const readSceneDerivedTree=async(node,expectedPlan,expectedOwner)=>{
  ...
  const read=async(current,generatedContext)=>{
    ...
    void "CALENDAR-EXTRACT-IGNORE-COPIED-INSTANCE-OWNERSHIP-KEY";
    if(explicit&&!generatedContext){
      ...
      if(expectedOwner){
        ...
        void "CALENDAR-EXTRACT-SET-ROOT-ENVELOPE-HASH";
    ...
    const TABLE_EXTRACT_MEASURE_HIDDEN_CONTENT_FILL="CALENDAR-EXTRACT-MEASURE-HIDDEN-CONTENT-FILL";
    const contentFillRoles=new Set(["calendar/day/label"]);
    ...
    if("children" in current){
      ...
      const walkingRowComponent=current.type==="COMPONENT"&&String(ownershipKey||"").indexOf("week/")===0;
      const TABLE_EXTRACT_SKIP_ROW_OWNED_CELL_LABEL_BIND_HOST="CALENDAR-EXTRACT-SKIP-WEEK-OWNED-DAY-LABEL-BIND-HOST";
      for(let childIndex=0;childIndex<current.children.length;childIndex++){
        ...
        const untaggedOwnedCellLabelBindHost=child.type==="TEXT"&&!child.getSharedPluginData(SCENE_READBACK_NS,"ownershipKey")&&String(child.name||"").indexOf("calendar/day/label/")===0&&child.componentPropertyReferences&&child.componentPropertyReferences.characters;
