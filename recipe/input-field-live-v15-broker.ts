...
import {
  ...
  INPUT_LIVE_V15_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V15_RESPONSE_CONTRACTS,
  type InputLiveV15RootProof,
} from "./input-field-live-v15-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V15_BROKER_VERSION = "input-live-v15-broker-v2";
export const INPUT_LIVE_V15_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V15_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V15_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V15_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V15_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V15_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV15RemotePhase =
  (typeof INPUT_LIVE_V15_REMOTE_PHASES)[number];
export type InputLiveV15AuthorizationMode = "live" | "simulated";

export interface InputLiveV15CommitPins {
...
export interface InputLiveV15TransactionAuthorization extends InputLiveV15CommitPins {
  mode: InputLiveV15AuthorizationMode;
...
export interface InputLiveV15Manifest {
  artifactVersion: "input-live-v15-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V15_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V15_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V15_TARGET;
...
export interface InputLiveV15ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV15RequestBody {
  ...
  commitPins: InputLiveV15CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V15_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV15ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV15Request extends InputLiveV15RequestBody {
...
export interface InputLiveV15Receipt {
  artifactVersion: "input-live-v15-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V15_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV15RemotePhase;
...
export interface InputLiveV15AcceptedResponse<T = unknown> {
  request: InputLiveV15Request;
  receipt: InputLiveV15Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV15IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV15ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V15_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V15_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V15_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V15_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V15_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V15_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V15_TARGET.editorType;
};
...
export const inputLiveV15RequestSequence = (
  phase: InputLiveV15RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV15RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV15RequestPath = (
  directory: string,
  phase: InputLiveV15RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV15ResponsePath = (
  directory: string,
  phase: InputLiveV15RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV15ReceiptPath = (
  directory: string,
  phase: InputLiveV15RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV15Manifest => {
  const manifest = json<InputLiveV15Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v15-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V15_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V15_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV15RemotePhase,
  captureIndex?: number,
): InputLiveV15Request | undefined => {
  const file = inputLiveV15RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV15Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV15RemotePhase,
  captureIndex?: number,
): InputLiveV15Receipt | undefined => {
  const file = inputLiveV15ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV15Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV15Request | undefined;
  receipt: InputLiveV15Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV15RemotePhase,
): InputLiveV15ResponseContract => ({
  ...INPUT_LIVE_V15_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V15_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V15_DYNAMIC_TOOL,
      target: INPUT_LIVE_V15_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v15-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V15_BROKER_VERSION,
  target: INPUT_LIVE_V15_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V15_PROGRAM_START*/${program}/*INPUT_LIVE_V15_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V15_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V15_PROGRAM_END*/";
...
export function createInputLiveV15Transaction(
  directory: string,
  ...
  const manifest: InputLiveV15Manifest = {
    artifactVersion: "input-live-v15-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V15_BROKER_VERSION,
...
export function validateInputLiveV15Request(
  directory: string,
  request: InputLiveV15Request,
): void {
  ...
  const expectedSequence = inputLiveV15RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v15-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V15_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V15_TARGET.fileKey ||
...
export function issueInputLiveV15Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV15RemotePhase,
  program: string,
  options: InputLiveV15IssueOptions = {},
): InputLiveV15Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV15RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV15Request>(targetPath);
    validateInputLiveV15Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV15ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV15TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV15RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    ...
    timeout:
      ...
      (phase === "writer"
        ...
        : INPUT_LIVE_V15_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV15RequestBody = {
    artifactVersion: "input-live-v15-broker-request-v2",
    brokerVersion: INPUT_LIVE_V15_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV15Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV15Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV15RawResponse(
  directory: string,
  phase: InputLiveV15RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV15RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV15Request>(requestFile);
  if (
  ...
  const target = inputLiveV15ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV15Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v15-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V15_BROKER_VERSION,
...
export function acceptInputLiveV15Response<T = unknown>(
  directory: string,
  phase: InputLiveV15RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV15AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV15ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV15Request>(
    inputLiveV15RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV15Request(directory, request);
  if (phase === "capture") validateInputLiveV15TechnicalGates(directory);
  const rawFile = inputLiveV15ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v15-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V15_BROKER_VERSION ||
...
export function persistInputLiveV15TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV15RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV15RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V15_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v15-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V15_BROKER_VERSION,
...
export function validateInputLiveV15TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v15-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V15_TECHNICAL_GATES.some(
...
export function validateInputLiveV15Receipt(
  directory: string,
  phase: InputLiveV15RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV15Request(directory, request);
  const receipt = json<InputLiveV15Receipt>(
    inputLiveV15ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV15ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v15-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V15_BROKER_VERSION ||
...
export function readInputLiveV15AcceptedRawPayload(
  directory: string,
  phase: InputLiveV15RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV15Receipt(directory, phase, captureIndex);
...
export function createInputLiveV15FakeDynamicResponse(
  request: InputLiveV15Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V15_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V15_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV15PersistedRequests(
  directory: string,
): InputLiveV15Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV15Request>(file)] : [];
    });
}

export const inputLiveV15Sha256 = sha256;
