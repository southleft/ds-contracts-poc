...
import {
  acceptComboboxLiveV27Response,
  createComboboxLiveV27Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV27Request,
  persistComboboxLiveV27RawResponse,
  persistComboboxLiveV27TechnicalGates,
  readComboboxLiveV27AcceptedRawPayload,
  type ComboboxLiveV27Request,
  type ComboboxLiveV27TransactionAuthorization,
} from "./combobox-live-v27-broker.js";
import {
  COMBOBOX_LIVE_V27_CAPTURE_COUNT,
  assertComboboxLiveV27CaptureResponses,
  ...
  evaluateComboboxLiveV27Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV27Roots,
  validateComboboxLiveV27CaptureManifest,
  validateComboboxLiveV27CapturePayload,
  validateComboboxLiveV27CleanupPayload,
  validateComboboxLiveV27ExtractPayload,
  validateComboboxLiveV27ProbePayload,
  validateComboboxLiveV27WriterPayload,
  type ComboboxLiveV27CaptureCell,
  type ComboboxLiveV27CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V27_PLAN_PATH,
  type ComboboxLiveV27ProofPlan,
} from "./build-combobox-live-proof-v27.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v27.js";

export const COMBOBOX_LIVE_V27_RUNNER_VERSION = "combobox-live-v27-runner-v1";

interface ComboboxLiveV27JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV27OrchestratorState {
  artifactVersion: "combobox-live-v27-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV27JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV27ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV27CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV27SourceIdentity & {
...
export interface ComboboxLiveV27NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV27OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v27 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V27_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V27_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV27ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV27CaptureCell[];
  validateComboboxLiveV27CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v27 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v27 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV27WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV27RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV27Request =>
  readJson<ComboboxLiveV27Request>(file);

export class ComboboxLiveV27Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV27Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV27OrchestratorState = {
      artifactVersion: "combobox-live-v27-orchestrator-state-v1",
    ...
    return new ComboboxLiveV27Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV27OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV27Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV27NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV27NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v27 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV27NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v27 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV27RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV27Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV27CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV27AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV27WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV27Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV27ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV27Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV27Response(this.#directory, "writer", {
        validate: validateComboboxLiveV27WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV27Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV27RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV27Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV27ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV27Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV27RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v27-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV27Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV27ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV27Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV27ProbePayload(payload, ownership),
      });
      persistComboboxLiveV27TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v27 capture cell missing");
      const accepted = acceptComboboxLiveV27Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV27CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V27_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV27CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV27Objective(
    ...
    throw new Error(`Combobox live v27 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV27Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV27CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV27Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV27RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV27Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV27CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV27CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV27CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV27Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV27TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV27SimulationKey = (): KeyObject =>
