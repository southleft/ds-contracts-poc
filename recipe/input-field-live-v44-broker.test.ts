...
import {
  ...
  createInputLiveV44FakeDynamicResponse,
  ...
  listInputLiveV44PersistedRequests,
  persistInputLiveV44RawResponse,
  persistInputLiveV44TechnicalGates,
  validateInputLiveV44Receipt,
  validateInputLiveV44Request,
  type InputLiveV44RemotePhase,
...
import {
  ...
  type InputLiveV44ExtractPayload,
  type InputLiveV44ProbePayload,
  type InputLiveV44RestorePayload,
  type InputLiveV44RootProof,
  type InputLiveV44WriterOwnership,
} from "./input-field-live-v44-contract.js";
import {
  InputLiveV44Orchestrator,
  simulatedInputLiveV44Authorization,
} from "./run-input-field-live-v44.js";
...
const captures: InputLiveV44CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV44Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v44-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV44Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV44CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV44FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV44FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V44_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V44_TARGET);
  persistInputLiveV44RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV44Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV44Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV44WriterOwnership,
): InputLiveV44RestorePayload => ({
...
const rootProofs = (): InputLiveV44RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV44CaptureCell,
): InputLiveV44CapturePayload => {
  const bytes = Buffer.from(`input-live-v44-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV44Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV44Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV44RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV44RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV44ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV44ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV44CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV44CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV44CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV44Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV44CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV44PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV44ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV44Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV44Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV44Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV44Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V44_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV44RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V44_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV44Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV44Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV44ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV44Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV44WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV44RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV44Request;
    validateInputLiveV44Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV44Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV44CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v44-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV44Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV44Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV44Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V44_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV44RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V44_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV44Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV44Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV44RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V44_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV44Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV44Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV44Request;
