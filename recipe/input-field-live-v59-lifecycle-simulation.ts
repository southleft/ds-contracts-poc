...
export interface InputLiveV59LifecycleSimulationReport {
  artifactVersion: "input-live-v59-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV59AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV59Lifecycle(): Promise<InputLiveV59LifecycleSimulationReport> {
  await buildInputLiveV59Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V59_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV59AntecedentIndex;
  const indexSha256 = inputLiveV59Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v59-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V59_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V59_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV59History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV59History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV59AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V59_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V59_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV59History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV59History(authorized, "pending");
    ...
    await buildInputLiveV59Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V59_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV59AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV59Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV59Lifecycle(), null, 2)}\n`,
  );
