...
import { COMBOBOX_LIVE_V4_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v4.js";
import { verifyComboboxLiveV4Authorization } from "./combobox-live-v4-authorization.js";
import { runComboboxLiveV4Preflight } from "./combobox-live-v4-preflight.js";
import { ComboboxLiveV4Orchestrator } from "./run-combobox-live-v4.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v4 requires exact authorization artifact ${COMBOBOX_LIVE_V4_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v4-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV4Authorization(root);
    runComboboxLiveV4Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV4Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV4Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
