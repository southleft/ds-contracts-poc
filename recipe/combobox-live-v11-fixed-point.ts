/**
 * Carried Combobox live v11 host verifier. Uses scene-readback-combobox-v11 so live
...
export const COMBOBOX_LIVE_V11_FIXED_VERIFIER_VERSION = "combobox-live-v11-fixed-point-v1";
export const COMBOBOX_LIVE_V11_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV11FixedThresholds {
...
export interface ComboboxLiveV11FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV11FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV11FixedSourceProbe {
...
export interface ComboboxLiveV11FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV11FixedFixedPoint;
}

export interface ComboboxLiveV11FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV11FixedHardGateInput {
  thresholds: ComboboxLiveV11FixedThresholds;
  materialRegression: ComboboxLiveV11FixedMaterialRegression;
  sourceProbes: ComboboxLiveV11FixedSourceProbe[];
  cells: ComboboxLiveV11FixedCellValidation[];
  sceneProofs: ComboboxLiveV11FixedSceneProof[];
  visualRows: ComboboxLiveV11FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV11FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V11_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV11FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV11FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV11FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV11FixedHardGates(
  input: ComboboxLiveV11FixedHardGateInput,
): ComboboxLiveV11FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV11FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV11FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V11_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV11FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV11FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV11FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
