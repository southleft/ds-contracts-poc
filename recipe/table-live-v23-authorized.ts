...
import { TABLE_LIVE_V23_AUTHORIZATION_PATH } from "./build-table-live-proof-v23.js";
import { verifyTableLiveV23Authorization } from "./table-live-v23-authorization.js";
import { runTableLiveV23Preflight } from "./table-live-v23-preflight.js";
import { TableLiveV23Orchestrator } from "./run-table-live-v23.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v23 requires exact authorization artifact ${TABLE_LIVE_V23_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v23-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV23Authorization(root);
    runTableLiveV23Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV23Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV23Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
