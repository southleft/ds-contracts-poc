/**
 * Table live v8 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V8_FIXED_VERIFIER_VERSION = "table-live-v8-fixed-point-v1";
export const TABLE_LIVE_V8_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV8FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV8FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV8FixedSourceProbe {
...
export interface TableLiveV8FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV8FixedFixedPoint;
}

export interface TableLiveV8FixedHardGateInput {
  thresholds: TableLiveV8FixedThresholds;
  sourceProbes: TableLiveV8FixedSourceProbe[];
  cells: TableLiveV8FixedCellValidation[];
  sceneProofs: TableLiveV8FixedSceneProof[];
...
export interface TableLiveV8FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V8_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV8FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV8FixedHardGates(
  input: TableLiveV8FixedHardGateInput,
): TableLiveV8FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV8FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  if (input.visualRows.length !== 0)
    failures.push("table live v8 must not invent a visual comparison");
  ...
  return {
    verifierVersion: TABLE_LIVE_V8_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV8FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV8FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
