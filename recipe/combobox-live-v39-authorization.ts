...
export type ComboboxLiveV39HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV39AuthorizationArtifact {
  artifactVersion: "combobox-live-v39-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V39_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV39AntecedentIndex {
...
export interface ComboboxLiveV39HistoryState {
  index?: ComboboxLiveV39AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV39AuthorizationArtifact;
...
export interface ComboboxLiveV39AuthorizationProof extends ComboboxLiveV39TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V39_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V39_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V39_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V39_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v39-authorization") ||
  artifactPath.includes("combobox-live-v39-preflight") ||
  artifactPath.includes("combobox-live-v39-authorized") ||
  artifactPath.includes("create-combobox-live-v39-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v39-status") ||
...
export function validateComboboxLiveV39AntecedentIndex(
  index: ComboboxLiveV39AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v39-antecedent-index-v1" ||
...
export function validateComboboxLiveV39UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV39History\(\s*["']/.test(source) ||
    /readComboboxLiveV39HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV39AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV39AuthorizationArtifact {
  ...
  ) as ComboboxLiveV39AntecedentIndex;
  const indexFailures = validateComboboxLiveV39AntecedentIndex(index);
...
export function validateComboboxLiveV39AuthorizationArtifact(
  artifact: ComboboxLiveV39AuthorizationArtifact | undefined,
  index: ComboboxLiveV39AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v39-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v39" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V39_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V39_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V39_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V39_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V39_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V39_SOURCE_ROOTS ||
  ...
  if (
    ...
    artifact?.execution?.v38AuthorizationReusable !== false ||
    artifact?.execution?.v39AuthorizationReusable !== false ||
...
export function validateComboboxLiveV39History(
  state: ComboboxLiveV39HistoryState,
  expected: ComboboxLiveV39HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV39AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV39AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV39HistoryState(
  root = process.cwd(),
): ComboboxLiveV39HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V39_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV39AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V39_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V39_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V39_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV39History(
  expected: ComboboxLiveV39HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV39HistoryState {
  const state = readComboboxLiveV39HistoryState(root);
  const failures = validateComboboxLiveV39History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v39 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV39Authorization(
  root = process.cwd(),
): ComboboxLiveV39AuthorizationProof {
  const state = verifyComboboxLiveV39History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V39_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v39/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v39.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V39_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V39_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V39_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV39HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV39History(expected);
  process.stdout.write(
    `Combobox live v39 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
