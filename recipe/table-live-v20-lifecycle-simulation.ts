...
import { tableLiveV2Sha256 } from "./table-live-v20-broker.js";

export interface TableLiveV20LifecycleSimulationReport {
  artifactVersion: "table-live-v20-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV20AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV20Lifecycle(): Promise<TableLiveV20LifecycleSimulationReport> {
  await buildTableLiveV20Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V20_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV20AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v20-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V20_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V20_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV20History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV20History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV20AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V20_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V20_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV20History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV20History(authorized, "pending");
    ...
    await buildTableLiveV20Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V20_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV20AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV20Lifecycle(), null, 2)}\n`,
  );
