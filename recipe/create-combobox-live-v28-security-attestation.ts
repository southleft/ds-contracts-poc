...
import { verifyComboboxLiveV28Authorization } from "./combobox-live-v28-authorization.js";
import { COMBOBOX_LIVE_V28_TARGET } from "./combobox-live-v28-broker.js";
import {
  COMBOBOX_LIVE_V28_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV28Authorization();
...
const attestation: ComboboxLiveV28SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV28SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v28 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V28_SECURITY_ATTESTATION_DEFAULT_PATH,
);
