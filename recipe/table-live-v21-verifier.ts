...
import {
  verifyTableLiveV21FixedSceneFixedPoint,
  type TableLiveV21FixedFixedPoint,
} from "./table-live-v21-fixed-point.js";

export const TABLE_LIVE_V21_VERIFIER_VERSION = "table-live-v21-verifier-v1";

export function buildTableLiveV21RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV21RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV21RawNode[];
}

export interface TableLiveV21NormalizedScene {
...
export interface TableLiveV21SceneProof {
...
export function normalizeTableLiveV21Scene(
  root: TableLiveV21RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV21NormalizedScene {
  const canonicalBindings: TableLiveV21NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV21RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV21Scene<Instance>(
  normalized: TableLiveV21NormalizedScene,
...
): TableLiveV21SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV21FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV21ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV21PreCaptureGates(payload: unknown): void {
