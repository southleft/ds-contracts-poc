...
import { CALENDAR_LIVE_V37_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v37.js";
import { verifyCalendarLiveV37Authorization } from "./calendar-live-v37-authorization.js";
import { runCalendarLiveV37Preflight } from "./calendar-live-v37-preflight.js";
import { CalendarLiveV37Orchestrator } from "./run-calendar-live-v37.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v37 requires exact authorization artifact ${CALENDAR_LIVE_V37_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v37-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV37Authorization(root);
    runCalendarLiveV37Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV37Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV37Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
