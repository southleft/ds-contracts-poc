/**
 * Carried Combobox live v10 host verifier. Uses scene-readback-combobox-v10 so live
...
export const COMBOBOX_LIVE_V10_FIXED_VERIFIER_VERSION = "combobox-live-v10-fixed-point-v1";
export const COMBOBOX_LIVE_V10_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV10FixedThresholds {
...
export interface ComboboxLiveV10FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV10FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV10FixedSourceProbe {
...
export interface ComboboxLiveV10FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV10FixedFixedPoint;
}

export interface ComboboxLiveV10FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV10FixedHardGateInput {
  thresholds: ComboboxLiveV10FixedThresholds;
  materialRegression: ComboboxLiveV10FixedMaterialRegression;
  sourceProbes: ComboboxLiveV10FixedSourceProbe[];
  cells: ComboboxLiveV10FixedCellValidation[];
  sceneProofs: ComboboxLiveV10FixedSceneProof[];
  visualRows: ComboboxLiveV10FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV10FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V10_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV10FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV10FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV10FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV10FixedHardGates(
  input: ComboboxLiveV10FixedHardGateInput,
): ComboboxLiveV10FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV10FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV10FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V10_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV10FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV10FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV10FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
