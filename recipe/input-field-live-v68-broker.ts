...
import {
  ...
  INPUT_LIVE_V68_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V68_RESPONSE_CONTRACTS,
  type InputLiveV68RootProof,
} from "./input-field-live-v68-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V68_BROKER_VERSION = "input-live-v68-broker-v2";
export const INPUT_LIVE_V68_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V68_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V68_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V68_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V68_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V68_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV68RemotePhase =
  (typeof INPUT_LIVE_V68_REMOTE_PHASES)[number];
export type InputLiveV68AuthorizationMode = "live" | "simulated";

export interface InputLiveV68CommitPins {
...
export interface InputLiveV68TransactionAuthorization extends InputLiveV68CommitPins {
  mode: InputLiveV68AuthorizationMode;
...
export interface InputLiveV68Manifest {
  artifactVersion: "input-live-v68-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V68_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V68_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V68_TARGET;
...
export interface InputLiveV68ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV68RequestBody {
  artifactVersion: "input-live-v68-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V68_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV68RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV68CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V68_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV68ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV68Request extends InputLiveV68RequestBody {
...
export interface InputLiveV68Receipt {
  artifactVersion: "input-live-v68-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V68_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV68RemotePhase;
...
export interface InputLiveV68AcceptedResponse<T = unknown> {
  request: InputLiveV68Request;
  receipt: InputLiveV68Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV68IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV68ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V68_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V68_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V68_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V68_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V68_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V68_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V68_TARGET.editorType;
};
...
export const inputLiveV68RequestSequence = (
  phase: InputLiveV68RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV68RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV68RequestPath = (
  directory: string,
  phase: InputLiveV68RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV68ResponsePath = (
  directory: string,
  phase: InputLiveV68RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV68ReceiptPath = (
  directory: string,
  phase: InputLiveV68RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV68Manifest => {
  const manifest = json<InputLiveV68Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v68-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V68_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V68_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV68RemotePhase,
  captureIndex?: number,
): InputLiveV68Request | undefined => {
  const file = inputLiveV68RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV68Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV68RemotePhase,
  captureIndex?: number,
): InputLiveV68Receipt | undefined => {
  const file = inputLiveV68ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV68Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV68Request | undefined;
  receipt: InputLiveV68Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV68RemotePhase,
): InputLiveV68ResponseContract => ({
  ...INPUT_LIVE_V68_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V68_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V68_DYNAMIC_TOOL,
      target: INPUT_LIVE_V68_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v68-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V68_BROKER_VERSION,
  target: INPUT_LIVE_V68_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V68_PROGRAM_START*/${program}/*INPUT_LIVE_V68_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V68_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V68_PROGRAM_END*/";
...
export function createInputLiveV68Transaction(
  directory: string,
  ...
  const manifest: InputLiveV68Manifest = {
    artifactVersion: "input-live-v68-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V68_BROKER_VERSION,
...
export function validateInputLiveV68Request(
  directory: string,
  request: InputLiveV68Request,
): void {
  ...
  const expectedSequence = inputLiveV68RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v68-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V68_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V68_TARGET.fileKey ||
...
export function issueInputLiveV68Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV68RemotePhase,
  program: string,
  options: InputLiveV68IssueOptions = {},
): InputLiveV68Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV68RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV68Request>(targetPath);
    validateInputLiveV68Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV68ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV68TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV68RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V68_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V68_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V68_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV68RequestBody = {
    artifactVersion: "input-live-v68-broker-request-v2",
    brokerVersion: INPUT_LIVE_V68_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV68Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV68Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV68RawResponse(
  directory: string,
  phase: InputLiveV68RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV68RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV68Request>(requestFile);
  if (
  ...
  const target = inputLiveV68ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV68Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v68-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V68_BROKER_VERSION,
...
export function acceptInputLiveV68Response<T = unknown>(
  directory: string,
  phase: InputLiveV68RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV68AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV68ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV68Request>(
    inputLiveV68RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV68Request(directory, request);
  if (phase === "capture") validateInputLiveV68TechnicalGates(directory);
  const rawFile = inputLiveV68ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v68-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V68_BROKER_VERSION ||
...
export function persistInputLiveV68TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV68RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV68RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V68_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v68-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V68_BROKER_VERSION,
...
export function validateInputLiveV68TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v68-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V68_TECHNICAL_GATES.some(
...
export function validateInputLiveV68Receipt(
  directory: string,
  phase: InputLiveV68RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV68Request(directory, request);
  const receipt = json<InputLiveV68Receipt>(
    inputLiveV68ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV68ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v68-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V68_BROKER_VERSION ||
...
export function readInputLiveV68AcceptedRawPayload(
  directory: string,
  phase: InputLiveV68RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV68Receipt(directory, phase, captureIndex);
...
export function createInputLiveV68FakeDynamicResponse(
  request: InputLiveV68Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V68_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V68_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV68PersistedRequests(
  directory: string,
): InputLiveV68Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV68Request>(file)] : [];
    });
}

export const inputLiveV68Sha256 = sha256;
