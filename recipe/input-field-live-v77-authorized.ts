...
import { INPUT_LIVE_V77_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v77.js";
import { verifyInputLiveV77Authorization } from "./input-field-live-v77-authorization.js";
import { runInputLiveV77Preflight } from "./input-field-live-v77-preflight.js";
import { InputLiveV77Orchestrator } from "./run-input-field-live-v77.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V77_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v77-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV77Authorization(root);
    runInputLiveV77Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV77Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
