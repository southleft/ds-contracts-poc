...
import {
  acceptComboboxLiveV34Response,
  createComboboxLiveV34Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV34Request,
  persistComboboxLiveV34RawResponse,
  persistComboboxLiveV34TechnicalGates,
  readComboboxLiveV34AcceptedRawPayload,
  type ComboboxLiveV34Request,
  type ComboboxLiveV34TransactionAuthorization,
} from "./combobox-live-v34-broker.js";
import {
  COMBOBOX_LIVE_V34_CAPTURE_COUNT,
  assertComboboxLiveV34CaptureResponses,
  ...
  evaluateComboboxLiveV34Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV34Roots,
  validateComboboxLiveV34CaptureManifest,
  validateComboboxLiveV34CapturePayload,
  validateComboboxLiveV34CleanupPayload,
  validateComboboxLiveV34ExtractPayload,
  validateComboboxLiveV34ProbePayload,
  validateComboboxLiveV34WriterPayload,
  type ComboboxLiveV34CaptureCell,
  type ComboboxLiveV34CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V34_PLAN_PATH,
  type ComboboxLiveV34ProofPlan,
} from "./build-combobox-live-proof-v34.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v34.js";

export const COMBOBOX_LIVE_V34_RUNNER_VERSION = "combobox-live-v34-runner-v1";

interface ComboboxLiveV34JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV34OrchestratorState {
  artifactVersion: "combobox-live-v34-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV34JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV34ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV34CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV34SourceIdentity & {
...
export interface ComboboxLiveV34NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV34OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v34 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V34_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V34_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV34ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV34CaptureCell[];
  validateComboboxLiveV34CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v34 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v34 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV34WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV34RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV34Request =>
  readJson<ComboboxLiveV34Request>(file);

export class ComboboxLiveV34Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV34Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV34OrchestratorState = {
      artifactVersion: "combobox-live-v34-orchestrator-state-v1",
    ...
    return new ComboboxLiveV34Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV34OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV34Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV34NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV34NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v34 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV34NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v34 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV34RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV34Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV34CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV34AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV34WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV34Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV34ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV34Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV34Response(this.#directory, "writer", {
        validate: validateComboboxLiveV34WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV34Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV34RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV34Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV34ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV34Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV34RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v34-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV34Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV34ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV34Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV34ProbePayload(payload, ownership),
      });
      persistComboboxLiveV34TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v34 capture cell missing");
      const accepted = acceptComboboxLiveV34Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV34CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V34_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV34CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV34Objective(
    ...
    throw new Error(`Combobox live v34 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV34Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV34CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV34Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV34RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV34Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV34CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV34CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV34CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV34Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV34TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV34SimulationKey = (): KeyObject =>
