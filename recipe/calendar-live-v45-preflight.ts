...
import {
  validateCalendarLiveV45AntecedentIndex,
  verifyCalendarLiveV45Authorization,
  type CalendarLiveV45AntecedentIndex,
  type CalendarLiveV45AuthorizationProof,
} from "./calendar-live-v45-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V45_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v45/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V45_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v45-security-attestation.json";

export interface CalendarLiveV45SecurityAttestation {
  artifactVersion: "calendar-live-v45-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV45PreflightReport {
  artifactVersion: "calendar-live-v45-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV45SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV45SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV45AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v45-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V45_TARGET) ||
...
export function validateCalendarLiveV45ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV45Authorization");
  const preflight = source.indexOf("runCalendarLiveV45Preflight");
  const initialize = source.indexOf("CalendarLiveV45Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV45Preflight(
  root: string,
  proof: CalendarLiveV45AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V45_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV45PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V45_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V45_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV45AntecedentIndex;
  failures.push(...validateCalendarLiveV45AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V45_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V45_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV45SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v45-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV45ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v45 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV45SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v45-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V45_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV45Preflight(
    process.cwd(),
    verifyCalendarLiveV45Authorization(),
    attestationPath,
