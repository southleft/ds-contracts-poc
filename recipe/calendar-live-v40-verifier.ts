...
import {
  verifyCalendarLiveV40FixedSceneFixedPoint,
  type CalendarLiveV40FixedFixedPoint,
} from "./calendar-live-v40-fixed-point.js";

export const CALENDAR_LIVE_V40_VERIFIER_VERSION = "calendar-live-v40-verifier-v1";

export function buildCalendarLiveV40RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV40RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV40RawNode[];
}

export interface CalendarLiveV40NormalizedScene {
...
export interface CalendarLiveV40SceneProof {
...
export function normalizeCalendarLiveV40Scene(
  root: CalendarLiveV40RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV40NormalizedScene {
  const canonicalBindings: CalendarLiveV40NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV40RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV40Scene<Instance>(
  normalized: CalendarLiveV40NormalizedScene,
...
): CalendarLiveV40SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV40FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV40ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV40PreCaptureGates(payload: unknown): void {
