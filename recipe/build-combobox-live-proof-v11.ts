...
import {
  ...
  COMBOBOX_LIVE_V11_HOST_PHASES,
  COMBOBOX_LIVE_V11_REMOTE_REQUESTS,
  COMBOBOX_LIVE_V11_SOURCE_ROOTS,
  COMBOBOX_LIVE_V11_VARIANT_COUNT,
  buildComboboxLiveV11CaptureProgram,
  buildComboboxLiveV11CleanupProgram,
  buildComboboxLiveV11ExtractProgram,
  buildComboboxLiveV11ProbeProgram,
  buildComboboxLiveV11RestoreProgram,
  comboboxLiveV5CaptureManifestSha256,
  type ComboboxLiveV11CaptureCell,
  type ComboboxLiveV11SourceIdentity,
  type ComboboxLiveV11WriterOwnership,
} from "./combobox-live-v11-contract.js";
import {
  COMBOBOX_LIVE_V11_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V11_TARGET,
  comboboxLiveV5RequestSequence,
} from "./combobox-live-v11-broker.js";
import {
...
export const COMBOBOX_LIVE_V11_EVIDENCE_ROOT =
  "recipe/evidence/combobox-live-pivot-v11";
export const COMBOBOX_LIVE_V11_PROTOCOL_PATH = `${COMBOBOX_LIVE_V11_EVIDENCE_ROOT}/protocol.json`;
export const COMBOBOX_LIVE_V11_PLAN_PATH = `${COMBOBOX_LIVE_V11_EVIDENCE_ROOT}/proof-plan.json`;
export const COMBOBOX_LIVE_V11_CAPTURE_MANIFEST_PATH = `${COMBOBOX_LIVE_V11_EVIDENCE_ROOT}/capture-manifest.json`;
export const COMBOBOX_LIVE_V11_REQUEST_MANIFEST_PATH = `${COMBOBOX_LIVE_V11_EVIDENCE_ROOT}/request-manifest.json`;
export const COMBOBOX_LIVE_V11_INDEX_PATH = `${COMBOBOX_LIVE_V11_EVIDENCE_ROOT}/antecedent-index.json`;
export const COMBOBOX_LIVE_V11_AUTHORIZATION_TEMPLATE_PATH = `${COMBOBOX_LIVE_V11_EVIDENCE_ROOT}/authorization-template.json`;
export const COMBOBOX_LIVE_V11_AUTHORIZATION_PATH = `${COMBOBOX_LIVE_V11_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface ComboboxLiveV11ProofPlan {
  artifactVersion: "combobox-live-v11-proof-plan-v1";
...
const captureCells = (
...
): ComboboxLiveV11CaptureCell[] => {
  const cells: ComboboxLiveV11CaptureCell[] = [];
  ...
  if (cells.length !== COMBOBOX_LIVE_V11_CAPTURE_COUNT)
    throw new Error(
      `Combobox live v11 capture cells ${cells.length}, expected ${COMBOBOX_LIVE_V11_CAPTURE_COUNT}`,
    );
...
const protocol = (expectedFacts: number) => ({
  artifactVersion: "combobox-live-v11-external-operator-protocol-v1",
  protocolId: "combobox-live-v11",
  status:
  ...
  denominator: {
    sourceRoots: COMBOBOX_LIVE_V11_SOURCE_ROOTS,
    sets: 4,
    variants: COMBOBOX_LIVE_V11_VARIANT_COUNT,
    captureCells: COMBOBOX_LIVE_V11_CAPTURE_COUNT,
    remoteRequests: COMBOBOX_LIVE_V11_REMOTE_REQUESTS,
    hostPhases: COMBOBOX_LIVE_V11_HOST_PHASES,
...
export async function buildComboboxLiveV11Proof(
  check = process.argv.includes("--check"),
): Promise<ComboboxLiveV11ProofPlan> {
  ...
  if (
    ...
    throw new Error("Combobox live v11 writer reused Input identity");
  const captures = captureCells(sources);
  const writerProgram = Buffer.from(writer.code);
  const writerOwnershipBlueprint: ComboboxLiveV11WriterOwnership = {
  ...
  const identities: ComboboxLiveV11SourceIdentity[] = sources.map((source) => ({
    source: source.source,
    adapterIdentity: source.adapterIdentity,
    recipeHash: source.recipeHash,
    envelopeHash: source.envelope.integrity.canonicalHash,
  ...
  const restoreBlueprint = Buffer.from(
    buildComboboxLiveV11RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildComboboxLiveV11ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildComboboxLiveV11ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildComboboxLiveV11CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );
  const cleanupBlueprint = Buffer.from(
    buildComboboxLiveV11CleanupProgram(writerOwnershipBlueprint),
  );
  ...
  const sourceMetadata = sources.map((source) => {
    ...
    const comboboxPath = `${COMBOBOX_LIVE_V11_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-combobox.json.gz`;
    const optionPath = `${COMBOBOX_LIVE_V11_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-option.json.gz`;
  ...
  antecedentOutputs.set(
    COMBOBOX_LIVE_V11_PROTOCOL_PATH,
    jsonBytes(protocol(expectedFacts)),
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V11_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V11_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V11_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V11_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V11_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V11_EVIDENCE_ROOT}/programs/cleanup-blueprint.js`,
    cleanupBlueprint,
  );

  const captureManifest = {
    artifactVersion: "combobox-live-v11-capture-manifest-v1",
  ...
  antecedentOutputs.set(
    COMBOBOX_LIVE_V11_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: ComboboxLiveV11ProofPlan = {
    artifactVersion: "combobox-live-v11-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: COMBOBOX_LIVE_V11_TARGET,
    namespace: COMBOBOX_FIGMA_NAMESPACE,
    writer: {
      programPath: `${COMBOBOX_LIVE_V11_EVIDENCE_ROOT}/programs/writer.txt`,
    ...
    captureManifest: {
      path: COMBOBOX_LIVE_V11_CAPTURE_MANIFEST_PATH,
      cells: 72,
      requests: 72,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(COMBOBOX_LIVE_V11_PLAN_PATH, jsonBytes(proofPlan));

  const requests = {
    artifactVersion: "combobox-live-v11-request-manifest-v1",
    expectedDynamicTool: COMBOBOX_LIVE_V11_DYNAMIC_TOOL,
    target: COMBOBOX_LIVE_V11_TARGET,
  ...
  if (requests.requestCount !== COMBOBOX_LIVE_V11_REMOTE_REQUESTS)
    throw new Error("Combobox live v11 remote request denominator drifted");
  antecedentOutputs.set(
    COMBOBOX_LIVE_V11_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const lifecycleExcludedPaths = [
    COMBOBOX_LIVE_V11_AUTHORIZATION_TEMPLATE_PATH,
    COMBOBOX_LIVE_V11_AUTHORIZATION_PATH,
    "recipe/combobox-live-v11-authorization.ts",
    "recipe/combobox-live-v11-authorization.test.ts",
    "recipe/combobox-live-v11-preflight.ts",
    "recipe/combobox-live-v11-authorized.ts",
    "recipe/write-combobox-live-v11-authorization.ts",
    "recipe/create-combobox-live-v11-security-attestation.ts",
    "recipe/evidence/combobox-live-pivot-v11/operator-security-attestation-template.json",
    "recipe/evidence/combobox-live-pivot-v11-status.json",
    "recipe/evidence/status-index.json",
  ];
  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/combobox-live-v11-broker.ts",
    "recipe/combobox-live-v11-contract.ts",
    "recipe/combobox-live-v11-restore.ts",
    "recipe/combobox-live-v11-cleanup.ts",
    "recipe/combobox-live-v11-verifier.ts",
    "recipe/combobox-live-v11-fixed-point.ts",
    "recipe/run-combobox-live-v11.ts",
    "recipe/build-combobox-live-proof-v11.ts",
    "recipe/combobox-live-v11-lifecycle-simulation.ts",
    "recipe/scene-readback-combobox-v11.ts",
    "recipe/scene-readback-runtime-combobox-v11.ts",
    "recipe/combobox-figma-writer.ts",
    "recipe/figma-property-normalizer-v8.ts",
    "recipe/figma-runtime-portability.ts",
    "recipe/normalize.ts",
  ].filter((artifactPath) => artifactPath !== COMBOBOX_LIVE_V11_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "combobox-live-v11-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
  ...
  antecedentOutputs.set(COMBOBOX_LIVE_V11_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "combobox-live-v11-authorization-template-v1",
    authorizationId: "combobox-live-v11",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: COMBOBOX_LIVE_V11_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
    ...
    denominator: {
      remoteRequests: COMBOBOX_LIVE_V11_REMOTE_REQUESTS,
      hostPhases: COMBOBOX_LIVE_V11_HOST_PHASES,
      captures: COMBOBOX_LIVE_V11_CAPTURE_COUNT,
      sourceRoots: COMBOBOX_LIVE_V11_SOURCE_ROOTS,
      expectedFacts,
    },
    execution: {
  ...
  const attestationTemplate = jsonBytes({
    artifactVersion: "combobox-live-v11-operator-security-attestation-template-v1",
    status: "template only; complete privately after PREPARE+AUTHORIZE",
    tokenValuesForbidden: true,
    privateDirectory: "private/combobox-live-v11-security-attestation.json",
  });
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Combobox live v11 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(COMBOBOX_LIVE_V11_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(COMBOBOX_LIVE_V11_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
    writeFileSync(
      `${COMBOBOX_LIVE_V11_EVIDENCE_ROOT}/operator-security-attestation-template.json`,
      attestationTemplate,
    );
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(COMBOBOX_LIVE_V11_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(COMBOBOX_LIVE_V11_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Combobox live v11 authorization template drift");
  }
  if (
    writer.namespace !== COMBOBOX_FIGMA_NAMESPACE ||
    writer.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    ...
    throw new Error("Combobox live v11 identity or cleanup invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildComboboxLiveV11Proof(), null, 2)}\n`,
  );
