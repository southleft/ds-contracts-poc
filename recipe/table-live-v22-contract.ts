...
import { buildTableLiveV22CleanupRuntime } from "./table-live-v22-cleanup.js";
import { type TableLiveV22FixedFixedPoint } from "./table-live-v22-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV22RestoreProgram,
  validateTableLiveV22RestorePayload,
  type TableLiveV22RestorePayload,
} from "./table-live-v22-restore.js";
import {
  buildTableLiveV22RawPropertyRuntime,
  normalizeTableLiveV22Scene,
  type TableLiveV22RawNode,
} from "./table-live-v22-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v22.js";
import {
...
export const TABLE_LIVE_V22_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V22_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V22_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V22_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V22_VARIANT_COUNT = 20;
export const TABLE_LIVE_V22_SET_COUNT = 6;
export const TABLE_LIVE_V22_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V22_HOST_PHASES = 3;
export const TABLE_LIVE_V22_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V22_RESTORE_COUNT = 8;
export const TABLE_LIVE_V22_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V22_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV22SourceId = (typeof TABLE_LIVE_V22_SOURCE_IDS)[number];
export type TableLiveV22Kind = "table" | "row" | "cell";

export interface TableLiveV22SourceIdentity {
  source: TableLiveV22SourceId;
...
export interface TableLiveV22WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV22ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV22SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV22RawNode;
    rowScene: TableLiveV22RawNode;
    cellScene: TableLiveV22RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV22RootProof {
  source: TableLiveV22SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV22FixedFixedPoint;
}

export interface TableLiveV22ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV22SourceId;
    adapterIdentity: string;
...
export interface TableLiveV22CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV22SourceId;
  adapterIdentity: string;
...
export interface TableLiveV22CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV22SourceId;
  frameWidth: number;
...
export interface TableLiveV22CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v22 ${label} must be an object`);
...
export function validateTableLiveV22WriterPayload(
  value: unknown,
): TableLiveV22WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V22_NAMESPACE ||
    ...
    throw new TypeError("Table live v22 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v22 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v22 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV22SourceIdentity,
  kind: TableLiveV22Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV22ExtractProgram(
  writer: TableLiveV22WriterOwnership,
  sources: readonly TableLiveV22SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v22 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V22_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V22_NAMESPACE)}
${buildTableLiveV22RawPropertyRuntime()}
...
export function validateTableLiveV22ExtractPayload(
  value: unknown,
  writer: TableLiveV22WriterOwnership,
): TableLiveV22ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v22 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V22_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v22 extract schema/two-root mismatch");
  return payload as TableLiveV22ExtractPayload;
}

export function proveTableLiveV22Roots<Instance>(
  extract: TableLiveV22ExtractPayload,
  sources: readonly (TableLiveV22SourceIdentity & {
...
): TableLiveV22RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v22 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v22 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV22Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV22Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV22Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV22FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v22 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v22 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v22 compile lost owned sets");
...
export function assertTableLiveV22RootProofs(
  proofs: readonly TableLiveV22RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v22 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v22 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV22ProbeProgram(
  writer: TableLiveV22WriterOwnership,
  sources: readonly TableLiveV22SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V22_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV22ProbePayload(
  value: unknown,
  writer: TableLiveV22WriterOwnership,
): TableLiveV22ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV22ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V22_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v22 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV22CaptureManifest(
  cells: readonly TableLiveV22CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V22_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V22_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V22_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V22_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v22 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV22CaptureCell[],
): string => {
  validateTableLiveV22CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV22CaptureProgram(
  writer: TableLiveV22WriterOwnership,
  cell: TableLiveV22CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V22_CAPTURE_COUNT
  )
    throw new TypeError("Table live v22 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v22 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V22_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV22CapturePayload(
  value: unknown,
  cell: TableLiveV22CaptureCell,
  rawResponseBytes: number,
): TableLiveV22CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV22CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V22_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V22_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v22 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV22CaptureResponses(
  manifest: readonly TableLiveV22CaptureCell[],
  responses: readonly TableLiveV22CapturePayload[],
): void {
  validateTableLiveV22CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v22 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV22ObjectiveReport {
  artifactVersion: "table-live-v22-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV22Objective(
  manifest: readonly TableLiveV22CaptureCell[],
  responses: readonly TableLiveV22CapturePayload[],
): TableLiveV22ObjectiveReport {
  assertTableLiveV22CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v22-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV22CleanupProgram(
  writer: TableLiveV22WriterOwnership,
): string {
  const baseProgram = buildTableLiveV22CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V22_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v22 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV22CleanupPayload(
  payload: unknown,
  writer: TableLiveV22WriterOwnership,
): TableLiveV22CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV22CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v22 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V22_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV22WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
