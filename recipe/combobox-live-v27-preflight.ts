...
import {
  validateComboboxLiveV27AntecedentIndex,
  verifyComboboxLiveV27Authorization,
  type ComboboxLiveV27AntecedentIndex,
  type ComboboxLiveV27AuthorizationProof,
} from "./combobox-live-v27-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V27_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v27/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V27_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v27-security-attestation.json";

export interface ComboboxLiveV27SecurityAttestation {
  artifactVersion: "combobox-live-v27-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV27PreflightReport {
  artifactVersion: "combobox-live-v27-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV27SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV27SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV27AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v27-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V27_TARGET) ||
...
export function validateComboboxLiveV27ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV27Authorization");
  const preflight = source.indexOf("runComboboxLiveV27Preflight");
  const initialize = source.indexOf("ComboboxLiveV27Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV27Preflight(
  root: string,
  proof: ComboboxLiveV27AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V27_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV27PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V27_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V27_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V27_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV27AntecedentIndex;
  failures.push(...validateComboboxLiveV27AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V27_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V27_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV27SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v27-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV27ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v27 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV27SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v27-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V27_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV27Preflight(
    process.cwd(),
    verifyComboboxLiveV27Authorization(),
    attestationPath,
