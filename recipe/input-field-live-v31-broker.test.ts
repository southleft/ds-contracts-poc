...
import {
  ...
  createInputLiveV31FakeDynamicResponse,
  ...
  listInputLiveV31PersistedRequests,
  persistInputLiveV31RawResponse,
  persistInputLiveV31TechnicalGates,
  validateInputLiveV31Receipt,
  validateInputLiveV31Request,
  type InputLiveV31RemotePhase,
...
import {
  ...
  type InputLiveV31ExtractPayload,
  type InputLiveV31ProbePayload,
  type InputLiveV31RestorePayload,
  type InputLiveV31RootProof,
  type InputLiveV31WriterOwnership,
} from "./input-field-live-v31-contract.js";
import {
  InputLiveV31Orchestrator,
  simulatedInputLiveV31Authorization,
} from "./run-input-field-live-v31.js";
...
const captures: InputLiveV31CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV31Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v31-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV31Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV31CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV31FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV31FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V31_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V31_TARGET);
  persistInputLiveV31RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV31Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV31Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV31WriterOwnership,
): InputLiveV31RestorePayload => ({
...
const rootProofs = (): InputLiveV31RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV31CaptureCell,
): InputLiveV31CapturePayload => {
  const bytes = Buffer.from(`input-live-v31-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV31Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV31Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV31RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV31RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV31ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV31ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV31CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV31CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV31CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV31Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV31CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV31PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV31ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV31Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV31Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV31Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV31Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V31_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV31RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V31_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV31Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV31Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV31ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV31Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV31WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV31RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV31Request;
    validateInputLiveV31Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV31Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV31CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v31-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV31Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV31Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV31Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V31_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV31RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V31_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV31Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV31Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV31RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V31_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV31Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV31Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV31Request;
