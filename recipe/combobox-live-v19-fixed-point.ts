/**
 * Carried Combobox live v19 host verifier. Uses scene-readback-combobox-v19 so live
...
export const COMBOBOX_LIVE_V19_FIXED_VERIFIER_VERSION = "combobox-live-v19-fixed-point-v1";
export const COMBOBOX_LIVE_V19_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV19FixedThresholds {
...
export interface ComboboxLiveV19FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV19FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV19FixedSourceProbe {
...
export interface ComboboxLiveV19FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV19FixedFixedPoint;
}

export interface ComboboxLiveV19FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV19FixedHardGateInput {
  thresholds: ComboboxLiveV19FixedThresholds;
  materialRegression: ComboboxLiveV19FixedMaterialRegression;
  sourceProbes: ComboboxLiveV19FixedSourceProbe[];
  cells: ComboboxLiveV19FixedCellValidation[];
  sceneProofs: ComboboxLiveV19FixedSceneProof[];
  visualRows: ComboboxLiveV19FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV19FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V19_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV19FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV19FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV19FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV19FixedHardGates(
  input: ComboboxLiveV19FixedHardGateInput,
): ComboboxLiveV19FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV19FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV19FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V19_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV19FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV19FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV19FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
