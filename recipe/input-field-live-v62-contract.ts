...
} from "./input-field-live-v3-verifier-v62.js";
import {
  buildInputLiveV62RawPropertyRuntime,
  normalizeInputLiveV62Scene,
  type InputLiveV62RawNode,
} from "./input-field-live-v62-verifier.js";
...
export const INPUT_LIVE_V62_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V62_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V62_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V62_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V62_VARIANT_COUNT = 256;
export const INPUT_LIVE_V62_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V62_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV62SourceId = (typeof INPUT_LIVE_V62_SOURCE_IDS)[number];

export interface InputLiveV62SourceIdentity {
  source: InputLiveV62SourceId;
...
export interface InputLiveV62WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV62ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV62SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV62RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV62RootProof {
  source: InputLiveV62SourceId;
  adapterIdentity: string;
...
export interface InputLiveV62ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV62SourceId;
    adapterIdentity: string;
...
export interface InputLiveV62CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV62SourceId;
  adapterIdentity: string;
...
export interface InputLiveV62CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV62SourceId;
  strata: InputLiveV62CaptureCell["strata"];
...
export interface InputLiveV62CleanupPayload {
...
export function validateInputLiveV62WriterPayload(
  value: unknown,
): InputLiveV62WriterOwnership {
...
const identityPlan = (source: InputLiveV62SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV62ExtractProgram(
  writer: InputLiveV62WriterOwnership,
  sources: readonly InputLiveV62SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V62_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V62_NAMESPACE)}
${buildInputLiveV62RawPropertyRuntime()}
...
export function validateInputLiveV62ExtractPayload(
  value: unknown,
  writer: InputLiveV62WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V62_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV62ExtractPayload;
}

export function proveInputLiveV62Roots<Instance>(
  extract: InputLiveV62ExtractPayload,
  sources: readonly (InputLiveV62SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV62Scene(
      root.scene,
...
export function assertInputLiveV62RootProofs(
  proofs: readonly InputLiveV62RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV62ProbeProgram(
  writer: InputLiveV62WriterOwnership,
  sources: readonly InputLiveV62SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V62_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV62ProbePayload(
  value: unknown,
  writer: InputLiveV62WriterOwnership,
): InputLiveV62ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV62ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V62_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V62_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV62CaptureManifest(
  cells: readonly InputLiveV62CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V62_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V62_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V62_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V62_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V62_SOURCE_IDS.some(
...
export const inputLiveV62CaptureManifestSha256 = (
  cells: readonly InputLiveV62CaptureCell[],
): string => {
  validateInputLiveV62CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV62CaptureProgram(
  writer: InputLiveV62WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V62_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V62_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V62_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV62CapturePayload(
  value: unknown,
  cell: InputLiveV62CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV62CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V62_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V62_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV62CaptureResponses(
  manifest: readonly InputLiveV62CaptureCell[],
  responses: readonly InputLiveV62CapturePayload[],
): void {
  validateInputLiveV62CaptureManifest(manifest);
  if (
...
export interface InputLiveV62ObjectiveReport {
  artifactVersion: "input-live-v62-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV62Objective(
  manifest: readonly InputLiveV62CaptureCell[],
  responses: readonly InputLiveV62CapturePayload[],
  readReference: (cell: InputLiveV62CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV62CaptureCell) => Buffer,
): InputLiveV62ObjectiveReport {
  assertInputLiveV62CaptureResponses(manifest, responses);
...
export function buildInputLiveV62CleanupProgram(
  writer: InputLiveV62WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V62_NAMESPACE,
...
export function validateInputLiveV62CleanupPayload(
  value: unknown,
  writer: InputLiveV62WriterOwnership,
): InputLiveV62CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV62CleanupPayload;
...
export const INPUT_LIVE_V62_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV62WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
