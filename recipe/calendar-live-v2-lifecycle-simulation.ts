...
import { calendarLiveV1Sha256 } from "./calendar-live-v2-broker.js";

export interface CalendarLiveV2LifecycleSimulationReport {
  artifactVersion: "calendar-live-v2-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV2AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV2Lifecycle(): Promise<CalendarLiveV2LifecycleSimulationReport> {
  await buildCalendarLiveV2Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V2_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV2AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v2-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V2_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V2_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV2History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV2History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV2AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V2_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V2_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV2History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV2History(authorized, "pending");
    ...
    await buildCalendarLiveV2Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V2_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV2AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV2Lifecycle(), null, 2)}\n`,
  );
