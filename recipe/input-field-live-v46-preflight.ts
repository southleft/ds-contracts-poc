...
import {
  ...
  verifyInputLiveV46Authorization,
  type InputLiveV46AntecedentIndex,
  type InputLiveV46AuthorizationProof,
} from "./input-field-live-v46-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V46_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v46/operator-security-attestation-template.json";
export const INPUT_LIVE_V46_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v46-security-attestation.json";

export interface InputLiveV46SecurityAttestation {
...
export interface InputLiveV46PreflightReport {
  artifactVersion: "input-live-v46-preflight-v1";
...
export const inputLiveV46AttestationSha256 = (
  value: Omit<InputLiveV46SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV46SecurityAttestation(
  value: unknown,
  proof: InputLiveV46AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v46-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V46_TARGET) ||
...
export function validateInputLiveV46ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV46Authorization");
  const preflight = source.indexOf("runInputLiveV46Preflight");
  const initialize = source.indexOf("InputLiveV46Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV46Preflight(
  root: string,
  proof: InputLiveV46AuthorizationProof,
  attestationPath = INPUT_LIVE_V46_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV46PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V46_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V46_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V46_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV46AntecedentIndex;
  failures.push(...validateInputLiveV46AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V46_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V46_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v46-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV46ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV46SecurityAttestation;
  return {
    artifactVersion: "input-live-v46-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V46_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV46Preflight(
    process.cwd(),
    verifyInputLiveV46Authorization(),
    attestationPath,
