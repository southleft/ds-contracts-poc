...
import { calendarLiveV1Sha256 } from "./calendar-live-v38-broker.js";

export interface CalendarLiveV38LifecycleSimulationReport {
  artifactVersion: "calendar-live-v38-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV38AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV38Lifecycle(): Promise<CalendarLiveV38LifecycleSimulationReport> {
  await buildCalendarLiveV38Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V38_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV38AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v38-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V38_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V38_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV38History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV38History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV38AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V38_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V38_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV38History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV38History(authorized, "pending");
    ...
    await buildCalendarLiveV38Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V38_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV38AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV38Lifecycle(), null, 2)}\n`,
  );
