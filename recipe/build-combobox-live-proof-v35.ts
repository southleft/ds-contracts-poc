...
import {
  ...
  COMBOBOX_LIVE_V35_HOST_PHASES,
  COMBOBOX_LIVE_V35_REMOTE_REQUESTS,
  COMBOBOX_LIVE_V35_SOURCE_ROOTS,
  COMBOBOX_LIVE_V35_VARIANT_COUNT,
  buildComboboxLiveV35CaptureProgram,
  buildComboboxLiveV35CleanupProgram,
  buildComboboxLiveV35ExtractProgram,
  buildComboboxLiveV35ProbeProgram,
  buildComboboxLiveV35RestoreProgram,
  comboboxLiveV5CaptureManifestSha256,
  type ComboboxLiveV35CaptureCell,
  type ComboboxLiveV35SourceIdentity,
  type ComboboxLiveV35WriterOwnership,
} from "./combobox-live-v35-contract.js";
import {
  COMBOBOX_LIVE_V35_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V35_TARGET,
  comboboxLiveV5RequestSequence,
} from "./combobox-live-v35-broker.js";
import {
...
export const COMBOBOX_LIVE_V35_EVIDENCE_ROOT =
  "recipe/evidence/combobox-live-pivot-v35";
export const COMBOBOX_LIVE_V35_PROTOCOL_PATH = `${COMBOBOX_LIVE_V35_EVIDENCE_ROOT}/protocol.json`;
export const COMBOBOX_LIVE_V35_PLAN_PATH = `${COMBOBOX_LIVE_V35_EVIDENCE_ROOT}/proof-plan.json`;
export const COMBOBOX_LIVE_V35_CAPTURE_MANIFEST_PATH = `${COMBOBOX_LIVE_V35_EVIDENCE_ROOT}/capture-manifest.json`;
export const COMBOBOX_LIVE_V35_REQUEST_MANIFEST_PATH = `${COMBOBOX_LIVE_V35_EVIDENCE_ROOT}/request-manifest.json`;
export const COMBOBOX_LIVE_V35_INDEX_PATH = `${COMBOBOX_LIVE_V35_EVIDENCE_ROOT}/antecedent-index.json`;
export const COMBOBOX_LIVE_V35_AUTHORIZATION_TEMPLATE_PATH = `${COMBOBOX_LIVE_V35_EVIDENCE_ROOT}/authorization-template.json`;
export const COMBOBOX_LIVE_V35_AUTHORIZATION_PATH = `${COMBOBOX_LIVE_V35_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface ComboboxLiveV35ProofPlan {
  artifactVersion: "combobox-live-v35-proof-plan-v1";
...
const captureCells = (
...
): ComboboxLiveV35CaptureCell[] => {
  const cells: ComboboxLiveV35CaptureCell[] = [];
  ...
  if (cells.length !== COMBOBOX_LIVE_V35_CAPTURE_COUNT)
    throw new Error(
      `Combobox live v35 capture cells ${cells.length}, expected ${COMBOBOX_LIVE_V35_CAPTURE_COUNT}`,
    );
...
const protocol = (expectedFacts: number) => ({
  artifactVersion: "combobox-live-v35-external-operator-protocol-v1",
  protocolId: "combobox-live-v35",
  status:
  ...
  denominator: {
    sourceRoots: COMBOBOX_LIVE_V35_SOURCE_ROOTS,
    sets: 4,
    variants: COMBOBOX_LIVE_V35_VARIANT_COUNT,
    captureCells: COMBOBOX_LIVE_V35_CAPTURE_COUNT,
    remoteRequests: COMBOBOX_LIVE_V35_REMOTE_REQUESTS,
    hostPhases: COMBOBOX_LIVE_V35_HOST_PHASES,
...
export async function buildComboboxLiveV35Proof(
  check = process.argv.includes("--check"),
): Promise<ComboboxLiveV35ProofPlan> {
  ...
  if (
    ...
    throw new Error("Combobox live v35 writer reused Input identity");
  const captures = captureCells(sources);
  const writerProgram = Buffer.from(writer.code);
  const writerOwnershipBlueprint: ComboboxLiveV35WriterOwnership = {
  ...
  const identities: ComboboxLiveV35SourceIdentity[] = sources.map((source) => ({
    source: source.source,
    adapterIdentity: source.adapterIdentity,
    recipeHash: source.recipeHash,
    envelopeHash: source.envelope.integrity.canonicalHash,
  ...
  const restoreBlueprint = Buffer.from(
    buildComboboxLiveV35RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildComboboxLiveV35ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildComboboxLiveV35ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildComboboxLiveV35CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );
  const cleanupBlueprint = Buffer.from(
    buildComboboxLiveV35CleanupProgram(writerOwnershipBlueprint),
  );
  ...
  const sourceMetadata = sources.map((source) => {
    ...
    const comboboxPath = `${COMBOBOX_LIVE_V35_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-combobox.json.gz`;
    const optionPath = `${COMBOBOX_LIVE_V35_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-option.json.gz`;
  ...
  antecedentOutputs.set(
    COMBOBOX_LIVE_V35_PROTOCOL_PATH,
    jsonBytes(protocol(expectedFacts)),
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V35_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V35_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V35_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V35_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V35_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V35_EVIDENCE_ROOT}/programs/cleanup-blueprint.js`,
    cleanupBlueprint,
  );

  const captureManifest = {
    artifactVersion: "combobox-live-v35-capture-manifest-v1",
  ...
  antecedentOutputs.set(
    COMBOBOX_LIVE_V35_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: ComboboxLiveV35ProofPlan = {
    artifactVersion: "combobox-live-v35-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: COMBOBOX_LIVE_V35_TARGET,
    namespace: COMBOBOX_FIGMA_NAMESPACE,
    writer: {
      programPath: `${COMBOBOX_LIVE_V35_EVIDENCE_ROOT}/programs/writer.txt`,
    ...
    captureManifest: {
      path: COMBOBOX_LIVE_V35_CAPTURE_MANIFEST_PATH,
      cells: 72,
      requests: 72,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(COMBOBOX_LIVE_V35_PLAN_PATH, jsonBytes(proofPlan));

  const requests = {
    artifactVersion: "combobox-live-v35-request-manifest-v1",
    expectedDynamicTool: COMBOBOX_LIVE_V35_DYNAMIC_TOOL,
    target: COMBOBOX_LIVE_V35_TARGET,
  ...
  if (requests.requestCount !== COMBOBOX_LIVE_V35_REMOTE_REQUESTS)
    throw new Error("Combobox live v35 remote request denominator drifted");
  antecedentOutputs.set(
    COMBOBOX_LIVE_V35_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const lifecycleExcludedPaths = [
    COMBOBOX_LIVE_V35_AUTHORIZATION_TEMPLATE_PATH,
    COMBOBOX_LIVE_V35_AUTHORIZATION_PATH,
    "recipe/combobox-live-v35-authorization.ts",
    "recipe/combobox-live-v35-authorization.test.ts",
    "recipe/combobox-live-v35-preflight.ts",
    "recipe/combobox-live-v35-authorized.ts",
    "recipe/write-combobox-live-v35-authorization.ts",
    "recipe/create-combobox-live-v35-security-attestation.ts",
    "recipe/evidence/combobox-live-pivot-v35/operator-security-attestation-template.json",
    "recipe/evidence/combobox-live-pivot-v35-status.json",
    "recipe/evidence/status-index.json",
  ];
  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/combobox-live-v35-broker.ts",
    "recipe/combobox-live-v35-contract.ts",
    "recipe/combobox-live-v35-restore.ts",
    "recipe/combobox-live-v35-cleanup.ts",
    "recipe/combobox-live-v35-verifier.ts",
    "recipe/combobox-live-v35-fixed-point.ts",
    "recipe/run-combobox-live-v35.ts",
    "recipe/build-combobox-live-proof-v35.ts",
    "recipe/combobox-live-v35-lifecycle-simulation.ts",
    "recipe/scene-readback-combobox-v35.ts",
    "recipe/scene-readback-runtime-combobox-v35.ts",
    "recipe/combobox-figma-writer.ts",
    "recipe/figma-property-normalizer-v8.ts",
    "recipe/figma-runtime-portability.ts",
    "recipe/normalize.ts",
  ].filter((artifactPath) => artifactPath !== COMBOBOX_LIVE_V35_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "combobox-live-v35-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
  ...
  antecedentOutputs.set(COMBOBOX_LIVE_V35_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "combobox-live-v35-authorization-template-v1",
    authorizationId: "combobox-live-v35",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: COMBOBOX_LIVE_V35_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
    ...
    denominator: {
      remoteRequests: COMBOBOX_LIVE_V35_REMOTE_REQUESTS,
      hostPhases: COMBOBOX_LIVE_V35_HOST_PHASES,
      captures: COMBOBOX_LIVE_V35_CAPTURE_COUNT,
      sourceRoots: COMBOBOX_LIVE_V35_SOURCE_ROOTS,
      expectedFacts,
    },
    execution: {
  ...
  const attestationTemplate = jsonBytes({
    artifactVersion: "combobox-live-v35-operator-security-attestation-template-v1",
    status: "template only; complete privately after PREPARE+AUTHORIZE",
    tokenValuesForbidden: true,
    privateDirectory: "private/combobox-live-v35-security-attestation.json",
  });
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Combobox live v35 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(COMBOBOX_LIVE_V35_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(COMBOBOX_LIVE_V35_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
    writeFileSync(
      `${COMBOBOX_LIVE_V35_EVIDENCE_ROOT}/operator-security-attestation-template.json`,
      attestationTemplate,
    );
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(COMBOBOX_LIVE_V35_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(COMBOBOX_LIVE_V35_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Combobox live v35 authorization template drift");
  }
  if (
    writer.namespace !== COMBOBOX_FIGMA_NAMESPACE ||
    writer.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    ...
    throw new Error("Combobox live v35 identity or cleanup invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildComboboxLiveV35Proof(), null, 2)}\n`,
  );
