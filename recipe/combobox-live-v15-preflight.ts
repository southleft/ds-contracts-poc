...
import {
  ...
  verifyComboboxLiveV15Authorization,
  type ComboboxLiveV15AntecedentIndex,
  type ComboboxLiveV15AuthorizationProof,
} from "./combobox-live-v15-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V15_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v15/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V15_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v15-security-attestation.json";

export interface ComboboxLiveV15SecurityAttestation {
...
export interface ComboboxLiveV15PreflightReport {
  artifactVersion: "combobox-live-v15-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV15SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV15SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV15AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v15-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V15_TARGET) ||
...
export function validateComboboxLiveV15ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV15Authorization");
  const preflight = source.indexOf("runComboboxLiveV15Preflight");
  const initialize = source.indexOf("ComboboxLiveV15Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV15Preflight(
  root: string,
  proof: ComboboxLiveV15AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V15_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV15PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V15_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V15_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V15_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV15AntecedentIndex;
  failures.push(...validateComboboxLiveV15AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V15_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V15_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v15-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV15ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v15 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV15SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v15-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V15_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV15Preflight(
    process.cwd(),
    verifyComboboxLiveV15Authorization(),
    attestationPath,
