...
import { TABLE_LIVE_V37_INDEX_PATH } from "./build-table-live-proof-v37.js";
import {
...
import {
  buildTableLiveV37AuthorizationArtifact,
  validateTableLiveV37AntecedentIndex,
  validateTableLiveV37AuthorizationArtifact,
  validateTableLiveV37History,
  validateTableLiveV37UnitTestSource,
  type TableLiveV37AntecedentIndex,
  type TableLiveV37AuthorizationArtifact,
  type TableLiveV37AuthorizationProof,
  type TableLiveV37HistoryState,
} from "./table-live-v37-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV37ControlFlowSource,
  validateTableLiveV37SecurityAttestation,
  type TableLiveV37SecurityAttestation,
} from "./table-live-v37-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V37_INDEX_PATH);
const index = (): TableLiveV37AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV37AntecedentIndex;

const artifact = (): TableLiveV37AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV37AuthorizationArtifact({
...
const pendingState = (): TableLiveV37HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV37HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV37AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v37/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV37AuthorizationArtifact,
): TableLiveV37SecurityAttestation => {
  const body: Omit<TableLiveV37SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V37_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV37History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV37History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV37History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV37History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV37History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV37UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v37-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV37UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV37AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV37AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV37AuthorizationArtifact(
        planted as TableLiveV37AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV37SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV37SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV37SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v37-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV37ControlFlowSource(source), []);
});
