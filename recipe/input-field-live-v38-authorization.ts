...
import {
  ...
  INPUT_LIVE_V38_INDEX_PATH,
} from "./build-input-field-live-proof-v38.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV38HistoryExpectation = "pending" | "authorized";

export interface InputLiveV38AuthorizationArtifact {
  artifactVersion: "input-live-v38-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V38_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV38AntecedentIndex {
...
export interface InputLiveV38HistoryState {
  index?: InputLiveV38AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV38AuthorizationArtifact;
...
export interface InputLiveV38AuthorizationProof extends InputLiveV38TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V38_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V38_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V38_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V38_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v38-status") ||
...
export function validateInputLiveV38AntecedentIndex(
  index: InputLiveV38AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v38-antecedent-index-v1" ||
...
export function validateInputLiveV38UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV38History\(\s*["']/.test(source) ||
    /readInputLiveV38HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV38AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV38AuthorizationArtifact {
  ...
  ) as InputLiveV38AntecedentIndex;
  const indexFailures = validateInputLiveV38AntecedentIndex(index);
...
export function validateInputLiveV38AuthorizationArtifact(
  artifact: InputLiveV38AuthorizationArtifact | undefined,
  index: InputLiveV38AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v38-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v38" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V38_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V38_TARGET) ||
    !equalJson(
...
export function validateInputLiveV38History(
  state: InputLiveV38HistoryState,
  expected: InputLiveV38HistoryExpectation,
): string[] {
  const failures = validateInputLiveV38AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV38AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV38HistoryState(
  root = process.cwd(),
): InputLiveV38HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V38_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV38AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V38_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V38_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V38_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV38History(
  expected: InputLiveV38HistoryExpectation,
  root = process.cwd(),
): InputLiveV38HistoryState {
  const state = readInputLiveV38HistoryState(root);
  const failures = validateInputLiveV38History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV38Authorization(
  root = process.cwd(),
): InputLiveV38AuthorizationProof {
  const state = verifyInputLiveV38History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V38_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v38/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v38.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V38_TARGET,
    expectedDynamicTool: INPUT_LIVE_V38_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V38_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV38HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV38History(expected);
