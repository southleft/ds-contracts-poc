...
import {
  acceptCalendarLiveV12Response,
  createCalendarLiveV12Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV12Request,
  persistCalendarLiveV12RawResponse,
  persistCalendarLiveV12TechnicalGates,
  readCalendarLiveV12AcceptedRawPayload,
  type CalendarLiveV12Request,
  type CalendarLiveV12TransactionAuthorization,
} from "./calendar-live-v12-broker.js";
import {
  CALENDAR_LIVE_V12_CAPTURE_COUNT,
  assertCalendarLiveV12CaptureResponses,
  ...
  evaluateCalendarLiveV12Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV12Roots,
  validateCalendarLiveV12CaptureManifest,
  validateCalendarLiveV12CapturePayload,
  validateCalendarLiveV12CleanupPayload,
  validateCalendarLiveV12ExtractPayload,
  validateCalendarLiveV12ProbePayload,
  validateCalendarLiveV12WriterPayload,
  type CalendarLiveV12CaptureCell,
  type CalendarLiveV12CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V12_PLAN_PATH,
  type CalendarLiveV12ProofPlan,
} from "./build-calendar-live-proof-v12.js";
import {
...
export const CALENDAR_LIVE_V12_RUNNER_VERSION = "calendar-live-v12-runner-v1";

interface CalendarLiveV12JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV12OrchestratorState {
  artifactVersion: "calendar-live-v12-orchestrator-state-v1";
  ...
  journal: CalendarLiveV12JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV12ProofPlan;
  planSha256: string;
  captures: CalendarLiveV12CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV12SourceIdentity & {
...
export interface CalendarLiveV12NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV12OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v12 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V12_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V12_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV12ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV12CaptureCell[];
  validateCalendarLiveV12CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v12 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v12 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV12WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV12RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV12Request =>
  readJson<CalendarLiveV12Request>(file);

export class CalendarLiveV12Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV12Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV12OrchestratorState = {
      artifactVersion: "calendar-live-v12-orchestrator-state-v1",
    ...
    return new CalendarLiveV12Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV12OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV12Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV12NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV12NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v12 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV12NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v12 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV12RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV12Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV12CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV12AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV12WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV12Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV12ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV12Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV12Response(this.#directory, "writer", {
        validate: validateCalendarLiveV12WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV12Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV12RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV12Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV12ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV12Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV12RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v12-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV12Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV12ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV12Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV12ProbePayload(payload, ownership),
      });
      persistCalendarLiveV12TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v12 capture cell missing");
      const accepted = acceptCalendarLiveV12Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV12CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V12_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV12CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV12Objective(
    ...
    throw new Error(`Calendar live v12 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV12Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV12CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV12Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV12RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV12Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV12CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV12CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV12CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV12Authorization = (
  privateKey: KeyObject,
): CalendarLiveV12TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV12SimulationKey = (): KeyObject =>
