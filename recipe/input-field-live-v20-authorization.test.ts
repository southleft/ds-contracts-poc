...
import { INPUT_LIVE_V20_INDEX_PATH } from "./build-input-field-live-proof-v20.js";
import {
  INPUT_LIVE_V20_DYNAMIC_TOOL,
  INPUT_LIVE_V20_TARGET,
} from "./input-field-live-v20-broker.js";
import {
  buildInputLiveV20AuthorizationArtifact,
  validateInputLiveV20AntecedentIndex,
  validateInputLiveV20AuthorizationArtifact,
  validateInputLiveV20History,
  validateInputLiveV20UnitTestSource,
  type InputLiveV20AntecedentIndex,
  type InputLiveV20AuthorizationArtifact,
  type InputLiveV20AuthorizationProof,
  type InputLiveV20HistoryState,
} from "./input-field-live-v20-authorization.js";
import {
  inputLiveV20AttestationSha256,
  validateInputLiveV20ControlFlowSource,
  validateInputLiveV20SecurityAttestation,
  type InputLiveV20SecurityAttestation,
} from "./input-field-live-v20-preflight.js";
import { inputLiveV20Sha256 } from "./input-field-live-v20-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V20_INDEX_PATH);
const index = (): InputLiveV20AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV20AntecedentIndex;

const artifact = (): InputLiveV20AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV20AuthorizationArtifact({
...
const pendingState = (): InputLiveV20HistoryState => ({
  index: index(),
  indexSha256: inputLiveV20Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV20HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV20AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV20Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V20_TARGET,
  expectedDynamicTool: INPUT_LIVE_V20_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v20/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV20AuthorizationArtifact,
): InputLiveV20SecurityAttestation => {
  const body: Omit<InputLiveV20SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v20-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V20_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV20AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV20History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV20History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV20History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV20History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV20History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV20UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV20UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV20AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV20Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV20AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV20AuthorizationArtifact(
        planted as InputLiveV20AuthorizationArtifact,
        index(),
        inputLiveV20Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV20SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV20AttestationSha256(
      body as Omit<InputLiveV20SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV20SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV20ControlFlowSource(source), []);
});
