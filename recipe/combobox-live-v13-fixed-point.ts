/**
 * Carried Combobox live v13 host verifier. Uses scene-readback-combobox-v13 so live
...
export const COMBOBOX_LIVE_V13_FIXED_VERIFIER_VERSION = "combobox-live-v13-fixed-point-v1";
export const COMBOBOX_LIVE_V13_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV13FixedThresholds {
...
export interface ComboboxLiveV13FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV13FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV13FixedSourceProbe {
...
export interface ComboboxLiveV13FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV13FixedFixedPoint;
}

export interface ComboboxLiveV13FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV13FixedHardGateInput {
  thresholds: ComboboxLiveV13FixedThresholds;
  materialRegression: ComboboxLiveV13FixedMaterialRegression;
  sourceProbes: ComboboxLiveV13FixedSourceProbe[];
  cells: ComboboxLiveV13FixedCellValidation[];
  sceneProofs: ComboboxLiveV13FixedSceneProof[];
  visualRows: ComboboxLiveV13FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV13FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V13_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV13FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV13FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV13FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV13FixedHardGates(
  input: ComboboxLiveV13FixedHardGateInput,
): ComboboxLiveV13FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV13FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV13FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V13_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV13FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV13FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV13FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
