...
export type CalendarLiveV27HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV27AuthorizationArtifact {
  artifactVersion: "calendar-live-v27-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V27_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV27AntecedentIndex {
...
export interface CalendarLiveV27HistoryState {
  index?: CalendarLiveV27AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV27AuthorizationArtifact;
...
export interface CalendarLiveV27AuthorizationProof
  extends CalendarLiveV27TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V27_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V27_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V27_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V27_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v27-authorization") ||
  artifactPath.includes("calendar-live-v27-preflight") ||
  artifactPath.includes("calendar-live-v27-authorized") ||
  artifactPath.includes("create-calendar-live-v27-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v27-status") ||
...
export function validateCalendarLiveV27AntecedentIndex(
  index: CalendarLiveV27AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v27-antecedent-index-v1" ||
...
export function validateCalendarLiveV27UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV27History\(\s*["']/.test(source) ||
    /readCalendarLiveV27HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV27AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV27AuthorizationArtifact {
  ...
  ) as CalendarLiveV27AntecedentIndex;
  const indexFailures = validateCalendarLiveV27AntecedentIndex(index);
...
export function validateCalendarLiveV27AuthorizationArtifact(
  artifact: CalendarLiveV27AuthorizationArtifact | undefined,
  index: CalendarLiveV27AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v27-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v27" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V27_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V27_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V27_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V27_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V27_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V27_SOURCE_ROOTS ||
...
export function validateCalendarLiveV27History(
  state: CalendarLiveV27HistoryState,
  expected: CalendarLiveV27HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV27AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV27AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV27HistoryState(
  root = process.cwd(),
): CalendarLiveV27HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V27_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV27AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V27_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V27_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V27_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV27History(
  expected: CalendarLiveV27HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV27HistoryState {
  const state = readCalendarLiveV27HistoryState(root);
  const failures = validateCalendarLiveV27History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v27 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV27Authorization(
  root = process.cwd(),
): CalendarLiveV27AuthorizationProof {
  const state = verifyCalendarLiveV27History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V27_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v27/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v27.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V27_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V27_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V27_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV27HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV27History(expected);
  process.stdout.write(
    `Calendar live v27 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
