...
import { calendarLiveV1Sha256 } from "./calendar-live-v37-broker.js";

export interface CalendarLiveV37LifecycleSimulationReport {
  artifactVersion: "calendar-live-v37-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV37AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV37Lifecycle(): Promise<CalendarLiveV37LifecycleSimulationReport> {
  await buildCalendarLiveV37Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V37_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV37AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v37-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V37_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V37_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV37History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV37History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV37AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V37_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V37_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV37History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV37History(authorized, "pending");
    ...
    await buildCalendarLiveV37Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V37_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV37AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV37Lifecycle(), null, 2)}\n`,
  );
