...
import {
  validateComboboxLiveV10AntecedentIndex,
  verifyComboboxLiveV10Authorization,
  type ComboboxLiveV10AntecedentIndex,
  type ComboboxLiveV10AuthorizationProof,
} from "./combobox-live-v10-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V10_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v10/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V10_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v10-security-attestation.json";

export interface ComboboxLiveV10SecurityAttestation {
  artifactVersion: "combobox-live-v10-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV10PreflightReport {
  artifactVersion: "combobox-live-v10-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV10SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV10SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV10AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v10-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V10_TARGET) ||
...
export function validateComboboxLiveV10ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV10Authorization");
  const preflight = source.indexOf("runComboboxLiveV10Preflight");
  const initialize = source.indexOf("ComboboxLiveV10Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV10Preflight(
  root: string,
  proof: ComboboxLiveV10AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V10_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV10PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V10_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V10_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V10_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV10AntecedentIndex;
  failures.push(...validateComboboxLiveV10AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V10_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V10_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV10SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v10-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV10ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v10 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV10SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v10-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V10_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV10Preflight(
    process.cwd(),
    verifyComboboxLiveV10Authorization(),
    attestationPath,
