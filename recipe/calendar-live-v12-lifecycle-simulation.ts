...
import { calendarLiveV1Sha256 } from "./calendar-live-v12-broker.js";

export interface CalendarLiveV12LifecycleSimulationReport {
  artifactVersion: "calendar-live-v12-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV12AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV12Lifecycle(): Promise<CalendarLiveV12LifecycleSimulationReport> {
  await buildCalendarLiveV12Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V12_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV12AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v12-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V12_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V12_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV12History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV12History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV12AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V12_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V12_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV12History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV12History(authorized, "pending");
    ...
    await buildCalendarLiveV12Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V12_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV12AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV12Lifecycle(), null, 2)}\n`,
  );
