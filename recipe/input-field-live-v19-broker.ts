...
import {
  assertInputLiveV19RootProofs,
  INPUT_LIVE_V19_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V19_RESPONSE_CONTRACTS,
  type InputLiveV19RootProof,
} from "./input-field-live-v19-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V19_BROKER_VERSION = "input-live-v19-broker-v2";
export const INPUT_LIVE_V19_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V19_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V19_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V19_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V19_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V19_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV19RemotePhase =
  (typeof INPUT_LIVE_V19_REMOTE_PHASES)[number];
export type InputLiveV19AuthorizationMode = "live" | "simulated";

export interface InputLiveV19CommitPins {
...
export interface InputLiveV19TransactionAuthorization extends InputLiveV19CommitPins {
  mode: InputLiveV19AuthorizationMode;
...
export interface InputLiveV19Manifest {
  artifactVersion: "input-live-v19-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V19_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V19_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V19_TARGET;
  authorization: InputLiveV19TransactionAuthorization;
...
export interface InputLiveV19ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV19RequestBody {
  artifactVersion: "input-live-v19-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V19_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV19RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV19CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V19_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV19ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV19Request extends InputLiveV19RequestBody {
...
export interface InputLiveV19Receipt {
  artifactVersion: "input-live-v19-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V19_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV19RemotePhase;
...
export interface InputLiveV19AcceptedResponse<T = unknown> {
  request: InputLiveV19Request;
  receipt: InputLiveV19Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV19IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV19ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V19_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V19_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V19_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V19_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V19_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V19_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V19_TARGET.editorType;
};
...
export const inputLiveV19RequestSequence = (
  phase: InputLiveV19RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV19RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV19RequestPath = (
  directory: string,
  phase: InputLiveV19RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV19ResponsePath = (
  directory: string,
  phase: InputLiveV19RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV19ReceiptPath = (
  directory: string,
  phase: InputLiveV19RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV19Manifest => {
  const manifest = json<InputLiveV19Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v19-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V19_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V19_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV19RemotePhase,
  captureIndex?: number,
): InputLiveV19Request | undefined => {
  const file = inputLiveV19RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV19Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV19RemotePhase,
  captureIndex?: number,
): InputLiveV19Receipt | undefined => {
  const file = inputLiveV19ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV19Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV19Request | undefined;
  receipt: InputLiveV19Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV19RemotePhase,
): InputLiveV19ResponseContract => ({
  ...INPUT_LIVE_V19_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V19_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V19_DYNAMIC_TOOL,
      target: INPUT_LIVE_V19_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v19-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V19_BROKER_VERSION,
  target: INPUT_LIVE_V19_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V19_PROGRAM_START*/${program}/*INPUT_LIVE_V19_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V19_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V19_PROGRAM_END*/";
...
export function createInputLiveV19Transaction(
  directory: string,
  ...
  const manifest: InputLiveV19Manifest = {
    artifactVersion: "input-live-v19-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V19_BROKER_VERSION,
...
export function validateInputLiveV19Request(
  directory: string,
  request: InputLiveV19Request,
): void {
  ...
  const expectedSequence = inputLiveV19RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v19-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V19_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V19_TARGET.fileKey ||
...
export function issueInputLiveV19Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV19RemotePhase,
  program: string,
  options: InputLiveV19IssueOptions = {},
): InputLiveV19Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV19RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV19Request>(targetPath);
    validateInputLiveV19Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV19ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV19TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV19RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V19_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V19_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V19_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV19RequestBody = {
    artifactVersion: "input-live-v19-broker-request-v2",
    brokerVersion: INPUT_LIVE_V19_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV19Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV19Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV19RawResponse(
  directory: string,
  phase: InputLiveV19RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV19RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV19Request>(requestFile);
  if (
  ...
  const target = inputLiveV19ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV19Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v19-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V19_BROKER_VERSION,
...
export function acceptInputLiveV19Response<T = unknown>(
  directory: string,
  phase: InputLiveV19RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV19AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV19ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV19Request>(
    inputLiveV19RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV19Request(directory, request);
  if (phase === "capture") validateInputLiveV19TechnicalGates(directory);
  const rawFile = inputLiveV19ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v19-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V19_BROKER_VERSION ||
...
export function persistInputLiveV19TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV19RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV19RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V19_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v19-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V19_BROKER_VERSION,
...
export function validateInputLiveV19TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v19-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V19_TECHNICAL_GATES.some(
...
export function validateInputLiveV19Receipt(
  directory: string,
  phase: InputLiveV19RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV19Request(directory, request);
  const receipt = json<InputLiveV19Receipt>(
    inputLiveV19ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV19ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v19-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V19_BROKER_VERSION ||
...
export function readInputLiveV19AcceptedRawPayload(
  directory: string,
  phase: InputLiveV19RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV19Receipt(directory, phase, captureIndex);
...
export function createInputLiveV19FakeDynamicResponse(
  request: InputLiveV19Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V19_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V19_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV19PersistedRequests(
  directory: string,
): InputLiveV19Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV19Request>(file)] : [];
    });
}

export const inputLiveV19Sha256 = sha256;
