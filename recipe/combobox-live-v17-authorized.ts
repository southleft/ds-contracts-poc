...
import { COMBOBOX_LIVE_V17_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v17.js";
import { verifyComboboxLiveV17Authorization } from "./combobox-live-v17-authorization.js";
import { runComboboxLiveV17Preflight } from "./combobox-live-v17-preflight.js";
import { ComboboxLiveV17Orchestrator } from "./run-combobox-live-v17.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v17 requires exact authorization artifact ${COMBOBOX_LIVE_V17_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v17-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV17Authorization(root);
    runComboboxLiveV17Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV17Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV17Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
