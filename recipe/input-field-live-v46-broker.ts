...
import {
  assertInputLiveV46RootProofs,
  INPUT_LIVE_V46_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V46_RESPONSE_CONTRACTS,
  type InputLiveV46RootProof,
} from "./input-field-live-v46-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V46_BROKER_VERSION = "input-live-v46-broker-v2";
export const INPUT_LIVE_V46_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V46_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V46_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V46_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V46_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V46_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV46RemotePhase =
  (typeof INPUT_LIVE_V46_REMOTE_PHASES)[number];
export type InputLiveV46AuthorizationMode = "live" | "simulated";

export interface InputLiveV46CommitPins {
...
export interface InputLiveV46TransactionAuthorization extends InputLiveV46CommitPins {
  mode: InputLiveV46AuthorizationMode;
...
export interface InputLiveV46Manifest {
  artifactVersion: "input-live-v46-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V46_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V46_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V46_TARGET;
  authorization: InputLiveV46TransactionAuthorization;
...
export interface InputLiveV46ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV46RequestBody {
  artifactVersion: "input-live-v46-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V46_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV46RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV46CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V46_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV46ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV46Request extends InputLiveV46RequestBody {
...
export interface InputLiveV46Receipt {
  artifactVersion: "input-live-v46-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V46_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV46RemotePhase;
...
export interface InputLiveV46AcceptedResponse<T = unknown> {
  request: InputLiveV46Request;
  receipt: InputLiveV46Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV46IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV46ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V46_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V46_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V46_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V46_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V46_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V46_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V46_TARGET.editorType;
};
...
export const inputLiveV46RequestSequence = (
  phase: InputLiveV46RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV46RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV46RequestPath = (
  directory: string,
  phase: InputLiveV46RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV46ResponsePath = (
  directory: string,
  phase: InputLiveV46RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV46ReceiptPath = (
  directory: string,
  phase: InputLiveV46RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV46Manifest => {
  const manifest = json<InputLiveV46Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v46-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V46_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V46_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV46RemotePhase,
  captureIndex?: number,
): InputLiveV46Request | undefined => {
  const file = inputLiveV46RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV46Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV46RemotePhase,
  captureIndex?: number,
): InputLiveV46Receipt | undefined => {
  const file = inputLiveV46ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV46Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV46Request | undefined;
  receipt: InputLiveV46Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV46RemotePhase,
): InputLiveV46ResponseContract => ({
  ...INPUT_LIVE_V46_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V46_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V46_DYNAMIC_TOOL,
      target: INPUT_LIVE_V46_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v46-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V46_BROKER_VERSION,
  target: INPUT_LIVE_V46_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V46_PROGRAM_START*/${program}/*INPUT_LIVE_V46_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V46_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V46_PROGRAM_END*/";
...
export function createInputLiveV46Transaction(
  directory: string,
  ...
  const manifest: InputLiveV46Manifest = {
    artifactVersion: "input-live-v46-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V46_BROKER_VERSION,
...
export function validateInputLiveV46Request(
  directory: string,
  request: InputLiveV46Request,
): void {
  ...
  const expectedSequence = inputLiveV46RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v46-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V46_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V46_TARGET.fileKey ||
...
export function issueInputLiveV46Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV46RemotePhase,
  program: string,
  options: InputLiveV46IssueOptions = {},
): InputLiveV46Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV46RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV46Request>(targetPath);
    validateInputLiveV46Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV46ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV46TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV46RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V46_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V46_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V46_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV46RequestBody = {
    artifactVersion: "input-live-v46-broker-request-v2",
    brokerVersion: INPUT_LIVE_V46_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV46Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV46Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV46RawResponse(
  directory: string,
  phase: InputLiveV46RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV46RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV46Request>(requestFile);
  if (
  ...
  const target = inputLiveV46ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV46Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v46-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V46_BROKER_VERSION,
...
export function acceptInputLiveV46Response<T = unknown>(
  directory: string,
  phase: InputLiveV46RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV46AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV46ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV46Request>(
    inputLiveV46RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV46Request(directory, request);
  if (phase === "capture") validateInputLiveV46TechnicalGates(directory);
  const rawFile = inputLiveV46ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v46-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V46_BROKER_VERSION ||
...
export function persistInputLiveV46TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV46RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV46RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V46_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v46-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V46_BROKER_VERSION,
...
export function validateInputLiveV46TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v46-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V46_TECHNICAL_GATES.some(
...
export function validateInputLiveV46Receipt(
  directory: string,
  phase: InputLiveV46RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV46Request(directory, request);
  const receipt = json<InputLiveV46Receipt>(
    inputLiveV46ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV46ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v46-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V46_BROKER_VERSION ||
...
export function readInputLiveV46AcceptedRawPayload(
  directory: string,
  phase: InputLiveV46RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV46Receipt(directory, phase, captureIndex);
...
export function createInputLiveV46FakeDynamicResponse(
  request: InputLiveV46Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V46_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V46_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV46PersistedRequests(
  directory: string,
): InputLiveV46Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV46Request>(file)] : [];
    });
}

export const inputLiveV46Sha256 = sha256;
