...
import {
  validateTableLiveV17AntecedentIndex,
  verifyTableLiveV17Authorization,
  type TableLiveV17AntecedentIndex,
  type TableLiveV17AuthorizationProof,
} from "./table-live-v17-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V17_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v17/operator-security-attestation-template.json";
export const TABLE_LIVE_V17_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v17-security-attestation.json";

export interface TableLiveV17SecurityAttestation {
  artifactVersion: "table-live-v17-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV17PreflightReport {
  artifactVersion: "table-live-v17-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV17SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV17SecurityAttestation(
  value: unknown,
  proof: TableLiveV17AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v17-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V17_TARGET) ||
...
export function validateTableLiveV17ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV17Authorization");
  const preflight = source.indexOf("runTableLiveV17Preflight");
  const initialize = source.indexOf("TableLiveV17Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV17Preflight(
  root: string,
  proof: TableLiveV17AuthorizationProof,
  attestationPath = TABLE_LIVE_V17_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV17PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V17_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V17_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV17AntecedentIndex;
  failures.push(...validateTableLiveV17AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V17_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V17_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV17SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v17-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV17ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v17 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV17SecurityAttestation;
  return {
    artifactVersion: "table-live-v17-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V17_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV17Preflight(
    process.cwd(),
    verifyTableLiveV17Authorization(),
    attestationPath,
