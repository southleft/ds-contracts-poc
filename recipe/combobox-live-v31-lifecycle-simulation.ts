...
export interface ComboboxLiveV31LifecycleSimulationReport {
  artifactVersion: "combobox-live-v31-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV31AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV31Lifecycle(): Promise<ComboboxLiveV31LifecycleSimulationReport> {
  await buildComboboxLiveV31Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V31_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV31AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v31-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V31_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V31_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV31History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV31History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV31AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V31_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V31_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV31History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV31History(authorized, "pending");
    ...
    await buildComboboxLiveV31Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V31_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV31AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV31Lifecycle(), null, 2)}\n`,
  );
