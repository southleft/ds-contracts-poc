...
import { CALENDAR_LIVE_V46_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v46.js";
import { verifyCalendarLiveV46Authorization } from "./calendar-live-v46-authorization.js";
import { runCalendarLiveV46Preflight } from "./calendar-live-v46-preflight.js";
import { CalendarLiveV46Orchestrator } from "./run-calendar-live-v46.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v46 requires exact authorization artifact ${CALENDAR_LIVE_V46_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v46-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV46Authorization(root);
    runCalendarLiveV46Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV46Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV46Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
