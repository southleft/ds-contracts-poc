...
import {
  acceptComboboxLiveV37Response,
  createComboboxLiveV37Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV37Request,
  persistComboboxLiveV37RawResponse,
  persistComboboxLiveV37TechnicalGates,
  readComboboxLiveV37AcceptedRawPayload,
  type ComboboxLiveV37Request,
  type ComboboxLiveV37TransactionAuthorization,
} from "./combobox-live-v37-broker.js";
import {
  COMBOBOX_LIVE_V37_CAPTURE_COUNT,
  assertComboboxLiveV37CaptureResponses,
  ...
  evaluateComboboxLiveV37Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV37Roots,
  validateComboboxLiveV37CaptureManifest,
  validateComboboxLiveV37CapturePayload,
  validateComboboxLiveV37CleanupPayload,
  validateComboboxLiveV37ExtractPayload,
  validateComboboxLiveV37ProbePayload,
  validateComboboxLiveV37WriterPayload,
  type ComboboxLiveV37CaptureCell,
  type ComboboxLiveV37CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V37_PLAN_PATH,
  type ComboboxLiveV37ProofPlan,
} from "./build-combobox-live-proof-v37.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v37.js";

export const COMBOBOX_LIVE_V37_RUNNER_VERSION = "combobox-live-v37-runner-v1";

interface ComboboxLiveV37JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV37OrchestratorState {
  artifactVersion: "combobox-live-v37-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV37JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV37ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV37CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV37SourceIdentity & {
...
export interface ComboboxLiveV37NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV37OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v37 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V37_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V37_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV37ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV37CaptureCell[];
  validateComboboxLiveV37CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v37 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v37 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV37WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV37RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV37Request =>
  readJson<ComboboxLiveV37Request>(file);

export class ComboboxLiveV37Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV37Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV37OrchestratorState = {
      artifactVersion: "combobox-live-v37-orchestrator-state-v1",
    ...
    return new ComboboxLiveV37Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV37OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV37Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV37NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV37NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v37 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV37NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v37 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV37RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV37Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV37CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV37AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV37WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV37Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV37ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV37Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV37Response(this.#directory, "writer", {
        validate: validateComboboxLiveV37WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV37Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV37RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV37Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV37ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV37Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV37RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v37-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV37Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV37ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV37Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV37ProbePayload(payload, ownership),
      });
      persistComboboxLiveV37TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v37 capture cell missing");
      const accepted = acceptComboboxLiveV37Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV37CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V37_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV37CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV37Objective(
    ...
    throw new Error(`Combobox live v37 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV37Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV37CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV37Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV37RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV37Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV37CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV37CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV37CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV37Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV37TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV37SimulationKey = (): KeyObject =>
