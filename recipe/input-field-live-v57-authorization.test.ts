...
import { INPUT_LIVE_V57_INDEX_PATH } from "./build-input-field-live-proof-v57.js";
import {
  INPUT_LIVE_V57_DYNAMIC_TOOL,
  INPUT_LIVE_V57_TARGET,
} from "./input-field-live-v57-broker.js";
import {
  buildInputLiveV57AuthorizationArtifact,
  validateInputLiveV57AntecedentIndex,
  validateInputLiveV57AuthorizationArtifact,
  validateInputLiveV57History,
  validateInputLiveV57UnitTestSource,
  type InputLiveV57AntecedentIndex,
  type InputLiveV57AuthorizationArtifact,
  type InputLiveV57AuthorizationProof,
  type InputLiveV57HistoryState,
} from "./input-field-live-v57-authorization.js";
import {
  inputLiveV57AttestationSha256,
  validateInputLiveV57ControlFlowSource,
  validateInputLiveV57SecurityAttestation,
  type InputLiveV57SecurityAttestation,
} from "./input-field-live-v57-preflight.js";
import { inputLiveV57Sha256 } from "./input-field-live-v57-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V57_INDEX_PATH);
const index = (): InputLiveV57AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV57AntecedentIndex;

const artifact = (): InputLiveV57AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV57AuthorizationArtifact({
...
const pendingState = (): InputLiveV57HistoryState => ({
  index: index(),
  indexSha256: inputLiveV57Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV57HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV57AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV57Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V57_TARGET,
  expectedDynamicTool: INPUT_LIVE_V57_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v57/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV57AuthorizationArtifact,
): InputLiveV57SecurityAttestation => {
  const body: Omit<InputLiveV57SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v57-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V57_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV57AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV57History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV57History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV57History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV57History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV57History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV57UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV57UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV57AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV57Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV57AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV57AuthorizationArtifact(
        planted as InputLiveV57AuthorizationArtifact,
        index(),
        inputLiveV57Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV57SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV57AttestationSha256(
      body as Omit<InputLiveV57SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV57SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV57ControlFlowSource(source), []);
});
