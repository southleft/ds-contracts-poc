...
import { CALENDAR_LIVE_V17_INDEX_PATH } from "./build-calendar-live-proof-v17.js";
import {
...
import {
  buildCalendarLiveV17AuthorizationArtifact,
  validateCalendarLiveV17AntecedentIndex,
  validateCalendarLiveV17AuthorizationArtifact,
  validateCalendarLiveV17History,
  validateCalendarLiveV17UnitTestSource,
  type CalendarLiveV17AntecedentIndex,
  type CalendarLiveV17AuthorizationArtifact,
  type CalendarLiveV17AuthorizationProof,
  type CalendarLiveV17HistoryState,
} from "./calendar-live-v17-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV17ControlFlowSource,
  validateCalendarLiveV17SecurityAttestation,
  type CalendarLiveV17SecurityAttestation,
} from "./calendar-live-v17-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V17_INDEX_PATH);
const index = (): CalendarLiveV17AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV17AntecedentIndex;

const artifact = (): CalendarLiveV17AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV17AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV17HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV17HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV17AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v17/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV17AuthorizationArtifact,
): CalendarLiveV17SecurityAttestation => {
  const body: Omit<CalendarLiveV17SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V17_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV17History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV17History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV17History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV17History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV17History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV17UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v17-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV17UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV17AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV17AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV17AuthorizationArtifact(
        planted as CalendarLiveV17AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV17SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV17SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV17SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v17-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV17ControlFlowSource(source), []);
});
