...
import { verifyComboboxLiveV25Authorization } from "./combobox-live-v25-authorization.js";
import { COMBOBOX_LIVE_V25_TARGET } from "./combobox-live-v25-broker.js";
import {
  COMBOBOX_LIVE_V25_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV25Authorization();
...
const attestation: ComboboxLiveV25SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV25SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v25 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V25_SECURITY_ATTESTATION_DEFAULT_PATH,
);
