...
import { calendarLiveV1Sha256 } from "./calendar-live-v43-broker.js";

export interface CalendarLiveV43LifecycleSimulationReport {
  artifactVersion: "calendar-live-v43-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV43AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV43Lifecycle(): Promise<CalendarLiveV43LifecycleSimulationReport> {
  await buildCalendarLiveV43Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V43_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV43AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v43-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V43_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V43_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV43History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV43History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV43AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V43_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V43_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV43History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV43History(authorized, "pending");
    ...
    await buildCalendarLiveV43Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V43_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV43AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV43Lifecycle(), null, 2)}\n`,
  );
