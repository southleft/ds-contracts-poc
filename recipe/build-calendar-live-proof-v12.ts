...
import {
  ...
  CALENDAR_LIVE_V12_HOST_PHASES,
  CALENDAR_LIVE_V12_REMOTE_REQUESTS,
  CALENDAR_LIVE_V12_SOURCE_ROOTS,
  CALENDAR_LIVE_V12_VARIANT_COUNT,
  buildCalendarLiveV12CaptureProgram,
  buildCalendarLiveV12CleanupProgram,
  buildCalendarLiveV12ExtractProgram,
  buildCalendarLiveV12ProbeProgram,
  buildCalendarLiveV12RestoreProgram,
  calendarLiveV1CaptureManifestSha256,
  type CalendarLiveV12CaptureCell,
  type CalendarLiveV12SourceIdentity,
  type CalendarLiveV12WriterOwnership,
} from "./calendar-live-v12-contract.js";
import {
  CALENDAR_LIVE_V12_DYNAMIC_TOOL,
...
export const CALENDAR_LIVE_V12_EVIDENCE_ROOT =
  "recipe/evidence/calendar-live-pivot-v12";
export const CALENDAR_LIVE_V12_PROTOCOL_PATH = `${CALENDAR_LIVE_V12_EVIDENCE_ROOT}/protocol.json`;
export const CALENDAR_LIVE_V12_PLAN_PATH = `${CALENDAR_LIVE_V12_EVIDENCE_ROOT}/proof-plan.json`;
export const CALENDAR_LIVE_V12_CAPTURE_MANIFEST_PATH = `${CALENDAR_LIVE_V12_EVIDENCE_ROOT}/capture-manifest.json`;
export const CALENDAR_LIVE_V12_REQUEST_MANIFEST_PATH = `${CALENDAR_LIVE_V12_EVIDENCE_ROOT}/request-manifest.json`;
export const CALENDAR_LIVE_V12_INDEX_PATH = `${CALENDAR_LIVE_V12_EVIDENCE_ROOT}/antecedent-index.json`;
export const CALENDAR_LIVE_V12_AUTHORIZATION_TEMPLATE_PATH = `${CALENDAR_LIVE_V12_EVIDENCE_ROOT}/authorization-template.json`;
export const CALENDAR_LIVE_V12_AUTHORIZATION_PATH = `${CALENDAR_LIVE_V12_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface CalendarLiveV12ProofPlan {
  artifactVersion: "calendar-live-v12-proof-plan-v1";
...
const captureCells = (
...
): CalendarLiveV12CaptureCell[] => {
  const cells: CalendarLiveV12CaptureCell[] = [];
  ...
  if (cells.length !== CALENDAR_LIVE_V12_CAPTURE_COUNT)
    throw new Error(
      `Calendar live v12 capture cells ${cells.length}, expected ${CALENDAR_LIVE_V12_CAPTURE_COUNT}`,
    );
...
const protocol = (expectedFacts: number) => ({
  artifactVersion: "calendar-live-v12-external-operator-protocol-v1",
  protocolId: "calendar-live-v12",
  status:
  ...
  denominator: {
    sourceRoots: CALENDAR_LIVE_V12_SOURCE_ROOTS,
    sets: 3,
    variants: CALENDAR_LIVE_V12_VARIANT_COUNT,
    captureCells: CALENDAR_LIVE_V12_CAPTURE_COUNT,
    remoteRequests: CALENDAR_LIVE_V12_REMOTE_REQUESTS,
    hostPhases: CALENDAR_LIVE_V12_HOST_PHASES,
...
export async function buildCalendarLiveV12Proof(
  check = process.argv.includes("--check"),
): Promise<CalendarLiveV12ProofPlan> {
  ...
  if (
    ...
    !writer.runIdentity.endsWith("-calendar-v12") ||
    ...
    throw new Error(
      "Calendar live v12 writer reused Input, Combobox, or Table v1–v24 identity",
    );
  const captures = captureCells(sources);
  const writerProgram = Buffer.from(writer.code);
  const writerOwnershipBlueprint: CalendarLiveV12WriterOwnership = {
  ...
  const identities: CalendarLiveV12SourceIdentity[] = sources.map((source) => ({
    source: source.source,
    adapterIdentity: source.adapterIdentity,
    recipeHash: source.recipeHash,
    envelopeHash: source.envelope.integrity.canonicalHash,
  ...
  const restoreBlueprint = Buffer.from(
    buildCalendarLiveV12RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildCalendarLiveV12ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildCalendarLiveV12ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildCalendarLiveV12CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );
  const cleanupBlueprint = Buffer.from(
    buildCalendarLiveV12CleanupProgram(writerOwnershipBlueprint),
  );
  ...
  const sourceMetadata = sources.map((source) => {
    ...
    const tablePath = `${CALENDAR_LIVE_V12_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-calendar.json.gz`;
    const rowPath = `${CALENDAR_LIVE_V12_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-week.json.gz`;
    const cellPath = `${CALENDAR_LIVE_V12_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-day.json.gz`;
  ...
  antecedentOutputs.set(
    CALENDAR_LIVE_V12_PROTOCOL_PATH,
    jsonBytes(protocol(expectedFacts)),
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V12_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V12_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V12_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V12_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V12_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V12_EVIDENCE_ROOT}/programs/cleanup-blueprint.js`,
    cleanupBlueprint,
  );

  const captureManifest = {
    artifactVersion: "calendar-live-v12-capture-manifest-v1",
  ...
  antecedentOutputs.set(
    CALENDAR_LIVE_V12_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: CalendarLiveV12ProofPlan = {
    artifactVersion: "calendar-live-v12-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: CALENDAR_LIVE_V12_TARGET,
    namespace: CALENDAR_FIGMA_NAMESPACE,
    writer: {
      programPath: `${CALENDAR_LIVE_V12_EVIDENCE_ROOT}/programs/writer.txt`,
    ...
    captureManifest: {
      path: CALENDAR_LIVE_V12_CAPTURE_MANIFEST_PATH,
      cells: 8,
      requests: 8,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(CALENDAR_LIVE_V12_PLAN_PATH, jsonBytes(proofPlan));

  const requests = {
    artifactVersion: "calendar-live-v12-request-manifest-v1",
    expectedDynamicTool: CALENDAR_LIVE_V12_DYNAMIC_TOOL,
    target: CALENDAR_LIVE_V12_TARGET,
    signedAtRuntime: true,
    requestCount: CALENDAR_LIVE_V12_REMOTE_REQUESTS,
    requests: [
      {
        requestId: "writer",
  ...
  if (requests.requestCount !== CALENDAR_LIVE_V12_REMOTE_REQUESTS)
    throw new Error("Calendar live v12 remote request denominator drifted");
  antecedentOutputs.set(
    CALENDAR_LIVE_V12_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const lifecycleExcludedPaths = [
    CALENDAR_LIVE_V12_AUTHORIZATION_TEMPLATE_PATH,
    CALENDAR_LIVE_V12_AUTHORIZATION_PATH,
    "recipe/calendar-live-v12-authorization.ts",
    "recipe/calendar-live-v12-authorization.test.ts",
    "recipe/calendar-live-v12-preflight.ts",
    "recipe/calendar-live-v12-authorized.ts",
    "recipe/write-calendar-live-v12-authorization.ts",
    "recipe/create-calendar-live-v12-security-attestation.ts",
    "recipe/evidence/calendar-live-pivot-v12/operator-security-attestation-template.json",
    "recipe/evidence/calendar-live-pivot-v12-status.json",
    "recipe/evidence/status-index.json",
  ];
  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/calendar-live-v12-broker.ts",
    "recipe/calendar-live-v12-contract.ts",
    "recipe/calendar-live-v12-restore.ts",
    "recipe/calendar-live-v12-cleanup.ts",
    "recipe/calendar-live-v12-verifier.ts",
    "recipe/calendar-live-v12-fixed-point.ts",
    "recipe/run-calendar-live-v12.ts",
    "recipe/build-calendar-live-proof-v12.ts",
    "recipe/calendar-live-v12-lifecycle-simulation.ts",
    ...
    "recipe/normalize.ts",
  ].filter((artifactPath) => artifactPath !== CALENDAR_LIVE_V12_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "calendar-live-v12-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
  ...
  antecedentOutputs.set(CALENDAR_LIVE_V12_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "calendar-live-v12-authorization-template-v1",
    authorizationId: "calendar-live-v12",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: CALENDAR_LIVE_V12_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
    ...
    denominator: {
      remoteRequests: CALENDAR_LIVE_V12_REMOTE_REQUESTS,
      hostPhases: CALENDAR_LIVE_V12_HOST_PHASES,
      captures: CALENDAR_LIVE_V12_CAPTURE_COUNT,
      sourceRoots: CALENDAR_LIVE_V12_SOURCE_ROOTS,
      expectedFacts,
    },
    execution: {
  ...
  const attestationTemplate = jsonBytes({
    artifactVersion: "calendar-live-v12-operator-security-attestation-template-v1",
    status: "template only; complete privately after PREPARE+AUTHORIZE",
    tokenValuesForbidden: true,
    privateDirectory: "private/calendar-live-v12-security-attestation.json",
  });
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Calendar live v12 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(CALENDAR_LIVE_V12_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(CALENDAR_LIVE_V12_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
    writeFileSync(
      `${CALENDAR_LIVE_V12_EVIDENCE_ROOT}/operator-security-attestation-template.json`,
      attestationTemplate,
    );
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(CALENDAR_LIVE_V12_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(CALENDAR_LIVE_V12_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Calendar live v12 authorization template drift");
  }
  if (
    writer.namespace !== CALENDAR_FIGMA_NAMESPACE ||
    writer.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    ...
    throw new Error("Calendar live v12 identity or cleanup invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildCalendarLiveV12Proof(), null, 2)}\n`,
  );
