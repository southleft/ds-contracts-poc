...
export type CalendarLiveV46HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV46AuthorizationArtifact {
  artifactVersion: "calendar-live-v46-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V46_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV46AntecedentIndex {
...
export interface CalendarLiveV46HistoryState {
  index?: CalendarLiveV46AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV46AuthorizationArtifact;
...
export interface CalendarLiveV46AuthorizationProof
  extends CalendarLiveV46TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V46_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V46_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V46_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V46_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v46-authorization") ||
  artifactPath.includes("calendar-live-v46-preflight") ||
  artifactPath.includes("calendar-live-v46-authorized") ||
  artifactPath.includes("create-calendar-live-v46-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v46-status") ||
...
export function validateCalendarLiveV46AntecedentIndex(
  index: CalendarLiveV46AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v46-antecedent-index-v1" ||
  ...
  if (
    index?.counts?.sources !== 1 ||
    index.counts.variants !== 8 ||
    !Number.isInteger(index.counts.expectedSceneFacts) ||
    index.counts.expectedSceneFacts <= 0 ||
    index.counts.captureCells !== 8 ||
    index.counts.remoteRequests !== 13 ||
...
export function validateCalendarLiveV46UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV46History\(\s*["']/.test(source) ||
    /readCalendarLiveV46HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV46AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV46AuthorizationArtifact {
  ...
  ) as CalendarLiveV46AntecedentIndex;
  const indexFailures = validateCalendarLiveV46AntecedentIndex(index);
...
export function validateCalendarLiveV46AuthorizationArtifact(
  artifact: CalendarLiveV46AuthorizationArtifact | undefined,
  index: CalendarLiveV46AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v46-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v46" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V46_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V46_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V46_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V46_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V46_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V46_SOURCE_ROOTS ||
  ...
  if (
    ...
    artifact?.execution?.comboboxLiveV46AuthorizationReusable !== false ||
    artifact?.execution?.comboboxLiveV1AuthorizationReusable !== false ||
    artifact?.execution?.tableLiveV1AuthorizationReusable !== false ||
    artifact?.execution?.calendarLiveV1AuthorizationReusable !== false ||
    artifact?.execution?.calendarLiveV2AuthorizationReusable !== false ||
    artifact?.execution?.calendarLiveV3AuthorizationReusable !== false ||
    artifact?.execution?.calendarLiveV7AuthorizationReusable !== false ||
...
export function validateCalendarLiveV46History(
  state: CalendarLiveV46HistoryState,
  expected: CalendarLiveV46HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV46AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV46AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV46HistoryState(
  root = process.cwd(),
): CalendarLiveV46HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V46_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV46AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V46_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V46_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V46_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV46History(
  expected: CalendarLiveV46HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV46HistoryState {
  const state = readCalendarLiveV46HistoryState(root);
  const failures = validateCalendarLiveV46History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v46 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV46Authorization(
  root = process.cwd(),
): CalendarLiveV46AuthorizationProof {
  const state = verifyCalendarLiveV46History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V46_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v46/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v46.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V46_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V46_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V46_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV46HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV46History(expected);
  process.stdout.write(
    `Calendar live v46 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
