...
export type CalendarLiveV12HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV12AuthorizationArtifact {
  artifactVersion: "calendar-live-v12-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V12_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV12AntecedentIndex {
...
export interface CalendarLiveV12HistoryState {
  index?: CalendarLiveV12AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV12AuthorizationArtifact;
...
export interface CalendarLiveV12AuthorizationProof
  extends CalendarLiveV12TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V12_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V12_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V12_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V12_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v12-authorization") ||
  artifactPath.includes("calendar-live-v12-preflight") ||
  artifactPath.includes("calendar-live-v12-authorized") ||
  artifactPath.includes("create-calendar-live-v12-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v12-status") ||
...
export function validateCalendarLiveV12AntecedentIndex(
  index: CalendarLiveV12AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v12-antecedent-index-v1" ||
...
export function validateCalendarLiveV12UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV12History\(\s*["']/.test(source) ||
    /readCalendarLiveV12HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV12AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV12AuthorizationArtifact {
  ...
  ) as CalendarLiveV12AntecedentIndex;
  const indexFailures = validateCalendarLiveV12AntecedentIndex(index);
...
export function validateCalendarLiveV12AuthorizationArtifact(
  artifact: CalendarLiveV12AuthorizationArtifact | undefined,
  index: CalendarLiveV12AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v12-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v12" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V12_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V12_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V12_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V12_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V12_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V12_SOURCE_ROOTS ||
...
export function validateCalendarLiveV12History(
  state: CalendarLiveV12HistoryState,
  expected: CalendarLiveV12HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV12AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV12AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV12HistoryState(
  root = process.cwd(),
): CalendarLiveV12HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V12_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV12AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V12_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V12_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V12_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV12History(
  expected: CalendarLiveV12HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV12HistoryState {
  const state = readCalendarLiveV12HistoryState(root);
  const failures = validateCalendarLiveV12History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v12 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV12Authorization(
  root = process.cwd(),
): CalendarLiveV12AuthorizationProof {
  const state = verifyCalendarLiveV12History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V12_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v12/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v12.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V12_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V12_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V12_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV12HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV12History(expected);
  process.stdout.write(
    `Calendar live v12 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
