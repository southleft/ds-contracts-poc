...
import { TABLE_LIVE_V5_INDEX_PATH } from "./build-table-live-proof-v5.js";
import {
...
import {
  buildTableLiveV5AuthorizationArtifact,
  validateTableLiveV5AntecedentIndex,
  validateTableLiveV5AuthorizationArtifact,
  validateTableLiveV5History,
  validateTableLiveV5UnitTestSource,
  type TableLiveV5AntecedentIndex,
  type TableLiveV5AuthorizationArtifact,
  type TableLiveV5AuthorizationProof,
  type TableLiveV5HistoryState,
} from "./table-live-v5-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV5ControlFlowSource,
  validateTableLiveV5SecurityAttestation,
  type TableLiveV5SecurityAttestation,
} from "./table-live-v5-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V5_INDEX_PATH);
const index = (): TableLiveV5AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV5AntecedentIndex;

const artifact = (): TableLiveV5AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV5AuthorizationArtifact({
...
const pendingState = (): TableLiveV5HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV5HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV5AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v5/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV5AuthorizationArtifact,
): TableLiveV5SecurityAttestation => {
  const body: Omit<TableLiveV5SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V5_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV5History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV5History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV5History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV5History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV5History(authorizedState(), "pending").join("\n"),
    /stale history mode.*authorization exists/,
  );
});

test("the exact v6 phase-sensitive self-test defect is planted and table v5 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV5UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v5-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV5UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV5AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV5AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV5AuthorizationArtifact(
        planted as TableLiveV5AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV5SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV5SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV5SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v5-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV5ControlFlowSource(source), []);
});
