...
import { INPUT_LIVE_V33_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v33.js";
import { verifyInputLiveV33Authorization } from "./input-field-live-v33-authorization.js";
import { runInputLiveV33Preflight } from "./input-field-live-v33-preflight.js";
import { InputLiveV33Orchestrator } from "./run-input-field-live-v33.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V33_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v33-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV33Authorization(root);
    runInputLiveV33Preflight(
      root,
      authorization,
    ...
    const orchestrator = InputLiveV33Orchestrator.initialize({
      root,
  ...
  const orchestrator = InputLiveV33Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
