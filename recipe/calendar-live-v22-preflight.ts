...
import {
  validateCalendarLiveV22AntecedentIndex,
  verifyCalendarLiveV22Authorization,
  type CalendarLiveV22AntecedentIndex,
  type CalendarLiveV22AuthorizationProof,
} from "./calendar-live-v22-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V22_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v22/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V22_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v22-security-attestation.json";

export interface CalendarLiveV22SecurityAttestation {
  artifactVersion: "calendar-live-v22-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV22PreflightReport {
  artifactVersion: "calendar-live-v22-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV22SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV22SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV22AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v22-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V22_TARGET) ||
...
export function validateCalendarLiveV22ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV22Authorization");
  const preflight = source.indexOf("runCalendarLiveV22Preflight");
  const initialize = source.indexOf("CalendarLiveV22Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV22Preflight(
  root: string,
  proof: CalendarLiveV22AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V22_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV22PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V22_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V22_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV22AntecedentIndex;
  failures.push(...validateCalendarLiveV22AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V22_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V22_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV22SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v22-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV22ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v22 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV22SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v22-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V22_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV22Preflight(
    process.cwd(),
    verifyCalendarLiveV22Authorization(),
    attestationPath,
