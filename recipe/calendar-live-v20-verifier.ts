...
import {
  verifyCalendarLiveV20FixedSceneFixedPoint,
  type CalendarLiveV20FixedFixedPoint,
} from "./calendar-live-v20-fixed-point.js";

export const CALENDAR_LIVE_V20_VERIFIER_VERSION = "calendar-live-v20-verifier-v1";

export function buildCalendarLiveV20RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV20RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV20RawNode[];
}

export interface CalendarLiveV20NormalizedScene {
...
export interface CalendarLiveV20SceneProof {
...
export function normalizeCalendarLiveV20Scene(
  root: CalendarLiveV20RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV20NormalizedScene {
  const canonicalBindings: CalendarLiveV20NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV20RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV20Scene<Instance>(
  normalized: CalendarLiveV20NormalizedScene,
...
): CalendarLiveV20SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV20FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV20ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV20PreCaptureGates(payload: unknown): void {
