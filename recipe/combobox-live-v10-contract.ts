...
import { buildComboboxLiveV10CleanupRuntime } from "./combobox-live-v10-cleanup.js";
import {
  type ComboboxLiveV10FixedFixedPoint,
} from "./combobox-live-v10-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV10RestoreProgram,
  validateComboboxLiveV10RestorePayload,
  type ComboboxLiveV10RestorePayload,
} from "./combobox-live-v10-restore.js";
import {
  buildComboboxLiveV10RawPropertyRuntime,
  normalizeComboboxLiveV10Scene,
  type ComboboxLiveV10RawNode,
} from "./combobox-live-v10-verifier.js";
...
export const COMBOBOX_LIVE_V10_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V10_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V10_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V10_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V10_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V10_SET_COUNT = 4;
export const COMBOBOX_LIVE_V10_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V10_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V10_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V10_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V10_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV10SourceId = (typeof COMBOBOX_LIVE_V10_SOURCE_IDS)[number];

export interface ComboboxLiveV10SourceIdentity {
  source: ComboboxLiveV10SourceId;
...
export interface ComboboxLiveV10WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV10ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV10SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV10RawNode;
    optionScene: ComboboxLiveV10RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV10RootProof {
  source: ComboboxLiveV10SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV10FixedFixedPoint;
}

export interface ComboboxLiveV10ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV10SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV10CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV10SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV10CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV10SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV10CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v10 ${label} must be an object`);
...
export function validateComboboxLiveV10WriterPayload(
  value: unknown,
): ComboboxLiveV10WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V10_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v10 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v10 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v10 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV10SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV10ExtractProgram(
  writer: ComboboxLiveV10WriterOwnership,
  sources: readonly ComboboxLiveV10SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v10 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V10_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V10_NAMESPACE)}
${buildComboboxLiveV10RawPropertyRuntime()}
...
export function validateComboboxLiveV10ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV10WriterOwnership,
): ComboboxLiveV10ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v10 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V10_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v10 extract schema/two-root mismatch");
  return payload as ComboboxLiveV10ExtractPayload;
}

export function proveComboboxLiveV10Roots<Instance>(
  extract: ComboboxLiveV10ExtractPayload,
  sources: readonly (ComboboxLiveV10SourceIdentity & {
...
): ComboboxLiveV10RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v10 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v10 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV10Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV10Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV10FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v10 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v10 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v10 compile lost owned sets");
...
export function assertComboboxLiveV10RootProofs(
  proofs: readonly ComboboxLiveV10RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v10 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v10 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV10ProbeProgram(
  writer: ComboboxLiveV10WriterOwnership,
  sources: readonly ComboboxLiveV10SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V10_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV10ProbePayload(
  value: unknown,
  writer: ComboboxLiveV10WriterOwnership,
): ComboboxLiveV10ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV10ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V10_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V10_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v10 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV10CaptureManifest(
  cells: readonly ComboboxLiveV10CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V10_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V10_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V10_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V10_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v10 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV10CaptureCell[],
): string => {
  validateComboboxLiveV10CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV10CaptureProgram(
  writer: ComboboxLiveV10WriterOwnership,
  cell: ComboboxLiveV10CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V10_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v10 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v10 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V10_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV10CapturePayload(
  value: unknown,
  cell: ComboboxLiveV10CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV10CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV10CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V10_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V10_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v10 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV10CaptureResponses(
  manifest: readonly ComboboxLiveV10CaptureCell[],
  responses: readonly ComboboxLiveV10CapturePayload[],
): void {
  validateComboboxLiveV10CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v10 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV10ObjectiveReport {
  artifactVersion: "combobox-live-v10-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV10Objective(
  manifest: readonly ComboboxLiveV10CaptureCell[],
  responses: readonly ComboboxLiveV10CapturePayload[],
): ComboboxLiveV10ObjectiveReport {
  assertComboboxLiveV10CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v10-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV10CleanupProgram(
  writer: ComboboxLiveV10WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV10CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V10_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v10 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV10CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV10WriterOwnership,
): ComboboxLiveV10CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV10CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v10 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V10_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV10WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
