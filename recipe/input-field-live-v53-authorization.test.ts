...
import { INPUT_LIVE_V53_INDEX_PATH } from "./build-input-field-live-proof-v53.js";
import {
  INPUT_LIVE_V53_DYNAMIC_TOOL,
  INPUT_LIVE_V53_TARGET,
} from "./input-field-live-v53-broker.js";
import {
  buildInputLiveV53AuthorizationArtifact,
  validateInputLiveV53AntecedentIndex,
  validateInputLiveV53AuthorizationArtifact,
  validateInputLiveV53History,
  validateInputLiveV53UnitTestSource,
  type InputLiveV53AntecedentIndex,
  type InputLiveV53AuthorizationArtifact,
  type InputLiveV53AuthorizationProof,
  type InputLiveV53HistoryState,
} from "./input-field-live-v53-authorization.js";
import {
  inputLiveV53AttestationSha256,
  validateInputLiveV53ControlFlowSource,
  validateInputLiveV53SecurityAttestation,
  type InputLiveV53SecurityAttestation,
} from "./input-field-live-v53-preflight.js";
import { inputLiveV53Sha256 } from "./input-field-live-v53-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V53_INDEX_PATH);
const index = (): InputLiveV53AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV53AntecedentIndex;

const artifact = (): InputLiveV53AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV53AuthorizationArtifact({
...
const pendingState = (): InputLiveV53HistoryState => ({
  index: index(),
  indexSha256: inputLiveV53Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV53HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV53AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV53Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V53_TARGET,
  expectedDynamicTool: INPUT_LIVE_V53_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v53/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV53AuthorizationArtifact,
): InputLiveV53SecurityAttestation => {
  const body: Omit<InputLiveV53SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v53-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V53_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV53AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV53History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV53History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV53History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV53History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV53History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV53UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV53UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV53AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV53Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV53AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV53AuthorizationArtifact(
        planted as InputLiveV53AuthorizationArtifact,
        index(),
        inputLiveV53Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV53SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV53AttestationSha256(
      body as Omit<InputLiveV53SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV53SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV53ControlFlowSource(source), []);
});
