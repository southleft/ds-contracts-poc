...
import {
  ...
  verifyInputLiveV34Authorization,
  type InputLiveV34AntecedentIndex,
  type InputLiveV34AuthorizationProof,
} from "./input-field-live-v34-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V34_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v34/operator-security-attestation-template.json";
export const INPUT_LIVE_V34_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v34-security-attestation.json";

export interface InputLiveV34SecurityAttestation {
...
export interface InputLiveV34PreflightReport {
  artifactVersion: "input-live-v34-preflight-v1";
...
export const inputLiveV34AttestationSha256 = (
  value: Omit<InputLiveV34SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV34SecurityAttestation(
  value: unknown,
  proof: InputLiveV34AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v34-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V34_TARGET) ||
...
export function validateInputLiveV34ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV34Authorization");
  const preflight = source.indexOf("runInputLiveV34Preflight");
  const initialize = source.indexOf("InputLiveV34Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV34Preflight(
  root: string,
  proof: InputLiveV34AuthorizationProof,
  attestationPath = INPUT_LIVE_V34_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV34PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V34_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V34_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V34_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV34AntecedentIndex;
  failures.push(...validateInputLiveV34AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V34_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V34_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v34-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV34ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV34SecurityAttestation;
  return {
    artifactVersion: "input-live-v34-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V34_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV34Preflight(
    process.cwd(),
    verifyInputLiveV34Authorization(),
    attestationPath,
