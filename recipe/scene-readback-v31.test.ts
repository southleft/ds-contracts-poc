...
import {
  INPUT_LIVE_V31_CONTENT_BINDING_COMPILE_ORDER_MARKER,
  INPUT_LIVE_V31_CONTENT_BINDING_EXTRAS_MARKER,
  INPUT_LIVE_V31_CONTENT_HIDDEN_HEIGHT_HUG_MARKER,
  INPUT_LIVE_V31_CONTENT_LETTER_SPACING_OMITTED_MARKER,
  INPUT_LIVE_V31_CONTENT_ROW_CLIPS_CONTENT_OMITTED_MARKER,
  INPUT_LIVE_V31_CONTENT_TEXT_CASE_OMITTED_MARKER,
  INPUT_LIVE_V31_CONTENT_TEXT_DECORATION_OMITTED_MARKER,
  INPUT_LIVE_V31_CONTENT_COMPILE_BINDING_FIELDS,
  INPUT_LIVE_V31_CONTENT_ROLES,
  INPUT_LIVE_V31_SLOT_COLOR_BINDING_MARKER,
  INPUT_LIVE_V31_SLOT_FILL_MARKER,
  INPUT_LIVE_V31_SLOT_ROLES,
  INPUT_LIVE_V31_SURFACE_BINDING_COMPILE_ORDER_MARKER,
  INPUT_LIVE_V31_SURFACE_BINDING_EXTRAS_MARKER,
  INPUT_LIVE_V31_SURFACE_COMPILE_BINDING_FIELDS,
  INPUT_LIVE_V31_SURFACE_LAYOUT_HEIGHT_ALIAS_MARKER,
  INPUT_LIVE_V31_SURFACE_LAYOUT_HEIGHT_MARKER,
  INPUT_LIVE_V31_SURFACE_STROKE_WEIGHT_MARKER,
  INPUT_LIVE_V31_VARIANT_LAYOUT_WIDTH_ALIAS_MARKER,
  INPUT_LIVE_V31_VARIANT_LAYOUT_WIDTH_COMPILE_INDEX_MARKER,
  INPUT_LIVE_V31_VARIANT_LAYOUT_WIDTH_MARKER,
...
test("v17 surfaces empty leading-slot fills from instancePayload", () => {
  ...
  assert.equal(INPUT_LIVE_V31_SLOT_FILL_MARKER, "INPUT-SLOT-FILL-FROM-PAYLOAD");
  assert.deepEqual([...INPUT_LIVE_V31_SLOT_ROLES], [
...
test("v26 drops duplicate mapped content color bindings", () => {
  ...
  assert.deepEqual([...INPUT_LIVE_V31_CONTENT_ROLES], [
...
test("v31 recovers hug height for hidden content text with FIXED vertical sizing", () => {
  const ir = sceneToNormalizedIr({
    ...contentScene("input-field/content/placeholder", [
      {
        field: "fills.0.color",
...
test("v31 does not rewrite visible content FIXED height or invent a height variable", () => {
  const ir = sceneToNormalizedIr({
    ...contentScene("input-field/content/value", [
      {
        field: "fontSize.0",
...
test("v31 does not lift hidden FIXED height onto non-content text", () => {
  const ir = sceneToNormalizedIr({
    ownershipKey: "label",
    type: "TEXT",
...
test("v31 omits content letterSpacing that compile never emits", () => {
  const ir = sceneToNormalizedIr({
    ...contentScene("input-field/content/placeholder", [
      {
        field: "fontSize.0",
...
test("v31 does not omit letterSpacing on non-content text", () => {
  const ir = sceneToNormalizedIr({
    ownershipKey: "label",
    type: "TEXT",
    name: "input-field/label :: Label",
...
test("v31 omits content textCase that compile never emits", () => {
  const ir = sceneToNormalizedIr({
    ...contentScene("input-field/content/placeholder", [
      {
        field: "fontSize.0",
...
test("v31 does not omit textCase on non-content text", () => {
  const ir = sceneToNormalizedIr({
    ownershipKey: "label",
    type: "TEXT",
    name: "input-field/label :: Label",
...
test("v31 omits content textDecoration that compile never emits", () => {
  const ir = sceneToNormalizedIr({
    ...contentScene("input-field/content/placeholder", [
      {
        field: "fontSize.0",
...
test("v31 does not omit textDecoration on non-content text", () => {
  const ir = sceneToNormalizedIr({
    ownershipKey: "label",
    type: "TEXT",
    name: "input-field/label :: Label",
...
test("v31 omits content-row clipsContent that compile never emits", () => {
  const ir = sceneToNormalizedIr({
    ...surfaceScene([]),
    name: "input-field/content-row :: Row",
    clipsContent: true,
...
test("v31 does not omit clipsContent on labels", () => {
  const ir = sceneToNormalizedIr({
    ...surfaceScene([]),
