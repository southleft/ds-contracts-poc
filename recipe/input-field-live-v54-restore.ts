...
export const INPUT_LIVE_V54_MEASURE_VISIBLE_FILL_MARKER =
  "INPUT-TEXT-FILL-MEASURE-VISIBLE";
export const INPUT_LIVE_V54_RESTORE_CONTENT_ROLES = [
  "input-field/content/placeholder",
  "input-field/content/value",
] as const;
export const INPUT_LIVE_V54_RESTORE_PARENT_ROLES = [
  "input-field/surface",
  "input-field/content-row",
] as const;

export interface InputLiveV54RestoreWriter {
  pageId: string;
  runIdentity: string;
  setIds: readonly [string, string];
}

export interface InputLiveV54RestorePayload {
  pageId: string;
  setIds: string[];
  restoredCount: 256;
  fixedBefore: number;
  ...
  marker: typeof INPUT_LIVE_V54_MEASURE_VISIBLE_FILL_MARKER;
}

export function buildInputLiveV54RestoreProgram(
  writer: InputLiveV54RestoreWriter,
): string {
  if (writer.setIds.length !== 2)
    throw new TypeError("Input live v54 restore requires two owned sets");
  return String.raw`
await figma.loadAllPagesAsync();
const V54_NS=${JSON.stringify(INPUT_LIVE_V54_NAMESPACE)};
const V54_PAGE_ID=${JSON.stringify(writer.pageId)};
const V54_SET_IDS=${JSON.stringify([...writer.setIds])};
const V54_ROLES=new Set(${JSON.stringify([...INPUT_LIVE_V54_RESTORE_CONTENT_ROLES])});
const V54_PARENTS=new Set(${JSON.stringify([...INPUT_LIVE_V54_RESTORE_PARENT_ROLES])});
const V54_MARKER=${JSON.stringify(INPUT_LIVE_V54_MEASURE_VISIBLE_FILL_MARKER)};
const page=await figma.getNodeByIdAsync(V54_PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("INPUT-V54-RESTORE-PAGE");
if(page.getSharedPluginData(V54_NS,"pageOwner")!=="recipe/input-field/"+${JSON.stringify(writer.runIdentity)})throw new Error("INPUT-V54-RESTORE-OWNER");
...
export function validateInputLiveV54RestorePayload(
  payload: unknown,
  writer: InputLiveV54RestoreWriter,
): InputLiveV54RestorePayload {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("Input live v54 restore payload must be an object");
  ...
  if (
    ...
    value.marker !== INPUT_LIVE_V54_MEASURE_VISIBLE_FILL_MARKER
  )
    throw new TypeError(
      "Input live v54 restore did not re-assert content FILL",
    );
  return value as InputLiveV54RestorePayload;
}
