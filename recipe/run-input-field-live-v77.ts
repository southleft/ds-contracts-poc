...
import {
  acceptInputLiveV77Response,
  createInputLiveV77Transaction,
  inputLiveV77ReceiptPath,
  inputLiveV77RequestPath,
  inputLiveV77ResponsePath,
  inputLiveV77Sha256,
  issueInputLiveV77Request,
  persistInputLiveV77RawResponse,
  persistInputLiveV77TechnicalGates,
  readInputLiveV77AcceptedRawPayload,
  type InputLiveV77Request,
  type InputLiveV77TransactionAuthorization,
} from "./input-field-live-v77-broker.js";
import {
  INPUT_LIVE_V77_CAPTURE_COUNT,
  assertInputLiveV77CaptureResponses,
  assertInputLiveV77RootProofs,
  buildInputLiveV77CaptureProgram,
  buildInputLiveV77CleanupProgram,
  buildInputLiveV77ExtractProgram,
  buildInputLiveV77ProbeProgram,
  buildInputLiveV77RestoreProgram,
  validateInputLiveV77RestorePayload,
  evaluateInputLiveV77Objective,
  inputLiveV77CaptureManifestSha256,
  proveInputLiveV77Roots,
  validateInputLiveV77CaptureManifest,
  validateInputLiveV77CapturePayload,
  validateInputLiveV77CleanupPayload,
  validateInputLiveV77ExtractPayload,
  validateInputLiveV77ProbePayload,
  validateInputLiveV77WriterPayload,
  type InputLiveV77CaptureCell,
  type InputLiveV77CapturePayload,
  type InputLiveV77RootProof,
  type InputLiveV77SourceIdentity,
  type InputLiveV77WriterOwnership,
} from "./input-field-live-v77-contract.js";
import {
  INPUT_LIVE_V77_CAPTURE_MANIFEST_PATH,
  INPUT_LIVE_V77_PLAN_PATH,
  type InputLiveV77ProofPlan,
} from "./build-input-field-live-proof-v77.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-v77.js";

export const INPUT_LIVE_V77_RUNNER_VERSION = "input-live-v77-runner-v1";

interface InputLiveV77JournalEntry {
  index: number;
  event: string;
...
interface InputLiveV77OrchestratorState {
  artifactVersion: "input-live-v77-orchestrator-state-v1";
  ...
  journal: InputLiveV77JournalEntry[];
}

interface LoadedProofInputs {
  plan: InputLiveV77ProofPlan;
  planSha256: string;
  captures: InputLiveV77CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    InputLiveV77SourceIdentity & {
...
export interface InputLiveV77NextAction {
...
const writeState = (
  directory: string,
  state: InputLiveV77OrchestratorState,
): void => {
...
const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, INPUT_LIVE_V77_PLAN_PATH);
  const capturePath = path.join(root, INPUT_LIVE_V77_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as InputLiveV77ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as InputLiveV77CaptureCell[];
  validateInputLiveV77CaptureManifest(captures);
  if (
    plan.captureManifest.sha256 !== inputLiveV77Sha256(captureBytes) ||
    captureArtifact.cellsSha256 !== inputLiveV77CaptureManifestSha256(captures)
  )
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (
      inputLiveV77Sha256(expectedBytes) !== source.expectedScenePlan.sha256 ||
      inputLiveV77Sha256(uncompressed) !==
  ...
  return {
    plan,
    planSha256: inputLiveV77Sha256(planBytes),
    captures,
    captureManifestSha256: inputLiveV77Sha256(captureBytes),
    sources,
...
const appendJournal = (
  ...
  const payloadSha256 = inputLiveV77Sha256(canonicalJson(payload));
  const body = {
  ...
  const entry = {
    ...body,
    entrySha256: inputLiveV77Sha256(canonicalJson(body)),
  };
  writeNew(
    path.join(
      directory,
...
const writerOwnership = (directory: string): InputLiveV77WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): InputLiveV77RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): InputLiveV77Request =>
  readJson<InputLiveV77Request>(file);

export class InputLiveV77Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueInputLiveV77Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: InputLiveV77OrchestratorState = {
      artifactVersion: "input-live-v77-orchestrator-state-v1",
      transactionDirectory: options.transactionDirectory,
      activeRequest: inputLiveV77RequestPath(
        options.transactionDirectory,
        "writer",
      ),
      cleanupRequest: null,
    ...
    return new InputLiveV77Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<InputLiveV77OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new InputLiveV77Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): InputLiveV77NextAction {
    const cleanup =
      this.#state.cleanupRequest &&
      !this.#state.cleanupAccepted &&
      !existsSync(inputLiveV77ReceiptPath(this.#directory, "cleanup"))
        ? this.#state.cleanupRequest
        : null;
  ...
  ingestAndAdvance(rawResponse: string): InputLiveV77NextAction {
  ...
  ingestCleanup(rawResponse: string): InputLiveV77NextAction {
    ...
    persistInputLiveV77RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptInputLiveV77Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV77CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      existsSync(inputLiveV77ReceiptPath(this.#directory, "writer")) &&
      !existsSync(path.join(this.#directory, OWNERSHIP_FILE))
    ) {
      const raw = readInputLiveV77AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateInputLiveV77WriterPayload(raw),
      );
    }
    if (existsSync(inputLiveV77RequestPath(this.#directory, "cleanup")))
      this.#state.cleanupRequest = inputLiveV77RequestPath(
        this.#directory,
        "cleanup",
      );
    if (
      existsSync(path.join(this.#directory, OWNERSHIP_FILE)) &&
      !existsSync(inputLiveV77RequestPath(this.#directory, "cleanup"))
    )
      this.#issueCleanupAndRestore(false);
    else if (
      existsSync(inputLiveV77RequestPath(this.#directory, "cleanup")) &&
      !existsSync(inputLiveV77RequestPath(this.#directory, "restore"))
    )
      this.#issueRestore(false);
    if (
      existsSync(inputLiveV77RequestPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV77ReceiptPath(this.#directory, "restore"))
    )
      this.#state.activeRequest = inputLiveV77RequestPath(
        this.#directory,
        "restore",
      );
    if (
      existsSync(inputLiveV77ReceiptPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV77RequestPath(this.#directory, "extract"))
    ) {
      const ownership = writerOwnership(this.#directory);
      const extract = issueInputLiveV77Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildInputLiveV77ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV77RequestPath(
        this.#directory,
        "extract",
    ...
    if (existsSync(inputLiveV77ReceiptPath(this.#directory, "cleanup")))
      this.#state.cleanupAccepted = true;
    this.#persist();
  }

  #acceptActive(request: InputLiveV77Request): void {
    if (request.phase === "writer") {
      const accepted = acceptInputLiveV77Response(this.#directory, "writer", {
        validate: validateInputLiveV77WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptInputLiveV77Response(this.#directory, "restore", {
        validate: (payload) =>
          validateInputLiveV77RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptInputLiveV77Response(this.#directory, "extract", {
        validate: (payload) =>
          validateInputLiveV77ExtractPayload(payload, ownership),
      });
      const proofs = proveInputLiveV77Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseInputFieldRecipe,
        compileInputFieldRecipe,
      );
      assertInputLiveV77RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "input-live-v77-host-root-proofs-v1",
      ...
      appendJournal(
        ...
        request.requestId,
        { proofsSha256: inputLiveV77Sha256(canonicalJson(proofs)) },
      );
      const next = issueInputLiveV77Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildInputLiveV77ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV77RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptInputLiveV77Response(this.#directory, "probe", {
        validate: (payload) =>
          validateInputLiveV77ProbePayload(payload, ownership),
      });
      persistInputLiveV77TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      const accepted = acceptInputLiveV77Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateInputLiveV77CapturePayload(payload, cell, rawBytes),
      });
      const png = Buffer.from(accepted.payload.pngBase64, "base64");
      const capturePath = path.join(
        this.#directory,
        "captures",
        `${String(cell.index).padStart(3, "0")}-${inputLiveV77Sha256(cell.cellKey).slice(0, 20)}.png`,
      );
      ...
      if (next < INPUT_LIVE_V77_CAPTURE_COUNT) this.#issueCapture(next);
      else {
          ...
          pngBase64: Buffer.from(
            readFileSync(
              path.join(
                this.#directory,
                "captures",
                `${String(response.index).padStart(3, "0")}-${inputLiveV77Sha256(response.cellKey).slice(0, 20)}.png`,
              ),
            ),
          ).toString("base64"),
        }));
        assertInputLiveV77CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateInputLiveV77Objective(
        ...
        appendJournal(
          ...
          request.requestId,
          {
            reportSha256: inputLiveV77Sha256(canonicalJson(objective)),
  ...
  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueInputLiveV77Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildInputLiveV77CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = inputLiveV77RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueInputLiveV77Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildInputLiveV77RestoreProgram(ownership),
    );
    this.#state.activeRequest = inputLiveV77RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueInputLiveV77Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildInputLiveV77CaptureProgram(
  ...
  #readCaptureResponseIndex(): InputLiveV77CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: InputLiveV77CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedInputLiveV77Authorization = (
  privateKey: KeyObject,
): InputLiveV77TransactionAuthorization => ({
  mode: "simulated",
  ...
  signingPublicKeySha256: inputLiveV77Sha256(
...
export const generateInputLiveV77SimulationKey = (): KeyObject =>
