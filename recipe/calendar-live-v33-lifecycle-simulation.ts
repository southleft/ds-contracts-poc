...
import { calendarLiveV1Sha256 } from "./calendar-live-v33-broker.js";

export interface CalendarLiveV33LifecycleSimulationReport {
  artifactVersion: "calendar-live-v33-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV33AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV33Lifecycle(): Promise<CalendarLiveV33LifecycleSimulationReport> {
  await buildCalendarLiveV33Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V33_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV33AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v33-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V33_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V33_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV33History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV33History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV33AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V33_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V33_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV33History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV33History(authorized, "pending");
    ...
    await buildCalendarLiveV33Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V33_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV33AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV33Lifecycle(), null, 2)}\n`,
  );
