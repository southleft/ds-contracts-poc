...
import {
  ...
  INPUT_LIVE_V29_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V29_RESPONSE_CONTRACTS,
  type InputLiveV29RootProof,
} from "./input-field-live-v29-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V29_BROKER_VERSION = "input-live-v29-broker-v2";
export const INPUT_LIVE_V29_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V29_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V29_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V29_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V29_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V29_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV29RemotePhase =
  (typeof INPUT_LIVE_V29_REMOTE_PHASES)[number];
export type InputLiveV29AuthorizationMode = "live" | "simulated";

export interface InputLiveV29CommitPins {
...
export interface InputLiveV29TransactionAuthorization extends InputLiveV29CommitPins {
  mode: InputLiveV29AuthorizationMode;
...
export interface InputLiveV29Manifest {
  artifactVersion: "input-live-v29-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V29_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V29_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V29_TARGET;
...
export interface InputLiveV29ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV29RequestBody {
  artifactVersion: "input-live-v29-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V29_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV29RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV29CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V29_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV29ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV29Request extends InputLiveV29RequestBody {
...
export interface InputLiveV29Receipt {
  artifactVersion: "input-live-v29-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V29_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV29RemotePhase;
...
export interface InputLiveV29AcceptedResponse<T = unknown> {
  request: InputLiveV29Request;
  receipt: InputLiveV29Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV29IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV29ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V29_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V29_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V29_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V29_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V29_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V29_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V29_TARGET.editorType;
};
...
export const inputLiveV29RequestSequence = (
  phase: InputLiveV29RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV29RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV29RequestPath = (
  directory: string,
  phase: InputLiveV29RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV29ResponsePath = (
  directory: string,
  phase: InputLiveV29RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV29ReceiptPath = (
  directory: string,
  phase: InputLiveV29RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV29Manifest => {
  const manifest = json<InputLiveV29Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v29-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V29_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V29_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV29RemotePhase,
  captureIndex?: number,
): InputLiveV29Request | undefined => {
  const file = inputLiveV29RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV29Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV29RemotePhase,
  captureIndex?: number,
): InputLiveV29Receipt | undefined => {
  const file = inputLiveV29ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV29Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV29Request | undefined;
  receipt: InputLiveV29Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV29RemotePhase,
): InputLiveV29ResponseContract => ({
  ...INPUT_LIVE_V29_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V29_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V29_DYNAMIC_TOOL,
      target: INPUT_LIVE_V29_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v29-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V29_BROKER_VERSION,
  target: INPUT_LIVE_V29_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V29_PROGRAM_START*/${program}/*INPUT_LIVE_V29_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V29_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V29_PROGRAM_END*/";
...
export function createInputLiveV29Transaction(
  directory: string,
  ...
  const manifest: InputLiveV29Manifest = {
    artifactVersion: "input-live-v29-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V29_BROKER_VERSION,
...
export function validateInputLiveV29Request(
  directory: string,
  request: InputLiveV29Request,
): void {
  ...
  const expectedSequence = inputLiveV29RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v29-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V29_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V29_TARGET.fileKey ||
...
export function issueInputLiveV29Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV29RemotePhase,
  program: string,
  options: InputLiveV29IssueOptions = {},
): InputLiveV29Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV29RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV29Request>(targetPath);
    validateInputLiveV29Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV29ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV29TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV29RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V29_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V29_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V29_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV29RequestBody = {
    artifactVersion: "input-live-v29-broker-request-v2",
    brokerVersion: INPUT_LIVE_V29_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV29Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV29Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV29RawResponse(
  directory: string,
  phase: InputLiveV29RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV29RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV29Request>(requestFile);
  if (
  ...
  const target = inputLiveV29ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV29Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v29-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V29_BROKER_VERSION,
...
export function acceptInputLiveV29Response<T = unknown>(
  directory: string,
  phase: InputLiveV29RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV29AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV29ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV29Request>(
    inputLiveV29RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV29Request(directory, request);
  if (phase === "capture") validateInputLiveV29TechnicalGates(directory);
  const rawFile = inputLiveV29ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v29-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V29_BROKER_VERSION ||
...
export function persistInputLiveV29TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV29RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV29RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V29_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v29-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V29_BROKER_VERSION,
...
export function validateInputLiveV29TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v29-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V29_TECHNICAL_GATES.some(
...
export function validateInputLiveV29Receipt(
  directory: string,
  phase: InputLiveV29RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV29Request(directory, request);
  const receipt = json<InputLiveV29Receipt>(
    inputLiveV29ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV29ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v29-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V29_BROKER_VERSION ||
...
export function readInputLiveV29AcceptedRawPayload(
  directory: string,
  phase: InputLiveV29RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV29Receipt(directory, phase, captureIndex);
...
export function createInputLiveV29FakeDynamicResponse(
  request: InputLiveV29Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V29_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V29_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV29PersistedRequests(
  directory: string,
): InputLiveV29Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV29Request>(file)] : [];
    });
}

export const inputLiveV29Sha256 = sha256;
