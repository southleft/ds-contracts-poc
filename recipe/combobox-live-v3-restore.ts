...
export const COMBOBOX_LIVE_V3_MEASURE_VISIBLE_FILL_MARKER =
  "COMBOBOX-TEXT-FILL-MEASURE-VISIBLE";
export const COMBOBOX_LIVE_V3_RESTORE_CONTENT_ROLES = [
  "combobox/input",
  "combobox/option/label",
] as const;
export const COMBOBOX_LIVE_V3_RESTORE_PARENT_ROLES = [
  "combobox/trigger",
] as const;
export const COMBOBOX_LIVE_V3_RESTORE_INPUT_SET_ROLE = "combobox/set";
export const COMBOBOX_LIVE_V3_RESTORE_OPTION_LABEL_SET_ROLE =
  "combobox/option-set";
export const COMBOBOX_LIVE_V3_RESTORE_COUNT = 144;
export const FORBIDDEN_INPUT_PAGE_ID = "115:295378";

export interface ComboboxLiveV3RestoreWriter {
  pageId: string;
  runIdentity: string;
  setIds: readonly string[];
}

export interface ComboboxLiveV3RestorePayload {
  pageId: string;
  setIds: string[];
  restoredCount: 144;
  fixedBefore: number;
  ...
  marker: typeof COMBOBOX_LIVE_V3_MEASURE_VISIBLE_FILL_MARKER;
}

export function buildComboboxLiveV3RestoreProgram(
  writer: ComboboxLiveV3RestoreWriter,
): string {
  if (writer.setIds.length !== 4)
    throw new TypeError("Combobox live v3 restore requires four owned sets");
...
const ROLES=new Set(${JSON.stringify([...COMBOBOX_LIVE_V3_RESTORE_CONTENT_ROLES])});
const PARENTS=new Set(${JSON.stringify([...COMBOBOX_LIVE_V3_RESTORE_PARENT_ROLES])});
const INPUT_SET=${JSON.stringify(COMBOBOX_LIVE_V3_RESTORE_INPUT_SET_ROLE)};
const OPTION_SET=${JSON.stringify(COMBOBOX_LIVE_V3_RESTORE_OPTION_LABEL_SET_ROLE)};
const MARKER=${JSON.stringify(COMBOBOX_LIVE_V3_MEASURE_VISIBLE_FILL_MARKER)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("COMBOBOX-V3-RESTORE-PAGE");
if(page.id==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
if(page.getSharedPluginData(NS,"pageOwner")!=="recipe/combobox/"+${JSON.stringify(writer.runIdentity)})throw new Error("COMBOBOX-V3-RESTORE-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>SET_IDS.includes(node.id));
if(sets.length!==4)throw new Error("COMBOBOX-V3-RESTORE-ROOTS:"+sets.length);
const parents=[];
const texts=[];
...
if(restored.length!==144)throw new Error("COMBOBOX-V3-RESTORE-COUNT:"+restored.length);
if(restored.some(entry=>entry.after!=="FILL"))throw new Error("COMBOBOX-V3-RESTORE-NOT-FILL");
...
export function validateComboboxLiveV3RestorePayload(
  payload: unknown,
  writer: ComboboxLiveV3RestoreWriter,
): ComboboxLiveV3RestorePayload {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("Combobox live v3 restore payload must be an object");
  ...
  if (
    ...
    value.marker !== COMBOBOX_LIVE_V3_MEASURE_VISIBLE_FILL_MARKER
  )
    throw new TypeError(
      "Combobox live v3 restore did not re-assert content FILL",
    );
  return value as ComboboxLiveV3RestorePayload;
}
