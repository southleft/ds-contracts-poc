...
import {
  TABLE_LIVE_V2_DYNAMIC_TOOL,
  TABLE_LIVE_V2_TARGET,
  type TableLiveV2TransactionAuthorization,
} from "./table-live-v2-broker.js";
import {
  TABLE_LIVE_V2_AUTHORIZATION_PATH,
  TABLE_LIVE_V2_INDEX_PATH,
} from "./build-table-live-proof-v2.js";
import {
  TABLE_LIVE_V2_CAPTURE_COUNT,
  TABLE_LIVE_V2_HOST_PHASES,
  TABLE_LIVE_V2_REMOTE_REQUESTS,
  TABLE_LIVE_V2_SOURCE_ROOTS,
} from "./table-live-v2-contract.js";
import { canonicalJson } from "./normalize.js";

export type TableLiveV2HistoryExpectation = "pending" | "authorized";

export interface TableLiveV2AuthorizationArtifact {
  artifactVersion: "table-live-v2-capture-authorization-v1";
  authorizationId: "table-live-v2";
  status: "authorization declared; runtime security prerequisites still mandatory";
  authorizationIntent: true;
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V2_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV2AntecedentIndex {
  artifactVersion: "table-live-v2-antecedent-index-v1";
  status: string;
...
export interface TableLiveV2HistoryState {
  index?: TableLiveV2AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV2AuthorizationArtifact;
...
export interface TableLiveV2AuthorizationProof
  extends TableLiveV2TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V2_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V2_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V2_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V2_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v2-authorization") ||
  artifactPath.includes("table-live-v2-preflight") ||
  artifactPath.includes("table-live-v2-authorized") ||
  artifactPath.includes("create-table-live-v2-security-attestation") ||
  artifactPath.includes("table-live-pivot-v2-status") ||
...
export function validateTableLiveV2AntecedentIndex(
  index: TableLiveV2AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v2-antecedent-index-v1" ||
    index.authorizationCanBeAddedWithoutAntecedentRebuild !== true
  )
    failures.push("table v2 antecedent index missing or malformed");
  const artifactEntries = Object.entries(index?.artifacts ?? {});
  if (artifactEntries.length === 0)
    failures.push("table v2 antecedent hash denominator is empty");
  for (const [artifactPath, metadata] of artifactEntries) {
    ...
    if (
      ...
      failures.push(`malformed table v2 antecedent hash: ${artifactPath}`);
  }
  ...
  if (index?.hashSetSha256 !== expectedHashSet)
    failures.push("table v2 antecedent hash-set digest mismatch");
  if (
    ...
    failures.push("table v2 antecedent counts missing or drifted");
  for (const required of [
    TABLE_LIVE_V2_AUTHORIZATION_PATH,
    "recipe/table-live-v2-authorization.ts",
    "recipe/table-live-v2-authorization.test.ts",
    "recipe/table-live-v2-preflight.ts",
    "recipe/evidence/table-live-pivot-v2/operator-security-attestation-template.json",
    "recipe/evidence/status-index.json",
  ])
    if (!index?.authorizationLifecycleExcluded?.includes(required))
      failures.push(`table v2 lifecycle exclusion missing: ${required}`);
  const leaks = forbiddenAntecedentFieldPaths(index);
  if (leaks.length)
    failures.push(`table v2 antecedent result leakage: ${leaks.join(",")}`);
  return failures;
}

export function validateTableLiveV2UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV2History\(\s*["']/.test(source) ||
    /readTableLiveV2HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV2AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV2AuthorizationArtifact {
  if (!SHA40.test(options.antecedentCommit))
    throw new TypeError("table v2 antecedent commit must be a full SHA");
  const index = JSON.parse(
    Buffer.from(options.antecedentIndexBytes).toString("utf8"),
  ) as TableLiveV2AntecedentIndex;
  const indexFailures = validateTableLiveV2AntecedentIndex(index);
  if (indexFailures.length)
    throw new TypeError(
      `table v2 antecedent index refused:\n${indexFailures.join("\n")}`,
    );
  const publicKey =
  ...
  return {
    ...
    antecedent: {
      ...
      indexPath: TABLE_LIVE_V2_INDEX_PATH,
    ...
    denominator: {
      remoteRequests: TABLE_LIVE_V2_REMOTE_REQUESTS,
      hostPhases: TABLE_LIVE_V2_HOST_PHASES,
      captures: TABLE_LIVE_V2_CAPTURE_COUNT,
      sourceRoots: TABLE_LIVE_V2_SOURCE_ROOTS,
...
export function validateTableLiveV2AuthorizationArtifact(
  artifact: TableLiveV2AuthorizationArtifact | undefined,
  index: TableLiveV2AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v2-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v2" ||
    artifact.authorizationIntent !== true
  )
    failures.push("table v2 authorization declaration missing or malformed");
  const resultLeaks = forbiddenAntecedentFieldPaths(artifact);
  if (resultLeaks.length)
    failures.push(`table v2 authorization result leakage: ${resultLeaks.join(",")}`);
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V2_INDEX_PATH ||
    artifact?.antecedent?.indexSha256 !== indexSha256 ||
    artifact?.antecedent?.hashSetSha256 !== index?.hashSetSha256
  )
    failures.push("table v2 authorization pins wrong antecedent index");
  if (privateMaterialPaths(artifact).length)
    failures.push("table v2 authorization contains private key or token material");
  try {
    ...
    if (
      ...
      failures.push("table v2 Ed25519 public-key identity mismatch");
  } catch {
    failures.push("table v2 Ed25519 public key is invalid");
  }
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V2_TARGET) ||
    !equalJson(
    ...
    failures.push("wrong table v2 target or dynamic tool");
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V2_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V2_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V2_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V2_SOURCE_ROOTS ||
    artifact?.denominator?.expectedFacts !== index?.counts.expectedSceneFacts
  )
    failures.push("table v2 two-root or capture denominator changed");
  if (
    ...
    failures.push(
      "table v2 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
    ...
    failures.push("table v2 security prerequisites weakened");
  if (
    artifact?.humanSignoff?.mandatory !== true ||
    artifact?.humanSignoff?.status !== "pending"
  )
    failures.push("table v2 human signoff overclaim");
  return failures;
}

export function validateTableLiveV2History(
  state: TableLiveV2HistoryState,
  expected: TableLiveV2HistoryExpectation,
): string[] {
  const failures = validateTableLiveV2AntecedentIndex(state.index);
  if (!SHA40.test(state.codeCommit))
    failures.push("table v2 code commit is not a full SHA");
  ...
  if (
    ...
    failures.push("table v2 antecedent first-add commit is not unique");
  if (!state.antecedentIsAncestorOfCode)
    failures.push("code commit does not descend from table v2 antecedent");
  if (!state.antecedentIndexBytesMatchFirstAddition)
    failures.push("table v2 antecedent index changed after first addition");
  if (!state.antecedentArtifactsMatchIndexAtCommit)
    failures.push("table v2 antecedent commit artifacts do not match index");
  if (!state.workingAntecedentArtifactsMatchIndex)
    failures.push("table v2 antecedent working bytes drifted");
  ...
  failures.push(
    ...validateTableLiveV2AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
  ...
  if (
    ...
    failures.push("table v2 authorization first-add lineage mismatch");
  if (!state.authorizationPresentAtCodeCommit)
    failures.push("table v2 authorization missing from code commit");
  if (!state.authorizationBytesMatchFirstAddition)
    failures.push("table v2 authorization bytes changed after first addition");
  if (!state.authorizationStrictlyDescendsFromAntecedent)
    failures.push("table v2 authorization does not strictly descend from antecedent");
  if (!state.authorizationIsAncestorOfCode)
    failures.push("code commit predates table v2 authorization");
  return failures;
}
...
export function readTableLiveV2HistoryState(
  root = process.cwd(),
): TableLiveV2HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V2_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV2AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V2_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V2_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V2_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV2History(
  expected: TableLiveV2HistoryExpectation,
  root = process.cwd(),
): TableLiveV2HistoryState {
  const state = readTableLiveV2HistoryState(root);
  const failures = validateTableLiveV2History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v2 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV2Authorization(
  root = process.cwd(),
): TableLiveV2AuthorizationProof {
  const state = verifyTableLiveV2History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V2_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v2/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v2.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V2_TARGET,
    expectedDynamicTool: TABLE_LIVE_V2_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V2_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV2HistoryExpectation => {
  ...
  if (pending === authorized)
    throw new Error(
      "Choose exactly one table v2 history mode: --expect-pending or --expect-authorized",
    );
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV2History(expected);
  process.stdout.write(
    `Table live v2 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
