...
import { buildCalendarLiveV15CleanupRuntime } from "./calendar-live-v15-cleanup.js";
import { type CalendarLiveV15FixedFixedPoint } from "./calendar-live-v15-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV15RestoreProgram,
  validateCalendarLiveV15RestorePayload,
  type CalendarLiveV15RestorePayload,
} from "./calendar-live-v15-restore.js";
import {
  buildCalendarLiveV15RawPropertyRuntime,
  normalizeCalendarLiveV15Scene,
  type CalendarLiveV15RawNode,
} from "./calendar-live-v15-verifier.js";
...
export const CALENDAR_LIVE_V15_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V15_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V15_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V15_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V15_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V15_SET_COUNT = 3;
export const CALENDAR_LIVE_V15_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V15_HOST_PHASES = 3;
export const CALENDAR_LIVE_V15_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V15_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V15_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V15_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV15SourceId = (typeof CALENDAR_LIVE_V15_SOURCE_IDS)[number];
export type CalendarLiveV15Kind = "calendar" | "week" | "day";

export interface CalendarLiveV15SourceIdentity {
  source: CalendarLiveV15SourceId;
...
export interface CalendarLiveV15WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV15ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV15SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV15RawNode;
    weekScene: CalendarLiveV15RawNode;
    dayScene: CalendarLiveV15RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV15RootProof {
  source: CalendarLiveV15SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV15FixedFixedPoint;
}

export interface CalendarLiveV15ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV15SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV15CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV15SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV15CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV15SourceId;
  frameWidth: number;
...
export interface CalendarLiveV15CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v15 ${label} must be an object`);
...
export function validateCalendarLiveV15WriterPayload(
  value: unknown,
): CalendarLiveV15WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v15") ||
    ...
    result.namespace !== CALENDAR_LIVE_V15_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v15 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v15 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v15 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV15SourceIdentity,
  kind: CalendarLiveV15Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV15ExtractProgram(
  writer: CalendarLiveV15WriterOwnership,
  sources: readonly CalendarLiveV15SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v15 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V15_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V15_NAMESPACE)}
${buildCalendarLiveV15RawPropertyRuntime()}
...
export function validateCalendarLiveV15ExtractPayload(
  value: unknown,
  writer: CalendarLiveV15WriterOwnership,
): CalendarLiveV15ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v15 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V15_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v15 extract schema/one-root mismatch");
  return payload as CalendarLiveV15ExtractPayload;
}

export function proveCalendarLiveV15Roots<Instance>(
  extract: CalendarLiveV15ExtractPayload,
  sources: readonly (CalendarLiveV15SourceIdentity & {
...
): CalendarLiveV15RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v15 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v15 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV15Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV15Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV15Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV15FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v15 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v15 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v15 compile lost owned sets");
...
export function assertCalendarLiveV15RootProofs(
  proofs: readonly CalendarLiveV15RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v15 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v15 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV15ProbeProgram(
  writer: CalendarLiveV15WriterOwnership,
  sources: readonly CalendarLiveV15SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V15_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV15ProbePayload(
  value: unknown,
  writer: CalendarLiveV15WriterOwnership,
): CalendarLiveV15ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV15ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V15_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V15_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v15 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV15CaptureManifest(
  cells: readonly CalendarLiveV15CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V15_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V15_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V15_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V15_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v15 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV15CaptureCell[],
): string => {
  validateCalendarLiveV15CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV15CaptureProgram(
  writer: CalendarLiveV15WriterOwnership,
  cell: CalendarLiveV15CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V15_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v15 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v15 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V15_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV15CapturePayload(
  value: unknown,
  cell: CalendarLiveV15CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV15CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV15CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V15_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V15_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v15 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV15CaptureResponses(
  manifest: readonly CalendarLiveV15CaptureCell[],
  responses: readonly CalendarLiveV15CapturePayload[],
): void {
  validateCalendarLiveV15CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v15 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV15ObjectiveReport {
  artifactVersion: "calendar-live-v15-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV15Objective(
  manifest: readonly CalendarLiveV15CaptureCell[],
  responses: readonly CalendarLiveV15CapturePayload[],
): CalendarLiveV15ObjectiveReport {
  assertCalendarLiveV15CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v15-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV15CleanupProgram(
  writer: CalendarLiveV15WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV15CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V15_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v15 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV15CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV15WriterOwnership,
): CalendarLiveV15CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV15CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v15 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V15_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV15WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
