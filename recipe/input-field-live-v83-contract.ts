...
} from "./input-field-live-v3-verifier-v83.js";
import {
  buildInputLiveV83RawPropertyRuntime,
  normalizeInputLiveV83Scene,
  type InputLiveV83RawNode,
} from "./input-field-live-v83-verifier.js";
...
export const INPUT_LIVE_V83_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V83_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V83_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V83_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V83_VARIANT_COUNT = 256;
export const INPUT_LIVE_V83_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V83_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV83SourceId = (typeof INPUT_LIVE_V83_SOURCE_IDS)[number];

export interface InputLiveV83SourceIdentity {
  source: InputLiveV83SourceId;
...
export interface InputLiveV83WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV83ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV83SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV83RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV83RootProof {
  source: InputLiveV83SourceId;
  adapterIdentity: string;
...
export interface InputLiveV83ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV83SourceId;
    adapterIdentity: string;
...
export interface InputLiveV83CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV83SourceId;
  adapterIdentity: string;
...
export interface InputLiveV83CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV83SourceId;
  strata: InputLiveV83CaptureCell["strata"];
...
export interface InputLiveV83CleanupPayload {
...
export function validateInputLiveV83WriterPayload(
  value: unknown,
): InputLiveV83WriterOwnership {
...
const identityPlan = (source: InputLiveV83SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV83ExtractProgram(
  writer: InputLiveV83WriterOwnership,
  sources: readonly InputLiveV83SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V83_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V83_NAMESPACE)}
${buildInputLiveV83RawPropertyRuntime()}
...
export function validateInputLiveV83ExtractPayload(
  value: unknown,
  writer: InputLiveV83WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V83_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV83ExtractPayload;
}

export function proveInputLiveV83Roots<Instance>(
  extract: InputLiveV83ExtractPayload,
  sources: readonly (InputLiveV83SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV83Scene(
      root.scene,
...
export function assertInputLiveV83RootProofs(
  proofs: readonly InputLiveV83RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV83ProbeProgram(
  writer: InputLiveV83WriterOwnership,
  sources: readonly InputLiveV83SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V83_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV83ProbePayload(
  value: unknown,
  writer: InputLiveV83WriterOwnership,
): InputLiveV83ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV83ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V83_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V83_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV83CaptureManifest(
  cells: readonly InputLiveV83CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V83_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V83_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V83_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V83_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V83_SOURCE_IDS.some(
...
export const inputLiveV83CaptureManifestSha256 = (
  cells: readonly InputLiveV83CaptureCell[],
): string => {
  validateInputLiveV83CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV83CaptureProgram(
  writer: InputLiveV83WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V83_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V83_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V83_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV83CapturePayload(
  value: unknown,
  cell: InputLiveV83CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV83CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V83_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V83_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV83CaptureResponses(
  manifest: readonly InputLiveV83CaptureCell[],
  responses: readonly InputLiveV83CapturePayload[],
): void {
  validateInputLiveV83CaptureManifest(manifest);
  if (
...
export interface InputLiveV83ObjectiveReport {
  artifactVersion: "input-live-v83-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV83Objective(
  manifest: readonly InputLiveV83CaptureCell[],
  responses: readonly InputLiveV83CapturePayload[],
  readReference: (cell: InputLiveV83CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV83CaptureCell) => Buffer,
): InputLiveV83ObjectiveReport {
  assertInputLiveV83CaptureResponses(manifest, responses);
...
export function buildInputLiveV83CleanupProgram(
  writer: InputLiveV83WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V83_NAMESPACE,
...
export function validateInputLiveV83CleanupPayload(
  value: unknown,
  writer: InputLiveV83WriterOwnership,
): InputLiveV83CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV83CleanupPayload;
...
export const INPUT_LIVE_V83_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV83WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
