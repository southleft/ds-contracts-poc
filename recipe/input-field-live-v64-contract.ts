...
} from "./input-field-live-v3-verifier-v64.js";
import {
  buildInputLiveV64RawPropertyRuntime,
  normalizeInputLiveV64Scene,
  type InputLiveV64RawNode,
} from "./input-field-live-v64-verifier.js";
...
export const INPUT_LIVE_V64_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V64_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V64_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V64_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V64_VARIANT_COUNT = 256;
export const INPUT_LIVE_V64_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V64_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV64SourceId = (typeof INPUT_LIVE_V64_SOURCE_IDS)[number];

export interface InputLiveV64SourceIdentity {
  source: InputLiveV64SourceId;
...
export interface InputLiveV64WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV64ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV64SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV64RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV64RootProof {
  source: InputLiveV64SourceId;
  adapterIdentity: string;
...
export interface InputLiveV64ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV64SourceId;
    adapterIdentity: string;
...
export interface InputLiveV64CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV64SourceId;
  adapterIdentity: string;
...
export interface InputLiveV64CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV64SourceId;
  strata: InputLiveV64CaptureCell["strata"];
...
export interface InputLiveV64CleanupPayload {
...
export function validateInputLiveV64WriterPayload(
  value: unknown,
): InputLiveV64WriterOwnership {
...
const identityPlan = (source: InputLiveV64SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV64ExtractProgram(
  writer: InputLiveV64WriterOwnership,
  sources: readonly InputLiveV64SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V64_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V64_NAMESPACE)}
${buildInputLiveV64RawPropertyRuntime()}
...
export function validateInputLiveV64ExtractPayload(
  value: unknown,
  writer: InputLiveV64WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V64_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV64ExtractPayload;
}

export function proveInputLiveV64Roots<Instance>(
  extract: InputLiveV64ExtractPayload,
  sources: readonly (InputLiveV64SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV64Scene(
      root.scene,
...
export function assertInputLiveV64RootProofs(
  proofs: readonly InputLiveV64RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV64ProbeProgram(
  writer: InputLiveV64WriterOwnership,
  sources: readonly InputLiveV64SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V64_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV64ProbePayload(
  value: unknown,
  writer: InputLiveV64WriterOwnership,
): InputLiveV64ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV64ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V64_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V64_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV64CaptureManifest(
  cells: readonly InputLiveV64CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V64_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V64_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V64_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V64_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V64_SOURCE_IDS.some(
...
export const inputLiveV64CaptureManifestSha256 = (
  cells: readonly InputLiveV64CaptureCell[],
): string => {
  validateInputLiveV64CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV64CaptureProgram(
  writer: InputLiveV64WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V64_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V64_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V64_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV64CapturePayload(
  value: unknown,
  cell: InputLiveV64CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV64CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V64_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V64_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV64CaptureResponses(
  manifest: readonly InputLiveV64CaptureCell[],
  responses: readonly InputLiveV64CapturePayload[],
): void {
  validateInputLiveV64CaptureManifest(manifest);
  if (
...
export interface InputLiveV64ObjectiveReport {
  artifactVersion: "input-live-v64-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV64Objective(
  manifest: readonly InputLiveV64CaptureCell[],
  responses: readonly InputLiveV64CapturePayload[],
  readReference: (cell: InputLiveV64CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV64CaptureCell) => Buffer,
): InputLiveV64ObjectiveReport {
  assertInputLiveV64CaptureResponses(manifest, responses);
...
export function buildInputLiveV64CleanupProgram(
  writer: InputLiveV64WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V64_NAMESPACE,
...
export function validateInputLiveV64CleanupPayload(
  value: unknown,
  writer: InputLiveV64WriterOwnership,
): InputLiveV64CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV64CleanupPayload;
...
export const INPUT_LIVE_V64_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV64WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
