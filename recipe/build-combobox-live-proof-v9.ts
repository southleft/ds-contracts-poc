...
export const COMBOBOX_LIVE_V9_EVIDENCE_ROOT =
  "recipe/evidence/combobox-live-pivot-v9";
export const COMBOBOX_LIVE_V9_PROTOCOL_PATH = `${COMBOBOX_LIVE_V9_EVIDENCE_ROOT}/protocol.json`;
export const COMBOBOX_LIVE_V9_PLAN_PATH = `${COMBOBOX_LIVE_V9_EVIDENCE_ROOT}/proof-plan.json`;
export const COMBOBOX_LIVE_V9_CAPTURE_MANIFEST_PATH = `${COMBOBOX_LIVE_V9_EVIDENCE_ROOT}/capture-manifest.json`;
export const COMBOBOX_LIVE_V9_REQUEST_MANIFEST_PATH = `${COMBOBOX_LIVE_V9_EVIDENCE_ROOT}/request-manifest.json`;
export const COMBOBOX_LIVE_V9_INDEX_PATH = `${COMBOBOX_LIVE_V9_EVIDENCE_ROOT}/antecedent-index.json`;
export const COMBOBOX_LIVE_V9_AUTHORIZATION_TEMPLATE_PATH = `${COMBOBOX_LIVE_V9_EVIDENCE_ROOT}/authorization-template.json`;
export const COMBOBOX_LIVE_V9_AUTHORIZATION_PATH = `${COMBOBOX_LIVE_V9_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface ComboboxLiveV9ProofPlan {
  artifactVersion: "combobox-live-v9-proof-plan-v1";
...
const captureCells = (
...
): ComboboxLiveV9CaptureCell[] => {
  const cells: ComboboxLiveV9CaptureCell[] = [];
  ...
  if (cells.length !== COMBOBOX_LIVE_V9_CAPTURE_COUNT)
    throw new Error(
      `Combobox live v9 capture cells ${cells.length}, expected ${COMBOBOX_LIVE_V9_CAPTURE_COUNT}`,
    );
...
const protocol = (expectedFacts: number) => ({
  artifactVersion: "combobox-live-v9-external-operator-protocol-v1",
  protocolId: "combobox-live-v9",
  status:
  ...
  denominator: {
    ...
    sets: 4,
    variants: COMBOBOX_LIVE_V9_VARIANT_COUNT,
    captureCells: COMBOBOX_LIVE_V9_CAPTURE_COUNT,
...
export async function buildComboboxLiveV9Proof(
  check = process.argv.includes("--check"),
): Promise<ComboboxLiveV9ProofPlan> {
  ...
  if (
    ...
    throw new Error("Combobox live v9 writer reused Input identity");
  const captures = captureCells(sources);
  const writerProgram = Buffer.from(writer.code);
  const writerOwnershipBlueprint: ComboboxLiveV9WriterOwnership = {
  ...
  const identities: ComboboxLiveV9SourceIdentity[] = sources.map((source) => ({
    source: source.source,
    adapterIdentity: source.adapterIdentity,
    recipeHash: source.recipeHash,
    envelopeHash: source.envelope.integrity.canonicalHash,
  ...
  const restoreBlueprint = Buffer.from(
    buildComboboxLiveV9RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildComboboxLiveV9ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildComboboxLiveV9ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildComboboxLiveV9CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );
  const cleanupBlueprint = Buffer.from(
    buildComboboxLiveV9CleanupProgram(writerOwnershipBlueprint),
  );
  ...
  const sourceMetadata = sources.map((source) => {
    ...
    const comboboxPath = `${COMBOBOX_LIVE_V9_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-combobox.json.gz`;
    const optionPath = `${COMBOBOX_LIVE_V9_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-option.json.gz`;
  ...
  antecedentOutputs.set(
    COMBOBOX_LIVE_V9_PROTOCOL_PATH,
    jsonBytes(protocol(expectedFacts)),
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V9_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V9_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V9_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V9_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V9_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  antecedentOutputs.set(
    `${COMBOBOX_LIVE_V9_EVIDENCE_ROOT}/programs/cleanup-blueprint.js`,
    cleanupBlueprint,
  );

  const captureManifest = {
    artifactVersion: "combobox-live-v9-capture-manifest-v1",
  ...
  const proofPlan: ComboboxLiveV9ProofPlan = {
    artifactVersion: "combobox-live-v9-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: COMBOBOX_LIVE_V9_TARGET,
    namespace: COMBOBOX_FIGMA_NAMESPACE,
    writer: {
      programPath: `${COMBOBOX_LIVE_V9_EVIDENCE_ROOT}/programs/writer.txt`,
    ...
    captureManifest: {
      path: COMBOBOX_LIVE_V9_CAPTURE_MANIFEST_PATH,
      cells: 72,
      requests: 72,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(COMBOBOX_LIVE_V9_PLAN_PATH, jsonBytes(proofPlan));

  const requests = {
    artifactVersion: "combobox-live-v9-request-manifest-v1",
    expectedDynamicTool: COMBOBOX_LIVE_V9_DYNAMIC_TOOL,
    target: COMBOBOX_LIVE_V9_TARGET,
  ...
  if (requests.requestCount !== COMBOBOX_LIVE_V9_REMOTE_REQUESTS)
    throw new Error("Combobox live v9 remote request denominator drifted");
  antecedentOutputs.set(
    COMBOBOX_LIVE_V9_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const lifecycleExcludedPaths = [
    COMBOBOX_LIVE_V9_AUTHORIZATION_TEMPLATE_PATH,
    COMBOBOX_LIVE_V9_AUTHORIZATION_PATH,
    "recipe/combobox-live-v9-authorization.ts",
    "recipe/combobox-live-v9-authorization.test.ts",
    "recipe/combobox-live-v9-preflight.ts",
    "recipe/combobox-live-v9-authorized.ts",
    "recipe/write-combobox-live-v9-authorization.ts",
    "recipe/create-combobox-live-v9-security-attestation.ts",
    "recipe/evidence/combobox-live-pivot-v9/operator-security-attestation-template.json",
    "recipe/evidence/combobox-live-pivot-v9-status.json",
    "recipe/evidence/status-index.json",
  ];
  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/combobox-live-v9-broker.ts",
    "recipe/combobox-live-v9-contract.ts",
    "recipe/combobox-live-v9-restore.ts",
    "recipe/combobox-live-v9-cleanup.ts",
    "recipe/combobox-live-v9-verifier.ts",
    "recipe/combobox-live-v9-fixed-point.ts",
    "recipe/run-combobox-live-v9.ts",
    "recipe/build-combobox-live-proof-v9.ts",
    "recipe/combobox-live-v9-lifecycle-simulation.ts",
    "recipe/scene-readback-combobox-v9.ts",
    "recipe/scene-readback-runtime-combobox-v9.ts",
    "recipe/combobox-figma-writer.ts",
    "recipe/figma-property-normalizer-v8.ts",
    "recipe/figma-runtime-portability.ts",
    "recipe/normalize.ts",
  ].filter((artifactPath) => artifactPath !== COMBOBOX_LIVE_V9_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "combobox-live-v9-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
  ...
  antecedentOutputs.set(COMBOBOX_LIVE_V9_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "combobox-live-v9-authorization-template-v1",
    authorizationId: "combobox-live-v9",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: COMBOBOX_LIVE_V9_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
  ...
  const attestationTemplate = jsonBytes({
    artifactVersion: "combobox-live-v9-operator-security-attestation-template-v1",
    status: "template only; complete privately after PREPARE+AUTHORIZE",
    tokenValuesForbidden: true,
    privateDirectory: "private/combobox-live-v9-security-attestation.json",
  });
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Combobox live v9 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(COMBOBOX_LIVE_V9_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(COMBOBOX_LIVE_V9_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
    writeFileSync(
      `${COMBOBOX_LIVE_V9_EVIDENCE_ROOT}/operator-security-attestation-template.json`,
      attestationTemplate,
    );
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(COMBOBOX_LIVE_V9_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(COMBOBOX_LIVE_V9_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Combobox live v9 authorization template drift");
  }
  if (
    writer.namespace !== COMBOBOX_FIGMA_NAMESPACE ||
    writer.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    ...
    throw new Error("Combobox live v9 identity or cleanup invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildComboboxLiveV9Proof(), null, 2)}\n`,
  );
