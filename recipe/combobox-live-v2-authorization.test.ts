...
import { COMBOBOX_LIVE_V2_INDEX_PATH } from "./build-combobox-live-proof-v2.js";
import {
  COMBOBOX_LIVE_V2_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V2_TARGET,
} from "./combobox-live-v2-broker.js";
import {
  buildComboboxLiveV2AuthorizationArtifact,
  validateComboboxLiveV2AntecedentIndex,
  validateComboboxLiveV2AuthorizationArtifact,
  validateComboboxLiveV2History,
  validateComboboxLiveV2UnitTestSource,
  type ComboboxLiveV2AntecedentIndex,
  type ComboboxLiveV2AuthorizationArtifact,
  type ComboboxLiveV2AuthorizationProof,
  type ComboboxLiveV2HistoryState,
} from "./combobox-live-v2-authorization.js";
import {
  comboboxLiveV2AttestationSha256,
  validateComboboxLiveV2ControlFlowSource,
  validateComboboxLiveV2SecurityAttestation,
  type ComboboxLiveV2SecurityAttestation,
} from "./combobox-live-v2-preflight.js";
import { comboboxLiveV2Sha256 } from "./combobox-live-v2-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V2_INDEX_PATH);
const index = (): ComboboxLiveV2AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV2AntecedentIndex;

const artifact = (): ComboboxLiveV2AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV2AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV2HistoryState => ({
  index: index(),
  indexSha256: comboboxLiveV2Sha256(indexBytes()),
...
const authorizedState = (): ComboboxLiveV2HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV2AuthorizationArtifact,
  ...
  antecedentIndexSha256: comboboxLiveV2Sha256(indexBytes()),
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v2/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV2AuthorizationArtifact,
): ComboboxLiveV2SecurityAttestation => {
  const body: Omit<ComboboxLiveV2SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "combobox-live-v2-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V2_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: comboboxLiveV2AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV2History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV2History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV2History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV2History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV2History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV2UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV2UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV2AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = comboboxLiveV2Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateComboboxLiveV2AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV2AuthorizationArtifact(
        planted as ComboboxLiveV2AuthorizationArtifact,
        index(),
        comboboxLiveV2Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV2SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV2AttestationSha256(
      body as Omit<ComboboxLiveV2SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV2SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV2ControlFlowSource(source), []);
});
