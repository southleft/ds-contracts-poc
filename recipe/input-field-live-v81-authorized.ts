...
import { INPUT_LIVE_V81_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v81.js";
import { verifyInputLiveV81Authorization } from "./input-field-live-v81-authorization.js";
import { runInputLiveV81Preflight } from "./input-field-live-v81-preflight.js";
import { InputLiveV81Orchestrator } from "./run-input-field-live-v81.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V81_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v81-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV81Authorization(root);
    runInputLiveV81Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV81Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
