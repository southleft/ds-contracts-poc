...
import {
  acceptCalendarLiveV5Response,
  createCalendarLiveV5Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV5Request,
  persistCalendarLiveV5RawResponse,
  persistCalendarLiveV5TechnicalGates,
  readCalendarLiveV5AcceptedRawPayload,
  type CalendarLiveV5Request,
  type CalendarLiveV5TransactionAuthorization,
} from "./calendar-live-v5-broker.js";
import {
  CALENDAR_LIVE_V5_CAPTURE_COUNT,
  assertCalendarLiveV5CaptureResponses,
  ...
  evaluateCalendarLiveV5Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV5Roots,
  validateCalendarLiveV5CaptureManifest,
  validateCalendarLiveV5CapturePayload,
  validateCalendarLiveV5CleanupPayload,
  validateCalendarLiveV5ExtractPayload,
  validateCalendarLiveV5ProbePayload,
  validateCalendarLiveV5WriterPayload,
  type CalendarLiveV5CaptureCell,
  type CalendarLiveV5CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V5_PLAN_PATH,
  type CalendarLiveV5ProofPlan,
} from "./build-calendar-live-proof-v5.js";
import {
...
export const CALENDAR_LIVE_V5_RUNNER_VERSION = "calendar-live-v5-runner-v1";

interface CalendarLiveV5JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV5OrchestratorState {
  artifactVersion: "calendar-live-v5-orchestrator-state-v1";
  ...
  journal: CalendarLiveV5JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV5ProofPlan;
  planSha256: string;
  captures: CalendarLiveV5CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV5SourceIdentity & {
...
export interface CalendarLiveV5NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV5OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v5 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V5_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V5_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV5ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV5CaptureCell[];
  validateCalendarLiveV5CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v5 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v5 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV5WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV5RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV5Request =>
  readJson<CalendarLiveV5Request>(file);

export class CalendarLiveV5Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV5Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV5OrchestratorState = {
      artifactVersion: "calendar-live-v5-orchestrator-state-v1",
    ...
    return new CalendarLiveV5Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV5OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV5Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV5NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV5NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v5 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV5NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v5 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV5RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV5Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV5CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV5AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV5WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV5Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV5ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV5Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV5Response(this.#directory, "writer", {
        validate: validateCalendarLiveV5WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV5Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV5RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV5Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV5ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV5Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV5RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v5-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV5Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV5ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV5Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV5ProbePayload(payload, ownership),
      });
      persistCalendarLiveV5TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v5 capture cell missing");
      const accepted = acceptCalendarLiveV5Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV5CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V5_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV5CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV5Objective(
    ...
    throw new Error(`Calendar live v5 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV5Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV5CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV5Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV5RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV5Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV5CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV5CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV5CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV5Authorization = (
  privateKey: KeyObject,
): CalendarLiveV5TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV5SimulationKey = (): KeyObject =>
