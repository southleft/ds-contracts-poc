...
} from "./input-field-live-v3-verifier-v36.js";
import {
  buildInputLiveV36RawPropertyRuntime,
  normalizeInputLiveV36Scene,
  type InputLiveV36RawNode,
} from "./input-field-live-v36-verifier.js";
...
export const INPUT_LIVE_V36_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V36_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V36_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V36_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V36_VARIANT_COUNT = 256;
export const INPUT_LIVE_V36_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V36_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV36SourceId = (typeof INPUT_LIVE_V36_SOURCE_IDS)[number];

export interface InputLiveV36SourceIdentity {
  source: InputLiveV36SourceId;
...
export interface InputLiveV36WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV36ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV36SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV36RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV36RootProof {
  source: InputLiveV36SourceId;
  adapterIdentity: string;
...
export interface InputLiveV36ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV36SourceId;
    adapterIdentity: string;
...
export interface InputLiveV36CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV36SourceId;
  adapterIdentity: string;
...
export interface InputLiveV36CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV36SourceId;
  strata: InputLiveV36CaptureCell["strata"];
...
export interface InputLiveV36CleanupPayload {
...
export function validateInputLiveV36WriterPayload(
  value: unknown,
): InputLiveV36WriterOwnership {
...
const identityPlan = (source: InputLiveV36SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV36ExtractProgram(
  writer: InputLiveV36WriterOwnership,
  sources: readonly InputLiveV36SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V36_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V36_NAMESPACE)}
${buildInputLiveV36RawPropertyRuntime()}
...
export function validateInputLiveV36ExtractPayload(
  value: unknown,
  writer: InputLiveV36WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V36_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV36ExtractPayload;
}

export function proveInputLiveV36Roots<Instance>(
  extract: InputLiveV36ExtractPayload,
  sources: readonly (InputLiveV36SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV36Scene(
      root.scene,
...
export function assertInputLiveV36RootProofs(
  proofs: readonly InputLiveV36RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV36ProbeProgram(
  writer: InputLiveV36WriterOwnership,
  sources: readonly InputLiveV36SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V36_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV36ProbePayload(
  value: unknown,
  writer: InputLiveV36WriterOwnership,
): InputLiveV36ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV36ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V36_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V36_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV36CaptureManifest(
  cells: readonly InputLiveV36CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V36_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V36_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V36_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V36_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V36_SOURCE_IDS.some(
...
export const inputLiveV36CaptureManifestSha256 = (
  cells: readonly InputLiveV36CaptureCell[],
): string => {
  validateInputLiveV36CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV36CaptureProgram(
  writer: InputLiveV36WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V36_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V36_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V36_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV36CapturePayload(
  value: unknown,
  cell: InputLiveV36CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV36CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V36_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V36_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV36CaptureResponses(
  manifest: readonly InputLiveV36CaptureCell[],
  responses: readonly InputLiveV36CapturePayload[],
): void {
  validateInputLiveV36CaptureManifest(manifest);
  if (
...
export interface InputLiveV36ObjectiveReport {
  artifactVersion: "input-live-v36-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV36Objective(
  manifest: readonly InputLiveV36CaptureCell[],
  responses: readonly InputLiveV36CapturePayload[],
  readReference: (cell: InputLiveV36CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV36CaptureCell) => Buffer,
): InputLiveV36ObjectiveReport {
  assertInputLiveV36CaptureResponses(manifest, responses);
...
export function buildInputLiveV36CleanupProgram(
  writer: InputLiveV36WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V36_NAMESPACE,
...
export function validateInputLiveV36CleanupPayload(
  value: unknown,
  writer: InputLiveV36WriterOwnership,
): InputLiveV36CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV36CleanupPayload;
...
export const INPUT_LIVE_V36_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV36WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
