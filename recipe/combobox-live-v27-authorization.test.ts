...
import { COMBOBOX_LIVE_V27_INDEX_PATH } from "./build-combobox-live-proof-v27.js";
import {
  COMBOBOX_LIVE_V27_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V27_TARGET,
} from "./combobox-live-v27-broker.js";
import {
  buildComboboxLiveV27AuthorizationArtifact,
  validateComboboxLiveV27AntecedentIndex,
  validateComboboxLiveV27AuthorizationArtifact,
  validateComboboxLiveV27History,
  validateComboboxLiveV27UnitTestSource,
  type ComboboxLiveV27AntecedentIndex,
  type ComboboxLiveV27AuthorizationArtifact,
  type ComboboxLiveV27AuthorizationProof,
  type ComboboxLiveV27HistoryState,
} from "./combobox-live-v27-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV27ControlFlowSource,
  validateComboboxLiveV27SecurityAttestation,
  type ComboboxLiveV27SecurityAttestation,
} from "./combobox-live-v27-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v27-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V27_INDEX_PATH);
const index = (): ComboboxLiveV27AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV27AntecedentIndex;

const artifact = (): ComboboxLiveV27AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV27AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV27HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV27HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV27AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v27/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV27AuthorizationArtifact,
): ComboboxLiveV27SecurityAttestation => {
  const body: Omit<ComboboxLiveV27SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V27_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV27History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV27History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV27History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV27History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV27History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV27UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV27UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV27AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV27AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV27AuthorizationArtifact(
        planted as ComboboxLiveV27AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV27SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV27SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV27SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV27ControlFlowSource(source), []);
});
