...
import {
  ...
  verifyInputLiveV65Authorization,
  type InputLiveV65AntecedentIndex,
  type InputLiveV65AuthorizationProof,
} from "./input-field-live-v65-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V65_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v65/operator-security-attestation-template.json";
export const INPUT_LIVE_V65_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v65-security-attestation.json";

export interface InputLiveV65SecurityAttestation {
...
export interface InputLiveV65PreflightReport {
  artifactVersion: "input-live-v65-preflight-v1";
...
export const inputLiveV65AttestationSha256 = (
  value: Omit<InputLiveV65SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV65SecurityAttestation(
  value: unknown,
  proof: InputLiveV65AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v65-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V65_TARGET) ||
...
export function validateInputLiveV65ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV65Authorization");
  const preflight = source.indexOf("runInputLiveV65Preflight");
  const initialize = source.indexOf("InputLiveV65Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV65Preflight(
  root: string,
  proof: InputLiveV65AuthorizationProof,
  attestationPath = INPUT_LIVE_V65_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV65PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V65_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V65_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V65_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV65AntecedentIndex;
  failures.push(...validateInputLiveV65AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V65_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V65_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v65-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV65ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV65SecurityAttestation;
  return {
    artifactVersion: "input-live-v65-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V65_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV65Preflight(
    process.cwd(),
    verifyInputLiveV65Authorization(),
    attestationPath,
