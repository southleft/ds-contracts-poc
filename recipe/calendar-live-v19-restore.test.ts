...
import {
  ...
  CALENDAR_LIVE_V19_MEASURE_HUG_MARKER,
  CALENDAR_LIVE_V19_RESTORE_CONTENT_ROLES,
  CALENDAR_LIVE_V19_RESTORE_COUNT,
  CALENDAR_LIVE_V19_RESTORE_DAY_SET_ROLE,
...
test("Calendar restore owns HUG labels from day-set only", () => {
  ...
  assert.equal(CALENDAR_LIVE_V19_RESTORE_COUNT, 4);
  assert.equal(CALENDAR_LIVE_V19_RESTORE_DAY_SET_ROLE, "calendar/day-set");
  assert.deepEqual([...CALENDAR_LIVE_V19_RESTORE_CONTENT_ROLES], [
    "calendar/day/label",
  ]);
  assert.throws(() =>
    buildCalendarLiveV19RestoreProgram({
      pageId: FORBIDDEN_INPUT_PAGE_ID,
      runIdentity: "x-calendar-v19",
      setIds: ["a", "b", "c"],
    }),
  );
  assert.throws(() =>
  ...
  const program = buildCalendarLiveV19RestoreProgram({
    pageId: "1:2",
    runIdentity: "x-calendar-v19",
    setIds: ["a", "b", "c"],
  });
  ...
  assert.match(program, new RegExp(CALENDAR_LIVE_V19_MEASURE_HUG_MARKER));
