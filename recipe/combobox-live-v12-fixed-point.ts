/**
 * Carried Combobox live v12 host verifier. Uses scene-readback-combobox-v12 so live
...
export const COMBOBOX_LIVE_V12_FIXED_VERIFIER_VERSION = "combobox-live-v12-fixed-point-v1";
export const COMBOBOX_LIVE_V12_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV12FixedThresholds {
...
export interface ComboboxLiveV12FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV12FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV12FixedSourceProbe {
...
export interface ComboboxLiveV12FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV12FixedFixedPoint;
}

export interface ComboboxLiveV12FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV12FixedHardGateInput {
  thresholds: ComboboxLiveV12FixedThresholds;
  materialRegression: ComboboxLiveV12FixedMaterialRegression;
  sourceProbes: ComboboxLiveV12FixedSourceProbe[];
  cells: ComboboxLiveV12FixedCellValidation[];
  sceneProofs: ComboboxLiveV12FixedSceneProof[];
  visualRows: ComboboxLiveV12FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV12FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V12_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV12FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV12FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV12FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV12FixedHardGates(
  input: ComboboxLiveV12FixedHardGateInput,
): ComboboxLiveV12FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV12FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV12FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V12_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV12FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV12FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV12FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
