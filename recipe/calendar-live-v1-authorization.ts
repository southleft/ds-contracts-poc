...
export type CalendarLiveV1HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV1AuthorizationArtifact {
  artifactVersion: "calendar-live-v1-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V1_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV1AntecedentIndex {
...
export interface CalendarLiveV1HistoryState {
  index?: CalendarLiveV1AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV1AuthorizationArtifact;
...
export interface CalendarLiveV1AuthorizationProof
  extends CalendarLiveV1TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V1_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V1_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V1_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V1_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v1-authorization") ||
  artifactPath.includes("calendar-live-v1-preflight") ||
  artifactPath.includes("calendar-live-v1-authorized") ||
  artifactPath.includes("create-calendar-live-v1-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v1-status") ||
...
export function validateCalendarLiveV1AntecedentIndex(
  index: CalendarLiveV1AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v1-antecedent-index-v1" ||
...
export function validateCalendarLiveV1UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV1History\(\s*["']/.test(source) ||
    /readCalendarLiveV1HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV1AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV1AuthorizationArtifact {
  ...
  ) as CalendarLiveV1AntecedentIndex;
  const indexFailures = validateCalendarLiveV1AntecedentIndex(index);
...
export function validateCalendarLiveV1AuthorizationArtifact(
  artifact: CalendarLiveV1AuthorizationArtifact | undefined,
  index: CalendarLiveV1AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v1-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v1" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V1_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V1_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V1_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V1_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V1_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V1_SOURCE_ROOTS ||
...
export function validateCalendarLiveV1History(
  state: CalendarLiveV1HistoryState,
  expected: CalendarLiveV1HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV1AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV1AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV1HistoryState(
  root = process.cwd(),
): CalendarLiveV1HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V1_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV1AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V1_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V1_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V1_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV1History(
  expected: CalendarLiveV1HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV1HistoryState {
  const state = readCalendarLiveV1HistoryState(root);
  const failures = validateCalendarLiveV1History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v1 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV1Authorization(
  root = process.cwd(),
): CalendarLiveV1AuthorizationProof {
  const state = verifyCalendarLiveV1History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V1_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v1/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v1.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V1_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V1_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V1_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV1HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV1History(expected);
  process.stdout.write(
    `Calendar live v1 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
