...
import {
  acceptCalendarLiveV10Response,
  createCalendarLiveV10Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV10Request,
  persistCalendarLiveV10RawResponse,
  persistCalendarLiveV10TechnicalGates,
  readCalendarLiveV10AcceptedRawPayload,
  type CalendarLiveV10Request,
  type CalendarLiveV10TransactionAuthorization,
} from "./calendar-live-v10-broker.js";
import {
  CALENDAR_LIVE_V10_CAPTURE_COUNT,
  assertCalendarLiveV10CaptureResponses,
  ...
  evaluateCalendarLiveV10Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV10Roots,
  validateCalendarLiveV10CaptureManifest,
  validateCalendarLiveV10CapturePayload,
  validateCalendarLiveV10CleanupPayload,
  validateCalendarLiveV10ExtractPayload,
  validateCalendarLiveV10ProbePayload,
  validateCalendarLiveV10WriterPayload,
  type CalendarLiveV10CaptureCell,
  type CalendarLiveV10CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V10_PLAN_PATH,
  type CalendarLiveV10ProofPlan,
} from "./build-calendar-live-proof-v10.js";
import {
...
export const CALENDAR_LIVE_V10_RUNNER_VERSION = "calendar-live-v10-runner-v1";

interface CalendarLiveV10JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV10OrchestratorState {
  artifactVersion: "calendar-live-v10-orchestrator-state-v1";
  ...
  journal: CalendarLiveV10JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV10ProofPlan;
  planSha256: string;
  captures: CalendarLiveV10CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV10SourceIdentity & {
...
export interface CalendarLiveV10NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV10OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v10 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V10_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V10_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV10ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV10CaptureCell[];
  validateCalendarLiveV10CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v10 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v10 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV10WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV10RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV10Request =>
  readJson<CalendarLiveV10Request>(file);

export class CalendarLiveV10Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV10Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV10OrchestratorState = {
      artifactVersion: "calendar-live-v10-orchestrator-state-v1",
    ...
    return new CalendarLiveV10Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV10OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV10Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV10NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV10NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v10 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV10NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v10 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV10RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV10Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV10CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV10AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV10WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV10Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV10ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV10Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV10Response(this.#directory, "writer", {
        validate: validateCalendarLiveV10WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV10Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV10RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV10Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV10ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV10Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV10RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v10-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV10Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV10ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV10Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV10ProbePayload(payload, ownership),
      });
      persistCalendarLiveV10TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v10 capture cell missing");
      const accepted = acceptCalendarLiveV10Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV10CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V10_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV10CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV10Objective(
    ...
    throw new Error(`Calendar live v10 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV10Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV10CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV10Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV10RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV10Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV10CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV10CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV10CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV10Authorization = (
  privateKey: KeyObject,
): CalendarLiveV10TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV10SimulationKey = (): KeyObject =>
