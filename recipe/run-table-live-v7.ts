...
import {
  acceptTableLiveV7Response,
  createTableLiveV7Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV7Request,
  persistTableLiveV7RawResponse,
  persistTableLiveV7TechnicalGates,
  readTableLiveV7AcceptedRawPayload,
  type TableLiveV7Request,
  type TableLiveV7TransactionAuthorization,
} from "./table-live-v7-broker.js";
import {
  TABLE_LIVE_V7_CAPTURE_COUNT,
  assertTableLiveV7CaptureResponses,
  ...
  evaluateTableLiveV7Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV7Roots,
  validateTableLiveV7CaptureManifest,
  validateTableLiveV7CapturePayload,
  validateTableLiveV7CleanupPayload,
  validateTableLiveV7ExtractPayload,
  validateTableLiveV7ProbePayload,
  validateTableLiveV7WriterPayload,
  type TableLiveV7CaptureCell,
  type TableLiveV7CapturePayload,
...
import {
  ...
  TABLE_LIVE_V7_PLAN_PATH,
  type TableLiveV7ProofPlan,
...
export const TABLE_LIVE_V7_RUNNER_VERSION = "table-live-v7-runner-v1";

interface TableLiveV7JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV7OrchestratorState {
  artifactVersion: "table-live-v7-orchestrator-state-v1";
  ...
  journal: TableLiveV7JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV7ProofPlan;
  planSha256: string;
  captures: TableLiveV7CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV7SourceIdentity & {
...
export interface TableLiveV7NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV7OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v7 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V7_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V7_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV7ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV7CaptureCell[];
  validateTableLiveV7CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v7 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v7 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV7WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV7RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV7Request =>
  readJson<TableLiveV7Request>(file);

export class TableLiveV7Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV7Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV7OrchestratorState = {
      artifactVersion: "table-live-v7-orchestrator-state-v1",
    ...
    return new TableLiveV7Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV7OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV7Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV7NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV7NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v7 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV7NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v7 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV7RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV7Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV7CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV7AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV7WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV7Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV7ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV7Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV7Response(this.#directory, "writer", {
        validate: validateTableLiveV7WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV7Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV7RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV7Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV7ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV7Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV7RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v7-host-root-proofs-v1",
      ...
      const next = issueTableLiveV7Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV7ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV7Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV7ProbePayload(payload, ownership),
      });
      persistTableLiveV7TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v7 capture cell missing");
      const accepted = acceptTableLiveV7Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV7CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V7_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV7CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV7Objective(
    ...
    throw new Error(`Table live v7 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV7Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV7CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV7Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV7RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV7Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV7CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV7CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV7CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV7Authorization = (
  privateKey: KeyObject,
): TableLiveV7TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV7SimulationKey = (): KeyObject =>
