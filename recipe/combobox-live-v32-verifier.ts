...
import {
  verifyComboboxLiveV32FixedSceneFixedPoint,
  type ComboboxLiveV32FixedFixedPoint,
} from "./combobox-live-v32-fixed-point.js";

export const COMBOBOX_LIVE_V32_VERIFIER_VERSION = "combobox-live-v32-verifier-v1";

export function buildComboboxLiveV32RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV32RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV32RawNode[];
}

export interface ComboboxLiveV32NormalizedScene {
...
export interface ComboboxLiveV32SceneProof {
...
export function normalizeComboboxLiveV32Scene(
  root: ComboboxLiveV32RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV32NormalizedScene {
  const canonicalBindings: ComboboxLiveV32NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV32RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV32Scene<Instance>(
  normalized: ComboboxLiveV32NormalizedScene,
...
): ComboboxLiveV32SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV32FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV32ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV32PreCaptureGates(payload: unknown): void {
