...
import { INPUT_LIVE_V18_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v18.js";
import { verifyInputLiveV18Authorization } from "./input-field-live-v18-authorization.js";
import { runInputLiveV18Preflight } from "./input-field-live-v18-preflight.js";
import { InputLiveV18Orchestrator } from "./run-input-field-live-v18.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V18_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v18-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV18Authorization(root);
    runInputLiveV18Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV18Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
