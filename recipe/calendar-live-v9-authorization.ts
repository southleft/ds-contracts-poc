...
export type CalendarLiveV9HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV9AuthorizationArtifact {
  artifactVersion: "calendar-live-v9-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V9_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV9AntecedentIndex {
...
export interface CalendarLiveV9HistoryState {
  index?: CalendarLiveV9AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV9AuthorizationArtifact;
...
export interface CalendarLiveV9AuthorizationProof
  extends CalendarLiveV9TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V9_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V9_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V9_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V9_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v9-authorization") ||
  artifactPath.includes("calendar-live-v9-preflight") ||
  artifactPath.includes("calendar-live-v9-authorized") ||
  artifactPath.includes("create-calendar-live-v9-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v9-status") ||
...
export function validateCalendarLiveV9AntecedentIndex(
  index: CalendarLiveV9AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v9-antecedent-index-v1" ||
...
export function validateCalendarLiveV9UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV9History\(\s*["']/.test(source) ||
    /readCalendarLiveV9HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV9AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV9AuthorizationArtifact {
  ...
  ) as CalendarLiveV9AntecedentIndex;
  const indexFailures = validateCalendarLiveV9AntecedentIndex(index);
...
export function validateCalendarLiveV9AuthorizationArtifact(
  artifact: CalendarLiveV9AuthorizationArtifact | undefined,
  index: CalendarLiveV9AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v9-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v9" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V9_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V9_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V9_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V9_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V9_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V9_SOURCE_ROOTS ||
...
export function validateCalendarLiveV9History(
  state: CalendarLiveV9HistoryState,
  expected: CalendarLiveV9HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV9AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV9AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV9HistoryState(
  root = process.cwd(),
): CalendarLiveV9HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V9_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV9AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V9_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V9_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V9_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV9History(
  expected: CalendarLiveV9HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV9HistoryState {
  const state = readCalendarLiveV9HistoryState(root);
  const failures = validateCalendarLiveV9History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v9 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV9Authorization(
  root = process.cwd(),
): CalendarLiveV9AuthorizationProof {
  const state = verifyCalendarLiveV9History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V9_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v9/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v9.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V9_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V9_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V9_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV9HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV9History(expected);
  process.stdout.write(
    `Calendar live v9 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
