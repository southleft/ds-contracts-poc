...
import { INPUT_LIVE_V43_INDEX_PATH } from "./build-input-field-live-proof-v43.js";
import {
  INPUT_LIVE_V43_DYNAMIC_TOOL,
  INPUT_LIVE_V43_TARGET,
} from "./input-field-live-v43-broker.js";
import {
  buildInputLiveV43AuthorizationArtifact,
  validateInputLiveV43AntecedentIndex,
  validateInputLiveV43AuthorizationArtifact,
  validateInputLiveV43History,
  validateInputLiveV43UnitTestSource,
  type InputLiveV43AntecedentIndex,
  type InputLiveV43AuthorizationArtifact,
  type InputLiveV43AuthorizationProof,
  type InputLiveV43HistoryState,
} from "./input-field-live-v43-authorization.js";
import {
  inputLiveV43AttestationSha256,
  validateInputLiveV43ControlFlowSource,
  validateInputLiveV43SecurityAttestation,
  type InputLiveV43SecurityAttestation,
} from "./input-field-live-v43-preflight.js";
import { inputLiveV43Sha256 } from "./input-field-live-v43-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V43_INDEX_PATH);
const index = (): InputLiveV43AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV43AntecedentIndex;

const artifact = (): InputLiveV43AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV43AuthorizationArtifact({
...
const pendingState = (): InputLiveV43HistoryState => ({
  index: index(),
  indexSha256: inputLiveV43Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV43HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV43AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV43Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V43_TARGET,
  expectedDynamicTool: INPUT_LIVE_V43_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v43/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV43AuthorizationArtifact,
): InputLiveV43SecurityAttestation => {
  const body: Omit<InputLiveV43SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v43-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V43_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV43AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV43History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV43History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV43History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV43History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV43History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV43UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV43UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV43AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV43Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV43AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV43AuthorizationArtifact(
        planted as InputLiveV43AuthorizationArtifact,
        index(),
        inputLiveV43Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV43SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV43AttestationSha256(
      body as Omit<InputLiveV43SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV43SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV43ControlFlowSource(source), []);
});
