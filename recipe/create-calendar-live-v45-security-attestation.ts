...
import { verifyCalendarLiveV45Authorization } from "./calendar-live-v45-authorization.js";
import { CALENDAR_LIVE_V45_TARGET } from "./calendar-live-v45-broker.js";
import {
  CALENDAR_LIVE_V45_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV45Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v45-session";
...
const attestation: CalendarLiveV45SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV45SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v45 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V45_SECURITY_ATTESTATION_DEFAULT_PATH,
);
