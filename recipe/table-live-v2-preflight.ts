...
import {
  validateTableLiveV2AntecedentIndex,
  verifyTableLiveV2Authorization,
  type TableLiveV2AntecedentIndex,
  type TableLiveV2AuthorizationProof,
} from "./table-live-v2-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V2_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v2/operator-security-attestation-template.json";
export const TABLE_LIVE_V2_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v2-security-attestation.json";

export interface TableLiveV2SecurityAttestation {
  artifactVersion: "table-live-v2-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV2PreflightReport {
  artifactVersion: "table-live-v2-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV2SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV2SecurityAttestation(
  value: unknown,
  proof: TableLiveV2AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v2-operator-security-attestation-v1" ||
    artifact.status !== "complete"
  )
    failures.push("missing completed table v2 security attestation");
  if (
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V2_TARGET) ||
...
export function validateTableLiveV2ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV2Authorization");
  const preflight = source.indexOf("runTableLiveV2Preflight");
  const initialize = source.indexOf("TableLiveV2Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV2Preflight(
  root: string,
  proof: TableLiveV2AuthorizationProof,
  attestationPath = TABLE_LIVE_V2_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV2PreflightReport {
  const failures: string[] = [];
  if (
    ...
    failures.push("table v2 attempt chronology invalid or exceeds maximum 3");
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V2_TARGET) ||
    canonicalJson(proof.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V2_DYNAMIC_TOOL)
  )
    failures.push("table v2 authorization proof target or tool mismatch");
  const indexPath = path.join(root, TABLE_LIVE_V2_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV2AntecedentIndex;
  failures.push(...validateTableLiveV2AntecedentIndex(index));
  if (
    sha256(indexBytes) !== proof.antecedentIndexSha256 ||
    index.hashSetSha256 !== proof.antecedentHashSetSha256
  )
    failures.push("table v2 authorization proof antecedent index mismatch");
  for (const [relativePath, metadata] of Object.entries(index.artifacts)) {
    ...
    if (
      ...
      failures.push(
        `table v2 pinned antecedent working bytes drift: ${relativePath}`,
      );
  }
  if (!existsSync(path.join(root, TABLE_LIVE_V2_AUTHORIZATION_PATH)))
    failures.push("table v2 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V2_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV2SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v2-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV2ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v2 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV2SecurityAttestation;
  return {
    artifactVersion: "table-live-v2-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V2_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV2Preflight(
    process.cwd(),
    verifyTableLiveV2Authorization(),
    attestationPath,
