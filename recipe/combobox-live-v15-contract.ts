...
import { buildComboboxLiveV15CleanupRuntime } from "./combobox-live-v15-cleanup.js";
import {
  type ComboboxLiveV15FixedFixedPoint,
} from "./combobox-live-v15-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV15RestoreProgram,
  validateComboboxLiveV15RestorePayload,
  type ComboboxLiveV15RestorePayload,
} from "./combobox-live-v15-restore.js";
import {
  buildComboboxLiveV15RawPropertyRuntime,
  normalizeComboboxLiveV15Scene,
  type ComboboxLiveV15RawNode,
} from "./combobox-live-v15-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v15.js";
import {
...
export const COMBOBOX_LIVE_V15_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V15_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V15_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V15_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V15_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V15_SET_COUNT = 4;
export const COMBOBOX_LIVE_V15_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V15_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V15_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V15_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V15_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV15SourceId = (typeof COMBOBOX_LIVE_V15_SOURCE_IDS)[number];

export interface ComboboxLiveV15SourceIdentity {
  source: ComboboxLiveV15SourceId;
...
export interface ComboboxLiveV15WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV15ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV15SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV15RawNode;
    optionScene: ComboboxLiveV15RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV15RootProof {
  source: ComboboxLiveV15SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV15ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV15SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV15CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV15SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV15CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV15SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV15CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v15 ${label} must be an object`);
...
export function validateComboboxLiveV15WriterPayload(
  value: unknown,
): ComboboxLiveV15WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V15_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v15 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v15 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v15 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV15SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV15ExtractProgram(
  writer: ComboboxLiveV15WriterOwnership,
  sources: readonly ComboboxLiveV15SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v15 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V15_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V15_NAMESPACE)}
${buildComboboxLiveV15RawPropertyRuntime()}
...
export function validateComboboxLiveV15ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV15WriterOwnership,
): ComboboxLiveV15ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v15 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V15_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v15 extract schema/two-root mismatch");
  return payload as ComboboxLiveV15ExtractPayload;
}

export function proveComboboxLiveV15Roots<Instance>(
  extract: ComboboxLiveV15ExtractPayload,
  sources: readonly (ComboboxLiveV15SourceIdentity & {
...
): ComboboxLiveV15RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v15 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v15 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV15Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV15Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV15FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v15 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v15 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v15 compile lost owned sets");
...
export function assertComboboxLiveV15RootProofs(
  proofs: readonly ComboboxLiveV15RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v15 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v15 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV15ProbeProgram(
  writer: ComboboxLiveV15WriterOwnership,
  sources: readonly ComboboxLiveV15SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V15_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV15ProbePayload(
  value: unknown,
  writer: ComboboxLiveV15WriterOwnership,
): ComboboxLiveV15ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV15ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V15_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V15_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v15 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV15CaptureManifest(
  cells: readonly ComboboxLiveV15CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V15_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V15_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V15_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V15_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v15 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV15CaptureCell[],
): string => {
  validateComboboxLiveV15CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV15CaptureProgram(
  writer: ComboboxLiveV15WriterOwnership,
  cell: ComboboxLiveV15CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V15_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v15 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v15 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V15_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV15CapturePayload(
  value: unknown,
  cell: ComboboxLiveV15CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV15CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV15CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V15_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V15_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v15 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV15CaptureResponses(
  manifest: readonly ComboboxLiveV15CaptureCell[],
  responses: readonly ComboboxLiveV15CapturePayload[],
): void {
  validateComboboxLiveV15CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v15 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV15ObjectiveReport {
  artifactVersion: "combobox-live-v15-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV15Objective(
  manifest: readonly ComboboxLiveV15CaptureCell[],
  responses: readonly ComboboxLiveV15CapturePayload[],
): ComboboxLiveV15ObjectiveReport {
  assertComboboxLiveV15CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v15-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV15CleanupProgram(
  writer: ComboboxLiveV15WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV15CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V15_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v15 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV15CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV15WriterOwnership,
): ComboboxLiveV15CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV15CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v15 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V15_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV15WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
