...
import { buildTableLiveV36CleanupRuntime } from "./table-live-v36-cleanup.js";
import { type TableLiveV36FixedFixedPoint } from "./table-live-v36-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV36RestoreProgram,
  validateTableLiveV36RestorePayload,
  type TableLiveV36RestorePayload,
} from "./table-live-v36-restore.js";
import {
  buildTableLiveV36RawPropertyRuntime,
  normalizeTableLiveV36Scene,
  type TableLiveV36RawNode,
} from "./table-live-v36-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v36.js";
import {
...
export const TABLE_LIVE_V36_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V36_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V36_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V36_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V36_VARIANT_COUNT = 20;
export const TABLE_LIVE_V36_SET_COUNT = 6;
export const TABLE_LIVE_V36_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V36_HOST_PHASES = 3;
export const TABLE_LIVE_V36_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V36_RESTORE_COUNT = 8;
export const TABLE_LIVE_V36_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V36_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV36SourceId = (typeof TABLE_LIVE_V36_SOURCE_IDS)[number];
export type TableLiveV36Kind = "table" | "row" | "cell";

export interface TableLiveV36SourceIdentity {
  source: TableLiveV36SourceId;
...
export interface TableLiveV36WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV36ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV36SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV36RawNode;
    rowScene: TableLiveV36RawNode;
    cellScene: TableLiveV36RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV36RootProof {
  source: TableLiveV36SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV36FixedFixedPoint;
}

export interface TableLiveV36ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV36SourceId;
    adapterIdentity: string;
...
export interface TableLiveV36CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV36SourceId;
  adapterIdentity: string;
...
export interface TableLiveV36CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV36SourceId;
  frameWidth: number;
...
export interface TableLiveV36CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v36 ${label} must be an object`);
...
export function validateTableLiveV36WriterPayload(
  value: unknown,
): TableLiveV36WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V36_NAMESPACE ||
    ...
    throw new TypeError("Table live v36 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Table live v36 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v36 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV36SourceIdentity,
  kind: TableLiveV36Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV36ExtractProgram(
  writer: TableLiveV36WriterOwnership,
  sources: readonly TableLiveV36SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
      "Table live v36 extract requires two source identities",
    );
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V36_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V36_NAMESPACE)}
${buildTableLiveV36RawPropertyRuntime()}
...
export function validateTableLiveV36ExtractPayload(
  value: unknown,
  writer: TableLiveV36WriterOwnership,
): TableLiveV36ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v36 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V36_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v36 extract schema/two-root mismatch");
  return payload as TableLiveV36ExtractPayload;
}

export function proveTableLiveV36Roots<Instance>(
  extract: TableLiveV36ExtractPayload,
  sources: readonly (TableLiveV36SourceIdentity & {
...
): TableLiveV36RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v36 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v36 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV36Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV36Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV36Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV36FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Table live v36 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v36 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v36 compile lost owned sets");
...
export function assertTableLiveV36RootProofs(
  proofs: readonly TableLiveV36RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v36 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v36 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV36ProbeProgram(
  writer: TableLiveV36WriterOwnership,
  sources: readonly TableLiveV36SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V36_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV36ProbePayload(
  value: unknown,
  writer: TableLiveV36WriterOwnership,
): TableLiveV36ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV36ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V36_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v36 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV36CaptureManifest(
  cells: readonly TableLiveV36CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V36_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V36_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V36_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V36_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v36 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV36CaptureCell[],
): string => {
  validateTableLiveV36CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV36CaptureProgram(
  writer: TableLiveV36WriterOwnership,
  cell: TableLiveV36CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V36_CAPTURE_COUNT
  )
    throw new TypeError("Table live v36 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Table live v36 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V36_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV36CapturePayload(
  value: unknown,
  cell: TableLiveV36CaptureCell,
  rawResponseBytes: number,
): TableLiveV36CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV36CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V36_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V36_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v36 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV36CaptureResponses(
  manifest: readonly TableLiveV36CaptureCell[],
  responses: readonly TableLiveV36CapturePayload[],
): void {
  validateTableLiveV36CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v36 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV36ObjectiveReport {
  artifactVersion: "table-live-v36-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV36Objective(
  manifest: readonly TableLiveV36CaptureCell[],
  responses: readonly TableLiveV36CapturePayload[],
): TableLiveV36ObjectiveReport {
  assertTableLiveV36CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v36-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV36CleanupProgram(
  writer: TableLiveV36WriterOwnership,
): string {
  const baseProgram = buildTableLiveV36CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V36_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v36 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV36CleanupPayload(
  payload: unknown,
  writer: TableLiveV36WriterOwnership,
): TableLiveV36CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as TableLiveV36CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v36 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V36_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV36WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
