...
import {
  ...
  COMBOBOX_LIVE_V4_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  COMBOBOX_LIVE_V4_RESPONSE_CONTRACTS,
  type ComboboxLiveV4RootProof,
} from "./combobox-live-v4-contract.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V4_BROKER_VERSION = "combobox-live-v4-broker-v2";
export const COMBOBOX_LIVE_V4_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const COMBOBOX_LIVE_V4_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const COMBOBOX_LIVE_V4_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const COMBOBOX_LIVE_V4_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const COMBOBOX_LIVE_V4_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const COMBOBOX_LIVE_V4_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type ComboboxLiveV4RemotePhase =
  (typeof COMBOBOX_LIVE_V4_REMOTE_PHASES)[number];
export type ComboboxLiveV4AuthorizationMode = "live" | "simulated";

export interface ComboboxLiveV4CommitPins {
...
export interface ComboboxLiveV4TransactionAuthorization extends ComboboxLiveV4CommitPins {
  mode: ComboboxLiveV4AuthorizationMode;
...
export interface ComboboxLiveV4Manifest {
  artifactVersion: "combobox-live-v4-broker-manifest-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V4_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V4_DYNAMIC_TOOL;
  target: typeof COMBOBOX_LIVE_V4_TARGET;
...
export interface ComboboxLiveV4ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface ComboboxLiveV4RequestBody {
  artifactVersion: "combobox-live-v4-broker-request-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V4_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: ComboboxLiveV4RemotePhase;
  sequence: number;
  ...
  commitPins: ComboboxLiveV4CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof COMBOBOX_LIVE_V4_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: ComboboxLiveV4ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface ComboboxLiveV4Request extends ComboboxLiveV4RequestBody {
...
export interface ComboboxLiveV4Receipt {
  artifactVersion: "combobox-live-v4-broker-receipt-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V4_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: ComboboxLiveV4RemotePhase;
...
export interface ComboboxLiveV4AcceptedResponse<T = unknown> {
  request: ComboboxLiveV4Request;
  receipt: ComboboxLiveV4Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface ComboboxLiveV4IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: ComboboxLiveV4ResponseContract;
...
const equalTarget = (value: unknown): value is typeof COMBOBOX_LIVE_V4_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === COMBOBOX_LIVE_V4_TARGET.fileKey &&
  (value as BrokerTarget).fileName === COMBOBOX_LIVE_V4_TARGET.fileName &&
  (value as BrokerTarget).editorType === COMBOBOX_LIVE_V4_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== COMBOBOX_LIVE_V4_TARGET.fileKey ||
    context.fileName !== COMBOBOX_LIVE_V4_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === COMBOBOX_LIVE_V4_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Combobox live v4 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= COMBOBOX_LIVE_V4_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v4 capture index must be 0..71");
  return `capture-${String(captureIndex).padStart(3, "0")}`;
};

export const comboboxLiveV4RequestSequence = (
  phase: ComboboxLiveV4RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(comboboxLiveV4RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const comboboxLiveV4RequestPath = (
  directory: string,
  phase: ComboboxLiveV4RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const comboboxLiveV4ResponsePath = (
  directory: string,
  phase: ComboboxLiveV4RemotePhase,
  captureIndex?: number,
): string =>
...
export const comboboxLiveV4ReceiptPath = (
  directory: string,
  phase: ComboboxLiveV4RemotePhase,
  captureIndex?: number,
): string =>
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Combobox live v4 authorization pins are malformed");
};

const readManifest = (directory: string): ComboboxLiveV4Manifest => {
  const manifest = json<ComboboxLiveV4Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Combobox live v4 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "combobox-live-v4-broker-manifest-v2" ||
    manifest.brokerVersion !== COMBOBOX_LIVE_V4_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== COMBOBOX_LIVE_V4_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(COMBOBOX_LIVE_V4_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Combobox live v4 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: ComboboxLiveV4Request,
): ComboboxLiveV4RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: ComboboxLiveV4RemotePhase,
  captureIndex?: number,
): ComboboxLiveV4Request | undefined => {
  const file = comboboxLiveV4RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<ComboboxLiveV4Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: ComboboxLiveV4RemotePhase,
  captureIndex?: number,
): ComboboxLiveV4Receipt | undefined => {
  const file = comboboxLiveV4ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<ComboboxLiveV4Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: ComboboxLiveV4Request | undefined;
  receipt: ComboboxLiveV4Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: ComboboxLiveV4RemotePhase,
): ComboboxLiveV4ResponseContract => ({
  ...COMBOBOX_LIVE_V4_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: ComboboxLiveV4Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: COMBOBOX_LIVE_V4_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: COMBOBOX_LIVE_V4_DYNAMIC_TOOL,
      target: COMBOBOX_LIVE_V4_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "combobox-live-v4-plugin-response-v2",
  brokerVersion: COMBOBOX_LIVE_V4_BROKER_VERSION,
  target: COMBOBOX_LIVE_V4_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*COMBOBOX_LIVE_V4_PROGRAM_START*/${program}/*COMBOBOX_LIVE_V4_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*COMBOBOX_LIVE_V4_PROGRAM_START*/";
  const endMarker = "/*COMBOBOX_LIVE_V4_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Combobox live v4 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createComboboxLiveV4Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Combobox live v4 private signing key does not match authorized public-key identity",
    );
  const manifest: ComboboxLiveV4Manifest = {
    artifactVersion: "combobox-live-v4-broker-manifest-v2",
    brokerVersion: COMBOBOX_LIVE_V4_BROKER_VERSION,
    ...
    captureRequests: COMBOBOX_LIVE_V4_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateComboboxLiveV4Request(
  directory: string,
  request: ComboboxLiveV4Request,
): void {
  ...
  const expectedSequence = comboboxLiveV4RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "combobox-live-v4-broker-request-v2" ||
    request.brokerVersion !== COMBOBOX_LIVE_V4_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== COMBOBOX_LIVE_V4_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Combobox live v4 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueComboboxLiveV4Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: ComboboxLiveV4RemotePhase,
  program: string,
  options: ComboboxLiveV4IssueOptions = {},
): ComboboxLiveV4Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = comboboxLiveV4RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<ComboboxLiveV4Request>(targetPath);
    validateComboboxLiveV4Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Combobox live v4 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(comboboxLiveV4ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Combobox live v4 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Combobox live v4 empty code envelope refused");
  if (phase === "capture") validateComboboxLiveV4TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Combobox live v4 out-of-order phase: ${id}`);
  const expectedResponse =
    options.expectedResponse ?? responseContractFor(phase);
  const programSha256 = sha256(program);
  const sequence = comboboxLiveV4RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: COMBOBOX_LIVE_V4_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? COMBOBOX_LIVE_V4_SIGNED_WRITER_TIMEOUT_MS
        : COMBOBOX_LIVE_V4_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: ComboboxLiveV4RequestBody = {
    artifactVersion: "combobox-live-v4-broker-request-v2",
    brokerVersion: COMBOBOX_LIVE_V4_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: ComboboxLiveV4Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateComboboxLiveV4Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistComboboxLiveV4RawResponse(
  directory: string,
  phase: ComboboxLiveV4RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = comboboxLiveV4RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Combobox live v4 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<ComboboxLiveV4Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Combobox live v4 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = comboboxLiveV4ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Combobox live v4 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): ComboboxLiveV4Receipt => {
  const body: Omit<ComboboxLiveV4Receipt, "receiptSha256"> = {
    artifactVersion: "combobox-live-v4-broker-receipt-v2",
    brokerVersion: COMBOBOX_LIVE_V4_BROKER_VERSION,
...
export function acceptComboboxLiveV4Response<T = unknown>(
  directory: string,
  phase: ComboboxLiveV4RemotePhase,
  options: {
    captureIndex?: number;
...
): ComboboxLiveV4AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = comboboxLiveV4ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Combobox live v4 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<ComboboxLiveV4Request>(
    comboboxLiveV4RequestPath(directory, phase, captureIndex),
  );
  validateComboboxLiveV4Request(directory, request);
  if (phase === "capture") validateComboboxLiveV4TechnicalGates(directory);
  const rawFile = comboboxLiveV4ResponsePath(directory, phase, captureIndex);
  if (!existsSync(rawFile))
    throw new Error(`Combobox live v4 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "combobox-live-v4-plugin-response-v2" ||
    plugin?.brokerVersion !== COMBOBOX_LIVE_V4_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Combobox live v4 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistComboboxLiveV4TechnicalGates(
  directory: string,
  rootProofs: readonly ComboboxLiveV4RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertComboboxLiveV4RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Combobox live v4 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Combobox live v4 probe payload hash malformed");
  const gates = Object.fromEntries(
    COMBOBOX_LIVE_V4_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "combobox-live-v4-host-technical-gates-v2",
    brokerVersion: COMBOBOX_LIVE_V4_BROKER_VERSION,
...
export function validateComboboxLiveV4TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "combobox-live-v4-host-technical-gates-v2" ||
    ...
    COMBOBOX_LIVE_V4_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Combobox live v4 technical gates artifact");
}

export function validateComboboxLiveV4Receipt(
  directory: string,
  phase: ComboboxLiveV4RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateComboboxLiveV4Request(directory, request);
  const receipt = json<ComboboxLiveV4Receipt>(
    comboboxLiveV4ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    comboboxLiveV4ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "combobox-live-v4-broker-receipt-v2" ||
    receipt.brokerVersion !== COMBOBOX_LIVE_V4_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Combobox live v4 accepted receipt: ${request.requestId}`,
    );
}

export function readComboboxLiveV4AcceptedRawPayload(
  directory: string,
  phase: ComboboxLiveV4RemotePhase,
  captureIndex?: number,
): unknown {
  validateComboboxLiveV4Receipt(directory, phase, captureIndex);
...
export function createComboboxLiveV4FakeDynamicResponse(
  request: ComboboxLiveV4Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? COMBOBOX_LIVE_V4_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? COMBOBOX_LIVE_V4_TARGET,
    ...overrides.extra,
  };
}

export function listComboboxLiveV4PersistedRequests(
  directory: string,
): ComboboxLiveV4Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<ComboboxLiveV4Request>(file)] : [];
    });
}

export const comboboxLiveV4Sha256 = sha256;
