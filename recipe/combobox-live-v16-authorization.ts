...
export type ComboboxLiveV16HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV16AuthorizationArtifact {
  artifactVersion: "combobox-live-v16-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V16_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV16AntecedentIndex {
...
export interface ComboboxLiveV16HistoryState {
  index?: ComboboxLiveV16AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV16AuthorizationArtifact;
...
export interface ComboboxLiveV16AuthorizationProof extends ComboboxLiveV16TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V16_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V16_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V16_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V16_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v16-authorization") ||
  artifactPath.includes("combobox-live-v16-preflight") ||
  artifactPath.includes("combobox-live-v16-authorized") ||
  artifactPath.includes("create-combobox-live-v16-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v16-status") ||
...
export function validateComboboxLiveV16AntecedentIndex(
  index: ComboboxLiveV16AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v16-antecedent-index-v1" ||
...
export function validateComboboxLiveV16UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV16History\(\s*["']/.test(source) ||
    /readComboboxLiveV16HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV16AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV16AuthorizationArtifact {
  ...
  ) as ComboboxLiveV16AntecedentIndex;
  const indexFailures = validateComboboxLiveV16AntecedentIndex(index);
...
export function validateComboboxLiveV16AuthorizationArtifact(
  artifact: ComboboxLiveV16AuthorizationArtifact | undefined,
  index: ComboboxLiveV16AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v16-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v16" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V16_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V16_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V16_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V16_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V16_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V16_SOURCE_ROOTS ||
  ...
  if (
    ...
    failures.push(
      "v16 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
...
export function validateComboboxLiveV16History(
  state: ComboboxLiveV16HistoryState,
  expected: ComboboxLiveV16HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV16AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV16AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV16HistoryState(
  root = process.cwd(),
): ComboboxLiveV16HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V16_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV16AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V16_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V16_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V16_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV16History(
  expected: ComboboxLiveV16HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV16HistoryState {
  const state = readComboboxLiveV16HistoryState(root);
  const failures = validateComboboxLiveV16History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v16 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV16Authorization(
  root = process.cwd(),
): ComboboxLiveV16AuthorizationProof {
  const state = verifyComboboxLiveV16History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V16_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v16/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v16.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V16_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V16_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V16_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV16HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV16History(expected);
  process.stdout.write(
    `Combobox live v16 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
