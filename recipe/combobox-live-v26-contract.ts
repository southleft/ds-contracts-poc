...
import { buildComboboxLiveV26CleanupRuntime } from "./combobox-live-v26-cleanup.js";
import {
  type ComboboxLiveV26FixedFixedPoint,
} from "./combobox-live-v26-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV26RestoreProgram,
  validateComboboxLiveV26RestorePayload,
  type ComboboxLiveV26RestorePayload,
} from "./combobox-live-v26-restore.js";
import {
  buildComboboxLiveV26RawPropertyRuntime,
  normalizeComboboxLiveV26Scene,
  type ComboboxLiveV26RawNode,
} from "./combobox-live-v26-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v26.js";
import {
...
} from "./scene-readback-combobox-v26.js";
...
export const COMBOBOX_LIVE_V26_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V26_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V26_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V26_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V26_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V26_SET_COUNT = 4;
export const COMBOBOX_LIVE_V26_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V26_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V26_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V26_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V26_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV26SourceId = (typeof COMBOBOX_LIVE_V26_SOURCE_IDS)[number];

export interface ComboboxLiveV26SourceIdentity {
  source: ComboboxLiveV26SourceId;
...
export interface ComboboxLiveV26WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV26ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV26SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV26RawNode;
    optionScene: ComboboxLiveV26RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV26RootProof {
  source: ComboboxLiveV26SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV26FixedFixedPoint;
}

export interface ComboboxLiveV26ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV26SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV26CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV26SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV26CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV26SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV26CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v26 ${label} must be an object`);
...
export function validateComboboxLiveV26WriterPayload(
  value: unknown,
): ComboboxLiveV26WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V26_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v26 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v26 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v26 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV26SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV26ExtractProgram(
  writer: ComboboxLiveV26WriterOwnership,
  sources: readonly ComboboxLiveV26SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v26 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V26_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V26_NAMESPACE)}
${buildComboboxLiveV26RawPropertyRuntime()}
...
export function validateComboboxLiveV26ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV26WriterOwnership,
): ComboboxLiveV26ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v26 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V26_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v26 extract schema/two-root mismatch");
  return payload as ComboboxLiveV26ExtractPayload;
}

export function proveComboboxLiveV26Roots<Instance>(
  extract: ComboboxLiveV26ExtractPayload,
  sources: readonly (ComboboxLiveV26SourceIdentity & {
...
): ComboboxLiveV26RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v26 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v26 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV26Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV26Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV26FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v26 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v26 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v26 compile lost owned sets");
...
export function assertComboboxLiveV26RootProofs(
  proofs: readonly ComboboxLiveV26RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v26 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v26 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV26ProbeProgram(
  writer: ComboboxLiveV26WriterOwnership,
  sources: readonly ComboboxLiveV26SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V26_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV26ProbePayload(
  value: unknown,
  writer: ComboboxLiveV26WriterOwnership,
): ComboboxLiveV26ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV26ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V26_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V26_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v26 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV26CaptureManifest(
  cells: readonly ComboboxLiveV26CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V26_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V26_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V26_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V26_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v26 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV26CaptureCell[],
): string => {
  validateComboboxLiveV26CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV26CaptureProgram(
  writer: ComboboxLiveV26WriterOwnership,
  cell: ComboboxLiveV26CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V26_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v26 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v26 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V26_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV26CapturePayload(
  value: unknown,
  cell: ComboboxLiveV26CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV26CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV26CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V26_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V26_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v26 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV26CaptureResponses(
  manifest: readonly ComboboxLiveV26CaptureCell[],
  responses: readonly ComboboxLiveV26CapturePayload[],
): void {
  validateComboboxLiveV26CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v26 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV26ObjectiveReport {
  artifactVersion: "combobox-live-v26-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV26Objective(
  manifest: readonly ComboboxLiveV26CaptureCell[],
  responses: readonly ComboboxLiveV26CapturePayload[],
): ComboboxLiveV26ObjectiveReport {
  assertComboboxLiveV26CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v26-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV26CleanupProgram(
  writer: ComboboxLiveV26WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV26CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V26_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v26 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV26CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV26WriterOwnership,
): ComboboxLiveV26CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV26CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v26 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V26_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV26WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
