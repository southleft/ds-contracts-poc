...
export interface InputLiveV56LifecycleSimulationReport {
  artifactVersion: "input-live-v56-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV56AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV56Lifecycle(): Promise<InputLiveV56LifecycleSimulationReport> {
  await buildInputLiveV56Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V56_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV56AntecedentIndex;
  const indexSha256 = inputLiveV56Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v56-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V56_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V56_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV56History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV56History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV56AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V56_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V56_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV56History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV56History(authorized, "pending");
    ...
    await buildInputLiveV56Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V56_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV56AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV56Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV56Lifecycle(), null, 2)}\n`,
  );
