...
import { TABLE_LIVE_V25_INDEX_PATH } from "./build-table-live-proof-v25.js";
import {
...
import {
  buildTableLiveV25AuthorizationArtifact,
  validateTableLiveV25AntecedentIndex,
  validateTableLiveV25AuthorizationArtifact,
  validateTableLiveV25History,
  validateTableLiveV25UnitTestSource,
  type TableLiveV25AntecedentIndex,
  type TableLiveV25AuthorizationArtifact,
  type TableLiveV25AuthorizationProof,
  type TableLiveV25HistoryState,
} from "./table-live-v25-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV25ControlFlowSource,
  validateTableLiveV25SecurityAttestation,
  type TableLiveV25SecurityAttestation,
} from "./table-live-v25-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V25_INDEX_PATH);
const index = (): TableLiveV25AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV25AntecedentIndex;

const artifact = (): TableLiveV25AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV25AuthorizationArtifact({
...
const pendingState = (): TableLiveV25HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV25HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV25AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v25/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV25AuthorizationArtifact,
): TableLiveV25SecurityAttestation => {
  const body: Omit<TableLiveV25SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V25_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV25History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV25History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV25History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV25History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV25History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV25UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v25-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV25UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV25AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV25AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV25AuthorizationArtifact(
        planted as TableLiveV25AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV25SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV25SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV25SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v25-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV25ControlFlowSource(source), []);
});
