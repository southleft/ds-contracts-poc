...
import { INPUT_LIVE_V24_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v24.js";
import { verifyInputLiveV24Authorization } from "./input-field-live-v24-authorization.js";
import { runInputLiveV24Preflight } from "./input-field-live-v24-preflight.js";
import { InputLiveV24Orchestrator } from "./run-input-field-live-v24.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V24_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v24-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV24Authorization(root);
    runInputLiveV24Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV24Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
