...
export interface InputLiveV32LifecycleSimulationReport {
  artifactVersion: "input-live-v32-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV32AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV32Lifecycle(): Promise<InputLiveV32LifecycleSimulationReport> {
  await buildInputLiveV32Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V32_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV32AntecedentIndex;
  const indexSha256 = inputLiveV32Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v32-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V32_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V32_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV32History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV32History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV32AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V32_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V32_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV32History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV32History(authorized, "pending");
    ...
    await buildInputLiveV32Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V32_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV32AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV32Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV32Lifecycle(), null, 2)}\n`,
  );
