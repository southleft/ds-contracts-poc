...
import {
  acceptCalendarLiveV20Response,
  createCalendarLiveV20Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV20Request,
  persistCalendarLiveV20RawResponse,
  persistCalendarLiveV20TechnicalGates,
  readCalendarLiveV20AcceptedRawPayload,
  type CalendarLiveV20Request,
  type CalendarLiveV20TransactionAuthorization,
} from "./calendar-live-v20-broker.js";
import {
  CALENDAR_LIVE_V20_CAPTURE_COUNT,
  assertCalendarLiveV20CaptureResponses,
  ...
  evaluateCalendarLiveV20Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV20Roots,
  validateCalendarLiveV20CaptureManifest,
  validateCalendarLiveV20CapturePayload,
  validateCalendarLiveV20CleanupPayload,
  validateCalendarLiveV20ExtractPayload,
  validateCalendarLiveV20ProbePayload,
  validateCalendarLiveV20WriterPayload,
  type CalendarLiveV20CaptureCell,
  type CalendarLiveV20CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V20_PLAN_PATH,
  type CalendarLiveV20ProofPlan,
} from "./build-calendar-live-proof-v20.js";
import {
...
export const CALENDAR_LIVE_V20_RUNNER_VERSION = "calendar-live-v20-runner-v1";

interface CalendarLiveV20JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV20OrchestratorState {
  artifactVersion: "calendar-live-v20-orchestrator-state-v1";
  ...
  journal: CalendarLiveV20JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV20ProofPlan;
  planSha256: string;
  captures: CalendarLiveV20CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV20SourceIdentity & {
...
export interface CalendarLiveV20NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV20OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v20 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V20_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V20_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV20ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV20CaptureCell[];
  validateCalendarLiveV20CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v20 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v20 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV20WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV20RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV20Request =>
  readJson<CalendarLiveV20Request>(file);

export class CalendarLiveV20Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV20Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV20OrchestratorState = {
      artifactVersion: "calendar-live-v20-orchestrator-state-v1",
    ...
    return new CalendarLiveV20Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV20OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV20Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV20NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV20NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v20 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV20NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v20 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV20RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV20Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV20CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV20AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV20WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV20Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV20ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV20Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV20Response(this.#directory, "writer", {
        validate: validateCalendarLiveV20WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV20Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV20RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV20Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV20ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV20Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV20RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v20-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV20Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV20ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV20Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV20ProbePayload(payload, ownership),
      });
      persistCalendarLiveV20TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v20 capture cell missing");
      const accepted = acceptCalendarLiveV20Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV20CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V20_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV20CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV20Objective(
    ...
    throw new Error(`Calendar live v20 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV20Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV20CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV20Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV20RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV20Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV20CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV20CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV20CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV20Authorization = (
  privateKey: KeyObject,
): CalendarLiveV20TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV20SimulationKey = (): KeyObject =>
