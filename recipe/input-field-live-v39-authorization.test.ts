...
import { INPUT_LIVE_V39_INDEX_PATH } from "./build-input-field-live-proof-v39.js";
import {
  INPUT_LIVE_V39_DYNAMIC_TOOL,
  INPUT_LIVE_V39_TARGET,
} from "./input-field-live-v39-broker.js";
import {
  buildInputLiveV39AuthorizationArtifact,
  validateInputLiveV39AntecedentIndex,
  validateInputLiveV39AuthorizationArtifact,
  validateInputLiveV39History,
  validateInputLiveV39UnitTestSource,
  type InputLiveV39AntecedentIndex,
  type InputLiveV39AuthorizationArtifact,
  type InputLiveV39AuthorizationProof,
  type InputLiveV39HistoryState,
} from "./input-field-live-v39-authorization.js";
import {
  inputLiveV39AttestationSha256,
  validateInputLiveV39ControlFlowSource,
  validateInputLiveV39SecurityAttestation,
  type InputLiveV39SecurityAttestation,
} from "./input-field-live-v39-preflight.js";
import { inputLiveV39Sha256 } from "./input-field-live-v39-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V39_INDEX_PATH);
const index = (): InputLiveV39AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV39AntecedentIndex;

const artifact = (): InputLiveV39AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV39AuthorizationArtifact({
...
const pendingState = (): InputLiveV39HistoryState => ({
  index: index(),
  indexSha256: inputLiveV39Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV39HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV39AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV39Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V39_TARGET,
  expectedDynamicTool: INPUT_LIVE_V39_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v39/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV39AuthorizationArtifact,
): InputLiveV39SecurityAttestation => {
  const body: Omit<InputLiveV39SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v39-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V39_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV39AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV39History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV39History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV39History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV39History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV39History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV39UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV39UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV39AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV39Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV39AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV39AuthorizationArtifact(
        planted as InputLiveV39AuthorizationArtifact,
        index(),
        inputLiveV39Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV39SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV39AttestationSha256(
      body as Omit<InputLiveV39SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV39SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV39ControlFlowSource(source), []);
});
