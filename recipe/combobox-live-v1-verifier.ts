...
import {
  verifyComboboxLiveV1FixedSceneFixedPoint,
  type ComboboxLiveV1FixedFixedPoint,
} from "./combobox-live-v1-fixed-point.js";

export const COMBOBOX_LIVE_V1_VERIFIER_VERSION = "combobox-live-v1-verifier-v1";

export function buildComboboxLiveV1RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV1RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV1RawNode[];
}

export interface ComboboxLiveV1NormalizedScene {
...
export interface ComboboxLiveV1SceneProof {
...
export function normalizeComboboxLiveV1Scene(
  root: ComboboxLiveV1RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV1NormalizedScene {
  const canonicalBindings: ComboboxLiveV1NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV1RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV1Scene<Instance>(
  normalized: ComboboxLiveV1NormalizedScene,
...
): ComboboxLiveV1SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV1FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV1ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV1PreCaptureGates(payload: unknown): void {
