...
import {
  ...
  INPUT_LIVE_V14_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V14_RESPONSE_CONTRACTS,
  type InputLiveV14RootProof,
} from "./input-field-live-v14-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V14_BROKER_VERSION = "input-live-v14-broker-v2";
export const INPUT_LIVE_V14_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V14_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V14_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V14_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V14_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V14_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV14RemotePhase =
  (typeof INPUT_LIVE_V14_REMOTE_PHASES)[number];
export type InputLiveV14AuthorizationMode = "live" | "simulated";

export interface InputLiveV14CommitPins {
...
export interface InputLiveV14TransactionAuthorization extends InputLiveV14CommitPins {
  mode: InputLiveV14AuthorizationMode;
...
export interface InputLiveV14Manifest {
  artifactVersion: "input-live-v14-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V14_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V14_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V14_TARGET;
...
export interface InputLiveV14ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV14RequestBody {
  artifactVersion: "input-live-v14-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V14_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV14RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV14CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V14_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV14ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV14Request extends InputLiveV14RequestBody {
...
export interface InputLiveV14Receipt {
  artifactVersion: "input-live-v14-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V14_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV14RemotePhase;
...
export interface InputLiveV14AcceptedResponse<T = unknown> {
  request: InputLiveV14Request;
  receipt: InputLiveV14Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV14IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV14ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V14_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V14_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V14_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V14_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V14_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V14_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V14_TARGET.editorType;
};
...
export const inputLiveV14RequestSequence = (
  phase: InputLiveV14RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV14RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV14RequestPath = (
  directory: string,
  phase: InputLiveV14RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV14ResponsePath = (
  directory: string,
  phase: InputLiveV14RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV14ReceiptPath = (
  directory: string,
  phase: InputLiveV14RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV14Manifest => {
  const manifest = json<InputLiveV14Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v14-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V14_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V14_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV14RemotePhase,
  captureIndex?: number,
): InputLiveV14Request | undefined => {
  const file = inputLiveV14RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV14Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV14RemotePhase,
  captureIndex?: number,
): InputLiveV14Receipt | undefined => {
  const file = inputLiveV14ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV14Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV14Request | undefined;
  receipt: InputLiveV14Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV14RemotePhase,
): InputLiveV14ResponseContract => ({
  ...INPUT_LIVE_V14_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V14_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V14_DYNAMIC_TOOL,
      target: INPUT_LIVE_V14_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v14-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V14_BROKER_VERSION,
  target: INPUT_LIVE_V14_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V14_PROGRAM_START*/${program}/*INPUT_LIVE_V14_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V14_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V14_PROGRAM_END*/";
...
export function createInputLiveV14Transaction(
  directory: string,
  ...
  const manifest: InputLiveV14Manifest = {
    artifactVersion: "input-live-v14-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V14_BROKER_VERSION,
...
export function validateInputLiveV14Request(
  directory: string,
  request: InputLiveV14Request,
): void {
  ...
  const expectedSequence = inputLiveV14RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v14-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V14_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V14_TARGET.fileKey ||
...
export function issueInputLiveV14Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV14RemotePhase,
  program: string,
  options: InputLiveV14IssueOptions = {},
): InputLiveV14Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV14RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV14Request>(targetPath);
    validateInputLiveV14Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV14ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV14TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV14RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V14_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V14_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V14_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV14RequestBody = {
    artifactVersion: "input-live-v14-broker-request-v2",
    brokerVersion: INPUT_LIVE_V14_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV14Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV14Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV14RawResponse(
  directory: string,
  phase: InputLiveV14RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV14RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV14Request>(requestFile);
  if (
  ...
  const target = inputLiveV14ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV14Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v14-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V14_BROKER_VERSION,
...
export function acceptInputLiveV14Response<T = unknown>(
  directory: string,
  phase: InputLiveV14RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV14AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV14ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV14Request>(
    inputLiveV14RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV14Request(directory, request);
  if (phase === "capture") validateInputLiveV14TechnicalGates(directory);
  const rawFile = inputLiveV14ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v14-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V14_BROKER_VERSION ||
...
export function persistInputLiveV14TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV14RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV14RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V14_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v14-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V14_BROKER_VERSION,
...
export function validateInputLiveV14TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v14-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V14_TECHNICAL_GATES.some(
...
export function validateInputLiveV14Receipt(
  directory: string,
  phase: InputLiveV14RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV14Request(directory, request);
  const receipt = json<InputLiveV14Receipt>(
    inputLiveV14ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV14ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v14-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V14_BROKER_VERSION ||
...
export function readInputLiveV14AcceptedRawPayload(
  directory: string,
  phase: InputLiveV14RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV14Receipt(directory, phase, captureIndex);
...
export function createInputLiveV14FakeDynamicResponse(
  request: InputLiveV14Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V14_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V14_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV14PersistedRequests(
  directory: string,
): InputLiveV14Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV14Request>(file)] : [];
    });
}

export const inputLiveV14Sha256 = sha256;
