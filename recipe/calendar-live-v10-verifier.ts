...
import {
  verifyCalendarLiveV10FixedSceneFixedPoint,
  type CalendarLiveV10FixedFixedPoint,
} from "./calendar-live-v10-fixed-point.js";

export const CALENDAR_LIVE_V10_VERIFIER_VERSION = "calendar-live-v10-verifier-v1";

export function buildCalendarLiveV10RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV10RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV10RawNode[];
}

export interface CalendarLiveV10NormalizedScene {
...
export interface CalendarLiveV10SceneProof {
...
export function normalizeCalendarLiveV10Scene(
  root: CalendarLiveV10RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV10NormalizedScene {
  const canonicalBindings: CalendarLiveV10NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV10RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV10Scene<Instance>(
  normalized: CalendarLiveV10NormalizedScene,
...
): CalendarLiveV10SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV10FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV10ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV10PreCaptureGates(payload: unknown): void {
