...
export interface InputLiveV25LifecycleSimulationReport {
  artifactVersion: "input-live-v25-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV25AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV25Lifecycle(): Promise<InputLiveV25LifecycleSimulationReport> {
  await buildInputLiveV25Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V25_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV25AntecedentIndex;
  const indexSha256 = inputLiveV25Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v25-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V25_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V25_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV25History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV25History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV25AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V25_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V25_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV25History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV25History(authorized, "pending");
    ...
    await buildInputLiveV25Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V25_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV25AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV25Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV25Lifecycle(), null, 2)}\n`,
  );
