...
import {
  acceptComboboxLiveV11Response,
  createComboboxLiveV11Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV11Request,
  persistComboboxLiveV11RawResponse,
  persistComboboxLiveV11TechnicalGates,
  readComboboxLiveV11AcceptedRawPayload,
  type ComboboxLiveV11Request,
  type ComboboxLiveV11TransactionAuthorization,
} from "./combobox-live-v11-broker.js";
import {
  COMBOBOX_LIVE_V11_CAPTURE_COUNT,
  assertComboboxLiveV11CaptureResponses,
  ...
  evaluateComboboxLiveV11Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV11Roots,
  validateComboboxLiveV11CaptureManifest,
  validateComboboxLiveV11CapturePayload,
  validateComboboxLiveV11CleanupPayload,
  validateComboboxLiveV11ExtractPayload,
  validateComboboxLiveV11ProbePayload,
  validateComboboxLiveV11WriterPayload,
  type ComboboxLiveV11CaptureCell,
  type ComboboxLiveV11CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V11_PLAN_PATH,
  type ComboboxLiveV11ProofPlan,
} from "./build-combobox-live-proof-v11.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v11.js";

export const COMBOBOX_LIVE_V11_RUNNER_VERSION = "combobox-live-v11-runner-v1";

interface ComboboxLiveV11JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV11OrchestratorState {
  artifactVersion: "combobox-live-v11-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV11JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV11ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV11CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV11SourceIdentity & {
...
export interface ComboboxLiveV11NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV11OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v11 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V11_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V11_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV11ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV11CaptureCell[];
  validateComboboxLiveV11CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v11 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v11 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV11WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV11RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV11Request =>
  readJson<ComboboxLiveV11Request>(file);

export class ComboboxLiveV11Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV11Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV11OrchestratorState = {
      artifactVersion: "combobox-live-v11-orchestrator-state-v1",
    ...
    return new ComboboxLiveV11Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV11OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV11Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV11NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV11NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v11 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV11NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v11 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV11RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV11Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV11CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV11AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV11WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV11Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV11ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV11Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV11Response(this.#directory, "writer", {
        validate: validateComboboxLiveV11WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV11Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV11RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV11Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV11ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV11Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV11RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v11-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV11Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV11ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV11Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV11ProbePayload(payload, ownership),
      });
      persistComboboxLiveV11TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v11 capture cell missing");
      const accepted = acceptComboboxLiveV11Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV11CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V11_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV11CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV11Objective(
    ...
    throw new Error(`Combobox live v11 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV11Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV11CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV11Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV11RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV11Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV11CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV11CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV11CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV11Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV11TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV11SimulationKey = (): KeyObject =>
