...
import { INPUT_LIVE_V33_INDEX_PATH } from "./build-input-field-live-proof-v33.js";
import {
  INPUT_LIVE_V33_DYNAMIC_TOOL,
  INPUT_LIVE_V33_TARGET,
} from "./input-field-live-v33-broker.js";
import {
  buildInputLiveV33AuthorizationArtifact,
  validateInputLiveV33AntecedentIndex,
  validateInputLiveV33AuthorizationArtifact,
  validateInputLiveV33History,
  validateInputLiveV33UnitTestSource,
  type InputLiveV33AntecedentIndex,
  type InputLiveV33AuthorizationArtifact,
  type InputLiveV33AuthorizationProof,
  type InputLiveV33HistoryState,
} from "./input-field-live-v33-authorization.js";
import {
  inputLiveV33AttestationSha256,
  validateInputLiveV33ControlFlowSource,
  validateInputLiveV33SecurityAttestation,
  type InputLiveV33SecurityAttestation,
} from "./input-field-live-v33-preflight.js";
import { inputLiveV33Sha256 } from "./input-field-live-v33-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V33_INDEX_PATH);
const index = (): InputLiveV33AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV33AntecedentIndex;

const artifact = (): InputLiveV33AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV33AuthorizationArtifact({
...
const pendingState = (): InputLiveV33HistoryState => ({
  index: index(),
  indexSha256: inputLiveV33Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV33HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV33AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV33Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V33_TARGET,
  expectedDynamicTool: INPUT_LIVE_V33_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v33/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV33AuthorizationArtifact,
): InputLiveV33SecurityAttestation => {
  const body: Omit<InputLiveV33SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v33-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V33_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV33AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV33History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV33History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV33History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV33History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV33History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV33UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV33UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV33AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV33Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV33AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV33AuthorizationArtifact(
        planted as InputLiveV33AuthorizationArtifact,
        index(),
        inputLiveV33Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV33SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV33AttestationSha256(
      body as Omit<InputLiveV33SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV33SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV33ControlFlowSource(source), []);
});
