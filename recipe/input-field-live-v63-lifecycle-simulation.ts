...
export interface InputLiveV63LifecycleSimulationReport {
  artifactVersion: "input-live-v63-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV63AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV63Lifecycle(): Promise<InputLiveV63LifecycleSimulationReport> {
  await buildInputLiveV63Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V63_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV63AntecedentIndex;
  const indexSha256 = inputLiveV63Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v63-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V63_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V63_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV63History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV63History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV63AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V63_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V63_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV63History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV63History(authorized, "pending");
    ...
    await buildInputLiveV63Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V63_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV63AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV63Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV63Lifecycle(), null, 2)}\n`,
  );
