...
import { buildCalendarLiveV32CleanupRuntime } from "./calendar-live-v32-cleanup.js";
import { type CalendarLiveV32FixedFixedPoint } from "./calendar-live-v32-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV32RestoreProgram,
  validateCalendarLiveV32RestorePayload,
  type CalendarLiveV32RestorePayload,
} from "./calendar-live-v32-restore.js";
import {
  buildCalendarLiveV32RawPropertyRuntime,
  normalizeCalendarLiveV32Scene,
  type CalendarLiveV32RawNode,
} from "./calendar-live-v32-verifier.js";
...
export const CALENDAR_LIVE_V32_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V32_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V32_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V32_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V32_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V32_SET_COUNT = 3;
export const CALENDAR_LIVE_V32_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V32_HOST_PHASES = 3;
export const CALENDAR_LIVE_V32_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V32_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V32_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V32_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV32SourceId = (typeof CALENDAR_LIVE_V32_SOURCE_IDS)[number];
export type CalendarLiveV32Kind = "calendar" | "week" | "day";

export interface CalendarLiveV32SourceIdentity {
  source: CalendarLiveV32SourceId;
...
export interface CalendarLiveV32WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV32ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV32SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV32RawNode;
    weekScene: CalendarLiveV32RawNode;
    dayScene: CalendarLiveV32RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV32RootProof {
  source: CalendarLiveV32SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV32FixedFixedPoint;
}

export interface CalendarLiveV32ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV32SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV32CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV32SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV32CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV32SourceId;
  frameWidth: number;
...
export interface CalendarLiveV32CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v32 ${label} must be an object`);
...
export function validateCalendarLiveV32WriterPayload(
  value: unknown,
): CalendarLiveV32WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v32") ||
    ...
    result.namespace !== CALENDAR_LIVE_V32_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v32 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v32 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v32 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV32SourceIdentity,
  kind: CalendarLiveV32Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV32ExtractProgram(
  writer: CalendarLiveV32WriterOwnership,
  sources: readonly CalendarLiveV32SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v32 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V32_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V32_NAMESPACE)}
${buildCalendarLiveV32RawPropertyRuntime()}
...
export function validateCalendarLiveV32ExtractPayload(
  value: unknown,
  writer: CalendarLiveV32WriterOwnership,
): CalendarLiveV32ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v32 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V32_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v32 extract schema/one-root mismatch");
  return payload as CalendarLiveV32ExtractPayload;
}

export function proveCalendarLiveV32Roots<Instance>(
  extract: CalendarLiveV32ExtractPayload,
  sources: readonly (CalendarLiveV32SourceIdentity & {
...
): CalendarLiveV32RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v32 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v32 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV32Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV32Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV32Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV32FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v32 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v32 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v32 compile lost owned sets");
...
export function assertCalendarLiveV32RootProofs(
  proofs: readonly CalendarLiveV32RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v32 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v32 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV32ProbeProgram(
  writer: CalendarLiveV32WriterOwnership,
  sources: readonly CalendarLiveV32SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V32_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV32ProbePayload(
  value: unknown,
  writer: CalendarLiveV32WriterOwnership,
): CalendarLiveV32ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV32ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V32_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V32_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v32 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV32CaptureManifest(
  cells: readonly CalendarLiveV32CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V32_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V32_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V32_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V32_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v32 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV32CaptureCell[],
): string => {
  validateCalendarLiveV32CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV32CaptureProgram(
  writer: CalendarLiveV32WriterOwnership,
  cell: CalendarLiveV32CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V32_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v32 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v32 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V32_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV32CapturePayload(
  value: unknown,
  cell: CalendarLiveV32CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV32CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV32CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V32_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V32_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v32 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV32CaptureResponses(
  manifest: readonly CalendarLiveV32CaptureCell[],
  responses: readonly CalendarLiveV32CapturePayload[],
): void {
  validateCalendarLiveV32CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v32 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV32ObjectiveReport {
  artifactVersion: "calendar-live-v32-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV32Objective(
  manifest: readonly CalendarLiveV32CaptureCell[],
  responses: readonly CalendarLiveV32CapturePayload[],
): CalendarLiveV32ObjectiveReport {
  assertCalendarLiveV32CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v32-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV32CleanupProgram(
  writer: CalendarLiveV32WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV32CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V32_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v32 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV32CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV32WriterOwnership,
): CalendarLiveV32CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV32CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v32 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V32_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV32WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
