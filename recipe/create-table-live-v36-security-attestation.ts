...
import { verifyTableLiveV36Authorization } from "./table-live-v36-authorization.js";
import { TABLE_LIVE_V36_TARGET } from "./table-live-v36-broker.js";
import {
  TABLE_LIVE_V36_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV36Authorization();
...
const attestation: TableLiveV36SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV36SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v36 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V36_SECURITY_ATTESTATION_DEFAULT_PATH,
);
