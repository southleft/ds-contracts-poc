...
import { calendarLiveV1Sha256 } from "./calendar-live-v9-broker.js";

export interface CalendarLiveV9LifecycleSimulationReport {
  artifactVersion: "calendar-live-v9-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV9AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV9Lifecycle(): Promise<CalendarLiveV9LifecycleSimulationReport> {
  await buildCalendarLiveV9Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V9_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV9AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v9-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V9_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V9_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV9History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV9History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV9AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V9_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V9_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV9History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV9History(authorized, "pending");
    ...
    await buildCalendarLiveV9Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V9_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV9AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV9Lifecycle(), null, 2)}\n`,
  );
