...
import { buildCalendarLiveV35CleanupRuntime } from "./calendar-live-v35-cleanup.js";
import { type CalendarLiveV35FixedFixedPoint } from "./calendar-live-v35-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV35RestoreProgram,
  validateCalendarLiveV35RestorePayload,
  type CalendarLiveV35RestorePayload,
} from "./calendar-live-v35-restore.js";
import {
  buildCalendarLiveV35RawPropertyRuntime,
  normalizeCalendarLiveV35Scene,
  type CalendarLiveV35RawNode,
} from "./calendar-live-v35-verifier.js";
...
export const CALENDAR_LIVE_V35_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V35_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V35_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V35_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V35_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V35_SET_COUNT = 3;
export const CALENDAR_LIVE_V35_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V35_HOST_PHASES = 3;
export const CALENDAR_LIVE_V35_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V35_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V35_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V35_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV35SourceId = (typeof CALENDAR_LIVE_V35_SOURCE_IDS)[number];
export type CalendarLiveV35Kind = "calendar" | "week" | "day";

export interface CalendarLiveV35SourceIdentity {
  source: CalendarLiveV35SourceId;
...
export interface CalendarLiveV35WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV35ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV35SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV35RawNode;
    weekScene: CalendarLiveV35RawNode;
    dayScene: CalendarLiveV35RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV35RootProof {
  source: CalendarLiveV35SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV35FixedFixedPoint;
}

export interface CalendarLiveV35ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV35SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV35CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV35SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV35CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV35SourceId;
  frameWidth: number;
...
export interface CalendarLiveV35CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v35 ${label} must be an object`);
...
export function validateCalendarLiveV35WriterPayload(
  value: unknown,
): CalendarLiveV35WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v35") ||
    ...
    result.namespace !== CALENDAR_LIVE_V35_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v35 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v35 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v35 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV35SourceIdentity,
  kind: CalendarLiveV35Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV35ExtractProgram(
  writer: CalendarLiveV35WriterOwnership,
  sources: readonly CalendarLiveV35SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v35 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V35_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V35_NAMESPACE)}
${buildCalendarLiveV35RawPropertyRuntime()}
...
export function validateCalendarLiveV35ExtractPayload(
  value: unknown,
  writer: CalendarLiveV35WriterOwnership,
): CalendarLiveV35ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v35 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V35_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v35 extract schema/one-root mismatch");
  return payload as CalendarLiveV35ExtractPayload;
}

export function proveCalendarLiveV35Roots<Instance>(
  extract: CalendarLiveV35ExtractPayload,
  sources: readonly (CalendarLiveV35SourceIdentity & {
...
): CalendarLiveV35RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v35 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v35 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV35Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV35Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV35Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV35FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v35 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v35 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v35 compile lost owned sets");
...
export function assertCalendarLiveV35RootProofs(
  proofs: readonly CalendarLiveV35RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v35 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v35 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV35ProbeProgram(
  writer: CalendarLiveV35WriterOwnership,
  sources: readonly CalendarLiveV35SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V35_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV35ProbePayload(
  value: unknown,
  writer: CalendarLiveV35WriterOwnership,
): CalendarLiveV35ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV35ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V35_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V35_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v35 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV35CaptureManifest(
  cells: readonly CalendarLiveV35CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V35_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V35_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V35_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V35_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v35 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV35CaptureCell[],
): string => {
  validateCalendarLiveV35CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV35CaptureProgram(
  writer: CalendarLiveV35WriterOwnership,
  cell: CalendarLiveV35CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V35_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v35 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v35 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V35_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV35CapturePayload(
  value: unknown,
  cell: CalendarLiveV35CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV35CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV35CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V35_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V35_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v35 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV35CaptureResponses(
  manifest: readonly CalendarLiveV35CaptureCell[],
  responses: readonly CalendarLiveV35CapturePayload[],
): void {
  validateCalendarLiveV35CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v35 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV35ObjectiveReport {
  artifactVersion: "calendar-live-v35-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV35Objective(
  manifest: readonly CalendarLiveV35CaptureCell[],
  responses: readonly CalendarLiveV35CapturePayload[],
): CalendarLiveV35ObjectiveReport {
  assertCalendarLiveV35CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v35-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV35CleanupProgram(
  writer: CalendarLiveV35WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV35CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V35_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v35 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV35CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV35WriterOwnership,
): CalendarLiveV35CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV35CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v35 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V35_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV35WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
