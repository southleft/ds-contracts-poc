...
import { buildTableLiveV2CleanupRuntime } from "./table-live-v2-cleanup.js";
import { type TableLiveV2FixedFixedPoint } from "./table-live-v2-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV2RestoreProgram,
  validateTableLiveV2RestorePayload,
  type TableLiveV2RestorePayload,
} from "./table-live-v2-restore.js";
import {
  buildTableLiveV2RawPropertyRuntime,
  normalizeTableLiveV2Scene,
  type TableLiveV2RawNode,
} from "./table-live-v2-verifier.js";
...
export const TABLE_LIVE_V2_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V2_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V2_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V2_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V2_VARIANT_COUNT = 20;
export const TABLE_LIVE_V2_SET_COUNT = 6;
export const TABLE_LIVE_V2_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V2_HOST_PHASES = 3;
export const TABLE_LIVE_V2_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V2_RESTORE_COUNT = 8;
export const TABLE_LIVE_V2_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V2_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV2SourceId = (typeof TABLE_LIVE_V2_SOURCE_IDS)[number];
export type TableLiveV2Kind = "table" | "row" | "cell";

export interface TableLiveV2SourceIdentity {
  source: TableLiveV2SourceId;
...
export interface TableLiveV2WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV2ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV2SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV2RawNode;
    rowScene: TableLiveV2RawNode;
    cellScene: TableLiveV2RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV2RootProof {
  source: TableLiveV2SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV2FixedFixedPoint;
}

export interface TableLiveV2ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV2SourceId;
    adapterIdentity: string;
...
export interface TableLiveV2CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV2SourceId;
  adapterIdentity: string;
...
export interface TableLiveV2CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV2SourceId;
  frameWidth: number;
...
export interface TableLiveV2CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v2 ${label} must be an object`);
...
export function validateTableLiveV2WriterPayload(
  value: unknown,
): TableLiveV2WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V2_NAMESPACE ||
    ...
    throw new TypeError("Table live v2 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v2 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v2 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV2SourceIdentity,
  kind: TableLiveV2Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV2ExtractProgram(
  writer: TableLiveV2WriterOwnership,
  sources: readonly TableLiveV2SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v2 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V2_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V2_NAMESPACE)}
${buildTableLiveV2RawPropertyRuntime()}
...
export function validateTableLiveV2ExtractPayload(
  value: unknown,
  writer: TableLiveV2WriterOwnership,
): TableLiveV2ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v2 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V2_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v2 extract schema/two-root mismatch");
  return payload as TableLiveV2ExtractPayload;
}

export function proveTableLiveV2Roots<Instance>(
  extract: TableLiveV2ExtractPayload,
  sources: readonly (TableLiveV2SourceIdentity & {
...
): TableLiveV2RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v2 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v2 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV2Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV2Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV2Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV2FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v2 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v2 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v2 compile lost owned sets");
...
export function assertTableLiveV2RootProofs(
  proofs: readonly TableLiveV2RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v2 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v2 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV2ProbeProgram(
  writer: TableLiveV2WriterOwnership,
  sources: readonly TableLiveV2SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V2_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
for(const row of byAdapter.values()){
 ...
 const cell0=propertyKey(rowInstance,"Cell 0"),cell0Before=cell0&&rowInstance.componentProperties[cell0].value;let textPropertiesRestored=false;if(cell0){rowInstance.setProperties({[cell0]:"Table v2 deterministic probe"});const changed=nodes(rowInstance).some(node=>node.type==="TEXT"&&node.characters==="Table v2 deterministic probe");rowInstance.setProperties({[cell0]:cell0Before});textPropertiesRestored=changed&&JSON.stringify(original)===JSON.stringify(plain(instance));}
 rowInstance.remove();
...
export function validateTableLiveV2ProbePayload(
  value: unknown,
  writer: TableLiveV2WriterOwnership,
): TableLiveV2ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV2ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V2_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V2_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Table live v2 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV2CaptureManifest(
  cells: readonly TableLiveV2CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V2_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V2_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V2_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V2_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v2 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV2CaptureCell[],
): string => {
  validateTableLiveV2CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV2CaptureProgram(
  writer: TableLiveV2WriterOwnership,
  cell: TableLiveV2CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V2_CAPTURE_COUNT
  )
    throw new TypeError("Table live v2 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v2 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
const frame=figma.createFrame();frame.name="Table v2 ephemeral / "+cell.cellKey;page.appendChild(frame);
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V2_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV2CapturePayload(
  value: unknown,
  cell: TableLiveV2CaptureCell,
  rawResponseBytes: number,
): TableLiveV2CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV2CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V2_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V2_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v2 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV2CaptureResponses(
  manifest: readonly TableLiveV2CaptureCell[],
  responses: readonly TableLiveV2CapturePayload[],
): void {
  validateTableLiveV2CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v2 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV2ObjectiveReport {
  artifactVersion: "table-live-v2-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV2Objective(
  manifest: readonly TableLiveV2CaptureCell[],
  responses: readonly TableLiveV2CapturePayload[],
): TableLiveV2ObjectiveReport {
  assertTableLiveV2CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v2-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV2CleanupProgram(
  writer: TableLiveV2WriterOwnership,
): string {
  const baseProgram = buildTableLiveV2CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V2_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v2 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV2CleanupPayload(
  payload: unknown,
  writer: TableLiveV2WriterOwnership,
): TableLiveV2CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV2CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v2 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V2_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV2WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
