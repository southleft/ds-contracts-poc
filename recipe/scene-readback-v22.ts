...
/**
 * Carried Input live v22 scene-readback. Keeps the v17/v18 slot-fill
 ...
 * layout-height teaching, and aliases those recipe fields from the
 * existing Figma `width` / `width.value` and `height` / `height.value`
 * FLOATs without leaving the source mapped field in place. Does not
 * invent variables, colors, or treat FIXED as a fill.
 */
...
export const INPUT_LIVE_V22_SLOT_FILL_MARKER = "INPUT-SLOT-FILL-FROM-PAYLOAD";
export const INPUT_LIVE_V22_SLOT_COLOR_BINDING_MARKER =
  "INPUT-SLOT-COLOR-BINDING-FROM-CHILD";
export const INPUT_LIVE_V22_SURFACE_STROKE_WEIGHT_MARKER =
  "INPUT-SURFACE-STROKE-WEIGHT-FROM-PER-SIDE";
export const INPUT_LIVE_V22_VARIANT_LAYOUT_WIDTH_MARKER =
  "INPUT-VARIANT-LAYOUT-WIDTH-FROM-WIDTH";
export const INPUT_LIVE_V22_SURFACE_LAYOUT_HEIGHT_MARKER =
  "INPUT-SURFACE-LAYOUT-HEIGHT-FROM-HEIGHT";
export const INPUT_LIVE_V22_VARIANT_LAYOUT_WIDTH_ALIAS_MARKER =
  "INPUT-VARIANT-LAYOUT-WIDTH-ALIAS-NO-DUPLICATE-WIDTH-VALUE";
export const INPUT_LIVE_V22_SURFACE_LAYOUT_HEIGHT_ALIAS_MARKER =
  "INPUT-SURFACE-LAYOUT-HEIGHT-ALIAS-NO-DUPLICATE-HEIGHT-VALUE";
export const INPUT_LIVE_V22_SLOT_ROLES = [
  "input-field/slot/leading",
  "input-field/slot/trailing",
] as const;
export const INPUT_LIVE_V22_SURFACE_ROLES = ["input-field/surface"] as const;
export const INPUT_LIVE_V22_VARIANT_ROLE_PREFIX = "input-field/variant/";
export const INPUT_LIVE_V22_PER_SIDE_STROKE_WEIGHT_FIELDS = [
  "strokeTopWeight",
  "strokeRightWeight",
  "strokeBottomWeight",
  "strokeLeftWeight",
...
const SURFACE_ROLES = new Set<string>(INPUT_LIVE_V22_SURFACE_ROLES);

const surfaceUniformStrokeWeightBinding = (
  scene: SceneNodeSnapshot,
  own: VariableBinding[],
): VariableBinding[] => {
  if (own.some((binding) => binding.field === "strokes.0.weight")) return own;
  const role = sceneRole(scene);
...
const surfaceVariantLayoutWidthBinding = (
  ...
  if (
    role === undefined ||
    !role.startsWith(INPUT_LIVE_V22_VARIANT_ROLE_PREFIX)
  )
    return own;
  ...
  return [
    ...own.filter((binding) => binding.field !== "width.value"),
    { field: "layout.width.value", type: "FLOAT", variable: width.variable },
  ];
};
...
const SLOT_ROLES = new Set<string>(INPUT_LIVE_V22_SLOT_ROLES);
...
export function sceneToNormalizedIr(scene: SceneNodeSnapshot): IRNode {
  const bindings = surfaceLayoutHeightBinding(
    scene,
