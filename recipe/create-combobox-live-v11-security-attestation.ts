...
import { verifyComboboxLiveV11Authorization } from "./combobox-live-v11-authorization.js";
import { COMBOBOX_LIVE_V11_TARGET } from "./combobox-live-v11-broker.js";
import {
  COMBOBOX_LIVE_V11_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV11Authorization();
...
const attestation: ComboboxLiveV11SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV11SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v11 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V11_SECURITY_ATTESTATION_DEFAULT_PATH,
);
