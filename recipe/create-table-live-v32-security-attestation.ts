...
import { verifyTableLiveV32Authorization } from "./table-live-v32-authorization.js";
import { TABLE_LIVE_V32_TARGET } from "./table-live-v32-broker.js";
import {
  TABLE_LIVE_V32_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV32Authorization();
...
const attestation: TableLiveV32SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV32SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v32 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V32_SECURITY_ATTESTATION_DEFAULT_PATH,
);
