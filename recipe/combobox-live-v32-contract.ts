...
import { buildComboboxLiveV32CleanupRuntime } from "./combobox-live-v32-cleanup.js";
import {
  type ComboboxLiveV32FixedFixedPoint,
} from "./combobox-live-v32-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV32RestoreProgram,
  validateComboboxLiveV32RestorePayload,
  type ComboboxLiveV32RestorePayload,
} from "./combobox-live-v32-restore.js";
import {
  buildComboboxLiveV32RawPropertyRuntime,
  normalizeComboboxLiveV32Scene,
  type ComboboxLiveV32RawNode,
} from "./combobox-live-v32-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v32.js";
import {
...
export const COMBOBOX_LIVE_V32_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V32_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V32_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V32_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V32_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V32_SET_COUNT = 4;
export const COMBOBOX_LIVE_V32_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V32_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V32_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V32_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V32_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV32SourceId = (typeof COMBOBOX_LIVE_V32_SOURCE_IDS)[number];

export interface ComboboxLiveV32SourceIdentity {
  source: ComboboxLiveV32SourceId;
...
export interface ComboboxLiveV32WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV32ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV32SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV32RawNode;
    optionScene: ComboboxLiveV32RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV32RootProof {
  source: ComboboxLiveV32SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV32FixedFixedPoint;
}

export interface ComboboxLiveV32ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV32SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV32CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV32SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV32CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV32SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV32CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v32 ${label} must be an object`);
...
export function validateComboboxLiveV32WriterPayload(
  value: unknown,
): ComboboxLiveV32WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V32_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v32 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v32 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v32 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV32SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV32ExtractProgram(
  writer: ComboboxLiveV32WriterOwnership,
  sources: readonly ComboboxLiveV32SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v32 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V32_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V32_NAMESPACE)}
${buildComboboxLiveV32RawPropertyRuntime()}
...
export function validateComboboxLiveV32ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV32WriterOwnership,
): ComboboxLiveV32ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v32 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V32_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v32 extract schema/two-root mismatch");
  return payload as ComboboxLiveV32ExtractPayload;
}

export function proveComboboxLiveV32Roots<Instance>(
  extract: ComboboxLiveV32ExtractPayload,
  sources: readonly (ComboboxLiveV32SourceIdentity & {
...
): ComboboxLiveV32RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v32 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v32 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV32Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV32Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV32FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v32 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v32 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v32 compile lost owned sets");
...
export function assertComboboxLiveV32RootProofs(
  proofs: readonly ComboboxLiveV32RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v32 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v32 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV32ProbeProgram(
  writer: ComboboxLiveV32WriterOwnership,
  sources: readonly ComboboxLiveV32SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V32_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV32ProbePayload(
  value: unknown,
  writer: ComboboxLiveV32WriterOwnership,
): ComboboxLiveV32ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV32ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V32_VARIANT_COUNT ||
    ...
    throw new TypeError("Combobox live v32 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV32CaptureManifest(
  cells: readonly ComboboxLiveV32CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V32_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V32_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V32_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V32_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v32 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV32CaptureCell[],
): string => {
  validateComboboxLiveV32CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV32CaptureProgram(
  writer: ComboboxLiveV32WriterOwnership,
  cell: ComboboxLiveV32CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V32_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v32 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v32 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V32_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV32CapturePayload(
  value: unknown,
  cell: ComboboxLiveV32CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV32CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV32CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V32_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V32_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v32 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV32CaptureResponses(
  manifest: readonly ComboboxLiveV32CaptureCell[],
  responses: readonly ComboboxLiveV32CapturePayload[],
): void {
  validateComboboxLiveV32CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v32 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV32ObjectiveReport {
  artifactVersion: "combobox-live-v32-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV32Objective(
  manifest: readonly ComboboxLiveV32CaptureCell[],
  responses: readonly ComboboxLiveV32CapturePayload[],
): ComboboxLiveV32ObjectiveReport {
  assertComboboxLiveV32CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v32-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV32CleanupProgram(
  writer: ComboboxLiveV32WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV32CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V32_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v32 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV32CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV32WriterOwnership,
): ComboboxLiveV32CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV32CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v32 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V32_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV32WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
