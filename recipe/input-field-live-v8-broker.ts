...
import {
  assertInputLiveV8RootProofs,
  INPUT_LIVE_V8_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V8_RESPONSE_CONTRACTS,
  type InputLiveV8RootProof,
} from "./input-field-live-v8-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V8_BROKER_VERSION = "input-live-v8-broker-v2";
export const INPUT_LIVE_V8_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V8_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V8_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V8_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V8_REMOTE_PHASES = [
  "writer",
  "extract",
  "probe",
  "capture",
...
export const INPUT_LIVE_V8_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV8RemotePhase =
  (typeof INPUT_LIVE_V8_REMOTE_PHASES)[number];
export type InputLiveV8AuthorizationMode = "live" | "simulated";

export interface InputLiveV8CommitPins {
...
export interface InputLiveV8TransactionAuthorization extends InputLiveV8CommitPins {
  mode: InputLiveV8AuthorizationMode;
...
export interface InputLiveV8Manifest {
  artifactVersion: "input-live-v8-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V8_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V8_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V8_TARGET;
  authorization: InputLiveV8TransactionAuthorization;
...
export interface InputLiveV8ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV8RequestBody {
  artifactVersion: "input-live-v8-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V8_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV8RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV8CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V8_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV8ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV8Request extends InputLiveV8RequestBody {
...
export interface InputLiveV8Receipt {
  artifactVersion: "input-live-v8-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V8_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV8RemotePhase;
...
export interface InputLiveV8AcceptedResponse<T = unknown> {
  request: InputLiveV8Request;
  receipt: InputLiveV8Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV8IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV8ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V8_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V8_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V8_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V8_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V8_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V8_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V8_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Input live v8 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    throw new TypeError("Input live v8 capture index must be 0..127");
  return `capture-${String(captureIndex).padStart(3, "0")}`;
};

export const inputLiveV8RequestSequence = (
  phase: InputLiveV8RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV8RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV8RequestPath = (
  directory: string,
  phase: InputLiveV8RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV8ResponsePath = (
  directory: string,
  phase: InputLiveV8RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV8ReceiptPath = (
  directory: string,
  phase: InputLiveV8RemotePhase,
  captureIndex?: number,
): string =>
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Input live v8 authorization pins are malformed");
};

const readManifest = (directory: string): InputLiveV8Manifest => {
  const manifest = json<InputLiveV8Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Input live v8 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "input-live-v8-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V8_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V8_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Input live v8 broker manifest");
  return manifest;
};

const unsignedBody = (request: InputLiveV8Request): InputLiveV8RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV8RemotePhase,
  captureIndex?: number,
): InputLiveV8Request | undefined => {
  const file = inputLiveV8RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV8Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV8RemotePhase,
  captureIndex?: number,
): InputLiveV8Receipt | undefined => {
  const file = inputLiveV8ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV8Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV8Request | undefined;
  receipt: InputLiveV8Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV8RemotePhase,
): InputLiveV8ResponseContract => ({
  ...INPUT_LIVE_V8_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V8_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v8-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V8_BROKER_VERSION,
  target: INPUT_LIVE_V8_TARGET,
  ...request,
})};
if(figma.fileKey!==__broker.target.fileKey||figma.root.name!==__broker.target.fileName||figma.editorType!==__broker.target.editorType)throw new Error("INPUT-LIVE-V8-WRONG-TARGET");
const payload=await (async()=>{/*INPUT_LIVE_V8_PROGRAM_START*/${program}/*INPUT_LIVE_V8_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V8_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V8_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Input live v8 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createInputLiveV8Transaction(
  directory: string,
  ...
  if (existsSync(path.join(directory, MANIFEST_FILE)))
    throw new Error("Input live v8 transaction already exists");
  validateAuthorization(options.authorization);
  if (
    !SHA256.test(options.proofPlanSha256) ||
    !SHA256.test(options.captureManifestSha256)
  )
    throw new TypeError("Input live v8 transaction plan hashes malformed");
  const attempt = options.attempt ?? 1;
  if (!Number.isInteger(attempt) || attempt < 1 || attempt > 3)
    throw new TypeError("Input live v8 attempt must be 1..3");
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Input live v8 private signing key does not match authorized public-key identity",
    );
  const manifest: InputLiveV8Manifest = {
    artifactVersion: "input-live-v8-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V8_BROKER_VERSION,
...
export function validateInputLiveV8Request(
  directory: string,
  request: InputLiveV8Request,
): void {
  ...
  const expectedSequence = inputLiveV8RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v8-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V8_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V8_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Input live v8 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueInputLiveV8Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV8RemotePhase,
  program: string,
  options: InputLiveV8IssueOptions = {},
): InputLiveV8Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV8RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV8Request>(targetPath);
    validateInputLiveV8Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v8 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV8ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Input live v8 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Input live v8 empty code envelope refused");
  if (phase === "capture") validateInputLiveV8TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Input live v8 out-of-order phase: ${id}`);
  const expectedResponse =
    options.expectedResponse ?? responseContractFor(phase);
  const programSha256 = sha256(program);
  const sequence = inputLiveV8RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V8_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V8_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V8_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV8RequestBody = {
    artifactVersion: "input-live-v8-broker-request-v2",
    brokerVersion: INPUT_LIVE_V8_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV8Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV8Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV8RawResponse(
  directory: string,
  phase: InputLiveV8RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV8RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v8 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV8Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Input live v8 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = inputLiveV8ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Input live v8 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV8Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v8-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V8_BROKER_VERSION,
...
export function acceptInputLiveV8Response<T = unknown>(
  directory: string,
  phase: InputLiveV8RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV8AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV8ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v8 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV8Request>(
    inputLiveV8RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV8Request(directory, request);
  if (phase === "capture") validateInputLiveV8TechnicalGates(directory);
  const rawFile = inputLiveV8ResponsePath(directory, phase, captureIndex);
  if (!existsSync(rawFile))
    throw new Error(`Input live v8 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v8-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V8_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Input live v8 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistInputLiveV8TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV8RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV8RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Input live v8 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Input live v8 probe payload hash malformed");
  const gates = Object.fromEntries(
    INPUT_LIVE_V8_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v8-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V8_BROKER_VERSION,
...
export function validateInputLiveV8TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v8-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V8_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Input live v8 technical gates artifact");
}

export function validateInputLiveV8Receipt(
  directory: string,
  phase: InputLiveV8RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV8Request(directory, request);
  const receipt = json<InputLiveV8Receipt>(
    inputLiveV8ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV8ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v8-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V8_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Input live v8 accepted receipt: ${request.requestId}`,
    );
}

export function readInputLiveV8AcceptedRawPayload(
  directory: string,
  phase: InputLiveV8RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV8Receipt(directory, phase, captureIndex);
...
export function createInputLiveV8FakeDynamicResponse(
  request: InputLiveV8Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V8_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V8_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV8PersistedRequests(
  directory: string,
): InputLiveV8Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV8Request>(file)] : [];
    });
}

export const inputLiveV8Sha256 = sha256;
