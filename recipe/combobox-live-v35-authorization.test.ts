...
import { COMBOBOX_LIVE_V35_INDEX_PATH } from "./build-combobox-live-proof-v35.js";
import {
  COMBOBOX_LIVE_V35_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V35_TARGET,
} from "./combobox-live-v35-broker.js";
import {
  buildComboboxLiveV35AuthorizationArtifact,
  validateComboboxLiveV35AntecedentIndex,
  validateComboboxLiveV35AuthorizationArtifact,
  validateComboboxLiveV35History,
  validateComboboxLiveV35UnitTestSource,
  type ComboboxLiveV35AntecedentIndex,
  type ComboboxLiveV35AuthorizationArtifact,
  type ComboboxLiveV35AuthorizationProof,
  type ComboboxLiveV35HistoryState,
} from "./combobox-live-v35-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV35ControlFlowSource,
  validateComboboxLiveV35SecurityAttestation,
  type ComboboxLiveV35SecurityAttestation,
} from "./combobox-live-v35-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v35-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V35_INDEX_PATH);
const index = (): ComboboxLiveV35AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV35AntecedentIndex;

const artifact = (): ComboboxLiveV35AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV35AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV35HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV35HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV35AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v35/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV35AuthorizationArtifact,
): ComboboxLiveV35SecurityAttestation => {
  const body: Omit<ComboboxLiveV35SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V35_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV35History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV35History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV35History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV35History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV35History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV35UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV35UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV35AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV35AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV35AuthorizationArtifact(
        planted as ComboboxLiveV35AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV35SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV35SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV35SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV35ControlFlowSource(source), []);
});
