...
export type CalendarLiveV6HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV6AuthorizationArtifact {
  artifactVersion: "calendar-live-v6-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V6_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV6AntecedentIndex {
...
export interface CalendarLiveV6HistoryState {
  index?: CalendarLiveV6AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV6AuthorizationArtifact;
...
export interface CalendarLiveV6AuthorizationProof
  extends CalendarLiveV6TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V6_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V6_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V6_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V6_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v6-authorization") ||
  artifactPath.includes("calendar-live-v6-preflight") ||
  artifactPath.includes("calendar-live-v6-authorized") ||
  artifactPath.includes("create-calendar-live-v6-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v6-status") ||
...
export function validateCalendarLiveV6AntecedentIndex(
  index: CalendarLiveV6AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v6-antecedent-index-v1" ||
...
export function validateCalendarLiveV6UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV6History\(\s*["']/.test(source) ||
    /readCalendarLiveV6HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV6AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV6AuthorizationArtifact {
  ...
  ) as CalendarLiveV6AntecedentIndex;
  const indexFailures = validateCalendarLiveV6AntecedentIndex(index);
...
export function validateCalendarLiveV6AuthorizationArtifact(
  artifact: CalendarLiveV6AuthorizationArtifact | undefined,
  index: CalendarLiveV6AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v6-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v6" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V6_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V6_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V6_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V6_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V6_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V6_SOURCE_ROOTS ||
  ...
  if (
    ...
    artifact?.execution?.calendarLiveV5AuthorizationReusable !== false ||
...
export function validateCalendarLiveV6History(
  state: CalendarLiveV6HistoryState,
  expected: CalendarLiveV6HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV6AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV6AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV6HistoryState(
  root = process.cwd(),
): CalendarLiveV6HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V6_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV6AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V6_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V6_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V6_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV6History(
  expected: CalendarLiveV6HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV6HistoryState {
  const state = readCalendarLiveV6HistoryState(root);
  const failures = validateCalendarLiveV6History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v6 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV6Authorization(
  root = process.cwd(),
): CalendarLiveV6AuthorizationProof {
  const state = verifyCalendarLiveV6History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V6_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v6/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v6.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V6_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V6_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V6_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV6HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV6History(expected);
  process.stdout.write(
    `Calendar live v6 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
