...
import {
  acceptTableLiveV38Response,
  createTableLiveV38Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV38Request,
  persistTableLiveV38RawResponse,
  persistTableLiveV38TechnicalGates,
  readTableLiveV38AcceptedRawPayload,
  type TableLiveV38Request,
  type TableLiveV38TransactionAuthorization,
} from "./table-live-v38-broker.js";
import {
  TABLE_LIVE_V38_CAPTURE_COUNT,
  assertTableLiveV38CaptureResponses,
  ...
  evaluateTableLiveV38Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV38Roots,
  validateTableLiveV38CaptureManifest,
  validateTableLiveV38CapturePayload,
  validateTableLiveV38CleanupPayload,
  validateTableLiveV38ExtractPayload,
  validateTableLiveV38ProbePayload,
  validateTableLiveV38WriterPayload,
  type TableLiveV38CaptureCell,
  type TableLiveV38CapturePayload,
...
import {
  ...
  TABLE_LIVE_V38_PLAN_PATH,
  type TableLiveV38ProofPlan,
...
export const TABLE_LIVE_V38_RUNNER_VERSION = "table-live-v38-runner-v1";

interface TableLiveV38JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV38OrchestratorState {
  artifactVersion: "table-live-v38-orchestrator-state-v1";
  ...
  journal: TableLiveV38JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV38ProofPlan;
  planSha256: string;
  captures: TableLiveV38CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV38SourceIdentity & {
...
export interface TableLiveV38NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV38OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v38 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V38_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V38_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV38ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV38CaptureCell[];
  validateTableLiveV38CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v38 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v38 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV38WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV38RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV38Request =>
  readJson<TableLiveV38Request>(file);

export class TableLiveV38Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV38Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV38OrchestratorState = {
      artifactVersion: "table-live-v38-orchestrator-state-v1",
    ...
    return new TableLiveV38Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV38OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV38Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV38NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV38NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v38 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV38NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v38 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV38RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV38Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV38CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV38AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV38WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV38Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV38ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV38Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV38Response(this.#directory, "writer", {
        validate: validateTableLiveV38WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV38Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV38RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV38Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV38ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV38Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV38RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v38-host-root-proofs-v1",
      ...
      const next = issueTableLiveV38Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV38ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV38Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV38ProbePayload(payload, ownership),
      });
      persistTableLiveV38TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v38 capture cell missing");
      const accepted = acceptTableLiveV38Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV38CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V38_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV38CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV38Objective(
    ...
    throw new Error(`Table live v38 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV38Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV38CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV38Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV38RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV38Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV38CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV38CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV38CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV38Authorization = (
  privateKey: KeyObject,
): TableLiveV38TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV38SimulationKey = (): KeyObject =>
