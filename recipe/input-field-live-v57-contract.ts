...
} from "./input-field-live-v3-verifier-v57.js";
import {
  buildInputLiveV57RawPropertyRuntime,
  normalizeInputLiveV57Scene,
  type InputLiveV57RawNode,
} from "./input-field-live-v57-verifier.js";
...
export {
  ...
  type InputLiveV57RestorePayload,
} from "./input-field-live-v57-restore.js";

export const INPUT_LIVE_V57_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V57_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V57_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V57_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V57_VARIANT_COUNT = 256;
export const INPUT_LIVE_V57_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V57_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV57SourceId = (typeof INPUT_LIVE_V57_SOURCE_IDS)[number];

export interface InputLiveV57SourceIdentity {
  source: InputLiveV57SourceId;
...
export interface InputLiveV57WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV57ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV57SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV57RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV57RootProof {
  source: InputLiveV57SourceId;
  adapterIdentity: string;
...
export interface InputLiveV57ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV57SourceId;
    adapterIdentity: string;
...
export interface InputLiveV57CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV57SourceId;
  adapterIdentity: string;
...
export interface InputLiveV57CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV57SourceId;
  strata: InputLiveV57CaptureCell["strata"];
...
export interface InputLiveV57CleanupPayload {
...
export function validateInputLiveV57WriterPayload(
  value: unknown,
): InputLiveV57WriterOwnership {
...
const identityPlan = (source: InputLiveV57SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV57ExtractProgram(
  writer: InputLiveV57WriterOwnership,
  sources: readonly InputLiveV57SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V57_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V57_NAMESPACE)}
${buildInputLiveV57RawPropertyRuntime()}
...
export function validateInputLiveV57ExtractPayload(
  value: unknown,
  writer: InputLiveV57WriterOwnership,
): InputLiveV57ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V57_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV57ExtractPayload;
}

export function proveInputLiveV57Roots<Instance>(
  extract: InputLiveV57ExtractPayload,
  sources: readonly (InputLiveV57SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV57Scene(
      root.scene,
...
export function assertInputLiveV57RootProofs(
  proofs: readonly InputLiveV57RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV57ProbeProgram(
  writer: InputLiveV57WriterOwnership,
  sources: readonly InputLiveV57SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V57_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV57ProbePayload(
  value: unknown,
  writer: InputLiveV57WriterOwnership,
): InputLiveV57ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV57ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V57_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V57_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV57CaptureManifest(
  cells: readonly InputLiveV57CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V57_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V57_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V57_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V57_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V57_SOURCE_IDS.some(
...
export const inputLiveV57CaptureManifestSha256 = (
  cells: readonly InputLiveV57CaptureCell[],
): string => {
  validateInputLiveV57CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV57CaptureProgram(
  writer: InputLiveV57WriterOwnership,
  cell: InputLiveV57CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V57_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V57_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V57_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV57CapturePayload(
  value: unknown,
  cell: InputLiveV57CaptureCell,
  rawResponseBytes: number,
): InputLiveV57CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV57CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V57_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V57_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV57CaptureResponses(
  manifest: readonly InputLiveV57CaptureCell[],
  responses: readonly InputLiveV57CapturePayload[],
): void {
  validateInputLiveV57CaptureManifest(manifest);
  if (
...
export interface InputLiveV57ObjectiveReport {
  artifactVersion: "input-live-v57-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV57Objective(
  manifest: readonly InputLiveV57CaptureCell[],
  responses: readonly InputLiveV57CapturePayload[],
  readReference: (cell: InputLiveV57CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV57CaptureCell) => Buffer,
): InputLiveV57ObjectiveReport {
  assertInputLiveV57CaptureResponses(manifest, responses);
...
export function buildInputLiveV57CleanupProgram(
  writer: InputLiveV57WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V57_NAMESPACE,
...
export function validateInputLiveV57CleanupPayload(
  value: unknown,
  writer: InputLiveV57WriterOwnership,
): InputLiveV57CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV57CleanupPayload;
...
export const INPUT_LIVE_V57_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV57WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
