...
import { COMBOBOX_LIVE_V9_TARGET } from "./combobox-live-v9-broker.js";
import {
  COMBOBOX_LIVE_V9_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV9Authorization();
...
const attestation: ComboboxLiveV9SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV9SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v9 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V9_SECURITY_ATTESTATION_DEFAULT_PATH,
);
