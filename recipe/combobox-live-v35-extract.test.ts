...
test("host omits empty extract instance payload; writer is unchanged", () => {
  const host = readFileSync("recipe/scene-readback-combobox-v35.ts", "utf8");
...
test("host emits compile-carried label Combobox option on combobox/option-set; writer is unchanged", () => {
  ...
  assert.match(
    host,
    /if \(sceneRole\(scene\) === "combobox\/option-set"\)\s+return COMBOBOX_LIVE_V35_OPTION_SET_COMPILE_CARRY_LABEL/,
  );
  assert.match(
    host,
    /COMBOBOX_LIVE_V35_OPTION_SET_COMPILE_CARRY_LABEL =\s+"Combobox option"/,
  );
