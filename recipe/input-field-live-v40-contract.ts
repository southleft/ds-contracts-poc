...
} from "./input-field-live-v3-verifier-v40.js";
import {
  buildInputLiveV40RawPropertyRuntime,
  normalizeInputLiveV40Scene,
  type InputLiveV40RawNode,
} from "./input-field-live-v40-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v40.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v40.js";
...
export const INPUT_LIVE_V40_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V40_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V40_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V40_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V40_VARIANT_COUNT = 256;
export const INPUT_LIVE_V40_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V40_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV40SourceId = (typeof INPUT_LIVE_V40_SOURCE_IDS)[number];

export interface InputLiveV40SourceIdentity {
  source: InputLiveV40SourceId;
...
export interface InputLiveV40WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV40ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV40SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV40RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV40RootProof {
  source: InputLiveV40SourceId;
  adapterIdentity: string;
...
export interface InputLiveV40ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV40SourceId;
    adapterIdentity: string;
...
export interface InputLiveV40CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV40SourceId;
  adapterIdentity: string;
...
export interface InputLiveV40CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV40SourceId;
  strata: InputLiveV40CaptureCell["strata"];
...
export interface InputLiveV40CleanupPayload {
...
export function validateInputLiveV40WriterPayload(
  value: unknown,
): InputLiveV40WriterOwnership {
...
const identityPlan = (source: InputLiveV40SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV40ExtractProgram(
  writer: InputLiveV40WriterOwnership,
  sources: readonly InputLiveV40SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V40_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V40_NAMESPACE)}
${buildInputLiveV40RawPropertyRuntime()}
...
export function validateInputLiveV40ExtractPayload(
  value: unknown,
  writer: InputLiveV40WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V40_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV40ExtractPayload;
}

export function proveInputLiveV40Roots<Instance>(
  extract: InputLiveV40ExtractPayload,
  sources: readonly (InputLiveV40SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV40Scene(
      root.scene,
...
export function assertInputLiveV40RootProofs(
  proofs: readonly InputLiveV40RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV40ProbeProgram(
  writer: InputLiveV40WriterOwnership,
  sources: readonly InputLiveV40SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V40_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV40ProbePayload(
  value: unknown,
  writer: InputLiveV40WriterOwnership,
): InputLiveV40ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV40ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V40_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V40_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV40CaptureManifest(
  cells: readonly InputLiveV40CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V40_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V40_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V40_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V40_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V40_SOURCE_IDS.some(
...
export const inputLiveV40CaptureManifestSha256 = (
  cells: readonly InputLiveV40CaptureCell[],
): string => {
  validateInputLiveV40CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV40CaptureProgram(
  writer: InputLiveV40WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V40_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V40_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V40_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV40CapturePayload(
  value: unknown,
  cell: InputLiveV40CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV40CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V40_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V40_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV40CaptureResponses(
  manifest: readonly InputLiveV40CaptureCell[],
  responses: readonly InputLiveV40CapturePayload[],
): void {
  validateInputLiveV40CaptureManifest(manifest);
  if (
...
export interface InputLiveV40ObjectiveReport {
  artifactVersion: "input-live-v40-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV40Objective(
  manifest: readonly InputLiveV40CaptureCell[],
  responses: readonly InputLiveV40CapturePayload[],
  readReference: (cell: InputLiveV40CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV40CaptureCell) => Buffer,
): InputLiveV40ObjectiveReport {
  assertInputLiveV40CaptureResponses(manifest, responses);
...
export function buildInputLiveV40CleanupProgram(
  writer: InputLiveV40WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V40_NAMESPACE,
...
export function validateInputLiveV40CleanupPayload(
  value: unknown,
  writer: InputLiveV40WriterOwnership,
): InputLiveV40CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV40CleanupPayload;
...
export const INPUT_LIVE_V40_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV40WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
