...
import {
  ...
  createInputLiveV77FakeDynamicResponse,
  ...
  listInputLiveV77PersistedRequests,
  persistInputLiveV77RawResponse,
  persistInputLiveV77TechnicalGates,
  validateInputLiveV77Receipt,
  validateInputLiveV77Request,
  type InputLiveV77RemotePhase,
...
import {
  ...
  type InputLiveV77ExtractPayload,
  type InputLiveV77ProbePayload,
...
import {
  ...
  simulatedInputLiveV77Authorization,
} from "./run-input-field-live-v77.js";
...
const captures: InputLiveV77CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV77Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v77-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV77Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV77CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV77FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV77FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V77_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V77_TARGET);
  persistInputLiveV77RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV77Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV77Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV77WriterOwnership,
...
const rootProofs = (): InputLiveV77RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v77-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV77Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV77Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV77RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV77RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV77ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV77ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV77CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV77CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV77CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV77Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV77CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV77PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV77ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV77Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV77Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV77Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV77Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V77_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV77RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V77_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV77Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV77Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV77ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV77Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV77WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV77RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV77Request;
    validateInputLiveV77Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV77Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV77CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v77-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV77Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV77Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV77Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V77_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV77RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V77_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV77Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV77Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV77RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V77_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV77Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV77Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV77Request;
