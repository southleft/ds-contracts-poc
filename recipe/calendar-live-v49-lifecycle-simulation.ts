...
import { calendarLiveV1Sha256 } from "./calendar-live-v49-broker.js";

export interface CalendarLiveV49LifecycleSimulationReport {
  artifactVersion: "calendar-live-v49-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV49AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV49Lifecycle(): Promise<CalendarLiveV49LifecycleSimulationReport> {
  await buildCalendarLiveV49Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V49_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV49AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v49-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V49_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V49_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV49History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV49History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV49AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V49_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V49_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV49History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV49History(authorized, "pending");
    ...
    await buildCalendarLiveV49Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V49_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV49AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV49Lifecycle(), null, 2)}\n`,
  );
