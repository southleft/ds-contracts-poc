...
import {
  ...
  COMBOBOX_LIVE_V17_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  COMBOBOX_LIVE_V17_RESPONSE_CONTRACTS,
  type ComboboxLiveV17RootProof,
} from "./combobox-live-v17-contract.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V17_BROKER_VERSION = "combobox-live-v17-broker-v2";
export const COMBOBOX_LIVE_V17_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const COMBOBOX_LIVE_V17_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const COMBOBOX_LIVE_V17_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const COMBOBOX_LIVE_V17_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const COMBOBOX_LIVE_V17_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const COMBOBOX_LIVE_V17_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type ComboboxLiveV17RemotePhase =
  (typeof COMBOBOX_LIVE_V17_REMOTE_PHASES)[number];
export type ComboboxLiveV17AuthorizationMode = "live" | "simulated";

export interface ComboboxLiveV17CommitPins {
...
export interface ComboboxLiveV17TransactionAuthorization extends ComboboxLiveV17CommitPins {
  mode: ComboboxLiveV17AuthorizationMode;
...
export interface ComboboxLiveV17Manifest {
  artifactVersion: "combobox-live-v17-broker-manifest-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V17_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V17_DYNAMIC_TOOL;
  target: typeof COMBOBOX_LIVE_V17_TARGET;
...
export interface ComboboxLiveV17ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface ComboboxLiveV17RequestBody {
  artifactVersion: "combobox-live-v17-broker-request-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V17_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: ComboboxLiveV17RemotePhase;
  sequence: number;
  ...
  commitPins: ComboboxLiveV17CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof COMBOBOX_LIVE_V17_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: ComboboxLiveV17ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface ComboboxLiveV17Request extends ComboboxLiveV17RequestBody {
...
export interface ComboboxLiveV17Receipt {
  artifactVersion: "combobox-live-v17-broker-receipt-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V17_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: ComboboxLiveV17RemotePhase;
...
export interface ComboboxLiveV17AcceptedResponse<T = unknown> {
  request: ComboboxLiveV17Request;
  receipt: ComboboxLiveV17Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface ComboboxLiveV17IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: ComboboxLiveV17ResponseContract;
...
const equalTarget = (value: unknown): value is typeof COMBOBOX_LIVE_V17_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === COMBOBOX_LIVE_V17_TARGET.fileKey &&
  (value as BrokerTarget).fileName === COMBOBOX_LIVE_V17_TARGET.fileName &&
  (value as BrokerTarget).editorType === COMBOBOX_LIVE_V17_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== COMBOBOX_LIVE_V17_TARGET.fileKey ||
    context.fileName !== COMBOBOX_LIVE_V17_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === COMBOBOX_LIVE_V17_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Combobox live v17 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= COMBOBOX_LIVE_V17_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v17 capture index must be 0..71");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Combobox live v17 authorization pins are malformed");
};

const readManifest = (directory: string): ComboboxLiveV17Manifest => {
  const manifest = json<ComboboxLiveV17Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Combobox live v17 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "combobox-live-v17-broker-manifest-v2" ||
    manifest.brokerVersion !== COMBOBOX_LIVE_V17_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== COMBOBOX_LIVE_V17_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(COMBOBOX_LIVE_V17_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Combobox live v17 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: ComboboxLiveV17Request,
): ComboboxLiveV17RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: ComboboxLiveV17RemotePhase,
  captureIndex?: number,
): ComboboxLiveV17Request | undefined => {
  const file = comboboxLiveV5RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<ComboboxLiveV17Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: ComboboxLiveV17RemotePhase,
  captureIndex?: number,
): ComboboxLiveV17Receipt | undefined => {
  const file = comboboxLiveV5ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<ComboboxLiveV17Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: ComboboxLiveV17Request | undefined;
  receipt: ComboboxLiveV17Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: ComboboxLiveV17RemotePhase,
): ComboboxLiveV17ResponseContract => ({
  ...COMBOBOX_LIVE_V17_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: ComboboxLiveV17Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: COMBOBOX_LIVE_V17_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: COMBOBOX_LIVE_V17_DYNAMIC_TOOL,
      target: COMBOBOX_LIVE_V17_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "combobox-live-v17-plugin-response-v2",
  brokerVersion: COMBOBOX_LIVE_V17_BROKER_VERSION,
  target: COMBOBOX_LIVE_V17_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*COMBOBOX_LIVE_V17_PROGRAM_START*/${program}/*COMBOBOX_LIVE_V17_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*COMBOBOX_LIVE_V17_PROGRAM_START*/";
  const endMarker = "/*COMBOBOX_LIVE_V17_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Combobox live v17 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createComboboxLiveV17Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Combobox live v17 private signing key does not match authorized public-key identity",
    );
  const manifest: ComboboxLiveV17Manifest = {
    artifactVersion: "combobox-live-v17-broker-manifest-v2",
    brokerVersion: COMBOBOX_LIVE_V17_BROKER_VERSION,
    ...
    captureRequests: COMBOBOX_LIVE_V17_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateComboboxLiveV17Request(
  directory: string,
  request: ComboboxLiveV17Request,
): void {
  ...
  if (
    request.artifactVersion !== "combobox-live-v17-broker-request-v2" ||
    request.brokerVersion !== COMBOBOX_LIVE_V17_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== COMBOBOX_LIVE_V17_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Combobox live v17 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueComboboxLiveV17Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: ComboboxLiveV17RemotePhase,
  program: string,
  options: ComboboxLiveV17IssueOptions = {},
): ComboboxLiveV17Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<ComboboxLiveV17Request>(targetPath);
    validateComboboxLiveV17Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Combobox live v17 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(comboboxLiveV5ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Combobox live v17 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Combobox live v17 empty code envelope refused");
  if (phase === "capture") validateComboboxLiveV17TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Combobox live v17 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: COMBOBOX_LIVE_V17_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? COMBOBOX_LIVE_V17_SIGNED_WRITER_TIMEOUT_MS
        : COMBOBOX_LIVE_V17_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: ComboboxLiveV17RequestBody = {
    artifactVersion: "combobox-live-v17-broker-request-v2",
    brokerVersion: COMBOBOX_LIVE_V17_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: ComboboxLiveV17Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateComboboxLiveV17Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistComboboxLiveV17RawResponse(
  directory: string,
  phase: ComboboxLiveV17RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Combobox live v17 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<ComboboxLiveV17Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Combobox live v17 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = comboboxLiveV5ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Combobox live v17 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): ComboboxLiveV17Receipt => {
  const body: Omit<ComboboxLiveV17Receipt, "receiptSha256"> = {
    artifactVersion: "combobox-live-v17-broker-receipt-v2",
    brokerVersion: COMBOBOX_LIVE_V17_BROKER_VERSION,
...
export function acceptComboboxLiveV17Response<T = unknown>(
  directory: string,
  phase: ComboboxLiveV17RemotePhase,
  options: {
    captureIndex?: number;
...
): ComboboxLiveV17AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = comboboxLiveV5ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Combobox live v17 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<ComboboxLiveV17Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Combobox live v17 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "combobox-live-v17-plugin-response-v2" ||
    plugin?.brokerVersion !== COMBOBOX_LIVE_V17_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Combobox live v17 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistComboboxLiveV17TechnicalGates(
  directory: string,
  rootProofs: readonly ComboboxLiveV17RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertComboboxLiveV17RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Combobox live v17 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Combobox live v17 probe payload hash malformed");
  const gates = Object.fromEntries(
    COMBOBOX_LIVE_V17_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "combobox-live-v17-host-technical-gates-v2",
    brokerVersion: COMBOBOX_LIVE_V17_BROKER_VERSION,
...
export function validateComboboxLiveV17TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "combobox-live-v17-host-technical-gates-v2" ||
    ...
    COMBOBOX_LIVE_V17_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Combobox live v17 technical gates artifact");
}

export function validateComboboxLiveV17Receipt(
  directory: string,
  phase: ComboboxLiveV17RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateComboboxLiveV17Request(directory, request);
  const receipt = json<ComboboxLiveV17Receipt>(
  ...
  if (
    receipt.artifactVersion !== "combobox-live-v17-broker-receipt-v2" ||
    receipt.brokerVersion !== COMBOBOX_LIVE_V17_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Combobox live v17 accepted receipt: ${request.requestId}`,
    );
}

export function readComboboxLiveV17AcceptedRawPayload(
  directory: string,
  phase: ComboboxLiveV17RemotePhase,
  captureIndex?: number,
): unknown {
  validateComboboxLiveV17Receipt(directory, phase, captureIndex);
...
export function createComboboxLiveV17FakeDynamicResponse(
  request: ComboboxLiveV17Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? COMBOBOX_LIVE_V17_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? COMBOBOX_LIVE_V17_TARGET,
    ...overrides.extra,
  };
}

export function listComboboxLiveV17PersistedRequests(
  directory: string,
): ComboboxLiveV17Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<ComboboxLiveV17Request>(file)] : [];
    });
}
