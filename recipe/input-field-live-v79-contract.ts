...
} from "./input-field-live-v3-verifier-v79.js";
import {
  buildInputLiveV79RawPropertyRuntime,
  normalizeInputLiveV79Scene,
  type InputLiveV79RawNode,
} from "./input-field-live-v79-verifier.js";
...
export {
  ...
  type InputLiveV79RestorePayload,
} from "./input-field-live-v79-restore.js";

export const INPUT_LIVE_V79_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V79_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V79_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V79_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V79_VARIANT_COUNT = 256;
export const INPUT_LIVE_V79_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V79_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV79SourceId = (typeof INPUT_LIVE_V79_SOURCE_IDS)[number];

export interface InputLiveV79SourceIdentity {
  source: InputLiveV79SourceId;
...
export interface InputLiveV79WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV79ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV79SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV79RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV79RootProof {
  source: InputLiveV79SourceId;
  adapterIdentity: string;
...
export interface InputLiveV79ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV79SourceId;
    adapterIdentity: string;
...
export interface InputLiveV79CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV79SourceId;
  adapterIdentity: string;
...
export interface InputLiveV79CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV79SourceId;
  strata: InputLiveV79CaptureCell["strata"];
...
export interface InputLiveV79CleanupPayload {
...
export function validateInputLiveV79WriterPayload(
  value: unknown,
): InputLiveV79WriterOwnership {
...
const identityPlan = (source: InputLiveV79SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV79ExtractProgram(
  writer: InputLiveV79WriterOwnership,
  sources: readonly InputLiveV79SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V79_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V79_NAMESPACE)}
${buildInputLiveV79RawPropertyRuntime()}
...
export function validateInputLiveV79ExtractPayload(
  value: unknown,
  writer: InputLiveV79WriterOwnership,
): InputLiveV79ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V79_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV79ExtractPayload;
}

export function proveInputLiveV79Roots<Instance>(
  extract: InputLiveV79ExtractPayload,
  sources: readonly (InputLiveV79SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV79Scene(
      root.scene,
...
export function assertInputLiveV79RootProofs(
  proofs: readonly InputLiveV79RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV79ProbeProgram(
  writer: InputLiveV79WriterOwnership,
  sources: readonly InputLiveV79SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V79_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV79ProbePayload(
  value: unknown,
  writer: InputLiveV79WriterOwnership,
): InputLiveV79ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV79ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V79_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V79_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV79CaptureManifest(
  cells: readonly InputLiveV79CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V79_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V79_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V79_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V79_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V79_SOURCE_IDS.some(
...
export const inputLiveV79CaptureManifestSha256 = (
  cells: readonly InputLiveV79CaptureCell[],
): string => {
  validateInputLiveV79CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV79CaptureProgram(
  writer: InputLiveV79WriterOwnership,
  cell: InputLiveV79CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V79_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V79_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V79_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV79CapturePayload(
  value: unknown,
  cell: InputLiveV79CaptureCell,
  rawResponseBytes: number,
): InputLiveV79CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV79CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V79_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V79_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV79CaptureResponses(
  manifest: readonly InputLiveV79CaptureCell[],
  responses: readonly InputLiveV79CapturePayload[],
): void {
  validateInputLiveV79CaptureManifest(manifest);
  if (
...
export interface InputLiveV79ObjectiveReport {
  artifactVersion: "input-live-v79-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV79Objective(
  manifest: readonly InputLiveV79CaptureCell[],
  responses: readonly InputLiveV79CapturePayload[],
  readReference: (cell: InputLiveV79CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV79CaptureCell) => Buffer,
): InputLiveV79ObjectiveReport {
  assertInputLiveV79CaptureResponses(manifest, responses);
...
export function buildInputLiveV79CleanupProgram(
  writer: InputLiveV79WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V79_NAMESPACE,
...
export function validateInputLiveV79CleanupPayload(
  value: unknown,
  writer: InputLiveV79WriterOwnership,
): InputLiveV79CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV79CleanupPayload;
...
export const INPUT_LIVE_V79_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV79WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
