...
import { TABLE_LIVE_V30_AUTHORIZATION_PATH } from "./build-table-live-proof-v30.js";
import { verifyTableLiveV30Authorization } from "./table-live-v30-authorization.js";
import { runTableLiveV30Preflight } from "./table-live-v30-preflight.js";
import { TableLiveV30Orchestrator } from "./run-table-live-v30.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v30 requires exact authorization artifact ${TABLE_LIVE_V30_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v30-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV30Authorization(root);
    runTableLiveV30Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV30Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV30Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
