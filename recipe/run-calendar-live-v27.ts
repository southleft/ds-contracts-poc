...
import {
  acceptCalendarLiveV27Response,
  createCalendarLiveV27Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV27Request,
  persistCalendarLiveV27RawResponse,
  persistCalendarLiveV27TechnicalGates,
  readCalendarLiveV27AcceptedRawPayload,
  type CalendarLiveV27Request,
  type CalendarLiveV27TransactionAuthorization,
} from "./calendar-live-v27-broker.js";
import {
  CALENDAR_LIVE_V27_CAPTURE_COUNT,
  assertCalendarLiveV27CaptureResponses,
  ...
  evaluateCalendarLiveV27Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV27Roots,
  validateCalendarLiveV27CaptureManifest,
  validateCalendarLiveV27CapturePayload,
  validateCalendarLiveV27CleanupPayload,
  validateCalendarLiveV27ExtractPayload,
  validateCalendarLiveV27ProbePayload,
  validateCalendarLiveV27WriterPayload,
  type CalendarLiveV27CaptureCell,
  type CalendarLiveV27CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V27_PLAN_PATH,
  type CalendarLiveV27ProofPlan,
} from "./build-calendar-live-proof-v27.js";
import {
...
export const CALENDAR_LIVE_V27_RUNNER_VERSION = "calendar-live-v27-runner-v1";

interface CalendarLiveV27JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV27OrchestratorState {
  artifactVersion: "calendar-live-v27-orchestrator-state-v1";
  ...
  journal: CalendarLiveV27JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV27ProofPlan;
  planSha256: string;
  captures: CalendarLiveV27CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV27SourceIdentity & {
...
export interface CalendarLiveV27NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV27OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v27 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V27_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V27_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV27ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV27CaptureCell[];
  validateCalendarLiveV27CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v27 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v27 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV27WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV27RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV27Request =>
  readJson<CalendarLiveV27Request>(file);

export class CalendarLiveV27Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV27Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV27OrchestratorState = {
      artifactVersion: "calendar-live-v27-orchestrator-state-v1",
    ...
    return new CalendarLiveV27Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV27OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV27Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV27NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV27NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v27 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV27NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v27 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV27RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV27Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV27CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV27AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV27WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV27Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV27ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV27Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV27Response(this.#directory, "writer", {
        validate: validateCalendarLiveV27WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV27Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV27RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV27Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV27ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV27Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV27RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v27-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV27Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV27ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV27Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV27ProbePayload(payload, ownership),
      });
      persistCalendarLiveV27TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v27 capture cell missing");
      const accepted = acceptCalendarLiveV27Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV27CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V27_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV27CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV27Objective(
    ...
    throw new Error(`Calendar live v27 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV27Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV27CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV27Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV27RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV27Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV27CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV27CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV27CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV27Authorization = (
  privateKey: KeyObject,
): CalendarLiveV27TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV27SimulationKey = (): KeyObject =>
