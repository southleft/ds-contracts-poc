...
import {
  ...
  INPUT_LIVE_V61_INDEX_PATH,
} from "./build-input-field-live-proof-v61.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV61HistoryExpectation = "pending" | "authorized";

export interface InputLiveV61AuthorizationArtifact {
  artifactVersion: "input-live-v61-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V61_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV61AntecedentIndex {
...
export interface InputLiveV61HistoryState {
  index?: InputLiveV61AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV61AuthorizationArtifact;
...
export interface InputLiveV61AuthorizationProof extends InputLiveV61TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V61_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V61_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V61_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V61_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v61-authorization") ||
  artifactPath.includes("input-field-live-v61-preflight") ||
  artifactPath.includes("input-field-live-v61-authorized") ||
  artifactPath.includes("input-field-live-pivot-v61-status") ||
...
export function validateInputLiveV61AntecedentIndex(
  index: InputLiveV61AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v61-antecedent-index-v1" ||
...
export function validateInputLiveV61UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV61History\(\s*["']/.test(source) ||
    /readInputLiveV61HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV61AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV61AuthorizationArtifact {
  ...
  ) as InputLiveV61AntecedentIndex;
  const indexFailures = validateInputLiveV61AntecedentIndex(index);
...
export function validateInputLiveV61AuthorizationArtifact(
  artifact: InputLiveV61AuthorizationArtifact | undefined,
  index: InputLiveV61AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v61-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v61" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V61_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V61_TARGET) ||
    !equalJson(
...
export function validateInputLiveV61History(
  state: InputLiveV61HistoryState,
  expected: InputLiveV61HistoryExpectation,
): string[] {
  const failures = validateInputLiveV61AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV61AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV61HistoryState(
  root = process.cwd(),
): InputLiveV61HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V61_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV61AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V61_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V61_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V61_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV61History(
  expected: InputLiveV61HistoryExpectation,
  root = process.cwd(),
): InputLiveV61HistoryState {
  const state = readInputLiveV61HistoryState(root);
  const failures = validateInputLiveV61History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV61Authorization(
  root = process.cwd(),
): InputLiveV61AuthorizationProof {
  const state = verifyInputLiveV61History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V61_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v61/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v61.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V61_TARGET,
    expectedDynamicTool: INPUT_LIVE_V61_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V61_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV61HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV61History(expected);
