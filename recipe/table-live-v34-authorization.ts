...
export type TableLiveV34HistoryExpectation = "pending" | "authorized";

export interface TableLiveV34AuthorizationArtifact {
  artifactVersion: "table-live-v34-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V34_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV34AntecedentIndex {
...
export interface TableLiveV34HistoryState {
  index?: TableLiveV34AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV34AuthorizationArtifact;
...
export interface TableLiveV34AuthorizationProof
  extends TableLiveV34TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V34_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V34_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V34_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V34_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v34-authorization") ||
  artifactPath.includes("table-live-v34-preflight") ||
  artifactPath.includes("table-live-v34-authorized") ||
  artifactPath.includes("create-table-live-v34-security-attestation") ||
  artifactPath.includes("table-live-pivot-v34-status") ||
...
export function validateTableLiveV34AntecedentIndex(
  index: TableLiveV34AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v34-antecedent-index-v1" ||
...
export function validateTableLiveV34UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV34History\(\s*["']/.test(source) ||
    /readTableLiveV34HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV34AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV34AuthorizationArtifact {
  ...
  ) as TableLiveV34AntecedentIndex;
  const indexFailures = validateTableLiveV34AntecedentIndex(index);
...
export function validateTableLiveV34AuthorizationArtifact(
  artifact: TableLiveV34AuthorizationArtifact | undefined,
  index: TableLiveV34AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v34-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v34" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V34_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V34_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V34_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V34_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V34_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V34_SOURCE_ROOTS ||
...
export function validateTableLiveV34History(
  state: TableLiveV34HistoryState,
  expected: TableLiveV34HistoryExpectation,
): string[] {
  const failures = validateTableLiveV34AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV34AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV34HistoryState(
  root = process.cwd(),
): TableLiveV34HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V34_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV34AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V34_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V34_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V34_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV34History(
  expected: TableLiveV34HistoryExpectation,
  root = process.cwd(),
): TableLiveV34HistoryState {
  const state = readTableLiveV34HistoryState(root);
  const failures = validateTableLiveV34History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v34 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV34Authorization(
  root = process.cwd(),
): TableLiveV34AuthorizationProof {
  const state = verifyTableLiveV34History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V34_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v34/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v34.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V34_TARGET,
    expectedDynamicTool: TABLE_LIVE_V34_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V34_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV34HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV34History(expected);
  process.stdout.write(
    `Table live v34 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
