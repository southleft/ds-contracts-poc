...
import {
  ...
  INPUT_LIVE_V63_INDEX_PATH,
} from "./build-input-field-live-proof-v63.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV63HistoryExpectation = "pending" | "authorized";

export interface InputLiveV63AuthorizationArtifact {
  artifactVersion: "input-live-v63-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V63_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV63AntecedentIndex {
...
export interface InputLiveV63HistoryState {
  index?: InputLiveV63AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV63AuthorizationArtifact;
...
export interface InputLiveV63AuthorizationProof extends InputLiveV63TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V63_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V63_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V63_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V63_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v63-authorization") ||
  artifactPath.includes("input-field-live-v63-preflight") ||
  artifactPath.includes("input-field-live-v63-authorized") ||
  artifactPath.includes("input-field-live-pivot-v63-status") ||
...
export function validateInputLiveV63AntecedentIndex(
  index: InputLiveV63AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v63-antecedent-index-v1" ||
...
export function validateInputLiveV63UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV63History\(\s*["']/.test(source) ||
    /readInputLiveV63HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV63AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV63AuthorizationArtifact {
  ...
  ) as InputLiveV63AntecedentIndex;
  const indexFailures = validateInputLiveV63AntecedentIndex(index);
...
export function validateInputLiveV63AuthorizationArtifact(
  artifact: InputLiveV63AuthorizationArtifact | undefined,
  index: InputLiveV63AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v63-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v63" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V63_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V63_TARGET) ||
    !equalJson(
...
export function validateInputLiveV63History(
  state: InputLiveV63HistoryState,
  expected: InputLiveV63HistoryExpectation,
): string[] {
  const failures = validateInputLiveV63AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV63AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV63HistoryState(
  root = process.cwd(),
): InputLiveV63HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V63_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV63AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V63_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V63_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V63_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV63History(
  expected: InputLiveV63HistoryExpectation,
  root = process.cwd(),
): InputLiveV63HistoryState {
  const state = readInputLiveV63HistoryState(root);
  const failures = validateInputLiveV63History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV63Authorization(
  root = process.cwd(),
): InputLiveV63AuthorizationProof {
  const state = verifyInputLiveV63History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V63_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v63/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v63.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V63_TARGET,
    expectedDynamicTool: INPUT_LIVE_V63_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V63_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV63HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV63History(expected);
