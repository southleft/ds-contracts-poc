...
import {
  acceptCalendarLiveV18Response,
  createCalendarLiveV18Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV18Request,
  persistCalendarLiveV18RawResponse,
  persistCalendarLiveV18TechnicalGates,
  readCalendarLiveV18AcceptedRawPayload,
  type CalendarLiveV18Request,
  type CalendarLiveV18TransactionAuthorization,
} from "./calendar-live-v18-broker.js";
import {
  CALENDAR_LIVE_V18_CAPTURE_COUNT,
  assertCalendarLiveV18CaptureResponses,
  ...
  evaluateCalendarLiveV18Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV18Roots,
  validateCalendarLiveV18CaptureManifest,
  validateCalendarLiveV18CapturePayload,
  validateCalendarLiveV18CleanupPayload,
  validateCalendarLiveV18ExtractPayload,
  validateCalendarLiveV18ProbePayload,
  validateCalendarLiveV18WriterPayload,
  type CalendarLiveV18CaptureCell,
  type CalendarLiveV18CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V18_PLAN_PATH,
  type CalendarLiveV18ProofPlan,
} from "./build-calendar-live-proof-v18.js";
import {
...
export const CALENDAR_LIVE_V18_RUNNER_VERSION = "calendar-live-v18-runner-v1";

interface CalendarLiveV18JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV18OrchestratorState {
  artifactVersion: "calendar-live-v18-orchestrator-state-v1";
  ...
  journal: CalendarLiveV18JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV18ProofPlan;
  planSha256: string;
  captures: CalendarLiveV18CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV18SourceIdentity & {
...
export interface CalendarLiveV18NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV18OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v18 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V18_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V18_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV18ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV18CaptureCell[];
  validateCalendarLiveV18CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v18 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v18 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV18WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV18RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV18Request =>
  readJson<CalendarLiveV18Request>(file);

export class CalendarLiveV18Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV18Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV18OrchestratorState = {
      artifactVersion: "calendar-live-v18-orchestrator-state-v1",
    ...
    return new CalendarLiveV18Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV18OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV18Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV18NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV18NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v18 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV18NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v18 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV18RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV18Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV18CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV18AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV18WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV18Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV18ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV18Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV18Response(this.#directory, "writer", {
        validate: validateCalendarLiveV18WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV18Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV18RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV18Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV18ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV18Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV18RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v18-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV18Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV18ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV18Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV18ProbePayload(payload, ownership),
      });
      persistCalendarLiveV18TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v18 capture cell missing");
      const accepted = acceptCalendarLiveV18Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV18CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V18_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV18CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV18Objective(
    ...
    throw new Error(`Calendar live v18 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV18Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV18CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV18Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV18RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV18Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV18CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV18CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV18CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV18Authorization = (
  privateKey: KeyObject,
): CalendarLiveV18TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV18SimulationKey = (): KeyObject =>
