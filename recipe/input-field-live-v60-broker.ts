...
import {
  assertInputLiveV60RootProofs,
  INPUT_LIVE_V60_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V60_RESPONSE_CONTRACTS,
  type InputLiveV60RootProof,
} from "./input-field-live-v60-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V60_BROKER_VERSION = "input-live-v60-broker-v2";
export const INPUT_LIVE_V60_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V60_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V60_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V60_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V60_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V60_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV60RemotePhase =
  (typeof INPUT_LIVE_V60_REMOTE_PHASES)[number];
export type InputLiveV60AuthorizationMode = "live" | "simulated";

export interface InputLiveV60CommitPins {
...
export interface InputLiveV60TransactionAuthorization extends InputLiveV60CommitPins {
  mode: InputLiveV60AuthorizationMode;
...
export interface InputLiveV60Manifest {
  artifactVersion: "input-live-v60-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V60_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V60_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V60_TARGET;
  authorization: InputLiveV60TransactionAuthorization;
...
export interface InputLiveV60ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV60RequestBody {
  artifactVersion: "input-live-v60-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V60_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV60RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV60CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V60_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV60ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV60Request extends InputLiveV60RequestBody {
...
export interface InputLiveV60Receipt {
  artifactVersion: "input-live-v60-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V60_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV60RemotePhase;
...
export interface InputLiveV60AcceptedResponse<T = unknown> {
  request: InputLiveV60Request;
  receipt: InputLiveV60Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV60IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV60ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V60_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V60_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V60_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V60_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V60_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V60_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V60_TARGET.editorType;
};
...
export const inputLiveV60RequestSequence = (
  phase: InputLiveV60RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV60RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV60RequestPath = (
  directory: string,
  phase: InputLiveV60RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV60ResponsePath = (
  directory: string,
  phase: InputLiveV60RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV60ReceiptPath = (
  directory: string,
  phase: InputLiveV60RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV60Manifest => {
  const manifest = json<InputLiveV60Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v60-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V60_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V60_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV60RemotePhase,
  captureIndex?: number,
): InputLiveV60Request | undefined => {
  const file = inputLiveV60RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV60Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV60RemotePhase,
  captureIndex?: number,
): InputLiveV60Receipt | undefined => {
  const file = inputLiveV60ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV60Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV60Request | undefined;
  receipt: InputLiveV60Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV60RemotePhase,
): InputLiveV60ResponseContract => ({
  ...INPUT_LIVE_V60_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V60_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V60_DYNAMIC_TOOL,
      target: INPUT_LIVE_V60_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v60-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V60_BROKER_VERSION,
  target: INPUT_LIVE_V60_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V60_PROGRAM_START*/${program}/*INPUT_LIVE_V60_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V60_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V60_PROGRAM_END*/";
...
export function createInputLiveV60Transaction(
  directory: string,
  ...
  const manifest: InputLiveV60Manifest = {
    artifactVersion: "input-live-v60-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V60_BROKER_VERSION,
...
export function validateInputLiveV60Request(
  directory: string,
  request: InputLiveV60Request,
): void {
  ...
  const expectedSequence = inputLiveV60RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v60-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V60_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V60_TARGET.fileKey ||
...
export function issueInputLiveV60Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV60RemotePhase,
  program: string,
  options: InputLiveV60IssueOptions = {},
): InputLiveV60Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV60RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV60Request>(targetPath);
    validateInputLiveV60Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV60ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV60TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV60RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V60_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V60_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V60_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV60RequestBody = {
    artifactVersion: "input-live-v60-broker-request-v2",
    brokerVersion: INPUT_LIVE_V60_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV60Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV60Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV60RawResponse(
  directory: string,
  phase: InputLiveV60RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV60RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV60Request>(requestFile);
  if (
  ...
  const target = inputLiveV60ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV60Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v60-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V60_BROKER_VERSION,
...
export function acceptInputLiveV60Response<T = unknown>(
  directory: string,
  phase: InputLiveV60RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV60AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV60ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV60Request>(
    inputLiveV60RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV60Request(directory, request);
  if (phase === "capture") validateInputLiveV60TechnicalGates(directory);
  const rawFile = inputLiveV60ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v60-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V60_BROKER_VERSION ||
...
export function persistInputLiveV60TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV60RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV60RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V60_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v60-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V60_BROKER_VERSION,
...
export function validateInputLiveV60TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v60-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V60_TECHNICAL_GATES.some(
...
export function validateInputLiveV60Receipt(
  directory: string,
  phase: InputLiveV60RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV60Request(directory, request);
  const receipt = json<InputLiveV60Receipt>(
    inputLiveV60ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV60ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v60-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V60_BROKER_VERSION ||
...
export function readInputLiveV60AcceptedRawPayload(
  directory: string,
  phase: InputLiveV60RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV60Receipt(directory, phase, captureIndex);
...
export function createInputLiveV60FakeDynamicResponse(
  request: InputLiveV60Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V60_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V60_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV60PersistedRequests(
  directory: string,
): InputLiveV60Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV60Request>(file)] : [];
    });
}

export const inputLiveV60Sha256 = sha256;
