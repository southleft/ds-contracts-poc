...
import { CALENDAR_LIVE_V13_INDEX_PATH } from "./build-calendar-live-proof-v13.js";
import {
...
import {
  buildCalendarLiveV13AuthorizationArtifact,
  validateCalendarLiveV13AntecedentIndex,
  validateCalendarLiveV13AuthorizationArtifact,
  validateCalendarLiveV13History,
  validateCalendarLiveV13UnitTestSource,
  type CalendarLiveV13AntecedentIndex,
  type CalendarLiveV13AuthorizationArtifact,
  type CalendarLiveV13AuthorizationProof,
  type CalendarLiveV13HistoryState,
} from "./calendar-live-v13-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV13ControlFlowSource,
  validateCalendarLiveV13SecurityAttestation,
  type CalendarLiveV13SecurityAttestation,
} from "./calendar-live-v13-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V13_INDEX_PATH);
const index = (): CalendarLiveV13AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV13AntecedentIndex;

const artifact = (): CalendarLiveV13AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV13AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV13HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV13HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV13AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v13/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV13AuthorizationArtifact,
): CalendarLiveV13SecurityAttestation => {
  const body: Omit<CalendarLiveV13SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V13_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV13History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV13History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV13History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV13History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV13History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV13UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v13-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV13UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV13AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV13AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV13AuthorizationArtifact(
        planted as CalendarLiveV13AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV13SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV13SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV13SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v13-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV13ControlFlowSource(source), []);
});
