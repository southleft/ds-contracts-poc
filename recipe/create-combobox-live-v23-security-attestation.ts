...
import { verifyComboboxLiveV23Authorization } from "./combobox-live-v23-authorization.js";
import { COMBOBOX_LIVE_V23_TARGET } from "./combobox-live-v23-broker.js";
import {
  COMBOBOX_LIVE_V23_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV23Authorization();
...
const attestation: ComboboxLiveV23SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV23SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v23 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V23_SECURITY_ATTESTATION_DEFAULT_PATH,
);
