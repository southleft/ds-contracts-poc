/**
 * Calendar live v9 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V9_FIXED_VERIFIER_VERSION = "calendar-live-v9-fixed-point-v1";
export const CALENDAR_LIVE_V9_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV9FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV9FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV9FixedSourceProbe {
...
export interface CalendarLiveV9FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV9FixedFixedPoint;
}

export interface CalendarLiveV9FixedHardGateInput {
  thresholds: CalendarLiveV9FixedThresholds;
  sourceProbes: CalendarLiveV9FixedSourceProbe[];
  cells: CalendarLiveV9FixedCellValidation[];
  sceneProofs: CalendarLiveV9FixedSceneProof[];
...
export interface CalendarLiveV9FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V9_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV9FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV9FixedHardGates(
  input: CalendarLiveV9FixedHardGateInput,
): CalendarLiveV9FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV9FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V9_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV9FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV9FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
