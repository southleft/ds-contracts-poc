...
import {
  ...
  verifyInputLiveV12Authorization,
  type InputLiveV12AntecedentIndex,
  type InputLiveV12AuthorizationProof,
} from "./input-field-live-v12-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V12_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v12/operator-security-attestation-template.json";
export const INPUT_LIVE_V12_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v12-security-attestation.json";

export interface InputLiveV12SecurityAttestation {
...
export interface InputLiveV12PreflightReport {
  artifactVersion: "input-live-v12-preflight-v1";
...
export const inputLiveV12AttestationSha256 = (
  value: Omit<InputLiveV12SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV12SecurityAttestation(
  value: unknown,
  proof: InputLiveV12AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v12-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V12_TARGET) ||
...
export function validateInputLiveV12ControlFlowSource(source: string): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV12Authorization");
  const preflight = source.indexOf("runInputLiveV12Preflight");
  const initialize = source.indexOf("InputLiveV12Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV12Preflight(
  root: string,
  proof: InputLiveV12AuthorizationProof,
  attestationPath = INPUT_LIVE_V12_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV12PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V12_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V12_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V12_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV12AntecedentIndex;
  failures.push(...validateInputLiveV12AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V12_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V12_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v12-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV12ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(`Input live v12 preflight refused:\n${failures.join("\n")}`);
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as InputLiveV12SecurityAttestation;
  return {
    artifactVersion: "input-live-v12-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V12_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV12Preflight(
    process.cwd(),
    verifyInputLiveV12Authorization(),
    attestationPath,
