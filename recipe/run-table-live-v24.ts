...
import {
  acceptTableLiveV24Response,
  createTableLiveV24Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV24Request,
  persistTableLiveV24RawResponse,
  persistTableLiveV24TechnicalGates,
  readTableLiveV24AcceptedRawPayload,
  type TableLiveV24Request,
  type TableLiveV24TransactionAuthorization,
} from "./table-live-v24-broker.js";
import {
  TABLE_LIVE_V24_CAPTURE_COUNT,
  assertTableLiveV24CaptureResponses,
  ...
  evaluateTableLiveV24Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV24Roots,
  validateTableLiveV24CaptureManifest,
  validateTableLiveV24CapturePayload,
  validateTableLiveV24CleanupPayload,
  validateTableLiveV24ExtractPayload,
  validateTableLiveV24ProbePayload,
  validateTableLiveV24WriterPayload,
  type TableLiveV24CaptureCell,
  type TableLiveV24CapturePayload,
...
import {
  ...
  TABLE_LIVE_V24_PLAN_PATH,
  type TableLiveV24ProofPlan,
} from "./build-table-live-proof-v24.js";
import {
...
export const TABLE_LIVE_V24_RUNNER_VERSION = "table-live-v24-runner-v1";

interface TableLiveV24JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV24OrchestratorState {
  artifactVersion: "table-live-v24-orchestrator-state-v1";
  ...
  journal: TableLiveV24JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV24ProofPlan;
  planSha256: string;
  captures: TableLiveV24CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV24SourceIdentity & {
...
export interface TableLiveV24NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV24OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v24 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V24_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V24_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV24ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV24CaptureCell[];
  validateTableLiveV24CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v24 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v24 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV24WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV24RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV24Request =>
  readJson<TableLiveV24Request>(file);

export class TableLiveV24Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV24Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV24OrchestratorState = {
      artifactVersion: "table-live-v24-orchestrator-state-v1",
    ...
    return new TableLiveV24Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV24OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV24Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV24NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV24NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v24 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV24NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v24 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV24RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV24Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV24CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV24AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV24WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV24Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV24ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV24Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV24Response(this.#directory, "writer", {
        validate: validateTableLiveV24WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV24Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV24RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV24Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV24ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV24Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV24RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v24-host-root-proofs-v1",
      ...
      const next = issueTableLiveV24Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV24ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV24Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV24ProbePayload(payload, ownership),
      });
      persistTableLiveV24TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v24 capture cell missing");
      const accepted = acceptTableLiveV24Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV24CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V24_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV24CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV24Objective(
    ...
    throw new Error(`Table live v24 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV24Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV24CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV24Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV24RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV24Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV24CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV24CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV24CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV24Authorization = (
  privateKey: KeyObject,
): TableLiveV24TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV24SimulationKey = (): KeyObject =>
