...
export interface InputLiveV28LifecycleSimulationReport {
  artifactVersion: "input-live-v28-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV28AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV28Lifecycle(): Promise<InputLiveV28LifecycleSimulationReport> {
  await buildInputLiveV28Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V28_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV28AntecedentIndex;
  const indexSha256 = inputLiveV28Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v28-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V28_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V28_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV28History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV28History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV28AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V28_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V28_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV28History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV28History(authorized, "pending");
    ...
    await buildInputLiveV28Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V28_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV28AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV28Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV28Lifecycle(), null, 2)}\n`,
  );
