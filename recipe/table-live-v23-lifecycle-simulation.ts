...
import { tableLiveV2Sha256 } from "./table-live-v23-broker.js";

export interface TableLiveV23LifecycleSimulationReport {
  artifactVersion: "table-live-v23-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV23AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV23Lifecycle(): Promise<TableLiveV23LifecycleSimulationReport> {
  await buildTableLiveV23Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V23_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV23AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v23-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V23_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V23_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV23History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV23History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV23AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V23_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V23_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV23History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV23History(authorized, "pending");
    ...
    await buildTableLiveV23Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V23_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV23AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV23Lifecycle(), null, 2)}\n`,
  );
