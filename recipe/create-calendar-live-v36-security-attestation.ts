...
import { verifyCalendarLiveV36Authorization } from "./calendar-live-v36-authorization.js";
import { CALENDAR_LIVE_V36_TARGET } from "./calendar-live-v36-broker.js";
import {
  CALENDAR_LIVE_V36_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV36Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v36-session";
...
const attestation: CalendarLiveV36SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV36SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v36 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V36_SECURITY_ATTESTATION_DEFAULT_PATH,
);
