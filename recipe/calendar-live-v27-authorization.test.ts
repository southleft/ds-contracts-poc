...
import { CALENDAR_LIVE_V27_INDEX_PATH } from "./build-calendar-live-proof-v27.js";
import {
...
import {
  buildCalendarLiveV27AuthorizationArtifact,
  validateCalendarLiveV27AntecedentIndex,
  validateCalendarLiveV27AuthorizationArtifact,
  validateCalendarLiveV27History,
  validateCalendarLiveV27UnitTestSource,
  type CalendarLiveV27AntecedentIndex,
  type CalendarLiveV27AuthorizationArtifact,
  type CalendarLiveV27AuthorizationProof,
  type CalendarLiveV27HistoryState,
} from "./calendar-live-v27-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV27ControlFlowSource,
  validateCalendarLiveV27SecurityAttestation,
  type CalendarLiveV27SecurityAttestation,
} from "./calendar-live-v27-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V27_INDEX_PATH);
const index = (): CalendarLiveV27AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV27AntecedentIndex;

const artifact = (): CalendarLiveV27AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV27AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV27HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV27HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV27AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v27/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV27AuthorizationArtifact,
): CalendarLiveV27SecurityAttestation => {
  const body: Omit<CalendarLiveV27SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V27_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV27History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV27History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV27History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV27History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV27History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV27UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v27-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV27UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV27AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV27AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV27AuthorizationArtifact(
        planted as CalendarLiveV27AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV27SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV27SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV27SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v27-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV27ControlFlowSource(source), []);
});
