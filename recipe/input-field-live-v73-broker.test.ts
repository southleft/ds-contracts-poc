...
import {
  ...
  createInputLiveV73FakeDynamicResponse,
  ...
  listInputLiveV73PersistedRequests,
  persistInputLiveV73RawResponse,
  persistInputLiveV73TechnicalGates,
  validateInputLiveV73Receipt,
  validateInputLiveV73Request,
  type InputLiveV73RemotePhase,
...
import {
  ...
  type InputLiveV73ExtractPayload,
  type InputLiveV73ProbePayload,
  type InputLiveV73RestorePayload,
  type InputLiveV73RootProof,
  type InputLiveV73WriterOwnership,
} from "./input-field-live-v73-contract.js";
import {
  InputLiveV73Orchestrator,
  simulatedInputLiveV73Authorization,
} from "./run-input-field-live-v73.js";
...
const captures: InputLiveV73CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV73Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v73-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV73Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV73CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV73FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV73FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V73_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V73_TARGET);
  persistInputLiveV73RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV73Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV73Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV73WriterOwnership,
): InputLiveV73RestorePayload => ({
...
const rootProofs = (): InputLiveV73RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV73CaptureCell,
): InputLiveV73CapturePayload => {
  const bytes = Buffer.from(`input-live-v73-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV73Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV73Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV73RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV73RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV73ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV73ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV73CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV73CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV73CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV73Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV73CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV73PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV73ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV73Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV73Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV73Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV73Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V73_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV73RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V73_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV73Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV73Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV73ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV73Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV73WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV73RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV73Request;
    validateInputLiveV73Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV73Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV73CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v73-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV73Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV73Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV73Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V73_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV73RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V73_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV73Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV73Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV73RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V73_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV73Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV73Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV73Request;
