...
import {
  validateCalendarLiveV40AntecedentIndex,
  verifyCalendarLiveV40Authorization,
  type CalendarLiveV40AntecedentIndex,
  type CalendarLiveV40AuthorizationProof,
} from "./calendar-live-v40-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V40_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v40/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V40_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v40-security-attestation.json";

export interface CalendarLiveV40SecurityAttestation {
  artifactVersion: "calendar-live-v40-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV40PreflightReport {
  artifactVersion: "calendar-live-v40-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV40SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV40SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV40AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v40-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V40_TARGET) ||
...
export function validateCalendarLiveV40ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV40Authorization");
  const preflight = source.indexOf("runCalendarLiveV40Preflight");
  const initialize = source.indexOf("CalendarLiveV40Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV40Preflight(
  root: string,
  proof: CalendarLiveV40AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V40_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV40PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V40_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V40_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV40AntecedentIndex;
  failures.push(...validateCalendarLiveV40AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V40_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V40_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV40SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v40-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV40ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v40 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV40SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v40-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V40_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV40Preflight(
    process.cwd(),
    verifyCalendarLiveV40Authorization(),
    attestationPath,
