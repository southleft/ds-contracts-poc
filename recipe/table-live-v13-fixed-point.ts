/**
 * Table live v13 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V13_FIXED_VERIFIER_VERSION = "table-live-v13-fixed-point-v1";
export const TABLE_LIVE_V13_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV13FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV13FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV13FixedSourceProbe {
...
export interface TableLiveV13FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV13FixedFixedPoint;
}

export interface TableLiveV13FixedHardGateInput {
  thresholds: TableLiveV13FixedThresholds;
  sourceProbes: TableLiveV13FixedSourceProbe[];
  cells: TableLiveV13FixedCellValidation[];
  sceneProofs: TableLiveV13FixedSceneProof[];
...
export interface TableLiveV13FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V13_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV13FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV13FixedHardGates(
  input: TableLiveV13FixedHardGateInput,
): TableLiveV13FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV13FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  if (input.visualRows.length !== 0)
    failures.push("table live v13 must not invent a visual comparison");
  ...
  return {
    verifierVersion: TABLE_LIVE_V13_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV13FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV13FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
