...
import { verifyComboboxLiveV12Authorization } from "./combobox-live-v12-authorization.js";
import { COMBOBOX_LIVE_V12_TARGET } from "./combobox-live-v12-broker.js";
import {
  COMBOBOX_LIVE_V12_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV12Authorization();
...
const attestation: ComboboxLiveV12SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV12SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v12 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V12_SECURITY_ATTESTATION_DEFAULT_PATH,
);
