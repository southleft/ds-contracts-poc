...
import {
  INPUT_LIVE_V41_DYNAMIC_TOOL,
  INPUT_LIVE_V41_TARGET,
  inputLiveV41RequestSequence,
...
} from "./input-field-live-v41-restore.js";

export const INPUT_LIVE_V41_EVIDENCE_ROOT =
  "recipe/evidence/input-field-live-pivot-v41";
export const INPUT_LIVE_V41_PROTOCOL_PATH = `${INPUT_LIVE_V41_EVIDENCE_ROOT}/protocol.json`;
export const INPUT_LIVE_V41_PLAN_PATH = `${INPUT_LIVE_V41_EVIDENCE_ROOT}/proof-plan.json`;
export const INPUT_LIVE_V41_CAPTURE_MANIFEST_PATH = `${INPUT_LIVE_V41_EVIDENCE_ROOT}/capture-manifest.json`;
export const INPUT_LIVE_V41_REQUEST_MANIFEST_PATH = `${INPUT_LIVE_V41_EVIDENCE_ROOT}/request-manifest.json`;
export const INPUT_LIVE_V41_INDEX_PATH = `${INPUT_LIVE_V41_EVIDENCE_ROOT}/antecedent-index.json`;
export const INPUT_LIVE_V41_AUTHORIZATION_TEMPLATE_PATH = `${INPUT_LIVE_V41_EVIDENCE_ROOT}/authorization-template.json`;
export const INPUT_LIVE_V41_AUTHORIZATION_PATH = `${INPUT_LIVE_V41_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface InputLiveV41ProofPlan {
  artifactVersion: "input-live-v41-proof-plan-v1";
...
interface GeneratedSource {
  ...
  recipeHash: string;
  envelopeHash: string;
  expectedScenePlan: InputLiveV41SourceIdentity["expectedScenePlan"];
  outputPath: string;
  compressed: Buffer;
  metadata: InputLiveV41ProofPlan["sources"][number]["expectedScenePlan"];
}
...
const sourcePlans = (): GeneratedSource[] => {
  ...
  return prior.sources.map((source: Record<string, any>) => {
    ...
    return {
      ...
      outputPath: `${INPUT_LIVE_V41_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
      compressed,
      metadata: {
        path: `${INPUT_LIVE_V41_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
...
const captureCells = (): InputLiveV41CaptureCell[] => {
  const cells = json<Record<string, any>>(V16_CAPTURE_MANIFEST)
    .cells as InputLiveV41CaptureCell[];
  inputLiveV41CaptureManifestSha256(cells);
  return cells;
};

const protocol = () => ({
  artifactVersion: "input-live-v41-external-operator-protocol-v1",
  protocolId: "input-live-v41",
  status:
  ...
  hostNormalization: {
    ...
    carriedSceneReadback: "recipe/scene-readback-v41.ts",
    carriedSceneReadbackRuntime: "recipe/scene-readback-runtime-v41.ts",
    carriedV3Verifier: "recipe/input-field-live-v3-verifier-v41.ts",
...
const requestManifest = (
  captures: readonly InputLiveV41CaptureCell[],
  programs: {
    writerSha256: string;
  ...
  const requests = [
    {
      requestId: "writer",
      sequence: inputLiveV41RequestSequence("writer"),
      phase: "writer",
      predecessorRequestId: null,
      programSha256: programs.writerSha256,
      response: INPUT_LIVE_V41_RESPONSE_CONTRACTS.writer,
    },
    {
      requestId: "cleanup",
      sequence: inputLiveV41RequestSequence("cleanup"),
      phase: "cleanup",
      predecessorRequestId: "writer",
      programBuilderSha256: programs.cleanupBuilderSha256,
      response: INPUT_LIVE_V41_RESPONSE_CONTRACTS.cleanup,
      availability: "persisted immediately after writer acceptance",
    },
    {
      requestId: "restore",
      sequence: inputLiveV41RequestSequence("restore"),
      phase: "restore",
      predecessorRequestId: "writer",
      programSha256: programs.restoreBlueprintSha256,
      response: INPUT_LIVE_V41_RESPONSE_CONTRACTS.restore,
      availability:
        "issued immediately after writer acceptance; before extract",
    },
    {
      requestId: "extract",
      sequence: inputLiveV41RequestSequence("extract"),
      phase: "extract",
      predecessorRequestId: "restore",
      programSha256: programs.extractBlueprintSha256,
      response: INPUT_LIVE_V41_RESPONSE_CONTRACTS.extract,
    },
    {
      requestId: "probe",
      sequence: inputLiveV41RequestSequence("probe"),
      phase: "probe",
      predecessorRequestId: "extract",
      programSha256: programs.probeBlueprintSha256,
      response: INPUT_LIVE_V41_RESPONSE_CONTRACTS.probe,
    },
    ...captures.map((cell) => ({
      requestId: `capture-${String(cell.index).padStart(3, "0")}`,
      sequence: inputLiveV41RequestSequence("capture", cell.index),
      phase: "capture",
      ...
      response: INPUT_LIVE_V41_RESPONSE_CONTRACTS.capture,
    })),
  ];
  return {
    artifactVersion: "input-live-v41-request-manifest-v1",
    expectedDynamicTool: INPUT_LIVE_V41_DYNAMIC_TOOL,
    target: INPUT_LIVE_V41_TARGET,
...
const lifecycleExcludedPaths = [
  INPUT_LIVE_V41_AUTHORIZATION_TEMPLATE_PATH,
  INPUT_LIVE_V41_AUTHORIZATION_PATH,
  "recipe/input-field-live-v41-authorization.ts",
  "recipe/input-field-live-v41-authorization.test.ts",
  "recipe/input-field-live-v41-preflight.ts",
  "recipe/input-field-live-v41-authorized.ts",
  "recipe/evidence/input-field-live-pivot-v41/operator-security-attestation-template.json",
...
const sourceNeutralDependencies = [
  ...
  "recipe/input-field-live-v41-verifier.ts",
  "recipe/figma-property-normalizer-v8.ts",
  "recipe/figma-runtime-portability.ts",
  "recipe/input-field-objective-comparison-v1.ts",
  "recipe/normalize.ts",
  "recipe/scene-readback-runtime-v41.ts",
  "recipe/scene-readback-v41.ts",
  "recipe/input-field-live-v41-restore.ts",
] as const;

export async function buildInputLiveV41Proof(
  check = process.argv.includes("--check"),
): Promise<InputLiveV41ProofPlan> {
  ...
  if (
    ...
    throw new Error(
      "Input live v41 exact committed v16 writer/restore/runtime/extract or hashed v17-v40 scene-readback bytes drifted",
    );

  const writerOwnershipBlueprint: InputLiveV41WriterOwnership = {
    pageId: "__WRITER_PAGE_ID__",
    pageName: "Recipe Pivot / Input Field / v9",
    runIdentity: "input-live-v41",
  ...
  const identities = sources.map(
    (source) =>
      ...
      }) satisfies InputLiveV41SourceIdentity,
  );
  const restoreBlueprint = Buffer.from(
    buildInputLiveV41RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildInputLiveV41ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildInputLiveV41ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildInputLiveV41CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );

  const antecedentOutputs = new Map<string, Buffer>();
  antecedentOutputs.set(INPUT_LIVE_V41_PROTOCOL_PATH, jsonBytes(protocol()));
  antecedentOutputs.set(
    `${INPUT_LIVE_V41_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V41_EVIDENCE_ROOT}/programs/writer-payload.js`,
    writerPayload,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V41_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V41_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V41_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V41_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  ...
  const captureManifest = {
    artifactVersion: "input-live-v41-capture-manifest-v1",
    ...
    cells: captures,
    cellsSha256: inputLiveV41CaptureManifestSha256(captures),
  };
  ...
  const proofPlan: InputLiveV41ProofPlan = {
    artifactVersion: "input-live-v41-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: INPUT_LIVE_V41_TARGET,
    namespace: "ds.contracts.input.recipe.v5",
    writer: {
      programPath: `${INPUT_LIVE_V41_EVIDENCE_ROOT}/programs/writer.txt`,
      programBytes: writerProgram.byteLength,
      programSha256: sha256(writerProgram),
      payloadPath: `${INPUT_LIVE_V41_EVIDENCE_ROOT}/programs/writer-payload.js`,
    ...
    captureManifest: {
      path: INPUT_LIVE_V41_CAPTURE_MANIFEST_PATH,
      cells: 128,
      requests: 128,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(INPUT_LIVE_V41_PLAN_PATH, proofPlanBytes);
  ...
  antecedentOutputs.set(
    INPUT_LIVE_V41_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/input-field-live-v41-broker.ts",
    "recipe/input-field-live-v41-contract.ts",
    "recipe/run-input-field-live-v41.ts",
    "recipe/build-input-field-live-proof-v41.ts",
    "recipe/input-field-live-v41-broker.test.ts",
    "recipe/figma-property-normalizer-v8.test.ts",
    "recipe/scene-readback-v41.test.ts",
    "recipe/input-field-live-v41-restore.ts",
    "recipe/input-field-live-v41-restore.test.ts",
    "recipe/input-field-live-v41-lifecycle-simulation.ts",
    ...sourceNeutralDependencies,
  ].filter((artifactPath) => artifactPath !== INPUT_LIVE_V41_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "input-live-v41-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    ...
    counts: {
      ...
      captureCells: INPUT_LIVE_V41_CAPTURE_COUNT,
  ...
  antecedentOutputs.set(INPUT_LIVE_V41_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "input-live-v41-authorization-template-v1",
    authorizationId: "input-live-v41",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: INPUT_LIVE_V41_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
  ...
  if (check) {
    ...
    mkdirSync(path.dirname(INPUT_LIVE_V41_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(INPUT_LIVE_V41_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(INPUT_LIVE_V41_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(INPUT_LIVE_V41_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildInputLiveV41Proof(), null, 2)}\n`,
  );
