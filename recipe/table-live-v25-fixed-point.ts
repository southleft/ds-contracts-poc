/**
 * Table live v25 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V25_FIXED_VERIFIER_VERSION = "table-live-v25-fixed-point-v1";
export const TABLE_LIVE_V25_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV25FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV25FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV25FixedSourceProbe {
...
export interface TableLiveV25FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV25FixedFixedPoint;
}

export interface TableLiveV25FixedHardGateInput {
  thresholds: TableLiveV25FixedThresholds;
  sourceProbes: TableLiveV25FixedSourceProbe[];
  cells: TableLiveV25FixedCellValidation[];
  sceneProofs: TableLiveV25FixedSceneProof[];
...
export interface TableLiveV25FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V25_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV25FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV25FixedHardGates(
  input: TableLiveV25FixedHardGateInput,
): TableLiveV25FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV25FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V25_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV25FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV25FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
