...
import { calendarLiveV1Sha256 } from "./calendar-live-v36-broker.js";

export interface CalendarLiveV36LifecycleSimulationReport {
  artifactVersion: "calendar-live-v36-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV36AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV36Lifecycle(): Promise<CalendarLiveV36LifecycleSimulationReport> {
  await buildCalendarLiveV36Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V36_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV36AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v36-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V36_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V36_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV36History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV36History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV36AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V36_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V36_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV36History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV36History(authorized, "pending");
    ...
    await buildCalendarLiveV36Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V36_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV36AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV36Lifecycle(), null, 2)}\n`,
  );
