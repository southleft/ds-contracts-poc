...
import { TABLE_LIVE_V4_AUTHORIZATION_PATH } from "./build-table-live-proof-v4.js";
import { verifyTableLiveV4Authorization } from "./table-live-v4-authorization.js";
import { runTableLiveV4Preflight } from "./table-live-v4-preflight.js";
import { TableLiveV4Orchestrator } from "./run-table-live-v4.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v4 requires exact authorization artifact ${TABLE_LIVE_V4_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v4-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV4Authorization(root);
    runTableLiveV4Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV4Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV4Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
