...
export type TableLiveV35HistoryExpectation = "pending" | "authorized";

export interface TableLiveV35AuthorizationArtifact {
  artifactVersion: "table-live-v35-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V35_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV35AntecedentIndex {
...
export interface TableLiveV35HistoryState {
  index?: TableLiveV35AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV35AuthorizationArtifact;
...
export interface TableLiveV35AuthorizationProof
  extends TableLiveV35TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V35_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V35_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V35_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V35_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v35-authorization") ||
  artifactPath.includes("table-live-v35-preflight") ||
  artifactPath.includes("table-live-v35-authorized") ||
  artifactPath.includes("create-table-live-v35-security-attestation") ||
  artifactPath.includes("table-live-pivot-v35-status") ||
...
export function validateTableLiveV35AntecedentIndex(
  index: TableLiveV35AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v35-antecedent-index-v1" ||
...
export function validateTableLiveV35UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV35History\(\s*["']/.test(source) ||
    /readTableLiveV35HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV35AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV35AuthorizationArtifact {
  ...
  ) as TableLiveV35AntecedentIndex;
  const indexFailures = validateTableLiveV35AntecedentIndex(index);
...
export function validateTableLiveV35AuthorizationArtifact(
  artifact: TableLiveV35AuthorizationArtifact | undefined,
  index: TableLiveV35AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v35-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v35" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V35_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V35_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V35_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V35_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V35_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V35_SOURCE_ROOTS ||
...
export function validateTableLiveV35History(
  state: TableLiveV35HistoryState,
  expected: TableLiveV35HistoryExpectation,
): string[] {
  const failures = validateTableLiveV35AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV35AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV35HistoryState(
  root = process.cwd(),
): TableLiveV35HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V35_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV35AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V35_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V35_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V35_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV35History(
  expected: TableLiveV35HistoryExpectation,
  root = process.cwd(),
): TableLiveV35HistoryState {
  const state = readTableLiveV35HistoryState(root);
  const failures = validateTableLiveV35History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v35 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV35Authorization(
  root = process.cwd(),
): TableLiveV35AuthorizationProof {
  const state = verifyTableLiveV35History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V35_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v35/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v35.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V35_TARGET,
    expectedDynamicTool: TABLE_LIVE_V35_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V35_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV35HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV35History(expected);
  process.stdout.write(
    `Table live v35 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
