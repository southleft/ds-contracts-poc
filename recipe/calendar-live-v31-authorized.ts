...
import { CALENDAR_LIVE_V31_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v31.js";
import { verifyCalendarLiveV31Authorization } from "./calendar-live-v31-authorization.js";
import { runCalendarLiveV31Preflight } from "./calendar-live-v31-preflight.js";
import { CalendarLiveV31Orchestrator } from "./run-calendar-live-v31.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v31 requires exact authorization artifact ${CALENDAR_LIVE_V31_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v31-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV31Authorization(root);
    runCalendarLiveV31Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV31Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV31Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
