...
import { verifyComboboxLiveV31Authorization } from "./combobox-live-v31-authorization.js";
import { COMBOBOX_LIVE_V31_TARGET } from "./combobox-live-v31-broker.js";
import {
  COMBOBOX_LIVE_V31_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV31Authorization();
...
const attestation: ComboboxLiveV31SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV31SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v31 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V31_SECURITY_ATTESTATION_DEFAULT_PATH,
);
