...
export type ComboboxLiveV37HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV37AuthorizationArtifact {
  artifactVersion: "combobox-live-v37-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V37_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV37AntecedentIndex {
...
export interface ComboboxLiveV37HistoryState {
  index?: ComboboxLiveV37AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV37AuthorizationArtifact;
...
export interface ComboboxLiveV37AuthorizationProof extends ComboboxLiveV37TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V37_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V37_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V37_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V37_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v37-authorization") ||
  artifactPath.includes("combobox-live-v37-preflight") ||
  artifactPath.includes("combobox-live-v37-authorized") ||
  artifactPath.includes("create-combobox-live-v37-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v37-status") ||
...
export function validateComboboxLiveV37AntecedentIndex(
  index: ComboboxLiveV37AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v37-antecedent-index-v1" ||
...
export function validateComboboxLiveV37UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV37History\(\s*["']/.test(source) ||
    /readComboboxLiveV37HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV37AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV37AuthorizationArtifact {
  ...
  ) as ComboboxLiveV37AntecedentIndex;
  const indexFailures = validateComboboxLiveV37AntecedentIndex(index);
...
export function validateComboboxLiveV37AuthorizationArtifact(
  artifact: ComboboxLiveV37AuthorizationArtifact | undefined,
  index: ComboboxLiveV37AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v37-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v37" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V37_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V37_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V37_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V37_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V37_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V37_SOURCE_ROOTS ||
  ...
  if (
    ...
    artifact?.execution?.v37AuthorizationReusable !== false ||
...
export function validateComboboxLiveV37History(
  state: ComboboxLiveV37HistoryState,
  expected: ComboboxLiveV37HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV37AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV37AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV37HistoryState(
  root = process.cwd(),
): ComboboxLiveV37HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V37_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV37AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V37_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V37_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V37_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV37History(
  expected: ComboboxLiveV37HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV37HistoryState {
  const state = readComboboxLiveV37HistoryState(root);
  const failures = validateComboboxLiveV37History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v37 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV37Authorization(
  root = process.cwd(),
): ComboboxLiveV37AuthorizationProof {
  const state = verifyComboboxLiveV37History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V37_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v37/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v37.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V37_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V37_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V37_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV37HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV37History(expected);
  process.stdout.write(
    `Combobox live v37 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
