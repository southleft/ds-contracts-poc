...
export interface ComboboxLiveV8LifecycleSimulationReport {
  artifactVersion: "combobox-live-v8-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV8AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV8Lifecycle(): Promise<ComboboxLiveV8LifecycleSimulationReport> {
  await buildComboboxLiveV8Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V8_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV8AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v8-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V8_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V8_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV8History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV8History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV8AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V8_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V8_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV8History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV8History(authorized, "pending");
    ...
    await buildComboboxLiveV8Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V8_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV8AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV8Lifecycle(), null, 2)}\n`,
  );
