/**
 * Carried Combobox live v1 host verifier. Uses scene-readback-combobox-v1 so live
...
export const COMBOBOX_LIVE_V1_FIXED_VERIFIER_VERSION = "combobox-live-v1-fixed-point-v1";
export const COMBOBOX_LIVE_V1_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV1FixedThresholds {
...
export interface ComboboxLiveV1FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV1FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV1FixedSourceProbe {
...
export interface ComboboxLiveV1FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV1FixedFixedPoint;
}

export interface ComboboxLiveV1FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV1FixedHardGateInput {
  thresholds: ComboboxLiveV1FixedThresholds;
  materialRegression: ComboboxLiveV1FixedMaterialRegression;
  sourceProbes: ComboboxLiveV1FixedSourceProbe[];
  cells: ComboboxLiveV1FixedCellValidation[];
  sceneProofs: ComboboxLiveV1FixedSceneProof[];
  visualRows: ComboboxLiveV1FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV1FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V1_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV1FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV1FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV1FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV1FixedHardGates(
  input: ComboboxLiveV1FixedHardGateInput,
): ComboboxLiveV1FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV1FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV1FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V1_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV1FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV1FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
