...
import {
  ...
  CALENDAR_LIVE_V23_HOST_PHASES,
  CALENDAR_LIVE_V23_REMOTE_REQUESTS,
  CALENDAR_LIVE_V23_SOURCE_ROOTS,
  CALENDAR_LIVE_V23_VARIANT_COUNT,
  buildCalendarLiveV23CaptureProgram,
  buildCalendarLiveV23CleanupProgram,
  buildCalendarLiveV23ExtractProgram,
  buildCalendarLiveV23ProbeProgram,
  buildCalendarLiveV23RestoreProgram,
  calendarLiveV1CaptureManifestSha256,
  type CalendarLiveV23CaptureCell,
  type CalendarLiveV23SourceIdentity,
  type CalendarLiveV23WriterOwnership,
} from "./calendar-live-v23-contract.js";
import {
  CALENDAR_LIVE_V23_DYNAMIC_TOOL,
  CALENDAR_LIVE_V23_TARGET,
  calendarLiveV1RequestSequence,
} from "./calendar-live-v23-broker.js";
import {
...
export const CALENDAR_LIVE_V23_EVIDENCE_ROOT =
  "recipe/evidence/calendar-live-pivot-v23";
export const CALENDAR_LIVE_V23_PROTOCOL_PATH = `${CALENDAR_LIVE_V23_EVIDENCE_ROOT}/protocol.json`;
export const CALENDAR_LIVE_V23_PLAN_PATH = `${CALENDAR_LIVE_V23_EVIDENCE_ROOT}/proof-plan.json`;
export const CALENDAR_LIVE_V23_CAPTURE_MANIFEST_PATH = `${CALENDAR_LIVE_V23_EVIDENCE_ROOT}/capture-manifest.json`;
export const CALENDAR_LIVE_V23_REQUEST_MANIFEST_PATH = `${CALENDAR_LIVE_V23_EVIDENCE_ROOT}/request-manifest.json`;
export const CALENDAR_LIVE_V23_INDEX_PATH = `${CALENDAR_LIVE_V23_EVIDENCE_ROOT}/antecedent-index.json`;
export const CALENDAR_LIVE_V23_AUTHORIZATION_TEMPLATE_PATH = `${CALENDAR_LIVE_V23_EVIDENCE_ROOT}/authorization-template.json`;
export const CALENDAR_LIVE_V23_AUTHORIZATION_PATH = `${CALENDAR_LIVE_V23_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface CalendarLiveV23ProofPlan {
  artifactVersion: "calendar-live-v23-proof-plan-v1";
...
const captureCells = (
...
): CalendarLiveV23CaptureCell[] => {
  const cells: CalendarLiveV23CaptureCell[] = [];
  ...
  if (cells.length !== CALENDAR_LIVE_V23_CAPTURE_COUNT)
    throw new Error(
      `Calendar live v23 capture cells ${cells.length}, expected ${CALENDAR_LIVE_V23_CAPTURE_COUNT}`,
    );
...
const protocol = (expectedFacts: number) => ({
  artifactVersion: "calendar-live-v23-external-operator-protocol-v1",
  protocolId: "calendar-live-v23",
  status:
  ...
  denominator: {
    sourceRoots: CALENDAR_LIVE_V23_SOURCE_ROOTS,
    sets: 3,
    variants: CALENDAR_LIVE_V23_VARIANT_COUNT,
    captureCells: CALENDAR_LIVE_V23_CAPTURE_COUNT,
    remoteRequests: CALENDAR_LIVE_V23_REMOTE_REQUESTS,
    hostPhases: CALENDAR_LIVE_V23_HOST_PHASES,
...
export async function buildCalendarLiveV23Proof(
  check = process.argv.includes("--check"),
): Promise<CalendarLiveV23ProofPlan> {
  ...
  if (
    ...
    !writer.runIdentity.endsWith("-calendar-v23") ||
    ...
    throw new Error(
      "Calendar live v23 writer reused Input, Combobox, or Table v1–v24 identity",
    );
  const captures = captureCells(sources);
  const writerProgram = Buffer.from(writer.code);
  const writerOwnershipBlueprint: CalendarLiveV23WriterOwnership = {
  ...
  const identities: CalendarLiveV23SourceIdentity[] = sources.map((source) => ({
    source: source.source,
    adapterIdentity: source.adapterIdentity,
    recipeHash: source.recipeHash,
    envelopeHash: source.envelope.integrity.canonicalHash,
  ...
  const restoreBlueprint = Buffer.from(
    buildCalendarLiveV23RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildCalendarLiveV23ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildCalendarLiveV23ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildCalendarLiveV23CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );
  const cleanupBlueprint = Buffer.from(
    buildCalendarLiveV23CleanupProgram(writerOwnershipBlueprint),
  );
  ...
  const sourceMetadata = sources.map((source) => {
    ...
    const tablePath = `${CALENDAR_LIVE_V23_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-calendar.json.gz`;
    const rowPath = `${CALENDAR_LIVE_V23_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-week.json.gz`;
    const cellPath = `${CALENDAR_LIVE_V23_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-day.json.gz`;
  ...
  antecedentOutputs.set(
    CALENDAR_LIVE_V23_PROTOCOL_PATH,
    jsonBytes(protocol(expectedFacts)),
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V23_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V23_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V23_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V23_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V23_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  antecedentOutputs.set(
    `${CALENDAR_LIVE_V23_EVIDENCE_ROOT}/programs/cleanup-blueprint.js`,
    cleanupBlueprint,
  );

  const captureManifest = {
    artifactVersion: "calendar-live-v23-capture-manifest-v1",
  ...
  antecedentOutputs.set(
    CALENDAR_LIVE_V23_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: CalendarLiveV23ProofPlan = {
    artifactVersion: "calendar-live-v23-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: CALENDAR_LIVE_V23_TARGET,
    namespace: CALENDAR_FIGMA_NAMESPACE,
    writer: {
      programPath: `${CALENDAR_LIVE_V23_EVIDENCE_ROOT}/programs/writer.txt`,
    ...
    captureManifest: {
      path: CALENDAR_LIVE_V23_CAPTURE_MANIFEST_PATH,
      cells: 8,
      requests: 8,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(CALENDAR_LIVE_V23_PLAN_PATH, jsonBytes(proofPlan));

  const requests = {
    artifactVersion: "calendar-live-v23-request-manifest-v1",
    expectedDynamicTool: CALENDAR_LIVE_V23_DYNAMIC_TOOL,
    target: CALENDAR_LIVE_V23_TARGET,
    signedAtRuntime: true,
    requestCount: CALENDAR_LIVE_V23_REMOTE_REQUESTS,
    requests: [
      {
        requestId: "writer",
  ...
  if (requests.requestCount !== CALENDAR_LIVE_V23_REMOTE_REQUESTS)
    throw new Error("Calendar live v23 remote request denominator drifted");
  antecedentOutputs.set(
    CALENDAR_LIVE_V23_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const lifecycleExcludedPaths = [
    CALENDAR_LIVE_V23_AUTHORIZATION_TEMPLATE_PATH,
    CALENDAR_LIVE_V23_AUTHORIZATION_PATH,
    "recipe/calendar-live-v23-authorization.ts",
    "recipe/calendar-live-v23-authorization.test.ts",
    "recipe/calendar-live-v23-preflight.ts",
    "recipe/calendar-live-v23-authorized.ts",
    "recipe/write-calendar-live-v23-authorization.ts",
    "recipe/create-calendar-live-v23-security-attestation.ts",
    "recipe/evidence/calendar-live-pivot-v23/operator-security-attestation-template.json",
    "recipe/evidence/calendar-live-pivot-v23-status.json",
    "recipe/evidence/status-index.json",
  ];
  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/calendar-live-v23-broker.ts",
    "recipe/calendar-live-v23-contract.ts",
    "recipe/calendar-live-v23-restore.ts",
    "recipe/calendar-live-v23-cleanup.ts",
    "recipe/calendar-live-v23-verifier.ts",
    "recipe/calendar-live-v23-fixed-point.ts",
    "recipe/run-calendar-live-v23.ts",
    "recipe/build-calendar-live-proof-v23.ts",
    "recipe/calendar-live-v23-lifecycle-simulation.ts",
    ...
    "recipe/normalize.ts",
  ].filter((artifactPath) => artifactPath !== CALENDAR_LIVE_V23_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "calendar-live-v23-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
  ...
  antecedentOutputs.set(CALENDAR_LIVE_V23_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "calendar-live-v23-authorization-template-v1",
    authorizationId: "calendar-live-v23",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: CALENDAR_LIVE_V23_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
    ...
    denominator: {
      remoteRequests: CALENDAR_LIVE_V23_REMOTE_REQUESTS,
      hostPhases: CALENDAR_LIVE_V23_HOST_PHASES,
      captures: CALENDAR_LIVE_V23_CAPTURE_COUNT,
      sourceRoots: CALENDAR_LIVE_V23_SOURCE_ROOTS,
      expectedFacts,
    },
    execution: {
  ...
  const attestationTemplate = jsonBytes({
    artifactVersion: "calendar-live-v23-operator-security-attestation-template-v1",
    status: "template only; complete privately after PREPARE+AUTHORIZE",
    tokenValuesForbidden: true,
    privateDirectory: "private/calendar-live-v23-security-attestation.json",
  });
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Calendar live v23 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(CALENDAR_LIVE_V23_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(CALENDAR_LIVE_V23_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
    writeFileSync(
      `${CALENDAR_LIVE_V23_EVIDENCE_ROOT}/operator-security-attestation-template.json`,
      attestationTemplate,
    );
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(CALENDAR_LIVE_V23_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(CALENDAR_LIVE_V23_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Calendar live v23 authorization template drift");
  }
  if (
    writer.namespace !== CALENDAR_FIGMA_NAMESPACE ||
    writer.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    ...
    !readFileSync("recipe/calendar-live-v23-contract.ts", "utf8").includes(
    ...
    throw new Error("Calendar live v23 identity or cleanup invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildCalendarLiveV23Proof(), null, 2)}\n`,
  );
