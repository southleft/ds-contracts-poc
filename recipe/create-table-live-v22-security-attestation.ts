...
import { verifyTableLiveV22Authorization } from "./table-live-v22-authorization.js";
import { TABLE_LIVE_V22_TARGET } from "./table-live-v22-broker.js";
import {
  TABLE_LIVE_V22_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV22Authorization();
...
const attestation: TableLiveV22SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV22SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v22 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V22_SECURITY_ATTESTATION_DEFAULT_PATH,
);
