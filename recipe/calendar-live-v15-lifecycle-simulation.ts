...
import { calendarLiveV1Sha256 } from "./calendar-live-v15-broker.js";

export interface CalendarLiveV15LifecycleSimulationReport {
  artifactVersion: "calendar-live-v15-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV15AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV15Lifecycle(): Promise<CalendarLiveV15LifecycleSimulationReport> {
  await buildCalendarLiveV15Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V15_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV15AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v15-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V15_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V15_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV15History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV15History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV15AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V15_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V15_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV15History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV15History(authorized, "pending");
    ...
    await buildCalendarLiveV15Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V15_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV15AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV15Lifecycle(), null, 2)}\n`,
  );
