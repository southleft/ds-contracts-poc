...
import { verifyCalendarLiveV18Authorization } from "./calendar-live-v18-authorization.js";
import { CALENDAR_LIVE_V18_TARGET } from "./calendar-live-v18-broker.js";
import {
  CALENDAR_LIVE_V18_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV18Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v18-session";
...
const attestation: CalendarLiveV18SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV18SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v18 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V18_SECURITY_ATTESTATION_DEFAULT_PATH,
);
