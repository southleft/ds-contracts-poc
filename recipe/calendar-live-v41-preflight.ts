...
import {
  validateCalendarLiveV41AntecedentIndex,
  verifyCalendarLiveV41Authorization,
  type CalendarLiveV41AntecedentIndex,
  type CalendarLiveV41AuthorizationProof,
} from "./calendar-live-v41-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V41_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v41/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V41_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v41-security-attestation.json";

export interface CalendarLiveV41SecurityAttestation {
  artifactVersion: "calendar-live-v41-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV41PreflightReport {
  artifactVersion: "calendar-live-v41-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV41SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV41SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV41AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v41-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V41_TARGET) ||
...
export function validateCalendarLiveV41ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV41Authorization");
  const preflight = source.indexOf("runCalendarLiveV41Preflight");
  const initialize = source.indexOf("CalendarLiveV41Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV41Preflight(
  root: string,
  proof: CalendarLiveV41AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V41_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV41PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V41_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V41_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV41AntecedentIndex;
  failures.push(...validateCalendarLiveV41AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V41_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V41_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV41SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v41-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV41ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v41 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV41SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v41-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V41_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV41Preflight(
    process.cwd(),
    verifyCalendarLiveV41Authorization(),
    attestationPath,
