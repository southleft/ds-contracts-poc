...
export type TableLiveV22HistoryExpectation = "pending" | "authorized";

export interface TableLiveV22AuthorizationArtifact {
  artifactVersion: "table-live-v22-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V22_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV22AntecedentIndex {
...
export interface TableLiveV22HistoryState {
  index?: TableLiveV22AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV22AuthorizationArtifact;
...
export interface TableLiveV22AuthorizationProof
  extends TableLiveV22TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V22_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V22_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V22_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V22_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v22-authorization") ||
  artifactPath.includes("table-live-v22-preflight") ||
  artifactPath.includes("table-live-v22-authorized") ||
  artifactPath.includes("create-table-live-v22-security-attestation") ||
  artifactPath.includes("table-live-pivot-v22-status") ||
...
export function validateTableLiveV22AntecedentIndex(
  index: TableLiveV22AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v22-antecedent-index-v1" ||
...
export function validateTableLiveV22UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV22History\(\s*["']/.test(source) ||
    /readTableLiveV22HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV22AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV22AuthorizationArtifact {
  ...
  ) as TableLiveV22AntecedentIndex;
  const indexFailures = validateTableLiveV22AntecedentIndex(index);
...
export function validateTableLiveV22AuthorizationArtifact(
  artifact: TableLiveV22AuthorizationArtifact | undefined,
  index: TableLiveV22AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v22-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v22" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V22_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V22_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V22_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V22_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V22_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V22_SOURCE_ROOTS ||
...
export function validateTableLiveV22History(
  state: TableLiveV22HistoryState,
  expected: TableLiveV22HistoryExpectation,
): string[] {
  const failures = validateTableLiveV22AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV22AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV22HistoryState(
  root = process.cwd(),
): TableLiveV22HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V22_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV22AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V22_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V22_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V22_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV22History(
  expected: TableLiveV22HistoryExpectation,
  root = process.cwd(),
): TableLiveV22HistoryState {
  const state = readTableLiveV22HistoryState(root);
  const failures = validateTableLiveV22History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v22 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV22Authorization(
  root = process.cwd(),
): TableLiveV22AuthorizationProof {
  const state = verifyTableLiveV22History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V22_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v22/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v22.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V22_TARGET,
    expectedDynamicTool: TABLE_LIVE_V22_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V22_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV22HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV22History(expected);
  process.stdout.write(
    `Table live v22 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
