...
import { verifyTableLiveV10Authorization } from "./table-live-v10-authorization.js";
import { TABLE_LIVE_V10_TARGET } from "./table-live-v10-broker.js";
import {
  TABLE_LIVE_V10_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV10Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-table-v10-session";
...
const attestation: TableLiveV10SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV10SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v10 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V10_SECURITY_ATTESTATION_DEFAULT_PATH,
);
