...
export const INPUT_LIVE_V58_VERIFIER_VERSION = "input-live-v58-verifier-v1";

export function buildInputLiveV58RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV58RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV58RawNode[];
}

export interface InputLiveV58NormalizedScene {
...
export interface InputLiveV58SceneProof {
...
export function normalizeInputLiveV58Scene(
  root: InputLiveV58RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV58NormalizedScene {
  const canonicalBindings: InputLiveV58NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV58RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV58Scene<Instance>(
  normalized: InputLiveV58NormalizedScene,
...
export function compileInputLiveV58ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV58PreCaptureGates(payload: unknown): void {
