...
import { INPUT_LIVE_V19_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v19.js";
import { verifyInputLiveV19Authorization } from "./input-field-live-v19-authorization.js";
import { runInputLiveV19Preflight } from "./input-field-live-v19-preflight.js";
import { InputLiveV19Orchestrator } from "./run-input-field-live-v19.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V19_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v19-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV19Authorization(root);
    runInputLiveV19Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV19Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
