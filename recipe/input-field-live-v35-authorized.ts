...
import { INPUT_LIVE_V35_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v35.js";
import { verifyInputLiveV35Authorization } from "./input-field-live-v35-authorization.js";
import { runInputLiveV35Preflight } from "./input-field-live-v35-preflight.js";
import { InputLiveV35Orchestrator } from "./run-input-field-live-v35.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V35_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v35-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV35Authorization(root);
    runInputLiveV35Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV35Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
