...
import { verifyCalendarLiveV11Authorization } from "./calendar-live-v11-authorization.js";
import { CALENDAR_LIVE_V11_TARGET } from "./calendar-live-v11-broker.js";
import {
  CALENDAR_LIVE_V11_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV11Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v11-session";
...
const attestation: CalendarLiveV11SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV11SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v11 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V11_SECURITY_ATTESTATION_DEFAULT_PATH,
);
