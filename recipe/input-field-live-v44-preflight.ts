...
import {
  ...
  verifyInputLiveV44Authorization,
  type InputLiveV44AntecedentIndex,
  type InputLiveV44AuthorizationProof,
} from "./input-field-live-v44-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V44_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v44/operator-security-attestation-template.json";
export const INPUT_LIVE_V44_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v44-security-attestation.json";

export interface InputLiveV44SecurityAttestation {
...
export interface InputLiveV44PreflightReport {
  artifactVersion: "input-live-v44-preflight-v1";
...
export const inputLiveV44AttestationSha256 = (
  value: Omit<InputLiveV44SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV44SecurityAttestation(
  value: unknown,
  proof: InputLiveV44AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v44-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V44_TARGET) ||
...
export function validateInputLiveV44ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV44Authorization");
  const preflight = source.indexOf("runInputLiveV44Preflight");
  const initialize = source.indexOf("InputLiveV44Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV44Preflight(
  root: string,
  proof: InputLiveV44AuthorizationProof,
  attestationPath = INPUT_LIVE_V44_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV44PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V44_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V44_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V44_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV44AntecedentIndex;
  failures.push(...validateInputLiveV44AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V44_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V44_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v44-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV44ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV44SecurityAttestation;
  return {
    artifactVersion: "input-live-v44-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V44_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV44Preflight(
    process.cwd(),
    verifyInputLiveV44Authorization(),
    attestationPath,
