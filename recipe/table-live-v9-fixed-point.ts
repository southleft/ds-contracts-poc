/**
 * Table live v9 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V9_FIXED_VERIFIER_VERSION = "table-live-v9-fixed-point-v1";
export const TABLE_LIVE_V9_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV9FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV9FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV9FixedSourceProbe {
...
export interface TableLiveV9FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV9FixedFixedPoint;
}

export interface TableLiveV9FixedHardGateInput {
  thresholds: TableLiveV9FixedThresholds;
  sourceProbes: TableLiveV9FixedSourceProbe[];
  cells: TableLiveV9FixedCellValidation[];
  sceneProofs: TableLiveV9FixedSceneProof[];
...
export interface TableLiveV9FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V9_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV9FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV9FixedHardGates(
  input: TableLiveV9FixedHardGateInput,
): TableLiveV9FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV9FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  if (input.visualRows.length !== 0)
    failures.push("table live v9 must not invent a visual comparison");
  ...
  return {
    verifierVersion: TABLE_LIVE_V9_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV9FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV9FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
