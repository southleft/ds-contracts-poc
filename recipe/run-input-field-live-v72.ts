...
import {
  acceptInputLiveV72Response,
  createInputLiveV72Transaction,
  inputLiveV72ReceiptPath,
  inputLiveV72RequestPath,
  inputLiveV72ResponsePath,
  inputLiveV72Sha256,
  issueInputLiveV72Request,
  persistInputLiveV72RawResponse,
  persistInputLiveV72TechnicalGates,
  readInputLiveV72AcceptedRawPayload,
  type InputLiveV72Request,
  type InputLiveV72TransactionAuthorization,
} from "./input-field-live-v72-broker.js";
import {
  INPUT_LIVE_V72_CAPTURE_COUNT,
  assertInputLiveV72CaptureResponses,
  assertInputLiveV72RootProofs,
  buildInputLiveV72CaptureProgram,
  buildInputLiveV72CleanupProgram,
  buildInputLiveV72ExtractProgram,
  buildInputLiveV72ProbeProgram,
  buildInputLiveV72RestoreProgram,
  validateInputLiveV72RestorePayload,
  evaluateInputLiveV72Objective,
  inputLiveV72CaptureManifestSha256,
  proveInputLiveV72Roots,
  validateInputLiveV72CaptureManifest,
  validateInputLiveV72CapturePayload,
  validateInputLiveV72CleanupPayload,
  validateInputLiveV72ExtractPayload,
  validateInputLiveV72ProbePayload,
  validateInputLiveV72WriterPayload,
  type InputLiveV72CaptureCell,
  type InputLiveV72CapturePayload,
  type InputLiveV72RootProof,
  type InputLiveV72SourceIdentity,
  type InputLiveV72WriterOwnership,
} from "./input-field-live-v72-contract.js";
import {
  INPUT_LIVE_V72_CAPTURE_MANIFEST_PATH,
  INPUT_LIVE_V72_PLAN_PATH,
  type InputLiveV72ProofPlan,
...
import type { ExpectedScenePlan } from "./scene-readback-v72.js";

export const INPUT_LIVE_V72_RUNNER_VERSION = "input-live-v72-runner-v1";

interface InputLiveV72JournalEntry {
  index: number;
  event: string;
...
interface InputLiveV72OrchestratorState {
  artifactVersion: "input-live-v72-orchestrator-state-v1";
  ...
  journal: InputLiveV72JournalEntry[];
}

interface LoadedProofInputs {
  plan: InputLiveV72ProofPlan;
  planSha256: string;
  captures: InputLiveV72CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    InputLiveV72SourceIdentity & {
...
export interface InputLiveV72NextAction {
...
const writeState = (
  directory: string,
  state: InputLiveV72OrchestratorState,
): void => {
...
const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, INPUT_LIVE_V72_PLAN_PATH);
  const capturePath = path.join(root, INPUT_LIVE_V72_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as InputLiveV72ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as InputLiveV72CaptureCell[];
  validateInputLiveV72CaptureManifest(captures);
  if (
    plan.captureManifest.sha256 !== inputLiveV72Sha256(captureBytes) ||
    captureArtifact.cellsSha256 !== inputLiveV72CaptureManifestSha256(captures)
  )
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (
      inputLiveV72Sha256(expectedBytes) !== source.expectedScenePlan.sha256 ||
      inputLiveV72Sha256(uncompressed) !==
  ...
  return {
    plan,
    planSha256: inputLiveV72Sha256(planBytes),
    captures,
    captureManifestSha256: inputLiveV72Sha256(captureBytes),
    sources,
...
const appendJournal = (
  ...
  const payloadSha256 = inputLiveV72Sha256(canonicalJson(payload));
  const body = {
  ...
  const entry = {
    ...body,
    entrySha256: inputLiveV72Sha256(canonicalJson(body)),
  };
  writeNew(
    path.join(
      directory,
...
const writerOwnership = (directory: string): InputLiveV72WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): InputLiveV72RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): InputLiveV72Request =>
  readJson<InputLiveV72Request>(file);

export class InputLiveV72Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueInputLiveV72Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: InputLiveV72OrchestratorState = {
      artifactVersion: "input-live-v72-orchestrator-state-v1",
      transactionDirectory: options.transactionDirectory,
      activeRequest: inputLiveV72RequestPath(
        options.transactionDirectory,
        "writer",
      ),
      cleanupRequest: null,
    ...
    return new InputLiveV72Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<InputLiveV72OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new InputLiveV72Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): InputLiveV72NextAction {
    const cleanup =
      this.#state.cleanupRequest &&
      !this.#state.cleanupAccepted &&
      !existsSync(inputLiveV72ReceiptPath(this.#directory, "cleanup"))
        ? this.#state.cleanupRequest
        : null;
  ...
  ingestAndAdvance(rawResponse: string): InputLiveV72NextAction {
  ...
  ingestCleanup(rawResponse: string): InputLiveV72NextAction {
    ...
    persistInputLiveV72RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptInputLiveV72Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV72CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      existsSync(inputLiveV72ReceiptPath(this.#directory, "writer")) &&
      !existsSync(path.join(this.#directory, OWNERSHIP_FILE))
    ) {
      const raw = readInputLiveV72AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateInputLiveV72WriterPayload(raw),
      );
    }
    if (existsSync(inputLiveV72RequestPath(this.#directory, "cleanup")))
      this.#state.cleanupRequest = inputLiveV72RequestPath(
        this.#directory,
        "cleanup",
      );
    if (
      existsSync(path.join(this.#directory, OWNERSHIP_FILE)) &&
      !existsSync(inputLiveV72RequestPath(this.#directory, "cleanup"))
    )
      this.#issueCleanupAndRestore(false);
    else if (
      existsSync(inputLiveV72RequestPath(this.#directory, "cleanup")) &&
      !existsSync(inputLiveV72RequestPath(this.#directory, "restore"))
    )
      this.#issueRestore(false);
    if (
      existsSync(inputLiveV72RequestPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV72ReceiptPath(this.#directory, "restore"))
    )
      this.#state.activeRequest = inputLiveV72RequestPath(
        this.#directory,
        "restore",
      );
    if (
      existsSync(inputLiveV72ReceiptPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV72RequestPath(this.#directory, "extract"))
    ) {
      const ownership = writerOwnership(this.#directory);
      const extract = issueInputLiveV72Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildInputLiveV72ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV72RequestPath(
        this.#directory,
        "extract",
    ...
    if (existsSync(inputLiveV72ReceiptPath(this.#directory, "cleanup")))
      this.#state.cleanupAccepted = true;
    this.#persist();
  }

  #acceptActive(request: InputLiveV72Request): void {
    if (request.phase === "writer") {
      const accepted = acceptInputLiveV72Response(this.#directory, "writer", {
        validate: validateInputLiveV72WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptInputLiveV72Response(this.#directory, "restore", {
        validate: (payload) =>
          validateInputLiveV72RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptInputLiveV72Response(this.#directory, "extract", {
        validate: (payload) =>
          validateInputLiveV72ExtractPayload(payload, ownership),
      });
      const proofs = proveInputLiveV72Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseInputFieldRecipe,
        compileInputFieldRecipe,
      );
      assertInputLiveV72RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "input-live-v72-host-root-proofs-v1",
      ...
      appendJournal(
        ...
        request.requestId,
        { proofsSha256: inputLiveV72Sha256(canonicalJson(proofs)) },
      );
      const next = issueInputLiveV72Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildInputLiveV72ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV72RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptInputLiveV72Response(this.#directory, "probe", {
        validate: (payload) =>
          validateInputLiveV72ProbePayload(payload, ownership),
      });
      persistInputLiveV72TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      const accepted = acceptInputLiveV72Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateInputLiveV72CapturePayload(payload, cell, rawBytes),
      });
      const png = Buffer.from(accepted.payload.pngBase64, "base64");
      const capturePath = path.join(
        this.#directory,
        "captures",
        `${String(cell.index).padStart(3, "0")}-${inputLiveV72Sha256(cell.cellKey).slice(0, 20)}.png`,
      );
      ...
      if (next < INPUT_LIVE_V72_CAPTURE_COUNT) this.#issueCapture(next);
      else {
          ...
          pngBase64: Buffer.from(
            readFileSync(
              path.join(
                this.#directory,
                "captures",
                `${String(response.index).padStart(3, "0")}-${inputLiveV72Sha256(response.cellKey).slice(0, 20)}.png`,
              ),
            ),
          ).toString("base64"),
        }));
        assertInputLiveV72CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateInputLiveV72Objective(
        ...
        appendJournal(
          ...
          request.requestId,
          {
            reportSha256: inputLiveV72Sha256(canonicalJson(objective)),
  ...
  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueInputLiveV72Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildInputLiveV72CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = inputLiveV72RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueInputLiveV72Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildInputLiveV72RestoreProgram(ownership),
    );
    this.#state.activeRequest = inputLiveV72RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueInputLiveV72Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildInputLiveV72CaptureProgram(
  ...
  #readCaptureResponseIndex(): InputLiveV72CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: InputLiveV72CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedInputLiveV72Authorization = (
  privateKey: KeyObject,
): InputLiveV72TransactionAuthorization => ({
  mode: "simulated",
  ...
  signingPublicKeySha256: inputLiveV72Sha256(
...
export const generateInputLiveV72SimulationKey = (): KeyObject =>
