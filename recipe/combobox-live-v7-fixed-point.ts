/**
 * Carried Combobox live v7 host verifier. Uses scene-readback-combobox-v7 so live
...
export const COMBOBOX_LIVE_V7_FIXED_VERIFIER_VERSION = "combobox-live-v7-fixed-point-v1";
export const COMBOBOX_LIVE_V7_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV7FixedThresholds {
...
export interface ComboboxLiveV7FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV7FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV7FixedSourceProbe {
...
export interface ComboboxLiveV7FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV7FixedFixedPoint;
}

export interface ComboboxLiveV7FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV7FixedHardGateInput {
  thresholds: ComboboxLiveV7FixedThresholds;
  materialRegression: ComboboxLiveV7FixedMaterialRegression;
  sourceProbes: ComboboxLiveV7FixedSourceProbe[];
  cells: ComboboxLiveV7FixedCellValidation[];
  sceneProofs: ComboboxLiveV7FixedSceneProof[];
  visualRows: ComboboxLiveV7FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV7FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V7_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV7FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV7FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV7FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV7FixedHardGates(
  input: ComboboxLiveV7FixedHardGateInput,
): ComboboxLiveV7FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV7FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV7FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V7_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV7FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV7FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
