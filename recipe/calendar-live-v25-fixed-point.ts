/**
 * Calendar live v25 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V25_FIXED_VERIFIER_VERSION = "calendar-live-v25-fixed-point-v1";
export const CALENDAR_LIVE_V25_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV25FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV25FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV25FixedSourceProbe {
...
export interface CalendarLiveV25FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV25FixedFixedPoint;
}

export interface CalendarLiveV25FixedHardGateInput {
  thresholds: CalendarLiveV25FixedThresholds;
  sourceProbes: CalendarLiveV25FixedSourceProbe[];
  cells: CalendarLiveV25FixedCellValidation[];
  sceneProofs: CalendarLiveV25FixedSceneProof[];
...
export interface CalendarLiveV25FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V25_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV25FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV25FixedHardGates(
  input: CalendarLiveV25FixedHardGateInput,
): CalendarLiveV25FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV25FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V25_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV25FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV25FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV25FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
