...
import {
  ...
  verifyCalendarLiveV36Authorization,
  type CalendarLiveV36AntecedentIndex,
  type CalendarLiveV36AuthorizationProof,
} from "./calendar-live-v36-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V36_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v36/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V36_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v36-security-attestation.json";

export interface CalendarLiveV36SecurityAttestation {
...
export interface CalendarLiveV36PreflightReport {
  artifactVersion: "calendar-live-v36-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV36SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV36SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV36AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v36-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V36_TARGET) ||
...
export function validateCalendarLiveV36ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV36Authorization");
  const preflight = source.indexOf("runCalendarLiveV36Preflight");
  const initialize = source.indexOf("CalendarLiveV36Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV36Preflight(
  root: string,
  proof: CalendarLiveV36AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V36_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV36PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V36_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V36_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV36AntecedentIndex;
  failures.push(...validateCalendarLiveV36AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V36_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V36_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v36-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV36ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v36 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV36SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v36-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V36_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV36Preflight(
    process.cwd(),
    verifyCalendarLiveV36Authorization(),
    attestationPath,
