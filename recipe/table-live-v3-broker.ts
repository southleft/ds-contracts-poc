...
import {
  assertTableLiveV3RootProofs,
  TABLE_LIVE_V3_CAPTURE_COUNT,
  TABLE_LIVE_V3_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  TABLE_LIVE_V3_RESPONSE_CONTRACTS,
  type TableLiveV3RootProof,
} from "./table-live-v3-contract.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V3_BROKER_VERSION = "table-live-v3-broker-v1";
export const TABLE_LIVE_V3_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const TABLE_LIVE_V3_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const TABLE_LIVE_V3_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const TABLE_LIVE_V3_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const TABLE_LIVE_V3_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const TABLE_LIVE_V3_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type TableLiveV3RemotePhase =
  (typeof TABLE_LIVE_V3_REMOTE_PHASES)[number];
export type TableLiveV3AuthorizationMode = "live" | "simulated";

export interface TableLiveV3CommitPins {
...
export interface TableLiveV3TransactionAuthorization extends TableLiveV3CommitPins {
  mode: TableLiveV3AuthorizationMode;
...
export interface TableLiveV3Manifest {
  artifactVersion: "table-live-v3-broker-manifest-v2";
  brokerVersion: typeof TABLE_LIVE_V3_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof TABLE_LIVE_V3_DYNAMIC_TOOL;
  target: typeof TABLE_LIVE_V3_TARGET;
  authorization: TableLiveV3TransactionAuthorization;
...
export interface TableLiveV3ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface TableLiveV3RequestBody {
  artifactVersion: "table-live-v3-broker-request-v2";
  brokerVersion: typeof TABLE_LIVE_V3_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: TableLiveV3RemotePhase;
  sequence: number;
  ...
  commitPins: TableLiveV3CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof TABLE_LIVE_V3_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: TableLiveV3ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface TableLiveV3Request extends TableLiveV3RequestBody {
...
export interface TableLiveV3Receipt {
  artifactVersion: "table-live-v3-broker-receipt-v2";
  brokerVersion: typeof TABLE_LIVE_V3_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: TableLiveV3RemotePhase;
...
export interface TableLiveV3AcceptedResponse<T = unknown> {
  request: TableLiveV3Request;
  receipt: TableLiveV3Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface TableLiveV3IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: TableLiveV3ResponseContract;
...
const equalTarget = (value: unknown): value is typeof TABLE_LIVE_V3_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === TABLE_LIVE_V3_TARGET.fileKey &&
  (value as BrokerTarget).fileName === TABLE_LIVE_V3_TARGET.fileName &&
  (value as BrokerTarget).editorType === TABLE_LIVE_V3_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== TABLE_LIVE_V3_TARGET.fileKey ||
    context.fileName !== TABLE_LIVE_V3_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === TABLE_LIVE_V3_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Table live v3 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= TABLE_LIVE_V3_CAPTURE_COUNT
  )
    throw new TypeError("Table live v3 capture index must be 0..19");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Table live v3 authorization pins are malformed");
};

const readManifest = (directory: string): TableLiveV3Manifest => {
  const manifest = json<TableLiveV3Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Table live v3 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "table-live-v3-broker-manifest-v2" ||
    manifest.brokerVersion !== TABLE_LIVE_V3_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== TABLE_LIVE_V3_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V3_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Table live v3 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: TableLiveV3Request,
): TableLiveV3RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: TableLiveV3RemotePhase,
  captureIndex?: number,
): TableLiveV3Request | undefined => {
  const file = tableLiveV2RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV3Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: TableLiveV3RemotePhase,
  captureIndex?: number,
): TableLiveV3Receipt | undefined => {
  const file = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV3Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: TableLiveV3Request | undefined;
  receipt: TableLiveV3Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: TableLiveV3RemotePhase,
): TableLiveV3ResponseContract => ({
  ...TABLE_LIVE_V3_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: TABLE_LIVE_V3_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: TABLE_LIVE_V3_DYNAMIC_TOOL,
...
const __broker=${JSON.stringify({
  artifactVersion: "table-live-v3-plugin-response-v2",
  brokerVersion: TABLE_LIVE_V3_BROKER_VERSION,
  target: TABLE_LIVE_V3_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*TABLE_LIVE_V3_PROGRAM_START*/${program}/*TABLE_LIVE_V3_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*TABLE_LIVE_V3_PROGRAM_START*/";
  const endMarker = "/*TABLE_LIVE_V3_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Table live v3 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createTableLiveV3Transaction(
  directory: string,
  ...
  if (existsSync(path.join(directory, MANIFEST_FILE)))
    throw new Error("Table live v3 transaction already exists");
  validateAuthorization(options.authorization);
  if (
    !SHA256.test(options.proofPlanSha256) ||
    !SHA256.test(options.captureManifestSha256)
  )
    throw new TypeError("Table live v3 transaction plan hashes malformed");
  const attempt = options.attempt ?? 1;
  if (!Number.isInteger(attempt) || attempt < 1 || attempt > 3)
    throw new TypeError("Table live v3 attempt must be 1..3");
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Table live v3 private signing key does not match authorized public-key identity",
    );
  const manifest: TableLiveV3Manifest = {
    artifactVersion: "table-live-v3-broker-manifest-v2",
    brokerVersion: TABLE_LIVE_V3_BROKER_VERSION,
    ...
    captureRequests: TABLE_LIVE_V3_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateTableLiveV3Request(
  directory: string,
  request: TableLiveV3Request,
): void {
  ...
  if (
    request.artifactVersion !== "table-live-v3-broker-request-v2" ||
    request.brokerVersion !== TABLE_LIVE_V3_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== TABLE_LIVE_V3_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Table live v3 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueTableLiveV3Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: TableLiveV3RemotePhase,
  program: string,
  options: TableLiveV3IssueOptions = {},
): TableLiveV3Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<TableLiveV3Request>(targetPath);
    validateTableLiveV3Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Table live v3 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(tableLiveV2ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Table live v3 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Table live v3 empty code envelope refused");
  if (phase === "capture") validateTableLiveV3TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Table live v3 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: TABLE_LIVE_V3_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? TABLE_LIVE_V3_SIGNED_WRITER_TIMEOUT_MS
        : TABLE_LIVE_V3_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: TableLiveV3RequestBody = {
    artifactVersion: "table-live-v3-broker-request-v2",
    brokerVersion: TABLE_LIVE_V3_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: TableLiveV3Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateTableLiveV3Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistTableLiveV3RawResponse(
  directory: string,
  phase: TableLiveV3RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Table live v3 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<TableLiveV3Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Table live v3 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = tableLiveV2ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Table live v3 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<TableLiveV3Receipt, "receiptSha256"> = {
    artifactVersion: "table-live-v3-broker-receipt-v2",
    brokerVersion: TABLE_LIVE_V3_BROKER_VERSION,
...
export function acceptTableLiveV3Response<T = unknown>(
  directory: string,
  phase: TableLiveV3RemotePhase,
  options: {
    captureIndex?: number;
...
): TableLiveV3AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Table live v3 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<TableLiveV3Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Table live v3 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "table-live-v3-plugin-response-v2" ||
    plugin?.brokerVersion !== TABLE_LIVE_V3_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Table live v3 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistTableLiveV3TechnicalGates(
  directory: string,
  rootProofs: readonly TableLiveV3RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertTableLiveV3RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Table live v3 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Table live v3 probe payload hash malformed");
  const gates = Object.fromEntries(
    TABLE_LIVE_V3_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "table-live-v3-host-technical-gates-v2",
    brokerVersion: TABLE_LIVE_V3_BROKER_VERSION,
...
export function validateTableLiveV3TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "table-live-v3-host-technical-gates-v2" ||
    ...
    TABLE_LIVE_V3_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Table live v3 technical gates artifact");
}

export function validateTableLiveV3Receipt(
  directory: string,
  phase: TableLiveV3RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateTableLiveV3Request(directory, request);
  const receipt = json<TableLiveV3Receipt>(
  ...
  if (
    receipt.artifactVersion !== "table-live-v3-broker-receipt-v2" ||
    receipt.brokerVersion !== TABLE_LIVE_V3_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Table live v3 accepted receipt: ${request.requestId}`,
    );
}

export function readTableLiveV3AcceptedRawPayload(
  directory: string,
  phase: TableLiveV3RemotePhase,
  captureIndex?: number,
): unknown {
  validateTableLiveV3Receipt(directory, phase, captureIndex);
...
export function createTableLiveV3FakeDynamicResponse(
  request: TableLiveV3Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? TABLE_LIVE_V3_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? TABLE_LIVE_V3_TARGET,
    ...overrides.extra,
  };
}

export function listTableLiveV3PersistedRequests(
  directory: string,
): TableLiveV3Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<TableLiveV3Request>(file)] : [];
    });
}
