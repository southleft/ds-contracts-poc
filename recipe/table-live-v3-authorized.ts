...
import { TABLE_LIVE_V3_AUTHORIZATION_PATH } from "./build-table-live-proof-v3.js";
import { verifyTableLiveV3Authorization } from "./table-live-v3-authorization.js";
import { runTableLiveV3Preflight } from "./table-live-v3-preflight.js";
import { TableLiveV3Orchestrator } from "./run-table-live-v3.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v3 requires exact authorization artifact ${TABLE_LIVE_V3_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v3-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV3Authorization(root);
    runTableLiveV3Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV3Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV3Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
