...
import {
  assertInputLiveV73RootProofs,
  INPUT_LIVE_V73_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V73_RESPONSE_CONTRACTS,
  type InputLiveV73RootProof,
} from "./input-field-live-v73-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V73_BROKER_VERSION = "input-live-v73-broker-v2";
export const INPUT_LIVE_V73_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V73_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V73_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V73_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V73_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V73_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV73RemotePhase =
  (typeof INPUT_LIVE_V73_REMOTE_PHASES)[number];
export type InputLiveV73AuthorizationMode = "live" | "simulated";

export interface InputLiveV73CommitPins {
...
export interface InputLiveV73TransactionAuthorization extends InputLiveV73CommitPins {
  mode: InputLiveV73AuthorizationMode;
...
export interface InputLiveV73Manifest {
  artifactVersion: "input-live-v73-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V73_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V73_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V73_TARGET;
  authorization: InputLiveV73TransactionAuthorization;
...
export interface InputLiveV73ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV73RequestBody {
  artifactVersion: "input-live-v73-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V73_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV73RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV73CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V73_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV73ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV73Request extends InputLiveV73RequestBody {
...
export interface InputLiveV73Receipt {
  artifactVersion: "input-live-v73-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V73_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV73RemotePhase;
...
export interface InputLiveV73AcceptedResponse<T = unknown> {
  request: InputLiveV73Request;
  receipt: InputLiveV73Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV73IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV73ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V73_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V73_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V73_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V73_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V73_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V73_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V73_TARGET.editorType;
};
...
export const inputLiveV73RequestSequence = (
  phase: InputLiveV73RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV73RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV73RequestPath = (
  directory: string,
  phase: InputLiveV73RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV73ResponsePath = (
  directory: string,
  phase: InputLiveV73RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV73ReceiptPath = (
  directory: string,
  phase: InputLiveV73RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV73Manifest => {
  const manifest = json<InputLiveV73Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v73-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V73_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V73_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV73RemotePhase,
  captureIndex?: number,
): InputLiveV73Request | undefined => {
  const file = inputLiveV73RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV73Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV73RemotePhase,
  captureIndex?: number,
): InputLiveV73Receipt | undefined => {
  const file = inputLiveV73ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV73Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV73Request | undefined;
  receipt: InputLiveV73Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV73RemotePhase,
): InputLiveV73ResponseContract => ({
  ...INPUT_LIVE_V73_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V73_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V73_DYNAMIC_TOOL,
      target: INPUT_LIVE_V73_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v73-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V73_BROKER_VERSION,
  target: INPUT_LIVE_V73_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V73_PROGRAM_START*/${program}/*INPUT_LIVE_V73_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V73_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V73_PROGRAM_END*/";
...
export function createInputLiveV73Transaction(
  directory: string,
  ...
  const manifest: InputLiveV73Manifest = {
    artifactVersion: "input-live-v73-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V73_BROKER_VERSION,
...
export function validateInputLiveV73Request(
  directory: string,
  request: InputLiveV73Request,
): void {
  ...
  const expectedSequence = inputLiveV73RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v73-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V73_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V73_TARGET.fileKey ||
...
export function issueInputLiveV73Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV73RemotePhase,
  program: string,
  options: InputLiveV73IssueOptions = {},
): InputLiveV73Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV73RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV73Request>(targetPath);
    validateInputLiveV73Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV73ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV73TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV73RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V73_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V73_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V73_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV73RequestBody = {
    artifactVersion: "input-live-v73-broker-request-v2",
    brokerVersion: INPUT_LIVE_V73_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV73Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV73Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV73RawResponse(
  directory: string,
  phase: InputLiveV73RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV73RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV73Request>(requestFile);
  if (
  ...
  const target = inputLiveV73ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV73Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v73-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V73_BROKER_VERSION,
...
export function acceptInputLiveV73Response<T = unknown>(
  directory: string,
  phase: InputLiveV73RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV73AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV73ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV73Request>(
    inputLiveV73RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV73Request(directory, request);
  if (phase === "capture") validateInputLiveV73TechnicalGates(directory);
  const rawFile = inputLiveV73ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v73-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V73_BROKER_VERSION ||
...
export function persistInputLiveV73TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV73RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV73RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V73_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v73-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V73_BROKER_VERSION,
...
export function validateInputLiveV73TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v73-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V73_TECHNICAL_GATES.some(
...
export function validateInputLiveV73Receipt(
  directory: string,
  phase: InputLiveV73RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV73Request(directory, request);
  const receipt = json<InputLiveV73Receipt>(
    inputLiveV73ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV73ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v73-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V73_BROKER_VERSION ||
...
export function readInputLiveV73AcceptedRawPayload(
  directory: string,
  phase: InputLiveV73RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV73Receipt(directory, phase, captureIndex);
...
export function createInputLiveV73FakeDynamicResponse(
  request: InputLiveV73Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V73_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V73_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV73PersistedRequests(
  directory: string,
): InputLiveV73Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV73Request>(file)] : [];
    });
}

export const inputLiveV73Sha256 = sha256;
