...
import { verifyTableLiveV30Authorization } from "./table-live-v30-authorization.js";
import { TABLE_LIVE_V30_TARGET } from "./table-live-v30-broker.js";
import {
  TABLE_LIVE_V30_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV30Authorization();
...
const attestation: TableLiveV30SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV30SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v30 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V30_SECURITY_ATTESTATION_DEFAULT_PATH,
);
