...
export type TableLiveV31HistoryExpectation = "pending" | "authorized";

export interface TableLiveV31AuthorizationArtifact {
  artifactVersion: "table-live-v31-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V31_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV31AntecedentIndex {
...
export interface TableLiveV31HistoryState {
  index?: TableLiveV31AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV31AuthorizationArtifact;
...
export interface TableLiveV31AuthorizationProof
  extends TableLiveV31TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V31_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V31_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V31_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V31_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v31-authorization") ||
  artifactPath.includes("table-live-v31-preflight") ||
  artifactPath.includes("table-live-v31-authorized") ||
  artifactPath.includes("create-table-live-v31-security-attestation") ||
  artifactPath.includes("table-live-pivot-v31-status") ||
...
export function validateTableLiveV31AntecedentIndex(
  index: TableLiveV31AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v31-antecedent-index-v1" ||
...
export function validateTableLiveV31UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV31History\(\s*["']/.test(source) ||
    /readTableLiveV31HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV31AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV31AuthorizationArtifact {
  ...
  ) as TableLiveV31AntecedentIndex;
  const indexFailures = validateTableLiveV31AntecedentIndex(index);
...
export function validateTableLiveV31AuthorizationArtifact(
  artifact: TableLiveV31AuthorizationArtifact | undefined,
  index: TableLiveV31AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v31-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v31" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V31_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V31_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V31_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V31_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V31_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V31_SOURCE_ROOTS ||
...
export function validateTableLiveV31History(
  state: TableLiveV31HistoryState,
  expected: TableLiveV31HistoryExpectation,
): string[] {
  const failures = validateTableLiveV31AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV31AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV31HistoryState(
  root = process.cwd(),
): TableLiveV31HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V31_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV31AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V31_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V31_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V31_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV31History(
  expected: TableLiveV31HistoryExpectation,
  root = process.cwd(),
): TableLiveV31HistoryState {
  const state = readTableLiveV31HistoryState(root);
  const failures = validateTableLiveV31History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v31 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV31Authorization(
  root = process.cwd(),
): TableLiveV31AuthorizationProof {
  const state = verifyTableLiveV31History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V31_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v31/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v31.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V31_TARGET,
    expectedDynamicTool: TABLE_LIVE_V31_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V31_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV31HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV31History(expected);
  process.stdout.write(
    `Table live v31 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
