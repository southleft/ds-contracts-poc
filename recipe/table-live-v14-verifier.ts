...
import {
  verifyTableLiveV14FixedSceneFixedPoint,
  type TableLiveV14FixedFixedPoint,
} from "./table-live-v14-fixed-point.js";

export const TABLE_LIVE_V14_VERIFIER_VERSION = "table-live-v14-verifier-v1";

export function buildTableLiveV14RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV14RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV14RawNode[];
}

export interface TableLiveV14NormalizedScene {
...
export interface TableLiveV14SceneProof {
...
export function normalizeTableLiveV14Scene(
  root: TableLiveV14RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV14NormalizedScene {
  const canonicalBindings: TableLiveV14NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV14RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV14Scene<Instance>(
  normalized: TableLiveV14NormalizedScene,
...
): TableLiveV14SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV14FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV14ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV14PreCaptureGates(payload: unknown): void {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("table live v14 pre-capture gate payload absent");
