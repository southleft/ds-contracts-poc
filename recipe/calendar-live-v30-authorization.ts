...
export type CalendarLiveV30HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV30AuthorizationArtifact {
  artifactVersion: "calendar-live-v30-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V30_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV30AntecedentIndex {
...
export interface CalendarLiveV30HistoryState {
  index?: CalendarLiveV30AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV30AuthorizationArtifact;
...
export interface CalendarLiveV30AuthorizationProof
  extends CalendarLiveV30TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V30_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V30_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V30_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V30_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v30-authorization") ||
  artifactPath.includes("calendar-live-v30-preflight") ||
  artifactPath.includes("calendar-live-v30-authorized") ||
  artifactPath.includes("create-calendar-live-v30-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v30-status") ||
...
export function validateCalendarLiveV30AntecedentIndex(
  index: CalendarLiveV30AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v30-antecedent-index-v1" ||
...
export function validateCalendarLiveV30UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV30History\(\s*["']/.test(source) ||
    /readCalendarLiveV30HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV30AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV30AuthorizationArtifact {
  ...
  ) as CalendarLiveV30AntecedentIndex;
  const indexFailures = validateCalendarLiveV30AntecedentIndex(index);
...
export function validateCalendarLiveV30AuthorizationArtifact(
  artifact: CalendarLiveV30AuthorizationArtifact | undefined,
  index: CalendarLiveV30AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v30-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v30" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V30_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V30_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V30_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V30_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V30_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V30_SOURCE_ROOTS ||
...
export function validateCalendarLiveV30History(
  state: CalendarLiveV30HistoryState,
  expected: CalendarLiveV30HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV30AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV30AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV30HistoryState(
  root = process.cwd(),
): CalendarLiveV30HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V30_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV30AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V30_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V30_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V30_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV30History(
  expected: CalendarLiveV30HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV30HistoryState {
  const state = readCalendarLiveV30HistoryState(root);
  const failures = validateCalendarLiveV30History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v30 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV30Authorization(
  root = process.cwd(),
): CalendarLiveV30AuthorizationProof {
  const state = verifyCalendarLiveV30History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V30_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v30/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v30.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V30_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V30_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V30_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV30HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV30History(expected);
  process.stdout.write(
    `Calendar live v30 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
