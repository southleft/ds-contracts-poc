...
import { buildTableLiveV24CleanupRuntime } from "./table-live-v24-cleanup.js";
import { type TableLiveV24FixedFixedPoint } from "./table-live-v24-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV24RestoreProgram,
  validateTableLiveV24RestorePayload,
  type TableLiveV24RestorePayload,
} from "./table-live-v24-restore.js";
import {
  buildTableLiveV24RawPropertyRuntime,
  normalizeTableLiveV24Scene,
  type TableLiveV24RawNode,
} from "./table-live-v24-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v24.js";
import {
...
export const TABLE_LIVE_V24_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V24_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V24_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V24_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V24_VARIANT_COUNT = 20;
export const TABLE_LIVE_V24_SET_COUNT = 6;
export const TABLE_LIVE_V24_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V24_HOST_PHASES = 3;
export const TABLE_LIVE_V24_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V24_RESTORE_COUNT = 8;
export const TABLE_LIVE_V24_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V24_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV24SourceId = (typeof TABLE_LIVE_V24_SOURCE_IDS)[number];
export type TableLiveV24Kind = "table" | "row" | "cell";

export interface TableLiveV24SourceIdentity {
  source: TableLiveV24SourceId;
...
export interface TableLiveV24WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV24ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV24SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV24RawNode;
    rowScene: TableLiveV24RawNode;
    cellScene: TableLiveV24RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV24RootProof {
  source: TableLiveV24SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV24FixedFixedPoint;
}

export interface TableLiveV24ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV24SourceId;
    adapterIdentity: string;
...
export interface TableLiveV24CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV24SourceId;
  adapterIdentity: string;
...
export interface TableLiveV24CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV24SourceId;
  frameWidth: number;
...
export interface TableLiveV24CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v24 ${label} must be an object`);
...
export function validateTableLiveV24WriterPayload(
  value: unknown,
): TableLiveV24WriterOwnership {
  ...
  if (
    ...
    result.runIdentity === FORBIDDEN_TABLE_V23_RUN_IDENTITY ||
    result.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    result.namespace === FORBIDDEN_COMBOBOX_NAMESPACE ||
    result.namespace !== TABLE_LIVE_V24_NAMESPACE ||
    ...
    throw new TypeError("Table live v24 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v24 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v24 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV24SourceIdentity,
  kind: TableLiveV24Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV24ExtractProgram(
  writer: TableLiveV24WriterOwnership,
  sources: readonly TableLiveV24SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v24 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V24_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V24_NAMESPACE)}
${buildTableLiveV24RawPropertyRuntime()}
...
export function validateTableLiveV24ExtractPayload(
  value: unknown,
  writer: TableLiveV24WriterOwnership,
): TableLiveV24ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v24 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V24_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v24 extract schema/two-root mismatch");
  return payload as TableLiveV24ExtractPayload;
}

export function proveTableLiveV24Roots<Instance>(
  extract: TableLiveV24ExtractPayload,
  sources: readonly (TableLiveV24SourceIdentity & {
...
): TableLiveV24RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v24 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v24 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV24Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV24Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV24Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV24FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v24 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v24 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v24 compile lost owned sets");
...
export function assertTableLiveV24RootProofs(
  proofs: readonly TableLiveV24RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v24 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v24 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV24ProbeProgram(
  writer: TableLiveV24WriterOwnership,
  sources: readonly TableLiveV24SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V24_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV24ProbePayload(
  value: unknown,
  writer: TableLiveV24WriterOwnership,
): TableLiveV24ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV24ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V24_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v24 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV24CaptureManifest(
  cells: readonly TableLiveV24CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V24_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V24_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V24_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V24_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v24 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV24CaptureCell[],
): string => {
  validateTableLiveV24CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV24CaptureProgram(
  writer: TableLiveV24WriterOwnership,
  cell: TableLiveV24CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V24_CAPTURE_COUNT
  )
    throw new TypeError("Table live v24 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v24 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V24_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV24CapturePayload(
  value: unknown,
  cell: TableLiveV24CaptureCell,
  rawResponseBytes: number,
): TableLiveV24CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV24CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V24_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V24_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v24 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV24CaptureResponses(
  manifest: readonly TableLiveV24CaptureCell[],
  responses: readonly TableLiveV24CapturePayload[],
): void {
  validateTableLiveV24CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v24 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV24ObjectiveReport {
  artifactVersion: "table-live-v24-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV24Objective(
  manifest: readonly TableLiveV24CaptureCell[],
  responses: readonly TableLiveV24CapturePayload[],
): TableLiveV24ObjectiveReport {
  assertTableLiveV24CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v24-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV24CleanupProgram(
  writer: TableLiveV24WriterOwnership,
): string {
  const baseProgram = buildTableLiveV24CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V24_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v24 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV24CleanupPayload(
  payload: unknown,
  writer: TableLiveV24WriterOwnership,
): TableLiveV24CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV24CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v24 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V24_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV24WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
