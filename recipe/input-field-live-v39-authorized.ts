...
import { INPUT_LIVE_V39_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v39.js";
import { verifyInputLiveV39Authorization } from "./input-field-live-v39-authorization.js";
import { runInputLiveV39Preflight } from "./input-field-live-v39-preflight.js";
import { InputLiveV39Orchestrator } from "./run-input-field-live-v39.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V39_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v39-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV39Authorization(root);
    runInputLiveV39Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV39Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
