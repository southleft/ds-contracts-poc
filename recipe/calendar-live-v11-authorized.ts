...
import { CALENDAR_LIVE_V11_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v11.js";
import { verifyCalendarLiveV11Authorization } from "./calendar-live-v11-authorization.js";
import { runCalendarLiveV11Preflight } from "./calendar-live-v11-preflight.js";
import { CalendarLiveV11Orchestrator } from "./run-calendar-live-v11.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v11 requires exact authorization artifact ${CALENDAR_LIVE_V11_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v11-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV11Authorization(root);
    runCalendarLiveV11Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV11Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV11Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
