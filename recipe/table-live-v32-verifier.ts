...
import {
  verifyTableLiveV32FixedSceneFixedPoint,
  type TableLiveV32FixedFixedPoint,
} from "./table-live-v32-fixed-point.js";

export const TABLE_LIVE_V32_VERIFIER_VERSION = "table-live-v32-verifier-v1";

export function buildTableLiveV32RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV32RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV32RawNode[];
}

export interface TableLiveV32NormalizedScene {
...
export interface TableLiveV32SceneProof {
...
export function normalizeTableLiveV32Scene(
  root: TableLiveV32RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV32NormalizedScene {
  const canonicalBindings: TableLiveV32NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV32RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV32Scene<Instance>(
  normalized: TableLiveV32NormalizedScene,
...
): TableLiveV32SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV32FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV32ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV32PreCaptureGates(payload: unknown): void {
