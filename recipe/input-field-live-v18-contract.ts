...
} from "./input-field-live-v3-verifier-v18.js";
import {
  buildInputLiveV18RawPropertyRuntime,
  normalizeInputLiveV18Scene,
  type InputLiveV18RawNode,
} from "./input-field-live-v18-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v18.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v18.js";
...
export const INPUT_LIVE_V18_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V18_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V18_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V18_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V18_VARIANT_COUNT = 256;
export const INPUT_LIVE_V18_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V18_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV18SourceId = (typeof INPUT_LIVE_V18_SOURCE_IDS)[number];

export interface InputLiveV18SourceIdentity {
  source: InputLiveV18SourceId;
...
export interface InputLiveV18WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV18ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV18SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV18RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV18RootProof {
  source: InputLiveV18SourceId;
  adapterIdentity: string;
...
export interface InputLiveV18ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV18SourceId;
    adapterIdentity: string;
...
export interface InputLiveV18CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV18SourceId;
  adapterIdentity: string;
...
export interface InputLiveV18CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV18SourceId;
  strata: InputLiveV18CaptureCell["strata"];
...
export interface InputLiveV18CleanupPayload {
...
export function validateInputLiveV18WriterPayload(
  value: unknown,
): InputLiveV18WriterOwnership {
...
const identityPlan = (source: InputLiveV18SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV18ExtractProgram(
  writer: InputLiveV18WriterOwnership,
  sources: readonly InputLiveV18SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V18_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V18_NAMESPACE)}
${buildInputLiveV18RawPropertyRuntime()}
...
export function validateInputLiveV18ExtractPayload(
  value: unknown,
  writer: InputLiveV18WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V18_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV18ExtractPayload;
}

export function proveInputLiveV18Roots<Instance>(
  extract: InputLiveV18ExtractPayload,
  sources: readonly (InputLiveV18SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV18Scene(
      root.scene,
...
export function assertInputLiveV18RootProofs(
  proofs: readonly InputLiveV18RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV18ProbeProgram(
  writer: InputLiveV18WriterOwnership,
  sources: readonly InputLiveV18SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V18_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV18ProbePayload(
  value: unknown,
  writer: InputLiveV18WriterOwnership,
): InputLiveV18ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV18ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V18_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V18_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV18CaptureManifest(
  cells: readonly InputLiveV18CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V18_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V18_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V18_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V18_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V18_SOURCE_IDS.some(
...
export const inputLiveV18CaptureManifestSha256 = (
  cells: readonly InputLiveV18CaptureCell[],
): string => {
  validateInputLiveV18CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV18CaptureProgram(
  writer: InputLiveV18WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V18_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V18_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V18_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV18CapturePayload(
  value: unknown,
  cell: InputLiveV18CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV18CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V18_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V18_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV18CaptureResponses(
  manifest: readonly InputLiveV18CaptureCell[],
  responses: readonly InputLiveV18CapturePayload[],
): void {
  validateInputLiveV18CaptureManifest(manifest);
  if (
...
export interface InputLiveV18ObjectiveReport {
  artifactVersion: "input-live-v18-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV18Objective(
  manifest: readonly InputLiveV18CaptureCell[],
  responses: readonly InputLiveV18CapturePayload[],
  readReference: (cell: InputLiveV18CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV18CaptureCell) => Buffer,
): InputLiveV18ObjectiveReport {
  assertInputLiveV18CaptureResponses(manifest, responses);
...
export function buildInputLiveV18CleanupProgram(
  writer: InputLiveV18WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V18_NAMESPACE,
...
export function validateInputLiveV18CleanupPayload(
  value: unknown,
  writer: InputLiveV18WriterOwnership,
): InputLiveV18CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV18CleanupPayload;
...
export const INPUT_LIVE_V18_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV18WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
