...
import { INPUT_LIVE_V36_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v36.js";
import { verifyInputLiveV36Authorization } from "./input-field-live-v36-authorization.js";
import { runInputLiveV36Preflight } from "./input-field-live-v36-preflight.js";
import { InputLiveV36Orchestrator } from "./run-input-field-live-v36.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V36_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v36-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV36Authorization(root);
    runInputLiveV36Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV36Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
