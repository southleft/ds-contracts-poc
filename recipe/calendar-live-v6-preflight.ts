...
import {
  ...
  verifyCalendarLiveV6Authorization,
  type CalendarLiveV6AntecedentIndex,
  type CalendarLiveV6AuthorizationProof,
} from "./calendar-live-v6-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V6_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v6/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V6_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v6-security-attestation.json";

export interface CalendarLiveV6SecurityAttestation {
...
export interface CalendarLiveV6PreflightReport {
  artifactVersion: "calendar-live-v6-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV6SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV6SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV6AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v6-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V6_TARGET) ||
...
export function validateCalendarLiveV6ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV6Authorization");
  const preflight = source.indexOf("runCalendarLiveV6Preflight");
  const initialize = source.indexOf("CalendarLiveV6Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV6Preflight(
  root: string,
  proof: CalendarLiveV6AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V6_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV6PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V6_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V6_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV6AntecedentIndex;
  failures.push(...validateCalendarLiveV6AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V6_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V6_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v6-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV6ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v6 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV6SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v6-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V6_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV6Preflight(
    process.cwd(),
    verifyCalendarLiveV6Authorization(),
    attestationPath,
