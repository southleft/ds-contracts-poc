...
import {
  verifyComboboxLiveV8FixedSceneFixedPoint,
  type ComboboxLiveV8FixedFixedPoint,
} from "./combobox-live-v8-fixed-point.js";

export const COMBOBOX_LIVE_V8_VERIFIER_VERSION = "combobox-live-v8-verifier-v1";

export function buildComboboxLiveV8RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV8RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV8RawNode[];
}

export interface ComboboxLiveV8NormalizedScene {
...
export interface ComboboxLiveV8SceneProof {
...
export function normalizeComboboxLiveV8Scene(
  root: ComboboxLiveV8RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV8NormalizedScene {
  const canonicalBindings: ComboboxLiveV8NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV8RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV8Scene<Instance>(
  normalized: ComboboxLiveV8NormalizedScene,
...
): ComboboxLiveV8SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV8FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV8ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV8PreCaptureGates(payload: unknown): void {
