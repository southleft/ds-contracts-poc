...
import {
  assertComboboxLiveV6RootProofs,
  COMBOBOX_LIVE_V6_CAPTURE_COUNT,
  COMBOBOX_LIVE_V6_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  COMBOBOX_LIVE_V6_RESPONSE_CONTRACTS,
  type ComboboxLiveV6RootProof,
} from "./combobox-live-v6-contract.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V6_BROKER_VERSION = "combobox-live-v6-broker-v2";
export const COMBOBOX_LIVE_V6_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const COMBOBOX_LIVE_V6_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const COMBOBOX_LIVE_V6_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const COMBOBOX_LIVE_V6_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const COMBOBOX_LIVE_V6_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const COMBOBOX_LIVE_V6_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type ComboboxLiveV6RemotePhase =
  (typeof COMBOBOX_LIVE_V6_REMOTE_PHASES)[number];
export type ComboboxLiveV6AuthorizationMode = "live" | "simulated";

export interface ComboboxLiveV6CommitPins {
...
export interface ComboboxLiveV6TransactionAuthorization extends ComboboxLiveV6CommitPins {
  mode: ComboboxLiveV6AuthorizationMode;
...
export interface ComboboxLiveV6Manifest {
  artifactVersion: "combobox-live-v6-broker-manifest-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V6_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V6_DYNAMIC_TOOL;
  target: typeof COMBOBOX_LIVE_V6_TARGET;
  authorization: ComboboxLiveV6TransactionAuthorization;
...
export interface ComboboxLiveV6ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface ComboboxLiveV6RequestBody {
  artifactVersion: "combobox-live-v6-broker-request-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V6_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: ComboboxLiveV6RemotePhase;
  sequence: number;
  ...
  commitPins: ComboboxLiveV6CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof COMBOBOX_LIVE_V6_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: ComboboxLiveV6ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface ComboboxLiveV6Request extends ComboboxLiveV6RequestBody {
...
export interface ComboboxLiveV6Receipt {
  artifactVersion: "combobox-live-v6-broker-receipt-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V6_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: ComboboxLiveV6RemotePhase;
...
export interface ComboboxLiveV6AcceptedResponse<T = unknown> {
  request: ComboboxLiveV6Request;
  receipt: ComboboxLiveV6Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface ComboboxLiveV6IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: ComboboxLiveV6ResponseContract;
...
const equalTarget = (value: unknown): value is typeof COMBOBOX_LIVE_V6_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === COMBOBOX_LIVE_V6_TARGET.fileKey &&
  (value as BrokerTarget).fileName === COMBOBOX_LIVE_V6_TARGET.fileName &&
  (value as BrokerTarget).editorType === COMBOBOX_LIVE_V6_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== COMBOBOX_LIVE_V6_TARGET.fileKey ||
    context.fileName !== COMBOBOX_LIVE_V6_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === COMBOBOX_LIVE_V6_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Combobox live v6 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= COMBOBOX_LIVE_V6_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v6 capture index must be 0..71");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Combobox live v6 authorization pins are malformed");
};

const readManifest = (directory: string): ComboboxLiveV6Manifest => {
  const manifest = json<ComboboxLiveV6Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Combobox live v6 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "combobox-live-v6-broker-manifest-v2" ||
    manifest.brokerVersion !== COMBOBOX_LIVE_V6_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== COMBOBOX_LIVE_V6_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(COMBOBOX_LIVE_V6_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Combobox live v6 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: ComboboxLiveV6Request,
): ComboboxLiveV6RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: ComboboxLiveV6RemotePhase,
  captureIndex?: number,
): ComboboxLiveV6Request | undefined => {
  const file = comboboxLiveV5RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<ComboboxLiveV6Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: ComboboxLiveV6RemotePhase,
  captureIndex?: number,
): ComboboxLiveV6Receipt | undefined => {
  const file = comboboxLiveV5ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<ComboboxLiveV6Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: ComboboxLiveV6Request | undefined;
  receipt: ComboboxLiveV6Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: ComboboxLiveV6RemotePhase,
): ComboboxLiveV6ResponseContract => ({
  ...COMBOBOX_LIVE_V6_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: ComboboxLiveV6Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: COMBOBOX_LIVE_V6_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: COMBOBOX_LIVE_V6_DYNAMIC_TOOL,
      target: COMBOBOX_LIVE_V6_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "combobox-live-v6-plugin-response-v2",
  brokerVersion: COMBOBOX_LIVE_V6_BROKER_VERSION,
  target: COMBOBOX_LIVE_V6_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*COMBOBOX_LIVE_V6_PROGRAM_START*/${program}/*COMBOBOX_LIVE_V6_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*COMBOBOX_LIVE_V6_PROGRAM_START*/";
  const endMarker = "/*COMBOBOX_LIVE_V6_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Combobox live v6 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createComboboxLiveV6Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Combobox live v6 private signing key does not match authorized public-key identity",
    );
  const manifest: ComboboxLiveV6Manifest = {
    artifactVersion: "combobox-live-v6-broker-manifest-v2",
    brokerVersion: COMBOBOX_LIVE_V6_BROKER_VERSION,
    ...
    captureRequests: COMBOBOX_LIVE_V6_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateComboboxLiveV6Request(
  directory: string,
  request: ComboboxLiveV6Request,
): void {
  ...
  if (
    request.artifactVersion !== "combobox-live-v6-broker-request-v2" ||
    request.brokerVersion !== COMBOBOX_LIVE_V6_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== COMBOBOX_LIVE_V6_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Combobox live v6 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueComboboxLiveV6Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: ComboboxLiveV6RemotePhase,
  program: string,
  options: ComboboxLiveV6IssueOptions = {},
): ComboboxLiveV6Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<ComboboxLiveV6Request>(targetPath);
    validateComboboxLiveV6Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Combobox live v6 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(comboboxLiveV5ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Combobox live v6 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Combobox live v6 empty code envelope refused");
  if (phase === "capture") validateComboboxLiveV6TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Combobox live v6 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: COMBOBOX_LIVE_V6_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? COMBOBOX_LIVE_V6_SIGNED_WRITER_TIMEOUT_MS
        : COMBOBOX_LIVE_V6_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: ComboboxLiveV6RequestBody = {
    artifactVersion: "combobox-live-v6-broker-request-v2",
    brokerVersion: COMBOBOX_LIVE_V6_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: ComboboxLiveV6Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateComboboxLiveV6Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistComboboxLiveV6RawResponse(
  directory: string,
  phase: ComboboxLiveV6RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Combobox live v6 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<ComboboxLiveV6Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Combobox live v6 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = comboboxLiveV5ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Combobox live v6 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): ComboboxLiveV6Receipt => {
  const body: Omit<ComboboxLiveV6Receipt, "receiptSha256"> = {
    artifactVersion: "combobox-live-v6-broker-receipt-v2",
    brokerVersion: COMBOBOX_LIVE_V6_BROKER_VERSION,
...
export function acceptComboboxLiveV6Response<T = unknown>(
  directory: string,
  phase: ComboboxLiveV6RemotePhase,
  options: {
    captureIndex?: number;
...
): ComboboxLiveV6AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = comboboxLiveV5ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Combobox live v6 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<ComboboxLiveV6Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Combobox live v6 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "combobox-live-v6-plugin-response-v2" ||
    plugin?.brokerVersion !== COMBOBOX_LIVE_V6_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Combobox live v6 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistComboboxLiveV6TechnicalGates(
  directory: string,
  rootProofs: readonly ComboboxLiveV6RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertComboboxLiveV6RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Combobox live v6 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Combobox live v6 probe payload hash malformed");
  const gates = Object.fromEntries(
    COMBOBOX_LIVE_V6_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "combobox-live-v6-host-technical-gates-v2",
    brokerVersion: COMBOBOX_LIVE_V6_BROKER_VERSION,
...
export function validateComboboxLiveV6TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "combobox-live-v6-host-technical-gates-v2" ||
    ...
    COMBOBOX_LIVE_V6_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Combobox live v6 technical gates artifact");
}

export function validateComboboxLiveV6Receipt(
  directory: string,
  phase: ComboboxLiveV6RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateComboboxLiveV6Request(directory, request);
  const receipt = json<ComboboxLiveV6Receipt>(
  ...
  if (
    receipt.artifactVersion !== "combobox-live-v6-broker-receipt-v2" ||
    receipt.brokerVersion !== COMBOBOX_LIVE_V6_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Combobox live v6 accepted receipt: ${request.requestId}`,
    );
}

export function readComboboxLiveV6AcceptedRawPayload(
  directory: string,
  phase: ComboboxLiveV6RemotePhase,
  captureIndex?: number,
): unknown {
  validateComboboxLiveV6Receipt(directory, phase, captureIndex);
...
export function createComboboxLiveV6FakeDynamicResponse(
  request: ComboboxLiveV6Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? COMBOBOX_LIVE_V6_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? COMBOBOX_LIVE_V6_TARGET,
    ...overrides.extra,
  };
}

export function listComboboxLiveV6PersistedRequests(
  directory: string,
): ComboboxLiveV6Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<ComboboxLiveV6Request>(file)] : [];
    });
}
