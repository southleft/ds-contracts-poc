...
export const INPUT_LIVE_V12_VERIFIER_VERSION = "input-live-v12-verifier-v1";

export function buildInputLiveV12RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV12RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV12RawNode[];
}

export interface InputLiveV12NormalizedScene {
...
export interface InputLiveV12SceneProof {
...
export function normalizeInputLiveV12Scene(
  root: InputLiveV12RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV12NormalizedScene {
  const canonicalBindings: InputLiveV12NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: InputLiveV12RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV12Scene<Instance>(
  normalized: InputLiveV12NormalizedScene,
...
export function compileInputLiveV12ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV12PreCaptureGates(payload: unknown): void {
