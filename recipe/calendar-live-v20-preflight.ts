...
import {
  ...
  verifyCalendarLiveV20Authorization,
  type CalendarLiveV20AntecedentIndex,
  type CalendarLiveV20AuthorizationProof,
} from "./calendar-live-v20-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V20_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v20/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V20_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v20-security-attestation.json";

export interface CalendarLiveV20SecurityAttestation {
...
export interface CalendarLiveV20PreflightReport {
  artifactVersion: "calendar-live-v20-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV20SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV20SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV20AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v20-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V20_TARGET) ||
...
export function validateCalendarLiveV20ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV20Authorization");
  const preflight = source.indexOf("runCalendarLiveV20Preflight");
  const initialize = source.indexOf("CalendarLiveV20Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV20Preflight(
  root: string,
  proof: CalendarLiveV20AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V20_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV20PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V20_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V20_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV20AntecedentIndex;
  failures.push(...validateCalendarLiveV20AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V20_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V20_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v20-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV20ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v20 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV20SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v20-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V20_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV20Preflight(
    process.cwd(),
    verifyCalendarLiveV20Authorization(),
    attestationPath,
