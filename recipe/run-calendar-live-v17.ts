...
import {
  acceptCalendarLiveV17Response,
  createCalendarLiveV17Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV17Request,
  persistCalendarLiveV17RawResponse,
  persistCalendarLiveV17TechnicalGates,
  readCalendarLiveV17AcceptedRawPayload,
  type CalendarLiveV17Request,
  type CalendarLiveV17TransactionAuthorization,
} from "./calendar-live-v17-broker.js";
import {
  CALENDAR_LIVE_V17_CAPTURE_COUNT,
  assertCalendarLiveV17CaptureResponses,
  ...
  evaluateCalendarLiveV17Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV17Roots,
  validateCalendarLiveV17CaptureManifest,
  validateCalendarLiveV17CapturePayload,
  validateCalendarLiveV17CleanupPayload,
  validateCalendarLiveV17ExtractPayload,
  validateCalendarLiveV17ProbePayload,
  validateCalendarLiveV17WriterPayload,
  type CalendarLiveV17CaptureCell,
  type CalendarLiveV17CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V17_PLAN_PATH,
  type CalendarLiveV17ProofPlan,
} from "./build-calendar-live-proof-v17.js";
import {
...
export const CALENDAR_LIVE_V17_RUNNER_VERSION = "calendar-live-v17-runner-v1";

interface CalendarLiveV17JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV17OrchestratorState {
  artifactVersion: "calendar-live-v17-orchestrator-state-v1";
  ...
  journal: CalendarLiveV17JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV17ProofPlan;
  planSha256: string;
  captures: CalendarLiveV17CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV17SourceIdentity & {
...
export interface CalendarLiveV17NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV17OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v17 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V17_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V17_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV17ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV17CaptureCell[];
  validateCalendarLiveV17CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v17 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v17 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV17WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV17RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV17Request =>
  readJson<CalendarLiveV17Request>(file);

export class CalendarLiveV17Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV17Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV17OrchestratorState = {
      artifactVersion: "calendar-live-v17-orchestrator-state-v1",
    ...
    return new CalendarLiveV17Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV17OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV17Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV17NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV17NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v17 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV17NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v17 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV17RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV17Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV17CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV17AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV17WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV17Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV17ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV17Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV17Response(this.#directory, "writer", {
        validate: validateCalendarLiveV17WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV17Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV17RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV17Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV17ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV17Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV17RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v17-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV17Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV17ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV17Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV17ProbePayload(payload, ownership),
      });
      persistCalendarLiveV17TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v17 capture cell missing");
      const accepted = acceptCalendarLiveV17Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV17CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V17_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV17CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV17Objective(
    ...
    throw new Error(`Calendar live v17 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV17Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV17CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV17Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV17RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV17Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV17CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV17CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV17CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV17Authorization = (
  privateKey: KeyObject,
): CalendarLiveV17TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV17SimulationKey = (): KeyObject =>
