...
import { verifyCalendarLiveV39Authorization } from "./calendar-live-v39-authorization.js";
import { CALENDAR_LIVE_V39_TARGET } from "./calendar-live-v39-broker.js";
import {
  CALENDAR_LIVE_V39_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV39Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v39-session";
...
const attestation: CalendarLiveV39SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV39SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v39 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V39_SECURITY_ATTESTATION_DEFAULT_PATH,
);
