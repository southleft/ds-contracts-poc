...
import { buildComboboxLiveV6CleanupRuntime } from "./combobox-live-v6-cleanup.js";
import {
  type ComboboxLiveV6FixedFixedPoint,
} from "./combobox-live-v6-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV6RestoreProgram,
  validateComboboxLiveV6RestorePayload,
  type ComboboxLiveV6RestorePayload,
} from "./combobox-live-v6-restore.js";
import {
  buildComboboxLiveV6RawPropertyRuntime,
  normalizeComboboxLiveV6Scene,
  type ComboboxLiveV6RawNode,
} from "./combobox-live-v6-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v6.js";
import {
...
export const COMBOBOX_LIVE_V6_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V6_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V6_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V6_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V6_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V6_SET_COUNT = 4;
export const COMBOBOX_LIVE_V6_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V6_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V6_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V6_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V6_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV6SourceId = (typeof COMBOBOX_LIVE_V6_SOURCE_IDS)[number];

export interface ComboboxLiveV6SourceIdentity {
  source: ComboboxLiveV6SourceId;
...
export interface ComboboxLiveV6WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV6ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV6SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV6RawNode;
    optionScene: ComboboxLiveV6RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV6RootProof {
  source: ComboboxLiveV6SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV6FixedFixedPoint;
}

export interface ComboboxLiveV6ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV6SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV6CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV6SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV6CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV6SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV6CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v6 ${label} must be an object`);
...
export function validateComboboxLiveV6WriterPayload(
  value: unknown,
): ComboboxLiveV6WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V6_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v6 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v6 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v6 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV6SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV6ExtractProgram(
  writer: ComboboxLiveV6WriterOwnership,
  sources: readonly ComboboxLiveV6SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v6 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V6_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V6_NAMESPACE)}
${buildComboboxLiveV6RawPropertyRuntime()}
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("COMBOBOX-V6-EXTRACT-PAGE");
...
if(get(page,"pageOwner")!=="recipe/combobox/"+${JSON.stringify(writer.runIdentity)})throw new Error("COMBOBOX-V6-EXTRACT-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>SET_IDS.has(node.id));
if(sets.length!==4)throw new Error("COMBOBOX-V6-EXTRACT-ROOTS:"+sets.length);
...
for(const set of sets){
  ...
  if(!IDENTITIES[adapterIdentity]||(kind!=="combobox"&&kind!=="option"))throw new Error("COMBOBOX-V6-EXTRACT-ADAPTER:"+adapterIdentity+":"+kind);
...
if(roots.length!==2||roots.some(root=>!root.comboboxScene||!root.optionScene))throw new Error("COMBOBOX-V6-EXTRACT-PAIR");
...
export function validateComboboxLiveV6ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV6WriterOwnership,
): ComboboxLiveV6ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v6 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V6_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v6 extract schema/two-root mismatch");
  return payload as ComboboxLiveV6ExtractPayload;
}

export function proveComboboxLiveV6Roots<Instance>(
  extract: ComboboxLiveV6ExtractPayload,
  sources: readonly (ComboboxLiveV6SourceIdentity & {
...
): ComboboxLiveV6RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v6 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v6 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV6Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV6Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV6FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v6 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v6 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v6 compile lost owned sets");
...
export function assertComboboxLiveV6RootProofs(
  proofs: readonly ComboboxLiveV6RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v6 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v6 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV6ProbeProgram(
  writer: ComboboxLiveV6WriterOwnership,
  sources: readonly ComboboxLiveV6SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V6_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV6ProbePayload(
  value: unknown,
  writer: ComboboxLiveV6WriterOwnership,
): ComboboxLiveV6ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV6ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V6_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V6_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v6 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV6CaptureManifest(
  cells: readonly ComboboxLiveV6CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V6_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V6_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V6_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V6_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v6 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV6CaptureCell[],
): string => {
  validateComboboxLiveV6CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV6CaptureProgram(
  writer: ComboboxLiveV6WriterOwnership,
  cell: ComboboxLiveV6CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V6_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v6 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v6 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V6_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V6-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV6CapturePayload(
  value: unknown,
  cell: ComboboxLiveV6CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV6CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV6CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V6_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V6_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v6 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV6CaptureResponses(
  manifest: readonly ComboboxLiveV6CaptureCell[],
  responses: readonly ComboboxLiveV6CapturePayload[],
): void {
  validateComboboxLiveV6CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v6 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV6ObjectiveReport {
  artifactVersion: "combobox-live-v6-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV6Objective(
  manifest: readonly ComboboxLiveV6CaptureCell[],
  responses: readonly ComboboxLiveV6CapturePayload[],
): ComboboxLiveV6ObjectiveReport {
  assertComboboxLiveV6CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v6-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV6CleanupProgram(
  writer: ComboboxLiveV6WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV6CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V6_NAMESPACE,
  ...
  const guardedResult = `if(JSON.stringify(requestedNodeIds)!==JSON.stringify([expectedPageId])||JSON.stringify([...requestedCollectionIds].sort())!==JSON.stringify([...expectedCollectionIds].sort()))throw new Error("COMBOBOX-V6-CLEANUP-EXACT-OWNERSHIP");${resultStatement}`;
  const program = baseProgram.replace(resultStatement, guardedResult);
  if (program === baseProgram)
    throw new Error("Combobox live v6 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV6CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV6WriterOwnership,
): ComboboxLiveV6CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV6CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v6 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V6_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV6WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
