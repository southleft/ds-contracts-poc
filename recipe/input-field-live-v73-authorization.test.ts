...
import { INPUT_LIVE_V73_INDEX_PATH } from "./build-input-field-live-proof-v73.js";
import {
  INPUT_LIVE_V73_DYNAMIC_TOOL,
  INPUT_LIVE_V73_TARGET,
} from "./input-field-live-v73-broker.js";
import {
  buildInputLiveV73AuthorizationArtifact,
  validateInputLiveV73AntecedentIndex,
  validateInputLiveV73AuthorizationArtifact,
  validateInputLiveV73History,
  validateInputLiveV73UnitTestSource,
  type InputLiveV73AntecedentIndex,
  type InputLiveV73AuthorizationArtifact,
  type InputLiveV73AuthorizationProof,
  type InputLiveV73HistoryState,
} from "./input-field-live-v73-authorization.js";
import {
  inputLiveV73AttestationSha256,
  validateInputLiveV73ControlFlowSource,
  validateInputLiveV73SecurityAttestation,
  type InputLiveV73SecurityAttestation,
} from "./input-field-live-v73-preflight.js";
import { inputLiveV73Sha256 } from "./input-field-live-v73-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V73_INDEX_PATH);
const index = (): InputLiveV73AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV73AntecedentIndex;

const artifact = (): InputLiveV73AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV73AuthorizationArtifact({
...
const pendingState = (): InputLiveV73HistoryState => ({
  index: index(),
  indexSha256: inputLiveV73Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV73HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV73AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV73Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V73_TARGET,
  expectedDynamicTool: INPUT_LIVE_V73_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v73/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV73AuthorizationArtifact,
): InputLiveV73SecurityAttestation => {
  const body: Omit<InputLiveV73SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v73-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V73_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV73AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV73History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV73History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV73History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV73History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV73History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV73UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV73UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV73AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV73Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV73AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV73AuthorizationArtifact(
        planted as InputLiveV73AuthorizationArtifact,
        index(),
        inputLiveV73Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV73SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV73AttestationSha256(
      body as Omit<InputLiveV73SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV73SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV73ControlFlowSource(source), []);
});
