...
import {
  INPUT_LIVE_V73_MEASURE_VISIBLE_FILL_MARKER,
  INPUT_LIVE_V73_RESTORE_CONTENT_ROLES,
  INPUT_LIVE_V73_RESTORE_PARENT_ROLES,
...
test("v16 restore still measures FILL while visible, then restores visibility", () => {
  const program = buildInputLiveV73RestoreProgram(writer);
  assert.match(program, new RegExp(INPUT_LIVE_V73_MEASURE_VISIBLE_FILL_MARKER));
  ...
  assert.match(program, /INPUT-V73-RESTORE-COUNT/);
  assert.match(program, /INPUT-V73-RESTORE-NOT-FILL/);
  ...
  for (const role of INPUT_LIVE_V73_RESTORE_CONTENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
  for (const role of INPUT_LIVE_V73_RESTORE_PARENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
});

test("v16 restore payload requires 256 filled content texts", () => {
  const accepted = validateInputLiveV73RestorePayload(
    {
      pageId: "1:1",
      setIds: ["2:2", "2:1"],
      restoredCount: 256,
      ...
      marker: INPUT_LIVE_V73_MEASURE_VISIBLE_FILL_MARKER,
    },
    writer,
  );
...
test("v16 writer bytes stay frozen and v17 writer assigns set HUG", () => {
  ...
  const v73Program = readFileSync(
    "recipe/evidence/input-field-live-pivot-v73/programs/writer.txt",
  );
  const v73Payload = readFileSync(
    "recipe/evidence/input-field-live-pivot-v73/programs/writer-payload.js",
  );
  assert.equal(sha256(v16Program), V12_WRITER_PROGRAM_SHA256);
  assert.equal(sha256(v16Payload), V12_WRITER_PAYLOAD_SHA256);
  assert.notEqual(sha256(v73Program), V12_WRITER_PROGRAM_SHA256);
  assert.notEqual(sha256(v73Payload), V12_WRITER_PAYLOAD_SHA256);
  assert.match(v73Payload.toString("utf8"), /set\.layoutSizingHorizontal="HUG"/);
