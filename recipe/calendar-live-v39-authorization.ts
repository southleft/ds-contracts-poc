...
export type CalendarLiveV39HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV39AuthorizationArtifact {
  artifactVersion: "calendar-live-v39-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V39_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV39AntecedentIndex {
...
export interface CalendarLiveV39HistoryState {
  index?: CalendarLiveV39AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV39AuthorizationArtifact;
...
export interface CalendarLiveV39AuthorizationProof
  extends CalendarLiveV39TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V39_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V39_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V39_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V39_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v39-authorization") ||
  artifactPath.includes("calendar-live-v39-preflight") ||
  artifactPath.includes("calendar-live-v39-authorized") ||
  artifactPath.includes("create-calendar-live-v39-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v39-status") ||
...
export function validateCalendarLiveV39AntecedentIndex(
  index: CalendarLiveV39AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v39-antecedent-index-v1" ||
...
export function validateCalendarLiveV39UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV39History\(\s*["']/.test(source) ||
    /readCalendarLiveV39HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV39AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV39AuthorizationArtifact {
  ...
  ) as CalendarLiveV39AntecedentIndex;
  const indexFailures = validateCalendarLiveV39AntecedentIndex(index);
...
export function validateCalendarLiveV39AuthorizationArtifact(
  artifact: CalendarLiveV39AuthorizationArtifact | undefined,
  index: CalendarLiveV39AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v39-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v39" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V39_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V39_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V39_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V39_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V39_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V39_SOURCE_ROOTS ||
...
export function validateCalendarLiveV39History(
  state: CalendarLiveV39HistoryState,
  expected: CalendarLiveV39HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV39AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV39AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV39HistoryState(
  root = process.cwd(),
): CalendarLiveV39HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V39_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV39AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V39_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V39_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V39_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV39History(
  expected: CalendarLiveV39HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV39HistoryState {
  const state = readCalendarLiveV39HistoryState(root);
  const failures = validateCalendarLiveV39History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v39 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV39Authorization(
  root = process.cwd(),
): CalendarLiveV39AuthorizationProof {
  const state = verifyCalendarLiveV39History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V39_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v39/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v39.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V39_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V39_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V39_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV39HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV39History(expected);
  process.stdout.write(
    `Calendar live v39 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
