...
import {
  ...
  INPUT_LIVE_V80_INDEX_PATH,
} from "./build-input-field-live-proof-v80.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV80HistoryExpectation = "pending" | "authorized";

export interface InputLiveV80AuthorizationArtifact {
  artifactVersion: "input-live-v80-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V80_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV80AntecedentIndex {
...
export interface InputLiveV80HistoryState {
  index?: InputLiveV80AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV80AuthorizationArtifact;
...
export interface InputLiveV80AuthorizationProof extends InputLiveV80TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V80_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V80_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V80_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V80_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v80-authorization") ||
  artifactPath.includes("input-field-live-v80-preflight") ||
  artifactPath.includes("input-field-live-v80-authorized") ||
  artifactPath.includes("input-field-live-pivot-v80-status") ||
...
export function validateInputLiveV80AntecedentIndex(
  index: InputLiveV80AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v80-antecedent-index-v1" ||
...
export function validateInputLiveV80UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV80History\(\s*["']/.test(source) ||
    /readInputLiveV80HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV80AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV80AuthorizationArtifact {
  ...
  ) as InputLiveV80AntecedentIndex;
  const indexFailures = validateInputLiveV80AntecedentIndex(index);
...
export function validateInputLiveV80AuthorizationArtifact(
  artifact: InputLiveV80AuthorizationArtifact | undefined,
  index: InputLiveV80AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v80-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v80" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V80_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V80_TARGET) ||
    !equalJson(
...
export function validateInputLiveV80History(
  state: InputLiveV80HistoryState,
  expected: InputLiveV80HistoryExpectation,
): string[] {
  const failures = validateInputLiveV80AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV80AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV80HistoryState(
  root = process.cwd(),
): InputLiveV80HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V80_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV80AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V80_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V80_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V80_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV80History(
  expected: InputLiveV80HistoryExpectation,
  root = process.cwd(),
): InputLiveV80HistoryState {
  const state = readInputLiveV80HistoryState(root);
  const failures = validateInputLiveV80History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV80Authorization(
  root = process.cwd(),
): InputLiveV80AuthorizationProof {
  const state = verifyInputLiveV80History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V80_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v80/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v80.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V80_TARGET,
    expectedDynamicTool: INPUT_LIVE_V80_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V80_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV80HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV80History(expected);
