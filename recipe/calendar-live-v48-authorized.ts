...
import { CALENDAR_LIVE_V48_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v48.js";
import { verifyCalendarLiveV48Authorization } from "./calendar-live-v48-authorization.js";
import { runCalendarLiveV48Preflight } from "./calendar-live-v48-preflight.js";
import { CalendarLiveV48Orchestrator } from "./run-calendar-live-v48.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v48 requires exact authorization artifact ${CALENDAR_LIVE_V48_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v48-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV48Authorization(root);
    runCalendarLiveV48Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV48Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV48Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
