...
import {
  ...
  CALENDAR_LIVE_V28_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  CALENDAR_LIVE_V28_RESPONSE_CONTRACTS,
  CALENDAR_LIVE_V28_SOURCE_ROOTS,
  type CalendarLiveV28RootProof,
} from "./calendar-live-v28-contract.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V28_BROKER_VERSION = "calendar-live-v28-broker-v1";
export const CALENDAR_LIVE_V28_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const CALENDAR_LIVE_V28_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const CALENDAR_LIVE_V28_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const CALENDAR_LIVE_V28_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const CALENDAR_LIVE_V28_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const CALENDAR_LIVE_V28_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type CalendarLiveV28RemotePhase =
  (typeof CALENDAR_LIVE_V28_REMOTE_PHASES)[number];
export type CalendarLiveV28AuthorizationMode = "live" | "simulated";

export interface CalendarLiveV28CommitPins {
...
export interface CalendarLiveV28TransactionAuthorization extends CalendarLiveV28CommitPins {
  mode: CalendarLiveV28AuthorizationMode;
...
export interface CalendarLiveV28Manifest {
  artifactVersion: "calendar-live-v28-broker-manifest-v2";
  brokerVersion: typeof CALENDAR_LIVE_V28_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof CALENDAR_LIVE_V28_DYNAMIC_TOOL;
  target: typeof CALENDAR_LIVE_V28_TARGET;
...
export interface CalendarLiveV28ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface CalendarLiveV28RequestBody {
  artifactVersion: "calendar-live-v28-broker-request-v2";
  brokerVersion: typeof CALENDAR_LIVE_V28_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: CalendarLiveV28RemotePhase;
  sequence: number;
  ...
  commitPins: CalendarLiveV28CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof CALENDAR_LIVE_V28_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: CalendarLiveV28ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface CalendarLiveV28Request extends CalendarLiveV28RequestBody {
...
export interface CalendarLiveV28Receipt {
  artifactVersion: "calendar-live-v28-broker-receipt-v2";
  brokerVersion: typeof CALENDAR_LIVE_V28_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: CalendarLiveV28RemotePhase;
...
export interface CalendarLiveV28AcceptedResponse<T = unknown> {
  request: CalendarLiveV28Request;
  receipt: CalendarLiveV28Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface CalendarLiveV28IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: CalendarLiveV28ResponseContract;
...
const equalTarget = (value: unknown): value is typeof CALENDAR_LIVE_V28_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === CALENDAR_LIVE_V28_TARGET.fileKey &&
  (value as BrokerTarget).fileName === CALENDAR_LIVE_V28_TARGET.fileName &&
  (value as BrokerTarget).editorType === CALENDAR_LIVE_V28_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== CALENDAR_LIVE_V28_TARGET.fileKey ||
    context.fileName !== CALENDAR_LIVE_V28_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === CALENDAR_LIVE_V28_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Calendar live v28 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= CALENDAR_LIVE_V28_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v28 capture index must be 0..7");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Calendar live v28 authorization pins are malformed");
};

const readManifest = (directory: string): CalendarLiveV28Manifest => {
  const manifest = json<CalendarLiveV28Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Calendar live v28 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "calendar-live-v28-broker-manifest-v2" ||
    manifest.brokerVersion !== CALENDAR_LIVE_V28_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== CALENDAR_LIVE_V28_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(CALENDAR_LIVE_V28_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Calendar live v28 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: CalendarLiveV28Request,
): CalendarLiveV28RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: CalendarLiveV28RemotePhase,
  captureIndex?: number,
): CalendarLiveV28Request | undefined => {
  const file = calendarLiveV1RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV28Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: CalendarLiveV28RemotePhase,
  captureIndex?: number,
): CalendarLiveV28Receipt | undefined => {
  const file = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV28Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: CalendarLiveV28Request | undefined;
  receipt: CalendarLiveV28Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: CalendarLiveV28RemotePhase,
): CalendarLiveV28ResponseContract => ({
  ...CALENDAR_LIVE_V28_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: CalendarLiveV28Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: CALENDAR_LIVE_V28_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: CALENDAR_LIVE_V28_DYNAMIC_TOOL,
      target: CALENDAR_LIVE_V28_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "calendar-live-v28-plugin-response-v2",
  brokerVersion: CALENDAR_LIVE_V28_BROKER_VERSION,
  target: CALENDAR_LIVE_V28_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*CALENDAR_LIVE_V28_PROGRAM_START*/${program}/*CALENDAR_LIVE_V28_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*CALENDAR_LIVE_V28_PROGRAM_START*/";
  const endMarker = "/*CALENDAR_LIVE_V28_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Calendar live v28 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createCalendarLiveV28Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Calendar live v28 private signing key does not match authorized public-key identity",
    );
  const manifest: CalendarLiveV28Manifest = {
    artifactVersion: "calendar-live-v28-broker-manifest-v2",
    brokerVersion: CALENDAR_LIVE_V28_BROKER_VERSION,
    ...
    captureRequests: CALENDAR_LIVE_V28_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateCalendarLiveV28Request(
  directory: string,
  request: CalendarLiveV28Request,
): void {
  ...
  if (
    request.artifactVersion !== "calendar-live-v28-broker-request-v2" ||
    request.brokerVersion !== CALENDAR_LIVE_V28_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== CALENDAR_LIVE_V28_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Calendar live v28 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueCalendarLiveV28Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: CalendarLiveV28RemotePhase,
  program: string,
  options: CalendarLiveV28IssueOptions = {},
): CalendarLiveV28Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<CalendarLiveV28Request>(targetPath);
    validateCalendarLiveV28Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Calendar live v28 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(calendarLiveV1ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Calendar live v28 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Calendar live v28 empty code envelope refused");
  if (phase === "capture") validateCalendarLiveV28TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Calendar live v28 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: CALENDAR_LIVE_V28_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? CALENDAR_LIVE_V28_SIGNED_WRITER_TIMEOUT_MS
        : CALENDAR_LIVE_V28_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: CalendarLiveV28RequestBody = {
    artifactVersion: "calendar-live-v28-broker-request-v2",
    brokerVersion: CALENDAR_LIVE_V28_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: CalendarLiveV28Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateCalendarLiveV28Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistCalendarLiveV28RawResponse(
  directory: string,
  phase: CalendarLiveV28RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Calendar live v28 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<CalendarLiveV28Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Calendar live v28 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = calendarLiveV1ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Calendar live v28 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): CalendarLiveV28Receipt => {
  const body: Omit<CalendarLiveV28Receipt, "receiptSha256"> = {
    artifactVersion: "calendar-live-v28-broker-receipt-v2",
    brokerVersion: CALENDAR_LIVE_V28_BROKER_VERSION,
...
export function acceptCalendarLiveV28Response<T = unknown>(
  directory: string,
  phase: CalendarLiveV28RemotePhase,
  options: {
    captureIndex?: number;
...
): CalendarLiveV28AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Calendar live v28 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<CalendarLiveV28Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Calendar live v28 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "calendar-live-v28-plugin-response-v2" ||
    plugin?.brokerVersion !== CALENDAR_LIVE_V28_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Calendar live v28 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistCalendarLiveV28TechnicalGates(
  directory: string,
  rootProofs: readonly CalendarLiveV28RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertCalendarLiveV28RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Calendar live v28 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Calendar live v28 probe payload hash malformed");
  const gates = Object.fromEntries(
    CALENDAR_LIVE_V28_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "calendar-live-v28-host-technical-gates-v2",
    brokerVersion: CALENDAR_LIVE_V28_BROKER_VERSION,
...
export function validateCalendarLiveV28TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "calendar-live-v28-host-technical-gates-v2" ||
    ...
    artifact.roots.length !== CALENDAR_LIVE_V28_SOURCE_ROOTS ||
    ...
    CALENDAR_LIVE_V28_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Calendar live v28 technical gates artifact");
}

export function validateCalendarLiveV28Receipt(
  directory: string,
  phase: CalendarLiveV28RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateCalendarLiveV28Request(directory, request);
  const receipt = json<CalendarLiveV28Receipt>(
  ...
  if (
    receipt.artifactVersion !== "calendar-live-v28-broker-receipt-v2" ||
    receipt.brokerVersion !== CALENDAR_LIVE_V28_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Calendar live v28 accepted receipt: ${request.requestId}`,
    );
}

export function readCalendarLiveV28AcceptedRawPayload(
  directory: string,
  phase: CalendarLiveV28RemotePhase,
  captureIndex?: number,
): unknown {
  validateCalendarLiveV28Receipt(directory, phase, captureIndex);
...
export function createCalendarLiveV28FakeDynamicResponse(
  request: CalendarLiveV28Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? CALENDAR_LIVE_V28_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? CALENDAR_LIVE_V28_TARGET,
    ...overrides.extra,
  };
}

export function listCalendarLiveV28PersistedRequests(
  directory: string,
): CalendarLiveV28Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<CalendarLiveV28Request>(file)] : [];
    });
}
