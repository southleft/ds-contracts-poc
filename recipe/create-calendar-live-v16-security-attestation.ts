...
import { CALENDAR_LIVE_V16_TARGET } from "./calendar-live-v16-broker.js";
import {
  CALENDAR_LIVE_V16_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV16Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v16-session";
...
const attestation: CalendarLiveV16SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV16SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v16 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V16_SECURITY_ATTESTATION_DEFAULT_PATH,
);
