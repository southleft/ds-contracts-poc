...
} from "./input-field-live-v3-verifier-v43.js";
import {
  buildInputLiveV43RawPropertyRuntime,
  normalizeInputLiveV43Scene,
  type InputLiveV43RawNode,
} from "./input-field-live-v43-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v43.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v43.js";
...
export const INPUT_LIVE_V43_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V43_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V43_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V43_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V43_VARIANT_COUNT = 256;
export const INPUT_LIVE_V43_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V43_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV43SourceId = (typeof INPUT_LIVE_V43_SOURCE_IDS)[number];

export interface InputLiveV43SourceIdentity {
  source: InputLiveV43SourceId;
...
export interface InputLiveV43WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV43ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV43SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV43RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV43RootProof {
  source: InputLiveV43SourceId;
  adapterIdentity: string;
...
export interface InputLiveV43ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV43SourceId;
    adapterIdentity: string;
...
export interface InputLiveV43CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV43SourceId;
  adapterIdentity: string;
...
export interface InputLiveV43CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV43SourceId;
  strata: InputLiveV43CaptureCell["strata"];
...
export interface InputLiveV43CleanupPayload {
...
export function validateInputLiveV43WriterPayload(
  value: unknown,
): InputLiveV43WriterOwnership {
...
const identityPlan = (source: InputLiveV43SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV43ExtractProgram(
  writer: InputLiveV43WriterOwnership,
  sources: readonly InputLiveV43SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V43_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V43_NAMESPACE)}
${buildInputLiveV43RawPropertyRuntime()}
...
export function validateInputLiveV43ExtractPayload(
  value: unknown,
  writer: InputLiveV43WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V43_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV43ExtractPayload;
}

export function proveInputLiveV43Roots<Instance>(
  extract: InputLiveV43ExtractPayload,
  sources: readonly (InputLiveV43SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV43Scene(
      root.scene,
...
export function assertInputLiveV43RootProofs(
  proofs: readonly InputLiveV43RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV43ProbeProgram(
  writer: InputLiveV43WriterOwnership,
  sources: readonly InputLiveV43SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V43_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV43ProbePayload(
  value: unknown,
  writer: InputLiveV43WriterOwnership,
): InputLiveV43ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV43ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V43_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V43_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV43CaptureManifest(
  cells: readonly InputLiveV43CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V43_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V43_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V43_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V43_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V43_SOURCE_IDS.some(
...
export const inputLiveV43CaptureManifestSha256 = (
  cells: readonly InputLiveV43CaptureCell[],
): string => {
  validateInputLiveV43CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV43CaptureProgram(
  writer: InputLiveV43WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V43_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V43_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V43_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV43CapturePayload(
  value: unknown,
  cell: InputLiveV43CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV43CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V43_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V43_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV43CaptureResponses(
  manifest: readonly InputLiveV43CaptureCell[],
  responses: readonly InputLiveV43CapturePayload[],
): void {
  validateInputLiveV43CaptureManifest(manifest);
  if (
...
export interface InputLiveV43ObjectiveReport {
  artifactVersion: "input-live-v43-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV43Objective(
  manifest: readonly InputLiveV43CaptureCell[],
  responses: readonly InputLiveV43CapturePayload[],
  readReference: (cell: InputLiveV43CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV43CaptureCell) => Buffer,
): InputLiveV43ObjectiveReport {
  assertInputLiveV43CaptureResponses(manifest, responses);
...
export function buildInputLiveV43CleanupProgram(
  writer: InputLiveV43WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V43_NAMESPACE,
...
export function validateInputLiveV43CleanupPayload(
  value: unknown,
  writer: InputLiveV43WriterOwnership,
): InputLiveV43CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV43CleanupPayload;
...
export const INPUT_LIVE_V43_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV43WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
