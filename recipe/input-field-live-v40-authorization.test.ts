...
import { INPUT_LIVE_V40_INDEX_PATH } from "./build-input-field-live-proof-v40.js";
import {
  INPUT_LIVE_V40_DYNAMIC_TOOL,
  INPUT_LIVE_V40_TARGET,
} from "./input-field-live-v40-broker.js";
import {
  buildInputLiveV40AuthorizationArtifact,
  validateInputLiveV40AntecedentIndex,
  validateInputLiveV40AuthorizationArtifact,
  validateInputLiveV40History,
  validateInputLiveV40UnitTestSource,
  type InputLiveV40AntecedentIndex,
  type InputLiveV40AuthorizationArtifact,
  type InputLiveV40AuthorizationProof,
  type InputLiveV40HistoryState,
} from "./input-field-live-v40-authorization.js";
import {
  inputLiveV40AttestationSha256,
  validateInputLiveV40ControlFlowSource,
  validateInputLiveV40SecurityAttestation,
  type InputLiveV40SecurityAttestation,
} from "./input-field-live-v40-preflight.js";
import { inputLiveV40Sha256 } from "./input-field-live-v40-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V40_INDEX_PATH);
const index = (): InputLiveV40AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV40AntecedentIndex;

const artifact = (): InputLiveV40AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV40AuthorizationArtifact({
...
const pendingState = (): InputLiveV40HistoryState => ({
  index: index(),
  indexSha256: inputLiveV40Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV40HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV40AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV40Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V40_TARGET,
  expectedDynamicTool: INPUT_LIVE_V40_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v40/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV40AuthorizationArtifact,
): InputLiveV40SecurityAttestation => {
  const body: Omit<InputLiveV40SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v40-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V40_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV40AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV40History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV40History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV40History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV40History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV40History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV40UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV40UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV40AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV40Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV40AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV40AuthorizationArtifact(
        planted as InputLiveV40AuthorizationArtifact,
        index(),
        inputLiveV40Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV40SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV40AttestationSha256(
      body as Omit<InputLiveV40SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV40SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV40ControlFlowSource(source), []);
});
