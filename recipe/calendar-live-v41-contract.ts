...
import { buildCalendarLiveV41CleanupRuntime } from "./calendar-live-v41-cleanup.js";
import { type CalendarLiveV41FixedFixedPoint } from "./calendar-live-v41-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV41RestoreProgram,
  validateCalendarLiveV41RestorePayload,
  type CalendarLiveV41RestorePayload,
} from "./calendar-live-v41-restore.js";
import {
  buildCalendarLiveV41RawPropertyRuntime,
  normalizeCalendarLiveV41Scene,
  type CalendarLiveV41RawNode,
} from "./calendar-live-v41-verifier.js";
...
export const CALENDAR_LIVE_V41_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V41_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V41_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V41_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V41_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V41_SET_COUNT = 3;
export const CALENDAR_LIVE_V41_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V41_HOST_PHASES = 3;
export const CALENDAR_LIVE_V41_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V41_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V41_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V41_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV41SourceId = (typeof CALENDAR_LIVE_V41_SOURCE_IDS)[number];
export type CalendarLiveV41Kind = "calendar" | "week" | "day";

export interface CalendarLiveV41SourceIdentity {
  source: CalendarLiveV41SourceId;
...
export interface CalendarLiveV41WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV41ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV41SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV41RawNode;
    weekScene: CalendarLiveV41RawNode;
    dayScene: CalendarLiveV41RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV41RootProof {
  source: CalendarLiveV41SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV41FixedFixedPoint;
}

export interface CalendarLiveV41ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV41SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV41CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV41SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV41CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV41SourceId;
  frameWidth: number;
...
export interface CalendarLiveV41CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v41 ${label} must be an object`);
...
export function validateCalendarLiveV41WriterPayload(
  value: unknown,
): CalendarLiveV41WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v41") ||
    ...
    result.namespace !== CALENDAR_LIVE_V41_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v41 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v41 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v41 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV41SourceIdentity,
  kind: CalendarLiveV41Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV41ExtractProgram(
  writer: CalendarLiveV41WriterOwnership,
  sources: readonly CalendarLiveV41SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v41 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V41_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V41_NAMESPACE)}
${buildCalendarLiveV41RawPropertyRuntime()}
...
export function validateCalendarLiveV41ExtractPayload(
  value: unknown,
  writer: CalendarLiveV41WriterOwnership,
): CalendarLiveV41ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v41 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V41_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v41 extract schema/one-root mismatch");
  return payload as CalendarLiveV41ExtractPayload;
}

export function proveCalendarLiveV41Roots<Instance>(
  extract: CalendarLiveV41ExtractPayload,
  sources: readonly (CalendarLiveV41SourceIdentity & {
...
): CalendarLiveV41RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v41 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v41 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV41Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV41Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV41Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV41FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v41 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v41 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v41 compile lost owned sets");
...
export function assertCalendarLiveV41RootProofs(
  proofs: readonly CalendarLiveV41RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v41 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v41 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV41ProbeProgram(
  writer: CalendarLiveV41WriterOwnership,
  sources: readonly CalendarLiveV41SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V41_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV41ProbePayload(
  value: unknown,
  writer: CalendarLiveV41WriterOwnership,
): CalendarLiveV41ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV41ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V41_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V41_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v41 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV41CaptureManifest(
  cells: readonly CalendarLiveV41CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V41_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V41_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V41_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V41_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v41 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV41CaptureCell[],
): string => {
  validateCalendarLiveV41CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV41CaptureProgram(
  writer: CalendarLiveV41WriterOwnership,
  cell: CalendarLiveV41CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V41_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v41 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v41 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V41_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV41CapturePayload(
  value: unknown,
  cell: CalendarLiveV41CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV41CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV41CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V41_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V41_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v41 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV41CaptureResponses(
  manifest: readonly CalendarLiveV41CaptureCell[],
  responses: readonly CalendarLiveV41CapturePayload[],
): void {
  validateCalendarLiveV41CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v41 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV41ObjectiveReport {
  artifactVersion: "calendar-live-v41-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV41Objective(
  manifest: readonly CalendarLiveV41CaptureCell[],
  responses: readonly CalendarLiveV41CapturePayload[],
): CalendarLiveV41ObjectiveReport {
  assertCalendarLiveV41CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v41-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV41CleanupProgram(
  writer: CalendarLiveV41WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV41CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V41_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v41 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV41CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV41WriterOwnership,
): CalendarLiveV41CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV41CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v41 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V41_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV41WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
