...
import { CALENDAR_LIVE_V30_INDEX_PATH } from "./build-calendar-live-proof-v30.js";
import {
...
import {
  buildCalendarLiveV30AuthorizationArtifact,
  validateCalendarLiveV30AntecedentIndex,
  validateCalendarLiveV30AuthorizationArtifact,
  validateCalendarLiveV30History,
  validateCalendarLiveV30UnitTestSource,
  type CalendarLiveV30AntecedentIndex,
  type CalendarLiveV30AuthorizationArtifact,
  type CalendarLiveV30AuthorizationProof,
  type CalendarLiveV30HistoryState,
} from "./calendar-live-v30-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV30ControlFlowSource,
  validateCalendarLiveV30SecurityAttestation,
  type CalendarLiveV30SecurityAttestation,
} from "./calendar-live-v30-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V30_INDEX_PATH);
const index = (): CalendarLiveV30AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV30AntecedentIndex;

const artifact = (): CalendarLiveV30AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV30AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV30HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV30HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV30AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v30/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV30AuthorizationArtifact,
): CalendarLiveV30SecurityAttestation => {
  const body: Omit<CalendarLiveV30SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V30_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV30History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV30History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV30History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV30History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV30History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV30UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v30-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV30UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV30AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV30AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV30AuthorizationArtifact(
        planted as CalendarLiveV30AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV30SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV30SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV30SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v30-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV30ControlFlowSource(source), []);
});
