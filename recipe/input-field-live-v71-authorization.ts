...
export type InputLiveV71HistoryExpectation = "pending" | "authorized";

export interface InputLiveV71AuthorizationArtifact {
  artifactVersion: "input-live-v71-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V71_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV71AntecedentIndex {
...
export interface InputLiveV71HistoryState {
  ...
  authorization?: InputLiveV71AuthorizationArtifact;
...
export interface InputLiveV71AuthorizationProof extends InputLiveV71TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V71_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V71_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V71_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V71_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v71-authorization") ||
  artifactPath.includes("input-field-live-v71-preflight") ||
  artifactPath.includes("input-field-live-v71-authorized") ||
  artifactPath.includes("input-field-live-pivot-v71-status") ||
...
export function validateInputLiveV71AntecedentIndex(
  index: InputLiveV71AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v71-antecedent-index-v1" ||
...
export function validateInputLiveV71UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV71History\(\s*["']/.test(source) ||
    /readInputLiveV71HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV71AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV71AuthorizationArtifact {
  ...
  const indexFailures = validateInputLiveV71AntecedentIndex(index);
...
export function validateInputLiveV71AuthorizationArtifact(
  artifact: InputLiveV71AuthorizationArtifact | undefined,
  index: InputLiveV71AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v71-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v71" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V71_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V71_TARGET) ||
    !equalJson(
...
export function validateInputLiveV71History(
  state: InputLiveV71HistoryState,
  expected: InputLiveV71HistoryExpectation,
): string[] {
  const failures = validateInputLiveV71AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV71AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV71HistoryState(
  root = process.cwd(),
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V71_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV71AuthorizationArtifact)
    : undefined;
  ...
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V71_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V71_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV71History(
  expected: InputLiveV71HistoryExpectation,
  root = process.cwd(),
): InputLiveV71HistoryState {
  const state = readInputLiveV71HistoryState(root);
  const failures = validateInputLiveV71History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV71Authorization(
  root = process.cwd(),
): InputLiveV71AuthorizationProof {
  const state = verifyInputLiveV71History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V71_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v71/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v71.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V71_TARGET,
    expectedDynamicTool: INPUT_LIVE_V71_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V71_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV71HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV71History(expected);
