...
import { INPUT_LIVE_V28_INDEX_PATH } from "./build-input-field-live-proof-v28.js";
import {
  INPUT_LIVE_V28_DYNAMIC_TOOL,
  INPUT_LIVE_V28_TARGET,
} from "./input-field-live-v28-broker.js";
import {
  buildInputLiveV28AuthorizationArtifact,
  validateInputLiveV28AntecedentIndex,
  validateInputLiveV28AuthorizationArtifact,
  validateInputLiveV28History,
  validateInputLiveV28UnitTestSource,
  type InputLiveV28AntecedentIndex,
  type InputLiveV28AuthorizationArtifact,
  type InputLiveV28AuthorizationProof,
  type InputLiveV28HistoryState,
} from "./input-field-live-v28-authorization.js";
import {
  inputLiveV28AttestationSha256,
  validateInputLiveV28ControlFlowSource,
  validateInputLiveV28SecurityAttestation,
  type InputLiveV28SecurityAttestation,
} from "./input-field-live-v28-preflight.js";
import { inputLiveV28Sha256 } from "./input-field-live-v28-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V28_INDEX_PATH);
const index = (): InputLiveV28AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV28AntecedentIndex;

const artifact = (): InputLiveV28AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV28AuthorizationArtifact({
...
const pendingState = (): InputLiveV28HistoryState => ({
  index: index(),
  indexSha256: inputLiveV28Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV28HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV28AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV28Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V28_TARGET,
  expectedDynamicTool: INPUT_LIVE_V28_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v28/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV28AuthorizationArtifact,
): InputLiveV28SecurityAttestation => {
  const body: Omit<InputLiveV28SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v28-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V28_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV28AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV28History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV28History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV28History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV28History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV28History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV28UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV28UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV28AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV28Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV28AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV28AuthorizationArtifact(
        planted as InputLiveV28AuthorizationArtifact,
        index(),
        inputLiveV28Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV28SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV28AttestationSha256(
      body as Omit<InputLiveV28SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV28SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV28ControlFlowSource(source), []);
});
