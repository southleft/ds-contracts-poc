...
import {
  acceptCalendarLiveV40Response,
  createCalendarLiveV40Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV40Request,
  persistCalendarLiveV40RawResponse,
  persistCalendarLiveV40TechnicalGates,
  readCalendarLiveV40AcceptedRawPayload,
  type CalendarLiveV40Request,
  type CalendarLiveV40TransactionAuthorization,
} from "./calendar-live-v40-broker.js";
import {
  CALENDAR_LIVE_V40_CAPTURE_COUNT,
  assertCalendarLiveV40CaptureResponses,
  ...
  evaluateCalendarLiveV40Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV40Roots,
  validateCalendarLiveV40CaptureManifest,
  validateCalendarLiveV40CapturePayload,
  validateCalendarLiveV40CleanupPayload,
  validateCalendarLiveV40ExtractPayload,
  validateCalendarLiveV40ProbePayload,
  validateCalendarLiveV40WriterPayload,
  type CalendarLiveV40CaptureCell,
  type CalendarLiveV40CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V40_PLAN_PATH,
  type CalendarLiveV40ProofPlan,
} from "./build-calendar-live-proof-v40.js";
import {
...
export const CALENDAR_LIVE_V40_RUNNER_VERSION = "calendar-live-v40-runner-v1";

interface CalendarLiveV40JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV40OrchestratorState {
  artifactVersion: "calendar-live-v40-orchestrator-state-v1";
  ...
  journal: CalendarLiveV40JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV40ProofPlan;
  planSha256: string;
  captures: CalendarLiveV40CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV40SourceIdentity & {
...
export interface CalendarLiveV40NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV40OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v40 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V40_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V40_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV40ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV40CaptureCell[];
  validateCalendarLiveV40CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v40 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v40 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV40WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV40RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV40Request =>
  readJson<CalendarLiveV40Request>(file);

export class CalendarLiveV40Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV40Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV40OrchestratorState = {
      artifactVersion: "calendar-live-v40-orchestrator-state-v1",
    ...
    return new CalendarLiveV40Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV40OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV40Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV40NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV40NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v40 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV40NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v40 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV40RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV40Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV40CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV40AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV40WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV40Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV40ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV40Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV40Response(this.#directory, "writer", {
        validate: validateCalendarLiveV40WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV40Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV40RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV40Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV40ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV40Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV40RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v40-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV40Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV40ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV40Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV40ProbePayload(payload, ownership),
      });
      persistCalendarLiveV40TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v40 capture cell missing");
      const accepted = acceptCalendarLiveV40Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV40CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V40_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV40CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV40Objective(
    ...
    throw new Error(`Calendar live v40 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV40Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV40CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV40Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV40RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV40Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV40CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV40CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV40CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV40Authorization = (
  privateKey: KeyObject,
): CalendarLiveV40TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV40SimulationKey = (): KeyObject =>
