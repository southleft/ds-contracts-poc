...
import { TABLE_LIVE_V4_INDEX_PATH } from "./build-table-live-proof-v4.js";
import {
...
import {
  buildTableLiveV4AuthorizationArtifact,
  validateTableLiveV4AntecedentIndex,
  validateTableLiveV4AuthorizationArtifact,
  validateTableLiveV4History,
  validateTableLiveV4UnitTestSource,
  type TableLiveV4AntecedentIndex,
  type TableLiveV4AuthorizationArtifact,
  type TableLiveV4AuthorizationProof,
  type TableLiveV4HistoryState,
} from "./table-live-v4-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV4ControlFlowSource,
  validateTableLiveV4SecurityAttestation,
  type TableLiveV4SecurityAttestation,
} from "./table-live-v4-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V4_INDEX_PATH);
const index = (): TableLiveV4AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV4AntecedentIndex;

const artifact = (): TableLiveV4AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV4AuthorizationArtifact({
...
const pendingState = (): TableLiveV4HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV4HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV4AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v4/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV4AuthorizationArtifact,
): TableLiveV4SecurityAttestation => {
  const body: Omit<TableLiveV4SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V4_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV4History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV4History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV4History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV4History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV4History(authorizedState(), "pending").join("\n"),
    /stale history mode.*authorization exists/,
  );
});

test("the exact v6 phase-sensitive self-test defect is planted and table v4 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV4UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v4-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV4UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV4AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV4AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV4AuthorizationArtifact(
        planted as TableLiveV4AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV4SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV4SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV4SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v4-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV4ControlFlowSource(source), []);
});
