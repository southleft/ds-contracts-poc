...
import {
  ...
  createInputLiveV7FakeDynamicResponse,
  ...
  listInputLiveV7PersistedRequests,
  persistInputLiveV7RawResponse,
  persistInputLiveV7TechnicalGates,
  validateInputLiveV7Receipt,
  validateInputLiveV7Request,
  type InputLiveV7RemotePhase,
...
import {
  ...
  type InputLiveV7ExtractPayload,
  type InputLiveV7ProbePayload,
  type InputLiveV7RootProof,
  type InputLiveV7WriterOwnership,
} from "./input-field-live-v7-contract.js";
import {
  InputLiveV7Orchestrator,
  simulatedInputLiveV7Authorization,
} from "./run-input-field-live-v7.js";
...
const captures: InputLiveV7CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV7Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v7-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV7Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV7CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV7FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV7FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V7_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V7_TARGET);
  persistInputLiveV7RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV7Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV7Receipt(directory, phase, options.captureIndex);
...
const extractPayload = (
  ownership: InputLiveV7WriterOwnership,
...
const rootProofs = (): InputLiveV7RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v7-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV7Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV7Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV7RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) => validateInputLiveV7ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      probePayload(writer),
      {
        validate: (payload) => validateInputLiveV7ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV7CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV7CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV7CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV7Response(directory, "cleanup", {
      validate: (payload) => validateInputLiveV7CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV7PersistedRequests(directory).length, 132);
    assert.ok(inputLiveV7ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV7Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV7Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV7Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV7Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V7_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV7RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V7_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV7Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV7Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV7ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV7Receipt(third.directory, "writer"),
      /invalid Input live v7 accepted receipt/,
    );
  } finally {
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV7WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV7RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV7Request;
    validateInputLiveV7Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV7Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV7CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active extract and surfaced cleanup", () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v7-state-"));
  ...
  try {
    const orchestrator = InputLiveV7Orchestrator.initialize({
      ...
      privateKey,
      authorization: simulatedInputLiveV7Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV7Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
const requestFrom = (file: string): InputLiveV7Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV7Request;
