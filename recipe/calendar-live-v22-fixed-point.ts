/**
 * Calendar live v22 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V22_FIXED_VERIFIER_VERSION = "calendar-live-v22-fixed-point-v1";
export const CALENDAR_LIVE_V22_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV22FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV22FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV22FixedSourceProbe {
...
export interface CalendarLiveV22FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV22FixedFixedPoint;
}

export interface CalendarLiveV22FixedHardGateInput {
  thresholds: CalendarLiveV22FixedThresholds;
  sourceProbes: CalendarLiveV22FixedSourceProbe[];
  cells: CalendarLiveV22FixedCellValidation[];
  sceneProofs: CalendarLiveV22FixedSceneProof[];
...
export interface CalendarLiveV22FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V22_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV22FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV22FixedHardGates(
  input: CalendarLiveV22FixedHardGateInput,
): CalendarLiveV22FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV22FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V22_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV22FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV22FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV22FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
