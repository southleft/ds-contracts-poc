...
import {
  TABLE_LIVE_V7_DYNAMIC_TOOL,
  TABLE_LIVE_V7_TARGET,
  type TableLiveV7TransactionAuthorization,
} from "./table-live-v7-broker.js";
import {
...
import {
  ...
  TABLE_LIVE_V7_HOST_PHASES,
  TABLE_LIVE_V7_REMOTE_REQUESTS,
  TABLE_LIVE_V7_SOURCE_ROOTS,
} from "./table-live-v7-contract.js";
import { canonicalJson } from "./normalize.js";

export type TableLiveV7HistoryExpectation = "pending" | "authorized";

export interface TableLiveV7AuthorizationArtifact {
  artifactVersion: "table-live-v7-capture-authorization-v1";
  authorizationId: "table-live-v7";
  status: "authorization declared; runtime security prerequisites still mandatory";
  authorizationIntent: true;
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V7_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV7AntecedentIndex {
  artifactVersion: "table-live-v7-antecedent-index-v1";
  status: string;
...
export interface TableLiveV7HistoryState {
  ...
  authorization?: TableLiveV7AuthorizationArtifact;
...
export interface TableLiveV7AuthorizationProof
  extends TableLiveV7TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V7_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V7_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V7_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V7_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v7-authorization") ||
  artifactPath.includes("table-live-v7-preflight") ||
  artifactPath.includes("table-live-v7-authorized") ||
  artifactPath.includes("create-table-live-v7-security-attestation") ||
  artifactPath.includes("table-live-pivot-v7-status") ||
...
export function validateTableLiveV7AntecedentIndex(
  index: TableLiveV7AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v7-antecedent-index-v1" ||
    index.authorizationCanBeAddedWithoutAntecedentRebuild !== true
  )
    failures.push("table v7 antecedent index missing or malformed");
  const artifactEntries = Object.entries(index?.artifacts ?? {});
  if (artifactEntries.length === 0)
    failures.push("table v7 antecedent hash denominator is empty");
  for (const [artifactPath, metadata] of artifactEntries) {
    ...
    if (
      ...
      failures.push(`malformed table v7 antecedent hash: ${artifactPath}`);
  }
  ...
  if (index?.hashSetSha256 !== expectedHashSet)
    failures.push("table v7 antecedent hash-set digest mismatch");
  if (
    ...
    failures.push("table v7 antecedent counts missing or drifted");
  for (const required of [
    TABLE_LIVE_V7_AUTHORIZATION_PATH,
    "recipe/table-live-v7-authorization.ts",
    "recipe/table-live-v7-authorization.test.ts",
    "recipe/table-live-v7-preflight.ts",
    "recipe/evidence/table-live-pivot-v7/operator-security-attestation-template.json",
    "recipe/evidence/status-index.json",
  ])
    if (!index?.authorizationLifecycleExcluded?.includes(required))
      failures.push(`table v7 lifecycle exclusion missing: ${required}`);
  const leaks = forbiddenAntecedentFieldPaths(index);
  if (leaks.length)
    failures.push(`table v7 antecedent result leakage: ${leaks.join(",")}`);
  return failures;
}

export function validateTableLiveV7UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV7History\(\s*["']/.test(source) ||
    /readTableLiveV7HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV7AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV7AuthorizationArtifact {
  if (!SHA40.test(options.antecedentCommit))
    throw new TypeError("table v7 antecedent commit must be a full SHA");
  const index = JSON.parse(
    Buffer.from(options.antecedentIndexBytes).toString("utf8"),
  ) as TableLiveV7AntecedentIndex;
  const indexFailures = validateTableLiveV7AntecedentIndex(index);
  if (indexFailures.length)
    throw new TypeError(
      `table v7 antecedent index refused:\n${indexFailures.join("\n")}`,
    );
  const publicKey =
  ...
  return {
    ...
    antecedent: {
      ...
      indexPath: TABLE_LIVE_V7_INDEX_PATH,
    ...
    denominator: {
      remoteRequests: TABLE_LIVE_V7_REMOTE_REQUESTS,
      hostPhases: TABLE_LIVE_V7_HOST_PHASES,
      captures: TABLE_LIVE_V7_CAPTURE_COUNT,
      sourceRoots: TABLE_LIVE_V7_SOURCE_ROOTS,
...
export function validateTableLiveV7AuthorizationArtifact(
  artifact: TableLiveV7AuthorizationArtifact | undefined,
  index: TableLiveV7AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v7-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v7" ||
    artifact.authorizationIntent !== true
  )
    failures.push("table v7 authorization declaration missing or malformed");
  const resultLeaks = forbiddenAntecedentFieldPaths(artifact);
  if (resultLeaks.length)
    failures.push(`table v7 authorization result leakage: ${resultLeaks.join(",")}`);
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V7_INDEX_PATH ||
    artifact?.antecedent?.indexSha256 !== indexSha256 ||
    artifact?.antecedent?.hashSetSha256 !== index?.hashSetSha256
  )
    failures.push("table v7 authorization pins wrong antecedent index");
  if (privateMaterialPaths(artifact).length)
    failures.push("table v7 authorization contains private key or token material");
  try {
    ...
    if (
      ...
      failures.push("table v7 Ed25519 public-key identity mismatch");
  } catch {
    failures.push("table v7 Ed25519 public key is invalid");
  }
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V7_TARGET) ||
    !equalJson(
    ...
    failures.push("wrong table v7 target or dynamic tool");
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V7_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V7_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V7_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V7_SOURCE_ROOTS ||
    artifact?.denominator?.expectedFacts !== index?.counts.expectedSceneFacts
  )
    failures.push("table v7 two-root or capture denominator changed");
  if (
    ...
    failures.push(
      "table v7 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
    ...
    failures.push("table v7 security prerequisites weakened");
  if (
    artifact?.humanSignoff?.mandatory !== true ||
    artifact?.humanSignoff?.status !== "pending"
  )
    failures.push("table v7 human signoff overclaim");
  return failures;
}

export function validateTableLiveV7History(
  state: TableLiveV7HistoryState,
  expected: TableLiveV7HistoryExpectation,
): string[] {
  const failures = validateTableLiveV7AntecedentIndex(state.index);
  if (!SHA40.test(state.codeCommit))
    failures.push("table v7 code commit is not a full SHA");
  ...
  if (
    ...
    failures.push("table v7 antecedent first-add commit is not unique");
  if (!state.antecedentIsAncestorOfCode)
    failures.push("code commit does not descend from table v7 antecedent");
  if (!state.antecedentIndexBytesMatchFirstAddition)
    failures.push("table v7 antecedent index changed after first addition");
  if (!state.antecedentArtifactsMatchIndexAtCommit)
    failures.push("table v7 antecedent commit artifacts do not match index");
  if (!state.workingAntecedentArtifactsMatchIndex)
    failures.push("table v7 antecedent working bytes drifted");
  ...
  failures.push(
    ...validateTableLiveV7AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
  ...
  if (
    ...
    failures.push("table v7 authorization first-add lineage mismatch");
  if (!state.authorizationPresentAtCodeCommit)
    failures.push("table v7 authorization missing from code commit");
  if (!state.authorizationBytesMatchFirstAddition)
    failures.push("table v7 authorization bytes changed after first addition");
  if (!state.authorizationStrictlyDescendsFromAntecedent)
    failures.push("table v7 authorization does not strictly descend from antecedent");
  if (!state.authorizationIsAncestorOfCode)
    failures.push("code commit predates table v7 authorization");
  return failures;
}
...
export function readTableLiveV7HistoryState(
  root = process.cwd(),
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V7_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV7AuthorizationArtifact)
    : undefined;
  ...
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V7_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V7_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV7History(
  expected: TableLiveV7HistoryExpectation,
  root = process.cwd(),
): TableLiveV7HistoryState {
  const state = readTableLiveV7HistoryState(root);
  const failures = validateTableLiveV7History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v7 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV7Authorization(
  root = process.cwd(),
): TableLiveV7AuthorizationProof {
  const state = verifyTableLiveV7History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V7_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v7/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v7.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V7_TARGET,
    expectedDynamicTool: TABLE_LIVE_V7_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V7_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV7HistoryExpectation => {
  ...
  if (pending === authorized)
    throw new Error(
      "Choose exactly one table v7 history mode: --expect-pending or --expect-authorized",
    );
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV7History(expected);
  process.stdout.write(
    `Table live v7 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
