...
import { CALENDAR_LIVE_V2_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v2.js";
import { verifyCalendarLiveV2Authorization } from "./calendar-live-v2-authorization.js";
import { runCalendarLiveV2Preflight } from "./calendar-live-v2-preflight.js";
import { CalendarLiveV2Orchestrator } from "./run-calendar-live-v2.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v2 requires exact authorization artifact ${CALENDAR_LIVE_V2_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v2-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV2Authorization(root);
    runCalendarLiveV2Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV2Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV2Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
