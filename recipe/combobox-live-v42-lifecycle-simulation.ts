...
export interface ComboboxLiveV42LifecycleSimulationReport {
  artifactVersion: "combobox-live-v42-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV42AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV42Lifecycle(): Promise<ComboboxLiveV42LifecycleSimulationReport> {
  await buildComboboxLiveV42Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V42_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV42AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v42-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V42_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V42_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV42History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV42History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV42AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V42_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V42_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV42History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV42History(authorized, "pending");
    ...
    await buildComboboxLiveV42Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V42_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV42AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV42Lifecycle(), null, 2)}\n`,
  );
