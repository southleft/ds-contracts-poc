...
import { CALENDAR_LIVE_V37_INDEX_PATH } from "./build-calendar-live-proof-v37.js";
import {
...
import {
  buildCalendarLiveV37AuthorizationArtifact,
  validateCalendarLiveV37AntecedentIndex,
  validateCalendarLiveV37AuthorizationArtifact,
  validateCalendarLiveV37History,
  validateCalendarLiveV37UnitTestSource,
  type CalendarLiveV37AntecedentIndex,
  type CalendarLiveV37AuthorizationArtifact,
  type CalendarLiveV37AuthorizationProof,
  type CalendarLiveV37HistoryState,
} from "./calendar-live-v37-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV37ControlFlowSource,
  validateCalendarLiveV37SecurityAttestation,
  type CalendarLiveV37SecurityAttestation,
} from "./calendar-live-v37-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V37_INDEX_PATH);
const index = (): CalendarLiveV37AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV37AntecedentIndex;

const artifact = (): CalendarLiveV37AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV37AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV37HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV37HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV37AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v37/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV37AuthorizationArtifact,
): CalendarLiveV37SecurityAttestation => {
  const body: Omit<CalendarLiveV37SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V37_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV37History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV37History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV37History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV37History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV37History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV37UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v37-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV37UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV37AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV37AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV37AuthorizationArtifact(
        planted as CalendarLiveV37AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV37SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV37SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV37SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v37-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV37ControlFlowSource(source), []);
});
