...
import {
  validateTableLiveV29AntecedentIndex,
  verifyTableLiveV29Authorization,
  type TableLiveV29AntecedentIndex,
  type TableLiveV29AuthorizationProof,
} from "./table-live-v29-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V29_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v29/operator-security-attestation-template.json";
export const TABLE_LIVE_V29_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v29-security-attestation.json";

export interface TableLiveV29SecurityAttestation {
  artifactVersion: "table-live-v29-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV29PreflightReport {
  artifactVersion: "table-live-v29-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV29SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV29SecurityAttestation(
  value: unknown,
  proof: TableLiveV29AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v29-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V29_TARGET) ||
...
export function validateTableLiveV29ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV29Authorization");
  const preflight = source.indexOf("runTableLiveV29Preflight");
  const initialize = source.indexOf("TableLiveV29Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV29Preflight(
  root: string,
  proof: TableLiveV29AuthorizationProof,
  attestationPath = TABLE_LIVE_V29_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV29PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V29_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V29_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV29AntecedentIndex;
  failures.push(...validateTableLiveV29AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V29_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V29_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV29SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v29-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV29ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v29 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV29SecurityAttestation;
  return {
    artifactVersion: "table-live-v29-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V29_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV29Preflight(
    process.cwd(),
    verifyTableLiveV29Authorization(),
    attestationPath,
