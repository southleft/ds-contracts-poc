...
import {
  ...
  verifyInputLiveV71Authorization,
  type InputLiveV71AntecedentIndex,
  type InputLiveV71AuthorizationProof,
} from "./input-field-live-v71-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V71_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v71/operator-security-attestation-template.json";
export const INPUT_LIVE_V71_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v71-security-attestation.json";

export interface InputLiveV71SecurityAttestation {
...
export interface InputLiveV71PreflightReport {
  artifactVersion: "input-live-v71-preflight-v1";
...
export const inputLiveV71AttestationSha256 = (
  value: Omit<InputLiveV71SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV71SecurityAttestation(
  value: unknown,
  proof: InputLiveV71AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v71-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V71_TARGET) ||
...
export function validateInputLiveV71ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV71Authorization");
  const preflight = source.indexOf("runInputLiveV71Preflight");
  const initialize = source.indexOf("InputLiveV71Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV71Preflight(
  root: string,
  proof: InputLiveV71AuthorizationProof,
  attestationPath = INPUT_LIVE_V71_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV71PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V71_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V71_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V71_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV71AntecedentIndex;
  failures.push(...validateInputLiveV71AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V71_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V71_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v71-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV71ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV71SecurityAttestation;
  return {
    artifactVersion: "input-live-v71-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V71_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV71Preflight(
    process.cwd(),
    verifyInputLiveV71Authorization(),
    attestationPath,
