...
import {
  ...
  INPUT_LIVE_V55_INDEX_PATH,
} from "./build-input-field-live-proof-v55.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV55HistoryExpectation = "pending" | "authorized";

export interface InputLiveV55AuthorizationArtifact {
  artifactVersion: "input-live-v55-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V55_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV55AntecedentIndex {
...
export interface InputLiveV55HistoryState {
  index?: InputLiveV55AntecedentIndex;
  indexSha256: string;
...
export interface InputLiveV55AuthorizationProof extends InputLiveV55TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V55_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V55_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V55_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V55_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v55-authorization") ||
  artifactPath.includes("input-field-live-v55-preflight") ||
  artifactPath.includes("input-field-live-v55-authorized") ||
  artifactPath.includes("input-field-live-pivot-v55-status") ||
...
export function validateInputLiveV55AntecedentIndex(
  index: InputLiveV55AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v55-antecedent-index-v1" ||
...
export function validateInputLiveV55UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV55History\(\s*["']/.test(source) ||
    /readInputLiveV55HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV55AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV55AuthorizationArtifact {
  ...
  ) as InputLiveV55AntecedentIndex;
  const indexFailures = validateInputLiveV55AntecedentIndex(index);
...
export function validateInputLiveV55AuthorizationArtifact(
  artifact: InputLiveV55AuthorizationArtifact | undefined,
  index: InputLiveV55AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v55-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v55" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V55_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V55_TARGET) ||
    !equalJson(
...
export function validateInputLiveV55History(
  state: InputLiveV55HistoryState,
  expected: InputLiveV55HistoryExpectation,
): string[] {
  const failures = validateInputLiveV55AntecedentIndex(state.index);
...
export function readInputLiveV55HistoryState(
  root = process.cwd(),
): InputLiveV55HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V55_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV55AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V55_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V55_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V55_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV55History(
  expected: InputLiveV55HistoryExpectation,
  root = process.cwd(),
): InputLiveV55HistoryState {
  const state = readInputLiveV55HistoryState(root);
  const failures = validateInputLiveV55History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV55Authorization(
  root = process.cwd(),
): InputLiveV55AuthorizationProof {
  const state = verifyInputLiveV55History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V55_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v55/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v55.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V55_TARGET,
    expectedDynamicTool: INPUT_LIVE_V55_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V55_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV55HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV55History(expected);
