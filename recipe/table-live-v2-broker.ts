...
import {
  assertTableLiveV2RootProofs,
  TABLE_LIVE_V2_CAPTURE_COUNT,
  TABLE_LIVE_V2_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  TABLE_LIVE_V2_RESPONSE_CONTRACTS,
  type TableLiveV2RootProof,
} from "./table-live-v2-contract.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V2_BROKER_VERSION = "table-live-v2-broker-v1";
export const TABLE_LIVE_V2_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const TABLE_LIVE_V2_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const TABLE_LIVE_V2_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const TABLE_LIVE_V2_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const TABLE_LIVE_V2_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const TABLE_LIVE_V2_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type TableLiveV2RemotePhase =
  (typeof TABLE_LIVE_V2_REMOTE_PHASES)[number];
export type TableLiveV2AuthorizationMode = "live" | "simulated";

export interface TableLiveV2CommitPins {
...
export interface TableLiveV2TransactionAuthorization extends TableLiveV2CommitPins {
  mode: TableLiveV2AuthorizationMode;
...
export interface TableLiveV2Manifest {
  artifactVersion: "table-live-v2-broker-manifest-v2";
  brokerVersion: typeof TABLE_LIVE_V2_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof TABLE_LIVE_V2_DYNAMIC_TOOL;
  target: typeof TABLE_LIVE_V2_TARGET;
  authorization: TableLiveV2TransactionAuthorization;
...
export interface TableLiveV2ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface TableLiveV2RequestBody {
  artifactVersion: "table-live-v2-broker-request-v2";
  brokerVersion: typeof TABLE_LIVE_V2_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: TableLiveV2RemotePhase;
  sequence: number;
  ...
  commitPins: TableLiveV2CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof TABLE_LIVE_V2_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: TableLiveV2ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface TableLiveV2Request extends TableLiveV2RequestBody {
...
export interface TableLiveV2Receipt {
  artifactVersion: "table-live-v2-broker-receipt-v2";
  brokerVersion: typeof TABLE_LIVE_V2_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: TableLiveV2RemotePhase;
...
export interface TableLiveV2AcceptedResponse<T = unknown> {
  request: TableLiveV2Request;
  receipt: TableLiveV2Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface TableLiveV2IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: TableLiveV2ResponseContract;
...
const equalTarget = (value: unknown): value is typeof TABLE_LIVE_V2_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === TABLE_LIVE_V2_TARGET.fileKey &&
  (value as BrokerTarget).fileName === TABLE_LIVE_V2_TARGET.fileName &&
  (value as BrokerTarget).editorType === TABLE_LIVE_V2_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== TABLE_LIVE_V2_TARGET.fileKey ||
    context.fileName !== TABLE_LIVE_V2_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === TABLE_LIVE_V2_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Table live v2 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= TABLE_LIVE_V2_CAPTURE_COUNT
  )
    throw new TypeError("Table live v2 capture index must be 0..19");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Table live v2 authorization pins are malformed");
};

const readManifest = (directory: string): TableLiveV2Manifest => {
  const manifest = json<TableLiveV2Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Table live v2 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "table-live-v2-broker-manifest-v2" ||
    manifest.brokerVersion !== TABLE_LIVE_V2_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== TABLE_LIVE_V2_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V2_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Table live v2 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: TableLiveV2Request,
): TableLiveV2RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: TableLiveV2RemotePhase,
  captureIndex?: number,
): TableLiveV2Request | undefined => {
  const file = tableLiveV2RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV2Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: TableLiveV2RemotePhase,
  captureIndex?: number,
): TableLiveV2Receipt | undefined => {
  const file = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV2Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: TableLiveV2Request | undefined;
  receipt: TableLiveV2Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: TableLiveV2RemotePhase,
): TableLiveV2ResponseContract => ({
  ...TABLE_LIVE_V2_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: TABLE_LIVE_V2_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: TABLE_LIVE_V2_DYNAMIC_TOOL,
      target: TABLE_LIVE_V2_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "table-live-v2-plugin-response-v2",
  brokerVersion: TABLE_LIVE_V2_BROKER_VERSION,
  target: TABLE_LIVE_V2_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*TABLE_LIVE_V2_PROGRAM_START*/${program}/*TABLE_LIVE_V2_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*TABLE_LIVE_V2_PROGRAM_START*/";
  const endMarker = "/*TABLE_LIVE_V2_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Table live v2 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createTableLiveV2Transaction(
  directory: string,
  ...
  if (existsSync(path.join(directory, MANIFEST_FILE)))
    throw new Error("Table live v2 transaction already exists");
  validateAuthorization(options.authorization);
  if (
    !SHA256.test(options.proofPlanSha256) ||
    !SHA256.test(options.captureManifestSha256)
  )
    throw new TypeError("Table live v2 transaction plan hashes malformed");
  const attempt = options.attempt ?? 1;
  if (!Number.isInteger(attempt) || attempt < 1 || attempt > 3)
    throw new TypeError("Table live v2 attempt must be 1..3");
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Table live v2 private signing key does not match authorized public-key identity",
    );
  const manifest: TableLiveV2Manifest = {
    artifactVersion: "table-live-v2-broker-manifest-v2",
    brokerVersion: TABLE_LIVE_V2_BROKER_VERSION,
    ...
    captureRequests: TABLE_LIVE_V2_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateTableLiveV2Request(
  directory: string,
  request: TableLiveV2Request,
): void {
  ...
  if (
    request.artifactVersion !== "table-live-v2-broker-request-v2" ||
    request.brokerVersion !== TABLE_LIVE_V2_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== TABLE_LIVE_V2_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Table live v2 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueTableLiveV2Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: TableLiveV2RemotePhase,
  program: string,
  options: TableLiveV2IssueOptions = {},
): TableLiveV2Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<TableLiveV2Request>(targetPath);
    validateTableLiveV2Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Table live v2 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(tableLiveV2ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Table live v2 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Table live v2 empty code envelope refused");
  if (phase === "capture") validateTableLiveV2TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Table live v2 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: TABLE_LIVE_V2_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? TABLE_LIVE_V2_SIGNED_WRITER_TIMEOUT_MS
        : TABLE_LIVE_V2_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: TableLiveV2RequestBody = {
    artifactVersion: "table-live-v2-broker-request-v2",
    brokerVersion: TABLE_LIVE_V2_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: TableLiveV2Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateTableLiveV2Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistTableLiveV2RawResponse(
  directory: string,
  phase: TableLiveV2RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Table live v2 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<TableLiveV2Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Table live v2 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = tableLiveV2ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Table live v2 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<TableLiveV2Receipt, "receiptSha256"> = {
    artifactVersion: "table-live-v2-broker-receipt-v2",
    brokerVersion: TABLE_LIVE_V2_BROKER_VERSION,
...
export function acceptTableLiveV2Response<T = unknown>(
  directory: string,
  phase: TableLiveV2RemotePhase,
  options: {
    captureIndex?: number;
...
): TableLiveV2AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Table live v2 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<TableLiveV2Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Table live v2 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "table-live-v2-plugin-response-v2" ||
    plugin?.brokerVersion !== TABLE_LIVE_V2_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Table live v2 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistTableLiveV2TechnicalGates(
  directory: string,
  rootProofs: readonly TableLiveV2RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertTableLiveV2RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Table live v2 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Table live v2 probe payload hash malformed");
  const gates = Object.fromEntries(
    TABLE_LIVE_V2_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "table-live-v2-host-technical-gates-v2",
    brokerVersion: TABLE_LIVE_V2_BROKER_VERSION,
...
export function validateTableLiveV2TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "table-live-v2-host-technical-gates-v2" ||
    ...
    TABLE_LIVE_V2_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Table live v2 technical gates artifact");
}

export function validateTableLiveV2Receipt(
  directory: string,
  phase: TableLiveV2RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateTableLiveV2Request(directory, request);
  const receipt = json<TableLiveV2Receipt>(
  ...
  if (
    receipt.artifactVersion !== "table-live-v2-broker-receipt-v2" ||
    receipt.brokerVersion !== TABLE_LIVE_V2_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Table live v2 accepted receipt: ${request.requestId}`,
    );
}

export function readTableLiveV2AcceptedRawPayload(
  directory: string,
  phase: TableLiveV2RemotePhase,
  captureIndex?: number,
): unknown {
  validateTableLiveV2Receipt(directory, phase, captureIndex);
...
export function createTableLiveV2FakeDynamicResponse(
  request: TableLiveV2Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? TABLE_LIVE_V2_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? TABLE_LIVE_V2_TARGET,
    ...overrides.extra,
  };
}

export function listTableLiveV2PersistedRequests(
  directory: string,
): TableLiveV2Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<TableLiveV2Request>(file)] : [];
    });
}
