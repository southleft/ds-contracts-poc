...
import {
  verifyComboboxLiveV23FixedSceneFixedPoint,
  type ComboboxLiveV23FixedFixedPoint,
} from "./combobox-live-v23-fixed-point.js";

export const COMBOBOX_LIVE_V23_VERIFIER_VERSION = "combobox-live-v23-verifier-v1";

export function buildComboboxLiveV23RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV23RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV23RawNode[];
}

export interface ComboboxLiveV23NormalizedScene {
...
export interface ComboboxLiveV23SceneProof {
...
export function normalizeComboboxLiveV23Scene(
  root: ComboboxLiveV23RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV23NormalizedScene {
  const canonicalBindings: ComboboxLiveV23NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV23RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV23Scene<Instance>(
  normalized: ComboboxLiveV23NormalizedScene,
...
): ComboboxLiveV23SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV23FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV23ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV23PreCaptureGates(payload: unknown): void {
