/**
 * Table live v23 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V23_FIXED_VERIFIER_VERSION = "table-live-v23-fixed-point-v1";
export const TABLE_LIVE_V23_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV23FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV23FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV23FixedSourceProbe {
...
export interface TableLiveV23FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV23FixedFixedPoint;
}

export interface TableLiveV23FixedHardGateInput {
  thresholds: TableLiveV23FixedThresholds;
  sourceProbes: TableLiveV23FixedSourceProbe[];
  cells: TableLiveV23FixedCellValidation[];
  sceneProofs: TableLiveV23FixedSceneProof[];
...
export interface TableLiveV23FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V23_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV23FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV23FixedHardGates(
  input: TableLiveV23FixedHardGateInput,
): TableLiveV23FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV23FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V23_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV23FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV23FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
