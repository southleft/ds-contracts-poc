...
import { COMBOBOX_LIVE_V10_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v10.js";
import { verifyComboboxLiveV10Authorization } from "./combobox-live-v10-authorization.js";
import { runComboboxLiveV10Preflight } from "./combobox-live-v10-preflight.js";
import { ComboboxLiveV10Orchestrator } from "./run-combobox-live-v10.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v10 requires exact authorization artifact ${COMBOBOX_LIVE_V10_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v10-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV10Authorization(root);
    runComboboxLiveV10Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV10Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV10Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
