...
import { TABLE_LIVE_V16_AUTHORIZATION_PATH } from "./build-table-live-proof-v16.js";
import { verifyTableLiveV16Authorization } from "./table-live-v16-authorization.js";
import { runTableLiveV16Preflight } from "./table-live-v16-preflight.js";
import { TableLiveV16Orchestrator } from "./run-table-live-v16.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v16 requires exact authorization artifact ${TABLE_LIVE_V16_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v16-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV16Authorization(root);
    runTableLiveV16Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV16Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV16Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
