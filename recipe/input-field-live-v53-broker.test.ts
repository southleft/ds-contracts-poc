...
import {
  ...
  createInputLiveV53FakeDynamicResponse,
  ...
  listInputLiveV53PersistedRequests,
  persistInputLiveV53RawResponse,
  persistInputLiveV53TechnicalGates,
  validateInputLiveV53Receipt,
  validateInputLiveV53Request,
  type InputLiveV53RemotePhase,
...
import {
  ...
  type InputLiveV53ExtractPayload,
  type InputLiveV53ProbePayload,
  type InputLiveV53RestorePayload,
  type InputLiveV53RootProof,
  type InputLiveV53WriterOwnership,
} from "./input-field-live-v53-contract.js";
import {
  InputLiveV53Orchestrator,
  simulatedInputLiveV53Authorization,
} from "./run-input-field-live-v53.js";
...
const captures: InputLiveV53CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV53Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v53-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV53Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV53CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV53FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV53FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V53_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V53_TARGET);
  persistInputLiveV53RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV53Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV53Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV53WriterOwnership,
): InputLiveV53RestorePayload => ({
...
const rootProofs = (): InputLiveV53RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV53CaptureCell,
): InputLiveV53CapturePayload => {
  const bytes = Buffer.from(`input-live-v53-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV53Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV53Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV53RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV53RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV53ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV53ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV53CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV53CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV53CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV53Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV53CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV53PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV53ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV53Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV53Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV53Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV53Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V53_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV53RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V53_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV53Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV53Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV53ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV53Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV53WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV53RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV53Request;
    validateInputLiveV53Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV53Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV53CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v53-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV53Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV53Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV53Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V53_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV53RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V53_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV53Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV53Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV53RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V53_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV53Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV53Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV53Request;
