...
import { INPUT_LIVE_V69_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v69.js";
import { verifyInputLiveV69Authorization } from "./input-field-live-v69-authorization.js";
import { runInputLiveV69Preflight } from "./input-field-live-v69-preflight.js";
import { InputLiveV69Orchestrator } from "./run-input-field-live-v69.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V69_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v69-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV69Authorization(root);
    runInputLiveV69Preflight(
      root,
      authorization,
    ...
    const orchestrator = InputLiveV69Orchestrator.initialize({
      root,
  ...
  const orchestrator = InputLiveV69Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
