...
import {
  acceptTableLiveV26Response,
  createTableLiveV26Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV26Request,
  persistTableLiveV26RawResponse,
  persistTableLiveV26TechnicalGates,
  readTableLiveV26AcceptedRawPayload,
  type TableLiveV26Request,
  type TableLiveV26TransactionAuthorization,
} from "./table-live-v26-broker.js";
import {
  TABLE_LIVE_V26_CAPTURE_COUNT,
  assertTableLiveV26CaptureResponses,
  ...
  evaluateTableLiveV26Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV26Roots,
  validateTableLiveV26CaptureManifest,
  validateTableLiveV26CapturePayload,
  validateTableLiveV26CleanupPayload,
  validateTableLiveV26ExtractPayload,
  validateTableLiveV26ProbePayload,
  validateTableLiveV26WriterPayload,
  type TableLiveV26CaptureCell,
  type TableLiveV26CapturePayload,
...
import {
  ...
  TABLE_LIVE_V26_PLAN_PATH,
  type TableLiveV26ProofPlan,
} from "./build-table-live-proof-v26.js";
import {
...
export const TABLE_LIVE_V26_RUNNER_VERSION = "table-live-v26-runner-v1";

interface TableLiveV26JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV26OrchestratorState {
  artifactVersion: "table-live-v26-orchestrator-state-v1";
  ...
  journal: TableLiveV26JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV26ProofPlan;
  planSha256: string;
  captures: TableLiveV26CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV26SourceIdentity & {
...
export interface TableLiveV26NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV26OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v26 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V26_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V26_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV26ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV26CaptureCell[];
  validateTableLiveV26CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v26 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v26 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV26WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV26RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV26Request =>
  readJson<TableLiveV26Request>(file);

export class TableLiveV26Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV26Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV26OrchestratorState = {
      artifactVersion: "table-live-v26-orchestrator-state-v1",
    ...
    return new TableLiveV26Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV26OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV26Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV26NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV26NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v26 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV26NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v26 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV26RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV26Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV26CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV26AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV26WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV26Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV26ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV26Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV26Response(this.#directory, "writer", {
        validate: validateTableLiveV26WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV26Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV26RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV26Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV26ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV26Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV26RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v26-host-root-proofs-v1",
      ...
      const next = issueTableLiveV26Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV26ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV26Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV26ProbePayload(payload, ownership),
      });
      persistTableLiveV26TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v26 capture cell missing");
      const accepted = acceptTableLiveV26Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV26CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V26_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV26CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV26Objective(
    ...
    throw new Error(`Table live v26 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV26Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV26CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV26Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV26RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV26Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV26CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV26CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV26CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV26Authorization = (
  privateKey: KeyObject,
): TableLiveV26TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV26SimulationKey = (): KeyObject =>
