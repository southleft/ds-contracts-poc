...
import { buildTableLiveV20CleanupRuntime } from "./table-live-v20-cleanup.js";
import { type TableLiveV20FixedFixedPoint } from "./table-live-v20-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV20RestoreProgram,
  validateTableLiveV20RestorePayload,
  type TableLiveV20RestorePayload,
} from "./table-live-v20-restore.js";
import {
  buildTableLiveV20RawPropertyRuntime,
  normalizeTableLiveV20Scene,
  type TableLiveV20RawNode,
} from "./table-live-v20-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v20.js";
import {
...
export const TABLE_LIVE_V20_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V20_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V20_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V20_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V20_VARIANT_COUNT = 20;
export const TABLE_LIVE_V20_SET_COUNT = 6;
export const TABLE_LIVE_V20_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V20_HOST_PHASES = 3;
export const TABLE_LIVE_V20_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V20_RESTORE_COUNT = 8;
export const TABLE_LIVE_V20_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V20_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV20SourceId = (typeof TABLE_LIVE_V20_SOURCE_IDS)[number];
export type TableLiveV20Kind = "table" | "row" | "cell";

export interface TableLiveV20SourceIdentity {
  source: TableLiveV20SourceId;
...
export interface TableLiveV20WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV20ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV20SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV20RawNode;
    rowScene: TableLiveV20RawNode;
    cellScene: TableLiveV20RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV20RootProof {
  source: TableLiveV20SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV20FixedFixedPoint;
}

export interface TableLiveV20ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV20SourceId;
    adapterIdentity: string;
...
export interface TableLiveV20CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV20SourceId;
  adapterIdentity: string;
...
export interface TableLiveV20CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV20SourceId;
  frameWidth: number;
...
export interface TableLiveV20CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v20 ${label} must be an object`);
...
export function validateTableLiveV20WriterPayload(
  value: unknown,
): TableLiveV20WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V20_NAMESPACE ||
    ...
    throw new TypeError("Table live v20 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v20 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v20 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV20SourceIdentity,
  kind: TableLiveV20Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV20ExtractProgram(
  writer: TableLiveV20WriterOwnership,
  sources: readonly TableLiveV20SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v20 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V20_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V20_NAMESPACE)}
${buildTableLiveV20RawPropertyRuntime()}
...
export function validateTableLiveV20ExtractPayload(
  value: unknown,
  writer: TableLiveV20WriterOwnership,
): TableLiveV20ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v20 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V20_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v20 extract schema/two-root mismatch");
  return payload as TableLiveV20ExtractPayload;
}

export function proveTableLiveV20Roots<Instance>(
  extract: TableLiveV20ExtractPayload,
  sources: readonly (TableLiveV20SourceIdentity & {
...
): TableLiveV20RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v20 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v20 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV20Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV20Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV20Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV20FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v20 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v20 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v20 compile lost owned sets");
...
export function assertTableLiveV20RootProofs(
  proofs: readonly TableLiveV20RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v20 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v20 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV20ProbeProgram(
  writer: TableLiveV20WriterOwnership,
  sources: readonly TableLiveV20SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V20_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV20ProbePayload(
  value: unknown,
  writer: TableLiveV20WriterOwnership,
): TableLiveV20ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV20ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V20_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v20 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV20CaptureManifest(
  cells: readonly TableLiveV20CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V20_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V20_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V20_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V20_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v20 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV20CaptureCell[],
): string => {
  validateTableLiveV20CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV20CaptureProgram(
  writer: TableLiveV20WriterOwnership,
  cell: TableLiveV20CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V20_CAPTURE_COUNT
  )
    throw new TypeError("Table live v20 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v20 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V20_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV20CapturePayload(
  value: unknown,
  cell: TableLiveV20CaptureCell,
  rawResponseBytes: number,
): TableLiveV20CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV20CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V20_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V20_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v20 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV20CaptureResponses(
  manifest: readonly TableLiveV20CaptureCell[],
  responses: readonly TableLiveV20CapturePayload[],
): void {
  validateTableLiveV20CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v20 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV20ObjectiveReport {
  artifactVersion: "table-live-v20-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV20Objective(
  manifest: readonly TableLiveV20CaptureCell[],
  responses: readonly TableLiveV20CapturePayload[],
): TableLiveV20ObjectiveReport {
  assertTableLiveV20CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v20-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV20CleanupProgram(
  writer: TableLiveV20WriterOwnership,
): string {
  const baseProgram = buildTableLiveV20CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V20_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v20 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV20CleanupPayload(
  payload: unknown,
  writer: TableLiveV20WriterOwnership,
): TableLiveV20CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV20CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v20 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V20_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV20WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
