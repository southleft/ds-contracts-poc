...
import {
  ...
  INPUT_LIVE_V43_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V43_RESPONSE_CONTRACTS,
  type InputLiveV43RootProof,
} from "./input-field-live-v43-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V43_BROKER_VERSION = "input-live-v43-broker-v2";
export const INPUT_LIVE_V43_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V43_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V43_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V43_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V43_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V43_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV43RemotePhase =
  (typeof INPUT_LIVE_V43_REMOTE_PHASES)[number];
export type InputLiveV43AuthorizationMode = "live" | "simulated";

export interface InputLiveV43CommitPins {
...
export interface InputLiveV43TransactionAuthorization extends InputLiveV43CommitPins {
  mode: InputLiveV43AuthorizationMode;
...
export interface InputLiveV43Manifest {
  artifactVersion: "input-live-v43-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V43_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V43_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V43_TARGET;
  authorization: InputLiveV43TransactionAuthorization;
...
export interface InputLiveV43ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV43RequestBody {
  ...
  commitPins: InputLiveV43CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V43_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV43ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV43Request extends InputLiveV43RequestBody {
...
export interface InputLiveV43Receipt {
  artifactVersion: "input-live-v43-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V43_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV43RemotePhase;
...
export interface InputLiveV43AcceptedResponse<T = unknown> {
  request: InputLiveV43Request;
  receipt: InputLiveV43Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV43IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV43ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V43_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V43_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V43_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V43_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V43_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V43_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V43_TARGET.editorType;
};
...
export const inputLiveV43RequestSequence = (
  phase: InputLiveV43RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV43RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV43RequestPath = (
  directory: string,
  phase: InputLiveV43RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV43ResponsePath = (
  directory: string,
  phase: InputLiveV43RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV43ReceiptPath = (
  directory: string,
  phase: InputLiveV43RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV43Manifest => {
  const manifest = json<InputLiveV43Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v43-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V43_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V43_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV43RemotePhase,
  captureIndex?: number,
): InputLiveV43Request | undefined => {
  const file = inputLiveV43RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV43Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV43RemotePhase,
  captureIndex?: number,
): InputLiveV43Receipt | undefined => {
  const file = inputLiveV43ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV43Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV43Request | undefined;
  receipt: InputLiveV43Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV43RemotePhase,
): InputLiveV43ResponseContract => ({
  ...INPUT_LIVE_V43_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V43_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V43_DYNAMIC_TOOL,
      target: INPUT_LIVE_V43_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v43-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V43_BROKER_VERSION,
  target: INPUT_LIVE_V43_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V43_PROGRAM_START*/${program}/*INPUT_LIVE_V43_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V43_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V43_PROGRAM_END*/";
...
export function createInputLiveV43Transaction(
  directory: string,
  ...
  const manifest: InputLiveV43Manifest = {
    artifactVersion: "input-live-v43-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V43_BROKER_VERSION,
...
export function validateInputLiveV43Request(
  directory: string,
  request: InputLiveV43Request,
): void {
  ...
  const expectedSequence = inputLiveV43RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v43-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V43_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V43_TARGET.fileKey ||
...
export function issueInputLiveV43Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV43RemotePhase,
  program: string,
  options: InputLiveV43IssueOptions = {},
): InputLiveV43Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV43RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV43Request>(targetPath);
    validateInputLiveV43Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV43ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV43TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV43RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    ...
    timeout:
      ...
      (phase === "writer"
        ...
        : INPUT_LIVE_V43_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV43RequestBody = {
    artifactVersion: "input-live-v43-broker-request-v2",
    brokerVersion: INPUT_LIVE_V43_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV43Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV43Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV43RawResponse(
  directory: string,
  phase: InputLiveV43RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV43RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV43Request>(requestFile);
  if (
  ...
  const target = inputLiveV43ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV43Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v43-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V43_BROKER_VERSION,
...
export function acceptInputLiveV43Response<T = unknown>(
  directory: string,
  phase: InputLiveV43RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV43AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV43ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV43Request>(
    inputLiveV43RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV43Request(directory, request);
  if (phase === "capture") validateInputLiveV43TechnicalGates(directory);
  const rawFile = inputLiveV43ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v43-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V43_BROKER_VERSION ||
...
export function persistInputLiveV43TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV43RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV43RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V43_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v43-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V43_BROKER_VERSION,
...
export function validateInputLiveV43TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v43-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V43_TECHNICAL_GATES.some(
...
export function validateInputLiveV43Receipt(
  directory: string,
  phase: InputLiveV43RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV43Request(directory, request);
  const receipt = json<InputLiveV43Receipt>(
    inputLiveV43ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV43ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v43-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V43_BROKER_VERSION ||
...
export function readInputLiveV43AcceptedRawPayload(
  directory: string,
  phase: InputLiveV43RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV43Receipt(directory, phase, captureIndex);
...
export function createInputLiveV43FakeDynamicResponse(
  request: InputLiveV43Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V43_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V43_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV43PersistedRequests(
  directory: string,
): InputLiveV43Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV43Request>(file)] : [];
    });
}

export const inputLiveV43Sha256 = sha256;
