...
import {
  INPUT_LIVE_V83_MEASURE_VISIBLE_FILL_MARKER,
  INPUT_LIVE_V83_RESTORE_CONTENT_ROLES,
  INPUT_LIVE_V83_RESTORE_PARENT_ROLES,
...
test("v83 probe reveals hidden content FILL before measuring, then restores visibility", () => {
  const program = readFileSync(
    "recipe/input-field-live-v83-contract.ts",
    "utf8",
  );
  const probe = program.slice(
    program.indexOf("export function buildInputLiveV83ProbeProgram"),
  );
...
test("v16 restore still measures FILL while visible, then restores visibility", () => {
  const program = buildInputLiveV83RestoreProgram(writer);
  assert.match(program, new RegExp(INPUT_LIVE_V83_MEASURE_VISIBLE_FILL_MARKER));
  ...
  for (const role of INPUT_LIVE_V83_RESTORE_CONTENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
  for (const role of INPUT_LIVE_V83_RESTORE_PARENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
});

test("v16 restore payload requires 256 filled content texts", () => {
  const accepted = validateInputLiveV83RestorePayload(
    {
      pageId: "1:1",
      setIds: ["2:2", "2:1"],
      restoredCount: 256,
      ...
      marker: INPUT_LIVE_V83_MEASURE_VISIBLE_FILL_MARKER,
    },
    writer,
  );
...
test("v16 writer bytes stay frozen and v17 writer assigns set HUG", () => {
  ...
  const v76Program = readFileSync(
    "recipe/evidence/input-field-live-pivot-v83/programs/writer.txt",
  );
  const v76Payload = readFileSync(
    "recipe/evidence/input-field-live-pivot-v83/programs/writer-payload.js",
  );
...
test("v83 probe excludes overlay-label AABB from visibleAreaLoss; overlap still uses full semantic", () => {
  const program = readFileSync(
    "recipe/input-field-live-v83-contract.ts",
    "utf8",
  );
...
test("v19 writer preserves hidden FILL occupancy; hashed v16/v17/v18 stay frozen", () => {
  ...
  assert.match(v83Payload, /INPUT-WRITER-HIDDEN-FILL-OCCUPANCY/);
  assert.equal(
    v83Payload.includes(
  ...
  assert.equal(sha256(v83Payload), V19_WRITER_PAYLOAD_SHA256);
  assert.equal(
    sha256(
      readFileSync(
        "recipe/evidence/input-field-live-pivot-v83/programs/writer.txt",
...
test("v83 expected-plan carries live occupancy opacity 0; Polar stays V82", () => {
  const mui = JSON.parse(
    gunzipSync(
      readFileSync(
        "recipe/evidence/input-field-live-pivot-v83/expected-scene-plan-mui.json.gz",
      ),
    ).toString("utf8"),
  ) as {
    facts: Array<{
