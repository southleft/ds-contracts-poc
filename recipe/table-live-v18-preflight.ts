...
import {
  validateTableLiveV18AntecedentIndex,
  verifyTableLiveV18Authorization,
  type TableLiveV18AntecedentIndex,
  type TableLiveV18AuthorizationProof,
} from "./table-live-v18-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V18_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v18/operator-security-attestation-template.json";
export const TABLE_LIVE_V18_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v18-security-attestation.json";

export interface TableLiveV18SecurityAttestation {
  artifactVersion: "table-live-v18-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV18PreflightReport {
  artifactVersion: "table-live-v18-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV18SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV18SecurityAttestation(
  value: unknown,
  proof: TableLiveV18AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v18-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V18_TARGET) ||
...
export function validateTableLiveV18ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV18Authorization");
  const preflight = source.indexOf("runTableLiveV18Preflight");
  const initialize = source.indexOf("TableLiveV18Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV18Preflight(
  root: string,
  proof: TableLiveV18AuthorizationProof,
  attestationPath = TABLE_LIVE_V18_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV18PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V18_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V18_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV18AntecedentIndex;
  failures.push(...validateTableLiveV18AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V18_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V18_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV18SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v18-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV18ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v18 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV18SecurityAttestation;
  return {
    artifactVersion: "table-live-v18-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V18_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV18Preflight(
    process.cwd(),
    verifyTableLiveV18Authorization(),
    attestationPath,
