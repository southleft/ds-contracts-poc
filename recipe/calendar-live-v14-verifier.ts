...
import {
  verifyCalendarLiveV14FixedSceneFixedPoint,
  type CalendarLiveV14FixedFixedPoint,
} from "./calendar-live-v14-fixed-point.js";

export const CALENDAR_LIVE_V14_VERIFIER_VERSION = "calendar-live-v14-verifier-v1";

export function buildCalendarLiveV14RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV14RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV14RawNode[];
}

export interface CalendarLiveV14NormalizedScene {
...
export interface CalendarLiveV14SceneProof {
...
export function normalizeCalendarLiveV14Scene(
  root: CalendarLiveV14RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV14NormalizedScene {
  const canonicalBindings: CalendarLiveV14NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV14RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV14Scene<Instance>(
  normalized: CalendarLiveV14NormalizedScene,
...
): CalendarLiveV14SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV14FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV14ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV14PreCaptureGates(payload: unknown): void {
