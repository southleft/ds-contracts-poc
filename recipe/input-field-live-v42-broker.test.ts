...
import {
  ...
  createInputLiveV42FakeDynamicResponse,
  ...
  listInputLiveV42PersistedRequests,
  persistInputLiveV42RawResponse,
  persistInputLiveV42TechnicalGates,
  validateInputLiveV42Receipt,
  validateInputLiveV42Request,
  type InputLiveV42RemotePhase,
...
import {
  ...
  type InputLiveV42ExtractPayload,
  type InputLiveV42ProbePayload,
  type InputLiveV42RestorePayload,
  type InputLiveV42RootProof,
  type InputLiveV42WriterOwnership,
} from "./input-field-live-v42-contract.js";
import {
  InputLiveV42Orchestrator,
  simulatedInputLiveV42Authorization,
} from "./run-input-field-live-v42.js";
...
const captures: InputLiveV42CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV42Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v42-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV42Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV42CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV42FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV42FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V42_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V42_TARGET);
  persistInputLiveV42RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV42Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV42Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV42WriterOwnership,
): InputLiveV42RestorePayload => ({
...
const rootProofs = (): InputLiveV42RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV42CaptureCell,
): InputLiveV42CapturePayload => {
  const bytes = Buffer.from(`input-live-v42-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV42Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV42Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV42RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV42RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV42ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV42ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV42CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV42CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV42CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV42Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV42CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV42PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV42ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV42Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV42Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV42Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV42Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V42_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV42RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V42_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV42Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV42Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV42ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV42Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV42WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV42RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV42Request;
    validateInputLiveV42Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV42Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV42CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v42-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV42Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV42Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV42Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V42_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV42RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V42_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV42Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV42Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV42RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V42_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV42Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV42Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV42Request;
