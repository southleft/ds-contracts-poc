...
import {
  validateTableLiveV32AntecedentIndex,
  verifyTableLiveV32Authorization,
  type TableLiveV32AntecedentIndex,
  type TableLiveV32AuthorizationProof,
} from "./table-live-v32-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V32_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v32/operator-security-attestation-template.json";
export const TABLE_LIVE_V32_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v32-security-attestation.json";

export interface TableLiveV32SecurityAttestation {
  artifactVersion: "table-live-v32-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV32PreflightReport {
  artifactVersion: "table-live-v32-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV32SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV32SecurityAttestation(
  value: unknown,
  proof: TableLiveV32AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v32-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V32_TARGET) ||
...
export function validateTableLiveV32ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV32Authorization");
  const preflight = source.indexOf("runTableLiveV32Preflight");
  const initialize = source.indexOf("TableLiveV32Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV32Preflight(
  root: string,
  proof: TableLiveV32AuthorizationProof,
  attestationPath = TABLE_LIVE_V32_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV32PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V32_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V32_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV32AntecedentIndex;
  failures.push(...validateTableLiveV32AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V32_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V32_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV32SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v32-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV32ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v32 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV32SecurityAttestation;
  return {
    artifactVersion: "table-live-v32-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V32_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV32Preflight(
    process.cwd(),
    verifyTableLiveV32Authorization(),
    attestationPath,
