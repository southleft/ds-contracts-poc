...
import {
  acceptCalendarLiveV7Response,
  createCalendarLiveV7Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV7Request,
  persistCalendarLiveV7RawResponse,
  persistCalendarLiveV7TechnicalGates,
  readCalendarLiveV7AcceptedRawPayload,
  type CalendarLiveV7Request,
  type CalendarLiveV7TransactionAuthorization,
} from "./calendar-live-v7-broker.js";
import {
  CALENDAR_LIVE_V7_CAPTURE_COUNT,
  assertCalendarLiveV7CaptureResponses,
  ...
  evaluateCalendarLiveV7Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV7Roots,
  validateCalendarLiveV7CaptureManifest,
  validateCalendarLiveV7CapturePayload,
  validateCalendarLiveV7CleanupPayload,
  validateCalendarLiveV7ExtractPayload,
  validateCalendarLiveV7ProbePayload,
  validateCalendarLiveV7WriterPayload,
  type CalendarLiveV7CaptureCell,
  type CalendarLiveV7CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V7_PLAN_PATH,
  type CalendarLiveV7ProofPlan,
} from "./build-calendar-live-proof-v7.js";
import {
...
export const CALENDAR_LIVE_V7_RUNNER_VERSION = "calendar-live-v7-runner-v1";

interface CalendarLiveV7JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV7OrchestratorState {
  artifactVersion: "calendar-live-v7-orchestrator-state-v1";
  ...
  journal: CalendarLiveV7JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV7ProofPlan;
  planSha256: string;
  captures: CalendarLiveV7CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV7SourceIdentity & {
...
export interface CalendarLiveV7NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV7OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v7 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V7_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V7_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV7ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV7CaptureCell[];
  validateCalendarLiveV7CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v7 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v7 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV7WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV7RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV7Request =>
  readJson<CalendarLiveV7Request>(file);

export class CalendarLiveV7Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV7Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV7OrchestratorState = {
      artifactVersion: "calendar-live-v7-orchestrator-state-v1",
    ...
    return new CalendarLiveV7Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV7OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV7Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV7NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV7NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v7 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV7NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v7 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV7RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV7Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV7CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV7AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV7WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV7Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV7ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV7Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV7Response(this.#directory, "writer", {
        validate: validateCalendarLiveV7WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV7Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV7RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV7Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV7ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV7Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV7RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v7-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV7Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV7ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV7Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV7ProbePayload(payload, ownership),
      });
      persistCalendarLiveV7TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v7 capture cell missing");
      const accepted = acceptCalendarLiveV7Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV7CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V7_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV7CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV7Objective(
    ...
    throw new Error(`Calendar live v7 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV7Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV7CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV7Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV7RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV7Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV7CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV7CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV7CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV7Authorization = (
  privateKey: KeyObject,
): CalendarLiveV7TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV7SimulationKey = (): KeyObject =>
