...
export interface ComboboxLiveV14LifecycleSimulationReport {
  artifactVersion: "combobox-live-v14-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV14AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV14Lifecycle(): Promise<ComboboxLiveV14LifecycleSimulationReport> {
  await buildComboboxLiveV14Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V14_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV14AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v14-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V14_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V14_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV14History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV14History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV14AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V14_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V14_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV14History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV14History(authorized, "pending");
    ...
    await buildComboboxLiveV14Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V14_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV14AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV14Lifecycle(), null, 2)}\n`,
  );
