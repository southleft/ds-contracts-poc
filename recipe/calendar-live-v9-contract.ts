...
import { buildCalendarLiveV9CleanupRuntime } from "./calendar-live-v9-cleanup.js";
import { type CalendarLiveV9FixedFixedPoint } from "./calendar-live-v9-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV9RestoreProgram,
  validateCalendarLiveV9RestorePayload,
  type CalendarLiveV9RestorePayload,
} from "./calendar-live-v9-restore.js";
import {
  buildCalendarLiveV9RawPropertyRuntime,
  normalizeCalendarLiveV9Scene,
  type CalendarLiveV9RawNode,
} from "./calendar-live-v9-verifier.js";
...
export const CALENDAR_LIVE_V9_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V9_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V9_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V9_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V9_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V9_SET_COUNT = 3;
export const CALENDAR_LIVE_V9_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V9_HOST_PHASES = 3;
export const CALENDAR_LIVE_V9_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V9_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V9_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V9_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV9SourceId = (typeof CALENDAR_LIVE_V9_SOURCE_IDS)[number];
export type CalendarLiveV9Kind = "calendar" | "week" | "day";

export interface CalendarLiveV9SourceIdentity {
  source: CalendarLiveV9SourceId;
...
export interface CalendarLiveV9WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV9ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV9SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV9RawNode;
    weekScene: CalendarLiveV9RawNode;
    dayScene: CalendarLiveV9RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV9RootProof {
  source: CalendarLiveV9SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV9FixedFixedPoint;
}

export interface CalendarLiveV9ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV9SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV9CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV9SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV9CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV9SourceId;
  frameWidth: number;
...
export interface CalendarLiveV9CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v9 ${label} must be an object`);
...
export function validateCalendarLiveV9WriterPayload(
  value: unknown,
): CalendarLiveV9WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v9") ||
    ...
    result.namespace !== CALENDAR_LIVE_V9_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v9 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v9 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v9 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV9SourceIdentity,
  kind: CalendarLiveV9Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV9ExtractProgram(
  writer: CalendarLiveV9WriterOwnership,
  sources: readonly CalendarLiveV9SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v9 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V9_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V9_NAMESPACE)}
${buildCalendarLiveV9RawPropertyRuntime()}
...
export function validateCalendarLiveV9ExtractPayload(
  value: unknown,
  writer: CalendarLiveV9WriterOwnership,
): CalendarLiveV9ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v9 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V9_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v9 extract schema/one-root mismatch");
  return payload as CalendarLiveV9ExtractPayload;
}

export function proveCalendarLiveV9Roots<Instance>(
  extract: CalendarLiveV9ExtractPayload,
  sources: readonly (CalendarLiveV9SourceIdentity & {
...
): CalendarLiveV9RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v9 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v9 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV9Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV9Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV9Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV9FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v9 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v9 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v9 compile lost owned sets");
...
export function assertCalendarLiveV9RootProofs(
  proofs: readonly CalendarLiveV9RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v9 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v9 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV9ProbeProgram(
  writer: CalendarLiveV9WriterOwnership,
  sources: readonly CalendarLiveV9SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V9_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV9ProbePayload(
  value: unknown,
  writer: CalendarLiveV9WriterOwnership,
): CalendarLiveV9ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV9ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V9_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V9_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v9 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV9CaptureManifest(
  cells: readonly CalendarLiveV9CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V9_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V9_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V9_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V9_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v9 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV9CaptureCell[],
): string => {
  validateCalendarLiveV9CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV9CaptureProgram(
  writer: CalendarLiveV9WriterOwnership,
  cell: CalendarLiveV9CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V9_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v9 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v9 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V9_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV9CapturePayload(
  value: unknown,
  cell: CalendarLiveV9CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV9CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV9CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V9_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V9_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v9 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV9CaptureResponses(
  manifest: readonly CalendarLiveV9CaptureCell[],
  responses: readonly CalendarLiveV9CapturePayload[],
): void {
  validateCalendarLiveV9CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v9 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV9ObjectiveReport {
  artifactVersion: "calendar-live-v9-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV9Objective(
  manifest: readonly CalendarLiveV9CaptureCell[],
  responses: readonly CalendarLiveV9CapturePayload[],
): CalendarLiveV9ObjectiveReport {
  assertCalendarLiveV9CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v9-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV9CleanupProgram(
  writer: CalendarLiveV9WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV9CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V9_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v9 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV9CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV9WriterOwnership,
): CalendarLiveV9CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV9CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v9 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V9_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV9WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
