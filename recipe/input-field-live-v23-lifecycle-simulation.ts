...
export interface InputLiveV23LifecycleSimulationReport {
  artifactVersion: "input-live-v23-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV23AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV23Lifecycle(): Promise<InputLiveV23LifecycleSimulationReport> {
  await buildInputLiveV23Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V23_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV23AntecedentIndex;
  const indexSha256 = inputLiveV23Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v23-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V23_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V23_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV23History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV23History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV23AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V23_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V23_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV23History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV23History(authorized, "pending");
    ...
    await buildInputLiveV23Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V23_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV23AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV23Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV23Lifecycle(), null, 2)}\n`,
  );
