...
import {
  ...
  verifyInputLiveV53Authorization,
  type InputLiveV53AntecedentIndex,
  type InputLiveV53AuthorizationProof,
} from "./input-field-live-v53-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V53_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v53/operator-security-attestation-template.json";
export const INPUT_LIVE_V53_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v53-security-attestation.json";

export interface InputLiveV53SecurityAttestation {
...
export interface InputLiveV53PreflightReport {
  artifactVersion: "input-live-v53-preflight-v1";
...
export const inputLiveV53AttestationSha256 = (
  value: Omit<InputLiveV53SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV53SecurityAttestation(
  value: unknown,
  proof: InputLiveV53AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v53-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V53_TARGET) ||
...
export function validateInputLiveV53ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV53Authorization");
  const preflight = source.indexOf("runInputLiveV53Preflight");
  const initialize = source.indexOf("InputLiveV53Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV53Preflight(
  root: string,
  proof: InputLiveV53AuthorizationProof,
  attestationPath = INPUT_LIVE_V53_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV53PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V53_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V53_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V53_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV53AntecedentIndex;
  failures.push(...validateInputLiveV53AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V53_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V53_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v53-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV53ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV53SecurityAttestation;
  return {
    artifactVersion: "input-live-v53-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V53_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV53Preflight(
    process.cwd(),
    verifyInputLiveV53Authorization(),
    attestationPath,
