...
import {
  ...
  verifyInputLiveV30Authorization,
  type InputLiveV30AntecedentIndex,
  type InputLiveV30AuthorizationProof,
} from "./input-field-live-v30-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V30_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v30/operator-security-attestation-template.json";
export const INPUT_LIVE_V30_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v30-security-attestation.json";

export interface InputLiveV30SecurityAttestation {
...
export interface InputLiveV30PreflightReport {
  artifactVersion: "input-live-v30-preflight-v1";
...
export const inputLiveV30AttestationSha256 = (
  value: Omit<InputLiveV30SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV30SecurityAttestation(
  value: unknown,
  proof: InputLiveV30AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v30-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V30_TARGET) ||
...
export function validateInputLiveV30ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV30Authorization");
  const preflight = source.indexOf("runInputLiveV30Preflight");
  const initialize = source.indexOf("InputLiveV30Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV30Preflight(
  root: string,
  proof: InputLiveV30AuthorizationProof,
  attestationPath = INPUT_LIVE_V30_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV30PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V30_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V30_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V30_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV30AntecedentIndex;
  failures.push(...validateInputLiveV30AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V30_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V30_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v30-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV30ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV30SecurityAttestation;
  return {
    artifactVersion: "input-live-v30-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V30_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV30Preflight(
    process.cwd(),
    verifyInputLiveV30Authorization(),
    attestationPath,
