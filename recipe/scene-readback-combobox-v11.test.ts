...
import {
  ...
  COMBOBOX_LIVE_V11_LEADING_SLOT_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V11_OMIT_EMPTY_INSTANCE_PAYLOAD_MARKER,
  COMBOBOX_LIVE_V11_PROJECT_LIVE_ROOT_OWNERSHIP_KEY_MARKER,
  COMBOBOX_LIVE_V11_RECOVER_COMPONENT_PROPERTY_NAME_MARKER,
  COMBOBOX_LIVE_V11_RECOVER_RECIPE_COMPONENT_REF_MARKER,
  COMBOBOX_LIVE_V11_TRIGGER_BINDING_COMPILE_ORDER_MARKER,
...
test("host orders combobox/trigger bindings to compile field order, not live or inherited Input surface list", () => {
  ...
  assert.ok(
    ...
    "v10/v11 compile-order list must rank padding.left before padding.right",
  );
  ...
  assert.deepEqual(
    ...
    "v11 host must keep compile left-then-right, not live extract order",
  );
...
test("host orders combobox/control/leading bindings to compile field order, not live or inherited fills-first slot list", () => {
  ...
  assert.deepEqual(
    ...
    "v11 compile-order list must rank width then height then fill",
  );
  ...
  assert.deepEqual(
    ...
    ["width.value", "height.value", "fills.0.color"],
    "v11 host must emit compile width then height then fill, not live extract order",
  );
});
