...
import { verifyTableLiveV25Authorization } from "./table-live-v25-authorization.js";
import { TABLE_LIVE_V25_TARGET } from "./table-live-v25-broker.js";
import {
  TABLE_LIVE_V25_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV25Authorization();
...
const attestation: TableLiveV25SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV25SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v25 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V25_SECURITY_ATTESTATION_DEFAULT_PATH,
);
