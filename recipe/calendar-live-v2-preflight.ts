...
import {
  validateCalendarLiveV2AntecedentIndex,
  verifyCalendarLiveV2Authorization,
  type CalendarLiveV2AntecedentIndex,
  type CalendarLiveV2AuthorizationProof,
} from "./calendar-live-v2-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V2_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v2/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V2_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v2-security-attestation.json";

export interface CalendarLiveV2SecurityAttestation {
  artifactVersion: "calendar-live-v2-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV2PreflightReport {
  artifactVersion: "calendar-live-v2-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV2SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV2SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV2AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v2-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V2_TARGET) ||
...
export function validateCalendarLiveV2ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV2Authorization");
  const preflight = source.indexOf("runCalendarLiveV2Preflight");
  const initialize = source.indexOf("CalendarLiveV2Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV2Preflight(
  root: string,
  proof: CalendarLiveV2AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V2_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV2PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V2_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V2_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV2AntecedentIndex;
  failures.push(...validateCalendarLiveV2AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V2_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V2_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV2SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v2-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV2ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v2 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV2SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v2-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V2_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV2Preflight(
    process.cwd(),
    verifyCalendarLiveV2Authorization(),
    attestationPath,
