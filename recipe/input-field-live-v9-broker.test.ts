...
import {
  ...
  createInputLiveV9FakeDynamicResponse,
  ...
  listInputLiveV9PersistedRequests,
  persistInputLiveV9RawResponse,
  persistInputLiveV9TechnicalGates,
  validateInputLiveV9Receipt,
  validateInputLiveV9Request,
  type InputLiveV9RemotePhase,
...
import {
  ...
  type InputLiveV9ExtractPayload,
  type InputLiveV9ProbePayload,
  type InputLiveV9RootProof,
  type InputLiveV9WriterOwnership,
} from "./input-field-live-v9-contract.js";
import {
  InputLiveV9Orchestrator,
  simulatedInputLiveV9Authorization,
} from "./run-input-field-live-v9.js";
...
const captures: InputLiveV9CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV9Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v9-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV9Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV9CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV9FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV9FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V9_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V9_TARGET);
  persistInputLiveV9RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV9Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV9Receipt(directory, phase, options.captureIndex);
...
const extractPayload = (
  ownership: InputLiveV9WriterOwnership,
...
const rootProofs = (): InputLiveV9RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v9-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV9Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV9Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV9RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) => validateInputLiveV9ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      probePayload(writer),
      {
        validate: (payload) => validateInputLiveV9ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV9CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV9CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV9CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV9Response(directory, "cleanup", {
      validate: (payload) => validateInputLiveV9CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV9PersistedRequests(directory).length, 132);
    assert.ok(inputLiveV9ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV9Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV9Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV9Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV9Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V9_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV9RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V9_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV9Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV9Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV9ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV9Receipt(third.directory, "writer"),
      /invalid Input live v9 accepted receipt/,
    );
  } finally {
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV9WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV9RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV9Request;
    validateInputLiveV9Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV9Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV9CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active extract and surfaced cleanup", () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v9-state-"));
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV9Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV9Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV9Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V9_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV9RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V9_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV9Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV9Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV9RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V9_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV9Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV9Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV9Request;
