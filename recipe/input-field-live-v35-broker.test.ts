...
import {
  ...
  createInputLiveV35FakeDynamicResponse,
  ...
  listInputLiveV35PersistedRequests,
  persistInputLiveV35RawResponse,
  persistInputLiveV35TechnicalGates,
  validateInputLiveV35Receipt,
  validateInputLiveV35Request,
  type InputLiveV35RemotePhase,
...
import {
  ...
  type InputLiveV35ExtractPayload,
  type InputLiveV35ProbePayload,
  type InputLiveV35RestorePayload,
  type InputLiveV35RootProof,
  type InputLiveV35WriterOwnership,
} from "./input-field-live-v35-contract.js";
import {
  InputLiveV35Orchestrator,
  simulatedInputLiveV35Authorization,
} from "./run-input-field-live-v35.js";
...
const captures: InputLiveV35CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV35Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v35-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV35Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV35CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV35FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV35FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V35_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V35_TARGET);
  persistInputLiveV35RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV35Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV35Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV35WriterOwnership,
): InputLiveV35RestorePayload => ({
...
const rootProofs = (): InputLiveV35RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV35CaptureCell,
): InputLiveV35CapturePayload => {
  const bytes = Buffer.from(`input-live-v35-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV35Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV35Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV35RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV35RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV35ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV35ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV35CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV35CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV35CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV35Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV35CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV35PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV35ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV35Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV35Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV35Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV35Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V35_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV35RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V35_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV35Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV35Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV35ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV35Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV35WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV35RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV35Request;
    validateInputLiveV35Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV35Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV35CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v35-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV35Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV35Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV35Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V35_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV35RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V35_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV35Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV35Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV35RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V35_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV35Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV35Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV35Request;
