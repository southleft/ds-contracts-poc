...
import { TABLE_LIVE_V36_INDEX_PATH } from "./build-table-live-proof-v36.js";
import {
...
import {
  buildTableLiveV36AuthorizationArtifact,
  validateTableLiveV36AntecedentIndex,
  validateTableLiveV36AuthorizationArtifact,
  validateTableLiveV36History,
  validateTableLiveV36UnitTestSource,
  type TableLiveV36AntecedentIndex,
  type TableLiveV36AuthorizationArtifact,
  type TableLiveV36AuthorizationProof,
  type TableLiveV36HistoryState,
} from "./table-live-v36-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV36ControlFlowSource,
  validateTableLiveV36SecurityAttestation,
  type TableLiveV36SecurityAttestation,
} from "./table-live-v36-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V36_INDEX_PATH);
const index = (): TableLiveV36AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV36AntecedentIndex;

const artifact = (): TableLiveV36AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV36AuthorizationArtifact({
...
const pendingState = (): TableLiveV36HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV36HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV36AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v36/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV36AuthorizationArtifact,
): TableLiveV36SecurityAttestation => {
  const body: Omit<TableLiveV36SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V36_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV36History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV36History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV36History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV36History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV36History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV36UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v36-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV36UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV36AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV36AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV36AuthorizationArtifact(
        planted as TableLiveV36AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV36SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV36SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV36SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v36-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV36ControlFlowSource(source), []);
});
