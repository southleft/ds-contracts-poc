...
import { INPUT_LIVE_V75_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v75.js";
import { verifyInputLiveV75Authorization } from "./input-field-live-v75-authorization.js";
import { runInputLiveV75Preflight } from "./input-field-live-v75-preflight.js";
import { InputLiveV75Orchestrator } from "./run-input-field-live-v75.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V75_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v75-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV75Authorization(root);
    runInputLiveV75Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV75Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
