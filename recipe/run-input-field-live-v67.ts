...
import {
  acceptInputLiveV67Response,
  createInputLiveV67Transaction,
  inputLiveV67ReceiptPath,
  inputLiveV67RequestPath,
  inputLiveV67ResponsePath,
  inputLiveV67Sha256,
  issueInputLiveV67Request,
  persistInputLiveV67RawResponse,
  persistInputLiveV67TechnicalGates,
  readInputLiveV67AcceptedRawPayload,
  type InputLiveV67Request,
  type InputLiveV67TransactionAuthorization,
} from "./input-field-live-v67-broker.js";
import {
  INPUT_LIVE_V67_CAPTURE_COUNT,
  assertInputLiveV67CaptureResponses,
  assertInputLiveV67RootProofs,
  buildInputLiveV67CaptureProgram,
  buildInputLiveV67CleanupProgram,
  buildInputLiveV67ExtractProgram,
  buildInputLiveV67ProbeProgram,
  buildInputLiveV67RestoreProgram,
  validateInputLiveV67RestorePayload,
  evaluateInputLiveV67Objective,
  inputLiveV67CaptureManifestSha256,
  proveInputLiveV67Roots,
  validateInputLiveV67CaptureManifest,
  validateInputLiveV67CapturePayload,
  validateInputLiveV67CleanupPayload,
  validateInputLiveV67ExtractPayload,
  validateInputLiveV67ProbePayload,
  validateInputLiveV67WriterPayload,
  type InputLiveV67CaptureCell,
  type InputLiveV67CapturePayload,
  type InputLiveV67RootProof,
  type InputLiveV67SourceIdentity,
  type InputLiveV67WriterOwnership,
} from "./input-field-live-v67-contract.js";
import {
  INPUT_LIVE_V67_CAPTURE_MANIFEST_PATH,
  INPUT_LIVE_V67_PLAN_PATH,
  type InputLiveV67ProofPlan,
} from "./build-input-field-live-proof-v67.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-v67.js";

export const INPUT_LIVE_V67_RUNNER_VERSION = "input-live-v67-runner-v1";

interface InputLiveV67JournalEntry {
  index: number;
  event: string;
...
interface InputLiveV67OrchestratorState {
  artifactVersion: "input-live-v67-orchestrator-state-v1";
  ...
  journal: InputLiveV67JournalEntry[];
}

interface LoadedProofInputs {
  plan: InputLiveV67ProofPlan;
  planSha256: string;
  captures: InputLiveV67CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    InputLiveV67SourceIdentity & {
...
export interface InputLiveV67NextAction {
...
const writeState = (
  directory: string,
  state: InputLiveV67OrchestratorState,
): void => {
...
const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, INPUT_LIVE_V67_PLAN_PATH);
  const capturePath = path.join(root, INPUT_LIVE_V67_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as InputLiveV67ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as InputLiveV67CaptureCell[];
  validateInputLiveV67CaptureManifest(captures);
  if (
    plan.captureManifest.sha256 !== inputLiveV67Sha256(captureBytes) ||
    captureArtifact.cellsSha256 !== inputLiveV67CaptureManifestSha256(captures)
  )
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (
      inputLiveV67Sha256(expectedBytes) !== source.expectedScenePlan.sha256 ||
      inputLiveV67Sha256(uncompressed) !==
  ...
  return {
    plan,
    planSha256: inputLiveV67Sha256(planBytes),
    captures,
    captureManifestSha256: inputLiveV67Sha256(captureBytes),
    sources,
...
const appendJournal = (
  ...
  const payloadSha256 = inputLiveV67Sha256(canonicalJson(payload));
  const body = {
  ...
  const entry = {
    ...body,
    entrySha256: inputLiveV67Sha256(canonicalJson(body)),
  };
  writeNew(
    path.join(
      directory,
...
const writerOwnership = (directory: string): InputLiveV67WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): InputLiveV67RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): InputLiveV67Request =>
  readJson<InputLiveV67Request>(file);

export class InputLiveV67Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueInputLiveV67Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: InputLiveV67OrchestratorState = {
      artifactVersion: "input-live-v67-orchestrator-state-v1",
      transactionDirectory: options.transactionDirectory,
      activeRequest: inputLiveV67RequestPath(
        options.transactionDirectory,
        "writer",
      ),
      cleanupRequest: null,
    ...
    return new InputLiveV67Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<InputLiveV67OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new InputLiveV67Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): InputLiveV67NextAction {
    const cleanup =
      this.#state.cleanupRequest &&
      !this.#state.cleanupAccepted &&
      !existsSync(inputLiveV67ReceiptPath(this.#directory, "cleanup"))
        ? this.#state.cleanupRequest
        : null;
  ...
  ingestAndAdvance(rawResponse: string): InputLiveV67NextAction {
  ...
  ingestCleanup(rawResponse: string): InputLiveV67NextAction {
    ...
    persistInputLiveV67RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptInputLiveV67Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV67CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      existsSync(inputLiveV67ReceiptPath(this.#directory, "writer")) &&
      !existsSync(path.join(this.#directory, OWNERSHIP_FILE))
    ) {
      const raw = readInputLiveV67AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateInputLiveV67WriterPayload(raw),
      );
    }
    if (existsSync(inputLiveV67RequestPath(this.#directory, "cleanup")))
      this.#state.cleanupRequest = inputLiveV67RequestPath(
        this.#directory,
        "cleanup",
      );
    if (
      existsSync(path.join(this.#directory, OWNERSHIP_FILE)) &&
      !existsSync(inputLiveV67RequestPath(this.#directory, "cleanup"))
    )
      this.#issueCleanupAndRestore(false);
    else if (
      existsSync(inputLiveV67RequestPath(this.#directory, "cleanup")) &&
      !existsSync(inputLiveV67RequestPath(this.#directory, "restore"))
    )
      this.#issueRestore(false);
    if (
      existsSync(inputLiveV67RequestPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV67ReceiptPath(this.#directory, "restore"))
    )
      this.#state.activeRequest = inputLiveV67RequestPath(
        this.#directory,
        "restore",
      );
    if (
      existsSync(inputLiveV67ReceiptPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV67RequestPath(this.#directory, "extract"))
    ) {
      const ownership = writerOwnership(this.#directory);
      const extract = issueInputLiveV67Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildInputLiveV67ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV67RequestPath(
        this.#directory,
        "extract",
    ...
    if (existsSync(inputLiveV67ReceiptPath(this.#directory, "cleanup")))
      this.#state.cleanupAccepted = true;
    this.#persist();
  }

  #acceptActive(request: InputLiveV67Request): void {
    if (request.phase === "writer") {
      const accepted = acceptInputLiveV67Response(this.#directory, "writer", {
        validate: validateInputLiveV67WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptInputLiveV67Response(this.#directory, "restore", {
        validate: (payload) =>
          validateInputLiveV67RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptInputLiveV67Response(this.#directory, "extract", {
        validate: (payload) =>
          validateInputLiveV67ExtractPayload(payload, ownership),
      });
      const proofs = proveInputLiveV67Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseInputFieldRecipe,
        compileInputFieldRecipe,
      );
      assertInputLiveV67RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "input-live-v67-host-root-proofs-v1",
      ...
      appendJournal(
        ...
        request.requestId,
        { proofsSha256: inputLiveV67Sha256(canonicalJson(proofs)) },
      );
      const next = issueInputLiveV67Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildInputLiveV67ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV67RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptInputLiveV67Response(this.#directory, "probe", {
        validate: (payload) =>
          validateInputLiveV67ProbePayload(payload, ownership),
      });
      persistInputLiveV67TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      const accepted = acceptInputLiveV67Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateInputLiveV67CapturePayload(payload, cell, rawBytes),
      });
      const png = Buffer.from(accepted.payload.pngBase64, "base64");
      const capturePath = path.join(
        this.#directory,
        "captures",
        `${String(cell.index).padStart(3, "0")}-${inputLiveV67Sha256(cell.cellKey).slice(0, 20)}.png`,
      );
      ...
      if (next < INPUT_LIVE_V67_CAPTURE_COUNT) this.#issueCapture(next);
      else {
          ...
          pngBase64: Buffer.from(
            readFileSync(
              path.join(
                this.#directory,
                "captures",
                `${String(response.index).padStart(3, "0")}-${inputLiveV67Sha256(response.cellKey).slice(0, 20)}.png`,
              ),
            ),
          ).toString("base64"),
        }));
        assertInputLiveV67CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateInputLiveV67Objective(
        ...
        appendJournal(
          ...
          request.requestId,
          {
            reportSha256: inputLiveV67Sha256(canonicalJson(objective)),
  ...
  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueInputLiveV67Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildInputLiveV67CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = inputLiveV67RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueInputLiveV67Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildInputLiveV67RestoreProgram(ownership),
    );
    this.#state.activeRequest = inputLiveV67RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueInputLiveV67Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildInputLiveV67CaptureProgram(
  ...
  #readCaptureResponseIndex(): InputLiveV67CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: InputLiveV67CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedInputLiveV67Authorization = (
  privateKey: KeyObject,
): InputLiveV67TransactionAuthorization => ({
  mode: "simulated",
  ...
  signingPublicKeySha256: inputLiveV67Sha256(
...
export const generateInputLiveV67SimulationKey = (): KeyObject =>
