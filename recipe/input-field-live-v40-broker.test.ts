...
import {
  ...
  createInputLiveV40FakeDynamicResponse,
  ...
  listInputLiveV40PersistedRequests,
  persistInputLiveV40RawResponse,
  persistInputLiveV40TechnicalGates,
  validateInputLiveV40Receipt,
  validateInputLiveV40Request,
  type InputLiveV40RemotePhase,
...
import {
  ...
  type InputLiveV40ExtractPayload,
  type InputLiveV40ProbePayload,
  type InputLiveV40RestorePayload,
  type InputLiveV40RootProof,
  type InputLiveV40WriterOwnership,
} from "./input-field-live-v40-contract.js";
import {
  InputLiveV40Orchestrator,
  simulatedInputLiveV40Authorization,
} from "./run-input-field-live-v40.js";
...
const captures: InputLiveV40CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV40Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v40-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV40Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV40CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV40FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV40FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V40_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V40_TARGET);
  persistInputLiveV40RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV40Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV40Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV40WriterOwnership,
): InputLiveV40RestorePayload => ({
...
const rootProofs = (): InputLiveV40RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV40CaptureCell,
): InputLiveV40CapturePayload => {
  const bytes = Buffer.from(`input-live-v40-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV40Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV40Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV40RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV40RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV40ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV40ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV40CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV40CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV40CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV40Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV40CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV40PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV40ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV40Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV40Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV40Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV40Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V40_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV40RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V40_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV40Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV40Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV40ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV40Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV40WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV40RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV40Request;
    validateInputLiveV40Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV40Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV40CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v40-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV40Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV40Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV40Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V40_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV40RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V40_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV40Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV40Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV40RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V40_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV40Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV40Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV40Request;
