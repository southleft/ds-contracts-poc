...
import { verifyCalendarLiveV46Authorization } from "./calendar-live-v46-authorization.js";
import { CALENDAR_LIVE_V46_TARGET } from "./calendar-live-v46-broker.js";
import {
  CALENDAR_LIVE_V46_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV46Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v46-session";
...
const attestation: CalendarLiveV46SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV46SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v46 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V46_SECURITY_ATTESTATION_DEFAULT_PATH,
);
