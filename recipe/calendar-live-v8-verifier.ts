...
import {
  verifyCalendarLiveV8FixedSceneFixedPoint,
  type CalendarLiveV8FixedFixedPoint,
} from "./calendar-live-v8-fixed-point.js";

export const CALENDAR_LIVE_V8_VERIFIER_VERSION = "calendar-live-v8-verifier-v1";

export function buildCalendarLiveV8RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV8RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV8RawNode[];
}

export interface CalendarLiveV8NormalizedScene {
...
export interface CalendarLiveV8SceneProof {
...
export function normalizeCalendarLiveV8Scene(
  root: CalendarLiveV8RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV8NormalizedScene {
  const canonicalBindings: CalendarLiveV8NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV8RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV8Scene<Instance>(
  normalized: CalendarLiveV8NormalizedScene,
...
): CalendarLiveV8SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV8FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV8ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV8PreCaptureGates(payload: unknown): void {
