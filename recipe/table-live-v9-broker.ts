...
import {
  ...
  TABLE_LIVE_V9_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  TABLE_LIVE_V9_RESPONSE_CONTRACTS,
  type TableLiveV9RootProof,
} from "./table-live-v9-contract.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V9_BROKER_VERSION = "table-live-v9-broker-v1";
export const TABLE_LIVE_V9_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const TABLE_LIVE_V9_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const TABLE_LIVE_V9_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const TABLE_LIVE_V9_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const TABLE_LIVE_V9_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const TABLE_LIVE_V9_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type TableLiveV9RemotePhase =
  (typeof TABLE_LIVE_V9_REMOTE_PHASES)[number];
export type TableLiveV9AuthorizationMode = "live" | "simulated";

export interface TableLiveV9CommitPins {
...
export interface TableLiveV9TransactionAuthorization extends TableLiveV9CommitPins {
  mode: TableLiveV9AuthorizationMode;
...
export interface TableLiveV9Manifest {
  artifactVersion: "table-live-v9-broker-manifest-v2";
  brokerVersion: typeof TABLE_LIVE_V9_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof TABLE_LIVE_V9_DYNAMIC_TOOL;
  target: typeof TABLE_LIVE_V9_TARGET;
...
export interface TableLiveV9ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface TableLiveV9RequestBody {
  artifactVersion: "table-live-v9-broker-request-v2";
  brokerVersion: typeof TABLE_LIVE_V9_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: TableLiveV9RemotePhase;
  sequence: number;
  ...
  commitPins: TableLiveV9CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof TABLE_LIVE_V9_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: TableLiveV9ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface TableLiveV9Request extends TableLiveV9RequestBody {
...
export interface TableLiveV9Receipt {
  artifactVersion: "table-live-v9-broker-receipt-v2";
  brokerVersion: typeof TABLE_LIVE_V9_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: TableLiveV9RemotePhase;
...
export interface TableLiveV9AcceptedResponse<T = unknown> {
  request: TableLiveV9Request;
  receipt: TableLiveV9Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface TableLiveV9IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: TableLiveV9ResponseContract;
...
const equalTarget = (value: unknown): value is typeof TABLE_LIVE_V9_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === TABLE_LIVE_V9_TARGET.fileKey &&
  (value as BrokerTarget).fileName === TABLE_LIVE_V9_TARGET.fileName &&
  (value as BrokerTarget).editorType === TABLE_LIVE_V9_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== TABLE_LIVE_V9_TARGET.fileKey ||
    context.fileName !== TABLE_LIVE_V9_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === TABLE_LIVE_V9_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Table live v9 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= TABLE_LIVE_V9_CAPTURE_COUNT
  )
    throw new TypeError("Table live v9 capture index must be 0..19");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Table live v9 authorization pins are malformed");
};

const readManifest = (directory: string): TableLiveV9Manifest => {
  const manifest = json<TableLiveV9Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Table live v9 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "table-live-v9-broker-manifest-v2" ||
    manifest.brokerVersion !== TABLE_LIVE_V9_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== TABLE_LIVE_V9_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V9_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Table live v9 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: TableLiveV9Request,
): TableLiveV9RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: TableLiveV9RemotePhase,
  captureIndex?: number,
): TableLiveV9Request | undefined => {
  const file = tableLiveV2RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV9Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: TableLiveV9RemotePhase,
  captureIndex?: number,
): TableLiveV9Receipt | undefined => {
  const file = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV9Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: TableLiveV9Request | undefined;
  receipt: TableLiveV9Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: TableLiveV9RemotePhase,
): TableLiveV9ResponseContract => ({
  ...TABLE_LIVE_V9_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: TABLE_LIVE_V9_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: TABLE_LIVE_V9_DYNAMIC_TOOL,
      target: TABLE_LIVE_V9_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "table-live-v9-plugin-response-v2",
  brokerVersion: TABLE_LIVE_V9_BROKER_VERSION,
  target: TABLE_LIVE_V9_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*TABLE_LIVE_V9_PROGRAM_START*/${program}/*TABLE_LIVE_V9_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*TABLE_LIVE_V9_PROGRAM_START*/";
  const endMarker = "/*TABLE_LIVE_V9_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Table live v9 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createTableLiveV9Transaction(
  directory: string,
  ...
  if (existsSync(path.join(directory, MANIFEST_FILE)))
    throw new Error("Table live v9 transaction already exists");
  validateAuthorization(options.authorization);
  if (
    !SHA256.test(options.proofPlanSha256) ||
    !SHA256.test(options.captureManifestSha256)
  )
    throw new TypeError("Table live v9 transaction plan hashes malformed");
  const attempt = options.attempt ?? 1;
  if (!Number.isInteger(attempt) || attempt < 1 || attempt > 3)
    throw new TypeError("Table live v9 attempt must be 1..3");
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Table live v9 private signing key does not match authorized public-key identity",
    );
  const manifest: TableLiveV9Manifest = {
    artifactVersion: "table-live-v9-broker-manifest-v2",
    brokerVersion: TABLE_LIVE_V9_BROKER_VERSION,
    ...
    captureRequests: TABLE_LIVE_V9_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateTableLiveV9Request(
  directory: string,
  request: TableLiveV9Request,
): void {
  ...
  if (
    request.artifactVersion !== "table-live-v9-broker-request-v2" ||
    request.brokerVersion !== TABLE_LIVE_V9_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== TABLE_LIVE_V9_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Table live v9 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueTableLiveV9Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: TableLiveV9RemotePhase,
  program: string,
  options: TableLiveV9IssueOptions = {},
): TableLiveV9Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<TableLiveV9Request>(targetPath);
    validateTableLiveV9Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Table live v9 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(tableLiveV2ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Table live v9 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Table live v9 empty code envelope refused");
  if (phase === "capture") validateTableLiveV9TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Table live v9 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: TABLE_LIVE_V9_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? TABLE_LIVE_V9_SIGNED_WRITER_TIMEOUT_MS
        : TABLE_LIVE_V9_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: TableLiveV9RequestBody = {
    artifactVersion: "table-live-v9-broker-request-v2",
    brokerVersion: TABLE_LIVE_V9_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: TableLiveV9Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateTableLiveV9Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistTableLiveV9RawResponse(
  directory: string,
  phase: TableLiveV9RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Table live v9 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<TableLiveV9Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Table live v9 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = tableLiveV2ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Table live v9 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<TableLiveV9Receipt, "receiptSha256"> = {
    artifactVersion: "table-live-v9-broker-receipt-v2",
    brokerVersion: TABLE_LIVE_V9_BROKER_VERSION,
...
export function acceptTableLiveV9Response<T = unknown>(
  directory: string,
  phase: TableLiveV9RemotePhase,
  options: {
    captureIndex?: number;
...
): TableLiveV9AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Table live v9 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<TableLiveV9Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Table live v9 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "table-live-v9-plugin-response-v2" ||
    plugin?.brokerVersion !== TABLE_LIVE_V9_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Table live v9 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistTableLiveV9TechnicalGates(
  directory: string,
  rootProofs: readonly TableLiveV9RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertTableLiveV9RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Table live v9 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Table live v9 probe payload hash malformed");
  const gates = Object.fromEntries(
    TABLE_LIVE_V9_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "table-live-v9-host-technical-gates-v2",
    brokerVersion: TABLE_LIVE_V9_BROKER_VERSION,
...
export function validateTableLiveV9TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "table-live-v9-host-technical-gates-v2" ||
    ...
    TABLE_LIVE_V9_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Table live v9 technical gates artifact");
}

export function validateTableLiveV9Receipt(
  directory: string,
  phase: TableLiveV9RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateTableLiveV9Request(directory, request);
  const receipt = json<TableLiveV9Receipt>(
  ...
  if (
    receipt.artifactVersion !== "table-live-v9-broker-receipt-v2" ||
    receipt.brokerVersion !== TABLE_LIVE_V9_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Table live v9 accepted receipt: ${request.requestId}`,
    );
}

export function readTableLiveV9AcceptedRawPayload(
  directory: string,
  phase: TableLiveV9RemotePhase,
  captureIndex?: number,
): unknown {
  validateTableLiveV9Receipt(directory, phase, captureIndex);
...
export function createTableLiveV9FakeDynamicResponse(
  request: TableLiveV9Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? TABLE_LIVE_V9_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? TABLE_LIVE_V9_TARGET,
    ...overrides.extra,
  };
}

export function listTableLiveV9PersistedRequests(
  directory: string,
): TableLiveV9Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<TableLiveV9Request>(file)] : [];
    });
}
