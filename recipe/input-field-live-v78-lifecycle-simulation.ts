...
export interface InputLiveV78LifecycleSimulationReport {
  artifactVersion: "input-live-v78-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV78AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV78Lifecycle(): Promise<InputLiveV78LifecycleSimulationReport> {
  await buildInputLiveV78Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V78_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV78AntecedentIndex;
  const indexSha256 = inputLiveV78Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v78-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V78_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V78_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV78History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV78History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV78AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V78_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V78_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV78History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV78History(authorized, "pending");
    ...
    await buildInputLiveV78Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V78_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV78AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV78Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV78Lifecycle(), null, 2)}\n`,
  );
