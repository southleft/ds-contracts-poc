...
import { buildCalendarLiveV13CleanupRuntime } from "./calendar-live-v13-cleanup.js";
import { type CalendarLiveV13FixedFixedPoint } from "./calendar-live-v13-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV13RestoreProgram,
  validateCalendarLiveV13RestorePayload,
  type CalendarLiveV13RestorePayload,
} from "./calendar-live-v13-restore.js";
import {
  buildCalendarLiveV13RawPropertyRuntime,
  normalizeCalendarLiveV13Scene,
  type CalendarLiveV13RawNode,
} from "./calendar-live-v13-verifier.js";
...
export const CALENDAR_LIVE_V13_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V13_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V13_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V13_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V13_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V13_SET_COUNT = 3;
export const CALENDAR_LIVE_V13_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V13_HOST_PHASES = 3;
export const CALENDAR_LIVE_V13_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V13_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V13_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V13_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV13SourceId = (typeof CALENDAR_LIVE_V13_SOURCE_IDS)[number];
export type CalendarLiveV13Kind = "calendar" | "week" | "day";

export interface CalendarLiveV13SourceIdentity {
  source: CalendarLiveV13SourceId;
...
export interface CalendarLiveV13WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV13ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV13SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV13RawNode;
    weekScene: CalendarLiveV13RawNode;
    dayScene: CalendarLiveV13RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV13RootProof {
  source: CalendarLiveV13SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV13FixedFixedPoint;
}

export interface CalendarLiveV13ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV13SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV13CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV13SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV13CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV13SourceId;
  frameWidth: number;
...
export interface CalendarLiveV13CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v13 ${label} must be an object`);
...
export function validateCalendarLiveV13WriterPayload(
  value: unknown,
): CalendarLiveV13WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v13") ||
    ...
    result.namespace !== CALENDAR_LIVE_V13_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v13 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v13 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v13 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV13SourceIdentity,
  kind: CalendarLiveV13Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV13ExtractProgram(
  writer: CalendarLiveV13WriterOwnership,
  sources: readonly CalendarLiveV13SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v13 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V13_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V13_NAMESPACE)}
${buildCalendarLiveV13RawPropertyRuntime()}
...
export function validateCalendarLiveV13ExtractPayload(
  value: unknown,
  writer: CalendarLiveV13WriterOwnership,
): CalendarLiveV13ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v13 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V13_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v13 extract schema/one-root mismatch");
  return payload as CalendarLiveV13ExtractPayload;
}

export function proveCalendarLiveV13Roots<Instance>(
  extract: CalendarLiveV13ExtractPayload,
  sources: readonly (CalendarLiveV13SourceIdentity & {
...
): CalendarLiveV13RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v13 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v13 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV13Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV13Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV13Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV13FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v13 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v13 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v13 compile lost owned sets");
...
export function assertCalendarLiveV13RootProofs(
  proofs: readonly CalendarLiveV13RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v13 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v13 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV13ProbeProgram(
  writer: CalendarLiveV13WriterOwnership,
  sources: readonly CalendarLiveV13SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V13_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV13ProbePayload(
  value: unknown,
  writer: CalendarLiveV13WriterOwnership,
): CalendarLiveV13ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV13ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V13_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V13_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v13 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV13CaptureManifest(
  cells: readonly CalendarLiveV13CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V13_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V13_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V13_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V13_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v13 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV13CaptureCell[],
): string => {
  validateCalendarLiveV13CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV13CaptureProgram(
  writer: CalendarLiveV13WriterOwnership,
  cell: CalendarLiveV13CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V13_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v13 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v13 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V13_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV13CapturePayload(
  value: unknown,
  cell: CalendarLiveV13CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV13CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV13CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V13_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V13_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v13 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV13CaptureResponses(
  manifest: readonly CalendarLiveV13CaptureCell[],
  responses: readonly CalendarLiveV13CapturePayload[],
): void {
  validateCalendarLiveV13CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v13 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV13ObjectiveReport {
  artifactVersion: "calendar-live-v13-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV13Objective(
  manifest: readonly CalendarLiveV13CaptureCell[],
  responses: readonly CalendarLiveV13CapturePayload[],
): CalendarLiveV13ObjectiveReport {
  assertCalendarLiveV13CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v13-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV13CleanupProgram(
  writer: CalendarLiveV13WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV13CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V13_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v13 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV13CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV13WriterOwnership,
): CalendarLiveV13CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV13CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v13 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V13_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV13WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
