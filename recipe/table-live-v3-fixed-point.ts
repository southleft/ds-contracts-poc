/**
 * Table live v3 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V3_FIXED_VERIFIER_VERSION = "table-live-v3-fixed-point-v1";
export const TABLE_LIVE_V3_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV3FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV3FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV3FixedSourceProbe {
...
export interface TableLiveV3FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV3FixedFixedPoint;
}

export interface TableLiveV3FixedHardGateInput {
  thresholds: TableLiveV3FixedThresholds;
  sourceProbes: TableLiveV3FixedSourceProbe[];
  cells: TableLiveV3FixedCellValidation[];
  sceneProofs: TableLiveV3FixedSceneProof[];
...
export interface TableLiveV3FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V3_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV3FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV3FixedHardGates(
  input: TableLiveV3FixedHardGateInput,
): TableLiveV3FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV3FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  if (input.visualRows.length !== 0)
    failures.push("table live v3 must not invent a visual comparison");
  ...
  return {
    verifierVersion: TABLE_LIVE_V3_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV3FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV3FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
