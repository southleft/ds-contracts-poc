...
import { verifyCalendarLiveV7Authorization } from "./calendar-live-v7-authorization.js";
import { CALENDAR_LIVE_V7_TARGET } from "./calendar-live-v7-broker.js";
import {
  CALENDAR_LIVE_V7_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV7Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v7-session";
...
const attestation: CalendarLiveV7SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV7SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v7 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V7_SECURITY_ATTESTATION_DEFAULT_PATH,
);
