...
import { CALENDAR_LIVE_V48_TARGET } from "./calendar-live-v48-broker.js";
import {
  CALENDAR_LIVE_V48_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV48Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v48-session";
...
const attestation: CalendarLiveV48SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV48SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v48 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V48_SECURITY_ATTESTATION_DEFAULT_PATH,
);
