...
export interface InputLiveV85LifecycleSimulationReport {
  artifactVersion: "input-live-v85-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV85AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV85Lifecycle(): Promise<InputLiveV85LifecycleSimulationReport> {
  await buildInputLiveV85Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V85_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV85AntecedentIndex;
  const indexSha256 = inputLiveV85Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v85-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V85_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V85_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV85History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV85History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV85AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V85_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V85_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV85History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV85History(authorized, "pending");
    ...
    await buildInputLiveV85Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V85_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV85AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV85Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV85Lifecycle(), null, 2)}\n`,
  );
