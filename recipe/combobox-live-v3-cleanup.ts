...
export interface ComboboxLiveV3CleanupPlan {
  fileKey: string;
  fileName: string;
  editorType: string;
  namespace: string;
...
export function buildComboboxLiveV3CleanupRuntime(
  plan: ComboboxLiveV3CleanupPlan,
): string {
  ...
  return String.raw`
if(figma.fileKey!==${JSON.stringify(plan.fileKey)}||figma.root.name!==${JSON.stringify(plan.fileName)}||figma.editorType!==${JSON.stringify(plan.editorType)})throw new Error("COMBOBOX-V3-CLEANUP-WRONG-TARGET");
