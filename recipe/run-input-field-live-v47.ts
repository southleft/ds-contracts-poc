...
import {
  acceptInputLiveV47Response,
  createInputLiveV47Transaction,
  inputLiveV47ReceiptPath,
  inputLiveV47RequestPath,
  inputLiveV47ResponsePath,
  inputLiveV47Sha256,
  issueInputLiveV47Request,
  persistInputLiveV47RawResponse,
  persistInputLiveV47TechnicalGates,
  readInputLiveV47AcceptedRawPayload,
  type InputLiveV47Request,
  type InputLiveV47TransactionAuthorization,
} from "./input-field-live-v47-broker.js";
import {
  INPUT_LIVE_V47_CAPTURE_COUNT,
  assertInputLiveV47CaptureResponses,
  assertInputLiveV47RootProofs,
  buildInputLiveV47CaptureProgram,
  buildInputLiveV47CleanupProgram,
  buildInputLiveV47ExtractProgram,
  buildInputLiveV47ProbeProgram,
  buildInputLiveV47RestoreProgram,
  validateInputLiveV47RestorePayload,
  evaluateInputLiveV47Objective,
  inputLiveV47CaptureManifestSha256,
  proveInputLiveV47Roots,
  validateInputLiveV47CaptureManifest,
  validateInputLiveV47CapturePayload,
  validateInputLiveV47CleanupPayload,
  validateInputLiveV47ExtractPayload,
  validateInputLiveV47ProbePayload,
  validateInputLiveV47WriterPayload,
  type InputLiveV47CaptureCell,
  type InputLiveV47CapturePayload,
...
import {
  ...
  INPUT_LIVE_V47_PLAN_PATH,
  type InputLiveV47ProofPlan,
} from "./build-input-field-live-proof-v47.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-v47.js";

export const INPUT_LIVE_V47_RUNNER_VERSION = "input-live-v47-runner-v1";

interface InputLiveV47JournalEntry {
  index: number;
  event: string;
...
interface InputLiveV47OrchestratorState {
  artifactVersion: "input-live-v47-orchestrator-state-v1";
  ...
  journal: InputLiveV47JournalEntry[];
}

interface LoadedProofInputs {
  plan: InputLiveV47ProofPlan;
  planSha256: string;
  captures: InputLiveV47CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    InputLiveV47SourceIdentity & {
...
export interface InputLiveV47NextAction {
...
const writeState = (
  directory: string,
  state: InputLiveV47OrchestratorState,
): void => {
...
const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, INPUT_LIVE_V47_PLAN_PATH);
  const capturePath = path.join(root, INPUT_LIVE_V47_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as InputLiveV47ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as InputLiveV47CaptureCell[];
  validateInputLiveV47CaptureManifest(captures);
  if (
    plan.captureManifest.sha256 !== inputLiveV47Sha256(captureBytes) ||
    captureArtifact.cellsSha256 !== inputLiveV47CaptureManifestSha256(captures)
  )
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (
      inputLiveV47Sha256(expectedBytes) !== source.expectedScenePlan.sha256 ||
      inputLiveV47Sha256(uncompressed) !==
  ...
  return {
    plan,
    planSha256: inputLiveV47Sha256(planBytes),
    captures,
    captureManifestSha256: inputLiveV47Sha256(captureBytes),
    sources,
...
const appendJournal = (
  ...
  const payloadSha256 = inputLiveV47Sha256(canonicalJson(payload));
  const body = {
  ...
  const entry = {
    ...body,
    entrySha256: inputLiveV47Sha256(canonicalJson(body)),
  };
  writeNew(
    path.join(
      directory,
...
const writerOwnership = (directory: string): InputLiveV47WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): InputLiveV47RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): InputLiveV47Request =>
  readJson<InputLiveV47Request>(file);

export class InputLiveV47Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueInputLiveV47Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: InputLiveV47OrchestratorState = {
      artifactVersion: "input-live-v47-orchestrator-state-v1",
      transactionDirectory: options.transactionDirectory,
      activeRequest: inputLiveV47RequestPath(
        options.transactionDirectory,
        "writer",
      ),
      cleanupRequest: null,
    ...
    return new InputLiveV47Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<InputLiveV47OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new InputLiveV47Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): InputLiveV47NextAction {
    const cleanup =
      this.#state.cleanupRequest &&
      !this.#state.cleanupAccepted &&
      !existsSync(inputLiveV47ReceiptPath(this.#directory, "cleanup"))
        ? this.#state.cleanupRequest
        : null;
  ...
  ingestAndAdvance(rawResponse: string): InputLiveV47NextAction {
  ...
  ingestCleanup(rawResponse: string): InputLiveV47NextAction {
    ...
    persistInputLiveV47RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptInputLiveV47Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV47CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      existsSync(inputLiveV47ReceiptPath(this.#directory, "writer")) &&
      !existsSync(path.join(this.#directory, OWNERSHIP_FILE))
    ) {
      const raw = readInputLiveV47AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateInputLiveV47WriterPayload(raw),
      );
    }
    if (existsSync(inputLiveV47RequestPath(this.#directory, "cleanup")))
      this.#state.cleanupRequest = inputLiveV47RequestPath(
        this.#directory,
        "cleanup",
      );
    if (
      existsSync(path.join(this.#directory, OWNERSHIP_FILE)) &&
      !existsSync(inputLiveV47RequestPath(this.#directory, "cleanup"))
    )
      this.#issueCleanupAndRestore(false);
    else if (
      existsSync(inputLiveV47RequestPath(this.#directory, "cleanup")) &&
      !existsSync(inputLiveV47RequestPath(this.#directory, "restore"))
    )
      this.#issueRestore(false);
    if (
      existsSync(inputLiveV47RequestPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV47ReceiptPath(this.#directory, "restore"))
    )
      this.#state.activeRequest = inputLiveV47RequestPath(
        this.#directory,
        "restore",
      );
    if (
      existsSync(inputLiveV47ReceiptPath(this.#directory, "restore")) &&
      !existsSync(inputLiveV47RequestPath(this.#directory, "extract"))
    ) {
      const ownership = writerOwnership(this.#directory);
      const extract = issueInputLiveV47Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildInputLiveV47ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV47RequestPath(
        this.#directory,
        "extract",
    ...
    if (existsSync(inputLiveV47ReceiptPath(this.#directory, "cleanup")))
      this.#state.cleanupAccepted = true;
    this.#persist();
  }

  #acceptActive(request: InputLiveV47Request): void {
    if (request.phase === "writer") {
      const accepted = acceptInputLiveV47Response(this.#directory, "writer", {
        validate: validateInputLiveV47WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptInputLiveV47Response(this.#directory, "restore", {
        validate: (payload) =>
          validateInputLiveV47RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptInputLiveV47Response(this.#directory, "extract", {
        validate: (payload) =>
          validateInputLiveV47ExtractPayload(payload, ownership),
      });
      const proofs = proveInputLiveV47Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseInputFieldRecipe,
        compileInputFieldRecipe,
      );
      assertInputLiveV47RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "input-live-v47-host-root-proofs-v1",
      ...
      appendJournal(
        ...
        request.requestId,
        { proofsSha256: inputLiveV47Sha256(canonicalJson(proofs)) },
      );
      const next = issueInputLiveV47Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildInputLiveV47ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV47RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptInputLiveV47Response(this.#directory, "probe", {
        validate: (payload) =>
          validateInputLiveV47ProbePayload(payload, ownership),
      });
      persistInputLiveV47TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      const accepted = acceptInputLiveV47Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateInputLiveV47CapturePayload(payload, cell, rawBytes),
      });
      const png = Buffer.from(accepted.payload.pngBase64, "base64");
      const capturePath = path.join(
        this.#directory,
        "captures",
        `${String(cell.index).padStart(3, "0")}-${inputLiveV47Sha256(cell.cellKey).slice(0, 20)}.png`,
      );
      ...
      if (next < INPUT_LIVE_V47_CAPTURE_COUNT) this.#issueCapture(next);
      else {
          ...
          pngBase64: Buffer.from(
            readFileSync(
              path.join(
                this.#directory,
                "captures",
                `${String(response.index).padStart(3, "0")}-${inputLiveV47Sha256(response.cellKey).slice(0, 20)}.png`,
              ),
            ),
          ).toString("base64"),
        }));
        assertInputLiveV47CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateInputLiveV47Objective(
        ...
        appendJournal(
          ...
          request.requestId,
          {
            reportSha256: inputLiveV47Sha256(canonicalJson(objective)),
  ...
  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueInputLiveV47Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildInputLiveV47CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = inputLiveV47RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueInputLiveV47Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildInputLiveV47RestoreProgram(ownership),
    );
    this.#state.activeRequest = inputLiveV47RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueInputLiveV47Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildInputLiveV47CaptureProgram(
  ...
  #readCaptureResponseIndex(): InputLiveV47CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: InputLiveV47CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedInputLiveV47Authorization = (
  privateKey: KeyObject,
): InputLiveV47TransactionAuthorization => ({
  mode: "simulated",
  ...
  signingPublicKeySha256: inputLiveV47Sha256(
...
export const generateInputLiveV47SimulationKey = (): KeyObject =>
