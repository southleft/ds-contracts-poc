...
import { verifyCalendarLiveV1Authorization } from "./calendar-live-v1-authorization.js";
import { CALENDAR_LIVE_V1_TARGET } from "./calendar-live-v1-broker.js";
import {
  CALENDAR_LIVE_V1_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV1Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v1-session";
...
const attestation: CalendarLiveV1SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV1SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v1 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V1_SECURITY_ATTESTATION_DEFAULT_PATH,
);
