...
import { INPUT_LIVE_V38_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v38.js";
import { verifyInputLiveV38Authorization } from "./input-field-live-v38-authorization.js";
import { runInputLiveV38Preflight } from "./input-field-live-v38-preflight.js";
import { InputLiveV38Orchestrator } from "./run-input-field-live-v38.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V38_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v38-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV38Authorization(root);
    runInputLiveV38Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV38Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
