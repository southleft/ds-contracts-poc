...
import { COMBOBOX_LIVE_V37_INDEX_PATH } from "./build-combobox-live-proof-v37.js";
import {
  COMBOBOX_LIVE_V37_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V37_TARGET,
} from "./combobox-live-v37-broker.js";
import {
  buildComboboxLiveV37AuthorizationArtifact,
  validateComboboxLiveV37AntecedentIndex,
  validateComboboxLiveV37AuthorizationArtifact,
  validateComboboxLiveV37History,
  validateComboboxLiveV37UnitTestSource,
  type ComboboxLiveV37AntecedentIndex,
  type ComboboxLiveV37AuthorizationArtifact,
  type ComboboxLiveV37AuthorizationProof,
  type ComboboxLiveV37HistoryState,
} from "./combobox-live-v37-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV37ControlFlowSource,
  validateComboboxLiveV37SecurityAttestation,
  type ComboboxLiveV37SecurityAttestation,
} from "./combobox-live-v37-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v37-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V37_INDEX_PATH);
const index = (): ComboboxLiveV37AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV37AntecedentIndex;

const artifact = (): ComboboxLiveV37AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV37AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV37HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV37HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV37AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v37/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV37AuthorizationArtifact,
): ComboboxLiveV37SecurityAttestation => {
  const body: Omit<ComboboxLiveV37SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V37_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV37History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV37History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV37History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV37History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV37History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV37UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV37UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV37AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV37AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV37AuthorizationArtifact(
        planted as ComboboxLiveV37AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV37SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV37SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV37SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV37ControlFlowSource(source), []);
});
