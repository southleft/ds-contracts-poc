...
export interface InputLiveV12LifecycleSimulationReport {
  artifactVersion: "input-live-v12-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV12AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV12Lifecycle(): Promise<InputLiveV12LifecycleSimulationReport> {
  await buildInputLiveV12Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V12_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV12AntecedentIndex;
  const indexSha256 = inputLiveV12Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v12-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V12_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V12_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV12History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV12History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV12AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V12_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V12_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV12History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV12History(authorized, "pending");
    ...
    await buildInputLiveV12Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V12_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV12AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV12Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV12Lifecycle(), null, 2)}\n`,
  );
