...
export type CalendarLiveV10HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV10AuthorizationArtifact {
  artifactVersion: "calendar-live-v10-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V10_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV10AntecedentIndex {
...
export interface CalendarLiveV10HistoryState {
  index?: CalendarLiveV10AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV10AuthorizationArtifact;
...
export interface CalendarLiveV10AuthorizationProof
  extends CalendarLiveV10TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V10_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V10_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V10_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V10_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v10-authorization") ||
  artifactPath.includes("calendar-live-v10-preflight") ||
  artifactPath.includes("calendar-live-v10-authorized") ||
  artifactPath.includes("create-calendar-live-v10-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v10-status") ||
...
export function validateCalendarLiveV10AntecedentIndex(
  index: CalendarLiveV10AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v10-antecedent-index-v1" ||
...
export function validateCalendarLiveV10UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV10History\(\s*["']/.test(source) ||
    /readCalendarLiveV10HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV10AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV10AuthorizationArtifact {
  ...
  ) as CalendarLiveV10AntecedentIndex;
  const indexFailures = validateCalendarLiveV10AntecedentIndex(index);
...
export function validateCalendarLiveV10AuthorizationArtifact(
  artifact: CalendarLiveV10AuthorizationArtifact | undefined,
  index: CalendarLiveV10AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v10-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v10" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V10_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V10_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V10_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V10_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V10_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V10_SOURCE_ROOTS ||
...
export function validateCalendarLiveV10History(
  state: CalendarLiveV10HistoryState,
  expected: CalendarLiveV10HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV10AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV10AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV10HistoryState(
  root = process.cwd(),
): CalendarLiveV10HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V10_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV10AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V10_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V10_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V10_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV10History(
  expected: CalendarLiveV10HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV10HistoryState {
  const state = readCalendarLiveV10HistoryState(root);
  const failures = validateCalendarLiveV10History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v10 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV10Authorization(
  root = process.cwd(),
): CalendarLiveV10AuthorizationProof {
  const state = verifyCalendarLiveV10History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V10_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v10/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v10.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V10_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V10_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V10_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV10HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV10History(expected);
  process.stdout.write(
    `Calendar live v10 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
