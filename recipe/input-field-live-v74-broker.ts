...
import {
  assertInputLiveV74RootProofs,
  INPUT_LIVE_V74_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V74_RESPONSE_CONTRACTS,
  type InputLiveV74RootProof,
} from "./input-field-live-v74-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V74_BROKER_VERSION = "input-live-v74-broker-v2";
export const INPUT_LIVE_V74_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V74_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V74_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V74_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V74_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V74_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV74RemotePhase =
  (typeof INPUT_LIVE_V74_REMOTE_PHASES)[number];
export type InputLiveV74AuthorizationMode = "live" | "simulated";

export interface InputLiveV74CommitPins {
...
export interface InputLiveV74TransactionAuthorization extends InputLiveV74CommitPins {
  mode: InputLiveV74AuthorizationMode;
...
export interface InputLiveV74Manifest {
  artifactVersion: "input-live-v74-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V74_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V74_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V74_TARGET;
  authorization: InputLiveV74TransactionAuthorization;
...
export interface InputLiveV74ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV74RequestBody {
  artifactVersion: "input-live-v74-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V74_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV74RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV74CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V74_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV74ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV74Request extends InputLiveV74RequestBody {
...
export interface InputLiveV74Receipt {
  artifactVersion: "input-live-v74-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V74_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV74RemotePhase;
...
export interface InputLiveV74AcceptedResponse<T = unknown> {
  request: InputLiveV74Request;
  receipt: InputLiveV74Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV74IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV74ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V74_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V74_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V74_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V74_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V74_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V74_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V74_TARGET.editorType;
};
...
export const inputLiveV74RequestSequence = (
  phase: InputLiveV74RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV74RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV74RequestPath = (
  directory: string,
  phase: InputLiveV74RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV74ResponsePath = (
  directory: string,
  phase: InputLiveV74RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV74ReceiptPath = (
  directory: string,
  phase: InputLiveV74RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV74Manifest => {
  const manifest = json<InputLiveV74Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v74-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V74_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V74_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV74RemotePhase,
  captureIndex?: number,
): InputLiveV74Request | undefined => {
  const file = inputLiveV74RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV74Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV74RemotePhase,
  captureIndex?: number,
): InputLiveV74Receipt | undefined => {
  const file = inputLiveV74ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV74Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV74Request | undefined;
  receipt: InputLiveV74Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV74RemotePhase,
): InputLiveV74ResponseContract => ({
  ...INPUT_LIVE_V74_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V74_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V74_DYNAMIC_TOOL,
      target: INPUT_LIVE_V74_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v74-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V74_BROKER_VERSION,
  target: INPUT_LIVE_V74_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V74_PROGRAM_START*/${program}/*INPUT_LIVE_V74_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V74_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V74_PROGRAM_END*/";
...
export function createInputLiveV74Transaction(
  directory: string,
  ...
  const manifest: InputLiveV74Manifest = {
    artifactVersion: "input-live-v74-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V74_BROKER_VERSION,
...
export function validateInputLiveV74Request(
  directory: string,
  request: InputLiveV74Request,
): void {
  ...
  const expectedSequence = inputLiveV74RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v74-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V74_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V74_TARGET.fileKey ||
...
export function issueInputLiveV74Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV74RemotePhase,
  program: string,
  options: InputLiveV74IssueOptions = {},
): InputLiveV74Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV74RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV74Request>(targetPath);
    validateInputLiveV74Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV74ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV74TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV74RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V74_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V74_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V74_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV74RequestBody = {
    artifactVersion: "input-live-v74-broker-request-v2",
    brokerVersion: INPUT_LIVE_V74_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV74Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV74Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV74RawResponse(
  directory: string,
  phase: InputLiveV74RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV74RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV74Request>(requestFile);
  if (
  ...
  const target = inputLiveV74ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV74Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v74-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V74_BROKER_VERSION,
...
export function acceptInputLiveV74Response<T = unknown>(
  directory: string,
  phase: InputLiveV74RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV74AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV74ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV74Request>(
    inputLiveV74RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV74Request(directory, request);
  if (phase === "capture") validateInputLiveV74TechnicalGates(directory);
  const rawFile = inputLiveV74ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v74-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V74_BROKER_VERSION ||
...
export function persistInputLiveV74TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV74RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV74RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V74_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v74-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V74_BROKER_VERSION,
...
export function validateInputLiveV74TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v74-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V74_TECHNICAL_GATES.some(
...
export function validateInputLiveV74Receipt(
  directory: string,
  phase: InputLiveV74RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV74Request(directory, request);
  const receipt = json<InputLiveV74Receipt>(
    inputLiveV74ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV74ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v74-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V74_BROKER_VERSION ||
...
export function readInputLiveV74AcceptedRawPayload(
  directory: string,
  phase: InputLiveV74RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV74Receipt(directory, phase, captureIndex);
...
export function createInputLiveV74FakeDynamicResponse(
  request: InputLiveV74Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V74_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V74_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV74PersistedRequests(
  directory: string,
): InputLiveV74Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV74Request>(file)] : [];
    });
}

export const inputLiveV74Sha256 = sha256;
