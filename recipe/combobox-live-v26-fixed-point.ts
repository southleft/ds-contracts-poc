/**
 * Carried Combobox live v26 host verifier. Uses scene-readback-combobox-v26 so live
...
export const COMBOBOX_LIVE_V26_FIXED_VERIFIER_VERSION = "combobox-live-v26-fixed-point-v1";
export const COMBOBOX_LIVE_V26_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV26FixedThresholds {
...
export interface ComboboxLiveV26FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV26FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV26FixedSourceProbe {
...
export interface ComboboxLiveV26FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV26FixedFixedPoint;
}

export interface ComboboxLiveV26FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV26FixedHardGateInput {
  thresholds: ComboboxLiveV26FixedThresholds;
  materialRegression: ComboboxLiveV26FixedMaterialRegression;
  sourceProbes: ComboboxLiveV26FixedSourceProbe[];
  cells: ComboboxLiveV26FixedCellValidation[];
  sceneProofs: ComboboxLiveV26FixedSceneProof[];
  visualRows: ComboboxLiveV26FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV26FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V26_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV26FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV26FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV26FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV26FixedHardGates(
  input: ComboboxLiveV26FixedHardGateInput,
): ComboboxLiveV26FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV26FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV26FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V26_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV26FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV26FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV26FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
