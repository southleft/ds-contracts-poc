...
import { COMBOBOX_LIVE_V32_TARGET } from "./combobox-live-v32-broker.js";
import {
  COMBOBOX_LIVE_V32_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV32Authorization();
...
const attestation: ComboboxLiveV32SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV32SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v32 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V32_SECURITY_ATTESTATION_DEFAULT_PATH,
);
