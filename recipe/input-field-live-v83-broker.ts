...
import {
  ...
  INPUT_LIVE_V83_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V83_RESPONSE_CONTRACTS,
  type InputLiveV83RootProof,
} from "./input-field-live-v83-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V83_BROKER_VERSION = "input-live-v83-broker-v2";
export const INPUT_LIVE_V83_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V83_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V83_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V83_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V83_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V83_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV83RemotePhase =
  (typeof INPUT_LIVE_V83_REMOTE_PHASES)[number];
export type InputLiveV83AuthorizationMode = "live" | "simulated";

export interface InputLiveV83CommitPins {
...
export interface InputLiveV83TransactionAuthorization extends InputLiveV83CommitPins {
  mode: InputLiveV83AuthorizationMode;
...
export interface InputLiveV83Manifest {
  artifactVersion: "input-live-v83-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V83_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V83_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V83_TARGET;
  authorization: InputLiveV83TransactionAuthorization;
...
export interface InputLiveV83ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV83RequestBody {
  ...
  commitPins: InputLiveV83CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V83_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV83ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV83Request extends InputLiveV83RequestBody {
...
export interface InputLiveV83Receipt {
  artifactVersion: "input-live-v83-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V83_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV83RemotePhase;
...
export interface InputLiveV83AcceptedResponse<T = unknown> {
  request: InputLiveV83Request;
  receipt: InputLiveV83Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV83IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV83ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V83_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V83_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V83_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V83_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V83_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V83_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V83_TARGET.editorType;
};
...
export const inputLiveV83RequestSequence = (
  phase: InputLiveV83RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV83RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV83RequestPath = (
  directory: string,
  phase: InputLiveV83RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV83ResponsePath = (
  directory: string,
  phase: InputLiveV83RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV83ReceiptPath = (
  directory: string,
  phase: InputLiveV83RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV83Manifest => {
  const manifest = json<InputLiveV83Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v83-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V83_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V83_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV83RemotePhase,
  captureIndex?: number,
): InputLiveV83Request | undefined => {
  const file = inputLiveV83RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV83Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV83RemotePhase,
  captureIndex?: number,
): InputLiveV83Receipt | undefined => {
  const file = inputLiveV83ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV83Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV83Request | undefined;
  receipt: InputLiveV83Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV83RemotePhase,
): InputLiveV83ResponseContract => ({
  ...INPUT_LIVE_V83_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V83_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v83-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V83_BROKER_VERSION,
  target: INPUT_LIVE_V83_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V83_PROGRAM_START*/${program}/*INPUT_LIVE_V83_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V83_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V83_PROGRAM_END*/";
...
export function createInputLiveV83Transaction(
  directory: string,
  ...
  const manifest: InputLiveV83Manifest = {
    artifactVersion: "input-live-v83-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V83_BROKER_VERSION,
...
export function validateInputLiveV83Request(
  directory: string,
  request: InputLiveV83Request,
): void {
  ...
  const expectedSequence = inputLiveV83RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v83-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V83_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V83_TARGET.fileKey ||
...
export function issueInputLiveV83Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV83RemotePhase,
  program: string,
  options: InputLiveV83IssueOptions = {},
): InputLiveV83Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV83RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV83Request>(targetPath);
    validateInputLiveV83Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV83ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV83TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV83RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    ...
    timeout:
      ...
      (phase === "writer"
        ...
        : INPUT_LIVE_V83_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV83RequestBody = {
    artifactVersion: "input-live-v83-broker-request-v2",
    brokerVersion: INPUT_LIVE_V83_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV83Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV83Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV83RawResponse(
  directory: string,
  phase: InputLiveV83RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV83RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV83Request>(requestFile);
  if (
  ...
  const target = inputLiveV83ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV83Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v83-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V83_BROKER_VERSION,
...
export function acceptInputLiveV83Response<T = unknown>(
  directory: string,
  phase: InputLiveV83RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV83AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV83ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV83Request>(
    inputLiveV83RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV83Request(directory, request);
  if (phase === "capture") validateInputLiveV83TechnicalGates(directory);
  const rawFile = inputLiveV83ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v83-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V83_BROKER_VERSION ||
...
export function persistInputLiveV83TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV83RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV83RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V83_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v83-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V83_BROKER_VERSION,
...
export function validateInputLiveV83TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v83-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V83_TECHNICAL_GATES.some(
...
export function validateInputLiveV83Receipt(
  directory: string,
  phase: InputLiveV83RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV83Request(directory, request);
  const receipt = json<InputLiveV83Receipt>(
    inputLiveV83ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV83ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v83-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V83_BROKER_VERSION ||
...
export function readInputLiveV83AcceptedRawPayload(
  directory: string,
  phase: InputLiveV83RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV83Receipt(directory, phase, captureIndex);
...
export function createInputLiveV83FakeDynamicResponse(
  request: InputLiveV83Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V83_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V83_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV83PersistedRequests(
  directory: string,
): InputLiveV83Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV83Request>(file)] : [];
    });
}

export const inputLiveV83Sha256 = sha256;
