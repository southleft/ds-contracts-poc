...
test("host omits empty extract instance payload; writer is unchanged", () => {
  const host = readFileSync("recipe/scene-readback-combobox-v41.ts", "utf8");
...
test("host observe recovers compile-carried option-set name, not the live display name; writer is unchanged", () => {
  ...
  assert.match(
    host,
    /if \(sceneRole\(scene\) === "combobox\/option-set"\)\s*return COMBOBOX_LIVE_V41_OPTION_SET_COMPILE_CARRY_NAME/,
  );
  assert.match(
    host,
    /COMBOBOX_LIVE_V41_OPTION_SET_COMPILE_CARRY_NAME =\s*`combobox\/option-set :: \$\{COMBOBOX_LIVE_V41_OPTION_SET_COMPILE_CARRY_LABEL\}`/,
  );
  assert.match(
    host,
    /COMBOBOX_LIVE_V41_OPTION_SET_COMPILE_CARRY_LABEL =\s*"Combobox option"/,
  );
...
test("host emits compile-carried trigger characters Choose a person on empty-content combobox/input; writer is unchanged", () => {
  ...
  assert.match(
    host,
    /if \(\s*sceneRole\(scene\) === "combobox\/input" &&\s*variantProperties\?\.Content === "empty"\s*\)\s*return COMBOBOX_LIVE_V41_TRIGGER_COMPILE_CARRY_CHARACTERS/,
  );
  assert.match(
    host,
    /COMBOBOX_LIVE_V41_TRIGGER_COMPILE_CARRY_CHARACTERS =\s+"Choose a person"/,
  );
  assert.doesNotMatch(host, /if \(combobox\) \{/);
  assert.doesNotMatch(
    host,
    /COMBOBOX_LIVE_V41_TRIGGER_COMPILE_CARRY_CHARACTERS =\s+"Ada Lovelace"/,
  );
...
test("host emits compile-carried label Combobox option on combobox/option-set; writer is unchanged", () => {
  ...
  assert.match(
    host,
    /if \(sceneRole\(scene\) === "combobox\/option-set"\)\s+return COMBOBOX_LIVE_V41_OPTION_SET_COMPILE_CARRY_LABEL/,
  );
  assert.match(
    host,
    /COMBOBOX_LIVE_V41_OPTION_SET_COMPILE_CARRY_LABEL =\s+"Combobox option"/,
...
test("host probe keeps exactSceneRestoration after the open-variant walk when resize and property restore pass; writer is unchanged", () => {
  const contract = readFileSync("recipe/combobox-live-v41-contract.ts", "utf8");
  assert.match(
    contract,
...
test("host probe measures open-options pairs then excludes overlay option/label against in-flow helper/error; writer is unchanged", () => {
  const contract = readFileSync("recipe/combobox-live-v41-contract.ts", "utf8");
  assert.match(
    contract,
  ...
  assert.match(
    contract,
    /overlayOptionRole=r=>r==="combobox\/option\/label"\|\|r==="combobox\/option\/selected-indicator"/,
  );
  assert.match(
    contract,
    /messageRole=r=>r==="combobox\/message\/helper"\|\|r==="combobox\/message\/error"/,
  );
  assert.match(contract, /overlapPairs:measuredPairs/);
  const hashed = readFileSync("recipe/combobox-live-v40-contract.ts", "utf8");
  assert.doesNotMatch(
    hashed,
    /COMBOBOX-PROBE-MEASURE-THEN-EXCLUDE-OVERLAY-OPTION-LABEL-MESSAGE-OVERLAP/,
  );
