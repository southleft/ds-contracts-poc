/**
 * Calendar live v18 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V18_FIXED_VERIFIER_VERSION = "calendar-live-v18-fixed-point-v1";
export const CALENDAR_LIVE_V18_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV18FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV18FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV18FixedSourceProbe {
...
export interface CalendarLiveV18FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV18FixedFixedPoint;
}

export interface CalendarLiveV18FixedHardGateInput {
  thresholds: CalendarLiveV18FixedThresholds;
  sourceProbes: CalendarLiveV18FixedSourceProbe[];
  cells: CalendarLiveV18FixedCellValidation[];
  sceneProofs: CalendarLiveV18FixedSceneProof[];
...
export interface CalendarLiveV18FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V18_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV18FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV18FixedHardGates(
  input: CalendarLiveV18FixedHardGateInput,
): CalendarLiveV18FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV18FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V18_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV18FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV18FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV18FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
