...
import {
  ...
  verifyInputLiveV8Authorization,
  type InputLiveV8AntecedentIndex,
  type InputLiveV8AuthorizationProof,
} from "./input-field-live-v8-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V8_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v8/operator-security-attestation-template.json";
export const INPUT_LIVE_V8_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v8-security-attestation.json";

export interface InputLiveV8SecurityAttestation {
...
export interface InputLiveV8PreflightReport {
  artifactVersion: "input-live-v8-preflight-v1";
...
export const inputLiveV8AttestationSha256 = (
  value: Omit<InputLiveV8SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV8SecurityAttestation(
  value: unknown,
  proof: InputLiveV8AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v8-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V8_TARGET) ||
...
export function validateInputLiveV8ControlFlowSource(source: string): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV8Authorization");
  const preflight = source.indexOf("runInputLiveV8Preflight");
  const initialize = source.indexOf("InputLiveV8Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV8Preflight(
  root: string,
  proof: InputLiveV8AuthorizationProof,
  attestationPath = INPUT_LIVE_V8_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV8PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V8_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V8_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V8_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV8AntecedentIndex;
  failures.push(...validateInputLiveV8AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V8_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V8_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v8-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV8ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(`Input live v8 preflight refused:\n${failures.join("\n")}`);
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as InputLiveV8SecurityAttestation;
  return {
    artifactVersion: "input-live-v8-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V8_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV8Preflight(
    process.cwd(),
    verifyInputLiveV8Authorization(),
    attestationPath,
