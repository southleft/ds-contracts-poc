...
import { tableLiveV2Sha256 } from "./table-live-v28-broker.js";

export interface TableLiveV28LifecycleSimulationReport {
  artifactVersion: "table-live-v28-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV28AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV28Lifecycle(): Promise<TableLiveV28LifecycleSimulationReport> {
  await buildTableLiveV28Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V28_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV28AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v28-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V28_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V28_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV28History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV28History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV28AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V28_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V28_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV28History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV28History(authorized, "pending");
    ...
    await buildTableLiveV28Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V28_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV28AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV28Lifecycle(), null, 2)}\n`,
  );
