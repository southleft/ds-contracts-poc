...
import {
  ...
  verifyInputLiveV20Authorization,
  type InputLiveV20AntecedentIndex,
  type InputLiveV20AuthorizationProof,
} from "./input-field-live-v20-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V20_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v20/operator-security-attestation-template.json";
export const INPUT_LIVE_V20_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v20-security-attestation.json";

export interface InputLiveV20SecurityAttestation {
...
export interface InputLiveV20PreflightReport {
  artifactVersion: "input-live-v20-preflight-v1";
...
export const inputLiveV20AttestationSha256 = (
  value: Omit<InputLiveV20SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV20SecurityAttestation(
  value: unknown,
  proof: InputLiveV20AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v20-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V20_TARGET) ||
...
export function validateInputLiveV20ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV20Authorization");
  const preflight = source.indexOf("runInputLiveV20Preflight");
  const initialize = source.indexOf("InputLiveV20Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV20Preflight(
  root: string,
  proof: InputLiveV20AuthorizationProof,
  attestationPath = INPUT_LIVE_V20_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV20PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V20_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V20_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V20_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV20AntecedentIndex;
  failures.push(...validateInputLiveV20AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V20_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V20_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v20-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV20ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV20SecurityAttestation;
  return {
    artifactVersion: "input-live-v20-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V20_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV20Preflight(
    process.cwd(),
    verifyInputLiveV20Authorization(),
    attestationPath,
