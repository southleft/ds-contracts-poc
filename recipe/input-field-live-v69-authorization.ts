...
import {
  ...
  INPUT_LIVE_V69_INDEX_PATH,
} from "./build-input-field-live-proof-v69.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV69HistoryExpectation = "pending" | "authorized";

export interface InputLiveV69AuthorizationArtifact {
  artifactVersion: "input-live-v69-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V69_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV69AntecedentIndex {
...
export interface InputLiveV69HistoryState {
  index?: InputLiveV69AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV69AuthorizationArtifact;
...
export interface InputLiveV69AuthorizationProof extends InputLiveV69TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V69_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V69_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V69_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V69_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v69-authorization") ||
  artifactPath.includes("input-field-live-v69-preflight") ||
  artifactPath.includes("input-field-live-v69-authorized") ||
  artifactPath.includes("input-field-live-pivot-v69-status") ||
...
export function validateInputLiveV69AntecedentIndex(
  index: InputLiveV69AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v69-antecedent-index-v1" ||
...
export function validateInputLiveV69UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV69History\(\s*["']/.test(source) ||
    /readInputLiveV69HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV69AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV69AuthorizationArtifact {
  ...
  ) as InputLiveV69AntecedentIndex;
  const indexFailures = validateInputLiveV69AntecedentIndex(index);
...
export function validateInputLiveV69AuthorizationArtifact(
  artifact: InputLiveV69AuthorizationArtifact | undefined,
  index: InputLiveV69AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v69-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v69" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V69_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V69_TARGET) ||
    !equalJson(
...
export function validateInputLiveV69History(
  state: InputLiveV69HistoryState,
  expected: InputLiveV69HistoryExpectation,
): string[] {
  const failures = validateInputLiveV69AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV69AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV69HistoryState(
  root = process.cwd(),
): InputLiveV69HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V69_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV69AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V69_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V69_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V69_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV69History(
  expected: InputLiveV69HistoryExpectation,
  root = process.cwd(),
): InputLiveV69HistoryState {
  const state = readInputLiveV69HistoryState(root);
  const failures = validateInputLiveV69History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV69Authorization(
  root = process.cwd(),
): InputLiveV69AuthorizationProof {
  const state = verifyInputLiveV69History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V69_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v69/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v69.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V69_TARGET,
    expectedDynamicTool: INPUT_LIVE_V69_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V69_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV69HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV69History(expected);
