...
import {
  validateComboboxLiveV37AntecedentIndex,
  verifyComboboxLiveV37Authorization,
  type ComboboxLiveV37AntecedentIndex,
  type ComboboxLiveV37AuthorizationProof,
} from "./combobox-live-v37-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V37_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v37/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V37_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v37-security-attestation.json";

export interface ComboboxLiveV37SecurityAttestation {
  artifactVersion: "combobox-live-v37-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV37PreflightReport {
  artifactVersion: "combobox-live-v37-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV37SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV37SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV37AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v37-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V37_TARGET) ||
...
export function validateComboboxLiveV37ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV37Authorization");
  const preflight = source.indexOf("runComboboxLiveV37Preflight");
  const initialize = source.indexOf("ComboboxLiveV37Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV37Preflight(
  root: string,
  proof: ComboboxLiveV37AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V37_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV37PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V37_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V37_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V37_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV37AntecedentIndex;
  failures.push(...validateComboboxLiveV37AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V37_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V37_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV37SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v37-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV37ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v37 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV37SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v37-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V37_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV37Preflight(
    process.cwd(),
    verifyComboboxLiveV37Authorization(),
    attestationPath,
