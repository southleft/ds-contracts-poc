...
export interface InputLiveV48LifecycleSimulationReport {
  artifactVersion: "input-live-v48-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV48AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV48Lifecycle(): Promise<InputLiveV48LifecycleSimulationReport> {
  await buildInputLiveV48Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V48_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV48AntecedentIndex;
  const indexSha256 = inputLiveV48Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v48-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V48_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V48_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV48History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV48History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV48AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V48_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V48_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV48History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV48History(authorized, "pending");
    ...
    await buildInputLiveV48Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V48_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV48AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV48Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV48Lifecycle(), null, 2)}\n`,
  );
