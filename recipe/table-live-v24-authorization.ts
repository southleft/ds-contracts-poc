...
export type TableLiveV24HistoryExpectation = "pending" | "authorized";

export interface TableLiveV24AuthorizationArtifact {
  artifactVersion: "table-live-v24-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V24_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV24AntecedentIndex {
...
export interface TableLiveV24HistoryState {
  index?: TableLiveV24AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV24AuthorizationArtifact;
...
export interface TableLiveV24AuthorizationProof
  extends TableLiveV24TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V24_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V24_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V24_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V24_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v24-authorization") ||
  artifactPath.includes("table-live-v24-preflight") ||
  artifactPath.includes("table-live-v24-authorized") ||
  artifactPath.includes("create-table-live-v24-security-attestation") ||
  artifactPath.includes("table-live-pivot-v24-status") ||
...
export function validateTableLiveV24AntecedentIndex(
  index: TableLiveV24AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v24-antecedent-index-v1" ||
...
export function validateTableLiveV24UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV24History\(\s*["']/.test(source) ||
    /readTableLiveV24HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV24AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV24AuthorizationArtifact {
  ...
  ) as TableLiveV24AntecedentIndex;
  const indexFailures = validateTableLiveV24AntecedentIndex(index);
...
export function validateTableLiveV24AuthorizationArtifact(
  artifact: TableLiveV24AuthorizationArtifact | undefined,
  index: TableLiveV24AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v24-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v24" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V24_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V24_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V24_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V24_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V24_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V24_SOURCE_ROOTS ||
...
export function validateTableLiveV24History(
  state: TableLiveV24HistoryState,
  expected: TableLiveV24HistoryExpectation,
): string[] {
  const failures = validateTableLiveV24AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV24AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV24HistoryState(
  root = process.cwd(),
): TableLiveV24HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V24_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV24AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V24_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V24_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V24_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV24History(
  expected: TableLiveV24HistoryExpectation,
  root = process.cwd(),
): TableLiveV24HistoryState {
  const state = readTableLiveV24HistoryState(root);
  const failures = validateTableLiveV24History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v24 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV24Authorization(
  root = process.cwd(),
): TableLiveV24AuthorizationProof {
  const state = verifyTableLiveV24History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V24_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v24/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v24.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V24_TARGET,
    expectedDynamicTool: TABLE_LIVE_V24_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V24_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV24HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV24History(expected);
  process.stdout.write(
    `Table live v24 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
