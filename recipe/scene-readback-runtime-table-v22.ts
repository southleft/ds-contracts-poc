...
/**
 * Table live v22 extract runtime. Keeps the v6 skip of untagged
 * `table/row/owned-cell-label` TEXT bind hosts. Extract bytes are
 * unchanged from v21. The v22 teaching lives in host-normalize
 * (`scene-readback-table-v1.ts`): omit `effects` on `table/row`
 * variants because compile row variants omit `effects`. Does not
 * omit cell-variant `clipsContent` or `cornerRadius`. Does not invent
 * `effects` onto compile row variants. Does not omit
