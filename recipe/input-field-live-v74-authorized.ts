...
import { INPUT_LIVE_V74_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v74.js";
import { verifyInputLiveV74Authorization } from "./input-field-live-v74-authorization.js";
import { runInputLiveV74Preflight } from "./input-field-live-v74-preflight.js";
import { InputLiveV74Orchestrator } from "./run-input-field-live-v74.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V74_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v74-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV74Authorization(root);
    runInputLiveV74Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV74Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
