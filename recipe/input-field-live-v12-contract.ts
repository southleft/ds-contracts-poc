...
} from "./input-field-live-v3-verifier-v12.js";
import {
  buildInputLiveV12RawPropertyRuntime,
  normalizeInputLiveV12Scene,
  type InputLiveV12RawNode,
} from "./input-field-live-v12-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v12.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v12.js";

export const INPUT_LIVE_V12_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V12_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V12_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V12_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V12_VARIANT_COUNT = 256;
export const INPUT_LIVE_V12_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V12_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV12SourceId = (typeof INPUT_LIVE_V12_SOURCE_IDS)[number];

export interface InputLiveV12SourceIdentity {
  source: InputLiveV12SourceId;
...
export interface InputLiveV12WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV12ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV12SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV12RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV12RootProof {
  source: InputLiveV12SourceId;
  adapterIdentity: string;
...
export interface InputLiveV12ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV12SourceId;
    adapterIdentity: string;
...
export interface InputLiveV12CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV12SourceId;
  adapterIdentity: string;
...
export interface InputLiveV12CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV12SourceId;
  strata: InputLiveV12CaptureCell["strata"];
...
export interface InputLiveV12CleanupPayload {
...
const exactUniqueStrings = (
  ...
  if (
    ...
    throw new TypeError(`Input live v12 ${label} cardinality/identity mismatch`);
  return value;
};
const finiteNonnegative = (value: unknown): value is number =>
  typeof value === "number" && Number.isFinite(value) && value >= 0;

export function validateInputLiveV12WriterPayload(
  value: unknown,
): InputLiveV12WriterOwnership {
...
const identityPlan = (source: InputLiveV12SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV12ExtractProgram(
  writer: InputLiveV12WriterOwnership,
  sources: readonly InputLiveV12SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Input live v12 extract requires two source identities");
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V12_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V12_NAMESPACE)}
${buildInputLiveV12RawPropertyRuntime()}
...
export function validateInputLiveV12ExtractPayload(
  value: unknown,
  writer: InputLiveV12WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V12_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV12ExtractPayload;
}

export function proveInputLiveV12Roots<Instance>(
  extract: InputLiveV12ExtractPayload,
  sources: readonly (InputLiveV12SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV12Scene(
      root.scene,
...
export function assertInputLiveV12RootProofs(
  proofs: readonly InputLiveV12RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV12ProbeProgram(
  writer: InputLiveV12WriterOwnership,
  sources: readonly InputLiveV12SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V12_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV12ProbePayload(
  value: unknown,
  writer: InputLiveV12WriterOwnership,
): InputLiveV12ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV12ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V12_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V12_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV12CaptureManifest(
  cells: readonly InputLiveV12CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V12_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V12_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V12_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V12_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V12_SOURCE_IDS.some(
...
export const inputLiveV12CaptureManifestSha256 = (
  cells: readonly InputLiveV12CaptureCell[],
): string => {
  validateInputLiveV12CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV12CaptureProgram(
  writer: InputLiveV12WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V12_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V12_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V12_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV12CapturePayload(
  value: unknown,
  cell: InputLiveV12CaptureCell,
  rawResponseBytes: number,
): InputLiveV12CapturePayload {
  const payload = record(value, "capture payload") as InputLiveV12CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V12_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V12_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV12CaptureResponses(
  manifest: readonly InputLiveV12CaptureCell[],
  responses: readonly InputLiveV12CapturePayload[],
): void {
  validateInputLiveV12CaptureManifest(manifest);
  if (
...
export interface InputLiveV12ObjectiveReport {
  artifactVersion: "input-live-v12-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV12Objective(
  manifest: readonly InputLiveV12CaptureCell[],
  responses: readonly InputLiveV12CapturePayload[],
  readReference: (cell: InputLiveV12CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV12CaptureCell) => Buffer,
): InputLiveV12ObjectiveReport {
  assertInputLiveV12CaptureResponses(manifest, responses);
...
export function buildInputLiveV12CleanupProgram(
  writer: InputLiveV12WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V12_NAMESPACE,
...
export function validateInputLiveV12CleanupPayload(
  value: unknown,
  writer: InputLiveV12WriterOwnership,
): InputLiveV12CleanupPayload {
  const payload = record(value, "cleanup payload") as InputLiveV12CleanupPayload;
...
export const INPUT_LIVE_V12_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV12WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
