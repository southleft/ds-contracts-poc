...
import {
  acceptTableLiveV2Response,
  createTableLiveV2Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV2Request,
  persistTableLiveV2RawResponse,
  persistTableLiveV2TechnicalGates,
  readTableLiveV2AcceptedRawPayload,
  type TableLiveV2Request,
  type TableLiveV2TransactionAuthorization,
} from "./table-live-v2-broker.js";
import {
  TABLE_LIVE_V2_CAPTURE_COUNT,
  assertTableLiveV2CaptureResponses,
  ...
  evaluateTableLiveV2Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV2Roots,
  validateTableLiveV2CaptureManifest,
  validateTableLiveV2CapturePayload,
  validateTableLiveV2CleanupPayload,
  validateTableLiveV2ExtractPayload,
  validateTableLiveV2ProbePayload,
  validateTableLiveV2WriterPayload,
  type TableLiveV2CaptureCell,
  type TableLiveV2CapturePayload,
...
import {
  ...
  TABLE_LIVE_V2_PLAN_PATH,
  type TableLiveV2ProofPlan,
...
export const TABLE_LIVE_V2_RUNNER_VERSION = "table-live-v2-runner-v1";

interface TableLiveV2JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV2OrchestratorState {
  artifactVersion: "table-live-v2-orchestrator-state-v1";
  ...
  journal: TableLiveV2JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV2ProofPlan;
  planSha256: string;
  captures: TableLiveV2CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV2SourceIdentity & {
...
export interface TableLiveV2NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV2OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v2 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V2_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V2_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV2ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV2CaptureCell[];
  validateTableLiveV2CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v2 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v2 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV2WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV2RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV2Request =>
  readJson<TableLiveV2Request>(file);

export class TableLiveV2Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV2Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV2OrchestratorState = {
      artifactVersion: "table-live-v2-orchestrator-state-v1",
    ...
    return new TableLiveV2Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV2OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV2Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV2NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV2NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v2 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV2NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v2 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV2RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV2Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV2CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV2AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV2WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV2Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV2ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV2Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV2Response(this.#directory, "writer", {
        validate: validateTableLiveV2WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV2Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV2RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV2Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV2ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV2Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV2RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v2-host-root-proofs-v1",
      ...
      const next = issueTableLiveV2Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV2ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV2Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV2ProbePayload(payload, ownership),
      });
      persistTableLiveV2TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v2 capture cell missing");
      const accepted = acceptTableLiveV2Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV2CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V2_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV2CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV2Objective(
    ...
    throw new Error(`Table live v2 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV2Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV2CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV2Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV2RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV2Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV2CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV2CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV2CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV2Authorization = (
  privateKey: KeyObject,
): TableLiveV2TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV2SimulationKey = (): KeyObject =>
