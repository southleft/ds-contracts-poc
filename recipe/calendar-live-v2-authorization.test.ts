...
import { CALENDAR_LIVE_V2_INDEX_PATH } from "./build-calendar-live-proof-v2.js";
import {
...
import {
  buildCalendarLiveV2AuthorizationArtifact,
  validateCalendarLiveV2AntecedentIndex,
  validateCalendarLiveV2AuthorizationArtifact,
  validateCalendarLiveV2History,
  validateCalendarLiveV2UnitTestSource,
  type CalendarLiveV2AntecedentIndex,
  type CalendarLiveV2AuthorizationArtifact,
  type CalendarLiveV2AuthorizationProof,
  type CalendarLiveV2HistoryState,
} from "./calendar-live-v2-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV2ControlFlowSource,
  validateCalendarLiveV2SecurityAttestation,
  type CalendarLiveV2SecurityAttestation,
} from "./calendar-live-v2-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V2_INDEX_PATH);
const index = (): CalendarLiveV2AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV2AntecedentIndex;

const artifact = (): CalendarLiveV2AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV2AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV2HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV2HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV2AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v2/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV2AuthorizationArtifact,
): CalendarLiveV2SecurityAttestation => {
  const body: Omit<CalendarLiveV2SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V2_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV2History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV2History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV2History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV2History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV2History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV2UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v2-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV2UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV2AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV2AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV2AuthorizationArtifact(
        planted as CalendarLiveV2AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV2SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV2SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV2SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v2-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV2ControlFlowSource(source), []);
});
