...
export const COMBOBOX_LIVE_V13_MEASURE_VISIBLE_FILL_MARKER =
  "COMBOBOX-TEXT-FILL-MEASURE-VISIBLE";
export const COMBOBOX_LIVE_V13_RESTORE_CONTENT_ROLES = [
  "combobox/input",
  "combobox/option/label",
] as const;
export const COMBOBOX_LIVE_V13_RESTORE_PARENT_ROLES = [
  "combobox/trigger",
] as const;
export const COMBOBOX_LIVE_V13_RESTORE_INPUT_SET_ROLE = "combobox/set";
export const COMBOBOX_LIVE_V13_RESTORE_OPTION_LABEL_SET_ROLE =
  "combobox/option-set";
export const COMBOBOX_LIVE_V13_RESTORE_COUNT = 144;
export const FORBIDDEN_INPUT_PAGE_ID = "115:295378";

export interface ComboboxLiveV13RestoreWriter {
  pageId: string;
  runIdentity: string;
  setIds: readonly string[];
}

export interface ComboboxLiveV13RestorePayload {
  pageId: string;
  setIds: string[];
  restoredCount: 144;
  fixedBefore: number;
  ...
  marker: typeof COMBOBOX_LIVE_V13_MEASURE_VISIBLE_FILL_MARKER;
}

export function buildComboboxLiveV13RestoreProgram(
  writer: ComboboxLiveV13RestoreWriter,
): string {
  if (writer.setIds.length !== 4)
    throw new TypeError("Combobox live v13 restore requires four owned sets");
...
const ROLES=new Set(${JSON.stringify([...COMBOBOX_LIVE_V13_RESTORE_CONTENT_ROLES])});
const PARENTS=new Set(${JSON.stringify([...COMBOBOX_LIVE_V13_RESTORE_PARENT_ROLES])});
const INPUT_SET=${JSON.stringify(COMBOBOX_LIVE_V13_RESTORE_INPUT_SET_ROLE)};
const OPTION_SET=${JSON.stringify(COMBOBOX_LIVE_V13_RESTORE_OPTION_LABEL_SET_ROLE)};
const MARKER=${JSON.stringify(COMBOBOX_LIVE_V13_MEASURE_VISIBLE_FILL_MARKER)};
...
export function validateComboboxLiveV13RestorePayload(
  payload: unknown,
  writer: ComboboxLiveV13RestoreWriter,
): ComboboxLiveV13RestorePayload {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("Combobox live v13 restore payload must be an object");
  ...
  if (
    ...
    value.marker !== COMBOBOX_LIVE_V13_MEASURE_VISIBLE_FILL_MARKER
  )
    throw new TypeError(
      "Combobox live v13 restore did not re-assert content FILL",
    );
  return value as ComboboxLiveV13RestorePayload;
}
