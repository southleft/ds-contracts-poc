...
import { COMBOBOX_LIVE_V10_INDEX_PATH } from "./build-combobox-live-proof-v10.js";
import {
  COMBOBOX_LIVE_V10_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V10_TARGET,
} from "./combobox-live-v10-broker.js";
import {
  buildComboboxLiveV10AuthorizationArtifact,
  validateComboboxLiveV10AntecedentIndex,
  validateComboboxLiveV10AuthorizationArtifact,
  validateComboboxLiveV10History,
  validateComboboxLiveV10UnitTestSource,
  type ComboboxLiveV10AntecedentIndex,
  type ComboboxLiveV10AuthorizationArtifact,
  type ComboboxLiveV10AuthorizationProof,
  type ComboboxLiveV10HistoryState,
} from "./combobox-live-v10-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV10ControlFlowSource,
  validateComboboxLiveV10SecurityAttestation,
  type ComboboxLiveV10SecurityAttestation,
} from "./combobox-live-v10-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v10-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V10_INDEX_PATH);
const index = (): ComboboxLiveV10AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV10AntecedentIndex;

const artifact = (): ComboboxLiveV10AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV10AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV10HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV10HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV10AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v10/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV10AuthorizationArtifact,
): ComboboxLiveV10SecurityAttestation => {
  const body: Omit<ComboboxLiveV10SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V10_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV10History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV10History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV10History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV10History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV10History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV10UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV10UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV10AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV10AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV10AuthorizationArtifact(
        planted as ComboboxLiveV10AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV10SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV10SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV10SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV10ControlFlowSource(source), []);
});
