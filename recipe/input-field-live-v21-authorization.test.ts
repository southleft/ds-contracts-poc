...
import { INPUT_LIVE_V21_INDEX_PATH } from "./build-input-field-live-proof-v21.js";
import {
  INPUT_LIVE_V21_DYNAMIC_TOOL,
  INPUT_LIVE_V21_TARGET,
} from "./input-field-live-v21-broker.js";
import {
  buildInputLiveV21AuthorizationArtifact,
  validateInputLiveV21AntecedentIndex,
  validateInputLiveV21AuthorizationArtifact,
  validateInputLiveV21History,
  validateInputLiveV21UnitTestSource,
  type InputLiveV21AntecedentIndex,
  type InputLiveV21AuthorizationArtifact,
  type InputLiveV21AuthorizationProof,
  type InputLiveV21HistoryState,
} from "./input-field-live-v21-authorization.js";
import {
  inputLiveV21AttestationSha256,
  validateInputLiveV21ControlFlowSource,
  validateInputLiveV21SecurityAttestation,
  type InputLiveV21SecurityAttestation,
} from "./input-field-live-v21-preflight.js";
import { inputLiveV21Sha256 } from "./input-field-live-v21-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V21_INDEX_PATH);
const index = (): InputLiveV21AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV21AntecedentIndex;

const artifact = (): InputLiveV21AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV21AuthorizationArtifact({
...
const pendingState = (): InputLiveV21HistoryState => ({
  index: index(),
  indexSha256: inputLiveV21Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV21HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV21AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV21Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V21_TARGET,
  expectedDynamicTool: INPUT_LIVE_V21_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v21/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV21AuthorizationArtifact,
): InputLiveV21SecurityAttestation => {
  const body: Omit<InputLiveV21SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v21-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V21_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV21AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV21History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV21History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV21History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV21History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV21History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV21UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV21UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV21AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV21Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV21AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV21AuthorizationArtifact(
        planted as InputLiveV21AuthorizationArtifact,
        index(),
        inputLiveV21Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV21SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV21AttestationSha256(
      body as Omit<InputLiveV21SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV21SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV21ControlFlowSource(source), []);
});
