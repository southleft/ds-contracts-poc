...
/**
 * Carried Input live v18 scene-readback. Keeps the v17 slot-fill teaching
 * and first-segment role recovery, and surfaces leading/trailing slot
 * `fills.0.color` from the adornment-content child's existing COLOR
 * binding when the slot node's own bindings lack it. Does not invent
 * variables, colors, or treat FIXED as a fill.
 */
...
export const INPUT_LIVE_V18_SLOT_FILL_MARKER = "INPUT-SLOT-FILL-FROM-PAYLOAD";
export const INPUT_LIVE_V18_SLOT_COLOR_BINDING_MARKER =
  "INPUT-SLOT-COLOR-BINDING-FROM-CHILD";
export const INPUT_LIVE_V18_SLOT_ROLES = [
...
const SLOT_ROLES = new Set<string>(INPUT_LIVE_V18_SLOT_ROLES);
...
export function sceneToNormalizedIr(scene: SceneNodeSnapshot): IRNode {
  const bindings = surfaceSlotColorBindings(scene, sceneBindings(scene));
  const common = {
