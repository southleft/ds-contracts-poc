...
import {
  INPUT_LIVE_V75_MEASURE_VISIBLE_FILL_MARKER,
  INPUT_LIVE_V75_RESTORE_CONTENT_ROLES,
  INPUT_LIVE_V75_RESTORE_PARENT_ROLES,
...
test("v75 probe teaches Polar reflow against content text, not a minted content-row", () => {
  const program = readFileSync(
    "recipe/input-field-live-v75-contract.ts",
    "utf8",
  );
  assert.match(program, /reflowTarget=contentRow\|\|contentText/);
  assert.match(
    program,
    /contentFillPassed=!!\(contentText&&contentText.layoutSizingHorizontal==="FILL"\)/,
  );
  assert.match(program, /contentFillPassed:!!contentFillPassed/);
  assert.equal(program.includes("contentFillPassed:!!reflowPassed"), false);
  assert.equal(program.includes("content&&content.width>contentWidth"), false);
});

test("v16 restore still measures FILL while visible, then restores visibility", () => {
  const program = buildInputLiveV75RestoreProgram(writer);
  assert.match(program, new RegExp(INPUT_LIVE_V75_MEASURE_VISIBLE_FILL_MARKER));
  ...
  assert.match(program, /INPUT-V75-RESTORE-COUNT/);
  assert.match(program, /INPUT-V75-RESTORE-NOT-FILL/);
  ...
  for (const role of INPUT_LIVE_V75_RESTORE_CONTENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
  for (const role of INPUT_LIVE_V75_RESTORE_PARENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
});

test("v16 restore payload requires 256 filled content texts", () => {
  const accepted = validateInputLiveV75RestorePayload(
    {
      pageId: "1:1",
      setIds: ["2:2", "2:1"],
      restoredCount: 256,
      ...
      marker: INPUT_LIVE_V75_MEASURE_VISIBLE_FILL_MARKER,
    },
    writer,
  );
...
test("v16 writer bytes stay frozen and v17 writer assigns set HUG", () => {
  ...
  const v75Program = readFileSync(
    "recipe/evidence/input-field-live-pivot-v75/programs/writer.txt",
  );
  ...
  assert.notEqual(sha256(v75Program), V12_WRITER_PROGRAM_SHA256);
  assert.notEqual(sha256(v75Payload), V12_WRITER_PAYLOAD_SHA256);
  assert.match(v75Payload.toString("utf8"), /set\.layoutSizingHorizontal="HUG"/);
