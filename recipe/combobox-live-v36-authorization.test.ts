...
import { COMBOBOX_LIVE_V36_INDEX_PATH } from "./build-combobox-live-proof-v36.js";
import {
  COMBOBOX_LIVE_V36_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V36_TARGET,
} from "./combobox-live-v36-broker.js";
import {
  buildComboboxLiveV36AuthorizationArtifact,
  validateComboboxLiveV36AntecedentIndex,
  validateComboboxLiveV36AuthorizationArtifact,
  validateComboboxLiveV36History,
  validateComboboxLiveV36UnitTestSource,
  type ComboboxLiveV36AntecedentIndex,
  type ComboboxLiveV36AuthorizationArtifact,
  type ComboboxLiveV36AuthorizationProof,
  type ComboboxLiveV36HistoryState,
} from "./combobox-live-v36-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV36ControlFlowSource,
  validateComboboxLiveV36SecurityAttestation,
  type ComboboxLiveV36SecurityAttestation,
} from "./combobox-live-v36-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v36-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V36_INDEX_PATH);
const index = (): ComboboxLiveV36AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV36AntecedentIndex;

const artifact = (): ComboboxLiveV36AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV36AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV36HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV36HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV36AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v36/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV36AuthorizationArtifact,
): ComboboxLiveV36SecurityAttestation => {
  const body: Omit<ComboboxLiveV36SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V36_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV36History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV36History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV36History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV36History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV36History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV36UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV36UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV36AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV36AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV36AuthorizationArtifact(
        planted as ComboboxLiveV36AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV36SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV36SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV36SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV36ControlFlowSource(source), []);
});
