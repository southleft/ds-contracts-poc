...
import { INPUT_LIVE_V52_INDEX_PATH } from "./build-input-field-live-proof-v52.js";
import {
  INPUT_LIVE_V52_DYNAMIC_TOOL,
  INPUT_LIVE_V52_TARGET,
} from "./input-field-live-v52-broker.js";
import {
  buildInputLiveV52AuthorizationArtifact,
  validateInputLiveV52AntecedentIndex,
  validateInputLiveV52AuthorizationArtifact,
  validateInputLiveV52History,
  validateInputLiveV52UnitTestSource,
  type InputLiveV52AntecedentIndex,
  type InputLiveV52AuthorizationArtifact,
  type InputLiveV52AuthorizationProof,
  type InputLiveV52HistoryState,
} from "./input-field-live-v52-authorization.js";
import {
  inputLiveV52AttestationSha256,
  validateInputLiveV52ControlFlowSource,
  validateInputLiveV52SecurityAttestation,
  type InputLiveV52SecurityAttestation,
} from "./input-field-live-v52-preflight.js";
import { inputLiveV52Sha256 } from "./input-field-live-v52-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V52_INDEX_PATH);
const index = (): InputLiveV52AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV52AntecedentIndex;

const artifact = (): InputLiveV52AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV52AuthorizationArtifact({
...
const pendingState = (): InputLiveV52HistoryState => ({
  index: index(),
  indexSha256: inputLiveV52Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV52HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV52AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV52Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V52_TARGET,
  expectedDynamicTool: INPUT_LIVE_V52_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v52/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV52AuthorizationArtifact,
): InputLiveV52SecurityAttestation => {
  const body: Omit<InputLiveV52SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v52-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V52_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV52AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV52History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV52History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV52History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV52History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV52History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV52UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV52UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV52AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV52Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV52AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV52AuthorizationArtifact(
        planted as InputLiveV52AuthorizationArtifact,
        index(),
        inputLiveV52Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV52SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV52AttestationSha256(
      body as Omit<InputLiveV52SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV52SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV52ControlFlowSource(source), []);
});
