...
import {
  ...
  createInputLiveV25FakeDynamicResponse,
  ...
  listInputLiveV25PersistedRequests,
  persistInputLiveV25RawResponse,
  persistInputLiveV25TechnicalGates,
  validateInputLiveV25Receipt,
  validateInputLiveV25Request,
  type InputLiveV25RemotePhase,
...
import {
  ...
  type InputLiveV25ExtractPayload,
  type InputLiveV25ProbePayload,
  type InputLiveV25RestorePayload,
  type InputLiveV25RootProof,
  type InputLiveV25WriterOwnership,
} from "./input-field-live-v25-contract.js";
import {
  InputLiveV25Orchestrator,
  simulatedInputLiveV25Authorization,
} from "./run-input-field-live-v25.js";
...
const captures: InputLiveV25CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV25Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v25-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV25Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV25CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV25FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV25FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V25_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V25_TARGET);
  persistInputLiveV25RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV25Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV25Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV25WriterOwnership,
): InputLiveV25RestorePayload => ({
...
const rootProofs = (): InputLiveV25RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV25CaptureCell,
): InputLiveV25CapturePayload => {
  const bytes = Buffer.from(`input-live-v25-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV25Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV25Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV25RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV25RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV25ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV25ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV25CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV25CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV25CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV25Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV25CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV25PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV25ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV25Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV25Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV25Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV25Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V25_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV25RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V25_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV25Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV25Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV25ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV25Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV25WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV25RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV25Request;
    validateInputLiveV25Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV25Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV25CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v25-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV25Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV25Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV25Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V25_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV25RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V25_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV25Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV25Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV25RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V25_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV25Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV25Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV25Request;
