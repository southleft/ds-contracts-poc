/**
 * Carried Combobox live v3 host verifier. Uses scene-readback-combobox-v1 so live
...
export const COMBOBOX_LIVE_V3_FIXED_VERIFIER_VERSION = "combobox-live-v3-fixed-point-v1";
export const COMBOBOX_LIVE_V3_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV3FixedThresholds {
...
export interface ComboboxLiveV3FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV3FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV3FixedSourceProbe {
...
export interface ComboboxLiveV3FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV3FixedFixedPoint;
}

export interface ComboboxLiveV3FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV3FixedHardGateInput {
  thresholds: ComboboxLiveV3FixedThresholds;
  materialRegression: ComboboxLiveV3FixedMaterialRegression;
  sourceProbes: ComboboxLiveV3FixedSourceProbe[];
  cells: ComboboxLiveV3FixedCellValidation[];
  sceneProofs: ComboboxLiveV3FixedSceneProof[];
  visualRows: ComboboxLiveV3FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV3FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V3_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV3FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV3FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV3FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV3FixedHardGates(
  input: ComboboxLiveV3FixedHardGateInput,
): ComboboxLiveV3FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV3FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV3FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V3_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV3FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV3FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
