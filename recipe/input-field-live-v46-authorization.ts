...
import {
  ...
  INPUT_LIVE_V46_INDEX_PATH,
} from "./build-input-field-live-proof-v46.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV46HistoryExpectation = "pending" | "authorized";

export interface InputLiveV46AuthorizationArtifact {
  artifactVersion: "input-live-v46-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V46_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV46AntecedentIndex {
...
export interface InputLiveV46HistoryState {
  index?: InputLiveV46AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV46AuthorizationArtifact;
...
export interface InputLiveV46AuthorizationProof extends InputLiveV46TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V46_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V46_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V46_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V46_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v46-status") ||
...
export function validateInputLiveV46AntecedentIndex(
  index: InputLiveV46AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v46-antecedent-index-v1" ||
...
export function validateInputLiveV46UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV46History\(\s*["']/.test(source) ||
    /readInputLiveV46HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV46AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV46AuthorizationArtifact {
  ...
  ) as InputLiveV46AntecedentIndex;
  const indexFailures = validateInputLiveV46AntecedentIndex(index);
...
export function validateInputLiveV46AuthorizationArtifact(
  artifact: InputLiveV46AuthorizationArtifact | undefined,
  index: InputLiveV46AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v46-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v46" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V46_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V46_TARGET) ||
    !equalJson(
...
export function validateInputLiveV46History(
  state: InputLiveV46HistoryState,
  expected: InputLiveV46HistoryExpectation,
): string[] {
  const failures = validateInputLiveV46AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV46AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV46HistoryState(
  root = process.cwd(),
): InputLiveV46HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V46_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV46AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V46_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V46_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V46_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV46History(
  expected: InputLiveV46HistoryExpectation,
  root = process.cwd(),
): InputLiveV46HistoryState {
  const state = readInputLiveV46HistoryState(root);
  const failures = validateInputLiveV46History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV46Authorization(
  root = process.cwd(),
): InputLiveV46AuthorizationProof {
  const state = verifyInputLiveV46History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V46_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v46/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v46.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V46_TARGET,
    expectedDynamicTool: INPUT_LIVE_V46_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V46_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV46HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV46History(expected);
