...
import { buildTableLiveV16CleanupRuntime } from "./table-live-v16-cleanup.js";
import { type TableLiveV16FixedFixedPoint } from "./table-live-v16-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV16RestoreProgram,
  validateTableLiveV16RestorePayload,
  type TableLiveV16RestorePayload,
} from "./table-live-v16-restore.js";
import {
  buildTableLiveV16RawPropertyRuntime,
  normalizeTableLiveV16Scene,
  type TableLiveV16RawNode,
} from "./table-live-v16-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v16.js";
import {
...
export const TABLE_LIVE_V16_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V16_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V16_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V16_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V16_VARIANT_COUNT = 20;
export const TABLE_LIVE_V16_SET_COUNT = 6;
export const TABLE_LIVE_V16_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V16_HOST_PHASES = 3;
export const TABLE_LIVE_V16_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V16_RESTORE_COUNT = 8;
export const TABLE_LIVE_V16_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V16_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV16SourceId = (typeof TABLE_LIVE_V16_SOURCE_IDS)[number];
export type TableLiveV16Kind = "table" | "row" | "cell";

export interface TableLiveV16SourceIdentity {
  source: TableLiveV16SourceId;
...
export interface TableLiveV16WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV16ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV16SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV16RawNode;
    rowScene: TableLiveV16RawNode;
    cellScene: TableLiveV16RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV16RootProof {
  source: TableLiveV16SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV16FixedFixedPoint;
}

export interface TableLiveV16ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV16SourceId;
    adapterIdentity: string;
...
export interface TableLiveV16CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV16SourceId;
  adapterIdentity: string;
...
export interface TableLiveV16CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV16SourceId;
  frameWidth: number;
...
export interface TableLiveV16CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v16 ${label} must be an object`);
...
export function validateTableLiveV16WriterPayload(
  value: unknown,
): TableLiveV16WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V16_NAMESPACE ||
    ...
    throw new TypeError("Table live v16 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v16 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v16 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV16SourceIdentity,
  kind: TableLiveV16Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV16ExtractProgram(
  writer: TableLiveV16WriterOwnership,
  sources: readonly TableLiveV16SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v16 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V16_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V16_NAMESPACE)}
${buildTableLiveV16RawPropertyRuntime()}
...
export function validateTableLiveV16ExtractPayload(
  value: unknown,
  writer: TableLiveV16WriterOwnership,
): TableLiveV16ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v16 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V16_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v16 extract schema/two-root mismatch");
  return payload as TableLiveV16ExtractPayload;
}

export function proveTableLiveV16Roots<Instance>(
  extract: TableLiveV16ExtractPayload,
  sources: readonly (TableLiveV16SourceIdentity & {
...
): TableLiveV16RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v16 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v16 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV16Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV16Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV16Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV16FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v16 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v16 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v16 compile lost owned sets");
...
export function assertTableLiveV16RootProofs(
  proofs: readonly TableLiveV16RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v16 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v16 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV16ProbeProgram(
  writer: TableLiveV16WriterOwnership,
  sources: readonly TableLiveV16SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V16_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV16ProbePayload(
  value: unknown,
  writer: TableLiveV16WriterOwnership,
): TableLiveV16ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV16ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V16_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V16_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Table live v16 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV16CaptureManifest(
  cells: readonly TableLiveV16CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V16_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V16_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V16_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V16_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v16 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV16CaptureCell[],
): string => {
  validateTableLiveV16CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV16CaptureProgram(
  writer: TableLiveV16WriterOwnership,
  cell: TableLiveV16CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V16_CAPTURE_COUNT
  )
    throw new TypeError("Table live v16 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v16 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V16_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV16CapturePayload(
  value: unknown,
  cell: TableLiveV16CaptureCell,
  rawResponseBytes: number,
): TableLiveV16CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV16CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V16_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V16_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v16 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV16CaptureResponses(
  manifest: readonly TableLiveV16CaptureCell[],
  responses: readonly TableLiveV16CapturePayload[],
): void {
  validateTableLiveV16CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v16 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV16ObjectiveReport {
  artifactVersion: "table-live-v16-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV16Objective(
  manifest: readonly TableLiveV16CaptureCell[],
  responses: readonly TableLiveV16CapturePayload[],
): TableLiveV16ObjectiveReport {
  assertTableLiveV16CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v16-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV16CleanupProgram(
  writer: TableLiveV16WriterOwnership,
): string {
  const baseProgram = buildTableLiveV16CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V16_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v16 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV16CleanupPayload(
  payload: unknown,
  writer: TableLiveV16WriterOwnership,
): TableLiveV16CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV16CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v16 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V16_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV16WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
