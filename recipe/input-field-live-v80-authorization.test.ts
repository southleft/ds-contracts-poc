...
import { INPUT_LIVE_V80_INDEX_PATH } from "./build-input-field-live-proof-v80.js";
import {
  INPUT_LIVE_V80_DYNAMIC_TOOL,
  INPUT_LIVE_V80_TARGET,
} from "./input-field-live-v80-broker.js";
import {
  buildInputLiveV80AuthorizationArtifact,
  validateInputLiveV80AntecedentIndex,
  validateInputLiveV80AuthorizationArtifact,
  validateInputLiveV80History,
  validateInputLiveV80UnitTestSource,
  type InputLiveV80AntecedentIndex,
  type InputLiveV80AuthorizationArtifact,
  type InputLiveV80AuthorizationProof,
  type InputLiveV80HistoryState,
} from "./input-field-live-v80-authorization.js";
import {
  inputLiveV80AttestationSha256,
  validateInputLiveV80ControlFlowSource,
  validateInputLiveV80SecurityAttestation,
  type InputLiveV80SecurityAttestation,
} from "./input-field-live-v80-preflight.js";
import { inputLiveV80Sha256 } from "./input-field-live-v80-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V80_INDEX_PATH);
const index = (): InputLiveV80AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV80AntecedentIndex;

const artifact = (): InputLiveV80AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV80AuthorizationArtifact({
...
const pendingState = (): InputLiveV80HistoryState => ({
  index: index(),
  indexSha256: inputLiveV80Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV80HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV80AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV80Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V80_TARGET,
  expectedDynamicTool: INPUT_LIVE_V80_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v80/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV80AuthorizationArtifact,
): InputLiveV80SecurityAttestation => {
  const body: Omit<InputLiveV80SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v80-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V80_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV80AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV80History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV80History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV80History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV80History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV80History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV80UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV80UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV80AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV80Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV80AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV80AuthorizationArtifact(
        planted as InputLiveV80AuthorizationArtifact,
        index(),
        inputLiveV80Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV80SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV80AttestationSha256(
      body as Omit<InputLiveV80SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV80SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV80ControlFlowSource(source), []);
});
