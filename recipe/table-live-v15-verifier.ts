...
import {
  verifyTableLiveV15FixedSceneFixedPoint,
  type TableLiveV15FixedFixedPoint,
} from "./table-live-v15-fixed-point.js";

export const TABLE_LIVE_V15_VERIFIER_VERSION = "table-live-v15-verifier-v1";

export function buildTableLiveV15RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV15RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV15RawNode[];
}

export interface TableLiveV15NormalizedScene {
...
export interface TableLiveV15SceneProof {
...
export function normalizeTableLiveV15Scene(
  root: TableLiveV15RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV15NormalizedScene {
  const canonicalBindings: TableLiveV15NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV15RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV15Scene<Instance>(
  normalized: TableLiveV15NormalizedScene,
...
): TableLiveV15SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV15FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV15ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV15PreCaptureGates(payload: unknown): void {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("table live v15 pre-capture gate payload absent");
