...
import { calendarLiveV1Sha256 } from "./calendar-live-v24-broker.js";

export interface CalendarLiveV24LifecycleSimulationReport {
  artifactVersion: "calendar-live-v24-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV24AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV24Lifecycle(): Promise<CalendarLiveV24LifecycleSimulationReport> {
  await buildCalendarLiveV24Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V24_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV24AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v24-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V24_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V24_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV24History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV24History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV24AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V24_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V24_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV24History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV24History(authorized, "pending");
    ...
    await buildCalendarLiveV24Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V24_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV24AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV24Lifecycle(), null, 2)}\n`,
  );
