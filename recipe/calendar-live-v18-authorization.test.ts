...
import { CALENDAR_LIVE_V18_INDEX_PATH } from "./build-calendar-live-proof-v18.js";
import {
...
import {
  buildCalendarLiveV18AuthorizationArtifact,
  validateCalendarLiveV18AntecedentIndex,
  validateCalendarLiveV18AuthorizationArtifact,
  validateCalendarLiveV18History,
  validateCalendarLiveV18UnitTestSource,
  type CalendarLiveV18AntecedentIndex,
  type CalendarLiveV18AuthorizationArtifact,
  type CalendarLiveV18AuthorizationProof,
  type CalendarLiveV18HistoryState,
} from "./calendar-live-v18-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV18ControlFlowSource,
  validateCalendarLiveV18SecurityAttestation,
  type CalendarLiveV18SecurityAttestation,
} from "./calendar-live-v18-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V18_INDEX_PATH);
const index = (): CalendarLiveV18AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV18AntecedentIndex;

const artifact = (): CalendarLiveV18AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV18AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV18HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV18HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV18AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v18/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV18AuthorizationArtifact,
): CalendarLiveV18SecurityAttestation => {
  const body: Omit<CalendarLiveV18SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "calendar-live-v18-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V18_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV18History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV18History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV18History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV18History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV18History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV18UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v18-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV18UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV18AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV18AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV18AuthorizationArtifact(
        planted as CalendarLiveV18AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV18SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV18SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV18SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v18-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV18ControlFlowSource(source), []);
});
