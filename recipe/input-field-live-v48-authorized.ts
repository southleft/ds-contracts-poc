...
import { INPUT_LIVE_V48_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v48.js";
import { verifyInputLiveV48Authorization } from "./input-field-live-v48-authorization.js";
import { runInputLiveV48Preflight } from "./input-field-live-v48-preflight.js";
import { InputLiveV48Orchestrator } from "./run-input-field-live-v48.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V48_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v48-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV48Authorization(root);
    runInputLiveV48Preflight(
      root,
      authorization,
    ...
    const orchestrator = InputLiveV48Orchestrator.initialize({
      root,
  ...
  const orchestrator = InputLiveV48Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
