...
import { calendarLiveV1Sha256 } from "./calendar-live-v40-broker.js";

export interface CalendarLiveV40LifecycleSimulationReport {
  artifactVersion: "calendar-live-v40-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV40AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV40Lifecycle(): Promise<CalendarLiveV40LifecycleSimulationReport> {
  await buildCalendarLiveV40Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V40_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV40AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v40-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V40_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V40_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV40History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV40History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV40AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V40_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V40_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV40History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV40History(authorized, "pending");
    ...
    await buildCalendarLiveV40Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V40_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV40AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV40Lifecycle(), null, 2)}\n`,
  );
