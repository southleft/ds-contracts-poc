/**
 * Carried Combobox live v37 host verifier. Uses scene-readback-combobox-v37 so live
...
export const COMBOBOX_LIVE_V37_FIXED_VERIFIER_VERSION = "combobox-live-v37-fixed-point-v1";
export const COMBOBOX_LIVE_V37_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV37FixedThresholds {
...
export interface ComboboxLiveV37FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV37FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV37FixedSourceProbe {
...
export interface ComboboxLiveV37FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV37FixedFixedPoint;
}

export interface ComboboxLiveV37FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV37FixedHardGateInput {
  thresholds: ComboboxLiveV37FixedThresholds;
  materialRegression: ComboboxLiveV37FixedMaterialRegression;
  sourceProbes: ComboboxLiveV37FixedSourceProbe[];
  cells: ComboboxLiveV37FixedCellValidation[];
  sceneProofs: ComboboxLiveV37FixedSceneProof[];
  visualRows: ComboboxLiveV37FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV37FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V37_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV37FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV37FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV37FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV37FixedHardGates(
  input: ComboboxLiveV37FixedHardGateInput,
): ComboboxLiveV37FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV37FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV37FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V37_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV37FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV37FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV37FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
