...
import {
  acceptComboboxLiveV15Response,
  createComboboxLiveV15Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV15Request,
  persistComboboxLiveV15RawResponse,
  persistComboboxLiveV15TechnicalGates,
  readComboboxLiveV15AcceptedRawPayload,
  type ComboboxLiveV15Request,
  type ComboboxLiveV15TransactionAuthorization,
} from "./combobox-live-v15-broker.js";
import {
  COMBOBOX_LIVE_V15_CAPTURE_COUNT,
  assertComboboxLiveV15CaptureResponses,
  assertComboboxLiveV15RootProofs,
  buildComboboxLiveV15CaptureProgram,
  buildComboboxLiveV15CleanupProgram,
  buildComboboxLiveV15ExtractProgram,
  buildComboboxLiveV15ProbeProgram,
  buildComboboxLiveV15RestoreProgram,
  validateComboboxLiveV15RestorePayload,
  evaluateComboboxLiveV15Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV15Roots,
  validateComboboxLiveV15CaptureManifest,
  validateComboboxLiveV15CapturePayload,
  validateComboboxLiveV15CleanupPayload,
  validateComboboxLiveV15ExtractPayload,
  validateComboboxLiveV15ProbePayload,
  validateComboboxLiveV15WriterPayload,
  type ComboboxLiveV15CaptureCell,
  type ComboboxLiveV15CapturePayload,
  type ComboboxLiveV15RootProof,
  type ComboboxLiveV15SourceIdentity,
  type ComboboxLiveV15WriterOwnership,
} from "./combobox-live-v15-contract.js";
import {
  COMBOBOX_LIVE_V15_CAPTURE_MANIFEST_PATH,
  COMBOBOX_LIVE_V15_PLAN_PATH,
  type ComboboxLiveV15ProofPlan,
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v15.js";

export const COMBOBOX_LIVE_V15_RUNNER_VERSION = "combobox-live-v15-runner-v1";

interface ComboboxLiveV15JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV15OrchestratorState {
  artifactVersion: "combobox-live-v15-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV15JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV15ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV15CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV15SourceIdentity & {
...
export interface ComboboxLiveV15NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV15OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v15 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V15_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V15_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV15ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV15CaptureCell[];
  validateComboboxLiveV15CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v15 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v15 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV15WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV15RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV15Request =>
  readJson<ComboboxLiveV15Request>(file);

export class ComboboxLiveV15Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV15Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV15OrchestratorState = {
      artifactVersion: "combobox-live-v15-orchestrator-state-v1",
    ...
    return new ComboboxLiveV15Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV15OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV15Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV15NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV15NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v15 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV15NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v15 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV15RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV15Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV15CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV15AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV15WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV15Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV15ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV15Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV15Response(this.#directory, "writer", {
        validate: validateComboboxLiveV15WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV15Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV15RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV15Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV15ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV15Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV15RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v15-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV15Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV15ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV15Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV15ProbePayload(payload, ownership),
      });
      persistComboboxLiveV15TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v15 capture cell missing");
      const accepted = acceptComboboxLiveV15Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV15CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V15_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV15CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV15Objective(
    ...
    throw new Error(`Combobox live v15 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV15Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV15CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV15Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV15RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV15Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV15CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV15CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV15CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV15Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV15TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV15SimulationKey = (): KeyObject =>
