...
import { buildTableLiveV32CleanupRuntime } from "./table-live-v32-cleanup.js";
import { type TableLiveV32FixedFixedPoint } from "./table-live-v32-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV32RestoreProgram,
  validateTableLiveV32RestorePayload,
  type TableLiveV32RestorePayload,
} from "./table-live-v32-restore.js";
import {
  buildTableLiveV32RawPropertyRuntime,
  normalizeTableLiveV32Scene,
  type TableLiveV32RawNode,
} from "./table-live-v32-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v32.js";
import {
...
export const TABLE_LIVE_V32_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V32_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V32_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V32_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V32_VARIANT_COUNT = 20;
export const TABLE_LIVE_V32_SET_COUNT = 6;
export const TABLE_LIVE_V32_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V32_HOST_PHASES = 3;
export const TABLE_LIVE_V32_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V32_RESTORE_COUNT = 8;
export const TABLE_LIVE_V32_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V32_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV32SourceId = (typeof TABLE_LIVE_V32_SOURCE_IDS)[number];
export type TableLiveV32Kind = "table" | "row" | "cell";

export interface TableLiveV32SourceIdentity {
  source: TableLiveV32SourceId;
...
export interface TableLiveV32WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV32ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV32SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV32RawNode;
    rowScene: TableLiveV32RawNode;
    cellScene: TableLiveV32RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV32RootProof {
  source: TableLiveV32SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV32FixedFixedPoint;
}

export interface TableLiveV32ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV32SourceId;
    adapterIdentity: string;
...
export interface TableLiveV32CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV32SourceId;
  adapterIdentity: string;
...
export interface TableLiveV32CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV32SourceId;
  frameWidth: number;
...
export interface TableLiveV32CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v32 ${label} must be an object`);
...
export function validateTableLiveV32WriterPayload(
  value: unknown,
): TableLiveV32WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V32_NAMESPACE ||
    ...
    throw new TypeError("Table live v32 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Table live v32 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v32 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV32SourceIdentity,
  kind: TableLiveV32Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV32ExtractProgram(
  writer: TableLiveV32WriterOwnership,
  sources: readonly TableLiveV32SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
      "Table live v32 extract requires two source identities",
    );
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V32_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V32_NAMESPACE)}
${buildTableLiveV32RawPropertyRuntime()}
...
export function validateTableLiveV32ExtractPayload(
  value: unknown,
  writer: TableLiveV32WriterOwnership,
): TableLiveV32ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v32 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V32_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v32 extract schema/two-root mismatch");
  return payload as TableLiveV32ExtractPayload;
}

export function proveTableLiveV32Roots<Instance>(
  extract: TableLiveV32ExtractPayload,
  sources: readonly (TableLiveV32SourceIdentity & {
...
): TableLiveV32RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v32 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v32 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV32Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV32Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV32Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV32FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Table live v32 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v32 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v32 compile lost owned sets");
...
export function assertTableLiveV32RootProofs(
  proofs: readonly TableLiveV32RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v32 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v32 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV32ProbeProgram(
  writer: TableLiveV32WriterOwnership,
  sources: readonly TableLiveV32SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V32_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
for(const row of byAdapter.values()){
 ...
 const visitSet=(set,kind)=>{
  for(const component of set.children){
   ...
   void "TABLE-PROBE-CONTAINMENT-IS-NOT-OVERLAP";
...
export function validateTableLiveV32ProbePayload(
  value: unknown,
  writer: TableLiveV32WriterOwnership,
): TableLiveV32ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV32ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V32_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v32 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV32CaptureManifest(
  cells: readonly TableLiveV32CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V32_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V32_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V32_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V32_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v32 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV32CaptureCell[],
): string => {
  validateTableLiveV32CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV32CaptureProgram(
  writer: TableLiveV32WriterOwnership,
  cell: TableLiveV32CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V32_CAPTURE_COUNT
  )
    throw new TypeError("Table live v32 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Table live v32 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V32_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV32CapturePayload(
  value: unknown,
  cell: TableLiveV32CaptureCell,
  rawResponseBytes: number,
): TableLiveV32CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV32CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V32_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V32_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v32 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV32CaptureResponses(
  manifest: readonly TableLiveV32CaptureCell[],
  responses: readonly TableLiveV32CapturePayload[],
): void {
  validateTableLiveV32CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v32 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV32ObjectiveReport {
  artifactVersion: "table-live-v32-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV32Objective(
  manifest: readonly TableLiveV32CaptureCell[],
  responses: readonly TableLiveV32CapturePayload[],
): TableLiveV32ObjectiveReport {
  assertTableLiveV32CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v32-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV32CleanupProgram(
  writer: TableLiveV32WriterOwnership,
): string {
  const baseProgram = buildTableLiveV32CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V32_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v32 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV32CleanupPayload(
  payload: unknown,
  writer: TableLiveV32WriterOwnership,
): TableLiveV32CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as TableLiveV32CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v32 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V32_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV32WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
