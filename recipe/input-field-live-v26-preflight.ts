...
import {
  ...
  verifyInputLiveV26Authorization,
  type InputLiveV26AntecedentIndex,
  type InputLiveV26AuthorizationProof,
} from "./input-field-live-v26-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V26_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v26/operator-security-attestation-template.json";
export const INPUT_LIVE_V26_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v26-security-attestation.json";

export interface InputLiveV26SecurityAttestation {
...
export interface InputLiveV26PreflightReport {
  artifactVersion: "input-live-v26-preflight-v1";
...
export const inputLiveV26AttestationSha256 = (
  value: Omit<InputLiveV26SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV26SecurityAttestation(
  value: unknown,
  proof: InputLiveV26AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v26-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V26_TARGET) ||
...
export function validateInputLiveV26ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV26Authorization");
  const preflight = source.indexOf("runInputLiveV26Preflight");
  const initialize = source.indexOf("InputLiveV26Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV26Preflight(
  root: string,
  proof: InputLiveV26AuthorizationProof,
  attestationPath = INPUT_LIVE_V26_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV26PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V26_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V26_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V26_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV26AntecedentIndex;
  failures.push(...validateInputLiveV26AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V26_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V26_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v26-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV26ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV26SecurityAttestation;
  return {
    artifactVersion: "input-live-v26-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V26_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV26Preflight(
    process.cwd(),
    verifyInputLiveV26Authorization(),
    attestationPath,
