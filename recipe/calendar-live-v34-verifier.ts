...
import {
  verifyCalendarLiveV34FixedSceneFixedPoint,
  type CalendarLiveV34FixedFixedPoint,
} from "./calendar-live-v34-fixed-point.js";

export const CALENDAR_LIVE_V34_VERIFIER_VERSION = "calendar-live-v34-verifier-v1";

export function buildCalendarLiveV34RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV34RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV34RawNode[];
}

export interface CalendarLiveV34NormalizedScene {
...
export interface CalendarLiveV34SceneProof {
...
export function normalizeCalendarLiveV34Scene(
  root: CalendarLiveV34RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV34NormalizedScene {
  const canonicalBindings: CalendarLiveV34NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV34RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV34Scene<Instance>(
  normalized: CalendarLiveV34NormalizedScene,
...
): CalendarLiveV34SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV34FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV34ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV34PreCaptureGates(payload: unknown): void {
