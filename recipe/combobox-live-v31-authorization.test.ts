...
import { COMBOBOX_LIVE_V31_INDEX_PATH } from "./build-combobox-live-proof-v31.js";
import {
  COMBOBOX_LIVE_V31_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V31_TARGET,
} from "./combobox-live-v31-broker.js";
import {
  buildComboboxLiveV31AuthorizationArtifact,
  validateComboboxLiveV31AntecedentIndex,
  validateComboboxLiveV31AuthorizationArtifact,
  validateComboboxLiveV31History,
  validateComboboxLiveV31UnitTestSource,
  type ComboboxLiveV31AntecedentIndex,
  type ComboboxLiveV31AuthorizationArtifact,
  type ComboboxLiveV31AuthorizationProof,
  type ComboboxLiveV31HistoryState,
} from "./combobox-live-v31-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV31ControlFlowSource,
  validateComboboxLiveV31SecurityAttestation,
  type ComboboxLiveV31SecurityAttestation,
} from "./combobox-live-v31-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v31-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V31_INDEX_PATH);
const index = (): ComboboxLiveV31AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV31AntecedentIndex;

const artifact = (): ComboboxLiveV31AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV31AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV31HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV31HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV31AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v31/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV31AuthorizationArtifact,
): ComboboxLiveV31SecurityAttestation => {
  const body: Omit<ComboboxLiveV31SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V31_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV31History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV31History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV31History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV31History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV31History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV31UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV31UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV31AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV31AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV31AuthorizationArtifact(
        planted as ComboboxLiveV31AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV31SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV31SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV31SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV31ControlFlowSource(source), []);
});
