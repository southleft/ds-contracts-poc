...
export type ComboboxLiveV11HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV11AuthorizationArtifact {
  artifactVersion: "combobox-live-v11-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V11_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV11AntecedentIndex {
...
export interface ComboboxLiveV11HistoryState {
  index?: ComboboxLiveV11AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV11AuthorizationArtifact;
...
export interface ComboboxLiveV11AuthorizationProof extends ComboboxLiveV11TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V11_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V11_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V11_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V11_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v11-authorization") ||
  artifactPath.includes("combobox-live-v11-preflight") ||
  artifactPath.includes("combobox-live-v11-authorized") ||
  artifactPath.includes("create-combobox-live-v11-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v11-status") ||
...
export function validateComboboxLiveV11AntecedentIndex(
  index: ComboboxLiveV11AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v11-antecedent-index-v1" ||
...
export function validateComboboxLiveV11UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV11History\(\s*["']/.test(source) ||
    /readComboboxLiveV11HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV11AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV11AuthorizationArtifact {
  ...
  ) as ComboboxLiveV11AntecedentIndex;
  const indexFailures = validateComboboxLiveV11AntecedentIndex(index);
...
export function validateComboboxLiveV11AuthorizationArtifact(
  artifact: ComboboxLiveV11AuthorizationArtifact | undefined,
  index: ComboboxLiveV11AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v11-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v11" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V11_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V11_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V11_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V11_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V11_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V11_SOURCE_ROOTS ||
  ...
  if (
    ...
    failures.push(
      "v11 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
...
export function validateComboboxLiveV11History(
  state: ComboboxLiveV11HistoryState,
  expected: ComboboxLiveV11HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV11AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV11AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV11HistoryState(
  root = process.cwd(),
): ComboboxLiveV11HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V11_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV11AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V11_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V11_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V11_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV11History(
  expected: ComboboxLiveV11HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV11HistoryState {
  const state = readComboboxLiveV11HistoryState(root);
  const failures = validateComboboxLiveV11History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v11 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV11Authorization(
  root = process.cwd(),
): ComboboxLiveV11AuthorizationProof {
  const state = verifyComboboxLiveV11History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V11_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v11/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v11.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V11_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V11_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V11_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV11HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV11History(expected);
  process.stdout.write(
    `Combobox live v11 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
