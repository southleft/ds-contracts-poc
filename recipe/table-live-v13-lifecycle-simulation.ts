...
import { tableLiveV2Sha256 } from "./table-live-v13-broker.js";

export interface TableLiveV13LifecycleSimulationReport {
  artifactVersion: "table-live-v13-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV13AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV13Lifecycle(): Promise<TableLiveV13LifecycleSimulationReport> {
  await buildTableLiveV13Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V13_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV13AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v13-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V13_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V13_INDEX_PATH]);
    git(temporary, ["commit", "-m", "synthetic table v13 antecedent"]);
    const antecedentCommit = git(temporary, ["rev-parse", "HEAD"]);
    const pending = historyState(index, indexSha256, antecedentCommit);
    const pendingFailures = validateTableLiveV13History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v13 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV13History(pending, "authorized");
    if (
      !staleAuthorized.some((failure) => failure.includes("stale history mode"))
    )
      throw new Error("table v13 lifecycle did not refuse stale authorized mode");

    const { publicKey } = generateKeyPairSync("ed25519");
    const authorization = buildTableLiveV13AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V13_AUTHORIZATION_PATH]);
    git(temporary, ["commit", "-m", "synthetic table v13 authorization"]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V13_INDEX_PATH}`,
    ]);
    if (!committedAntecedentBefore.equals(committedAntecedentAfter))
      throw new Error(
        "table v13 antecedent index changed across authorization commit",
      );
    ...
    const authorizedFailures = validateTableLiveV13History(
      authorized,
      "authorized",
    );
    if (authorizedFailures.length)
      throw new Error(
        `table v13 lifecycle authorized phase failed:\n${authorizedFailures.join("\n")}`,
      );
    const stalePending = validateTableLiveV13History(authorized, "pending");
    if (!stalePending.some((failure) => failure.includes("stale history mode")))
      throw new Error("table v13 lifecycle did not refuse stale pending mode");

    await buildTableLiveV13Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V13_INDEX_PATH);
    if (!indexBefore.equals(indexAfter))
      throw new Error(
        "table v13 generated check changed after synthetic authorization",
      );
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV13AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
      throw new Error("table v13 antecedent hash set changed after authorization");
    return {
      artifactVersion: "table-live-v13-lifecycle-simulation-v1",
      syntheticGitCommits: 2,
      antecedentCheckBeforeAuthorization: true,
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV13Lifecycle(), null, 2)}\n`,
  );
