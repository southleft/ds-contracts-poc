...
import {
  buildInputLiveV80CaptureProgram,
  buildInputLiveV80ExtractProgram,
  buildInputLiveV80ProbeProgram,
  buildInputLiveV80RestoreProgram,
  INPUT_LIVE_V80_CAPTURE_COUNT,
  INPUT_LIVE_V80_RESPONSE_CONTRACTS,
  inputLiveV80CaptureManifestSha256,
  type InputLiveV80CaptureCell,
  type InputLiveV80SourceIdentity,
  type InputLiveV80WriterOwnership,
} from "./input-field-live-v80-contract.js";
import {
  INPUT_LIVE_V80_DYNAMIC_TOOL,
  INPUT_LIVE_V80_TARGET,
  inputLiveV80RequestSequence,
} from "./input-field-live-v80-broker.js";
import {
...
} from "./input-field-live-v80-restore.js";

export const INPUT_LIVE_V80_EVIDENCE_ROOT =
  "recipe/evidence/input-field-live-pivot-v80";
export const INPUT_LIVE_V80_PROTOCOL_PATH = `${INPUT_LIVE_V80_EVIDENCE_ROOT}/protocol.json`;
export const INPUT_LIVE_V80_PLAN_PATH = `${INPUT_LIVE_V80_EVIDENCE_ROOT}/proof-plan.json`;
export const INPUT_LIVE_V80_CAPTURE_MANIFEST_PATH = `${INPUT_LIVE_V80_EVIDENCE_ROOT}/capture-manifest.json`;
export const INPUT_LIVE_V80_REQUEST_MANIFEST_PATH = `${INPUT_LIVE_V80_EVIDENCE_ROOT}/request-manifest.json`;
export const INPUT_LIVE_V80_INDEX_PATH = `${INPUT_LIVE_V80_EVIDENCE_ROOT}/antecedent-index.json`;
export const INPUT_LIVE_V80_AUTHORIZATION_TEMPLATE_PATH = `${INPUT_LIVE_V80_EVIDENCE_ROOT}/authorization-template.json`;
export const INPUT_LIVE_V80_AUTHORIZATION_PATH = `${INPUT_LIVE_V80_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface InputLiveV80ProofPlan {
  artifactVersion: "input-live-v80-proof-plan-v1";
...
interface GeneratedSource {
  ...
  recipeHash: string;
  envelopeHash: string;
  expectedScenePlan: InputLiveV80SourceIdentity["expectedScenePlan"];
  outputPath: string;
  compressed: Buffer;
  metadata: InputLiveV80ProofPlan["sources"][number]["expectedScenePlan"];
}
...
const sourcePlans = (): GeneratedSource[] => {
  ...
  return prior.sources.map((source: Record<string, any>) => {
    ...
    if (!rewrittenText.includes(needle))
      throw new Error(
        `Input live v80 missing set layout.mode vertical fact: ${inputPath}`,
      );
    ...
    if (!modeRewritten.includes(paddingNeedle))
      throw new Error(
        `Input live v80 missing set layout.padding 0 fact: ${inputPath}`,
      );
    ...
    if (setMode?.value !== "horizontal")
      throw new Error(
        `Input live v80 set layout.mode rewrite failed: ${inputPath}`,
      );
    ...
    if (
      ...
      throw new Error(
        `Input live v80 set layout.padding rewrite failed: ${inputPath}`,
      );
    ...
    return {
      ...
      outputPath: `${INPUT_LIVE_V80_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
      compressed: rewrittenCompressed,
      metadata: {
        path: `${INPUT_LIVE_V80_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
...
const captureCells = (): InputLiveV80CaptureCell[] => {
  const cells = json<Record<string, any>>(V16_CAPTURE_MANIFEST)
    .cells as InputLiveV80CaptureCell[];
  inputLiveV80CaptureManifestSha256(cells);
  return cells;
};

const protocol = () => ({
  artifactVersion: "input-live-v80-external-operator-protocol-v1",
  protocolId: "input-live-v80",
  status:
  ...
  hostNormalization: {
    ...
    carriedSceneReadback: "recipe/scene-readback-v80.ts",
    carriedSceneReadbackRuntime: "recipe/scene-readback-runtime-v80.ts",
    carriedV3Verifier: "recipe/input-field-live-v3-verifier-v80.ts",
...
const requestManifest = (
  captures: readonly InputLiveV80CaptureCell[],
  programs: {
    writerSha256: string;
  ...
  const requests = [
    {
      requestId: "writer",
      sequence: inputLiveV80RequestSequence("writer"),
      phase: "writer",
      predecessorRequestId: null,
      programSha256: programs.writerSha256,
      response: INPUT_LIVE_V80_RESPONSE_CONTRACTS.writer,
    },
    {
      requestId: "cleanup",
      sequence: inputLiveV80RequestSequence("cleanup"),
      phase: "cleanup",
      predecessorRequestId: "writer",
      programBuilderSha256: programs.cleanupBuilderSha256,
      response: INPUT_LIVE_V80_RESPONSE_CONTRACTS.cleanup,
      availability: "persisted immediately after writer acceptance",
    },
    {
      requestId: "restore",
      sequence: inputLiveV80RequestSequence("restore"),
      phase: "restore",
      predecessorRequestId: "writer",
      programSha256: programs.restoreBlueprintSha256,
      response: INPUT_LIVE_V80_RESPONSE_CONTRACTS.restore,
      availability:
        "issued immediately after writer acceptance; before extract",
    },
    {
      requestId: "extract",
      sequence: inputLiveV80RequestSequence("extract"),
      phase: "extract",
      predecessorRequestId: "restore",
      programSha256: programs.extractBlueprintSha256,
      response: INPUT_LIVE_V80_RESPONSE_CONTRACTS.extract,
    },
    {
      requestId: "probe",
      sequence: inputLiveV80RequestSequence("probe"),
      phase: "probe",
      predecessorRequestId: "extract",
      programSha256: programs.probeBlueprintSha256,
      response: INPUT_LIVE_V80_RESPONSE_CONTRACTS.probe,
    },
    ...captures.map((cell) => ({
      requestId: `capture-${String(cell.index).padStart(3, "0")}`,
      sequence: inputLiveV80RequestSequence("capture", cell.index),
      phase: "capture",
      ...
      response: INPUT_LIVE_V80_RESPONSE_CONTRACTS.capture,
    })),
  ];
  return {
    artifactVersion: "input-live-v80-request-manifest-v1",
    expectedDynamicTool: INPUT_LIVE_V80_DYNAMIC_TOOL,
    target: INPUT_LIVE_V80_TARGET,
...
const lifecycleExcludedPaths = [
  INPUT_LIVE_V80_AUTHORIZATION_TEMPLATE_PATH,
  INPUT_LIVE_V80_AUTHORIZATION_PATH,
  "recipe/input-field-live-v80-authorization.ts",
  "recipe/input-field-live-v80-authorization.test.ts",
  "recipe/input-field-live-v80-preflight.ts",
  "recipe/input-field-live-v80-authorized.ts",
  "recipe/evidence/input-field-live-pivot-v80/operator-security-attestation-template.json",
...
const sourceNeutralDependencies = [
  ...
  "recipe/normalize.ts",
  "recipe/scene-readback-runtime-v80.ts",
  "recipe/scene-readback-v80.ts",
  "recipe/input-field-live-v80-restore.ts",
  "recipe/input-field-figma-writer-v18.ts",
  "recipe/input-field-figma-writer-v19.ts",
] as const;

export async function buildInputLiveV80Proof(
  check = process.argv.includes("--check"),
): Promise<InputLiveV80ProofPlan> {
  ...
  if (
    ...
    throw new Error("Input live v80 frozen v16 writer bytes drifted");
  ...
  if (
    ...
    throw new Error(
      "Input live v80 exact committed v16 writer/restore/runtime/extract or hashed v16/v17/v18/v19 writer or v17-v66/v70/v79 scene-readback bytes drifted",
    );

  const writerOwnershipBlueprint: InputLiveV80WriterOwnership = {
    pageId: "__WRITER_PAGE_ID__",
    pageName: "Recipe Pivot / Input Field / v9",
    runIdentity: "input-live-v80",
  ...
  const identities = sources.map(
    (source) =>
      ...
      }) satisfies InputLiveV80SourceIdentity,
  );
  const restoreBlueprint = Buffer.from(
    buildInputLiveV80RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildInputLiveV80ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildInputLiveV80ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildInputLiveV80CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );

  const antecedentOutputs = new Map<string, Buffer>();
  antecedentOutputs.set(INPUT_LIVE_V80_PROTOCOL_PATH, jsonBytes(protocol()));
  antecedentOutputs.set(
    `${INPUT_LIVE_V80_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V80_EVIDENCE_ROOT}/programs/writer-payload.js`,
    writerPayload,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V80_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V80_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V80_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V80_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  ...
  const captureManifest = {
    artifactVersion: "input-live-v80-capture-manifest-v1",
    ...
    cells: captures,
    cellsSha256: inputLiveV80CaptureManifestSha256(captures),
  };
  const captureManifestBytes = jsonBytes(captureManifest);
  antecedentOutputs.set(
    INPUT_LIVE_V80_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: InputLiveV80ProofPlan = {
    artifactVersion: "input-live-v80-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: INPUT_LIVE_V80_TARGET,
    namespace: "ds.contracts.input.recipe.v5",
    writer: {
      programPath: `${INPUT_LIVE_V80_EVIDENCE_ROOT}/programs/writer.txt`,
      programBytes: writerProgram.byteLength,
      programSha256: sha256(writerProgram),
      payloadPath: `${INPUT_LIVE_V80_EVIDENCE_ROOT}/programs/writer-payload.js`,
    ...
    captureManifest: {
      path: INPUT_LIVE_V80_CAPTURE_MANIFEST_PATH,
      cells: 128,
      requests: 128,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(INPUT_LIVE_V80_PLAN_PATH, proofPlanBytes);
  ...
  antecedentOutputs.set(
    INPUT_LIVE_V80_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/input-field-live-v80-broker.ts",
    "recipe/input-field-live-v80-contract.ts",
    "recipe/run-input-field-live-v80.ts",
    "recipe/build-input-field-live-proof-v80.ts",
    "recipe/input-field-live-v80-broker.test.ts",
    "recipe/figma-property-normalizer-v8.test.ts",
    "recipe/scene-readback-v80.test.ts",
    "recipe/input-field-live-v80-restore.ts",
    "recipe/input-field-live-v80-restore.test.ts",
    "recipe/input-field-live-v80-lifecycle-simulation.ts",
    ...sourceNeutralDependencies,
  ].filter((artifactPath) => artifactPath !== INPUT_LIVE_V80_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "input-live-v80-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    ...
    counts: {
      ...
      captureCells: INPUT_LIVE_V80_CAPTURE_COUNT,
  ...
  antecedentOutputs.set(INPUT_LIVE_V80_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "input-live-v80-authorization-template-v1",
    authorizationId: "input-live-v80",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: INPUT_LIVE_V80_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
  ...
  if (check) {
    ...
    mkdirSync(path.dirname(INPUT_LIVE_V80_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(INPUT_LIVE_V80_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(INPUT_LIVE_V80_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(INPUT_LIVE_V80_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildInputLiveV80Proof(), null, 2)}\n`,
  );
