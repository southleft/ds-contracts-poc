...
import { TABLE_LIVE_V35_INDEX_PATH } from "./build-table-live-proof-v35.js";
import {
...
import {
  buildTableLiveV35AuthorizationArtifact,
  validateTableLiveV35AntecedentIndex,
  validateTableLiveV35AuthorizationArtifact,
  validateTableLiveV35History,
  validateTableLiveV35UnitTestSource,
  type TableLiveV35AntecedentIndex,
  type TableLiveV35AuthorizationArtifact,
  type TableLiveV35AuthorizationProof,
  type TableLiveV35HistoryState,
} from "./table-live-v35-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV35ControlFlowSource,
  validateTableLiveV35SecurityAttestation,
  type TableLiveV35SecurityAttestation,
} from "./table-live-v35-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V35_INDEX_PATH);
const index = (): TableLiveV35AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV35AntecedentIndex;

const artifact = (): TableLiveV35AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV35AuthorizationArtifact({
...
const pendingState = (): TableLiveV35HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV35HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV35AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v35/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV35AuthorizationArtifact,
): TableLiveV35SecurityAttestation => {
  const body: Omit<TableLiveV35SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V35_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV35History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV35History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV35History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV35History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV35History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV35UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v35-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV35UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV35AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV35AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV35AuthorizationArtifact(
        planted as TableLiveV35AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV35SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV35SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV35SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v35-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV35ControlFlowSource(source), []);
});
