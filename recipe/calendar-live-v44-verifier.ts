...
import {
  verifyCalendarLiveV44FixedSceneFixedPoint,
  type CalendarLiveV44FixedFixedPoint,
} from "./calendar-live-v44-fixed-point.js";

export const CALENDAR_LIVE_V44_VERIFIER_VERSION = "calendar-live-v44-verifier-v1";

export function buildCalendarLiveV44RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV44RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV44RawNode[];
}

export interface CalendarLiveV44NormalizedScene {
...
export interface CalendarLiveV44SceneProof {
...
export function normalizeCalendarLiveV44Scene(
  root: CalendarLiveV44RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV44NormalizedScene {
  const canonicalBindings: CalendarLiveV44NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV44RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV44Scene<Instance>(
  normalized: CalendarLiveV44NormalizedScene,
...
): CalendarLiveV44SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV44FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV44ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV44PreCaptureGates(payload: unknown): void {
