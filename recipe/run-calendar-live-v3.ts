...
import {
  acceptCalendarLiveV3Response,
  createCalendarLiveV3Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV3Request,
  persistCalendarLiveV3RawResponse,
  persistCalendarLiveV3TechnicalGates,
  readCalendarLiveV3AcceptedRawPayload,
  type CalendarLiveV3Request,
  type CalendarLiveV3TransactionAuthorization,
} from "./calendar-live-v3-broker.js";
import {
  CALENDAR_LIVE_V3_CAPTURE_COUNT,
  assertCalendarLiveV3CaptureResponses,
  ...
  evaluateCalendarLiveV3Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV3Roots,
  validateCalendarLiveV3CaptureManifest,
  validateCalendarLiveV3CapturePayload,
  validateCalendarLiveV3CleanupPayload,
  validateCalendarLiveV3ExtractPayload,
  validateCalendarLiveV3ProbePayload,
  validateCalendarLiveV3WriterPayload,
  type CalendarLiveV3CaptureCell,
  type CalendarLiveV3CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V3_PLAN_PATH,
  type CalendarLiveV3ProofPlan,
} from "./build-calendar-live-proof-v3.js";
import {
...
export const CALENDAR_LIVE_V3_RUNNER_VERSION = "calendar-live-v3-runner-v1";

interface CalendarLiveV3JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV3OrchestratorState {
  artifactVersion: "calendar-live-v3-orchestrator-state-v1";
  ...
  journal: CalendarLiveV3JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV3ProofPlan;
  planSha256: string;
  captures: CalendarLiveV3CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV3SourceIdentity & {
...
export interface CalendarLiveV3NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV3OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v3 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V3_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V3_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV3ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV3CaptureCell[];
  validateCalendarLiveV3CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v3 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v3 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV3WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV3RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV3Request =>
  readJson<CalendarLiveV3Request>(file);

export class CalendarLiveV3Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV3Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV3OrchestratorState = {
      artifactVersion: "calendar-live-v3-orchestrator-state-v1",
    ...
    return new CalendarLiveV3Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV3OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV3Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV3NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV3NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v3 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV3NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v3 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV3RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV3Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV3CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV3AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV3WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV3Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV3ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV3Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV3Response(this.#directory, "writer", {
        validate: validateCalendarLiveV3WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV3Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV3RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV3Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV3ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV3Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV3RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v3-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV3Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV3ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV3Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV3ProbePayload(payload, ownership),
      });
      persistCalendarLiveV3TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v3 capture cell missing");
      const accepted = acceptCalendarLiveV3Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV3CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V3_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV3CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV3Objective(
    ...
    throw new Error(`Calendar live v3 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV3Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV3CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV3Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV3RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV3Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV3CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV3CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV3CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV3Authorization = (
  privateKey: KeyObject,
): CalendarLiveV3TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV3SimulationKey = (): KeyObject =>
