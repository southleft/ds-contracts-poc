...
import { INPUT_LIVE_V60_INDEX_PATH } from "./build-input-field-live-proof-v60.js";
import {
  INPUT_LIVE_V60_DYNAMIC_TOOL,
  INPUT_LIVE_V60_TARGET,
} from "./input-field-live-v60-broker.js";
import {
  buildInputLiveV60AuthorizationArtifact,
  validateInputLiveV60AntecedentIndex,
  validateInputLiveV60AuthorizationArtifact,
  validateInputLiveV60History,
  validateInputLiveV60UnitTestSource,
  type InputLiveV60AntecedentIndex,
  type InputLiveV60AuthorizationArtifact,
  type InputLiveV60AuthorizationProof,
  type InputLiveV60HistoryState,
} from "./input-field-live-v60-authorization.js";
import {
  inputLiveV60AttestationSha256,
  validateInputLiveV60ControlFlowSource,
  validateInputLiveV60SecurityAttestation,
  type InputLiveV60SecurityAttestation,
} from "./input-field-live-v60-preflight.js";
import { inputLiveV60Sha256 } from "./input-field-live-v60-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V60_INDEX_PATH);
const index = (): InputLiveV60AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV60AntecedentIndex;

const artifact = (): InputLiveV60AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV60AuthorizationArtifact({
...
const pendingState = (): InputLiveV60HistoryState => ({
  index: index(),
  indexSha256: inputLiveV60Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV60HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV60AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV60Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V60_TARGET,
  expectedDynamicTool: INPUT_LIVE_V60_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v60/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV60AuthorizationArtifact,
): InputLiveV60SecurityAttestation => {
  const body: Omit<InputLiveV60SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v60-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V60_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV60AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV60History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV60History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV60History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV60History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV60History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV60UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV60UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV60AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV60Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV60AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV60AuthorizationArtifact(
        planted as InputLiveV60AuthorizationArtifact,
        index(),
        inputLiveV60Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV60SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV60AttestationSha256(
      body as Omit<InputLiveV60SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV60SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV60ControlFlowSource(source), []);
});
