...
import {
  verifyTableLiveV34FixedSceneFixedPoint,
  type TableLiveV34FixedFixedPoint,
} from "./table-live-v34-fixed-point.js";

export const TABLE_LIVE_V34_VERIFIER_VERSION = "table-live-v34-verifier-v1";

export function buildTableLiveV34RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV34RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV34RawNode[];
}

export interface TableLiveV34NormalizedScene {
...
export interface TableLiveV34SceneProof {
...
export function normalizeTableLiveV34Scene(
  root: TableLiveV34RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV34NormalizedScene {
  const canonicalBindings: TableLiveV34NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV34RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV34Scene<Instance>(
  normalized: TableLiveV34NormalizedScene,
...
): TableLiveV34SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV34FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV34ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV34PreCaptureGates(payload: unknown): void {
