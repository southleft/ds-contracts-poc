...
import {
  verifyCalendarLiveV39FixedSceneFixedPoint,
  type CalendarLiveV39FixedFixedPoint,
} from "./calendar-live-v39-fixed-point.js";

export const CALENDAR_LIVE_V39_VERIFIER_VERSION = "calendar-live-v39-verifier-v1";

export function buildCalendarLiveV39RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV39RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV39RawNode[];
}

export interface CalendarLiveV39NormalizedScene {
...
export interface CalendarLiveV39SceneProof {
...
export function normalizeCalendarLiveV39Scene(
  root: CalendarLiveV39RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV39NormalizedScene {
  const canonicalBindings: CalendarLiveV39NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV39RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV39Scene<Instance>(
  normalized: CalendarLiveV39NormalizedScene,
...
): CalendarLiveV39SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV39FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV39ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV39PreCaptureGates(payload: unknown): void {
