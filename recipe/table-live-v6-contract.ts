...
import { buildTableLiveV6CleanupRuntime } from "./table-live-v6-cleanup.js";
import { type TableLiveV6FixedFixedPoint } from "./table-live-v6-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV6RestoreProgram,
  validateTableLiveV6RestorePayload,
  type TableLiveV6RestorePayload,
} from "./table-live-v6-restore.js";
import {
  buildTableLiveV6RawPropertyRuntime,
  normalizeTableLiveV6Scene,
  type TableLiveV6RawNode,
} from "./table-live-v6-verifier.js";
...
export const TABLE_LIVE_V6_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V6_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V6_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V6_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V6_VARIANT_COUNT = 20;
export const TABLE_LIVE_V6_SET_COUNT = 6;
export const TABLE_LIVE_V6_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V6_HOST_PHASES = 3;
export const TABLE_LIVE_V6_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V6_RESTORE_COUNT = 8;
export const TABLE_LIVE_V6_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V6_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV6SourceId = (typeof TABLE_LIVE_V6_SOURCE_IDS)[number];
export type TableLiveV6Kind = "table" | "row" | "cell";

export interface TableLiveV6SourceIdentity {
  source: TableLiveV6SourceId;
...
export interface TableLiveV6WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV6ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV6SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV6RawNode;
    rowScene: TableLiveV6RawNode;
    cellScene: TableLiveV6RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV6RootProof {
  source: TableLiveV6SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV6FixedFixedPoint;
}

export interface TableLiveV6ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV6SourceId;
    adapterIdentity: string;
...
export interface TableLiveV6CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV6SourceId;
  adapterIdentity: string;
...
export interface TableLiveV6CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV6SourceId;
  frameWidth: number;
...
export interface TableLiveV6CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v6 ${label} must be an object`);
...
export function validateTableLiveV6WriterPayload(
  value: unknown,
): TableLiveV6WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V6_NAMESPACE ||
    ...
    throw new TypeError("Table live v6 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v6 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v6 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV6SourceIdentity,
  kind: TableLiveV6Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV6ExtractProgram(
  writer: TableLiveV6WriterOwnership,
  sources: readonly TableLiveV6SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v6 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V6_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V6_NAMESPACE)}
${buildTableLiveV6RawPropertyRuntime()}
...
export function validateTableLiveV6ExtractPayload(
  value: unknown,
  writer: TableLiveV6WriterOwnership,
): TableLiveV6ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v6 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V6_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v6 extract schema/two-root mismatch");
  return payload as TableLiveV6ExtractPayload;
}

export function proveTableLiveV6Roots<Instance>(
  extract: TableLiveV6ExtractPayload,
  sources: readonly (TableLiveV6SourceIdentity & {
...
): TableLiveV6RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v6 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v6 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV6Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV6Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV6Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV6FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v6 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v6 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v6 compile lost owned sets");
...
export function assertTableLiveV6RootProofs(
  proofs: readonly TableLiveV6RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v6 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v6 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV6ProbeProgram(
  writer: TableLiveV6WriterOwnership,
  sources: readonly TableLiveV6SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V6_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
for(const row of byAdapter.values()){
 ...
 const cell0=propertyKey(rowInstance,"Cell 0"),cell0Before=cell0&&rowInstance.componentProperties[cell0].value;let textPropertiesRestored=false;if(cell0){rowInstance.setProperties({[cell0]:"Table v6 deterministic probe"});const changed=nodes(rowInstance).some(node=>node.type==="TEXT"&&node.characters==="Table v6 deterministic probe");rowInstance.setProperties({[cell0]:cell0Before});textPropertiesRestored=changed&&JSON.stringify(original)===JSON.stringify(plain(instance));}
 rowInstance.remove();
...
export function validateTableLiveV6ProbePayload(
  value: unknown,
  writer: TableLiveV6WriterOwnership,
): TableLiveV6ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV6ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V6_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V6_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Table live v6 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV6CaptureManifest(
  cells: readonly TableLiveV6CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V6_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V6_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V6_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V6_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v6 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV6CaptureCell[],
): string => {
  validateTableLiveV6CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV6CaptureProgram(
  writer: TableLiveV6WriterOwnership,
  cell: TableLiveV6CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V6_CAPTURE_COUNT
  )
    throw new TypeError("Table live v6 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v6 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
const frame=figma.createFrame();frame.name="Table v6 ephemeral / "+cell.cellKey;page.appendChild(frame);
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V6_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV6CapturePayload(
  value: unknown,
  cell: TableLiveV6CaptureCell,
  rawResponseBytes: number,
): TableLiveV6CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV6CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V6_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V6_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v6 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV6CaptureResponses(
  manifest: readonly TableLiveV6CaptureCell[],
  responses: readonly TableLiveV6CapturePayload[],
): void {
  validateTableLiveV6CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v6 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV6ObjectiveReport {
  artifactVersion: "table-live-v6-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV6Objective(
  manifest: readonly TableLiveV6CaptureCell[],
  responses: readonly TableLiveV6CapturePayload[],
): TableLiveV6ObjectiveReport {
  assertTableLiveV6CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v6-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV6CleanupProgram(
  writer: TableLiveV6WriterOwnership,
): string {
  const baseProgram = buildTableLiveV6CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V6_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v6 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV6CleanupPayload(
  payload: unknown,
  writer: TableLiveV6WriterOwnership,
): TableLiveV6CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV6CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v6 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V6_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV6WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
