...
export const INPUT_LIVE_V35_VERIFIER_VERSION = "input-live-v35-verifier-v1";

export function buildInputLiveV35RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV35RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV35RawNode[];
}

export interface InputLiveV35NormalizedScene {
...
export interface InputLiveV35SceneProof {
...
export function normalizeInputLiveV35Scene(
  root: InputLiveV35RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV35NormalizedScene {
  const canonicalBindings: InputLiveV35NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV35RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV35Scene<Instance>(
  normalized: InputLiveV35NormalizedScene,
...
export function compileInputLiveV35ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV35PreCaptureGates(payload: unknown): void {
