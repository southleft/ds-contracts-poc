/**
 * Table live v27 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V27_FIXED_VERIFIER_VERSION = "table-live-v27-fixed-point-v1";
export const TABLE_LIVE_V27_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV27FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV27FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV27FixedSourceProbe {
...
export interface TableLiveV27FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV27FixedFixedPoint;
}

export interface TableLiveV27FixedHardGateInput {
  thresholds: TableLiveV27FixedThresholds;
  sourceProbes: TableLiveV27FixedSourceProbe[];
  cells: TableLiveV27FixedCellValidation[];
  sceneProofs: TableLiveV27FixedSceneProof[];
...
export interface TableLiveV27FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V27_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV27FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV27FixedHardGates(
  input: TableLiveV27FixedHardGateInput,
): TableLiveV27FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV27FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V27_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV27FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV27FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
