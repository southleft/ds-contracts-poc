...
import { COMBOBOX_LIVE_V36_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v36.js";
import { verifyComboboxLiveV36Authorization } from "./combobox-live-v36-authorization.js";
import { runComboboxLiveV36Preflight } from "./combobox-live-v36-preflight.js";
import { ComboboxLiveV36Orchestrator } from "./run-combobox-live-v36.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v36 requires exact authorization artifact ${COMBOBOX_LIVE_V36_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v36-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV36Authorization(root);
    runComboboxLiveV36Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV36Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV36Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
