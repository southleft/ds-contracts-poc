...
import {
  ...
  verifyInputLiveV9Authorization,
  type InputLiveV9AntecedentIndex,
  type InputLiveV9AuthorizationProof,
} from "./input-field-live-v9-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V9_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v9/operator-security-attestation-template.json";
export const INPUT_LIVE_V9_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v9-security-attestation.json";

export interface InputLiveV9SecurityAttestation {
...
export interface InputLiveV9PreflightReport {
  artifactVersion: "input-live-v9-preflight-v1";
...
export const inputLiveV9AttestationSha256 = (
  value: Omit<InputLiveV9SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV9SecurityAttestation(
  value: unknown,
  proof: InputLiveV9AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v9-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V9_TARGET) ||
...
export function validateInputLiveV9ControlFlowSource(source: string): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV9Authorization");
  const preflight = source.indexOf("runInputLiveV9Preflight");
  const initialize = source.indexOf("InputLiveV9Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV9Preflight(
  root: string,
  proof: InputLiveV9AuthorizationProof,
  attestationPath = INPUT_LIVE_V9_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV9PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V9_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V9_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V9_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV9AntecedentIndex;
  failures.push(...validateInputLiveV9AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V9_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V9_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v9-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV9ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(`Input live v9 preflight refused:\n${failures.join("\n")}`);
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as InputLiveV9SecurityAttestation;
  return {
    artifactVersion: "input-live-v9-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V9_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV9Preflight(
    process.cwd(),
    verifyInputLiveV9Authorization(),
    attestationPath,
