...
export const INPUT_LIVE_V69_VERIFIER_VERSION = "input-live-v69-verifier-v1";

export function buildInputLiveV69RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV69RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV69RawNode[];
}

export interface InputLiveV69NormalizedScene {
...
export interface InputLiveV69SceneProof {
...
export function normalizeInputLiveV69Scene(
  root: InputLiveV69RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV69NormalizedScene {
  const canonicalBindings: InputLiveV69NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV69RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV69Scene<Instance>(
  normalized: InputLiveV69NormalizedScene,
...
export function compileInputLiveV69ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV69PreCaptureGates(payload: unknown): void {
