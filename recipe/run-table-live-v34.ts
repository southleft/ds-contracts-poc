...
import {
  acceptTableLiveV34Response,
  createTableLiveV34Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV34Request,
  persistTableLiveV34RawResponse,
  persistTableLiveV34TechnicalGates,
  readTableLiveV34AcceptedRawPayload,
  type TableLiveV34Request,
  type TableLiveV34TransactionAuthorization,
} from "./table-live-v34-broker.js";
import {
  TABLE_LIVE_V34_CAPTURE_COUNT,
  assertTableLiveV34CaptureResponses,
  ...
  evaluateTableLiveV34Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV34Roots,
  validateTableLiveV34CaptureManifest,
  validateTableLiveV34CapturePayload,
  validateTableLiveV34CleanupPayload,
  validateTableLiveV34ExtractPayload,
  validateTableLiveV34ProbePayload,
  validateTableLiveV34WriterPayload,
  type TableLiveV34CaptureCell,
  type TableLiveV34CapturePayload,
...
import {
  ...
  TABLE_LIVE_V34_PLAN_PATH,
  type TableLiveV34ProofPlan,
} from "./build-table-live-proof-v34.js";
import {
...
export const TABLE_LIVE_V34_RUNNER_VERSION = "table-live-v34-runner-v1";

interface TableLiveV34JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV34OrchestratorState {
  artifactVersion: "table-live-v34-orchestrator-state-v1";
  ...
  journal: TableLiveV34JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV34ProofPlan;
  planSha256: string;
  captures: TableLiveV34CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV34SourceIdentity & {
...
export interface TableLiveV34NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV34OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v34 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V34_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V34_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV34ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV34CaptureCell[];
  validateTableLiveV34CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v34 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v34 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV34WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV34RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV34Request =>
  readJson<TableLiveV34Request>(file);

export class TableLiveV34Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV34Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV34OrchestratorState = {
      artifactVersion: "table-live-v34-orchestrator-state-v1",
    ...
    return new TableLiveV34Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV34OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV34Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV34NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV34NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v34 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV34NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v34 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV34RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV34Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV34CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV34AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV34WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV34Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV34ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV34Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV34Response(this.#directory, "writer", {
        validate: validateTableLiveV34WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV34Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV34RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV34Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV34ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV34Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV34RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v34-host-root-proofs-v1",
      ...
      const next = issueTableLiveV34Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV34ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV34Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV34ProbePayload(payload, ownership),
      });
      persistTableLiveV34TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v34 capture cell missing");
      const accepted = acceptTableLiveV34Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV34CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V34_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV34CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV34Objective(
    ...
    throw new Error(`Table live v34 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV34Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV34CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV34Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV34RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV34Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV34CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV34CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV34CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV34Authorization = (
  privateKey: KeyObject,
): TableLiveV34TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV34SimulationKey = (): KeyObject =>
