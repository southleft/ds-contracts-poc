...
export interface InputLiveV27LifecycleSimulationReport {
  artifactVersion: "input-live-v27-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV27AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV27Lifecycle(): Promise<InputLiveV27LifecycleSimulationReport> {
  await buildInputLiveV27Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V27_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV27AntecedentIndex;
  const indexSha256 = inputLiveV27Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v27-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V27_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V27_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV27History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV27History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV27AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V27_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V27_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV27History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV27History(authorized, "pending");
    ...
    await buildInputLiveV27Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V27_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV27AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV27Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV27Lifecycle(), null, 2)}\n`,
  );
