...
import { calendarLiveV1Sha256 } from "./calendar-live-v20-broker.js";

export interface CalendarLiveV20LifecycleSimulationReport {
  artifactVersion: "calendar-live-v20-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV20AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV20Lifecycle(): Promise<CalendarLiveV20LifecycleSimulationReport> {
  await buildCalendarLiveV20Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V20_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV20AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v20-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V20_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V20_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV20History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV20History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV20AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V20_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V20_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV20History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV20History(authorized, "pending");
    ...
    await buildCalendarLiveV20Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V20_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV20AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV20Lifecycle(), null, 2)}\n`,
  );
