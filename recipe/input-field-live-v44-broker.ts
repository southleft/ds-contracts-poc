...
import {
  assertInputLiveV44RootProofs,
  INPUT_LIVE_V44_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V44_RESPONSE_CONTRACTS,
  type InputLiveV44RootProof,
} from "./input-field-live-v44-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V44_BROKER_VERSION = "input-live-v44-broker-v2";
export const INPUT_LIVE_V44_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V44_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V44_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V44_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V44_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V44_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV44RemotePhase =
  (typeof INPUT_LIVE_V44_REMOTE_PHASES)[number];
export type InputLiveV44AuthorizationMode = "live" | "simulated";

export interface InputLiveV44CommitPins {
...
export interface InputLiveV44TransactionAuthorization extends InputLiveV44CommitPins {
  mode: InputLiveV44AuthorizationMode;
...
export interface InputLiveV44Manifest {
  artifactVersion: "input-live-v44-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V44_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V44_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V44_TARGET;
  authorization: InputLiveV44TransactionAuthorization;
...
export interface InputLiveV44ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV44RequestBody {
  artifactVersion: "input-live-v44-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V44_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV44RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV44CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V44_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV44ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV44Request extends InputLiveV44RequestBody {
...
export interface InputLiveV44Receipt {
  artifactVersion: "input-live-v44-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V44_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV44RemotePhase;
...
export interface InputLiveV44AcceptedResponse<T = unknown> {
  request: InputLiveV44Request;
  receipt: InputLiveV44Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV44IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV44ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V44_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V44_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V44_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V44_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V44_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V44_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V44_TARGET.editorType;
};
...
export const inputLiveV44RequestSequence = (
  phase: InputLiveV44RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV44RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV44RequestPath = (
  directory: string,
  phase: InputLiveV44RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV44ResponsePath = (
  directory: string,
  phase: InputLiveV44RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV44ReceiptPath = (
  directory: string,
  phase: InputLiveV44RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV44Manifest => {
  const manifest = json<InputLiveV44Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v44-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V44_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V44_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV44RemotePhase,
  captureIndex?: number,
): InputLiveV44Request | undefined => {
  const file = inputLiveV44RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV44Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV44RemotePhase,
  captureIndex?: number,
): InputLiveV44Receipt | undefined => {
  const file = inputLiveV44ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV44Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV44Request | undefined;
  receipt: InputLiveV44Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV44RemotePhase,
): InputLiveV44ResponseContract => ({
  ...INPUT_LIVE_V44_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V44_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V44_DYNAMIC_TOOL,
      target: INPUT_LIVE_V44_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v44-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V44_BROKER_VERSION,
  target: INPUT_LIVE_V44_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V44_PROGRAM_START*/${program}/*INPUT_LIVE_V44_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V44_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V44_PROGRAM_END*/";
...
export function createInputLiveV44Transaction(
  directory: string,
  ...
  const manifest: InputLiveV44Manifest = {
    artifactVersion: "input-live-v44-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V44_BROKER_VERSION,
...
export function validateInputLiveV44Request(
  directory: string,
  request: InputLiveV44Request,
): void {
  ...
  const expectedSequence = inputLiveV44RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v44-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V44_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V44_TARGET.fileKey ||
...
export function issueInputLiveV44Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV44RemotePhase,
  program: string,
  options: InputLiveV44IssueOptions = {},
): InputLiveV44Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV44RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV44Request>(targetPath);
    validateInputLiveV44Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV44ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV44TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV44RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V44_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V44_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V44_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV44RequestBody = {
    artifactVersion: "input-live-v44-broker-request-v2",
    brokerVersion: INPUT_LIVE_V44_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV44Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV44Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV44RawResponse(
  directory: string,
  phase: InputLiveV44RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV44RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV44Request>(requestFile);
  if (
  ...
  const target = inputLiveV44ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV44Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v44-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V44_BROKER_VERSION,
...
export function acceptInputLiveV44Response<T = unknown>(
  directory: string,
  phase: InputLiveV44RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV44AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV44ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV44Request>(
    inputLiveV44RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV44Request(directory, request);
  if (phase === "capture") validateInputLiveV44TechnicalGates(directory);
  const rawFile = inputLiveV44ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v44-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V44_BROKER_VERSION ||
...
export function persistInputLiveV44TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV44RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV44RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V44_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v44-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V44_BROKER_VERSION,
...
export function validateInputLiveV44TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v44-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V44_TECHNICAL_GATES.some(
...
export function validateInputLiveV44Receipt(
  directory: string,
  phase: InputLiveV44RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV44Request(directory, request);
  const receipt = json<InputLiveV44Receipt>(
    inputLiveV44ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV44ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v44-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V44_BROKER_VERSION ||
...
export function readInputLiveV44AcceptedRawPayload(
  directory: string,
  phase: InputLiveV44RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV44Receipt(directory, phase, captureIndex);
...
export function createInputLiveV44FakeDynamicResponse(
  request: InputLiveV44Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V44_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V44_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV44PersistedRequests(
  directory: string,
): InputLiveV44Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV44Request>(file)] : [];
    });
}

export const inputLiveV44Sha256 = sha256;
