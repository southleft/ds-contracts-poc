...
export interface InputLiveV74LifecycleSimulationReport {
  artifactVersion: "input-live-v74-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV74AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV74Lifecycle(): Promise<InputLiveV74LifecycleSimulationReport> {
  await buildInputLiveV74Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V74_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV74AntecedentIndex;
  const indexSha256 = inputLiveV74Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v74-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V74_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V74_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV74History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV74History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV74AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V74_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V74_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV74History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV74History(authorized, "pending");
    ...
    await buildInputLiveV74Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V74_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV74AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV74Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV74Lifecycle(), null, 2)}\n`,
  );
