...
import {
  ...
  INPUT_LIVE_V50_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V50_RESPONSE_CONTRACTS,
  type InputLiveV50RootProof,
} from "./input-field-live-v50-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V50_BROKER_VERSION = "input-live-v50-broker-v2";
export const INPUT_LIVE_V50_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V50_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V50_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V50_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V50_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V50_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV50RemotePhase =
  (typeof INPUT_LIVE_V50_REMOTE_PHASES)[number];
export type InputLiveV50AuthorizationMode = "live" | "simulated";

export interface InputLiveV50CommitPins {
...
export interface InputLiveV50TransactionAuthorization extends InputLiveV50CommitPins {
  mode: InputLiveV50AuthorizationMode;
...
export interface InputLiveV50Manifest {
  artifactVersion: "input-live-v50-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V50_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V50_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V50_TARGET;
...
export interface InputLiveV50ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV50RequestBody {
  artifactVersion: "input-live-v50-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V50_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV50RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV50CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V50_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV50ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV50Request extends InputLiveV50RequestBody {
...
export interface InputLiveV50Receipt {
  artifactVersion: "input-live-v50-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V50_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV50RemotePhase;
...
export interface InputLiveV50AcceptedResponse<T = unknown> {
  request: InputLiveV50Request;
  receipt: InputLiveV50Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV50IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV50ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V50_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V50_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V50_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V50_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V50_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V50_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V50_TARGET.editorType;
};
...
export const inputLiveV50RequestSequence = (
  phase: InputLiveV50RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV50RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV50RequestPath = (
  directory: string,
  phase: InputLiveV50RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV50ResponsePath = (
  directory: string,
  phase: InputLiveV50RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV50ReceiptPath = (
  directory: string,
  phase: InputLiveV50RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV50Manifest => {
  const manifest = json<InputLiveV50Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v50-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V50_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V50_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV50RemotePhase,
  captureIndex?: number,
): InputLiveV50Request | undefined => {
  const file = inputLiveV50RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV50Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV50RemotePhase,
  captureIndex?: number,
): InputLiveV50Receipt | undefined => {
  const file = inputLiveV50ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV50Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV50Request | undefined;
  receipt: InputLiveV50Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV50RemotePhase,
): InputLiveV50ResponseContract => ({
  ...INPUT_LIVE_V50_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V50_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v50-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V50_BROKER_VERSION,
  target: INPUT_LIVE_V50_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V50_PROGRAM_START*/${program}/*INPUT_LIVE_V50_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V50_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V50_PROGRAM_END*/";
...
export function createInputLiveV50Transaction(
  directory: string,
  ...
  const manifest: InputLiveV50Manifest = {
    artifactVersion: "input-live-v50-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V50_BROKER_VERSION,
...
export function validateInputLiveV50Request(
  directory: string,
  request: InputLiveV50Request,
): void {
  ...
  const expectedSequence = inputLiveV50RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v50-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V50_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V50_TARGET.fileKey ||
...
export function issueInputLiveV50Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV50RemotePhase,
  program: string,
  options: InputLiveV50IssueOptions = {},
): InputLiveV50Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV50RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV50Request>(targetPath);
    validateInputLiveV50Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV50ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV50TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV50RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V50_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V50_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V50_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV50RequestBody = {
    artifactVersion: "input-live-v50-broker-request-v2",
    brokerVersion: INPUT_LIVE_V50_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV50Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV50Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV50RawResponse(
  directory: string,
  phase: InputLiveV50RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV50RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV50Request>(requestFile);
  if (
  ...
  const target = inputLiveV50ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV50Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v50-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V50_BROKER_VERSION,
...
export function acceptInputLiveV50Response<T = unknown>(
  directory: string,
  phase: InputLiveV50RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV50AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV50ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV50Request>(
    inputLiveV50RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV50Request(directory, request);
  if (phase === "capture") validateInputLiveV50TechnicalGates(directory);
  const rawFile = inputLiveV50ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v50-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V50_BROKER_VERSION ||
...
export function persistInputLiveV50TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV50RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV50RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V50_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v50-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V50_BROKER_VERSION,
...
export function validateInputLiveV50TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v50-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V50_TECHNICAL_GATES.some(
...
export function validateInputLiveV50Receipt(
  directory: string,
  phase: InputLiveV50RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV50Request(directory, request);
  const receipt = json<InputLiveV50Receipt>(
    inputLiveV50ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV50ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v50-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V50_BROKER_VERSION ||
...
export function readInputLiveV50AcceptedRawPayload(
  directory: string,
  phase: InputLiveV50RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV50Receipt(directory, phase, captureIndex);
...
export function createInputLiveV50FakeDynamicResponse(
  request: InputLiveV50Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V50_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V50_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV50PersistedRequests(
  directory: string,
): InputLiveV50Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV50Request>(file)] : [];
    });
}

export const inputLiveV50Sha256 = sha256;
