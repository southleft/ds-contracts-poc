...
import { buildComboboxLiveV36CleanupRuntime } from "./combobox-live-v36-cleanup.js";
import {
  type ComboboxLiveV36FixedFixedPoint,
} from "./combobox-live-v36-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV36RestoreProgram,
  validateComboboxLiveV36RestorePayload,
  type ComboboxLiveV36RestorePayload,
} from "./combobox-live-v36-restore.js";
import {
  buildComboboxLiveV36RawPropertyRuntime,
  normalizeComboboxLiveV36Scene,
  type ComboboxLiveV36RawNode,
} from "./combobox-live-v36-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v36.js";
import {
...
} from "./scene-readback-combobox-v36.js";
...
export const COMBOBOX_LIVE_V36_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V36_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V36_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V36_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V36_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V36_SET_COUNT = 4;
export const COMBOBOX_LIVE_V36_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V36_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V36_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V36_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V36_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV36SourceId = (typeof COMBOBOX_LIVE_V36_SOURCE_IDS)[number];

export interface ComboboxLiveV36SourceIdentity {
  source: ComboboxLiveV36SourceId;
...
export interface ComboboxLiveV36WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV36ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV36SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV36RawNode;
    optionScene: ComboboxLiveV36RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV36RootProof {
  source: ComboboxLiveV36SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV36FixedFixedPoint;
}

export interface ComboboxLiveV36ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV36SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV36CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV36SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV36CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV36SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV36CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v36 ${label} must be an object`);
...
export function validateComboboxLiveV36WriterPayload(
  value: unknown,
): ComboboxLiveV36WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V36_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v36 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v36 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v36 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV36SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV36ExtractProgram(
  writer: ComboboxLiveV36WriterOwnership,
  sources: readonly ComboboxLiveV36SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v36 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V36_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V36_NAMESPACE)}
${buildComboboxLiveV36RawPropertyRuntime()}
...
export function validateComboboxLiveV36ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV36WriterOwnership,
): ComboboxLiveV36ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v36 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V36_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v36 extract schema/two-root mismatch");
  return payload as ComboboxLiveV36ExtractPayload;
}

export function proveComboboxLiveV36Roots<Instance>(
  extract: ComboboxLiveV36ExtractPayload,
  sources: readonly (ComboboxLiveV36SourceIdentity & {
...
): ComboboxLiveV36RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v36 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v36 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV36Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV36Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV36FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v36 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v36 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v36 compile lost owned sets");
...
export function assertComboboxLiveV36RootProofs(
  proofs: readonly ComboboxLiveV36RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v36 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v36 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV36ProbeProgram(
  writer: ComboboxLiveV36WriterOwnership,
  sources: readonly ComboboxLiveV36SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V36_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV36ProbePayload(
  value: unknown,
  writer: ComboboxLiveV36WriterOwnership,
): ComboboxLiveV36ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV36ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V36_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V36_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v36 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV36CaptureManifest(
  cells: readonly ComboboxLiveV36CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V36_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V36_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V36_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V36_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v36 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV36CaptureCell[],
): string => {
  validateComboboxLiveV36CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV36CaptureProgram(
  writer: ComboboxLiveV36WriterOwnership,
  cell: ComboboxLiveV36CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V36_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v36 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v36 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V36_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV36CapturePayload(
  value: unknown,
  cell: ComboboxLiveV36CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV36CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV36CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V36_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V36_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v36 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV36CaptureResponses(
  manifest: readonly ComboboxLiveV36CaptureCell[],
  responses: readonly ComboboxLiveV36CapturePayload[],
): void {
  validateComboboxLiveV36CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v36 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV36ObjectiveReport {
  artifactVersion: "combobox-live-v36-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV36Objective(
  manifest: readonly ComboboxLiveV36CaptureCell[],
  responses: readonly ComboboxLiveV36CapturePayload[],
): ComboboxLiveV36ObjectiveReport {
  assertComboboxLiveV36CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v36-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV36CleanupProgram(
  writer: ComboboxLiveV36WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV36CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V36_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v36 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV36CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV36WriterOwnership,
): ComboboxLiveV36CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV36CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v36 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V36_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV36WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
