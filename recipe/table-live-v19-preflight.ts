...
import {
  ...
  verifyTableLiveV19Authorization,
  type TableLiveV19AntecedentIndex,
  type TableLiveV19AuthorizationProof,
} from "./table-live-v19-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V19_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v19/operator-security-attestation-template.json";
export const TABLE_LIVE_V19_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v19-security-attestation.json";

export interface TableLiveV19SecurityAttestation {
...
export interface TableLiveV19PreflightReport {
  artifactVersion: "table-live-v19-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV19SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV19SecurityAttestation(
  value: unknown,
  proof: TableLiveV19AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v19-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V19_TARGET) ||
...
export function validateTableLiveV19ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV19Authorization");
  const preflight = source.indexOf("runTableLiveV19Preflight");
  const initialize = source.indexOf("TableLiveV19Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV19Preflight(
  root: string,
  proof: TableLiveV19AuthorizationProof,
  attestationPath = TABLE_LIVE_V19_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV19PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V19_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V19_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV19AntecedentIndex;
  failures.push(...validateTableLiveV19AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V19_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V19_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v19-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV19ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v19 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV19SecurityAttestation;
  return {
    artifactVersion: "table-live-v19-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V19_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV19Preflight(
    process.cwd(),
    verifyTableLiveV19Authorization(),
    attestationPath,
