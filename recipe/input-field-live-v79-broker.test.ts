...
import {
  ...
  createInputLiveV79FakeDynamicResponse,
  ...
  listInputLiveV79PersistedRequests,
  persistInputLiveV79RawResponse,
  persistInputLiveV79TechnicalGates,
  validateInputLiveV79Receipt,
  validateInputLiveV79Request,
  type InputLiveV79RemotePhase,
...
import {
  ...
  type InputLiveV79ExtractPayload,
  type InputLiveV79ProbePayload,
...
import {
  ...
  simulatedInputLiveV79Authorization,
} from "./run-input-field-live-v79.js";
...
const captures: InputLiveV79CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV79Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v79-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV79Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV79CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV79FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV79FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V79_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V79_TARGET);
  persistInputLiveV79RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV79Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV79Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV79WriterOwnership,
...
const rootProofs = (): InputLiveV79RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v79-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV79Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV79Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV79RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV79RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV79ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV79ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV79CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV79CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV79CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV79Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV79CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV79PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV79ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV79Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV79Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV79Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV79Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V79_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV79RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V79_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV79Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV79Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV79ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV79Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV79WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV79RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV79Request;
    validateInputLiveV79Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV79Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV79CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v79-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV79Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV79Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV79Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V79_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV79RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V79_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV79Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV79Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV79RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V79_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV79Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV79Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV79Request;
