...
import {
  validateComboboxLiveV23AntecedentIndex,
  verifyComboboxLiveV23Authorization,
  type ComboboxLiveV23AntecedentIndex,
  type ComboboxLiveV23AuthorizationProof,
} from "./combobox-live-v23-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V23_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v23/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V23_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v23-security-attestation.json";

export interface ComboboxLiveV23SecurityAttestation {
  artifactVersion: "combobox-live-v23-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV23PreflightReport {
  artifactVersion: "combobox-live-v23-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV23SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV23SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV23AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v23-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V23_TARGET) ||
...
export function validateComboboxLiveV23ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV23Authorization");
  const preflight = source.indexOf("runComboboxLiveV23Preflight");
  const initialize = source.indexOf("ComboboxLiveV23Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV23Preflight(
  root: string,
  proof: ComboboxLiveV23AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V23_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV23PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V23_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V23_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V23_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV23AntecedentIndex;
  failures.push(...validateComboboxLiveV23AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V23_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V23_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV23SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v23-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV23ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v23 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV23SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v23-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V23_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV23Preflight(
    process.cwd(),
    verifyComboboxLiveV23Authorization(),
    attestationPath,
