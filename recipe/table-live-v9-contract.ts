...
import { buildTableLiveV9CleanupRuntime } from "./table-live-v9-cleanup.js";
import { type TableLiveV9FixedFixedPoint } from "./table-live-v9-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV9RestoreProgram,
  validateTableLiveV9RestorePayload,
  type TableLiveV9RestorePayload,
} from "./table-live-v9-restore.js";
import {
  buildTableLiveV9RawPropertyRuntime,
  normalizeTableLiveV9Scene,
  type TableLiveV9RawNode,
} from "./table-live-v9-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v9.js";
import {
...
export const TABLE_LIVE_V9_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V9_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V9_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V9_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V9_VARIANT_COUNT = 20;
export const TABLE_LIVE_V9_SET_COUNT = 6;
export const TABLE_LIVE_V9_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V9_HOST_PHASES = 3;
export const TABLE_LIVE_V9_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V9_RESTORE_COUNT = 8;
export const TABLE_LIVE_V9_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V9_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV9SourceId = (typeof TABLE_LIVE_V9_SOURCE_IDS)[number];
export type TableLiveV9Kind = "table" | "row" | "cell";

export interface TableLiveV9SourceIdentity {
  source: TableLiveV9SourceId;
...
export interface TableLiveV9WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV9ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV9SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV9RawNode;
    rowScene: TableLiveV9RawNode;
    cellScene: TableLiveV9RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV9RootProof {
  source: TableLiveV9SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV9FixedFixedPoint;
}

export interface TableLiveV9ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV9SourceId;
    adapterIdentity: string;
...
export interface TableLiveV9CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV9SourceId;
  adapterIdentity: string;
...
export interface TableLiveV9CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV9SourceId;
  frameWidth: number;
...
export interface TableLiveV9CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v9 ${label} must be an object`);
...
export function validateTableLiveV9WriterPayload(
  value: unknown,
): TableLiveV9WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V9_NAMESPACE ||
    ...
    throw new TypeError("Table live v9 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v9 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v9 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV9SourceIdentity,
  kind: TableLiveV9Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV9ExtractProgram(
  writer: TableLiveV9WriterOwnership,
  sources: readonly TableLiveV9SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v9 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V9_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V9_NAMESPACE)}
${buildTableLiveV9RawPropertyRuntime()}
...
export function validateTableLiveV9ExtractPayload(
  value: unknown,
  writer: TableLiveV9WriterOwnership,
): TableLiveV9ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v9 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V9_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v9 extract schema/two-root mismatch");
  return payload as TableLiveV9ExtractPayload;
}

export function proveTableLiveV9Roots<Instance>(
  extract: TableLiveV9ExtractPayload,
  sources: readonly (TableLiveV9SourceIdentity & {
...
): TableLiveV9RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v9 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v9 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV9Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV9Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV9Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV9FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v9 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v9 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v9 compile lost owned sets");
...
export function assertTableLiveV9RootProofs(
  proofs: readonly TableLiveV9RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v9 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v9 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV9ProbeProgram(
  writer: TableLiveV9WriterOwnership,
  sources: readonly TableLiveV9SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V9_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV9ProbePayload(
  value: unknown,
  writer: TableLiveV9WriterOwnership,
): TableLiveV9ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV9ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V9_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V9_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Table live v9 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV9CaptureManifest(
  cells: readonly TableLiveV9CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V9_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V9_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V9_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V9_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v9 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV9CaptureCell[],
): string => {
  validateTableLiveV9CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV9CaptureProgram(
  writer: TableLiveV9WriterOwnership,
  cell: TableLiveV9CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V9_CAPTURE_COUNT
  )
    throw new TypeError("Table live v9 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v9 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
const frame=figma.createFrame();frame.name="Table v9 ephemeral / "+cell.cellKey;page.appendChild(frame);
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V9_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV9CapturePayload(
  value: unknown,
  cell: TableLiveV9CaptureCell,
  rawResponseBytes: number,
): TableLiveV9CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV9CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V9_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V9_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v9 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV9CaptureResponses(
  manifest: readonly TableLiveV9CaptureCell[],
  responses: readonly TableLiveV9CapturePayload[],
): void {
  validateTableLiveV9CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v9 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV9ObjectiveReport {
  artifactVersion: "table-live-v9-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV9Objective(
  manifest: readonly TableLiveV9CaptureCell[],
  responses: readonly TableLiveV9CapturePayload[],
): TableLiveV9ObjectiveReport {
  assertTableLiveV9CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v9-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV9CleanupProgram(
  writer: TableLiveV9WriterOwnership,
): string {
  const baseProgram = buildTableLiveV9CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V9_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v9 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV9CleanupPayload(
  payload: unknown,
  writer: TableLiveV9WriterOwnership,
): TableLiveV9CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV9CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v9 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V9_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV9WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
