...
import { CALENDAR_LIVE_V21_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v21.js";
import { verifyCalendarLiveV21Authorization } from "./calendar-live-v21-authorization.js";
import { runCalendarLiveV21Preflight } from "./calendar-live-v21-preflight.js";
import { CalendarLiveV21Orchestrator } from "./run-calendar-live-v21.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v21 requires exact authorization artifact ${CALENDAR_LIVE_V21_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v21-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV21Authorization(root);
    runCalendarLiveV21Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV21Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV21Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
