...
import { buildCalendarLiveV12CleanupRuntime } from "./calendar-live-v12-cleanup.js";
import { type CalendarLiveV12FixedFixedPoint } from "./calendar-live-v12-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV12RestoreProgram,
  validateCalendarLiveV12RestorePayload,
  type CalendarLiveV12RestorePayload,
} from "./calendar-live-v12-restore.js";
import {
  buildCalendarLiveV12RawPropertyRuntime,
  normalizeCalendarLiveV12Scene,
  type CalendarLiveV12RawNode,
} from "./calendar-live-v12-verifier.js";
...
export const CALENDAR_LIVE_V12_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V12_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V12_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V12_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V12_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V12_SET_COUNT = 3;
export const CALENDAR_LIVE_V12_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V12_HOST_PHASES = 3;
export const CALENDAR_LIVE_V12_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V12_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V12_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V12_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV12SourceId = (typeof CALENDAR_LIVE_V12_SOURCE_IDS)[number];
export type CalendarLiveV12Kind = "calendar" | "week" | "day";

export interface CalendarLiveV12SourceIdentity {
  source: CalendarLiveV12SourceId;
...
export interface CalendarLiveV12WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV12ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV12SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV12RawNode;
    weekScene: CalendarLiveV12RawNode;
    dayScene: CalendarLiveV12RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV12RootProof {
  source: CalendarLiveV12SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV12FixedFixedPoint;
}

export interface CalendarLiveV12ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV12SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV12CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV12SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV12CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV12SourceId;
  frameWidth: number;
...
export interface CalendarLiveV12CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v12 ${label} must be an object`);
...
export function validateCalendarLiveV12WriterPayload(
  value: unknown,
): CalendarLiveV12WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v12") ||
    ...
    result.namespace !== CALENDAR_LIVE_V12_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v12 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v12 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v12 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV12SourceIdentity,
  kind: CalendarLiveV12Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV12ExtractProgram(
  writer: CalendarLiveV12WriterOwnership,
  sources: readonly CalendarLiveV12SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v12 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V12_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V12_NAMESPACE)}
${buildCalendarLiveV12RawPropertyRuntime()}
...
export function validateCalendarLiveV12ExtractPayload(
  value: unknown,
  writer: CalendarLiveV12WriterOwnership,
): CalendarLiveV12ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v12 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V12_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v12 extract schema/one-root mismatch");
  return payload as CalendarLiveV12ExtractPayload;
}

export function proveCalendarLiveV12Roots<Instance>(
  extract: CalendarLiveV12ExtractPayload,
  sources: readonly (CalendarLiveV12SourceIdentity & {
...
): CalendarLiveV12RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v12 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v12 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV12Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV12Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV12Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV12FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v12 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v12 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v12 compile lost owned sets");
...
export function assertCalendarLiveV12RootProofs(
  proofs: readonly CalendarLiveV12RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v12 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v12 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV12ProbeProgram(
  writer: CalendarLiveV12WriterOwnership,
  sources: readonly CalendarLiveV12SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V12_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV12ProbePayload(
  value: unknown,
  writer: CalendarLiveV12WriterOwnership,
): CalendarLiveV12ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV12ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V12_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V12_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v12 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV12CaptureManifest(
  cells: readonly CalendarLiveV12CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V12_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V12_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V12_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V12_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v12 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV12CaptureCell[],
): string => {
  validateCalendarLiveV12CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV12CaptureProgram(
  writer: CalendarLiveV12WriterOwnership,
  cell: CalendarLiveV12CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V12_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v12 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v12 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V12_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV12CapturePayload(
  value: unknown,
  cell: CalendarLiveV12CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV12CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV12CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V12_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V12_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v12 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV12CaptureResponses(
  manifest: readonly CalendarLiveV12CaptureCell[],
  responses: readonly CalendarLiveV12CapturePayload[],
): void {
  validateCalendarLiveV12CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v12 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV12ObjectiveReport {
  artifactVersion: "calendar-live-v12-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV12Objective(
  manifest: readonly CalendarLiveV12CaptureCell[],
  responses: readonly CalendarLiveV12CapturePayload[],
): CalendarLiveV12ObjectiveReport {
  assertCalendarLiveV12CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v12-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV12CleanupProgram(
  writer: CalendarLiveV12WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV12CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V12_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v12 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV12CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV12WriterOwnership,
): CalendarLiveV12CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV12CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v12 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V12_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV12WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
