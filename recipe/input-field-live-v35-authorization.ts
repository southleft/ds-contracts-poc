...
import {
  ...
  INPUT_LIVE_V35_INDEX_PATH,
} from "./build-input-field-live-proof-v35.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV35HistoryExpectation = "pending" | "authorized";

export interface InputLiveV35AuthorizationArtifact {
  artifactVersion: "input-live-v35-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V35_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV35AntecedentIndex {
...
export interface InputLiveV35HistoryState {
  index?: InputLiveV35AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV35AuthorizationArtifact;
...
export interface InputLiveV35AuthorizationProof extends InputLiveV35TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V35_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V35_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V35_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V35_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v35-status") ||
...
export function validateInputLiveV35AntecedentIndex(
  index: InputLiveV35AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v35-antecedent-index-v1" ||
...
export function validateInputLiveV35UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV35History\(\s*["']/.test(source) ||
    /readInputLiveV35HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV35AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV35AuthorizationArtifact {
  ...
  ) as InputLiveV35AntecedentIndex;
  const indexFailures = validateInputLiveV35AntecedentIndex(index);
...
export function validateInputLiveV35AuthorizationArtifact(
  artifact: InputLiveV35AuthorizationArtifact | undefined,
  index: InputLiveV35AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v35-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v35" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V35_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V35_TARGET) ||
    !equalJson(
...
export function validateInputLiveV35History(
  state: InputLiveV35HistoryState,
  expected: InputLiveV35HistoryExpectation,
): string[] {
  const failures = validateInputLiveV35AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV35AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV35HistoryState(
  root = process.cwd(),
): InputLiveV35HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V35_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV35AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V35_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V35_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V35_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV35History(
  expected: InputLiveV35HistoryExpectation,
  root = process.cwd(),
): InputLiveV35HistoryState {
  const state = readInputLiveV35HistoryState(root);
  const failures = validateInputLiveV35History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV35Authorization(
  root = process.cwd(),
): InputLiveV35AuthorizationProof {
  const state = verifyInputLiveV35History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V35_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v35/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v35.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V35_TARGET,
    expectedDynamicTool: INPUT_LIVE_V35_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V35_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV35HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV35History(expected);
