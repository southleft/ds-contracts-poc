...
export interface ComboboxLiveV9LifecycleSimulationReport {
  artifactVersion: "combobox-live-v9-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV9AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV9Lifecycle(): Promise<ComboboxLiveV9LifecycleSimulationReport> {
  await buildComboboxLiveV9Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V9_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV9AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v9-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V9_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V9_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV9History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV9History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV9AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V9_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V9_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV9History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV9History(authorized, "pending");
    ...
    await buildComboboxLiveV9Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V9_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV9AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV9Lifecycle(), null, 2)}\n`,
  );
