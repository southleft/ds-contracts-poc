...
export type CalendarLiveV26HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV26AuthorizationArtifact {
  artifactVersion: "calendar-live-v26-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V26_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV26AntecedentIndex {
...
export interface CalendarLiveV26HistoryState {
  index?: CalendarLiveV26AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV26AuthorizationArtifact;
...
export interface CalendarLiveV26AuthorizationProof
  extends CalendarLiveV26TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V26_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V26_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V26_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V26_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v26-authorization") ||
  artifactPath.includes("calendar-live-v26-preflight") ||
  artifactPath.includes("calendar-live-v26-authorized") ||
  artifactPath.includes("create-calendar-live-v26-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v26-status") ||
...
export function validateCalendarLiveV26AntecedentIndex(
  index: CalendarLiveV26AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v26-antecedent-index-v1" ||
...
export function validateCalendarLiveV26UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV26History\(\s*["']/.test(source) ||
    /readCalendarLiveV26HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV26AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV26AuthorizationArtifact {
  ...
  ) as CalendarLiveV26AntecedentIndex;
  const indexFailures = validateCalendarLiveV26AntecedentIndex(index);
...
export function validateCalendarLiveV26AuthorizationArtifact(
  artifact: CalendarLiveV26AuthorizationArtifact | undefined,
  index: CalendarLiveV26AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v26-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v26" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V26_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V26_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V26_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V26_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V26_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V26_SOURCE_ROOTS ||
...
export function validateCalendarLiveV26History(
  state: CalendarLiveV26HistoryState,
  expected: CalendarLiveV26HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV26AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV26AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV26HistoryState(
  root = process.cwd(),
): CalendarLiveV26HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V26_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV26AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V26_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V26_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V26_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV26History(
  expected: CalendarLiveV26HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV26HistoryState {
  const state = readCalendarLiveV26HistoryState(root);
  const failures = validateCalendarLiveV26History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v26 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV26Authorization(
  root = process.cwd(),
): CalendarLiveV26AuthorizationProof {
  const state = verifyCalendarLiveV26History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V26_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v26/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v26.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V26_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V26_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V26_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV26HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV26History(expected);
  process.stdout.write(
    `Calendar live v26 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
