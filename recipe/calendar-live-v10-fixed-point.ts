/**
 * Calendar live v10 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V10_FIXED_VERIFIER_VERSION = "calendar-live-v10-fixed-point-v1";
export const CALENDAR_LIVE_V10_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV10FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV10FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV10FixedSourceProbe {
...
export interface CalendarLiveV10FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV10FixedFixedPoint;
}

export interface CalendarLiveV10FixedHardGateInput {
  thresholds: CalendarLiveV10FixedThresholds;
  sourceProbes: CalendarLiveV10FixedSourceProbe[];
  cells: CalendarLiveV10FixedCellValidation[];
  sceneProofs: CalendarLiveV10FixedSceneProof[];
...
export interface CalendarLiveV10FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V10_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV10FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV10FixedHardGates(
  input: CalendarLiveV10FixedHardGateInput,
): CalendarLiveV10FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV10FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V10_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV10FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV10FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV10FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
