...
import {
  ...
  verifyInputLiveV35Authorization,
  type InputLiveV35AntecedentIndex,
  type InputLiveV35AuthorizationProof,
} from "./input-field-live-v35-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V35_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v35/operator-security-attestation-template.json";
export const INPUT_LIVE_V35_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v35-security-attestation.json";

export interface InputLiveV35SecurityAttestation {
...
export interface InputLiveV35PreflightReport {
  artifactVersion: "input-live-v35-preflight-v1";
...
export const inputLiveV35AttestationSha256 = (
  value: Omit<InputLiveV35SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV35SecurityAttestation(
  value: unknown,
  proof: InputLiveV35AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v35-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V35_TARGET) ||
...
export function validateInputLiveV35ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV35Authorization");
  const preflight = source.indexOf("runInputLiveV35Preflight");
  const initialize = source.indexOf("InputLiveV35Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV35Preflight(
  root: string,
  proof: InputLiveV35AuthorizationProof,
  attestationPath = INPUT_LIVE_V35_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV35PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V35_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V35_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V35_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV35AntecedentIndex;
  failures.push(...validateInputLiveV35AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V35_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V35_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v35-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV35ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV35SecurityAttestation;
  return {
    artifactVersion: "input-live-v35-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V35_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV35Preflight(
    process.cwd(),
    verifyInputLiveV35Authorization(),
    attestationPath,
