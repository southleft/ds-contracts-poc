...
import {
  verifyComboboxLiveV22FixedSceneFixedPoint,
  type ComboboxLiveV22FixedFixedPoint,
} from "./combobox-live-v22-fixed-point.js";

export const COMBOBOX_LIVE_V22_VERIFIER_VERSION = "combobox-live-v22-verifier-v1";

export function buildComboboxLiveV22RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV22RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV22RawNode[];
}

export interface ComboboxLiveV22NormalizedScene {
...
export interface ComboboxLiveV22SceneProof {
...
export function normalizeComboboxLiveV22Scene(
  root: ComboboxLiveV22RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV22NormalizedScene {
  const canonicalBindings: ComboboxLiveV22NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV22RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV22Scene<Instance>(
  normalized: ComboboxLiveV22NormalizedScene,
...
): ComboboxLiveV22SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV22FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV22ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV22PreCaptureGates(payload: unknown): void {
