...
import { buildTableLiveV4CleanupRuntime } from "./table-live-v4-cleanup.js";
import { type TableLiveV4FixedFixedPoint } from "./table-live-v4-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV4RestoreProgram,
  validateTableLiveV4RestorePayload,
  type TableLiveV4RestorePayload,
} from "./table-live-v4-restore.js";
import {
  buildTableLiveV4RawPropertyRuntime,
  normalizeTableLiveV4Scene,
  type TableLiveV4RawNode,
} from "./table-live-v4-verifier.js";
...
export const TABLE_LIVE_V4_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V4_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V4_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V4_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V4_VARIANT_COUNT = 20;
export const TABLE_LIVE_V4_SET_COUNT = 6;
export const TABLE_LIVE_V4_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V4_HOST_PHASES = 3;
export const TABLE_LIVE_V4_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V4_RESTORE_COUNT = 8;
export const TABLE_LIVE_V4_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V4_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV4SourceId = (typeof TABLE_LIVE_V4_SOURCE_IDS)[number];
export type TableLiveV4Kind = "table" | "row" | "cell";

export interface TableLiveV4SourceIdentity {
  source: TableLiveV4SourceId;
...
export interface TableLiveV4WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV4ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV4SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV4RawNode;
    rowScene: TableLiveV4RawNode;
    cellScene: TableLiveV4RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV4RootProof {
  source: TableLiveV4SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV4FixedFixedPoint;
}

export interface TableLiveV4ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV4SourceId;
    adapterIdentity: string;
...
export interface TableLiveV4CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV4SourceId;
  adapterIdentity: string;
...
export interface TableLiveV4CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV4SourceId;
  frameWidth: number;
...
export interface TableLiveV4CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v4 ${label} must be an object`);
...
export function validateTableLiveV4WriterPayload(
  value: unknown,
): TableLiveV4WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V4_NAMESPACE ||
    ...
    throw new TypeError("Table live v4 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v4 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v4 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV4SourceIdentity,
  kind: TableLiveV4Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV4ExtractProgram(
  writer: TableLiveV4WriterOwnership,
  sources: readonly TableLiveV4SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v4 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V4_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V4_NAMESPACE)}
${buildTableLiveV4RawPropertyRuntime()}
...
export function validateTableLiveV4ExtractPayload(
  value: unknown,
  writer: TableLiveV4WriterOwnership,
): TableLiveV4ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v4 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V4_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v4 extract schema/two-root mismatch");
  return payload as TableLiveV4ExtractPayload;
}

export function proveTableLiveV4Roots<Instance>(
  extract: TableLiveV4ExtractPayload,
  sources: readonly (TableLiveV4SourceIdentity & {
...
): TableLiveV4RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v4 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v4 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV4Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV4Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV4Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV4FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v4 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v4 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v4 compile lost owned sets");
...
export function assertTableLiveV4RootProofs(
  proofs: readonly TableLiveV4RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v4 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v4 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV4ProbeProgram(
  writer: TableLiveV4WriterOwnership,
  sources: readonly TableLiveV4SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V4_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
for(const row of byAdapter.values()){
 ...
 const cell0=propertyKey(rowInstance,"Cell 0"),cell0Before=cell0&&rowInstance.componentProperties[cell0].value;let textPropertiesRestored=false;if(cell0){rowInstance.setProperties({[cell0]:"Table v4 deterministic probe"});const changed=nodes(rowInstance).some(node=>node.type==="TEXT"&&node.characters==="Table v4 deterministic probe");rowInstance.setProperties({[cell0]:cell0Before});textPropertiesRestored=changed&&JSON.stringify(original)===JSON.stringify(plain(instance));}
 rowInstance.remove();
...
export function validateTableLiveV4ProbePayload(
  value: unknown,
  writer: TableLiveV4WriterOwnership,
): TableLiveV4ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV4ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V4_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V4_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Table live v4 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV4CaptureManifest(
  cells: readonly TableLiveV4CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V4_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V4_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V4_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V4_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v4 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV4CaptureCell[],
): string => {
  validateTableLiveV4CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV4CaptureProgram(
  writer: TableLiveV4WriterOwnership,
  cell: TableLiveV4CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V4_CAPTURE_COUNT
  )
    throw new TypeError("Table live v4 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v4 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
const frame=figma.createFrame();frame.name="Table v4 ephemeral / "+cell.cellKey;page.appendChild(frame);
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V4_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV4CapturePayload(
  value: unknown,
  cell: TableLiveV4CaptureCell,
  rawResponseBytes: number,
): TableLiveV4CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV4CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V4_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V4_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v4 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV4CaptureResponses(
  manifest: readonly TableLiveV4CaptureCell[],
  responses: readonly TableLiveV4CapturePayload[],
): void {
  validateTableLiveV4CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v4 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV4ObjectiveReport {
  artifactVersion: "table-live-v4-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV4Objective(
  manifest: readonly TableLiveV4CaptureCell[],
  responses: readonly TableLiveV4CapturePayload[],
): TableLiveV4ObjectiveReport {
  assertTableLiveV4CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v4-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV4CleanupProgram(
  writer: TableLiveV4WriterOwnership,
): string {
  const baseProgram = buildTableLiveV4CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V4_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v4 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV4CleanupPayload(
  payload: unknown,
  writer: TableLiveV4WriterOwnership,
): TableLiveV4CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV4CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v4 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V4_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV4WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
