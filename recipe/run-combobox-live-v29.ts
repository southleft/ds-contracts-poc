...
import {
  acceptComboboxLiveV29Response,
  createComboboxLiveV29Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV29Request,
  persistComboboxLiveV29RawResponse,
  persistComboboxLiveV29TechnicalGates,
  readComboboxLiveV29AcceptedRawPayload,
  type ComboboxLiveV29Request,
  type ComboboxLiveV29TransactionAuthorization,
} from "./combobox-live-v29-broker.js";
import {
  COMBOBOX_LIVE_V29_CAPTURE_COUNT,
  assertComboboxLiveV29CaptureResponses,
  ...
  evaluateComboboxLiveV29Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV29Roots,
  validateComboboxLiveV29CaptureManifest,
  validateComboboxLiveV29CapturePayload,
  validateComboboxLiveV29CleanupPayload,
  validateComboboxLiveV29ExtractPayload,
  validateComboboxLiveV29ProbePayload,
  validateComboboxLiveV29WriterPayload,
  type ComboboxLiveV29CaptureCell,
  type ComboboxLiveV29CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V29_PLAN_PATH,
  type ComboboxLiveV29ProofPlan,
} from "./build-combobox-live-proof-v29.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v29.js";

export const COMBOBOX_LIVE_V29_RUNNER_VERSION = "combobox-live-v29-runner-v1";

interface ComboboxLiveV29JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV29OrchestratorState {
  artifactVersion: "combobox-live-v29-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV29JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV29ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV29CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV29SourceIdentity & {
...
export interface ComboboxLiveV29NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV29OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v29 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V29_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V29_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV29ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV29CaptureCell[];
  validateComboboxLiveV29CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v29 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v29 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV29WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV29RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV29Request =>
  readJson<ComboboxLiveV29Request>(file);

export class ComboboxLiveV29Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV29Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV29OrchestratorState = {
      artifactVersion: "combobox-live-v29-orchestrator-state-v1",
    ...
    return new ComboboxLiveV29Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV29OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV29Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV29NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV29NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v29 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV29NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v29 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV29RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV29Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV29CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV29AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV29WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV29Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV29ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV29Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV29Response(this.#directory, "writer", {
        validate: validateComboboxLiveV29WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV29Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV29RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV29Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV29ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV29Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV29RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v29-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV29Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV29ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV29Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV29ProbePayload(payload, ownership),
      });
      persistComboboxLiveV29TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v29 capture cell missing");
      const accepted = acceptComboboxLiveV29Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV29CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V29_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV29CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV29Objective(
    ...
    throw new Error(`Combobox live v29 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV29Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV29CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV29Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV29RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV29Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV29CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV29CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV29CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV29Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV29TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV29SimulationKey = (): KeyObject =>
