...
export interface ComboboxLiveV11LifecycleSimulationReport {
  artifactVersion: "combobox-live-v11-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV11AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV11Lifecycle(): Promise<ComboboxLiveV11LifecycleSimulationReport> {
  await buildComboboxLiveV11Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V11_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV11AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v11-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V11_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V11_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV11History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV11History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV11AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V11_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V11_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV11History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV11History(authorized, "pending");
    ...
    await buildComboboxLiveV11Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V11_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV11AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV11Lifecycle(), null, 2)}\n`,
  );
