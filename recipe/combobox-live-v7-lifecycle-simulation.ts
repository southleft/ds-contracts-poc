...
export interface ComboboxLiveV7LifecycleSimulationReport {
  artifactVersion: "combobox-live-v7-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV7AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV7Lifecycle(): Promise<ComboboxLiveV7LifecycleSimulationReport> {
  await buildComboboxLiveV7Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V7_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV7AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v7-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V7_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V7_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV7History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV7History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV7AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V7_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V7_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV7History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV7History(authorized, "pending");
    ...
    await buildComboboxLiveV7Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V7_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV7AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV7Lifecycle(), null, 2)}\n`,
  );
