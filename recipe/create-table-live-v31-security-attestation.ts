...
import { verifyTableLiveV31Authorization } from "./table-live-v31-authorization.js";
import { TABLE_LIVE_V31_TARGET } from "./table-live-v31-broker.js";
import {
  TABLE_LIVE_V31_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV31Authorization();
...
const attestation: TableLiveV31SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV31SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v31 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V31_SECURITY_ATTESTATION_DEFAULT_PATH,
);
