...
import { INPUT_LIVE_V38_INDEX_PATH } from "./build-input-field-live-proof-v38.js";
import {
  INPUT_LIVE_V38_DYNAMIC_TOOL,
  INPUT_LIVE_V38_TARGET,
} from "./input-field-live-v38-broker.js";
import {
  buildInputLiveV38AuthorizationArtifact,
  validateInputLiveV38AntecedentIndex,
  validateInputLiveV38AuthorizationArtifact,
  validateInputLiveV38History,
  validateInputLiveV38UnitTestSource,
  type InputLiveV38AntecedentIndex,
  type InputLiveV38AuthorizationArtifact,
  type InputLiveV38AuthorizationProof,
  type InputLiveV38HistoryState,
} from "./input-field-live-v38-authorization.js";
import {
  inputLiveV38AttestationSha256,
  validateInputLiveV38ControlFlowSource,
  validateInputLiveV38SecurityAttestation,
  type InputLiveV38SecurityAttestation,
} from "./input-field-live-v38-preflight.js";
import { inputLiveV38Sha256 } from "./input-field-live-v38-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V38_INDEX_PATH);
const index = (): InputLiveV38AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV38AntecedentIndex;

const artifact = (): InputLiveV38AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV38AuthorizationArtifact({
...
const pendingState = (): InputLiveV38HistoryState => ({
  index: index(),
  indexSha256: inputLiveV38Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV38HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV38AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV38Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V38_TARGET,
  expectedDynamicTool: INPUT_LIVE_V38_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v38/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV38AuthorizationArtifact,
): InputLiveV38SecurityAttestation => {
  const body: Omit<InputLiveV38SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v38-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V38_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV38AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV38History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV38History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV38History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV38History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV38History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV38UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV38UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV38AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV38Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV38AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV38AuthorizationArtifact(
        planted as InputLiveV38AuthorizationArtifact,
        index(),
        inputLiveV38Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV38SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV38AttestationSha256(
      body as Omit<InputLiveV38SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV38SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV38ControlFlowSource(source), []);
});
