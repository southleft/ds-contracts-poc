...
import { verifyCalendarLiveV49Authorization } from "./calendar-live-v49-authorization.js";
import { CALENDAR_LIVE_V49_TARGET } from "./calendar-live-v49-broker.js";
import {
  CALENDAR_LIVE_V49_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV49Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v49-session";
...
const attestation: CalendarLiveV49SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV49SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v49 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V49_SECURITY_ATTESTATION_DEFAULT_PATH,
);
