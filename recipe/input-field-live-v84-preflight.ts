...
import {
  ...
  verifyInputLiveV84Authorization,
  type InputLiveV84AntecedentIndex,
  type InputLiveV84AuthorizationProof,
} from "./input-field-live-v84-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V84_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v84/operator-security-attestation-template.json";
export const INPUT_LIVE_V84_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v84-security-attestation.json";

export interface InputLiveV84SecurityAttestation {
...
export interface InputLiveV84PreflightReport {
  artifactVersion: "input-live-v84-preflight-v1";
...
export const inputLiveV84AttestationSha256 = (
  value: Omit<InputLiveV84SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV84SecurityAttestation(
  value: unknown,
  proof: InputLiveV84AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v84-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V84_TARGET) ||
...
export function validateInputLiveV84ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV84Authorization");
  const preflight = source.indexOf("runInputLiveV84Preflight");
  const initialize = source.indexOf("InputLiveV84Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV84Preflight(
  root: string,
  proof: InputLiveV84AuthorizationProof,
  attestationPath = INPUT_LIVE_V84_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV84PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V84_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V84_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V84_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV84AntecedentIndex;
  failures.push(...validateInputLiveV84AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V84_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V84_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v84-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV84ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV84SecurityAttestation;
  return {
    artifactVersion: "input-live-v84-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V84_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV84Preflight(
    process.cwd(),
    verifyInputLiveV84Authorization(),
    attestationPath,
