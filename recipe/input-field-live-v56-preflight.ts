...
import {
  ...
  verifyInputLiveV56Authorization,
  type InputLiveV56AntecedentIndex,
  type InputLiveV56AuthorizationProof,
} from "./input-field-live-v56-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V56_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v56/operator-security-attestation-template.json";
export const INPUT_LIVE_V56_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v56-security-attestation.json";

export interface InputLiveV56SecurityAttestation {
...
export interface InputLiveV56PreflightReport {
  artifactVersion: "input-live-v56-preflight-v1";
...
export const inputLiveV56AttestationSha256 = (
  value: Omit<InputLiveV56SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV56SecurityAttestation(
  value: unknown,
  proof: InputLiveV56AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v56-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V56_TARGET) ||
...
export function validateInputLiveV56ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV56Authorization");
  const preflight = source.indexOf("runInputLiveV56Preflight");
  const initialize = source.indexOf("InputLiveV56Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV56Preflight(
  root: string,
  proof: InputLiveV56AuthorizationProof,
  attestationPath = INPUT_LIVE_V56_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV56PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V56_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V56_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V56_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV56AntecedentIndex;
  failures.push(...validateInputLiveV56AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V56_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V56_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v56-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV56ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV56SecurityAttestation;
  return {
    artifactVersion: "input-live-v56-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V56_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV56Preflight(
    process.cwd(),
    verifyInputLiveV56Authorization(),
    attestationPath,
