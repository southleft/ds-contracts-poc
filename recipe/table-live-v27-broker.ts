...
import {
  assertTableLiveV27RootProofs,
  TABLE_LIVE_V27_CAPTURE_COUNT,
  TABLE_LIVE_V27_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  TABLE_LIVE_V27_RESPONSE_CONTRACTS,
  type TableLiveV27RootProof,
} from "./table-live-v27-contract.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V27_BROKER_VERSION = "table-live-v27-broker-v1";
export const TABLE_LIVE_V27_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const TABLE_LIVE_V27_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const TABLE_LIVE_V27_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const TABLE_LIVE_V27_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const TABLE_LIVE_V27_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const TABLE_LIVE_V27_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type TableLiveV27RemotePhase =
  (typeof TABLE_LIVE_V27_REMOTE_PHASES)[number];
export type TableLiveV27AuthorizationMode = "live" | "simulated";

export interface TableLiveV27CommitPins {
...
export interface TableLiveV27TransactionAuthorization extends TableLiveV27CommitPins {
  mode: TableLiveV27AuthorizationMode;
...
export interface TableLiveV27Manifest {
  artifactVersion: "table-live-v27-broker-manifest-v2";
  brokerVersion: typeof TABLE_LIVE_V27_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof TABLE_LIVE_V27_DYNAMIC_TOOL;
  target: typeof TABLE_LIVE_V27_TARGET;
  authorization: TableLiveV27TransactionAuthorization;
...
export interface TableLiveV27ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface TableLiveV27RequestBody {
  artifactVersion: "table-live-v27-broker-request-v2";
  brokerVersion: typeof TABLE_LIVE_V27_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: TableLiveV27RemotePhase;
  sequence: number;
  ...
  commitPins: TableLiveV27CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof TABLE_LIVE_V27_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: TableLiveV27ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface TableLiveV27Request extends TableLiveV27RequestBody {
...
export interface TableLiveV27Receipt {
  artifactVersion: "table-live-v27-broker-receipt-v2";
  brokerVersion: typeof TABLE_LIVE_V27_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: TableLiveV27RemotePhase;
...
export interface TableLiveV27AcceptedResponse<T = unknown> {
  request: TableLiveV27Request;
  receipt: TableLiveV27Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface TableLiveV27IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: TableLiveV27ResponseContract;
...
const equalTarget = (value: unknown): value is typeof TABLE_LIVE_V27_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === TABLE_LIVE_V27_TARGET.fileKey &&
  (value as BrokerTarget).fileName === TABLE_LIVE_V27_TARGET.fileName &&
  (value as BrokerTarget).editorType === TABLE_LIVE_V27_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== TABLE_LIVE_V27_TARGET.fileKey ||
    context.fileName !== TABLE_LIVE_V27_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === TABLE_LIVE_V27_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Table live v27 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= TABLE_LIVE_V27_CAPTURE_COUNT
  )
    throw new TypeError("Table live v27 capture index must be 0..19");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Table live v27 authorization pins are malformed");
};

const readManifest = (directory: string): TableLiveV27Manifest => {
  const manifest = json<TableLiveV27Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Table live v27 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "table-live-v27-broker-manifest-v2" ||
    manifest.brokerVersion !== TABLE_LIVE_V27_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== TABLE_LIVE_V27_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V27_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Table live v27 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: TableLiveV27Request,
): TableLiveV27RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: TableLiveV27RemotePhase,
  captureIndex?: number,
): TableLiveV27Request | undefined => {
  const file = tableLiveV2RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV27Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: TableLiveV27RemotePhase,
  captureIndex?: number,
): TableLiveV27Receipt | undefined => {
  const file = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV27Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: TableLiveV27Request | undefined;
  receipt: TableLiveV27Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: TableLiveV27RemotePhase,
): TableLiveV27ResponseContract => ({
  ...TABLE_LIVE_V27_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: TABLE_LIVE_V27_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "table-live-v27-plugin-response-v2",
  brokerVersion: TABLE_LIVE_V27_BROKER_VERSION,
  target: TABLE_LIVE_V27_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*TABLE_LIVE_V27_PROGRAM_START*/${program}/*TABLE_LIVE_V27_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*TABLE_LIVE_V27_PROGRAM_START*/";
  const endMarker = "/*TABLE_LIVE_V27_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Table live v27 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createTableLiveV27Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Table live v27 private signing key does not match authorized public-key identity",
    );
  const manifest: TableLiveV27Manifest = {
    artifactVersion: "table-live-v27-broker-manifest-v2",
    brokerVersion: TABLE_LIVE_V27_BROKER_VERSION,
    ...
    captureRequests: TABLE_LIVE_V27_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateTableLiveV27Request(
  directory: string,
  request: TableLiveV27Request,
): void {
  ...
  if (
    request.artifactVersion !== "table-live-v27-broker-request-v2" ||
    request.brokerVersion !== TABLE_LIVE_V27_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== TABLE_LIVE_V27_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Table live v27 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueTableLiveV27Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: TableLiveV27RemotePhase,
  program: string,
  options: TableLiveV27IssueOptions = {},
): TableLiveV27Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<TableLiveV27Request>(targetPath);
    validateTableLiveV27Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Table live v27 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(tableLiveV2ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Table live v27 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Table live v27 empty code envelope refused");
  if (phase === "capture") validateTableLiveV27TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Table live v27 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: TABLE_LIVE_V27_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? TABLE_LIVE_V27_SIGNED_WRITER_TIMEOUT_MS
        : TABLE_LIVE_V27_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: TableLiveV27RequestBody = {
    artifactVersion: "table-live-v27-broker-request-v2",
    brokerVersion: TABLE_LIVE_V27_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: TableLiveV27Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateTableLiveV27Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistTableLiveV27RawResponse(
  directory: string,
  phase: TableLiveV27RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Table live v27 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<TableLiveV27Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Table live v27 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = tableLiveV2ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Table live v27 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<TableLiveV27Receipt, "receiptSha256"> = {
    artifactVersion: "table-live-v27-broker-receipt-v2",
    brokerVersion: TABLE_LIVE_V27_BROKER_VERSION,
...
export function acceptTableLiveV27Response<T = unknown>(
  directory: string,
  phase: TableLiveV27RemotePhase,
  options: {
    captureIndex?: number;
...
): TableLiveV27AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Table live v27 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<TableLiveV27Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Table live v27 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "table-live-v27-plugin-response-v2" ||
    plugin?.brokerVersion !== TABLE_LIVE_V27_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Table live v27 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistTableLiveV27TechnicalGates(
  directory: string,
  rootProofs: readonly TableLiveV27RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertTableLiveV27RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Table live v27 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Table live v27 probe payload hash malformed");
  const gates = Object.fromEntries(
    TABLE_LIVE_V27_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "table-live-v27-host-technical-gates-v2",
    brokerVersion: TABLE_LIVE_V27_BROKER_VERSION,
...
export function validateTableLiveV27TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "table-live-v27-host-technical-gates-v2" ||
    ...
    TABLE_LIVE_V27_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Table live v27 technical gates artifact");
}

export function validateTableLiveV27Receipt(
  directory: string,
  phase: TableLiveV27RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateTableLiveV27Request(directory, request);
  const receipt = json<TableLiveV27Receipt>(
  ...
  if (
    receipt.artifactVersion !== "table-live-v27-broker-receipt-v2" ||
    receipt.brokerVersion !== TABLE_LIVE_V27_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Table live v27 accepted receipt: ${request.requestId}`,
    );
}

export function readTableLiveV27AcceptedRawPayload(
  directory: string,
  phase: TableLiveV27RemotePhase,
  captureIndex?: number,
): unknown {
  validateTableLiveV27Receipt(directory, phase, captureIndex);
...
export function createTableLiveV27FakeDynamicResponse(
  request: TableLiveV27Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? TABLE_LIVE_V27_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? TABLE_LIVE_V27_TARGET,
    ...overrides.extra,
  };
}

export function listTableLiveV27PersistedRequests(
  directory: string,
): TableLiveV27Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<TableLiveV27Request>(file)] : [];
    });
}
