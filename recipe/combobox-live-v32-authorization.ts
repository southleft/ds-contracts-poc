...
export type ComboboxLiveV32HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV32AuthorizationArtifact {
  artifactVersion: "combobox-live-v32-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V32_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV32AntecedentIndex {
...
export interface ComboboxLiveV32HistoryState {
  index?: ComboboxLiveV32AntecedentIndex;
  indexSha256: string;
...
export interface ComboboxLiveV32AuthorizationProof extends ComboboxLiveV32TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V32_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V32_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V32_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V32_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v32-authorization") ||
  artifactPath.includes("combobox-live-v32-preflight") ||
  artifactPath.includes("combobox-live-v32-authorized") ||
  artifactPath.includes("create-combobox-live-v32-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v32-status") ||
...
export function validateComboboxLiveV32AntecedentIndex(
  index: ComboboxLiveV32AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v32-antecedent-index-v1" ||
...
export function validateComboboxLiveV32UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV32History\(\s*["']/.test(source) ||
    /readComboboxLiveV32HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV32AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV32AuthorizationArtifact {
  ...
  ) as ComboboxLiveV32AntecedentIndex;
  const indexFailures = validateComboboxLiveV32AntecedentIndex(index);
...
export function validateComboboxLiveV32AuthorizationArtifact(
  artifact: ComboboxLiveV32AuthorizationArtifact | undefined,
  index: ComboboxLiveV32AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v32-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v32" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V32_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V32_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V32_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V32_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V32_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V32_SOURCE_ROOTS ||
...
export function validateComboboxLiveV32History(
  state: ComboboxLiveV32HistoryState,
  expected: ComboboxLiveV32HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV32AntecedentIndex(state.index);
...
export function readComboboxLiveV32HistoryState(
  root = process.cwd(),
): ComboboxLiveV32HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V32_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV32AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V32_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V32_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V32_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV32History(
  expected: ComboboxLiveV32HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV32HistoryState {
  const state = readComboboxLiveV32HistoryState(root);
  const failures = validateComboboxLiveV32History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v32 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV32Authorization(
  root = process.cwd(),
): ComboboxLiveV32AuthorizationProof {
  const state = verifyComboboxLiveV32History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V32_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v32/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v32.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V32_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V32_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V32_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV32HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV32History(expected);
  process.stdout.write(
    `Combobox live v32 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
