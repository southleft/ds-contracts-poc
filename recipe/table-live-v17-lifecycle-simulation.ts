...
import { tableLiveV2Sha256 } from "./table-live-v17-broker.js";

export interface TableLiveV17LifecycleSimulationReport {
  artifactVersion: "table-live-v17-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV17AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV17Lifecycle(): Promise<TableLiveV17LifecycleSimulationReport> {
  await buildTableLiveV17Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V17_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV17AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v17-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V17_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V17_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV17History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV17History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV17AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V17_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V17_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV17History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV17History(authorized, "pending");
    ...
    await buildTableLiveV17Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V17_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV17AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV17Lifecycle(), null, 2)}\n`,
  );
