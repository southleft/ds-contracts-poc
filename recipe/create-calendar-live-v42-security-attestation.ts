...
import { verifyCalendarLiveV42Authorization } from "./calendar-live-v42-authorization.js";
import { CALENDAR_LIVE_V42_TARGET } from "./calendar-live-v42-broker.js";
import {
  CALENDAR_LIVE_V42_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV42Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v42-session";
...
const attestation: CalendarLiveV42SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV42SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v42 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V42_SECURITY_ATTESTATION_DEFAULT_PATH,
);
