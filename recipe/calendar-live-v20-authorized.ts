...
import { CALENDAR_LIVE_V20_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v20.js";
import { verifyCalendarLiveV20Authorization } from "./calendar-live-v20-authorization.js";
import { runCalendarLiveV20Preflight } from "./calendar-live-v20-preflight.js";
import { CalendarLiveV20Orchestrator } from "./run-calendar-live-v20.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v20 requires exact authorization artifact ${CALENDAR_LIVE_V20_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v20-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV20Authorization(root);
    runCalendarLiveV20Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV20Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV20Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
