...
import { CALENDAR_LIVE_V43_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v43.js";
import { verifyCalendarLiveV43Authorization } from "./calendar-live-v43-authorization.js";
import { runCalendarLiveV43Preflight } from "./calendar-live-v43-preflight.js";
import { CalendarLiveV43Orchestrator } from "./run-calendar-live-v43.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v43 requires exact authorization artifact ${CALENDAR_LIVE_V43_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v43-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV43Authorization(root);
    runCalendarLiveV43Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV43Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV43Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
