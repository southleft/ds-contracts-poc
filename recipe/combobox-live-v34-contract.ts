...
import { buildComboboxLiveV34CleanupRuntime } from "./combobox-live-v34-cleanup.js";
import {
  type ComboboxLiveV34FixedFixedPoint,
} from "./combobox-live-v34-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV34RestoreProgram,
  validateComboboxLiveV34RestorePayload,
  type ComboboxLiveV34RestorePayload,
} from "./combobox-live-v34-restore.js";
import {
  buildComboboxLiveV34RawPropertyRuntime,
  normalizeComboboxLiveV34Scene,
  type ComboboxLiveV34RawNode,
} from "./combobox-live-v34-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v34.js";
import {
...
export const COMBOBOX_LIVE_V34_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V34_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V34_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V34_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V34_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V34_SET_COUNT = 4;
export const COMBOBOX_LIVE_V34_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V34_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V34_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V34_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V34_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV34SourceId = (typeof COMBOBOX_LIVE_V34_SOURCE_IDS)[number];

export interface ComboboxLiveV34SourceIdentity {
  source: ComboboxLiveV34SourceId;
...
export interface ComboboxLiveV34WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV34ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV34SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV34RawNode;
    optionScene: ComboboxLiveV34RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV34RootProof {
  source: ComboboxLiveV34SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV34FixedFixedPoint;
}

export interface ComboboxLiveV34ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV34SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV34CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV34SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV34CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV34SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV34CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v34 ${label} must be an object`);
...
export function validateComboboxLiveV34WriterPayload(
  value: unknown,
): ComboboxLiveV34WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V34_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v34 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v34 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v34 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV34SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV34ExtractProgram(
  writer: ComboboxLiveV34WriterOwnership,
  sources: readonly ComboboxLiveV34SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v34 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V34_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V34_NAMESPACE)}
${buildComboboxLiveV34RawPropertyRuntime()}
...
export function validateComboboxLiveV34ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV34WriterOwnership,
): ComboboxLiveV34ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v34 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V34_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v34 extract schema/two-root mismatch");
  return payload as ComboboxLiveV34ExtractPayload;
}

export function proveComboboxLiveV34Roots<Instance>(
  extract: ComboboxLiveV34ExtractPayload,
  sources: readonly (ComboboxLiveV34SourceIdentity & {
...
): ComboboxLiveV34RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v34 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v34 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV34Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV34Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV34FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v34 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v34 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v34 compile lost owned sets");
...
export function assertComboboxLiveV34RootProofs(
  proofs: readonly ComboboxLiveV34RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v34 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v34 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV34ProbeProgram(
  writer: ComboboxLiveV34WriterOwnership,
  sources: readonly ComboboxLiveV34SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V34_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV34ProbePayload(
  value: unknown,
  writer: ComboboxLiveV34WriterOwnership,
): ComboboxLiveV34ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV34ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V34_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V34_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v34 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV34CaptureManifest(
  cells: readonly ComboboxLiveV34CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V34_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V34_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V34_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V34_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v34 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV34CaptureCell[],
): string => {
  validateComboboxLiveV34CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV34CaptureProgram(
  writer: ComboboxLiveV34WriterOwnership,
  cell: ComboboxLiveV34CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V34_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v34 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v34 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V34_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV34CapturePayload(
  value: unknown,
  cell: ComboboxLiveV34CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV34CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV34CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V34_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V34_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v34 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV34CaptureResponses(
  manifest: readonly ComboboxLiveV34CaptureCell[],
  responses: readonly ComboboxLiveV34CapturePayload[],
): void {
  validateComboboxLiveV34CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v34 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV34ObjectiveReport {
  artifactVersion: "combobox-live-v34-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV34Objective(
  manifest: readonly ComboboxLiveV34CaptureCell[],
  responses: readonly ComboboxLiveV34CapturePayload[],
): ComboboxLiveV34ObjectiveReport {
  assertComboboxLiveV34CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v34-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV34CleanupProgram(
  writer: ComboboxLiveV34WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV34CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V34_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v34 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV34CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV34WriterOwnership,
): ComboboxLiveV34CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV34CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v34 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V34_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV34WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
