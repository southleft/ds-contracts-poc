...
import {
  assertTableLiveV1RootProofs,
  TABLE_LIVE_V1_CAPTURE_COUNT,
  TABLE_LIVE_V1_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  TABLE_LIVE_V1_RESPONSE_CONTRACTS,
  type TableLiveV1RootProof,
} from "./table-live-v1-contract.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V1_BROKER_VERSION = "table-live-v1-broker-v1";
export const TABLE_LIVE_V1_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const TABLE_LIVE_V1_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const TABLE_LIVE_V1_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const TABLE_LIVE_V1_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const TABLE_LIVE_V1_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const TABLE_LIVE_V1_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type TableLiveV1RemotePhase =
  (typeof TABLE_LIVE_V1_REMOTE_PHASES)[number];
export type TableLiveV1AuthorizationMode = "live" | "simulated";

export interface TableLiveV1CommitPins {
...
export interface TableLiveV1TransactionAuthorization extends TableLiveV1CommitPins {
  mode: TableLiveV1AuthorizationMode;
...
export interface TableLiveV1Manifest {
  artifactVersion: "table-live-v1-broker-manifest-v2";
  brokerVersion: typeof TABLE_LIVE_V1_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof TABLE_LIVE_V1_DYNAMIC_TOOL;
  target: typeof TABLE_LIVE_V1_TARGET;
  authorization: TableLiveV1TransactionAuthorization;
...
export interface TableLiveV1ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface TableLiveV1RequestBody {
  artifactVersion: "table-live-v1-broker-request-v2";
  brokerVersion: typeof TABLE_LIVE_V1_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: TableLiveV1RemotePhase;
  sequence: number;
  ...
  commitPins: TableLiveV1CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof TABLE_LIVE_V1_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: TableLiveV1ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface TableLiveV1Request extends TableLiveV1RequestBody {
...
export interface TableLiveV1Receipt {
  artifactVersion: "table-live-v1-broker-receipt-v2";
  brokerVersion: typeof TABLE_LIVE_V1_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: TableLiveV1RemotePhase;
...
export interface TableLiveV1AcceptedResponse<T = unknown> {
  request: TableLiveV1Request;
  receipt: TableLiveV1Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface TableLiveV1IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: TableLiveV1ResponseContract;
...
const equalTarget = (value: unknown): value is typeof TABLE_LIVE_V1_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === TABLE_LIVE_V1_TARGET.fileKey &&
  (value as BrokerTarget).fileName === TABLE_LIVE_V1_TARGET.fileName &&
  (value as BrokerTarget).editorType === TABLE_LIVE_V1_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== TABLE_LIVE_V1_TARGET.fileKey ||
    context.fileName !== TABLE_LIVE_V1_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === TABLE_LIVE_V1_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Table live v1 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= TABLE_LIVE_V1_CAPTURE_COUNT
  )
    throw new TypeError("Table live v1 capture index must be 0..19");
  return `capture-${String(captureIndex).padStart(3, "0")}`;
};

export const tableLiveV1RequestSequence = (
  phase: TableLiveV1RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(tableLiveV1RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const tableLiveV1RequestPath = (
  directory: string,
  phase: TableLiveV1RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const tableLiveV1ResponsePath = (
  directory: string,
  phase: TableLiveV1RemotePhase,
  captureIndex?: number,
): string =>
...
export const tableLiveV1ReceiptPath = (
  directory: string,
  phase: TableLiveV1RemotePhase,
  captureIndex?: number,
): string =>
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Table live v1 authorization pins are malformed");
};

const readManifest = (directory: string): TableLiveV1Manifest => {
  const manifest = json<TableLiveV1Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Table live v1 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "table-live-v1-broker-manifest-v2" ||
    manifest.brokerVersion !== TABLE_LIVE_V1_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== TABLE_LIVE_V1_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V1_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Table live v1 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: TableLiveV1Request,
): TableLiveV1RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: TableLiveV1RemotePhase,
  captureIndex?: number,
): TableLiveV1Request | undefined => {
  const file = tableLiveV1RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV1Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: TableLiveV1RemotePhase,
  captureIndex?: number,
): TableLiveV1Receipt | undefined => {
  const file = tableLiveV1ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV1Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: TableLiveV1Request | undefined;
  receipt: TableLiveV1Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: TableLiveV1RemotePhase,
): TableLiveV1ResponseContract => ({
  ...TABLE_LIVE_V1_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: TABLE_LIVE_V1_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "table-live-v1-plugin-response-v2",
  brokerVersion: TABLE_LIVE_V1_BROKER_VERSION,
  target: TABLE_LIVE_V1_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*TABLE_LIVE_V1_PROGRAM_START*/${program}/*TABLE_LIVE_V1_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*TABLE_LIVE_V1_PROGRAM_START*/";
  const endMarker = "/*TABLE_LIVE_V1_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Table live v1 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createTableLiveV1Transaction(
  directory: string,
  ...
  if (existsSync(path.join(directory, MANIFEST_FILE)))
    throw new Error("Table live v1 transaction already exists");
  validateAuthorization(options.authorization);
  if (
    !SHA256.test(options.proofPlanSha256) ||
    !SHA256.test(options.captureManifestSha256)
  )
    throw new TypeError("Table live v1 transaction plan hashes malformed");
  const attempt = options.attempt ?? 1;
  if (!Number.isInteger(attempt) || attempt < 1 || attempt > 3)
    throw new TypeError("Table live v1 attempt must be 1..3");
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Table live v1 private signing key does not match authorized public-key identity",
    );
  const manifest: TableLiveV1Manifest = {
    artifactVersion: "table-live-v1-broker-manifest-v2",
    brokerVersion: TABLE_LIVE_V1_BROKER_VERSION,
    ...
    captureRequests: TABLE_LIVE_V1_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateTableLiveV1Request(
  directory: string,
  request: TableLiveV1Request,
): void {
  ...
  const expectedSequence = tableLiveV1RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "table-live-v1-broker-request-v2" ||
    request.brokerVersion !== TABLE_LIVE_V1_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== TABLE_LIVE_V1_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Table live v1 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueTableLiveV1Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: TableLiveV1RemotePhase,
  program: string,
  options: TableLiveV1IssueOptions = {},
): TableLiveV1Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = tableLiveV1RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<TableLiveV1Request>(targetPath);
    validateTableLiveV1Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Table live v1 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(tableLiveV1ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Table live v1 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Table live v1 empty code envelope refused");
  if (phase === "capture") validateTableLiveV1TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Table live v1 out-of-order phase: ${id}`);
  const expectedResponse =
    options.expectedResponse ?? responseContractFor(phase);
  const programSha256 = sha256(program);
  const sequence = tableLiveV1RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: TABLE_LIVE_V1_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? TABLE_LIVE_V1_SIGNED_WRITER_TIMEOUT_MS
        : TABLE_LIVE_V1_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: TableLiveV1RequestBody = {
    artifactVersion: "table-live-v1-broker-request-v2",
    brokerVersion: TABLE_LIVE_V1_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: TableLiveV1Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateTableLiveV1Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistTableLiveV1RawResponse(
  directory: string,
  phase: TableLiveV1RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = tableLiveV1RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Table live v1 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<TableLiveV1Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Table live v1 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = tableLiveV1ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Table live v1 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<TableLiveV1Receipt, "receiptSha256"> = {
    artifactVersion: "table-live-v1-broker-receipt-v2",
    brokerVersion: TABLE_LIVE_V1_BROKER_VERSION,
...
export function acceptTableLiveV1Response<T = unknown>(
  directory: string,
  phase: TableLiveV1RemotePhase,
  options: {
    captureIndex?: number;
...
): TableLiveV1AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = tableLiveV1ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Table live v1 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<TableLiveV1Request>(
    tableLiveV1RequestPath(directory, phase, captureIndex),
  );
  validateTableLiveV1Request(directory, request);
  if (phase === "capture") validateTableLiveV1TechnicalGates(directory);
  const rawFile = tableLiveV1ResponsePath(directory, phase, captureIndex);
  if (!existsSync(rawFile))
    throw new Error(`Table live v1 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "table-live-v1-plugin-response-v2" ||
    plugin?.brokerVersion !== TABLE_LIVE_V1_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Table live v1 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistTableLiveV1TechnicalGates(
  directory: string,
  rootProofs: readonly TableLiveV1RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertTableLiveV1RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Table live v1 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Table live v1 probe payload hash malformed");
  const gates = Object.fromEntries(
    TABLE_LIVE_V1_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "table-live-v1-host-technical-gates-v2",
    brokerVersion: TABLE_LIVE_V1_BROKER_VERSION,
...
export function validateTableLiveV1TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "table-live-v1-host-technical-gates-v2" ||
    ...
    TABLE_LIVE_V1_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Table live v1 technical gates artifact");
}

export function validateTableLiveV1Receipt(
  directory: string,
  phase: TableLiveV1RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateTableLiveV1Request(directory, request);
  const receipt = json<TableLiveV1Receipt>(
    tableLiveV1ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    tableLiveV1ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "table-live-v1-broker-receipt-v2" ||
    receipt.brokerVersion !== TABLE_LIVE_V1_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Table live v1 accepted receipt: ${request.requestId}`,
    );
}

export function readTableLiveV1AcceptedRawPayload(
  directory: string,
  phase: TableLiveV1RemotePhase,
  captureIndex?: number,
): unknown {
  validateTableLiveV1Receipt(directory, phase, captureIndex);
...
export function createTableLiveV1FakeDynamicResponse(
  request: TableLiveV1Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? TABLE_LIVE_V1_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? TABLE_LIVE_V1_TARGET,
    ...overrides.extra,
  };
}

export function listTableLiveV1PersistedRequests(
  directory: string,
): TableLiveV1Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<TableLiveV1Request>(file)] : [];
    });
}

export const tableLiveV1Sha256 = sha256;
