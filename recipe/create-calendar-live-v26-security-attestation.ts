...
import { verifyCalendarLiveV26Authorization } from "./calendar-live-v26-authorization.js";
import { CALENDAR_LIVE_V26_TARGET } from "./calendar-live-v26-broker.js";
import {
  CALENDAR_LIVE_V26_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV26Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v26-session";
...
const attestation: CalendarLiveV26SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV26SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v26 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V26_SECURITY_ATTESTATION_DEFAULT_PATH,
);
