...
import { COMBOBOX_LIVE_V13_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v13.js";
import { verifyComboboxLiveV13Authorization } from "./combobox-live-v13-authorization.js";
import { runComboboxLiveV13Preflight } from "./combobox-live-v13-preflight.js";
import { ComboboxLiveV13Orchestrator } from "./run-combobox-live-v13.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v13 requires exact authorization artifact ${COMBOBOX_LIVE_V13_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v13-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV13Authorization(root);
    runComboboxLiveV13Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV13Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV13Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
