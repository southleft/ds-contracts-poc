...
import { verifyComboboxLiveV24Authorization } from "./combobox-live-v24-authorization.js";
import { COMBOBOX_LIVE_V24_TARGET } from "./combobox-live-v24-broker.js";
import {
  COMBOBOX_LIVE_V24_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV24Authorization();
...
const attestation: ComboboxLiveV24SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV24SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v24 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V24_SECURITY_ATTESTATION_DEFAULT_PATH,
);
