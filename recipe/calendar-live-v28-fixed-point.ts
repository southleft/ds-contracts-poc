/**
 * Calendar live v28 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V28_FIXED_VERIFIER_VERSION = "calendar-live-v28-fixed-point-v1";
export const CALENDAR_LIVE_V28_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV28FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV28FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV28FixedSourceProbe {
...
export interface CalendarLiveV28FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV28FixedFixedPoint;
}

export interface CalendarLiveV28FixedHardGateInput {
  thresholds: CalendarLiveV28FixedThresholds;
  sourceProbes: CalendarLiveV28FixedSourceProbe[];
  cells: CalendarLiveV28FixedCellValidation[];
  sceneProofs: CalendarLiveV28FixedSceneProof[];
...
export interface CalendarLiveV28FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V28_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV28FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV28FixedHardGates(
  input: CalendarLiveV28FixedHardGateInput,
): CalendarLiveV28FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV28FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V28_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV28FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV28FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV28FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
