...
import {
  assertInputLiveV41RootProofs,
  INPUT_LIVE_V41_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V41_RESPONSE_CONTRACTS,
  type InputLiveV41RootProof,
} from "./input-field-live-v41-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V41_BROKER_VERSION = "input-live-v41-broker-v2";
export const INPUT_LIVE_V41_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V41_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V41_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V41_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V41_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V41_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV41RemotePhase =
  (typeof INPUT_LIVE_V41_REMOTE_PHASES)[number];
export type InputLiveV41AuthorizationMode = "live" | "simulated";

export interface InputLiveV41CommitPins {
...
export interface InputLiveV41TransactionAuthorization extends InputLiveV41CommitPins {
  mode: InputLiveV41AuthorizationMode;
...
export interface InputLiveV41Manifest {
  artifactVersion: "input-live-v41-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V41_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V41_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V41_TARGET;
  authorization: InputLiveV41TransactionAuthorization;
...
export interface InputLiveV41ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV41RequestBody {
  artifactVersion: "input-live-v41-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V41_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV41RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV41CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V41_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV41ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV41Request extends InputLiveV41RequestBody {
...
export interface InputLiveV41Receipt {
  artifactVersion: "input-live-v41-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V41_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV41RemotePhase;
...
export interface InputLiveV41AcceptedResponse<T = unknown> {
  request: InputLiveV41Request;
  receipt: InputLiveV41Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV41IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV41ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V41_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V41_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V41_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V41_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V41_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V41_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V41_TARGET.editorType;
};
...
export const inputLiveV41RequestSequence = (
  phase: InputLiveV41RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV41RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV41RequestPath = (
  directory: string,
  phase: InputLiveV41RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV41ResponsePath = (
  directory: string,
  phase: InputLiveV41RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV41ReceiptPath = (
  directory: string,
  phase: InputLiveV41RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV41Manifest => {
  const manifest = json<InputLiveV41Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v41-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V41_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V41_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV41RemotePhase,
  captureIndex?: number,
): InputLiveV41Request | undefined => {
  const file = inputLiveV41RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV41Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV41RemotePhase,
  captureIndex?: number,
): InputLiveV41Receipt | undefined => {
  const file = inputLiveV41ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV41Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV41Request | undefined;
  receipt: InputLiveV41Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV41RemotePhase,
): InputLiveV41ResponseContract => ({
  ...INPUT_LIVE_V41_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V41_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V41_DYNAMIC_TOOL,
      target: INPUT_LIVE_V41_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v41-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V41_BROKER_VERSION,
  target: INPUT_LIVE_V41_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V41_PROGRAM_START*/${program}/*INPUT_LIVE_V41_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V41_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V41_PROGRAM_END*/";
...
export function createInputLiveV41Transaction(
  directory: string,
  ...
  const manifest: InputLiveV41Manifest = {
    artifactVersion: "input-live-v41-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V41_BROKER_VERSION,
...
export function validateInputLiveV41Request(
  directory: string,
  request: InputLiveV41Request,
): void {
  ...
  const expectedSequence = inputLiveV41RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v41-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V41_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V41_TARGET.fileKey ||
...
export function issueInputLiveV41Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV41RemotePhase,
  program: string,
  options: InputLiveV41IssueOptions = {},
): InputLiveV41Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV41RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV41Request>(targetPath);
    validateInputLiveV41Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV41ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV41TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV41RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V41_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V41_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V41_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV41RequestBody = {
    artifactVersion: "input-live-v41-broker-request-v2",
    brokerVersion: INPUT_LIVE_V41_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV41Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV41Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV41RawResponse(
  directory: string,
  phase: InputLiveV41RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV41RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV41Request>(requestFile);
  if (
  ...
  const target = inputLiveV41ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV41Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v41-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V41_BROKER_VERSION,
...
export function acceptInputLiveV41Response<T = unknown>(
  directory: string,
  phase: InputLiveV41RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV41AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV41ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV41Request>(
    inputLiveV41RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV41Request(directory, request);
  if (phase === "capture") validateInputLiveV41TechnicalGates(directory);
  const rawFile = inputLiveV41ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v41-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V41_BROKER_VERSION ||
...
export function persistInputLiveV41TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV41RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV41RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V41_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v41-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V41_BROKER_VERSION,
...
export function validateInputLiveV41TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v41-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V41_TECHNICAL_GATES.some(
...
export function validateInputLiveV41Receipt(
  directory: string,
  phase: InputLiveV41RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV41Request(directory, request);
  const receipt = json<InputLiveV41Receipt>(
    inputLiveV41ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV41ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v41-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V41_BROKER_VERSION ||
...
export function readInputLiveV41AcceptedRawPayload(
  directory: string,
  phase: InputLiveV41RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV41Receipt(directory, phase, captureIndex);
...
export function createInputLiveV41FakeDynamicResponse(
  request: InputLiveV41Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V41_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V41_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV41PersistedRequests(
  directory: string,
): InputLiveV41Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV41Request>(file)] : [];
    });
}

export const inputLiveV41Sha256 = sha256;
