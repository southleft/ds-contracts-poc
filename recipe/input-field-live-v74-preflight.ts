...
import {
  ...
  verifyInputLiveV74Authorization,
  type InputLiveV74AntecedentIndex,
  type InputLiveV74AuthorizationProof,
} from "./input-field-live-v74-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V74_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v74/operator-security-attestation-template.json";
export const INPUT_LIVE_V74_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v74-security-attestation.json";

export interface InputLiveV74SecurityAttestation {
...
export interface InputLiveV74PreflightReport {
  artifactVersion: "input-live-v74-preflight-v1";
...
export const inputLiveV74AttestationSha256 = (
  value: Omit<InputLiveV74SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV74SecurityAttestation(
  value: unknown,
  proof: InputLiveV74AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v74-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V74_TARGET) ||
...
export function validateInputLiveV74ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV74Authorization");
  const preflight = source.indexOf("runInputLiveV74Preflight");
  const initialize = source.indexOf("InputLiveV74Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV74Preflight(
  root: string,
  proof: InputLiveV74AuthorizationProof,
  attestationPath = INPUT_LIVE_V74_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV74PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V74_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V74_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V74_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV74AntecedentIndex;
  failures.push(...validateInputLiveV74AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V74_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V74_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v74-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV74ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV74SecurityAttestation;
  return {
    artifactVersion: "input-live-v74-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V74_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV74Preflight(
    process.cwd(),
    verifyInputLiveV74Authorization(),
    attestationPath,
