...
import {
  ...
  verifyInputLiveV24Authorization,
  type InputLiveV24AntecedentIndex,
  type InputLiveV24AuthorizationProof,
} from "./input-field-live-v24-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V24_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v24/operator-security-attestation-template.json";
export const INPUT_LIVE_V24_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v24-security-attestation.json";

export interface InputLiveV24SecurityAttestation {
...
export interface InputLiveV24PreflightReport {
  artifactVersion: "input-live-v24-preflight-v1";
...
export const inputLiveV24AttestationSha256 = (
  value: Omit<InputLiveV24SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV24SecurityAttestation(
  value: unknown,
  proof: InputLiveV24AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v24-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V24_TARGET) ||
...
export function validateInputLiveV24ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV24Authorization");
  const preflight = source.indexOf("runInputLiveV24Preflight");
  const initialize = source.indexOf("InputLiveV24Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV24Preflight(
  root: string,
  proof: InputLiveV24AuthorizationProof,
  attestationPath = INPUT_LIVE_V24_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV24PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V24_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V24_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V24_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV24AntecedentIndex;
  failures.push(...validateInputLiveV24AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V24_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V24_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v24-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV24ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV24SecurityAttestation;
  return {
    artifactVersion: "input-live-v24-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V24_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV24Preflight(
    process.cwd(),
    verifyInputLiveV24Authorization(),
    attestationPath,
