...
import { tableLiveV2Sha256 } from "./table-live-v22-broker.js";

export interface TableLiveV22LifecycleSimulationReport {
  artifactVersion: "table-live-v22-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV22AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV22Lifecycle(): Promise<TableLiveV22LifecycleSimulationReport> {
  await buildTableLiveV22Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V22_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV22AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v22-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V22_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V22_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV22History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV22History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV22AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V22_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V22_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV22History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV22History(authorized, "pending");
    ...
    await buildTableLiveV22Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V22_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV22AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV22Lifecycle(), null, 2)}\n`,
  );
