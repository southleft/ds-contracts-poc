...
import {
  validateComboboxLiveV18AntecedentIndex,
  verifyComboboxLiveV18Authorization,
  type ComboboxLiveV18AntecedentIndex,
  type ComboboxLiveV18AuthorizationProof,
} from "./combobox-live-v18-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V18_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v18/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V18_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v18-security-attestation.json";

export interface ComboboxLiveV18SecurityAttestation {
...
export interface ComboboxLiveV18PreflightReport {
  artifactVersion: "combobox-live-v18-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV18SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV18SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV18AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v18-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V18_TARGET) ||
...
export function validateComboboxLiveV18ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV18Authorization");
  const preflight = source.indexOf("runComboboxLiveV18Preflight");
  const initialize = source.indexOf("ComboboxLiveV18Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV18Preflight(
  root: string,
  proof: ComboboxLiveV18AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V18_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV18PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V18_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V18_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V18_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV18AntecedentIndex;
  failures.push(...validateComboboxLiveV18AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V18_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V18_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v18-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV18ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v18 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV18SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v18-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V18_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV18Preflight(
    process.cwd(),
    verifyComboboxLiveV18Authorization(),
    attestationPath,
