...
} from "./input-field-live-v3-verifier-v23.js";
import {
  buildInputLiveV23RawPropertyRuntime,
  normalizeInputLiveV23Scene,
  type InputLiveV23RawNode,
} from "./input-field-live-v23-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v23.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v23.js";
...
export const INPUT_LIVE_V23_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V23_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V23_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V23_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V23_VARIANT_COUNT = 256;
export const INPUT_LIVE_V23_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V23_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV23SourceId = (typeof INPUT_LIVE_V23_SOURCE_IDS)[number];

export interface InputLiveV23SourceIdentity {
  source: InputLiveV23SourceId;
...
export interface InputLiveV23WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV23ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV23SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV23RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV23RootProof {
  source: InputLiveV23SourceId;
  adapterIdentity: string;
...
export interface InputLiveV23ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV23SourceId;
    adapterIdentity: string;
...
export interface InputLiveV23CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV23SourceId;
  adapterIdentity: string;
...
export interface InputLiveV23CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV23SourceId;
  strata: InputLiveV23CaptureCell["strata"];
...
export interface InputLiveV23CleanupPayload {
...
export function validateInputLiveV23WriterPayload(
  value: unknown,
): InputLiveV23WriterOwnership {
...
const identityPlan = (source: InputLiveV23SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV23ExtractProgram(
  writer: InputLiveV23WriterOwnership,
  sources: readonly InputLiveV23SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V23_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V23_NAMESPACE)}
${buildInputLiveV23RawPropertyRuntime()}
...
export function validateInputLiveV23ExtractPayload(
  value: unknown,
  writer: InputLiveV23WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V23_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV23ExtractPayload;
}

export function proveInputLiveV23Roots<Instance>(
  extract: InputLiveV23ExtractPayload,
  sources: readonly (InputLiveV23SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV23Scene(
      root.scene,
...
export function assertInputLiveV23RootProofs(
  proofs: readonly InputLiveV23RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV23ProbeProgram(
  writer: InputLiveV23WriterOwnership,
  sources: readonly InputLiveV23SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V23_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV23ProbePayload(
  value: unknown,
  writer: InputLiveV23WriterOwnership,
): InputLiveV23ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV23ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V23_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V23_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV23CaptureManifest(
  cells: readonly InputLiveV23CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V23_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V23_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V23_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V23_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V23_SOURCE_IDS.some(
...
export const inputLiveV23CaptureManifestSha256 = (
  cells: readonly InputLiveV23CaptureCell[],
): string => {
  validateInputLiveV23CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV23CaptureProgram(
  writer: InputLiveV23WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V23_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V23_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V23_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV23CapturePayload(
  value: unknown,
  cell: InputLiveV23CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV23CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V23_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V23_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV23CaptureResponses(
  manifest: readonly InputLiveV23CaptureCell[],
  responses: readonly InputLiveV23CapturePayload[],
): void {
  validateInputLiveV23CaptureManifest(manifest);
  if (
...
export interface InputLiveV23ObjectiveReport {
  artifactVersion: "input-live-v23-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV23Objective(
  manifest: readonly InputLiveV23CaptureCell[],
  responses: readonly InputLiveV23CapturePayload[],
  readReference: (cell: InputLiveV23CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV23CaptureCell) => Buffer,
): InputLiveV23ObjectiveReport {
  assertInputLiveV23CaptureResponses(manifest, responses);
...
export function buildInputLiveV23CleanupProgram(
  writer: InputLiveV23WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V23_NAMESPACE,
...
export function validateInputLiveV23CleanupPayload(
  value: unknown,
  writer: InputLiveV23WriterOwnership,
): InputLiveV23CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV23CleanupPayload;
...
export const INPUT_LIVE_V23_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV23WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
