...
import { INPUT_LIVE_V10_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v10.js";
import { verifyInputLiveV10Authorization } from "./input-field-live-v10-authorization.js";
import { runInputLiveV10Preflight } from "./input-field-live-v10-preflight.js";
import { InputLiveV10Orchestrator } from "./run-input-field-live-v10.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v10 requires exact authorization artifact ${INPUT_LIVE_V10_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v10-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV10Authorization(root);
    runInputLiveV10Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV10Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
