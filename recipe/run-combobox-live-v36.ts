...
import {
  acceptComboboxLiveV36Response,
  createComboboxLiveV36Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV36Request,
  persistComboboxLiveV36RawResponse,
  persistComboboxLiveV36TechnicalGates,
  readComboboxLiveV36AcceptedRawPayload,
  type ComboboxLiveV36Request,
  type ComboboxLiveV36TransactionAuthorization,
} from "./combobox-live-v36-broker.js";
import {
  COMBOBOX_LIVE_V36_CAPTURE_COUNT,
  assertComboboxLiveV36CaptureResponses,
  ...
  evaluateComboboxLiveV36Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV36Roots,
  validateComboboxLiveV36CaptureManifest,
  validateComboboxLiveV36CapturePayload,
  validateComboboxLiveV36CleanupPayload,
  validateComboboxLiveV36ExtractPayload,
  validateComboboxLiveV36ProbePayload,
  validateComboboxLiveV36WriterPayload,
  type ComboboxLiveV36CaptureCell,
  type ComboboxLiveV36CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V36_PLAN_PATH,
  type ComboboxLiveV36ProofPlan,
} from "./build-combobox-live-proof-v36.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v36.js";

export const COMBOBOX_LIVE_V36_RUNNER_VERSION = "combobox-live-v36-runner-v1";

interface ComboboxLiveV36JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV36OrchestratorState {
  artifactVersion: "combobox-live-v36-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV36JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV36ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV36CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV36SourceIdentity & {
...
export interface ComboboxLiveV36NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV36OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v36 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V36_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V36_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV36ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV36CaptureCell[];
  validateComboboxLiveV36CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v36 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v36 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV36WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV36RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV36Request =>
  readJson<ComboboxLiveV36Request>(file);

export class ComboboxLiveV36Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV36Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV36OrchestratorState = {
      artifactVersion: "combobox-live-v36-orchestrator-state-v1",
    ...
    return new ComboboxLiveV36Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV36OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV36Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV36NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV36NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v36 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV36NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v36 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV36RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV36Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV36CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV36AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV36WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV36Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV36ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV36Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV36Response(this.#directory, "writer", {
        validate: validateComboboxLiveV36WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV36Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV36RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV36Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV36ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV36Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV36RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v36-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV36Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV36ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV36Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV36ProbePayload(payload, ownership),
      });
      persistComboboxLiveV36TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v36 capture cell missing");
      const accepted = acceptComboboxLiveV36Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV36CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V36_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV36CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV36Objective(
    ...
    throw new Error(`Combobox live v36 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV36Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV36CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV36Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV36RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV36Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV36CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV36CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV36CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV36Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV36TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV36SimulationKey = (): KeyObject =>
