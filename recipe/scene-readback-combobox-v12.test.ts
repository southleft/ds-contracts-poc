...
import {
  ...
  COMBOBOX_LIVE_V12_LEADING_SLOT_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V12_LEADING_SLOT_COMPILE_CARRY_VISIBLE_MARKER,
  COMBOBOX_LIVE_V12_OMIT_EMPTY_INSTANCE_PAYLOAD_MARKER,
  COMBOBOX_LIVE_V12_PROJECT_LIVE_ROOT_OWNERSHIP_KEY_MARKER,
  COMBOBOX_LIVE_V12_RECOVER_COMPONENT_PROPERTY_NAME_MARKER,
  COMBOBOX_LIVE_V12_RECOVER_RECIPE_COMPONENT_REF_MARKER,
  COMBOBOX_LIVE_V12_TRIGGER_BINDING_COMPILE_ORDER_MARKER,
...
test("host orders combobox/control/leading bindings to compile field order, not live or inherited fills-first slot list", () => {
  ...
  assert.deepEqual(
    ...
    ["width.value", "height.value", "fills.0.color"],
    "v12 host must keep compile width then height then fill, not live extract order",
  );
});
...
test("host emits compile-carried visible: true on leading-slot instances instead of omitting default-true", () => {
  ...
  assert.equal(
    observed.visible,
    true,
    "v12 host must emit compile-carried visible: true on leading-slot instances",
  );
  assert.equal(
    observed.opacity,
    undefined,
    "v12 must not change occupancy opacity omit; default-1 opacity stays omitted",
  );
});
