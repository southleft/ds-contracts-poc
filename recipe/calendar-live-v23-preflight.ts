...
import {
  validateCalendarLiveV23AntecedentIndex,
  verifyCalendarLiveV23Authorization,
  type CalendarLiveV23AntecedentIndex,
  type CalendarLiveV23AuthorizationProof,
} from "./calendar-live-v23-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V23_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v23/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V23_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v23-security-attestation.json";

export interface CalendarLiveV23SecurityAttestation {
  artifactVersion: "calendar-live-v23-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV23PreflightReport {
  artifactVersion: "calendar-live-v23-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV23SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV23SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV23AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v23-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V23_TARGET) ||
...
export function validateCalendarLiveV23ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV23Authorization");
  const preflight = source.indexOf("runCalendarLiveV23Preflight");
  const initialize = source.indexOf("CalendarLiveV23Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV23Preflight(
  root: string,
  proof: CalendarLiveV23AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V23_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV23PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V23_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V23_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV23AntecedentIndex;
  failures.push(...validateCalendarLiveV23AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V23_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V23_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV23SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v23-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV23ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v23 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV23SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v23-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V23_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV23Preflight(
    process.cwd(),
    verifyCalendarLiveV23Authorization(),
    attestationPath,
