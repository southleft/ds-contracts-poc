...
import { COMBOBOX_LIVE_V4_INDEX_PATH } from "./build-combobox-live-proof-v4.js";
import {
  COMBOBOX_LIVE_V4_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V4_TARGET,
} from "./combobox-live-v4-broker.js";
import {
  buildComboboxLiveV4AuthorizationArtifact,
  validateComboboxLiveV4AntecedentIndex,
  validateComboboxLiveV4AuthorizationArtifact,
  validateComboboxLiveV4History,
  validateComboboxLiveV4UnitTestSource,
  type ComboboxLiveV4AntecedentIndex,
  type ComboboxLiveV4AuthorizationArtifact,
  type ComboboxLiveV4AuthorizationProof,
  type ComboboxLiveV4HistoryState,
} from "./combobox-live-v4-authorization.js";
import {
  comboboxLiveV4AttestationSha256,
  validateComboboxLiveV4ControlFlowSource,
  validateComboboxLiveV4SecurityAttestation,
  type ComboboxLiveV4SecurityAttestation,
} from "./combobox-live-v4-preflight.js";
import { comboboxLiveV4Sha256 } from "./combobox-live-v4-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V4_INDEX_PATH);
const index = (): ComboboxLiveV4AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV4AntecedentIndex;

const artifact = (): ComboboxLiveV4AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV4AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV4HistoryState => ({
  index: index(),
  indexSha256: comboboxLiveV4Sha256(indexBytes()),
...
const authorizedState = (): ComboboxLiveV4HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV4AuthorizationArtifact,
  ...
  antecedentIndexSha256: comboboxLiveV4Sha256(indexBytes()),
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v4/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV4AuthorizationArtifact,
): ComboboxLiveV4SecurityAttestation => {
  const body: Omit<ComboboxLiveV4SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "combobox-live-v4-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V4_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: comboboxLiveV4AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV4History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV4History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV4History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV4History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV4History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV4UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV4UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV4AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = comboboxLiveV4Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateComboboxLiveV4AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV4AuthorizationArtifact(
        planted as ComboboxLiveV4AuthorizationArtifact,
        index(),
        comboboxLiveV4Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV4SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV4AttestationSha256(
      body as Omit<ComboboxLiveV4SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV4SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV4ControlFlowSource(source), []);
});
