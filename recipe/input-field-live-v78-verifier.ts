...
export const INPUT_LIVE_V78_VERIFIER_VERSION = "input-live-v78-verifier-v1";

export function buildInputLiveV78RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV78RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV78RawNode[];
}

export interface InputLiveV78NormalizedScene {
...
export interface InputLiveV78SceneProof {
...
export function normalizeInputLiveV78Scene(
  root: InputLiveV78RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV78NormalizedScene {
  const canonicalBindings: InputLiveV78NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV78RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV78Scene<Instance>(
  normalized: InputLiveV78NormalizedScene,
...
export function compileInputLiveV78ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV78PreCaptureGates(payload: unknown): void {
