...
} from "./input-field-live-v3-verifier-v16.js";
import {
  buildInputLiveV16RawPropertyRuntime,
  normalizeInputLiveV16Scene,
  type InputLiveV16RawNode,
} from "./input-field-live-v16-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v16.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v16.js";

export {
  buildInputLiveV16RestoreProgram,
  validateInputLiveV16RestorePayload,
  type InputLiveV16RestorePayload,
} from "./input-field-live-v16-restore.js";

export const INPUT_LIVE_V16_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V16_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V16_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V16_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V16_VARIANT_COUNT = 256;
export const INPUT_LIVE_V16_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V16_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV16SourceId = (typeof INPUT_LIVE_V16_SOURCE_IDS)[number];

export interface InputLiveV16SourceIdentity {
  source: InputLiveV16SourceId;
...
export interface InputLiveV16WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV16ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV16SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV16RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV16RootProof {
  source: InputLiveV16SourceId;
  adapterIdentity: string;
...
export interface InputLiveV16ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV16SourceId;
    adapterIdentity: string;
...
export interface InputLiveV16CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV16SourceId;
  adapterIdentity: string;
...
export interface InputLiveV16CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV16SourceId;
  strata: InputLiveV16CaptureCell["strata"];
...
export interface InputLiveV16CleanupPayload {
...
export function validateInputLiveV16WriterPayload(
  value: unknown,
): InputLiveV16WriterOwnership {
...
const identityPlan = (source: InputLiveV16SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV16ExtractProgram(
  writer: InputLiveV16WriterOwnership,
  sources: readonly InputLiveV16SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V16_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V16_NAMESPACE)}
${buildInputLiveV16RawPropertyRuntime()}
...
export function validateInputLiveV16ExtractPayload(
  value: unknown,
  writer: InputLiveV16WriterOwnership,
): InputLiveV16ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V16_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV16ExtractPayload;
}

export function proveInputLiveV16Roots<Instance>(
  extract: InputLiveV16ExtractPayload,
  sources: readonly (InputLiveV16SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV16Scene(
      root.scene,
...
export function assertInputLiveV16RootProofs(
  proofs: readonly InputLiveV16RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV16ProbeProgram(
  writer: InputLiveV16WriterOwnership,
  sources: readonly InputLiveV16SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V16_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV16ProbePayload(
  value: unknown,
  writer: InputLiveV16WriterOwnership,
): InputLiveV16ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV16ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V16_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V16_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV16CaptureManifest(
  cells: readonly InputLiveV16CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V16_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V16_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V16_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V16_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V16_SOURCE_IDS.some(
...
export const inputLiveV16CaptureManifestSha256 = (
  cells: readonly InputLiveV16CaptureCell[],
): string => {
  validateInputLiveV16CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV16CaptureProgram(
  writer: InputLiveV16WriterOwnership,
  cell: InputLiveV16CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V16_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V16_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V16_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV16CapturePayload(
  value: unknown,
  cell: InputLiveV16CaptureCell,
  rawResponseBytes: number,
): InputLiveV16CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV16CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V16_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V16_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV16CaptureResponses(
  manifest: readonly InputLiveV16CaptureCell[],
  responses: readonly InputLiveV16CapturePayload[],
): void {
  validateInputLiveV16CaptureManifest(manifest);
  if (
...
export interface InputLiveV16ObjectiveReport {
  artifactVersion: "input-live-v16-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV16Objective(
  manifest: readonly InputLiveV16CaptureCell[],
  responses: readonly InputLiveV16CapturePayload[],
  readReference: (cell: InputLiveV16CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV16CaptureCell) => Buffer,
): InputLiveV16ObjectiveReport {
  assertInputLiveV16CaptureResponses(manifest, responses);
...
export function buildInputLiveV16CleanupProgram(
  writer: InputLiveV16WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V16_NAMESPACE,
...
export function validateInputLiveV16CleanupPayload(
  value: unknown,
  writer: InputLiveV16WriterOwnership,
): InputLiveV16CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV16CleanupPayload;
...
export const INPUT_LIVE_V16_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV16WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
