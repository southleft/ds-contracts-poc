...
import {
  acceptComboboxLiveV7Response,
  createComboboxLiveV7Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV7Request,
  persistComboboxLiveV7RawResponse,
  persistComboboxLiveV7TechnicalGates,
  readComboboxLiveV7AcceptedRawPayload,
  type ComboboxLiveV7Request,
  type ComboboxLiveV7TransactionAuthorization,
} from "./combobox-live-v7-broker.js";
import {
  COMBOBOX_LIVE_V7_CAPTURE_COUNT,
  assertComboboxLiveV7CaptureResponses,
  ...
  evaluateComboboxLiveV7Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV7Roots,
  validateComboboxLiveV7CaptureManifest,
  validateComboboxLiveV7CapturePayload,
  validateComboboxLiveV7CleanupPayload,
  validateComboboxLiveV7ExtractPayload,
  validateComboboxLiveV7ProbePayload,
  validateComboboxLiveV7WriterPayload,
  type ComboboxLiveV7CaptureCell,
  type ComboboxLiveV7CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V7_PLAN_PATH,
  type ComboboxLiveV7ProofPlan,
} from "./build-combobox-live-proof-v7.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v7.js";

export const COMBOBOX_LIVE_V7_RUNNER_VERSION = "combobox-live-v7-runner-v1";

interface ComboboxLiveV7JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV7OrchestratorState {
  artifactVersion: "combobox-live-v7-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV7JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV7ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV7CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV7SourceIdentity & {
...
export interface ComboboxLiveV7NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV7OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v7 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V7_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V7_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV7ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV7CaptureCell[];
  validateComboboxLiveV7CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v7 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v7 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV7WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV7RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV7Request =>
  readJson<ComboboxLiveV7Request>(file);

export class ComboboxLiveV7Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV7Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV7OrchestratorState = {
      artifactVersion: "combobox-live-v7-orchestrator-state-v1",
    ...
    return new ComboboxLiveV7Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV7OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV7Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV7NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV7NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v7 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV7NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v7 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV7RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV7Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV7CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV7AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV7WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV7Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV7ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV7Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV7Response(this.#directory, "writer", {
        validate: validateComboboxLiveV7WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV7Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV7RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV7Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV7ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV7Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV7RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v7-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV7Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV7ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV7Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV7ProbePayload(payload, ownership),
      });
      persistComboboxLiveV7TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v7 capture cell missing");
      const accepted = acceptComboboxLiveV7Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV7CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V7_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV7CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV7Objective(
    ...
    throw new Error(`Combobox live v7 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV7Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV7CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV7Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV7RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV7Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV7CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV7CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV7CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV7Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV7TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV7SimulationKey = (): KeyObject =>
