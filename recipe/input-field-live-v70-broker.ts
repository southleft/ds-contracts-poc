...
import {
  ...
  INPUT_LIVE_V70_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V70_RESPONSE_CONTRACTS,
  type InputLiveV70RootProof,
} from "./input-field-live-v70-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V70_BROKER_VERSION = "input-live-v70-broker-v2";
export const INPUT_LIVE_V70_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V70_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V70_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V70_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V70_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V70_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV70RemotePhase =
  (typeof INPUT_LIVE_V70_REMOTE_PHASES)[number];
export type InputLiveV70AuthorizationMode = "live" | "simulated";

export interface InputLiveV70CommitPins {
...
export interface InputLiveV70TransactionAuthorization extends InputLiveV70CommitPins {
  mode: InputLiveV70AuthorizationMode;
...
export interface InputLiveV70Manifest {
  artifactVersion: "input-live-v70-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V70_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V70_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V70_TARGET;
...
export interface InputLiveV70ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV70RequestBody {
  artifactVersion: "input-live-v70-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V70_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV70RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV70CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V70_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV70ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV70Request extends InputLiveV70RequestBody {
...
export interface InputLiveV70Receipt {
  artifactVersion: "input-live-v70-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V70_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV70RemotePhase;
...
export interface InputLiveV70AcceptedResponse<T = unknown> {
  request: InputLiveV70Request;
  receipt: InputLiveV70Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV70IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV70ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V70_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V70_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V70_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V70_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V70_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V70_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V70_TARGET.editorType;
};
...
export const inputLiveV70RequestSequence = (
  phase: InputLiveV70RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV70RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV70RequestPath = (
  directory: string,
  phase: InputLiveV70RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV70ResponsePath = (
  directory: string,
  phase: InputLiveV70RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV70ReceiptPath = (
  directory: string,
  phase: InputLiveV70RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV70Manifest => {
  const manifest = json<InputLiveV70Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v70-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V70_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V70_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV70RemotePhase,
  captureIndex?: number,
): InputLiveV70Request | undefined => {
  const file = inputLiveV70RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV70Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV70RemotePhase,
  captureIndex?: number,
): InputLiveV70Receipt | undefined => {
  const file = inputLiveV70ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV70Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV70Request | undefined;
  receipt: InputLiveV70Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV70RemotePhase,
): InputLiveV70ResponseContract => ({
  ...INPUT_LIVE_V70_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V70_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V70_DYNAMIC_TOOL,
      target: INPUT_LIVE_V70_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v70-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V70_BROKER_VERSION,
  target: INPUT_LIVE_V70_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V70_PROGRAM_START*/${program}/*INPUT_LIVE_V70_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V70_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V70_PROGRAM_END*/";
...
export function createInputLiveV70Transaction(
  directory: string,
  ...
  const manifest: InputLiveV70Manifest = {
    artifactVersion: "input-live-v70-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V70_BROKER_VERSION,
...
export function validateInputLiveV70Request(
  directory: string,
  request: InputLiveV70Request,
): void {
  ...
  const expectedSequence = inputLiveV70RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v70-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V70_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V70_TARGET.fileKey ||
...
export function issueInputLiveV70Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV70RemotePhase,
  program: string,
  options: InputLiveV70IssueOptions = {},
): InputLiveV70Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV70RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV70Request>(targetPath);
    validateInputLiveV70Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV70ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV70TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV70RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V70_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V70_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V70_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV70RequestBody = {
    artifactVersion: "input-live-v70-broker-request-v2",
    brokerVersion: INPUT_LIVE_V70_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV70Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV70Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV70RawResponse(
  directory: string,
  phase: InputLiveV70RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV70RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV70Request>(requestFile);
  if (
  ...
  const target = inputLiveV70ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV70Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v70-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V70_BROKER_VERSION,
...
export function acceptInputLiveV70Response<T = unknown>(
  directory: string,
  phase: InputLiveV70RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV70AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV70ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV70Request>(
    inputLiveV70RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV70Request(directory, request);
  if (phase === "capture") validateInputLiveV70TechnicalGates(directory);
  const rawFile = inputLiveV70ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v70-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V70_BROKER_VERSION ||
...
export function persistInputLiveV70TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV70RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV70RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V70_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v70-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V70_BROKER_VERSION,
...
export function validateInputLiveV70TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v70-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V70_TECHNICAL_GATES.some(
...
export function validateInputLiveV70Receipt(
  directory: string,
  phase: InputLiveV70RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV70Request(directory, request);
  const receipt = json<InputLiveV70Receipt>(
    inputLiveV70ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV70ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v70-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V70_BROKER_VERSION ||
...
export function readInputLiveV70AcceptedRawPayload(
  directory: string,
  phase: InputLiveV70RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV70Receipt(directory, phase, captureIndex);
...
export function createInputLiveV70FakeDynamicResponse(
  request: InputLiveV70Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V70_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V70_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV70PersistedRequests(
  directory: string,
): InputLiveV70Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV70Request>(file)] : [];
    });
}

export const inputLiveV70Sha256 = sha256;
