...
export const INPUT_LIVE_V85_VERIFIER_VERSION = "input-live-v85-verifier-v1";

export function buildInputLiveV85RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV85RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV85RawNode[];
}

export interface InputLiveV85NormalizedScene {
...
export interface InputLiveV85SceneProof {
...
export function normalizeInputLiveV85Scene(
  root: InputLiveV85RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV85NormalizedScene {
  const canonicalBindings: InputLiveV85NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV85RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV85Scene<Instance>(
  normalized: InputLiveV85NormalizedScene,
...
export function compileInputLiveV85ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV85PreCaptureGates(payload: unknown): void {
