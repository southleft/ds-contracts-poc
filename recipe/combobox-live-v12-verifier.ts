...
import {
  verifyComboboxLiveV12FixedSceneFixedPoint,
  type ComboboxLiveV12FixedFixedPoint,
} from "./combobox-live-v12-fixed-point.js";

export const COMBOBOX_LIVE_V12_VERIFIER_VERSION = "combobox-live-v12-verifier-v1";

export function buildComboboxLiveV12RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV12RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV12RawNode[];
}

export interface ComboboxLiveV12NormalizedScene {
...
export interface ComboboxLiveV12SceneProof {
...
export function normalizeComboboxLiveV12Scene(
  root: ComboboxLiveV12RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV12NormalizedScene {
  const canonicalBindings: ComboboxLiveV12NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV12RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV12Scene<Instance>(
  normalized: ComboboxLiveV12NormalizedScene,
...
): ComboboxLiveV12SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV12FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV12ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV12PreCaptureGates(payload: unknown): void {
