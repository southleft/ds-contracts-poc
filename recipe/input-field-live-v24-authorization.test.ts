...
import { INPUT_LIVE_V24_INDEX_PATH } from "./build-input-field-live-proof-v24.js";
import {
...
import {
  ...
  validateInputLiveV24AntecedentIndex,
  validateInputLiveV24AuthorizationArtifact,
  validateInputLiveV24History,
  validateInputLiveV24UnitTestSource,
  type InputLiveV24AntecedentIndex,
  type InputLiveV24AuthorizationArtifact,
  type InputLiveV24AuthorizationProof,
  type InputLiveV24HistoryState,
} from "./input-field-live-v24-authorization.js";
import {
  inputLiveV24AttestationSha256,
  validateInputLiveV24ControlFlowSource,
  validateInputLiveV24SecurityAttestation,
  type InputLiveV24SecurityAttestation,
} from "./input-field-live-v24-preflight.js";
import { inputLiveV24Sha256 } from "./input-field-live-v24-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V24_INDEX_PATH);
const index = (): InputLiveV24AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV24AntecedentIndex;

const artifact = (): InputLiveV24AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV24AuthorizationArtifact({
...
const pendingState = (): InputLiveV24HistoryState => ({
  index: index(),
  indexSha256: inputLiveV24Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV24HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV24AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV24Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V24_TARGET,
  expectedDynamicTool: INPUT_LIVE_V24_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v24/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV24AuthorizationArtifact,
): InputLiveV24SecurityAttestation => {
  const body: Omit<InputLiveV24SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v24-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V24_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV24AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV24History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV24History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV24History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV24History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV24History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV24UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV24UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV24AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV24Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV24AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV24AuthorizationArtifact(
        planted as InputLiveV24AuthorizationArtifact,
        index(),
        inputLiveV24Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV24SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV24AttestationSha256(
      body as Omit<InputLiveV24SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV24SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV24ControlFlowSource(source), []);
});
