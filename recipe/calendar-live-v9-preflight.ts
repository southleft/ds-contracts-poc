...
import {
  validateCalendarLiveV9AntecedentIndex,
  verifyCalendarLiveV9Authorization,
  type CalendarLiveV9AntecedentIndex,
  type CalendarLiveV9AuthorizationProof,
} from "./calendar-live-v9-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V9_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v9/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V9_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v9-security-attestation.json";

export interface CalendarLiveV9SecurityAttestation {
  artifactVersion: "calendar-live-v9-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV9PreflightReport {
  artifactVersion: "calendar-live-v9-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV9SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV9SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV9AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v9-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V9_TARGET) ||
...
export function validateCalendarLiveV9ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV9Authorization");
  const preflight = source.indexOf("runCalendarLiveV9Preflight");
  const initialize = source.indexOf("CalendarLiveV9Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV9Preflight(
  root: string,
  proof: CalendarLiveV9AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V9_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV9PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V9_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V9_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV9AntecedentIndex;
  failures.push(...validateCalendarLiveV9AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V9_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V9_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV9SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v9-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV9ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v9 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV9SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v9-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V9_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV9Preflight(
    process.cwd(),
    verifyCalendarLiveV9Authorization(),
    attestationPath,
