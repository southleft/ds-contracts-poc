...
} from "./input-field-live-v3-verifier-v37.js";
import {
  buildInputLiveV37RawPropertyRuntime,
  normalizeInputLiveV37Scene,
  type InputLiveV37RawNode,
} from "./input-field-live-v37-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v37.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v37.js";
...
export const INPUT_LIVE_V37_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V37_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V37_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V37_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V37_VARIANT_COUNT = 256;
export const INPUT_LIVE_V37_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V37_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV37SourceId = (typeof INPUT_LIVE_V37_SOURCE_IDS)[number];

export interface InputLiveV37SourceIdentity {
  source: InputLiveV37SourceId;
...
export interface InputLiveV37WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV37ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV37SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV37RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV37RootProof {
  source: InputLiveV37SourceId;
  adapterIdentity: string;
...
export interface InputLiveV37ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV37SourceId;
    adapterIdentity: string;
...
export interface InputLiveV37CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV37SourceId;
  adapterIdentity: string;
...
export interface InputLiveV37CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV37SourceId;
  strata: InputLiveV37CaptureCell["strata"];
...
export interface InputLiveV37CleanupPayload {
...
export function validateInputLiveV37WriterPayload(
  value: unknown,
): InputLiveV37WriterOwnership {
...
const identityPlan = (source: InputLiveV37SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV37ExtractProgram(
  writer: InputLiveV37WriterOwnership,
  sources: readonly InputLiveV37SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V37_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V37_NAMESPACE)}
${buildInputLiveV37RawPropertyRuntime()}
...
export function validateInputLiveV37ExtractPayload(
  value: unknown,
  writer: InputLiveV37WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V37_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV37ExtractPayload;
}

export function proveInputLiveV37Roots<Instance>(
  extract: InputLiveV37ExtractPayload,
  sources: readonly (InputLiveV37SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV37Scene(
      root.scene,
...
export function assertInputLiveV37RootProofs(
  proofs: readonly InputLiveV37RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV37ProbeProgram(
  writer: InputLiveV37WriterOwnership,
  sources: readonly InputLiveV37SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V37_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV37ProbePayload(
  value: unknown,
  writer: InputLiveV37WriterOwnership,
): InputLiveV37ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV37ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V37_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V37_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV37CaptureManifest(
  cells: readonly InputLiveV37CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V37_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V37_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V37_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V37_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V37_SOURCE_IDS.some(
...
export const inputLiveV37CaptureManifestSha256 = (
  cells: readonly InputLiveV37CaptureCell[],
): string => {
  validateInputLiveV37CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV37CaptureProgram(
  writer: InputLiveV37WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V37_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V37_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V37_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV37CapturePayload(
  value: unknown,
  cell: InputLiveV37CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV37CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V37_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V37_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV37CaptureResponses(
  manifest: readonly InputLiveV37CaptureCell[],
  responses: readonly InputLiveV37CapturePayload[],
): void {
  validateInputLiveV37CaptureManifest(manifest);
  if (
...
export interface InputLiveV37ObjectiveReport {
  artifactVersion: "input-live-v37-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV37Objective(
  manifest: readonly InputLiveV37CaptureCell[],
  responses: readonly InputLiveV37CapturePayload[],
  readReference: (cell: InputLiveV37CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV37CaptureCell) => Buffer,
): InputLiveV37ObjectiveReport {
  assertInputLiveV37CaptureResponses(manifest, responses);
...
export function buildInputLiveV37CleanupProgram(
  writer: InputLiveV37WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V37_NAMESPACE,
...
export function validateInputLiveV37CleanupPayload(
  value: unknown,
  writer: InputLiveV37WriterOwnership,
): InputLiveV37CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV37CleanupPayload;
...
export const INPUT_LIVE_V37_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV37WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
