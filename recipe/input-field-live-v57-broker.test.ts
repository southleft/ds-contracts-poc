...
import {
  ...
  createInputLiveV57FakeDynamicResponse,
  ...
  listInputLiveV57PersistedRequests,
  persistInputLiveV57RawResponse,
  persistInputLiveV57TechnicalGates,
  validateInputLiveV57Receipt,
  validateInputLiveV57Request,
  type InputLiveV57RemotePhase,
...
import {
  ...
  type InputLiveV57ExtractPayload,
  type InputLiveV57ProbePayload,
...
import {
  ...
  simulatedInputLiveV57Authorization,
} from "./run-input-field-live-v57.js";
...
const captures: InputLiveV57CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV57Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v57-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV57Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV57CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV57FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV57FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V57_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V57_TARGET);
  persistInputLiveV57RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV57Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV57Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV57WriterOwnership,
): InputLiveV57RestorePayload => ({
...
const rootProofs = (): InputLiveV57RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v57-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV57Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV57Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV57RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV57RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV57ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV57ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV57CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV57CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV57CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV57Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV57CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV57PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV57ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV57Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV57Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV57Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV57Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V57_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV57RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V57_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV57Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV57Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV57ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV57Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV57WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV57RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV57Request;
    validateInputLiveV57Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV57Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV57CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v57-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV57Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV57Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV57Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V57_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV57RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V57_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV57Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV57Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV57RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V57_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV57Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV57Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV57Request;
