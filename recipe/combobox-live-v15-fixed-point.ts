/**
 * Carried Combobox live v15 host verifier. Uses scene-readback-combobox-v15 so live
...
export const COMBOBOX_LIVE_V15_FIXED_VERIFIER_VERSION = "combobox-live-v15-fixed-point-v1";
export const COMBOBOX_LIVE_V15_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV15FixedThresholds {
...
export interface ComboboxLiveV15FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV15FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV15FixedSourceProbe {
...
export interface ComboboxLiveV15FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV15FixedFixedPoint;
}

export interface ComboboxLiveV15FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV15FixedHardGateInput {
  thresholds: ComboboxLiveV15FixedThresholds;
  materialRegression: ComboboxLiveV15FixedMaterialRegression;
  sourceProbes: ComboboxLiveV15FixedSourceProbe[];
  cells: ComboboxLiveV15FixedCellValidation[];
  sceneProofs: ComboboxLiveV15FixedSceneProof[];
  visualRows: ComboboxLiveV15FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV15FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V15_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV15FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV15FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV15FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV15FixedHardGates(
  input: ComboboxLiveV15FixedHardGateInput,
): ComboboxLiveV15FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV15FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV15FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V15_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV15FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV15FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV15FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
