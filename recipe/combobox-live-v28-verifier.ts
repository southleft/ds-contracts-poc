...
import {
  verifyComboboxLiveV28FixedSceneFixedPoint,
  type ComboboxLiveV28FixedFixedPoint,
} from "./combobox-live-v28-fixed-point.js";

export const COMBOBOX_LIVE_V28_VERIFIER_VERSION = "combobox-live-v28-verifier-v1";

export function buildComboboxLiveV28RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV28RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV28RawNode[];
}

export interface ComboboxLiveV28NormalizedScene {
...
export interface ComboboxLiveV28SceneProof {
...
export function normalizeComboboxLiveV28Scene(
  root: ComboboxLiveV28RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV28NormalizedScene {
  const canonicalBindings: ComboboxLiveV28NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV28RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV28Scene<Instance>(
  normalized: ComboboxLiveV28NormalizedScene,
...
): ComboboxLiveV28SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV28FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV28ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV28PreCaptureGates(payload: unknown): void {
