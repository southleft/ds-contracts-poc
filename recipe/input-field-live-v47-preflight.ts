...
import {
  ...
  verifyInputLiveV47Authorization,
  type InputLiveV47AntecedentIndex,
  type InputLiveV47AuthorizationProof,
} from "./input-field-live-v47-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V47_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v47/operator-security-attestation-template.json";
export const INPUT_LIVE_V47_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v47-security-attestation.json";

export interface InputLiveV47SecurityAttestation {
...
export interface InputLiveV47PreflightReport {
  artifactVersion: "input-live-v47-preflight-v1";
...
export const inputLiveV47AttestationSha256 = (
  value: Omit<InputLiveV47SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV47SecurityAttestation(
  value: unknown,
  proof: InputLiveV47AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v47-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V47_TARGET) ||
...
export function validateInputLiveV47ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV47Authorization");
  const preflight = source.indexOf("runInputLiveV47Preflight");
  const initialize = source.indexOf("InputLiveV47Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV47Preflight(
  root: string,
  proof: InputLiveV47AuthorizationProof,
  attestationPath = INPUT_LIVE_V47_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV47PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V47_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V47_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V47_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV47AntecedentIndex;
  failures.push(...validateInputLiveV47AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V47_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V47_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v47-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV47ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV47SecurityAttestation;
  return {
    artifactVersion: "input-live-v47-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V47_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV47Preflight(
    process.cwd(),
    verifyInputLiveV47Authorization(),
    attestationPath,
