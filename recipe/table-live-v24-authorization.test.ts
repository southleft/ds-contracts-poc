...
import { TABLE_LIVE_V24_INDEX_PATH } from "./build-table-live-proof-v24.js";
import {
...
import {
  buildTableLiveV24AuthorizationArtifact,
  validateTableLiveV24AntecedentIndex,
  validateTableLiveV24AuthorizationArtifact,
  validateTableLiveV24History,
  validateTableLiveV24UnitTestSource,
  type TableLiveV24AntecedentIndex,
  type TableLiveV24AuthorizationArtifact,
  type TableLiveV24AuthorizationProof,
  type TableLiveV24HistoryState,
} from "./table-live-v24-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV24ControlFlowSource,
  validateTableLiveV24SecurityAttestation,
  type TableLiveV24SecurityAttestation,
} from "./table-live-v24-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V24_INDEX_PATH);
const index = (): TableLiveV24AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV24AntecedentIndex;

const artifact = (): TableLiveV24AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV24AuthorizationArtifact({
...
const pendingState = (): TableLiveV24HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV24HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV24AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v24/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV24AuthorizationArtifact,
): TableLiveV24SecurityAttestation => {
  const body: Omit<TableLiveV24SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V24_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV24History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV24History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV24History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV24History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV24History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV24UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v24-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV24UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV24AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV24AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV24AuthorizationArtifact(
        planted as TableLiveV24AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV24SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV24SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV24SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v24-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV24ControlFlowSource(source), []);
});
