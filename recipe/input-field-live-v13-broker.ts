...
import {
  assertInputLiveV13RootProofs,
  INPUT_LIVE_V13_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V13_RESPONSE_CONTRACTS,
  type InputLiveV13RootProof,
} from "./input-field-live-v13-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V13_BROKER_VERSION = "input-live-v13-broker-v2";
export const INPUT_LIVE_V13_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V13_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V13_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V13_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V13_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V13_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV13RemotePhase =
  (typeof INPUT_LIVE_V13_REMOTE_PHASES)[number];
export type InputLiveV13AuthorizationMode = "live" | "simulated";

export interface InputLiveV13CommitPins {
...
export interface InputLiveV13TransactionAuthorization extends InputLiveV13CommitPins {
  mode: InputLiveV13AuthorizationMode;
...
export interface InputLiveV13Manifest {
  artifactVersion: "input-live-v13-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V13_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V13_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V13_TARGET;
  authorization: InputLiveV13TransactionAuthorization;
...
export interface InputLiveV13ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV13RequestBody {
  artifactVersion: "input-live-v13-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V13_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV13RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV13CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V13_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV13ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV13Request extends InputLiveV13RequestBody {
...
export interface InputLiveV13Receipt {
  artifactVersion: "input-live-v13-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V13_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV13RemotePhase;
...
export interface InputLiveV13AcceptedResponse<T = unknown> {
  request: InputLiveV13Request;
  receipt: InputLiveV13Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV13IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV13ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V13_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V13_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V13_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V13_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V13_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V13_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V13_TARGET.editorType;
};
...
export const inputLiveV13RequestSequence = (
  phase: InputLiveV13RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV13RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV13RequestPath = (
  directory: string,
  phase: InputLiveV13RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV13ResponsePath = (
  directory: string,
  phase: InputLiveV13RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV13ReceiptPath = (
  directory: string,
  phase: InputLiveV13RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV13Manifest => {
  const manifest = json<InputLiveV13Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v13-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V13_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V13_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV13RemotePhase,
  captureIndex?: number,
): InputLiveV13Request | undefined => {
  const file = inputLiveV13RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV13Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV13RemotePhase,
  captureIndex?: number,
): InputLiveV13Receipt | undefined => {
  const file = inputLiveV13ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV13Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV13Request | undefined;
  receipt: InputLiveV13Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV13RemotePhase,
): InputLiveV13ResponseContract => ({
  ...INPUT_LIVE_V13_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V13_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V13_DYNAMIC_TOOL,
      target: INPUT_LIVE_V13_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v13-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V13_BROKER_VERSION,
  target: INPUT_LIVE_V13_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V13_PROGRAM_START*/${program}/*INPUT_LIVE_V13_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V13_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V13_PROGRAM_END*/";
...
export function createInputLiveV13Transaction(
  directory: string,
  ...
  const manifest: InputLiveV13Manifest = {
    artifactVersion: "input-live-v13-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V13_BROKER_VERSION,
...
export function validateInputLiveV13Request(
  directory: string,
  request: InputLiveV13Request,
): void {
  ...
  const expectedSequence = inputLiveV13RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v13-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V13_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V13_TARGET.fileKey ||
...
export function issueInputLiveV13Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV13RemotePhase,
  program: string,
  options: InputLiveV13IssueOptions = {},
): InputLiveV13Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV13RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV13Request>(targetPath);
    validateInputLiveV13Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV13ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV13TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV13RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V13_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V13_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V13_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV13RequestBody = {
    artifactVersion: "input-live-v13-broker-request-v2",
    brokerVersion: INPUT_LIVE_V13_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV13Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV13Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV13RawResponse(
  directory: string,
  phase: InputLiveV13RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV13RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV13Request>(requestFile);
  if (
  ...
  const target = inputLiveV13ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV13Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v13-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V13_BROKER_VERSION,
...
export function acceptInputLiveV13Response<T = unknown>(
  directory: string,
  phase: InputLiveV13RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV13AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV13ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV13Request>(
    inputLiveV13RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV13Request(directory, request);
  if (phase === "capture") validateInputLiveV13TechnicalGates(directory);
  const rawFile = inputLiveV13ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v13-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V13_BROKER_VERSION ||
...
export function persistInputLiveV13TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV13RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV13RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V13_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v13-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V13_BROKER_VERSION,
...
export function validateInputLiveV13TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v13-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V13_TECHNICAL_GATES.some(
...
export function validateInputLiveV13Receipt(
  directory: string,
  phase: InputLiveV13RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV13Request(directory, request);
  const receipt = json<InputLiveV13Receipt>(
    inputLiveV13ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV13ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v13-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V13_BROKER_VERSION ||
...
export function readInputLiveV13AcceptedRawPayload(
  directory: string,
  phase: InputLiveV13RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV13Receipt(directory, phase, captureIndex);
...
export function createInputLiveV13FakeDynamicResponse(
  request: InputLiveV13Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V13_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V13_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV13PersistedRequests(
  directory: string,
): InputLiveV13Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV13Request>(file)] : [];
    });
}

export const inputLiveV13Sha256 = sha256;
