/**
 * Table live v16 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V16_FIXED_VERIFIER_VERSION = "table-live-v16-fixed-point-v1";
export const TABLE_LIVE_V16_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV16FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV16FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV16FixedSourceProbe {
...
export interface TableLiveV16FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV16FixedFixedPoint;
}

export interface TableLiveV16FixedHardGateInput {
  thresholds: TableLiveV16FixedThresholds;
  sourceProbes: TableLiveV16FixedSourceProbe[];
  cells: TableLiveV16FixedCellValidation[];
  sceneProofs: TableLiveV16FixedSceneProof[];
...
export interface TableLiveV16FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V16_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV16FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV16FixedHardGates(
  input: TableLiveV16FixedHardGateInput,
): TableLiveV16FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV16FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  if (input.visualRows.length !== 0)
    failures.push("table live v16 must not invent a visual comparison");
  ...
  return {
    verifierVersion: TABLE_LIVE_V16_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV16FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV16FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
