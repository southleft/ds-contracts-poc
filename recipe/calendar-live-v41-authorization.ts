...
export type CalendarLiveV41HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV41AuthorizationArtifact {
  artifactVersion: "calendar-live-v41-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V41_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV41AntecedentIndex {
...
export interface CalendarLiveV41HistoryState {
  ...
  authorization?: CalendarLiveV41AuthorizationArtifact;
...
export interface CalendarLiveV41AuthorizationProof
  extends CalendarLiveV41TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V41_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V41_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V41_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V41_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v41-authorization") ||
  artifactPath.includes("calendar-live-v41-preflight") ||
  artifactPath.includes("calendar-live-v41-authorized") ||
  artifactPath.includes("create-calendar-live-v41-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v41-status") ||
...
export function validateCalendarLiveV41AntecedentIndex(
  index: CalendarLiveV41AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v41-antecedent-index-v1" ||
...
export function validateCalendarLiveV41UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV41History\(\s*["']/.test(source) ||
    /readCalendarLiveV41HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV41AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV41AuthorizationArtifact {
  ...
  const indexFailures = validateCalendarLiveV41AntecedentIndex(index);
...
export function validateCalendarLiveV41AuthorizationArtifact(
  artifact: CalendarLiveV41AuthorizationArtifact | undefined,
  index: CalendarLiveV41AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v41-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v41" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V41_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V41_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V41_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V41_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V41_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V41_SOURCE_ROOTS ||
...
export function validateCalendarLiveV41History(
  state: CalendarLiveV41HistoryState,
  expected: CalendarLiveV41HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV41AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV41AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV41HistoryState(
  root = process.cwd(),
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V41_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV41AuthorizationArtifact)
    : undefined;
  ...
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V41_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V41_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV41History(
  expected: CalendarLiveV41HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV41HistoryState {
  const state = readCalendarLiveV41HistoryState(root);
  const failures = validateCalendarLiveV41History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v41 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV41Authorization(
  root = process.cwd(),
): CalendarLiveV41AuthorizationProof {
  const state = verifyCalendarLiveV41History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V41_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v41/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v41.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V41_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V41_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V41_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV41HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV41History(expected);
  process.stdout.write(
    `Calendar live v41 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
