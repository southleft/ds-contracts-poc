...
import { COMBOBOX_LIVE_V5_INDEX_PATH } from "./build-combobox-live-proof-v5.js";
import {
  COMBOBOX_LIVE_V5_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V5_TARGET,
} from "./combobox-live-v5-broker.js";
import {
  buildComboboxLiveV5AuthorizationArtifact,
  validateComboboxLiveV5AntecedentIndex,
  validateComboboxLiveV5AuthorizationArtifact,
  validateComboboxLiveV5History,
  validateComboboxLiveV5UnitTestSource,
  type ComboboxLiveV5AntecedentIndex,
  type ComboboxLiveV5AuthorizationArtifact,
  type ComboboxLiveV5AuthorizationProof,
  type ComboboxLiveV5HistoryState,
} from "./combobox-live-v5-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV5ControlFlowSource,
  validateComboboxLiveV5SecurityAttestation,
  type ComboboxLiveV5SecurityAttestation,
} from "./combobox-live-v5-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v5-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V5_INDEX_PATH);
const index = (): ComboboxLiveV5AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV5AntecedentIndex;

const artifact = (): ComboboxLiveV5AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV5AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV5HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV5HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV5AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v5/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV5AuthorizationArtifact,
): ComboboxLiveV5SecurityAttestation => {
  const body: Omit<ComboboxLiveV5SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V5_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV5History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV5History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV5History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV5History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV5History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV5UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV5UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV5AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV5AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV5AuthorizationArtifact(
        planted as ComboboxLiveV5AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV5SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV5SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV5SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV5ControlFlowSource(source), []);
});
