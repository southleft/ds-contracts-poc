...
/**
 * Table live v14 extract runtime. Keeps the v6 skip of untagged
 * `table/row/owned-cell-label` TEXT bind hosts. Extract bytes are
 * unchanged from v13. The v14 teaching lives in host-normalize
 * (`scene-readback-table-v1.ts`): omit `clipsContent` on
 * `table/variant` because compile variants omit `clipsContent`.
 * Does not omit variant `strokes`, `cornerRadius`, or empty `effects`.
 * Does not invent `clipsContent` onto compile.
 */
