...
import { COMBOBOX_LIVE_V23_INDEX_PATH } from "./build-combobox-live-proof-v23.js";
import {
  COMBOBOX_LIVE_V23_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V23_TARGET,
} from "./combobox-live-v23-broker.js";
import {
  buildComboboxLiveV23AuthorizationArtifact,
  validateComboboxLiveV23AntecedentIndex,
  validateComboboxLiveV23AuthorizationArtifact,
  validateComboboxLiveV23History,
  validateComboboxLiveV23UnitTestSource,
  type ComboboxLiveV23AntecedentIndex,
  type ComboboxLiveV23AuthorizationArtifact,
  type ComboboxLiveV23AuthorizationProof,
  type ComboboxLiveV23HistoryState,
} from "./combobox-live-v23-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV23ControlFlowSource,
  validateComboboxLiveV23SecurityAttestation,
  type ComboboxLiveV23SecurityAttestation,
} from "./combobox-live-v23-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v23-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V23_INDEX_PATH);
const index = (): ComboboxLiveV23AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV23AntecedentIndex;

const artifact = (): ComboboxLiveV23AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV23AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV23HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV23HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV23AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v23/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV23AuthorizationArtifact,
): ComboboxLiveV23SecurityAttestation => {
  const body: Omit<ComboboxLiveV23SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V23_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV23History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV23History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV23History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV23History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV23History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV23UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV23UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV23AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV23AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV23AuthorizationArtifact(
        planted as ComboboxLiveV23AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV23SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV23SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV23SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV23ControlFlowSource(source), []);
});
