...
import { verifyCalendarLiveV30Authorization } from "./calendar-live-v30-authorization.js";
import { CALENDAR_LIVE_V30_TARGET } from "./calendar-live-v30-broker.js";
import {
  CALENDAR_LIVE_V30_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV30Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v30-session";
...
const attestation: CalendarLiveV30SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV30SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v30 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V30_SECURITY_ATTESTATION_DEFAULT_PATH,
);
