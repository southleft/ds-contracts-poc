...
import {
  INPUT_LIVE_V78_MEASURE_VISIBLE_FILL_MARKER,
  INPUT_LIVE_V78_RESTORE_CONTENT_ROLES,
  INPUT_LIVE_V78_RESTORE_PARENT_ROLES,
...
test("v78 probe reveals hidden content FILL before measuring, then restores visibility", () => {
  const program = readFileSync(
    "recipe/input-field-live-v78-contract.ts",
    "utf8",
  );
  const probe = program.slice(
    program.indexOf("export function buildInputLiveV78ProbeProgram"),
  );
...
test("v16 restore still measures FILL while visible, then restores visibility", () => {
  const program = buildInputLiveV78RestoreProgram(writer);
  assert.match(program, new RegExp(INPUT_LIVE_V78_MEASURE_VISIBLE_FILL_MARKER));
  ...
  assert.match(program, /INPUT-V78-RESTORE-COUNT/);
  assert.match(program, /INPUT-V78-RESTORE-NOT-FILL/);
  ...
  for (const role of INPUT_LIVE_V78_RESTORE_CONTENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
  for (const role of INPUT_LIVE_V78_RESTORE_PARENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
});

test("v16 restore payload requires 256 filled content texts", () => {
  const accepted = validateInputLiveV78RestorePayload(
    {
      pageId: "1:1",
      setIds: ["2:2", "2:1"],
      restoredCount: 256,
      ...
      marker: INPUT_LIVE_V78_MEASURE_VISIBLE_FILL_MARKER,
    },
    writer,
  );
...
test("v78 probe excludes overlay-label AABB from visibleAreaLoss; overlap still uses full semantic", () => {
  const program = readFileSync(
    "recipe/input-field-live-v78-contract.ts",
    "utf8",
  );
