...
import {
  ...
  buildInputLiveV85ExtractProgram,
  buildInputLiveV85ProbeProgram,
  buildInputLiveV85RestoreProgram,
  INPUT_LIVE_V85_CAPTURE_COUNT,
  INPUT_LIVE_V85_RESPONSE_CONTRACTS,
  inputLiveV85CaptureManifestSha256,
  type InputLiveV85CaptureCell,
  type InputLiveV85SourceIdentity,
  type InputLiveV85WriterOwnership,
} from "./input-field-live-v85-contract.js";
import {
  INPUT_LIVE_V85_DYNAMIC_TOOL,
  INPUT_LIVE_V85_TARGET,
  inputLiveV85RequestSequence,
} from "./input-field-live-v85-broker.js";
import {
...
export const INPUT_LIVE_V85_EVIDENCE_ROOT =
  "recipe/evidence/input-field-live-pivot-v85";
export const INPUT_LIVE_V85_PROTOCOL_PATH = `${INPUT_LIVE_V85_EVIDENCE_ROOT}/protocol.json`;
export const INPUT_LIVE_V85_PLAN_PATH = `${INPUT_LIVE_V85_EVIDENCE_ROOT}/proof-plan.json`;
export const INPUT_LIVE_V85_CAPTURE_MANIFEST_PATH = `${INPUT_LIVE_V85_EVIDENCE_ROOT}/capture-manifest.json`;
export const INPUT_LIVE_V85_REQUEST_MANIFEST_PATH = `${INPUT_LIVE_V85_EVIDENCE_ROOT}/request-manifest.json`;
export const INPUT_LIVE_V85_INDEX_PATH = `${INPUT_LIVE_V85_EVIDENCE_ROOT}/antecedent-index.json`;
export const INPUT_LIVE_V85_AUTHORIZATION_TEMPLATE_PATH = `${INPUT_LIVE_V85_EVIDENCE_ROOT}/authorization-template.json`;
export const INPUT_LIVE_V85_AUTHORIZATION_PATH = `${INPUT_LIVE_V85_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface InputLiveV85ProofPlan {
  artifactVersion: "input-live-v85-proof-plan-v1";
  status: "draft antecedent; pending separate authorization; live execution forbidden";
  target: typeof INPUT_LIVE_V85_TARGET;
...
interface GeneratedSource {
  ...
  recipeHash: string;
  envelopeHash: string;
  expectedScenePlan: InputLiveV85SourceIdentity["expectedScenePlan"];
  outputPath: string;
  compressed: Buffer;
  metadata: InputLiveV85ProofPlan["sources"][number]["expectedScenePlan"];
}
...
const sourcePlans = (): GeneratedSource[] => {
  ...
  return prior.sources.map((source: Record<string, any>) => {
    ...
    if (!rewrittenText.includes(needle))
      throw new Error(
        `Input live v85 missing set layout.mode vertical fact: ${inputPath}`,
      );
    ...
    if (!modeRewritten.includes(paddingNeedle))
      throw new Error(
        `Input live v85 missing set layout.padding 0 fact: ${inputPath}`,
      );
    const paddingRewritten = modeRewritten.replace(
      paddingNeedle,
      paddingReplacement,
    );
    const visibleNeedle = '"channel":"visible","value":false';
    const visibleReplacement = '"channel":"visible","value":true';
    const visibleFalseCount = paddingRewritten.split(visibleNeedle).length - 1;
    if (sourceId === "mui" && visibleFalseCount !== 24)
      throw new Error(
        `Input live v85 MUI expected-plan visible:false count ${visibleFalseCount}, expected 24`,
      );
    if (sourceId === "polaris" && visibleFalseCount !== 0)
      throw new Error(
        `Input live v85 Polar expected-plan has ${visibleFalseCount} visible:false facts; do not restamp Polar`,
      );
    const visibleRewritten =
      sourceId === "mui"
        ? paddingRewritten.split(visibleNeedle).join(visibleReplacement)
        : paddingRewritten;
    const occupancyKeys = [
      ...new Set(
        (
          JSON.parse(paddingRewritten) as {
            facts: Array<{
              channel?: string;
              value?: unknown;
          ...
          .filter(
            (fact) => fact.channel === "visible" && fact.value === false,
          )
          .map((fact) => fact.nodeOwnershipKey)
          .filter((key): key is string => typeof key === "string"),
      ),
    ];
    if (sourceId === "mui" && occupancyKeys.length !== 24)
      throw new Error(
        `Input live v85 MUI occupancy keys ${occupancyKeys.length}, expected 24`,
      );
    if (sourceId === "polaris" && occupancyKeys.length !== 0)
      throw new Error(
        `Input live v85 Polar expected-plan has ${occupancyKeys.length} occupancy keys; do not restamp Polar`,
      );
    let opacityRewritten = visibleRewritten;
    if (sourceId === "mui") {
      for (const key of occupancyKeys) {
        const opacityNeedle = `"nodeOwnershipKey":${JSON.stringify(key)},"channel":"opacity","value":1`;
        const opacityReplacement = `"nodeOwnershipKey":${JSON.stringify(key)},"channel":"opacity","value":0`;
        const occurrences = opacityRewritten.split(opacityNeedle).length - 1;
        if (occurrences !== 1)
          throw new Error(
            `Input live v85 occupancy opacity:1 fact count ${occurrences} for ${key}`,
          );
        opacityRewritten = opacityRewritten.replace(
          opacityNeedle,
          opacityReplacement,
        );
      }
    }
    const rewritten = Buffer.from(opacityRewritten);
    ...
    const occupancyOpacity = plan.facts.filter(
      (fact) =>
        fact.channel === "opacity" &&
        occupancyKeys.includes(fact.nodeOwnershipKey ?? ""),
    );
    const occupancyOpacityZero = occupancyOpacity.filter(
      (fact) => fact.value === 0,
    ).length;
    const occupancyOpacityOne = occupancyOpacity.filter(
      (fact) => fact.value === 1,
    ).length;
    const otherOpacityOne = plan.facts.filter(
      (fact) =>
        fact.channel === "opacity" &&
        fact.value === 1 &&
        !occupancyKeys.includes(fact.nodeOwnershipKey ?? ""),
    ...
    if (
      sourceId === "mui" &&
      (occupancyOpacityZero !== 24 ||
        occupancyOpacityOne !== 0 ||
        otherOpacityOne !== 1193)
    )
      throw new Error(
        `Input live v85 MUI occupancy opacity rewrite failed: zero=${occupancyOpacityZero} one=${occupancyOpacityOne} otherOne=${otherOpacityOne}`,
      );
    if (
      sourceId === "polaris" &&
      (occupancyOpacity.length !== 0 ||
        plan.facts.filter((fact) => fact.channel === "opacity" && fact.value === 1)
          .length !== 1089)
    )
      throw new Error(
        `Input live v85 Polar opacity facts drifted; do not restamp Polar`,
      );
    const visibleFalseAfter = plan.facts.filter(
      (fact) => fact.channel === "visible" && fact.value === false,
    ).length;
    const visibleTrueAfter = plan.facts.filter(
      (fact) => fact.channel === "visible" && fact.value === true,
    ).length;
    if (sourceId === "mui" && (visibleFalseAfter !== 0 || visibleTrueAfter !== 1217))
      throw new Error(
        `Input live v85 MUI visible rewrite failed: false=${visibleFalseAfter} true=${visibleTrueAfter}`,
      );
    if (sourceId === "polaris" && (visibleFalseAfter !== 0 || visibleTrueAfter !== 1089))
      throw new Error(
        `Input live v85 Polar visible facts drifted: false=${visibleFalseAfter} true=${visibleTrueAfter}`,
      );
    const setMode = plan.facts.find((fact) => fact.id === "root#layout.mode@0000");
    if (setMode?.value !== "horizontal")
      throw new Error(
        `Input live v85 set layout.mode rewrite failed: ${inputPath}`,
      );
    ...
    if (
      ...
      throw new Error(
        `Input live v85 set layout.padding rewrite failed: ${inputPath}`,
      );
    ...
    if (sourceId === "polaris") {
      ...
      if (!priorUncompressed.equals(rewritten))
        throw new Error(
          "Input live v85 Polar uncompressed drifted from V83; do not restamp Polar",
        );
    }
    return {
      source: sourceId,
      ...
      outputPath: `${INPUT_LIVE_V85_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
      compressed: rewrittenCompressed,
      metadata: {
        path: `${INPUT_LIVE_V85_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
...
const captureCells = (): InputLiveV85CaptureCell[] => {
  const cells = json<Record<string, any>>(V16_CAPTURE_MANIFEST)
    .cells as InputLiveV85CaptureCell[];
  inputLiveV85CaptureManifestSha256(cells);
  return cells;
};

const protocol = () => ({
  artifactVersion: "input-live-v85-external-operator-protocol-v1",
  protocolId: "input-live-v85",
  status:
  ...
  lifecycle: {
    ...
    taughtContentOpacityOmitted: true,
    taughtCompileCarryLiveVisible: true,
    inventHostVisibleFalseForbidden: true,
    taughtCompileCarryLiveOpacity: true,
    inventOpacityVariableForbidden: true,
    inventCompileTextOpacityForbidden: true,
    taughtCollapseOmitInventedContentTextOpacity: true,
    taughtProbeExcludeOpacityZeroOccupancyOverlap: true,
    taughtCleanupOnFailureOnly: true,
    v82SceneReadbackUnchanged: true,
    v82AuthorizationReusable: false,
    v83SceneReadbackUnchanged: true,
    v83AuthorizationReusable: false,
    v84SceneReadbackUnchanged: true,
    v84AuthorizationReusable: false,
    v81SceneReadbackUnchanged: true,
    v81AuthorizationReusable: false,
    v80SceneReadbackUnchanged: true,
    v80AuthorizationReusable: false,
    v79SceneReadbackUnchanged: true,
    v79AuthorizationReusable: false,
  ...
  operatorBoundary: {
    expectedDynamicTool: INPUT_LIVE_V85_DYNAMIC_TOOL,
    target: INPUT_LIVE_V85_TARGET,
  ...
  hostNormalization: {
    ...
    carriedSceneReadback: "recipe/scene-readback-v85.ts",
    carriedSceneReadbackRuntime: "recipe/scene-readback-runtime-v85.ts",
    carriedV3Verifier: "recipe/input-field-live-v3-verifier-v85.ts",
...
const requestManifest = (
  captures: readonly InputLiveV85CaptureCell[],
  programs: {
    writerSha256: string;
  ...
  const requests = [
    {
      requestId: "writer",
      sequence: inputLiveV85RequestSequence("writer"),
      phase: "writer",
      predecessorRequestId: null,
      programSha256: programs.writerSha256,
      response: INPUT_LIVE_V85_RESPONSE_CONTRACTS.writer,
    },
    {
      requestId: "cleanup",
      sequence: inputLiveV85RequestSequence("cleanup"),
      phase: "cleanup",
      predecessorRequestId: "writer",
      programBuilderSha256: programs.cleanupBuilderSha256,
      response: INPUT_LIVE_V85_RESPONSE_CONTRACTS.cleanup,
      availability: "persisted immediately after writer acceptance",
    },
    {
      requestId: "restore",
      sequence: inputLiveV85RequestSequence("restore"),
      phase: "restore",
      predecessorRequestId: "writer",
      programSha256: programs.restoreBlueprintSha256,
      response: INPUT_LIVE_V85_RESPONSE_CONTRACTS.restore,
      availability:
        "issued immediately after writer acceptance; before extract",
    },
    {
      requestId: "extract",
      sequence: inputLiveV85RequestSequence("extract"),
      phase: "extract",
      predecessorRequestId: "restore",
      programSha256: programs.extractBlueprintSha256,
      response: INPUT_LIVE_V85_RESPONSE_CONTRACTS.extract,
    },
    {
      requestId: "probe",
      sequence: inputLiveV85RequestSequence("probe"),
      phase: "probe",
      predecessorRequestId: "extract",
      programSha256: programs.probeBlueprintSha256,
      response: INPUT_LIVE_V85_RESPONSE_CONTRACTS.probe,
    },
    ...captures.map((cell) => ({
      requestId: `capture-${String(cell.index).padStart(3, "0")}`,
      sequence: inputLiveV85RequestSequence("capture", cell.index),
      phase: "capture",
      ...
      response: INPUT_LIVE_V85_RESPONSE_CONTRACTS.capture,
    })),
  ];
  return {
    artifactVersion: "input-live-v85-request-manifest-v1",
    expectedDynamicTool: INPUT_LIVE_V85_DYNAMIC_TOOL,
    target: INPUT_LIVE_V85_TARGET,
...
const lifecycleExcludedPaths = [
  INPUT_LIVE_V85_AUTHORIZATION_TEMPLATE_PATH,
  INPUT_LIVE_V85_AUTHORIZATION_PATH,
  "recipe/input-field-live-v85-authorization.ts",
  "recipe/input-field-live-v85-authorization.test.ts",
  "recipe/input-field-live-v85-preflight.ts",
  "recipe/input-field-live-v85-authorized.ts",
  "recipe/evidence/input-field-live-pivot-v85/operator-security-attestation-template.json",
...
const sourceNeutralDependencies = [
  ...
  "recipe/normalize.ts",
  "recipe/scene-readback-runtime-v85.ts",
  "recipe/scene-readback-v85.ts",
  "recipe/input-field-live-v85-restore.ts",
  "recipe/input-field-figma-writer-v18.ts",
  "recipe/input-field-figma-writer-v19.ts",
] as const;

export async function buildInputLiveV85Proof(
  check = process.argv.includes("--check"),
): Promise<InputLiveV85ProofPlan> {
  ...
  if (
    ...
    throw new Error("Input live v85 frozen v16 writer bytes drifted");
  ...
  const priorV79SceneReadback = readFileSync("recipe/scene-readback-v79.ts");
  const priorV80SceneReadback = readFileSync("recipe/scene-readback-v80.ts");
  const priorV81SceneReadback = readFileSync("recipe/scene-readback-v81.ts");
  const priorV82SceneReadback = readFileSync("recipe/scene-readback-v82.ts");
  const priorV83SceneReadback = readFileSync("recipe/scene-readback-v83.ts");
  const priorV84SceneReadback = readFileSync("recipe/scene-readback-v84.ts");
  ...
  if (
    ...
    sha256(priorV79SceneReadback) !== V79_SCENE_READBACK_SHA256 ||
    sha256(priorV80SceneReadback) !== V80_SCENE_READBACK_SHA256 ||
    sha256(priorV81SceneReadback) !== V81_SCENE_READBACK_SHA256 ||
    sha256(priorV82SceneReadback) !== V82_SCENE_READBACK_SHA256 ||
    sha256(priorV83SceneReadback) !== V83_SCENE_READBACK_SHA256 ||
    sha256(priorV84SceneReadback) !== V84_SCENE_READBACK_SHA256 ||
    sha256(priorV17WriterSource) !== V17_WRITER_SOURCE_SHA256 ||
    sha256(priorV18WriterSource) !== V18_WRITER_SOURCE_SHA256
  )
    throw new Error(
      "Input live v85 exact committed v16 writer/restore/runtime/extract or hashed v16/v17/v18/v19 writer or v17-v66/v70/v84 scene-readback bytes drifted",
    );

  const writerOwnershipBlueprint: InputLiveV85WriterOwnership = {
    pageId: "__WRITER_PAGE_ID__",
    pageName: "Recipe Pivot / Input Field / v9",
    runIdentity: "input-live-v85",
  ...
  const identities = sources.map(
    (source) =>
      ...
      }) satisfies InputLiveV85SourceIdentity,
  );
  const restoreBlueprint = Buffer.from(
    buildInputLiveV85RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildInputLiveV85ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildInputLiveV85ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildInputLiveV85CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );

  const antecedentOutputs = new Map<string, Buffer>();
  antecedentOutputs.set(INPUT_LIVE_V85_PROTOCOL_PATH, jsonBytes(protocol()));
  antecedentOutputs.set(
    `${INPUT_LIVE_V85_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V85_EVIDENCE_ROOT}/programs/writer-payload.js`,
    writerPayload,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V85_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V85_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V85_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V85_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  ...
  const captureManifest = {
    artifactVersion: "input-live-v85-capture-manifest-v1",
    ...
    cells: captures,
    cellsSha256: inputLiveV85CaptureManifestSha256(captures),
  };
  const captureManifestBytes = jsonBytes(captureManifest);
  antecedentOutputs.set(
    INPUT_LIVE_V85_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: InputLiveV85ProofPlan = {
    artifactVersion: "input-live-v85-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: INPUT_LIVE_V85_TARGET,
    namespace: "ds.contracts.input.recipe.v5",
    writer: {
      programPath: `${INPUT_LIVE_V85_EVIDENCE_ROOT}/programs/writer.txt`,
      programBytes: writerProgram.byteLength,
      programSha256: sha256(writerProgram),
      payloadPath: `${INPUT_LIVE_V85_EVIDENCE_ROOT}/programs/writer-payload.js`,
    ...
    captureManifest: {
      path: INPUT_LIVE_V85_CAPTURE_MANIFEST_PATH,
      cells: 128,
      requests: 128,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(INPUT_LIVE_V85_PLAN_PATH, proofPlanBytes);
  ...
  antecedentOutputs.set(
    INPUT_LIVE_V85_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/input-field-live-v85-broker.ts",
    "recipe/input-field-live-v85-contract.ts",
    "recipe/run-input-field-live-v85.ts",
    "recipe/build-input-field-live-proof-v85.ts",
    "recipe/input-field-live-v85-broker.test.ts",
    "recipe/figma-property-normalizer-v8.test.ts",
    "recipe/scene-readback-v85.test.ts",
    "recipe/input-field-live-v85-restore.ts",
    "recipe/input-field-live-v85-restore.test.ts",
    "recipe/input-field-live-v85-lifecycle-simulation.ts",
    ...sourceNeutralDependencies,
  ].filter((artifactPath) => artifactPath !== INPUT_LIVE_V85_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "input-live-v85-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    ...
    counts: {
      ...
      captureCells: INPUT_LIVE_V85_CAPTURE_COUNT,
  ...
  antecedentOutputs.set(INPUT_LIVE_V85_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "input-live-v85-authorization-template-v1",
    authorizationId: "input-live-v85",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: INPUT_LIVE_V85_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
  ...
  if (check) {
    ...
    mkdirSync(path.dirname(INPUT_LIVE_V85_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(INPUT_LIVE_V85_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(INPUT_LIVE_V85_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(INPUT_LIVE_V85_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
  ...
  if (
    ...
    protocol().execution.requestOrder.at(-1) === "cleanup" ||
    !protocol().execution.requestOrder.includes(
      "persist signed cleanup recovery request",
    ) ||
    protocol().execution.cleanupOnFailureOnly !== true ||
    protocol().execution.cleanupMustNotExecuteOnMainComplete !== true ||
    protocol().proof.cleanupOnFailureOnly !== true
  )
    throw new Error(
      "Input live v85 deterministic hash/count or cleanup-on-failure-only invariant failed",
    );
  return proofPlan;
}
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildInputLiveV85Proof(), null, 2)}\n`,
  );
