...
import {
  ...
  verifyInputLiveV48Authorization,
  type InputLiveV48AntecedentIndex,
  type InputLiveV48AuthorizationProof,
} from "./input-field-live-v48-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V48_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v48/operator-security-attestation-template.json";
export const INPUT_LIVE_V48_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v48-security-attestation.json";

export interface InputLiveV48SecurityAttestation {
...
export interface InputLiveV48PreflightReport {
  artifactVersion: "input-live-v48-preflight-v1";
...
export const inputLiveV48AttestationSha256 = (
  value: Omit<InputLiveV48SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV48SecurityAttestation(
  value: unknown,
  proof: InputLiveV48AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v48-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V48_TARGET) ||
...
export function validateInputLiveV48ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV48Authorization");
  const preflight = source.indexOf("runInputLiveV48Preflight");
  const initialize = source.indexOf("InputLiveV48Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV48Preflight(
  root: string,
  proof: InputLiveV48AuthorizationProof,
  attestationPath = INPUT_LIVE_V48_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV48PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V48_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V48_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V48_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV48AntecedentIndex;
  failures.push(...validateInputLiveV48AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V48_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V48_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v48-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV48ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV48SecurityAttestation;
  return {
    artifactVersion: "input-live-v48-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V48_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV48Preflight(
    process.cwd(),
    verifyInputLiveV48Authorization(),
    attestationPath,
