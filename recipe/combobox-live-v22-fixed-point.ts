/**
 * Carried Combobox live v22 host verifier. Uses scene-readback-combobox-v22 so live
...
export const COMBOBOX_LIVE_V22_FIXED_VERIFIER_VERSION = "combobox-live-v22-fixed-point-v1";
export const COMBOBOX_LIVE_V22_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV22FixedThresholds {
...
export interface ComboboxLiveV22FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV22FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV22FixedSourceProbe {
...
export interface ComboboxLiveV22FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV22FixedFixedPoint;
}

export interface ComboboxLiveV22FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV22FixedHardGateInput {
  thresholds: ComboboxLiveV22FixedThresholds;
  materialRegression: ComboboxLiveV22FixedMaterialRegression;
  sourceProbes: ComboboxLiveV22FixedSourceProbe[];
  cells: ComboboxLiveV22FixedCellValidation[];
  sceneProofs: ComboboxLiveV22FixedSceneProof[];
  visualRows: ComboboxLiveV22FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV22FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V22_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV22FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV22FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV22FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV22FixedHardGates(
  input: ComboboxLiveV22FixedHardGateInput,
): ComboboxLiveV22FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV22FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV22FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V22_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV22FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV22FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV22FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
