...
import {
  assertCalendarLiveV15RootProofs,
  CALENDAR_LIVE_V15_CAPTURE_COUNT,
  CALENDAR_LIVE_V15_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  CALENDAR_LIVE_V15_RESPONSE_CONTRACTS,
  type CalendarLiveV15RootProof,
} from "./calendar-live-v15-contract.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V15_BROKER_VERSION = "calendar-live-v15-broker-v1";
export const CALENDAR_LIVE_V15_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const CALENDAR_LIVE_V15_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const CALENDAR_LIVE_V15_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const CALENDAR_LIVE_V15_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const CALENDAR_LIVE_V15_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const CALENDAR_LIVE_V15_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type CalendarLiveV15RemotePhase =
  (typeof CALENDAR_LIVE_V15_REMOTE_PHASES)[number];
export type CalendarLiveV15AuthorizationMode = "live" | "simulated";

export interface CalendarLiveV15CommitPins {
...
export interface CalendarLiveV15TransactionAuthorization extends CalendarLiveV15CommitPins {
  mode: CalendarLiveV15AuthorizationMode;
...
export interface CalendarLiveV15Manifest {
  artifactVersion: "calendar-live-v15-broker-manifest-v2";
  brokerVersion: typeof CALENDAR_LIVE_V15_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof CALENDAR_LIVE_V15_DYNAMIC_TOOL;
  target: typeof CALENDAR_LIVE_V15_TARGET;
  authorization: CalendarLiveV15TransactionAuthorization;
...
export interface CalendarLiveV15ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface CalendarLiveV15RequestBody {
  artifactVersion: "calendar-live-v15-broker-request-v2";
  brokerVersion: typeof CALENDAR_LIVE_V15_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: CalendarLiveV15RemotePhase;
  sequence: number;
  ...
  commitPins: CalendarLiveV15CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof CALENDAR_LIVE_V15_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: CalendarLiveV15ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface CalendarLiveV15Request extends CalendarLiveV15RequestBody {
...
export interface CalendarLiveV15Receipt {
  artifactVersion: "calendar-live-v15-broker-receipt-v2";
  brokerVersion: typeof CALENDAR_LIVE_V15_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: CalendarLiveV15RemotePhase;
...
export interface CalendarLiveV15AcceptedResponse<T = unknown> {
  request: CalendarLiveV15Request;
  receipt: CalendarLiveV15Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface CalendarLiveV15IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: CalendarLiveV15ResponseContract;
...
const equalTarget = (value: unknown): value is typeof CALENDAR_LIVE_V15_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === CALENDAR_LIVE_V15_TARGET.fileKey &&
  (value as BrokerTarget).fileName === CALENDAR_LIVE_V15_TARGET.fileName &&
  (value as BrokerTarget).editorType === CALENDAR_LIVE_V15_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== CALENDAR_LIVE_V15_TARGET.fileKey ||
    context.fileName !== CALENDAR_LIVE_V15_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === CALENDAR_LIVE_V15_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Calendar live v15 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= CALENDAR_LIVE_V15_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v15 capture index must be 0..7");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Calendar live v15 authorization pins are malformed");
};

const readManifest = (directory: string): CalendarLiveV15Manifest => {
  const manifest = json<CalendarLiveV15Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Calendar live v15 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "calendar-live-v15-broker-manifest-v2" ||
    manifest.brokerVersion !== CALENDAR_LIVE_V15_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== CALENDAR_LIVE_V15_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(CALENDAR_LIVE_V15_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Calendar live v15 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: CalendarLiveV15Request,
): CalendarLiveV15RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: CalendarLiveV15RemotePhase,
  captureIndex?: number,
): CalendarLiveV15Request | undefined => {
  const file = calendarLiveV1RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV15Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: CalendarLiveV15RemotePhase,
  captureIndex?: number,
): CalendarLiveV15Receipt | undefined => {
  const file = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV15Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: CalendarLiveV15Request | undefined;
  receipt: CalendarLiveV15Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: CalendarLiveV15RemotePhase,
): CalendarLiveV15ResponseContract => ({
  ...CALENDAR_LIVE_V15_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: CalendarLiveV15Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: CALENDAR_LIVE_V15_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: CALENDAR_LIVE_V15_DYNAMIC_TOOL,
      target: CALENDAR_LIVE_V15_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "calendar-live-v15-plugin-response-v2",
  brokerVersion: CALENDAR_LIVE_V15_BROKER_VERSION,
  target: CALENDAR_LIVE_V15_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*CALENDAR_LIVE_V15_PROGRAM_START*/${program}/*CALENDAR_LIVE_V15_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*CALENDAR_LIVE_V15_PROGRAM_START*/";
  const endMarker = "/*CALENDAR_LIVE_V15_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Calendar live v15 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createCalendarLiveV15Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Calendar live v15 private signing key does not match authorized public-key identity",
    );
  const manifest: CalendarLiveV15Manifest = {
    artifactVersion: "calendar-live-v15-broker-manifest-v2",
    brokerVersion: CALENDAR_LIVE_V15_BROKER_VERSION,
    ...
    captureRequests: CALENDAR_LIVE_V15_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateCalendarLiveV15Request(
  directory: string,
  request: CalendarLiveV15Request,
): void {
  ...
  if (
    request.artifactVersion !== "calendar-live-v15-broker-request-v2" ||
    request.brokerVersion !== CALENDAR_LIVE_V15_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== CALENDAR_LIVE_V15_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Calendar live v15 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueCalendarLiveV15Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: CalendarLiveV15RemotePhase,
  program: string,
  options: CalendarLiveV15IssueOptions = {},
): CalendarLiveV15Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<CalendarLiveV15Request>(targetPath);
    validateCalendarLiveV15Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Calendar live v15 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(calendarLiveV1ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Calendar live v15 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Calendar live v15 empty code envelope refused");
  if (phase === "capture") validateCalendarLiveV15TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Calendar live v15 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: CALENDAR_LIVE_V15_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? CALENDAR_LIVE_V15_SIGNED_WRITER_TIMEOUT_MS
        : CALENDAR_LIVE_V15_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: CalendarLiveV15RequestBody = {
    artifactVersion: "calendar-live-v15-broker-request-v2",
    brokerVersion: CALENDAR_LIVE_V15_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: CalendarLiveV15Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateCalendarLiveV15Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistCalendarLiveV15RawResponse(
  directory: string,
  phase: CalendarLiveV15RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Calendar live v15 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<CalendarLiveV15Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Calendar live v15 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = calendarLiveV1ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Calendar live v15 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): CalendarLiveV15Receipt => {
  const body: Omit<CalendarLiveV15Receipt, "receiptSha256"> = {
    artifactVersion: "calendar-live-v15-broker-receipt-v2",
    brokerVersion: CALENDAR_LIVE_V15_BROKER_VERSION,
...
export function acceptCalendarLiveV15Response<T = unknown>(
  directory: string,
  phase: CalendarLiveV15RemotePhase,
  options: {
    captureIndex?: number;
...
): CalendarLiveV15AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Calendar live v15 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<CalendarLiveV15Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Calendar live v15 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "calendar-live-v15-plugin-response-v2" ||
    plugin?.brokerVersion !== CALENDAR_LIVE_V15_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Calendar live v15 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistCalendarLiveV15TechnicalGates(
  directory: string,
  rootProofs: readonly CalendarLiveV15RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertCalendarLiveV15RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Calendar live v15 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Calendar live v15 probe payload hash malformed");
  const gates = Object.fromEntries(
    CALENDAR_LIVE_V15_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "calendar-live-v15-host-technical-gates-v2",
    brokerVersion: CALENDAR_LIVE_V15_BROKER_VERSION,
...
export function validateCalendarLiveV15TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "calendar-live-v15-host-technical-gates-v2" ||
    ...
    CALENDAR_LIVE_V15_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Calendar live v15 technical gates artifact");
}

export function validateCalendarLiveV15Receipt(
  directory: string,
  phase: CalendarLiveV15RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateCalendarLiveV15Request(directory, request);
  const receipt = json<CalendarLiveV15Receipt>(
  ...
  if (
    receipt.artifactVersion !== "calendar-live-v15-broker-receipt-v2" ||
    receipt.brokerVersion !== CALENDAR_LIVE_V15_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Calendar live v15 accepted receipt: ${request.requestId}`,
    );
}

export function readCalendarLiveV15AcceptedRawPayload(
  directory: string,
  phase: CalendarLiveV15RemotePhase,
  captureIndex?: number,
): unknown {
  validateCalendarLiveV15Receipt(directory, phase, captureIndex);
...
export function createCalendarLiveV15FakeDynamicResponse(
  request: CalendarLiveV15Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? CALENDAR_LIVE_V15_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? CALENDAR_LIVE_V15_TARGET,
    ...overrides.extra,
  };
}

export function listCalendarLiveV15PersistedRequests(
  directory: string,
): CalendarLiveV15Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<CalendarLiveV15Request>(file)] : [];
    });
}
