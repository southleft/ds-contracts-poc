...
} from "./input-field-live-v3-verifier-v44.js";
import {
  buildInputLiveV44RawPropertyRuntime,
  normalizeInputLiveV44Scene,
  type InputLiveV44RawNode,
} from "./input-field-live-v44-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v44.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v44.js";
...
export const INPUT_LIVE_V44_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V44_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V44_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V44_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V44_VARIANT_COUNT = 256;
export const INPUT_LIVE_V44_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V44_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV44SourceId = (typeof INPUT_LIVE_V44_SOURCE_IDS)[number];

export interface InputLiveV44SourceIdentity {
  source: InputLiveV44SourceId;
...
export interface InputLiveV44WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV44ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV44SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV44RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV44RootProof {
  source: InputLiveV44SourceId;
  adapterIdentity: string;
...
export interface InputLiveV44ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV44SourceId;
    adapterIdentity: string;
...
export interface InputLiveV44CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV44SourceId;
  adapterIdentity: string;
...
export interface InputLiveV44CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV44SourceId;
  strata: InputLiveV44CaptureCell["strata"];
...
export interface InputLiveV44CleanupPayload {
...
export function validateInputLiveV44WriterPayload(
  value: unknown,
): InputLiveV44WriterOwnership {
...
const identityPlan = (source: InputLiveV44SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV44ExtractProgram(
  writer: InputLiveV44WriterOwnership,
  sources: readonly InputLiveV44SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V44_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V44_NAMESPACE)}
${buildInputLiveV44RawPropertyRuntime()}
...
export function validateInputLiveV44ExtractPayload(
  value: unknown,
  writer: InputLiveV44WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V44_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV44ExtractPayload;
}

export function proveInputLiveV44Roots<Instance>(
  extract: InputLiveV44ExtractPayload,
  sources: readonly (InputLiveV44SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV44Scene(
      root.scene,
...
export function assertInputLiveV44RootProofs(
  proofs: readonly InputLiveV44RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV44ProbeProgram(
  writer: InputLiveV44WriterOwnership,
  sources: readonly InputLiveV44SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V44_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV44ProbePayload(
  value: unknown,
  writer: InputLiveV44WriterOwnership,
): InputLiveV44ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV44ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V44_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V44_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV44CaptureManifest(
  cells: readonly InputLiveV44CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V44_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V44_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V44_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V44_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V44_SOURCE_IDS.some(
...
export const inputLiveV44CaptureManifestSha256 = (
  cells: readonly InputLiveV44CaptureCell[],
): string => {
  validateInputLiveV44CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV44CaptureProgram(
  writer: InputLiveV44WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V44_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V44_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V44_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV44CapturePayload(
  value: unknown,
  cell: InputLiveV44CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV44CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V44_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V44_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV44CaptureResponses(
  manifest: readonly InputLiveV44CaptureCell[],
  responses: readonly InputLiveV44CapturePayload[],
): void {
  validateInputLiveV44CaptureManifest(manifest);
  if (
...
export interface InputLiveV44ObjectiveReport {
  artifactVersion: "input-live-v44-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV44Objective(
  manifest: readonly InputLiveV44CaptureCell[],
  responses: readonly InputLiveV44CapturePayload[],
  readReference: (cell: InputLiveV44CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV44CaptureCell) => Buffer,
): InputLiveV44ObjectiveReport {
  assertInputLiveV44CaptureResponses(manifest, responses);
...
export function buildInputLiveV44CleanupProgram(
  writer: InputLiveV44WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V44_NAMESPACE,
...
export function validateInputLiveV44CleanupPayload(
  value: unknown,
  writer: InputLiveV44WriterOwnership,
): InputLiveV44CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV44CleanupPayload;
...
export const INPUT_LIVE_V44_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV44WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
