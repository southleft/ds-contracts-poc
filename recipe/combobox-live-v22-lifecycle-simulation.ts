...
export interface ComboboxLiveV22LifecycleSimulationReport {
  artifactVersion: "combobox-live-v22-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV22AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV22Lifecycle(): Promise<ComboboxLiveV22LifecycleSimulationReport> {
  await buildComboboxLiveV22Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V22_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV22AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v22-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V22_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V22_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV22History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV22History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV22AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V22_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V22_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV22History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV22History(authorized, "pending");
    ...
    await buildComboboxLiveV22Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V22_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV22AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV22Lifecycle(), null, 2)}\n`,
  );
