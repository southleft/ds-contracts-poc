/**
 * Calendar live v46 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V46_FIXED_VERIFIER_VERSION = "calendar-live-v46-fixed-point-v1";
export const CALENDAR_LIVE_V46_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV46FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV46FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV46FixedSourceProbe {
...
export interface CalendarLiveV46FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV46FixedFixedPoint;
}

export interface CalendarLiveV46FixedHardGateInput {
  thresholds: CalendarLiveV46FixedThresholds;
  sourceProbes: CalendarLiveV46FixedSourceProbe[];
  cells: CalendarLiveV46FixedCellValidation[];
  sceneProofs: CalendarLiveV46FixedSceneProof[];
...
export interface CalendarLiveV46FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V46_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV46FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV46FixedHardGates(
  input: CalendarLiveV46FixedHardGateInput,
): CalendarLiveV46FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV46FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V46_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV46FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV46FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV46FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
