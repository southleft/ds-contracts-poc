...
export type ComboboxLiveV23HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV23AuthorizationArtifact {
  artifactVersion: "combobox-live-v23-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V23_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV23AntecedentIndex {
...
export interface ComboboxLiveV23HistoryState {
  index?: ComboboxLiveV23AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV23AuthorizationArtifact;
...
export interface ComboboxLiveV23AuthorizationProof extends ComboboxLiveV23TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V23_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V23_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V23_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V23_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v23-authorization") ||
  artifactPath.includes("combobox-live-v23-preflight") ||
  artifactPath.includes("combobox-live-v23-authorized") ||
  artifactPath.includes("create-combobox-live-v23-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v23-status") ||
...
export function validateComboboxLiveV23AntecedentIndex(
  index: ComboboxLiveV23AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v23-antecedent-index-v1" ||
...
export function validateComboboxLiveV23UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV23History\(\s*["']/.test(source) ||
    /readComboboxLiveV23HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV23AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV23AuthorizationArtifact {
  ...
  ) as ComboboxLiveV23AntecedentIndex;
  const indexFailures = validateComboboxLiveV23AntecedentIndex(index);
...
export function validateComboboxLiveV23AuthorizationArtifact(
  artifact: ComboboxLiveV23AuthorizationArtifact | undefined,
  index: ComboboxLiveV23AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v23-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v23" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V23_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V23_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V23_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V23_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V23_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V23_SOURCE_ROOTS ||
  ...
  if (
    ...
    failures.push(
      "v23 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
...
export function validateComboboxLiveV23History(
  state: ComboboxLiveV23HistoryState,
  expected: ComboboxLiveV23HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV23AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV23AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV23HistoryState(
  root = process.cwd(),
): ComboboxLiveV23HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V23_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV23AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V23_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V23_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V23_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV23History(
  expected: ComboboxLiveV23HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV23HistoryState {
  const state = readComboboxLiveV23HistoryState(root);
  const failures = validateComboboxLiveV23History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v23 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV23Authorization(
  root = process.cwd(),
): ComboboxLiveV23AuthorizationProof {
  const state = verifyComboboxLiveV23History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V23_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v23/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v23.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V23_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V23_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V23_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV23HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV23History(expected);
  process.stdout.write(
    `Combobox live v23 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
