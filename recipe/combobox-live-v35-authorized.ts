...
import { COMBOBOX_LIVE_V35_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v35.js";
import { verifyComboboxLiveV35Authorization } from "./combobox-live-v35-authorization.js";
import { runComboboxLiveV35Preflight } from "./combobox-live-v35-preflight.js";
import { ComboboxLiveV35Orchestrator } from "./run-combobox-live-v35.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v35 requires exact authorization artifact ${COMBOBOX_LIVE_V35_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v35-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV35Authorization(root);
    runComboboxLiveV35Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV35Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV35Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
