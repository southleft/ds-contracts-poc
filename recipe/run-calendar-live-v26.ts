...
import {
  acceptCalendarLiveV26Response,
  createCalendarLiveV26Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV26Request,
  persistCalendarLiveV26RawResponse,
  persistCalendarLiveV26TechnicalGates,
  readCalendarLiveV26AcceptedRawPayload,
  type CalendarLiveV26Request,
  type CalendarLiveV26TransactionAuthorization,
} from "./calendar-live-v26-broker.js";
import {
  CALENDAR_LIVE_V26_CAPTURE_COUNT,
  assertCalendarLiveV26CaptureResponses,
  ...
  evaluateCalendarLiveV26Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV26Roots,
  validateCalendarLiveV26CaptureManifest,
  validateCalendarLiveV26CapturePayload,
  validateCalendarLiveV26CleanupPayload,
  validateCalendarLiveV26ExtractPayload,
  validateCalendarLiveV26ProbePayload,
  validateCalendarLiveV26WriterPayload,
  type CalendarLiveV26CaptureCell,
  type CalendarLiveV26CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V26_PLAN_PATH,
  type CalendarLiveV26ProofPlan,
} from "./build-calendar-live-proof-v26.js";
import {
...
export const CALENDAR_LIVE_V26_RUNNER_VERSION = "calendar-live-v26-runner-v1";

interface CalendarLiveV26JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV26OrchestratorState {
  artifactVersion: "calendar-live-v26-orchestrator-state-v1";
  ...
  journal: CalendarLiveV26JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV26ProofPlan;
  planSha256: string;
  captures: CalendarLiveV26CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV26SourceIdentity & {
...
export interface CalendarLiveV26NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV26OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v26 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V26_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V26_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV26ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV26CaptureCell[];
  validateCalendarLiveV26CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v26 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v26 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV26WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV26RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV26Request =>
  readJson<CalendarLiveV26Request>(file);

export class CalendarLiveV26Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV26Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV26OrchestratorState = {
      artifactVersion: "calendar-live-v26-orchestrator-state-v1",
    ...
    return new CalendarLiveV26Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV26OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV26Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV26NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV26NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v26 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV26NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v26 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV26RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV26Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV26CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV26AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV26WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV26Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV26ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV26Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV26Response(this.#directory, "writer", {
        validate: validateCalendarLiveV26WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV26Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV26RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV26Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV26ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV26Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV26RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v26-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV26Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV26ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV26Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV26ProbePayload(payload, ownership),
      });
      persistCalendarLiveV26TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v26 capture cell missing");
      const accepted = acceptCalendarLiveV26Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV26CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V26_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV26CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV26Objective(
    ...
    throw new Error(`Calendar live v26 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV26Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV26CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV26Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV26RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV26Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV26CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV26CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV26CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV26Authorization = (
  privateKey: KeyObject,
): CalendarLiveV26TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV26SimulationKey = (): KeyObject =>
