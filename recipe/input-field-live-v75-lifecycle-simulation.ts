...
export interface InputLiveV75LifecycleSimulationReport {
  artifactVersion: "input-live-v75-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV75AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV75Lifecycle(): Promise<InputLiveV75LifecycleSimulationReport> {
  await buildInputLiveV75Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V75_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV75AntecedentIndex;
  const indexSha256 = inputLiveV75Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v75-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V75_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V75_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV75History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV75History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV75AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V75_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V75_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV75History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV75History(authorized, "pending");
    ...
    await buildInputLiveV75Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V75_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV75AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV75Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV75Lifecycle(), null, 2)}\n`,
  );
