...
import { INPUT_LIVE_V69_INDEX_PATH } from "./build-input-field-live-proof-v69.js";
import {
  INPUT_LIVE_V69_DYNAMIC_TOOL,
  INPUT_LIVE_V69_TARGET,
} from "./input-field-live-v69-broker.js";
import {
  buildInputLiveV69AuthorizationArtifact,
  validateInputLiveV69AntecedentIndex,
  validateInputLiveV69AuthorizationArtifact,
  validateInputLiveV69History,
  validateInputLiveV69UnitTestSource,
  type InputLiveV69AntecedentIndex,
  type InputLiveV69AuthorizationArtifact,
  type InputLiveV69AuthorizationProof,
  type InputLiveV69HistoryState,
} from "./input-field-live-v69-authorization.js";
import {
  inputLiveV69AttestationSha256,
  validateInputLiveV69ControlFlowSource,
  validateInputLiveV69SecurityAttestation,
  type InputLiveV69SecurityAttestation,
} from "./input-field-live-v69-preflight.js";
import { inputLiveV69Sha256 } from "./input-field-live-v69-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V69_INDEX_PATH);
const index = (): InputLiveV69AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV69AntecedentIndex;

const artifact = (): InputLiveV69AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV69AuthorizationArtifact({
...
const pendingState = (): InputLiveV69HistoryState => ({
  index: index(),
  indexSha256: inputLiveV69Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV69HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV69AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV69Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V69_TARGET,
  expectedDynamicTool: INPUT_LIVE_V69_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v69/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV69AuthorizationArtifact,
): InputLiveV69SecurityAttestation => {
  const body: Omit<InputLiveV69SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v69-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V69_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV69AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV69History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV69History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV69History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV69History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV69History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV69UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV69UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV69AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV69Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV69AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV69AuthorizationArtifact(
        planted as InputLiveV69AuthorizationArtifact,
        index(),
        inputLiveV69Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV69SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV69AttestationSha256(
      body as Omit<InputLiveV69SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV69SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV69ControlFlowSource(source), []);
});
