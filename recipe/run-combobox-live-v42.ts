...
import {
  acceptComboboxLiveV42Response,
  createComboboxLiveV42Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV42Request,
  persistComboboxLiveV42RawResponse,
  persistComboboxLiveV42TechnicalGates,
  readComboboxLiveV42AcceptedRawPayload,
  type ComboboxLiveV42Request,
  type ComboboxLiveV42TransactionAuthorization,
} from "./combobox-live-v42-broker.js";
import {
  COMBOBOX_LIVE_V42_CAPTURE_COUNT,
  assertComboboxLiveV42CaptureResponses,
  ...
  evaluateComboboxLiveV42Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV42Roots,
  validateComboboxLiveV42CaptureManifest,
  validateComboboxLiveV42CapturePayload,
  validateComboboxLiveV42CleanupPayload,
  validateComboboxLiveV42ExtractPayload,
  validateComboboxLiveV42ProbePayload,
  validateComboboxLiveV42WriterPayload,
  type ComboboxLiveV42CaptureCell,
  type ComboboxLiveV42CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V42_PLAN_PATH,
  type ComboboxLiveV42ProofPlan,
} from "./build-combobox-live-proof-v42.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v42.js";

export const COMBOBOX_LIVE_V42_RUNNER_VERSION = "combobox-live-v42-runner-v1";

interface ComboboxLiveV42JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV42OrchestratorState {
  artifactVersion: "combobox-live-v42-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV42JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV42ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV42CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV42SourceIdentity & {
...
export interface ComboboxLiveV42NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV42OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v42 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V42_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V42_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV42ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV42CaptureCell[];
  validateComboboxLiveV42CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v42 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v42 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV42WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV42RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV42Request =>
  readJson<ComboboxLiveV42Request>(file);

export class ComboboxLiveV42Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV42Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV42OrchestratorState = {
      artifactVersion: "combobox-live-v42-orchestrator-state-v1",
    ...
    return new ComboboxLiveV42Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV42OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV42Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV42NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV42NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v42 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV42NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v42 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV42RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV42Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV42CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV42AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV42WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV42Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV42ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV42Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV42Response(this.#directory, "writer", {
        validate: validateComboboxLiveV42WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV42Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV42RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV42Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV42ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV42Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV42RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v42-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV42Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV42ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV42Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV42ProbePayload(payload, ownership),
      });
      persistComboboxLiveV42TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v42 capture cell missing");
      const accepted = acceptComboboxLiveV42Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV42CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V42_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV42CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV42Objective(
    ...
    throw new Error(`Combobox live v42 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV42Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV42CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV42Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV42RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV42Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV42CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV42CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV42CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV42Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV42TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV42SimulationKey = (): KeyObject =>
