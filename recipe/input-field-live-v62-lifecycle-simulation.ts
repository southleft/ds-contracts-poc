...
import {
  buildInputLiveV62Proof,
  INPUT_LIVE_V62_AUTHORIZATION_PATH,
  INPUT_LIVE_V62_INDEX_PATH,
} from "./build-input-field-live-proof-v62.js";
import {
  buildInputLiveV62AuthorizationArtifact,
  validateInputLiveV62History,
  type InputLiveV62AntecedentIndex,
  type InputLiveV62HistoryState,
} from "./input-field-live-v62-authorization.js";
import { inputLiveV62Sha256 } from "./input-field-live-v62-broker.js";

export interface InputLiveV62LifecycleSimulationReport {
  artifactVersion: "input-live-v62-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV62AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV62Lifecycle(): Promise<InputLiveV62LifecycleSimulationReport> {
  await buildInputLiveV62Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V62_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV62AntecedentIndex;
  const indexSha256 = inputLiveV62Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v62-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V62_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V62_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV62History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV62History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV62AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V62_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V62_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV62History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV62History(authorized, "pending");
    ...
    await buildInputLiveV62Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V62_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV62AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV62Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV62Lifecycle(), null, 2)}\n`,
  );
