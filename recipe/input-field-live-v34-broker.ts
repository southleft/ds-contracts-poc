...
import {
  ...
  INPUT_LIVE_V34_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V34_RESPONSE_CONTRACTS,
  type InputLiveV34RootProof,
} from "./input-field-live-v34-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V34_BROKER_VERSION = "input-live-v34-broker-v2";
export const INPUT_LIVE_V34_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V34_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V34_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V34_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V34_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V34_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV34RemotePhase =
  (typeof INPUT_LIVE_V34_REMOTE_PHASES)[number];
export type InputLiveV34AuthorizationMode = "live" | "simulated";

export interface InputLiveV34CommitPins {
...
export interface InputLiveV34TransactionAuthorization extends InputLiveV34CommitPins {
  mode: InputLiveV34AuthorizationMode;
...
export interface InputLiveV34Manifest {
  artifactVersion: "input-live-v34-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V34_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V34_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V34_TARGET;
...
export interface InputLiveV34ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV34RequestBody {
  artifactVersion: "input-live-v34-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V34_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV34RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV34CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V34_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV34ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV34Request extends InputLiveV34RequestBody {
...
export interface InputLiveV34Receipt {
  artifactVersion: "input-live-v34-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V34_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV34RemotePhase;
...
export interface InputLiveV34AcceptedResponse<T = unknown> {
  request: InputLiveV34Request;
  receipt: InputLiveV34Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV34IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV34ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V34_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V34_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V34_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V34_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V34_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V34_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V34_TARGET.editorType;
};
...
export const inputLiveV34RequestSequence = (
  phase: InputLiveV34RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV34RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV34RequestPath = (
  directory: string,
  phase: InputLiveV34RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV34ResponsePath = (
  directory: string,
  phase: InputLiveV34RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV34ReceiptPath = (
  directory: string,
  phase: InputLiveV34RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV34Manifest => {
  const manifest = json<InputLiveV34Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v34-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V34_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V34_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV34RemotePhase,
  captureIndex?: number,
): InputLiveV34Request | undefined => {
  const file = inputLiveV34RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV34Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV34RemotePhase,
  captureIndex?: number,
): InputLiveV34Receipt | undefined => {
  const file = inputLiveV34ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV34Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV34Request | undefined;
  receipt: InputLiveV34Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV34RemotePhase,
): InputLiveV34ResponseContract => ({
  ...INPUT_LIVE_V34_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V34_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V34_DYNAMIC_TOOL,
      target: INPUT_LIVE_V34_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v34-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V34_BROKER_VERSION,
  target: INPUT_LIVE_V34_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V34_PROGRAM_START*/${program}/*INPUT_LIVE_V34_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V34_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V34_PROGRAM_END*/";
...
export function createInputLiveV34Transaction(
  directory: string,
  ...
  const manifest: InputLiveV34Manifest = {
    artifactVersion: "input-live-v34-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V34_BROKER_VERSION,
...
export function validateInputLiveV34Request(
  directory: string,
  request: InputLiveV34Request,
): void {
  ...
  const expectedSequence = inputLiveV34RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v34-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V34_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V34_TARGET.fileKey ||
...
export function issueInputLiveV34Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV34RemotePhase,
  program: string,
  options: InputLiveV34IssueOptions = {},
): InputLiveV34Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV34RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV34Request>(targetPath);
    validateInputLiveV34Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV34ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV34TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV34RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V34_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V34_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V34_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV34RequestBody = {
    artifactVersion: "input-live-v34-broker-request-v2",
    brokerVersion: INPUT_LIVE_V34_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV34Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV34Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV34RawResponse(
  directory: string,
  phase: InputLiveV34RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV34RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV34Request>(requestFile);
  if (
  ...
  const target = inputLiveV34ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV34Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v34-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V34_BROKER_VERSION,
...
export function acceptInputLiveV34Response<T = unknown>(
  directory: string,
  phase: InputLiveV34RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV34AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV34ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV34Request>(
    inputLiveV34RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV34Request(directory, request);
  if (phase === "capture") validateInputLiveV34TechnicalGates(directory);
  const rawFile = inputLiveV34ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v34-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V34_BROKER_VERSION ||
...
export function persistInputLiveV34TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV34RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV34RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V34_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v34-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V34_BROKER_VERSION,
...
export function validateInputLiveV34TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v34-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V34_TECHNICAL_GATES.some(
...
export function validateInputLiveV34Receipt(
  directory: string,
  phase: InputLiveV34RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV34Request(directory, request);
  const receipt = json<InputLiveV34Receipt>(
    inputLiveV34ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV34ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v34-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V34_BROKER_VERSION ||
...
export function readInputLiveV34AcceptedRawPayload(
  directory: string,
  phase: InputLiveV34RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV34Receipt(directory, phase, captureIndex);
...
export function createInputLiveV34FakeDynamicResponse(
  request: InputLiveV34Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V34_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V34_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV34PersistedRequests(
  directory: string,
): InputLiveV34Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV34Request>(file)] : [];
    });
}

export const inputLiveV34Sha256 = sha256;
