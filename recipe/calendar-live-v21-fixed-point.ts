/**
 * Calendar live v21 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V21_FIXED_VERIFIER_VERSION = "calendar-live-v21-fixed-point-v1";
export const CALENDAR_LIVE_V21_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV21FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV21FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV21FixedSourceProbe {
...
export interface CalendarLiveV21FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV21FixedFixedPoint;
}

export interface CalendarLiveV21FixedHardGateInput {
  thresholds: CalendarLiveV21FixedThresholds;
  sourceProbes: CalendarLiveV21FixedSourceProbe[];
  cells: CalendarLiveV21FixedCellValidation[];
  sceneProofs: CalendarLiveV21FixedSceneProof[];
...
export interface CalendarLiveV21FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V21_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV21FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV21FixedHardGates(
  input: CalendarLiveV21FixedHardGateInput,
): CalendarLiveV21FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV21FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V21_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV21FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV21FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV21FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
