...
import {
  assertInputLiveV59RootProofs,
  INPUT_LIVE_V59_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V59_RESPONSE_CONTRACTS,
  type InputLiveV59RootProof,
} from "./input-field-live-v59-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V59_BROKER_VERSION = "input-live-v59-broker-v2";
export const INPUT_LIVE_V59_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V59_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V59_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V59_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V59_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V59_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV59RemotePhase =
  (typeof INPUT_LIVE_V59_REMOTE_PHASES)[number];
export type InputLiveV59AuthorizationMode = "live" | "simulated";

export interface InputLiveV59CommitPins {
...
export interface InputLiveV59TransactionAuthorization extends InputLiveV59CommitPins {
  mode: InputLiveV59AuthorizationMode;
...
export interface InputLiveV59Manifest {
  artifactVersion: "input-live-v59-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V59_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V59_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V59_TARGET;
  authorization: InputLiveV59TransactionAuthorization;
...
export interface InputLiveV59ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV59RequestBody {
  artifactVersion: "input-live-v59-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V59_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV59RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV59CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V59_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV59ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV59Request extends InputLiveV59RequestBody {
...
export interface InputLiveV59Receipt {
  artifactVersion: "input-live-v59-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V59_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV59RemotePhase;
...
export interface InputLiveV59AcceptedResponse<T = unknown> {
  request: InputLiveV59Request;
  receipt: InputLiveV59Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV59IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV59ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V59_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V59_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V59_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V59_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V59_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V59_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V59_TARGET.editorType;
};
...
export const inputLiveV59RequestSequence = (
  phase: InputLiveV59RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV59RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV59RequestPath = (
  directory: string,
  phase: InputLiveV59RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV59ResponsePath = (
  directory: string,
  phase: InputLiveV59RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV59ReceiptPath = (
  directory: string,
  phase: InputLiveV59RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV59Manifest => {
  const manifest = json<InputLiveV59Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v59-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V59_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V59_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV59RemotePhase,
  captureIndex?: number,
): InputLiveV59Request | undefined => {
  const file = inputLiveV59RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV59Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV59RemotePhase,
  captureIndex?: number,
): InputLiveV59Receipt | undefined => {
  const file = inputLiveV59ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV59Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV59Request | undefined;
  receipt: InputLiveV59Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV59RemotePhase,
): InputLiveV59ResponseContract => ({
  ...INPUT_LIVE_V59_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V59_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V59_DYNAMIC_TOOL,
      target: INPUT_LIVE_V59_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v59-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V59_BROKER_VERSION,
  target: INPUT_LIVE_V59_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V59_PROGRAM_START*/${program}/*INPUT_LIVE_V59_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V59_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V59_PROGRAM_END*/";
...
export function createInputLiveV59Transaction(
  directory: string,
  ...
  const manifest: InputLiveV59Manifest = {
    artifactVersion: "input-live-v59-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V59_BROKER_VERSION,
...
export function validateInputLiveV59Request(
  directory: string,
  request: InputLiveV59Request,
): void {
  ...
  const expectedSequence = inputLiveV59RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v59-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V59_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V59_TARGET.fileKey ||
...
export function issueInputLiveV59Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV59RemotePhase,
  program: string,
  options: InputLiveV59IssueOptions = {},
): InputLiveV59Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV59RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV59Request>(targetPath);
    validateInputLiveV59Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV59ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV59TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV59RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V59_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V59_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V59_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV59RequestBody = {
    artifactVersion: "input-live-v59-broker-request-v2",
    brokerVersion: INPUT_LIVE_V59_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV59Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV59Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV59RawResponse(
  directory: string,
  phase: InputLiveV59RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV59RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV59Request>(requestFile);
  if (
  ...
  const target = inputLiveV59ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV59Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v59-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V59_BROKER_VERSION,
...
export function acceptInputLiveV59Response<T = unknown>(
  directory: string,
  phase: InputLiveV59RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV59AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV59ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV59Request>(
    inputLiveV59RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV59Request(directory, request);
  if (phase === "capture") validateInputLiveV59TechnicalGates(directory);
  const rawFile = inputLiveV59ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v59-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V59_BROKER_VERSION ||
...
export function persistInputLiveV59TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV59RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV59RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V59_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v59-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V59_BROKER_VERSION,
...
export function validateInputLiveV59TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v59-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V59_TECHNICAL_GATES.some(
...
export function validateInputLiveV59Receipt(
  directory: string,
  phase: InputLiveV59RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV59Request(directory, request);
  const receipt = json<InputLiveV59Receipt>(
    inputLiveV59ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV59ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v59-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V59_BROKER_VERSION ||
...
export function readInputLiveV59AcceptedRawPayload(
  directory: string,
  phase: InputLiveV59RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV59Receipt(directory, phase, captureIndex);
...
export function createInputLiveV59FakeDynamicResponse(
  request: InputLiveV59Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V59_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V59_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV59PersistedRequests(
  directory: string,
): InputLiveV59Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV59Request>(file)] : [];
    });
}

export const inputLiveV59Sha256 = sha256;
