...
import {
  ...
  verifyInputLiveV13Authorization,
  type InputLiveV13AntecedentIndex,
  type InputLiveV13AuthorizationProof,
} from "./input-field-live-v13-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V13_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v13/operator-security-attestation-template.json";
export const INPUT_LIVE_V13_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v13-security-attestation.json";

export interface InputLiveV13SecurityAttestation {
...
export interface InputLiveV13PreflightReport {
  artifactVersion: "input-live-v13-preflight-v1";
...
export const inputLiveV13AttestationSha256 = (
  value: Omit<InputLiveV13SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV13SecurityAttestation(
  value: unknown,
  proof: InputLiveV13AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v13-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V13_TARGET) ||
...
export function validateInputLiveV13ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV13Authorization");
  const preflight = source.indexOf("runInputLiveV13Preflight");
  const initialize = source.indexOf("InputLiveV13Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV13Preflight(
  root: string,
  proof: InputLiveV13AuthorizationProof,
  attestationPath = INPUT_LIVE_V13_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV13PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V13_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V13_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V13_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV13AntecedentIndex;
  failures.push(...validateInputLiveV13AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V13_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V13_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v13-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV13ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV13SecurityAttestation;
  return {
    artifactVersion: "input-live-v13-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V13_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV13Preflight(
    process.cwd(),
    verifyInputLiveV13Authorization(),
    attestationPath,
