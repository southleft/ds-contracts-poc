...
import {
  ...
  verifyTableLiveV16Authorization,
  type TableLiveV16AntecedentIndex,
  type TableLiveV16AuthorizationProof,
} from "./table-live-v16-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V16_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v16/operator-security-attestation-template.json";
export const TABLE_LIVE_V16_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v16-security-attestation.json";

export interface TableLiveV16SecurityAttestation {
...
export interface TableLiveV16PreflightReport {
  artifactVersion: "table-live-v16-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV16SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV16SecurityAttestation(
  value: unknown,
  proof: TableLiveV16AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v16-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V16_TARGET) ||
...
export function validateTableLiveV16ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV16Authorization");
  const preflight = source.indexOf("runTableLiveV16Preflight");
  const initialize = source.indexOf("TableLiveV16Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV16Preflight(
  root: string,
  proof: TableLiveV16AuthorizationProof,
  attestationPath = TABLE_LIVE_V16_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV16PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V16_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V16_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV16AntecedentIndex;
  failures.push(...validateTableLiveV16AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V16_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V16_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v16-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV16ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v16 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV16SecurityAttestation;
  return {
    artifactVersion: "table-live-v16-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V16_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV16Preflight(
    process.cwd(),
    verifyTableLiveV16Authorization(),
    attestationPath,
