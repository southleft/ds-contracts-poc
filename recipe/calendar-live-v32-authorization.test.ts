...
import { CALENDAR_LIVE_V32_INDEX_PATH } from "./build-calendar-live-proof-v32.js";
import {
...
import {
  buildCalendarLiveV32AuthorizationArtifact,
  validateCalendarLiveV32AntecedentIndex,
  validateCalendarLiveV32AuthorizationArtifact,
  validateCalendarLiveV32History,
  validateCalendarLiveV32UnitTestSource,
  type CalendarLiveV32AntecedentIndex,
  type CalendarLiveV32AuthorizationArtifact,
  type CalendarLiveV32AuthorizationProof,
  type CalendarLiveV32HistoryState,
} from "./calendar-live-v32-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV32ControlFlowSource,
  validateCalendarLiveV32SecurityAttestation,
  type CalendarLiveV32SecurityAttestation,
} from "./calendar-live-v32-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V32_INDEX_PATH);
const index = (): CalendarLiveV32AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV32AntecedentIndex;

const artifact = (): CalendarLiveV32AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV32AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV32HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV32HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV32AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v32/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV32AuthorizationArtifact,
): CalendarLiveV32SecurityAttestation => {
  const body: Omit<CalendarLiveV32SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V32_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV32History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV32History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV32History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV32History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV32History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV32UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v32-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV32UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV32AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV32AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV32AuthorizationArtifact(
        planted as CalendarLiveV32AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV32SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV32SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV32SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v32-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV32ControlFlowSource(source), []);
});
