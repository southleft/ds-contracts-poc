...
import { chakraAvatarAdapterConfig as chakraAvatarProposedConfig, chakraAvatarSource as chakraAvatarProposedSource } from "./fixtures/generated/avatar.chakra.js";
...
const EVIDENCE = "recipe/evidence/avatar-live-pivot-v7";
...
const sources = [
  ...
  {
    // PROPOSED from a capture made today: Chakra Avatar, captured today from a config entry authored as the person's step.
    adapterIdentity: "chakra-avatar-proposed-v1",
    displayName: "Chakra (proposed)",
    source: chakraAvatarProposedSource,
    config: chakraAvatarProposedConfig,
  },
].map((entry) => {
...
const receiptOwned = {
      artifactVersion: "avatar-live-pivot-v7-prepare",
      teaching:
...
if (process.argv.includes("--check")) {
  ...
  if (writer.sourcePlans.length !== 9)
    throw new Error("avatar prepare requires 9 library sources (reviewed + proposed)");
  console.log(
    JSON.stringify({
      check: "ok",
      runIdentity: writer.runIdentity,
