...
import {
  validateComboboxLiveV6AntecedentIndex,
  verifyComboboxLiveV6Authorization,
  type ComboboxLiveV6AntecedentIndex,
  type ComboboxLiveV6AuthorizationProof,
} from "./combobox-live-v6-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V6_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v6/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V6_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v6-security-attestation.json";

export interface ComboboxLiveV6SecurityAttestation {
  artifactVersion: "combobox-live-v6-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV6PreflightReport {
  artifactVersion: "combobox-live-v6-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV6SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV6SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV6AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v6-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V6_TARGET) ||
...
export function validateComboboxLiveV6ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV6Authorization");
  const preflight = source.indexOf("runComboboxLiveV6Preflight");
  const initialize = source.indexOf("ComboboxLiveV6Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV6Preflight(
  root: string,
  proof: ComboboxLiveV6AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V6_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV6PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V6_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V6_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V6_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV6AntecedentIndex;
  failures.push(...validateComboboxLiveV6AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V6_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V6_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV6SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v6-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV6ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v6 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV6SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v6-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V6_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV6Preflight(
    process.cwd(),
    verifyComboboxLiveV6Authorization(),
    attestationPath,
