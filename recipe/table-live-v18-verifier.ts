...
import {
  verifyTableLiveV18FixedSceneFixedPoint,
  type TableLiveV18FixedFixedPoint,
} from "./table-live-v18-fixed-point.js";

export const TABLE_LIVE_V18_VERIFIER_VERSION = "table-live-v18-verifier-v1";

export function buildTableLiveV18RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV18RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV18RawNode[];
}

export interface TableLiveV18NormalizedScene {
...
export interface TableLiveV18SceneProof {
...
export function normalizeTableLiveV18Scene(
  root: TableLiveV18RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV18NormalizedScene {
  const canonicalBindings: TableLiveV18NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV18RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV18Scene<Instance>(
  normalized: TableLiveV18NormalizedScene,
...
): TableLiveV18SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV18FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV18ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV18PreCaptureGates(payload: unknown): void {
