/**
 * Table live v31 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V31_FIXED_VERIFIER_VERSION = "table-live-v31-fixed-point-v1";
export const TABLE_LIVE_V31_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV31FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV31FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV31FixedSourceProbe {
...
export interface TableLiveV31FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV31FixedFixedPoint;
}

export interface TableLiveV31FixedHardGateInput {
  thresholds: TableLiveV31FixedThresholds;
  sourceProbes: TableLiveV31FixedSourceProbe[];
  cells: TableLiveV31FixedCellValidation[];
  sceneProofs: TableLiveV31FixedSceneProof[];
...
export interface TableLiveV31FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V31_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV31FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV31FixedHardGates(
  input: TableLiveV31FixedHardGateInput,
): TableLiveV31FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV31FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V31_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV31FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV31FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
