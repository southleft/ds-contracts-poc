/**
 * Calendar live v47 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V47_FIXED_VERIFIER_VERSION = "calendar-live-v47-fixed-point-v1";
export const CALENDAR_LIVE_V47_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV47FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV47FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV47FixedSourceProbe {
...
export interface CalendarLiveV47FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV47FixedFixedPoint;
}

export interface CalendarLiveV47FixedHardGateInput {
  thresholds: CalendarLiveV47FixedThresholds;
  sourceProbes: CalendarLiveV47FixedSourceProbe[];
  cells: CalendarLiveV47FixedCellValidation[];
  sceneProofs: CalendarLiveV47FixedSceneProof[];
...
export interface CalendarLiveV47FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V47_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV47FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV47FixedHardGates(
  input: CalendarLiveV47FixedHardGateInput,
): CalendarLiveV47FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV47FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V47_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV47FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV47FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV47FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
