...
import {
  acceptCalendarLiveV34Response,
  createCalendarLiveV34Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV34Request,
  persistCalendarLiveV34RawResponse,
  persistCalendarLiveV34TechnicalGates,
  readCalendarLiveV34AcceptedRawPayload,
  type CalendarLiveV34Request,
  type CalendarLiveV34TransactionAuthorization,
} from "./calendar-live-v34-broker.js";
import {
  CALENDAR_LIVE_V34_CAPTURE_COUNT,
  assertCalendarLiveV34CaptureResponses,
  ...
  evaluateCalendarLiveV34Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV34Roots,
  validateCalendarLiveV34CaptureManifest,
  validateCalendarLiveV34CapturePayload,
  validateCalendarLiveV34CleanupPayload,
  validateCalendarLiveV34ExtractPayload,
  validateCalendarLiveV34ProbePayload,
  validateCalendarLiveV34WriterPayload,
  type CalendarLiveV34CaptureCell,
  type CalendarLiveV34CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V34_PLAN_PATH,
  type CalendarLiveV34ProofPlan,
} from "./build-calendar-live-proof-v34.js";
import {
...
export const CALENDAR_LIVE_V34_RUNNER_VERSION = "calendar-live-v34-runner-v1";

interface CalendarLiveV34JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV34OrchestratorState {
  artifactVersion: "calendar-live-v34-orchestrator-state-v1";
  ...
  journal: CalendarLiveV34JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV34ProofPlan;
  planSha256: string;
  captures: CalendarLiveV34CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV34SourceIdentity & {
...
export interface CalendarLiveV34NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV34OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v34 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V34_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V34_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV34ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV34CaptureCell[];
  validateCalendarLiveV34CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v34 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v34 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV34WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV34RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV34Request =>
  readJson<CalendarLiveV34Request>(file);

export class CalendarLiveV34Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV34Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV34OrchestratorState = {
      artifactVersion: "calendar-live-v34-orchestrator-state-v1",
    ...
    return new CalendarLiveV34Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV34OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV34Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV34NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV34NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v34 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV34NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v34 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV34RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV34Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV34CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV34AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV34WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV34Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV34ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV34Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV34Response(this.#directory, "writer", {
        validate: validateCalendarLiveV34WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV34Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV34RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV34Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV34ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV34Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV34RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v34-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV34Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV34ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV34Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV34ProbePayload(payload, ownership),
      });
      persistCalendarLiveV34TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v34 capture cell missing");
      const accepted = acceptCalendarLiveV34Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV34CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V34_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV34CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV34Objective(
    ...
    throw new Error(`Calendar live v34 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV34Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV34CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV34Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV34RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV34Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV34CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV34CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV34CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV34Authorization = (
  privateKey: KeyObject,
): CalendarLiveV34TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV34SimulationKey = (): KeyObject =>
