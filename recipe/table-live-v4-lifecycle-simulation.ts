...
import { tableLiveV2Sha256 } from "./table-live-v4-broker.js";

export interface TableLiveV4LifecycleSimulationReport {
  artifactVersion: "table-live-v4-lifecycle-simulation-v1";
...
const gitEnvironment = {
  ...process.env,
  GIT_AUTHOR_NAME: "Table v4 Lifecycle Simulation",
  GIT_AUTHOR_EMAIL: "table-v2-simulation@invalid",
  GIT_COMMITTER_NAME: "Table v4 Lifecycle Simulation",
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV4AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV4Lifecycle(): Promise<TableLiveV4LifecycleSimulationReport> {
  await buildTableLiveV4Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V4_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV4AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v4-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V4_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V4_INDEX_PATH]);
    git(temporary, ["commit", "-m", "synthetic table v4 antecedent"]);
    const antecedentCommit = git(temporary, ["rev-parse", "HEAD"]);
    const pending = historyState(index, indexSha256, antecedentCommit);
    const pendingFailures = validateTableLiveV4History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v4 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV4History(pending, "authorized");
    if (
      !staleAuthorized.some((failure) => failure.includes("stale history mode"))
    )
      throw new Error("table v4 lifecycle did not refuse stale authorized mode");

    const { publicKey } = generateKeyPairSync("ed25519");
    const authorization = buildTableLiveV4AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V4_AUTHORIZATION_PATH]);
    git(temporary, ["commit", "-m", "synthetic table v4 authorization"]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V4_INDEX_PATH}`,
    ]);
    if (!committedAntecedentBefore.equals(committedAntecedentAfter))
      throw new Error(
        "table v4 antecedent index changed across authorization commit",
      );
    ...
    const authorizedFailures = validateTableLiveV4History(
      authorized,
      "authorized",
    );
    if (authorizedFailures.length)
      throw new Error(
        `table v4 lifecycle authorized phase failed:\n${authorizedFailures.join("\n")}`,
      );
    const stalePending = validateTableLiveV4History(authorized, "pending");
    if (!stalePending.some((failure) => failure.includes("stale history mode")))
      throw new Error("table v4 lifecycle did not refuse stale pending mode");

    await buildTableLiveV4Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V4_INDEX_PATH);
    if (!indexBefore.equals(indexAfter))
      throw new Error(
        "table v4 generated check changed after synthetic authorization",
      );
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV4AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
      throw new Error("table v4 antecedent hash set changed after authorization");
    return {
      artifactVersion: "table-live-v4-lifecycle-simulation-v1",
      syntheticGitCommits: 2,
      antecedentCheckBeforeAuthorization: true,
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV4Lifecycle(), null, 2)}\n`,
  );
