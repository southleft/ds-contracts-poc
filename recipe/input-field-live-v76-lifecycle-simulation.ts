...
export interface InputLiveV76LifecycleSimulationReport {
  artifactVersion: "input-live-v76-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV76AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV76Lifecycle(): Promise<InputLiveV76LifecycleSimulationReport> {
  await buildInputLiveV76Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V76_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV76AntecedentIndex;
  const indexSha256 = inputLiveV76Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v76-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V76_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V76_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV76History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV76History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV76AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V76_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V76_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV76History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV76History(authorized, "pending");
    ...
    await buildInputLiveV76Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V76_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV76AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV76Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV76Lifecycle(), null, 2)}\n`,
  );
