...
import { COMBOBOX_LIVE_V41_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v41.js";
import { verifyComboboxLiveV41Authorization } from "./combobox-live-v41-authorization.js";
import { runComboboxLiveV41Preflight } from "./combobox-live-v41-preflight.js";
import { ComboboxLiveV41Orchestrator } from "./run-combobox-live-v41.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v41 requires exact authorization artifact ${COMBOBOX_LIVE_V41_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v41-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV41Authorization(root);
    runComboboxLiveV41Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV41Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV41Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
