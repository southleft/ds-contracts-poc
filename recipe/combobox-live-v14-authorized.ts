...
import { COMBOBOX_LIVE_V14_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v14.js";
import { verifyComboboxLiveV14Authorization } from "./combobox-live-v14-authorization.js";
import { runComboboxLiveV14Preflight } from "./combobox-live-v14-preflight.js";
import { ComboboxLiveV14Orchestrator } from "./run-combobox-live-v14.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v14 requires exact authorization artifact ${COMBOBOX_LIVE_V14_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v14-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV14Authorization(root);
    runComboboxLiveV14Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV14Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV14Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
