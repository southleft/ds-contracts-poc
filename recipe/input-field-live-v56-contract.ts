...
} from "./input-field-live-v3-verifier-v56.js";
import {
  buildInputLiveV56RawPropertyRuntime,
  normalizeInputLiveV56Scene,
  type InputLiveV56RawNode,
} from "./input-field-live-v56-verifier.js";
...
export const INPUT_LIVE_V56_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V56_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V56_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V56_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V56_VARIANT_COUNT = 256;
export const INPUT_LIVE_V56_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V56_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV56SourceId = (typeof INPUT_LIVE_V56_SOURCE_IDS)[number];

export interface InputLiveV56SourceIdentity {
  source: InputLiveV56SourceId;
...
export interface InputLiveV56WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV56ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV56SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV56RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV56RootProof {
  source: InputLiveV56SourceId;
  adapterIdentity: string;
...
export interface InputLiveV56ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV56SourceId;
    adapterIdentity: string;
...
export interface InputLiveV56CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV56SourceId;
  adapterIdentity: string;
...
export interface InputLiveV56CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV56SourceId;
  strata: InputLiveV56CaptureCell["strata"];
...
export interface InputLiveV56CleanupPayload {
...
export function validateInputLiveV56WriterPayload(
  value: unknown,
): InputLiveV56WriterOwnership {
...
const identityPlan = (source: InputLiveV56SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV56ExtractProgram(
  writer: InputLiveV56WriterOwnership,
  sources: readonly InputLiveV56SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V56_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V56_NAMESPACE)}
${buildInputLiveV56RawPropertyRuntime()}
...
export function validateInputLiveV56ExtractPayload(
  value: unknown,
  writer: InputLiveV56WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V56_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV56ExtractPayload;
}

export function proveInputLiveV56Roots<Instance>(
  extract: InputLiveV56ExtractPayload,
  sources: readonly (InputLiveV56SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV56Scene(
      root.scene,
...
export function assertInputLiveV56RootProofs(
  proofs: readonly InputLiveV56RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV56ProbeProgram(
  writer: InputLiveV56WriterOwnership,
  sources: readonly InputLiveV56SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V56_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV56ProbePayload(
  value: unknown,
  writer: InputLiveV56WriterOwnership,
): InputLiveV56ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV56ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V56_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V56_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV56CaptureManifest(
  cells: readonly InputLiveV56CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V56_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V56_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V56_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V56_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V56_SOURCE_IDS.some(
...
export const inputLiveV56CaptureManifestSha256 = (
  cells: readonly InputLiveV56CaptureCell[],
): string => {
  validateInputLiveV56CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV56CaptureProgram(
  writer: InputLiveV56WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V56_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V56_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V56_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV56CapturePayload(
  value: unknown,
  cell: InputLiveV56CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV56CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V56_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V56_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV56CaptureResponses(
  manifest: readonly InputLiveV56CaptureCell[],
  responses: readonly InputLiveV56CapturePayload[],
): void {
  validateInputLiveV56CaptureManifest(manifest);
  if (
...
export interface InputLiveV56ObjectiveReport {
  artifactVersion: "input-live-v56-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV56Objective(
  manifest: readonly InputLiveV56CaptureCell[],
  responses: readonly InputLiveV56CapturePayload[],
  readReference: (cell: InputLiveV56CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV56CaptureCell) => Buffer,
): InputLiveV56ObjectiveReport {
  assertInputLiveV56CaptureResponses(manifest, responses);
...
export function buildInputLiveV56CleanupProgram(
  writer: InputLiveV56WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V56_NAMESPACE,
...
export function validateInputLiveV56CleanupPayload(
  value: unknown,
  writer: InputLiveV56WriterOwnership,
): InputLiveV56CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV56CleanupPayload;
...
export const INPUT_LIVE_V56_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV56WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
