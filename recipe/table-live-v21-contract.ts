...
import { buildTableLiveV21CleanupRuntime } from "./table-live-v21-cleanup.js";
import { type TableLiveV21FixedFixedPoint } from "./table-live-v21-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV21RestoreProgram,
  validateTableLiveV21RestorePayload,
  type TableLiveV21RestorePayload,
} from "./table-live-v21-restore.js";
import {
  buildTableLiveV21RawPropertyRuntime,
  normalizeTableLiveV21Scene,
  type TableLiveV21RawNode,
} from "./table-live-v21-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v21.js";
import {
...
export const TABLE_LIVE_V21_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V21_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V21_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V21_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V21_VARIANT_COUNT = 20;
export const TABLE_LIVE_V21_SET_COUNT = 6;
export const TABLE_LIVE_V21_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V21_HOST_PHASES = 3;
export const TABLE_LIVE_V21_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V21_RESTORE_COUNT = 8;
export const TABLE_LIVE_V21_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V21_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV21SourceId = (typeof TABLE_LIVE_V21_SOURCE_IDS)[number];
export type TableLiveV21Kind = "table" | "row" | "cell";

export interface TableLiveV21SourceIdentity {
  source: TableLiveV21SourceId;
...
export interface TableLiveV21WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV21ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV21SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV21RawNode;
    rowScene: TableLiveV21RawNode;
    cellScene: TableLiveV21RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV21RootProof {
  source: TableLiveV21SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV21FixedFixedPoint;
}

export interface TableLiveV21ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV21SourceId;
    adapterIdentity: string;
...
export interface TableLiveV21CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV21SourceId;
  adapterIdentity: string;
...
export interface TableLiveV21CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV21SourceId;
  frameWidth: number;
...
export interface TableLiveV21CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v21 ${label} must be an object`);
...
export function validateTableLiveV21WriterPayload(
  value: unknown,
): TableLiveV21WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V21_NAMESPACE ||
    ...
    throw new TypeError("Table live v21 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v21 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v21 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV21SourceIdentity,
  kind: TableLiveV21Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV21ExtractProgram(
  writer: TableLiveV21WriterOwnership,
  sources: readonly TableLiveV21SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v21 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V21_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V21_NAMESPACE)}
${buildTableLiveV21RawPropertyRuntime()}
...
export function validateTableLiveV21ExtractPayload(
  value: unknown,
  writer: TableLiveV21WriterOwnership,
): TableLiveV21ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v21 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V21_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v21 extract schema/two-root mismatch");
  return payload as TableLiveV21ExtractPayload;
}

export function proveTableLiveV21Roots<Instance>(
  extract: TableLiveV21ExtractPayload,
  sources: readonly (TableLiveV21SourceIdentity & {
...
): TableLiveV21RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v21 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v21 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV21Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV21Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV21Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV21FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v21 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v21 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v21 compile lost owned sets");
...
export function assertTableLiveV21RootProofs(
  proofs: readonly TableLiveV21RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v21 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v21 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV21ProbeProgram(
  writer: TableLiveV21WriterOwnership,
  sources: readonly TableLiveV21SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V21_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV21ProbePayload(
  value: unknown,
  writer: TableLiveV21WriterOwnership,
): TableLiveV21ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV21ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V21_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v21 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV21CaptureManifest(
  cells: readonly TableLiveV21CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V21_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V21_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V21_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V21_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v21 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV21CaptureCell[],
): string => {
  validateTableLiveV21CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV21CaptureProgram(
  writer: TableLiveV21WriterOwnership,
  cell: TableLiveV21CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V21_CAPTURE_COUNT
  )
    throw new TypeError("Table live v21 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v21 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V21_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV21CapturePayload(
  value: unknown,
  cell: TableLiveV21CaptureCell,
  rawResponseBytes: number,
): TableLiveV21CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV21CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V21_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V21_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v21 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV21CaptureResponses(
  manifest: readonly TableLiveV21CaptureCell[],
  responses: readonly TableLiveV21CapturePayload[],
): void {
  validateTableLiveV21CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v21 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV21ObjectiveReport {
  artifactVersion: "table-live-v21-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV21Objective(
  manifest: readonly TableLiveV21CaptureCell[],
  responses: readonly TableLiveV21CapturePayload[],
): TableLiveV21ObjectiveReport {
  assertTableLiveV21CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v21-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV21CleanupProgram(
  writer: TableLiveV21WriterOwnership,
): string {
  const baseProgram = buildTableLiveV21CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V21_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v21 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV21CleanupPayload(
  payload: unknown,
  writer: TableLiveV21WriterOwnership,
): TableLiveV21CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV21CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v21 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V21_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV21WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
