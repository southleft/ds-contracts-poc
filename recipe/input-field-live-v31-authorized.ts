...
import { INPUT_LIVE_V31_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v31.js";
import { verifyInputLiveV31Authorization } from "./input-field-live-v31-authorization.js";
import { runInputLiveV31Preflight } from "./input-field-live-v31-preflight.js";
import { InputLiveV31Orchestrator } from "./run-input-field-live-v31.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V31_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v31-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV31Authorization(root);
    runInputLiveV31Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV31Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
