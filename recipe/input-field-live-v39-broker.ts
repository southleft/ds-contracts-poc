...
import {
  assertInputLiveV39RootProofs,
  INPUT_LIVE_V39_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V39_RESPONSE_CONTRACTS,
  type InputLiveV39RootProof,
} from "./input-field-live-v39-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V39_BROKER_VERSION = "input-live-v39-broker-v2";
export const INPUT_LIVE_V39_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V39_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V39_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V39_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V39_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V39_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV39RemotePhase =
  (typeof INPUT_LIVE_V39_REMOTE_PHASES)[number];
export type InputLiveV39AuthorizationMode = "live" | "simulated";

export interface InputLiveV39CommitPins {
...
export interface InputLiveV39TransactionAuthorization extends InputLiveV39CommitPins {
  mode: InputLiveV39AuthorizationMode;
...
export interface InputLiveV39Manifest {
  artifactVersion: "input-live-v39-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V39_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V39_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V39_TARGET;
  authorization: InputLiveV39TransactionAuthorization;
...
export interface InputLiveV39ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV39RequestBody {
  artifactVersion: "input-live-v39-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V39_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV39RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV39CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V39_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV39ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV39Request extends InputLiveV39RequestBody {
...
export interface InputLiveV39Receipt {
  artifactVersion: "input-live-v39-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V39_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV39RemotePhase;
...
export interface InputLiveV39AcceptedResponse<T = unknown> {
  request: InputLiveV39Request;
  receipt: InputLiveV39Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV39IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV39ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V39_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V39_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V39_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V39_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V39_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V39_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V39_TARGET.editorType;
};
...
export const inputLiveV39RequestSequence = (
  phase: InputLiveV39RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV39RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV39RequestPath = (
  directory: string,
  phase: InputLiveV39RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV39ResponsePath = (
  directory: string,
  phase: InputLiveV39RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV39ReceiptPath = (
  directory: string,
  phase: InputLiveV39RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV39Manifest => {
  const manifest = json<InputLiveV39Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v39-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V39_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V39_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV39RemotePhase,
  captureIndex?: number,
): InputLiveV39Request | undefined => {
  const file = inputLiveV39RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV39Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV39RemotePhase,
  captureIndex?: number,
): InputLiveV39Receipt | undefined => {
  const file = inputLiveV39ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV39Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV39Request | undefined;
  receipt: InputLiveV39Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV39RemotePhase,
): InputLiveV39ResponseContract => ({
  ...INPUT_LIVE_V39_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V39_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V39_DYNAMIC_TOOL,
      target: INPUT_LIVE_V39_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v39-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V39_BROKER_VERSION,
  target: INPUT_LIVE_V39_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V39_PROGRAM_START*/${program}/*INPUT_LIVE_V39_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V39_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V39_PROGRAM_END*/";
...
export function createInputLiveV39Transaction(
  directory: string,
  ...
  const manifest: InputLiveV39Manifest = {
    artifactVersion: "input-live-v39-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V39_BROKER_VERSION,
...
export function validateInputLiveV39Request(
  directory: string,
  request: InputLiveV39Request,
): void {
  ...
  const expectedSequence = inputLiveV39RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v39-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V39_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V39_TARGET.fileKey ||
...
export function issueInputLiveV39Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV39RemotePhase,
  program: string,
  options: InputLiveV39IssueOptions = {},
): InputLiveV39Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV39RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV39Request>(targetPath);
    validateInputLiveV39Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV39ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV39TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV39RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V39_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V39_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V39_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV39RequestBody = {
    artifactVersion: "input-live-v39-broker-request-v2",
    brokerVersion: INPUT_LIVE_V39_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV39Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV39Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV39RawResponse(
  directory: string,
  phase: InputLiveV39RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV39RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV39Request>(requestFile);
  if (
  ...
  const target = inputLiveV39ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV39Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v39-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V39_BROKER_VERSION,
...
export function acceptInputLiveV39Response<T = unknown>(
  directory: string,
  phase: InputLiveV39RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV39AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV39ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV39Request>(
    inputLiveV39RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV39Request(directory, request);
  if (phase === "capture") validateInputLiveV39TechnicalGates(directory);
  const rawFile = inputLiveV39ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v39-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V39_BROKER_VERSION ||
...
export function persistInputLiveV39TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV39RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV39RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V39_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v39-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V39_BROKER_VERSION,
...
export function validateInputLiveV39TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v39-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V39_TECHNICAL_GATES.some(
...
export function validateInputLiveV39Receipt(
  directory: string,
  phase: InputLiveV39RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV39Request(directory, request);
  const receipt = json<InputLiveV39Receipt>(
    inputLiveV39ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV39ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v39-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V39_BROKER_VERSION ||
...
export function readInputLiveV39AcceptedRawPayload(
  directory: string,
  phase: InputLiveV39RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV39Receipt(directory, phase, captureIndex);
...
export function createInputLiveV39FakeDynamicResponse(
  request: InputLiveV39Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V39_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V39_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV39PersistedRequests(
  directory: string,
): InputLiveV39Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV39Request>(file)] : [];
    });
}

export const inputLiveV39Sha256 = sha256;
