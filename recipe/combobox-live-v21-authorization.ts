...
export type ComboboxLiveV21HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV21AuthorizationArtifact {
  artifactVersion: "combobox-live-v21-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V21_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV21AntecedentIndex {
...
export interface ComboboxLiveV21HistoryState {
  ...
  authorization?: ComboboxLiveV21AuthorizationArtifact;
...
export interface ComboboxLiveV21AuthorizationProof extends ComboboxLiveV21TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V21_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V21_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V21_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V21_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v21-authorization") ||
  artifactPath.includes("combobox-live-v21-preflight") ||
  artifactPath.includes("combobox-live-v21-authorized") ||
  artifactPath.includes("create-combobox-live-v21-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v21-status") ||
...
export function validateComboboxLiveV21AntecedentIndex(
  index: ComboboxLiveV21AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v21-antecedent-index-v1" ||
...
export function validateComboboxLiveV21UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV21History\(\s*["']/.test(source) ||
    /readComboboxLiveV21HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV21AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV21AuthorizationArtifact {
  ...
  const indexFailures = validateComboboxLiveV21AntecedentIndex(index);
...
export function validateComboboxLiveV21AuthorizationArtifact(
  artifact: ComboboxLiveV21AuthorizationArtifact | undefined,
  index: ComboboxLiveV21AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v21-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v21" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V21_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V21_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V21_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V21_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V21_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V21_SOURCE_ROOTS ||
  ...
  if (
    ...
    failures.push(
      "v21 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
...
export function validateComboboxLiveV21History(
  state: ComboboxLiveV21HistoryState,
  expected: ComboboxLiveV21HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV21AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV21AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV21HistoryState(
  root = process.cwd(),
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V21_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV21AuthorizationArtifact)
    : undefined;
  ...
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V21_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V21_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV21History(
  expected: ComboboxLiveV21HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV21HistoryState {
  const state = readComboboxLiveV21HistoryState(root);
  const failures = validateComboboxLiveV21History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v21 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV21Authorization(
  root = process.cwd(),
): ComboboxLiveV21AuthorizationProof {
  const state = verifyComboboxLiveV21History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V21_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v21/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v21.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V21_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V21_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V21_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV21HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV21History(expected);
  process.stdout.write(
    `Combobox live v21 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
