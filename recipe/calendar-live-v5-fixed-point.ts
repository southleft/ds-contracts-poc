/**
 * Calendar live v5 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V5_FIXED_VERIFIER_VERSION = "calendar-live-v5-fixed-point-v1";
export const CALENDAR_LIVE_V5_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV5FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV5FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV5FixedSourceProbe {
...
export interface CalendarLiveV5FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV5FixedFixedPoint;
}

export interface CalendarLiveV5FixedHardGateInput {
  thresholds: CalendarLiveV5FixedThresholds;
  sourceProbes: CalendarLiveV5FixedSourceProbe[];
  cells: CalendarLiveV5FixedCellValidation[];
  sceneProofs: CalendarLiveV5FixedSceneProof[];
...
export interface CalendarLiveV5FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V5_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV5FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV5FixedHardGates(
  input: CalendarLiveV5FixedHardGateInput,
): CalendarLiveV5FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV5FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V5_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV5FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV5FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
