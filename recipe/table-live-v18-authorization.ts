...
export type TableLiveV18HistoryExpectation = "pending" | "authorized";

export interface TableLiveV18AuthorizationArtifact {
  artifactVersion: "table-live-v18-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V18_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV18AntecedentIndex {
...
export interface TableLiveV18HistoryState {
  index?: TableLiveV18AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV18AuthorizationArtifact;
...
export interface TableLiveV18AuthorizationProof
  extends TableLiveV18TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V18_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V18_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V18_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V18_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v18-authorization") ||
  artifactPath.includes("table-live-v18-preflight") ||
  artifactPath.includes("table-live-v18-authorized") ||
  artifactPath.includes("create-table-live-v18-security-attestation") ||
  artifactPath.includes("table-live-pivot-v18-status") ||
...
export function validateTableLiveV18AntecedentIndex(
  index: TableLiveV18AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v18-antecedent-index-v1" ||
...
export function validateTableLiveV18UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV18History\(\s*["']/.test(source) ||
    /readTableLiveV18HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV18AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV18AuthorizationArtifact {
  ...
  ) as TableLiveV18AntecedentIndex;
  const indexFailures = validateTableLiveV18AntecedentIndex(index);
...
export function validateTableLiveV18AuthorizationArtifact(
  artifact: TableLiveV18AuthorizationArtifact | undefined,
  index: TableLiveV18AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v18-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v18" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V18_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V18_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V18_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V18_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V18_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V18_SOURCE_ROOTS ||
...
export function validateTableLiveV18History(
  state: TableLiveV18HistoryState,
  expected: TableLiveV18HistoryExpectation,
): string[] {
  const failures = validateTableLiveV18AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV18AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV18HistoryState(
  root = process.cwd(),
): TableLiveV18HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V18_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV18AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V18_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V18_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V18_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV18History(
  expected: TableLiveV18HistoryExpectation,
  root = process.cwd(),
): TableLiveV18HistoryState {
  const state = readTableLiveV18HistoryState(root);
  const failures = validateTableLiveV18History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v18 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV18Authorization(
  root = process.cwd(),
): TableLiveV18AuthorizationProof {
  const state = verifyTableLiveV18History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V18_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v18/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v18.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V18_TARGET,
    expectedDynamicTool: TABLE_LIVE_V18_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V18_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV18HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV18History(expected);
  process.stdout.write(
    `Table live v18 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
