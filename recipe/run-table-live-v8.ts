...
import {
  acceptTableLiveV8Response,
  createTableLiveV8Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV8Request,
  persistTableLiveV8RawResponse,
  persistTableLiveV8TechnicalGates,
  readTableLiveV8AcceptedRawPayload,
  type TableLiveV8Request,
  type TableLiveV8TransactionAuthorization,
} from "./table-live-v8-broker.js";
import {
  TABLE_LIVE_V8_CAPTURE_COUNT,
  assertTableLiveV8CaptureResponses,
  ...
  evaluateTableLiveV8Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV8Roots,
  validateTableLiveV8CaptureManifest,
  validateTableLiveV8CapturePayload,
  validateTableLiveV8CleanupPayload,
  validateTableLiveV8ExtractPayload,
  validateTableLiveV8ProbePayload,
  validateTableLiveV8WriterPayload,
  type TableLiveV8CaptureCell,
  type TableLiveV8CapturePayload,
...
import {
  ...
  TABLE_LIVE_V8_PLAN_PATH,
  type TableLiveV8ProofPlan,
...
export const TABLE_LIVE_V8_RUNNER_VERSION = "table-live-v8-runner-v1";

interface TableLiveV8JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV8OrchestratorState {
  artifactVersion: "table-live-v8-orchestrator-state-v1";
  ...
  journal: TableLiveV8JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV8ProofPlan;
  planSha256: string;
  captures: TableLiveV8CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV8SourceIdentity & {
...
export interface TableLiveV8NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV8OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v8 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V8_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V8_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV8ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV8CaptureCell[];
  validateTableLiveV8CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v8 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v8 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV8WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV8RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV8Request =>
  readJson<TableLiveV8Request>(file);

export class TableLiveV8Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV8Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV8OrchestratorState = {
      artifactVersion: "table-live-v8-orchestrator-state-v1",
    ...
    return new TableLiveV8Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV8OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV8Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV8NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV8NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v8 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV8NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v8 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV8RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV8Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV8CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV8AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV8WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV8Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV8ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV8Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV8Response(this.#directory, "writer", {
        validate: validateTableLiveV8WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV8Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV8RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV8Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV8ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV8Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV8RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v8-host-root-proofs-v1",
      ...
      const next = issueTableLiveV8Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV8ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV8Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV8ProbePayload(payload, ownership),
      });
      persistTableLiveV8TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v8 capture cell missing");
      const accepted = acceptTableLiveV8Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV8CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V8_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV8CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV8Objective(
    ...
    throw new Error(`Table live v8 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV8Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV8CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV8Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV8RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV8Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV8CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV8CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV8CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV8Authorization = (
  privateKey: KeyObject,
): TableLiveV8TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV8SimulationKey = (): KeyObject =>
