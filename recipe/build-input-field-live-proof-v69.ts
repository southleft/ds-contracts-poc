...
import {
  buildInputLiveV69CaptureProgram,
  buildInputLiveV69ExtractProgram,
  buildInputLiveV69ProbeProgram,
  buildInputLiveV69RestoreProgram,
  INPUT_LIVE_V69_CAPTURE_COUNT,
  INPUT_LIVE_V69_RESPONSE_CONTRACTS,
  inputLiveV69CaptureManifestSha256,
  type InputLiveV69CaptureCell,
  type InputLiveV69SourceIdentity,
  type InputLiveV69WriterOwnership,
} from "./input-field-live-v69-contract.js";
import {
  INPUT_LIVE_V69_DYNAMIC_TOOL,
  INPUT_LIVE_V69_TARGET,
  inputLiveV69RequestSequence,
} from "./input-field-live-v69-broker.js";
import {
...
} from "./input-field-live-v69-restore.js";

export const INPUT_LIVE_V69_EVIDENCE_ROOT =
  "recipe/evidence/input-field-live-pivot-v69";
export const INPUT_LIVE_V69_PROTOCOL_PATH = `${INPUT_LIVE_V69_EVIDENCE_ROOT}/protocol.json`;
export const INPUT_LIVE_V69_PLAN_PATH = `${INPUT_LIVE_V69_EVIDENCE_ROOT}/proof-plan.json`;
export const INPUT_LIVE_V69_CAPTURE_MANIFEST_PATH = `${INPUT_LIVE_V69_EVIDENCE_ROOT}/capture-manifest.json`;
export const INPUT_LIVE_V69_REQUEST_MANIFEST_PATH = `${INPUT_LIVE_V69_EVIDENCE_ROOT}/request-manifest.json`;
export const INPUT_LIVE_V69_INDEX_PATH = `${INPUT_LIVE_V69_EVIDENCE_ROOT}/antecedent-index.json`;
export const INPUT_LIVE_V69_AUTHORIZATION_TEMPLATE_PATH = `${INPUT_LIVE_V69_EVIDENCE_ROOT}/authorization-template.json`;
export const INPUT_LIVE_V69_AUTHORIZATION_PATH = `${INPUT_LIVE_V69_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface InputLiveV69ProofPlan {
  artifactVersion: "input-live-v69-proof-plan-v1";
...
interface GeneratedSource {
  ...
  recipeHash: string;
  envelopeHash: string;
  expectedScenePlan: InputLiveV69SourceIdentity["expectedScenePlan"];
  outputPath: string;
  compressed: Buffer;
  metadata: InputLiveV69ProofPlan["sources"][number]["expectedScenePlan"];
}
...
const sourcePlans = (): GeneratedSource[] => {
  ...
  return prior.sources.map((source: Record<string, any>) => {
    ...
    if (!rewrittenText.includes(needle))
      throw new Error(
        `Input live v69 missing set layout.mode vertical fact: ${inputPath}`,
      );
    ...
    if (!modeRewritten.includes(paddingNeedle))
      throw new Error(
        `Input live v69 missing set layout.padding 0 fact: ${inputPath}`,
      );
    ...
    if (setMode?.value !== "horizontal")
      throw new Error(
        `Input live v69 set layout.mode rewrite failed: ${inputPath}`,
      );
    ...
    if (
      ...
      throw new Error(
        `Input live v69 set layout.padding rewrite failed: ${inputPath}`,
      );
    ...
    return {
      ...
      outputPath: `${INPUT_LIVE_V69_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
      compressed: rewrittenCompressed,
      metadata: {
        path: `${INPUT_LIVE_V69_EVIDENCE_ROOT}/expected-scene-plan-${sourceId}.json.gz`,
...
const captureCells = (): InputLiveV69CaptureCell[] => {
  const cells = json<Record<string, any>>(V16_CAPTURE_MANIFEST)
    .cells as InputLiveV69CaptureCell[];
  inputLiveV69CaptureManifestSha256(cells);
  return cells;
};

const protocol = () => ({
  artifactVersion: "input-live-v69-external-operator-protocol-v1",
  protocolId: "input-live-v69",
  status:
  ...
  hostNormalization: {
    ...
    carriedSceneReadback: "recipe/scene-readback-v69.ts",
    carriedSceneReadbackRuntime: "recipe/scene-readback-runtime-v69.ts",
    carriedV3Verifier: "recipe/input-field-live-v3-verifier-v69.ts",
...
const requestManifest = (
  captures: readonly InputLiveV69CaptureCell[],
  programs: {
    writerSha256: string;
  ...
  const requests = [
    {
      requestId: "writer",
      sequence: inputLiveV69RequestSequence("writer"),
      phase: "writer",
      predecessorRequestId: null,
      programSha256: programs.writerSha256,
      response: INPUT_LIVE_V69_RESPONSE_CONTRACTS.writer,
    },
    {
      requestId: "cleanup",
      sequence: inputLiveV69RequestSequence("cleanup"),
      phase: "cleanup",
      predecessorRequestId: "writer",
      programBuilderSha256: programs.cleanupBuilderSha256,
      response: INPUT_LIVE_V69_RESPONSE_CONTRACTS.cleanup,
      availability: "persisted immediately after writer acceptance",
    },
    {
      requestId: "restore",
      sequence: inputLiveV69RequestSequence("restore"),
      phase: "restore",
      predecessorRequestId: "writer",
      programSha256: programs.restoreBlueprintSha256,
      response: INPUT_LIVE_V69_RESPONSE_CONTRACTS.restore,
      availability:
        "issued immediately after writer acceptance; before extract",
    },
    {
      requestId: "extract",
      sequence: inputLiveV69RequestSequence("extract"),
      phase: "extract",
      predecessorRequestId: "restore",
      programSha256: programs.extractBlueprintSha256,
      response: INPUT_LIVE_V69_RESPONSE_CONTRACTS.extract,
    },
    {
      requestId: "probe",
      sequence: inputLiveV69RequestSequence("probe"),
      phase: "probe",
      predecessorRequestId: "extract",
      programSha256: programs.probeBlueprintSha256,
      response: INPUT_LIVE_V69_RESPONSE_CONTRACTS.probe,
    },
    ...captures.map((cell) => ({
      requestId: `capture-${String(cell.index).padStart(3, "0")}`,
      sequence: inputLiveV69RequestSequence("capture", cell.index),
      phase: "capture",
      ...
      response: INPUT_LIVE_V69_RESPONSE_CONTRACTS.capture,
    })),
  ];
  return {
    artifactVersion: "input-live-v69-request-manifest-v1",
    expectedDynamicTool: INPUT_LIVE_V69_DYNAMIC_TOOL,
    target: INPUT_LIVE_V69_TARGET,
...
const lifecycleExcludedPaths = [
  INPUT_LIVE_V69_AUTHORIZATION_TEMPLATE_PATH,
  INPUT_LIVE_V69_AUTHORIZATION_PATH,
  "recipe/input-field-live-v69-authorization.ts",
  "recipe/input-field-live-v69-authorization.test.ts",
  "recipe/input-field-live-v69-preflight.ts",
  "recipe/input-field-live-v69-authorized.ts",
  "recipe/evidence/input-field-live-pivot-v69/operator-security-attestation-template.json",
...
const sourceNeutralDependencies = [
  ...
  "recipe/normalize.ts",
  "recipe/scene-readback-runtime-v69.ts",
  "recipe/scene-readback-v69.ts",
  "recipe/input-field-live-v69-restore.ts",
  "recipe/input-field-figma-writer-v17.ts",
] as const;

export async function buildInputLiveV69Proof(
  check = process.argv.includes("--check"),
): Promise<InputLiveV69ProofPlan> {
  ...
  if (
    ...
    throw new Error("Input live v69 frozen v16 writer bytes drifted");
  ...
  if (
    ...
    throw new Error(
      "Input live v69 exact committed v16 writer/restore/runtime/extract or hashed v16 writer or v17-v66 scene-readback bytes drifted",
    );

  const writerOwnershipBlueprint: InputLiveV69WriterOwnership = {
    pageId: "__WRITER_PAGE_ID__",
    pageName: "Recipe Pivot / Input Field / v9",
    runIdentity: "input-live-v69",
  ...
  const identities = sources.map(
    (source) =>
      ...
      }) satisfies InputLiveV69SourceIdentity,
  );
  const restoreBlueprint = Buffer.from(
    buildInputLiveV69RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildInputLiveV69ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildInputLiveV69ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildInputLiveV69CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );

  const antecedentOutputs = new Map<string, Buffer>();
  antecedentOutputs.set(INPUT_LIVE_V69_PROTOCOL_PATH, jsonBytes(protocol()));
  antecedentOutputs.set(
    `${INPUT_LIVE_V69_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V69_EVIDENCE_ROOT}/programs/writer-payload.js`,
    writerPayload,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V69_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V69_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V69_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${INPUT_LIVE_V69_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  ...
  const captureManifest = {
    artifactVersion: "input-live-v69-capture-manifest-v1",
    ...
    cells: captures,
    cellsSha256: inputLiveV69CaptureManifestSha256(captures),
  };
  const captureManifestBytes = jsonBytes(captureManifest);
  antecedentOutputs.set(
    INPUT_LIVE_V69_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: InputLiveV69ProofPlan = {
    artifactVersion: "input-live-v69-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: INPUT_LIVE_V69_TARGET,
    namespace: "ds.contracts.input.recipe.v5",
    writer: {
      programPath: `${INPUT_LIVE_V69_EVIDENCE_ROOT}/programs/writer.txt`,
      programBytes: writerProgram.byteLength,
      programSha256: sha256(writerProgram),
      payloadPath: `${INPUT_LIVE_V69_EVIDENCE_ROOT}/programs/writer-payload.js`,
    ...
    captureManifest: {
      path: INPUT_LIVE_V69_CAPTURE_MANIFEST_PATH,
      cells: 128,
      requests: 128,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(INPUT_LIVE_V69_PLAN_PATH, proofPlanBytes);
  ...
  antecedentOutputs.set(
    INPUT_LIVE_V69_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/input-field-live-v69-broker.ts",
    "recipe/input-field-live-v69-contract.ts",
    "recipe/run-input-field-live-v69.ts",
    "recipe/build-input-field-live-proof-v69.ts",
    "recipe/input-field-live-v69-broker.test.ts",
    "recipe/figma-property-normalizer-v8.test.ts",
    "recipe/scene-readback-v69.test.ts",
    "recipe/input-field-live-v69-restore.ts",
    "recipe/input-field-live-v69-restore.test.ts",
    "recipe/input-field-live-v69-lifecycle-simulation.ts",
    ...sourceNeutralDependencies,
  ].filter((artifactPath) => artifactPath !== INPUT_LIVE_V69_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "input-live-v69-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    ...
    counts: {
      ...
      captureCells: INPUT_LIVE_V69_CAPTURE_COUNT,
  ...
  antecedentOutputs.set(INPUT_LIVE_V69_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "input-live-v69-authorization-template-v1",
    authorizationId: "input-live-v69",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: INPUT_LIVE_V69_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
  ...
  if (check) {
    ...
    mkdirSync(path.dirname(INPUT_LIVE_V69_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(INPUT_LIVE_V69_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(INPUT_LIVE_V69_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(INPUT_LIVE_V69_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildInputLiveV69Proof(), null, 2)}\n`,
  );
