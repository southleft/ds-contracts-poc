...
import {
  validateCalendarLiveV32AntecedentIndex,
  verifyCalendarLiveV32Authorization,
  type CalendarLiveV32AntecedentIndex,
  type CalendarLiveV32AuthorizationProof,
} from "./calendar-live-v32-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V32_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v32/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V32_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v32-security-attestation.json";

export interface CalendarLiveV32SecurityAttestation {
  artifactVersion: "calendar-live-v32-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV32PreflightReport {
  artifactVersion: "calendar-live-v32-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV32SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV32SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV32AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v32-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V32_TARGET) ||
...
export function validateCalendarLiveV32ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV32Authorization");
  const preflight = source.indexOf("runCalendarLiveV32Preflight");
  const initialize = source.indexOf("CalendarLiveV32Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV32Preflight(
  root: string,
  proof: CalendarLiveV32AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V32_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV32PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V32_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V32_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV32AntecedentIndex;
  failures.push(...validateCalendarLiveV32AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V32_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V32_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV32SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v32-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV32ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v32 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV32SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v32-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V32_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV32Preflight(
    process.cwd(),
    verifyCalendarLiveV32Authorization(),
    attestationPath,
