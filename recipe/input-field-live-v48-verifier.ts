...
export const INPUT_LIVE_V48_VERIFIER_VERSION = "input-live-v48-verifier-v1";

export function buildInputLiveV48RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV48RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV48RawNode[];
}

export interface InputLiveV48NormalizedScene {
...
export interface InputLiveV48SceneProof {
...
export function normalizeInputLiveV48Scene(
  root: InputLiveV48RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV48NormalizedScene {
  const canonicalBindings: InputLiveV48NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV48RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV48Scene<Instance>(
  normalized: InputLiveV48NormalizedScene,
...
export function compileInputLiveV48ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV48PreCaptureGates(payload: unknown): void {
