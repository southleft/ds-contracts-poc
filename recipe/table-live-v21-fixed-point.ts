/**
 * Table live v21 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V21_FIXED_VERIFIER_VERSION = "table-live-v21-fixed-point-v1";
export const TABLE_LIVE_V21_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV21FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV21FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV21FixedSourceProbe {
...
export interface TableLiveV21FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV21FixedFixedPoint;
}

export interface TableLiveV21FixedHardGateInput {
  thresholds: TableLiveV21FixedThresholds;
  sourceProbes: TableLiveV21FixedSourceProbe[];
  cells: TableLiveV21FixedCellValidation[];
  sceneProofs: TableLiveV21FixedSceneProof[];
...
export interface TableLiveV21FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V21_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV21FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV21FixedHardGates(
  input: TableLiveV21FixedHardGateInput,
): TableLiveV21FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV21FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V21_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV21FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV21FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
