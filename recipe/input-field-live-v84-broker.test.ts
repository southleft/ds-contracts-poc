...
import {
  ...
  createInputLiveV84FakeDynamicResponse,
  ...
  listInputLiveV84PersistedRequests,
  persistInputLiveV84RawResponse,
  persistInputLiveV84TechnicalGates,
  validateInputLiveV84Receipt,
  validateInputLiveV84Request,
  type InputLiveV84RemotePhase,
...
import {
  ...
  type InputLiveV84ExtractPayload,
  type InputLiveV84ProbePayload,
...
import {
  ...
  simulatedInputLiveV84Authorization,
} from "./run-input-field-live-v84.js";
...
const captures: InputLiveV84CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV84Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v84-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV84Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV84CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV84FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV84FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V84_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V84_TARGET);
  persistInputLiveV84RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV84Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV84Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV84WriterOwnership,
...
const rootProofs = (): InputLiveV84RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v84-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV84Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV84Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV84RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV84RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV84ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV84ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV84CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV84CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV84CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV84Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV84CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV84PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV84ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV84Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV84Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV84Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV84Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V84_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV84RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V84_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV84Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV84Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV84ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV84Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV84WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV84RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV84Request;
    validateInputLiveV84Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV84Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV84CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v84-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV84Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV84Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV84Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V84_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV84RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V84_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV84Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV84Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV84RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V84_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV84Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV84Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV84Request;
