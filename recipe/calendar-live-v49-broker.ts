...
import {
  assertCalendarLiveV49RootProofs,
  CALENDAR_LIVE_V49_CAPTURE_COUNT,
  CALENDAR_LIVE_V49_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  CALENDAR_LIVE_V49_RESPONSE_CONTRACTS,
  CALENDAR_LIVE_V49_SOURCE_ROOTS,
  type CalendarLiveV49RootProof,
} from "./calendar-live-v49-contract.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V49_BROKER_VERSION = "calendar-live-v49-broker-v1";
export const CALENDAR_LIVE_V49_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const CALENDAR_LIVE_V49_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const CALENDAR_LIVE_V49_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const CALENDAR_LIVE_V49_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const CALENDAR_LIVE_V49_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const CALENDAR_LIVE_V49_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type CalendarLiveV49RemotePhase =
  (typeof CALENDAR_LIVE_V49_REMOTE_PHASES)[number];
export type CalendarLiveV49AuthorizationMode = "live" | "simulated";

export interface CalendarLiveV49CommitPins {
...
export interface CalendarLiveV49TransactionAuthorization extends CalendarLiveV49CommitPins {
  mode: CalendarLiveV49AuthorizationMode;
...
export interface CalendarLiveV49Manifest {
  artifactVersion: "calendar-live-v49-broker-manifest-v2";
  brokerVersion: typeof CALENDAR_LIVE_V49_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof CALENDAR_LIVE_V49_DYNAMIC_TOOL;
  target: typeof CALENDAR_LIVE_V49_TARGET;
  authorization: CalendarLiveV49TransactionAuthorization;
...
export interface CalendarLiveV49ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface CalendarLiveV49RequestBody {
  artifactVersion: "calendar-live-v49-broker-request-v2";
  brokerVersion: typeof CALENDAR_LIVE_V49_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: CalendarLiveV49RemotePhase;
  sequence: number;
  ...
  commitPins: CalendarLiveV49CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof CALENDAR_LIVE_V49_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: CalendarLiveV49ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface CalendarLiveV49Request extends CalendarLiveV49RequestBody {
...
export interface CalendarLiveV49Receipt {
  artifactVersion: "calendar-live-v49-broker-receipt-v2";
  brokerVersion: typeof CALENDAR_LIVE_V49_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: CalendarLiveV49RemotePhase;
...
export interface CalendarLiveV49AcceptedResponse<T = unknown> {
  request: CalendarLiveV49Request;
  receipt: CalendarLiveV49Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface CalendarLiveV49IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: CalendarLiveV49ResponseContract;
...
const equalTarget = (value: unknown): value is typeof CALENDAR_LIVE_V49_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === CALENDAR_LIVE_V49_TARGET.fileKey &&
  (value as BrokerTarget).fileName === CALENDAR_LIVE_V49_TARGET.fileName &&
  (value as BrokerTarget).editorType === CALENDAR_LIVE_V49_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== CALENDAR_LIVE_V49_TARGET.fileKey ||
    context.fileName !== CALENDAR_LIVE_V49_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === CALENDAR_LIVE_V49_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Calendar live v49 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= CALENDAR_LIVE_V49_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v49 capture index must be 0..7");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Calendar live v49 authorization pins are malformed");
};

const readManifest = (directory: string): CalendarLiveV49Manifest => {
  const manifest = json<CalendarLiveV49Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Calendar live v49 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "calendar-live-v49-broker-manifest-v2" ||
    manifest.brokerVersion !== CALENDAR_LIVE_V49_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== CALENDAR_LIVE_V49_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(CALENDAR_LIVE_V49_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Calendar live v49 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: CalendarLiveV49Request,
): CalendarLiveV49RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: CalendarLiveV49RemotePhase,
  captureIndex?: number,
): CalendarLiveV49Request | undefined => {
  const file = calendarLiveV1RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV49Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: CalendarLiveV49RemotePhase,
  captureIndex?: number,
): CalendarLiveV49Receipt | undefined => {
  const file = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV49Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: CalendarLiveV49Request | undefined;
  receipt: CalendarLiveV49Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: CalendarLiveV49RemotePhase,
): CalendarLiveV49ResponseContract => ({
  ...CALENDAR_LIVE_V49_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: CalendarLiveV49Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: CALENDAR_LIVE_V49_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: CALENDAR_LIVE_V49_DYNAMIC_TOOL,
      target: CALENDAR_LIVE_V49_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "calendar-live-v49-plugin-response-v2",
  brokerVersion: CALENDAR_LIVE_V49_BROKER_VERSION,
  target: CALENDAR_LIVE_V49_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*CALENDAR_LIVE_V49_PROGRAM_START*/${program}/*CALENDAR_LIVE_V49_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*CALENDAR_LIVE_V49_PROGRAM_START*/";
  const endMarker = "/*CALENDAR_LIVE_V49_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Calendar live v49 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createCalendarLiveV49Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Calendar live v49 private signing key does not match authorized public-key identity",
    );
  const manifest: CalendarLiveV49Manifest = {
    artifactVersion: "calendar-live-v49-broker-manifest-v2",
    brokerVersion: CALENDAR_LIVE_V49_BROKER_VERSION,
    ...
    captureRequests: CALENDAR_LIVE_V49_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateCalendarLiveV49Request(
  directory: string,
  request: CalendarLiveV49Request,
): void {
  ...
  if (
    request.artifactVersion !== "calendar-live-v49-broker-request-v2" ||
    request.brokerVersion !== CALENDAR_LIVE_V49_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== CALENDAR_LIVE_V49_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Calendar live v49 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueCalendarLiveV49Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: CalendarLiveV49RemotePhase,
  program: string,
  options: CalendarLiveV49IssueOptions = {},
): CalendarLiveV49Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<CalendarLiveV49Request>(targetPath);
    validateCalendarLiveV49Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Calendar live v49 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(calendarLiveV1ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Calendar live v49 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Calendar live v49 empty code envelope refused");
  if (phase === "capture") validateCalendarLiveV49TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Calendar live v49 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: CALENDAR_LIVE_V49_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? CALENDAR_LIVE_V49_SIGNED_WRITER_TIMEOUT_MS
        : CALENDAR_LIVE_V49_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: CalendarLiveV49RequestBody = {
    artifactVersion: "calendar-live-v49-broker-request-v2",
    brokerVersion: CALENDAR_LIVE_V49_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: CalendarLiveV49Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateCalendarLiveV49Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistCalendarLiveV49RawResponse(
  directory: string,
  phase: CalendarLiveV49RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Calendar live v49 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<CalendarLiveV49Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Calendar live v49 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = calendarLiveV1ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Calendar live v49 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): CalendarLiveV49Receipt => {
  const body: Omit<CalendarLiveV49Receipt, "receiptSha256"> = {
    artifactVersion: "calendar-live-v49-broker-receipt-v2",
    brokerVersion: CALENDAR_LIVE_V49_BROKER_VERSION,
...
export function acceptCalendarLiveV49Response<T = unknown>(
  directory: string,
  phase: CalendarLiveV49RemotePhase,
  options: {
    captureIndex?: number;
...
): CalendarLiveV49AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Calendar live v49 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<CalendarLiveV49Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Calendar live v49 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "calendar-live-v49-plugin-response-v2" ||
    plugin?.brokerVersion !== CALENDAR_LIVE_V49_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Calendar live v49 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistCalendarLiveV49TechnicalGates(
  directory: string,
  rootProofs: readonly CalendarLiveV49RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertCalendarLiveV49RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Calendar live v49 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Calendar live v49 probe payload hash malformed");
  const gates = Object.fromEntries(
    CALENDAR_LIVE_V49_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "calendar-live-v49-host-technical-gates-v2",
    brokerVersion: CALENDAR_LIVE_V49_BROKER_VERSION,
...
export function validateCalendarLiveV49TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "calendar-live-v49-host-technical-gates-v2" ||
    ...
    artifact.roots.length !== CALENDAR_LIVE_V49_SOURCE_ROOTS ||
    ...
    CALENDAR_LIVE_V49_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Calendar live v49 technical gates artifact");
}

export function validateCalendarLiveV49Receipt(
  directory: string,
  phase: CalendarLiveV49RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateCalendarLiveV49Request(directory, request);
  const receipt = json<CalendarLiveV49Receipt>(
  ...
  if (
    receipt.artifactVersion !== "calendar-live-v49-broker-receipt-v2" ||
    receipt.brokerVersion !== CALENDAR_LIVE_V49_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Calendar live v49 accepted receipt: ${request.requestId}`,
    );
}

export function readCalendarLiveV49AcceptedRawPayload(
  directory: string,
  phase: CalendarLiveV49RemotePhase,
  captureIndex?: number,
): unknown {
  validateCalendarLiveV49Receipt(directory, phase, captureIndex);
...
export function createCalendarLiveV49FakeDynamicResponse(
  request: CalendarLiveV49Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? CALENDAR_LIVE_V49_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? CALENDAR_LIVE_V49_TARGET,
    ...overrides.extra,
  };
}

export function listCalendarLiveV49PersistedRequests(
  directory: string,
): CalendarLiveV49Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<CalendarLiveV49Request>(file)] : [];
    });
}
