...
import {
  acceptTableLiveV22Response,
  createTableLiveV22Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV22Request,
  persistTableLiveV22RawResponse,
  persistTableLiveV22TechnicalGates,
  readTableLiveV22AcceptedRawPayload,
  type TableLiveV22Request,
  type TableLiveV22TransactionAuthorization,
} from "./table-live-v22-broker.js";
import {
  TABLE_LIVE_V22_CAPTURE_COUNT,
  assertTableLiveV22CaptureResponses,
  ...
  evaluateTableLiveV22Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV22Roots,
  validateTableLiveV22CaptureManifest,
  validateTableLiveV22CapturePayload,
  validateTableLiveV22CleanupPayload,
  validateTableLiveV22ExtractPayload,
  validateTableLiveV22ProbePayload,
  validateTableLiveV22WriterPayload,
  type TableLiveV22CaptureCell,
  type TableLiveV22CapturePayload,
...
import {
  ...
  TABLE_LIVE_V22_PLAN_PATH,
  type TableLiveV22ProofPlan,
} from "./build-table-live-proof-v22.js";
import {
...
export const TABLE_LIVE_V22_RUNNER_VERSION = "table-live-v22-runner-v1";

interface TableLiveV22JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV22OrchestratorState {
  artifactVersion: "table-live-v22-orchestrator-state-v1";
  ...
  journal: TableLiveV22JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV22ProofPlan;
  planSha256: string;
  captures: TableLiveV22CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV22SourceIdentity & {
...
export interface TableLiveV22NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV22OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v22 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V22_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V22_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV22ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV22CaptureCell[];
  validateTableLiveV22CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v22 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v22 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV22WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV22RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV22Request =>
  readJson<TableLiveV22Request>(file);

export class TableLiveV22Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV22Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV22OrchestratorState = {
      artifactVersion: "table-live-v22-orchestrator-state-v1",
    ...
    return new TableLiveV22Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV22OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV22Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV22NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV22NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v22 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV22NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v22 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV22RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV22Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV22CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV22AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV22WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV22Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV22ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV22Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV22Response(this.#directory, "writer", {
        validate: validateTableLiveV22WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV22Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV22RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV22Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV22ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV22Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV22RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v22-host-root-proofs-v1",
      ...
      const next = issueTableLiveV22Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV22ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV22Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV22ProbePayload(payload, ownership),
      });
      persistTableLiveV22TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v22 capture cell missing");
      const accepted = acceptTableLiveV22Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV22CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V22_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV22CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV22Objective(
    ...
    throw new Error(`Table live v22 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV22Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV22CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV22Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV22RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV22Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV22CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV22CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV22CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV22Authorization = (
  privateKey: KeyObject,
): TableLiveV22TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV22SimulationKey = (): KeyObject =>
