...
import {
  verifyCalendarLiveV29FixedSceneFixedPoint,
  type CalendarLiveV29FixedFixedPoint,
} from "./calendar-live-v29-fixed-point.js";

export const CALENDAR_LIVE_V29_VERIFIER_VERSION = "calendar-live-v29-verifier-v1";

export function buildCalendarLiveV29RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV29RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV29RawNode[];
}

export interface CalendarLiveV29NormalizedScene {
...
export interface CalendarLiveV29SceneProof {
...
export function normalizeCalendarLiveV29Scene(
  root: CalendarLiveV29RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV29NormalizedScene {
  const canonicalBindings: CalendarLiveV29NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV29RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV29Scene<Instance>(
  normalized: CalendarLiveV29NormalizedScene,
...
): CalendarLiveV29SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV29FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV29ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV29PreCaptureGates(payload: unknown): void {
