...
import {
  ...
  COMBOBOX_LIVE_V10_OMIT_EMPTY_INSTANCE_PAYLOAD_MARKER,
  COMBOBOX_LIVE_V10_PROJECT_LIVE_ROOT_OWNERSHIP_KEY_MARKER,
  COMBOBOX_LIVE_V10_RECOVER_COMPONENT_PROPERTY_NAME_MARKER,
  COMBOBOX_LIVE_V10_RECOVER_RECIPE_COMPONENT_REF_MARKER,
  COMBOBOX_LIVE_V10_TRIGGER_BINDING_COMPILE_ORDER_MARKER,
...
test("host orders combobox/trigger bindings to compile field order, not live or inherited Input surface list", () => {
  ...
  assert.ok(
    ...
    "v10 compile-order list must rank padding.left before padding.right",
  );
  ...
  assert.deepEqual(
    ...
    "v10 host must emit compile left-then-right, not live extract order",
  );
