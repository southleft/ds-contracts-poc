...
export interface InputLiveV45LifecycleSimulationReport {
  artifactVersion: "input-live-v45-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV45AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV45Lifecycle(): Promise<InputLiveV45LifecycleSimulationReport> {
  await buildInputLiveV45Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V45_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV45AntecedentIndex;
  const indexSha256 = inputLiveV45Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v45-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V45_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V45_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV45History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV45History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV45AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V45_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V45_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV45History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV45History(authorized, "pending");
    ...
    await buildInputLiveV45Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V45_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV45AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV45Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV45Lifecycle(), null, 2)}\n`,
  );
