...
import {
  validateComboboxLiveV36AntecedentIndex,
  verifyComboboxLiveV36Authorization,
  type ComboboxLiveV36AntecedentIndex,
  type ComboboxLiveV36AuthorizationProof,
} from "./combobox-live-v36-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V36_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v36/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V36_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v36-security-attestation.json";

export interface ComboboxLiveV36SecurityAttestation {
...
export interface ComboboxLiveV36PreflightReport {
  artifactVersion: "combobox-live-v36-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV36SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV36SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV36AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v36-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V36_TARGET) ||
...
export function validateComboboxLiveV36ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV36Authorization");
  const preflight = source.indexOf("runComboboxLiveV36Preflight");
  const initialize = source.indexOf("ComboboxLiveV36Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV36Preflight(
  root: string,
  proof: ComboboxLiveV36AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V36_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV36PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V36_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V36_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V36_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV36AntecedentIndex;
  failures.push(...validateComboboxLiveV36AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V36_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V36_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v36-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV36ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v36 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV36SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v36-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V36_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV36Preflight(
    process.cwd(),
    verifyComboboxLiveV36Authorization(),
    attestationPath,
