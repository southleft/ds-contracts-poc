...
import { CALENDAR_LIVE_V12_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v12.js";
import { verifyCalendarLiveV12Authorization } from "./calendar-live-v12-authorization.js";
import { runCalendarLiveV12Preflight } from "./calendar-live-v12-preflight.js";
import { CalendarLiveV12Orchestrator } from "./run-calendar-live-v12.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v12 requires exact authorization artifact ${CALENDAR_LIVE_V12_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v12-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV12Authorization(root);
    runCalendarLiveV12Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV12Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV12Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
