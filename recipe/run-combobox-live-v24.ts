...
import {
  acceptComboboxLiveV24Response,
  createComboboxLiveV24Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV24Request,
  persistComboboxLiveV24RawResponse,
  persistComboboxLiveV24TechnicalGates,
  readComboboxLiveV24AcceptedRawPayload,
  type ComboboxLiveV24Request,
  type ComboboxLiveV24TransactionAuthorization,
} from "./combobox-live-v24-broker.js";
import {
  COMBOBOX_LIVE_V24_CAPTURE_COUNT,
  assertComboboxLiveV24CaptureResponses,
  ...
  evaluateComboboxLiveV24Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV24Roots,
  validateComboboxLiveV24CaptureManifest,
  validateComboboxLiveV24CapturePayload,
  validateComboboxLiveV24CleanupPayload,
  validateComboboxLiveV24ExtractPayload,
  validateComboboxLiveV24ProbePayload,
  validateComboboxLiveV24WriterPayload,
  type ComboboxLiveV24CaptureCell,
  type ComboboxLiveV24CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V24_PLAN_PATH,
  type ComboboxLiveV24ProofPlan,
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v24.js";

export const COMBOBOX_LIVE_V24_RUNNER_VERSION = "combobox-live-v24-runner-v1";

interface ComboboxLiveV24JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV24OrchestratorState {
  artifactVersion: "combobox-live-v24-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV24JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV24ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV24CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV24SourceIdentity & {
...
export interface ComboboxLiveV24NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV24OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v24 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V24_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V24_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV24ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV24CaptureCell[];
  validateComboboxLiveV24CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v24 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v24 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV24WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV24RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV24Request =>
  readJson<ComboboxLiveV24Request>(file);

export class ComboboxLiveV24Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV24Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV24OrchestratorState = {
      artifactVersion: "combobox-live-v24-orchestrator-state-v1",
    ...
    return new ComboboxLiveV24Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV24OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV24Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV24NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV24NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v24 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV24NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v24 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV24RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV24Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV24CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV24AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV24WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV24Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV24ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV24Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV24Response(this.#directory, "writer", {
        validate: validateComboboxLiveV24WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV24Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV24RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV24Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV24ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV24Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV24RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v24-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV24Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV24ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV24Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV24ProbePayload(payload, ownership),
      });
      persistComboboxLiveV24TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v24 capture cell missing");
      const accepted = acceptComboboxLiveV24Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV24CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V24_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV24CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV24Objective(
    ...
    throw new Error(`Combobox live v24 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV24Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV24CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV24Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV24RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV24Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV24CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV24CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV24CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV24Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV24TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV24SimulationKey = (): KeyObject =>
