...
import { INPUT_LIVE_V56_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v56.js";
import { verifyInputLiveV56Authorization } from "./input-field-live-v56-authorization.js";
import { runInputLiveV56Preflight } from "./input-field-live-v56-preflight.js";
import { InputLiveV56Orchestrator } from "./run-input-field-live-v56.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V56_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v56-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV56Authorization(root);
    runInputLiveV56Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV56Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
