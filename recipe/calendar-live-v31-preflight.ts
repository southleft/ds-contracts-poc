...
import {
  validateCalendarLiveV31AntecedentIndex,
  verifyCalendarLiveV31Authorization,
  type CalendarLiveV31AntecedentIndex,
  type CalendarLiveV31AuthorizationProof,
} from "./calendar-live-v31-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V31_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v31/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V31_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v31-security-attestation.json";

export interface CalendarLiveV31SecurityAttestation {
...
export interface CalendarLiveV31PreflightReport {
  artifactVersion: "calendar-live-v31-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV31SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV31SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV31AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v31-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V31_TARGET) ||
...
export function validateCalendarLiveV31ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV31Authorization");
  const preflight = source.indexOf("runCalendarLiveV31Preflight");
  const initialize = source.indexOf("CalendarLiveV31Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV31Preflight(
  root: string,
  proof: CalendarLiveV31AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V31_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV31PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V31_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V31_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV31AntecedentIndex;
  failures.push(...validateCalendarLiveV31AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V31_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V31_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v31-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV31ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v31 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV31SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v31-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V31_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV31Preflight(
    process.cwd(),
    verifyCalendarLiveV31Authorization(),
    attestationPath,
