...
} from "./input-field-live-v3-verifier-v28.js";
import {
  buildInputLiveV28RawPropertyRuntime,
  normalizeInputLiveV28Scene,
  type InputLiveV28RawNode,
} from "./input-field-live-v28-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v28.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v28.js";
...
export const INPUT_LIVE_V28_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V28_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V28_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V28_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V28_VARIANT_COUNT = 256;
export const INPUT_LIVE_V28_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V28_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV28SourceId = (typeof INPUT_LIVE_V28_SOURCE_IDS)[number];

export interface InputLiveV28SourceIdentity {
  source: InputLiveV28SourceId;
...
export interface InputLiveV28WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV28ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV28SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV28RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV28RootProof {
  source: InputLiveV28SourceId;
  adapterIdentity: string;
...
export interface InputLiveV28ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV28SourceId;
    adapterIdentity: string;
...
export interface InputLiveV28CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV28SourceId;
  adapterIdentity: string;
...
export interface InputLiveV28CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV28SourceId;
  strata: InputLiveV28CaptureCell["strata"];
...
export interface InputLiveV28CleanupPayload {
...
export function validateInputLiveV28WriterPayload(
  value: unknown,
): InputLiveV28WriterOwnership {
...
const identityPlan = (source: InputLiveV28SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV28ExtractProgram(
  writer: InputLiveV28WriterOwnership,
  sources: readonly InputLiveV28SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V28_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V28_NAMESPACE)}
${buildInputLiveV28RawPropertyRuntime()}
...
export function validateInputLiveV28ExtractPayload(
  value: unknown,
  writer: InputLiveV28WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V28_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV28ExtractPayload;
}

export function proveInputLiveV28Roots<Instance>(
  extract: InputLiveV28ExtractPayload,
  sources: readonly (InputLiveV28SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV28Scene(
      root.scene,
...
export function assertInputLiveV28RootProofs(
  proofs: readonly InputLiveV28RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV28ProbeProgram(
  writer: InputLiveV28WriterOwnership,
  sources: readonly InputLiveV28SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V28_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV28ProbePayload(
  value: unknown,
  writer: InputLiveV28WriterOwnership,
): InputLiveV28ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV28ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V28_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V28_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV28CaptureManifest(
  cells: readonly InputLiveV28CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V28_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V28_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V28_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V28_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V28_SOURCE_IDS.some(
...
export const inputLiveV28CaptureManifestSha256 = (
  cells: readonly InputLiveV28CaptureCell[],
): string => {
  validateInputLiveV28CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV28CaptureProgram(
  writer: InputLiveV28WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V28_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V28_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V28_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV28CapturePayload(
  value: unknown,
  cell: InputLiveV28CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV28CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V28_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V28_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV28CaptureResponses(
  manifest: readonly InputLiveV28CaptureCell[],
  responses: readonly InputLiveV28CapturePayload[],
): void {
  validateInputLiveV28CaptureManifest(manifest);
  if (
...
export interface InputLiveV28ObjectiveReport {
  artifactVersion: "input-live-v28-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV28Objective(
  manifest: readonly InputLiveV28CaptureCell[],
  responses: readonly InputLiveV28CapturePayload[],
  readReference: (cell: InputLiveV28CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV28CaptureCell) => Buffer,
): InputLiveV28ObjectiveReport {
  assertInputLiveV28CaptureResponses(manifest, responses);
...
export function buildInputLiveV28CleanupProgram(
  writer: InputLiveV28WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V28_NAMESPACE,
...
export function validateInputLiveV28CleanupPayload(
  value: unknown,
  writer: InputLiveV28WriterOwnership,
): InputLiveV28CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV28CleanupPayload;
...
export const INPUT_LIVE_V28_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV28WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
