...
import { COMBOBOX_LIVE_V32_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v32.js";
import { verifyComboboxLiveV32Authorization } from "./combobox-live-v32-authorization.js";
import { runComboboxLiveV32Preflight } from "./combobox-live-v32-preflight.js";
import { ComboboxLiveV32Orchestrator } from "./run-combobox-live-v32.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v32 requires exact authorization artifact ${COMBOBOX_LIVE_V32_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v32-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV32Authorization(root);
    runComboboxLiveV32Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV32Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV32Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
