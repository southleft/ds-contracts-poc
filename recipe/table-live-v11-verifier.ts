...
import {
  verifyTableLiveV11FixedSceneFixedPoint,
  type TableLiveV11FixedFixedPoint,
} from "./table-live-v11-fixed-point.js";

export const TABLE_LIVE_V11_VERIFIER_VERSION = "table-live-v11-verifier-v1";

export function buildTableLiveV11RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV11RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV11RawNode[];
}

export interface TableLiveV11NormalizedScene {
...
export interface TableLiveV11SceneProof {
...
export function normalizeTableLiveV11Scene(
  root: TableLiveV11RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV11NormalizedScene {
  const canonicalBindings: TableLiveV11NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV11RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV11Scene<Instance>(
  normalized: TableLiveV11NormalizedScene,
...
): TableLiveV11SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV11FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV11ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV11PreCaptureGates(payload: unknown): void {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("table live v11 pre-capture gate payload absent");
