...
import { verifyComboboxLiveV29Authorization } from "./combobox-live-v29-authorization.js";
import { COMBOBOX_LIVE_V29_TARGET } from "./combobox-live-v29-broker.js";
import {
  COMBOBOX_LIVE_V29_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV29Authorization();
...
const attestation: ComboboxLiveV29SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV29SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v29 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V29_SECURITY_ATTESTATION_DEFAULT_PATH,
);
