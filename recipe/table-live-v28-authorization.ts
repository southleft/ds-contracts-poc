...
export type TableLiveV28HistoryExpectation = "pending" | "authorized";

export interface TableLiveV28AuthorizationArtifact {
  artifactVersion: "table-live-v28-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V28_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV28AntecedentIndex {
...
export interface TableLiveV28HistoryState {
  index?: TableLiveV28AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV28AuthorizationArtifact;
...
export interface TableLiveV28AuthorizationProof
  extends TableLiveV28TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V28_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V28_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V28_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V28_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v28-authorization") ||
  artifactPath.includes("table-live-v28-preflight") ||
  artifactPath.includes("table-live-v28-authorized") ||
  artifactPath.includes("create-table-live-v28-security-attestation") ||
  artifactPath.includes("table-live-pivot-v28-status") ||
...
export function validateTableLiveV28AntecedentIndex(
  index: TableLiveV28AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v28-antecedent-index-v1" ||
...
export function validateTableLiveV28UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV28History\(\s*["']/.test(source) ||
    /readTableLiveV28HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV28AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV28AuthorizationArtifact {
  ...
  ) as TableLiveV28AntecedentIndex;
  const indexFailures = validateTableLiveV28AntecedentIndex(index);
...
export function validateTableLiveV28AuthorizationArtifact(
  artifact: TableLiveV28AuthorizationArtifact | undefined,
  index: TableLiveV28AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v28-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v28" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V28_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V28_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V28_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V28_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V28_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V28_SOURCE_ROOTS ||
...
export function validateTableLiveV28History(
  state: TableLiveV28HistoryState,
  expected: TableLiveV28HistoryExpectation,
): string[] {
  const failures = validateTableLiveV28AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV28AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV28HistoryState(
  root = process.cwd(),
): TableLiveV28HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V28_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV28AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V28_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V28_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V28_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV28History(
  expected: TableLiveV28HistoryExpectation,
  root = process.cwd(),
): TableLiveV28HistoryState {
  const state = readTableLiveV28HistoryState(root);
  const failures = validateTableLiveV28History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v28 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV28Authorization(
  root = process.cwd(),
): TableLiveV28AuthorizationProof {
  const state = verifyTableLiveV28History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V28_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v28/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v28.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V28_TARGET,
    expectedDynamicTool: TABLE_LIVE_V28_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V28_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV28HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV28History(expected);
  process.stdout.write(
    `Table live v28 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
