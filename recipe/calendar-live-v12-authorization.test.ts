...
import { CALENDAR_LIVE_V12_INDEX_PATH } from "./build-calendar-live-proof-v12.js";
import {
...
import {
  buildCalendarLiveV12AuthorizationArtifact,
  validateCalendarLiveV12AntecedentIndex,
  validateCalendarLiveV12AuthorizationArtifact,
  validateCalendarLiveV12History,
  validateCalendarLiveV12UnitTestSource,
  type CalendarLiveV12AntecedentIndex,
  type CalendarLiveV12AuthorizationArtifact,
  type CalendarLiveV12AuthorizationProof,
  type CalendarLiveV12HistoryState,
} from "./calendar-live-v12-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV12ControlFlowSource,
  validateCalendarLiveV12SecurityAttestation,
  type CalendarLiveV12SecurityAttestation,
} from "./calendar-live-v12-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V12_INDEX_PATH);
const index = (): CalendarLiveV12AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV12AntecedentIndex;

const artifact = (): CalendarLiveV12AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV12AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV12HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV12HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV12AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v12/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV12AuthorizationArtifact,
): CalendarLiveV12SecurityAttestation => {
  const body: Omit<CalendarLiveV12SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V12_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV12History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV12History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV12History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV12History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV12History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV12UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v12-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV12UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV12AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV12AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV12AuthorizationArtifact(
        planted as CalendarLiveV12AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV12SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV12SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV12SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v12-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV12ControlFlowSource(source), []);
});
