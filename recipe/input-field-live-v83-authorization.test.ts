...
import { INPUT_LIVE_V83_INDEX_PATH } from "./build-input-field-live-proof-v83.js";
import {
  INPUT_LIVE_V83_DYNAMIC_TOOL,
  INPUT_LIVE_V83_TARGET,
} from "./input-field-live-v83-broker.js";
import {
  buildInputLiveV83AuthorizationArtifact,
  validateInputLiveV83AntecedentIndex,
  validateInputLiveV83AuthorizationArtifact,
  validateInputLiveV83History,
  validateInputLiveV83UnitTestSource,
  type InputLiveV83AntecedentIndex,
  type InputLiveV83AuthorizationArtifact,
  type InputLiveV83AuthorizationProof,
  type InputLiveV83HistoryState,
} from "./input-field-live-v83-authorization.js";
import {
  inputLiveV83AttestationSha256,
  validateInputLiveV83ControlFlowSource,
  validateInputLiveV83SecurityAttestation,
  type InputLiveV83SecurityAttestation,
} from "./input-field-live-v83-preflight.js";
import { inputLiveV83Sha256 } from "./input-field-live-v83-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V83_INDEX_PATH);
const index = (): InputLiveV83AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV83AntecedentIndex;

const artifact = (): InputLiveV83AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV83AuthorizationArtifact({
...
const pendingState = (): InputLiveV83HistoryState => ({
  index: index(),
  indexSha256: inputLiveV83Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV83HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV83AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV83Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V83_TARGET,
  expectedDynamicTool: INPUT_LIVE_V83_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v83/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV83AuthorizationArtifact,
): InputLiveV83SecurityAttestation => {
  const body: Omit<InputLiveV83SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v83-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V83_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV83AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV83History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV83History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV83History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV83History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV83History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV83UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV83UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV83AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV83Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV83AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV83AuthorizationArtifact(
        planted as InputLiveV83AuthorizationArtifact,
        index(),
        inputLiveV83Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV83SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV83AttestationSha256(
      body as Omit<InputLiveV83SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV83SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV83ControlFlowSource(source), []);
});
