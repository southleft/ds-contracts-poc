...
import { COMBOBOX_LIVE_V11_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v11.js";
import { verifyComboboxLiveV11Authorization } from "./combobox-live-v11-authorization.js";
import { runComboboxLiveV11Preflight } from "./combobox-live-v11-preflight.js";
import { ComboboxLiveV11Orchestrator } from "./run-combobox-live-v11.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v11 requires exact authorization artifact ${COMBOBOX_LIVE_V11_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v11-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV11Authorization(root);
    runComboboxLiveV11Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV11Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV11Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
