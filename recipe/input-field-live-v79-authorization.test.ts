...
import { INPUT_LIVE_V79_INDEX_PATH } from "./build-input-field-live-proof-v79.js";
import {
  INPUT_LIVE_V79_DYNAMIC_TOOL,
  INPUT_LIVE_V79_TARGET,
} from "./input-field-live-v79-broker.js";
import {
  buildInputLiveV79AuthorizationArtifact,
  validateInputLiveV79AntecedentIndex,
  validateInputLiveV79AuthorizationArtifact,
  validateInputLiveV79History,
  validateInputLiveV79UnitTestSource,
  type InputLiveV79AntecedentIndex,
  type InputLiveV79AuthorizationArtifact,
  type InputLiveV79AuthorizationProof,
  type InputLiveV79HistoryState,
} from "./input-field-live-v79-authorization.js";
import {
  inputLiveV79AttestationSha256,
  validateInputLiveV79ControlFlowSource,
  validateInputLiveV79SecurityAttestation,
  type InputLiveV79SecurityAttestation,
} from "./input-field-live-v79-preflight.js";
import { inputLiveV79Sha256 } from "./input-field-live-v79-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V79_INDEX_PATH);
const index = (): InputLiveV79AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV79AntecedentIndex;

const artifact = (): InputLiveV79AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV79AuthorizationArtifact({
...
const pendingState = (): InputLiveV79HistoryState => ({
  index: index(),
  indexSha256: inputLiveV79Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV79HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV79AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV79Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V79_TARGET,
  expectedDynamicTool: INPUT_LIVE_V79_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v79/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV79AuthorizationArtifact,
): InputLiveV79SecurityAttestation => {
  const body: Omit<InputLiveV79SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v79-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V79_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV79AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV79History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV79History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV79History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV79History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV79History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV79UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV79UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV79AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV79Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV79AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV79AuthorizationArtifact(
        planted as InputLiveV79AuthorizationArtifact,
        index(),
        inputLiveV79Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV79SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV79AttestationSha256(
      body as Omit<InputLiveV79SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV79SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV79ControlFlowSource(source), []);
});
