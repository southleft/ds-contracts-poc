...
import { INPUT_LIVE_V44_INDEX_PATH } from "./build-input-field-live-proof-v44.js";
import {
  INPUT_LIVE_V44_DYNAMIC_TOOL,
  INPUT_LIVE_V44_TARGET,
} from "./input-field-live-v44-broker.js";
import {
  buildInputLiveV44AuthorizationArtifact,
  validateInputLiveV44AntecedentIndex,
  validateInputLiveV44AuthorizationArtifact,
  validateInputLiveV44History,
  validateInputLiveV44UnitTestSource,
  type InputLiveV44AntecedentIndex,
  type InputLiveV44AuthorizationArtifact,
  type InputLiveV44AuthorizationProof,
  type InputLiveV44HistoryState,
} from "./input-field-live-v44-authorization.js";
import {
  inputLiveV44AttestationSha256,
  validateInputLiveV44ControlFlowSource,
  validateInputLiveV44SecurityAttestation,
  type InputLiveV44SecurityAttestation,
} from "./input-field-live-v44-preflight.js";
import { inputLiveV44Sha256 } from "./input-field-live-v44-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V44_INDEX_PATH);
const index = (): InputLiveV44AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV44AntecedentIndex;

const artifact = (): InputLiveV44AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV44AuthorizationArtifact({
...
const pendingState = (): InputLiveV44HistoryState => ({
  index: index(),
  indexSha256: inputLiveV44Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV44HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV44AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV44Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V44_TARGET,
  expectedDynamicTool: INPUT_LIVE_V44_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v44/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV44AuthorizationArtifact,
): InputLiveV44SecurityAttestation => {
  const body: Omit<InputLiveV44SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v44-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V44_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV44AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV44History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV44History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV44History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV44History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV44History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV44UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV44UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV44AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV44Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV44AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV44AuthorizationArtifact(
        planted as InputLiveV44AuthorizationArtifact,
        index(),
        inputLiveV44Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV44SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV44AttestationSha256(
      body as Omit<InputLiveV44SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV44SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV44ControlFlowSource(source), []);
});
