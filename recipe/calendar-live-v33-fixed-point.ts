/**
 * Calendar live v33 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V33_FIXED_VERIFIER_VERSION = "calendar-live-v33-fixed-point-v1";
export const CALENDAR_LIVE_V33_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV33FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV33FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV33FixedSourceProbe {
...
export interface CalendarLiveV33FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV33FixedFixedPoint;
}

export interface CalendarLiveV33FixedHardGateInput {
  thresholds: CalendarLiveV33FixedThresholds;
  sourceProbes: CalendarLiveV33FixedSourceProbe[];
  cells: CalendarLiveV33FixedCellValidation[];
  sceneProofs: CalendarLiveV33FixedSceneProof[];
...
export interface CalendarLiveV33FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V33_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV33FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV33FixedHardGates(
  input: CalendarLiveV33FixedHardGateInput,
): CalendarLiveV33FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV33FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V33_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV33FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV33FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV33FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
