...
import { CALENDAR_LIVE_V41_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v41.js";
import { verifyCalendarLiveV41Authorization } from "./calendar-live-v41-authorization.js";
import { runCalendarLiveV41Preflight } from "./calendar-live-v41-preflight.js";
import { CalendarLiveV41Orchestrator } from "./run-calendar-live-v41.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v41 requires exact authorization artifact ${CALENDAR_LIVE_V41_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v41-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV41Authorization(root);
    runCalendarLiveV41Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV41Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV41Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
