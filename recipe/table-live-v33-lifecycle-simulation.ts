...
import { tableLiveV2Sha256 } from "./table-live-v33-broker.js";

export interface TableLiveV33LifecycleSimulationReport {
  artifactVersion: "table-live-v33-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV33AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV33Lifecycle(): Promise<TableLiveV33LifecycleSimulationReport> {
  await buildTableLiveV33Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V33_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV33AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v33-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V33_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V33_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV33History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV33History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV33AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V33_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V33_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV33History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV33History(authorized, "pending");
    ...
    await buildTableLiveV33Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V33_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV33AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV33Lifecycle(), null, 2)}\n`,
  );
