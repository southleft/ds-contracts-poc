...
import {
  validateCalendarLiveV43AntecedentIndex,
  verifyCalendarLiveV43Authorization,
  type CalendarLiveV43AntecedentIndex,
  type CalendarLiveV43AuthorizationProof,
} from "./calendar-live-v43-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V43_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v43/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V43_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v43-security-attestation.json";

export interface CalendarLiveV43SecurityAttestation {
  artifactVersion: "calendar-live-v43-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV43PreflightReport {
  artifactVersion: "calendar-live-v43-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV43SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV43SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV43AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v43-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V43_TARGET) ||
...
export function validateCalendarLiveV43ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV43Authorization");
  const preflight = source.indexOf("runCalendarLiveV43Preflight");
  const initialize = source.indexOf("CalendarLiveV43Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV43Preflight(
  root: string,
  proof: CalendarLiveV43AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V43_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV43PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V43_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V43_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV43AntecedentIndex;
  failures.push(...validateCalendarLiveV43AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V43_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V43_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV43SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v43-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV43ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v43 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV43SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v43-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V43_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV43Preflight(
    process.cwd(),
    verifyCalendarLiveV43Authorization(),
    attestationPath,
