...
import { TABLE_LIVE_V35_AUTHORIZATION_PATH } from "./build-table-live-proof-v35.js";
import { verifyTableLiveV35Authorization } from "./table-live-v35-authorization.js";
import { runTableLiveV35Preflight } from "./table-live-v35-preflight.js";
import { TableLiveV35Orchestrator } from "./run-table-live-v35.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v35 requires exact authorization artifact ${TABLE_LIVE_V35_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v35-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV35Authorization(root);
    runTableLiveV35Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV35Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV35Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
