/**
 * Table live v20 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V20_FIXED_VERIFIER_VERSION = "table-live-v20-fixed-point-v1";
export const TABLE_LIVE_V20_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV20FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV20FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV20FixedSourceProbe {
...
export interface TableLiveV20FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV20FixedFixedPoint;
}

export interface TableLiveV20FixedHardGateInput {
  thresholds: TableLiveV20FixedThresholds;
  sourceProbes: TableLiveV20FixedSourceProbe[];
  cells: TableLiveV20FixedCellValidation[];
  sceneProofs: TableLiveV20FixedSceneProof[];
...
export interface TableLiveV20FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V20_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV20FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV20FixedHardGates(
  input: TableLiveV20FixedHardGateInput,
): TableLiveV20FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV20FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: TABLE_LIVE_V20_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV20FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV20FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
