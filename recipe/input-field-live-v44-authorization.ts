...
import {
  ...
  INPUT_LIVE_V44_INDEX_PATH,
} from "./build-input-field-live-proof-v44.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV44HistoryExpectation = "pending" | "authorized";

export interface InputLiveV44AuthorizationArtifact {
  artifactVersion: "input-live-v44-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V44_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV44AntecedentIndex {
...
export interface InputLiveV44HistoryState {
  index?: InputLiveV44AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV44AuthorizationArtifact;
...
export interface InputLiveV44AuthorizationProof extends InputLiveV44TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V44_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V44_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V44_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V44_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v44-status") ||
...
export function validateInputLiveV44AntecedentIndex(
  index: InputLiveV44AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v44-antecedent-index-v1" ||
...
export function validateInputLiveV44UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV44History\(\s*["']/.test(source) ||
    /readInputLiveV44HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV44AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV44AuthorizationArtifact {
  ...
  ) as InputLiveV44AntecedentIndex;
  const indexFailures = validateInputLiveV44AntecedentIndex(index);
...
export function validateInputLiveV44AuthorizationArtifact(
  artifact: InputLiveV44AuthorizationArtifact | undefined,
  index: InputLiveV44AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v44-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v44" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V44_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V44_TARGET) ||
    !equalJson(
...
export function validateInputLiveV44History(
  state: InputLiveV44HistoryState,
  expected: InputLiveV44HistoryExpectation,
): string[] {
  const failures = validateInputLiveV44AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV44AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV44HistoryState(
  root = process.cwd(),
): InputLiveV44HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V44_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV44AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V44_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V44_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V44_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV44History(
  expected: InputLiveV44HistoryExpectation,
  root = process.cwd(),
): InputLiveV44HistoryState {
  const state = readInputLiveV44HistoryState(root);
  const failures = validateInputLiveV44History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV44Authorization(
  root = process.cwd(),
): InputLiveV44AuthorizationProof {
  const state = verifyInputLiveV44History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V44_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v44/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v44.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V44_TARGET,
    expectedDynamicTool: INPUT_LIVE_V44_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V44_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV44HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV44History(expected);
