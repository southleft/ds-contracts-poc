...
import {
  verifyCalendarLiveV24FixedSceneFixedPoint,
  type CalendarLiveV24FixedFixedPoint,
} from "./calendar-live-v24-fixed-point.js";

export const CALENDAR_LIVE_V24_VERIFIER_VERSION = "calendar-live-v24-verifier-v1";

export function buildCalendarLiveV24RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV24RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV24RawNode[];
}

export interface CalendarLiveV24NormalizedScene {
...
export interface CalendarLiveV24SceneProof {
...
export function normalizeCalendarLiveV24Scene(
  root: CalendarLiveV24RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV24NormalizedScene {
  const canonicalBindings: CalendarLiveV24NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV24RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV24Scene<Instance>(
  normalized: CalendarLiveV24NormalizedScene,
...
): CalendarLiveV24SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV24FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV24ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV24PreCaptureGates(payload: unknown): void {
