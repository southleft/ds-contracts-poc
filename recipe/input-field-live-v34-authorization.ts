...
import {
  ...
  INPUT_LIVE_V34_INDEX_PATH,
} from "./build-input-field-live-proof-v34.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV34HistoryExpectation = "pending" | "authorized";

export interface InputLiveV34AuthorizationArtifact {
  artifactVersion: "input-live-v34-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V34_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV34AntecedentIndex {
...
export interface InputLiveV34HistoryState {
  index?: InputLiveV34AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV34AuthorizationArtifact;
...
export interface InputLiveV34AuthorizationProof extends InputLiveV34TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V34_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V34_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V34_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V34_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v34-status") ||
...
export function validateInputLiveV34AntecedentIndex(
  index: InputLiveV34AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v34-antecedent-index-v1" ||
...
export function validateInputLiveV34UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV34History\(\s*["']/.test(source) ||
    /readInputLiveV34HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV34AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV34AuthorizationArtifact {
  ...
  ) as InputLiveV34AntecedentIndex;
  const indexFailures = validateInputLiveV34AntecedentIndex(index);
...
export function validateInputLiveV34AuthorizationArtifact(
  artifact: InputLiveV34AuthorizationArtifact | undefined,
  index: InputLiveV34AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v34-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v34" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V34_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V34_TARGET) ||
    !equalJson(
...
export function validateInputLiveV34History(
  state: InputLiveV34HistoryState,
  expected: InputLiveV34HistoryExpectation,
): string[] {
  const failures = validateInputLiveV34AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV34AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV34HistoryState(
  root = process.cwd(),
): InputLiveV34HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V34_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV34AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V34_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V34_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V34_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV34History(
  expected: InputLiveV34HistoryExpectation,
  root = process.cwd(),
): InputLiveV34HistoryState {
  const state = readInputLiveV34HistoryState(root);
  const failures = validateInputLiveV34History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV34Authorization(
  root = process.cwd(),
): InputLiveV34AuthorizationProof {
  const state = verifyInputLiveV34History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V34_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v34/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v34.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V34_TARGET,
    expectedDynamicTool: INPUT_LIVE_V34_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V34_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV34HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV34History(expected);
