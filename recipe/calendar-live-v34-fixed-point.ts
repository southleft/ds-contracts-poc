/**
 * Calendar live v34 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V34_FIXED_VERIFIER_VERSION = "calendar-live-v34-fixed-point-v1";
export const CALENDAR_LIVE_V34_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV34FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV34FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV34FixedSourceProbe {
...
export interface CalendarLiveV34FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV34FixedFixedPoint;
}

export interface CalendarLiveV34FixedHardGateInput {
  thresholds: CalendarLiveV34FixedThresholds;
  sourceProbes: CalendarLiveV34FixedSourceProbe[];
  cells: CalendarLiveV34FixedCellValidation[];
  sceneProofs: CalendarLiveV34FixedSceneProof[];
...
export interface CalendarLiveV34FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V34_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV34FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV34FixedHardGates(
  input: CalendarLiveV34FixedHardGateInput,
): CalendarLiveV34FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV34FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V34_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV34FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV34FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV34FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
