...
import {
  verifyCalendarLiveV21FixedSceneFixedPoint,
  type CalendarLiveV21FixedFixedPoint,
} from "./calendar-live-v21-fixed-point.js";

export const CALENDAR_LIVE_V21_VERIFIER_VERSION = "calendar-live-v21-verifier-v1";

export function buildCalendarLiveV21RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV21RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV21RawNode[];
}

export interface CalendarLiveV21NormalizedScene {
...
export interface CalendarLiveV21SceneProof {
...
export function normalizeCalendarLiveV21Scene(
  root: CalendarLiveV21RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV21NormalizedScene {
  const canonicalBindings: CalendarLiveV21NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV21RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV21Scene<Instance>(
  normalized: CalendarLiveV21NormalizedScene,
...
): CalendarLiveV21SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV21FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV21ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV21PreCaptureGates(payload: unknown): void {
