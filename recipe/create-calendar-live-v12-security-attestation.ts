...
import { verifyCalendarLiveV12Authorization } from "./calendar-live-v12-authorization.js";
import { CALENDAR_LIVE_V12_TARGET } from "./calendar-live-v12-broker.js";
import {
  CALENDAR_LIVE_V12_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV12Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v12-session";
...
const attestation: CalendarLiveV12SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV12SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v12 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V12_SECURITY_ATTESTATION_DEFAULT_PATH,
);
