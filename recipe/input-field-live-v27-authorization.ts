...
import {
  ...
  INPUT_LIVE_V27_INDEX_PATH,
} from "./build-input-field-live-proof-v27.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV27HistoryExpectation = "pending" | "authorized";

export interface InputLiveV27AuthorizationArtifact {
  artifactVersion: "input-live-v27-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V27_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV27AntecedentIndex {
...
export interface InputLiveV27HistoryState {
  index?: InputLiveV27AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV27AuthorizationArtifact;
...
export interface InputLiveV27AuthorizationProof extends InputLiveV27TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V27_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V27_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V27_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V27_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v27-status") ||
...
export function validateInputLiveV27AntecedentIndex(
  index: InputLiveV27AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v27-antecedent-index-v1" ||
...
export function validateInputLiveV27UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV27History\(\s*["']/.test(source) ||
    /readInputLiveV27HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV27AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV27AuthorizationArtifact {
  ...
  ) as InputLiveV27AntecedentIndex;
  const indexFailures = validateInputLiveV27AntecedentIndex(index);
...
export function validateInputLiveV27AuthorizationArtifact(
  artifact: InputLiveV27AuthorizationArtifact | undefined,
  index: InputLiveV27AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v27-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v27" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V27_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V27_TARGET) ||
    !equalJson(
...
export function validateInputLiveV27History(
  state: InputLiveV27HistoryState,
  expected: InputLiveV27HistoryExpectation,
): string[] {
  const failures = validateInputLiveV27AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV27AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV27HistoryState(
  root = process.cwd(),
): InputLiveV27HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V27_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV27AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V27_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V27_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V27_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV27History(
  expected: InputLiveV27HistoryExpectation,
  root = process.cwd(),
): InputLiveV27HistoryState {
  const state = readInputLiveV27HistoryState(root);
  const failures = validateInputLiveV27History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV27Authorization(
  root = process.cwd(),
): InputLiveV27AuthorizationProof {
  const state = verifyInputLiveV27History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V27_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v27/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v27.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V27_TARGET,
    expectedDynamicTool: INPUT_LIVE_V27_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V27_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV27HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV27History(expected);
