...
import { INPUT_LIVE_V83_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v83.js";
import { verifyInputLiveV83Authorization } from "./input-field-live-v83-authorization.js";
import { runInputLiveV83Preflight } from "./input-field-live-v83-preflight.js";
import { InputLiveV83Orchestrator } from "./run-input-field-live-v83.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V83_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v83-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV83Authorization(root);
    runInputLiveV83Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV83Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
