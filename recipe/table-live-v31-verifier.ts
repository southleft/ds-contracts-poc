...
import {
  verifyTableLiveV31FixedSceneFixedPoint,
  type TableLiveV31FixedFixedPoint,
} from "./table-live-v31-fixed-point.js";

export const TABLE_LIVE_V31_VERIFIER_VERSION = "table-live-v31-verifier-v1";

export function buildTableLiveV31RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV31RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV31RawNode[];
}

export interface TableLiveV31NormalizedScene {
...
export interface TableLiveV31SceneProof {
...
export function normalizeTableLiveV31Scene(
  root: TableLiveV31RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV31NormalizedScene {
  const canonicalBindings: TableLiveV31NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV31RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV31Scene<Instance>(
  normalized: TableLiveV31NormalizedScene,
...
): TableLiveV31SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV31FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV31ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV31PreCaptureGates(payload: unknown): void {
