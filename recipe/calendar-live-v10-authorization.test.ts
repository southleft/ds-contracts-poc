...
import { CALENDAR_LIVE_V10_INDEX_PATH } from "./build-calendar-live-proof-v10.js";
import {
...
import {
  buildCalendarLiveV10AuthorizationArtifact,
  validateCalendarLiveV10AntecedentIndex,
  validateCalendarLiveV10AuthorizationArtifact,
  validateCalendarLiveV10History,
  validateCalendarLiveV10UnitTestSource,
  type CalendarLiveV10AntecedentIndex,
  type CalendarLiveV10AuthorizationArtifact,
  type CalendarLiveV10AuthorizationProof,
  type CalendarLiveV10HistoryState,
} from "./calendar-live-v10-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV10ControlFlowSource,
  validateCalendarLiveV10SecurityAttestation,
  type CalendarLiveV10SecurityAttestation,
} from "./calendar-live-v10-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V10_INDEX_PATH);
const index = (): CalendarLiveV10AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV10AntecedentIndex;

const artifact = (): CalendarLiveV10AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV10AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV10HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV10HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV10AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v10/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV10AuthorizationArtifact,
): CalendarLiveV10SecurityAttestation => {
  const body: Omit<CalendarLiveV10SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V10_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV10History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV10History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV10History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV10History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV10History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV10UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v10-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV10UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV10AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV10AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV10AuthorizationArtifact(
        planted as CalendarLiveV10AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV10SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV10SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV10SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v10-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV10ControlFlowSource(source), []);
});
