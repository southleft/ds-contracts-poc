...
import {
  ...
  INPUT_LIVE_V68_INDEX_PATH,
} from "./build-input-field-live-proof-v68.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV68HistoryExpectation = "pending" | "authorized";

export interface InputLiveV68AuthorizationArtifact {
  artifactVersion: "input-live-v68-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V68_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV68AntecedentIndex {
...
export interface InputLiveV68HistoryState {
  index?: InputLiveV68AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV68AuthorizationArtifact;
...
export interface InputLiveV68AuthorizationProof extends InputLiveV68TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V68_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V68_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V68_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V68_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v68-authorization") ||
  artifactPath.includes("input-field-live-v68-preflight") ||
  artifactPath.includes("input-field-live-v68-authorized") ||
  artifactPath.includes("input-field-live-pivot-v68-status") ||
...
export function validateInputLiveV68AntecedentIndex(
  index: InputLiveV68AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v68-antecedent-index-v1" ||
...
export function validateInputLiveV68UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV68History\(\s*["']/.test(source) ||
    /readInputLiveV68HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV68AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV68AuthorizationArtifact {
  ...
  ) as InputLiveV68AntecedentIndex;
  const indexFailures = validateInputLiveV68AntecedentIndex(index);
...
export function validateInputLiveV68AuthorizationArtifact(
  artifact: InputLiveV68AuthorizationArtifact | undefined,
  index: InputLiveV68AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v68-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v68" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V68_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V68_TARGET) ||
    !equalJson(
...
export function validateInputLiveV68History(
  state: InputLiveV68HistoryState,
  expected: InputLiveV68HistoryExpectation,
): string[] {
  const failures = validateInputLiveV68AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV68AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV68HistoryState(
  root = process.cwd(),
): InputLiveV68HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V68_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV68AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V68_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V68_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V68_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV68History(
  expected: InputLiveV68HistoryExpectation,
  root = process.cwd(),
): InputLiveV68HistoryState {
  const state = readInputLiveV68HistoryState(root);
  const failures = validateInputLiveV68History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV68Authorization(
  root = process.cwd(),
): InputLiveV68AuthorizationProof {
  const state = verifyInputLiveV68History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V68_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v68/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v68.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V68_TARGET,
    expectedDynamicTool: INPUT_LIVE_V68_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V68_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV68HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV68History(expected);
