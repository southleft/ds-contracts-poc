...
import { buildComboboxLiveV3CleanupRuntime } from "./combobox-live-v3-cleanup.js";
import {
  type ComboboxLiveV3FixedFixedPoint,
} from "./combobox-live-v3-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV3RestoreProgram,
  validateComboboxLiveV3RestorePayload,
  type ComboboxLiveV3RestorePayload,
} from "./combobox-live-v3-restore.js";
import {
  buildComboboxLiveV3RawPropertyRuntime,
  normalizeComboboxLiveV3Scene,
  type ComboboxLiveV3RawNode,
} from "./combobox-live-v3-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v3.js";
import {
...
export const COMBOBOX_LIVE_V3_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V3_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V3_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V3_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V3_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V3_SET_COUNT = 4;
export const COMBOBOX_LIVE_V3_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V3_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V3_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V3_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V3_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV3SourceId = (typeof COMBOBOX_LIVE_V3_SOURCE_IDS)[number];

export interface ComboboxLiveV3SourceIdentity {
  source: ComboboxLiveV3SourceId;
...
export interface ComboboxLiveV3WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV3ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV3SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV3RawNode;
    optionScene: ComboboxLiveV3RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV3RootProof {
  source: ComboboxLiveV3SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV3FixedFixedPoint;
}

export interface ComboboxLiveV3ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV3SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV3CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV3SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV3CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV3SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV3CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v3 ${label} must be an object`);
...
export function validateComboboxLiveV3WriterPayload(
  value: unknown,
): ComboboxLiveV3WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V3_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v3 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v3 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v3 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV3SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV3ExtractProgram(
  writer: ComboboxLiveV3WriterOwnership,
  sources: readonly ComboboxLiveV3SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v3 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V3_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V3_NAMESPACE)}
${buildComboboxLiveV3RawPropertyRuntime()}
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("COMBOBOX-V3-EXTRACT-PAGE");
...
if(get(page,"pageOwner")!=="recipe/combobox/"+${JSON.stringify(writer.runIdentity)})throw new Error("COMBOBOX-V3-EXTRACT-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>SET_IDS.has(node.id));
if(sets.length!==4)throw new Error("COMBOBOX-V3-EXTRACT-ROOTS:"+sets.length);
...
for(const set of sets){
  ...
  if(!IDENTITIES[adapterIdentity]||(kind!=="combobox"&&kind!=="option"))throw new Error("COMBOBOX-V3-EXTRACT-ADAPTER:"+adapterIdentity+":"+kind);
...
if(roots.length!==2||roots.some(root=>!root.comboboxScene||!root.optionScene))throw new Error("COMBOBOX-V3-EXTRACT-PAIR");
...
export function validateComboboxLiveV3ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV3WriterOwnership,
): ComboboxLiveV3ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v3 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V3_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v3 extract schema/two-root mismatch");
  return payload as ComboboxLiveV3ExtractPayload;
}

export function proveComboboxLiveV3Roots<Instance>(
  extract: ComboboxLiveV3ExtractPayload,
  sources: readonly (ComboboxLiveV3SourceIdentity & {
...
): ComboboxLiveV3RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v3 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v3 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV3Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV3Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV3FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v3 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v3 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v3 compile lost owned sets");
...
export function assertComboboxLiveV3RootProofs(
  proofs: readonly ComboboxLiveV3RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v3 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v3 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV3ProbeProgram(
  writer: ComboboxLiveV3WriterOwnership,
  sources: readonly ComboboxLiveV3SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V3_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE"||page.id==="115:295378")throw new Error("COMBOBOX-V3-PROBE-PAGE");
...
if(sets.length!==4)throw new Error("COMBOBOX-V3-PROBE-ROOTS:"+sets.length);
...
for(const row of byAdapter.values()){
 ...
 if(!source||!combobox||!option||combobox.children.length!==64||option.children.length!==8)throw new Error("COMBOBOX-V3-PROBE-VARIANTS:"+adapterIdentity);
 ...
 for(const component of combobox.children){const target=axes(component),updates={};for(const name of axisNames){const key=propertyKey(instance,name);if(!key)throw new Error("COMBOBOX-V3-PROBE-AXIS:"+name);updates[key]=target[name];}instance.setProperties(updates);const main=await instance.getMainComponentAsync();if(main)visited.add(main.id);}
 ...
 for(const component of option.children){const target=axes(component),updates={};for(const name of ["Size","Option state"]){const key=propertyKey(optionInstance,name);if(!key)throw new Error("COMBOBOX-V3-PROBE-OPTION-AXIS:"+name);updates[key]=target[name];}optionInstance.setProperties(updates);const main=await optionInstance.getMainComponentAsync();if(main)optionVisited.add(main.id);}
...
export function validateComboboxLiveV3ProbePayload(
  value: unknown,
  writer: ComboboxLiveV3WriterOwnership,
): ComboboxLiveV3ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV3ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V3_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V3_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v3 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV3CaptureManifest(
  cells: readonly ComboboxLiveV3CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V3_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V3_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V3_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V3_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v3 capture manifest is malformed");
}

export const comboboxLiveV3CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV3CaptureCell[],
): string => {
  validateComboboxLiveV3CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV3CaptureProgram(
  writer: ComboboxLiveV3WriterOwnership,
  cell: ComboboxLiveV3CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V3_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v3 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v3 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
if(!page||page.type!=="PAGE"||page.id==="115:295378"||!set||set.type!=="COMPONENT_SET")throw new Error("COMBOBOX-V3-CAPTURE-TARGET");
const axes=node=>Object.fromEntries(node.name.split(",").map(part=>part.trim()).filter(part=>part.includes("=")).map(part=>{const at=part.indexOf("=");return[part.slice(0,at),part.slice(at+1)];}));
...
if(!component)throw new Error("COMBOBOX-V3-CAPTURE-CELL:"+cell.cellKey);
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V3_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V3-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV3CapturePayload(
  value: unknown,
  cell: ComboboxLiveV3CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV3CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV3CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V3_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V3_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v3 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV3CaptureResponses(
  manifest: readonly ComboboxLiveV3CaptureCell[],
  responses: readonly ComboboxLiveV3CapturePayload[],
): void {
  validateComboboxLiveV3CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v3 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV3ObjectiveReport {
  artifactVersion: "combobox-live-v3-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV3Objective(
  manifest: readonly ComboboxLiveV3CaptureCell[],
  responses: readonly ComboboxLiveV3CapturePayload[],
): ComboboxLiveV3ObjectiveReport {
  assertComboboxLiveV3CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v3-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV3CleanupProgram(
  writer: ComboboxLiveV3WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV3CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V3_NAMESPACE,
  ...
  const guardedResult = `if(JSON.stringify(requestedNodeIds)!==JSON.stringify([expectedPageId])||JSON.stringify([...requestedCollectionIds].sort())!==JSON.stringify([...expectedCollectionIds].sort()))throw new Error("COMBOBOX-V3-CLEANUP-EXACT-OWNERSHIP");${resultStatement}`;
  const program = baseProgram.replace(resultStatement, guardedResult);
  if (program === baseProgram)
    throw new Error("Combobox live v3 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV3CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV3WriterOwnership,
): ComboboxLiveV3CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV3CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v3 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V3_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV3WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
