/**
 * Calendar live v19 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V19_FIXED_VERIFIER_VERSION = "calendar-live-v19-fixed-point-v1";
export const CALENDAR_LIVE_V19_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV19FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV19FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV19FixedSourceProbe {
...
export interface CalendarLiveV19FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV19FixedFixedPoint;
}

export interface CalendarLiveV19FixedHardGateInput {
  thresholds: CalendarLiveV19FixedThresholds;
  sourceProbes: CalendarLiveV19FixedSourceProbe[];
  cells: CalendarLiveV19FixedCellValidation[];
  sceneProofs: CalendarLiveV19FixedSceneProof[];
...
export interface CalendarLiveV19FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V19_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV19FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV19FixedHardGates(
  input: CalendarLiveV19FixedHardGateInput,
): CalendarLiveV19FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV19FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V19_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV19FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV19FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV19FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
