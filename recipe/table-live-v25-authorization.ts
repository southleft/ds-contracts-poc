...
export type TableLiveV25HistoryExpectation = "pending" | "authorized";

export interface TableLiveV25AuthorizationArtifact {
  artifactVersion: "table-live-v25-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V25_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV25AntecedentIndex {
...
export interface TableLiveV25HistoryState {
  index?: TableLiveV25AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV25AuthorizationArtifact;
...
export interface TableLiveV25AuthorizationProof
  extends TableLiveV25TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V25_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V25_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V25_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V25_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v25-authorization") ||
  artifactPath.includes("table-live-v25-preflight") ||
  artifactPath.includes("table-live-v25-authorized") ||
  artifactPath.includes("create-table-live-v25-security-attestation") ||
  artifactPath.includes("table-live-pivot-v25-status") ||
...
export function validateTableLiveV25AntecedentIndex(
  index: TableLiveV25AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v25-antecedent-index-v1" ||
...
export function validateTableLiveV25UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV25History\(\s*["']/.test(source) ||
    /readTableLiveV25HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV25AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV25AuthorizationArtifact {
  ...
  ) as TableLiveV25AntecedentIndex;
  const indexFailures = validateTableLiveV25AntecedentIndex(index);
...
export function validateTableLiveV25AuthorizationArtifact(
  artifact: TableLiveV25AuthorizationArtifact | undefined,
  index: TableLiveV25AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v25-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v25" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V25_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V25_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V25_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V25_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V25_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V25_SOURCE_ROOTS ||
...
export function validateTableLiveV25History(
  state: TableLiveV25HistoryState,
  expected: TableLiveV25HistoryExpectation,
): string[] {
  const failures = validateTableLiveV25AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV25AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV25HistoryState(
  root = process.cwd(),
): TableLiveV25HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V25_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV25AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V25_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V25_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V25_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV25History(
  expected: TableLiveV25HistoryExpectation,
  root = process.cwd(),
): TableLiveV25HistoryState {
  const state = readTableLiveV25HistoryState(root);
  const failures = validateTableLiveV25History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v25 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV25Authorization(
  root = process.cwd(),
): TableLiveV25AuthorizationProof {
  const state = verifyTableLiveV25History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V25_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v25/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v25.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V25_TARGET,
    expectedDynamicTool: TABLE_LIVE_V25_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V25_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV25HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV25History(expected);
  process.stdout.write(
    `Table live v25 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
