...
import { buildComboboxLiveV14CleanupRuntime } from "./combobox-live-v14-cleanup.js";
import {
  type ComboboxLiveV14FixedFixedPoint,
} from "./combobox-live-v14-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV14RestoreProgram,
  validateComboboxLiveV14RestorePayload,
  type ComboboxLiveV14RestorePayload,
} from "./combobox-live-v14-restore.js";
import {
  buildComboboxLiveV14RawPropertyRuntime,
  normalizeComboboxLiveV14Scene,
  type ComboboxLiveV14RawNode,
} from "./combobox-live-v14-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v14.js";
import {
...
} from "./scene-readback-combobox-v14.js";
...
export const COMBOBOX_LIVE_V14_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V14_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V14_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V14_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V14_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V14_SET_COUNT = 4;
export const COMBOBOX_LIVE_V14_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V14_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V14_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V14_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V14_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV14SourceId = (typeof COMBOBOX_LIVE_V14_SOURCE_IDS)[number];

export interface ComboboxLiveV14SourceIdentity {
  source: ComboboxLiveV14SourceId;
...
export interface ComboboxLiveV14WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV14ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV14SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV14RawNode;
    optionScene: ComboboxLiveV14RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV14RootProof {
  source: ComboboxLiveV14SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV14FixedFixedPoint;
}

export interface ComboboxLiveV14ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV14SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV14CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV14SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV14CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV14SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV14CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v14 ${label} must be an object`);
...
export function validateComboboxLiveV14WriterPayload(
  value: unknown,
): ComboboxLiveV14WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V14_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v14 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v14 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v14 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV14SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV14ExtractProgram(
  writer: ComboboxLiveV14WriterOwnership,
  sources: readonly ComboboxLiveV14SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v14 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V14_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V14_NAMESPACE)}
${buildComboboxLiveV14RawPropertyRuntime()}
...
export function validateComboboxLiveV14ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV14WriterOwnership,
): ComboboxLiveV14ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v14 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V14_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v14 extract schema/two-root mismatch");
  return payload as ComboboxLiveV14ExtractPayload;
}

export function proveComboboxLiveV14Roots<Instance>(
  extract: ComboboxLiveV14ExtractPayload,
  sources: readonly (ComboboxLiveV14SourceIdentity & {
...
): ComboboxLiveV14RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v14 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v14 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV14Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV14Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV14FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v14 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v14 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v14 compile lost owned sets");
...
export function assertComboboxLiveV14RootProofs(
  proofs: readonly ComboboxLiveV14RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v14 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v14 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV14ProbeProgram(
  writer: ComboboxLiveV14WriterOwnership,
  sources: readonly ComboboxLiveV14SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V14_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV14ProbePayload(
  value: unknown,
  writer: ComboboxLiveV14WriterOwnership,
): ComboboxLiveV14ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV14ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V14_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V14_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v14 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV14CaptureManifest(
  cells: readonly ComboboxLiveV14CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V14_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V14_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V14_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V14_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v14 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV14CaptureCell[],
): string => {
  validateComboboxLiveV14CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV14CaptureProgram(
  writer: ComboboxLiveV14WriterOwnership,
  cell: ComboboxLiveV14CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V14_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v14 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v14 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V14_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV14CapturePayload(
  value: unknown,
  cell: ComboboxLiveV14CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV14CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV14CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V14_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V14_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v14 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV14CaptureResponses(
  manifest: readonly ComboboxLiveV14CaptureCell[],
  responses: readonly ComboboxLiveV14CapturePayload[],
): void {
  validateComboboxLiveV14CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v14 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV14ObjectiveReport {
  artifactVersion: "combobox-live-v14-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV14Objective(
  manifest: readonly ComboboxLiveV14CaptureCell[],
  responses: readonly ComboboxLiveV14CapturePayload[],
): ComboboxLiveV14ObjectiveReport {
  assertComboboxLiveV14CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v14-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV14CleanupProgram(
  writer: ComboboxLiveV14WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV14CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V14_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v14 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV14CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV14WriterOwnership,
): ComboboxLiveV14CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV14CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v14 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V14_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV14WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
