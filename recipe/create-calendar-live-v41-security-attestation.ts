...
import { verifyCalendarLiveV41Authorization } from "./calendar-live-v41-authorization.js";
import { CALENDAR_LIVE_V41_TARGET } from "./calendar-live-v41-broker.js";
import {
  CALENDAR_LIVE_V41_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV41Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v41-session";
...
const attestation: CalendarLiveV41SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV41SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v41 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V41_SECURITY_ATTESTATION_DEFAULT_PATH,
);
