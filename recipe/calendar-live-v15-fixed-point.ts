/**
 * Calendar live v15 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V15_FIXED_VERIFIER_VERSION = "calendar-live-v15-fixed-point-v1";
export const CALENDAR_LIVE_V15_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV15FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV15FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV15FixedSourceProbe {
...
export interface CalendarLiveV15FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV15FixedFixedPoint;
}

export interface CalendarLiveV15FixedHardGateInput {
  thresholds: CalendarLiveV15FixedThresholds;
  sourceProbes: CalendarLiveV15FixedSourceProbe[];
  cells: CalendarLiveV15FixedCellValidation[];
  sceneProofs: CalendarLiveV15FixedSceneProof[];
...
export interface CalendarLiveV15FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V15_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV15FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV15FixedHardGates(
  input: CalendarLiveV15FixedHardGateInput,
): CalendarLiveV15FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV15FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V15_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV15FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV15FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV15FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
