...
import { tableLiveV2Sha256 } from "./table-live-v38-broker.js";

export interface TableLiveV38LifecycleSimulationReport {
  artifactVersion: "table-live-v38-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV38AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV38Lifecycle(): Promise<TableLiveV38LifecycleSimulationReport> {
  await buildTableLiveV38Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V38_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV38AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v38-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V38_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V38_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV38History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV38History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV38AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V38_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V38_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV38History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV38History(authorized, "pending");
    ...
    await buildTableLiveV38Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V38_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV38AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV38Lifecycle(), null, 2)}\n`,
  );
