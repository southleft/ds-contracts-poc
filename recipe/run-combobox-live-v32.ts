...
import {
  acceptComboboxLiveV32Response,
  createComboboxLiveV32Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV32Request,
  persistComboboxLiveV32RawResponse,
  persistComboboxLiveV32TechnicalGates,
  readComboboxLiveV32AcceptedRawPayload,
  type ComboboxLiveV32Request,
  type ComboboxLiveV32TransactionAuthorization,
} from "./combobox-live-v32-broker.js";
import {
  COMBOBOX_LIVE_V32_CAPTURE_COUNT,
  assertComboboxLiveV32CaptureResponses,
  ...
  evaluateComboboxLiveV32Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV32Roots,
  validateComboboxLiveV32CaptureManifest,
  validateComboboxLiveV32CapturePayload,
  validateComboboxLiveV32CleanupPayload,
  validateComboboxLiveV32ExtractPayload,
  validateComboboxLiveV32ProbePayload,
  validateComboboxLiveV32WriterPayload,
  type ComboboxLiveV32CaptureCell,
  type ComboboxLiveV32CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V32_PLAN_PATH,
  type ComboboxLiveV32ProofPlan,
} from "./build-combobox-live-proof-v32.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v32.js";

export const COMBOBOX_LIVE_V32_RUNNER_VERSION = "combobox-live-v32-runner-v1";

interface ComboboxLiveV32JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV32OrchestratorState {
  artifactVersion: "combobox-live-v32-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV32JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV32ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV32CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV32SourceIdentity & {
...
export interface ComboboxLiveV32NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV32OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v32 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V32_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V32_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV32ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV32CaptureCell[];
  validateComboboxLiveV32CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v32 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v32 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV32WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV32RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV32Request =>
  readJson<ComboboxLiveV32Request>(file);

export class ComboboxLiveV32Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV32Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV32OrchestratorState = {
      artifactVersion: "combobox-live-v32-orchestrator-state-v1",
    ...
    return new ComboboxLiveV32Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV32OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV32Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV32NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV32NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v32 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV32NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v32 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV32RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV32Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV32CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV32AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV32WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV32Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV32ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV32Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV32Response(this.#directory, "writer", {
        validate: validateComboboxLiveV32WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV32Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV32RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV32Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV32ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV32Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV32RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v32-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV32Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV32ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV32Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV32ProbePayload(payload, ownership),
      });
      persistComboboxLiveV32TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v32 capture cell missing");
      const accepted = acceptComboboxLiveV32Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV32CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V32_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV32CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV32Objective(
    ...
    throw new Error(`Combobox live v32 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV32Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV32CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV32Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV32RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV32Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV32CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV32CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV32CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV32Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV32TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV32SimulationKey = (): KeyObject =>
