/**
 * Carried Combobox live v39 host verifier. Uses scene-readback-combobox-v39 so live
...
export const COMBOBOX_LIVE_V39_FIXED_VERIFIER_VERSION = "combobox-live-v39-fixed-point-v1";
export const COMBOBOX_LIVE_V39_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV39FixedThresholds {
...
export interface ComboboxLiveV39FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV39FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV39FixedSourceProbe {
...
export interface ComboboxLiveV39FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV39FixedFixedPoint;
}

export interface ComboboxLiveV39FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV39FixedHardGateInput {
  thresholds: ComboboxLiveV39FixedThresholds;
  materialRegression: ComboboxLiveV39FixedMaterialRegression;
  sourceProbes: ComboboxLiveV39FixedSourceProbe[];
  cells: ComboboxLiveV39FixedCellValidation[];
  sceneProofs: ComboboxLiveV39FixedSceneProof[];
  visualRows: ComboboxLiveV39FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV39FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V39_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV39FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV39FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV39FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV39FixedHardGates(
  input: ComboboxLiveV39FixedHardGateInput,
): ComboboxLiveV39FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV39FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV39FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V39_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV39FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV39FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
      ...
      comparison: compareSceneToExpectedPlan(
        ...
        scene,
        {
          omitOpacityOwnershipKeys:
            contentTextOwnershipKeysWithoutCompileOpacity(compiled.ir),
        },
      ),
    };
  };
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV39FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
