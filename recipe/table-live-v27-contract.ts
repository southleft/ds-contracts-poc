...
import { buildTableLiveV27CleanupRuntime } from "./table-live-v27-cleanup.js";
import { type TableLiveV27FixedFixedPoint } from "./table-live-v27-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV27RestoreProgram,
  validateTableLiveV27RestorePayload,
  type TableLiveV27RestorePayload,
} from "./table-live-v27-restore.js";
import {
  buildTableLiveV27RawPropertyRuntime,
  normalizeTableLiveV27Scene,
  type TableLiveV27RawNode,
} from "./table-live-v27-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v27.js";
import {
...
export const TABLE_LIVE_V27_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V27_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V27_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V27_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V27_VARIANT_COUNT = 20;
export const TABLE_LIVE_V27_SET_COUNT = 6;
export const TABLE_LIVE_V27_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V27_HOST_PHASES = 3;
export const TABLE_LIVE_V27_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V27_RESTORE_COUNT = 8;
export const TABLE_LIVE_V27_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V27_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV27SourceId = (typeof TABLE_LIVE_V27_SOURCE_IDS)[number];
export type TableLiveV27Kind = "table" | "row" | "cell";

export interface TableLiveV27SourceIdentity {
  source: TableLiveV27SourceId;
...
export interface TableLiveV27WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV27ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV27SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV27RawNode;
    rowScene: TableLiveV27RawNode;
    cellScene: TableLiveV27RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV27RootProof {
  source: TableLiveV27SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV27FixedFixedPoint;
}

export interface TableLiveV27ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV27SourceId;
    adapterIdentity: string;
...
export interface TableLiveV27CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV27SourceId;
  adapterIdentity: string;
...
export interface TableLiveV27CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV27SourceId;
  frameWidth: number;
...
export interface TableLiveV27CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v27 ${label} must be an object`);
...
export function validateTableLiveV27WriterPayload(
  value: unknown,
): TableLiveV27WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V27_NAMESPACE ||
    ...
    throw new TypeError("Table live v27 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Table live v27 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v27 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV27SourceIdentity,
  kind: TableLiveV27Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV27ExtractProgram(
  writer: TableLiveV27WriterOwnership,
  sources: readonly TableLiveV27SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
      "Table live v27 extract requires two source identities",
    );
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V27_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V27_NAMESPACE)}
${buildTableLiveV27RawPropertyRuntime()}
...
export function validateTableLiveV27ExtractPayload(
  value: unknown,
  writer: TableLiveV27WriterOwnership,
): TableLiveV27ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v27 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V27_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v27 extract schema/two-root mismatch");
  return payload as TableLiveV27ExtractPayload;
}

export function proveTableLiveV27Roots<Instance>(
  extract: TableLiveV27ExtractPayload,
  sources: readonly (TableLiveV27SourceIdentity & {
...
): TableLiveV27RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v27 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v27 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV27Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV27Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV27Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV27FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Table live v27 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v27 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v27 compile lost owned sets");
...
export function assertTableLiveV27RootProofs(
  proofs: readonly TableLiveV27RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v27 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v27 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV27ProbeProgram(
  writer: TableLiveV27WriterOwnership,
  sources: readonly TableLiveV27SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V27_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV27ProbePayload(
  value: unknown,
  writer: TableLiveV27WriterOwnership,
): TableLiveV27ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV27ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V27_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v27 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV27CaptureManifest(
  cells: readonly TableLiveV27CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V27_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V27_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V27_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V27_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v27 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV27CaptureCell[],
): string => {
  validateTableLiveV27CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV27CaptureProgram(
  writer: TableLiveV27WriterOwnership,
  cell: TableLiveV27CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V27_CAPTURE_COUNT
  )
    throw new TypeError("Table live v27 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Table live v27 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V27_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV27CapturePayload(
  value: unknown,
  cell: TableLiveV27CaptureCell,
  rawResponseBytes: number,
): TableLiveV27CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV27CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V27_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V27_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v27 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV27CaptureResponses(
  manifest: readonly TableLiveV27CaptureCell[],
  responses: readonly TableLiveV27CapturePayload[],
): void {
  validateTableLiveV27CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v27 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV27ObjectiveReport {
  artifactVersion: "table-live-v27-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV27Objective(
  manifest: readonly TableLiveV27CaptureCell[],
  responses: readonly TableLiveV27CapturePayload[],
): TableLiveV27ObjectiveReport {
  assertTableLiveV27CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v27-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV27CleanupProgram(
  writer: TableLiveV27WriterOwnership,
): string {
  const baseProgram = buildTableLiveV27CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V27_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v27 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV27CleanupPayload(
  payload: unknown,
  writer: TableLiveV27WriterOwnership,
): TableLiveV27CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as TableLiveV27CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v27 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V27_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV27WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
