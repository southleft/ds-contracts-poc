...
import {
  compareSceneToExpectedPlan as compareSceneToExpectedPlanV37,
  compileExpectedScenePlan as compileExpectedScenePlanV37,
  sceneToNormalizedIr as sceneToNormalizedIrV37,
} from "./scene-readback-combobox-v37.js";
import { sceneToNormalizedIr as sceneToNormalizedIrV38 } from "./scene-readback-combobox-v38.js";
import { sceneToNormalizedIr as sceneToNormalizedIrV39 } from "./scene-readback-combobox-v39.js";
import { sceneToNormalizedIr as sceneToNormalizedIrV40 } from "./scene-readback-combobox-v40.js";
import {
  ...
  COMBOBOX_LIVE_V42_LEADING_SLOT_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V42_LEADING_SLOT_COMPILE_CARRY_VISIBLE_MARKER,
  COMBOBOX_LIVE_V42_LISTBOX_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V42_LISTBOX_CLIPS_CONTENT_OMITTED_MARKER,
  COMBOBOX_LIVE_V42_LISTBOX_CORNER_RADIUS_OMITTED_MARKER,
  COMBOBOX_LIVE_V42_LISTBOX_EMPTY_EFFECTS_OMITTED_MARKER,
  COMBOBOX_LIVE_V42_LISTBOX_EMPTY_STROKES_OMITTED_MARKER,
  COMBOBOX_LIVE_V42_OMIT_EMPTY_INSTANCE_PAYLOAD_MARKER,
  COMBOBOX_LIVE_V42_OBSERVE_OMIT_EMPTY_INSTANCE_PAYLOAD_MARKER,
  COMBOBOX_LIVE_V42_OBSERVE_OMIT_OPTION_INSTANCE_PAYLOAD_MARKER,
  COMBOBOX_LIVE_V42_OBSERVE_OPTION_SET_COMPILE_CARRY_NAME_MARKER,
  COMBOBOX_LIVE_V42_OPTION_SET_COMPILE_CARRY_LABEL,
  COMBOBOX_LIVE_V42_OPTION_SET_COMPILE_CARRY_LABEL_MARKER,
  COMBOBOX_LIVE_V42_OPTION_SET_COMPILE_CARRY_NAME,
  COMBOBOX_LIVE_V42_OPTION_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V42_OPTION_CLIPS_CONTENT_OMITTED_MARKER,
  COMBOBOX_LIVE_V42_OPTION_LAYOUT_HEIGHT_ALIAS_MARKER,
  COMBOBOX_LIVE_V42_OPTION_INSTANCE_BINDING_EXTRAS_MARKER,
  COMBOBOX_LIVE_V42_OPTION_INSTANCE_FILLS_OMITTED_MARKER,
  COMBOBOX_LIVE_V42_OPTION_INSTANCE_PAYLOAD_OMITTED_MARKER,
  COMBOBOX_LIVE_V42_OVERLAY_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V42_OVERLAY_EMPTY_DASH_PATTERN_OMITTED_MARKER,
  COMBOBOX_LIVE_V42_SET_CLIPS_CONTENT_OMITTED_MARKER,
  COMBOBOX_LIVE_V42_OVERLAY_LAYOUT_WIDTH_ALIAS_MARKER,
  COMBOBOX_LIVE_V42_PROJECT_LIVE_ROOT_OWNERSHIP_KEY_MARKER,
  COMBOBOX_LIVE_V42_RECOVER_COMPONENT_PROPERTY_NAME_MARKER,
  COMBOBOX_LIVE_V42_RECOVER_RECIPE_COMPONENT_REF_MARKER,
  COMBOBOX_LIVE_V42_SELECTED_INDICATOR_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V42_SELECTED_INDICATOR_COMPILE_CARRY_VISIBLE_MARKER,
  COMBOBOX_LIVE_V42_TRAILING_SLOT_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V42_TRAILING_SLOT_COMPILE_CARRY_VISIBLE_MARKER,
  COMBOBOX_LIVE_V42_TRIGGER_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V42_TRIGGER_COMPILE_CARRY_CHARACTERS,
  COMBOBOX_LIVE_V42_TRIGGER_COMPILE_CARRY_CHARACTERS_MARKER,
  COMBOBOX_LIVE_V42_TRIGGER_EMPTY_EFFECTS_OMITTED_MARKER,
  COMBOBOX_LIVE_V42_TRIGGER_SLOT_COMPONENT_REF_COMPILE_SIBLING_ORDER_MARKER,
  COMBOBOX_LIVE_V42_TRIGGER_SLOT_COMPILE_SIBLING_COMPONENT_REFS,
  COMBOBOX_LIVE_V42_TRIGGER_SLOT_COMPILE_SIBLING_ROLES,
...
test("compile trigger-slot componentRefs are prefix then clear then popup", () => {
  ...
  const refs = walkIr(trigger)
    .filter(
      (node) =>
        node.kind === "instance" &&
        (COMBOBOX_LIVE_V42_TRIGGER_SLOT_COMPILE_SIBLING_ROLES as readonly string[]).includes(
          node.role ?? "",
        ),
    )
    .map((node) =>
...
test("hashed v37 scene-readback bytes stay frozen", () => {
  const bytes = readFileSync("recipe/scene-readback-combobox-v37.ts", "utf8");
  assert.doesNotMatch(
    bytes,
    /COMBOBOX-HOST-OBSERVE-OPTION-SET-COMPILE-CARRY-NAME/,
  );
  assert.match(bytes, /COMBOBOX-TRIGGER-COMPILE-CARRY-CHARACTERS/);
  assert.match(bytes, /canonicalizeObservedFontProvenanceName\(scene\.name\)/);
});

test("hashed v38 scene-readback bytes stay frozen", () => {
  const bytes = readFileSync("recipe/scene-readback-combobox-v38.ts", "utf8");
  assert.match(
    bytes,
  ...
  assert.match(bytes, /COMBOBOX-HOST-OBSERVE-OPTION-SET-COMPILE-CARRY-NAME/);
  assert.match(bytes, /canonicalizeObservedTriggerSlotComponentRef/);
});

test("hashed v40 scene-readback bytes stay frozen", () => {
  const bytes = readFileSync("recipe/scene-readback-combobox-v40.ts", "utf8");
  assert.doesNotMatch(
    bytes,
    /COMBOBOX-PROBE-MEASURE-THEN-EXCLUDE-OVERLAY-OPTION-LABEL-MESSAGE-OVERLAP/,
  );
  assert.doesNotMatch(
    bytes,
  ...
  const live = liveTriggerSelectedFirstScene();
  assert.deepEqual(
    walkIr(sceneToNormalizedIrV40(live))
      .filter(
        (node) =>
          node.kind === "instance" &&
          (
      ...
      .map((node) =>
        node.kind === "instance"
          ? componentRefLastSegment(node.componentRef)
          : "",
      ),
    [...COMBOBOX_LIVE_V42_TRIGGER_SLOT_COMPILE_SIBLING_COMPONENT_REFS],
  );
...
test("hashed v39 scene-readback bytes stay frozen", () => {
  const bytes = readFileSync("recipe/scene-readback-combobox-v39.ts", "utf8");
  assert.match(
    bytes,
    /kind: "instance",[\s\S]*?componentRef: canonicalizeObservedTriggerSlotComponentRef\(/,
  );
  ...
  const live = liveTriggerSelectedFirstScene();
  assert.deepEqual(
    walkIr(sceneToNormalizedIrV39(live))
      .filter(
        (node) =>
          node.kind === "instance" &&
          (
...
test("host sceneToNormalizedIr recovers trigger-slot componentRefs in compile sibling order, not live selected-first order", () => {
  const live = liveTriggerSelectedFirstScene();
  const hashed = sceneToNormalizedIrV38(live);
  const host = sceneToNormalizedIr(live);
  const refsOf = (root: IRNode): string[] =>
    walkIr(root)
      .filter(
        (node) =>
          node.kind === "instance" &&
  ...
  assert.deepEqual(
    refsOf(hashed),
    ["selected", "prefix", "clear"],
    "hashed v38 sceneToNormalizedIr still emits live selected-first refs into collapse",
  );
  assert.deepEqual(
    refsOf(host),
    [...COMBOBOX_LIVE_V42_TRIGGER_SLOT_COMPILE_SIBLING_COMPONENT_REFS],
    "v40 sceneToNormalizedIr must recover prefix then clear then popup",
  );
  assert.equal(
    canonicalizeObservedTriggerSlotComponentRef(
      "selected",
...
test("host observe recovers compile-carried option-set name, not the live display name", () => {
  assert.equal(
    COMBOBOX_LIVE_V42_OBSERVE_OPTION_SET_COMPILE_CARRY_NAME_MARKER,
    "COMBOBOX-HOST-OBSERVE-OPTION-SET-COMPILE-CARRY-NAME",
  );
  assert.equal(
    COMBOBOX_LIVE_V42_OPTION_SET_COMPILE_CARRY_NAME,
    "combobox/option-set :: Combobox option",
    "v40 must recover compile's option-set name, not invent a name or parse the live fixture string",
  );
  assert.equal(
    COMBOBOX_LIVE_V42_OPTION_SET_COMPILE_CARRY_LABEL,
    "Combobox option",
    "v40 reuses the v33 compile-carried Combobox option label",
  );
  const fixtures = [
    {
      displayName: "MUI Autocomplete / combobox@1 offline fixture",
  ...
  for (const fixture of fixtures) {
    const live = liveOptionSetLabelScene(fixture.displayName);
    const hashedIr = sceneToNormalizedIrV37(live);
    const hashedPlan = compileExpectedScenePlanV37(hashedIr, {
      rootOwnershipKey: live.ownershipKey,
    });
    const hashedName = hashedPlan.facts.find((fact) => fact.channel === "name");
    assert.equal(
      hashedName?.value,
      "combobox/option-set :: Combobox option",
      "hashed v37 IR already compile-carries the option-set label into the projected name",
    );
    const hashedComparison = compareSceneToExpectedPlanV37(hashedPlan, live);
    const hashedRootNameMismatch = hashedComparison.mismatched.find(
      (pair) =>
        pair.expected.channel === "name" &&
        pair.expected.nodeOwnershipKey === live.ownershipKey,
    );
    assert.ok(
      hashedRootNameMismatch,
      "hashed v37 observe still overlays the live option-set display name",
    );
    assert.equal(
      hashedRootNameMismatch.expected.value,
      "combobox/option-set :: Combobox option",
    );
    assert.equal(
      hashedRootNameMismatch.observed.value,
      `combobox/option-set :: ${fixture.displayName}`,
      "hashed v37 observed option-set name is the live fixture display name",
    );
    const hostIr = sceneToNormalizedIr(live);
    const hostPlan = compileExpectedScenePlan(hostIr, {
      rootOwnershipKey: live.ownershipKey,
    ...
    const hostComparison = compareSceneToExpectedPlan(hostPlan, live);
    const hostRootNameMismatch = hostComparison.mismatched.find(
      (pair) =>
        pair.expected.channel === "name" &&
        pair.expected.nodeOwnershipKey === live.ownershipKey,
    );
    assert.equal(
      hostRootNameMismatch,
      undefined,
      "v40 observe must recover compile-carried combobox/option-set :: Combobox option",
    );
    const hostName = hostPlan.facts.find(
      (fact) =>
        fact.channel === "name" && fact.nodeOwnershipKey === live.ownershipKey,
    );
    assert.equal(hostName?.value, "combobox/option-set :: Combobox option");
    const setLive = liveComboboxSetLabelScene(fixture.displayName);
    const setHashed = compareSceneToExpectedPlanV37(
      compileExpectedScenePlanV37(sceneToNormalizedIrV37(setLive), {
        rootOwnershipKey: setLive.ownershipKey,
      }),
      setLive,
    );
    const setHost = compareSceneToExpectedPlan(
      compileExpectedScenePlan(sceneToNormalizedIr(setLive), {
        rootOwnershipKey: setLive.ownershipKey,
      }),
      setLive,
    );
    assert.equal(
      setHashed.mismatched.some(
        (pair) =>
          pair.expected.channel === "name" &&
          pair.expected.nodeOwnershipKey === setLive.ownershipKey,
      ),
      false,
      "hashed v37 already matches combobox/set names that compile emits as identity.name",
    );
    assert.equal(
      setHost.mismatched.some(
        (pair) =>
          pair.expected.channel === "name" &&
          pair.expected.nodeOwnershipKey === setLive.ownershipKey,
      ),
      false,
      "v40 must not change combobox/set names that already match compile identity.name",
    );
    const instance = adaptReviewedCombobox(fixture.reviewed, fixture.config);
    const envelope = compileComboboxRecipe(instance);
    const compiledOptionSet = walkIr(envelope.ir).find(
    ...
    assert.ok(compiledOptionSet);
    assert.equal(compiledOptionSet.label, "Combobox option");
    assert.equal(
      compiledOptionSet.role,
      "combobox/option-set",
      "compile already emits combobox/option-set :: Combobox option; observe must recover it",
    );
  }
});
