...
import { buildCalendarLiveV44CleanupRuntime } from "./calendar-live-v44-cleanup.js";
import { type CalendarLiveV44FixedFixedPoint } from "./calendar-live-v44-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV44RestoreProgram,
  validateCalendarLiveV44RestorePayload,
  type CalendarLiveV44RestorePayload,
} from "./calendar-live-v44-restore.js";
import {
  buildCalendarLiveV44RawPropertyRuntime,
  normalizeCalendarLiveV44Scene,
  type CalendarLiveV44RawNode,
} from "./calendar-live-v44-verifier.js";
...
export const CALENDAR_LIVE_V44_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V44_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V44_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V44_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V44_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V44_SET_COUNT = 3;
export const CALENDAR_LIVE_V44_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V44_HOST_PHASES = 3;
export const CALENDAR_LIVE_V44_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V44_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V44_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V44_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV44SourceId = (typeof CALENDAR_LIVE_V44_SOURCE_IDS)[number];
export type CalendarLiveV44Kind = "calendar" | "week" | "day";

export interface CalendarLiveV44SourceIdentity {
  source: CalendarLiveV44SourceId;
...
export interface CalendarLiveV44WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV44ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV44SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV44RawNode;
    weekScene: CalendarLiveV44RawNode;
    dayScene: CalendarLiveV44RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV44RootProof {
  source: CalendarLiveV44SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV44FixedFixedPoint;
}

export interface CalendarLiveV44ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV44SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV44CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV44SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV44CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV44SourceId;
  frameWidth: number;
...
export interface CalendarLiveV44CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v44 ${label} must be an object`);
...
export function validateCalendarLiveV44WriterPayload(
  value: unknown,
): CalendarLiveV44WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v44") ||
    ...
    result.namespace !== CALENDAR_LIVE_V44_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v44 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v44 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v44 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV44SourceIdentity,
  kind: CalendarLiveV44Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV44ExtractProgram(
  writer: CalendarLiveV44WriterOwnership,
  sources: readonly CalendarLiveV44SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v44 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V44_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V44_NAMESPACE)}
${buildCalendarLiveV44RawPropertyRuntime()}
...
export function validateCalendarLiveV44ExtractPayload(
  value: unknown,
  writer: CalendarLiveV44WriterOwnership,
): CalendarLiveV44ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v44 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V44_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v44 extract schema/one-root mismatch");
  return payload as CalendarLiveV44ExtractPayload;
}

export function proveCalendarLiveV44Roots<Instance>(
  extract: CalendarLiveV44ExtractPayload,
  sources: readonly (CalendarLiveV44SourceIdentity & {
...
): CalendarLiveV44RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v44 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v44 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV44Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV44Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV44Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV44FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v44 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v44 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v44 compile lost owned sets");
...
export function assertCalendarLiveV44RootProofs(
  proofs: readonly CalendarLiveV44RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v44 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v44 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV44ProbeProgram(
  writer: CalendarLiveV44WriterOwnership,
  sources: readonly CalendarLiveV44SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V44_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV44ProbePayload(
  value: unknown,
  writer: CalendarLiveV44WriterOwnership,
): CalendarLiveV44ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV44ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V44_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V44_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v44 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV44CaptureManifest(
  cells: readonly CalendarLiveV44CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V44_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V44_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V44_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V44_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v44 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV44CaptureCell[],
): string => {
  validateCalendarLiveV44CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV44CaptureProgram(
  writer: CalendarLiveV44WriterOwnership,
  cell: CalendarLiveV44CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V44_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v44 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v44 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V44_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV44CapturePayload(
  value: unknown,
  cell: CalendarLiveV44CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV44CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV44CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V44_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V44_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v44 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV44CaptureResponses(
  manifest: readonly CalendarLiveV44CaptureCell[],
  responses: readonly CalendarLiveV44CapturePayload[],
): void {
  validateCalendarLiveV44CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v44 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV44ObjectiveReport {
  artifactVersion: "calendar-live-v44-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV44Objective(
  manifest: readonly CalendarLiveV44CaptureCell[],
  responses: readonly CalendarLiveV44CapturePayload[],
): CalendarLiveV44ObjectiveReport {
  assertCalendarLiveV44CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v44-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV44CleanupProgram(
  writer: CalendarLiveV44WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV44CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V44_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v44 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV44CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV44WriterOwnership,
): CalendarLiveV44CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV44CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v44 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V44_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV44WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
