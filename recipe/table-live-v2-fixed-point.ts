/**
 * Table live v2 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V2_FIXED_VERIFIER_VERSION = "table-live-v2-fixed-point-v1";
export const TABLE_LIVE_V2_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV2FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV2FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV2FixedSourceProbe {
...
export interface TableLiveV2FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV2FixedFixedPoint;
}

export interface TableLiveV2FixedHardGateInput {
  thresholds: TableLiveV2FixedThresholds;
  sourceProbes: TableLiveV2FixedSourceProbe[];
  cells: TableLiveV2FixedCellValidation[];
  sceneProofs: TableLiveV2FixedSceneProof[];
...
export interface TableLiveV2FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V2_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV2FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV2FixedHardGates(
  input: TableLiveV2FixedHardGateInput,
): TableLiveV2FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV2FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  if (input.visualRows.length !== 0)
    failures.push("table live v2 must not invent a visual comparison");
  ...
  return {
    verifierVersion: TABLE_LIVE_V2_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV2FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV2FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
