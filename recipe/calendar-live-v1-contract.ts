...
import { buildCalendarLiveV1CleanupRuntime } from "./calendar-live-v1-cleanup.js";
import { type CalendarLiveV1FixedFixedPoint } from "./calendar-live-v1-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV1RestoreProgram,
  validateCalendarLiveV1RestorePayload,
  type CalendarLiveV1RestorePayload,
} from "./calendar-live-v1-restore.js";
import {
  buildCalendarLiveV1RawPropertyRuntime,
  normalizeCalendarLiveV1Scene,
  type CalendarLiveV1RawNode,
} from "./calendar-live-v1-verifier.js";
...
export const CALENDAR_LIVE_V1_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V1_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V1_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V1_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V1_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V1_SET_COUNT = 3;
export const CALENDAR_LIVE_V1_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V1_HOST_PHASES = 3;
export const CALENDAR_LIVE_V1_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V1_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V1_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V1_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV1SourceId = (typeof CALENDAR_LIVE_V1_SOURCE_IDS)[number];
export type CalendarLiveV1Kind = "calendar" | "week" | "day";

export interface CalendarLiveV1SourceIdentity {
  source: CalendarLiveV1SourceId;
...
export interface CalendarLiveV1WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV1ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV1SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV1RawNode;
    weekScene: CalendarLiveV1RawNode;
    dayScene: CalendarLiveV1RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV1RootProof {
  source: CalendarLiveV1SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV1FixedFixedPoint;
}

export interface CalendarLiveV1ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV1SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV1CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV1SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV1CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV1SourceId;
  frameWidth: number;
...
export interface CalendarLiveV1CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v1 ${label} must be an object`);
...
export function validateCalendarLiveV1WriterPayload(
  value: unknown,
): CalendarLiveV1WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v1") ||
    ...
    result.namespace !== CALENDAR_LIVE_V1_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v1 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v1 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v1 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV1SourceIdentity,
  kind: CalendarLiveV1Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV1ExtractProgram(
  writer: CalendarLiveV1WriterOwnership,
  sources: readonly CalendarLiveV1SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v1 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V1_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V1_NAMESPACE)}
${buildCalendarLiveV1RawPropertyRuntime()}
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("CALENDAR-V1-EXTRACT-PAGE");
...
if(get(page,"pageOwner")!=="recipe/calendar/"+${JSON.stringify(writer.runIdentity)})throw new Error("CALENDAR-V1-EXTRACT-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>SET_IDS.has(node.id));
if(sets.length!==3)throw new Error("CALENDAR-V1-EXTRACT-ROOTS:"+sets.length);
...
for(const set of sets){
  ...
  if(!IDENTITIES[adapterIdentity]||(kind!=="calendar"&&kind!=="week"&&kind!=="day"))throw new Error("CALENDAR-V1-EXTRACT-ADAPTER:"+adapterIdentity+":"+kind);
...
if(roots.length!==1||roots.some(root=>!root.calendarScene||!root.weekScene||!root.dayScene))throw new Error("CALENDAR-V1-EXTRACT-TRIPLE");
...
export function validateCalendarLiveV1ExtractPayload(
  value: unknown,
  writer: CalendarLiveV1WriterOwnership,
): CalendarLiveV1ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v1 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V1_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v1 extract schema/one-root mismatch");
  return payload as CalendarLiveV1ExtractPayload;
}

export function proveCalendarLiveV1Roots<Instance>(
  extract: CalendarLiveV1ExtractPayload,
  sources: readonly (CalendarLiveV1SourceIdentity & {
...
): CalendarLiveV1RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v1 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v1 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV1Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV1Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV1Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV1FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v1 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v1 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v1 compile lost owned sets");
...
export function assertCalendarLiveV1RootProofs(
  proofs: readonly CalendarLiveV1RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v1 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v1 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV1ProbeProgram(
  writer: CalendarLiveV1WriterOwnership,
  sources: readonly CalendarLiveV1SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V1_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE"||page.id==="115:295378"||page.id==="163:35981")throw new Error("CALENDAR-V1-PROBE-PAGE");
...
if(sets.length!==3)throw new Error("CALENDAR-V1-PROBE-ROOTS:"+sets.length);
...
for(const row of byAdapter.values()){
 ...
 if(!source||!calendar||!week||!day||calendar.children.length!==2||week.children.length!==2||day.children.length!==4)throw new Error("CALENDAR-V1-PROBE-VARIANTS:"+adapterIdentity);
...
export function validateCalendarLiveV1ProbePayload(
  value: unknown,
  writer: CalendarLiveV1WriterOwnership,
): CalendarLiveV1ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV1ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V1_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V1_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v1 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV1CaptureManifest(
  cells: readonly CalendarLiveV1CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V1_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V1_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V1_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V1_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v1 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV1CaptureCell[],
): string => {
  validateCalendarLiveV1CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV1CaptureProgram(
  writer: CalendarLiveV1WriterOwnership,
  cell: CalendarLiveV1CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V1_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v1 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v1 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
if(!page||page.type!=="PAGE"||page.id==="115:295378"||page.id==="163:35981"||!set||set.type!=="COMPONENT_SET")throw new Error("CALENDAR-V1-CAPTURE-TARGET");
const axes=node=>Object.fromEntries(node.name.split(",").map(part=>part.trim()).filter(part=>part.includes("=")).map(part=>{const at=part.indexOf("=");return[part.slice(0,at),part.slice(at+1)];}));
...
if(!component)throw new Error("CALENDAR-V1-CAPTURE-CELL:"+cell.cellKey);
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V1_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V1-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV1CapturePayload(
  value: unknown,
  cell: CalendarLiveV1CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV1CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV1CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V1_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V1_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v1 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV1CaptureResponses(
  manifest: readonly CalendarLiveV1CaptureCell[],
  responses: readonly CalendarLiveV1CapturePayload[],
): void {
  validateCalendarLiveV1CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v1 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV1ObjectiveReport {
  artifactVersion: "calendar-live-v1-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV1Objective(
  manifest: readonly CalendarLiveV1CaptureCell[],
  responses: readonly CalendarLiveV1CapturePayload[],
): CalendarLiveV1ObjectiveReport {
  assertCalendarLiveV1CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v1-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV1CleanupProgram(
  writer: CalendarLiveV1WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV1CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V1_NAMESPACE,
  ...
  const guardedResult = `if(JSON.stringify(requestedNodeIds)!==JSON.stringify([expectedPageId])||JSON.stringify([...requestedCollectionIds].sort())!==JSON.stringify([...expectedCollectionIds].sort()))throw new Error("CALENDAR-V1-CLEANUP-EXACT-OWNERSHIP");${resultStatement}`;
  const program = baseProgram.replace(resultStatement, guardedResult);
  if (program === baseProgram)
    throw new Error("Calendar live v1 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV1CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV1WriterOwnership,
): CalendarLiveV1CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV1CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v1 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V1_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV1WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
