...
import { calendarLiveV1Sha256 } from "./calendar-live-v31-broker.js";

export interface CalendarLiveV31LifecycleSimulationReport {
  artifactVersion: "calendar-live-v31-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV31AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV31Lifecycle(): Promise<CalendarLiveV31LifecycleSimulationReport> {
  await buildCalendarLiveV31Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V31_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV31AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v31-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V31_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V31_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV31History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV31History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV31AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V31_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V31_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV31History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV31History(authorized, "pending");
    ...
    await buildCalendarLiveV31Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V31_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV31AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV31Lifecycle(), null, 2)}\n`,
  );
