...
import {
  verifyTableLiveV10FixedSceneFixedPoint,
  type TableLiveV10FixedFixedPoint,
} from "./table-live-v10-fixed-point.js";

export const TABLE_LIVE_V10_VERIFIER_VERSION = "table-live-v10-verifier-v1";

export function buildTableLiveV10RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV10RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV10RawNode[];
}

export interface TableLiveV10NormalizedScene {
...
export interface TableLiveV10SceneProof {
...
export function normalizeTableLiveV10Scene(
  root: TableLiveV10RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV10NormalizedScene {
  const canonicalBindings: TableLiveV10NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV10RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV10Scene<Instance>(
  normalized: TableLiveV10NormalizedScene,
...
): TableLiveV10SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV10FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV10ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV10PreCaptureGates(payload: unknown): void {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("table live v10 pre-capture gate payload absent");
