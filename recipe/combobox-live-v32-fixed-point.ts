/**
 * Carried Combobox live v32 host verifier. Uses scene-readback-combobox-v32 so live
...
export const COMBOBOX_LIVE_V32_FIXED_VERIFIER_VERSION = "combobox-live-v32-fixed-point-v1";
export const COMBOBOX_LIVE_V32_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV32FixedThresholds {
...
export interface ComboboxLiveV32FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV32FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV32FixedSourceProbe {
...
export interface ComboboxLiveV32FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV32FixedFixedPoint;
}

export interface ComboboxLiveV32FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV32FixedHardGateInput {
  thresholds: ComboboxLiveV32FixedThresholds;
  materialRegression: ComboboxLiveV32FixedMaterialRegression;
  sourceProbes: ComboboxLiveV32FixedSourceProbe[];
  cells: ComboboxLiveV32FixedCellValidation[];
  sceneProofs: ComboboxLiveV32FixedSceneProof[];
  visualRows: ComboboxLiveV32FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV32FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V32_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV32FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV32FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV32FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV32FixedHardGates(
  input: ComboboxLiveV32FixedHardGateInput,
): ComboboxLiveV32FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV32FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV32FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V32_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV32FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV32FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV32FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
