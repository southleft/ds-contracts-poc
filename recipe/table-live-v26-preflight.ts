...
import {
  validateTableLiveV26AntecedentIndex,
  verifyTableLiveV26Authorization,
  type TableLiveV26AntecedentIndex,
  type TableLiveV26AuthorizationProof,
} from "./table-live-v26-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V26_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v26/operator-security-attestation-template.json";
export const TABLE_LIVE_V26_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v26-security-attestation.json";

export interface TableLiveV26SecurityAttestation {
  artifactVersion: "table-live-v26-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV26PreflightReport {
  artifactVersion: "table-live-v26-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV26SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV26SecurityAttestation(
  value: unknown,
  proof: TableLiveV26AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v26-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V26_TARGET) ||
...
export function validateTableLiveV26ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV26Authorization");
  const preflight = source.indexOf("runTableLiveV26Preflight");
  const initialize = source.indexOf("TableLiveV26Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV26Preflight(
  root: string,
  proof: TableLiveV26AuthorizationProof,
  attestationPath = TABLE_LIVE_V26_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV26PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V26_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V26_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV26AntecedentIndex;
  failures.push(...validateTableLiveV26AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V26_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V26_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV26SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v26-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV26ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v26 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV26SecurityAttestation;
  return {
    artifactVersion: "table-live-v26-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V26_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV26Preflight(
    process.cwd(),
    verifyTableLiveV26Authorization(),
    attestationPath,
