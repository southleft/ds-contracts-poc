...
import {
  ...
  TABLE_LIVE_V27_MEASURE_HUG_MARKER,
  TABLE_LIVE_V27_RESTORE_CELL_SET_ROLE,
  TABLE_LIVE_V27_RESTORE_CONTENT_ROLES,
  TABLE_LIVE_V27_RESTORE_COUNT,
...
test("Table restore owns HUG labels from cell-set only, never 144 FILL texts", () => {
  ...
  assert.equal(TABLE_LIVE_V27_RESTORE_COUNT, 8);
  assert.equal(TABLE_LIVE_V27_RESTORE_CELL_SET_ROLE, "table/cell-set");
  assert.deepEqual([...TABLE_LIVE_V27_RESTORE_CONTENT_ROLES], [
    "table/cell/label",
  ]);
  assert.throws(() =>
    buildTableLiveV27RestoreProgram({
  ...
  const program = buildTableLiveV27RestoreProgram({
    pageId: "1:2",
    runIdentity: "x-table-v2",
    setIds: ["a", "b", "c", "d", "e", "f"],
  });
  ...
  assert.match(program, new RegExp(TABLE_LIVE_V27_MEASURE_HUG_MARKER));
