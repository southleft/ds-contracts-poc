...
export interface InputLiveV70LifecycleSimulationReport {
  artifactVersion: "input-live-v70-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV70AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV70Lifecycle(): Promise<InputLiveV70LifecycleSimulationReport> {
  await buildInputLiveV70Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V70_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV70AntecedentIndex;
  const indexSha256 = inputLiveV70Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v70-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V70_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V70_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV70History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV70History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV70AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V70_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V70_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV70History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV70History(authorized, "pending");
    ...
    await buildInputLiveV70Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V70_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV70AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV70Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV70Lifecycle(), null, 2)}\n`,
  );
