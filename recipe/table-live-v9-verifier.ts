...
import {
  verifyTableLiveV9FixedSceneFixedPoint,
  type TableLiveV9FixedFixedPoint,
} from "./table-live-v9-fixed-point.js";

export const TABLE_LIVE_V9_VERIFIER_VERSION = "table-live-v9-verifier-v1";

export function buildTableLiveV9RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV9RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV9RawNode[];
}

export interface TableLiveV9NormalizedScene {
...
export interface TableLiveV9SceneProof {
...
export function normalizeTableLiveV9Scene(
  root: TableLiveV9RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV9NormalizedScene {
  const canonicalBindings: TableLiveV9NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV9RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV9Scene<Instance>(
  normalized: TableLiveV9NormalizedScene,
...
): TableLiveV9SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV9FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV9ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV9PreCaptureGates(payload: unknown): void {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("table live v9 pre-capture gate payload absent");
