...
/**
 * Carried Combobox live v10 scene-readback. Keeps v1 host-normalize,
 * v2–v4 extract teachings, v5 empty-payload omit, v6 live-root
 * observeSceneFacts, v7 recipe componentRef recovery, v8 writer
 * ARIA stamps, and v9 name-before-`#` property recovery. Teaches the
 * existing surface compile-order sort to rank `combobox/trigger`
 * bindings by recipe compile field order so `layout.padding.left`
 * precedes `layout.padding.right`, not live extract order or the
 * inherited Input surface list that ranks padding.right first. Same
 * class as Input binding extras-drop + compile-order teachings. Does
 * not invent a Combobox-only `if (combobox)`. Does not patch hashed
 * v1–v9 lineage bytes. Writer bytes stay frozen.
 */
...
export const COMBOBOX_LIVE_V10_OMIT_EMPTY_INSTANCE_PAYLOAD_MARKER =
  "COMBOBOX-HOST-OMIT-EMPTY-INSTANCE-PAYLOAD";
export const COMBOBOX_LIVE_V10_PROJECT_LIVE_ROOT_OWNERSHIP_KEY_MARKER =
  "COMBOBOX-HOST-PROJECT-LIVE-ROOT-OWNERSHIP-KEY";
export const COMBOBOX_LIVE_V10_RECOVER_RECIPE_COMPONENT_REF_MARKER =
  "COMBOBOX-HOST-RECOVER-RECIPE-COMPONENT-REF";
export const COMBOBOX_LIVE_V10_RECOVER_COMPONENT_PROPERTY_NAME_MARKER =
  "COMBOBOX-HOST-RECOVER-COMPONENT-PROPERTY-NAME-BEFORE-HASH";
export const COMBOBOX_LIVE_V10_TRIGGER_BINDING_COMPILE_ORDER_MARKER =
...
export function canonicalizeObservedComponentRef(
  ...
  void COMBOBOX_LIVE_V10_RECOVER_RECIPE_COMPONENT_REF_MARKER;
...
export function canonicalizeObservedComponentPropertyName(key: string): string {
  void COMBOBOX_LIVE_V10_RECOVER_COMPONENT_PROPERTY_NAME_MARKER;
...
const shouldOmitEmptyInstancePayload = (
  ...
  void COMBOBOX_LIVE_V10_OMIT_EMPTY_INSTANCE_PAYLOAD_MARKER;
...
const observeSceneFacts = (root: SceneNodeSnapshot): SceneFact[] => {
  void COMBOBOX_LIVE_V10_PROJECT_LIVE_ROOT_OWNERSHIP_KEY_MARKER;
