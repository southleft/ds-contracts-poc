...
import { COMBOBOX_LIVE_V20_INDEX_PATH } from "./build-combobox-live-proof-v20.js";
import {
  COMBOBOX_LIVE_V20_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V20_TARGET,
} from "./combobox-live-v20-broker.js";
import {
  buildComboboxLiveV20AuthorizationArtifact,
  validateComboboxLiveV20AntecedentIndex,
  validateComboboxLiveV20AuthorizationArtifact,
  validateComboboxLiveV20History,
  validateComboboxLiveV20UnitTestSource,
  type ComboboxLiveV20AntecedentIndex,
  type ComboboxLiveV20AuthorizationArtifact,
  type ComboboxLiveV20AuthorizationProof,
  type ComboboxLiveV20HistoryState,
} from "./combobox-live-v20-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV20ControlFlowSource,
  validateComboboxLiveV20SecurityAttestation,
  type ComboboxLiveV20SecurityAttestation,
} from "./combobox-live-v20-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v20-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V20_INDEX_PATH);
const index = (): ComboboxLiveV20AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV20AntecedentIndex;

const artifact = (): ComboboxLiveV20AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV20AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV20HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV20HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV20AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v20/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV20AuthorizationArtifact,
): ComboboxLiveV20SecurityAttestation => {
  const body: Omit<ComboboxLiveV20SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V20_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV20History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV20History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV20History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV20History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV20History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV20UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV20UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV20AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV20AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV20AuthorizationArtifact(
        planted as ComboboxLiveV20AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV20SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV20SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV20SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV20ControlFlowSource(source), []);
});
