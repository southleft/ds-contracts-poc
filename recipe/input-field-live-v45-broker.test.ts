...
import {
  ...
  createInputLiveV45FakeDynamicResponse,
  ...
  listInputLiveV45PersistedRequests,
  persistInputLiveV45RawResponse,
  persistInputLiveV45TechnicalGates,
  validateInputLiveV45Receipt,
  validateInputLiveV45Request,
  type InputLiveV45RemotePhase,
...
import {
  ...
  type InputLiveV45ExtractPayload,
  type InputLiveV45ProbePayload,
...
import {
  ...
  simulatedInputLiveV45Authorization,
} from "./run-input-field-live-v45.js";
...
const captures: InputLiveV45CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV45Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v45-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV45Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV45CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV45FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV45FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V45_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V45_TARGET);
  persistInputLiveV45RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV45Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV45Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV45WriterOwnership,
): InputLiveV45RestorePayload => ({
...
const rootProofs = (): InputLiveV45RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v45-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV45Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV45Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV45RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV45RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV45ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV45ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV45CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV45CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV45CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV45Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV45CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV45PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV45ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV45Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV45Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV45Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV45Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V45_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV45RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V45_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV45Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV45Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV45ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV45Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV45WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV45RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV45Request;
    validateInputLiveV45Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV45Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV45CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v45-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV45Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV45Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV45Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V45_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV45RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V45_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV45Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV45Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV45RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V45_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV45Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV45Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV45Request;
