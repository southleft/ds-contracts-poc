...
import {
  ...
  verifyComboboxLiveV40Authorization,
  type ComboboxLiveV40AntecedentIndex,
  type ComboboxLiveV40AuthorizationProof,
} from "./combobox-live-v40-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V40_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v40/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V40_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v40-security-attestation.json";

export interface ComboboxLiveV40SecurityAttestation {
...
export interface ComboboxLiveV40PreflightReport {
  artifactVersion: "combobox-live-v40-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV40SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV40SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV40AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v40-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V40_TARGET) ||
...
export function validateComboboxLiveV40ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV40Authorization");
  const preflight = source.indexOf("runComboboxLiveV40Preflight");
  const initialize = source.indexOf("ComboboxLiveV40Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV40Preflight(
  root: string,
  proof: ComboboxLiveV40AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V40_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV40PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V40_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V40_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V40_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV40AntecedentIndex;
  failures.push(...validateComboboxLiveV40AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V40_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V40_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v40-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV40ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v40 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV40SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v40-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V40_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV40Preflight(
    process.cwd(),
    verifyComboboxLiveV40Authorization(),
    attestationPath,
