...
import {
  TABLE_LIVE_V26_CAPTURE_COUNT,
  TABLE_LIVE_V26_HOST_PHASES,
  TABLE_LIVE_V26_REMOTE_REQUESTS,
  TABLE_LIVE_V26_SOURCE_ROOTS,
  TABLE_LIVE_V26_VARIANT_COUNT,
  buildTableLiveV26CaptureProgram,
  buildTableLiveV26CleanupProgram,
  buildTableLiveV26ExtractProgram,
  buildTableLiveV26ProbeProgram,
  buildTableLiveV26RestoreProgram,
  tableLiveV2CaptureManifestSha256,
  type TableLiveV26CaptureCell,
  type TableLiveV26SourceIdentity,
  type TableLiveV26WriterOwnership,
} from "./table-live-v26-contract.js";
import {
  TABLE_LIVE_V26_DYNAMIC_TOOL,
  TABLE_LIVE_V26_TARGET,
  tableLiveV2RequestSequence,
} from "./table-live-v26-broker.js";
import {
  ...
  FORBIDDEN_TABLE_V23_RUN_IDENTITY,
  FORBIDDEN_TABLE_V24_RUN_IDENTITY,
...
export const TABLE_LIVE_V26_EVIDENCE_ROOT =
  "recipe/evidence/table-live-pivot-v26";
export const TABLE_LIVE_V26_PROTOCOL_PATH = `${TABLE_LIVE_V26_EVIDENCE_ROOT}/protocol.json`;
export const TABLE_LIVE_V26_PLAN_PATH = `${TABLE_LIVE_V26_EVIDENCE_ROOT}/proof-plan.json`;
export const TABLE_LIVE_V26_CAPTURE_MANIFEST_PATH = `${TABLE_LIVE_V26_EVIDENCE_ROOT}/capture-manifest.json`;
export const TABLE_LIVE_V26_REQUEST_MANIFEST_PATH = `${TABLE_LIVE_V26_EVIDENCE_ROOT}/request-manifest.json`;
export const TABLE_LIVE_V26_INDEX_PATH = `${TABLE_LIVE_V26_EVIDENCE_ROOT}/antecedent-index.json`;
export const TABLE_LIVE_V26_AUTHORIZATION_TEMPLATE_PATH = `${TABLE_LIVE_V26_EVIDENCE_ROOT}/authorization-template.json`;
export const TABLE_LIVE_V26_AUTHORIZATION_PATH = `${TABLE_LIVE_V26_EVIDENCE_ROOT}/capture-authorization.json`;
...
export interface TableLiveV26ProofPlan {
  artifactVersion: "table-live-v26-proof-plan-v1";
  status: "draft antecedent; pending separate authorization; live execution forbidden";
  target: typeof TABLE_LIVE_V26_TARGET;
...
const frameFor = (
  ...
  // Optional: a source without a declared cell min-width contributes no
  // minimum to the capture frame. Geometry only -- this sizes the screenshot
  // bounding box, it is not a contract fact.
  const minW = tokens.minWidth?.fallback ?? 0;
...
const captureCells = (
...
): TableLiveV26CaptureCell[] => {
  const cells: TableLiveV26CaptureCell[] = [];
  ...
  if (cells.length !== TABLE_LIVE_V26_CAPTURE_COUNT)
    throw new Error(
      `Table live v26 capture cells ${cells.length}, expected ${TABLE_LIVE_V26_CAPTURE_COUNT}`,
    );
...
const protocol = (expectedFacts: number) => ({
  artifactVersion: "table-live-v26-external-operator-protocol-v1",
  protocolId: "table-live-v26",
  status:
  ...
  identity: {
    ...
    forbiddenTableV12RunIdentity: FORBIDDEN_TABLE_V12_RUN_IDENTITY,
    forbiddenTableV13RunIdentity: FORBIDDEN_TABLE_V13_RUN_IDENTITY,
    forbiddenTableV14RunIdentity: FORBIDDEN_TABLE_V14_RUN_IDENTITY,
    forbiddenTableV15RunIdentity: FORBIDDEN_TABLE_V15_RUN_IDENTITY,
    forbiddenTableV16RunIdentity: FORBIDDEN_TABLE_V16_RUN_IDENTITY,
    forbiddenTableV17RunIdentity: FORBIDDEN_TABLE_V17_RUN_IDENTITY,
    forbiddenTableV18RunIdentity: FORBIDDEN_TABLE_V18_RUN_IDENTITY,
    forbiddenTableV19RunIdentity: FORBIDDEN_TABLE_V19_RUN_IDENTITY,
    forbiddenTableV20RunIdentity: FORBIDDEN_TABLE_V20_RUN_IDENTITY,
    forbiddenTableV21RunIdentity: FORBIDDEN_TABLE_V21_RUN_IDENTITY,
    forbiddenTableV22RunIdentity: FORBIDDEN_TABLE_V22_RUN_IDENTITY,
    forbiddenTableV23RunIdentity: FORBIDDEN_TABLE_V23_RUN_IDENTITY,
    forbiddenTableV24RunIdentity: FORBIDDEN_TABLE_V24_RUN_IDENTITY,
  },
  target: TABLE_LIVE_V26_TARGET,
  expectedDynamicTool: TABLE_LIVE_V26_DYNAMIC_TOOL,
  denominator: {
    sourceRoots: TABLE_LIVE_V26_SOURCE_ROOTS,
    sets: 6,
    variants: TABLE_LIVE_V26_VARIANT_COUNT,
    captureCells: TABLE_LIVE_V26_CAPTURE_COUNT,
    remoteRequests: TABLE_LIVE_V26_REMOTE_REQUESTS,
    hostPhases: TABLE_LIVE_V26_HOST_PHASES,
...
export async function buildTableLiveV26Proof(
  check = process.argv.includes("--check"),
): Promise<TableLiveV26ProofPlan> {
  ...
  if (
    ...
    writer.runIdentity === FORBIDDEN_TABLE_V12_RUN_IDENTITY ||
    writer.runIdentity === FORBIDDEN_TABLE_V13_RUN_IDENTITY ||
    writer.runIdentity === FORBIDDEN_TABLE_V14_RUN_IDENTITY ||
    writer.runIdentity === FORBIDDEN_TABLE_V15_RUN_IDENTITY ||
    writer.runIdentity === FORBIDDEN_TABLE_V16_RUN_IDENTITY ||
    writer.runIdentity === FORBIDDEN_TABLE_V17_RUN_IDENTITY ||
    writer.runIdentity === FORBIDDEN_TABLE_V18_RUN_IDENTITY ||
    writer.runIdentity === FORBIDDEN_TABLE_V19_RUN_IDENTITY ||
    writer.runIdentity === FORBIDDEN_TABLE_V20_RUN_IDENTITY ||
    writer.runIdentity === FORBIDDEN_TABLE_V21_RUN_IDENTITY ||
    writer.runIdentity === FORBIDDEN_TABLE_V22_RUN_IDENTITY ||
    writer.runIdentity === FORBIDDEN_TABLE_V23_RUN_IDENTITY ||
    writer.runIdentity === FORBIDDEN_TABLE_V24_RUN_IDENTITY ||
    writer.pageName.includes(FORBIDDEN_INPUT_PAGE_ID) ||
    writer.pageName.includes(FORBIDDEN_COMBOBOX_PAGE_ID)
  )
    throw new Error(
      "Table live v26 writer reused Input, Combobox, or Table v1–v24 identity",
    );
  const captures = captureCells(sources);
  const writerProgram = Buffer.from(writer.code);
  const writerOwnershipBlueprint: TableLiveV26WriterOwnership = {
  ...
  const identities: TableLiveV26SourceIdentity[] = sources.map((source) => ({
    source: source.source,
    adapterIdentity: source.adapterIdentity,
    recipeHash: source.recipeHash,
    envelopeHash: source.envelope.integrity.canonicalHash,
  ...
  const restoreBlueprint = Buffer.from(
    buildTableLiveV26RestoreProgram(writerOwnershipBlueprint),
  );
  const extractBlueprint = Buffer.from(
    buildTableLiveV26ExtractProgram(writerOwnershipBlueprint, identities),
  );
  const probeBlueprint = Buffer.from(
    buildTableLiveV26ProbeProgram(writerOwnershipBlueprint, identities),
  );
  const captureBlueprint = Buffer.from(
    buildTableLiveV26CaptureProgram(writerOwnershipBlueprint, captures[0]!),
  );
  const cleanupBlueprint = Buffer.from(
    buildTableLiveV26CleanupProgram(writerOwnershipBlueprint),
  );
  ...
  const sourceMetadata = sources.map((source) => {
    ...
    const tablePath = `${TABLE_LIVE_V26_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-table.json.gz`;
    const rowPath = `${TABLE_LIVE_V26_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-row.json.gz`;
    const cellPath = `${TABLE_LIVE_V26_EVIDENCE_ROOT}/expected-scene-plan-${source.source}-cell.json.gz`;
  ...
  antecedentOutputs.set(
    TABLE_LIVE_V26_PROTOCOL_PATH,
    jsonBytes(protocol(expectedFacts)),
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V26_EVIDENCE_ROOT}/programs/writer.txt`,
    writerProgram,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V26_EVIDENCE_ROOT}/programs/restore-blueprint.js`,
    restoreBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V26_EVIDENCE_ROOT}/programs/extract-blueprint.js`,
    extractBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V26_EVIDENCE_ROOT}/programs/probe-blueprint.js`,
    probeBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V26_EVIDENCE_ROOT}/programs/capture-blueprint.js`,
    captureBlueprint,
  );
  antecedentOutputs.set(
    `${TABLE_LIVE_V26_EVIDENCE_ROOT}/programs/cleanup-blueprint.js`,
    cleanupBlueprint,
  );

  const captureManifest = {
    artifactVersion: "table-live-v26-capture-manifest-v1",
  ...
  antecedentOutputs.set(
    TABLE_LIVE_V26_CAPTURE_MANIFEST_PATH,
    captureManifestBytes,
  );

  const proofPlan: TableLiveV26ProofPlan = {
    artifactVersion: "table-live-v26-proof-plan-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
    target: TABLE_LIVE_V26_TARGET,
    namespace: TABLE_FIGMA_NAMESPACE,
    writer: {
      programPath: `${TABLE_LIVE_V26_EVIDENCE_ROOT}/programs/writer.txt`,
    ...
    captureManifest: {
      path: TABLE_LIVE_V26_CAPTURE_MANIFEST_PATH,
      cells: 20,
      requests: 20,
      cellsPerRequest: 1,
  ...
  antecedentOutputs.set(TABLE_LIVE_V26_PLAN_PATH, jsonBytes(proofPlan));

  const requests = {
    artifactVersion: "table-live-v26-request-manifest-v1",
    expectedDynamicTool: TABLE_LIVE_V26_DYNAMIC_TOOL,
    target: TABLE_LIVE_V26_TARGET,
  ...
  if (requests.requestCount !== TABLE_LIVE_V26_REMOTE_REQUESTS)
    throw new Error("Table live v26 remote request denominator drifted");
  antecedentOutputs.set(
    TABLE_LIVE_V26_REQUEST_MANIFEST_PATH,
    jsonBytes(requests),
  );

  const lifecycleExcludedPaths = [
    TABLE_LIVE_V26_AUTHORIZATION_TEMPLATE_PATH,
    TABLE_LIVE_V26_AUTHORIZATION_PATH,
    "recipe/table-live-v26-authorization.ts",
    "recipe/table-live-v26-authorization.test.ts",
    "recipe/table-live-v26-preflight.ts",
    "recipe/table-live-v26-authorized.ts",
    "recipe/write-table-live-v26-authorization.ts",
    "recipe/create-table-live-v26-security-attestation.ts",
    "recipe/evidence/table-live-pivot-v26/operator-security-attestation-template.json",
    "recipe/evidence/table-live-pivot-v26-status.json",
    "recipe/evidence/status-index.json",
  ];
  const indexedPaths = [
    ...antecedentOutputs.keys(),
    "recipe/table-live-v26-broker.ts",
    "recipe/table-live-v26-contract.ts",
    "recipe/table-live-v26-restore.ts",
    "recipe/table-live-v26-cleanup.ts",
    "recipe/table-live-v26-verifier.ts",
    "recipe/table-live-v26-fixed-point.ts",
    "recipe/run-table-live-v26.ts",
    "recipe/build-table-live-proof-v26.ts",
    "recipe/table-live-v26-lifecycle-simulation.ts",
    "recipe/scene-readback-table-v1.ts",
    "recipe/scene-readback-runtime-table-v26.ts",
    "recipe/table-figma-writer.ts",
    "recipe/figma-property-normalizer-v8.ts",
    "recipe/figma-runtime-portability.ts",
    "recipe/normalize.ts",
  ].filter((artifactPath) => artifactPath !== TABLE_LIVE_V26_INDEX_PATH);
  ...
  const index = {
    artifactVersion: "table-live-v26-antecedent-index-v1",
    status:
      "draft antecedent; pending separate authorization; live execution forbidden",
  ...
  antecedentOutputs.set(TABLE_LIVE_V26_INDEX_PATH, jsonBytes(index));

  const authorizationTemplate = {
    artifactVersion: "table-live-v26-authorization-template-v1",
    authorizationId: "table-live-v26",
    status: "template only; no authorization",
    authorizationIntent: false,
    antecedent: {
      commit: null,
      indexPath: TABLE_LIVE_V26_INDEX_PATH,
      indexSha256: null,
      hashSetSha256,
    },
    signingPublicKey: {
    ...
    denominator: {
      remoteRequests: TABLE_LIVE_V26_REMOTE_REQUESTS,
      hostPhases: TABLE_LIVE_V26_HOST_PHASES,
      captures: TABLE_LIVE_V26_CAPTURE_COUNT,
      sourceRoots: TABLE_LIVE_V26_SOURCE_ROOTS,
      expectedFacts,
    },
    execution: {
  ...
  const attestationTemplate = jsonBytes({
    artifactVersion: "table-live-v26-operator-security-attestation-template-v1",
    status: "template only; complete privately after PREPARE+AUTHORIZE",
    tokenValuesForbidden: true,
    privateDirectory: "private/table-live-v26-security-attestation.json",
  });
  ...
  if (check) {
    ...
    if (drift.length)
      throw new Error(
        `Table live v26 antecedent generated artifact drift:\n${drift.join("\n")}`,
      );
  } else {
    ...
    mkdirSync(path.dirname(TABLE_LIVE_V26_AUTHORIZATION_TEMPLATE_PATH), {
      recursive: true,
    });
    writeFileSync(TABLE_LIVE_V26_AUTHORIZATION_TEMPLATE_PATH, templateBytes);
    writeFileSync(
      `${TABLE_LIVE_V26_EVIDENCE_ROOT}/operator-security-attestation-template.json`,
      attestationTemplate,
    );
  }
  if (process.argv.includes("--check-authorization-template")) {
    if (
      !existsSync(TABLE_LIVE_V26_AUTHORIZATION_TEMPLATE_PATH) ||
      !readFileSync(TABLE_LIVE_V26_AUTHORIZATION_TEMPLATE_PATH).equals(
        templateBytes,
      )
    )
      throw new Error("Table live v26 authorization template drift");
  }
  if (
    writer.namespace !== TABLE_FIGMA_NAMESPACE ||
    writer.namespace === FORBIDDEN_INPUT_NAMESPACE ||
    ...
    !writerProgram.toString("utf8").includes("TABLE-V12-IDENTITY-REUSE") ||
    !writerProgram.toString("utf8").includes("TABLE-V13-IDENTITY-REUSE") ||
    !writerProgram.toString("utf8").includes("TABLE-V14-IDENTITY-REUSE") ||
    !writerProgram.toString("utf8").includes("TABLE-V15-IDENTITY-REUSE") ||
    !writerProgram.toString("utf8").includes("TABLE-V16-IDENTITY-REUSE") ||
    !writerProgram.toString("utf8").includes("TABLE-V17-IDENTITY-REUSE") ||
    !writerProgram.toString("utf8").includes("TABLE-V18-IDENTITY-REUSE") ||
    !writerProgram.toString("utf8").includes("TABLE-V19-IDENTITY-REUSE") ||
    !writerProgram.toString("utf8").includes("TABLE-V20-IDENTITY-REUSE") ||
    !writerProgram.toString("utf8").includes("TABLE-V21-IDENTITY-REUSE") ||
    !writerProgram
      .toString("utf8")
      .includes("TABLE-WRITER-ROW-OWNED-TEXT-CHARACTERS") ||
    !writerProgram
      .toString("utf8")
      .includes("TABLE-WRITER-MIN-WIDTH-ZERO-UNSET") ||
    protocol(expectedFacts).execution.cleanupOnFailureOnly !== true
  )
    throw new Error("Table live v26 identity or cleanup invariant failed");
  return proofPlan;
}

if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await buildTableLiveV26Proof(), null, 2)}\n`,
  );
