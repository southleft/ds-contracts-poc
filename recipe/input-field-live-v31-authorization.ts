...
import {
  ...
  INPUT_LIVE_V31_INDEX_PATH,
} from "./build-input-field-live-proof-v31.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV31HistoryExpectation = "pending" | "authorized";

export interface InputLiveV31AuthorizationArtifact {
  artifactVersion: "input-live-v31-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V31_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV31AntecedentIndex {
...
export interface InputLiveV31HistoryState {
  index?: InputLiveV31AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV31AuthorizationArtifact;
...
export interface InputLiveV31AuthorizationProof extends InputLiveV31TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V31_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V31_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V31_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V31_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v31-status") ||
...
export function validateInputLiveV31AntecedentIndex(
  index: InputLiveV31AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v31-antecedent-index-v1" ||
...
export function validateInputLiveV31UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV31History\(\s*["']/.test(source) ||
    /readInputLiveV31HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV31AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV31AuthorizationArtifact {
  ...
  ) as InputLiveV31AntecedentIndex;
  const indexFailures = validateInputLiveV31AntecedentIndex(index);
...
export function validateInputLiveV31AuthorizationArtifact(
  artifact: InputLiveV31AuthorizationArtifact | undefined,
  index: InputLiveV31AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v31-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v31" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V31_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V31_TARGET) ||
    !equalJson(
...
export function validateInputLiveV31History(
  state: InputLiveV31HistoryState,
  expected: InputLiveV31HistoryExpectation,
): string[] {
  const failures = validateInputLiveV31AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV31AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV31HistoryState(
  root = process.cwd(),
): InputLiveV31HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V31_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV31AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V31_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V31_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V31_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV31History(
  expected: InputLiveV31HistoryExpectation,
  root = process.cwd(),
): InputLiveV31HistoryState {
  const state = readInputLiveV31HistoryState(root);
  const failures = validateInputLiveV31History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV31Authorization(
  root = process.cwd(),
): InputLiveV31AuthorizationProof {
  const state = verifyInputLiveV31History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V31_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v31/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v31.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V31_TARGET,
    expectedDynamicTool: INPUT_LIVE_V31_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V31_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV31HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV31History(expected);
