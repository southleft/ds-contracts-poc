...
import { COMBOBOX_LIVE_V8_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v8.js";
import { verifyComboboxLiveV8Authorization } from "./combobox-live-v8-authorization.js";
import { runComboboxLiveV8Preflight } from "./combobox-live-v8-preflight.js";
import { ComboboxLiveV8Orchestrator } from "./run-combobox-live-v8.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v8 requires exact authorization artifact ${COMBOBOX_LIVE_V8_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v8-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV8Authorization(root);
    runComboboxLiveV8Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV8Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV8Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
