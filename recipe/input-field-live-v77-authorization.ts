...
import {
  ...
  INPUT_LIVE_V77_INDEX_PATH,
} from "./build-input-field-live-proof-v77.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV77HistoryExpectation = "pending" | "authorized";

export interface InputLiveV77AuthorizationArtifact {
  artifactVersion: "input-live-v77-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V77_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV77AntecedentIndex {
...
export interface InputLiveV77HistoryState {
  index?: InputLiveV77AntecedentIndex;
  indexSha256: string;
...
export interface InputLiveV77AuthorizationProof extends InputLiveV77TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V77_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V77_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V77_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V77_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v77-authorization") ||
  artifactPath.includes("input-field-live-v77-preflight") ||
  artifactPath.includes("input-field-live-v77-authorized") ||
  artifactPath.includes("input-field-live-pivot-v77-status") ||
...
export function validateInputLiveV77AntecedentIndex(
  index: InputLiveV77AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v77-antecedent-index-v1" ||
...
export function validateInputLiveV77UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV77History\(\s*["']/.test(source) ||
    /readInputLiveV77HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV77AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV77AuthorizationArtifact {
  ...
  ) as InputLiveV77AntecedentIndex;
  const indexFailures = validateInputLiveV77AntecedentIndex(index);
...
export function validateInputLiveV77AuthorizationArtifact(
  artifact: InputLiveV77AuthorizationArtifact | undefined,
  index: InputLiveV77AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v77-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v77" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V77_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V77_TARGET) ||
    !equalJson(
...
export function validateInputLiveV77History(
  state: InputLiveV77HistoryState,
  expected: InputLiveV77HistoryExpectation,
): string[] {
  const failures = validateInputLiveV77AntecedentIndex(state.index);
...
export function readInputLiveV77HistoryState(
  root = process.cwd(),
): InputLiveV77HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V77_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV77AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V77_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V77_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V77_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV77History(
  expected: InputLiveV77HistoryExpectation,
  root = process.cwd(),
): InputLiveV77HistoryState {
  const state = readInputLiveV77HistoryState(root);
  const failures = validateInputLiveV77History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV77Authorization(
  root = process.cwd(),
): InputLiveV77AuthorizationProof {
  const state = verifyInputLiveV77History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V77_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v77/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v77.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V77_TARGET,
    expectedDynamicTool: INPUT_LIVE_V77_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V77_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV77HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV77History(expected);
