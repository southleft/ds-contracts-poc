...
import {
  ...
  verifyInputLiveV50Authorization,
  type InputLiveV50AntecedentIndex,
  type InputLiveV50AuthorizationProof,
} from "./input-field-live-v50-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V50_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v50/operator-security-attestation-template.json";
export const INPUT_LIVE_V50_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v50-security-attestation.json";

export interface InputLiveV50SecurityAttestation {
...
export interface InputLiveV50PreflightReport {
  artifactVersion: "input-live-v50-preflight-v1";
...
export const inputLiveV50AttestationSha256 = (
  value: Omit<InputLiveV50SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV50SecurityAttestation(
  value: unknown,
  proof: InputLiveV50AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v50-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V50_TARGET) ||
...
export function validateInputLiveV50ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV50Authorization");
  const preflight = source.indexOf("runInputLiveV50Preflight");
  const initialize = source.indexOf("InputLiveV50Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV50Preflight(
  root: string,
  proof: InputLiveV50AuthorizationProof,
  attestationPath = INPUT_LIVE_V50_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV50PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V50_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V50_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V50_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV50AntecedentIndex;
  failures.push(...validateInputLiveV50AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V50_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V50_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v50-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV50ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV50SecurityAttestation;
  return {
    artifactVersion: "input-live-v50-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V50_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV50Preflight(
    process.cwd(),
    verifyInputLiveV50Authorization(),
    attestationPath,
