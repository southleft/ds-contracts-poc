...
import {
  validateTableLiveV34AntecedentIndex,
  verifyTableLiveV34Authorization,
  type TableLiveV34AntecedentIndex,
  type TableLiveV34AuthorizationProof,
} from "./table-live-v34-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V34_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v34/operator-security-attestation-template.json";
export const TABLE_LIVE_V34_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v34-security-attestation.json";

export interface TableLiveV34SecurityAttestation {
  artifactVersion: "table-live-v34-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV34PreflightReport {
  artifactVersion: "table-live-v34-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV34SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV34SecurityAttestation(
  value: unknown,
  proof: TableLiveV34AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v34-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V34_TARGET) ||
...
export function validateTableLiveV34ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV34Authorization");
  const preflight = source.indexOf("runTableLiveV34Preflight");
  const initialize = source.indexOf("TableLiveV34Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV34Preflight(
  root: string,
  proof: TableLiveV34AuthorizationProof,
  attestationPath = TABLE_LIVE_V34_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV34PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V34_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V34_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV34AntecedentIndex;
  failures.push(...validateTableLiveV34AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V34_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V34_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV34SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v34-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV34ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v34 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV34SecurityAttestation;
  return {
    artifactVersion: "table-live-v34-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V34_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV34Preflight(
    process.cwd(),
    verifyTableLiveV34Authorization(),
    attestationPath,
