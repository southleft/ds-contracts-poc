...
import {
  assertCalendarLiveV43RootProofs,
  CALENDAR_LIVE_V43_CAPTURE_COUNT,
  CALENDAR_LIVE_V43_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  CALENDAR_LIVE_V43_RESPONSE_CONTRACTS,
  CALENDAR_LIVE_V43_SOURCE_ROOTS,
  type CalendarLiveV43RootProof,
} from "./calendar-live-v43-contract.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V43_BROKER_VERSION = "calendar-live-v43-broker-v1";
export const CALENDAR_LIVE_V43_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const CALENDAR_LIVE_V43_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const CALENDAR_LIVE_V43_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const CALENDAR_LIVE_V43_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const CALENDAR_LIVE_V43_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const CALENDAR_LIVE_V43_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type CalendarLiveV43RemotePhase =
  (typeof CALENDAR_LIVE_V43_REMOTE_PHASES)[number];
export type CalendarLiveV43AuthorizationMode = "live" | "simulated";

export interface CalendarLiveV43CommitPins {
...
export interface CalendarLiveV43TransactionAuthorization extends CalendarLiveV43CommitPins {
  mode: CalendarLiveV43AuthorizationMode;
...
export interface CalendarLiveV43Manifest {
  artifactVersion: "calendar-live-v43-broker-manifest-v2";
  brokerVersion: typeof CALENDAR_LIVE_V43_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof CALENDAR_LIVE_V43_DYNAMIC_TOOL;
  target: typeof CALENDAR_LIVE_V43_TARGET;
  authorization: CalendarLiveV43TransactionAuthorization;
...
export interface CalendarLiveV43ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface CalendarLiveV43RequestBody {
  artifactVersion: "calendar-live-v43-broker-request-v2";
  brokerVersion: typeof CALENDAR_LIVE_V43_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: CalendarLiveV43RemotePhase;
  sequence: number;
  ...
  commitPins: CalendarLiveV43CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof CALENDAR_LIVE_V43_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: CalendarLiveV43ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface CalendarLiveV43Request extends CalendarLiveV43RequestBody {
...
export interface CalendarLiveV43Receipt {
  artifactVersion: "calendar-live-v43-broker-receipt-v2";
  brokerVersion: typeof CALENDAR_LIVE_V43_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: CalendarLiveV43RemotePhase;
...
export interface CalendarLiveV43AcceptedResponse<T = unknown> {
  request: CalendarLiveV43Request;
  receipt: CalendarLiveV43Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface CalendarLiveV43IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: CalendarLiveV43ResponseContract;
...
const equalTarget = (value: unknown): value is typeof CALENDAR_LIVE_V43_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === CALENDAR_LIVE_V43_TARGET.fileKey &&
  (value as BrokerTarget).fileName === CALENDAR_LIVE_V43_TARGET.fileName &&
  (value as BrokerTarget).editorType === CALENDAR_LIVE_V43_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== CALENDAR_LIVE_V43_TARGET.fileKey ||
    context.fileName !== CALENDAR_LIVE_V43_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === CALENDAR_LIVE_V43_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Calendar live v43 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= CALENDAR_LIVE_V43_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v43 capture index must be 0..7");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Calendar live v43 authorization pins are malformed");
};

const readManifest = (directory: string): CalendarLiveV43Manifest => {
  const manifest = json<CalendarLiveV43Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Calendar live v43 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "calendar-live-v43-broker-manifest-v2" ||
    manifest.brokerVersion !== CALENDAR_LIVE_V43_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== CALENDAR_LIVE_V43_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(CALENDAR_LIVE_V43_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Calendar live v43 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: CalendarLiveV43Request,
): CalendarLiveV43RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: CalendarLiveV43RemotePhase,
  captureIndex?: number,
): CalendarLiveV43Request | undefined => {
  const file = calendarLiveV1RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV43Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: CalendarLiveV43RemotePhase,
  captureIndex?: number,
): CalendarLiveV43Receipt | undefined => {
  const file = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV43Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: CalendarLiveV43Request | undefined;
  receipt: CalendarLiveV43Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: CalendarLiveV43RemotePhase,
): CalendarLiveV43ResponseContract => ({
  ...CALENDAR_LIVE_V43_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: CalendarLiveV43Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: CALENDAR_LIVE_V43_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: CALENDAR_LIVE_V43_DYNAMIC_TOOL,
      target: CALENDAR_LIVE_V43_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "calendar-live-v43-plugin-response-v2",
  brokerVersion: CALENDAR_LIVE_V43_BROKER_VERSION,
  target: CALENDAR_LIVE_V43_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*CALENDAR_LIVE_V43_PROGRAM_START*/${program}/*CALENDAR_LIVE_V43_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*CALENDAR_LIVE_V43_PROGRAM_START*/";
  const endMarker = "/*CALENDAR_LIVE_V43_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Calendar live v43 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createCalendarLiveV43Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Calendar live v43 private signing key does not match authorized public-key identity",
    );
  const manifest: CalendarLiveV43Manifest = {
    artifactVersion: "calendar-live-v43-broker-manifest-v2",
    brokerVersion: CALENDAR_LIVE_V43_BROKER_VERSION,
    ...
    captureRequests: CALENDAR_LIVE_V43_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateCalendarLiveV43Request(
  directory: string,
  request: CalendarLiveV43Request,
): void {
  ...
  if (
    request.artifactVersion !== "calendar-live-v43-broker-request-v2" ||
    request.brokerVersion !== CALENDAR_LIVE_V43_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== CALENDAR_LIVE_V43_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Calendar live v43 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueCalendarLiveV43Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: CalendarLiveV43RemotePhase,
  program: string,
  options: CalendarLiveV43IssueOptions = {},
): CalendarLiveV43Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<CalendarLiveV43Request>(targetPath);
    validateCalendarLiveV43Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Calendar live v43 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(calendarLiveV1ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Calendar live v43 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Calendar live v43 empty code envelope refused");
  if (phase === "capture") validateCalendarLiveV43TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Calendar live v43 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: CALENDAR_LIVE_V43_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? CALENDAR_LIVE_V43_SIGNED_WRITER_TIMEOUT_MS
        : CALENDAR_LIVE_V43_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: CalendarLiveV43RequestBody = {
    artifactVersion: "calendar-live-v43-broker-request-v2",
    brokerVersion: CALENDAR_LIVE_V43_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: CalendarLiveV43Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateCalendarLiveV43Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistCalendarLiveV43RawResponse(
  directory: string,
  phase: CalendarLiveV43RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Calendar live v43 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<CalendarLiveV43Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Calendar live v43 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = calendarLiveV1ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Calendar live v43 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): CalendarLiveV43Receipt => {
  const body: Omit<CalendarLiveV43Receipt, "receiptSha256"> = {
    artifactVersion: "calendar-live-v43-broker-receipt-v2",
    brokerVersion: CALENDAR_LIVE_V43_BROKER_VERSION,
...
export function acceptCalendarLiveV43Response<T = unknown>(
  directory: string,
  phase: CalendarLiveV43RemotePhase,
  options: {
    captureIndex?: number;
...
): CalendarLiveV43AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Calendar live v43 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<CalendarLiveV43Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Calendar live v43 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "calendar-live-v43-plugin-response-v2" ||
    plugin?.brokerVersion !== CALENDAR_LIVE_V43_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Calendar live v43 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistCalendarLiveV43TechnicalGates(
  directory: string,
  rootProofs: readonly CalendarLiveV43RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertCalendarLiveV43RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Calendar live v43 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Calendar live v43 probe payload hash malformed");
  const gates = Object.fromEntries(
    CALENDAR_LIVE_V43_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "calendar-live-v43-host-technical-gates-v2",
    brokerVersion: CALENDAR_LIVE_V43_BROKER_VERSION,
...
export function validateCalendarLiveV43TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "calendar-live-v43-host-technical-gates-v2" ||
    ...
    artifact.roots.length !== CALENDAR_LIVE_V43_SOURCE_ROOTS ||
    ...
    CALENDAR_LIVE_V43_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Calendar live v43 technical gates artifact");
}

export function validateCalendarLiveV43Receipt(
  directory: string,
  phase: CalendarLiveV43RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateCalendarLiveV43Request(directory, request);
  const receipt = json<CalendarLiveV43Receipt>(
  ...
  if (
    receipt.artifactVersion !== "calendar-live-v43-broker-receipt-v2" ||
    receipt.brokerVersion !== CALENDAR_LIVE_V43_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Calendar live v43 accepted receipt: ${request.requestId}`,
    );
}

export function readCalendarLiveV43AcceptedRawPayload(
  directory: string,
  phase: CalendarLiveV43RemotePhase,
  captureIndex?: number,
): unknown {
  validateCalendarLiveV43Receipt(directory, phase, captureIndex);
...
export function createCalendarLiveV43FakeDynamicResponse(
  request: CalendarLiveV43Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? CALENDAR_LIVE_V43_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? CALENDAR_LIVE_V43_TARGET,
    ...overrides.extra,
  };
}

export function listCalendarLiveV43PersistedRequests(
  directory: string,
): CalendarLiveV43Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<CalendarLiveV43Request>(file)] : [];
    });
}
