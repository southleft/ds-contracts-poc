...
/**
 * Carried Combobox live v9 scene-readback. Keeps v1 host-normalize,
 * v2–v4 extract teachings, v5 empty-payload omit, v6 live-root
 * observeSceneFacts, v7 recipe componentRef recovery, and v8 writer
 * ARIA stamps. Teaches that sceneToNormalizedIr recovers instance
 * component-property names from the Figma key before `#` (same class
 * as Button/Input first-segment / name-before-`#` recoveries), carrying
 * the already-stamped source values. Live keys such as
 * `Label#135:173522` / `Value#135:173531` / `Disabled#135:173540`
 * therefore become `Label` / `Value` / `Disabled` with source values
 * including `ada`. Does not invent `ada` from the label. Does not
 * patch hashed v1–v8 lineage bytes.
 */
...
export const COMBOBOX_LIVE_V9_OMIT_EMPTY_INSTANCE_PAYLOAD_MARKER =
  "COMBOBOX-HOST-OMIT-EMPTY-INSTANCE-PAYLOAD";
export const COMBOBOX_LIVE_V9_PROJECT_LIVE_ROOT_OWNERSHIP_KEY_MARKER =
  "COMBOBOX-HOST-PROJECT-LIVE-ROOT-OWNERSHIP-KEY";
export const COMBOBOX_LIVE_V9_RECOVER_RECIPE_COMPONENT_REF_MARKER =
  "COMBOBOX-HOST-RECOVER-RECIPE-COMPONENT-REF";
export const COMBOBOX_LIVE_V9_RECOVER_COMPONENT_PROPERTY_NAME_MARKER =
...
export function canonicalizeObservedComponentRef(
  ...
  void COMBOBOX_LIVE_V9_RECOVER_RECIPE_COMPONENT_REF_MARKER;
...
export function canonicalizeObservedComponentPropertyName(key: string): string {
  void COMBOBOX_LIVE_V9_RECOVER_COMPONENT_PROPERTY_NAME_MARKER;
...
const shouldOmitEmptyInstancePayload = (
  ...
  void COMBOBOX_LIVE_V9_OMIT_EMPTY_INSTANCE_PAYLOAD_MARKER;
...
const observeSceneFacts = (root: SceneNodeSnapshot): SceneFact[] => {
  void COMBOBOX_LIVE_V9_PROJECT_LIVE_ROOT_OWNERSHIP_KEY_MARKER;
