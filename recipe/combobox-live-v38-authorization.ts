...
export type ComboboxLiveV38HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV38AuthorizationArtifact {
  artifactVersion: "combobox-live-v38-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V38_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV38AntecedentIndex {
...
export interface ComboboxLiveV38HistoryState {
  index?: ComboboxLiveV38AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV38AuthorizationArtifact;
...
export interface ComboboxLiveV38AuthorizationProof extends ComboboxLiveV38TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V38_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V38_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V38_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V38_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v38-authorization") ||
  artifactPath.includes("combobox-live-v38-preflight") ||
  artifactPath.includes("combobox-live-v38-authorized") ||
  artifactPath.includes("create-combobox-live-v38-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v38-status") ||
...
export function validateComboboxLiveV38AntecedentIndex(
  index: ComboboxLiveV38AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v38-antecedent-index-v1" ||
...
export function validateComboboxLiveV38UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV38History\(\s*["']/.test(source) ||
    /readComboboxLiveV38HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV38AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV38AuthorizationArtifact {
  ...
  ) as ComboboxLiveV38AntecedentIndex;
  const indexFailures = validateComboboxLiveV38AntecedentIndex(index);
...
export function validateComboboxLiveV38AuthorizationArtifact(
  artifact: ComboboxLiveV38AuthorizationArtifact | undefined,
  index: ComboboxLiveV38AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v38-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v38" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V38_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V38_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V38_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V38_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V38_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V38_SOURCE_ROOTS ||
...
export function validateComboboxLiveV38History(
  state: ComboboxLiveV38HistoryState,
  expected: ComboboxLiveV38HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV38AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV38AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV38HistoryState(
  root = process.cwd(),
): ComboboxLiveV38HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V38_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV38AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V38_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V38_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V38_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV38History(
  expected: ComboboxLiveV38HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV38HistoryState {
  const state = readComboboxLiveV38HistoryState(root);
  const failures = validateComboboxLiveV38History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v38 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV38Authorization(
  root = process.cwd(),
): ComboboxLiveV38AuthorizationProof {
  const state = verifyComboboxLiveV38History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V38_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v38/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v38.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V38_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V38_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V38_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV38HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV38History(expected);
  process.stdout.write(
    `Combobox live v38 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
