...
import {
  verifyTableLiveV12FixedSceneFixedPoint,
  type TableLiveV12FixedFixedPoint,
} from "./table-live-v12-fixed-point.js";

export const TABLE_LIVE_V12_VERIFIER_VERSION = "table-live-v12-verifier-v1";

export function buildTableLiveV12RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV12RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV12RawNode[];
}

export interface TableLiveV12NormalizedScene {
...
export interface TableLiveV12SceneProof {
...
export function normalizeTableLiveV12Scene(
  root: TableLiveV12RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV12NormalizedScene {
  const canonicalBindings: TableLiveV12NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV12RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV12Scene<Instance>(
  normalized: TableLiveV12NormalizedScene,
...
): TableLiveV12SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV12FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV12ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV12PreCaptureGates(payload: unknown): void {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("table live v12 pre-capture gate payload absent");
