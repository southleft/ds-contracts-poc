...
import { buildTableLiveV18CleanupRuntime } from "./table-live-v18-cleanup.js";
import { type TableLiveV18FixedFixedPoint } from "./table-live-v18-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV18RestoreProgram,
  validateTableLiveV18RestorePayload,
  type TableLiveV18RestorePayload,
} from "./table-live-v18-restore.js";
import {
  buildTableLiveV18RawPropertyRuntime,
  normalizeTableLiveV18Scene,
  type TableLiveV18RawNode,
} from "./table-live-v18-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v18.js";
import {
...
export const TABLE_LIVE_V18_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V18_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V18_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V18_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V18_VARIANT_COUNT = 20;
export const TABLE_LIVE_V18_SET_COUNT = 6;
export const TABLE_LIVE_V18_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V18_HOST_PHASES = 3;
export const TABLE_LIVE_V18_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V18_RESTORE_COUNT = 8;
export const TABLE_LIVE_V18_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V18_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV18SourceId = (typeof TABLE_LIVE_V18_SOURCE_IDS)[number];
export type TableLiveV18Kind = "table" | "row" | "cell";

export interface TableLiveV18SourceIdentity {
  source: TableLiveV18SourceId;
...
export interface TableLiveV18WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV18ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV18SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV18RawNode;
    rowScene: TableLiveV18RawNode;
    cellScene: TableLiveV18RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV18RootProof {
  source: TableLiveV18SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV18FixedFixedPoint;
}

export interface TableLiveV18ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV18SourceId;
    adapterIdentity: string;
...
export interface TableLiveV18CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV18SourceId;
  adapterIdentity: string;
...
export interface TableLiveV18CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV18SourceId;
  frameWidth: number;
...
export interface TableLiveV18CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v18 ${label} must be an object`);
...
export function validateTableLiveV18WriterPayload(
  value: unknown,
): TableLiveV18WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V18_NAMESPACE ||
    ...
    throw new TypeError("Table live v18 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v18 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v18 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV18SourceIdentity,
  kind: TableLiveV18Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV18ExtractProgram(
  writer: TableLiveV18WriterOwnership,
  sources: readonly TableLiveV18SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v18 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V18_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V18_NAMESPACE)}
${buildTableLiveV18RawPropertyRuntime()}
...
export function validateTableLiveV18ExtractPayload(
  value: unknown,
  writer: TableLiveV18WriterOwnership,
): TableLiveV18ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v18 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V18_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v18 extract schema/two-root mismatch");
  return payload as TableLiveV18ExtractPayload;
}

export function proveTableLiveV18Roots<Instance>(
  extract: TableLiveV18ExtractPayload,
  sources: readonly (TableLiveV18SourceIdentity & {
...
): TableLiveV18RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v18 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v18 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV18Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV18Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV18Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV18FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v18 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v18 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v18 compile lost owned sets");
...
export function assertTableLiveV18RootProofs(
  proofs: readonly TableLiveV18RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v18 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v18 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV18ProbeProgram(
  writer: TableLiveV18WriterOwnership,
  sources: readonly TableLiveV18SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V18_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV18ProbePayload(
  value: unknown,
  writer: TableLiveV18WriterOwnership,
): TableLiveV18ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV18ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V18_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v18 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV18CaptureManifest(
  cells: readonly TableLiveV18CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V18_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V18_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V18_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V18_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v18 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV18CaptureCell[],
): string => {
  validateTableLiveV18CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV18CaptureProgram(
  writer: TableLiveV18WriterOwnership,
  cell: TableLiveV18CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V18_CAPTURE_COUNT
  )
    throw new TypeError("Table live v18 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v18 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V18_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV18CapturePayload(
  value: unknown,
  cell: TableLiveV18CaptureCell,
  rawResponseBytes: number,
): TableLiveV18CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV18CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V18_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V18_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v18 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV18CaptureResponses(
  manifest: readonly TableLiveV18CaptureCell[],
  responses: readonly TableLiveV18CapturePayload[],
): void {
  validateTableLiveV18CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v18 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV18ObjectiveReport {
  artifactVersion: "table-live-v18-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV18Objective(
  manifest: readonly TableLiveV18CaptureCell[],
  responses: readonly TableLiveV18CapturePayload[],
): TableLiveV18ObjectiveReport {
  assertTableLiveV18CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v18-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV18CleanupProgram(
  writer: TableLiveV18WriterOwnership,
): string {
  const baseProgram = buildTableLiveV18CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V18_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v18 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV18CleanupPayload(
  payload: unknown,
  writer: TableLiveV18WriterOwnership,
): TableLiveV18CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV18CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v18 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V18_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV18WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
