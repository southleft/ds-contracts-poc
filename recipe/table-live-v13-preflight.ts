...
import {
  validateTableLiveV13AntecedentIndex,
  verifyTableLiveV13Authorization,
  type TableLiveV13AntecedentIndex,
  type TableLiveV13AuthorizationProof,
} from "./table-live-v13-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V13_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v13/operator-security-attestation-template.json";
export const TABLE_LIVE_V13_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v13-security-attestation.json";

export interface TableLiveV13SecurityAttestation {
  artifactVersion: "table-live-v13-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV13PreflightReport {
  artifactVersion: "table-live-v13-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV13SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV13SecurityAttestation(
  value: unknown,
  proof: TableLiveV13AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v13-operator-security-attestation-v1" ||
    artifact.status !== "complete"
  )
    failures.push("missing completed table v13 security attestation");
  if (
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V13_TARGET) ||
...
export function validateTableLiveV13ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV13Authorization");
  const preflight = source.indexOf("runTableLiveV13Preflight");
  const initialize = source.indexOf("TableLiveV13Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV13Preflight(
  root: string,
  proof: TableLiveV13AuthorizationProof,
  attestationPath = TABLE_LIVE_V13_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV13PreflightReport {
  const failures: string[] = [];
  if (
    ...
    failures.push("table v13 attempt chronology invalid or exceeds maximum 3");
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V13_TARGET) ||
    canonicalJson(proof.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V13_DYNAMIC_TOOL)
  )
    failures.push("table v13 authorization proof target or tool mismatch");
  const indexPath = path.join(root, TABLE_LIVE_V13_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV13AntecedentIndex;
  failures.push(...validateTableLiveV13AntecedentIndex(index));
  if (
    sha256(indexBytes) !== proof.antecedentIndexSha256 ||
    index.hashSetSha256 !== proof.antecedentHashSetSha256
  )
    failures.push("table v13 authorization proof antecedent index mismatch");
  for (const [relativePath, metadata] of Object.entries(index.artifacts)) {
    ...
    if (
      ...
      failures.push(
        `table v13 pinned antecedent working bytes drift: ${relativePath}`,
      );
  }
  if (!existsSync(path.join(root, TABLE_LIVE_V13_AUTHORIZATION_PATH)))
    failures.push("table v13 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V13_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV13SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v13-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV13ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v13 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV13SecurityAttestation;
  return {
    artifactVersion: "table-live-v13-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V13_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV13Preflight(
    process.cwd(),
    verifyTableLiveV13Authorization(),
    attestationPath,
