...
import {
  assertCalendarLiveV30RootProofs,
  CALENDAR_LIVE_V30_CAPTURE_COUNT,
  CALENDAR_LIVE_V30_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  CALENDAR_LIVE_V30_RESPONSE_CONTRACTS,
  CALENDAR_LIVE_V30_SOURCE_ROOTS,
  type CalendarLiveV30RootProof,
} from "./calendar-live-v30-contract.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V30_BROKER_VERSION = "calendar-live-v30-broker-v1";
export const CALENDAR_LIVE_V30_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const CALENDAR_LIVE_V30_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const CALENDAR_LIVE_V30_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const CALENDAR_LIVE_V30_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const CALENDAR_LIVE_V30_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const CALENDAR_LIVE_V30_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type CalendarLiveV30RemotePhase =
  (typeof CALENDAR_LIVE_V30_REMOTE_PHASES)[number];
export type CalendarLiveV30AuthorizationMode = "live" | "simulated";

export interface CalendarLiveV30CommitPins {
...
export interface CalendarLiveV30TransactionAuthorization extends CalendarLiveV30CommitPins {
  mode: CalendarLiveV30AuthorizationMode;
...
export interface CalendarLiveV30Manifest {
  artifactVersion: "calendar-live-v30-broker-manifest-v2";
  brokerVersion: typeof CALENDAR_LIVE_V30_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof CALENDAR_LIVE_V30_DYNAMIC_TOOL;
  target: typeof CALENDAR_LIVE_V30_TARGET;
  authorization: CalendarLiveV30TransactionAuthorization;
...
export interface CalendarLiveV30ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface CalendarLiveV30RequestBody {
  artifactVersion: "calendar-live-v30-broker-request-v2";
  brokerVersion: typeof CALENDAR_LIVE_V30_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: CalendarLiveV30RemotePhase;
  sequence: number;
  ...
  commitPins: CalendarLiveV30CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof CALENDAR_LIVE_V30_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: CalendarLiveV30ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface CalendarLiveV30Request extends CalendarLiveV30RequestBody {
...
export interface CalendarLiveV30Receipt {
  artifactVersion: "calendar-live-v30-broker-receipt-v2";
  brokerVersion: typeof CALENDAR_LIVE_V30_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: CalendarLiveV30RemotePhase;
...
export interface CalendarLiveV30AcceptedResponse<T = unknown> {
  request: CalendarLiveV30Request;
  receipt: CalendarLiveV30Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface CalendarLiveV30IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: CalendarLiveV30ResponseContract;
...
const equalTarget = (value: unknown): value is typeof CALENDAR_LIVE_V30_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === CALENDAR_LIVE_V30_TARGET.fileKey &&
  (value as BrokerTarget).fileName === CALENDAR_LIVE_V30_TARGET.fileName &&
  (value as BrokerTarget).editorType === CALENDAR_LIVE_V30_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== CALENDAR_LIVE_V30_TARGET.fileKey ||
    context.fileName !== CALENDAR_LIVE_V30_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === CALENDAR_LIVE_V30_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Calendar live v30 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= CALENDAR_LIVE_V30_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v30 capture index must be 0..7");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Calendar live v30 authorization pins are malformed");
};

const readManifest = (directory: string): CalendarLiveV30Manifest => {
  const manifest = json<CalendarLiveV30Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Calendar live v30 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "calendar-live-v30-broker-manifest-v2" ||
    manifest.brokerVersion !== CALENDAR_LIVE_V30_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== CALENDAR_LIVE_V30_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(CALENDAR_LIVE_V30_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Calendar live v30 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: CalendarLiveV30Request,
): CalendarLiveV30RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: CalendarLiveV30RemotePhase,
  captureIndex?: number,
): CalendarLiveV30Request | undefined => {
  const file = calendarLiveV1RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV30Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: CalendarLiveV30RemotePhase,
  captureIndex?: number,
): CalendarLiveV30Receipt | undefined => {
  const file = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV30Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: CalendarLiveV30Request | undefined;
  receipt: CalendarLiveV30Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: CalendarLiveV30RemotePhase,
): CalendarLiveV30ResponseContract => ({
  ...CALENDAR_LIVE_V30_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: CalendarLiveV30Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: CALENDAR_LIVE_V30_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: CALENDAR_LIVE_V30_DYNAMIC_TOOL,
      target: CALENDAR_LIVE_V30_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "calendar-live-v30-plugin-response-v2",
  brokerVersion: CALENDAR_LIVE_V30_BROKER_VERSION,
  target: CALENDAR_LIVE_V30_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*CALENDAR_LIVE_V30_PROGRAM_START*/${program}/*CALENDAR_LIVE_V30_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*CALENDAR_LIVE_V30_PROGRAM_START*/";
  const endMarker = "/*CALENDAR_LIVE_V30_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Calendar live v30 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createCalendarLiveV30Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Calendar live v30 private signing key does not match authorized public-key identity",
    );
  const manifest: CalendarLiveV30Manifest = {
    artifactVersion: "calendar-live-v30-broker-manifest-v2",
    brokerVersion: CALENDAR_LIVE_V30_BROKER_VERSION,
    ...
    captureRequests: CALENDAR_LIVE_V30_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateCalendarLiveV30Request(
  directory: string,
  request: CalendarLiveV30Request,
): void {
  ...
  if (
    request.artifactVersion !== "calendar-live-v30-broker-request-v2" ||
    request.brokerVersion !== CALENDAR_LIVE_V30_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== CALENDAR_LIVE_V30_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Calendar live v30 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueCalendarLiveV30Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: CalendarLiveV30RemotePhase,
  program: string,
  options: CalendarLiveV30IssueOptions = {},
): CalendarLiveV30Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<CalendarLiveV30Request>(targetPath);
    validateCalendarLiveV30Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Calendar live v30 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(calendarLiveV1ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Calendar live v30 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Calendar live v30 empty code envelope refused");
  if (phase === "capture") validateCalendarLiveV30TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Calendar live v30 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: CALENDAR_LIVE_V30_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? CALENDAR_LIVE_V30_SIGNED_WRITER_TIMEOUT_MS
        : CALENDAR_LIVE_V30_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: CalendarLiveV30RequestBody = {
    artifactVersion: "calendar-live-v30-broker-request-v2",
    brokerVersion: CALENDAR_LIVE_V30_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: CalendarLiveV30Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateCalendarLiveV30Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistCalendarLiveV30RawResponse(
  directory: string,
  phase: CalendarLiveV30RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Calendar live v30 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<CalendarLiveV30Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Calendar live v30 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = calendarLiveV1ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Calendar live v30 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): CalendarLiveV30Receipt => {
  const body: Omit<CalendarLiveV30Receipt, "receiptSha256"> = {
    artifactVersion: "calendar-live-v30-broker-receipt-v2",
    brokerVersion: CALENDAR_LIVE_V30_BROKER_VERSION,
...
export function acceptCalendarLiveV30Response<T = unknown>(
  directory: string,
  phase: CalendarLiveV30RemotePhase,
  options: {
    captureIndex?: number;
...
): CalendarLiveV30AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Calendar live v30 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<CalendarLiveV30Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Calendar live v30 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "calendar-live-v30-plugin-response-v2" ||
    plugin?.brokerVersion !== CALENDAR_LIVE_V30_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Calendar live v30 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistCalendarLiveV30TechnicalGates(
  directory: string,
  rootProofs: readonly CalendarLiveV30RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertCalendarLiveV30RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Calendar live v30 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Calendar live v30 probe payload hash malformed");
  const gates = Object.fromEntries(
    CALENDAR_LIVE_V30_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "calendar-live-v30-host-technical-gates-v2",
    brokerVersion: CALENDAR_LIVE_V30_BROKER_VERSION,
...
export function validateCalendarLiveV30TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "calendar-live-v30-host-technical-gates-v2" ||
    ...
    artifact.roots.length !== CALENDAR_LIVE_V30_SOURCE_ROOTS ||
    ...
    CALENDAR_LIVE_V30_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Calendar live v30 technical gates artifact");
}

export function validateCalendarLiveV30Receipt(
  directory: string,
  phase: CalendarLiveV30RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateCalendarLiveV30Request(directory, request);
  const receipt = json<CalendarLiveV30Receipt>(
  ...
  if (
    receipt.artifactVersion !== "calendar-live-v30-broker-receipt-v2" ||
    receipt.brokerVersion !== CALENDAR_LIVE_V30_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Calendar live v30 accepted receipt: ${request.requestId}`,
    );
}

export function readCalendarLiveV30AcceptedRawPayload(
  directory: string,
  phase: CalendarLiveV30RemotePhase,
  captureIndex?: number,
): unknown {
  validateCalendarLiveV30Receipt(directory, phase, captureIndex);
...
export function createCalendarLiveV30FakeDynamicResponse(
  request: CalendarLiveV30Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? CALENDAR_LIVE_V30_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? CALENDAR_LIVE_V30_TARGET,
    ...overrides.extra,
  };
}

export function listCalendarLiveV30PersistedRequests(
  directory: string,
): CalendarLiveV30Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<CalendarLiveV30Request>(file)] : [];
    });
}
