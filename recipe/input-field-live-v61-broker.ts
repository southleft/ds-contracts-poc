...
import {
  assertInputLiveV61RootProofs,
  INPUT_LIVE_V61_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V61_RESPONSE_CONTRACTS,
  type InputLiveV61RootProof,
} from "./input-field-live-v61-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V61_BROKER_VERSION = "input-live-v61-broker-v2";
export const INPUT_LIVE_V61_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V61_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V61_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V61_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V61_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V61_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV61RemotePhase =
  (typeof INPUT_LIVE_V61_REMOTE_PHASES)[number];
export type InputLiveV61AuthorizationMode = "live" | "simulated";

export interface InputLiveV61CommitPins {
...
export interface InputLiveV61TransactionAuthorization extends InputLiveV61CommitPins {
  mode: InputLiveV61AuthorizationMode;
...
export interface InputLiveV61Manifest {
  artifactVersion: "input-live-v61-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V61_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V61_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V61_TARGET;
  authorization: InputLiveV61TransactionAuthorization;
...
export interface InputLiveV61ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV61RequestBody {
  artifactVersion: "input-live-v61-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V61_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV61RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV61CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V61_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV61ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV61Request extends InputLiveV61RequestBody {
...
export interface InputLiveV61Receipt {
  artifactVersion: "input-live-v61-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V61_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV61RemotePhase;
...
export interface InputLiveV61AcceptedResponse<T = unknown> {
  request: InputLiveV61Request;
  receipt: InputLiveV61Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV61IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV61ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V61_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V61_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V61_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V61_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V61_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V61_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V61_TARGET.editorType;
};
...
export const inputLiveV61RequestSequence = (
  phase: InputLiveV61RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV61RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV61RequestPath = (
  directory: string,
  phase: InputLiveV61RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV61ResponsePath = (
  directory: string,
  phase: InputLiveV61RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV61ReceiptPath = (
  directory: string,
  phase: InputLiveV61RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV61Manifest => {
  const manifest = json<InputLiveV61Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v61-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V61_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V61_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV61RemotePhase,
  captureIndex?: number,
): InputLiveV61Request | undefined => {
  const file = inputLiveV61RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV61Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV61RemotePhase,
  captureIndex?: number,
): InputLiveV61Receipt | undefined => {
  const file = inputLiveV61ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV61Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV61Request | undefined;
  receipt: InputLiveV61Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV61RemotePhase,
): InputLiveV61ResponseContract => ({
  ...INPUT_LIVE_V61_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V61_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V61_DYNAMIC_TOOL,
      target: INPUT_LIVE_V61_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v61-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V61_BROKER_VERSION,
  target: INPUT_LIVE_V61_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V61_PROGRAM_START*/${program}/*INPUT_LIVE_V61_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V61_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V61_PROGRAM_END*/";
...
export function createInputLiveV61Transaction(
  directory: string,
  ...
  const manifest: InputLiveV61Manifest = {
    artifactVersion: "input-live-v61-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V61_BROKER_VERSION,
...
export function validateInputLiveV61Request(
  directory: string,
  request: InputLiveV61Request,
): void {
  ...
  const expectedSequence = inputLiveV61RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v61-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V61_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V61_TARGET.fileKey ||
...
export function issueInputLiveV61Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV61RemotePhase,
  program: string,
  options: InputLiveV61IssueOptions = {},
): InputLiveV61Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV61RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV61Request>(targetPath);
    validateInputLiveV61Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV61ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV61TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV61RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V61_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V61_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V61_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV61RequestBody = {
    artifactVersion: "input-live-v61-broker-request-v2",
    brokerVersion: INPUT_LIVE_V61_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV61Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV61Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV61RawResponse(
  directory: string,
  phase: InputLiveV61RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV61RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV61Request>(requestFile);
  if (
  ...
  const target = inputLiveV61ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV61Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v61-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V61_BROKER_VERSION,
...
export function acceptInputLiveV61Response<T = unknown>(
  directory: string,
  phase: InputLiveV61RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV61AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV61ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV61Request>(
    inputLiveV61RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV61Request(directory, request);
  if (phase === "capture") validateInputLiveV61TechnicalGates(directory);
  const rawFile = inputLiveV61ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v61-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V61_BROKER_VERSION ||
...
export function persistInputLiveV61TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV61RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV61RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V61_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v61-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V61_BROKER_VERSION,
...
export function validateInputLiveV61TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v61-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V61_TECHNICAL_GATES.some(
...
export function validateInputLiveV61Receipt(
  directory: string,
  phase: InputLiveV61RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV61Request(directory, request);
  const receipt = json<InputLiveV61Receipt>(
    inputLiveV61ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV61ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v61-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V61_BROKER_VERSION ||
...
export function readInputLiveV61AcceptedRawPayload(
  directory: string,
  phase: InputLiveV61RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV61Receipt(directory, phase, captureIndex);
...
export function createInputLiveV61FakeDynamicResponse(
  request: InputLiveV61Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V61_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V61_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV61PersistedRequests(
  directory: string,
): InputLiveV61Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV61Request>(file)] : [];
    });
}

export const inputLiveV61Sha256 = sha256;
