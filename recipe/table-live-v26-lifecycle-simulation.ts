...
import { tableLiveV2Sha256 } from "./table-live-v26-broker.js";

export interface TableLiveV26LifecycleSimulationReport {
  artifactVersion: "table-live-v26-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV26AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV26Lifecycle(): Promise<TableLiveV26LifecycleSimulationReport> {
  await buildTableLiveV26Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V26_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV26AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v26-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V26_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V26_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV26History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV26History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV26AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V26_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V26_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV26History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV26History(authorized, "pending");
    ...
    await buildTableLiveV26Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V26_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV26AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV26Lifecycle(), null, 2)}\n`,
  );
