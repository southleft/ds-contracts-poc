...
import { INPUT_LIVE_V25_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v25.js";
import { verifyInputLiveV25Authorization } from "./input-field-live-v25-authorization.js";
import { runInputLiveV25Preflight } from "./input-field-live-v25-preflight.js";
import { InputLiveV25Orchestrator } from "./run-input-field-live-v25.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V25_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v25-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV25Authorization(root);
    runInputLiveV25Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV25Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
