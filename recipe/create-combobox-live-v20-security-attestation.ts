...
import { verifyComboboxLiveV20Authorization } from "./combobox-live-v20-authorization.js";
import { COMBOBOX_LIVE_V20_TARGET } from "./combobox-live-v20-broker.js";
import {
  COMBOBOX_LIVE_V20_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV20Authorization();
...
const attestation: ComboboxLiveV20SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV20SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v20 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V20_SECURITY_ATTESTATION_DEFAULT_PATH,
);
