...
import { buildTableLiveV19CleanupRuntime } from "./table-live-v19-cleanup.js";
import { type TableLiveV19FixedFixedPoint } from "./table-live-v19-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV19RestoreProgram,
  validateTableLiveV19RestorePayload,
  type TableLiveV19RestorePayload,
} from "./table-live-v19-restore.js";
import {
  buildTableLiveV19RawPropertyRuntime,
  normalizeTableLiveV19Scene,
  type TableLiveV19RawNode,
} from "./table-live-v19-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v19.js";
import {
...
export const TABLE_LIVE_V19_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V19_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V19_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V19_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V19_VARIANT_COUNT = 20;
export const TABLE_LIVE_V19_SET_COUNT = 6;
export const TABLE_LIVE_V19_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V19_HOST_PHASES = 3;
export const TABLE_LIVE_V19_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V19_RESTORE_COUNT = 8;
export const TABLE_LIVE_V19_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V19_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV19SourceId = (typeof TABLE_LIVE_V19_SOURCE_IDS)[number];
export type TableLiveV19Kind = "table" | "row" | "cell";

export interface TableLiveV19SourceIdentity {
  source: TableLiveV19SourceId;
...
export interface TableLiveV19WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV19ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV19SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV19RawNode;
    rowScene: TableLiveV19RawNode;
    cellScene: TableLiveV19RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV19RootProof {
  source: TableLiveV19SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV19FixedFixedPoint;
}

export interface TableLiveV19ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV19SourceId;
    adapterIdentity: string;
...
export interface TableLiveV19CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV19SourceId;
  adapterIdentity: string;
...
export interface TableLiveV19CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV19SourceId;
  frameWidth: number;
...
export interface TableLiveV19CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v19 ${label} must be an object`);
...
export function validateTableLiveV19WriterPayload(
  value: unknown,
): TableLiveV19WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V19_NAMESPACE ||
    ...
    throw new TypeError("Table live v19 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v19 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v19 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV19SourceIdentity,
  kind: TableLiveV19Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV19ExtractProgram(
  writer: TableLiveV19WriterOwnership,
  sources: readonly TableLiveV19SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v19 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V19_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V19_NAMESPACE)}
${buildTableLiveV19RawPropertyRuntime()}
...
export function validateTableLiveV19ExtractPayload(
  value: unknown,
  writer: TableLiveV19WriterOwnership,
): TableLiveV19ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v19 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V19_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v19 extract schema/two-root mismatch");
  return payload as TableLiveV19ExtractPayload;
}

export function proveTableLiveV19Roots<Instance>(
  extract: TableLiveV19ExtractPayload,
  sources: readonly (TableLiveV19SourceIdentity & {
...
): TableLiveV19RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v19 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v19 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV19Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV19Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV19Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV19FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v19 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v19 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v19 compile lost owned sets");
...
export function assertTableLiveV19RootProofs(
  proofs: readonly TableLiveV19RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v19 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v19 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV19ProbeProgram(
  writer: TableLiveV19WriterOwnership,
  sources: readonly TableLiveV19SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V19_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV19ProbePayload(
  value: unknown,
  writer: TableLiveV19WriterOwnership,
): TableLiveV19ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV19ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V19_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v19 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV19CaptureManifest(
  cells: readonly TableLiveV19CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V19_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V19_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V19_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V19_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v19 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV19CaptureCell[],
): string => {
  validateTableLiveV19CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV19CaptureProgram(
  writer: TableLiveV19WriterOwnership,
  cell: TableLiveV19CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V19_CAPTURE_COUNT
  )
    throw new TypeError("Table live v19 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v19 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V19_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV19CapturePayload(
  value: unknown,
  cell: TableLiveV19CaptureCell,
  rawResponseBytes: number,
): TableLiveV19CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV19CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V19_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V19_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v19 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV19CaptureResponses(
  manifest: readonly TableLiveV19CaptureCell[],
  responses: readonly TableLiveV19CapturePayload[],
): void {
  validateTableLiveV19CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v19 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV19ObjectiveReport {
  artifactVersion: "table-live-v19-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV19Objective(
  manifest: readonly TableLiveV19CaptureCell[],
  responses: readonly TableLiveV19CapturePayload[],
): TableLiveV19ObjectiveReport {
  assertTableLiveV19CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v19-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV19CleanupProgram(
  writer: TableLiveV19WriterOwnership,
): string {
  const baseProgram = buildTableLiveV19CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V19_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v19 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV19CleanupPayload(
  payload: unknown,
  writer: TableLiveV19WriterOwnership,
): TableLiveV19CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV19CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v19 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V19_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV19WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
