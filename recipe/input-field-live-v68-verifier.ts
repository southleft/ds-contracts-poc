...
export const INPUT_LIVE_V68_VERIFIER_VERSION = "input-live-v68-verifier-v1";

export function buildInputLiveV68RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV68RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV68RawNode[];
}

export interface InputLiveV68NormalizedScene {
...
export interface InputLiveV68SceneProof {
...
export function normalizeInputLiveV68Scene(
  root: InputLiveV68RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV68NormalizedScene {
  const canonicalBindings: InputLiveV68NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV68RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV68Scene<Instance>(
  normalized: InputLiveV68NormalizedScene,
...
export function compileInputLiveV68ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV68PreCaptureGates(payload: unknown): void {
