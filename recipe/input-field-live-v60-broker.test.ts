...
import {
  ...
  createInputLiveV60FakeDynamicResponse,
  ...
  listInputLiveV60PersistedRequests,
  persistInputLiveV60RawResponse,
  persistInputLiveV60TechnicalGates,
  validateInputLiveV60Receipt,
  validateInputLiveV60Request,
  type InputLiveV60RemotePhase,
...
import {
  ...
  type InputLiveV60ExtractPayload,
  type InputLiveV60ProbePayload,
  type InputLiveV60RestorePayload,
  type InputLiveV60RootProof,
  type InputLiveV60WriterOwnership,
} from "./input-field-live-v60-contract.js";
import {
  InputLiveV60Orchestrator,
  simulatedInputLiveV60Authorization,
} from "./run-input-field-live-v60.js";
...
const captures: InputLiveV60CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV60Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v60-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV60Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV60CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV60FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV60FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V60_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V60_TARGET);
  persistInputLiveV60RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV60Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV60Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV60WriterOwnership,
): InputLiveV60RestorePayload => ({
...
const rootProofs = (): InputLiveV60RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV60CaptureCell,
): InputLiveV60CapturePayload => {
  const bytes = Buffer.from(`input-live-v60-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV60Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV60Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV60RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV60RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV60ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV60ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV60CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV60CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV60CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV60Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV60CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV60PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV60ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV60Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV60Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV60Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV60Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V60_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV60RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V60_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV60Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV60Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV60ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV60Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV60WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV60RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV60Request;
    validateInputLiveV60Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV60Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV60CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v60-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV60Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV60Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV60Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V60_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV60RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V60_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV60Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV60Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV60RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V60_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV60Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV60Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV60Request;
