...
import { INPUT_LIVE_V61_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v61.js";
import { verifyInputLiveV61Authorization } from "./input-field-live-v61-authorization.js";
import { runInputLiveV61Preflight } from "./input-field-live-v61-preflight.js";
import { InputLiveV61Orchestrator } from "./run-input-field-live-v61.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V61_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v61-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV61Authorization(root);
    runInputLiveV61Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV61Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
