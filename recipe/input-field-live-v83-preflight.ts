...
import {
  ...
  verifyInputLiveV83Authorization,
  type InputLiveV83AntecedentIndex,
  type InputLiveV83AuthorizationProof,
} from "./input-field-live-v83-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V83_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v83/operator-security-attestation-template.json";
export const INPUT_LIVE_V83_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v83-security-attestation.json";

export interface InputLiveV83SecurityAttestation {
...
export interface InputLiveV83PreflightReport {
  artifactVersion: "input-live-v83-preflight-v1";
...
export const inputLiveV83AttestationSha256 = (
  value: Omit<InputLiveV83SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV83SecurityAttestation(
  value: unknown,
  proof: InputLiveV83AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v83-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V83_TARGET) ||
...
export function validateInputLiveV83ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV83Authorization");
  const preflight = source.indexOf("runInputLiveV83Preflight");
  const initialize = source.indexOf("InputLiveV83Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV83Preflight(
  root: string,
  proof: InputLiveV83AuthorizationProof,
  attestationPath = INPUT_LIVE_V83_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV83PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V83_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V83_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V83_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV83AntecedentIndex;
  failures.push(...validateInputLiveV83AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V83_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V83_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v83-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV83ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV83SecurityAttestation;
  return {
    artifactVersion: "input-live-v83-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V83_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV83Preflight(
    process.cwd(),
    verifyInputLiveV83Authorization(),
    attestationPath,
