...
import { calendarLiveV1Sha256 } from "./calendar-live-v29-broker.js";

export interface CalendarLiveV29LifecycleSimulationReport {
  artifactVersion: "calendar-live-v29-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV29AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV29Lifecycle(): Promise<CalendarLiveV29LifecycleSimulationReport> {
  await buildCalendarLiveV29Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V29_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV29AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v29-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V29_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V29_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV29History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV29History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV29AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V29_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V29_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV29History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV29History(authorized, "pending");
    ...
    await buildCalendarLiveV29Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V29_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV29AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV29Lifecycle(), null, 2)}\n`,
  );
