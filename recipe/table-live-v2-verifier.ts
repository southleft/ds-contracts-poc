...
import {
  verifyTableLiveV2FixedSceneFixedPoint,
  type TableLiveV2FixedFixedPoint,
} from "./table-live-v2-fixed-point.js";

export const TABLE_LIVE_V2_VERIFIER_VERSION = "table-live-v2-verifier-v1";

export function buildTableLiveV2RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV2RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV2RawNode[];
}

export interface TableLiveV2NormalizedScene {
...
export interface TableLiveV2SceneProof {
...
export function normalizeTableLiveV2Scene(
  root: TableLiveV2RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV2NormalizedScene {
  const canonicalBindings: TableLiveV2NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV2RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV2Scene<Instance>(
  normalized: TableLiveV2NormalizedScene,
...
): TableLiveV2SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV2FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV2ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV2PreCaptureGates(payload: unknown): void {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("table live v2 pre-capture gate payload absent");
