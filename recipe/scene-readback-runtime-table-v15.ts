...
/**
 * Table live v15 extract runtime. Keeps the v6 skip of untagged
 * `table/row/owned-cell-label` TEXT bind hosts. Extract bytes are
 * unchanged from v14. The v15 teaching lives in host-normalize
 * (`scene-readback-table-v1.ts`): omit `effects` on
 * `table/variant` because compile variants omit `effects`.
 * Does not omit variant `strokes` or `cornerRadius`.
 * Does not invent `effects` onto compile.
 */
