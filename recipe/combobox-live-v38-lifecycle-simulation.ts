...
export interface ComboboxLiveV38LifecycleSimulationReport {
  artifactVersion: "combobox-live-v38-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV38AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV38Lifecycle(): Promise<ComboboxLiveV38LifecycleSimulationReport> {
  await buildComboboxLiveV38Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V38_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV38AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v38-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V38_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V38_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV38History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV38History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV38AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V38_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V38_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV38History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV38History(authorized, "pending");
    ...
    await buildComboboxLiveV38Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V38_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV38AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV38Lifecycle(), null, 2)}\n`,
  );
