...
import { CALENDAR_LIVE_V42_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v42.js";
import { verifyCalendarLiveV42Authorization } from "./calendar-live-v42-authorization.js";
import { runCalendarLiveV42Preflight } from "./calendar-live-v42-preflight.js";
import { CalendarLiveV42Orchestrator } from "./run-calendar-live-v42.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v42 requires exact authorization artifact ${CALENDAR_LIVE_V42_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v42-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV42Authorization(root);
    runCalendarLiveV42Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV42Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV42Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
