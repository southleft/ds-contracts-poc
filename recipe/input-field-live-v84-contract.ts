...
} from "./input-field-live-v3-verifier-v84.js";
import {
  buildInputLiveV84RawPropertyRuntime,
  normalizeInputLiveV84Scene,
  type InputLiveV84RawNode,
} from "./input-field-live-v84-verifier.js";
...
export {
  ...
  validateInputLiveV84RestorePayload,
  type InputLiveV84RestorePayload,
} from "./input-field-live-v84-restore.js";

export const INPUT_LIVE_V84_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V84_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V84_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V84_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V84_VARIANT_COUNT = 256;
export const INPUT_LIVE_V84_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V84_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV84SourceId = (typeof INPUT_LIVE_V84_SOURCE_IDS)[number];

export interface InputLiveV84SourceIdentity {
  source: InputLiveV84SourceId;
...
export interface InputLiveV84WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV84ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV84SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV84RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV84RootProof {
  source: InputLiveV84SourceId;
  adapterIdentity: string;
...
export interface InputLiveV84ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV84SourceId;
    adapterIdentity: string;
...
export interface InputLiveV84CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV84SourceId;
  adapterIdentity: string;
...
export interface InputLiveV84CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV84SourceId;
  strata: InputLiveV84CaptureCell["strata"];
...
export interface InputLiveV84CleanupPayload {
...
export function validateInputLiveV84WriterPayload(
  value: unknown,
): InputLiveV84WriterOwnership {
...
const identityPlan = (source: InputLiveV84SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV84ExtractProgram(
  writer: InputLiveV84WriterOwnership,
  sources: readonly InputLiveV84SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V84_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V84_NAMESPACE)}
${buildInputLiveV84RawPropertyRuntime()}
...
export function validateInputLiveV84ExtractPayload(
  value: unknown,
  writer: InputLiveV84WriterOwnership,
): InputLiveV84ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V84_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV84ExtractPayload;
}

export function proveInputLiveV84Roots<Instance>(
  extract: InputLiveV84ExtractPayload,
  sources: readonly (InputLiveV84SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV84Scene(
      root.scene,
...
export function assertInputLiveV84RootProofs(
  proofs: readonly InputLiveV84RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV84ProbeProgram(
  writer: InputLiveV84WriterOwnership,
  sources: readonly InputLiveV84SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V84_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
for(const set of sets){
 ...
 for(const component of components){
  ...
  const overlayLabelRow=all.find(node=>role(node)==="input-field/label-row"&&node.layoutPositioning==="ABSOLUTE");void "INPUT-PROBE-EXCLUDE-OVERLAY-LABEL-AABB";const clipSemantic=semantic.filter(node=>!(overlayLabelRow&&(role(node)==="input-field/label"||role(node)==="input-field/required-indicator")));
  const occupancySpacer=node=>node.opacity===0&&(role(node)==="input-field/content/placeholder"||role(node)==="input-field/content/value");void "INPUT-PROBE-EXCLUDE-OPACITY-ZERO-OCCUPANCY-OVERLAP";const overlapSemantic=semantic.filter(node=>!occupancySpacer(node));let maximumOverlap=0;for(let i=0;i<overlapSemantic.length;i++)for(let j=i+1;j<overlapSemantic.length;j++)maximumOverlap=Math.max(maximumOverlap,overlap(box(overlapSemantic[i]),box(overlapSemantic[j])));
  ...
  cells.push({source,adapterIdentity,cellKey:[source,axis.Size,axis.State,axis.Content,axis.Required,axis.Adornments].join("/"),rolesExact:expected.every(name=>byRole(name).length===1)&&required&&leading&&trailing,stateSemanticsExact,adornmentPayloadExact,noFakeLayout,visibleAreaLoss:Math.max(0,...clipSemantic.map(node=>visibleLoss(box(node),componentBox))),overlapPixels:maximumOverlap});
 }
 const instance=set.defaultVariant.createInstance();page.appendChild(instance);const before=snapshot(instance),beforeWidth=instance.width,original=plain(instance),allBefore=nodes(instance),surface=allBefore.find(node=>role(node)==="input-field/surface"),contentRow=allBefore.find(node=>role(node)==="input-field/content-row"),contentText=allBefore.find(node=>node.type==="TEXT"&&(role(node)==="input-field/content/placeholder"||role(node)==="input-field/content/value")),reflowTarget=contentRow||contentText,surfaceWidth=surface&&surface.width,reflowTargetWidth=reflowTarget&&reflowTarget.width;
 instance.resizeWithoutConstraints(beforeWidth+64,instance.height);const reflowPassed=instance.width===beforeWidth+64&&surface&&surface.width>surfaceWidth&&reflowTarget&&reflowTarget.width>reflowTargetWidth;const measureContentFill=node=>{if(!node||node.type!=="TEXT")return false;const hidden=node.visible===false;if(hidden){void "INPUT-PROBE-MEASURE-HIDDEN-CONTENT-FILL";node.visible=true;}const fill=node.layoutSizingHorizontal==="FILL";if(hidden)node.visible=false;return fill;};const contentFillPassed=!!measureContentFill(contentText);instance.resizeWithoutConstraints(beforeWidth,instance.height);
 ...
 sources.push({source,adapterIdentity,variants:128,visitedVariants:visited.size,reflowPassed:!!reflowPassed,contentFillPassed:!!contentFillPassed,bindingCompatibilityPassed,noFakeLayoutPassed:sourceCells.every(cell=>cell.noFakeLayout),adornmentPayloadPassed:sourceCells.every(cell=>cell.adornmentPayloadExact),stateSemanticsPassed:sourceCells.every(cell=>cell.stateSemanticsExact),switchingRestored,textPropertiesRestored,exactSceneRestoration:before===after});
}
...
export function validateInputLiveV84ProbePayload(
  value: unknown,
  writer: InputLiveV84WriterOwnership,
): InputLiveV84ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV84ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V84_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V84_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV84CaptureManifest(
  cells: readonly InputLiveV84CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V84_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V84_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V84_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V84_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V84_SOURCE_IDS.some(
...
export const inputLiveV84CaptureManifestSha256 = (
  cells: readonly InputLiveV84CaptureCell[],
): string => {
  validateInputLiveV84CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV84CaptureProgram(
  writer: InputLiveV84WriterOwnership,
  cell: InputLiveV84CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V84_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V84_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V84_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV84CapturePayload(
  value: unknown,
  cell: InputLiveV84CaptureCell,
  rawResponseBytes: number,
): InputLiveV84CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV84CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V84_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V84_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV84CaptureResponses(
  manifest: readonly InputLiveV84CaptureCell[],
  responses: readonly InputLiveV84CapturePayload[],
): void {
  validateInputLiveV84CaptureManifest(manifest);
  if (
...
export interface InputLiveV84ObjectiveReport {
  artifactVersion: "input-live-v84-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV84Objective(
  manifest: readonly InputLiveV84CaptureCell[],
  responses: readonly InputLiveV84CapturePayload[],
  readReference: (cell: InputLiveV84CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV84CaptureCell) => Buffer,
): InputLiveV84ObjectiveReport {
  assertInputLiveV84CaptureResponses(manifest, responses);
...
export function buildInputLiveV84CleanupProgram(
  writer: InputLiveV84WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V84_NAMESPACE,
...
export function validateInputLiveV84CleanupPayload(
  value: unknown,
  writer: InputLiveV84WriterOwnership,
): InputLiveV84CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV84CleanupPayload;
...
export const INPUT_LIVE_V84_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV84WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
