...
export type ComboboxLiveV7HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV7AuthorizationArtifact {
  artifactVersion: "combobox-live-v7-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V7_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV7AntecedentIndex {
...
export interface ComboboxLiveV7HistoryState {
  index?: ComboboxLiveV7AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV7AuthorizationArtifact;
...
export interface ComboboxLiveV7AuthorizationProof extends ComboboxLiveV7TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V7_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V7_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V7_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V7_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v7-authorization") ||
  artifactPath.includes("combobox-live-v7-preflight") ||
  artifactPath.includes("combobox-live-v7-authorized") ||
  artifactPath.includes("create-combobox-live-v7-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v7-status") ||
...
export function validateComboboxLiveV7AntecedentIndex(
  index: ComboboxLiveV7AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v7-antecedent-index-v1" ||
...
export function validateComboboxLiveV7UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV7History\(\s*["']/.test(source) ||
    /readComboboxLiveV7HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV7AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV7AuthorizationArtifact {
  ...
  ) as ComboboxLiveV7AntecedentIndex;
  const indexFailures = validateComboboxLiveV7AntecedentIndex(index);
...
export function validateComboboxLiveV7AuthorizationArtifact(
  artifact: ComboboxLiveV7AuthorizationArtifact | undefined,
  index: ComboboxLiveV7AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v7-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v7" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V7_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V7_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V7_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V7_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V7_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V7_SOURCE_ROOTS ||
...
export function validateComboboxLiveV7History(
  state: ComboboxLiveV7HistoryState,
  expected: ComboboxLiveV7HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV7AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV7AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV7HistoryState(
  root = process.cwd(),
): ComboboxLiveV7HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V7_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV7AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V7_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V7_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V7_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV7History(
  expected: ComboboxLiveV7HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV7HistoryState {
  const state = readComboboxLiveV7HistoryState(root);
  const failures = validateComboboxLiveV7History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v7 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV7Authorization(
  root = process.cwd(),
): ComboboxLiveV7AuthorizationProof {
  const state = verifyComboboxLiveV7History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V7_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v7/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v7.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V7_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V7_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V7_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV7HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV7History(expected);
  process.stdout.write(
    `Combobox live v7 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
