...
} from "./input-field-live-v3-verifier-v45.js";
import {
  buildInputLiveV45RawPropertyRuntime,
  normalizeInputLiveV45Scene,
  type InputLiveV45RawNode,
} from "./input-field-live-v45-verifier.js";
...
export {
  ...
  type InputLiveV45RestorePayload,
} from "./input-field-live-v45-restore.js";

export const INPUT_LIVE_V45_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V45_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V45_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V45_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V45_VARIANT_COUNT = 256;
export const INPUT_LIVE_V45_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V45_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV45SourceId = (typeof INPUT_LIVE_V45_SOURCE_IDS)[number];

export interface InputLiveV45SourceIdentity {
  source: InputLiveV45SourceId;
...
export interface InputLiveV45WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV45ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV45SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV45RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV45RootProof {
  source: InputLiveV45SourceId;
  adapterIdentity: string;
...
export interface InputLiveV45ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV45SourceId;
    adapterIdentity: string;
...
export interface InputLiveV45CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV45SourceId;
  adapterIdentity: string;
...
export interface InputLiveV45CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV45SourceId;
  strata: InputLiveV45CaptureCell["strata"];
...
export interface InputLiveV45CleanupPayload {
...
export function validateInputLiveV45WriterPayload(
  value: unknown,
): InputLiveV45WriterOwnership {
...
const identityPlan = (source: InputLiveV45SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV45ExtractProgram(
  writer: InputLiveV45WriterOwnership,
  sources: readonly InputLiveV45SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V45_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V45_NAMESPACE)}
${buildInputLiveV45RawPropertyRuntime()}
...
export function validateInputLiveV45ExtractPayload(
  value: unknown,
  writer: InputLiveV45WriterOwnership,
): InputLiveV45ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V45_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV45ExtractPayload;
}

export function proveInputLiveV45Roots<Instance>(
  extract: InputLiveV45ExtractPayload,
  sources: readonly (InputLiveV45SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV45Scene(
      root.scene,
...
export function assertInputLiveV45RootProofs(
  proofs: readonly InputLiveV45RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV45ProbeProgram(
  writer: InputLiveV45WriterOwnership,
  sources: readonly InputLiveV45SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V45_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV45ProbePayload(
  value: unknown,
  writer: InputLiveV45WriterOwnership,
): InputLiveV45ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV45ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V45_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V45_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV45CaptureManifest(
  cells: readonly InputLiveV45CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V45_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V45_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V45_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V45_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V45_SOURCE_IDS.some(
...
export const inputLiveV45CaptureManifestSha256 = (
  cells: readonly InputLiveV45CaptureCell[],
): string => {
  validateInputLiveV45CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV45CaptureProgram(
  writer: InputLiveV45WriterOwnership,
  cell: InputLiveV45CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V45_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V45_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V45_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV45CapturePayload(
  value: unknown,
  cell: InputLiveV45CaptureCell,
  rawResponseBytes: number,
): InputLiveV45CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV45CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V45_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V45_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV45CaptureResponses(
  manifest: readonly InputLiveV45CaptureCell[],
  responses: readonly InputLiveV45CapturePayload[],
): void {
  validateInputLiveV45CaptureManifest(manifest);
  if (
...
export interface InputLiveV45ObjectiveReport {
  artifactVersion: "input-live-v45-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV45Objective(
  manifest: readonly InputLiveV45CaptureCell[],
  responses: readonly InputLiveV45CapturePayload[],
  readReference: (cell: InputLiveV45CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV45CaptureCell) => Buffer,
): InputLiveV45ObjectiveReport {
  assertInputLiveV45CaptureResponses(manifest, responses);
...
export function buildInputLiveV45CleanupProgram(
  writer: InputLiveV45WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V45_NAMESPACE,
...
export function validateInputLiveV45CleanupPayload(
  value: unknown,
  writer: InputLiveV45WriterOwnership,
): InputLiveV45CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV45CleanupPayload;
...
export const INPUT_LIVE_V45_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV45WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
