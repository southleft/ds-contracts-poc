...
import {
  TABLE_LIVE_V4_DYNAMIC_TOOL,
  TABLE_LIVE_V4_TARGET,
  type TableLiveV4TransactionAuthorization,
} from "./table-live-v4-broker.js";
import {
  TABLE_LIVE_V4_AUTHORIZATION_PATH,
  TABLE_LIVE_V4_INDEX_PATH,
} from "./build-table-live-proof-v4.js";
import {
  TABLE_LIVE_V4_CAPTURE_COUNT,
  TABLE_LIVE_V4_HOST_PHASES,
  TABLE_LIVE_V4_REMOTE_REQUESTS,
  TABLE_LIVE_V4_SOURCE_ROOTS,
} from "./table-live-v4-contract.js";
import { canonicalJson } from "./normalize.js";

export type TableLiveV4HistoryExpectation = "pending" | "authorized";

export interface TableLiveV4AuthorizationArtifact {
  artifactVersion: "table-live-v4-capture-authorization-v1";
  authorizationId: "table-live-v4";
  status: "authorization declared; runtime security prerequisites still mandatory";
  authorizationIntent: true;
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V4_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV4AntecedentIndex {
  artifactVersion: "table-live-v4-antecedent-index-v1";
  status: string;
...
export interface TableLiveV4HistoryState {
  index?: TableLiveV4AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV4AuthorizationArtifact;
...
export interface TableLiveV4AuthorizationProof
  extends TableLiveV4TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V4_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V4_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V4_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V4_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v4-authorization") ||
  artifactPath.includes("table-live-v4-preflight") ||
  artifactPath.includes("table-live-v4-authorized") ||
  artifactPath.includes("create-table-live-v4-security-attestation") ||
  artifactPath.includes("table-live-pivot-v4-status") ||
...
export function validateTableLiveV4AntecedentIndex(
  index: TableLiveV4AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v4-antecedent-index-v1" ||
    index.authorizationCanBeAddedWithoutAntecedentRebuild !== true
  )
    failures.push("table v4 antecedent index missing or malformed");
  const artifactEntries = Object.entries(index?.artifacts ?? {});
  if (artifactEntries.length === 0)
    failures.push("table v4 antecedent hash denominator is empty");
  for (const [artifactPath, metadata] of artifactEntries) {
    ...
    if (
      ...
      failures.push(`malformed table v4 antecedent hash: ${artifactPath}`);
  }
  ...
  if (index?.hashSetSha256 !== expectedHashSet)
    failures.push("table v4 antecedent hash-set digest mismatch");
  if (
    ...
    failures.push("table v4 antecedent counts missing or drifted");
  for (const required of [
    TABLE_LIVE_V4_AUTHORIZATION_PATH,
    "recipe/table-live-v4-authorization.ts",
    "recipe/table-live-v4-authorization.test.ts",
    "recipe/table-live-v4-preflight.ts",
    "recipe/evidence/table-live-pivot-v4/operator-security-attestation-template.json",
    "recipe/evidence/status-index.json",
  ])
    if (!index?.authorizationLifecycleExcluded?.includes(required))
      failures.push(`table v4 lifecycle exclusion missing: ${required}`);
  const leaks = forbiddenAntecedentFieldPaths(index);
  if (leaks.length)
    failures.push(`table v4 antecedent result leakage: ${leaks.join(",")}`);
  return failures;
}

export function validateTableLiveV4UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV4History\(\s*["']/.test(source) ||
    /readTableLiveV4HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV4AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV4AuthorizationArtifact {
  if (!SHA40.test(options.antecedentCommit))
    throw new TypeError("table v4 antecedent commit must be a full SHA");
  const index = JSON.parse(
    Buffer.from(options.antecedentIndexBytes).toString("utf8"),
  ) as TableLiveV4AntecedentIndex;
  const indexFailures = validateTableLiveV4AntecedentIndex(index);
  if (indexFailures.length)
    throw new TypeError(
      `table v4 antecedent index refused:\n${indexFailures.join("\n")}`,
    );
  const publicKey =
  ...
  return {
    ...
    antecedent: {
      ...
      indexPath: TABLE_LIVE_V4_INDEX_PATH,
    ...
    denominator: {
      remoteRequests: TABLE_LIVE_V4_REMOTE_REQUESTS,
      hostPhases: TABLE_LIVE_V4_HOST_PHASES,
      captures: TABLE_LIVE_V4_CAPTURE_COUNT,
      sourceRoots: TABLE_LIVE_V4_SOURCE_ROOTS,
...
export function validateTableLiveV4AuthorizationArtifact(
  artifact: TableLiveV4AuthorizationArtifact | undefined,
  index: TableLiveV4AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v4-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v4" ||
    artifact.authorizationIntent !== true
  )
    failures.push("table v4 authorization declaration missing or malformed");
  const resultLeaks = forbiddenAntecedentFieldPaths(artifact);
  if (resultLeaks.length)
    failures.push(`table v4 authorization result leakage: ${resultLeaks.join(",")}`);
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V4_INDEX_PATH ||
    artifact?.antecedent?.indexSha256 !== indexSha256 ||
    artifact?.antecedent?.hashSetSha256 !== index?.hashSetSha256
  )
    failures.push("table v4 authorization pins wrong antecedent index");
  if (privateMaterialPaths(artifact).length)
    failures.push("table v4 authorization contains private key or token material");
  try {
    ...
    if (
      ...
      failures.push("table v4 Ed25519 public-key identity mismatch");
  } catch {
    failures.push("table v4 Ed25519 public key is invalid");
  }
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V4_TARGET) ||
    !equalJson(
    ...
    failures.push("wrong table v4 target or dynamic tool");
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V4_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V4_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V4_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V4_SOURCE_ROOTS ||
    artifact?.denominator?.expectedFacts !== index?.counts.expectedSceneFacts
  )
    failures.push("table v4 two-root or capture denominator changed");
  if (
    ...
    failures.push(
      "table v4 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
    ...
    failures.push("table v4 security prerequisites weakened");
  if (
    artifact?.humanSignoff?.mandatory !== true ||
    artifact?.humanSignoff?.status !== "pending"
  )
    failures.push("table v4 human signoff overclaim");
  return failures;
}

export function validateTableLiveV4History(
  state: TableLiveV4HistoryState,
  expected: TableLiveV4HistoryExpectation,
): string[] {
  const failures = validateTableLiveV4AntecedentIndex(state.index);
  if (!SHA40.test(state.codeCommit))
    failures.push("table v4 code commit is not a full SHA");
  ...
  if (
    ...
    failures.push("table v4 antecedent first-add commit is not unique");
  if (!state.antecedentIsAncestorOfCode)
    failures.push("code commit does not descend from table v4 antecedent");
  if (!state.antecedentIndexBytesMatchFirstAddition)
    failures.push("table v4 antecedent index changed after first addition");
  if (!state.antecedentArtifactsMatchIndexAtCommit)
    failures.push("table v4 antecedent commit artifacts do not match index");
  if (!state.workingAntecedentArtifactsMatchIndex)
    failures.push("table v4 antecedent working bytes drifted");
  ...
  failures.push(
    ...validateTableLiveV4AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
  ...
  if (
    ...
    failures.push("table v4 authorization first-add lineage mismatch");
  if (!state.authorizationPresentAtCodeCommit)
    failures.push("table v4 authorization missing from code commit");
  if (!state.authorizationBytesMatchFirstAddition)
    failures.push("table v4 authorization bytes changed after first addition");
  if (!state.authorizationStrictlyDescendsFromAntecedent)
    failures.push("table v4 authorization does not strictly descend from antecedent");
  if (!state.authorizationIsAncestorOfCode)
    failures.push("code commit predates table v4 authorization");
  return failures;
}
...
export function readTableLiveV4HistoryState(
  root = process.cwd(),
): TableLiveV4HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V4_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV4AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V4_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V4_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V4_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV4History(
  expected: TableLiveV4HistoryExpectation,
  root = process.cwd(),
): TableLiveV4HistoryState {
  const state = readTableLiveV4HistoryState(root);
  const failures = validateTableLiveV4History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v4 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV4Authorization(
  root = process.cwd(),
): TableLiveV4AuthorizationProof {
  const state = verifyTableLiveV4History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V4_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v4/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v4.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V4_TARGET,
    expectedDynamicTool: TABLE_LIVE_V4_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V4_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV4HistoryExpectation => {
  ...
  if (pending === authorized)
    throw new Error(
      "Choose exactly one table v4 history mode: --expect-pending or --expect-authorized",
    );
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV4History(expected);
  process.stdout.write(
    `Table live v4 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
