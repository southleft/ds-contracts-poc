...
import {
  verifyCalendarLiveV36FixedSceneFixedPoint,
  type CalendarLiveV36FixedFixedPoint,
} from "./calendar-live-v36-fixed-point.js";

export const CALENDAR_LIVE_V36_VERIFIER_VERSION = "calendar-live-v36-verifier-v1";

export function buildCalendarLiveV36RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV36RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV36RawNode[];
}

export interface CalendarLiveV36NormalizedScene {
...
export interface CalendarLiveV36SceneProof {
...
export function normalizeCalendarLiveV36Scene(
  root: CalendarLiveV36RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV36NormalizedScene {
  const canonicalBindings: CalendarLiveV36NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV36RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV36Scene<Instance>(
  normalized: CalendarLiveV36NormalizedScene,
...
): CalendarLiveV36SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV36FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV36ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV36PreCaptureGates(payload: unknown): void {
