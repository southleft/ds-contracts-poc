...
import { verifyComboboxLiveV17Authorization } from "./combobox-live-v17-authorization.js";
import { COMBOBOX_LIVE_V17_TARGET } from "./combobox-live-v17-broker.js";
import {
  COMBOBOX_LIVE_V17_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV17Authorization();
...
const attestation: ComboboxLiveV17SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV17SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v17 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V17_SECURITY_ATTESTATION_DEFAULT_PATH,
);
