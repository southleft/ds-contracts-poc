...
import {
  validateComboboxLiveV17AntecedentIndex,
  verifyComboboxLiveV17Authorization,
  type ComboboxLiveV17AntecedentIndex,
  type ComboboxLiveV17AuthorizationProof,
} from "./combobox-live-v17-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V17_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v17/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V17_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v17-security-attestation.json";

export interface ComboboxLiveV17SecurityAttestation {
  artifactVersion: "combobox-live-v17-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV17PreflightReport {
  artifactVersion: "combobox-live-v17-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV17SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV17SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV17AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v17-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V17_TARGET) ||
...
export function validateComboboxLiveV17ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV17Authorization");
  const preflight = source.indexOf("runComboboxLiveV17Preflight");
  const initialize = source.indexOf("ComboboxLiveV17Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV17Preflight(
  root: string,
  proof: ComboboxLiveV17AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V17_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV17PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V17_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V17_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V17_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV17AntecedentIndex;
  failures.push(...validateComboboxLiveV17AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V17_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V17_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV17SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v17-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV17ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v17 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV17SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v17-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V17_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV17Preflight(
    process.cwd(),
    verifyComboboxLiveV17Authorization(),
    attestationPath,
