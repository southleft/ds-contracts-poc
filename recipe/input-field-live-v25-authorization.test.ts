...
import { INPUT_LIVE_V25_INDEX_PATH } from "./build-input-field-live-proof-v25.js";
import {
  INPUT_LIVE_V25_DYNAMIC_TOOL,
  INPUT_LIVE_V25_TARGET,
} from "./input-field-live-v25-broker.js";
import {
  buildInputLiveV25AuthorizationArtifact,
  validateInputLiveV25AntecedentIndex,
  validateInputLiveV25AuthorizationArtifact,
  validateInputLiveV25History,
  validateInputLiveV25UnitTestSource,
  type InputLiveV25AntecedentIndex,
  type InputLiveV25AuthorizationArtifact,
  type InputLiveV25AuthorizationProof,
  type InputLiveV25HistoryState,
} from "./input-field-live-v25-authorization.js";
import {
  inputLiveV25AttestationSha256,
  validateInputLiveV25ControlFlowSource,
  validateInputLiveV25SecurityAttestation,
  type InputLiveV25SecurityAttestation,
} from "./input-field-live-v25-preflight.js";
import { inputLiveV25Sha256 } from "./input-field-live-v25-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V25_INDEX_PATH);
const index = (): InputLiveV25AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV25AntecedentIndex;

const artifact = (): InputLiveV25AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV25AuthorizationArtifact({
...
const pendingState = (): InputLiveV25HistoryState => ({
  index: index(),
  indexSha256: inputLiveV25Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV25HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV25AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV25Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V25_TARGET,
  expectedDynamicTool: INPUT_LIVE_V25_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v25/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV25AuthorizationArtifact,
): InputLiveV25SecurityAttestation => {
  const body: Omit<InputLiveV25SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v25-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V25_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV25AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV25History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV25History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV25History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV25History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV25History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV25UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV25UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV25AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV25Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV25AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV25AuthorizationArtifact(
        planted as InputLiveV25AuthorizationArtifact,
        index(),
        inputLiveV25Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV25SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV25AttestationSha256(
      body as Omit<InputLiveV25SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV25SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV25ControlFlowSource(source), []);
});
