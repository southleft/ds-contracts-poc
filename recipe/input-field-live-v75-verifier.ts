...
export const INPUT_LIVE_V75_VERIFIER_VERSION = "input-live-v75-verifier-v1";

export function buildInputLiveV75RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV75RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV75RawNode[];
}

export interface InputLiveV75NormalizedScene {
...
export interface InputLiveV75SceneProof {
...
export function normalizeInputLiveV75Scene(
  root: InputLiveV75RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV75NormalizedScene {
  const canonicalBindings: InputLiveV75NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV75RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV75Scene<Instance>(
  normalized: InputLiveV75NormalizedScene,
...
export function compileInputLiveV75ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV75PreCaptureGates(payload: unknown): void {
