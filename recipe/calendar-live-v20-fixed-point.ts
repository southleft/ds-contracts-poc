/**
 * Calendar live v20 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V20_FIXED_VERIFIER_VERSION = "calendar-live-v20-fixed-point-v1";
export const CALENDAR_LIVE_V20_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV20FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV20FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV20FixedSourceProbe {
...
export interface CalendarLiveV20FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV20FixedFixedPoint;
}

export interface CalendarLiveV20FixedHardGateInput {
  thresholds: CalendarLiveV20FixedThresholds;
  sourceProbes: CalendarLiveV20FixedSourceProbe[];
  cells: CalendarLiveV20FixedCellValidation[];
  sceneProofs: CalendarLiveV20FixedSceneProof[];
...
export interface CalendarLiveV20FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V20_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV20FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV20FixedHardGates(
  input: CalendarLiveV20FixedHardGateInput,
): CalendarLiveV20FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV20FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V20_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV20FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV20FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV20FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
