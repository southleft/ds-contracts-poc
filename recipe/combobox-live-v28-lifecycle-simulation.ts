...
export interface ComboboxLiveV28LifecycleSimulationReport {
  artifactVersion: "combobox-live-v28-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV28AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV28Lifecycle(): Promise<ComboboxLiveV28LifecycleSimulationReport> {
  await buildComboboxLiveV28Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V28_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV28AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v28-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V28_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V28_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV28History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV28History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV28AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V28_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V28_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV28History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV28History(authorized, "pending");
    ...
    await buildComboboxLiveV28Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V28_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV28AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV28Lifecycle(), null, 2)}\n`,
  );
