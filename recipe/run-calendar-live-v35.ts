...
import {
  acceptCalendarLiveV35Response,
  createCalendarLiveV35Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV35Request,
  persistCalendarLiveV35RawResponse,
  persistCalendarLiveV35TechnicalGates,
  readCalendarLiveV35AcceptedRawPayload,
  type CalendarLiveV35Request,
  type CalendarLiveV35TransactionAuthorization,
} from "./calendar-live-v35-broker.js";
import {
  CALENDAR_LIVE_V35_CAPTURE_COUNT,
  assertCalendarLiveV35CaptureResponses,
  ...
  evaluateCalendarLiveV35Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV35Roots,
  validateCalendarLiveV35CaptureManifest,
  validateCalendarLiveV35CapturePayload,
  validateCalendarLiveV35CleanupPayload,
  validateCalendarLiveV35ExtractPayload,
  validateCalendarLiveV35ProbePayload,
  validateCalendarLiveV35WriterPayload,
  type CalendarLiveV35CaptureCell,
  type CalendarLiveV35CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V35_PLAN_PATH,
  type CalendarLiveV35ProofPlan,
} from "./build-calendar-live-proof-v35.js";
import {
...
export const CALENDAR_LIVE_V35_RUNNER_VERSION = "calendar-live-v35-runner-v1";

interface CalendarLiveV35JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV35OrchestratorState {
  artifactVersion: "calendar-live-v35-orchestrator-state-v1";
  ...
  journal: CalendarLiveV35JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV35ProofPlan;
  planSha256: string;
  captures: CalendarLiveV35CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV35SourceIdentity & {
...
export interface CalendarLiveV35NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV35OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v35 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V35_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V35_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV35ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV35CaptureCell[];
  validateCalendarLiveV35CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v35 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v35 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV35WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV35RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV35Request =>
  readJson<CalendarLiveV35Request>(file);

export class CalendarLiveV35Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV35Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV35OrchestratorState = {
      artifactVersion: "calendar-live-v35-orchestrator-state-v1",
    ...
    return new CalendarLiveV35Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV35OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV35Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV35NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV35NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v35 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV35NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v35 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV35RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV35Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV35CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV35AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV35WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV35Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV35ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV35Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV35Response(this.#directory, "writer", {
        validate: validateCalendarLiveV35WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV35Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV35RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV35Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV35ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV35Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV35RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v35-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV35Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV35ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV35Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV35ProbePayload(payload, ownership),
      });
      persistCalendarLiveV35TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v35 capture cell missing");
      const accepted = acceptCalendarLiveV35Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV35CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V35_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV35CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV35Objective(
    ...
    throw new Error(`Calendar live v35 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV35Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV35CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV35Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV35RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV35Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV35CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV35CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV35CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV35Authorization = (
  privateKey: KeyObject,
): CalendarLiveV35TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV35SimulationKey = (): KeyObject =>
