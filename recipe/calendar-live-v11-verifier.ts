...
import {
  verifyCalendarLiveV11FixedSceneFixedPoint,
  type CalendarLiveV11FixedFixedPoint,
} from "./calendar-live-v11-fixed-point.js";

export const CALENDAR_LIVE_V11_VERIFIER_VERSION = "calendar-live-v11-verifier-v1";

export function buildCalendarLiveV11RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV11RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV11RawNode[];
}

export interface CalendarLiveV11NormalizedScene {
...
export interface CalendarLiveV11SceneProof {
...
export function normalizeCalendarLiveV11Scene(
  root: CalendarLiveV11RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV11NormalizedScene {
  const canonicalBindings: CalendarLiveV11NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV11RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV11Scene<Instance>(
  normalized: CalendarLiveV11NormalizedScene,
...
): CalendarLiveV11SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV11FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV11ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV11PreCaptureGates(payload: unknown): void {
