...
export type CalendarLiveV16HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV16AuthorizationArtifact {
  artifactVersion: "calendar-live-v16-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V16_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV16AntecedentIndex {
...
export interface CalendarLiveV16HistoryState {
  index?: CalendarLiveV16AntecedentIndex;
  indexSha256: string;
...
export interface CalendarLiveV16AuthorizationProof
  extends CalendarLiveV16TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V16_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V16_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V16_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V16_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v16-authorization") ||
  artifactPath.includes("calendar-live-v16-preflight") ||
  artifactPath.includes("calendar-live-v16-authorized") ||
  artifactPath.includes("create-calendar-live-v16-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v16-status") ||
...
export function validateCalendarLiveV16AntecedentIndex(
  index: CalendarLiveV16AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v16-antecedent-index-v1" ||
...
export function validateCalendarLiveV16UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV16History\(\s*["']/.test(source) ||
    /readCalendarLiveV16HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV16AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV16AuthorizationArtifact {
  ...
  ) as CalendarLiveV16AntecedentIndex;
  const indexFailures = validateCalendarLiveV16AntecedentIndex(index);
...
export function validateCalendarLiveV16AuthorizationArtifact(
  artifact: CalendarLiveV16AuthorizationArtifact | undefined,
  index: CalendarLiveV16AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v16-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v16" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V16_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V16_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V16_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V16_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V16_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V16_SOURCE_ROOTS ||
...
export function validateCalendarLiveV16History(
  state: CalendarLiveV16HistoryState,
  expected: CalendarLiveV16HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV16AntecedentIndex(state.index);
...
export function readCalendarLiveV16HistoryState(
  root = process.cwd(),
): CalendarLiveV16HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V16_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV16AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V16_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V16_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V16_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV16History(
  expected: CalendarLiveV16HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV16HistoryState {
  const state = readCalendarLiveV16HistoryState(root);
  const failures = validateCalendarLiveV16History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v16 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV16Authorization(
  root = process.cwd(),
): CalendarLiveV16AuthorizationProof {
  const state = verifyCalendarLiveV16History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V16_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v16/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v16.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V16_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V16_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V16_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV16HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV16History(expected);
  process.stdout.write(
    `Calendar live v16 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
