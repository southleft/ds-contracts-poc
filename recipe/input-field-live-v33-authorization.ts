...
import {
  ...
  INPUT_LIVE_V33_INDEX_PATH,
} from "./build-input-field-live-proof-v33.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV33HistoryExpectation = "pending" | "authorized";

export interface InputLiveV33AuthorizationArtifact {
  artifactVersion: "input-live-v33-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V33_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV33AntecedentIndex {
...
export interface InputLiveV33HistoryState {
  index?: InputLiveV33AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV33AuthorizationArtifact;
...
export interface InputLiveV33AuthorizationProof extends InputLiveV33TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V33_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V33_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V33_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V33_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v33-status") ||
...
export function validateInputLiveV33AntecedentIndex(
  index: InputLiveV33AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v33-antecedent-index-v1" ||
...
export function validateInputLiveV33UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV33History\(\s*["']/.test(source) ||
    /readInputLiveV33HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV33AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV33AuthorizationArtifact {
  ...
  ) as InputLiveV33AntecedentIndex;
  const indexFailures = validateInputLiveV33AntecedentIndex(index);
...
export function validateInputLiveV33AuthorizationArtifact(
  artifact: InputLiveV33AuthorizationArtifact | undefined,
  index: InputLiveV33AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v33-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v33" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V33_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V33_TARGET) ||
    !equalJson(
...
export function validateInputLiveV33History(
  state: InputLiveV33HistoryState,
  expected: InputLiveV33HistoryExpectation,
): string[] {
  const failures = validateInputLiveV33AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV33AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV33HistoryState(
  root = process.cwd(),
): InputLiveV33HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V33_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV33AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V33_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V33_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V33_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV33History(
  expected: InputLiveV33HistoryExpectation,
  root = process.cwd(),
): InputLiveV33HistoryState {
  const state = readInputLiveV33HistoryState(root);
  const failures = validateInputLiveV33History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV33Authorization(
  root = process.cwd(),
): InputLiveV33AuthorizationProof {
  const state = verifyInputLiveV33History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V33_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v33/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v33.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V33_TARGET,
    expectedDynamicTool: INPUT_LIVE_V33_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V33_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV33HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV33History(expected);
