...
import {
  ...
  verifyInputLiveV16Authorization,
  type InputLiveV16AntecedentIndex,
  type InputLiveV16AuthorizationProof,
} from "./input-field-live-v16-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V16_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v16/operator-security-attestation-template.json";
export const INPUT_LIVE_V16_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v16-security-attestation.json";

export interface InputLiveV16SecurityAttestation {
...
export interface InputLiveV16PreflightReport {
  artifactVersion: "input-live-v16-preflight-v1";
...
export const inputLiveV16AttestationSha256 = (
  value: Omit<InputLiveV16SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV16SecurityAttestation(
  value: unknown,
  proof: InputLiveV16AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v16-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V16_TARGET) ||
...
export function validateInputLiveV16ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV16Authorization");
  const preflight = source.indexOf("runInputLiveV16Preflight");
  const initialize = source.indexOf("InputLiveV16Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV16Preflight(
  root: string,
  proof: InputLiveV16AuthorizationProof,
  attestationPath = INPUT_LIVE_V16_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV16PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V16_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V16_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V16_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV16AntecedentIndex;
  failures.push(...validateInputLiveV16AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V16_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V16_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v16-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV16ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV16SecurityAttestation;
  return {
    artifactVersion: "input-live-v16-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V16_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV16Preflight(
    process.cwd(),
    verifyInputLiveV16Authorization(),
    attestationPath,
