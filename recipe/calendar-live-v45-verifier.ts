...
import {
  verifyCalendarLiveV45FixedSceneFixedPoint,
  type CalendarLiveV45FixedFixedPoint,
} from "./calendar-live-v45-fixed-point.js";

export const CALENDAR_LIVE_V45_VERIFIER_VERSION = "calendar-live-v45-verifier-v1";

export function buildCalendarLiveV45RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV45RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV45RawNode[];
}

export interface CalendarLiveV45NormalizedScene {
...
export interface CalendarLiveV45SceneProof {
...
export function normalizeCalendarLiveV45Scene(
  root: CalendarLiveV45RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV45NormalizedScene {
  const canonicalBindings: CalendarLiveV45NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV45RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV45Scene<Instance>(
  normalized: CalendarLiveV45NormalizedScene,
...
): CalendarLiveV45SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV45FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV45ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV45PreCaptureGates(payload: unknown): void {
