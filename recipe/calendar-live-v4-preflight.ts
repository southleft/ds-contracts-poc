...
import {
  validateCalendarLiveV4AntecedentIndex,
  verifyCalendarLiveV4Authorization,
  type CalendarLiveV4AntecedentIndex,
  type CalendarLiveV4AuthorizationProof,
} from "./calendar-live-v4-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V4_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v4/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V4_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v4-security-attestation.json";

export interface CalendarLiveV4SecurityAttestation {
  artifactVersion: "calendar-live-v4-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV4PreflightReport {
  artifactVersion: "calendar-live-v4-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV4SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV4SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV4AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v4-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V4_TARGET) ||
...
export function validateCalendarLiveV4ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV4Authorization");
  const preflight = source.indexOf("runCalendarLiveV4Preflight");
  const initialize = source.indexOf("CalendarLiveV4Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV4Preflight(
  root: string,
  proof: CalendarLiveV4AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V4_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV4PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V4_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V4_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV4AntecedentIndex;
  failures.push(...validateCalendarLiveV4AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V4_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V4_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV4SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v4-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV4ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v4 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV4SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v4-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V4_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV4Preflight(
    process.cwd(),
    verifyCalendarLiveV4Authorization(),
    attestationPath,
