...
import { chakraLinkAdapterConfig as chakraLinkProposedConfig, chakraLinkSource as chakraLinkProposedSource } from "./fixtures/generated/link.chakra.js";
...
const EVIDENCE = "recipe/evidence/link-live-pivot-v8";
...
const sources = [
  ...
  {
    // PROPOSED from a capture made today: Chakra Link, captured today (contract path quarantined; capture record intact).
    adapterIdentity: "chakra-link-proposed-v1",
    displayName: "Chakra (proposed)",
    source: chakraLinkProposedSource,
    config: chakraLinkProposedConfig,
  },
].map((entry) => {
...
const receiptOwned = {
      artifactVersion: "link-live-pivot-v8-prepare",
      teaching:
...
if (process.argv.includes("--check")) {
  ...
  if (writer.sourcePlans.length !== 6)
    throw new Error("link prepare requires 6 library sources (reviewed + proposed)");
  console.log(
    JSON.stringify({
      check: "ok",
      runIdentity: writer.runIdentity,
