...
import { INPUT_LIVE_V19_INDEX_PATH } from "./build-input-field-live-proof-v19.js";
import {
  INPUT_LIVE_V19_DYNAMIC_TOOL,
  INPUT_LIVE_V19_TARGET,
} from "./input-field-live-v19-broker.js";
import {
  buildInputLiveV19AuthorizationArtifact,
  validateInputLiveV19AntecedentIndex,
  validateInputLiveV19AuthorizationArtifact,
  validateInputLiveV19History,
  validateInputLiveV19UnitTestSource,
  type InputLiveV19AntecedentIndex,
  type InputLiveV19AuthorizationArtifact,
  type InputLiveV19AuthorizationProof,
  type InputLiveV19HistoryState,
} from "./input-field-live-v19-authorization.js";
import {
  inputLiveV19AttestationSha256,
  validateInputLiveV19ControlFlowSource,
  validateInputLiveV19SecurityAttestation,
  type InputLiveV19SecurityAttestation,
} from "./input-field-live-v19-preflight.js";
import { inputLiveV19Sha256 } from "./input-field-live-v19-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V19_INDEX_PATH);
const index = (): InputLiveV19AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV19AntecedentIndex;

const artifact = (): InputLiveV19AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV19AuthorizationArtifact({
...
const pendingState = (): InputLiveV19HistoryState => ({
  index: index(),
  indexSha256: inputLiveV19Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV19HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV19AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV19Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V19_TARGET,
  expectedDynamicTool: INPUT_LIVE_V19_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v19/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV19AuthorizationArtifact,
): InputLiveV19SecurityAttestation => {
  const body: Omit<InputLiveV19SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v19-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V19_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV19AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV19History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV19History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV19History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV19History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV19History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV19UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV19UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV19AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV19Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV19AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV19AuthorizationArtifact(
        planted as InputLiveV19AuthorizationArtifact,
        index(),
        inputLiveV19Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV19SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV19AttestationSha256(
      body as Omit<InputLiveV19SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV19SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV19ControlFlowSource(source), []);
});
