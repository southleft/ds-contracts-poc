...
import {
  acceptCalendarLiveV24Response,
  createCalendarLiveV24Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV24Request,
  persistCalendarLiveV24RawResponse,
  persistCalendarLiveV24TechnicalGates,
  readCalendarLiveV24AcceptedRawPayload,
  type CalendarLiveV24Request,
  type CalendarLiveV24TransactionAuthorization,
} from "./calendar-live-v24-broker.js";
import {
  CALENDAR_LIVE_V24_CAPTURE_COUNT,
  assertCalendarLiveV24CaptureResponses,
  ...
  evaluateCalendarLiveV24Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV24Roots,
  validateCalendarLiveV24CaptureManifest,
  validateCalendarLiveV24CapturePayload,
  validateCalendarLiveV24CleanupPayload,
  validateCalendarLiveV24ExtractPayload,
  validateCalendarLiveV24ProbePayload,
  validateCalendarLiveV24WriterPayload,
  type CalendarLiveV24CaptureCell,
  type CalendarLiveV24CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V24_PLAN_PATH,
  type CalendarLiveV24ProofPlan,
} from "./build-calendar-live-proof-v24.js";
import {
...
export const CALENDAR_LIVE_V24_RUNNER_VERSION = "calendar-live-v24-runner-v1";

interface CalendarLiveV24JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV24OrchestratorState {
  artifactVersion: "calendar-live-v24-orchestrator-state-v1";
  ...
  journal: CalendarLiveV24JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV24ProofPlan;
  planSha256: string;
  captures: CalendarLiveV24CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV24SourceIdentity & {
...
export interface CalendarLiveV24NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV24OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v24 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V24_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V24_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV24ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV24CaptureCell[];
  validateCalendarLiveV24CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v24 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v24 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV24WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV24RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV24Request =>
  readJson<CalendarLiveV24Request>(file);

export class CalendarLiveV24Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV24Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV24OrchestratorState = {
      artifactVersion: "calendar-live-v24-orchestrator-state-v1",
    ...
    return new CalendarLiveV24Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV24OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV24Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV24NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV24NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v24 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV24NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v24 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV24RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV24Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV24CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV24AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV24WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV24Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV24ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV24Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV24Response(this.#directory, "writer", {
        validate: validateCalendarLiveV24WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV24Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV24RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV24Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV24ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV24Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV24RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v24-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV24Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV24ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV24Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV24ProbePayload(payload, ownership),
      });
      persistCalendarLiveV24TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v24 capture cell missing");
      const accepted = acceptCalendarLiveV24Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV24CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V24_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV24CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV24Objective(
    ...
    throw new Error(`Calendar live v24 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV24Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV24CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV24Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV24RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV24Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV24CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV24CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV24CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV24Authorization = (
  privateKey: KeyObject,
): CalendarLiveV24TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV24SimulationKey = (): KeyObject =>
