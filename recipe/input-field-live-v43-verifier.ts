...
export const INPUT_LIVE_V43_VERIFIER_VERSION = "input-live-v43-verifier-v1";

export function buildInputLiveV43RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV43RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV43RawNode[];
}

export interface InputLiveV43NormalizedScene {
...
export interface InputLiveV43SceneProof {
...
export function normalizeInputLiveV43Scene(
  root: InputLiveV43RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV43NormalizedScene {
  const canonicalBindings: InputLiveV43NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV43RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV43Scene<Instance>(
  normalized: InputLiveV43NormalizedScene,
...
export function compileInputLiveV43ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV43PreCaptureGates(payload: unknown): void {
