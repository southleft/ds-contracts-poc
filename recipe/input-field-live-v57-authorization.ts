...
import {
  ...
  INPUT_LIVE_V57_INDEX_PATH,
} from "./build-input-field-live-proof-v57.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV57HistoryExpectation = "pending" | "authorized";

export interface InputLiveV57AuthorizationArtifact {
  artifactVersion: "input-live-v57-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V57_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV57AntecedentIndex {
...
export interface InputLiveV57HistoryState {
  index?: InputLiveV57AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV57AuthorizationArtifact;
...
export interface InputLiveV57AuthorizationProof extends InputLiveV57TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V57_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V57_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V57_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V57_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v57-authorization") ||
  artifactPath.includes("input-field-live-v57-preflight") ||
  artifactPath.includes("input-field-live-v57-authorized") ||
  artifactPath.includes("input-field-live-pivot-v57-status") ||
...
export function validateInputLiveV57AntecedentIndex(
  index: InputLiveV57AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v57-antecedent-index-v1" ||
...
export function validateInputLiveV57UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV57History\(\s*["']/.test(source) ||
    /readInputLiveV57HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV57AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV57AuthorizationArtifact {
  ...
  ) as InputLiveV57AntecedentIndex;
  const indexFailures = validateInputLiveV57AntecedentIndex(index);
...
export function validateInputLiveV57AuthorizationArtifact(
  artifact: InputLiveV57AuthorizationArtifact | undefined,
  index: InputLiveV57AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v57-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v57" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V57_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V57_TARGET) ||
    !equalJson(
...
export function validateInputLiveV57History(
  state: InputLiveV57HistoryState,
  expected: InputLiveV57HistoryExpectation,
): string[] {
  const failures = validateInputLiveV57AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV57AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV57HistoryState(
  root = process.cwd(),
): InputLiveV57HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V57_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV57AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V57_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V57_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V57_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV57History(
  expected: InputLiveV57HistoryExpectation,
  root = process.cwd(),
): InputLiveV57HistoryState {
  const state = readInputLiveV57HistoryState(root);
  const failures = validateInputLiveV57History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV57Authorization(
  root = process.cwd(),
): InputLiveV57AuthorizationProof {
  const state = verifyInputLiveV57History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V57_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v57/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v57.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V57_TARGET,
    expectedDynamicTool: INPUT_LIVE_V57_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V57_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV57HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV57History(expected);
