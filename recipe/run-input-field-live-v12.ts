...
import {
  acceptInputLiveV12Response,
  createInputLiveV12Transaction,
  inputLiveV12ReceiptPath,
  inputLiveV12RequestPath,
  inputLiveV12ResponsePath,
  inputLiveV12Sha256,
  issueInputLiveV12Request,
  persistInputLiveV12RawResponse,
  persistInputLiveV12TechnicalGates,
  readInputLiveV12AcceptedRawPayload,
  type InputLiveV12Request,
  type InputLiveV12TransactionAuthorization,
} from "./input-field-live-v12-broker.js";
import {
  INPUT_LIVE_V12_CAPTURE_COUNT,
  assertInputLiveV12CaptureResponses,
  assertInputLiveV12RootProofs,
  buildInputLiveV12CaptureProgram,
  buildInputLiveV12CleanupProgram,
  buildInputLiveV12ExtractProgram,
  buildInputLiveV12ProbeProgram,
  evaluateInputLiveV12Objective,
  inputLiveV12CaptureManifestSha256,
  proveInputLiveV12Roots,
  validateInputLiveV12CaptureManifest,
  validateInputLiveV12CapturePayload,
  validateInputLiveV12CleanupPayload,
  validateInputLiveV12ExtractPayload,
  validateInputLiveV12ProbePayload,
  validateInputLiveV12WriterPayload,
  type InputLiveV12CaptureCell,
  type InputLiveV12CapturePayload,
  type InputLiveV12RootProof,
  type InputLiveV12SourceIdentity,
  type InputLiveV12WriterOwnership,
} from "./input-field-live-v12-contract.js";
import {
  INPUT_LIVE_V12_CAPTURE_MANIFEST_PATH,
  INPUT_LIVE_V12_PLAN_PATH,
  type InputLiveV12ProofPlan,
} from "./build-input-field-live-proof-v12.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-v12.js";

export const INPUT_LIVE_V12_RUNNER_VERSION = "input-live-v12-runner-v1";

interface InputLiveV12JournalEntry {
  index: number;
  event: string;
...
interface InputLiveV12OrchestratorState {
  artifactVersion: "input-live-v12-orchestrator-state-v1";
  ...
  journal: InputLiveV12JournalEntry[];
}

interface LoadedProofInputs {
  plan: InputLiveV12ProofPlan;
  planSha256: string;
  captures: InputLiveV12CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    InputLiveV12SourceIdentity & {
...
export interface InputLiveV12NextAction {
...
const writeState = (
  directory: string,
  state: InputLiveV12OrchestratorState,
): void => {
...
const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, INPUT_LIVE_V12_PLAN_PATH);
  const capturePath = path.join(root, INPUT_LIVE_V12_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as InputLiveV12ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as InputLiveV12CaptureCell[];
  validateInputLiveV12CaptureManifest(captures);
  if (
    plan.captureManifest.sha256 !== inputLiveV12Sha256(captureBytes) ||
    captureArtifact.cellsSha256 !== inputLiveV12CaptureManifestSha256(captures)
  )
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (
      inputLiveV12Sha256(expectedBytes) !== source.expectedScenePlan.sha256 ||
      inputLiveV12Sha256(uncompressed) !==
  ...
  return {
    plan,
    planSha256: inputLiveV12Sha256(planBytes),
    captures,
    captureManifestSha256: inputLiveV12Sha256(captureBytes),
    sources,
...
const appendJournal = (
  ...
  const payloadSha256 = inputLiveV12Sha256(canonicalJson(payload));
  const body = {
  ...
  const entry = {
    ...body,
    entrySha256: inputLiveV12Sha256(canonicalJson(body)),
  };
  writeNew(
    path.join(
      directory,
...
const writerOwnership = (directory: string): InputLiveV12WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): InputLiveV12RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): InputLiveV12Request =>
  readJson<InputLiveV12Request>(file);

export class InputLiveV12Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueInputLiveV12Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: InputLiveV12OrchestratorState = {
      artifactVersion: "input-live-v12-orchestrator-state-v1",
      transactionDirectory: options.transactionDirectory,
      activeRequest: inputLiveV12RequestPath(
        options.transactionDirectory,
        "writer",
      ),
      cleanupRequest: null,
    ...
    return new InputLiveV12Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<InputLiveV12OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new InputLiveV12Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): InputLiveV12NextAction {
    const cleanup =
      this.#state.cleanupRequest &&
      !this.#state.cleanupAccepted &&
      !existsSync(inputLiveV12ReceiptPath(this.#directory, "cleanup"))
        ? this.#state.cleanupRequest
        : null;
  ...
  ingestAndAdvance(rawResponse: string): InputLiveV12NextAction {
  ...
  ingestCleanup(rawResponse: string): InputLiveV12NextAction {
    ...
    persistInputLiveV12RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptInputLiveV12Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV12CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      existsSync(inputLiveV12ReceiptPath(this.#directory, "writer")) &&
      !existsSync(path.join(this.#directory, OWNERSHIP_FILE))
    ) {
      const raw = readInputLiveV12AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateInputLiveV12WriterPayload(raw),
      );
    }
    if (
      existsSync(path.join(this.#directory, OWNERSHIP_FILE)) &&
      !existsSync(inputLiveV12RequestPath(this.#directory, "cleanup"))
    )
      this.#issueCleanupAndExtract(false);
    if (existsSync(inputLiveV12ReceiptPath(this.#directory, "cleanup")))
      this.#state.cleanupAccepted = true;
    this.#persist();
  }

  #acceptActive(request: InputLiveV12Request): void {
    if (request.phase === "writer") {
      const accepted = acceptInputLiveV12Response(this.#directory, "writer", {
        validate: validateInputLiveV12WriterPayload,
      });
    ...
    if (request.phase === "extract") {
      const accepted = acceptInputLiveV12Response(this.#directory, "extract", {
        validate: (payload) =>
          validateInputLiveV12ExtractPayload(payload, ownership),
      });
      const proofs = proveInputLiveV12Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseInputFieldRecipe,
        compileInputFieldRecipe,
      );
      assertInputLiveV12RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "input-live-v12-host-root-proofs-v1",
      ...
      appendJournal(
        ...
        request.requestId,
        { proofsSha256: inputLiveV12Sha256(canonicalJson(proofs)) },
      );
      const next = issueInputLiveV12Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildInputLiveV12ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = inputLiveV12RequestPath(
        this.#directory,
        "probe",
      );
      appendJournal(
    ...
    if (request.phase === "probe") {
      const accepted = acceptInputLiveV12Response(this.#directory, "probe", {
        validate: (payload) =>
          validateInputLiveV12ProbePayload(payload, ownership),
      });
      persistInputLiveV12TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      const accepted = acceptInputLiveV12Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateInputLiveV12CapturePayload(payload, cell, rawBytes),
      });
      const png = Buffer.from(accepted.payload.pngBase64, "base64");
      const capturePath = path.join(
        this.#directory,
        "captures",
        `${String(cell.index).padStart(3, "0")}-${inputLiveV12Sha256(cell.cellKey).slice(0, 20)}.png`,
      );
      ...
      if (next < INPUT_LIVE_V12_CAPTURE_COUNT) this.#issueCapture(next);
      else {
          ...
          pngBase64: Buffer.from(
            readFileSync(
              path.join(
                this.#directory,
                "captures",
                `${String(response.index).padStart(3, "0")}-${inputLiveV12Sha256(response.cellKey).slice(0, 20)}.png`,
              ),
            ),
          ).toString("base64"),
        }));
        assertInputLiveV12CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateInputLiveV12Objective(
        ...
        appendJournal(
          ...
          request.requestId,
          {
            reportSha256: inputLiveV12Sha256(canonicalJson(objective)),
  ...
  #issueCleanupAndExtract(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueInputLiveV12Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildInputLiveV12CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = inputLiveV12RequestPath(
      this.#directory,
      "cleanup",
    );
    const extract = issueInputLiveV12Request(
      this.#directory,
      this.#privateKey,
      "extract",
      buildInputLiveV12ExtractProgram(ownership, this.#inputs.sources),
    );
    this.#state.activeRequest = inputLiveV12RequestPath(
      this.#directory,
      "extract",
  ...
  #issueCapture(index: number): void {
    const request = issueInputLiveV12Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildInputLiveV12CaptureProgram(
  ...
  #readCaptureResponseIndex(): InputLiveV12CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: InputLiveV12CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedInputLiveV12Authorization = (
  privateKey: KeyObject,
): InputLiveV12TransactionAuthorization => ({
  mode: "simulated",
  ...
  signingPublicKeySha256: inputLiveV12Sha256(
...
export const generateInputLiveV12SimulationKey = (): KeyObject =>
