...
import { buildComboboxLiveV24CleanupRuntime } from "./combobox-live-v24-cleanup.js";
import {
  type ComboboxLiveV24FixedFixedPoint,
} from "./combobox-live-v24-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV24RestoreProgram,
  validateComboboxLiveV24RestorePayload,
  type ComboboxLiveV24RestorePayload,
} from "./combobox-live-v24-restore.js";
import {
  buildComboboxLiveV24RawPropertyRuntime,
  normalizeComboboxLiveV24Scene,
  type ComboboxLiveV24RawNode,
} from "./combobox-live-v24-verifier.js";
...
} from "./scene-readback-combobox-v24.js";
...
export const COMBOBOX_LIVE_V24_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V24_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V24_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V24_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V24_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V24_SET_COUNT = 4;
export const COMBOBOX_LIVE_V24_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V24_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V24_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V24_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V24_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV24SourceId = (typeof COMBOBOX_LIVE_V24_SOURCE_IDS)[number];

export interface ComboboxLiveV24SourceIdentity {
  source: ComboboxLiveV24SourceId;
...
export interface ComboboxLiveV24WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV24ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV24SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV24RawNode;
    optionScene: ComboboxLiveV24RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV24RootProof {
  source: ComboboxLiveV24SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV24FixedFixedPoint;
}

export interface ComboboxLiveV24ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV24SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV24CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV24SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV24CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV24SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV24CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v24 ${label} must be an object`);
...
export function validateComboboxLiveV24WriterPayload(
  value: unknown,
): ComboboxLiveV24WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V24_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v24 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v24 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v24 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV24SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV24ExtractProgram(
  writer: ComboboxLiveV24WriterOwnership,
  sources: readonly ComboboxLiveV24SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v24 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V24_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V24_NAMESPACE)}
${buildComboboxLiveV24RawPropertyRuntime()}
...
export function validateComboboxLiveV24ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV24WriterOwnership,
): ComboboxLiveV24ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v24 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V24_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v24 extract schema/two-root mismatch");
  return payload as ComboboxLiveV24ExtractPayload;
}

export function proveComboboxLiveV24Roots<Instance>(
  extract: ComboboxLiveV24ExtractPayload,
  sources: readonly (ComboboxLiveV24SourceIdentity & {
...
): ComboboxLiveV24RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v24 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v24 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV24Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV24Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV24FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v24 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v24 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v24 compile lost owned sets");
...
export function assertComboboxLiveV24RootProofs(
  proofs: readonly ComboboxLiveV24RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v24 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v24 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV24ProbeProgram(
  writer: ComboboxLiveV24WriterOwnership,
  sources: readonly ComboboxLiveV24SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V24_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV24ProbePayload(
  value: unknown,
  writer: ComboboxLiveV24WriterOwnership,
): ComboboxLiveV24ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV24ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V24_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V24_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v24 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV24CaptureManifest(
  cells: readonly ComboboxLiveV24CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V24_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V24_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V24_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V24_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v24 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV24CaptureCell[],
): string => {
  validateComboboxLiveV24CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV24CaptureProgram(
  writer: ComboboxLiveV24WriterOwnership,
  cell: ComboboxLiveV24CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V24_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v24 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v24 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V24_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV24CapturePayload(
  value: unknown,
  cell: ComboboxLiveV24CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV24CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV24CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V24_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V24_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v24 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV24CaptureResponses(
  manifest: readonly ComboboxLiveV24CaptureCell[],
  responses: readonly ComboboxLiveV24CapturePayload[],
): void {
  validateComboboxLiveV24CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v24 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV24ObjectiveReport {
  artifactVersion: "combobox-live-v24-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV24Objective(
  manifest: readonly ComboboxLiveV24CaptureCell[],
  responses: readonly ComboboxLiveV24CapturePayload[],
): ComboboxLiveV24ObjectiveReport {
  assertComboboxLiveV24CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v24-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV24CleanupProgram(
  writer: ComboboxLiveV24WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV24CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V24_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v24 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV24CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV24WriterOwnership,
): ComboboxLiveV24CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV24CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v24 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V24_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV24WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
