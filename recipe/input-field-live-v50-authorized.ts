...
import { INPUT_LIVE_V50_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v50.js";
import { verifyInputLiveV50Authorization } from "./input-field-live-v50-authorization.js";
import { runInputLiveV50Preflight } from "./input-field-live-v50-preflight.js";
import { InputLiveV50Orchestrator } from "./run-input-field-live-v50.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V50_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v50-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV50Authorization(root);
    runInputLiveV50Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV50Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
