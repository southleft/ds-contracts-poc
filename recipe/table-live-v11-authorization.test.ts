...
import { TABLE_LIVE_V11_INDEX_PATH } from "./build-table-live-proof-v11.js";
import {
...
import {
  buildTableLiveV11AuthorizationArtifact,
  validateTableLiveV11AntecedentIndex,
  validateTableLiveV11AuthorizationArtifact,
  validateTableLiveV11History,
  validateTableLiveV11UnitTestSource,
  type TableLiveV11AntecedentIndex,
  type TableLiveV11AuthorizationArtifact,
  type TableLiveV11AuthorizationProof,
  type TableLiveV11HistoryState,
} from "./table-live-v11-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV11ControlFlowSource,
  validateTableLiveV11SecurityAttestation,
  type TableLiveV11SecurityAttestation,
} from "./table-live-v11-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V11_INDEX_PATH);
const index = (): TableLiveV11AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV11AntecedentIndex;

const artifact = (): TableLiveV11AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV11AuthorizationArtifact({
...
const pendingState = (): TableLiveV11HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV11HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV11AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v11/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV11AuthorizationArtifact,
): TableLiveV11SecurityAttestation => {
  const body: Omit<TableLiveV11SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V11_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV11History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV11History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV11History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV11History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV11History(authorizedState(), "pending").join("\n"),
    /stale history mode.*authorization exists/,
  );
});

test("the exact v6 phase-sensitive self-test defect is planted and table v11 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV11UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v11-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV11UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV11AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV11AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV11AuthorizationArtifact(
        planted as TableLiveV11AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV11SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV11SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV11SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v11-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV11ControlFlowSource(source), []);
});
