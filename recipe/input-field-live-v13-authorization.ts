...
import {
  ...
  INPUT_LIVE_V13_INDEX_PATH,
} from "./build-input-field-live-proof-v13.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV13HistoryExpectation = "pending" | "authorized";

export interface InputLiveV13AuthorizationArtifact {
  artifactVersion: "input-live-v13-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V13_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV13AntecedentIndex {
...
export interface InputLiveV13HistoryState {
  index?: InputLiveV13AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV13AuthorizationArtifact;
...
export interface InputLiveV13AuthorizationProof extends InputLiveV13TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V13_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V13_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V13_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V13_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v13-status") ||
...
export function validateInputLiveV13AntecedentIndex(
  index: InputLiveV13AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v13-antecedent-index-v1" ||
...
export function validateInputLiveV13UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV13History\(\s*["']/.test(source) ||
    /readInputLiveV13HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV13AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV13AuthorizationArtifact {
  ...
  ) as InputLiveV13AntecedentIndex;
  const indexFailures = validateInputLiveV13AntecedentIndex(index);
...
export function validateInputLiveV13AuthorizationArtifact(
  artifact: InputLiveV13AuthorizationArtifact | undefined,
  index: InputLiveV13AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v13-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v13" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V13_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V13_TARGET) ||
    !equalJson(
...
export function validateInputLiveV13History(
  state: InputLiveV13HistoryState,
  expected: InputLiveV13HistoryExpectation,
): string[] {
  const failures = validateInputLiveV13AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV13AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV13HistoryState(
  root = process.cwd(),
): InputLiveV13HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V13_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV13AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V13_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V13_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V13_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV13History(
  expected: InputLiveV13HistoryExpectation,
  root = process.cwd(),
): InputLiveV13HistoryState {
  const state = readInputLiveV13HistoryState(root);
  const failures = validateInputLiveV13History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV13Authorization(
  root = process.cwd(),
): InputLiveV13AuthorizationProof {
  const state = verifyInputLiveV13History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V13_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v13/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v13.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V13_TARGET,
    expectedDynamicTool: INPUT_LIVE_V13_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V13_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV13HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV13History(expected);
