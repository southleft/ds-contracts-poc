...
import {
  ...
  verifyTableLiveV23Authorization,
  type TableLiveV23AntecedentIndex,
  type TableLiveV23AuthorizationProof,
} from "./table-live-v23-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V23_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v23/operator-security-attestation-template.json";
export const TABLE_LIVE_V23_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v23-security-attestation.json";

export interface TableLiveV23SecurityAttestation {
...
export interface TableLiveV23PreflightReport {
  artifactVersion: "table-live-v23-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV23SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV23SecurityAttestation(
  value: unknown,
  proof: TableLiveV23AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v23-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V23_TARGET) ||
...
export function validateTableLiveV23ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV23Authorization");
  const preflight = source.indexOf("runTableLiveV23Preflight");
  const initialize = source.indexOf("TableLiveV23Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV23Preflight(
  root: string,
  proof: TableLiveV23AuthorizationProof,
  attestationPath = TABLE_LIVE_V23_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV23PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V23_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V23_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV23AntecedentIndex;
  failures.push(...validateTableLiveV23AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V23_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V23_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v23-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV23ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v23 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV23SecurityAttestation;
  return {
    artifactVersion: "table-live-v23-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V23_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV23Preflight(
    process.cwd(),
    verifyTableLiveV23Authorization(),
    attestationPath,
