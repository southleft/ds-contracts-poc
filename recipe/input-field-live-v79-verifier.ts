...
export const INPUT_LIVE_V79_VERIFIER_VERSION = "input-live-v79-verifier-v1";

export function buildInputLiveV79RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV79RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV79RawNode[];
}

export interface InputLiveV79NormalizedScene {
...
export interface InputLiveV79SceneProof {
...
export function normalizeInputLiveV79Scene(
  root: InputLiveV79RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV79NormalizedScene {
  const canonicalBindings: InputLiveV79NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV79RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV79Scene<Instance>(
  normalized: InputLiveV79NormalizedScene,
...
export function compileInputLiveV79ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV79PreCaptureGates(payload: unknown): void {
