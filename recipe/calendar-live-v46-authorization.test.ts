...
import { CALENDAR_LIVE_V46_INDEX_PATH } from "./build-calendar-live-proof-v46.js";
import {
...
import {
  buildCalendarLiveV46AuthorizationArtifact,
  validateCalendarLiveV46AntecedentIndex,
  validateCalendarLiveV46AuthorizationArtifact,
  validateCalendarLiveV46History,
  validateCalendarLiveV46UnitTestSource,
  type CalendarLiveV46AntecedentIndex,
  type CalendarLiveV46AuthorizationArtifact,
  type CalendarLiveV46AuthorizationProof,
  type CalendarLiveV46HistoryState,
} from "./calendar-live-v46-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV46ControlFlowSource,
  validateCalendarLiveV46SecurityAttestation,
  type CalendarLiveV46SecurityAttestation,
} from "./calendar-live-v46-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V46_INDEX_PATH);
const index = (): CalendarLiveV46AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV46AntecedentIndex;

const artifact = (): CalendarLiveV46AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV46AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV46HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV46HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV46AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v46/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV46AuthorizationArtifact,
): CalendarLiveV46SecurityAttestation => {
  const body: Omit<CalendarLiveV46SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V46_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV46History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV46History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV46History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV46History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV46History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV46UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v46-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV46UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV46AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV46AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV46AuthorizationArtifact(
        planted as CalendarLiveV46AuthorizationArtifact,
        index(),
...
test("one-root denominator, capture gate, cleanup, and prior-lineage reuse are rejected", () => {
  ...
  for (const mutate of [
    ...
    (value: Record<string, any>) => {
      value.execution.comboboxLiveV46AuthorizationReusable = true;
    },
    (value: Record<string, any>) => {
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV46SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV46SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV46SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v46-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV46ControlFlowSource(source), []);
});
