...
import { INPUT_LIVE_V54_INDEX_PATH } from "./build-input-field-live-proof-v54.js";
import {
  INPUT_LIVE_V54_DYNAMIC_TOOL,
  INPUT_LIVE_V54_TARGET,
} from "./input-field-live-v54-broker.js";
import {
  buildInputLiveV54AuthorizationArtifact,
  validateInputLiveV54AntecedentIndex,
  validateInputLiveV54AuthorizationArtifact,
  validateInputLiveV54History,
  validateInputLiveV54UnitTestSource,
  type InputLiveV54AntecedentIndex,
  type InputLiveV54AuthorizationArtifact,
  type InputLiveV54AuthorizationProof,
  type InputLiveV54HistoryState,
} from "./input-field-live-v54-authorization.js";
import {
  inputLiveV54AttestationSha256,
  validateInputLiveV54ControlFlowSource,
  validateInputLiveV54SecurityAttestation,
  type InputLiveV54SecurityAttestation,
} from "./input-field-live-v54-preflight.js";
import { inputLiveV54Sha256 } from "./input-field-live-v54-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V54_INDEX_PATH);
const index = (): InputLiveV54AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV54AntecedentIndex;

const artifact = (): InputLiveV54AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV54AuthorizationArtifact({
...
const pendingState = (): InputLiveV54HistoryState => ({
  index: index(),
  indexSha256: inputLiveV54Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV54HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV54AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV54Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V54_TARGET,
  expectedDynamicTool: INPUT_LIVE_V54_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v54/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV54AuthorizationArtifact,
): InputLiveV54SecurityAttestation => {
  const body: Omit<InputLiveV54SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v54-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V54_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV54AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV54History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV54History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV54History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV54History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV54History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV54UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV54UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV54AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV54Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV54AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV54AuthorizationArtifact(
        planted as InputLiveV54AuthorizationArtifact,
        index(),
        inputLiveV54Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV54SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV54AttestationSha256(
      body as Omit<InputLiveV54SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV54SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV54ControlFlowSource(source), []);
});
