...
import {
  ...
  createInputLiveV26FakeDynamicResponse,
  ...
  listInputLiveV26PersistedRequests,
  persistInputLiveV26RawResponse,
  persistInputLiveV26TechnicalGates,
  validateInputLiveV26Receipt,
  validateInputLiveV26Request,
  type InputLiveV26RemotePhase,
...
import {
  ...
  type InputLiveV26ExtractPayload,
  type InputLiveV26ProbePayload,
...
import {
  ...
  simulatedInputLiveV26Authorization,
} from "./run-input-field-live-v26.js";
...
const captures: InputLiveV26CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV26Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v26-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV26Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV26CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV26FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV26FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V26_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V26_TARGET);
  persistInputLiveV26RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV26Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV26Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV26WriterOwnership,
...
const rootProofs = (): InputLiveV26RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v26-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV26Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV26Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV26RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV26RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV26ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV26ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV26CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV26CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV26CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV26Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV26CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV26PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV26ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV26Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV26Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV26Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV26Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V26_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV26RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V26_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV26Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV26Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV26ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV26Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV26WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV26RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV26Request;
    validateInputLiveV26Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV26Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV26CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v26-state-"),
  );
  ...
  try {
    const orchestrator = InputLiveV26Orchestrator.initialize({
      ...
      privateKey,
      authorization: simulatedInputLiveV26Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV26Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V26_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV26RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V26_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV26Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV26Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV26RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V26_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV26Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV26Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV26Request;
