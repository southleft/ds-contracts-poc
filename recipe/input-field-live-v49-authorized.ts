...
import { INPUT_LIVE_V49_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v49.js";
import { verifyInputLiveV49Authorization } from "./input-field-live-v49-authorization.js";
import { runInputLiveV49Preflight } from "./input-field-live-v49-preflight.js";
import { InputLiveV49Orchestrator } from "./run-input-field-live-v49.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V49_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v49-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV49Authorization(root);
    runInputLiveV49Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV49Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
