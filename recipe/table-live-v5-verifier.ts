...
import {
  verifyTableLiveV5FixedSceneFixedPoint,
  type TableLiveV5FixedFixedPoint,
} from "./table-live-v5-fixed-point.js";

export const TABLE_LIVE_V5_VERIFIER_VERSION = "table-live-v5-verifier-v1";

export function buildTableLiveV5RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV5RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV5RawNode[];
}

export interface TableLiveV5NormalizedScene {
...
export interface TableLiveV5SceneProof {
...
export function normalizeTableLiveV5Scene(
  root: TableLiveV5RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV5NormalizedScene {
  const canonicalBindings: TableLiveV5NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV5RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV5Scene<Instance>(
  normalized: TableLiveV5NormalizedScene,
...
): TableLiveV5SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV5FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV5ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV5PreCaptureGates(payload: unknown): void {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("table live v5 pre-capture gate payload absent");
