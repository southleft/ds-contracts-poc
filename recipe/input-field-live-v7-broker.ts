...
import {
  ...
  INPUT_LIVE_V7_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V7_RESPONSE_CONTRACTS,
  type InputLiveV7RootProof,
} from "./input-field-live-v7-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V7_BROKER_VERSION = "input-live-v7-broker-v2";
export const INPUT_LIVE_V7_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V7_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V7_REMOTE_PHASES = [
  "writer",
  "extract",
  "probe",
  "capture",
...
export const INPUT_LIVE_V7_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV7RemotePhase =
  (typeof INPUT_LIVE_V7_REMOTE_PHASES)[number];
export type InputLiveV7AuthorizationMode = "live" | "simulated";

export interface InputLiveV7CommitPins {
...
export interface InputLiveV7TransactionAuthorization extends InputLiveV7CommitPins {
  mode: InputLiveV7AuthorizationMode;
...
export interface InputLiveV7Manifest {
  artifactVersion: "input-live-v7-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V7_BROKER_VERSION;
...
export interface InputLiveV7ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV7RequestBody {
  artifactVersion: "input-live-v7-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V7_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV7RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV7CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V7_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV7ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV7Request extends InputLiveV7RequestBody {
...
export interface InputLiveV7Receipt {
  artifactVersion: "input-live-v7-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V7_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV7RemotePhase;
...
export interface InputLiveV7AcceptedResponse<T = unknown> {
  request: InputLiveV7Request;
  receipt: InputLiveV7Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV7IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV7ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V7_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V7_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V7_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V7_TARGET.editorType;
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Input live v7 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    throw new TypeError("Input live v7 capture index must be 0..127");
  return `capture-${String(captureIndex).padStart(3, "0")}`;
};

export const inputLiveV7RequestSequence = (
  phase: InputLiveV7RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV7RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV7RequestPath = (
  directory: string,
  phase: InputLiveV7RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV7ResponsePath = (
  directory: string,
  phase: InputLiveV7RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV7ReceiptPath = (
  directory: string,
  phase: InputLiveV7RemotePhase,
  captureIndex?: number,
): string =>
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Input live v7 authorization pins are malformed");
};

const readManifest = (directory: string): InputLiveV7Manifest => {
  const manifest = json<InputLiveV7Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Input live v7 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "input-live-v7-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V7_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V7_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Input live v7 broker manifest");
  return manifest;
};

const unsignedBody = (request: InputLiveV7Request): InputLiveV7RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV7RemotePhase,
  captureIndex?: number,
): InputLiveV7Request | undefined => {
  const file = inputLiveV7RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV7Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV7RemotePhase,
  captureIndex?: number,
): InputLiveV7Receipt | undefined => {
  const file = inputLiveV7ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV7Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV7Request | undefined;
  receipt: InputLiveV7Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV7RemotePhase,
): InputLiveV7ResponseContract => ({
  ...INPUT_LIVE_V7_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V7_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v7-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V7_BROKER_VERSION,
  target: INPUT_LIVE_V7_TARGET,
  ...request,
})};
if(figma.fileKey!==__broker.target.fileKey||figma.root.name!==__broker.target.fileName||figma.editorType!==__broker.target.editorType)throw new Error("INPUT-LIVE-V7-WRONG-TARGET");
const payload=await (async()=>{/*INPUT_LIVE_V7_PROGRAM_START*/${program}/*INPUT_LIVE_V7_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V7_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V7_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Input live v7 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createInputLiveV7Transaction(
  directory: string,
  ...
  if (existsSync(path.join(directory, MANIFEST_FILE)))
    throw new Error("Input live v7 transaction already exists");
  validateAuthorization(options.authorization);
  if (
    !SHA256.test(options.proofPlanSha256) ||
    !SHA256.test(options.captureManifestSha256)
  )
    throw new TypeError("Input live v7 transaction plan hashes malformed");
  const attempt = options.attempt ?? 1;
  if (!Number.isInteger(attempt) || attempt < 1 || attempt > 3)
    throw new TypeError("Input live v7 attempt must be 1..3");
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Input live v7 private signing key does not match authorized public-key identity",
    );
  const manifest: InputLiveV7Manifest = {
    artifactVersion: "input-live-v7-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V7_BROKER_VERSION,
...
export function validateInputLiveV7Request(
  directory: string,
  request: InputLiveV7Request,
): void {
  ...
  const expectedSequence = inputLiveV7RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v7-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V7_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V7_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Input live v7 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueInputLiveV7Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV7RemotePhase,
  program: string,
  options: InputLiveV7IssueOptions = {},
): InputLiveV7Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV7RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV7Request>(targetPath);
    validateInputLiveV7Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v7 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV7ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Input live v7 replay refused: ${id} already accepted`);
  if (phase === "capture") validateInputLiveV7TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Input live v7 out-of-order phase: ${id}`);
  const expectedResponse =
    options.expectedResponse ?? responseContractFor(phase);
  const programSha256 = sha256(program);
  const sequence = inputLiveV7RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V7_TARGET.fileKey,
    timeout: options.timeout ?? (phase === "writer" ? 300_000 : 120_000),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV7RequestBody = {
    artifactVersion: "input-live-v7-broker-request-v2",
    brokerVersion: INPUT_LIVE_V7_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV7Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV7Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV7RawResponse(
  directory: string,
  phase: InputLiveV7RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV7RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v7 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV7Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Input live v7 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = inputLiveV7ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Input live v7 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV7Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v7-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V7_BROKER_VERSION,
...
export function acceptInputLiveV7Response<T = unknown>(
  directory: string,
  phase: InputLiveV7RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV7AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV7ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v7 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV7Request>(
    inputLiveV7RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV7Request(directory, request);
  if (phase === "capture") validateInputLiveV7TechnicalGates(directory);
  const rawFile = inputLiveV7ResponsePath(directory, phase, captureIndex);
  if (!existsSync(rawFile))
    throw new Error(`Input live v7 missing response: ${request.requestId}`);
  ...
  if (
    ...
    !equalTarget(response.fileContext) ||
    plugin?.artifactVersion !== "input-live-v7-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V7_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Input live v7 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistInputLiveV7TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV7RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV7RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Input live v7 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Input live v7 probe payload hash malformed");
  const gates = Object.fromEntries(
    INPUT_LIVE_V7_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v7-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V7_BROKER_VERSION,
...
export function validateInputLiveV7TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v7-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V7_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Input live v7 technical gates artifact");
}

export function validateInputLiveV7Receipt(
  directory: string,
  phase: InputLiveV7RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV7Request(directory, request);
  const receipt = json<InputLiveV7Receipt>(
    inputLiveV7ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV7ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v7-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V7_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Input live v7 accepted receipt: ${request.requestId}`,
    );
}

export function readInputLiveV7AcceptedRawPayload(
  directory: string,
  phase: InputLiveV7RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV7Receipt(directory, phase, captureIndex);
...
export function createInputLiveV7FakeDynamicResponse(
  request: InputLiveV7Request,
  payload: unknown,
  overrides: {
    fileContext?: BrokerTarget;
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V7_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V7_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV7PersistedRequests(
  directory: string,
): InputLiveV7Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV7Request>(file)] : [];
    });
}

export const inputLiveV7Sha256 = sha256;
