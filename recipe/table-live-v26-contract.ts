...
import { buildTableLiveV26CleanupRuntime } from "./table-live-v26-cleanup.js";
import { type TableLiveV26FixedFixedPoint } from "./table-live-v26-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV26RestoreProgram,
  validateTableLiveV26RestorePayload,
  type TableLiveV26RestorePayload,
} from "./table-live-v26-restore.js";
import {
  buildTableLiveV26RawPropertyRuntime,
  normalizeTableLiveV26Scene,
  type TableLiveV26RawNode,
} from "./table-live-v26-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v26.js";
import {
...
export const TABLE_LIVE_V26_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V26_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V26_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V26_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V26_VARIANT_COUNT = 20;
export const TABLE_LIVE_V26_SET_COUNT = 6;
export const TABLE_LIVE_V26_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V26_HOST_PHASES = 3;
export const TABLE_LIVE_V26_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V26_RESTORE_COUNT = 8;
export const TABLE_LIVE_V26_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V26_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV26SourceId = (typeof TABLE_LIVE_V26_SOURCE_IDS)[number];
export type TableLiveV26Kind = "table" | "row" | "cell";

export interface TableLiveV26SourceIdentity {
  source: TableLiveV26SourceId;
...
export interface TableLiveV26WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV26ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV26SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV26RawNode;
    rowScene: TableLiveV26RawNode;
    cellScene: TableLiveV26RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV26RootProof {
  source: TableLiveV26SourceId;
  adapterIdentity: string;
...
export interface TableLiveV26ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV26SourceId;
    adapterIdentity: string;
...
export interface TableLiveV26CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV26SourceId;
  adapterIdentity: string;
...
export interface TableLiveV26CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV26SourceId;
  frameWidth: number;
...
export interface TableLiveV26CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v26 ${label} must be an object`);
...
export function validateTableLiveV26WriterPayload(
  value: unknown,
): TableLiveV26WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V26_NAMESPACE ||
    ...
    throw new TypeError("Table live v26 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v26 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v26 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV26SourceIdentity,
  kind: TableLiveV26Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV26ExtractProgram(
  writer: TableLiveV26WriterOwnership,
  sources: readonly TableLiveV26SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v26 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V26_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V26_NAMESPACE)}
${buildTableLiveV26RawPropertyRuntime()}
...
export function validateTableLiveV26ExtractPayload(
  value: unknown,
  writer: TableLiveV26WriterOwnership,
): TableLiveV26ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v26 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V26_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v26 extract schema/two-root mismatch");
  return payload as TableLiveV26ExtractPayload;
}

export function proveTableLiveV26Roots<Instance>(
  extract: TableLiveV26ExtractPayload,
  sources: readonly (TableLiveV26SourceIdentity & {
...
): TableLiveV26RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v26 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v26 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV26Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV26Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV26Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV26FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v26 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v26 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v26 compile lost owned sets");
...
export function assertTableLiveV26RootProofs(
  proofs: readonly TableLiveV26RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v26 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v26 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV26ProbeProgram(
  writer: TableLiveV26WriterOwnership,
  sources: readonly TableLiveV26SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V26_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV26ProbePayload(
  value: unknown,
  writer: TableLiveV26WriterOwnership,
): TableLiveV26ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV26ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V26_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v26 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV26CaptureManifest(
  cells: readonly TableLiveV26CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V26_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V26_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V26_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V26_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v26 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV26CaptureCell[],
): string => {
  validateTableLiveV26CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV26CaptureProgram(
  writer: TableLiveV26WriterOwnership,
  cell: TableLiveV26CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V26_CAPTURE_COUNT
  )
    throw new TypeError("Table live v26 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v26 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V26_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV26CapturePayload(
  value: unknown,
  cell: TableLiveV26CaptureCell,
  rawResponseBytes: number,
): TableLiveV26CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV26CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V26_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V26_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v26 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV26CaptureResponses(
  manifest: readonly TableLiveV26CaptureCell[],
  responses: readonly TableLiveV26CapturePayload[],
): void {
  validateTableLiveV26CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v26 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV26ObjectiveReport {
  artifactVersion: "table-live-v26-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV26Objective(
  manifest: readonly TableLiveV26CaptureCell[],
  responses: readonly TableLiveV26CapturePayload[],
): TableLiveV26ObjectiveReport {
  assertTableLiveV26CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v26-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV26CleanupProgram(
  writer: TableLiveV26WriterOwnership,
): string {
  const baseProgram = buildTableLiveV26CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V26_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v26 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV26CleanupPayload(
  payload: unknown,
  writer: TableLiveV26WriterOwnership,
): TableLiveV26CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV26CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v26 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V26_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV26WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
