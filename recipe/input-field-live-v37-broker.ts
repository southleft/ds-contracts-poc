...
import {
  assertInputLiveV37RootProofs,
  INPUT_LIVE_V37_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V37_RESPONSE_CONTRACTS,
  type InputLiveV37RootProof,
} from "./input-field-live-v37-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V37_BROKER_VERSION = "input-live-v37-broker-v2";
export const INPUT_LIVE_V37_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V37_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V37_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V37_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V37_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V37_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV37RemotePhase =
  (typeof INPUT_LIVE_V37_REMOTE_PHASES)[number];
export type InputLiveV37AuthorizationMode = "live" | "simulated";

export interface InputLiveV37CommitPins {
...
export interface InputLiveV37TransactionAuthorization extends InputLiveV37CommitPins {
  mode: InputLiveV37AuthorizationMode;
...
export interface InputLiveV37Manifest {
  artifactVersion: "input-live-v37-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V37_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V37_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V37_TARGET;
  authorization: InputLiveV37TransactionAuthorization;
...
export interface InputLiveV37ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV37RequestBody {
  artifactVersion: "input-live-v37-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V37_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV37RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV37CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V37_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV37ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV37Request extends InputLiveV37RequestBody {
...
export interface InputLiveV37Receipt {
  artifactVersion: "input-live-v37-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V37_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV37RemotePhase;
...
export interface InputLiveV37AcceptedResponse<T = unknown> {
  request: InputLiveV37Request;
  receipt: InputLiveV37Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV37IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV37ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V37_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V37_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V37_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V37_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V37_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V37_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V37_TARGET.editorType;
};
...
export const inputLiveV37RequestSequence = (
  phase: InputLiveV37RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV37RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV37RequestPath = (
  directory: string,
  phase: InputLiveV37RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV37ResponsePath = (
  directory: string,
  phase: InputLiveV37RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV37ReceiptPath = (
  directory: string,
  phase: InputLiveV37RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV37Manifest => {
  const manifest = json<InputLiveV37Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v37-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V37_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V37_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV37RemotePhase,
  captureIndex?: number,
): InputLiveV37Request | undefined => {
  const file = inputLiveV37RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV37Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV37RemotePhase,
  captureIndex?: number,
): InputLiveV37Receipt | undefined => {
  const file = inputLiveV37ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV37Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV37Request | undefined;
  receipt: InputLiveV37Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV37RemotePhase,
): InputLiveV37ResponseContract => ({
  ...INPUT_LIVE_V37_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V37_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V37_DYNAMIC_TOOL,
      target: INPUT_LIVE_V37_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v37-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V37_BROKER_VERSION,
  target: INPUT_LIVE_V37_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V37_PROGRAM_START*/${program}/*INPUT_LIVE_V37_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V37_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V37_PROGRAM_END*/";
...
export function createInputLiveV37Transaction(
  directory: string,
  ...
  const manifest: InputLiveV37Manifest = {
    artifactVersion: "input-live-v37-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V37_BROKER_VERSION,
...
export function validateInputLiveV37Request(
  directory: string,
  request: InputLiveV37Request,
): void {
  ...
  const expectedSequence = inputLiveV37RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v37-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V37_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V37_TARGET.fileKey ||
...
export function issueInputLiveV37Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV37RemotePhase,
  program: string,
  options: InputLiveV37IssueOptions = {},
): InputLiveV37Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV37RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV37Request>(targetPath);
    validateInputLiveV37Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV37ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV37TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV37RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V37_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V37_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V37_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV37RequestBody = {
    artifactVersion: "input-live-v37-broker-request-v2",
    brokerVersion: INPUT_LIVE_V37_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV37Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV37Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV37RawResponse(
  directory: string,
  phase: InputLiveV37RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV37RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV37Request>(requestFile);
  if (
  ...
  const target = inputLiveV37ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV37Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v37-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V37_BROKER_VERSION,
...
export function acceptInputLiveV37Response<T = unknown>(
  directory: string,
  phase: InputLiveV37RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV37AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV37ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV37Request>(
    inputLiveV37RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV37Request(directory, request);
  if (phase === "capture") validateInputLiveV37TechnicalGates(directory);
  const rawFile = inputLiveV37ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v37-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V37_BROKER_VERSION ||
...
export function persistInputLiveV37TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV37RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV37RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V37_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v37-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V37_BROKER_VERSION,
...
export function validateInputLiveV37TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v37-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V37_TECHNICAL_GATES.some(
...
export function validateInputLiveV37Receipt(
  directory: string,
  phase: InputLiveV37RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV37Request(directory, request);
  const receipt = json<InputLiveV37Receipt>(
    inputLiveV37ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV37ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v37-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V37_BROKER_VERSION ||
...
export function readInputLiveV37AcceptedRawPayload(
  directory: string,
  phase: InputLiveV37RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV37Receipt(directory, phase, captureIndex);
...
export function createInputLiveV37FakeDynamicResponse(
  request: InputLiveV37Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V37_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V37_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV37PersistedRequests(
  directory: string,
): InputLiveV37Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV37Request>(file)] : [];
    });
}

export const inputLiveV37Sha256 = sha256;
