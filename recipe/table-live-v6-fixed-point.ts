/**
 * Table live v6 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V6_FIXED_VERIFIER_VERSION = "table-live-v6-fixed-point-v1";
export const TABLE_LIVE_V6_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV6FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV6FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV6FixedSourceProbe {
...
export interface TableLiveV6FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV6FixedFixedPoint;
}

export interface TableLiveV6FixedHardGateInput {
  thresholds: TableLiveV6FixedThresholds;
  sourceProbes: TableLiveV6FixedSourceProbe[];
  cells: TableLiveV6FixedCellValidation[];
  sceneProofs: TableLiveV6FixedSceneProof[];
...
export interface TableLiveV6FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V6_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV6FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV6FixedHardGates(
  input: TableLiveV6FixedHardGateInput,
): TableLiveV6FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV6FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  if (input.visualRows.length !== 0)
    failures.push("table live v6 must not invent a visual comparison");
  ...
  return {
    verifierVersion: TABLE_LIVE_V6_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV6FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV6FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
