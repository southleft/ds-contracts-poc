...
import { INPUT_LIVE_V10_INDEX_PATH } from "./build-input-field-live-proof-v10.js";
import {
  INPUT_LIVE_V10_DYNAMIC_TOOL,
  INPUT_LIVE_V10_TARGET,
} from "./input-field-live-v10-broker.js";
import {
  buildInputLiveV10AuthorizationArtifact,
  validateInputLiveV10AntecedentIndex,
  validateInputLiveV10AuthorizationArtifact,
  validateInputLiveV10History,
  validateInputLiveV10UnitTestSource,
  type InputLiveV10AntecedentIndex,
  type InputLiveV10AuthorizationArtifact,
  type InputLiveV10AuthorizationProof,
  type InputLiveV10HistoryState,
} from "./input-field-live-v10-authorization.js";
import {
  inputLiveV10AttestationSha256,
  validateInputLiveV10ControlFlowSource,
  validateInputLiveV10SecurityAttestation,
  type InputLiveV10SecurityAttestation,
} from "./input-field-live-v10-preflight.js";
import { inputLiveV10Sha256 } from "./input-field-live-v10-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V10_INDEX_PATH);
const index = (): InputLiveV10AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV10AntecedentIndex;

const artifact = (): InputLiveV10AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV10AuthorizationArtifact({
...
const pendingState = (): InputLiveV10HistoryState => ({
  index: index(),
  indexSha256: inputLiveV10Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV10HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV10AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV10Sha256(indexBytes()),
...
const attestation = (
...
): InputLiveV10SecurityAttestation => {
  const body: Omit<InputLiveV10SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v10-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V10_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV10AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV10History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV10History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV10History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV10History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV10History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV10UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV10UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV10AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV10Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV10AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV10AuthorizationArtifact(
        planted as InputLiveV10AuthorizationArtifact,
        index(),
        inputLiveV10Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV10SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV10AttestationSha256(
      body as Omit<InputLiveV10SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV10SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV10ControlFlowSource(source), []);
});
