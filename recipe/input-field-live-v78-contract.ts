...
} from "./input-field-live-v3-verifier-v78.js";
import {
  buildInputLiveV78RawPropertyRuntime,
  normalizeInputLiveV78Scene,
  type InputLiveV78RawNode,
} from "./input-field-live-v78-verifier.js";
...
export {
  buildInputLiveV78RestoreProgram,
  validateInputLiveV78RestorePayload,
  type InputLiveV78RestorePayload,
} from "./input-field-live-v78-restore.js";

export const INPUT_LIVE_V78_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V78_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V78_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V78_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V78_VARIANT_COUNT = 256;
export const INPUT_LIVE_V78_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V78_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV78SourceId = (typeof INPUT_LIVE_V78_SOURCE_IDS)[number];

export interface InputLiveV78SourceIdentity {
  source: InputLiveV78SourceId;
...
export interface InputLiveV78WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV78ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV78SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV78RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV78RootProof {
  source: InputLiveV78SourceId;
  adapterIdentity: string;
...
export interface InputLiveV78ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV78SourceId;
    adapterIdentity: string;
...
export interface InputLiveV78CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV78SourceId;
  adapterIdentity: string;
...
export interface InputLiveV78CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV78SourceId;
  strata: InputLiveV78CaptureCell["strata"];
...
export interface InputLiveV78CleanupPayload {
...
export function validateInputLiveV78WriterPayload(
  value: unknown,
): InputLiveV78WriterOwnership {
...
const identityPlan = (source: InputLiveV78SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV78ExtractProgram(
  writer: InputLiveV78WriterOwnership,
  sources: readonly InputLiveV78SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V78_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V78_NAMESPACE)}
${buildInputLiveV78RawPropertyRuntime()}
...
export function validateInputLiveV78ExtractPayload(
  value: unknown,
  writer: InputLiveV78WriterOwnership,
): InputLiveV78ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V78_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV78ExtractPayload;
}

export function proveInputLiveV78Roots<Instance>(
  extract: InputLiveV78ExtractPayload,
  sources: readonly (InputLiveV78SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV78Scene(
      root.scene,
...
export function assertInputLiveV78RootProofs(
  proofs: readonly InputLiveV78RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV78ProbeProgram(
  writer: InputLiveV78WriterOwnership,
  sources: readonly InputLiveV78SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V78_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV78ProbePayload(
  value: unknown,
  writer: InputLiveV78WriterOwnership,
): InputLiveV78ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV78ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V78_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V78_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV78CaptureManifest(
  cells: readonly InputLiveV78CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V78_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V78_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V78_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V78_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V78_SOURCE_IDS.some(
...
export const inputLiveV78CaptureManifestSha256 = (
  cells: readonly InputLiveV78CaptureCell[],
): string => {
  validateInputLiveV78CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV78CaptureProgram(
  writer: InputLiveV78WriterOwnership,
  cell: InputLiveV78CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V78_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V78_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V78_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV78CapturePayload(
  value: unknown,
  cell: InputLiveV78CaptureCell,
  rawResponseBytes: number,
): InputLiveV78CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV78CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V78_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V78_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV78CaptureResponses(
  manifest: readonly InputLiveV78CaptureCell[],
  responses: readonly InputLiveV78CapturePayload[],
): void {
  validateInputLiveV78CaptureManifest(manifest);
  if (
...
export interface InputLiveV78ObjectiveReport {
  artifactVersion: "input-live-v78-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV78Objective(
  manifest: readonly InputLiveV78CaptureCell[],
  responses: readonly InputLiveV78CapturePayload[],
  readReference: (cell: InputLiveV78CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV78CaptureCell) => Buffer,
): InputLiveV78ObjectiveReport {
  assertInputLiveV78CaptureResponses(manifest, responses);
...
export function buildInputLiveV78CleanupProgram(
  writer: InputLiveV78WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V78_NAMESPACE,
...
export function validateInputLiveV78CleanupPayload(
  value: unknown,
  writer: InputLiveV78WriterOwnership,
): InputLiveV78CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV78CleanupPayload;
...
export const INPUT_LIVE_V78_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV78WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
