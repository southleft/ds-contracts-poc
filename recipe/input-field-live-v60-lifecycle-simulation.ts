...
export interface InputLiveV60LifecycleSimulationReport {
  artifactVersion: "input-live-v60-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV60AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV60Lifecycle(): Promise<InputLiveV60LifecycleSimulationReport> {
  await buildInputLiveV60Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V60_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV60AntecedentIndex;
  const indexSha256 = inputLiveV60Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v60-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V60_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V60_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV60History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV60History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV60AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V60_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V60_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV60History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV60History(authorized, "pending");
    ...
    await buildInputLiveV60Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V60_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV60AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV60Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV60Lifecycle(), null, 2)}\n`,
  );
