...
import { INPUT_LIVE_V59_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v59.js";
import { verifyInputLiveV59Authorization } from "./input-field-live-v59-authorization.js";
import { runInputLiveV59Preflight } from "./input-field-live-v59-preflight.js";
import { InputLiveV59Orchestrator } from "./run-input-field-live-v59.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V59_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v59-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV59Authorization(root);
    runInputLiveV59Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV59Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
