...
import { calendarLiveV1Sha256 } from "./calendar-live-v10-broker.js";

export interface CalendarLiveV10LifecycleSimulationReport {
  artifactVersion: "calendar-live-v10-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV10AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV10Lifecycle(): Promise<CalendarLiveV10LifecycleSimulationReport> {
  await buildCalendarLiveV10Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V10_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV10AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v10-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V10_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V10_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV10History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV10History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV10AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V10_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V10_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV10History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV10History(authorized, "pending");
    ...
    await buildCalendarLiveV10Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V10_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV10AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV10Lifecycle(), null, 2)}\n`,
  );
