...
/**
 * Carried Combobox live v8 scene-readback. Keeps v1 host-normalize,
 * v2–v4 extract teachings, v5 empty-payload omit, and v6 live-root
 * observeSceneFacts. Teaches that sceneToNormalizedIr recovers instance
 * componentRef via last-segment / recipe identity (Button class): last
 * segment after ` / ` when it is already a recipe ref; otherwise
 * `{recipe}/{part}-instance/{n}` recovers `{recipe}@1/{part}`. Live
 * Figma option-instance main-component names such as
 * `Size=small, Option state=selected` therefore become
 * `combobox@1/option`. Does not invent glyph text. Does not patch
 * hashed v1–v6 lineage bytes.
 */
...
export const COMBOBOX_LIVE_V8_OMIT_EMPTY_INSTANCE_PAYLOAD_MARKER =
  "COMBOBOX-HOST-OMIT-EMPTY-INSTANCE-PAYLOAD";
export const COMBOBOX_LIVE_V8_PROJECT_LIVE_ROOT_OWNERSHIP_KEY_MARKER =
  "COMBOBOX-HOST-PROJECT-LIVE-ROOT-OWNERSHIP-KEY";
export const COMBOBOX_LIVE_V8_RECOVER_RECIPE_COMPONENT_REF_MARKER =
...
export function canonicalizeObservedComponentRef(
  ...
  void COMBOBOX_LIVE_V8_RECOVER_RECIPE_COMPONENT_REF_MARKER;
...
const shouldOmitEmptyInstancePayload = (
  ...
  void COMBOBOX_LIVE_V8_OMIT_EMPTY_INSTANCE_PAYLOAD_MARKER;
...
const observeSceneFacts = (root: SceneNodeSnapshot): SceneFact[] => {
  void COMBOBOX_LIVE_V8_PROJECT_LIVE_ROOT_OWNERSHIP_KEY_MARKER;
