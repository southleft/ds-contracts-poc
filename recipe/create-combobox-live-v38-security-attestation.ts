...
import { verifyComboboxLiveV38Authorization } from "./combobox-live-v38-authorization.js";
import { COMBOBOX_LIVE_V38_TARGET } from "./combobox-live-v38-broker.js";
import {
  COMBOBOX_LIVE_V38_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV38Authorization();
...
const attestation: ComboboxLiveV38SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV38SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v38 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V38_SECURITY_ATTESTATION_DEFAULT_PATH,
);
