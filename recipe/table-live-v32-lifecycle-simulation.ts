...
import { tableLiveV2Sha256 } from "./table-live-v32-broker.js";

export interface TableLiveV32LifecycleSimulationReport {
  artifactVersion: "table-live-v32-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildTableLiveV32AuthorizationArtifact> | undefined,
...
export async function simulateTableLiveV32Lifecycle(): Promise<TableLiveV32LifecycleSimulationReport> {
  await buildTableLiveV32Proof(true);
  const indexBefore = readFileSync(TABLE_LIVE_V32_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as TableLiveV32AntecedentIndex;
  const indexSha256 = tableLiveV2Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "table-live-v32-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, TABLE_LIVE_V32_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", TABLE_LIVE_V32_INDEX_PATH]);
    ...
    const pendingFailures = validateTableLiveV32History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateTableLiveV32History(pending, "authorized");
    if (
    ...
    const authorization = buildTableLiveV32AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", TABLE_LIVE_V32_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${TABLE_LIVE_V32_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateTableLiveV32History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateTableLiveV32History(authorized, "pending");
    ...
    await buildTableLiveV32Proof(true);
    const indexAfter = readFileSync(TABLE_LIVE_V32_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as TableLiveV32AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateTableLiveV32Lifecycle(), null, 2)}\n`,
  );
