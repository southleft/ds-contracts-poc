...
export type TableLiveV26HistoryExpectation = "pending" | "authorized";

export interface TableLiveV26AuthorizationArtifact {
  artifactVersion: "table-live-v26-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V26_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV26AntecedentIndex {
...
export interface TableLiveV26HistoryState {
  index?: TableLiveV26AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV26AuthorizationArtifact;
...
export interface TableLiveV26AuthorizationProof
  extends TableLiveV26TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V26_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V26_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V26_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V26_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v26-authorization") ||
  artifactPath.includes("table-live-v26-preflight") ||
  artifactPath.includes("table-live-v26-authorized") ||
  artifactPath.includes("create-table-live-v26-security-attestation") ||
  artifactPath.includes("table-live-pivot-v26-status") ||
...
export function validateTableLiveV26AntecedentIndex(
  index: TableLiveV26AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v26-antecedent-index-v1" ||
...
export function validateTableLiveV26UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV26History\(\s*["']/.test(source) ||
    /readTableLiveV26HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV26AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV26AuthorizationArtifact {
  ...
  ) as TableLiveV26AntecedentIndex;
  const indexFailures = validateTableLiveV26AntecedentIndex(index);
...
export function validateTableLiveV26AuthorizationArtifact(
  artifact: TableLiveV26AuthorizationArtifact | undefined,
  index: TableLiveV26AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v26-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v26" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V26_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V26_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V26_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V26_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V26_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V26_SOURCE_ROOTS ||
...
export function validateTableLiveV26History(
  state: TableLiveV26HistoryState,
  expected: TableLiveV26HistoryExpectation,
): string[] {
  const failures = validateTableLiveV26AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV26AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV26HistoryState(
  root = process.cwd(),
): TableLiveV26HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V26_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV26AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V26_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V26_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V26_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV26History(
  expected: TableLiveV26HistoryExpectation,
  root = process.cwd(),
): TableLiveV26HistoryState {
  const state = readTableLiveV26HistoryState(root);
  const failures = validateTableLiveV26History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v26 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV26Authorization(
  root = process.cwd(),
): TableLiveV26AuthorizationProof {
  const state = verifyTableLiveV26History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V26_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v26/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v26.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V26_TARGET,
    expectedDynamicTool: TABLE_LIVE_V26_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V26_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV26HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV26History(expected);
  process.stdout.write(
    `Table live v26 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
