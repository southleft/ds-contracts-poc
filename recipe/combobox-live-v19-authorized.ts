...
import { COMBOBOX_LIVE_V19_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v19.js";
import { verifyComboboxLiveV19Authorization } from "./combobox-live-v19-authorization.js";
import { runComboboxLiveV19Preflight } from "./combobox-live-v19-preflight.js";
import { ComboboxLiveV19Orchestrator } from "./run-combobox-live-v19.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v19 requires exact authorization artifact ${COMBOBOX_LIVE_V19_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v19-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV19Authorization(root);
    runComboboxLiveV19Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV19Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV19Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
