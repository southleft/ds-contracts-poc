...
import {
  ...
  INPUT_LIVE_V28_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V28_RESPONSE_CONTRACTS,
  type InputLiveV28RootProof,
} from "./input-field-live-v28-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V28_BROKER_VERSION = "input-live-v28-broker-v2";
export const INPUT_LIVE_V28_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V28_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V28_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V28_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V28_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V28_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV28RemotePhase =
  (typeof INPUT_LIVE_V28_REMOTE_PHASES)[number];
export type InputLiveV28AuthorizationMode = "live" | "simulated";

export interface InputLiveV28CommitPins {
...
export interface InputLiveV28TransactionAuthorization extends InputLiveV28CommitPins {
  mode: InputLiveV28AuthorizationMode;
...
export interface InputLiveV28Manifest {
  artifactVersion: "input-live-v28-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V28_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V28_DYNAMIC_TOOL;
...
export interface InputLiveV28ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV28RequestBody {
  ...
  commitPins: InputLiveV28CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V28_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV28ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV28Request extends InputLiveV28RequestBody {
...
export interface InputLiveV28Receipt {
  artifactVersion: "input-live-v28-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V28_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV28RemotePhase;
...
export interface InputLiveV28AcceptedResponse<T = unknown> {
  request: InputLiveV28Request;
  receipt: InputLiveV28Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV28IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV28ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V28_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V28_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V28_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V28_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V28_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V28_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V28_TARGET.editorType;
};
...
export const inputLiveV28RequestSequence = (
  phase: InputLiveV28RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV28RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV28RequestPath = (
  directory: string,
  phase: InputLiveV28RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV28ResponsePath = (
  directory: string,
  phase: InputLiveV28RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV28ReceiptPath = (
  directory: string,
  phase: InputLiveV28RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV28Manifest => {
  const manifest = json<InputLiveV28Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v28-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V28_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V28_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV28RemotePhase,
  captureIndex?: number,
): InputLiveV28Request | undefined => {
  const file = inputLiveV28RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV28Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV28RemotePhase,
  captureIndex?: number,
): InputLiveV28Receipt | undefined => {
  const file = inputLiveV28ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV28Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV28Request | undefined;
  receipt: InputLiveV28Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV28RemotePhase,
): InputLiveV28ResponseContract => ({
  ...INPUT_LIVE_V28_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V28_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v28-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V28_BROKER_VERSION,
  target: INPUT_LIVE_V28_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V28_PROGRAM_START*/${program}/*INPUT_LIVE_V28_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V28_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V28_PROGRAM_END*/";
...
export function createInputLiveV28Transaction(
  directory: string,
  ...
  const manifest: InputLiveV28Manifest = {
    artifactVersion: "input-live-v28-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V28_BROKER_VERSION,
...
export function validateInputLiveV28Request(
  directory: string,
  request: InputLiveV28Request,
): void {
  ...
  const expectedSequence = inputLiveV28RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v28-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V28_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V28_TARGET.fileKey ||
...
export function issueInputLiveV28Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV28RemotePhase,
  program: string,
  options: InputLiveV28IssueOptions = {},
): InputLiveV28Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV28RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV28Request>(targetPath);
    validateInputLiveV28Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV28ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV28TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV28RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    ...
    timeout:
      ...
      (phase === "writer"
        ...
        : INPUT_LIVE_V28_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV28RequestBody = {
    artifactVersion: "input-live-v28-broker-request-v2",
    brokerVersion: INPUT_LIVE_V28_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV28Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV28Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV28RawResponse(
  directory: string,
  phase: InputLiveV28RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV28RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV28Request>(requestFile);
  if (
  ...
  const target = inputLiveV28ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV28Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v28-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V28_BROKER_VERSION,
...
export function acceptInputLiveV28Response<T = unknown>(
  directory: string,
  phase: InputLiveV28RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV28AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV28ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV28Request>(
    inputLiveV28RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV28Request(directory, request);
  if (phase === "capture") validateInputLiveV28TechnicalGates(directory);
  const rawFile = inputLiveV28ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v28-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V28_BROKER_VERSION ||
...
export function persistInputLiveV28TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV28RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV28RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V28_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v28-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V28_BROKER_VERSION,
...
export function validateInputLiveV28TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v28-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V28_TECHNICAL_GATES.some(
...
export function validateInputLiveV28Receipt(
  directory: string,
  phase: InputLiveV28RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV28Request(directory, request);
  const receipt = json<InputLiveV28Receipt>(
    inputLiveV28ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV28ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v28-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V28_BROKER_VERSION ||
...
export function readInputLiveV28AcceptedRawPayload(
  directory: string,
  phase: InputLiveV28RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV28Receipt(directory, phase, captureIndex);
...
export function createInputLiveV28FakeDynamicResponse(
  request: InputLiveV28Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V28_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V28_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV28PersistedRequests(
  directory: string,
): InputLiveV28Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV28Request>(file)] : [];
    });
}

export const inputLiveV28Sha256 = sha256;
