...
import {
  acceptComboboxLiveV12Response,
  createComboboxLiveV12Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV12Request,
  persistComboboxLiveV12RawResponse,
  persistComboboxLiveV12TechnicalGates,
  readComboboxLiveV12AcceptedRawPayload,
  type ComboboxLiveV12Request,
  type ComboboxLiveV12TransactionAuthorization,
} from "./combobox-live-v12-broker.js";
import {
  COMBOBOX_LIVE_V12_CAPTURE_COUNT,
  assertComboboxLiveV12CaptureResponses,
  ...
  evaluateComboboxLiveV12Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV12Roots,
  validateComboboxLiveV12CaptureManifest,
  validateComboboxLiveV12CapturePayload,
  validateComboboxLiveV12CleanupPayload,
  validateComboboxLiveV12ExtractPayload,
  validateComboboxLiveV12ProbePayload,
  validateComboboxLiveV12WriterPayload,
  type ComboboxLiveV12CaptureCell,
  type ComboboxLiveV12CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V12_PLAN_PATH,
  type ComboboxLiveV12ProofPlan,
} from "./build-combobox-live-proof-v12.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v12.js";

export const COMBOBOX_LIVE_V12_RUNNER_VERSION = "combobox-live-v12-runner-v1";

interface ComboboxLiveV12JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV12OrchestratorState {
  artifactVersion: "combobox-live-v12-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV12JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV12ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV12CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV12SourceIdentity & {
...
export interface ComboboxLiveV12NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV12OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v12 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V12_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V12_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV12ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV12CaptureCell[];
  validateComboboxLiveV12CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v12 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v12 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV12WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV12RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV12Request =>
  readJson<ComboboxLiveV12Request>(file);

export class ComboboxLiveV12Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV12Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV12OrchestratorState = {
      artifactVersion: "combobox-live-v12-orchestrator-state-v1",
    ...
    return new ComboboxLiveV12Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV12OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV12Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV12NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV12NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v12 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV12NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v12 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV12RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV12Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV12CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV12AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV12WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV12Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV12ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV12Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV12Response(this.#directory, "writer", {
        validate: validateComboboxLiveV12WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV12Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV12RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV12Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV12ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV12Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV12RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v12-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV12Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV12ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV12Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV12ProbePayload(payload, ownership),
      });
      persistComboboxLiveV12TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v12 capture cell missing");
      const accepted = acceptComboboxLiveV12Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV12CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V12_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV12CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV12Objective(
    ...
    throw new Error(`Combobox live v12 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV12Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV12CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV12Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV12RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV12Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV12CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV12CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV12CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV12Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV12TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV12SimulationKey = (): KeyObject =>
