...
import { verifyComboboxLiveV34Authorization } from "./combobox-live-v34-authorization.js";
import { COMBOBOX_LIVE_V34_TARGET } from "./combobox-live-v34-broker.js";
import {
  COMBOBOX_LIVE_V34_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyComboboxLiveV34Authorization();
...
const attestation: ComboboxLiveV34SecurityAttestation = {
  ...body,
  attestationSha256: comboboxLiveV5AttestationSha256(body),
};
const failures = validateComboboxLiveV34SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Combobox live v34 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? COMBOBOX_LIVE_V34_SECURITY_ATTESTATION_DEFAULT_PATH,
);
