...
import { INPUT_LIVE_V8_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v8.js";
import { verifyInputLiveV8Authorization } from "./input-field-live-v8-authorization.js";
import { runInputLiveV8Preflight } from "./input-field-live-v8-preflight.js";
import { InputLiveV8Orchestrator } from "./run-input-field-live-v8.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v8 requires exact authorization artifact ${INPUT_LIVE_V8_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v8-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV8Authorization(root);
    runInputLiveV8Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV8Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
