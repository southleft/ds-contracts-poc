...
import {
  acceptComboboxLiveV22Response,
  createComboboxLiveV22Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV22Request,
  persistComboboxLiveV22RawResponse,
  persistComboboxLiveV22TechnicalGates,
  readComboboxLiveV22AcceptedRawPayload,
  type ComboboxLiveV22Request,
  type ComboboxLiveV22TransactionAuthorization,
} from "./combobox-live-v22-broker.js";
import {
  COMBOBOX_LIVE_V22_CAPTURE_COUNT,
  assertComboboxLiveV22CaptureResponses,
  ...
  evaluateComboboxLiveV22Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV22Roots,
  validateComboboxLiveV22CaptureManifest,
  validateComboboxLiveV22CapturePayload,
  validateComboboxLiveV22CleanupPayload,
  validateComboboxLiveV22ExtractPayload,
  validateComboboxLiveV22ProbePayload,
  validateComboboxLiveV22WriterPayload,
  type ComboboxLiveV22CaptureCell,
  type ComboboxLiveV22CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V22_PLAN_PATH,
  type ComboboxLiveV22ProofPlan,
} from "./build-combobox-live-proof-v22.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v22.js";

export const COMBOBOX_LIVE_V22_RUNNER_VERSION = "combobox-live-v22-runner-v1";

interface ComboboxLiveV22JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV22OrchestratorState {
  artifactVersion: "combobox-live-v22-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV22JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV22ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV22CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV22SourceIdentity & {
...
export interface ComboboxLiveV22NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV22OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v22 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V22_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V22_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV22ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV22CaptureCell[];
  validateComboboxLiveV22CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v22 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v22 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV22WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV22RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV22Request =>
  readJson<ComboboxLiveV22Request>(file);

export class ComboboxLiveV22Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV22Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV22OrchestratorState = {
      artifactVersion: "combobox-live-v22-orchestrator-state-v1",
    ...
    return new ComboboxLiveV22Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV22OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV22Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV22NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV22NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v22 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV22NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v22 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV22RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV22Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV22CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV22AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV22WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV22Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV22ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV22Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV22Response(this.#directory, "writer", {
        validate: validateComboboxLiveV22WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV22Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV22RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV22Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV22ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV22Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV22RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v22-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV22Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV22ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV22Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV22ProbePayload(payload, ownership),
      });
      persistComboboxLiveV22TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v22 capture cell missing");
      const accepted = acceptComboboxLiveV22Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV22CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V22_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV22CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV22Objective(
    ...
    throw new Error(`Combobox live v22 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV22Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV22CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV22Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV22RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV22Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV22CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV22CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV22CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV22Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV22TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV22SimulationKey = (): KeyObject =>
