...
import { INPUT_LIVE_V7_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v7.js";
import { verifyInputLiveV7Authorization } from "./input-field-live-v7-authorization.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v7 requires exact authorization artifact ${INPUT_LIVE_V7_AUTHORIZATION_PATH}`,
    );
  return expected;
};
