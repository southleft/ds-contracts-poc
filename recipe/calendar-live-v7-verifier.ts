...
import {
  verifyCalendarLiveV7FixedSceneFixedPoint,
  type CalendarLiveV7FixedFixedPoint,
} from "./calendar-live-v7-fixed-point.js";

export const CALENDAR_LIVE_V7_VERIFIER_VERSION = "calendar-live-v7-verifier-v1";

export function buildCalendarLiveV7RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV7RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV7RawNode[];
}

export interface CalendarLiveV7NormalizedScene {
...
export interface CalendarLiveV7SceneProof {
...
export function normalizeCalendarLiveV7Scene(
  root: CalendarLiveV7RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV7NormalizedScene {
  const canonicalBindings: CalendarLiveV7NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV7RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV7Scene<Instance>(
  normalized: CalendarLiveV7NormalizedScene,
...
): CalendarLiveV7SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV7FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV7ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV7PreCaptureGates(payload: unknown): void {
