...
import {
  acceptCalendarLiveV23Response,
  createCalendarLiveV23Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV23Request,
  persistCalendarLiveV23RawResponse,
  persistCalendarLiveV23TechnicalGates,
  readCalendarLiveV23AcceptedRawPayload,
  type CalendarLiveV23Request,
  type CalendarLiveV23TransactionAuthorization,
} from "./calendar-live-v23-broker.js";
import {
  CALENDAR_LIVE_V23_CAPTURE_COUNT,
  assertCalendarLiveV23CaptureResponses,
  ...
  evaluateCalendarLiveV23Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV23Roots,
  validateCalendarLiveV23CaptureManifest,
  validateCalendarLiveV23CapturePayload,
  validateCalendarLiveV23CleanupPayload,
  validateCalendarLiveV23ExtractPayload,
  validateCalendarLiveV23ProbePayload,
  validateCalendarLiveV23WriterPayload,
  type CalendarLiveV23CaptureCell,
  type CalendarLiveV23CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V23_PLAN_PATH,
  type CalendarLiveV23ProofPlan,
} from "./build-calendar-live-proof-v23.js";
import {
...
export const CALENDAR_LIVE_V23_RUNNER_VERSION = "calendar-live-v23-runner-v1";

interface CalendarLiveV23JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV23OrchestratorState {
  artifactVersion: "calendar-live-v23-orchestrator-state-v1";
  ...
  journal: CalendarLiveV23JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV23ProofPlan;
  planSha256: string;
  captures: CalendarLiveV23CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV23SourceIdentity & {
...
export interface CalendarLiveV23NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV23OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v23 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V23_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V23_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV23ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV23CaptureCell[];
  validateCalendarLiveV23CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v23 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v23 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV23WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV23RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV23Request =>
  readJson<CalendarLiveV23Request>(file);

export class CalendarLiveV23Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV23Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV23OrchestratorState = {
      artifactVersion: "calendar-live-v23-orchestrator-state-v1",
    ...
    return new CalendarLiveV23Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV23OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV23Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV23NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV23NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v23 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV23NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v23 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV23RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV23Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV23CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV23AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV23WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV23Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV23ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV23Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV23Response(this.#directory, "writer", {
        validate: validateCalendarLiveV23WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV23Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV23RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV23Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV23ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV23Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV23RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v23-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV23Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV23ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV23Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV23ProbePayload(payload, ownership),
      });
      persistCalendarLiveV23TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v23 capture cell missing");
      const accepted = acceptCalendarLiveV23Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV23CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V23_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV23CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV23Objective(
    ...
    throw new Error(`Calendar live v23 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV23Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV23CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV23Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV23RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV23Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV23CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV23CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV23CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV23Authorization = (
  privateKey: KeyObject,
): CalendarLiveV23TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV23SimulationKey = (): KeyObject =>
