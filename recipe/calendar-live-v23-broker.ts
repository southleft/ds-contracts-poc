...
import {
  assertCalendarLiveV23RootProofs,
  CALENDAR_LIVE_V23_CAPTURE_COUNT,
  CALENDAR_LIVE_V23_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  CALENDAR_LIVE_V23_RESPONSE_CONTRACTS,
  CALENDAR_LIVE_V23_SOURCE_ROOTS,
  type CalendarLiveV23RootProof,
} from "./calendar-live-v23-contract.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V23_BROKER_VERSION = "calendar-live-v23-broker-v1";
export const CALENDAR_LIVE_V23_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const CALENDAR_LIVE_V23_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const CALENDAR_LIVE_V23_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const CALENDAR_LIVE_V23_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const CALENDAR_LIVE_V23_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const CALENDAR_LIVE_V23_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type CalendarLiveV23RemotePhase =
  (typeof CALENDAR_LIVE_V23_REMOTE_PHASES)[number];
export type CalendarLiveV23AuthorizationMode = "live" | "simulated";

export interface CalendarLiveV23CommitPins {
...
export interface CalendarLiveV23TransactionAuthorization extends CalendarLiveV23CommitPins {
  mode: CalendarLiveV23AuthorizationMode;
...
export interface CalendarLiveV23Manifest {
  artifactVersion: "calendar-live-v23-broker-manifest-v2";
  brokerVersion: typeof CALENDAR_LIVE_V23_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof CALENDAR_LIVE_V23_DYNAMIC_TOOL;
  target: typeof CALENDAR_LIVE_V23_TARGET;
  authorization: CalendarLiveV23TransactionAuthorization;
...
export interface CalendarLiveV23ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface CalendarLiveV23RequestBody {
  artifactVersion: "calendar-live-v23-broker-request-v2";
  brokerVersion: typeof CALENDAR_LIVE_V23_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: CalendarLiveV23RemotePhase;
  sequence: number;
  ...
  commitPins: CalendarLiveV23CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof CALENDAR_LIVE_V23_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: CalendarLiveV23ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface CalendarLiveV23Request extends CalendarLiveV23RequestBody {
...
export interface CalendarLiveV23Receipt {
  artifactVersion: "calendar-live-v23-broker-receipt-v2";
  brokerVersion: typeof CALENDAR_LIVE_V23_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: CalendarLiveV23RemotePhase;
...
export interface CalendarLiveV23AcceptedResponse<T = unknown> {
  request: CalendarLiveV23Request;
  receipt: CalendarLiveV23Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface CalendarLiveV23IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: CalendarLiveV23ResponseContract;
...
const equalTarget = (value: unknown): value is typeof CALENDAR_LIVE_V23_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === CALENDAR_LIVE_V23_TARGET.fileKey &&
  (value as BrokerTarget).fileName === CALENDAR_LIVE_V23_TARGET.fileName &&
  (value as BrokerTarget).editorType === CALENDAR_LIVE_V23_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== CALENDAR_LIVE_V23_TARGET.fileKey ||
    context.fileName !== CALENDAR_LIVE_V23_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === CALENDAR_LIVE_V23_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Calendar live v23 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= CALENDAR_LIVE_V23_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v23 capture index must be 0..7");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Calendar live v23 authorization pins are malformed");
};

const readManifest = (directory: string): CalendarLiveV23Manifest => {
  const manifest = json<CalendarLiveV23Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Calendar live v23 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "calendar-live-v23-broker-manifest-v2" ||
    manifest.brokerVersion !== CALENDAR_LIVE_V23_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== CALENDAR_LIVE_V23_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(CALENDAR_LIVE_V23_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Calendar live v23 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: CalendarLiveV23Request,
): CalendarLiveV23RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: CalendarLiveV23RemotePhase,
  captureIndex?: number,
): CalendarLiveV23Request | undefined => {
  const file = calendarLiveV1RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV23Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: CalendarLiveV23RemotePhase,
  captureIndex?: number,
): CalendarLiveV23Receipt | undefined => {
  const file = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV23Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: CalendarLiveV23Request | undefined;
  receipt: CalendarLiveV23Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: CalendarLiveV23RemotePhase,
): CalendarLiveV23ResponseContract => ({
  ...CALENDAR_LIVE_V23_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: CalendarLiveV23Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: CALENDAR_LIVE_V23_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: CALENDAR_LIVE_V23_DYNAMIC_TOOL,
      target: CALENDAR_LIVE_V23_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "calendar-live-v23-plugin-response-v2",
  brokerVersion: CALENDAR_LIVE_V23_BROKER_VERSION,
  target: CALENDAR_LIVE_V23_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*CALENDAR_LIVE_V23_PROGRAM_START*/${program}/*CALENDAR_LIVE_V23_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*CALENDAR_LIVE_V23_PROGRAM_START*/";
  const endMarker = "/*CALENDAR_LIVE_V23_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Calendar live v23 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createCalendarLiveV23Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Calendar live v23 private signing key does not match authorized public-key identity",
    );
  const manifest: CalendarLiveV23Manifest = {
    artifactVersion: "calendar-live-v23-broker-manifest-v2",
    brokerVersion: CALENDAR_LIVE_V23_BROKER_VERSION,
    ...
    captureRequests: CALENDAR_LIVE_V23_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateCalendarLiveV23Request(
  directory: string,
  request: CalendarLiveV23Request,
): void {
  ...
  if (
    request.artifactVersion !== "calendar-live-v23-broker-request-v2" ||
    request.brokerVersion !== CALENDAR_LIVE_V23_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== CALENDAR_LIVE_V23_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Calendar live v23 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueCalendarLiveV23Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: CalendarLiveV23RemotePhase,
  program: string,
  options: CalendarLiveV23IssueOptions = {},
): CalendarLiveV23Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<CalendarLiveV23Request>(targetPath);
    validateCalendarLiveV23Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Calendar live v23 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(calendarLiveV1ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Calendar live v23 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Calendar live v23 empty code envelope refused");
  if (phase === "capture") validateCalendarLiveV23TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Calendar live v23 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: CALENDAR_LIVE_V23_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? CALENDAR_LIVE_V23_SIGNED_WRITER_TIMEOUT_MS
        : CALENDAR_LIVE_V23_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: CalendarLiveV23RequestBody = {
    artifactVersion: "calendar-live-v23-broker-request-v2",
    brokerVersion: CALENDAR_LIVE_V23_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: CalendarLiveV23Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateCalendarLiveV23Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistCalendarLiveV23RawResponse(
  directory: string,
  phase: CalendarLiveV23RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Calendar live v23 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<CalendarLiveV23Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Calendar live v23 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = calendarLiveV1ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Calendar live v23 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): CalendarLiveV23Receipt => {
  const body: Omit<CalendarLiveV23Receipt, "receiptSha256"> = {
    artifactVersion: "calendar-live-v23-broker-receipt-v2",
    brokerVersion: CALENDAR_LIVE_V23_BROKER_VERSION,
...
export function acceptCalendarLiveV23Response<T = unknown>(
  directory: string,
  phase: CalendarLiveV23RemotePhase,
  options: {
    captureIndex?: number;
...
): CalendarLiveV23AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Calendar live v23 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<CalendarLiveV23Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Calendar live v23 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "calendar-live-v23-plugin-response-v2" ||
    plugin?.brokerVersion !== CALENDAR_LIVE_V23_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Calendar live v23 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistCalendarLiveV23TechnicalGates(
  directory: string,
  rootProofs: readonly CalendarLiveV23RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertCalendarLiveV23RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Calendar live v23 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Calendar live v23 probe payload hash malformed");
  const gates = Object.fromEntries(
    CALENDAR_LIVE_V23_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "calendar-live-v23-host-technical-gates-v2",
    brokerVersion: CALENDAR_LIVE_V23_BROKER_VERSION,
...
export function validateCalendarLiveV23TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "calendar-live-v23-host-technical-gates-v2" ||
    ...
    artifact.roots.length !== CALENDAR_LIVE_V23_SOURCE_ROOTS ||
    ...
    CALENDAR_LIVE_V23_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Calendar live v23 technical gates artifact");
}

export function validateCalendarLiveV23Receipt(
  directory: string,
  phase: CalendarLiveV23RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateCalendarLiveV23Request(directory, request);
  const receipt = json<CalendarLiveV23Receipt>(
  ...
  if (
    receipt.artifactVersion !== "calendar-live-v23-broker-receipt-v2" ||
    receipt.brokerVersion !== CALENDAR_LIVE_V23_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Calendar live v23 accepted receipt: ${request.requestId}`,
    );
}

export function readCalendarLiveV23AcceptedRawPayload(
  directory: string,
  phase: CalendarLiveV23RemotePhase,
  captureIndex?: number,
): unknown {
  validateCalendarLiveV23Receipt(directory, phase, captureIndex);
...
export function createCalendarLiveV23FakeDynamicResponse(
  request: CalendarLiveV23Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? CALENDAR_LIVE_V23_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? CALENDAR_LIVE_V23_TARGET,
    ...overrides.extra,
  };
}

export function listCalendarLiveV23PersistedRequests(
  directory: string,
): CalendarLiveV23Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<CalendarLiveV23Request>(file)] : [];
    });
}
