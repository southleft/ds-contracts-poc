...
import {
  verifyTableLiveV38FixedSceneFixedPoint,
  type TableLiveV38FixedFixedPoint,
} from "./table-live-v38-fixed-point.js";

export const TABLE_LIVE_V38_VERIFIER_VERSION = "table-live-v38-verifier-v1";

export function buildTableLiveV38RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV38RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV38RawNode[];
}

export interface TableLiveV38NormalizedScene {
...
export interface TableLiveV38SceneProof {
...
export function normalizeTableLiveV38Scene(
  root: TableLiveV38RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV38NormalizedScene {
  const canonicalBindings: TableLiveV38NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV38RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV38Scene<Instance>(
  normalized: TableLiveV38NormalizedScene,
...
): TableLiveV38SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV38FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV38ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV38PreCaptureGates(payload: unknown): void {
