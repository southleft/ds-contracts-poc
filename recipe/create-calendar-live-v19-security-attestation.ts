...
import { verifyCalendarLiveV19Authorization } from "./calendar-live-v19-authorization.js";
import { CALENDAR_LIVE_V19_TARGET } from "./calendar-live-v19-broker.js";
import {
  CALENDAR_LIVE_V19_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV19Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v19-session";
...
const attestation: CalendarLiveV19SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV19SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v19 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V19_SECURITY_ATTESTATION_DEFAULT_PATH,
);
