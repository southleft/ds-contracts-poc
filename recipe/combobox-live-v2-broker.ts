...
import {
  assertComboboxLiveV2RootProofs,
  COMBOBOX_LIVE_V2_CAPTURE_COUNT,
  COMBOBOX_LIVE_V2_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  COMBOBOX_LIVE_V2_RESPONSE_CONTRACTS,
  type ComboboxLiveV2RootProof,
} from "./combobox-live-v2-contract.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V2_BROKER_VERSION = "combobox-live-v2-broker-v2";
export const COMBOBOX_LIVE_V2_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const COMBOBOX_LIVE_V2_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const COMBOBOX_LIVE_V2_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const COMBOBOX_LIVE_V2_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const COMBOBOX_LIVE_V2_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const COMBOBOX_LIVE_V2_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type ComboboxLiveV2RemotePhase =
  (typeof COMBOBOX_LIVE_V2_REMOTE_PHASES)[number];
export type ComboboxLiveV2AuthorizationMode = "live" | "simulated";

export interface ComboboxLiveV2CommitPins {
...
export interface ComboboxLiveV2TransactionAuthorization extends ComboboxLiveV2CommitPins {
  mode: ComboboxLiveV2AuthorizationMode;
...
export interface ComboboxLiveV2Manifest {
  artifactVersion: "combobox-live-v2-broker-manifest-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V2_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V2_DYNAMIC_TOOL;
  target: typeof COMBOBOX_LIVE_V2_TARGET;
  authorization: ComboboxLiveV2TransactionAuthorization;
...
export interface ComboboxLiveV2ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface ComboboxLiveV2RequestBody {
  artifactVersion: "combobox-live-v2-broker-request-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V2_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: ComboboxLiveV2RemotePhase;
  sequence: number;
  ...
  commitPins: ComboboxLiveV2CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof COMBOBOX_LIVE_V2_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: ComboboxLiveV2ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface ComboboxLiveV2Request extends ComboboxLiveV2RequestBody {
...
export interface ComboboxLiveV2Receipt {
  artifactVersion: "combobox-live-v2-broker-receipt-v2";
  brokerVersion: typeof COMBOBOX_LIVE_V2_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: ComboboxLiveV2RemotePhase;
...
export interface ComboboxLiveV2AcceptedResponse<T = unknown> {
  request: ComboboxLiveV2Request;
  receipt: ComboboxLiveV2Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface ComboboxLiveV2IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: ComboboxLiveV2ResponseContract;
...
const equalTarget = (value: unknown): value is typeof COMBOBOX_LIVE_V2_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === COMBOBOX_LIVE_V2_TARGET.fileKey &&
  (value as BrokerTarget).fileName === COMBOBOX_LIVE_V2_TARGET.fileName &&
  (value as BrokerTarget).editorType === COMBOBOX_LIVE_V2_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== COMBOBOX_LIVE_V2_TARGET.fileKey ||
    context.fileName !== COMBOBOX_LIVE_V2_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === COMBOBOX_LIVE_V2_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Combobox live v1 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= COMBOBOX_LIVE_V2_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v1 capture index must be 0..71");
  return `capture-${String(captureIndex).padStart(3, "0")}`;
};

export const comboboxLiveV2RequestSequence = (
  phase: ComboboxLiveV2RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(comboboxLiveV2RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const comboboxLiveV2RequestPath = (
  directory: string,
  phase: ComboboxLiveV2RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const comboboxLiveV2ResponsePath = (
  directory: string,
  phase: ComboboxLiveV2RemotePhase,
  captureIndex?: number,
): string =>
...
export const comboboxLiveV2ReceiptPath = (
  directory: string,
  phase: ComboboxLiveV2RemotePhase,
  captureIndex?: number,
): string =>
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Combobox live v1 authorization pins are malformed");
};

const readManifest = (directory: string): ComboboxLiveV2Manifest => {
  const manifest = json<ComboboxLiveV2Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Combobox live v1 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "combobox-live-v2-broker-manifest-v2" ||
    manifest.brokerVersion !== COMBOBOX_LIVE_V2_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== COMBOBOX_LIVE_V2_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(COMBOBOX_LIVE_V2_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Combobox live v1 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: ComboboxLiveV2Request,
): ComboboxLiveV2RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: ComboboxLiveV2RemotePhase,
  captureIndex?: number,
): ComboboxLiveV2Request | undefined => {
  const file = comboboxLiveV2RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<ComboboxLiveV2Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: ComboboxLiveV2RemotePhase,
  captureIndex?: number,
): ComboboxLiveV2Receipt | undefined => {
  const file = comboboxLiveV2ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<ComboboxLiveV2Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: ComboboxLiveV2Request | undefined;
  receipt: ComboboxLiveV2Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: ComboboxLiveV2RemotePhase,
): ComboboxLiveV2ResponseContract => ({
  ...COMBOBOX_LIVE_V2_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: ComboboxLiveV2Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: COMBOBOX_LIVE_V2_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: COMBOBOX_LIVE_V2_DYNAMIC_TOOL,
      target: COMBOBOX_LIVE_V2_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "combobox-live-v2-plugin-response-v2",
  brokerVersion: COMBOBOX_LIVE_V2_BROKER_VERSION,
  target: COMBOBOX_LIVE_V2_TARGET,
  ...request,
})};
if(figma.fileKey!==__broker.target.fileKey||figma.root.name!==__broker.target.fileName||figma.editorType!==__broker.target.editorType)throw new Error("COMBOBOX-LIVE-V1-WRONG-TARGET");
const payload=await (async()=>{/*COMBOBOX_LIVE_V2_PROGRAM_START*/${program}/*COMBOBOX_LIVE_V2_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*COMBOBOX_LIVE_V2_PROGRAM_START*/";
  const endMarker = "/*COMBOBOX_LIVE_V2_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Combobox live v1 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createComboboxLiveV2Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Combobox live v1 private signing key does not match authorized public-key identity",
    );
  const manifest: ComboboxLiveV2Manifest = {
    artifactVersion: "combobox-live-v2-broker-manifest-v2",
    brokerVersion: COMBOBOX_LIVE_V2_BROKER_VERSION,
    ...
    captureRequests: COMBOBOX_LIVE_V2_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateComboboxLiveV2Request(
  directory: string,
  request: ComboboxLiveV2Request,
): void {
  ...
  const expectedSequence = comboboxLiveV2RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "combobox-live-v2-broker-request-v2" ||
    request.brokerVersion !== COMBOBOX_LIVE_V2_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== COMBOBOX_LIVE_V2_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Combobox live v1 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueComboboxLiveV2Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: ComboboxLiveV2RemotePhase,
  program: string,
  options: ComboboxLiveV2IssueOptions = {},
): ComboboxLiveV2Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = comboboxLiveV2RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<ComboboxLiveV2Request>(targetPath);
    validateComboboxLiveV2Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Combobox live v1 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(comboboxLiveV2ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Combobox live v1 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Combobox live v1 empty code envelope refused");
  if (phase === "capture") validateComboboxLiveV2TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Combobox live v1 out-of-order phase: ${id}`);
  const expectedResponse =
    options.expectedResponse ?? responseContractFor(phase);
  const programSha256 = sha256(program);
  const sequence = comboboxLiveV2RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: COMBOBOX_LIVE_V2_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? COMBOBOX_LIVE_V2_SIGNED_WRITER_TIMEOUT_MS
        : COMBOBOX_LIVE_V2_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: ComboboxLiveV2RequestBody = {
    artifactVersion: "combobox-live-v2-broker-request-v2",
    brokerVersion: COMBOBOX_LIVE_V2_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: ComboboxLiveV2Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateComboboxLiveV2Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistComboboxLiveV2RawResponse(
  directory: string,
  phase: ComboboxLiveV2RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = comboboxLiveV2RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Combobox live v1 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<ComboboxLiveV2Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Combobox live v1 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = comboboxLiveV2ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Combobox live v1 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): ComboboxLiveV2Receipt => {
  const body: Omit<ComboboxLiveV2Receipt, "receiptSha256"> = {
    artifactVersion: "combobox-live-v2-broker-receipt-v2",
    brokerVersion: COMBOBOX_LIVE_V2_BROKER_VERSION,
...
export function acceptComboboxLiveV2Response<T = unknown>(
  directory: string,
  phase: ComboboxLiveV2RemotePhase,
  options: {
    captureIndex?: number;
...
): ComboboxLiveV2AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = comboboxLiveV2ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Combobox live v1 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<ComboboxLiveV2Request>(
    comboboxLiveV2RequestPath(directory, phase, captureIndex),
  );
  validateComboboxLiveV2Request(directory, request);
  if (phase === "capture") validateComboboxLiveV2TechnicalGates(directory);
  const rawFile = comboboxLiveV2ResponsePath(directory, phase, captureIndex);
  if (!existsSync(rawFile))
    throw new Error(`Combobox live v1 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "combobox-live-v2-plugin-response-v2" ||
    plugin?.brokerVersion !== COMBOBOX_LIVE_V2_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Combobox live v1 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistComboboxLiveV2TechnicalGates(
  directory: string,
  rootProofs: readonly ComboboxLiveV2RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertComboboxLiveV2RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Combobox live v1 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Combobox live v1 probe payload hash malformed");
  const gates = Object.fromEntries(
    COMBOBOX_LIVE_V2_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "combobox-live-v2-host-technical-gates-v2",
    brokerVersion: COMBOBOX_LIVE_V2_BROKER_VERSION,
...
export function validateComboboxLiveV2TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "combobox-live-v2-host-technical-gates-v2" ||
    ...
    COMBOBOX_LIVE_V2_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Combobox live v1 technical gates artifact");
}

export function validateComboboxLiveV2Receipt(
  directory: string,
  phase: ComboboxLiveV2RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateComboboxLiveV2Request(directory, request);
  const receipt = json<ComboboxLiveV2Receipt>(
    comboboxLiveV2ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    comboboxLiveV2ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "combobox-live-v2-broker-receipt-v2" ||
    receipt.brokerVersion !== COMBOBOX_LIVE_V2_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Combobox live v1 accepted receipt: ${request.requestId}`,
    );
}

export function readComboboxLiveV2AcceptedRawPayload(
  directory: string,
  phase: ComboboxLiveV2RemotePhase,
  captureIndex?: number,
): unknown {
  validateComboboxLiveV2Receipt(directory, phase, captureIndex);
...
export function createComboboxLiveV2FakeDynamicResponse(
  request: ComboboxLiveV2Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? COMBOBOX_LIVE_V2_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? COMBOBOX_LIVE_V2_TARGET,
    ...overrides.extra,
  };
}

export function listComboboxLiveV2PersistedRequests(
  directory: string,
): ComboboxLiveV2Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<ComboboxLiveV2Request>(file)] : [];
    });
}

export const comboboxLiveV2Sha256 = sha256;
