...
import {
  ...
  INPUT_LIVE_V17_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V17_RESPONSE_CONTRACTS,
  type InputLiveV17RootProof,
} from "./input-field-live-v17-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V17_BROKER_VERSION = "input-live-v17-broker-v2";
export const INPUT_LIVE_V17_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V17_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V17_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V17_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V17_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V17_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV17RemotePhase =
  (typeof INPUT_LIVE_V17_REMOTE_PHASES)[number];
export type InputLiveV17AuthorizationMode = "live" | "simulated";

export interface InputLiveV17CommitPins {
...
export interface InputLiveV17TransactionAuthorization extends InputLiveV17CommitPins {
  mode: InputLiveV17AuthorizationMode;
...
export interface InputLiveV17Manifest {
  artifactVersion: "input-live-v17-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V17_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V17_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V17_TARGET;
...
export interface InputLiveV17ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV17RequestBody {
  artifactVersion: "input-live-v17-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V17_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV17RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV17CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V17_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV17ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV17Request extends InputLiveV17RequestBody {
...
export interface InputLiveV17Receipt {
  artifactVersion: "input-live-v17-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V17_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV17RemotePhase;
...
export interface InputLiveV17AcceptedResponse<T = unknown> {
  request: InputLiveV17Request;
  receipt: InputLiveV17Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV17IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV17ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V17_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V17_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V17_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V17_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V17_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V17_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V17_TARGET.editorType;
};
...
export const inputLiveV17RequestSequence = (
  phase: InputLiveV17RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV17RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV17RequestPath = (
  directory: string,
  phase: InputLiveV17RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV17ResponsePath = (
  directory: string,
  phase: InputLiveV17RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV17ReceiptPath = (
  directory: string,
  phase: InputLiveV17RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV17Manifest => {
  const manifest = json<InputLiveV17Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v17-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V17_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V17_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV17RemotePhase,
  captureIndex?: number,
): InputLiveV17Request | undefined => {
  const file = inputLiveV17RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV17Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV17RemotePhase,
  captureIndex?: number,
): InputLiveV17Receipt | undefined => {
  const file = inputLiveV17ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV17Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV17Request | undefined;
  receipt: InputLiveV17Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV17RemotePhase,
): InputLiveV17ResponseContract => ({
  ...INPUT_LIVE_V17_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V17_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V17_DYNAMIC_TOOL,
      target: INPUT_LIVE_V17_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v17-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V17_BROKER_VERSION,
  target: INPUT_LIVE_V17_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V17_PROGRAM_START*/${program}/*INPUT_LIVE_V17_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V17_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V17_PROGRAM_END*/";
...
export function createInputLiveV17Transaction(
  directory: string,
  ...
  const manifest: InputLiveV17Manifest = {
    artifactVersion: "input-live-v17-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V17_BROKER_VERSION,
...
export function validateInputLiveV17Request(
  directory: string,
  request: InputLiveV17Request,
): void {
  ...
  const expectedSequence = inputLiveV17RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v17-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V17_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V17_TARGET.fileKey ||
...
export function issueInputLiveV17Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV17RemotePhase,
  program: string,
  options: InputLiveV17IssueOptions = {},
): InputLiveV17Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV17RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV17Request>(targetPath);
    validateInputLiveV17Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV17ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV17TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV17RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V17_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V17_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V17_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV17RequestBody = {
    artifactVersion: "input-live-v17-broker-request-v2",
    brokerVersion: INPUT_LIVE_V17_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV17Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV17Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV17RawResponse(
  directory: string,
  phase: InputLiveV17RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV17RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV17Request>(requestFile);
  if (
  ...
  const target = inputLiveV17ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV17Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v17-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V17_BROKER_VERSION,
...
export function acceptInputLiveV17Response<T = unknown>(
  directory: string,
  phase: InputLiveV17RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV17AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV17ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV17Request>(
    inputLiveV17RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV17Request(directory, request);
  if (phase === "capture") validateInputLiveV17TechnicalGates(directory);
  const rawFile = inputLiveV17ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v17-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V17_BROKER_VERSION ||
...
export function persistInputLiveV17TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV17RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV17RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V17_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v17-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V17_BROKER_VERSION,
...
export function validateInputLiveV17TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v17-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V17_TECHNICAL_GATES.some(
...
export function validateInputLiveV17Receipt(
  directory: string,
  phase: InputLiveV17RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV17Request(directory, request);
  const receipt = json<InputLiveV17Receipt>(
    inputLiveV17ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV17ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v17-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V17_BROKER_VERSION ||
...
export function readInputLiveV17AcceptedRawPayload(
  directory: string,
  phase: InputLiveV17RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV17Receipt(directory, phase, captureIndex);
...
export function createInputLiveV17FakeDynamicResponse(
  request: InputLiveV17Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V17_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V17_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV17PersistedRequests(
  directory: string,
): InputLiveV17Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV17Request>(file)] : [];
    });
}

export const inputLiveV17Sha256 = sha256;
