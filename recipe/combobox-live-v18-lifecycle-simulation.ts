...
export interface ComboboxLiveV18LifecycleSimulationReport {
  artifactVersion: "combobox-live-v18-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV18AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV18Lifecycle(): Promise<ComboboxLiveV18LifecycleSimulationReport> {
  await buildComboboxLiveV18Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V18_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV18AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v18-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V18_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V18_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV18History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV18History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV18AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V18_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V18_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV18History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV18History(authorized, "pending");
    ...
    await buildComboboxLiveV18Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V18_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV18AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV18Lifecycle(), null, 2)}\n`,
  );
