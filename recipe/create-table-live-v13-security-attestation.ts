...
import { TABLE_LIVE_V13_TARGET } from "./table-live-v13-broker.js";
import {
  TABLE_LIVE_V13_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyTableLiveV13Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-table-v13-session";
...
const attestation: TableLiveV13SecurityAttestation = {
  ...body,
  attestationSha256: tableLiveV2AttestationSha256(body),
};
const failures = validateTableLiveV13SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Table live v13 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? TABLE_LIVE_V13_SECURITY_ATTESTATION_DEFAULT_PATH,
);
