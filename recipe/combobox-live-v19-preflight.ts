...
import {
  validateComboboxLiveV19AntecedentIndex,
  verifyComboboxLiveV19Authorization,
  type ComboboxLiveV19AntecedentIndex,
  type ComboboxLiveV19AuthorizationProof,
} from "./combobox-live-v19-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V19_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v19/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V19_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v19-security-attestation.json";

export interface ComboboxLiveV19SecurityAttestation {
  artifactVersion: "combobox-live-v19-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV19PreflightReport {
  artifactVersion: "combobox-live-v19-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV19SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV19SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV19AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v19-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V19_TARGET) ||
...
export function validateComboboxLiveV19ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV19Authorization");
  const preflight = source.indexOf("runComboboxLiveV19Preflight");
  const initialize = source.indexOf("ComboboxLiveV19Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV19Preflight(
  root: string,
  proof: ComboboxLiveV19AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V19_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV19PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V19_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V19_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V19_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV19AntecedentIndex;
  failures.push(...validateComboboxLiveV19AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V19_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V19_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV19SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v19-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV19ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v19 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV19SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v19-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V19_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV19Preflight(
    process.cwd(),
    verifyComboboxLiveV19Authorization(),
    attestationPath,
