...
} from "./input-field-live-v3-verifier-v71.js";
import {
  buildInputLiveV71RawPropertyRuntime,
  normalizeInputLiveV71Scene,
  type InputLiveV71RawNode,
} from "./input-field-live-v71-verifier.js";
...
export {
  ...
  type InputLiveV71RestorePayload,
} from "./input-field-live-v71-restore.js";

export const INPUT_LIVE_V71_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V71_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V71_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V71_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V71_VARIANT_COUNT = 256;
export const INPUT_LIVE_V71_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V71_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV71SourceId = (typeof INPUT_LIVE_V71_SOURCE_IDS)[number];

export interface InputLiveV71SourceIdentity {
  source: InputLiveV71SourceId;
...
export interface InputLiveV71WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV71ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV71SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV71RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV71RootProof {
  source: InputLiveV71SourceId;
  adapterIdentity: string;
...
export interface InputLiveV71ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV71SourceId;
    adapterIdentity: string;
...
export interface InputLiveV71CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV71SourceId;
  adapterIdentity: string;
...
export interface InputLiveV71CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV71SourceId;
  strata: InputLiveV71CaptureCell["strata"];
...
export interface InputLiveV71CleanupPayload {
...
export function validateInputLiveV71WriterPayload(
  value: unknown,
): InputLiveV71WriterOwnership {
...
const identityPlan = (source: InputLiveV71SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV71ExtractProgram(
  writer: InputLiveV71WriterOwnership,
  sources: readonly InputLiveV71SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V71_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V71_NAMESPACE)}
${buildInputLiveV71RawPropertyRuntime()}
...
export function validateInputLiveV71ExtractPayload(
  value: unknown,
  writer: InputLiveV71WriterOwnership,
): InputLiveV71ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V71_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV71ExtractPayload;
}

export function proveInputLiveV71Roots<Instance>(
  extract: InputLiveV71ExtractPayload,
  sources: readonly (InputLiveV71SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV71Scene(
      root.scene,
...
export function assertInputLiveV71RootProofs(
  proofs: readonly InputLiveV71RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV71ProbeProgram(
  writer: InputLiveV71WriterOwnership,
  sources: readonly InputLiveV71SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V71_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV71ProbePayload(
  value: unknown,
  writer: InputLiveV71WriterOwnership,
): InputLiveV71ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV71ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V71_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V71_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV71CaptureManifest(
  cells: readonly InputLiveV71CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V71_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V71_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V71_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V71_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V71_SOURCE_IDS.some(
...
export const inputLiveV71CaptureManifestSha256 = (
  cells: readonly InputLiveV71CaptureCell[],
): string => {
  validateInputLiveV71CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV71CaptureProgram(
  writer: InputLiveV71WriterOwnership,
  cell: InputLiveV71CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V71_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V71_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V71_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV71CapturePayload(
  value: unknown,
  cell: InputLiveV71CaptureCell,
  rawResponseBytes: number,
): InputLiveV71CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV71CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V71_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V71_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV71CaptureResponses(
  manifest: readonly InputLiveV71CaptureCell[],
  responses: readonly InputLiveV71CapturePayload[],
): void {
  validateInputLiveV71CaptureManifest(manifest);
  if (
...
export interface InputLiveV71ObjectiveReport {
  artifactVersion: "input-live-v71-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV71Objective(
  manifest: readonly InputLiveV71CaptureCell[],
  responses: readonly InputLiveV71CapturePayload[],
  readReference: (cell: InputLiveV71CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV71CaptureCell) => Buffer,
): InputLiveV71ObjectiveReport {
  assertInputLiveV71CaptureResponses(manifest, responses);
...
export function buildInputLiveV71CleanupProgram(
  writer: InputLiveV71WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V71_NAMESPACE,
...
export function validateInputLiveV71CleanupPayload(
  value: unknown,
  writer: InputLiveV71WriterOwnership,
): InputLiveV71CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV71CleanupPayload;
...
export const INPUT_LIVE_V71_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV71WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
