...
import { buildComboboxLiveV22CleanupRuntime } from "./combobox-live-v22-cleanup.js";
import {
  type ComboboxLiveV22FixedFixedPoint,
} from "./combobox-live-v22-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV22RestoreProgram,
  validateComboboxLiveV22RestorePayload,
  type ComboboxLiveV22RestorePayload,
} from "./combobox-live-v22-restore.js";
import {
  buildComboboxLiveV22RawPropertyRuntime,
  normalizeComboboxLiveV22Scene,
  type ComboboxLiveV22RawNode,
} from "./combobox-live-v22-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v22.js";
import {
...
} from "./scene-readback-combobox-v22.js";
...
export const COMBOBOX_LIVE_V22_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V22_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V22_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V22_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V22_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V22_SET_COUNT = 4;
export const COMBOBOX_LIVE_V22_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V22_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V22_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V22_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V22_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV22SourceId = (typeof COMBOBOX_LIVE_V22_SOURCE_IDS)[number];

export interface ComboboxLiveV22SourceIdentity {
  source: ComboboxLiveV22SourceId;
...
export interface ComboboxLiveV22WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV22ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV22SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV22RawNode;
    optionScene: ComboboxLiveV22RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV22RootProof {
  source: ComboboxLiveV22SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV22FixedFixedPoint;
}

export interface ComboboxLiveV22ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV22SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV22CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV22SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV22CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV22SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV22CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v22 ${label} must be an object`);
...
export function validateComboboxLiveV22WriterPayload(
  value: unknown,
): ComboboxLiveV22WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V22_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v22 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v22 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v22 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV22SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV22ExtractProgram(
  writer: ComboboxLiveV22WriterOwnership,
  sources: readonly ComboboxLiveV22SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v22 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V22_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V22_NAMESPACE)}
${buildComboboxLiveV22RawPropertyRuntime()}
...
export function validateComboboxLiveV22ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV22WriterOwnership,
): ComboboxLiveV22ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v22 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V22_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v22 extract schema/two-root mismatch");
  return payload as ComboboxLiveV22ExtractPayload;
}

export function proveComboboxLiveV22Roots<Instance>(
  extract: ComboboxLiveV22ExtractPayload,
  sources: readonly (ComboboxLiveV22SourceIdentity & {
...
): ComboboxLiveV22RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v22 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v22 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV22Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV22Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV22FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v22 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v22 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v22 compile lost owned sets");
...
export function assertComboboxLiveV22RootProofs(
  proofs: readonly ComboboxLiveV22RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v22 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v22 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV22ProbeProgram(
  writer: ComboboxLiveV22WriterOwnership,
  sources: readonly ComboboxLiveV22SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V22_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV22ProbePayload(
  value: unknown,
  writer: ComboboxLiveV22WriterOwnership,
): ComboboxLiveV22ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV22ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V22_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V22_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v22 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV22CaptureManifest(
  cells: readonly ComboboxLiveV22CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V22_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V22_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V22_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V22_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v22 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV22CaptureCell[],
): string => {
  validateComboboxLiveV22CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV22CaptureProgram(
  writer: ComboboxLiveV22WriterOwnership,
  cell: ComboboxLiveV22CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V22_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v22 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v22 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V22_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV22CapturePayload(
  value: unknown,
  cell: ComboboxLiveV22CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV22CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV22CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V22_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V22_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v22 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV22CaptureResponses(
  manifest: readonly ComboboxLiveV22CaptureCell[],
  responses: readonly ComboboxLiveV22CapturePayload[],
): void {
  validateComboboxLiveV22CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v22 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV22ObjectiveReport {
  artifactVersion: "combobox-live-v22-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV22Objective(
  manifest: readonly ComboboxLiveV22CaptureCell[],
  responses: readonly ComboboxLiveV22CapturePayload[],
): ComboboxLiveV22ObjectiveReport {
  assertComboboxLiveV22CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v22-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV22CleanupProgram(
  writer: ComboboxLiveV22WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV22CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V22_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v22 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV22CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV22WriterOwnership,
): ComboboxLiveV22CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV22CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v22 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V22_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV22WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
