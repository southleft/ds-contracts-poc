...
} from "./input-field-live-v3-verifier-v17.js";
import {
  buildInputLiveV17RawPropertyRuntime,
  normalizeInputLiveV17Scene,
  type InputLiveV17RawNode,
} from "./input-field-live-v17-verifier.js";
...
export {
  ...
  type InputLiveV17RestorePayload,
} from "./input-field-live-v17-restore.js";

export const INPUT_LIVE_V17_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V17_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V17_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V17_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V17_VARIANT_COUNT = 256;
export const INPUT_LIVE_V17_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V17_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV17SourceId = (typeof INPUT_LIVE_V17_SOURCE_IDS)[number];

export interface InputLiveV17SourceIdentity {
  source: InputLiveV17SourceId;
...
export interface InputLiveV17WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV17ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV17SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV17RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV17RootProof {
  source: InputLiveV17SourceId;
  adapterIdentity: string;
...
export interface InputLiveV17ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV17SourceId;
    adapterIdentity: string;
...
export interface InputLiveV17CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV17SourceId;
  adapterIdentity: string;
...
export interface InputLiveV17CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV17SourceId;
  strata: InputLiveV17CaptureCell["strata"];
...
export interface InputLiveV17CleanupPayload {
...
export function validateInputLiveV17WriterPayload(
  value: unknown,
): InputLiveV17WriterOwnership {
...
const identityPlan = (source: InputLiveV17SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV17ExtractProgram(
  writer: InputLiveV17WriterOwnership,
  sources: readonly InputLiveV17SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V17_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V17_NAMESPACE)}
${buildInputLiveV17RawPropertyRuntime()}
...
export function validateInputLiveV17ExtractPayload(
  value: unknown,
  writer: InputLiveV17WriterOwnership,
): InputLiveV17ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V17_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV17ExtractPayload;
}

export function proveInputLiveV17Roots<Instance>(
  extract: InputLiveV17ExtractPayload,
  sources: readonly (InputLiveV17SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV17Scene(
      root.scene,
...
export function assertInputLiveV17RootProofs(
  proofs: readonly InputLiveV17RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV17ProbeProgram(
  writer: InputLiveV17WriterOwnership,
  sources: readonly InputLiveV17SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V17_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV17ProbePayload(
  value: unknown,
  writer: InputLiveV17WriterOwnership,
): InputLiveV17ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV17ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V17_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V17_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV17CaptureManifest(
  cells: readonly InputLiveV17CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V17_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V17_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V17_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V17_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V17_SOURCE_IDS.some(
...
export const inputLiveV17CaptureManifestSha256 = (
  cells: readonly InputLiveV17CaptureCell[],
): string => {
  validateInputLiveV17CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV17CaptureProgram(
  writer: InputLiveV17WriterOwnership,
  cell: InputLiveV17CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V17_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V17_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V17_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV17CapturePayload(
  value: unknown,
  cell: InputLiveV17CaptureCell,
  rawResponseBytes: number,
): InputLiveV17CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV17CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V17_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V17_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV17CaptureResponses(
  manifest: readonly InputLiveV17CaptureCell[],
  responses: readonly InputLiveV17CapturePayload[],
): void {
  validateInputLiveV17CaptureManifest(manifest);
  if (
...
export interface InputLiveV17ObjectiveReport {
  artifactVersion: "input-live-v17-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV17Objective(
  manifest: readonly InputLiveV17CaptureCell[],
  responses: readonly InputLiveV17CapturePayload[],
  readReference: (cell: InputLiveV17CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV17CaptureCell) => Buffer,
): InputLiveV17ObjectiveReport {
  assertInputLiveV17CaptureResponses(manifest, responses);
...
export function buildInputLiveV17CleanupProgram(
  writer: InputLiveV17WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V17_NAMESPACE,
...
export function validateInputLiveV17CleanupPayload(
  value: unknown,
  writer: InputLiveV17WriterOwnership,
): InputLiveV17CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV17CleanupPayload;
...
export const INPUT_LIVE_V17_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV17WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
