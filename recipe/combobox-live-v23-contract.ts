...
import { buildComboboxLiveV23CleanupRuntime } from "./combobox-live-v23-cleanup.js";
import {
  type ComboboxLiveV23FixedFixedPoint,
} from "./combobox-live-v23-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV23RestoreProgram,
  validateComboboxLiveV23RestorePayload,
  type ComboboxLiveV23RestorePayload,
} from "./combobox-live-v23-restore.js";
import {
  buildComboboxLiveV23RawPropertyRuntime,
  normalizeComboboxLiveV23Scene,
  type ComboboxLiveV23RawNode,
} from "./combobox-live-v23-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v23.js";
import {
...
export const COMBOBOX_LIVE_V23_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V23_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V23_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V23_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V23_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V23_SET_COUNT = 4;
export const COMBOBOX_LIVE_V23_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V23_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V23_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V23_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V23_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV23SourceId = (typeof COMBOBOX_LIVE_V23_SOURCE_IDS)[number];

export interface ComboboxLiveV23SourceIdentity {
  source: ComboboxLiveV23SourceId;
...
export interface ComboboxLiveV23WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV23ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV23SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV23RawNode;
    optionScene: ComboboxLiveV23RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV23RootProof {
  source: ComboboxLiveV23SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV23FixedFixedPoint;
}

export interface ComboboxLiveV23ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV23SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV23CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV23SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV23CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV23SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV23CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v23 ${label} must be an object`);
...
export function validateComboboxLiveV23WriterPayload(
  value: unknown,
): ComboboxLiveV23WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V23_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v23 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v23 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v23 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV23SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV23ExtractProgram(
  writer: ComboboxLiveV23WriterOwnership,
  sources: readonly ComboboxLiveV23SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v23 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V23_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V23_NAMESPACE)}
${buildComboboxLiveV23RawPropertyRuntime()}
...
export function validateComboboxLiveV23ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV23WriterOwnership,
): ComboboxLiveV23ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v23 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V23_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v23 extract schema/two-root mismatch");
  return payload as ComboboxLiveV23ExtractPayload;
}

export function proveComboboxLiveV23Roots<Instance>(
  extract: ComboboxLiveV23ExtractPayload,
  sources: readonly (ComboboxLiveV23SourceIdentity & {
...
): ComboboxLiveV23RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v23 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v23 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV23Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV23Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV23FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v23 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v23 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v23 compile lost owned sets");
...
export function assertComboboxLiveV23RootProofs(
  proofs: readonly ComboboxLiveV23RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v23 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v23 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV23ProbeProgram(
  writer: ComboboxLiveV23WriterOwnership,
  sources: readonly ComboboxLiveV23SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V23_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV23ProbePayload(
  value: unknown,
  writer: ComboboxLiveV23WriterOwnership,
): ComboboxLiveV23ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV23ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V23_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V23_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v23 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV23CaptureManifest(
  cells: readonly ComboboxLiveV23CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V23_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V23_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V23_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V23_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v23 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV23CaptureCell[],
): string => {
  validateComboboxLiveV23CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV23CaptureProgram(
  writer: ComboboxLiveV23WriterOwnership,
  cell: ComboboxLiveV23CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V23_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v23 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v23 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V23_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV23CapturePayload(
  value: unknown,
  cell: ComboboxLiveV23CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV23CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV23CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V23_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V23_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v23 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV23CaptureResponses(
  manifest: readonly ComboboxLiveV23CaptureCell[],
  responses: readonly ComboboxLiveV23CapturePayload[],
): void {
  validateComboboxLiveV23CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v23 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV23ObjectiveReport {
  artifactVersion: "combobox-live-v23-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV23Objective(
  manifest: readonly ComboboxLiveV23CaptureCell[],
  responses: readonly ComboboxLiveV23CapturePayload[],
): ComboboxLiveV23ObjectiveReport {
  assertComboboxLiveV23CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v23-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV23CleanupProgram(
  writer: ComboboxLiveV23WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV23CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V23_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v23 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV23CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV23WriterOwnership,
): ComboboxLiveV23CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV23CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v23 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V23_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV23WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
