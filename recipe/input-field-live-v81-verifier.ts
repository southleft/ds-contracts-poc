...
export const INPUT_LIVE_V81_VERIFIER_VERSION = "input-live-v81-verifier-v1";

export function buildInputLiveV81RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV81RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV81RawNode[];
}

export interface InputLiveV81NormalizedScene {
...
export interface InputLiveV81SceneProof {
...
export function normalizeInputLiveV81Scene(
  root: InputLiveV81RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV81NormalizedScene {
  const canonicalBindings: InputLiveV81NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV81RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV81Scene<Instance>(
  normalized: InputLiveV81NormalizedScene,
...
export function compileInputLiveV81ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV81PreCaptureGates(payload: unknown): void {
