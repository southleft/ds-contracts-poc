...
import {
  acceptTableLiveV23Response,
  createTableLiveV23Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV23Request,
  persistTableLiveV23RawResponse,
  persistTableLiveV23TechnicalGates,
  readTableLiveV23AcceptedRawPayload,
  type TableLiveV23Request,
  type TableLiveV23TransactionAuthorization,
} from "./table-live-v23-broker.js";
import {
  TABLE_LIVE_V23_CAPTURE_COUNT,
  assertTableLiveV23CaptureResponses,
  ...
  evaluateTableLiveV23Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV23Roots,
  validateTableLiveV23CaptureManifest,
  validateTableLiveV23CapturePayload,
  validateTableLiveV23CleanupPayload,
  validateTableLiveV23ExtractPayload,
  validateTableLiveV23ProbePayload,
  validateTableLiveV23WriterPayload,
  type TableLiveV23CaptureCell,
  type TableLiveV23CapturePayload,
...
import {
  ...
  TABLE_LIVE_V23_PLAN_PATH,
  type TableLiveV23ProofPlan,
} from "./build-table-live-proof-v23.js";
import {
...
export const TABLE_LIVE_V23_RUNNER_VERSION = "table-live-v23-runner-v1";

interface TableLiveV23JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV23OrchestratorState {
  artifactVersion: "table-live-v23-orchestrator-state-v1";
  ...
  journal: TableLiveV23JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV23ProofPlan;
  planSha256: string;
  captures: TableLiveV23CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV23SourceIdentity & {
...
export interface TableLiveV23NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV23OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v23 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V23_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V23_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV23ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV23CaptureCell[];
  validateTableLiveV23CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v23 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v23 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV23WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV23RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV23Request =>
  readJson<TableLiveV23Request>(file);

export class TableLiveV23Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV23Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV23OrchestratorState = {
      artifactVersion: "table-live-v23-orchestrator-state-v1",
    ...
    return new TableLiveV23Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV23OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV23Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV23NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV23NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v23 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV23NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v23 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV23RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV23Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV23CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV23AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV23WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV23Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV23ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV23Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV23Response(this.#directory, "writer", {
        validate: validateTableLiveV23WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV23Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV23RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV23Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV23ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV23Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV23RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v23-host-root-proofs-v1",
      ...
      const next = issueTableLiveV23Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV23ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV23Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV23ProbePayload(payload, ownership),
      });
      persistTableLiveV23TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v23 capture cell missing");
      const accepted = acceptTableLiveV23Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV23CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V23_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV23CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV23Objective(
    ...
    throw new Error(`Table live v23 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV23Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV23CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV23Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV23RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV23Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV23CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV23CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV23CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV23Authorization = (
  privateKey: KeyObject,
): TableLiveV23TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV23SimulationKey = (): KeyObject =>
