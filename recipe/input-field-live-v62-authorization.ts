...
import {
  ...
  INPUT_LIVE_V62_INDEX_PATH,
} from "./build-input-field-live-proof-v62.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV62HistoryExpectation = "pending" | "authorized";

export interface InputLiveV62AuthorizationArtifact {
  artifactVersion: "input-live-v62-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V62_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV62AntecedentIndex {
...
export interface InputLiveV62HistoryState {
  index?: InputLiveV62AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV62AuthorizationArtifact;
...
export interface InputLiveV62AuthorizationProof extends InputLiveV62TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V62_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V62_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V62_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V62_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v62-authorization") ||
  artifactPath.includes("input-field-live-v62-preflight") ||
  artifactPath.includes("input-field-live-v62-authorized") ||
  artifactPath.includes("input-field-live-pivot-v62-status") ||
...
export function validateInputLiveV62AntecedentIndex(
  index: InputLiveV62AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v62-antecedent-index-v1" ||
...
export function validateInputLiveV62UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV62History\(\s*["']/.test(source) ||
    /readInputLiveV62HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV62AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV62AuthorizationArtifact {
  ...
  ) as InputLiveV62AntecedentIndex;
  const indexFailures = validateInputLiveV62AntecedentIndex(index);
...
export function validateInputLiveV62AuthorizationArtifact(
  artifact: InputLiveV62AuthorizationArtifact | undefined,
  index: InputLiveV62AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v62-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v62" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V62_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V62_TARGET) ||
    !equalJson(
...
export function validateInputLiveV62History(
  state: InputLiveV62HistoryState,
  expected: InputLiveV62HistoryExpectation,
): string[] {
  const failures = validateInputLiveV62AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV62AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV62HistoryState(
  root = process.cwd(),
): InputLiveV62HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V62_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV62AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V62_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V62_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V62_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV62History(
  expected: InputLiveV62HistoryExpectation,
  root = process.cwd(),
): InputLiveV62HistoryState {
  const state = readInputLiveV62HistoryState(root);
  const failures = validateInputLiveV62History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV62Authorization(
  root = process.cwd(),
): InputLiveV62AuthorizationProof {
  const state = verifyInputLiveV62History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V62_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v62/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v62.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V62_TARGET,
    expectedDynamicTool: INPUT_LIVE_V62_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V62_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV62HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV62History(expected);
