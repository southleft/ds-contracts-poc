...
import { TABLE_LIVE_V22_AUTHORIZATION_PATH } from "./build-table-live-proof-v22.js";
import { verifyTableLiveV22Authorization } from "./table-live-v22-authorization.js";
import { runTableLiveV22Preflight } from "./table-live-v22-preflight.js";
import { TableLiveV22Orchestrator } from "./run-table-live-v22.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v22 requires exact authorization artifact ${TABLE_LIVE_V22_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v22-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV22Authorization(root);
    runTableLiveV22Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV22Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV22Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
