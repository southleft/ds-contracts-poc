...
import { INPUT_LIVE_V65_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v65.js";
import { verifyInputLiveV65Authorization } from "./input-field-live-v65-authorization.js";
import { runInputLiveV65Preflight } from "./input-field-live-v65-preflight.js";
import { InputLiveV65Orchestrator } from "./run-input-field-live-v65.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V65_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v65-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV65Authorization(root);
    runInputLiveV65Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV65Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
