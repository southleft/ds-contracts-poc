...
export interface CalendarLiveV2CleanupPlan {
  fileKey: string;
  fileName: string;
  editorType: string;
  namespace: string;
...
export function buildCalendarLiveV2CleanupRuntime(
  plan: CalendarLiveV2CleanupPlan,
): string {
  ...
  return String.raw`
if(figma.fileKey!==${JSON.stringify(plan.fileKey)}||figma.root.name!==${JSON.stringify(plan.fileName)}||figma.editorType!==${JSON.stringify(plan.editorType)})throw new Error("CALENDAR-V2-CLEANUP-WRONG-TARGET");
