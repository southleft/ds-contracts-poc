...
export type ComboboxLiveV9HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV9AuthorizationArtifact {
  artifactVersion: "combobox-live-v9-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V9_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV9AntecedentIndex {
...
export interface ComboboxLiveV9HistoryState {
  index?: ComboboxLiveV9AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV9AuthorizationArtifact;
...
export interface ComboboxLiveV9AuthorizationProof extends ComboboxLiveV9TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V9_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V9_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V9_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V9_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v9-authorization") ||
  artifactPath.includes("combobox-live-v9-preflight") ||
  artifactPath.includes("combobox-live-v9-authorized") ||
  artifactPath.includes("create-combobox-live-v9-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v9-status") ||
...
export function validateComboboxLiveV9AntecedentIndex(
  index: ComboboxLiveV9AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v9-antecedent-index-v1" ||
...
export function validateComboboxLiveV9UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV9History\(\s*["']/.test(source) ||
    /readComboboxLiveV9HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV9AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV9AuthorizationArtifact {
  ...
  const indexFailures = validateComboboxLiveV9AntecedentIndex(index);
...
export function validateComboboxLiveV9AuthorizationArtifact(
  artifact: ComboboxLiveV9AuthorizationArtifact | undefined,
  index: ComboboxLiveV9AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v9-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v9" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V9_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V9_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V9_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V9_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V9_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V9_SOURCE_ROOTS ||
...
export function validateComboboxLiveV9History(
  state: ComboboxLiveV9HistoryState,
  expected: ComboboxLiveV9HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV9AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV9AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV9HistoryState(
  root = process.cwd(),
): ComboboxLiveV9HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V9_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV9AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V9_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V9_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V9_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV9History(
  expected: ComboboxLiveV9HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV9HistoryState {
  const state = readComboboxLiveV9HistoryState(root);
  const failures = validateComboboxLiveV9History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v9 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV9Authorization(
  root = process.cwd(),
): ComboboxLiveV9AuthorizationProof {
  const state = verifyComboboxLiveV9History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V9_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v9/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v9.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V9_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V9_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V9_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV9HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV9History(expected);
  process.stdout.write(
    `Combobox live v9 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
