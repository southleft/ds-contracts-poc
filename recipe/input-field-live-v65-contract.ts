...
} from "./input-field-live-v3-verifier-v65.js";
import {
  buildInputLiveV65RawPropertyRuntime,
  normalizeInputLiveV65Scene,
  type InputLiveV65RawNode,
} from "./input-field-live-v65-verifier.js";
...
export {
  ...
  type InputLiveV65RestorePayload,
} from "./input-field-live-v65-restore.js";

export const INPUT_LIVE_V65_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V65_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V65_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V65_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V65_VARIANT_COUNT = 256;
export const INPUT_LIVE_V65_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V65_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV65SourceId = (typeof INPUT_LIVE_V65_SOURCE_IDS)[number];

export interface InputLiveV65SourceIdentity {
  source: InputLiveV65SourceId;
...
export interface InputLiveV65WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV65ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV65SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV65RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV65RootProof {
  source: InputLiveV65SourceId;
  adapterIdentity: string;
...
export interface InputLiveV65ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV65SourceId;
    adapterIdentity: string;
...
export interface InputLiveV65CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV65SourceId;
  adapterIdentity: string;
...
export interface InputLiveV65CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV65SourceId;
  strata: InputLiveV65CaptureCell["strata"];
...
export interface InputLiveV65CleanupPayload {
...
export function validateInputLiveV65WriterPayload(
  value: unknown,
): InputLiveV65WriterOwnership {
...
const identityPlan = (source: InputLiveV65SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV65ExtractProgram(
  writer: InputLiveV65WriterOwnership,
  sources: readonly InputLiveV65SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V65_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V65_NAMESPACE)}
${buildInputLiveV65RawPropertyRuntime()}
...
export function validateInputLiveV65ExtractPayload(
  value: unknown,
  writer: InputLiveV65WriterOwnership,
): InputLiveV65ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V65_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV65ExtractPayload;
}

export function proveInputLiveV65Roots<Instance>(
  extract: InputLiveV65ExtractPayload,
  sources: readonly (InputLiveV65SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV65Scene(
      root.scene,
...
export function assertInputLiveV65RootProofs(
  proofs: readonly InputLiveV65RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV65ProbeProgram(
  writer: InputLiveV65WriterOwnership,
  sources: readonly InputLiveV65SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V65_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV65ProbePayload(
  value: unknown,
  writer: InputLiveV65WriterOwnership,
): InputLiveV65ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV65ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V65_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V65_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV65CaptureManifest(
  cells: readonly InputLiveV65CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V65_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V65_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V65_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V65_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V65_SOURCE_IDS.some(
...
export const inputLiveV65CaptureManifestSha256 = (
  cells: readonly InputLiveV65CaptureCell[],
): string => {
  validateInputLiveV65CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV65CaptureProgram(
  writer: InputLiveV65WriterOwnership,
  cell: InputLiveV65CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V65_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V65_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V65_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV65CapturePayload(
  value: unknown,
  cell: InputLiveV65CaptureCell,
  rawResponseBytes: number,
): InputLiveV65CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV65CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V65_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V65_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV65CaptureResponses(
  manifest: readonly InputLiveV65CaptureCell[],
  responses: readonly InputLiveV65CapturePayload[],
): void {
  validateInputLiveV65CaptureManifest(manifest);
  if (
...
export interface InputLiveV65ObjectiveReport {
  artifactVersion: "input-live-v65-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV65Objective(
  manifest: readonly InputLiveV65CaptureCell[],
  responses: readonly InputLiveV65CapturePayload[],
  readReference: (cell: InputLiveV65CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV65CaptureCell) => Buffer,
): InputLiveV65ObjectiveReport {
  assertInputLiveV65CaptureResponses(manifest, responses);
...
export function buildInputLiveV65CleanupProgram(
  writer: InputLiveV65WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V65_NAMESPACE,
...
export function validateInputLiveV65CleanupPayload(
  value: unknown,
  writer: InputLiveV65WriterOwnership,
): InputLiveV65CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV65CleanupPayload;
...
export const INPUT_LIVE_V65_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV65WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
