...
import {
  verifyComboboxLiveV33FixedSceneFixedPoint,
  type ComboboxLiveV33FixedFixedPoint,
} from "./combobox-live-v33-fixed-point.js";

export const COMBOBOX_LIVE_V33_VERIFIER_VERSION = "combobox-live-v33-verifier-v1";

export function buildComboboxLiveV33RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV33RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV33RawNode[];
}

export interface ComboboxLiveV33NormalizedScene {
...
export interface ComboboxLiveV33SceneProof {
...
export function normalizeComboboxLiveV33Scene(
  root: ComboboxLiveV33RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV33NormalizedScene {
  const canonicalBindings: ComboboxLiveV33NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV33RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV33Scene<Instance>(
  normalized: ComboboxLiveV33NormalizedScene,
...
): ComboboxLiveV33SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV33FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV33ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV33PreCaptureGates(payload: unknown): void {
