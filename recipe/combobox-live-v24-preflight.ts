...
import {
  validateComboboxLiveV24AntecedentIndex,
  verifyComboboxLiveV24Authorization,
  type ComboboxLiveV24AntecedentIndex,
  type ComboboxLiveV24AuthorizationProof,
} from "./combobox-live-v24-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V24_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v24/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V24_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v24-security-attestation.json";

export interface ComboboxLiveV24SecurityAttestation {
  artifactVersion: "combobox-live-v24-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV24PreflightReport {
  artifactVersion: "combobox-live-v24-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV24SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV24SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV24AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v24-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V24_TARGET) ||
...
export function validateComboboxLiveV24ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV24Authorization");
  const preflight = source.indexOf("runComboboxLiveV24Preflight");
  const initialize = source.indexOf("ComboboxLiveV24Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV24Preflight(
  root: string,
  proof: ComboboxLiveV24AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V24_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV24PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V24_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V24_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V24_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV24AntecedentIndex;
  failures.push(...validateComboboxLiveV24AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V24_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V24_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV24SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v24-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV24ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v24 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV24SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v24-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V24_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV24Preflight(
    process.cwd(),
    verifyComboboxLiveV24Authorization(),
    attestationPath,
