...
export type ComboboxLiveV28HistoryExpectation = "pending" | "authorized";

export interface ComboboxLiveV28AuthorizationArtifact {
  artifactVersion: "combobox-live-v28-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof COMBOBOX_LIVE_V28_INDEX_PATH;
    indexSha256: string;
...
export interface ComboboxLiveV28AntecedentIndex {
...
export interface ComboboxLiveV28HistoryState {
  index?: ComboboxLiveV28AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: ComboboxLiveV28AuthorizationArtifact;
...
export interface ComboboxLiveV28AuthorizationProof extends ComboboxLiveV28TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof COMBOBOX_LIVE_V28_TARGET;
  expectedDynamicTool: typeof COMBOBOX_LIVE_V28_DYNAMIC_TOOL;
  authorizationPath: typeof COMBOBOX_LIVE_V28_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === COMBOBOX_LIVE_V28_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("combobox-live-v28-authorization") ||
  artifactPath.includes("combobox-live-v28-preflight") ||
  artifactPath.includes("combobox-live-v28-authorized") ||
  artifactPath.includes("create-combobox-live-v28-security-attestation") ||
  artifactPath.includes("combobox-live-pivot-v28-status") ||
...
export function validateComboboxLiveV28AntecedentIndex(
  index: ComboboxLiveV28AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "combobox-live-v28-antecedent-index-v1" ||
...
export function validateComboboxLiveV28UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyComboboxLiveV28History\(\s*["']/.test(source) ||
    /readComboboxLiveV28HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildComboboxLiveV28AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): ComboboxLiveV28AuthorizationArtifact {
  ...
  ) as ComboboxLiveV28AntecedentIndex;
  const indexFailures = validateComboboxLiveV28AntecedentIndex(index);
...
export function validateComboboxLiveV28AuthorizationArtifact(
  artifact: ComboboxLiveV28AuthorizationArtifact | undefined,
  index: ComboboxLiveV28AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "combobox-live-v28-capture-authorization-v1" ||
    artifact.authorizationId !== "combobox-live-v28" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== COMBOBOX_LIVE_V28_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, COMBOBOX_LIVE_V28_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== COMBOBOX_LIVE_V28_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== COMBOBOX_LIVE_V28_HOST_PHASES ||
    artifact?.denominator?.captures !== COMBOBOX_LIVE_V28_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== COMBOBOX_LIVE_V28_SOURCE_ROOTS ||
...
export function validateComboboxLiveV28History(
  state: ComboboxLiveV28HistoryState,
  expected: ComboboxLiveV28HistoryExpectation,
): string[] {
  const failures = validateComboboxLiveV28AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateComboboxLiveV28AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readComboboxLiveV28HistoryState(
  root = process.cwd(),
): ComboboxLiveV28HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, COMBOBOX_LIVE_V28_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as ComboboxLiveV28AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${COMBOBOX_LIVE_V28_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${COMBOBOX_LIVE_V28_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${COMBOBOX_LIVE_V28_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyComboboxLiveV28History(
  expected: ComboboxLiveV28HistoryExpectation,
  root = process.cwd(),
): ComboboxLiveV28HistoryState {
  const state = readComboboxLiveV28HistoryState(root);
  const failures = validateComboboxLiveV28History(state, expected);
  if (failures.length)
    throw new Error(
      `Combobox live v28 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyComboboxLiveV28Authorization(
  root = process.cwd(),
): ComboboxLiveV28AuthorizationProof {
  const state = verifyComboboxLiveV28History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, COMBOBOX_LIVE_V28_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/combobox-live-pivot-v28/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-combobox-live-v28.ts"]!.sha256,
    ...
    target: COMBOBOX_LIVE_V28_TARGET,
    expectedDynamicTool: COMBOBOX_LIVE_V28_DYNAMIC_TOOL,
    authorizationPath: COMBOBOX_LIVE_V28_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): ComboboxLiveV28HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyComboboxLiveV28History(expected);
  process.stdout.write(
    `Combobox live v28 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
