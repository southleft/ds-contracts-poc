...
import {
  buildInputLiveV9RawPropertyRuntime,
  normalizeInputLiveV9Scene,
  type InputLiveV9RawNode,
} from "./input-field-live-v9-verifier.js";
...
export const INPUT_LIVE_V9_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V9_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V9_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V9_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V9_VARIANT_COUNT = 256;
export const INPUT_LIVE_V9_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V9_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV9SourceId = (typeof INPUT_LIVE_V9_SOURCE_IDS)[number];

export interface InputLiveV9SourceIdentity {
  source: InputLiveV9SourceId;
...
export interface InputLiveV9WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV9ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV9SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV9RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV9RootProof {
  source: InputLiveV9SourceId;
  adapterIdentity: string;
...
export interface InputLiveV9ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV9SourceId;
    adapterIdentity: string;
...
export interface InputLiveV9CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV9SourceId;
  adapterIdentity: string;
...
export interface InputLiveV9CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV9SourceId;
  strata: InputLiveV9CaptureCell["strata"];
...
export interface InputLiveV9CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Input live v9 ${label} must be an object`);
...
const exactUniqueStrings = (
  ...
  if (
    ...
    throw new TypeError(`Input live v9 ${label} cardinality/identity mismatch`);
  return value;
};
const finiteNonnegative = (value: unknown): value is number =>
  typeof value === "number" && Number.isFinite(value) && value >= 0;

export function validateInputLiveV9WriterPayload(
  value: unknown,
): InputLiveV9WriterOwnership {
  ...
  if (
    ...
    throw new TypeError("Input live v9 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Input live v9 writer created-node denominator invalid",
    );
  return {
...
const identityPlan = (source: InputLiveV9SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV9ExtractProgram(
  writer: InputLiveV9WriterOwnership,
  sources: readonly InputLiveV9SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Input live v9 extract requires two source identities");
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V9_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V9_NAMESPACE)}
${buildInputLiveV9RawPropertyRuntime()}
...
export function validateInputLiveV9ExtractPayload(
  value: unknown,
  writer: InputLiveV9WriterOwnership,
): InputLiveV9ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Input live v9 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V9_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Input live v9 extract schema/two-root mismatch");
  return payload as InputLiveV9ExtractPayload;
}

export function proveInputLiveV9Roots<Instance>(
  extract: InputLiveV9ExtractPayload,
  sources: readonly (InputLiveV9SourceIdentity & {
...
): InputLiveV9RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Input live v9 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Input live v9 omitted ${source.source} root`);
    const normalized = normalizeInputLiveV9Scene(
      root.scene,
...
export function assertInputLiveV9RootProofs(
  proofs: readonly InputLiveV9RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Input live v9 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Input live v9 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildInputLiveV9ProbeProgram(
  writer: InputLiveV9WriterOwnership,
  sources: readonly InputLiveV9SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V9_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV9ProbePayload(
  value: unknown,
  writer: InputLiveV9WriterOwnership,
): InputLiveV9ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV9ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V9_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V9_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Input live v9 probe/usability/restoration failed");
  return payload;
}

export function validateInputLiveV9CaptureManifest(
  cells: readonly InputLiveV9CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V9_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V9_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V9_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V9_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V9_SOURCE_IDS.some(
      (source) => cells.filter((cell) => cell.source === source).length !== 64,
    )
  )
    throw new TypeError(
      "Input live v9 capture manifest must be exact 128 cells",
    );
}

export const inputLiveV9CaptureManifestSha256 = (
  cells: readonly InputLiveV9CaptureCell[],
): string => {
  validateInputLiveV9CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV9CaptureProgram(
  writer: InputLiveV9WriterOwnership,
  cell: InputLiveV9CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V9_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V9_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
    ...
    throw new TypeError("Input live v9 capture cell is malformed");
  const expectedSet = writer.sources.find(
    (source) => source.adapterIdentity === cell.adapterIdentity,
  )?.setId;
  if (!expectedSet)
    throw new TypeError(
      `Input live v9 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V9_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV9CapturePayload(
  value: unknown,
  cell: InputLiveV9CaptureCell,
  rawResponseBytes: number,
): InputLiveV9CapturePayload {
  const payload = record(value, "capture payload") as InputLiveV9CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V9_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V9_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Input live v9 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertInputLiveV9CaptureResponses(
  manifest: readonly InputLiveV9CaptureCell[],
  responses: readonly InputLiveV9CapturePayload[],
): void {
  validateInputLiveV9CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Input live v9 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface InputLiveV9ObjectiveReport {
  artifactVersion: "input-live-v9-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV9Objective(
  manifest: readonly InputLiveV9CaptureCell[],
  responses: readonly InputLiveV9CapturePayload[],
  readReference: (cell: InputLiveV9CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV9CaptureCell) => Buffer,
): InputLiveV9ObjectiveReport {
  assertInputLiveV9CaptureResponses(manifest, responses);
  const rows = manifest.map((cell, index) => {
    ...
    if (
      ...
      throw new TypeError(
        `Input live v9 objective byte drift: ${cell.cellKey}`,
      );
    ...
    if (
      ...
      throw new TypeError(
        `Input live v9 objective measurement refused: ${cell.cellKey}`,
      );
    return {
      index: cell.index,
...
export function buildInputLiveV9CleanupProgram(
  writer: InputLiveV9WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V9_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Input live v9 cleanup runtime result hook absent");
  return String.raw`
...
export function validateInputLiveV9CleanupPayload(
  value: unknown,
  writer: InputLiveV9WriterOwnership,
): InputLiveV9CleanupPayload {
  const payload = record(value, "cleanup payload") as InputLiveV9CleanupPayload;
  ...
  if (
    ...
    throw new TypeError("Input live v9 cleanup is incomplete or overbroad");
  return payload;
}

export const INPUT_LIVE_V9_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV9WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
