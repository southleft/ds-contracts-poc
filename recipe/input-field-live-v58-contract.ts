...
} from "./input-field-live-v3-verifier-v58.js";
import {
  buildInputLiveV58RawPropertyRuntime,
  normalizeInputLiveV58Scene,
  type InputLiveV58RawNode,
} from "./input-field-live-v58-verifier.js";
...
export {
  ...
  type InputLiveV58RestorePayload,
} from "./input-field-live-v58-restore.js";

export const INPUT_LIVE_V58_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V58_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V58_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V58_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V58_VARIANT_COUNT = 256;
export const INPUT_LIVE_V58_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V58_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV58SourceId = (typeof INPUT_LIVE_V58_SOURCE_IDS)[number];

export interface InputLiveV58SourceIdentity {
  source: InputLiveV58SourceId;
...
export interface InputLiveV58WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV58ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV58SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV58RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV58RootProof {
  source: InputLiveV58SourceId;
  adapterIdentity: string;
...
export interface InputLiveV58ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV58SourceId;
    adapterIdentity: string;
...
export interface InputLiveV58CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV58SourceId;
  adapterIdentity: string;
...
export interface InputLiveV58CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV58SourceId;
  strata: InputLiveV58CaptureCell["strata"];
...
export interface InputLiveV58CleanupPayload {
...
export function validateInputLiveV58WriterPayload(
  value: unknown,
): InputLiveV58WriterOwnership {
...
const identityPlan = (source: InputLiveV58SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV58ExtractProgram(
  writer: InputLiveV58WriterOwnership,
  sources: readonly InputLiveV58SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V58_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V58_NAMESPACE)}
${buildInputLiveV58RawPropertyRuntime()}
...
export function validateInputLiveV58ExtractPayload(
  value: unknown,
  writer: InputLiveV58WriterOwnership,
): InputLiveV58ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V58_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV58ExtractPayload;
}

export function proveInputLiveV58Roots<Instance>(
  extract: InputLiveV58ExtractPayload,
  sources: readonly (InputLiveV58SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV58Scene(
      root.scene,
...
export function assertInputLiveV58RootProofs(
  proofs: readonly InputLiveV58RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV58ProbeProgram(
  writer: InputLiveV58WriterOwnership,
  sources: readonly InputLiveV58SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V58_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV58ProbePayload(
  value: unknown,
  writer: InputLiveV58WriterOwnership,
): InputLiveV58ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV58ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V58_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V58_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV58CaptureManifest(
  cells: readonly InputLiveV58CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V58_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V58_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V58_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V58_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V58_SOURCE_IDS.some(
...
export const inputLiveV58CaptureManifestSha256 = (
  cells: readonly InputLiveV58CaptureCell[],
): string => {
  validateInputLiveV58CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV58CaptureProgram(
  writer: InputLiveV58WriterOwnership,
  cell: InputLiveV58CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V58_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V58_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V58_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV58CapturePayload(
  value: unknown,
  cell: InputLiveV58CaptureCell,
  rawResponseBytes: number,
): InputLiveV58CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV58CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V58_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V58_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV58CaptureResponses(
  manifest: readonly InputLiveV58CaptureCell[],
  responses: readonly InputLiveV58CapturePayload[],
): void {
  validateInputLiveV58CaptureManifest(manifest);
  if (
...
export interface InputLiveV58ObjectiveReport {
  artifactVersion: "input-live-v58-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV58Objective(
  manifest: readonly InputLiveV58CaptureCell[],
  responses: readonly InputLiveV58CapturePayload[],
  readReference: (cell: InputLiveV58CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV58CaptureCell) => Buffer,
): InputLiveV58ObjectiveReport {
  assertInputLiveV58CaptureResponses(manifest, responses);
...
export function buildInputLiveV58CleanupProgram(
  writer: InputLiveV58WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V58_NAMESPACE,
...
export function validateInputLiveV58CleanupPayload(
  value: unknown,
  writer: InputLiveV58WriterOwnership,
): InputLiveV58CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV58CleanupPayload;
...
export const INPUT_LIVE_V58_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV58WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
