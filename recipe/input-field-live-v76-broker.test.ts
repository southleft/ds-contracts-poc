...
import {
  ...
  createInputLiveV76FakeDynamicResponse,
  ...
  listInputLiveV76PersistedRequests,
  persistInputLiveV76RawResponse,
  persistInputLiveV76TechnicalGates,
  validateInputLiveV76Receipt,
  validateInputLiveV76Request,
  type InputLiveV76RemotePhase,
...
import {
  ...
  type InputLiveV76ExtractPayload,
  type InputLiveV76ProbePayload,
...
import {
  ...
  simulatedInputLiveV76Authorization,
} from "./run-input-field-live-v76.js";
...
const captures: InputLiveV76CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV76Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v76-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV76Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV76CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV76FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV76FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V76_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V76_TARGET);
  persistInputLiveV76RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV76Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV76Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV76WriterOwnership,
...
const rootProofs = (): InputLiveV76RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v76-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV76Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV76Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV76RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV76RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV76ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV76ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV76CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV76CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV76CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV76Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV76CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV76PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV76ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV76Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV76Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV76Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV76Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V76_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV76RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V76_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV76Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV76Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV76ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV76Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV76WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV76RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV76Request;
    validateInputLiveV76Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV76Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV76CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v76-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV76Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV76Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV76Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V76_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV76RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V76_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV76Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV76Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV76RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V76_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV76Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV76Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV76Request;
