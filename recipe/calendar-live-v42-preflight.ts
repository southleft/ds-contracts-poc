...
import {
  ...
  verifyCalendarLiveV42Authorization,
  type CalendarLiveV42AntecedentIndex,
  type CalendarLiveV42AuthorizationProof,
} from "./calendar-live-v42-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V42_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v42/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V42_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v42-security-attestation.json";

export interface CalendarLiveV42SecurityAttestation {
...
export interface CalendarLiveV42PreflightReport {
  artifactVersion: "calendar-live-v42-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV42SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV42SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV42AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v42-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V42_TARGET) ||
...
export function validateCalendarLiveV42ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV42Authorization");
  const preflight = source.indexOf("runCalendarLiveV42Preflight");
  const initialize = source.indexOf("CalendarLiveV42Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV42Preflight(
  root: string,
  proof: CalendarLiveV42AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V42_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV42PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V42_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V42_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV42AntecedentIndex;
  failures.push(...validateCalendarLiveV42AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V42_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V42_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v42-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV42ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v42 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV42SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v42-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V42_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV42Preflight(
    process.cwd(),
    verifyCalendarLiveV42Authorization(),
    attestationPath,
