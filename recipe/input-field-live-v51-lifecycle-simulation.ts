...
export interface InputLiveV51LifecycleSimulationReport {
  artifactVersion: "input-live-v51-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV51AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV51Lifecycle(): Promise<InputLiveV51LifecycleSimulationReport> {
  await buildInputLiveV51Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V51_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV51AntecedentIndex;
  const indexSha256 = inputLiveV51Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v51-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V51_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V51_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV51History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV51History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV51AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V51_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V51_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV51History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV51History(authorized, "pending");
    ...
    await buildInputLiveV51Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V51_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV51AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV51Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV51Lifecycle(), null, 2)}\n`,
  );
