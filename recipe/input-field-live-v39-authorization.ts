...
import {
  ...
  INPUT_LIVE_V39_INDEX_PATH,
} from "./build-input-field-live-proof-v39.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV39HistoryExpectation = "pending" | "authorized";

export interface InputLiveV39AuthorizationArtifact {
  artifactVersion: "input-live-v39-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V39_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV39AntecedentIndex {
...
export interface InputLiveV39HistoryState {
  index?: InputLiveV39AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV39AuthorizationArtifact;
...
export interface InputLiveV39AuthorizationProof extends InputLiveV39TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V39_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V39_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V39_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V39_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v39-status") ||
...
export function validateInputLiveV39AntecedentIndex(
  index: InputLiveV39AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v39-antecedent-index-v1" ||
...
export function validateInputLiveV39UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV39History\(\s*["']/.test(source) ||
    /readInputLiveV39HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV39AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV39AuthorizationArtifact {
  ...
  ) as InputLiveV39AntecedentIndex;
  const indexFailures = validateInputLiveV39AntecedentIndex(index);
...
export function validateInputLiveV39AuthorizationArtifact(
  artifact: InputLiveV39AuthorizationArtifact | undefined,
  index: InputLiveV39AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v39-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v39" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V39_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V39_TARGET) ||
    !equalJson(
...
export function validateInputLiveV39History(
  state: InputLiveV39HistoryState,
  expected: InputLiveV39HistoryExpectation,
): string[] {
  const failures = validateInputLiveV39AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV39AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV39HistoryState(
  root = process.cwd(),
): InputLiveV39HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V39_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV39AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V39_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V39_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V39_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV39History(
  expected: InputLiveV39HistoryExpectation,
  root = process.cwd(),
): InputLiveV39HistoryState {
  const state = readInputLiveV39HistoryState(root);
  const failures = validateInputLiveV39History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV39Authorization(
  root = process.cwd(),
): InputLiveV39AuthorizationProof {
  const state = verifyInputLiveV39History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V39_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v39/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v39.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V39_TARGET,
    expectedDynamicTool: INPUT_LIVE_V39_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V39_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV39HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV39History(expected);
