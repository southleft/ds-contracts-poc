...
} from "./input-field-live-v3-verifier-v35.js";
import {
  buildInputLiveV35RawPropertyRuntime,
  normalizeInputLiveV35Scene,
  type InputLiveV35RawNode,
} from "./input-field-live-v35-verifier.js";
import { measureVisualPair } from "./input-field-objective-comparison-v1.js";
import { canonicalJson } from "./normalize.js";
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-v35.js";
import {
  compareSceneToExpectedPlan,
  type ExpectedScenePlan,
  type SceneComparison,
} from "./scene-readback-v35.js";
...
export const INPUT_LIVE_V35_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V35_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V35_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V35_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V35_VARIANT_COUNT = 256;
export const INPUT_LIVE_V35_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V35_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV35SourceId = (typeof INPUT_LIVE_V35_SOURCE_IDS)[number];

export interface InputLiveV35SourceIdentity {
  source: InputLiveV35SourceId;
...
export interface InputLiveV35WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV35ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV35SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV35RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV35RootProof {
  source: InputLiveV35SourceId;
  adapterIdentity: string;
...
export interface InputLiveV35ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV35SourceId;
    adapterIdentity: string;
...
export interface InputLiveV35CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV35SourceId;
  adapterIdentity: string;
...
export interface InputLiveV35CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV35SourceId;
  strata: InputLiveV35CaptureCell["strata"];
...
export interface InputLiveV35CleanupPayload {
...
export function validateInputLiveV35WriterPayload(
  value: unknown,
): InputLiveV35WriterOwnership {
...
const identityPlan = (source: InputLiveV35SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV35ExtractProgram(
  writer: InputLiveV35WriterOwnership,
  sources: readonly InputLiveV35SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V35_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V35_NAMESPACE)}
${buildInputLiveV35RawPropertyRuntime()}
...
export function validateInputLiveV35ExtractPayload(
  value: unknown,
  writer: InputLiveV35WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V35_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV35ExtractPayload;
}

export function proveInputLiveV35Roots<Instance>(
  extract: InputLiveV35ExtractPayload,
  sources: readonly (InputLiveV35SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV35Scene(
      root.scene,
...
export function assertInputLiveV35RootProofs(
  proofs: readonly InputLiveV35RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV35ProbeProgram(
  writer: InputLiveV35WriterOwnership,
  sources: readonly InputLiveV35SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V35_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV35ProbePayload(
  value: unknown,
  writer: InputLiveV35WriterOwnership,
): InputLiveV35ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV35ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V35_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V35_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV35CaptureManifest(
  cells: readonly InputLiveV35CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V35_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V35_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V35_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V35_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V35_SOURCE_IDS.some(
...
export const inputLiveV35CaptureManifestSha256 = (
  cells: readonly InputLiveV35CaptureCell[],
): string => {
  validateInputLiveV35CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV35CaptureProgram(
  writer: InputLiveV35WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V35_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V35_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V35_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV35CapturePayload(
  value: unknown,
  cell: InputLiveV35CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV35CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V35_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V35_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV35CaptureResponses(
  manifest: readonly InputLiveV35CaptureCell[],
  responses: readonly InputLiveV35CapturePayload[],
): void {
  validateInputLiveV35CaptureManifest(manifest);
  if (
...
export interface InputLiveV35ObjectiveReport {
  artifactVersion: "input-live-v35-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV35Objective(
  manifest: readonly InputLiveV35CaptureCell[],
  responses: readonly InputLiveV35CapturePayload[],
  readReference: (cell: InputLiveV35CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV35CaptureCell) => Buffer,
): InputLiveV35ObjectiveReport {
  assertInputLiveV35CaptureResponses(manifest, responses);
...
export function buildInputLiveV35CleanupProgram(
  writer: InputLiveV35WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V35_NAMESPACE,
...
export function validateInputLiveV35CleanupPayload(
  value: unknown,
  writer: InputLiveV35WriterOwnership,
): InputLiveV35CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV35CleanupPayload;
...
export const INPUT_LIVE_V35_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV35WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
