...
import {
  acceptComboboxLiveV13Response,
  createComboboxLiveV13Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV13Request,
  persistComboboxLiveV13RawResponse,
  persistComboboxLiveV13TechnicalGates,
  readComboboxLiveV13AcceptedRawPayload,
  type ComboboxLiveV13Request,
  type ComboboxLiveV13TransactionAuthorization,
} from "./combobox-live-v13-broker.js";
import {
  COMBOBOX_LIVE_V13_CAPTURE_COUNT,
  assertComboboxLiveV13CaptureResponses,
  ...
  evaluateComboboxLiveV13Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV13Roots,
  validateComboboxLiveV13CaptureManifest,
  validateComboboxLiveV13CapturePayload,
  validateComboboxLiveV13CleanupPayload,
  validateComboboxLiveV13ExtractPayload,
  validateComboboxLiveV13ProbePayload,
  validateComboboxLiveV13WriterPayload,
  type ComboboxLiveV13CaptureCell,
  type ComboboxLiveV13CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V13_PLAN_PATH,
  type ComboboxLiveV13ProofPlan,
} from "./build-combobox-live-proof-v13.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v13.js";

export const COMBOBOX_LIVE_V13_RUNNER_VERSION = "combobox-live-v13-runner-v1";

interface ComboboxLiveV13JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV13OrchestratorState {
  artifactVersion: "combobox-live-v13-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV13JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV13ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV13CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV13SourceIdentity & {
...
export interface ComboboxLiveV13NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV13OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v13 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V13_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V13_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV13ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV13CaptureCell[];
  validateComboboxLiveV13CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v13 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v13 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV13WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV13RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV13Request =>
  readJson<ComboboxLiveV13Request>(file);

export class ComboboxLiveV13Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV13Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV13OrchestratorState = {
      artifactVersion: "combobox-live-v13-orchestrator-state-v1",
    ...
    return new ComboboxLiveV13Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV13OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV13Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV13NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV13NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v13 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV13NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v13 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV13RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV13Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV13CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV13AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV13WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV13Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV13ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV13Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV13Response(this.#directory, "writer", {
        validate: validateComboboxLiveV13WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV13Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV13RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV13Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV13ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV13Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV13RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v13-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV13Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV13ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV13Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV13ProbePayload(payload, ownership),
      });
      persistComboboxLiveV13TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v13 capture cell missing");
      const accepted = acceptComboboxLiveV13Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV13CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V13_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV13CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV13Objective(
    ...
    throw new Error(`Combobox live v13 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV13Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV13CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV13Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV13RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV13Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV13CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV13CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV13CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV13Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV13TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV13SimulationKey = (): KeyObject =>
