...
export type TableLiveV29HistoryExpectation = "pending" | "authorized";

export interface TableLiveV29AuthorizationArtifact {
  artifactVersion: "table-live-v29-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V29_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV29AntecedentIndex {
...
export interface TableLiveV29HistoryState {
  ...
  authorization?: TableLiveV29AuthorizationArtifact;
...
export interface TableLiveV29AuthorizationProof
  extends TableLiveV29TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V29_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V29_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V29_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V29_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v29-authorization") ||
  artifactPath.includes("table-live-v29-preflight") ||
  artifactPath.includes("table-live-v29-authorized") ||
  artifactPath.includes("create-table-live-v29-security-attestation") ||
  artifactPath.includes("table-live-pivot-v29-status") ||
...
export function validateTableLiveV29AntecedentIndex(
  index: TableLiveV29AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v29-antecedent-index-v1" ||
...
export function validateTableLiveV29UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV29History\(\s*["']/.test(source) ||
    /readTableLiveV29HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV29AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV29AuthorizationArtifact {
  ...
  const indexFailures = validateTableLiveV29AntecedentIndex(index);
...
export function validateTableLiveV29AuthorizationArtifact(
  artifact: TableLiveV29AuthorizationArtifact | undefined,
  index: TableLiveV29AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v29-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v29" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V29_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V29_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V29_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V29_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V29_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V29_SOURCE_ROOTS ||
...
export function validateTableLiveV29History(
  state: TableLiveV29HistoryState,
  expected: TableLiveV29HistoryExpectation,
): string[] {
  const failures = validateTableLiveV29AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateTableLiveV29AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readTableLiveV29HistoryState(
  root = process.cwd(),
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V29_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV29AuthorizationArtifact)
    : undefined;
  ...
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V29_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V29_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV29History(
  expected: TableLiveV29HistoryExpectation,
  root = process.cwd(),
): TableLiveV29HistoryState {
  const state = readTableLiveV29HistoryState(root);
  const failures = validateTableLiveV29History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v29 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV29Authorization(
  root = process.cwd(),
): TableLiveV29AuthorizationProof {
  const state = verifyTableLiveV29History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V29_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v29/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v29.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V29_TARGET,
    expectedDynamicTool: TABLE_LIVE_V29_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V29_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV29HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV29History(expected);
  process.stdout.write(
    `Table live v29 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
