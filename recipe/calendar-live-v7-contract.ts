...
import { buildCalendarLiveV7CleanupRuntime } from "./calendar-live-v7-cleanup.js";
import { type CalendarLiveV7FixedFixedPoint } from "./calendar-live-v7-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV7RestoreProgram,
  validateCalendarLiveV7RestorePayload,
  type CalendarLiveV7RestorePayload,
} from "./calendar-live-v7-restore.js";
import {
  buildCalendarLiveV7RawPropertyRuntime,
  normalizeCalendarLiveV7Scene,
  type CalendarLiveV7RawNode,
} from "./calendar-live-v7-verifier.js";
...
export const CALENDAR_LIVE_V7_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V7_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V7_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V7_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V7_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V7_SET_COUNT = 3;
export const CALENDAR_LIVE_V7_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V7_HOST_PHASES = 3;
export const CALENDAR_LIVE_V7_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V7_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V7_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V7_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV7SourceId = (typeof CALENDAR_LIVE_V7_SOURCE_IDS)[number];
export type CalendarLiveV7Kind = "calendar" | "week" | "day";

export interface CalendarLiveV7SourceIdentity {
  source: CalendarLiveV7SourceId;
...
export interface CalendarLiveV7WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV7ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV7SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV7RawNode;
    weekScene: CalendarLiveV7RawNode;
    dayScene: CalendarLiveV7RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV7RootProof {
  source: CalendarLiveV7SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV7FixedFixedPoint;
}

export interface CalendarLiveV7ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV7SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV7CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV7SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV7CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV7SourceId;
  frameWidth: number;
...
export interface CalendarLiveV7CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v7 ${label} must be an object`);
...
export function validateCalendarLiveV7WriterPayload(
  value: unknown,
): CalendarLiveV7WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v7") ||
    ...
    result.namespace !== CALENDAR_LIVE_V7_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v7 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v7 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v7 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV7SourceIdentity,
  kind: CalendarLiveV7Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV7ExtractProgram(
  writer: CalendarLiveV7WriterOwnership,
  sources: readonly CalendarLiveV7SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v7 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V7_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V7_NAMESPACE)}
${buildCalendarLiveV7RawPropertyRuntime()}
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("CALENDAR-V7-EXTRACT-PAGE");
...
if(get(page,"pageOwner")!=="recipe/calendar/"+${JSON.stringify(writer.runIdentity)})throw new Error("CALENDAR-V7-EXTRACT-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>SET_IDS.has(node.id));
if(sets.length!==3)throw new Error("CALENDAR-V7-EXTRACT-ROOTS:"+sets.length);
...
for(const set of sets){
  ...
  if(!IDENTITIES[adapterIdentity]||(kind!=="calendar"&&kind!=="week"&&kind!=="day"))throw new Error("CALENDAR-V7-EXTRACT-ADAPTER:"+adapterIdentity+":"+kind);
...
if(roots.length!==1||roots.some(root=>!root.calendarScene||!root.weekScene||!root.dayScene))throw new Error("CALENDAR-V7-EXTRACT-TRIPLE");
...
export function validateCalendarLiveV7ExtractPayload(
  value: unknown,
  writer: CalendarLiveV7WriterOwnership,
): CalendarLiveV7ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v7 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V7_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v7 extract schema/one-root mismatch");
  return payload as CalendarLiveV7ExtractPayload;
}

export function proveCalendarLiveV7Roots<Instance>(
  extract: CalendarLiveV7ExtractPayload,
  sources: readonly (CalendarLiveV7SourceIdentity & {
...
): CalendarLiveV7RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v7 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v7 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV7Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV7Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV7Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV7FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v7 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v7 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v7 compile lost owned sets");
...
export function assertCalendarLiveV7RootProofs(
  proofs: readonly CalendarLiveV7RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v7 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v7 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV7ProbeProgram(
  writer: CalendarLiveV7WriterOwnership,
  sources: readonly CalendarLiveV7SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V7_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV7ProbePayload(
  value: unknown,
  writer: CalendarLiveV7WriterOwnership,
): CalendarLiveV7ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV7ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V7_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V7_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v7 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV7CaptureManifest(
  cells: readonly CalendarLiveV7CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V7_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V7_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V7_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V7_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v7 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV7CaptureCell[],
): string => {
  validateCalendarLiveV7CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV7CaptureProgram(
  writer: CalendarLiveV7WriterOwnership,
  cell: CalendarLiveV7CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V7_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v7 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v7 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V7_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV7CapturePayload(
  value: unknown,
  cell: CalendarLiveV7CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV7CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV7CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V7_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V7_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v7 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV7CaptureResponses(
  manifest: readonly CalendarLiveV7CaptureCell[],
  responses: readonly CalendarLiveV7CapturePayload[],
): void {
  validateCalendarLiveV7CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v7 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV7ObjectiveReport {
  artifactVersion: "calendar-live-v7-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV7Objective(
  manifest: readonly CalendarLiveV7CaptureCell[],
  responses: readonly CalendarLiveV7CapturePayload[],
): CalendarLiveV7ObjectiveReport {
  assertCalendarLiveV7CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v7-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV7CleanupProgram(
  writer: CalendarLiveV7WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV7CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V7_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v7 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV7CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV7WriterOwnership,
): CalendarLiveV7CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV7CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v7 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V7_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV7WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
