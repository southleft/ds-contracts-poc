...
export type TableLiveV12HistoryExpectation = "pending" | "authorized";

export interface TableLiveV12AuthorizationArtifact {
  artifactVersion: "table-live-v12-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V12_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV12AntecedentIndex {
...
export interface TableLiveV12HistoryState {
  ...
  authorization?: TableLiveV12AuthorizationArtifact;
...
export interface TableLiveV12AuthorizationProof
  extends TableLiveV12TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V12_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V12_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V12_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V12_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v12-authorization") ||
  artifactPath.includes("table-live-v12-preflight") ||
  artifactPath.includes("table-live-v12-authorized") ||
  artifactPath.includes("create-table-live-v12-security-attestation") ||
  artifactPath.includes("table-live-pivot-v12-status") ||
...
export function validateTableLiveV12AntecedentIndex(
  index: TableLiveV12AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v12-antecedent-index-v1" ||
    index.authorizationCanBeAddedWithoutAntecedentRebuild !== true
  )
    failures.push("table v12 antecedent index missing or malformed");
  const artifactEntries = Object.entries(index?.artifacts ?? {});
  if (artifactEntries.length === 0)
    failures.push("table v12 antecedent hash denominator is empty");
  for (const [artifactPath, metadata] of artifactEntries) {
    ...
    if (
      ...
      failures.push(`malformed table v12 antecedent hash: ${artifactPath}`);
  }
  ...
  if (index?.hashSetSha256 !== expectedHashSet)
    failures.push("table v12 antecedent hash-set digest mismatch");
  if (
    ...
    failures.push("table v12 antecedent counts missing or drifted");
  for (const required of [
    ...
    if (!index?.authorizationLifecycleExcluded?.includes(required))
      failures.push(`table v12 lifecycle exclusion missing: ${required}`);
  const leaks = forbiddenAntecedentFieldPaths(index);
  if (leaks.length)
    failures.push(`table v12 antecedent result leakage: ${leaks.join(",")}`);
  return failures;
}

export function validateTableLiveV12UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV12History\(\s*["']/.test(source) ||
    /readTableLiveV12HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV12AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV12AuthorizationArtifact {
  if (!SHA40.test(options.antecedentCommit))
    throw new TypeError("table v12 antecedent commit must be a full SHA");
  const index = JSON.parse(
    Buffer.from(options.antecedentIndexBytes).toString("utf8"),
  ) as TableLiveV12AntecedentIndex;
  const indexFailures = validateTableLiveV12AntecedentIndex(index);
  if (indexFailures.length)
    throw new TypeError(
      `table v12 antecedent index refused:\n${indexFailures.join("\n")}`,
    );
  const publicKey =
...
export function validateTableLiveV12AuthorizationArtifact(
  artifact: TableLiveV12AuthorizationArtifact | undefined,
  index: TableLiveV12AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v12-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v12" ||
    artifact.authorizationIntent !== true
  )
    failures.push("table v12 authorization declaration missing or malformed");
  const resultLeaks = forbiddenAntecedentFieldPaths(artifact);
  if (resultLeaks.length)
    failures.push(`table v12 authorization result leakage: ${resultLeaks.join(",")}`);
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V12_INDEX_PATH ||
    artifact?.antecedent?.indexSha256 !== indexSha256 ||
    artifact?.antecedent?.hashSetSha256 !== index?.hashSetSha256
  )
    failures.push("table v12 authorization pins wrong antecedent index");
  if (privateMaterialPaths(artifact).length)
    failures.push("table v12 authorization contains private key or token material");
  try {
    ...
    if (
      ...
      failures.push("table v12 Ed25519 public-key identity mismatch");
  } catch {
    failures.push("table v12 Ed25519 public key is invalid");
  }
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V12_TARGET) ||
    !equalJson(
    ...
    failures.push("wrong table v12 target or dynamic tool");
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V12_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V12_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V12_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V12_SOURCE_ROOTS ||
    artifact?.denominator?.expectedFacts !== index?.counts.expectedSceneFacts
  )
    failures.push("table v12 two-root or capture denominator changed");
  if (
    ...
    failures.push(
      "table v12 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
    ...
    failures.push("table v12 security prerequisites weakened");
  if (
    artifact?.humanSignoff?.mandatory !== true ||
    artifact?.humanSignoff?.status !== "pending"
  )
    failures.push("table v12 human signoff overclaim");
  return failures;
}

export function validateTableLiveV12History(
  state: TableLiveV12HistoryState,
  expected: TableLiveV12HistoryExpectation,
): string[] {
  const failures = validateTableLiveV12AntecedentIndex(state.index);
  if (!SHA40.test(state.codeCommit))
    failures.push("table v12 code commit is not a full SHA");
  ...
  if (
    ...
    failures.push("table v12 antecedent first-add commit is not unique");
  if (!state.antecedentIsAncestorOfCode)
    failures.push("code commit does not descend from table v12 antecedent");
  if (!state.antecedentIndexBytesMatchFirstAddition)
    failures.push("table v12 antecedent index changed after first addition");
  if (!state.antecedentArtifactsMatchIndexAtCommit)
    failures.push("table v12 antecedent commit artifacts do not match index");
  if (!state.workingAntecedentArtifactsMatchIndex)
    failures.push("table v12 antecedent working bytes drifted");
  ...
  failures.push(
    ...validateTableLiveV12AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
  ...
  if (
    ...
    failures.push("table v12 authorization first-add lineage mismatch");
  if (!state.authorizationPresentAtCodeCommit)
    failures.push("table v12 authorization missing from code commit");
  if (!state.authorizationBytesMatchFirstAddition)
    failures.push("table v12 authorization bytes changed after first addition");
  if (!state.authorizationStrictlyDescendsFromAntecedent)
    failures.push("table v12 authorization does not strictly descend from antecedent");
  if (!state.authorizationIsAncestorOfCode)
    failures.push("code commit predates table v12 authorization");
  return failures;
}
...
export function readTableLiveV12HistoryState(
  root = process.cwd(),
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V12_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV12AuthorizationArtifact)
    : undefined;
  ...
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V12_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V12_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV12History(
  expected: TableLiveV12HistoryExpectation,
  root = process.cwd(),
): TableLiveV12HistoryState {
  const state = readTableLiveV12HistoryState(root);
  const failures = validateTableLiveV12History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v12 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV12Authorization(
  root = process.cwd(),
): TableLiveV12AuthorizationProof {
  const state = verifyTableLiveV12History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V12_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v12/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v12.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V12_TARGET,
    expectedDynamicTool: TABLE_LIVE_V12_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V12_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV12HistoryExpectation => {
  ...
  if (pending === authorized)
    throw new Error(
      "Choose exactly one table v12 history mode: --expect-pending or --expect-authorized",
    );
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV12History(expected);
  process.stdout.write(
    `Table live v12 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
