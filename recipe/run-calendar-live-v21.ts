...
import {
  acceptCalendarLiveV21Response,
  createCalendarLiveV21Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV21Request,
  persistCalendarLiveV21RawResponse,
  persistCalendarLiveV21TechnicalGates,
  readCalendarLiveV21AcceptedRawPayload,
  type CalendarLiveV21Request,
  type CalendarLiveV21TransactionAuthorization,
} from "./calendar-live-v21-broker.js";
import {
  CALENDAR_LIVE_V21_CAPTURE_COUNT,
  assertCalendarLiveV21CaptureResponses,
  ...
  evaluateCalendarLiveV21Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV21Roots,
  validateCalendarLiveV21CaptureManifest,
  validateCalendarLiveV21CapturePayload,
  validateCalendarLiveV21CleanupPayload,
  validateCalendarLiveV21ExtractPayload,
  validateCalendarLiveV21ProbePayload,
  validateCalendarLiveV21WriterPayload,
  type CalendarLiveV21CaptureCell,
  type CalendarLiveV21CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V21_PLAN_PATH,
  type CalendarLiveV21ProofPlan,
...
export const CALENDAR_LIVE_V21_RUNNER_VERSION = "calendar-live-v21-runner-v1";

interface CalendarLiveV21JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV21OrchestratorState {
  artifactVersion: "calendar-live-v21-orchestrator-state-v1";
  ...
  journal: CalendarLiveV21JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV21ProofPlan;
  planSha256: string;
  captures: CalendarLiveV21CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV21SourceIdentity & {
...
export interface CalendarLiveV21NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV21OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v21 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V21_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V21_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV21ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV21CaptureCell[];
  validateCalendarLiveV21CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v21 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v21 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV21WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV21RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV21Request =>
  readJson<CalendarLiveV21Request>(file);

export class CalendarLiveV21Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV21Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV21OrchestratorState = {
      artifactVersion: "calendar-live-v21-orchestrator-state-v1",
    ...
    return new CalendarLiveV21Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV21OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV21Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV21NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV21NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v21 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV21NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v21 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV21RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV21Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV21CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV21AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV21WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV21Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV21ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV21Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV21Response(this.#directory, "writer", {
        validate: validateCalendarLiveV21WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV21Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV21RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV21Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV21ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV21Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV21RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v21-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV21Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV21ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV21Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV21ProbePayload(payload, ownership),
      });
      persistCalendarLiveV21TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v21 capture cell missing");
      const accepted = acceptCalendarLiveV21Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV21CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V21_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV21CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV21Objective(
    ...
    throw new Error(`Calendar live v21 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV21Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV21CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV21Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV21RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV21Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV21CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV21CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV21CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV21Authorization = (
  privateKey: KeyObject,
): CalendarLiveV21TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV21SimulationKey = (): KeyObject =>
