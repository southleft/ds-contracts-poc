...
import {
  verifyComboboxLiveV14FixedSceneFixedPoint,
  type ComboboxLiveV14FixedFixedPoint,
} from "./combobox-live-v14-fixed-point.js";

export const COMBOBOX_LIVE_V14_VERIFIER_VERSION = "combobox-live-v14-verifier-v1";

export function buildComboboxLiveV14RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV14RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV14RawNode[];
}

export interface ComboboxLiveV14NormalizedScene {
...
export interface ComboboxLiveV14SceneProof {
...
export function normalizeComboboxLiveV14Scene(
  root: ComboboxLiveV14RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV14NormalizedScene {
  const canonicalBindings: ComboboxLiveV14NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV14RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV14Scene<Instance>(
  normalized: ComboboxLiveV14NormalizedScene,
...
): ComboboxLiveV14SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV14FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV14ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV14PreCaptureGates(payload: unknown): void {
