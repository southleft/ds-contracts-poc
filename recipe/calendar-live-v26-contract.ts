...
import { buildCalendarLiveV26CleanupRuntime } from "./calendar-live-v26-cleanup.js";
import { type CalendarLiveV26FixedFixedPoint } from "./calendar-live-v26-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildCalendarLiveV26RestoreProgram,
  validateCalendarLiveV26RestorePayload,
  type CalendarLiveV26RestorePayload,
} from "./calendar-live-v26-restore.js";
import {
  buildCalendarLiveV26RawPropertyRuntime,
  normalizeCalendarLiveV26Scene,
  type CalendarLiveV26RawNode,
} from "./calendar-live-v26-verifier.js";
...
export const CALENDAR_LIVE_V26_NAMESPACE = CALENDAR_FIGMA_NAMESPACE;
export const CALENDAR_LIVE_V26_SOURCE_IDS = ["astryx"] as const;
export const CALENDAR_LIVE_V26_ADAPTERS = [
  "astryx-calendar-reviewed-v1",
] as const;
export const CALENDAR_LIVE_V26_CAPTURE_COUNT = 8;
export const CALENDAR_LIVE_V26_VARIANT_COUNT = 8;
export const CALENDAR_LIVE_V26_SET_COUNT = 3;
export const CALENDAR_LIVE_V26_REMOTE_REQUESTS = 13;
export const CALENDAR_LIVE_V26_HOST_PHASES = 3;
export const CALENDAR_LIVE_V26_SOURCE_ROOTS = 1;
export const CALENDAR_LIVE_V26_RESTORE_COUNT = 4;
export const CALENDAR_LIVE_V26_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const CALENDAR_LIVE_V26_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type CalendarLiveV26SourceId = (typeof CALENDAR_LIVE_V26_SOURCE_IDS)[number];
export type CalendarLiveV26Kind = "calendar" | "week" | "day";

export interface CalendarLiveV26SourceIdentity {
  source: CalendarLiveV26SourceId;
...
export interface CalendarLiveV26WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface CalendarLiveV26ExtractPayload {
  pageId: string;
  roots: Array<{
    source: CalendarLiveV26SourceId;
    adapterIdentity: string;
    ...
    weekSetId: string;
    daySetId: string;
    calendarScene: CalendarLiveV26RawNode;
    weekScene: CalendarLiveV26RawNode;
    dayScene: CalendarLiveV26RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface CalendarLiveV26RootProof {
  source: CalendarLiveV26SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: CalendarLiveV26FixedFixedPoint;
}

export interface CalendarLiveV26ProbePayload {
  pageId: string;
  sources: Array<{
    source: CalendarLiveV26SourceId;
    adapterIdentity: string;
...
export interface CalendarLiveV26CaptureCell {
  index: number;
  cellKey: string;
  source: CalendarLiveV26SourceId;
  adapterIdentity: string;
...
export interface CalendarLiveV26CapturePayload {
  index: number;
  cellKey: string;
  source: CalendarLiveV26SourceId;
  frameWidth: number;
...
export interface CalendarLiveV26CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Calendar live v26 ${label} must be an object`);
...
export function validateCalendarLiveV26WriterPayload(
  value: unknown,
): CalendarLiveV26WriterOwnership {
  ...
  if (
    ...
    !String(result.runIdentity).endsWith("-calendar-v26") ||
    ...
    result.namespace !== CALENDAR_LIVE_V26_NAMESPACE ||
    ...
    throw new TypeError("Calendar live v26 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Calendar live v26 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 3)
    throw new TypeError("Calendar live v26 writer set identity collision");
  return {
...
const identityPlan = (
  source: CalendarLiveV26SourceIdentity,
  kind: CalendarLiveV26Kind,
) => {
  const plan =
    kind === "calendar"
...
export function buildCalendarLiveV26ExtractProgram(
  writer: CalendarLiveV26WriterOwnership,
  sources: readonly CalendarLiveV26SourceIdentity[],
): string {
  if (sources.length !== 1)
    throw new TypeError(
      "Calendar live v26 extract requires one source identity",
    );
  if (
...
const NS=${JSON.stringify(CALENDAR_LIVE_V26_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(CALENDAR_LIVE_V26_NAMESPACE)}
${buildCalendarLiveV26RawPropertyRuntime()}
...
export function validateCalendarLiveV26ExtractPayload(
  value: unknown,
  writer: CalendarLiveV26WriterOwnership,
): CalendarLiveV26ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Calendar live v26 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !CALENDAR_LIVE_V26_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Calendar live v26 extract schema/one-root mismatch");
  return payload as CalendarLiveV26ExtractPayload;
}

export function proveCalendarLiveV26Roots<Instance>(
  extract: CalendarLiveV26ExtractPayload,
  sources: readonly (CalendarLiveV26SourceIdentity & {
...
): CalendarLiveV26RootProof[] {
  if (extract.roots.length !== 1 || sources.length !== 1)
    throw new TypeError("Calendar live v26 proof requires one independent root");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Calendar live v26 omitted ${source.source} root`);
    const tableNormalized = normalizeCalendarLiveV26Scene(
      root.calendarScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeCalendarLiveV26Scene(
      root.weekScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeCalendarLiveV26Scene(
      root.dayScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): CalendarLiveV26FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Calendar live v26 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Calendar live v26 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Calendar live v26 compile lost owned sets");
...
export function assertCalendarLiveV26RootProofs(
  proofs: readonly CalendarLiveV26RootProof[],
): void {
  if (
    proofs.length !== 1 ||
    new Set(proofs.map((proof) => proof.source)).size !== 1
  )
    throw new TypeError("Calendar live v26 one-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Calendar live v26 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildCalendarLiveV26ProbeProgram(
  writer: CalendarLiveV26WriterOwnership,
  sources: readonly CalendarLiveV26SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(CALENDAR_LIVE_V26_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("CALENDAR-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("CALENDAR-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateCalendarLiveV26ProbePayload(
  value: unknown,
  writer: CalendarLiveV26WriterOwnership,
): CalendarLiveV26ProbePayload {
  const payload = record(value, "probe payload") as CalendarLiveV26ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== CALENDAR_LIVE_V26_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V26_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Calendar live v26 probe/usability/restoration failed");
  return payload;
}

export function validateCalendarLiveV26CaptureManifest(
  cells: readonly CalendarLiveV26CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== CALENDAR_LIVE_V26_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      CALENDAR_LIVE_V26_CAPTURE_COUNT ||
    new Set(order).size !== CALENDAR_LIVE_V26_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !CALENDAR_LIVE_V26_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Calendar live v26 capture manifest is malformed");
}

export const calendarLiveV1CaptureManifestSha256 = (
  cells: readonly CalendarLiveV26CaptureCell[],
): string => {
  validateCalendarLiveV26CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildCalendarLiveV26CaptureProgram(
  writer: CalendarLiveV26WriterOwnership,
  cell: CalendarLiveV26CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= CALENDAR_LIVE_V26_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v26 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Calendar live v26 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${CALENDAR_LIVE_V26_CAPTURE_MAX_PNG_BYTES})throw new Error("CALENDAR-V5-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateCalendarLiveV26CapturePayload(
  value: unknown,
  cell: CalendarLiveV26CaptureCell,
  rawResponseBytes: number,
): CalendarLiveV26CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as CalendarLiveV26CapturePayload;
  if (
    rawResponseBytes > CALENDAR_LIVE_V26_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > CALENDAR_LIVE_V26_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Calendar live v26 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertCalendarLiveV26CaptureResponses(
  manifest: readonly CalendarLiveV26CaptureCell[],
  responses: readonly CalendarLiveV26CapturePayload[],
): void {
  validateCalendarLiveV26CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Calendar live v26 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface CalendarLiveV26ObjectiveReport {
  artifactVersion: "calendar-live-v26-objective-report-v1";
  denominator: 20;
...
export function evaluateCalendarLiveV26Objective(
  manifest: readonly CalendarLiveV26CaptureCell[],
  responses: readonly CalendarLiveV26CapturePayload[],
): CalendarLiveV26ObjectiveReport {
  assertCalendarLiveV26CaptureResponses(manifest, responses);
  return {
    artifactVersion: "calendar-live-v26-objective-report-v1",
    denominator: 20,
...
export function buildCalendarLiveV26CleanupProgram(
  writer: CalendarLiveV26WriterOwnership,
): string {
  const baseProgram = buildCalendarLiveV26CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: CALENDAR_LIVE_V26_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Calendar live v26 cleanup runtime result hook absent");
  return String.raw`
...
export function validateCalendarLiveV26CleanupPayload(
  payload: unknown,
  writer: CalendarLiveV26WriterOwnership,
): CalendarLiveV26CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as CalendarLiveV26CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Calendar live v26 cleanup is incomplete or overbroad");
  return value;
}

export const CALENDAR_LIVE_V26_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "CalendarLiveV26WriterPayload",
    cardinality: {
      pages: 1,
      sections: 1,
      sets: 3,
