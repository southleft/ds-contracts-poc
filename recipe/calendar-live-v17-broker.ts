...
import {
  assertCalendarLiveV17RootProofs,
  CALENDAR_LIVE_V17_CAPTURE_COUNT,
  CALENDAR_LIVE_V17_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  CALENDAR_LIVE_V17_RESPONSE_CONTRACTS,
  type CalendarLiveV17RootProof,
} from "./calendar-live-v17-contract.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V17_BROKER_VERSION = "calendar-live-v17-broker-v1";
export const CALENDAR_LIVE_V17_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const CALENDAR_LIVE_V17_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const CALENDAR_LIVE_V17_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const CALENDAR_LIVE_V17_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const CALENDAR_LIVE_V17_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const CALENDAR_LIVE_V17_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type CalendarLiveV17RemotePhase =
  (typeof CALENDAR_LIVE_V17_REMOTE_PHASES)[number];
export type CalendarLiveV17AuthorizationMode = "live" | "simulated";

export interface CalendarLiveV17CommitPins {
...
export interface CalendarLiveV17TransactionAuthorization extends CalendarLiveV17CommitPins {
  mode: CalendarLiveV17AuthorizationMode;
...
export interface CalendarLiveV17Manifest {
  artifactVersion: "calendar-live-v17-broker-manifest-v2";
  brokerVersion: typeof CALENDAR_LIVE_V17_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof CALENDAR_LIVE_V17_DYNAMIC_TOOL;
  target: typeof CALENDAR_LIVE_V17_TARGET;
  authorization: CalendarLiveV17TransactionAuthorization;
...
export interface CalendarLiveV17ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface CalendarLiveV17RequestBody {
  artifactVersion: "calendar-live-v17-broker-request-v2";
  brokerVersion: typeof CALENDAR_LIVE_V17_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: CalendarLiveV17RemotePhase;
  sequence: number;
  ...
  commitPins: CalendarLiveV17CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof CALENDAR_LIVE_V17_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: CalendarLiveV17ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface CalendarLiveV17Request extends CalendarLiveV17RequestBody {
...
export interface CalendarLiveV17Receipt {
  artifactVersion: "calendar-live-v17-broker-receipt-v2";
  brokerVersion: typeof CALENDAR_LIVE_V17_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: CalendarLiveV17RemotePhase;
...
export interface CalendarLiveV17AcceptedResponse<T = unknown> {
  request: CalendarLiveV17Request;
  receipt: CalendarLiveV17Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface CalendarLiveV17IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: CalendarLiveV17ResponseContract;
...
const equalTarget = (value: unknown): value is typeof CALENDAR_LIVE_V17_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === CALENDAR_LIVE_V17_TARGET.fileKey &&
  (value as BrokerTarget).fileName === CALENDAR_LIVE_V17_TARGET.fileName &&
  (value as BrokerTarget).editorType === CALENDAR_LIVE_V17_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== CALENDAR_LIVE_V17_TARGET.fileKey ||
    context.fileName !== CALENDAR_LIVE_V17_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === CALENDAR_LIVE_V17_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Calendar live v17 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= CALENDAR_LIVE_V17_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v17 capture index must be 0..7");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Calendar live v17 authorization pins are malformed");
};

const readManifest = (directory: string): CalendarLiveV17Manifest => {
  const manifest = json<CalendarLiveV17Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Calendar live v17 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "calendar-live-v17-broker-manifest-v2" ||
    manifest.brokerVersion !== CALENDAR_LIVE_V17_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== CALENDAR_LIVE_V17_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(CALENDAR_LIVE_V17_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Calendar live v17 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: CalendarLiveV17Request,
): CalendarLiveV17RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: CalendarLiveV17RemotePhase,
  captureIndex?: number,
): CalendarLiveV17Request | undefined => {
  const file = calendarLiveV1RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV17Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: CalendarLiveV17RemotePhase,
  captureIndex?: number,
): CalendarLiveV17Receipt | undefined => {
  const file = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV17Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: CalendarLiveV17Request | undefined;
  receipt: CalendarLiveV17Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: CalendarLiveV17RemotePhase,
): CalendarLiveV17ResponseContract => ({
  ...CALENDAR_LIVE_V17_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: CalendarLiveV17Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: CALENDAR_LIVE_V17_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: CALENDAR_LIVE_V17_DYNAMIC_TOOL,
      target: CALENDAR_LIVE_V17_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "calendar-live-v17-plugin-response-v2",
  brokerVersion: CALENDAR_LIVE_V17_BROKER_VERSION,
  target: CALENDAR_LIVE_V17_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*CALENDAR_LIVE_V17_PROGRAM_START*/${program}/*CALENDAR_LIVE_V17_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*CALENDAR_LIVE_V17_PROGRAM_START*/";
  const endMarker = "/*CALENDAR_LIVE_V17_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Calendar live v17 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createCalendarLiveV17Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Calendar live v17 private signing key does not match authorized public-key identity",
    );
  const manifest: CalendarLiveV17Manifest = {
    artifactVersion: "calendar-live-v17-broker-manifest-v2",
    brokerVersion: CALENDAR_LIVE_V17_BROKER_VERSION,
    ...
    captureRequests: CALENDAR_LIVE_V17_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateCalendarLiveV17Request(
  directory: string,
  request: CalendarLiveV17Request,
): void {
  ...
  if (
    request.artifactVersion !== "calendar-live-v17-broker-request-v2" ||
    request.brokerVersion !== CALENDAR_LIVE_V17_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== CALENDAR_LIVE_V17_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Calendar live v17 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueCalendarLiveV17Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: CalendarLiveV17RemotePhase,
  program: string,
  options: CalendarLiveV17IssueOptions = {},
): CalendarLiveV17Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<CalendarLiveV17Request>(targetPath);
    validateCalendarLiveV17Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Calendar live v17 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(calendarLiveV1ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Calendar live v17 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Calendar live v17 empty code envelope refused");
  if (phase === "capture") validateCalendarLiveV17TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Calendar live v17 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: CALENDAR_LIVE_V17_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? CALENDAR_LIVE_V17_SIGNED_WRITER_TIMEOUT_MS
        : CALENDAR_LIVE_V17_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: CalendarLiveV17RequestBody = {
    artifactVersion: "calendar-live-v17-broker-request-v2",
    brokerVersion: CALENDAR_LIVE_V17_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: CalendarLiveV17Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateCalendarLiveV17Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistCalendarLiveV17RawResponse(
  directory: string,
  phase: CalendarLiveV17RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Calendar live v17 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<CalendarLiveV17Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Calendar live v17 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = calendarLiveV1ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Calendar live v17 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): CalendarLiveV17Receipt => {
  const body: Omit<CalendarLiveV17Receipt, "receiptSha256"> = {
    artifactVersion: "calendar-live-v17-broker-receipt-v2",
    brokerVersion: CALENDAR_LIVE_V17_BROKER_VERSION,
...
export function acceptCalendarLiveV17Response<T = unknown>(
  directory: string,
  phase: CalendarLiveV17RemotePhase,
  options: {
    captureIndex?: number;
...
): CalendarLiveV17AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Calendar live v17 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<CalendarLiveV17Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Calendar live v17 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "calendar-live-v17-plugin-response-v2" ||
    plugin?.brokerVersion !== CALENDAR_LIVE_V17_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Calendar live v17 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistCalendarLiveV17TechnicalGates(
  directory: string,
  rootProofs: readonly CalendarLiveV17RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertCalendarLiveV17RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Calendar live v17 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Calendar live v17 probe payload hash malformed");
  const gates = Object.fromEntries(
    CALENDAR_LIVE_V17_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "calendar-live-v17-host-technical-gates-v2",
    brokerVersion: CALENDAR_LIVE_V17_BROKER_VERSION,
...
export function validateCalendarLiveV17TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "calendar-live-v17-host-technical-gates-v2" ||
    ...
    CALENDAR_LIVE_V17_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Calendar live v17 technical gates artifact");
}

export function validateCalendarLiveV17Receipt(
  directory: string,
  phase: CalendarLiveV17RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateCalendarLiveV17Request(directory, request);
  const receipt = json<CalendarLiveV17Receipt>(
  ...
  if (
    receipt.artifactVersion !== "calendar-live-v17-broker-receipt-v2" ||
    receipt.brokerVersion !== CALENDAR_LIVE_V17_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Calendar live v17 accepted receipt: ${request.requestId}`,
    );
}

export function readCalendarLiveV17AcceptedRawPayload(
  directory: string,
  phase: CalendarLiveV17RemotePhase,
  captureIndex?: number,
): unknown {
  validateCalendarLiveV17Receipt(directory, phase, captureIndex);
...
export function createCalendarLiveV17FakeDynamicResponse(
  request: CalendarLiveV17Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? CALENDAR_LIVE_V17_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? CALENDAR_LIVE_V17_TARGET,
    ...overrides.extra,
  };
}

export function listCalendarLiveV17PersistedRequests(
  directory: string,
): CalendarLiveV17Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<CalendarLiveV17Request>(file)] : [];
    });
}
