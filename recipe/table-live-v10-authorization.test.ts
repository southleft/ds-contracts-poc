...
import { TABLE_LIVE_V10_INDEX_PATH } from "./build-table-live-proof-v10.js";
import {
...
import {
  ...
  validateTableLiveV10History,
  validateTableLiveV10UnitTestSource,
  type TableLiveV10AntecedentIndex,
  type TableLiveV10AuthorizationArtifact,
  type TableLiveV10AuthorizationProof,
  type TableLiveV10HistoryState,
} from "./table-live-v10-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV10ControlFlowSource,
  validateTableLiveV10SecurityAttestation,
  type TableLiveV10SecurityAttestation,
} from "./table-live-v10-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V10_INDEX_PATH);
const index = (): TableLiveV10AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV10AntecedentIndex;

const artifact = (): TableLiveV10AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV10AuthorizationArtifact({
...
const pendingState = (): TableLiveV10HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV10HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV10AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v10/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV10AuthorizationArtifact,
): TableLiveV10SecurityAttestation => {
  const body: Omit<TableLiveV10SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V10_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV10History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV10History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV10History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV10History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV10History(authorizedState(), "pending").join("\n"),
    /stale history mode.*authorization exists/,
  );
});

test("the exact v6 phase-sensitive self-test defect is planted and table v10 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV10UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v10-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV10UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV10AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV10AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV10AuthorizationArtifact(
        planted as TableLiveV10AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV10SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV10SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV10SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v10-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV10ControlFlowSource(source), []);
});
