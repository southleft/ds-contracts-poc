...
import { COMBOBOX_LIVE_V28_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v28.js";
import { verifyComboboxLiveV28Authorization } from "./combobox-live-v28-authorization.js";
import { runComboboxLiveV28Preflight } from "./combobox-live-v28-preflight.js";
import { ComboboxLiveV28Orchestrator } from "./run-combobox-live-v28.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v28 requires exact authorization artifact ${COMBOBOX_LIVE_V28_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v28-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV28Authorization(root);
    runComboboxLiveV28Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV28Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV28Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
