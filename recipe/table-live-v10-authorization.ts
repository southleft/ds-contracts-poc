...
import {
  TABLE_LIVE_V10_DYNAMIC_TOOL,
  TABLE_LIVE_V10_TARGET,
  type TableLiveV10TransactionAuthorization,
} from "./table-live-v10-broker.js";
import {
...
import {
  ...
  TABLE_LIVE_V10_HOST_PHASES,
  TABLE_LIVE_V10_REMOTE_REQUESTS,
  TABLE_LIVE_V10_SOURCE_ROOTS,
} from "./table-live-v10-contract.js";
import { canonicalJson } from "./normalize.js";

export type TableLiveV10HistoryExpectation = "pending" | "authorized";

export interface TableLiveV10AuthorizationArtifact {
  artifactVersion: "table-live-v10-capture-authorization-v1";
  authorizationId: "table-live-v10";
  status: "authorization declared; runtime security prerequisites still mandatory";
  authorizationIntent: true;
  antecedent: {
    commit: string;
    indexPath: typeof TABLE_LIVE_V10_INDEX_PATH;
    indexSha256: string;
...
export interface TableLiveV10AntecedentIndex {
  artifactVersion: "table-live-v10-antecedent-index-v1";
  status: string;
...
export interface TableLiveV10HistoryState {
  index?: TableLiveV10AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: TableLiveV10AuthorizationArtifact;
...
export interface TableLiveV10AuthorizationProof
  extends TableLiveV10TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof TABLE_LIVE_V10_TARGET;
  expectedDynamicTool: typeof TABLE_LIVE_V10_DYNAMIC_TOOL;
  authorizationPath: typeof TABLE_LIVE_V10_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === TABLE_LIVE_V10_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("table-live-v10-authorization") ||
  artifactPath.includes("table-live-v10-preflight") ||
  artifactPath.includes("table-live-v10-authorized") ||
  artifactPath.includes("create-table-live-v10-security-attestation") ||
  artifactPath.includes("table-live-pivot-v10-status") ||
...
export function validateTableLiveV10AntecedentIndex(
  index: TableLiveV10AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "table-live-v10-antecedent-index-v1" ||
    index.authorizationCanBeAddedWithoutAntecedentRebuild !== true
  )
    failures.push("table v10 antecedent index missing or malformed");
  const artifactEntries = Object.entries(index?.artifacts ?? {});
  if (artifactEntries.length === 0)
    failures.push("table v10 antecedent hash denominator is empty");
  for (const [artifactPath, metadata] of artifactEntries) {
    ...
    if (
      ...
      failures.push(`malformed table v10 antecedent hash: ${artifactPath}`);
  }
  ...
  if (index?.hashSetSha256 !== expectedHashSet)
    failures.push("table v10 antecedent hash-set digest mismatch");
  if (
    ...
    failures.push("table v10 antecedent counts missing or drifted");
  for (const required of [
    TABLE_LIVE_V10_AUTHORIZATION_PATH,
    "recipe/table-live-v10-authorization.ts",
    "recipe/table-live-v10-authorization.test.ts",
    "recipe/table-live-v10-preflight.ts",
    "recipe/evidence/table-live-pivot-v10/operator-security-attestation-template.json",
    "recipe/evidence/status-index.json",
  ])
    if (!index?.authorizationLifecycleExcluded?.includes(required))
      failures.push(`table v10 lifecycle exclusion missing: ${required}`);
  const leaks = forbiddenAntecedentFieldPaths(index);
  if (leaks.length)
    failures.push(`table v10 antecedent result leakage: ${leaks.join(",")}`);
  return failures;
}

export function validateTableLiveV10UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyTableLiveV10History\(\s*["']/.test(source) ||
    /readTableLiveV10HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildTableLiveV10AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): TableLiveV10AuthorizationArtifact {
  if (!SHA40.test(options.antecedentCommit))
    throw new TypeError("table v10 antecedent commit must be a full SHA");
  const index = JSON.parse(
    Buffer.from(options.antecedentIndexBytes).toString("utf8"),
  ) as TableLiveV10AntecedentIndex;
  const indexFailures = validateTableLiveV10AntecedentIndex(index);
  if (indexFailures.length)
    throw new TypeError(
      `table v10 antecedent index refused:\n${indexFailures.join("\n")}`,
    );
  const publicKey =
  ...
  return {
    ...
    antecedent: {
      ...
      indexPath: TABLE_LIVE_V10_INDEX_PATH,
    ...
    denominator: {
      remoteRequests: TABLE_LIVE_V10_REMOTE_REQUESTS,
      hostPhases: TABLE_LIVE_V10_HOST_PHASES,
      captures: TABLE_LIVE_V10_CAPTURE_COUNT,
      sourceRoots: TABLE_LIVE_V10_SOURCE_ROOTS,
...
export function validateTableLiveV10AuthorizationArtifact(
  artifact: TableLiveV10AuthorizationArtifact | undefined,
  index: TableLiveV10AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "table-live-v10-capture-authorization-v1" ||
    artifact.authorizationId !== "table-live-v10" ||
    artifact.authorizationIntent !== true
  )
    failures.push("table v10 authorization declaration missing or malformed");
  const resultLeaks = forbiddenAntecedentFieldPaths(artifact);
  if (resultLeaks.length)
    failures.push(`table v10 authorization result leakage: ${resultLeaks.join(",")}`);
  if (
    artifact?.antecedent?.indexPath !== TABLE_LIVE_V10_INDEX_PATH ||
    artifact?.antecedent?.indexSha256 !== indexSha256 ||
    artifact?.antecedent?.hashSetSha256 !== index?.hashSetSha256
  )
    failures.push("table v10 authorization pins wrong antecedent index");
  if (privateMaterialPaths(artifact).length)
    failures.push("table v10 authorization contains private key or token material");
  try {
    ...
    if (
      ...
      failures.push("table v10 Ed25519 public-key identity mismatch");
  } catch {
    failures.push("table v10 Ed25519 public key is invalid");
  }
  if (
    !equalJson(artifact?.operatorBoundary?.target, TABLE_LIVE_V10_TARGET) ||
    !equalJson(
    ...
    failures.push("wrong table v10 target or dynamic tool");
  if (
    artifact?.denominator?.remoteRequests !== TABLE_LIVE_V10_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== TABLE_LIVE_V10_HOST_PHASES ||
    artifact?.denominator?.captures !== TABLE_LIVE_V10_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== TABLE_LIVE_V10_SOURCE_ROOTS ||
    artifact?.denominator?.expectedFacts !== index?.counts.expectedSceneFacts
  )
    failures.push("table v10 two-root or capture denominator changed");
  if (
    ...
    failures.push(
      "table v10 execution, capture, cleanup, or prior-lineage reuse policy weakened",
    );
  if (
    ...
    failures.push("table v10 security prerequisites weakened");
  if (
    artifact?.humanSignoff?.mandatory !== true ||
    artifact?.humanSignoff?.status !== "pending"
  )
    failures.push("table v10 human signoff overclaim");
  return failures;
}

export function validateTableLiveV10History(
  state: TableLiveV10HistoryState,
  expected: TableLiveV10HistoryExpectation,
): string[] {
  const failures = validateTableLiveV10AntecedentIndex(state.index);
  if (!SHA40.test(state.codeCommit))
    failures.push("table v10 code commit is not a full SHA");
  ...
  if (
    ...
    failures.push("table v10 antecedent first-add commit is not unique");
  if (!state.antecedentIsAncestorOfCode)
    failures.push("code commit does not descend from table v10 antecedent");
  if (!state.antecedentIndexBytesMatchFirstAddition)
    failures.push("table v10 antecedent index changed after first addition");
  if (!state.antecedentArtifactsMatchIndexAtCommit)
    failures.push("table v10 antecedent commit artifacts do not match index");
  if (!state.workingAntecedentArtifactsMatchIndex)
    failures.push("table v10 antecedent working bytes drifted");
  ...
  failures.push(
    ...validateTableLiveV10AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
  ...
  if (
    ...
    failures.push("table v10 authorization first-add lineage mismatch");
  if (!state.authorizationPresentAtCodeCommit)
    failures.push("table v10 authorization missing from code commit");
  if (!state.authorizationBytesMatchFirstAddition)
    failures.push("table v10 authorization bytes changed after first addition");
  if (!state.authorizationStrictlyDescendsFromAntecedent)
    failures.push("table v10 authorization does not strictly descend from antecedent");
  if (!state.authorizationIsAncestorOfCode)
    failures.push("code commit predates table v10 authorization");
  return failures;
}
...
export function readTableLiveV10HistoryState(
  root = process.cwd(),
): TableLiveV10HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, TABLE_LIVE_V10_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as TableLiveV10AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${TABLE_LIVE_V10_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${TABLE_LIVE_V10_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${TABLE_LIVE_V10_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyTableLiveV10History(
  expected: TableLiveV10HistoryExpectation,
  root = process.cwd(),
): TableLiveV10HistoryState {
  const state = readTableLiveV10HistoryState(root);
  const failures = validateTableLiveV10History(state, expected);
  if (failures.length)
    throw new Error(
      `Table live v10 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyTableLiveV10Authorization(
  root = process.cwd(),
): TableLiveV10AuthorizationProof {
  const state = verifyTableLiveV10History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, TABLE_LIVE_V10_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/table-live-pivot-v10/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-table-live-v10.ts"]!.sha256,
    ...
    target: TABLE_LIVE_V10_TARGET,
    expectedDynamicTool: TABLE_LIVE_V10_DYNAMIC_TOOL,
    authorizationPath: TABLE_LIVE_V10_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): TableLiveV10HistoryExpectation => {
  ...
  if (pending === authorized)
    throw new Error(
      "Choose exactly one table v10 history mode: --expect-pending or --expect-authorized",
    );
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyTableLiveV10History(expected);
  process.stdout.write(
    `Table live v10 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
