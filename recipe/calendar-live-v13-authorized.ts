...
import { CALENDAR_LIVE_V13_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v13.js";
import { verifyCalendarLiveV13Authorization } from "./calendar-live-v13-authorization.js";
import { runCalendarLiveV13Preflight } from "./calendar-live-v13-preflight.js";
import { CalendarLiveV13Orchestrator } from "./run-calendar-live-v13.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v13 requires exact authorization artifact ${CALENDAR_LIVE_V13_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v13-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV13Authorization(root);
    runCalendarLiveV13Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV13Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV13Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
