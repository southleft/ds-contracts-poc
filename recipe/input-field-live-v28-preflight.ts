...
import {
  ...
  verifyInputLiveV28Authorization,
  type InputLiveV28AntecedentIndex,
  type InputLiveV28AuthorizationProof,
} from "./input-field-live-v28-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V28_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v28/operator-security-attestation-template.json";
export const INPUT_LIVE_V28_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v28-security-attestation.json";

export interface InputLiveV28SecurityAttestation {
...
export interface InputLiveV28PreflightReport {
  artifactVersion: "input-live-v28-preflight-v1";
...
export const inputLiveV28AttestationSha256 = (
  value: Omit<InputLiveV28SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV28SecurityAttestation(
  value: unknown,
  proof: InputLiveV28AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v28-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V28_TARGET) ||
...
export function validateInputLiveV28ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV28Authorization");
  const preflight = source.indexOf("runInputLiveV28Preflight");
  const initialize = source.indexOf("InputLiveV28Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV28Preflight(
  root: string,
  proof: InputLiveV28AuthorizationProof,
  attestationPath = INPUT_LIVE_V28_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV28PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V28_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V28_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V28_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV28AntecedentIndex;
  failures.push(...validateInputLiveV28AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V28_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V28_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v28-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV28ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV28SecurityAttestation;
  return {
    artifactVersion: "input-live-v28-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V28_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV28Preflight(
    process.cwd(),
    verifyInputLiveV28Authorization(),
    attestationPath,
