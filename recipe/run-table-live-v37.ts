...
import {
  acceptTableLiveV37Response,
  createTableLiveV37Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV37Request,
  persistTableLiveV37RawResponse,
  persistTableLiveV37TechnicalGates,
  readTableLiveV37AcceptedRawPayload,
  type TableLiveV37Request,
  type TableLiveV37TransactionAuthorization,
} from "./table-live-v37-broker.js";
import {
  TABLE_LIVE_V37_CAPTURE_COUNT,
  assertTableLiveV37CaptureResponses,
  ...
  evaluateTableLiveV37Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV37Roots,
  validateTableLiveV37CaptureManifest,
  validateTableLiveV37CapturePayload,
  validateTableLiveV37CleanupPayload,
  validateTableLiveV37ExtractPayload,
  validateTableLiveV37ProbePayload,
  validateTableLiveV37WriterPayload,
  type TableLiveV37CaptureCell,
  type TableLiveV37CapturePayload,
...
import {
  ...
  TABLE_LIVE_V37_PLAN_PATH,
  type TableLiveV37ProofPlan,
} from "./build-table-live-proof-v37.js";
import {
...
export const TABLE_LIVE_V37_RUNNER_VERSION = "table-live-v37-runner-v1";

interface TableLiveV37JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV37OrchestratorState {
  artifactVersion: "table-live-v37-orchestrator-state-v1";
  ...
  journal: TableLiveV37JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV37ProofPlan;
  planSha256: string;
  captures: TableLiveV37CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV37SourceIdentity & {
...
export interface TableLiveV37NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV37OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v37 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V37_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V37_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV37ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV37CaptureCell[];
  validateTableLiveV37CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v37 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v37 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV37WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV37RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV37Request =>
  readJson<TableLiveV37Request>(file);

export class TableLiveV37Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV37Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV37OrchestratorState = {
      artifactVersion: "table-live-v37-orchestrator-state-v1",
    ...
    return new TableLiveV37Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV37OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV37Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV37NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV37NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v37 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV37NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v37 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV37RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV37Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV37CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV37AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV37WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV37Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV37ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV37Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV37Response(this.#directory, "writer", {
        validate: validateTableLiveV37WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV37Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV37RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV37Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV37ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV37Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV37RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v37-host-root-proofs-v1",
      ...
      const next = issueTableLiveV37Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV37ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV37Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV37ProbePayload(payload, ownership),
      });
      persistTableLiveV37TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v37 capture cell missing");
      const accepted = acceptTableLiveV37Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV37CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V37_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV37CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV37Objective(
    ...
    throw new Error(`Table live v37 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV37Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV37CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV37Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV37RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV37Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV37CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV37CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV37CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV37Authorization = (
  privateKey: KeyObject,
): TableLiveV37TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV37SimulationKey = (): KeyObject =>
