...
export interface ComboboxLiveV12LifecycleSimulationReport {
  artifactVersion: "combobox-live-v12-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildComboboxLiveV12AuthorizationArtifact> | undefined,
...
export async function simulateComboboxLiveV12Lifecycle(): Promise<ComboboxLiveV12LifecycleSimulationReport> {
  await buildComboboxLiveV12Proof(true);
  const indexBefore = readFileSync(COMBOBOX_LIVE_V12_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as ComboboxLiveV12AntecedentIndex;
  const indexSha256 = comboboxLiveV5Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "combobox-live-v12-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, COMBOBOX_LIVE_V12_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", COMBOBOX_LIVE_V12_INDEX_PATH]);
    ...
    const pendingFailures = validateComboboxLiveV12History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v1 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateComboboxLiveV12History(pending, "authorized");
    if (
    ...
    const authorization = buildComboboxLiveV12AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", COMBOBOX_LIVE_V12_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${COMBOBOX_LIVE_V12_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateComboboxLiveV12History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateComboboxLiveV12History(authorized, "pending");
    ...
    await buildComboboxLiveV12Proof(true);
    const indexAfter = readFileSync(COMBOBOX_LIVE_V12_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as ComboboxLiveV12AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateComboboxLiveV12Lifecycle(), null, 2)}\n`,
  );
