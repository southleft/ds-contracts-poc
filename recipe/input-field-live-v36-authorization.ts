...
import {
  ...
  INPUT_LIVE_V36_INDEX_PATH,
} from "./build-input-field-live-proof-v36.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV36HistoryExpectation = "pending" | "authorized";

export interface InputLiveV36AuthorizationArtifact {
  artifactVersion: "input-live-v36-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V36_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV36AntecedentIndex {
...
export interface InputLiveV36HistoryState {
  index?: InputLiveV36AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV36AuthorizationArtifact;
...
export interface InputLiveV36AuthorizationProof extends InputLiveV36TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V36_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V36_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V36_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V36_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v36-status") ||
...
export function validateInputLiveV36AntecedentIndex(
  index: InputLiveV36AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v36-antecedent-index-v1" ||
...
export function validateInputLiveV36UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV36History\(\s*["']/.test(source) ||
    /readInputLiveV36HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV36AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV36AuthorizationArtifact {
  ...
  ) as InputLiveV36AntecedentIndex;
  const indexFailures = validateInputLiveV36AntecedentIndex(index);
...
export function validateInputLiveV36AuthorizationArtifact(
  artifact: InputLiveV36AuthorizationArtifact | undefined,
  index: InputLiveV36AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v36-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v36" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V36_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V36_TARGET) ||
    !equalJson(
...
export function validateInputLiveV36History(
  state: InputLiveV36HistoryState,
  expected: InputLiveV36HistoryExpectation,
): string[] {
  const failures = validateInputLiveV36AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV36AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV36HistoryState(
  root = process.cwd(),
): InputLiveV36HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V36_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV36AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V36_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V36_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V36_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV36History(
  expected: InputLiveV36HistoryExpectation,
  root = process.cwd(),
): InputLiveV36HistoryState {
  const state = readInputLiveV36HistoryState(root);
  const failures = validateInputLiveV36History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV36Authorization(
  root = process.cwd(),
): InputLiveV36AuthorizationProof {
  const state = verifyInputLiveV36History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V36_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v36/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v36.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V36_TARGET,
    expectedDynamicTool: INPUT_LIVE_V36_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V36_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV36HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV36History(expected);
