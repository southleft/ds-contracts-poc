...
import {
  assertInputLiveV77RootProofs,
  INPUT_LIVE_V77_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V77_RESPONSE_CONTRACTS,
  type InputLiveV77RootProof,
} from "./input-field-live-v77-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V77_BROKER_VERSION = "input-live-v77-broker-v2";
export const INPUT_LIVE_V77_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V77_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V77_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V77_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V77_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V77_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV77RemotePhase =
  (typeof INPUT_LIVE_V77_REMOTE_PHASES)[number];
export type InputLiveV77AuthorizationMode = "live" | "simulated";

export interface InputLiveV77CommitPins {
...
export interface InputLiveV77TransactionAuthorization extends InputLiveV77CommitPins {
  mode: InputLiveV77AuthorizationMode;
...
export interface InputLiveV77Manifest {
  artifactVersion: "input-live-v77-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V77_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V77_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V77_TARGET;
  authorization: InputLiveV77TransactionAuthorization;
...
export interface InputLiveV77ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV77RequestBody {
  artifactVersion: "input-live-v77-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V77_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV77RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV77CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V77_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV77ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV77Request extends InputLiveV77RequestBody {
...
export interface InputLiveV77Receipt {
  artifactVersion: "input-live-v77-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V77_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV77RemotePhase;
...
export interface InputLiveV77AcceptedResponse<T = unknown> {
  request: InputLiveV77Request;
  receipt: InputLiveV77Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV77IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV77ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V77_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V77_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V77_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V77_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V77_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V77_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V77_TARGET.editorType;
};
...
export const inputLiveV77RequestSequence = (
  phase: InputLiveV77RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV77RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV77RequestPath = (
  directory: string,
  phase: InputLiveV77RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV77ResponsePath = (
  directory: string,
  phase: InputLiveV77RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV77ReceiptPath = (
  directory: string,
  phase: InputLiveV77RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV77Manifest => {
  const manifest = json<InputLiveV77Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v77-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V77_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V77_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV77RemotePhase,
  captureIndex?: number,
): InputLiveV77Request | undefined => {
  const file = inputLiveV77RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV77Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV77RemotePhase,
  captureIndex?: number,
): InputLiveV77Receipt | undefined => {
  const file = inputLiveV77ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV77Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV77Request | undefined;
  receipt: InputLiveV77Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV77RemotePhase,
): InputLiveV77ResponseContract => ({
  ...INPUT_LIVE_V77_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V77_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v77-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V77_BROKER_VERSION,
  target: INPUT_LIVE_V77_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V77_PROGRAM_START*/${program}/*INPUT_LIVE_V77_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V77_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V77_PROGRAM_END*/";
...
export function createInputLiveV77Transaction(
  directory: string,
  ...
  const manifest: InputLiveV77Manifest = {
    artifactVersion: "input-live-v77-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V77_BROKER_VERSION,
...
export function validateInputLiveV77Request(
  directory: string,
  request: InputLiveV77Request,
): void {
  ...
  const expectedSequence = inputLiveV77RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v77-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V77_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V77_TARGET.fileKey ||
...
export function issueInputLiveV77Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV77RemotePhase,
  program: string,
  options: InputLiveV77IssueOptions = {},
): InputLiveV77Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV77RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV77Request>(targetPath);
    validateInputLiveV77Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV77ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV77TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV77RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V77_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V77_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V77_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV77RequestBody = {
    artifactVersion: "input-live-v77-broker-request-v2",
    brokerVersion: INPUT_LIVE_V77_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV77Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV77Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV77RawResponse(
  directory: string,
  phase: InputLiveV77RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV77RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV77Request>(requestFile);
  if (
  ...
  const target = inputLiveV77ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV77Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v77-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V77_BROKER_VERSION,
...
export function acceptInputLiveV77Response<T = unknown>(
  directory: string,
  phase: InputLiveV77RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV77AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV77ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV77Request>(
    inputLiveV77RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV77Request(directory, request);
  if (phase === "capture") validateInputLiveV77TechnicalGates(directory);
  const rawFile = inputLiveV77ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v77-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V77_BROKER_VERSION ||
...
export function persistInputLiveV77TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV77RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV77RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V77_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v77-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V77_BROKER_VERSION,
...
export function validateInputLiveV77TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v77-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V77_TECHNICAL_GATES.some(
...
export function validateInputLiveV77Receipt(
  directory: string,
  phase: InputLiveV77RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV77Request(directory, request);
  const receipt = json<InputLiveV77Receipt>(
    inputLiveV77ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV77ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v77-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V77_BROKER_VERSION ||
...
export function readInputLiveV77AcceptedRawPayload(
  directory: string,
  phase: InputLiveV77RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV77Receipt(directory, phase, captureIndex);
...
export function createInputLiveV77FakeDynamicResponse(
  request: InputLiveV77Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V77_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V77_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV77PersistedRequests(
  directory: string,
): InputLiveV77Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV77Request>(file)] : [];
    });
}

export const inputLiveV77Sha256 = sha256;
