...
import { verifyCalendarLiveV33Authorization } from "./calendar-live-v33-authorization.js";
import { CALENDAR_LIVE_V33_TARGET } from "./calendar-live-v33-broker.js";
import {
  CALENDAR_LIVE_V33_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV33Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v33-session";
...
const attestation: CalendarLiveV33SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV33SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v33 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V33_SECURITY_ATTESTATION_DEFAULT_PATH,
);
