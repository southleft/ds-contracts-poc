...
import {
  ...
  createInputLiveV61FakeDynamicResponse,
  ...
  listInputLiveV61PersistedRequests,
  persistInputLiveV61RawResponse,
  persistInputLiveV61TechnicalGates,
  validateInputLiveV61Receipt,
  validateInputLiveV61Request,
  type InputLiveV61RemotePhase,
...
import {
  ...
  type InputLiveV61ExtractPayload,
  type InputLiveV61ProbePayload,
  type InputLiveV61RestorePayload,
  type InputLiveV61RootProof,
  type InputLiveV61WriterOwnership,
} from "./input-field-live-v61-contract.js";
import {
  InputLiveV61Orchestrator,
  simulatedInputLiveV61Authorization,
} from "./run-input-field-live-v61.js";
...
const captures: InputLiveV61CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV61Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v61-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV61Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV61CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV61FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV61FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V61_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V61_TARGET);
  persistInputLiveV61RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV61Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV61Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV61WriterOwnership,
): InputLiveV61RestorePayload => ({
...
const rootProofs = (): InputLiveV61RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV61CaptureCell,
): InputLiveV61CapturePayload => {
  const bytes = Buffer.from(`input-live-v61-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV61Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV61Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV61RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV61RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV61ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV61ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV61CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV61CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV61CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV61Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV61CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV61PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV61ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV61Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV61Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV61Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV61Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V61_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV61RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V61_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV61Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV61Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV61ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV61Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV61WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV61RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV61Request;
    validateInputLiveV61Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV61Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV61CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v61-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV61Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV61Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV61Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V61_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV61RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V61_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV61Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV61Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV61RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V61_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV61Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV61Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV61Request;
