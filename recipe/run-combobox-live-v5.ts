...
import {
  acceptComboboxLiveV5Response,
  createComboboxLiveV5Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV5Request,
  persistComboboxLiveV5RawResponse,
  persistComboboxLiveV5TechnicalGates,
  readComboboxLiveV5AcceptedRawPayload,
  type ComboboxLiveV5Request,
  type ComboboxLiveV5TransactionAuthorization,
} from "./combobox-live-v5-broker.js";
import {
  COMBOBOX_LIVE_V5_CAPTURE_COUNT,
  assertComboboxLiveV5CaptureResponses,
  ...
  evaluateComboboxLiveV5Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV5Roots,
  validateComboboxLiveV5CaptureManifest,
  validateComboboxLiveV5CapturePayload,
  validateComboboxLiveV5CleanupPayload,
  validateComboboxLiveV5ExtractPayload,
  validateComboboxLiveV5ProbePayload,
  validateComboboxLiveV5WriterPayload,
  type ComboboxLiveV5CaptureCell,
  type ComboboxLiveV5CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V5_PLAN_PATH,
  type ComboboxLiveV5ProofPlan,
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v5.js";

export const COMBOBOX_LIVE_V5_RUNNER_VERSION = "combobox-live-v5-runner-v1";

interface ComboboxLiveV5JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV5OrchestratorState {
  artifactVersion: "combobox-live-v5-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV5JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV5ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV5CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV5SourceIdentity & {
...
export interface ComboboxLiveV5NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV5OrchestratorState,
): void => {
...
const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V5_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V5_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV5ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV5CaptureCell[];
  validateComboboxLiveV5CaptureManifest(captures);
  if (
...
const writerOwnership = (directory: string): ComboboxLiveV5WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV5RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV5Request =>
  readJson<ComboboxLiveV5Request>(file);

export class ComboboxLiveV5Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV5Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV5OrchestratorState = {
      artifactVersion: "combobox-live-v5-orchestrator-state-v1",
    ...
    return new ComboboxLiveV5Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV5OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV5Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV5NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV5NextAction {
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV5NextAction {
    ...
    persistComboboxLiveV5RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV5Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV5CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV5AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV5WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV5Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV5ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV5Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV5Response(this.#directory, "writer", {
        validate: validateComboboxLiveV5WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV5Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV5RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV5Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV5ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV5Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV5RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v5-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV5Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV5ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV5Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV5ProbePayload(payload, ownership),
      });
      persistComboboxLiveV5TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      const accepted = acceptComboboxLiveV5Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV5CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V5_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV5CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV5Objective(
  ...
  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV5Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV5CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV5Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV5RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV5Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV5CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV5CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV5CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV5Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV5TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV5SimulationKey = (): KeyObject =>
