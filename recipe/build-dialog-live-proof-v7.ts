...
import { muiDialogAdapterConfig as muiDialogProposedConfig, muiDialogSource as muiDialogProposedSource } from "./fixtures/generated/dialog.mui.js";
import { chakraDialogAdapterConfig as chakraDialogProposedConfig, chakraDialogSource as chakraDialogProposedSource } from "./fixtures/generated/dialog.chakra.js";
import { compileDialogRecipe, dialogRecipe } from "./recipes/dialog.js";

const EVIDENCE = "recipe/evidence/dialog-live-pivot-v7";
...
const sources = [
  ...
  {
    // PROPOSED from MUI's own title + body capture (17 read, 0 invented).
    adapterIdentity: "mui-dialog-proposed-v1",
    displayName: "MUI (proposed)",
    source: muiDialogProposedSource,
    config: muiDialogProposedConfig,
  },
  {
    // HELD OUT: Chakra's Dialog, captured today through a Portal composition (17 read, 0 invented).
    adapterIdentity: "chakra-dialog-proposed-v1",
    displayName: "Chakra (proposed)",
    source: chakraDialogProposedSource,
    config: chakraDialogProposedConfig,
  },
].map((entry) => {
...
const receiptOwned = {
      artifactVersion: "dialog-live-pivot-v7-prepare",
      teaching:
...
if (process.argv.includes("--check")) {
  ...
  if (writer.sourcePlans.length !== 5)
    throw new Error("dialog prepare requires 5 library sources (reviewed + proposed)");
  console.log(
    JSON.stringify({
      check: "ok",
      runIdentity: writer.runIdentity,
