...
import { INPUT_LIVE_V11_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v11.js";
import { verifyInputLiveV11Authorization } from "./input-field-live-v11-authorization.js";
import { runInputLiveV11Preflight } from "./input-field-live-v11-preflight.js";
import { InputLiveV11Orchestrator } from "./run-input-field-live-v11.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v11 requires exact authorization artifact ${INPUT_LIVE_V11_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v11-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV11Authorization(root);
    runInputLiveV11Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV11Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
