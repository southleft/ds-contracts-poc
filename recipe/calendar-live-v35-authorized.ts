...
import { CALENDAR_LIVE_V35_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v35.js";
import { verifyCalendarLiveV35Authorization } from "./calendar-live-v35-authorization.js";
import { runCalendarLiveV35Preflight } from "./calendar-live-v35-preflight.js";
import { CalendarLiveV35Orchestrator } from "./run-calendar-live-v35.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v35 requires exact authorization artifact ${CALENDAR_LIVE_V35_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v35-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV35Authorization(root);
    runCalendarLiveV35Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV35Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV35Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
