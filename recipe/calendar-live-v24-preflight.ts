...
import {
  validateCalendarLiveV24AntecedentIndex,
  verifyCalendarLiveV24Authorization,
  type CalendarLiveV24AntecedentIndex,
  type CalendarLiveV24AuthorizationProof,
} from "./calendar-live-v24-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V24_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v24/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V24_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v24-security-attestation.json";

export interface CalendarLiveV24SecurityAttestation {
  artifactVersion: "calendar-live-v24-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface CalendarLiveV24PreflightReport {
  artifactVersion: "calendar-live-v24-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV24SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV24SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV24AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v24-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V24_TARGET) ||
...
export function validateCalendarLiveV24ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV24Authorization");
  const preflight = source.indexOf("runCalendarLiveV24Preflight");
  const initialize = source.indexOf("CalendarLiveV24Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV24Preflight(
  root: string,
  proof: CalendarLiveV24AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V24_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV24PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V24_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V24_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV24AntecedentIndex;
  failures.push(...validateCalendarLiveV24AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V24_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V24_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateCalendarLiveV24SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v24-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV24ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v24 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV24SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v24-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V24_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV24Preflight(
    process.cwd(),
    verifyCalendarLiveV24Authorization(),
    attestationPath,
