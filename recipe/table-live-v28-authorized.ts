...
import { TABLE_LIVE_V28_AUTHORIZATION_PATH } from "./build-table-live-proof-v28.js";
import { verifyTableLiveV28Authorization } from "./table-live-v28-authorization.js";
import { runTableLiveV28Preflight } from "./table-live-v28-preflight.js";
import { TableLiveV28Orchestrator } from "./run-table-live-v28.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v28 requires exact authorization artifact ${TABLE_LIVE_V28_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v28-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV28Authorization(root);
    runTableLiveV28Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV28Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV28Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
