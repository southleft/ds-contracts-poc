...
import {
  ...
  createInputLiveV14FakeDynamicResponse,
  ...
  listInputLiveV14PersistedRequests,
  persistInputLiveV14RawResponse,
  persistInputLiveV14TechnicalGates,
  validateInputLiveV14Receipt,
  validateInputLiveV14Request,
  type InputLiveV14RemotePhase,
...
import {
  ...
  type InputLiveV14ExtractPayload,
  type InputLiveV14ProbePayload,
...
import {
  ...
  simulatedInputLiveV14Authorization,
} from "./run-input-field-live-v14.js";
...
const captures: InputLiveV14CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV14Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v14-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV14Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV14CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV14FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV14FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V14_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V14_TARGET);
  persistInputLiveV14RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV14Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV14Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV14WriterOwnership,
  ...
  marker: "INPUT-TEXT-FILL-TWO-PASS",
});
...
const rootProofs = (): InputLiveV14RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v14-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV14Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV14Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV14RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV14RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV14ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV14ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV14CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV14CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV14CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV14Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV14CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV14PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV14ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV14Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV14Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV14Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV14Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V14_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV14RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V14_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV14Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV14Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV14ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV14Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV14WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV14RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV14Request;
    validateInputLiveV14Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV14Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV14CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v14-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV14Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV14Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV14Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V14_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV14RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V14_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV14Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV14Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV14RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V14_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV14Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV14Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV14Request;
