...
import {
  ...
  verifyInputLiveV17Authorization,
  type InputLiveV17AntecedentIndex,
  type InputLiveV17AuthorizationProof,
} from "./input-field-live-v17-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V17_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v17/operator-security-attestation-template.json";
export const INPUT_LIVE_V17_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v17-security-attestation.json";

export interface InputLiveV17SecurityAttestation {
...
export interface InputLiveV17PreflightReport {
  artifactVersion: "input-live-v17-preflight-v1";
...
export const inputLiveV17AttestationSha256 = (
  value: Omit<InputLiveV17SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV17SecurityAttestation(
  value: unknown,
  proof: InputLiveV17AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v17-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V17_TARGET) ||
...
export function validateInputLiveV17ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV17Authorization");
  const preflight = source.indexOf("runInputLiveV17Preflight");
  const initialize = source.indexOf("InputLiveV17Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV17Preflight(
  root: string,
  proof: InputLiveV17AuthorizationProof,
  attestationPath = INPUT_LIVE_V17_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV17PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V17_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V17_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V17_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV17AntecedentIndex;
  failures.push(...validateInputLiveV17AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V17_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V17_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v17-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV17ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV17SecurityAttestation;
  return {
    artifactVersion: "input-live-v17-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V17_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV17Preflight(
    process.cwd(),
    verifyInputLiveV17Authorization(),
    attestationPath,
