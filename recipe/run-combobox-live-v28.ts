...
import {
  acceptComboboxLiveV28Response,
  createComboboxLiveV28Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV28Request,
  persistComboboxLiveV28RawResponse,
  persistComboboxLiveV28TechnicalGates,
  readComboboxLiveV28AcceptedRawPayload,
  type ComboboxLiveV28Request,
  type ComboboxLiveV28TransactionAuthorization,
} from "./combobox-live-v28-broker.js";
import {
  COMBOBOX_LIVE_V28_CAPTURE_COUNT,
  assertComboboxLiveV28CaptureResponses,
  ...
  evaluateComboboxLiveV28Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV28Roots,
  validateComboboxLiveV28CaptureManifest,
  validateComboboxLiveV28CapturePayload,
  validateComboboxLiveV28CleanupPayload,
  validateComboboxLiveV28ExtractPayload,
  validateComboboxLiveV28ProbePayload,
  validateComboboxLiveV28WriterPayload,
  type ComboboxLiveV28CaptureCell,
  type ComboboxLiveV28CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V28_PLAN_PATH,
  type ComboboxLiveV28ProofPlan,
} from "./build-combobox-live-proof-v28.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v28.js";

export const COMBOBOX_LIVE_V28_RUNNER_VERSION = "combobox-live-v28-runner-v1";

interface ComboboxLiveV28JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV28OrchestratorState {
  artifactVersion: "combobox-live-v28-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV28JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV28ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV28CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV28SourceIdentity & {
...
export interface ComboboxLiveV28NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV28OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v28 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V28_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V28_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV28ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV28CaptureCell[];
  validateComboboxLiveV28CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v28 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v28 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV28WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV28RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV28Request =>
  readJson<ComboboxLiveV28Request>(file);

export class ComboboxLiveV28Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV28Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV28OrchestratorState = {
      artifactVersion: "combobox-live-v28-orchestrator-state-v1",
    ...
    return new ComboboxLiveV28Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV28OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV28Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV28NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV28NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v28 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV28NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v28 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV28RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV28Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV28CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV28AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV28WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV28Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV28ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV28Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV28Response(this.#directory, "writer", {
        validate: validateComboboxLiveV28WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV28Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV28RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV28Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV28ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV28Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV28RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v28-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV28Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV28ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV28Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV28ProbePayload(payload, ownership),
      });
      persistComboboxLiveV28TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v28 capture cell missing");
      const accepted = acceptComboboxLiveV28Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV28CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V28_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV28CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV28Objective(
    ...
    throw new Error(`Combobox live v28 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV28Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV28CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV28Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV28RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV28Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV28CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV28CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV28CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV28Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV28TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV28SimulationKey = (): KeyObject =>
