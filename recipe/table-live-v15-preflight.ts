...
import {
  validateTableLiveV15AntecedentIndex,
  verifyTableLiveV15Authorization,
  type TableLiveV15AntecedentIndex,
  type TableLiveV15AuthorizationProof,
} from "./table-live-v15-authorization.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V15_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/table-live-pivot-v15/operator-security-attestation-template.json";
export const TABLE_LIVE_V15_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/table-live-v15-security-attestation.json";

export interface TableLiveV15SecurityAttestation {
  artifactVersion: "table-live-v15-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface TableLiveV15PreflightReport {
  artifactVersion: "table-live-v15-preflight-v1";
...
export const tableLiveV2AttestationSha256 = (
  value: Omit<TableLiveV15SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateTableLiveV15SecurityAttestation(
  value: unknown,
  proof: TableLiveV15AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "table-live-v15-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(TABLE_LIVE_V15_TARGET) ||
...
export function validateTableLiveV15ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyTableLiveV15Authorization");
  const preflight = source.indexOf("runTableLiveV15Preflight");
  const initialize = source.indexOf("TableLiveV15Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runTableLiveV15Preflight(
  root: string,
  proof: TableLiveV15AuthorizationProof,
  attestationPath = TABLE_LIVE_V15_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): TableLiveV15PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(TABLE_LIVE_V15_TARGET) ||
  ...
  const indexPath = path.join(root, TABLE_LIVE_V15_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as TableLiveV15AntecedentIndex;
  failures.push(...validateTableLiveV15AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, TABLE_LIVE_V15_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${TABLE_LIVE_V15_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateTableLiveV15SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/table-live-v15-authorized.ts"),
    "utf8",
  );
  failures.push(...validateTableLiveV15ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Table live v15 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as TableLiveV15SecurityAttestation;
  return {
    artifactVersion: "table-live-v15-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${TABLE_LIVE_V15_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runTableLiveV15Preflight(
    process.cwd(),
    verifyTableLiveV15Authorization(),
    attestationPath,
