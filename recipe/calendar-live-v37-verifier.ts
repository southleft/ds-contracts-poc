...
import {
  verifyCalendarLiveV37FixedSceneFixedPoint,
  type CalendarLiveV37FixedFixedPoint,
} from "./calendar-live-v37-fixed-point.js";

export const CALENDAR_LIVE_V37_VERIFIER_VERSION = "calendar-live-v37-verifier-v1";

export function buildCalendarLiveV37RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV37RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV37RawNode[];
}

export interface CalendarLiveV37NormalizedScene {
...
export interface CalendarLiveV37SceneProof {
...
export function normalizeCalendarLiveV37Scene(
  root: CalendarLiveV37RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV37NormalizedScene {
  const canonicalBindings: CalendarLiveV37NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV37RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV37Scene<Instance>(
  normalized: CalendarLiveV37NormalizedScene,
...
): CalendarLiveV37SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV37FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV37ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV37PreCaptureGates(payload: unknown): void {
