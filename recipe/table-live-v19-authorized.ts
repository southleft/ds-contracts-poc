...
import { TABLE_LIVE_V19_AUTHORIZATION_PATH } from "./build-table-live-proof-v19.js";
import { verifyTableLiveV19Authorization } from "./table-live-v19-authorization.js";
import { runTableLiveV19Preflight } from "./table-live-v19-preflight.js";
import { TableLiveV19Orchestrator } from "./run-table-live-v19.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v19 requires exact authorization artifact ${TABLE_LIVE_V19_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v19-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV19Authorization(root);
    runTableLiveV19Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV19Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV19Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
