...
import {
  ...
  createInputLiveV63FakeDynamicResponse,
  ...
  listInputLiveV63PersistedRequests,
  persistInputLiveV63RawResponse,
  persistInputLiveV63TechnicalGates,
  validateInputLiveV63Receipt,
  validateInputLiveV63Request,
  type InputLiveV63RemotePhase,
...
import {
  ...
  type InputLiveV63ExtractPayload,
  type InputLiveV63ProbePayload,
  type InputLiveV63RestorePayload,
  type InputLiveV63RootProof,
  type InputLiveV63WriterOwnership,
} from "./input-field-live-v63-contract.js";
import {
  InputLiveV63Orchestrator,
  simulatedInputLiveV63Authorization,
} from "./run-input-field-live-v63.js";
...
const captures: InputLiveV63CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV63Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v63-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV63Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV63CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV63FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV63FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V63_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V63_TARGET);
  persistInputLiveV63RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV63Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV63Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV63WriterOwnership,
): InputLiveV63RestorePayload => ({
...
const rootProofs = (): InputLiveV63RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV63CaptureCell,
): InputLiveV63CapturePayload => {
  const bytes = Buffer.from(`input-live-v63-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV63Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV63Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV63RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV63RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV63ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV63ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV63CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV63CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV63CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV63Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV63CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV63PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV63ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV63Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV63Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV63Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV63Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V63_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV63RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V63_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV63Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV63Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV63ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV63Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV63WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV63RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV63Request;
    validateInputLiveV63Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV63Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV63CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v63-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV63Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV63Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV63Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V63_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV63RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V63_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV63Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV63Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV63RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V63_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV63Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV63Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV63Request;
