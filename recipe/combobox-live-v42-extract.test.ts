...
test("host omits empty extract instance payload; writer is unchanged", () => {
  const host = readFileSync("recipe/scene-readback-combobox-v42.ts", "utf8");
...
test("host observe recovers compile-carried option-set name, not the live display name; writer is unchanged", () => {
  ...
  assert.match(
    host,
    /if \(sceneRole\(scene\) === "combobox\/option-set"\)\s*return COMBOBOX_LIVE_V42_OPTION_SET_COMPILE_CARRY_NAME/,
  );
  assert.match(
    host,
    /COMBOBOX_LIVE_V42_OPTION_SET_COMPILE_CARRY_NAME =\s*`combobox\/option-set :: \$\{COMBOBOX_LIVE_V42_OPTION_SET_COMPILE_CARRY_LABEL\}`/,
  );
  assert.match(
    host,
    /COMBOBOX_LIVE_V42_OPTION_SET_COMPILE_CARRY_LABEL =\s*"Combobox option"/,
  );
...
test("host emits compile-carried trigger characters Choose a person on empty-content combobox/input; writer is unchanged", () => {
  ...
  assert.match(
    host,
    /if \(\s*sceneRole\(scene\) === "combobox\/input" &&\s*variantProperties\?\.Content === "empty"\s*\)\s*return COMBOBOX_LIVE_V42_TRIGGER_COMPILE_CARRY_CHARACTERS/,
  );
  assert.match(
    host,
    /COMBOBOX_LIVE_V42_TRIGGER_COMPILE_CARRY_CHARACTERS =\s+"Choose a person"/,
  );
  assert.doesNotMatch(host, /if \(combobox\) \{/);
  assert.doesNotMatch(
    host,
    /COMBOBOX_LIVE_V42_TRIGGER_COMPILE_CARRY_CHARACTERS =\s+"Ada Lovelace"/,
  );
...
test("host emits compile-carried label Combobox option on combobox/option-set; writer is unchanged", () => {
  ...
  assert.match(
    host,
    /if \(sceneRole\(scene\) === "combobox\/option-set"\)\s+return COMBOBOX_LIVE_V42_OPTION_SET_COMPILE_CARRY_LABEL/,
  );
  assert.match(
    host,
    /COMBOBOX_LIVE_V42_OPTION_SET_COMPILE_CARRY_LABEL =\s+"Combobox option"/,
...
test("host probe keeps exactSceneRestoration after the open-variant walk when resize and property restore pass; writer is unchanged", () => {
  const contract = readFileSync("recipe/combobox-live-v42-contract.ts", "utf8");
  assert.match(
    contract,
