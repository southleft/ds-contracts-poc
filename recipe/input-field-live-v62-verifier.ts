...
export const INPUT_LIVE_V62_VERIFIER_VERSION = "input-live-v62-verifier-v1";

export function buildInputLiveV62RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV62RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV62RawNode[];
}

export interface InputLiveV62NormalizedScene {
...
export interface InputLiveV62SceneProof {
...
export function normalizeInputLiveV62Scene(
  root: InputLiveV62RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV62NormalizedScene {
  const canonicalBindings: InputLiveV62NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV62RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV62Scene<Instance>(
  normalized: InputLiveV62NormalizedScene,
...
export function compileInputLiveV62ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV62PreCaptureGates(payload: unknown): void {
