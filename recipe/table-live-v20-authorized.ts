...
import { TABLE_LIVE_V20_AUTHORIZATION_PATH } from "./build-table-live-proof-v20.js";
import { verifyTableLiveV20Authorization } from "./table-live-v20-authorization.js";
import { runTableLiveV20Preflight } from "./table-live-v20-preflight.js";
import { TableLiveV20Orchestrator } from "./run-table-live-v20.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Table live v20 requires exact authorization artifact ${TABLE_LIVE_V20_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/table-live-v20-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyTableLiveV20Authorization(root);
    runTableLiveV20Preflight(
      root,
      authorization,
    ...
    const orchestrator = TableLiveV20Orchestrator.initialize({
      root,
  ...
  const orchestrator = TableLiveV20Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
