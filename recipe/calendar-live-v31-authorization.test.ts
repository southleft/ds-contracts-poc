...
import { CALENDAR_LIVE_V31_INDEX_PATH } from "./build-calendar-live-proof-v31.js";
import {
...
import {
  buildCalendarLiveV31AuthorizationArtifact,
  validateCalendarLiveV31AntecedentIndex,
  validateCalendarLiveV31AuthorizationArtifact,
  validateCalendarLiveV31History,
  validateCalendarLiveV31UnitTestSource,
  type CalendarLiveV31AntecedentIndex,
  type CalendarLiveV31AuthorizationArtifact,
  type CalendarLiveV31AuthorizationProof,
  type CalendarLiveV31HistoryState,
} from "./calendar-live-v31-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV31ControlFlowSource,
  validateCalendarLiveV31SecurityAttestation,
  type CalendarLiveV31SecurityAttestation,
} from "./calendar-live-v31-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V31_INDEX_PATH);
const index = (): CalendarLiveV31AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV31AntecedentIndex;

const artifact = (): CalendarLiveV31AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV31AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV31HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV31HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV31AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v31/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV31AuthorizationArtifact,
): CalendarLiveV31SecurityAttestation => {
  const body: Omit<CalendarLiveV31SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V31_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV31History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV31History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV31History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV31History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV31History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV31UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v31-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV31UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV31AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV31AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV31AuthorizationArtifact(
        planted as CalendarLiveV31AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV31SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV31SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV31SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v31-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV31ControlFlowSource(source), []);
});
