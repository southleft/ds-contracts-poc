...
import { CALENDAR_LIVE_V47_INDEX_PATH } from "./build-calendar-live-proof-v47.js";
import {
...
import {
  buildCalendarLiveV47AuthorizationArtifact,
  validateCalendarLiveV47AntecedentIndex,
  validateCalendarLiveV47AuthorizationArtifact,
  validateCalendarLiveV47History,
  validateCalendarLiveV47UnitTestSource,
  type CalendarLiveV47AntecedentIndex,
  type CalendarLiveV47AuthorizationArtifact,
  type CalendarLiveV47AuthorizationProof,
  type CalendarLiveV47HistoryState,
} from "./calendar-live-v47-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV47ControlFlowSource,
  validateCalendarLiveV47SecurityAttestation,
  type CalendarLiveV47SecurityAttestation,
} from "./calendar-live-v47-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V47_INDEX_PATH);
const index = (): CalendarLiveV47AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV47AntecedentIndex;

const artifact = (): CalendarLiveV47AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV47AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV47HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV47HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV47AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v47/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV47AuthorizationArtifact,
): CalendarLiveV47SecurityAttestation => {
  const body: Omit<CalendarLiveV47SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V47_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV47History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV47History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV47History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV47History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV47History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV47UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v47-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV47UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV47AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV47AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV47AuthorizationArtifact(
        planted as CalendarLiveV47AuthorizationArtifact,
        index(),
...
test("one-root denominator, capture gate, cleanup, and prior-lineage reuse are rejected", () => {
  ...
  for (const mutate of [
    ...
    (value: Record<string, any>) => {
      value.execution.comboboxLiveV47AuthorizationReusable = true;
    },
    (value: Record<string, any>) => {
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV47SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV47SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV47SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v47-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV47ControlFlowSource(source), []);
});
