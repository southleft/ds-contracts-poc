...
import {
  ...
  createInputLiveV75FakeDynamicResponse,
  ...
  listInputLiveV75PersistedRequests,
  persistInputLiveV75RawResponse,
  persistInputLiveV75TechnicalGates,
  validateInputLiveV75Receipt,
  validateInputLiveV75Request,
  type InputLiveV75RemotePhase,
...
import {
  ...
  type InputLiveV75ExtractPayload,
  type InputLiveV75ProbePayload,
...
import {
  ...
  simulatedInputLiveV75Authorization,
} from "./run-input-field-live-v75.js";
...
const captures: InputLiveV75CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV75Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v75-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV75Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV75CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV75FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV75FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V75_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V75_TARGET);
  persistInputLiveV75RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV75Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV75Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV75WriterOwnership,
): InputLiveV75RestorePayload => ({
...
const rootProofs = (): InputLiveV75RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v75-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV75Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV75Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV75RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV75RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV75ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV75ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV75CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV75CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV75CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV75Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV75CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV75PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV75ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV75Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV75Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV75Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV75Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V75_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV75RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V75_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV75Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV75Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV75ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV75Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV75WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV75RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV75Request;
    validateInputLiveV75Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV75Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV75CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v75-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV75Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV75Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV75Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V75_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV75RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V75_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV75Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV75Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV75RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V75_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV75Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV75Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV75Request;
