...
import { calendarLiveV1Sha256 } from "./calendar-live-v32-broker.js";

export interface CalendarLiveV32LifecycleSimulationReport {
  artifactVersion: "calendar-live-v32-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV32AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV32Lifecycle(): Promise<CalendarLiveV32LifecycleSimulationReport> {
  await buildCalendarLiveV32Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V32_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV32AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v32-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V32_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V32_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV32History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV32History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV32AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V32_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V32_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV32History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV32History(authorized, "pending");
    ...
    await buildCalendarLiveV32Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V32_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV32AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV32Lifecycle(), null, 2)}\n`,
  );
