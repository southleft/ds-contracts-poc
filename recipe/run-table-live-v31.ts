...
import {
  acceptTableLiveV31Response,
  createTableLiveV31Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV31Request,
  persistTableLiveV31RawResponse,
  persistTableLiveV31TechnicalGates,
  readTableLiveV31AcceptedRawPayload,
  type TableLiveV31Request,
  type TableLiveV31TransactionAuthorization,
} from "./table-live-v31-broker.js";
import {
  TABLE_LIVE_V31_CAPTURE_COUNT,
  assertTableLiveV31CaptureResponses,
  ...
  evaluateTableLiveV31Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV31Roots,
  validateTableLiveV31CaptureManifest,
  validateTableLiveV31CapturePayload,
  validateTableLiveV31CleanupPayload,
  validateTableLiveV31ExtractPayload,
  validateTableLiveV31ProbePayload,
  validateTableLiveV31WriterPayload,
  type TableLiveV31CaptureCell,
  type TableLiveV31CapturePayload,
...
import {
  ...
  TABLE_LIVE_V31_PLAN_PATH,
  type TableLiveV31ProofPlan,
} from "./build-table-live-proof-v31.js";
import {
...
export const TABLE_LIVE_V31_RUNNER_VERSION = "table-live-v31-runner-v1";

interface TableLiveV31JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV31OrchestratorState {
  artifactVersion: "table-live-v31-orchestrator-state-v1";
  ...
  journal: TableLiveV31JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV31ProofPlan;
  planSha256: string;
  captures: TableLiveV31CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV31SourceIdentity & {
...
export interface TableLiveV31NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV31OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v31 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V31_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V31_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV31ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV31CaptureCell[];
  validateTableLiveV31CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v31 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v31 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV31WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV31RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV31Request =>
  readJson<TableLiveV31Request>(file);

export class TableLiveV31Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV31Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV31OrchestratorState = {
      artifactVersion: "table-live-v31-orchestrator-state-v1",
    ...
    return new TableLiveV31Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV31OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV31Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV31NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV31NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v31 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV31NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v31 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV31RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV31Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV31CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV31AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV31WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV31Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV31ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV31Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV31Response(this.#directory, "writer", {
        validate: validateTableLiveV31WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV31Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV31RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV31Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV31ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV31Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV31RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v31-host-root-proofs-v1",
      ...
      const next = issueTableLiveV31Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV31ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV31Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV31ProbePayload(payload, ownership),
      });
      persistTableLiveV31TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v31 capture cell missing");
      const accepted = acceptTableLiveV31Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV31CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V31_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV31CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV31Objective(
    ...
    throw new Error(`Table live v31 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV31Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV31CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV31Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV31RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV31Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV31CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV31CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV31CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV31Authorization = (
  privateKey: KeyObject,
): TableLiveV31TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV31SimulationKey = (): KeyObject =>
