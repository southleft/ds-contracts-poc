...
import { INPUT_LIVE_V84_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v84.js";
import { verifyInputLiveV84Authorization } from "./input-field-live-v84-authorization.js";
import { runInputLiveV84Preflight } from "./input-field-live-v84-preflight.js";
import { InputLiveV84Orchestrator } from "./run-input-field-live-v84.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V84_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v84-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV84Authorization(root);
    runInputLiveV84Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV84Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
