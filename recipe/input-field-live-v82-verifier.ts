...
export const INPUT_LIVE_V82_VERIFIER_VERSION = "input-live-v82-verifier-v1";

export function buildInputLiveV82RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV82RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV82RawNode[];
}

export interface InputLiveV82NormalizedScene {
...
export interface InputLiveV82SceneProof {
...
export function normalizeInputLiveV82Scene(
  root: InputLiveV82RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV82NormalizedScene {
  const canonicalBindings: InputLiveV82NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV82RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV82Scene<Instance>(
  normalized: InputLiveV82NormalizedScene,
...
export function compileInputLiveV82ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV82PreCaptureGates(payload: unknown): void {
