...
import { CALENDAR_LIVE_V24_INDEX_PATH } from "./build-calendar-live-proof-v24.js";
import {
...
import {
  buildCalendarLiveV24AuthorizationArtifact,
  validateCalendarLiveV24AntecedentIndex,
  validateCalendarLiveV24AuthorizationArtifact,
  validateCalendarLiveV24History,
  validateCalendarLiveV24UnitTestSource,
  type CalendarLiveV24AntecedentIndex,
  type CalendarLiveV24AuthorizationArtifact,
  type CalendarLiveV24AuthorizationProof,
  type CalendarLiveV24HistoryState,
} from "./calendar-live-v24-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV24ControlFlowSource,
  validateCalendarLiveV24SecurityAttestation,
  type CalendarLiveV24SecurityAttestation,
} from "./calendar-live-v24-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V24_INDEX_PATH);
const index = (): CalendarLiveV24AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV24AntecedentIndex;

const artifact = (): CalendarLiveV24AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV24AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV24HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV24HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV24AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v24/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV24AuthorizationArtifact,
): CalendarLiveV24SecurityAttestation => {
  const body: Omit<CalendarLiveV24SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V24_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV24History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV24History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV24History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV24History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV24History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV24UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v24-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV24UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV24AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV24AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV24AuthorizationArtifact(
        planted as CalendarLiveV24AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV24SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV24SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV24SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v24-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV24ControlFlowSource(source), []);
});
