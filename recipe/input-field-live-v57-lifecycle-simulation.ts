...
export interface InputLiveV57LifecycleSimulationReport {
  artifactVersion: "input-live-v57-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV57AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV57Lifecycle(): Promise<InputLiveV57LifecycleSimulationReport> {
  await buildInputLiveV57Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V57_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV57AntecedentIndex;
  const indexSha256 = inputLiveV57Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v57-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V57_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V57_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV57History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV57History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV57AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V57_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V57_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV57History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV57History(authorized, "pending");
    ...
    await buildInputLiveV57Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V57_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV57AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV57Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV57Lifecycle(), null, 2)}\n`,
  );
