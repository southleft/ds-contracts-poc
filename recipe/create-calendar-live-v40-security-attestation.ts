...
import { verifyCalendarLiveV40Authorization } from "./calendar-live-v40-authorization.js";
import { CALENDAR_LIVE_V40_TARGET } from "./calendar-live-v40-broker.js";
import {
  CALENDAR_LIVE_V40_SECURITY_ATTESTATION_DEFAULT_PATH,
...
const proof = verifyCalendarLiveV40Authorization();
...
const sessionIdentity =
  argument("--session-identity") ?? "scratch-only-calendar-v40-session";
...
const attestation: CalendarLiveV40SecurityAttestation = {
  ...body,
  attestationSha256: calendarLiveV1AttestationSha256(body),
};
const failures = validateCalendarLiveV40SecurityAttestation(attestation, proof);
if (failures.length)
  throw new Error(
    `Calendar live v40 security attestation refused:\n${failures.join("\n")}`,
  );

const output = path.resolve(
  argument("--output") ?? CALENDAR_LIVE_V40_SECURITY_ATTESTATION_DEFAULT_PATH,
);
