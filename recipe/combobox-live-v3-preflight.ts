...
import {
  validateComboboxLiveV3AntecedentIndex,
  verifyComboboxLiveV3Authorization,
  type ComboboxLiveV3AntecedentIndex,
  type ComboboxLiveV3AuthorizationProof,
} from "./combobox-live-v3-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V3_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v3/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V3_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v3-security-attestation.json";

export interface ComboboxLiveV3SecurityAttestation {
...
export interface ComboboxLiveV3PreflightReport {
  artifactVersion: "combobox-live-v3-preflight-v1";
...
export const comboboxLiveV3AttestationSha256 = (
  value: Omit<ComboboxLiveV3SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV3SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV3AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v3-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V3_TARGET) ||
...
export function validateComboboxLiveV3ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV3Authorization");
  const preflight = source.indexOf("runComboboxLiveV3Preflight");
  const initialize = source.indexOf("ComboboxLiveV3Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV3Preflight(
  root: string,
  proof: ComboboxLiveV3AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V3_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV3PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V3_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V3_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V3_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV3AntecedentIndex;
  failures.push(...validateComboboxLiveV3AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V3_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V3_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v3-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV3ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v3 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV3SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v3-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V3_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV3Preflight(
    process.cwd(),
    verifyComboboxLiveV3Authorization(),
    attestationPath,
