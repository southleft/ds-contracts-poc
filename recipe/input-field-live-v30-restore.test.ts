...
import {
  INPUT_LIVE_V30_MEASURE_VISIBLE_FILL_MARKER,
  INPUT_LIVE_V30_RESTORE_CONTENT_ROLES,
  INPUT_LIVE_V30_RESTORE_PARENT_ROLES,
...
test("v16 restore still measures FILL while visible, then restores visibility", () => {
  const program = buildInputLiveV30RestoreProgram(writer);
  assert.match(program, new RegExp(INPUT_LIVE_V30_MEASURE_VISIBLE_FILL_MARKER));
  ...
  assert.match(program, /INPUT-V30-RESTORE-COUNT/);
  assert.match(program, /INPUT-V30-RESTORE-NOT-FILL/);
  ...
  for (const role of INPUT_LIVE_V30_RESTORE_CONTENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
  for (const role of INPUT_LIVE_V30_RESTORE_PARENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
});

test("v16 restore payload requires 256 filled content texts", () => {
  const accepted = validateInputLiveV30RestorePayload(
    {
      pageId: "1:1",
      setIds: ["2:2", "2:1"],
      restoredCount: 256,
      ...
      marker: INPUT_LIVE_V30_MEASURE_VISIBLE_FILL_MARKER,
    },
    writer,
  );
