...
import { CALENDAR_LIVE_V10_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v10.js";
import { verifyCalendarLiveV10Authorization } from "./calendar-live-v10-authorization.js";
import { runCalendarLiveV10Preflight } from "./calendar-live-v10-preflight.js";
import { CalendarLiveV10Orchestrator } from "./run-calendar-live-v10.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v10 requires exact authorization artifact ${CALENDAR_LIVE_V10_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v10-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV10Authorization(root);
    runCalendarLiveV10Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV10Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV10Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
