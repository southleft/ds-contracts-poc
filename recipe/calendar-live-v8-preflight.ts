...
import {
  ...
  verifyCalendarLiveV8Authorization,
  type CalendarLiveV8AntecedentIndex,
  type CalendarLiveV8AuthorizationProof,
} from "./calendar-live-v8-authorization.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V8_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/calendar-live-pivot-v8/operator-security-attestation-template.json";
export const CALENDAR_LIVE_V8_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/calendar-live-v8-security-attestation.json";

export interface CalendarLiveV8SecurityAttestation {
...
export interface CalendarLiveV8PreflightReport {
  artifactVersion: "calendar-live-v8-preflight-v1";
...
export const calendarLiveV1AttestationSha256 = (
  value: Omit<CalendarLiveV8SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateCalendarLiveV8SecurityAttestation(
  value: unknown,
  proof: CalendarLiveV8AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "calendar-live-v8-operator-security-attestation-v1" ||
  ...
  if (
    ...
    canonicalJson(artifact.scratchReadOnlyProbe?.target) !==
      canonicalJson(CALENDAR_LIVE_V8_TARGET) ||
...
export function validateCalendarLiveV8ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyCalendarLiveV8Authorization");
  const preflight = source.indexOf("runCalendarLiveV8Preflight");
  const initialize = source.indexOf("CalendarLiveV8Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runCalendarLiveV8Preflight(
  root: string,
  proof: CalendarLiveV8AuthorizationProof,
  attestationPath = CALENDAR_LIVE_V8_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): CalendarLiveV8PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    canonicalJson(proof.target) !== canonicalJson(CALENDAR_LIVE_V8_TARGET) ||
  ...
  const indexPath = path.join(root, CALENDAR_LIVE_V8_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as CalendarLiveV8AntecedentIndex;
  failures.push(...validateCalendarLiveV8AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, CALENDAR_LIVE_V8_AUTHORIZATION_PATH)))
    failures.push("table v15 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${CALENDAR_LIVE_V8_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/calendar-live-v8-authorized.ts"),
    "utf8",
  );
  failures.push(...validateCalendarLiveV8ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Calendar live v8 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as CalendarLiveV8SecurityAttestation;
  return {
    artifactVersion: "calendar-live-v8-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${CALENDAR_LIVE_V8_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runCalendarLiveV8Preflight(
    process.cwd(),
    verifyCalendarLiveV8Authorization(),
    attestationPath,
