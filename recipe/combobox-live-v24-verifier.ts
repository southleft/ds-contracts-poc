...
import {
  verifyComboboxLiveV24FixedSceneFixedPoint,
  type ComboboxLiveV24FixedFixedPoint,
} from "./combobox-live-v24-fixed-point.js";

export const COMBOBOX_LIVE_V24_VERIFIER_VERSION = "combobox-live-v24-verifier-v1";

export function buildComboboxLiveV24RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV24RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV24RawNode[];
}

export interface ComboboxLiveV24NormalizedScene {
...
export interface ComboboxLiveV24SceneProof {
...
export function normalizeComboboxLiveV24Scene(
  root: ComboboxLiveV24RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV24NormalizedScene {
  const canonicalBindings: ComboboxLiveV24NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV24RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV24Scene<Instance>(
  normalized: ComboboxLiveV24NormalizedScene,
...
): ComboboxLiveV24SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV24FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV24ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV24PreCaptureGates(payload: unknown): void {
