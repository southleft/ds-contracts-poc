...
import { INPUT_LIVE_V34_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v34.js";
import { verifyInputLiveV34Authorization } from "./input-field-live-v34-authorization.js";
import { runInputLiveV34Preflight } from "./input-field-live-v34-preflight.js";
import { InputLiveV34Orchestrator } from "./run-input-field-live-v34.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V34_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v34-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV34Authorization(root);
    runInputLiveV34Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV34Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
