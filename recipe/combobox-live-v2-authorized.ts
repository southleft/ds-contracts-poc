...
import { COMBOBOX_LIVE_V2_AUTHORIZATION_PATH } from "./build-combobox-live-proof-v2.js";
import { verifyComboboxLiveV2Authorization } from "./combobox-live-v2-authorization.js";
import { runComboboxLiveV2Preflight } from "./combobox-live-v2-preflight.js";
import { ComboboxLiveV2Orchestrator } from "./run-combobox-live-v2.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Combobox live v1 requires exact authorization artifact ${COMBOBOX_LIVE_V2_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/combobox-live-v2-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyComboboxLiveV2Authorization(root);
    runComboboxLiveV2Preflight(
      root,
      authorization,
    ...
    const orchestrator = ComboboxLiveV2Orchestrator.initialize({
      root,
  ...
  const orchestrator = ComboboxLiveV2Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
