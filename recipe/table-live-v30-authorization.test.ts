...
import { TABLE_LIVE_V30_INDEX_PATH } from "./build-table-live-proof-v30.js";
import {
...
import {
  buildTableLiveV30AuthorizationArtifact,
  validateTableLiveV30AntecedentIndex,
  validateTableLiveV30AuthorizationArtifact,
  validateTableLiveV30History,
  validateTableLiveV30UnitTestSource,
  type TableLiveV30AntecedentIndex,
  type TableLiveV30AuthorizationArtifact,
  type TableLiveV30AuthorizationProof,
  type TableLiveV30HistoryState,
} from "./table-live-v30-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV30ControlFlowSource,
  validateTableLiveV30SecurityAttestation,
  type TableLiveV30SecurityAttestation,
} from "./table-live-v30-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V30_INDEX_PATH);
const index = (): TableLiveV30AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV30AntecedentIndex;

const artifact = (): TableLiveV30AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV30AuthorizationArtifact({
...
const pendingState = (): TableLiveV30HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV30HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV30AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v30/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV30AuthorizationArtifact,
): TableLiveV30SecurityAttestation => {
  const body: Omit<TableLiveV30SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V30_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV30History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV30History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV30History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV30History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV30History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV30UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v30-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV30UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV30AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV30AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV30AuthorizationArtifact(
        planted as TableLiveV30AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV30SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV30SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV30SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v30-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV30ControlFlowSource(source), []);
});
