...
import { INPUT_LIVE_V16_INDEX_PATH } from "./build-input-field-live-proof-v16.js";
import {
  INPUT_LIVE_V16_DYNAMIC_TOOL,
  INPUT_LIVE_V16_TARGET,
} from "./input-field-live-v16-broker.js";
import {
  buildInputLiveV16AuthorizationArtifact,
  validateInputLiveV16AntecedentIndex,
  validateInputLiveV16AuthorizationArtifact,
  validateInputLiveV16History,
  validateInputLiveV16UnitTestSource,
  type InputLiveV16AntecedentIndex,
  type InputLiveV16AuthorizationArtifact,
  type InputLiveV16AuthorizationProof,
  type InputLiveV16HistoryState,
} from "./input-field-live-v16-authorization.js";
import {
  inputLiveV16AttestationSha256,
  validateInputLiveV16ControlFlowSource,
  validateInputLiveV16SecurityAttestation,
  type InputLiveV16SecurityAttestation,
} from "./input-field-live-v16-preflight.js";
import { inputLiveV16Sha256 } from "./input-field-live-v16-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V16_INDEX_PATH);
const index = (): InputLiveV16AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV16AntecedentIndex;

const artifact = (): InputLiveV16AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV16AuthorizationArtifact({
...
const pendingState = (): InputLiveV16HistoryState => ({
  index: index(),
  indexSha256: inputLiveV16Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV16HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV16AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV16Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V16_TARGET,
  expectedDynamicTool: INPUT_LIVE_V16_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v16/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV16AuthorizationArtifact,
): InputLiveV16SecurityAttestation => {
  const body: Omit<InputLiveV16SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v16-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V16_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV16AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV16History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV16History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV16History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV16History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV16History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV16UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV16UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV16AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV16Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV16AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV16AuthorizationArtifact(
        planted as InputLiveV16AuthorizationArtifact,
        index(),
        inputLiveV16Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV16SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV16AttestationSha256(
      body as Omit<InputLiveV16SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV16SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV16ControlFlowSource(source), []);
});
