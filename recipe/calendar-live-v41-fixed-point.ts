/**
 * Calendar live v41 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V41_FIXED_VERIFIER_VERSION = "calendar-live-v41-fixed-point-v1";
export const CALENDAR_LIVE_V41_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV41FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV41FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV41FixedSourceProbe {
...
export interface CalendarLiveV41FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV41FixedFixedPoint;
}

export interface CalendarLiveV41FixedHardGateInput {
  thresholds: CalendarLiveV41FixedThresholds;
  sourceProbes: CalendarLiveV41FixedSourceProbe[];
  cells: CalendarLiveV41FixedCellValidation[];
  sceneProofs: CalendarLiveV41FixedSceneProof[];
...
export interface CalendarLiveV41FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V41_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV41FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV41FixedHardGates(
  input: CalendarLiveV41FixedHardGateInput,
): CalendarLiveV41FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV41FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V41_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV41FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV41FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV41FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
