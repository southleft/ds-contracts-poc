/**
 * Calendar live v11 host verifier. Table-shaped: table/row/cell ownership,
...
export const CALENDAR_LIVE_V11_FIXED_VERIFIER_VERSION = "calendar-live-v11-fixed-point-v1";
export const CALENDAR_LIVE_V11_FIXED_REQUIRED_GATE_IDS = [
...
export interface CalendarLiveV11FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface CalendarLiveV11FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface CalendarLiveV11FixedSourceProbe {
...
export interface CalendarLiveV11FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: CalendarLiveV11FixedFixedPoint;
}

export interface CalendarLiveV11FixedHardGateInput {
  thresholds: CalendarLiveV11FixedThresholds;
  sourceProbes: CalendarLiveV11FixedSourceProbe[];
  cells: CalendarLiveV11FixedCellValidation[];
  sceneProofs: CalendarLiveV11FixedSceneProof[];
...
export interface CalendarLiveV11FixedHardGateReport {
  verifierVersion: typeof CALENDAR_LIVE_V11_FIXED_VERIFIER_VERSION;
...
export interface CalendarLiveV11FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyCalendarLiveV11FixedHardGates(
  input: CalendarLiveV11FixedHardGateInput,
): CalendarLiveV11FixedHardGateReport {
  ...
  const exactFields: Array<keyof CalendarLiveV11FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  return {
    verifierVersion: CALENDAR_LIVE_V11_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyCalendarLiveV11FixedSceneFixedPoint<Instance>(
...
): CalendarLiveV11FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<CalendarLiveV11FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
