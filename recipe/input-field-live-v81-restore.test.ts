...
import {
  INPUT_LIVE_V81_MEASURE_VISIBLE_FILL_MARKER,
  INPUT_LIVE_V81_RESTORE_CONTENT_ROLES,
  INPUT_LIVE_V81_RESTORE_PARENT_ROLES,
...
test("v81 probe reveals hidden content FILL before measuring, then restores visibility", () => {
  const program = readFileSync(
    "recipe/input-field-live-v81-contract.ts",
    "utf8",
  );
  const probe = program.slice(
    program.indexOf("export function buildInputLiveV81ProbeProgram"),
  );
...
test("v16 restore still measures FILL while visible, then restores visibility", () => {
  const program = buildInputLiveV81RestoreProgram(writer);
  assert.match(program, new RegExp(INPUT_LIVE_V81_MEASURE_VISIBLE_FILL_MARKER));
  ...
  for (const role of INPUT_LIVE_V81_RESTORE_CONTENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
  for (const role of INPUT_LIVE_V81_RESTORE_PARENT_ROLES)
    assert.match(program, new RegExp(role.replaceAll("/", "\\/")));
});

test("v16 restore payload requires 256 filled content texts", () => {
  const accepted = validateInputLiveV81RestorePayload(
    {
      pageId: "1:1",
      setIds: ["2:2", "2:1"],
      restoredCount: 256,
      ...
      marker: INPUT_LIVE_V81_MEASURE_VISIBLE_FILL_MARKER,
    },
    writer,
  );
...
test("v16 writer bytes stay frozen and v17 writer assigns set HUG", () => {
  ...
  const v76Program = readFileSync(
    "recipe/evidence/input-field-live-pivot-v81/programs/writer.txt",
  );
  const v76Payload = readFileSync(
    "recipe/evidence/input-field-live-pivot-v81/programs/writer-payload.js",
  );
...
test("v81 probe excludes overlay-label AABB from visibleAreaLoss; overlap still uses full semantic", () => {
  const program = readFileSync(
    "recipe/input-field-live-v81-contract.ts",
    "utf8",
  );
...
test("v19 writer preserves hidden FILL occupancy; hashed v16/v17/v18 stay frozen", () => {
  ...
  assert.match(v81Payload, /INPUT-WRITER-HIDDEN-FILL-OCCUPANCY/);
  assert.equal(
    v81Payload.includes(
  ...
  assert.equal(sha256(v81Payload), V19_WRITER_PAYLOAD_SHA256);
  assert.equal(
    sha256(
      readFileSync(
        "recipe/evidence/input-field-live-pivot-v81/programs/writer.txt",
