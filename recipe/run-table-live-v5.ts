...
import {
  acceptTableLiveV5Response,
  createTableLiveV5Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV5Request,
  persistTableLiveV5RawResponse,
  persistTableLiveV5TechnicalGates,
  readTableLiveV5AcceptedRawPayload,
  type TableLiveV5Request,
  type TableLiveV5TransactionAuthorization,
} from "./table-live-v5-broker.js";
import {
  TABLE_LIVE_V5_CAPTURE_COUNT,
  assertTableLiveV5CaptureResponses,
  ...
  evaluateTableLiveV5Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV5Roots,
  validateTableLiveV5CaptureManifest,
  validateTableLiveV5CapturePayload,
  validateTableLiveV5CleanupPayload,
  validateTableLiveV5ExtractPayload,
  validateTableLiveV5ProbePayload,
  validateTableLiveV5WriterPayload,
  type TableLiveV5CaptureCell,
  type TableLiveV5CapturePayload,
...
import {
  ...
  TABLE_LIVE_V5_PLAN_PATH,
  type TableLiveV5ProofPlan,
...
export const TABLE_LIVE_V5_RUNNER_VERSION = "table-live-v5-runner-v1";

interface TableLiveV5JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV5OrchestratorState {
  artifactVersion: "table-live-v5-orchestrator-state-v1";
  ...
  journal: TableLiveV5JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV5ProofPlan;
  planSha256: string;
  captures: TableLiveV5CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV5SourceIdentity & {
...
export interface TableLiveV5NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV5OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v5 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V5_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V5_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV5ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV5CaptureCell[];
  validateTableLiveV5CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v5 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v5 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV5WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV5RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV5Request =>
  readJson<TableLiveV5Request>(file);

export class TableLiveV5Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV5Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV5OrchestratorState = {
      artifactVersion: "table-live-v5-orchestrator-state-v1",
    ...
    return new TableLiveV5Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV5OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV5Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV5NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV5NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v5 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV5NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v5 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV5RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV5Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV5CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV5AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV5WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV5Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV5ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV5Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV5Response(this.#directory, "writer", {
        validate: validateTableLiveV5WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV5Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV5RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV5Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV5ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV5Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV5RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v5-host-root-proofs-v1",
      ...
      const next = issueTableLiveV5Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV5ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV5Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV5ProbePayload(payload, ownership),
      });
      persistTableLiveV5TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v5 capture cell missing");
      const accepted = acceptTableLiveV5Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV5CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V5_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV5CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV5Objective(
    ...
    throw new Error(`Table live v5 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV5Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV5CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV5Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV5RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV5Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV5CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV5CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV5CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV5Authorization = (
  privateKey: KeyObject,
): TableLiveV5TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV5SimulationKey = (): KeyObject =>
