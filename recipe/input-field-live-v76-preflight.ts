...
import {
  ...
  verifyInputLiveV76Authorization,
  type InputLiveV76AntecedentIndex,
  type InputLiveV76AuthorizationProof,
} from "./input-field-live-v76-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V76_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v76/operator-security-attestation-template.json";
export const INPUT_LIVE_V76_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v76-security-attestation.json";

export interface InputLiveV76SecurityAttestation {
...
export interface InputLiveV76PreflightReport {
  artifactVersion: "input-live-v76-preflight-v1";
...
export const inputLiveV76AttestationSha256 = (
  value: Omit<InputLiveV76SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV76SecurityAttestation(
  value: unknown,
  proof: InputLiveV76AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v76-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V76_TARGET) ||
...
export function validateInputLiveV76ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV76Authorization");
  const preflight = source.indexOf("runInputLiveV76Preflight");
  const initialize = source.indexOf("InputLiveV76Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV76Preflight(
  root: string,
  proof: InputLiveV76AuthorizationProof,
  attestationPath = INPUT_LIVE_V76_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV76PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V76_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V76_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V76_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV76AntecedentIndex;
  failures.push(...validateInputLiveV76AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V76_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V76_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v76-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV76ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV76SecurityAttestation;
  return {
    artifactVersion: "input-live-v76-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V76_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV76Preflight(
    process.cwd(),
    verifyInputLiveV76Authorization(),
    attestationPath,
