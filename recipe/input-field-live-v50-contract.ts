...
} from "./input-field-live-v3-verifier-v50.js";
import {
  buildInputLiveV50RawPropertyRuntime,
  normalizeInputLiveV50Scene,
  type InputLiveV50RawNode,
} from "./input-field-live-v50-verifier.js";
...
export {
  ...
  type InputLiveV50RestorePayload,
} from "./input-field-live-v50-restore.js";

export const INPUT_LIVE_V50_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V50_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V50_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V50_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V50_VARIANT_COUNT = 256;
export const INPUT_LIVE_V50_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V50_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV50SourceId = (typeof INPUT_LIVE_V50_SOURCE_IDS)[number];

export interface InputLiveV50SourceIdentity {
  source: InputLiveV50SourceId;
...
export interface InputLiveV50WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV50ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV50SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV50RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV50RootProof {
  source: InputLiveV50SourceId;
  adapterIdentity: string;
...
export interface InputLiveV50ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV50SourceId;
    adapterIdentity: string;
...
export interface InputLiveV50CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV50SourceId;
  adapterIdentity: string;
...
export interface InputLiveV50CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV50SourceId;
  strata: InputLiveV50CaptureCell["strata"];
...
export interface InputLiveV50CleanupPayload {
...
export function validateInputLiveV50WriterPayload(
  value: unknown,
): InputLiveV50WriterOwnership {
...
const identityPlan = (source: InputLiveV50SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV50ExtractProgram(
  writer: InputLiveV50WriterOwnership,
  sources: readonly InputLiveV50SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V50_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V50_NAMESPACE)}
${buildInputLiveV50RawPropertyRuntime()}
...
export function validateInputLiveV50ExtractPayload(
  value: unknown,
  writer: InputLiveV50WriterOwnership,
): InputLiveV50ExtractPayload {
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V50_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV50ExtractPayload;
}

export function proveInputLiveV50Roots<Instance>(
  extract: InputLiveV50ExtractPayload,
  sources: readonly (InputLiveV50SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV50Scene(
      root.scene,
...
export function assertInputLiveV50RootProofs(
  proofs: readonly InputLiveV50RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV50ProbeProgram(
  writer: InputLiveV50WriterOwnership,
  sources: readonly InputLiveV50SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V50_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV50ProbePayload(
  value: unknown,
  writer: InputLiveV50WriterOwnership,
): InputLiveV50ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV50ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V50_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V50_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV50CaptureManifest(
  cells: readonly InputLiveV50CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V50_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V50_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V50_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V50_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V50_SOURCE_IDS.some(
...
export const inputLiveV50CaptureManifestSha256 = (
  cells: readonly InputLiveV50CaptureCell[],
): string => {
  validateInputLiveV50CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV50CaptureProgram(
  writer: InputLiveV50WriterOwnership,
  cell: InputLiveV50CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V50_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V50_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V50_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV50CapturePayload(
  value: unknown,
  cell: InputLiveV50CaptureCell,
  rawResponseBytes: number,
): InputLiveV50CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV50CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V50_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V50_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV50CaptureResponses(
  manifest: readonly InputLiveV50CaptureCell[],
  responses: readonly InputLiveV50CapturePayload[],
): void {
  validateInputLiveV50CaptureManifest(manifest);
  if (
...
export interface InputLiveV50ObjectiveReport {
  artifactVersion: "input-live-v50-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV50Objective(
  manifest: readonly InputLiveV50CaptureCell[],
  responses: readonly InputLiveV50CapturePayload[],
  readReference: (cell: InputLiveV50CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV50CaptureCell) => Buffer,
): InputLiveV50ObjectiveReport {
  assertInputLiveV50CaptureResponses(manifest, responses);
...
export function buildInputLiveV50CleanupProgram(
  writer: InputLiveV50WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V50_NAMESPACE,
...
export function validateInputLiveV50CleanupPayload(
  value: unknown,
  writer: InputLiveV50WriterOwnership,
): InputLiveV50CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV50CleanupPayload;
...
export const INPUT_LIVE_V50_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV50WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
