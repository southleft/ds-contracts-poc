...
import { chakraCheckboxAdapterConfig, chakraCheckboxSource } from "./fixtures/generated/checkbox.chakra.js";
import { shadcnCheckboxAdapterConfig, shadcnCheckboxSource } from "./fixtures/generated/checkbox.shadcn.js";
...
const EVIDENCE = "recipe/evidence/checkbox-live-pivot-v12";
...
const sources = [
  ...
  {
    // Not transcribed: proposed by recipe/fixture-reader/propose-fixture.ts
    // from extract/computed/out/chakra/checkbox — the first library the
    // recipe path was pointed at rather than taught.
    adapterIdentity: "chakra-checkbox-proposed-v1",
    displayName: "Chakra",
    source: chakraCheckboxSource,
    config: chakraCheckboxAdapterConfig,
  },
  {
    // Proposed from extract/computed/out/shadcn/checkbox — the first BARE
    // control (no label part): the recipe's label-less cell, oklch colours
    // read through the ledger, and the indeterminate glyph a NAMED GAP.
    adapterIdentity: "shadcn-checkbox-proposed-v1",
    displayName: "shadcn",
    source: shadcnCheckboxSource,
    config: shadcnCheckboxAdapterConfig,
  },
].map((entry) => {
...
const pluginWriter = emitCheckboxFigmaWriter(sources, { runIdentity: `${writer.runIdentity}-plugin`, target: "plugin" });
...
const receiptOwned = {
      artifactVersion: "checkbox-live-pivot-v12-prepare",
      teaching:
...
if (process.argv.includes("--check")) {
  ...
  if (writer.sourcePlans.length !== 5)
    throw new Error("checkbox prepare requires five library sources (three reviewed + the proposed Chakra and shadcn)");
  console.log(
    JSON.stringify({
      check: "ok",
      runIdentity: writer.runIdentity,
