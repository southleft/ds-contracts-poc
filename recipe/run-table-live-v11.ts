...
import {
  acceptTableLiveV11Response,
  createTableLiveV11Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV11Request,
  persistTableLiveV11RawResponse,
  persistTableLiveV11TechnicalGates,
  readTableLiveV11AcceptedRawPayload,
  type TableLiveV11Request,
  type TableLiveV11TransactionAuthorization,
} from "./table-live-v11-broker.js";
import {
  TABLE_LIVE_V11_CAPTURE_COUNT,
  assertTableLiveV11CaptureResponses,
  ...
  evaluateTableLiveV11Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV11Roots,
  validateTableLiveV11CaptureManifest,
  validateTableLiveV11CapturePayload,
  validateTableLiveV11CleanupPayload,
  validateTableLiveV11ExtractPayload,
  validateTableLiveV11ProbePayload,
  validateTableLiveV11WriterPayload,
  type TableLiveV11CaptureCell,
  type TableLiveV11CapturePayload,
...
import {
  ...
  TABLE_LIVE_V11_PLAN_PATH,
  type TableLiveV11ProofPlan,
...
export const TABLE_LIVE_V11_RUNNER_VERSION = "table-live-v11-runner-v1";

interface TableLiveV11JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV11OrchestratorState {
  artifactVersion: "table-live-v11-orchestrator-state-v1";
  ...
  journal: TableLiveV11JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV11ProofPlan;
  planSha256: string;
  captures: TableLiveV11CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV11SourceIdentity & {
...
export interface TableLiveV11NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV11OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v11 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V11_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V11_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV11ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV11CaptureCell[];
  validateTableLiveV11CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v11 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v11 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV11WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV11RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV11Request =>
  readJson<TableLiveV11Request>(file);

export class TableLiveV11Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV11Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV11OrchestratorState = {
      artifactVersion: "table-live-v11-orchestrator-state-v1",
    ...
    return new TableLiveV11Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV11OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV11Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV11NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV11NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v11 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV11NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v11 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV11RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV11Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV11CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV11AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV11WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV11Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV11ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV11Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV11Response(this.#directory, "writer", {
        validate: validateTableLiveV11WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV11Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV11RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV11Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV11ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV11Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV11RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v11-host-root-proofs-v1",
      ...
      const next = issueTableLiveV11Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV11ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV11Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV11ProbePayload(payload, ownership),
      });
      persistTableLiveV11TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v11 capture cell missing");
      const accepted = acceptTableLiveV11Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV11CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V11_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV11CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV11Objective(
    ...
    throw new Error(`Table live v11 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV11Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV11CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV11Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV11RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV11Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV11CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV11CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV11CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV11Authorization = (
  privateKey: KeyObject,
): TableLiveV11TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV11SimulationKey = (): KeyObject =>
