...
import { buildComboboxLiveV16CleanupRuntime } from "./combobox-live-v16-cleanup.js";
import {
  type ComboboxLiveV16FixedFixedPoint,
} from "./combobox-live-v16-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV16RestoreProgram,
  validateComboboxLiveV16RestorePayload,
  type ComboboxLiveV16RestorePayload,
} from "./combobox-live-v16-restore.js";
import {
  buildComboboxLiveV16RawPropertyRuntime,
  normalizeComboboxLiveV16Scene,
  type ComboboxLiveV16RawNode,
} from "./combobox-live-v16-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v16.js";
import {
...
} from "./scene-readback-combobox-v16.js";
...
export const COMBOBOX_LIVE_V16_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V16_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V16_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V16_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V16_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V16_SET_COUNT = 4;
export const COMBOBOX_LIVE_V16_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V16_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V16_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V16_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V16_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV16SourceId = (typeof COMBOBOX_LIVE_V16_SOURCE_IDS)[number];

export interface ComboboxLiveV16SourceIdentity {
  source: ComboboxLiveV16SourceId;
...
export interface ComboboxLiveV16WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV16ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV16SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV16RawNode;
    optionScene: ComboboxLiveV16RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV16RootProof {
  source: ComboboxLiveV16SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV16FixedFixedPoint;
}

export interface ComboboxLiveV16ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV16SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV16CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV16SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV16CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV16SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV16CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v16 ${label} must be an object`);
...
export function validateComboboxLiveV16WriterPayload(
  value: unknown,
): ComboboxLiveV16WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V16_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v16 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v16 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v16 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV16SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV16ExtractProgram(
  writer: ComboboxLiveV16WriterOwnership,
  sources: readonly ComboboxLiveV16SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v16 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V16_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V16_NAMESPACE)}
${buildComboboxLiveV16RawPropertyRuntime()}
...
export function validateComboboxLiveV16ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV16WriterOwnership,
): ComboboxLiveV16ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v16 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V16_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v16 extract schema/two-root mismatch");
  return payload as ComboboxLiveV16ExtractPayload;
}

export function proveComboboxLiveV16Roots<Instance>(
  extract: ComboboxLiveV16ExtractPayload,
  sources: readonly (ComboboxLiveV16SourceIdentity & {
...
): ComboboxLiveV16RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v16 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v16 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV16Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV16Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV16FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v16 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v16 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v16 compile lost owned sets");
...
export function assertComboboxLiveV16RootProofs(
  proofs: readonly ComboboxLiveV16RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v16 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v16 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV16ProbeProgram(
  writer: ComboboxLiveV16WriterOwnership,
  sources: readonly ComboboxLiveV16SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V16_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV16ProbePayload(
  value: unknown,
  writer: ComboboxLiveV16WriterOwnership,
): ComboboxLiveV16ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV16ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V16_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V16_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v16 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV16CaptureManifest(
  cells: readonly ComboboxLiveV16CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V16_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V16_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V16_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V16_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v16 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV16CaptureCell[],
): string => {
  validateComboboxLiveV16CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV16CaptureProgram(
  writer: ComboboxLiveV16WriterOwnership,
  cell: ComboboxLiveV16CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V16_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v16 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v16 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V16_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV16CapturePayload(
  value: unknown,
  cell: ComboboxLiveV16CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV16CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV16CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V16_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V16_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v16 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV16CaptureResponses(
  manifest: readonly ComboboxLiveV16CaptureCell[],
  responses: readonly ComboboxLiveV16CapturePayload[],
): void {
  validateComboboxLiveV16CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v16 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV16ObjectiveReport {
  artifactVersion: "combobox-live-v16-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV16Objective(
  manifest: readonly ComboboxLiveV16CaptureCell[],
  responses: readonly ComboboxLiveV16CapturePayload[],
): ComboboxLiveV16ObjectiveReport {
  assertComboboxLiveV16CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v16-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV16CleanupProgram(
  writer: ComboboxLiveV16WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV16CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V16_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v16 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV16CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV16WriterOwnership,
): ComboboxLiveV16CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV16CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v16 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V16_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV16WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
