...
import { buildTableLiveV29CleanupRuntime } from "./table-live-v29-cleanup.js";
import { type TableLiveV29FixedFixedPoint } from "./table-live-v29-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV29RestoreProgram,
  validateTableLiveV29RestorePayload,
  type TableLiveV29RestorePayload,
} from "./table-live-v29-restore.js";
import {
  buildTableLiveV29RawPropertyRuntime,
  normalizeTableLiveV29Scene,
  type TableLiveV29RawNode,
} from "./table-live-v29-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v29.js";
import {
...
export const TABLE_LIVE_V29_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V29_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V29_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V29_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V29_VARIANT_COUNT = 20;
export const TABLE_LIVE_V29_SET_COUNT = 6;
export const TABLE_LIVE_V29_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V29_HOST_PHASES = 3;
export const TABLE_LIVE_V29_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V29_RESTORE_COUNT = 8;
export const TABLE_LIVE_V29_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V29_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV29SourceId = (typeof TABLE_LIVE_V29_SOURCE_IDS)[number];
export type TableLiveV29Kind = "table" | "row" | "cell";

export interface TableLiveV29SourceIdentity {
  source: TableLiveV29SourceId;
...
export interface TableLiveV29WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV29ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV29SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV29RawNode;
    rowScene: TableLiveV29RawNode;
    cellScene: TableLiveV29RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV29RootProof {
  source: TableLiveV29SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV29FixedFixedPoint;
}

export interface TableLiveV29ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV29SourceId;
    adapterIdentity: string;
...
export interface TableLiveV29CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV29SourceId;
  adapterIdentity: string;
...
export interface TableLiveV29CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV29SourceId;
  frameWidth: number;
...
export interface TableLiveV29CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v29 ${label} must be an object`);
...
export function validateTableLiveV29WriterPayload(
  value: unknown,
): TableLiveV29WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V29_NAMESPACE ||
    ...
    throw new TypeError("Table live v29 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Table live v29 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v29 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV29SourceIdentity,
  kind: TableLiveV29Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV29ExtractProgram(
  writer: TableLiveV29WriterOwnership,
  sources: readonly TableLiveV29SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
      "Table live v29 extract requires two source identities",
    );
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V29_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V29_NAMESPACE)}
${buildTableLiveV29RawPropertyRuntime()}
...
export function validateTableLiveV29ExtractPayload(
  value: unknown,
  writer: TableLiveV29WriterOwnership,
): TableLiveV29ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v29 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V29_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v29 extract schema/two-root mismatch");
  return payload as TableLiveV29ExtractPayload;
}

export function proveTableLiveV29Roots<Instance>(
  extract: TableLiveV29ExtractPayload,
  sources: readonly (TableLiveV29SourceIdentity & {
...
): TableLiveV29RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v29 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v29 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV29Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV29Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV29Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV29FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Table live v29 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v29 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v29 compile lost owned sets");
...
export function assertTableLiveV29RootProofs(
  proofs: readonly TableLiveV29RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v29 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v29 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV29ProbeProgram(
  writer: TableLiveV29WriterOwnership,
  sources: readonly TableLiveV29SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V29_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV29ProbePayload(
  value: unknown,
  writer: TableLiveV29WriterOwnership,
): TableLiveV29ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV29ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V29_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v29 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV29CaptureManifest(
  cells: readonly TableLiveV29CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V29_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V29_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V29_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V29_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v29 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV29CaptureCell[],
): string => {
  validateTableLiveV29CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV29CaptureProgram(
  writer: TableLiveV29WriterOwnership,
  cell: TableLiveV29CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V29_CAPTURE_COUNT
  )
    throw new TypeError("Table live v29 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Table live v29 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V29_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV29CapturePayload(
  value: unknown,
  cell: TableLiveV29CaptureCell,
  rawResponseBytes: number,
): TableLiveV29CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV29CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V29_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V29_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v29 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV29CaptureResponses(
  manifest: readonly TableLiveV29CaptureCell[],
  responses: readonly TableLiveV29CapturePayload[],
): void {
  validateTableLiveV29CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v29 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV29ObjectiveReport {
  artifactVersion: "table-live-v29-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV29Objective(
  manifest: readonly TableLiveV29CaptureCell[],
  responses: readonly TableLiveV29CapturePayload[],
): TableLiveV29ObjectiveReport {
  assertTableLiveV29CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v29-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV29CleanupProgram(
  writer: TableLiveV29WriterOwnership,
): string {
  const baseProgram = buildTableLiveV29CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V29_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v29 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV29CleanupPayload(
  payload: unknown,
  writer: TableLiveV29WriterOwnership,
): TableLiveV29CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as TableLiveV29CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v29 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V29_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV29WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
