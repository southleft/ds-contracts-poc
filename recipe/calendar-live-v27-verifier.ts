...
import {
  verifyCalendarLiveV27FixedSceneFixedPoint,
  type CalendarLiveV27FixedFixedPoint,
} from "./calendar-live-v27-fixed-point.js";

export const CALENDAR_LIVE_V27_VERIFIER_VERSION = "calendar-live-v27-verifier-v1";

export function buildCalendarLiveV27RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV27RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV27RawNode[];
}

export interface CalendarLiveV27NormalizedScene {
...
export interface CalendarLiveV27SceneProof {
...
export function normalizeCalendarLiveV27Scene(
  root: CalendarLiveV27RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV27NormalizedScene {
  const canonicalBindings: CalendarLiveV27NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV27RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV27Scene<Instance>(
  normalized: CalendarLiveV27NormalizedScene,
...
): CalendarLiveV27SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV27FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV27ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV27PreCaptureGates(payload: unknown): void {
