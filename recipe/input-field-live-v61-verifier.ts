...
export const INPUT_LIVE_V61_VERIFIER_VERSION = "input-live-v61-verifier-v1";

export function buildInputLiveV61RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV61RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV61RawNode[];
}

export interface InputLiveV61NormalizedScene {
...
export interface InputLiveV61SceneProof {
...
export function normalizeInputLiveV61Scene(
  root: InputLiveV61RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV61NormalizedScene {
  const canonicalBindings: InputLiveV61NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV61RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV61Scene<Instance>(
  normalized: InputLiveV61NormalizedScene,
...
export function compileInputLiveV61ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV61PreCaptureGates(payload: unknown): void {
