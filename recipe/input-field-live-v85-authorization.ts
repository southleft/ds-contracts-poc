...
import {
  ...
  INPUT_LIVE_V85_INDEX_PATH,
} from "./build-input-field-live-proof-v85.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV85HistoryExpectation = "pending" | "authorized";

export interface InputLiveV85AuthorizationArtifact {
  artifactVersion: "input-live-v85-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V85_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV85AntecedentIndex {
...
export interface InputLiveV85HistoryState {
  index?: InputLiveV85AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV85AuthorizationArtifact;
...
export interface InputLiveV85AuthorizationProof extends InputLiveV85TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V85_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V85_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V85_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V85_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("input-field-live-v85-authorization") ||
  artifactPath.includes("input-field-live-v85-preflight") ||
  artifactPath.includes("input-field-live-v85-authorized") ||
  artifactPath.includes("input-field-live-pivot-v85-status") ||
...
export function validateInputLiveV85AntecedentIndex(
  index: InputLiveV85AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v85-antecedent-index-v1" ||
...
export function validateInputLiveV85UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV85History\(\s*["']/.test(source) ||
    /readInputLiveV85HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV85AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV85AuthorizationArtifact {
  ...
  ) as InputLiveV85AntecedentIndex;
  const indexFailures = validateInputLiveV85AntecedentIndex(index);
...
export function validateInputLiveV85AuthorizationArtifact(
  artifact: InputLiveV85AuthorizationArtifact | undefined,
  index: InputLiveV85AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v85-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v85" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V85_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V85_TARGET) ||
    !equalJson(
...
export function validateInputLiveV85History(
  state: InputLiveV85HistoryState,
  expected: InputLiveV85HistoryExpectation,
): string[] {
  const failures = validateInputLiveV85AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV85AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV85HistoryState(
  root = process.cwd(),
): InputLiveV85HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V85_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV85AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V85_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V85_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V85_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV85History(
  expected: InputLiveV85HistoryExpectation,
  root = process.cwd(),
): InputLiveV85HistoryState {
  const state = readInputLiveV85HistoryState(root);
  const failures = validateInputLiveV85History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV85Authorization(
  root = process.cwd(),
): InputLiveV85AuthorizationProof {
  const state = verifyInputLiveV85History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V85_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v85/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v85.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V85_TARGET,
    expectedDynamicTool: INPUT_LIVE_V85_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V85_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV85HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV85History(expected);
