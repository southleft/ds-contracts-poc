/**
 * Carried Combobox live v25 host verifier. Uses scene-readback-combobox-v25 so live
...
export const COMBOBOX_LIVE_V25_FIXED_VERIFIER_VERSION = "combobox-live-v25-fixed-point-v1";
export const COMBOBOX_LIVE_V25_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV25FixedThresholds {
...
export interface ComboboxLiveV25FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV25FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV25FixedSourceProbe {
...
export interface ComboboxLiveV25FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV25FixedFixedPoint;
}

export interface ComboboxLiveV25FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV25FixedHardGateInput {
  thresholds: ComboboxLiveV25FixedThresholds;
  materialRegression: ComboboxLiveV25FixedMaterialRegression;
  sourceProbes: ComboboxLiveV25FixedSourceProbe[];
  cells: ComboboxLiveV25FixedCellValidation[];
  sceneProofs: ComboboxLiveV25FixedSceneProof[];
  visualRows: ComboboxLiveV25FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV25FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V25_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV25FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV25FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV25FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV25FixedHardGates(
  input: ComboboxLiveV25FixedHardGateInput,
): ComboboxLiveV25FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV25FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV25FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V25_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV25FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV25FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV25FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
