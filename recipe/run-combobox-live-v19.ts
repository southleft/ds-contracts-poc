...
import {
  acceptComboboxLiveV19Response,
  createComboboxLiveV19Transaction,
  comboboxLiveV5ReceiptPath,
  comboboxLiveV5RequestPath,
  comboboxLiveV5Sha256,
  issueComboboxLiveV19Request,
  persistComboboxLiveV19RawResponse,
  persistComboboxLiveV19TechnicalGates,
  readComboboxLiveV19AcceptedRawPayload,
  type ComboboxLiveV19Request,
  type ComboboxLiveV19TransactionAuthorization,
} from "./combobox-live-v19-broker.js";
import {
  COMBOBOX_LIVE_V19_CAPTURE_COUNT,
  assertComboboxLiveV19CaptureResponses,
  ...
  evaluateComboboxLiveV19Objective,
  comboboxLiveV5CaptureManifestSha256,
  proveComboboxLiveV19Roots,
  validateComboboxLiveV19CaptureManifest,
  validateComboboxLiveV19CapturePayload,
  validateComboboxLiveV19CleanupPayload,
  validateComboboxLiveV19ExtractPayload,
  validateComboboxLiveV19ProbePayload,
  validateComboboxLiveV19WriterPayload,
  type ComboboxLiveV19CaptureCell,
  type ComboboxLiveV19CapturePayload,
...
import {
  ...
  COMBOBOX_LIVE_V19_PLAN_PATH,
  type ComboboxLiveV19ProofPlan,
} from "./build-combobox-live-proof-v19.js";
import {
...
import type { ExpectedScenePlan } from "./scene-readback-combobox-v19.js";

export const COMBOBOX_LIVE_V19_RUNNER_VERSION = "combobox-live-v19-runner-v1";

interface ComboboxLiveV19JournalEntry {
  index: number;
  event: string;
...
interface ComboboxLiveV19OrchestratorState {
  artifactVersion: "combobox-live-v19-orchestrator-state-v1";
  ...
  journal: ComboboxLiveV19JournalEntry[];
}

interface LoadedProofInputs {
  plan: ComboboxLiveV19ProofPlan;
  planSha256: string;
  captures: ComboboxLiveV19CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    ComboboxLiveV19SourceIdentity & {
...
export interface ComboboxLiveV19NextAction {
...
const writeState = (
  directory: string,
  state: ComboboxLiveV19OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Combobox live v19 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, COMBOBOX_LIVE_V19_PLAN_PATH);
  const capturePath = path.join(root, COMBOBOX_LIVE_V19_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as ComboboxLiveV19ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as ComboboxLiveV19CaptureCell[];
  validateComboboxLiveV19CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Combobox live v19 capture manifest hash drift");
  const descriptors = [
    {
      source: "mui" as const,
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Combobox live v19 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): ComboboxLiveV19WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): ComboboxLiveV19RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): ComboboxLiveV19Request =>
  readJson<ComboboxLiveV19Request>(file);

export class ComboboxLiveV19Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueComboboxLiveV19Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: ComboboxLiveV19OrchestratorState = {
      artifactVersion: "combobox-live-v19-orchestrator-state-v1",
    ...
    return new ComboboxLiveV19Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<ComboboxLiveV19OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new ComboboxLiveV19Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): ComboboxLiveV19NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): ComboboxLiveV19NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Combobox live v19 has no active main request");
  ...
  ingestCleanup(rawResponse: string): ComboboxLiveV19NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Combobox live v19 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistComboboxLiveV19RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptComboboxLiveV19Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateComboboxLiveV19CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readComboboxLiveV19AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateComboboxLiveV19WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueComboboxLiveV19Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildComboboxLiveV19ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: ComboboxLiveV19Request): void {
    if (request.phase === "writer") {
      const accepted = acceptComboboxLiveV19Response(this.#directory, "writer", {
        validate: validateComboboxLiveV19WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptComboboxLiveV19Response(this.#directory, "restore", {
        validate: (payload) =>
          validateComboboxLiveV19RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptComboboxLiveV19Response(this.#directory, "extract", {
        validate: (payload) =>
          validateComboboxLiveV19ExtractPayload(payload, ownership),
      });
      const proofs = proveComboboxLiveV19Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseComboboxRecipe,
        compileComboboxRecipe,
      );
      assertComboboxLiveV19RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "combobox-live-v19-host-root-proofs-v1",
      ...
      const next = issueComboboxLiveV19Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildComboboxLiveV19ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = comboboxLiveV5RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptComboboxLiveV19Response(this.#directory, "probe", {
        validate: (payload) =>
          validateComboboxLiveV19ProbePayload(payload, ownership),
      });
      persistComboboxLiveV19TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Combobox live v19 capture cell missing");
      const accepted = acceptComboboxLiveV19Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateComboboxLiveV19CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < COMBOBOX_LIVE_V19_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertComboboxLiveV19CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateComboboxLiveV19Objective(
    ...
    throw new Error(`Combobox live v19 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueComboboxLiveV19Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildComboboxLiveV19CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueComboboxLiveV19Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildComboboxLiveV19RestoreProgram(ownership),
    );
    this.#state.activeRequest = comboboxLiveV5RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueComboboxLiveV19Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildComboboxLiveV19CaptureProgram(
  ...
  #readCaptureResponseIndex(): ComboboxLiveV19CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: ComboboxLiveV19CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedComboboxLiveV19Authorization = (
  privateKey: KeyObject,
): ComboboxLiveV19TransactionAuthorization => ({
  mode: "simulated",
...
export const generateComboboxLiveV19SimulationKey = (): KeyObject =>
