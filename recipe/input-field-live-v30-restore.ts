...
export const INPUT_LIVE_V30_MEASURE_VISIBLE_FILL_MARKER =
  "INPUT-TEXT-FILL-MEASURE-VISIBLE";
export const INPUT_LIVE_V30_RESTORE_CONTENT_ROLES = [
  "input-field/content/placeholder",
  "input-field/content/value",
] as const;
export const INPUT_LIVE_V30_RESTORE_PARENT_ROLES = [
  "input-field/surface",
  "input-field/content-row",
] as const;

export interface InputLiveV30RestoreWriter {
  pageId: string;
  runIdentity: string;
  setIds: readonly [string, string];
}

export interface InputLiveV30RestorePayload {
  pageId: string;
  setIds: string[];
  restoredCount: 256;
  fixedBefore: number;
  ...
  marker: typeof INPUT_LIVE_V30_MEASURE_VISIBLE_FILL_MARKER;
}

export function buildInputLiveV30RestoreProgram(
  writer: InputLiveV30RestoreWriter,
): string {
  if (writer.setIds.length !== 2)
    throw new TypeError("Input live v30 restore requires two owned sets");
  return String.raw`
await figma.loadAllPagesAsync();
const V30_NS=${JSON.stringify(INPUT_LIVE_V30_NAMESPACE)};
const V30_PAGE_ID=${JSON.stringify(writer.pageId)};
const V30_SET_IDS=${JSON.stringify([...writer.setIds])};
const V30_ROLES=new Set(${JSON.stringify([...INPUT_LIVE_V30_RESTORE_CONTENT_ROLES])});
const V30_PARENTS=new Set(${JSON.stringify([...INPUT_LIVE_V30_RESTORE_PARENT_ROLES])});
const V30_MARKER=${JSON.stringify(INPUT_LIVE_V30_MEASURE_VISIBLE_FILL_MARKER)};
const page=await figma.getNodeByIdAsync(V30_PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("INPUT-V30-RESTORE-PAGE");
if(page.getSharedPluginData(V30_NS,"pageOwner")!=="recipe/input-field/"+${JSON.stringify(writer.runIdentity)})throw new Error("INPUT-V30-RESTORE-OWNER");
...
export function validateInputLiveV30RestorePayload(
  payload: unknown,
  writer: InputLiveV30RestoreWriter,
): InputLiveV30RestorePayload {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("Input live v30 restore payload must be an object");
  ...
  if (
    ...
    value.marker !== INPUT_LIVE_V30_MEASURE_VISIBLE_FILL_MARKER
  )
    throw new TypeError(
      "Input live v30 restore did not re-assert content FILL",
    );
  return value as InputLiveV30RestorePayload;
}
