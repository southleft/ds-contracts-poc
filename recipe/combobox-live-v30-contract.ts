...
import { buildComboboxLiveV30CleanupRuntime } from "./combobox-live-v30-cleanup.js";
import {
  type ComboboxLiveV30FixedFixedPoint,
} from "./combobox-live-v30-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV30RestoreProgram,
  validateComboboxLiveV30RestorePayload,
  type ComboboxLiveV30RestorePayload,
} from "./combobox-live-v30-restore.js";
import {
  buildComboboxLiveV30RawPropertyRuntime,
  normalizeComboboxLiveV30Scene,
  type ComboboxLiveV30RawNode,
} from "./combobox-live-v30-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v30.js";
import {
...
} from "./scene-readback-combobox-v30.js";
...
export const COMBOBOX_LIVE_V30_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V30_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V30_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V30_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V30_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V30_SET_COUNT = 4;
export const COMBOBOX_LIVE_V30_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V30_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V30_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V30_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V30_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV30SourceId = (typeof COMBOBOX_LIVE_V30_SOURCE_IDS)[number];

export interface ComboboxLiveV30SourceIdentity {
  source: ComboboxLiveV30SourceId;
...
export interface ComboboxLiveV30WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV30ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV30SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV30RawNode;
    optionScene: ComboboxLiveV30RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV30RootProof {
  source: ComboboxLiveV30SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV30FixedFixedPoint;
}

export interface ComboboxLiveV30ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV30SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV30CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV30SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV30CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV30SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV30CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v30 ${label} must be an object`);
...
export function validateComboboxLiveV30WriterPayload(
  value: unknown,
): ComboboxLiveV30WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V30_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v30 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v30 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v30 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV30SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV30ExtractProgram(
  writer: ComboboxLiveV30WriterOwnership,
  sources: readonly ComboboxLiveV30SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v30 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V30_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V30_NAMESPACE)}
${buildComboboxLiveV30RawPropertyRuntime()}
...
export function validateComboboxLiveV30ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV30WriterOwnership,
): ComboboxLiveV30ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v30 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V30_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v30 extract schema/two-root mismatch");
  return payload as ComboboxLiveV30ExtractPayload;
}

export function proveComboboxLiveV30Roots<Instance>(
  extract: ComboboxLiveV30ExtractPayload,
  sources: readonly (ComboboxLiveV30SourceIdentity & {
...
): ComboboxLiveV30RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v30 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v30 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV30Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV30Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV30FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v30 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v30 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v30 compile lost owned sets");
...
export function assertComboboxLiveV30RootProofs(
  proofs: readonly ComboboxLiveV30RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v30 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v30 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV30ProbeProgram(
  writer: ComboboxLiveV30WriterOwnership,
  sources: readonly ComboboxLiveV30SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V30_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateComboboxLiveV30ProbePayload(
  value: unknown,
  writer: ComboboxLiveV30WriterOwnership,
): ComboboxLiveV30ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV30ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V30_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V30_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v30 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV30CaptureManifest(
  cells: readonly ComboboxLiveV30CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V30_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V30_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V30_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V30_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v30 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV30CaptureCell[],
): string => {
  validateComboboxLiveV30CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV30CaptureProgram(
  writer: ComboboxLiveV30WriterOwnership,
  cell: ComboboxLiveV30CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V30_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v30 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v30 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V30_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V8-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV30CapturePayload(
  value: unknown,
  cell: ComboboxLiveV30CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV30CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV30CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V30_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V30_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v30 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV30CaptureResponses(
  manifest: readonly ComboboxLiveV30CaptureCell[],
  responses: readonly ComboboxLiveV30CapturePayload[],
): void {
  validateComboboxLiveV30CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v30 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV30ObjectiveReport {
  artifactVersion: "combobox-live-v30-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV30Objective(
  manifest: readonly ComboboxLiveV30CaptureCell[],
  responses: readonly ComboboxLiveV30CapturePayload[],
): ComboboxLiveV30ObjectiveReport {
  assertComboboxLiveV30CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v30-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV30CleanupProgram(
  writer: ComboboxLiveV30WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV30CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V30_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Combobox live v30 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV30CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV30WriterOwnership,
): ComboboxLiveV30CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV30CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v30 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V30_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV30WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
