...
import {
  ...
  createInputLiveV85FakeDynamicResponse,
  ...
  listInputLiveV85PersistedRequests,
  persistInputLiveV85RawResponse,
  persistInputLiveV85TechnicalGates,
  validateInputLiveV85Receipt,
  validateInputLiveV85Request,
  type InputLiveV85RemotePhase,
...
import {
  ...
  type InputLiveV85ExtractPayload,
  type InputLiveV85ProbePayload,
...
import {
  ...
  simulatedInputLiveV85Authorization,
} from "./run-input-field-live-v85.js";
...
const captures: InputLiveV85CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV85Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v85-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV85Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV85CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV85FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV85FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V85_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V85_TARGET);
  persistInputLiveV85RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV85Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV85Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV85WriterOwnership,
...
const rootProofs = (): InputLiveV85RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v85-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV85Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV85Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV85RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV85RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV85ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV85ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV85CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV85CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV85CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV85Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV85CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV85PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV85ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV85Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV85Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV85Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV85Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V85_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV85RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V85_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV85Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV85Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV85ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV85Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV85WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV85RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV85Request;
    validateInputLiveV85Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV85Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV85CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v85-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV85Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV85Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV85Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("main-complete surfaces cleanup for recovery and does not instruct execution", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v85-main-complete-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV85Orchestrator.initialize({
      ...
      authorization: simulatedInputLiveV85Authorization(privateKey),
      attempt: 1,
      transactionId: "00000000-0000-4000-8000-000000000085",
      now: "2026-08-28T12:00:00.000Z",
    });
    const writerRequest = requestFrom(
      orchestrator.nextAction().activeRequestPath!,
    );
    orchestrator.ingestAndAdvance(rawResponse(writerRequest, writerPayload()));
    const statePath = path.join(directory, "orchestrator-state.json");
    const state = JSON.parse(readFileSync(statePath, "utf8"));
    state.mainComplete = true;
    state.activeRequest = null;
    writeFileSync(statePath, `${JSON.stringify(state, null, 2)}\n`);
    const next = InputLiveV85Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
    assert.equal(next.status, "main-complete");
    assert.equal(next.activeRequestPath, null);
    assert.match(next.cleanupRequestPath ?? "", /cleanup/);
    assert.equal(next.expectedDynamicTool, null);
    assert.match(next.instructions, /failure\/abort recovery only/);
    assert.doesNotMatch(next.instructions, /Execute the persisted cleanup/);
  } finally {
    rmSync(directory, { recursive: true, force: true });
  }
});
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V85_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV85RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V85_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV85Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV85Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV85RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V85_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV85Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV85Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV85Request;
