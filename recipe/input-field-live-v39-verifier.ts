...
export const INPUT_LIVE_V39_VERIFIER_VERSION = "input-live-v39-verifier-v1";

export function buildInputLiveV39RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV39RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV39RawNode[];
}

export interface InputLiveV39NormalizedScene {
...
export interface InputLiveV39SceneProof {
...
export function normalizeInputLiveV39Scene(
  root: InputLiveV39RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV39NormalizedScene {
  const canonicalBindings: InputLiveV39NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV39RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV39Scene<Instance>(
  normalized: InputLiveV39NormalizedScene,
...
export function compileInputLiveV39ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV39PreCaptureGates(payload: unknown): void {
