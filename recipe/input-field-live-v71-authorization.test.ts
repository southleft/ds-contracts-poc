...
import { INPUT_LIVE_V71_INDEX_PATH } from "./build-input-field-live-proof-v71.js";
import {
...
import {
  ...
  validateInputLiveV71AntecedentIndex,
  validateInputLiveV71AuthorizationArtifact,
  validateInputLiveV71History,
  validateInputLiveV71UnitTestSource,
  type InputLiveV71AntecedentIndex,
  type InputLiveV71AuthorizationArtifact,
  type InputLiveV71AuthorizationProof,
  type InputLiveV71HistoryState,
} from "./input-field-live-v71-authorization.js";
import {
  inputLiveV71AttestationSha256,
  validateInputLiveV71ControlFlowSource,
  validateInputLiveV71SecurityAttestation,
  type InputLiveV71SecurityAttestation,
} from "./input-field-live-v71-preflight.js";
import { inputLiveV71Sha256 } from "./input-field-live-v71-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V71_INDEX_PATH);
const index = (): InputLiveV71AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV71AntecedentIndex;

const artifact = (): InputLiveV71AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV71AuthorizationArtifact({
...
const pendingState = (): InputLiveV71HistoryState => ({
  index: index(),
  indexSha256: inputLiveV71Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV71HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV71AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV71Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V71_TARGET,
  expectedDynamicTool: INPUT_LIVE_V71_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v71/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV71AuthorizationArtifact,
): InputLiveV71SecurityAttestation => {
  const body: Omit<InputLiveV71SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v71-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V71_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV71AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV71History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV71History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV71History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV71History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV71History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV71UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV71UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV71AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV71Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV71AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV71AuthorizationArtifact(
        planted as InputLiveV71AuthorizationArtifact,
        index(),
        inputLiveV71Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV71SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV71AttestationSha256(
      body as Omit<InputLiveV71SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV71SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV71ControlFlowSource(source), []);
});
