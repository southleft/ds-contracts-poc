...
export type CalendarLiveV19HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV19AuthorizationArtifact {
  artifactVersion: "calendar-live-v19-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V19_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV19AntecedentIndex {
...
export interface CalendarLiveV19HistoryState {
  index?: CalendarLiveV19AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV19AuthorizationArtifact;
...
export interface CalendarLiveV19AuthorizationProof
  extends CalendarLiveV19TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V19_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V19_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V19_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V19_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v19-authorization") ||
  artifactPath.includes("calendar-live-v19-preflight") ||
  artifactPath.includes("calendar-live-v19-authorized") ||
  artifactPath.includes("create-calendar-live-v19-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v19-status") ||
...
export function validateCalendarLiveV19AntecedentIndex(
  index: CalendarLiveV19AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v19-antecedent-index-v1" ||
...
export function validateCalendarLiveV19UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV19History\(\s*["']/.test(source) ||
    /readCalendarLiveV19HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV19AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV19AuthorizationArtifact {
  ...
  ) as CalendarLiveV19AntecedentIndex;
  const indexFailures = validateCalendarLiveV19AntecedentIndex(index);
...
export function validateCalendarLiveV19AuthorizationArtifact(
  artifact: CalendarLiveV19AuthorizationArtifact | undefined,
  index: CalendarLiveV19AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v19-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v19" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V19_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V19_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V19_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V19_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V19_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V19_SOURCE_ROOTS ||
...
export function validateCalendarLiveV19History(
  state: CalendarLiveV19HistoryState,
  expected: CalendarLiveV19HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV19AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV19AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV19HistoryState(
  root = process.cwd(),
): CalendarLiveV19HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V19_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV19AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V19_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V19_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V19_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV19History(
  expected: CalendarLiveV19HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV19HistoryState {
  const state = readCalendarLiveV19HistoryState(root);
  const failures = validateCalendarLiveV19History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v19 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV19Authorization(
  root = process.cwd(),
): CalendarLiveV19AuthorizationProof {
  const state = verifyCalendarLiveV19History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V19_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v19/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v19.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V19_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V19_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V19_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV19HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV19History(expected);
  process.stdout.write(
    `Calendar live v19 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
