...
import {
  acceptCalendarLiveV41Response,
  createCalendarLiveV41Transaction,
  calendarLiveV1ReceiptPath,
  calendarLiveV1RequestPath,
  calendarLiveV1Sha256,
  issueCalendarLiveV41Request,
  persistCalendarLiveV41RawResponse,
  persistCalendarLiveV41TechnicalGates,
  readCalendarLiveV41AcceptedRawPayload,
  type CalendarLiveV41Request,
  type CalendarLiveV41TransactionAuthorization,
} from "./calendar-live-v41-broker.js";
import {
  CALENDAR_LIVE_V41_CAPTURE_COUNT,
  assertCalendarLiveV41CaptureResponses,
  ...
  evaluateCalendarLiveV41Objective,
  calendarLiveV1CaptureManifestSha256,
  proveCalendarLiveV41Roots,
  validateCalendarLiveV41CaptureManifest,
  validateCalendarLiveV41CapturePayload,
  validateCalendarLiveV41CleanupPayload,
  validateCalendarLiveV41ExtractPayload,
  validateCalendarLiveV41ProbePayload,
  validateCalendarLiveV41WriterPayload,
  type CalendarLiveV41CaptureCell,
  type CalendarLiveV41CapturePayload,
...
import {
  ...
  CALENDAR_LIVE_V41_PLAN_PATH,
  type CalendarLiveV41ProofPlan,
...
export const CALENDAR_LIVE_V41_RUNNER_VERSION = "calendar-live-v41-runner-v1";

interface CalendarLiveV41JournalEntry {
  index: number;
  event: string;
...
interface CalendarLiveV41OrchestratorState {
  artifactVersion: "calendar-live-v41-orchestrator-state-v1";
  ...
  journal: CalendarLiveV41JournalEntry[];
}

interface LoadedProofInputs {
  plan: CalendarLiveV41ProofPlan;
  planSha256: string;
  captures: CalendarLiveV41CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    CalendarLiveV41SourceIdentity & {
...
export interface CalendarLiveV41NextAction {
...
const writeState = (
  directory: string,
  state: CalendarLiveV41OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Calendar live v41 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, CALENDAR_LIVE_V41_PLAN_PATH);
  const capturePath = path.join(root, CALENDAR_LIVE_V41_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as CalendarLiveV41ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as CalendarLiveV41CaptureCell[];
  validateCalendarLiveV41CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Calendar live v41 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Calendar live v41 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): CalendarLiveV41WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): CalendarLiveV41RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): CalendarLiveV41Request =>
  readJson<CalendarLiveV41Request>(file);

export class CalendarLiveV41Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueCalendarLiveV41Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: CalendarLiveV41OrchestratorState = {
      artifactVersion: "calendar-live-v41-orchestrator-state-v1",
    ...
    return new CalendarLiveV41Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<CalendarLiveV41OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new CalendarLiveV41Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): CalendarLiveV41NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): CalendarLiveV41NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Calendar live v41 has no active main request");
  ...
  ingestCleanup(rawResponse: string): CalendarLiveV41NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Calendar live v41 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistCalendarLiveV41RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptCalendarLiveV41Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateCalendarLiveV41CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readCalendarLiveV41AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateCalendarLiveV41WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueCalendarLiveV41Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildCalendarLiveV41ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: CalendarLiveV41Request): void {
    if (request.phase === "writer") {
      const accepted = acceptCalendarLiveV41Response(this.#directory, "writer", {
        validate: validateCalendarLiveV41WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptCalendarLiveV41Response(this.#directory, "restore", {
        validate: (payload) =>
          validateCalendarLiveV41RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptCalendarLiveV41Response(this.#directory, "extract", {
        validate: (payload) =>
          validateCalendarLiveV41ExtractPayload(payload, ownership),
      });
      const proofs = proveCalendarLiveV41Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseCalendarRecipe,
        compileCalendarRecipe,
      );
      assertCalendarLiveV41RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "calendar-live-v41-host-root-proofs-v1",
      ...
      const next = issueCalendarLiveV41Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildCalendarLiveV41ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = calendarLiveV1RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptCalendarLiveV41Response(this.#directory, "probe", {
        validate: (payload) =>
          validateCalendarLiveV41ProbePayload(payload, ownership),
      });
      persistCalendarLiveV41TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Calendar live v41 capture cell missing");
      const accepted = acceptCalendarLiveV41Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateCalendarLiveV41CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < CALENDAR_LIVE_V41_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertCalendarLiveV41CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateCalendarLiveV41Objective(
    ...
    throw new Error(`Calendar live v41 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueCalendarLiveV41Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildCalendarLiveV41CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = calendarLiveV1RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueCalendarLiveV41Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildCalendarLiveV41RestoreProgram(ownership),
    );
    this.#state.activeRequest = calendarLiveV1RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueCalendarLiveV41Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildCalendarLiveV41CaptureProgram(
  ...
  #readCaptureResponseIndex(): CalendarLiveV41CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: CalendarLiveV41CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedCalendarLiveV41Authorization = (
  privateKey: KeyObject,
): CalendarLiveV41TransactionAuthorization => ({
  mode: "simulated",
...
export const generateCalendarLiveV41SimulationKey = (): KeyObject =>
