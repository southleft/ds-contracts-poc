...
export const INPUT_LIVE_V36_VERIFIER_VERSION = "input-live-v36-verifier-v1";

export function buildInputLiveV36RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV36RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV36RawNode[];
}

export interface InputLiveV36NormalizedScene {
...
export interface InputLiveV36SceneProof {
...
export function normalizeInputLiveV36Scene(
  root: InputLiveV36RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV36NormalizedScene {
  const canonicalBindings: InputLiveV36NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV36RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV36Scene<Instance>(
  normalized: InputLiveV36NormalizedScene,
...
export function compileInputLiveV36ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV36PreCaptureGates(payload: unknown): void {
