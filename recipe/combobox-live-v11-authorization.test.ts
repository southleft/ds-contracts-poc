...
import { COMBOBOX_LIVE_V11_INDEX_PATH } from "./build-combobox-live-proof-v11.js";
import {
  COMBOBOX_LIVE_V11_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V11_TARGET,
} from "./combobox-live-v11-broker.js";
import {
  buildComboboxLiveV11AuthorizationArtifact,
  validateComboboxLiveV11AntecedentIndex,
  validateComboboxLiveV11AuthorizationArtifact,
  validateComboboxLiveV11History,
  validateComboboxLiveV11UnitTestSource,
  type ComboboxLiveV11AntecedentIndex,
  type ComboboxLiveV11AuthorizationArtifact,
  type ComboboxLiveV11AuthorizationProof,
  type ComboboxLiveV11HistoryState,
} from "./combobox-live-v11-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV11ControlFlowSource,
  validateComboboxLiveV11SecurityAttestation,
  type ComboboxLiveV11SecurityAttestation,
} from "./combobox-live-v11-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v11-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V11_INDEX_PATH);
const index = (): ComboboxLiveV11AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV11AntecedentIndex;

const artifact = (): ComboboxLiveV11AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV11AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV11HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV11HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV11AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v11/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV11AuthorizationArtifact,
): ComboboxLiveV11SecurityAttestation => {
  const body: Omit<ComboboxLiveV11SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V11_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV11History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV11History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV11History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV11History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV11History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV11UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV11UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV11AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV11AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV11AuthorizationArtifact(
        planted as ComboboxLiveV11AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV11SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV11SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV11SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV11ControlFlowSource(source), []);
});
