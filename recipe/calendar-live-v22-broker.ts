...
import {
  ...
  CALENDAR_LIVE_V22_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  CALENDAR_LIVE_V22_RESPONSE_CONTRACTS,
  type CalendarLiveV22RootProof,
} from "./calendar-live-v22-contract.js";
import { canonicalJson } from "./normalize.js";

export const CALENDAR_LIVE_V22_BROKER_VERSION = "calendar-live-v22-broker-v1";
export const CALENDAR_LIVE_V22_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const CALENDAR_LIVE_V22_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const CALENDAR_LIVE_V22_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const CALENDAR_LIVE_V22_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const CALENDAR_LIVE_V22_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const CALENDAR_LIVE_V22_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type CalendarLiveV22RemotePhase =
  (typeof CALENDAR_LIVE_V22_REMOTE_PHASES)[number];
export type CalendarLiveV22AuthorizationMode = "live" | "simulated";

export interface CalendarLiveV22CommitPins {
...
export interface CalendarLiveV22TransactionAuthorization extends CalendarLiveV22CommitPins {
  mode: CalendarLiveV22AuthorizationMode;
...
export interface CalendarLiveV22Manifest {
  artifactVersion: "calendar-live-v22-broker-manifest-v2";
  brokerVersion: typeof CALENDAR_LIVE_V22_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof CALENDAR_LIVE_V22_DYNAMIC_TOOL;
  target: typeof CALENDAR_LIVE_V22_TARGET;
...
export interface CalendarLiveV22ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface CalendarLiveV22RequestBody {
  artifactVersion: "calendar-live-v22-broker-request-v2";
  brokerVersion: typeof CALENDAR_LIVE_V22_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: CalendarLiveV22RemotePhase;
  sequence: number;
  ...
  commitPins: CalendarLiveV22CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof CALENDAR_LIVE_V22_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: CalendarLiveV22ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface CalendarLiveV22Request extends CalendarLiveV22RequestBody {
...
export interface CalendarLiveV22Receipt {
  artifactVersion: "calendar-live-v22-broker-receipt-v2";
  brokerVersion: typeof CALENDAR_LIVE_V22_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: CalendarLiveV22RemotePhase;
...
export interface CalendarLiveV22AcceptedResponse<T = unknown> {
  request: CalendarLiveV22Request;
  receipt: CalendarLiveV22Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface CalendarLiveV22IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: CalendarLiveV22ResponseContract;
...
const equalTarget = (value: unknown): value is typeof CALENDAR_LIVE_V22_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === CALENDAR_LIVE_V22_TARGET.fileKey &&
  (value as BrokerTarget).fileName === CALENDAR_LIVE_V22_TARGET.fileName &&
  (value as BrokerTarget).editorType === CALENDAR_LIVE_V22_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== CALENDAR_LIVE_V22_TARGET.fileKey ||
    context.fileName !== CALENDAR_LIVE_V22_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === CALENDAR_LIVE_V22_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Calendar live v22 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= CALENDAR_LIVE_V22_CAPTURE_COUNT
  )
    throw new TypeError("Calendar live v22 capture index must be 0..7");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Calendar live v22 authorization pins are malformed");
};

const readManifest = (directory: string): CalendarLiveV22Manifest => {
  const manifest = json<CalendarLiveV22Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Calendar live v22 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "calendar-live-v22-broker-manifest-v2" ||
    manifest.brokerVersion !== CALENDAR_LIVE_V22_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== CALENDAR_LIVE_V22_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(CALENDAR_LIVE_V22_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Calendar live v22 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: CalendarLiveV22Request,
): CalendarLiveV22RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: CalendarLiveV22RemotePhase,
  captureIndex?: number,
): CalendarLiveV22Request | undefined => {
  const file = calendarLiveV1RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV22Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: CalendarLiveV22RemotePhase,
  captureIndex?: number,
): CalendarLiveV22Receipt | undefined => {
  const file = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<CalendarLiveV22Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: CalendarLiveV22Request | undefined;
  receipt: CalendarLiveV22Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: CalendarLiveV22RemotePhase,
): CalendarLiveV22ResponseContract => ({
  ...CALENDAR_LIVE_V22_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  manifest: CalendarLiveV22Manifest,
  value: {
    requestId: string;
    sequence: number;
    predecessorRequestSha256: string | null;
  ...
  sha256(
    canonicalJson({
      brokerVersion: CALENDAR_LIVE_V22_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: CALENDAR_LIVE_V22_DYNAMIC_TOOL,
      target: CALENDAR_LIVE_V22_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "calendar-live-v22-plugin-response-v2",
  brokerVersion: CALENDAR_LIVE_V22_BROKER_VERSION,
  target: CALENDAR_LIVE_V22_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*CALENDAR_LIVE_V22_PROGRAM_START*/${program}/*CALENDAR_LIVE_V22_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*CALENDAR_LIVE_V22_PROGRAM_START*/";
  const endMarker = "/*CALENDAR_LIVE_V22_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Calendar live v22 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createCalendarLiveV22Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Calendar live v22 private signing key does not match authorized public-key identity",
    );
  const manifest: CalendarLiveV22Manifest = {
    artifactVersion: "calendar-live-v22-broker-manifest-v2",
    brokerVersion: CALENDAR_LIVE_V22_BROKER_VERSION,
    ...
    captureRequests: CALENDAR_LIVE_V22_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateCalendarLiveV22Request(
  directory: string,
  request: CalendarLiveV22Request,
): void {
  ...
  if (
    request.artifactVersion !== "calendar-live-v22-broker-request-v2" ||
    request.brokerVersion !== CALENDAR_LIVE_V22_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== CALENDAR_LIVE_V22_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Calendar live v22 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueCalendarLiveV22Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: CalendarLiveV22RemotePhase,
  program: string,
  options: CalendarLiveV22IssueOptions = {},
): CalendarLiveV22Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<CalendarLiveV22Request>(targetPath);
    validateCalendarLiveV22Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Calendar live v22 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(calendarLiveV1ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Calendar live v22 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Calendar live v22 empty code envelope refused");
  if (phase === "capture") validateCalendarLiveV22TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Calendar live v22 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: CALENDAR_LIVE_V22_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? CALENDAR_LIVE_V22_SIGNED_WRITER_TIMEOUT_MS
        : CALENDAR_LIVE_V22_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: CalendarLiveV22RequestBody = {
    artifactVersion: "calendar-live-v22-broker-request-v2",
    brokerVersion: CALENDAR_LIVE_V22_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: CalendarLiveV22Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateCalendarLiveV22Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistCalendarLiveV22RawResponse(
  directory: string,
  phase: CalendarLiveV22RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Calendar live v22 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<CalendarLiveV22Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Calendar live v22 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = calendarLiveV1ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Calendar live v22 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  payload: unknown,
  acceptedAt: string,
): CalendarLiveV22Receipt => {
  const body: Omit<CalendarLiveV22Receipt, "receiptSha256"> = {
    artifactVersion: "calendar-live-v22-broker-receipt-v2",
    brokerVersion: CALENDAR_LIVE_V22_BROKER_VERSION,
...
export function acceptCalendarLiveV22Response<T = unknown>(
  directory: string,
  phase: CalendarLiveV22RemotePhase,
  options: {
    captureIndex?: number;
...
): CalendarLiveV22AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = calendarLiveV1ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Calendar live v22 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<CalendarLiveV22Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Calendar live v22 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "calendar-live-v22-plugin-response-v2" ||
    plugin?.brokerVersion !== CALENDAR_LIVE_V22_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Calendar live v22 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistCalendarLiveV22TechnicalGates(
  directory: string,
  rootProofs: readonly CalendarLiveV22RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertCalendarLiveV22RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Calendar live v22 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Calendar live v22 probe payload hash malformed");
  const gates = Object.fromEntries(
    CALENDAR_LIVE_V22_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "calendar-live-v22-host-technical-gates-v2",
    brokerVersion: CALENDAR_LIVE_V22_BROKER_VERSION,
...
export function validateCalendarLiveV22TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "calendar-live-v22-host-technical-gates-v2" ||
    ...
    CALENDAR_LIVE_V22_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Calendar live v22 technical gates artifact");
}

export function validateCalendarLiveV22Receipt(
  directory: string,
  phase: CalendarLiveV22RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateCalendarLiveV22Request(directory, request);
  const receipt = json<CalendarLiveV22Receipt>(
  ...
  if (
    receipt.artifactVersion !== "calendar-live-v22-broker-receipt-v2" ||
    receipt.brokerVersion !== CALENDAR_LIVE_V22_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Calendar live v22 accepted receipt: ${request.requestId}`,
    );
}

export function readCalendarLiveV22AcceptedRawPayload(
  directory: string,
  phase: CalendarLiveV22RemotePhase,
  captureIndex?: number,
): unknown {
  validateCalendarLiveV22Receipt(directory, phase, captureIndex);
...
export function createCalendarLiveV22FakeDynamicResponse(
  request: CalendarLiveV22Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? CALENDAR_LIVE_V22_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? CALENDAR_LIVE_V22_TARGET,
    ...overrides.extra,
  };
}

export function listCalendarLiveV22PersistedRequests(
  directory: string,
): CalendarLiveV22Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<CalendarLiveV22Request>(file)] : [];
    });
}
