...
import {
  ...
  createInputLiveV78FakeDynamicResponse,
  ...
  listInputLiveV78PersistedRequests,
  persistInputLiveV78RawResponse,
  persistInputLiveV78TechnicalGates,
  validateInputLiveV78Receipt,
  validateInputLiveV78Request,
  type InputLiveV78RemotePhase,
...
import {
  ...
  type InputLiveV78ExtractPayload,
  type InputLiveV78ProbePayload,
...
import {
  ...
  simulatedInputLiveV78Authorization,
} from "./run-input-field-live-v78.js";
...
const captures: InputLiveV78CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV78Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v78-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV78Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV78CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV78FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV78FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V78_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V78_TARGET);
  persistInputLiveV78RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV78Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV78Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV78WriterOwnership,
...
const rootProofs = (): InputLiveV78RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v78-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV78Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV78Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV78RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV78RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV78ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV78ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV78CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV78CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV78CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV78Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV78CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV78PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV78ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV78Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV78Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV78Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV78Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V78_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV78RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V78_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV78Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV78Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV78ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV78Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV78WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV78RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV78Request;
    validateInputLiveV78Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV78Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV78CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v78-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV78Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV78Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV78Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V78_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV78RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V78_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV78Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV78Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV78RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V78_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV78Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV78Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV78Request;
