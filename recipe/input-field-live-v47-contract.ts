...
} from "./input-field-live-v3-verifier-v47.js";
import {
  buildInputLiveV47RawPropertyRuntime,
  normalizeInputLiveV47Scene,
  type InputLiveV47RawNode,
} from "./input-field-live-v47-verifier.js";
...
export const INPUT_LIVE_V47_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V47_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V47_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V47_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V47_VARIANT_COUNT = 256;
export const INPUT_LIVE_V47_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V47_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV47SourceId = (typeof INPUT_LIVE_V47_SOURCE_IDS)[number];

export interface InputLiveV47SourceIdentity {
  source: InputLiveV47SourceId;
...
export interface InputLiveV47WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV47ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV47SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV47RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV47RootProof {
  source: InputLiveV47SourceId;
  adapterIdentity: string;
...
export interface InputLiveV47ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV47SourceId;
    adapterIdentity: string;
...
export interface InputLiveV47CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV47SourceId;
  adapterIdentity: string;
...
export interface InputLiveV47CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV47SourceId;
  strata: InputLiveV47CaptureCell["strata"];
...
export interface InputLiveV47CleanupPayload {
...
export function validateInputLiveV47WriterPayload(
  value: unknown,
): InputLiveV47WriterOwnership {
...
const identityPlan = (source: InputLiveV47SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV47ExtractProgram(
  writer: InputLiveV47WriterOwnership,
  sources: readonly InputLiveV47SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V47_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V47_NAMESPACE)}
${buildInputLiveV47RawPropertyRuntime()}
...
export function validateInputLiveV47ExtractPayload(
  value: unknown,
  writer: InputLiveV47WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V47_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV47ExtractPayload;
}

export function proveInputLiveV47Roots<Instance>(
  extract: InputLiveV47ExtractPayload,
  sources: readonly (InputLiveV47SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV47Scene(
      root.scene,
...
export function assertInputLiveV47RootProofs(
  proofs: readonly InputLiveV47RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV47ProbeProgram(
  writer: InputLiveV47WriterOwnership,
  sources: readonly InputLiveV47SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V47_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV47ProbePayload(
  value: unknown,
  writer: InputLiveV47WriterOwnership,
): InputLiveV47ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV47ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V47_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V47_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV47CaptureManifest(
  cells: readonly InputLiveV47CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V47_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V47_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V47_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V47_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V47_SOURCE_IDS.some(
...
export const inputLiveV47CaptureManifestSha256 = (
  cells: readonly InputLiveV47CaptureCell[],
): string => {
  validateInputLiveV47CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV47CaptureProgram(
  writer: InputLiveV47WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V47_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V47_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V47_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV47CapturePayload(
  value: unknown,
  cell: InputLiveV47CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV47CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V47_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V47_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV47CaptureResponses(
  manifest: readonly InputLiveV47CaptureCell[],
  responses: readonly InputLiveV47CapturePayload[],
): void {
  validateInputLiveV47CaptureManifest(manifest);
  if (
...
export interface InputLiveV47ObjectiveReport {
  artifactVersion: "input-live-v47-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV47Objective(
  manifest: readonly InputLiveV47CaptureCell[],
  responses: readonly InputLiveV47CapturePayload[],
  readReference: (cell: InputLiveV47CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV47CaptureCell) => Buffer,
): InputLiveV47ObjectiveReport {
  assertInputLiveV47CaptureResponses(manifest, responses);
...
export function buildInputLiveV47CleanupProgram(
  writer: InputLiveV47WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V47_NAMESPACE,
...
export function validateInputLiveV47CleanupPayload(
  value: unknown,
  writer: InputLiveV47WriterOwnership,
): InputLiveV47CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV47CleanupPayload;
...
export const INPUT_LIVE_V47_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV47WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
