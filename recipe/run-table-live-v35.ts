...
import {
  acceptTableLiveV35Response,
  createTableLiveV35Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV35Request,
  persistTableLiveV35RawResponse,
  persistTableLiveV35TechnicalGates,
  readTableLiveV35AcceptedRawPayload,
  type TableLiveV35Request,
  type TableLiveV35TransactionAuthorization,
} from "./table-live-v35-broker.js";
import {
  TABLE_LIVE_V35_CAPTURE_COUNT,
  assertTableLiveV35CaptureResponses,
  ...
  evaluateTableLiveV35Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV35Roots,
  validateTableLiveV35CaptureManifest,
  validateTableLiveV35CapturePayload,
  validateTableLiveV35CleanupPayload,
  validateTableLiveV35ExtractPayload,
  validateTableLiveV35ProbePayload,
  validateTableLiveV35WriterPayload,
  type TableLiveV35CaptureCell,
  type TableLiveV35CapturePayload,
...
import {
  ...
  TABLE_LIVE_V35_PLAN_PATH,
  type TableLiveV35ProofPlan,
} from "./build-table-live-proof-v35.js";
import {
...
export const TABLE_LIVE_V35_RUNNER_VERSION = "table-live-v35-runner-v1";

interface TableLiveV35JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV35OrchestratorState {
  artifactVersion: "table-live-v35-orchestrator-state-v1";
  ...
  journal: TableLiveV35JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV35ProofPlan;
  planSha256: string;
  captures: TableLiveV35CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV35SourceIdentity & {
...
export interface TableLiveV35NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV35OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v35 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V35_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V35_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV35ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV35CaptureCell[];
  validateTableLiveV35CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v35 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v35 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV35WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV35RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV35Request =>
  readJson<TableLiveV35Request>(file);

export class TableLiveV35Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV35Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV35OrchestratorState = {
      artifactVersion: "table-live-v35-orchestrator-state-v1",
    ...
    return new TableLiveV35Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV35OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV35Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV35NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV35NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v35 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV35NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v35 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV35RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV35Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV35CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV35AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV35WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV35Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV35ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV35Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV35Response(this.#directory, "writer", {
        validate: validateTableLiveV35WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV35Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV35RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV35Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV35ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV35Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV35RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v35-host-root-proofs-v1",
      ...
      const next = issueTableLiveV35Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV35ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV35Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV35ProbePayload(payload, ownership),
      });
      persistTableLiveV35TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v35 capture cell missing");
      const accepted = acceptTableLiveV35Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV35CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V35_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV35CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV35Objective(
    ...
    throw new Error(`Table live v35 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV35Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV35CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV35Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV35RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV35Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV35CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV35CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV35CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV35Authorization = (
  privateKey: KeyObject,
): TableLiveV35TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV35SimulationKey = (): KeyObject =>
