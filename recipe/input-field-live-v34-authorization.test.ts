...
import { INPUT_LIVE_V34_INDEX_PATH } from "./build-input-field-live-proof-v34.js";
import {
  INPUT_LIVE_V34_DYNAMIC_TOOL,
  INPUT_LIVE_V34_TARGET,
} from "./input-field-live-v34-broker.js";
import {
  buildInputLiveV34AuthorizationArtifact,
  validateInputLiveV34AntecedentIndex,
  validateInputLiveV34AuthorizationArtifact,
  validateInputLiveV34History,
  validateInputLiveV34UnitTestSource,
  type InputLiveV34AntecedentIndex,
  type InputLiveV34AuthorizationArtifact,
  type InputLiveV34AuthorizationProof,
  type InputLiveV34HistoryState,
} from "./input-field-live-v34-authorization.js";
import {
  inputLiveV34AttestationSha256,
  validateInputLiveV34ControlFlowSource,
  validateInputLiveV34SecurityAttestation,
  type InputLiveV34SecurityAttestation,
} from "./input-field-live-v34-preflight.js";
import { inputLiveV34Sha256 } from "./input-field-live-v34-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V34_INDEX_PATH);
const index = (): InputLiveV34AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV34AntecedentIndex;

const artifact = (): InputLiveV34AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV34AuthorizationArtifact({
...
const pendingState = (): InputLiveV34HistoryState => ({
  index: index(),
  indexSha256: inputLiveV34Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV34HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV34AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV34Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V34_TARGET,
  expectedDynamicTool: INPUT_LIVE_V34_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v34/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV34AuthorizationArtifact,
): InputLiveV34SecurityAttestation => {
  const body: Omit<InputLiveV34SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v34-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V34_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV34AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV34History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV34History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV34History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV34History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV34History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV34UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV34UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV34AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV34Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV34AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV34AuthorizationArtifact(
        planted as InputLiveV34AuthorizationArtifact,
        index(),
        inputLiveV34Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV34SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV34AttestationSha256(
      body as Omit<InputLiveV34SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV34SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV34ControlFlowSource(source), []);
});
