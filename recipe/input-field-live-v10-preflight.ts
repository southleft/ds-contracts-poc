...
import {
  ...
  verifyInputLiveV10Authorization,
  type InputLiveV10AntecedentIndex,
  type InputLiveV10AuthorizationProof,
} from "./input-field-live-v10-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V10_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v10/operator-security-attestation-template.json";
export const INPUT_LIVE_V10_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v10-security-attestation.json";

export interface InputLiveV10SecurityAttestation {
...
export interface InputLiveV10PreflightReport {
  artifactVersion: "input-live-v10-preflight-v1";
...
export const inputLiveV10AttestationSha256 = (
  value: Omit<InputLiveV10SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV10SecurityAttestation(
  value: unknown,
  proof: InputLiveV10AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v10-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V10_TARGET) ||
...
export function validateInputLiveV10ControlFlowSource(source: string): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV10Authorization");
  const preflight = source.indexOf("runInputLiveV10Preflight");
  const initialize = source.indexOf("InputLiveV10Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV10Preflight(
  root: string,
  proof: InputLiveV10AuthorizationProof,
  attestationPath = INPUT_LIVE_V10_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV10PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V10_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V10_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V10_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV10AntecedentIndex;
  failures.push(...validateInputLiveV10AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V10_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V10_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v10-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV10ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(`Input live v10 preflight refused:\n${failures.join("\n")}`);
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as InputLiveV10SecurityAttestation;
  return {
    artifactVersion: "input-live-v10-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V10_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV10Preflight(
    process.cwd(),
    verifyInputLiveV10Authorization(),
    attestationPath,
