...
import {
  assertInputLiveV85RootProofs,
  INPUT_LIVE_V85_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V85_RESPONSE_CONTRACTS,
  type InputLiveV85RootProof,
} from "./input-field-live-v85-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V85_BROKER_VERSION = "input-live-v85-broker-v2";
export const INPUT_LIVE_V85_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V85_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V85_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V85_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V85_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V85_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV85RemotePhase =
  (typeof INPUT_LIVE_V85_REMOTE_PHASES)[number];
export type InputLiveV85AuthorizationMode = "live" | "simulated";

export interface InputLiveV85CommitPins {
...
export interface InputLiveV85TransactionAuthorization extends InputLiveV85CommitPins {
  mode: InputLiveV85AuthorizationMode;
...
export interface InputLiveV85Manifest {
  artifactVersion: "input-live-v85-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V85_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V85_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V85_TARGET;
  authorization: InputLiveV85TransactionAuthorization;
...
export interface InputLiveV85ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV85RequestBody {
  artifactVersion: "input-live-v85-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V85_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV85RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV85CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V85_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV85ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV85Request extends InputLiveV85RequestBody {
...
export interface InputLiveV85Receipt {
  artifactVersion: "input-live-v85-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V85_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV85RemotePhase;
...
export interface InputLiveV85AcceptedResponse<T = unknown> {
  request: InputLiveV85Request;
  receipt: InputLiveV85Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV85IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV85ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V85_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V85_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V85_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V85_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V85_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V85_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V85_TARGET.editorType;
};
...
export const inputLiveV85RequestSequence = (
  phase: InputLiveV85RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV85RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV85RequestPath = (
  directory: string,
  phase: InputLiveV85RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV85ResponsePath = (
  directory: string,
  phase: InputLiveV85RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV85ReceiptPath = (
  directory: string,
  phase: InputLiveV85RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV85Manifest => {
  const manifest = json<InputLiveV85Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v85-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V85_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V85_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV85RemotePhase,
  captureIndex?: number,
): InputLiveV85Request | undefined => {
  const file = inputLiveV85RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV85Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV85RemotePhase,
  captureIndex?: number,
): InputLiveV85Receipt | undefined => {
  const file = inputLiveV85ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV85Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV85Request | undefined;
  receipt: InputLiveV85Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV85RemotePhase,
): InputLiveV85ResponseContract => ({
  ...INPUT_LIVE_V85_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V85_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v85-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V85_BROKER_VERSION,
  target: INPUT_LIVE_V85_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V85_PROGRAM_START*/${program}/*INPUT_LIVE_V85_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V85_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V85_PROGRAM_END*/";
...
export function createInputLiveV85Transaction(
  directory: string,
  ...
  const manifest: InputLiveV85Manifest = {
    artifactVersion: "input-live-v85-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V85_BROKER_VERSION,
...
export function validateInputLiveV85Request(
  directory: string,
  request: InputLiveV85Request,
): void {
  ...
  const expectedSequence = inputLiveV85RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v85-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V85_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V85_TARGET.fileKey ||
...
export function issueInputLiveV85Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV85RemotePhase,
  program: string,
  options: InputLiveV85IssueOptions = {},
): InputLiveV85Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV85RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV85Request>(targetPath);
    validateInputLiveV85Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV85ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV85TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV85RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V85_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V85_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V85_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV85RequestBody = {
    artifactVersion: "input-live-v85-broker-request-v2",
    brokerVersion: INPUT_LIVE_V85_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV85Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV85Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV85RawResponse(
  directory: string,
  phase: InputLiveV85RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV85RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV85Request>(requestFile);
  if (
  ...
  const target = inputLiveV85ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV85Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v85-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V85_BROKER_VERSION,
...
export function acceptInputLiveV85Response<T = unknown>(
  directory: string,
  phase: InputLiveV85RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV85AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV85ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV85Request>(
    inputLiveV85RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV85Request(directory, request);
  if (phase === "capture") validateInputLiveV85TechnicalGates(directory);
  const rawFile = inputLiveV85ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v85-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V85_BROKER_VERSION ||
...
export function persistInputLiveV85TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV85RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV85RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V85_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v85-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V85_BROKER_VERSION,
...
export function validateInputLiveV85TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v85-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V85_TECHNICAL_GATES.some(
...
export function validateInputLiveV85Receipt(
  directory: string,
  phase: InputLiveV85RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV85Request(directory, request);
  const receipt = json<InputLiveV85Receipt>(
    inputLiveV85ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV85ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v85-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V85_BROKER_VERSION ||
...
export function readInputLiveV85AcceptedRawPayload(
  directory: string,
  phase: InputLiveV85RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV85Receipt(directory, phase, captureIndex);
...
export function createInputLiveV85FakeDynamicResponse(
  request: InputLiveV85Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V85_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V85_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV85PersistedRequests(
  directory: string,
): InputLiveV85Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV85Request>(file)] : [];
    });
}

export const inputLiveV85Sha256 = sha256;
