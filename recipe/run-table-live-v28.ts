...
import {
  acceptTableLiveV28Response,
  createTableLiveV28Transaction,
  tableLiveV2ReceiptPath,
  tableLiveV2RequestPath,
  tableLiveV2Sha256,
  issueTableLiveV28Request,
  persistTableLiveV28RawResponse,
  persistTableLiveV28TechnicalGates,
  readTableLiveV28AcceptedRawPayload,
  type TableLiveV28Request,
  type TableLiveV28TransactionAuthorization,
} from "./table-live-v28-broker.js";
import {
  TABLE_LIVE_V28_CAPTURE_COUNT,
  assertTableLiveV28CaptureResponses,
  ...
  evaluateTableLiveV28Objective,
  tableLiveV2CaptureManifestSha256,
  proveTableLiveV28Roots,
  validateTableLiveV28CaptureManifest,
  validateTableLiveV28CapturePayload,
  validateTableLiveV28CleanupPayload,
  validateTableLiveV28ExtractPayload,
  validateTableLiveV28ProbePayload,
  validateTableLiveV28WriterPayload,
  type TableLiveV28CaptureCell,
  type TableLiveV28CapturePayload,
...
import {
  ...
  TABLE_LIVE_V28_PLAN_PATH,
  type TableLiveV28ProofPlan,
...
export const TABLE_LIVE_V28_RUNNER_VERSION = "table-live-v28-runner-v1";

interface TableLiveV28JournalEntry {
  index: number;
  event: string;
...
interface TableLiveV28OrchestratorState {
  artifactVersion: "table-live-v28-orchestrator-state-v1";
  ...
  journal: TableLiveV28JournalEntry[];
}

interface LoadedProofInputs {
  plan: TableLiveV28ProofPlan;
  planSha256: string;
  captures: TableLiveV28CaptureCell[];
  captureManifestSha256: string;
  sources: Array<
    TableLiveV28SourceIdentity & {
...
export interface TableLiveV28NextAction {
...
const writeState = (
  directory: string,
  state: TableLiveV28OrchestratorState,
): void => {
...
const loadExpectedPlan = (
  ...
  if (
    ...
    throw new TypeError(`Table live v28 expected plan drift: ${label}`);
  return JSON.parse(uncompressed.toString("utf8")) as ExpectedScenePlan;
};

const loadProofInputs = (root: string): LoadedProofInputs => {
  const planPath = path.join(root, TABLE_LIVE_V28_PLAN_PATH);
  const capturePath = path.join(root, TABLE_LIVE_V28_CAPTURE_MANIFEST_PATH);
  const planBytes = readFileSync(planPath);
  const captureBytes = readFileSync(capturePath);
  const plan = JSON.parse(planBytes.toString("utf8")) as TableLiveV28ProofPlan;
  const captureArtifact = JSON.parse(captureBytes.toString("utf8")) as Record<
    string,
    any
  >;
  const captures = captureArtifact.cells as TableLiveV28CaptureCell[];
  validateTableLiveV28CaptureManifest(captures);
  if (
    ...
    throw new TypeError("Table live v28 capture manifest hash drift");
  const descriptors = [
    {
  ...
  const sources = plan.sources.map((source) => {
    ...
    if (!descriptor)
      throw new TypeError(`Table live v28 unknown source ${source.source}`);
...
const writerOwnership = (directory: string): TableLiveV28WriterOwnership =>
  readJson(path.join(directory, OWNERSHIP_FILE));

const rootProofs = (directory: string): TableLiveV28RootProof[] =>
  readJson<Record<string, any>>(path.join(directory, ROOT_PROOFS_FILE)).proofs;

const requestFromPath = (file: string): TableLiveV28Request =>
  readJson<TableLiveV28Request>(file);

export class TableLiveV28Orchestrator {
  ...
  static initialize(options: {
    ...
    const writer = issueTableLiveV28Request(
      options.transactionDirectory,
      options.privateKey,
      "writer",
    ...
    const state: TableLiveV28OrchestratorState = {
      artifactVersion: "table-live-v28-orchestrator-state-v1",
    ...
    return new TableLiveV28Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
      inputs,
  ...
  static resume(options: {
    ...
    const state = readJson<TableLiveV28OrchestratorState>(
      path.join(options.transactionDirectory, STATE_FILE),
    );
    const orchestrator = new TableLiveV28Orchestrator(
      options.root,
      options.transactionDirectory,
      options.privateKey,
  ...
  nextAction(): TableLiveV28NextAction {
    const cleanup =
  ...
  ingestAndAdvance(rawResponse: string): TableLiveV28NextAction {
    if (!this.#state.activeRequest)
      throw new Error("Table live v28 has no active main request");
  ...
  ingestCleanup(rawResponse: string): TableLiveV28NextAction {
    if (!this.#state.cleanupRequest)
      throw new Error("Table live v28 cleanup is not available");
    const request = requestFromPath(this.#state.cleanupRequest);
    persistTableLiveV28RawResponse(this.#directory, "cleanup", rawResponse);
    const ownership = writerOwnership(this.#directory);
    const accepted = acceptTableLiveV28Response(this.#directory, "cleanup", {
      validate: (payload) =>
        validateTableLiveV28CleanupPayload(payload, ownership),
    });
    appendJournal(
      this.#directory,
      this.#state,
  ...
  recover(): void {
    if (
      ...
      const raw = readTableLiveV28AcceptedRawPayload(this.#directory, "writer");
      writeNew(
        path.join(this.#directory, OWNERSHIP_FILE),
        validateTableLiveV28WriterPayload(raw),
      );
    }
    ...
    if (
      ...
      const ownership = writerOwnership(this.#directory);
      const extract = issueTableLiveV28Request(
        this.#directory,
        this.#privateKey,
        "extract",
        buildTableLiveV28ExtractProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "extract",
  ...
  #acceptActive(request: TableLiveV28Request): void {
    if (request.phase === "writer") {
      const accepted = acceptTableLiveV28Response(this.#directory, "writer", {
        validate: validateTableLiveV28WriterPayload,
      });
    ...
    if (request.phase === "restore") {
      const accepted = acceptTableLiveV28Response(this.#directory, "restore", {
        validate: (payload) =>
          validateTableLiveV28RestorePayload(payload, ownership),
      });
      appendJournal(
        this.#directory,
        this.#state,
    ...
    if (request.phase === "extract") {
      const accepted = acceptTableLiveV28Response(this.#directory, "extract", {
        validate: (payload) =>
          validateTableLiveV28ExtractPayload(payload, ownership),
      });
      const proofs = proveTableLiveV28Roots(
        accepted.payload,
        this.#inputs.sources,
        collapseTableRecipe,
        compileTableRecipe,
      );
      assertTableLiveV28RootProofs(proofs);
      writeNew(path.join(this.#directory, ROOT_PROOFS_FILE), {
        artifactVersion: "table-live-v28-host-root-proofs-v1",
      ...
      const next = issueTableLiveV28Request(
        this.#directory,
        this.#privateKey,
        "probe",
        buildTableLiveV28ProbeProgram(ownership, this.#inputs.sources),
      );
      this.#state.activeRequest = tableLiveV2RequestPath(
        this.#directory,
        "probe",
    ...
    if (request.phase === "probe") {
      const accepted = acceptTableLiveV28Response(this.#directory, "probe", {
        validate: (payload) =>
          validateTableLiveV28ProbePayload(payload, ownership),
      });
      persistTableLiveV28TechnicalGates(
        this.#directory,
    ...
    if (request.phase === "capture") {
      ...
      if (!cell) throw new TypeError("Table live v28 capture cell missing");
      const accepted = acceptTableLiveV28Response(this.#directory, "capture", {
        captureIndex: cell.index,
        validate: (payload, rawBytes) =>
          validateTableLiveV28CapturePayload(payload, cell, rawBytes),
      });
      ...
      if (next < TABLE_LIVE_V28_CAPTURE_COUNT) this.#issueCapture(next);
      else {
        ...
        assertTableLiveV28CaptureResponses(this.#inputs.captures, responses);
        const objective = evaluateTableLiveV28Objective(
    ...
    throw new Error(`Table live v28 cleanup is not a main-lane request`);
  }

  #issueCleanupAndRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const cleanup = issueTableLiveV28Request(
      this.#directory,
      this.#privateKey,
      "cleanup",
      buildTableLiveV28CleanupProgram(ownership),
    );
    this.#state.cleanupRequest = tableLiveV2RequestPath(
      this.#directory,
      "cleanup",
  ...
  #issueRestore(journal: boolean): void {
    const ownership = writerOwnership(this.#directory);
    const restore = issueTableLiveV28Request(
      this.#directory,
      this.#privateKey,
      "restore",
      buildTableLiveV28RestoreProgram(ownership),
    );
    this.#state.activeRequest = tableLiveV2RequestPath(
      this.#directory,
      "restore",
  ...
  #issueCapture(index: number): void {
    const request = issueTableLiveV28Request(
      this.#directory,
      this.#privateKey,
      "capture",
      buildTableLiveV28CaptureProgram(
  ...
  #readCaptureResponseIndex(): TableLiveV28CapturePayload[] {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    return existsSync(file) ? readJson(file) : [];
  }

  #writeCaptureResponseIndex(value: TableLiveV28CapturePayload[]): void {
    const file = path.join(this.#directory, CAPTURE_RESPONSES_FILE);
    writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`);
  }
...
export const simulatedTableLiveV28Authorization = (
  privateKey: KeyObject,
): TableLiveV28TransactionAuthorization => ({
  mode: "simulated",
...
export const generateTableLiveV28SimulationKey = (): KeyObject =>
