...
export const INPUT_LIVE_V16_VERIFIER_VERSION = "input-live-v16-verifier-v1";

export function buildInputLiveV16RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV16RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV16RawNode[];
}

export interface InputLiveV16NormalizedScene {
...
export interface InputLiveV16SceneProof {
...
export function normalizeInputLiveV16Scene(
  root: InputLiveV16RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV16NormalizedScene {
  const canonicalBindings: InputLiveV16NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV16RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV16Scene<Instance>(
  normalized: InputLiveV16NormalizedScene,
...
export function compileInputLiveV16ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV16PreCaptureGates(payload: unknown): void {
