...
import {
  ...
  INPUT_LIVE_V45_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V45_RESPONSE_CONTRACTS,
  type InputLiveV45RootProof,
} from "./input-field-live-v45-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V45_BROKER_VERSION = "input-live-v45-broker-v2";
export const INPUT_LIVE_V45_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V45_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V45_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V45_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V45_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V45_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV45RemotePhase =
  (typeof INPUT_LIVE_V45_REMOTE_PHASES)[number];
export type InputLiveV45AuthorizationMode = "live" | "simulated";

export interface InputLiveV45CommitPins {
...
export interface InputLiveV45TransactionAuthorization extends InputLiveV45CommitPins {
  mode: InputLiveV45AuthorizationMode;
...
export interface InputLiveV45Manifest {
  artifactVersion: "input-live-v45-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V45_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V45_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V45_TARGET;
...
export interface InputLiveV45ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV45RequestBody {
  artifactVersion: "input-live-v45-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V45_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV45RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV45CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V45_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV45ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV45Request extends InputLiveV45RequestBody {
...
export interface InputLiveV45Receipt {
  artifactVersion: "input-live-v45-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V45_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV45RemotePhase;
...
export interface InputLiveV45AcceptedResponse<T = unknown> {
  request: InputLiveV45Request;
  receipt: InputLiveV45Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV45IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV45ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V45_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V45_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V45_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V45_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V45_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V45_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V45_TARGET.editorType;
};
...
export const inputLiveV45RequestSequence = (
  phase: InputLiveV45RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV45RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV45RequestPath = (
  directory: string,
  phase: InputLiveV45RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV45ResponsePath = (
  directory: string,
  phase: InputLiveV45RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV45ReceiptPath = (
  directory: string,
  phase: InputLiveV45RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV45Manifest => {
  const manifest = json<InputLiveV45Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v45-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V45_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V45_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV45RemotePhase,
  captureIndex?: number,
): InputLiveV45Request | undefined => {
  const file = inputLiveV45RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV45Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV45RemotePhase,
  captureIndex?: number,
): InputLiveV45Receipt | undefined => {
  const file = inputLiveV45ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV45Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV45Request | undefined;
  receipt: InputLiveV45Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV45RemotePhase,
): InputLiveV45ResponseContract => ({
  ...INPUT_LIVE_V45_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V45_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V45_DYNAMIC_TOOL,
      target: INPUT_LIVE_V45_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v45-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V45_BROKER_VERSION,
  target: INPUT_LIVE_V45_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V45_PROGRAM_START*/${program}/*INPUT_LIVE_V45_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V45_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V45_PROGRAM_END*/";
...
export function createInputLiveV45Transaction(
  directory: string,
  ...
  const manifest: InputLiveV45Manifest = {
    artifactVersion: "input-live-v45-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V45_BROKER_VERSION,
...
export function validateInputLiveV45Request(
  directory: string,
  request: InputLiveV45Request,
): void {
  ...
  const expectedSequence = inputLiveV45RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v45-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V45_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V45_TARGET.fileKey ||
...
export function issueInputLiveV45Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV45RemotePhase,
  program: string,
  options: InputLiveV45IssueOptions = {},
): InputLiveV45Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV45RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV45Request>(targetPath);
    validateInputLiveV45Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV45ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV45TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV45RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V45_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V45_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V45_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV45RequestBody = {
    artifactVersion: "input-live-v45-broker-request-v2",
    brokerVersion: INPUT_LIVE_V45_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV45Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV45Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV45RawResponse(
  directory: string,
  phase: InputLiveV45RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV45RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV45Request>(requestFile);
  if (
  ...
  const target = inputLiveV45ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV45Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v45-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V45_BROKER_VERSION,
...
export function acceptInputLiveV45Response<T = unknown>(
  directory: string,
  phase: InputLiveV45RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV45AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV45ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV45Request>(
    inputLiveV45RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV45Request(directory, request);
  if (phase === "capture") validateInputLiveV45TechnicalGates(directory);
  const rawFile = inputLiveV45ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v45-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V45_BROKER_VERSION ||
...
export function persistInputLiveV45TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV45RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV45RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V45_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v45-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V45_BROKER_VERSION,
...
export function validateInputLiveV45TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v45-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V45_TECHNICAL_GATES.some(
...
export function validateInputLiveV45Receipt(
  directory: string,
  phase: InputLiveV45RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV45Request(directory, request);
  const receipt = json<InputLiveV45Receipt>(
    inputLiveV45ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV45ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v45-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V45_BROKER_VERSION ||
...
export function readInputLiveV45AcceptedRawPayload(
  directory: string,
  phase: InputLiveV45RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV45Receipt(directory, phase, captureIndex);
...
export function createInputLiveV45FakeDynamicResponse(
  request: InputLiveV45Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V45_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V45_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV45PersistedRequests(
  directory: string,
): InputLiveV45Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV45Request>(file)] : [];
    });
}

export const inputLiveV45Sha256 = sha256;
