...
export interface InputLiveV30LifecycleSimulationReport {
  artifactVersion: "input-live-v30-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV30AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV30Lifecycle(): Promise<InputLiveV30LifecycleSimulationReport> {
  await buildInputLiveV30Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V30_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV30AntecedentIndex;
  const indexSha256 = inputLiveV30Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v30-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V30_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V30_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV30History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV30History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV30AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V30_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V30_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV30History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV30History(authorized, "pending");
    ...
    await buildInputLiveV30Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V30_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV30AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV30Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV30Lifecycle(), null, 2)}\n`,
  );
