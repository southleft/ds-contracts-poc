...
import {
  ...
  INPUT_LIVE_V15_INDEX_PATH,
} from "./build-input-field-live-proof-v15.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV15HistoryExpectation = "pending" | "authorized";

export interface InputLiveV15AuthorizationArtifact {
  artifactVersion: "input-live-v15-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V15_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV15AntecedentIndex {
...
export interface InputLiveV15HistoryState {
  index?: InputLiveV15AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV15AuthorizationArtifact;
...
export interface InputLiveV15AuthorizationProof extends InputLiveV15TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V15_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V15_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V15_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V15_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v15-status") ||
...
export function validateInputLiveV15AntecedentIndex(
  index: InputLiveV15AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v15-antecedent-index-v1" ||
...
export function validateInputLiveV15UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV15History\(\s*["']/.test(source) ||
    /readInputLiveV15HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV15AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV15AuthorizationArtifact {
  ...
  ) as InputLiveV15AntecedentIndex;
  const indexFailures = validateInputLiveV15AntecedentIndex(index);
...
export function validateInputLiveV15AuthorizationArtifact(
  artifact: InputLiveV15AuthorizationArtifact | undefined,
  index: InputLiveV15AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v15-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v15" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V15_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V15_TARGET) ||
    !equalJson(
...
export function validateInputLiveV15History(
  state: InputLiveV15HistoryState,
  expected: InputLiveV15HistoryExpectation,
): string[] {
  const failures = validateInputLiveV15AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV15AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV15HistoryState(
  root = process.cwd(),
): InputLiveV15HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V15_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV15AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V15_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V15_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V15_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV15History(
  expected: InputLiveV15HistoryExpectation,
  root = process.cwd(),
): InputLiveV15HistoryState {
  const state = readInputLiveV15HistoryState(root);
  const failures = validateInputLiveV15History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV15Authorization(
  root = process.cwd(),
): InputLiveV15AuthorizationProof {
  const state = verifyInputLiveV15History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V15_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v15/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v15.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V15_TARGET,
    expectedDynamicTool: INPUT_LIVE_V15_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V15_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV15HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV15History(expected);
