...
} from "./input-field-live-v3-verifier-v74.js";
import {
  buildInputLiveV74RawPropertyRuntime,
  normalizeInputLiveV74Scene,
  type InputLiveV74RawNode,
} from "./input-field-live-v74-verifier.js";
...
export const INPUT_LIVE_V74_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V74_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V74_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V74_VARIANT_COUNT = 256;
export const INPUT_LIVE_V74_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V74_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV74SourceId = (typeof INPUT_LIVE_V74_SOURCE_IDS)[number];

export interface InputLiveV74SourceIdentity {
  source: InputLiveV74SourceId;
...
export interface InputLiveV74WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV74ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV74SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV74RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV74RootProof {
  source: InputLiveV74SourceId;
  adapterIdentity: string;
...
export interface InputLiveV74ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV74SourceId;
    adapterIdentity: string;
...
export interface InputLiveV74CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV74SourceId;
  adapterIdentity: string;
...
export interface InputLiveV74CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV74SourceId;
  strata: InputLiveV74CaptureCell["strata"];
...
export interface InputLiveV74CleanupPayload {
...
export function validateInputLiveV74WriterPayload(
  value: unknown,
): InputLiveV74WriterOwnership {
...
const identityPlan = (source: InputLiveV74SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV74ExtractProgram(
  writer: InputLiveV74WriterOwnership,
  sources: readonly InputLiveV74SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V74_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V74_NAMESPACE)}
${buildInputLiveV74RawPropertyRuntime()}
...
export function validateInputLiveV74ExtractPayload(
  value: unknown,
  writer: InputLiveV74WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V74_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV74ExtractPayload;
}

export function proveInputLiveV74Roots<Instance>(
  extract: InputLiveV74ExtractPayload,
  sources: readonly (InputLiveV74SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV74Scene(
      root.scene,
...
export function assertInputLiveV74RootProofs(
  proofs: readonly InputLiveV74RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV74ProbeProgram(
  writer: InputLiveV74WriterOwnership,
  sources: readonly InputLiveV74SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V74_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV74ProbePayload(
  value: unknown,
  writer: InputLiveV74WriterOwnership,
): InputLiveV74ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV74ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V74_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V74_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV74CaptureManifest(
  cells: readonly InputLiveV74CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V74_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V74_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V74_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V74_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V74_SOURCE_IDS.some(
...
export const inputLiveV74CaptureManifestSha256 = (
  cells: readonly InputLiveV74CaptureCell[],
): string => {
  validateInputLiveV74CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV74CaptureProgram(
  writer: InputLiveV74WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V74_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V74_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V74_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV74CapturePayload(
  value: unknown,
  cell: InputLiveV74CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV74CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V74_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V74_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV74CaptureResponses(
  manifest: readonly InputLiveV74CaptureCell[],
  responses: readonly InputLiveV74CapturePayload[],
): void {
  validateInputLiveV74CaptureManifest(manifest);
  if (
...
export interface InputLiveV74ObjectiveReport {
  artifactVersion: "input-live-v74-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV74Objective(
  manifest: readonly InputLiveV74CaptureCell[],
  responses: readonly InputLiveV74CapturePayload[],
  readReference: (cell: InputLiveV74CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV74CaptureCell) => Buffer,
): InputLiveV74ObjectiveReport {
  assertInputLiveV74CaptureResponses(manifest, responses);
...
export function buildInputLiveV74CleanupProgram(
  writer: InputLiveV74WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V74_NAMESPACE,
...
export function validateInputLiveV74CleanupPayload(
  value: unknown,
  writer: InputLiveV74WriterOwnership,
): InputLiveV74CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV74CleanupPayload;
...
export const INPUT_LIVE_V74_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV74WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
