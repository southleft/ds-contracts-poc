...
import {
  assertInputLiveV49RootProofs,
  INPUT_LIVE_V49_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V49_RESPONSE_CONTRACTS,
  type InputLiveV49RootProof,
} from "./input-field-live-v49-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V49_BROKER_VERSION = "input-live-v49-broker-v2";
export const INPUT_LIVE_V49_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V49_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V49_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V49_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V49_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V49_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV49RemotePhase =
  (typeof INPUT_LIVE_V49_REMOTE_PHASES)[number];
export type InputLiveV49AuthorizationMode = "live" | "simulated";

export interface InputLiveV49CommitPins {
...
export interface InputLiveV49TransactionAuthorization extends InputLiveV49CommitPins {
  mode: InputLiveV49AuthorizationMode;
...
export interface InputLiveV49Manifest {
  artifactVersion: "input-live-v49-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V49_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V49_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V49_TARGET;
  authorization: InputLiveV49TransactionAuthorization;
...
export interface InputLiveV49ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV49RequestBody {
  artifactVersion: "input-live-v49-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V49_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV49RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV49CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V49_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV49ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV49Request extends InputLiveV49RequestBody {
...
export interface InputLiveV49Receipt {
  artifactVersion: "input-live-v49-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V49_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV49RemotePhase;
...
export interface InputLiveV49AcceptedResponse<T = unknown> {
  request: InputLiveV49Request;
  receipt: InputLiveV49Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV49IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV49ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V49_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V49_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V49_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V49_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V49_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V49_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V49_TARGET.editorType;
};
...
export const inputLiveV49RequestSequence = (
  phase: InputLiveV49RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV49RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV49RequestPath = (
  directory: string,
  phase: InputLiveV49RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV49ResponsePath = (
  directory: string,
  phase: InputLiveV49RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV49ReceiptPath = (
  directory: string,
  phase: InputLiveV49RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV49Manifest => {
  const manifest = json<InputLiveV49Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v49-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V49_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V49_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV49RemotePhase,
  captureIndex?: number,
): InputLiveV49Request | undefined => {
  const file = inputLiveV49RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV49Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV49RemotePhase,
  captureIndex?: number,
): InputLiveV49Receipt | undefined => {
  const file = inputLiveV49ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV49Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV49Request | undefined;
  receipt: InputLiveV49Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV49RemotePhase,
): InputLiveV49ResponseContract => ({
  ...INPUT_LIVE_V49_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V49_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V49_DYNAMIC_TOOL,
      target: INPUT_LIVE_V49_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v49-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V49_BROKER_VERSION,
  target: INPUT_LIVE_V49_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V49_PROGRAM_START*/${program}/*INPUT_LIVE_V49_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V49_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V49_PROGRAM_END*/";
...
export function createInputLiveV49Transaction(
  directory: string,
  ...
  const manifest: InputLiveV49Manifest = {
    artifactVersion: "input-live-v49-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V49_BROKER_VERSION,
...
export function validateInputLiveV49Request(
  directory: string,
  request: InputLiveV49Request,
): void {
  ...
  const expectedSequence = inputLiveV49RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v49-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V49_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V49_TARGET.fileKey ||
...
export function issueInputLiveV49Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV49RemotePhase,
  program: string,
  options: InputLiveV49IssueOptions = {},
): InputLiveV49Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV49RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV49Request>(targetPath);
    validateInputLiveV49Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV49ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV49TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV49RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V49_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V49_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V49_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV49RequestBody = {
    artifactVersion: "input-live-v49-broker-request-v2",
    brokerVersion: INPUT_LIVE_V49_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV49Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV49Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV49RawResponse(
  directory: string,
  phase: InputLiveV49RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV49RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV49Request>(requestFile);
  if (
  ...
  const target = inputLiveV49ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV49Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v49-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V49_BROKER_VERSION,
...
export function acceptInputLiveV49Response<T = unknown>(
  directory: string,
  phase: InputLiveV49RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV49AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV49ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV49Request>(
    inputLiveV49RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV49Request(directory, request);
  if (phase === "capture") validateInputLiveV49TechnicalGates(directory);
  const rawFile = inputLiveV49ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v49-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V49_BROKER_VERSION ||
...
export function persistInputLiveV49TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV49RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV49RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V49_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v49-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V49_BROKER_VERSION,
...
export function validateInputLiveV49TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v49-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V49_TECHNICAL_GATES.some(
...
export function validateInputLiveV49Receipt(
  directory: string,
  phase: InputLiveV49RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV49Request(directory, request);
  const receipt = json<InputLiveV49Receipt>(
    inputLiveV49ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV49ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v49-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V49_BROKER_VERSION ||
...
export function readInputLiveV49AcceptedRawPayload(
  directory: string,
  phase: InputLiveV49RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV49Receipt(directory, phase, captureIndex);
...
export function createInputLiveV49FakeDynamicResponse(
  request: InputLiveV49Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V49_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V49_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV49PersistedRequests(
  directory: string,
): InputLiveV49Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV49Request>(file)] : [];
    });
}

export const inputLiveV49Sha256 = sha256;
