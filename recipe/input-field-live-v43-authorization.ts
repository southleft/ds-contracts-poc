...
import {
  ...
  INPUT_LIVE_V43_INDEX_PATH,
} from "./build-input-field-live-proof-v43.js";
import { canonicalJson } from "./normalize.js";

export type InputLiveV43HistoryExpectation = "pending" | "authorized";

export interface InputLiveV43AuthorizationArtifact {
  artifactVersion: "input-live-v43-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof INPUT_LIVE_V43_INDEX_PATH;
    indexSha256: string;
...
export interface InputLiveV43AntecedentIndex {
...
export interface InputLiveV43HistoryState {
  index?: InputLiveV43AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: InputLiveV43AuthorizationArtifact;
...
export interface InputLiveV43AuthorizationProof extends InputLiveV43TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof INPUT_LIVE_V43_TARGET;
  expectedDynamicTool: typeof INPUT_LIVE_V43_DYNAMIC_TOOL;
  authorizationPath: typeof INPUT_LIVE_V43_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === INPUT_LIVE_V43_AUTHORIZATION_PATH ||
  ...
  artifactPath.includes("input-field-live-pivot-v43-status") ||
...
export function validateInputLiveV43AntecedentIndex(
  index: InputLiveV43AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "input-live-v43-antecedent-index-v1" ||
...
export function validateInputLiveV43UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyInputLiveV43History\(\s*["']/.test(source) ||
    /readInputLiveV43HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildInputLiveV43AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): InputLiveV43AuthorizationArtifact {
  ...
  ) as InputLiveV43AntecedentIndex;
  const indexFailures = validateInputLiveV43AntecedentIndex(index);
...
export function validateInputLiveV43AuthorizationArtifact(
  artifact: InputLiveV43AuthorizationArtifact | undefined,
  index: InputLiveV43AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "input-live-v43-capture-authorization-v1" ||
    artifact.authorizationId !== "input-live-v43" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== INPUT_LIVE_V43_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, INPUT_LIVE_V43_TARGET) ||
    !equalJson(
...
export function validateInputLiveV43History(
  state: InputLiveV43HistoryState,
  expected: InputLiveV43HistoryExpectation,
): string[] {
  const failures = validateInputLiveV43AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateInputLiveV43AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readInputLiveV43HistoryState(
  root = process.cwd(),
): InputLiveV43HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, INPUT_LIVE_V43_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as InputLiveV43AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${INPUT_LIVE_V43_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${INPUT_LIVE_V43_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${INPUT_LIVE_V43_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyInputLiveV43History(
  expected: InputLiveV43HistoryExpectation,
  root = process.cwd(),
): InputLiveV43HistoryState {
  const state = readInputLiveV43HistoryState(root);
  const failures = validateInputLiveV43History(state, expected);
  if (failures.length)
    throw new Error(
...
export function verifyInputLiveV43Authorization(
  root = process.cwd(),
): InputLiveV43AuthorizationProof {
  const state = verifyInputLiveV43History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, INPUT_LIVE_V43_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/input-field-live-pivot-v43/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-input-field-live-v43.ts"]!.sha256,
    ...
    target: INPUT_LIVE_V43_TARGET,
    expectedDynamicTool: INPUT_LIVE_V43_DYNAMIC_TOOL,
    authorizationPath: INPUT_LIVE_V43_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): InputLiveV43HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyInputLiveV43History(expected);
