...
import {
  verifyTableLiveV17FixedSceneFixedPoint,
  type TableLiveV17FixedFixedPoint,
} from "./table-live-v17-fixed-point.js";

export const TABLE_LIVE_V17_VERIFIER_VERSION = "table-live-v17-verifier-v1";

export function buildTableLiveV17RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV17RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV17RawNode[];
}

export interface TableLiveV17NormalizedScene {
...
export interface TableLiveV17SceneProof {
...
export function normalizeTableLiveV17Scene(
  root: TableLiveV17RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV17NormalizedScene {
  const canonicalBindings: TableLiveV17NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV17RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV17Scene<Instance>(
  normalized: TableLiveV17NormalizedScene,
...
): TableLiveV17SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV17FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV17ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV17PreCaptureGates(payload: unknown): void {
