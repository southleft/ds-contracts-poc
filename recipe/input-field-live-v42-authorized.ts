...
import { INPUT_LIVE_V42_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v42.js";
import { verifyInputLiveV42Authorization } from "./input-field-live-v42-authorization.js";
import { runInputLiveV42Preflight } from "./input-field-live-v42-preflight.js";
import { InputLiveV42Orchestrator } from "./run-input-field-live-v42.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V42_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v42-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV42Authorization(root);
    runInputLiveV42Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV42Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
