...
import { buildComboboxLiveV7CleanupRuntime } from "./combobox-live-v7-cleanup.js";
import {
  type ComboboxLiveV7FixedFixedPoint,
} from "./combobox-live-v7-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildComboboxLiveV7RestoreProgram,
  validateComboboxLiveV7RestorePayload,
  type ComboboxLiveV7RestorePayload,
} from "./combobox-live-v7-restore.js";
import {
  buildComboboxLiveV7RawPropertyRuntime,
  normalizeComboboxLiveV7Scene,
  type ComboboxLiveV7RawNode,
} from "./combobox-live-v7-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-combobox-v7.js";
import {
...
} from "./scene-readback-combobox-v7.js";
...
export const COMBOBOX_LIVE_V7_NAMESPACE = COMBOBOX_FIGMA_NAMESPACE;
export const COMBOBOX_LIVE_V7_SOURCE_IDS = ["mui", "antd"] as const;
export const COMBOBOX_LIVE_V7_ADAPTERS = [
  "material-combobox-reviewed-v1",
  "commerce-combobox-reviewed-v1",
] as const;
export const COMBOBOX_LIVE_V7_CAPTURE_COUNT = 72;
export const COMBOBOX_LIVE_V7_VARIANT_COUNT = 144;
export const COMBOBOX_LIVE_V7_SET_COUNT = 4;
export const COMBOBOX_LIVE_V7_REMOTE_REQUESTS = 77;
export const COMBOBOX_LIVE_V7_HOST_PHASES = 3;
export const COMBOBOX_LIVE_V7_SOURCE_ROOTS = 2;
export const COMBOBOX_LIVE_V7_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const COMBOBOX_LIVE_V7_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type ComboboxLiveV7SourceId = (typeof COMBOBOX_LIVE_V7_SOURCE_IDS)[number];

export interface ComboboxLiveV7SourceIdentity {
  source: ComboboxLiveV7SourceId;
...
export interface ComboboxLiveV7WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface ComboboxLiveV7ExtractPayload {
  pageId: string;
  roots: Array<{
    source: ComboboxLiveV7SourceId;
    adapterIdentity: string;
    comboboxSetId: string;
    optionSetId: string;
    comboboxScene: ComboboxLiveV7RawNode;
    optionScene: ComboboxLiveV7RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface ComboboxLiveV7RootProof {
  source: ComboboxLiveV7SourceId;
  adapterIdentity: string;
  comboboxAccounting: SceneComparison;
  optionAccounting: SceneComparison;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV7FixedFixedPoint;
}

export interface ComboboxLiveV7ProbePayload {
  pageId: string;
  sources: Array<{
    source: ComboboxLiveV7SourceId;
    adapterIdentity: string;
...
export interface ComboboxLiveV7CaptureCell {
  index: number;
  cellKey: string;
  source: ComboboxLiveV7SourceId;
  adapterIdentity: string;
...
export interface ComboboxLiveV7CapturePayload {
  index: number;
  cellKey: string;
  source: ComboboxLiveV7SourceId;
  frameWidth: number;
...
export interface ComboboxLiveV7CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Combobox live v7 ${label} must be an object`);
...
export function validateComboboxLiveV7WriterPayload(
  value: unknown,
): ComboboxLiveV7WriterOwnership {
  ...
  if (
    ...
    result.namespace !== COMBOBOX_LIVE_V7_NAMESPACE ||
    ...
    throw new TypeError("Combobox live v7 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Combobox live v7 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 4)
    throw new TypeError("Combobox live v7 writer set identity collision");
  return {
...
const identityPlan = (
  source: ComboboxLiveV7SourceIdentity,
  kind: "combobox" | "option",
) => {
  const plan =
    kind === "combobox"
...
export function buildComboboxLiveV7ExtractProgram(
  writer: ComboboxLiveV7WriterOwnership,
  sources: readonly ComboboxLiveV7SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Combobox live v7 extract requires two source identities");
...
const NS=${JSON.stringify(COMBOBOX_LIVE_V7_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(COMBOBOX_LIVE_V7_NAMESPACE)}
${buildComboboxLiveV7RawPropertyRuntime()}
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE")throw new Error("COMBOBOX-V7-EXTRACT-PAGE");
...
if(get(page,"pageOwner")!=="recipe/combobox/"+${JSON.stringify(writer.runIdentity)})throw new Error("COMBOBOX-V7-EXTRACT-OWNER");
const sets=page.findAllWithCriteria({types:["COMPONENT_SET"]}).filter(node=>SET_IDS.has(node.id));
if(sets.length!==4)throw new Error("COMBOBOX-V7-EXTRACT-ROOTS:"+sets.length);
...
for(const set of sets){
  ...
  if(!IDENTITIES[adapterIdentity]||(kind!=="combobox"&&kind!=="option"))throw new Error("COMBOBOX-V7-EXTRACT-ADAPTER:"+adapterIdentity+":"+kind);
...
if(roots.length!==2||roots.some(root=>!root.comboboxScene||!root.optionScene))throw new Error("COMBOBOX-V7-EXTRACT-PAIR");
...
export function validateComboboxLiveV7ExtractPayload(
  value: unknown,
  writer: ComboboxLiveV7WriterOwnership,
): ComboboxLiveV7ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Combobox live v7 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !COMBOBOX_LIVE_V7_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Combobox live v7 extract schema/two-root mismatch");
  return payload as ComboboxLiveV7ExtractPayload;
}

export function proveComboboxLiveV7Roots<Instance>(
  extract: ComboboxLiveV7ExtractPayload,
  sources: readonly (ComboboxLiveV7SourceIdentity & {
...
): ComboboxLiveV7RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Combobox live v7 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Combobox live v7 omitted ${source.source} root`);
    const comboboxNormalized = normalizeComboboxLiveV7Scene(
      root.comboboxScene,
      extract.variableTable,
    );
    const optionNormalized = normalizeComboboxLiveV7Scene(
      root.optionScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): ComboboxLiveV7FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Combobox live v7 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Combobox live v7 compile lost library frame");
        ...
        if (!compiledCombobox || !compiledOption)
          throw new TypeError("Combobox live v7 compile lost owned sets");
...
export function assertComboboxLiveV7RootProofs(
  proofs: readonly ComboboxLiveV7RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Combobox live v7 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Combobox live v7 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildComboboxLiveV7ProbeProgram(
  writer: ComboboxLiveV7WriterOwnership,
  sources: readonly ComboboxLiveV7SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(COMBOBOX_LIVE_V7_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("COMBOBOX-MUST-NOT-WRITE-INPUT-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
if(!page||page.type!=="PAGE"||page.id==="115:295378")throw new Error("COMBOBOX-V7-PROBE-PAGE");
...
if(sets.length!==4)throw new Error("COMBOBOX-V7-PROBE-ROOTS:"+sets.length);
...
for(const row of byAdapter.values()){
 ...
 if(!source||!combobox||!option||combobox.children.length!==64||option.children.length!==8)throw new Error("COMBOBOX-V7-PROBE-VARIANTS:"+adapterIdentity);
 ...
 for(const component of combobox.children){const target=axes(component),updates={};for(const name of axisNames){const key=propertyKey(instance,name);if(!key)throw new Error("COMBOBOX-V7-PROBE-AXIS:"+name);updates[key]=target[name];}instance.setProperties(updates);const main=await instance.getMainComponentAsync();if(main)visited.add(main.id);}
 ...
 for(const component of option.children){const target=axes(component),updates={};for(const name of ["Size","Option state"]){const key=propertyKey(optionInstance,name);if(!key)throw new Error("COMBOBOX-V7-PROBE-OPTION-AXIS:"+name);updates[key]=target[name];}optionInstance.setProperties(updates);const main=await optionInstance.getMainComponentAsync();if(main)optionVisited.add(main.id);}
...
export function validateComboboxLiveV7ProbePayload(
  value: unknown,
  writer: ComboboxLiveV7WriterOwnership,
): ComboboxLiveV7ProbePayload {
  const payload = record(value, "probe payload") as ComboboxLiveV7ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== COMBOBOX_LIVE_V7_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V7_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Combobox live v7 probe/usability/restoration failed");
  return payload;
}

export function validateComboboxLiveV7CaptureManifest(
  cells: readonly ComboboxLiveV7CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== COMBOBOX_LIVE_V7_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      COMBOBOX_LIVE_V7_CAPTURE_COUNT ||
    new Set(order).size !== COMBOBOX_LIVE_V7_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !COMBOBOX_LIVE_V7_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Combobox live v7 capture manifest is malformed");
}

export const comboboxLiveV5CaptureManifestSha256 = (
  cells: readonly ComboboxLiveV7CaptureCell[],
): string => {
  validateComboboxLiveV7CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildComboboxLiveV7CaptureProgram(
  writer: ComboboxLiveV7WriterOwnership,
  cell: ComboboxLiveV7CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= COMBOBOX_LIVE_V7_CAPTURE_COUNT
  )
    throw new TypeError("Combobox live v7 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Combobox live v7 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
if(!page||page.type!=="PAGE"||page.id==="115:295378"||!set||set.type!=="COMPONENT_SET")throw new Error("COMBOBOX-V7-CAPTURE-TARGET");
const axes=node=>Object.fromEntries(node.name.split(",").map(part=>part.trim()).filter(part=>part.includes("=")).map(part=>{const at=part.indexOf("=");return[part.slice(0,at),part.slice(at+1)];}));
...
if(!component)throw new Error("COMBOBOX-V7-CAPTURE-CELL:"+cell.cellKey);
...
try{
 ...
 if(bytes.byteLength>${COMBOBOX_LIVE_V7_CAPTURE_MAX_PNG_BYTES})throw new Error("COMBOBOX-V7-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateComboboxLiveV7CapturePayload(
  value: unknown,
  cell: ComboboxLiveV7CaptureCell,
  rawResponseBytes: number,
): ComboboxLiveV7CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as ComboboxLiveV7CapturePayload;
  if (
    rawResponseBytes > COMBOBOX_LIVE_V7_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > COMBOBOX_LIVE_V7_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Combobox live v7 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertComboboxLiveV7CaptureResponses(
  manifest: readonly ComboboxLiveV7CaptureCell[],
  responses: readonly ComboboxLiveV7CapturePayload[],
): void {
  validateComboboxLiveV7CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Combobox live v7 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface ComboboxLiveV7ObjectiveReport {
  artifactVersion: "combobox-live-v7-objective-report-v1";
  denominator: 72;
...
export function evaluateComboboxLiveV7Objective(
  manifest: readonly ComboboxLiveV7CaptureCell[],
  responses: readonly ComboboxLiveV7CapturePayload[],
): ComboboxLiveV7ObjectiveReport {
  assertComboboxLiveV7CaptureResponses(manifest, responses);
  return {
    artifactVersion: "combobox-live-v7-objective-report-v1",
    denominator: 72,
...
export function buildComboboxLiveV7CleanupProgram(
  writer: ComboboxLiveV7WriterOwnership,
): string {
  const baseProgram = buildComboboxLiveV7CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: COMBOBOX_LIVE_V7_NAMESPACE,
  ...
  const guardedResult = `if(JSON.stringify(requestedNodeIds)!==JSON.stringify([expectedPageId])||JSON.stringify([...requestedCollectionIds].sort())!==JSON.stringify([...expectedCollectionIds].sort()))throw new Error("COMBOBOX-V7-CLEANUP-EXACT-OWNERSHIP");${resultStatement}`;
  const program = baseProgram.replace(resultStatement, guardedResult);
  if (program === baseProgram)
    throw new Error("Combobox live v7 cleanup runtime result hook absent");
  return String.raw`
...
export function validateComboboxLiveV7CleanupPayload(
  payload: unknown,
  writer: ComboboxLiveV7WriterOwnership,
): ComboboxLiveV7CleanupPayload {
  const value = record(payload, "cleanup payload") as ComboboxLiveV7CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Combobox live v7 cleanup is incomplete or overbroad");
  return value;
}

export const COMBOBOX_LIVE_V7_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "ComboboxLiveV7WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 4,
