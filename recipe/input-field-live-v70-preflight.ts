...
import {
  ...
  verifyInputLiveV70Authorization,
  type InputLiveV70AntecedentIndex,
  type InputLiveV70AuthorizationProof,
} from "./input-field-live-v70-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V70_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v70/operator-security-attestation-template.json";
export const INPUT_LIVE_V70_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v70-security-attestation.json";

export interface InputLiveV70SecurityAttestation {
...
export interface InputLiveV70PreflightReport {
  artifactVersion: "input-live-v70-preflight-v1";
...
export const inputLiveV70AttestationSha256 = (
  value: Omit<InputLiveV70SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV70SecurityAttestation(
  value: unknown,
  proof: InputLiveV70AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v70-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V70_TARGET) ||
...
export function validateInputLiveV70ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV70Authorization");
  const preflight = source.indexOf("runInputLiveV70Preflight");
  const initialize = source.indexOf("InputLiveV70Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV70Preflight(
  root: string,
  proof: InputLiveV70AuthorizationProof,
  attestationPath = INPUT_LIVE_V70_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV70PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V70_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V70_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V70_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV70AntecedentIndex;
  failures.push(...validateInputLiveV70AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V70_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V70_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v70-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV70ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV70SecurityAttestation;
  return {
    artifactVersion: "input-live-v70-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V70_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV70Preflight(
    process.cwd(),
    verifyInputLiveV70Authorization(),
    attestationPath,
