...
} from "./input-field-live-v3-verifier-v60.js";
import {
  buildInputLiveV60RawPropertyRuntime,
  normalizeInputLiveV60Scene,
  type InputLiveV60RawNode,
} from "./input-field-live-v60-verifier.js";
...
export const INPUT_LIVE_V60_NAMESPACE = "ds.contracts.input.recipe.v5";
export const INPUT_LIVE_V60_SOURCE_IDS = ["mui", "polaris"] as const;
export const INPUT_LIVE_V60_ADAPTERS = [
  "material-text-field-reviewed-v1",
  "commerce-text-field-reviewed-v1",
] as const;
export const INPUT_LIVE_V60_CAPTURE_COUNT = 128;
export const INPUT_LIVE_V60_VARIANT_COUNT = 256;
export const INPUT_LIVE_V60_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const INPUT_LIVE_V60_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type InputLiveV60SourceId = (typeof INPUT_LIVE_V60_SOURCE_IDS)[number];

export interface InputLiveV60SourceIdentity {
  source: InputLiveV60SourceId;
...
export interface InputLiveV60WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
...
export interface InputLiveV60ExtractPayload {
  pageId: string;
  roots: Array<{
    source: InputLiveV60SourceId;
    adapterIdentity: string;
    setId: string;
    scene: InputLiveV60RawNode;
    lineage: Array<{
      nodeId: string;
...
export interface InputLiveV60RootProof {
  source: InputLiveV60SourceId;
  adapterIdentity: string;
...
export interface InputLiveV60ProbePayload {
  pageId: string;
  sources: Array<{
    source: InputLiveV60SourceId;
    adapterIdentity: string;
...
export interface InputLiveV60CaptureCell {
  index: number;
  cellKey: string;
  source: InputLiveV60SourceId;
  adapterIdentity: string;
...
export interface InputLiveV60CapturePayload {
  index: number;
  cellKey: string;
  source: InputLiveV60SourceId;
  strata: InputLiveV60CaptureCell["strata"];
...
export interface InputLiveV60CleanupPayload {
...
export function validateInputLiveV60WriterPayload(
  value: unknown,
): InputLiveV60WriterOwnership {
...
const identityPlan = (source: InputLiveV60SourceIdentity) => ({
  directOwnershipKeys: [
    ...new Set(
...
export function buildInputLiveV60ExtractProgram(
  writer: InputLiveV60WriterOwnership,
  sources: readonly InputLiveV60SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
...
const V7_NS=${JSON.stringify(INPUT_LIVE_V60_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(INPUT_LIVE_V60_NAMESPACE)}
${buildInputLiveV60RawPropertyRuntime()}
...
export function validateInputLiveV60ExtractPayload(
  value: unknown,
  writer: InputLiveV60WriterOwnership,
  ...
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !INPUT_LIVE_V60_SOURCE_IDS.includes(root.source) ||
  ...
  return payload as InputLiveV60ExtractPayload;
}

export function proveInputLiveV60Roots<Instance>(
  extract: InputLiveV60ExtractPayload,
  sources: readonly (InputLiveV60SourceIdentity & {
  ...
  return sources.map((source) => {
    ...
    const normalized = normalizeInputLiveV60Scene(
      root.scene,
...
export function assertInputLiveV60RootProofs(
  proofs: readonly InputLiveV60RootProof[],
): void {
  if (
    proofs.length !== 2 ||
...
export function buildInputLiveV60ProbeProgram(
  writer: InputLiveV60WriterOwnership,
  sources: readonly InputLiveV60SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const V7_NS=${JSON.stringify(INPUT_LIVE_V60_NAMESPACE)},V7_PAGE_ID=${JSON.stringify(writer.pageId)},V7_SET_IDS=new Set(${JSON.stringify(writer.setIds)}),V7_SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
const page=await figma.getNodeByIdAsync(V7_PAGE_ID);
...
export function validateInputLiveV60ProbePayload(
  value: unknown,
  writer: InputLiveV60WriterOwnership,
): InputLiveV60ProbePayload {
  const payload = record(value, "probe payload") as InputLiveV60ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== INPUT_LIVE_V60_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V60_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
...
export function validateInputLiveV60CaptureManifest(
  cells: readonly InputLiveV60CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== INPUT_LIVE_V60_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      INPUT_LIVE_V60_CAPTURE_COUNT ||
    new Set(order).size !== INPUT_LIVE_V60_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !INPUT_LIVE_V60_SOURCE_IDS.includes(cell.source) ||
    ...
    INPUT_LIVE_V60_SOURCE_IDS.some(
...
export const inputLiveV60CaptureManifestSha256 = (
  cells: readonly InputLiveV60CaptureCell[],
): string => {
  validateInputLiveV60CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildInputLiveV60CaptureProgram(
  writer: InputLiveV60WriterOwnership,
  ...
  if (
    ...
    cell.index < 0 ||
    cell.index >= INPUT_LIVE_V60_CAPTURE_COUNT ||
    !cell.cellKey ||
    !INPUT_LIVE_V60_SOURCE_IDS.includes(cell.source) ||
    cell.strata.source !== cell.source ||
    cell.strata.state !== cell.axes.state ||
...
try{
 ...
 if(bytes.byteLength>${INPUT_LIVE_V60_CAPTURE_MAX_PNG_BYTES})throw new Error("INPUT-V9-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateInputLiveV60CapturePayload(
  value: unknown,
  cell: InputLiveV60CaptureCell,
  ...
  const payload = record(
    value,
    "capture payload",
  ) as InputLiveV60CapturePayload;
  if (
    rawResponseBytes > INPUT_LIVE_V60_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > INPUT_LIVE_V60_CAPTURE_MAX_PNG_BYTES ||
...
export function assertInputLiveV60CaptureResponses(
  manifest: readonly InputLiveV60CaptureCell[],
  responses: readonly InputLiveV60CapturePayload[],
): void {
  validateInputLiveV60CaptureManifest(manifest);
  if (
...
export interface InputLiveV60ObjectiveReport {
  artifactVersion: "input-live-v60-objective-report-v1";
  denominator: 128;
...
export function evaluateInputLiveV60Objective(
  manifest: readonly InputLiveV60CaptureCell[],
  responses: readonly InputLiveV60CapturePayload[],
  readReference: (cell: InputLiveV60CaptureCell) => Buffer,
  readCapture: (cell: InputLiveV60CaptureCell) => Buffer,
): InputLiveV60ObjectiveReport {
  assertInputLiveV60CaptureResponses(manifest, responses);
...
export function buildInputLiveV60CleanupProgram(
  writer: InputLiveV60WriterOwnership,
): string {
  const baseProgram = buildInputLiveV3CleanupRuntime({
    ...
    editorType: "figma",
    namespace: INPUT_LIVE_V60_NAMESPACE,
...
export function validateInputLiveV60CleanupPayload(
  value: unknown,
  writer: InputLiveV60WriterOwnership,
): InputLiveV60CleanupPayload {
  const payload = record(
    value,
    "cleanup payload",
  ) as InputLiveV60CleanupPayload;
...
export const INPUT_LIVE_V60_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "InputLiveV60WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 2,
