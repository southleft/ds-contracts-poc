...
export type CalendarLiveV7HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV7AuthorizationArtifact {
  artifactVersion: "calendar-live-v7-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V7_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV7AntecedentIndex {
...
export interface CalendarLiveV7HistoryState {
  index?: CalendarLiveV7AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV7AuthorizationArtifact;
...
export interface CalendarLiveV7AuthorizationProof
  extends CalendarLiveV7TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V7_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V7_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V7_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V7_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v7-authorization") ||
  artifactPath.includes("calendar-live-v7-preflight") ||
  artifactPath.includes("calendar-live-v7-authorized") ||
  artifactPath.includes("create-calendar-live-v7-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v7-status") ||
...
export function validateCalendarLiveV7AntecedentIndex(
  index: CalendarLiveV7AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v7-antecedent-index-v1" ||
...
export function validateCalendarLiveV7UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV7History\(\s*["']/.test(source) ||
    /readCalendarLiveV7HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV7AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV7AuthorizationArtifact {
  ...
  ) as CalendarLiveV7AntecedentIndex;
  const indexFailures = validateCalendarLiveV7AntecedentIndex(index);
...
export function validateCalendarLiveV7AuthorizationArtifact(
  artifact: CalendarLiveV7AuthorizationArtifact | undefined,
  index: CalendarLiveV7AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v7-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v7" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V7_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V7_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V7_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V7_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V7_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V7_SOURCE_ROOTS ||
  ...
  if (
    ...
    artifact?.execution?.calendarLiveV6AuthorizationReusable !== false ||
...
export function validateCalendarLiveV7History(
  state: CalendarLiveV7HistoryState,
  expected: CalendarLiveV7HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV7AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV7AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV7HistoryState(
  root = process.cwd(),
): CalendarLiveV7HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V7_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV7AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V7_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V7_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V7_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV7History(
  expected: CalendarLiveV7HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV7HistoryState {
  const state = readCalendarLiveV7HistoryState(root);
  const failures = validateCalendarLiveV7History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v7 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV7Authorization(
  root = process.cwd(),
): CalendarLiveV7AuthorizationProof {
  const state = verifyCalendarLiveV7History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V7_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v7/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v7.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V7_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V7_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V7_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV7HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV7History(expected);
  process.stdout.write(
    `Calendar live v7 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
