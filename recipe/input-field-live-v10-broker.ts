...
import {
  ...
  INPUT_LIVE_V10_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V10_RESPONSE_CONTRACTS,
  type InputLiveV10RootProof,
} from "./input-field-live-v10-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V10_BROKER_VERSION = "input-live-v10-broker-v2";
export const INPUT_LIVE_V10_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V10_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V10_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V10_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V10_REMOTE_PHASES = [
  "writer",
  "extract",
  "probe",
  "capture",
...
export const INPUT_LIVE_V10_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV10RemotePhase =
  (typeof INPUT_LIVE_V10_REMOTE_PHASES)[number];
export type InputLiveV10AuthorizationMode = "live" | "simulated";

export interface InputLiveV10CommitPins {
...
export interface InputLiveV10TransactionAuthorization extends InputLiveV10CommitPins {
  mode: InputLiveV10AuthorizationMode;
...
export interface InputLiveV10Manifest {
  artifactVersion: "input-live-v10-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V10_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V10_DYNAMIC_TOOL;
...
export interface InputLiveV10ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV10RequestBody {
  artifactVersion: "input-live-v10-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V10_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV10RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV10CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V10_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV10ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV10Request extends InputLiveV10RequestBody {
...
export interface InputLiveV10Receipt {
  artifactVersion: "input-live-v10-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V10_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV10RemotePhase;
...
export interface InputLiveV10AcceptedResponse<T = unknown> {
  request: InputLiveV10Request;
  receipt: InputLiveV10Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV10IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV10ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V10_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V10_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V10_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V10_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V10_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V10_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V10_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Input live v10 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    throw new TypeError("Input live v10 capture index must be 0..127");
  return `capture-${String(captureIndex).padStart(3, "0")}`;
};

export const inputLiveV10RequestSequence = (
  phase: InputLiveV10RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV10RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV10RequestPath = (
  directory: string,
  phase: InputLiveV10RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV10ResponsePath = (
  directory: string,
  phase: InputLiveV10RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV10ReceiptPath = (
  directory: string,
  phase: InputLiveV10RemotePhase,
  captureIndex?: number,
): string =>
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Input live v10 authorization pins are malformed");
};

const readManifest = (directory: string): InputLiveV10Manifest => {
  const manifest = json<InputLiveV10Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Input live v10 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "input-live-v10-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V10_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V10_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Input live v10 broker manifest");
  return manifest;
};

const unsignedBody = (request: InputLiveV10Request): InputLiveV10RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV10RemotePhase,
  captureIndex?: number,
): InputLiveV10Request | undefined => {
  const file = inputLiveV10RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV10Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV10RemotePhase,
  captureIndex?: number,
): InputLiveV10Receipt | undefined => {
  const file = inputLiveV10ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV10Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV10Request | undefined;
  receipt: InputLiveV10Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV10RemotePhase,
): InputLiveV10ResponseContract => ({
  ...INPUT_LIVE_V10_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V10_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v10-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V10_BROKER_VERSION,
  target: INPUT_LIVE_V10_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V10_PROGRAM_START*/${program}/*INPUT_LIVE_V10_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V10_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V10_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Input live v10 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createInputLiveV10Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Input live v10 private signing key does not match authorized public-key identity",
    );
  const manifest: InputLiveV10Manifest = {
    artifactVersion: "input-live-v10-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V10_BROKER_VERSION,
...
export function validateInputLiveV10Request(
  directory: string,
  request: InputLiveV10Request,
): void {
  ...
  const expectedSequence = inputLiveV10RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v10-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V10_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V10_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Input live v10 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueInputLiveV10Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV10RemotePhase,
  program: string,
  options: InputLiveV10IssueOptions = {},
): InputLiveV10Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV10RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV10Request>(targetPath);
    validateInputLiveV10Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v10 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV10ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Input live v10 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Input live v10 empty code envelope refused");
  if (phase === "capture") validateInputLiveV10TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Input live v10 out-of-order phase: ${id}`);
  const expectedResponse =
    options.expectedResponse ?? responseContractFor(phase);
  const programSha256 = sha256(program);
  const sequence = inputLiveV10RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V10_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V10_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V10_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV10RequestBody = {
    artifactVersion: "input-live-v10-broker-request-v2",
    brokerVersion: INPUT_LIVE_V10_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV10Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV10Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV10RawResponse(
  directory: string,
  phase: InputLiveV10RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV10RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v10 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV10Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Input live v10 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = inputLiveV10ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Input live v10 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV10Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v10-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V10_BROKER_VERSION,
...
export function acceptInputLiveV10Response<T = unknown>(
  directory: string,
  phase: InputLiveV10RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV10AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV10ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v10 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV10Request>(
    inputLiveV10RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV10Request(directory, request);
  if (phase === "capture") validateInputLiveV10TechnicalGates(directory);
  const rawFile = inputLiveV10ResponsePath(directory, phase, captureIndex);
  if (!existsSync(rawFile))
    throw new Error(`Input live v10 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v10-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V10_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Input live v10 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistInputLiveV10TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV10RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV10RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Input live v10 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Input live v10 probe payload hash malformed");
  const gates = Object.fromEntries(
    INPUT_LIVE_V10_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v10-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V10_BROKER_VERSION,
...
export function validateInputLiveV10TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v10-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V10_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Input live v10 technical gates artifact");
}

export function validateInputLiveV10Receipt(
  directory: string,
  phase: InputLiveV10RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV10Request(directory, request);
  const receipt = json<InputLiveV10Receipt>(
    inputLiveV10ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV10ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v10-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V10_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Input live v10 accepted receipt: ${request.requestId}`,
    );
}

export function readInputLiveV10AcceptedRawPayload(
  directory: string,
  phase: InputLiveV10RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV10Receipt(directory, phase, captureIndex);
...
export function createInputLiveV10FakeDynamicResponse(
  request: InputLiveV10Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V10_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V10_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV10PersistedRequests(
  directory: string,
): InputLiveV10Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV10Request>(file)] : [];
    });
}

export const inputLiveV10Sha256 = sha256;
