...
import {
  validateComboboxLiveV28AntecedentIndex,
  verifyComboboxLiveV28Authorization,
  type ComboboxLiveV28AntecedentIndex,
  type ComboboxLiveV28AuthorizationProof,
} from "./combobox-live-v28-authorization.js";
import { canonicalJson } from "./normalize.js";

export const COMBOBOX_LIVE_V28_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/combobox-live-pivot-v28/operator-security-attestation-template.json";
export const COMBOBOX_LIVE_V28_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/combobox-live-v28-security-attestation.json";

export interface ComboboxLiveV28SecurityAttestation {
  artifactVersion: "combobox-live-v28-operator-security-attestation-v1";
  status: "complete";
  createdAt: string;
  rotation: {
    completedAt: string;
...
export interface ComboboxLiveV28PreflightReport {
  artifactVersion: "combobox-live-v28-preflight-v1";
...
export const comboboxLiveV5AttestationSha256 = (
  value: Omit<ComboboxLiveV28SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateComboboxLiveV28SecurityAttestation(
  value: unknown,
  proof: ComboboxLiveV28AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "combobox-live-v28-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, COMBOBOX_LIVE_V28_TARGET) ||
...
export function validateComboboxLiveV28ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyComboboxLiveV28Authorization");
  const preflight = source.indexOf("runComboboxLiveV28Preflight");
  const initialize = source.indexOf("ComboboxLiveV28Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runComboboxLiveV28Preflight(
  root: string,
  proof: ComboboxLiveV28AuthorizationProof,
  attestationPath = COMBOBOX_LIVE_V28_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): ComboboxLiveV28PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, COMBOBOX_LIVE_V28_TARGET) ||
    !equalJson(proof.expectedDynamicTool, COMBOBOX_LIVE_V28_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, COMBOBOX_LIVE_V28_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as ComboboxLiveV28AntecedentIndex;
  failures.push(...validateComboboxLiveV28AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, COMBOBOX_LIVE_V28_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${COMBOBOX_LIVE_V28_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
        ...validateComboboxLiveV28SecurityAttestation(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/combobox-live-v28-authorized.ts"),
    "utf8",
  );
  failures.push(...validateComboboxLiveV28ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
      `Combobox live v28 preflight refused:\n${failures.join("\n")}`,
    );
  const security = JSON.parse(
    readFileSync(securityPath, "utf8"),
  ) as ComboboxLiveV28SecurityAttestation;
  return {
    artifactVersion: "combobox-live-v28-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${COMBOBOX_LIVE_V28_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runComboboxLiveV28Preflight(
    process.cwd(),
    verifyComboboxLiveV28Authorization(),
    attestationPath,
