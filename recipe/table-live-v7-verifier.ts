...
import {
  verifyTableLiveV7FixedSceneFixedPoint,
  type TableLiveV7FixedFixedPoint,
} from "./table-live-v7-fixed-point.js";

export const TABLE_LIVE_V7_VERIFIER_VERSION = "table-live-v7-verifier-v1";

export function buildTableLiveV7RawPropertyRuntime(): string {
  return String.raw`
...
export interface TableLiveV7RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: TableLiveV7RawNode[];
}

export interface TableLiveV7NormalizedScene {
...
export interface TableLiveV7SceneProof {
...
export function normalizeTableLiveV7Scene(
  root: TableLiveV7RawNode,
  variableTable: readonly LocalVariableRecord[],
): TableLiveV7NormalizedScene {
  const canonicalBindings: TableLiveV7NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: TableLiveV7RawNode): SceneNodeSnapshot => {
...
export function proveTableLiveV7Scene<Instance>(
  normalized: TableLiveV7NormalizedScene,
...
): TableLiveV7SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyTableLiveV7FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileTableLiveV7ExpectedScenePlan(
  ir: IRNode,
...
export function assertTableLiveV7PreCaptureGates(payload: unknown): void {
  if (payload === null || typeof payload !== "object")
    throw new TypeError("table live v7 pre-capture gate payload absent");
