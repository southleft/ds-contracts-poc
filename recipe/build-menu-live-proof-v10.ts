...
import { muiMenuAdapterConfig as muiMenuProposedConfig, muiMenuSource as muiMenuProposedSource } from "./fixtures/generated/menu.mui.js";
import { chakraMenuAdapterConfig as chakraMenuProposedConfig, chakraMenuSource as chakraMenuProposedSource } from "./fixtures/generated/menu.chakra.js";
import { compileMenuRecipe, menuRecipe } from "./recipes/menu.js";

const EVIDENCE = "recipe/evidence/menu-live-pivot-v10";
...
const sources = [
  ...
  {
    // PROPOSED from MUI's own two-item capture (14 read, 0 invented).
    adapterIdentity: "mui-menu-proposed-v1",
    displayName: "MUI (proposed)",
    source: muiMenuProposedSource,
    config: muiMenuProposedConfig,
  },
  {
    // HELD OUT: Chakra's Menu, captured today through a Portal composition (14 read, 0 invented).
    adapterIdentity: "chakra-menu-proposed-v1",
    displayName: "Chakra (proposed)",
    source: chakraMenuProposedSource,
    config: chakraMenuProposedConfig,
  },
].map((entry) => {
...
const receiptOwned = {
      artifactVersion: "menu-live-pivot-v10-prepare",
      teaching:
...
if (process.argv.includes("--check")) {
  ...
  if (writer.sourcePlans.length !== 5)
    throw new Error("menu prepare requires 5 library sources (reviewed + proposed)");
  console.log(
    JSON.stringify({
      check: "ok",
      runIdentity: writer.runIdentity,
