...
import {
  ...
  createInputLiveV82FakeDynamicResponse,
  ...
  listInputLiveV82PersistedRequests,
  persistInputLiveV82RawResponse,
  persistInputLiveV82TechnicalGates,
  validateInputLiveV82Receipt,
  validateInputLiveV82Request,
  type InputLiveV82RemotePhase,
...
import {
  ...
  type InputLiveV82ExtractPayload,
  type InputLiveV82ProbePayload,
...
import {
  ...
  simulatedInputLiveV82Authorization,
} from "./run-input-field-live-v82.js";
...
const captures: InputLiveV82CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV82Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v82-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV82Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV82CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV82FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV82FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V82_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V82_TARGET);
  persistInputLiveV82RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV82Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV82Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV82WriterOwnership,
...
const rootProofs = (): InputLiveV82RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v82-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV82Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV82Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV82RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV82RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV82ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV82ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV82CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV82CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV82CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV82Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV82CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV82PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV82ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV82Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV82Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV82Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV82Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V82_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV82RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V82_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV82Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV82Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV82ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV82Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV82WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV82RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV82Request;
    validateInputLiveV82Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV82Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV82CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v82-state-"),
  );
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV82Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV82Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV82Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V82_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV82RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V82_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV82Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV82Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV82RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V82_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV82Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV82Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV82Request;
