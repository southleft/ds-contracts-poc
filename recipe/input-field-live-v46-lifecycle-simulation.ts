...
export interface InputLiveV46LifecycleSimulationReport {
  artifactVersion: "input-live-v46-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildInputLiveV46AuthorizationArtifact> | undefined,
...
export async function simulateInputLiveV46Lifecycle(): Promise<InputLiveV46LifecycleSimulationReport> {
  await buildInputLiveV46Proof(true);
  const indexBefore = readFileSync(INPUT_LIVE_V46_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as InputLiveV46AntecedentIndex;
  const indexSha256 = inputLiveV46Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "input-live-v46-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, INPUT_LIVE_V46_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", INPUT_LIVE_V46_INDEX_PATH]);
    ...
    const pendingFailures = validateInputLiveV46History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `v8 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateInputLiveV46History(pending, "authorized");
    if (
    ...
    const authorization = buildInputLiveV46AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", INPUT_LIVE_V46_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${INPUT_LIVE_V46_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateInputLiveV46History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateInputLiveV46History(authorized, "pending");
    ...
    await buildInputLiveV46Proof(true);
    const indexAfter = readFileSync(INPUT_LIVE_V46_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as InputLiveV46AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
    ...
    return {
      ...
      authorizationSha256: inputLiveV46Sha256(authorizationBytes),
    };
  } finally {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateInputLiveV46Lifecycle(), null, 2)}\n`,
  );
