...
import {
  ...
  createInputLiveV11FakeDynamicResponse,
  ...
  listInputLiveV11PersistedRequests,
  persistInputLiveV11RawResponse,
  persistInputLiveV11TechnicalGates,
  validateInputLiveV11Receipt,
  validateInputLiveV11Request,
  type InputLiveV11RemotePhase,
...
import {
  ...
  type InputLiveV11ExtractPayload,
  type InputLiveV11ProbePayload,
  type InputLiveV11RootProof,
  type InputLiveV11WriterOwnership,
} from "./input-field-live-v11-contract.js";
import {
  InputLiveV11Orchestrator,
  simulatedInputLiveV11Authorization,
} from "./run-input-field-live-v11.js";
...
const captures: InputLiveV11CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV11Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v11-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV11Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV11CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV11FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV11FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V11_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V11_TARGET);
  persistInputLiveV11RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV11Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV11Receipt(directory, phase, options.captureIndex);
...
const extractPayload = (
  ownership: InputLiveV11WriterOwnership,
...
const rootProofs = (): InputLiveV11RootProof[] =>
...
const capturePayload = (
  ...
  const bytes = Buffer.from(`input-live-v11-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV11Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV11Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV11RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) => validateInputLiveV11ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      probePayload(writer),
      {
        validate: (payload) => validateInputLiveV11ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV11CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV11CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV11CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV11Response(directory, "cleanup", {
      validate: (payload) => validateInputLiveV11CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV11PersistedRequests(directory).length, 132);
    assert.ok(inputLiveV11ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV11Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV11Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV11Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV11Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V11_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV11RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V11_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV11Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV11Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV11ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV11Receipt(third.directory, "writer"),
      /invalid Input live v11 accepted receipt/,
    );
  } finally {
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV11WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV11RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV11Request;
    validateInputLiveV11Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV11Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV11CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active extract and surfaced cleanup", () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v11-state-"));
  const privateKey = generateKeyPairSync("ed25519").privateKey;
  try {
    const orchestrator = InputLiveV11Orchestrator.initialize({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
      authorization: simulatedInputLiveV11Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV11Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V11_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV11RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V11_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV11Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV11Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV11RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V11_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV11Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV11Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV11Request;
