...
export const INPUT_LIVE_V24_VERIFIER_VERSION = "input-live-v24-verifier-v1";

export function buildInputLiveV24RawPropertyRuntime(): string {
  return String.raw`
...
export interface InputLiveV24RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: InputLiveV24RawNode[];
}

export interface InputLiveV24NormalizedScene {
...
export interface InputLiveV24SceneProof {
...
export function normalizeInputLiveV24Scene(
  root: InputLiveV24RawNode,
  variableTable: readonly LocalVariableRecord[],
): InputLiveV24NormalizedScene {
  const canonicalBindings: InputLiveV24NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: InputLiveV24RawNode): SceneNodeSnapshot => {
...
export function proveInputLiveV24Scene<Instance>(
  normalized: InputLiveV24NormalizedScene,
...
export function compileInputLiveV24ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertInputLiveV24PreCaptureGates(payload: unknown): void {
