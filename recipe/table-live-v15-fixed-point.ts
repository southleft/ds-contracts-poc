/**
 * Table live v15 host verifier. Table-shaped: table/row/cell ownership,
...
export const TABLE_LIVE_V15_FIXED_VERIFIER_VERSION = "table-live-v15-fixed-point-v1";
export const TABLE_LIVE_V15_FIXED_REQUIRED_GATE_IDS = [
...
export interface TableLiveV15FixedThresholds {
  clipping: { maximumVisibleAreaLoss: number };
  overlap: { maximumPixels: number };
}

export interface TableLiveV15FixedCellValidation {
  cellKey: string;
  source: string;
  kind: "table" | "row" | "cell";
  rolesExact: boolean;
...
export interface TableLiveV15FixedSourceProbe {
...
export interface TableLiveV15FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: TableLiveV15FixedFixedPoint;
}

export interface TableLiveV15FixedHardGateInput {
  thresholds: TableLiveV15FixedThresholds;
  sourceProbes: TableLiveV15FixedSourceProbe[];
  cells: TableLiveV15FixedCellValidation[];
  sceneProofs: TableLiveV15FixedSceneProof[];
...
export interface TableLiveV15FixedHardGateReport {
  verifierVersion: typeof TABLE_LIVE_V15_FIXED_VERIFIER_VERSION;
...
export interface TableLiveV15FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
export function verifyTableLiveV15FixedHardGates(
  input: TableLiveV15FixedHardGateInput,
): TableLiveV15FixedHardGateReport {
  ...
  const exactFields: Array<keyof TableLiveV15FixedCellValidation> = [
    "rolesExact",
    "stateSemanticsExact",
    "noFakeLayout",
  ];
  ...
  if (input.visualRows.length !== 0)
    failures.push("table live v15 must not invent a visual comparison");
  ...
  return {
    verifierVersion: TABLE_LIVE_V15_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyTableLiveV15FixedSceneFixedPoint<Instance>(
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<TableLiveV15FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
