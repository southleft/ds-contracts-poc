/**
 * Carried Combobox live v38 host verifier. Uses scene-readback-combobox-v38 so live
...
export const COMBOBOX_LIVE_V38_FIXED_VERIFIER_VERSION = "combobox-live-v38-fixed-point-v1";
export const COMBOBOX_LIVE_V38_FIXED_REQUIRED_GATE_IDS = [
...
export interface ComboboxLiveV38FixedThresholds {
...
export interface ComboboxLiveV38FixedMaterialRegression {
  aggregateRelativeIncrease: number;
  catastrophicCellRule: string;
}

export interface ComboboxLiveV38FixedCellValidation {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV38FixedSourceProbe {
...
export interface ComboboxLiveV38FixedSceneProof {
  adapterIdentity: string;
  accounting: SceneComparison;
  fixedPoint: ComboboxLiveV38FixedFixedPoint;
}

export interface ComboboxLiveV38FixedVisualRow {
  cellKey: string;
  source: string;
  state: string;
  adornment: string;
...
export interface ComboboxLiveV38FixedHardGateInput {
  thresholds: ComboboxLiveV38FixedThresholds;
  materialRegression: ComboboxLiveV38FixedMaterialRegression;
  sourceProbes: ComboboxLiveV38FixedSourceProbe[];
  cells: ComboboxLiveV38FixedCellValidation[];
  sceneProofs: ComboboxLiveV38FixedSceneProof[];
  visualRows: ComboboxLiveV38FixedVisualRow[];
  safety: {
...
export interface ComboboxLiveV38FixedHardGateReport {
  verifierVersion: typeof COMBOBOX_LIVE_V38_FIXED_VERIFIER_VERSION;
...
export interface ComboboxLiveV38FixedFixedPoint {
  stable: boolean;
  sourceIrRead: false;
...
const validateThresholds = (thresholds: ComboboxLiveV38FixedThresholds): string[] => {
...
const stratumKeys = (row: ComboboxLiveV38FixedVisualRow): Array<[string, string]> => [
  ["source", row.source],
  ["state", row.state],
  ["adornment", row.adornment],
];

export function verifyComboboxLiveV38FixedHardGates(
  input: ComboboxLiveV38FixedHardGateInput,
): ComboboxLiveV38FixedHardGateReport {
  ...
  const exactFields: Array<keyof ComboboxLiveV38FixedCellValidation> = [
    "rolesExact",
    "textExact",
  ...
  const strata = new Map<string, ComboboxLiveV38FixedVisualRow[]>();
  ...
  return {
    verifierVersion: COMBOBOX_LIVE_V38_FIXED_VERIFIER_VERSION,
    technicalPassed,
    overallInputSuccess:
      technicalPassed &&
      input.humanSignoff.status === "passed" &&
...
export function verifyComboboxLiveV38FixedSceneFixedPoint<Instance>(
...
): ComboboxLiveV38FixedFixedPoint {
  const runCycle = (): {
    sceneIr: string;
    compiledIr: string;
...
export function proveSceneMultiset(
  ...
  scene: SceneNodeSnapshot,
): Pick<ComboboxLiveV38FixedSceneProof, "adapterIdentity" | "accounting"> {
  return {
    adapterIdentity,
