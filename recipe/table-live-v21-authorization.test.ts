...
import { TABLE_LIVE_V21_INDEX_PATH } from "./build-table-live-proof-v21.js";
import {
...
import {
  buildTableLiveV21AuthorizationArtifact,
  validateTableLiveV21AntecedentIndex,
  validateTableLiveV21AuthorizationArtifact,
  validateTableLiveV21History,
  validateTableLiveV21UnitTestSource,
  type TableLiveV21AntecedentIndex,
  type TableLiveV21AuthorizationArtifact,
  type TableLiveV21AuthorizationProof,
  type TableLiveV21HistoryState,
} from "./table-live-v21-authorization.js";
import {
  tableLiveV2AttestationSha256,
  validateTableLiveV21ControlFlowSource,
  validateTableLiveV21SecurityAttestation,
  type TableLiveV21SecurityAttestation,
} from "./table-live-v21-preflight.js";

const indexBytes = (): Buffer => readFileSync(TABLE_LIVE_V21_INDEX_PATH);
const index = (): TableLiveV21AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as TableLiveV21AntecedentIndex;

const artifact = (): TableLiveV21AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildTableLiveV21AuthorizationArtifact({
...
const pendingState = (): TableLiveV21HistoryState => ({
  index: index(),
...
const authorizedState = (): TableLiveV21HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: TableLiveV21AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/table-live-pivot-v21/capture-authorization.json",
});

const attestation = (
  authorization: TableLiveV21AuthorizationArtifact,
): TableLiveV21SecurityAttestation => {
  const body: Omit<TableLiveV21SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(TABLE_LIVE_V21_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateTableLiveV21History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateTableLiveV21History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateTableLiveV21History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateTableLiveV21History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateTableLiveV21History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateTableLiveV21UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/table-live-v21-authorization.test.ts", "utf8");
  assert.deepEqual(validateTableLiveV21UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateTableLiveV21AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateTableLiveV21AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateTableLiveV21AuthorizationArtifact(
        planted as TableLiveV21AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateTableLiveV21SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = tableLiveV2AttestationSha256(
      body as Omit<TableLiveV21SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateTableLiveV21SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/table-live-v21-authorized.ts", "utf8");
  assert.deepEqual(validateTableLiveV21ControlFlowSource(source), []);
});
