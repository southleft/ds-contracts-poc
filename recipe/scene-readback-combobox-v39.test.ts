...
import {
  ...
  COMBOBOX_LIVE_V39_LEADING_SLOT_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V39_LEADING_SLOT_COMPILE_CARRY_VISIBLE_MARKER,
  COMBOBOX_LIVE_V39_LISTBOX_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V39_LISTBOX_CLIPS_CONTENT_OMITTED_MARKER,
  COMBOBOX_LIVE_V39_LISTBOX_CORNER_RADIUS_OMITTED_MARKER,
  COMBOBOX_LIVE_V39_LISTBOX_EMPTY_EFFECTS_OMITTED_MARKER,
  COMBOBOX_LIVE_V39_LISTBOX_EMPTY_STROKES_OMITTED_MARKER,
  COMBOBOX_LIVE_V39_OMIT_EMPTY_INSTANCE_PAYLOAD_MARKER,
  COMBOBOX_LIVE_V39_OBSERVE_OMIT_EMPTY_INSTANCE_PAYLOAD_MARKER,
  COMBOBOX_LIVE_V39_OBSERVE_OMIT_OPTION_INSTANCE_PAYLOAD_MARKER,
  COMBOBOX_LIVE_V39_OBSERVE_OPTION_SET_COMPILE_CARRY_NAME_MARKER,
  COMBOBOX_LIVE_V39_OPTION_SET_COMPILE_CARRY_LABEL,
  COMBOBOX_LIVE_V39_OPTION_SET_COMPILE_CARRY_LABEL_MARKER,
  COMBOBOX_LIVE_V39_OPTION_SET_COMPILE_CARRY_NAME,
  COMBOBOX_LIVE_V39_OPTION_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V39_OPTION_CLIPS_CONTENT_OMITTED_MARKER,
  COMBOBOX_LIVE_V39_OPTION_LAYOUT_HEIGHT_ALIAS_MARKER,
  COMBOBOX_LIVE_V39_OPTION_INSTANCE_BINDING_EXTRAS_MARKER,
  COMBOBOX_LIVE_V39_OPTION_INSTANCE_FILLS_OMITTED_MARKER,
  COMBOBOX_LIVE_V39_OPTION_INSTANCE_PAYLOAD_OMITTED_MARKER,
  COMBOBOX_LIVE_V39_OVERLAY_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V39_OVERLAY_EMPTY_DASH_PATTERN_OMITTED_MARKER,
  COMBOBOX_LIVE_V39_SET_CLIPS_CONTENT_OMITTED_MARKER,
  COMBOBOX_LIVE_V39_OVERLAY_LAYOUT_WIDTH_ALIAS_MARKER,
  COMBOBOX_LIVE_V39_PROJECT_LIVE_ROOT_OWNERSHIP_KEY_MARKER,
  COMBOBOX_LIVE_V39_RECOVER_COMPONENT_PROPERTY_NAME_MARKER,
  COMBOBOX_LIVE_V39_RECOVER_RECIPE_COMPONENT_REF_MARKER,
  COMBOBOX_LIVE_V39_SELECTED_INDICATOR_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V39_SELECTED_INDICATOR_COMPILE_CARRY_VISIBLE_MARKER,
  COMBOBOX_LIVE_V39_TRAILING_SLOT_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V39_TRAILING_SLOT_COMPILE_CARRY_VISIBLE_MARKER,
  COMBOBOX_LIVE_V39_TRIGGER_BINDING_COMPILE_ORDER_MARKER,
  COMBOBOX_LIVE_V39_TRIGGER_COMPILE_CARRY_CHARACTERS,
  COMBOBOX_LIVE_V39_TRIGGER_COMPILE_CARRY_CHARACTERS_MARKER,
  COMBOBOX_LIVE_V39_TRIGGER_EMPTY_EFFECTS_OMITTED_MARKER,
  COMBOBOX_LIVE_V39_TRIGGER_SLOT_COMPONENT_REF_COMPILE_SIBLING_ORDER_MARKER,
  COMBOBOX_LIVE_V39_TRIGGER_SLOT_COMPILE_SIBLING_COMPONENT_REFS,
  COMBOBOX_LIVE_V39_TRIGGER_SLOT_COMPILE_SIBLING_ROLES,
...
test("compile trigger-slot componentRefs are prefix then clear then popup", () => {
  ...
  const refs = walkIr(trigger)
    .filter(
      (node) =>
        node.kind === "instance" &&
        (COMBOBOX_LIVE_V39_TRIGGER_SLOT_COMPILE_SIBLING_ROLES as readonly string[]).includes(
          node.role ?? "",
        ),
    )
    .map((node) =>
...
test("host sceneToNormalizedIr recovers trigger-slot componentRefs in compile sibling order, not live selected-first order", () => {
  ...
  assert.deepEqual(
    refsOf(host),
    [...COMBOBOX_LIVE_V39_TRIGGER_SLOT_COMPILE_SIBLING_COMPONENT_REFS],
    "v39 sceneToNormalizedIr must recover prefix then clear then popup",
  );
  assert.equal(
    canonicalizeObservedTriggerSlotComponentRef(
      "selected",
...
test("host observe recovers compile-carried option-set name, not the live display name", () => {
  ...
  assert.equal(
    COMBOBOX_LIVE_V39_OPTION_SET_COMPILE_CARRY_NAME,
    "combobox/option-set :: Combobox option",
    "v39 must recover compile's option-set name, not invent a name or parse the live fixture string",
  );
  assert.equal(
    COMBOBOX_LIVE_V39_OPTION_SET_COMPILE_CARRY_LABEL,
    "Combobox option",
    "v39 reuses the v33 compile-carried Combobox option label",
  );
  const fixtures = [
    {
      displayName: "MUI Autocomplete / combobox@1 offline fixture",
  ...
  for (const fixture of fixtures) {
    ...
    assert.equal(
      ...
      undefined,
      "v39 observe must recover compile-carried combobox/option-set :: Combobox option",
    );
    ...
    assert.equal(
      ...
      false,
      "v39 must not change combobox/set names that already match compile identity.name",
    );
    const instance = adaptReviewedCombobox(fixture.reviewed, fixture.config);
    const envelope = compileComboboxRecipe(instance);
    const compiledOptionSet = walkIr(envelope.ir).find(
