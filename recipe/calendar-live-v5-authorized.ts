...
import { CALENDAR_LIVE_V5_AUTHORIZATION_PATH } from "./build-calendar-live-proof-v5.js";
import { verifyCalendarLiveV5Authorization } from "./calendar-live-v5-authorization.js";
import { runCalendarLiveV5Preflight } from "./calendar-live-v5-preflight.js";
import { CalendarLiveV5Orchestrator } from "./run-calendar-live-v5.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Calendar live v5 requires exact authorization artifact ${CALENDAR_LIVE_V5_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/calendar-live-v5-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyCalendarLiveV5Authorization(root);
    runCalendarLiveV5Preflight(
      root,
      authorization,
    ...
    const orchestrator = CalendarLiveV5Orchestrator.initialize({
      root,
  ...
  const orchestrator = CalendarLiveV5Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
