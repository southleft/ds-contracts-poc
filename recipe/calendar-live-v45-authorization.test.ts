...
import { CALENDAR_LIVE_V45_INDEX_PATH } from "./build-calendar-live-proof-v45.js";
import {
...
import {
  buildCalendarLiveV45AuthorizationArtifact,
  validateCalendarLiveV45AntecedentIndex,
  validateCalendarLiveV45AuthorizationArtifact,
  validateCalendarLiveV45History,
  validateCalendarLiveV45UnitTestSource,
  type CalendarLiveV45AntecedentIndex,
  type CalendarLiveV45AuthorizationArtifact,
  type CalendarLiveV45AuthorizationProof,
  type CalendarLiveV45HistoryState,
} from "./calendar-live-v45-authorization.js";
import {
  calendarLiveV1AttestationSha256,
  validateCalendarLiveV45ControlFlowSource,
  validateCalendarLiveV45SecurityAttestation,
  type CalendarLiveV45SecurityAttestation,
} from "./calendar-live-v45-preflight.js";

const indexBytes = (): Buffer => readFileSync(CALENDAR_LIVE_V45_INDEX_PATH);
const index = (): CalendarLiveV45AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as CalendarLiveV45AntecedentIndex;

const artifact = (): CalendarLiveV45AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildCalendarLiveV45AuthorizationArtifact({
...
const pendingState = (): CalendarLiveV45HistoryState => ({
  index: index(),
...
const authorizedState = (): CalendarLiveV45HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: CalendarLiveV45AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/calendar-live-pivot-v45/capture-authorization.json",
});

const attestation = (
  authorization: CalendarLiveV45AuthorizationArtifact,
): CalendarLiveV45SecurityAttestation => {
  const body: Omit<CalendarLiveV45SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(CALENDAR_LIVE_V45_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateCalendarLiveV45History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateCalendarLiveV45History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateCalendarLiveV45History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateCalendarLiveV45History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateCalendarLiveV45History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and table v15 is hermetic", () => {
  ...
  assert.match(
    validateCalendarLiveV45UnitTestSource(v6).join("\n"),
    /phase-sensitive unit test/,
  );
  const own = readFileSync("recipe/calendar-live-v45-authorization.test.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV45UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateCalendarLiveV45AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateCalendarLiveV45AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateCalendarLiveV45AuthorizationArtifact(
        planted as CalendarLiveV45AuthorizationArtifact,
        index(),
...
test("one-root denominator, capture gate, cleanup, and prior-lineage reuse are rejected", () => {
  ...
  for (const mutate of [
    ...
    (value: Record<string, any>) => {
      value.execution.comboboxLiveV45AuthorizationReusable = true;
    },
    (value: Record<string, any>) => {
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateCalendarLiveV45SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = calendarLiveV1AttestationSha256(
      body as Omit<CalendarLiveV45SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateCalendarLiveV45SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  const source = readFileSync("recipe/calendar-live-v45-authorized.ts", "utf8");
  assert.deepEqual(validateCalendarLiveV45ControlFlowSource(source), []);
});
