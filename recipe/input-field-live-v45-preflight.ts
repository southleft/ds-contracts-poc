...
import {
  ...
  verifyInputLiveV45Authorization,
  type InputLiveV45AntecedentIndex,
  type InputLiveV45AuthorizationProof,
} from "./input-field-live-v45-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V45_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v45/operator-security-attestation-template.json";
export const INPUT_LIVE_V45_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v45-security-attestation.json";

export interface InputLiveV45SecurityAttestation {
...
export interface InputLiveV45PreflightReport {
  artifactVersion: "input-live-v45-preflight-v1";
...
export const inputLiveV45AttestationSha256 = (
  value: Omit<InputLiveV45SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV45SecurityAttestation(
  value: unknown,
  proof: InputLiveV45AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v45-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V45_TARGET) ||
...
export function validateInputLiveV45ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV45Authorization");
  const preflight = source.indexOf("runInputLiveV45Preflight");
  const initialize = source.indexOf("InputLiveV45Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV45Preflight(
  root: string,
  proof: InputLiveV45AuthorizationProof,
  attestationPath = INPUT_LIVE_V45_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV45PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V45_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V45_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V45_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV45AntecedentIndex;
  failures.push(...validateInputLiveV45AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V45_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V45_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v45-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV45ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV45SecurityAttestation;
  return {
    artifactVersion: "input-live-v45-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V45_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV45Preflight(
    process.cwd(),
    verifyInputLiveV45Authorization(),
    attestationPath,
