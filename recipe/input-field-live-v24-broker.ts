...
import {
  assertInputLiveV24RootProofs,
  INPUT_LIVE_V24_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V24_RESPONSE_CONTRACTS,
  type InputLiveV24RootProof,
} from "./input-field-live-v24-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V24_BROKER_VERSION = "input-live-v24-broker-v2";
export const INPUT_LIVE_V24_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V24_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V24_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V24_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V24_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const INPUT_LIVE_V24_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV24RemotePhase =
  (typeof INPUT_LIVE_V24_REMOTE_PHASES)[number];
export type InputLiveV24AuthorizationMode = "live" | "simulated";

export interface InputLiveV24CommitPins {
...
export interface InputLiveV24TransactionAuthorization extends InputLiveV24CommitPins {
  mode: InputLiveV24AuthorizationMode;
...
export interface InputLiveV24Manifest {
  artifactVersion: "input-live-v24-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V24_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V24_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V24_TARGET;
  authorization: InputLiveV24TransactionAuthorization;
...
export interface InputLiveV24ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV24RequestBody {
  artifactVersion: "input-live-v24-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V24_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV24RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV24CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V24_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV24ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV24Request extends InputLiveV24RequestBody {
...
export interface InputLiveV24Receipt {
  artifactVersion: "input-live-v24-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V24_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV24RemotePhase;
...
export interface InputLiveV24AcceptedResponse<T = unknown> {
  request: InputLiveV24Request;
  receipt: InputLiveV24Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV24IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV24ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V24_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V24_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V24_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V24_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V24_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V24_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V24_TARGET.editorType;
};
...
export const inputLiveV24RequestSequence = (
  phase: InputLiveV24RemotePhase,
  captureIndex?: number,
): number => {
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV24RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV24RequestPath = (
  directory: string,
  phase: InputLiveV24RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV24ResponsePath = (
  directory: string,
  phase: InputLiveV24RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV24ReceiptPath = (
  directory: string,
  phase: InputLiveV24RemotePhase,
  captureIndex?: number,
): string =>
...
const readManifest = (directory: string): InputLiveV24Manifest => {
  const manifest = json<InputLiveV24Manifest>(
  ...
  if (
    manifest.artifactVersion !== "input-live-v24-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V24_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V24_DYNAMIC_TOOL) ||
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV24RemotePhase,
  captureIndex?: number,
): InputLiveV24Request | undefined => {
  const file = inputLiveV24RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV24Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV24RemotePhase,
  captureIndex?: number,
): InputLiveV24Receipt | undefined => {
  const file = inputLiveV24ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV24Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV24Request | undefined;
  receipt: InputLiveV24Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: InputLiveV24RemotePhase,
): InputLiveV24ResponseContract => ({
  ...INPUT_LIVE_V24_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V24_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V24_DYNAMIC_TOOL,
      target: INPUT_LIVE_V24_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v24-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V24_BROKER_VERSION,
  target: INPUT_LIVE_V24_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V24_PROGRAM_START*/${program}/*INPUT_LIVE_V24_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V24_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V24_PROGRAM_END*/";
...
export function createInputLiveV24Transaction(
  directory: string,
  ...
  const manifest: InputLiveV24Manifest = {
    artifactVersion: "input-live-v24-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V24_BROKER_VERSION,
...
export function validateInputLiveV24Request(
  directory: string,
  request: InputLiveV24Request,
): void {
  ...
  const expectedSequence = inputLiveV24RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v24-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V24_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V24_TARGET.fileKey ||
...
export function issueInputLiveV24Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV24RemotePhase,
  program: string,
  options: InputLiveV24IssueOptions = {},
): InputLiveV24Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV24RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV24Request>(targetPath);
    validateInputLiveV24Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v12 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV24ReceiptPath(directory, phase, captureIndex)))
  ...
  if (phase === "capture") validateInputLiveV24TechnicalGates(directory);
  const manifest = readManifest(directory);
  ...
  const sequence = inputLiveV24RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V24_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V24_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V24_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV24RequestBody = {
    artifactVersion: "input-live-v24-broker-request-v2",
    brokerVersion: INPUT_LIVE_V24_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV24Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV24Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV24RawResponse(
  directory: string,
  phase: InputLiveV24RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV24RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v12 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV24Request>(requestFile);
  if (
  ...
  const target = inputLiveV24ResponsePath(directory, phase, captureIndex);
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV24Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v24-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V24_BROKER_VERSION,
...
export function acceptInputLiveV24Response<T = unknown>(
  directory: string,
  phase: InputLiveV24RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV24AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV24ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v12 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV24Request>(
    inputLiveV24RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV24Request(directory, request);
  if (phase === "capture") validateInputLiveV24TechnicalGates(directory);
  const rawFile = inputLiveV24ResponsePath(directory, phase, captureIndex);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v24-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V24_BROKER_VERSION ||
...
export function persistInputLiveV24TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV24RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV24RootProofs(rootProofs);
  ...
  const gates = Object.fromEntries(
    INPUT_LIVE_V24_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v24-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V24_BROKER_VERSION,
...
export function validateInputLiveV24TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v24-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V24_TECHNICAL_GATES.some(
...
export function validateInputLiveV24Receipt(
  directory: string,
  phase: InputLiveV24RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV24Request(directory, request);
  const receipt = json<InputLiveV24Receipt>(
    inputLiveV24ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV24ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v24-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V24_BROKER_VERSION ||
...
export function readInputLiveV24AcceptedRawPayload(
  directory: string,
  phase: InputLiveV24RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV24Receipt(directory, phase, captureIndex);
...
export function createInputLiveV24FakeDynamicResponse(
  request: InputLiveV24Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V24_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V24_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV24PersistedRequests(
  directory: string,
): InputLiveV24Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV24Request>(file)] : [];
    });
}

export const inputLiveV24Sha256 = sha256;
