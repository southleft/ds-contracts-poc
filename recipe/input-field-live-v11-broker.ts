...
import {
  assertInputLiveV11RootProofs,
  INPUT_LIVE_V11_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  INPUT_LIVE_V11_RESPONSE_CONTRACTS,
  type InputLiveV11RootProof,
} from "./input-field-live-v11-contract.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V11_BROKER_VERSION = "input-live-v11-broker-v2";
export const INPUT_LIVE_V11_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const INPUT_LIVE_V11_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const INPUT_LIVE_V11_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const INPUT_LIVE_V11_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const INPUT_LIVE_V11_REMOTE_PHASES = [
  "writer",
  "extract",
  "probe",
  "capture",
...
export const INPUT_LIVE_V11_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type InputLiveV11RemotePhase =
  (typeof INPUT_LIVE_V11_REMOTE_PHASES)[number];
export type InputLiveV11AuthorizationMode = "live" | "simulated";

export interface InputLiveV11CommitPins {
...
export interface InputLiveV11TransactionAuthorization extends InputLiveV11CommitPins {
  mode: InputLiveV11AuthorizationMode;
...
export interface InputLiveV11Manifest {
  artifactVersion: "input-live-v11-broker-manifest-v2";
  brokerVersion: typeof INPUT_LIVE_V11_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof INPUT_LIVE_V11_DYNAMIC_TOOL;
  target: typeof INPUT_LIVE_V11_TARGET;
  authorization: InputLiveV11TransactionAuthorization;
...
export interface InputLiveV11ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface InputLiveV11RequestBody {
  artifactVersion: "input-live-v11-broker-request-v2";
  brokerVersion: typeof INPUT_LIVE_V11_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: InputLiveV11RemotePhase;
  sequence: number;
  ...
  commitPins: InputLiveV11CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof INPUT_LIVE_V11_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: InputLiveV11ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface InputLiveV11Request extends InputLiveV11RequestBody {
...
export interface InputLiveV11Receipt {
  artifactVersion: "input-live-v11-broker-receipt-v2";
  brokerVersion: typeof INPUT_LIVE_V11_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: InputLiveV11RemotePhase;
...
export interface InputLiveV11AcceptedResponse<T = unknown> {
  request: InputLiveV11Request;
  receipt: InputLiveV11Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface InputLiveV11IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: InputLiveV11ResponseContract;
...
const equalTarget = (value: unknown): value is typeof INPUT_LIVE_V11_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === INPUT_LIVE_V11_TARGET.fileKey &&
  (value as BrokerTarget).fileName === INPUT_LIVE_V11_TARGET.fileName &&
  (value as BrokerTarget).editorType === INPUT_LIVE_V11_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== INPUT_LIVE_V11_TARGET.fileKey ||
    context.fileName !== INPUT_LIVE_V11_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === INPUT_LIVE_V11_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Input live v11 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    throw new TypeError("Input live v11 capture index must be 0..127");
  return `capture-${String(captureIndex).padStart(3, "0")}`;
};

export const inputLiveV11RequestSequence = (
  phase: InputLiveV11RemotePhase,
  captureIndex?: number,
): number => {
  if (phase === "writer") return 1;
  if (phase === "cleanup") return 2;
  if (phase === "extract") return 3;
  if (phase === "probe") return 4;
  requestId(phase, captureIndex);
  return 5 + captureIndex!;
};
...
const requestDirectory = (
  ...
  path.join(
    directory,
    "requests",
    `${String(inputLiveV11RequestSequence(phase, captureIndex)).padStart(3, "0")}-${requestId(phase, captureIndex)}`,
  );
export const inputLiveV11RequestPath = (
  directory: string,
  phase: InputLiveV11RemotePhase,
  captureIndex?: number,
): string =>
  path.join(requestDirectory(directory, phase, captureIndex), "request.json");
export const inputLiveV11ResponsePath = (
  directory: string,
  phase: InputLiveV11RemotePhase,
  captureIndex?: number,
): string =>
...
export const inputLiveV11ReceiptPath = (
  directory: string,
  phase: InputLiveV11RemotePhase,
  captureIndex?: number,
): string =>
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Input live v11 authorization pins are malformed");
};

const readManifest = (directory: string): InputLiveV11Manifest => {
  const manifest = json<InputLiveV11Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Input live v11 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "input-live-v11-broker-manifest-v2" ||
    manifest.brokerVersion !== INPUT_LIVE_V11_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(INPUT_LIVE_V11_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Input live v11 broker manifest");
  return manifest;
};

const unsignedBody = (request: InputLiveV11Request): InputLiveV11RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: InputLiveV11RemotePhase,
  captureIndex?: number,
): InputLiveV11Request | undefined => {
  const file = inputLiveV11RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV11Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: InputLiveV11RemotePhase,
  captureIndex?: number,
): InputLiveV11Receipt | undefined => {
  const file = inputLiveV11ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<InputLiveV11Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: InputLiveV11Request | undefined;
  receipt: InputLiveV11Receipt | undefined;
} => {
  if (phase === "writer") return { request: undefined, receipt: undefined };
  if (phase === "cleanup" || phase === "extract")
    return {
...
const responseContractFor = (
  phase: InputLiveV11RemotePhase,
): InputLiveV11ResponseContract => ({
  ...INPUT_LIVE_V11_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: INPUT_LIVE_V11_BROKER_VERSION,
      transactionId: manifest.transactionId,
      attempt: manifest.attempt,
      ...value,
      expectedDynamicTool: INPUT_LIVE_V11_DYNAMIC_TOOL,
      target: INPUT_LIVE_V11_TARGET,
      commitPins: {
...
const __broker=${JSON.stringify({
  artifactVersion: "input-live-v11-plugin-response-v2",
  brokerVersion: INPUT_LIVE_V11_BROKER_VERSION,
  target: INPUT_LIVE_V11_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*INPUT_LIVE_V11_PROGRAM_START*/${program}/*INPUT_LIVE_V11_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*INPUT_LIVE_V11_PROGRAM_START*/";
  const endMarker = "/*INPUT_LIVE_V11_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Input live v11 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createInputLiveV11Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Input live v11 private signing key does not match authorized public-key identity",
    );
  const manifest: InputLiveV11Manifest = {
    artifactVersion: "input-live-v11-broker-manifest-v2",
    brokerVersion: INPUT_LIVE_V11_BROKER_VERSION,
...
export function validateInputLiveV11Request(
  directory: string,
  request: InputLiveV11Request,
): void {
  ...
  const expectedSequence = inputLiveV11RequestSequence(
    request.phase,
    request.captureIndex === null ? undefined : request.captureIndex,
  );
  ...
  if (
    request.artifactVersion !== "input-live-v11-broker-request-v2" ||
    request.brokerVersion !== INPUT_LIVE_V11_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== INPUT_LIVE_V11_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Input live v11 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueInputLiveV11Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: InputLiveV11RemotePhase,
  program: string,
  options: InputLiveV11IssueOptions = {},
): InputLiveV11Request {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const id = requestId(phase, captureIndex);
  const targetPath = inputLiveV11RequestPath(directory, phase, captureIndex);
  if (existsSync(targetPath)) {
    const existing = json<InputLiveV11Request>(targetPath);
    validateInputLiveV11Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Input live v11 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(inputLiveV11ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Input live v11 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Input live v11 empty code envelope refused");
  if (phase === "capture") validateInputLiveV11TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Input live v11 out-of-order phase: ${id}`);
  const expectedResponse =
    options.expectedResponse ?? responseContractFor(phase);
  const programSha256 = sha256(program);
  const sequence = inputLiveV11RequestSequence(phase, captureIndex);
  ...
  const arguments_ = {
    fileKey: INPUT_LIVE_V11_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? INPUT_LIVE_V11_SIGNED_WRITER_TIMEOUT_MS
        : INPUT_LIVE_V11_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: InputLiveV11RequestBody = {
    artifactVersion: "input-live-v11-broker-request-v2",
    brokerVersion: INPUT_LIVE_V11_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: InputLiveV11Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateInputLiveV11Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistInputLiveV11RawResponse(
  directory: string,
  phase: InputLiveV11RemotePhase,
  rawResponse: string,
  captureIndex?: number,
): string {
  const requestFile = inputLiveV11RequestPath(directory, phase, captureIndex);
  if (!existsSync(requestFile))
    throw new Error(
      `Input live v11 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<InputLiveV11Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Input live v11 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = inputLiveV11ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Input live v11 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<InputLiveV11Receipt, "receiptSha256"> = {
    artifactVersion: "input-live-v11-broker-receipt-v2",
    brokerVersion: INPUT_LIVE_V11_BROKER_VERSION,
...
export function acceptInputLiveV11Response<T = unknown>(
  directory: string,
  phase: InputLiveV11RemotePhase,
  options: {
    captureIndex?: number;
...
): InputLiveV11AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = inputLiveV11ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Input live v11 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<InputLiveV11Request>(
    inputLiveV11RequestPath(directory, phase, captureIndex),
  );
  validateInputLiveV11Request(directory, request);
  if (phase === "capture") validateInputLiveV11TechnicalGates(directory);
  const rawFile = inputLiveV11ResponsePath(directory, phase, captureIndex);
  if (!existsSync(rawFile))
    throw new Error(`Input live v11 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "input-live-v11-plugin-response-v2" ||
    plugin?.brokerVersion !== INPUT_LIVE_V11_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Input live v11 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistInputLiveV11TechnicalGates(
  directory: string,
  rootProofs: readonly InputLiveV11RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertInputLiveV11RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Input live v11 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Input live v11 probe payload hash malformed");
  const gates = Object.fromEntries(
    INPUT_LIVE_V11_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "input-live-v11-host-technical-gates-v2",
    brokerVersion: INPUT_LIVE_V11_BROKER_VERSION,
...
export function validateInputLiveV11TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "input-live-v11-host-technical-gates-v2" ||
    ...
    INPUT_LIVE_V11_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Input live v11 technical gates artifact");
}

export function validateInputLiveV11Receipt(
  directory: string,
  phase: InputLiveV11RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateInputLiveV11Request(directory, request);
  const receipt = json<InputLiveV11Receipt>(
    inputLiveV11ReceiptPath(directory, phase, captureIndex),
  );
  const raw = readFileSync(
    inputLiveV11ResponsePath(directory, phase, captureIndex),
  );
  const response = JSON.parse(raw.toString("utf8")) as Record<string, any>;
  const { receiptSha256, ...body } = receipt;
  if (
    receipt.artifactVersion !== "input-live-v11-broker-receipt-v2" ||
    receipt.brokerVersion !== INPUT_LIVE_V11_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Input live v11 accepted receipt: ${request.requestId}`,
    );
}

export function readInputLiveV11AcceptedRawPayload(
  directory: string,
  phase: InputLiveV11RemotePhase,
  captureIndex?: number,
): unknown {
  validateInputLiveV11Receipt(directory, phase, captureIndex);
...
export function createInputLiveV11FakeDynamicResponse(
  request: InputLiveV11Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? INPUT_LIVE_V11_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? INPUT_LIVE_V11_TARGET,
    ...overrides.extra,
  };
}

export function listInputLiveV11PersistedRequests(
  directory: string,
): InputLiveV11Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<InputLiveV11Request>(file)] : [];
    });
}

export const inputLiveV11Sha256 = sha256;
