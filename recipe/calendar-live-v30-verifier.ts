...
import {
  verifyCalendarLiveV30FixedSceneFixedPoint,
  type CalendarLiveV30FixedFixedPoint,
} from "./calendar-live-v30-fixed-point.js";

export const CALENDAR_LIVE_V30_VERIFIER_VERSION = "calendar-live-v30-verifier-v1";

export function buildCalendarLiveV30RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV30RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV30RawNode[];
}

export interface CalendarLiveV30NormalizedScene {
...
export interface CalendarLiveV30SceneProof {
...
export function normalizeCalendarLiveV30Scene(
  root: CalendarLiveV30RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV30NormalizedScene {
  const canonicalBindings: CalendarLiveV30NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV30RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV30Scene<Instance>(
  normalized: CalendarLiveV30NormalizedScene,
...
): CalendarLiveV30SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV30FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV30ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV30PreCaptureGates(payload: unknown): void {
