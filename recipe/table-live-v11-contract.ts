...
import { buildTableLiveV11CleanupRuntime } from "./table-live-v11-cleanup.js";
import { type TableLiveV11FixedFixedPoint } from "./table-live-v11-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV11RestoreProgram,
  validateTableLiveV11RestorePayload,
  type TableLiveV11RestorePayload,
} from "./table-live-v11-restore.js";
import {
  buildTableLiveV11RawPropertyRuntime,
  normalizeTableLiveV11Scene,
  type TableLiveV11RawNode,
} from "./table-live-v11-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v11.js";
import {
...
export const TABLE_LIVE_V11_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V11_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V11_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V11_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V11_VARIANT_COUNT = 20;
export const TABLE_LIVE_V11_SET_COUNT = 6;
export const TABLE_LIVE_V11_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V11_HOST_PHASES = 3;
export const TABLE_LIVE_V11_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V11_RESTORE_COUNT = 8;
export const TABLE_LIVE_V11_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V11_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV11SourceId = (typeof TABLE_LIVE_V11_SOURCE_IDS)[number];
export type TableLiveV11Kind = "table" | "row" | "cell";

export interface TableLiveV11SourceIdentity {
  source: TableLiveV11SourceId;
...
export interface TableLiveV11WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV11ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV11SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV11RawNode;
    rowScene: TableLiveV11RawNode;
    cellScene: TableLiveV11RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV11RootProof {
  source: TableLiveV11SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV11FixedFixedPoint;
}

export interface TableLiveV11ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV11SourceId;
    adapterIdentity: string;
...
export interface TableLiveV11CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV11SourceId;
  adapterIdentity: string;
...
export interface TableLiveV11CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV11SourceId;
  frameWidth: number;
...
export interface TableLiveV11CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v11 ${label} must be an object`);
...
export function validateTableLiveV11WriterPayload(
  value: unknown,
): TableLiveV11WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V11_NAMESPACE ||
    ...
    throw new TypeError("Table live v11 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError("Table live v11 writer created-node denominator invalid");
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v11 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV11SourceIdentity,
  kind: TableLiveV11Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV11ExtractProgram(
  writer: TableLiveV11WriterOwnership,
  sources: readonly TableLiveV11SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError("Table live v11 extract requires two source identities");
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V11_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V11_NAMESPACE)}
${buildTableLiveV11RawPropertyRuntime()}
...
export function validateTableLiveV11ExtractPayload(
  value: unknown,
  writer: TableLiveV11WriterOwnership,
): TableLiveV11ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v11 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V11_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v11 extract schema/two-root mismatch");
  return payload as TableLiveV11ExtractPayload;
}

export function proveTableLiveV11Roots<Instance>(
  extract: TableLiveV11ExtractPayload,
  sources: readonly (TableLiveV11SourceIdentity & {
...
): TableLiveV11RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v11 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v11 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV11Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV11Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV11Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV11FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError("Table live v11 compile root must be library frame");
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v11 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v11 compile lost owned sets");
...
export function assertTableLiveV11RootProofs(
  proofs: readonly TableLiveV11RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v11 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v11 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV11ProbeProgram(
  writer: TableLiveV11WriterOwnership,
  sources: readonly TableLiveV11SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V11_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
for(const row of byAdapter.values()){
 ...
 const cell0=propertyKey(rowInstance,"Cell 0"),cell0Before=cell0&&rowInstance.componentProperties[cell0].value;let textPropertiesRestored=false;if(cell0){rowInstance.setProperties({[cell0]:"Table v11 deterministic probe"});const changed=nodes(rowInstance).some(node=>node.type==="TEXT"&&node.characters==="Table v11 deterministic probe");rowInstance.setProperties({[cell0]:cell0Before});textPropertiesRestored=changed&&JSON.stringify(original)===JSON.stringify(plain(instance));}
 rowInstance.remove();
...
export function validateTableLiveV11ProbePayload(
  value: unknown,
  writer: TableLiveV11WriterOwnership,
): TableLiveV11ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV11ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V11_VARIANT_COUNT ||
    new Set(payload.cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V11_VARIANT_COUNT ||
    payload.cells.some(
      (cell) =>
    ...
    throw new TypeError("Table live v11 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV11CaptureManifest(
  cells: readonly TableLiveV11CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V11_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V11_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V11_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V11_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v11 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV11CaptureCell[],
): string => {
  validateTableLiveV11CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV11CaptureProgram(
  writer: TableLiveV11WriterOwnership,
  cell: TableLiveV11CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V11_CAPTURE_COUNT
  )
    throw new TypeError("Table live v11 capture cell is malformed");
  ...
  const expectedSet =
    cell.kind === "table"
      ...
      : cell.kind === "row"
        ...
        : source?.cellSetId;
  if (!expectedSet)
    throw new TypeError(`Table live v11 capture adapter absent: ${cell.cellKey}`);
  return String.raw`
...
const frame=figma.createFrame();frame.name="Table v11 ephemeral / "+cell.cellKey;page.appendChild(frame);
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V11_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV11CapturePayload(
  value: unknown,
  cell: TableLiveV11CaptureCell,
  rawResponseBytes: number,
): TableLiveV11CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV11CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V11_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V11_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v11 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV11CaptureResponses(
  manifest: readonly TableLiveV11CaptureCell[],
  responses: readonly TableLiveV11CapturePayload[],
): void {
  validateTableLiveV11CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v11 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV11ObjectiveReport {
  artifactVersion: "table-live-v11-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV11Objective(
  manifest: readonly TableLiveV11CaptureCell[],
  responses: readonly TableLiveV11CapturePayload[],
): TableLiveV11ObjectiveReport {
  assertTableLiveV11CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v11-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV11CleanupProgram(
  writer: TableLiveV11WriterOwnership,
): string {
  const baseProgram = buildTableLiveV11CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V11_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v11 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV11CleanupPayload(
  payload: unknown,
  writer: TableLiveV11WriterOwnership,
): TableLiveV11CleanupPayload {
  const value = record(payload, "cleanup payload") as TableLiveV11CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v11 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V11_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV11WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
