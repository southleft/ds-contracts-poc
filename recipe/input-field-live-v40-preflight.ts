...
import {
  ...
  verifyInputLiveV40Authorization,
  type InputLiveV40AntecedentIndex,
  type InputLiveV40AuthorizationProof,
} from "./input-field-live-v40-authorization.js";
import { canonicalJson } from "./normalize.js";

export const INPUT_LIVE_V40_SECURITY_ATTESTATION_TEMPLATE_PATH =
  "recipe/evidence/input-field-live-pivot-v40/operator-security-attestation-template.json";
export const INPUT_LIVE_V40_SECURITY_ATTESTATION_DEFAULT_PATH =
  "private/input-live-v40-security-attestation.json";

export interface InputLiveV40SecurityAttestation {
...
export interface InputLiveV40PreflightReport {
  artifactVersion: "input-live-v40-preflight-v1";
...
export const inputLiveV40AttestationSha256 = (
  value: Omit<InputLiveV40SecurityAttestation, "attestationSha256">,
): string => sha256(canonicalJson(value));

export function validateInputLiveV40SecurityAttestation(
  value: unknown,
  proof: InputLiveV40AuthorizationProof,
): string[] {
  const artifact =
  ...
  if (
    artifact.artifactVersion !==
      "input-live-v40-operator-security-attestation-v1" ||
  ...
  if (
    ...
    !equalJson(artifact.scratchReadOnlyProbe?.target, INPUT_LIVE_V40_TARGET) ||
...
export function validateInputLiveV40ControlFlowSource(
  source: string,
): string[] {
  const failures: string[] = [];
  const authorization = source.indexOf("verifyInputLiveV40Authorization");
  const preflight = source.indexOf("runInputLiveV40Preflight");
  const initialize = source.indexOf("InputLiveV40Orchestrator.initialize");
  if (
    authorization < 0 ||
    preflight < 0 ||
    initialize < 0 ||
...
export function runInputLiveV40Preflight(
  root: string,
  proof: InputLiveV40AuthorizationProof,
  attestationPath = INPUT_LIVE_V40_SECURITY_ATTESTATION_DEFAULT_PATH,
  attempt = 1,
  completedAttempts: readonly number[] = [],
): InputLiveV40PreflightReport {
  const failures: string[] = [];
  if (
  ...
  if (
    proof.mode !== "live" ||
    !equalJson(proof.target, INPUT_LIVE_V40_TARGET) ||
    !equalJson(proof.expectedDynamicTool, INPUT_LIVE_V40_DYNAMIC_TOOL)
  )
    failures.push("v8 authorization proof target or tool mismatch");
  const indexPath = path.join(root, INPUT_LIVE_V40_INDEX_PATH);
  const indexBytes = readFileSync(indexPath);
  const index = JSON.parse(
    indexBytes.toString("utf8"),
  ) as InputLiveV40AntecedentIndex;
  failures.push(...validateInputLiveV40AntecedentIndex(index));
  if (
  ...
  if (!existsSync(path.join(root, INPUT_LIVE_V40_AUTHORIZATION_PATH)))
    failures.push("v8 authorization artifact is absent");
  const securityPath = path.resolve(root, attestationPath);
  if (!existsSync(securityPath))
    failures.push(
      `missing security attestation; create ${INPUT_LIVE_V40_SECURITY_ATTESTATION_DEFAULT_PATH} only after PAT rotation, MCP restart, owner-only env, zero secret scan, and exact Scratch probe`,
    );
  else {
    try {
      failures.push(
  ...
  const runnerSource = readFileSync(
    path.join(root, "recipe/input-field-live-v40-authorized.ts"),
    "utf8",
  );
  failures.push(...validateInputLiveV40ControlFlowSource(runnerSource));
  if (failures.length)
    throw new Error(
  ...
  ) as InputLiveV40SecurityAttestation;
  return {
    artifactVersion: "input-live-v40-preflight-v1",
    authorizationMode: "live",
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  if (!attestationPath)
    throw new Error(
      `--security-attestation is required; ${INPUT_LIVE_V40_SECURITY_ATTESTATION_TEMPLATE_PATH} is a template only`,
    );
  const report = runInputLiveV40Preflight(
    process.cwd(),
    verifyInputLiveV40Authorization(),
    attestationPath,
