...
import { INPUT_LIVE_V73_AUTHORIZATION_PATH } from "./build-input-field-live-proof-v73.js";
import { verifyInputLiveV73Authorization } from "./input-field-live-v73-authorization.js";
import { runInputLiveV73Preflight } from "./input-field-live-v73-preflight.js";
import { InputLiveV73Orchestrator } from "./run-input-field-live-v73.js";
...
const exactAuthorizationPath = (value: string): string => {
  ...
  if (path.resolve(value) !== expected)
    throw new Error(
      `Input live v12 requires exact authorization artifact ${INPUT_LIVE_V73_AUTHORIZATION_PATH}`,
    );
  return expected;
};
...
async function main(): Promise<void> {
  ...
  const directory = path.resolve(
    argument("--transaction") ??
      path.join(root, "private/input-live-v73-transaction"),
  );
  ...
  if (command === "init") {
    ...
    const completedAttempts = (argument("--completed-attempts") ?? "")
      ...
      .filter(Boolean)
      .map(Number);
    const authorization = verifyInputLiveV73Authorization(root);
    runInputLiveV73Preflight(
      root,
      authorization,
  ...
  const orchestrator = InputLiveV73Orchestrator.resume({
    root,
    transactionDirectory: directory,
    privateKey,
  });
