...
import {
  ...
  createInputLiveV32FakeDynamicResponse,
  ...
  listInputLiveV32PersistedRequests,
  persistInputLiveV32RawResponse,
  persistInputLiveV32TechnicalGates,
  validateInputLiveV32Receipt,
  validateInputLiveV32Request,
  type InputLiveV32RemotePhase,
...
import {
  ...
  type InputLiveV32ExtractPayload,
  type InputLiveV32ProbePayload,
  type InputLiveV32RestorePayload,
  type InputLiveV32RootProof,
  type InputLiveV32WriterOwnership,
} from "./input-field-live-v32-contract.js";
import {
  InputLiveV32Orchestrator,
  simulatedInputLiveV32Authorization,
} from "./run-input-field-live-v32.js";
...
const captures: InputLiveV32CaptureCell[] = Array.from(
  { length: 128 },
  (_, index) => {
    ...
    return {
      ...
      reference: {
        ...
        sha256: inputLiveV32Sha256(`reference-${index}`),
        width: 486,
        height: 240,
...
const fixture = () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "input-live-v32-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  createInputLiveV32Transaction(directory, privateKey, {
    authorization: simulatedAuthorization(privateKey),
    proofPlanSha256: "9".repeat(64),
    captureManifestSha256: inputLiveV32CaptureManifestSha256(captures),
...
const rawResponse = (
  ...
  payload: unknown,
  overrides?: Parameters<typeof createInputLiveV32FakeDynamicResponse>[2],
): string =>
  JSON.stringify(
    createInputLiveV32FakeDynamicResponse(request, payload, overrides),
  );
...
const issueAndAccept = <T>(
  ...
  assert.deepEqual(request.expectedDynamicTool, INPUT_LIVE_V32_DYNAMIC_TOOL);
  assert.deepEqual(request.target, INPUT_LIVE_V32_TARGET);
  persistInputLiveV32RawResponse(
    directory,
    phase,
  ...
  const accepted = acceptInputLiveV32Response<T>(directory, phase, {
    captureIndex: options.captureIndex,
    validate: options.validate,
  });
  validateInputLiveV32Receipt(directory, phase, options.captureIndex);
...
const restorePayload = (
  ownership: InputLiveV32WriterOwnership,
): InputLiveV32RestorePayload => ({
...
const rootProofs = (): InputLiveV32RootProof[] =>
...
const capturePayload = (
  cell: InputLiveV32CaptureCell,
): InputLiveV32CapturePayload => {
  const bytes = Buffer.from(`input-live-v32-${cell.index}`);
  return {
    index: cell.index,
    ...
    pngSha256: inputLiveV32Sha256(bytes),
...
test("same signed fake-operator path completes two roots and 128 ordered captures", () => {
  ...
  try {
    ...
    const cleanupRequest = issueInputLiveV32Request(
      directory,
      privateKey,
      "cleanup",
    ...
    assert.ok(readFileSync(inputLiveV32RequestPath(directory, "cleanup")));

    issueAndAccept(directory, privateKey, "restore", restorePayload(writer), {
      validate: (payload) =>
        validateInputLiveV32RestorePayload(payload, writer),
    });
    issueAndAccept(directory, privateKey, "extract", extractPayload(writer), {
      validate: (payload) =>
        validateInputLiveV32ExtractPayload(payload, writer),
    });
    const probe = issueAndAccept(
      ...
      {
        validate: (payload) =>
          validateInputLiveV32ProbePayload(payload, writer),
      },
    );
    ...
    const acceptedCaptures: InputLiveV32CapturePayload[] = [];
      ...
      acceptedCaptures.push(
        issueAndAccept(directory, privateKey, "capture", payload, {
          ...
          validate: (value, rawBytes) =>
            validateInputLiveV32CapturePayload(value, cell, rawBytes),
        }).accepted.payload,
      );
    }
    assertInputLiveV32CaptureResponses(captures, acceptedCaptures);
    ...
    acceptInputLiveV32Response(directory, "cleanup", {
      validate: (payload) =>
        validateInputLiveV32CleanupPayload(payload, writer),
    });
    assert.equal(listInputLiveV32PersistedRequests(directory).length, 133);
    assert.ok(inputLiveV32ReceiptPath(directory, "capture", 127));
  } finally {
...
test("offline objective evaluates every locked cell without sample reduction", () => {
  ...
  const manifest = captures.map((cell) => ({
    ...
    reference: {
      ...cell.reference,
      sha256: inputLiveV32Sha256(bytes),
      width: 4,
      height: 4,
  ...
  const report = evaluateInputLiveV32Objective(
    manifest,
    responses,
    () => bytes,
    () => bytes,
...
test("replay, wrong sequence, target, and dynamic tool fail closed", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV32Request(directory, {
          ...request,
          sequence: 9,
        }),
      ...
      () =>
        validateInputLiveV32Request(directory, {
          ...
          expectedDynamicTool: {
            namespace: "wrong",
            tool: "wrong",
          } as unknown as typeof INPUT_LIVE_V32_DYNAMIC_TOOL,
        }),
      /signature\/sequence\/target mismatch/,
    );
    persistInputLiveV32RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: { ...INPUT_LIVE_V32_TARGET, fileKey: "wrong" },
      }),
    );
    assert.throws(
      () => acceptInputLiveV32Response(directory, "writer"),
...
test("request tamper, response tamper, and signing-key substitution are rejected", () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        validateInputLiveV32Request(first.directory, {
          ...request,
          arguments: {
            ...request.arguments,
  ...
  try {
    ...
    const responseFile = inputLiveV32ResponsePath(third.directory, "writer");
    ...
    assert.throws(
      () => validateInputLiveV32Receipt(third.directory, "writer"),
...
test("one-root omission and expected-plan accounting mismatch are rejected", () => {
  const ownership = validateInputLiveV32WriterPayload(writerPayload());
...
test("persisted cleanup remains executable after phase crash and restart", () => {
  ...
  try {
    ...
    const persisted = JSON.parse(
      readFileSync(inputLiveV32RequestPath(directory, "cleanup"), "utf8"),
    ) as InputLiveV32Request;
    validateInputLiveV32Request(directory, persisted);
    const payload = {
      requestedNodeIds: [writer.pageId],
      removedNodeIds: [writer.pageId],
      requestedCollectionIds: writer.collectionIds,
    ...
    assert.doesNotThrow(() =>
      acceptInputLiveV32Response(directory, "cleanup", {
        validate: (value) => validateInputLiveV32CleanupPayload(value, writer),
      }),
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("durable orchestrator resumes with active restore and surfaced cleanup", () => {
  const directory = mkdtempSync(
    path.join(os.tmpdir(), "input-live-v32-state-"),
  );
  ...
  try {
    const orchestrator = InputLiveV32Orchestrator.initialize({
      ...
      privateKey,
      authorization: simulatedInputLiveV32Authorization(privateKey),
      attempt: 1,
    ...
    const resumed = InputLiveV32Orchestrator.resume({
      root: process.cwd(),
      transactionDirectory: directory,
      privateKey,
    }).nextAction();
...
test("signed writer timeout is 300000 and empty code is refused", () => {
  ...
  try {
    ...
    assert.equal(INPUT_LIVE_V32_SIGNED_WRITER_TIMEOUT_MS, 300_000);
  } finally {
    ...
    persistInputLiveV32RawResponse(
      ...
      rawResponse(request, writerPayload(), {
        fileContext: {
          ...
          fileName: INPUT_LIVE_V32_TARGET.fileName,
        },
      }),
    );
    const accepted = acceptInputLiveV32Response(directory, "writer");
    assert.equal(accepted.request.target.editorType, "figma");

    const second = fixture();
    try {
      const other = issueInputLiveV32Request(
        second.directory,
        second.privateKey,
        "writer",
        "return {};",
      ...
      persistInputLiveV32RawResponse(
        ...
        rawResponse(other, writerPayload(), {
          fileContext: { ...INPUT_LIVE_V32_TARGET, editorType: "figjam" },
        }),
      );
      assert.throws(
        () => acceptInputLiveV32Response(second.directory, "writer"),
        /binding\/target mismatch/,
      );
    } finally {
      rmSync(second.directory, { recursive: true, force: true });
...
const requestFrom = (file: string): InputLiveV32Request =>
  JSON.parse(readFileSync(file, "utf8")) as InputLiveV32Request;
