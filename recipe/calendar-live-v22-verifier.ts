...
import {
  verifyCalendarLiveV22FixedSceneFixedPoint,
  type CalendarLiveV22FixedFixedPoint,
} from "./calendar-live-v22-fixed-point.js";

export const CALENDAR_LIVE_V22_VERIFIER_VERSION = "calendar-live-v22-verifier-v1";

export function buildCalendarLiveV22RawPropertyRuntime(): string {
  return String.raw`
...
export interface CalendarLiveV22RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: CalendarLiveV22RawNode[];
}

export interface CalendarLiveV22NormalizedScene {
...
export interface CalendarLiveV22SceneProof {
...
export function normalizeCalendarLiveV22Scene(
  root: CalendarLiveV22RawNode,
  variableTable: readonly LocalVariableRecord[],
): CalendarLiveV22NormalizedScene {
  const canonicalBindings: CalendarLiveV22NormalizedScene["canonicalBindings"] = [];
  const visit = (raw: CalendarLiveV22RawNode): SceneNodeSnapshot => {
...
export function proveCalendarLiveV22Scene<Instance>(
  normalized: CalendarLiveV22NormalizedScene,
...
): CalendarLiveV22SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyCalendarLiveV22FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileCalendarLiveV22ExpectedScenePlan(
  ir: IRNode,
...
export function assertCalendarLiveV22PreCaptureGates(payload: unknown): void {
