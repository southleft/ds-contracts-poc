...
import { calendarLiveV1Sha256 } from "./calendar-live-v18-broker.js";

export interface CalendarLiveV18LifecycleSimulationReport {
  artifactVersion: "calendar-live-v18-lifecycle-simulation-v1";
...
const historyState = (
  ...
  authorization = undefined as
    ReturnType<typeof buildCalendarLiveV18AuthorizationArtifact> | undefined,
...
export async function simulateCalendarLiveV18Lifecycle(): Promise<CalendarLiveV18LifecycleSimulationReport> {
  await buildCalendarLiveV18Proof(true);
  const indexBefore = readFileSync(CALENDAR_LIVE_V18_INDEX_PATH);
  const index = JSON.parse(
    indexBefore.toString("utf8"),
  ) as CalendarLiveV18AntecedentIndex;
  const indexSha256 = calendarLiveV1Sha256(indexBefore);
  const temporary = mkdtempSync(path.join(os.tmpdir(), "calendar-live-v18-life-"));
  try {
    git(temporary, ["init", "-b", "main"]);
    const syntheticIndexPath = path.join(temporary, CALENDAR_LIVE_V18_INDEX_PATH);
    mkdirSync(path.dirname(syntheticIndexPath), { recursive: true });
    writeFileSync(syntheticIndexPath, indexBefore);
    git(temporary, ["add", CALENDAR_LIVE_V18_INDEX_PATH]);
    ...
    const pendingFailures = validateCalendarLiveV18History(pending, "pending");
    if (pendingFailures.length)
      throw new Error(
        `table v15 lifecycle pending phase failed:\n${pendingFailures.join("\n")}`,
      );
    const staleAuthorized = validateCalendarLiveV18History(pending, "authorized");
    if (
    ...
    const authorization = buildCalendarLiveV18AuthorizationArtifact({
      antecedentCommit,
    ...
    git(temporary, ["add", CALENDAR_LIVE_V18_AUTHORIZATION_PATH]);
    ...
    const committedAntecedentAfter = gitBytes(temporary, [
      "show",
      `${authorizationCommit}:${CALENDAR_LIVE_V18_INDEX_PATH}`,
    ]);
    ...
    const authorizedFailures = validateCalendarLiveV18History(
      authorized,
      "authorized",
    );
    ...
    const stalePending = validateCalendarLiveV18History(authorized, "pending");
    ...
    await buildCalendarLiveV18Proof(true);
    const indexAfter = readFileSync(CALENDAR_LIVE_V18_INDEX_PATH);
    ...
    const stableHashSet =
      index.hashSetSha256 ===
      (JSON.parse(indexAfter.toString("utf8")) as CalendarLiveV18AntecedentIndex)
        .hashSetSha256;
    if (!stableHashSet)
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`)
  process.stdout.write(
    `${JSON.stringify(await simulateCalendarLiveV18Lifecycle(), null, 2)}\n`,
  );
