...
import { COMBOBOX_LIVE_V17_INDEX_PATH } from "./build-combobox-live-proof-v17.js";
import {
  COMBOBOX_LIVE_V17_DYNAMIC_TOOL,
  COMBOBOX_LIVE_V17_TARGET,
} from "./combobox-live-v17-broker.js";
import {
  buildComboboxLiveV17AuthorizationArtifact,
  validateComboboxLiveV17AntecedentIndex,
  validateComboboxLiveV17AuthorizationArtifact,
  validateComboboxLiveV17History,
  validateComboboxLiveV17UnitTestSource,
  type ComboboxLiveV17AntecedentIndex,
  type ComboboxLiveV17AuthorizationArtifact,
  type ComboboxLiveV17AuthorizationProof,
  type ComboboxLiveV17HistoryState,
} from "./combobox-live-v17-authorization.js";
import {
  comboboxLiveV5AttestationSha256,
  validateComboboxLiveV17ControlFlowSource,
  validateComboboxLiveV17SecurityAttestation,
  type ComboboxLiveV17SecurityAttestation,
} from "./combobox-live-v17-preflight.js";
import { comboboxLiveV5Sha256 } from "./combobox-live-v17-broker.js";

const indexBytes = (): Buffer => readFileSync(COMBOBOX_LIVE_V17_INDEX_PATH);
const index = (): ComboboxLiveV17AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as ComboboxLiveV17AntecedentIndex;

const artifact = (): ComboboxLiveV17AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildComboboxLiveV17AuthorizationArtifact({
...
const pendingState = (): ComboboxLiveV17HistoryState => ({
  index: index(),
...
const authorizedState = (): ComboboxLiveV17HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: ComboboxLiveV17AuthorizationArtifact,
  ...
  authorizationPath:
    "recipe/evidence/combobox-live-pivot-v17/capture-authorization.json",
});

const attestation = (
  authorization: ComboboxLiveV17AuthorizationArtifact,
): ComboboxLiveV17SecurityAttestation => {
  const body: Omit<ComboboxLiveV17SecurityAttestation, "attestationSha256"> = {
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(COMBOBOX_LIVE_V17_TARGET),
      readOnly: true,
...
test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateComboboxLiveV17History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateComboboxLiveV17History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateComboboxLiveV17History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateComboboxLiveV17History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateComboboxLiveV17History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateComboboxLiveV17UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateComboboxLiveV17UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateComboboxLiveV17AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    assert.match(
      validateComboboxLiveV17AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateComboboxLiveV17AuthorizationArtifact(
        planted as ComboboxLiveV17AuthorizationArtifact,
        index(),
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateComboboxLiveV17SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = comboboxLiveV5AttestationSha256(
      body as Omit<ComboboxLiveV17SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateComboboxLiveV17SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateComboboxLiveV17ControlFlowSource(source), []);
});
