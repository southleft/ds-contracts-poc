...
import { buildTableLiveV28CleanupRuntime } from "./table-live-v28-cleanup.js";
import { type TableLiveV28FixedFixedPoint } from "./table-live-v28-fixed-point.js";
import { hashRecipeEnvelope } from "./hash.js";
import {
  buildTableLiveV28RestoreProgram,
  validateTableLiveV28RestorePayload,
  type TableLiveV28RestorePayload,
} from "./table-live-v28-restore.js";
import {
  buildTableLiveV28RawPropertyRuntime,
  normalizeTableLiveV28Scene,
  type TableLiveV28RawNode,
} from "./table-live-v28-verifier.js";
...
import { buildFigmaSceneReadbackRuntime } from "./scene-readback-runtime-table-v28.js";
import {
...
export const TABLE_LIVE_V28_NAMESPACE = TABLE_FIGMA_NAMESPACE;
export const TABLE_LIVE_V28_SOURCE_IDS = ["first-party", "mui"] as const;
export const TABLE_LIVE_V28_ADAPTERS = [
  "first-party-table-reviewed-v1",
  "material-table-reviewed-v1",
] as const;
export const TABLE_LIVE_V28_CAPTURE_COUNT = 20;
export const TABLE_LIVE_V28_VARIANT_COUNT = 20;
export const TABLE_LIVE_V28_SET_COUNT = 6;
export const TABLE_LIVE_V28_REMOTE_REQUESTS = 25;
export const TABLE_LIVE_V28_HOST_PHASES = 3;
export const TABLE_LIVE_V28_SOURCE_ROOTS = 2;
export const TABLE_LIVE_V28_RESTORE_COUNT = 8;
export const TABLE_LIVE_V28_CAPTURE_MAX_PNG_BYTES = 1_500_000;
export const TABLE_LIVE_V28_CAPTURE_MAX_RAW_RESPONSE_BYTES = 2_100_000;

export type TableLiveV28SourceId = (typeof TABLE_LIVE_V28_SOURCE_IDS)[number];
export type TableLiveV28Kind = "table" | "row" | "cell";

export interface TableLiveV28SourceIdentity {
  source: TableLiveV28SourceId;
...
export interface TableLiveV28WriterOwnership {
  pageId: string;
  pageName: string;
  runIdentity: string;
  namespace: string;
...
export interface TableLiveV28ExtractPayload {
  pageId: string;
  roots: Array<{
    source: TableLiveV28SourceId;
    adapterIdentity: string;
    ...
    rowSetId: string;
    cellSetId: string;
    tableScene: TableLiveV28RawNode;
    rowScene: TableLiveV28RawNode;
    cellScene: TableLiveV28RawNode;
  }>;
  variableTable: LocalVariableRecord[];
}

export interface TableLiveV28RootProof {
  source: TableLiveV28SourceId;
  adapterIdentity: string;
  ...
  fixedPoint: TableLiveV28FixedFixedPoint;
}

export interface TableLiveV28ProbePayload {
  pageId: string;
  sources: Array<{
    source: TableLiveV28SourceId;
    adapterIdentity: string;
...
export interface TableLiveV28CaptureCell {
  index: number;
  cellKey: string;
  source: TableLiveV28SourceId;
  adapterIdentity: string;
...
export interface TableLiveV28CapturePayload {
  index: number;
  cellKey: string;
  source: TableLiveV28SourceId;
  frameWidth: number;
...
export interface TableLiveV28CleanupPayload {
...
const record = (value: unknown, label: string): Record<string, any> => {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError(`Table live v28 ${label} must be an object`);
...
export function validateTableLiveV28WriterPayload(
  value: unknown,
): TableLiveV28WriterOwnership {
  ...
  if (
    ...
    result.namespace !== TABLE_LIVE_V28_NAMESPACE ||
    ...
    throw new TypeError("Table live v28 writer schema/cardinality mismatch");
  ...
  if (
    ...
    throw new TypeError(
      "Table live v28 writer created-node denominator invalid",
    );
  ...
  if (new Set(setIds).size !== 6)
    throw new TypeError("Table live v28 writer set identity collision");
  return {
...
const identityPlan = (
  source: TableLiveV28SourceIdentity,
  kind: TableLiveV28Kind,
) => {
  const plan =
    kind === "table"
...
export function buildTableLiveV28ExtractProgram(
  writer: TableLiveV28WriterOwnership,
  sources: readonly TableLiveV28SourceIdentity[],
): string {
  if (sources.length !== 2)
    throw new TypeError(
      "Table live v28 extract requires two source identities",
    );
  if (
...
const NS=${JSON.stringify(TABLE_LIVE_V28_NAMESPACE)};
...
${buildFigmaSceneReadbackRuntime(TABLE_LIVE_V28_NAMESPACE)}
${buildTableLiveV28RawPropertyRuntime()}
...
export function validateTableLiveV28ExtractPayload(
  value: unknown,
  writer: TableLiveV28WriterOwnership,
): TableLiveV28ExtractPayload {
  const payload = record(value, "extract payload");
  const leaks = forbiddenExtractKeys(payload);
  if (leaks.length)
    throw new TypeError(
      `Table live v28 extract contains source IR facts: ${leaks.join(",")}`,
    );
  if (
    ...
    payload.roots.some(
      (root: any) =>
        !TABLE_LIVE_V28_SOURCE_IDS.includes(root.source) ||
    ...
    throw new TypeError("Table live v28 extract schema/two-root mismatch");
  return payload as TableLiveV28ExtractPayload;
}

export function proveTableLiveV28Roots<Instance>(
  extract: TableLiveV28ExtractPayload,
  sources: readonly (TableLiveV28SourceIdentity & {
...
): TableLiveV28RootProof[] {
  if (extract.roots.length !== 2 || sources.length !== 2)
    throw new TypeError("Table live v28 proof requires two independent roots");
  return sources.map((source) => {
    ...
    if (!root)
      throw new TypeError(`Table live v28 omitted ${source.source} root`);
    const tableNormalized = normalizeTableLiveV28Scene(
      root.tableScene,
      extract.variableTable,
    );
    const rowNormalized = normalizeTableLiveV28Scene(
      root.rowScene,
      extract.variableTable,
    );
    const cellNormalized = normalizeTableLiveV28Scene(
      root.cellScene,
      extract.variableTable,
    );
    ...
    const runFixedPoint = (): TableLiveV28FixedFixedPoint => {
      const cycle = () => {
        const observedEnvelope = structuredClone(source.envelope);
        if (observedEnvelope.ir.kind !== "frame")
          throw new TypeError(
            "Table live v28 compile root must be library frame",
          );
        ...
        if (compiled.ir.kind !== "frame")
          throw new TypeError("Table live v28 compile lost library frame");
        ...
        if (!compiledTable || !compiledRow || !compiledCell)
          throw new TypeError("Table live v28 compile lost owned sets");
...
export function assertTableLiveV28RootProofs(
  proofs: readonly TableLiveV28RootProof[],
): void {
  if (
    proofs.length !== 2 ||
    new Set(proofs.map((proof) => proof.source)).size !== 2
  )
    throw new TypeError("Table live v28 two-root proof denominator invalid");
  ...
  if (failures.length)
    throw new TypeError(
      `Table live v28 independent root accounting failed: ${failures.join(";")}`,
    );
}

export function buildTableLiveV28ProbeProgram(
  writer: TableLiveV28WriterOwnership,
  sources: readonly TableLiveV28SourceIdentity[],
): string {
  ...
  return String.raw`
await figma.loadAllPagesAsync();
const NS=${JSON.stringify(TABLE_LIVE_V28_NAMESPACE)},PAGE_ID=${JSON.stringify(writer.pageId)},SET_IDS=new Set(${JSON.stringify(writer.setIds)}),SOURCE_BY_ADAPTER=${JSON.stringify(sourceByAdapter)};
if(PAGE_ID==="115:295378")throw new Error("TABLE-MUST-NOT-WRITE-INPUT-PAGE");
if(PAGE_ID==="163:35981")throw new Error("TABLE-MUST-NOT-WRITE-COMBOBOX-PAGE");
const page=await figma.getNodeByIdAsync(PAGE_ID);
...
export function validateTableLiveV28ProbePayload(
  value: unknown,
  writer: TableLiveV28WriterOwnership,
): TableLiveV28ProbePayload {
  const payload = record(value, "probe payload") as TableLiveV28ProbePayload;
  ...
  if (
    ...
    payload.cells.length !== TABLE_LIVE_V28_VARIANT_COUNT ||
    ...
    throw new TypeError("Table live v28 probe/usability/restoration failed");
  return payload;
}

export function validateTableLiveV28CaptureManifest(
  cells: readonly TableLiveV28CaptureCell[],
): void {
  const order = cells.map((cell) => cell.index);
  if (
    cells.length !== TABLE_LIVE_V28_CAPTURE_COUNT ||
    new Set(cells.map((cell) => cell.cellKey)).size !==
      TABLE_LIVE_V28_CAPTURE_COUNT ||
    new Set(order).size !== TABLE_LIVE_V28_CAPTURE_COUNT ||
    order.some((index, expected) => index !== expected) ||
    cells.some(
      (cell) =>
        !TABLE_LIVE_V28_SOURCE_IDS.includes(cell.source) ||
    ...
    throw new TypeError("Table live v28 capture manifest is malformed");
}

export const tableLiveV2CaptureManifestSha256 = (
  cells: readonly TableLiveV28CaptureCell[],
): string => {
  validateTableLiveV28CaptureManifest(cells);
  return sha256(canonicalJson(cells));
};

export function buildTableLiveV28CaptureProgram(
  writer: TableLiveV28WriterOwnership,
  cell: TableLiveV28CaptureCell,
): string {
  if (
    !Number.isInteger(cell.index) ||
    cell.index < 0 ||
    cell.index >= TABLE_LIVE_V28_CAPTURE_COUNT
  )
    throw new TypeError("Table live v28 capture cell is malformed");
  ...
  if (!expectedSet)
    throw new TypeError(
      `Table live v28 capture adapter absent: ${cell.cellKey}`,
    );
  return String.raw`
...
try{
 ...
 if(bytes.byteLength>${TABLE_LIVE_V28_CAPTURE_MAX_PNG_BYTES})throw new Error("TABLE-V2-CAPTURE-SIZE:"+bytes.byteLength);
...
export function validateTableLiveV28CapturePayload(
  value: unknown,
  cell: TableLiveV28CaptureCell,
  rawResponseBytes: number,
): TableLiveV28CapturePayload {
  const payload = record(
    value,
    "capture payload",
  ) as TableLiveV28CapturePayload;
  if (
    rawResponseBytes > TABLE_LIVE_V28_CAPTURE_MAX_RAW_RESPONSE_BYTES ||
    ...
    payload.pngBytes <= 0 ||
    payload.pngBytes > TABLE_LIVE_V28_CAPTURE_MAX_PNG_BYTES ||
    ...
    throw new TypeError(
      `Table live v28 capture truncated/mismatched: ${cell.cellKey}`,
    );
  return payload;
}

export function assertTableLiveV28CaptureResponses(
  manifest: readonly TableLiveV28CaptureCell[],
  responses: readonly TableLiveV28CapturePayload[],
): void {
  validateTableLiveV28CaptureManifest(manifest);
  if (
    ...
    throw new TypeError(
      "Table live v28 capture responses contain truncation/duplicate/missing cells",
    );
}

export interface TableLiveV28ObjectiveReport {
  artifactVersion: "table-live-v28-objective-report-v1";
  denominator: 20;
...
export function evaluateTableLiveV28Objective(
  manifest: readonly TableLiveV28CaptureCell[],
  responses: readonly TableLiveV28CapturePayload[],
): TableLiveV28ObjectiveReport {
  assertTableLiveV28CaptureResponses(manifest, responses);
  return {
    artifactVersion: "table-live-v28-objective-report-v1",
    denominator: 20,
...
export function buildTableLiveV28CleanupProgram(
  writer: TableLiveV28WriterOwnership,
): string {
  const baseProgram = buildTableLiveV28CleanupRuntime({
    fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
    fileName: "Scratch Project",
    editorType: "figma",
    namespace: TABLE_LIVE_V28_NAMESPACE,
  ...
  if (program === baseProgram)
    throw new Error("Table live v28 cleanup runtime result hook absent");
  return String.raw`
...
export function validateTableLiveV28CleanupPayload(
  payload: unknown,
  writer: TableLiveV28WriterOwnership,
): TableLiveV28CleanupPayload {
  const value = record(
    payload,
    "cleanup payload",
  ) as TableLiveV28CleanupPayload;
  if (
    value.complete !== true ||
    ...
    throw new TypeError("Table live v28 cleanup is incomplete or overbroad");
  return value;
}

export const TABLE_LIVE_V28_RESPONSE_CONTRACTS = Object.freeze({
  writer: {
    schema: "TableLiveV28WriterPayload",
    cardinality: {
      pages: 1,
      sections: 2,
      sets: 6,
