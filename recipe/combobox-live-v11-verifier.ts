...
import {
  verifyComboboxLiveV11FixedSceneFixedPoint,
  type ComboboxLiveV11FixedFixedPoint,
} from "./combobox-live-v11-fixed-point.js";

export const COMBOBOX_LIVE_V11_VERIFIER_VERSION = "combobox-live-v11-verifier-v1";

export function buildComboboxLiveV11RawPropertyRuntime(): string {
  return String.raw`
...
export interface ComboboxLiveV11RawNode extends Omit<
  SceneNodeSnapshot,
...
> {
  ...
  children: ComboboxLiveV11RawNode[];
}

export interface ComboboxLiveV11NormalizedScene {
...
export interface ComboboxLiveV11SceneProof {
...
export function normalizeComboboxLiveV11Scene(
  root: ComboboxLiveV11RawNode,
  variableTable: readonly LocalVariableRecord[],
): ComboboxLiveV11NormalizedScene {
  const canonicalBindings: ComboboxLiveV11NormalizedScene["canonicalBindings"] =
    [];
  const visit = (raw: ComboboxLiveV11RawNode): SceneNodeSnapshot => {
...
export function proveComboboxLiveV11Scene<Instance>(
  normalized: ComboboxLiveV11NormalizedScene,
...
): ComboboxLiveV11SceneProof {
  return {
    accounting: compareSceneToExpectedPlan(expected, normalized.scene),
    fixedPoint: verifyComboboxLiveV11FixedSceneFixedPoint(
      normalized.scene,
      envelope,
      selection,
      collapse,
...
export function compileComboboxLiveV11ExpectedScenePlan(
  ir: IRNode,
): ExpectedScenePlan {
  return compileExpectedScenePlan(ir);
}

export function assertComboboxLiveV11PreCaptureGates(payload: unknown): void {
