...
import {
  assertTableLiveV29RootProofs,
  TABLE_LIVE_V29_CAPTURE_COUNT,
  TABLE_LIVE_V29_CAPTURE_MAX_RAW_RESPONSE_BYTES,
  TABLE_LIVE_V29_RESPONSE_CONTRACTS,
  type TableLiveV29RootProof,
} from "./table-live-v29-contract.js";
import { canonicalJson } from "./normalize.js";

export const TABLE_LIVE_V29_BROKER_VERSION = "table-live-v29-broker-v1";
export const TABLE_LIVE_V29_SIGNED_WRITER_TIMEOUT_MS = 300_000;
export const TABLE_LIVE_V29_SIGNED_NON_WRITER_TIMEOUT_MS = 120_000;
export const TABLE_LIVE_V29_DYNAMIC_TOOL = Object.freeze({
  namespace: "user-Figma Console",
  tool: "figma_execute",
});
export const TABLE_LIVE_V29_TARGET = Object.freeze({
  fileKey: "byMp6lt0Ij9b2QbkDGFwBh",
  fileName: "Scratch Project",
  editorType: "figma",
});
export const TABLE_LIVE_V29_REMOTE_PHASES = [
  "writer",
  "restore",
  "extract",
  "probe",
...
export const TABLE_LIVE_V29_TECHNICAL_GATES = [
  "sceneExtraction",
  "hostNormalization",
  "accounting",
  "fixedPoint",
...
export type TableLiveV29RemotePhase =
  (typeof TABLE_LIVE_V29_REMOTE_PHASES)[number];
export type TableLiveV29AuthorizationMode = "live" | "simulated";

export interface TableLiveV29CommitPins {
...
export interface TableLiveV29TransactionAuthorization extends TableLiveV29CommitPins {
  mode: TableLiveV29AuthorizationMode;
...
export interface TableLiveV29Manifest {
  artifactVersion: "table-live-v29-broker-manifest-v2";
  brokerVersion: typeof TABLE_LIVE_V29_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  createdAt: string;
  expectedDynamicTool: typeof TABLE_LIVE_V29_DYNAMIC_TOOL;
  target: typeof TABLE_LIVE_V29_TARGET;
  authorization: TableLiveV29TransactionAuthorization;
...
export interface TableLiveV29ResponseContract {
  schema: string;
  cardinality: Readonly<Record<string, number>>;
  maximumRawResponseBytes: number;
}

interface TableLiveV29RequestBody {
  artifactVersion: "table-live-v29-broker-request-v2";
  brokerVersion: typeof TABLE_LIVE_V29_BROKER_VERSION;
  transactionId: string;
  attempt: number;
  requestId: string;
  phase: TableLiveV29RemotePhase;
  sequence: number;
  ...
  commitPins: TableLiveV29CommitPins;
  authorizationSha256: string;
  proofPlanSha256: string;
  captureManifestSha256: string;
  programBytes: number;
  ...
  arguments: {
    fileKey: typeof TABLE_LIVE_V29_TARGET.fileKey;
    timeout: number;
    code: string;
  };
  argumentsSha256: string;
  expectedResponse: TableLiveV29ResponseContract;
  oneMcpCallOnly: true;
  capture: boolean;
}

export interface TableLiveV29Request extends TableLiveV29RequestBody {
...
export interface TableLiveV29Receipt {
  artifactVersion: "table-live-v29-broker-receipt-v2";
  brokerVersion: typeof TABLE_LIVE_V29_BROKER_VERSION;
  transactionId: string;
  requestId: string;
  phase: TableLiveV29RemotePhase;
...
export interface TableLiveV29AcceptedResponse<T = unknown> {
  request: TableLiveV29Request;
  receipt: TableLiveV29Receipt;
  payload: T;
  rawPayload: unknown;
}

export interface TableLiveV29IssueOptions {
  captureIndex?: number;
  timeout?: number;
  now?: string;
  expectedResponse?: TableLiveV29ResponseContract;
...
const equalTarget = (value: unknown): value is typeof TABLE_LIVE_V29_TARGET =>
  value !== null &&
  typeof value === "object" &&
  (value as BrokerTarget).fileKey === TABLE_LIVE_V29_TARGET.fileKey &&
  (value as BrokerTarget).fileName === TABLE_LIVE_V29_TARGET.fileName &&
  (value as BrokerTarget).editorType === TABLE_LIVE_V29_TARGET.editorType;

const acceptFileContext = (value: unknown): boolean => {
  if (value === null || typeof value !== "object") return false;
  const context = value as Partial<BrokerTarget>;
  if (
    context.fileKey !== TABLE_LIVE_V29_TARGET.fileKey ||
    context.fileName !== TABLE_LIVE_V29_TARGET.fileName
  )
    return false;
  if (context.editorType === undefined || context.editorType === null)
    return true;
  return context.editorType === TABLE_LIVE_V29_TARGET.editorType;
};
...
const requestId = (
  ...
  if (phase !== "capture") {
    if (captureIndex !== undefined)
      throw new TypeError(`Table live v29 ${phase} cannot have capture index`);
    return phase;
  }
  if (
    ...
    captureIndex < 0 ||
    captureIndex >= TABLE_LIVE_V29_CAPTURE_COUNT
  )
    throw new TypeError("Table live v29 capture index must be 0..19");
...
const validateAuthorization = (
  ...
  if (
    ...
    throw new TypeError("Table live v29 authorization pins are malformed");
};

const readManifest = (directory: string): TableLiveV29Manifest => {
  const manifest = json<TableLiveV29Manifest>(
  ...
  if (resultLeaks.length)
    throw new TypeError(
      `Table live v29 antecedent result leakage: ${resultLeaks.join(",")}`,
    );
  ...
  if (
    manifest.artifactVersion !== "table-live-v29-broker-manifest-v2" ||
    manifest.brokerVersion !== TABLE_LIVE_V29_BROKER_VERSION ||
    manifest.attempt < 1 ||
    manifest.attempt > 3 ||
    manifest.maximumAttempts !== 3 ||
    manifest.captureRequests !== TABLE_LIVE_V29_CAPTURE_COUNT ||
    ...
    canonicalJson(manifest.expectedDynamicTool) !==
      canonicalJson(TABLE_LIVE_V29_DYNAMIC_TOOL) ||
    ...
    throw new TypeError("invalid Table live v29 broker manifest");
  return manifest;
};

const unsignedBody = (
  request: TableLiveV29Request,
): TableLiveV29RequestBody => {
  const {
...
const readRequestIfPresent = (
  ...
  phase: TableLiveV29RemotePhase,
  captureIndex?: number,
): TableLiveV29Request | undefined => {
  const file = tableLiveV2RequestPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV29Request>(file) : undefined;
};
...
const readReceiptIfPresent = (
  ...
  phase: TableLiveV29RemotePhase,
  captureIndex?: number,
): TableLiveV29Receipt | undefined => {
  const file = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  return existsSync(file) ? json<TableLiveV29Receipt>(file) : undefined;
};
...
const predecessor = (
  ...
  request: TableLiveV29Request | undefined;
  receipt: TableLiveV29Receipt | undefined;
} => {
...
const responseContractFor = (
  phase: TableLiveV29RemotePhase,
): TableLiveV29ResponseContract => ({
  ...TABLE_LIVE_V29_RESPONSE_CONTRACTS[phase],
...
const requestBinding = (
  ...
  sha256(
    canonicalJson({
      brokerVersion: TABLE_LIVE_V29_BROKER_VERSION,
...
const __broker=${JSON.stringify({
  artifactVersion: "table-live-v29-plugin-response-v2",
  brokerVersion: TABLE_LIVE_V29_BROKER_VERSION,
  target: TABLE_LIVE_V29_TARGET,
  ...request,
})};
...
const payload=await (async()=>{/*TABLE_LIVE_V29_PROGRAM_START*/${program}/*TABLE_LIVE_V29_PROGRAM_END*/})();
return {broker:__broker,payload};
})();`;

const wrappedProgram = (code: string): string => {
  const startMarker = "/*TABLE_LIVE_V29_PROGRAM_START*/";
  const endMarker = "/*TABLE_LIVE_V29_PROGRAM_END*/";
  ...
  if (
    ...
    throw new TypeError("Table live v29 wrapped program markers invalid");
  return code.slice(start + startMarker.length, end);
};

export function createTableLiveV29Transaction(
  directory: string,
  ...
  if (signingPublicKeySha256 !== options.authorization.signingPublicKeySha256)
    throw new TypeError(
      "Table live v29 private signing key does not match authorized public-key identity",
    );
  const manifest: TableLiveV29Manifest = {
    artifactVersion: "table-live-v29-broker-manifest-v2",
    brokerVersion: TABLE_LIVE_V29_BROKER_VERSION,
    ...
    captureRequests: TABLE_LIVE_V29_CAPTURE_COUNT,
    maximumAttempts: 3,
    signing: {
      algorithm: "Ed25519",
...
export function validateTableLiveV29Request(
  directory: string,
  request: TableLiveV29Request,
): void {
  ...
  if (
    request.artifactVersion !== "table-live-v29-broker-request-v2" ||
    request.brokerVersion !== TABLE_LIVE_V29_BROKER_VERSION ||
    ...
    request.arguments.fileKey !== TABLE_LIVE_V29_TARGET.fileKey ||
    ...
    throw new TypeError(
      "Table live v29 request hash/signature/sequence/target mismatch",
    );
  assertNoIdentityLeak(request.arguments, "request");
}

export function issueTableLiveV29Request(
  directory: string,
  signingPrivateKey: KeyObject,
  phase: TableLiveV29RemotePhase,
  program: string,
  options: TableLiveV29IssueOptions = {},
): TableLiveV29Request {
  ...
  if (existsSync(targetPath)) {
    const existing = json<TableLiveV29Request>(targetPath);
    validateTableLiveV29Request(directory, existing);
    if (existing.programSha256 !== sha256(program))
      throw new Error(`Table live v29 ${id} request pins other program bytes`);
    return existing;
  }
  if (existsSync(tableLiveV2ReceiptPath(directory, phase, captureIndex)))
    throw new Error(`Table live v29 replay refused: ${id} already accepted`);
  if (program.length === 0)
    throw new TypeError("Table live v29 empty code envelope refused");
  if (phase === "capture") validateTableLiveV29TechnicalGates(directory);
  const manifest = readManifest(directory);
  const prior = predecessor(directory, phase, captureIndex);
  if (phase !== "writer" && (!prior.request || !prior.receipt))
    throw new Error(`Table live v29 out-of-order phase: ${id}`);
  ...
  const arguments_ = {
    fileKey: TABLE_LIVE_V29_TARGET.fileKey,
    timeout:
      options.timeout ??
      (phase === "writer"
        ? TABLE_LIVE_V29_SIGNED_WRITER_TIMEOUT_MS
        : TABLE_LIVE_V29_SIGNED_NON_WRITER_TIMEOUT_MS),
    code: wrapProgram(
      {
        transactionId: manifest.transactionId,
        requestId: id,
  ...
  const body: TableLiveV29RequestBody = {
    artifactVersion: "table-live-v29-broker-request-v2",
    brokerVersion: TABLE_LIVE_V29_BROKER_VERSION,
    transactionId: manifest.transactionId,
    attempt: manifest.attempt,
  ...
  const request: TableLiveV29Request = {
    ...body,
    requestSha256,
    signature: {
      algorithm: "Ed25519",
  ...
  validateTableLiveV29Request(directory, request);
  atomicWrite(targetPath, `${JSON.stringify(request, null, 2)}\n`);
  return request;
}

export function persistTableLiveV29RawResponse(
  directory: string,
  phase: TableLiveV29RemotePhase,
  rawResponse: string,
  captureIndex?: number,
  ...
  if (!existsSync(requestFile))
    throw new Error(
      `Table live v29 response has no request: ${requestId(phase, captureIndex)}`,
    );
  const request = json<TableLiveV29Request>(requestFile);
  if (
    Buffer.byteLength(rawResponse) >
    request.expectedResponse.maximumRawResponseBytes
  )
    throw new Error(
      `Table live v29 response exceeds bounded transport: ${request.requestId}`,
    );
  const target = tableLiveV2ResponsePath(directory, phase, captureIndex);
  if (existsSync(target))
    throw new Error(
      `Table live v29 response replay refused: ${request.requestId}`,
    );
...
const createReceipt = (
  ...
  const body: Omit<TableLiveV29Receipt, "receiptSha256"> = {
    artifactVersion: "table-live-v29-broker-receipt-v2",
    brokerVersion: TABLE_LIVE_V29_BROKER_VERSION,
...
export function acceptTableLiveV29Response<T = unknown>(
  directory: string,
  phase: TableLiveV29RemotePhase,
  options: {
    captureIndex?: number;
...
): TableLiveV29AcceptedResponse<T> {
  const captureIndex = phase === "capture" ? options.captureIndex : undefined;
  const receiptFile = tableLiveV2ReceiptPath(directory, phase, captureIndex);
  if (existsSync(receiptFile))
    throw new Error(
      `Table live v29 replay refused: ${requestId(phase, captureIndex)} already accepted`,
    );
  const request = json<TableLiveV29Request>(
  ...
  if (!existsSync(rawFile))
    throw new Error(`Table live v29 missing response: ${request.requestId}`);
  ...
  if (
    ...
    plugin?.artifactVersion !== "table-live-v29-plugin-response-v2" ||
    plugin?.brokerVersion !== TABLE_LIVE_V29_BROKER_VERSION ||
    ...
    throw new TypeError(
      `Table live v29 ${request.requestId} response hash/binding/target mismatch`,
    );
...
export function persistTableLiveV29TechnicalGates(
  directory: string,
  rootProofs: readonly TableLiveV29RootProof[],
  probePayloadSha256: string,
  now = new Date().toISOString(),
): string {
  assertTableLiveV29RootProofs(rootProofs);
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (!extract || !probe)
    throw new Error(
      "Table live v29 host gates require accepted extract and probe",
    );
  if (!SHA256.test(probePayloadSha256))
    throw new TypeError("Table live v29 probe payload hash malformed");
  const gates = Object.fromEntries(
    TABLE_LIVE_V29_TECHNICAL_GATES.map((gate) => [gate, true]),
  );
  const body = {
    artifactVersion: "table-live-v29-host-technical-gates-v2",
    brokerVersion: TABLE_LIVE_V29_BROKER_VERSION,
...
export function validateTableLiveV29TechnicalGates(directory: string): void {
  const artifact = json<Record<string, any>>(path.join(directory, GATES_FILE));
  const { artifactSha256, ...body } = artifact;
  const extract = readReceiptIfPresent(directory, "extract");
  const probe = readReceiptIfPresent(directory, "probe");
  if (
    artifact.artifactVersion !== "table-live-v29-host-technical-gates-v2" ||
    ...
    TABLE_LIVE_V29_TECHNICAL_GATES.some(
      (gate) => artifact.gates?.[gate] !== true,
    ) ||
    artifact.capture !== false ||
    artifactSha256 !== sha256(canonicalJson(body))
  )
    throw new TypeError("invalid Table live v29 technical gates artifact");
}

export function validateTableLiveV29Receipt(
  directory: string,
  phase: TableLiveV29RemotePhase,
  captureIndex?: number,
): void {
  ...
  validateTableLiveV29Request(directory, request);
  const receipt = json<TableLiveV29Receipt>(
  ...
  if (
    receipt.artifactVersion !== "table-live-v29-broker-receipt-v2" ||
    receipt.brokerVersion !== TABLE_LIVE_V29_BROKER_VERSION ||
    ...
    throw new TypeError(
      `invalid Table live v29 accepted receipt: ${request.requestId}`,
    );
}

export function readTableLiveV29AcceptedRawPayload(
  directory: string,
  phase: TableLiveV29RemotePhase,
  captureIndex?: number,
): unknown {
  validateTableLiveV29Receipt(directory, phase, captureIndex);
...
export function createTableLiveV29FakeDynamicResponse(
  request: TableLiveV29Request,
  payload: unknown,
  overrides: {
    fileContext?: {
  ...
  return {
    ...
    result: {
      broker: {
        ...
        target: overrides.pluginTarget ?? TABLE_LIVE_V29_TARGET,
      },
      payload,
    },
    fileContext: overrides.fileContext ?? TABLE_LIVE_V29_TARGET,
    ...overrides.extra,
  };
}

export function listTableLiveV29PersistedRequests(
  directory: string,
): TableLiveV29Request[] {
  ...
  return readdirSync(root)
    ...
    .flatMap((entry) => {
      ...
      return existsSync(file) ? [json<TableLiveV29Request>(file)] : [];
    });
}
