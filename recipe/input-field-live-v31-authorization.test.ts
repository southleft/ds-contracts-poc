...
import { INPUT_LIVE_V31_INDEX_PATH } from "./build-input-field-live-proof-v31.js";
import {
  INPUT_LIVE_V31_DYNAMIC_TOOL,
  INPUT_LIVE_V31_TARGET,
} from "./input-field-live-v31-broker.js";
import {
  buildInputLiveV31AuthorizationArtifact,
  validateInputLiveV31AntecedentIndex,
  validateInputLiveV31AuthorizationArtifact,
  validateInputLiveV31History,
  validateInputLiveV31UnitTestSource,
  type InputLiveV31AntecedentIndex,
  type InputLiveV31AuthorizationArtifact,
  type InputLiveV31AuthorizationProof,
  type InputLiveV31HistoryState,
} from "./input-field-live-v31-authorization.js";
import {
  inputLiveV31AttestationSha256,
  validateInputLiveV31ControlFlowSource,
  validateInputLiveV31SecurityAttestation,
  type InputLiveV31SecurityAttestation,
} from "./input-field-live-v31-preflight.js";
import { inputLiveV31Sha256 } from "./input-field-live-v31-broker.js";

const indexBytes = (): Buffer => readFileSync(INPUT_LIVE_V31_INDEX_PATH);
const index = (): InputLiveV31AntecedentIndex =>
  JSON.parse(indexBytes().toString("utf8")) as InputLiveV31AntecedentIndex;

const artifact = (): InputLiveV31AuthorizationArtifact => {
  const { publicKey } = generateKeyPairSync("ed25519");
  return buildInputLiveV31AuthorizationArtifact({
...
const pendingState = (): InputLiveV31HistoryState => ({
  index: index(),
  indexSha256: inputLiveV31Sha256(indexBytes()),
...
const authorizedState = (): InputLiveV31HistoryState => ({
  ...pendingState(),
...
const proof = (
  value: InputLiveV31AuthorizationArtifact,
  ...
  antecedentIndexSha256: inputLiveV31Sha256(indexBytes()),
  antecedentHashSetSha256: index().hashSetSha256,
  target: INPUT_LIVE_V31_TARGET,
  expectedDynamicTool: INPUT_LIVE_V31_DYNAMIC_TOOL,
  authorizationPath:
    "recipe/evidence/input-field-live-pivot-v31/capture-authorization.json",
});

const attestation = (
  authorization: InputLiveV31AuthorizationArtifact,
): InputLiveV31SecurityAttestation => {
  const body: Omit<InputLiveV31SecurityAttestation, "attestationSha256"> = {
    artifactVersion: "input-live-v31-operator-security-attestation-v1",
    status: "complete",
    ...
    scratchReadOnlyProbe: {
      ...
      target: structuredClone(INPUT_LIVE_V31_TARGET),
      readOnly: true,
  ...
  return { ...body, attestationSha256: inputLiveV31AttestationSha256(body) };
};

test("pending, changed, and committed authorization states are pure fixtures", () => {
  assert.deepEqual(validateInputLiveV31History(pendingState(), "pending"), []);
  assert.deepEqual(
    validateInputLiveV31History(authorizedState(), "authorized"),
    [],
  );
  ...
  assert.match(
    validateInputLiveV31History(changed, "authorized").join("\n"),
    /bytes changed/,
  );
});
...
test("stale expected history modes fail closed", () => {
  assert.match(
    validateInputLiveV31History(pendingState(), "authorized").join("\n"),
    /stale history mode.*pending/,
  );
  assert.match(
    validateInputLiveV31History(authorizedState(), "pending").join("\n"),
...
test("the exact v6 phase-sensitive self-test defect is planted and v8 is hermetic", () => {
  ...
  assert.match(
    validateInputLiveV31UnitTestSource(v6).join("\n"),
  ...
  assert.deepEqual(validateInputLiveV31UnitTestSource(own), []);
});

test("authorization lifecycle files cannot enter the antecedent hash set", () => {
  assert.deepEqual(validateInputLiveV31AntecedentIndex(index()), []);
  for (const plantedPath of [
    ...
    planted.hashSetSha256 = inputLiveV31Sha256(
      JSON.stringify(
    ...
    assert.match(
      validateInputLiveV31AntecedentIndex(planted).join("\n"),
...
test("wrong target, tool, signing key, and private material are rejected", () => {
  ...
  for (const mutate of [
    ...
    assert.ok(
      validateInputLiveV31AuthorizationArtifact(
        planted as InputLiveV31AuthorizationArtifact,
        index(),
        inputLiveV31Sha256(indexBytes()),
      ).length > 0,
    );
  }
});
...
test("security remains mandatory: rotation, restart, owner-only env, scan, and exact Scratch", () => {
  ...
  assert.deepEqual(
    validateInputLiveV31SecurityAttestation(valid, proof(authorization)),
    [],
  );
  for (const mutate of [
    (value: Record<string, any>) => {
    ...
    planted.attestationSha256 = inputLiveV31AttestationSha256(
      body as Omit<InputLiveV31SecurityAttestation, "attestationSha256">,
    );
    assert.ok(
      validateInputLiveV31SecurityAttestation(planted, proof(authorization))
        .length > 0,
    );
  }
});
...
test("authorized entrypoint gates initialization behind history and preflight", () => {
  ...
  assert.deepEqual(validateInputLiveV31ControlFlowSource(source), []);
});
