...
export type CalendarLiveV5HistoryExpectation = "pending" | "authorized";

export interface CalendarLiveV5AuthorizationArtifact {
  artifactVersion: "calendar-live-v5-capture-authorization-v1";
  ...
  antecedent: {
    commit: string;
    indexPath: typeof CALENDAR_LIVE_V5_INDEX_PATH;
    indexSha256: string;
...
export interface CalendarLiveV5AntecedentIndex {
...
export interface CalendarLiveV5HistoryState {
  index?: CalendarLiveV5AntecedentIndex;
  indexSha256: string;
  ...
  authorization?: CalendarLiveV5AuthorizationArtifact;
...
export interface CalendarLiveV5AuthorizationProof
  extends CalendarLiveV5TransactionAuthorization {
  upstreamCommit: string;
  antecedentIndexSha256: string;
  antecedentHashSetSha256: string;
  target: typeof CALENDAR_LIVE_V5_TARGET;
  expectedDynamicTool: typeof CALENDAR_LIVE_V5_DYNAMIC_TOOL;
  authorizationPath: typeof CALENDAR_LIVE_V5_AUTHORIZATION_PATH;
}
...
const lifecyclePath = (artifactPath: string): boolean =>
  artifactPath === CALENDAR_LIVE_V5_AUTHORIZATION_PATH ||
  artifactPath.endsWith("/authorization-template.json") ||
  artifactPath.endsWith("/operator-security-attestation-template.json") ||
  artifactPath.includes("calendar-live-v5-authorization") ||
  artifactPath.includes("calendar-live-v5-preflight") ||
  artifactPath.includes("calendar-live-v5-authorized") ||
  artifactPath.includes("create-calendar-live-v5-security-attestation") ||
  artifactPath.includes("calendar-live-pivot-v5-status") ||
...
export function validateCalendarLiveV5AntecedentIndex(
  index: CalendarLiveV5AntecedentIndex | undefined,
): string[] {
  const failures: string[] = [];
  if (
    index?.artifactVersion !== "calendar-live-v5-antecedent-index-v1" ||
...
export function validateCalendarLiveV5UnitTestSource(source: string): string[] {
  const failures: string[] = [];
  if (
    /assert\.throws\([\s\S]{0,300}verifyInputLiveV\d+Authorization\(\)/.test(
      source,
    ) ||
    /verifyCalendarLiveV5History\(\s*["']/.test(source) ||
    /readCalendarLiveV5HistoryState\(/.test(source)
  )
    failures.push(
...
export function buildCalendarLiveV5AuthorizationArtifact(options: {
  antecedentCommit: string;
  antecedentIndexBytes: Uint8Array;
  signingPublicKey: KeyObject | string | Buffer;
}): CalendarLiveV5AuthorizationArtifact {
  ...
  ) as CalendarLiveV5AntecedentIndex;
  const indexFailures = validateCalendarLiveV5AntecedentIndex(index);
...
export function validateCalendarLiveV5AuthorizationArtifact(
  artifact: CalendarLiveV5AuthorizationArtifact | undefined,
  index: CalendarLiveV5AntecedentIndex | undefined,
  indexSha256: string,
): string[] {
  const failures: string[] = [];
  if (
    artifact?.artifactVersion !== "calendar-live-v5-capture-authorization-v1" ||
    artifact.authorizationId !== "calendar-live-v5" ||
  ...
  if (
    artifact?.antecedent?.indexPath !== CALENDAR_LIVE_V5_INDEX_PATH ||
  ...
  if (
    !equalJson(artifact?.operatorBoundary?.target, CALENDAR_LIVE_V5_TARGET) ||
    !equalJson(
  ...
  if (
    artifact?.denominator?.remoteRequests !== CALENDAR_LIVE_V5_REMOTE_REQUESTS ||
    artifact?.denominator?.hostPhases !== CALENDAR_LIVE_V5_HOST_PHASES ||
    artifact?.denominator?.captures !== CALENDAR_LIVE_V5_CAPTURE_COUNT ||
    artifact?.denominator?.sourceRoots !== CALENDAR_LIVE_V5_SOURCE_ROOTS ||
  ...
  if (
    ...
    artifact?.execution?.calendarLiveV4AuthorizationReusable !== false ||
...
export function validateCalendarLiveV5History(
  state: CalendarLiveV5HistoryState,
  expected: CalendarLiveV5HistoryExpectation,
): string[] {
  const failures = validateCalendarLiveV5AntecedentIndex(state.index);
  ...
  failures.push(
    ...validateCalendarLiveV5AuthorizationArtifact(
      state.authorization,
      state.index,
      state.indexSha256,
    ),
...
export function readCalendarLiveV5HistoryState(
  root = process.cwd(),
): CalendarLiveV5HistoryState {
  ...
  const indexFile = path.join(repositoryRoot, CALENDAR_LIVE_V5_INDEX_PATH);
  ...
  const authorization = existsSync(authorizationFile)
    ? (JSON.parse(
      ...
      ) as CalendarLiveV5AuthorizationArtifact)
    : undefined;
  const firstIndexBytes = antecedentCommit
    ? git(repositoryRoot, [
        "show",
        `${antecedentCommit}:${CALENDAR_LIVE_V5_INDEX_PATH}`,
      ])
    : Buffer.alloc(0);
  const firstAuthorizationBytes = authorizationCommit
    ? git(repositoryRoot, [
        "show",
        `${authorizationCommit}:${CALENDAR_LIVE_V5_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  const currentAuthorizationBytes = authorizationAtCode
    ? git(repositoryRoot, [
        "show",
        `${codeCommit}:${CALENDAR_LIVE_V5_AUTHORIZATION_PATH}`,
      ])
    : Buffer.alloc(0);
  return {
    index,
...
export function verifyCalendarLiveV5History(
  expected: CalendarLiveV5HistoryExpectation,
  root = process.cwd(),
): CalendarLiveV5HistoryState {
  const state = readCalendarLiveV5HistoryState(root);
  const failures = validateCalendarLiveV5History(state, expected);
  if (failures.length)
    throw new Error(
      `Calendar live v5 ${expected} history refused:\n${failures.join("\n")}`,
    );
  return state;
}

export function verifyCalendarLiveV5Authorization(
  root = process.cwd(),
): CalendarLiveV5AuthorizationProof {
  const state = verifyCalendarLiveV5History("authorized", root);
  ...
  return {
    ...
    authorizationSha256: sha256(
      readFileSync(
        path.join(repositoryRoot, CALENDAR_LIVE_V5_AUTHORIZATION_PATH),
      ),
    ),
    protocolSha256:
      state.index!.artifacts[
        "recipe/evidence/calendar-live-pivot-v5/protocol.json"
      ]!.sha256,
    runnerSha256:
      state.index!.artifacts["recipe/run-calendar-live-v5.ts"]!.sha256,
    ...
    target: CALENDAR_LIVE_V5_TARGET,
    expectedDynamicTool: CALENDAR_LIVE_V5_DYNAMIC_TOOL,
    authorizationPath: CALENDAR_LIVE_V5_AUTHORIZATION_PATH,
  };
}

const expectedMode = (): CalendarLiveV5HistoryExpectation => {
...
if (import.meta.url === `file://${path.resolve(process.argv[1] ?? "")}`) {
  ...
  const state = verifyCalendarLiveV5History(expected);
  process.stdout.write(
    `Calendar live v5 history ${expected}: antecedent=${state.antecedentCommit} authorization=${state.authorizationCommit ?? "pending"} code=${state.codeCommit}\n`,
  );
}
